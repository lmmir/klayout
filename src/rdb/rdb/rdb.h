 ...
 *  A category is described by a name and a description string. An Id is
 * provided to reference this category from actual report items. Categories can
 * be organized hierarchically for which a category collection is provided and
 * member of the individual category.
 *
 *  A category can only be created by the database object, since the
 *  database object issues id's.
 */
class RDB_PUBLIC Category : public tl::Object {
public:
  /**
   *  @brief The default constructor with a category reference
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. Use Database::create_category instead.
   */
   ...
   *
   *  This method must not be used for items in the database to keep the
   * database consistent.
   */
   ...
   * database consistent.
   */
  void set_name(const std::string &d) { m_name = d; }

  /**
   *  @brief Get the path of this category
   *
   *  The path contains all components leading to this category starting from a
   * top level category. The path string can be used with category_by_name to
   * localize the current category.
   */
   ...
   */
  void set_description(const std::string &d) { m_description = d; }
   ...
   */
  const Category *parent() const { return mp_parent; }

  /**
   *  @brief The parent (owner) of this category (getter)
   */
  Category *parent() { return mp_parent; }

  /**
   *  @brief Import categories as sub categories.
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. The Category object will take over ownership over the sub
   * categories.
   */
   ...
   */
  size_t num_items() const { return m_num_items; }

  /**
   *  @brief Report the number of items visited
   */
  size_t num_items_visited() const { return m_num_items_visited; }

  /**
   *  @brief Get the database reference
   */
  Database *database() { return mp_database; }

  /**
   *  @brief Get the database reference (const version)
   */
  const Database *database() const { return mp_database; }
...
private:
   ...
   */
  void set_parent(Category *parent_category) { mp_parent = parent_category; }

  /**
   *  @brief Add an offset to the number of items visited
   */
  void add_to_num_items_visited(int d) { m_num_items_visited += d; }

  /**
   *  @brief Add an offset to the number of items
   */
  void add_to_num_items(int d) { m_num_items += d; }
...
public:
   ...
   * used otherwise.
   */
  Categories(Database *database) : mp_database(database) {
    // .. nothing yet ..
  }
   ...
   * used otherwise.
   */
  Categories(Category *category) : mp_database(category->database()) {
    // .. nothing yet ..
  }
   ...
   *
   *  The name is actually a path expression which specifies the category
   * starting from the given node with a '.' separated path notation. I.e. 'a.b'
   * is the sub-category 'b' of category 'a'. If no such category can be found,
   * 0 is returned.
   */
   ...
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. This will take over ownership over the category.
   */
   ...
   */
  Database *database() { return mp_database.get(); }
 ...
 *  A value has a value (as the name says) and an optional tag id. Tag id's
 * identify the value and make the value a named one.
 */
...
public:
  ...
  static ValueBase *create_from_shape(const db::Shape &shape,
 ...
 */
template <class T> int type_index_of();
 ...
 */
template <class C> class RDB_PUBLIC_TEMPLATE Value : public ValueBase {
public:
  Value() : m_value() {
    //  .. nothing yet ..
  }

  Value(const C &value) : m_value(value) {
    //  .. nothing yet ..
  }

  void set_value(const C &value) { m_value = value; }

  const C &value() const { return m_value; }

  C &value() { return m_value; }

  int type_index() const { return type_index_of<C>(); }
  ...
  ValueBase *clone() const { return new Value<C>(m_value); }
 ...
 *  @brief Type bindings
 */
template <class T> RDB_PUBLIC_TEMPLATE ValueBase *make_value(const T &value) {
...
public:
   ...
   *  @brief Default constructor
   */
  ValueWrapper() : mp_ptr(0), m_tag_id(0) {
    //  .. nothing yet ..
  }
   ...
   */
  ValueWrapper(ValueBase *ptr) : mp_ptr(ptr), m_tag_id(0) {
    //  .. nothing yet ..
  }
   ...
   */
  ValueBase *get() { return mp_ptr; }

  /**
   *  @brief Get the pointer (const)
   */
  const ValueBase *get() const { return mp_ptr; }
  ...
  void set_tag_id(id_type id) { m_tag_id = id; }

  id_type tag_id() const { return m_tag_id; }
...
public:
   ...
   *  @brief Copy constructor
   */
  Values(const Values &d) { operator=(d); }
   ...
   */
  void add(const ValueWrapper &value) { m_values.push_back(value); }

  /**
   *  @brief Swaps the values with other values
   */
  void swap(Values &other) { m_values.swap(other.m_values); }
 ...
 *  Each value can be of different types where the type is specified by a type
 * Id.
 */
class RDB_PUBLIC Item : public tl::Object {
public:
  /**
   ...
   */
  id_type cell_id() const { return m_cell_id; }
   ...
   * database consistent.
   */
  void set_cell_id(id_type id) { m_cell_id = id; }

  /**
   *  @brief Get the cell that this item is located inside (by name or
   ...
   */
  id_type category_id() const { return m_category_id; }
   ...
   * database consistent.
   */
  void set_category_id(id_type id) { m_category_id = id; }
   ...
   */
  template <class V> ValueBase *add_value(const V &v, id_type tag_id = 0) {
   ...
   */
  void set_values(const Values &values) { m_values = values; }

  /**
   *  @brief Set the multiplicity of the item
   */
  void set_multiplicity(size_t n) { m_multiplicity = n; }

  /**
   *  @brief Get the multiplicity of this item
   */
  size_t multiplicity() const { return m_multiplicity; }
   ...
   */
  void set_visited(bool v) { m_visited = v; }
   ...
   *
   *  Note: if neither PNG support for Qt are compiled in, this string will be
   * empty
   */
...
private:
   ...
   *  @brief Set the database reference
   */
  void set_database(Database *database) { mp_database = database; }
};
...
public:
   ...
   *  @brief Constructor
   */
  ItemRef(Item *item) : mp_item(item) {
    //  .. nothing yet ..
  }
   ...
   *  @brief const access
   */
  const Item *operator->() const { return mp_item; }
...
public:
   ...
   */
  Items(Database *database) : mp_database(database) {
    // .. nothing yet ..
  }
...
public:
  /**
   *  @brief Constructs a reference with a unit transformation and a empty
   * parent cell id
   */
  Reference() : m_trans(), m_parent_cell_id(0), mp_database(0) {
    //  .. nothing yet ..
  }
  ...
  /**
   *  @brief Constructs a reference with the given transformation and the given
   * parent cell id
   *
   *  @param trans The transformation which transforms anything of this cell
   ...
   */
  void set_parent_cell_id(id_type id) { m_parent_cell_id = id; }

  /**
   *  @brief Get the parent cell id
   */
  id_type parent_cell_id() const { return m_parent_cell_id; }

  /**
   *  @brief Set the parent cell name (plus optionally the variant id separated
   * by a colon - i.e. cell:var1)
   */
  void set_parent_cell_qname(const std::string &qname);

  /**
   *  @brief Get the transformation as string (plus optionally the variant id
   * separated by a colon - i.e. cell:var1)
   */
   ...
   */
  void set_trans(const db::DCplxTrans &trans) { m_trans = trans; }

  /**
   *  @brief Get the transformation
   */
  const db::DCplxTrans &trans() const { return m_trans; }
 ...
 */
class RDB_PUBLIC Cell : public tl::Object {
public:
  ...
  /**
   *  @brief The constructor taking the database reference from the Cells
   * collection
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise.
   ...
   * database consistent.
   */
  void set_variant(const std::string &v) { m_variant = v; }

  /**
   *  @brief Get the qualified name (name plus optionally the variant id
   ...
   */
  References &references() { return m_references; }
  ...
  /**
   *  @brief Get one example transformation leading from this cell to a given
   * parent cell
   *
   *  This method will try to determine one path from the given cell to the
   * given parent cell and return the accumulated transformation for this path.
   *  If no path is found, the first parameter of the returned pair is false,
   * otherwise it's true.
   */
  std::pair<bool, db::DCplxTrans> path_to(id_type parent_cell_id,
                                          const rdb::Database *db) const;
...
public:
   ...
   * used otherwise.
   */
  Cells(Database *database) : mp_database(database) {
    //  .. nothing yet ..
  }
   ...
   *  @brief Get the database reference
   */
  const Database *database() const { return mp_database.get(); }
...
public:
   ...
   *  @brief Default constructor
   */
  Tag() : m_id(0), m_is_user_tag(false) {
    //  .. nothing yet ..
  }
  ...
  /**
   *  @brief Gets a flag indicating whether the tag is a user tag or a system
   * tag
   *
   *  If this flag is false, the tag is a system tag used for tagging "waived"
   * and similar conditions. Otherwise it is a user tag which can be used freely
   * to tag arbitrary conditions.
   */
  bool is_user_tag() const { return m_is_user_tag; }

  /**
   *  @brief Sets a flag indicating whether the tag is a user tag or a system
   * tag
   *
   *  See \is_user_tag for details.
   */
  void set_user_tag(bool user) { m_is_user_tag = user; }
   ...
   *  @brief Sets the name
   *
   *  The name of the tag must not be changed when the tag is already part of a
   * Tags collection. Otherwise, the tag collection becomes inconsistent.
   */
...
public:
   ...
   */
  const_iterator begin_tags() const { return m_tags.begin(); }

  /**
   *  @brief Get the tag iterator (end)
   */
  const_iterator end_tags() const { return m_tags.end(); }

  /**
   *  @brief Import a tag
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. This will assign a new id to the tag and replace any tag
   * with that name.
   */
 ...
 */
class RDB_PUBLIC Database : public gsi::ObjectBase, public tl::Object {
public:
   ...
   */
  const std::string &top_cell_name() const { return m_topcell; }
   ...
   */
  const Categories &categories() const { return *mp_categories; }

  /**
   *  @brief Import categories
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. This will take over the ownership over the Categories
   * object.
   */
   ...
   */
  const Cells &cells() const { return m_cells; }
   ...
   *
   *  @return a vector of id's corresponding to the given variants or an empty
   * vector if the name is not valid or the cell has no variants
   */
  const std::vector<id_type> &variants(const std::string &name);

  /**
   *  @brief Get the cell pointer for a cell name or name:variant combination
   * (const version)
   *
   *  This method returns 0 if the cell name or name:variant combination is
   * invalid.
   */
  ...
  /**
   *  @brief Create a new item for the given cell and category (both given by
   * id)
   */
   ...
   */
  const Items &items() const { return *mp_items; }

  /**
   *  @brief Set the items collection
   *
   *  This method is provided for persistency application only. It should not be
   * used otherwise. This will take ownership over the items collection.
   */
  void set_items(Items *items);

  /**
   *  @brief Get an iterator pair that delivers the const items (ItemRef) for a
   * given cell
   */
  std::pair<const_item_ref_iterator, const_item_ref_iterator>
  items_by_cell(id_type cell_id) const;

  /**
   *  @brief Get an iterator that delivers the const items (ItemRef) for a given
   * category
   */
  std::pair<const_item_ref_iterator, const_item_ref_iterator>
  items_by_category(id_type category_id) const;
   ...
   */
  std::pair<const_item_ref_iterator, const_item_ref_iterator>
  items_by_cell_and_category(id_type cell_id, id_type category_id) const;
...
private:
  ...
  std::map<std::pair<id_type, id_type>, std::list<ItemRef>>
      m_items_by_cell_and_category_id;
  std::map<std::pair<id_type, id_type>, size_t>
      m_num_items_by_cell_and_category;
  std::map<std::pair<id_type, id_type>, size_t>
      m_num_items_visited_by_cell_and_category;
   ...
   */
  Cells &cells_non_const() { return m_cells; }
