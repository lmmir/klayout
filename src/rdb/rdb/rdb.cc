...
template <>
RDB_PUBLIC std::string Value<std::string>::to_display_string() const {
  return m_value;
}

template <>
RDB_PUBLIC std::string Value<db::DPolygon>::to_display_string() const {
  return to_string();
}
...
template <>
RDB_PUBLIC std::string Value<db::DEdgePair>::to_display_string() const {
  return to_string();
}
...
template <> RDB_PUBLIC bool Value<double>::is_shape() const { return false; }
...
template <> RDB_PUBLIC bool Value<db::DEdge>::is_shape() const { return true; }

template <> RDB_PUBLIC bool Value<db::DEdgePair>::is_shape() const {
  return true;
}

template <> RDB_PUBLIC bool Value<db::DBox>::is_shape() const { return true; }

template <> RDB_PUBLIC bool Value<db::DPath>::is_shape() const { return true; }

template <> RDB_PUBLIC bool Value<db::DText>::is_shape() const { return true; }

bool ValueBase::compare(const ValueBase *a, const ValueBase *b) {
  //  compare is the intrinsic compare of equal type and type index for
  //  different types.
...
ValueBase *ValueBase::create_from_string(const std::string &s) {
  tl::Extractor ex(s.c_str());
  return create_from_string(ex);
}

ValueBase *ValueBase::create_from_string(tl::Extractor &ex) {
  if (ex.test("polygon")) {
    ...
    throw tl::Exception(tl::to_string(tr("Invalid value string at '...%s'")),
                        ex.skip());
  }
}

ValueBase *ValueBase::create_from_shape(const db::Shape &shape,
...
std::string ValueWrapper::to_string(const Database *rdb) const {
  std::string r;
  r.reserve(200);
...
Values &Values::operator=(const Values &d) {
  m_values = d.m_values;
  return *this;
}

std::string Values::to_string(const Database *rdb) const {
  std::string r;
  r.reserve(200);
...
void Cells::import_cell(const Cell &c) {
  ...
  for (References::const_iterator r = c.references().begin();
       r != c.references().end(); ++r) {
...
Cell::Cell() : m_id(0), m_num_items(0), m_num_items_visited(0), mp_database(0) {
  //  .. nothing yet ..
}

Cell::Cell(Cells *cells)
    : m_id(0), m_num_items(0), m_num_items_visited(0),
      mp_database(cells->database()) {
  m_references.set_database(mp_database);
}

Cell::Cell(id_type id, const std::string &name)
    : m_id(id), m_name(name), m_num_items(0), m_num_items_visited(0),
      mp_database(0) {
  //  .. nothing yet ..
}

Cell::Cell(id_type id, const std::string &name, const std::string &variant)
    : m_id(id), m_name(name), m_variant(variant), m_num_items(0),
      m_num_items_visited(0), mp_database(0) {
  //  .. nothing yet ..
}
...
std::pair<bool, db::DCplxTrans> Cell::path_to(id_type parent_cell_id,
...
Cell::path_to(id_type parent_cell_id, const Database *db,
              std::set<id_type> &visited, const db::DCplxTrans &trans) const {
  for (reference_iterator r = references().begin(); r != references().end();
       ++r) {
    ...
    if (r->parent_cell_id() == parent_cell_id) {
      ...
      if (rc) {
        std::pair<bool, db::DCplxTrans> path =
            rc->path_to(parent_cell_id, db, visited, r->trans() * trans);
        if (path.first) {
          return path;
        }
      }
...
void Cell::import_references(const References &references) {
  for (References::const_iterator r = references.begin(); r != references.end();
       ++r) {
    m_references.insert(*r);
  }
}

std::string Cell::qname() const {
  if (m_variant.empty()) {
    return m_name;
  } else {
...
References::References() : mp_database(0) {
  //  .. nothing yet ..
}

References::References(Cell *cell) : mp_database(cell->database()) {
  //  .. nothing yet ..
}
...
std::string Reference::trans_str() const { return m_trans.to_string(); }
...
void Reference::set_parent_cell_qname(const std::string &qname) {
  ...
  if (!cell) {
    throw tl::Exception(
        tl::to_string(
            tr("%s is not a valid cell name or name/variant combination")),
        qname);
  }
  m_parent_cell_id = cell->id();
}

std::string Reference::parent_cell_qname() const {
  tl_assert(mp_database != 0);
...
Category::Category(Categories *categories)
    : m_id(0), mp_parent(0), mp_sub_categories(0), m_num_items(0),
      m_num_items_visited(0), mp_database(categories->database()) {
  // .. nothing yet ..
}
...
Category::Category(const std::string &name)
    : m_id(0), m_name(name), mp_parent(0), mp_sub_categories(0), m_num_items(0),
      m_num_items_visited(0), mp_database(0) {
  // .. nothing yet ..
}
...
const Categories &Category::sub_categories() const {
  if (!mp_sub_categories) {
...
std::string Category::path() const {
  ...
  std::string r;
  for (std::vector<std::string>::const_iterator p = path_elements.begin();
       p != path_elements.end(); ++p) {
...
void Category::import_sub_categories(Categories *categories) {
  ...
  if (mp_sub_categories != 0) {
    for (Categories::iterator c = mp_sub_categories->begin();
         c != mp_sub_categories->end(); ++c) {
      c->set_parent(this);
    }
  }
}
...
Category *Categories::category_by_name(const char *path) {
  std::string component;
  tl::Extractor ex(path);
  ex.read_word_or_quoted(component, "_$");

  std::map<std::string, Category *>::iterator ref =
      m_categories_by_name.find(component);
...
const Tag &Tags::tag(const std::string &name, bool user_tag) const {
  return (const_cast<Tags *>(this)->tag(name, user_tag));
}

Tag &Tags::tag(const std::string &name, bool user_tag) {
  std::map<std::pair<std::string, bool>, id_type>::const_iterator i =
      m_ids_for_names.find(std::make_pair(name, user_tag));
  if (i == m_ids_for_names.end()) {
    i = m_ids_for_names
            .insert(std::make_pair(std::make_pair(name, user_tag),
                                   m_tags.size() + 1))
            .first;
...
const Tag &Tags::tag(id_type id) const {
...
bool Tags::has_tag(const std::string &name, bool user_tag) const {
  return m_ids_for_names.find(std::make_pair(name, user_tag)) !=
         m_ids_for_names.end();
}
...
Item::Item(Items *items)
    : m_cell_id(0), m_category_id(0), m_multiplicity(1), m_visited(false),
      mp_database(items->database()) {
  // .. nothing yet ..
}
...
Item::Item(const Item &d)
    : tl::Object(d), m_cell_id(0), m_category_id(0), m_multiplicity(1),
      m_visited(false), mp_database(d.mp_database) {
  operator=(d);
}
...
std::string Item::category_name() const {
  tl_assert(mp_database != 0);
...
void Item::set_category_name(const std::string &path) {
  ...
  const Category *category =
      mp_database->categories().category_by_name(path.c_str());
  if (!category) {
    throw tl::Exception(tl::to_string(tr("%s is not a valid category path")),
                        path);
  }
  m_category_id = category->id();
}

std::string Item::tag_str() const {
  tl_assert(mp_database != 0);
  ...
  if (!m_tag_ids.empty()) {
    ...
    id_type tag_id = 0;
    for (std::vector<bool>::const_iterator t = m_tag_ids.begin();
         t != m_tag_ids.end(); ++t, ++tag_id) {
      if (*t) {
        if (!r.empty()) {
          r += ",";
        }
...
QImage Item::image() const {
  if (m_image_str.empty()) {
    ...
    QByteArray img_data(QByteArray::fromBase64(
        QByteArray::fromRawData(m_image_str.c_str(), int(m_image_str.size()))));
...
tl::PixelBuffer Item::image_pixels() const {
  std::vector<unsigned char> data = tl::from_base64(m_image_str.c_str());
  tl::InputStream stream(new tl::InputMemoryStream(
      (const char *)data.begin().operator->(), data.size()));
...
bool Item::has_image() const { return !m_image_str.empty(); }

std::string Item::image_str() const { return m_image_str; }

void Item::set_image_str(const std::string &s) { m_image_str = s; }
...
void Database::set_items(Items *items) {
  ...
  for (std::map<id_type, Category *>::iterator c = m_categories_by_id.begin();
       c != m_categories_by_id.end(); ++c) {
    c->second->reset_num_items();
  }

  for (std::map<id_type, Cell *>::iterator c = m_cells_by_id.begin();
       c != m_cells_by_id.end(); ++c) {
    c->second->reset_num_items();
  }
  ...
  for (Items::iterator i = mp_items->begin(); i != mp_items->end(); ++i) {
    ...
    if (cell != 0 && category != 0) {
      ...
      m_items_by_cell_id.insert(std::make_pair(cell_id, std::list<ItemRef>()))
          .first->second.push_back(ItemRef(&*i));

      if (i->visited()) {
        cell->add_to_num_items_visited(1);
      }

      m_items_by_category_id
          .insert(std::make_pair(category_id, std::list<ItemRef>()))
          .first->second.push_back(ItemRef(&*i));
      m_items_by_cell_and_category_id
          .insert(std::make_pair(std::make_pair(cell_id, category_id),
                                 std::list<ItemRef>()))
          .first->second.push_back(ItemRef(&*i));
      ...
      while (category) {
        ...
        m_num_items_by_cell_and_category
            .insert(std::make_pair(std::make_pair(cell_id, category->id()), 0))
            .first->second += 1;
        ...
        if (i->visited()) {
          m_num_items_visited_by_cell_and_category
              .insert(
                  std::make_pair(std::make_pair(cell_id, category->id()), 0))
              .first->second += 1;
...
Category *Database::create_category(Category *parent, const std::string &name) {
  set_modified();
...
Category *Database::create_category(const std::string &name) {
  set_modified();

  return create_category(mp_categories, name);
}

Category *Database::create_category(Categories *container,
...
Category *Database::category_by_name_non_const(const std::string &name) {
  return mp_categories->category_by_name(name.c_str());
}

Category *Database::category_by_id_non_const(id_type id) {
...
Cell *Database::create_cell(const std::string &name,
  ...
  if (variant.empty()) {
    ...
    std::map<std::string, std::vector<id_type>>::iterator variant =
        m_cell_variants.find(name);

    //  If another cell with that name already exists, rename it to variant
    //  "cell:1":
    ...
    if (other_cell != 0) {
      ...
      variant =
          m_cell_variants.insert(std::make_pair(name, std::vector<id_type>()))
              .first;
    ...
    if (variant != m_cell_variants.end()) {
      ...
      for (unsigned int n = (std::numeric_limits<unsigned int>::max() / 2) + 1;
           n > 0; n /= 2) {
    ...
    //  If another cell with that name already exists, rename it to a suitable
    //  variant
    Cell *other_cell = cell_by_qname_non_const(name);
    if (other_cell != 0) {
...
const std::vector<id_type> &Database::variants(const std::string &name) {
  std::map<std::string, std::vector<id_type>>::iterator variant =
      m_cell_variants.find(name);
  if (variant != m_cell_variants.end()) {
...
Cell *Database::cell_by_qname_non_const(const std::string &qname) {
  std::map<std::string, Cell *>::const_iterator c =
      m_cells_by_qname.find(qname);
...
Cell *Database::cell_by_id_non_const(id_type id) {
...
void Database::set_tag_description(id_type tag_id,
...
void Database::set_item_image_str(const Item *item,
...
void Database::set_item_visited(const Item *item_c, bool visited) {
  if (visited != item_c->visited()) {
    ...
    while (cat != 0) {
      ...
      m_num_items_visited_by_cell_and_category
          .insert(std::make_pair(std::make_pair(item->cell_id(), cat->id()), 0))
          .first->second += (visited ? 1 : -1);
      cat = cat->parent();
    }
  }
}

Item *Database::create_item(id_type cell_id, id_type category_id) {
  set_modified();
  ...
  m_items_by_cell_id.insert(std::make_pair(cell_id, std::list<ItemRef>()))
      .first->second.push_back(ItemRef(item));
  m_items_by_category_id
      .insert(std::make_pair(category_id, std::list<ItemRef>()))
      .first->second.push_back(ItemRef(item));
  m_items_by_cell_and_category_id
...
Database::items_by_cell_and_category(id_type cell_id,
                                     id_type category_id) const {
  std::map<std::pair<id_type, id_type>, std::list<ItemRef>>::const_iterator i =
      m_items_by_cell_and_category_id.find(
          std::make_pair(cell_id, category_id));
...
Database::items_by_cell(id_type cell_id) const {
  std::map<id_type, std::list<ItemRef>>::const_iterator i =
      m_items_by_cell_id.find(cell_id);
...
Database::items_by_category(id_type category_id) const {
  std::map<id_type, std::list<ItemRef>>::const_iterator i =
      m_items_by_category_id.find(category_id);
...
size_t Database::num_items(id_type cell_id, id_type category_id) const {
  std::map<std::pair<id_type, id_type>, size_t>::const_iterator n =
      m_num_items_by_cell_and_category.find(
          std::make_pair(cell_id, category_id));
...
size_t Database::num_items_visited(id_type cell_id, id_type category_id) const {
  std::map<std::pair<id_type, id_type>, size_t>::const_iterator n =
      m_num_items_visited_by_cell_and_category.find(
          std::make_pair(cell_id, category_id));
...
void Database::load(const std::string &fn) {
