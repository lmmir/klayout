...
class RVEReaderException : public ReaderException {
public:
  RVEReaderException(const std::string &msg, size_t line)
      : ReaderException(
            tl::sprintf(tl::to_string(tr("%s (line=%lu)")), msg, line)) {}
};

class RVEReader : public ReaderBase {
public:
  ...
  void do_read(Database &db) {
    ...
    if (res < 0.001 || res > 1e6) {
      error(
          tl::sprintf(tl::to_string(tr("Invalid resolution value: %g")), res));
    }
    ...
    while (!at_end()) {
      ...
      //  cell name is reset. Any shape not having a specific cell will go into
      //  the top cell then.
      cell_name.clear();

      //  Read the category name unless we have some already (that we got when
      //  parsing the shapes).
      if (cat_name.empty()) {
        ex = tl::Extractor(get_line().c_str());
      ...
      for (size_t shape = 0; shape < n1; ++shape) {
        ...
        if (at_end()) {
          warn(tl::to_string(tr("Unexpected end of file before the specified "
                                "number of shapes was read - stopping.")));
          break;
        }
        ...
        if (!valid) {
          ...
          if (!*start) {
            ...
            cat_name = std::string(start, end - start);
            warn(tl::to_string(
                tr("Obviously reaching end of shapes list before the specified "
                   "number of shapes was read - parsing next category.")));
          }
          break;
        }
          ...
          if (isalpha(c)) {
            ...
            if (prop_name == "CN") {
              ...
              if (cspace) {
                ...
                trans = db::DCplxTrans(
                    db::DTrans(f, db::DVector(x * dbu, y * dbu)));
              } else {
                shape_trans = db::DCplxTrans(
                    db::DTrans(f, db::DVector(x * dbu, y * dbu)));
        ...
        //  Use the last cell or the top cell if no cell is specified since the
        ...
        if (shape_type == 'p') {
          ...
          for (size_t point = 0; point < npoints; ++point) {
            ...
            edges.push_back(db::DEdge(db::DPoint(x1 * dbu, y1 * dbu),
                                      db::DPoint(x2 * dbu, y2 * dbu))
                                .transformed(shape_trans));
          }

          if (edges.size() == 2) {
            //  produce an edge pair from two edges
            values.add(
                new Value<db::DEdgePair>(db::DEdgePair(edges[0], edges[1])));
          } else {
            for (std::vector<db::DEdge>::const_iterator e = edges.begin();
                 e != edges.end(); ++e) {
          ...
          error(tl::sprintf(tl::to_string(tr("Invalid shape type: %c")),
                            shape_type));
        }
  ...
  virtual const char *format() const { return "RVE"; }
...
private:
  ...
  bool at_end() { return m_input_stream.at_end(); }
  ...
  void warn(const std::string &msg) {
    tl::warn << tl::sprintf(tl::to_string(tr("%s (line=%lu)")), msg,
                            m_input_stream.line_number());
  }
...
class RVEFormatDeclaration : public FormatDeclaration {
  ...
  virtual std::string file_format() const {
    return "RVE files (*.rve *.rve.gz *.db *.db.gz)";
  }
  ...
  virtual bool detect(tl::InputStream &stream) const {
    ...
    //  If we are at the end, this is probably an empty file. No checks and no
    //  results.
    if (text_stream.at_end()) {
      return true;
    }
...
static tl::RegisteredClass<rdb::FormatDeclaration>
    format_decl(new RVEFormatDeclaration(), 0, "RVE");
