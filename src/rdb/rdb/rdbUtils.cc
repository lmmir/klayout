...
void scan_layer(rdb::Category *cat, const db::Layout &layout,
                unsigned int layer, const db::Cell *from, int levels,
  ...
  for (db::Layout::const_iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    if (cell.shapes(layer).size() > 0) {
      ...
      if (!rdb_cell) {
        ...
        if (from) {
          std::pair<bool, db::ICplxTrans> ctx = db::find_layout_context(
              layout, cell.cell_index(), from->cell_index());
          if (ctx.first) {
      ...
      create_items_from_shapes(rdb, rdb_cell->id(), cat->id(),
                               db::CplxTrans(layout.dbu()), cell.shapes(layer),
                               with_properties);
    }
  }
}
...
class CreateItemsRecursiveReceiver : public db::RecursiveShapeReceiver {
public:
  CreateItemsRecursiveReceiver(rdb::Category *cat, const db::CplxTrans &trans,
                               rdb::Cell *cell, bool with_properties)
  ...
  virtual void begin(const db::RecursiveShapeIterator *iter) {
    ...
    if (!m_cell_stack.empty() && rdb_cell != m_cell_stack.front() &&
        (rdb_cell->references().begin() == rdb_cell->references().end())) {
      //  If the actual top cell is not the one specified, add a dummy reference
      //  so we find the real top cell under the given one.
      //  TODO: get rid of the const_cast
      (const_cast<rdb::Cell *>(rdb_cell))
          ->references()
          .insert(rdb::Reference(db::DCplxTrans(), m_cell_stack.front()->id()));
    }
  ...
  virtual void enter_cell(const db::RecursiveShapeIterator *iter,
    ...
    if (rdb_cell->references().begin() == rdb_cell->references().end()) {
      ...
      (const_cast<rdb::Cell *>(rdb_cell))
          ->references()
          .insert(rdb::Reference(t, m_cell_stack.front()->id()));
    }
  }
  ...
  virtual void shape(const db::RecursiveShapeIterator * /*iter*/,
    ...
    create_item_from_shape(mp_rdb, m_cell_stack.back()->id(), mp_cat->id(),
                           m_trans, shape, m_with_properties);
  }
...
public:
  ...
  const rdb::Cell *cell_for_id(const db::Layout *layout,
...
class CreateItemsFlatReceiver : public db::RecursiveShapeReceiver {
public:
  CreateItemsFlatReceiver(rdb::Category *cat, const db::CplxTrans &trans,
                          rdb::Cell *cell, bool with_properties)
      : mp_cat(cat), mp_rdb(cat->database()), m_trans(trans), mp_rdb_cell(cell),
        m_with_properties(with_properties) {
    //  .. nothing yet ..
  ...
  virtual void shape(const db::RecursiveShapeIterator * /*iter*/,
                     ...
                     const box_tree_type * /*complex_region*/) {
    create_item_from_shape(mp_rdb, mp_rdb_cell->id(), mp_cat->id(),
                           m_trans * trans, shape, m_with_properties);
  }
...
void scan_layer(rdb::Category *cat, const db::RecursiveShapeIterator &iter,
...
void scan_layer(rdb::Category *cat, rdb::Cell *cell, const db::CplxTrans &trans,
  ...
  if (flat) {
    ...
    rec.reset(
        new CreateItemsRecursiveReceiver(cat, trans, cell, with_properties));
  }
...
void create_items_from_iterator(rdb::Database *db, rdb::id_type cell_id,
                                rdb::id_type cat_id,
  ...
  for (db::RecursiveShapeIterator i = iter; !i.at_end(); ++i) {
    create_item_from_shape(db, cell_id, cat_id, db::CplxTrans(dbu) * i.trans(),
                           *i, with_properties);
  }
}

void create_items_from_shapes(rdb::Database *db, rdb::id_type cell_id,
                              rdb::id_type cat_id, const db::CplxTrans &trans,
                              const db::Shapes &shapes, bool with_properties) {
  for (db::Shapes::shape_iterator s = shapes.begin(db::ShapeIterator::All);
       !s.at_end(); ++s) {
    create_item_from_shape(db, cell_id, cat_id, trans, *s, with_properties);
  }
}

void create_item_from_shape(rdb::Database *db, rdb::id_type cell_id,
                            rdb::id_type cat_id, const db::CplxTrans &trans,
                            const db::Shape &shape, bool with_properties) {
  std::unique_ptr<rdb::ValueBase> value(
      rdb::ValueBase::create_from_shape(shape, trans));
  if (!value.get()) {
    return;
  }
  ...
  if (with_properties && shape.has_prop_id() && shape.shapes() &&
      shape.shapes()->cell()) {
    ...
    if (layout) {
      ...
      for (auto i = ps.begin(); i != ps.end(); ++i) {
        id_type tag_id = db->tags()
                             .tag(layout->properties_repository()
                                      .prop_name(i->first)
                                      .to_string(),
                                  true /*user tag*/)
                             .id();
...
void create_items_from_region(rdb::Database *db, rdb::id_type cell_id,
                              rdb::id_type cat_id, const db::CplxTrans &trans,
                              const db::Region &collection) {
...
void create_items_from_edges(rdb::Database *db, rdb::id_type cell_id,
                             rdb::id_type cat_id, const db::CplxTrans &trans,
                             const db::Edges &collection) {
...
void create_items_from_edge_pairs(rdb::Database *db, rdb::id_type cell_id,
                                  rdb::id_type cat_id,
                                  const db::CplxTrans &trans,
                                  const db::EdgePairs &collection) {
...
ValueBase *add_item_value(rdb::Item *item, const tl::Variant &v,
                          const db::CplxTrans &trans, rdb::id_type tag_id) {
  if (v.is_user<db::Box>()) {
  ...
  } else if (v.is_double() || v.is_long() || v.is_ulong() || v.is_longlong() ||
             v.is_ulonglong()) {
...
ValueBase *add_item_value(rdb::Item *item, const tl::Variant &v, double dbu,
                          rdb::id_type tag_id) {
  if (dbu > 0 && v.is_user<db::Box>()) {
    ...
    return item->add_value(db::CplxTrans(dbu) * v.to_user<db::Polygon>(),
                           tag_id);
    ...
    return item->add_value(db::CplxTrans(dbu) * v.to_user<db::EdgePair>(),
                           tag_id);
