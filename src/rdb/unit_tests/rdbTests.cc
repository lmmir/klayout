...
TEST(5) {
...

  {
    ...
    c2->references().insert(rdb::Reference(
        db::DCplxTrans(db::DTrans(db::DVector(17.5, -25))), c1->id()));
    rdb::Cell *c3 = db.create_cell("c3");
    c3->references().insert(rdb::Reference(db::DCplxTrans(), c2->id()));
    c3->references().insert(rdb::Reference(
        db::DCplxTrans(1.5, 45, true, db::DVector(10.0, 20.0)), c1->id()));
    ...
    rdb::Item *i2 = db.create_item(c2->id(), cath2->id());
    i2->values().add(new rdb::Value<db::DEdge>(
        db::DEdge(db::DPoint(1.0, -1.0), db::DPoint(10.0, 11.0))));
    i2->values().add(
        new rdb::Value<db::DBox>(db::DBox(10.0, -10.0, 100.0, 110.0)));
...

  {
    ...
    EXPECT_EQ(db2.category_by_name("cath2.cc")->description(),
              "cath2.cc description");
    ...
    be = db2.items_by_cell_and_category(
        db2.cell_by_qname("c1")->id(), db2.category_by_name("cath_name")->id());
    EXPECT_EQ(be.first != be.second, true);
    ...
    be = db2.items_by_cell_and_category(
        db2.cell_by_qname("c2")->id(), db2.category_by_name("cath_name")->id());
    EXPECT_EQ(be.first != be.second, false);

    be = db2.items_by_cell_and_category(db2.cell_by_qname("c2")->id(),
                                        db2.category_by_name("cath2")->id());
    EXPECT_EQ(be.first != be.second, true);
    ...
    be = db2.items_by_cell_and_category(db2.cell_by_qname("c1")->id(),
                                        db2.category_by_name("cath2.cc")->id());
    EXPECT_EQ(be.first != be.second, true);
    EXPECT_EQ((*be.first)->visited(), true);
  }
}
...
TEST(5a) {
...

  {
    ...
    be =
        db2.items_by_cell_and_category(db2.cell_by_qname("c1:1")->id(),
                                       db2.category_by_name("cath_name")->id());
    EXPECT_EQ(be.first != be.second, true);
    ...
    be =
        db2.items_by_cell_and_category(db2.cell_by_qname("c1:2")->id(),
                                       db2.category_by_name("cath_name")->id());
    EXPECT_EQ(be.first != be.second, false);

    be = db2.items_by_cell_and_category(db2.cell_by_qname("c1:2")->id(),
                                        db2.category_by_name("cath2")->id());
    EXPECT_EQ(be.first != be.second, true);
    ...
    be = db2.items_by_cell_and_category(db2.cell_by_qname("c1:1")->id(),
                                        db2.category_by_name("cath2.cc")->id());
    EXPECT_EQ(be.first != be.second, true);
    EXPECT_EQ((*be.first)->visited(), true);
  }
...
TEST(7) {
...

  {
    ...
    EXPECT_EQ(tl::PixelBuffer::from_image(img) ==
                  tl::PixelBuffer::from_image(img2),
              true);
  }
