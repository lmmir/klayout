...
void CombinedChangeApplicator::add(ChangeApplicator *a) { m_appl.push_back(a); }

CombinedChangeApplicator::~CombinedChangeApplicator() {
  for (std::vector<ChangeApplicator *>::const_iterator a = m_appl.begin();
       a != m_appl.end(); ++a) {
    delete *a;
  }
  m_appl.clear();
...
db::Shape CombinedChangeApplicator::do_apply(db::Shapes &shapes,
...
db::Instance
CombinedChangeApplicator::do_apply_inst(db::Cell &cell,
...
ChangePropertiesApplicator::ChangePropertiesApplicator(
    db::properties_id_type prop_id)
    : m_prop_id(prop_id) {
  //  .. nothing yet ...
}

db::Shape ChangePropertiesApplicator::do_apply(db::Shapes &shapes,
...
db::Instance ChangePropertiesApplicator::do_apply_inst(
...
BoxDimensionsChangeApplicator::BoxDimensionsChangeApplicator(
    db::Coord dl, db::Coord db, db::Coord dr, db::Coord dt, db::Coord l,
    db::Coord b, db::Coord r, db::Coord t)
    : m_dl(dl), m_db(db), m_dr(dr), m_dt(dt), m_l(l), m_b(b), m_r(r), m_t(t) {
  //  .. nothing yet ..
}

db::Shape BoxDimensionsChangeApplicator::do_apply(db::Shapes &shapes,
                                                  const db::Shape &shape,
                                                  double /*dbu*/,
                                                  bool relative) const {
  db::Box org_box;
  ...
  db::Box new_box;
  if (relative) {
    new_box = db::Box(org_box.left() + m_dl, org_box.bottom() + m_db,
                      org_box.right() + m_dr, org_box.top() + m_dt);
  } else {
...
PointDimensionsChangeApplicator::PointDimensionsChangeApplicator(
    const db::Point &point, const db::Point &org_point)
    : m_point(point), m_org_point(org_point) {
  //  .. nothing yet ..
}

db::Shape PointDimensionsChangeApplicator::do_apply(db::Shapes &shapes,
                                                    const db::Shape &shape,
                                                    double /*dbu*/,
                                                    bool relative) const {
  db::Point org_point;
...
PolygonChangeApplicator::PolygonChangeApplicator(const db::Polygon &poly,
                                                 const db::Polygon &org_poly)
    : m_poly(poly), m_org_poly(org_poly) {
  //  .. nothing yet ..
}

db::Shape PolygonChangeApplicator::do_apply(db::Shapes &shapes,
                                            const db::Shape &shape,
                                            double /*dbu*/,
                                            bool relative) const {
  db::Polygon org_poly;
  ...
  if (relative) {
    ...
    db::Polygon new_poly =
        m_poly.moved(org_poly.box().p1() - m_poly.box().p1());
...
TextOrientationChangeApplicator::TextOrientationChangeApplicator(
    const db::FTrans &trans)
    : ChangeApplicator(), m_trans(trans) {
  //  .. nothing yet ..
}

db::Shape TextOrientationChangeApplicator::do_apply(db::Shapes &shapes,
                                                    const db::Shape &shape,
                                                    double /*dbu*/,
                                                    bool /*relative*/) const {
  db::Text org_text;
...
TextPositionChangeApplicator::TextPositionChangeApplicator(
    const db::Vector &disp, const db::Vector &org_disp)
    : ChangeApplicator(), m_disp(disp), m_org_disp(org_disp) {
  //  .. nothing yet ..
}

db::Shape TextPositionChangeApplicator::do_apply(db::Shapes &shapes,
                                                 const db::Shape &shape,
                                                 double /*dbu*/,
                                                 bool relative) const {
  db::Text org_text;
...
db::Shape TextHAlignChangeApplicator::do_apply(db::Shapes &shapes,
                                               const db::Shape &shape,
                                               double /*dbu*/,
                                               bool /*relative*/) const {
  db::Text org_text;
...
db::Shape TextVAlignChangeApplicator::do_apply(db::Shapes &shapes,
                                               const db::Shape &shape,
                                               double /*dbu*/,
                                               bool /*relative*/) const {
  db::Text org_text;
...
db::Shape TextSizeChangeApplicator::do_apply(db::Shapes &shapes,
                                             const db::Shape &shape,
                                             double /*dbu*/,
                                             bool /*relative*/) const {
  db::Text org_text;
...
TextStringChangeApplicator::TextStringChangeApplicator(
    const std::string &string)
    : ChangeApplicator(), m_string(string) {
  //  .. nothing yet ..
}

db::Shape TextStringChangeApplicator::do_apply(db::Shapes &shapes,
                                               const db::Shape &shape,
                                               double /*dbu*/,
                                               bool /*relative*/) const {
  db::Text org_text;
...
PathPointsChangeApplicator::PathPointsChangeApplicator(
...
db::Shape PathPointsChangeApplicator::do_apply(db::Shapes &shapes,
                                               const db::Shape &shape,
                                               double /*dbu*/,
                                               bool relative) const {
  db::Path org_path;
  ...
  if (relative && !m_org_points.empty() && new_path.begin() != new_path.end()) {
    ...
    for (std::vector<db::Point>::iterator p = new_points.begin();
         p != new_points.end(); ++p) {
...
PathWidthChangeApplicator::PathWidthChangeApplicator(db::Coord w,
                                                     db::Coord org_w)
    : ChangeApplicator(), m_width(w), m_org_width(org_w) {
  //  .. nothing yet ..
}

db::Shape PathWidthChangeApplicator::do_apply(db::Shapes &shapes,
                                              const db::Shape &shape,
                                              double /*dbu*/,
                                              bool relative) const {
  db::Path org_path;
...
PathStartExtensionChangeApplicator::PathStartExtensionChangeApplicator(
    db::Coord e)
    : ChangeApplicator(), m_ext(e) {
  //  .. nothing yet ..
}

db::Shape PathStartExtensionChangeApplicator::do_apply(
...
db::Shape PathEndExtensionChangeApplicator::do_apply(db::Shapes &shapes,
                                                     const db::Shape &shape,
                                                     double /*dbu*/,
                                                     bool /*relative*/) const {
  db::Path org_path;
...
db::Shape PathRoundEndChangeApplicator::do_apply(db::Shapes &shapes,
                                                 const db::Shape &shape,
                                                 double /*dbu*/,
                                                 bool /*relative*/) const {
  db::Path org_path;
...
ChangeTargetCellApplicator::ChangeTargetCellApplicator(
    db::cell_index_type cell_index)
    : m_cell_index(cell_index) {
  //  .. nothing yet ..
}

db::Instance ChangeTargetCellApplicator::do_apply_inst(
    db::Cell &cell, const db::Instance &instance, double /*dbu*/,
    bool /*relative*/) const {
  ...
  if (m_cell_index == cell.cell_index() ||
      called.find(cell.cell_index()) != called.end()) {
    throw tl::Exception(
...
ChangeTargetPCellApplicator::ChangeTargetPCellApplicator(
    db::pcell_id_type pcell_id, bool apply_new_id, db::Library *new_lib,
    bool apply_new_lib,
    const std::map<std::string, tl::Variant> &modified_parameters)
    : m_pcell_id(pcell_id), m_apply_new_id(apply_new_id), mp_new_lib(new_lib),
      m_apply_new_lib(apply_new_lib),
      m_modified_parameters(modified_parameters) {
  //  .. nothing yet ..
}

db::Instance ChangeTargetPCellApplicator::do_apply_inst(
    db::Cell &cell, const db::Instance &instance, double /*dbu*/,
    bool /*relative*/) const {
  tl_assert(cell.layout());
  ...
  std::pair<bool, db::pcell_id_type> pci =
      layout->is_pcell_instance(instance.cell_index());
  ...
  if (pci.first) {
    named_parameters =
        layout->get_named_pcell_parameters(instance.cell_index());
  }
  for (std::map<std::string, tl::Variant>::const_iterator p =
           m_modified_parameters.begin();
       p != m_modified_parameters.end(); ++p) {
    named_parameters[p->first] = p->second;
  }

  if ((m_apply_new_lib && lib != mp_new_lib) ||
      (m_apply_new_id &&
       (lib != mp_new_lib || !pci.first || pci.second != m_pcell_id))) {
    ...
    if (m_apply_new_id) {
      ...
      if (!pci.first) {
        std::string cell_name =
            (lib ? &lib->layout() : layout)->cell_name(instance.cell_index());
        ci = (mp_new_lib ? &mp_new_lib->layout() : layout)
                 ->cell_by_name(cell_name.c_str());
      } else {
        std::string pcell_name = (lib ? &lib->layout() : layout)
                                     ->pcell_declaration(pci.second)
                                     ->name();
        pci = (mp_new_lib ? &mp_new_lib->layout() : layout)
                  ->pcell_by_name(pcell_name.c_str());
      }
  ...
  if (ci.first || pci.first) {
    ...
    if (pci.first) {
      inst_cell_index =
          (lib ? &lib->layout() : layout)
              ->get_pcell_variant_dict(pci.second, named_parameters);
    } else {
...
ChangeInstanceTransApplicator::ChangeInstanceTransApplicator(
    double a, double org_a, bool mirror, bool org_mirror, double m,
    double org_m, const db::DVector &disp, const db::DVector &org_disp)
    : m_angle(a), m_org_angle(org_a), m_mirror(mirror),
      m_org_mirror(org_mirror), m_mag(m), m_org_mag(org_m), m_disp(disp),
      m_org_disp(org_disp) {
  //  .. nothing yet ..
}

db::Instance
ChangeInstanceTransApplicator::do_apply_inst(db::Cell &cell,
                                             const db::Instance &instance,
                                             double dbu, bool relative) const {
  ...
  if (relative) {
    ...
    tr = db::CellInstArray::complex_trans_type(
             m_mag, m_angle, m_mirror, db::Vector(m_disp * (1.0 / dbu))) *
         db::CellInstArray::complex_trans_type(
             m_org_mag, m_org_angle, m_org_mirror,
             db::Vector(m_org_disp * (1.0 / dbu)))
             .inverted() *
         tr;
  ...
  if (instance.is_regular_array(a, b, na, nb)) {
    ...
    if (is_complex) {
      new_inst = db::CellInstArray(db::CellInst(instance.cell_index()), tr, a,
                                   b, na, nb);
    } else {
      new_inst =
          db::CellInstArray(db::CellInst(instance.cell_index()),
                            db::Trans(tr.rot(), tr.disp()), a, b, na, nb);
    }
    ...
    if (is_complex) {
      ...
      new_inst = db::CellInstArray(db::CellInst(instance.cell_index()),
                                   db::Trans(tr.rot(), tr.disp()));
    }
  }
...
ChangeInstanceArrayApplicator::ChangeInstanceArrayApplicator(
    const db::DVector &a, bool set_a, const db::DVector &b, bool set_b,
    unsigned long na, bool set_na, unsigned long nb, bool set_nb)
    : m_a(a), m_set_a(set_a), m_b(b), m_set_b(set_b), m_na(na),
      m_set_na(set_na), m_nb(nb), m_set_nb(set_nb) {
  //  .. nothing yet ..
}

db::Instance ChangeInstanceArrayApplicator::do_apply_inst(
    db::Cell &cell, const db::Instance &instance, double dbu,
    bool /*relative*/) const {
  db::CellInstArray new_inst;
  ...
  if (instance.is_complex()) {
    new_inst = db::CellInstArray(db::CellInst(instance.cell_index()),
                                 instance.complex_trans(), a, b, na, nb);
  } else {
    new_inst = db::CellInstArray(db::CellInst(instance.cell_index()),
                                 instance.front(), a, b, na, nb);
  }
...
db::Instance InstanceRemoveArrayApplicator::do_apply_inst(
    db::Cell &cell, const db::Instance &instance, double /*dbu*/,
    bool /*relative*/) const {
  db::CellInstArray new_inst;
  if (instance.is_complex()) {
    new_inst = db::CellInstArray(db::CellInst(instance.cell_index()),
                                 instance.complex_trans());
  } else {
    new_inst = db::CellInstArray(db::CellInst(instance.cell_index()),
                                 instance.front());
  }
...
std::string coords_to_string(const db::DPoint &dp, double dbu, bool du,
                             const char *sep) {
  return coord_to_string(dp.x(), dbu, du) + sep +
         coord_to_string(dp.y(), dbu, du);
}
