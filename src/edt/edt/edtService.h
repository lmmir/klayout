...
class EDT_PUBLIC Service : public lay::EditorServiceBase, public db::Object {
public:
  enum SnapMode {
    Any = 0,
    Diagonal,
  ...
  /**
   *  @brief Deliver the selection's bbox (reimplementation of lay::Editable
   * interface)
   */
   ...
   */
  virtual void transform(const db::DCplxTrans &tr) { transform(tr, 0); }
   ...
   */
  virtual bool begin_move(lay::Editable::MoveMode mode, const db::DPoint &p,
  ...
  /**
   *  @brief Returns true, if the given selected object is handled by this
   * service
   */
   ...
   *
   *  This version of the transformation operation allows one to specify a
   * transformation per selected object. The transformations in the vector must
   * be ordered in the order the selection is delivered by the selection
   * iterator. Either global or per-object transformations can be specified. If
   * a per-object transformation is specified, the global transformation is
   * ignored.
   *
   *  @param tr The global transformation (ignored if p_trv != 0)
   *  @param p_trv The per-object transformation (can be 0 for one global
   * transformation)
   */
   ...
   */
  tl::Color color() const { return m_color; }

  /**
   *  @brief Get the selection container
   */
  const objects &selection() const { return m_selection; }

  /**
   *  @brief Get the transient selection container
   */
  const objects &transient_selection() const { return m_transient_selection; }
   ...
   */
  bool select(const lay::ObjectInstPath &obj,
              lay::Editable::SelectionMode mode);
   ...
   */
  void clear_selection() { select(db::DBox(), lay::Editable::Reset); }

  /**
   *  @brief Set the selection
   */
  void set_selection(std::vector<lay::ObjectInstPath>::const_iterator s1,
  ...
  /**
   *  @brief Triggered by tap - gives the new layer and if required the initial
   * point
   */
   ...
   *
   *  @return true, if PCells have been updated, indicating that our selection
   * is no longer valid
   *
   *  This version assumes there is only one guiding shape selected and will
   * update the selection. It will also call layout.cleanup() if required.
   */
  bool handle_guiding_shape_changes();

  /**
   *  @brief Handle changes in a specific guiding shape, i.e. create new PCell
   * variants if required
   *
   *  @return A pair of bool (indicating that the object path has changed) and
   * the new guiding shape path
   */
  std::pair<bool, lay::ObjectInstPath>
  handle_guiding_shape_changes(const lay::ObjectInstPath &obj) const;
...
protected:
   ...
   *  If the edit operation shall begin immediately, this method is supposed to
   * return true.
   */
   ...
   *  @brief Reimplemented by the specific implementation of the shape editors
   *
   *  This method is called when the mouse is moved but editing has not started
   * yet
   */
   ...
   */
  virtual void do_mouse_transform(const db::DPoint & /*p*/,
                                  db::DFTrans /*trans*/) {}
  ...
  /**
   *  @brief Called when a configuration parameter provided by the service base
   * class has changed
   */
   ...
   *  @param connect true, if the point is an connection vertex, false if it is
   * a move target point
   */
  db::DPoint snap(const db::DPoint &p, const db::DPoint &plast,
                  bool connect) const;
  ...
  /**
   *  @brief Snap a point to the edit grid with advanced snapping (including
   * object snapping)
   *
   *  @param p The point to snap
   *  @param connect true, if the point is an connection vertex, false if it is
  ...
  /**
   *  @brief Snap a point to the edit grid with an angle constraint  with
   * advanced snapping (including object snapping)
   *
   *  @param p The point to snap
   *  @param plast The last point of the connection/move vector
   *  @param connect true, if the point is an connection vertex, false if it is
   * a move target point
   */
  db::DPoint snap2(const db::DPoint &p, const db::DPoint &plast,
                   bool connect = true) const;

protected:
  lay::angle_constraint_type connect_ac() const;
  lay::angle_constraint_type move_ac() const;

  bool show_shapes_of_instances() const { return m_show_shapes_of_instances; }
...
private:
  ...
  //  True, if on the first mouse move an immediate do_begin_edit should be
  //  issued.
  bool m_immediate;
  ...
  //  The previous selection (used for cycling through different selections for
  //  single clicks)
  ...
  /**
   *  @brief Update m_markers to reflect the selection (called through deferred
   * method)
   */
  void do_selection_to_view();

  /**
   *  @brief Update m_markers with the new transformation to reflect the
   * movement
   */
