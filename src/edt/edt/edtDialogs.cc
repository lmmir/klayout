...
InstantiationForm::InstantiationForm(QWidget *parent)
    : QDialog(parent), mp_view(0), mp_path(0), mp_marker(0),
...
void InstantiationForm::double_clicked(QListWidgetItem *item) {
  ...
  lay::CellView::unspecific_cell_path_type path(
      mp_view->cellview(mp_path->cv_index()).combined_unspecific_path());
  int nrow = 0;
  for (lay::ObjectInstPath::iterator p = mp_path->begin();
       p != mp_path->end() && nrow < row; ++p, ++nrow) {
  ...
  for (lay::ObjectInstPath::iterator p = mp_path->begin();
       p != mp_path->end() && nrow < row; ++p, ++nrow) {
    abs_trans =
        abs_trans * (p->inst_ptr.cell_inst().complex_trans(*(p->array_inst)));
  }

  mp_marker->set(box, abs_trans,
                 mp_view->cv_transform_variants(mp_path->cv_index()));
}

void InstantiationForm::show(lay::LayoutViewBase *view,
                             const lay::ObjectInstPath &path) {
  mp_view = view;
  mp_path = &path;
...
void InstantiationForm::update() {
  ...
  //  first include the context path of the cellview in order to tell the path
  //  within the cell shown
  for (lay::CellView::specific_cell_path_type::const_iterator p =
           cv.specific_path().begin();
       p != cv.specific_path().end(); ++p) {

    //  build the instance information from the path

    db::CplxTrans trans(
        p->inst_ptr.cell_inst().complex_trans(*(p->array_inst)));
  ...
  for (lay::ObjectInstPath::iterator p = mp_path->begin(); p != mp_path->end();
       ++p) {
...
CopyModeDialog::CopyModeDialog(QWidget *parent) : QDialog(parent) {
...
bool ChangeLayerOptionsDialog::exec_dialog(lay::LayoutViewBase *view,
                                           int cv_index,
  ...
  int initial_sel = -1;
  int i = 0;
  for (std::vector<std::pair<db::LayerProperties, unsigned int>>::const_iterator
           lp = ll.begin();
       lp != ll.end(); ++lp, ++i) {
    if (lp->second == new_layer) {
      initial_sel = i;
    }
    target_cbx->addItem(tl::to_qstring(
        lay::ParsedLayerSource(lp->first, cv_index).to_string()));
  }
  target_cbx->setCurrentIndex(initial_sel);

  if (QDialog::exec() && target_cbx->currentIndex() >= 0 &&
      target_cbx->currentIndex() < int(ll.size())) {
...
AlignOptionsDialog::AlignOptionsDialog(QWidget *parent) : QDialog(parent) {
  setObjectName(QString::fromUtf8("change_layer_options_dialog"));
...
bool AlignOptionsDialog::exec_dialog(int &hmode, int &vmode,
                                     bool &visible_layers) {
  QRadioButton *hmode_buttons[] = {this->h_none_rb, this->h_left_rb,
                                   this->h_center_rb, this->h_right_rb};
  QRadioButton *vmode_buttons[] = {this->v_none_rb, this->v_top_rb,
                                   this->v_center_rb, this->v_bottom_rb};
  QRadioButton *layers_buttons[] = {this->all_layers_rb,
                                    this->visible_layers_rb};
...
bool DistributeOptionsDialog::exec_dialog(bool &hdistribute, int &hmode,
                                          double &hpitch, double &hspace,
                                          bool &vdistribute, int &vmode,
                                          double &vpitch, double &vspace,
                                          bool &visible_layers) {
  QRadioButton *hmode_buttons[] = {this->h_none_rb, this->h_left_rb,
                                   this->h_center_rb, this->h_right_rb};
  QRadioButton *vmode_buttons[] = {this->v_none_rb, this->v_top_rb,
...
bool MakeCellOptionsDialog::exec_dialog(const db::Layout &layout,
                                        std::string &name, int &mode_x,
                                        int &mode_y) {
  do {
    BEGIN_PROTECTED
    ...
    if (QDialog::exec()) {
      ...
      if (name.empty()) {
        throw tl::Exception(
            tl::to_string(QObject::tr("Cell name must not be empty")));
...
RoundCornerOptionsDialog::RoundCornerOptionsDialog(QWidget *parent)
    : QDialog(parent), mp_layout(0), m_router_extracted(0.0),
      m_rinner_extracted(0.0), m_npoints_extracted(64), m_has_extracted(false) {
...
void RoundCornerOptionsDialog::amend_changed() {
  if (amend_cb->isChecked() && m_has_extracted) {
    ...
    if (db::coord_traits<double>::equal(m_router_extracted,
                                        m_rinner_extracted)) {
...
bool RoundCornerOptionsDialog::exec_dialog(
    const db::Layout &layout, double &router, double &rinner,
    unsigned int &npoints, bool &undo_before_apply, double router_extracted,
    double rinner_extracted, unsigned int npoints_extracted,
    bool has_extracted) {
  ...
  unsigned int n =
      undo_before_apply && has_extracted ? npoints_extracted : npoints;
...
void RoundCornerOptionsDialog::accept() {
  ...
  if (npoints < min_points) {
    throw tl::Exception(
        tl::to_string(
            QObject::tr("Number of points is too small (must be %d at least)")),
        min_points);
  }
  ...
  if ((rholes > 0.0 &&
       dbu * seg_thr > (rholes * M_PI * 2.0) / double(npoints)) ||
      (rhull > 0.0 && dbu * seg_thr > (rhull * M_PI * 2.0) / double(npoints))) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Number of points is too large (one segment "
                                  "must be larger than %g database units)")),
        seg_thr);
  }
  ...
  if (fabs(rholes - 2.0 * dbu * floor(rholes * 0.5 / dbu + 0.5)) > 1e-6 ||
    ...
    throw tl::Exception(tl::to_string(
        QObject::tr("Radius must be a even multiple of the database unit")));
  }
...
bool MakeArrayOptionsDialog::exec_dialog(db::DVector &a, unsigned int &na,
                                         db::DVector &b, unsigned int &nb) {
...
void MakeArrayOptionsDialog::accept() {
  ...
  if (na < 1 || nb < 1) {
    throw tl::Exception(tl::to_string(QObject::tr(
        "Invalid row or column count (must be larger or equal one)")));
  }
