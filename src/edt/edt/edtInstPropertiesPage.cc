...
static std::string cell_name_from_sel(const edt::Service::obj_iterator &pos,
                                      edt::Service *service) {
  if (!pos->is_cell_inst()) {
    return std::string();
  }
  ...
  std::pair<db::Library *, db::cell_index_type> dl =
      def_layout->defining_library(def_cell_index);
  if (dl.first) {
  ...
  std::pair<bool, db::pcell_id_type> pci =
      def_layout->is_pcell_instance(def_cell_index);
...
struct SelectionPtrSort {
  SelectionPtrSort(edt::Service *service) : mp_service(service) {
    //  .. nothing yet ..
  }

  bool operator()(const edt::Service::obj_iterator &a,
                  const edt::Service::obj_iterator &b) {
    return cell_name_from_sel(a, mp_service) <
           cell_name_from_sel(b, mp_service);
  }
...
static bool is_orthogonal(const db::DVector &rv, const db::DVector &cv) {
  return (db::coord_traits<db::DCoord>::equal(rv.x(), 0) &&
          db::coord_traits<db::DCoord>::equal(cv.y(), 0)) ||
         (db::coord_traits<db::DCoord>::equal(rv.y(), 0) &&
          db::coord_traits<db::DCoord>::equal(cv.x(), 0));
}

InstPropertiesPage::InstPropertiesPage(edt::Service *service,
                                       db::Manager *manager, QWidget *parent)
    : lay::PropertiesPage(parent, manager, service), mp_service(service),
      m_enable_cb_callback(true), mp_pcell_parameters(0) {
  m_selection_ptrs.reserve(service->selection().size());
  for (edt::Service::obj_iterator s = service->selection().begin();
       s != service->selection().end(); ++s) {
    m_selection_ptrs.push_back(s);
  }

  std::sort(m_selection_ptrs.begin(), m_selection_ptrs.end(),
            SelectionPtrSort(service));
  ...
  connect(dbu_cb, SIGNAL(toggled(bool)), this,
          SLOT(display_mode_changed(bool)));
  connect(abs_cb, SIGNAL(toggled(bool)), this,
          SLOT(display_mode_changed(bool)));
  connect(browse_pb, SIGNAL(clicked()), this, SLOT(browse_cell()));
  connect(lib_cbx, SIGNAL(currentIndexChanged(int)), this,
          SLOT(library_changed(int)));
  connect(cell_name_le, SIGNAL(textChanged(const QString &)), this,
          SLOT(cell_name_changed(const QString &)));
...
InstPropertiesPage::~InstPropertiesPage() { mp_service->restore_highlights(); }
...
static void
get_cell_or_pcell_ids_by_name(const db::Layout *layout, const std::string &name,
...
void InstPropertiesPage::browse_cell() {
  ...
  //  find the layout the cell has to be looked up: that is either the layout of
  //  the current instance or the library selected
  db::Layout *layout = 0;
  db::Library *lib = 0;
  ...
  lay::LibraryCellSelectionForm form(this, layout, "browse_lib_cell", false,
                                     lib != 0 /*for libs show top cells only*/);
  if (lib) {
    form.setWindowTitle(
        tl::to_qstring(tl::to_string(QObject::tr("Select Cell - Library: ")) +
                       lib->get_description()));
  }

  std::pair<bool, db::pcell_id_type> pc;
  std::pair<bool, db::cell_index_type> c;
  get_cell_or_pcell_ids_by_name(layout, tl::to_string(cell_name_le->text()), c,
                                pc);
  if (pc.first) {
  ...
  if (form.exec()) {
    ...
    if (form.selected_cell_is_pcell()) {
      cell_name_le->setText(tl::to_qstring(
          layout->pcell_header(form.selected_pcell_id())->get_name()));
    } else if (layout->is_valid_cell_index(form.selected_cell_index())) {
      cell_name_le->setText(
          tl::to_qstring(layout->cell_name(form.selected_cell_index())));
    }
...
void InstPropertiesPage::show_props() {
  ...
  if (props_form.show(mp_service->view(),
                      m_selection_ptrs[m_indexes.front()]->cv_index(),
                      m_prop_id)) {
    emit edited();
  }
}
...
size_t InstPropertiesPage::count() const { return m_selection_ptrs.size(); }

void InstPropertiesPage::select_entries(const std::vector<size_t> &entries) {
  m_indexes = entries;
}

std::string InstPropertiesPage::description(size_t entry) const {
...
std::string InstPropertiesPage::description() const {
  return tl::to_string(tr("Instances"));
}

void InstPropertiesPage::leave() { mp_service->clear_highlights(); }
...
void InstPropertiesPage::update() {
  ...
  if (pci.first && def_layout->pcell_declaration(pci.second)) {
    cell_name_le->setText(
        tl::to_qstring(def_layout->pcell_header(pci.second)->get_name()));
  } else {
    cell_name_le->setText(
        tl::to_qstring(def_layout->cell_name(def_cell_index)));
  }
  cell_name_le->blockSignals(false);
  ...
  if (cell_bbox.empty()) {
    ...
    ch_le->setText(
        tl::to_qstring(coord_to_string(cell_bbox.height(), dbu, du)));
  }
  ...
  if (pos->back().inst_ptr.is_regular_array(rowv, columnv, rows, columns)) {
    ...
    row_x_le->setText(
        tl::to_qstring(coord_to_string((gt * rowv).x(), dbu, du)));
    row_y_le->setText(
        tl::to_qstring(coord_to_string((gt * rowv).y(), dbu, du)));
    column_x_le->setText(
        tl::to_qstring(coord_to_string((gt * columnv).x(), dbu, du)));
    column_y_le->setText(
        tl::to_qstring(coord_to_string((gt * columnv).y(), dbu, du)));
    ...
    if (!pos->back().array_inst.at_end()) {
      ...
      inst_lbl->setText(tl::to_qstring(
          tl::sprintf(tl::to_string(QObject::tr(
                          "This is instance [%ld,%ld] of array with")),
                      row, column)));
    }

    ortho_warning_frame->setEnabled(!is_orthogonal(
        db::CplxTrans(dbu) * rowv, db::CplxTrans(dbu) * columnv));
  ...
  pos_x_le->setText(tl::to_qstring(
      coord_to_string((gt * db::ICplxTrans(t)).disp().x(), dbu, du)));
  pos_y_le->setText(tl::to_qstring(
      coord_to_string((gt * db::ICplxTrans(t)).disp().y(), dbu, du)));
  angle_le->setText(tl::to_qstring(
      tl::to_string(pos->back().inst_ptr.complex_trans().angle())));
  mirror_cbx->setChecked(t.is_mirror());
  mag_le->setText(tl::to_qstring(
      tl::to_string(pos->back().inst_ptr.complex_trans().mag())));
...
void InstPropertiesPage::show_cell() {
  ...
  lay::CellView::unspecific_cell_path_type path(
      mp_service->view()->cellview(pos->cv_index()).combined_unspecific_path());
...
ChangeApplicator *InstPropertiesPage::create_applicator(
    db::Cell & /*cell*/, const db::Instance & /*inst*/, double dbu) {
  tl_assert(!m_indexes.empty());

  bool has_error = false;
  bool has_pcell_error = false;

  std::unique_ptr<CombinedChangeApplicator> appl(
      new CombinedChangeApplicator());
  ...
  try {
    ...
    std::pair<bool, db::cell_index_type> ci;
    std::pair<bool, db::pcell_id_type> pci;
    get_cell_or_pcell_ids_by_name(layout, tl::to_string(cell_name_le->text()),
                                  ci, pci);
    if (!ci.first && !pci.first) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Not a valid cell or PCell name: %s"))
              .c_str(),
          tl::to_string(cell_name_le->text()).c_str());
    }
    ...
    if (lib == 0 && ci.first) {
      ...
      if (ci.second == cv.cell_index() ||
          called.find(cv.cell_index()) != called.end()) {
        throw tl::Exception(
            tl::to_string(QObject::tr("Trying to build a recursive hierarchy"))
                .c_str());
      }
    }
  ...
  try {
    ...
    std::pair<bool, db::pcell_id_type> current_pci =
        current_layout->is_pcell_instance(current_ci);
    std::pair<db::Library *, db::cell_index_type> l =
        current_layout->defining_library(current_ci);
    ...
    if (!ci.first && !pci.first) {
    ...
    } else if (pci.first != current_pci.first ||
               (!pci.first && std::string(layout->cell_name(ci.second)) !=
                                  current_layout->cell_name(current_ci))) {

      //  a cell has been changed into pcell or vice versa, or the cell name has
      //  changed -> we can generate a new proxy and exchange cell indexes
      ...
      if (pci.first) {
        ...
        tl_assert(layout->pcell_declaration(pci.second) ==
                  mp_pcell_parameters->pcell_decl());
        inst_cell_index = layout->get_pcell_variant(
            pci.second, mp_pcell_parameters->get_parameters());
      } else {
      ...
      //  pcell name has changed -> apply parameter deltas to other selected
      //  cells or pcells otherwise keep pcell or cell name, change library if
      //  possible and required and apply parameter deltas to other selected
      //  cells or pcells

      bool adjust_pcell_id =
          layout->pcell_declaration(pci.second)->name() !=
          current_layout->pcell_declaration(current_pci.second)->name();
      ...
      std::vector<tl::Variant> initial_param =
          mp_pcell_parameters->initial_parameters();

      const std::vector<db::PCellParameterDeclaration> &pcp =
          mp_pcell_parameters->pcell_decl()->parameter_declarations();
      for (std::vector<db::PCellParameterDeclaration>::const_iterator pd =
               pcp.begin();
           pd != pcp.end(); ++pd) {
        unsigned int index = pd - pcp.begin();
        if (index < param.size() && index < initial_param.size() &&
            param[index] != initial_param[index]) {
          modified_param_by_name.insert(
              std::make_pair(pd->get_name(), param[index]));
        }
      }

      if (adjust_pcell_id || lib != current_lib ||
          !modified_param_by_name.empty()) {
        appl->add(new ChangeTargetPCellApplicator(pci.second, adjust_pcell_id,
                                                  lib, lib != current_lib,
                                                  modified_param_by_name));
      }

    } else if (lib != current_lib) {

      //  only library name has changed -> try to apply library to all selected
      //  instances keeping the cell name

      //  NOTE: changing the library only is a special case of the
      //  ChangeTargetPCellApplicator
      appl->add(new ChangeTargetPCellApplicator(
          0, false, lib, true, std::map<std::string, tl::Variant>()));
    }
  ...
  db::DVector disp =
      db::DVector(dpoint_from_dpoint(db::DPoint(x, y), dbu, du, t));
  ...
  db::CellInstArray::complex_trans_type trans =
      pos->back().inst_ptr.complex_trans();

  if (fabs(angle - trans.angle()) > 1e-6 || mirror != trans.is_mirror() ||
      fabs(mag - trans.mag()) > 1e-6 || !disp.equal(trans.disp() * dbu)) {
    appl->add(new ChangeInstanceTransApplicator(
        angle, trans.angle(), mirror, trans.is_mirror(), mag, trans.mag(), disp,
        trans.disp() * dbu));
  }
  ...
  bool is_array_org =
      pos->back().inst_ptr.is_regular_array(a_org, b_org, na_org, nb_org);
  ...
  if (array_grp->isChecked()) {
    ...
    if (set_a || set_b || set_na || set_nb) {
      appl->add(new ChangeInstanceArrayApplicator(rv, set_a, cv, set_b, rows,
                                                  set_na, cols, set_nb));
    }
  ...
  if (has_error || has_pcell_error) {
    throw tl::Exception(tl::to_string(
        tr("At least one value and PCell parameter is not correct - see "
           "hightlighted entry fields or the PCell error indicator")));
  } else if (has_error) {
    throw tl::Exception(tl::to_string(tr(
        "At least one value is not correct - see hightlighted entry fields")));
  } else if (has_pcell_error) {
    throw tl::Exception(tl::to_string(
        tr("At least one PCell parameter is not correct - see hightlighted "
           "entry fields or the PCell error indicator")));
  }

  return appl.release();
}

void InstPropertiesPage::recompute_selection_ptrs(
    const std::vector<lay::ObjectInstPath> &new_sel) {
  std::map<lay::ObjectInstPath, edt::Service::obj_iterator> ptrs;

  for (edt::Service::obj_iterator pos = mp_service->selection().begin();
       pos != mp_service->selection().end(); ++pos) {
    ptrs.insert(std::make_pair(*pos, pos));
  }

  m_selection_ptrs.clear();
  for (std::vector<lay::ObjectInstPath>::const_iterator s = new_sel.begin();
       s != new_sel.end(); ++s) {
    std::map<lay::ObjectInstPath, edt::Service::obj_iterator>::const_iterator
        pm = ptrs.find(*s);
...
void InstPropertiesPage::do_apply(bool current_only, bool relative) {
...

  {
    ...
    if (m_prop_id != pos->back().inst_ptr.prop_id()) {
      applicator.reset(new CombinedChangeApplicator(
          applicator.release(), new ChangePropertiesApplicator(m_prop_id)));
    }
  }
  ...
  //  Note: using the apply-all scheme for applying a single change may look
  //  like overhead. But it avoids issues with duplicate selections of the same
  //  instance which may happen when an instance is selected multiple times
  //  through different hierarchy branches.
  ...
  for (std::vector<edt::Service::obj_iterator>::const_iterator p =
           m_selection_ptrs.begin();
       p != m_selection_ptrs.end(); ++p) {
    new_sel.push_back(**p);
  }
  ...
  try {
    ...
    for (auto ii = m_indexes.begin(); ii != m_indexes.end(); ++ii) {
      ...
      //  only update objects from the same layout - this is not practical
      //  limitation but saves a lot of effort for managing different property
      //  id's etc.
      ...
      std::map<db::Instance, db::Instance>::const_iterator i =
          insts_seen.find(pos->back().inst_ptr);
      if (i == insts_seen.end()) {
        ...
        if (!current_only || pos->back().inst_ptr == current) {
          new_inst = applicator->do_apply_inst(cell, pos->back().inst_ptr, dbu,
                                               relative_mode);
        }
...
void InstPropertiesPage::apply() { do_apply(true, false); }

bool InstPropertiesPage::can_apply_to_all() const {
  return m_selection_ptrs.size() > 1;
}

void InstPropertiesPage::apply_to_all(bool relative) {
...
void InstPropertiesPage::update_pcell_parameters() {
  ...
  get_cell_or_pcell_ids_by_name(layout, tl::to_string(cell_name_le->text()), cc,
                                pc);
  ...
  if (pc.first && layout->pcell_declaration(pc.second)) {
    ...
    const db::PCellDeclaration *pcell_decl =
        layout->pcell_declaration(pc.second);
    ...
    db::Cell &cell = cv->layout().cell(pos->cell_index());
    std::pair<bool, db::pcell_id_type> pci =
        cell.is_pcell_instance(pos->back().inst_ptr);
    const db::Library *pci_lib =
        cv->layout().defining_library(pos->back().inst_ptr.cell_index()).first;

    //  fetch the parameters of the current instance if it matches the selected
    //  PCell and in that case use the current parameters
    if (pci.first) {
      if (pci.second == pc.second && pci_lib == lib_cbx->current_library()) {
        ...
        //  otherwise: update the parameter whose name matches, use default for
        //  others
        parameters = pcell_decl->map_parameters(
            cell.get_named_pcell_parameters(pos->back().inst_ptr));
      }
    }

    if (mp_pcell_parameters &&
        mp_pcell_parameters->pcell_decl() == pcell_decl) {

      //  For identical parameter declarations just set the new values
      //  TODO: formally the declaration could change - compare current
      //  declarations vs. new ones? Better: provide a way to install custom
      //  parameter pages ...
      mp_pcell_parameters->set_parameters(parameters);

    } else {

      //  Hint: we shall not delete the page immediately. This gives a
      ...
      mp_pcell_parameters =
          new PCellParametersPage(pcell_tab, mp_service->view()->dispatcher());
      connect(mp_pcell_parameters, SIGNAL(edited()), this, SIGNAL(edited()));
      mp_pcell_parameters->setup(mp_service->view(), pos->cv_index(),
                                 layout->pcell_declaration(pc.second),
                                 parameters);
