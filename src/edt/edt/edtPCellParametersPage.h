 ...
 */
class PCellParametersPage : public QFrame, public tl::Object {
  Q_OBJECT
...
public:
   ...
   */
  PCellParametersPage(QWidget *parent, lay::Dispatcher *dispatcher,
                      bool dense = false);

  /**
   *  @brief initialization
   *
   *  Use this method to setup when the arguments are not available in the
   * constructor
   *
   ...
   *  @param parameters The parameter values to show (if empty, the default
   * values are used)
   */
  void setup(lay::LayoutViewBase *view, int cv_index,
   ...
   *
   *  *ok is set to true, if there is no error. In case of an error it's set to
   * false. The error is indicated in the error label in the editor page. If ok
   ...
   * is null, an exception is thrown.
   *
   *  The value fields of the ParameterState members is set to the parameter
   * value. The other attributes are not changed. Parameters not present inside
   * the ParameterStates object are created with their corresponding name.
   */
   ...
   */
  const db::PCellDeclaration *pcell_decl() const { return mp_pcell_decl.get(); }
...
private:
  ...
  void set_parameters_internal(const db::ParameterStates &states,
                               bool tentatively);
  ...
  parameter_from_states(const db::ParameterStates &states) const;
  void states_from_parameters(db::ParameterStates &states,
