...
void RecentConfigurationPage::init() {
  ...
  connect(mp_tree_widget, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
          SLOT(item_clicked(QTreeWidgetItem *)));
  view()->layer_list_changed_event.add(
      this, &RecentConfigurationPage::layers_changed);

  mp_tree_widget->setColumnCount(int(m_cfg.size()));

  QStringList column_labels;
  for (std::list<ConfigurationDescriptor>::const_iterator c = m_cfg.begin();
       c != m_cfg.end(); ++c) {
...
int RecentConfigurationPage::order() const { return 100; }
...
RecentConfigurationPage::get_stored_values() const {
  ...
  try {
    ...
    tl::error << tl::to_string(tr("Error reading configuration item "))
              << m_recent_cfg_name << ": " << ex.msg();
    values.clear();
  }
...
void RecentConfigurationPage::set_stored_values(
    const std::list<std::vector<std::string>> &values) const {
  std::string serialized_list;
  for (std::list<std::vector<std::string>>::const_iterator v = values.begin();
       v != values.end(); ++v) {
    if (v != values.begin()) {
      serialized_list += ";";
    }
    for (std::vector<std::string>::const_iterator s = v->begin(); s != v->end();
         ++s) {
...
void RecentConfigurationPage::render_to(
    QTreeWidgetItem *item, int column, const std::vector<std::string> &values,
  ...
  case RecentConfigurationPage::Bool: {
    ...
    try {
      ...
      tl::error << tl::to_string(tr("Configuration error (ArrayFlag/Bool): "))
                << ex.msg();
    }
  ...
  case RecentConfigurationPage::Layer: {
    int icon_size =
        item->treeWidget()->style()->pixelMetric(QStyle::PM_ButtonIconSize);
    lay::LayerPropertiesConstIterator l;
    try {
      l = lp_iter_from_string(view(), values[column]);
    } catch (tl::Exception &ex) {
      tl::error << tl::to_string(tr("Configuration error (Layer): "))
                << ex.msg();
    }
    if (!l.is_null() && !l.at_end()) {
      item->setIcon(column, lay::LayerTreeModel::icon_for_layer(
                                l, view(), icon_size, icon_size, dpr, 0, true));
  ...
  case RecentConfigurationPage::DoubleIfArray: {
    ...
    int flag_column = 0;
    for (std::list<ConfigurationDescriptor>::const_iterator c = m_cfg.begin();
         c != m_cfg.end(); ++c, ++flag_column) {
      if (c->rendering == RecentConfigurationPage::ArrayFlag) {
        try {
          tl::from_string(values[flag_column], is_array);
        } catch (tl::Exception &ex) {
          tl::error << tl::to_string(tr(
                           "Configuration error (IntIfArray/DoubleIfArray): "))
                    << ex.msg();
        }
        break;
      }
  ...
  case RecentConfigurationPage::CellDisplayName: {
      ...
      if (c->rendering == RecentConfigurationPage::CellLibraryName) {
        if (view()->active_cellview().is_valid()) {
          lib = db::LibraryManager::instance().lib_ptr_by_name(
              values[libname_column], view()->active_cellview()->tech_name());
        } else {
          lib = db::LibraryManager::instance().lib_ptr_by_name(
    ...
    if (lib) {
      ...
      std::pair<bool, db::Layout::pcell_id_type> pcid =
          lib->layout().pcell_by_name(values[column].c_str());
      if (pcid.first) {
        const db::PCellDeclaration *pc_decl =
            lib->layout().pcell_declaration(pcid.second);
        if (pc_decl) {
          item->setText(column, tl::to_qstring(pc_decl->get_display_name(
                                    pc_decl->map_parameters(pcp))));
          break;
        }
      }
    }
  ...
  case RecentConfigurationPage::PCellParameters: {
    ...
    try {
      ...
      tl::error << tl::to_string(tr("Configuration error (PCellParameters): "))
                << ex.msg();
    }
    std::string r;
    for (std::map<std::string, tl::Variant>::const_iterator p = pcp.begin();
         p != pcp.end(); ++p) {
      if (p != pcp.begin()) {
        r += ",";
      }
...
void RecentConfigurationPage::layers_changed(int) { dm_update_list(); }
...
void RecentConfigurationPage::update_list(
    const std::list<std::vector<std::string>> &stored_values) {
  int row = 0;
  for (std::list<std::vector<std::string>>::const_iterator v =
           stored_values.begin();
       v != stored_values.end(); ++v, ++row) {
...
static bool set_or_request_current_layer(QWidget *parent,
  ...
  if (QMessageBox::question(parent, tr("Create Layer"),
                            tr("Layer %1 does not exist yet. Create it now?")
                                .arg(tl::to_qstring(lp.to_string()))) ==
      QMessageBox::Yes) {
    ...
    view->set_current_layer(lay::LayerPropertiesConstIterator(
        &view->insert_layer(view->end_layers(), lpn)));
    view->commit();
...
void RecentConfigurationPage::commit_recent(lay::Dispatcher *root) {
  ...
  for (std::list<ConfigurationDescriptor>::const_iterator c = m_cfg.begin();
    ...
    if (c->rendering == Layer) {
      ...
      if (!(view()->current_layer().is_null() ||
            view()->current_layer().at_end()) &&
          view()->current_layer()->is_visual()) {
  ...
  for (std::list<std::vector<std::string>>::iterator v = stored_values.begin();
       v != stored_values.end(); ++v) {
    if (*v == values) {
