...
MainService::MainService(db::Manager *manager, lay::LayoutViewBase *view,
                         lay::Dispatcher *root)
    : lay::Plugin(view), lay::Editable(view), db::Object(manager),
      mp_view(view), mp_root(root), m_needs_update(false),
      m_flatten_insts_levels(std::numeric_limits<int>::max()),
      m_flatten_prune(false), m_align_hmode(0), m_align_vmode(0),
      m_align_visible_layers(false), m_hdistribute(true), m_distribute_hmode(1),
      m_distribute_hpitch(0.0), m_distribute_hspace(0.0), m_vdistribute(true),
      m_distribute_vmode(1), m_distribute_vpitch(0.0), m_distribute_vspace(0.0),
      m_distribute_visible_layers(false), m_origin_mode_x(-1),
      m_origin_mode_y(-1), m_origin_visible_layers_for_bbox(false),
      m_array_a(0.0, 1.0), m_array_b(1.0, 0.0), m_array_na(1), m_array_nb(1),
...
edt::RoundCornerOptionsDialog *MainService::round_corners_dialog() {
  if (!mp_round_corners_dialog) {
    mp_round_corners_dialog =
        new edt::RoundCornerOptionsDialog(lay::widget_from_view(view()));
  }
  return mp_round_corners_dialog;
}

edt::AreaAndPerimeterDialog *MainService::area_and_perimeter_dialog() {
  if (!mp_area_and_perimeter_dialog) {
    mp_area_and_perimeter_dialog =
        new edt::AreaAndPerimeterDialog(lay::widget_from_view(view()));
  }
  return mp_area_and_perimeter_dialog;
}

edt::AlignOptionsDialog *MainService::align_options_dialog() {
  if (!mp_align_options_dialog) {
    mp_align_options_dialog =
        new edt::AlignOptionsDialog(lay::widget_from_view(view()));
  }
  return mp_align_options_dialog;
}

edt::DistributeOptionsDialog *MainService::distribute_options_dialog() {
  if (!mp_distribute_options_dialog) {
    mp_distribute_options_dialog =
        new edt::DistributeOptionsDialog(lay::widget_from_view(view()));
  }
  return mp_distribute_options_dialog;
}

lay::FlattenInstOptionsDialog *MainService::flatten_inst_options_dialog() {
  if (!mp_flatten_inst_options_dialog) {
    mp_flatten_inst_options_dialog = new lay::FlattenInstOptionsDialog(
        lay::widget_from_view(view()), false /*don't allow pruning*/);
  }
  return mp_flatten_inst_options_dialog;
}

edt::MakeCellOptionsDialog *MainService::make_cell_options_dialog() {
  if (!mp_make_cell_options_dialog) {
    mp_make_cell_options_dialog =
        new edt::MakeCellOptionsDialog(lay::widget_from_view(view()));
  }
  return mp_make_cell_options_dialog;
}

edt::MakeArrayOptionsDialog *MainService::make_array_options_dialog() {
  if (!mp_make_array_options_dialog) {
    mp_make_array_options_dialog =
        new edt::MakeArrayOptionsDialog(lay::widget_from_view(view()));
  }
...
/**
 *  @brief A helper class to determine the common part of a set of instance
 * elements
 */
class CommonInsts {
public:
  ...
  void add(const lay::ObjectInstPath &path, unsigned int n) {
    if (!m_valid) {
      ...
      unsigned int nn = 0;
      for (lay::ObjectInstPath::iterator p = path.begin();
           p != path.end() && nn < n; ++nn) {
  ...
  bool anything() const { return !m_first; }

  bool ambiguous() const { return m_ambiguous && m_common_inst.empty(); }

  bool empty() const { return m_common_inst.empty(); }

  unsigned int size() const { return (unsigned int)m_common_inst.size(); }
...
void MainService::cm_descend() {
  ...
  std::vector<edt::Service *> edt_services =
      view()->get_plugins<edt::Service>();
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end() && common_inst.valid(); ++es) {
    for (edt::Service::objects::const_iterator sel = (*es)->selection().begin();
         sel != (*es)->selection().end() && common_inst.valid(); ++sel) {
  ...
  if (!common_inst.anything()) {
    throw tl::Exception(tl::to_string(
        tr("Select an object to determine into which instance to descend")));
  }
  if (!common_inst.valid() || common_inst.ambiguous()) {
    throw tl::Exception(
        tl::to_string(tr("Selection is ambiguous - cannot determine into which "
                         "instance to descend")));
  }
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::objects::const_iterator sel = (*es)->selection().begin();
      ...
      //  it may happen that this way we dive into the instance selected. The
      //  resulting selection is not pointing to any instance any more and must
      //  be discarded therefore:
  ...
  view()->set_current_cell_path(
      common_inst.cv_index(),
      view()->cellview(common_inst.cv_index()).combined_unspecific_path());

  //  set the new selections
  unsigned int index = 0;
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es, ++index) {
    (*es)->set_selection(new_selections[index].begin(),
                         new_selections[index].end());
  }
}
...
void MainService::cm_ascend() {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es) {
    new_selections.push_back(std::vector<lay::ObjectInstPath>());
    new_selections.back().insert(new_selections.back().end(),
                                 (*es)->selection().begin(),
                                 (*es)->selection().end());
  }
  ...
  for (int cv_index = 0; cv_index < int(view()->cellviews()); ++cv_index) {
    ...
    if (removed != db::InstElement()) {
      ...
      view()->set_current_cell_path(
          cv_index, view()->cellview(cv_index).combined_unspecific_path());

      //  create and the new selections
      unsigned int index = 0;
      for (std::vector<edt::Service *>::const_iterator es =
               edt_services.begin();
           es != edt_services.end(); ++es, ++index) {
        for (std::vector<lay::ObjectInstPath>::iterator sel =
                 new_selections[index].begin();
             sel != new_selections[index].end(); ++sel) {
...
void MainService::cm_flatten_insts() {
  ...
  if (!(flatten_inst_options_dialog()->exec_dialog(m_flatten_insts_levels,
                                                   m_flatten_prune) &&
        m_flatten_insts_levels != 0)) {
    return;
  }
#endif
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::objects::const_iterator r = (*es)->selection().begin();
         r != (*es)->selection().end(); ++r) {
      ...
      if (cv.is_valid()) {
        ...
        if (r->is_cell_inst() && target_cell.is_valid(r->back().inst_ptr)) {
          ...
          //  because we select whole arrays in editable mode, we can iterator
          //  over them
          db::CellInstArray cell_inst = r->back().inst_ptr.cell_inst();
          for (db::CellInstArray::iterator a = cell_inst.begin(); !a.at_end();
               ++a) {
            cv->layout().flatten(cv->layout().cell(r->cell_index_tot()),
                                 target_cell, cell_inst.complex_trans(*a),
                                 m_flatten_insts_levels < 0
                                     ? m_flatten_insts_levels
                                     : m_flatten_insts_levels - 1);
          }
  ...
  for (std::set<db::Layout *>::const_iterator l = needs_cleanup.begin();
       l != needs_cleanup.end(); ++l) {
    (*l)->cleanup();
  }
    ...
    for (edt::Service::objects::const_iterator r = (*es)->selection().begin();
      ...
      if (cv.is_valid()) {
        ...
        if (r->is_cell_inst()) {
          ...
          new_inst =
              target_cell.transform(new_inst, db::ICplxTrans(r->trans()));
          ...
          new_selection.back().add_path(
              db::InstElement(new_inst, db::CellInstArray::iterator()));
          ...
          new_shape =
              target_shapes.transform(new_shape, db::ICplxTrans(r->trans()));
 ...
 *
 *  The purpose of this class is to implement instance resolution for variant
 * building. Resolution means converting to an instance of a different cell. A
 * normal (single) instance is easy to convert: a new instance is created and
 * the cell index set to point to the new one. For an array instance however
 * that is more complicated. Since the instance to resolve is usually just a
 * part of that array, it is required to split the original array and create new
 * instances for the parts. These parts must be maintained, because one task of
 * the resolver is to map other array members of the original array to new
 * instances. In that case, the resolver has to look for a suitable piece and
 * return an array member instance to that one.
 */
class ArrayResolver {
public:
  /**
   ...
   *  The new instance is simply taken as the given one. This can be used to map
   * array instances unconditionally or for mapping single instances.
   */
  ArrayResolver(const db::Instance &new_inst)
      : m_new_inst(new_inst), m_na_before(0), m_na_after(0), m_nb_before(0),
        m_nb_after(0) {
    //  .. nothing yet ..
  }

  /**
   *  @brief Resolve the given instance (elem) to a new one with the given new
   * cell (new_cell_index) This method will resolve elem, create a corresponding
   * instance with the new cell index. If necessary, new instances for array
   * pieces are created (see above) and inserted into the cell. The resolver
   * will map other instances to these pieces when necessary in the "resolve"
   * function.
   */
  void resolve(db::Cell &parent_cell, const db::InstElement &elem,
               db::cell_index_type new_cell_index) {
    ...
    if (ia >= 0 && ia < long(na) && ib >= 0 && ib <= long(nb)) {
      ...
      if (m_na_before > 0) {
        ...
        if (elem.inst_ptr.is_complex()) {
          ia = db::CellInstArray(db::CellInst(org_cell_index),
                                 elem.inst_ptr.complex_trans(), a, b,
                                 m_na_before, nb);
        } else {
          ia = db::CellInstArray(db::CellInst(org_cell_index),
                                 elem.inst_ptr.front(), a, b, m_na_before, nb);
        }
      ...
      if (m_na_after > 0) {
        ...
        if (elem.inst_ptr.is_complex()) {
          ia = db::CellInstArray(db::CellInst(org_cell_index),
                                 elem.inst_ptr.complex_trans() *
                                     db::CellInstArray::complex_trans_type(
                                         a * long(m_na_before + 1)),
                                 a, b, m_na_after, nb);
        } else {
          ia = db::CellInstArray(
              db::CellInst(org_cell_index),
              elem.inst_ptr.front() *
                  db::CellInstArray::trans_type(a * long(m_na_before + 1)),
              a, b, m_na_after, nb);
        }
      ...
      if (m_nb_before > 0) {
        ...
        if (elem.inst_ptr.is_complex()) {
          ia = db::CellInstArray(
              db::CellInst(org_cell_index),
              elem.inst_ptr.complex_trans() *
                  db::CellInstArray::complex_trans_type(a * long(m_na_before)),
              a, b, 1, m_nb_before);
        } else {
          ia = db::CellInstArray(
              db::CellInst(org_cell_index),
              elem.inst_ptr.front() *
                  db::CellInstArray::trans_type(a * long(m_na_before)),
              a, b, 1, m_nb_before);
        }
      ...
      if (m_nb_after > 0) {
        ...
        if (elem.inst_ptr.is_complex()) {
          ia = db::CellInstArray(
              ...
              elem.inst_ptr.complex_trans() *
                  db::CellInstArray::complex_trans_type(
                      a * long(m_na_before) + b * long(m_nb_before + 1)),
              a, b, 1, m_nb_after);
        } else {
          ia = db::CellInstArray(
              db::CellInst(org_cell_index),
   ...
   *  "elem" is the original instance which should be resolved. The instance
   * part of elem must be identical to the one originally passed to "resolve" or
   * the plain instance resolution constructor, but the array iterator part may
   * be different. In the latter case, a suitable array piece is selected.
   */
    ...
    if (ia >= 0 && ib >= 0 /* we have an array member */) {
      ...
      if (ia == long(m_na_before) && ib == long(m_nb_before)) {
        ...
        return db::InstElement(
            m_inst_n, m_inst_n.cell_inst().begin(0, ib - m_nb_before - 1));
      } else {
        return db::InstElement(
            m_inst_e, m_inst_e.cell_inst().begin(ia - m_na_before - 1, ib));
      }
...
void MainService::cm_make_cell_variants() {
  ...
  //  TODO: this limitation is not really necessary, but makes the code somewhat
  //  simpler
  int cv_index = -1;
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es) {
    for (edt::Service::objects::const_iterator r = (*es)->selection().begin();
      ...
      if (cv_index < 0) {
        ...
        throw tl::Exception(
            tl::to_string(tr("The selection must not contain objects from "
                             "different layouts for 'make cell variants'")));
      }
    }
  }
  ...
  if (manager()) {
    manager()->transaction(
        tl::to_string(tr("Make cell variants for selection")));
  }

  std::vector<lay::ObjectInstPath> new_selection;
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es) {
    new_selection.insert(new_selection.end(), (*es)->selection().begin(),
                         (*es)->selection().end());
  }

  size_t num_sel = new_selection.size();

  //  TODO: the algorithm is basically O(2) in the number of selected items. A
  //  first step to mitigate that problem is to provide a progress and hence a
  //  way to cancel it.
  tl::RelativeProgress progress(
      tl::to_string(tr("Make cell variants for selection")), num_sel, 1);
  ...
  for (size_t nsel = 0; nsel < num_sel; ++nsel) {
    ...
    //  A map for a part of the selection path to a new instance (value.first is
    //  the next original cell of the path and value.second the ArrayResolver
    //  which can be used to fetch the corresponding new instance in the new
    //  target cell).
    std::map<std::pair<db::cell_index_type, db::Instance>,
             std::pair<db::cell_index_type, ArrayResolver>>
        new_instances;
    ...
    for (lay::ObjectInstPath::iterator p = selection[nsel].begin();
         p != selection[nsel].end() &&
         !layout.cell(p->inst_ptr.cell_index()).is_proxy();
         ++p) {
    ...
    if (!path.empty()) {
      ...
      //  create variants for each part of the path if required. While doing so,
      //  store information about the mapping to the new path in new_instances.
      for (std::vector<std::pair<db::cell_index_type, db::InstElement>>::
               const_iterator p = path.begin();
           p != path.end(); ++p) {

        db::Cell &parent_cell = layout.cell(parent_cell_index);
        db::Cell &org_cell = layout.cell(elem.inst_ptr.cell_index());

        //  if the selection is concerning a single instance of an array, we
        //  always need to create variants.
        if (elem.inst_ptr.cell_inst().size() > 1) {
          needs_variant = true;
        }

        if (!needs_variant) {
          //  needs a variant if more than one instance of it exists and then
          //  all child cells need a variant as well.
        ...
        if (needs_variant) {
          ...
          db::cell_index_type new_cell_index =
              layout.add_cell(layout, elem.inst_ptr.cell_index());

          //  prepare a new variant cell
          db::Cell &new_cell = layout.cell(new_cell_index);

          //  copy the shapes
          for (db::Layout::layer_iterator l = layout.begin_layers();
               l != layout.end_layers(); ++l) {
            //  insert shape by shape (the container assignment does not support
            //  undo currently)
            db::Shapes &target = new_cell.shapes((*l).first);
            for (db::ShapeIterator s =
                     org_cell.shapes((*l).first).begin(db::ShapeIterator::All);
                 !s.at_end(); ++s) {
              target.insert(*s);
            }
          }
          ...
          for (db::Cell::const_iterator i = org_cell.begin(); !i.at_end();
               ++i) {
            ...
            if (p + 1 != path.end() && p[1].second.inst_ptr == *i) {
              ...
              new_instances.insert(std::make_pair(
                  std::make_pair(org_cell.cell_index(), *i),
                  std::make_pair(i->cell_index(), ArrayResolver(ni))));
            }
          }

          db::cell_index_type next_org = p->second.inst_ptr.cell_index();

          //  Resolve the original instance into a new one (and split arrays
          //  while doing so)
          new_instances
              .insert(
                  std::make_pair(std::make_pair(p->first, p->second.inst_ptr),
                                 std::make_pair(next_org, ArrayResolver())))
              .first->second.second.resolve(parent_cell, elem, new_cell_index);
    ...
    for (std::vector<lay::ObjectInstPath>::const_iterator r = selection.begin();
         r != selection.end(); ++r) {
      ...
      for (lay::ObjectInstPath::iterator p = r->begin(); p != r->end(); ++p) {
        std::map<std::pair<db::cell_index_type, db::Instance>,
                 std::pair<db::cell_index_type, ArrayResolver>>::const_iterator
            ni = new_instances.end();
        if (needs_translate) {
        ...
        if (ni != new_instances.end()) {
          ...
          //  resolve may fold the path back to the original one if a side piece
          //  of an array instance was taken - stop translating in that case
          //  now.
          ...
          needs_translate =
              false; // stop translating the path in the first original cell
        }
      }

      if (!r->is_cell_inst()) {
        //  map the shape as well. Note that "cell" is the original cell where
        //  the shape came from.
        new_path.set_layer(r->layer());
        if (new_path.begin() != new_path.end() &&
            cell != new_path.back().inst_ptr.cell_index()) {
          db::Cell &new_cell =
              layout.cell(new_path.back().inst_ptr.cell_index());
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    if (n + i0 <= new_selection.size()) {
      (*es)->set_selection(new_selection.begin() + i0,
                           new_selection.begin() + i0 + n);
    }
    i0 += n;
  }
...
void MainService::cm_resolve_arefs() {
  ...
  for (edt::Service::objects::const_iterator r =
           inst_service->selection().begin();
       r != inst_service->selection().end(); ++r) {
    if (r->is_cell_inst() && r->back().inst_ptr.size() > 1) {
      if (cv_index < 0) {
        cv_index = r->cv_index();
      } else if (cv_index != int(r->cv_index())) {
        //  TODO: this limitation is not really necessary, but makes the code
        //  somewhat simpler
        throw tl::Exception(tl::to_string(
            tr("The selection must not contain objects from different layouts "
               "for 'resolve array references'")));
      }
  ...
  for (std::vector<lay::ObjectInstPath>::const_iterator p =
           insts_to_resolve.begin();
       p != insts_to_resolve.end(); ++p) {
    ...
    for (db::CellInstArray::iterator ai = aa.begin(); !ai.at_end(); ++ai) {
      ...
      if (p->back().inst_ptr.has_prop_id()) {
        new_inst = parent_cell.insert(
            db::CellInstArrayWithProperties(na, p->back().inst_ptr.prop_id()));
      } else {
...
void MainService::cm_make_cell() {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::objects::const_iterator r = (*es)->selection().begin();
      ...
      if (cv_index < 0) {
        ...
        throw tl::Exception(
            tl::to_string(tr("The selection must not contain objects from "
                             "different layouts for 'make cell'")));
      }
    }
  }
  ...
  if (cv_index >= 0) {
...
#if defined(HAVE_QT)
    //  TODO: make parameters persistent so we can set them externally
    if (!make_cell_options_dialog()->exec_dialog(
            cv->layout(), m_make_cell_name, m_origin_mode_x, m_origin_mode_y)) {
      return;
    }
#endif
    ...
    if (selection_bbox.empty()) {
      throw tl::Exception(
          tl::to_string(tr("The selection is empty. Cannot create a cell from "
                           "an empty selection.")));
    }
    ...
    db::cell_index_type target_ci =
        cv->layout().add_cell(m_make_cell_name.c_str());
    //  create target cell
    ...
    if (m_origin_mode_x >= -1) {
      ref =
          db::Vector(selection_bbox.left() +
                         ((m_origin_mode_x + 1) * selection_bbox.width()) / 2,
                     selection_bbox.bottom() +
                         ((m_origin_mode_y + 1) * selection_bbox.height()) / 2);
    }

    db::Instance target_cell_inst = cv.cell()->insert(
        db::CellInstArray(db::CellInst(target_ci), db::Trans(ref)));
...
void MainService::cm_convert_to_cell() {
  ...
  try {
    ...
    for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
      ...
      for (edt::Service::obj_iterator s = (*es)->selection().begin();
           s != (*es)->selection().end(); ++s) {
        ...
        if (cv.is_valid() && s->is_cell_inst() &&
            cv->layout().cell(ci).is_proxy()) {

          db::Cell &parent = cv->layout().cell(parent_ci);
          if (parent.is_valid(s->back().inst_ptr)) {

            //  convert the cell to static and replace the instances with the
            //  new cell
            db::cell_index_type new_ci =
                cv->layout().convert_cell_to_static(ci);
            if (new_ci != ci) {
    ...
    if (needs_cleanup.empty()) {
      throw tl::Exception(
          tl::to_string(tr("No instance of a PCell or library cell selected - "
                           "nothing to convert")));
    }
...
void MainService::cm_convert_to_pcell() {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (s->is_cell_inst()) {
        throw tl::Exception(
            tl::to_string(tr("Selection contains instances - they cannot be "
                             "converted to PCells.")));
      }
    }
  }
  ...
  //  Collect the libraries and PCells within these libraries that are
  //  candidates here
  std::vector<std::pair<db::Library *, db::pcell_id_type>> pcells;
  for (db::LibraryManager::iterator l = db::LibraryManager::instance().begin();
       l != db::LibraryManager::instance().end(); ++l) {

    db::Library *lib = db::LibraryManager::instance().lib(l->second);
    for (db::Layout::pcell_iterator pc = lib->layout().begin_pcells();
         pc != lib->layout().end_pcells(); ++pc) {

      try {

        const db::PCellDeclaration *pc_decl =
            lib->layout().pcell_declaration(pc->second);
        size_t n = 1000; // 1000 tries max.
        for (std::vector<edt::Service *>::const_iterator es =
                 edt_services.begin();
             n > 0 && pc_decl && es != edt_services.end(); ++es) {
          for (edt::Service::obj_iterator s = (*es)->selection().begin();
               n > 0 && pc_decl && s != (*es)->selection().end(); ++s) {
            const lay::CellView &cv = view()->cellview(s->cv_index());
            if (pc_decl->can_create_from_shape(cv->layout(), s->shape(),
                                               s->layer())) {
              --n;
            } else {
              pc_decl = 0; // stop
  ...
  if (pcell_items.empty()) {
    throw tl::Exception(tl::to_string(
        tr("No PCell found that accepts the selected shapes for conversion.")));
  }

  int index = 0;

#if defined(HAVE_QT)
  //  TODO: In Qt-less case keep selection persistent so we can set it
  //  externally
  QStringList items;
  ...
  QString item = QInputDialog::getItem(
      lay::widget_from_view(view()), tr("Select Target PCell"),
      tr("Select the PCell the shape should be converted into"), items, 0,
      false, &ok);
  if (!ok) {
    return;
  }
  ...
  const db::PCellDeclaration *pcell_decl =
      lib->layout().pcell_declaration(pcid);
  ...
  try {
    ...
    {
      tl::RelativeProgress progress(tl::to_string(tr("Convert to PCell")),
                                    num_selected, 1000);
      ...
      for (std::vector<edt::Service *>::const_iterator es =
        ...
        for (edt::Service::obj_iterator s = (*es)->selection().begin();
          ...
          if (!s->is_cell_inst()) {
            ...
            if (pcell_decl->can_create_from_shape(cv->layout(), s->shape(),
                                                  s->layer())) {

              db::pcell_parameters_type parameters =
                  pcell_decl->parameters_from_shape(cv->layout(), s->shape(),
                                                    s->layer());
              db::Trans trans = pcell_decl->transformation_from_shape(
                  cv->layout(), s->shape(), s->layer());

              pcell_decl->coerce_parameters(cv->layout(), parameters);
              db::cell_index_type pcell_cid =
                  lib->layout().get_pcell_variant(pcid, parameters);
              db::cell_index_type cid =
                  cv->layout().get_lib_proxy(lib, pcell_cid);

              db::Instance cell_inst = cv.cell()->insert(
                  db::CellInstArray(db::CellInst(cid), trans));
    ...
    if (!any_converted) {
      throw tl::Exception(tl::to_string(
          tr("None of the shapes could be converted to the desired PCell")));
    }

    //  Delete the shapes which have been converted
    for (std::vector<edt::Service::obj_iterator>::const_iterator td =
             to_delete.begin();
         td != to_delete.end(); ++td) {
      db::Cell &cell = view()
                           ->cellview((*td)->cv_index())
                           ->layout()
                           .cell((*td)->cell_index());
    ...
    if (any_non_converted) {
      tl::warn << tl::to_string(
          tr("Some of the shapes could not be converted to the desired PCell"));
#if defined(HAVE_QT)
      QMessageBox::warning(
          lay::widget_from_view(view()), tr("Warning"),
          tr("Some of the shapes could not be converted to the desired PCell"));
#endif
    }
...
void MainService::cm_area_perimeter() {
  ...
  if (region.count() > 100000) {
    if (QMessageBox::warning(
            lay::widget_from_view(view()), tr("Warning: Big Selection"),
            tr("The selection contains many shapes. Area and perimeter "
...
static bool extract_rad(std::vector<db::Polygon> &poly, double &rinner,
                        double &router, unsigned int &n) {
  std::vector<db::Point> new_pts;
  bool any_extracted = false;

  for (std::vector<db::Polygon>::iterator p = poly.begin(); p != poly.end();
       ++p) {

    db::Polygon new_poly;

    new_pts.clear();
    if (!extract_rad_from_contour(p->begin_hull(), p->end_hull(), rinner,
                                  router, n, &new_pts) &&
        !extract_rad_from_contour(p->begin_hull(), p->end_hull(), rinner,
                                  router, n, &new_pts, true)) {
      //  ultimate fallback: assign original contour
      new_poly.assign_hull(p->begin_hull(), p->end_hull(),
    ...
    for (unsigned int h = 0; h < p->holes(); ++h) {
      ...
      new_pts.clear();
      if (!extract_rad_from_contour(p->begin_hole(h), p->end_hole(h), rinner,
                                    router, n, &new_pts) &&
          !extract_rad_from_contour(p->begin_hole(h), p->end_hole(h), rinner,
                                    router, n, &new_pts, true)) {
        //  ultimate fallback: assign original contour
        new_poly.insert_hole(p->begin_hole(h), p->end_hole(h),
                             false /*don't compress*/);
      } else {
...
void MainService::cm_round_corners() {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (!s->is_cell_inst() && (s->shape().is_polygon() ||
                                 s->shape().is_path() || s->shape().is_box())) {

        if (cv_index >= 0 && cv_index != int(s->cv_index())) {
          throw tl::Exception(
              tl::to_string(tr("Selection originates from different layouts - "
                               "cannot compute result in this case.")));
        }
        cv_index = int(s->cv_index());

        if (layer_index >= 0 && layer_index != int(s->layer())) {
          throw tl::Exception(
              tl::to_string(tr("Selection originates from different layers - "
                               "cannot compute result in this case.")));
        }
  ...
  if (cv_index < 0 || layer_index < 0) {
    throw tl::Exception(
        tl::to_string(tr("Selection does not contain polygons")));
  }

  //  prepare: merge to remove cutlines and smooth to remove effects of cutlines
  db::EdgeProcessor ep;
  std::vector<db::Polygon> in;
  ep.merge(primary, in, 0 /*min_wc*/, false /*resolve holes*/,
           true /*min coherence*/);
...
#if defined(HAVE_QT)
  //  TODO: make parameters persistent so we can set them externally
  if (!round_corners_dialog()->exec_dialog(cv->layout(), m_router, m_rinner,
                                           m_npoints, m_undo_before_apply,
                                           router, rinner, n, has_extracted)) {
    return;
  }
#endif
  ...
  for (std::vector<db::Polygon>::iterator p = out.begin(); p != out.end();
       ++p) {
  ...
  primary.clear();
  ep.merge(out, primary, 0 /*min_wc*/, true /*resolve holes*/,
           true /*min coherence*/);

  view()->cancel_edits();
  if (manager()) {
    manager()->transaction(
        tl::to_string(tr("Corner rounding operation on selection")));
  }
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (!s->is_cell_inst() && (s->shape().is_polygon() ||
                                 s->shape().is_path() || s->shape().is_box())) {
        db::Cell &cell =
            view()->cellview(s->cv_index())->layout().cell(s->cell_index());
  ...
  db::Shapes &target_shapes =
      cv->layout().cell(cv.cell_index()).shapes(layer_index);
  for (std::vector<db::Polygon>::const_iterator p = primary.begin();
       p != primary.end(); ++p) {
  ...
  //  set the new selection on the polygon service (because now we have
  //  polygons)
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es) {
...
void MainService::cm_size() {
...
#if defined(HAVE_QT)
  //  TODO: keep the value persistent so we can set it externally in the Qt-less
  //  case
  bool ok = false;
  QString s =
      QInputDialog::getText(lay::widget_from_view(view()), tr("Sizing"),
                            tr("Sizing (in micron, positive or negative). Two "
                               "values (dx, dy) for anisotropic sizing."),
                            QLineEdit::Normal, QString::fromUtf8("0.0"), &ok);
  ...
  db::EdgeProcessor ep;
  ep.size(primary, idx, idy, out, 2 /*mode, TODO: make variable*/,
          true /*resolve holes*/, true /*min coherence*/);
  ...
  db::Shapes &target_shapes =
      cv->layout().cell(cv.cell_index()).shapes(layer_index);
  for (std::vector<db::Polygon>::const_iterator p = out.begin(); p != out.end();
       ++p) {
    db::Shape new_shape = target_shapes.insert(*p);
    new_selection.push_back(lay::ObjectInstPath());
    new_selection.back().set_topcell(cv.cell_index());
...
void MainService::boolean_op(int mode) {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (s->seq() == 0 && !s->is_cell_inst() &&
          (s->shape().is_polygon() || s->shape().is_path() ||
           s->shape().is_box())) {

        if (cv_index >= 0 && cv_index != int(s->cv_index())) {
          throw tl::Exception(tl::to_string(
              tr("Primary selection originates from different layouts - cannot "
                 "compute result in this case.")));
        }
        cv_index = int(s->cv_index());

        if (layer_index >= 0 && layer_index != int(s->layer())) {
          throw tl::Exception(tl::to_string(
              tr("Primary selection originates from different layers - cannot "
                 "compute result in this case.")));
        }
        layer_index = int(s->layer());
  ...
  if (cv_index < 0 || layer_index < 0) {
    throw tl::Exception(
        tl::to_string(tr("Primary selection does not contain polygons")));
  }
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (s->seq() > 0 && !s->is_cell_inst() &&
        ...
        //  HINT: this transforms the shape without any grid snapping
        //  precautions ..
        secondary.back().transform(db::CplxTrans(dbu_sec / dbu_prim) *
                                   s->trans());
      }
    }
  }
  ...
  //  Delete the current selection
  //  NOTE: we delete only those shapes from the primary layer and keep shapes
  //  from other layers. Let's see whether this heuristics is more accepted.

  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
       es != edt_services.end(); ++es) {
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
         s != (*es)->selection().end(); ++s) {
      if (!s->is_cell_inst() && int(s->layer()) == layer_index &&
          (s->shape().is_polygon() || s->shape().is_path() ||
           s->shape().is_box())) {
        db::Cell &cell =
            view()->cellview(s->cv_index())->layout().cell(s->cell_index());
...
void MainService::cm_union() { boolean_op(db::BooleanOp::Or); }

void MainService::cm_intersection() { boolean_op(db::BooleanOp::And); }

void MainService::cm_difference() { boolean_op(db::BooleanOp::ANotB); }

void MainService::cm_separate() {
  boolean_op(-1); // == separate (And + ANotB)
}

static db::DVector compute_alignment_vector(const db::DBox &prim_box,
                                            const db::DBox &box, int hmode,
                                            int vmode) {
  double dx = 0.0;
  if (hmode == 1) {
...
static db::DBox inst_bbox(const db::CplxTrans &tr, lay::LayoutViewBase *view,
                          int cv_index, const db::InstElement &inst_element,
                          bool visible_only) {
  db::DBox box;
  ...
  if (visible_only) {
    for (lay::LayerPropertiesConstIterator l = view->begin_layers();
         !l.at_end(); ++l) {
...
void MainService::cm_align() {
...
#if defined(HAVE_QT)
  //  TODO: make parameters persistent so we can set them externally
  if (!align_options_dialog()->exec_dialog(m_align_hmode, m_align_vmode,
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (s->seq() == 0) {
        ...
        if (!s->is_cell_inst()) {
          ...
          prim_box += inst_bbox(tr, view(), s->cv_index(), s->back(),
                                m_align_visible_layers);
        }
  ...
  if (!prim_box.empty()) {
    ...
    for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
      ...
      //  create a transformation vector that describes each shape's
      //  transformation
      ...
      for (edt::Service::obj_iterator s = (*es)->selection().begin();
        ...
        if (s->seq() > 0 || !has_secondary) {
          ...
          if (!s->is_cell_inst()) {
            ...
            v = compute_alignment_vector(prim_box, box, m_align_hmode,
                                         m_align_vmode);

          } else {

            db::DBox box = inst_bbox(tr, view(), s->cv_index(), s->back(),
                                     m_align_visible_layers);
            v = compute_alignment_vector(prim_box, box, m_align_hmode,
                                         m_align_vmode);
          }
      ...
      //  use the "transform" method to transform the shapes and instances (with
...
void MainService::cm_distribute() {
...
#if defined(HAVE_QT)
  //  TODO: make parameters persistent so we can set them externally
  if (!distribute_options_dialog()->exec_dialog(
          m_hdistribute, m_distribute_hmode, m_distribute_hpitch,
          m_distribute_hspace, m_vdistribute, m_distribute_vmode,
          m_distribute_vpitch, m_distribute_vspace,
...

  {
    ...
    for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
      ...
      for (edt::Service::obj_iterator s = (*es)->selection().begin();
        ...
        if (!s->is_cell_inst()) {
          ...
          box = inst_bbox(tr, view(), s->cv_index(), s->back(),
                          m_distribute_visible_layers);
        }
    ...
    for (edt::distributed_placer<db::DBox, size_t>::iterator i = placer.begin();
         i != placer.end(); ++i) {
      transformations[i->second] =
          db::DCplxTrans(i->first.p1() - org_boxes[i->second].p1());
    }
  }

  {
    ...
    for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
      ...
      //  transformation
      std::vector<db::DCplxTrans> tv(
          transformations.begin() + objects_for_service[ie].first,
          transformations.begin() + objects_for_service[ie].second);
...
void MainService::cm_make_array() {
  ...
  if (n == 0) {
    throw tl::Exception(
        tl::to_string(tr("Nothing selected to make arrays of")));
  }

#if defined(HAVE_QT)
  //  TODO: make parameters persistent so we can set them externally
  if (!make_array_options_dialog()->exec_dialog(m_array_a, m_array_na,
                                                m_array_b, m_array_nb)) {
    return;
  }
#endif
  ...
  //  No undo support currently - the undo buffering is pretty inefficient right
  //  now.
  if (manager()) {
    if (!has_undo) {
      manager()->clear();
  ...
  tl::RelativeProgress progress(
      tl::to_string(tr("Make array")),
      (size_t(m_array_na) * size_t(m_array_nb) - 1) * n, 1000);
      ...
      if (s->is_cell_inst()) {
        ...
        for (unsigned int ia = 0; ia < m_array_na; ++ia) {
          for (unsigned int ib = 0; ib < m_array_nb; ++ib) {
            ...
            db::DCplxTrans dtrans(m_array_a * double(ia) +
                                  m_array_b * double(ib));
            db::ICplxTrans itrans(db::DCplxTrans(s->trans()).inverted() *
                                  db::DCplxTrans(1.0 / cv->layout().dbu()) *
                                  dtrans * db::DCplxTrans(cv->layout().dbu()) *
                                  db::DCplxTrans(s->trans()));
...
void MainService::cm_tap() {
  ...
  lay::ShapeFinder finder(true /*point mode*/, false /*all hierarchy levels*/,
                          db::ShapeIterator::All, 0);
  ...
  for (lay::ShapeFinder::iterator f = finder.begin(); f != finder.end(); ++f) {
    ...
    if (f->layer() !=
        view()->cellview(f->cv_index())->layout().guiding_shape_layer()) {
      layers_in_selection.insert(std::make_pair(f->cv_index(), f->layer()));
    }
  }

  std::vector<lay::LayerPropertiesConstIterator> tapped_layers;
  for (lay::LayerPropertiesConstIterator lp =
           view()->begin_layers(view()->current_layer_list());
       !lp.at_end(); ++lp) {
    const lay::LayerPropertiesNode *ln = lp.operator->();
    if (layers_in_selection.find(std::make_pair(
            (unsigned int)ln->cellview_index(),
            (unsigned int)ln->layer_index())) != layers_in_selection.end()) {
...
#if defined(HAVE_QT)
  //  TODO: what to do here in Qt-less case? Store results in configuration so
  //  they can be retrieved externally?
  ...
  QPoint mp = view()->canvas()->widget()->mapToGlobal(
      QPoint(mp_local.x(), mp_local.y()));

  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator l =
           tapped_layers.begin();
       l != tapped_layers.end(); ++l) {
    QAction *a = menu->addAction(
        lay::LayerTreeModel::icon_for_layer(*l, view(), icon_size, icon_size,
                                            dpr, 0, true),
        tl::to_qstring(
            (*l)->display_string(view(), true, true /*with source*/)));
  ...
  if (action) {
    ...
    edt::Service *es =
        dynamic_cast<edt::Service *>(view()->canvas()->active_service());
    if (es) {
      es->tap(pt);
    }
  }
...
void MainService::cm_change_layer() {
  ...
  for (SelectionIterator s(view()); !s.at_end(); ++s) {
    if (cv_index >= 0 && cv_index != int(s->cv_index())) {
      throw tl::Exception(
          tl::to_string(tr("Selections originate from different layouts - "
                           "cannot switch layer in this case.")));
    }
    cv_index = int(s->cv_index());
  }
  ...
  if (cv_index >= 0) {
    ...
    //  HINT: the following code is copied from ShapeEditService::get_edit_layer
    //  - it should be at some common place

    lay::LayerPropertiesConstIterator cl = view()->current_layer();
    if (cl.is_null()) {
      throw tl::Exception(
          tl::to_string(tr("Please select a layer first")).c_str());
    }

    if (cv_index != cl->cellview_index()) {
      throw tl::Exception(
          tl::to_string(tr("Shapes cannot be moved to a different layout"))
              .c_str());
    }
    ...
    if (!cv.is_valid()) {
      throw tl::Exception(
          tl::to_string(tr("Please select a cell first")).c_str());
    }
    ...
    if (layer < 0 || !cv->layout().is_valid_layer((unsigned int)layer)) {
      ...
      if (cl->has_children()) {
        throw tl::Exception(
            tl::to_string(tr("Please select a valid drawing layer first"))
                .c_str());
      } else {
    ...
    //  Insert and delete the shape. This exploits the fact, that a shape can be
    //  erased multiple times - this is important since the selection
    //  potentially contains the same shape multiple times.
    for (SelectionIterator s(view()); !s.at_end(); ++s) {
      ...
      if (!s->is_cell_inst() && int(s->layer()) != layer) {
        ...
        //  If the selected object is a PCell instance, and there is exactly one
        //  visible, writable layer type parameter, change this one
        ...
        if (cell.is_valid(inst)) {
          ...
          const db::PCellDeclaration *pcell_decl =
              layout.pcell_declaration_for_pcell_variant(inst.cell_index());
          if (pcell_decl) {

            size_t layer_par_index = 0;
            int n_layer_par = 0;
            for (std::vector<db::PCellParameterDeclaration>::const_iterator d =
                     pcell_decl->parameter_declarations().begin();
                 d != pcell_decl->parameter_declarations().end() &&
                 n_layer_par < 2;
                 ++d) {
              if (d->get_type() == db::PCellParameterDeclaration::t_layer &&
                  !d->is_hidden() && !d->is_readonly()) {
                ++n_layer_par;
                layer_par_index =
                    size_t(d - pcell_decl->parameter_declarations().begin());
              }
            }

            if (n_layer_par == 1) {
              std::vector<tl::Variant> parameters =
                  cell.get_pcell_parameters(inst);
    ...
    throw tl::Exception(
        tl::to_string(tr("Nothing selected to switch layers for")));
  }
}
...
void MainService::check_no_guiding_shapes() {
  ...
  for (std::vector<edt::Service *>::const_iterator es = edt_services.begin();
    ...
    for (edt::Service::obj_iterator s = (*es)->selection().begin();
      ...
      if (!s->is_cell_inst()) {
        if (s->layer() ==
            view()->cellview(s->cv_index())->layout().guiding_shape_layer()) {
          throw tl::Exception(tl::to_string(
              tr("This function cannot be applied to PCell guiding shapes")));
        }
      }
    }
  }
 ...
 *  new shapes inserted.
 */
class NewObjectsSelection : public db::ClipboardDataInsertReceiver {
public:
  NewObjectsSelection(int cv_index, db::cell_index_type topcell,
  ...
  void shape_inserted(db::cell_index_type cell, int layer,
  ...
  void instance_inserted(db::cell_index_type cell,
...
void MainService::paste() {
  if (view()->is_editable()) {
    ...
    for (db::Clipboard::iterator c = db::Clipboard::instance().begin();
         c != db::Clipboard::instance().end(); ++c) {
      const db::ClipboardValue<edt::ClipboardData> *value =
          dynamic_cast<const db::ClipboardValue<edt::ClipboardData> *>(*c);
      if (value) {

        if (!cv.is_valid()) {
          throw tl::Exception(
              tl::to_string(tr("No cell selected to paste something into")));
        }

        std::vector<unsigned int> nl =
            value->get().insert(cv->layout(), cv.context_trans().inverted(),
                                &cv->layout().cell(cv.cell_index()),
                                0 /*new_tops*/, &insert_notification);
