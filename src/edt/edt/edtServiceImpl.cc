...
ShapeEditService::ShapeEditService(db::Manager *manager,
                                   lay::LayoutViewBase *view,
                                   db::ShapeIterator::flags_type shape_types)
    : edt::Service(manager, view, shape_types), m_layer(0), m_cv_index(0),
      mp_cell(0), mp_layout(0), m_combine_mode(CM_Add) {
  view->current_layer_changed_event.add(this,
                                        &ShapeEditService::update_edit_layer);
}

bool ShapeEditService::configure(const std::string &name,
...
void ShapeEditService::get_edit_layer() {
  ...
  if (!cl->visible(true)) {
    lay::TipDialog td(QApplication::activeWindow(),
                      tl::to_string(tr("You are about to draw on a hidden "
                                       "layer. The result won't be visible.")),
  ...
  m_trans = (cl->trans().front() * db::CplxTrans(cv->layout().dbu()) *
             cv.context_trans())
                .inverted();
  mp_layout = &(cv->layout());
  ...
  if (mp_cell->is_proxy()) {
    throw tl::Exception(
        tl::to_string(tr("Cannot put a shape into a PCell or library cell")));
  }
}

void ShapeEditService::update_edit_layer(
    const lay::LayerPropertiesConstIterator &cl) {
  if (!editing()) {
    return;
  }
  ...
  if (!cl->visible(true)) {
    lay::TipDialog td(QApplication::activeWindow(),
                      tl::to_string(tr("You are now drawing on a hidden layer. "
                                       "The result won't be visible.")),
                      "drawing-on-invisible-layer");
    td.exec_dialog();
  }
#endif
 ...
 *
 *  This method uses an intermediate point o to determine the edge that is
 * emerged from point m. An edge is searched that emerges from p and intersects
 * with the m->o edge in a way that the intersection point is closest to o.
 *
 *  This method returns the intersection point ("new o") and a flag if the
 * search was successful (.first of return value).
 */
std::pair<bool, db::DPoint>
ShapeEditService::interpolate(const db::DPoint &m, const db::DPoint &o,
                              const db::DPoint &p) const {
  ...
  bool c_set = false;
  db::DPoint c;
  for (std::vector<db::DVector>::const_iterator d = delta.begin();
       d != delta.end(); ++d) {
    std::pair<bool, db::DPoint> ip =
        db::DEdge(m, o).cut_point(db::DEdge(p - *d, p));
...
void ShapeEditService::deliver_shape(const db::Polygon &poly) {
  if (m_combine_mode == CM_Add) {
    ...
    db::ShapeIterator s = cell().shapes(layer()).begin_touching(
        poly.box(), db::ShapeIterator::Polygons | db::ShapeIterator::Paths |
    ...
    if (manager()) {
      manager()->transaction(
          tl::to_string(tr("Combine shape with background")));
    }

    //  Erase existing shapes
    for (std::vector<db::Shape>::const_iterator s = shapes.begin();
         s != shapes.end(); ++s) {
      cell().shapes(layer()).erase_shape(*s);
    }

    //  Add new shapes
    for (std::vector<db::Polygon>::const_iterator p = result.begin();
         p != result.end(); ++p) {
      cell().shapes(layer()).insert(*p);
    }
    for (std::vector<db::Polygon>::const_iterator p = input_left.begin();
         p != input_left.end(); ++p) {
      cell().shapes(layer()).insert(*p);
    }
...
void PolygonService::set_last_point(const db::DPoint &p) {
  ...
  if (m_points.size() >= 3 && connect_ac() == lay::AC_Ortho) {
    ...
    std::pair<bool, db::DPoint> ip =
        interpolate(m_points.end()[-3], m_last, p_grid);
    if (ip.first) {
...
bool PolygonService::do_mouse_click(const db::DPoint &p) {
  ...
  //  do not do a add_closure here - this will not work since we may have two
  //  identical points on top.
  return false;
}

void PolygonService::do_finish_edit() {
  deliver_shape(get_polygon());
  commit_recent(view());
}

db::Polygon PolygonService::get_polygon() const {
  db::Polygon poly;

  if (m_points.size() < 4) {
    throw tl::Exception(
        tl::to_string(tr("A polygon must have at least 3 points")));
  }
  ...
  for (std::vector<db::DPoint>::const_iterator p = m_points.begin();
       p + 1 != m_points.end(); ++p) {
  ...
  poly.assign_hull(points_dbu.begin(), points_dbu.end(), true,
                   true /*remove reflected*/);

  if (poly.hull().size() < 3) {
    throw tl::Exception(
        tl::to_string(tr("A polygon must have at least 3 effective points")));
  }
...
//  TODO: for Diagonal mode, this scheme does not work pretty well.
    ...
    //  Determine the closing point by determining the one of the possible
    //  closing points (given the angle constraints) that is closest to the
    //  current one.
    ...
    if (cp.first &&
        db::sprod(p1 - m_points[0], cp.second - m_points[0]) <
            0.99 * p1.distance(m_points[0]) * cp.second.distance(m_points[0]) +
                1e-6 &&
        db::sprod(pl - m_points.back(), cp.second - m_points.back()) <
            0.99 * pl.distance(m_points.back()) *
                    cp.second.distance(m_points.back()) +
                1e-6) {
      m_closure = cp.second;
      m_closure_set = true;
    }

    //  if that is not working out, try to keep one edge any vary the possible
    //  edges emerging from the other point
    if (!m_closure_set) {

      for (std::vector<db::DVector>::const_iterator d1 = delta.begin();
           d1 != delta.end(); ++d1) {
        ...
        std::pair<bool, db::DPoint> cp = e1.cut_point(e2);
        if (cp.first &&
            (!m_closure_set || cp.second.sq_distance(m_points.back()) <
                                   m_closure.sq_distance(m_points.back())) &&
            db::sprod(p1 - m_points[0], cp.second - m_points[0]) <
                0.99 * p1.distance(m_points[0]) *
                    cp.second.distance(m_points[0]) &&
            db::sprod(pl - m_points.back(), cp.second - m_points.back()) <
                0.99 * pl.distance(m_points.back()) *
                    cp.second.distance(m_points.back())) {
          m_closure = cp.second;
    ...
    if (!m_closure_set) {
      ...
      for (std::vector<db::DVector>::const_iterator d2 = delta.begin();
           d2 != delta.end(); ++d2) {
    ...
    //  if that is not working out, try each possible variations of edges from
    //  start and end point
    if (!m_closure_set) {
      for (std::vector<db::DVector>::const_iterator d1 = delta.begin();
           d1 != delta.end(); ++d1) {
...
void PolygonService::update_marker() {
  if (m_points.size() == 2) {
    ...
    marker->set(edge,
                db::VCplxTrans(1.0 / layout().dbu()) * trans().inverted());
...
void BoxService::update_marker() {
  ...
  if (marker) {
    ...
    view()->message(
        std::string("lx: ") + tl::micron_to_string(m_p2.x() - m_p1.x()) +
        std::string("  ly: ") + tl::micron_to_string(m_p2.y() - m_p1.y()));
  }
}
...
void BoxService::do_finish_edit() {
...
db::Point PointService::get_point() const { return db::Point(trans() * m_p); }
...
void PointService::update_marker() {
  ...
  if (marker) {
    ...
    marker->set(db::Box(pt, pt),
                db::VCplxTrans(1.0 / layout().dbu()) * trans().inverted());

    view()->message(std::string("x: ") + tl::micron_to_string(m_p.x()) +
                    std::string("  y: ") + tl::micron_to_string(m_p.y()));
  }
}
...
void PointService::do_finish_edit() {
...
TextService::TextService(db::Manager *manager, lay::LayoutViewBase *view)
    : ShapeEditService(manager, view, db::ShapeIterator::Texts), m_rot(0) {
  //  .. nothing yet ..
}
...
void TextService::update_marker() {
  ...
  if (marker) {
    ...
    std::string pos =
        std::string("x: ") + tl::micron_to_string(m_text.trans().disp().x()) +
        std::string("  y: ") + tl::micron_to_string(m_text.trans().disp().y());
    if (m_text.trans().rot() != 0) {
      pos +=
          std::string("  ") + ((const db::DFTrans &)m_text.trans()).to_string();
    }
...
db::Text TextService::get_text() const {
  db::Point p_dbu = trans() * (db::DPoint() + m_text.trans().disp());
  return db::Text(
      m_text.string(), db::Trans(m_text.trans().rot(), p_dbu - db::Point()),
      db::coord_traits<db::Coord>::rounded(trans().ctrans(m_text.size())),
      db::NoFont, m_text.halign(), m_text.valign());
}

void TextService::do_finish_edit() {
  get_edit_layer();
  ...
  if (!view()->text_visible()) {
    ...
    lay::TipDialog td(
        ...
        tl::to_string(
            tr("A text object is created but texts are disabled for drawing "
               "and are not visible. Do you want to enable drawing of "
               "texts?\n\nChoose \"Yes\" to enable text drawing now.")),
        "text-created-but-not-visible", lay::TipDialog::yesno_buttons);
...
PathService::PathService(db::Manager *manager, lay::LayoutViewBase *view)
    : ShapeEditService(manager, view, db::ShapeIterator::Paths), m_width(0.1),
      m_bgnext(0.0), m_endext(0.0), m_type(Flush), m_needs_update(true) {
  //  .. nothing yet ..
}
...
void PathService::do_finish_edit() {
  //  one point is reserved for the "current one"
  if (m_points.size() < 3) {
    throw tl::Exception(
        tl::to_string(tr("A path must have at least 2 points")));
  }
  m_points.pop_back();
...
db::Path PathService::get_path() const {
  db::Path path;
...
InstService::InstService(db::Manager *manager, lay::LayoutViewBase *view)
    : edt::Service(manager, view), m_angle(0.0), m_scale(1.0), m_mirror(false),
      m_is_pcell(false), m_array(false), m_rows(1), m_columns(1), m_row_x(0.0),
      m_row_y(0.0), m_column_x(0.0), m_column_y(0.0), m_place_origin(false),
      m_reference_transaction_id(0), m_needs_update(true),
      m_parameters_changed(false), m_has_valid_cell(false),
      m_in_drag_drop(false), m_current_cell(0), mp_current_layout(0),
      mp_pcell_decl(0), m_cv_index(-1) {
  //  .. nothing yet ..
}
...
tl::Variant InstService::get_default_layer_for_pcell() {
  lay::LayerPropertiesConstIterator cl = view()->current_layer();
  if (!cl.is_null() && !cl->has_children() &&
      (cl->source(true).cv_index() < 0 ||
       cl->source(true).cv_index() == view()->active_cellview_index())) {
...
#if defined(HAVE_QT)
bool InstService::drag_enter_event(const db::DPoint &p,
                                   const lay::DragDropDataBase *data) {
  const lay::CellDragDropData *cd =
      dynamic_cast<const lay::CellDragDropData *>(data);
  if (view()->is_editable() && cd &&
      (cd->layout() == &view()->active_cellview()->layout() || cd->library())) {
    ...
    if (cd->library()) {
      ...
      if (cd->library()->for_technologies() &&
          view()->cellview(view()->active_cellview_index()).is_valid()) {
        if (!cd->library()->is_for_technology(
                view()
    ...
    if (cd->is_pcell()) {
      ...
      const db::PCellDeclaration *pcell_decl =
          cd->layout()->pcell_declaration(cd->cell_index());
      if (!pcell_decl) {
        return false;
      }
...
bool InstService::drag_move_event(const db::DPoint &p,
                                  const lay::DragDropDataBase * /*data*/) {
  if (m_in_drag_drop) {
    do_mouse_move(p);
    return true;
...
bool InstService::drop_event(const db::DPoint & /*p*/,
                             const lay::DragDropDataBase * /*data*/) {
...
void InstService::sync_to_config() {
  //  push the current setup to configuration so the instance dialog will take
  //  these as default and "apply" of these instance properties doesn't fail
  //  because of insistency.
  ...
  if (m_is_pcell) {
    dispatcher()->config_set(cfg_edit_inst_pcell_parameters,
                             pcell_parameters_to_string(m_pcell_parameters));
  } else {
...
void InstService::do_begin_edit(const db::DPoint &p) {
  ...
  if (cv.cell()->is_proxy()) {
    throw tl::Exception(tl::to_string(
        tr("Cannot put an instance into a PCell or library cell")));
  }
  ...
  if (ci.first) {
    // use the snapped lower left corner of the bbox unless the origin is inside
    // the bbox
    ...
    if (!m_place_origin && !cell_bbox.contains(db::Point())) {
      ...
      m_disp =
          db::DPoint() + (m_disp - snap(cell_bbox.transformed(ct).lower_left() *
                                        cv->layout().dbu()));
    }
  }
  ...
  if (!tv.empty()) {
    m_trans = db::VCplxTrans(1.0 / cv->layout().dbu()) * tv[0] *
              db::CplxTrans(cv->layout().dbu()) * cv.context_trans();
  }
...
InstService::make_cell(const lay::CellView &cv) {
  ...
  //  NOTE: do this at the beginning: creating a transaction might delete
  //  transactions behind the head transaction, hence releasing (thus: deleting)
  //  cells. To prevert interference, create the transaction at the beginning.
  db::Transaction transaction(manager(),
                              tl::to_string(tr("Create reference cell")),
                              m_reference_transaction_id);

  lay::LayerState layer_state = view()->layer_snapshot();

  db::Library *lib = db::LibraryManager::instance().lib_ptr_by_name(
      m_lib_name, cv->tech_name());
  ...
  if (pci.first) {
    ...
    if (mp_pcell_decl) {
      ...
      //  make the parameters fit (i.e. PCells may not define consistent default
      //  parameters)
...
void InstService::do_mouse_move(const db::DPoint &p) {
  ...
  std::pair<bool, db::cell_index_type> ci = make_cell(cv);
  if (ci.first) {
    //  use the snapped lower left corner of the bbox unless the origin is
    //  inside the bbox
    db::Box cell_bbox = cv->layout().cell(ci.second).bbox();
    if (!m_place_origin && !cell_bbox.contains(db::Point())) {
      db::CplxTrans ct(1.0, m_angle, m_mirror, db::DVector());
...
void InstService::do_finish_edit() {
  try {
    ...
    if (get_inst(inst)) {
      ...
      cv->layout()
          .cell(inst.object().cell_index())
          .collect_called_cells(called);
      ...
      std::set_intersection(called.begin(), called.end(), callers.begin(),
                            callers.end(), std::back_inserter(intersection));
      if (!intersection.empty()) {
        throw tl::Exception(tl::to_string(
            tr("Inserting this instance would create a recursive hierarchy")));
      }

      if (manager()) {
        manager()->transaction(tl::to_string(tr("Create instance")),
                               m_reference_transaction_id);
      }
...
void InstService::do_cancel_edit() {
  //  Undo "create reference" transactions which basically unfinished "create
  //  instance" transactions
  if (m_reference_transaction_id > 0 &&
      manager()->transaction_id_for_undo() == m_reference_transaction_id) {
    manager()->undo();
  }
...
void InstService::service_configuration_changed() { m_needs_update = true; }
...
bool InstService::configure(const std::string &name, const std::string &value) {
  ...
  if (name == cfg_edit_inst_pcell_parameters) {
    ...
    std::map<std::string, tl::Variant> pcp =
        pcell_parameters_from_string(value);
...
void InstService::switch_cell_or_pcell(bool switch_parameters) {
  //  if the library or cell name has changed, store the current pcell
  //  parameters and try to reuse an existing parameter set
  if (!m_cell_or_pcell_name_previous.empty() &&
      (m_cell_or_pcell_name_previous != m_cell_or_pcell_name ||
       m_lib_name_previous != m_lib_name)) {

    m_stored_pcell_parameters[std::make_pair(m_cell_or_pcell_name_previous,
                                             m_lib_name_previous)] =
        m_pcell_parameters;
    ...
    if (switch_parameters) {
      ...
      std::map<std::pair<std::string, std::string>,
               std::map<std::string, tl::Variant>>::const_iterator p =
          m_stored_pcell_parameters.find(
              std::make_pair(m_cell_or_pcell_name, m_lib_name));
...
void InstService::config_finalize() {
  if (m_needs_update) {
    ...
    //  since the last "config_finalize". This means the sender of the
    //  configuration events wants the parameters to be set in a specific way.
    //  Don't interfere.
    ...
    if (switch_parameters) {
      ...
      //  TODO: it's somewhat questionable to do this inside "config_finalize"
      //  as this method is supposed to reflect changes rather than induce some.
      if (m_is_pcell) {
...
void InstService::update_marker() {
  if (editing()) {
    ...
    lay::Marker *marker = new lay::Marker(
        view(), m_cv_index, !show_shapes_of_instances(),
        show_shapes_of_instances() ? max_shapes_of_instances() : 0);
    marker->set_vertex_shape(lay::ViewOp::Cross);
    marker->set_vertex_size(9 /*cross vertex size*/);
    set_edit_marker(marker);
...
bool InstService::get_inst(db::CellInstArray &inst) {
  ...
  if (cv.is_valid()) {
    ...
    if (ci.first) {
      ...
      db::VCplxTrans pt =
          (db::CplxTrans(cv->layout().dbu()) * m_trans).inverted();
      db::ICplxTrans trans =
          db::ICplxTrans(m_scale, m_angle, m_mirror, pt * m_disp - db::Point());

      if (m_array && m_rows > 0 && m_columns > 0) {
        db::Vector row = db::Vector(pt * db::DVector(m_row_x, m_row_y));
        db::Vector column =
            db::Vector(pt * db::DVector(m_column_x, m_column_y));
        inst = db::CellInstArray(db::CellInst(ci.second), trans, row, column,
                                 m_rows, m_columns);
      } else {
