...
std::string
    cfg_edit_pcell_show_parameter_names("edit-pcell-show-parameter-names");
...
std::string CMConverter::to_string(const edt::combine_mode_type &m) {
  if (m == edt::CM_Add) {
    return "add";
...
std::string PathExtConverter::to_string(const edt::path_ext_type &m) {
  if (m == edt::Flush) {
    return "flush";
...
void PathExtConverter::from_string(const std::string &tt,
                                   edt::path_ext_type &m) {
  std::string t(tl::trim(tt));
  if (t == "flush") {
    m = edt::Flush;
...
std::string HAlignConverter::to_string(db::HAlign a) {
...
std::string VAlignConverter::to_string(db::VAlign a) {
...
std::string EditGridConverter::to_string(const db::DVector &eg) {
  if (eg == db::DVector()) {
    return "global";
  } else if (eg.x() < 1e-6) {
    return "none";
...
void EditGridConverter::from_string_picky(const std::string &s,
                                          db::DVector &eg) {
  tl::Extractor ex(s.c_str());
  ...
  if (ex.test("global")) {
    ...
    if (x < 1e-6 || y < 1e-6) {
      throw tl::Exception(
          tl::to_string(tr("The grid must be larger than zero")));
    }
    eg = db::DVector(x, y);
  }
  ex.expect_end();
