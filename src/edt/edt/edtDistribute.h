 ...
 */
template <class Box, class Value, bool horizontally> class box_compare {
public:
  typedef typename Box::coord_type coord_type;

  box_compare(int ref) : m_ref(ref) {
    //  .. nothing yet ..
  }

  bool operator()(const std::pair<Box, Value> &a,
                  const std::pair<Box, Value> &b) const {
 ...
 */
template <class Box, bool horizontally>
void do_bin(typename std::vector<std::pair<Box, size_t>>::const_iterator b,
            typename std::vector<std::pair<Box, size_t>>::const_iterator e,
            int ref, std::vector<std::vector<size_t>> &bins) {
  typedef typename Box::coord_type coord_type;

  //  determine maximum distance between adjacent coordinates

  coord_type max_dist = 0;
  for (typename std::vector<std::pair<Box, size_t>>::const_iterator i = b + 1;
       i != e; ++i) {
    max_dist = std::max(
        max_dist, box_position<Box, horizontally>(i->first, ref) -
                      box_position<Box, horizontally>((i - 1)->first, ref));
  }

  //  heuristically, everything that has a distance of less than 1/3 of the
  //  maximum distance falls into one bin
...
inline typename Box::coord_type eff_dim(const Box &box,
                                        typename Box::coord_type pitch,
                                        typename Box::coord_type space) {
  typedef typename Box::coord_type coord_type;

  coord_type d = (horizontal ? box.width() : box.height()) + space;
  if (pitch > 0) {
    d = db::coord_traits<coord_type>::rounded(
        ceil(double(d) / double(pitch) - 1e-10) * double(pitch));
  }
  return d;
}

template <class Coord> struct max_coord_join_op {
  void operator()(Coord &a, const Coord &b) const { a = std::max(a, b); }
};

/**
 *  @brief Implements an algorithm for 2d-distributing rectangular objects
 */
template <class Box, class Value> class distributed_placer {
public:
  typedef typename Box::coord_type coord_type;
   ...
   *  @param refp The alignment in the other (horizontal) direction (-1: left,
   * 0: center, 1: right, other: leave as is)
   ...
   *  @param refp The alignment in the other (vertical) direction (-1: bottom,
   * 0: center, 1: top, other: leave as is)
   *  @param pitch The distribution pitch (grid) or 0 for no pitch
   *  @param space The minimum space between the objects
   */
   ...
   *
   *  @param href The horizontal reference location (-1: left, 0: center, 1:
   * right)
   ...
   *  @param vref The vertical reference location (-1: bottom, 0: center, 1:
   * top)
   ...
   */
  void distribute_matrix(int href, coord_type hpitch, coord_type hspace,
                         int vref, coord_type vpitch, coord_type vspace) {
    if (m_objects.size() < 2) {
      return;
    }

    //  The algorithm is this:
    //  1.) Bin the boxes according to their positions in horizontal and
    //  vertical direction.
    //      This forms the potential columns and rows
    //  2.) Compute the row and column widths and heights as the maximum of
    //  their content 3.) position the objects inside these cells

    std::vector<std::pair<Box, size_t>> indexed_boxes;
    indexed_boxes.reserve(m_objects.size());

    Box all;
    for (typename objects::iterator i = m_objects.begin(); i != m_objects.end();
         ++i) {
      all += i->first;
    }
    ...
    std::sort(indexed_boxes.begin(), indexed_boxes.end(),
              box_compare<Box, size_t, true>(href));
    do_bin<Box, true>(indexed_boxes.begin(), indexed_boxes.end(), href, hbins);

    std::sort(indexed_boxes.begin(), indexed_boxes.end(),
              box_compare<Box, size_t, false>(vref));
...

    {
      ...
      for (std::vector<std::vector<size_t>>::const_iterator i = hbins.begin();
           i != hbins.end(); ++i) {
        for (std::vector<size_t>::const_iterator j = i->begin(); j != i->end();
             ++j) {
          hbin_for_index[*j] = i - hbins.begin();
        }
      }

      for (std::vector<std::vector<size_t>>::const_iterator i = vbins.begin();
           i != vbins.end(); ++i) {
        for (std::vector<size_t>::const_iterator j = i->begin(); j != i->end();
             ++j) {
    ...
    for (std::vector<std::vector<std::vector<size_t>>>::const_iterator i =
             cells.begin();
         i != cells.end(); ++i) {

      for (std::vector<std::vector<size_t>>::const_iterator j = i->begin();
           j != i->end(); ++j) {

        coord_type wcell = 0, hcell = 0;
        for (std::vector<size_t>::const_iterator k = j->begin(); k != j->end();
             ++k) {
          //  NOTE: intra-cell objects are distributed horizontally
          wcell += eff_dim<Box, true>(m_objects[*k].first, hpitch, hspace);
          hcell = std::max(
              hcell, eff_dim<Box, false>(m_objects[*k].first, vpitch, vspace));
        }

        cell_widths[i - cells.begin()] =
            std::max(cell_widths[i - cells.begin()], wcell);
        cell_heights[j - i->begin()] =
            std::max(cell_heights[j - i->begin()], hcell);
      }
    }
    ...
    coord_type x = 0, y = 0;
    for (typename std::vector<coord_type>::const_iterator i =
             cell_widths.begin();
         i != cell_widths.end(); ++i) {
    ...
    for (std::vector<std::vector<std::vector<size_t>>>::const_iterator i =
      ...
      for (std::vector<std::vector<size_t>>::const_iterator j = i->begin();
        ...
        for (std::vector<size_t>::const_iterator k = j->begin(); k != j->end();
          ...
          coord_type w =
              eff_dim<Box, true>(m_objects[*k].first, hpitch, hspace);
          coord_type h =
              eff_dim<Box, false>(m_objects[*k].first, vpitch, vspace);
          ...
          m_objects[*k].first.move(db::point<coord_type>(x, y) -
                                   m_objects[*k].first.p1());
    ...
    //  Final adjustments - align the whole matrix with the original bounding
    //  box
    ...
    coord_type dh = box_position<Box, true>(all, href) -
                    box_position<Box, true>(new_all, href);
    coord_type dv = box_position<Box, false>(all, vref) -
                    box_position<Box, false>(new_all, vref);
...
private:
  ...
  void do_distribute_1d(int ref, int refp, coord_type pitch, coord_type space) {
    ...
    Box all;
    for (typename objects::const_iterator i = m_objects.begin() + 1;
         i != m_objects.end(); ++i) {
      all += i->first;
    }

    std::sort(m_objects.begin(), m_objects.end(),
              box_compare<Box, Value, horizontally>(ref));

    Box current = m_objects.front().first;
    coord_type p0 = box_position<Box, horizontally>(current, ref);

    for (typename objects::iterator i = m_objects.begin() + 1;
         i != m_objects.end(); ++i) {
      ...
      if (db::coord_traits<coord_type>::less(0, pitch)) {
        pnew = coord_type(
                   ceil(double(pnew + offset - p0) / double(pitch) - 1e-10)) *
                   pitch -
               offset + p0;
      }
    ...
    db::vector<coord_type> mv;
    coord_type d = box_position<Box, horizontally>(all, ref) -
                   box_position<Box, horizontally>(new_all, ref);
    if (horizontally) {
    ...
    for (typename objects::iterator i = m_objects.begin(); i != m_objects.end();
      ...
      if (refp >= -1 && refp <= 1) {
        ...
        coord_type dp = box_position<Box, !horizontally>(all, refp) -
                        box_position<Box, !horizontally>(i->first, refp);
