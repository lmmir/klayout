...
lay::angle_constraint_type ac_from_buttons(unsigned int buttons) {
...
Service::Service(db::Manager *manager, lay::LayoutViewBase *view,
    ...
    : lay::EditorServiceBase(view), db::Object(manager), mp_view(view),
      mp_transient_marker(0), m_editing(false), m_immediate(false),
      m_cell_inst_service(false), m_flags(flags), m_move_sel(false),
      m_moving(false), m_connect_ac(lay::AC_Any), m_move_ac(lay::AC_Any),
      m_alt_ac(lay::AC_Global), m_snap_to_objects(false),
      m_top_level_sel(false), m_show_shapes_of_instances(true),
      m_max_shapes_of_instances(1000), m_hier_copy_mode(-1),
      m_indicate_secondary_selection(false), m_seq(0),
...
Service::Service(db::Manager *manager, lay::LayoutViewBase *view)
    : lay::EditorServiceBase(view), db::Object(manager), mp_view(view),
      mp_transient_marker(0), m_editing(false), m_immediate(false),
      m_cell_inst_service(true), m_flags(db::ShapeIterator::Nothing),
      m_move_sel(false), m_moving(false), m_connect_ac(lay::AC_Any),
...
Service::~Service() {
  for (std::vector<lay::ViewObject *>::iterator r = m_markers.begin();
       r != m_markers.end(); ++r) {
    delete *r;
  }
  m_markers.clear();

  for (std::vector<lay::ViewObject *>::iterator r = m_edit_markers.begin();
       r != m_edit_markers.end(); ++r) {
    delete *r;
  }
...
lay::angle_constraint_type Service::connect_ac() const {
...
db::DVector Service::snap(db::DVector v) const {
  //  snap according to the grid
  if (m_edit_grid == db::DVector()) {
...
db::DVector Service::snap(const db::DVector &v, bool connect) const {
  return snap(lay::snap_angle(v, connect ? connect_ac() : move_ac()));
}

db::DPoint Service::snap(const db::DPoint &p, const db::DPoint &plast,
                         bool connect) const {
  db::DPoint ps = plast + lay::snap_angle(db::DVector(p - plast),
                                          connect ? connect_ac() : move_ac());
  return snap(ps);
}

const int sr_pixels = 8; // TODO: make variable

lay::PointSnapToObjectResult Service::snap2_details(const db::DPoint &p) const {
  double snap_range = ui()->mouse_event_trans().inverted().ctrans(sr_pixels);
  return lay::obj_snap(
      m_snap_to_objects ? view() : 0, p,
      m_edit_grid == db::DVector() ? m_global_grid : m_edit_grid, snap_range);
}

db::DPoint Service::snap2(const db::DPoint &p) const {
  return snap2_details(p).snapped_point;
}

db::DPoint Service::snap2(const db::DPoint &p, const db::DPoint &plast,
                          bool connect) const {
  double snap_range = ui()->mouse_event_trans().inverted().ctrans(sr_pixels);
  return lay::obj_snap(m_snap_to_objects ? view() : 0, plast, p,
                       m_edit_grid == db::DVector() ? m_global_grid
                                                    : m_edit_grid,
                       connect ? connect_ac() : move_ac(), snap_range)
      .snapped_point;
}
...
void Service::highlight(const std::vector<size_t> &n) {
  m_highlights_selected = true;
...
void Service::apply_highlights() {
  for (std::vector<lay::ViewObject *>::iterator r = m_markers.begin();
       r != m_markers.end(); ++r) {
    (*r)->visible(!m_highlights_selected ||
                  m_selected_highlights.find(r - m_markers.begin()) !=
                      m_selected_highlights.end());
  }
}
...
void Service::copy_selected() {
  ...
  if (m_hier_copy_mode < 0) {
    for (objects::const_iterator r = m_selection.begin();
         r != m_selection.end() && !need_to_ask_for_copy_mode; ++r) {
      if (r->is_cell_inst()) {
        const db::Cell &cell = view()
                                   ->cellview(r->cv_index())
                                   ->layout()
                                   .cell(r->back().inst_ptr.cell_index());
        if (!cell.is_proxy()) {
  ...
  if (!need_to_ask_for_copy_mode ||
      mode_dialog.exec_dialog(inst_mode, dont_ask_again)) {

    //  store the given value "forever"
    if (dont_ask_again) {
      dispatcher()->config_set(cfg_edit_hier_copy_mode,
                               tl::to_string(inst_mode));
...
void Service::copy_selected(unsigned int inst_mode) {
  //  create one ClipboardData object per cv_index because, this one assumes
  //  that there is only one source layout object.
  std::set<unsigned int> cv_indices;
  for (objects::const_iterator r = m_selection.begin(); r != m_selection.end();
       ++r) {
    cv_indices.insert(r->cv_index());
  }

  for (std::set<unsigned int>::const_iterator cvi = cv_indices.begin();
       cvi != cv_indices.end(); ++cvi) {

    db::ClipboardValue<edt::ClipboardData> *cd =
        new db::ClipboardValue<edt::ClipboardData>();
    ...
    for (objects::const_iterator r = m_selection.begin();
      ...
      if (r->cv_index() == *cvi) {
        if (!r->is_cell_inst()) {
          cd->get().add(cv->layout(), r->layer(), r->shape(),
                        cv.context_trans() * r->trans());
        } else {
          cd->get().add(cv->layout(), r->back().inst_ptr, inst_mode,
                        cv.context_trans() * r->trans());
        }
      }
    }
...
bool Service::begin_move(lay::Editable::MoveMode mode, const db::DPoint &p,
  ...
  if (view()->is_editable() && mode == lay::Editable::Selected) {
    ...
    for (std::vector<lay::ViewObject *>::iterator r = m_markers.begin();
      ...
      lay::InstanceMarker *inst_marker =
          dynamic_cast<lay::InstanceMarker *>(*r);
      if (inst_marker) {
        inst_marker->set_draw_outline(!m_show_shapes_of_instances);
        inst_marker->set_max_shapes(
            m_show_shapes_of_instances ? m_max_shapes_of_instances : 0);
      }
    }
  }
...
void Service::move(const db::DPoint &pu, lay::angle_constraint_type ac) {
  ...
  if (view()->is_editable() && m_moving) {
    move_markers(db::DTrans(p - db::DPoint()) *
                 db::DTrans(m_move_trans.fp_trans()) *
                 db::DTrans(db::DPoint() - snap(m_move_start)));
  }
  m_alt_ac = lay::AC_Global;
}

void Service::move_transform(const db::DPoint &pu, db::DFTrans tr,
                             lay::angle_constraint_type ac) {
  m_alt_ac = ac;
  db::DPoint p = snap(m_move_start) + snap(pu - m_move_start, false);
  if (view()->is_editable() && m_moving) {
    move_markers(db::DTrans(p - db::DPoint()) *
                 db::DTrans(m_move_trans.fp_trans() * tr) *
                 db::DTrans(db::DPoint() - snap(m_move_start)));
  }
  m_alt_ac = lay::AC_Global;
}

void Service::end_move(const db::DPoint & /*p*/,
                       lay::angle_constraint_type ac) {
  m_alt_ac = ac;
  if (view()->is_editable() && m_moving) {
...
db::DBox Service::selection_bbox() {
  ...
  for (objects::const_iterator r = m_selection.begin(); r != m_selection.end();
    ...
    db::CplxTrans ctx_trans =
        db::CplxTrans(layout.dbu()) * cv.context_trans() * r->trans();

    db::box_convert<db::CellInst> bc(layout);
    if (!r->is_cell_inst()) {

      const std::vector<db::DCplxTrans> *tv_list =
          tv.per_cv_and_layer(r->cv_index(), r->layer());
      if (tv_list != 0) {
...
lay::ViewObject *Service::edit_marker() {
...
void Service::transform(const db::DCplxTrans &trans,
                        const std::vector<db::DCplxTrans> *p_trv) {
  //  ignore this function in non-editable mode
  if (!view()->is_editable()) {
    return;
  }

  //  HINT: sorting the selected shapes/instances ensures that a shape/instance
  //  is not moved twice. This may happen due to per-instance selection of
  //  lower-level shapes.
  ...
  n = 0;
  for (objects::iterator r = m_selection.begin(); r != m_selection.end();
       ++r, ++n) {
    ...
    if (!r->is_cell_inst()) {
      shapes_by_cell
          .insert(std::make_pair(
              std::make_pair(r->cell_index(),
                             std::make_pair(r->cv_index(), r->layer())),
              std::vector<size_t>()))
          .first->second.push_back(n);
    }
  }

  for (std::map<std::pair<db::cell_index_type,
                          std::pair<unsigned int, unsigned int>>,
                std::vector<size_t>>::iterator sbc = shapes_by_cell.begin();
       sbc != shapes_by_cell.end(); ++sbc) {

    const lay::CellView &cv = view()->cellview(sbc->first.second.first);
    if (cv.is_valid()) {

      const std::vector<db::DCplxTrans> *tv_list = tv.per_cv_and_layer(
          sbc->first.second.first, sbc->first.second.second);
      if (tv_list != 0) {
        ...
        db::Shapes &shapes = cv->layout()
                                 .cell(sbc->first.first)
                                 .shapes(sbc->first.second.second);

        std::map<db::Shape, db::Shape> new_shapes;

        for (std::vector<size_t>::iterator si = sbc->second.begin();
             si != sbc->second.end(); ++si) {
          ...
          std::map<db::Shape, db::Shape>::iterator ns =
              new_shapes.find(s->shape());

          if (ns == new_shapes.end()) {
            new_shapes.insert(
                std::make_pair(s->shape(), shapes.transform(s->shape(), mt)));
          } else {
  ...
  std::map<std::pair<db::cell_index_type, unsigned int>, std::vector<size_t>>
      insts_by_cell;
  n = 0;
  for (objects::iterator r = m_selection.begin(); r != m_selection.end();
       ++r, ++n) {
    if (r->is_cell_inst()) {
      insts_by_cell
          .insert(std::make_pair(std::make_pair(r->cell_index(), r->cv_index()),
                                 std::vector<size_t>()))
          .first->second.push_back(n);
    }
  }
  ...
  for (std::map<std::pair<db::cell_index_type, unsigned int>,
                std::vector<size_t>>::iterator ibc = insts_by_cell.begin();
    ...
    if (cv.is_valid()) {
      ...
      if (tv_list != 0) {
        ...
        for (std::vector<size_t>::iterator ii = ibc->second.begin();
             ii != ibc->second.end(); ++ii) {
          ...
          std::map<db::Instance, db::Instance>::iterator ni =
              new_insts.find(inst_ptr);

          if (ni == new_insts.end()) {
            new_insts.insert(
                std::make_pair(inst_ptr, cell.transform(inst_ptr, mt)));
          } else {
...
bool Service::mouse_move_event(const db::DPoint &p, unsigned int buttons,
  ...
  if (view()->is_editable() && prio) {
    ...
    if (m_editing || m_immediate) {
      ...
      if (!m_editing) {
        //  in this mode, ignore exceptions here since it is rather annoying to
        //  have messages popping up then.
        try {
          begin_edit(p);
        } catch (...) {
          set_edit_marker(0);
  ...
  return false; // not taken to allow the mouse tracker to receive events as
                // well
}
  ...
  if (view()->is_editable() && prio) {
    ...
    if ((buttons & lay::LeftButton) != 0) {
      ...
      if (!m_editing) {
        ...
        view()->cancel(); //  cancel any pending edit operations and clear the
                          //  selection
        set_edit_marker(0);
        begin_edit(p);
...
bool Service::mouse_click_event(const db::DPoint &p, unsigned int buttons,
                                bool prio) {
  if (view()->is_editable() && prio && (buttons & lay::RightButton) != 0 &&
      m_editing) {
    m_alt_ac = ac_from_buttons(buttons);
...
bool Service::key_event(unsigned int key, unsigned int buttons) {
  if (view()->is_editable() && m_editing && buttons == 0 &&
      key == lay::KeyBackspace) {
    do_delete();
    return true;
  } else {
    return false;
...
void Service::del_selected() {
  ...
  for (objects::const_iterator r = m_selection.begin(); r != m_selection.end();
    ...
    if (cv.is_valid()) {
      ...
      if (!r->is_cell_inst()) {
        if (r->layer() != cv->layout().guiding_shape_layer() &&
            cell.shapes(r->layer()).is_valid(r->shape())) {
...
bool Service::has_selection() { return !m_selection.empty(); }

size_t Service::selection_size() { return m_selection.size(); }
...
double Service::click_proximity(const db::DPoint &pos,
  ...
  if (m_cell_inst_service) {
    ...
    lay::InstFinder finder(
        true, view()->is_editable() && m_top_level_sel,
        view()->is_editable() /*full arrays in editable mode*/,
        true /*enclose_inst*/, exclude, true /*visible layers*/);

    //  go through all cell views
    std::set<std::pair<db::DCplxTrans, int>> variants =
        view()->cv_transform_variants();
    for (std::set<std::pair<db::DCplxTrans, int>>::const_iterator v =
             variants.begin();
         v != variants.end(); ++v) {
    ...
    lay::ShapeFinder finder(true, view()->is_editable() && m_top_level_sel,
                            m_flags, exclude);
...
bool Service::transient_select(const db::DPoint &pos) {
  ...
  if (m_cell_inst_service) {
    ...
    lay::InstFinder finder(
        true, view()->is_editable() && m_top_level_sel,
        view()->is_editable() /*full arrays in editable mode*/,
        true /*enclose instances*/, &m_previous_selection,
        true /*visible layers only*/);
    ...
    if (r != finder.end()) {
      ...
      db::ICplxTrans gt = db::VCplxTrans(1.0 / dbu) *
                          db::DCplxTrans(m_move_trans) * db::CplxTrans(dbu) *
                          cv.context_trans() * r->trans();
      ...
      std::vector<db::DCplxTrans> tv =
          mp_view->cv_transform_variants(r->cv_index());
      if (view()->is_editable()) {
        ...
        lay::InstanceMarker *marker =
            new lay::InstanceMarker(view(), r->cv_index());
#endif
        ...
        //  In viewer mode, individual instances of arrays can be selected.
        //  Since that is not supported by InstanceMarker, we just indicate the
        //  individual instance's bounding box.
        lay::Marker *marker = new lay::Marker(view(), r->cv_index());
        db::box_convert<db::CellInst> bc(cv->layout());
        marker->set(bc(r->back().inst_ptr.cell_inst().object()),
                    gt * r->back().inst_ptr.cell_inst().complex_trans(
                             *r->back().array_inst),
                    tv);
    ...
    if (r != finder.end()) {
      ...
      marker->set(r->shape(), gt,
                  mp_view->cv_transform_variants(r->cv_index(), r->layer()));
...
static std::string path_to_string(const db::Layout &layout,
                                  const lay::ObjectInstPath &p) {
  std::string r;
  ...
  if (selection->size() == 1) {
    ...
    if (m_cell_inst_service) {
      ...
      if (!inst.is_regular_array(a, b, amax, bmax)) {
        msg += tl::sprintf(tl::to_string(tr("instance(\"%s\" %s)")),
                           layout.display_name(inst.cell_index()),
                           inst.complex_trans().to_string());
      } else {
        msg += tl::sprintf(tl::to_string(tr("instance(\"%s\" %s %ldx%ld)")),
                           layout.display_name(inst.cell_index()),
                           inst.complex_trans().to_string(), amax, bmax);
      }
      ...
      if (r->shape().is_box()) {
        ...
        msg += tl::sprintf(tl::to_string(tr("box(%d,%d %d,%d)")), int(b.left()),
                           int(b.bottom()), int(b.right()), int(b.top()));
      } else if (r->shape().is_text()) {
        msg += tl::sprintf(tl::to_string(tr("text(\"%s\" %s)")),
                           tl::escape_string(r->shape().text_string()),
                           r->shape().text_trans().to_string());
      } else if (r->shape().is_polygon()) {
        size_t npoints = 0;
        for (db::Shape::polygon_edge_iterator e = r->shape().begin_edge();
             !e.at_end(); ++e) {
          ++npoints;
        }
        ...
        size_t npoints = 0;
        for (db::Shape::point_iterator p = r->shape().begin_point();
             p != r->shape().end_point(); ++p) {
          ++npoints;
        }
        msg += tl::sprintf(tl::to_string(tr("path(w=%d #points=%lu)")),
                           int(r->shape().path_width()), npoints);
      }
      ...
      if (!msg.empty()) {
        ...
        for (lay::LayerPropertiesConstIterator lp = view()->begin_layers();
             !lp.at_end(); ++lp) {
          if (lp->layer_index() == int(r->layer()) &&
              lp->cellview_index() == int(r->cv_index())) {
...
void Service::transient_to_selection() {
  if (!m_transient_selection.empty()) {
    m_selection.insert(m_transient_selection.begin(),
                       m_transient_selection.end());
    selection_to_view();
  }
}
...
bool Service::select(const db::DBox &box, lay::Editable::SelectionMode mode) {
  ...
  if (box.empty()) {
    ...
    lay::InstFinder finder(
        box.is_point(), view()->is_editable() && m_top_level_sel,
        view()->is_editable() /*full arrays in editable mode*/,
        true /*enclose_inst*/, exclude, true /*only visible layers*/);
    ...
    lay::ShapeFinder finder(box.is_point(),
                            view()->is_editable() && m_top_level_sel, m_flags,
                            exclude);

    //  go through all visible layers of all cellviews
    finder.find(view(), search_box);

    //  guiding shapes are only selected in point selection mode and even then,
    //  we only select the first shape
    lay::ShapeFinder::iterator f0 = finder.begin();
    if (box.is_point() && f0 != finder.end() &&
        f0->layer() ==
            view()->cellview(f0->cv_index())->layout().guiding_shape_layer()) {
      ...
      //  collect the founds from the finder
      for (lay::ShapeFinder::iterator f = finder.begin(); f != finder.end();
           ++f) {
        if (f->layer() !=
...
void Service::get_selection(std::vector<lay::ObjectInstPath> &sel) const {
  ...
  for (std::set<lay::ObjectInstPath>::const_iterator r = m_selection.begin();
       r != m_selection.end(); ++r) {
    sel.push_back(*r);
  }
}

bool Service::select(const lay::ObjectInstPath &obj,
                     lay::Editable::SelectionMode mode) {
  ...
  if (mode == lay::Editable::Replace || mode == lay::Editable::Add) {
    ...
    if (m_selection.find(obj) == m_selection.end()) {
      ...
      (const_cast<lay::ObjectInstPath &>(*o))
          .set_seq(m_seq); // we can do that since the sequence number is not
                           // part of the less operator
      selection_to_view();
      return true;
    }
...
void Service::move_markers(const db::DTrans &t) {
  if (m_move_trans != t) {
    ...
    if (has_selection()) {
      std::string pos = std::string("dx: ") +
                        tl::micron_to_string(t.disp().x()) +
                        "  dy: " + tl::micron_to_string(t.disp().y());
      if (t.rot() != 0) {
    ...
    for (std::vector<lay::ViewObject *>::iterator r = m_markers.begin();
      ...
      lay::GenericMarkerBase *marker =
          dynamic_cast<lay::GenericMarkerBase *>(*r);
      if (marker) {
        db::DCplxTrans dt =
            db::DCplxTrans(t) * db::DCplxTrans(m_move_trans).inverted();
...
void Service::tap(const db::DPoint & /*initial*/) {
  //  .. nothing here ..
}

void Service::selection_to_view() {
  //  we don't handle the transient selection properly, so clear it for safety
  //  reasons
  clear_transient_selection();
...
void Service::do_selection_to_view() {
  ...
  for (std::set<lay::ObjectInstPath>::iterator r = m_selection.begin();
       r != m_selection.end(); ++r) {
    if (!r->is_valid(view())) {
...
void Service::set_selection(
    std::vector<lay::ObjectInstPath>::const_iterator s1,
    std::vector<lay::ObjectInstPath>::const_iterator s2) {
  m_selection.clear();
...
Service::handle_guiding_shape_changes(const lay::ObjectInstPath &obj) const {
  ...
  const db::PropertiesRepository::properties_set &input_props =
      layout->properties_repository().properties(obj.shape().prop_id());
  ...
  //  after that.
  if (!get_parameters_from_pcell_and_guiding_shapes(layout, obj.cell_index(),
...
bool Service::handle_guiding_shape_changes() {
  ...
  std::pair<bool, lay::ObjectInstPath> gs =
      handle_guiding_shape_changes(*m_selection.begin());
  if (gs.first) {
