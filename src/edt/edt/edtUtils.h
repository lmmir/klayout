...
/**
 *  @brief Fetch PCell parameters from a cell and merge the guiding shapes into
 * them
 *
 ...
 */
class ClipboardData : public db::ClipboardData {
public:
  ClipboardData() {}
};

/**
 *  @brief A cache for the transformation variants for a certain layer and cell
 * view index for a lay::LayoutView
 */
class TransformationVariants {
public:
  TransformationVariants(const lay::LayoutViewBase *view,
                         bool per_cv_and_layer = true, bool per_cv = true);

  const std::vector<db::DCplxTrans> *per_cv_and_layer(unsigned int cv,
                                                      unsigned int layer) const;
  const std::vector<db::DCplxTrans> *per_cv(unsigned int cv) const;

private:
  std::map<unsigned int, std::vector<db::DCplxTrans>> m_per_cv_tv;
  std::map<std::pair<unsigned int, unsigned int>, std::vector<db::DCplxTrans>>
      m_per_cv_and_layer_tv;
};

/**
 *  @brief An iterator for the selected objects of all edt services in a layout
 * view
 */
...
public:
  ...
  /**
   *  @brief Creates a new iterator iterating over all selected edt objects from
   * the given view
   *
   *  If "including_transient" is true, the transient selection will be used as
   * fallback.
   */
   ...
   */
  bool is_transient() const { return m_transient_mode; }
   ...
   */
  const lay::ObjectInstPath *operator->() const { return &operator*(); }
