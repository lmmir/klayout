...
public:
   ...
   */
  Constraint() : m_mode(Free) {}

  /**
   *  @brief Construct an "fixed" constraint
   */
  Constraint(int) : m_mode(Fixed) {}

  /**
   *  @brief Construct an "1-dimensional" constraint
   *
   *  @param axis Gives the direction in which the point can move, if 0 same as
   * fixed constraint
   */
  Constraint(db::Vector axis) : m_axis(axis) {
    if (axis == db::Vector()) {
      m_mode = Fixed;
    } else {
      m_mode = OneDim;
   ...
   *  @brief Type accessor
   */
  constraint_mode mode() const { return m_mode; }

  /**
   *  @brief Add constraints
   *
   *  Merging of constraints means to allow movement additionally in the same
   * directions than given by the second constraint.
   */
   ...
   *
   *  The movement is performed "as far as possible", i.e. projecting the axis
   * to the move vector, not vice versa.
   *
   *  @param p The point to move
   *  @param v The vector by which to move (desired direction)
   *  @return first: true, if the move was successful, second: the resulting
   * point
   */
  std::pair<bool, db::Point> move(const db::Point &p, const db::DVector &v) {
    if (v == db::DVector()) {
      ...
      //  check for overflow and return false if that happens
      db::DPoint dp =
          db::DPoint(p) + db::DVector(m_axis) * (v.sq_double_length() / proj);
      if (dp.x() <= double(std::numeric_limits<db::Point::coord_type>::min()) ||
          dp.x() >= double(std::numeric_limits<db::Point::coord_type>::max()) ||
          dp.y() <= double(std::numeric_limits<db::Point::coord_type>::min()) ||
          dp.y() >= double(std::numeric_limits<db::Point::coord_type>::max())) {
        return std::make_pair(false, p);
      } else {
   ...
   */
  template <class T> Constraint &transform(const T &t) {
    m_axis.transform(t);
    return *this;
  }
   ...
   */
  template <class T> Constraint transformed(const T &t) const {
    Constraint c(*this);
...
static bool insert_point_path(const db::Path &p,
                              const std::set<EdgeWithIndex> &sel,
                              db::Point &ins, db::Path &new_path) {
  ...
  for (db::Path::iterator pt = p.begin(); pt != p.end(); ++n) {
    ...
    if (pt != p.end()) {
      ...
      ctr.push_back(p1);
      if (!found &&
          sel.find(EdgeWithIndex(db::Edge(p1, p2), n, n + 1, 0)) != sel.end()) {
        //  project the point onto the edge
        db::Edge e(p1, p2);
        std::pair<bool, db::Point> projected = e.projected(ins);
        if (projected.first) {
          if (e.is_ortho()) {
            //  NOTE: for skew edges we use the original point as the projected
            //  one usually is off-grid.
...
static void remove_redundant_points(std::vector<db::Point> &ctr, bool cyclic) {
...
static db::Path del_points_path(const db::Path &p,
                                const std::set<EdgeWithIndex> &sel) {
  db::Path new_path;
  new_path.width(p.width());
  new_path.round(p.round());
...
static void modify_path(db::Path &p,
...
bool insert_point_poly(const db::Polygon &p, const std::set<EdgeWithIndex> &sel,
                       db::Point &ins, db::Polygon &new_poly) {
  for (unsigned int c = 0; c < p.holes() + 1; ++c) {
    ...
    for (db::Shape::polygon_edge_iterator e = p.begin_edge(c); !e.at_end();
         e = ee, ++n) {
      ...
      if (!found && sel.find(EdgeWithIndex(*e, n, nn, c)) != sel.end()) {
        //  project the point onto the edge - use the first edge the point
        //  projects to
        std::pair<bool, db::Point> projected = (*e).projected(ins);
        if (projected.first) {
          if ((*e).is_ortho()) {
    ...
    if (found) {
      ...
      if (c == 0) {
        ...
        new_poly.assign_hole(c - 1, ctr.begin(), ctr.end(),
...
static db::Polygon del_points_poly(const db::Polygon &p,
                                   const std::set<EdgeWithIndex> &sel) {
...
static void modify_polygon(
    db::Polygon &p, const std::map<PointWithIndex, db::Point> &new_points,
    const std::map<EdgeWithIndex, db::Edge> &new_edges, bool compress = false) {
  for (unsigned int c = 0; c < p.holes() + 1; ++c) {
...
static void constrain(std::map<PointWithIndex, Constraint> &constr,
                      const EdgeWithIndex &edge) {
  constr.insert(std::make_pair(edge.pi1(), Constraint())).first->second *=
      Constraint(edge.d());
  constr.insert(std::make_pair(edge.pi2(), Constraint())).first->second *=
      Constraint(edge.d());
}

static void create_shift_sets(const db::Shape &shape,
                              const std::set<EdgeWithIndex> &sel,
  ...
  for (std::set<EdgeWithIndex>::const_iterator e = sel.begin(); e != sel.end();
       ++e) {
    if (e->p1() != e->p2()) {
      new_edges.insert(std::make_pair(*e, db::Edge(*e)));
    } else {
      new_points.insert(
          std::make_pair(PointWithIndex(e->p1(), e->n, e->c), e->p1()));
    }
  }

  //  new_points should only contain the selected points, not the start and end
  ...
  if (shape.is_polygon()) {
    ...
    for (unsigned int c = 0; c < shape.holes() + 1; ++c) {
      ...
      unsigned int n = 0;
      db::Shape::polygon_edge_iterator ee;
      for (db::Shape::polygon_edge_iterator e = shape.begin_edge(c);
           !e.at_end(); e = ee, ++n) {
        ee = e;
        ++ee;
        unsigned int nn = ee.at_end() ? 0 : n + 1;
        if ((*e).p1() != (*e).p2() &&
            sel.find(EdgeWithIndex(*e, n, nn, c)) == sel.end()) {
    ...
    unsigned int n = 0;
    db::Shape::polygon_edge_iterator ee;
    for (db::Shape::polygon_edge_iterator e = poly.begin_edge(); !e.at_end();
         e = ee, ++n) {
      ee = e;
      ++ee;
      unsigned int nn = ee.at_end() ? 0 : n + 1;
  ...
  for (std::map<PointWithIndex, db::Point>::iterator np = new_points.begin();
       np != new_points.end(); ++np) {
    np->second += mv;
  }

  //  The edges are treated somewhat more elaborate:
  for (std::map<EdgeWithIndex, db::Edge>::iterator ne = new_edges.begin();
       ne != new_edges.end(); ++ne) {
    ...
    if (ne->first.d() != db::Vector()) {
      nmv = db::DVector(mv) -
            db::DVector(ne->first.d()) * (double(db::sprod(mv, ne->first.d())) /
                                          ne->first.d().sq_double_length());
    }
    ...
    //    ( commented out: 2. inverted (the direction has changed) or the length
    //    grows 4x larger than the move distance )
    ...
    if (c1 != point_constr.end() && c2 != point_constr.end() &&
        (/* db::sprod_sign (ve, vo) <= 0 || */ (ve - vo).double_length() >
         4.0 * nmv.double_length())) {
    ...
    //  insert the end points into the point list in order to find them by
    //  looking up a point alone
    new_points.insert(std::make_pair(ne->first.pi1(), db::Point()))
        .first->second = p1;
    new_points.insert(std::make_pair(ne->first.pi2(), db::Point()))
        .first->second = p2;
  }
}
 ...
 */
class PartialShapeFinder : public lay::ShapeFinder {
public:
  typedef std::vector<
      std::pair<lay::ObjectInstPath, std::vector<EdgeWithIndex>>>
      founds_vector_type;
  typedef founds_vector_type::const_iterator iterator;

  PartialShapeFinder(bool point_mode, bool top_level_sel,
...
PartialShapeFinder::PartialShapeFinder(bool point_mode, bool top_level_sel,
                                       db::ShapeIterator::flags_type flags)
    : lay::ShapeFinder(point_mode, top_level_sel, flags, 0) {
  set_test_count(point_sel_tests);
}

void PartialShapeFinder::visit_cell(const db::Cell &cell,
                                    const db::Box &hit_box,
                                    const db::Box &scan_box,
                                    const db::DCplxTrans &vp,
                                    const db::ICplxTrans &t, int /*level*/) {
  if (!point_mode()) {

    for (std::vector<int>::const_iterator l = layers().begin();
         l != layers().end(); ++l) {

      if (layers().size() == 1 ||
          (layers().size() > 1 &&
           cell.bbox((unsigned int)*l).touches(scan_box))) {

        checkpoint();

        const db::Shapes &shapes = cell.shapes(*l);

        db::ShapeIterator shape = shapes.begin_touching(
            scan_box, flags(), prop_sel(), inv_prop_sel());
        while (!shape.at_end()) {
          ...
          //  in box mode, select the edges depending on whether an endpoint is
          ...
          if (shape->is_polygon()) {
            ...
            for (unsigned int c = 0; c < shape->holes() + 1; ++c) {
              ...
              for (db::Shape::polygon_edge_iterator e = shape->begin_edge(c);
                ...
                if (hit_box.contains((*e).p1())) {
                  edges.push_back(
                      EdgeWithIndex(db::Edge((*e).p1(), (*e).p1()), n, n, c));
            ...
            db::Point pl;
            unsigned int n = 0;
            for (db::Shape::point_iterator pt = shape->begin_point();
                 pt != shape->end_point(); ++pt, ++n) {
              if (hit_box.contains(*pt)) {
                edges.push_back(EdgeWithIndex(db::Edge(*pt, *pt), n, n, 0));
                if (pl_set && hit_box.contains(pl)) {
                  edges.push_back(
                      EdgeWithIndex(db::Edge(pl, *pt), n - 1, n, 0));
                }
              }
              pl = *pt;
              pl_set = true;
            ...
            unsigned int n = 0;
            db::Shape::polygon_edge_iterator ee;
            for (db::Shape::polygon_edge_iterator e = poly.begin_edge();
                 !e.at_end(); e = ee, ++n) {
              ...
              if (hit_box.contains((*e).p1())) {
                edges.push_back(
                    EdgeWithIndex(db::Edge((*e).p1(), (*e).p1()), n, n, 0));
                if (hit_box.contains((*e).p2())) {
            ...
            if (text_info() && !text_info()->point_mode()) {
              ...
              shape->text(text);
              db::Box tb =
                  t_dbu.inverted() * text_info()->bbox(t_dbu * text, vp);
    ...
    for (std::vector<int>::const_iterator l = layers().begin();
      ...
      if (layers().size() == 1 ||
          (layers().size() > 1 &&
           cell.bbox((unsigned int)*l).touches(hit_box))) {
        ...
        for (int pass = 0; pass < 2 && !any; ++pass) {
          ...
          while (!shape.at_end()) {
            ...
            if (shape->is_polygon()) {
              ...
              for (unsigned int c = 0; c < shape->holes() + 1; ++c) {
                ...
                for (db::Shape::polygon_edge_iterator e = shape->begin_edge(c);
                  ...
                  if (r) {
                    edge_sel.clear();
                    if ((r & 1) != 0) {
                      edge_sel.push_back(EdgeWithIndex(
                          db::Edge((*e).p1(), (*e).p1()), n, n, c));
                    }
                    if ((r & 2) != 0) {
                      edge_sel.push_back(EdgeWithIndex(
                          db::Edge((*e).p2(), (*e).p2()), nn, nn, c));
                    }
                    if (r == 3) {
              ...
              if (pt != shape->end_point()) {
                ...
                for (; pt != shape->end_point(); ++pt, ++n) {
                  unsigned int r =
                      test_edge(t, db::Edge(p, *pt), pass == 0, d, match);
                  if (r) {
                    edge_sel.clear();
                    if ((r & 1) != 0) {
                      edge_sel.push_back(
                          EdgeWithIndex(db::Edge(p, p), n, n, 0));
                    }
                    if ((r & 2) != 0) {
                      edge_sel.push_back(
                          EdgeWithIndex(db::Edge(*pt, *pt), n + 1, n + 1, 0));
                    }
                    if (r == 3) {
                      edge_sel.push_back(
                          EdgeWithIndex(db::Edge(p, *pt), n, n + 1, 0));
                    }
                  }
                  p = *pt;
                }
              ...
              if (text_info() && !text_info()->point_mode()) {
                ...
                db::Text text;
                shape->text(text);
                db::Box tb(t_dbu.inverted() *
                           text_info()->bbox(t_dbu * text, vp));
            ...
            if (match && closer(d)) {
              ...
              //  in point mode just store that found that has the least
              //  "distance"
              if (m_founds.empty()) {
                m_founds.push_back(founds_vector_type::value_type());
              }
...
PartialService::PartialService(db::Manager *manager, lay::LayoutViewBase *view,
                               lay::Dispatcher *root)
    :
#if defined(HAVE_QT)
      QObject(),
#endif
      lay::EditorServiceBase(view), db::Object(manager), mp_view(view),
      mp_root(root), m_dragging(false), m_keep_selection(true), mp_box(0),
      m_color(0), m_buttons(0), m_connect_ac(lay::AC_Any),
      m_move_ac(lay::AC_Any), m_alt_ac(lay::AC_Global), m_snap_to_objects(true),
      m_top_level_sel(false), m_hover(false), m_hover_wait(false),
  ...
  mp_view->geom_changed_event.add(this,
                                  &edt::PartialService::selection_to_view);
}
...
lay::angle_constraint_type PartialService::connect_ac() const {
  //  m_alt_ac (which is set from mouse buttons) can override the specified
  //  connect angle constraint
  return m_alt_ac != lay::AC_Global ? m_alt_ac : m_connect_ac;
}

lay::angle_constraint_type PartialService::move_ac() const {
  //  m_alt_ac (which is set from mouse buttons) can override the specified move
  //  angle constraint
  return m_alt_ac != lay::AC_Global ? m_alt_ac : m_move_ac;
}

void PartialService::deactivated() { clear_partial_transient_selection(); }

void PartialService::activated() {
  //  .. nothing yet ..
}

void PartialService::hover_reset() {
  if (m_hover_wait) {
#if defined(HAVE_QT)
    m_timer.stop();
#endif
...
void PartialService::timeout() {
  ...
  db::DBox search_box =
      db::DBox(m_hover_point, m_hover_point).enlarged(db::DVector(l, l));
  ...
  if (finder.begin() != finder.end()) {
    ...
    transient_selection.insert(
        std::make_pair(finder.begin()->first,
                       std::set<EdgeWithIndex>(finder.begin()->second.begin(),
                                               finder.begin()->second.end())));
    ...
    //  compute the global transformation including context and explicit
    //  transformation
    db::ICplxTrans gt = (cv.context_trans() * r->first.trans());

    if (!r->first.is_cell_inst()) {

      const std::vector<db::DCplxTrans> *tv_list =
          tv.per_cv_and_layer(r->first.cv_index(), r->first.layer());
      if (tv_list && !tv_list->empty()) {
        ...
        if (r->first.shape().is_polygon()) {
          ...
          for (unsigned int c = 0; c < r->first.shape().holes() + 1; ++c) {
            ...
            unsigned int n = 0;
            db::Shape::polygon_edge_iterator ee;
            for (db::Shape::polygon_edge_iterator e =
                     r->first.shape().begin_edge(c);
                 !e.at_end(); e = ee, ++n) {
              ee = e;
              ++ee;
              unsigned int nn = ee.at_end() ? 0 : n + 1;
              enter_edge(EdgeWithIndex(*e, n, nn, c), n_marker, r, new_points,
                         new_edges, gt, *tv_list, true);
            }
          }
          ...
          if (r->first.shape().begin_point() != r->first.shape().end_point()) {
            ...
            while (true) {
              ...
              enter_edge(EdgeWithIndex(db::Edge(p1, *pt), n, n + 1, 0),
                         n_marker, r, new_points, new_edges, gt, *tv_list,
                         true);
          ...
          for (db::Shape::polygon_edge_iterator e = poly.begin_edge();
            ...
            ++ee;
            unsigned int nn = ee.at_end() ? 0 : n + 1;
            enter_edge(EdgeWithIndex(*e, n, nn, 0), n_marker, r, new_points,
                       new_edges, gt, *tv_list, true);
          }
          ...
          enter_edge(EdgeWithIndex(db::Edge(tp, tp), 0, 0, 0), n_marker, r,
                     new_points, new_edges, gt, *tv_list, true);
      ...
      const std::vector<db::DCplxTrans> *tv_list =
          tv.per_cv(r->first.cv_index());
      if (tv_list && !tv_list->empty()) {
        lay::InstanceMarker *marker =
            new_inst_marker(n_inst_marker, r->first.cv_index(), true);
...
void PartialService::clear_partial_transient_selection() {
  mp_view->clear_transient_selection();
...
void PartialService::set_colors(tl::Color /*background*/, tl::Color color) {
  m_color = color.rgb();
  if (mp_box) {
...
void PartialService::menu_activated(const std::string & /*symbol*/) {
  // ...
}

bool PartialService::configure(const std::string &name,
...
void PartialService::config_finalize() {
  // ...
}

db::DPoint PartialService::snap(const db::DPoint &p) const {
...
db::DVector PartialService::snap(const db::DVector &v_org) const {
...
lay::PointSnapToObjectResult PartialService::snap2(const db::DPoint &p) const {
  double snap_range = ui()->mouse_event_trans().inverted().ctrans(sr_pixels);
  return lay::obj_snap(m_snap_to_objects ? view() : 0, m_start, p,
                       m_edit_grid == db::DVector() ? m_global_grid
                                                    : m_edit_grid,
                       move_ac(), snap_range);
}

void PartialService::transform(const db::DCplxTrans &tr) {
...
void PartialService::transform_selection(const db::DTrans &move_trans) {
  //  build the transformation variants cache
  TransformationVariants tv(view());
  ...
  for (partial_objects::iterator r = m_selection.begin();
       r != m_selection.end(); ++r) {
    if (!r->first.is_cell_inst()) {
      const std::vector<db::DCplxTrans> *tv_list =
          tv.per_cv_and_layer(r->first.cv_index(), r->first.layer());
      if (tv_list && !tv_list->empty()) {
        sel_per_shape
            .insert(std::make_pair(r->first.shape(),
                                   std::vector<partial_objects::iterator>()))
            .first->second.push_back(r);
      }
    }
  }

  for (std::map<db::Shape, std::vector<partial_objects::iterator>>::iterator
           sps = sel_per_shape.begin();
       sps != sel_per_shape.end(); ++sps) {

    db::Shape shape = sps->first;

    for (std::vector<partial_objects::iterator>::const_iterator rr =
             sps->second.begin();
         rr != sps->second.end(); ++rr) {
      ...
      //  TODO: clarify how this can be implemented in a more generic form or
      //  leave it thus.
      ...
      db::Vector move_vector =
          db::Vector((tt.inverted() * db::DCplxTrans(move_trans) * tt).disp());
      ...
      db::Shapes &shapes =
          cv->layout().cell(r->first.cell_index()).shapes(r->first.layer());
      ...
      if (shape.is_polygon()) {
        ...
        db::Point tp(shape.text_trans() * db::Point());
        std::map<PointWithIndex, db::Point>::const_iterator np =
            new_points.find(PointWithIndex(tp, 0, 0));
        ...
        std::map<PointWithIndex, db::Point>::const_iterator np =
            new_points.find(PointWithIndex(p, 0, 0));
      ...
      for (std::set<EdgeWithIndex>::const_iterator s = new_sel.begin();
           s != new_sel.end() && m_keep_selection; ++s) {
        if (s->p1() == s->p2()) {
          std::map<PointWithIndex, db::Point>::const_iterator np =
              new_points.find(s->pi1());
          if (np != new_points.end()) {
            r->second.insert(EdgeWithIndex(db::Edge(np->second, np->second),
                                           s->n, s->n, s->c));
          } else {
            r->second.insert(*s);
          }
    ...
    if (shape != sps->first) {
      ...
      for (std::vector<partial_objects::iterator>::const_iterator rr =
        ...
        m_selection.insert(std::make_pair(inst_path, std::set<EdgeWithIndex>()))
            .first->second.swap(sel);
      }
    }
  }
  ...
  std::map<std::pair<db::cell_index_type, unsigned int>,
           std::vector<partial_objects::const_iterator>>
      insts_by_cell;
  for (partial_objects::const_iterator r = m_selection.begin();
       r != m_selection.end(); ++r) {
    if (r->first.is_cell_inst()) {
      insts_by_cell
          .insert(std::make_pair(
              std::make_pair(r->first.cell_index(), r->first.cv_index()),
              std::vector<partial_objects::const_iterator>()))
          .first->second.push_back(r);
    }
  }
  ...
  for (std::map<std::pair<db::cell_index_type, unsigned int>,
                std::vector<partial_objects::const_iterator>>::const_iterator
           ibc = insts_by_cell.begin();
       ibc != insts_by_cell.end(); ++ibc) {

    insts_to_transform.clear();
    insts_to_transform.reserve(ibc->second.size());
    for (std::vector<partial_objects::const_iterator>::const_iterator i =
             ibc->second.begin();
         i != ibc->second.end(); ++i) {
      insts_to_transform.push_back(
          std::make_pair((*i)->first.back().inst_ptr, (*i)->first.trans()));
    }
    ...
    if (cv.is_valid()) {
      ...
      if (tv_list && !tv_list->empty()) {
        ...
        db::CplxTrans tt = (*tv_list)[0] * db::CplxTrans(cv->layout().dbu()) *
                           cv.context_trans();
        db::ICplxTrans move_trans_dbu(tt.inverted() *
                                      db::DCplxTrans(move_trans) * tt);

        std::sort(insts_to_transform.begin(), insts_to_transform.end());
        std::vector<std::pair<db::Instance, db::ICplxTrans>>::const_iterator
            unique_end = std::unique(insts_to_transform.begin(),
                                     insts_to_transform.end());
        db::Cell &cell = cv->layout().cell(ibc->first.first);
        for (std::vector<
                 std::pair<db::Instance, db::ICplxTrans>>::const_iterator inst =
                 insts_to_transform.begin();
             inst != unique_end; ++inst) {

          db::ICplxTrans mt(inst->second.inverted() * move_trans_dbu *
                            inst->second);
...
void PartialService::edit_cancel() {
...
bool PartialService::wheel_event(int /*delta*/, bool /*horizontal*/,
                                 const db::DPoint & /*p*/,
...
bool PartialService::mouse_move_event(const db::DPoint &p, unsigned int buttons,
                                      bool prio) {
  clear_mouse_cursors();
  ...
  if (m_dragging) {
    ...
    if (is_single_point_selection() || is_single_edge_selection()) {
      ...
      //  for a single selected point or edge, m_start is the original position
      //  and we snap the target - thus, we can bring the point on grid or to an
      //  object's edge or vertex
...
static db::DPoint projected_to_edge(const db::DEdge &edge,
                                    const db::DPoint &p) {
...
bool PartialService::mouse_press_event(const db::DPoint &p,
                                       unsigned int buttons, bool prio) {
  hover_reset();
  ...
  if (m_dragging) {
    ...
    if (m_selection.empty() || ((buttons & lay::ShiftButton) != 0) ||
        ((buttons & lay::ControlButton) != 0)) {

      //  if nothing was selected by this point or Ctrl or Shift was pressed,
      ...
      if (is_single_point_selection()) {
        //  for a single selected point we use the original point as the start
        //  location which allows bringing it to grid
        m_current = m_start = single_selected_point();
      } else if (is_single_edge_selection()) {
        //  for an edge selection use the point projected to edge as the start
        //  location which allows bringing it to grid
...
bool PartialService::mouse_click_event(const db::DPoint &p,
                                       unsigned int buttons, bool prio) {
  hover_reset();
  ...
  if (m_dragging) {
    ...
    if (m_current != m_start) {
      ...
      //  heuristically, if there is just one edge selected: do not confine to
      //  the movement angle constraint - the edge usually is confined enough
    ...
    try {
      ...
      //  check, if there is a selected shape under the mouse - in this case, we
      //  do not do a new selection
      PartialShapeFinder finder(true /*point mode*/, m_top_level_sel,
                                db::ShapeIterator::All);
      finder.find(view(), search_box);

      //  check, if there is a selected shape under the mouse - in this case, we
      //  do not do a new selection
      lay::InstFinder inst_finder(true /*point mode*/, m_top_level_sel,
                                  true /*full arrays*/, true /*enclose*/,
                                  0 /*no excludes*/, true /*visible layers*/);
      inst_finder.find(view(), search_box);

      //  collect the founds from the finder
      //  consider a new selection if new objects are selected or the current
      //  selection is shape-only (this may happen if points have been inserted)
      bool new_selection = ((finder.begin() == finder.end() &&
                             inst_finder.begin() == inst_finder.end()) ||
                            mode != lay::Editable::Replace);

      for (PartialShapeFinder::iterator f = finder.begin();
           f != finder.end() && !new_selection; ++f) {
        partial_objects::const_iterator sel = m_selection.find(f->first);
        new_selection = true;
        if (sel != m_selection.end()) {
          for (std::vector<edt::EdgeWithIndex>::const_iterator e =
                   f->second.begin();
               e != f->second.end() && new_selection; ++e) {
      ...
      if (finder.begin() == finder.end()) {
        ...
        for (lay::InstFinder::iterator f = inst_finder.begin();
             f != inst_finder.end() && !new_selection; ++f) {
      ...
      if (new_selection) {
        ...
        //  clear the selection if we now select a guiding shape or if it was
        //  consisting of a guiding shape before (that way we ensure there is
        //  only a guiding shape selected)
        PartialShapeFinder::iterator f0 = finder.begin();
        if (f0 != finder.end() &&
            f0->first.layer() == view()
                                     ->cellview(f0->first.cv_index())
                                     ->layout()
                                     .guiding_shape_layer()) {
          m_selection.clear();
        } else {
          partial_objects::const_iterator s0 = m_selection.begin();
          if (s0 != m_selection.end() &&
              s0->first.layer() == view()
                                       ->cellview(s0->first.cv_index())
                                       ->layout()
                                       .guiding_shape_layer()) {
            m_selection.clear();
          }
        ...
        //  collect the founds from the finder
        for (PartialShapeFinder::iterator f = finder.begin(); f != finder.end();
             ++f) {
          ...
          if (mode == lay::Editable::Replace || mode == lay::Editable::Add) {
            ...
            if (sel == m_selection.end()) {
              sel = m_selection
                        .insert(
                            std::make_pair(f->first, std::set<EdgeWithIndex>()))
                        .first;
            }
            ...
            if (m_selection.find(f->first) != m_selection.end()) {
              ...
              m_selection
                  .insert(std::make_pair(f->first, std::set<EdgeWithIndex>()))
                  .first->second.insert(f->second.begin(), f->second.end());
            }
          }
        }
      }
...
bool PartialService::mouse_double_click_event(const db::DPoint &p,
                                              unsigned int buttons, bool prio) {
  hover_reset();
  ...
  if ((buttons & lay::LeftButton) != 0 && prio) {
    ...
    if (!m_selection.empty()) {
      ...
      partial_objects::iterator r =
          m_selection.begin(); // we assert above that we have at least one
                               // selected element
      if (!r->first.is_cell_inst()) {
        ...
        //  build the transformation variants cache
        TransformationVariants tv(view(), true /*per cv and layer*/,
                                  false /*per cv*/);
        ...
        if (tv_list && !tv_list->empty()) {
          ...
          db::CplxTrans tt = (*tv_list)[0] * db::CplxTrans(cv->layout().dbu()) *
                             (cv.context_trans() * r->first.trans());
          ...
          if (shape.is_polygon()) {
            ...
            //  convert the box into a polygon unless the shape is on a guiding
            //  shape layer (if it's a guiding shape we must not change it's
            //  nature)

            if (r->first.layer() != view()
                                        ->cellview(r->first.cv_index())
                                        ->layout()
                                        .guiding_shape_layer()) {
...
bool PartialService::mouse_release_event(const db::DPoint &p,
                                         unsigned int buttons, bool prio) {
  hover_reset();
...
bool PartialService::begin_move(MoveMode mode, const db::DPoint &p,
...
void PartialService::move(const db::DPoint &p, lay::angle_constraint_type ac) {
  m_alt_ac = ac;
...
void PartialService::end_move(const db::DPoint & /*p*/,
                              lay::angle_constraint_type ac) {
  m_alt_ac = ac;
  ...
  if (m_current != m_start) {
    ...
    //  heuristically, if there is just one edge selected: do not confine to the
    //  movement angle constraint - the edge usually is confined enough
    db::DTrans move_trans = db::DTrans(m_current - m_start);
...
bool PartialService::has_selection() { return !m_selection.empty(); }

size_t PartialService::selection_size() { return m_selection.size(); }

db::DBox PartialService::selection_bbox() {
  //  build the transformation variants cache
  ...
  for (partial_objects::const_iterator r = m_selection.begin();
    ...
    db::CplxTrans ctx_trans =
        db::CplxTrans(layout.dbu()) * cv.context_trans() * r->first.trans();
    ...
    if (!r->first.is_cell_inst()) {
      ...
      const std::vector<db::DCplxTrans> *tv_list =
          tv.per_cv_and_layer(r->first.cv_index(), r->first.layer());
      if (tv_list != 0) {
        for (std::vector<db::DCplxTrans>::const_iterator t = tv_list->begin();
             t != tv_list->end(); ++t) {
...
bool PartialService::has_transient_selection() {
  //  there is no specific transient selection for the partial editor
  return false;
}

void PartialService::del() {
  ...
  std::map<
      std::pair<db::cell_index_type, std::pair<unsigned int, unsigned int>>,
      std::vector<partial_objects::const_iterator>>
      shapes_to_delete_by_cell;
  ...
  for (partial_objects::iterator r = m_selection.begin();
    ...
    if (!r->first.is_cell_inst()) {
      ...
      if (shape.is_polygon()) {
        ...
        if (new_poly.hull().size() < 3) {
          shapes_to_delete_by_cell
              .insert(std::make_pair(
                  std::make_pair(
                      r->first.cell_index(),
                      std::make_pair(r->first.cv_index(), r->first.layer())),
                  std::vector<partial_objects::const_iterator>()))
              .first->second.push_back(r);
        } else {
        ...
        //  if more than one point is deleted, the box basically collapses, if
        //  one point is deleted nothing changes on the box.
        if (r->second.size() > 1) {
          shapes_to_delete_by_cell
              .insert(std::make_pair(
                  std::make_pair(
  ...
  for (std::map<std::pair<db::cell_index_type,
                          std::pair<unsigned int, unsigned int>>,
                std::vector<partial_objects::const_iterator>>::const_iterator
           sbc = shapes_to_delete_by_cell.begin();
       sbc != shapes_to_delete_by_cell.end(); ++sbc) {
    ...
    if (cv.is_valid()) {
      ...
      if (sbc->first.second.second != cv->layout().guiding_shape_layer()) {
        for (std::vector<partial_objects::const_iterator>::const_iterator s =
                 sbc->second.begin();
             s != sbc->second.end(); ++s) {
          cv->layout()
              .cell(sbc->first.first)
              .shapes(sbc->first.second.second)
              .erase_shape((*s)->first.shape());
        }
      }
    }
  }
  ...
  for (partial_objects::const_iterator r = m_selection.begin();
    ...
    if (r->first.is_cell_inst()) {
      ...
      if (cv.is_valid()) {
        if (cv->layout()
                .cell(r->first.back().inst_ptr.cell_index())
                .is_proxy()) {
          needs_cleanup.insert(&cv->layout());
        }
        cv->layout()
            .cell(r->first.cell_index())
            .erase(r->first.back().inst_ptr);
      }
    }
  }

  //  Hint: calling this method is somewhat dangerous since the selection is not
  //  necessarily valid (the shapes may have been deleted). However, since we
  //  did not delete guiding shapes before and this method in particular handles
  //  guiding shapes, this should be fairly safe.
  handle_guiding_shape_changes();
...
lay::InstanceMarker *PartialService::new_inst_marker(size_t &nmarker,
...
lay::Marker *PartialService::new_marker(size_t &nmarker, unsigned int cv_index,
                                        bool transient) {
  lay::Marker *marker;
...
void PartialService::enter_path(
    db::Path &p, size_t &nmarker, partial_objects::const_iterator sel,
    const std::map<PointWithIndex, db::Point> &new_points,
    const std::map<EdgeWithIndex, db::Edge> &new_edges,
    const db::ICplxTrans &gt, const std::vector<db::DCplxTrans> &tv,
    bool transient) {
...
void PartialService::enter_polygon(
    db::Polygon &p, size_t &nmarker, partial_objects::const_iterator sel,
    const std::map<PointWithIndex, db::Point> &new_points,
    const std::map<EdgeWithIndex, db::Edge> &new_edges,
    const db::ICplxTrans &gt, const std::vector<db::DCplxTrans> &tv,
    bool transient) {
...
void PartialService::enter_vertices(
    size_t &nmarker, partial_objects::const_iterator sel,
    const std::map<PointWithIndex, db::Point> &new_points,
    const std::map<EdgeWithIndex, db::Edge> & /*new_edges*/,
    const db::ICplxTrans &gt, const std::vector<db::DCplxTrans> &tv,
    bool transient) {
  ...
  for (std::set<EdgeWithIndex>::const_iterator e = sel->second.begin();
       e != sel->second.end(); ++e) {

    if (e->p1() == e->p2()) {

      lay::Marker *marker =
          new_marker(nmarker, sel->first.cv_index(), transient);

      db::Point pnew = e->p1();
      std::map<PointWithIndex, db::Point>::const_iterator np =
          new_points.find(PointWithIndex(pnew, e->n, e->c));
      if (np != new_points.end()) {
        pnew = np->second;
      }
...
void PartialService::enter_edge(
    const EdgeWithIndex &e, size_t &nmarker,
  ...
  bool p1_sel = sel->second.find(EdgeWithIndex(db::Edge(ep1, ep1), e.n, e.n,
                                               e.c)) != sel->second.end();
  bool p2_sel = sel->second.find(EdgeWithIndex(db::Edge(ep2, ep2), e.nn, e.nn,
                                               e.c)) != sel->second.end();
  ...
  if (p1_sel || p2_sel || p12_sel) {
    ...
    if (p2_sel && !p12_sel) {
      ...
      db::DEdge ee = db::DEdge(db::DPoint(ep2) +
                                   ((db::DPoint(ep1) - db::DPoint(ep2)) * 0.25),
                               db::DPoint(ep2));
    ...
    if (p1_sel && !p12_sel) {
      ...
      db::DEdge ee = db::DEdge(
          db::DPoint(ep1),
          db::DPoint(ep1) + ((db::DPoint(ep2) - db::DPoint(ep1)) * 0.25));
      marker->set(ee, db::DCplxTrans(gt), tv);
...
double PartialService::catch_distance() {
  return double(view()->search_range()) / ui()->mouse_event_trans().mag();
}

double PartialService::catch_distance_box() {
  return double(view()->search_range_box()) / ui()->mouse_event_trans().mag();
}

db::DPoint PartialService::single_selected_point() const {
  //  build the transformation variants cache and
  //  use only the first one of the explicit transformations
  //  TODO: clarify how this can be implemented in a more generic form or leave
  //  it thus.
  TransformationVariants tv(view());
  const std::vector<db::DCplxTrans> *tv_list =
      tv.per_cv_and_layer(m_selection.begin()->first.cv_index(),
                          m_selection.begin()->first.layer());

  const lay::CellView &cv =
      view()->cellview(m_selection.begin()->first.cv_index());
...
bool PartialService::is_single_point_selection() const {
  return (m_selection.size() == 1 &&
          !m_selection.begin()->first.is_cell_inst() &&
          m_selection.begin()->second.size() == 1 /*p*/);
}

db::DEdge PartialService::single_selected_edge() const {
  //  build the transformation variants cache and
  //  use only the first one of the explicit transformations
  //  TODO: clarify how this can be implemented in a more generic form or leave
  //  it thus.
  ...
  //  pick the edge from the selection (there is: p1, p2 and the edge between
  //  them)
  for (auto s = m_selection.begin()->second.begin();
       s != m_selection.begin()->second.end(); ++s) {
    if (s->n != s->nn) {
      return tt * *s;
    }
  }
...
bool PartialService::is_single_edge_selection() const {
  return (m_selection.size() == 1 &&
          !m_selection.begin()->first.is_cell_inst() &&
          m_selection.begin()->second.size() == 3 /*p1,p2,edge*/);
}

bool PartialService::select(const db::DBox &box, SelectionMode mode) {
...
void PartialService::selection_to_view() { dm_selection_to_view(); }

void PartialService::do_selection_to_view() {
  ...
  if (m_dragging) {
    ...
    //  heuristically, if there is just one edge selected: do not confine to the
    //  movement angle constraint - the edge usually is confined enough
    if (m_selection.size() == 1 && !m_selection.begin()->first.is_cell_inst() &&
        m_selection.begin()->second.size() == 3 /*p1,p2,edge*/) {
    ...
    //  display vector
    view()->message(
        std::string("dx: ") + tl::micron_to_string(move_trans.disp().x()) +
        std::string("  dy: ") + tl::micron_to_string(move_trans.disp().y()) +
        std::string("  d: ") +
        tl::micron_to_string(move_trans.disp().length()));
  }
    ...
    for (partial_objects::const_iterator r = m_selection.begin();
      ...
      if (!r->first.is_cell_inst()) {
        ...
        if (tv_list && !tv_list->empty()) {
          ...
          //  use only the first one of the explicit transformations
          //  TODO: clarify how this can be implemented in a more generic form
          //  or leave it thus.
          db::ICplxTrans gt(cv.context_trans() * r->first.trans());
          db::CplxTrans tt =
              (*tv_list)[0] * db::CplxTrans(cv->layout().dbu()) * gt;
          db::Vector move_vector(tt.inverted() *
                                 (move_trans * (tt * db::Point())));

          //  create the shift sets describing how points and edges are being
          //  moved
          ...
          if (m_dragging) {
            create_shift_sets(r->first.shape(), r->second, new_points,
                              new_edges, move_vector);
          }

          //  create the markers to represent vertices and edges

          enter_vertices(n_marker, r, new_points, new_edges, gt, *tv_list,
                         false);
          ...
          if (r->first.shape().is_polygon()) {
            ...
            //  warning: poly is modified:
            enter_polygon(poly, n_marker, r, new_points, new_edges, gt,
                          *tv_list, false);

          } else if (r->first.shape().is_path()) {

            if (r->first.shape().begin_point() !=
                r->first.shape().end_point()) {
              ...
              //  warning: path is modified:
              enter_path(path, n_marker, r, new_points, new_edges, gt, *tv_list,
                         false);
            }
            ...
            db::Point tp(r->first.shape().text_trans() * db::Point());
            enter_edge(EdgeWithIndex(db::Edge(tp, tp), 0, 0, 0), n_marker, r,
                       new_points, new_edges, gt, *tv_list, false);
        ...
        //  compute the global transformation including movement, context and
        //  explicit transformation
        db::ICplxTrans gt = db::VCplxTrans(1.0 / cv->layout().dbu()) *
                            db::DCplxTrans(move_trans) *
                            db::CplxTrans(cv->layout().dbu());
        ...
        if (tv_list && !tv_list->empty()) {
          lay::InstanceMarker *marker =
              new_inst_marker(n_inst_marker, r->first.cv_index(), false);
          marker->set(r->first.back().inst_ptr, gt, *tv_list);
        }
      }
    }
...
void PartialService::resize_markers(size_t n, bool transient) {
  if (transient) {
    for (std::vector<lay::Marker *>::iterator r =
             m_transient_markers.begin() + n;
         r != m_transient_markers.end(); ++r) {
      delete *r;
    }
    m_transient_markers.erase(m_transient_markers.begin() + n,
                              m_transient_markers.end());
  } else {
    for (std::vector<lay::Marker *>::iterator r = m_markers.begin() + n;
         r != m_markers.end(); ++r) {
      delete *r;
    }
...
void PartialService::resize_inst_markers(size_t n, bool transient) {
  if (transient) {
    for (std::vector<lay::InstanceMarker *>::iterator r =
             m_transient_inst_markers.begin() + n;
         r != m_transient_inst_markers.end(); ++r) {
      delete *r;
    }
    m_transient_inst_markers.erase(m_transient_inst_markers.begin() + n,
                                   m_transient_inst_markers.end());
  } else {
    for (std::vector<lay::InstanceMarker *>::iterator r =
             m_inst_markers.begin() + n;
         r != m_inst_markers.end(); ++r) {
      delete *r;
    }
...
bool PartialService::partial_select(const db::DBox &box,
                                    lay::Editable::SelectionMode mode) {
  clear_partial_transient_selection();
  ...
  if (box.empty()) {
    ...
    PartialShapeFinder finder(box.is_point(), m_top_level_sel,
                              db::ShapeIterator::flags_type(shape_flags));
    finder.find(view(), search_box);

    //  We must make sure that guiding shapes are only selected alone. The first
    //  selected object will determine whether we take guiding shapes into
    //  account or not.
    bool gs_mode = (finder.begin() != finder.end() &&
                    finder.begin()->first.layer() ==
                        view()
                            ->cellview(finder.begin()->first.cv_index())
                            ->layout()
                            .guiding_shape_layer());

    //  Clear the selection if it was consisting of a guiding shape or
    //  non-guiding shape before (depending on mode). This way we make sure
    //  there is not mixture between guiding shapes and others.
    if (m_selection.begin() != m_selection.end()) {
      if (gs_mode != (m_selection.begin()->first.layer() ==
                      view()
                          ->cellview(m_selection.begin()->first.cv_index())
                          ->layout()
                          .guiding_shape_layer())) {
        m_selection.clear();
        needs_update = true;
    ...
    for (PartialShapeFinder::iterator f = finder.begin(); f != finder.end();
      ...
      if (gs_mode == (f->first.layer() == view()
                                              ->cellview(f->first.cv_index())
                                              ->layout()
                                              .guiding_shape_layer())) {
    ...
    //  check, if there is a selected instance inside the box - in this case, we
    //  do not do a new selection
    if (!box.is_point() && edt::instances_enabled()) {

      lay::InstFinder inst_finder(box.is_point(), m_top_level_sel,
                                  true /*full arrays*/, true /*enclose*/,
                                  0 /*no excludes*/, true /*visible layers*/);
      inst_finder.find(view(), search_box);
...
bool PartialService::handle_guiding_shape_changes() {
  ...
  if (s->first.is_cell_inst() ||
      s->first.layer() != layout->guiding_shape_layer()) {
    return false;
  }
  ...
  db::cell_index_type parent_cell =
      std::numeric_limits<db::cell_index_type>::max();
  ...
  db::property_names_id_type pn =
      layout->properties_repository().prop_name_id("name");

  const db::PropertiesRepository::properties_set &input_props =
      layout->properties_repository().properties(s->first.shape().prop_id());
  db::PropertiesRepository::properties_set::const_iterator input_pv =
      input_props.find(pn);
  ...
  //  Hint: get_parameters_from_pcell_and_guiding_shapes invalidates the shapes
  //  because it resets the changed guiding shapes. We must not access s->shape
  //  after that.
  if (!get_parameters_from_pcell_and_guiding_shapes(
          layout, s->first.cell_index(), parameters_for_pcell)) {
    return false;
  }
  ...
  if (parent_cell != std::numeric_limits<db::cell_index_type>::max()) {
    ...
    db::Instance new_inst =
        layout->cell(parent_cell)
            .change_pcell_parameters(parent_inst, parameters_for_pcell);

    //  try to identify the selected shape in the new shapes and select this one
    db::Shapes::shape_iterator sh = layout->cell(new_inst.cell_index())
                                        .shapes(layout->guiding_shape_layer())
                                        .begin(db::ShapeIterator::All);
    while (!sh.at_end()) {
      const db::PropertiesRepository::properties_set &props =
          layout->properties_repository().properties(sh->prop_id());
      db::PropertiesRepository::properties_set::const_iterator pv =
          props.find(pn);
      if (pv != props.end()) {
