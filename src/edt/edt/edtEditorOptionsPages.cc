...
static void configure_from_line_edit(lay::Dispatcher *dispatcher, QLineEdit *le,
...
EditorOptionsGeneric::EditorOptionsGeneric(lay::LayoutViewBase *view,
                                           lay::Dispatcher *dispatcher)
    : EditorOptionsPage(view, dispatcher) {
  mp_ui = new Ui::EditorOptionsGeneric();
  mp_ui->setupUi(this);

  connect(mp_ui->grid_cb, SIGNAL(activated(int)), this,
          SLOT(grid_changed(int)));
  ...
  connect(mp_ui->hier_copy_mode_cbx, SIGNAL(activated(int)), this,
          SLOT(edited()));
  connect(mp_ui->snap_objects_cbx, SIGNAL(clicked()), this, SLOT(edited()));
  connect(mp_ui->max_shapes_le, SIGNAL(editingFinished()), this,
          SLOT(edited()));
...
std::string EditorOptionsGeneric::title() const {
...
void EditorOptionsGeneric::apply(lay::Dispatcher *root) {
  ...
  ACConverter acc;
  root->config_set(cfg_edit_move_angle_mode,
                   acc.to_string(lay::angle_constraint_type(
                       mp_ui->move_angle_cb->currentIndex())));
  root->config_set(cfg_edit_connect_angle_mode,
                   acc.to_string(lay::angle_constraint_type(
                       mp_ui->conn_angle_cb->currentIndex())));

  root->config_set(cfg_edit_top_level_selection,
                   tl::to_string(mp_ui->hier_sel_cbx->isChecked()));
  int cpm = mp_ui->hier_copy_mode_cbx->currentIndex();
  root->config_set(cfg_edit_hier_copy_mode,
                   tl::to_string((cpm < 0 || cpm > 1) ? -1 : cpm));
  root->config_set(cfg_edit_snap_to_objects,
                   tl::to_string(mp_ui->snap_objects_cbx->isChecked()));

  configure_from_line_edit<unsigned int>(root, mp_ui->max_shapes_le,
                                         cfg_edit_max_shapes_of_instances);
  root->config_set(cfg_edit_show_shapes_of_instances,
                   tl::to_string(mp_ui->show_shapes_cbx->isChecked()));
}
...
EditorOptionsText::EditorOptionsText(lay::LayoutViewBase *view,
                                     lay::Dispatcher *dispatcher)
...
std::string EditorOptionsText::title() const {
  return tl::to_string(QObject::tr("Text"));
}

void EditorOptionsText::apply(lay::Dispatcher *root) {
  //  Text string
  root->config_set(cfg_edit_text_string,
                   tl::unescape_string(tl::to_string(mp_ui->text_le->text())));

  //  HAlign
  HAlignConverter hac;
  root->config_set(
      cfg_edit_text_halign,
      hac.to_string(db::HAlign(mp_ui->halign_cbx->currentIndex() - 1)));

  //  VAlign
  VAlignConverter vac;
  root->config_set(
      cfg_edit_text_valign,
      vac.to_string(db::VAlign(mp_ui->valign_cbx->currentIndex() - 1)));
...
EditorOptionsPath::EditorOptionsPath(lay::LayoutViewBase *view,
                                     lay::Dispatcher *dispatcher)
    : lay::EditorOptionsPage(view, dispatcher) {
  mp_ui = new Ui::EditorOptionsPath();
  mp_ui->setupUi(this);

  connect(mp_ui->type_cb, SIGNAL(currentIndexChanged(int)), this,
...
std::string EditorOptionsPath::title() const {
...
void EditorOptionsPath::apply(lay::Dispatcher *root) {
  ...
  if (mp_ui->type_cb->currentIndex() == 0) {
    ...
    configure_from_line_edit<double>(root, mp_ui->start_ext_le,
                                     cfg_edit_path_ext_var_begin);
    configure_from_line_edit<double>(root, mp_ui->end_ext_le,
...
EditorOptionsInst::EditorOptionsInst(lay::LayoutViewBase *view,
                                     lay::Dispatcher *dispatcher)
    : lay::EditorOptionsPage(view, dispatcher) {
  ...
  connect(mp_ui->lib_cbx, SIGNAL(currentIndexChanged(int)), this,
          SLOT(library_changed()));
...
std::string EditorOptionsInst::title() const {
...
void EditorOptionsInst::update_cell_edits() {
  ...
  if (layout->cells() < max_cells) {
    ...
    for (db::Layout::pcell_iterator pc = layout->begin_pcells();
         pc != layout->end_pcells() && size_t(cellnames.size()) < max_cells;
         ++pc) {
  ...
  std::pair<bool, db::pcell_id_type> pc =
      layout->pcell_by_name(tl::to_string(mp_ui->cell_le->text()).c_str());
  std::pair<bool, db::cell_index_type> cc =
      layout->cell_by_name(tl::to_string(mp_ui->cell_le->text()).c_str());
...
void EditorOptionsInst::browse_cell() {
  ...
  if (m_cv_index >= 0 && view()->cellview(m_cv_index).is_valid()) {
    ...
    //  find the layout the cell has to be looked up: that is either the layout
    //  of the current instance or the library selected
    db::Layout *layout = 0;
    db::Library *lib = 0;
    if (mp_ui->lib_cbx->current_library()) {
...
void EditorOptionsInst::apply(lay::Dispatcher *root) {
  //  cell name
  root->config_set(cfg_edit_inst_cell_name,
                   tl::to_string(mp_ui->cell_le->text()));

  //  lib name
  if (mp_ui->lib_cbx->current_library()) {
    root->config_set(cfg_edit_inst_lib_name,
                     mp_ui->lib_cbx->current_library()->get_name());
  } else {
  ...
  configure_from_line_edit<double>(root, mp_ui->column_x_le,
                                   cfg_edit_inst_column_x);
  configure_from_line_edit<double>(root, mp_ui->column_y_le,
                                   cfg_edit_inst_column_y);
...
void EditorOptionsInst::setup(lay::Dispatcher *root) {
  ...
  try {
    ...
    mp_ui->lib_cbx->set_current_library(
        db::LibraryManager::instance().lib_ptr_by_name(l, techname));
...
EditorOptionsInstPCellParam::EditorOptionsInstPCellParam(
    lay::LayoutViewBase *view, lay::Dispatcher *dispatcher)
    : lay::EditorOptionsPage(view, dispatcher), mp_pcell_parameters(0),
...
std::string EditorOptionsInstPCellParam::title() const {
...
void EditorOptionsInstPCellParam::apply(lay::Dispatcher *root) {
  ...
  db::Library *lib = db::LibraryManager::instance().lib_ptr_by_name(
      m_lib_name, view()->active_cellview().is_valid()
                      ? view()->active_cellview()->tech_name()
                      : std::string());
  if (lib) {
    layout = &lib->layout();
  ...
  if (layout && mp_pcell_parameters) {
    std::pair<bool, db::pcell_id_type> pc =
        layout->pcell_by_name(tl::to_string(m_cell_name).c_str());
    if (pc.first) {
      const db::PCellDeclaration *pc_decl =
          layout->pcell_declaration(pc.second);
      if (pc_decl) {
        param = pcell_parameters_to_string(pc_decl->named_parameters(
            mp_pcell_parameters->get_parameters(&ok)));
      }
    }
  }
      ...
      if (pc_decl) {
        ...
        try {
          ...
          while (!ex.at_end()) {
            ...
            ex.test(":");
            ex.read(parameters.insert(std::make_pair(n, tl::Variant()))
                        .first->second);
            ex.test(";");
          }
        } catch (...) {
...
void EditorOptionsInstPCellParam::update_pcell_parameters(
  ...
  //  TODO: don't re-generate the PCellParametersPage unless the PCell has
  //  changed

  PCellParametersPage::State pcp_state;

  //  Hint: we shall not delete the page immediately. This gives a segmentation
  //  fault in some cases.
  ...
  if (pc.first && layout->pcell_declaration(pc.second) &&
      view()->cellview(m_cv_index).is_valid()) {

    mp_pcell_parameters =
        new PCellParametersPage(this, dispatcher(), true /*dense*/);
    mp_pcell_parameters->setup(
        view(), m_cv_index, layout->pcell_declaration(pc.second), parameters);
