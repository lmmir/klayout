...
ShapePropertiesPage::ShapePropertiesPage(const std::string &description,
                                         edt::Service *service,
                                         db::Manager *manager, QWidget *parent)
    : lay::PropertiesPage(parent, manager, service), m_description(description),
...
void ShapePropertiesPage::setup() {
  connect(dbu_checkbox(), SIGNAL(toggled(bool)), this,
          SLOT(display_mode_changed(bool)));
  connect(abs_checkbox(), SIGNAL(toggled(bool)), this,
          SLOT(display_mode_changed(bool)));
...
size_t ShapePropertiesPage::count() const { return m_selection_ptrs.size(); }

void ShapePropertiesPage::select_entries(const std::vector<size_t> &entries) {
  m_indexes = entries;
}

lay::LayoutViewBase *ShapePropertiesPage::view() const {
  return mp_service->view();
}

const db::Shape &ShapePropertiesPage::shape(size_t entry) const {
  return m_selection_ptrs[entry]->shape();
}

double ShapePropertiesPage::dbu(size_t entry) const {
  unsigned int cv_index = m_selection_ptrs[entry]->cv_index();
  return view()->cellview(cv_index)->layout().dbu();
}

std::string ShapePropertiesPage::description(size_t entry) const {
  unsigned int cv_index = m_selection_ptrs[entry]->cv_index();
  unsigned int layer = m_selection_ptrs[entry]->layer();

  if (view()->cellview(cv_index).is_valid()) {
    const db::LayerProperties &lp =
        view()->cellview(cv_index)->layout().get_properties(layer);
    if (view()->cellviews() > 1) {
...
QIcon ShapePropertiesPage::icon(size_t entry, int w, int h) const {
  ...
  for (auto lp = view->begin_layers(view->current_layer_list()); !lp.at_end();
       ++lp) {
    const lay::LayerPropertiesNode *ln = lp.operator->();
    if (ln->cellview_index() == cv_index && ln->layer_index() == layer) {
      return QIcon(
          QPixmap::fromImage(view->icon_for_layer(lp, w, h).to_image()));
    }
  }

  return QIcon();
}
std::string ShapePropertiesPage::description() const { return m_description; }

void ShapePropertiesPage::leave() { mp_service->clear_highlights(); }
...
db::ICplxTrans ShapePropertiesPage::trans() const {
...
void ShapePropertiesPage::recompute_selection_ptrs(
...
void ShapePropertiesPage::do_apply(bool current_only, bool relative) {
  if (m_indexes.empty()) {
    return;
  }
...

  {
    ...
    db::Shapes &shapes =
        cv->layout().cell(pos->cell_index()).shapes(pos->layer());
  ...
  //  shape which may happen when a shape is selected multiple times through
    ...
    for (auto i = m_indexes.begin(); i != m_indexes.end(); ++i) {
      ...
      if (pos->shape().is_array_member()) {
        throw tl::Exception(tl::to_string(
            QObject::tr("Shape array members cannot be changed")));
      }

      db::Shape new_shape = pos->shape();

      //  Don't apply the same change twice
      std::map<db::Shape, db::Shape>::const_iterator s =
          shapes_seen.find(pos->shape());
      if (s == shapes_seen.end()) {

        db::Shapes &shapes =
            layout.cell(pos->cell_index()).shapes(pos->layer());
        double dbu = layout.dbu();

        if (!current_only || pos->shape() == current) {
          new_shape =
              applicator->do_apply(shapes, pos->shape(), dbu, relative_mode);
        }
      ...
      std::pair<bool, lay::ObjectInstPath> gs =
          mp_service->handle_guiding_shape_changes(new_sel[index]);
      if (gs.first) {
...
void ShapePropertiesPage::apply() { do_apply(true, false); }

bool ShapePropertiesPage::can_apply_to_all() const {
  return m_selection_ptrs.size() > 1;
}

void ShapePropertiesPage::apply_to_all(bool relative) {
...
void ShapePropertiesPage::update_shape() {
  ...
  for (lay::LayerPropertiesConstIterator lp =
           mp_service->view()->begin_layers();
       !lp.at_end(); ++lp) {
    if (lp->cellview_index() == int(pos->cv_index()) &&
        lp->layer_index() == int(pos->layer())) {
  ...
  mp_service->view()->set_current_layer(
      pos->cv_index(), cv->layout().get_properties(pos->layer()));
...
PolygonPropertiesPage::PolygonPropertiesPage(edt::Service *service,
                                             db::Manager *manager,
                                             QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Polygons")), service, manager,
                          parent),
...
std::string PolygonPropertiesPage::description(size_t entry) const {
  const db::Shape &sh = shape(entry);

  size_t npts = count_polygon_points(sh);
  if (sh.holes() == 0 && npts > 4) {
    return ShapePropertiesPage::description(entry) + " - " +
           tl::sprintf(tl::to_string(tr("Polygon(%d points)")), npts);
  } else if (sh.holes() > 0) {
    return ShapePropertiesPage::description(entry) + " - " +
           tl::sprintf(tl::to_string(tr("Polygon(%d points, %d holes)")), npts,
                       sh.holes());
  } else {
    db::Polygon poly;
    sh.polygon(poly);
    ...
    return ShapePropertiesPage::description(entry) + " - " +
           tl::sprintf(tl::to_string(tr("Polygon%s")),
                       (dbu_trans * poly).to_string());
  }
}
...
void PolygonPropertiesPage::do_update(const db::Shape &shape, double dbu,
  ...
  bool first = true;
  for (db::Polygon::polygon_contour_iterator pt = poly.begin_hull();
       pt != poly.end_hull(); ++pt) {
    if (!first) {
      ptlist += "\n";
    } else {
  ...
  for (unsigned int h = 0; h < poly.holes(); ++h) {
    ...
    for (db::Polygon::polygon_contour_iterator pt = poly.begin_hole(h);
         pt != poly.end_hole(h); ++pt) {
      ptlist += "\n";
      ptlist += coords_to_string(t * *pt, dbu, du);
    }
  }
  ...
  pointCountLabel->setText(tl::to_qstring(tl::sprintf(
      tl::to_string(QObject::tr("(%lu points)")), poly.vertices())));
}
...
ChangeApplicator *
PolygonPropertiesPage::create_applicator(db::Shapes & /*shapes*/,
  ...
  try {
    ...
    if (*ex.skip() == '(') {
      ...
      poly = db::Polygon(dp.transformed(db::DCplxTrans(t) *
                                        db::DCplxTrans(du ? 1.0 : 1.0 / dbu)));
      ...
      while (!ex.at_end()) {
        ...
        if (points.size() < 3) {
          throw tl::Exception(tl::to_string(
              QObject::tr("Polygon must have at least three points")));
        }
        ...
        if (h == 0) {
          ...
          poly.insert_hole(points.begin(), points.end(),
                           false /*not compressed*/);
        }
...
BoxPropertiesPage::BoxPropertiesPage(edt::Service *service,
                                     db::Manager *manager, QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Boxes")), service, manager, parent),
      m_recursion_sentinel(false), m_tab_index(0), m_dbu(1.0),
      m_lr_swapped(false), m_tb_swapped(false) {
  setupUi(this);
  setup();
...
std::string BoxPropertiesPage::description(size_t entry) const {
  const db::Shape &sh = shape(entry);
  db::CplxTrans dbu_trans(dbu(entry));
  return ShapePropertiesPage::description(entry) + " - " +
         tl::sprintf(tl::to_string(tr("Box%s")),
                     (dbu_trans * sh.box()).to_string());
}
...
ChangeApplicator *BoxPropertiesPage::create_applicator(db::Shapes & /*shapes*/,
  ...
  if (box != org_box) {
    return new BoxDimensionsChangeApplicator(
        box.left() - org_box.left(), box.bottom() - org_box.bottom(),
        box.right() - org_box.right(), box.top() - org_box.top(), box.left(),
...
db::Box BoxPropertiesPage::get_box(int mode) const {
  if (mode == 0) {
    ...
    if (has_error) {
      throw tl::Exception(
          tl::to_string(tr("Invalid values - see highlighted entry boxes")));
    }
    ...
    return db::Box(point_from_dpoint(db::DPoint(x1, y1), m_dbu, du, t),
                   point_from_dpoint(db::DPoint(x2, y2), m_dbu, du, t));
    ...
    return db::Box(
        point_from_dpoint(db::DPoint(cx - w * 0.5, cy - h * 0.5), m_dbu, du, t),
        point_from_dpoint(db::DPoint(cx + w * 0.5, cy + h * 0.5), m_dbu, du,
                          t));
  }
}
...
void BoxPropertiesPage::set_box(const db::Box &box) {
  ...
  (m_lr_swapped ? x2_le_1 : x1_le_1)
      ->setText(
          tl::to_qstring(coord_to_string(bt.lower_left().x(), m_dbu, du)));
  (m_tb_swapped ? y2_le_1 : y1_le_1)
      ->setText(
          tl::to_qstring(coord_to_string(bt.lower_left().y(), m_dbu, du)));
  (m_lr_swapped ? x1_le_1 : x2_le_1)
      ->setText(
          tl::to_qstring(coord_to_string(bt.upper_right().x(), m_dbu, du)));
  (m_tb_swapped ? y1_le_1 : y2_le_1)
      ->setText(
          tl::to_qstring(coord_to_string(bt.upper_right().y(), m_dbu, du)));
  ...
  (m_lr_swapped ? x2_le_2 : x1_le_2)
      ->setText(
          tl::to_qstring(coord_to_string(bt.lower_left().x(), m_dbu, du)));
  (m_tb_swapped ? y2_le_2 : y1_le_2)
      ->setText(
          tl::to_qstring(coord_to_string(bt.lower_left().y(), m_dbu, du)));
  (m_lr_swapped ? x1_le_2 : x2_le_2)
      ->setText(
          tl::to_qstring(coord_to_string(bt.upper_right().x(), m_dbu, du)));
  (m_tb_swapped ? y1_le_2 : y2_le_2)
      ->setText(
          tl::to_qstring(coord_to_string(bt.upper_right().y(), m_dbu, du)));
...
PointPropertiesPage::PointPropertiesPage(edt::Service *service,
                                         db::Manager *manager, QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Points")), service, manager,
                          parent),
      m_dbu(1.0) {
  setupUi(this);
  setup();
...
std::string PointPropertiesPage::description(size_t entry) const {
  const db::Shape &sh = shape(entry);
  db::CplxTrans dbu_trans(dbu(entry));
  return ShapePropertiesPage::description(entry) + " - " +
         tl::sprintf(tl::to_string(tr("Point%s")),
                     (dbu_trans * sh.point()).to_string());
}
...
ChangeApplicator *
PointPropertiesPage::create_applicator(db::Shapes & /*shapes*/,
                                       const db::Shape &shape, double dbu) {
  m_dbu = dbu;
...
db::Point PointPropertiesPage::get_point() const {
  bool has_error = false;
  double x = 0.0, y = 0.0;
...
TextPropertiesPage::TextPropertiesPage(edt::Service *service,
                                       db::Manager *manager, QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Texts")), service, manager,
                          parent) {
  setupUi(this);
  setup();
...
std::string TextPropertiesPage::description(size_t entry) const {
  const db::Shape &sh = shape(entry);
  db::Text text;
  sh.text(text);
  db::CplxTrans dbu_trans(dbu(entry));
  return ShapePropertiesPage::description(entry) + " - " +
         tl::sprintf(tl::to_string(tr("Text%s")),
                     (dbu_trans * text).to_string());
}
...
void TextPropertiesPage::do_update(const db::Shape &shape, double dbu,
  ...
  if (text.size() != 0) {
    size_le->setText(
        tl::to_qstring(coord_to_string(t.ctrans(text.size()), dbu, du)));
  } else {
...
ChangeApplicator *TextPropertiesPage::create_applicator(db::Shapes & /*shapes*/,
                                                        const db::Shape &shape,
                                                        double dbu) {
  bool has_error = false;
  ...
  if (tt.disp() != org_text.trans().disp()) {
    appl->add(
        new TextPositionChangeApplicator(tt.disp(), org_text.trans().disp()));
  }
  if (db::HAlign(halign_cbx->currentIndex() - 1) != org_text.halign()) {
    appl->add(new TextHAlignChangeApplicator(
        db::HAlign(halign_cbx->currentIndex() - 1)));
  }
  if (db::VAlign(valign_cbx->currentIndex() - 1) != org_text.valign()) {
    appl->add(new TextVAlignChangeApplicator(
        db::VAlign(valign_cbx->currentIndex() - 1)));
  }
  ...
  if (!size_le->text().isEmpty()) {
    try {
      size =
          coord_from_string(tl::to_string(size_le->text()).c_str(), dbu, du, t);
...
PathPropertiesPage::PathPropertiesPage(edt::Service *service,
                                       db::Manager *manager, QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Paths")), service, manager, parent),
      m_in_text_changed(false) {
  setupUi(this);
  setup();
...
static std::string path_description(const db::Shape &sh, double dbu) {
  ...
  if (npts > 4) {
    return tl::sprintf(tl::to_string(tr("Path(%d points, w=%.12g)")), npts,
                       sh.path_width() * dbu);
  } else {
    db::CplxTrans dbu_trans(dbu);
    db::Path path;
    sh.path(path);
    return tl::sprintf(tl::to_string(tr("Path%s")),
                       (dbu_trans * path).to_string());
  }
}

std::string PathPropertiesPage::description(size_t entry) const {
  return ShapePropertiesPage::description(entry) + " - " +
         path_description(shape(entry), dbu(entry));
}
...
void PathPropertiesPage::do_update(const db::Shape &shape, double dbu,
  ...
  width_le->setText(
      tl::to_qstring(coord_to_string(t.ctrans(path.width()), dbu, du)));
  start_ext_le->setText(tl::to_qstring(
      coord_to_string(t.mag() * path.extensions().first, dbu, du)));
  end_ext_le->setText(tl::to_qstring(
      coord_to_string(t.mag() * path.extensions().second, dbu, du)));
  round_cb->setChecked(path.round());
}

ChangeApplicator *PathPropertiesPage::create_applicator(
...
EditablePathPropertiesPage::EditablePathPropertiesPage(edt::Service *service,
                                                       db::Manager *manager,
                                                       QWidget *parent)
    : ShapePropertiesPage(tl::to_string(tr("Paths")), service, manager, parent),
      m_in_text_changed(false) {
  ...
  connect(inst_pb, SIGNAL(clicked()), this, SLOT(show_inst()));
  connect(prop_pb, SIGNAL(clicked()), this, SLOT(show_props()));
  connect(type_cb, SIGNAL(currentIndexChanged(int)), this,
          SLOT(type_selected(int)));
...
static int path_type_choice(const db::Path &path) {
...
std::string EditablePathPropertiesPage::description(size_t entry) const {
  return ShapePropertiesPage::description(entry) + " - " +
         path_description(shape(entry), dbu(entry));
}
...
ChangeApplicator *EditablePathPropertiesPage::create_applicator(
    db::Shapes & /*shapes*/, const db::Shape &shape, double dbu) {
  bool has_error = false;
  ...
  try {
    ...
    if (points.size() < 1) {
      throw tl::Exception(
          tl::to_string(QObject::tr("The path must have at least one point")));
    }
  ...
  case 2: // variable
    try {
      se = coord_from_string(tl::to_string(start_ext_le->text()).c_str(), dbu,
                             du, t);
    ...
    try {
      ee = coord_from_string(tl::to_string(end_ext_le->text()).c_str(), dbu, du,
                             t);
  ...
  if (type_cb->currentIndex() != path_type_choice(org_path) ||
      (type_cb->currentIndex() == 2 && (se != org_path.extensions().first ||
                                        ee != org_path.extensions().second))) {
