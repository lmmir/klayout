...
static void set_value(const db::PCellParameterDeclaration &p, QWidget *widget,
  ...
  if (p.get_choices().empty()) {
    ...
    case db::PCellParameterDeclaration::t_layer: {
      lay::LayerSelectionComboBox *ly =
          dynamic_cast<lay::LayerSelectionComboBox *>(widget);
      if (ly) {
    ...
    if (cb) {
      int i = 0;
      for (std::vector<tl::Variant>::const_iterator c = p.get_choices().begin();
           c != p.get_choices().end(); ++c, ++i) {
        if (*c == value) {
...
PCellParametersPage::PCellParametersPage(QWidget *parent,
                                         ...
                                         bool dense)
    : QFrame(parent), mp_dispatcher(dispatcher), m_dense(dense),
      m_show_parameter_names(false),
      dm_parameter_changed(this, &PCellParametersPage::do_parameter_changed) {
  if (mp_dispatcher) {
    mp_dispatcher->config_get(cfg_edit_pcell_show_parameter_names,
                              m_show_parameter_names);
  }

  init();
}

void PCellParametersPage::init() {
  QPalette palette;
  QFont font;
  ...
  connect(mp_update_button, SIGNAL(clicked()), this,
          SLOT(update_button_pressed()));
  ...
  QHBoxLayout *show_parameter_names_frame_layout =
      new QHBoxLayout(show_parameter_names_frame);
  ...
  connect(mp_show_parameter_names_cb, SIGNAL(clicked(bool)), this,
          SLOT(show_parameter_names(bool)));
}

bool PCellParametersPage::lazy_evaluation() {
  return mp_pcell_decl.get() && mp_pcell_decl->wants_lazy_evaluation();
}

void PCellParametersPage::show_parameter_names(bool f) {
  ...
  if (mp_dispatcher) {
    mp_dispatcher->config_set(cfg_edit_pcell_show_parameter_names,
                              m_show_parameter_names);
  }
...
void PCellParametersPage::setup(lay::LayoutViewBase *view, int cv_index,
                                const db::PCellDeclaration *pcell_decl,
                                const db::pcell_parameters_type &parameters) {
  mp_pcell_decl.reset(
      const_cast<db::PCellDeclaration *>(pcell_decl)); //  no const weak_ptr ...
  mp_view = view;
  m_cv_index = cv_index;
  ...
  for (std::vector<db::PCellParameterDeclaration>::const_iterator p =
    ...
    if (tab != std::string::npos) {
      ...
      description =
          std::string(p->get_description(), tab + 1, std::string::npos);
    } else {
    ...
    if (p->get_type() != db::PCellParameterDeclaration::t_callback) {
      ...
      QLabel *l = new QLabel(tl::to_qstring(tl::sprintf("[%s]", p->get_name())),
                             inner_frame);
      inner_grid->addWidget(l, row, 1);
      m_all_widgets.back().push_back(l);
    }
    ...
    if (p->get_choices().empty()) {
      ...
      case db::PCellParameterDeclaration::t_layer: {
        lay::LayerSelectionComboBox *ly =
            new lay::LayerSelectionComboBox(inner_frame);
      ...
      case db::PCellParameterDeclaration::t_boolean: {
        ...
        //  this makes the checkbox not stretch over the full width - better
        ...
        connect(cbx, SIGNAL(stateChanged(int)), this,
                SLOT(parameter_changed()));
      } break;
      ...
      int i = 0;
      for (std::vector<tl::Variant>::const_iterator c =
               p->get_choices().begin();
           c != p->get_choices().end(); ++c, ++i) {
  ...
  try {
    mp_pcell_decl->callback(mp_view->cellview(m_cv_index)->layout(),
                            std::string(), m_states);
...
PCellParametersPage::State PCellParametersPage::get_state() {
  State s;
...
void PCellParametersPage::set_state(const State &s) {
  if (s.valid) {
...
void PCellParametersPage::parameter_changed() {
  if (!mp_pcell_decl) {
    return;
  }
  ...
  const std::vector<db::PCellParameterDeclaration> &pcp =
      mp_pcell_decl->parameter_declarations();
  ...
  try {
    ...
    //  Silent and without coerce - this will be done later in
    //  do_parameter_changed(). This is just about providing the inputs for the
    //  callback.
    ...
    if (!edit_error) {
      mp_pcell_decl->callback(mp_view->cellview(m_cv_index)->layout(),
                              pd ? pd->get_name() : std::string(), states);
      m_states = states;
    }
...
void PCellParametersPage::do_parameter_changed() {
  bool ok = true;
  db::ParameterStates states = m_states;
...
void PCellParametersPage::update_button_pressed() {
  if (update_current_parameters()) {
    emit edited();
  }
}

bool PCellParametersPage::update_current_parameters() {
  bool ok = true;
  db::ParameterStates states = m_states;
  get_parameters(states, &ok); //  includes coerce
  if (ok) {
...
void PCellParametersPage::get_parameters_internal(db::ParameterStates &states,
                                                  bool &edit_error) {
  edit_error = false;
  ...
  for (std::vector<db::PCellParameterDeclaration>::const_iterator p =
    ...
    if (!ps.is_visible() || !ps.is_enabled() || ps.is_readonly() ||
        p->get_type() == db::PCellParameterDeclaration::t_shape) {
      continue;
    }
    ...
    if (p->get_choices().empty()) {
      ...
      case db::PCellParameterDeclaration::t_layer: {
        lay::LayerSelectionComboBox *ly =
            dynamic_cast<lay::LayerSelectionComboBox *>(m_widgets[r]);
        if (ly) {
      ...
      if (cb && cb->currentIndex() >= 0 &&
          cb->currentIndex() < int(p->get_choices().size())) {
...
void PCellParametersPage::get_parameters(db::ParameterStates &states,
                                         bool *ok) {
  try {
    ...
    if (edit_error) {
      throw tl::Exception(tl::to_string(tr(
          "There are errors. See the highlighted edit fields for details.")));
    }
    ...
    if (mp_view->cellview(m_cv_index).is_valid()) {
      ...
      mp_pcell_decl->coerce_parameters(mp_view->cellview(m_cv_index)->layout(),
                                       parameters);
...
std::vector<tl::Variant> PCellParametersPage::get_parameters(bool *ok) {
  db::ParameterStates states = m_states;
...
void PCellParametersPage::set_parameters(
...
void PCellParametersPage::update_widgets_from_states(
    const db::ParameterStates &states) {
  if (!mp_pcell_decl) {
    return;
  }
  ...
  for (std::vector<db::PCellParameterDeclaration>::const_iterator p =
           pcp.begin();
       p != pcp.end() && i < m_widgets.size(); ++p, ++i) {
...
void PCellParametersPage::set_parameters_internal(
    const db::ParameterStates &states, bool tentatively) {
  if (!mp_pcell_decl) {
    return;
  }
...
std::vector<tl::Variant> PCellParametersPage::parameter_from_states(
    const db::ParameterStates &states) const {
...
void PCellParametersPage::states_from_parameters(
    db::ParameterStates &states, const std::vector<tl::Variant> &parameters) {
  if (!mp_pcell_decl) {
    return;
  }
