...
A::A(const A &a) : gsi::ObjectBase(a) {
  ++a_count;
  operator=(a);
}
...
int A::instance_count() { return a_count; }

std::string A::to_s() const { return tl::sprintf("A: %d", n); }
...
const char *A::a_static() { return "static_a"; }

tl::Variant A::new_a_by_variant() { return tl::Variant(A()); }

#if defined(HAVE_QT)

std::vector<int> A::qba_cref_to_ia(const QByteArray &ba) {
...
QByteArray A::ia_cref_to_qba(const std::vector<int> &ia) {
  QByteArray ba;
...
QByteArray &A::ia_cref_to_qba_ref(const std::vector<int> &ia) {
...
std::vector<int> A::qbav_cref_to_ia(const QByteArrayView &ba) {
  const char *cp = ba.constData();
  size_t n = ba.size();
  std::vector<int> ia;
  for (size_t i = 0; i < n; ++i) {
...
QByteArrayView A::ia_cref_to_qbav(const std::vector<int> &ia) {
  static QByteArray ba;
  ba.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    ba.push_back(char(*i));
...
QByteArrayView &A::ia_cref_to_qbav_ref(const std::vector<int> &ia) {
  static QByteArray ba;
  ba.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    ba.push_back(char(*i));
...
std::vector<int> A::qs_cref_to_ia(const QString &qs) {
...
QString A::ia_cref_to_qs(const std::vector<int> &ia) {
  QString s;
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    s.push_back(char(*i));
  }
...
QString &A::ia_cref_to_qs_ref(const std::vector<int> &ia) {
  static QString s;
  s.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    s.push_back(char(*i));
...
std::vector<int> A::ql1s_cref_to_ia(const QLatin1String &ql1s) {
  std::vector<int> ia;
...
QLatin1String A::ia_cref_to_ql1s(const std::vector<int> &ia) {
...
QLatin1String &A::ia_cref_to_ql1s_ref(const std::vector<int> &ia) {
  static std::string str;
  str.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    str += char(*i);
...
std::vector<int> A::qsv_cref_to_ia(const QStringView &qs) {
  const QChar *cp = qs.constData();
  size_t n = qs.size();
  std::vector<int> ia;
  for (size_t i = 0; i < n; ++i) {
...
QStringView A::ia_cref_to_qsv(const std::vector<int> &ia) {
  static QString s;
  s.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    s.push_back(char(*i));
...
QStringView &A::ia_cref_to_qsv_ref(const std::vector<int> &ia) {
  static QString s;
  s.clear();
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    s.push_back(char(*i));
...
std::vector<int> A::ba_cref_to_ia(const std::vector<char> &ba) {
  std::vector<int> ia;
...
std::vector<char> A::ia_cref_to_ba(const std::vector<int> &ia) {
  std::vector<char> ba;
  for (std::vector<int>::const_iterator i = ia.begin(); i != ia.end(); ++i) {
    ba.push_back(char(*i));
  }
...
std::vector<char> &A::ia_cref_to_ba_ref(const std::vector<int> &ia) {
...
static A *a_ctor(int i) { return new A(i); }
...
int B::instance_count() { return b_count; }
...
B &B::operator=(const B &d) {
  ...
  for (std::vector<A_NC *>::const_iterator i = d.av_nc.begin();
       i != d.av_nc.end(); ++i) {
  ...
  for (std::vector<const A_NC *>::const_iterator i = d.avc_nc.begin();
       i != d.avc_nc.end(); ++i) {
...
void B::set_inst(B *b) { b_inst = b; }

void B::del_inst() { delete b_inst; }

B *B::inst() { return b_inst; }

bool B::has_inst() { return b_inst != 0; }

tl::Variant B::new_b_by_variant() { return tl::Variant(B()); }
...
static int b3_ext(const B *b, A *aptr) { return b->b3(aptr); }

static std::vector<A>::const_iterator b10b_ext(const B *b) { return b->b10b(); }

static std::vector<A>::const_iterator b10e_ext(const B *b) { return b->b10e(); }
...
int C::s1() { return 4451; }

std::vector<int>::const_iterator C::s1a() { return m_v.begin(); }

std::vector<int>::const_iterator C::s1b() { return m_v.end(); }
...
void C::s2clr() { m_v.clear(); }

std::string C::s3(double x) { return tl::sprintf("%.3f", x); }
...
int E::inst_count() { return e_count; }
...
void E::reset_inst() { e_inst.reset(0); }
...
static X *make_x(const char *x) { return new X(x); }

static int s_xinst = 0;

X::X() { ++s_xinst; }

X::X(const char *x) : m_s(x) { ++s_xinst; }

X::X(const X &x) : gsi::ObjectBase() {
  *this = x;
  ++s_xinst;
}
...
int X::instances() { return s_xinst; }
...
const X *X::x_cptr() { return sp_a.get(); }

X *X::x_ptr() { return sp_a.get(); }
...
std::string X::cls_name() const { return "X"; }

std::string X::s() const { return m_s; }

void X::set_s(const std::string &s) { m_s = s; }

void X::set_si(int v) { m_s = tl::to_string(v + 1); }
...
static Y *make_y(const char *x) { return new Y(x); }

Y::Y() : mp_c(0) { ++s_dyn_count; }

Y::Y(const char *x) : X(x), mp_c(0) { ++s_dyn_count; }
...
const X *Y::y_cptr() { return sp_a.get(); }

X *Y::y_ptr() { return sp_a.get(); }

int Y::vx_dyn_count() { return s_dyn_count; }

void Y::vx_dyn_make() { mp_c = new Y(); }
...
std::string Y::cls_name() const { return "Y"; }

int Y::i() const { return (int)m_s.size(); }
...
YY::YY(const char *x) : Y(x) {
  //  .. nothing yet ..
}

std::string YY::cls_name() const { return "YY"; }
...
std::string Z::f(X *x) { return x ? tl::to_string(x->s()) : "(nil)"; }

void Z::set_x(X *x) { mp_x = x; }
...
std::string Z_P::f(X *x) { return f_cb.issue<Z, std::string, X *>(&Z::f, x); }

std::string Z_P::f_org(X *x) { return Z::f(x); }
...
void SQ::set_tag(int t) { m_tag = t; }

void SQ::trigger_s0() { emit s0(); }

void SQ::trigger_s1(int x) { emit s1(x); }

void SQ::trigger_s2(const QString &s) { emit s2(s, this); }
...
void SE::set_tag(int t) { m_tag = t; }

void SE::trigger_s0() { s0(); }

void SE::trigger_s1(int x) { s1(x); }

void SE::trigger_s2(const std::string &s) { s2(s, this); }
...
static gsi::Class<A> decl_a(
    ...
    gsi::constructor("new_a|new", &a_ctor) +
        ...
        gsi::method("br", &A::br) + gsi::method("get_e", &A::get_e) +
...
#endif
        gsi::method("push_ev", &A::push_ev) + gsi::method("ev", &A::ev) +
        gsi::method("af=", &A::set_af) + gsi::method("af?|af", &A::af0) +
        gsi::method("af?|af", &A::af1) + gsi::method("aa", &A::a) +
        gsi::method("aa", &A::a_static) + gsi::method("a1", &A::a1) +
        gsi::method("a1c", &A::a1c) + gsi::method("a2", &A::a2) +
        gsi::method("a3", &A::a3) +
#if defined(HAVE_QT)
...
#endif
        gsi::method("a4", &A::a4) + gsi::method("a5|n=", &A::a5) +
...
#endif
        gsi::method("*a10_prot", &A::a10_d) + gsi::method("a10_f", &A::a10_f) +
        gsi::method("a10_s", &A::a10_s) + gsi::method("a10_us", &A::a10_us) +
        gsi::method("a10_i", &A::a10_i) + gsi::method("a10_l", &A::a10_l) +
        gsi::method("a10_ll", &A::a10_ll) + gsi::method("a10_ui", &A::a10_ui) +
        ...
        gsi::method("a11_s", &A::a11_s) + gsi::method("a11_us", &A::a11_us) +
        gsi::method("a11_i", &A::a11_i) + gsi::method("a11_l", &A::a11_l) +
        gsi::method("a11_ll", &A::a11_ll) + gsi::method("a11_ui", &A::a11_ui) +
        gsi::method("a11_ul", &A::a11_ul) +
        gsi::method("a11_ull", &A::a11_ull) + gsi::method("a_vp1", &A::a_vp1) +
        gsi::method("a_vp2", &A::a_vp2) + gsi::method("a9a", &A::a9a) +
        gsi::method("a9b", &A::a9b) + gsi::method("a20", &A::a20) +
        gsi::method("a20_get", &A::a20_get) + gsi::method("to_s", &A::to_s) +
...
static gsi::Class<B> decl_b(
    ...
    gsi::method("int_to_optional", &B::int_to_optional) +
...
#endif
        gsi::method("inst", &B::inst) + gsi::method("has_inst", &B::has_inst) +
        ...
        gsi::method("addr", &B::addr) + gsi::method("always_5", &B::always_5) +
        gsi::method("str", &B::str) + gsi::method("set_str", &B::set_str) +
        ...
        gsi::method("make_a", &B::make_a) + gsi::method("set_an", &B::set_an) +
        gsi::method("an", &B::an) +
        ...
        gsi::method("b17e|av_ptr=", &B::b17e) + gsi::iterator("b18", &B::b18) +
        gsi::iterator("b18b", &B::b18b) + gsi::iterator("b18c", &B::b18b) +
        ...
        gsi::method("b22a", &B::b22a) + gsi::method("set_vars", &B::set_vars) +
        gsi::method("b22b", &B::b22b) + gsi::method("b22c", &B::b22c) +
        gsi::method("b22d", &B::b22d) + gsi::method("var", &B::var) +
        ...
        gsi::method("push_vvs", &B::push_vvs) + gsi::method("vvs", &B::vvs) +
        ...
        gsi::method("push_ls", &B::push_ls) + gsi::method("ls", &B::ls) +
        gsi::method("set_ls", &B::set_ls) +
        gsi::method("push_ss", &B::push_ss) + gsi::method("ss", &B::ss) +
        gsi::method("set_ss", &B::set_ss)
#if defined(HAVE_QT)
        + gsi::method("push_qls", &B::push_qls) + gsi::method("qls", &B::qls) +
        gsi::method("set_qls", &B::set_qls) +
        gsi::method("push_qlv", &B::push_qlv) + gsi::method("qlv", &B::qlv) +
        gsi::method("set_qlv", &B::set_qlv) +
        gsi::method("push_qsl", &B::push_qsl) + gsi::method("qsl", &B::qsl) +
        gsi::method("set_qsl", &B::set_qsl) +
        gsi::method("push_qvs", &B::push_qvs) + gsi::method("qvs", &B::qvs) +
        gsi::method("set_qvs", &B::set_qvs) +
        gsi::method("push_qss", &B::push_qss) + gsi::method("qss", &B::qss) +
...
//  extending B
static gsi::ClassExt<B> b_ext(gsi::iterator_ext("b10", &b10b_ext, &b10e_ext) +
                              gsi::iterator_ext("b10p", &b10bp_ext,
                                                &b10ep_ext));

CopyDetector *new_cd(int x) { return new CopyDetector(x); }

static gsi::Class<CopyDetector>
    decl_copy_detector("", "CopyDetector",
                       gsi::constructor("new", &new_cd) +
                           gsi::method("x", &CopyDetector::x) +
                           gsi::method("xx", &CopyDetector::xx));

static gsi::Class<C_P>
    decl_c("", "C",
           gsi::callback("f", &C_P::f, &C_P::f_cb) +
               ...
               gsi::method("pass_cd_cref_as_copy", gsi::return_copy(),
                           &C_P::pass_cd_cref) +
               gsi::method("pass_cd_cref_as_ref", gsi::return_reference(),
                           &C_P::pass_cd_cref) +
               gsi::method("pass_cd_cptr", &C_P::pass_cd_cptr) +
               gsi::method("pass_cd_cptr_as_copy", gsi::return_copy(),
                           &C_P::pass_cd_cptr) +
               gsi::method("pass_cd_cptr_as_ref", gsi::return_reference(),
                           &C_P::pass_cd_cptr) +
               gsi::method("pass_cd_ref", &C_P::pass_cd_ref) +
               gsi::method("pass_cd_ref_as_copy", gsi::return_copy(),
                           &C_P::pass_cd_ref) +
               gsi::method("pass_cd_ref_as_ref", gsi::return_reference(),
                           &C_P::pass_cd_ref) +
               gsi::method("pass_cd_ptr", &C_P::pass_cd_ptr) +
               gsi::method("pass_cd_ptr_as_copy", gsi::return_copy(),
                           &C_P::pass_cd_ptr) +
               gsi::method("pass_cd_ptr_as_ref", gsi::return_reference(),
                           &C_P::pass_cd_ptr) +
               gsi::method("g", &C_P::g) + gsi::method("s1", &C::s1) +
               gsi::method("s2", &C::s2) + gsi::method("s2clr", &C::s2clr) +
               gsi::method("s3", &C::s3) +
...
static gsi::Class<E>
    decl_e("", "E",
           gsi::event("e0", &E::ev0) + gsi::event_ext("e1", &ev1_ext) +
               gsi::event("e2", &E::ev2) +
               // No events with return available currently
               // gsi::event ("e0r", &E::ev0r) +
               gsi::method("s1", &E::s1) + gsi::method("s2", &E::s2) +
               gsi::method("s3", &E::s3) +
               // No events with return available currently
               // gsi::method ("s1r", &E::s1r) +
               gsi::method("ic", &E::ic) + gsi::method("inc", &E::inc) +
               gsi::method("icref", &E::icref) +
               gsi::method("incref", &E::incref) +
               gsi::method("x=", &E::set_x) + gsi::method("x", &E::get_x) +
...
static gsi::Class<F>
    decl_f("", "F",
           gsi::method("ic", &F::ic) + gsi::method("inc", &F::inc) +
               gsi::method("icref", &F::icref) +
               gsi::method("incref", &F::incref) +
               gsi::method("x=", &F::set_x) + gsi::method("x", &F::get_x));

static gsi::Class<G> decl_g(
    "", "G",
    gsi::method("iv", &G::iv) + gsi::method("sv", &G::sv) +
        ...
        gsi::method("set_vvc", &G::set_vv, gsi::arg("", 1),
                    gsi::arg("", "value")));

static gsi::Class<X> decl_x(
    "", "X",
    gsi::constructor("new", &make_x) + gsi::method("instances", &X::instances) +
        gsi::method("x1", &X::x1) + gsi::method("x2", &X::x2) +
        gsi::method("x_ptr", &X::x_ptr) + gsi::method("x_cptr", &X::x_cptr) +
        gsi::method("vx", &X::vx) + gsi::method("vx_ptr", &X::vx_ptr) +
        gsi::method("vx_cptr", &X::vx_cptr) +
        gsi::method("cls_name", &X::cls_name) + gsi::method("init", &X::init) +
        gsi::method("s", &X::s) + gsi::method("s=", &X::set_s) +
        gsi::method("s=", &X::set_si));

static gsi::Class<Y>
    decl_y(decl_x, "", "Y",
           gsi::constructor("new", &make_y) + gsi::method("x1", &Y::x1) +
               gsi::method("y1", &Y::y1) + gsi::method("y_ptr", &Y::y_ptr) +
               ...
               gsi::method("init", &Y::init) + gsi::method("i", &Y::i) +
...
static gsi::SubClass<Y2, X> decl_y2("", "Y2", gsi::method("x1", &Y2::x1));

static gsi::ChildSubClass<Z_P, Y3, X> decl_y3("", "Y3",
                                              gsi::method("x1", &Y3::x1));

static gsi::ChildClass<Z_P, Y4> decl_y4("", "Y4", gsi::method("x1", &Y4::x1));
...
#if defined(HAVE_QT)
gsi::Class<SQ> decl_sq(
    "", "SQ",
    gsi::method("trigger_s0", &SQ::trigger_s0) +
        gsi::method("trigger_s1", &SQ::trigger_s1) +
        gsi::method("trigger_s2", &SQ::trigger_s2) +
        gsi::method("tag=", &SQ::set_tag) + gsi::method("tag", &SQ::tag) +
        gsi::qt_signal("s0()", "s0") + gsi::qt_signal<int>("s1(int)", "s1") +
        gsi::qt_signal<const QString &, SQ *>("s2(const QString &, SQ *)",
                                              "s2"));
#endif
...
gsi::Class<SE>
    decl_se("", "SE",
            gsi::method("trigger_s0", &SE::trigger_s0) +
                ...
                gsi::method("tag", &SE::tag) + gsi::event("s0", &SE::s0) +
                gsi::event("s1", &SE::s1) + gsi::event("s2", &SE::s2));

// ------------------------------------------------------------------
//  G and GFactory implementation and GSI declarations

GObject::GObject() { ++s_g_inst_count; }

GObject::~GObject() { --s_g_inst_count; }

size_t GObject::s_g_inst_count = 0;

GObject_P::GObject_P() : GObject() {
  //  .. nothing yet ..
}

int GObject_P::g() {
  return g_cb.can_issue() ? g_cb.issue<GObject, int>(&GObject::g)
                          : GObject::g();
}
...
GObject *GFactory_P::f(int z) {
  return f_cb.can_issue()
             ? f_cb.issue<GFactory, GObject *, int>(&GFactory::f, z)
             : GFactory::f(z);
}

int g_org(GObject_P *go) { return go->GObject::g(); }

int g_virtual(GObject *go) { return go->g(); }

static gsi::Class<GObject> decl_gobject_base("", "GObjectBase",
                                             gsi::method_ext("g_virtual",
                                                             &g_virtual) +
                                                 gsi::Methods());

static gsi::Class<GObject_P>
    decl_gobject(decl_gobject_base, "", "GObject",
                 gsi::method_ext("g_org", &g_org) +
                     gsi::callback("g", &GObject_P::g, &GObject_P::g_cb) +
                     gsi::method("g_inst_count", &GObject::g_inst_count));

GObject *f_org(GFactory_P *fo, int z) { return fo->GFactory::f(z); }

static gsi::Class<GFactory>
    decl_gfactory_base("", "GFactoryBase",
                       gsi::factory("create_f", &GFactory::create_f));

static gsi::Class<GFactory_P> decl_gfactory(
    decl_gfactory_base, "", "GFactory",
...
static gsi::Class<B2> decl_b2("", "B2", gsi::constant("C2", 17));

static gsi::Class<B3> decl_b3("", "B3", gsi::constant("C3", -1));
...
//  3 base classes
static gsi::Class<BB> decl_bb(decl_b1, "", "BB", gsi::method("d3", &BB::d3));
