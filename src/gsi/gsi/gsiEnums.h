 ...
 *  We will later bind this class to an enum. Binding is automatically resolved
 * by GSI. The adaptor provides a client-side object which connects to the enum
 * features. It employs the declaration class to retrieve the constants defined.
 */
template <class E> class EnumAdaptor {
public:
  EnumAdaptor() : m_e(E(0)) {}
 ...
 */
template <class E> struct EnumSpec {
...
public:
  ...
  virtual MethodBase *clone() const { return new EnumConst(*this); }
 ...
 *  This list is passed to the declaration class to represent the list of
 * constants in the enum. Building of the list is facilitated by the
 * "gsi::enum_const" function. The use case is supposed to be this:
 *
 *  @code
 ...
 *  @endcode
 */
template <class E> class EnumSpecs {
public:
  ...
  iterator begin() const { return m_specs.begin(); }

  iterator end() const { return m_specs.end(); }

  EnumSpecs &operator+(const EnumSpecs &other) { return operator+=(other); }
  ...
  E enum_from_string(const std::string &s) const {
    for (typename std::vector<EnumSpec<E>>::const_iterator spec =
             m_specs.begin();
         spec != m_specs.end(); ++spec) {
      if (spec->str == s) {
        return spec->evalue;
      }
    }
  ...
  static int enum_to_int(const E *e) { return int(*e); }

  static bool enum_eq(const E *e, const E &other) { return *e == other; }

  static bool enum_eq_with_int(const E *e, int other) {
    return int(*e) == other;
  }

  static bool enum_ne(const E *e, const E &other) { return *e != other; }

  static bool enum_ne_with_int(const E *e, int other) {
    return int(*e) != other;
  }

  static bool enum_lt(const E *e, const E &other) { return *e < other; }

  static bool enum_lt_with_int(const E *e, int other) {
    return int(*e) < other;
  }

  static E *new_enum_from_int(int i) { return new E(E(i)); }
  ...
  gsi::Methods methods() const {
    gsi::Methods m =
        gsi::constructor("new", &new_enum_from_int, gsi::arg("i"),
                         "@brief Creates an enum from an integer value") +
        gsi::constructor("new", &new_enum_from_string, gsi::arg("s"),
                         "@brief Creates an enum from a string value") +
        gsi::method_ext("to_s", &enum_to_string_ext,
                        "@brief Gets the symbolic string from an enum") +
        gsi::method_ext("inspect", &enum_to_string_inspect_ext,
                        "@brief Converts an enum to a visual string") +
        gsi::method_ext("to_i", &enum_to_int,
                        "@brief Gets the integer value from the enum") +
        gsi::method_ext("==", &enum_eq, gsi::arg("other"),
                        "@brief Compares two enums") +
        gsi::method_ext("==", &enum_eq_with_int, gsi::arg("other"),
                        "@brief Compares an enum with an integer value") +
        gsi::method_ext("!=", &enum_ne, gsi::arg("other"),
                        "@brief Compares two enums for inequality") +
        gsi::method_ext(
            "!=", &enum_ne_with_int, gsi::arg("other"),
            "@brief Compares an enum with an integer for inequality") +
        gsi::method_ext("<", &enum_lt, gsi::arg("other"),
                        "@brief Returns true if the first enum is less (in the "
                        "enum symbol order) than the second") +
        gsi::method_ext("<", &enum_lt_with_int, gsi::arg("other"),
                        "@brief Returns true if the enum is less (in the enum "
                        "symbol order) than the integer value");
 ...
 */
template <class E>
EnumSpecs<E> enum_const(const std::string &estr, E evalue,
 ...
 */
template <class E> class EnumImpl {
public:
  EnumImpl(const EnumSpecs<E> &specs) : m_specs(specs) {}

  gsi::Methods defs() const { return m_specs.defs(); }

  std::string enum_to_string(E e) const { return m_specs.enum_to_string(e); }
  ...
  const EnumSpecs<E> &specs() const { return m_specs; }
 ...
 */
template <class E>
class Enum : public Class<EnumAdaptor<E>, E>, public EnumImpl<E> {
public:
  Enum(const std::string &module, const std::string &name,
       const EnumSpecs<E> &specs, const std::string &doc = std::string())
      : Class<EnumAdaptor<E>, E>(module, name, specs.methods(), doc),
        EnumImpl<E>(specs) {}
};
 ...
 *  @endcode
 */
template <class P, class E> class EnumIn : public Enum<E> {
public:
  EnumIn(const std::string &module, const std::string &name,
         const EnumSpecs<E> &specs, const std::string &doc = std::string())
...
template <class E> class QFlagsClass;

/**
 *  @brief An adaptor class for the QFlags<E> template
 */
template <class E> class QFlagsAdaptor {
public:
  QFlagsAdaptor() : m_qf() {}
  ...
  QFlagsAdaptor(const std::string &s) {
    const QFlagsClass<E> *ecls =
        dynamic_cast<const QFlagsClass<E> *>(cls_decl<QFlags<E>>());
    tl_assert(ecls != 0);
 ...
 */
template <class E>
class QFlagsClass : public Class<QFlagsAdaptor<E>, QFlags<E>> {
public:
  ...
  QFlagsClass(const std::string &module, const std::string &name,
              const std::string &doc = std::string())
      : Class<QFlagsAdaptor<E>, QFlags<E>>(module, name, methods(), doc),
        m_enum_ext(ext_methods()) {}

  static QFlags<E> *new_from_i(int_repr i) { return new QFlags<E>(i); }

  static QFlags<E> *new_from_e(E e) { return new QFlags<E>(e); }
  ...
  static QFlags<E> *new_from_s(const std::string &s) {
    ...
    while (!ex.at_end()) {
      ...
      bool any = false;
      for (typename EnumSpecs<E>::iterator s = ecls->specs().begin();
           s != ecls->specs().end() && !any; ++s) {
  ...
  static int_repr to_i(const QFlags<E> *self) { return int_repr(*self); }
  ...
  static QFlags<E> invert(const QFlags<E> *self) { return ~*self; }
  ...
  static bool equal(const QFlags<E> *self, const QFlags<E> &other) {
    //  NOTE: in order to avoid ambiguities with non-explicit constructors of
    //  objects taking a QFlag as an argument, we compare int's explicitly. An
    //  example for such an ambiguity is QSurfaceFormat in Qt 5.5.1 which takes
    //  a QFlags<FormatOption> object in a non-explicit constructor.
    return int_repr(*self) == int_repr(other);
  }

  static gsi::Methods methods() {
    return gsi::constructor("new", &new_from_i, gsi::arg("i"),
                            "@brief Creates a flag set from an integer value") +
           gsi::constructor("new", &new_from_s, gsi::arg("s"),
                            "@brief Creates a flag set from a string") +
           gsi::constructor("new", &new_from_e, gsi::arg("e"),
                            "@brief Creates a flag set from an enum") +
           gsi::method_ext("to_s", &to_s,
                           "@brief Converts the flag set to a string") +
           gsi::method_ext("to_i", &to_i,
                           "@brief Converts the flag set to an integer") +
           gsi::method_ext(
               "testFlag", &test_flag, gsi::arg("flag"),
               "@brief Tests whether the flag set contains the given flag") +
           gsi::method_ext("inspect", &inspect,
                           "@brief Converts the flag set to a visual string") +
           gsi::method_ext("|", &or_op, gsi::arg("other"),
                           "@brief Computes the union of two flag sets") +
           gsi::method_ext("|", &or_op_with_e, gsi::arg("flag"),
                           "@brief Adds the given flag to the flag set and "
                           "returns the new flag set") +
           gsi::method_ext(
               "&", &and_op, gsi::arg("other"),
               "@brief Computes the intersection between the two flag sets") +
           gsi::method_ext(
               "&", &and_op_with_e, gsi::arg("flag"),
               "@brief Tests whether the given flag is contained in the flag "
               "set and returns a null flag set if not") +
           gsi::method_ext("^", &xor_op, gsi::arg("other"),
                           "@brief Computes the exclusive-or between the flag "
                           "set and the other flag set") +
           gsi::method_ext("^", &xor_op_with_e, gsi::arg("flag"),
                           "@brief Inverts the given flag in the flag set and "
                           "returns the new flag set") +
           gsi::method_ext("==", &equal_with_i, gsi::arg("other"),
                           "@brief Returns true if the flag set equals the "
                           "given integer value") +
           gsi::method_ext("==", &equal, gsi::arg("i"),
                           "@brief Returns true if the flag set equals the "
                           "given other flag set") +
           gsi::method_ext("!=", &not_equal_with_i, gsi::arg("other"),
                           "@brief Returns true if the flag set is not equal "
                           "to the given integer value") +
           gsi::method_ext("!=", &not_equal, gsi::arg("i"),
                           "@brief Returns true if the flag set is not equal "
                           "to the given other flag set") +
           gsi::method_ext("~", &invert,
                           "@brief Returns the inverted flag set");
  }
  ...
  static gsi::Methods ext_methods() {
    return gsi::method_ext(
               "|", &e_or_e, gsi::arg("other"),
               "@brief Creates a flag set by combining the two flags") +
           gsi::method_ext("|", &e_or_ee, gsi::arg("other"),
                           "@brief Combines the flag and the flag set");
  }
