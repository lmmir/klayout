...
Proxy::Proxy(const gsi::ClassBase *_cls_decl)
    : m_cls_decl(_cls_decl), m_obj(0), m_owned(false), m_const_ref(false),
      m_destroyed(false), m_can_destroy(false) {
  //  .. nothing yet ..
}
  ...
  if (cls) {
    ...
    if (o) {
      if (cls->is_managed()) {
        ...
        //  Fallback: the object is not gsi-enabled: we use the ownership flag
        //  in this case to keep it alive. This will not reset the ownership
        //  flag for all clients.
        m_owned = false;
      }
    }
  }
...
void *Proxy::set_internal(void *obj, bool owned, bool const_ref,
                          bool can_destroy) {
      ...
      if (cls->is_managed()) {
        ...
        if (gsi_object) {
          gsi_object->status_changed_event().remove(
              this, &Proxy::object_status_changed);
        }
      }
