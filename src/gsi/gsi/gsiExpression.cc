...
public:
  ...
  ExpressionMethodTableEntry(const std::string &name) : m_name(name) {}
 ...
 * strategy.
 */
class ExpressionMethodTable : public gsi::PerClassClientSpecificData {
public:
  /**
  ...
  std::pair<bool, size_t> find(bool st, const std::string &name) const {
    std::map<std::pair<bool, std::string>, size_t>::const_iterator t =
   ...
   */
  const std::string &name(size_t mid) const { return m_table[mid].name(); }
  ...
  static const ExpressionMethodTable *
  method_table_by_class(const gsi::ClassBase *cls_decl) {
    const ExpressionMethodTable *mt =
        dynamic_cast<const ExpressionMethodTable *>(cls_decl->gsi_data());
    tl_assert(mt != 0);
    return mt;
  }
...
private:
  ...
  /**
   *  @brief Adds the given method with the given name to the list of methods
  ...
  ExpressionMethodTable(const gsi::ClassBase *cls_decl)
    ...
    for (gsi::ClassBase::method_iterator m = cls_decl->begin_methods();
         m != cls_decl->end_methods(); ++m) {

      if (!(*m)->is_callback()) {

        for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
             syn != (*m)->end_synonyms(); ++syn) {
          if (syn->is_setter) {
    ...
    //  do some cleanup
    for (std::vector<ExpressionMethodTableEntry>::iterator m = m_table.begin();
         m != m_table.end(); ++m) {
      m->finish();
    }
  }
 ...
 */
inline void *get_object(tl::Variant &var) { return var.to_user(); }
...
template <class R> struct test_arg_func {
  void operator()(bool *ret, const tl::Variant &arg,
                  const gsi::ArgType & /*atype*/, bool /*loose*/) {
    *ret = arg.can_convert_to<R>();
  }
};

template <> struct test_arg_func<gsi::VoidType> {
  void operator()(bool *ret, const tl::Variant & /*arg*/,
                  const gsi::ArgType & /*atype*/, bool /*loose*/) {
    *ret = true;
  }
...
template <> struct test_arg_func<gsi::ObjectType> {
  void operator()(bool *ret, const tl::Variant &arg, const gsi::ArgType &atype,
    ...
    if (!cls) {
      *ret = false;
    } else if (!cls->gsi_cls()->is_derived_from(atype.cls()) &&
               (!loose || !cls->gsi_cls()->can_convert_to(atype.cls()))) {
      *ret = false;
...
template <> struct test_arg_func<gsi::VectorType> {
  void operator()(bool *ret, const tl::Variant &arg, const gsi::ArgType &atype,
                  bool loose) {
    if (!arg.is_list()) {
      *ret = false;
    ...
    *ret = true;
    for (tl::Variant::const_iterator v = arg.begin(); v != arg.end() && *ret;
         ++v) {
...
template <> struct test_arg_func<gsi::MapType> {
  void operator()(bool *ret, const tl::Variant &arg, const gsi::ArgType &atype,
                  bool loose) {
    //  Note: delegating that to the function avoids "injected class name used
    //  as template template expression" warning
    if (!arg.is_array()) {
      *ret = false;
      return;
    }
    ...
    *ret = true;
    for (tl::Variant::const_array_iterator a = arg.begin_array();
         a != arg.end_array() && *ret; ++a) {
...
template <class R> struct var2c {
  static R get(const tl::Variant &rval) { return rval.to<R>(); }
};

template <> struct var2c<tl::Variant> {
  static const tl::Variant &get(const tl::Variant &rval) { return rval; }
};
 ...
 */
class VariantBasedVectorAdaptorIterator : public gsi::VectorAdaptorIterator {
public:
  VariantBasedVectorAdaptorIterator(tl::Variant::iterator b,
                                    tl::Variant::iterator e,
 ...
 *  @brief An adaptor for a vector which uses the tl::Variant's list perspective
 */
class VariantBasedVectorAdaptor : public gsi::VectorAdaptor {
public:
 ...
 */
class VariantBasedMapAdaptorIterator : public gsi::MapAdaptorIterator {
public:
  VariantBasedMapAdaptorIterator(tl::Variant::array_iterator b,
                                 tl::Variant::array_iterator e,
                                 const gsi::ArgType *ainner,
 ...
 *  @brief An adaptor for a vector which uses the tl::Variant's list perspective
 */
class VariantBasedMapAdaptor : public gsi::MapAdaptor {
public:
  VariantBasedMapAdaptor(tl::Variant *var, const gsi::ArgType *ainner,
                         const gsi::ArgType *ainner_k);
 ...
 */
template <class R> struct writer {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    if (arg->is_nil() && atype.type() != gsi::T_var) {

      if (!(atype.is_ptr() || atype.is_cptr())) {
        throw tl::Exception(tl::to_string(
            tr("Arguments of reference or direct type cannot be passed nil")));
      ...
      if (atype.is_ref() || atype.is_ptr()) {
        ...
        // TODO: morph the variant to the requested type and pass its pointer
        // (requires a non-const reference for arg)
        // -> we would have a reference that can modify the argument (out
        // parameter).
        ...
        //  Note: POD's are written as copies for const refs, so we can pass a
        //  temporary here: (avoids having to create a temp object)
        aa->write<const R &>(var2c<R>::get(*arg));
      } else if (atype.is_cptr()) {
        //  Note: POD's are written as copies for const ptrs, so we can pass a
        //  temporary here: (avoids having to create a temp object)
        R r = var2c<R>::get(*arg);
        aa->write<const R *>(&r);
      } else {
 ...
 */
template <> struct writer<StringType> {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg,
                  const gsi::ArgType &atype, tl::Heap *) {
    ...
    if (arg->is_nil()) {
      ...
      if (!(atype.is_ptr() || atype.is_cptr())) {
        ...
        aa->write<void *>(
            (void *)new StringAdaptorImpl<std::string>(std::string()));
      } else {
      ...
      // (requires a non-const reference for arg)
      // -> we would have a reference that can modify the argument (out
      // parameter). NOTE: by convention we pass the ownership to the receiver
      // for adaptors.
      aa->write<void *>(
          (void *)new StringAdaptorImpl<std::string>(arg->to_string()));
    }
  }
};
 ...
 */
template <> struct writer<VariantType> {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg, const gsi::ArgType &,
                  tl::Heap *) {
    //  TODO: clarify: is nil a zero-pointer to a variant or a pointer to a
    //  "nil" variant?
 ...
 */
template <> struct writer<VectorType> {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg,
                  const gsi::ArgType &atype, tl::Heap *) {
    if (arg->is_nil()) {
      if (!(atype.is_ptr() || atype.is_cptr())) {
      ...
      tl_assert(atype.inner() != 0);
      aa->write<void *>(
          (void *)new VariantBasedVectorAdaptor(arg, atype.inner()));
    }
  }
};
 ...
 */
template <> struct writer<MapType> {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg,
                  const gsi::ArgType &atype, tl::Heap *) {
    if (arg->is_nil()) {
      if (!(atype.is_ptr() || atype.is_cptr())) {
      ...
      tl_assert(atype.inner() != 0);
      tl_assert(atype.inner_k() != 0);
      aa->write<void *>((void *)new VariantBasedMapAdaptor(arg, atype.inner(),
                                                           atype.inner_k()));
    }
  }
};
 ...
 */
template <> struct writer<gsi::VoidType> {
  void operator()(gsi::SerialArgs *, tl::Variant *, const gsi::ArgType &,
                  tl::Heap *) {
 ...
 *  @brief Specialization for void
 */
template <> struct writer<gsi::ObjectType> {
  void operator()(gsi::SerialArgs *aa, tl::Variant *arg,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    if (atype.is_ref() || atype.is_cref() || atype.is_ptr() ||
        atype.is_cptr()) {
      ...
      if (arg->is_nil()) {
        ...
        if (atype.is_ref() || atype.is_cref()) {
          throw tl::Exception(
              tl::to_string(tr("Cannot pass nil to reference parameters")));
        }
        ...
        if (!arg->is_user()) {
          throw tl::Exception(tl::sprintf(
              tl::to_string(
                  tr("Unexpected object type (expected argument of class %s)")),
              atype.cls()->name()));
        }
        ...
        if (cls->is_const() && (atype.is_ref() || atype.is_ptr())) {
          throw tl::Exception(tl::sprintf(
              tl::to_string(tr("Cannot pass a const reference of class %s to a "
                               "non-const reference or pointer parameter")),
              atype.cls()->name()));
        }
        ...
        if (cls->gsi_cls()->is_derived_from(atype.cls())) {
          ...
          if (cls->gsi_cls()->adapted_type_info()) {
            ...
            aa->write<void *>(
                (void *)cls->gsi_cls()->adapted_from_obj(get_object(*arg)));
          } else {
            aa->write<void *>(get_object(*arg));
          }

        } else if ((atype.is_cref() || atype.is_cptr()) &&
                   cls->gsi_cls()->can_convert_to(atype.cls())) {

          //  We can convert objects for cref and cptr, but ownership over these
          //  objects is not transferred. Hence we have to keep them on the
          //  heap.
          void *new_obj =
              atype.cls()->create_obj_from(cls->gsi_cls(), get_object(*arg));
      ...
      if (cls->gsi_cls()->is_derived_from(atype.cls())) {
        ...
        if (cls->gsi_cls()->adapted_type_info()) {
          aa->write<void *>(
              cls->gsi_cls()->create_adapted_from_obj(get_object(*arg)));
        } else {
 ...
 */
template <class R> struct reader {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    if (atype.is_ref()) {
 ...
 */
template <> struct reader<gsi::StringType> {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr, const gsi::ArgType &,
                  tl::Heap *heap) {
 ...
 */
template <> struct reader<gsi::VariantType> {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr, const gsi::ArgType &,
                  tl::Heap *heap) {
    std::unique_ptr<VariantAdaptor> a(
        (VariantAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *out = tl::Variant();
    } else {
      *out = a->var();
 ...
 */
template <> struct reader<MapType> {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr,
                  const gsi::ArgType &atype, tl::Heap *heap) {
 ...
 */
template <> struct reader<VectorType> {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr,
                  const gsi::ArgType &atype, tl::Heap *heap) {
 ...
 */
template <> struct reader<ObjectType> {
  void operator()(tl::Variant *out, gsi::SerialArgs *rr,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    ...
    bool owner = true;
    if (atype.is_ptr() || atype.is_cptr() || atype.is_ref() ||
        atype.is_cref()) {
    ...
    if (obj == 0) {
      ...
      *out = tl::Variant();
      ...
      const tl::VariantUserClassBase *cls =
          clsact->var_cls(atype.is_cref() || atype.is_cptr());
      tl_assert(cls != 0);
 ...
 */
template <> struct reader<VoidType> {
...
VariantBasedVectorAdaptorIterator::VariantBasedVectorAdaptorIterator(
    tl::Variant::iterator b, tl::Variant::iterator e,
    const gsi::ArgType *ainner)
    : m_b(b), m_e(e), mp_ainner(ainner) {
  //  .. nothing yet ..
}

void VariantBasedVectorAdaptorIterator::get(SerialArgs &w,
                                            tl::Heap &heap) const {
  gsi::do_on_type<writer>()(mp_ainner->type(), &w, &*m_b, *mp_ainner, &heap);
}

bool VariantBasedVectorAdaptorIterator::at_end() const { return m_b == m_e; }

void VariantBasedVectorAdaptorIterator::inc() { ++m_b; }

// ---------------------------------------------------------------------
//  VariantBasedVectorAdaptor implementation

VariantBasedVectorAdaptor::VariantBasedVectorAdaptor(tl::Variant *var,
                                                     const gsi::ArgType *ainner)
    : mp_ainner(ainner), mp_var(var) {}

VectorAdaptorIterator *VariantBasedVectorAdaptor::create_iterator() const {
  return new VariantBasedVectorAdaptorIterator(mp_var->begin(), mp_var->end(),
                                               mp_ainner);
}
...
void VariantBasedVectorAdaptor::clear() { mp_var->set_list(); }

size_t VariantBasedVectorAdaptor::size() const { return mp_var->size(); }
...
VariantBasedMapAdaptorIterator::VariantBasedMapAdaptorIterator(
    tl::Variant::array_iterator b, tl::Variant::array_iterator e,
    const gsi::ArgType *ainner, const gsi::ArgType *ainner_k)
    : m_b(b), m_e(e), mp_ainner(ainner), mp_ainner_k(ainner_k) {
  //  .. nothing yet ..
}

void VariantBasedMapAdaptorIterator::get(SerialArgs &w, tl::Heap &heap) const {
  //  Note: the const_cast is ugly but in this context we won't modify the
  //  variant given as the key. And it lets us keep the interface tidy.
  gsi::do_on_type<writer>()(mp_ainner_k->type(), &w,
                            const_cast<tl::Variant *>(&m_b->first),
                            *mp_ainner_k, &heap);
  gsi::do_on_type<writer>()(mp_ainner->type(), &w, &m_b->second, *mp_ainner,
                            &heap);
}

bool VariantBasedMapAdaptorIterator::at_end() const { return m_b == m_e; }

void VariantBasedMapAdaptorIterator::inc() { ++m_b; }

// ---------------------------------------------------------------------
//  VariantBasedMapAdaptor implementation

VariantBasedMapAdaptor::VariantBasedMapAdaptor(tl::Variant *var,
                                               const gsi::ArgType *ainner,
                                               const gsi::ArgType *ainner_k)
    : mp_ainner(ainner), mp_ainner_k(ainner_k), mp_var(var) {}

MapAdaptorIterator *VariantBasedMapAdaptor::create_iterator() const {
  return new VariantBasedMapAdaptorIterator(
      mp_var->begin_array(), mp_var->end_array(), mp_ainner, mp_ainner_k);
}
...
void VariantBasedMapAdaptor::clear() { mp_var->set_array(); }

size_t VariantBasedMapAdaptor::size() const { return mp_var->array_size(); }
...
class EvalClassFunction : public tl::EvalFunction {
public:
  ...
  void execute(const tl::ExpressionParserContext & /*context*/,
               tl::Variant &out, const std::vector<tl::Variant> &args) const {
    if (!args.empty()) {
      throw tl::Exception(tl::to_string(tr("Class '%s' is not a function - use "
...
void GSI_PUBLIC initialize_expressions() {
  //  just in case this did not happen yet ...
  gsi::initialize();

  //  Go through all classes (maybe again)
  std::list<const gsi::ClassBase *> classes =
      gsi::ClassBase::classes_in_definition_order();
  for (std::list<const gsi::ClassBase *>::const_iterator c = classes.begin();
       c != classes.end(); ++c) {
    ...
    //  register a function that creates a class object (use a function to avoid
    //  issues with late destruction of global variables which the class object
    //  is already gone)
...
void VariantUserClassImpl::initialize(
    const gsi::ClassBase *cls, const tl::VariantUserClassBase *self,
...
VariantUserClassImpl::~VariantUserClassImpl() { mp_cls = 0; }

bool VariantUserClassImpl::has_method(const std::string &method) const {
  const gsi::ClassBase *cls = mp_cls;

  while (cls) {
    if (ExpressionMethodTable::method_table_by_class(cls)
            ->find(false, method)
            .first) {
      return true;
    }
...
std::string VariantUserClassImpl::to_string_impl(void *obj) const {
  if (obj) {
...
tl::Variant VariantUserClassImpl::to_variant_impl(void *obj) const {
  if (obj) {
...
double VariantUserClassImpl::to_double_impl(void *obj) const {
  if (obj) {
...
void VariantUserClassImpl::execute(const tl::ExpressionParserContext &context,
  ...
  if (mp_object_cls == 0 && method == "is_a") {
    ...
    if (args.size() != 1) {
      throw tl::EvalError(
          tl::to_string(tr("'is_a' method requires exactly one argument")),
          context);
    }
    ...
    if (args.size() != 0) {
      throw tl::EvalError(
          tl::to_string(tr("'dup' method does not allow arguments")), context);
    }
...
static tl::Variant special_method_impl(gsi::MethodBase::special_method_type smt,
                                       tl::Variant &self,
                                       const std::vector<tl::Variant> &args) {
  if (smt == gsi::MethodBase::Destroy) {
    ...
    if (!args.front().is_user() || self.user_cls() != args.front().user_cls()) {
      throw tl::Exception(tl::to_string(tr(
          "Source and target object must be of the same type for assignment")));
    }
...
static std::pair<const ExpressionMethodTable *, size_t>
find_method(const gsi::ClassBase *cls, bool as_static,
            const std::string &method) {
  ...
  while (cls) {
    ...
    for (auto cc = cls->begin_child_classes(); cc != cls->end_child_classes();
         ++cc) {
      if (cc->name().empty()) {
        std::pair<const ExpressionMethodTable *, size_t> m =
            find_method(cc->declaration(), true, method);
        if (m.first) {
          return m;
        }
      }
...
static const gsi::ClassBase *find_class_scope(const gsi::ClassBase *cls,
...
void VariantUserClassImpl::execute_gsi(
    const tl::ExpressionParserContext & /*context*/, tl::Variant &out,
  ...
  if (!mt) {
    ...
    if (scope) {
      ...
      if (!args.empty()) {
        throw tl::Exception(
            tl::to_string(
                tr("'%s' is not a function and cannot have parameters")),
            method);
      }
      ...
      throw tl::Exception(
          tl::to_string(tr("Unknown method '%s' of class '%s'")), method,
          clsact->name());
    }
  }
  ...
  for (ExpressionMethodTableEntry::method_iterator m = mt->begin(mid);
       m != mt->end(mid); ++m) {

    if ((*m)->is_signal()) {
      throw tl::Exception(tl::sprintf(
          tl::to_string(
              tr("Signals are not supported inside expressions (event %s)")),
          method.c_str()));
  ...
  if (!meth) {
    ...
    throw tl::Exception(
        tl::sprintf(tl::to_string(tr("Invalid number of arguments for method "
                                     "%s, class %s (got %d, expected %s)")),
                    method.c_str(), mp_cls->name(), int(args.size()), nargs_s));
  }
  ...
  if (candidates > 1) {
    ...
    for (ExpressionMethodTableEntry::method_iterator m = mt->begin(mid);
      ...
      if (!(*m)->is_callback() && !(*m)->is_signal()) {
        ...
        bool is_valid =
            (*m)->compatible_with_num_args((unsigned int)args.size());
        int sc = 0;
        int i = 0;
        for (gsi::MethodBase::argument_iterator a = (*m)->begin_arguments();
             is_valid && i < int(args.size()) && a != (*m)->end_arguments();
             ++a, ++i) {
  ...
  if (!meth) {
    throw tl::Exception(tl::sprintf(
        tl::to_string(
            tr("No method with matching arguments for method %s, class %s")),
        method.c_str(), mp_cls->name()));
  }

  if (candidates > 1) {
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr("Ambiguous overload variants for method %s, class %s "
                         "- multiple method declarations match arguments")),
        method.c_str(), mp_cls->name()));
  }
  ...
  if (m_is_const && !meth->is_const()) {
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr(
            "Cannot call non-const method %s, class %s on a const reference")),
        method.c_str(), mp_cls->name()));
  }
  ...
  if (meth->is_signal()) {
    ...
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr("Signals not supported yet (method %s, class %s)")),
        method.c_str(), mp_cls->name()));
    ...
    for (gsi::MethodBase::argument_iterator a = meth->begin_arguments();
         a != meth->end_arguments() && narg < args.size(); ++a, ++narg) {
      try {
        //  Note: this const_cast is ugly, but it will basically enable "out"
        //  parameters
        //  TODO: clean this up.
        gsi::do_on_type<writer>()(a->type(), &arglist,
                                  const_cast<tl::Variant *>(&args[narg]), *a,
                                  &heap);
      } catch (tl::Exception &ex) {
        std::string msg =
            ex.msg() + tl::sprintf(tl::to_string(tr(" (argument '%s')")),
                                   a->spec()->name());
    ...
    if (meth->ret_type().is_iter()) {
      ...
      throw tl::Exception(
          tl::sprintf(tl::to_string(tr(
                          "Iterators not supported yet (method %s, class %s)")),
                      method.c_str(), mp_cls->name()));
    } else {
      out = tl::Variant();
      try {
        gsi::do_on_type<reader>()(meth->ret_type().type(), &out, &retlist,
                                  meth->ret_type(), &heap);
      } catch (tl::Exception &ex) {
