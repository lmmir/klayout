 ...
 *
 *  This object represents a GSI class. It provides the methods to translate the
 * typeless object pointer to a real C++ object and to perform some basic
 * operations on the object. It also provides metainformation such as method
 * declarations and similar.
 */
class GSI_PUBLIC ClassBase : public tl::Object {
public:
   ...
   *  @brief Constructor
   *
   *  The constructor supplies information about the connector class (can be 0),
   * a documentation string and the method declarations.
   */
   ...
   */
  const ClassBase *base() const { return mp_base; }

  /**
   *  @brief Returns a pointer to the type_info object if the class is an
   * adaptor
   *
   ...
   */
  virtual const std::type_info *adapted_type_info() const { return 0; }

  /**
   *  @brief Returns the "real" (consolidated) declaration object
   *
   *  The actual declaration object may be different from this declaration
   * because of the class extension mechanism. Using that mechanism, class
   * declarations can extend over multiple ClassBase objects. The individual
   * declarations are merged and a consolidated class declaration object is
   * provided.
   */
  virtual const ClassBase *declaration() const { return 0; }

  /**
   *  @brief Consolidates the declaration
   *
   *  Before all classes are finalized and the script interpreters start to
   * instantiate them, all class declarations are consolidated. If the
   * consolidate method returns true, the class declaration is kept. Otherwise
   * it is discarded.
   */
  virtual bool consolidate() const { return false; }
   ...
   */
  const ClassBase *parent() const { return mp_parent; }
   ...
   */
  static class_iterator begin_classes() { return collection().begin(); }

  /**
   *  @brief Iterates all classes present (end)
   */
  static class_iterator end_classes() { return collection().end(); }

  /**
   *  @brief Iterates all freshly registered classes (begin)
   *  This collection is emptied on "merge_declarations".
   */
  static class_iterator begin_new_classes() { return new_collection().begin(); }

  /**
   *  @brief Iterates all freshly registered classes (begin)
   */
  static class_iterator end_new_classes() { return new_collection().end(); }
  ...
  classes_in_definition_order(const char *mod_name = 0);

  /**
   *  @brief Iterates the methods (begin)
   */
  method_iterator begin_methods() const { return m_methods.begin(); }

  /**
   *  @brief Iterates the methods (end)
   */
  method_iterator end_methods() const { return m_methods.end(); }

  /**
   *  @brief Iterates the constructor methods (begin)
   */
  method_iterator begin_constructors() const { return m_constructors.begin(); }

  /**
   *  @brief Iterates the constructor methods (end)
   */
  method_iterator end_constructors() const { return m_constructors.end(); }

  /**
   *  @brief Iterates the callback methods (begin)
   */
  method_iterator begin_callbacks() const { return m_callbacks.begin(); }

  /**
   *  @brief Iterates the callback methods (end)
   */
  method_iterator end_callbacks() const { return m_callbacks.end(); }
  ...
  /**
   *  @brief Returns true, if this class can be used to initialize an object
   * with type "target"
   */
   ...
   */
  virtual void destroy(void * /*obj*/) const { tl_assert(false); }
   ...
   *  available if there is no default constructor (can_default_create is
   * false). In that case, some static factory method must be provided.
   */
   ...
   *
   *  This flag is true, if the class supports destruction of objects of this
   * kind.
   */
   ...
   *
   *  This flag is true, if the class supports default construction of objects
   * of this kind.
   */
  ...
  /**
   *  @brief Class implementation: returns true if this class binds to a script
   * class
   */
   ...
   *
   *  When this method is called, it is guaranteed that the object is at least
   * of the base class type. This implies that there is a base class when this
   * method is called.
   */
   ...
   *  @brief Class implementation: gets C++ type of object
   *
   *  This method delivers the class declaration for an subclassed object p. p
   * must be of a subclass (derived class) of this class. This method will the
   * declaration object of object p.
   */
   ...
   *  no dynamic allocation of a gsi::ObjectBase has happened yet. This is
   * useful in case of the Qt-GSI bridge which means a special Qt object needs
   * to be created in order to provide the gsi::ObjectBase interface. If
   * required is false, this initialization does not need to happen.
   */
  virtual gsi::ObjectBase *gsi_object(void * /*p*/,
  ...
  /**
   *  @brief Class Implementation: provide variant binding information
   * (constness)
   */
  ...
  /**
   *  @brief Returns true, if the class is an external class provided by Python
   * or Ruby code
   */
   ...
   *
   *  This method will be called by the GSI system to provide initialization
   * after the static initialization. Some schemes cannot be implementation
   * statically, plus the initialization order is undetermined for static
   * initialization. In that case, this initialization step is useful. It will
   * call the initialize method on all method declarations.
   */
   ...
   */
  PerClassClientSpecificData *data(int ch) const { return mp_data[ch].get(); }

  /**
   *  @brief Sets the per-client data for gsi::Expressions
   *  This is a special slot since no client index is available for
   * gsi::Expressions.
   */
...
protected:
  ...
  void set_module(const std::string &m) { m_module = m; }
...
private:
  ...
  mutable std::unique_ptr<PerClassClientSpecificData>
      mp_data[ClientIndex::MaxClientIndex];
