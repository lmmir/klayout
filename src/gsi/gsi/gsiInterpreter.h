 ...
 *
 *  Using this interface, the interpreter can output text on stdout or stderr
 * streams. This interface is implemented by ConsoleImpl.
 */
...
public:
  ...
  /**
   *  @brief Returns true, if the console is a TTY (will probably enable
   * coloring)
   */
...
/**
 *  @brief An interface delivering a stack trace for the ExecutionHandler's
 * trace function
 */
 ...
 *  is executed, the handler allows intercepting the execution and implement
...
public:
   ...
   *
   *  This method is called when the execution begins. It can be used to clear
   * any caches for example.
   */
   ...
   */
  virtual void
  exception_thrown(Interpreter * /*interpreter*/, size_t /*file_id*/,
                   int /*line*/, const std::string & /*eclass*/,
                   const std::string & /*emsg*/,
                   const StackTraceProvider * /*stack_trace_provider*/) {}

  /**
   *  @brief This method is called during execution
   */
  virtual void trace(Interpreter * /*interpreter*/, size_t /*file_id*/,
                     int /*line*/,
                     const StackTraceProvider * /*stack_trace_provider*/) {}
   ...
   *
   *  This method is supposed to deliver an arbitrary integer ID (which can hold
   * a pointer for example) for a given file path string. When the trace method
   * is called, this ID is used to identify the file instead of the heavy file
   * path.
   */
  virtual size_t id_for_path(Interpreter * /*interpreter*/,
 ...
 */
class GSI_PUBLIC Interpreter : public tl::RegisteredClass<Interpreter> {
public:
  /**
   *  @brief Constructor
   */
   ...
   *
   *  The filename and line gives the location at which the evaluation should
   * begin. This location is indicated in the stack trace and error messages
   *
   *  "context" is the evaluation context index: -1 is the global context, 0 is
   * the first context on the call stack, 1 the second and so on.
   */
  virtual void eval_string(const char *string, const char *filename = 0,
                           int line = 1, int context = -1) = 0;

  /**
   *  @brief Evaluates the given expression string and returns the results as a
   * variant
   *
   *  The filename and line gives the location at which the evaluation should
   * begin. This location is indicated in the stack trace and error messages The
   * result is converted to a variant and returned. When that is not possible,
   * the result is converted to a string and then put into the variant.
   *
   *  "context" is the evaluation context index: -1 is the global context, 0 is
   * the first context on the call stack, 1 the second and so on.
   */
  virtual tl::Variant eval_expr(const char *string, const char *filename = 0,
                                int line = 1, int context = -1) = 0;
   ...
   * the first context on the call stack, 1 the second and so on.
   */
  virtual void eval_string_and_print(const char *string,
                                     const char *filename = 0, int line = 1,
                                     int context = -1) = 0;
