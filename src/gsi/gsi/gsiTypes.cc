...
ArgType::ArgType()
    : m_type(T_void), mp_spec(0), mp_inner(0), mp_inner_k(0), m_is_ref(false),
      m_is_ptr(false), m_is_cref(false), m_is_cptr(false), m_is_iter(false),
      m_owns_spec(false), m_pass_obj(false), m_prefer_copy(false), mp_cls(0),
      m_size(0) {}
...
ArgType &ArgType::operator=(const ArgType &other) {
  if (this != &other) {
...
bool ArgType::operator==(const ArgType &b) const {
  ...
  return m_type == b.m_type && m_is_iter == b.m_is_iter &&
         m_is_ref == b.m_is_ref && m_is_cref == b.m_is_cref &&
         m_is_ptr == b.m_is_ptr && m_is_cptr == b.m_is_cptr &&
         mp_cls == b.mp_cls && m_pass_obj == b.m_pass_obj &&
         m_prefer_copy == b.m_prefer_copy;
}

void ArgType::release_spec() {
