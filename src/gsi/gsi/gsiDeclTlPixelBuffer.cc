...
tl::color_t get_pixel_from_pixel_buffer(const tl::PixelBuffer *pb,
...
void set_pixel_in_pixel_buffer(tl::PixelBuffer *pb, unsigned int x,
                               unsigned int y, tl::color_t c) {
  if (!pb->transparent()) {
...
#else
  throw tl::Exception(
      tl::to_string(tr("No PNG support compiled in for PixelBuffer")));
  return tl::PixelBuffer();
#endif
}

//  TODO: there should be some more efficient version of byte strings which
//  avoid copies
static tl::PixelBuffer pixel_buffer_from_png(const std::vector<char> &data) {
  ...
  QImage img = QImage::fromData((const uchar *)data.begin().operator->(),
                                int(data.size()));
  return tl::PixelBuffer::from_image(img);
#else
  throw tl::Exception(
...
static void write_pixel_buffer(const tl::PixelBuffer *pb,
...
static std::vector<char> pixel_buffer_to_png(const tl::PixelBuffer *pb) {
  ...
  return std::vector<char>(data_stream.data(),
...
Class<tl::PixelBuffer> decl_PixelBuffer(
    "lay", "PixelBuffer",
    gsi::constructor("new", &create_pixel_buffer, gsi::arg("width"),
                     gsi::arg("height"),
                     ...
                     "\n"
                     "The pixels are basically uninitialized. You will need to "
                     "use \\fill to initialize them to a certain value.") +
        gsi::method("==", &tl::PixelBuffer::operator==, gsi::arg("other"),
                    "@brief Returns a value indicating whether self is "
                    "identical to the other image\n") +
        gsi::method("!=", &tl::PixelBuffer::operator!=, gsi::arg("other"),
                    "@brief Returns a value indicating whether self is not "
                    "identical to the other image\n") +
        gsi::method("transparent=", &tl::PixelBuffer::set_transparent,
                    gsi::arg("t"),
                    "@brief Sets a flag indicating whether the pixel buffer "
                    "supports an alpha channel\n"
                    "\n"
                    "By default, the pixel buffer does not support an alpha "
                    "channel.\n") +
        gsi::method("transparent", &tl::PixelBuffer::transparent,
                    "@brief Gets a flag indicating whether the pixel buffer "
                    "supports an alpha channel\n") +
        gsi::method(
            "fill", &tl::PixelBuffer::fill, gsi::arg("color"),
        ...
        gsi::method_ext(
            "set_pixel", &set_pixel_in_pixel_buffer, gsi::arg("x"),
            gsi::arg("y"), gsi::arg("c"),
            "@brief Sets the value of the pixel at position x, y\n") +
        gsi::method_ext(
            "pixel", &get_pixel_from_pixel_buffer, gsi::arg("x"), gsi::arg("y"),
        ...
        gsi::method("read_png", &read_pixel_buffer, gsi::arg("file"),
                    ...
                    "\n"
                    "This method may not be available if PNG support is not "
                    "compiled into KLayout.") +
        ...
        gsi::method_ext("write_png", &write_pixel_buffer, gsi::arg("file"),
                        ...
                        "\n"
                        "This method may not be available if PNG support is "
                        "not compiled into KLayout.") +
        ...
        gsi::method("patch", &tl::PixelBuffer::patch, gsi::arg("other"),
                    ...
                    "\n"
                    "This method is the inverse of \\diff - it will patch the "
                    "difference image created by diff into this "
                    "pixel buffer. Note that this method will not do true "
                    "alpha blending and requires the other pixel buffer "
                    "to have the same format than self. Self will be modified "
                    "by this operation.") +
        gsi::method("diff", &tl::PixelBuffer::diff, gsi::arg("other"),
                    "@brief Creates a difference image\n"
                    "\n"
                    "This method is provided to support transfer of image "
                    "differences - i.e. small updates instead of full images. "
                    "It works for non-transparent images only and generates an "
                    "image with transpareny enabled and with the new pixel "
                    "values for pixels that have changed. "
                    "The alpha value will be 0 for identical images and 255 "
                    "for pixels with different values. "
                    "This way, the difference image can be painted over the "
                    "original image to generate the new image."),
    "@brief A simplistic pixel buffer representing an image of ARGB32 or RGB32 "
    "values\n"
    "\n"
    "This object is mainly provided for offline rendering of layouts in "
    "Qt-less environments.\n"
    "It supports a rectangular pixel space with color values encoded in 32bit "
    "integers. It supports "
    "transparency through an optional alpha channel. The color format for a "
    "pixel is "
    "\"0xAARRGGBB\" where 'AA' is the alpha value which is ignored in "
    "non-transparent mode.\n"
    "\n"
    "This class supports basic operations such as initialization, single-pixel "
    "access and I/O to PNG.\n"
    "\n"
...
bool get_pixel_from_bitmap_buffer(const tl::BitmapBuffer *pb, unsigned int x,
                                  unsigned int y) {
  if (x < pb->width() && y < pb->height()) {
...
void set_pixel_in_bitmap_buffer(tl::BitmapBuffer *pb, unsigned int x,
                                unsigned int y, bool c) {
  if (x < pb->width() && y < pb->height()) {
    if (c) {
...
#else
  throw tl::Exception(
      tl::to_string(tr("No PNG support compiled in for BitmapBuffer")));
...
static void write_bitmap_buffer(const tl::BitmapBuffer *pb,
                                const std::string &file) {
#if defined(HAVE_PNG)
  tl::OutputStream stream(file);
  pb->write_png(stream);
...
Class<tl::BitmapBuffer> decl_BitmapBuffer(
    "lay", "BitmapBuffer",
    gsi::constructor("new", &create_bitmap_buffer, gsi::arg("width"),
                     gsi::arg("height"),
                     "@brief Creates a pixel buffer object\n"
                     "\n"
                     "@param width The width in pixels\n"
        ...
        gsi::method("!=", &tl::BitmapBuffer::operator!=, gsi::arg("other"),
                    ...
                    "identical to the other image\n") +
        gsi::method(
            "fill", &tl::BitmapBuffer::fill, gsi::arg("color"),
            "@brief Fills the pixel buffer with the given pixel value\n") +
        ...
        gsi::method("height", &tl::BitmapBuffer::height,
                    "@brief Gets the height of the pixel buffer in pixels\n") +
        gsi::method_ext(
            "set_pixel", &set_pixel_in_bitmap_buffer, gsi::arg("x"),
            gsi::arg("y"), gsi::arg("c"),
            "@brief Sets the value of the pixel at position x, y\n") +
        gsi::method_ext(
            "pixel", &get_pixel_from_bitmap_buffer, gsi::arg("x"),
            gsi::arg("y"),
            "@brief Gets the value of the pixel at position x, y\n") +
    ...
    "This object is mainly provided for offline rendering of layouts in "
    "Qt-less environments.\n"
    "It supports a rectangular pixel space with color values encoded in single "
    "bits.\n"
    "\n"
    "This class supports basic operations such as initialization, single-pixel "
    "access and I/O to PNG.\n"
