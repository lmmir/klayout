 ...
 *
 *  The client index is used to indicate a certain client for the object
 * (Python, Ruby ...)
 */
struct ClientIndex {
  enum { Basic = 0, Ruby = 1, Python = 2, MaxClientIndex = 3 };
};

/**
 *  @brief A base class for objects that are connected to their script
 * counterpart
 */
...
public:
   ...
   *  @brief Constructor
   */
  ObjectBase() : mp_status_changed_event(0) {}

  /**
   *  @brief Copy constructor
   */
  ObjectBase(const ObjectBase &) : mp_status_changed_event(0) {}
   ...
   *  This function is supposed to capture the case where an object was kept in
   * the constructor. The keep status cannot be propagated then and doing so
   * needs to be delayed until a client attaches to it. This flag is reset when
   * "status_changed_event ()" is called.
   */
  bool already_kept() const { return size_t(mp_status_changed_event) == 1; }
   ...
   *  After calling this method, the script clients no longer will own the
   * object. If the script object is deleted, the C++ object is not affected.
   */
  void keep() {
    if (has_status_changed_event()) {
      status_changed_event()(ObjectKeep);
    } else {
      mp_status_changed_event =
          reinterpret_cast<tl::event<StatusEventType> *>(1);
    }
  }

  /**
   *  @brief Marks this object as no longer owned by the C++ side
   *  After calling this method, the script clients (more specifically: the
   * first one) will own the object. If the script object is deleted, the C++
   ...
   */
  template <class T> T *find_client() {
    if (has_status_changed_event()) {
 ...
 */
class GSI_PUBLIC Proxy : public tl::Object {
public:
