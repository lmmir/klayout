...
ClassBase::ClassBase(const std::string &doc, const Methods &mm,
                     bool do_register)
    : m_initialized(false), mp_base(0), mp_parent(0), m_doc(doc),
      m_methods(mm) {
  if (do_register) {
...
std::string ClassBase::qname() const {
  std::string qn = name();
...
/**
 *  @brief This function returns true if the given method m of target can
 * convert an object of type from into target.
 */
static bool is_constructor_of(const ClassBase *target, const MethodBase *m,
                              const ClassBase *from) {
...
bool ClassBase::can_convert_to(const ClassBase *target) const {
  for (method_iterator m = target->begin_constructors();
       m != target->end_constructors(); ++m) {
...
void *ClassBase::create_obj_from(const ClassBase *from, void *obj) const {
  ...
  for (method_iterator m = begin_constructors(); m != end_constructors(); ++m) {
    if (is_constructor_of(this, *m, from)) {
      if (ctor) {
        throw tl::Exception(
            tl::to_string(
                tr("There are multiple conversion constructors available to "
                   "convert object of type %s to type %s")),
            from->name(), name());
      }
      ctor = *m;
    }
...
const ClassBase::class_collection &ClassBase::collection() {
...
const ClassBase::class_collection &ClassBase::new_collection() {
  if (!mp_new_class_collection) {
    static const class_collection empty;
    return empty;
  } else {
...
static SpecialMethod *sm_default_ctor(const char *name,
                                      const gsi::ClassBase *cls) {
  SpecialMethod *sm = new SpecialMethod(
      name, tl::to_string(tr("@brief Creates a new object of this class\n")),
      false, //  non-const
      true,  //  static
...
static SpecialMethod *sm_destroy(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(
          tr("@brief Explicitly destroys the object\nExplicitly destroys the "
             "object on C++ side if it was owned by the script interpreter. "
             "Subsequent access to this object will throw an exception.\n"
             "If the object is not owned by the script, this method will do "
             "nothing.")),
      false, //  non-const
      false, //  non-static
      MethodBase::Destroy);
...
static SpecialMethod *sm_create(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(tr("@brief Ensures the C++ object is created\n"
                       "Use this method to ensure the C++ object is created, "
                       "for example to ensure that resources are allocated. "
                       "Usually C++ objects are created on demand and not "
                       "necessarily when the script object is created.")),
      false, //  non-const
      false, //  non-static
      MethodBase::Create);
...
static SpecialMethod *sm_keep(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(tr(
          "@brief Marks the object as no longer owned by the script side.\n"
          "Calling this method will make this object no longer owned by the "
          "script's memory management. "
          "Instead, the object must be managed in some other way. Usually this "
          "method may be called if it is known that some C++ object holds and "
          "manages this object. "
          "Technically speaking, this method will turn the script's reference "
          "into a weak reference. "
          "After the script engine decides to delete the reference, the object "
          "itself will still exist. "
          "If the object is not managed otherwise, memory leaks will occur.\n\n"
          "Usually it's not required to call this method. It has been "
...
static SpecialMethod *sm_release(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(
          tr("@brief Marks the object as managed by the script side.\n"
             "After calling this method on an object, the script side will be "
             "responsible for the management of the object. "
             "This method may be called if an object is returned from a C++ "
             "function and the object is known not to be owned by any C++ "
             "instance. "
             "If necessary, the script side may delete the object if the "
...
static SpecialMethod *sm_is_const(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(tr(
          "@brief Returns a value indicating whether the reference is a const "
...
static SpecialMethod *sm_destroyed(const char *name) {
  SpecialMethod *sm = new SpecialMethod(
      name,
      tl::to_string(
          tr("@brief Returns a value indicating whether the object was already "
             "destroyed\nThis method returns true, if the object was "
             "destroyed, either explicitly or by the C++ side.\n"
             "The latter may happen, if the object is owned by a C++ object "
             "which got destroyed itself.")),
      true,  //  const
      false, //  non-static
...
static SpecialMethod *sm_dup(const char *name, const gsi::ClassBase *cls) {
  SpecialMethod *sm = new SpecialMethod(
      name, tl::to_string(tr("@brief Creates a copy of self\n")),
      true,  //  const
      false, //  non-static
      MethodBase::Dup);
...
static SpecialMethod *sm_assign(const char *name, const gsi::ClassBase *cls) {
  SpecialMethod *sm = new SpecialMethod(
      name, tl::to_string(tr("@brief Assigns another object to self")),
      false, //  non-const
      false, //  non-static
      MethodBase::Assign);
...
static const std::set<std::pair<std::string, bool>> &name_map_for_class(
    const gsi::ClassBase *cls,
    std::map<const gsi::ClassBase *, std::set<std::pair<std::string, bool>>>
        &cache) {
  if (!cls) {
  ...
  std::map<const gsi::ClassBase *,
           std::set<std::pair<std::string, bool>>>::iterator cc =
      cache.find(cls);
  if (cc != cache.end()) {
    return cc->second;
  }
  ...
  cc = cache
           .insert(std::make_pair((const gsi::ClassBase *)0,
                                  std::set<std::pair<std::string, bool>>()))
           .first;
  cc->second = name_map_for_class(cls->base(), cache);

  for (gsi::ClassBase::method_iterator m = cls->begin_methods();
       m != cls->end_methods(); ++m) {
...
static std::string signature(const gsi::MethodBase *m,
                             const gsi::MethodBase::MethodSynonym &s) {
  std::string res;
...
void ClassBase::merge_declarations() {
  ...
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
    ...
    if (c->declaration() == &*c &&
        !names.insert(c->declaration()->name()).second) {
      tl::warn << "Duplicate GSI declaration of name "
               << c->declaration()->name();
    }
  }
  ...
  for (std::vector<const gsi::ClassBase *>::const_iterator ed =
           to_remove.begin();
       ed != to_remove.end(); ++ed) {
    //  TODO: ugly const_cast hack
  ...
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_new_classes();
    ...
    const std::set<std::pair<std::string, bool>> &name_map =
        name_map_for_class(c.operator->(), name_maps);

    //  We don't want the declaration object to be non-const except for this
    //  case. So we const_cast here.
    gsi::ClassBase *non_const_decl =
        const_cast<gsi::ClassBase *>(c.operator->());
    ...
    if (name_map.find(std::make_pair("is_const_object", false)) ==
        name_map.end()) {
      //  deprecate "is_const"
      non_const_decl->add_method(
          sm_is_const("_is_const_object?|#is_const_object?"));
    } else {
  ...
  //  do a full re-initialization - maybe merge_declarations modified existing
  //  classes too
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
       c != gsi::ClassBase::end_classes(); ++c) {
    //  Initialize the method table once again after we have merged the
    //  declarations
  ...
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
    ...
    for (gsi::ClassBase::method_iterator m = c->begin_methods();
         m != c->end_methods(); ++m) {
      if (!(*m)->is_callback()) {
        for (gsi::MethodBase::synonym_iterator s = (*m)->begin_synonyms();
             s != (*m)->end_synonyms(); ++s) {
    ...
    for (std::map<std::string, int>::const_iterator mc = method_counts.begin();
         mc != method_counts.end(); ++mc) {
      if (mc->second > 1) {
        tl::warn << "Ambiguous method declarations in class " << c->name()
                 << " for method " << mc->first;
      }
    }
  }
...
static void
collect_classes(const gsi::ClassBase *cls,
...
static bool all_parts_available(const gsi::ClassBase *cls,
                                const std::set<const gsi::ClassBase *> &taken) {
  if (cls->declaration() && cls->declaration() != cls &&
      taken.find(cls->declaration()) == taken.end()) {
    return false;
  }
...
ClassBase::classes_in_definition_order(const char *mod_name) {
  ...
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
    ...
    if (!mod_name || c->module() == mod_name) {
      ...
      //  (children or base classes from outside the module may be part of the
      //  returned list!)
      collect_classes(c.operator->(), unsorted_classes);
    } else {
      //  we assume that these classes are taken by another run (i.e. "import x"
      //  in Python)
  ...
  while (!unsorted_classes.empty()) {
    ...
    for (std::list<const gsi::ClassBase *>::const_iterator c =
             unsorted_classes.begin();
         c != unsorted_classes.end(); ++c) {
      ...
      if (!all_parts_available(*c, taken)) {
        //  can't produce this class yet - it's a reference to another class
      ...
      if ((*c)->parent() != 0 && taken.find((*c)->parent()) == taken.end()) {
        //  can't produce this class yet - it's a child of a parent that is not
        //  produced yet.
        more_classes.push_back(*c);
        continue;
      }
      ...
      if ((*c)->base() != 0 && taken.find((*c)->base()) == taken.end()) {
        //  can't produce this class yet. The base class needs to be handled
        //  first.
        more_classes.push_back(*c);
        continue;
      }
    ...
    if (!any && !more_classes.empty()) {
      ...
      for (std::list<const gsi::ClassBase *>::const_iterator c =
               more_classes.begin();
           c != more_classes.end(); ++c) {

        //  don't handle classes twice
        if (taken.find(*c) != taken.end()) {
          //  not considered.
        } else if ((*c)->declaration() && (*c)->declaration() != *c &&
                   taken.find((*c)->declaration()) == taken.end()) {
          //  can't produce this class yet - it refers to a class which is not
          //  available.
          tl::error << tl::sprintf("class %s.%s refers to another class "
                                   "(%s.%s) which is not available",
                                   (*c)->module(), (*c)->name(),
                                   (*c)->declaration()->module(),
                                   (*c)->declaration()->name());
        } else if ((*c)->parent() != 0 &&
                   taken.find((*c)->parent()) == taken.end()) {
          //  can't produce this class yet - it's a child of a parent that is
          //  not produced yet.
          tl::error << tl::sprintf(
              "parent of class %s.%s not available (%s.%s)", (*c)->module(),
              (*c)->name(), (*c)->parent()->module(), (*c)->parent()->name());
        } else if ((*c)->base() != 0 &&
                   taken.find((*c)->base()) == taken.end()) {
          //  can't produce this class yet. The base class needs to be handled
          //  first.
          tl::error << tl::sprintf(
              "base of class %s.%s not available (%s.%s)", (*c)->module(),
              (*c)->name(), (*c)->base()->module(), (*c)->base()->name());
        }
      }

      //  prevent infinite recursion
      throw tl::Exception("Internal error: infinite recursion on class "
                          "building. See error log for analysis");
    }
...
const ClassBase *class_by_name_no_assert(const std::string &name) {
  if (s_name_to_class.empty()) {
    ...
    for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
      ...
      if (!s_name_to_class.insert(std::make_pair(c->name(), c.operator->()))
               .second) {
  ...
  std::map<std::string, const ClassBase *>::const_iterator c =
      s_name_to_class.find(name);
...
static void add_class_to_map(const gsi::ClassBase *c) {
  ...
  auto c2i =
      sp_class_to_index->insert(std::make_pair(c, sp_classes->size())).first;
  ...
  if (!sp_ti_to_class_index->insert(std::make_pair(ti, c2i->second)).second) {
    //  Duplicate registration of this class
    tl::error << "Duplicate registration of class " << c->name() << " (type "
              << ti->name() << ")";
    tl_assert(false);
  } else {
...
const ClassBase *class_by_typeinfo_no_assert(const std::type_info &ti) {
  if (!sp_ti_to_class_index || sp_ti_to_class_index->empty()) {
    for (auto c = gsi::ClassBase::begin_classes();
         c != gsi::ClassBase::end_classes(); ++c) {
      add_class_to_map(c.operator->());
    }
    for (auto c = gsi::ClassBase::begin_new_classes();
         c != gsi::ClassBase::end_new_classes(); ++c) {
      add_class_to_map(c.operator->());
    }
  }
  ...
  if (!sp_ti_to_class_index) {
    ...
    if (c != sp_ti_to_class_index->end()) {
      ...
      if (cn != sp_tname_to_class->end()) {
        ...
        sp_ti_to_class_index->insert(
            std::make_pair(&ti, sp_class_to_index->operator[](cn->second)));
        return cn->second;
      } else {
        return 0;
      }
