...
template <class T> struct _var_user_equal_impl<T, true> {
  static bool call(const T *a, const T *b,
                   const VariantUserClassImpl * /*delegate*/) {
    return *a == *b;
  }
};

template <class T> struct _var_user_equal_impl<T, false> {
  static bool call(const T *a, const T *b,
                   const VariantUserClassImpl *delegate) {
...
template <class T> struct _var_user_less_impl<T, true> {
  static bool call(const T *a, const T *b,
                   const VariantUserClassImpl * /*delegate*/) {
    return *a < *b;
  }
...
template <class T> struct _var_user_less_impl<T, false> {
  static bool call(const T *a, const T *b,
                   const VariantUserClassImpl *delegate) {
...
template <class T> struct _var_user_to_string_impl<T, true> {
  static std::string call(const T *a,
                          const VariantUserClassImpl * /*delegate*/) {
    return a->to_string();
  }
};

template <class T> struct _var_user_to_string_impl<T, false> {
...
template <class T> struct _var_user_to_variant_impl<T, true> {
  static tl::Variant call(const T *a,
                          const VariantUserClassImpl * /*delegate*/) {
    return a->to_variant();
  }
};

template <class T> struct _var_user_to_variant_impl<T, false> {
  static tl::Variant call(const T *a, const VariantUserClassImpl *delegate) {
    return delegate->to_variant_impl((void *)a);
  }
};
...
template <class T> struct _var_user_to_int_impl<T, true> {
  static int call(const T *a, const VariantUserClassImpl * /*delegate*/) {
    return a->to_int();
  }
};

template <class T> struct _var_user_to_int_impl<T, false> {
  static int call(const T *a, const VariantUserClassImpl *delegate) {
    return delegate->to_int_impl((void *)a);
  }
};
...
template <class T> struct _var_user_to_double_impl<T, true> {
  static double call(const T *a, const VariantUserClassImpl * /*delegate*/) {
    return a->to_double();
  }
};

template <class T> struct _var_user_to_double_impl<T, false> {
  static double call(const T *a, const VariantUserClassImpl *delegate) {
    return delegate->to_double_impl((void *)a);
  }
};

/**
 *  @brief A VariantUserClassBase specialization that links GSI classes and
 * Variant classes
 */
template <class T>
class GSI_PUBLIC_TEMPLATE VariantUserClass : public tl::VariantUserClass<T>,
                                             private VariantUserClassImpl {
public:
  VariantUserClass()
      : VariantUserClassImpl(), mp_cls(0), mp_object_cls(0), m_is_const(false) {
    //  .. nothing yet ..
  }
  ...
  const tl::EvalClass *eval_cls() const { return this; }
  ...
  virtual bool equal(const void *a, const void *b) const {
    return gsi::_var_user_equal_impl<T, tl::has_equal_operator<T>::value>::call(
        (const T *)a, (const T *)b, this);
  }

  virtual bool less(const void *a, const void *b) const {
    return gsi::_var_user_less_impl<T, tl::has_less_operator<T>::value>::call(
        (const T *)a, (const T *)b, this);
  }

  virtual void to_variant(const void *a, tl::Variant &var) const {
    var = gsi::_var_user_to_variant_impl<T, tl::has_to_variant<T>::value>::call(
        (const T *)a, this);
  }

  virtual std::string to_string(const void *a) const {
    return gsi::_var_user_to_string_impl<T, tl::has_to_string<T>::value>::call(
        (const T *)a, this);
  }

  virtual int to_int(const void *a) const {
    return gsi::_var_user_to_int_impl<T, tl::has_to_int<T>::value>::call(
        (const T *)a, this);
  }

  virtual double to_double(const void *a) const {
    return gsi::_var_user_to_double_impl<T, tl::has_to_double<T>::value>::call(
        (const T *)a, this);
  }
  ...
  void *create() const { return mp_cls->create(); }
  ...
  const char *name() const { return mp_cls ? mp_cls->name().c_str() : 0; }
  ...
  const gsi::ClassBase *gsi_cls() const { return mp_cls; }

  bool is_class() const { return mp_object_cls != 0; }
...
template <class X> struct _assign<X, false> {
  static void call(void *, const void *) {
    throw tl::Exception(tl::to_string(tr("Object cannot be copied here")));
  }
};
template <class X> struct _assign<X, true> {
  static void call(void *dest, const void *src) {
    *(X *)dest = *(const X *)src;
  }
};
 ...
 */
template <class X, class B, bool B_IS_POLYMORPHIC> class SubClassTester;

/**
 *  @brief Specialization for polymorphic types - we can use dynamic_cast to
 * tests whether B object can be cast to X
 */
template <class X, class B>
class SubClassTester<X, B, true> : public SubClassTesterBase {
public:
 ...
 */
template <class X, class B>
class SubClassTester<X, B, false> : public SubClassTesterBase {
public:
  virtual bool can_upcast(const void *) const {
    //  Non-polymorphic classes can't be upcast, hence we always return false
    //  here
    return false;
  }
};
 ...
 *  the given methods.
 */
template <class X> class GSI_PUBLIC_TEMPLATE ClassExt : public ClassBase {
public:
   ...
   *  This constructor will pull in another class into this one as a kind of
   * link using a different name. This feature is not quite useful usually and
   * is reserved for special use cases such as including enums into a
   * declaration namespace.
   */
  ClassExt(const ClassBase &import, const std::string &name = std::string(),
  ...
  virtual const std::type_info &type() const { return typeid(X); }
   ...
   */
  virtual const ClassBase *declaration() const { return mp_declaration; }
  ...
  virtual bool consolidate() const {
    ...
    for (gsi::ClassBase::method_iterator m = begin_methods();
         m != end_methods(); ++m) {
      non_const_decl->add_method((*m)->clone());
    }

    //  Treat class imports (extensions with a base class): import the class as
    //  a child class plus import constants into the class (intended for enum
    //  import).
    if (declaration()) {
 ...
 */
template <class X, class Adapted> struct adaptor_type_info {
  static const std::type_info *type_info() { return &typeid(Adapted); }

  static X *create(const Adapted *a) { return new X(*a); }
  ...
  static const Adapted *get(const X *x) { return &x->value(); }

  static Adapted *create_adapted(const X *x) { return new Adapted(x->value()); }
};

template <class X> struct adaptor_type_info<X, NoAdaptorTag> {
  static const std::type_info *type_info() { return 0; }
 ...
 *
 *  This class declares all methods that are required to instantiate or copy an
 * object or to call it's methods in some generic way.
 */
template <class X, class Adapted = NoAdaptorTag>
class GSI_PUBLIC_TEMPLATE Class : public ClassBase {
public:
  Class(const std::string &module, const std::string &name, const Methods &mm,
        const std::string &doc = std::string(), bool do_register = true)
  ...
  template <class B>
  Class(const Class<B> &base, const std::string &module,
        const std::string &name, const Methods &mm,
        const std::string &doc = std::string(), bool do_register = true)
      : ClassBase(doc, mm, do_register),
        m_subclass_tester(
            new SubClassTester<X, B, std::is_polymorphic<B>::value>()) {
    set_module(module);
    set_name(name);
    set_base(&base);
  }

  Class(const std::string &module, const std::string &name,
        const std::string &doc = std::string(), bool do_register = true)
  ...
  template <class B>
  Class(const Class<B> &base, const std::string &module,
        const std::string &name, const std::string &doc = std::string(),
  ...
  virtual const ClassBase *declaration() const { return this; }

  virtual bool consolidate() const { return true; }
  ...
  bool is_managed() const { return tl::is_derived<gsi::ObjectBase, X>(); }
  ...
  virtual bool can_destroy() const { return std::is_destructible<X>::value; }

  virtual bool can_copy() const { return std::is_copy_constructible<X>::value; }
  ...
  virtual const ClassBase *subclass_decl(const void *p) const {
    if (p) {
      for (tl::weak_collection<ClassBase>::const_iterator s =
               subclasses().begin();
           s != subclasses().end(); ++s) {
        if (s->can_upcast(p)) {
  ...
  virtual bool binds() const { return true; }
 ...
 *
 *  This is an alternative to using the Class constructor with a base class
 * reference. It employs a template parameter rather than a gsi::Class object.
 * This way it's easier to use if the declaration object of the base class is
 * not available.
 */
template <class X, class B, class Adapted = NoAdaptorTag>
class SubClass : public Class<X, Adapted> {
public:
  SubClass(const std::string &module, const std::string &name,
           const Methods &mm, const std::string &doc = std::string())
  ...
  virtual bool consolidate() const {
    ...
    SubClass<X, B, Adapted> *non_const_this =
        const_cast<SubClass<X, B, Adapted> *>(this);
 ...
 *
 *  This declaration is similar to the gsi::Class declaration but makes the
 * given class a subclass of the parent.
 */
template <class P, class X, class Adapted = NoAdaptorTag>
class GSI_PUBLIC_TEMPLATE ChildClass : public Class<X, Adapted> {
public:
  ChildClass(const std::string &module, const std::string &name,
             const Methods &mm, const std::string &doc = std::string())
      : Class<X, Adapted>(module, name, mm, doc) {
    //  .. nothing yet ..
  }
...
class GSI_PUBLIC_TEMPLATE ChildSubClass : public SubClass<X, B, Adapted> {
public:
  ChildSubClass(const std::string &module, const std::string &name,
                const Methods &mm, const std::string &doc = std::string())
...
template <class X> const ClassBase *cls_decl() {
  //  TODO: needs thread safety? It's rather unlikely that two threads enter
  //  this piece of code at the same time and they interfere when storing the
  //  results.
