...
extern "C" int WINAPI WinMain(HINSTANCE /*hInstance*/,
                              HINSTANCE /*prevInstance*/, LPSTR /*lpCmdLine*/,
                              int /*nShowCmd*/) {
  int argCount = 0;
  ...
  //  NOTE: this needs to happen after the Ruby interpreter went down since
  //  otherwise the GC will access objects that are already cleaned up.
...
static bool run_test(tl::TestBase *t, bool editable, bool slow, int repeat) {
#if defined(HAVE_QT)
...
static int run_tests(const std::vector<tl::TestBase *> &selected_tests,
                     bool editable, bool non_editable, bool slow, int repeat,
                     bool gsi_coverage,
                     const std::vector<std::string> &class_names_vector) {
  ...
  for (int e = 0; e < 2; ++e) {
    ...
    if ((non_editable && e == 0) || (editable && e == 1)) {
      ...
      ut::noctrl << tl::replicate("=",
                                  ut::TestConsole::instance()->real_columns());
      ...
      try {
        ...
        for (std::vector<tl::TestBase *>::const_iterator t =
                 selected_tests.begin();
             t != selected_tests.end(); ++t) {
          (*t)->remove_tmp_folder();
        }
        ...
        for (std::vector<tl::TestBase *>::const_iterator t =
          ...
          try {
            ...
            if (!run_test(*t, e != 0, slow, repeat)) {
              ...
              ut::ctrl << "<error message=\""
                       << "Test " << tl::escaped_to_html((*t)->name(), false)
                       << " failed (continued mode - see previous messages)"
                       << "\"/>";
              tl::error << "Test " << (*t)->name()
                        << " failed (continued mode - see previous messages)";
            ...
            ut::noctrl << "Time: " << timer.sec_wall() << "s (wall) "
                       << timer.sec_user() << "s (user) " << timer.sec_sys()
                       << "s (sys)";
            ut::noctrl << "Memory: " << timer.memory_size() / 1024 << "k";
            ut::ctrl << "<x-testcase-times wall=\"" << timer.sec_wall()
                     << "\" user=\"" << timer.sec_user() << "\" sys=\""
                     << timer.sec_sys() << "\" memory=\"" << timer.memory_size()
                     << "\"/>";
            ...
            ut::ctrl << "</system-out>";
            ut::ctrl << "<failure message=\""
                     << tl::escaped_to_html(ex.msg(), false) << "\"/>";
      ...
      ut::noctrl << "Total time: " << timer.sec_wall() << "s (wall) "
                 << timer.sec_user() << "s (user) " << timer.sec_sys()
                 << "s (sys)";
      ut::ctrl << "<x-summary-times mode=\"" << mode << "\" wall=\""
               << timer.sec_wall() << "\" user=\"" << timer.sec_user()
               << "\" sys=\"" << timer.sec_sys() << "\"/>";
  ...
  if (gsi_coverage) {
    ...
    for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
      ...
      if (gsi_coverage && !class_names.empty() &&
          class_names.find(c->name()) == class_names.end()) {
        continue;
      }
      ...
      for (gsi::ClassBase::method_iterator m = c->begin_methods();
        ...
        if (!dynamic_cast<const gsi::SpecialMethod *>(*m) &&
            !(*m)->was_called()) {

          if (first) {
            first = false;
            tl::warn << "GSI coverage test failed - the following methods were "
                        "not called:";
          }
          if (first_of_class) {
            tl::warn << tl::replicate(" ", tl::indent()) << "Class "
                     << c->name();
  ...
  if (skipped_e + skipped_ne > 0) {
    if (non_editable) {
      ...
      for (std::vector<tl::TestBase *>::const_iterator f =
               skipped_tests_ne.begin();
           f != skipped_tests_ne.end(); ++f) {
    ...
    if (editable) {
      ...
      for (std::vector<tl::TestBase *>::const_iterator f =
               skipped_tests_e.begin();
           f != skipped_tests_e.end(); ++f) {
        tl::warn << tl::replicate(" ", tl::indent()) << (*f)->name();
      }
    }
  ...
  if (result > 0) {
    if (non_editable) {
      ...
      for (std::vector<tl::TestBase *>::const_iterator f =
               failed_tests_ne.begin();
           f != failed_tests_ne.end(); ++f) {
        tl::warn << tl::replicate(" ", tl::indent()) << (*f)->name();
      }
    }
    if (editable) {
      ...
      for (std::vector<tl::TestBase *>::const_iterator f =
               failed_tests_e.begin();
           f != failed_tests_e.end(); ++f) {
        tl::warn << tl::replicate(" ", tl::indent()) << (*f)->name();
      }
    }
  ...
  ut::noctrl << "Grand total time: " << grand_timer.sec_wall() << "s (wall) "
             << grand_timer.sec_user() << "s (user) " << grand_timer.sec_sys()
             << "s (sys)";
  ut::ctrl << "<x-grand-summary-times wall=\"" << grand_timer.sec_wall()
           << "\" user=\"" << grand_timer.sec_user() << "\" sys=\""
           << grand_timer.sec_sys() << "\"/>";
...
static int main_cont(int &argc, char **argv) {
  ...
  try {
    ...
    std::vector<std::string> inst_modules =
        tl::dir_entries(inst_dir, true, false);
    std::sort(inst_modules.begin(), inst_modules.end());

    for (std::vector<std::string>::const_iterator im = inst_modules.begin();
      ...
      std::string ut_file =
          tl::absolute_file_path(tl::combine_path(inst_dir, *im));
      if (tl::file_exists(ut_file)) {

        tl::log << "Loading unit tests " << ut_file;

        //  NOTE: since we are using a different suffix ("*.ut"), we can't use
        //  QLibrary.
#ifdef _WIN32
        ...
        if (!handle) {
          throw tl::Exception(tl::sprintf(
              "Unable to load plugin tests: %s with error message: %s",
              ut_file.c_str(), GetLastError()));
        }
#else
        void *handle;
        ...
        if (!handle) {
          throw tl::Exception(
              tl::sprintf("Unable to load plugin tests: %s", ut_file.c_str()));
        }
#endif
      }
    }
    ...
    if (!tl::TestRegistrar::instance()) {
      throw tl::Exception("No test libraries found - make sure, the *.ut files "
                          "are next to the ut_runner executable.");
    }

#if defined(HAVE_QT)

    //  NOTE: we need an application object, but we don't call parse_cmd. This
    //  makes the object behave neutral as far as possible.
    ...
    lym_root.add_folder(tl::to_string(tr("Built-In")), ":/built-in-macros",
                        "macros", true);
    lym_root.add_folder(tl::to_string(tr("Built-In")), ":/built-in-pymacros",
                        "pymacros", true);
    ...
    cmd << tl::arg("-a", &xml_format,
                   "Provide XML output format (JUnit format)")
        ...
        << tl::arg("-i", &debug_mode, "Uses debug mode",
                   "In debug mode, execution stops after an error and if "
                   "possible, fix instructions are "
                   "printed.")
        << tl::arg("-s", &slow, "Includes slow (long runner) tests")
        << tl::arg("-v", &verbose, "Provides verbose output")
        << tl::arg("-g", &gsi_coverage,
                   "Produces a GSI test coverage statistics")
        << tl::arg("-r=n", &repeat, "Repeat the tests n times each")
        << tl::arg(
               "*-gg=class", &class_names,
               "Produces a specific GDS coverage statistics",
               "With this specification, coverage will be printed for this "
               "specific class. "
               "This option can be used multiple times to add more classes.")
        << tl::arg("-x=test", &exclude_test_list, "Exclude the following tests",
                   "This option can be given multiple times or with a "
                   "comma-separated list "
                   "of pattern. Test tests matching one of the exclude pattern "
                   "are not executed.")
        << tl::arg("?*test", &test_list,
                   "The pattern for the tests to execute");
    ...
    if (list_tests) {
      ...
      for (std::vector<tl::TestBase *>::const_iterator i =
               tl::TestRegistrar::instance()->tests().begin();
           i != tl::TestRegistrar::instance()->tests().end(); ++i) {
        tl::info << "  " << (*i)->name();
      }
    ...
    if (rba::RubyInterpreter::instance()) {
      rba::RubyInterpreter::instance()->define_variable("ut_inst_path",
                                                        tl::get_inst_path());
    }
    if (pya::PythonInterpreter::instance()) {
      pya::PythonInterpreter::instance()->define_variable("ut_inst_path",
                                                          tl::get_inst_path());
    }
    ...
    try {
      ...
      if (!output.empty()) {
        ...
        if (!output_file) {
          throw tl::Exception(
              std::string("Unable to open log file for writing :") + output);
        }
      ...
      for (std::vector<tl::TestBase *>::const_iterator i =
        ...
        for (std::vector<std::string>::const_iterator m =
                 exclude_test_list.begin();
             m != exclude_test_list.end() && !exclude; ++m) {
          tl::GlobPattern re(*m);
        ...
        if (test_list.empty()) {
          ...
          for (std::vector<std::string>::const_iterator m = test_list.begin();
               !exclude && m != test_list.end(); ++m) {
            tl::GlobPattern re(*m);
            re.set_case_sensitive(false);
            re.set_header_match(true);
            if (re.match((*i)->name())) {
      ...
      result = run_tests(subset, editable, non_editable, slow, repeat,
                         gsi_coverage, class_names);
