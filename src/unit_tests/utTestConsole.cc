...
public:
  InfoChannel(int verbosity) : m_verbosity(verbosity) {
    //  .. nothing yet ..
  }
...
protected:
  ...
  virtual void end() { TestConsole::instance()->flush(); }
...
protected:
  virtual void puts(const char *s) { TestConsole::instance()->basic_write(s); }

  virtual void endl() { TestConsole::instance()->basic_write("\n"); }
  ...
  virtual void begin() { TestConsole::instance()->begin_warn(); }
...
protected:
  ...
  virtual void begin() { TestConsole::instance()->begin_error(); }
...
public:
  CtrlChannel(bool with_xml) : m_with_xml(with_xml) {
    //  .. nothing yet ..
  }
...
TestConsole::TestConsole(FILE *file)
    : m_file(file), m_col(0), m_max_col(400), m_columns(50), m_rows(0),
      m_file_is_tty(false) {
  ms_instance = this;
...
void TestConsole::raw_write(const char *text) { fputs(text, m_file); }

void TestConsole::flush() { fflush(m_file); }
...
void TestConsole::basic_write(const char *s) {
  if (tl::xml_format()) {
    ...
    //  line length limitation - this assumes we are always printing to the same
    //  terminal or we don't mix stderr/stdout.
    const char *cp;
    for (cp = s; *cp; ++cp) {
      if (*cp == '\n' || *cp == '\r') {
        m_col = 0;
