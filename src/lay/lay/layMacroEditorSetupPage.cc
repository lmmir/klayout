...
struct MacroEditorSetupDialogData : public QObject {
  MacroEditorSetupDialogData(QObject *parent)
      : QObject(parent), basic_attributes(0), tab_width(8), indent(2),
        save_all_on_run(true), stop_on_exception(true),
        file_watcher_enabled(true), font_size(0) {}
  ...
  void setup(lay::Dispatcher *root) {
    ...
    for (lay::MacroEditorHighlighters::const_iterator a = highlighters.begin();
         a != highlighters.end(); ++a) {
      specific_attributes.push_back(std::make_pair(
          a->first, GenericSyntaxHighlighterAttributes(&basic_attributes)));
      specific_attributes.back().second.assign(a->second);
    }

    root->config_get(cfg_macro_editor_save_all_on_run, save_all_on_run);
    root->config_get(cfg_macro_editor_file_watcher_enabled,
  ...
  void commit(lay::Dispatcher *root) {
    ...
    for (MacroEditorHighlighters::iterator a = highlighters.begin();
         a != highlighters.end(); ++a) {
      for (std::vector<std::pair<
               std::string, GenericSyntaxHighlighterAttributes>>::const_iterator
               i = specific_attributes.begin();
           i != specific_attributes.end(); ++i) {
        if (i->first == a->first) {
    ...
    root->config_set(cfg_macro_editor_file_watcher_enabled,
                     file_watcher_enabled);
    ...
    for (std::set<std::string>::const_iterator i =
             ignore_exceptions_list.begin();
         i != ignore_exceptions_list.end(); ++i) {
      if (!il.empty()) {
        il += ";";
      }
...
static void update_item(QListWidgetItem *item, QTextCharFormat format) {
...
MacroEditorSetupPage::MacroEditorSetupPage(QWidget *parent)
    : lay::ConfigPage(parent), mp_data(new MacroEditorSetupDialogData(this)) {
  setupUi(this);

  connect(
      styles_list,
      SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)), this,
      SLOT(current_attribute_changed(QListWidgetItem *, QListWidgetItem *)));
  ...
  connect(text_color_button, SIGNAL(color_changed(QColor)), this,
          SLOT(color_changed(QColor)));
  connect(background_color_button, SIGNAL(color_changed(QColor)), this,
          SLOT(color_changed(QColor)));
  connect(font_sel, SIGNAL(currentFontChanged(const QFont &)), this,
          SLOT(update_font()));
...
void MacroEditorSetupPage::color_changed(QColor) {
  commit_attributes(styles_list->currentItem());
}

void MacroEditorSetupPage::cb_changed(int) {
  commit_attributes(styles_list->currentItem());
}

void MacroEditorSetupPage::clear_exception_list() {
  mp_data->ignore_exceptions_list.clear();
  update_ignore_exception_list();
}

void MacroEditorSetupPage::update_ignore_exception_list() {
  exception_list->clear();
  for (std::set<std::string>::const_iterator i =
           mp_data->ignore_exceptions_list.begin();
       i != mp_data->ignore_exceptions_list.end(); ++i) {
    exception_list->addItem(tl::to_qstring(*i));
  }
}

void MacroEditorSetupPage::update_font() {
  QFont f;
...
void MacroEditorSetupPage::setup(Dispatcher *root) {
  delete mp_data;
  ...
  for (GenericSyntaxHighlighterAttributes::const_iterator a =
           mp_data->basic_attributes.begin();
       a != mp_data->basic_attributes.end(); ++a) {
    QListWidgetItem *item = new QListWidgetItem(styles_list);
    QString n =
        tl::to_qstring(tl::to_string(QObject::tr("(basic)")) + " ") + a->first;
    item->setText(n);
  ...
  for (std::vector<std::pair<std::string, GenericSyntaxHighlighterAttributes>>::
           const_iterator sa = mp_data->specific_attributes.begin();
       sa != mp_data->specific_attributes.end(); ++sa, ++na) {
    ...
    for (GenericSyntaxHighlighterAttributes::const_iterator a =
      ...
      item->setData(Qt::UserRole + 1, a->second);
      std::map<int, QString>::const_iterator bn =
          basic_names.find(sa->second.basic_id(a->second));
...
void MacroEditorSetupPage::commit(Dispatcher *root) {
...
void MacroEditorSetupPage::current_attribute_changed(
...
void MacroEditorSetupPage::commit_attributes(QListWidgetItem *to_item) {
  if (!to_item) {
    return;
  }
  ...
  if (attributes) {
    ...
    if (underline_cb->checkState() == Qt::PartiallyChecked) {
      ...
      style.setProperty(QTextFormat::FontUnderline,
                        underline_cb->checkState() == Qt::Checked);
    }

    if (italic_cb->checkState() == Qt::PartiallyChecked) {
      style.clearProperty(QTextFormat::FontItalic);
    } else {
      style.setProperty(QTextFormat::FontItalic,
                        italic_cb->checkState() == Qt::Checked);
    }

    if (strikeout_cb->checkState() == Qt::PartiallyChecked) {
      style.clearProperty(QTextFormat::FontStrikeOut);
    } else {
      style.setProperty(QTextFormat::FontStrikeOut,
                        strikeout_cb->checkState() == Qt::Checked);
    }
    ...
    if (bold_cb->checkState() == Qt::PartiallyChecked) {
      ...
      style.setProperty(QTextFormat::FontWeight,
                        bold_cb->checkState() == Qt::Checked ? QFont::Bold
                                                             : QFont::Normal);
    }
    ...
    if (text_color_button->get_color() == QColor()) {
      ...
      style.setProperty(QTextFormat::ForegroundBrush,
                        QBrush(text_color_button->get_color()));
    }
    ...
    if (background_color_button->get_color() == QColor()) {
      ...
      style.setProperty(QTextFormat::BackgroundBrush,
                        QBrush(background_color_button->get_color()));
    }

    attributes->set_style(id, style);
  }

  //  update all list styles (because we may modify a basic entry, we have to
  //  update dependent ones as well)
...
void MacroEditorSetupPage::update_attributes(QListWidgetItem *from_item) {
  if (from_item) {
    ...
    if (attributes) {
      ...
      if (style.hasProperty(QTextFormat::FontUnderline)) {
        underline_cb->setCheckState(
            style.boolProperty(QTextFormat::FontUnderline) ? Qt::Checked
                                                           : Qt::Unchecked);
      } else {
      ...
      if (style.hasProperty(QTextFormat::FontStrikeOut)) {
        strikeout_cb->setCheckState(
            style.boolProperty(QTextFormat::FontStrikeOut) ? Qt::Checked
                                                           : Qt::Unchecked);
      } else {
      ...
      if (style.hasProperty(QTextFormat::FontItalic)) {
        italic_cb->setCheckState(style.boolProperty(QTextFormat::FontItalic)
                                     ? Qt::Checked
                                     : Qt::Unchecked);
      } else {
      ...
      if (style.hasProperty(QTextFormat::FontWeight)) {
        bold_cb->setCheckState(style.intProperty(QTextFormat::FontWeight) ==
                                       QFont::Bold
                                   ? Qt::Checked
                                   : Qt::Unchecked);
      } else {
      ...
      if (style.hasProperty(QTextFormat::ForegroundBrush)) {
        text_color_button->set_color(
            style.brushProperty(QTextFormat::ForegroundBrush).color());
      } else {
        text_color_button->set_color(QColor());
      }

      if (style.hasProperty(QTextFormat::BackgroundBrush)) {
        background_color_button->set_color(
            style.brushProperty(QTextFormat::BackgroundBrush).color());
      } else {
