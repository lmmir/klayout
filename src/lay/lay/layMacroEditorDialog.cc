...
const std::string
    cfg_macro_editor_save_all_on_run("macro-editor-save-all-on-run");
const std::string
    cfg_macro_editor_stop_on_exception("macro-editor-stop-on-exception");
const std::string
    cfg_macro_editor_file_watcher_enabled("macro-editor-file-watcher-enabled");
...
const std::string
    cfg_macro_editor_console_interpreter("macro-editor-console-interpreter");
...
const std::string
    cfg_macro_editor_watch_expressions("macro-editor-watch-expressions");
const std::string
    cfg_macro_editor_debugging_enabled("macro-editor-debugging-enabled");
const std::string cfg_macro_editor_ignore_exception_list(
    "macro-editor-ignore-exception-list");
...
class MacroTemplateSelectionDialog : public QDialog,
                                     private Ui::MacroTemplateSelectionDialog {
public:
  MacroTemplateSelectionDialog(QWidget *parent,
                               const std::vector<lym::Macro *> &templates,
    ...
    //  Build a tree from the templates. Groups are formed by prepending a group
    //  title in the description separated from the actual description by ";;"
    int index = 0;
    for (std::vector<lym::Macro *>::const_iterator t = templates.begin();
         t != templates.end(); ++t, ++index) {
      ...
      if ((cat.empty() || cat == "macros") && c.empty()) {
        ...
        //  others are checked whether the category name is part of the category
        //  list
        std::vector<std::string> cc = tl::split(c, ",");
        for (std::vector<std::string>::const_iterator ic = cc.begin();
             ic != cc.end() && !take; ++ic) {
          if (*ic == cat) {
            take = true;
          }
  ...
  int exec_dialog() {
    ...
    if (m_template_count <= 1) {
      ...
      if (templateView->currentItem() &&
          templateView->currentItem()->data(0, Qt::UserRole) != QVariant()) {
...
class EditRoleDelegate : public QItemDelegate {
public:
  EditRoleDelegate(QWidget *parent) : QItemDelegate(parent) {
    //  .. nothing yet ..
  }
...
MacroEditorDialog::MacroEditorDialog(lay::Dispatcher *pr,
                                     lym::MacroCollection *root)
    : QDialog(0 /*show as individual top widget*/, Qt::Window), lay::Plugin(
                                                                    pr, true),
      mp_plugin_root(pr), mp_root(root), m_first_show(true),
      m_debugging_on(true), mp_run_macro(0),
      md_update_console_text(this, &MacroEditorDialog::update_console_text),
      m_in_event_handler(false), m_os(OS_none), m_new_line(true),
      m_highlighters(this), m_in_exec(false), m_in_breakpoint(false),
      m_ignore_exec_events(false), mp_exec_controller(0),
      mp_current_interpreter(0), m_continue(false), m_trace_count(0),
      m_current_stack_depth(-1), m_stop_stack_depth(-1), m_eval_context(-1),
      m_process_events_interval(0.0), m_window_closed(true),
      m_needs_update(true), m_ntab(8), m_nindent(2), m_save_all_on_run(false),
      m_stop_on_exception(true), m_file_watcher_enabled(true), m_font_size(0),
      m_edit_trace_index(-1), m_add_edit_trace_enabled(true),
      dm_refresh_file_watcher(this,
                              &MacroEditorDialog::do_refresh_file_watcher),
      dm_update_ui_to_run_mode(this,
                               &MacroEditorDialog::do_update_ui_to_run_mode) {
  //  Makes this dialog receive events while progress bars are on - this way we
  //  can set breakpoints during execution of a macro even if anything lengthy
  //  is running.
  ...
  connect(mp_root, SIGNAL(macro_changed(lym::Macro *)), this,
          SLOT(macro_changed(lym::Macro *)));
  connect(mp_root, SIGNAL(macro_deleted(lym::Macro *)), this,
          SLOT(macro_deleted(lym::Macro *)));
  connect(mp_root, SIGNAL(macro_collection_deleted(lym::MacroCollection *)),
          this, SLOT(macro_collection_deleted(lym::MacroCollection *)));
  connect(mp_root, SIGNAL(macro_collection_changed(lym::MacroCollection *)),
          this, SLOT(macro_collection_changed(lym::MacroCollection *)));
  ...
  for (size_t i = 0; i < m_categories.size(); ++i) {
    ...
    lay::MacroEditorTree *macro_tree =
        new lay::MacroEditorTree(treeTab, m_categories[i].name);
    ...
    macro_tree->setObjectName(tl::to_qstring(m_categories[i].name) +
                              QString::fromUtf8("_tree"));
    ...
    //  TODO: that is supposed to enable the horizontal scroll bar, but it
    //  doesn't:
    ...
    connect(macro_tree, SIGNAL(macro_double_clicked(lym::Macro *)), this,
            SLOT(item_double_clicked(lym::Macro *)));
    connect(macro_tree,
            SIGNAL(move_macro(lym::Macro *, lym::MacroCollection *)), this,
            SLOT(move_macro(lym::Macro *, lym::MacroCollection *)));
    connect(macro_tree,
            SIGNAL(move_folder(lym::MacroCollection *, lym::MacroCollection *)),
            this,
            SLOT(move_folder(lym::MacroCollection *, lym::MacroCollection *)));
    connect(macro_tree, SIGNAL(folder_renamed(lym::MacroCollection *)), this,
            SLOT(folder_renamed(lym::MacroCollection *)));
    connect(macro_tree, SIGNAL(macro_renamed(lym::Macro *)), this,
            SLOT(macro_renamed(lym::Macro *)));
  }
  ...
  connect(actionUseRegularExpressions, SIGNAL(triggered()), this,
          SLOT(search_editing()));
  connect(actionCaseSensitive, SIGNAL(triggered()), this,
          SLOT(search_editing()));
  ...
  connect(input_field, SIGNAL(editTextChanged(const QString &)), this,
          SLOT(immediate_command_text_changed(const QString &)));
  ...
  connect(mp_tabs_menu, SIGNAL(aboutToShow()), this,
          SLOT(tabs_menu_about_to_show()));
  tabWidget->addAction(action);
  ...
  connect(actionRemoveLocation, SIGNAL(triggered()), this,
          SLOT(remove_location()));
  ...
  connect(actionAddMacro, SIGNAL(triggered()), this,
          SLOT(add_button_clicked()));
  connect(deleteButton, SIGNAL(clicked()), this, SLOT(delete_button_clicked()));
  connect(actionDelete, SIGNAL(triggered()), this,
          SLOT(delete_button_clicked()));
  connect(renameButton, SIGNAL(clicked()), this, SLOT(rename_button_clicked()));
  connect(actionRename, SIGNAL(triggered()), this,
          SLOT(rename_button_clicked()));
  connect(importButton, SIGNAL(clicked()), this, SLOT(import_button_clicked()));
  connect(actionImport, SIGNAL(triggered()), this,
          SLOT(import_button_clicked()));
  connect(newFolderButton, SIGNAL(clicked()), this,
          SLOT(new_folder_button_clicked()));
  connect(actionNewFolder, SIGNAL(triggered()), this,
          SLOT(new_folder_button_clicked()));
  connect(saveAllButton, SIGNAL(clicked()), this,
          SLOT(save_all_button_clicked()));
  connect(actionSaveAll, SIGNAL(triggered()), this,
          SLOT(save_all_button_clicked()));
  ...
  connect(runThisButton, SIGNAL(clicked()), this,
          SLOT(run_this_button_clicked()));
  ...
  connect(breakpointButton, SIGNAL(clicked()), this,
          SLOT(breakpoint_button_clicked()));
  connect(clearBreakpointsButton, SIGNAL(clicked()), this,
          SLOT(clear_breakpoints_button_clicked()));
  connect(propertiesButton, SIGNAL(clicked()), this,
          SLOT(properties_button_clicked()));
  connect(setupButton, SIGNAL(clicked()), this, SLOT(setup_button_clicked()));
  connect(tabWidget, SIGNAL(currentChanged(int)), this,
          SLOT(current_tab_changed(int)));
  connect(callStack, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
          SLOT(stack_element_double_clicked(QListWidgetItem *)));
  connect(singleStepButton, SIGNAL(clicked()), this,
          SLOT(single_step_button_clicked()));
  connect(nextStepButton, SIGNAL(clicked()), this,
          SLOT(next_step_button_clicked()));
  connect(searchEditBox, SIGNAL(textEdited(const QString &)), this,
          SLOT(search_editing()));
  connect(searchEditBox, SIGNAL(returnPressed()), this, SLOT(search_edited()));
  connect(searchEditBox, SIGNAL(editingFinished()), this,
          SLOT(search_edited()));
  connect(searchEditBox, SIGNAL(esc_pressed()), this, SLOT(search_finished()));
  connect(searchEditBox, SIGNAL(tab_pressed()), this,
          SLOT(find_next_button_clicked()));
  connect(searchEditBox, SIGNAL(backtab_pressed()), this,
          SLOT(find_prev_button_clicked()));
  connect(replaceText, SIGNAL(esc_pressed()), this, SLOT(search_finished()));
  connect(replaceText, SIGNAL(tab_pressed()), this,
          SLOT(find_next_button_clicked()));
  connect(replaceText, SIGNAL(backtab_pressed()), this,
          SLOT(find_prev_button_clicked()));
  connect(replaceText, SIGNAL(returnPressed()), this,
          SLOT(replace_next_button_clicked()));
  connect(replaceModeButton, SIGNAL(clicked()), this,
          SLOT(replace_mode_button_clicked()));
  connect(replaceNextButton, SIGNAL(clicked()), this,
          SLOT(replace_next_button_clicked()));
  connect(findNextButton, SIGNAL(clicked()), this,
          SLOT(find_next_button_clicked()));
  connect(findPrevButton, SIGNAL(clicked()), this,
          SLOT(find_prev_button_clicked()));
  connect(replaceAllButton, SIGNAL(clicked()), this,
          SLOT(replace_all_button_clicked()));
  connect(allVariables, SIGNAL(clicked(bool)), variableList,
          SLOT(set_show_all(bool)));
  ...
  std::vector<std::string> lines =
      tl::split(std::string(data.constData(), data.size()), "\n");
    ...
    if (!ll.empty() && ll[0] != '#') {
      ...
      if (ll[0] == '[') {
        ...
        try {
          ...
          if (tl::verbosity() >= 20) {
            tl::info << "Using macro template from " << url << " (with name "
                     << m->name() << ")";
          }
  ...
  for (std::vector<std::string>::const_iterator p =
           lay::ApplicationBase::instance()->klayout_path().begin();
       p != lay::ApplicationBase::instance()->klayout_path().end(); ++p) {

    QDir dir(
        QDir(tl::to_qstring(*p)).filePath(tl::to_qstring("macro-templates")));
    ...
    for (tl::Registrar<lym::MacroInterpreter>::iterator cls =
             tl::Registrar<lym::MacroInterpreter>::begin();
         cls != tl::Registrar<lym::MacroInterpreter>::end(); ++cls) {
      if (!cls->suffix().empty()) {
    ...
    for (QStringList::ConstIterator f = files.begin(); f != files.end(); ++f) {
      ...
      try {
        ...
        m_macro_templates.push_back(m);
        if (tl::verbosity() >= 20) {
          tl::info << "Using macro template from "
                   << tl::to_string(dir.filePath(*f)) << " (with name "
                   << m->name() << ")";
        }
  ...
  for (tl::Registrar<lym::MacroInterpreter>::iterator cls =
    ...
    if (tl::verbosity() >= 20) {
      for (std::vector<lym::Macro *>::const_iterator t =
               m_macro_templates.begin() + n;
           t != m_macro_templates.end(); ++t) {
        tl::info << "Using DSL macro template for " << (*t)->dsl_interpreter()
                 << " with name " << (*t)->name();
      }
    }
  }
  ...
  connect(m_file_watcher, SIGNAL(fileChanged(const QString &)), this,
          SLOT(file_changed(const QString &)));
  connect(m_file_watcher, SIGNAL(fileRemoved(const QString &)), this,
  ...
  //  Install a global event filter that allows us to lock out the application
  //  while a script is running or we are inside a breakpoint and other
  //  modifications.
...
MacroEditorDialog::~MacroEditorDialog() {
  ...
  for (std::vector<lym::Macro *>::iterator t = m_macro_templates.begin();
       t != m_macro_templates.end(); ++t) {
    delete *t;
  }
...
MacroEditorDialog *MacroEditorDialog::instance() {
...
void MacroEditorDialog::tabs_menu_about_to_show() {
  ...
  for (int i = 0; i < tabWidget->count(); ++i) {
    MacroEditorPage *page =
        dynamic_cast<MacroEditorPage *>(tabWidget->widget(i));
    if (page) {
...
void MacroEditorDialog::show(const std::string &cat, bool force_add) {
  ...
  if (m_first_show) {
    ...
    if (!collection || collection->is_readonly()) {
      for (lym::MacroCollection::const_child_iterator c =
               mp_root->begin_children();
           c != mp_root->end_children(); ++c) {
        if (c->second->category() == ct->category() &&
            !c->second->is_readonly()) {
    ...
    if (!force_add && collection &&
        (collection->begin() == collection->end() &&
         collection->begin_children() == collection->end_children())) {
      TipDialog td(this,
                   tl::to_string(QObject::tr(
                       "<html><body>To get started with the macro development "
                       "feature, read the documentation provided: <a "
                       "href=\"int:/about/macro_editor.xml\">About Macro "
                       "Development</a>.</body></html>")),
...
lay::MacroEditorTree *MacroEditorDialog::current_macro_tree() {
  lay::MacroEditorTree *t =
      dynamic_cast<lay::MacroEditorTree *>(treeTab->currentWidget());
  tl_assert(t != 0);
  return t;
}
...
bool MacroEditorDialog::configure(const std::string &name,
...
void MacroEditorDialog::showEvent(QShowEvent *) {
  ...
  mp_plugin_root->config_get(cfg_macro_editor_debugging_enabled,
                             m_debugging_on);
  ...
  if (m_debugging_on) {
    lay::ApplicationBase::instance()->ruby_interpreter().push_exec_handler(
        this);
  }
  lay::ApplicationBase::instance()->python_interpreter().push_console(this);
  if (m_debugging_on) {
    lay::ApplicationBase::instance()->python_interpreter().push_exec_handler(
        this);
  }
  ...
  try {
    ...
    while (!ex.at_end()) {
      ...
      if (ip == "ruby") {
        m_watch_expressions.push_back(std::make_pair(
            &lay::ApplicationBase::instance()->ruby_interpreter(), expr));
      } else if (ip == "python") {
        m_watch_expressions.push_back(std::make_pair(
            &lay::ApplicationBase::instance()->python_interpreter(), expr));
      }
    }
  ...
  for (std::map<lym::Macro *, MacroEditorPage *>::const_iterator page =
           m_tab_widgets.begin();
       page != m_tab_widgets.end(); ++page) {
    page->second->set_debugging_on(m_debugging_on);
  }

  //  clear the navigator on show - this way we get rid of the pseudo trace
  //  events we got while we built the pages
  clear_edit_trace();
...
void MacroEditorDialog::closeEvent(QCloseEvent *) {
  ...
  mp_plugin_root->config_set(cfg_macro_editor_debugging_enabled,
                             m_debugging_on);

  //  save the window state
  mp_plugin_root->config_set(cfg_macro_editor_window_state,
                             lay::save_dialog_state(this));

  //  save the console history (at maximum the last 200 entries)
  std::string hi;
  for (int i = std::max(0, input_field->count() - 200);
       i < input_field->count(); ++i) {
    if (!hi.empty()) {
      hi += ";";
    }
  ...
  std::string we;
  for (std::vector<std::pair<gsi::Interpreter *, std::string>>::const_iterator
           i = m_watch_expressions.begin();
       i != m_watch_expressions.end(); ++i) {
    if (!om.empty()) {
      om += ";";
    }
  ...
  mp_plugin_root->config_set(cfg_macro_editor_active_macro,
                             mp_run_macro ? mp_run_macro->path()
                                          : std::string());

  //  save the current macro
  MacroEditorPage *page =
      dynamic_cast<MacroEditorPage *>(tabWidget->currentWidget());
  std::string cm =
      page && page->macro() ? page->macro()->path() : std::string();
  ...
  lay::ApplicationBase::instance()->ruby_interpreter().remove_exec_handler(
      this);
  lay::ApplicationBase::instance()->python_interpreter().remove_console(this);
  lay::ApplicationBase::instance()->python_interpreter().remove_exec_handler(
      this);
}
  ...
  if (m_debugging_on != on) {
    ...
    if (isVisible()) {
      if (on) {
        lay::ApplicationBase::instance()->ruby_interpreter().push_exec_handler(
            this);
        lay::ApplicationBase::instance()
            ->python_interpreter()
            .push_exec_handler(this);
      } else {
        lay::ApplicationBase::instance()
            ->ruby_interpreter()
            .remove_exec_handler(this);
        lay::ApplicationBase::instance()
            ->python_interpreter()
            .remove_exec_handler(this);
      }
...
void MacroEditorDialog::process_events(QEventLoop::ProcessEventsFlags flags) {
  if (lay::ApplicationBase::instance()) {
    //  NOTE: we disable execution of deferred methods to avoid undesired
    //  execution of code while we are inside a Ruby callback through the silent
    //  mode NOTE: process_events will set BusySection::is_busy
    lay::ApplicationBase::instance()->process_events(flags, true /*silent*/);
  }
}

static bool any_modified(lym::MacroCollection *parent) {
  for (lym::MacroCollection::child_iterator c = parent->begin_children();
       c != parent->end_children(); ++c) {
    if (any_modified(c->second)) {
      return true;
    }
  }
  for (lym::MacroCollection::iterator c = parent->begin(); c != parent->end();
       ++c) {
...
bool MacroEditorDialog::can_exit() {
  ...
  //  simulate close event so we do a clean shut down and save the console MRU
  //  list for example
  if (isVisible()) {
    closeEvent(0);
  }
...
void MacroEditorDialog::add_edit_trace(bool compress) {
  ...
  if (m_edit_trace.size() > m_edit_trace_index + 1) {
    m_edit_trace.erase(m_edit_trace.begin() + m_edit_trace_index + 1,
                       m_edit_trace.end());
  }

  if (compress && m_edit_trace[m_edit_trace_index].path == path &&
...
void MacroEditorDialog::execute(const QString &cmd) {
  try {
    ...
    if (interpreter) {
      ...
      interpreter->eval_string_and_print(tl::to_string(cmd).c_str(), 0, 1,
                                         context);
    }
...
void MacroEditorDialog::update_inspected() {
  if (!m_in_breakpoint || !m_in_exec || !mp_current_interpreter) {
    ...
    std::unique_ptr<gsi::Inspector> ci(
        mp_current_interpreter->inspector(m_eval_context));
...
void MacroEditorDialog::update_watches() {
  ...
  for (std::vector<std::pair<gsi::Interpreter *, std::string>>::const_iterator
           w = m_watch_expressions.begin();
       w != m_watch_expressions.end(); ++w) {
    expressions.insert(w->second);
  }

  for (int i = 0; i < watchList->topLevelItemCount();) {
    if (expressions.find(tl::to_string(watchList->topLevelItem(i)->text(0))) ==
        expressions.end()) {
  ...
  for (std::vector<std::pair<gsi::Interpreter *, std::string>>::const_iterator
    ...
    if (w->first != mp_current_interpreter) {
      ...
      try {
        value = pretty_print(
            w->first->eval_expr(w->second.c_str(), 0, 1, m_eval_context));
      } catch (tl::ScriptError &ex) {
        value = tr("Error") + QString::fromUtf8(": ") +
                tl::to_qstring(ex.basic_msg());
      } catch (tl::Exception &ex) {
        value =
            tr("Error") + QString::fromUtf8(": ") + tl::to_qstring(ex.msg());
      } catch (...) {
...
void MacroEditorDialog::edit_watch() {
  ...
  if (index >= 0) {
    ...
    bool ok = false;
    QString we = QInputDialog::getText(
        this, tr("Add Watch Expressions"), tr("Enter expression to evaluate:"),
        QLineEdit::Normal, watchList->currentItem()->text(0), &ok);
    if (ok && !we.isEmpty()) {
      s_watch_expr = we;
...
void MacroEditorDialog::add_watch() {
  if (mp_current_interpreter) {
    ...
    bool ok = false;
    QString we = QInputDialog::getText(this, tr("Add Watch Expressions"),
                                       tr("Enter expression to evaluate:"),
                                       QLineEdit::Normal, s_watch_expr, &ok);
    if (ok && !we.isEmpty()) {
      s_watch_expr = we;
      m_watch_expressions.push_back(
          std::make_pair(mp_current_interpreter, tl::to_string(we)));
    }

    update_watches();

    watchList->setCurrentItem(
        watchList->topLevelItem(int(m_watch_expressions.size()) - 1));
  }
}
...
bool MacroEditorDialog::eventFilter(QObject *obj, QEvent *event) {
  ...
  if (lay::BusySection::is_busy() && (m_in_breakpoint || m_in_exec) &&
      (dynamic_cast<QInputEvent *>(event) != 0 ||
       dynamic_cast<QPaintEvent *>(event) != 0)) {

    //  In breakpoint or execution mode and while processing the events from the
    //  debugger, ignore all input or paint events targeted to widgets which are
    //  not children of this or the assistant dialog. Ignoring the paint event
    //  is required because otherwise a repaint action would be triggered on a
    //  layout which is potentially unstable or inconsistent. We nevertheless
    //  allow events send to a HelpDialog or ProgressWidget since those are
    //  vital for the application's functionality and are known not to cause any
    //  interference.
    QObject *rec = obj;
    while (rec && (rec != this && !dynamic_cast<lay::HelpDialog *>(rec) &&
                   !dynamic_cast<lay::ProgressWidget *>(rec))) {
      rec = rec->parent();
    }
    if (!rec) {
    ...
    //  While no explicit event processing is in progress and we are executing,
    //  this is an indication that "real" events are processed. In that case, we
    //  can postpone excplit processing. This avoids interference with GUI code
    //  run in the debugger.
    m_last_process_events = tl::Clock::current();
  }

  //  Handle events targeted towards the input edit box. This implements the
  //  special behavior of the command line.
  if (obj == input_field && event->type() == QEvent::KeyPress) {
    ...
    if (key_event && key_event->key() == Qt::Key_Return) {
      ...
      if (!cmd.isEmpty()) {
        ...
        if (m_history_index >= 0 && m_history_index < input_field->count() &&
            cmd == input_field->itemText(m_history_index)) {
  ...
  } else if (obj == tab_bar_of(tabWidget) &&
...
void MacroEditorDialog::macro_collection_deleted(
    lym::MacroCollection *collection) {
  ...
  for (std::set<lym::Macro *>::iterator mc = used_macros.begin();
       mc != used_macros.end(); ++mc) {

    if (mp_run_macro == *mc) {
      mp_run_macro = 0;
    }

    std::map<lym::Macro *, MacroEditorPage *>::iterator p =
        m_tab_widgets.find(*mc);
...
void MacroEditorDialog::macro_deleted(lym::Macro *macro) {
  ...
  std::map<lym::Macro *, MacroEditorPage *>::iterator page =
      m_tab_widgets.find(macro);
...
void MacroEditorDialog::macro_collection_changed(
    lym::MacroCollection * /*collection*/) {
  refresh_file_watcher();
}
...
void MacroEditorDialog::current_tab_changed(int index) {
  ...
  MacroEditorPage *page =
      dynamic_cast<MacroEditorPage *>(tabWidget->widget(index));
  if (page) {
    int tab_index = 0;
    for (std::vector<lay::MacroEditorTree *>::const_iterator mt =
             m_macro_trees.begin();
         mt != m_macro_trees.end(); ++mt, ++tab_index) {
  ...
  replaceFrame->setEnabled(page && page->macro() &&
                           !page->macro()->is_readonly());
...
lym::Macro *MacroEditorDialog::create_macro_here(const char *prefix) {
  ...
  if (!collection || collection->is_readonly()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Cannot add a macro here - the folder is read-only")));
  }
...
void MacroEditorDialog::move_macro(lym::Macro *source,
                                   lym::MacroCollection *target) {
  if (m_in_exec) {
    return;
  }
  ...
  if (source->parent() != target) {
    ...
    std::map<lym::Macro *, MacroEditorPage *>::iterator page =
        m_tab_widgets.find(source);
    if (page != m_tab_widgets.end()) {
      MacroEditorPage *w = page->second;
      w->connect_macro(m);
      m_tab_widgets.erase(page);
      m_tab_widgets.insert(std::make_pair(m, w));
      tabWidget->setTabToolTip(tabWidget->indexOf(w),
                               tl::to_qstring(m->summary()));
...
void MacroEditorDialog::move_subfolder(lym::MacroCollection *source,
                                       lym::MacroCollection *target) {
  ...
  for (lym::MacroCollection::iterator mm = source->begin(); mm != source->end();
       ++mm) {
  ...
  for (std::vector<lym::MacroCollection::iterator>::const_iterator d =
           m_del.begin();
       d != m_del.end(); ++d) {
    source->erase(*d);
  }
  ...
  for (lym::MacroCollection::child_iterator m = source->begin_children();
       m != source->end_children(); ++m) {
  ...
  for (std::vector<lym::MacroCollection::child_iterator>::const_iterator d =
           mc_del.begin();
       d != mc_del.end(); ++d) {
    source->erase(*d);
  }
...
void MacroEditorDialog::move_folder(lym::MacroCollection *source,
                                    lym::MacroCollection *target) {
  if (m_in_exec) {
    return;
  }
...
void MacroEditorDialog::apply_search(bool if_needed) {
  ...
  if (!searchEditBox->text().isEmpty()) {
    QRegExp re(searchEditBox->text(),
               actionCaseSensitive->isChecked() ? Qt::CaseSensitive
                                                : Qt::CaseInsensitive,
               actionUseRegularExpressions->isChecked() ? QRegExp::RegExp
                                                        : QRegExp::FixedString);
...
void MacroEditorDialog::save_as_button_clicked() {
  ...
  lay::FileDialog file_dialog(lay::MainWindow::instance(),
                              tl::to_string(QObject::tr("Save Macro As")),
                              tl::to_string(QObject::tr("All files (*)")), "");
...
lym::Macro *MacroEditorDialog::new_macro() {
  ...
  if (!ct->current_macro() && !ct->current_macro_collection()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Select a position where to add the macro")));
  }
  ...
  lay::MacroTemplateSelectionDialog template_dialog(this, m_macro_templates,
                                                    cat);
  ...
  lym::Macro *m =
      create_macro_here(m_macro_templates[template_index]->name().c_str());
  ...
  //  NOTE: we save to make the file watcher go silent and to keep the file
  //  system in sync
  m->save();

  ct->set_current(m);
  if (ct->currentIndex().isValid() &&
      (ct->model()->flags(ct->currentIndex()) & Qt::ItemIsEditable)) {
...
void MacroEditorDialog::close_all() {
  ...
  for (std::map<lym::Macro *, MacroEditorPage *>::iterator p =
           m_tab_widgets.begin();
       p != m_tab_widgets.end(); ++p) {
    if (p->second) {
      p->second->connect_macro(0);
    }
    delete p->second;
...
void MacroEditorDialog::close_all_but_this() { close_many(0); }

void MacroEditorDialog::close_all_left() { close_many(-1); }

void MacroEditorDialog::close_all_right() { close_many(1); }
...
void MacroEditorDialog::close_many(int r2c) {
  ...
  for (int i = tabWidget->count(); i > 0;) {
    --i;
    if ((r2c == 0 && i != ci) || (r2c < 0 && i < ci) || (r2c > 0 && i > ci)) {
...
void MacroEditorDialog::delete_button_clicked() {
  ...
  if (collection) {
    ...
    if (p) {
      ...
      if (!collection->del()) {
        throw tl::Exception("Can't delete this folder - there may still be "
                            "some other files inside it");
      }
      p->erase(collection);
    }
    ...
    if (collection) {
      ...
      if (QMessageBox::question(
              this, QObject::tr("Delete Macro File"),
              tl::to_qstring(tl::to_string(QObject::tr(
                                 "Are you sure to delete the macro file ")) +
                             m->path() + "?"),
              QMessageBox::Ok, QMessageBox::Cancel) != QMessageBox::Ok) {
        return;
      }
...
void MacroEditorDialog::rename_button_clicked() {
  ...
  if (index.isValid()) {
    if (ct->model()->flags(index) & Qt::ItemIsEditable) {
      ...
      throw tl::Exception(
          tl::to_string(QObject::tr("Cannot edit this item's name")));
    }
  }
...
void MacroEditorDialog::ensure_writeable_collection_selected() {
  ...
  if (!collection) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Cannot perform that operation - no place selected")));
  }
  if (collection->is_readonly()) {
    throw tl::Exception(tl::to_string(QObject::tr(
        "Cannot perform that operation here - this place is read-only")));
  }
}
...
static void set_custom_paths(
    lay::Dispatcher *root,
    const std::vector<std::pair<std::string, std::string>> &paths) {
  std::string mp;

  //  add paths from our category
  for (std::vector<std::pair<std::string, std::string>>::const_iterator p =
           paths.begin();
       p != paths.end(); ++p) {
    if (!mp.empty()) {
      mp += ";";
...
void MacroEditorDialog::file_changed_timer() {
  ...
  m_removed_files.erase(
      std::unique(m_removed_files.begin(), m_removed_files.end()),
      m_removed_files.end());
  ...
  for (std::vector<QString>::const_iterator f = m_changed_files.begin();
       f != m_changed_files.end(); ++f) {
    ...
    if (w->second->macro() && w->second->macro()->is_modified()) {
      ...
      lay::MacroEditorNotification n(
          "reload",
          tl::to_string(tr("Macro has changed on disk, but was modified")),
          tl::Variant(fn));
      ...
      lay::MacroEditorNotification n(
          "reload", tl::to_string(tr("Macro has changed on disk")),
          tl::Variant(fn));
  ...
  for (std::vector<QString>::const_iterator f = m_removed_files.begin();
       f != m_removed_files.end(); ++f) {
    ...
    if (w->second->macro() && w->second->macro()->is_modified()) {
      ...
      lay::MacroEditorNotification n(
          "close",
          tl::to_string(tr("Macro has been removed on disk, but was modified")),
          tl::Variant(fn));
      ...
      lay::MacroEditorNotification n(
          "close", tl::to_string(tr("Macro has been removed on disk")),
          tl::Variant(fn));
...
void MacroEditorDialog::sync_file_watcher(
    lym::MacroCollection * /*collection*/) {
#if 0
  ...
  for (std::map<lym::Macro *, MacroEditorPage *>::const_iterator m =
           m_tab_widgets.begin();
       m != m_tab_widgets.end(); ++m) {
...
void MacroEditorDialog::add_location() {
  ...
  QString new_dir =
      QFileDialog::getExistingDirectory(this, QObject::tr("Add Location"));
  if (new_dir.isNull()) {
    return;
  }
  ...
  std::vector<std::pair<std::string, std::string>> paths =
      get_custom_paths(mp_plugin_root);
  std::string new_path = tl::to_string(QFileInfo(new_dir).absoluteFilePath());
  paths.push_back(std::make_pair(new_path, cat));

  lym::MacroCollection *c = mp_root->add_folder(
      tl::to_string(QObject::tr("Project")) + " - " + new_path, new_path, cat,
      false /* writeable */, false /* do not auto-create folders */);
  if (!c) {
    throw tl::Exception(tl::to_string(QObject::tr(
        "The selected directory is already installed as custom location")));
  }
  ...
  if (c->has_autorun()) {
    if (QMessageBox::question(
            this, QObject::tr("Run Macros"),
            QObject::tr("The selected folder has macros configured to run "
                        "automatically.\n\nChoose 'Yes' to run these macros "
                        "now. Choose 'No' to not run them."),
            QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {
      c->autorun();
    }
  }
...
void MacroEditorDialog::remove_location() {
  ...
  if (!collection) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Select tree location to remove")));
  }
  ...
  for (std::vector<std::pair<std::string, std::string>>::iterator p =
           paths.begin();
       p != paths.end(); ++p) {
  ...
  if (!found) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Unable to remove that location")));
  }
...
void MacroEditorDialog::import_button_clicked() {
  ...
  if (!ct->current_macro() && !ct->current_macro_collection()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Select a position where to import the macro")));
  }

  //  TODO: risky: file_dialog might be deleted because the MainWindow is
  //  deleted (it's the parent)
  static lay::FileDialog *file_dialog = 0;
  if (!file_dialog) {

    std::string filters = tl::to_string(
        QObject::tr("All files (*);;KLayout macro files (*.lym);;Ruby files "
                    "(*.rb);;Python files (*.py)"));
    ...
    file_dialog = new lay::FileDialog(
        lay::MainWindow::instance(),
        tl::to_string(QObject::tr("Import Macro File")), filters, "lym");
  }
  ...
  if (file_dialog->get_open(fn)) {
    ...
    lym::Macro *m = create_macro_here(
        tl::to_string(QFileInfo(tl::to_qstring(fn)).baseName()).c_str());
...
void MacroEditorDialog::new_folder_button_clicked() {
  ...
  if (!collection || collection->is_readonly()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Cannot create a folder here")));
  }

  lym::MacroCollection *mm = collection->create_folder();
  if (!mm) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Failed to create the folder here")));
  }
...
void MacroEditorDialog::start_exec(gsi::Interpreter *ec) {
  ...
  try {
    ...
    for (std::map<lym::Macro *, MacroEditorPage *>::const_iterator f =
             m_tab_widgets.begin();
         f != m_tab_widgets.end(); ++f) {
...
size_t MacroEditorDialog::id_for_path(gsi::Interpreter *,
...
void MacroEditorDialog::translate_pseudo_id(size_t &file_id, int &line) {
  if (file_id >= pseudo_file_offset) {
    ...
    std::map<std::pair<size_t, int>, std::pair<size_t, int>>::iterator ic =
        m_include_file_id_cache.find(ck);
    if (ic != m_include_file_id_cache.end()) {
      ...
      if (file_id < m_include_expanders.size()) {
        ...
        std::pair<std::string, int> fp =
            m_include_expanders[file_id].translate_to_original(line);
        line = fp.second;

        std::map<std::string, size_t>::const_iterator i =
            m_include_paths_to_ids.find(fp.first);
      ...
      m_include_file_id_cache.insert(
          std::make_pair(ck, std::make_pair(file_id, line)));
    }
  }
}

void MacroEditorDialog::exception_thrown(
    gsi::Interpreter *interpreter, size_t file_id, int line,
    const std::string &eclass, const std::string &emsg,
  ...
  //  avoid recursive breakpoints and exception catches from the console while
  //  in a breakpoint or exception stop
  if (lay::BusySection::is_busy()) {
    return;
  }

  //  translate the pseudo file ID and line to the real one (include file
  //  processing)
  translate_pseudo_id(file_id, line);

  try {

    //  If the exception is thrown in code that is inside a file managed by the
    //  macro collection, offer to stop the debugger there.
    ...
    std::string p;
    if (file_id > 0 && file_id <= m_file_to_widget.size() &&
        m_file_to_widget[file_id - 1].first) {
    ...
    int res = QMessageBox::critical(
        this, QObject::tr("Exception Caught"),
        tl::to_qstring(
            tl::to_string(QObject::tr("Caught the following exception:\n")) +
            emsg + " (Class " + eclass + ")\n\n" +
            tl::to_string(
                QObject::tr("Press 'Ok' to continue.\nPress 'Ignore' to ignore "
                            "this and future exceptions from this file.\nPress "
                            "'Cancel' to stop in the debugger"))),
    ...
    if (res == QMessageBox::Ok) {
      ...
      for (std::set<std::string>::const_iterator i =
               m_ignore_exception_list.begin();
           i != m_ignore_exception_list.end(); ++i) {
        il += ";";
    ...
    if (file_id > 0 && file_id <= m_file_to_widget.size() &&
        m_file_to_widget[file_id - 1].second) {
    ...
    if (QApplication::activeModalWidget() &&
      ...
      //  apparently that is the only way to override the event handling
      //  mechanism of Qt: if the breakpoint is issued from inside an event
      //  handler of a modal dialog, the editor window does not receive events,
      //  not even if we requested filtering.
      hide();
      exec();
      show();
...
void MacroEditorDialog::exit_if_needed() {
  ...
  //  NOTE: we must not raise ExitException from outside events (e.g. PyQt5
  //  events) as ExitException would otherwise terminate the application.
...
void MacroEditorDialog::trace(
  ...
  //  Note: only scripts running in the context of the execution controller (the
  //  one who called start_exec) can be interrupted and single-stepped, but
  //  breakpoints can make the debugger stop in other interpreters.
  if (file_id > 0 &&
      ((interpreter == mp_exec_controller && m_stop_stack_depth >= 0 &&
        stack_trace_provider->stack_depth() <= m_stop_stack_depth) ||
       (interpreter == mp_exec_controller && !m_continue) ||
       (file_id <= m_file_to_widget.size() &&
        m_file_to_widget[file_id - 1].second &&
        m_file_to_widget[file_id - 1].second->exec_model()->is_breakpoint(
            line)))) {
    ...
    try {
      ...
      if (QApplication::activeModalWidget() &&
        ...
        //  apparently that is the only way to override the event handling
        //  mechanism of Qt: if the breakpoint is issued from inside an event
        //  handler of a modal dialog, the editor window does not receive
        //  events, not even if we requested filtering.
        hide();
        exec();
        show();
    ...
    if ((tl::Clock::current() - m_last_process_events).seconds() >
      ...
      m_last_process_events = tl::Clock::current();
      m_process_events_interval = std::max(
          0.05, std::min(2.0, (m_last_process_events - start).seconds() * 5.0));
...
void MacroEditorDialog::enter_breakpoint_mode(
    gsi::Interpreter *interpreter,
    const gsi::StackTraceProvider *stack_trace_provider) {
  ...
  callStack->clear();
  std::vector<tl::BacktraceElement> bt = stack_trace_provider->stack_trace();
  for (std::vector<tl::BacktraceElement>::const_iterator b = bt.begin();
       b != bt.end(); ++b) {
  ...
  //  Hint: apparently it's necessary to process the events to make the layout
  //  system recognize that we have hidden parts from the edit field by the
  //  runtime frame.
  process_events(QEventLoop::ExcludeUserInputEvents);

  if (!bt.empty()) {
    set_exec_point(&bt[scope_index].file, bt[scope_index].line,
                   int(scope_index));
  }
...
void MacroEditorDialog::update_ui_to_run_mode() { dm_update_ui_to_run_mode(); }
...
void MacroEditorDialog::do_update_ui_to_run_mode() {
  ...
  runButton->setEnabled(
      (!m_in_exec &&
       (mp_run_macro || (page && page->macro() &&
                         page->macro()->interpreter() != lym::Macro::None))) ||
      m_in_breakpoint);
  runThisButton->setEnabled(
      (!m_in_exec && page && page->macro() &&
       page->macro()->interpreter() != lym::Macro::None) ||
      m_in_breakpoint);
  ...
  for (std::vector<lay::MacroEditorTree *>::const_iterator mt =
           m_macro_trees.begin();
       mt != m_macro_trees.end(); ++mt) {
    (*mt)->setEditTriggers(m_in_exec ? QAbstractItemView::NoEditTriggers
  ...
  propertiesButton->setEnabled(
      !m_in_exec && page && page->macro() &&
      (page->macro()->format() == lym::Macro::MacroFormat ||
       page->macro()->format() ==
           lym::Macro::PlainTextWithHashAnnotationsFormat));
  ...
  if (m_in_exec) {
    ...
    if (m_in_breakpoint && mp_current_interpreter) {
      ...
      base_color =
          QColor(base_color.red(), int(0.5 + base_color.green() * alpha),
                 int(0.5 + base_color.blue() * alpha));
      alt_base_color = QColor(alt_base_color.red(),
                              int(0.5 + alt_base_color.green() * alpha),
                              int(0.5 + alt_base_color.blue() * alpha));
      runtimeFrame->show();

    } else {

      base_color =
          QColor(int(0.5 + base_color.red() * alpha), base_color.green(),
                 int(0.5 + base_color.blue() * alpha));
      alt_base_color = QColor(int(0.5 + alt_base_color.red() * alpha),
                              alt_base_color.green(),
                              int(0.5 + alt_base_color.blue() * alpha));
      runtimeFrame->hide();
    }
  ...
  //  for some reason, callStack, variableList and watchList don't inherit the
  //  palette ...
  callStack->setPalette(p);
  variableList->setPalette(p);
  watchList->setPalette(p);

  std::map<lym::Macro *, MacroEditorPage *>::const_iterator t =
      m_tab_widgets.find(mp_run_macro);
  if (t != m_tab_widgets.end()) {
    int index = tabWidget->indexOf(t->second);
    if (index >= 0) {
      tabWidget->setTabIcon(
          index, QIcon(QString::fromUtf8(m_in_exec ? (m_in_breakpoint
                                                          ? ":/pause_16px.png"
                                                          : ":/stop_16px.png")
                                                   : ":/run_16px.png")));
    }
...
MacroEditorPage *MacroEditorDialog::create_page(lym::Macro *macro) {
  std::unique_ptr<MacroEditorPage> editor(
      new MacroEditorPage(this, &m_highlighters));
  ...
  connect(editor.get(), SIGNAL(close_requested()), this,
          SLOT(close_requested()));
  connect(editor.get(), SIGNAL(help_requested(const QString &)), this,
          SLOT(help_requested(const QString &)));
  connect(editor.get(), SIGNAL(search_requested(const QString &, bool)), this,
          SLOT(search_requested(const QString &, bool)));
  connect(editor.get(), SIGNAL(edit_trace(bool)), this,
          SLOT(add_edit_trace(bool)));
  return editor.release();
}

MacroEditorPage *MacroEditorDialog::editor_for_macro(lym::Macro *macro) {
  for (std::vector<lay::MacroEditorTree *>::const_iterator mt =
           m_macro_trees.begin();
       mt != m_macro_trees.end(); ++mt) {
  ...
  if (page == m_tab_widgets.end()) {
    ...
    for (std::vector<std::pair<lym::Macro *, MacroEditorPage *>>::iterator f =
             m_file_to_widget.begin();
         f != m_file_to_widget.end(); ++f) {
      if (f->first == macro) {
        f->second = editor;
        break;
      }
...
MacroEditorPage *MacroEditorDialog::editor_for_file(const std::string &path) {
  lym::Macro *macro = mp_root->find_macro(path);
  if (macro) {
...
void MacroEditorDialog::set_exec_point(const std::string *file, int line,
                                       int eval_context) {
  MacroEditorPage *editor = 0;
  if (file) {
  ...
  for (std::map<lym::Macro *, MacroEditorPage *>::const_iterator f =
           m_tab_widgets.begin();
       f != m_tab_widgets.end(); ++f) {
    f->second->exec_model()->set_current_line(f->second == editor ? line : -1,
                                              true);
  }
...
void MacroEditorDialog::pause_button_clicked() { m_continue = false; }
...
void MacroEditorDialog::next_step_button_clicked() {
  BEGIN_PROTECTED
  run(m_in_exec ? std::max(0, m_current_stack_depth)
...
lym::Macro *MacroEditorDialog::current_run_macro() {
  //  validate the current run macro against the macros present in the
  //  collection and return 0 if invalid (that takes the current one)
...
void MacroEditorDialog::run(int stop_stack_depth, lym::Macro *macro) {
  ...
  if (m_in_breakpoint) {
    ...
    if (!m_save_all_on_run && any_modified(mp_root)) {
      if (QMessageBox::question(
              this, QObject::tr("Save Macros"),
              QObject::tr("Some files are modified and need to be saved before "
                          "running the macro. Do you want to save them?"),
    ...
    //  save all macros
    //  Hint: although it looks like to touch decision, it's important to save
    //  every change since files may be included/loaded/required by other files.
    commit();
    mp_root->save();
    ...
    try {
      ...
      write_str(
          tl::sprintf(tl::to_string(tr("Running macro %s\n")), macro->path())
              .c_str(),
          OS_echo);
...
void MacroEditorDialog::set_run_macro(lym::Macro *m) {
  if (m != mp_run_macro) {
    ...
    if (t != m_tab_widgets.end()) {
      int index = tabWidget->indexOf(t->second);
      if (index >= 0) {
        tabWidget->setTabIcon(index,
                              QIcon(QString::fromUtf8(":/run_16px.png")));
      }
    }
...
class MacroEditorPluginDeclaration : public lay::PluginDeclaration {
public:
  ...
  get_options(std::vector<std::pair<std::string, std::string>> &options) const {
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_styles, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_save_all_on_run, "false"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_debugging_enabled, "true"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_file_watcher_enabled, "true"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_font_size, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_font_family, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_stop_on_exception, "true"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_tab_width, "8"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_indent, "2"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_window_state, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_console_mru, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_console_interpreter, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_open_macros, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_current_macro, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_macro_editor_active_macro, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_macro_editor_watch_expressions, ""));
  }
};

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new MacroEditorPluginDeclaration(), 1500, "MacroEditor");
