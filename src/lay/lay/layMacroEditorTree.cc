...
static QIcon tree_icon_for_format(const lym::Macro *m, bool active) {
  ...
  if (m->interpreter() == lym::Macro::Text) {
    ...
    if (m->format() == lym::Macro::PlainTextFormat ||
        m->format() == lym::Macro::PlainTextWithHashAnnotationsFormat) {
      if (active) {
...
struct FilteredMacroCollectionIter {
  FilteredMacroCollectionIter(const lym::MacroCollection *mc,
  ...
  const std::pair<const std::string, lym::MacroCollection *> *
  operator->() const {
    return m_b.operator->();
  }
...
private:
  ...
  void next() {
    while (m_b != m_e && (!m_b->second->category().empty() &&
                          m_b->second->category() != m_category)) {
      ++m_b;
    }
  }
};

MacroTreeModel::MacroTreeModel(QObject *parent, lay::MacroEditorDialog *dialog,
                               lym::MacroCollection *root,
                               const std::string &cat)
    : QAbstractItemModel(parent), mp_dialog(dialog), mp_parent(dialog),
      mp_root(root), m_category(cat) {
  connect(root, SIGNAL(macro_changed(lym::Macro *)), this,
          SLOT(macro_changed()));
  connect(root, SIGNAL(macro_deleted(lym::Macro *)), this,
          SLOT(macro_deleted(lym::Macro *)));
  connect(root, SIGNAL(macro_collection_deleted(lym::MacroCollection *)), this,
          SLOT(macro_collection_deleted(lym::MacroCollection *)));
  connect(root, SIGNAL(macro_collection_changed(lym::MacroCollection *)), this,
          SLOT(macro_collection_changed()));
  connect(root, SIGNAL(about_to_change()), this, SLOT(about_to_change()));
}

MacroTreeModel::MacroTreeModel(QWidget *parent, lym::MacroCollection *root,
                               const std::string &cat)
    : QAbstractItemModel(parent), mp_dialog(0), mp_parent(parent),
      mp_root(root), m_category(cat) {
  connect(root, SIGNAL(macro_changed(lym::Macro *)), this,
          SLOT(macro_changed()));
  connect(root, SIGNAL(macro_deleted(lym::Macro *)), this,
...
void MacroTreeModel::macro_changed() { update_data(); }

void MacroTreeModel::update_data() {
  int rc = rowCount(QModelIndex());
  if (rc > 0) {
    emit dataChanged(index(0, 0, QModelIndex()),
                     index(rc - 1, 0, QModelIndex()));
  }
}

void MacroTreeModel::about_to_change() { emit layoutAboutToBeChanged(); }
...
void MacroTreeModel::macro_collection_changed() {
  ...
  for (QModelIndexList::const_iterator i = pi.begin(); i != pi.end(); ++i) {
    if (is_valid_pointer(i->internalPointer())) {
      lym::Macro *macro =
          dynamic_cast<lym::Macro *>((QObject *)i->internalPointer());
      lym::MacroCollection *mc =
          dynamic_cast<lym::MacroCollection *>((QObject *)i->internalPointer());
      if (macro) {
...
void MacroTreeModel::invalidate_cache() { m_valid_objects.clear(); }
...
bool MacroTreeModel::is_valid_pointer(void *ptr) const {
  if (m_valid_objects.empty()) {
    ...
    for (std::set<lym::Macro *>::const_iterator m = macros.begin();
         m != macros.end(); ++m) {
      m_valid_objects.insert((void *)(QObject *)*m);
    }
    for (std::set<lym::MacroCollection *>::const_iterator m =
             macro_collections.begin();
         m != macro_collections.end(); ++m) {
      m_valid_objects.insert((void *)(QObject *)*m);
    }
  }
...
QMimeData *MacroTreeModel::mimeData(const QModelIndexList &indexes) const {
  ...
  mimeData->setData(QString::fromUtf8("application/klayout-macros.list"),
                    encodedData);
  return mimeData;
}

bool MacroTreeModel::dropMimeData(const QMimeData *data,
                                  Qt::DropAction /*action*/, int /*row*/,
                                  int /*column*/, const QModelIndex &parent) {
  QByteArray encodedData =
      data->data(QString::fromUtf8("application/klayout-macros.list"));
  ...
  lym::MacroCollection *to_mc =
      dynamic_cast<lym::MacroCollection *>((QObject *)parent.internalPointer());
  if (!to_mc) {
    return false;
  }
  ...
  while (!stream.atEnd()) {
    ...
    if (is_valid_pointer((void *)p)) {
      ...
      lym::MacroCollection *from_mc =
          dynamic_cast<lym::MacroCollection *>(from_object);
...
bool MacroTreeModel::setData(const QModelIndex &index, const QVariant &v,
                             int role) {
  if (!index.isValid() || role != Qt::UserRole ||
      !is_valid_pointer(index.internalPointer())) {
    return false;
  }
  ...
  if (macro) {
    if (macro->parent() &&
        macro->parent()->macro_by_name(tl::to_string(v.toString()),
                                       macro->format()) != 0) {
    ...
    if (mc->parent() &&
        mc->parent()->folder_by_name(tl::to_string(v.toString())) != 0) {
...
QVariant MacroTreeModel::data(const QModelIndex &index, int role) const {
  ...
  const lym::MacroCollection *mc =
      dynamic_cast<const lym::MacroCollection *>(object);
  if (macro) {
    if (role == Qt::DisplayRole) {
      return QVariant(tl::to_qstring(macro->display_string()));
    } else if (role == Qt::DecorationRole) {
      return QVariant(tree_icon_for_format(
          macro, mp_dialog && macro == mp_dialog->run_macro()));
...
Qt::ItemFlags MacroTreeModel::flags(const QModelIndex &index) const {
  if ((mp_dialog && mp_dialog->in_exec()) || !index.isValid() ||
      !is_valid_pointer(index.internalPointer())) {
...
bool MacroTreeModel::hasChildren(const QModelIndex &parent) const {
  ...
  if (!parent.isValid()) {
    ...
    mc = dynamic_cast<const lym::MacroCollection *>(
        (QObject *)parent.internalPointer());
  }
  if (mc) {
    return !FilteredMacroCollectionIter(mc, m_category).at_end() ||
           mc->begin() != mc->end();
  } else {
    return false;
  }
...
QModelIndex MacroTreeModel::parent(const QModelIndex &index) const {
  ...
  if (p) {
    ...
    if (pp) {
      int row = 0;
      for (FilteredMacroCollectionIter i(pp, m_category); !i.at_end();
           ++i, ++row) {
        if (i->second == p) {
...
QModelIndex MacroTreeModel::index(int row, int column,
...
int MacroTreeModel::rowCount(const QModelIndex &parent) const {
  ...
  if (mc) {
    ...
    for (lym::MacroCollection::const_iterator i = mc->begin(); i != mc->end();
         ++i) {
      ++n;
    }
  }
...
QModelIndex MacroTreeModel::index_for(lym::Macro *macro) const {
  ...
  for (lym::MacroCollection::const_iterator i = pp->begin(); i != pp->end();
       ++i, ++row) {
    if (i->second == macro) {
...
QModelIndex MacroTreeModel::index_for(lym::MacroCollection *mc) const {
  if (!mc || !mc->parent()) {
    return QModelIndex();
  }
...
void MacroEditorTree::model_move_macro(lym::Macro *source,
                                       lym::MacroCollection *target) {
  emit move_macro(source, target);
}

void MacroEditorTree::model_move_folder(lym::MacroCollection *source,
                                        lym::MacroCollection *target) {
...
lym::MacroCollection *MacroEditorTree::current_macro_collection() const {
  QModelIndex ci = mp_proxyModel->mapToSource(currentIndex());
  if (ci.isValid() && mp_model->is_valid_pointer(ci.internalPointer())) {
    return dynamic_cast<lym::MacroCollection *>(
        (QObject *)ci.internalPointer());
  } else {
    return 0;
  }
}
...
void MacroEditorTree::setup(lay::MacroEditorDialog *dialog) {
  mp_model = new MacroTreeModel(this, dialog, &lym::MacroCollection::root(),
                                m_category);
  ...
  connect(this, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(double_clicked_slot(const QModelIndex &)));
  connect(mp_model, SIGNAL(macro_renamed(lym::Macro *)), this,
          SLOT(model_macro_renamed(lym::Macro *)));
  connect(mp_model, SIGNAL(folder_renamed(lym::MacroCollection *)), this,
          SLOT(model_folder_renamed(lym::MacroCollection *)));
  connect(mp_model, SIGNAL(move_macro(lym::Macro *, lym::MacroCollection *)),
          this, SLOT(model_move_macro(lym::Macro *, lym::MacroCollection *)));
  connect(
      mp_model,
      SIGNAL(move_folder(lym::MacroCollection *, lym::MacroCollection *)), this,
      SLOT(model_move_folder(lym::MacroCollection *, lym::MacroCollection *)));
}
