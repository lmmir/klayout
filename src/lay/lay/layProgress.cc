...
ProgressReporter::ProgressReporter() : mp_pb(0), m_pw_visible(false) {
  //  .. nothing yet ..
}

ProgressReporter::~ProgressReporter() { mp_pb = 0; }
...
void ProgressReporter::trigger(tl::Progress *progress) {
  ...
  if (q != m_queued.end() &&
      (tl::Clock::current() - q->second).seconds() > visibility_delay) {
    if (!m_pw_visible) {
      set_visible(true);
    }
    m_active.insert(progress);
...
void ProgressReporter::process_events() {
  if (m_pw_visible && lay::MainWindow::instance() &&
      lay::ApplicationBase::instance()) {
    lay::ApplicationBase::instance()->process_events(
        QEventLoop::AllEvents, true /* no deferred methods */);
  }
}
...
void ProgressReporter::set_visible(bool vis) {
  ...
  if (vis != m_pw_visible) {
    ...
    //  prevent deferred method execution inside progress events - this might
...
bool ProgressReporter::eventFilter(QObject *obj, QEvent *event) {
  ...
  if (QApplication::activeModalWidget() &&
      !dynamic_cast<lay::MainWindow *>(QApplication::activeModalWidget())) {
    return false;
  }
  ...
  if (dynamic_cast<QInputEvent *>(event)) {
    ...
    QObject *o = obj;
    while (o) {
      //  If the watched object is a child of the progress widget or the macro
      //  editor, pass the event on to this. Including the macro editor keeps it
      //  alive while progress events are processed.
