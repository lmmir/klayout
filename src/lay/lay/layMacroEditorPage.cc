...
void MacroEditorTextWidget::paintEvent(QPaintEvent *event) {
  //  lacking any other good way to detect scrolling, we catch the paint events
  //  of the viewport to detect a change in the geometry
...
MacroEditorNotificationWidget::MacroEditorNotificationWidget(
    MacroEditorPage *parent, const MacroEditorNotification *notification)
  ...
  for (auto a = notification->actions().begin();
       a != notification->actions().end(); ++a) {
...
void MacroEditorNotificationWidget::action_triggered() {
...
void MacroEditorNotificationWidget::close_triggered() {
...
MacroEditorHighlighters::MacroEditorHighlighters(QObject *parent)
    : m_basic_attributes() {
  //  TODO: more languages
  m_attributes.push_back(std::make_pair(
      "ruby", GenericSyntaxHighlighterAttributes(&m_basic_attributes)));
  m_attributes.push_back(std::make_pair(
      "python", GenericSyntaxHighlighterAttributes(&m_basic_attributes)));

  for (std::vector<std::pair<std::string, GenericSyntaxHighlighterAttributes>>::
           iterator a = m_attributes.begin();
...
QSyntaxHighlighter *MacroEditorHighlighters::highlighter_for(
    QObject *parent, lym::Macro::Interpreter lang, const std::string &dsl_name,
    bool initialize) {
...
lay::GenericSyntaxHighlighter *MacroEditorHighlighters::highlighter_for_scheme(
    QObject *parent, const std::string &scheme,
    GenericSyntaxHighlighterAttributes *attributes, bool initialize) {
  if (!scheme.empty()) {
    ...
    lay::GenericSyntaxHighlighter *hl =
        new GenericSyntaxHighlighter(parent, input, attributes, initialize);
    input.close();
...
MacroEditorHighlighters::attributes_for(lym::Macro::Interpreter lang,
...
std::string MacroEditorHighlighters::to_string() const {
  std::string s = "basic:" + m_basic_attributes.to_string();

  for (std::vector<std::pair<std::string, GenericSyntaxHighlighterAttributes>>::
           const_iterator a = m_attributes.begin();
       a != m_attributes.end(); ++a) {
...
void MacroEditorHighlighters::load(const std::string &s) {
  try {
    ...
    while (!ex.at_end()) {
      ...
      if (t == "basic") {
        ...
        for (std::vector<std::pair<
                 std::string, GenericSyntaxHighlighterAttributes>>::iterator a =
                 m_attributes.begin();
             a != m_attributes.end(); ++a) {
          if (a->first == t) {
            attributes = &a->second;
...
std::string MacroEditorHighlighters::scheme_for(lym::Macro::Interpreter lang,
                                                const std::string &dsl_name) {
...
MacroEditorExecutionModel::MacroEditorExecutionModel(QObject *parent)
    : QObject(parent), m_current_line(-1), m_run_mode(false),
      m_interpreter(lym::Macro::None) {
  // .. nothing yet ..
}
...
MacroEditorSidePanel::MacroEditorSidePanel(
    QWidget *parent, MacroEditorTextWidget *text,
    ...
    : QWidget(parent), mp_text(text), mp_exec_model(exec_model),
      ...
      m_breakpoint_disabled_pixmap(
          QString::fromUtf8(":/breakpointmarkdisabled_16px.png")),
...
QSize MacroEditorSidePanel::sizeHint() const {
  int w;
#if QT_VERSION >= 0x60000
  w = QFontMetrics(mp_text->font())
          .horizontalAdvance(QString::fromUtf8("12345"));
#else
...
void MacroEditorSidePanel::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    ...
    QTextBlock b =
        mp_text
            ->cursorForPosition(QPoint(0, -mp_text->viewport()->rect().top()))
            .block();
    ...
    while (b.isValid()) {
      ...
      rc.translate(0,
                   mp_text->frameWidth() + mp_text->viewport()->rect().top());
...
void MacroEditorSidePanel::redraw() { update(); }
...
void MacroEditorSidePanel::paintEvent(QPaintEvent *) {
  ...
  //  paint background
  for (QTextBlock b = mp_text
                          ->cursorForPosition(
                              QPoint(0, -mp_text->viewport()->rect().top()))
                          .block();
       b.isValid(); b = b.next()) {
    ...
    QRect rt(sidePanelMargin + m_breakpoint_pixmap.width(), rc.top(), width(),
             rc.height());
  ...
  if (!m_watermark.isEmpty()) {
    ...
    p.drawText(QRect(-height(), 0, height(), width()),
  ...
  for (QTextBlock b = mp_text
    ...
    p.drawText(rt.adjusted(sidePanelMargin, 0, 0, 0),
    ...
    if (mp_exec_model->breakpoints().find(l) !=
        mp_exec_model->breakpoints().end()) {
      ...
      if (m_debugging_on) {
        ...
        p.drawPixmap(rpt, m_breakpoint_disabled_pixmap,
                     m_breakpoint_pixmap.rect());
      }
    }

    if (mp_exec_model->run_mode() && mp_exec_model->current_line() == l) {
      int icon_size = std::min(m_exec_point_pixmap.height(), rt.height());
      QRect rpt(sidePanelMargin, rt.center().y() - icon_size / 2 + 1, icon_size,
                icon_size);
...
MacroEditorPage::MacroEditorPage(QWidget * /*parent*/,
                                 MacroEditorHighlighters *highlighters)
    : mp_macro(0), mp_highlighters(highlighters), mp_highlighter(0),
      m_error_line(-1), m_ntab(8), m_nindent(2),
  ...
  mp_readonly_label->setText(
      QObject::tr("Macro is read-only and cannot be edited"));
...
#if QT_VERSION >= 0x60000
  mp_text->setTabStopDistance(
      m_ntab *
      QFontMetrics(mp_text->font()).horizontalAdvance(QString::fromUtf8("x")));
#else
  mp_text->setTabStopWidth(
      m_ntab * QFontMetrics(mp_text->font()).width(QString::fromUtf8("x")));
#endif
  m_is_modified = false;

  connect(mp_text, SIGNAL(textChanged()), this, SLOT(text_changed()));
  connect(mp_text, SIGNAL(cursorPositionChanged()), this,
          SLOT(cursor_position_changed()));
  connect(mp_text->horizontalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(hide_completer()));
  connect(mp_text->verticalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(hide_completer()));
  connect(mp_exec_model, SIGNAL(breakpoints_changed()), this,
          SLOT(breakpoints_changed()));
  connect(mp_exec_model, SIGNAL(current_line_changed()), this,
          SLOT(current_line_changed()));
  connect(mp_exec_model, SIGNAL(run_mode_changed()), this,
          SLOT(run_mode_changed()));
...
void MacroEditorPage::update() {
  if (mp_macro) {
    ...
    if (mt != et) {
      ...
      //  Leave trailing lines as far as they are identical - that way we deal
      //  with the "header changed" case gracefully and don't destroy
      //  breakpoints if we change macro properties
...
void MacroEditorPage::run_mode_changed() {
  //  this prevents recursion when the following lines trigger anything that
  ...
  try {
    ...
    mp_text->setReadOnly(!mp_macro || mp_macro->is_readonly() ||
...
void MacroEditorPage::breakpoints_changed() {
  ...
  if (!mp_exec_model->breakpoints().empty()) {
    ...
    for (QTextBlock b = doc->begin(); b != doc->end(); b = b.next()) {
      if (mp_exec_model->breakpoints().find(firstLineNumber(b) + 1) !=
          mp_exec_model->breakpoints().end()) {
...
static bool valid_element(const SyntaxHighlighterElement &e) {
  return e.basic_attribute_id != lay::dsComment &&
         e.basic_attribute_id != lay::dsString;
}
...
void MacroEditorPage::fill_completer_list() {
  ...
  while (true) {
    ...
    if (i > 0 &&
        (text[i - 1].isLetterOrNumber() || text[i - 1].toLatin1() == '_')) {
  ...
  for (std::set<QString>::const_iterator w = words.begin(); w != words.end();
       ++w) {
...
void MacroEditorPage::completer_timer() {
  ...
  if (mp_completer_list->count() > 0) {
    ...
    mp_completer_popup->setGeometry(pos.x(), pos.y() + r.height() / 3,
                                    sz.width(),
                                    4 + 4 * fm.height() /*sz.height ()*/);
...
void MacroEditorPage::hide_completer() { mp_completer_popup->hide(); }
...
void MacroEditorPage::cursor_position_changed() {
  ...
  SyntaxHighlighterUserData *user_data =
      dynamic_cast<SyntaxHighlighterUserData *>(b.userData());
  if (user_data) {
    ...
    //  elements not being comment or string. So we need to iterate over
    //  elements and over characters inside these elements.
    ...
    for (e = user_data->elements().begin(); e != user_data->elements().end();
         ++e) {
    ...
    if (e != user_data->elements().end() && valid_element(*e)) {
      ...
      forward =
          (t == open_rbracket || t == open_sqbracket || t == open_cbracket);
    }
    if (e != user_data->elements().begin() &&
        e[-1].start_offset + e[-1].length >= pos && valid_element(e[-1])) {
      QString t = b.text().mid(int(pos) - 1, 1);
      backward =
          (t == close_rbracket || t == close_sqbracket || t == close_cbracket);
    }
    ...
    if (forward || backward) {
      ...
      while (true) {
        ...
        QString t =
            b.text().mid(int(e->start_offset), int(e->length)).trimmed();
      ...
      if (found >= 0) {
        ...
        QList<QTextEdit::ExtraSelection> extra_selections =
            mp_text->extraSelections();
        for (QList<QTextEdit::ExtraSelection>::iterator i =
                 extra_selections.begin();
             i != extra_selections.end();) {
          if (i->format == fmt) {
        ...
        es.cursor.movePosition(QTextCursor::NextCharacter,
...
void MacroEditorPage::text_changed() {
  ...
  std::set<int> bl;
  for (std::set<QTextBlock>::const_iterator b = m_breakpoints.begin();
       b != m_breakpoints.end(); ++b) {
...
void MacroEditorPage::set_ntab(int n) {
  if (n != m_ntab) {
    m_ntab = n;
#if QT_VERSION >= 0x60000
    mp_text->setTabStopDistance(m_ntab *
                                QFontMetrics(mp_text->font())
                                    .horizontalAdvance(QString::fromUtf8("x")));
#else
    mp_text->setTabStopWidth(
        m_ntab * QFontMetrics(mp_text->font()).width(QString::fromUtf8("x")));
#endif
...
void MacroEditorPage::set_nindent(int n) { m_nindent = n; }
...
void MacroEditorPage::connect_macro(lym::Macro *macro) {
  if (mp_macro != macro) {
    ...
    if (mp_macro) {
      ...
      mp_highlighter = mp_highlighters->highlighter_for(
          mp_text, mp_macro->interpreter(), mp_macro->dsl_interpreter(), false);
      if (mp_highlighter) {
    ...
    mp_side_panel->set_watermark(
        mp_macro ? tl::to_qstring(mp_macro->interpreter_name()) : QString());
  }
}

void MacroEditorPage::find_reset() {
  m_ignore_cursor_changed_event = true;
  mp_text->setTextCursor(m_edit_cursor);
  m_ignore_cursor_changed_event = false;
}

bool MacroEditorPage::find_prev() {
  update_extra_selections();
...
bool MacroEditorPage::find_next() {
  update_extra_selections();
...
bool MacroEditorPage::select_match_here() {
  if (m_current_search == QRegExp()) {
    return false;
  }
...
void MacroEditorPage::set_editor_focus() {
...
static QString interpolate_string(const QString &replace, const QRegExp &re) {
  if (re.patternSyntax() == QRegExp::FixedString) {
    ...
    //  TODO: this is simple implementation for the \\ sequence which also
    //  implies that "\ " is an escape sequence
...
void MacroEditorPage::replace_and_find_next(const QString &replace) {
...
void MacroEditorPage::replace_in_selection(const QString &replace, bool first) {
  const QTextDocument *doc = mp_text->document();
...
void MacroEditorPage::set_search(const QRegExp &text) {
  m_current_search = text;
  m_error_line = -1;
  update_extra_selections();
}

void MacroEditorPage::set_error_line(int line) {
  m_error_line = line - 1;
  goto_line(line);
  update_extra_selections();
}

void MacroEditorPage::goto_line(int line) {
  if (line > 0) {
    const QTextDocument *doc = mp_text->document();
    for (QTextBlock b = doc->begin(); b != doc->end(); b = b.next()) {
...
void MacroEditorPage::goto_position(int line, int pos) {
  if (line > 0) {
    const QTextDocument *doc = mp_text->document();
    for (QTextBlock b = doc->begin(); b != doc->end(); b = b.next()) {
      if (firstLineNumber(b) + 1 == line) {
...
void MacroEditorPage::update_extra_selections() {
  QList<QTextEdit::ExtraSelection> extra_selections;
...
int MacroEditorPage::current_line() const {
  return firstLineNumber(mp_text->textCursor().block()) + 1;
}

int MacroEditorPage::current_pos() const {
  return mp_text->textCursor().position() -
         mp_text->textCursor().block().position();
}

bool MacroEditorPage::has_multi_block_selection() const {
  QTextCursor c = mp_text->textCursor();
  if (c.selectionStart() != c.selectionEnd()) {
...
bool MacroEditorPage::tab_key_pressed() {
  ...
  if (indent) {
    ...
    if (adjust_end) {
      ...
      c.setPosition(be.position() + be.text().length(),
                    QTextCursor::KeepAnchor);
    }
    mp_text->setTextCursor(c);
...
bool MacroEditorPage::back_tab_key_pressed() {
  ...
  //  tab in
  QTextBlock bs =
      mp_text->document()->findBlock(mp_text->textCursor().selectionStart());
  QTextBlock be =
      mp_text->document()->findBlock(mp_text->textCursor().selectionEnd());
  bool adjust_end = false;
  if (be != bs) {
    QTextCursor se(mp_text->document());
    se.setPosition(mp_text->textCursor().selectionEnd());
...
bool MacroEditorPage::backspace_pressed() {
  if (mp_text->textCursor().hasSelection() || mp_text->isReadOnly()) {
    return false;
  }
  ...
  if (col > 0) {
    ...
    if (only_space_before) {
      ...
      c.insertText(QString(std::max(0, ((p - 1) / m_nindent) * m_nindent),
                           QChar::fromLatin1(' ')));
      mp_text->setTextCursor(c);
...
bool MacroEditorPage::return_pressed() {
  if (mp_text->isReadOnly()) {
    return false;
  }
...
static bool is_escape_key(QKeyEvent *ke) { return ke->key() == Qt::Key_Escape; }

static bool is_return_key(QKeyEvent *ke) { return ke->key() == Qt::Key_Return; }

static bool is_help_key(QKeyEvent *ke) { return ke->key() == Qt::Key_F1; }

static bool is_find_next_key(QKeyEvent *ke) { return ke->key() == Qt::Key_F3; }
...
static bool is_find_backwards_key(QKeyEvent *ke) {
  return ke->key() == Qt::Key_F &&
         (ke->modifiers() & Qt::ControlModifier) != 0 &&
         (ke->modifiers() & Qt::ShiftModifier) != 0;
}

static bool is_up_key(QKeyEvent *ke) { return ke->key() == Qt::Key_Up; }

static bool is_down_key(QKeyEvent *ke) { return ke->key() == Qt::Key_Down; }

static bool is_any_known_key(QKeyEvent *ke) {
  return is_tab_key(ke) || is_backtab_key(ke) || is_backspace_key(ke) ||
         is_escape_key(ke) || is_return_key(ke) || is_help_key(ke) ||
         is_find_next_key(ke) || is_find_key(ke) || is_find_backwards_key(ke) ||
         is_up_key(ke) || is_down_key(ke);
}

bool MacroEditorPage::eventFilter(QObject *watched, QEvent *event) {
  if (watched == mp_text) {
    ...
    if (event->type() == QEvent::ShortcutOverride) {
      ...
      if (is_tab_key(ke)) {
        ...
        //  Handle Esc to return to the before-find position and clear the
        //  selection or to hide popup
      ...
      } else if (mp_completer_popup->isVisible() &&
                 (is_up_key(ke) || is_down_key(ke))) {
...
void MacroEditorPage::add_notification(
    const MacroEditorNotification &notification) {
  if (m_notification_widgets.find(&notification) ==
      m_notification_widgets.end()) {
    m_notifications.push_back(notification);
    QWidget *w =
        new MacroEditorNotificationWidget(this, &m_notifications.back());
    m_notification_widgets.insert(std::make_pair(&m_notifications.back(), w));
    mp_layout->insertWidget(0, w);
  }
}

void MacroEditorPage::remove_notification(
    const MacroEditorNotification &notification) {
...
void MacroEditorPage::notification_action(
    const MacroEditorNotification &notification, const std::string &action) {
  if (action == "close") {
