...
Salt::Salt(const Salt &other) : QObject() { operator=(other); }
...
SaltGrains &Salt::root() { return m_root; }

Salt::flat_iterator Salt::begin_flat() {
  validate();
  return mp_flat_grains.begin();
}

Salt::flat_iterator Salt::end_flat() {
  validate();
  return mp_flat_grains.end();
}

SaltGrain *Salt::grain_by_name(const std::string &name) {
  validate();
  std::map<std::string, SaltGrain *>::const_iterator g =
      m_grains_by_name.find(name);
...
void Salt::add_location(const std::string &path) {
  ...
  if (path[0] != ':') {
    ...
    for (lay::SaltGrains::collection_iterator g = m_root.begin_collections();
         g != m_root.end_collections(); ++g) {
...
void Salt::add_collection_to_flat(SaltGrains &gg) {
  for (lay::SaltGrains::grain_iterator g = gg.begin_grains();
       g != gg.end_grains(); ++g) {
    //  TODO: get rid of the const cast - would require a non-const grain
    //  iterator
    mp_flat_grains.push_back(const_cast<SaltGrain *>(g.operator->()));
  }
  for (lay::SaltGrains::collection_iterator g = gg.begin_collections();
       g != gg.end_collections(); ++g) {
    //  TODO: get rid of the const cast - would require a non-const grain
    //  collection iterator
...
struct NameAndTopoIndexCompare {
  ...
  bool operator()(lay::SaltGrain *a, lay::SaltGrain *b) const {
    std::map<std::string, int>::const_iterator ti_a =
        mp_topo_index->find(a->name());
    std::map<std::string, int>::const_iterator ti_b =
        mp_topo_index->find(b->name());
...
void Salt::validate() {
  if (mp_flat_grains.empty()) {
    ...
    for (std::vector<SaltGrain *>::const_iterator i = mp_flat_grains.begin();
         i != mp_flat_grains.end(); ++i) {
      m_grains_by_name.insert(std::make_pair((*i)->name(), *i));
    }

    //  Compute a set of topological indexes. Packages which serve dependencies
    //  of other packages have a higher topological index. Later we sort the
    //  packages by descending topo index to ensure the packages which are input
    //  to others come first.

    std::map<std::string, int> topological_index;
    for (std::map<std::string, SaltGrain *>::const_iterator g =
             m_grains_by_name.begin();
         g != m_grains_by_name.end(); ++g) {
      topological_index.insert(std::make_pair(g->first, 0));
    }

    //  NOTE: we allow max. 10 levels of dependencies. That should be
    //  sufficient. Limiting the levels of dependencies prevents infinite
    //  recursion due to faulty recursive dependencies.
    for (int n = 0; n < 10; ++n) {
      ...
      for (std::map<std::string, SaltGrain *>::const_iterator g =
        ...
        for (std::vector<SaltGrainDependency>::const_iterator d =
                 g->second->dependencies().begin();
             d != g->second->dependencies().end(); ++d) {
          std::map<std::string, int>::iterator ti =
              topological_index.find(d->name);
    ...
    //  NOTE: we intentionally sort after the name list has been built - this
    //  way the first entry will win in the name to grain map.
    std::sort(mp_flat_grains.begin(), mp_flat_grains.end(),
              NameAndTopoIndexCompare(topological_index));
  }
}
...
static bool remove_from_collection(SaltGrains &collection,
                                   const std::string &name) {
  bool res = false;

  for (SaltGrains::grain_iterator g = collection.begin_grains();
       g != collection.end_grains(); ++g) {
    if (g->name() == name) {
  ...
  for (SaltGrains::collection_iterator gg = collection.begin_collections();
       gg != collection.end_collections(); ++gg) {
...
bool Salt::remove_grain(const SaltGrain &grain) {
  ...
  try {
    QFile uninstall_lym(
        QDir(tl::to_qstring(grain.path()))
            .absoluteFilePath(tl::to_qstring("_uninstall.lym")));
    ...
    //  Errors in the uninstallation script are only logged, but do not prevent
    //  uninstallation
    tl::error << ex.msg();
  }
...
/**
 *  @brief A helper class required because directory traversal is not supported
 * by QResource directly This class supports resource file trees and extraction
 * of a tree from the latter
 */
class ResourceDir : public QResource {
public:
   ...
   */
  ResourceDir(const QString &path) : QResource(path) {
    //  .. nothing yet ..
  }
  ...
  bool copy_to(const QDir &target) {
    if (isDir()) {
      ...
      for (QStringList::const_iterator t = templ_dir.begin();
           t != templ_dir.end(); ++t) {
        ...
        if (child_res.isFile()) {
          ...
          if (!file.open(QIODevice::WriteOnly)) {
            tl::error << QObject::tr(
                             "Unable to open target file for writing: %1")
                             .arg(target.absoluteFilePath(*t));
            return false;
          }
...
#endif
            data = qUncompress((const unsigned char *)child_res.data(),
                               (int)child_res.size());
          } else {
            data = QByteArray((const char *)child_res.data(),
                              (int)child_res.size());
          }
          ...
          if (!child_dir.exists()) {
            if (!target.mkdir(*t)) {
              tl::error << QObject::tr("Unable to create target directory: %1")
                               .arg(child_dir.path());
              return false;
            }
          } else if (!child_dir.isDir()) {
            tl::error << QObject::tr("Unable to create target directory (is a "
                                     "file already): %1")
                             .arg(child_dir.path());
            return false;
          }
...
bool Salt::create_grain(const SaltGrain &templ, SaltGrain &target,
                        double timeout, tl::InputHttpStreamCallback *callback) {
  ...
  if (!path.empty()) {
    coll = 0;
    for (SaltGrains::collection_iterator gg = m_root.begin_collections();
         gg != m_root.end_collections(); ++gg) {
  ...
  tl::info << QObject::tr("Installing package '%1' ..")
                  .arg(tl::to_qstring(target.name()));

  QDir target_dir(tl::to_qstring(coll->path()));

  try {

    //  change down to the desired target location and create the directory
    //  structure while doing so
    std::vector<std::string> name_parts = tl::split(target.name(), "/");
    for (std::vector<std::string>::const_iterator n = name_parts.begin();
         n != name_parts.end(); ++n) {

      QFileInfo subdir(target_dir.filePath(tl::to_qstring(*n)));
      if (subdir.exists() && !subdir.isDir()) {
        throw tl::Exception(
            tl::to_string(tr("Unable to create target directory '%1' for "
                             "installing package - is already a file")
                              .arg(subdir.path())));
        ...
        if (!target_dir.cd(tl::to_qstring(*n))) {
          throw tl::Exception(
              tl::to_string(tr("Unable to change to target directory '%1' for "
                               "installing package")
                                .arg(subdir.path())));
        }
      } else {
  ...
  if (!templ.path().empty()) {
    ...
    if (templ.path()[0] != ':') {
      ...
      tl::info << QObject::tr("Copying package from '%1' to '%2' ..")
                      .arg(tl::to_qstring(templ.path()))
                      .arg(tl::to_qstring(target.path()));
      ...
      tl::info << QObject::tr(
                      "Installing package from resource '%1' to '%2' ..")
                      .arg(tl::to_qstring(templ.path()))
                      .arg(tl::to_qstring(target.path()));
      res = ResourceDir(tl::to_qstring(templ.path()))
                .copy_to(QDir(tl::to_qstring(target.path())));
    }
    ...
    if (templ.url().find("http:") == 0 || templ.url().find("https:") == 0) {
      ...
      tl::info << QObject::tr("Downloading package from '%1' to '%2' ..")
                      .arg(tl::to_qstring(templ.url()))
                      .arg(tl::to_qstring(target.path()));
      res = tl::WebDAVObject::download(templ.url(), target.path(), timeout,
                                       callback);
  ...
  if (res) {
    ...
    try {
      QFile install_lym(QDir(tl::to_qstring(target.path()))
                            .absoluteFilePath(tl::to_qstring("_install.lym")));
      ...
      //  Errors in the installation script are only logged, but do not prevent
      //  installation
      tl::error << ex.msg();
    }

    tl::info << QObject::tr("Package '%1' installed")
                    .arg(tl::to_qstring(target.name()));

    //  NOTE: this is a bit brute force .. we could as well try to insert the
    //  new grain into the existing structure
    refresh();

  } else {

    tl::warn << QObject::tr(
                    "Failed to install package '%1' - removing files ..")
                    .arg(tl::to_qstring(target.name()));
    if (!tl::rm_dir_recursive(target.path())) {
      tl::warn << QObject::tr("Failed to remove files")
                      .arg(tl::to_qstring(target.name()));
    }
  }
