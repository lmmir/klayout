...
public:
  ...
  /**
   *  @brief Perform all operations on application exit and return true if this
   * is possible
   */
  bool can_exit();
   ...
   */
  lym::Macro *run_macro() const { return mp_run_macro; }

  /**
   *  @brief Returns true while the macro is executing
   */
  bool in_exec() const { return m_in_exec; }
...
protected slots:
  ...
  void clear_log();
  void apply_search() { apply_search(false); }
...
private:
  ...
  void move_subfolder(lym::MacroCollection *source,
  ...
  void trace(gsi::Interpreter *interpreter, size_t file_id, int line,
             const gsi::StackTraceProvider *stack_trace_provider);
  void exception_thrown(gsi::Interpreter *interpreter, size_t file_id, int line,
                        const std::string &eclass, const std::string &emsg,
                        const gsi::StackTraceProvider *stack_trace_provider);
  void enter_exec_mode();
  void leave_exec_mode();
  void
  enter_breakpoint_mode(gsi::Interpreter *interpreter,
                        const gsi::StackTraceProvider *stack_trace_provider);
  ...
  void
  process_events(QEventLoop::ProcessEventsFlags flags = QEventLoop::AllEvents);
  ...
  std::map<std::pair<size_t, int>, std::pair<size_t, int>>
      m_include_file_id_cache;
