...
class FillDialogPluginDeclaration : public lay::PluginDeclaration {
public:
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::menu_item(
        "fill_tool::show", "fill_tool:edit_mode", "edit_menu.utils_menu.end",
        tl::to_string(QObject::tr("Fill Tool"))));
  }
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new FillDialogPluginDeclaration(), 20000, "FillDialogPlugin");

// ------------------------------------------------------------

FillDialog::FillDialog(QWidget *parent, LayoutViewBase *view)
    : QDialog(parent), lay::Plugin(view), Ui::FillDialog(), mp_view(view) {
  ...
  connect(fill_area_cbx, SIGNAL(currentIndexChanged(int)), this,
          SLOT(fill_area_changed(int)));
...
void FillDialog::choose_fc() {
  ...
  if (form.exec()) {
    ...
    fill_cell_le->setText(
        tl::to_qstring(cv->layout().cell_name(cv.cell_index())));
  }
}
...
void FillDialog::generate_fill(const FillParameters &fp) {
  ...
  if (fp.exclude_all_layers) {
    ...
    for (std::vector<db::LayerProperties>::const_iterator l =
             fp.exclude_layers.begin();
         l != fp.exclude_layers.end(); ++l) {
  ...
  db::Coord exclude_x =
      db::coord_traits<db::Coord>::rounded(fp.exclude_distance.x() / ly.dbu());
  db::Coord exclude_y =
      db::coord_traits<db::Coord>::rounded(fp.exclude_distance.y() / ly.dbu());

  db::Coord distance_x =
      db::coord_traits<db::Coord>::rounded(fp.border_distance.x() / ly.dbu());
  db::Coord distance_y =
      db::coord_traits<db::Coord>::rounded(fp.border_distance.y() / ly.dbu());

  db::Vector fill_margin =
      db::CplxTrans(ly.dbu()).inverted() * fp.fill_cell_margin;
  db::Vector fill_margin2 =
      db::CplxTrans(ly.dbu()).inverted() * fp.fill_cell_margin2;

  std::pair<bool, db::cell_index_type> fc =
      cv->layout().cell_by_name(fp.fill_cell_name.c_str());
  if (!fc.first) {
    throw tl::Exception(tl::to_string(QObject::tr("Fill cell not found: ")) +
                        fp.fill_cell_name);
  }
  ...
  if (!fp.fill_cell_name2.empty()) {
    std::pair<bool, db::cell_index_type> fc2 =
        cv->layout().cell_by_name(fp.fill_cell_name2.c_str());
    if (!fc2.first) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Secondary fill cell not found: ")) +
          fp.fill_cell_name2);
    }
  ...
  db::Vector column_step2 =
      db::CplxTrans(ly.dbu()).inverted() * fp.column_step2;
  ...
  if (fp.fill_region_mode == FillParameters::Region) {
    ...
    fill_region = db::Region(
        db::RecursiveShapeIterator(cv->layout(), *cv.cell(), layer_index));
  }
  ...
  if (!fill_region.empty()) {
    ...
    for (std::vector<unsigned int>::const_iterator l = exclude_layers.begin();
         l != exclude_layers.end(); ++l) {

      db::Region exclude(
          db::RecursiveShapeIterator(cv->layout(), *cv.cell(), *l));
      exclude.enable_progress(tl::to_string(tr("Preparing exclude layer: ")) +
                              cv->layout().get_properties(*l).to_string());
    ...
    do {
      ...
      if (!enhanced_fill) {
        db::fill_region(cv.cell(), fill_region, fill_cell->cell_index(),
                        fc_bbox, row_step, column_step, fr_bbox.p1(), false,
                        fill_cell2 ? &fill_region : 0, fill_margin,
                        fill_cell2 ? &fill_region : 0);
      } else {
        db::fill_region_repeat(cv.cell(), fill_region, fill_cell->cell_index(),
                               fc_bbox, row_step, column_step, fill_margin,
                               fill_cell2 ? &fill_region : 0);
      }
...
FillParameters FillDialog::get_fill_parameters() {
  FillParameters fp;
  ...
  if (layer_spec_cbx->currentIndex() == 0) {
    ...
    for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
      ...
      if (!l->has_children() && l->visible(true) &&
          cv->layout().is_valid_layer(l->layer_index())) {
        fp.exclude_layers.push_back(
            cv->layout().get_properties(l->layer_index()));
      }
    }
  ...
  if (fill_area_cbx->currentIndex() == 3) {
    ...
    if (le_x1->text().isEmpty() || le_x2->text().isEmpty() ||
      ...
      throw tl::Exception(tl::to_string(
          QObject::tr("All four coordinates of the fill box must be given")));
    }
    ...
    fp.fill_region.insert(
        db::Box(db::DBox(db::DPoint(x1, y1), db::DPoint(x2, y2)) *
                (1.0 / cv->layout().dbu())));
    ...
    if (ant_service) {
      ...
      while (!ant.at_end()) {
        fp.fill_region.insert(db::Box(db::DBox(ant->p1(), ant->p2()) *
                                      (1.0 / cv->layout().dbu())));
        ++ant;
      }
    }
    ...
    if (sel_layer < 0 || !cv->layout().is_valid_layer(sel_layer)) {
      throw tl::Exception(tl::to_string(
          QObject::tr("No valid layer selected to get fill regions from")));
    }
    ...
    std::vector<edt::Service *> edt_services =
        mp_view->get_plugins<edt::Service>();
    for (std::vector<edt::Service *>::const_iterator s = edt_services.begin();
         s != edt_services.end(); ++s) {
      for (edt::Service::objects::const_iterator sel =
               (*s)->selection().begin();
           sel != (*s)->selection().end(); ++sel) {
        if (!sel->is_cell_inst() &&
            (sel->shape().is_polygon() || sel->shape().is_path() ||
             sel->shape().is_box())) {
          db::Polygon poly;
  ...
  if (!fc.first) {
    throw tl::Exception(tl::to_string(QObject::tr("Fill cell not found: ")) +
                        tl::to_string(fill_cell_le->text()));
  }
  ...
  if (fc_bbox_layer >= 0 && !cv->layout().is_valid_layer(fc_bbox_layer)) {
    throw tl::Exception(tl::to_string(QObject::tr(
        "No valid layer selected to get fill cell's bounding box from")));
  }
  ...
  if (fc_bbox.empty()) {
    if (fc_bbox_layer >= 0) {
      throw tl::Exception(tl::to_string(
          QObject::tr("No valid layer selected to get fill cell's bounding box "
                      "from - layer is empty for the fill cell")));
    } else {
  ...
  if (second_order_fill_cb->isChecked()) {
    ...
    if (!fc.first) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Second order fill cell not found: ")) +
          tl::to_string(fill_cell_2nd_le->text()));
    }

    const db::Cell *fill_cell2 = &cv->layout().cell(fc.second);

    fc_bbox2 = db::CplxTrans(cv->layout().dbu()) *
               (fc_bbox_layer < 0 ? fill_cell2->bbox()
                                  : fill_cell2->bbox(fc_bbox_layer));
    if (fc_bbox2.empty()) {
      throw tl::Exception(tl::to_string(QObject::tr(
          "Second order fill cell is empty for the given boundary layer")));
    }
...
bool FillDialog::configure(const std::string & /*name*/,
