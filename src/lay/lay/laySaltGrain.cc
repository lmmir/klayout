...
SaltGrain::SaltGrain() : m_hidden(false) {
  //  .. nothing yet ..
}

bool SaltGrain::operator==(const SaltGrain &other) const {
  return m_name == other.m_name && m_path == other.m_path &&
         m_version == other.m_version && m_api_version == other.m_api_version &&
         m_url == other.m_url && m_title == other.m_title &&
         m_doc == other.m_doc && m_doc_url == other.m_doc_url &&
         m_icon == other.m_icon && m_screenshot == other.m_screenshot &&
         m_dependencies == other.m_dependencies && m_author == other.m_author &&
         m_author_contact == other.m_author_contact &&
         m_license == other.m_license && m_hidden == other.m_hidden &&
         m_authored_time == other.m_authored_time &&
         m_installed_time == other.m_installed_time;
}

void SaltGrain::set_name(const std::string &n) { m_name = n; }

void SaltGrain::set_token(const std::string &t) { m_token = t; }

void SaltGrain::set_hidden(bool f) { m_hidden = f; }

void SaltGrain::set_version(const std::string &v) { m_version = v; }

void SaltGrain::set_api_version(const std::string &v) { m_api_version = v; }

void SaltGrain::set_path(const std::string &p) { m_path = p; }

void SaltGrain::set_url(const std::string &u) { m_url = u; }

void SaltGrain::set_title(const std::string &t) { m_title = t; }

void SaltGrain::set_doc(const std::string &t) { m_doc = t; }

void SaltGrain::set_doc_url(const std::string &u) { m_doc_url = u; }

std::string SaltGrain::eff_doc_url() const {
  if (m_doc_url.empty()) {
    return std::string();
  }
  ...
  if (!p.isEmpty()) {
    ...
    //  if the URL is a relative URL, make it absolute relative to the grain's
    ...
    if (!dp.isEmpty() &&
        tl::is_parent_path(tl::to_string(p), tl::to_string(dp))) {
...
void SaltGrain::set_author(const std::string &a) { m_author = a; }

void SaltGrain::set_author_contact(const std::string &a) {
  m_author_contact = a;
}

void SaltGrain::set_license(const std::string &l) { m_license = l; }

void SaltGrain::set_authored_time(const QDateTime &t) { m_authored_time = t; }

void SaltGrain::set_installed_time(const QDateTime &t) { m_installed_time = t; }

void SaltGrain::set_screenshot(const QImage &i) { m_screenshot = i; }

void SaltGrain::set_icon(const QImage &i) { m_icon = i; }
...
std::string SaltGrain::spec_url(const std::string &url) {
  std::string res = url;
  if (!res.empty()) {
...
struct ImageConverter {
  ...
  void from_string(const std::string &image, QImage &res) const {
    if (image.empty()) {
      ...
      res = QImage::fromData(
          QByteArray::fromBase64(QByteArray(image.c_str(), int(image.size()))));
    }
  }
};
...
tl::XMLElementList &SaltGrain::xml_elements() {
  if (!sp_xml_elements) {
    sp_xml_elements = new tl::XMLElementList(
        ...
        tl::make_member(&SaltGrain::is_hidden, &SaltGrain::set_hidden,
                        "hidden") +
        tl::make_member(&SaltGrain::version, &SaltGrain::set_version,
                        "version") +
        tl::make_member(&SaltGrain::api_version, &SaltGrain::set_api_version,
                        "api-version") +
        tl::make_member(&SaltGrain::title, &SaltGrain::set_title, "title") +
        tl::make_member(&SaltGrain::doc, &SaltGrain::set_doc, "doc") +
        tl::make_member(&SaltGrain::doc_url, &SaltGrain::set_doc_url,
                        "doc-url") +
        tl::make_member(&SaltGrain::url, &SaltGrain::set_url, "url") +
        tl::make_member(&SaltGrain::license, &SaltGrain::set_license,
                        "license") +
        tl::make_member(&SaltGrain::author, &SaltGrain::set_author, "author") +
        tl::make_member(&SaltGrain::author_contact,
                        &SaltGrain::set_author_contact, "author-contact") +
        tl::make_member(&SaltGrain::authored_time,
                        &SaltGrain::set_authored_time, "authored-time",
                        TimeConverter()) +
        tl::make_member(&SaltGrain::installed_time,
                        &SaltGrain::set_installed_time, "installed-time",
                        TimeConverter()) +
        tl::make_member(&SaltGrain::icon, &SaltGrain::set_icon, "icon",
                        ImageConverter()) +
        tl::make_member(&SaltGrain::screenshot, &SaltGrain::set_screenshot,
                        "screenshot", ImageConverter()) +
        tl::make_element(
            &SaltGrain::begin_dependencies, &SaltGrain::end_dependencies,
            &SaltGrain::add_dependency, "depends",
...
static tl::XMLStruct<lay::SaltGrain> xml_struct() {
  return tl::XMLStruct<lay::SaltGrain>("salt-grain", SaltGrain::xml_elements());
}

bool SaltGrain::is_readonly() const {
  //  A grain is readonly if the directory is not writable or there is a
  //  download URL (this means the grain has been installed from an URL).
...
void SaltGrain::save() const {
  save(tl::to_string(
      QDir(tl::to_qstring(path())).filePath(tl::to_qstring(grain_filename))));
}
...
SaltGrain SaltGrain::from_path(const std::string &path) {
...
tl::InputStream *
SaltGrain::stream_from_url(std::string &url, double timeout,
                           tl::InputHttpStreamCallback *callback) {
  if (url.empty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No download link available")));
  }

  //  base relative URL's on the salt mine URL
  if (url.find("http:") != 0 && url.find("https:") != 0 &&
      url.find("file:") != 0 && !url.empty() && url[0] != '/' &&
      url[0] != '\\' && lay::SaltController::instance()) {

    //  replace the last component ("repository.xml") by the given path
    QUrl sami_url(
        tl::to_qstring(lay::SaltController::instance()->salt_mine_url()));
...
SaltGrain SaltGrain::from_url(const std::string &url_in, double timeout,
                              tl::InputHttpStreamCallback *callback) {
  std::string url = url_in;
  std::unique_ptr<tl::InputStream> stream(
      stream_from_url(url, timeout, callback));
