...
static const std::string
    cfg_tech_editor_window_state("tech-editor-window-state");
...
TechnologyController::TechnologyController()
    : PluginDeclaration(), mp_editor(0), mp_mw(0), mp_dispatcher(0),
      mp_active_technology(0) {
  m_configure_enabled = true;
  m_current_technology_updated = false;
  m_technologies_configured = false;
}

TechnologyController *TechnologyController::instance() {
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
    ...
    TechnologyController *tc =
        dynamic_cast<TechnologyController *>(cls.operator->());
    if (tc) {
      return tc;
    }
  }
...
void TechnologyController::get_options(
    std::vector<std::pair<std::string, std::string>> &options) const {
  options.push_back(
      std::pair<std::string, std::string>(cfg_initial_technology, ""));
  options.push_back(
      std::pair<std::string, std::string>(cfg_tech_editor_window_state, ""));
}

void TechnologyController::get_menu_entries(
    std::vector<lay::MenuEntry> &menu_entries) const {
  lay::PluginDeclaration::get_menu_entries(menu_entries);
  menu_entries.push_back(lay::menu_item(
      "technology_selector:apply_technology",
      "technology_selector:tech_selector_group", "@toolbar.end",
      tl::to_string(QObject::tr(
          "Technology<:techs_24px.png>{Select technology (click to apply)}"))));
}
...
void TechnologyController::view_changed() {
  ...
  db::Technologies::instance()->technology_changed_event.add(
      this, &TechnologyController::technology_changed);
  db::Technologies::instance()->technologies_changed_event.add(
      this, &TechnologyController::technologies_changed);

  if (mp_mw) {

    //  NOTE: the "real" call needs to come before the re-connect handler
    //  because the latter will remove the update call
    mp_mw->current_view_changed_event.add(this,
    ...
    if (mp_mw->current_view()) {
      ...
      //  NOTE: the "real" call needs to come before the re-connect handler
      //  because the latter will remove the update call
      mp_mw->current_view()->active_cellview_changed_event.add(
          this, &TechnologyController::view_changed);

      if (mp_mw->current_view()->active_cellview_index() >= 0 &&
          mp_mw->current_view()->active_cellview_index() <=
              int(mp_mw->current_view()->cellviews())) {
        mp_mw->current_view()->active_cellview()->technology_changed_event.add(
            this, &TechnologyController::update_active_technology);
      }
    }
  }
}

db::Technology *TechnologyController::active_technology() const {
...
void TechnologyController::update_active_technology() {
  ...
  if (mp_mw && mp_mw->current_view() &&
      mp_mw->current_view()->active_cellview_index() >= 0 &&
      mp_mw->current_view()->active_cellview_index() <=
          int(mp_mw->current_view()->cellviews())) {
...
void TechnologyController::technologies_changed() {
  ...
  if (dispatcher) {
    ...
    try {
      dispatcher->config_set(cfg_technologies,
                             db::Technologies::instance()->to_xml());
      m_configure_enabled = true;
    } catch (...) {
      m_configure_enabled = true;
      throw;
...
bool TechnologyController::configure(const std::string &name,
...
bool TechnologyController::menu_activated(const std::string &symbol) const {
  if (symbol == "technology_selector:apply_technology") {
    ...
    if (lay::LayoutView::current() &&
        lay::LayoutView::current()->active_cellview().is_valid()) {

      if (mp_mw) {

        //  apply technology with undo
        mp_mw->manager().transaction(tl::sprintf(
            tl::to_string(tr("Apply technology '%s'")), m_current_technology));
        try {
          lay::LayoutView::current()->active_cellview()->apply_technology(
              m_current_technology);
...
void TechnologyController::update_current_technology(
  ...
  std::vector<std::string> menu_entries =
      dispatcher->menu()->group("tech_selector_group");
  ...
  for (std::map<std::string, const db::Technology *>::const_iterator t =
           tech_by_name.begin();
       t != tech_by_name.end() && it < m_tech_actions.size(); ++t, ++it) {
...
void TechnologyController::update_menu(lay::Dispatcher *dispatcher) {
  ...
  if (lay::LayoutView::current() &&
      lay::LayoutView::current()->active_cellview().is_valid()) {
    m_current_technology =
        lay::LayoutView::current()->active_cellview()->tech_name();
  }
  ...
  std::vector<std::string> tech_group =
      dispatcher->menu()->group("tech_selector_group");

  for (std::vector<std::string>::const_iterator t = tech_group.begin();
       t != tech_group.end(); ++t) {
  ...
  std::map<std::string, std::map<std::string, const db::Technology *>>
      tech_by_name_and_group;
  for (db::Technologies::const_iterator t =
           db::Technologies::instance()->begin();
       t != db::Technologies::instance()->end(); ++t) {
    tech_by_name_and_group[tl::trim(t->group())].insert(
        std::make_pair(t->name(), t.operator->()));
  }

  for (std::vector<std::string>::const_iterator tg = tech_group.begin();
       tg != tech_group.end(); ++tg) {

    int ig = 0;
    for (std::map<std::string,
                  std::map<std::string, const db::Technology *>>::const_iterator
             g = tech_by_name_and_group.begin();
         g != tech_by_name_and_group.end(); ++g) {
      ...
      for (std::map<std::string, const db::Technology *>::const_iterator t =
        ...
        m_tech_actions.push_back(
            new lay::ConfigureAction("", cfg_initial_technology, t->first));
        m_tech_actions.back()->set_title(
            title); // setting the title here avoids interpretation of '(...)'
                    // etc.
        m_tech_actions.back()->set_checkable(true);
        m_tech_actions.back()->set_checked(t->first == m_current_technology);
        dispatcher->menu()->insert_item(tp, "technology_" + tl::to_string(it),
                                        m_tech_actions.back());
      }
    }
  }
...
void TechnologyController::replace_technologies(
    const db::Technologies &technologies) {
  ...
  *db::Technologies().instance() = technologies;
  ...
  if (has_active_tech) {
    mp_active_technology =
        db::Technologies::instance()->technology_by_name(active_tech_name);
  }
}
...
void TechnologyController::show_editor() {
  ...
  if (mp_editor && mp_editor->exec_dialog(new_tech)) {
    ...
    for (db::Technologies::const_iterator t = new_tech.begin();
         t != new_tech.end(); ++t) {
    ...
    for (db::Technologies::const_iterator t =
         ...
         t != db::Technologies::instance()->end(); ++t) {
      if (!t->tech_file_path().empty() && !t->is_persisted() &&
          files_before.find(t->tech_file_path()) == files_before.end()) {
    ...
    for (db::Technologies::const_iterator t =
      ...
      if (!t->tech_file_path().empty() && !t->is_persisted()) {
        ...
        try {
          ...
          QDir dir =
              QFileInfo(tl::to_qstring(t->tech_file_path())).absoluteDir();
    ...
    if (!err_msg.empty()) {
      QMessageBox::critical(
          mp_mw, QObject::tr("Error Saving Technology Files"),
  ...
  mp_dispatcher->config_set(cfg_tech_editor_window_state,
                            lay::save_dialog_state(mp_editor));
}

const std::string &TechnologyController::default_root() {
  tl_assert(!m_paths.empty());
  return m_paths.front();
}

void TechnologyController::load() { rescan(*db::Technologies::instance()); }
...
void TechnologyController::rescan(db::Technologies &technologies) {
  ...
  technologies.clear();
  for (db::Technologies::const_iterator t = current.begin(); t != current.end();
       ++t) {
    if (t->is_persisted()) {
  ...
  if (sc) {
    for (lay::Salt::flat_iterator g = sc->salt().begin_flat();
         g != sc->salt().end_flat(); ++g) {
  ...
  for (std::vector<std::string>::const_iterator p = paths.begin();
    ...
    std::map<std::string, std::string>::const_iterator gn =
        grain_names.find(*p);
    ...
    QDirIterator di(dir.path(), name_filters, QDir::Files,
    ...
    for (QStringList::const_iterator lf = lyt_files.begin();
         lf != lyt_files.end(); ++lf) {
      ...
      try {
        ...
        tl::warn << tl::to_string(
                        QObject::tr("Unable to auto-import technology file "))
                 << tl::to_string(*lf) << ": " << ex.msg();
      }
    }
  }

  for (std::vector<db::Technology>::const_iterator t = m_temp_tech.begin();
       t != m_temp_tech.end(); ++t) {

    db::Technology *tech = new db::Technology(*t);
    if (tl::verbosity() >= 20) {
      tl::info << "Registering special technology from "
               << tech->tech_file_path() << " as " << tech->name();
    }
...
void TechnologyController::add_path(const std::string &p) {
  std::string tp = tl::to_string(
      QDir(tl::to_qstring(p)).filePath(QString::fromUtf8("tech")));
  m_paths.push_back(tp);
}

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new TechnologyController(), 110, "TechnologyController");
