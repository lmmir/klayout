...
static std::string title_for_technology(const db::Technology *t) {
  std::string d;
  if (t->name().empty()) {
    d = t->description();
  } else {
    ...
    if (!t->grain_name().empty()) {
      d += " ";
      d += tl::to_string(
          QObject::tr("[Package %1]").arg(tl::to_qstring(t->grain_name())));
    }
...
TechBaseEditorPage::TechBaseEditorPage(QWidget *parent)
  ...
  connect(mp_ui->browse_lyp_pb, SIGNAL(clicked()), this,
          SLOT(browse_lyp_clicked()));
}
...
void TechBaseEditorPage::setup() {
  ...
  mp_ui->base_path_le->setPlaceholderText(
      tl::to_qstring(tech()->default_base_path()));
#endif
  ...
  if (!tech()->name().empty()) {
    ...
    for (db::LibraryManager::iterator l =
             db::LibraryManager::instance().begin();
    ...
    for (std::vector<std::string>::const_iterator l = libs.begin();
         l != libs.end(); ++l) {
...
void TechBaseEditorPage::browse_clicked() {
  QString p = QFileDialog::getExistingDirectory(
      this, QObject::tr("Choose Base Path"), mp_ui->base_path_le->text());
  if (!p.isNull()) {
...
void TechBaseEditorPage::browse_lyp_clicked() {
  lay::FileDialog open_dialog(
      this, tl::to_string(QObject::tr("Browse Layer Properties File")),
      tl::to_string(QObject::tr("Layer properties files (*.lyp);;Text files "
                                "(*.txt);;All files (*)")));
...
TechLoadOptionsEditorPage::TechLoadOptionsEditorPage(QWidget *parent)
  ...
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator fmt =
       ...
       fmt != tl::Registrar<db::StreamFormatDeclaration>::end(); ++fmt) {

    StreamReaderOptionsPage *page = 0;

    //  obtain the config page from the plugin which we identify by format name
    const StreamReaderPluginDeclaration *decl =
        StreamReaderPluginDeclaration::plugin_for_format(fmt->format_name());
    if (decl) {
      ...
      if (page) {
        ...
        mp_ui->options_tab->addTab(page_host,
                                   tl::to_qstring(fmt->format_desc()));
        any_option = true;
      } else {
        delete page_host;
      }
...
void TechLoadOptionsEditorPage::setup() {
  for (std::vector<std::pair<StreamReaderOptionsPage *, std::string>>::iterator
           page = m_pages.begin();
       page != m_pages.end(); ++page) {
    if (page->first) {
      page->first->setup(
          tech()->load_layout_options().get_options(page->second), tech());
    }
  }
}
...
void TechLoadOptionsEditorPage::commit() {
  ...
  for (std::vector<std::pair<StreamReaderOptionsPage *, std::string>>::iterator
       ...
       page != m_pages.end(); ++page) {
    if (page->first) {
      db::FormatSpecificReaderOptions *specific_options =
          options.get_options(page->second);
      if (!specific_options) {
        specific_options =
            StreamReaderPluginDeclaration::plugin_for_format(page->second)
...
TechSaveOptionsEditorPage::TechSaveOptionsEditorPage(QWidget *parent)
  ...
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator fmt =
    ...
    const StreamWriterPluginDeclaration *decl =
        StreamWriterPluginDeclaration::plugin_for_format(fmt->format_name());
    if (decl) {
      QScrollArea *page_host = new QScrollArea(mp_ui->options_tab);
      page_host->setFrameStyle(QFrame::NoFrame);
      page_host->setWidgetResizable(true);
...
void TechSaveOptionsEditorPage::setup() {
  for (std::vector<std::pair<StreamWriterOptionsPage *, std::string>>::iterator
           page = m_pages.begin();
       page != m_pages.end(); ++page) {
    if (page->first) {
      const db::FormatSpecificWriterOptions *specific_options =
          tech()->save_layout_options().get_options(page->second);
      std::unique_ptr<db::FormatSpecificWriterOptions> default_options;
      if (!specific_options) {
        //  In case there is no option object yet, create a first one for
        //  initialization
        default_options.reset(
            StreamWriterPluginDeclaration::plugin_for_format(page->second)
                ->create_specific_options());
...
void TechSaveOptionsEditorPage::commit() {
  ...
  for (std::vector<std::pair<StreamWriterOptionsPage *, std::string>>::iterator
       ...
       page != m_pages.end(); ++page) {
    if (page->first) {
      db::FormatSpecificWriterOptions *specific_options =
          options.get_options(page->second);
      if (!specific_options) {
...
TechMacrosPage::TechMacrosPage(QWidget *parent, const std::string &cat,
                               const std::string &cat_desc)
    : TechnologyComponentEditor(parent), m_cat(cat), m_cat_desc(cat_desc) {
  mp_ui = new Ui::TechMacrosPage();
  mp_ui->setupUi(this);

  m_original_labels.push_back(
      std::make_pair(mp_ui->title_label, mp_ui->title_label->text()));
  m_original_labels.push_back(
      std::make_pair(mp_ui->note_label, mp_ui->note_label->text()));
  m_original_labels.push_back(
      std::make_pair(mp_ui->empty_label1, mp_ui->empty_label1->text()));
  m_original_labels.push_back(
      std::make_pair(mp_ui->empty_label2, mp_ui->empty_label2->text()));
  m_original_labels.push_back(
      std::make_pair(mp_ui->empty_label3, mp_ui->empty_label3->text()));

  mp_ui->folder_tree->header()->hide();
  connect(mp_ui->folder_tree, SIGNAL(clicked(const QModelIndex &)), this,
          SLOT(macro_selected(const QModelIndex &)));
  ...
  connect(mp_ui->create_folder_button, SIGNAL(clicked()), this,
          SLOT(create_folder_clicked()));
}
...
void TechMacrosPage::setup() {
  ...
  for (lym::MacroCollection::const_child_iterator m = root->begin_children();
       m != root->end_children() && !original; ++m) {
    if (m->second->virtual_mode() == lym::MacroCollection::TechFolder &&
        m->second->category() == m_cat &&
        QDir(tl::to_qstring(m->second->path())).canonicalPath() == cp) {
      original = m->second;
    }
  }
  ...
  //  adjust labels
  for (std::vector<std::pair<QLabel *, QString>>::const_iterator ol =
           m_original_labels.begin();
       ol != m_original_labels.end(); ++ol) {
    QString l = ol->second;
    l.replace(QString::fromUtf8("%CAT%"), tl::to_qstring(m_cat));
    l.replace(QString::fromUtf8("%CAT_DESC%"), tl::to_qstring(m_cat_desc));
    l.replace(QString::fromUtf8("%BASE_PATH%"),
              tl::to_qstring(tech()->base_path()));
    if (alt) {
      l.replace(QString::fromUtf8("%ALT_DESC%"),
                tl::to_qstring(alt->description()));
    } else {
      l.replace(QString::fromUtf8("%ALT_DESC%"),
    ...
    if (!macro_dir.exists()) {
      ...
      if (mp_collection.get() && m_current_path == mp) {
        ...
        if (!original && alt) {
          ...
          //  this can happen, if the macro collection is already there in a
          //  different context. Show a message indicating that
          mp_ui->title_label->hide();
          ...
          if (original) {
            ...
            desc = tl::to_string(QObject::tr("Technology")) + " - " +
                   tech()->name();
          }
          ...
          delete mp_ui->folder_tree->model();
          mp_ui->folder_tree->setModel(
              new lay::MacroTreeModel(this, mc, m_cat));
...
void TechMacrosPage::create_folder_clicked() {
  BEGIN_PROTECTED
  QString macro_dir = QDir(tl::to_qstring(tech()->base_path()))
                          .absoluteFilePath(tl::to_qstring(m_cat));
  if (!QDir::root().mkpath(macro_dir)) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Failed to create folder: ")) +
        tl::to_string(macro_dir));
  }
  setup();
  END_PROTECTED
}
...
void TechMacrosPage::macro_selected(const QModelIndex &index) {
  const lym::Macro *m = 0;
  lay::MacroTreeModel *model =
      dynamic_cast<lay::MacroTreeModel *>(mp_ui->folder_tree->model());
...
TechSetupDialog::TechSetupDialog(QWidget *parent)
    : QDialog(parent), mp_current_tech(0), mp_current_editor(0),
      mp_current_tech_component(0), m_current_tech_changed_enabled(true) {
  ...
  connect(mp_ui->tech_tree,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          this,
          SLOT(current_tech_changed(QTreeWidgetItem *, QTreeWidgetItem *)));
...
void TechSetupDialog::clear_components() {
  for (std::map<std::string, db::TechnologyComponent *>::iterator tc =
           m_technology_components.begin();
       tc != m_technology_components.end(); ++tc) {
    delete tc->second;
  }
  m_technology_components.clear();

  for (std::map<std::string, lay::TechnologyComponentEditor *>::iterator tce =
           m_component_editors.begin();
       tce != m_component_editors.end(); ++tce) {
...
void TechSetupDialog::refresh_clicked() {
  ...
  for (int i = 0; i < mp_ui->tech_tree->topLevelItemCount(); ++i) {
    ...
    QVariant d = item->data(0, Qt::UserRole);
    bool expand =
        (d != QVariant() && expanded_techs.find(tl::to_string(d.toString())) !=
                                expanded_techs.end());
  ...
  if (s_first_show) {
    TipDialog td(this,
                 tl::to_string(QObject::tr(
                     "<html><body>To get started with the technology manager, "
                     "read the documentation provided: <a "
                     "href=\"int:/about/technology_manager.xml\">About "
                     "Technology Management</a>.</body></html>")),
...
void TechSetupDialog::add_clicked() {
  ...
  bool ok = false;
  QString tn = QInputDialog::getText(
      this, QObject::tr("Add Technology"),
      tl::to_qstring(tl::sprintf(
          tl::to_string(QObject::tr(
              "This will create a new technology based on the selected "
              "technology '%s'.\nChoose a name for the new technology.")),
          d)),
  ...
  if (ok && !tn.isEmpty()) {
    ...
    if (m_technologies.has_technology(tl::to_string(tn))) {
      throw tl::Exception(tl::to_string(
          QObject::tr("A technology with this name already exists")));
    }

    QDir root = QDir(
        tl::to_qstring(lay::TechnologyController::instance()->default_root()));
    QDir tech_dir(root.filePath(tn));
    if (tech_dir.exists()) {
      if (QMessageBox::question(
              this, QObject::tr("Creating Technology"),
              QObject::tr("A target folder with path '%1' already exists\nUse "
                          "this directory for the new technology?")
                  .arg(tech_dir.path()),
    ...
    nt->set_tech_file_path(tl::to_string(
        tech_dir.absoluteFilePath(tn + QString::fromUtf8(".lyt"))));
...
void TechSetupDialog::delete_clicked() {
  ...
  if (t->name().empty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("The default technology cannot be deleted")));
  }

  if (t->is_readonly()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("This technology is read-only and cannot be deleted")));
  }

  if (QMessageBox::question(
          this, QObject::tr("Deleting Technology"),
          QObject::tr(
              "Are you sure to delete this technology?\nThis operation cannot "
              "be undone, except by cancelling the technology manager."),
          QMessageBox::No | QMessageBox::Yes) == QMessageBox::Yes) {

    for (db::Technologies::const_iterator i = m_technologies.begin();
         i != m_technologies.end(); ++i) {
...
void TechSetupDialog::rename_clicked() {
  ...
  if (t->name().empty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("The default technology cannot be renamed")));
  }

  if (t->is_readonly()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("This technology is read-only and cannot be renamed")));
  }

  bool ok = false;
  QString tn =
      QInputDialog::getText(this, QObject::tr("Rename Technology"),
                            QObject::tr("Choose a name for the technology"),
                            QLineEdit::Normal, tl::to_qstring(t->name()), &ok);
  if (ok && !tn.isEmpty()) {
    ...
    if (t->name() != tl::to_string(tn)) {
      ...
      if (!t->is_persisted() && !t->tech_file_path().empty()) {
        TipDialog td(
            this,
            tl::to_string(QObject::tr(
                "<html><body>Renaming of a technology will neither rename the "
                "technology file or the folder the file is stored in.<br/>The "
                "file or folder needs to be renamed manually.</body></html>")),
...
void TechSetupDialog::import_clicked() {
  ...
  lay::FileDialog open_dialog(
      this, tl::to_string(QObject::tr("Import Technology")),
      tl::to_string(
          QObject::tr("KLayout technology files (*.lyt);;All files (*)")));
...
void TechSetupDialog::export_clicked() {
  ...
  lay::FileDialog save_dialog(
      this, tl::to_string(QObject::tr("Export Technology")),
      tl::to_string(
          QObject::tr("KLayout technology files (*.lyt);;All files (*)")));
...
void TechSetupDialog::update_tech_tree() {
  ...
  for (db::Technologies::const_iterator t = m_technologies.begin();
       t != m_technologies.end(); ++t) {
    tech_by_name.insert(std::make_pair(t->name(), &*t));
  }

  for (std::map<std::string, const db::Technology *>::const_iterator t =
           tech_by_name.begin();
       t != tech_by_name.end(); ++t) {

    QFont f(mp_ui->tech_tree->font());
    f.setItalic(t->second->is_readonly());

    QTreeWidgetItem *ti = new QTreeWidgetItem(mp_ui->tech_tree);
    ti->setData(0, Qt::DisplayRole,
                QVariant(tl::to_qstring(title_for_technology(t->second))));
    ...
    if (!t->second->tech_file_path().empty()) {
      ti->setData(0, Qt::ToolTipRole,
                  QVariant(tl::to_qstring(t->second->tech_file_path())));
    }

    std::vector<std::string> tc_names = t->second->component_names();
    std::map<std::string, const db::TechnologyComponent *> tc_by_name;
    for (std::vector<std::string>::const_iterator n = tc_names.begin();
         n != tc_names.end(); ++n) {
    ...
    tci->setData(0, Qt::UserRole + 1,
                 QVariant(tl::to_qstring("_load_options")));
    tci->setData(0, Qt::FontRole, QVariant(f));

    tci = new QTreeWidgetItem(ti);
    tci->setData(0, Qt::DisplayRole, QVariant(QObject::tr("Writer Options")));
    tci->setData(0, Qt::UserRole + 1,
                 QVariant(tl::to_qstring("_save_options")));
    tci->setData(0, Qt::FontRole, QVariant(f));

    if (lay::MacroController::instance()) {
      const std::vector<lay::MacroController::MacroCategory> &mc =
          lay::MacroController::instance()->macro_categories();
      for (std::vector<lay::MacroController::MacroCategory>::const_iterator c =
               mc.begin();
           c != mc.end(); ++c) {
        tci = new QTreeWidgetItem(ti);
        tci->setData(0, Qt::DisplayRole,
                     QVariant(tl::to_qstring(c->description)));
        tci->setData(
            0, Qt::UserRole + 1,
            QVariant(tl::to_qstring(std::string("_macros_") + c->name)));
        tci->setData(0, Qt::FontRole, QVariant(f));
      }
    }

    for (std::map<std::string, const db::TechnologyComponent *>::const_iterator
             c = tc_by_name.begin();
         c != tc_by_name.end(); ++c) {
      tci = new QTreeWidgetItem(ti);
      tci->setData(0, Qt::DisplayRole,
                   QVariant(tl::to_qstring(c->second->description())));
...
void TechSetupDialog::update_tech(db::Technology *t) {
  ...
  if (t) {
    ...
    m_component_editors.insert(
        std::make_pair(std::string("_general"), tce_widget));
    ...
    if (lay::MacroController::instance()) {
      ...
      for (std::vector<lay::MacroController::MacroCategory>::const_iterator c =
        ...
        mp_ui->tc_stack->addWidget(tce_widget);
        m_component_editors.insert(
            std::make_pair(std::string("_macros_") + c->name, tce_widget));
      }
    }
    ...
    mp_ui->tc_stack->addWidget(tce_widget);
    m_component_editors.insert(
        std::make_pair(std::string("_load_options"), tce_widget));
    ...
    mp_ui->tc_stack->addWidget(tce_widget);
    m_component_editors.insert(
        std::make_pair(std::string("_save_options"), tce_widget));
    ...
    for (std::vector<std::string>::const_iterator n = tc_names.begin();
      ...
      tce_widget = 0;
      for (tl::Registrar<lay::TechnologyEditorProvider>::iterator cls =
               tl::Registrar<lay::TechnologyEditorProvider>::begin();
           cls != tl::Registrar<lay::TechnologyEditorProvider>::end() &&
           !tce_widget;
           ++cls) {
...
void TechSetupDialog::update_tech_component() {
  ...
  std::map<std::string, lay::TechnologyComponentEditor *>::const_iterator tce =
      m_component_editors.find(tc_name);
  if (tce != m_component_editors.end()) {

    std::map<std::string, db::TechnologyComponent *>::const_iterator tc =
        m_technology_components.find(tc_name);
...
void TechSetupDialog::current_tech_changed(QTreeWidgetItem *current,
...
void TechSetupDialog::commit_tech_component() {
  ...
  if (mp_current_tech && !mp_current_tech->is_readonly()) {
    ...
    //  because commit may have changed the description text, update the
    //  technology titles
    for (int i = mp_ui->tech_tree->topLevelItemCount(); i > 0; --i) {

      QTreeWidgetItem *item = mp_ui->tech_tree->topLevelItem(i - 1);

      db::Technology *t = m_technologies.technology_by_name(
          tl::to_string(item->data(0, Qt::UserRole).toString()));
      item->setData(0, Qt::DisplayRole,
                    QVariant(tl::to_qstring(title_for_technology(t))));
    }
  }
}

std::string TechSetupDialog::selected_tech_component_name() {
...
db::Technology *TechSetupDialog::selected_tech() {
  QTreeWidgetItem *item = mp_ui->tech_tree->currentItem();
  while (item) {
...
TechComponentSetupDialog::TechComponentSetupDialog(
    QWidget *parent, db::Technology *tech, const std::string &component_name)
    : QDialog(parent), mp_tech(tech), mp_component(0), mp_editor(0) {
  ...
  if (tech->name().empty()) {
    setWindowTitle(
        tl::to_qstring(tl::to_string(QObject::tr("Edit Technology")) + " - " +
                       tl::to_string(QObject::tr("(Default)"))));
  } else {
    setWindowTitle(tl::to_qstring(
        tl::to_string(QObject::tr("Edit Technology")) + " - " + tech->name()));
  }

  const db::TechnologyComponent *component =
      tech->component_by_name(component_name);
  if (component) {
