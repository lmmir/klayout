...
static const std::string
    cfg_salt_manager_window_state("salt-manager-window-state");
...
void SaltController::initialized(lay::Dispatcher * /*root*/) {
  ...
  connect(&m_salt, SIGNAL(collections_changed()), this,
          SLOT(emit_salt_changed()));
}

void SaltController::uninitialize(lay::Dispatcher * /*root*/) {
  disconnect(&m_salt, SIGNAL(collections_changed()), this,
             SLOT(emit_salt_changed()));
...
void SaltController::get_options(
    std::vector<std::pair<std::string, std::string>> &options) const {
  options.push_back(
      std::pair<std::string, std::string>(cfg_salt_manager_window_state, ""));
}

void SaltController::get_menu_entries(
    std::vector<lay::MenuEntry> & /*menu_entries*/) const {
  //  .. nothing yet ..
}

bool SaltController::configure(const std::string & /*name*/,
...
void SaltController::show_editor() {
  if (mp_mw && !mp_salt_dialog) {
    ...
    mp_salt_dialog =
        new lay::SaltManagerDialog(mp_mw, &m_salt, m_salt_mine_url);
  }
  ...
  if (mp_salt_dialog) {
    ...
    {
      //  while running the dialog, don't watch file events - that would
      //  interfere with the changes applied by the dialog itself.
      lay::BusySection busy_section; //  disable file watcher
      mp_salt_dialog->exec();
    }

    mp_plugin_root->config_set(cfg_salt_manager_window_state,
                               lay::save_dialog_state(mp_salt_dialog));
...
void SaltController::sync_file_watcher() {
  ...
  if (m_file_watcher) {
    ...
    for (lay::Salt::flat_iterator g = m_salt.begin_flat();
         g != m_salt.end_flat(); ++g) {
...
void SaltController::sync_files() {
  tl::log << tl::to_string(
      tr("Detected file system change in packages - updating"));
  emit_salt_changed();
}

bool SaltController::install_packages(const std::vector<std::string> &packages,
                                      bool with_dep) {
  ...
  for (std::vector<std::string>::const_iterator p = packages.begin();
       p != packages.end(); ++p) {
    ...
    if (n.find("http:") == 0 || n.find("https:") == 0 || n.find("file:") == 0 ||
        n[0] == '/' || n[0] == '\\') {
      //  its a URL
  ...
  {
    //  while running the dialog, don't watch file events - that would interfere
    //  with the changes applied by the dialog itself.
    lay::BusySection busy_section; //  disable file watcher
...
void SaltController::add_path(const std::string &path) {
  try {
    ...
    std::string tp = tl::to_string(
        QDir(tl::to_qstring(path)).filePath(QString::fromUtf8("salt")));

    tl::log << tl::to_string(
        tr("Scanning %1 for packages").arg(tl::to_qstring(tp)));
...
void SaltController::file_watcher_triggered() { dm_sync_files(); }
...
SaltController *SaltController::instance() {
...
static tl::RegisteredClass<lay::PluginDeclaration>
    salt_controller_decl(new lay::SaltController(), 100, "SaltController");
