...
// A workaround for the issue of Qt 4.8.x when handling "File Reference URL" in
// OSX By Kazunari Sekigawa (November 12, 2015) Search down for my name for more
// details!
...
MainWindow *MainWindow::instance() { return mw_instance; }

// -------------------------------------------------------------

static void show_dock_widget(QDockWidget *dock_widget, bool visible) {
  if (visible) {

    dock_widget->show();
    dock_widget->setFocus();

    //  NOTE: this is a clumsy way to make sure the dock widget is made the
    //  current tab if it's in a tabbed dock
    //  TODO: is there a better way to do this?
    QMainWindow *main_window =
        dynamic_cast<QMainWindow *>(dock_widget->parent());
    if (!main_window) {
      return;
    }

    //  Look up all children of the main window and find the QTabBars. These are
    //  the dock tabs (we don't create others). Inside these, look up the right
    //  tab by checking the titles.
    QList<QObject *> mw_children = main_window->children();
    for (QList<QObject *>::const_iterator i = mw_children.begin();
         i != mw_children.end(); ++i) {
...
MainWindow::MainWindow(QApplication *app, const char *name, bool undo_enabled)
    : QMainWindow(0), tl::Object(), lay::DispatcherDelegate(),
      m_dispatcher(this), m_text_progress(this, 10 /*verbosity threshold*/),
      m_mode(std::numeric_limits<unsigned int>::max()), mp_setup_form(0),
      m_open_mode(0), m_keep_backups(0), m_disable_tab_selected(false),
      m_exited(false), dm_do_update_menu(this, &MainWindow::do_update_menu),
      dm_do_update_mru_menus(this, &MainWindow::do_update_mru_menus),
      dm_exit(this, &MainWindow::exit), m_grid_micron(0.001),
      m_default_grids_updated(true), m_new_layout_current_panel(false),
      m_synchronized_views(false), m_synchronous(false), m_busy(false),
      mp_app(app), m_manager(undo_enabled) {
  ...
  if (mw_instance != 0) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Only one instance of MainWindow may be created")));
  }
  mw_instance = this;

  lay::register_help_handler(this, SLOT(show_help(const QString &)),
                             SLOT(show_modal_help(const QString &)));

  mp_setup_form = new SettingsForm(0, dispatcher(), "setup_form"),

  db::LibraryManager::instance().changed_event.add(
      this, &MainWindow::libraries_changed);
  ...
  connect(mp_tab_bar, SIGNAL(currentChanged(int)), this,
          SLOT(view_selected(int)));
#if QT_VERSION >= 0x040500
  mp_tab_bar->setTabsClosable(true);
  connect(mp_tab_bar, SIGNAL(tabCloseRequested(int)), this,
  ...
  connect(mp_hp_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  m_hp_visible = true;
  ...
  connect(mp_libs_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  m_libs_visible = true;
  ...
  connect(mp_eo_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  m_eo_visible = true;
  ...
  connect(mp_bm_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  m_bm_visible = true;
  ...
  mp_layer_toolbox_dock_widget =
      new QDockWidget(QObject::tr("Layer Toolbox"), this);
  mp_layer_toolbox_dock_widget->setObjectName(
      QString::fromUtf8("lt_dock_widget"));
  mp_layer_toolbox_stack = new ControlWidgetStack(mp_layer_toolbox_dock_widget,
                                                  "layer_toolbox_stack", true);
  ...
  connect(mp_layer_toolbox_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  ...
  connect(mp_lp_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  m_lp_visible = true;

  mp_navigator_dock_widget = new QDockWidget(QObject::tr("Navigator"), this);
  mp_navigator_dock_widget->setObjectName(
      QString::fromUtf8("navigator_dock_widget"));
  ...
  connect(mp_navigator_dock_widget, SIGNAL(visibilityChanged(bool)), this,
          SLOT(dock_widget_visibility_changed(bool)));
  ...
  sel_status_icon->setText(
      QString::fromUtf8("<html><b>&nbsp;&nbsp;G</b></html>"));
  ...
  xy_status_icon->setText(
      QString::fromUtf8("<html><b>&nbsp;&nbsp;xy</b></html>"));
  ...
  //  session file dialog
  mp_session_fdia = new lay::FileDialog(
      this, tl::to_string(QObject::tr("Session File")),
      tl::to_string(QObject::tr("Session files (*.lys);;All files (*)")),
      "lys");

  //  bookmarks file dialog
  mp_bookmarks_fdia = new lay::FileDialog(
      this, tl::to_string(QObject::tr("Bookmarks File")),
      tl::to_string(QObject::tr("Bookmark files (*.lyb);;All files (*)")),
      "lyb");
  //  layer properties
  mp_lprops_fdia = new lay::FileDialog(
      this, tl::to_string(QObject::tr("Layer Properties File")),
      tl::to_string(
          QObject::tr("Layer properties files (*.lyp);;All files (*)")),
      "lyp");
  //  screenshots
  mp_screenshot_fdia = new lay::FileDialog(
      this, tl::to_string(QObject::tr("Screenshot")),
      tl::to_string(QObject::tr("PNG files (*.png);;All files (*)")), "png");

  //  layout file dialog
  mp_layout_fdia =
      new lay::FileDialog(this, tl::to_string(QObject::tr("Layout File")),
                          all_layout_file_formats());

  //  save & load layout options
  mp_layout_save_as_options = new lay::SaveLayoutAsOptionsDialog(
      this, tl::to_string(QObject::tr("Save Layout Options")));
  mp_layout_save_options = new lay::SaveLayoutOptionsDialog(
      this, tl::to_string(QObject::tr("Layout Writer Options")));
  mp_layout_load_options = new lay::LoadLayoutOptionsDialog(
      this, tl::to_string(QObject::tr("Layout Reader Options")));
  ...
  connect(&m_file_changed_timer, SIGNAL(timeout()), this,
          SLOT(file_changed_timer()));
  m_file_changed_timer.setSingleShot(true);

  //  install timer for menu update a
  connect(&m_menu_update_timer, SIGNAL(timeout()), this,
          SLOT(update_action_states()));
  m_menu_update_timer.setSingleShot(false);
  m_menu_update_timer.start(200);

  connect(&lay::LayoutHandle::file_watcher(),
          SIGNAL(fileChanged(const QString &)), this,
          SLOT(file_changed(const QString &)));
  connect(&lay::LayoutHandle::file_watcher(),
          SIGNAL(fileRemoved(const QString &)), this,
          SLOT(file_removed(const QString &)));
...
MainWindow::~MainWindow() {
  ...
  //  delete the Menu after the views because they may want to access them in
  //  the destructor
...
std::string MainWindow::all_layout_file_formats() const {
  std::string fmts = tl::to_string(QObject::tr("All layout files ("));
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator rdr =
           tl::Registrar<db::StreamFormatDeclaration>::begin();
       rdr != tl::Registrar<db::StreamFormatDeclaration>::end(); ++rdr) {
...
void MainWindow::init_menu() {
  ...
  if ((lay::ApplicationBase::instance() &&
       lay::ApplicationBase::instance()->is_vo_mode())) {
    std::vector<std::string> hide_vo_grp = menu()->group("hide_vo");
    for (std::vector<std::string>::const_iterator g = hide_vo_grp.begin();
         g != hide_vo_grp.end(); ++g) {
  ...
  //  TODO: later do this on each change of the view - each view might get its
  //  own editable mode
  bool view_mode = (lay::ApplicationBase::instance() &&
                    !lay::ApplicationBase::instance()->is_editable());

  std::vector<std::string> edit_mode_grp = menu()->group("edit_mode");
  for (std::vector<std::string>::const_iterator g = edit_mode_grp.begin();
       g != edit_mode_grp.end(); ++g) {
    menu()->action(*g)->set_visible(!view_mode);
  }

  std::vector<std::string> view_mode_grp = menu()->group("view_mode");
  for (std::vector<std::string>::const_iterator g = view_mode_grp.begin();
       g != view_mode_grp.end(); ++g) {
    menu()->action(*g)->set_visible(view_mode);
  }
}

void MainWindow::dock_widget_visibility_changed(bool visible) {
  if (sender() == mp_lp_dock_widget) {
    dispatcher()->config_set(cfg_show_layer_panel,
                             tl::to_string(!mp_lp_dock_widget->isHidden()));
  } else if (sender() == mp_hp_dock_widget) {
    dispatcher()->config_set(cfg_show_hierarchy_panel,
                             tl::to_string(!mp_hp_dock_widget->isHidden()));
  } else if (sender() == mp_libs_dock_widget) {
    dispatcher()->config_set(cfg_show_libraries_view,
                             tl::to_string(!mp_libs_dock_widget->isHidden()));
  } else if (sender() == mp_bm_dock_widget) {
    dispatcher()->config_set(cfg_show_bookmarks_view,
                             tl::to_string(!mp_bm_dock_widget->isHidden()));
  } else if (sender() == mp_navigator_dock_widget) {
    dispatcher()->config_set(
        cfg_show_navigator,
        tl::to_string(!mp_navigator_dock_widget->isHidden()));
  } else if (sender() == mp_layer_toolbox_dock_widget) {
    dispatcher()->config_set(
        cfg_show_layer_toolbox,
        tl::to_string(!mp_layer_toolbox_dock_widget->isHidden()));
  } else if (sender() == mp_eo_dock_widget) {
    m_eo_visible = visible;
  }
}

void MainWindow::file_changed_timer() {
  ...
  m_changed_files.erase(
      std::unique(m_changed_files.begin(), m_changed_files.end()),
      m_changed_files.end());
  ...
  std::map<QString, std::vector<std::pair<lay::LayoutViewWidget *, int>>>
      views_per_file;

  for (auto v = mp_views.begin(); v != mp_views.end(); ++v) {
    for (int cv = 0; cv < int((*v)->view()->cellviews()); ++cv) {
      views_per_file[tl::to_qstring((*v)->view()->cellview(cv)->filename())]
          .push_back(std::make_pair(*v, cv));
    }
  }
    ...
    if (v != views_per_file.end()) {
      ...
      for (auto w = v->second.begin(); w != v->second.end(); ++w) {
        ...
        if (w->first->view()->cellviews() > 1) {
          title = tl::sprintf(
              tl::to_string(tr("Layout file @%d (%s) has changed on disk")),
              w->second + 1, tl::filename(tl::to_string(*f)));
        } else {
          title = tl::sprintf(
              tl::to_string(tr("Layout file (%s) has changed on disk")),
              tl::filename(tl::to_string(*f)));
        }

        lay::LayoutViewNotification n("reload", title,
                                      tl::Variant(tl::to_string(*f)));
...
void MainWindow::file_removed(const QString & /*path*/) {
  // .. nothing yet ..
}
...
void MainWindow::about_to_exec() {
  ...
  if (!f) {
    TipDialog td(
        ...
        tl::to_string(QObject::tr(
            "<html><body>"
            "<p>With the current settings, only the top cell's content is "
            "shown initially, but the child cells are not drawn.</p>"
            "<p>This can be confusing, since the full layout becomes visible "
            "only after selecting "
            "all hierarchy levels manually.</p>"
            "<p>This setting can be changed now. It can also be changed any "
            "time later using \"File/Setup\", \"Navigation/New Cell\": "
            "\"Select all hierarchy levels\".</p>"
            "<ul>"
            "<li>Press <b>Yes</b> to enable <b>Show full hierarchy</b> mode "
            "now.</li>\n"
            "<li>With <b>No</b>, the mode will remain <b>Show top level "
            "only</b>.</li>"
            "</ul>"
            "</body></html>")),
        "only-top-level-shown-by-default", lay::TipDialog::yesno_buttons);
  ...
  if (lay::ApplicationBase::instance() &&
    ...
    TipDialog td(
        this,
        tl::to_string(QObject::tr(
            "KLayout has been started in viewer mode. In this mode, editor "
            "functions are not available.\n\nTo enable these functions, start "
            "KLayout in editor mode by using the \"-e\" command line switch or "
            "select it as the default mode in the setup dialog. Choose "
            "\"Setup\" in the \"File\" menu and check \"Use editing mode by "
            "default\" on the \"Editing Mode\" page in the \"Application\" "
            "section.")),
        "editor-mode");
    if (td.exec_dialog()) {
      //  Don't bother the user with more dialogs.
  ...
  if (f) {
    TipDialog td(
        this,
        tl::to_string(QObject::tr(
            "Layers are shown without fill because fill has been intentionally "
            "turned off. This can be confusing since selecting a stipple does "
            "not have an effect in this case.\n\nTo turn this feature off, "
            "uncheck \"Show Layers Without Fill\" in the \"View\" menu.")),
        "no-stipple");
    if (td.exec_dialog()) {
      //  Don't bother the user with more dialogs.
      return;
  ...
  if (!f) {
    TipDialog td(
        this,
        tl::to_string(QObject::tr(
            "Markers are not visible because they have been turned off.\nYou "
            "may not see markers when using the marker browser feature.\n\nTo "
            "turn markers on, check \"Show Markers\" in the \"View\" menu.")),
        "show-markers");
    if (td.exec_dialog()) {
      //  Don't bother the user with more dialogs.
      return;
  ...
  if (f) {
    TipDialog td(this,
                 tl::to_string(QObject::tr(
                     "The \"Hide Empty Layers\" feature is enabled. This can "
                     "be confusing, in particular in edit mode, because layers "
                     "are not shown although they are actually present.\n\nTo "
                     "disable this feature, uncheck \"Hide Empty Layers\" in "
                     "the layer panel's context menu.")),
                 "hide-empty-layers");
    if (td.exec_dialog()) {
      //  Don't bother the user with more dialogs.
      return;
...
void MainWindow::tech_message(const std::string &s) {
...
void MainWindow::format_message() {
  ...
  do {
    ...
    bool in_drop = false;
    prev_len =
        ndrop > 0 ? short_message.size() : std::numeric_limits<size_t>::max();
    ...
    ++ndrop;
      ...
      short_message.size() < prev_len &&
      fm_width(fm, QString::fromUtf8(" ") + tl::to_qstring(short_message)) >
          mp_msg_label->width());
...
void MainWindow::message(const std::string &s, int ms) {
  m_message = s;
  format_message();
...
void MainWindow::message_timer() {
  m_message.clear();
  format_message();
}

void MainWindow::config_finalize() {
  ...
  if (m_default_grids_updated) {
    ...
    for (std::vector<std::string>::const_iterator t = group.begin();
         t != group.end(); ++t) {
    ...
    int i = 1;
    for (std::vector<double>::const_iterator g = m_default_grids.begin();
         g != m_default_grids.end(); ++g, ++i) {

      std::string name = "default_grid_" + tl::to_string(i);

      lay::Action *ga = new lay::ConfigureAction(
          tl::to_string(*g) + tl::to_string(QObject::tr(" um")), cfg_grid,
          tl::to_string(*g));
...
bool MainWindow::configure(const std::string &name, const std::string &value) {
  if (name == cfg_grid) {
    ...
    std::vector<std::pair<std::string, bool>> hidden =
        unpack_menu_items_hidden(value);
    apply_hidden(hidden);
    return true;
...
void MainWindow::apply_hidden(
    const std::vector<std::pair<std::string, bool>> &hidden) {
  for (std::vector<std::pair<std::string, bool>>::const_iterator hf =
           hidden.begin();
       hf != hidden.end(); ++hf) {
...
void MainWindow::apply_key_bindings() {
  for (std::vector<std::pair<std::string, std::string>>::const_iterator kb =
           m_key_bindings.begin();
       kb != m_key_bindings.end(); ++kb) {
...
bool MainWindow::edits_enabled() const {
  //  NOTE: "edits_enabled" does not - contrary to the name - indicate that
  //  editing is enabled but that the system is accepting changes of any kind.
  //  Hence if there is no view open, we need to accept changes, otherwise we
  //  cannot open new files.
  return !current_view() || current_view()->edits_enabled();
}

void MainWindow::edits_enabled_changed() {
  bool enable = edits_enabled();

  std::vector<std::string> edit_grp = menu()->group("edit");
  for (std::vector<std::string>::const_iterator g = edit_grp.begin();
       g != edit_grp.end(); ++g) {
    menu()->action(*g)->set_enabled(enable);
  }
}

void MainWindow::menu_needs_update() {
  lay::LayoutView::update_menu(current_view(), *menu());
}

void MainWindow::libraries_changed() {
  //  if the libraries have changed, remove all selections and cancel any
  //  operations to avoid that the view refers to an invalid instance or shape
  for (std::vector<lay::LayoutViewWidget *>::iterator vp = mp_views.begin();
       vp != mp_views.end(); ++vp) {
...
void MainWindow::read_dock_widget_state() {
  dispatcher()->config_set(cfg_show_layer_panel,
                           tl::to_string(!mp_lp_dock_widget->isHidden()));
  dispatcher()->config_set(cfg_show_hierarchy_panel,
                           tl::to_string(!mp_hp_dock_widget->isHidden()));
  ...
  dispatcher()->config_set(cfg_show_bookmarks_view,
                           tl::to_string(!mp_bm_dock_widget->isHidden()));
  dispatcher()->config_set(
      cfg_show_navigator, tl::to_string(!mp_navigator_dock_widget->isHidden()));
  dispatcher()->config_set(
      cfg_show_layer_toolbox,
      tl::to_string(!mp_layer_toolbox_dock_widget->isHidden()));
}

void MainWindow::update_dock_widget_state() {
  if (m_hp_visible) {
    mp_hp_dock_widget->show();
  } else {
    mp_hp_dock_widget->hide();
...
void MainWindow::exit() {
  ...
  //  If there is a operation ongoing, request a break and delay execution of
  //  the exit operation.
  ...
  //  We also don't exit if a dialog is shown (deferred execution may be called
  //  from the dialog's exec loop).
  if (QApplication::activeModalWidget()) {
    dm_exit();
    return;
  }

  //  Only after other operation has finished we ask whether to save and close
  //  eventually
  if (can_close()) {
...
int MainWindow::dirty_files(std::string &dirty_files) {
  int dirty_layouts = 0;
...
bool MainWindow::can_close() {
  if (m_busy) {
    ...
    can_close =
        (QMessageBox::warning(
             this, QObject::tr("Application Busy"),
             QObject::tr("The application is busy.\nYou can close the "
                         "application now, but any unsaved data will be "
  ...
  if (m_always_exit_without_saving || (dirty_layouts == 0)) {
    ...
    mbox.setText(tl::to_qstring(
        tl::to_string(QObject::tr("The following layouts need saving:\n\n")) +
        df_list +
        "\n\nPress 'Exit Without Saving' to exit anyhow and discard changes."));
    mbox.setWindowTitle(QObject::tr("Save Needed"));
    mbox.setIcon(QMessageBox::Warning);
    QAbstractButton *exit_button = mbox.addButton(
        QObject::tr("Exit Without Saving"), QMessageBox::YesRole);
...
void MainWindow::save_state_to_config() {
  //  save the dock widget state with all views closed (that state can be
  //  used for staring klayout without any layout)
  dispatcher()->config_set(cfg_window_geometry,
                           (const char *)saveGeometry().toBase64().data());
  dispatcher()->config_set(cfg_window_state,
                           (const char *)saveState().toBase64().data());
}

void MainWindow::resizeEvent(QResizeEvent *) { format_message(); }
...
void MainWindow::cm_navigator_freeze() {
  if (mp_navigator) {
    mp_navigator->freeze_clicked();
  }
}

void MainWindow::cm_navigator_close() {
  if (mp_navigator) {
    mp_navigator->close();
  }
}

void MainWindow::cm_view_log() { mp_log_viewer_dialog->show(); }

void MainWindow::cm_print() {
  ...
  std::string v =
      std::string(lay::Version::name()) + " " + lay::Version::version();
  ...
  if (print_dialog.exec() == QDialog::Accepted) {
    ...
    if (current_view()) {
      ...
      mp_printer->setResolution(
          int(floor(0.5 + mp_printer->resolution() * rf)));
      ...
      QImage img = current_view()->get_image_with_options(
          page_rect.width(), page_rect.height() - 4 * hh, scale_factor, 1,
          1.0 / scale_factor,
      ...
      painter.drawImage(QPoint(page_rect.left(), page_rect.top() + hh * 2),
                        img);
      ...
      painter.drawText(text_rect, Qt::AlignHCenter | Qt::AlignTop,
                       tl::to_qstring(current_view()->title()));
      painter.drawText(text_rect, Qt::AlignLeft | Qt::AlignBottom,
                       QDateTime::currentDateTime().toString());

      db::DBox vp = current_view()->viewport().box();
      std::string vp_string = "(" + tl::micron_to_string(vp.left()) + ", " +
                              tl::micron_to_string(vp.bottom()) + " ... " +
                              tl::micron_to_string(vp.right()) + ", " +
                              tl::micron_to_string(vp.top()) + ")";
      painter.drawText(text_rect, Qt::AlignRight | Qt::AlignBottom,
                       tl::to_qstring(vp_string));
...
void MainWindow::cm_exit() { exit(); }

lay::LayoutView *MainWindow::view(int index) {
...
const lay::LayoutView *MainWindow::view(int index) const {
  if (index >= 0 && index < int(mp_views.size())) {
    return mp_views[index]->view();
  } else {
    return 0;
...
int MainWindow::index_of(const lay::LayoutView *view) const {
...
int MainWindow::current_view_index() const { return index_of(current_view()); }

lay::LayoutView *MainWindow::current_view() const {
  return lay::LayoutView::current();
}

void MainWindow::select_mode(int m) {
  if (m_mode != m) {
    ...
    //  Update the actions by checking the one that is selected
    //  programmatically. Use the toolbar menu for reference.
    //  TODO: this code needs to be kept aligned with the implementation of
    //  PluginDeclaration::init_menu () It's not easy to move the functionality
    //  to PluginDeclaration because some of the modes are not mapped to
    ...
    if (current_view() && eo_visible) {
      lay::EditorOptionsPages *eo_pages =
          current_view()->editor_options_pages();
...
void MainWindow::cm_goto_position() {
  if (current_view()) {
    ...
    while (true) {
      ...
      std::string pos;
      pos += tl::micron_to_string(box.center().x()) + "," +
             tl::micron_to_string(box.center().y());
      pos += ",";
      pos += tl::micron_to_string(std::min(box.width(), box.height()));

      QString text = QInputDialog::getText(
          this, QObject::tr("Enter Position"),
          QObject::tr("Enter position either as pair (x,y) or with window size "
                      "(x,y,s)\n(x,y) will be the new window center position, "
                      "s (if given) the new window size"),
...
void MainWindow::cm_manage_bookmarks() {
  if (current_view()) {
    current_view()->manage_bookmarks();
  }
}

void MainWindow::cm_bookmark_view() {
  if (current_view()) {
...
void MainWindow::update_action_states() {
  try {
    ...
    if (menu()->is_valid("edit_menu.paste")) {
      ...
      paste_action->set_enabled(!db::Clipboard::instance().empty() &&
                                edits_enabled());
    }

    if (menu()->is_valid("zoom_menu.next_display_state")) {
      Action *next_display_state_action =
          menu()->action("zoom_menu.next_display_state");
      next_display_state_action->set_enabled(has_next_display_state());
    }

    if (menu()->is_valid("zoom_menu.prev_display_state")) {
      Action *prev_display_state_action =
          menu()->action("zoom_menu.prev_display_state");
...
void MainWindow::cm_cancel() { cancel(); }

/**
 *  @brief Commits any operations, cancels any pending edit operations and
 * clears the selection.
 */
void MainWindow::cancel() {
  //  if any transaction is pending (this may happen when an operation threw an
  //  exception) close transactions.
...
void MainWindow::load_layer_properties(const std::string &fn, bool all_views,
                                       bool add_default) {
  if (all_views) {
    for (std::vector<lay::LayoutViewWidget *>::iterator vp = mp_views.begin();
         vp != mp_views.end(); ++vp) {
...
void MainWindow::load_layer_properties(const std::string &fn, int cv_index,
                                       bool all_views, bool add_default) {
  if (all_views) {
    for (std::vector<lay::LayoutViewWidget *>::iterator vp = mp_views.begin();
         vp != mp_views.end(); ++vp) {
...
void MainWindow::cm_save_layer_props() {
  if (current_view()) {
    std::string fn;
    if (mp_lprops_fdia->get_save(
            fn, tl::to_string(QObject::tr("Save Layer Properties File")))) {
      current_view()->save_layer_props(fn);
      add_to_other_mru(fn, cfg_mru_layer_properties);
    }
  } else {
    throw tl::Exception(tl::to_string(
        QObject::tr("No view open to save the layer properties from")));
  }
}

void MainWindow::cm_load_layer_props() {
  if (current_view()) {
    std::string fn;
    if (mp_lprops_fdia->get_open(
            fn, tl::to_string(QObject::tr("Load Layer Properties File")))) {
      load_layer_props_from_file(fn);
      add_to_other_mru(fn, cfg_mru_layer_properties);
    }
  } else {
    throw tl::Exception(tl::to_string(
        QObject::tr("No view open to load the layer properties for")));
  }
}

void MainWindow::load_layer_props_from_file(const std::string &fn) {
  if (!current_view()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("No view open to load the layer properties for")));
  }
  ...
  if (current_view()->cellviews() > 1 &&
      lay::LayoutView::is_single_cv_layer_properties_file(fn)) {
    ...
    for (unsigned int i = 0; i < current_view()->cellviews(); ++i) {
      items << QString(tl::to_qstring(tl::to_string(QObject::tr("Apply to ")) +
                                      current_view()->cellview(i)->name() +
                                      " (@" + tl::to_string(i + 1) + ")"));
    }

    bool ok;
    QString item = QInputDialog::getItem(
        this, QObject::tr("Apply Layer Properties File"),
        QObject::tr(
            "There are multiple layouts in that panel but the layer properties "
            "file contains properties for a single one.\nWhat should be done?"),
        items, 1, false, &ok);
    if (!ok || item.isEmpty()) {
      return;
    }
  ...
  if (target_cv_index > -2) {
    load_layer_properties(fn, target_cv_index, false /*current view only*/,
                          false /*don't add default*/);
  } else {
    load_layer_properties(fn, false /*current view only*/,
                          false /*don't add default*/);
  }
}

void MainWindow::save_session(const std::string &fn) {
...
void MainWindow::restore_session(const std::string &fn) {
  m_current_session = fn;
  lay::Session session;
  session.load(fn);
...
void MainWindow::cm_save_session() {
  std::string df_list;
  int dirty_layouts = dirty_files(df_list);

  if (dirty_layouts == 0 ||
      QMessageBox::warning(
          this, QObject::tr("Save Needed For Some Layouts"),
          tl::to_qstring(tl::to_string(QObject::tr(
                             "The following layouts need saving.\nThese "
                             "layouts must be saved manually:\n\n")) +
                         df_list + "\n\nPress 'Ok' to continue."),
          QMessageBox::Ok | QMessageBox::Cancel,
          QMessageBox::Cancel) == QMessageBox::Ok) {

    std::string fn = m_current_session;
    if (mp_session_fdia->get_save(
            fn, tl::to_string(QObject::tr("Save Session File")))) {
      save_session(fn);
...
void MainWindow::cm_restore_session() {
  std::string fn = m_current_session;
  if (mp_session_fdia->get_open(
          fn, tl::to_string(QObject::tr("Load Session File")))) {
    ...
    if (dirty_layouts == 0) {
      ...
      mbox.setText(tl::to_qstring(
          tl::to_string(QObject::tr("The following layouts need saving:\n\n")) +
          df_list +
          "\n\nPress 'Discard Changes' to close them anyhow and discard "
          "changes."));
      mbox.setWindowTitle(QObject::tr("Save Needed"));
      mbox.setIcon(QMessageBox::Warning);
      QAbstractButton *discard_button =
          mbox.addButton(QObject::tr("Discard Changes"), QMessageBox::YesRole);
      mbox.addButton(QMessageBox::Cancel);
...
void MainWindow::cm_save_bookmarks() {
  if (current_view()) {
    std::string fn;
    if (mp_bookmarks_fdia->get_save(
            fn, tl::to_string(QObject::tr("Save Bookmarks File")))) {
      current_view()->bookmarks().save(fn);
      add_to_other_mru(fn, cfg_mru_bookmarks);
    }
  } else {
    throw tl::Exception(
        tl::to_string(QObject::tr("No view open to save the bookmarks from")));
  }
}

void MainWindow::cm_load_bookmarks() {
  if (current_view()) {
    std::string fn;
    if (mp_bookmarks_fdia->get_open(
            fn, tl::to_string(QObject::tr("Load Bookmarks File")))) {
    ...
    throw tl::Exception(
        tl::to_string(QObject::tr("No view open to load the bookmarks for")));
  }
}

void MainWindow::cm_screenshot() {
  if (current_view()) {
    std::string fn;
    if (mp_screenshot_fdia->get_save(
            fn, tl::to_string(QObject::tr("Save Screenshot")))) {
      current_view()->save_screenshot(fn);
    }
  } else {
    throw tl::Exception(
        tl::to_string(QObject::tr("No view open to create a screenshot from")));
  }
}

void MainWindow::cm_screenshot_to_clipboard() {
  if (current_view()) {
...
void MainWindow::cm_save_current_cell_as() {
  if (current_view()) {
    ...
    if (cv_index >= 0 && cv_index < int(current_view()->cellviews())) {
      ...
      if (!path.empty()) {
        ...
        std::string fn =
            std::string(cv->layout().cell_name(path.back())) + "." + suffix;
        if (mp_layout_fdia->get_save(
                fn, tl::to_string(QObject::tr("Save Layout File")))) {
          ...
          if (mp_layout_save_as_options->get_options(current_view(), cv_index,
                                                     fn, om, options)) {
            ...
            for (std::vector<lay::LayoutView::cell_path_type>::const_iterator
                     p = paths.begin();
                 p != paths.end(); ++p) {
              if (!p->empty()) {
...
void MainWindow::cm_save() { do_save(false); }

void MainWindow::cm_save_as() { do_save(true); }

void MainWindow::do_save(bool as) {
  if (current_view()) {

    std::vector<int> cv_indexes;
    if (current_view()->cellviews() > 1) {
      SelectCellViewForm form(
          0, current_view(),
          tl::to_string(QObject::tr("Select Layout To Save")),
    ...
    if (!cv_indexes.empty()) {
      ...
      std::string fn;
      for (std::vector<int>::const_iterator i = cv_indexes.begin();
           i != cv_indexes.end(); ++i) {
        ...
        if (!(as || fn.empty()) ||
            mp_layout_fdia->get_save(
                fn,
                tl::to_string(
                    tr("Save Layout '%1'").arg(tl::to_qstring(cv->name()))))) {

          //  Here are the options we are going to take:
          //  - if the layout's save options are valid we take the options from
          //  there, otherwise we take the options from the technology
          ...
          if (as && !mp_layout_save_as_options->get_options(
                        current_view(), cv_index, fn, om, options)) {
            break;
          }
...
void MainWindow::cm_save_all() {
  for (int view_index = 0; view_index < int(views()); ++view_index) {

    for (unsigned int cv_index = 0; cv_index < view(view_index)->cellviews();
         ++cv_index) {
      ...
      if (!fn.empty() ||
        ...
        for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
                 tl::Registrar<lay::PluginDeclaration>::begin();
             cls != tl::Registrar<lay::PluginDeclaration>::end(); ++cls) {
          const StreamWriterPluginDeclaration *decl =
              dynamic_cast<const StreamWriterPluginDeclaration *>(&*cls);
          if (decl) {
        ...
        view(view_index)
            ->save_as(cv_index, fn, om, options, true, m_keep_backups);
        add_mru(fn, current_view()->cellview(cv_index)->tech_name());
      }
    }
  }
...
void MainWindow::cm_setup() {
  mp_setup_form->show();
  mp_setup_form->setup();
}

void MainWindow::view_selected(int index) {
  if (index >= 0 && index < int(views())) {

    //  Hint: setting the focus to the tab bar avoids problem with dangling
    //  keyboard focus. Sometimes, the focus was set to the hierarchy level spin
    //  buttons which caught Copy&Paste events in effect.
    mp_tab_bar->setFocus();
...
void MainWindow::select_view(int index) {
...
void MainWindow::cm_open_too() { open(2); }

void MainWindow::cm_open_new_view() { open(1); }

void MainWindow::cm_open() { open(0); }

void MainWindow::cm_pull_in() {
  std::vector<std::string> names;
  lay::LayoutHandle::get_names(names);
  ...
  bool ok = false;
  QString item = QInputDialog::getItem(
      this, QObject::tr("Choose Layout"),
      QObject::tr("Choose an existing layout for being opened in the current "
                  "view\nadditionally to the layouts already shown"),
      layouts, 0, false, &ok);
  if (ok) {

    lay::LayoutHandle *layout_handle =
        lay::LayoutHandle::find(tl::to_string(item));
    if (layout_handle) {
      ...
      if (current_view()) {
        ...
        //  If there is another view holding that layout already, take the layer
        //  properties from there
        int other_cv_index = -1;
        const lay::LayoutView *other_view = 0;
        for (unsigned int i = 0; i < views() && other_cv_index < 0; ++i) {
          for (unsigned int cvi = 0;
               cvi < view(i)->cellviews() && other_cv_index < 0; ++cvi) {
        ...
        if (!other_view) {
          ...
          unsigned int cv_index = current_view()->add_layout(
              layout_handle, true, false /*don't initialize layers*/);
...
void MainWindow::cm_reader_options() {
  mp_layout_load_options->edit_global_options(dispatcher(),
                                              db::Technologies::instance());
}

void MainWindow::cm_writer_options() {
  mp_layout_save_options->edit_global_options(dispatcher(),
                                              db::Technologies::instance());
}

void MainWindow::cm_new_panel() { create_view(); }

void MainWindow::cm_new_layout() {
  ...
  if (dialog.exec_dialog(technology, s_new_cell_cell_name, dbu,
                         s_new_cell_window_size, s_layers,
                         m_new_layout_current_panel)) {

    std::unique_ptr<lay::LayoutHandle> handle(
        new lay::LayoutHandle(new db::Layout(&manager()), std::string()));
    ...
    db::cell_index_type new_ci = handle->layout().add_cell(
        s_new_cell_cell_name.empty() ? 0 : s_new_cell_cell_name.c_str());

    for (std::vector<db::LayerProperties>::const_iterator l = s_layers.begin();
         l != s_layers.end(); ++l) {
      handle->layout().insert_layer(*l);
    }

    lay::LayoutView *mp_view = (m_new_layout_current_panel && current_view())
                                   ? current_view()
                                   : view(create_view());
    ...
    mp_view->zoom_box_and_set_hier_levels(
        db::DBox(-0.5 * s_new_cell_window_size, -0.5 * s_new_cell_window_size,
                 0.5 * s_new_cell_window_size, 0.5 * s_new_cell_window_size),
        std::make_pair(0, 1));
  }
}

void MainWindow::cm_clone() { clone_current_view(); }

void MainWindow::clone_current_view() {
  ...
  //  create a new view
  view_widget =
      new lay::LayoutViewWidget(current_view(), &m_manager,
                                lay::ApplicationBase::instance()->is_editable(),
                                dispatcher(), mp_view_stack);
...
void MainWindow::cm_close_all() {
  interactive_close_view(0, views(), false, false);
}

void MainWindow::cm_close() {
...
void MainWindow::interactive_close_view(int from, int to, bool invert_range,
                                        bool all_cellviews) {
  //  closes views in range [from, to[ (invert_range=false) or outside this
  //  range (invert_range=true)
  if (invert_range || from + 1 != to) {
    ...
    if (dirty_layouts != 0) {
      ...
      mbox.setText(tl::to_qstring(
          tl::to_string(QObject::tr("The following layouts need saving:\n\n")) +
          dirty_files +
          tl::to_string(QObject::tr("\n\nPress 'Close Without Saving' to close "
                                    "them anyway and discard changes\n")))),
          mbox.setWindowTitle(QObject::tr("Save Needed"));
      mbox.setIcon(QMessageBox::Warning);
      QAbstractButton *can_close_button = mbox.addButton(
          QObject::tr("Close Without Saving"), QMessageBox::YesRole);
      mbox.addButton(QMessageBox::Cancel);
    ...
    if (view(from)->cellviews() > 1) {
      ...
      if (all_cellviews) {
        ...
        SelectCellViewForm form(
            0, view(from),
            tl::to_string(QObject::tr("Select Layouts To Close")));
    ...
    if (selected.size() > 0) {
      ...
      for (std::vector<int>::const_iterator i = selected.begin();
           i != selected.end(); ++i) {
        ...
        if (cv->layout().is_editable() && cv->is_dirty()) {
          ...
          for (std::vector<lay::LayoutViewWidget *>::const_iterator v =
                   mp_views.begin();
               v != mp_views.end(); ++v) {
          ...
          for (std::vector<int>::const_iterator ii = selected.begin();
               ii != selected.end(); ++ii) {
          ...
          //  only report layouts as dirty which will vanish if we would close
          //  all layouts
          if (count <= 0) {
            ++dirty_layouts;
            if (dirty_layouts == max_dirty_files) {
      ...
      if (dirty_layouts != 0) {
        ...
        mbox.setText(tl::to_qstring(
            tl::to_string(
                QObject::tr("The following layouts need saving:\n\n")) +
            dirty_files +
            tl::to_string(
                QObject::tr("\n\nPress 'Close Without Saving' to close them "
...
void MainWindow::clone() { clone_current_view(); }

void MainWindow::close_current_view() {
  close_view(index_of(lay::LayoutView::current()));
}

void MainWindow::close_all_views() { cm_close_all(); }

void MainWindow::close_all_except_this() {
...
void MainWindow::close_all_views_left() {
  int index = mp_tab_bar->tabAt(m_mouse_pos);
  if (index >= 0) {
...
void MainWindow::close_all_views_right() {
  int index = mp_tab_bar->tabAt(m_mouse_pos);
  if (index >= 0) {
...
void MainWindow::close_view(int index) {
  if (view(index)) {
    ...
    //  delete the view later as it may still be needed by event handlers or
    //  similar
...
void MainWindow::add_mru(const std::string &fn_rel) {
  add_mru(fn_rel, m_initial_technology);
}

const size_t max_mru = 16; //  TODO: make configurable?

void MainWindow::add_mru(const std::string &fn_rel, const std::string &tech) {
  ...
  for (auto mru = m_mru.begin(); mru != m_mru.end(); ++mru) {
    if (mru->first !=
        fn /* delete non-existing files: && tl::is_readable (mru->first) */) {
...
void MainWindow::add_to_other_mru(const std::string &fn_rel,
  ...
  std::string config_str;
  for (std::vector<std::string>::const_iterator mru = new_mru.begin();
       mru != new_mru.end(); ++mru) {
    if (!config_str.empty()) {
      config_str += " ";
    }
...
class OpenRecentAction : public lay::Action {
public:
  OpenRecentAction(lay::MainWindow *mw, size_t n,
                   void (lay::MainWindow::*open_meth)(size_t),
                   bool (lay::MainWindow::*avail_meth)(size_t))
      : lay::Action(), mp_mw(mw), m_n(n), m_open_meth(open_meth),
        m_avail_meth(avail_meth) {}

  void triggered() { (mp_mw->*m_open_meth)(m_n); }

  bool wants_enabled() const { return (mp_mw->*m_avail_meth)(m_n); }
...
class ClearRecentAction : public lay::Action {
public:
  ...
  void triggered() { dispatcher()->config_set(m_cfg, std::string()); }
...
void MainWindow::do_update_mru_menus() {
  ...
  if (menu()->is_valid(mru_menu)) {
    ...
    if (m_mru.size() > 0 && edits_enabled()) {
      ...
      for (std::vector<std::pair<std::string, std::string>>::iterator mru =
               m_mru.end();
           mru != m_mru.begin();) {
        --mru;
        size_t i = std::distance(m_mru.begin(), mru);
        Action *action =
            new OpenRecentAction(this, i, &lay::MainWindow::open_recent,
                                 &lay::MainWindow::is_available_recent);
        action->set_title(mru->first);
        menu()->insert_item(mru_menu + ".end",
                            tl::sprintf("open_recent_%d", i + 1), action);
      }

      menu()->insert_separator(mru_menu + ".end", "clear_sep");
      menu()->insert_item(mru_menu + ".end", "clear_recent",
                          new ClearRecentAction(this, cfg_mru));
  ...
  if (menu()->is_valid(mru_menu)) {
    ...
    if (m_mru_sessions.size() > 0 && edits_enabled()) {
      ...
      for (std::vector<std::string>::iterator mru = m_mru_sessions.end();
           mru != m_mru_sessions.begin();) {
        --mru;
        size_t i = std::distance(m_mru_sessions.begin(), mru);
        Action *action =
            new OpenRecentAction(this, i, &lay::MainWindow::open_recent_session,
                                 &lay::MainWindow::is_available_recent_session);
      ...
      menu()->insert_separator(mru_menu + ".end", "clear_sep");
      menu()->insert_item(mru_menu + ".end", "clear_recent",
                          new ClearRecentAction(this, cfg_mru_sessions));
  ...
  if (menu()->is_valid(mru_menu)) {
    ...
    if (m_mru_layer_properties.size() > 0 && edits_enabled()) {
      ...
      for (std::vector<std::string>::iterator mru =
               m_mru_layer_properties.end();
           mru != m_mru_layer_properties.begin();) {
        --mru;
        size_t i = std::distance(m_mru_layer_properties.begin(), mru);
        Action *action = new OpenRecentAction(
            this, i, &lay::MainWindow::open_recent_layer_properties,
            &lay::MainWindow::is_available_recent_layer_properties);
        action->set_title(*mru);
        menu()->insert_item(mru_menu + ".end",
                            tl::sprintf("open_recent_%d", i + 1), action);
      }
      ...
      menu()->insert_separator(mru_menu + ".end", "clear_sep");
      menu()->insert_item(
          mru_menu + ".end", "clear_recent",
          new ClearRecentAction(this, cfg_mru_layer_properties));
  ...
  if (menu()->is_valid(mru_menu)) {
    ...
    if (m_mru_bookmarks.size() > 0 && edits_enabled()) {
      ...
      for (std::vector<std::string>::iterator mru = m_mru_bookmarks.end();
           mru != m_mru_bookmarks.begin();) {
        --mru;
        size_t i = std::distance(m_mru_bookmarks.begin(), mru);
        Action *action = new OpenRecentAction(
            this, i, &lay::MainWindow::open_recent_bookmarks,
            &lay::MainWindow::is_available_recent_bookmarks);
        action->set_title(*mru);
        menu()->insert_item(mru_menu + ".end",
                            tl::sprintf("open_recent_%d", i + 1), action);
      }
      ...
      menu()->insert_separator(mru_menu + ".end", "clear_sep");
      menu()->insert_item(mru_menu + ".end", "clear_recent",
                          new ClearRecentAction(this, cfg_mru_bookmarks));
...
void MainWindow::open_recent(size_t n) {
  BEGIN_PROTECTED
  ...
  if (mp_layout_load_options->show_always() &&
      !mp_layout_load_options->edit_global_options(
          dispatcher(), db::Technologies::instance())) {
    return;
  }
  ...
  if (m_open_mode == 0) {
    ...
    if (dirty_layouts != 0) {
      ...
      mbox.setText(tl::to_qstring(
          tl::to_string(QObject::tr("The following layouts need saving:\n\n")) +
          df_list +
          "\n\nPress 'Close Without Saving' to open the layout and discard "
          "changes."));
      mbox.setWindowTitle(QObject::tr("Save Needed"));
      mbox.setIcon(QMessageBox::Warning);
      QAbstractButton *yes_button = mbox.addButton(
          QObject::tr("Close Without Saving"), QMessageBox::YesRole);
      mbox.addButton(QMessageBox::Cancel);
...
static bool is_file_available(const std::string &fn) {
  tl::URI uri(fn);
  if (uri.scheme().empty()) {
...
bool MainWindow::is_available_recent(size_t n) {
  return (n < m_mru.size() && is_file_available(m_mru[n].first));
}

void MainWindow::open_recent_session(size_t n) {
  BEGIN_PROTECTED
...
bool MainWindow::is_available_recent_session(size_t n) {
  return (n < m_mru_sessions.size() && is_file_available(m_mru_sessions[n]));
}

void MainWindow::open_recent_layer_properties(size_t n) {
  BEGIN_PROTECTED
...
bool MainWindow::is_available_recent_layer_properties(size_t n) {
  return (n < m_mru_layer_properties.size() &&
          is_file_available(m_mru_layer_properties[n]));
}

void MainWindow::open_recent_bookmarks(size_t n) {
  BEGIN_PROTECTED
...
bool MainWindow::is_available_recent_bookmarks(size_t n) {
  return (n < m_mru_bookmarks.size() && is_file_available(m_mru_bookmarks[n]));
}

void MainWindow::open(int mode) {
  BEGIN_PROTECTED

  static std::vector<std::string> files;
  if (!mp_layout_fdia->get_open(
          files, std::string(),
          tl::to_string(QObject::tr("Open Layout Files")))) {
    return;
  }
  ...
  if (can_open) {
    ...
    for (std::vector<std::string>::const_iterator fn = files.begin();
         fn != files.end(); ++fn) {
      load_layout(*fn, m_initial_technology, mode);
      //  open next layout in "add view" mode, if the current view was
      //  overridden - otherwise that would happen once again.
      if (mode == 0) {
        mode = 1;
      }
...
void MainWindow::reload_layout(unsigned int cv_index) {
...
lay::CellViewRef MainWindow::load_layout(const std::string &filename,
...
lay::CellViewRef MainWindow::create_layout(const std::string &technology,
                                           int mode) {
  return create_or_load_layout(0, 0, technology, mode);
}

void MainWindow::add_view(lay::LayoutViewWidget *view) {
  connect(view, SIGNAL(title_changed(lay::LayoutView *)), this,
          SLOT(view_title_changed(lay::LayoutView *)));
  connect(view, SIGNAL(dirty_changed(lay::LayoutView *)), this,
          SLOT(view_title_changed(lay::LayoutView *)));
  connect(view, SIGNAL(edits_enabled_changed()), this,
          SLOT(edits_enabled_changed()));
  connect(view, SIGNAL(menu_needs_update()), this, SLOT(menu_needs_update()));
  connect(view, SIGNAL(show_message(const std::string &, int)), this,
          SLOT(message(const std::string &, int)));
  connect(view, SIGNAL(current_pos_changed(double, double, bool)), this,
          SLOT(current_pos(double, double, bool)));
...
int MainWindow::do_create_view() {
  //  create a new view
  lay::LayoutViewWidget *view_widget = new lay::LayoutViewWidget(
      &m_manager, lay::ApplicationBase::instance()->is_editable(), dispatcher(),
      mp_view_stack);
  add_view(view_widget);
...
int MainWindow::create_view() {
  ...
  m_disable_tab_selected = true;
  int index =
      mp_tab_bar->insertTab(-1, tl::to_qstring(current_view()->title()));
  m_disable_tab_selected = f;
...
lay::CellViewRef
MainWindow::create_or_load_layout(const std::string *filename,
                                  const db::LoadLayoutOptions *options,
                                  const std::string &technology, int mode) {
  lay::LayoutView *vw = 0;
  ...
  try {
    ...
    if (mode == 1) {
      ...
      mp_view_stack->add_widget(mp_views.back());
      mp_lp_stack->add_widget(mp_views.back()->layer_control_frame());
      mp_layer_toolbox_stack->add_widget(
          mp_views.back()->layer_toolbox_frame());
      mp_hp_stack->add_widget(mp_views.back()->hierarchy_control_frame());
      mp_libs_stack->add_widget(mp_views.back()->libraries_frame());
      mp_eo_stack->add_widget(mp_views.back()->editor_options_frame());
      mp_bm_stack->add_widget(mp_views.back()->bookmarks_frame());
...
void MainWindow::update_tab_title(int i) {
  std::string title;
...
void MainWindow::view_title_changed(lay::LayoutView *view) {
  int i = index_of(view);
  if (i >= 0) {
    update_tab_title(i);
  }
...
void MainWindow::update_window_title() {
  if (current_view()) {
    ...
    setWindowTitle(tl::to_qstring(lay::ApplicationBase::version() + sep +
                                  current_view()->title()));
  } else {
...
double MainWindow::grid_micron() const { return m_grid_micron; }

void MainWindow::set_hier_levels(std::pair<int, int> l) {
  if (current_view() && l != get_hier_levels()) {
    current_view()->set_hier_levels(l);
  }
}

std::pair<int, int> MainWindow::get_hier_levels() const {
  if (current_view()) {
...
bool MainWindow::has_prev_display_state() {
  if (current_view()) {
...
bool MainWindow::has_next_display_state() {
  if (current_view()) {
...
void MainWindow::set_synchronous(bool sync_mode) {
...
void MainWindow::current_pos(double x, double y, bool dbu_units) {
  mp_cpx_label->setText(tl::to_qstring(dbu_units ? tl::db_to_string(x)
                                                 : tl::micron_to_string(x)));
  mp_cpy_label->setText(tl::to_qstring(dbu_units ? tl::db_to_string(y)
                                                 : tl::micron_to_string(y)));
}

void MainWindow::clear_current_pos() {
  mp_cpx_label->setText(QString());
  mp_cpy_label->setText(QString());
}

QWidget *MainWindow::progress_get_widget() const {
...
bool MainWindow::update_progress(tl::Progress *progress) {
  if (mp_progress_dialog) {
...
bool MainWindow::progress_wants_widget() const { return true; }

void MainWindow::progress_add_widget(QWidget *widget) {
  if (mp_progress_dialog) {
...
void MainWindow::progress_remove_widget() {
  if (mp_progress_dialog) {
...
bool MainWindow::show_progress_bar(bool show) {
  if (!isVisible()) {
...
void MainWindow::cm_technologies() {
...
void MainWindow::show_macro_editor(const std::string &cat, bool add) {
...
void MainWindow::show_help(const QString &url) {
  //  NOTE: from inside a modal widget we show the help dialog modal too
  //  (otherwise it's not usable)
  show_assistant_url(tl::to_string(url),
                     QApplication::activeModalWidget() != 0);
}

void MainWindow::show_modal_help(const QString &url) {
  show_assistant_url(tl::to_string(url), true);
}

void MainWindow::show_assistant_url(const std::string &url, bool modal) {
  if (modal) {

    lay::HelpDialog dialog(
        QApplication::activeWindow() ? QApplication::activeWindow() : this,
        true);
    dialog.show(); //  TODO: this is required to establish a proper geometry.
                   //  Without this, the splitter is not set up correctly.
    dialog.load(url);
    dialog.exec();
...
void MainWindow::show_assistant_topic(const std::string &s, bool modal) {
  if (modal) {
...
void MainWindow::cm_show_all_tips() {
  dispatcher()->config_set(cfg_tip_window_hidden, "");
}

void MainWindow::cm_help_about() {
  HelpAboutDialog help_about_dialog(this);
  help_about_dialog.exec();
}

void MainWindow::cm_help_about_qt() { QApplication::aboutQt(); }

std::vector<std::string> MainWindow::menu_symbols() {
  //  TODO: currently these are all symbols from all plugins
  return lay::PluginDeclaration::menu_symbols();
}

void MainWindow::menu_activated(const std::string &symbol) {
  if (symbol == "cm_navigator_freeze") {
    ...
    //  TODO: this can be part of the Plugin scheme, but the plugin root has no
    //  idea which is the active view.
    if (current_view()) {
      current_view()->menu_activated(symbol);
    } else {
      throw tl::Exception(tl::to_string(
          QObject::tr("This function needs a layout but none was available")));
    }
  }
}

void MainWindow::menu_changed() {
  //  delay actual rebuilding of the menu to collect multiple change events.
  dm_do_update_menu();
}

void MainWindow::do_update_menu() {
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event) {
  //  spy on the mouse events of the tab bar so we can tell which tab the menu
  //  was issued on
...
void MainWindow::dropEvent(QDropEvent *event) {
  ...
  if (event->mimeData() && event->mimeData()->hasUrls()) {
    ...
    for (QList<QUrl>::const_iterator url = urls.begin(); url != urls.end();
         ++url) {
      ...
      if (eff_url.scheme() == QString::fromUtf8("file")) {
        ...
        //
        // [Issue]
        //   When drag & dropping a GDS2/OASIS file from Finder, an error like
        //   below flags on:
        //     Unable to open file: /.file/id=6571367.1783076 (errno=20)
        //   http://klayout.de/forum/comments.php?DiscussionID=733&page=1#Item_0
        //
        //   Such a URL is called "File Reference URL" in OSX and iOS
        //   terminology. This has to be converted back to an ordinary full
        //   path. But due to a bug in Qt-4.8.x, this conversion fails.
        //
        // [Refs for workaround]
        //   https://bugreports.qt.io/browse/QTBUG-40449
        //   Sub: OS X Yosemite drag and drop file QUrl in this format:
        //   "file:///.file/id=......"
        //----------------------------------------------------------------------------------------
        // By Kazunari Sekigawa (December 12, 2017)
        //
        // This bug has been fixed in Qt 5.4.1.
        // When KLayout 0.25 is built with Qt 5.8.0 or later, this workaround is
        // not required.
        //----------------------------------------------------------------------------------------
        ...
        if (path.startsWith(keystring)) {
          CFStringRef relCFStringRef = CFStringCreateWithCString(
              kCFAllocatorDefault, path.toUtf8().constData(),
      ...
      } else if (eff_url.scheme() == QString::fromUtf8("http") ||
                 eff_url.scheme() == QString::fromUtf8("https")) {
      ...
      for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
               tl::Registrar<lay::PluginDeclaration>::begin();
           cls != tl::Registrar<lay::PluginDeclaration>::end(); ++cls) {
        lay::PluginDeclaration *pd =
            const_cast<lay::PluginDeclaration *>(&*cls);
      ...
      if (current_view() &&
          current_view()->accepts_drop(tl::to_string(eff_url.toString()))) {
      ...
      if (suffix == QString::fromUtf8("lyp")) {
        ...
        load_layer_properties(tl::to_string(path), false /*current view only*/,
                              false /*don't add a default*/);
            ...
            if (dirty_layouts != 0) {
              ...
              mbox.setText(tl::to_qstring(
                  tl::to_string(
                      QObject::tr("The following layouts need saving:\n\n")) +
                  df_list +
                  "\n\nPress 'Close Without Saving' to open the layout and "
                  "discard changes."));
              mbox.setWindowTitle(QObject::tr("Save Needed"));
              mbox.setIcon(QMessageBox::Warning);
              QAbstractButton *yes_button = mbox.addButton(
...
void MainWindow::plugin_registered(lay::PluginDeclaration *cls) {
...
void MainWindow::plugin_removed(lay::PluginDeclaration *cls) {
...
class MainWindowPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::submenu("file_menu", at, tl::to_string(QObject::tr("&File"))));
    menu_entries.push_back(
        lay::submenu("edit_menu", at, tl::to_string(QObject::tr("&Edit"))));
    menu_entries.push_back(
        lay::submenu("view_menu", at, tl::to_string(QObject::tr("&View"))));
    menu_entries.push_back(lay::submenu(
        "bookmark_menu", at, tl::to_string(QObject::tr("&Bookmarks"))));
    menu_entries.push_back(
        lay::submenu("zoom_menu", at, tl::to_string(QObject::tr("&Display"))));
    menu_entries.push_back(
        lay::submenu("tools_menu", at, tl::to_string(QObject::tr("&Tools"))));
    menu_entries.push_back(
        lay::submenu("macros_menu", at, tl::to_string(QObject::tr("&Macros"))));
    menu_entries.push_back(lay::separator("help_group", at));
    menu_entries.push_back(
        lay::submenu("help_menu", at, tl::to_string(QObject::tr("&Help"))));
    menu_entries.push_back(lay::submenu(
        "@secrets", at, tl::to_string(QObject::tr("Secret Features"))));
    menu_entries.push_back(lay::submenu("@toolbar", at, std::string()));

    at = "edit_menu.end";
    menu_entries.push_back(
        lay::separator("edit_options_group:edit_mode", "edit_menu.end"));
    menu_entries.push_back(lay::menu_item(
        "cm_edit_options", "edit_options:edit_mode", "edit_menu.end",
        tl::to_string(QObject::tr("Editor Options")) + "(F3)"));

    at = "file_menu.end";
    menu_entries.push_back(
        lay::menu_item("cm_new_layout", "new_layout:edit:edit_mode", at,
                       tl::to_string(QObject::tr("New Layout"))));
    menu_entries.push_back(
        lay::menu_item("cm_new_panel", "new_panel:edit:edit_mode", at,
                       tl::to_string(QObject::tr("New Panel"))));
    menu_entries.push_back(lay::separator("post_new_group:edit_mode", at));
    menu_entries.push_back(lay::menu_item("cm_open", "open:edit", at,
                                          tl::to_string(QObject::tr("Open"))));
    menu_entries.push_back(lay::menu_item(
        "cm_open_too", "open_same_panel:edit", at,
        tl::to_string(QObject::tr("Open In Same Panel(Shift+Ctrl+O)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_open_new_view", "open_new_panel", at,
        tl::to_string(QObject::tr("Open In New Panel(Ctrl+O)"))));
    menu_entries.push_back(
        lay::menu_item("cm_close", "close:edit", at,
                       tl::to_string(QObject::tr("Close(Ctrl+W)"))));
    menu_entries.push_back(
        lay::menu_item("cm_close_all", "close_all:edit", at,
                       tl::to_string(QObject::tr("Close All(Shift+Ctrl+W)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_clone", "clone", at, tl::to_string(QObject::tr("Clone Panel"))));
    menu_entries.push_back(
        lay::menu_item("cm_reload", "reload:edit", at,
                       tl::to_string(QObject::tr("Reload(Ctrl+R)"))));
    menu_entries.push_back(
        lay::menu_item("cm_pull_in", "pull_in:edit", at,
                       tl::to_string(QObject::tr("Pull In Other Layout"))));
    menu_entries.push_back(
        lay::menu_item("cm_reader_options", "reader_options", at,
                       tl::to_string(QObject::tr("Reader Options"))));
    menu_entries.push_back(lay::separator("open_recent_group", at));
    menu_entries.push_back(
        lay::submenu("open_recent_menu:edit", at,
                     tl::to_string(QObject::tr("Open Recent"))));
    menu_entries.push_back(lay::separator("import_group", at));
    menu_entries.push_back(lay::submenu("import_menu:edit", at,
                                        tl::to_string(QObject::tr("Import"))));
    menu_entries.push_back(lay::separator("save_group", at));
    menu_entries.push_back(lay::menu_item("cm_save", "save:hide_vo", at,
                                          tl::to_string(QObject::tr("Save"))));
    menu_entries.push_back(
        lay::menu_item("cm_save_as", "save_as:hide_vo", at,
                       tl::to_string(QObject::tr("Save As"))));
    menu_entries.push_back(
        lay::menu_item("cm_save_all", "save_all:hide_vo", at,
                       tl::to_string(QObject::tr("Save All"))));
    menu_entries.push_back(
        lay::menu_item("cm_writer_options", "writer_options:hide_vo", at,
                       tl::to_string(QObject::tr("Writer Options"))));
    menu_entries.push_back(lay::separator("setup_group", at));
    menu_entries.push_back(lay::menu_item("cm_setup", "setup:edit", at,
                                          tl::to_string(QObject::tr("Setup"))));
    menu_entries.push_back(lay::separator("misc_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_screenshot", "screenshot:edit", at,
                       tl::to_string(QObject::tr("Screenshot(Print)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_screenshot_to_clipboard", "screenshot_to_clipboard:edit", at,
        tl::to_string(QObject::tr("Screenshot to clipboard"))));
    menu_entries.push_back(
        lay::menu_item("cm_layout_props", "layout_props:edit", at,
                       tl::to_string(QObject::tr("Layout Properties"))));
    menu_entries.push_back(
        lay::menu_item("cm_layout_stats", "layout_stats:edit", at,
                       tl::to_string(QObject::tr("Layout Statistics"))));
    menu_entries.push_back(lay::separator("layer_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_load_layer_props", "load_layer_props:edit", at,
                       tl::to_string(QObject::tr("Load Layer Properties"))));
    menu_entries.push_back(
        lay::menu_item("cm_save_layer_props", "save_layer_props:edit", at,
                       tl::to_string(QObject::tr("Save Layer Properties"))));
    menu_entries.push_back(
        lay::submenu("open_recent_menu_layer_props:edit", at,
                     tl::to_string(QObject::tr("Recent Layer Properties"))));
    menu_entries.push_back(lay::separator("session_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_restore_session", "restore_session:edit", at,
                       tl::to_string(QObject::tr("Restore Session"))));
    menu_entries.push_back(
        lay::menu_item("cm_save_session", "save_session", at,
                       tl::to_string(QObject::tr("Save Session"))));
    menu_entries.push_back(
        lay::submenu("open_recent_menu_sessions:edit", at,
                     tl::to_string(QObject::tr("Recent Sessions"))));
    menu_entries.push_back(lay::separator("log_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_view_log", "view_log", at,
                       tl::to_string(QObject::tr("Log Viewer"))));
    menu_entries.push_back(lay::separator("print_group", at));
    menu_entries.push_back(lay::menu_item(
        "cm_print", "print", at, tl::to_string(QObject::tr("Print(Ctrl+P)"))));
    menu_entries.push_back(lay::separator("exit_group", at));
    menu_entries.push_back(lay::menu_item(
        "cm_exit", "exit", at, tl::to_string(QObject::tr("Exit(Ctrl+Q)"))));

    at = "view_menu.end";
    menu_entries.push_back(lay::config_menu_item(
        "show_grid", at, tl::to_string(QObject::tr("Show Grid")),
        cfg_grid_visible, "?")),
        menu_entries.push_back(
            lay::submenu("default_grid:default_grids_group", at,
                         tl::to_string(QObject::tr("Grid"))));
    menu_entries.push_back(lay::separator("layout_group", at));
    menu_entries.push_back(lay::config_menu_item(
        "show_markers", at, tl::to_string(QObject::tr("Show Markers")),
        cfg_markers_visible, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_texts", at, tl::to_string(QObject::tr("Show Texts")),
        cfg_text_visible, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_cell_boxes", at, tl::to_string(QObject::tr("Show Cell Frames")),
        cfg_cell_box_visible, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "no_stipples", at,
        tl::to_string(QObject::tr("Show Layers Without Fill")), cfg_no_stipple,
        "?"));
    menu_entries.push_back(
        lay::config_menu_item("synchronized_views", at,
                              tl::to_string(QObject::tr("Synchronized Views")),
                              cfg_synchronized_views, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "edit_top_level_selection:edit_mode", at,
        tl::to_string(QObject::tr("Select Top Level Objects")),
        edt::cfg_edit_top_level_selection, "?"));
    menu_entries.push_back(lay::separator("panels_group", at));
    menu_entries.push_back(lay::config_menu_item(
        "show_toolbar", at, tl::to_string(QObject::tr("Toolbar")),
        cfg_show_toolbar, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_navigator", at, tl::to_string(QObject::tr("Navigator")),
        cfg_show_navigator, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_layer_panel", at, tl::to_string(QObject::tr("Layers")),
        cfg_show_layer_panel, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_layer_toolbox", at, tl::to_string(QObject::tr("Layer Toolbox")),
        cfg_show_layer_toolbox, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_hierarchy_panel", at, tl::to_string(QObject::tr("Cells")),
        cfg_show_hierarchy_panel, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_libraries_view", at, tl::to_string(QObject::tr("Libraries")),
        cfg_show_libraries_view, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "show_bookmarks_view", at, tl::to_string(QObject::tr("Bookmarks")),
        cfg_show_bookmarks_view, "?"));
    menu_entries.push_back(
        lay::menu_item("cm_reset_window_state", "reset_window_state", at,
                       tl::to_string(QObject::tr("Restore Window")))),
        menu_entries.push_back(lay::separator("selection_group", at));
    menu_entries.push_back(lay::config_menu_item(
        "transient_selection", at,
        tl::to_string(QObject::tr("Highlight Object Under Mouse")),
        cfg_sel_transient_mode, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "mouse_tracking", at, tl::to_string(QObject::tr("Mouse tracking")),
        cfg_tracking_cursor_enabled, "?"));

    at = "help_menu.end";
    menu_entries.push_back(
        lay::menu_item("cm_show_all_tips", "show_all_tips", at,
                       tl::to_string(QObject::tr("Show All Tips"))));
    menu_entries.push_back(lay::separator("help_topics_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_show_assistant", "assistant", at,
                       tl::to_string(QObject::tr("Assistant"))));
    menu_entries.push_back(lay::menu_item("cm_help_about", "about", at,
                                          tl::to_string(QObject::tr("About"))));
    menu_entries.push_back(
        lay::menu_item("cm_help_about_qt", "about_qt", at,
                       tl::to_string(QObject::tr("About Qt"))));

    at = "tools_menu.end";
    menu_entries.push_back(
        lay::menu_item("cm_packages", "packages", at,
                       tl::to_string(QObject::tr("Manage Packages"))));
    menu_entries.push_back(
        lay::menu_item("cm_technologies", "technologies", at,
                       tl::to_string(QObject::tr("Manage Technologies"))));
    ...
    at = "macros_menu.end";
    menu_entries.push_back(
        lay::menu_item("cm_macro_editor", "macro_development", at,
                       tl::to_string(QObject::tr("Macro Development(F5)"))));
    menu_entries.push_back(lay::separator("macros_group", at));

    at = "@toolbar.end";
    menu_entries.push_back(
        lay::menu_item("cm_prev_display_state", "prev_display_state", at,
                       tl::to_string(QObject::tr("Back<:/back_24px.png>"))));
    menu_entries.push_back(lay::menu_item(
        "cm_next_display_state", "next_display_state", at,
        tl::to_string(QObject::tr("Forward<:/forward_24px.png>"))));
    menu_entries.push_back(lay::separator("toolbar_post_navigation_group", at));
  }
};

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new MainWindowPluginDeclaration(), -100, "MainWindowPlugin");
