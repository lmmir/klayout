...
MacroController::MacroController()
    : mp_macro_editor(0), mp_mw(0), m_no_implicit_macros(false),
      ...
      dm_do_update_menu_with_macros(
          this, &MacroController::do_update_menu_with_macros),
      dm_do_sync_with_external_sources(
          this, &MacroController::do_sync_with_external_sources),
      dm_sync_file_watcher(this, &MacroController::sync_file_watcher),
      dm_sync_files(this, &MacroController::sync_files) {
  //  .. nothing yet ..
}

void MacroController::add_macro_category(
...
void MacroController::finish() {
  ...
  //  These macros are always taken, even if there are no macros requested (they
  //  are required to fully form the API).
  lym::MacroCollection::root().add_folder(
      tl::to_string(QObject::tr("Built-In")), ":/built-in-macros", "macros",
      true);
  lym::MacroCollection::root().add_folder(
      tl::to_string(QObject::tr("Built-In")), ":/built-in-pymacros", "pymacros",
      true);

  //  scans the macros from techs and packages (this will allow autorun-early on
  //  them) and updates m_external_paths
  sync_macro_sources();

  //  Scan for macros and set interpreter path
  for (std::vector<InternalPathDescriptor>::const_iterator p =
           m_internal_paths.begin();
       p != m_internal_paths.end(); ++p) {
      ...
      for (size_t c = 0; c < m_macro_categories.size(); ++c) {
        ...
        if (p->cat.empty()) {
          ...
          for (std::vector<std::string>::const_iterator f =
                   m_macro_categories[c].folders.begin();
               f != m_macro_categories[c].folders.end(); ++f) {

            std::string mp =
                tl::to_string(QDir(tl::to_qstring(p->path))
                                  .absoluteFilePath(tl::to_qstring(*f)));

            std::string description = p->description;
            if (*f != m_macro_categories[c].name) {
              description +=
                  " - " +
                  tl::to_string(tr("%1 branch").arg(tl::to_qstring(*f)));
            }

            lym::MacroCollection::root().add_folder(
                description, mp, m_macro_categories[c].name, p->readonly);
          }

        } else if (p->cat == m_macro_categories[c].name) {

          lym::MacroCollection::root().add_folder(
              p->description, p->path, m_macro_categories[c].name, p->readonly);
        }
      }
    }

    //  Add the unspecific paths as "package locations", so we get "ruby",
    //  "python" and similar folders as path components inside the interpreters.
    if (p->cat.empty()) {
      for (tl::Registrar<gsi::Interpreter>::iterator i =
               gsi::interpreters.begin();
           i != gsi::interpreters.end(); ++i) {
  ...
  if (!m_no_implicit_macros) {
    for (std::vector<ExternalPathDescriptor>::const_iterator p =
             m_external_paths.begin();
         p != m_external_paths.end(); ++p) {
      lym::MacroCollection *mc = lym::MacroCollection::root().add_folder(
          p->description, p->path, p->cat, p->readonly);
      if (mc) {
...
void MacroController::initialized(lay::Dispatcher *root) {
  connect(&m_temp_macros, SIGNAL(menu_needs_update()), this,
          SLOT(macro_collection_changed()));
  connect(&m_temp_macros,
          SIGNAL(macro_collection_changed(lym::MacroCollection *)), this,
          SLOT(macro_collection_changed()));

  mp_mw = lay::MainWindow::instance();
  if (mp_mw) {
    mp_macro_editor =
        new lay::MacroEditorDialog(root, &lym::MacroCollection::root());
  ...
  if (!m_file_watcher) {
    ...
    connect(m_file_watcher, SIGNAL(fileChanged(const QString &)), this,
            SLOT(file_watcher_triggered()));
    connect(m_file_watcher, SIGNAL(fileRemoved(const QString &)), this,
            SLOT(file_watcher_triggered()));
  }

  connect(&lym::MacroCollection::root(), SIGNAL(menu_needs_update()), this,
          SLOT(macro_collection_changed()));
  connect(&lym::MacroCollection::root(),
          SIGNAL(macro_collection_changed(lym::MacroCollection *)), this,
          SLOT(macro_collection_changed()));
  if (lay::TechnologyController::instance()) {
    connect(lay::TechnologyController::instance(),
            SIGNAL(active_technology_changed()), this,
            SLOT(macro_collection_changed()));
    connect(lay::TechnologyController::instance(),
            SIGNAL(technologies_edited()), this,
            SLOT(sync_with_external_sources()));
  }
  if (lay::SaltController::instance()) {
    connect(lay::SaltController::instance(), SIGNAL(salt_changed()), this,
            SLOT(sync_with_external_sources()));
  }
...
void MacroController::uninitialize(lay::Dispatcher * /*root*/) {
  disconnect(&lym::MacroCollection::root(), SIGNAL(menu_needs_update()), this,
             SLOT(macro_collection_changed()));
  disconnect(&lym::MacroCollection::root(),
             SIGNAL(macro_collection_changed(lym::MacroCollection *)), this,
             SLOT(macro_collection_changed()));
  if (lay::TechnologyController::instance()) {
    disconnect(lay::TechnologyController::instance(),
               SIGNAL(active_technology_changed()), this,
               SLOT(macro_collection_changed()));
    disconnect(lay::TechnologyController::instance(),
               SIGNAL(technologies_edited()), this,
  ...
  if (lay::SaltController::instance()) {
    disconnect(lay::SaltController::instance(), SIGNAL(salt_changed()), this,
               SLOT(sync_with_external_sources()));
  }

  if (m_file_watcher) {
    disconnect(m_file_watcher, SIGNAL(fileChanged(const QString &)), this,
               SLOT(file_watcher_triggered()));
    disconnect(m_file_watcher, SIGNAL(fileRemoved(const QString &)), this,
               SLOT(file_watcher_triggered()));
...
bool MacroController::configure(const std::string &key,
...
bool MacroController::accepts_drop(const std::string &path_or_url) const {
  ...
  if (suffix == QString::fromUtf8("rb") || suffix == QString::fromUtf8("py") ||
...
void MacroController::drop_url(const std::string &path_or_url) {
  ...
  if ((macro->is_autorun() || macro->show_in_menu()) &&
      !lay::ApplicationBase::instance()->appdata_path().empty()) {

    //  install macro permanently
    if (QMessageBox::question(
            mp_mw, QObject::tr("Install Macro"),
            QObject::tr(
                "Install macro '%1' permanently?\n\nPress 'Yes' to install the "
                "macro in the application settings folder permanently.")
                .arg(file_name),
      ...
      QDir folder(
          tl::to_qstring(lay::ApplicationBase::instance()->appdata_path()));
      ...
      if (!folder.cd(tl::to_qstring(cat))) {
        throw tl::Exception(
            tl::to_string(
                QObject::tr("Folder '%s' does not exists in installation path "
                            "'%s' - cannot install"))
                .c_str(),
            cat, lay::ApplicationBase::instance()->appdata_path());
      }

      QFileInfo target(folder, file_name);

      if (!target.exists() ||
          QMessageBox::question(mp_mw, QObject::tr("Overwrite Macro"),
        ...
        if (target.exists() && !target_file.remove()) {
          throw tl::Exception(
              tl::to_string(QObject::tr("Unable to remove file '%1'")
                                .arg(target.filePath())));
        }

        macro->set_file_path(tl::to_string(target.filePath()));

        //  run the macro now - if it fails, it is not installed, but the file
        //  path is already set to the target path.
        if (macro->is_autorun()) {
          macro->run();
        }
...
void MacroController::show_editor(const std::string &cat, bool force_add) {
  if (macro_categories().empty()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Application has not been compiled with scripting support "
                    "- no macro IDE available")));
  }
...
void MacroController::sync_package_paths() {
  ...
  if (sc) {
    ...
    for (lay::Salt::flat_iterator i = salt.begin_flat(); i != salt.end_flat();
         ++i) {
      package_locations.push_back((*i)->path());
    }
  }

  //  refresh the package locations by first removing the package locations and
  //  then rebuilding
  //  TODO: maybe that is a performance bottleneck, but right now,
  //  remove_package_location doesn't do a lot.

  for (std::vector<std::string>::const_iterator p = m_package_locations.begin();
       p != m_package_locations.end(); ++p) {
    for (tl::Registrar<gsi::Interpreter>::iterator i =
             gsi::interpreters.begin();
         i != gsi::interpreters.end(); ++i) {
...
void MacroController::sync_implicit_macros(bool ask_before_autorun) {
  if (m_no_implicit_macros) {
    ...
    //  delete macro collections which are no longer required or update
    //  description
    ...
    for (lym::MacroCollection::child_iterator m = root->begin_children();
         m != root->end_children(); ++m) {
      if (m->second->virtual_mode() == lym::MacroCollection::TechFolder ||
          m->second->virtual_mode() == lym::MacroCollection::SaltFolder) {
        std::map<std::string, const ExternalPathDescriptor *>::const_iterator
            u = new_folders_by_path.find(m->second->path());
    ...
    for (std::vector<lym::MacroCollection *>::iterator m =
             folders_to_delete.begin();
         m != folders_to_delete.end(); ++m) {
      if (tl::verbosity() >= 20) {
        tl::info << "Removing macro folder " << (*m)->path() << ", category '"
                 << (*m)->category() << "' because no longer in use";
      }
      root->erase(*m);
    }
    ...
    for (std::vector<ExternalPathDescriptor>::const_iterator p =
      ...
      if (pf != prev_folders_by_path.end()) {
        ...
        if (pf->second.version != p->version) {
          ...
          if (tl::verbosity() >= 20) {
            tl::info << "New version (" << p->version << " vs. "
                     << pf->second.version << ") of macro folder " << p->path
                     << ", category '" << p->cat << "' for '" << p->description
                     << "'";
          }

          lym::MacroCollection *mc =
              lym::MacroCollection::root().folder_by_name(p->path);
          if (mc) {
        ...
        if (tl::verbosity() >= 20) {
          tl::info << "Adding macro folder " << p->path << ", category '"
                   << p->cat << "' for '" << p->description << "'";
        }

        //  Add the folder. Note: it may happen that a macro folder for the tech
        //  specific macros already exists in a non-tech context. In that case,
        //  the add_folder method will return 0.
...

    {
      ...
      for (std::vector<lym::MacroCollection *>::const_iterator m =
               new_folders.begin();
           m != new_folders.end() && !has_autorun; ++m) {
        has_autorun = (*m)->has_autorun();
      }

      if (has_autorun) {
        if (!ask_before_autorun ||
            QMessageBox::question(
                mp_mw, QObject::tr("Run Macros"),
                QObject::tr(
                    "Some macros associated with new items are configured to "
...
void MacroController::sync_macro_sources() {
  ...
  for (db::Technologies::const_iterator t =
           db::Technologies::instance()->begin();
  ...
  for (std::map<std::string, std::vector<std::string>>::const_iterator t =
           tech_names_by_path.begin();
       t != tech_names_by_path.end(); ++t) {

    for (size_t c = 0; c < macro_categories().size(); ++c) {

      for (std::vector<std::string>::const_iterator f =
               macro_categories()[c].folders.begin();
           f != macro_categories()[c].folders.end(); ++f) {
        ...
        if (macro_dir.exists()) {
          ...
          if (t->second.size() == 1) {
            description = tl::to_string(
                tr("Technology %1").arg(tl::to_qstring(t->second.front())));
          } else {
            description = tl::to_string(
                tr("Technologies %1")
                    .arg(tl::to_qstring(tl::join(t->second, ","))));
          }

          std::map<std::string, std::vector<std::string>>::const_iterator gn =
              grain_names_by_path.find(t->first);
          if (gn != grain_names_by_path.end()) {
            description += " - ";
            if (gn->second.size() == 1) {
              description += tl::to_string(
                  tr("Package %1").arg(tl::to_qstring(gn->second.front())));
            } else {
              description += tl::to_string(
                  tr("Packages %1")
                      .arg(tl::to_qstring(tl::join(gn->second, ","))));
            }
          }
          ...
          external_paths.push_back(ExternalPathDescriptor(
              tl::to_string(macro_dir.path()), description,
              macro_categories()[c].name, lym::MacroCollection::TechFolder,
              readonly_paths.find(t->first) != readonly_paths.end()));
        }
      }
    }
  }
      ...
      for (size_t c = 0; c < macro_categories().size(); ++c) {
        ...
        for (std::vector<std::string>::const_iterator f =
          ...
          if (macro_dir.exists()) {
            ...
            std::string description =
                tl::to_string(tr("Package %1").arg(tl::to_qstring(g->name())));
            if (*f != macro_categories()[c].name) {
              description +=
                  " - " +
                  tl::to_string(tr("%1 branch").arg(tl::to_qstring(*f)));
            ...
            external_paths.push_back(ExternalPathDescriptor(
                tl::to_string(macro_dir.path()), description,
                macro_categories()[c].name, lym::MacroCollection::SaltFolder,
                g->is_readonly(), g->version()));
          }
        }
      }
    }
...
void MacroController::add_path(const std::string &path,
                               const std::string &description,
                               const std::string &category, bool readonly) {
  m_internal_paths.push_back(
      InternalPathDescriptor(path, description, category, readonly));
}
...
static std::string menu_name(std::set<std::string> &used_names,
...
class RunMacroAction : public lay::Action {
public:
  RunMacroAction(lym::Macro *lym) : lay::Action(), mp_lym(lym) {
  ...
  lym::Macro *macro() const { return const_cast<lym::Macro *>(mp_lym.get()); }
...
void MacroController::add_macro_items_to_menu(lym::MacroCollection &collection,
                                              std::set<std::string> &used_names,
                                              std::set<std::string> &groups,
                                              const db::Technology *tech) {
  for (lym::MacroCollection::child_iterator c = collection.begin_children();
       c != collection.end_children(); ++c) {

    //  check whether the macro collection is associated with the selected
    //  technology (if there is one)
    bool consider = false;
    if (!tech ||
        c->second->virtual_mode() != lym::MacroCollection::TechFolder) {
      consider = true;
    } else {
      const std::vector<lay::MacroController::MacroCategory> &mc =
          macro_categories();
      for (std::vector<lay::MacroController::MacroCategory>::const_iterator cc =
               mc.begin();
           cc != mc.end() && !consider; ++cc) {
        consider = (c->second->path() ==
                    tl::to_string(QDir(tl::to_qstring(tech->base_path()))
                                      .filePath(tl::to_qstring(cc->name))));
      }
    }

    if (consider) {
      add_macro_items_to_menu(*c->second, used_names, groups,
                              0 /*don't check 2nd level and below*/);
    }
  }

  for (lym::MacroCollection::iterator c = collection.begin();
       c != collection.end(); ++c) {
    ...
    if (c->second->show_in_menu()) {
      ...
      if (!gn.empty() && groups.find(gn) == groups.end()) {
        ...
        mp_mw->menu()->insert_item(mp, menu_name(used_names, std::string()),
                                   as);
      }
      ...
      mp_mw->menu()->insert_item(mp, menu_name(used_names, c->second->name()),
                                 a);

    } else if (!sc.empty()) {

      //  Create actions for shortcut-only actions too and add them to the main
      //  window to register their shortcut.
...
void MacroController::do_update_menu_with_macros() {
  ...
  for (tl::weak_collection<lay::Action>::iterator a = m_macro_actions.begin();
       a != m_macro_actions.end(); ++a) {
    if (a.operator->()) {
      actions.push_back(a.operator->());
    }
  }
  for (std::vector<lay::Action *>::const_iterator a = actions.begin();
       a != actions.end(); ++a) {
  ...
  add_macro_items_to_menu(lym::MacroCollection::root(), used_names, groups,
                          tech);
  ...
  for (std::vector<std::pair<std::string, bool>>::const_iterator hf =
           m_menu_items_hidden.begin();
       hf != m_menu_items_hidden.end(); ++hf) {
...
void MacroController::file_watcher_triggered() { dm_sync_files(); }

static void
add_collections_to_file_watcher(const lym::MacroCollection &collection,
                                tl::FileSystemWatcher *watcher) {
  for (lym::MacroCollection::const_child_iterator c =
           collection.begin_children();
       c != collection.end_children(); ++c) {
...
void MacroController::sync_file_watcher() {
  if (m_file_watcher) {
    ...
    add_collections_to_file_watcher(lym::MacroCollection::root(),
                                    m_file_watcher);
...
void MacroController::sync_files() {
  tl::log << tl::to_string(
      tr("Detected file system change in macro folders - updating"));
  lym::MacroCollection::root().reload(true /*safe*/);
}

MacroController *MacroController::instance() {
...
static tl::RegisteredClass<lay::PluginDeclaration>
    macro_controller_decl(new lay::MacroController(), 120, "MacroController");
...
static gsi::ClassExt<lay::Action> decl_ext_action(
    gsi::method_ext("macro", &macro_for_action,
                    ...
                    "If the action is associated with a macro, this method "
                    "returns a reference to the \\Macro object. "
