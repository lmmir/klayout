...
static const char *cfg_suffix_instance_cellname_value =
...
static const char *cfg_suffix_polygon_perimeter_value =
    "-polygon-perimeter-value";
...
static const char *cfg_suffix_text_orientation_value =
...
static std::string escape_string(const std::string &s,
                                 bool process_substring_refs = false,
                                 bool *has_substring_refs = 0) {
  std::string r = "\"";
...
static void configure_cbx(QComboBox *cbx, const std::string &value) {
  cbx->setCurrentIndex(cbx->findText(QString(tl::to_qstring(value))));
}

static void add_layer_clause(std::string &expr, LayerSelectionComboBox *layer) {
  db::LayerMap lm;
...
static void add_numerical_condition(std::string &expr, QComboBox *op,
                                    QLineEdit *value, const char *attribute,
                                    const char *unit = 0) {
...
static void add_string_condition(std::string &expr, QComboBox *op,
                                 QLineEdit *value, const char *attribute) {
  std::string v = tl::to_string(value->text());
  if (!v.empty()) {
...
static void add_orientation_condition(std::string &expr, QComboBox *op,
                                      QComboBox *value, const char *attribute) {
...
static void add_layer_assignment(std::string &expr,
                                 LayerSelectionComboBox *value,
...
static void add_cell_index_assignment(std::string &expr, QLineEdit *value,
                                      const char *attribute) {
  std::string v = tl::to_string(value->text());
  if (!v.empty()) {
...
static void add_numerical_assignment(std::string &expr, QLineEdit *value,
                                     const char *attribute, const char *unit) {
  std::string v = tl::to_string(value->text());
  if (!v.empty()) {
...
static void add_string_assignment(std::string &expr, QLineEdit *value,
                                  const char *attribute) {
  std::string v = tl::to_string(value->text());
  if (!v.empty()) {
...
static void add_orientation_assignment(std::string &expr, QComboBox *value,
                                       const char *attribute) {
  std::string v = tl::to_string(value->currentText());
  if (!v.empty()) {
...
class SearchInstanceProperties : public SearchPropertiesWidget,
                                 private Ui::SearchPropertiesInstance {
public:
  SearchInstanceProperties(QStackedWidget *sw, lay::LayoutViewBase * /*view*/,
                           int /*cv_index*/)
      : SearchPropertiesWidget(sw) {
    setupUi(this);
  }

  void restore_state(const std::string &pfx,
                     lay::Dispatcher *config_root) const {
    std::string v;
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_instance_cellname_op,
                            tl::to_string(instance_cellname_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_instance_cellname_value,
                            tl::to_string(instance_cellname_value->text()));
  }

  std::string search_expression(const std::string &cell_expr) const {
    std::string r = "instances of ";
    //  the cell expression may start of "instances of " itself - remove that
    //  one in that case
    ...
    std::string expr;
    add_string_condition(expr, instance_cellname_op, instance_cellname_value,
                         "cell_name");
    if (!expr.empty()) {
      r += " where ";
      r += expr;
...
class SearchShapeProperties : public SearchPropertiesWidget,
                              protected Ui::SearchPropertiesShape {
public:
  SearchShapeProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                        int cv_index)
      : SearchPropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_shape_layer,
                            tl::to_string(shape_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_shape_area_op,
                            tl::to_string(shape_area_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_shape_area_value,
                            tl::to_string(shape_area_value->text()));
    config_root->config_set(pfx + cfg_suffix_shape_perimeter_op,
                            tl::to_string(shape_perimeter_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_shape_perimeter_value,
                            tl::to_string(shape_perimeter_value->text()));
  }
  ...
  std::string search_expression(const std::string &cell_expr) const {
    ...
    std::string expr;
    add_numerical_condition(expr, shape_area_op, shape_area_value, "shape.area",
                            "um2");
    add_numerical_condition(expr, shape_perimeter_op, shape_perimeter_value,
                            "shape.perimeter", "um");
...
class SearchPolygonProperties : public SearchShapeProperties {
public:
  SearchPolygonProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                          int cv_index)
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_polygon_layer,
                            tl::to_string(shape_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_polygon_area_op,
                            tl::to_string(shape_area_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_polygon_area_value,
                            tl::to_string(shape_area_value->text()));
    config_root->config_set(pfx + cfg_suffix_polygon_perimeter_op,
                            tl::to_string(shape_perimeter_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_polygon_perimeter_value,
                            tl::to_string(shape_perimeter_value->text()));
  }
...
class SearchBoxProperties : public SearchPropertiesWidget,
                            private Ui::SearchPropertiesBox {
public:
  SearchBoxProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                      int cv_index)
      : SearchPropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_box_layer,
                            tl::to_string(box_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_box_width_op,
                            tl::to_string(box_width_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_box_width_value,
                            tl::to_string(box_width_value->text()));
    config_root->config_set(pfx + cfg_suffix_box_height_op,
                            tl::to_string(box_height_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_box_height_value,
                            tl::to_string(box_height_value->text()));
  }
  ...
  std::string search_expression(const std::string &cell_expr) const {
    ...
    std::string expr;
    add_numerical_condition(expr, box_area_op, box_area_value, "shape.area",
                            "um2");
    add_numerical_condition(expr, box_perimeter_op, box_perimeter_value,
                            "shape.perimeter", "um");
    add_numerical_condition(expr, box_width_op, box_width_value,
                            "shape.box_width", "um");
    add_numerical_condition(expr, box_height_op, box_height_value,
                            "shape.box_height", "um");
  ...
  std::string description() const { return tl::to_string(QObject::tr("Box")); }
};

// ----------------------------------------------------------------------------

class SearchPathProperties : public SearchPropertiesWidget,
                             private Ui::SearchPropertiesPath {
public:
  SearchPathProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                       int cv_index)
      : SearchPropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_path_layer,
                            tl::to_string(path_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_path_width_op,
                            tl::to_string(path_width_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_path_width_value,
                            tl::to_string(path_width_value->text()));
    config_root->config_set(pfx + cfg_suffix_path_length_op,
                            tl::to_string(path_length_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_path_length_value,
                            tl::to_string(path_length_value->text()));
  }
  ...
  std::string search_expression(const std::string &cell_expr) const {
    ...
    std::string expr;
    add_numerical_condition(expr, path_width_op, path_width_value,
                            "shape.path_width", "um");
    add_numerical_condition(expr, path_length_op, path_length_value,
                            "shape.path_length", "um");
  ...
  std::string description() const { return tl::to_string(QObject::tr("Path")); }
};

// ----------------------------------------------------------------------------

class SearchTextProperties : public SearchPropertiesWidget,
                             private Ui::SearchPropertiesText {
public:
  SearchTextProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                       int cv_index)
      : SearchPropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_text_layer,
                            tl::to_string(text_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_string_op,
                            tl::to_string(text_string_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_string_value,
                            tl::to_string(text_string_value->text()));
    config_root->config_set(pfx + cfg_suffix_text_orientation_op,
                            tl::to_string(text_orientation_op->currentText()));
    config_root->config_set(
        pfx + cfg_suffix_text_orientation_value,
        tl::to_string(text_orientation_value->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_size_op,
                            tl::to_string(text_size_op->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_size_value,
                            tl::to_string(text_size_value->text()));
  }
  ...
  std::string search_expression(const std::string &cell_expr) const {
    ...
    std::string expr;
    add_numerical_condition(expr, text_size_op, text_size_value,
                            "shape.text_size", "um");
    add_string_condition(expr, text_string_op, text_string_value,
                         "shape.text_string");
    add_orientation_condition(expr, text_orientation_op, text_orientation_value,
                              "shape.text_rot");
  ...
  std::string description() const { return tl::to_string(QObject::tr("Text")); }
};

// ----------------------------------------------------------------------------

class ReplaceInstanceProperties : public ReplacePropertiesWidget,
                                  private Ui::ReplacePropertiesInstance {
public:
  ReplaceInstanceProperties(QStackedWidget *sw, lay::LayoutViewBase * /*view*/,
                            int /*cv_index*/)
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_instance_cellname_value,
                            tl::to_string(instance_cellname->text()));
  }
...
class ReplaceShapeProperties : public ReplacePropertiesWidget,
                               protected Ui::ReplacePropertiesShape {
public:
  ReplaceShapeProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                         int cv_index)
      : ReplacePropertiesWidget(sw) {
    setupUi(this);
...
class ReplacePolygonProperties : public ReplaceShapeProperties {
public:
  ReplacePolygonProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                           int cv_index)
...
class ReplaceBoxProperties : public ReplacePropertiesWidget,
                             private Ui::ReplacePropertiesBox {
public:
  ReplaceBoxProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                       int cv_index)
      : ReplacePropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_box_layer,
                            tl::to_string(box_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_box_width_value,
                            tl::to_string(box_width->text()));
    config_root->config_set(pfx + cfg_suffix_box_height_value,
                            tl::to_string(box_height->text()));
  }
...
class ReplacePathProperties : public ReplacePropertiesWidget,
                              private Ui::ReplacePropertiesPath {
public:
  ReplacePathProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                        int cv_index)
      : ReplacePropertiesWidget(sw) {
    setupUi(this);
...
class ReplaceTextProperties : public ReplacePropertiesWidget,
                              private Ui::ReplacePropertiesText {
public:
  ReplaceTextProperties(QStackedWidget *sw, lay::LayoutViewBase *view,
                        int cv_index)
      : ReplacePropertiesWidget(sw) {
    setupUi(this);
  ...
  void save_state(const std::string &pfx, lay::Dispatcher *config_root) const {
    config_root->config_set(pfx + cfg_suffix_text_layer,
                            tl::to_string(text_layer->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_string_value,
                            tl::to_string(text_string->text()));
    config_root->config_set(pfx + cfg_suffix_text_orientation_value,
                            tl::to_string(text_orientation->currentText()));
    config_root->config_set(pfx + cfg_suffix_text_size_value,
                            tl::to_string(text_size->text()));
  }
...
void fill_find_pages(QStackedWidget *sw, lay::LayoutViewBase *view,
                     int cv_index) {
  while (sw->count() > 0) {
...
void fill_replace_pages(QStackedWidget *sw, lay::LayoutViewBase *view,
                        int cv_index) {
  while (sw->count() > 0) {
    sw->removeWidget(sw->widget(0));
  }
...
static const char *object_ids[] = {"instance", "shape", "box",
                                   "polygon",  "path",  "text"};
