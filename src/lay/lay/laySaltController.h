 ...
 */
class SaltController : public lay::PluginDeclaration, public tl::Object {
  Q_OBJECT
...
public:
   ...
   *
   *  The list is a list of names or URL's, optionally with a version in round
   * brackets. If a package with the given name or URL is installed already, it
   * is skipped. Otherwise, it's downloaded and installed. If URL's are given,
   * the URL is used for download. If names are given, the URL's are taken from
   * the salt mine index.
   *
   ...
   *  @brief Gets the salt
   */
  lay::Salt &salt() { return m_salt; }

  /**
   *  @brief Gets the salt (const version)
   */
  const lay::Salt &salt() const { return m_salt; }
