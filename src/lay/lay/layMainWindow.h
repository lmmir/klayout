 ...
 */
class LAY_PUBLIC MainWindow : public QMainWindow,
                              public tl::Object,
...
public:
   ...
   *
   *  This is the function that is called after the application has set up the
   * window but before the window is populated and configured
   */
  void show();
   ...
   *  It is supposed to perform some startup procedure, i.e. displaying
  ...
  /**
   *  @brief Load a (new) file into the layout and associate it with the given
   * technology
   *
   *  The mode param controls how to open the file:
   *    0 = replace current view, 1 = create new view, 2 = add to current view
   */
  lay::CellViewRef load_layout(const std::string &filename,
                               const std::string &technology, int mode = 0) {
    return load_layout(filename,
  ...
  /**
   *  @brief Load a (new) file into the layout using the given options and
   * technology
   *
   *  The mode param controls how to open the file:
   *    0 = replace current view, 1 = create new view, 2 = add to current view
   ...
   *
   *  This version will load the layer properties file "as it is". No mapping of
   * cellview index is performed. Only adding of missing layers is supported.
   *  The file name can be an empty string.
   *
   *  @param fn The name of the .lyp file to load.
   *  @param all_views Apply the .lyp file to all views. If false, load the
   * layer properties for the current view.
   *  @param add_default Add the missing layers for each view.
   */
  void load_layer_properties(const std::string &fn, bool all_views,
                             bool add_default);

  /**
   *  @brief Load a layer definition file for a specific cv
   *
   *  This version will load the layer properties file and apply it to the given
   * cv. The cv_index parameter will denote the cv index to which to apply the
   * layer properties file. It is assumed that the .lyp file will contain
   * definitions for a single layout only. This definition is mapped to the
   * specified cv_index after all definitions for this layout have been removed
   *  from the layer properties list.
   *  "cv_index" can be -1. In that case, the layer properties file will be
   * loaded multiple times, once for each cv present.
   *
   *  @param fn The name of the .lyp file to load.
   ...
   *  @param add_default Add the missing layers for each view.
   */
  void load_layer_properties(const std::string &fn, int cv_index,
   ...
   */
  db::Manager &manager() { return m_manager; }

  /**
   *  @brief Implementation of the lay::ProgressBar interface: set the flag
   * indicating whether we can cancel the operation
   */
  bool update_progress(tl::Progress *progress);

  /**
   *  @brief Implementation of the lay::ProgressBar interface: Returns a value
   * indicating whether a progress widget is wanted
   */
  bool progress_wants_widget() const;

  /**
   *  @brief Implementation of the lay::ProgressBar interface: adds a progress
   * widget
   */
  void progress_add_widget(QWidget *widget);

  /**
   *  @brief Implementation of the lay::ProgressBar interface: gets the progress
   * widget
   */
  QWidget *progress_get_widget() const;

  /**
   *  @brief Implementation of the lay::ProgressBar interface: removes a
   * progress widget
   */
  void progress_remove_widget();

  /**
   *  @brief Implementation of the lay::ProgressBar interface: Make the progress
   * widget visible or invisible
   */
   ...
   *
   *  This method returns true if exit () has been called. This state is never
   * reset. The main purpose is to detect "exit ()" calls before the event loop
   * is executed.
   */
   ...
   *  The intention of the busy state is to implement a special behavior while
   * inside a processEvent loop.
   */
  void enter_busy_mode(bool bm) { m_busy = bm; }
   ...
   *
   *  This is just a convience method. It's equivalent to
   * lay::LayoutView::current ().
   */
   ...
   */
  unsigned int views() const { return (unsigned int)mp_views.size(); }
   ...
   */
  lay::HelpDialog *assistant() const { return mp_assistant; }
   ...
   *
   *  The category can be given to specify the category to show. If empty, the
   * current category is shown. "add" can be true, to create a new macro
   * initially.
   */
  void show_macro_editor(const std::string &cat = std::string(),
                         bool add = false);

  /**
   *  @brief Gets the main window's menu
   */
  AbstractMenu *menu() { return m_dispatcher.menu(); }
   ...
   *
   *  If mode is 2, the layout is opened in the current view in addition to the
   * existing ones. If mode is 1, the layout is opened in a new view. If mode is
   * 0, the layout is opened in the current view deleting the existing ones.
   */
  void open(int mode);
  ...
  /**
   *  @brief Adds an entry to a specific MRU list given by the "cfg"
   ...
   */
  const std::string &initial_technology() { return m_initial_technology; }
   ...
   *  The integer parameter will receive the index of the view about to be
   * closed.
   */
...
private:
  ...
  QDockWidget *mp_hp_dock_widget, *mp_lp_dock_widget, *mp_libs_dock_widget,
      *mp_eo_dock_widget, *mp_bm_dock_widget;
  ControlWidgetStack *mp_hp_stack, *mp_lp_stack, *mp_layer_toolbox_stack,
      *mp_libs_stack, *mp_eo_stack, *mp_bm_stack;
  bool m_hp_visible, m_lp_visible, m_libs_visible, m_eo_visible, m_bm_visible,
      m_navigator_visible, m_layer_toolbox_visible,
      m_always_exit_without_saving;
  ...
  std::vector<std::string> m_mru_sessions, m_mru_layer_properties,
      m_mru_bookmarks;
  ...
  void interactive_close_view(int from, int to, bool invert_range,
                              bool all_cellviews);
  void call_on_current_view(void (lay::LayoutView::*func)(),
  ...
  bool can_close();
  lay::CellViewRef create_or_load_layout(const std::string *filename,
                                         const db::LoadLayoutOptions *options,
