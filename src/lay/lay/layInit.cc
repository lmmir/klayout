...
const std::list<lay::PluginDescriptor> &plugins() { return s_plugins; }
...
static PluginDescriptor do_load_plugin(const std::string &pp) {
  ...
  //  NOTE: since we are using a different suffix ("*.klp"), we can't use
  //  QLibrary.
#ifdef _WIN32
  //  there is no "dlopen" on mingw, so we need to emulate it.
  HINSTANCE handle =
      LoadLibraryW((const wchar_t *)tl::to_qstring(pp).constData());
  if (!handle) {
    throw tl::Exception(
        tl::to_string(
            QObject::tr("Unable to load plugin: %s with error message: %s ")),
        pp, GetLastError());
  }
  init_func =
      reinterpret_cast<klp_init_func_t>(GetProcAddress(handle, init_func_name));
#else
  void *handle;
  handle = dlopen(tl::string_to_system(pp).c_str(), RTLD_LAZY);
  if (!handle) {
    throw tl::Exception(tl::to_string(QObject::tr("Unable to load plugin: %s")),
                        pp);
  }
  init_func = reinterpret_cast<klp_init_func_t>(dlsym(handle, init_func_name));
#endif

  //  If present, call the initialization function to fetch some details from
  //  the plugin
  if (init_func) {
...
void init(const std::vector<std::string> &_paths) {
  ...
  for (std::vector<std::string>::const_iterator p = paths.begin();
    ...
    for (std::vector<std::string>::const_iterator e = ee.begin(); e != ee.end();
         ++e) {
      if (pattern.match(*e)) {
