...
void SearchReplaceResults::has_more(bool hm) { m_has_more = hm; }
...
void SearchReplaceResults::begin_changes(const db::Layout *layout) {
  ...
  //  In order to be independent from the layout object we save the mapping
  //  tables here
  m_lp_map.clear();
  ...
  if (layout) {
    ...
    for (db::cell_index_type ci = 0; ci < layout->cells(); ++ci) {
      if (layout->is_valid_cell_index(ci)) {
        m_cellname_map.insert(
            std::make_pair(ci, std::string(layout->cell_name(ci))));
      }
    }
...
size_t SearchReplaceResults::size() const {
  return std::max(std::max(m_cell_result.size(), m_data_result.size()),
                  std::max(m_shape_result.size(), m_inst_result.size())) +
         (m_has_more ? 1 : 0);
}

int SearchReplaceResults::columnCount(const QModelIndex & /*parent*/) const {
  //  Note: keep last column count for empty model to avoid resize events for
  //  the header
...
QVariant SearchReplaceResults::headerData(int section,
...
static std::string
instance_to_string(const db::Instance &inst,
                   const db::ICplxTrans &t = db::ICplxTrans()) {
  if (inst.is_null()) {
    return std::string();
  }
  ...
  std::string r;
  double dbu = 1.0;
  if (inst.instances() && inst.instances()->cell() &&
      inst.instances()->cell()->layout()) {
    r = inst.instances()
            ->cell()
            ->layout()
            ->cell(ci.object().cell_index())
            .get_qualified_name();
    dbu = inst.instances()->cell()->layout()->dbu();
  }

  r += " " +
       (db::CplxTrans(dbu) * t * ci.complex_trans() * db::CplxTrans(1.0 / dbu))
           .to_string();

  db::Vector a, b;
  unsigned long amax, bmax;
  if (ci.is_regular_array(a, b, amax, bmax)) {
    r += " array=(" + (db::CplxTrans(dbu) * t * a).to_string() + "," +
         (db::CplxTrans(dbu) * t * b).to_string() + " " + tl::to_string(amax) +
         "x" + tl::to_string(bmax) + ")";
  }
...
static std::string shape_to_string(const db::Shape &shape,
                                   const db::ICplxTrans &t = db::ICplxTrans()) {
  double dbu = 1.0;
  if (shape.shapes() && shape.shapes()->cell() &&
  ...
  if (shape.is_text()) {
    ...
    if (t.is_ortho()) {
      ...
      return "polygon " +
             db::Polygon(box).transformed(db::CplxTrans(dbu) * t).to_string();
    }
...
QVariant SearchReplaceResults::data(const QModelIndex &index, int role) const {
  if (role == Qt::DisplayRole) {
    ...
    if (index.row() < int(m_data_result.size())) {
      ...
      if (index.column() == 0 && !v.is_list()) {
        ...
        return QVariant(
            tl::to_qstring(v.get_list()[index.column()].to_string()));
      }
      ...
      if (index.column() == 0) {
        ...
        std::map<unsigned int, db::LayerProperties>::const_iterator lm =
            m_lp_map.find(layer);
        if (lm != m_lp_map.end()) {
        ...
        std::map<db::cell_index_type, std::string>::const_iterator cn =
            m_cellname_map.find(cell_index);
        ...
        if (result.trans != db::ICplxTrans()) {
          return QVariant(
              tl::to_qstring(shape_to_string(result.shape, result.trans)));
        }
      ...
      if (index.column() == 0) {
        ...
        if (result.trans != db::ICplxTrans()) {
          return QVariant(
              tl::to_qstring(instance_to_string(result.inst, result.trans)));
        }
...
Qt::ItemFlags SearchReplaceResults::flags(const QModelIndex & /*index*/) const {
...
bool SearchReplaceResults::hasIndex(int row, int /*column*/,
                                    const QModelIndex &parent) const {
  return !parent.isValid() && row < int(size());
}

QModelIndex SearchReplaceResults::index(int row, int column,
                                        const QModelIndex &parent) const {
  if (!parent.isValid()) {
...
QModelIndex SearchReplaceResults::parent(const QModelIndex & /*index*/) const {
  return QModelIndex();
}
...
void SearchReplaceResults::export_csv(const std::string &file) {
  ...
  for (size_t c = 0; c < n_columns; ++c) {
    ...
    output << escape_csv(tl::to_string(
        headerData(int(c), Qt::Horizontal, Qt::DisplayRole).toString()));
  }
  output << std::endl;
  ...
  for (size_t r = 0; r < n_rows; ++r) {
    ...
    for (size_t c = 0; c < n_columns; ++c) {
      ...
      //  TODO: optimize
      output << escape_csv(tl::to_string(
          data(index(int(r), int(c), parent), Qt::DisplayRole).toString()));
    }
...
void SearchReplaceResults::export_layout(db::Layout &layout) {
  if (!m_data_result.empty() || !m_cell_result.empty() ||
      !m_inst_result.empty()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Query produces something other than shapes - such results "
                    "cannot be converted to layout currently.")));
  }

  db::Cell &top_cell = layout.cell(layout.add_cell("RESULTS"));
  db::LayerMap insert_lm;

  for (std::vector<QueryShapeResult>::const_iterator s = m_shape_result.begin();
       s != m_shape_result.end(); ++s) {
...
void SearchReplaceResults::export_rdb(rdb::Database &rdb, double dbu) {
  if (!m_cell_result.empty()) {

    throw tl::Exception(
        tl::to_string(QObject::tr("Query produces cells - such results cannot "
                                  "be exported to a report database.")));
    ...
    for (std::vector<tl::Variant>::const_iterator v = m_data_result.begin();
         v != m_data_result.end(); ++v) {
      ...
      if (!v->is_list()) {
        ...
        for (std::vector<tl::Variant>::const_iterator i = v->get_list().begin();
             i != v->get_list().end(); ++i) {
    ...
    std::map<std::pair<db::cell_index_type, db::CplxTrans>, rdb::Cell *>
        cells_by_variant;
    for (std::map<db::cell_index_type, std::string>::const_iterator cn =
             m_cellname_map.begin();
         cn != m_cellname_map.end(); ++cn) {
      if (cn->second == rdb.top_cell_name()) {
        cells_by_variant.insert(std::make_pair(
            std::make_pair(cn->first, db::CplxTrans()), rdb_top_cell));
      }
    }

    for (std::vector<QueryInstResult>::const_iterator i = m_inst_result.begin();
         i != m_inst_result.end(); ++i) {

      std::map<std::pair<db::cell_index_type, db::CplxTrans>,
               rdb::Cell *>::const_iterator v =
          cells_by_variant.find(
              std::make_pair(i->cell_index, db::CplxTrans(i->trans)));
      if (v == cells_by_variant.end()) {
        std::map<db::cell_index_type, std::string>::const_iterator cn =
            m_cellname_map.find(i->cell_index);
        if (cn != m_cellname_map.end()) {
          v = cells_by_variant
                  .insert(std::make_pair(
                      std::make_pair(i->cell_index, db::CplxTrans(i->trans)),
                      rdb.create_cell(cn->second)))
                  .first;
          v->second->references().insert(rdb::Reference(
              db::CplxTrans(dbu) * i->trans * db::VCplxTrans(1.0 / dbu),
              rdb_top_cell->id()));
        }
      }
    ...
    for (std::vector<QueryShapeResult>::const_iterator s =
             m_shape_result.begin();
         s != m_shape_result.end(); ++s) {
      ...
      std::map<unsigned int, db::LayerProperties>::const_iterator lm =
          m_lp_map.find(layer);
      if (lm != m_lp_map.end()) {
        std::map<unsigned int, rdb::Category *>::const_iterator cm =
            categories.find(layer);
    ...
    for (std::vector<QueryShapeResult>::const_iterator s =
      ...
      if (cm != categories.end()) {
        ...
        std::map<std::pair<db::cell_index_type, db::CplxTrans>,
                 rdb::Cell *>::const_iterator v =
            cells_by_variant.find(
                std::make_pair(s->cell_index, db::CplxTrans(s->trans)));
        if (v == cells_by_variant.end()) {
          std::map<db::cell_index_type, std::string>::const_iterator cn =
              m_cellname_map.find(s->cell_index);
          if (cn != m_cellname_map.end()) {
            v = cells_by_variant
                    .insert(std::make_pair(
                        std::make_pair(s->cell_index, db::CplxTrans(s->trans)),
                    ...
                    .first;
            v->second->references().insert(rdb::Reference(
                db::CplxTrans(dbu) * s->trans * db::VCplxTrans(1.0 / dbu),
                rdb_top_cell->id()));
          }
        }
        ...
        if (v != cells_by_variant.end()) {
          ...
          if (s->shape.is_polygon()) {
            ...
            rdb::Item *item =
                rdb.create_item(v->second->id(), cm->second->id());
            item->values().add(new rdb::Value<db::DPolygon>(
                poly.transformed(db::CplxTrans(dbu))));
            ...
            rdb::Item *item =
                rdb.create_item(v->second->id(), cm->second->id());
            item->values().add(new rdb::Value<db::DPath>(
                path.transformed(db::CplxTrans(dbu))));
            ...
            rdb::Item *item =
                rdb.create_item(v->second->id(), cm->second->id());
            item->values().add(
                new rdb::Value<db::DBox>(box.transformed(db::CplxTrans(dbu))));
            ...
            rdb::Item *item =
                rdb.create_item(v->second->id(), cm->second->id());
            item->values().add(new rdb::Value<db::DText>(
                text.transformed(db::CplxTrans(dbu))));
            ...
            rdb::Item *item =
                rdb.create_item(v->second->id(), cm->second->id());
            item->values().add(new rdb::Value<db::DEdge>(
                edge.transformed(db::CplxTrans(dbu))));
          }
        }
      }
    }
...
static const char *ctx_values[] = {"current-cell", "current-cell-hierarchy",
                                   "all-cells"};

static void fill_ctx_cbx(QComboBox *cbx) {
...
SearchReplaceDialog::SearchReplaceDialog(lay::Dispatcher *root,
                                         LayoutViewBase *view)
    : lay::Browser(root, view), Ui::SearchReplaceDialog(), mp_view(view),
      m_current_mode(0), m_window(FitMarker), m_window_dim(0.0),
      m_max_item_count(0), m_last_query_cv_index(0) {
  setObjectName(QString::fromUtf8("search_replace_dialog"));

  Ui::SearchReplaceDialog::setupUi(this);

  connect(find_all_button, SIGNAL(clicked()), this,
          SLOT(find_all_button_clicked()));
  connect(delete_button, SIGNAL(clicked()), this,
          SLOT(delete_button_clicked()));
  connect(delete_all_button, SIGNAL(clicked()), this,
          SLOT(delete_all_button_clicked()));
  connect(replace_button, SIGNAL(clicked()), this,
          SLOT(replace_button_clicked()));
  connect(replace_all_button, SIGNAL(clicked()), this,
          SLOT(replace_all_button_clicked()));
  connect(execute_all_button, SIGNAL(clicked()), this,
          SLOT(execute_all_button_clicked()));
  connect(add_saved_button, SIGNAL(clicked()), this,
          SLOT(add_saved_button_clicked()));
  connect(replace_saved_button, SIGNAL(clicked()), this,
          SLOT(replace_saved_button_clicked()));
  connect(delete_saved_button, SIGNAL(clicked()), this,
          SLOT(delete_saved_button_clicked()));
  connect(rename_saved_button, SIGNAL(clicked()), this,
          SLOT(rename_saved_button_clicked()));
  connect(configure_button, SIGNAL(clicked()), this,
          SLOT(configure_button_clicked()));
  connect(mode_tab, SIGNAL(currentChanged(int)), this,
          SLOT(tab_index_changed(int)));
  connect(saved_queries, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
          SLOT(saved_query_double_clicked()));
  connect(recent_queries, SIGNAL(activated(int)), this,
          SLOT(recent_query_index_changed(int)));
  connect(cancel_button, SIGNAL(clicked()), this, SLOT(cancel_exec()));
  connect(delete_selected_button, SIGNAL(clicked()), this,
          SLOT(execute_selected_button_clicked()));
  connect(replace_selected_button, SIGNAL(clicked()), this,
          SLOT(execute_selected_button_clicked()));
  ...
  connect(
      ...
      this, SLOT(result_selection_changed()));
  connect(results->header(), SIGNAL(sectionCountChanged(int, int)), this,
          SLOT(header_columns_changed(int, int)));
...
SearchReplaceDialog::~SearchReplaceDialog() { remove_markers(); }

static void save_states(QStackedWidget *sw, const std::string &pfx,
                        lay::Dispatcher *config_root) {
  for (int i = 0; i < sw->count(); ++i) {
    SearchReplacePropertiesWidget *pw =
        dynamic_cast<SearchReplacePropertiesWidget *>(sw->widget(i));
    if (pw) {
...
static void restore_states(QStackedWidget *sw, const std::string &pfx,
                           lay::Dispatcher *config_root) {
  for (int i = 0; i < sw->count(); ++i) {
    SearchReplacePropertiesWidget *pw =
        dynamic_cast<SearchReplacePropertiesWidget *>(sw->widget(i));
...
static int ctx_to_index(const std::string &ctx) {
...
static std::string ctx_from_index(int index) {
...
void SearchReplaceDialog::restore_state() {
  ...
  if (config_root->config_get(cfg_sr_mode, v)) {
    for (int i = 0; i < int(sizeof(mode_values) / sizeof(mode_values[0]));
         ++i) {
      if (v == mode_values[i]) {
...
void SearchReplaceDialog::save_state() {
  ...
  if (m == find_mode_index) {
    ...
    config_root->config_set(
        cfg_sr_object, index_to_find_object_id(find_objects->currentIndex()));
    config_root->config_set(cfg_sr_ctx,
                            ctx_from_index(find_context->currentIndex()));
  } else if (m == delete_mode_index) {
    save_states(delete_properties, "sr-find", config_root);
    config_root->config_set(
        cfg_sr_object, index_to_find_object_id(delete_objects->currentIndex()));
    config_root->config_set(cfg_sr_ctx,
                            ctx_from_index(delete_context->currentIndex()));
    ...
    config_root->config_set(
        cfg_sr_object,
        index_to_find_object_id(replace_objects->currentIndex()));
    config_root->config_set(cfg_sr_ctx,
                            ctx_from_index(replace_context->currentIndex()));
  }

  {
    std::string v;
    for (std::vector<std::string>::const_iterator i = m_mru.begin();
         i != m_mru.end(); ++i) {
      if (!v.empty()) {
        v += ";";
      }
  ...
  {
    std::string v;
    for (std::vector<SavedQuery>::const_iterator i = m_saved.begin();
         i != m_saved.end(); ++i) {
      if (!v.empty()) {
        v += ";";
      }
...
void SearchReplaceDialog::export_csv() {
  ...
  lay::FileDialog file_dialog(
      this, tl::to_string(QObject::tr("Export CSV")),
      tl::to_string(QObject::tr("CSV Files (*.csv);;All Files (*)")), "csv");
...
void SearchReplaceDialog::export_rdb() {
  ...
  rdb->set_description(tl::to_string(QObject::tr("Query results: ")) +
                       m_last_query);
...
void SearchReplaceDialog::export_layout() {
  ...
  std::unique_ptr<lay::LayoutHandle> handle(
      new lay::LayoutHandle(new db::Layout(mp_view->manager()), std::string()));
...
static void sync_cbx(QComboBox *cbx, QStackedWidget *sw) {
  cbx->clear();
  for (int i = 0; i < sw->count(); ++i) {
    SearchPropertiesWidget *pw =
        dynamic_cast<SearchPropertiesWidget *>(sw->widget(i));
    tl_assert(pw != 0);
...
bool SearchReplaceDialog::configure(const std::string &name,
...
static std::string cell_expr(int ctx, const lay::CellView &cv) {
  std::string ce;
  if (ctx == 0) {
    ce = "cell ";
...
std::string
SearchReplaceDialog::build_find_expression(QStackedWidget *prop_page,
                                           QComboBox *context) {
  ...
  lay::SearchPropertiesWidget *p =
      dynamic_cast<lay::SearchPropertiesWidget *>(prop_page->currentWidget());
  if (p) {
...
std::string SearchReplaceDialog::build_delete_expression() {
  const lay::CellView &cv = mp_view->cellview(mp_view->active_cellview_index());
  if (!cv.is_valid()) {
    throw tl::Exception(tl::to_string(QObject::tr("No layout loaded")));
  }
  ...
  lay::SearchPropertiesWidget *p = dynamic_cast<lay::SearchPropertiesWidget *>(
      delete_properties->currentWidget());
  if (p) {
    expr = "delete ";
...
std::string SearchReplaceDialog::build_replace_expression() {
  const lay::CellView &cv = mp_view->cellview(mp_view->active_cellview_index());
  if (!cv.is_valid()) {
    throw tl::Exception(tl::to_string(QObject::tr("No layout loaded")));
  }
  ...
  lay::SearchPropertiesWidget *pf = dynamic_cast<lay::SearchPropertiesWidget *>(
      find_replace_properties->currentWidget());
  lay::ReplacePropertiesWidget *pr =
      dynamic_cast<lay::ReplacePropertiesWidget *>(
          replace_properties->currentWidget());
  if (pf && pr) {
    expr = "with ";
    expr +=
        pf->search_expression(cell_expr(replace_context->currentIndex(), cv));
    expr += " do ";
    std::string re = pr->replace_expression();
    if (re.empty()) {
      throw tl::Exception(
          tl::to_string(QObject::tr("No replacement action specified - replace "
...
void SearchReplaceDialog::update_saved_list() {
  ...
  for (std::vector<SavedQuery>::const_iterator s = m_saved.begin();
       s != m_saved.end(); ++s) {
...
void SearchReplaceDialog::update_mru_list() {
  ...
  for (std::vector<std::string>::const_iterator mru = m_mru.begin();
       mru != m_mru.end(); ++mru) {
...
void SearchReplaceDialog::issue_query(const std::string &q,
                                      const std::set<size_t> *selected_items,
                                      bool with_results) {
  ...
  if (with_results) {
    ...
    if (tl::verbosity() >= 10) {
      tl::log << tl::to_string(
                     QObject::tr("Running full query (without results): "))
              << q;
    }
    ...
    if (tl::verbosity() >= 10) {
      tl::log << tl::to_string(QObject::tr("Running query on selection: "))
              << q;
    }
    ...
    size_t n = 0;
    for (db::LayoutQueryIterator iq(lq, &cv->layout(), 0, &progress);
         !iq.at_end(); ++n) {
...
void SearchReplaceDialog::attach_layout(db::Layout *layout) {
  ...
  layout->bboxes_changed_any_event.add(this,
                                       &SearchReplaceDialog::layout_changed);
  layout->cell_name_changed_event.add(this,
                                      &SearchReplaceDialog::layout_changed);
  layout->layer_properties_changed_event.add(
      this, &SearchReplaceDialog::layout_changed);
}
...
bool SearchReplaceDialog::query_to_model(SearchReplaceResults &model,
                                         const db::LayoutQuery &lq,
                                         db::LayoutQueryIterator &iq,
                                         size_t max_item_count, bool all) {
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(QObject::tr("Query run")));

  size_t n = 0;
  bool res = false;

  int data_prop_id =
      lq.has_property("data") ? int(lq.property_by_name("data")) : -1;
  int shape_prop_id =
      lq.has_property("shape") ? int(lq.property_by_name("shape")) : -1;
  int layer_index_prop_id = lq.has_property("layer_index")
                                ? int(lq.property_by_name("layer_index"))
                                : -1;
  int instance_prop_id =
      lq.has_property("inst") ? int(lq.property_by_name("inst")) : -1;
  int path_trans_prop_id = lq.has_property("path_trans")
                               ? int(lq.property_by_name("path_trans"))
                               : -1;
  int trans_prop_id =
      lq.has_property("trans") ? int(lq.property_by_name("trans")) : -1;
  int cell_index_prop_id = lq.has_property("cell_index")
                               ? int(lq.property_by_name("cell_index"))
                               : -1;
  int parent_cell_index_prop_id =
      lq.has_property("parent_cell_index")
          ? int(lq.property_by_name("parent_cell_index"))
          : -1;
  int initial_cell_index_prop_id =
      lq.has_property("initial_cell_index")
          ? int(lq.property_by_name("initial_cell_index"))
          : -1;
  ...
  while (!iq.at_end()) {
    ...
    if (data_prop_id >= 0 && iq.get(data_prop_id, v)) {
      ...
      if (path_trans_prop_id >= 0 && iq.get(path_trans_prop_id, v)) {
        ...
        if (initial_cell_index_prop_id >= 0 &&
            iq.get(initial_cell_index_prop_id, v)) {
          initial_cell_index = db::cell_index_type(v.to_int());
        }
      }

      model.push_back(SearchReplaceResults::QueryShapeResult(
          shape, layer_index, trans, cell_index, initial_cell_index));
      ...
      if (parent_cell_index_prop_id >= 0 &&
          iq.get(parent_cell_index_prop_id, v)) {
        cell_index = db::cell_index_type(v.to_int());
      }
      if (path_trans_prop_id >= 0 && iq.get(path_trans_prop_id, v)) {
        trans = v.to_user<db::ICplxTrans>();
      ...
      model.push_back(SearchReplaceResults::QueryInstResult(
          instance, trans, cell_index, initial_cell_index));
      ...
      model.push_back(
          SearchReplaceResults::QueryCellResult(cell_index, parent_cell_index));
...
bool SearchReplaceDialog::fill_model(const db::LayoutQuery &lq,
                                     db::LayoutQueryIterator &iq,
...
void SearchReplaceDialog::remove_markers() {
  for (std::vector<lay::MarkerBase *>::const_iterator m = mp_markers.begin();
       m != mp_markers.end(); ++m) {
    delete *m;
  }
  mp_markers.clear();
...
void SearchReplaceDialog::result_selection_changed() {
  try {
    ...
    //  collect the transformation variants for this cellview - this way we can
    //  paint the cell boxes for each global transformation
    std::vector<db::DCplxTrans> global_trans =
        view()->cv_transform_variants(cv_index);
    std::map<unsigned int, std::vector<db::DCplxTrans>> tv_map =
        view()->cv_transform_variants_by_layer(cv_index);
    ...
    for (QModelIndexList::const_iterator s = sel.begin(); s != sel.end(); ++s) {
      ...
      if (index < 0) {
        ...
        const SearchReplaceResults::QueryShapeResult &sr =
            m_model.shapes()[index];

        if (!sr.shape.is_null()) {

          db::ICplxTrans tr_context;
          if (layout.is_valid_cell_index(sr.initial_cell_index)) {
            tr_context = db::find_layout_context(layout, sr.initial_cell_index,
                                                 cv.cell_index())
                             .second;
          }
          ...
          std::map<unsigned int, std::vector<db::DCplxTrans>>::const_iterator
              tv = tv_map.find(sr.layer_index);
          if (tv != tv_map.end()) {
        ...
        const SearchReplaceResults::QueryInstResult &ir =
            m_model.instances()[index];

        if (!ir.inst.is_null()) {

          db::ICplxTrans tr_context;
          if (layout.is_valid_cell_index(ir.initial_cell_index)) {
            tr_context = db::find_layout_context(layout, ir.initial_cell_index,
                                                 cv.cell_index())
                             .second;
          }

          lay::InstanceMarker *marker =
              new lay::InstanceMarker(view(), cv_index);
        ...
        const SearchReplaceResults::QueryCellResult &ir =
            m_model.cells()[index];

        std::pair<bool, db::ICplxTrans> si =
            db::find_layout_context(layout, ir.cell_index, cv.cell_index());
        if (si.first) {
        ...
        for (tl::Variant::const_iterator v = dr.begin(); v != dr.end(); ++v) {
          ...
          if (v->is_user<db::DBox>()) {
            ...
            marker->set(v->to_user<db::Polygon>(), db::ICplxTrans(),
                        global_trans);
...
void SearchReplaceDialog::replace_button_clicked() {
  ...
  m_find_query =
      build_find_expression(find_replace_properties, replace_context);
  issue_query(m_find_query, 0, true);
  ...
  if (!sel.empty()) {
    ...
    if (mp_view->manager()) {
      if (sender() == delete_selected_button) {
        mp_view->manager()->transaction(
            tl::to_string(QObject::tr("Delete selected")));
      } else {
        mp_view->manager()->transaction(
            tl::to_string(QObject::tr("Replace selected")));
      }
    }
...
void SearchReplaceDialog::execute_all_button_clicked() {
  ...
  if (mp_view->manager()) {
    mp_view->manager()->transaction(
        tl::to_string(QObject::tr("Execute custom query")));
  }
  mp_view->cancel();
    ...
    if (m_current_mode == find_mode_index) {
      ...
      try {
        custom_query->setText(tl::to_qstring(
            build_find_expression(find_properties, find_context)));
      } catch (...) {
        //  ignore errors
        custom_query->setText(tl::to_qstring(""));
      }
...
void SearchReplaceDialog::add_saved_button_clicked() {
  ...
  QString desc = QInputDialog::getText(
      this, QObject::tr("Enter Description"),
      QObject::tr("Enter a description text for the current query.\nThat text "
                  "will be shown in the selection box."),
...
void SearchReplaceDialog::rename_saved_button_clicked() {
  ...
  if (index >= 0 && index < int(m_saved.size())) {
    ...
    QString desc = QInputDialog::getText(
        this, QObject::tr("Enter Description"),
        QObject::tr("Enter a description text for the current query.\nThat "
                    "text will be shown in the selection box."),
        QLineEdit::Normal, tl::to_qstring(m_saved[index].description), &ok);
    if (ok) {
