...
struct IndexEntry {
  IndexEntry() {}
  IndexEntry(const std::string &_key, const std::string &_title,
...
/**
 *  @brief A specialization of BrowserSource for delivering the generated
 * documentation
 */
class LAY_PUBLIC HelpSource : public BrowserSource {
public:
  HelpSource();
  ...
  void push_index(const IndexEntry &index) { m_index.push_back(index); }
  ...
  void set_klayout_version(const std::string &v) { m_klayout_version = v; }
...
private:
  ...
  std::string process(const QDomDocument &doc, const std::string &path,
                      BrowserOutline &ol);
  void process_child_nodes(const QDomElement &element, const std::string &path,
                           QXmlStreamWriter &writer, BrowserOutline &old);
  void process(const QDomElement &element, const std::string &path,
               QXmlStreamWriter &writer, BrowserOutline &ol);
  std::string read(const std::string &u);
  void writeElement(const QDomElement &element, const std::string &path,
                    QXmlStreamWriter &writer, BrowserOutline &ol);
  void scan(const std::string &path, tl::AbsoluteProgress &progress);
  void scan_child_nodes(const QDomElement &element, const std::string &path,
                        std::vector<std::string> &subtopics, std::string &title,
                        std::string &section);
  void scan(const QDomElement &element, const std::string &path,
            std::vector<std::string> &subtopics, std::string &title,
            std::string &section);
};
