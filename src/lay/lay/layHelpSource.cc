...
static QString class_doc_path(const QString &c) {
  ...
  if (cm_re.indexIn(c) == 0) {
    ...
    return QString::fromUtf8("/code/class_") + cn +
           QString::fromUtf8(".xml#m_") + cm_re.cap(2);
  } else {
    QString cn = c;
    cn.replace(QString::fromUtf8("::"), QString::fromUtf8("_"));
...
static QString class_title(const QString &c) { return c; }
...
/**
 *  @brief A modified Levenshtein distance for determining in a fuzzy way
 * whether a string is contained in another
 */
static int search_pattern_distance(const std::string &a, const std::string &b) {
  ...
  for (int i = 0; i < int(b.size()); ++i) {
    ...
    for (int j = 0; j < int(a.size()); ++j) {
      ...
      row1[j + 1] =
          std::min(row0[j] + cost, std::min(row0[j + 1], row1[j]) + 1);
    }
...
struct EditDistanceSorter {
  EditDistanceSorter(const std::string &subject,
  ...
  bool operator()(int a, int b) const {
    ...
    if (d1 == d2) {
      return m_index[a].normalized_key.size() <
             m_index[b].normalized_key.size();
    } else {
      return d1 < d2;
    }
  }
...
struct StringLengthSorter {
  StringLengthSorter(const std::vector<IndexEntry> &index) : m_index(index) {
    // .. nothing yet ..
  }
...
IndexEntry::IndexEntry(const std::string &_key, const std::string &_title,
...
/**
 *  @brief A specialization of tl::make_element that is capable of taking a
 * std::map::const_iterator
 *
 *  The original tl::make_element gives a compiler warning (taking address of
 * temporary)
 *  TODO: consolidate this version and tl::make_element.
 */
template <class Value, class Iter, class Parent>
tl::XMLElement<Value, Parent, tl::XMLMemberIterReadAdaptor<Value, Iter, Parent>,
               tl::XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element_iter(Iter (Parent::*begin)() const, Iter (Parent::*end)() const,
                  void (Parent::*setter)(const Value &),
                  const std::string &name, const tl::XMLElementList &children) {
  return tl::XMLElement<Value, Parent,
                        tl::XMLMemberIterReadAdaptor<Value, Iter, Parent>,
                        tl::XMLMemberAccRefWriteAdaptor<Value, Parent>>(
      tl::XMLMemberIterReadAdaptor<Value, Iter, Parent>(begin, end),
      tl::XMLMemberAccRefWriteAdaptor<Value, Parent>(setter), name, children);
}

static const tl::XMLStruct<HelpSource> help_index_structure(
    "help-index",
    tl::make_member<std::string, HelpSource>(&HelpSource::klayout_version,
                                             &HelpSource::set_klayout_version,
                                             "program-version") +
        tl::make_element<IndexEntry, std::vector<IndexEntry>::const_iterator,
                         HelpSource>(
            &HelpSource::begin_index, &HelpSource::end_index,
            &HelpSource::push_index, "index",
            tl::make_member<std::string, IndexEntry>(&IndexEntry::key,
                                                     "literal-key") +
                tl::make_member<std::string, IndexEntry>(
                    &IndexEntry::normalized_key, "key") +
                tl::make_member<std::string, IndexEntry>(&IndexEntry::title,
                                                         "title") +
                tl::make_member<std::string, IndexEntry>(&IndexEntry::path,
                                                         "path")) +
        make_element_iter<std::pair<std::string, std::string>,
                          std::map<std::string, std::string>::const_iterator,
                          HelpSource>(
            &HelpSource::begin_parents, &HelpSource::end_parents,
            &HelpSource::push_parent, "parent",
            tl::make_member<std::string, std::pair<std::string, std::string>>(
                &std::pair<std::string, std::string>::first, "path") +
                tl::make_member<std::string,
                                std::pair<std::string, std::string>>(
                    &std::pair<std::string, std::string>::second, "parent")) +
        make_element_iter<
            std::pair<std::string, std::string>,
            std::vector<std::pair<std::string, std::string>>::const_iterator,
            HelpSource>(
            &HelpSource::begin_titles, &HelpSource::end_titles,
            &HelpSource::push_title, "title",
            tl::make_member<std::string, std::pair<std::string, std::string>>(
                &std::pair<std::string, std::string>::first, "path") +
                tl::make_member<std::string,
                                std::pair<std::string, std::string>>(
                    &std::pair<std::string, std::string>::second, "title")));

HelpSource::HelpSource() : m_kindex(0) { initialize_index(); }

HelpSource::HelpSource(bool make_index) : m_kindex(0) {
  if (make_index) {
    initialize_index();
  }
}
...
void HelpSource::initialize_index() {
  try {
    ...
    tl::SelfTimer timer(tl::verbosity() >= 21,
                        tl::to_string(QObject::tr("Initializing help index")));
    ...
    if (!lay::ApplicationBase::instance()->appdata_path().empty()) {
      per_user_cache_file = tl::to_string(
          QDir(tl::to_qstring(lay::ApplicationBase::instance()->appdata_path()))
              .absoluteFilePath(help_index_cache_file));
    }

    //  Try to obtain the help index from the installation or application path

    std::vector<std::string> cache_files;
    cache_files.push_back(tl::to_string(
        QDir(tl::to_qstring(lay::ApplicationBase::instance()->inst_path()))
            .absoluteFilePath(help_index_cache_file)));
    if (!per_user_cache_file.empty()) {
      cache_files.push_back(per_user_cache_file);
    }

    for (std::vector<std::string>::const_iterator c = cache_files.begin();
         !ok && c != cache_files.end(); ++c) {
        ...
        if (m_klayout_version == lay::ApplicationBase::version()) {
          ...
          if (tl::verbosity() >= 10) {
            tl::warn << tl::to_string(
                            tr("Help index ignored (wrong version) from "))
                     << *c;
          }
        }
...
void HelpSource::scan() {
  ...
  tl::AbsoluteProgress progress(
      tl::to_string(QObject::tr("Initializing help index")), 1);
  scan(index_url, progress);
}

std::string HelpSource::klayout_version() const {
...
void HelpSource::scan(const std::string &path, tl::AbsoluteProgress &progress) {
  ...
  for (std::vector<std::string>::const_iterator st = subtopics.begin();
       st != subtopics.end(); ++st) {
    scan(*st, progress);
  }
}

void HelpSource::scan_child_nodes(const QDomElement &element,
...
void HelpSource::scan(const QDomElement &element, const std::string &path,
                      std::vector<std::string> &subtopics, std::string &title,
                      std::string &section) {
  if (element.localName() == topic_ref_element) {

    //  remember topic
    std::string href =
        tl::to_string(element.attribute(href_attribute, QString()));
    ...
    m_index.push_back(IndexEntry(tl::to_string(name), t,
                                 path + "#k_" + tl::to_string(m_kindex)));
    ...
    m_index.push_back(IndexEntry(tl::to_string(element.text()), t,
                                 path + "#k_" + tl::to_string(m_kindex)));
...
QDomDocument HelpSource::get_dom(const std::string &u) {
  QUrl url = QUrl::fromEncoded(u.c_str());
  QString path = url.path();

  for (tl::Registrar<lay::HelpProvider>::iterator cls =
           tl::Registrar<lay::HelpProvider>::begin();
       cls != tl::Registrar<lay::HelpProvider>::end(); ++cls) {
...
#if QT_VERSION >= 0x050000
    return produce_search(
        tl::to_string(QUrlQuery(url.query())
                          .queryItemValue(QString::fromUtf8("string"))
                          .toLower()));
#else
    return produce_search(tl::to_string(
        url.queryItemValue(QString::fromUtf8("string")).toLower()));
#endif
...
QImage HelpSource::get_image(const std::string &u) {
  QResource res(resource_url(QUrl::fromEncoded(u.c_str()).path()));
  if (res.size() == 0) {
    throw tl::Exception(
        tl::to_string(QObject::tr("ERROR: no data found for resource ")) + u);
  }
...
std::string HelpSource::get_css(const std::string &u) {
  std::ifstream t(
      tl::to_string(
          QDir(tl::to_qstring(lay::ApplicationBase::instance()->inst_path()))
              .absoluteFilePath(QString::fromUtf8("help_format.css")))
          .c_str());
  if (t.good()) {
    std::string c;
    while (t.good()) {
...
std::string HelpSource::get(const std::string &u) {
  BrowserOutline ol;
  return process(get_dom(u), u, ol);
}

BrowserOutline HelpSource::get_outline(const std::string &u) {
  BrowserOutline ol;
  process(get_dom(u), u, ol);
  return ol;
}

void HelpSource::search_completers(const std::string &string,
                                   std::list<std::string> &completers) {
  size_t n = 0;
  const size_t max_completers = 100;

  //  first produce all hits with match
  for (std::vector<IndexEntry>::const_iterator i = m_index.begin();
       i < m_index.end() && n < max_completers; ++i) {
...
std::string HelpSource::next_topic(const std::string &url) {
...
std::string HelpSource::prev_topic(const std::string &url) {
  std::string u = tl::to_string(QUrl::fromEncoded(url.c_str()).path());
...
QDomDocument HelpSource::produce_main_index() {
  ...
  os << "<p>"
     << tl::to_string(QObject::tr(
            ...
            "For a brief introduction read the User Manual. 'Various Topics' "
            "is a collection of brief articles about specific topics.\n"
            "For Ruby programming see the 'Programming Ruby Scripts' chapter "
            "and for a complete Ruby class reference see the 'Class "
            "Index'.\n"));
  os << "</p>" << std::endl;
  ...
  QString errorMsg;
  int errorLine = 0;
  if (!doc.setContent(QByteArray(text.c_str(), int(text.size())), true,
                      &errorMsg, &errorLine)) {
    throw tl::Exception(tl::to_string(errorMsg) + ", in line " +
                        tl::to_string(errorLine) + " of main index");
  }

  return doc;
}

QDomDocument HelpSource::produce_search(const std::string &string) {
  std::ostringstream os;

  os << "<doc><title>" << tl::to_string(QObject::tr("Search results for"))
     << " \"" << escape_xml(string) << "\"</title>" << std::endl;

  os << "<h2>" << tl::to_string(QObject::tr("Search results for")) << " \""
     << escape_xml(string) << "\"</h2><p/>" << std::endl;
  ...
  if (!exact_hit_indices.empty()) {
    ...
    if (exact_hit_indices.size() > 1) {
      os << "<p>" << exact_hit_indices.size() << " "
         << tl::to_string(QObject::tr("exact hits found")) << "</p>"
         << std::endl;
    } else {
    ...
    std::sort(exact_hit_indices.begin(), exact_hit_indices.end(),
              StringLengthSorter(m_index));

    int max_n = 100;

    n = 0;
    for (std::vector<int>::const_iterator i = exact_hit_indices.begin();
         i < exact_hit_indices.end() && n < max_n; ++i, ++n) {

      const IndexEntry &ie = m_index[*i];
      size_t f = ie.normalized_key.find(string);

      os << "<tr>" << std::endl;
      os << "<td>" << escape_xml(std::string(ie.key, 0, f)) << "<b>"
         << escape_xml(std::string(ie.key, f, string.size())) << "</b>"
         << escape_xml(std::string(ie.key, f + string.size())) << "</td>"
         << std::endl;
      os << "<td><a href=\"" << ie.path << "\"><nobr>" << escape_xml(ie.title)
         << "</nobr></a></td>" << std::endl;
      os << "</tr>" << std::endl;
    }
    ...
    if (int(indices.size()) > max_n) {
      std::partial_sort(indices.begin(), indices.begin() + max_n, indices.end(),
                        EditDistanceSorter(string, m_index));
    } else {
...
const std::string &HelpSource::parent_of(const std::string &path) {
...
std::string HelpSource::process(const QDomDocument &doc,
                                const std::string &path, BrowserOutline &ol) {
  QBuffer output;
...
void HelpSource::process_child_nodes(const QDomElement &element,
                                     const std::string &path,
...
void HelpSource::writeElement(const QDomElement &element,
                              const std::string &path, QXmlStreamWriter &writer,
                              BrowserOutline &ol) {
...
static void add_outline_at_level(int i, BrowserOutline &ol,
                                 const BrowserOutline &child) {
  if (i == 0) {
    ol.add_child(child);
    ...
    ol.add_child(
        BrowserOutline(tl::to_string(QObject::tr("(empty")), std::string()));
    add_outline_at_level(i - 1, *(--ol.end()), child);
  }
}

void HelpSource::process(const QDomElement &element, const std::string &path,
                         QXmlStreamWriter &writer, BrowserOutline &ol) {
  if (element.localName() == keyword_element) {
    ...
    writer.writeAttribute(QString::fromUtf8("name"),
                          QString::fromUtf8("k_") + QString::number(m_kindex));
    writer.writeEndElement();
    ...
    QDomNodeList index =
        element.ownerDocument().documentElement().elementsByTagName(h2_element);
    for (int i = 0; i != index.count(); ++i) {
      writer.writeStartElement(QString::fromUtf8("li"));
      writer.writeStartElement(QString::fromUtf8("a"));
      writer.writeAttribute(QString::fromUtf8("href"),
                            QString::fromUtf8("#") + index.at(i).localName() +
                                QString::fromUtf8("-") +
                                QString::number(index.at(i).lineNumber()));
    ...
    writer.writeEndElement();
             ...
             element.localName() == h3_element) {

    int level = (element.localName() == h2_element ? 1 : 2);

    QString name = element.localName() + QString::fromUtf8("-") +
                   QString::number(element.lineNumber());
    ...
    add_outline_at_level(
        level, ol,
        BrowserOutline(tl::to_string(title),
                       path_wo_anchor + "#" + tl::to_string(name)));
    ...
    add_outline_at_level(0, ol,
                         BrowserOutline(tl::to_string(element.text()), path));
    ...
    //  replace "doc" by "<html><head><link rel='stylesheet' type='text/css'
    //  href='/css/help_format.css'><title>(document
    //  title</title></head><body>(content)</body></html>" and the navigator
    //  bar.
    ...
    std::string pu =
        parent_of(tl::to_string(QUrl::fromEncoded(path.c_str()).path()));
    while (!pu.empty()) {
    ...
    writer.writeDTD(
        QString::fromUtf8("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 "
                          "Strict//EN\"\n\"http://www.w3.org/TR/xhtml1/DTD/"
                          "xhtml1-strict.dtd\">"));
    writer.writeDefaultNamespace(
        QString::fromUtf8("http://www.w3.org/1999/xhtml"));
    ...
    writer.writeAttribute(QString::fromUtf8("rel"),
                          QString::fromUtf8("stylesheet"));
    writer.writeAttribute(QString::fromUtf8("type"),
                          QString::fromUtf8("text/css"));
    writer.writeAttribute(QString::fromUtf8("href"),
                          QString::fromUtf8("/css/help_format.css"));
    writer.writeEndElement();
    ...
    writer.writeAttribute(QString::fromUtf8("class"),
                          QString::fromUtf8("navigator"));
    for (std::vector<std::pair<std::string, std::string>>::const_iterator p =
             pp.begin();
         p != pp.end(); ++p) {
      if (p != pp.begin()) {
        writer.writeCharacters(QString::fromUtf8(" ") + QString(QChar(187)) +
                               QString::fromUtf8(" ")); // &raquo;
      }
      if (p->first.empty()) {
        ...
        writer.writeStartElement(QString::fromUtf8("a"));
        writer.writeAttribute(QString::fromUtf8("href"),
                              relative_url(path, tl::to_qstring(p->first)));
        writer.writeCharacters(tl::to_qstring(p->second));
        writer.writeEndElement();
      }
    }
    ...
    //  replace "<topic href='...'>" by "<li><a href="(path)">(topic
    //  title></a></li>"
    writer.writeStartElement(QString::fromUtf8("li"));
    writer.writeStartElement(QString::fromUtf8("a"));
    ...
    if (new_el.hasAttribute(href_attribute)) {
      new_el.setAttribute(href_attribute,
                          relative_url(path, new_el.attribute(href_attribute)));
    }
    ...
    if (new_el.hasAttribute(src_attribute)) {
      new_el.setAttribute(src_attribute,
                          relative_url(path, new_el.attribute(src_attribute)));
    }
    writeElement(new_el, path, writer, ol);
    ...
    writer.writeStartElement(QString::fromUtf8("a"));
    writer.writeAttribute(QString::fromUtf8("href"),
                          relative_url(path, class_doc_path(href)));
...
std::string HelpSource::title_for(const std::string &path) {
  if (m_title_map.empty()) {
    for (std::vector<std::pair<std::string, std::string>>::const_iterator t =
             m_titles.begin();
         t != m_titles.end(); ++t) {
...
std::vector<std::string> HelpSource::urls() {
...
const tl::Variant &HelpSource::get_option(const std::string &key) const {
