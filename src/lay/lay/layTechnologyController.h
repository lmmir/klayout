 ...
 */
class LAY_PUBLIC TechnologyController : public PluginDeclaration,
                                        public tl::Object {
  Q_OBJECT
...
public:
   ...
   *  Temporary technologies are additional technologies which are added to the
   * list of technologies but are not persisted or editable. "load" needs to be
   * called after temp technologies have been added.
   */
  void add_temp_tech(const db::Technology &t);

  /**
   *  @brief Updates the given technology collection with the technologies from
   * the search path and the temp technologies
   */
