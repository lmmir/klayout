...
static void split_path(const std::string &path, std::vector<std::string> &pc) {
  QString sep;
#ifdef _WIN32
...
std::vector<std::string> get_klayout_path() {
  if (s_klayout_path_set) {
    ...
    //  generate the klayout path: the first component is always the appdata
    //  path
...
std::string salt_mine_url() {
  return tl::get_env("KLAYOUT_SALT_MINE",
                     "http://sami.klayout.org/repository.xml");
}
