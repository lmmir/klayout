...
static void ui_exception_handler_tl(const tl::Exception &ex, QWidget *parent) {
  ...
  if (lay::MainWindow::instance() &&
      lay::MainWindow::instance()->manager().transacting()) {
    lay::MainWindow::instance()->manager().commit();
  }

  const tl::ExitException *gsi_exit =
      dynamic_cast<const tl::ExitException *>(&ex);
  const tl::BreakException *gsi_break =
      dynamic_cast<const tl::BreakException *>(&ex);
  const tl::ScriptError *gsi_excpt = dynamic_cast<const tl::ScriptError *>(&ex);
  const db::ReaderUnknownFormatException *reader_excpt =
      dynamic_cast<const db::ReaderUnknownFormatException *>(&ex);

  if (gsi_exit || gsi_break) {
    //  exit and break exceptions are not shown - they are issued when a script
    //  is aborted or an operation is cancelled
  } else if (gsi_excpt) {

    //  show and GSI errors in the context of the macro editor if that is open
    if (!parent && lay::MacroEditorDialog::instance() &&
        lay::MacroEditorDialog::instance()->isVisible()) {
      parent = lay::MacroEditorDialog::instance();
    }
    if (!parent) {
      parent = QApplication::activeWindow() ? QApplication::activeWindow()
    ...
    if (reader_excpt) {
      lay::ReaderErrorForm error_dialog(parent, "reader_error_form",
                                        reader_excpt);
      error_dialog.exec();
    } else {
      QMessageBox::critical(parent, QObject::tr("Error"),
                            tl::to_qstring(ex.msg()));
    }
  }
}

static void ui_exception_handler_std(const std::exception &ex,
                                     QWidget *parent) {
  //  Prevents severe side effects if there are pending deferred methods
  tl::NoDeferredMethods silent;
...
static void ui_exception_handler_def(QWidget *parent) {
  ...
  QMessageBox::critical(parent, QObject::tr("Error"),
                        QObject::tr("An unspecific error occurred"));
}
...
ApplicationBase::ApplicationBase(bool non_ui_mode)
    : gsi::ObjectBase(), m_lyp_map_all_cvs(true), m_lyp_add_default(false),
      m_run_macro_and_exit(true), m_packages_with_dep(false),
      m_write_config_file(false), m_gtf_replay_rate(0), m_gtf_replay_stop(-1),
      m_gtf_record(), m_gtf_save_incremental(false), m_no_macros(false),
      m_same_view(false), m_sync_mode(false), m_no_gui(non_ui_mode),
      m_vo_mode(false), m_editable(false), m_editable_set(false),
      m_enable_undo(true), mp_ruby_interpreter(0), mp_python_interpreter(0) {
  // TODO: offer a strict mode for exception handling where this takes place:
  // lay::ApplicationBase::instance ()->exit (1);
  if (!m_no_gui) {
    tl::set_ui_exception_handlers(ui_exception_handler_tl,
                                  ui_exception_handler_std,
                                  ui_exception_handler_def);
  }

  gsi::make_application_decl(m_no_gui);

  //  initialize the system codecs (Hint: this must be done after the
  //  QApplication is initialized because it will call setlocale)
...
void ApplicationBase::parse_cmd(int &argc, char **argv) {
  ...
  if (!m_no_gui) {
    ...
    //  create the configuration files paths and collect the initialization
    //  config files (the ones used for reset) into m_initial_config_files.
    {
      //  Fallback to ~/.layviewrc for backward compatibility
      QDir qd(QDir::home());
      QString filename = QString::fromUtf8(".layviewrc");
      if (qd.exists(filename) &&
          QFileInfo(qd.absoluteFilePath(filename)).isReadable()) {
    ...
    if (!m_appdata_path.empty()) {
      m_config_file_to_write =
          tl::to_string(QDir(tl::to_qstring(m_appdata_path))
                            .absoluteFilePath(QString::fromUtf8("klayoutrc")));
    }

    //  Hint: the order is reverse in the sense that the first one wins ...
    for (std::vector<std::string>::const_iterator p = m_klayout_path.end();
         p != m_klayout_path.begin();) {
      --p;
  ...
  if (!args.empty() &&
      std::string(
          tl::to_string(QFileInfo(tl::to_qstring(args.front())).fileName()), 0,
          vo_exe_name.size()) == vo_exe_name) {
    m_vo_mode = true;
  }
#endif
  ...
  // currently: technology is always set to make "default" technology the
  // default
  bool tech_set = true;
  std::string tech;
    ...
    if (a == "-d" && (i + 1) < argc) {
      ...
      if (!m_run_macro.empty()) {
        throw tl::Exception(tl::to_string(
            QObject::tr("Option -r or -rr can only be used once")));
      }
      m_run_macro = args[++i];
      ...
      m_custom_macro_paths.push_back(
          std::pair<std::string, std::string>(args[++i], std::string()));
      ...
      m_files.push_back(std::make_pair(
          rdb_file, std::make_pair(std::string(args[++i]), std::string())));

    } else if (a == "-mn" && (i + 1) < argc) {

      m_files.push_back(std::make_pair(
          l2ndb_file, std::make_pair(std::string(args[++i]), std::string())));
      ...
      if (!tech.empty()) {
        m_files.push_back(
            std::make_pair(layout_file_with_tech, std::make_pair(a, tech)));
      } else if (!tech_file.empty()) {
        m_files.push_back(std::make_pair(layout_file_with_tech_file,
                                         std::make_pair(a, tech_file)));
      } else {
        m_files.push_back(
            std::make_pair(layout_file, std::make_pair(a, std::string())));
      }
...
void ApplicationBase::init_app() {
  ...
  //    - in a folder named by the architecture (i.e. "i686-win32-mingw" or
  //    "x86_64-linux-gcc") below
  ...
  for (std::vector<std::string>::const_iterator p = m_klayout_path.begin();
       p != m_klayout_path.end(); ++p) {

    klp_paths.push_back(*p);
    klp_paths.push_back(
        tl::to_string(QDir(tl::to_qstring(klp_paths.back()))
                          .filePath(tl::to_qstring(tl::arch_string()))));

    lay::Salt salt;
    salt.add_location(tl::to_string(
        QDir(tl::to_qstring(*p)).filePath(QString::fromUtf8("salt"))));

    //  Build the search path for the plugin locations. The search priority is
    //  for example:
    ...
    for (lay::Salt::flat_iterator g = salt.begin_flat(); g != salt.end_flat();
         ++g) {
      QDir dir = QDir(tl::to_qstring((*g)->path()));
      klp_paths.push_back(tl::to_string(dir.filePath(
          tl::to_qstring(arch_string + "-" + lay::Version::version()))));
      if (vv.size() >= 2) {
        klp_paths.push_back(tl::to_string(dir.filePath(
            tl::to_qstring(arch_string + "-" + vv[0] + "." + vv[1]))));
      }
      if (vv.size() >= 1) {
        klp_paths.push_back(tl::to_string(
            dir.filePath(tl::to_qstring(arch_string + "-" + vv[0]))));
      }
      klp_paths.push_back(tl::to_string(dir.filePath(tl::to_qstring(
          arch_string + "-" + tl::to_string(lay::Version::version())))));
      klp_paths.push_back(
          tl::to_string(dir.filePath(tl::to_qstring(arch_string))));
      klp_paths.push_back(
          tl::to_string(dir.filePath(tl::to_qstring(short_arch_string))));
  ...
  //  We have to do this now since plugins may register GSI classes and before
  //  the ruby interpreter, because it depends on a proper class system.
  gsi::initialize();
  ...
  //  Hint: we do this after load_plugin, because that way the plugins can
  //  register GSI classes and methods.
...

  {
    ...
    for (std::vector<std::string>::const_iterator c = m_config_files.begin();
         c != m_config_files.end(); ++c) {
      try {
        cfg.read_config(*c);
      } catch (...) {
      }
    ...
    try {
      ...
      while (!ex.at_end()) {
        ...
        m_custom_macro_paths.push_back(
            std::pair<std::string, std::string>(p, std::string()));
        if (ex.test(":")) {
  ...
  if (tc) {
    ...
    for (auto t = db::Technologies::instance()->begin();
         t != db::Technologies::instance()->end(); ++t) {
      tech_disambiguator.insert(std::make_pair(t->name(), 0));
    }

    //  import technologies from the command line
    for (std::vector<std::pair<
             file_type, std::pair<std::string, std::string>>>::iterator f =
             m_files.begin();
         f != m_files.end(); ++f) {
      ...
      if (f->first == layout_file_with_tech_file) {
        ...
        //  disambiguate the name, so we do not overwrite technologies with the
        //  same name from the config
        if (tech_name_for_file.find(tech_file) != tech_name_for_file.end()) {
          t.set_name(tech_name_for_file[tech_file]);
        } else if (tech_disambiguator.find(t.name()) !=
                   tech_disambiguator.end()) {
  ...
  if (mc) {
    ...
    m_load_macros.insert(m_load_macros.begin(), global_modules.begin(),
                         global_modules.end());
    ...
    for (std::vector<std::string>::const_iterator p = m_klayout_path.begin();
      ...
      if (p - m_klayout_path.begin() < local_folders) {
        mc->add_path(*p, tl::to_string(QObject::tr("Local")), std::string(),
                     false);
      } else if (m_klayout_path.size() == 1 + local_folders) {
        mc->add_path(*p, tl::to_string(QObject::tr("Global")), std::string(),
                     true);
      } else {
        mc->add_path(*p, tl::to_string(QObject::tr("Global")) + " - " + *p,
                     std::string(), true);
      }
    }
    ...
    for (std::vector<std::pair<std::string, std::string>>::const_iterator p =
             m_custom_macro_paths.begin();
         p != m_custom_macro_paths.end(); ++p) {
      mc->add_path(p->first,
                   tl::to_string(QObject::tr("Project")) + " - " + p->first,
                   p->second, false);
    }
  ...
  if (!m_gtf_record.empty()) {
    //  since the recorder tracks QAction connections etc., it must be
    //  instantiated before every other object performing a gtf::action_connect
    //  for example
  ...
  for (std::list<lay::PluginDescriptor>::const_iterator p =
           lay::plugins().begin();
       p != lay::plugins().end(); ++p) {
    if (p->autorun_early) {
      (*p->autorun_early)();
  ...
  //  redo gsi::initialize as the macros may have registered new external
  //  classes through the "doc to external class" mechanism.
  gsi::initialize();

  //  autorun_early may have added macro categories, so we need to call finish()
  //  again
  if (mc) {

    mc->finish();

    //  as this regenerates the macro collection, autorun_early is required
    //  again note: this does no re-execute macros that have been executed
    //  already
    lym::MacroCollection::root().autorun_early(&already_executed);
  }
  ...
  if (tl::verbosity() >= 20) {
    ...
    for (std::vector<std::string>::const_iterator c = m_klayout_path.begin();
         c != m_klayout_path.end(); ++c) {
      tl::info << "  " << *c;
    }
...
void ApplicationBase::add_macro_category(
...
std::vector<std::string> ApplicationBase::scan_global_modules() {
  //  NOTE:
  //  this is deprecated functionality - for backward compatibility, global
  //  "*.rbm" and "*.pym" modules are still considered. The desired solution is
  //  autorun macros.
  ...
  for (std::vector<std::string>::const_iterator p = m_klayout_path.begin();
    ...
    for (QStringList::const_iterator im = inst_modules.begin();
         im != inst_modules.end(); ++im) {
      ...
      if (rbm_file.exists() && rbm_file.isReadable()) {
        ...
        if (modules.find(m) == modules.end()) {
          tl::warn << tl::to_string(
              QObject::tr(
                  "Global modules are deprecated. Turn '%1' into an autorun "
                  "macro instead and put it into 'macros' or 'pymacros'.")
                  .arg(tl::to_qstring(m)));
          global_modules.push_back(m);
          modules.insert(m);
        }
...
void ApplicationBase::finish() {
  if (dispatcher() && m_write_config_file) {
    ...
    if (!m_config_file_to_write.empty()) {
      if (tl::verbosity() >= 20) {
        tl::info << tl::to_string(QObject::tr("Updating configuration file "))
                 << m_config_file_to_write;
      }
      dispatcher()->write_config(m_config_file_to_write);
    }
    if (!m_config_file_to_delete.empty() &&
        m_config_file_to_delete != m_config_file_to_write) {
      if (tl::verbosity() >= 20) {
        tl::info << tl::to_string(QObject::tr("Deleting configuration file "))
                 << m_config_file_to_delete;
      }
...
void ApplicationBase::prepare_recording(const std::string & /*gtf_record*/,
                                        bool /*gtf_record_incremental*/) {
...
ApplicationBase *ApplicationBase::instance() { return ms_instance; }

std::string ApplicationBase::version() {
  return std::string(lay::Version::name()) + " " + lay::Version::version();
}

std::string ApplicationBase::usage() {
  std::string r;
  r = std::string(lay::Version::exe_name()) + " [<options>] [<file>] ..\n";
  r += tl::to_string(QObject::tr("options")) + "\n";
  r += tl::to_string(QObject::tr(
           "  -b                  Batch mode (same as -zz -nc -rx)")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -c <config file>    Use this configuration file")) +
       "\n";
  r += tl::to_string(QObject::tr("  -nc                 Don't use a "
                                 "configuration file (implies -t)")) +
       "\n";
  ...
  r += tl::to_string(QObject::tr(
       ...
       "\n";
  r += tl::to_string(QObject::tr("  -ne                 Readonly mode (editing "
                                 "of files is disabled)")) +
       "\n";
  r +=
      tl::to_string(QObject::tr("  -gr <file name>     Record GUI test file")) +
      "\n";
  r +=
      tl::to_string(QObject::tr("  -gp <file name>     Replay GUI test file")) +
      "\n";
  r += tl::to_string(QObject::tr("  -gb <line number>   Replay GUI test file "
                                 "up to (including) line")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -gx <millisec>      Replay rate for GUI test file")) +
       "\n";
  r += tl::to_string(QObject::tr(
           "  -gi                 Incremental logs for GUI test file")) +
       "\n";
  r += tl::to_string(QObject::tr("  -i                  Disable undo buffering "
                                 "(less memory requirements)")) +
       "\n";
  r += tl::to_string(QObject::tr("  -ni                 Enable undo buffering "
                                 "(default, overrides previous -i option)")) +
       "\n";
  r += tl::to_string(QObject::tr("  -j <path>           Add the given path to "
                                 "the macro project paths")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -l <lyp file>       Use layer properties file")) +
       "\n";
  r += tl::to_string(QObject::tr(
           "  -lx                 With -l: add other layers as well")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -lf                 With -l: use the lyp file as it "
                       "is (no expansion to multiple layouts)")) +
       "\n";
  r +=
      tl::to_string(QObject::tr("  -m <database file>  Load RDB (report "
                                "database) file (into previous layout view)")) +
      "\n";
  r += tl::to_string(
           QObject::tr("  -mn <database file> Load L2NDB (layout to netlist "
                       "database) file (into previous layout view)")) +
       "\n";
  r += tl::to_string(QObject::tr("  -n <technology>     Technology to use for "
                                 "next layout(s) on command line")) +
       "\n";
  r += tl::to_string(QObject::tr("  -nn <tech file>     Technology file (.lyt) "
                                 "to use for next layout(s) on command line")) +
       "\n";
  r += tl::to_string(QObject::tr("  -p <plugin>         Load the plugin (can "
                                 "be used multiple times)")) +
       "\n";
  r += tl::to_string(QObject::tr("  -r <script>         Execute main script on "
                                 "startup (after having loaded files etc.)")) +
       "\n";
  r += tl::to_string(QObject::tr("  -rr <script>        Like -r, but does not "
                                 "exit after executing the script")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -rm <script>        Execute script on startup before "
                       "loading files (can be used multiple times)")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -rd <name>=<value>  Specify script variable")) +
       "\n";
  r += tl::to_string(QObject::tr("  -rx                 Ignore all implicit "
                                 "macros (*.rbm, rbainit, *.lym)")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -s                  Load files into same view")) +
       "\n";
  r += tl::to_string(QObject::tr("  -t                  Don't update the "
                                 "configuration file on exit")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -nt                 Update the configuration file on "
                       "exit (default, overrides previous -t option)")) +
       "\n";
  r += tl::to_string(QObject::tr(
  ...
  r += tl::to_string(QObject::tr(
       ...
       "\n";
  r += tl::to_string(
           QObject::tr("  -x                  Synchronous drawing mode")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -y <package>        Package installation: install "
                       "package(s) and exit - can be used more than once")) +
       "\n";
  r += tl::to_string(
           QObject::tr("                      ('package' is a name, an URL and "
                       "optionally a version in round brackets)")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -yd                 With -y: include dependencies")) +
       "\n";
  r += tl::to_string(
           QObject::tr("  -z                  Non-GUI mode (hidden views)")) +
       "\n";
  r += tl::to_string(QObject::tr(
...
int ApplicationBase::run() {
  ...
  for (std::vector<std::string>::const_iterator c = m_config_files.begin();
    ...
    config_failed = false;
    END_PROTECTED_CLEANUP { config_failed = true; }
  }
  ...
  for (std::vector<std::pair<std::string, std::string>>::const_iterator v =
           m_variables.begin();
       v != m_variables.end(); ++v) {
  ...
  for (std::vector<std::string>::const_iterator m = m_load_macros.begin();
       m != m_load_macros.end(); ++m) {
  ...
  if (mw) {
    ...
    for (std::vector<std::pair<
             file_type, std::pair<std::string, std::string>>>::const_iterator
             f = m_files.begin();
         f != m_files.end(); ++f) {
      ...
      if (f->first == layout_file || f->first == layout_file_with_tech) {
        ...
        if (f->first != layout_file_with_tech) {
          ...
          mw->load_layout(f->second.first,
                          m_same_view ? 2 /*same view*/ : 1 /*new view*/);
        } else {
          mw->add_mru(f->second.first, f->second.second);
          mw->load_layout(f->second.first, f->second.second,
                          m_same_view ? 2 /*same view*/ : 1 /*new view*/);
        }
        ...
        if (mw->current_view() != 0) {
          ...
          mw->current_view()->open_rdb_browser(
              rdb_index, mw->current_view()->active_cellview_index());
        }
        ...
        if (mw->current_view() != 0) {
          int l2ndb_index = mw->current_view()->add_l2ndb(
              db::LayoutToNetlist::create_from_file(f->second.first));
          mw->current_view()->open_l2ndb_browser(
              l2ndb_index, mw->current_view()->active_cellview_index());
        }
      }
    }
    ...
    if (!m_layer_props_file.empty()) {
      ...
      mw->load_layer_properties(m_layer_props_file, true /*all views*/,
                                m_lyp_add_default);

      tl::log << "Layer properties loaded '" << m_layer_props_file << "'";

      //  because the layer may carry transformations, we need to refit the
      //  cellviews.
    ...
    for (std::vector<std::pair<
      ...
      if (f->first == layout_file || f->first == layout_file_with_tech) {
        ...
        if (batch_mode_view.get() != 0 && !m_same_view) {
          tl::warn << tl::sprintf(
              tl::to_string(
                  tr("Ignoring additional views in batch mode (file %s)")),
              filename);
          continue;
        }
...
lay::LayoutView *ApplicationBase::create_view(db::Manager &manager) {
  //  create a new view
  lay::LayoutView *view = new lay::LayoutView(
      &manager, lay::ApplicationBase::instance()->is_editable(), dispatcher());
...
static void dump_children(QObject *obj, int level = 0) {
  ...
  if (!children.isEmpty() || !obj->objectName().isEmpty()) {
    ...
    tl::info << info;
    for (QObjectList::const_iterator child = children.begin();
...
void ApplicationBase::process_events_impl(
    QEventLoop::ProcessEventsFlags /*flags*/, bool /*silent*/) {
  //  in the non-UI case there are no events, but we can at least schedule
  //  deferred method calls.
  tl::DeferredMethodScheduler::execute();
}

bool ApplicationBase::write_config(const std::string &config_file) {
  return dispatcher() ? dispatcher()->write_config(config_file) : 0;
}

void ApplicationBase::reset_config() {
  clear_config();
  for (std::vector<std::string>::const_iterator c =
           m_initial_config_files.begin();
       c != m_initial_config_files.end(); ++c) {
    try {
      read_config(*c);
    } catch (...) {
    }
...
void ApplicationBase::set_config(const std::string &name,
...
std::string ApplicationBase::get_config(const std::string &name) const {
  if (dispatcher()) {
...
std::vector<std::string> ApplicationBase::get_config_names() const {
...
GuiApplication::GuiApplication(int &argc, char **argv)
    : QApplication(argc, argv), ApplicationBase(false), mp_mw(0),
      mp_recorder(0), m_in_notify(0) {
  //  install a special style proxy to overcome the issue of black-on-black tree
  //  expanders
...
bool GuiApplication::notify(QObject *receiver, QEvent *e) {
  ...
  if (dynamic_cast<QPaintEvent *>(e)) {
    //  NOTE: we don't want recursive paint events - the painters are not
    //  reentrant. Hence we disable process_events_impl (specifically for
    //  progress reporters).
...
bool GuiApplication::do_notify(QObject *receiver, QEvent *e) {
  ...
  //  Note: due to a bug in some Qt versions (i.e. 4.8.3) throwing exceptions
  //  across signals may not be safe. Hence the local BEGIN_PROTECTED ..
  //  END_PROTECTED approach is still preferred over the global solution through
  //  "notify". Because END_PROTECTED may raise other events (message box) and
  //  this may cause other exceptions, we use silent mode inside notify to avoid
  //  recursion.
...
bool GuiApplication::is_busy() const { return mp_mw && mp_mw->is_busy(); }
...
bool GuiApplication::event(QEvent *event) {
  ...
  //
  // This event interceptor catches MacOS "Open With" event, and KLayout should
  // respond similarly to the Drop event in MainWindow::dropEvent.
  //
  // This particular implementation always creates a new window.
  //
  // This was implemented with the inspiration of
...
int GuiApplication::exec() {
  ...
  if (tl::verbosity() >= 40) {
    ...
    for (QWidgetList::const_iterator tl = tl_widgets.begin();
         tl != tl_widgets.end(); ++tl) {
    ...
    for (QWidgetList::const_iterator tl = tl_widgets.begin();
      ...
      if (!(*tl)->objectName().isEmpty()) {
        ...
        if (!actions.isEmpty()) {
          ...
          for (QList<QAction *>::const_iterator a = actions.begin();
               a != actions.end(); ++a) {
...
void GuiApplication::shutdown() {
  //  avoid deferred execution later on where there isn't a valid main window
  //  anymore (problem case: showing a dialog inside main windows's destroyed
  //  signal - this will process events and trigger execution if not disabled)
  ...
  //  detach all top level widgets from Ruby/Python - we don't want the
  //  interpreter do this for us. Qt will delete all top level widgets itself.
  //  NOTE: we must only detach (= "keep" on C++ side), not delete them as top
  //  level widgets may be owned by someone else.
  QWidgetList tl_widgets = topLevelWidgets();
  for (QWidgetList::iterator w = tl_widgets.begin(); w != tl_widgets.end();
       ++w) {
...
void GuiApplication::prepare_recording(const std::string &gtf_record,
                                       bool gtf_save_incremental) {
...
lay::Dispatcher *GuiApplication::dispatcher() const {
...
void GuiApplication::process_events_impl(QEventLoop::ProcessEventsFlags flags,
  ...
  if (mp_mw) {
    ...
    try {
      ...
      //  Qt seems not to send posted UserEvents in some cases (e.g. in the unit
      //  test application with GLib? Glib not doing this without a main window
      //  visible?). Hence we do this explicitly here.
...
NonGuiApplication::NonGuiApplication(int &argc, char **argv)
    : QCoreApplication(argc, argv), ApplicationBase(true), mp_pr(0), mp_pb(0),
      mp_dispatcher(0) {
  //  .. nothing yet ..
}
