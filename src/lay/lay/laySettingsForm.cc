...
SettingsForm::SettingsForm(QWidget *parent, lay::Dispatcher *dispatcher,
                           const char *name)
    : QDialog(parent), Ui::SettingsForm(), mp_dispatcher(dispatcher),
  ...
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
    ...
    std::vector<std::pair<std::string, lay::ConfigPage *>> pp =
        cls->config_pages(0);
  ...
  for (std::vector<std::pair<std::string, lay::ConfigPage *>>::iterator p =
           pages.begin();
       p != pages.end(); ++p) {
    ...
    if (config_page) {
      ...
      std::map<std::string, int>::const_iterator t =
          stack_headers.find(config_title);
      if (t == stack_headers.end()) {
        ...
        while (!config_title.empty()) {
          ...
          int count =
              parent ? parent->childCount() : items_tree->topLevelItemCount();
          ...
          } else if (child->data(0, Qt::UserRole).toInt() == 0 &&
                     current_index > 0) {
            child->setData(0, Qt::UserRole, QVariant(current_index));
          }
        ...
        QScrollArea *page =
            dynamic_cast<QScrollArea *>(pages_stack->widget(t->second));
        if (page) {
  ...
  for (std::vector<QTreeWidgetItem *>::iterator i = items.begin();
       i != items.end(); ++i) {
  ...
  for (std::map<std::string, int>::const_iterator t = stack_headers.begin();
       t != stack_headers.end(); ++t) {
    QScrollArea *page =
        dynamic_cast<QScrollArea *>(pages_stack->widget(t->second));
    if (page) {
      QFrame *page_frame = dynamic_cast<QFrame *>(page->widget());
...
void SettingsForm::setup() {
  ...
  for (std::vector<lay::ConfigPage *>::iterator cp = m_config_pages.begin();
       cp != m_config_pages.end(); ++cp) {
...
void SettingsForm::reset_clicked() {
  if (QMessageBox::question(
          this, QObject::tr("Confirm Reset"),
          QObject::tr(
              "Are you sure to reset the configuration?\nThis operation will "
              "clear all custom settings and cannot be undone."),
