...
bool SaltGrains::operator==(const SaltGrains &other) const {
  return m_name == other.m_name && m_path == other.m_path &&
         m_title == other.m_title && m_collections == other.m_collections &&
         m_grains == other.m_grains;
}

void SaltGrains::set_name(const std::string &n) { m_name = n; }

void SaltGrains::set_title(const std::string &t) { m_title = t; }

void SaltGrains::set_path(const std::string &p) { m_path = p; }
...
bool SaltGrains::remove_collection(collection_iterator iter, bool with_files) {
  //  NOTE: this is kind of inefficient, but in order to maintain the const
  //  iterator semantics this approach is required
  for (collections_type::iterator i = m_collections.begin();
       i != m_collections.end(); ++i) {
    if (i == iter) {
...
bool SaltGrains::is_empty() const {
  ...
  for (collections_type::const_iterator i = m_collections.begin();
       i != m_collections.end(); ++i) {
    if (!i->is_empty()) {
      return false;
    }
 ...
 * by QResource directly
 */
class OpenResource : public QResource {
public:
  using QResource::children;
  using QResource::isDir;
  using QResource::isFile;

  OpenResource(const QString &path) : QResource(path) {
    //  .. nothing yet ..
  }
};
...
SaltGrains SaltGrains::from_path(const std::string &path,
  ...
  if (path[0] != ':') {
    ...
    QStringList entries =
        dir.entryList(QDir::NoDotAndDotDot | QDir::Dirs, QDir::Name);
...
void SaltGrains::merge_with(const lay::SaltGrains &other) {
  for (lay::SaltGrains::collection_iterator c = other.begin_collections();
       c != other.end_collections(); ++c) {
    add_collection(*c);
  }
  for (lay::SaltGrains::grain_iterator g = other.begin_grains();
       g != other.end_grains(); ++g) {
    add_grain(*g);
  }
  consolidate();
}
...
void SaltGrains::consolidate() {
  ...
  for (collections_type::iterator c = m_collections.begin();
       c != m_collections.end(); ++c) {

    std::map<std::string, collections_type::iterator>::iterator cn =
        collection_by_name.find(c->name());
  ...
  for (std::vector<collections_type::iterator>::reverse_iterator i =
           collection_to_delete.rbegin();
       i != collection_to_delete.rend(); ++i) {
    remove_collection(*i);
  }
  ...
  for (lay::SaltGrains::grain_iterator g = begin_grains(); g != end_grains();
       ++g) {

    std::map<std::string, lay::SaltGrains::grain_iterator>::iterator gn =
        grain_by_name.find(g->name());
    if (gn != grain_by_name.end()) {

      //  take the one with the higher version. On equal version use the first
      //  one.
      if (lay::SaltGrain::compare_versions(gn->second->version(),
                                           g->version()) < 0) {
  ...
  for (std::vector<lay::SaltGrains::grain_iterator>::reverse_iterator i =
           to_delete.rbegin();
       i != to_delete.rend(); ++i) {
    remove_grain(*i);
  }
...
static tl::XMLElementList s_group_struct =
    ...
    tl::make_element(&SaltGrains::begin_collections,
                     &SaltGrains::end_collections, &SaltGrains::add_collection,
                     "group", &s_group_struct) +
    tl::make_element(&SaltGrains::begin_grains, &SaltGrains::end_grains,
                     &SaltGrains::add_grain, "salt-grain",
...
void SaltGrains::include(const std::string &src_in) {
  if (!src_in.empty()) {
    ...
    if (!m_url.empty() && src.find("http:") != 0 && src.find("https:") != 0 &&
        src.find("file:") != 0 && !src.empty() && src[0] != '/' &&
        src[0] != '\\') {
