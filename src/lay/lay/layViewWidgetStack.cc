...
#endif
  mp_bglabel->setText(
      QObject::tr("<html><body><p><img src=\":/%1\" width=\"256\" "
                  "height=\"256\"/></p><p>Use File/Open to open a "
                  "layout</p></body></html>")
...
void ViewWidgetStack::raise_widget(size_t index) {
  ...
  size_t i = 0;
  for (std::vector<LayoutViewWidget *>::iterator child = m_widgets.begin();
       child != m_widgets.end(); ++child, ++i) {
    if (i != index) {
      (*child)->hide();
    }
  }
...
QWidget *ViewWidgetStack::background_widget() { return mp_bglabel; }

void ViewWidgetStack::resize_children() {
  //  set the geometry of all children
  for (std::vector<LayoutViewWidget *>::iterator child = m_widgets.begin();
       child != m_widgets.end(); ++child) {
