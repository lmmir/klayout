...
static std::string format_hex_dump(const std::string &s) {
  ...
  hex_dump.reserve((s.size() / bytes_per_line + 1) * (8 + bytes_per_line * 4) +
                   100);

  const char *ce = s.c_str() + s.size();
  for (const char *c = s.c_str(); c + bytes_per_line <= ce;
       c += bytes_per_line) {
...
ReaderErrorForm::ReaderErrorForm(QWidget *parent, const char *name,
                                 const db::ReaderUnknownFormatException *error)
  ...
  QMessageBox *mb =
      new QMessageBox(QMessageBox::Critical, QString(), QString());
