...
class ClipDialogPluginDeclaration : public lay::PluginDeclaration {
public:
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::menu_item(
        "clip_tool::show", "clip_tool:edit_mode", "edit_menu.utils_menu.end",
        tl::to_string(QObject::tr("Clip Tool"))));
  }
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new ClipDialogPluginDeclaration(), 20000, "ClipDialogPlugin");

// ------------------------------------------------------------

ClipDialog::ClipDialog(lay::Dispatcher *root, LayoutViewBase *vw)
    : lay::Browser(root, vw), Ui::ClipDialog() {
...
void ClipDialog::ok_pressed() {
  ...
  if (clip_cell_name.empty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Clip cell name must not be empty")));
  }
  ...
  if (rb_box1->isChecked()) {
    ...
    if (le_x1->text().isEmpty() || le_x2->text().isEmpty() ||
      ...
      throw tl::Exception(tl::to_string(
          QObject::tr("All four coordinates of the clip box must be given")));
    }
    ...
    clip_boxes.push_back(
        db::Box(db::DBox(db::DPoint(x - 0.5 * w, y - 0.5 * h),
                         db::DPoint(x + 0.5 * w, y + 0.5 * h)) *
                (1.0 / cv->layout().dbu())));
    ...
    if (ant_service) {
      ...
      while (!ant.at_end()) {
        clip_boxes.push_back(db::Box(db::DBox(ant->p1(), ant->p2()) *
                                     (1.0 / cv->layout().dbu())));
        ++ant;
      }
    }
    ...
    if (sel_layer < 0 || !cv->layout().is_valid_layer(sel_layer)) {
      throw tl::Exception(tl::to_string(
          QObject::tr("No valid layer selected to get clip boxes from")));
    }

    db::collect_clip_boxes(cv->layout(), cv.cell_index(),
                           (unsigned int)sel_layer, clip_boxes);
  }
  ...
  if (!clip_boxes.empty()) {
    ...
    clip_boxes.erase(std::unique(clip_boxes.begin(), clip_boxes.end()),
                     clip_boxes.end());

    std::vector<db::cell_index_type> new_cells = db::clip_layout(
        cv->layout(), cv->layout(), cv.cell_index(), clip_boxes, false);

    if (new_cells.size() > 1) {

      //  need to create a new master top cell
      db::cell_index_type clip_top =
          cv->layout().add_cell(clip_cell_name.c_str());
      ...
      cv->layout().rename_cell(
          new_cells[0],
          cv->layout().uniquify_cell_name(clip_cell_name.c_str()).c_str());
...
bool ClipDialog::configure(const std::string & /*name*/,
