 ...
 */
class LAY_PUBLIC SaltGrain : public tl::Object {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  @brief Inequality
   */
  bool operator!=(const SaltGrain &other) const { return !operator==(other); }

  /**
   *  @brief Gets the name of the grain
   *
   *  The name is either a plain name (a word) or a path into a collection.
   *  Name paths are formed using the "/" separator. "mypackage" is a plain
   * name, while "mycollection/mypackage" is a package within a collection.
   * Collections can be used to group packages. Names are case sensitive in
   * general, but names differing only in case should be avoided.
   */
   ...
   */
  const std::string &doc_url() const { return m_doc_url; }
   ...
   */
  const std::string &api_version() const { return m_api_version; }
   ...
   */
  const std::string &author_contact() const { return m_author_contact; }
   ...
   */
  const QDateTime &authored_time() const { return m_authored_time; }
   ...
   */
  const QDateTime &installed_time() const { return m_installed_time; }
   ...
   */
  const QImage &icon() const { return m_icon; }
   ...
   *  The download URL is the place from which the grain was installed
   * originally.
   */
   ...
   */
  std::vector<SaltGrainDependency> &dependencies() { return m_dependencies; }
  ...
  /**
   *  @brief Saves the data to the path inside the grain folder given by the
   * "path" property
   */
  void save() const;
   ...
   *  Malformed versions are read gracefully. Letters and non-digits are
   * skipped. Missing numbers are read as 0. Hence "1.0 == 1" for example.
   */
  static int compare_versions(const std::string &v1, const std::string &v2);

  /**
   *  @brief Gets a value indicating whether the given version string is a valid
   * version
   */
   ...
   *  exception if an error occurs during reading.
   */
  static SaltGrain from_url(const std::string &url, double timeout = 60.0,
  ...
  stream_from_url(std::string &url, double timeout = 60.0,
                  tl::InputHttpStreamCallback *callback = 0);
  ...
  /**
   *  @brief Returns a value indicating whether the given path represents is a
   * grain
   */
