...
public:
  ...
  void add_info(const std::string &name, bool update,
 ...
 *   + "register_download" (multiple times) to register the packages intended
 * for download
 ...
 */
class LAY_PUBLIC SaltDownloadManager : public QObject {
  Q_OBJECT
...
public:
  ...
  /**
   *  @brief Registers an URL (with version) for download in the given target
   * directory
   *
   *  The target directory can be empty. In this case, the downloader will pick
   * an appropriate one.
   */
  void register_download(const std::string &name, const std::string &token,
                         const std::string &url, const std::string &version);

  /**
   *  @brief Computes the dependencies after all required packages have been
   * registered
   *
   *  This method will compute the dependencies. Packages not present in the
   * list of packages ("salt" argument), will be scheduled for download too.
   * Dependency packages are looked up in "salt_mine" if no download URL is
   * given.
   */
  void compute_dependencies(const lay::Salt &salt, const Salt &salt_mine);

  /**
   *  @brief Computes the list of packages after all required packages have been
   * registered
   *
   *  This method will removes packages that are already installed and satisfy
   * the version requirements. Packages not present in the list of packages
   * ("salt" argument), will be scheduled for download too. No dependencies are
   ...
   *
   *  If parent is non-null, this method will show a confirmation dialog and
   * start installation if this dialog is confirmed. It will return false if the
   * dialog was cancelled and an exception if something goes wrong.
   *
   *  If dialog is null, no confirmation dialog will be shown and installation
...
private:
  struct Descriptor {
    Descriptor(const std::string &_name, const std::string &_token,
               const std::string &_url, const std::string &_version)
        : name(_name), token(_token), url(_url), version(_version),
          downloaded(false) {}
  ...
  void fetch_missing(const lay::Salt &salt, const lay::Salt &salt_mine,
                     tl::AbsoluteProgress &progress);
  lay::ConfirmationDialog *make_confirmation_dialog(QWidget *parent,
                                                    const lay::Salt &salt);
  void compute_list(const lay::Salt &salt, const lay::Salt &salt_mine,
                    bool with_dep);
};
