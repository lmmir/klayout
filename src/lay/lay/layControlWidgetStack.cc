...
ControlWidgetStack::ControlWidgetStack(QWidget *parent, const char *name,
                                       bool size_follows_content)
    : QFrame(parent), mp_current_widget(0),
      m_size_follows_content(size_follows_content) {
...
QWidget *ControlWidgetStack::background_widget() { return mp_bglabel; }

void ControlWidgetStack::resize_children() {
  //  set the geometry of all children
  for (std::vector<QWidget *>::iterator child = m_widgets.begin();
