 ...
 *  Two specializations exist: one for the GUI-less version (derived from
 * QCoreApplication) and one for the GUI version (derived from QApplication).
 */
class LAY_PUBLIC ApplicationBase : public gsi::ObjectBase, public tl::Object {
public:
  /**
   ...
   *
   *  If the application has not been initialized properly or does not support
   * GUI, this pointer is 0.
   */
   ...
   *
   *  This method issues all the lower level methods required in order to
   * perform the applications main code. Depending on the arguments and UI
   * capabilities, this method will either execute the command line macros for
   ...
   *
   *  This is basically a wrapper for the QApplication::processEvents method. It
   * provides some special handling for the "close application window" case and
   * a "silent" mode. In that mode, processing of deferred methods is disabled.
   */
  void process_events(QEventLoop::ProcessEventsFlags flags,
                      bool silent = false) {
   ...
   */
  void process_events() { process_events_impl(QEventLoop::AllEvents); }
   ...
   */
  gsi::Interpreter &ruby_interpreter() { return *mp_ruby_interpreter; }

  /**
   *  @brief Return a reference to the Ruby interpreter
   */
  gsi::Interpreter &python_interpreter() { return *mp_python_interpreter; }
   ...
   */
  bool is_undo_enabled() const { return m_enable_undo; }

  /**
   *  @brief Returns true, if the application is in pure "viewer only" mode
   */
  bool is_vo_mode() const { return m_vo_mode; }
   ...
   *  Setting this flag does not have an immediate effect. It will only be
   * effective for new views.
   */
   ...
   */
  bool has_gui() const { return !m_no_gui; }
   ...
   */
  const std::string &inst_path() const { return m_inst_path; }

  /**
   *  @brief Obtain the application data path
   */
  const std::string &appdata_path() const { return m_appdata_path; }
  ...
  /**
   *  @brief Parses the given command line arguments and configures the
   * application object accordingly.
   */
  void parse_cmd(int &argc, char **argv);

  /**
   *  @brief Initializes the application
   *  This method needs to be called after "parse_cmd" and before the
   * application is used.
   */
  void init_app();
   ...
   *  This method will return non-null only if a GUI-enabled application is
   * present.
   */
...
protected:
  ...
  virtual void prepare_recording(const std::string &gtf_record,
                                 bool gtf_record_incremental);
  virtual void start_recording();
  virtual lay::Dispatcher *dispatcher() const = 0;
  virtual void finish();
  virtual void process_events_impl(QEventLoop::ProcessEventsFlags flags,
                                   bool silent = false);
...
private:
  ...
  std::vector<std::pair<file_type, std::pair<std::string, std::string>>>
      m_files;
  ...
  //  in order to maintain a valid MainWindow reference for ruby scripts and
  //  Ruby's GC all the time.
 ...
 */
class LAY_PUBLIC GuiApplication : public QApplication,
...
public:
   ...
   */
  void exit(int result) { ApplicationBase::exit(result); }

  /**
   *  @brief Returns the main window's reference
   */
  virtual MainWindow *main_window() const { return mp_mw; }

  /**
   *  @brief Enters busy mode (true) or leaves it (false)
   *
   *  Use lay::BusySection to declare a section in "busy" mode. In busy mode,
   * some features are disabled to prevent recursion in processing of events.
   */
 ...
 */
class LAY_PUBLIC NonGuiApplication : public QCoreApplication,
                                     public ApplicationBase {
public:
   ...
   */
  virtual MainWindow *main_window() const { return 0; }

protected:
  virtual void setup();
  virtual void shutdown();

  virtual lay::Dispatcher *dispatcher() const { return mp_dispatcher; }
