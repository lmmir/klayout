...
class NavigatorService : public ViewService, public tl::Object {
public:
  enum drag_mode_type { DM_none, DM_move, DM_l, DM_r, DM_t, DM_b };

  NavigatorService(LayoutView *view)
      : ViewService(view->canvas()), mp_view(view), mp_source_view(0),
        mp_viewport_marker(0), m_drag_mode(DM_none), m_dragging(false),
        mp_box(0), m_color(0) {
    //  .. nothing yet ..
  }
  ...
  void background_color_changed() {
    ...
    if (!c.is_valid()) {
      if (mp_view->widget()) {
        c = tl::Color(mp_view->widget()
                          ->palette()
                          .color(QPalette::Normal, QPalette::Base)
                          .rgb());
      } else {
  ...
  bool mouse_release_event(const db::DPoint & /*p*/, unsigned int /*buttons*/,
                           bool /*prio*/) {
    if (mp_box) {
  ...
  bool mouse_press_event(const db::DPoint &p, unsigned int buttons, bool prio) {
    ...
    } else if (prio && (buttons & lay::LeftButton) != 0 &&
               m_drag_mode != DM_none && mp_source_view) {
  ...
  bool wheel_event(int delta, bool /*horizontal*/, const db::DPoint &p,
    ...
    if (!prio) {
      ...
      if (mp_source_view) {
        ...
        if (direction == vertical) {
          ...
          mp_source_view->zoom_box(
              db::DBox(p.x() - (p.x() - m_box.left()) * f,
  ...
  bool mouse_move_event(const db::DPoint &p, unsigned int /*buttons*/,
                        bool prio) {
    ...
    if (mp_box) {
      ...
      if (prio) {
        ...
        mp_source_view->message(
            "w: " + tl::micron_to_string(fabs(m_p2.x() - m_p1.x())) +
            "  h: " + tl::micron_to_string(fabs(m_p2.y() - m_p1.y())));
      ...
      if (!m_box.empty()) {
        ...
        double mw =
            5.0 /*pixel*/ / mp_view->viewport().trans().ctrans(1.0 /*micron*/);
        db::DVector d = db::DVector(std::max(mw, m_box.width() * 0.5 - mw),
                                    std::max(mw, m_box.height() * 0.5 - mw));
        db::DBox move_box = db::DBox(m_box.center() - d, m_box.center() + d);
        db::DBox l_box = db::DBox(m_box.left() - mw, m_box.bottom(),
                                  m_box.left() + mw, m_box.top());
        db::DBox r_box = db::DBox(m_box.right() - mw, m_box.bottom(),
                                  m_box.right() + mw, m_box.top());
        db::DBox t_box = db::DBox(m_box.left(), m_box.top() - mw, m_box.right(),
                                  m_box.top() + mw);
        db::DBox b_box = db::DBox(m_box.left(), m_box.bottom() - mw,
                                  m_box.right(), m_box.bottom() + mw);
      ...
      if (m_drag_mode == DM_move) {
        ...
        new_box = db::DBox(m_b0.left() + dp.x(), m_b0.bottom() - dy,
                           m_b0.right(), m_b0.top() + dy);
      } else if (m_drag_mode == DM_r) {
        double new_h = m_b0.height() / m_b0.width() * (m_b0.width() + dp.x());
        double dy = (new_h - m_b0.height()) * 0.5;
        new_box = db::DBox(m_b0.left(), m_b0.bottom() - dy,
                           m_b0.right() + dp.x(), m_b0.top() + dy);
      } else if (m_drag_mode == DM_t) {
        double new_w = m_b0.width() / m_b0.height() * (m_b0.height() + dp.y());
        double dx = (new_w - m_b0.width()) * 0.5;
        new_box = db::DBox(m_b0.left() - dx, m_b0.bottom(), m_b0.right() + dx,
                           m_b0.top() + dp.y());
      } else if (m_drag_mode == DM_b) {
        double new_w = m_b0.width() / m_b0.height() * (m_b0.height() - dp.y());
        double dx = (new_w - m_b0.width()) * 0.5;
        new_box = db::DBox(m_b0.left() - dx, m_b0.bottom() + dp.y(),
                           m_b0.right() + dx, m_b0.top());
      }
  ...
  void update_marker() {
    ...
    if (mp_source_view) {
      ...
      // correct the box by a few pixels so it is more precisely reflecting the
      // actual dimensions
  ...
  void attach_view(LayoutView *source_view) {
    if (mp_source_view != source_view) {
      ...
      mp_source_view->viewport_changed_event.add(
          this, &NavigatorService::update_marker);

      mp_view->background_color_changed_event.add(
          this, &NavigatorService::background_color_changed);
...
const std::string
    freeze_action_path("@@navigator_menu.navigator_main_menu.navigator_freeze");

Navigator::Navigator(MainWindow *main_window)
    : QFrame(main_window), m_show_all_hier_levels(false), m_show_images(true),
      m_update_layers_needed(true), m_update_needed(true),
      mp_main_window(main_window), mp_source_view(0), mp_service(0),
  ...
  mp_placeholder_label->setSizePolicy(QSizePolicy::Expanding,
  ...
  mp_main_window->current_view_changed_event.add(this,
                                                 &Navigator::view_changed);
  mp_main_window->view_closed_event.add(this, &Navigator::view_closed);

  do_update_menu();
  connect(mp_main_window->menu(), SIGNAL(changed()), this,
          SLOT(menu_changed()));
}
...
void Navigator::freeze_clicked() {
  ...
  if (freeze_action->is_checked() && mp_source_view) {
    NavigatorFrozenViewInfo &info =
        m_frozen_list
            .insert(std::make_pair(mp_source_view, NavigatorFrozenViewInfo()))
            .first->second;
...
void Navigator::attach_view() { attach_view(mp_main_window->current_view()); }
...
void Navigator::attach_view(LayoutView *view) {
  if (view != mp_source_view) {
    ...
    if (mp_source_view) {
      ...
      mp_view = new LayoutViewWidget(
          0, false, mp_source_view, this,
          LayoutView::LV_Naked + LayoutView::LV_NoZoom +
              LayoutView::LV_NoServices + LayoutView::LV_NoGrid);
      ...
      mp_source_view->cellviews_changed_event.add(this,
                                                  &Navigator::content_changed);
      mp_source_view->cellview_changed_event.add(
          this, &Navigator::content_changed_with_int);
      mp_source_view->geom_changed_event.add(this, &Navigator::content_changed);
      mp_source_view->layer_list_changed_event.add(this,
                                                   &Navigator::layers_changed),
          mp_source_view->hier_levels_changed_event.add(
              this, &Navigator::hier_levels_changed);

      img::Service *image_plugin = mp_source_view->get_plugin<img::Service>();
      if (image_plugin) {
        image_plugin->images_changed_event.add(this,
                                               &Navigator::content_changed);
      }
      ...
      for (std::map<lay::LayoutView *, NavigatorFrozenViewInfo>::const_iterator
               f = m_frozen_list.begin();
           f != m_frozen_list.end(); ++f) {
      ...
      for (std::set<lay::LayoutView *>::const_iterator v = all_views.begin();
           v != all_views.end(); ++v) {
        all_views.erase(*v);
      }

      Action *freeze_action =
          mp_main_window->menu()->action(freeze_action_path);
      freeze_action->set_checked(m_frozen_list.find(mp_source_view) !=
                                 m_frozen_list.end());
...
void Navigator::hier_levels_changed() {
  if (m_show_all_hier_levels && mp_source_view &&
      m_frozen_list.find(mp_source_view) == m_frozen_list.end()) {
...
void Navigator::update() {
  ...
  if (m_frozen_list.find(mp_source_view) == m_frozen_list.end()) {
    ...
    mp_view->view()->select_cellviews(mp_source_view->cellview_list());
    mp_view->view()->set_properties(
        m_frozen_list[mp_source_view].layer_properties);
  }
  ...
  if (img_target) {
    ...
    if (m_show_images) {
      ...
      if (img_source) {
        for (img::ImageIterator i = img_source->begin_images(); !i.at_end();
             ++i) {
  ...
  if (m_show_all_hier_levels && mp_source_view) {
    if (m_frozen_list.find(mp_source_view) == m_frozen_list.end()) {
      ...
      mp_view->view()->set_hier_levels(
          m_frozen_list[mp_source_view].hierarchy_levels);
    }
  } else {
...
class NavigatorPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::submenu("navigator_main_menu", at,
                                        tl::to_string(QObject::tr("Options"))));

    at = "@@navigator_menu.navigator_main_menu.end";
    menu_entries.push_back(lay::config_menu_item(
        "navigator_show_images", at, tl::to_string(QObject::tr("Show Images")),
        cfg_navigator_show_images, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "navigator_all_hier_levels", at,
        tl::to_string(QObject::tr("Show All Hierarchy Levels")),
        cfg_navigator_all_hier_levels, "?"));
    menu_entries.push_back(lay::separator("navigator_options_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_navigator_freeze", "navigator_freeze", at,
                       tl::to_string(QObject::tr("Freeze"))));
    menu_entries.back().checkable = true;
  }
};

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new NavigatorPluginDeclaration(), -1, "NavigatorPlugin");
