...
static std::string full_name(const gsi::MethodBase::MethodSynonym &syn) {
  if (syn.is_predicate) {
    return syn.name + "?";
...
struct DocumentationParser {
  ...
  DocumentationParser(const gsi::ClassBase *cls) { parse_doc(cls->doc()); }
...
static DocumentationParser &cls_documentation(const gsi::ClassBase *cls) {
  ...
  std::map<const gsi::ClassBase *, DocumentationParser>::iterator cd =
      s_cls_doc.find(cls);
  if (cd != s_cls_doc.end()) {
    return cd->second;
  } else {
    return s_cls_doc.insert(std::make_pair(cls, DocumentationParser(cls)))
        .first->second;
  }
}
...
static const gsi::ClassBase *real_class(const gsi::ClassBase *cls) {
...
public:
  ...
  RecursiveClassIterator() {
    if (gsi::ClassBase::begin_classes() != gsi::ClassBase::end_classes()) {
      m_cls_iter_stack.push_back(std::make_pair(gsi::ClassBase::begin_classes(),
                                                gsi::ClassBase::end_classes()));
    }
  }

  bool at_end() const { return m_cls_iter_stack.empty(); }

  RecursiveClassIterator &operator++() {
    if (operator*().begin_child_classes() != operator*().end_child_classes()) {
      m_cls_iter_stack.push_back(std::make_pair(
          operator*().begin_child_classes(), operator*().end_child_classes()));
    } else {
      while (!m_cls_iter_stack.empty() && ++m_cls_iter_stack.back().first ==
                                              m_cls_iter_stack.back().second) {
...
private:
  std::list<
      std::pair<gsi::ClassBase::class_iterator, gsi::ClassBase::class_iterator>>
      m_cls_iter_stack;
};

} // namespace

static std::string replace_references(const std::string &t,
                                      const gsi::ClassBase *cls_base) {
  cls_base = real_class(cls_base);
  tl_assert(cls_base);

  //  build a name map for simple name replacement
  std::set<std::string> name_map;
  for (gsi::ClassBase::method_iterator m = cls_base->begin_methods();
       m != cls_base->end_methods(); ++m) {
  ...
  while ((p = t.find("\\", q)) != std::string::npos) {
    ...
    if (name_map.find(id) != name_map.end()) {
      r += "<a href=\"" + escape_xml(class_doc_url(std::string(), id)) + "\">" +
           escape_xml(id) + "</a>";
      found = true;
    }
...
std::string GSIHelpProvider::folder(lay::HelpSource * /*src*/) const {
  return "code";
}

std::string GSIHelpProvider::title(lay::HelpSource * /*src*/) const {
  return tl::to_string(QObject::tr("API Reference"));
}

static void produce_toc(const gsi::ClassBase *cls,
  ...
  for (tl::weak_collection<gsi::ClassBase>::const_iterator cc =
           cls->begin_child_classes();
       cc != cls->end_child_classes(); ++cc) {
    produce_toc(cc.operator->(), toc);
  }
}

void GSIHelpProvider::toc(lay::HelpSource * /*src*/,
  ...
  for (std::set<std::string>::const_iterator m = mod_names.begin();
       m != mod_names.end(); ++m) {
    t.push_back(module_doc_url(*m));
  }
}

QDomDocument GSIHelpProvider::get(lay::HelpSource *src,
  ...
  if (url_path == QString::fromUtf8("/code/index.xml")) {
    ...
    text = produce_class_index(
        src, from_encoded_class_name(tl::to_string(module_index_url.cap(1)))
                 .c_str());
  } else if (class_doc_url.indexIn(url_path) == 0) {
    text = produce_class_doc(
        from_encoded_class_name(tl::to_string(class_doc_url.cap(1))));
  } else {
  ...
  if (!doc.setContent(QByteArray(text.c_str(), int(text.size())), true,
    ...
    std::string fallback_text =
        ...
        "<b>XML Parser Error: </b>" + escape_xml(tl::to_string(errorMsg)) +
        ", in line " + tl::to_string(errorLine) + " of " + path + "\n" +
        "</p><pre>\n" + escape_xml(text) + "\n" + "</pre></doc>";

    doc.setContent(QByteArray(fallback_text.c_str(), int(fallback_text.size())),
                   true, &errorMsg, &errorLine);
  }

  return doc;
}

static void collect_class_info(
    const gsi::ClassBase *cls, const std::string &module,
  ...
  //  Only list the name if the class is not hidden, it's a top-level class or
  //  the path is an expanded one (the last criterion avoids generating classes
  //  such as A::B_C)
  if (!doc.hidden) {
    if (!doc.qt_class) {
      class_names.push_back(
          std::make_pair(qname, std::make_pair(module, doc.brief_doc)));
    } else {
      qt_class_names.push_back(
          std::make_pair(qname, std::make_pair(module, doc.brief_doc)));
    }
  }
...
std::string
GSIHelpProvider::produce_class_index(lay::HelpSource *src,
  ...
  if (!module_name) {
    ...
    os << "<title>" << tl::to_string(QObject::tr("Class Index for Module "))
       << escape_xml(module_name) << "</title>" << std::endl;
  ...
  for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
    ...
    if (!module_name || c->module() == module_name) {
      collect_class_info(c.operator->(), c->module(), class_names,
                         qt_class_names);
    }
  }
  ...
  if (!module_name) {
    ...
    for (class_index_t::const_iterator i = class_names.begin();
         i != class_names.end(); ++i) {
      os << "<topic-ref href=\"" << escape_xml(class_doc_url(i->first))
         << "\"/>" << std::endl;
    }
    for (class_index_t::const_iterator i = qt_class_names.begin();
         i != qt_class_names.end(); ++i) {
      os << "<topic-ref href=\"" << escape_xml(class_doc_url(i->first))
         << "\"/>" << std::endl;
    }
    ...
    for (class_index_t::iterator i = class_names.begin();
         i != class_names.end(); ++i) {
      mod_names.insert(i->second.first);
    }

    for (class_index_t::iterator i = qt_class_names.begin();
         i != qt_class_names.end(); ++i) {
    ...
    for (std::set<std::string>::const_iterator m = qt_mod_names.begin();
         m != qt_mod_names.end(); ++m) {
      os << "<topic-ref href=\"" << escape_xml(module_doc_url(*m)) << "\"/>"
         << std::endl;
    }

    os << "<p>" << tl::to_string(QObject::tr("Per-Module documentation:"))
       << "</p>";

    os << "<ul>";
    for (std::set<std::string>::const_iterator m = mod_names.begin();
         m != mod_names.end(); ++m) {
      os << "<li><a href=\"" << escape_xml(module_doc_url(*m)) << "\">"
         << tl::to_string(QObject::tr("Core Module")) << " " << escape_xml(*m)
         << "</a></li>";
    }
    for (std::set<std::string>::const_iterator m = qt_mod_names.begin();
         m != qt_mod_names.end(); ++m) {
      os << "<li><a href=\"" << escape_xml(module_doc_url(*m)) << "\">"
         << tl::to_string(QObject::tr("Qt Module")) << " " << escape_xml(*m)
         << "</a></li>";
    }
    os << "</ul>";
  }
  ...
  if (!qt_class_names.empty()) {
    os << "<p>" << tl::to_string(QObject::tr("Find Qt class documentation"))
       << " <a href=\"#qtclasses\">" << tl::to_string(QObject::tr("here"))
       << "</a></p>" << std::endl;
  }
  ...
  if (!class_names.empty()) {
    ...
    os << "<table>" << std::endl;
    int n = 0;
    for (class_index_t::const_iterator cc = class_names.begin();
         cc != class_names.end(); ++cc, ++n) {
      os << "<tr class=\"row" << (n % 2) << "\">" << std::endl;
      os << "<td><a href=\"" << escape_xml(class_doc_url(cc->first)) << "\">"
         << escape_xml(cc->first) << "</a></td>";
      if (!module_name) {
        os << "<td><a href=\"" << module_doc_url(cc->second.first) << "\">"
           << escape_xml(cc->second.first) << "</a></td>";
      }
  ...
  if (!qt_class_names.empty()) {
    ...
    os << "<table>" << std::endl;
    int n = 0;
    for (class_index_t::const_iterator cc = qt_class_names.begin();
         cc != qt_class_names.end(); ++cc, ++n) {
      os << "<tr class=\"row" << (n % 2) << "\">" << std::endl;
      os << "<td><a href=\"" << escape_xml(class_doc_url(cc->first)) << "\">"
         << escape_xml(cc->first) << "</a></td>";
      if (!module_name) {
...
static std::string type_to_s(const gsi::ArgType &a, bool linked,
                             bool for_return) {
  std::string s;
  switch (a.type()) {
  case gsi::T_void_ptr:
  ...
  case gsi::T_object:
    ...
    if (linked) {
      s += "<a href=\"" +
           escape_xml(class_doc_url(make_qualified_name(a.cls()))) + "\">" +
           escape_xml(make_qualified_name(a.cls())) + "</a>";
    } else {
...
static std::string method_attributes(const gsi::MethodBase *method,
                                     DocumentationParser & /*doc*/,
                                     bool without_static = false,
                                     bool without_prot = true) {
  std::string r;
  if (method->is_signal()) {
    if (!r.empty()) {
      r += ",";
...
static std::string method_return(const gsi::MethodBase *method,
                                 DocumentationParser & /*doc*/,
                                 bool linked = false) {
  return type_to_s(method->ret_type(), linked, true);
}

static std::string method_arguments(const gsi::MethodBase *method,
                                    const gsi::ClassBase *cls_obj,
                                    DocumentationParser &doc,
                                    bool linked = false,
                                    const char *sep = "<br/>") {
  std::string r;
  if (method->begin_arguments() == method->end_arguments()) {
    ...
    int n = 0;
    for (gsi::MethodBase::argument_iterator a = method->begin_arguments();
         a != method->end_arguments(); ++a, ++n) {
      if (n > 0) {
        r += ",";
        r += sep;
      }
        ...
        if (a->spec()->has_default()) {
          ...
          if (!a->spec()->init_doc().empty()) {
            ...
            try {
              ...
              tl::error << cls_obj->name() << "#"
                        << method->begin_synonyms()->name << ": " << ex.msg();
              r += "?";
            }
          }
        }
...
const gsi::ClassBase *find_child_with_declaration(const gsi::ClassBase *pc,
                                                  const gsi::ClassBase *decl) {
  while (pc) {

    for (tl::weak_collection<gsi::ClassBase>::const_iterator sc =
             pc->begin_child_classes();
         sc != pc->end_child_classes(); ++sc) {

      if (sc->declaration() == decl) {
        return sc.operator->();
      }

      const gsi::ClassBase *cc =
          find_child_with_declaration(sc.operator->(), decl);
      if (cc) {
        return cc;
      }
    }

    //  Try the base classes - due to skipping of the hidden classes we might
    //  pick a parent class initially.
    pc = pc->base();
  }
...
std::string GSIHelpProvider::produce_class_doc(const std::string &cls) const {
  std::ostringstream os;
  ...
  while (!cp.empty()) {
    ...
    while (pc && !cls_obj) {
      ...
      //  Try the base classes too - since we might have skipped some of the
      //  classes in the inheritance hierarchy, the child may be in a base
      //  class.
      pc = pc->base();
    }
  ...
  if (cls_obj != cls_obj->declaration()) {
    ...
    //  check if there is a top-level alias class for this one the other way
    //  around (A::B from A_B)
    for (gsi::ClassBase::class_iterator c = gsi::ClassBase::begin_classes();
         c != gsi::ClassBase::end_classes() && !tl_alias; ++c) {
  ...
  os << "<doc><title>" << tl::to_string(QObject::tr("API reference - Class"))
     << " " << escape_xml(cls) << "</title>" << std::endl;
  ...
  os << "<p><b>" << tl::to_string(QObject::tr("Module")) << "</b>: <a href=\""
     << escape_xml(module_doc_url(cls_obj->module())) << "\">"
     << escape_xml(cls_obj->module()) << "</a></p>";

  os << "<p><b>" << tl::to_string(QObject::tr("Description"))
     << "</b>: " << escape_xml(class_doc.brief_doc) << "</p>" << std::endl;
  ...
  if (base) {
    ...
    os << "<p><b>" << tl::to_string(QObject::tr("Class hierarchy"))
       << "</b>: " << make_qualified_name(cls_obj);
    while (base) {
      ...
      if (!bdoc.alias.empty()) {
        ...
        if (bdoc.alias != last_cls->name()) {
          os << " &#187; <a href=\"" << escape_xml(class_doc_url(bdoc.alias))
             << "\">" << escape_xml(bdoc.alias) << "</a>";
          all_collected = true;
        } else if (!all_collected) {
          classes.push_back(base);
        }
      } else if (!bdoc.hidden) {
        os << " &#187; <a href=\"" << escape_xml(class_doc_url(base->name()))
           << "\">" << escape_xml(base->name()) << "</a>";
        all_collected = true;
      } else if (!all_collected) {
  ...
  if (tl_alias) {
    os << "<p>"
       << tl::to_string(QObject::tr("This class is equivalent to the class "));
    std::string n = make_qualified_name(tl_alias);
    os << "<a href=\"" << escape_xml(class_doc_url(n)) << "\">" << escape_xml(n)
       << "</a>";
    os << "</p>" << std::endl;
  }
  ...
  for (std::vector<const gsi::ClassBase *>::const_iterator c = classes.begin();
       c != classes.end(); ++c) {
    for (tl::weak_collection<gsi::ClassBase>::const_iterator cc =
             (*c)->begin_child_classes();
         cc != (*c)->end_child_classes(); ++cc) {
      ...
      if (!cdoc.hidden || !cdoc.alias.empty()) {
        ...
        if (any) {
          ...
          os << "<p><b>" << tl::to_string(QObject::tr("Sub-classes"))
             << "</b>: ";
          any = true;
        }
  ...
  //  (in the reverse order so that derived classes override their super classes
  //  methods)
  ...
  for (size_t n = classes.size(); n > 0;) {
    ...
    for (gsi::ClassBase::method_iterator m = classes[n]->begin_methods();
         m != classes[n]->end_methods(); ++m) {
      for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
           syn != (*m)->end_synonyms(); ++syn) {
        std::multimap<std::string,
                      std::pair<const gsi::MethodBase *, size_t>>::iterator im;
    ...
    for (gsi::ClassBase::method_iterator m = classes[n]->begin_methods();
      ...
      for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
        ...
        if (!mdoc.hidden) {
          mm.insert(
              std::make_pair(full_name(*syn),
                             std::make_pair(*m, syn - (*m)->begin_synonyms())));
        }
      }
    }
  }
  ...
  for (std::multimap<std::string,
                     std::pair<const gsi::MethodBase *, size_t>>::const_iterator
           i = mm.begin();
       i != mm.end(); ++i, ++n) {

    const gsi::MethodBase::MethodSynonym &syn =
        i->second.first->begin_synonyms()[i->second.second];

    if (i->second.first->is_static() && syn.name == "new" && !syn.deprecated &&
        !i->second.first->is_protected()) {

      if (!any) {
        os << "<h2>" << tl::to_string(QObject::tr("Public constructors"))
           << "</h2>" << std::endl;
        os << "<table>" << std::endl;
        any = true;
      }
      ...
      os << "<td><b><a href=\"#method" << n << "\">" << escape_xml(i->first)
         << "</a></b></td>";
      os << "<td>" << method_arguments(i->second.first, cls_obj, method_doc)
         << "</td>";
      os << "<td>"
         << replace_references(escape_xml(method_doc.brief_doc), cls_obj)
         << "</td>";
      os << "</tr>" << std::endl;
    }
  }
  ...
  for (std::multimap<std::string,
    ...
    if (!i->second.first->is_static() && !syn.deprecated &&
        !i->second.first->is_protected()) {
      ...
      if (!attr.empty()) {
        os << "<td><i>["
           << escape_xml(method_attributes(i->second.first, method_doc))
           << "]</i></td>";
      } else {
        os << "<td></td>";
      }
  ...
  for (std::multimap<std::string,
    ...
    if (i->second.first->is_static() && syn.name != "new" && !syn.deprecated &&
      ...
      if (!any) {
        any = true;
        os << "<h2>"
           << tl::to_string(QObject::tr("Public static methods and constants"))
           << "</h2>" << std::endl;
        os << "<table>" << std::endl;
      }
  ...
  for (std::multimap<std::string,
    ...
    if (syn.deprecated) {
      ...
      gsi::MethodBase::synonym_iterator nd_syn =
          i->second.first->begin_synonyms();
      ...
      if (!any) {
        ...
        os << "<h2>"
           << tl::to_string(
                  QObject::tr("Deprecated methods (protected, public, static, "
                              "non-static and constructors)"))
           << "</h2>" << std::endl;
        os << "<table>" << std::endl;
      }
      ...
      ++row;
      std::string attr =
          method_attributes(i->second.first, method_doc, false, false);
      if (!attr.empty()) {
        os << "<td><i>["
           << escape_xml(method_attributes(i->second.first, method_doc))
           << "]</i></td>";
      ...
      if (nd_syn != i->second.first->end_synonyms()) {
        os << tl::to_string(
                  QObject::tr("Use of this method is deprecated. Use"))
           << " " << full_name(*nd_syn) << " "
           << tl::to_string(QObject::tr("instead"));
      } else {
  ...
  os << "<a name=\"detailed\"/><h2>"
     << tl::to_string(QObject::tr("Detailed description")) << "</h2>"
     << std::endl;
  ...
  for (std::multimap<std::string,
    ...
    if (i->first != prev_title) {
      int rows = 0;
      for (std::multimap<std::string, std::pair<const gsi::MethodBase *,
                                                size_t>>::const_iterator j = i;
           j != mm.end() && j->first == i->first; ++j) {
        ++rows;
      }
      if (rows > 1) {
        os << "<td rowspan=\"" << rows << "\">";
    ...
    os << "<a name=\"method" << n << "\"/>"
       ...
       << "<keyword title=\""
       << tl::to_string(QObject::tr("API reference - Class")) << " "
       << escape_xml(cls) << ", " << tl::to_string(QObject::tr("Method")) << " "
       << escape_xml(i->first) << "\" name=\"" << escape_xml(cls) << "#"
       << escape_xml(i->first) << "\"/>" << std::endl;
    ...
    os << method_return(i->second.first, method_doc, true) << " <b> "
       << escape_xml(i->first) << " </b> "
       << method_arguments(i->second.first, cls_obj, method_doc, true, " ");
    os << "</p>" << std::endl;

    os << "<p><b>" << tl::to_string(QObject::tr("Description")) << "</b>: "
       << replace_references(escape_xml(method_doc.brief_doc), cls_obj)
       << "</p>" << std::endl;
    ...
    if (!method_doc.params.empty() || !method_doc.ret_val.empty()) {
      ...
      for (std::vector<std::pair<std::string, std::string>>::const_iterator p =
               method_doc.params.begin();
           p != method_doc.params.end(); ++p) {
        os << "<tr><td><b>" << escape_xml(p->first) << "</b>:</td><td>"
           << replace_references(escape_xml(p->second), cls_obj) << "</td></tr>"
           << std::endl;
      }
      if (!method_doc.ret_val.empty()) {
        os << "<tr><td><b>" << tl::to_string(QObject::tr("Returns"))
           << "</b>:</td><td>"
           << replace_references(escape_xml(method_doc.ret_val), cls_obj)
           << "</td></tr>" << std::endl;
      }
      os << "</table>" << std::endl;
    }
      ...
      if (nd_syn != i->second.first->end_synonyms()) {
        os << "<p>"
           << tl::to_string(
                  QObject::tr("Use of this method is deprecated. Use"))
           << " " << full_name(*nd_syn) << " "
           << tl::to_string(QObject::tr("instead")) << "</p>" << std::endl;
      } else {
        os << "<p>"
    ...
    if (!tl::Extractor(dh.c_str()).at_end()) {
      os << "<p>" << replace_references(method_doc.doc_html(), cls_obj)
         << "</p>" << std::endl;
    }
...
static tl::RegisteredClass<lay::HelpProvider>
    gsi_help_provider(new GSIHelpProvider(), 1000);
