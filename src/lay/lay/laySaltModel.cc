...
SaltModel::SaltModel(QObject *parent, lay::Salt *salt, lay::Salt *salt_filtered,
                     bool salt_exclude)
...
Qt::ItemFlags SaltModel::flags(const QModelIndex &index) const {
  ...
  if (g && (!is_enabled(g->name()) || g->is_hidden())) {
    //  We use a custom flag to indicate "disabled" display without actually
    //  disabling the item
...
QVariant SaltModel::data(const QModelIndex &index, int role) const {
  if (role == Qt::DisplayRole) {
    ...
    const lay::SaltGrain *g = grain_from_index(index);
    if (!g) {
      return QVariant(tr("<html><body><h4>There are no items to show in this "
                         "list</h4><p>%1</p></body></html>")
    ...
    std::map<std::string, std::pair<Severity, std::string>>::const_iterator m =
        m_messages.find(g->name());
    if (m != m_messages.end()) {
      if (m->second.first == Warning || m->second.first == Error) {
        text += "<p><font color=\"#ff0000\"><b>" +
                tl::escaped_to_html(m->second.second) + "</b></font></p>";
      } else if (m->second.first == Info) {
        text += "<p><font color=\"#c0c0c0\">" +
                tl::escaped_to_html(m->second.second) + "</font></p>";
      } else {
    ...
    if (hidden) {
      text += "<p>";
      text += tl::to_string(tr(
          "This package is an auxiliary package for use with other packages."));
      text += "</p></i>";
    }
    text += "</body></html>";
    ...
    if (img.width() != icon_dim || img.height() != icon_dim) {
      ...
      QImage scaled = img.scaled(QSize(icon_dim, icon_dim), Qt::KeepAspectRatio,
      ...
      painter.drawImage((icon_dim - scaled.width()) / 2,
                        (icon_dim - scaled.height()) / 2, scaled);
    }
...
QModelIndex SaltModel::parent(const QModelIndex & /*index*/) const {
  return QModelIndex();
}

int SaltModel::columnCount(const QModelIndex & /*parent*/) const { return 1; }
...
SaltGrain *SaltModel::grain_from_index(const QModelIndex &index) const {
  if (index.isValid() && index.row() >= 0 &&
      index.row() < int(m_ordered_grains.size())) {
...
void SaltModel::mark_all() {
  m_marked.clear();
  for (std::vector<SaltGrain *>::const_iterator g = m_ordered_grains.begin();
       g != m_ordered_grains.end(); ++g) {
...
void SaltModel::clear_order() { m_display_order.clear(); }
...
void SaltModel::set_message(const std::string &name, Severity severity,
  ...
  if (message.empty()) {
    ...
    std::map<std::string, std::pair<Severity, std::string>>::iterator m =
        m_messages.find(name);
    if (m == m_messages.end() || m->second.second != message ||
        m->second.first != severity) {
      m_messages.insert(
          std::make_pair(name, std::make_pair(severity, message)));
      needs_update = true;
    }
  }
...
void SaltModel::create_ordered_list() {
  ...
  if (m_display_order.empty()) {
    ...
    for (Salt::flat_iterator i = mp_salt->begin_flat();
         i != mp_salt->end_flat(); ++i) {
      //  filter the grains by looking them up in the reference salt
      if (mp_salt_filtered && (mp_salt_filtered->grain_by_name((*i)->name()) !=
                               0) == m_salt_exclude) {
        continue;
      }
    ...
    for (std::map<std::string, int>::const_iterator i = m_display_order.begin();
         i != m_display_order.end(); ++i) {
    ...
    for (int o = min_order; o <= max_order; ++o) {
      for (Salt::flat_iterator i = mp_salt->begin_flat();
           i != mp_salt->end_flat(); ++i) {
        //  filter the grains by looking them up in the reference salt
        if (mp_salt_filtered && (mp_salt_filtered->grain_by_name(
                                     (*i)->name()) != 0) == m_salt_exclude) {
          continue;
        }
        std::map<std::string, int>::const_iterator d =
            m_display_order.find((*i)->name());
        int oi = 0;
