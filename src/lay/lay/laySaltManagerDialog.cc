...
public:
  ...
  std::string name() const { return tl::to_string(name_edit->text()); }
  ...
  void accept() {
    ...
    if (name.empty()) {
      ...
      name_alert->error() << tr(
          "Name is not valid (must be composed of letters, digits, dots or "
          "underscores.\nGroups and names need to be separated with slashes.");
    } else {

      //  check, if this name does not exist yet
      for (Salt::flat_iterator g = mp_salt->begin_flat();
           g != mp_salt->end_flat(); ++g) {
        if ((*g)->name() == name) {
...
public:
  struct APIFeature {
    APIFeature(const std::string &_name, const std::string &_version,
...
SaltAPIVersionCheck::SaltAPIVersionCheck() { populate_features(); }
  ...
  while (!ex.at_end()) {
    ...
    while (!ex.at_end() && !ex.test(";")) {
      ...
      if (ex.try_read(n)) {
        ...
        m_message =
            tl::to_string(tr("API version string malformed - cannot check."));
        return false;
      }
    }
    ...
    if (!f) {
      ...
      m_message +=
          tl::sprintf(tl::to_string(tr("Feature %s not available.")), fname);

      good = false;
      any_not_available = true;

    } else if (!f->version.empty() && !v.empty() &&
               SaltGrain::compare_versions(f->version, v) < 0) {
      ...
      m_message += tl::sprintf(
          tl::to_string(tr("%s required with version %s or later (is %s).")),
          f->description, v, fv_short);
...
std::string SaltAPIVersionCheck::feature_list() const {
  std::string fl;
  for (std::vector<APIFeature>::const_iterator f = m_features.begin();
       f != m_features.end(); ++f) {
    if (!fl.empty()) {
      fl += ", ";
    }
...
void SaltAPIVersionCheck::populate_features() {
  m_features.push_back(
      APIFeature(std::string(), lay::Version::version(), "KLayout API"));

  if (rba::RubyInterpreter::instance() &&
      rba::RubyInterpreter::instance()->available()) {
  ...
  if (pya::PythonInterpreter::instance() &&
      pya::PythonInterpreter::instance()->available()) {
  ...
  m_features.push_back(
      APIFeature("qt_binding", std::string(), "Qt Binding for RBA or PYA"));
#endif
#if defined(HAVE_QT)
  ...
  m_features.push_back(
      APIFeature("wide-coords", std::string(), "64 bit coordinates"));
#endif
}
...
SaltManagerDialog::SaltManagerDialog(QWidget *parent, lay::Salt *salt,
                                     const std::string &salt_mine_url)
    : QDialog(parent), m_salt_mine_url(salt_mine_url),
      dm_update_models(this, &SaltManagerDialog::update_models),
      m_current_tab(-1), mp_downloaded_target(0) {
  ...
  //  This model will show only the grains of mp_salt_mine which are not present
  //  in mp_salt yet.
  ...
  //  This model will show only the grains of mp_salt_mine which are present in
  //  mp_salt already.
  ...
  connect(mp_salt, SIGNAL(collections_about_to_change()), this,
          SLOT(salt_about_to_change()));
  connect(&m_salt_mine, SIGNAL(collections_changed()), this,
          SLOT(salt_mine_changed()));
  connect(&m_salt_mine, SIGNAL(collections_about_to_change()), this,
          SLOT(salt_mine_about_to_change()));

  connect(
      salt_view->selectionModel(),
      SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
      this, SLOT(selected_changed()));
  connect(salt_view, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(edit_properties()));
  connect(
      salt_mine_view_new->selectionModel(),
      SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
      this, SLOT(mine_new_selected_changed()));
  connect(
      salt_mine_view_update->selectionModel(),
      SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
      this, SLOT(mine_update_selected_changed()));
  connect(salt_mine_view_new, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(mark_clicked()));
  connect(salt_mine_view_update, SIGNAL(doubleClicked(const QModelIndex &)),
          this, SLOT(mark_clicked()));
  ...
  connect(search_installed_edit, SIGNAL(textChanged(const QString &)), this,
          SLOT(search_text_changed(const QString &)));
  connect(search_new_edit, SIGNAL(textChanged(const QString &)), this,
          SLOT(search_text_changed(const QString &)));
  connect(search_update_edit, SIGNAL(textChanged(const QString &)), this,
          SLOT(search_text_changed(const QString &)));
  ...
  connect(actionUnmarkAllNew, SIGNAL(triggered()), this,
          SLOT(unmark_all_new()));
  connect(actionMarkAllNew, SIGNAL(triggered()), this, SLOT(mark_all_new()));
  connect(actionShowMarkedOnlyNew, SIGNAL(triggered()), this,
          SLOT(show_marked_only_new()));
  connect(actionUnmarkAllUpdate, SIGNAL(triggered()), this,
          SLOT(unmark_all_update()));
  connect(actionMarkAllUpdate, SIGNAL(triggered()), this,
          SLOT(mark_all_update()));
  connect(actionShowMarkedOnlyUpdate, SIGNAL(triggered()), this,
          SLOT(show_marked_only_update()));
  ...
  connect(actionUnmarkForUpdate, SIGNAL(triggered()), this,
          SLOT(mark_clicked()));
...
void SaltManagerDialog::show_marked_only_new() {
  ...
  for (int i = model->rowCount(QModelIndex()); i > 0;) {
    ...
    salt_mine_view_new->setRowHidden(
        i, show_marked_only && !(g && model->is_marked(g->name())));
    mine_new_selected_changed();
  }
}
...
void SaltManagerDialog::show_marked_only_update() {
  ...
  for (int i = model->rowCount(QModelIndex()); i > 0;) {
    --i;
    SaltGrain *g = model->grain_from_index(model->index(i, 0, QModelIndex()));
    salt_mine_view_update->setRowHidden(
        i, show_marked_only && !(g && model->is_marked(g->name())));
    mine_update_selected_changed();
  }
}
...
void SaltManagerDialog::mark_clicked() {
  ...
  if (sender() == salt_mine_view_new || sender() == mark_new_button ||
      sender() == actionMarkNew || sender() == actionUnmarkNew) {
    view = salt_mine_view_new;
  } else {
    view = salt_mine_view_update;
  }
  ...
  bool toggle =
      (sender() != actionMarkNew && sender() != actionUnmarkNew &&
       sender() != actionMarkForUpdate && sender() != actionUnmarkForUpdate);
...
void SaltManagerDialog::update_apply_state() {
  ...
  if (model) {
    ...
    if (marked == 0) {
      apply_label_new->setText(
          tr("Select at least one package for installation (check button)"));
    } else if (marked == 1) {
...
void SaltManagerDialog::apply() {
  ...
  if (model) {
    for (int i = model->rowCount(QModelIndex()); i > 0;) {
      ...
      QModelIndex index = model->index(i, 0, QModelIndex());
      SaltGrain *g = model->grain_from_index(index);
      //  NOTE: checking for valid_name prevents bad entries inside the download
      //  list
      if (g && model->is_marked(g->name()) &&
          SaltGrain::valid_name(g->name())) {
        manager.register_download(g->name(), g->token(), g->url(),
                                  g->version());
        any = true;
      }
    }
  ...
  if (!any) {
    if (update) {
      ...
      throw tl::Exception(
          tl::to_string(tr("No packages marked for installation")));
    }
  }
  ...
  if (g) {
    if (g->is_readonly()) {
      QMessageBox::critical(
          this, tr("Package is not Editable"),
          tr("This package cannot be edited.\n\nEither you don't have write "
             "permissions on the directory or the package was installed from a "
             "repository."));
  ...
  if (temp_dialog.exec()) {
    ...
    if (mp_salt->create_grain(temp_dialog.templ(), target)) {
      ...
      if (model) {
        ...
        //  NOTE: this is basically redundant (because it happens in the
        //  background later through dm_update_models). But we need this now to
      ...
      throw tl::Exception(
          tl::to_string(tr("Initialization of new package failed - see log "
                           "window (File/Log Viewer) for details")));
    }
  }
...
void SaltManagerDialog::delete_grain() {
  ...
  if (gg.size() == 1) {
    ...
    if (QMessageBox::question(this, tr("Delete Package"),
                              tr("Are you sure to delete package '%1'?")
                                  .arg(tl::to_qstring(g->name())),
                              QMessageBox::Yes,
    ...
    if (QMessageBox::question(
            this, tr("Delete Packages"),
            tr("Are you sure to delete the selected %1 packages?")
                .arg(int(gg.size())),
            QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
      for (std::vector<SaltGrain *>::const_iterator i = gg.begin();
           i != gg.end(); ++i) {
  ...
  if (failed.size() == 1) {
    throw tl::Exception(tl::to_string(
        tr("Failed to remove package %1 (no write permissions on directory?)")
            .arg(tl::to_qstring(failed.front()))));
  } else if (failed.size() > 1) {
    throw tl::Exception(
        tl::to_string(tr("Failed to remove the following packages:\n  %1")
                          .arg(tl::to_qstring(tl::join(failed, "\n  ")))));
  }
...
void SaltManagerDialog::refresh() {
  if (!m_salt_mine_url.empty()) {
    ...
    tl::log << tl::to_string(tr("Downloading package repository from %1")
                                 .arg(tl::to_qstring(m_salt_mine_url)));

    m_salt_mine_reader.reset(new tl::InputStream(m_salt_mine_url));
    salt_mine_download_started();

    tl::InputHttpStream *http =
        dynamic_cast<tl::InputHttpStream *>(m_salt_mine_reader->base());
    if (http) {
...
void SaltManagerDialog::salt_mine_changed() { dm_update_models(); }
...
void SaltManagerDialog::update_models() {
  ...
  if (salt_view->currentIndex().isValid()) {
    const lay::SaltGrain *g =
        model->grain_from_index(salt_view->currentIndex());
    if (g) {
      current = g->name();
    }
  }
  ...
  for (Salt::flat_iterator g = mp_salt->begin_flat(); g != mp_salt->end_flat();
       ++g) {
    SaltGrain *gm = m_salt_mine.grain_by_name((*g)->name());
    if (gm && SaltGrain::compare_versions(gm->version(), (*g)->version()) > 0) {
      model->set_message(
          (*g)->name(), SaltModel::Warning,
          tl::to_string(tr("An update to version %1 is available")
                            .arg(tl::to_qstring(gm->version()))));
    }
  }
  ...
  for (Salt::flat_iterator g = mp_salt->begin_flat(); g != mp_salt->end_flat();
    ...
    if (gm && SaltGrain::compare_versions(gm->version(), (*g)->version()) > 0) {
      has_warning = true;
      mine_model->set_message(
          (*g)->name(), SaltModel::Warning,
          tl::to_string(tr("The installed version is outdated (%1)")
                            .arg(tl::to_qstring((*g)->version()))));
      mine_model->set_order((*g)->name(), -1);
    } else if (gm) {
      mine_model->set_message((*g)->name(), SaltModel::None,
                              tl::to_string(tr("This package is up to date")));
      mine_model->set_order((*g)->name(), 1);
  ...
  for (Salt::flat_iterator g = m_salt_mine.begin_flat();
       g != m_salt_mine.end_flat(); ++g) {
  ...
  if (mine_model->rowCount(QModelIndex()) > 0) {
    ...
    salt_mine_view_update->setCurrentIndex(
        mine_model->index(0, 0, QModelIndex()));
...
lay::SaltGrain *SaltManagerDialog::current_grain() {
  SaltModel *model = dynamic_cast<SaltModel *>(salt_view->model());
...
std::vector<lay::SaltGrain *> SaltManagerDialog::current_grains() {
...
void SaltManagerDialog::mine_update_selected_changed() {
  ...
  QModelIndexList indexes =
      salt_mine_view_update->selectionModel()->selectedIndexes();
  if (indexes.size() == 1) {
...
void SaltManagerDialog::mine_new_selected_changed() {
  ...
  SaltGrain *g = 0;
  QModelIndexList indexes =
      salt_mine_view_new->selectionModel()->selectedIndexes();
  if (indexes.size() == 1) {
    g = model->grain_from_index(indexes.front());
  }
...
void SaltManagerDialog::get_remote_grain_info(
    lay::SaltGrain *g, SaltGrainDetailsTextWidget *details) {
  if (!g) {
  ...
  try {
    ...
    tl::InputHttpStream *http =
        dynamic_cast<tl::InputHttpStream *>(m_downloaded_grain_reader->base());
    if (http) {
      //  async reading on HTTP
...
void SaltManagerDialog::data_ready() {
  if (!m_salt_mine_grain.get() || !m_downloaded_grain.get() ||
      !m_downloaded_grain_reader.get() || !mp_downloaded_target) {
    return;
  }

  //  Load the grain file (save URL as it is overwritten by the grain.xml
  //  content)
  ...
  try {
    ...
    if (m_salt_mine_grain->name() != m_downloaded_grain->name()) {
      throw tl::Exception(
          tl::to_string(tr("Name mismatch between repository and actual "
                           "package (repository: %1, package: %2)")
                            .arg(tl::to_qstring(m_salt_mine_grain->name()))
                            .arg(tl::to_qstring(m_downloaded_grain->name()))));
    }
    if (SaltGrain::compare_versions(m_salt_mine_grain->version(),
                                    m_downloaded_grain->version()) != 0) {
      throw tl::Exception(tl::to_string(
          tr("Version mismatch between repository and actual package "
             "(repository: %1, package: %2)")
              .arg(tl::to_qstring(m_salt_mine_grain->version()))
              .arg(tl::to_qstring(m_downloaded_grain->version()))));
    }
...
void SaltManagerDialog::show_error(tl::Exception &ex) {
  QString html =
      tr("<html>"
         ...
         "</html>")
          .arg(tl::to_qstring(
              m_downloaded_grain.get() ? m_downloaded_grain->url() : ""))
