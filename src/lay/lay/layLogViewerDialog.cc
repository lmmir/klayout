...
LogReceiver::LogReceiver(LogFile *file, int verbosity,
...
void LogReceiver::yield() { mp_file->yield(); }
...
LogFile::LogFile(size_t max_entries, bool register_global)
    : m_error_receiver(this, 0, &LogFile::add_error),
      ...
      m_info_receiver(this, 0, &LogFile::add_info), m_max_entries(max_entries),
      m_generation_id(0), m_last_generation_id(0), m_has_errors(false),
      m_has_warnings(false), m_last_attn(false) {
...
bool LogFile::has_errors() const { return m_has_errors; }

bool LogFile::has_warnings() const { return m_has_warnings; }

void LogFile::separator() {
  m_lock.lock();
  bool has_separator = false;
  if (m_messages.size() > 0 &&
      m_messages.back().mode() == LogFileEntry::Separator) {
    has_separator = true;
  }
  m_lock.unlock();

  if (!has_separator) {
    add(LogFileEntry::Separator,
        tl::to_string(QObject::tr("<-- New section -->")), false);
  }
}
...
void LogFile::copy() {
  ...
  QString text;
  for (std::deque<LogFileEntry>::const_iterator m = m_messages.begin();
       m != m_messages.end(); ++m) {
...
size_t LogFile::max_entries() const { return m_max_entries; }

void LogFile::add(LogFileEntry::mode_type mode, const std::string &msg,
                  bool continued) {
  QMutexLocker locker(&m_lock);
  ...
  if (mode == LogFileEntry::Warning || mode == LogFileEntry::WarningContinued) {
    m_has_warnings = true;
  } else if (mode == LogFileEntry::Error ||
             mode == LogFileEntry::ErrorContinued) {
    m_has_errors = true;
  }
...
void LogFile::yield() {
  ...
  if (lay::ApplicationBase::instance()->qapp_gui() &&
      QThread::currentThread() ==
          lay::ApplicationBase::instance()->qapp_gui()->thread()) {
...
QVariant LogFile::data(const QModelIndex &index, int role) const {
  QMutexLocker locker(&m_lock);
    ...
    if (index.row() < int(m_messages.size()) && index.row() >= 0) {
      ...
      if (mode == LogFileEntry::Separator) {
        ...
        return QColor(255, 0, 0);
                 ...
                 mode == LogFileEntry::WarningContinued) {
...
LogViewerDialog::LogViewerDialog(QWidget *parent, bool register_global,
                                 bool interactive)
    : QDialog(parent),
      m_file(50000, register_global) //  TODO: make this variable ..
{
  ...
  if (!register_global) {
    ...
    connect(verbosity_cbx, SIGNAL(currentIndexChanged(int)), this,
            SLOT(verbosity_changed(int)));
  }
...
AlertLogButton::AlertLogButton(QWidget *parent) : QToolButton(parent) {
  mp_logger = new LogViewerDialog(this, false, false);
  hide();
  connect(&mp_logger->file(), SIGNAL(attention_changed(bool)), this,
          SLOT(attention_changed(bool)));
...
void AlertLogButton::attention_changed(bool attn) {
  ...
  //  this feature allows putting the alert button together with other entry
  //  fields into a frame and make this frame highlighted on error or warning.
