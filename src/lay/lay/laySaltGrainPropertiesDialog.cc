 ...
 */
class SaltGrainEditDelegate : public QItemDelegate {
public:
  SaltGrainEditDelegate(QWidget *parent) : QItemDelegate(parent) {
    //  .. nothing yet ..
  }
  ...
  QSize sizeHint(const QStyleOptionViewItem &option,
                 const QModelIndex & /*index*/) const {
    QSize sz =
        option.fontMetrics.size(Qt::TextSingleLine, QString::fromUtf8("M"));
    sz += QSize(0, 8);
    return sz;
  }
};
 ...
 *  @brief A delegate for editing a field of the dependency list
 */
class SaltGrainNameEditDelegate : public SaltGrainEditDelegate {
public:
  ...
  QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option,
                        const QModelIndex &index) const {
    QWidget *editor =
        SaltGrainEditDelegate::createEditor(parent, option, index);
...
SaltGrainPropertiesDialog::SaltGrainPropertiesDialog(QWidget *parent)
  ...
  connect(screenshot_delete_button, SIGNAL(clicked()), this,
          SLOT(reset_screenshot()));
  connect(screenshot_config_button, SIGNAL(clicked()), this,
          SLOT(set_screenshot()));
  connect(doc_url, SIGNAL(textChanged(const QString &)), this,
          SLOT(url_changed(const QString &)));
  connect(add_dependency, SIGNAL(clicked()), this,
          SLOT(add_dependency_clicked()));
  connect(remove_dependency, SIGNAL(clicked()), this,
          SLOT(remove_dependency_clicked()));
  connect(dependencies, SIGNAL(itemChanged(QTreeWidgetItem *, int)), this,
          SLOT(dependency_changed(QTreeWidgetItem *, int)));

  dependencies->setItemDelegateForColumn(
      1, new SaltGrainEditDelegate(dependencies));
  dependencies->setItemDelegateForColumn(
      2, new SaltGrainEditDelegate(dependencies));
...
void SaltGrainPropertiesDialog::update_controls() {
  ...
  for (std::vector<SaltGrainDependency>::const_iterator d =
           m_grain.dependencies().begin();
       d != m_grain.dependencies().end(); ++d) {
...
void SaltGrainPropertiesDialog::update_icon() {
  if (m_grain.icon().isNull()) {
    ...
    if (img.width() == icon_config_button->iconSize().width()) {
      ...
      icon_config_button->setIcon(QIcon(QPixmap::fromImage(
          img.scaled(icon_config_button->iconSize(), Qt::KeepAspectRatio,
...
void SaltGrainPropertiesDialog::update_screenshot() {
  if (m_grain.screenshot().isNull()) {
    ...
    if (img.width() == screenshot_config_button->iconSize().width()) {
      ...
      screenshot_config_button->setIcon(QIcon(QPixmap::fromImage(
          img.scaled(screenshot_config_button->iconSize(), Qt::KeepAspectRatio,
                     Qt::SmoothTransformation))));
    }
  }
}
...
void SaltGrainPropertiesDialog::dependency_changed(QTreeWidgetItem *item,
                                                   int column) {
  if (!m_update_enabled) {
    return;
  }
  ...
  std::string name =
      tl::to_string(item->data(0, Qt::UserRole).toString().simplified());
...
void SaltGrainPropertiesDialog::set_icon() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Pick Icon Image File"), m_image_dir,
  ...
  if (!fileName.isNull()) {
    ...
    if (img.width() > max_dim || img.height() > max_dim) {
      if (QMessageBox::warning(this, tr("Image Too Big"),
                               tr("Icon image too big - must be %1x%2 pixels "
                                  "max, but is %3x%4.\n\nScale image?")
                                   .arg(max_dim)
                                   .arg(max_dim)
                                   .arg(img.width())
...
void SaltGrainPropertiesDialog::set_screenshot() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Pick Showcase Image File"), m_image_dir,
      tr("Images (*.png *.jpg);;All Files (*)"));
  if (!fileName.isNull()) {
    ...
    if (img.width() > max_dim || img.height() > max_dim) {
      if (QMessageBox::warning(this, tr("Image Too Big"),
                               tr("Showcase image too big - must be %1x%2 "
                                  "pixels max, but is %3x%4.\n\nScale image?")
                                   .arg(max_dim)
                                   .arg(max_dim)
                                   .arg(img.width())
                                   .arg(img.height()),
...
void SaltGrainPropertiesDialog::add_dependency_clicked() {
  ...
  item->setFlags(item->flags() | Qt::ItemIsEditable);
  dependencies->addTopLevelItem(item);
  dependencies->setCurrentItem(
      dependencies->topLevelItem(dependencies->topLevelItemCount() - 1));
}
...
public:
  ...
  const lay::SaltGrain *grain_for_name(const std::string &name) const {
    std::map<std::string, const lay::SaltGrain *>::const_iterator n =
        m_name_to_grain.find(name);
  ...
  void check_circular(const lay::SaltGrain *current,
                      const lay::SaltGrain *new_dep) {
...
private:
  ...
  void check_circular_follow(const lay::SaltGrain *current,
                             std::vector<const lay::SaltGrain *> &path) {
    if (!current) {
      return;
    }
    ...
    for (std::vector<const lay::SaltGrain *>::const_iterator p = path.begin();
         p != path.end() - 1; ++p) {
      if (*p == current) {
    ...
    for (std::vector<SaltGrainDependency>::const_iterator d =
             current->dependencies().begin();
         d != current->dependencies().end(); ++d) {
  ...
  void circular_reference_error(std::vector<const lay::SaltGrain *> &path) {
    std::string msg = tl::to_string(
        QObject::tr("The following path forms a circular dependency: "));
    for (std::vector<const lay::SaltGrain *>::const_iterator p = path.begin();
         p != path.end(); ++p) {
      if (p != path.begin()) {
        msg += "->";
...
void SaltGrainPropertiesDialog::accept() {
  ...
  if (m_grain.license().empty()) {
    license_alert->warn() << tr("License field is empty. Please consider "
                                "specifying a license model.")
                          << tl::endl
                          << tr("A license model tells users whether and how "
                                "to use the source code of the package.");
  }
  ...
  if (m_grain.version().empty()) {
    version_alert->warn()
        << tr("Version field is empty. Please consider specifying a version "
              "number.")
        << tl::endl
        << tr("Versions help the system to apply upgrades if required.");
  } else if (!SaltGrain::valid_version(m_grain.version())) {
    version_alert->error()
        << tr("'%1' is not a valid version string. A version string needs to "
              "be numeric (like '1.2.3' or '4.5'').")
               .arg(tl::to_qstring(m_grain.version()));
  }

  //  API version
  api_version_alert->clear();
  if (!m_grain.api_version().empty() &&
      !SaltGrain::valid_api_version(m_grain.api_version())) {
    api_version_alert->error()
        << tr("'%1' is not a valid API version string. An API version string "
              "needs to be a semicolon-separated list of features with "
              "optional numeric versions (like '0.26' or 'ruby 2.0; python').")
               .arg(tl::to_qstring(m_grain.api_version()));
  }
  ...
  if (!m_grain.doc_url().empty()) {
    try {
      ...
      doc_url_alert->error() << tr("Attempt to read documentation URL failed. "
  ...
  for (std::vector<SaltGrainDependency>::const_iterator d =
    ...
    if (!SaltGrain::valid_name(d->name)) {
      dependencies_alert->error()
          << tr("'%1' is not a valid package name").arg(tl::to_qstring(d->name))
          << tl::endl
          << tr("Valid package names are words (letters, digits, underscores).")
          << tl::endl
    ...
    if (dep_seen.find(d->name) != dep_seen.end()) {
      dependencies_alert->error()
          << tr("Duplicate dependency '%1'").arg(tl::to_qstring(d->name))
          << tl::endl
          << tr("A package cannot be dependent on the same package twice. "
                "Remove on entry.");
      continue;
    }
    ...
    if (dep.is_valid_name(d->name)) {
      try {
        dep.check_circular(dep.grain_for_name(m_grain.name()),
                           dep.grain_for_name(d->name));
      } catch (tl::Exception &ex) {
        dependencies_alert->error()
            << ex.msg() << tl::endl
            << tr("Circular dependency means, a package is eventually "
                  "depending on itself.");
      }
    }

    if (d->version.empty()) {
      dependencies_alert->warn()
          << tr("No version specified for dependency '%1'")
                 .arg(tl::to_qstring(d->name))
          << tl::endl
          << tr("Please consider giving a version here. Versions help deciding "
                "whether a package needs to be updated.")
          << tl::endl
          << tr("If the dependency package has a version itself, the version "
                "is automatically set to its current version.");
    }
    ...
    if (!d->url.empty()) {
      ...
      try {
        ...
        if (gdep.name() != d->name) {
          dependencies_alert->error()
              << tr("Package name obtained from download URL is not the "
                    "expected name.")
              << tl::endl
        ...
        dependencies_alert->error()
            << tr("Attempt to test-download package from URL failed. Error "
                  "details follow.")
            << tl::endl
  ...
  if (!license_alert->needs_attention() && !doc_url_alert->needs_attention() &&
    ...
    if (QMessageBox::warning(
            this, tr("Issues Encountered"),
            tr("Some issues have been found when inspecting the package "
               "details.\nThe respective fields are marked with warning "
...
bool SaltGrainPropertiesDialog::exec_dialog(lay::SaltGrain *grain,
                                            lay::Salt *salt) {
  m_grain = *grain;
  mp_salt = salt;

  dependencies->setItemDelegateForColumn(
      0, new SaltGrainNameEditDelegate(dependencies, mp_salt));
