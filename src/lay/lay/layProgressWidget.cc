...
ProgressBarWidget::ProgressBarWidget(QWidget *parent, const char *name)
    : QWidget(parent), m_value(0.0), m_width(200), m_length(0), m_fw(1),
      m_bw(0) {
...
void ProgressBarWidget::set_value(double v, const std::string &value) {
  ...
  int l = 0;
  if (m_width > 0) {
    l = int(floor((v < 0.0 ? 0.0 : v) * 0.01 * (double(m_width - 2) - 1e-6))) %
        m_width;
  }
  if (l != m_length) {
    m_length = l;
...
QSize ProgressBarWidget::sizeHint() const {
  ...
  return QSize(fm.horizontalAdvance(QString::fromUtf8("100%")) * 4,
               fm.height() + 2);
#else
...
QSize ProgressBarWidget::minimumSizeHint() const { return QSize(50, 1); }
...
void ProgressBarWidget::paintEvent(QPaintEvent *) {
  ...
  painter.fillRect(
      QRect(QPoint(m_fw, m_fw), QPoint(m_length + m_fw - 1, bottom - 1 - m_fw)),
      palette().brush(QPalette::Highlight));
  painter.fillRect(QRect(QPoint(m_length + m_fw, m_fw),
                         QPoint(right - 1 - m_fw, bottom - 1 - m_fw)),
                   palette().brush(QPalette::Base));
  painter.setPen(palette().color(QPalette::Text));

  for (int d = 0; d < m_bw; ++d) {
    painter.drawRect(
        QRect(QPoint(d, d), QPoint(right - 1 - d, bottom - 1 - d)));
  }

  painter.setFont(font());

  painter.setClipRect(
      QRect(QPoint(m_fw, m_fw), QPoint(m_length + m_fw - 1, bottom - m_fw)));
  painter.setPen(palette().color(QPalette::HighlightedText));
  painter.drawText(rect(), Qt::AlignHCenter | Qt::AlignVCenter,
                   tl::to_qstring(m_value_string));

  painter.setClipRect(
      QRect(QPoint(m_length + m_fw, 0), QPoint(right - m_fw, bottom - m_fw)));
  painter.setPen(palette().color(QPalette::Text));
  painter.drawText(rect(), Qt::AlignHCenter | Qt::AlignVCenter,
                   tl::to_qstring(m_value_string));
}
...
ProgressWidget::ProgressWidget(ProgressReporter *pr, QWidget *parent, bool fw)
    : QFrame(parent), mp_widget(0), mp_pr(pr), m_log_file(0, true),
      m_log_visible(false) {
  ...
  connect(&m_log_file, SIGNAL(layoutChanged()), log_view,
          SLOT(scrollToBottom()));
  connect(&m_log_file, SIGNAL(attention_changed(bool)), attn_frame,
  ...
  //  this does not allow the label to control the overall size, so a long
  //  string does not hurt:
  ...
  layout->addItem(
      new QSpacerItem(8, 8, QSizePolicy::Expanding, QSizePolicy::Expanding), 0,
      col, 1, 1);
  m_left_col = col++;
  ...
  mp_progress_bar_frame->setSizePolicy(QSizePolicy::Expanding,
  ...
  layout->addItem(
      new QSpacerItem(10, 10, QSizePolicy::Fixed, QSizePolicy::Fixed), 1, 0, 1,
      col);
...
void ProgressWidget::set_log_visible(tl::Progress *progress) {
  if ((progress != 0) != m_log_visible) {
    ...
    mp_log_label->setText(progress ? tl::to_qstring(progress->desc())
                                   : QString());
...
bool ProgressWidget::full_width() const { return m_full_width; }

QWidget *ProgressWidget::get_widget() const { return mp_widget; }
...
void ProgressWidget::set_progress(tl::Progress *progress) {
  lay::ProgressBarWidget *progress_bars[] = {mp_progress_bar1, mp_progress_bar2,
                                             mp_progress_bar3};
  ...
  for (size_t i = 0; i < sizeof(progress_bars) / sizeof(progress_bars[0]);
       ++i) {
  ...
  //  according to the doc this should not be required, but without, the
  //  progress bar does not resize
  mp_progress_bar1->parentWidget()->updateGeometry();
}

void ProgressWidget::signal_break() { mp_pr->signal_break(); }

QSize ProgressWidget::sizeHint() const { return QSize(400, 50); }
