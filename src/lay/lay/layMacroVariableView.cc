 ...
 */
class PlaceholderItem : public QTreeWidgetItem {
public:
  PlaceholderItem(gsi::Inspector *inspector) : mp_inspector(inspector) {
    // .. nothing yet ..
  }

  gsi::Inspector *inspector() { return mp_inspector.get(); }
...
QString pretty_print(const tl::Variant &v) {
  ...
  } else if (v.is_ulong() || v.is_long() || v.is_ulonglong() ||
             v.is_longlong()) {
 ...
 */
static QString inspector_description(const gsi::Inspector *inspector) {
  try {
    return tl::to_qstring(inspector->description());
  } catch (tl::ScriptError &ex) {
    return QObject::tr("Error") + QString::fromUtf8(": ") +
 ...
 *  This function also adds an error catch to show evaluation errors
 */
static QString inspector_value(const gsi::Inspector *inspector, int index) {
  try {
 ...
 */
static void update_value(QTreeWidgetItem *item, const QString &text,
                         bool fresh) {
  int column = 1;
...
void MacroVariableView::sync_item(QTreeWidgetItem *parent,
                                  gsi::Inspector *inspector, const QString &key,
                                  size_t index, int pos, bool fresh) {
...
void MacroVariableView::sync(QTreeWidgetItem *parent, gsi::Inspector *inspector,
                             bool fresh) {
  if (inspector->has_keys()) {
    ...
    for (size_t n = inspector->count(); n-- > 0;) {
      ...
      if (vis == gsi::Inspector::Always ||
          (m_show_all && vis == gsi::Inspector::IfRequested)) {
    ...
    int i = 0;
    for (std::map<QString, size_t>::const_iterator k = keys.begin();
         k != keys.end(); ++k, ++i) {
    ...
    for (size_t i = 0; i < n; ++i) {
      sync_item(parent, inspector, QString::fromUtf8("[%1]").arg(i), i, int(i),
                fresh);
    }
  }
}
