...
ConfirmationDialog::ConfirmationDialog(QWidget *parent)
    : QDialog(parent), m_confirmed(false), m_cancelled(false), m_aborted(false),
      m_file(50000, true) {
  ...
  connect(&m_file, SIGNAL(attention_changed(bool)), attn_frame,
          SLOT(setVisible(bool)));
}

void ConfirmationDialog::add_info(const std::string &name, bool update,
  ...
  for (int column = 0; column < list->colorCount(); ++column) {
    item->setData(
        column, Qt::ForegroundRole,
        QVariant(QBrush(update ? QColor(Qt::blue) : QColor(Qt::black))));
  }
}

void ConfirmationDialog::separator() { m_file.separator(); }

void ConfirmationDialog::mark_fetching(const std::string &name) {
  std::map<std::string, QTreeWidgetItem *>::const_iterator i =
      m_items_by_name.find(name);
...
void ConfirmationDialog::set_progress(const std::string &name,
                                      double progress) {
  std::map<std::string, QTreeWidgetItem *>::const_iterator i =
      m_items_by_name.find(name);
  if (i != m_items_by_name.end()) {
    i->second->setData(1, Qt::DisplayRole,
                       tl::to_qstring(tl::sprintf("%.1f%%", progress)));
  }
}

void ConfirmationDialog::set_icon_for_name(const std::string &name,
                                           const QIcon &icon) {
  std::map<std::string, QTreeWidgetItem *>::const_iterator i =
      m_items_by_name.find(name);
  if (i != m_items_by_name.end()) {
...
void SaltDownloadManager::register_download(const std::string &name,
...
void SaltDownloadManager::compute_dependencies(const lay::Salt &salt,
                                               const lay::Salt &salt_mine) {
  compute_list(salt, salt_mine, true);
}

void SaltDownloadManager::compute_packages(const lay::Salt &salt,
                                           const lay::Salt &salt_mine) {
  compute_list(salt, salt_mine, false);
}

void SaltDownloadManager::compute_list(const lay::Salt &salt,
                                       const lay::Salt &salt_mine,
                                       bool with_dep) {
  tl::AbsoluteProgress progress(
      tl::to_string(QObject::tr("Computing package dependencies ..")));
  ...
  while (needs_iteration()) {
    ...
    for (std::vector<Descriptor>::const_iterator p = m_registry.begin();
         p != m_registry.end(); ++p) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      for (std::vector<SaltGrainDependency>::const_iterator d =
               p.grain.dependencies().begin();
           d != p.grain.dependencies().end(); ++d) {

        std::map<std::string, size_t>::iterator r = reg_by_name.find(d->name);
        if (r != reg_by_name.end()) {

          //  Dependency is already scheduled for installation - check if we
          ...
          if (SaltGrain::compare_versions(pd.version, d->version) < 0) {
            ...
            //  Grain is present, but too old -> update version and reload in
            //  the next iteration
            if (tl::verbosity() >= 20) {
              tl::log
                  << "Upgrading installation request as required by package "
                  << p.name << ": " << d->name << " (" << d->version
                  << ") with URL " << d->url;
            }
          ...
          if (g) {
            ...
            if (SaltGrain::compare_versions(g->version(), d->version) < 0) {
              ...
              if (tl::verbosity() >= 20) {
                tl::log << "Considering for update as dependency: " << d->name
                        << " (" << d->version << ") with URL " << d->url;
              }
              m_registry.push_back(
                  Descriptor(d->name, std::string(), d->url, d->version));

            } else {
              if (tl::verbosity() >= 20) {
                tl::log << "Dependency already satisfied: " << d->name << "("
                        << d->version << ")";
              }
            }
            ...
            if (tl::verbosity() >= 20) {
              tl::log << "Considering for download as dependency: " << d->name
                      << " (" << d->version << ") with URL " << d->url;
            }
            m_registry.push_back(
                Descriptor(d->name, std::string(), d->url, d->version));
...
void SaltDownloadManager::fetch_missing(const lay::Salt &salt,
                                        const lay::Salt &salt_mine,
                                        tl::AbsoluteProgress &progress) {
  ...
  for (std::vector<Descriptor>::const_iterator p = registry.begin();
       p != registry.end(); ++p) {
    if (p + 1 != registry.end()) {
      if (p->name != p[1].name) {
  ...
  for (std::vector<Descriptor>::iterator p = m_registry.begin();
       p != m_registry.end(); ++p) {
    ...
    if (!p->downloaded) {
      ...
      if (!p->name.empty()) {
        ...
        if (!g) {
          if (p->url.empty()) {
            throw tl::Exception(
                tl::to_string(QObject::tr("Package '%1' not found in index - "
                                          "cannot resolve download URL")
                                  .arg(tl::to_qstring(p->name))));
          }
        } else {
          if (p->url.empty()) {
            if (tl::verbosity() >= 20) {
              tl::log << "Resolved package URL for package " << p->name << ": "
                      << g->url();
            }
            p->url = g->url();
          }
      ...
      try {
        ...
        throw tl::Exception(tl::to_string(
            QObject::tr("Error fetching spec file for package '%1': %2")
                .arg(tl::to_qstring(p->name))
                .arg(tl::to_qstring(ex.msg()))));
      }
      ...
      if (SaltGrain::compare_versions(p->grain.version(), p->version) < 0) {
        throw tl::Exception(tl::to_string(
            QObject::tr("Package '%1': package in repository is too old (%2) "
                        "to satisfy requirements (%3)")
                .arg(tl::to_qstring(p->name))
                .arg(tl::to_qstring(p->grain.version()))
                .arg(tl::to_qstring(p->version))));
      }
    }
  }

  //  remove those registered entries which don't need to be updated (we do this
  //  after download since now the names should be known when only the URL is
  //  given)
  ...
  for (std::vector<Descriptor>::const_iterator p = registry.begin();
    ...
    if (g) {
      if (SaltGrain::compare_versions(p->version, g->version()) <= 0 &&
          p->url == g->url()) {
        if (tl::verbosity() >= 20) {
          tl::log << "Package already present with sufficient version - not "
                     "installed again: "
                  << p->name << " (" << p->version << ")";
        }
      } else {
        if (tl::verbosity() >= 20) {
          tl::log << "Considering package for upgrade or URL switch: "
                  << p->name << ", from " << g->url() << "(" << g->version()
                  << ") to " << p->url << "(" << p->version << ")";
        }
        m_registry.push_back(*p);
      }
...
SaltDownloadManager::make_confirmation_dialog(QWidget *parent,
                                              const lay::Salt &salt) {
  ...
  for (std::vector<Descriptor>::const_iterator p = m_registry.begin();
    ...
    if (g) {
      ...
      dialog->add_info(p->name, true,
                       g->version() + " \342\206\222 " + p->version, p->url);
    }
  }
...
namespace {
class DownloadProgressAdaptor : public tl::ProgressAdaptor,
                                public tl::InputHttpStreamCallback {
public:
  DownloadProgressAdaptor(lay::ConfirmationDialog *dialog,
  ...
  virtual void wait_for_input() { yield(0); }

  virtual void trigger(tl::Progress *progress) {
    mp_dialog->set_progress(m_name, progress->value());
  }

  void error() { mp_dialog->mark_error(m_name); }

  void success() { mp_dialog->mark_success(m_name); }
...
bool SaltDownloadManager::execute(lay::SaltManagerDialog *parent,
                                  lay::Salt &salt) {
  bool result = true;
  ...
  if (parent) {
    ...
    if (m_registry.empty()) {
      QMessageBox::warning(
          parent, tr("Nothing to do"),
          tr("No packages need update or are marked for installation"));
      return true;
    }

    std::unique_ptr<lay::ConfirmationDialog> dialog(
        make_confirmation_dialog(parent, salt));
    ...
    for (std::vector<Descriptor>::const_iterator p = m_registry.begin();
      ...
      try {
        ...
        if (!p->token.empty()) {
          std::string fb_url = parent->salt_mine_url() + "?token=" + p->token +
                               "&status=" + tl::to_string(status);
    ...
    for (std::vector<Descriptor>::const_iterator p = m_registry.begin();
      ...
      if (!salt.create_grain(p->grain, target,
                             60.0 /*timeout for offline installation*/)) {
        tl::error << tl::to_string(
            QObject::tr("Installation failed for package %1")
                .arg(tl::to_qstring(target.name())));
        result = false;
      } else {
        tl::log << tl::to_string(
            QObject::tr("Package %1 installed successfully")
                .arg(tl::to_qstring(target.name())));
      }
    }
  }
