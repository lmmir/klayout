...
FontController::FontController()
    : m_file_watcher(0), dm_sync_dirs(this, &FontController::sync_dirs) {}

void FontController::initialize(lay::Dispatcher * /*root*/) {
  //  NOTE: we initialize the dirs in the stage once to have them available for
  //  the autorun macros. We'll do that later again in order to pull in the dirs
  //  from the packages.
  sync_dirs();
}
...
void FontController::get_options(
    std::vector<std::pair<std::string, std::string>> & /*options*/) const {
  //  .. nothing yet ..
}

void FontController::get_menu_entries(
    std::vector<lay::MenuEntry> & /*menu_entries*/) const {
  //  .. nothing yet ..
}

bool FontController::configure(const std::string & /*name*/,
...
void FontController::file_watcher_triggered() {
  tl::log << tl::to_string(
      tr("Detected file system change in fonts - updating"));
  dm_sync_dirs();
}
...
static tl::RegisteredClass<lay::PluginDeclaration>
    font_controller_decl(new lay::FontController(), 160, "FontController");
