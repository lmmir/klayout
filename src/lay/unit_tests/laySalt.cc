...
static std::string grains_to_string(const lay::SaltGrains &gg,
  ...
  for (lay::SaltGrains::collection_iterator gc = gg.begin_collections();
       gc != gg.end_collections(); ++gc) {
    if (!first) {
      res += ",";
    }
    first = false;
...
TEST(1) {
  ...
  EXPECT_EQ(QDateTime::fromMSecsSinceEpoch(0).msecsTo(g.installed_time()),
            2000000000);
#endif
...
TEST(3) {
  ...
  EXPECT_EQ(gg.begin_collections()->path(),
            tl::to_string(dir_c.absolutePath()));
...
TEST(4) {
  ...
  (const_cast<lay::SaltGrains &>(*salt_copy.begin()))
      .remove_grain(salt_copy.begin()->begin_grains(), true);
...
TEST(6) {
  ...
  gg1.merge_with(gg2);
  EXPECT_EQ(grains_to_string(gg1, true),
            "[a[b(),a(1.1:url2),c()],b[x()],c[y()]]");
}
