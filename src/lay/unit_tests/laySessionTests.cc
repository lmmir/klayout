...
TEST(1) {
  ...
  if (ant_service) {
    ...
    ant_service->insert_ruler(ruler,
                              false /*do not observe the ruler count limit*/);
  }
  ...
  std::unique_ptr<db::LayoutToNetlist> l2ndb(
      db::LayoutToNetlist::create_from_file(tl::testdata() +
                                            "/sessions/test.l2n"));
  view->add_l2ndb(l2ndb.release());

  std::unique_ptr<db::LayoutToNetlist> lvsdb(
      db::LayoutToNetlist::create_from_file(tl::testdata() +
                                            "/sessions/test.lvsdb"));
  ...
  EXPECT_EQ(dynamic_cast<db::LayoutVsSchematic *>(view->get_l2ndb(1)) != 0,
            true);
...
TEST(2) {
  ...
  EXPECT_EQ(mw->views(), (unsigned int)0);
  mw->restore_session(tl::testdata() +
                      "/sessions/test_with_relative_paths.lys");
