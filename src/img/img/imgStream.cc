...
public:
  ImageProxy(const img::Object *img = 0)
      : mp_img(img), m_width(1), m_height(1), m_min_value(0.0),
        m_max_value(1.0), m_color(false) {
    init();
  }

  bool is_color() const { return mp_img->is_color(); }

  void set_color(bool f) { m_color = f; }

  size_t width() const { return mp_img->width(); }

  void set_width(size_t w) { m_width = w; }

  size_t height() const { return mp_img->height(); }
  ...
  void push_byte_data(const std::string &s) { m_byte_data.push_back(s); }
  ...
  void push_data(const std::string &s) { m_data.push_back(s); }

  const db::Matrix3d &matrix() const { return mp_img->matrix(); }

  void set_matrix(const db::Matrix3d &m) { m_matrix = m; }

  double min_value() const { return mp_img->min_value(); }

  void set_min_value(double h) { m_min_value = h; }

  double max_value() const { return mp_img->max_value(); }

  void set_max_value(double h) { m_max_value = h; }

  const img::DataMapping &data_mapping() const {
    return mp_img->data_mapping();
  }

  void set_data_mapping(const img::DataMapping &dm) { m_data_mapping = dm; }
...
static void string_to_pixels(img::Object *img, const std::string &s, size_t row,
                             size_t w, bool color) {
...
img::Object *ImageProxy::get_image() const {
  std::unique_ptr<img::Object> img(
      new Object(std::max(size_t(1), m_width), std::max(size_t(1), m_height),
                 m_matrix, m_color, !m_byte_data.empty()));
  ...
  if (!m_byte_data.empty()) {
    ...
    for (size_t i = 0; i < m_height; ++i) {
      string_to_pixels<unsigned char, unsigned char>(img.get(), *s++, i,
                                                     m_width, m_color);
    }
    ...
    for (size_t i = 0; i < m_height; ++i) {
      string_to_pixels<float, unsigned char>(img.get(), *s++, i, m_width,
                                             m_color);
    }
  }
...
static const std::string &data_to_string(std::string &heap, size_t l,
                                         const T1 *r, const T2 *g, const T3 *b,
                                         const T4 *m) {
  heap.clear();
  ...
  if (mp_img->is_color()) {
    ...
    if (mp_img->is_byte_data()) {
      ...
      for (size_t i = 0; i < h; ++i) {
        m_byte_data.push_back(data_to_string(s, w, r + i * w, g + i * w,
                                             b + i * w, m ? (m + i * w) : 0));
      }
      ...
      for (size_t i = 0; i < h; ++i) {
        m_data.push_back(data_to_string(s, w, r + i * w, g + i * w, b + i * w,
                                        m ? (m + i * w) : 0));
      }
    }
    ...
    if (mp_img->is_byte_data()) {
      ...
      for (size_t i = 0; i < h; ++i) {
        m_byte_data.push_back(
            data_to_string(s, w, g + i * w, (const unsigned char *)0,
                           (const unsigned char *)0, m ? (m + i * w) : 0));
      }
      ...
      for (size_t i = 0; i < h; ++i) {
        m_data.push_back(data_to_string(s, w, g + i * w, (const float *)0,
                                        (const float *)0, m ? (m + i * w) : 0));
      }
    }
  }
}
...
struct PointConverter {
  std::string to_string(const db::DPoint &p) const { return p.to_string(); }
...
struct ColorMapConverter {
  std::string to_string(
      const std::pair<double, std::pair<tl::Color, tl::Color>> &cm) const {
    std::string s;
  ...
  from_string(const std::string &s,
              std::pair<double, std::pair<tl::Color, tl::Color>> &cm) const {
    tl::Extractor ex(s.c_str());
...
tl::XMLStruct<ImageProxy> s_img_structure(
    "image-data",
    tl::make_member(&ImageProxy::is_color, &ImageProxy::set_color, "color") +
        tl::make_member(&ImageProxy::width, &ImageProxy::set_width, "width") +
        tl::make_member(&ImageProxy::height, &ImageProxy::set_height,
                        "height") +
        tl::make_member(&ImageProxy::matrix, &ImageProxy::set_matrix,
                        "matrix") +
        tl::make_member(&ImageProxy::min_value, &ImageProxy::set_min_value,
                        "min-value") +
        tl::make_member(&ImageProxy::max_value, &ImageProxy::set_max_value,
                        "max-value") +
        tl::make_element(
            &ImageProxy::data_mapping, &ImageProxy::set_data_mapping,
            "data-mapping",
            tl::make_element(
                &img::DataMapping::false_color_nodes, "color-map",
                tl::make_member<
                    std::pair<double, std::pair<tl::Color, tl::Color>>,
                    img::DataMapping::false_color_nodes_type::const_iterator,
                    img::DataMapping::false_color_nodes_type,
                    ColorMapConverter>(
                    &img::DataMapping::false_color_nodes_type::begin,
                    &img::DataMapping::false_color_nodes_type::end,
                    &img::DataMapping::false_color_nodes_type::push_back,
                    "color-map-entry", ColorMapConverter())) +
        ...
        tl::make_element(
            &ImageProxy::landmarks, &ImageProxy::set_landmarks, "landmarks",
            tl::make_member<db::DPoint,
                            img::Object::landmarks_type::const_iterator,
                            img::Object::landmarks_type, PointConverter>(
                &img::Object::landmarks_type::begin,
                &img::Object::landmarks_type::end,
                &img::Object::landmarks_type::push_back, "landmark",
                PointConverter())) +
        tl::make_member(&ImageProxy::begin_byte_data,
                        &ImageProxy::end_byte_data, &ImageProxy::push_byte_data,
                        "byte-data") +
        tl::make_member(&ImageProxy::begin_data, &ImageProxy::end_data,
                        &ImageProxy::push_data, "data"));

// --------------------------------------------------------------------------------------------------------------------------

img::Object *ImageStreamer::read(tl::InputStream &stream) {
  ImageProxy proxy;

  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Reading image file: ")) +
                          stream.source());
...
void ImageStreamer::write(tl::OutputStream &stream, const img::Object &img) {
  ...
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Writing image file: ")) +
                          stream.path());
