...
PropertiesPage::PropertiesPage(img::Service *service, db::Manager *manager,
                               QWidget *parent)
...
void PropertiesPage::init() {
  ...
  connect(colors, SIGNAL(color_changed(std::pair<QColor, QColor>)),
          false_color_control,
          SLOT(set_current_color(std::pair<QColor, QColor>)));
  connect(false_color_control,
          SIGNAL(selection_changed(std::pair<QColor, QColor>)), colors,
          SLOT(set_color(std::pair<QColor, QColor>)));
  connect(false_color_control,
          SIGNAL(selection_changed(std::pair<QColor, QColor>)), this,
          SLOT(color_mapping_changed()));
  connect(false_color_control, SIGNAL(color_mapping_changed()), this,
          SLOT(color_mapping_changed()));

  connect(brightness_slider, SIGNAL(valueChanged(int)), this,
          SLOT(brightness_slider_changed(int)));
  connect(brightness_sb, SIGNAL(valueChanged(int)), this,
          SLOT(brightness_spinbox_changed(int)));
  connect(contrast_slider, SIGNAL(valueChanged(int)), this,
          SLOT(contrast_slider_changed(int)));
  connect(contrast_sb, SIGNAL(valueChanged(int)), this,
          SLOT(contrast_spinbox_changed(int)));
  connect(gamma_slider, SIGNAL(valueChanged(int)), this,
          SLOT(gamma_slider_changed(int)));
  connect(gamma_sb, SIGNAL(valueChanged(double)), this,
          SLOT(gamma_spinbox_changed(double)));
  connect(r_slider, SIGNAL(valueChanged(int)), this,
          SLOT(red_slider_changed(int)));
  connect(r_sb, SIGNAL(valueChanged(double)), this,
          SLOT(red_spinbox_changed(double)));
  connect(g_slider, SIGNAL(valueChanged(int)), this,
          SLOT(green_slider_changed(int)));
  connect(g_sb, SIGNAL(valueChanged(double)), this,
          SLOT(green_spinbox_changed(double)));
  connect(b_slider, SIGNAL(valueChanged(int)), this,
          SLOT(blue_slider_changed(int)));
  connect(b_sb, SIGNAL(valueChanged(double)), this,
          SLOT(blue_spinbox_changed(double)));

  connect(from_le, SIGNAL(editingFinished()), this,
          SLOT(min_max_value_changed()));
  connect(to_le, SIGNAL(editingFinished()), this,
          SLOT(min_max_value_changed()));
  ...
  connect(define_landmarks_pb, SIGNAL(clicked()), this,
          SLOT(define_landmarks_pressed()));
}
...
size_t PropertiesPage::count() const { return m_selection.size(); }
...
std::string PropertiesPage::description(size_t entry) const {
  const img::Object *obj =
      dynamic_cast<const img::Object *>(m_selection[entry]->ptr());
  if (!obj) {
...
std::string PropertiesPage::description() const {
  return tl::to_string(tr("Images"));
}

void PropertiesPage::leave() { mp_service->clear_highlights(); }
...
bool PropertiesPage::readonly() { return false; }

void PropertiesPage::get_xmin_xmax(double &xmin, double &xmax,
                                   bool &has_error_out) {
  ...
  if (!has_error && xmin >= xmax) {
    tl::Exception ex(tl::to_string(QObject::tr(
        "Invalid data value range (min. value must be less than max. value)")));
    lay::indicate_error(from_le, &ex);
    lay::indicate_error(to_le, &ex);
    has_error = true;
  }
...
void PropertiesPage::min_max_value_changed() {
  ...
  if (false_color_control->has_selection() &&
      false_color_control->selected_node() > 0 &&
      false_color_control->selected_node() <
          int(false_color_control->nodes().size()) - 1) {

    double x =
        false_color_control->nodes()[false_color_control->selected_node()]
            .first;
...
void PropertiesPage::value_changed() {
  ...
  if (!has_error && (xx < 0 || xx > 1.0)) {
    tl::Exception ex(
        tl::to_string(QObject::tr("The position entered (%g) must be between "
                                  "the minimum (%g) and maximum (%g) value")),
        x, xmin, xmax);
    lay::indicate_error(value_le, &ex);
    has_error = true;
  }
...
inline double round_to_zero(double x) { return 1e-6 * floor(0.5 + 1e6 * x); }
...
void PropertiesPage::update() {
  ...
  if (mp_service) {
    ...
    if (!mp_direct_image) {
      const img::Object *image =
          dynamic_cast<const img::Object *>(m_selection[m_index]->ptr());
  ...
  if (!mp_direct_image->is_empty()) {
    file_info_lbl->setText(tl::to_qstring(tl::sprintf(
        tl::to_string(QObject::tr("%lu x %lu pixels (%s)")),
        mp_direct_image->width(), mp_direct_image->height(), mode)));
  } else {
  ...
  //  The observer distance for perspective distortion is the average of the
  //  images width and height. The coordinate space origin in the center of the
  //  image.
  ...
  x_offset_le->setText(
      tl::to_qstring(tl::micron_to_string(round_to_zero(matrix.disp().x()))));
  x_offset_le->setCursorPosition(0);

  y_offset_le->setText(
      tl::to_qstring(tl::micron_to_string(round_to_zero(matrix.disp().y()))));
  y_offset_le->setCursorPosition(0);

  angle_le->setText(
      tl::to_qstring(tl::to_string(round_to_zero(matrix.angle()))));
  angle_le->setCursorPosition(0);

  shear_le->setText(
      tl::to_qstring(tl::to_string(round_to_zero(matrix.shear_angle()))));
  shear_le->setCursorPosition(0);

  persp_tx_le->setText(tl::to_qstring(
      tl::to_string(round_to_zero(matrix.perspective_tilt_x(z)))));
  persp_tx_le->setCursorPosition(0);

  persp_ty_le->setText(tl::to_qstring(
      tl::to_string(round_to_zero(matrix.perspective_tilt_y(z)))));
  ...
  false_color_control->set_nodes(
      mp_direct_image->data_mapping().false_color_nodes);

  brightness_slider->setValue(
      int(floor(mp_direct_image->data_mapping().brightness * 100 + 0.5)));
  brightness_sb->setValue(
      int(floor(mp_direct_image->data_mapping().brightness * 100 + 0.5)));

  contrast_slider->setValue(
      int(floor(mp_direct_image->data_mapping().contrast * 100 + 0.5)));
  contrast_sb->setValue(
      int(floor(mp_direct_image->data_mapping().contrast * 100 + 0.5)));
  ...
  if (mp_direct_image->data_mapping().gamma < 1.0) {
    gamma_slider->setValue(
        50 - int(0.5 + (1.0 / mp_direct_image->data_mapping().gamma - 1.0) /
                           (1.0 / min_gamma - 1.0) * 50.0));
  } else {
    gamma_slider->setValue(
        50 + int(0.5 + (mp_direct_image->data_mapping().gamma - 1.0) /
                           (max_gamma - 1.0) * 50.0));
  }

  r_slider->setValue(
      int(0.5 + mp_direct_image->data_mapping().red_gain * 50.0));
  g_slider->setValue(
      int(0.5 + mp_direct_image->data_mapping().green_gain * 50.0));
  b_slider->setValue(
      int(0.5 + mp_direct_image->data_mapping().blue_gain * 50.0));
...
void PropertiesPage::gamma_spinbox_changed(double value) {
  ...
  if (value < 1.0) {
    gamma_slider->setValue(
        50 - int(0.5 + (1.0 / value - 1.0) / (1.0 / min_gamma - 1.0) * 50.0));
  } else {
    gamma_slider->setValue(50 +
                           int(0.5 + (value - 1.0) / (max_gamma - 1.0) * 50.0));
  }
...
void PropertiesPage::white_to_black() {
  std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>> nodes;
  nodes.push_back(std::make_pair(
      0.0, std::make_pair(tl::Color(255, 255, 255), tl::Color(255, 255, 255))));
  nodes.push_back(std::make_pair(
      1.0, std::make_pair(tl::Color(0, 0, 0), tl::Color(0, 0, 0))));
  false_color_control->set_nodes(nodes);
  emit edited();
}
...
void PropertiesPage::red_to_blue() {
  std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>> nodes;
  nodes.push_back(std::make_pair(
      0.0, std::make_pair(tl::Color(255, 0, 0), tl::Color(255, 0, 0))));
  nodes.push_back(std::make_pair(
      1.0, std::make_pair(tl::Color(0, 0, 255), tl::Color(0, 0, 255))));
  false_color_control->set_nodes(nodes);
  emit edited();
}
...
void PropertiesPage::blue_to_red() {
  std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>> nodes;
  nodes.push_back(std::make_pair(
      0.0, std::make_pair(tl::Color(0, 0, 255), tl::Color(0, 0, 255))));
  nodes.push_back(std::make_pair(
      1.0, std::make_pair(tl::Color(255, 0, 0), tl::Color(255, 0, 0))));
  false_color_control->set_nodes(nodes);
  emit edited();
}
...
void PropertiesPage::apply() {
  ...
  double w = matrix.mag_x(), h = matrix.mag_y(), x = matrix.disp().x(),
         y = matrix.disp().y(), a = matrix.angle(), sa = matrix.shear_angle(),
         tx = matrix.perspective_tilt_x(z), ty = matrix.perspective_tilt_y(z);

  try {
    tl::from_string_ext(tl::to_string(width_le->text()), w);
    if (w <= 0.0 || h <= 0.0) {
      throw tl::Exception(tl::to_string(QObject::tr(
          "Pixel width or height must be positive, non-null values")));
    }
  ...
  try {
    ...
    if (sa <= -45 || sa >= 45) {
      throw tl::Exception(tl::to_string(QObject::tr(
          "The shear angle must be larger than -45 and less than 45 degree")));
    }
  ...
  try {
    ...
    if (tx <= -90 || tx >= 90) {
      throw tl::Exception(tl::to_string(
          QObject::tr("The perspective tilt angles must be larger than -90 and "
                      "less than 90 degree")));
    }
  ...
  if (has_error) {
    throw tl::Exception(tl::to_string(
        tr("At least one value is invalid - see highlighted entry fields")));
  }
  ...
  matrix = db::Matrix3d::disp(db::DVector(x, y)) *
           db::Matrix3d::perspective(tx, ty, z) * db::Matrix3d::rotation(a) *
           db::Matrix3d::shear(sa) * db::Matrix3d::mag(w, h) *
           db::Matrix3d::mirror(mirror);
...
void PropertiesPage::browse() {
  ...
  lay::FileDialog file_dialog(this,
                              tl::to_string(QObject::tr("Load Image File")),
...
void PropertiesPage::save_pressed() {
  ...
  lay::FileDialog file_dialog(
      this, tl::to_string(QObject::tr("Save As KLayout Image File")),
      tl::to_string(
          QObject::tr("KLayout image files (*.lyimg);;All files (*)")));
