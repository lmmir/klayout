...
TwoColorWidget::TwoColorWidget(QWidget *parent) : QFrame(parent) {
  ...
  connect(mp_left, SIGNAL(color_changed(QColor)), this,
          SLOT(lcolor_changed(QColor)));
  connect(mp_right, SIGNAL(color_changed(QColor)), this,
          SLOT(rcolor_changed(QColor)));
...
void TwoColorWidget::set_single_mode(bool f) { mp_lock->setEnabled(!f); }
...
void TwoColorWidget::lcolor_changed(QColor) {
  ...
  emit color_changed(
      std::make_pair(mp_left->get_color(), mp_right->get_color()));
}
...
void TwoColorWidget::lock_changed(bool checked) {
  if (checked) {
    ...
    QColor ca((cl.red() + cr.red()) / 2, (cl.green() + cr.green()) / 2,
              (cl.blue() + cr.blue()) / 2);
...
void ColorBar::set_current_color(std::pair<QColor, QColor> c) {
  if (has_selection()) {
    m_nodes[m_selected].second =
        std::make_pair(tl::Color(c.first.rgb()), tl::Color(c.second.rgb()));
    emit color_mapping_changed();
    update();
  }
}
...
void ColorBar::set_current_position(double x) {
  if (has_selection() && x > min_value_interval &&
      x < 1.0 - min_value_interval) {

    //  some heuristics to keep the list ordered and to avoid small intervals
    m_nodes[m_selected].first = x;

    while (m_selected > 0 &&
           m_nodes[m_selected].first < m_nodes[m_selected - 1].first) {
      std::swap(m_nodes[m_selected], m_nodes[m_selected - 1]);
      --m_selected;
    }

    while (m_selected < int(m_nodes.size() - 1) &&
           m_nodes[m_selected].first > m_nodes[m_selected + 1].first) {
      std::swap(m_nodes[m_selected], m_nodes[m_selected + 1]);
      ++m_selected;
    }
    ...
    while (m_selected < int(m_nodes.size() - 1) &&
           fabs(m_nodes[m_selected].first - m_nodes[m_selected + 1].first) <
               min_value_interval) {
...
void ColorBar::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Delete && has_selection() && m_selected > 0 &&
      m_selected < int(m_nodes.size()) - 1) {
...
void ColorBar::set_nodes(
    const std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>
        &nodes) {
  m_nodes = nodes;
  ...
  if (m_nodes.size() == 0 || fabs(m_nodes[0].first) > epsilon) {
    m_nodes.insert(m_nodes.begin(),
                   std::make_pair(0.0, std::make_pair(tl::Color(0, 0, 0),
                                                      tl::Color(0, 0, 0))));
  } else {
    m_nodes[0].first = 0.0;
  }

  std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::iterator w =
      m_nodes.begin();
  std::vector<
      std::pair<double, std::pair<tl::Color, tl::Color>>>::const_iterator nn =
      m_nodes.begin();
  for (std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::
           const_iterator n = m_nodes.begin() + 1;
       n != m_nodes.end(); ++n) {
  ...
  *w++ = *nn;
  ...
  if (m_nodes.back().first > 1.0 - min_value_interval) {
    ...
    m_nodes.push_back(
        std::make_pair(1.0, std::make_pair(tl::Color(255, 255, 255),
...
void ColorBar::mousePressEvent(QMouseEvent *event) {
  ...
  if (event->x() > xl - 5 && event->x() < xr + 5 && event->y() > yb - 5 &&
      event->y() < yb + 5 + indicator_height + indicator_spacing) {

    double xx = double(event->x() - xl) / double(xr - xl);

    double dmin = 100.0;
    std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::
        const_iterator pmin = m_nodes.end();
    for (std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::
             const_iterator p = m_nodes.begin();
         p != m_nodes.end(); ++p) {
    ...
    if (pmin != m_nodes.end()) {
      m_selected = int(std::distance(
          std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::
              const_iterator(m_nodes.begin()),
          pmin));
      emit selection_changed();
      std::pair<tl::Color, tl::Color> cp = m_nodes[m_selected].second;
      emit selection_changed(
          std::make_pair(QColor(cp.first.rgb()), QColor(cp.second.rgb())));
      m_dragging = true;
      update();
    } else {
      m_selected = -1;
...
void ColorBar::mouseDoubleClickEvent(QMouseEvent *event) {
  ...
  if (event->x() > xl && event->x() < xr && event->y() > yb - 5 &&
      event->y() < yb + 5 + indicator_height + indicator_spacing) {

    double xx = double(event->x() - xl) / double(xr - xl);

    std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>::iterator
        p = std::lower_bound(
            m_nodes.begin(), m_nodes.end(),
            std::make_pair(xx, std::make_pair(tl::Color(), tl::Color())),
...
QSize ColorBar::sizeHint() const {
  return QSize(100, frame_width * 2 + indicator_height + indicator_spacing +
                        nominal_bar_height);
}
...
void ColorBar::paintEvent(QPaintEvent *) {
  ...
  size_t h_max = 0;
  for (std::vector<size_t>::const_iterator h = m_histogram.begin();
       h != m_histogram.end(); ++h) {
    if (*h > h_max) {
      h_max = *h;
    }
  ...
  for (int x = xl; x <= xr; ++x) {
    ...
    if (m_histogram.size() > 0 && xr > xl) {
      ...
      hbar = int((hbar - min_bar_height) * double(h) / double(n) + 0.5 +
                 min_bar_height);
    }
  ...
  for (unsigned int i = 0; i < m_nodes.size(); ++i) {
    ...
    QPoint points[3] = {
        ...
        QPoint(x - indicator_height / 2,
               yb + indicator_spacing + indicator_height),
        QPoint(x + indicator_height / 2,
               yb + indicator_spacing + indicator_height),
    };
