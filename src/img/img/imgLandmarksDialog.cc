...
class IMG_PUBLIC LandmarkMarker : public lay::ViewObject {
public:
  /**
   *  @brief Constructor attaching to a certain object
   */
  LandmarkMarker(lay::ViewService *service, const db::DPoint &pos,
                 bool selected)
      : lay::ViewObject(service->ui()), mp_service(service), m_pos(pos),
        m_selected(selected), m_position_set(true) {
    //  .. nothing yet ..
  }
  ...
  LandmarkMarker(lay::ViewService *service, bool selected)
      : lay::ViewObject(service->ui()), mp_service(service), m_pos(),
        m_selected(selected), m_position_set(false) {
    //  .. nothing yet ..
  }
   ...
   */
  const db::DPoint &position() const { return m_pos; }
...
private:
  ...
  virtual void render(const lay::Viewport &vp, lay::ViewObjectCanvas &canvas) {
    ...
    canvas.renderer().draw(
        db::DBox(m_pos - db::DVector(s, s), m_pos + db::DVector(s, s)),
        vp.trans(), plane_fill, plane_frame, 0, 0);
    canvas.renderer().draw(db::DEdge(m_pos - db::DVector(0, s * 3.0),
                                     m_pos + db::DVector(0, s * 3.0)),
                           vp.trans(), plane_fill, plane_frame, 0, 0);
    canvas.renderer().draw(db::DEdge(m_pos - db::DVector(s * 3.0, 0),
                                     m_pos + db::DVector(s * 3.0, 0)),
                           vp.trans(), plane_fill, plane_frame, 0, 0);
  }
...
class LandmarkEditorService : public lay::ViewService {
public:
  LandmarkEditorService(lay::LayoutViewBase *view, img::Object *img)
      : lay::ViewService(view->canvas()), mp_image(img), m_selected(-1),
        m_dragging(false), m_mode(LandmarksDialog::None) {
    update();
  }
  ...
  bool mouse_click_event(const db::DPoint &p, unsigned int /*buttons*/,
                         bool prio) {
    if (prio) {
      ...
      if (m_dragging) {
        ...
        m_selected = -1;
        int li = 0;
        for (std::vector<db::DPoint>::const_iterator l =
                 mp_image->landmarks().begin();
             l != mp_image->landmarks().end(); ++l, ++li) {
  ...
  bool mouse_press_event(const db::DPoint & /*p*/, unsigned int /*buttons*/,
                         bool /*prio*/) {
    // ..
    return false;
  }
...
private:
  ...
  void clear() {
    for (std::vector<LandmarkMarker *>::iterator m = m_markers.begin();
         m != m_markers.end(); ++m) {
      delete *m;
    }
    m_markers.clear();
...
void LandmarksDialog::update_mode() {
  ...
  if (new_mode == Delete && sel.size() > 0) {
    ...
    for (QList<QListWidgetItem *>::const_iterator s = sel.begin();
         s != sel.end(); ++s) {
    ...
    int i = 0;
    for (std::vector<db::DPoint>::const_iterator r = lm.begin(); r != lm.end();
         ++r, ++i) {
...
void LandmarksDialog::landmarks_updated() {
  ...
  for (std::vector<db::DPoint>::const_iterator l =
           mp_image->landmarks().begin();
       l != mp_image->landmarks().end(); ++l) {
    landmark_list->addItem(
        tl::to_qstring(tl::sprintf("%.0f, %.0f", l->x(), l->y())));
  }
