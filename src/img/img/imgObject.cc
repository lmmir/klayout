...
DataMapping::DataMapping()
    : brightness(0.0), contrast(0.0), gamma(1.0), red_gain(1.0),
      green_gain(1.0), blue_gain(1.0) {
  false_color_nodes.push_back(std::make_pair(
      0.0, std::make_pair(tl::Color(0, 0, 0), tl::Color(0, 0, 0))));
  false_color_nodes.push_back(std::make_pair(
      1.0, std::make_pair(tl::Color(255, 255, 255), tl::Color(255, 255, 255))));
}
...
bool DataMapping::operator==(const DataMapping &d) const {
  ...
  for (unsigned int i = 0; i < false_color_nodes.size(); ++i) {
    if (fabs(false_color_nodes[i].first - d.false_color_nodes[i].first) >
        epsilon) {
      return false;
    }
    if (false_color_nodes[i].second.first !=
        d.false_color_nodes[i].second.first) {
      return false;
    }
    if (false_color_nodes[i].second.second !=
        d.false_color_nodes[i].second.second) {
      return false;
    }
  }
...
bool DataMapping::operator<(const DataMapping &d) const {
  ...
  for (unsigned int i = 0; i < false_color_nodes.size(); ++i) {
    ...
    if (false_color_nodes[i].second.first !=
        d.false_color_nodes[i].second.first) {
      return false_color_nodes[i].second.first.rgb() <
             d.false_color_nodes[i].second.first.rgb();
    }
    if (false_color_nodes[i].second.second !=
        d.false_color_nodes[i].second.second) {
      return false_color_nodes[i].second.second.rgb() <
             d.false_color_nodes[i].second.second.rgb();
    }
  }
...
tl::DataMappingBase *
DataMapping::create_data_mapping(bool monochrome, double xmin, double xmax,
  ...
  double m =
      contrast < 0.0 ? 1.0 / (1.0 - contrast * 2.0) : 1.0 + contrast * 2.0;
  ...
  if (monochrome && false_color_nodes.size() > 1) {
    ...
    for (unsigned int i = 1; i < false_color_nodes.size(); ++i) {
      ...
      //  The number of steps is chosen such that the full HSV band divides into
      //  approximately 200 steps
      double nsteps =
          0.5 * sqrt(double(dh * dh) + double(ds * ds) + double(dv * dv));
      int n = int(floor(nsteps + 1.0));
      double dx =
          (false_color_nodes[i].first - false_color_nodes[i - 1].first) / n;
    ...
    dm = new tl::CombinedDataMapping(
        ...
        new tl::LinearCombinationDataMapping(
            0.0,
            new tl::CombinedDataMapping(
                linear, new tl::CombinedDataMapping(gray_to_color, x_norm)),
            scale));

  } else {

    dm = new tl::CombinedDataMapping(
        to_pixel, new tl::LinearCombinationDataMapping(
...
struct compare_first_of_node {
  bool operator()(
      const std::pair<double, std::pair<tl::Color, tl::Color>> &a,
      const std::pair<double, std::pair<tl::Color, tl::Color>> &b) const {
...
tl::Color interpolated_color(const DataMapping::false_color_nodes_type &nodes,
  ...
  if (nodes.size() < 1) {
    ...
    std::vector<
        std::pair<double, std::pair<tl::Color, tl::Color>>>::const_iterator p =
        std::lower_bound(
            nodes.begin(), nodes.end(),
            std::make_pair(x, std::make_pair(tl::Color(), tl::Color())),
            compare_first_of_node());
    if (p == nodes.end()) {
      ...
      int h = int(0.5 + h1 +
                  double(x - x1) * double(int(h2) - int(h1)) / double(x2 - x1));
      int s = int(0.5 + s1 +
                  double(x - x1) * double(int(s2) - int(s1)) / double(x2 - x1));
      int v = int(0.5 + v1 +
                  double(x - x1) * double(int(v2) - int(v1)) / double(x2 - x1));

      return tl::Color::from_hsv((unsigned int)h, (unsigned int)s,
                                 (unsigned int)v);
    }
  }
}
...
public:
  ...
  DataHeader(size_t w, size_t h, unsigned char *red, unsigned char *green,
             unsigned char *blue, unsigned char *mask = 0)
      : m_width(w), m_height(h), m_ref_count(0) {
    mp_mask = mask;
    mp_byte_data = 0;
    mp_data = 0;
  ...
  DataHeader(size_t w, size_t h, float *red, float *green, float *blue,
  ...
  size_t data_length() const { return m_width * m_height; }

  unsigned char *mask() { return mp_mask; }
  ...
  unsigned char *byte_data(unsigned int i) { return mp_color_byte_data[i]; }

  float *float_data(unsigned int i) { return mp_color_data[i]; }

  unsigned char *byte_data() { return mp_byte_data; }

  float *float_data() { return mp_data; }

  bool has_mask() const { return mp_mask != 0; }
  ...
  void mem_stat(db::MemStatistics *stat, db::MemStatistics::purpose_t purpose,
    ...
    for (unsigned int i = 0; i < 3; ++i) {
      if (mp_color_data[i]) {
        stat->add(typeid(float[]), (void *)mp_color_data[i], n * sizeof(float),
                  n * sizeof(float), (void *)this, purpose, cat);
      }
      if (mp_color_byte_data[i]) {
        stat->add(typeid(unsigned char[]), (void *)mp_color_byte_data[i],
                  n * sizeof(unsigned char), n * sizeof(unsigned char),
                  (void *)this, purpose, cat);
      }
    }

    if (mp_mask) {
      stat->add(typeid(unsigned char[]), (void *)mp_mask,
                n * sizeof(unsigned char), n * sizeof(unsigned char),
                (void *)this, purpose, cat);
    }
    if (mp_data) {
      stat->add(typeid(float[]), (void *)mp_data, n * sizeof(float),
                n * sizeof(float), (void *)this, purpose, cat);
    }
    if (mp_byte_data) {
      stat->add(typeid(unsigned char[]), (void *)mp_byte_data,
                n * sizeof(unsigned char), n * sizeof(unsigned char),
                (void *)this, purpose, cat);
    }
  }
...
Object::Object()
    : m_trans(1.0), mp_data(0), m_id(make_id()), m_min_value(0.0),
      m_max_value(1.0), m_min_value_set(false), m_max_value_set(false),
      m_visible(true), m_z_position(0) {
...
Object::Object(size_t w, size_t h, const db::DCplxTrans &trans, bool color,
               bool byte_data)
    : m_trans(trans), m_id(make_id()), m_min_value(0.0), m_max_value(1.0),
      m_min_value_set(false), m_max_value_set(false), m_visible(true),
      m_z_position(0) {
  m_updates_enabled = false;
  mp_pixel_data = 0;
...
Object::Object(size_t w, size_t h, const db::DCplxTrans &trans,
               unsigned char *d)
    : m_trans(trans), m_id(make_id()), m_min_value(0.0), m_max_value(255.0),
      m_min_value_set(false), m_max_value_set(false), m_visible(true),
      m_z_position(0) {
  m_updates_enabled = false;
  mp_pixel_data = 0;
...
Object::Object(size_t w, size_t h, const db::DCplxTrans &trans, float *red,
...
Object::Object(const std::string &filename, const db::DCplxTrans &trans)
    : m_filename(filename), m_trans(trans), m_id(make_id()), m_min_value(0.0),
      m_max_value(1.0), m_min_value_set(false), m_max_value_set(false),
      m_visible(true), m_z_position(0) {
  m_updates_enabled = false;
  mp_pixel_data = 0;
...
Object::Object(const tl::PixelBuffer &pixel_buffer, const db::DCplxTrans &trans)
    : m_filename("<object>"), m_trans(trans), m_id(make_id()), m_min_value(0.0),
      m_max_value(1.0), m_min_value_set(false), m_max_value_set(false),
      m_visible(true), m_z_position(0) {
  m_updates_enabled = false;
  mp_pixel_data = 0;
...
Object::Object(size_t w, size_t h, const db::Matrix3d &trans,
               const std::vector<double> &d)
    : m_trans(trans), m_id(make_id()), m_min_value(0.0), m_max_value(1.0),
      m_min_value_set(false), m_max_value_set(false), m_visible(true),
      m_z_position(0) {
...
Object::Object(size_t w, size_t h, const db::Matrix3d &trans, float *red,
               float *green, float *blue)
    : m_trans(trans), m_id(make_id()), m_min_value(0.0), m_max_value(1.0),
      m_min_value_set(false), m_max_value_set(false), m_visible(true),
      m_z_position(0) {
...
Object &Object::operator=(const img::Object &d) {
  if (this != &d) {
...
unsigned int Object::class_id() const {
  static unsigned int cid = db::get_unique_user_object_class_id();
  return cid;
}

db::DUserObjectBase *Object::clone() const { return new img::Object(*this); }
...
db::DPolygon Object::image_box_poly(const db::DBox vp,
  ...
  db::DPolygon image_box_poly(
      db::DBox(-0.5 * width(), -0.5 * height(), 0.5 * width(), 0.5 * height()));
...
db::DBox Object::box() const {
  ...
  // include landmarks
  for (std::vector<db::DPoint>::const_iterator l = m_landmarks.begin();
       l != m_landmarks.end(); ++l) {
  ...
  try {
    ...
    *this = img::Object();
    ...
    while (!ex.at_end()) {
      ...
      if (ex.test("trans=")) {
        ...
        while (!ex.at_end() && !ex.test("]")) {
          ...
          m_data_mapping.false_color_nodes.push_back(
              std::make_pair(x, std::make_pair(cl, cr)));
    ...
    if (compatibility_mode) {
      m_trans = db::Matrix3d(tr) * db::Matrix3d::mag(pw, ph) *
                db::Matrix3d::disp(db::DVector(0.5 * width(), 0.5 * height()));
    }
...
std::string Object::to_string() const {
  ...
  if (is_empty()) {
    ...
    for (unsigned int i = 0; i < data_mapping().false_color_nodes.size(); ++i) {
      ...
      os << ",";
      const std::pair<tl::Color, tl::Color> &clr =
          data_mapping().false_color_nodes[i].second;
...
size_t Object::width() const { return mp_data ? mp_data->width() : 0; }

size_t Object::height() const { return mp_data ? mp_data->height() : 0; }

size_t Object::data_length() const {
  return mp_data ? mp_data->data_length() : 0;
}

bool Object::is_empty() const { return mp_data == 0; }

bool Object::is_byte_data() const {
  return mp_data ? mp_data->is_byte_data() : false;
}

bool Object::is_color() const { return mp_data ? mp_data->is_color() : false; }

const unsigned char *Object::mask() const {
  return mp_data ? mp_data->mask() : 0;
}

const unsigned char *Object::byte_data() const {
  return mp_data ? mp_data->byte_data() : 0;
}

const unsigned char *Object::byte_data(unsigned int component) const {
  tl_assert(component < 3);
  return mp_data ? mp_data->byte_data(component) : 0;
}

const float *Object::float_data() const {
  return mp_data ? mp_data->float_data() : 0;
}

const float *Object::float_data(unsigned int component) const {
  tl_assert(component < 3);
...
double Object::pixel(size_t x, size_t y) const {
...
double Object::pixel(size_t x, size_t y, unsigned int component) const {
  if (mp_data && x < width() && y < height()) {
    if (!is_color()) {
      if (is_byte_data()) {
        return mp_data->byte_data()[x + y * width()];
...
void Object::set_pixel(size_t x, size_t y, double red, double green,
                       double blue) {
  if (mp_data && x < width() && y < height() && is_color()) {
    invalidate_pixel_data();
    size_t i = x + y * width();
...
void Object::set_data(size_t w, size_t h, unsigned char *red,
...
void Object::set_data(size_t w, size_t h, float *red, float *green,
                      float *blue) {
  release();
...
void Object::set_data(size_t w, size_t h, const std::vector<double> &red,
...
void Object::validate_pixel_data() const {
  if (mp_data != 0 && mp_pixel_data == 0 && !is_empty()) {
    ...
    for (unsigned int i = 0; i < 3; ++i) {
      ...
      lut[i].set_data_mapping(m_data_mapping.create_data_mapping(
          !mp_data->is_color(), m_min_value, m_max_value, i));
...
const std::vector<db::DPoint> &Object::landmarks() const { return m_landmarks; }
...
bool Object::is_valid_matrix(const db::Matrix3d &matrix) {
  db::DPoint p[] = {db::DPoint(-0.5 * width(), -0.5 * height()),
                    db::DPoint(-0.5 * width(), 0.5 * height()),
                    db::DPoint(0.5 * width(), -0.5 * height()),
                    db::DPoint(0.5 * width(), 0.5 * height())};

  for (unsigned int i = 0; i < sizeof(p) / sizeof(p[0]); ++i) {
    double z = matrix.m()[2][0] * p[i].x() + matrix.m()[2][1] * p[i].y() +
               matrix.m()[2][2];
    if (z < 1e-10) {
      return false;
    }
...
void Object::mem_stat(db::MemStatistics *stat,
...
const char *Object::class_name() const { return "img::Object"; }

/**
 *  @brief Registration of the img::Object class in the DUserObject space
 */
static db::DUserObjectDeclaration class_registrar(
    new db::user_object_factory_impl<img::Object, db::DCoord>("img::Object"));
