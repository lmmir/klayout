...
class IMG_PUBLIC View : public lay::ViewObject {
public:
...
public:
  ...
  ImageIterator(lay::AnnotationShapes::iterator begin,
                lay::AnnotationShapes::iterator end)
      : m_current(begin), m_end(end) {
    next_valid();
  }

  ImageIterator() : m_current(), m_end() {
    //  .. nothing yet ..
  }
  ...
  lay::AnnotationShapes::iterator basic_iterator() const { return m_current; }

private:
  void next_valid() {
    while (m_current != m_end &&
           dynamic_cast<const img::Object *>(m_current->ptr()) == 0) {
      ++m_current;
    }
  }
...
class IMG_PUBLIC Service : public lay::BackgroundViewObject,
                           public lay::Editable,
                           public lay::Plugin,
                           public db::Object {
public:
   ...
   */
  bool images_visible() const { return m_images_visible; }
  ...
  /**
   *  @brief Return the iterator that delivers the image annotations (and only
   * these)
   */
   ...
   *  This event is triggered if the properties of one image have changed. The
   * event argument is the ID of the image that has changed.
   */
...
private:
  ...
  //  The view objects representing the selection and the moved images in move
  //  mode
  ...
  db::DPoint m_p1;
  //  The image object representing the image being moved as it was before it
  //  was moved
  ...
  //  Flag indicating that we want to keep the selection after the landmark was
  //  moved
   ...
   */
  const db::DUserObject *find_image(
      const db::DPoint &p, const db::DBox &search_box, double l, double &dmin,
      const std::map<img::Service::obj_iterator, unsigned int> *exclude = 0);
