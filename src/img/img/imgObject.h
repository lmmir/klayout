...
public:
  typedef std::vector<std::pair<double, std::pair<tl::Color, tl::Color>>>
      false_color_nodes_type;
   ...
   *
   *  Each node is a pair or x-value (normalized to a range of 0..1) and a
   * corresponding color. The list should have an element with x value of 0.0
   * and one with an x value of 1.0.
   */
   ...
   *
   *  This value is the multiplier by which the red channel is scaled after
   * applying false color transformation and contrast/brightness/gamma.
   *
   ...
   *
   *  This value is the multiplier by which the green channel is scaled after
   * applying false color transformation and contrast/brightness/gamma.
   *
   *  1.0 is a neutral value. The gain should be >=0.0.
   */
   ...
   *
   *  This value is the multiplier by which the blue channel is scaled after
   * applying false color transformation and contrast/brightness/gamma.
   *
   *  1.0 is a neutral value. The gain should be >=0.0.
   */
   ...
   */
  tl::DataMappingBase *create_data_mapping(bool monochrome, double xmin,
                                           double xmax,
                                           unsigned int channel) const;
};

/**
 *  @brief A helper function to interpolate a color in the color bar at a given
 * x
 */
tl::Color interpolated_color(const DataMapping::false_color_nodes_type &nodes,
                             double x);
 ...
 */
class IMG_PUBLIC Object : public db::DUserObjectBase {
public:
   ...
   *
   *  This will create an empty image with no defined bounding box and a unit
   * transformation.
   */
  Object();
   ...
   *
   *  This constructor creates an image object from a data set describing one
   * monochrome channel or three color channels. Each channel consists of an
   * array of x*y values where the first "x" values describe the first (lowest!)
   * row and so on. Note, that the rows are oriented in the mathematical sense
   * (first one is the lowest) contrary to the common convention for image data.
   *  The data fields can be accessed with the "data", "set_data", "pixel" or
   * "set_pixel" methods.
   *
   ...
   *
   *  This constructor creates an image from the given pixel values. The values
   * have to be organized line by line. Each line must consist of "w" values
   * where the first value is the leftmost pixel. Note, that the rows are
   * oriented in the mathematical sense (first one is the lowest) contrary to
   *  the common convention for image data.
   *  The pixel values are given as unsigned char values with a data range of 0
   * to 255.
   *
   *  @param w The width of the image
   *  @param h The height of the image
   ...
   */
  Object(size_t w, size_t h, const db::DCplxTrans &trans,
   ...
   *
   *  This constructor creates an image from the given pixel values. The values
   * have to be organized line by line and separated by color channel. Each line
   * must consist of "w" values where the first value is the leftmost pixel.
   *  Note, that the rows are oriented in the mathematical sense (first one is
   * the lowest) contrary to the common convention for image data. The pixel
   * values are given as unsigned char values with a data range of 0 to 255.
   *
   *  @param w The width of the image
   *  @param h The height of the image
   *  @param trans The transformation from pixel space to micron space
   *  @param red The red channel data set which will become owned by the image
   *  @param green The green channel data set which will become owned by the
   * image
   *  @param blue The blue channel data set which will become owned by the image
   */
  Object(size_t w, size_t h, const db::DCplxTrans &trans, unsigned char *red,
   ...
   * must consist of "w" values where the first value is the leftmost pixel.
   *  Note, that the rows are oriented in the mathematical sense (first one is
   * the lowest) contrary to the common convention for image data.
   *
   *  @param w The width of the image
   *  @param h The height of the image
   *  @param trans The transformation from pixel space to micron space
   ...
   *  @param blue The blue channel data set which will become owned by the image
   */
  Object(size_t w, size_t h, const db::DCplxTrans &trans, float *red,
   ...
   */
  Object(size_t w, size_t h, const db::DCplxTrans &trans,
         const std::vector<double> &red, const std::vector<double> &green,
         const std::vector<double> &blue);

  /**
   *  @brief Constructor from a image file
   *
   *  This constructor creates an image object from a file (which can have any
   * format supported by Qt) and a transformation. The image will originally be
   * put to position 0, 0 (lower left corner) and each pixel will have a size
   * of 1. The transformation describes how to transform this image into micron
   * space.
   */
   ...
   *  The image will originally be put to position 0, 0 (lower left corner) and
   * each pixel will have a size of 1. The transformation describes how to
   ...
   *  @param w The width of the image
   *  @param h The height of the image
   *  @param matrix The 3d transformation matrix from pixel space to micron
   * space
   *  @param color True to create a color image.
   ...
   *  @param d The data set
   */
  Object(size_t w, size_t h, const db::Matrix3d &trans,
         const std::vector<double> &d);
   ...
   *  @param blue The blue channel data set which will become owned by the image
   */
  Object(size_t w, size_t h, const db::Matrix3d &matrix, unsigned char *red,
         unsigned char *green, unsigned char *blue);
   ...
   *  @param blue The blue channel data set which will become owned by the image
   */
  Object(size_t w, size_t h, const db::Matrix3d &matrix, float *red,
         float *green, float *blue);
   ...
   *  @param blue The blue channel data set
   */
  Object(size_t w, size_t h, const db::Matrix3d &matrix,
         const std::vector<double> &red, const std::vector<double> &green,
         const std::vector<double> &blue);
  ...
  /**
   *  @brief Get the class Id which internally distinguishes the img::Object's
   ...
   *  This method reimplements the db::DUserObjectBase interface.
   *
   *  The bounding box can be a world bbox since under certain perspective
   * transformations the image may extend into the forbidden z space. For a more
   * exact representation use the image_box_polygon method which delivers the
   * box clipped to a certain viewport.
   */
  virtual db::DBox box() const;

  /**
   *  @brief Get the transformed image box clipped by the given viewport in
   * post-transformation space
   *
   *  Under certain perspective transformations, the image may not be
   * representable by a finite bounding box. This happens if corner points of
   * the image are located in the invalid z space (z <= 0). In this case this
   * method can be used which delivers a polygon which is the transformed image
   * box clipped by the given viewport box which and viewport transformation.
   * More precisely, the viewport box vp is transformed with vpt.inverted() and
   * the resulting polygon is used as the clip polygon to determine the outer
   * contour of the image in viewport space (micron space plus vpt). This method
   * may deliver an empty polygon if the image is outside the viewport area.
   */
  db::DPolygon image_box_poly(const db::DBox vp,
                              const db::DCplxTrans &vpt) const;
   ...
   */
  template <class Trans> img::Object transformed(const Trans &t) const {
    img::Object obj(*this);
    obj.transform(t);
    return obj;
  }
   ...
   *  @param y The y coordinate of the pixel (mathematical order: 0 is the
   * lowest, 0..height()-1)
   ...
   *  @param v The value
   *
   *  If the component index, x or y value exceeds the image bounds of the image
   * is a color image, this method does nothing.
   */
   ...
   *
   *  If the component index, x or y value exceeds the image bounds of the image
   * is not a color image, this method does nothing.
   */
   ...
   * lowest, 0..height()-1)
   *
   *  If the x or y value exceeds the image bounds or the image is not a
   * monochrome image, this method returns 0.0.
   */
   ...
   *
   *  @return The pointer to an unsigned char field with w*h entries or 0 if no
   * mask is set.
   */
   ...
   *
   *  @return The pointer to an unsigned char field with w*h entries or 0 if the
   * image is a color image. This method returns 0 if the data is not stored as
   * bytes (is_byte_data() is false).
   */
   ...
   *
   *  @return The pointer to an unsigned char field with w*h entries or 0 if the
   * image is not a color image. This method returns 0 if the data is not stored
   * as bytes (is_byte_data() is false).
   *
   *  @param component 0 for red, 1 for green and 2 for blue component.
   *  @return The pointer to an float field with w*h entries or 0 if the image
   * is not a color image.
   */
   ...
   *
   *  @return The pointer to an float field with w*h entries or 0 if the image
   * is a color image. This method returns 0 if the data is stored as bytes
   * (is_byte_data() is true).
   */
   ...
   *
   *  @return The pointer to an float field with w*h entries or 0 if the image
   * is not a color image. This method returns 0 if the data is stored as bytes
   * (is_byte_data() is true).
   *
   *  @param component 0 for red, 1 for green and 2 for blue component.
   *  @return The pointer to an float field with w*h entries or 0 if the image
   ...
   *
   *  This method must be passed a new'd pointer to a unsigned char array with a
   * length of width * height. The pointer to the data set becomes owned by the
   * image object.
   */
   ...
   *  @brief Write accessor to the image data field (monochrome)
   *
   *  This method must be passed a new'd pointer to a float array with a length
   * of width * height. The pointer to the data set becomes owned by the image
   * object.
   */
   ...
   *
   *  This method must be passed new'd pointers to unsigned char arrays with a
   * length of "data_length()". The pointers to the data set become owned by the
   * image object.
   */
  void set_data(size_t width, size_t height, unsigned char *red,
                unsigned char *green, unsigned char *blue);

  /**
   *  @brief Write accessor to the image data field (color)
   *
   *  This method must be passed new'd pointers to float arrays with a length of
   * "data_length()". The pointers to the data set become owned by the image
   * object.
   */
  void set_data(size_t width, size_t height, float *red, float *green,
                float *blue);
   ...
   */
  void set_data(size_t width, size_t height, const std::vector<double> &red,
   ...
   *
   *  This transformation matrix converts pixel coordinates (0,0 being the lower
   * left corner and each pixel having the dimension of pixel_width and
   * pixel_height) to micron coordinates. The coordinate of the pixel is the
   ...
   *
   *  This transformation converts pixel coordinates (0,0 being the lower left
   * corner and each pixel having the dimension of pixel_width and pixel_height)
   *  to micron coordinates. The coordinate of the pixel is the lower left
   * corner of the pixel.
   */
  const db::Matrix3d &matrix() const { return m_trans; }

  /**
   *  @brief Get the upper limit of the values in the data set
   *
   *  This value determines the upper end of the data mapping (i.e. white value
   * etc.). It does not necessarily correspond to the minimum value of the data
   * set but it must be larger than that.
   */
   ...
   *
   *  See the \min_value method for the description of the minimum value
   * property.
   */
   ...
   *
   *  This value determines the upper end of the data mapping (i.e. white value
   * etc.). It does not necessarily correspond to the maximum value of the data
   * set but it must be larger than that.
   */
   ...
   *
   *  See the \max_value method for the description of the maximum value
   * property.
   */
   ...
   *  @brief Get the Id
   *
   *  Upon initialization, an unique Id is given to the object. The Id is used
   * to identify the object in the context of the service.
   */
   ...
   *  @brief Sets the ID
   *
   *  NOTE: this method is only to be used for internal purposes. Don't change
   * the ID of an object intentionally.
   */
   ...
   */
  const DataMapping &data_mapping() const { return m_data_mapping; }
   ...
   */
  int z_position() const { return m_z_position; }
   ...
   *  @brief Inequality
   */
  bool operator!=(const img::Object &d) const { return !operator==(d); }
   ...
   */
  virtual void mem_stat(db::MemStatistics *stat,
                        db::MemStatistics::purpose_t purpose, int cat,
