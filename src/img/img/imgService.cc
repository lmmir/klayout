...
#if defined(HAVE_QT)
class AddNewImageDialog : public QDialog, public Ui::AddNewImageDialog {
public:
...
static void draw_scanline(unsigned int level, const img::Object &image_object,
                          tl::PixelBuffer &pxbuffer, int y,
                          const db::Matrix3d &t, const db::Matrix3d &it,
                          const db::DPoint &q1, const db::DPoint &q2) {
  ...
  int xstart =
      int(std::max(0.0, std::min(floor(x1), double(pxbuffer.width()))));
  int xstop =
      int(std::max(0.0, std::min(ceil(x2) + 1.0, double(pxbuffer.width()))));
  ...
  if (level < 7 && xstop > xstart + 1 &&
      fabs(xm - (xstart + xstop) / 2) > 1.0 && xm > xstart + 1 &&
      xm < xstop - 1) {
    ...
    tl::color_t *scanline_data =
        pxbuffer.scan_line(pxbuffer.height() - y - 1) + xstart;
...
static void draw_image(const img::Object &image_object, const lay::Viewport &vp,
  ...
  lay::BitmapViewObjectCanvas *bmp_canvas =
      dynamic_cast<lay::BitmapViewObjectCanvas *>(&canvas);
  if (!bmp_canvas || !bmp_canvas->bg_image()) {
    return;
  }

  tl::PixelBuffer &image = *bmp_canvas->bg_image();
  db::DBox source_image_box(0.0, 0.0, image_object.width(),
                            image_object.height());
  ...
  //  it the transformation from QImage pixel coordinates (in the "bottom first"
  //  orientation) into the image object's coordinate space.
  db::DVector dp(0.5 * image_object.width(), 0.5 * image_object.height());
  db::Matrix3d t = db::Matrix3d(vp.trans()) * image_object.matrix() *
                   db::Matrix3d::disp(-dp);
  ...
  for (int y = y1; y <= y2; ++y) {
    ...
    db::DEdge scanline(db::DPoint(image_box.left(), y),
                       db::DPoint(image_box.right(), y));
    scanline.transform(it);

    //  clip the transformed scanline to the original image
    std::pair<bool, db::DEdge> clipped =
        scanline.clipped_line(source_image_box);
    if (clipped.first) {
      draw_scanline(0, image_object, image, y, t, it, clipped.second.p1(),
                    clipped.second.p2());
    }
  }
}

static bool is_selected(const img::Object &image, const db::DPoint &pos,
                        const db::DBox &vpbox, double enl, double &distance) {
  ...
  for (std::vector<db::DPoint>::const_iterator l = image.landmarks().begin();
       l != image.landmarks().end(); ++l) {
  ...
  if (db::inside_poly(b.begin_edge(), pos) < 0) {
    ...
    bool first = true;
    for (db::DPolygon::polygon_edge_iterator e = b.begin_edge(); !e.at_end();
         ++e) {
...
static bool is_selected(const img::Object &image, const db::DBox &box) {
  db::DBox b(image.box());
  return (box.contains(b.p1()) && box.contains(b.p2()));
}

static int obj2id(const db::DUserObject &obj) {
  if (!obj.ptr()) {
    return 0;
  } else {
...
struct SortImagePtrByZOrder {
  ...
  bool operator()(const db::DUserObject *a, const db::DUserObject *b) const {
    return dynamic_cast<const img::Object &>(*a->ptr()).z_position() <
           dynamic_cast<const img::Object &>(*b->ptr()).z_position();
  }
};
...
View::View(img::Service *service, obj_iterator image_ref, img::View::Mode mode)
    : lay::ViewObject(service->widget()), mp_service(service), m_mode(mode),
      mp_image_object(0), m_image_ref(image_ref) {
  //  .. nothing else ..
}

View::View(img::Service *service, const img::Object *object,
           img::View::Mode mode)
    : lay::ViewObject(service->widget()), mp_service(service), m_mode(mode),
      mp_image_object(object) {
  //  .. nothing else ..
}
...
void View::render(const lay::Viewport &vp, lay::ViewObjectCanvas &canvas) {
  ...
  db::DPolygon image_box_poly = image->image_box_poly(
      db::DBox(-10, -10, vp.width() + 10, vp.height() + 10),
      vp.trans() * m_trans);
  ...
  if (m_mode == mode_normal) {
    ...
    vops.reserve(2);
    vops.push_back(lay::ViewOp(canvas.background_color().rgb(),
                               lay::ViewOp::Copy, 0, 0, 0, lay::ViewOp::Rect, 1,
                               1));
    vops.push_back(lay::ViewOp(canvas.foreground_color().rgb(),
                               lay::ViewOp::Copy, 0, 6, 0, lay::ViewOp::Rect, 1,
                               2));
    plane = canvas.plane(vops);

    for (std::vector<db::Polygon>::const_iterator r = result.begin();
         r != result.end(); ++r) {
    ...
    if (!image->landmarks().empty()) {
      ...
      //  obtain bitmap to render on (handles are located over the usual
      //  content)
      lay::CanvasPlane *plane_landmarks, *plane_frame;
      plane_frame = canvas.plane(lay::ViewOp(canvas.foreground_color().rgb(),
                                             lay::ViewOp::Copy, 0, 0, 0,
                                             lay::ViewOp::Rect, 1, -1));
      //  plane_fill and plane are prio 3 and 4 to be above the normal selection
      //  which is 1 and 2
      std::vector<lay::ViewOp> ops;
      ops.push_back(lay::ViewOp(canvas.background_color().rgb(),
                                lay::ViewOp::Copy, 0, 0, 0, lay::ViewOp::Rect,
                                3, 3));
      ops.push_back(lay::ViewOp(canvas.foreground_color().rgb(),
                                lay::ViewOp::Copy, 0, 0, 0, lay::ViewOp::Rect,
                                1, 4));
      plane_landmarks = canvas.plane(ops);

      canvas.renderer().draw(image_box_poly, db::DCplxTrans(), 0, plane_frame,
                             0, 0);

      const std::vector<db::DPoint> &handles = image->landmarks();
      for (std::vector<db::DPoint>::const_iterator hb = handles.begin();
           hb != handles.end(); ++hb) {
        db::DPoint p(hb->transformed(t));
        db::DBox box(p, p);
        double d = 2 / canvas.resolution();
        canvas.renderer().draw(box.enlarged(db::DVector(d, d)),
                               db::DCplxTrans(), 0, plane_landmarks, 0, 0);
        canvas.renderer().draw(
            db::DEdge(p + db::DVector(3.0 * d, 0), p - db::DVector(3.0 * d, 0)),
            db::DCplxTrans(), 0, plane_landmarks, 0, 0);
        canvas.renderer().draw(
            db::DEdge(p + db::DVector(0, 3.0 * d), p - db::DVector(0, 3.0 * d)),
            db::DCplxTrans(), 0, plane_landmarks, 0, 0);
      }
      ...
      plane = canvas.plane(lay::ViewOp(canvas.foreground_color().rgb(),
                                       lay::ViewOp::Copy, 0, 0, 0));
      //  plane_fill is prio 3 to be above the normal selection which is 1 and 2
      plane_fill = canvas.plane(lay::ViewOp(canvas.foreground_color().rgb(),
                                            lay::ViewOp::Copy, 0, 0, 0,
                                            lay::ViewOp::Rect, 1, 3));
      ...
      for (std::vector<db::DPoint>::const_iterator hb = handles.begin();
        ...
        db::DPolygon handle_box_poly(box.enlarged(
            db::DVector(3 / canvas.resolution(), 3 / canvas.resolution())));
        canvas.renderer().draw(handle_box_poly, db::DCplxTrans(), plane_fill,
                               plane, 0, 0);
      }
    }
...
Service::Service(db::Manager *manager, lay::LayoutViewBase *view)
    : lay::BackgroundViewObject(view->canvas()), lay::Editable(view),
      lay::Plugin(view), db::Object(manager), mp_view(view),
      mp_transient_view(0), m_move_mode(Service::move_none),
      m_moved_landmark(0), m_keep_selection_for_move(false),
...
Service::~Service() {
  for (std::vector<img::View *>::iterator v = m_selected_image_views.begin();
       v != m_selected_image_views.end(); ++v) {
    delete *v;
  }
...
img::Object *Service::insert_image(const img::Object &image) {
  //  create the image and insert
  img::Object *new_image = new img::Object(image);
  const db::DUserObject &s =
      mp_view->annotation_shapes().insert(db::DUserObject(new_image));

  //  NOTE: the const_cast will allow us to modify the object begin the
  //  DUserObject - that is not really clean
 ...
 * certain search box and img::Object
 */
static bool dragging_what(const img::Object *iobj, const db::DBox &search_dbox,
                          img::Service::MoveMode &mode, size_t &landmark,
                          db::DPoint &p1) {
  //  are we dragging a landmark?

  for (std::vector<db::DPoint>::const_iterator p = iobj->landmarks().begin();
       p != iobj->landmarks().end(); ++p) {
  ...
  db::DBox ref_box(search_dbox.transformed(
      (iobj->matrix() * db::Matrix3d::disp(-dp)).inverted()));

  bool lo = ref_box.overlaps(db::DBox(0, 0, 0, iobj->height()));
  bool ro = ref_box.overlaps(
      db::DBox(iobj->width(), 0, iobj->width(), iobj->height()));
  bool bo = ref_box.overlaps(db::DBox(0, 0, iobj->width(), 0));
  bool to = ref_box.overlaps(
      db::DBox(0, iobj->height(), iobj->width(), iobj->height()));
...
bool Service::mouse_move_event(const db::DPoint & /*p*/,
...
bool Service::begin_move(lay::Editable::MoveMode mode, const db::DPoint &p,
  ...
  widget()->drag_cancel(); // KLUDGE: every service does this to the same
                           // service manager
  ...
  if (mode == lay::Editable::Selected) {
    ...
    selection_to_view();
    for (std::vector<img::View *>::iterator r = m_selected_image_views.begin();
         r != m_selected_image_views.end(); ++r) {
      (*r)->thaw();
    }
    ...
    for (std::map<obj_iterator, unsigned int>::const_iterator s =
      ...
      if (iobj && dragging_what(iobj, search_dbox, mm, ml, m_p1) &&
          mm != move_all) {
        ...
        //  found a handle of one of the selected object: make the moved image
        //  the selection
        clear_selection();
        m_selected.insert(std::make_pair(si, 0));
        m_current = *iobj;
        m_initial = m_current;
        m_selected_image_views.push_back(
            new img::View(this, &m_current, img::View::mode_transient_move));
      ...
      if (iobj) {
        ...
        if (dragging_what(iobj, search_dbox, mm, ml, m_p1)) {
          ...
          m_selected.insert(std::make_pair(
              mp_view->annotation_shapes().iterator_from_pointer(robj), 0));
          m_current = *iobj;
          m_initial = m_current;
          m_selected_image_views.push_back(
              new img::View(this, &m_current, img::View::mode_transient_move));
...
void Service::move(const db::DPoint &p, lay::angle_constraint_type ac) {
  ...
  if (m_move_mode == move_selected) {
    ...
    if (m_move_mode == move_all) {
      ...
      db::Matrix3d it =
          (m_current.matrix() * db::Matrix3d::disp(-dx)).inverted();
      ...
      vv = db::DVector(std::max(-w * (1.0 - min_scale), vv.x()),
                       std::max(-h * (1.0 - min_scale), vv.y()));

      if (m_move_mode == move_ll || m_move_mode == move_lr ||
          m_move_mode == move_tl || m_move_mode == move_tr) {
      ...
      db::Matrix3d m = m_current.matrix() * db::Matrix3d::disp(v + vv * 0.5) *
                       db::Matrix3d::mag(pw, ph);
...
void Service::show_message() {
  ...
  std::string pos = std::string ("lx: ") + tl::micron_to_string (m_current.p2
  ().x () - m_current.p1 ().x ())
                      + "  ly: " + tl::micron_to_string (m_current.p2 ().y () -
  m_current.p1 ().y ())
                      + "  l: " + tl::micron_to_string (m_current.p2 ().distance
  (m_current.p1 ())); view ()->message (pos);
  */
}
  ...
  if (!m_selected_image_views.empty() && !m_selected.empty()) {
    ...
    if (m_move_mode == move_selected) {
      ...
      for (std::map<obj_iterator, unsigned int>::const_iterator s =
        ...
        const img::Object *iobj =
            dynamic_cast<const img::Object *>(s->first->ptr());
        ...
        int id = obj2id(mp_view->annotation_shapes().replace(
            s->first, db::DUserObject(inew)));
      ...
      int id = obj2id(mp_view->annotation_shapes().replace(
          m_selected.begin()->first, db::DUserObject(inew)));
      image_changed_event(id);
...
const db::DUserObject *Service::find_image(
    const db::DPoint &p, const db::DBox &search_box, double l, double &dmin,
    const std::map<img::Service::obj_iterator, unsigned int> *exclude) {
  if (!m_images_visible) {
    return 0;
  }
  ...
  lay::AnnotationShapes::touching_iterator r =
      mp_view->annotation_shapes().begin_touching(search_box);
  while (!r.at_end()) {
  ...
  for (std::vector<const db::DUserObject *>::const_iterator robj =
           images.begin();
       robj != images.end(); ++robj) {
    double d = std::numeric_limits<double>::max();
    if (is_selected(*dynamic_cast<const img::Object *>((*robj)->ptr()), p,
                    mp_view->box(), l, d)) {
      found = *robj;
      dmin = d;
    }
...
db::DBox Service::selection_bbox() {
  ...
  for (std::map<obj_iterator, unsigned int>::iterator r = m_selected.begin();
       r != m_selected.end(); ++r) {
    const img::Object *iobj =
        dynamic_cast<const img::Object *>(r->first->ptr());
    if (iobj) {
      box += iobj->box();
    }
  }
...
void Service::paste() {
  if (db::Clipboard::instance().begin() != db::Clipboard::instance().end()) {
    ...
    for (db::Clipboard::iterator c = db::Clipboard::instance().begin();
      ...
      const db::ClipboardValue<img::Object> *value =
          dynamic_cast<const db::ClipboardValue<img::Object> *>(*c);
      if (value) {
...
bool Service::has_transient_selection() { return mp_transient_view != 0; }
...
void Service::clear_selection() {
  ...
  //  clear the transient selection as well so there is no reference to any
  //  image left
  clear_transient_selection();
}
...
bool Service::transient_select(const db::DPoint &pos) {
  ...
  double dmin = std::numeric_limits<double>::max();
  const db::DUserObject *robj =
      find_image(pos, search_dbox, l, dmin, &m_previous_selection);

  //  create the transient marker for the object found
  if (robj) {

    obj_iterator imin =
        mp_view->annotation_shapes().iterator_from_pointer(robj);

    //  if in move mode (which also receives transient_select requests) the move
    //  will take the selection, hence only highlight the transient selection if
    //  it's part of the current selection.
    if (view()->has_selection() && view()->is_move_mode() &&
        m_selected.find(imin) == m_selected.end()) {
      return false;
    }
    ...
    if (mp_view->is_move_mode()) {
      mp_transient_view =
          new img::View(this, imin, img::View::mode_transient_move);
    } else {
...
bool Service::select(const db::DBox &box, lay::Editable::SelectionMode mode) {
  ...
  if (box.empty()) {
    ...
    if (!box.is_point()) {
      ...
      double dmin = std::numeric_limits<double>::max();
      const db::DUserObject *robj =
          find_image(box.p1(), search_dbox, l, dmin, exclude);
      ...
      if (robj) {
        ...
        m_previous_selection.insert(std::make_pair(
            mp_view->annotation_shapes().iterator_from_pointer(robj), mode));
        needs_update = true;
      }
    }
  }
  ...
  if (needs_update) {
    selection_to_view(box.is_point() && view()->is_move_mode()
                          ? img::View::mode_transient_move
                          : img::View::mode_normal);
  }
...
void Service::display_status(bool transient) {
  View *selected_view = transient ? mp_transient_view
                                  : (m_selected_image_views.size() == 1
                                         ? m_selected_image_views[0]
                                         : 0);
  if (!selected_view) {
    ...
    msg += tl::sprintf(tl::to_string(tr("image(%dx%d)")), image->width(),
                       image->height());
    view()->message(msg);
  }
}
...
void Service::change_image(obj_iterator pos, const img::Object &to) {
  ...
  int id =
      obj2id(mp_view->annotation_shapes().replace(pos, db::DUserObject(inew)));
  image_changed_event(id);
...
void Service::render_bg(const lay::Viewport &vp,
                        lay::ViewObjectCanvas &canvas) {
  if (!m_images_visible) {
    return;
  }
  ...
  lay::AnnotationShapes::touching_iterator user_object =
      mp_view->annotation_shapes().begin_touching(vp.box());
  while (!user_object.at_end()) {
    const img::Object *image =
        dynamic_cast<const img::Object *>((*user_object).ptr());
    if (image && image->is_visible()) {
  ...
  for (std::vector<const img::Object *>::const_iterator i = images.begin();
       i != images.end(); ++i) {
    draw_image(**i, vp, canvas);
  }
}

ImageIterator Service::begin_images() const {
  return ImageIterator(mp_view->annotation_shapes().begin(),
  ...
  if (symbol == "img::clear_all_images") {
    ...
    if (!images_visible()) {
      lay::TipDialog td(
          ...
          tl::to_string(
              tr("Images are not visible. If you add an image you will not see "
                 "it.\n\n"
                 "Choose 'View/Show Images' to make images visible.")),
          "add-image-while-not-visible", lay::TipDialog::okcancel_buttons);
...
void Service::bring_to_back() {
  ...
  for (obj_iterator user_object = mp_view->annotation_shapes().begin();
       user_object != mp_view->annotation_shapes().end(); ++user_object) {
    const img::Object *i =
        dynamic_cast<const img::Object *>((*user_object).ptr());
    if (i) {
...
const Object *Service::object_by_id(size_t id) const {
...
Service::obj_iterator Service::object_iter_by_id(size_t id) const {
  //  TODO: this is a O(1) lookup, thus potentially slow.
  //  However, in non-editable mode, maintaining a table is not that
  //  straightforward ...
  for (obj_iterator user_object = mp_view->annotation_shapes().begin();
       user_object != mp_view->annotation_shapes().end(); ++user_object) {
    const img::Object *i =
