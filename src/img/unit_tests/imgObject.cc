...
TEST(1) {
  ...
  image.set_matrix(
      db::Matrix3d(db::DCplxTrans(2.5, 90, false, db::DVector(1, -2))));
  ...
  image.set_matrix(
      db::Matrix3d(db::DCplxTrans(2.5, 90, false, db::DVector(1, -2))) *
      db::Matrix3d::mag(2.5, 1.0));
  EXPECT_EQ(image.matrix().mag_x(), 2.5 * 2.5);
  image.set_matrix(
      db::Matrix3d(db::DCplxTrans(2.5, 90, false, db::DVector(1, -2))) *
      db::Matrix3d::mag(2.5, 1.25));
  ...
  dm.false_color_nodes.insert(dm.false_color_nodes.begin() + 1,
                              std::make_pair(0.5, std::make_pair(c, c)));
  ...
  dm.false_color_nodes.insert(dm.false_color_nodes.begin() + 1,
                              std::make_pair(0.75, std::make_pair(c, c2)));
  image.set_data_mapping(dm);
  EXPECT_EQ(copy1.equals(&image), false);
  EXPECT_EQ(from_s(image.to_string()).equals(&image), true);
  copy1 = image;
...
TEST(3) {
  unsigned char d[] = {
      11, 12, 13, 14, 15, 16, 17, 18, 19, 11, 11, 11, 21, 22, 23, 24,
      25, 26, 27, 28, 29, 21, 21, 21, 31, 32, 33, 34, 35, 36, 37, 38,
      39, 31, 31, 31, 41, 42, 43, 44, 45, 46, 47, 48, 49, 41, 41, 41,
      51, 52, 53, 54, 55, 56, 57, 58, 59, 51, 51, 51, 61, 62, 63, 64,
      65, 66, 67, 68, 69, 61, 61, 61, 71, 72, 73, 74, 75, 76, 77, 78,
      79, 71, 71, 71, 81, 82, 83, 84, 85, 86, 87, 88, 89, 81, 81, 81,
  };
