...
VALUE rb_get_top_self() { return ruby_top_self; }
...
void rba_get_backtrace_from_array(VALUE backtrace,
  ...
  if (TYPE(backtrace) == T_ARRAY) {
    ...
    while (len-- > 0) {
      if (skip > 0) {
        ...
        bt.push_back(
            rba_split_bt_information(RSTRING_PTR(str), RSTRING_LEN(str)));
      }
    }
...
bool exceptions_blocked() {
  return RubyInterpreter::instance()
             ? RubyInterpreter::instance()->exceptions_blocked()
             : false;
}
...
void rba_check_error() {
  ...
  std::vector<tl::BacktraceElement> bt;
  rba_get_backtrace_from_array(rb_funcall(lasterr, rb_intern("backtrace"), 0),
                               bt, 0);

  if (RubyInterpreter::instance()) {
    const std::string &ds = RubyInterpreter::instance()->debugger_scope();
    bt.erase(bt.begin(),
             bt.begin() + RubyStackTraceProvider::scope_index(bt, ds));
  }
  ...
  if (info.line > 0) {
    throw RubyError(lasterr, emsg.c_str(), info.file.c_str(), info.line,
                    eclass.c_str(), bt);
  } else {
 ...
 */
VALUE
rba_string_value_f(VALUE obj) { return StringValue(obj); }
 ...
 */
VALUE
rba_safe_inspect(VALUE obj) { return rba_safe_func(rb_inspect, obj); }

/**
 *  @brief needed because NUM2INT may be a macro:
 */
int rba_num2int_f(VALUE obj) { return NUM2INT(obj); }

/**
 *  @brief A safe NUM2INT implementation
 */
int rba_safe_num2int(VALUE obj) { return rba_safe_func(rba_num2int_f, obj); }

/**
 *  @brief needed because NUM2UINT may be a macro:
 */
unsigned int rba_num2uint_f(VALUE obj) { return NUM2UINT(obj); }
 ...
 */
long rba_num2long_f(VALUE obj) { return NUM2LONG(obj); }

/**
 *  @brief A safe NUM2LONG implementation
 */
long rba_safe_num2long(VALUE obj) { return rba_safe_func(rba_num2long_f, obj); }

/**
 *  @brief needed because NUM2ULONG may be a macro:
 */
unsigned long rba_num2ulong_f(VALUE obj) { return NUM2ULONG(obj); }
 ...
 */
long long rba_num2ll_f(VALUE obj) { return NUM2LL(obj); }
 ...
 */
unsigned long long rba_num2ull_f(VALUE obj) { return NUM2ULL(obj); }
 ...
 */
double rba_num2dbl_f(VALUE obj) { return NUM2DBL(obj); }

/**
 *  @brief A safe NUM2DBL implementation
 */
double rba_safe_num2dbl(VALUE obj) { return rba_safe_func(rba_num2dbl_f, obj); }
...
static VALUE rb_class_new_instance_wrap(VALUE args) {
  //  HINT: this ugly cast is required since there is only one form of
  //  rb_protect
  const rb_class_new_instance_param *p =
      (const rb_class_new_instance_param *)args;
...
rba_class_new_instance_checked(int argc, VALUE *argv, VALUE klass) {
  ...
  int error = 0;
  //  HINT: the ugly (VALUE) cast is required since there is only one form of
  //  rb_protect
...
static VALUE rb_funcall2_wrap(VALUE args) {
  //  HINT: this ugly cast is required since there is only one form of
  //  rb_protect
...
#if HAVE_RUBY_VERSION_CODE >= 10900
  //  Hint: calling of methods on terminated objects cannot really be avoided in
  //  all cases - for example when the destructor triggers some callback as is
  //  the case in Qt events (i.e. childEvent is triggered when a child is
  //  removed and may happen on a parent which is terminated but not destroyed
  //  yet). To avoid problems in that case we simply ignore the call.
...
static VALUE rb_f_eval_wrap(VALUE args) {
  //  HINT: this ugly cast is required since there is only one form of
  //  rb_protect
...
#if HAVE_RUBY_VERSION_CODE >= 20200
static VALUE
debug_inspector_get_binding(const rb_debug_inspector_t *dbg_context,
                            void *data) {
...
VALUE rba_eval_string_in_context(const char *expr, const char *file, int line,
                                 int context) {
  rb_set_errinfo(Qnil);
  ...
  //  use the current binding if there is one. This allows using "eval" in the
  //  context of a current trace callback when eval is called from the trace
  //  handler.
  if (context < 0) {
    args[1] = rb_const_get(rb_cObject,
                           rb_intern("TOPLEVEL_BINDING")); // taken from ruby.c
  }
#if HAVE_RUBY_VERSION_CODE >= 20200
  else if (context > 0) {
    args[1] = rb_debug_inspector_open(debug_inspector_get_binding,
                                      reinterpret_cast<void *>(long(context)));
  }
#endif
  else {
