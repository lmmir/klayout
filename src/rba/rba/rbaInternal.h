 ...
 */
class RubyError : public tl::ScriptError {
public:
  RubyError(VALUE exc, const char *msg, const char *cls,
            const std::vector<tl::BacktraceElement> &backtrace)
      : tl::ScriptError(msg, cls, backtrace), m_exc(exc) {}

  RubyError(VALUE exc, const char *msg, const char *sourcefile, int line,
            const char *cls, const std::vector<tl::BacktraceElement> &backtrace)
      : tl::ScriptError(msg, sourcefile, line, cls, backtrace), m_exc(exc) {}

  RubyError(const RubyError &d) : tl::ScriptError(d), m_exc(d.m_exc) {}

  VALUE exc() const { return m_exc; }
...
public:
  ...
  void reset() { set(0, false, false, false, Qnil); }
...
public:
  ...
  virtual void call(const gsi::MethodBase *meth, gsi::SerialArgs &args,
...
/**
 *  @brief Gets a value indicating whether a Ruby class is registered for a GSI
 * class
 */
...
class GCLocker {
public:
  GCLocker(VALUE value) : m_value(value) { gc_lock_object(m_value); }

  ~GCLocker() { gc_unlock_object(m_value); }
...
/**
 *  @brief Makes the locked object vault required for gc_lock_object and
 * gc_unlock_object
 *
