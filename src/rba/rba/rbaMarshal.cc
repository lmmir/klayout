 ...
 */
class RubyBasedStringAdaptor : public gsi::StringAdaptor {
public:
  ...
  ~RubyBasedStringAdaptor() { gc_unlock_object(m_string); }

  virtual const char *c_str() const { return RSTRING_PTR(m_string); }

  virtual size_t size() const { return RSTRING_LEN(m_string); }
 ...
 */
class RubyBasedByteArrayAdaptor : public gsi::ByteArrayAdaptor {
public:
  ...
  ~RubyBasedByteArrayAdaptor() { gc_unlock_object(m_bytes); }

  virtual const char *c_str() const { return RSTRING_PTR(m_bytes); }

  virtual size_t size() const { return RSTRING_LEN(m_bytes); }
 ...
 */
class RubyBasedVariantAdaptor : public gsi::VariantAdaptor {
public:
 ...
 */
class RubyBasedVectorAdaptorIterator : public gsi::VectorAdaptorIterator {
public:
 ...
 */
class RubyBasedVectorAdaptor : public gsi::VectorAdaptor {
public:
 ...
 */
class RubyBasedMapAdaptorIterator : public gsi::MapAdaptorIterator {
public:
  RubyBasedMapAdaptorIterator(VALUE hash, const gsi::ArgType *ainner,
 ...
 */
class RubyBasedMapAdaptor : public gsi::MapAdaptor {
public:
  RubyBasedMapAdaptor(VALUE hash, const gsi::ArgType *ainner,
                      const gsi::ArgType *ainner_k);
  ~RubyBasedMapAdaptor();
...
template <class R> struct get_boxed_value_func {
      ...
      *value = v;
      ...
      if (!p->cls_decl()->is_derived_from(bt)) {
        throw tl::Exception(tl::sprintf(
            tl::to_string(tr("Passing an object to pointer or reference "
                             "requires a boxed type (RBA::%s)")),
            bt->name()));
      }
...
template <>
struct get_boxed_value_func<gsi::VariantType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::StringType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::ByteArrayType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::ObjectType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::VectorType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::MapType> : get_boxed_value_func_error {};
template <>
struct get_boxed_value_func<gsi::VoidType> : get_boxed_value_func_error {};
...
template <class R> struct writer {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
                  tl::Heap *heap) {
    if (arg == Qnil) {

      if (atype.is_ref() || atype.is_cref()) {
        throw tl::Exception(
            tl::to_string(tr("Arguments or return values of reference type "
                             "cannot be passed nil")));
      ...
      if (atype.is_ref() || atype.is_ptr()) {
        // references or pointers require a boxed object. Pointers also allow
        // nil.
        void *vc = boxed_value_ptr(atype.type(), arg, *heap);
        if (!vc && atype.is_ref()) {
          throw tl::Exception(tl::to_string(
              tr("Arguments or return values of reference or direct type "
                 "cannot be passed nil or an empty boxed value object")));
        }
        aa->write<void *>(vc);
 ...
 */
template <> struct writer<gsi::StringType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
                  tl::Heap *heap) {
    ...
    if (arg == Qnil) {
      ...
      if (!(atype.is_ptr() || atype.is_cptr())) {
        ...
        aa->write<void *>(
            (void *)new gsi::StringAdaptorImpl<std::string>(std::string()));
      } else {
      ...
      if (atype.is_ref() || atype.is_ptr()) {
        ...
        //  NOTE: by convention we pass the ownership to the receiver for
        //  adaptors.
        if (!vc) {
          aa->write<void *>(0);
        } else {
          aa->write<void *>((void *)new gsi::StringAdaptorImpl<std::string>(
              (std::string *)vc));
        }
 ...
 *  @brief Serialization for strings
 */
template <> struct writer<gsi::ByteArrayType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
                  tl::Heap *heap) {
    //  Cannot pass ownership currently
    tl_assert(!atype.pass_obj());
    ...
    if (arg == Qnil) {
      ...
      if (!(atype.is_ptr() || atype.is_cptr())) {
        //  nil is treated as an empty string for references
        aa->write<void *>(
            (void *)new gsi::ByteArrayAdaptorImpl<std::vector<char>>(
                std::vector<char>()));
      } else {
        aa->write<void *>((void *)0);
      }
 ...
 */
template <> struct writer<gsi::VariantType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &,
                  tl::Heap *) {
 ...
 */
template <> struct writer<gsi::VectorType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
                  tl::Heap *) {
    if (arg == Qnil) {
      if (!(atype.is_ptr() || atype.is_cptr())) {
      ...
      if (TYPE(arg) != T_ARRAY) {
        throw tl::Exception(
            tl::sprintf(tl::to_string(tr(
                            "Unexpected object type (expected array, got %s)")),
                        rba_class_name(arg).c_str()));
      }
 ...
 */
template <> struct writer<gsi::MapType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
                  tl::Heap *) {
    if (arg == Qnil) {
      if (!(atype.is_ptr() || atype.is_cptr())) {
      ...
      if (TYPE(arg) != T_HASH) {
        throw tl::Exception(tl::sprintf(
            tl::to_string(tr("Unexpected object type (expected hash, got %s)")),
            rba_class_name(arg).c_str()));
      }
      ...
      aa->write<void *>(
          (void *)new RubyBasedMapAdaptor(arg, atype.inner(), atype.inner_k()));
    }
  }
};
...
template <> struct writer<gsi::ObjectType> {
  void operator()(gsi::SerialArgs *aa, VALUE arg, const gsi::ArgType &atype,
    ...
    if (arg == Qnil) {
      ...
      if (TYPE(arg) != T_DATA) {
        throw tl::Exception(
            tl::sprintf(tl::to_string(tr("Unexpected object type (expected "
                                         "argument of class %s, got %s)")),
                        atype.cls()->name(), rba_class_name(arg).c_str()));
      }
      ...
      if (atype.is_ptr() || atype.is_cptr() || atype.is_ref() ||
        ...
        if (p->cls_decl()->is_derived_from(atype.cls())) {
          ...
          if (p->cls_decl()->adapted_type_info()) {
            ...
            aa->write<void *>(
                (void *)p->cls_decl()->adapted_from_obj(p->obj()));
          } else {
            aa->write<void *>(p->obj());
          }

        } else if ((atype.is_cref() || atype.is_cptr()) &&
                   p->cls_decl()->can_convert_to(atype.cls())) {
        ...
        if (p->cls_decl()->is_derived_from(atype.cls())) {
          ...
          aa->write<void *>(
              atype.cls()->create_obj_from(p->cls_decl(), p->obj()));
...
template <> struct writer<gsi::VoidType> {
  void operator()(gsi::SerialArgs * /*aa*/, VALUE /*arg*/,
...
void push_arg(const gsi::ArgType &atype, gsi::SerialArgs &aserial, VALUE arg,
              tl::Heap &heap) {
...
template <class R> struct reader {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy * /*self*/,
                  const gsi::ArgType &arg, tl::Heap *heap) {
    if (arg.is_ref()) {
...
template <> struct reader<gsi::StringType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy * /*self*/,
                  const gsi::ArgType &, tl::Heap *heap) {
    std::unique_ptr<gsi::StringAdaptor> a(
        (gsi::StringAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *ret = Qnil;
    } else {
 ...
 *  @brief Deserialization wrapper: specialization for strings
 */
template <> struct reader<gsi::ByteArrayType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy * /*self*/,
                  const gsi::ArgType &, tl::Heap *heap) {
    std::unique_ptr<gsi::ByteArrayAdaptor> a(
        (gsi::ByteArrayAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *ret = Qnil;
    } else {
      *ret = rb_str_new(a->c_str(), long(a->size()));
...
static VALUE object_from_variant(tl::Variant &var, Proxy *self,
                                 const gsi::ArgType &atype,
                                 bool transfer = false) {
  if (var.is_user()) {

    bool is_direct = (!atype.is_cptr() && !atype.is_ptr() && !atype.is_cref() &&
                      !atype.is_ref());
    ...
    gsi::Proxy *holder =
        dynamic_cast<gsi::Proxy *>(const_cast<tl::Object *>(var.to_object()));
    ...
    if (pass_obj || transfer) {
      ...
      if (holder) {
        ...
        //  transfer ownership of object: when the transfer mode indicates a
        //  transfer of ownership (pass_obj == true) and the holder is owning
        //  the object, we transfer ownership (case 2). If the variant itself is
        //  a reference, this indicates a variable reference within
        //  gsi::Expressions or another case, where the object (through the
        //  holder and another variant) is actually held otherwise. In that
        //  case, we leave the ownership where it is (case 1, pass by
        //  reference).
        if (var.user_is_ref()) {
        ...
        //  If the object was not owned before, it is not owned after (bears
        //  risk of invalid pointers, but it's probably rarely the case.
        //  Non-managed objects are usually copied between the ownership spaces.
        //  If the variant holds the user object, we can take it from it and
        //  claim ownership.
        if (var.user_is_ref()) {
      ...
      //  This is the case for return values that prefer to be copied (e.g. from
      //  const &)
      prefer_copy = atype.prefer_copy();
    }

    return object_to_ruby(obj, self, cls, pass_obj, is_const, prefer_copy,
                          can_destroy);
...
template <> struct reader<gsi::VariantType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy *self,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    std::unique_ptr<gsi::VariantAdaptor> a(
        (gsi::VariantAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *ret = Qnil;
    } else {
      gsi::VariantAdaptorImpl<tl::Variant> *aa =
          dynamic_cast<gsi::VariantAdaptorImpl<tl::Variant> *>(a.get());
      RubyBasedVariantAdaptor *pa =
          dynamic_cast<RubyBasedVariantAdaptor *>(a.get());
      if (aa) {
 ...
 */
template <> struct reader<gsi::VectorType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy * /*self*/,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    std::unique_ptr<gsi::VectorAdaptor> a(
        (gsi::VectorAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *ret = Qnil;
    } else {
      *ret = rb_ary_new();
 ...
 */
template <> struct reader<gsi::MapType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy * /*self*/,
                  const gsi::ArgType &atype, tl::Heap *heap) {
    std::unique_ptr<gsi::MapAdaptor> a(
        (gsi::MapAdaptor *)rr->read<void *>(*heap));
    if (!a.get()) {
      *ret = Qnil;
    } else {
      *ret = rb_hash_new();
 ...
 */
template <> struct reader<gsi::ObjectType> {
  void operator()(gsi::SerialArgs *rr, VALUE *ret, Proxy *self,
                  const gsi::ArgType &atype, tl::Heap *heap) {
...
template <> struct reader<gsi::VoidType> {
  void operator()(gsi::SerialArgs * /*rr*/, VALUE * /*ret*/, Proxy * /*self*/,
                  const gsi::ArgType & /*atype*/, tl::Heap * /*heap*/) {
    //  .. nothing: void is not serialized
  }
...
RubyBasedVariantAdaptor::RubyBasedVariantAdaptor(VALUE var) : m_var(var) {
  gc_lock_object(m_var);
}

RubyBasedVariantAdaptor::~RubyBasedVariantAdaptor() { gc_unlock_object(m_var); }
...
RubyBasedVectorAdaptorIterator::RubyBasedVectorAdaptorIterator(
    VALUE array, const gsi::ArgType *ainner)
    : m_array(array), m_i(0), m_len(RARRAY_LEN(array)), mp_ainner(ainner) {
  //  .. nothing yet ..
}

void RubyBasedVectorAdaptorIterator::get(gsi::SerialArgs &w,
                                         tl::Heap &heap) const {
  gsi::do_on_type<writer>()(mp_ainner->type(), &w,
                            rb_ary_entry(m_array, long(m_i)), *mp_ainner,
                            &heap);
}

bool RubyBasedVectorAdaptorIterator::at_end() const { return m_i == m_len; }

void RubyBasedVectorAdaptorIterator::inc() { ++m_i; }

// ---------------------------------------------------------------------
//  RubyBasedVectorAdaptor implementation

RubyBasedVectorAdaptor::RubyBasedVectorAdaptor(VALUE array,
                                               const gsi::ArgType *ainner)
    : mp_ainner(ainner), m_array(array) {
  gc_lock_object(m_array);
}

RubyBasedVectorAdaptor::~RubyBasedVectorAdaptor() { gc_unlock_object(m_array); }
...
void RubyBasedVectorAdaptor::push(gsi::SerialArgs &r, tl::Heap &heap) {
  VALUE member;
  gsi::do_on_type<reader>()(mp_ainner->type(), &r, &member, (Proxy *)0,
                            *mp_ainner, &heap);
  rb_ary_push(m_array, member);
}

void RubyBasedVectorAdaptor::clear() { rb_ary_clear(m_array); }

size_t RubyBasedVectorAdaptor::size() const { return RARRAY_LEN(m_array); }

size_t RubyBasedVectorAdaptor::serial_size() const { return mp_ainner->size(); }

// ---------------------------------------------------------------------
//  RubyBasedMapAdaptorIterator implementation

static int push_map_i(VALUE key, VALUE value, VALUE arg) {
  std::vector<std::pair<VALUE, VALUE>> *v =
      (std::vector<std::pair<VALUE, VALUE>> *)arg;
  v->push_back(std::make_pair(key, value));
  return ST_CONTINUE;
}

RubyBasedMapAdaptorIterator::RubyBasedMapAdaptorIterator(
    VALUE hash, const gsi::ArgType *ainner, const gsi::ArgType *ainner_k)
...
void RubyBasedMapAdaptorIterator::get(gsi::SerialArgs &w,
                                      tl::Heap &heap) const {
  gsi::do_on_type<writer>()(mp_ainner_k->type(), &w, m_b->first, *mp_ainner_k,
                            &heap);
  gsi::do_on_type<writer>()(mp_ainner->type(), &w, m_b->second, *mp_ainner,
                            &heap);
}

bool RubyBasedMapAdaptorIterator::at_end() const { return m_b == m_e; }

void RubyBasedMapAdaptorIterator::inc() { ++m_b; }

// ---------------------------------------------------------------------
//  RubyBasedMapAdaptor implementation

RubyBasedMapAdaptor::RubyBasedMapAdaptor(VALUE hash, const gsi::ArgType *ainner,
                                         const gsi::ArgType *ainner_k)
    : mp_ainner(ainner), mp_ainner_k(ainner_k), m_hash(hash) {
  gc_lock_object(m_hash);
}

RubyBasedMapAdaptor::~RubyBasedMapAdaptor() { gc_unlock_object(m_hash); }
...
void RubyBasedMapAdaptor::insert(gsi::SerialArgs &r, tl::Heap &heap) {
  VALUE k, v;
  gsi::do_on_type<reader>()(mp_ainner_k->type(), &r, &k, (Proxy *)0,
                            *mp_ainner_k, &heap);
  gsi::do_on_type<reader>()(mp_ainner->type(), &r, &v, (Proxy *)0, *mp_ainner,
                            &heap);
  rb_hash_aset(m_hash, k, v);
}

void RubyBasedMapAdaptor::clear() { rb_hash_clear(m_hash); }

size_t RubyBasedMapAdaptor::size() const { return RHASH_SIZE(m_hash); }
...
VALUE
pop_arg(const gsi::ArgType &atype, Proxy *self, gsi::SerialArgs &aserial,
        tl::Heap &heap) {
  VALUE ret = Qnil;
...
//  if atype is a ref:
//      argument must be a boxed type of the required type or an object of the
//      requested class
//  if atype is a ptr:
//      argument must be a boxed type of the required type or an object of the
//      requested class or nil
//  if atype is a cptr:
  ...
  void operator()(bool *ret, VALUE arg, const gsi::ArgType &atype, bool loose) {
    if ((atype.is_cptr() || atype.is_ptr()) && arg == Qnil) {
      ...
      *ret = false;
      ...
      if (!*ret) {
        //  otherwise try a normal match and let the serializer sort out the
        //  wrong arguments with a good error message.
 ...
 */
template <class R> struct test_vector {
  void operator()(bool *ret, VALUE arr, const gsi::ArgType &ainner,
                  bool loose) {
    *ret = true;
    ...
    if ((atype.is_cptr() || atype.is_ptr()) && arg == Qnil) {
      ...
      *ret = (TYPE(arg) == T_DATA);
      ...
      if (*ret) {
        ...
        //  in loose mode (second pass) try to match the types via implicit
        //  constructors, in strict mode (first pass) require direct type match
        if (p->cls_decl() == atype.cls() ||
            (loose && (p->cls_decl()->is_derived_from(atype.cls()) ||
                       p->cls_decl()->can_convert_to(atype.cls())))) {
