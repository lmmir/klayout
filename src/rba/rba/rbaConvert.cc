...
template <> tl::Variant ruby2c<tl::Variant>(VALUE rval) {
  if (FIXNUM_P(rval)) {
    ...
    //  Note: we use this scheme rather than directly inserting into the variant
    //  because the rb_hash_foreach is not exception-safe.
    rb_hash_foreach(rval, (int (*)(...)) & push_map_i, (VALUE)&kv);

    tl::Variant r;
    r.set_array();
    for (std::vector<std::pair<VALUE, VALUE>>::const_iterator i = kv.begin();
         i != kv.end(); ++i) {
    ...
    //  employ the tl::Variant binding capabilities of the Expression binding to
    //  derive the variant value.
 ...
 * is required HINT: this is a workaround for the fact that unlike C++, Ruby
...
VALUE
object_to_ruby(void *obj, Proxy *self, const gsi::ClassBase *cls, bool pass_obj,
...
template <> VALUE c2ruby<tl::Variant>(const tl::Variant &c) {
  if (c.is_double()) {
    ...
    for (tl::Variant::const_array_iterator i = c.begin_array();
         i != c.end_array(); ++i) {
      rb_hash_aset(ret, c2ruby<tl::Variant>(i->first),
                   c2ruby<tl::Variant>(i->second));
    }
    return ret;
  } else if (c.is_list()) {
    ...
    if (cls) {
      ...
      return object_to_ruby(obj, 0, c.user_cls()->gsi_cls(), false, false, true,
                            false);
    } else {
