...
class RBAArrayInspector : public gsi::Inspector {
public:
  RBAArrayInspector(VALUE array) : m_array(array) {
  ...
  bool has_keys() const { return false; }
  ...
  virtual std::string type(size_t index) const {
    return ruby2c<std::string>(
        rba_safe_obj_as_string(rb_class_of(rb_value(index))));
  }
...
class RBAHashInspector : public gsi::Inspector {
public:
  RBAHashInspector(VALUE hash) : m_hash(hash) {
  ...
  VALUE rb_key(size_t index) const { return rb_ary_entry(m_keys, long(index)); }
  ...
  virtual size_t count() const { return RARRAY_LEN(m_keys); }
...
class RBAObjectInspector : public gsi::Inspector {
public:
  RBAObjectInspector(VALUE obj) : m_obj(obj), m_members(Qnil) {
  ...
  virtual size_t count() const { return 1 + RARRAY_LEN(m_members); }
  ...
  virtual bool equiv(const gsi::Inspector *other) const {
    const RBAObjectInspector *o =
        dynamic_cast<const RBAObjectInspector *>(other);
...
static void collect_getters(
    const gsi::ClassBase *cls,
    std::vector<std::pair<std::string, const gsi::MethodBase *>> &getters) {
  if (cls->base()) {
    collect_getters(cls->base(), getters);
  }

  //  properties are identified by a setter - the corresponding non-setter
  //  method is the getter then.
  ...
  for (gsi::ClassBase::method_iterator m = cls->begin_methods();
    ...
    if (!(*m)->is_callback()) {
      for (gsi::MethodBase::synonym_iterator s = (*m)->begin_synonyms();
           s != (*m)->end_synonyms(); ++s) {
        if (s->is_getter || (!s->is_setter && setter_names.find(s->name) !=
                                                  setter_names.end())) {
...
class RBADataInspector : public gsi::Inspector {
public:
  RBADataInspector(VALUE obj) : m_obj(obj), mp_cls(0), m_members(Qnil) {
    rb_gc_register_address(&m_obj);
...
class RBAClassInspector : public gsi::Inspector {
public:
  RBAClassInspector(VALUE cls) : m_class(cls), m_members(Qnil) {
  ...
  virtual size_t count() const { return RARRAY_LEN(m_members); }
...
class RBABindingInspector : public gsi::Inspector {
public:
  RBABindingInspector(int context) : m_context(context) {
    m_local_variables =
        rba_eval_string_in_context("local_variables", 0, 0, m_context);
  ...
  virtual std::string key(size_t index) const {
    return ruby2c<std::string>(
        rba_safe_obj_as_string(rb_ary_entry(m_local_variables, long(index))));
  }
  ...
  virtual size_t count() const {
    return TYPE(m_local_variables) == T_ARRAY ? RARRAY_LEN(m_local_variables)
                                              : 0;
  }
  ...
  virtual bool equiv(const gsi::Inspector *other) const {
    const RBABindingInspector *o =
        dynamic_cast<const RBABindingInspector *>(other);
...
static bool has_inspector(VALUE value) {
  return (TYPE(value) == T_ARRAY || TYPE(value) == T_HASH ||
          TYPE(value) == T_DATA || TYPE(value) == T_OBJECT ||
          TYPE(value) == T_CLASS);
}
...
gsi::Inspector *create_inspector(int) { return 0; }
