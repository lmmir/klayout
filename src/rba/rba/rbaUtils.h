...
inline VALUE rb_errinfo() { return rb_gv_get("$!"); }

inline void rb_set_errinfo(VALUE v) { rb_gv_set("$!", v); }
...
inline void rb_hash_clear(VALUE hash) {
  ...
  for (std::vector<VALUE>::const_iterator k = keys.begin(); k != keys.end();
       ++k) {
 ...
 */
inline void rb_set_progname(VALUE pn) { rb_gv_set("PROGRAM_NAME", pn); }

/**
 *  @brief Sets up a block for protected evaluation
 *
 *  It was learned (by bitter experience), that in particular rb_protect cannot
 * simply be called but requires some safety measures, at least in Ruby 1.8.5 as
 * possibly 1.8.x. What happens in rb_protect is that a RNode object is
 * instantiated in some kind of compile steps that builds a temporary executable
 * statement. Deep inside this functionality, the rb_sourcefile () variable is
 * copied into the RNode object, as this variable is usually updated with the
 * current sourcefile name. In effect, the string referenced by this variable
 * participates in the GC mark&sweep steps which leads to unpredictable results,
 * if this variable is not set to a valid string (ruby) buffer or 0.
 *
 *  As a consequence, this function must be called before rb_protect and likely
 * other xx_protect derivatives.
 */
 ...
 */
template <class R, class A> struct rba_func_callparam {
 ...
 */
template <class R, class A> VALUE rba_safe_func_caller(VALUE a) {
 ...
 */
template <class R, class A> R rba_safe_func(R (*f)(A), A arg) {
  ...
  RUBY_BEGIN_EXEC
  //  NOTE: we do not want exceptions to be seen in the debugger here - later
  //  they are rethrown after being annotated. This is when we want to see them.
