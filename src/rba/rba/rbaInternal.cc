...
public:
  ...
  static LockedObjectVault *instance() { return mp_instance; }
...
LockedObjectVault::LockedObjectVault() { mp_instance = this; }
...
void LockedObjectVault::mark(void *p) { ((LockedObjectVault *)p)->mark_this(); }

void LockedObjectVault::free(void *p) { delete ((LockedObjectVault *)p); }
...
LockedObjectVault::alloc(VALUE klass) {
  ...
  return Data_Wrap_Struct(klass, &LockedObjectVault::mark,
                          &LockedObjectVault::free, vault);
}
...
Proxy::Proxy(const gsi::ClassBase *_cls_decl)
    : m_cls_decl(_cls_decl), m_obj(0), m_owned(false), m_const_ref(false),
      m_destroyed(false), m_can_destroy(false), m_self(Qnil) {
  //  .. nothing yet ..
}
...
void Proxy::mark() {
  for (std::map<const gsi::MethodBase *, VALUE>::const_iterator pt =
           m_signal_handlers.begin();
       pt != m_signal_handlers.end(); ++pt) {
...
Proxy::signal_handler(const gsi::MethodBase *meth) {
  std::map<const gsi::MethodBase *, VALUE>::iterator pt =
      m_signal_handlers.find(meth);
...
bool Proxy::can_call() const { return rba::RubyInterpreter::instance() != 0; }
...
void Proxy::call(int id, gsi::SerialArgs &args, gsi::SerialArgs &ret) const {
  ...
  try {
    ...
    VALUE argv = rb_ary_new2(
        long(std::distance(meth->begin_arguments(), meth->end_arguments())));
    RB_GC_GUARD(argv);
    ...
    VALUE rb_ret =
        rba_funcall2_checked(m_self, mid, RARRAY_LEN(argv), RARRAY_PTR(argv));
...
void Proxy::detach() {
  if (!m_destroyed && m_cls_decl && m_cls_decl->is_managed()) {
    ...
    if (gsi_object) {
      gsi_object->status_changed_event().remove(this,
                                                &Proxy::object_status_changed);
    }
...
void Proxy::initialize_callbacks() {
  ...
  while (cls) {
    ...
    for (gsi::ClassBase::method_iterator m = cls->begin_callbacks();
      ...
      //  There is no place in the ruby API to determine whether a method is
      //  defined. Instead we explicitly call "method_defined?" to check,
      //  whether the given method is defined.
      ...
      for (int prot = 0; prot < 2; ++prot) {
        ...
        VALUE rb_ret;
        if (prot) {
          rb_ret = rba_funcall2_checked(rb_class_of(m_self),
                                        rb_intern("protected_method_defined?"),
                                        1, &name);
        } else {
          rb_ret = rba_funcall2_checked(rb_class_of(m_self),
                                        rb_intern("method_defined?"), 1, &name);
        }
        if (RTEST(rb_ret)) {

          //  Only if the class defines that method we can link the virtual
          //  method call to the Ruby method
          int id = add_callback(Proxy::CallbackFunction(rb_intern(nstr), *m));
          (*m)->set_callback(
              m_obj, gsi::Callback(id, this, (*m)->argsize(), (*m)->retsize()));
...
void Proxy::keep_internal() {
  if (m_owned) {
    //  Fallback: the object is not gsi-enabled: we use the ownership flag in
    //  this case to keep it alive. This will not reset the ownership flag for
    //  all clients.
    m_owned = false;
...
void Proxy::set(void *obj, bool owned, bool const_ref, bool can_destroy,
  ...
  if (obj != m_obj) {
    ...
    if (m_obj) {
      ...
      if (cls->is_managed()) {
        ...
        gsi_object->status_changed_event().add(this,
                                               &Proxy::object_status_changed);
  ...
  //  now we have a valid object (or nil) - we can reset "destroyed" state.
  //  Note: this has to be done here because before detach might be called on
  //  *this which resets m_destroyed.
  m_destroyed = false;
}
  ...
  if (!m_obj) {
    if (m_destroyed) {
      ...
      throw tl::Exception(tl::to_string(tr(
          "Object cannot be default-created (missing arguments to 'new'?)")));
    }
  }
...
void Proxy::object_status_changed(gsi::ObjectBase::StatusEventType type) {
  if (type == gsi::ObjectBase::ObjectDestroyed) {
    m_destroyed = true; //  NOTE: must be set before detach and indicates that
                        //  the object was destroyed externally.
    detach();
...
SignalHandler::SignalHandler() : m_obj(Qnil) {}

SignalHandler::~SignalHandler() { clear_procs(); }

void SignalHandler::initialize(VALUE obj) { m_obj = obj; }
...
void SignalHandler::clear() { clear_procs(); }
...
void SignalHandler::remove(VALUE proc) {
  for (std::list<VALUE>::iterator p = m_procs.begin(); p != m_procs.end();
       ++p) {
    if (*p == proc) {
      m_procs.erase(p);
      break;
...
void SignalHandler::clear_procs() { m_procs.clear(); }

void SignalHandler::free(void *p) { delete ((SignalHandler *)p); }

void SignalHandler::mark(void *p) { ((SignalHandler *)p)->mark_this(); }
...
SignalHandler::alloc(VALUE klass) {
  ...
  return Data_Wrap_Struct(klass, &SignalHandler::mark, &SignalHandler::free,
                          signal);
}
...
SignalHandler::static_assign(VALUE self, VALUE proc) {
  ...
  if (proc != self) {
    ...
    if (TYPE(proc) != T_DATA || rb_obj_is_kind_of(proc, rb_cProc) != Qtrue) {
      std::string msg =
          tl::to_string(tr("Single argument to signal must be a Proc object"));
      VALUE args[1];
...
SignalHandler::static_add(VALUE self, VALUE proc) {
  if (TYPE(proc) != T_DATA || rb_obj_is_kind_of(proc, rb_cProc) != Qtrue) {
    std::string msg = tl::to_string(
        tr("Single argument to signal's add method must be a Proc object"));
    VALUE args[1];
    args[0] = rb_str_new2(msg.c_str());
    rb_exc_raise(rb_class_new_instance(1, args, rb_eRuntimeError));
  }
...
void SignalHandler::define_class(VALUE module, const char *name) {
  ...
  rb_define_method(klass, "initialize",
                   (ruby_func)&SignalHandler::static_initialize, 1);
...
void SignalHandler::call(const gsi::MethodBase *meth, gsi::SerialArgs &args,
  ...
  VALUE rb_ret = Qnil;
  for (std::list<VALUE>::const_iterator p = m_procs.begin(); p != m_procs.end();
       ++p) {
    rb_ret = rba_funcall2_checked(*p, rb_intern("call"), RARRAY_LEN(argv),
                                  RARRAY_PTR(argv));
  }
...
void register_class(VALUE ruby_cls, const gsi::ClassBase *gsi_cls,
  ...
  rev_cls_map.insert(
      std::make_pair(std::make_pair(gsi_cls, as_static), ruby_cls));
}
