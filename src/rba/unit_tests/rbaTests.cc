...
// On Windows, ruby.h is not compatible with windows.h which is included by
// utHead - at least not if windows.h is included before ruby.h ...
...
TEST(basic) {
  ...
  try {
    ...
    EXPECT_EQ(std::string(ex.msg(), 0, 22),
              std::string("RuntimeError: an error"));
    err = true;
  }
  ...
  try {
    ...
    EXPECT_EQ(
        std::string(ex.msg(), 0, 41) ==
                std::string("NameError: uninitialized constant Quatsch") ||
            std::string(ex.msg(), 0, 49) ==
                std::string(
                    "NameError: uninitialized constant Object::Quatsch"),
        true);
    err = true;
  }
