...
/**
 *  @brief An object connecting child widget signals with methods from
 * LayoutView
 */
class LayoutViewSignalConnector : public QObject {
  Q_OBJECT
...
public:
   ...
   */
  int mode() const { return LayoutViewBase::mode(); }
   ...
   */
  void selected_cells_paths(int cv_index,
  ...
  /**
   *  @brief Internal method: check, if the layer tree is and an consistent
   * state.
   *
   *  This method is used by the layer tree model to check, if the tree has been
   * brought into a consistent state. HINT: for the layout this is solved more
   * consistently with the "under construction" attribute of the layout. There
   * is no equivalent object for the layer tree currently.
   */
   ...
   */
  virtual bool always_show_source() const { return m_always_show_source; }

  /**
   *  @brief Returns true, if the layer/datatype shall be shown always in the
   * layer properties tree
   */
  virtual bool always_show_ld() const { return m_always_show_ld; }
  ...
  /**
   *  @brief Asks for a bookmark name and bookmark the current view under this
   * name
   */
   ...
   */
  virtual lay::LayerControlPanel *control_panel() { return mp_control_panel; }
   ...
   */
  virtual lay::LayerToolbox *layer_toolbox() { return mp_toolbox; }
   ...
   */
  void store_state() { LayoutViewBase::store_state(); }

  /**
   *  @brief Cancels all edit operations, clears the selection and resets the
   * mode to "Select"
   */
  void cancel_esc() { LayoutViewBase::cancel_esc(); }
   ...
   */
  void max_hier() { LayoutViewBase::max_hier(); }

  /**
   *  @brief Stop activities like shape browser and redrawing thread
   */
  void stop() { LayoutViewBase::stop(); }

  /**
   *  @brief Stop redrawing thread
   */
  void stop_redraw() { LayoutViewBase::stop_redraw(); }

  /**
   *  @brief Select last display state
   */
  void prev_display_state() { LayoutViewBase::prev_display_state(); }

  /**
   *  @brief Select next display state
   */
  void next_display_state() { LayoutViewBase::next_display_state(); }
  ...
  //  zoom slots
  void zoom_fit() { LayoutViewBase::zoom_fit(); }

  void zoom_fit_sel() { LayoutViewBase::zoom_fit_sel(); }

  void zoom_in() { LayoutViewBase::zoom_in(); }

  void zoom_out() { LayoutViewBase::zoom_out(); }

  void pan_left() { LayoutViewBase::pan_left(); }

  void pan_up() { LayoutViewBase::pan_up(); }

  void pan_right() { LayoutViewBase::pan_right(); }

  void pan_down() { LayoutViewBase::pan_down(); }

  void pan_left_fast() { LayoutViewBase::pan_left_fast(); }

  void pan_up_fast() { LayoutViewBase::pan_up_fast(); }

  void pan_right_fast() { LayoutViewBase::pan_right_fast(); }

  void pan_down_fast() { LayoutViewBase::pan_down_fast(); }

  //  called by children and owner
  void redraw() { LayoutViewBase::redraw(); }

  void redraw_layer(unsigned int index) { LayoutViewBase::redraw_layer(index); }

  void redraw_deco_layer() { LayoutViewBase::redraw_deco_layer(); }

  void redraw_cell_boxes() { LayoutViewBase::redraw_cell_boxes(); }
...
private:
  ...
  QWidget *mp_control_frame, *mp_toolbox_frame, *mp_hierarchy_frame,
      *mp_libraries_frame, *mp_bookmarks_frame;
...
public:
  LayoutViewNotification(const std::string &name, const std::string &title,
 ...
 */
class LAYVIEW_PUBLIC LayoutViewNotificationWidget : public QFrame {
  Q_OBJECT

public:
  LayoutViewNotificationWidget(LayoutViewWidget *parent,
 ...
 *
 *  This is a LayoutView which actually is a widget. It can be used in a widget
 * tree but only created if a QApplication is present.
 */
class LAYVIEW_PUBLIC LayoutViewWidget : public QFrame, public gsi::ObjectBase {
  Q_OBJECT
...
public:
   ...
   *  @brief Constructor
   */
  LayoutViewWidget(db::Manager *mgr, bool editable, lay::Plugin *plugin_parent,
                   QWidget *parent = 0,
                   unsigned int options = (unsigned int)LayoutView::LV_Normal);

  /**
   *  @brief Constructor (clone from another view)
   */
  LayoutViewWidget(lay::LayoutView *source, db::Manager *mgr, bool editable,
                   lay::Plugin *plugin_parent, QWidget *parent = 0,
                   unsigned int options = (unsigned int)LayoutView::LV_Normal);
   ...
   */
  lay::LayoutView *view() { return mp_view; }
  ...
  void emit_show_message(const std::string &s, int ms) {
    emit show_message(s, ms);
  }
  void emit_current_pos_changed(double x, double y, bool dbu_units) {
    emit current_pos_changed(x, y, dbu_units);
  }
  ...
  void emit_current_layer_changed(const lay::LayerPropertiesConstIterator &l) {
    emit current_layer_changed(l);
  }
...
signals:
  ...
  /**
   *  @brief This signal is emitted when the view wants to show a message for
   * the given time (of infinitely for ms == 0)
   */
  void show_message(const std::string &s, int ms);

  /**
   *  @brief This signal is emitted when the view wants to indicate a mouse
   * position change
   */
  void current_pos_changed(double x, double y, bool dbu_units);

  /**
   *  @brief This signal is emitted when the view wants to clear the mouse
   * position
   */
...
private:
  ...
  void notification_action(const LayoutViewNotification &notification,
                           const std::string &action);

  void resizeEvent(QResizeEvent *event);

  struct CompareNotificationPointers {
    bool operator()(const LayoutViewNotification *a,
                    const LayoutViewNotification *b) const {
      return *a < *b;
    }
  };
  ...
  std::map<const LayoutViewNotification *, QWidget *,
