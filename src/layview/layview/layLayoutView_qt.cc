...
LayoutViewNotificationWidget::LayoutViewNotificationWidget(
    LayoutViewWidget *parent, const LayoutViewNotification *notification)
...
LayoutViewWidget::LayoutViewWidget(db::Manager *mgr, bool editable,
                                   lay::Plugin *plugin_parent, QWidget *parent,
  ...
  //  NOTE: construction the LayoutView may trigger events (script code executed
  //  etc.) which must not meet an invalid mp_view pointer (e.g. in
  //  eventFilter). Hence, mp_view is 0 first, and set only after the LayoutView
  //  is successfully constructed.
  std::unique_ptr<LayoutView> view(
      new LayoutView(mgr, editable, plugin_parent, this, options));
  mp_view = view.release();
}

LayoutViewWidget::LayoutViewWidget(lay::LayoutView *source, db::Manager *mgr,
  ...
  //  is successfully constructed.
  std::unique_ptr<LayoutView> view(
      new LayoutView(source, mgr, editable, plugin_parent, this, options));
  mp_view = view.release();
}
...
void LayoutViewWidget::add_notification(
    const LayoutViewNotification &notificaton) {
  if (m_notification_widgets.find(&notificaton) ==
      m_notification_widgets.end()) {
    m_notifications.push_back(notificaton);
    QWidget *w =
        new LayoutViewNotificationWidget(this, &m_notifications.back());
    m_notification_widgets.insert(std::make_pair(&m_notifications.back(), w));
    mp_layout->insertWidget(0, w);
  }
}

void LayoutViewWidget::remove_notification(
    const LayoutViewNotification &notification) {
...
void LayoutViewWidget::notification_action(
    const LayoutViewNotification &notification, const std::string &action) {
  if (action == "reload") {
...
void LayoutViewWidget::view_deleted(lay::LayoutView *view) {
  ...
  mp_view = new LayoutView(view->manager(), view->is_editable(),
                           view->plugin_parent(), this, view->options());
}
...
LayoutViewSignalConnector::LayoutViewSignalConnector(QObject *parent,
                                                     lay::LayoutView *view)
...
void LayoutViewSignalConnector::select_cell_dispatch(
    const lay::LayoutViewBase::cell_path_type &path, int cellview_index) {
  mp_view->select_cell_dispatch(path, cellview_index);
}

void LayoutViewSignalConnector::current_layer_changed_slot(
    const lay::LayerPropertiesConstIterator &iter) {
  mp_view->current_layer_changed_slot(iter);
}

void LayoutViewSignalConnector::timer() { mp_view->timer(); }
...
void LayoutViewSignalConnector::app_terminated() { mp_view->close(); }
...
LayoutView::LayoutView(db::Manager *manager, bool editable,
                       lay::Plugin *plugin_parent, unsigned int options)
    : LayoutViewBase(this, manager, editable, plugin_parent, options),
      mp_widget(0), dm_setup_editor_option_pages(
                        this, &LayoutView::do_setup_editor_options_pages) {
...
LayoutView::LayoutView(lay::LayoutView *source, db::Manager *manager,
                       bool editable, lay::Plugin *plugin_parent,
                       unsigned int options)
    : LayoutViewBase(this, manager, editable, plugin_parent, options),
      mp_widget(0), dm_setup_editor_option_pages(
...
LayoutView::LayoutView(db::Manager *manager, bool editable,
                       lay::Plugin *plugin_parent, LayoutViewWidget *widget,
                       unsigned int options)
    : LayoutViewBase(this, manager, editable, plugin_parent, options),
      mp_widget(widget), dm_setup_editor_option_pages(
                             this, &LayoutView::do_setup_editor_options_pages) {
  //  ensures the deferred method scheduler is present
  tl::DeferredMethodScheduler::instance();
...
LayoutView::LayoutView(lay::LayoutView *source, db::Manager *manager,
                       bool editable, lay::Plugin *plugin_parent,
                       LayoutViewWidget *widget, unsigned int options)
    : LayoutViewBase(this, manager, editable, plugin_parent, options),
      mp_widget(widget), dm_setup_editor_option_pages(
                             this, &LayoutView::do_setup_editor_options_pages) {
  //  ensures the deferred method scheduler is present
...
bool LayoutView::event_filter(QObject *obj, QEvent *event, bool &taken) {
  if (obj == mp_min_hier_spbx || obj == mp_max_hier_spbx) {
    ...
    if (keyEvent && keyEvent->key() != Qt::Key_Home &&
        keyEvent->key() != Qt::Key_End && keyEvent->key() != Qt::Key_Delete &&
...
void LayoutView::init_ui(db::Manager *mgr) {
  ...
  //  NOTE: it's important to call LayoutViewBase::init from LayoutView because
  //  creating the plugins will need a fully constructed LayoutView (issue
  //  #1360)
  ...
  if (mp_widget) {
    ...
    if ((options() & LV_NoHierarchyPanel) == 0 && (options() & LV_Naked) == 0) {
      ...
      mp_hierarchy_panel =
          new lay::HierarchyControlPanel(this, hierarchy_frame, "hcp");
      left_frame_ly->addWidget(mp_hierarchy_panel, 1 /*stretch*/);

      QObject::connect(mp_hierarchy_panel,
                       SIGNAL(cell_selected(cell_path_type, int)), mp_connector,
                       SLOT(select_cell_dispatch(cell_path_type, int)));
      QObject::connect(mp_hierarchy_panel, SIGNAL(active_cellview_changed(int)),
                       mp_connector, SLOT(active_cellview_changed(int)));
      QObject::connect(mp_hierarchy_frame, SIGNAL(destroyed()), mp_connector,
                       SLOT(side_panel_destroyed()));
      ...
      QLabel *level_l1 =
          new QLabel(tl::to_qstring(" " + tl::to_string(QObject::tr("Levels"))),
                     levels_frame);
      ...
      QObject::connect(mp_min_hier_spbx, SIGNAL(valueChanged(int)),
                       mp_connector, SLOT(min_hier_changed(int)));
      QObject::connect(mp_max_hier_spbx, SIGNAL(valueChanged(int)),
                       mp_connector, SLOT(max_hier_changed(int)));
    }
    ...
    if ((options() & LV_NoBookmarksView) == 0 && (options() & LV_Naked) == 0) {
      ...
      mp_bookmarks_view =
          new lay::BookmarksView(this, bookmarks_frame, "bookmarks");
      left_frame_ly->addWidget(mp_bookmarks_view, 1 /*stretch*/);

      QObject::connect(mp_bookmarks_frame, SIGNAL(destroyed()), mp_connector,
                       SLOT(side_panel_destroyed()));
    }
    ...
    if ((options() & LV_NoLibrariesView) == 0 && (options() & LV_Naked) == 0) {
      ...
      mp_libraries_view =
          new lay::LibrariesView(this, mp_libraries_frame, "libs");
      left_frame_ly->addWidget(mp_libraries_view, 1 /*stretch*/);

      QObject::connect(mp_libraries_view, SIGNAL(active_library_changed(int)),
                       mp_connector, SLOT(active_library_changed(int)));
      QObject::connect(mp_libraries_frame, SIGNAL(destroyed()), mp_connector,
                       SLOT(side_panel_destroyed()));
    }

    if ((options() & LV_NoEditorOptionsPanel) == 0 &&
        (options() & LV_Naked) == 0) {

      mp_editor_options_frame = new lay::EditorOptionsFrame(0);
      mp_editor_options_frame->populate(this);

      QObject::connect(mp_editor_options_frame, SIGNAL(destroyed()),
                       mp_connector, SLOT(side_panel_destroyed()));
    }
    ...
    if ((options() & LV_NoLayers) == 0 && (options() & LV_Naked) == 0) {
      ...
      QObject::connect(mp_control_frame, SIGNAL(destroyed()), mp_connector,
                       SLOT(side_panel_destroyed()));
      QObject::connect(mp_control_panel, SIGNAL(tab_changed()), mp_connector,
                       SLOT(layer_tab_changed()));
      QObject::connect(mp_control_panel, SIGNAL(order_changed()), mp_connector,
                       SLOT(layer_order_changed()));
      QObject::connect(mp_control_panel,
                       SIGNAL(current_layer_changed(
                           const lay::LayerPropertiesConstIterator &)),
                       mp_connector,
                       SLOT(current_layer_changed_slot(
                           const lay::LayerPropertiesConstIterator &)));
      ...
      QObject::connect(mp_toolbox_frame, SIGNAL(destroyed()), mp_connector,
                       SLOT(side_panel_destroyed()));
    }
...
QWidget *LayoutView::widget() { return mp_widget; }
...
void LayoutView::show_properties() {
  ...
  //  if launched from a dialog, do not use "show" as this blocks user
  //  interaction
...
void LayoutView::do_change_active_cellview() { dm_setup_editor_option_pages(); }
...
void LayoutView::set_current() { set_current(this); }
...
LayoutView *LayoutView::current() { return ms_current; }
...
class GotoBookmarkAction : public lay::Action {
public:
  ...
  if (menu.is_valid(bm_menu)) {
    ...
    if (view && view->bookmarks().size() > 0) {
      ...
      for (size_t i = 0; i < bookmarks.size(); ++i) {
        ...
        menu.insert_item(bm_menu + ".end", tl::sprintf("bookmark_%d", i + 1),
                         action);
      }
...
lay::LayerPropertiesConstIterator LayoutView::current_layer() const {
  if (mp_control_panel) {
...
void LayoutView::set_selected_layers(
...
bool LayoutView::layer_model_updated() {
  //  because check_updated is called in the initialization phase, we check if
  //  the pointers to the widgets are non-null:
  if (mp_control_panel) {
...
void LayoutView::bookmark_current_view() {
  ...
  QString proposed_name =
      tl::to_qstring(bookmarks().propose_new_bookmark_name());

  while (true) {
    bool ok = false;
    QString text = QInputDialog::getText(
        mp_widget, QObject::tr("Enter Bookmark Name"),
        QObject::tr("Bookmark name"), QLineEdit::Normal, proposed_name, &ok);
    if (!ok) {
      break;
    } else if (text.isEmpty()) {
      QMessageBox::critical(mp_widget, QObject::tr("Error"),
                            QObject::tr("Enter a name for the bookmark"));
    } else {
...
void LayoutView::manage_bookmarks() {
  ...
  BookmarkManagementForm dialog(mp_widget, "bookmark_form", bookmarks(),
                                selected_bm);
  if (dialog.exec()) {
...
void LayoutView::layer_tab_changed() { update_content(); }

void LayoutView::layer_order_changed() { update_content(); }
...
tl::Color LayoutView::default_background_color() {
  if (!mp_widget) {
    return LayoutViewBase::default_background_color();
  } else {
    return tl::Color(
        mp_widget->palette().color(QPalette::Normal, QPalette::Base).rgb());
  }
}
...
void LayoutView::active_library_changed(int /*index*/) {
  ...
  //  TODO: could be passed through the LibraryController (like through some
  //  LibraryController::active_library)
...
void LayoutView::selected_cells_paths(
...
void LayoutView::set_current_cell_path(int cv_index,
...
bool LayoutView::is_activated() const { return m_activated; }
...
void LayoutView::signal_selection_changed() {
  if (selection_size() > 1) {
    message(tl::sprintf(tl::to_string(tr("selected: %ld objects")),
                        selection_size()));
  }
...
void LayoutView::message(const std::string &s, int timeout) {
  if (mp_widget) {
...
void LayoutView::activate_editor_option_pages() {
  ...
  if (eo_pages) {
    ...
    //  TODO: this is very inefficient as each "activate" will regenerate the
    //  tabs
    for (std::vector<lay::EditorOptionsPage *>::const_iterator op =
             eo_pages->pages().begin();
         op != eo_pages->pages().end(); ++op) {
      ...
      if ((*op)->plugin_declaration() == 0) {
        is_active = true;
      } else if (active_plugin() && active_plugin()->plugin_declaration() ==
                                        (*op)->plugin_declaration()) {
        is_active = true;
      }
...
void LayoutView::open_l2ndb_browser(int l2ndb_index, int cv_index) {
  lay::NetlistBrowserDialog *l2ndb_browser =
      get_plugin<lay::NetlistBrowserDialog>();
  if (l2ndb_browser) {
...
void LayoutView::open_rdb_browser(int rdb_index, int cv_index) {
  rdb::MarkerBrowserDialog *rdb_browser =
      get_plugin<rdb::MarkerBrowserDialog>();
  if (rdb_browser) {
...
QSize LayoutView::size_hint() const {
  ...
  } else if ((options() & LV_NoLayers) != 0 ||
             (options() & LV_NoHierarchyPanel) != 0 ||
             (options() & LV_NoLibrariesView) != 0) {
