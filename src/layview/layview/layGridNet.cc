...
static struct {
  ...
  const char *string;
} grid_styles[] = {{lay::GridNet::Invisible, "invisible"},
...
void GridNetStyleConverter::from_string(const std::string &value,
                                        lay::GridNet::GridStyle &style) {
  for (unsigned int i = 0; i < sizeof(grid_styles) / sizeof(grid_styles[0]);
       ++i) {
...
std::string GridNetStyleConverter::to_string(lay::GridNet::GridStyle style) {
  for (unsigned int i = 0; i < sizeof(grid_styles) / sizeof(grid_styles[0]);
       ++i) {
...
void GridNetPluginDeclaration::get_options(
    std::vector<std::pair<std::string, std::string>> &options) const {
  options.push_back(
      std::pair<std::string, std::string>(cfg_grid_color, "auto"));
  options.push_back(
      std::pair<std::string, std::string>(cfg_grid_ruler_color, "auto"));
  options.push_back(
      std::pair<std::string, std::string>(cfg_grid_axis_color, "auto"));
  options.push_back(
      std::pair<std::string, std::string>(cfg_grid_grid_color, "auto"));
  options.push_back(std::pair<std::string, std::string>(
      cfg_grid_style0,
      GridNetStyleConverter().to_string(lay::GridNet::Invisible)));
  options.push_back(std::pair<std::string, std::string>(
      cfg_grid_style1, GridNetStyleConverter().to_string(lay::GridNet::Dots)));
  options.push_back(std::pair<std::string, std::string>(
      cfg_grid_style2,
      GridNetStyleConverter().to_string(lay::GridNet::TenthDottedLines)));
  options.push_back(std::pair<std::string, std::string>(cfg_grid_visible,
                                                        tl::to_string(true)));
  options.push_back(std::pair<std::string, std::string>(cfg_grid_show_ruler,
                                                        tl::to_string(true)));
...
#if defined(HAVE_QT)
lay::ConfigPage *
GridNetPluginDeclaration::config_page(QWidget *parent,
...
lay::Plugin *
GridNetPluginDeclaration::create_plugin(db::Manager *, Dispatcher *,
                                        lay::LayoutViewBase *view) const {
  return new lay::GridNet(view);
}

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new GridNetPluginDeclaration(), 2010, "GridNetPlugin");

// ------------------------------------------------------------
//  Implementation of the GridNet object

GridNet::GridNet(LayoutViewBase *view)
    : lay::BackgroundViewObject(view->canvas()), lay::Plugin(view),
      mp_view(view), m_visible(false), m_show_ruler(true), m_grid(1.0),
...
void GridNet::render_bg(const lay::Viewport &vp, ViewObjectCanvas &canvas) {
  if (m_visible) {
    ...
    if (m_color.is_valid()) {
      ...
      color = tl::Color(128, 128,
                        128); // TODO: this is not a "real" automatic color ..
    }
    ...
    BitmapViewObjectCanvas *bmp_canvas =
        dynamic_cast<BitmapViewObjectCanvas *>(&canvas);
    ...
    db::DBox dbworld(trans_inv * db::DBox(0.0, 0.0, double(vp.width()),
                                          double(vp.height())));

    //  fw is the basic unit of the ruler geometry
    const lay::FixedFont &ff =
        lay::FixedFont::get_font(bmp_canvas->resolution());
    int fw = ff.width();
    ...
    if (m_show_ruler && dgrid < vp.width() * 0.2) {
      ...
      painter.fill_rect(db::Point(xoffset, vp.height() - yoffset - rh / 2),
                        db::Point(xoffset + int(floor(0.5 + dgrid)),
                                  vp.height() - yoffset + rh / 2),
                        ruler_color);

      painter.draw_rect(db::Point(xoffset + int(floor(0.5 + dgrid)),
                                  vp.height() - yoffset - rh / 2),
                        db::Point(xoffset + int(floor(0.5 + 2 * dgrid)),
                                  vp.height() - yoffset + rh / 2),
                        ruler_color);
      ...
      if (mp_view->global_trans().fp_trans() != db::DFTrans()) {
        ...
        db::Point pts[] = {
            db::Point(-4, -5), db::Point(-4, 5), db::Point(4, 5),
            db::Point(4, 3),   db::Point(-2, 3), db::Point(-2, 1),
            db::Point(3, 1),   db::Point(3, -1), db::Point(-2, -1),
            db::Point(-2, -5), db::Point(-4, -5)};
        ...
        for (db::Polygon::polygon_edge_iterator e = poly.begin_edge();
          ...
          db::Point p1 =
              p0 + db::Vector(int(floor(0.5 + (*e).p1().x() * 0.1 * rh * 4)),
                              -int(floor(0.5 + (*e).p1().y() * 0.1 * rh * 4)));
          db::Point p2 =
              p0 + db::Vector(int(floor(0.5 + (*e).p2().x() * 0.1 * rh * 4)),
                              -int(floor(0.5 + (*e).p2().y() * 0.1 * rh * 4)));
    ...
    //  draw grid
    if (style == Dots || style == TenthDottedLines || style == DottedLines ||
        style == LightDottedLines) {
      ...
      for (db::DCoord x = x1; x < x2 + grid * eps; x += grid) {
        for (db::DCoord y = y1; y < y2 + grid * eps; y += grid) {
          ...
          painter.draw_line(p - db::Vector(2, 0), p + db::Vector(2, 0),
                            grid_color);
          painter.draw_line(p - db::Vector(0, 2), p + db::Vector(0, 2),
                            grid_color);
        }
      }
      ...
      for (db::DCoord x = x1; x < x2 + grid * eps; x += grid) {
        for (db::DCoord y = y1; y < y2 + grid * eps; y += grid) {
          ...
          if (idx - 2.0 * floor(idx * 0.5) < 0.5) {
            ...
            db::Point p2(draw_round(trans * db::DPoint(x + grid, y + grid),
                                    vp.height()));
    ...
    if (m_style0 != Invisible && (draw_xaxis || draw_yaxis)) {
      ...
      if (m_style0 == Dots || m_style0 == TenthDottedLines ||
        ...
        for (db::DCoord x = x1; n > 0 && draw_xaxis; x += grid, --n) {
          painter.set(draw_round(trans * db::DPoint(x, 0.0), vp.height()),
                      axis_color);
        }
        for (db::DCoord y = y1; y < y2 + g * eps && draw_yaxis; y += g) {
          painter.set(draw_round(trans * db::DPoint(0.0, y), vp.height()),
                      axis_color);
        }
