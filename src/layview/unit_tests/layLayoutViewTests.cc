...
static bool compare_images_mono(const QImage &qimg, const std::string &au) {
  ...
  if (qimg2.width() == (int)qimg.width() &&
    ...
    for (int j = 0; j < qimg.height(); ++j) {
      for (int i = 0; i < qimg.width(); ++i) {
        if ((qimg.scanLine(j)[i / 8] & (0x80 >> (i % 8))) !=
            (qimg2.scanLine(j)[i / 8] & (0x80 >> (i % 8)))) {
          return false;
        }
      }
    }
...
TEST(1) {
  ...
  QImage qimg;
  qimg = lv.get_image_with_options(500, 500, 1, 1, 1.0,
                                   tl::Color(255, 255, 255), tl::Color(0, 0, 0),
                                   tl::Color(128, 128, 128), db::DBox(), false);
...
TEST(3) {
  ...
  qimg = lv.get_image_with_options(500, 500, 1, 1, 1.0,
                                   tl::Color(255, 255, 255), tl::Color(0, 0, 0),
                                   tl::Color(128, 128, 128), db::DBox(), true);
  ...
  EXPECT_EQ(compare_images_mono(qimg.convertToFormat(QImage::Format_Mono), au),
            true);
}
#endif
...
TEST(11) {
  ...
  tl::PixelBuffer img;
  img = lv.get_pixels_with_options(500, 500, 1, 1, 1.0,
                                   tl::Color(255, 255, 255), tl::Color(0, 0, 0),
                                   tl::Color(128, 128, 128), db::DBox());
...
TEST(13) {
  ...
  tl::BitmapBuffer img;
  img = lv.get_pixels_with_options_mono(500, 500, 1, tl::Color(255, 255, 255),
                                        tl::Color(0, 0, 0),
                                        tl::Color(128, 128, 128), db::DBox());
...
TEST(21) {
  ...
  std::string tmp = tmp_file("test.png");
  lv.save_image_with_options(tmp, 500, 500, 1, 1, 1.0, tl::Color(255, 255, 255),
                             tl::Color(0, 0, 0), tl::Color(128, 128, 128),
                             db::DBox(), false);
