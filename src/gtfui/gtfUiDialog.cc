 ...
 *    "o1" those that only appear in stream 1, "o2" only those that appear in
 * stream 2, "d" will receive std::pair's of elements that differ between 1
 * and 2. Two parameters govern the algorithm: "max_lookahead": tell how far to
 * look into the future to find "synchronized" parts. "min_sync": how many
 * elements must be equal (in sequence) to resync.
 */
template <class I, class O1, class O2, class O3, class O4, class EQ>
void diff(
    I b1, I e1, I b2, I e2, O1 common, O2 o1, O3 o2, O4 d,
    EQ equal = std::equal_to<typename std::iterator_traits<I>::value_type>(),
    unsigned long max_lookahead = 100, unsigned long min_sync = 3) {
  I i1 = b1;
  I i2 = b2;
    ...
    if (equal(*i1, *i2)) {
      ...
      *common = std::make_pair(*i1, *i2);
      ...
      for (I ii1 = i1; ++ii1 != e1 && s1 < max_lookahead; ++s1) {
        ...
        for (I j1 = ii1, j2 = i2;
             s > 0 && j1 != e1 && j2 != e2 && equal(*j1, *j2); ++j1, ++j2) {
          --s;
        }
        if (!s) {
          sync1 = true;
...
StripedBar::StripedBar(QWidget *parent) : QFrame(parent), mp_tv(0) {
  QObjectList cc = children();
  for (QObjectList::const_iterator child = cc.begin(); child != cc.end();
       ++child) {
...
void StripedBar::paintEvent(QPaintEvent *e) {
  ...
  if (mp_tv) {
    ...
    if (ytop >= 0) {
      painter.fillRect(lw, ytop, width() - 2 * lw, ybottom - ytop,
...
void StripedBar::set_treeview(QTreeView *tv) {
  mp_tv = tv;
  connect(mp_tv->verticalScrollBar() /* Qt4.2 only*/, SIGNAL(valueChanged(int)),
          this, SLOT(force_update(int)));
  connect(mp_tv, SIGNAL(expanded(const QModelIndex &)), this,
          SLOT(force_update(const QModelIndex &)));
  connect(mp_tv, SIGNAL(collapsed(const QModelIndex &)), this,
          SLOT(force_update(const QModelIndex &)));
}

void StripedBar::force_update(int) { update(); }

void StripedBar::force_update(const QModelIndex &) { update(); }

// --------------------------------------------------------------------------
//  UiDialog implementation

static void diff_log_event_list(QTreeWidget *tv, QTreeWidgetItem *parent,
                                const tl::Variant &dleft,
                                const tl::Variant &dright);
static void diff_log_event_data(QTreeWidget *tv, QTreeWidgetItem *parent,
                                const tl::Variant &data_left,
                                const tl::Variant &data_right);
static void add_log_event_list(QTreeWidget *tv, int column,
                               QTreeWidgetItem *parent,
                               const tl::Variant &dlist);
static void add_log_event_data(QTreeWidget *tv, int column,
                               QTreeWidgetItem *parent,
                               const tl::Variant &data);
...
static std::string log_event_to_text(const gtf::LogEventBase *e) {
  std::string t = e->name();
  /* too much:
  std::vector< std::pair<std::string, std::string> > attrs;
  e->attributes (attrs);
  for (std::vector< std::pair<std::string, std::string> >::const_iterator a =
  attrs.begin (); a != attrs.end (); ++a) { t += " "; t += a->first; t += ":";
    t += a->second;
  }
  */
  return t;
...
struct make_entry_both {
  ...
  void operator=(
      std::pair<const gtf::LogEventBase *, const gtf::LogEventBase *> e) const {
...
struct make_entry_diff {
  ...
  void operator=(
    ...
    item->setData(0, Qt::BackgroundRole,
                  QVariant(dep ? left_diff_brush_dep : left_diff_brush));
    item->setData(0, Qt::UserRole, QVariant(true));
    item->setData(0, Qt::UserRole + 2, QVariant::fromValue((void *)e.first));
    item->setText(1, tright.c_str());
    item->setData(1, Qt::BackgroundRole,
                  QVariant(dep ? right_diff_brush_dep : right_diff_brush));
    item->setData(1, Qt::UserRole, QVariant(true));
    item->setData(1, Qt::UserRole + 2, QVariant::fromValue((void *)e.second));
...
struct ptr_eq {
  bool operator()(const gtf::LogEventBase *a,
                  const gtf::LogEventBase *b) const {
    return *a == *b;
  }
};

static void add_log_event(QTreeWidget *tv, int column, QTreeWidgetItem *parent,
                          const tl::Variant &d, QTreeWidgetItem *p = 0) {
  if (!p) {
...
struct enter_data {
  enter_data(QTreeWidget *tv, QTreeWidgetItem *item, int column)
      : mp_tv(tv), mp_item(item), m_column(column) {}

  enter_data &operator=(const tl::Variant &data) {
    add_log_event(mp_tv, m_column, mp_item, data);
    return *this;
  }
  enter_data &operator=(const std::pair<tl::Variant, tl::Variant> &data) {
...
struct enter_data_diff {
  enter_data_diff(QTreeWidget *tv, QTreeWidgetItem *item)
      : mp_tv(tv), mp_item(item) {}
...
static void diff_log_event_list(QTreeWidget *tv, QTreeWidgetItem *parent,
  ...
  diff(dleft.begin(), dleft.end(), dright.begin(), dright.end(), both, left,
       right, delta, std::equal_to<tl::Variant>());
}
...
UiDialog::UiDialog() {
  ...
  mp_ui->curr_event_list->header()->setResizeMode(
  ...
  connect(mp_ui->log_list,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          this, SLOT(item_selected(QTreeWidgetItem *, QTreeWidgetItem *)));
...
void UiDialog::open_files(const std::string &fn_au,
                          const std::string &fn_current) {
  m_fn_au = fn_au;
  ...
  diff(m_au_events.begin(), m_au_events.end(), m_current_events.begin(),
       m_current_events.end(), b, l, r, d, ptr_eq());
}
...
void UiDialog::item_selected(QTreeWidgetItem *current, QTreeWidgetItem *) {
  if (current->data(0, Qt::UserRole + 1) != QVariant() ||
    ...
    if (current->data(0, Qt::UserRole + 2) != QVariant()) {
      eleft = (const LogEventBase *)current->data(0, Qt::UserRole + 2)
                  .value<void *>();
      eleft->attributes(attrs_left);
    }

    if (current->data(1, Qt::UserRole + 2) != QVariant()) {
      eright = (const LogEventBase *)current->data(1, Qt::UserRole + 2)
                   .value<void *>();
    ...
    if (eleft != 0 && eright != 0 &&
        std::string(eleft->name()) == eright->name()) {
      same_type = true;
    }

    std::vector<std::pair<std::string, std::string>>::const_iterator l =
        attrs_left.begin();
    std::vector<std::pair<std::string, std::string>>::const_iterator r =
        attrs_right.begin();
