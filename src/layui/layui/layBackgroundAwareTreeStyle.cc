...
void BackgroundAwareTreeStyle::drawPrimitive(QStyle::PrimitiveElement pe,
  ...
  if (pe == PE_IndicatorBranch) {
    ...
    if (opt->state & State_Children) {
      ...
      if ((opt->state & State_Selected) &&
          styleHint(QStyle::SH_ItemView_ShowDecorationSelected, opt, w)) {
      ...
      if (!(opt->state & State_MouseOver)) {
        if (c.green() < 128) {
          c = QColor((c.red() * 2 + 255) / 3, (c.green() * 2 + 255) / 3,
                     (c.blue() * 2 + 255) / 3);
        } else {
          c = QColor((c.red() * 8) / 9, (c.green() * 8) / 9,
                     (c.blue() * 8) / 9);
        }
      }
