...
LayerSourceDialog::LayerSourceDialog(QWidget *parent) : QDialog(parent) {
...
NewLayoutPropertiesDialog::NewLayoutPropertiesDialog(QWidget *parent)
  ...
  connect(mp_ui->tech_cbx, SIGNAL(currentIndexChanged(int)), this,
          SLOT(tech_changed()));
}
...
void NewLayoutPropertiesDialog::tech_changed() {
  ...
  if (technology_index >= 0 &&
      technology_index < (int)db::Technologies::instance()->technologies()) {
...
bool NewLayoutPropertiesDialog::exec_dialog(
    std::string &technology, std::string &cell_name, double &dbu, double &size,
    std::vector<db::LayerProperties> &layers, bool &current_panel) {
  mp_ui->tech_cbx->clear();
  unsigned int technology_index = 0;
  for (db::Technologies::const_iterator t =
           db::Technologies::instance()->begin();
       t != db::Technologies::instance()->end(); ++t, ++technology_index) {
  ...
  for (std::vector<db::LayerProperties>::const_iterator l = layers.begin();
       l != layers.end(); ++l) {
    if (l != layers.begin()) {
      layer_string += ", ";
    }
  ...
  if (QDialog::exec()) {
    ...
    if (technology_index >= 0 &&
        technology_index < (int)db::Technologies::instance()->technologies()) {
      technology =
          db::Technologies::instance()->begin()[technology_index].name();
    } else {
...
void NewLayoutPropertiesDialog::accept() {
  ...
  if (mp_ui->topcell_le->text().isEmpty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("The topcell must be specified")));
  }
...
bool NewCellPropertiesDialog::exec_dialog(const db::Layout *layout,
...
void NewCellPropertiesDialog::accept() {
  ...
  if (mp_layout->cell_by_name(tl::to_string(mp_ui->name_le->text()).c_str())
          .first) {
    throw tl::Exception(
        tl::to_string(QObject::tr("A cell with that name already exists: %s")),
        tl::to_string(mp_ui->name_le->text()));
  }
...
bool NewLayerPropertiesDialog::exec_dialog(const lay::CellView &cv,
                                           db::LayerProperties &src) {
  if (cv.is_valid()) {
    mp_ui->layout_lbl->setText(tl::to_qstring(
        (tl::to_string(QObject::tr("Layer for layout: ")) + cv->name())));
...
void NewLayerPropertiesDialog::accept() {
  ...
  if (lp.layer < 0 && lp.datatype < 0) {
    if (lp.name.empty()) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Either a layer/datatype combination or a "
                                    "name must be specified for a layer")));
    }
  } else if (lp.layer < 0 || lp.datatype < 0) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Both layer and datatype must be specified for a layer")));
  }
  QDialog::accept();
  END_PROTECTED;
}
...
MoveOptionsDialog::MoveOptionsDialog(QWidget *parent) : QDialog(parent) {
...
MoveToOptionsDialog::MoveToOptionsDialog(QWidget *parent) : QDialog(parent) {
  setObjectName(QString::fromUtf8("move_to_options_dialog"));

  mp_ui = new Ui::MoveToOptionsDialog();
  mp_ui->setupUi(this);

  QToolButton *buttons[3][3] = {{mp_ui->lb, mp_ui->cb, mp_ui->rb},
                                {mp_ui->lc, mp_ui->cc, mp_ui->rc},
                                {mp_ui->lt, mp_ui->ct, mp_ui->rt}};
...
bool MoveToOptionsDialog::exec_dialog(int &mode_x, int &mode_y,
                                      db::DPoint &target) {
...
void RenameCellDialog::accept() {
  ...
  if (mp_layout->cell_by_name(tl::to_string(mp_ui->name_le->text()).c_str())
          .first) {
    throw tl::Exception(
        tl::to_string(QObject::tr("A cell with that name already exists")));
  }
  QDialog::accept();
  END_PROTECTED;
}

bool RenameCellDialog::exec_dialog(const db::Layout &layout,
                                   std::string &name) {
  mp_layout = &layout;
...
CopyCellModeDialog::CopyCellModeDialog(QWidget *parent) : QDialog(parent) {
...
bool CopyCellModeDialog::exec_dialog(int &copy_mode) {
...
DeleteCellModeDialog::DeleteCellModeDialog(QWidget *parent) : QDialog(parent) {
...
bool ReplaceCellOptionsDialog::exec_dialog(const lay::CellView &cv,
                                           int &replace_mode,
  ...
  lay::CellTreeModel *model = new lay::CellTreeModel(
      mp_ui->cell_selection_cbx, &cv->layout(),
      lay::CellTreeModel::Flat | lay::CellTreeModel::NoPadding);
  mp_ui->cell_selection_cbx->setModel(model);
  mp_ui->cell_selection_cbx->setEditText(
      tl::to_qstring(cv->layout().cell_name(cell_index)));
  ...
  if (QDialog::exec()) {
    ...
    std::string cn =
        tl::to_string(mp_ui->cell_selection_cbx->lineEdit()->text());
    std::pair<bool, db::cell_index_type> cc =
        find_cell_by_display_name(cv->layout(), cn.c_str());
...
void ReplaceCellOptionsDialog::accept() {
  ...
  lay::CellTreeModel *model =
      dynamic_cast<lay::CellTreeModel *>(mp_ui->cell_selection_cbx->model());
  if (model) {
    std::string cn =
        tl::to_string(mp_ui->cell_selection_cbx->lineEdit()->text());
    std::pair<bool, db::cell_index_type> cc =
        find_cell_by_display_name(*model->layout(), cn.c_str());
    if (!cc.first) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Not a valid cell name: ")) + cn);
    }
  }
...
ClearLayerModeDialog::ClearLayerModeDialog(QWidget *parent) : QDialog(parent) {
...
bool ClearLayerModeDialog::exec_dialog(int &clear_mode) {
  QRadioButton *buttons[3] = {mp_ui->local_rb, mp_ui->hierarchically_rb,
                              mp_ui->layout_rb};
...
OpenLayoutModeDialog::OpenLayoutModeDialog(QWidget *parent) : QDialog(parent) {
...
bool DuplicateLayerDialog::exec_dialog(lay::LayoutViewBase *view, int &cv,
                                       int &layer, int &cv_r, int &layer_r,
                                       int &hier_mode, bool &clear_before) {
  mp_view = view;
...
void DuplicateLayerDialog::accept() {
  ...
  if (cv < 0) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No layout specified for source")));
  }

  int cv_r = mp_ui->cvr_cbx->current_cv_index();
  if (cv_r < 0) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No layout specified for result")));
  }

  if (fabs(mp_view->cellview(cv)->layout().dbu() -
           mp_view->cellview(cv_r)->layout().dbu()) > db::epsilon) {
    throw tl::Exception(tl::to_string(QObject::tr(
        "Source and result layouts must have the same database unit")));
  }

  if (mp_ui->layer_cbx->current_layer() < 0) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No layer specified for source")));
  }
  if (mp_ui->layerr_cbx->current_layer() < 0) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No layer specified for result")));
  }

  if (mp_ui->hier_mode_cbx->currentIndex() == 2 &&
      mp_ui->cv_cbx->current_cv_index() != mp_ui->cvr_cbx->current_cv_index()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Source layout and result layout must be "
                                  "same in 'cell by cell' mode")));
  }

  if (mp_ui->cv_cbx->current_cv_index() == mp_ui->cvr_cbx->current_cv_index() &&
      mp_ui->layer_cbx->current_layer() == mp_ui->layerr_cbx->current_layer()) {
    throw tl::Exception(tl::to_string(
        QObject::tr("Source and target layer must not be identical")));
  }
...
FlattenInstOptionsDialog::FlattenInstOptionsDialog(QWidget *parent,
                                                   bool enable_pruning)
    : QDialog(parent) {
...
bool FlattenInstOptionsDialog::exec_dialog(int &levels, bool &prune) {
  ...
  mp_ui->levels_sb->setValue(
      (levels < 0 || levels > mp_ui->levels_sb->maximum())
          ? mp_ui->levels_sb->maximum()
          : levels);
...
UserPropertiesForm::UserPropertiesForm(QWidget *parent)
  ...
  connect(mp_ui->prop_list, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)),
          this, SLOT(dbl_clicked(QTreeWidgetItem *, int)));
  connect(mp_ui->mode_tab, SIGNAL(currentChanged(int)), this,
  ...
  mp_hl_attributes.reset(
      new GenericSyntaxHighlighterAttributes(mp_hl_basic_attributes.get()));
  lay::GenericSyntaxHighlighter *hl = new GenericSyntaxHighlighter(
      mp_ui->text_edit, input, mp_hl_attributes.get(), true);
  input.close();
...
void UserPropertiesForm::set_properties(
  ...
  for (db::PropertiesRepository::properties_set::const_iterator p =
    ...
    entry->setText(
        0, tl::to_qstring(mp_prep->prop_name(p->first).to_parsable_string()));
...
void UserPropertiesForm::set_meta_info(
    db::Layout::meta_info_iterator begin_meta,
    db::Layout::meta_info_iterator end_meta, const db::Layout &layout) {
  ...
  for (auto m = m_begin_meta; m != m_end_meta; ++m) {
    ...
    entry->setText(0, tl::to_qstring((m->second.persisted ? "*" : "") +
                                     layout.meta_info_name(m->first)));
    entry->setText(1, tl::to_qstring(m->second.description));
    entry->setText(2, tl::to_qstring(m->second.value.to_parsable_string()));
  }
}

bool UserPropertiesForm::show(LayoutViewBase *view, unsigned int cv_index,
                              db::properties_id_type &prop_id) {
  return show(view, cv_index, prop_id, db::Layout::meta_info_iterator(),
              db::Layout::meta_info_iterator());
}
...
void UserPropertiesForm::dbl_clicked(QTreeWidgetItem *, int) { edit(); }
