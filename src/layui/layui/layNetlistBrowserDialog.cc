...
NetlistBrowserDialog::NetlistBrowserDialog(lay::Dispatcher *root,
                                           LayoutViewBase *vw)
    : lay::Browser(root, vw), lay::ViewService(vw->canvas()),
      m_window(lay::NetlistBrowserConfig::FitNet), m_window_dim(0.0),
      m_max_shape_count(0), m_auto_color_enabled(false),
      m_marker_line_width(-1), m_marker_vertex_size(-1), m_marker_halo(-1),
      m_marker_dither_pattern(-1), m_marker_intensity(0),
      m_use_original_colors(false), m_cv_index(-1), m_l2n_index(-1),
      m_mouse_state(0) {
  ...
  if (view()) {
    view()->cellviews_changed_event.add(
        this, &NetlistBrowserDialog::cellviews_changed);
    view()->cellview_changed_event.add(this,
                                       &NetlistBrowserDialog::cellview_changed);
    view()->l2ndb_list_changed_event.add(this,
  ...
  connect(mp_ui->l2ndb_cb, SIGNAL(activated(int)), this,
          SLOT(l2ndb_index_changed(int)));
  connect(mp_ui->configure_pb, SIGNAL(clicked()), this,
          SLOT(configure_clicked()));
  connect(mp_ui->probe_pb, SIGNAL(clicked()), this,
          SLOT(probe_button_pressed()));
  connect(mp_ui->sticky_cbx, SIGNAL(clicked()), this,
          SLOT(sticky_mode_clicked()));

  cellviews_changed();

  mp_ui->browser_page->selection_changed_event.add(
      this, &NetlistBrowserDialog::selection_changed);
}
...
db::LayoutToNetlist *NetlistBrowserDialog::db() {
  return mp_ui->browser_page->db();
}

const lay::NetlistObjectsPath &NetlistBrowserDialog::current_path() const {
...
void NetlistBrowserDialog::configure_clicked() {
  ...
  lay::ConfigurationDialog config_dialog(this, lay::Dispatcher::instance(),
                                         "NetlistBrowserPlugin");
  config_dialog.exec();
}

bool NetlistBrowserDialog::mouse_move_event(const db::DPoint & /*p*/,
...
bool NetlistBrowserDialog::mouse_click_event(const db::DPoint &p,
                                             unsigned int buttons, bool prio) {
  if (prio && (buttons & lay::LeftButton) != 0 && m_mouse_state != 0) {

    //  TODO: not used yet, borrowed from net tracer ... TODO: implement short
    //  locator!
    if (m_mouse_state == 2) {
      ...
      view()->message(
          tl::to_string(QObject::tr("Click on the second point in the net")));
...
void NetlistBrowserDialog::probe_net(const db::DPoint &p, bool trace_path) {
  ...
  if (trace_path) {
    stop_search_box = db::DBox(m_mouse_first_point, m_mouse_first_point)
                          .enlarged(db::DVector(l, l));
  }
  ...
  //  locate the seed shape to figure out the cv index and layer
  {
    ...
    lay::ShapeFinder finder(true /*point mode*/, false /*all levels*/,
                            db::ShapeIterator::All);

    //  go through all visible layers of all cellviews and find a seed shape
    for (lay::LayerPropertiesConstIterator lprop = view()->begin_layers();
         !lprop.at_end(); ++lprop) {
      if (lprop->is_visual()) {
  ...
  //  if the cv index is not corresponding to the one of the current netlist,
  //  ignore this event
  ...
  //  determine the start point
  {
    ...
    std::vector<db::DCplxTrans> tv =
        view()->cv_transform_variants(m_cv_index, start_layer);
    if (tv.empty()) {
      return;
    }

    db::CplxTrans tt =
        tv.front() * db::CplxTrans(cv->layout().dbu()) * cv.context_trans();
  ...
  if (l2ndb) {
    ...
    root = l2ndb->netlist()->circuit_by_name(
        cv->layout().cell_name(cv.cell_index()));
    if (root) {

      //  determines the corresponding layer inside the database and probe the
      //  net from this region and the start point.
      ...
      for (db::Connectivity::layer_iterator layer = conn.begin_layers();
        ...
        db::LayerProperties lp =
            l2ndb->internal_layout()->get_properties(*layer);
        if (!lp.is_null()) {
  ...
  if (net) {
    ...
    path.net = net;
    path.path =
        lay::NetlistObjectPath::path_type(sc_path.begin(), sc_path.end());
  }

  mp_ui->browser_page->select_path(path);

  //  emits the probe event
  //  NOTE: browser_page->current_path () will hold the paired path with the
  //  schematic side being expanded.
  probe_event(mp_ui->browser_page->current_path().first(),
              mp_ui->browser_page->current_path().second());
}
...
lay::ViewService *NetlistBrowserDialog::view_service_interface() {
  return this;
}
  ...
  if (m_l2n_index < int(view()->num_l2ndbs()) && m_l2n_index >= 0) {
    ...
    if (lvsdb && !mp_ui->browser_page->is_netlist_mode()) {
      ...
      lay::FileDialog save_dialog(
          this, tl::to_string(QObject::tr("Save LVS Database")),
          "KLayout LVS DB files (*.lvsdb)");
      ...
      if (save_dialog.get_save(fn)) {
        ...
        tl::SelfTimer timer(tl::verbosity() >= 11,
                            tl::to_string(QObject::tr("Saving")));
      ...
      //  prepare and open the file dialog
      lay::FileDialog save_dialog(
          this, tl::to_string(QObject::tr("Save Netlist Database")),
          "KLayout L2N DB files (*.l2n)");
...
void NetlistBrowserDialog::reload_clicked() {
  ...
  if (m_l2n_index < int(view()->num_l2ndbs()) && m_l2n_index >= 0) {
    ...
    if (l2ndb && !l2ndb->filename().empty()) {
      ...
      tl::log << tl::to_string(QObject::tr("Loading file: "))
              << l2ndb->filename();
      tl::SelfTimer timer(tl::verbosity() >= 11,
                          tl::to_string(QObject::tr("Loading")));

      mp_ui->browser_page->set_db(0);
      try {

        m_l2ndb_name = l2ndb->name();
        db::LayoutToNetlist *new_l2ndb =
            db::LayoutToNetlist::create_from_file(l2ndb->filename());
...
void NetlistBrowserDialog::open_clicked() {
  ...
  lay::FileDialog open_dialog(
      this, tl::to_string(QObject::tr("Load Netlist/LVS Database File")), fmts);
  if (open_dialog.get_open(m_open_filename)) {
    ...
    int l2n_index = view()->add_l2ndb(
        db::LayoutToNetlist::create_from_file(m_open_filename));
...
bool NetlistBrowserDialog::configure(const std::string &name,
  ...
  if (active() && need_update) {
    ...
    mp_ui->browser_page->set_highlight_style(
        m_marker_color, m_marker_line_width, m_marker_vertex_size,
        m_marker_halo, m_marker_dither_pattern, m_marker_intensity,
        m_use_original_colors, m_auto_color_enabled ? &m_auto_colors : 0);
  }
...
void NetlistBrowserDialog::activated() {
  ...
  lay::restore_dialog_state(this, state,
                            false /*don't adjust the section sizes*/);
...
void NetlistBrowserDialog::update_content() {
  ...
  mp_ui->browser_page->enable_updates(
      false); //  Avoid building the internal lists several times ...
  db_changed = mp_ui->browser_page->set_db(l2ndb);
  mp_ui->browser_page->set_max_shape_count(m_max_shape_count);
  mp_ui->browser_page->set_highlight_style(
      m_marker_color, m_marker_line_width, m_marker_vertex_size, m_marker_halo,
      m_marker_dither_pattern, m_marker_intensity, m_use_original_colors,
...
void NetlistBrowserDialog::deactivated() {
  ...
  if (lay::Dispatcher::instance()) {
    lay::Dispatcher::instance()->config_set(
        cfg_l2ndb_window_state,
        lay::save_dialog_state(this, false /*don't store the section sizes*/)
            .c_str());
  }
