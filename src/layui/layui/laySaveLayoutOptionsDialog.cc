...
SaveLayoutOptionsDialog::SaveLayoutOptionsDialog(QWidget *parent,
                                                 const std::string &title)
    : QDialog(parent), m_technology_index(-1) {
  ...
  connect(mp_ui->buttonBox, SIGNAL(accepted()), this,
          SLOT(ok_button_pressed()));
  connect(mp_ui->buttonBox, SIGNAL(clicked(QAbstractButton *)), this,
          SLOT(button_pressed(QAbstractButton *)));
...
void SaveLayoutOptionsDialog::button_pressed(QAbstractButton *button) {
  if (button == mp_ui->buttonBox->button(QDialogButtonBox::Reset)) {
    reset_button_pressed();
  }
}

void SaveLayoutOptionsDialog::current_tech_changed(int index) {
...
void SaveLayoutOptionsDialog::reset_button_pressed() {
  BEGIN_PROTECTED
...
void SaveLayoutOptionsDialog::ok_button_pressed() {
  BEGIN_PROTECTED
...
void SaveLayoutOptionsDialog::commit() {
  if (m_technology_index < 0) {
    return;
  }
  ...
  for (std::vector<std::pair<StreamWriterOptionsPage *, std::string>>::iterator
    ...
    if (page->first) {
      ...
      db::FormatSpecificWriterOptions *specific_options =
          m_opt_array[m_technology_index].get_options(page->second);
      if (!specific_options) {
      ...
      page->first->commit(specific_options, m_tech_array[m_technology_index],
                          false);
    }
  }
}

void SaveLayoutOptionsDialog::update() {
  if (m_technology_index < 0) {
    return;
  }
  ...
  for (std::vector<std::pair<StreamWriterOptionsPage *, std::string>>::iterator
    ...
    if (page->first) {
      ...
      if (!specific_options) {
        //  Create a container for the options unless there is one already
        std::unique_ptr<db::FormatSpecificWriterOptions> new_options(
            StreamWriterPluginDeclaration::plugin_for_format(page->second)
                ->create_specific_options());
...
bool SaveLayoutOptionsDialog::edit_global_options(
    lay::Dispatcher *config_root, db::Technologies *technologies) {
  m_opt_array.clear();
  m_tech_array.clear();
  ...
  for (db::Technologies::const_iterator t = technologies->begin();
       t != technologies->end(); ++t, ++i) {
  ...
  if (get_options_internal()) {
    ...
    unsigned int i = 0;
    for (db::Technologies::iterator t = technologies->begin();
         t != technologies->end() && i < m_opt_array.size(); ++t, ++i) {
...
bool SaveLayoutOptionsDialog::get_options(db::SaveLayoutOptions &options) {
  mp_ui->tech_cbx->hide();
...
bool SaveLayoutOptionsDialog::get_options_internal() {
  update();
  if (exec()) {
    commit();
    return true;
...
SaveLayoutAsOptionsDialog::SaveLayoutAsOptionsDialog(QWidget *parent,
                                                     const std::string &title)
  ...
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator fmt =
    ...
    if (fmt->can_write()) {
      ...
      //  obtain the config page from the plugin which we identify by format
      //  name
      const StreamWriterPluginDeclaration *decl =
          plugin_for_format(fmt->format_name());
      ...
      if (decl) {
        ...
        if (alias) {
          ...
          int index = -1;
          int n = 0;
          for (tl::Registrar<db::StreamFormatDeclaration>::iterator i =
                   tl::Registrar<db::StreamFormatDeclaration>::begin();
               i != tl::Registrar<db::StreamFormatDeclaration>::end(); ++i) {
          ...
          if (index >= 0 && index < int(m_tab_positions.size())) {
            m_pages.push_back(
                std::make_pair(m_pages[index].first, fmt->format_name()));
            m_tab_positions.push_back(m_tab_positions[index]);
          } else {
            m_pages.push_back(std::make_pair((StreamWriterOptionsPage *)0,
                                             fmt->format_name()));
          ...
          StreamWriterOptionsPage *page =
              decl->format_specific_options_page(mp_ui->options_stack);

          m_pages.push_back(std::make_pair(page, fmt->format_name()));
          m_tab_positions.push_back(page ? mp_ui->options_stack->addWidget(page)
                                         : empty_widget_index);
        }
        ...
        m_pages.push_back(
            std::make_pair((StreamWriterOptionsPage *)0, fmt->format_name()));
        m_tab_positions.push_back(empty_widget_index);
      }
    }
  }
  ...
  connect(mp_ui->buttonBox, SIGNAL(accepted()), this,
          SLOT(ok_button_pressed()));
  connect(mp_ui->fmt_cbx, SIGNAL(activated(int)), this,
          SLOT(fmt_cbx_changed(int)));
}
...
void SaveLayoutAsOptionsDialog::ok_button_pressed() {
  BEGIN_PROTECTED
  ...
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator fmt =
           tl::Registrar<db::StreamFormatDeclaration>::begin();
       fmt != tl::Registrar<db::StreamFormatDeclaration>::end() && index >= 0;
       ++fmt) {
    if (fmt->can_write()) {
      if (index-- == 0) {
  ...
  if (decl) {
    for (std::vector<
             std::pair<StreamWriterOptionsPage *, std::string>>::iterator page =
             m_pages.begin();
         page != m_pages.end(); ++page) {
      if (page->second == fmt_name) {
        if (page->first) {
          std::unique_ptr<db::FormatSpecificWriterOptions> options(
              decl->create_specific_options());
          if (options.get()) {
            page->first->commit(
                options.get(), mp_tech,
                tl::OutputStream::output_mode_from_filename(
                    m_filename,
                    index_to_om(mp_ui->compression->currentIndex())) !=
                    tl::OutputStream::OM_Plain);
          }
        }
        break;
      }
...
bool SaveLayoutAsOptionsDialog::get_options(
    lay::LayoutViewBase *view, unsigned int cv_index, const std::string &fn,
    tl::OutputStream::OutputStreamMode &om, db::SaveLayoutOptions &options) {
  ...
  if (exec()) {
    ...
    if (mp_ui->layersel_cbx->currentIndex() == 0 /*all*/) {
      ...
      for (LayerPropertiesConstIterator layer = view->begin_layers();
           layer != view->end_layers(); ++layer) {
    ...
    //  identify the plugin in charge of creating the particular options and
    //  commit only those options
    const StreamWriterPluginDeclaration *decl =
        plugin_for_format(options.format());
    if (decl) {
      for (std::vector<std::pair<StreamWriterOptionsPage *,
                                 std::string>>::iterator page = m_pages.begin();
           page != m_pages.end(); ++page) {
...
void SaveLayoutAsOptionsDialog::fmt_cbx_changed(int index) {
  if (index >= 0 && index < int(m_tab_positions.size())) {
