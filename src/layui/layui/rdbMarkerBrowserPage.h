 ...
 */
class MarkerBrowserPage : public QFrame, public Ui::MarkerBrowserPage {
  Q_OBJECT
...
public:
  ...
  /**
   *  @brief Gets a value indicating whether all items in the directory tree are
   * shown
   */
  bool show_all() const { return m_show_all; }

  /**
   *  @brief Sets a value indicating whether all items in the directory tree are
   * shown
   *
  ...
  /**
   *  @brief Gets a value indicating whether to list the shapes in the info
   * panel
   */
  bool list_shapes() const { return m_list_shapes; }

  /**
   *  @brief Sets a value indicating whether to list the shapes in the info
   * panel
   *
   *  If this property is set to false, shapes will not be listed in the info
   * panel.
   */
   ...
   *  Unlike attachment to a RDB, this method will try to keep the current
   * selections, etc.
   */
  void update_content();
   ...
   *
   *  This method should be called if the cellview has changed so the markers
   * can be recomputed and shown in the new cell context.
   */
  void update_markers();
   ...
   */
  void set_window(rdb::window_type window_type, const lay::Margin &window_dim,
                  rdb::context_mode_type context);

  /**
   *  @brief Set the maximum number of markers shown in the marker selection
   * list
   */
   ...
   */
  void set_marker_style(tl::Color color, int line_width, int vertex_size,
                        int halo, int dither_pattern);
...
public slots:
  ...
  void directory_selection_changed(const QItemSelection &selected,
                                   const QItemSelection &deselected);
  void markers_header_clicked(int section);
  void markers_sorting_changed(int section, Qt::SortOrder order);
  void markers_selection_changed(const QItemSelection &selected,
                                 const QItemSelection &deselected);
  void markers_current_changed(const QModelIndex &current,
