...
void DitherPatternSelectionButton::update_pattern() {
  ...
  QRect rt(fm.boundingRect(
      text)); // dummy text to be compliant with the other color button
  ...
  if (m_dither_pattern < 0) {
    ...
    QRectF r(0, 0, rt.width() - pxpainter.pen().widthF(),
             rt.height() - pxpainter.pen().widthF());
    pxpainter.drawText(r,
    ...
    if (mp_view) {
      dp_info = &mp_view->dither_pattern()
                     .pattern((unsigned int)m_dither_pattern)
                     .scaled(dpr);
    } else {
      static lay::DitherPattern default_pattern;
      dp_info =
          &default_pattern.pattern((unsigned int)m_dither_pattern).scaled(dpr);
    }

    QPushButton::setIcon(
        dp_info->get_bitmap(rt.width() * dpr, rt.height() * dpr, dpr));
  }
}

void DitherPatternSelectionButton::menu_about_to_show() { update_menu(); }

void DitherPatternSelectionButton::update_menu() {
  menu()->clear();
  menu()
      ->addAction(QObject::tr("None"), this, SLOT(menu_selected()))
      ->setData(-1);
  ...
  try {
    ...
    for (unsigned int i = 0; i < palette.stipples(); ++i) {
      ...
      if (int(n) < std::distance(patterns.begin(), patterns.end())) {
        ...
        menu()
            ->addAction(QIcon(info.get_bitmap(-1, -1, dpr)),
                        tl::to_qstring(name), this, SLOT(menu_selected()))
            ->setData(n);
      }
    }
...
const lay::LayoutViewBase *CellViewSelectionComboBox::layout_view() const {
  return mp_private->layout_view;
}

void CellViewSelectionComboBox::set_layout_view(
    const lay::LayoutViewBase *layout_view) {
  ...
  for (unsigned int cv = 0; cv < layout_view->cellviews(); ++cv) {
    if (layout_view->cellview(cv).is_valid()) {
      addItem(tl::to_qstring(layout_view->cellview(cv)->name() + ", " +
                             tl::to_string(QObject::tr("Cell")) + " '" +
                             layout_view->cellview(cv)->layout().cell_name(
                                 layout_view->cellview(cv).cell_index()) +
                             "'"));
    } else {
      addItem(tl::to_qstring(layout_view->cellview(cv)->name() + ", " +
                             tl::to_string(QObject::tr("Undefined cell"))));
    }
  }
...
void LayerSelectionComboBox::item_selected(int index) {
  ...
  if (mp_private->view != 0 && index == count() - 1 &&
      mp_private->new_layer_enabled) {
    ...
    if (!mp_private->view->current_layer().is_null()) {
      ...
      if (li >= 0) {
        lp = mp_private->view
                 ->cellview(mp_private->view->current_layer()->cellview_index())
                 ->layout()
                 .get_properties(li);
      }
    }
    ...
    if (prop_dia.exec_dialog(cv, lp)) {
      ...
      for (unsigned int l = 0; l < cv->layout().layers(); ++l) {
        if (cv->layout().is_valid_layer(l) &&
            cv->layout().get_properties(l).log_equal(lp)) {
          throw tl::Exception(
              tl::to_string(
                  QObject::tr("A layer with that signature already exists: ")) +
              lp.to_string());
        }
      }

      if (mp_private->view->manager()) {
        mp_private->view->manager()->transaction(
            tl::to_string(QObject::tr("New layer")));
      }
      ...
      //  NOTE: add_new_layers has triggered update_layer_list which already
      //  added the new layer
...
struct LPIPairCompareOp {
  bool operator()(const std::pair<db::LayerProperties, int> &a,
                  const std::pair<db::LayerProperties, int> &b) const {
...
void LayerSelectionComboBox::set_view(lay::LayoutViewBase *view, int cv_index,
                                      bool all_layers) {
  ...
  view->layer_list_changed_event.add(
      this, &LayerSelectionComboBox::on_layer_list_changed);

  update_layer_list();
}

void LayerSelectionComboBox::on_layer_list_changed(int) { update_layer_list(); }
...
void LayerSelectionComboBox::update_layer_list() { dm_update_layer_list(); }
...
void LayerSelectionComboBox::do_update_layer_list() {
  ...
  if (mp_private->view) {
    ...
    if (!layout) {
      ...
      std::map<std::pair<db::LayerProperties, int>, std::string,
               LPIPairCompareOp>
          name_for_layer(cmp_op);
      LayerPropertiesConstIterator lp = mp_private->view->begin_layers();
      while (!lp.at_end()) {
        if (lp->cellview_index() == mp_private->cv_index &&
            !lp->has_children() &&
            (mp_private->all_layers || lp->layer_index() >= 0) &&
            lp->source(true).layer_props() != db::LayerProperties()) {
          std::pair<db::LayerProperties, int> k(lp->source(true).layer_props(),
                                                lp->layer_index());
          name_for_layer.insert(std::make_pair(
              k, lp->display_string(mp_private->view, true,
                                    true /*always show source*/)));
      ...
      for (unsigned int l = 0; l < layout->layers(); ++l) {
        if (layout->is_valid_layer(l)) {
          std::pair<db::LayerProperties, int> k(layout->get_properties(l),
                                                int(l));
      ...
      for (std::vector<std::pair<db::LayerProperties, int>>::iterator ll =
               mp_private->layers.begin();
           ll != mp_private->layers.end(); ++ll) {
        std::map<std::pair<db::LayerProperties, int>, std::string,
                 LPIPairCompareOp>::const_iterator ln =
            name_for_layer.find(*ll);
    ...
    for (unsigned int l = 0; l < mp_private->layout->layers(); ++l) {
      if (mp_private->layout->is_valid_layer(l)) {
        mp_private->layers.push_back(
            std::make_pair(mp_private->layout->get_properties(l), int(l)));
      }
    }
...
void LayerSelectionComboBox::set_current_layer(
...
db::LayerProperties LayerSelectionComboBox::current_layer_props() const {
  int i = currentIndex();
  if (i < 0 || i > int(mp_private->layers.size())) {
...
void LibrarySelectionComboBox::set_technology_filter(const std::string &tech,
                                                     bool enabled) {
      ...
      if (m_tech_set && lib->for_technologies()) {
        ...
        std::string tn =
            tl::join(std::vector<std::string>(lib->get_technologies().begin(),
                     ...
                     ",");
        item_text += tl::to_string(
            QObject::tr("[Technology %1]").arg(tl::to_qstring(tn)));
      }
...
void LibrarySelectionComboBox::set_current_library(db::Library *lib) {
  if (lib != current_library()) {
    ...
    for (int i = 0; i < count(); ++i) {
      ...
      if (!data.isNull()) {
        item_lib =
            db::LibraryManager::instance().lib(data.value<db::lib_id_type>());
      }
      if (item_lib == lib) {
        setCurrentIndex(i);
        return;
...
db::Library *LibrarySelectionComboBox::current_library() const {
...
void SimpleColorButton::set_color(QColor c) { set_color_internal(c); }

void SimpleColorButton::set_color_internal(QColor c) {
  m_color = c;

  QFontMetrics fm(font(), this);
  QRect rt(fm.boundingRect(QObject::tr(
      "Auto"))); // dummy text to be compliant with the other color button
...
QColor SimpleColorButton::get_color() const { return m_color; }
...
const char *color_icon = "xxxxxaaxxxxbbxxxxx"
                         "xxxxA00AxxB11Bxxxx"
                         "xxxa0000ab1111bxxx"
                         "xxxa0000ab1111bxxx"
                         "xxxxA00AxxB11Bxxxx"
...
const char *color_icon_2x = "xxxxxxxxxxaaaaxxxxxxxxbbbbxxxxxxxxxx"
...
void ColorButton::build_color_menu(QMenu *menu, QObject *receiver,
                                   const char *browse_slot,
                                   const char *selected_slot) {
  tl_assert(selected_slot != 0);

  menu->clear();

  menu->addAction(QObject::tr("Automatic"), receiver, selected_slot)
      ->setData(QVariant(QColor()));
  if (browse_slot) {
    ...
    for (unsigned int i = 0; i < palette.colors(); ++i) {
      ...
      if ((i % 6) == 0) {
        ...
        submenu = menu->addMenu(
            ...
            tl::to_qstring(tl::sprintf("#%d .. %d", i + 1,
                                       std::min(i + 6, palette.colors()))));
      }
      ...
      submenu
          ->addAction(QIcon(icon), tl::to_qstring(name), receiver,
                      selected_slot)
          ->setData(QVariant(color));
    }
...
void ColorButton::build_menu() {
  build_color_menu(menu(), this, SLOT(browse_selected()),
                   SLOT(menu_selected()));
}

void ColorButton::set_color(QColor c) { set_color_internal(c); }
...
QColor ColorButton::get_color() const { return m_color; }

void ColorButton::menu_about_to_show() { build_menu(); }
...
MarginWidget::MarginWidget(QWidget *parent, const char *name)
  ...
  connect(mode, SIGNAL(currentIndexChanged(int)), this,
          SLOT(mode_selection_changed()));

  set_margin(lay::Margin());
}

lay::Margin MarginWidget::get_margin() const {
...
void MarginWidget::set_margin(const lay::Margin &margin) {
  ...
  mp_rel_edit->setText(
      tl::to_qstring(tl::to_string(margin.relative_value() * 100.0)));
...
const int le_decoration_space =
    2; //  additional distance between decoration icons and text

DecoratedLineEdit::DecoratedLineEdit(QWidget *parent)
    : QLineEdit(parent), m_clear_button_enabled(false),
      m_options_button_enabled(false), m_escape_signal_enabled(false),
      m_tab_signal_enabled(false), mp_options_menu(0) {
...
bool DecoratedLineEdit::focusNextPrevChild(bool next) {
  if (m_tab_signal_enabled && isEnabled()) {
    QKeyEvent event(QEvent::KeyPress, next ? Qt::Key_Tab : Qt::Key_Backtab,
                    Qt::NoModifier);
    keyPressEvent(&event);
    if (event.isAccepted()) {
      return true;
...
void DecoratedLineEdit::set_options_button_enabled(bool en) {
  if (en != m_options_button_enabled) {
    ...
    if (!en) {
      ...
      margins.setLeft(m_default_left_margin +
                      mp_options_label->sizeHint().width() +
                      le_decoration_space);
    }
    setTextMargins(margins);
...
void DecoratedLineEdit::resizeEvent(QResizeEvent *event) {
  ...
  if (m_clear_button_enabled) {
    ...
    QRect r = geometry();
    mp_clear_label->setGeometry(r.width() - fw - label_size.width(), 0,
                                label_size.width(), r.height());
  }
...
std::vector<std::string> InteractiveListWidget::get_values() {
...
void InteractiveListWidget::refresh_flags() {
  for (int i = 0; i < count(); ++i) {
    item(i)->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEditable |
