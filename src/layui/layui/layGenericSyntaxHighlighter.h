 ...
 *
 *  The user data will contain useful data for bracket detection and other
 * things
 */
class LAYUI_PUBLIC SyntaxHighlighterUserData : public QTextBlockUserData {
public:
  /**
   *  @brief Constructor
   */
   ...
   */
  std::vector<SyntaxHighlighterElement> &elements() { return m_elements; }
 ...
 *
 *  Rule implementations must implement this base class in order to plug into
 * the generic syntax highlighter framework.
 */
...
public:
   ...
   *
   *  On success, this method returns true. It will use the input arguments for
   * dynamic rules to replace the %N placeholders. If the rule matches, the
   * end_index will be set to the end of the sequence and output_args will
   * contain any arguments matched. generationId is a counter that will
   * incremented when a new block is highlighted. This allows caching matches in
   * some cases (i.e. regexp matcher).
   */
  virtual bool match(const QString &input, unsigned int generation_id,
                     int index, int &end_index,
                     const QList<QString> &input_args,
                     QList<QString> &output_args) const = 0;
...
public:
  ...
  /**
   *  @brief Constructor from a rule base class, attribute ID and target context
   * ID
   *
   *  This object will become owner of the rule object.
   */
  GenericSyntaxHighlighterRule(GenericSyntaxHighlighterRuleBase *rule,
                               int attribute_id = -1, int target_context_id = 0,
   ...
   */
  GenericSyntaxHighlighterRule &
  operator=(const GenericSyntaxHighlighterRule &d);

  /**
   *  @brief Returns true, if this rule does not have an implementation
   */
  bool is_null() const { return mp_rule == 0; }

  /**
   *  @brief Sets the attribute associated with this context
   */
  void set_attribute_id(int id) { m_attribute_id = id; }

  /**
   *  @brief Gets the attribute associated with this context
   */
  int attribute_id() const { return m_attribute_id; }

  /**
   *  @brief Sets the target context ID
   */
  void set_target_context_id(int id) { m_target_context_id = id; }

  /**
   *  @brief Gets the target context ID
   */
  int target_context_id() const { return m_target_context_id; }
   ...
   */
  void set_lookahead(bool f) { m_lookahead = f; }
   ...
   */
  void set_column(int c) { m_column = c; }
   ...
   *
   *  If true, this rule only matches if the match is the first non-space
   * character after the current position
   */
  void set_first_non_space(bool f) { m_first_non_space = f; }

  /**
   *  @brief Gets the first non-space flag
   */
  bool first_non_space() const { return m_first_non_space; }
   ...
   *
   *  Returns true, if the rule matches. In this case, end_index will be set to
   * the end of the sequence and output_args will contain any arguments matched.
   */
  bool match(const QString &input, unsigned int generation_id, int index,
 ...
 *
 *  A context is a number of rules that are applied sequentially. The first
 * match is taken and determines the target context. Target contexts are
 * identified by an integer context ID, corresponding to the context names in
 * the Kate highlighter scripts. Special context ID's are 0 (#stay), -n (#pop n
 * times). A context is associated with an attribute, which is given by an
 * integer ID.
 */
class LAYUI_PUBLIC GenericSyntaxHighlighterContext {
public:
  enum { no_context = 0x7ffffff };
   ...
   */
  int fallthrough_context() const { return m_fallthrough_context; }
   ...
   */
  int linebegin_context() const { return m_linebegin_context; }
   ...
   */
  void set_lineend_context(int context_id) { m_lineend_context = context_id; }
   ...
   */
  int lineend_context() const { return m_lineend_context; }
  ...
  /**
   *  @brief Match the context against the given string and return true, if any
   * rule matches
   *
   *  If any rule matches, this method will return true. In that case, end_index
   * to the to the next character inside "string", output_args will contain the
   * new arguments (if there are any) and new_context will be set to the new
   *  context ID (0 to stay, negative for #pop's).
   *  attribute_id will be set to the ID of the attribute of the rule that was
   * found.
   */
  bool match(const QString &string, unsigned int generation_id, int index,
             int &end_index, const QList<QString> &input_args,
             QList<QString> &output_args, int &new_context,
...
private:
   ...
   */
  void set_name(const QString &n) { m_name = n; }
};
...
public:
   ...
   *
   *  The initial context is by default the first one that is inserted with
   * "insert".
   */
  int initial_context_id() const { return m_initial_context; }

  /**
   *  @brief Gets a context by name and creates the context if it does not exist
   * already.
   *
   ...
   */
  bool is_empty() const { return m_contexts_by_name.empty(); }
...
public:
  ...
  GenericSyntaxHighlighterAttributes(
      const GenericSyntaxHighlighterAttributes *basic_attributes = 0);
...
private:
  ...
  void add(const QString &name, int id, bool bold, bool italic, bool underline,
...
public:
   ...
   *
   *  This method will match the given string based on the current state. On
   * success, this method modifies the state and returns true. On success,
   * end_index to the end of the sequence found, attribute_id will hold the
   * attribute ID of the rule found. def_attribute_id will always contain the
   * attribute ID of the previous state, also if the method returns false.
   */
  bool match(const QString &string, unsigned int generation_id, int index,
             int &end_index, int &def_attribute_id, int &attribute_id);
 ...
 */
class LAYUI_PUBLIC GenericSyntaxHighlighter : public QSyntaxHighlighter {
public:
  /**
   ...
   *  @param initialize_attributes If true, the attributes are initialized from
   * the itemData lines
   */
  GenericSyntaxHighlighter(QObject *parent, QIODevice &input,
