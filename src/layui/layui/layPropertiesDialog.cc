...
public:
  PropertiesTreeModel(PropertiesDialog *dialog, int icon_width, int icon_height)
      : QAbstractItemModel(dialog), mp_dialog(dialog), m_icon_width(icon_width),
  ...
  QVariant data(const QModelIndex &index, int role) const {
    if (role == Qt::DisplayRole) {
      if (tree_id_type(index.internalId()) <
          tree_id_type(mp_dialog->properties_pages().size())) {
        return tl::to_qstring(
            mp_dialog->properties_pages()[index.internalId()]->description(
                index.row()));
      } else if (index.row() < int(mp_dialog->properties_pages().size())) {
        return tl::to_qstring(
            mp_dialog->properties_pages()[index.row()]->description());
      }
    } else if (role == Qt::DecorationRole) {
      QIcon icon;
      if (tree_id_type(index.internalId()) <
          tree_id_type(mp_dialog->properties_pages().size())) {
        icon = mp_dialog->properties_pages()[index.internalId()]->icon(
            index.row(), m_icon_width, m_icon_height);
      } else if (index.row() < int(mp_dialog->properties_pages().size())) {
        icon = mp_dialog->properties_pages()[index.row()]->icon(m_icon_width,
                                                                m_icon_height);
      }
      if (!icon.isNull()) {
  ...
  Qt::ItemFlags flags(const QModelIndex &index) const {
    ...
    if (tree_id_type(index.internalId()) >=
            tree_id_type(mp_dialog->properties_pages().size()) &&
        !mp_dialog->properties_pages()[index.row()]->can_apply_to_all()) {
  ...
  bool hasChildren(const QModelIndex &parent) const {
    return (!parent.isValid() ||
            tree_id_type(parent.internalId()) >=
                tree_id_type(mp_dialog->properties_pages().size()));
  }
  ...
  QModelIndex parent(const QModelIndex &child) const {
    if (tree_id_type(child.internalId()) <
        tree_id_type(mp_dialog->properties_pages().size())) {
      return createIndex(int(child.internalId()), child.column(),
                         tree_id_type(mp_dialog->properties_pages().size()));
    } else {
      return QModelIndex();
    }
  ...
  int page_index(const QModelIndex &index) { return int(index.internalId()); }

  int object_index(const QModelIndex &index) { return int(index.row()); }
  ...
  QModelIndex index_for(int page_index) {
    if (page_index < 0) {
      ...
      return createIndex(page_index, 0,
                         tree_id_type(mp_dialog->properties_pages().size()));
    }
  }
...
PropertiesDialog::PropertiesDialog(QWidget * /*parent*/, db::Manager *manager,
                                   lay::Editables *editables)
    : QDialog(0 /*parent*/), mp_manager(manager), mp_editables(editables),
      m_index(0), m_prev_index(-1), m_auto_applied(false), m_transaction_id(0),
  ...
  for (lay::Editables::iterator e = mp_editables->begin();
       e != mp_editables->end(); ++e) {
  ...
  //  Necessary to maintain the page order for UI regression testing of 0.18 vs.
  //  0.19 (because tl::Collection has changed to order) ..
  std::reverse(mp_properties_pages.begin(), mp_properties_pages.end());

  //  Add a label as a dummy
  mp_none = new QLabel(QObject::tr("No object with properties to display"),
                       mp_ui->content_frame);
  ...
  //  if at end disable the "Next" button and return (this may only happen at
  //  the first call)
  mp_tree_model = new PropertiesTreeModel(this, mp_ui->tree->iconSize().width(),
                                          mp_ui->tree->iconSize().height());
  ...
  connect(mp_ui->cancel_button, SIGNAL(clicked()), this,
          SLOT(cancel_pressed()));
  ...
  connect(mp_ui->apply_to_all_cbx, SIGNAL(clicked()), this,
          SLOT(apply_to_all_pressed()));
  connect(
      mp_ui->tree->selectionModel(),
      SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)), this,
      SLOT(current_index_changed(const QModelIndex &, const QModelIndex &)));
  connect(
      mp_ui->tree->selectionModel(),
...
void PropertiesDialog::disconnect() {
  ...
  for (std::vector<lay::PropertiesPage *>::iterator p =
           mp_properties_pages.begin();
       p != mp_properties_pages.end(); ++p) {
    delete *p;
  }
...
void PropertiesDialog::apply_to_all_pressed() {
  ...
  if (mp_ui->apply_to_all_cbx->isChecked()) {
    ...
    mp_ui->tree->setCurrentIndex(
        mp_tree_model->index_for(m_index, int(m_object_indexes.front())));
  }
  m_signals_enabled = true;
}
...
void PropertiesDialog::current_index_changed(const QModelIndex &index,
    ...
    if (mp_tree_model->parent(index).isValid()) {
      ...
      if (mp_properties_pages[m_index]->can_apply_to_all()) {
        ...
        for (auto i = selection.begin(); i != selection.end(); ++i) {
          if (mp_tree_model->parent(*i).isValid() &&
              mp_tree_model->page_index(*i) == m_index) {
      ...
      m_index = index.row();
      mp_ui->apply_to_all_cbx->setChecked(
          mp_properties_pages[m_index]->can_apply_to_all());
...
void PropertiesDialog::update_controls() {
  ...
  if (m_index < 0) {
    ...
    mp_ui->apply_to_all_cbx->setEnabled(
        !mp_properties_pages[m_index]->readonly() &&
        mp_properties_pages[m_index]->can_apply_to_all());
    mp_ui->relative_cbx->setEnabled(mp_ui->apply_to_all_cbx->isEnabled() &&
                                    mp_ui->apply_to_all_cbx->isChecked());
...
void PropertiesDialog::next_pressed() {
  ...
  if (object_index >= int(mp_properties_pages[m_index]->count())) {
    ...
    //  because we checked that there are any further elements, this should not
    //  happen:
    if (m_index >= int(mp_properties_pages.size())) {
      return;
    }
...
void PropertiesDialog::update_title() {
  if (m_index < 0) {
    ...
    setWindowTitle(tl::to_qstring(
        tl::to_string(QObject::tr("Object Properties - ")) +
        tl::to_string(m_current_object + 1) +
        tl::to_string(QObject::tr(" of ")) + tl::to_string(m_objects)));
  }
}
...
void PropertiesDialog::apply() {
  ...
  try {
    ...
    //  we assume the page somehow indicates the error and does not apply the
    //  values
  }
