...
void LCPRemitter::the_slot() { emit the_signal(m_index); }
...
void LCPDitherPalette::create_pixmap_for(LCPActiveLabel *b, int n) {
  lay::DitherPattern pattern = !mp_view ? lay::DitherPattern::default_pattern()
                                        : mp_view->dither_pattern();
...
void LCPStylePalette::create_pixmap_for_line_style(LCPActiveLabel *b, int n) {
  const lay::LineStyles &styles =
      !mp_view ? lay::LineStyles::default_style() : mp_view->line_styles();
...
LayerToolbox::LayerToolbox(QWidget *parent, const char *name)
  ...
  connect(vp, SIGNAL(visibility_change(bool)), this,
          SLOT(visibility_changed(bool)));
  connect(vp, SIGNAL(transparency_change(bool)), this,
          SLOT(transparency_changed(bool)));
  ...
  connect(sp, SIGNAL(line_style_selected(int)), this,
          SLOT(line_style_changed(int)));
  connect(sp, SIGNAL(line_styles_changed(const lay::LineStyles &)), this,
          SLOT(line_styles_changed(const lay::LineStyles &)));

  LCPAnimationPalette *ap = new LCPAnimationPalette(this, "anim");
  add_panel(ap, tl::to_string(QObject::tr("Animation")).c_str());
  connect(ap, SIGNAL(animation_selected(int)), this,
          SLOT(animation_changed(int)));
  ...
  connect(dp, SIGNAL(pattern_changed(const lay::DitherPattern &)), this,
          SLOT(dither_pattern_changed(const lay::DitherPattern &)));

  LCPColorPalette *pf = new LCPColorPalette(this, "colors");
  mp_frame_palette = pf;
  add_panel(pf, tl::to_string(QObject::tr("Frame color")).c_str());
  connect(pf, SIGNAL(color_selected(QColor)), this,
          SLOT(frame_color_changed(QColor)));
  connect(pf, SIGNAL(color_brightness_selected(int)), this,
          SLOT(frame_color_brightness(int)));

  LCPColorPalette *p = new LCPColorPalette(this, "colors_frame");
  mp_palette = p;
  add_panel(p, tl::to_string(QObject::tr("Color")).c_str());
  connect(p, SIGNAL(color_selected(QColor)), this,
          SLOT(fill_color_changed(QColor)));
  connect(p, SIGNAL(color_brightness_selected(int)), this,
          SLOT(fill_color_brightness(int)));
...
void LayerToolbox::add_panel(QWidget *panel_widget, const char *text) {
  ...
  b->setMaximumSize(
      QSize(b->maximumSize().width(), b->sizeHint().height() - 4));
...
QSize LayerToolbox::sizeHint() const {
  ...
  int w = 148;
  for (std::vector<std::pair<QWidget *, QWidget *>>::const_iterator i =
           m_tool_panels.begin();
       i != m_tool_panels.end(); ++i) {
    w = std::max(
        std::max(i->first->sizeHint().width(), i->second->sizeHint().width()),
        w);
  }
...
void LayerToolbox::rearrange(int w, int h) {
  for (std::vector<std::pair<QWidget *, QWidget *>>::iterator i =
           m_tool_panels.begin();
       i != m_tool_panels.end(); ++i) {
...
template <class Op> void LayerToolbox::foreach_selected(const Op &op) {
...
struct SetColor {
   ...
   *
   *  @param flags Bitmask that defines which colors to change:
   * #0=fill,#1=frame,#2=vertex,#3=text
   *  @param c The color to apply
   */
  SetColor(QColor c, unsigned int flags) : m_color(c), m_flags(flags) {
    // .. nothing yet ..
  }
...
void LayerToolbox::fill_color_changed(QColor c) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change fill color")));
...
void LayerToolbox::frame_color_changed(QColor c) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change frame color")));
...
void LayerToolbox::fill_color_brightness(int delta) {
  ...
  db::Transaction tr(mp_view->manager(), tl::to_string(QObject::tr(
                                             "Change fill color brightness")));
...
void LayerToolbox::line_styles_changed(const lay::LineStyles &styles) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Edit line styles")));
...
void LayerToolbox::dither_pattern_changed(const lay::DitherPattern &pattern) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Edit stipple pattern")));
...
void LayerToolbox::dither_changed(int di) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Set stipple pattern")));
...
void LayerToolbox::visibility_changed(bool visible) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(visible ? QObject::tr("Show layer")
                                           : QObject::tr("Hide layer")));
...
struct SetTransparency {
  SetTransparency(bool t) : m_transparent(t) {}
...
void LayerToolbox::transparency_changed(bool transparent) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change transparency")));
...
struct SetAnimation {
  SetAnimation(int mode) : m_mode(mode) {}
...
void LayerToolbox::animation_changed(int mode) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change animation mode")));
...
void LayerToolbox::width_changed(int width) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change line width")));
...
void LayerToolbox::xfill_changed(bool xf) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change cross fill")));
...
void LayerToolbox::line_style_changed(int ls) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change line style")));
...
void LayerToolbox::marked_changed(bool marked) {
  ...
  db::Transaction tr(mp_view->manager(),
                     tl::to_string(QObject::tr("Change marked vertices")));
