...
MarkerBrowserDialog::MarkerBrowserDialog(lay::Dispatcher *root,
                                         lay::LayoutViewBase *vw)
    : lay::Browser(root, vw), m_context(rdb::AnyCell), m_window(rdb::FitMarker),
      m_window_dim(0.0), m_max_marker_count(0), m_marker_line_width(-1),
      m_marker_vertex_size(-1), m_marker_halo(-1), m_marker_dither_pattern(-1),
      m_cv_index(-1), m_rdb_index(-1) {
  ...
  if (view()) {
    view()->cellviews_changed_event.add(
        this, &MarkerBrowserDialog::cellviews_changed);
    view()->cellview_changed_event.add(this,
                                       &MarkerBrowserDialog::cellview_changed);
    view()->rdb_list_changed_event.add(this,
  ...
  m_export_action =
      new QAction(QObject::tr("Export To Layout"), mp_ui->file_menu);
  ...
  m_unload_all_action =
      new QAction(QObject::tr("Unload All"), mp_ui->file_menu);
  ...
  connect(m_unload_all_action, SIGNAL(triggered()), this,
          SLOT(unload_all_clicked()));
  ...
  connect(mp_ui->layout_cb, SIGNAL(activated(int)), this,
          SLOT(cv_index_changed(int)));
  connect(mp_ui->rdb_cb, SIGNAL(activated(int)), this,
          SLOT(rdb_index_changed(int)));
  connect(mp_ui->configure_pb, SIGNAL(clicked()), this,
          SLOT(configure_clicked()));
...
void MarkerBrowserDialog::configure_clicked() {
  lay::ConfigurationDialog config_dialog(this, lay::Dispatcher::instance(),
                                         "MarkerBrowserPlugin");
  config_dialog.exec();
}
...
void MarkerBrowserDialog::unload_all_clicked() {
  ...
  if (modified) {
    ...
    QMessageBox msgbox(
        QMessageBox::Question, QObject::tr("Unload Without Saving"),
        QObject::tr(
            "At least one database was not saved.\nPress 'Continue' to "
            "continue anyway or 'Cancel' for not unloading the database."));
    QPushButton *ok =
    ...
    if (rdb && rdb->is_modified()) {
      ...
      QMessageBox msgbox(
          ...
          QObject::tr(
              ...
              "anyway or 'Cancel' for not unloading the database."));
      QPushButton *ok =
          msgbox.addButton(QObject::tr("Continue"), QMessageBox::AcceptRole);
      msgbox.setDefaultButton(msgbox.addButton(QMessageBox::Cancel));
...
static void collect_categories(const Category *cat,
  ...
  if (cat->sub_categories().begin() == cat->sub_categories().end()) {
    ...
    for (Categories::const_iterator subcat = cat->sub_categories().begin();
...
void MarkerBrowserDialog::export_clicked() {
  ...
  QString text = QInputDialog::getText(
      this, QObject::tr("Layer Offset"),
      QObject::tr("Enter the first GDS layer that is produced.\nLeave empty "
                  "for not producing GDS layer numbers at all:"),
  ...
  if (!text.isEmpty()) {
    lnum = text.toInt(&ok);
    if (!ok) {
      throw tl::Exception(tl::to_string(QObject::tr("Invalid layer number: ")) +
                          tl::to_string(text));
    }
  }
  ...
  try {
    ...
    db::Transaction transaction(view()->is_editable() ? view()->manager() : 0,
                                tl::to_string(QObject::tr("Export Markers")));

    std::vector<const Category *> categories;
    for (rdb::Categories::const_iterator cat = rdb->categories().begin();
         cat != rdb->categories().end(); ++cat) {
      collect_categories(&*cat, categories);
    }

    for (std::vector<const Category *>::const_iterator cat = categories.begin();
      ...
      for (Cells::const_iterator cell = rdb->cells().begin();
           cell != rdb->cells().end(); ++cell) {

        std::pair<Database::const_item_ref_iterator,
                  Database::const_item_ref_iterator>
            be = rdb->items_by_cell_and_category(cell->id(), (*cat)->id());
        ...
        if (cc.first) {
          ...
          if (top_cell) {
            std::pair<bool, db::DCplxTrans> context =
                cell->path_to(top_cell->id(), rdb);
            if (context.first) {
        ...
        for (Database::const_item_ref_iterator item = be.first;
             item != be.second; ++item) {
          ...
          for (rdb::Values::const_iterator v = (*item)->values().begin();
            ...
            if (polygon_value) {
              ...
              db::Polygon polygon =
                  db::Polygon(db::DCplxTrans(1.0 / cv->layout().dbu()) * trans *
                              polygon_value->value());
              ...
              db::Edge edge =
                  db::Edge(db::DCplxTrans(1.0 / cv->layout().dbu()) * trans *
                           edge_value->value());
              cv->layout().cell(target_cell).shapes(layer).insert(edge);

            } else if (edge_pair_value) {

              //  Note: there is no edge pair inside the database currently.
              //  Hence we convert it to a polygon
              db::EdgePair edge_pair =
                  db::EdgePair(db::DCplxTrans(1.0 / cv->layout().dbu()) *
                               trans * edge_pair_value->value());
              cv->layout()
                  .cell(target_cell)
                  .shapes(layer)
                  .insert(edge_pair.to_polygon(1));

            } else if (box_value) {

              db::Polygon polygon =
                  db::Polygon(db::DCplxTrans(1.0 / cv->layout().dbu()) * trans *
                              db::DPolygon(box_value->value()));
              cv->layout().cell(target_cell).shapes(layer).insert(polygon);
            }
          }
        }
  ...
  if (m_rdb_index < int(view()->num_rdbs()) && m_rdb_index >= 0) {
    ...
    if (rdb) {
      ...
      lay::FileDialog save_dialog(
          this, tl::to_string(QObject::tr("Save Marker Database File")),
          "KLayout RDB files (*.lyrdb)");
...
void MarkerBrowserDialog::open_clicked() {
  ...
  for (tl::Registrar<rdb::FormatDeclaration>::iterator rdr =
       ...
       rdr != tl::Registrar<rdb::FormatDeclaration>::end(); ++rdr) {
    fmts += ";;" + rdr->file_format();
  }

  //  prepare and open the file dialog
  lay::FileDialog open_dialog(
      this, tl::to_string(QObject::tr("Load Marker Database File")), fmts);
...
bool MarkerBrowserDialog::configure(const std::string &name,
  ...
  if (active() && need_update) {
    ...
    mp_ui->browser_frame->set_marker_style(m_marker_color, m_marker_line_width,
                                           m_marker_vertex_size, m_marker_halo,
...
void MarkerBrowserDialog::update_content() {
  ...
  mp_ui->browser_frame->enable_updates(
      false); //  Avoid building the internal lists several times ...
  ...
  if (rdb) {
    //  Note: it appears to be required to show the browser page after it has
    //  been configured. Otherwise the header gets messed up and the
...
void MarkerBrowserDialog::deactivated() {
  if (lay::Dispatcher::instance()) {
    lay::Dispatcher::instance()->config_set(
        cfg_rdb_window_state, lay::save_dialog_state(this).c_str());
  }
...
void MarkerBrowserDialog::scan_layer() {
  std::vector<lay::LayerPropertiesConstIterator> layers =
      view()->selected_layers();
  if (layers.empty()) {
    throw tl::Exception(
        tl::to_string(QObject::tr("No layer selected to get shapes from")));
  }
      ...
      if (cv_index < 0) {
        ...
        if (cv_index != (*l)->cellview_index()) {
          throw tl::Exception(tl::to_string(
              QObject::tr("All layers must originate from the same layout")));
        }
      }
    }
  }
  ...
  tl::AbsoluteProgress progress(tl::to_string(QObject::tr("Shapes To Markers")),
                                10000);
  ...
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator l =
           layers.begin();
       l != layers.end(); ++l) {
    if (!(*l)->has_children() && (*l)->cellview_index() >= 0 &&
        layout.is_valid_layer((*l)->layer_index())) {
      if (!desc.empty()) {
        desc += ", ";
      }
  ...
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator l =
    ...
    if (!(*l)->has_children() && (*l)->cellview_index() >= 0 &&
      ...
      rdb::Category *cat = rdb->create_category(
          layout.get_properties((*l)->layer_index()).to_string());

      for (db::Layout::const_iterator cid = layout.begin(); cid != layout.end();
           ++cid) {
        ...
        if (cell.shapes((*l)->layer_index()).size() > 0) {
          ...
          if (!rdb_cell) {
            ...
            std::pair<bool, db::ICplxTrans> ctx = db::find_layout_context(
                layout, cell.cell_index(), cv.cell_index());
            if (ctx.first) {
              db::DCplxTrans t = db::DCplxTrans(layout.dbu()) *
                                 db::DCplxTrans(ctx.second) *
                                 db::DCplxTrans(1.0 / layout.dbu());
              rdb_cell_nc->references().insert(
                  Reference(t, rdb_top_cell->id()));
            }
          }

          for (db::ShapeIterator shape = cell.shapes((*l)->layer_index())
                                             .begin(db::ShapeIterator::All);
               !shape.at_end(); ++shape) {

            rdb::create_item_from_shape(rdb.get(), rdb_cell->id(), cat->id(),
                                        db::CplxTrans(layout.dbu()), *shape);
  ...
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator l =
    ...
    if (!(*l)->has_children() && (*l)->cellview_index() >= 0 &&
      ...
      while (!shape.at_end()) {
        ...
        rdb::create_item_from_shape(rdb.get(), rdb_top_cell->id(), cat->id(),
                                    db::CplxTrans(layout.dbu()) * shape.trans(),
                                    *shape);
