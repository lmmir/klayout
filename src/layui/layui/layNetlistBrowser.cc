...
extern const std::string
    cfg_l2ndb_marker_cycle_colors("l2ndb-marker-cycle-colors");
extern const std::string
    cfg_l2ndb_marker_cycle_colors_enabled("l2ndb-marker-cycle-colors-enabled");
extern const std::string
    cfg_l2ndb_marker_dither_pattern("l2ndb-marker-dither-pattern");
extern const std::string cfg_l2ndb_marker_line_width("l2ndb-marker-line-width");
extern const std::string
    cfg_l2ndb_marker_vertex_size("l2ndb-marker-vertex-size");
...
extern const std::string
    cfg_l2ndb_marker_use_original_colors("l2ndb-marker-use-original-colors");
...
extern const std::string
    cfg_l2ndb_max_shapes_highlighted("l2ndb-max-shapes-highlighted");
...
extern const std::string
    cfg_l2ndb_export_net_cell_prefix("l2ndb-export-net-cell-prefix");
extern const std::string
    cfg_l2ndb_export_net_propname("l2ndb-export-net-propname");
extern const std::string
    cfg_l2ndb_export_start_layer_number("l2ndb-export-start-layer-number");
extern const std::string
    cfg_l2ndb_export_circuit_cell_prefix("l2ndb-export-circuit-cell-prefix");
extern const std::string cfg_l2ndb_export_produce_circuit_cells(
    "l2ndb-export-produce-circuit-cells");
extern const std::string
    cfg_l2ndb_export_device_cell_prefix("l2ndb-export-device-cell-prefix");
extern const std::string
    cfg_l2ndb_export_produce_device_cells("l2ndb-export-produce-device-cells");
...
static struct {
  ...
  const char *string;
} window_modes[] = {{lay::NetlistBrowserConfig::DontChange, "dont-change"},
...
void NetlistBrowserWindowModeConverter::from_string(
  ...
  throw tl::Exception(
      tl::to_string(QObject::tr("Invalid net tracer window mode: ")) + value);
}

std::string NetlistBrowserWindowModeConverter::to_string(
    lay::NetlistBrowserConfig::net_window_type mode) {
  for (unsigned int i = 0; i < sizeof(window_modes) / sizeof(window_modes[0]);
       ++i) {
...
NetlistBrowserConfigPage::NetlistBrowserConfigPage(QWidget *parent)
  ...
  connect(mp_ui->cbx_window, SIGNAL(currentIndexChanged(int)), this,
...
void NetlistBrowserConfigPage::setup(lay::Dispatcher *root) {
  //  window mode
  lay::NetlistBrowserConfig::net_window_type wmode =
      lay::NetlistBrowserConfig::FitNet;
  root->config_get(cfg_l2ndb_window_mode, wmode,
                   NetlistBrowserWindowModeConverter());
  ...
  mp_ui->le_max_markers->setText(
      tl::to_qstring(tl::to_string(max_marker_count)));
...
void NetlistBrowserConfigPage::window_changed(int m) {
  mp_ui->le_window->setEnabled(m == int(lay::NetlistBrowserConfig::FitNet) ||
                               m == int(lay::NetlistBrowserConfig::CenterSize));
}
...
void NetlistBrowserConfigPage::commit(lay::Dispatcher *root) {
  ...
  tl::from_string_ext(tl::to_string(mp_ui->le_max_markers->text()),
                      max_shapes_highlighted);

  root->config_set(cfg_l2ndb_window_mode,
                   lay::NetlistBrowserConfig::net_window_type(
                       mp_ui->cbx_window->currentIndex()),
                   NetlistBrowserWindowModeConverter());
...
static QToolButton *(Ui::NetlistBrowserConfigPage2::*cc_buttons[]) = {
    &Ui::NetlistBrowserConfigPage2::cc0, &Ui::NetlistBrowserConfigPage2::cc1,
    &Ui::NetlistBrowserConfigPage2::cc2, &Ui::NetlistBrowserConfigPage2::cc3,
    &Ui::NetlistBrowserConfigPage2::cc4, &Ui::NetlistBrowserConfigPage2::cc5,
    &Ui::NetlistBrowserConfigPage2::cc6, &Ui::NetlistBrowserConfigPage2::cc7};
...
NetlistBrowserConfigPage2::NetlistBrowserConfigPage2(QWidget *parent)
  ...
  for (unsigned int i = 0; i < sizeof(cc_buttons) / sizeof(cc_buttons[0]);
       ++i) {
    connect(mp_ui->*(cc_buttons[i]), SIGNAL(clicked()), this,
...
void NetlistBrowserConfigPage2::setup(lay::Dispatcher *root) {
  ...
  mp_ui->halo_cb->setCheckState(
      halo < 0 ? Qt::PartiallyChecked : (halo ? Qt::Checked : Qt::Unchecked));
}
...
void NetlistBrowserConfigPage2::commit(lay::Dispatcher *root) {
  root->config_set(cfg_l2ndb_marker_cycle_colors_enabled,
                   mp_ui->cycle_colors_cb->isChecked());
  ...
  root->config_set(cfg_l2ndb_marker_dither_pattern,
                   mp_ui->stipple_pb->dither_pattern());
  ...
  root->config_set(cfg_l2ndb_marker_use_original_colors,
                   mp_ui->brightness_cb->isChecked());
}

// ------------------------------------------------------------
//  Declaration and implementation of the browser plugin declaration object

class NetlistBrowserPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_options(std::vector<std::pair<std::string, std::string>> &options) const {
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_window_mode, "fit-net"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_window_dim, "1.0"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_max_shapes_highlighted, "10000"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_color, lay::ColorConverter().to_string(QColor())));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_cycle_colors_enabled, "false"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_cycle_colors,
        "255,0,0 0,255,0 0,0,255 255,255,0 255,0,255 0,255,255 160,80,255 "
        "255,160,0"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_marker_line_width, "-1"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_vertex_size, "-1"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_marker_halo, "-1"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_use_original_colors, "false"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_marker_dither_pattern, "-1"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_marker_intensity, "50"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_show_all, "true"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_window_state, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_l2ndb_export_net_propname, ""));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_net_cell_prefix, "NET_"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_start_layer_number, "1000"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_produce_circuit_cells, "false"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_circuit_cell_prefix, "CIRCUIT_"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_produce_device_cells, "false"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_l2ndb_export_device_cell_prefix, "DEVICE_"));
  }

  virtual std::vector<std::pair<std::string, lay::ConfigPage *>>
  config_pages(QWidget *parent) const {
    std::vector<std::pair<std::string, lay::ConfigPage *>> pages;
    pages.push_back(
        std::make_pair(tl::to_string(QObject::tr("Netlist Browser|Setup")),
                       new NetlistBrowserConfigPage(parent)));
    pages.push_back(std::make_pair(
        tl::to_string(QObject::tr("Netlist Browser|Net Appearance")),
        new NetlistBrowserConfigPage2(parent)));
    return pages;
  }
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::menu_item(
        "netlist_browser::show", "browse_netlists", "tools_menu.end",
        tl::to_string(QObject::tr("Netlist Browser"))));
  }
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new NetlistBrowserPluginDeclaration(), 12100,
