...
class BookmarkListModel : public QAbstractItemModel {
public:
  ...
  QVariant data(const QModelIndex &index, int role) const {
    if (role == Qt::DisplayRole && index.row() >= 0 &&
        index.row() < int(mp_bookmarks->size())) {
...
BookmarksView::BookmarksView(LayoutViewBase *view, QWidget *parent,
                             const char *name)
  ...
  connect(mp_bookmarks, SIGNAL(customContextMenuRequested(const QPoint &)),
          this, SLOT(context_menu(const QPoint &)));
  connect(mp_bookmarks, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(bookmark_triggered(const QModelIndex &)));
  connect(mp_bookmarks->selectionModel(),
          SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
          this, SLOT(current_bookmark_changed(const QModelIndex &)));
}
...
std::set<size_t> BookmarksView::selected_bookmarks() {
...
void BookmarksView::follow_selection(bool f) { m_follow_selection = f; }
...
void BookmarksView::refresh() {
  BookmarkListModel *model =
      dynamic_cast<BookmarkListModel *>(mp_bookmarks->model());
  if (model) {
    model->refresh();
  }
}
...
class BookmarksViewPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::submenu("@bookmarks_context_menu", at, std::string()));

    at = "@bookmarks_context_menu.end";
    menu_entries.push_back(lay::config_menu_item(
        "follow_selection", at, tl::to_string(QObject::tr("Follow Selection")),
        cfg_bookmarks_follow_selection, "?")),
        menu_entries.push_back(lay::separator("ops_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_manage_bookmarks", "manage_bookmarks", at,
                       tl::to_string(QObject::tr("Manage Bookmarks"))));
    menu_entries.push_back(
        lay::menu_item("cm_load_bookmarks", "load_bookmarks", at,
                       tl::to_string(QObject::tr("Load Bookmarks"))));
    menu_entries.push_back(
        lay::menu_item("cm_save_bookmarks", "save_bookmarks", at,
                       tl::to_string(QObject::tr("Save Bookmarks"))));
  }
};

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new BookmarksViewPluginDeclaration(), -7,
                "BookmarksViewPlugin");
