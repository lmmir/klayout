...
struct cmp_cell_tree_items_f {
  cmp_cell_tree_items_f(CellTreeModel::Sorting s) : m_sorting(s) {}
...
CellTreeItem::CellTreeItem(const db::Layout *layout, bool is_pcell,
                           unsigned int cell_or_pcell_index, bool flat,
                           CellTreeModel::Sorting s)
    : mp_layout(layout), mp_parent(0), m_sorting(s), m_is_pcell(is_pcell),
      m_index(0), m_tree_index(0), m_children(),
      m_cell_or_pcell_index(cell_or_pcell_index) {
  if (!flat && !is_pcell) {
    m_child_count =
        int(mp_layout->cell(db::cell_index_type(cell_or_pcell_index))
                .child_cells());
  } else {
    m_child_count = 0;
  }
...
CellTreeItem::~CellTreeItem() {
  for (std::vector<CellTreeItem *>::iterator c = m_children.begin();
       c != m_children.end(); ++c) {
    delete *c;
  }
  m_children.clear();
...
size_t CellTreeItem::assign_serial(size_t index,
                                   std::map<CellTreeItem *, size_t> &serial) {
...
bool CellTreeItem::is_valid() const {
  return m_is_pcell || mp_layout->is_valid_cell_index(cell_or_pcell_index());
}

std::string CellTreeItem::display_text() const {
  if (m_is_pcell) {
    return name();
...
int CellTreeItem::children() const { return m_child_count; }

int CellTreeItem::children_in(const std::set<const CellTreeItem *> &sel) const {
  int count = 0;
  for (std::vector<CellTreeItem *>::const_iterator c = m_children.begin();
       c != m_children.end(); ++c) {
...
void CellTreeItem::ensure_children() {
  if (!m_is_pcell && m_children.empty()) {
    ...
    for (db::Cell::child_cell_iterator child = cell->begin_child_cells();
         !child.at_end(); ++child) {
...
CellTreeItem *CellTreeItem::child(int index) {
  ensure_children();
  return m_children[index];
}

CellTreeItem *CellTreeItem::child_in(const std::set<const CellTreeItem *> &sel,
                                     int index) {
  ensure_children();
...
void CellTreeItem::add_child(CellTreeItem *item) {
  //  explicitly added
...
void CellTreeItem::finish_children() {
  std::sort(m_children.begin(), m_children.end(),
            cmp_cell_tree_items_f(m_sorting));
...
db::cell_index_type CellTreeItem::cell_or_pcell_index() const {
  return db::cell_index_type(m_cell_or_pcell_index);
}

CellTreeItem *CellTreeItem::parent() const { return mp_parent; }

const char *CellTreeItem::name() const {
  if (!m_is_pcell) {
...
bool CellTreeItem::by_name_less_than(const CellTreeItem *b) const {
#if 0 // with name:
...
bool CellTreeItem::name_less_than(const char *n) const {
#if 0 // with name:
...
bool CellTreeItem::name_equals(const char *n) const {
#if 0 // with name:
  return strcmp (name (), n) == 0;
#else // with display text:
  return display_text() == n;
...
bool CellTreeItem::name_matches(const tl::GlobPattern &p) const {
#if 0 // with name:
  return p.match (name ());
#else // with display text:
...
bool CellTreeItem::by_area_less_than(const CellTreeItem *b) const {
  if (m_is_pcell || b->is_pcell()) {
    return m_is_pcell > b->is_pcell();
  }
  // Hint: since mp_layout == b.mp_layout, not conversion to um^2 is required
  // because of different DBU
  return mp_layout->cell(cell_or_pcell_index()).bbox().area() <
         b->mp_layout->cell(b->cell_or_pcell_index()).bbox().area();
}

bool CellTreeItem::by_area_equal_than(const CellTreeItem *b) const {
  if (m_is_pcell != b->is_pcell()) {
    return false;
  }
  // Hint: since mp_layout == b.mp_layout, not conversion to um^2 is required
...
//  Hint: it may happen that the cell tree model gets engaged while the layout
//  is not valid ("under construction"). In this case, the model will return
//  defaults or void objects.

CellTreeModel::CellTreeModel(QWidget *parent, lay::LayoutViewBase *view,
                             int cv_index, unsigned int flags,
                             const db::Cell *base, Sorting sorting)
    : QAbstractItemModel(parent), m_flags(flags), m_sorting(sorting),
      mp_parent(parent), mp_view(view), m_cv_index(cv_index), mp_base(base) {
  mp_view->cell_visibility_changed_event.add(
      this, &CellTreeModel::signal_data_changed);
  mp_view->cellview_changed_event.add(
      this, &CellTreeModel::signal_data_changed_with_int);
  ...
  mp_library = 0;
  tl_assert(!mp_layout->under_construction() &&
            !(mp_layout->manager() && mp_layout->manager()->transacting()));
...
CellTreeModel::CellTreeModel(QWidget *parent, db::Layout *layout,
                             unsigned int flags, const db::Cell *base,
                             Sorting sorting)
    : QAbstractItemModel(parent), m_flags(flags), m_sorting(sorting),
      mp_parent(parent), mp_view(0), m_cv_index(-1), mp_base(base) {
  m_flat = ((flags & Flat) != 0) && ((flags & TopCells) == 0);
  m_pad = ((flags & NoPadding) == 0);
  m_filter_mode = false;
...
CellTreeModel::CellTreeModel(QWidget *parent, db::Library *library,
                             unsigned int flags, const db::Cell *base,
                             Sorting sorting)
    : QAbstractItemModel(parent), m_flags(flags), m_sorting(sorting),
      mp_parent(parent), mp_view(0), m_cv_index(-1), mp_base(base) {
...
CellTreeModel::~CellTreeModel() { clear_top_level(); }

void CellTreeModel::configure(lay::LayoutViewBase *view, int cv_index,
                              unsigned int flags, const db::Cell *base,
                              Sorting sorting) {
...
void CellTreeModel::configure(db::Layout *layout, unsigned int flags,
                              const db::Cell *base, Sorting sorting) {
  do_configure(layout, 0, 0, -1, flags, base, sorting);
}

void CellTreeModel::configure(db::Library *library, unsigned int flags,
                              const db::Cell *base, Sorting sorting) {
  do_configure(&library->layout(), library, 0, -1, flags, base, sorting);
}

void CellTreeModel::do_configure(db::Layout *layout, db::Library *library,
                                 lay::LayoutViewBase *view, int cv_index,
                                 unsigned int flags, const db::Cell *base,
                                 Sorting sorting) {
  ...
  if (view != mp_view) {
    ...
    if (mp_view) {
      mp_view->cell_visibility_changed_event.remove(
          this, &CellTreeModel::signal_data_changed);
      mp_view->cellview_changed_event.remove(
          this, &CellTreeModel::signal_data_changed_with_int);
    }
  ...
  if (need_reset) {
    ...
    //  Translate persistent indexes: translation happens according to the path
    //  given by a sequence of cell indexes.

    QModelIndexList indexes = persistentIndexList();
    QModelIndexList new_indexes;

    for (QModelIndexList::const_iterator index = indexes.begin();
      ...
      while (item) {
        path.push_back(
            std::make_pair(item->is_pcell(), item->cell_or_pcell_index()));
        item = item->parent();
      }
      ...
      if (!path.empty()) {
        ...
        for (std::vector<std::pair<bool, db::cell_index_type>>::const_iterator
                 ci = path.begin();
             ci != path.end(); ++ci) {

          CellTreeItem *new_parent = 0;

          if ((!ci->first && !layout->is_valid_cell_index(ci->second)) ||
              (ci->first && !layout->pcell_declaration(ci->second))) {
            //  can't translate this index
          } else if (parent == 0) {
            for (int i = 0; i < int(m_toplevel.size()) && !new_parent; ++i) {
              if (m_toplevel[i]->cell_or_pcell_index() == ci->second &&
                  m_toplevel[i]->is_pcell() == ci->first) {
            ...
            for (int i = 0; i < parent->children() && !new_parent; ++i) {
              if (parent->child(i)->cell_or_pcell_index() == ci->second &&
                  parent->child(i)->is_pcell() == ci->first) {
  ...
  for (std::vector<lay::CellTreeItem *>::iterator t =
           old_toplevel_items.begin();
       t != old_toplevel_items.end(); ++t) {
    delete *t;
  }
}

void CellTreeModel::set_filter_mode(bool f) {
  if (f != m_filter_mode) {
    m_filter_mode = f;
    signal_data_changed();
  }
...
void CellTreeModel::set_sorting(Sorting s) {
  if (s != m_sorting) {
    do_configure(mp_layout, mp_library, mp_view, m_cv_index, m_flags, mp_base,
                 s);
  }
}

void CellTreeModel::signal_data_changed() {
  emit layoutAboutToBeChanged();
  emit layoutChanged();
}

void CellTreeModel::clear_top_level() {
  for (std::vector<CellTreeItem *>::iterator c = m_toplevel.begin();
       c != m_toplevel.end(); ++c) {
    delete *c;
  }
  m_toplevel.clear();
...
void CellTreeModel::build_top_level() {
  if ((m_flags & Children) != 0) {
    ...
    if (mp_base) {
      ...
      for (db::Cell::child_cell_iterator child = mp_base->begin_child_cells();
           !child.at_end(); ++child) {
        CellTreeItem *item =
            new CellTreeItem(mp_layout, false, *child, true, m_sorting);
    ...
    if (mp_base) {
      ...
      for (db::Cell::parent_cell_iterator parent =
               mp_base->begin_parent_cells();
           parent != mp_base->end_parent_cells(); ++parent) {
        CellTreeItem *item =
            new CellTreeItem(mp_layout, false, *parent, true, m_sorting);
        m_toplevel.push_back(item);
      }
    }
    ...
    while (top != mp_layout->end_top_down()) {
      ...
      if (m_flat) {
        if ((m_flags & BasicCells) == 0 || !mp_layout->cell(*top).is_proxy()) {
          CellTreeItem *item =
              new CellTreeItem(mp_layout, false, *top, true, m_sorting);
          m_toplevel.push_back(item);
        }
      } else if (mp_layout->cell(*top).is_top()) {
        if ((m_flags & BasicCells) == 0 || !mp_layout->cell(*top).is_proxy()) {
          CellTreeItem *item = new CellTreeItem(
              mp_layout, false, *top, (m_flags & TopCells) != 0, m_sorting);
          m_toplevel.push_back(item);
        }
      } else {
        break;
    ...
    if ((m_flags & BasicCells) != 0) {
      ...
      for (db::Layout::pcell_iterator pc = mp_layout->begin_pcells();
           pc != mp_layout->end_pcells(); ++pc) {

        CellTreeItem *item =
            new CellTreeItem(mp_layout, true, pc->second, true, m_sorting);
        m_toplevel.push_back(item);

        if ((m_flags & WithVariants) != 0) {

          const db::PCellHeader *pcell_header =
              mp_layout->pcell_header(pc->second);
          for (db::PCellHeader::variant_iterator v = pcell_header->begin();
               v != pcell_header->end(); ++v) {
            if (mp_library && mp_library->is_retired(v->second->cell_index())) {
              //  skip retired cells - this means we won't show variants which
              //  are just kept as shadow variants for the transactions.
            } else {
              item->add_child(new CellTreeItem(
                  mp_layout, false, v->second->cell_index(), true, m_sorting));
            }
          }
  ...
  std::sort(m_toplevel.begin(), m_toplevel.end(),
            cmp_cell_tree_items_f(m_sorting));
...
Qt::ItemFlags CellTreeModel::flags(const QModelIndex &index) const {
  return Qt::ItemIsDragEnabled | QAbstractItemModel::flags(index);
}

QStringList CellTreeModel::mimeTypes() const {
  QStringList types;
  types << QString::fromUtf8(lay::drag_drop_mime_type());
  return types;
}

QMimeData *CellTreeModel::mimeData(const QModelIndexList &indexes) const {
  for (QModelIndexList::const_iterator i = indexes.begin(); i != indexes.end();
       ++i) {
    ...
    if (is_pcell(*i)) {
      ...
      if (c) {
        ...
        while (layout != 0 &&
               (lib_proxy = dynamic_cast<const db::LibraryProxy *>(c)) != 0) {
        ...
        const db::PCellVariant *pcell_var =
            dynamic_cast<const db::PCellVariant *>(c);
        if (pcell_var) {
          lay::CellDragDropData data(layout, library, pcell_var->pcell_id(),
                                     true, pcell_var->parameters());
          return data.to_mime_data();
        } else if (c) {
...
int CellTreeModel::columnCount(const QModelIndex &) const { return 1; }

QVariant CellTreeModel::data(const QModelIndex &index, int role) const {
  CellTreeItem *item = (CellTreeItem *)index.internalPointer();
  if (!item || mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return QVariant();
  }
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    if (!mp_view) {
      ...
      const lay::CellView::unspecific_cell_path_type &path =
          mp_view->cellview(m_cv_index).unspecific_path();
      const lay::CellView::specific_cell_path_type &ctx_path =
          mp_view->cellview(m_cv_index).specific_path();
      ...
      if (!path.empty()) {
        if (item->cell_or_pcell_index() == path.back()) {
          if (m_flat) {
            ...
            lay::CellView::unspecific_cell_path_type::const_iterator p =
                path.end();
            ...
            if (!it && p == path.begin()) {
              f.setBold(true);
                   ...
                   item->cell_or_pcell_index() ==
                       ctx_path.back().inst_ptr.cell_index()) {
          if (m_flat) {
            f.setUnderline(true);
          } else {
            CellTreeItem *it = item;
            lay::CellView::specific_cell_path_type::const_iterator cp =
                ctx_path.end();
    ...
    if (m_selected_indexes_set.find(index.internalPointer()) !=
        m_selected_indexes_set.end()) {
      ...
      return QVariant(QColor((c1.red() + cb.red()) / 2,
                             (c1.green() + cb.green()) / 2,
                             (c1.blue() + cb.blue()) / 2));
    } else {
      return QVariant();
...
QVariant CellTreeModel::headerData(int /*section*/,
...
int CellTreeModel::rowCount(const QModelIndex &parent) const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return 0;
    ...
    if (m_filter_mode && m_is_filtered) {
      int n = 0;
      for (std::vector<CellTreeItem *>::const_iterator i = m_toplevel.begin();
           i != m_toplevel.end(); ++i) {
...
int CellTreeModel::toplevel_items() const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return 0;
  } else {
...
CellTreeItem *CellTreeModel::toplevel_item(int index) {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return 0;
  } else {
...
QModelIndex CellTreeModel::model_index(CellTreeItem *item) const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return QModelIndex();
  } else if (m_filter_mode && m_is_filtered) {
...
bool CellTreeModel::is_pcell(const QModelIndex &index) const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return false;
  } else {
...
db::pcell_id_type CellTreeModel::pcell_id(const QModelIndex &index) const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return 0;
  } else {
...
db::cell_index_type CellTreeModel::cell_index(const QModelIndex &index) const {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return 0;
  } else {
...
const db::Cell *CellTreeModel::cell(const QModelIndex &index) const {
  if (index.isValid() && !mp_layout->under_construction() &&
      !(mp_layout->manager() && mp_layout->manager()->transacting())) {
    CellTreeItem *item = (CellTreeItem *)index.internalPointer();
...
const char *CellTreeModel::cell_name(const QModelIndex &index) const {
  if (index.isValid() && !mp_layout->under_construction() &&
      !(mp_layout->manager() && mp_layout->manager()->transacting())) {
    CellTreeItem *item = (CellTreeItem *)index.internalPointer();
    if (item->is_pcell()) {
      return mp_layout->pcell_header(item->cell_or_pcell_index())
          ->get_name()
          .c_str();
    } else {
...
void CellTreeModel::clear_locate() {
  m_selected_indexes.clear();
  ...
  if (m_filter_mode) {
    ...
    QModelIndexList new_indexes;
    for (QModelIndexList::iterator i = indexes.begin(); i != indexes.end();
         ++i) {
...
QModelIndex CellTreeModel::locate_next(const QModelIndex &index) {
  ...
  for (std::vector<QModelIndex>::const_iterator i = m_selected_indexes.begin();
       i != m_selected_indexes.end(); ++i) {
...
QModelIndex CellTreeModel::locate_next() {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return QModelIndex();
  }
...
QModelIndex CellTreeModel::locate_prev() {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return QModelIndex();
  }
...
bool CellTreeModel::search_children(const tl::GlobPattern &pattern,
                                    CellTreeItem *item) {
  bool any = false;
  size_t ti = 0;
...
QModelIndex CellTreeModel::locate(const char *name, bool glob_pattern,
                                  bool case_sensitive, bool top_only) {
  if (mp_layout->under_construction() ||
      (mp_layout->manager() && mp_layout->manager()->transacting())) {
    return QModelIndex();
  }
  ...
  for (std::vector<CellTreeItem *>::const_iterator lc = m_toplevel.begin();
       lc != m_toplevel.end(); ++lc) {
  ...
  if (m_filter_mode) {
    ...
    for (std::vector<CellTreeItem *>::const_iterator item =
             persistent_index_cells.begin();
         item != persistent_index_cells.end(); ++item) {
