 ...
 *
 *  The outline is a hierarchical tree of items. Each node has a title, a URL to
 * navigate to and optional child items.
 */
...
public:
   ...
   *  @brief Sets the URL
   */
  void set_url(const std::string &u) { m_url = u; }

  /**
   *  @brief Returns the begin iterator for the children
   */
  const_child_iterator begin() const { return m_children.begin(); }

  /**
   *  @brief Returns the end iterator for the children
   */
  const_child_iterator end() const { return m_children.end(); }

  /**
   *  @brief Returns the non-const begin iterator for the children
   */
  child_iterator begin() { return m_children.begin(); }

  /**
   *  @brief Returns the non-const end iterator for the children
   */
  child_iterator end() { return m_children.end(); }

  /**
   *  @brief Adds a child entry at the end of the list
   */
  void add_child(const BrowserOutline &ol) { m_children.push_back(ol); }
 ...
 */
class LAYUI_PUBLIC BrowserSource : public gsi::ObjectBase, public tl::Object {
public:
  /**
   ...
   *
   *  The default HTML string is sent when no specific implementation is
   * provided.
   */
   ...
   *  a new location. This allows implementing any functionality behind such
   * links.
   */
   ...
   */
  virtual void search_completers(const std::string &search_string,
...
/**
 *  @brief A specialization of QTextBrowser that allows loading a specific
 * resource through BrowserPanel
 */
class LAYUI_PUBLIC BrowserTextWidget : public QTextBrowser {
public:
  BrowserTextWidget(QWidget *w) : QTextBrowser(w), mp_panel(0) {
    //  .. nothing yet ..
  }

  void set_panel(BrowserPanel *panel) { mp_panel = panel; }
...
/**
 *  @brief A specialization of QWidget around a TextBrowser that allows loading
 * a specific resource
 */
class LAYUI_PUBLIC BrowserPanel : public QWidget {
...
public:
   ...
   *
   *  @param dispatcher If given, this interface will be used to retrieve and
   * store the bookmark list
   *  @param cfg_bookmarks If dispatcher is given, this will be the
   * configuration key to store the bookmarks
   */
  void set_dispatcher(lay::Dispatcher *dispatcher,
   ...
   *
   *  Set the browser to the given URL. This will also be the URL that is
   * navigated to when the "home" button is clicked.
   */
  ...
  /**
   *  @brief Enables the search bx and sets the Url and query item name for the
   * search
   */
