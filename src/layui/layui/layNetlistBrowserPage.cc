...
template <class Obj> inline const db::Circuit *deref_circuit(const Obj *obj) {
  return obj->circuit();
}

template <> inline const db::Circuit *deref_circuit(const db::Circuit *obj) {
  return obj;
}

template <class Obj>
static std::pair<bool, db::DCplxTrans>
trans_for(const Obj *objs, const db::Layout &ly, const db::Cell &cell,
          db::ContextCache &cc,
          const db::DCplxTrans &initial = db::DCplxTrans()) {
  ...
  if (circuit && ly.is_valid_cell_index(circuit->cell_index())) {
    std::pair<bool, db::ICplxTrans> tc =
        cc.find_layout_context(circuit->cell_index(), cell.cell_index());
    if (tc.first) {
...
NetlistBrowserPage::NetlistBrowserPage(QWidget * /*parent*/)
    : m_show_all(true), m_window(lay::NetlistBrowserConfig::FitNet),
      m_window_dim(0.0), m_max_shape_count(1000), m_marker_line_width(-1),
      m_marker_vertex_size(-1), m_marker_halo(-1), m_marker_dither_pattern(-1),
      m_marker_intensity(0), m_use_original_colors(false), mp_view(0),
      m_cv_index(0), mp_plugin_root(0), mp_last_db(0), m_history_ptr(0),
      m_signals_enabled(true), m_enable_updates(true), m_update_needed(true),
      mp_info_dialog(0),
  ...
  QTreeView *dt[] = {nl_directory_tree, sch_directory_tree,
                     xref_directory_tree};
  ...
  for (int m = 0; m < int(sizeof(dt) / sizeof(dt[0])); ++m) {
    ...
    QAction *color_action =
        new QAction(QObject::tr("Colorize Nets"), directory_tree);
    QMenu *menu = new QMenu(directory_tree);
    lay::ColorButton::build_color_menu(menu, this, SLOT(browse_color_for_net()),
                                       SLOT(select_color_for_net()));
    color_action->setMenu(menu);

    QAction *sep;
    directory_tree->addAction(m_show_all_action);
    QAction *collapse_all =
        new QAction(QObject::tr("Collapse All"), directory_tree);
    connect(collapse_all, SIGNAL(triggered()), directory_tree,
            SLOT(collapseAll()));
    directory_tree->addAction(collapse_all);
    //  TODO: this gives a too big tree - confine to single branches?
    //  QAction *expand_all = new QAction (QObject::tr ("Expand All"),
    //  directory_tree); connect (expand_all, SIGNAL (triggered ()),
    //  directory_tree, SLOT (expandAll ())); directory_tree->addAction
    //  (actionExpandAll);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      connect(delegate, SIGNAL(anchor_clicked(const QString &)), this,
              SLOT(anchor_clicked(const QString &)));
  ...
  QTreeView *ht[] = {nl_hierarchy_tree, sch_hierarchy_tree,
                     xref_hierarchy_tree};
  ...
  connect(actionExportSelected, SIGNAL(triggered()), this,
          SLOT(export_selected()));

  forward->setEnabled(false);
  backward->setEnabled(false);
}

NetlistBrowserPage::~NetlistBrowserPage() { clear_markers(); }
...
void NetlistBrowserPage::set_highlight_style(
    tl::Color color, int line_width, int vertex_size, int halo,
    int dither_pattern, int marker_intensity, bool use_original_colors,
...
void NetlistBrowserPage::set_view(lay::LayoutViewBase *view, int cv_index) {
  if (mp_view) {
    mp_view->layer_list_changed_event.remove(
        this, &NetlistBrowserPage::layer_list_changed);
  }
  ...
  if (mp_view) {
    mp_view->layer_list_changed_event.add(
        this, &NetlistBrowserPage::layer_list_changed);
  }
...
void NetlistBrowserPage::set_window(
    lay::NetlistBrowserConfig::net_window_type window, double window_dim) {
...
bool NetlistBrowserPage::eventFilter(QObject *watched, QEvent *event) {
  ...
  if (tree != nl_directory_tree && tree != sch_directory_tree &&
      tree != xref_directory_tree) {
    return false;
  }
...
void NetlistBrowserPage::layer_list_changed(int) { dm_update_highlights(); }

QTreeView *NetlistBrowserPage::current_hierarchy_tree() {
...
QTreeView *NetlistBrowserPage::current_directory_tree() {
  switch (mode_tab->currentIndex()) {
  case 0:
...
void NetlistBrowserPage::anchor_clicked(const QString &a) {
  ...
  if (directory_tree) {
    netlist_model =
        dynamic_cast<NetlistBrowserModel *>(directory_tree->model());
  }
  if (netlist_model) {
...
void NetlistBrowserPage::nl_current_tree_index_changed(
    const QModelIndex &index) {
  current_tree_index_changed(nl_hierarchy_tree, nl_directory_tree, index);
}

void NetlistBrowserPage::sch_current_tree_index_changed(
    const QModelIndex &index) {
  current_tree_index_changed(sch_hierarchy_tree, sch_directory_tree, index);
}

void NetlistBrowserPage::xref_current_tree_index_changed(
    const QModelIndex &index) {
  current_tree_index_changed(xref_hierarchy_tree, xref_directory_tree, index);
}

void NetlistBrowserPage::current_tree_index_changed(QTreeView *hierarchy_tree,
                                                    QTreeView *directory_tree,
                                                    const QModelIndex &index) {
  if (index.isValid() && m_signals_enabled) {

    NetlistBrowserTreeModel *tree_model =
        dynamic_cast<NetlistBrowserTreeModel *>(hierarchy_tree->model());
    NetlistBrowserModel *netlist_model =
        dynamic_cast<NetlistBrowserModel *>(directory_tree->model());
    ...
    std::pair<const db::Circuit *, const db::Circuit *> circuits =
        tree_model->circuits_from_index(index);
...
void NetlistBrowserPage::current_index_changed(QTreeView *hierarchy_tree,
                                               QTreeView *directory_tree,
                                               const QModelIndex &index) {
  if (index.isValid() && m_signals_enabled) {
...
std::vector<const db::Net *> NetlistBrowserPage::selected_nets() {
  ...
  for (QModelIndexList::const_iterator i = selection.begin();
       i != selection.end(); ++i) {
    if (i->column() == 0) {
...
std::vector<const db::Circuit *> NetlistBrowserPage::selected_circuits() {
...
std::vector<const db::SubCircuit *> NetlistBrowserPage::selected_subcircuits() {
...
std::vector<const db::Device *> NetlistBrowserPage::selected_devices() {
...
void NetlistBrowserPage::sch_selection_changed() {
  ...
  for (QModelIndexList::const_iterator i = selected.begin();
    ...
    if (i->column() == 0) {
      ...
      if (!lay::NetlistObjectsPath::translate(selected_paths.back(),
                                              *lvsdb->cross_ref())) {
...
void NetlistBrowserPage::selection_changed(QTreeView * /*hierarchy_tree*/,
                                           QTreeView *directory_tree) {
  NetlistBrowserModel *model =
      dynamic_cast<NetlistBrowserModel *>(directory_tree->model());
  tl_assert(model != 0);
...
void NetlistBrowserPage::select_color_for_net() {
  ...
  if (action) {
    set_color_for_selected_nets(
        tl::Color(action->data().value<QColor>().rgb()));
  }
}
...
static QModelIndex find_next(QTreeView *view, QAbstractItemModel *model,
                             const QRegExp &to_find, const QModelIndex &from) {
  ...
  while (index.isValid()) {
    ...
    rows_stack.push_back(
        std::make_pair(index.row(), model->rowCount(parent_stack.back())));
  ...
  do {
    ...
    if (has_next) {
      ...
      if (to_find.indexIn(text) >= 0 &&
          !view->isRowHidden(rows_stack.back().first, parent_stack.back())) {
        return current;
      }
    }
...
void NetlistBrowserPage::find_button_pressed() {
  ...
  QModelIndex next = find_next(directory_tree, directory_tree->model(), re,
                               directory_tree->currentIndex());
  if (next.isValid()) {
    navigate_to(next);
  }
}
...
void NetlistBrowserPage::show_all_clicked() {
  if (mp_plugin_root) {
    mp_plugin_root->config_set(cfg_l2ndb_show_all,
...
void NetlistBrowserPage::show_all(bool f) {
  if (f != m_show_all) {
    ...
    NetlistBrowserModel *model =
        dynamic_cast<NetlistBrowserModel *>(xref_directory_tree->model());
    if (model) {
      model->set_item_visibility(xref_directory_tree, m_show_all,
                                 false /*show warnings only with 'show all'*/);
    }
  }
}

bool NetlistBrowserPage::set_db(db::LayoutToNetlist *l2ndb) {
  //  NOTE: mp_last_db mirrors mp_database, but does not automatically fall back
  //  to 0 when the DB is deleted. This way we can call set_db(0) with the
  //  correct behavior after the DB has been destroyed.
  if (l2ndb == mp_last_db) {
    //  not change
    return false;
  }
  ...
  rerun_button->setEnabled(mp_database.get() &&
                           !mp_database->generator().empty());
  ...
  m_cell_context_cache =
      db::ContextCache(mp_database.get() ? mp_database->internal_layout() : 0);
...
static void set_abstract_tree_model(QTreeView *view,
                                    QAbstractItemModel *new_model) {
  ...
  if (columns < new_columns) {
    ...
    for (int i = std::max(0, columns - 1); i < new_columns; ++i) {
      view->header()->resizeSection(
          i, i == 1 ? view->header()->minimumSectionSize()
                    : view->header()->defaultSectionSize());
    }
  }
}

static void set_tree_model(QTreeView *view, NetlistBrowserModel *new_model) {
...
static void set_tree_model(QTreeView *view,
                           NetlistBrowserTreeModel *new_model) {
  set_abstract_tree_model(view, new_model);
...
void NetlistBrowserPage::setup_trees() {
  ...
  if (lvsdb && lvsdb->cross_ref()) {
    ...
    NetlistLogModel *new_model =
        new NetlistLogModel(log_view, lvsdb->cross_ref());
    delete log_view->model();
  ...
  {
    //  NOTE: with the tree as the parent, the tree will take over ownership of
    //  the model
    NetlistBrowserModel *new_model =
        new NetlistBrowserModel(nl_directory_tree, l2ndb, &m_colorizer);

    set_tree_model(nl_directory_tree, new_model);

    connect(nl_directory_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(nl_current_index_changed(const QModelIndex &)));
    connect(nl_directory_tree->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &,
                                    const QItemSelection &)),
            this, SLOT(nl_selection_changed()));

    //  establish visibility according to "show all"
    new_model->set_item_visibility(
        nl_directory_tree, m_show_all,
        false /*show warnings only with 'show all'*/);
  }
  ...
  if (lvsdb) {
    ...
    //  NOTE: with the tree as the parent, the tree will take over ownership of
    //  the model
    NetlistBrowserModel *new_model = new NetlistBrowserModel(
        sch_directory_tree, lvsdb->reference_netlist(), &m_colorizer);

    set_tree_model(sch_directory_tree, new_model);

    connect(sch_directory_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(sch_current_index_changed(const QModelIndex &)));
    connect(sch_directory_tree->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &,
                                    const QItemSelection &)),
            this, SLOT(sch_selection_changed()));

    //  establish visibility according to "show all"
    new_model->set_item_visibility(
        sch_directory_tree, m_show_all,
        false /*show warnings only with 'show all'*/);
  ...
  if (lvsdb) {
    ...
    //  the model
    NetlistBrowserModel *new_model =
        new NetlistBrowserModel(xref_directory_tree, lvsdb, &m_colorizer);

    set_tree_model(xref_directory_tree, new_model);

    connect(xref_directory_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(xref_current_index_changed(const QModelIndex &)));
    connect(xref_directory_tree->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &,
                                    const QItemSelection &)),
            this, SLOT(xref_selection_changed()));

    //  establish visibility according to "show all"
    new_model->set_item_visibility(
        xref_directory_tree, m_show_all,
        false /*show warnings only with 'show all'*/);
...

  {
    //  NOTE: with the tree as the parent, the tree will take over ownership of
    //  the model
    NetlistBrowserTreeModel *new_hierarchy_model =
        new NetlistBrowserTreeModel(nl_hierarchy_tree, l2ndb);
    set_tree_model(nl_hierarchy_tree, new_hierarchy_model);

    connect(nl_hierarchy_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(nl_current_tree_index_changed(const QModelIndex &)));
  }
  ...
  if (lvsdb) {
    ...
    //  NOTE: with the tree as the parent, the tree will take over ownership of
    //  the model
    NetlistBrowserTreeModel *new_hierarchy_model = new NetlistBrowserTreeModel(
        sch_hierarchy_tree, lvsdb->reference_netlist());
    set_tree_model(sch_hierarchy_tree, new_hierarchy_model);

    connect(sch_hierarchy_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(sch_current_tree_index_changed(const QModelIndex &)));
  ...
  if (lvsdb) {
    ...
    //  the model
    NetlistBrowserTreeModel *new_hierarchy_model =
        new NetlistBrowserTreeModel(xref_hierarchy_tree, lvsdb);
    set_tree_model(xref_hierarchy_tree, new_hierarchy_model);

    connect(xref_hierarchy_tree->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this, SLOT(xref_current_tree_index_changed(const QModelIndex &)));
...
void NetlistBrowserPage::highlight(
    const NetlistObjectsPath &current_path,
    const std::vector<NetlistObjectsPath> &selected_paths) {
...
static db::Box
bbox_for_device_abstract(const db::Layout *layout,
                         const db::DeviceAbstract *device_abstract,
                         const db::DCplxTrans &trans) {
  if (!device_abstract ||
      !layout->is_valid_cell_index(device_abstract->cell_index())) {
    return db::Box();
  }

  return layout->cell(device_abstract->cell_index())
      .bbox()
      .transformed(db::CplxTrans(layout->dbu()).inverted() * trans *
                   db::CplxTrans(layout->dbu()));
}

static db::Box bbox_for_circuit(const db::Layout *layout,
                                const db::Circuit *circuit) {
...
void NetlistBrowserPage::adjust_view() {
  ...
  if (m_window != lay::NetlistBrowserConfig::FitNet &&
      m_window != lay::NetlistBrowserConfig::Center &&
      m_window != lay::NetlistBrowserConfig::CenterSize) {
    return;
  }

  const db::Layout &original_layout = cv->layout();
  const db::Circuit *top_circuit = mp_database->netlist()->circuit_by_name(
      original_layout.cell_name(cv.cell_index()));
  if (!top_circuit) {
    return;
  }

  const db::Layout *layout = mp_database->internal_layout();
  const db::Cell *cell = layout->is_valid_cell_index(top_circuit->cell_index())
                             ? &layout->cell(top_circuit->cell_index())
                             : mp_database->internal_top_cell();
  if (!layout || !cell) {
    return;
  }
  ...
  for (std::vector<lay::NetlistObjectsPath>::const_iterator path =
           m_selected_paths.begin();
       path != m_selected_paths.end(); ++path) {
    ...
    std::pair<bool, db::DCplxTrans> tr = trans_for(
        circuit, *layout, *cell, m_cell_context_cache, cv.context_dtrans());
    if (!tr.first) {
      continue;
    }
    ...
    if (device) {
      ...
      ebox += bbox_for_device_abstract(layout, device->device_abstract(),
                                       db::DCplxTrans());

      const std::vector<db::DeviceAbstractRef> &oda = device->other_abstracts();
      for (std::vector<db::DeviceAbstractRef>::const_iterator a = oda.begin();
           a != oda.end(); ++a) {
      ...
      for (db::Connectivity::layer_iterator layer = conn.begin_layers();
        ...
        db::Box layer_bbox;
        db::recursive_cluster_shape_iterator<db::NetShape> shapes(
            mp_database->net_clusters(), *layer, cell_index, cluster_id);
        while (!shapes.at_end()) {
  ...
  if (!bbox.empty()) {
    ...
    db::DBox tv_bbox;
    for (std::vector<db::DCplxTrans>::const_iterator t = tv.begin();
         t != tv.end(); ++t) {
      tv_bbox += *t * bbox;
    }
    ...
    if (m_window == lay::NetlistBrowserConfig::FitNet) {
      ...
      mp_view->zoom_box(
          tv_bbox.enlarged(db::DVector(m_window_dim, m_window_dim)));
...
tl::Color NetlistBrowserPage::make_valid_color(const tl::Color &color) {
  if (!color.is_valid() && mp_view) {
    return mp_view->background_color().to_mono() ? tl::Color(0, 0, 0)
...
bool NetlistBrowserPage::produce_highlights_for_device(
    const db::Device *device, size_t &n_markers,
    const std::vector<db::DCplxTrans> &tv) {
  const db::Layout *layout = mp_database->internal_layout();

  tl::Color color = make_valid_color(m_colorizer.marker_color());

  db::Box device_bbox = bbox_for_device_abstract(
      layout, device->device_abstract(), device->trans());
  ...
  for (std::vector<db::DeviceAbstractRef>::const_iterator a = oda.begin();
    ...
    db::Box da_box = bbox_for_device_abstract(layout, a->device_abstract,
                                              device->trans() * a->trans);
    if (!da_box.empty()) {
...
bool NetlistBrowserPage::produce_highlights_for_circuit(
    const db::Circuit *circuit, size_t &n_markers,
    const std::vector<db::DCplxTrans> &tv) {
  const db::Layout *layout = mp_database->internal_layout();
...
bool NetlistBrowserPage::produce_highlights_for_net(
    const db::Net *net, size_t &n_markers,
    const std::map<db::LayerProperties, lay::LayerPropertiesConstIterator>
        &display_by_lp,
    const std::vector<db::DCplxTrans> &tv) {
  const db::Layout *layout = mp_database->internal_layout();
  ...
  for (db::Connectivity::layer_iterator layer = conn.begin_layers();
    ...
    std::map<db::LayerProperties,
             lay::LayerPropertiesConstIterator>::const_iterator display =
        display_by_lp.find(lp);
    ...
    while (!shapes.at_end()) {
      ...
      if (net_color.is_valid()) {
        ...
        mp_markers.back()->set_dither_pattern(
            display->second->dither_pattern(true));
        if (mp_view->background_color().green() < 128) {
          mp_markers.back()->set_color(display->second->eff_fill_color_brighter(
              true, (m_marker_intensity * 255) / 100));
          mp_markers.back()->set_frame_color(
              display->second->eff_frame_color_brighter(
                  true, (m_marker_intensity * 255) / 100));
        } else {
          mp_markers.back()->set_color(display->second->eff_fill_color_brighter(
              true, (-m_marker_intensity * 255) / 100));
...
void NetlistBrowserPage::update_highlights() {
  ...
  std::map<db::LayerProperties, lay::LayerPropertiesConstIterator>
      display_by_lp;
  for (lay::LayerPropertiesConstIterator lp = mp_view->begin_layers();
       !lp.at_end(); ++lp) {
    if (!lp->has_children() && lp->cellview_index() == int(m_cv_index) &&
        lp->layer_index() >= 0 &&
        (unsigned int)lp->layer_index() < original_layout.layers()) {
      display_by_lp.insert(std::make_pair(
          original_layout.get_properties(lp->layer_index()), lp));
    }
  }
  ...
  for (std::vector<lay::NetlistObjectsPath>::const_iterator path =
    ...
    //  correct DBU differences between the storage layout and the original
    //  layout
    std::vector<db::DCplxTrans> tv = mp_view->cv_transform_variants(m_cv_index);
    for (std::vector<db::DCplxTrans>::iterator t = tv.begin(); t != tv.end();
         ++t) {
      *t = *t * trans * db::DCplxTrans(layout->dbu() / original_layout.dbu());
    }

    if (path->net.first) {
      if (produce_highlights_for_net(path->net.first, n_markers, display_by_lp,
                                     tv)) {
...
void NetlistBrowserPage::clear_markers() {
  for (std::vector<lay::Marker *>::iterator m = mp_markers.begin();
       m != mp_markers.end(); ++m) {
    delete *m;
  }
...
void NetlistBrowserPage::export_nets(const std::vector<const db::Net *> *nets) {
  ...
  const db::Cell &source_top =
      source_layout.cell(*source_layout.begin_top_down());
  ...
  if (dialog->exec_dialog(mp_plugin_root)) {
    ...
    db::cell_index_type target_top_index = target_layout.add_cell(
        source_layout.cell_name(source_top.cell_index()));

    db::CellMapping cm;
    if (!nets) {
      cm = database->cell_mapping_into(target_layout,
                                       target_layout.cell(target_top_index));
    } else {
      cm = database->cell_mapping_into(
          target_layout, target_layout.cell(target_top_index), *nets);
    }

    std::map<unsigned int, const db::Region *> lm =
        database->create_layermap(target_layout, dialog->start_layer_number());

    database->build_nets(
        nets, cm, target_layout, lm,
        dialog->net_prefix().empty() ? 0 : dialog->net_prefix().c_str(),
        db::NPM_AllProperties, dialog->net_propname(),
        dialog->produce_circuit_cells() ? db::BNH_SubcircuitCells
                                        : db::BNH_Flatten,
        dialog->produce_circuit_cells() ? dialog->circuit_cell_prefix().c_str()
                                        : 0,
        dialog->produce_device_cells() ? dialog->device_cell_prefix().c_str()
                                       : 0);
