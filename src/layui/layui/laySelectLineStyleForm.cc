...
SelectLineStyleForm::SelectLineStyleForm(QWidget *parent,
                                         const lay::LineStyles &styles,
                                         bool include_nil)
    : QDialog(parent), m_selected(-1), m_styles(styles),
...
void SelectLineStyleForm::set_selected(int selected) {
  if (selected != m_selected) {
    m_selected = selected;
    mp_ui->style_items->setCurrentRow(
...
void SelectLineStyleForm::update() {
  ...
  for (lay::LineStyles::iterator i = m_styles.begin();
    ...
    new QListWidgetItem(QIcon(i->get_bitmap(36, 26)), tl::to_qstring(name),
                        mp_ui->style_items);
  }
...
void SelectLineStyleForm::sel_changed(QListWidgetItem *citem,
                                      QListWidgetItem *) {
