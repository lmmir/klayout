...
NetInfoDialog::NetInfoDialog(QWidget *parent)
  ...
  connect(ui->detailed_cb, SIGNAL(stateChanged(int)), this,
          SLOT(detailed_checkbox_clicked()));
}
...
void NetInfoDialog::set_nets(const db::LayoutToNetlist *l2ndb,
...
void NetInfoDialog::detailed_checkbox_clicked() { needs_update(); }
...
size_t count_shapes(db::LayoutToNetlist *l2ndb, db::Net *net,
  ...
  size_t n = 0;
  for (db::recursive_cluster_shape_iterator<db::NetShape> shapes(
           l2ndb->net_clusters(), layer, cell_index, cluster_id);
       !shapes.at_end(); ++shapes) {
    ++n;
  }
  return n;
}
...
static std::string layer_string(const db::LayoutToNetlist *l2ndb,
                                unsigned int layer) {
...
void NetInfoDialog::update_info_text() {
  ...
  if (!mp_l2ndb.get() || mp_nets.empty() || !mp_l2ndb->internal_layout()) {
    ...
    for (tl::weak_collection<db::Net>::iterator net = mp_nets.begin();
         net != mp_nets.end(); ++net) {
    ...
    if (mp_nets.size() == 1 && detailed) {
      ...
      std::map<unsigned int, db::coord_traits<db::Coord>::area_type>
          statinfo_area;
      std::map<unsigned int, db::coord_traits<db::Coord>::perimeter_type>
          statinfo_perimeter;
      ...
      for (db::Connectivity::layer_iterator layer = conn.begin_layers();
        ...
        for (db::recursive_cluster_shape_iterator<db::NetShape> si(
                 mp_l2ndb->net_clusters(), *layer, cell_index, cluster_id);
             !si.at_end(); ++si) {
          ...
          //  Check if layer is already detected, otherwise create
          //  vector-of-Shape object to hold shapes plus initialize the
          //  perimeter and area sums
          std::map<unsigned int, std::vector<db::Polygon>>::iterator s =
              shapes_by_layer.find(*layer);
          if (s == shapes_by_layer.end()) {
            s = shapes_by_layer
                    .insert(std::make_pair(*layer, std::vector<db::Polygon>()))
                    .first;
            layer_names.insert(std::make_pair(*layer, l));
            statinfo_perimeter.insert(std::make_pair(
                *layer, db::coord_traits<db::Coord>::perimeter_type(0)));
            statinfo_area.insert(std::make_pair(
                *layer, db::coord_traits<db::Coord>::area_type(0)));
          }
          ...
          if (s->second.back().is_box()) {
            t = tl::to_string(QObject::tr("box on ")) + l + ": " +
                (dbu * s->second.back().box()).to_string();
          } else {
            t = tl::to_string(QObject::tr("polygon on ")) + l + ": " +
                (dbu * s->second.back()).to_string();
          }

          shapes.insert(std::make_pair(c, std::set<std::string>()))
              .first->second.insert(t);
        }
      }

      //  Try to merge all shaped to polygons, use Map of (layernumber, group of
      //  polygons by layer)
      std::map<unsigned int, std::vector<db::Polygon>> polygons_by_layer;
      for (std::map<unsigned int, std::vector<db::Polygon>>::iterator i =
               shapes_by_layer.begin();
           i != shapes_by_layer.end(); ++i) {
        ...
        std::vector<db::Polygon> &merged =
            polygons_by_layer
                .insert(std::make_pair(l, std::vector<db::Polygon>()))
                .first->second;
        ...
        //  Despite merging, a multitude of separate non-touching polygons can
        //  exist.
        for (std::vector<db::Polygon>::iterator j = merged.begin();
             j != merged.end(); ++j) {
          //  Sum area
          area += j->area();
      ...
      if (!shapes.empty()) {
        ...
        if (!incomplete) {
          ...
          for (std::map<unsigned int,
                        db::coord_traits<db::Coord>::area_type>::iterator i =
                   statinfo_area.begin();
               i != statinfo_area.end(); ++i) {
        ...
        for (std::map<std::string, std::set<std::string>>::const_iterator s =
                 shapes.begin();
             s != shapes.end(); ++s) {
          ...
          for (std::set<std::string>::const_iterator l = s->second.begin();
               l != s->second.end(); ++l) {
            info.start_element("br");
            info.end_element("br");
            info.cdata(*l);
