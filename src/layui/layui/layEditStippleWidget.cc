...
struct PatternStorageOp : public db::Op {
...
EditStippleWidget::EditStippleWidget(QWidget *parent)
    : QFrame(parent), db::Object(), m_last_mx(-1), m_last_my(0), m_last_sx(32),
      m_last_sy(32), m_last_pattern_saved(false), m_clearing(false),
      m_readonly(false), m_sx(32), m_sy(32) {
...
QSize EditStippleWidget::sizeHint() const {
  return QSize(stipple_pixel_size * full_size + 1,
               stipple_pixel_size * full_size + 1);
}
...
void EditStippleWidget::set_pixel(unsigned int xx, unsigned int yy,
                                  bool value) {
  if (xx >= 32 || yy >= 32) {
    return;
  }
...
bool EditStippleWidget::mouse_to_pixel(const QPoint &pt, unsigned int &x,
                                       unsigned int &y) {
...
void EditStippleWidget::mouseReleaseEvent(QMouseEvent *) {
  if (m_last_pattern_saved) {
    ...
    if (manager()) {
      ...
      manager()->queue(this, new PatternStorageOp(m_last_pattern, m_last_sx,
                                                  m_last_sy, true));
      manager()->queue(this,
                       new PatternStorageOp(m_pattern, m_sx, m_sy, false));
      manager()->commit();
    }
  }
}
...
void EditStippleWidget::paintEvent(QPaintEvent *) {
  ...
  for (unsigned int i = 0; i < (unsigned int)full_size; ++i) {
    ...
    painter.drawLine(i * stipple_pixel_size, 0, i * stipple_pixel_size,
                     full_size * stipple_pixel_size);
    painter.drawLine(0, i * stipple_pixel_size, full_size * stipple_pixel_size,
                     i * stipple_pixel_size);
    ...
    for (unsigned int j = 0; j < (unsigned int)full_size; ++j) {
      QRect r(i * stipple_pixel_size + 1, j * stipple_pixel_size + 1,
              stipple_pixel_size - 1, stipple_pixel_size - 1);
  ...
  painter.drawLine(full_size * stipple_pixel_size, 0,
                   full_size * stipple_pixel_size,
                   full_size * stipple_pixel_size);
  painter.drawLine(0, full_size * stipple_pixel_size,
                   full_size * stipple_pixel_size,
                   full_size * stipple_pixel_size);
...
void EditStippleWidget::set_pattern(const uint32_t *const *pattern,
...
void EditStippleWidget::set_pattern(const uint32_t *pattern, unsigned int w,
                                    unsigned int h) {
  if (w != m_sx || h != m_sy) {
    m_sx = w;
    m_sy = h;
