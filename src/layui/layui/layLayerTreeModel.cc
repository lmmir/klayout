...
void EmptyWithinViewCache::clear() { m_cache.clear(); }

bool EmptyWithinViewCache::is_empty_within_view(const db::Layout *layout,
                                                unsigned int cell_index,
                                                const db::Box &box,
                                                unsigned int layer) {
  std::pair<cache_t::iterator, bool> c = m_cache.insert(
      std::make_pair(std::make_pair(std::make_pair(layout, cell_index), box),
                     std::set<unsigned int>()));
  if (c.second) {

    tl::SelfTimer timer(
        tl::verbosity() >= 21,
        tl::to_string(QObject::tr("Recomputing layers with shapes in view")));

    const db::Cell &cell(layout->cell(cell_index));

    //  determine layers with shapes on the given layout and within the given
    //  box
    ...
    for (db::Layout::layer_iterator l = layout->begin_layers();
      ...
      } else if (!cell.shapes((*l).first)
                      .begin_touching(box, db::ShapeIterator::All, 0, false)
                      .at_end()) {
        ;
      } else {
...
void EmptyWithinViewCache::determine_empty_layers(
    const db::Layout *layout, unsigned int cell_index, const db::Box &box,
    std::vector<unsigned int> &layers) {
  if (layers.empty()) {
    return;
  }

  //  Hint: this implementation counts all hierarchy levels - also the ones not
  //  shown ...

  db::box_convert<db::CellInst> bc(*layout);

  db::Cell::touching_iterator inst =
      layout->cell(cell_index).begin_touching(box);
  while (!inst.at_end() && !layers.empty()) {
    ...
    if (!m_cells_done[ci]) {
      ...
      if (inst->bbox(bc).inside(box)) {
        ...
        //  The instance is fully inside the search box: remove the non-empty
        //  layers from the list and mark the cell as "done".
        ...
        //  Remove every layer from the list which has at least one shape on it
        //  within the box and temporarily create a list of layers with only the
        //  relevant layers in ll
        ...
        if (!ll.empty()) {
          ...
          db::CellInstArray::iterator inst_array =
              inst->cell_inst().begin_touching(box, bc);
          while (!inst_array.at_end() && !ll.empty()) {

            db::Box new_box =
                db::Box(inst->complex_trans(*inst_array).inverted() * box);

            //  remove all layers which became populated in that instance
            std::vector<unsigned int>::iterator llw = ll.begin();
            for (std::vector<unsigned int>::const_iterator l = ll.begin();
                 l != ll.end(); ++l) {
              if (cell.shapes(*l)
                      .begin_touching(new_box, db::ShapeIterator::All, 0, false)
                      .at_end()) {
                *llw++ = *l;
              }
            }
...
LayerTreeModel::LayerTreeModel(QWidget *parent, lay::LayoutViewBase *view)
    : QAbstractItemModel(parent), mp_parent(parent), mp_view(view),
      m_filter_mode(false), m_id_start(0), m_id_end(0),
      m_phase((unsigned int)-1), m_test_shapes_in_view(false),
...
void LayerTreeModel::set_phase(unsigned int ph) { m_phase = ph; }
...
void LayerTreeModel::set_font_no_signal(const QFont &font) { m_font = font; }
...
int LayerTreeModel::columnCount(const QModelIndex &) const { return 2; }
...
int LayerTreeModel::rowCount(const QModelIndex &parent) const {
  if (mp_view->layer_model_updated()) {
    ...
    if (parent.isValid()) {
      ...
      int n = int(mp_view->get_properties().end_const() -
                  mp_view->get_properties().begin_const());
      return n;
    }
...
QModelIndex LayerTreeModel::index(int row, int column,
...
QModelIndex LayerTreeModel::locate_next() {
...
QModelIndex LayerTreeModel::locate_prev() {
  if (m_current_index == m_selected_indexes.end()) {
    return QModelIndex();
  } else {
...
void LayerTreeModel::search_children(const tl::GlobPattern &pattern,
...
QModelIndex LayerTreeModel::locate(const char *name, bool glob_pattern,
...
void LayerTreeModel::signal_layers_changed() {
  ...
  size_t max_id = 0;
  for (lay::LayerPropertiesConstIterator iter = mp_view->get_properties();
       !iter.at_end(); ++iter) {
  ...
  for (QModelIndexList::const_iterator i = indexes.begin(); i != indexes.end();
    ...
    if (!li.at_end()) {
      new_indexes.push_back(createIndex(int(li.child_index()), i->column(),
                                        (void *)(li.uint() + m_id_start)));
    } else {
...
QModelIndex LayerTreeModel::upperLeft() const {
  if (mp_view->layer_model_updated()) {
    lay::LayerPropertiesConstIterator iter(mp_view->begin_layers());
    iter.next_sibling(0);
...
QModelIndex LayerTreeModel::bottomRight() const {
  if (mp_view->layer_model_updated()) {
...
QModelIndex LayerTreeModel::parent(const QModelIndex &index) const {
  try {
    ...
    if (mp_view->layer_model_updated()) {
      ...
      if (iter.is_null() || iter.at_end()) {
        ...
        if (iter.is_null()) {
          ...
          return createIndex(int(iter.child_index()), 0,
                             (void *)(iter.uint() + m_id_start));
        }
      }
    } else {
      return QModelIndex();
    ...
    //  this can happen because some internal indices might not be in place
    //  properly - in particular the element the mouse was over (hover index)
    tl::warn << ex.msg();
    return QModelIndex();
  }
}
...
bool LayerTreeModel::is_hidden(const QModelIndex &index) const {
  if (m_filter_mode && !m_selected_ids.empty() &&
      m_selected_ids.find(size_t(index.internalPointer())) ==
          m_selected_ids.end()) {
    return true;
  }
...
bool LayerTreeModel::empty_within_view_predicate(
  ...
  if (iter.is_null() || iter.at_end()) {
    ...
    for (std::vector<db::DCplxTrans>::const_iterator t = trans.begin();
      ...
      db::CplxTrans ct =
          vp_trans * *t * db::CplxTrans(layout.dbu()) * ctx_trans;

      // the following scheme to compute the region avoids problems with
      // accessing designs through very large viewports:
      db::Coord lim = std::numeric_limits<db::Coord>::max();
      db::DBox world(ct * db::Box(db::Point(-lim, -lim), db::Point(lim, lim)));
      db::Box region =
          ct.inverted() *
          (world & db::DBox(db::DPoint(0.0, 0.0), db::DPoint(width, height)));
      region &= cell.bbox();

      if (!m_test_shapes_cache.is_empty_within_view(&layout, ci, region,
                                                    layer_id)) {
        return false;
      }
    }
...
QIcon LayerTreeModel::icon_for_layer(
    const lay::LayerPropertiesConstIterator &iter, lay::LayoutViewBase *view,
    unsigned int w, unsigned int h, double dpr, unsigned int di_off,
    bool no_state) {
  QImage img =
      view->icon_for_layer(iter, w, h, dpr, di_off, no_state).to_image_copy();
...
QVariant LayerTreeModel::data(const QModelIndex &index, int role) const {
  if (mp_view->layer_model_updated()) {
    ...
    if (iter.is_null() || iter.at_end()) {
      ...
      if (index.column() == 0) {
        //  NOTE: for some reason, the widget clips the icon when inside a tree
        //  and needs a some what bigger width ..
        QSize is = icon_size();
      ...
      if (index.column() == 1) {
        return QVariant(
            tl::to_qstring(iter->display_string(mp_view, true /*real*/)));
      }
      ...
      if (index.column() == 0) {
        ...
        if (animate_visible) {
          ...
          double dpr = 1.0;
#endif
          return QVariant(icon_for_layer(iter, mp_view, is.width(), is.height(),
                                         dpr, di_off));
        } else {
      ...
      if (mp_parent && m_selected_ids.find(size_t(index.internalPointer())) !=
                           m_selected_ids.end()) {
      ...
      if (index.column() == 1) {
        ...
        QColor c0 =
            QColor((c1.red() + cb.red()) / 2, (c1.green() + cb.green()) / 2,
                   (c1.blue() + cb.blue()) / 2);
        ...
        //  set the color to one with less contrast if there is nothing on this
        //  layer here
        if (role == Qt::FontRole) {
          QFont f = m_font;
          f.setBold(!empty);
...
LayerTreeModel::iterator(const QModelIndex &index) const {
  if (index.isValid()) {
    ...
    if (mp_view->layer_lists() > 0 && iter_index >= m_id_start &&
        iter_index < m_id_end) {
      return lay::LayerPropertiesConstIterator(mp_view->get_properties(),
                                               iter_index - m_id_start);
    }
  }
...
LayerTreeModel::iterator_nc(const QModelIndex &index) {
  if (index.isValid()) {
    ...
    if (mp_view->layer_lists() > 0 && iter_index >= m_id_start &&
        iter_index < m_id_end) {
      return lay::LayerPropertiesIterator(mp_view->get_properties(),
                                          iter_index - m_id_start);
    }
  }
...
QModelIndex LayerTreeModel::index(lay::LayerPropertiesConstIterator iter,
                                  int column) const {
  try {
    ...
    for (std::vector<int>::reverse_iterator r = rows.rbegin(); r != rows.rend();
         ++r) {
