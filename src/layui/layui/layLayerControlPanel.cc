...
LCPTreeWidget::LCPTreeWidget(QWidget *parent, lay::LayerTreeModel *model,
                             const char *name)
...
QSize LCPTreeWidget::sizeHint() const { return QSize(0, 0); }

void LCPTreeWidget::set_selection(
    const std::vector<lay::LayerPropertiesConstIterator> &sel) {
  clearSelection();
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator s =
           sel.begin();
       s != sel.end(); ++s) {
    selectionModel()->select(mp_model->index(*s, 0),
  ...
  if (sel.empty()) {
    ...
    selectionModel()->setCurrentIndex(mp_model->index(sel.front(), 1),
                                      QItemSelectionModel::Current |
                                          QItemSelectionModel::Rows);
  }
}
...
void LCPTreeWidget::set_current(const lay::LayerPropertiesConstIterator &sel) {
  selectionModel()->select(mp_model->index(sel, 0),
                           QItemSelectionModel::ClearAndSelect |
                               QItemSelectionModel::Rows);
  selectionModel()->setCurrentIndex(mp_model->index(sel, 1),
                                    QItemSelectionModel::Current |
                                        QItemSelectionModel::Rows);
}
...
bool LCPTreeWidget::focusNextPrevChild(bool /*next*/) { return false; }
...
void LCPTreeWidget::collapse_all() {
  ...
  for (std::vector<QModelIndex>::const_iterator i = indices.begin();
       i != indices.end(); ++i) {
    collapse(*i);
  }
#endif
...
LayerControlPanel::LayerControlPanel(lay::LayoutViewBase *view,
                                     db::Manager *manager, QWidget *parent,
                                     const char *name)
    : QFrame(parent), db::Object(manager), mp_view(view), m_needs_update(true),
      m_expanded_state_needs_update(false), m_tabs_need_update(true),
      m_hidden_flags_need_update(true), m_in_update(false), m_phase(0),
      m_oversampling(1), m_hrm(false),
  ...
  mp_search_close_cb->setSizePolicy(
      QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Preferred));
  QPalette pl(mp_search_close_cb->palette());
  pl.setColor(QPalette::WindowText,
              pl.color(QPalette::Active, QPalette::HighlightedText));
  mp_search_close_cb->setPalette(pl);
  mp_search_close_cb->setMaximumSize(
      QSize(mp_search_close_cb->maximumSize().width(),
            mp_search_close_cb->sizeHint().height() - 4));
  connect(mp_search_close_cb, SIGNAL(clicked()), this,
          SLOT(search_editing_finished()));

  mp_search_edit_box = new lay::DecoratedLineEdit(mp_search_frame);
  mp_search_edit_box->setObjectName(
      QString::fromUtf8("cellview_search_edit_box"));
  mp_search_edit_box->set_escape_signal_enabled(true);
  mp_search_edit_box->set_tab_signal_enabled(true);
  connect(mp_search_edit_box, SIGNAL(returnPressed()), this,
          SLOT(search_editing_finished()));
  connect(mp_search_edit_box, SIGNAL(textEdited(const QString &)), this,
          SLOT(search_edited()));
  connect(mp_search_edit_box, SIGNAL(esc_pressed()), this,
          SLOT(search_editing_finished()));
  connect(mp_search_edit_box, SIGNAL(tab_pressed()), this, SLOT(search_next()));
  connect(mp_search_edit_box, SIGNAL(backtab_pressed()), this,
          SLOT(search_prev()));
  ...
  mp_use_regular_expressions->setText(
      tr("Use expressions (use * and ? for any character)"));
  ...
  connect(mp_use_regular_expressions, SIGNAL(triggered()), this,
          SLOT(search_edited()));
  ...
  connect(mp_tab_bar, SIGNAL(customContextMenuRequested(const QPoint &)), this,
  ...
  connect(mp_layer_list,
          SIGNAL(double_clicked(const QModelIndex &, Qt::KeyboardModifiers)),
          this,
          SLOT(double_clicked(const QModelIndex &, Qt::KeyboardModifiers)));
  connect(mp_layer_list, SIGNAL(collapsed(const QModelIndex &)), this,
          SLOT(group_collapsed(const QModelIndex &)));
  connect(mp_layer_list, SIGNAL(expanded(const QModelIndex &)), this,
          SLOT(group_expanded(const QModelIndex &)));
  connect(mp_layer_list, SIGNAL(search_triggered(const QString &)), this,
          SLOT(search_triggered(const QString &)));
  connect(mp_layer_list->selectionModel(),
          SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
          this, SLOT(current_index_changed(const QModelIndex &)));
  mp_layer_list->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(mp_layer_list, SIGNAL(customContextMenuRequested(const QPoint &)),
  ...
  mp_view->layer_list_changed_event.add(this,
                                        &LayerControlPanel::update_required);
  mp_view->layer_list_inserted_event.add(this,
                                         &LayerControlPanel::signal_ll_changed);
  mp_view->layer_list_deleted_event.add(this,
                                        &LayerControlPanel::signal_ll_changed);
  mp_view->current_layer_list_changed_event.add(
      this, &LayerControlPanel::signal_li_changed);
  mp_view->geom_changed_event.add(this, &LayerControlPanel::signal_cv_changed);
  mp_view->cellview_changed_event.add(
      this, &LayerControlPanel::signal_cv_changed_with_int);
  mp_view->viewport_changed_event.add(this,
                                      &LayerControlPanel::signal_vp_changed);
  mp_view->hier_levels_changed_event.add(this,
                                         &LayerControlPanel::signal_vp_changed);
  ...
  m_no_stipples_label->setPixmap(
      QPixmap(QString::fromUtf8(":/warn_16px@2x.png")));
  m_no_stipples_label->setToolTip(
      tr("Stipples are disabled - unselect \"View/Show Layers Without Fill\" "
         "to re-enable them"));
  ltb->addWidget(m_no_stipples_label);

  connect(mp_model, SIGNAL(hidden_flags_need_update()), this,
          SLOT(update_hidden_flags()));
}
...
struct LayerSelectionClearOp : public db::Op {
  LayerSelectionClearOp() : db::Op() {}
};
...
void LayerControlPanel::cm_insert() {
  ...
  if (dialog.exec_dialog(n)) {
    ...
    //  HINT: this must be the last action in this method since it will trigger
    //  the event loop which will dispatch further actions.
    if (mp_view->is_editable() && lp.layer_index() < 0 &&
        lp.cellview_index() >= 0 &&
        lp.source(true).special_purpose() == ParsedLayerSource::SP_None) {
      QMessageBox::warning(
          0, QObject::tr("Layer does not exist"),
          QObject::tr("The layer specified does not exist. To create that "
                      "layer, use 'New/Layer' from the 'Edit' menu"));
    }
...
void LayerControlPanel::cm_group() {
  ...
  if (!sel.empty()) {
    ...
    //  The delete_layer method invalidates the iterator and tries to set it to
    //  the next available object. However, for the insert position, we still
    //  need the original location. Therefore we have to save the insert
    //  position:
...
void LayerControlPanel::cm_ungroup() {
  ...
  if (!sel.is_null() && sel->has_children()) {
    ...
    for (lay::LayerPropertiesNode::const_iterator c = node.end_children();
         c != node.begin_children();) {
      --c;
    ...
    set_selection(
        std::vector<lay::LayerPropertiesConstIterator>()); // clear selection
...
bool LayerControlPanel::has_focus() const { return mp_layer_list->hasFocus(); }
...
static void collect_dpi(const lay::LayerPropertiesNode &node,
                        std::set<unsigned int> &dpi) {
  if (node.dither_pattern(false) >= 0) {
    dpi.insert(node.dither_pattern(false));
  }
  for (lay::LayerPropertiesNode::const_iterator c = node.begin_children();
       c != node.end_children(); ++c) {
    collect_dpi(*c, dpi);
  }
}

static void update_dpi(lay::LayerPropertiesNode &node,
                       const std::map<unsigned int, unsigned int> &dpi_map) {
  std::map<unsigned int, unsigned int>::const_iterator new_dpi =
      dpi_map.find((unsigned int)node.dither_pattern(false));
  if (new_dpi != dpi_map.end()) {
    node.set_dither_pattern(new_dpi->second);
  }
  for (lay::LayerPropertiesNode::iterator c = node.begin_children();
       c != node.end_children(); ++c) {
...
void LayerControlPanel::do_copy() {
  ...
  for (std::set<unsigned int>::const_iterator dp = dp_to_save.begin();
       dp != dp_to_save.end(); ++dp) {
    if (*dp >=
        (unsigned int)std::distance(mp_view->dither_pattern().begin(),
                                    mp_view->dither_pattern().begin_custom())) {
      ...
      db::Clipboard::instance() +=
          new db::ClipboardValue<lay::DitherPatternInfo>(dpi);
    }
  }
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator l =
           sel.begin();
       l != sel.end(); ++l) {
    db::Clipboard::instance() +=
        new db::ClipboardValue<lay::LayerPropertiesNode>(**l);
  }
}
...
void LayerControlPanel::paste() {
  try {
    ...
    for (db::Clipboard::iterator obj = db::Clipboard::instance().begin();
         obj != db::Clipboard::instance().end(); ++obj) {

      const db::ClipboardValue<lay::DitherPatternInfo> *dp_obj =
          dynamic_cast<const db::ClipboardValue<lay::DitherPatternInfo> *>(
              *obj);
      if (dp_obj) {

        //  try to locate the corresponding pattern or insert as a new one if
        //  required
        int found_dpi = -1;
        for (lay::DitherPattern::iterator dp = dither_pattern.begin_custom();
             dp != dither_pattern.end() && found_dpi < 0; ++dp) {
        ...
        dpi_map.insert(std::make_pair(dp_obj->get().order_index(),
                                      (unsigned int)found_dpi));
      }
    }
    ...
    for (db::Clipboard::iterator obj = db::Clipboard::instance().begin();
         obj != db::Clipboard::instance().end(); ++obj) {
      const db::ClipboardValue<lay::LayerPropertiesNode> *lp_obj =
          dynamic_cast<const db::ClipboardValue<lay::LayerPropertiesNode> *>(
              *obj);
      if (lp_obj) {
...
void LayerControlPanel::cm_rename() {
  ...
  if (!sel.is_null()) {
    ...
    bool ok = false;
    QString n = QInputDialog::getText(this, QObject::tr("Rename layer"),
                                      QObject::tr("Enter new name of layer"),
                                      QLineEdit::Normal,
                                      tl::to_qstring(props.name()), &ok);
...
void LayerControlPanel::cm_show_only() {
  ...
  //  now hide all non-selected nodes which don't have a parent or are children
  //  of a selected node
  for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
       !l.at_end(); ++l) {
    if (sel_set.find(l) == sel_set.end() &&
        (l.parent().is_null() || sel_set.find(l.parent()) != sel_set.end())) {
      lay::LayerProperties props(*l);
...
void LayerControlPanel::cm_rename_tab() {
  ...
  QString n = QInputDialog::getText(
      this, QObject::tr("Rename Layer Tab"), QObject::tr("New layer tab name"),
      QLineEdit::Normal,
      tl::to_qstring(
          mp_view->get_properties(mp_view->current_layer_list()).name()),
      &ok);
...
void LayerControlPanel::cm_remove_tab() {
  ...
  if (mp_view->layer_lists() == 1) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Cannot remove last layer tab")));
  }
...
void LayerControlPanel::cm_new_tab() {
  ...
  begin_updates();
  mp_view->insert_layer_list(mp_view->current_layer_list() + 1,
                             mp_view->get_properties());
  end_updates();
...
void LayerControlPanel::cm_hide_all() {
  ...
  for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
       !l.at_end(); ++l) {
    if (l.parent().is_null()) {
      //  only hide the top-level entries - this way, nothing will be visible,
      //  but the child nodes maintain their state
      lay::LayerProperties props(*l);
      props.set_visible(false);
      mp_view->set_properties(l, props);
    }
...
void LayerControlPanel::set_selection(
    const std::vector<lay::LayerPropertiesConstIterator> &new_sel) {
  //  If the tree has changed we need to delay the selection update until the
  //  model has been updated.
  if (m_in_update) {

    //  store only the uint's of the selected items to become independent from
    //  the list reference
    m_new_sel.clear();
    for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator s =
             new_sel.begin();
...
void LayerControlPanel::search_edited() {
  ...
  if (t.isEmpty()) {
    ...
    QModelIndex found = mp_model->locate(
        t.toUtf8().constData(), mp_use_regular_expressions->isChecked(),
        mp_case_sensitive->isChecked(), false);
...
struct LDSortingProps : std::vector<int> {
...
struct LayerSorter {
  LayerSorter(const lay::LayoutViewBase *view,
              lay::LayerControlPanel::SortOrder order)
      : m_order(order), mp_view(view) {
    //  .. nothing yet ..
  }

  bool operator()(const lay::LayerPropertiesNode &a,
                  const lay::LayerPropertiesNode &b) const {
    if (m_order == lay::LayerControlPanel::ByName) {

      return a.display_string(mp_view, false) <
             b.display_string(mp_view, false);
...
void LayerControlPanel::sort_layers(SortOrder order) {
  ...
  if (p.is_null()) {
    new_props.assign(mp_view->get_properties().begin_const(),
                     mp_view->get_properties().end_const());
  } else {
  ...
  if (p.is_null()) {
    ...
    for (std::vector<lay::LayerPropertiesNode>::const_iterator np =
             new_props.begin();
         np != new_props.end(); ++np) {
      //  :KLUDGE: the list should have an insert with a begin..end iterator
      //  pair ..
...
struct LayerRegroupSorter {
  LayerRegroupSorter(lay::LayerControlPanel::RegroupMode mode) : m_mode(mode) {
    //  .. nothing yet ..
  }
...
void LayerControlPanel::cm_expand_all() { mp_layer_list->expand_all(); }
...
void LayerControlPanel::double_clicked(const QModelIndex &index,
...
void LayerControlPanel::begin_updates() {
  if (!m_in_update) {
    ...
    mp_model
        ->signal_begin_layer_changed(); //  this makes the view redraw the data
...
bool LayerControlPanel::model_updated() { return !m_in_update; }
...
void LayerControlPanel::end_updates() { m_do_update_content_dm(); }
...
void LayerControlPanel::set_highres_mode(bool hrm) {
  if (m_hrm != hrm) {
    m_hrm = hrm;
    m_do_update_content_dm();
  }
...
void LayerControlPanel::set_oversampling(int os) {
...
static void set_hidden_flags_rec(LayerTreeModel *model, QTreeView *tree_view,
...
void LayerControlPanel::do_update_content() {
  ...
  if (m_tabs_need_update) {
    ...
    //  temporarily disconnect tab selected signals because those get issued on
    //  insertTab
    disconnect(mp_tab_bar, SIGNAL(currentChanged(int)), this,
               SLOT(tab_selected(int)));
    ...
    if (mp_view->layer_lists() <= 1) {
      ...
      for (unsigned int ll = 0; ll < mp_view->layer_lists(); ++ll) {
        if (mp_view->get_properties(ll).name().empty()) {
          mp_tab_bar->setTabText(int(ll),
                                 tl::to_qstring(tl::to_string(ll + 1)));
        } else {
          mp_tab_bar->setTabText(
              int(ll), tl::to_qstring(mp_view->get_properties(ll).name()));
        }
      }
    }
  ...
  if (m_in_update) {
    ...
    //  HACK: reset the internal hover state to avoid badly indexed items being
    //  addressed.
    ...
    //  this makes the view redraw the data and establishes a valid selection
    //  scheme
    ...
    if (!m_new_sel.empty()) {
      std::vector<lay::LayerPropertiesConstIterator> new_sel;
      for (std::vector<size_t>::const_iterator s = m_new_sel.begin();
           s != m_new_sel.end(); ++s) {
        new_sel.push_back(
            lay::LayerPropertiesConstIterator(mp_view->get_properties(), *s));
      }
      set_selection(new_sel);
      m_new_sel.clear();
    }
    ...
    for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
         l != mp_view->end_layers() && !has_children; ++l) {
      if (l->has_children()) {
        has_children = true;
      }
    }
...
void LayerControlPanel::set_current_layer(
...
lay::LayerPropertiesConstIterator LayerControlPanel::current_layer() const {
...
LayerControlPanel::selected_layers() const {
  ...
  std::vector<lay::LayerPropertiesConstIterator>::iterator write =
      llist.begin();
  for (std::vector<lay::LayerPropertiesConstIterator>::iterator read =
           llist.begin();
       read != llist.end();) {
...
void LayerControlPanel::signal_cv_changed() { update_required(1); }

void LayerControlPanel::signal_cv_changed_with_int(int) { update_required(1); }
...
void LayerControlPanel::update_required(int f) {
  ...
  if ((f & 2) != 0) {
    ...
    //  if the signal arises from any action performed externally, we cannot
    //  rely on getting a end_updates - we have to do this explicitly here.
    if (!m_in_update) {
      begin_updates();
    }
  }
...
move_algo(std::vector<lay::LayerPropertiesConstIterator>::const_iterator from,
          ...
          std::vector<lay::LayerPropertiesConstIterator> &new_sel, int mode) {
  ...
  for (c.down_first_child(), nc.down_first_child(); !c.at_end();
       c.next_sibling(), nc.next_sibling()) {
  ...
  if (mode == 0 /*down*/) {
    ...
    for (std::vector<lay::LayerPropertiesConstIterator>::iterator i =
             new_sel.end();
         i != new_sel.begin() + nsel;) {
  ...
  std::vector<lay::LayerPropertiesConstIterator>::const_iterator inew =
      new_sel.begin() + nsel;
  std::vector<lay::LayerPropertiesConstIterator>::const_iterator iorg =
      org_sel.begin();

  lay::LayerPropertiesIterator ins = new_parent;
  ins.down_first_child();

  std::vector<std::pair<lay::LayerPropertiesConstIterator,
  ...
  //  now treat all nodes with children (it is important to do this at last,
  //  because then the child iterators will be valid finally and can be inserted
  //  into "new_sel")
  for (std::vector<std::pair<lay::LayerPropertiesConstIterator,
                             lay::LayerPropertiesIterator>>::iterator r =
           rec.begin();
       r != rec.end(); ++r) {
...
void LayerControlPanel::do_move(int mode) {
  ...
  move_algo(
      sel.begin(), sel.end(),
      lay::LayerPropertiesConstIterator(mp_view->get_properties(), size_t(0)),
      lay::LayerPropertiesIterator(new_props, size_t(0)), new_sel, mode);
...
class LayerControlPanelPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::submenu("@lcp_context_menu", at, std::string()));

    at = "@lcp_context_menu.end";

    menu_entries.push_back(
        lay::menu_item("cm_lv_select_all", "select_all", at,
                       tl::to_string(QObject::tr("Select All"))));
    //  It is not sure, whether "expandAll" destabilizes the tree widget:
    //  menu_entries.push_back (lay::menu_item ("cm_lv_expand_all",
    //  "expand_all", at, tl::to_string (QObject::tr ("Expand All"))));
    menu_entries.push_back(lay::separator("tab_group", at));
    menu_entries.push_back(
        lay::submenu("tab_menu", at, tl::to_string(QObject::tr("Tabs"))));

    {
      std::string at = "@lcp_context_menu.tab_menu.end";
      menu_entries.push_back(
          lay::menu_item("cm_lv_new_tab", "new_tab", at,
                         tl::to_string(QObject::tr("New Tab"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_remove_tab", "remove_tab", at,
                         tl::to_string(QObject::tr("Remove Tab"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_rename_tab", "rename_tab", at,
                         tl::to_string(QObject::tr("Rename Tab"))));
    }

    menu_entries.push_back(lay::separator("visibility_group", at));
    menu_entries.push_back(lay::menu_item("cm_lv_hide", "hide", at,
                                          tl::to_string(QObject::tr("Hide"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_hide_all", "hide_all", at,
                       tl::to_string(QObject::tr("Hide All"))));
    menu_entries.push_back(lay::menu_item("cm_lv_show", "show", at,
                                          tl::to_string(QObject::tr("Show"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_show_all", "show_all", at,
                       tl::to_string(QObject::tr("Show All"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_show_only", "show_only", at,
                       tl::to_string(QObject::tr("Show Only Selected"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_make_valid", "valid", at,
                       tl::to_string(QObject::tr("Make Valid"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_make_invalid", "invvalid", at,
                       tl::to_string(QObject::tr("Make Invalid"))));
    menu_entries.push_back(lay::menu_item(
        "cm_lv_rename", "rename", at, tl::to_string(QObject::tr("Rename"))));
    menu_entries.push_back(lay::separator("options_group", at));
    menu_entries.push_back(
        lay::config_menu_item("hide_empty_layers", at,
                              tl::to_string(QObject::tr("Hide Empty Layers")),
                              cfg_hide_empty_layers, "?"));
    menu_entries.push_back(lay::config_menu_item(
        "test_shapes_in_view", at,
        tl::to_string(QObject::tr("Test For Shapes In View")),
        cfg_test_shapes_in_view, "?"));
    menu_entries.push_back(lay::separator("source_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_lv_source", "select_source", at,
                       tl::to_string(QObject::tr("Select Source"))));
    menu_entries.push_back(lay::separator("sort_group", at));
    menu_entries.push_back(
        lay::submenu("sort_menu", at, tl::to_string(QObject::tr("Sort By"))));

    {
      std::string at = "@lcp_context_menu.sort_menu.end";
      menu_entries.push_back(lay::menu_item(
          "cm_lv_sort_by_ild", "sort_ild", at,
          tl::to_string(QObject::tr("Layout Index, Layer And Datatype"))));
      menu_entries.push_back(lay::menu_item(
          "cm_lv_sort_by_idl", "sort_idl", at,
          tl::to_string(QObject::tr("Layout Index, Datatype And Layer"))));
      menu_entries.push_back(lay::menu_item(
          "cm_lv_sort_by_ldi", "sort_ldi", at,
          tl::to_string(QObject::tr("Layer, Datatype And Layout Index"))));
      menu_entries.push_back(lay::menu_item(
          "cm_lv_sort_by_dli", "sort_dli", at,
          tl::to_string(QObject::tr("Datatype, Layer And Layout Index"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_sort_by_name", "sort_name", at,
                         tl::to_string(QObject::tr("Name"))));
    }

    menu_entries.push_back(lay::separator("view_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_lv_delete", "del", at,
                       tl::to_string(QObject::tr("Delete Layer Entry"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_insert", "insert", at,
                       tl::to_string(QObject::tr("Insert Layer Entry"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_add_missing", "add_others", at,
                       tl::to_string(QObject::tr("Add Other Layer Entries"))));
    menu_entries.push_back(
        lay::menu_item("cm_lv_remove_unused", "clean_up", at,
                       tl::to_string(QObject::tr("Clean Up Layer Entries"))));
    menu_entries.push_back(lay::separator("grouping_group", at));
    menu_entries.push_back(lay::menu_item("cm_lv_group", "group", at,
                                          tl::to_string(QObject::tr("Group"))));
    menu_entries.push_back(lay::menu_item(
        "cm_lv_ungroup", "ungroup", at, tl::to_string(QObject::tr("Ungroup"))));
    menu_entries.push_back(
        lay::submenu("regroup_menu", at,
                     tl::to_string(QObject::tr("Regroup Layer Entries"))));

    {
      std::string at = "@lcp_context_menu.regroup_menu.end";
      menu_entries.push_back(
          lay::menu_item("cm_lv_regroup_by_index", "grp_i", at,
                         tl::to_string(QObject::tr("By Layout Index"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_regroup_by_datatype", "grp_d", at,
                         tl::to_string(QObject::tr("By Datatype"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_regroup_by_layer", "grp_l", at,
                         tl::to_string(QObject::tr("By Layer"))));
      menu_entries.push_back(
          lay::menu_item("cm_lv_regroup_flatten", "flatten", at,
                         tl::to_string(QObject::tr("Flatten"))));
    }

    menu_entries.push_back(lay::separator("copy_paste_group", at));
    menu_entries.push_back(lay::menu_item("cm_lv_copy", "copy", at,
                                          tl::to_string(QObject::tr("Copy"))));
    menu_entries.push_back(lay::menu_item("cm_lv_cut", "cut", at,
                                          tl::to_string(QObject::tr("Cut"))));
    menu_entries.push_back(lay::menu_item("cm_lv_paste", "paste", at,
                                          tl::to_string(QObject::tr("Paste"))));

    at = ".end";
    menu_entries.push_back(
        lay::submenu("@lcp_tabs_context_menu", at, std::string()));
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new LayerControlPanelPluginDeclaration(), -9,
