...
class BookmarkListLVI : public QListWidgetItem {
public:
  BookmarkListLVI(QListWidget *parent, const std::string &name,
                  const lay::DisplayState &state)
      : QListWidgetItem(tl::to_qstring(name), parent), m_state(state) {
    setFlags(flags() | Qt::ItemIsEditable | Qt::ItemIsDragEnabled |
             Qt::ItemIsDropEnabled);
  }

  const lay::DisplayState &state() const { return m_state; }
...
BookmarkManagementForm::BookmarkManagementForm(
    QWidget *parent, const char *name, const lay::BookmarkList &bookmarks,
  ...
  for (size_t i = 0; i < m_bookmarks.size(); ++i) {
    QListWidgetItem *item = new BookmarkListLVI(
        mp_ui->bookmark_list, m_bookmarks.name(i), m_bookmarks.state(i));
...
void BookmarkManagementForm::delete_pressed() {
  ...
  for (QList<QListWidgetItem *>::const_iterator i = sel.begin(); i != sel.end();
       ++i) {
    delete *i;
  }
}
...
void BookmarkManagementForm::accept() {
  ...
  //  TODO: is there an iterator? Here we use the trick to select all and then
