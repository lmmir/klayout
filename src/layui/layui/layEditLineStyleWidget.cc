...
struct StyleStorageOp : public db::Op {
...
EditLineStyleWidget::EditLineStyleWidget(QWidget *parent)
    : QFrame(parent), db::Object(), m_last_mx(-1), m_last_sx(32),
      m_last_style_saved(false), m_clearing(false), m_readonly(false),
      m_sx(32) {
  m_last_style = 0;
  m_style = 0;
...
QSize EditLineStyleWidget::sizeHint() const {
  return QSize(stipple_pixel_size * full_size + 1,
               stipple_pixel_size * full_height + 1);
}
...
void EditLineStyleWidget::paintEvent(QPaintEvent *) {
  ...
  for (unsigned int i = 0; i < (unsigned int)full_size; ++i) {
    painter.drawLine(i * stipple_pixel_size, 0, i * stipple_pixel_size,
                     full_height * stipple_pixel_size);
  }
  ...
  painter.drawLine(full_size * stipple_pixel_size, 0,
                   ...
                   full_height * stipple_pixel_size);
  painter.drawLine(0, full_height * stipple_pixel_size,
                   full_size * stipple_pixel_size,
                   full_height * stipple_pixel_size);
