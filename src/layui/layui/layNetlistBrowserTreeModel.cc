...
NetlistBrowserTreeModel::NetlistBrowserTreeModel(QWidget *parent,
                                                 db::Netlist *netlist)
...
QVariant NetlistBrowserTreeModel::data(const QModelIndex &index,
                                       int role) const {
  if (!index.isValid()) {
    return QVariant();
  }
...
QString NetlistBrowserTreeModel::text(const QModelIndex &index) const {
  std::pair<const db::Circuit *, const db::Circuit *> circuits =
      circuits_from_index(index);
...
QString NetlistBrowserTreeModel::search_text(const QModelIndex &index) const {
  std::pair<const db::Circuit *, const db::Circuit *> circuits =
      circuits_from_index(index);
  return tl::to_qstring(search_string_from_names(circuits));
}

std::pair<std::pair<const db::Circuit *, const db::Circuit *>,
          std::pair<db::NetlistCrossReference::Status, std::string>>
NetlistBrowserTreeModel::cp_status_from_index(const QModelIndex &index,
                                              size_t &nprod, size_t &nlast,
                                              size_t &nnlast) const {
  typedef std::pair<std::pair<const db::Circuit *, const db::Circuit *>,
                    std::pair<db::NetlistCrossReference::Status, std::string>>
      cp_status;
...
void NetlistBrowserTreeModel::build_circuits_to_index(
    size_t nprod,
    const std::pair<const db::Circuit *, const db::Circuit *> &circuits,
    IndexedNetlistModel *model, const QModelIndex &index,
    std::map<std::pair<const db::Circuit *, const db::Circuit *>, QModelIndex>
        &map) const {
  ...
  for (size_t n = count; n > 0;) {
    std::pair<std::pair<const db::Circuit *, const db::Circuit *>,
              std::pair<db::NetlistCrossReference::Status, std::string>>
        cp = mp_indexer->child_circuit_from_index(circuits, n - 1);
    QModelIndex child_index = createIndex(
        int(n - 1), 0,
        reinterpret_cast<void *>(size_t(index.internalPointer()) + nprod * n));
...
static bool
is_compatible(const std::pair<const db::Circuit *, const db::Circuit *> &a,
              const std::pair<const db::Circuit *, const db::Circuit *> &b) {
...
QModelIndex NetlistBrowserTreeModel::index_from_netpath(
    const NetlistObjectsPath &path) const {
  QModelIndex idx;

  idx = index_from_circuits(path.root);

  for (NetlistObjectsPath::path_iterator p = path.path.begin();
       p != path.path.end() && idx.isValid(); ++p) {

    std::pair<const db::Circuit *, const db::Circuit *> sc(
        p->first ? p->first->circuit_ref() : 0,
        p->second ? p->second->circuit_ref() : 0);
    std::pair<const db::Circuit *, const db::Circuit *> circuit =
        circuits_from_index(idx);

    size_t count = mp_indexer->child_circuit_count(circuit);
    for (size_t n = count; n > 0;) {
      --n;
      std::pair<const db::Circuit *, const db::Circuit *> cc =
          mp_indexer->child_circuit_from_index(circuit, n).first;
...
QModelIndex NetlistBrowserTreeModel::index_from_circuits(
  ...
  if (m_circuits_to_index.empty()) {
    ...
    for (size_t n = count; n > 0;) {
      std::pair<std::pair<const db::Circuit *, const db::Circuit *>,
                std::pair<db::NetlistCrossReference::Status, std::string>>
          cp = mp_indexer->top_circuit_from_index(n - 1);
      build_circuits_to_index(
          count + 1, cp.first, mp_indexer.get(),
          createIndex(int(n - 1), 0, reinterpret_cast<void *>(n)),
          m_circuits_to_index);
      --n;
    }
  }
  ...
  std::map<std::pair<const db::Circuit *, const db::Circuit *>,
           QModelIndex>::const_iterator m = m_circuits_to_index.find(circuits);
...
QVariant NetlistBrowserTreeModel::tooltip(const QModelIndex &index) const {
  typedef std::pair<std::pair<const db::Circuit *, const db::Circuit *>,
                    std::pair<db::NetlistCrossReference::Status, std::string>>
      cp_status;
  size_t nlast = 0;
...
QVariant NetlistBrowserTreeModel::headerData(int section,
...
QModelIndex NetlistBrowserTreeModel::index(int row, int column,
  ...
  if (!parent.isValid()) {
    ...
    return createIndex(row, column,
                       reinterpret_cast<void *>(reinterpret_cast<size_t>(id) +
                                                size_t(row + 1) * nprod));
  }
}

QModelIndex NetlistBrowserTreeModel::parent(const QModelIndex &index) const {
  if (index.isValid()) {
    ...
    if (nprod > nlast) {
      ...
      return createIndex(int(ids / nprod - 1), 0,
