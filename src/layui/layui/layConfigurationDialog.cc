...
ConfigurationDialog::ConfigurationDialog(QWidget *parent, lay::Dispatcher *root,
                                         const std::string &plugin_name,
                                         const char *name)
    : QDialog(parent), mp_root(root) {
  mp_ui = 0;
  ...
  setWindowTitle(tl::to_qstring(
      (tl::to_string(QObject::tr("Configuration")) + " - " + config_title)));

  for (tl::Registrar<lay::PluginDeclaration>::iterator p =
...
void ConfigurationDialog::init(const lay::PluginDeclaration *decl) {
  ...
  std::vector<std::pair<std::string, lay::ConfigPage *>> pages =
      decl->config_pages(mp_ui->centralFrame);
  ...
  for (std::vector<lay::ConfigPage *>::iterator p = m_config_pages.begin();
       p != m_config_pages.end(); ++p) {
    if ((*p)->layout()) {
  ...
  setWindowTitle(tl::to_qstring(tl::to_string(QObject::tr("Configuration")) +
                                " - " + config_title));
}
