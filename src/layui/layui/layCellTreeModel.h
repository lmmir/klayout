...
class CellTreeModel : public QAbstractItemModel, public tl::Object {
public:
  enum Flags {
    ...
    NoPadding = 128    //  disable padding of display string with a blank at the
                       //  beginning and end
  };
   ...
   * current state flags. The flags member is a combination of the flags given
   * by If flags "Children" or "Parents" are given, "base" must be set to the
   * cell of which the children or parents should be derived.
   */
  CellTreeModel(QWidget *parent, lay::LayoutViewBase *view, int cv_index,
                unsigned int flags = 0, const db::Cell *base = 0,
                Sorting sorting = ByName);

  /**
   *  @brief Constructor
   *
   *  This constructor does not take a view but rather a layout. It does not
   * display hidden status or similar.
   */
  CellTreeModel(QWidget *parent, db::Layout *layout, unsigned int flags = 0,
                const db::Cell *base = 0, Sorting sorting = ByName);

  /**
   *  @brief Constructor
   *
   *  This constructor does not take a view but rather a layout from a library.
   * It does not display hidden status or similar.
   */
  CellTreeModel(QWidget *parent, db::Library *library, unsigned int flags = 0,
                const db::Cell *base = 0, Sorting sorting = ByName);
   ...
   */
  void configure(LayoutViewBase *view, int cv_index, unsigned int flags = 0,
                 const db::Cell *base = 0, Sorting sorting = ByName);

  /**
   *  @brief Reconfigures the model with a pure Layout
   */
  void configure(db::Layout *layout, unsigned int flags = 0,
                 const db::Cell *base = 0, Sorting sorting = ByName);

  /**
   *  @brief Reconfigures the model with a pure Layout from a library
   */
  void configure(db::Library *library, unsigned int flags = 0,
                 const db::Cell *base = 0, Sorting sorting = ByName);
   ...
   */
  Sorting sorting() const { return m_sorting; }
  ...
  /**
   *  @brief Signal to the owner of the model that the data has changed (with an
   * int parameter)
   */
  void signal_data_changed_with_int(int) { signal_data_changed(); }
...
private:
  ...
  void do_configure(db::Layout *layout, db::Library *library,
                    LayoutViewBase *view, int cv_index, unsigned int flags,
...
class CellTreeItem {
public:
  CellTreeItem(const db::Layout *layout, bool is_pcell,
               unsigned int cell_or_pcell_index, bool flat,
               CellTreeModel::Sorting sorting);
  ~CellTreeItem();
  ...
  size_t tree_index() const { return m_tree_index; }

  void set_tree_index(size_t index) { m_tree_index = index; }
