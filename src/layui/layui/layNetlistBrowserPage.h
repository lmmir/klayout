 ...
 */
class NetlistBrowserPage : public QFrame,
...
public:
   ...
   *  the view and provide highlights for the selected markers inside the given
   * cellview.
   */
   ...
   */
  db::LayoutToNetlist *db() { return mp_database.get(); }
  ...
  /**
   *  @brief Selects a netlist object (a circuit, a subcircuit, a net or a
   * device)
   */
   ...
   * device)
   *
   *  This variant allows specifying a paired path using either an object from
   * the first, the second netlist of both.
   */
   ...
   */
  void set_window(lay::NetlistBrowserConfig::net_window_type window_type,
                  double window_dim);
   ...
   *
   *  @param color The color or an invalid color to take the default color for
   * selection
   ...
   */
  void set_highlight_style(tl::Color color, int line_width, int vertex_size,
                           int halo, int dither_pattern, int marker_intensity,
                           bool use_original_colors,
                           const lay::ColorPalette *auto_colors);

  /**
   *  @brief Gets a value indicating whether all items in the netlist tree are
   * shown (specifically for cross-reference DBs)
   */
  bool show_all() const { return m_show_all; }

  /**
   *  @brief Sets a value indicating whether all items in the netlist tree are
   * shown (specifically for cross-reference DBs)
   *
   *  If this property is set to false, only cross-reference entries with error
   * are shown.
   */
   ...
   */
  const lay::NetlistObjectsPath &current_path() const { return m_current_path; }
...
private:
  ...
  void adjust_view();
  void clear_markers();
  void highlight(const NetlistObjectsPath &current_path,
  ...
  bool produce_highlights_for_net(
  ...
  bool produce_highlights_for_device(const db::Device *device,
                                     size_t &n_markers,
                                     const std::vector<db::DCplxTrans> &tv);
  bool produce_highlights_for_circuit(const db::Circuit *circuit,
                                      size_t &n_markers,
                                      const std::vector<db::DCplxTrans> &tv);
  ...
  void current_tree_index_changed(QTreeView *hierarchy_tree,
                                  QTreeView *directory_tree,
                                  const QModelIndex &index);
  void current_index_changed(QTreeView *hierarchy_tree,
                             QTreeView *directory_tree,
                             const QModelIndex &index);
