...
class BrowseShapesConfigPage : public lay::ConfigPage,
...
class BrowseShapesForm : public lay::Browser, private Ui::BrowseShapesForm {
  Q_OBJECT
...
private:
  ...
  bool fill_cell_instances(const db::ICplxTrans &t, const db::Layout &layout,
                           const db::Cell *from, const db::Cell *to,
                           bool to_parent, const std::string &path,
