...
void HTMLItemDelegate::set_plain_text(bool pt) { m_plain_text = pt; }

void HTMLItemDelegate::set_icon_margin(int m) { m_icon_margin = m; }

void HTMLItemDelegate::set_icon_spacing(int s) { m_icon_spacing = s; }

void HTMLItemDelegate::set_text_margin(int m) { m_text_margin = m; }

void HTMLItemDelegate::set_text_height(int h) { m_text_height = h; }

void HTMLItemDelegate::set_text_width(int w) { m_text_width = w; }
...
void HTMLItemDelegate::paint(QPainter *painter,
  ...
  QStyle *style =
      option_v4.widget ? option_v4.widget->style() : QApplication::style();
  ...
  if (option_v4.state & QStyle::State_Selected) {
    ctx.palette.setColor(
        QPalette::Text,
        option_v4.palette.color(QPalette::Active, QPalette::HighlightedText));
  } else if (!is_enabled) {
    ctx.palette.setColor(
        QPalette::Text,
        option_v4.palette.color(QPalette::Disabled, QPalette::Text));
  } else {
    ctx.palette.setColor(QPalette::Text,
                         option_v4.palette.color(QPalette::Text));
  }

  QRect text_rect =
      style->subElementRect(QStyle::SE_ItemViewItemText, &option_v4);
  painter->save();
...
QSize HTMLItemDelegate::sizeHint(const QStyleOptionViewItem &option,
  ...
  const QAbstractItemView *view =
      dynamic_cast<const QAbstractItemView *>(option_v4.widget);
  QSize icon_size(0, 0);
  if (view) {
  ...
  return QSize(
      m_text_width + (has_icon ? icon_size.width() + m_icon_spacing : 0),
      std::max(has_icon ? icon_size.height() + 2 * m_icon_margin : 0, th));
}

bool HTMLItemDelegate::editorEvent(QEvent *event,
                                   QAbstractItemModel * /*model*/,
                                   const QStyleOptionViewItem &option,
                                   const QModelIndex &index) {
  if ((event->type() == QEvent::MouseButtonRelease ||
       event->type() == QEvent::MouseButtonPress) &&
      !m_plain_text && m_anchors_clickable) {
    ...
    QString a = doc.documentLayout()->anchorAt(mouse_event->pos() -
                                               text_rect.topLeft());
    if (!a.isNull()) {
      if (event->type() == QEvent::MouseButtonRelease) {
