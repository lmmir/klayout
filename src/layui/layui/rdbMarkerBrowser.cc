...
static struct {
  ...
  const char *string;
    ...
    {rdb::AnyCell, "any-cell"},     {rdb::DatabaseTop, "database-top"},
    {rdb::Current, "current-cell"}, {rdb::CurrentOrAny, "current-or-any-cell"},
    {rdb::Local, "local-cell"},
};

void MarkerBrowserContextModeConverter::from_string(
    const std::string &value, rdb::context_mode_type &mode) {
  for (unsigned int i = 0; i < sizeof(context_modes) / sizeof(context_modes[0]);
       ++i) {
  ...
  throw tl::Exception(tl::to_string(QObject::tr(
                          "Invalid marker database browser context mode: ")) +
                      value);
}
...
static struct {
  ...
  const char *string;
} window_modes[] = {{rdb::DontChange, "dont-change"},
                    {rdb::FitCell, "fit-cell"},
                    {rdb::FitMarker, "fit-marker"},
                    {rdb::Center, "center"},
                    {rdb::CenterSize, "center-size"}};

void MarkerBrowserWindowModeConverter::from_string(const std::string &value,
                                                   rdb::window_type &mode) {
  ...
  throw tl::Exception(tl::to_string(QObject::tr(
                          "Invalid marker database browser window mode: ")) +
                      value);
}

std::string MarkerBrowserWindowModeConverter::to_string(rdb::window_type mode) {
  for (unsigned int i = 0; i < sizeof(window_modes) / sizeof(window_modes[0]);
       ++i) {
...
void MarkerBrowserConfigPage::setup(lay::Dispatcher *root) {
  ...
  root->config_get(cfg_rdb_context_mode, cmode,
                   MarkerBrowserContextModeConverter());
  mp_ui->cbx_context->setCurrentIndex(int(cmode));

  //  window mode
  rdb::window_type wmode = rdb::FitMarker;
  root->config_get(cfg_rdb_window_mode, wmode,
                   MarkerBrowserWindowModeConverter());
...
void MarkerBrowserConfigPage::window_changed(int m) {
  mp_ui->mgn_window->setEnabled(m == int(rdb::FitMarker) ||
                                m == int(rdb::CenterSize));
}
...
void MarkerBrowserConfigPage::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_rdb_context_mode,
                   rdb::context_mode_type(mp_ui->cbx_context->currentIndex()),
                   MarkerBrowserContextModeConverter());
  root->config_set(cfg_rdb_window_mode,
                   rdb::window_type(mp_ui->cbx_window->currentIndex()),
                   MarkerBrowserWindowModeConverter());
  root->config_set(cfg_rdb_window_dim,
                   mp_ui->mgn_window->get_margin().to_string());
...
void MarkerBrowserConfigPage2::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_rdb_marker_dither_pattern,
...
class MarkerBrowserPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_options(std::vector<std::pair<std::string, std::string>> &options) const {
    options.push_back(std::pair<std::string, std::string>(cfg_rdb_context_mode,
                                                          "database-top"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_window_mode, "fit-marker"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_window_state, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_window_dim, "1.0"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_max_marker_count, "1000"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_rdb_marker_color, lay::ColorConverter().to_string(QColor())));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_marker_line_width, "-1"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_marker_vertex_size, "-1"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_rdb_marker_halo, "-1"));
    options.push_back(std::pair<std::string, std::string>(
        cfg_rdb_marker_dither_pattern, "-1"));
  }
  ...
  config_pages(QWidget *parent) const {
    ...
    pages.push_back(std::make_pair(
        tl::to_string(QObject::tr("Marker Database Browser|Setup")),
        new MarkerBrowserConfigPage(parent)));
    pages.push_back(std::make_pair(
        tl::to_string(QObject::tr("Marker Database Browser|Marker Appearance")),
        new MarkerBrowserConfigPage2(parent)));
    return pages;
  }
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::menu_item(
        "marker_browser::show", "browse_markers", "tools_menu.end",
        tl::to_string(QObject::tr("Marker Browser"))));
    menu_entries.push_back(lay::submenu(
        "marker_browser::show", "shapes_to_markers", "tools_menu.end",
        tl::to_string(QObject::tr("Shapes To Markers"))));
    menu_entries.push_back(
        lay::menu_item("marker_browser::scan_layers", "scan_layers",
                       "tools_menu.shapes_to_markers.end",
                       tl::to_string(QObject::tr("Hierarchical"))));
    menu_entries.push_back(lay::menu_item("marker_browser::scan_layers_flat",
                                          "scan_layers_flat",
                                          "tools_menu.shapes_to_markers.end",
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new MarkerBrowserPluginDeclaration(), 12000,
                "MarkerBrowserPlugin");
