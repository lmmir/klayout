...
LibraryTreeWidget::LibraryTreeWidget(QWidget *parent, const char *name,
...
bool LibraryTreeWidget::event(QEvent *event) {
#if 0
...
bool LibraryTreeWidget::focusNextPrevChild(bool /*next*/) { return false; }
...
void LibraryTreeWidget::mouseReleaseEvent(QMouseEvent *event) {
  if (event->button() == Qt::MiddleButton) {
    QModelIndex index(indexAt(event->pos()));
    if (index.isValid()) {
...
LibrariesView::LibrariesView(lay::LayoutViewBase *view, QWidget *parent,
                             const char *name)
    : QFrame(parent), m_enable_cb(true), mp_view(view), m_active_index(-1),
      m_split_mode(false),
      m_do_update_content_dm(this, &LibrariesView::do_update_content),
      m_do_full_update_content_dm(this,
...
QSize LibrariesView::sizeHint() const {
  int w = 120; // TODO: better(?): mp_cell_list->sizeHint ().width ();
  return QSize(w, 0);
}

bool LibrariesView::event(QEvent *e) {
...
void LibrariesView::context_menu(const QPoint &p) {
...
void LibrariesView::set_split_mode(bool f) {
  if (f != m_split_mode) {
    m_split_mode = f;
...
void LibrariesView::search_triggered(const QString &t) {
  mp_search_model = 0;
...
void LibrariesView::search_edited() {
  ...
  for (std::vector<QTreeView *>::const_iterator v = mp_cell_lists.begin();
    ...
    if ((*v)->model() == mp_search_model) {
      ...
      if (t.isEmpty()) {
        ...
        QModelIndex found = mp_search_model->locate(
...
void LibrariesView::search_next() {
  for (std::vector<QTreeView *>::const_iterator v = mp_cell_lists.begin();
       v != mp_cell_lists.end(); ++v) {
    if ((*v)->model() == mp_search_model) {
...
void LibrariesView::search_prev() {
  for (std::vector<QTreeView *>::const_iterator v = mp_cell_lists.begin();
       v != mp_cell_lists.end(); ++v) {
    if ((*v)->model() == mp_search_model) {
...
void LibrariesView::search_editing_finished() {
...
void LibrariesView::middle_clicked(const QModelIndex & /*index*/) {
  //  ... nothing yet ..
}

void LibrariesView::header_clicked() {
  //  ... nothing yet ..
}

void LibrariesView::clicked(const QModelIndex & /*index*/) {
  //  ... nothing yet ..
}

void LibrariesView::double_clicked(const QModelIndex & /*index*/) {
  //  ... nothing yet ..
}

void LibrariesView::set_background_color(tl::Color c) {
...
void LibrariesView::set_text_color(tl::Color c) {
  m_text_color = c;
  for (std::vector<QTreeView *>::const_iterator f = mp_cell_lists.begin();
       f != mp_cell_lists.end(); ++f) {
    QPalette pl((*f)->palette());
...
void LibrariesView::update_required() { m_do_full_update_content_dm(); }

void LibrariesView::do_full_update_content() {
  size_t i = 0;
  for (db::LibraryManager::iterator lib =
           db::LibraryManager::instance().begin();
       lib != db::LibraryManager::instance().end(); ++lib, ++i) {
...
void LibrariesView::do_update_content(int lib_index) {
  ...
  size_t imax =
      (lib_index < 0 ? std::numeric_limits<size_t>::max() : (size_t)lib_index);
  std::string tech_name;

  //  rebuild all events
  detach_from_all_events();

  mp_view->active_cellview_changed_event.add(this,
  ...
  db::LibraryManager::instance().changed_event.add(
      this, &LibrariesView::update_required);

  std::vector<db::Library *> libraries;
  for (db::LibraryManager::iterator lib =
           db::LibraryManager::instance().begin();
       lib != db::LibraryManager::instance().end(); ++lib) {
    ...
    if (!lib_ptr->for_technologies() || lib_ptr->is_for_technology(tech_name)) {
      ...
      libraries.back()->layout().hier_changed_event.add(
          this, &LibrariesView::update_required);
      libraries.back()->retired_state_changed_event.add(
          this, &LibrariesView::update_required);
    }
  }
  ...
  if (m_libraries.size() < libraries.size()) {
    ...
    m_libraries.erase(m_libraries.begin() + libraries.size(),
                      m_libraries.end());
  }
  ...
  while (mp_cell_lists.size() < m_libraries.size()) {
    ...
    LibraryTreeWidget *cell_list =
        new LibraryTreeWidget(cl_frame, "tree", mp_view->canvas()->widget());
    cl_ly->addWidget(cell_list);
    cell_list->setModel(new CellTreeModel(
        cell_list, m_libraries[i].get(),
        CellTreeModel::Flat | CellTreeModel::TopCells |
            CellTreeModel::BasicCells | CellTreeModel::WithVariants |
            CellTreeModel::WithIcons,
        0));
  ...
  for (size_t i = imin;
       i < m_libraries.size() && i < size_t(mp_selector->count()) && i <= imax;
       ++i) {
  ...
  for (size_t i = imin; i < m_libraries.size() && i <= imax; ++i) {
    ...
    if (m_needs_update[i]) {
      ...
      if (m_force_close[i]) {
        ...
        if (model) {
          model->configure(m_libraries[i].get(),
                           CellTreeModel::Flat | CellTreeModel::TopCells |
                               CellTreeModel::BasicCells |
                               CellTreeModel::WithVariants |
                               CellTreeModel::WithIcons,
                           0);
        }
      }
      ...
      mp_cell_lists[i]
          ->doItemsLayout(); //  triggers a redraw -> the model might need this
    }
...
void LibrariesView::select_active_lib_by_name(const std::string &name) {
  for (std::vector<tl::weak_ptr<db::Library>>::const_iterator i =
           m_libraries.begin();
       i != m_libraries.end(); ++i) {
...
void LibrariesView::select_active(int lib_index) {
...
db::Library *LibrariesView::active_lib() {
...
void LibrariesView::selection_changed(int index) {
...
std::string LibrariesView::display_string(int n) const {
  ...
  if (lib->for_technologies()) {
    text += " ";
    std::string tn =
        tl::join(std::vector<std::string>(lib->get_technologies().begin(),
                                          lib->get_technologies().end()),
                 ",");
    text +=
        tl::to_string(QObject::tr("[Technology %1]").arg(tl::to_qstring(tn)));
  }
  return text;
}

CellTreeItem *LibrariesView::current_item() const {
...
bool LibrariesView::has_focus() const {
...
class LibraryViewPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::submenu("@lib_context_menu", at, std::string()));
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new LibraryViewPluginDeclaration(), -7, "LibraryViewPlugin");
