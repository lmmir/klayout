 ...
 */
class LAYUI_PUBLIC CellSelectionForm : public QDialog {
  Q_OBJECT

public:
  CellSelectionForm(QWidget *parent, LayoutViewBase *view, const char *name,
 ...
 */
class LAYUI_PUBLIC LibraryCellSelectionForm : public QDialog {
  Q_OBJECT
...
public:
   ...
   *  always return 0.
   *  If all_cells is true, all cells (not just top cells and basic cells) are
   * shown. If top_cells_only is false, child cells are shown as well.
   */
  LibraryCellSelectionForm(QWidget *parent, db::Layout *layout,
                           const char *name, bool all_cells = false,
                           bool top_cells_only = true);

  /**
   *  @brief Create a selection form for cells plus the library
   *
   *  If all_cells is true, all cells (not only top cells and basic cells) are
   * shown. If top_cells_only is false, child cells are shown as well.
   */
  LibraryCellSelectionForm(QWidget *parent, const char *name,
                           bool all_cells = false, bool top_cells_only = true);
   ...
   */
  db::Library *get_current_library() const { return mp_lib; }
  ...
  /**
   *  @brief Select the PCell with the given ID (when BasicCells is selected in
   * the flags)
   */
   ...
   */
  db::cell_index_type selected_cell_index() const { return m_cell_index; }

  /**
   *  @brief Returns true, if the selected cell is a pcell
   */
  bool selected_cell_is_pcell() const { return m_is_pcell; }

  /**
   *  @brief Get the PCell ID of the selected cell (if it's a PCell)
   */
  db::pcell_id_type selected_pcell_id() const { return m_pcell_id; }
