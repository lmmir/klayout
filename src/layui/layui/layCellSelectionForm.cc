...
static const std::string cfg_cell_selection_search_case_sensitive(
    "cell-selection-search-case-sensitive");
static const std::string cfg_cell_selection_search_use_expressions(
    "cell-selection-search-use-expression");

// ------------------------------------------------------------

CellSelectionForm::CellSelectionForm(QWidget *parent, LayoutViewBase *view,
                                     const char *name, bool simple_mode)
    : QDialog(parent), mp_view(view), m_current_cv(-1), m_name_cb_enabled(true),
      m_cells_cb_enabled(true), m_children_cb_enabled(true),
  ...
  if (lay::Dispatcher::instance()) {
    bool cs = true;
    lay::Dispatcher::instance()->config_get(
        cfg_cell_selection_search_case_sensitive, cs);
    mp_case_sensitive->setChecked(cs);
    bool ue = true;
    lay::Dispatcher::instance()->config_get(
        cfg_cell_selection_search_use_expressions, ue);
  ...
  connect(mp_ui->cb_views, SIGNAL(activated(int)), this,
          SLOT(view_changed(int)));
  ...
  connect(mp_ui->le_cell_name, SIGNAL(textChanged(const QString &)), this,
          SLOT(name_changed()));
  ...
  connect(mp_ui->le_cell_name, SIGNAL(tab_pressed()), this,
          SLOT(find_next_clicked()));
  connect(mp_ui->le_cell_name, SIGNAL(backtab_pressed()), this,
          SLOT(find_prev_clicked()));

  connect(mp_ui->lv_parents, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(parent_changed(const QModelIndex &)));
  connect(mp_ui->lv_children, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(child_changed(const QModelIndex &)));
  ...
  if (!m_cellviews.empty()) {
    ...
    for (std::vector<lay::CellView>::const_iterator cv = m_cellviews.begin();
         cv != m_cellviews.end(); ++cv, ++cvi) {
      mp_ui->cb_views->addItem(
          tl::to_qstring((*cv)->name() + " (@" + tl::to_string(cvi + 1) + ")"));
    }
...
void CellSelectionForm::update_cell_list() {
  ...
  lay::CellTreeModel *model = new lay::CellTreeModel(
      mp_ui->lv_cells, mp_view, m_current_cv, lay::CellTreeModel::Flat);

  mp_ui->lv_cells->setModel(model);
  //  connect can only happen after setModel()
  connect(mp_ui->lv_cells->selectionModel(),
          SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
          this, SLOT(cell_changed(const QModelIndex &, const QModelIndex &)));

  lay::CellView::unspecific_cell_path_type path(
      m_cellviews[m_current_cv].combined_unspecific_path());
  if (!path.empty()) {
...
void CellSelectionForm::update_parents_list() {
  ...
  if (m_current_cv >= 0 && m_current_cv < int(m_cellviews.size())) {
    ...
    lay::CellTreeModel *model =
        dynamic_cast<lay::CellTreeModel *>(mp_ui->lv_cells->model());
    if (model) {

      if (mp_ui->lv_parents->model()) {
        delete mp_ui->lv_parents->model();
      }
      mp_ui->lv_parents->setModel(new lay::CellTreeModel(
          mp_ui->lv_parents, mp_view, m_current_cv,
          lay::CellTreeModel::Flat | lay::CellTreeModel::Parents,
          model->cell(mp_ui->lv_cells->selectionModel()->currentIndex())));
    }
  }
...
void CellSelectionForm::update_children_list() {
  ...
  if (m_current_cv >= 0 && m_current_cv < int(m_cellviews.size())) {
    ...
    if (model) {
      ...
      mp_ui->lv_children->setModel(new lay::CellTreeModel(
          mp_ui->lv_children, mp_view, m_current_cv,
          lay::CellTreeModel::Flat | lay::CellTreeModel::Children,
          model->cell(mp_ui->lv_cells->selectionModel()->currentIndex())));
    }
  }
...
int CellSelectionForm::selected_cellview_index() const { return m_current_cv; }

const lay::CellView &CellSelectionForm::selected_cellview() const {
...
void CellSelectionForm::store_config() {
  if (lay::Dispatcher::instance()) {
    lay::Dispatcher::instance()->config_set(
        cfg_cell_selection_search_case_sensitive,
        mp_case_sensitive->isChecked());
    lay::Dispatcher::instance()->config_set(
        cfg_cell_selection_search_use_expressions,
        mp_use_regular_expressions->isChecked());
  }
}
...
void CellSelectionForm::cell_changed(const QModelIndex &current,
                                     const QModelIndex &) {
...
void CellSelectionForm::child_changed(const QModelIndex &current) {
  if (m_children_cb_enabled && current.isValid()) {
    if (m_current_cv >= 0 && m_current_cv < int(m_cellviews.size())) {
      lay::CellTreeModel *model =
          dynamic_cast<lay::CellTreeModel *>(mp_ui->lv_children->model());
      if (model) {
        select_entry(model->cell_index(
            mp_ui->lv_children->selectionModel()->currentIndex()));
      }
    }
  }
}
...
void CellSelectionForm::parent_changed(const QModelIndex &current) {
  if (m_parents_cb_enabled && current.isValid()) {
    if (m_current_cv >= 0 && m_current_cv < int(m_cellviews.size())) {
      lay::CellTreeModel *model =
          dynamic_cast<lay::CellTreeModel *>(mp_ui->lv_parents->model());
      if (model) {
        select_entry(model->cell_index(
            mp_ui->lv_parents->selectionModel()->currentIndex()));
      }
    }
  }
}
...
void CellSelectionForm::select_entry(lay::CellView::cell_index_type ci) {
  ...
  if (mi.isValid()) {
    ...
    m_cells_cb_enabled = false;
    mp_ui->lv_cells->selectionModel()->setCurrentIndex(
        mi, QItemSelectionModel::Clear | QItemSelectionModel::SelectCurrent);
    ...
    //  do child list updates in a user event handler. Otherwise changing the
    //  models immediately interferes with Qt's internal logic. So we do an
    //  deferred update.
    m_update_all_dm();
  }
...
void CellSelectionForm::find_next_clicked() {
  ...
  if (mi.isValid()) {
    ...
    m_cells_cb_enabled = false;
    mp_ui->lv_cells->selectionModel()->setCurrentIndex(
        mi, QItemSelectionModel::SelectCurrent);
    mp_ui->lv_cells->scrollTo(mi);
    update_children_list();
    update_parents_list();
    m_cells_cb_enabled = true;
...
void CellSelectionForm::name_changed() {
  if (m_name_cb_enabled) {
    ...
    QModelIndex mi;
    if (!s.isEmpty()) {
      mi = model->locate(tl::to_string(s).c_str(),
                         mp_use_regular_expressions->isChecked(),
                         mp_case_sensitive->isChecked());
    } else {
      model->clear_locate();
...
LibraryCellSelectionForm::LibraryCellSelectionForm(QWidget *parent,
                                                   db::Layout *layout,
                                                   const char *name,
                                                   bool all_cells,
                                                   bool top_cells_only)
    : QDialog(parent), mp_lib(0), mp_layout(layout), m_name_cb_enabled(true),
      m_cells_cb_enabled(true), m_cell_index(-1), m_pcell_id(-1),
      m_is_pcell(false), m_all_cells(all_cells),
  ...
  connect(mp_ui->find_next, SIGNAL(clicked()), this, SLOT(find_next_clicked()));
  connect(mp_ui->cb_show_all_cells, SIGNAL(clicked()), this,
          SLOT(show_all_changed()));
...
LibraryCellSelectionForm::LibraryCellSelectionForm(QWidget *parent,
                                                   ...
                                                   bool all_cells,
                                                   bool top_cells_only)
    : QDialog(parent), mp_lib(0), mp_layout(0), m_name_cb_enabled(true),
      m_cells_cb_enabled(true), m_cell_index(-1), m_pcell_id(-1),
      m_is_pcell(false), m_all_cells(all_cells),
      m_top_cells_only(top_cells_only) {
  mp_ui = new Ui::LibraryCellSelectionForm();
  ...
  connect(mp_ui->le_cell_name, SIGNAL(textChanged(const QString &)), this,
          SLOT(name_changed(const QString &)));
  connect(mp_ui->find_next, SIGNAL(clicked()), this, SLOT(find_next_clicked()));
  connect(mp_ui->lib_cb, SIGNAL(currentIndexChanged(int)), this,
          SLOT(lib_changed()));
  connect(mp_ui->cb_show_all_cells, SIGNAL(clicked()), this,
          SLOT(show_all_changed()));
...
void LibraryCellSelectionForm::update_cell_list() {
  ...
  if (mp_layout) {
    ...
    lay::CellTreeModel *model = new lay::CellTreeModel(
        mp_ui->lv_cells, const_cast<db::Layout *>(mp_layout), flags);
...
void LibraryCellSelectionForm::cell_changed(const QModelIndex &current,
                                            const QModelIndex &) {
  if (m_cells_cb_enabled) {
