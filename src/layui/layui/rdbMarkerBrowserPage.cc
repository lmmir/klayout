...
struct FlagDescriptor {
  FlagDescriptor(const std::string &i, const std::string &t,
                 const std::string &x)
      : icon(i), text(t), tag(x) {}
...
static FlagDescriptor flag_descriptors[] = {
    FlagDescriptor(":no_flag_16px.png", tl::to_string(QObject::tr("No flag")),
                   ""),
    FlagDescriptor(":red_flag_16px.png", tl::to_string(QObject::tr("Red flag")),
                   "red"),
    FlagDescriptor(":green_flag_16px.png",
                   tl::to_string(QObject::tr("Green flag")), "green"),
    FlagDescriptor(":blue_flag_16px.png",
                   tl::to_string(QObject::tr("Blue flag")), "blue"),
    FlagDescriptor(":yellow_flag_16px.png",
                   tl::to_string(QObject::tr("Yellow flag")), "yellow")};
...
struct SortByKeyCompareFunc {
  ...
  bool operator()(MarkerBrowserTreeViewModelCacheEntry *a,
                  MarkerBrowserTreeViewModelCacheEntry *b);
...
public:
  ...
  ~MarkerBrowserTreeViewModelCacheEntry() { clear(); }

  void clear() {
    for (std::vector<MarkerBrowserTreeViewModelCacheEntry *>::iterator i =
             m_ids.begin();
         i != m_ids.end(); ++i) {
      delete *i;
    }
  ...
  MarkerBrowserTreeViewModelCacheEntry *parent() const { return mp_parent; }

  void set_cache_valid(bool c) { m_id = (m_id & ~1l) + c; }

  bool cache_valid() const { return (m_id & 1) != 0; }

  void set_branch(unsigned int b) { m_id = (m_id & ~6l) + b; }

  unsigned int branch() const { return (m_id & 6l) >> 1; }

  void set_id(rdb::id_type id) { m_id = (id << 3) + (m_id & 7l); }

  rdb::id_type id() const { return (m_id >> 3); }
  ...
  size_t children() const { return m_ids.size(); }
  ...
  void sort_by_key_name(bool ascending, const rdb::Database *database) {
    std::sort(m_ids.begin(), m_ids.end(),
              SortByKeyCompareFunc(ascending, database));
    unsigned int r = 0;
    for (std::vector<MarkerBrowserTreeViewModelCacheEntry *>::iterator c =
             m_ids.begin();
         c != m_ids.end(); ++c) {
      (*c)->m_row = r++;
  ...
  void sort_by_count(bool ascending, const rdb::Database *database) {
    std::sort(m_ids.begin(), m_ids.end(),
              SortByCountCompareFunc(ascending, database));
    unsigned int r = 0;
    for (std::vector<MarkerBrowserTreeViewModelCacheEntry *>::iterator c =
             m_ids.begin();
         c != m_ids.end(); ++c) {
...
bool SortByKeyCompareFunc::operator()(MarkerBrowserTreeViewModelCacheEntry *a,
                                      MarkerBrowserTreeViewModelCacheEntry *b) {
...
bool SortByCountCompareFunc::operator()(
    MarkerBrowserTreeViewModelCacheEntry *a,
    MarkerBrowserTreeViewModelCacheEntry *b) {
...
static bool cell_matches_filter(const rdb::Cell *cell, const QString &filter) {
  return tl::to_qstring(cell->name()).indexOf(filter, 0, Qt::CaseInsensitive) >=
         0;
}

/**
 *  @brief Returns true if the given category or one of the sub-categories
 * matches the given filter
 */
static bool cat_matches_filter(const rdb::Category *cat, const QString &filter,
                               bool recursive) {
  if (tl::to_qstring(cat->name()).indexOf(filter, 0, Qt::CaseInsensitive) >=
      0) {
    return true;
  }

  if (recursive) {
    for (rdb::Categories::const_iterator c = cat->sub_categories().begin();
         c != cat->sub_categories().end(); ++c) {
...
class MarkerBrowserTreeViewModel : public QAbstractItemModel {
public:
  enum Sorting { ByKeyName, ByCount };

  MarkerBrowserTreeViewModel() : mp_database(0), m_show_empty_ones(true) {
    // .. nothing yet ..
  }
    ...
    if (mp_database) {
      ...
      for (QModelIndexList::const_iterator i = pi.begin(); i != pi.end(); ++i) {
        ids.push_back(std::make_pair(
            (MarkerBrowserTreeViewModelCacheEntry *)i->internalPointer(),
            i->column()));
      }
      ...
      for (std::vector<std::pair<MarkerBrowserTreeViewModelCacheEntry *,
                                 int>>::const_iterator i = ids.begin();
           i != ids.end(); ++i) {
  ...
  bool cat_matches(const QModelIndex &index, const QString &filter) const {
    MarkerBrowserTreeViewModelCacheEntry *node =
        (MarkerBrowserTreeViewModelCacheEntry *)(index.internalPointer());
    if (node && mp_database) {
    ...
    if (role == Qt::DisplayRole) {
      ...
      if (node) {
        ...
        if (index.column() == 1) {
          ...
          if (node->count() > 0) {
            ...
            if (visited < node->count()) {
              return QVariant(tl::to_qstring(
                  tl::sprintf(tl::to_string(QObject::tr("%lu (%lu)")),
                              node->count(), node->count() - visited)));
            } else {
          ...
          //  On the first level, the id is 0, 1 or 2 reflecting the three top
          //  nodes
          if (id == 0) {
  ...
  QModelIndex index(int row, int column, const QModelIndex &parent) const {
    if (!mp_database) {
      ...
      MarkerBrowserTreeViewModelCacheEntry *parent_node =
          (MarkerBrowserTreeViewModelCacheEntry *)parent.internalPointer();
      if (parent_node) {
  ...
  QModelIndex next_index(QModelIndex current_index, bool up) {
    ...
    MarkerBrowserTreeViewModelCacheEntry *node =
        (MarkerBrowserTreeViewModelCacheEntry *)current_index.internalPointer();
    ...
    while (current_index.isValid()) {
      ...
      current_index = index(current_index.row() + (up ? -1 : 1),
                            current_index.column(), parent_index);
      ...
      if (current_index.isValid()) {
        ...
        while (current_index.isValid()) {
          ...
          node = (MarkerBrowserTreeViewModelCacheEntry *)
                     current_index.internalPointer();
          rdb::id_type id = node->id();
          if (mp_database->cell_by_id(id) != 0 && !descend_into_cell) {
            break;
          } else if (mp_database->category_by_id(id) != 0 &&
                     !descend_into_category) {
            break;
          }

          parent_index = current_index;
          current_index = index(up ? (rowCount(current_index) - 1) : 0,
                                current_index.column(), current_index);
        }
...
private:
  ...
  void invalidate() {
    ...
    MarkerBrowserTreeViewModelCacheEntry *by_cell_node =
        new MarkerBrowserTreeViewModelCacheEntry(0, 0);
    m_cache.add_child(by_cell_node);

    MarkerBrowserTreeViewModelCacheEntry *by_category_node =
        new MarkerBrowserTreeViewModelCacheEntry(0, 1);
    m_cache.add_child(by_category_node);

    MarkerBrowserTreeViewModelCacheEntry *all_node =
        new MarkerBrowserTreeViewModelCacheEntry(0, 2);
  ...
  QModelIndex createNodeIndex(MarkerBrowserTreeViewModelCacheEntry *node,
                              int column) const {
    if (node) {
  ...
  void add_sub_categories(MarkerBrowserTreeViewModelCacheEntry *node) const {
    ...
    if (category) {
      for (rdb::Categories::const_iterator c =
               category->sub_categories().begin();
           c != category->sub_categories().end(); ++c) {

        node->set_cache_valid(true);

        MarkerBrowserTreeViewModelCacheEntry *child =
            new MarkerBrowserTreeViewModelCacheEntry(c->id(), node->branch());
  ...
  void add_sub_categories(id_type cell_id,
                          MarkerBrowserTreeViewModelCacheEntry *node,
                          std::set<rdb::id_type> &partial_tree) const {
    node->set_cache_valid(true);
  ...
  void update_cache(MarkerBrowserTreeViewModelCacheEntry *node) const {
    ...
    if (id == 0) {
      ...
      if (branch == 0) {
        ...
        for (rdb::Database::const_cell_iterator c =
                 mp_database->cells().begin();
             c != mp_database->cells().end(); ++c) {
          if (mp_database->cell_by_id(c->id()) &&
              (m_show_empty_ones ||
               mp_database->cell_by_id(c->id())->num_items() != 0)) {
            MarkerBrowserTreeViewModelCacheEntry *child =
                new MarkerBrowserTreeViewModelCacheEntry(c->id(), branch);
        ...
        for (rdb::Categories::const_iterator c =
                 mp_database->categories().begin();
             c != mp_database->categories().end(); ++c) {
          if (mp_database->category_by_id(c->id()) &&
              (m_show_empty_ones ||
               mp_database->category_by_id(c->id())->num_items() != 0)) {
            MarkerBrowserTreeViewModelCacheEntry *child =
                new MarkerBrowserTreeViewModelCacheEntry(c->id(), branch);
            child->set_count(mp_database->category_by_id(c->id())->num_items());
            node->add_child(child);
      ...
      if (cell) {
        ...
        //  look up all categories used inside this cell and determine top-level
        //  categories to insert into the cell node.

        std::set<rdb::id_type> category_ids;
        std::pair<rdb::Database::const_item_ref_iterator,
                  rdb::Database::const_item_ref_iterator>
            be = mp_database->items_by_cell(id);
        for (rdb::Database::const_item_ref_iterator c = be.first;
             c != be.second; ++c) {
            ...
            if (cat) {
              ...
              do {
                ...
                cat = cat->parent();
                       ...
                       category_ids.find(cat->id()) == category_ids.end());
            }
          }
        }
      ...
      if (category) {
        ...
        std::pair<rdb::Database::const_item_ref_iterator,
                  rdb::Database::const_item_ref_iterator>
            be = mp_database->items_by_category(id);
        for (rdb::Database::const_item_ref_iterator c = be.first;
             c != be.second; ++c) {
          cell_ids.insert((*c)->cell_id());
        }

        for (std::set<rdb::id_type>::const_iterator c = cell_ids.begin();
             c != cell_ids.end(); ++c) {

          MarkerBrowserTreeViewModelCacheEntry *child =
              new MarkerBrowserTreeViewModelCacheEntry(*c, branch);
...
template <class Iter> struct ValueIterSorter {
  ValueIterSorter(rdb::id_type tag_id) : m_tag_id(tag_id) {}
  ...
  bool operator()(Iter a, Iter b) {
    ...
    for (rdb::Values::const_iterator i = ia.values().begin();
         i != ia.values().end() && !va; ++i) {
      if (i->tag_id() == m_tag_id) {
        va = i->get();
      }
    }

    for (rdb::Values::const_iterator i = ib.values().begin();
         i != ib.values().end() && !vb; ++i) {
      if (i->tag_id() == m_tag_id) {
        vb = i->get();
      }
    }
...
class MarkerBrowserListViewModel : public QAbstractItemModel {
public:
  MarkerBrowserListViewModel()
      : mp_database(0), m_sorting(-1), m_sorting_order(false) {
    for (size_t i = 0; i < sizeof(m_flag_tag_ids) / sizeof(m_flag_tag_ids[0]);
         ++i) {
      m_flag_tag_ids[i] = 0;
    }
    m_waived_tag_id = 0;
  ...
  QModelIndex index_of_row(int row) { return createIndex(row, 0, (void *)0); }
  ...
  template <class Iter>
  bool set_items(const std::vector<std::pair<Iter, Iter>> &be_vector,
                 size_t max_marker_count) {
    beginResetModel();

    typedef Iter iterator_type;
    typedef std::pair<Iter, Iter> iterator_pair_type;
    typedef typename std::vector<iterator_pair_type>::const_iterator
        ipv_iterator_type;

    size_t n = 0;
    bool clipped = false;

    for (ipv_iterator_type be = be_vector.begin();
         be != be_vector.end() && n < max_marker_count; ++be) {
    ...
    if (m_sorting == 0 || m_sorting == 1 || m_sorting == 2) {
      ...
      id_type
          tags_in_order[sizeof(flag_descriptors) / sizeof(flag_descriptors[0])];
      size_t n_tags = 0;
      ...
      tl::sort(ii.begin(), ii.end(),
               ValueIterSorter<Iter>(m_user_tags[m_sorting - 4].second));
  ...
  void set_database(const rdb::Database *db) {
    ...
    if (mp_database) {
      ...
      m_user_tags.clear();
      for (rdb::Tags::const_iterator t = mp_database->tags().begin_tags();
           t != mp_database->tags().end_tags(); ++t) {
        if (t->is_user_tag()) {
      ...
      for (unsigned int j = 1;
           j < sizeof(flag_descriptors) / sizeof(flag_descriptors[0]); ++j) {
        m_flag_tag_ids[j] =
            mp_database->tags().tag(flag_descriptors[j].tag).id();
      }
  ...
  QVariant headerData(int section, Qt::Orientation /*orientation*/,
    ...
    if (role == Qt::DisplayRole) {
      if (section == 0) {
        ...
        return QVariant(
            QString::fromUtf8(m_user_tags[section - 4].first.c_str()));
      }
    }
    ...
    if (role == Qt::DecorationRole) {
      ...
      if (index.column() == 0) {
        ...
        if (i != 0) {
          ...
          for (unsigned int j = 1;
               j < sizeof(flag_descriptors) / sizeof(flag_descriptors[0]);
               ++j) {
      ...
      if (index.column() > 3 && index.column() - 4 < int(m_user_tags.size())) {
        ...
        if (i != 0) {
          ...
          for (rdb::Values::const_iterator v = i->values().begin();
               v != i->values().end(); ++v) {
            ...
            if (v->get() && v->tag_id() == tag_id) {
              ...
              if (value_string.size() > max_length) {
                value_string = std::string(value_string.begin(),
                                           value_string.begin() + max_length) +
                               "...";
              }
        ...
        if (i == 0) {
          ...
          const rdb::Category *cat =
              mp_database->category_by_id(i->category_id());
          ...
          for (rdb::Values::const_iterator v = i->values().begin();
               v != i->values().end(); ++v) {
            if (v->tag_id() == 0 && v->get() &&
                (v->get()->type_index() == rdb::type_index_of<std::string>() ||
  ...
  QModelIndex index(int row, int column, const QModelIndex &parent) const {
    if (mp_database && !parent.isValid() && row >= 0 &&
        row < int(m_item_list.size())) {
...
private:
  ...
  id_type
      m_flag_tag_ids[sizeof(flag_descriptors) / sizeof(flag_descriptors[0])];
...
class MarkerBrowserSnapshotView : public QDialog,
...
MarkerBrowserPage::MarkerBrowserPage(QWidget * /*parent*/)
    : m_enable_updates(true), m_update_needed(false), mp_database(0),
      m_show_all(true), m_list_shapes(true), mp_view(0), m_cv_index(0),
      m_num_items(0), m_view_changed(false), m_recursion_sentinel(false),
      m_in_directory_selection_change(false), m_context(rdb::DatabaseTop),
      m_window(rdb::FitMarker), m_window_dim(0.0), m_max_marker_count(1000),
      m_marker_line_width(-1), m_marker_vertex_size(-1), m_marker_halo(-1),
      m_marker_dither_pattern(-1), m_current_flag(0),
      m_marker_list_sorted_section(-1),
      m_marker_list_sort_order(Qt::DescendingOrder),
      m_directory_tree_sorted_section(-1),
      m_directory_tree_sort_order(Qt::DescendingOrder), mp_plugin_root(0),
  ...
  for (unsigned int i = 0;
       i < sizeof(flag_descriptors) / sizeof(flag_descriptors[0]); ++i) {
    QAction *action =
        flags_menu->addAction(QIcon(tl::to_qstring(flag_descriptors[i].icon)),
                              tl::to_qstring(flag_descriptors[i].text), this,
                              SLOT(flag_menu_selected()));
  ...
  connect(directory_tree->header(),
          SIGNAL(sortIndicatorChanged(int, Qt::SortOrder)), this,
          SLOT(directory_sorting_changed(int, Qt::SortOrder)));
  connect(markers_list->header(),
          SIGNAL(sortIndicatorChanged(int, Qt::SortOrder)), this,
          SLOT(markers_sorting_changed(int, Qt::SortOrder)));
#else
  connect(directory_tree->header(), SIGNAL(sectionClicked(int)), this,
          SLOT(directory_header_clicked(int)));
  connect(markers_list->header(), SIGNAL(sectionClicked(int)), this,
          SLOT(markers_header_clicked(int)));
#endif
  ...
  connect(markers_list, SIGNAL(doubleClicked(const QModelIndex &)), this,
          SLOT(marker_double_clicked(const QModelIndex &)));
  ...
  connect(important_pb, SIGNAL(clicked()), this,
          SLOT(important_button_clicked()));
  ...
  connect(nophoto_pb, SIGNAL(clicked()), this,
          SLOT(remove_snapshot_button_clicked()));
  connect(info_text, SIGNAL(anchorClicked(const QUrl &)), this,
          SLOT(info_anchor_clicked(const QUrl &)));
  connect(cat_filter, SIGNAL(textEdited(const QString &)), this,
          SLOT(filter_changed()));
  connect(cell_filter, SIGNAL(textEdited(const QString &)), this,
          SLOT(filter_changed()));
  ...
  connect(m_show_all_action, SIGNAL(triggered()), this,
          SLOT(show_all_clicked()));

  QAction *revisit_non_waived_action =
      new QAction(QObject::tr("Revisit Non-Waived Markers"), this);
  connect(revisit_non_waived_action, SIGNAL(triggered()), this,
          SLOT(revisit_non_waived()));
  QAction *revisit_important_action =
      new QAction(QObject::tr("Revisit Important Markers"), this);
  connect(revisit_important_action, SIGNAL(triggered()), this,
          SLOT(revisit_important()));
  ...
  QAction *mark_important_action =
      new QAction(QObject::tr("Mark Important"), this);
  connect(mark_important_action, SIGNAL(triggered()), this,
          SLOT(mark_important()));
  QAction *mark_unimportant_action =
      new QAction(QObject::tr("Mark Unimportant"), this);
  connect(mark_unimportant_action, SIGNAL(triggered()), this,
          SLOT(mark_unimportant()));
  ...
  QAction *mark_notvisited_action =
      new QAction(QObject::tr("Mark Not Visited"), this);
  connect(mark_notvisited_action, SIGNAL(triggered()), this,
          SLOT(mark_notvisited()));
...
void MarkerBrowserPage::set_dispatcher(lay::Dispatcher *pr) {
  mp_plugin_root = pr;
}

void MarkerBrowserPage::set_marker_style(tl::Color color, int line_width,
                                         int vertex_size, int halo,
                                         int dither_pattern) {
...
void MarkerBrowserPage::set_view(lay::LayoutViewBase *view,
...
static void set_hidden_rec(MarkerBrowserTreeViewModel *model,
                           QTreeView *tree_view, const QModelIndex &parent,
                           bool show_all, const QString &cat_filter,
                           const QString &cell_filter) {
  int rows = model->rowCount(parent);
  for (int r = 0; r < rows; ++r) {

    QModelIndex index = model->index(r, 0, parent);
    bool hidden =
        (!show_all && model->no_errors(index)) ||
...
void MarkerBrowserPage::filter_changed() {
  MarkerBrowserTreeViewModel *tree_model =
      dynamic_cast<MarkerBrowserTreeViewModel *>(directory_tree->model());
  if (tree_model) {
    set_hidden_rec(tree_model, directory_tree, QModelIndex(), m_show_all,
                   cat_filter->text(), cell_filter->text());
  }

  update_marker_list(2 /*select all*/);
}

void MarkerBrowserPage::show_all(bool f) {
  if (f != m_show_all) {
...
void MarkerBrowserPage::list_shapes(bool f) {
  if (f != m_list_shapes) {
...
void MarkerBrowserPage::set_rdb(rdb::Database *database) {
  if (database != mp_database) {
    ...
    if (rerun_button->isEnabled()) {
      ...
      if (!rerun_button->shortcut().isEmpty()) {
        shortcut =
            QString::fromUtf8(" (%1)").arg(rerun_button->shortcut().toString());
      }
      rerun_button->setToolTip(
          tl::to_qstring(tl::to_string(tr("Run ")) + mp_database->generator()) +
          shortcut);
    } else {
    ...
    connect(directory_tree->selectionModel(),
            ...
            this,
            SLOT(directory_selection_changed(const QItemSelection &,
                                             const QItemSelection &)));
    ...
    set_hidden_rec(new_model, directory_tree, QModelIndex(), m_show_all,
                   QString(), QString());
    ...
    MarkerBrowserListViewModel *new_list_model =
        new MarkerBrowserListViewModel();
    new_list_model->set_database(database);
    markers_list->setModel(new_list_model);
    connect(markers_list->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &,
                                    const QItemSelection &)),
            this,
            SLOT(markers_selection_changed(const QItemSelection &,
                                           const QItemSelection &)));
    connect(markers_list->selectionModel(),
            SIGNAL(currentChanged(const QModelIndex &, const QModelIndex &)),
            this,
            SLOT(markers_current_changed(const QModelIndex &,
                                         const QModelIndex &)));
...
void MarkerBrowserPage::update_content() {

  // ...
}

void MarkerBrowserPage::markers_header_clicked(int section) {
...
void MarkerBrowserPage::markers_sorting_changed(int section,
                                                Qt::SortOrder order) {
  MarkerBrowserListViewModel *list_model =
      dynamic_cast<MarkerBrowserListViewModel *>(markers_list->model());
  if (list_model) {
    list_model->set_sorting(section >= 0 ? section : -1,
                            order == Qt::AscendingOrder);
    update_marker_list(1 /*select first*/);
  }
}

void MarkerBrowserPage::directory_header_clicked(int section) {
...
void MarkerBrowserPage::directory_sorting_changed(int section,
                                                  Qt::SortOrder order) {
  MarkerBrowserTreeViewModel *tree_model =
      dynamic_cast<MarkerBrowserTreeViewModel *>(directory_tree->model());
  if (tree_model) {
    ...
    if (section == 0) {
      tree_model->sort_by(MarkerBrowserTreeViewModel::ByKeyName,
                          order == Qt::AscendingOrder);
    } else if (section == 1) {
      tree_model->sort_by(MarkerBrowserTreeViewModel::ByCount,
                          order == Qt::AscendingOrder);
    }
...
void MarkerBrowserPage::markers_selection_changed(const QItemSelection &,
                                                  const QItemSelection &) {
  update_markers();
  update_info_text();
}

void MarkerBrowserPage::marker_double_clicked(const QModelIndex &) {
  if (!m_markers_bbox.empty() && mp_view) {
    mp_view->zoom_box(m_markers_bbox.enlarged(db::DVector(
        m_markers_bbox.width() * 0.1, m_markers_bbox.height() * 0.1)));
  }
}

void MarkerBrowserPage::set_window(rdb::window_type window,
                                   const lay::Margin &window_dim,
                                   rdb::context_mode_type context) {
...
void MarkerBrowserPage::set_max_marker_count(size_t max_marker_count) {
...
void MarkerBrowserPage::enable_updates(bool f) {
...
void MarkerBrowserPage::update_markers() {
  if (!m_enable_updates) {
...
void MarkerBrowserPage::update_info_text() {
  if (!m_enable_updates) {
    m_update_needed = true;
    return;
  }
  ...
  if (list_model) {
    ...
    for (QModelIndexList::const_iterator selected_item = selected.begin();
         selected_item != selected.end(); ++selected_item) {
      ...
      if (selected_item->column() == 0) {
        ...
        if (i) {
          ...
          const rdb::Category *x =
              mp_database->category_by_id(item->category_id());
          if (x && x != category) {
            category = x;
            ++n_category;
          }
    ...
    if (item && n_item == 1) {
      ...
      for (rdb::Values::const_iterator v = item->values().begin();
           v != item->values().end(); ++v) {
      ...
      if (!image.isNull()) {
        info += "<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" "
                "style=\"border-color:blue; "
                "border-style:solid\"><tr><td><p>Snapshot image<br/>(click to "
                "enlarge)</p><p><a href=\"show-snapshot\"><img "
                "src=\"item.overview-image\"/></a></p></td></tr></table>";
...
void MarkerBrowserPage::do_update_markers() {
  release_markers();
  ...
  if (list_model) {
    ...
    if (mp_view) {
      ...
      if (m_context == rdb::AnyCell) {
        ...
        if (cv.is_valid()) {
          ...
          std::pair<bool, db::cell_index_type> cc =
              cv->layout().cell_by_name(mp_database->top_cell_name().c_str());
          if (cc.first && cc.second != cv.cell_index()) {
            mp_view->select_cell(cc.second, m_cv_index);
            current_cell =
                mp_database->cell_by_qname(mp_database->top_cell_name());
            m_view_changed = true;
          }
        }
        ...
        if (cv.is_valid()) {
          ...
          std::pair<bool, db::cell_index_type> cc =
              cv->layout().cell_by_name(cell->name().c_str());
          if (cc.first && cc.second != cv.cell_index()) {
            mp_view->select_cell(cc.second, m_cv_index);
            current_cell = cell;
            m_view_changed = true;
      ...
      if (!current_cell && cv.is_valid()) {
        current_cell =
            mp_database->cell_by_qname(cv->layout().cell_name(cv.cell_index()));
      }

      std::vector<db::DCplxTrans> tv =
          mp_view->cv_transform_variants(m_cv_index);
      if (tv.empty()) {
      ...
      for (QModelIndexList::const_iterator selected_item = selected.begin();
        ...
        if (!context.first && cv.is_valid()) {
          //  If we could not find a transformation in the RDB, try to find one
          //  in the layout DB:
          std::pair<bool, db::cell_index_type> cc =
              cv->layout().cell_by_name(c->name().c_str());
          if (cc.first) {
            std::pair<bool, db::ICplxTrans> ic = db::find_layout_context(
                cv->layout(), cc.second, cv.cell_index());
            if (ic.first) {
              context.first = true;
              context.second = db::DCplxTrans(cv->layout().dbu()) *
                               db::DCplxTrans(ic.second) *
        ...
        if (!context.first && cv.is_valid()) {
          ...
          if (m_context == rdb::AnyCell || m_context == rdb::CurrentOrAny) {
            //  Ultimate fallback in "any cell" mode is to take whatever cell we
            //  have ..
            context = std::pair<bool, db::DCplxTrans>(true, db::DCplxTrans());
          } else if (!current_cell) {
            m_error_text = tl::sprintf(
                tl::to_string(QObject::tr(
                    "Current layout cell '%s' not found in marker database and "
                    "no path found from marker's cell '%s' to current cell in "
                    "the layout database.")),
                cv->layout().cell_name(cv.cell_index()), c->name());
          } else {
            m_error_text = tl::sprintf(
                tl::to_string(QObject::tr(
                    "No example instantiation given in marker database for "
                    "marker's cell '%s' to current cell '%s' and no such path "
                    "in the layout database either.")),
        ...
        if (context.first) {
          ...
          for (rdb::Values::const_iterator v = i->values().begin();
            ...
            const rdb::Value<db::DPolygon> *polygon_value =
                dynamic_cast<const rdb::Value<db::DPolygon> *>(v->get());
            const rdb::Value<db::DBox> *box_value =
                dynamic_cast<const rdb::Value<db::DBox> *>(v->get());
            const rdb::Value<db::DEdge> *edge_value =
                dynamic_cast<const rdb::Value<db::DEdge> *>(v->get());
            const rdb::Value<db::DEdgePair> *edge_pair_value =
                dynamic_cast<const rdb::Value<db::DEdgePair> *>(v->get());
            const rdb::Value<db::DPath> *path_value =
                dynamic_cast<const rdb::Value<db::DPath> *>(v->get());
            const rdb::Value<db::DText> *text_value =
            ...
            if (polygon_value) {
              ...
              mp_markers.back()->set_dismissable(true);
              m_markers_bbox +=
                  trans * db::DBox(edge_pair_value->value().bbox());
      ...
      for (std::vector<lay::DMarker *>::const_iterator m = mp_markers.begin();
           m != mp_markers.end(); ++m) {
    ...
    const rdb::Item *current_item =
        list_model->item(markers_list->selectionModel()->currentIndex().row());
...
void MarkerBrowserPage::markers_current_changed(
    const QModelIndex & /*current*/, const QModelIndex & /*previous*/) {
  //  The current item changed event cannot be used for setting the visited
  //  flag, since it appears to occur too often - i.e. when the widgets gets the
  //  focus.
}

void MarkerBrowserPage::release_markers() {
  for (std::vector<lay::DMarker *>::iterator m = mp_markers.begin();
       m != mp_markers.end(); ++m) {
    delete *m;
  }
  mp_markers.clear();
...
void MarkerBrowserPage::directory_selection_changed(const QItemSelection &,
                                                    const QItemSelection &) {
...
static void collect_items_of_category(
    const rdb::Database *rdb, rdb::id_type cat_id, const QString &cat_f,
    std::vector<std::pair<rdb::Database::const_item_ref_iterator,
                          rdb::Database::const_item_ref_iterator>> &be_vector) {
  QString cat_f_sub;
  ...
  for (rdb::Categories::const_iterator subcat = cat->sub_categories().begin();
       subcat != cat->sub_categories().end(); ++subcat) {
    collect_items_of_category(rdb, subcat->id(), cat_f_sub, be_vector);
  }
}

static void collect_items_of_cell_and_category(
    const rdb::Database *rdb, rdb::id_type cell_id, rdb::id_type cat_id,
    const QString &cat_f,
    std::vector<std::pair<rdb::Database::const_item_ref_iterator,
                          rdb::Database::const_item_ref_iterator>> &be_vector) {
  QString cat_f_sub;
  ...
  for (rdb::Categories::const_iterator subcat = cat->sub_categories().begin();
       subcat != cat->sub_categories().end(); ++subcat) {
    collect_items_of_cell_and_category(rdb, cell_id, subcat->id(), cat_f_sub,
                                       be_vector);
  }
}

void MarkerBrowserPage::update_marker_list(int /*selection_mode*/) {
  if (!mp_database) {
    return;
  }
  ...
  std::vector<std::pair<rdb::Database::const_item_ref_iterator,
                        rdb::Database::const_item_ref_iterator>>
      be_vector;
  std::vector<std::pair<rdb::Database::const_item_iterator,
                        rdb::Database::const_item_iterator>>
      be_vector_all;
  m_num_items = 0;

  QModelIndexList selected =
      directory_tree->selectionModel()->selectedIndexes();
  ...
  for (QModelIndexList::const_iterator s = selected.begin();
       s != selected.end(); ++s) {
    ...
    //  ignore selected items whose parent is selected too - the parent will
    //  include the selections output.
    if (selected_set.find(tree_model->parent(selected_item)) !=
        selected_set.end()) {
      continue;
    }

    const rdb::Cell *cell = 0;
    for (MarkerBrowserTreeViewModelCacheEntry *entry =
             (MarkerBrowserTreeViewModelCacheEntry *)
                 selected_item.internalPointer();
         entry && !cell; entry = entry->parent()) {
    ...
    if (cell == 0 && cat == 0) {
      ...
      be_vector_all.push_back(std::make_pair(mp_database->items().begin(),
                                             mp_database->items().end()));
      ...
      if (cell != 0 && cat == 0 && cat_f.isEmpty()) {
        ...
        if (cell_f.isEmpty() || cell_matches_filter(cell, cell_f)) {
          for (rdb::Categories::const_iterator x =
                   mp_database->categories().begin();
               x != mp_database->categories().end(); ++x) {
            collect_items_of_cell_and_category(mp_database, cell->id(), x->id(),
                                               cat_f, be_vector);
          }
        }
        ...
        for (rdb::Database::const_cell_iterator c =
          ...
          if (cell_f.isEmpty() || cell_matches_filter(c.operator->(), cell_f)) {
            collect_items_of_cell_and_category(mp_database, c->id(), cat->id(),
                                               cat_f, be_vector);
          }
        }
        ...
        if (cell_f.isEmpty() || cell_matches_filter(cell, cell_f)) {
          collect_items_of_cell_and_category(mp_database, cell->id(), cat->id(),
                                             cat_f, be_vector);
        }
      }

      for (std::vector<std::pair<rdb::Database::const_item_ref_iterator,
                                 rdb::Database::const_item_ref_iterator>>::
               const_iterator be = be_vector.begin();
           be != be_vector.end(); ++be) {
        for (rdb::Database::const_item_ref_iterator i = be->first;
             i != be->second; ++i) {
          ++m_num_items;
        }
      }
  ...
  //  in case of given filter, the "all" categories are reduced to the filtered
  //  ones
  if (!be_vector_all.empty() && (!cat_f.isEmpty() || !cell_f.isEmpty())) {
    ...
    if (cat_f.isEmpty()) {
      ...
      //  filter by cell
      for (rdb::Database::const_cell_iterator c = mp_database->cells().begin();
           c != mp_database->cells().end(); ++c) {
      ...
      for (rdb::Database::const_cell_iterator c = mp_database->cells().begin();
        ...
        if (cell_matches_filter(c.operator->(), cell_f)) {
          for (rdb::Categories::const_iterator x =
                   mp_database->categories().begin();
               x != mp_database->categories().end(); ++x) {
            collect_items_of_cell_and_category(mp_database, c->id(), x->id(),
                                               cat_f, be_vector);
          }
        }
      }
  ...
  if (list_model) {
    ...
    if (m_num_items > 0) {
      ...
      if (m_num_items == 1) {
        //  set the current to the first entry
        markers_list->selectionModel()->setCurrentIndex(
            list_model->index_of_row(0),
...
bool MarkerBrowserPage::adv_tree(bool up) {
  MarkerBrowserTreeViewModel *tree_model =
      dynamic_cast<MarkerBrowserTreeViewModel *>(directory_tree->model());
  if (!tree_model) {
    return false;
  ...
  while (index.isValid()) {
    ...
    if (index.isValid() &&
        !directory_tree->isRowHidden(index.row(), tree_model->parent(index))) {
      break;
    }
  }

  if (index.isValid()) {
    directory_tree->selectionModel()->setCurrentIndex(
...
bool MarkerBrowserPage::adv_list(bool up) {
  MarkerBrowserListViewModel *list_model =
      dynamic_cast<MarkerBrowserListViewModel *>(markers_list->model());
  if (!list_model) {
    return false;
  ...
  //  don't consider selection changed events since they might interfere with
  //  the markers list that we deal with currently.
  ...
  if (index.isValid()) {
    index = list_model->index(index.row() + (up ? -1 : 1), index.column(),
                              QModelIndex());
  }

  if (!index.isValid() && adv_tree(up)) {
    index = list_model->index(
        up ? (list_model->rowCount(QModelIndex()) - 1) : 0, 0, QModelIndex());
  }
...
void MarkerBrowserPage::dir_up_clicked() { adv_tree(true); }

void MarkerBrowserPage::dir_down_clicked() { adv_tree(false); }

void MarkerBrowserPage::list_up_clicked() { adv_list(true); }

void MarkerBrowserPage::list_down_clicked() { adv_list(false); }

void MarkerBrowserPage::flag_button_clicked() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::rerun_button_pressed() {
  //  NOTE: we use deferred execution, because otherwise the button won't get
  //  repainted properly
  dm_rerun_macro();
}

void MarkerBrowserPage::rerun_macro() {
  BEGIN_PROTECTED
...
void MarkerBrowserPage::flag_menu_selected() {
  QAction *action = dynamic_cast<QAction *>(sender());
  if (action) {
    int flag_index = action->data().toInt();
    if (flag_index >= 0 && flag_index < int(sizeof(flag_descriptors) /
                                            sizeof(flag_descriptors[0]))) {
      flags_pb->setIcon(
          QIcon(tl::to_qstring(flag_descriptors[flag_index].icon)));
...
void MarkerBrowserPage::waived_button_clicked() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::important_button_clicked() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::remove_snapshot_button_clicked() {
  if (!mp_database) {
    return;
  }
  ...
  QMessageBox msgbox(
      QMessageBox::Question, QObject::tr("Remove All Snapshots"),
      QObject::tr("Are you sure to remove the snapshot from all markers?"),
      QMessageBox::Yes | QMessageBox::No);
  if (msgbox.exec() == QMessageBox::Yes) {

    QModelIndexList selected =
        markers_list->selectionModel()->selectedIndexes();
    for (QModelIndexList::const_iterator selected_item = selected.begin();
         selected_item != selected.end(); ++selected_item) {
      if (selected_item->column() == 0) {
        const rdb::Item *i = list_model->item(selected_item->row());
...
void MarkerBrowserPage::snapshot_button_clicked() {
  if (!mp_database) {
    return;
  }
  ...
  for (QModelIndexList::const_iterator selected_item = selected.begin();
    ...
    if (i) {
      ...
      // Currently, don't add the snapshot to all selected items - this would
      // create some overhead since snapshots are not shared currently.
      break;
    }
  }
}

void MarkerBrowserPage::show_all_clicked() {
  if (mp_plugin_root) {
    mp_plugin_root->config_set(cfg_rdb_show_all,
                               tl::to_string(m_show_all_action->isChecked()));
  }
}

void MarkerBrowserPage::list_shapes_clicked() {
  if (mp_plugin_root) {
    mp_plugin_root->config_set(cfg_rdb_list_shapes,
                               tl::to_string(list_shapes_cb->isChecked()));
  }
}

void MarkerBrowserPage::unwaive_all() {
  if (!mp_database) {
    return;
  }
  ...
  QMessageBox msgbox(
      QMessageBox::Question, QObject::tr("Remove All Waived"),
  ...
  if (msgbox.exec() == QMessageBox::Yes) {
    ...
    for (Items::const_iterator i = mp_database->items().begin();
         i != mp_database->items().end(); ++i) {
...
void MarkerBrowserPage::revisit_all() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::revisit_non_waived() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::revisit_important() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::mark_important() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::mark_unimportant() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::mark_visited() { mark_visited(true); }

void MarkerBrowserPage::mark_notvisited() { mark_visited(false); }

void MarkerBrowserPage::mark_visited(bool f) {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::unwaive() {
  if (!mp_database) {
    return;
  }
...
void MarkerBrowserPage::info_anchor_clicked(const QUrl &link) {
  if (link.isRelative() && link.path() == QString::fromUtf8("show-snapshot")) {
    ...
    if (i && i->has_image()) {
      MarkerBrowserSnapshotView *snapshot_view =
          new MarkerBrowserSnapshotView(this, i->image());
      snapshot_view->exec();
...
bool MarkerBrowserPage::eventFilter(QObject *watched, QEvent *event) {
