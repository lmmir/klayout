...
NetExportDialog::NetExportDialog(QWidget *parent) : QDialog(parent) {
...
void NetExportDialog::set_net_prefix(const std::string &net_prefix) {
  ui->net_cell_prefix->setText(tl::to_qstring(net_prefix));
}

std::string NetExportDialog::net_prefix() {
  return tl::to_string(ui->net_cell_prefix->text());
}

void NetExportDialog::set_net_propname(const tl::Variant &net_propname) {
  if (net_propname.is_nil()) {
    ui->net_propname->setText(QString());
  } else {
    ui->net_propname->setText(
        tl::to_qstring(net_propname.to_parsable_string()));
  }
}

tl::Variant NetExportDialog::net_propname() {
...
void NetExportDialog::set_produce_circuit_cells(bool f) {
  ui->produce_circuit_cells_cb->setChecked(f);
}

bool NetExportDialog::produce_circuit_cells() {
  return ui->produce_circuit_cells_cb->isChecked();
}

void NetExportDialog::set_circuit_cell_prefix(const std::string &cell_prefix) {
  ui->circuit_cell_prefix->setText(tl::to_qstring(cell_prefix));
}

std::string NetExportDialog::circuit_cell_prefix() {
  return tl::to_string(ui->circuit_cell_prefix->text());
}

void NetExportDialog::set_produce_device_cells(bool f) {
  ui->produce_device_cells_cb->setChecked(f);
}

bool NetExportDialog::produce_device_cells() {
  return ui->produce_device_cells_cb->isChecked();
}

void NetExportDialog::set_device_cell_prefix(const std::string &cell_prefix) {
  ui->device_cell_prefix->setText(tl::to_qstring(cell_prefix));
}

std::string NetExportDialog::device_cell_prefix() {
  return tl::to_string(ui->device_cell_prefix->text());
}

void NetExportDialog::set_start_layer_number(int ln) {
  ui->layernum->setText(tl::to_qstring(tl::to_string(ln)));
}

int NetExportDialog::start_layer_number() {
  int ln = 0;
  tl::from_string_ext(tl::to_string(ui->layernum->text()), ln);
  return ln;
}

void NetExportDialog::accept() {
  BEGIN_PROTECTED
  start_layer_number();
  net_propname();
  QDialog::accept();
...
int NetExportDialog::exec_dialog(lay::Dispatcher *plugin_root) {
  std::string v;
  ...
  if (ret) {
    ...
    plugin_root->config_set(cfg_l2ndb_export_start_layer_number,
                            tl::to_string(start_layer_number()));
    plugin_root->config_set(cfg_l2ndb_export_produce_circuit_cells,
                            tl::to_string(produce_circuit_cells()));
    plugin_root->config_set(cfg_l2ndb_export_circuit_cell_prefix,
                            circuit_cell_prefix());
    plugin_root->config_set(cfg_l2ndb_export_produce_device_cells,
                            tl::to_string(produce_device_cells()));
    plugin_root->config_set(cfg_l2ndb_export_device_cell_prefix,
                            device_cell_prefix());
  }
