...
EditorOptionsPages::EditorOptionsPages(
    QWidget *parent, const std::vector<lay::EditorOptionsPage *> &pages,
  ...
  mp_pages->setSizePolicy(
      QSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored));
  ly1->addWidget(mp_pages);

  m_pages = pages;
  for (std::vector<lay::EditorOptionsPage *>::const_iterator p =
           m_pages.begin();
       p != m_pages.end(); ++p) {
    (*p)->set_owner(this);
  }
...
bool EditorOptionsPages::has_content() const {
  for (std::vector<lay::EditorOptionsPage *>::const_iterator p =
           m_pages.begin();
       p != m_pages.end(); ++p) {
    //  NOTE: we ignore unspecific pages because they are always visible and
    //  don't contribute specific content
...
void EditorOptionsPages::update(lay::EditorOptionsPage *page) {
  ...
  int index = -1;
  for (std::vector<lay::EditorOptionsPage *>::iterator p = sorted_pages.begin();
       p != sorted_pages.end(); ++p) {
    if ((*p)->active()) {
      if ((*p) == page) {
...
void EditorOptionsPages::setup() {
  try {
    ...
    for (std::vector<lay::EditorOptionsPage *>::iterator p = m_pages.begin();
         p != m_pages.end(); ++p) {
      if ((*p)->active()) {
...
void EditorOptionsPages::do_apply() {
  for (std::vector<lay::EditorOptionsPage *>::iterator p = m_pages.begin();
       p != m_pages.end(); ++p) {
    if ((*p)->active()) {
      //  NOTE: we apply to the root dispatcher, so other dispatchers (views)
      //  get informed too.
