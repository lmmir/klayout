...
/**
 *  @brief A comparison operator to sort layers by layer and datatype and then
 * by name
 */
struct CompareLDName {
  CompareLDName(const db::Layout &l) : mp_layout(&l) {
    //  .. nothing yet ..
  }
...
/**
 *  @brief A comparison operator to sort layers by name and then by layer and
 * datatype
 */
struct CompareNameLD {
  CompareNameLD(const db::Layout &l) : mp_layout(&l) {
    //  .. nothing yet ..
  }
 ...
 *  The template processor is used from the browser page by using an extension
 * .stxml. It reads a XML file from a resource path ":/st/<path>" and converts
 * it into HTML.
 */
...
public:
  StatisticsTemplateProcessor(const tl::URI &url, const db::Layout *layout)
      : mp_layout(layout) {
    QResource res(QString::fromUtf8(":/st/") +
                  QString::fromUtf8(url.path().c_str()));
  ...
  const QByteArray &get() const { return m_output.buffer(); }
...
private:
  ...
  void process(const QDomElement &element, tl::Eval &eval,
               QXmlStreamWriter &writer);
  void process_child_nodes(const QDomElement &element, tl::Eval &eval,
                           QXmlStreamWriter &writer);
};

bool StatisticsTemplateProcessor::process() {
  ...
  try {
    ...
    writer << tl::to_string(QObject::tr("ERROR: evaluating template: ")).c_str()
           << ex.msg().c_str();
  }
...
void StatisticsTemplateProcessor::process_child_nodes(
    const QDomElement &element, tl::Eval &eval, QXmlStreamWriter &writer) {
  if (element.isNull()) {
    return;
  }
  ...
  for (QDomNode n = element.firstChild(); !n.isNull(); n = n.nextSibling()) {
    ...
    if (n.isElement()) {
      ...
      writer.writeCDATA(tl::to_qstring(
          eval.interpolate(tl::to_string(n.toCDATASection().data()))));
      ...
      writer.writeCharacters(
          tl::to_qstring(eval.interpolate(tl::to_string(t))));
    }
  }
}

void StatisticsTemplateProcessor::process(const QDomElement &element,
                                          tl::Eval &eval,
                                          QXmlStreamWriter &writer) {
  static QString template_namespace_uri =
      QString::fromUtf8("www.klayout.org/layout-statistics-template");
  ...
  if (element.namespaceURI() == template_namespace_uri) {
    ...
    if (element.localName() == template_cmd_eval) {
      ...
      tl::Expression expr;
      eval.parse(expr, tl::to_string(element.attribute(template_name_expr,
                                                       template_value_true)));
      expr.execute();
      ...
      if (begin_node.isNull() && end_node.isNull() && max_node.isNull() &&
          each_node.isNull()) {
        each_node = element;
      }
      ...
      db::LayoutQuery q(tl::to_string(
          element.attribute(template_name_expr, template_value_empty_query)));
    ...
    if (element.hasAttributes()) {
      ...
      for (int i = 0; i < attributes.count(); ++i) {
        ...
        if (!a.isNull()) {
          writer.writeAttribute(a.nodeName(), tl::to_qstring(eval.interpolate(
                                                  tl::to_string(a.value()))));
        }
      }
    }
...
public:
  ...
  void operator*=(size_t f) {
    for (std::map<db::Shape::object_type, size_t>::iterator c = m_count.begin();
         c != m_count.end(); ++c) {
      c->second *= f;
    }
  }
  ...
  void operator+=(const ShapeStatistics &other) {
    for (std::map<db::Shape::object_type, size_t>::const_iterator c =
             other.m_count.begin();
         c != other.m_count.end(); ++c) {
      m_count[c->first] += c->second;
    }
  }

  size_t count(db::Shape::object_type t) const {
    std::map<db::Shape::object_type, size_t>::const_iterator c =
        m_count.find(t);
    if (c != m_count.end()) {
      return c->second;
    } else {
  ...
  size_t box_total() const {
    return count(db::Shape::Box) + count(db::Shape::BoxArrayMember) +
           count(db::Shape::ShortBox) + count(db::Shape::ShortBoxArrayMember);
  }
  ...
  size_t polygon_total() const {
    return count(db::Shape::Polygon) + count(db::Shape::PolygonRef) +
           count(db::Shape::PolygonPtrArrayMember) +
           count(db::Shape::SimplePolygon) +
           count(db::Shape::SimplePolygonRef) +
           count(db::Shape::SimplePolygonPtrArrayMember);
  }
  ...
  size_t polygon_array() const {
    return count(db::Shape::PolygonPtrArray) +
           count(db::Shape::SimplePolygonPtrArray);
  }

  size_t path_total() const {
    return count(db::Shape::Path) + count(db::Shape::PathRef) +
           count(db::Shape::PathPtrArrayMember);
  }
  ...
  size_t path_array() const { return count(db::Shape::PathPtrArray); }

  size_t text_total() const {
    return count(db::Shape::Text) + count(db::Shape::TextRef) +
           count(db::Shape::TextPtrArrayMember);
  }
  ...
  size_t text_array() const { return count(db::Shape::TextPtrArray); }

  size_t edge_total() const { return count(db::Shape::Edge); }

  size_t edge_pair_total() const { return count(db::Shape::EdgePair); }

  size_t user_total() const { return count(db::Shape::UserObject); }

  size_t all_total() const {
    return box_total() + polygon_total() + path_total() + text_total() +
           edge_total() + edge_pair_total() + user_total();
  }
...
class StatisticsSource : public lay::BrowserSource {
public:
  StatisticsSource(const lay::LayoutHandleRef &h) : m_h(h) {
    //  .. nothing yet ..
  }
  ...
  void clear_cache() { m_page_cache.clear(); }
...
std::string StatisticsSource::per_layer_stat_page(const tl::URI &uri) const {
  ...
  os << "<html>" << std::endl
     << "<body>" << std::endl
     << "<h2>" << tl::to_string(QObject::tr("Detailed Layer Statistics for '"))
     << m_h->name() << "'</h2>" << std::endl

     << "<p>" << std::endl
     << "<table cellspacing=\"5\" cellpadding=\"5\">" << std::endl

     << "<tr>" << std::endl
     << "<th bgcolor=\"#f0f0f0\">" << tl::to_string(QObject::tr("Layer"))
     << "</th>" << std::endl
     << "<th bgcolor=\"#f0f0f0\">" << tl::to_string(QObject::tr("All"))
     << "</th>" << std::endl
     << "<th colspan=\"3\" bgcolor=\"#f0f0f0\">"
     << tl::to_string(QObject::tr("Boxes")) << "</th>" << std::endl
     << "<th colspan=\"3\" bgcolor=\"#f0f0f0\">"
     << tl::to_string(QObject::tr("Polygons")) << "</th>" << std::endl
     << "<th colspan=\"3\" bgcolor=\"#f0f0f0\">"
     << tl::to_string(QObject::tr("Paths")) << "</th>" << std::endl
     << "<th colspan=\"3\" bgcolor=\"#f0f0f0\">"
     << tl::to_string(QObject::tr("Texts")) << "</th>" << std::endl
     << "<th bgcolor=\"#f0f0f0\">" << tl::to_string(QObject::tr("Edges"))
     << "</th>" << std::endl
     << "<th bgcolor=\"#f0f0f0\">" << tl::to_string(QObject::tr("Edge Pairs"))
     << "</th>" << std::endl
     << "<th bgcolor=\"#f0f0f0\">" << tl::to_string(QObject::tr("User objects"))
     << "</th>" << std::endl
     << "</tr>" << std::endl
     ...
     << "<th>" << tl::to_string(QObject::tr("(total)")) << "</th><th>"
     << tl::to_string(QObject::tr("(single)")) << "</th><th>"
     << tl::to_string(QObject::tr("(arrays)")) << "</th>" << std::endl
     << "<th>" << tl::to_string(QObject::tr("(total)")) << "</th><th>"
     << tl::to_string(QObject::tr("(single)")) << "</th><th>"
     << tl::to_string(QObject::tr("(arrays)")) << "</th>" << std::endl
     << "<th>" << tl::to_string(QObject::tr("(total)")) << "</th><th>"
  ...
  tl::RelativeProgress progress(
      tl::to_string(QObject::tr("Collecting statistics")),
      layers.size() * layout.cells(), 100000);
  for (std::vector<unsigned int>::const_iterator l = layers.begin();
    ...
    os << "<tr>" << std::endl
       << "<td>"
       << tl::escaped_to_html(layout.get_properties(*l).to_string(), true)
       << "</td>" << std::endl
       << "<td>" << st_hier.all_total() << "<br></br>" << st_flat.all_total()
       << "</td>"
       << std::endl
       // Boxes (total, single, array)
       << "<td>" << st_hier.box_total() << "<br></br>" << st_flat.box_total()
       << "</td>" << std::endl
       << "<td>" << st_hier.box_single() << "<br></br>" << st_flat.box_single()
       << "</td>" << std::endl
       << "<td>" << st_hier.box_array() << "<br></br>" << st_flat.box_array()
       << "</td>"
       << std::endl
       // Polygons (total, single, array)
       << "<td>" << st_hier.polygon_total() << "<br></br>"
       << st_flat.polygon_total() << "</td>" << std::endl
       << "<td>" << st_hier.polygon_single() << "<br></br>"
       << st_flat.polygon_single() << "</td>" << std::endl
       << "<td>" << st_hier.polygon_array() << "<br></br>"
       << st_flat.polygon_array() << "</td>"
       << std::endl
       // Paths (total, single, array)
       << "<td>" << st_hier.path_total() << "<br></br>" << st_flat.path_total()
       << "</td>" << std::endl
       << "<td>" << st_hier.path_single() << "<br></br>"
       << st_flat.path_single() << "</td>" << std::endl
       << "<td>" << st_hier.path_array() << "<br></br>" << st_flat.path_array()
       << "</td>"
       << std::endl
       // Texts (total, single, array)
       << "<td>" << st_hier.text_total() << "<br></br>" << st_flat.text_total()
       << "</td>" << std::endl
       << "<td>" << st_hier.text_single() << "<br></br>"
       << st_flat.text_single() << "</td>" << std::endl
       << "<td>" << st_hier.text_array() << "<br></br>" << st_flat.text_array()
       << "</td>"
       << std::endl
       // Edges (total)
       << "<td>" << st_hier.edge_total() << "<br></br>" << st_flat.edge_total()
       << "</td>"
       << std::endl
       // EdgePairs (total)
       << "<td>" << st_hier.edge_pair_total() << "<br></br>"
       << st_flat.edge_pair_total() << "</td>"
       << std::endl
       // User objects (total)
       << "<td>" << st_hier.user_total() << "<br></br>" << st_flat.user_total()
       << "</td>"
       << std::endl
       // ...
       << "<td>" << tl::to_string(QObject::tr("(hier)")) << "<br></br>"
       << tl::to_string(QObject::tr("(flat)")) << "</td>" << std::endl
       << "</tr>" << std::endl;
  }
  ...
  os << "</table>" << std::endl
     ...
     << tl::to_string(QObject::tr(
            ...
            "\"(flat)\" is the \"as if flat\" count where the cells count as "
            "many times as they are seen from the top cells."
            "</p>"
            "<p>"
            "\"(total)\" is the effective number of shapes. \"(single)\" are "
            "the single shapes. "
            "\"(arrays)\" is the number of shape arrays where each array "
            "counts as one, but contributes many individual shapes to "
            "\"(total)\"."
            "</p>"))
     << "</body>" << std::endl
     << "</html>";
...
std::string StatisticsSource::index_page(const tl::URI & /*uri*/) const {
  //  maybe later ...
  ...
  os << "<html>" << std::endl
     << "<body>" << std::endl
     << "<h2>" << tl::to_string(QObject::tr("Common Statistics For '"))
     << tl::escaped_to_html(m_h->name(), true) << "'</h2>" << std::endl
     << "<p>" << std::endl
     << "<table>" << std::endl
     << "<tr>"
     << "<td>" << tl::to_string(QObject::tr("Path")) << ":&nbsp;</td><td>"
     << tl::escaped_to_html(m_h->filename(), true) << "</td>"
     << "</tr>" << std::endl;
  if (!m_h->save_options().format().empty()) {
    os << "<tr>"
       << "<td>" << tl::to_string(QObject::tr("Format")) << ":&nbsp;</td><td>"
       << tl::escaped_to_html(m_h->save_options().format(), true) << "</td>"
       << "</tr>" << std::endl;
  }
  os << "<tr>"
     << "<td>" << tl::to_string(QObject::tr("Technology")) << ":&nbsp;</td><td>"
     << tl::escaped_to_html(m_h->technology()->description(), true)
     << tl::escaped_to_html(format_tech_name(m_h->tech_name()), true) << "</td>"
     << "</tr>" << std::endl
     << "<tr>"
     << "<td>" << tl::to_string(QObject::tr("Database unit"))
     << ":&nbsp;</td><td>" << tl::sprintf("%.12g ", layout.dbu())
     << tl::to_string(QObject::tr("micron")) << "</td>"
     << "</tr>" << std::endl
     << "<tr>"
     << "<td>" << tl::to_string(QObject::tr("Number of cells"))
     << ":&nbsp;</td><td>" << num_cells << "</td>"
     << "</tr>" << std::endl
     << "<tr>"
     << "<td>" << tl::to_string(QObject::tr("Number of layers"))
     << ":&nbsp;</td><td>" << num_layers << "</td>"
     << "</tr>" << std::endl;
  for (db::Layout::meta_info_iterator meta = layout.begin_meta();
    ...
    os << "<tr><td>" << tl::escaped_to_html(d, true) << "</td><td>"
       << tl::escaped_to_html(meta->second.value.to_string(), true)
       << "</td></tr>" << std::endl;
  }
  os << "</table>" << std::endl
     << "<h2>" << tl::to_string(QObject::tr("Top Cells")) << "</h2>"
     << std::endl
     << "<table>" << std::endl;
  for (db::Layout::top_down_const_iterator tc = layout.begin_top_down();
       tc != layout.end_top_cells(); ++tc) {
    os << "<tr><td>" << tl::escaped_to_html(layout.cell_name(*tc), true)
       << "</td></tr>" << std::endl;
  }
  os << "</table>" << std::endl;
  os << "</p>" << std::endl;
  ...
  std::sort(layers_sorted_by_ld.begin(), layers_sorted_by_ld.end(),
            CompareLDName(layout));
  std::sort(layers_with_oasis_names.begin(), layers_with_oasis_names.end(),
            CompareNameLD(layout));
  ...
  if (!layers_sorted_by_ld.empty()) {
    ...
    os << "<h2>"
       ...
       << "</h2>" << std::endl
       << "<p><a href=\""
       << tl::escaped_to_html(tl::to_string(s_per_layer_stat_path_ld), true)
       << "\">Detailed layer statistics</a></p>" << std::endl
       << "<p>" << std::endl
       << "<table>" << std::endl
       << "<tr><td><b>" << tl::to_string(QObject::tr("Layer/Datatype"))
       << "</b>&nbsp;&nbsp;</td>";
    if (!layers_with_oasis_names.empty()) {
      os << "<td><b>" << tl::to_string(QObject::tr("Layer name"))
         << "</b></td>";
    }
    if (with_shape_statistics) {
      os << "<td><b>" << tl::to_string(QObject::tr("Shape count (hier)"))
         << "</b></td>";
      os << "<td><b>" << tl::to_string(QObject::tr("Shape count (flat)"))
         << "</b></td>";
    }
    os << "</tr>" << std::endl;
    ...
    tl::RelativeProgress progress(
        tl::to_string(QObject::tr("Collecting statistics")),
        layers_sorted_by_ld.size() * layout.cells(), 100000);
    for (std::vector<unsigned int>::const_iterator i =
             layers_sorted_by_ld.begin();
         i != layers_sorted_by_ld.end(); ++i) {
  ...
  if (!layers_with_oasis_names.empty()) {
    ...
    os << "<h2>" << tl::to_string(QObject::tr("Layers (sorted by layer names)"))
       << "</h2>" << std::endl
       << "<p><a href=\""
       << tl::escaped_to_html(tl::to_string(s_per_layer_stat_path_name), true)
       << "\">Detailed layer statistics</a></p>" << std::endl
       << "<p>" << std::endl
       << "<table>" << std::endl
       << "<tr><td><b>" << tl::to_string(QObject::tr("Layer name"))
       << "</b>&nbsp;&nbsp;</td><td><b>"
       << tl::to_string(QObject::tr("Layer/Datatype")) << "</b></td></tr>"
       << std::endl;

    for (std::vector<unsigned int>::const_iterator i =
             layers_with_oasis_names.begin();
         i != layers_with_oasis_names.end(); ++i) {
      if (layout.is_valid_layer(*i)) {
        const db::LayerProperties &lp = layout.get_properties(*i);
        if (!lp.name.empty()) {
          os << "<tr>"
  ...
  os << "</body>" << std::endl << "</html>" << std::endl;
  ;

  return os.str();
}

std::string StatisticsSource::get_impl(const std::string &url) {
  tl::URI uri(url);
  ...
  } else if (page == s_per_layer_stat_path_ld ||
             page == s_per_layer_stat_path_name) {
...
LayoutStatisticsForm::LayoutStatisticsForm(QWidget *parent,
...
void LayoutStatisticsForm::layout_selected(int index) {
