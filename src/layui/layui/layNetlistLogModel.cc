...
struct CircuitsCompareByName {
  bool operator()(
      ...
      const std::pair<
          ...
          const db::NetlistCrossReference::PerCircuitData::log_entries_type *>
          &b) const {
    return sort_pair<db::Circuit, sort_single_by_name<db::Circuit>>()(a.first,
                                                                      b.first);
  }
};
...
NetlistLogModel::NetlistLogModel(QWidget *parent,
  ...
  if (!cross_ref->other_log_entries().empty()) {
    m_circuits.push_back(std::make_pair(
        std::make_pair((const db::Circuit *)0, (const db::Circuit *)0),
        &cross_ref->other_log_entries()));
  }

  for (auto i = cross_ref->begin_circuits(); i != cross_ref->end_circuits();
       ++i) {
    const db::NetlistCrossReference::PerCircuitData *pcd =
        cross_ref->per_circuit_data_for(*i);
...
QModelIndex NetlistLogModel::index(int row, int column,
...
QModelIndex NetlistLogModel::parent(const QModelIndex &child) const {
  if (child.internalPointer() == (void *)0) {
    return QModelIndex();
  } else {
    const circuit_entry *ce = (const circuit_entry *)child.internalPointer();
    return createIndex(int(ce - &m_circuits.front()), child.column(),
                       (void *)(0));
  }
}
...
QVariant NetlistLogModel::data(const QModelIndex &index, int role) const {
...
QVariant NetlistLogModel::headerData(int section,
