...
LayoutPropertiesForm::LayoutPropertiesForm(QWidget *parent,
...
void LayoutPropertiesForm::commit() {
  ...
  if (fabs(dbu - layout.dbu()) > 1e-6) {
    if (mp_view->manager()) {
      mp_view->manager()->transaction(
          tl::to_string(QObject::tr("Change layout's database unit")));
    }
    layout.dbu(dbu);
    if (mp_view->manager()) {
  ...
  if (tech) {
    ...
    if (tech_has_changed && !lyp_file.empty()) {
      ...
      //  if the new technology has a layer properties file attached, ask
      //  whether to load it
      if (QMessageBox::question(
              this, QObject::tr("Load Layer Properties File"),
              tl::to_qstring(
                  tl::to_string(QObject::tr("The new technology specifies a "
...
void LayoutPropertiesForm::prop_pb_clicked() {
  ...
  if (props_form.show(mp_view, m_index, prop_id, layout.begin_meta(),
                      layout.end_meta())) {

    if (mp_view->manager()) {
      mp_view->manager()->transaction(
          tl::to_string(QObject::tr("Edit layout's user properties")));
    }
