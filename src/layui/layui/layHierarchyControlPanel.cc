...
class CellClipboardData : public db::ClipboardData {
public:
  CellClipboardData() {}
};
...
HCPCellTreeWidget::~HCPCellTreeWidget() {
  //  NOTE: this should not be required, but I got a strange crash on closing
  //  the app with Qt 5.12.8 after using changePersistentIndex inside the model
  //  when ~QTreeWidget tried to clean up its persistent indexes and only found
  //  a model which was deleted already.
...
bool HCPCellTreeWidget::focusNextPrevChild(bool /*next*/) { return false; }
...
void HCPCellTreeWidget::keyPressEvent(QKeyEvent *event) {
  ...
  if (!t.isEmpty() && t[0].isPrint()) {
    ...
    //  send other key events to the alternative receiver - this way we can make
    //  the view receive arrow keys for panning.
...
HierarchyControlPanel::HierarchyControlPanel(lay::LayoutViewBase *view,
                                             QWidget *parent, const char *name)
    : QFrame(parent), m_enable_cb(true), mp_view(view),
      m_visibility_needs_update(true), m_active_index(0), m_flat(false),
      m_split_mode(false), m_sorting(CellTreeModel::ByName),
      m_do_update_content_dm(this, &HierarchyControlPanel::do_update_content),
      m_do_full_update_content_dm(
          this, &HierarchyControlPanel::do_full_update_content) {
  setObjectName(QString::fromUtf8(name));
  ...
  connect(mp_selector, SIGNAL(activated(int)), this,
          SLOT(selection_changed(int)));
  ...
  mp_view->cellviews_changed_event.add(this,
                                       &HierarchyControlPanel::update_required);
  mp_view->hier_changed_event.add(this,
                                  &HierarchyControlPanel::update_required);
...
void HierarchyControlPanel::set_sorting(CellTreeModel::Sorting sorting) {
  if (sorting != m_sorting) {
    ...
    for (size_t i = 0; i < mp_cell_lists.size(); ++i) {
      CellTreeModel *model =
          dynamic_cast<CellTreeModel *>(mp_cell_lists[i]->model());
      if (model) {
  ...
  if (w) {
    for (size_t i = 0; i < mp_cell_lists.size(); ++i) {
      if (mp_cell_lists[i] == w) {
        //  Switch the active list for split mode -> CAUTION: this may trigger a
        //  search_editing_finished call
        select_active(int(i));
...
void HierarchyControlPanel::search_edited() {
  ...
  if (m_search_index >= 0 && m_search_index < int(mp_cell_lists.size())) {
    ...
    lay::CellTreeModel *search_model = dynamic_cast<lay::CellTreeModel *>(
        mp_cell_lists[m_search_index]->model());
    ...
    if (t.isEmpty()) {
      ...
      QModelIndex found = search_model->locate(
...
void HierarchyControlPanel::search_next() {
  if (m_search_index >= 0 && m_search_index < int(mp_cell_lists.size())) {
    lay::CellTreeModel *search_model = dynamic_cast<lay::CellTreeModel *>(
        mp_cell_lists[m_search_index]->model());
    QModelIndex found = search_model->locate_next(
        mp_cell_lists[m_search_index]->currentIndex());
    if (found.isValid()) {
      mp_cell_lists[m_search_index]->setCurrentIndex(found);
      mp_cell_lists[m_search_index]->scrollTo(found);
    }
...
void HierarchyControlPanel::search_editing_finished() {
  ...
  for (std::vector<QTreeView *>::const_iterator v = mp_cell_lists.begin();
       v != mp_cell_lists.end(); ++v) {
...
void HierarchyControlPanel::path_from_index(const QModelIndex &index,
                                            int cv_index,
...
void HierarchyControlPanel::set_current_cell(int cv_index,
...
void HierarchyControlPanel::selected_cells(
    int cv_index,
    std::vector<HierarchyControlPanel::cell_path_type> &paths) const {
  if (cv_index >= 0 && cv_index < int(mp_cell_lists.size())) {
    QModelIndexList sel =
        mp_cell_lists[cv_index]->selectionModel()->selectedIndexes();
...
void HierarchyControlPanel::current_cell(
    int cv_index, HierarchyControlPanel::cell_path_type &path) const {
  if (cv_index >= 0 && cv_index < int(mp_cell_lists.size())) {
...
void HierarchyControlPanel::set_background_color(tl::Color c) {
  ...
  for (std::vector<QTreeView *>::const_iterator f = mp_cell_lists.begin();
       f != mp_cell_lists.end(); ++f) {
...
void HierarchyControlPanel::do_full_update_content() {
  size_t i = 0;
  for (std::vector<lay::CellView>::const_iterator cv = m_cellviews.begin();
       cv != m_cellviews.end(); ++cv, ++i) {
...
void HierarchyControlPanel::update_required() { m_do_full_update_content_dm(); }

void HierarchyControlPanel::select_active(int cellview_index) {
...
void HierarchyControlPanel::selection_changed(int index) {
  if (index != m_active_index) {
    ...
    //  for more than max_cellviews_in_split_mode cellviews, switch to overlay
    //  mode
    ...
    int i = 0;
    for (std::vector<QFrame *>::const_iterator f = mp_cell_list_frames.begin();
         f != mp_cell_list_frames.end(); ++f, ++i) {
    ...
    i = 0;
    for (std::vector<QToolButton *>::const_iterator f =
             mp_cell_list_headers.begin();
         f != mp_cell_list_headers.end(); ++f, ++i) {
...
QModelIndex HierarchyControlPanel::index_from_path(const cell_path_type &path,
                                                   int cv_index) {
  if (cv_index >= 0 && cv_index < int(mp_cell_lists.size()) && !path.empty()) {

    CellTreeModel *model =
        dynamic_cast<CellTreeModel *>(mp_cell_lists[cv_index]->model());
    if (!model) {
      return QModelIndex();
    }
...
CellTreeItem *
HierarchyControlPanel::find_child_item(cell_path_type::const_iterator start,
                                       cell_path_type::const_iterator end,
                                       CellTreeItem *p) {
  if (start == end) {
    return p;
  } else {
...
std::string HierarchyControlPanel::display_string(int n) const {
...
void HierarchyControlPanel::do_update_content(int cv_index) {
  ...
  unsigned int imax = (cv_index < 0 ? std::numeric_limits<unsigned int>::max()
  ...
  unsigned int n =
      std::min((unsigned int)m_cellviews.size(), mp_view->cellviews());
  for (unsigned int i = imin; i < n && i <= imax; ++i) {
    ...
    if (&m_cellviews[i]->layout() != &mp_view->cellview(i)->layout()) {
      ...
      m_needs_update[i] = true;
    } else if (m_cellviews[i].combined_unspecific_path() !=
  ...
  if (m_cellviews.size() < mp_view->cellviews()) {
    ...
    m_cellviews.erase(m_cellviews.begin() + mp_view->cellviews(),
                      m_cellviews.end());
  }
  ...
  while (mp_cell_lists.size() < m_cellviews.size()) {
    ...
    HCPCellTreeWidget *cell_list =
        new HCPCellTreeWidget(cl_frame, "tree", mp_view->canvas()->widget());
    cl_ly->addWidget(cell_list);
    cell_list->setModel(new CellTreeModel(cell_list, mp_view, cv_index,
                                          m_flat ? CellTreeModel::Flat : 0, 0,
                                          m_sorting));
    ...
    connect(cell_list, SIGNAL(customContextMenuRequested(const QPoint &)), this,
            SLOT(context_menu(const QPoint &)));
    connect(cell_list, SIGNAL(cell_clicked(const QModelIndex &)), this,
            SLOT(clicked(const QModelIndex &)));
    connect(cell_list, SIGNAL(cell_double_clicked(const QModelIndex &)), this,
            SLOT(double_clicked(const QModelIndex &)));
    connect(cell_list, SIGNAL(cell_middle_clicked(const QModelIndex &)), this,
            SLOT(middle_clicked(const QModelIndex &)));
    connect(cell_list, SIGNAL(search_triggered(const QString &)), this,
  ...
  for (unsigned int i = imin;
       i < m_cellviews.size() && i < (unsigned int)mp_selector->count() &&
       i <= imax;
       ++i) {
    ...
    if (m_needs_update[i]) {
      ...
      if (m_force_close[i]) {
        ...
        CellTreeModel *model =
            dynamic_cast<CellTreeModel *>(mp_cell_lists[i]->model());
        if (model) {
          model->configure(mp_view, i, m_flat ? CellTreeModel::Flat : 0, 0,
                           m_sorting);
        }
      }
...
bool HierarchyControlPanel::has_focus() const {
  return m_active_index >= 0 && m_active_index < int(mp_cell_lists.size()) &&
         mp_cell_lists[m_active_index]->hasFocus();
}
...
void HierarchyControlPanel::cut() {
  ...
  //  collect the called cells of the cells to copy, so we don't copy a cell
  //  twice
  ...
  for (std::vector<cell_path_type>::const_iterator p = paths.begin();
       p != paths.end(); ++p) {
    if (!p->empty()) {
  ...
  for (std::vector<cell_path_type>::const_iterator p = paths.begin();
    ...
    if (!p->empty() && called_cells.find(p->back()) == called_cells.end()) {
      db::ClipboardValue<lay::CellClipboardData> *cd =
          new db::ClipboardValue<lay::CellClipboardData>();
      cd->get().add(layout, layout.cell(p->back()),
                    cut_mode == 1 ? 1 /*with subcells*/
                                  : 2 /*first level, then without subcells*/);
  ...
  cell_path_type cell_path(
      mp_view->cellview(m_active_index).combined_unspecific_path());
...
bool HierarchyControlPanel::has_selection() { return (current_item() != 0); }
...
void HierarchyControlPanel::paste() {
  ...
  for (db::Clipboard::iterator c = db::Clipboard::instance().begin();
    ...
    const db::ClipboardValue<lay::CellClipboardData> *value =
        dynamic_cast<const db::ClipboardValue<lay::CellClipboardData> *>(*c);
    if (value) {
  ...
  if (!new_tops.empty()) {
    //  TODO: this does not work properly: since we are inside a transaction,
    //  bboxes are not updated correctly. Thus, the cell_fit does not work
    //  properly.
...
class HierarchyControlPanelPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::submenu("@hcp_context_menu", at, std::string()));

    at = "@hcp_context_menu.end";

    menu_entries.push_back(lay::config_menu_item(
        "flat_mode", at, tl::to_string(QObject::tr("Flat Cell List")),
        cfg_flat_cell_list, "?")),
        menu_entries.push_back(lay::config_menu_item(
            "split_mode", at, tl::to_string(QObject::tr("Split Mode")),
            cfg_split_cell_list, "?")),
        menu_entries.push_back(
            lay::submenu("sorting", at, tl::to_string(QObject::tr("Sorting"))));

    {
      std::string at = "@hcp_context_menu.sorting.end";
      menu_entries.push_back(lay::config_menu_item(
          "by_name", at, tl::to_string(QObject::tr("By Name")),
          cfg_cell_list_sorting, "?by-name"));
      menu_entries.push_back(lay::config_menu_item(
          "by_area", at, tl::to_string(QObject::tr("By Area - Small To Large")),
          cfg_cell_list_sorting, "?by-area"));
      menu_entries.push_back(lay::config_menu_item(
          "by_area_reverse", at,
          tl::to_string(QObject::tr("By Area - Large To Small")),
          cfg_cell_list_sorting, "?by-area-reverse"));
    }

    menu_entries.push_back(lay::separator("operations_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_new_cell", "new_cell:edit:edit_mode", at,
                       tl::to_string(QObject::tr("New Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_delete", "delete_cell:edit:edit_mode", at,
                       tl::to_string(QObject::tr("Delete Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_rename", "rename_cell:edit:edit_mode", at,
                       tl::to_string(QObject::tr("Rename Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_replace", "replace_cell:edit:edit_mode", at,
                       tl::to_string(QObject::tr("Replace Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_flatten", "flatten_cell:edit:edit_mode", at,
                       tl::to_string(QObject::tr("Flatten Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_user_properties", "cell_user_properties", at,
                       tl::to_string(QObject::tr("User Properties"))));
    menu_entries.push_back(lay::separator("clipboard_group:edit_mode", at));
    menu_entries.push_back(lay::menu_item("cm_cell_copy", "copy:edit_mode", at,
                                          tl::to_string(QObject::tr("Copy"))));
    menu_entries.push_back(lay::menu_item("cm_cell_cut", "cut:edit_mode", at,
                                          tl::to_string(QObject::tr("Cut"))));
    menu_entries.push_back(lay::menu_item("cm_cell_paste", "paste:edit_mode",
                                          at,
                                          tl::to_string(QObject::tr("Paste"))));
    menu_entries.push_back(lay::separator("select_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_cell_select", "show_as_top", at,
                       tl::to_string(QObject::tr("Show As New Top"))));
    menu_entries.push_back(lay::separator("visibility_group", at));
    menu_entries.push_back(lay::menu_item("cm_cell_hide", "hide_cell", at,
                                          tl::to_string(QObject::tr("Hide"))));
    menu_entries.push_back(lay::menu_item("cm_cell_show", "show_cell", at,
                                          tl::to_string(QObject::tr("Show"))));
    menu_entries.push_back(
        lay::menu_item("cm_cell_show_all", "show_all", at,
                       tl::to_string(QObject::tr("Show All"))));
    menu_entries.push_back(lay::separator("utils_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_open_current_cell", "open_current", at,
                       tl::to_string(QObject::tr("Where am I?"))));
    menu_entries.push_back(lay::separator("file_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_save_current_cell_as", "save_cell_as:hide_vo", at,
                       tl::to_string(QObject::tr("Save Selected Cells As"))));
  }
};

static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new HierarchyControlPanelPluginDeclaration(), -8,
