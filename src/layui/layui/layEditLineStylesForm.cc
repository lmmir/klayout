...
struct CurrentStyleOp : public db::Op {
  CurrentStyleOp(int pi, int ni) : db::Op(), prev_index(pi), new_index(ni) {
    //  .. nothing yet ..
  }
...
EditLineStylesForm::EditLineStylesForm(QWidget *parent,
                                       lay::LayoutViewBase *view,
                                       const lay::LineStyles &styles)
    : QDialog(parent), db::Object(0), m_selected(-1), m_styles(styles),
  ...
  mp_ui->style_items->setCurrentItem(
      mp_ui->style_items->item(mp_ui->style_items->count() - 1));
...
void EditLineStylesForm::update() {
  ...
  for (lay::LineStyles::iterator i = m_styles.begin_custom();
       i != m_styles.end(); ++i) {
    iters.push_back(i);
  }
  ...
  for (lay::LineStyles::iterator i = m_styles.begin();
       i != m_styles.begin_custom(); ++i) {
    std::string name(i->name());
    if (name.empty()) {
      name = tl::sprintf("#%d", std::distance(m_styles.begin(), i));
    }
    QListWidgetItem *item = new QListWidgetItem(
        icon_from_data(*i), tl::to_qstring(name), mp_ui->style_items);
    item->setForeground(cdis);
  }
  for (std::vector<lay::LineStyles::iterator>::const_iterator i = iters.begin();
       i != iters.end(); ++i) {
    if ((*i)->order_index() > 0) {
      std::string name((*i)->name());
      if (name.empty()) {
      ...
      new QListWidgetItem(icon_from_data(**i), tl::to_qstring(name),
                          mp_ui->style_items);
    }
  }
...
void EditLineStylesForm::double_clicked(QListWidgetItem *citem) {
  ...
  if (i != m_styles.end() && i >= m_styles.begin_custom()) {
    ...
    QString new_name = QInputDialog::getText(
        this, QObject::tr("Edit Style Description"),
        QObject::tr("Enter new description of style"), QLineEdit::Normal,
...
void EditLineStylesForm::sel_changed(QListWidgetItem *, QListWidgetItem *) {
  ...
  if (manager()) {
    ...
    manager()->queue(
        this, new CurrentStyleOp(m_selected, mp_ui->style_items->currentRow()));
    manager()->commit();
  }
...
void EditLineStylesForm::delete_button_clicked() {
  ...
  if (i != m_styles.end() && i >= m_styles.begin_custom()) {
    ...
    for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
         !l.at_end(); ++l) {
      if (int(l->eff_line_style(true /*real*/)) ==
          std::distance(m_styles.begin(), i)) {
        throw tl::Exception(
            tl::to_string(QObject::tr(
                "Cannot delete style: style is being used by layer '")) +
    ...
    if (oi > 1) {
      ...
      for (lay::LineStyles::iterator i = m_styles.begin_custom();
        ...
        if (i->order_index() == oi - 1) {
          ...
          update();
          select_item(oi - 2 +
                      std::distance(m_styles.begin(), m_styles.begin_custom()));
...
lay::LineStyles::iterator EditLineStylesForm::current() {
  return index_of(mp_ui->style_items->currentItem());
}

lay::LineStyles::iterator EditLineStylesForm::index_of(QListWidgetItem *item) {
...
void EditLineStylesForm::handle_op(db::Op *op, bool undo) {
  ...
  if (cp_op) {
    ...
    mp_ui->style_items->setCurrentItem(
        mp_ui->style_items->item(undo ? cp_op->prev_index : cp_op->new_index));
    update_current_item();
...
void EditLineStylesForm::undo(db::Op *op) { handle_op(op, true); }

void EditLineStylesForm::redo(db::Op *op) { handle_op(op, false); }
