...
LayerMappingWidget::LayerMappingWidget(QWidget *parent)
  ...
  connect(mp_ui->delete_pb, SIGNAL(clicked()), this,
          SLOT(delete_button_pressed()));
  ...
  connect(mp_ui->tabs, SIGNAL(currentChanged(int)), this,
          SLOT(current_tab_changed(int)));

  mp_layer_table_file_dialog = new lay::FileDialog(
      this, tl::to_string(QObject::tr("Load Layer Table")),
      tl::to_string(QObject::tr(
          "Layer properties and text files (*.lyp *.txt);;Layer properties "
          "files (*.lyp);;Text files (*.txt);;All files (*)")));
}
...
void LayerMappingWidget::set_layer_map(const db::LayerMap &lm) {
  ...
  for (std::vector<unsigned int>::const_iterator layer_id = layer_ids.begin();
       layer_id != layer_ids.end(); ++layer_id) {
...
db::LayerMap LayerMappingWidget::get_layer_map() const {
  return get_layer_map_from_tab(mp_ui->tabs->currentIndex());
}

db::LayerMap LayerMappingWidget::get_layer_map_from_tab(int tab) const {
  db::LayerMap lm;

  if (tab == 0) {

    for (int i = 0; i < mp_ui->layer_lv->count(); ++i) {
      std::string t = tl::to_string(
          mp_ui->layer_lv->item(i)->data(Qt::DisplayRole).toString());
      try {
    ...
    lm = db::LayerMap::from_string_file_format(
        tl::to_string(mp_ui->text_edit->toPlainText()));
  }
...
void LayerMappingWidget::load_button_pressed() {
  ...
  if (mp_layer_table_file_dialog->get_open(m_layer_table_file)) {
    ...
    try {
      ...
      unsigned int n = 0;
      for (LayerPropertiesConstIterator lay_iter =
               props.begin_const_recursive();
           !lay_iter.at_end(); ++lay_iter) {
        if (!lay_iter->has_children() &&
            lay_iter->source(true /*=real*/).cv_index() == 0) {
          db::LayerProperties db_lp =
              lay_iter->source(true /*=real*/).layer_props();
    ...
    if (!success) {
      ...
      if (input.good()) {
        std::string text = std::string(std::istreambuf_iterator<char>(input),
...
void LayerMappingWidget::delete_button_pressed() {
  ...
  if (mp_ui->layer_lv->currentItem()) {
    ...
    for (QList<QListWidgetItem *>::const_iterator sel_item = sel_items.begin();
         sel_item != sel_items.end(); ++sel_item) {
      delete *sel_item;
    }
