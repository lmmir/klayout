...
TipDialog::TipDialog(QWidget *parent, const std::string &text,
                     const std::string &key, buttons_type buttons)
...
static std::pair<bool, int> tip_dialog_status(const std::string &key) {
  std::string th;
...
bool TipDialog::will_be_shown() { return tip_dialog_status(m_key).first; }
