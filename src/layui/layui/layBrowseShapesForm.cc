...
class BrowseShapesPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_options(std::vector<std::pair<std::string, std::string>> &options) const {
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_context_cell, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_context_mode, "any-top"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_window_mode, "fit-marker"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_window_state, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_window_dim, "1.0"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_max_inst_count, "1000"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_shb_max_shape_count, "1000"));
  }
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::menu_item("browse_shapes::show", "browse_shapes", "tools_menu.end",
                       tl::to_string(QObject::tr("Browse Shapes"))));
  }
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new BrowseShapesPluginDeclaration(), 10000,
                "BrowseShapesPlugin");
...
static struct {
  ...
  const char *string;
} context_modes[] = {{BrowseShapesForm::AnyTop, "any-top"},
...
public:
  void from_string(const std::string &value,
    ...
    for (unsigned int i = 0;
         i < sizeof(context_modes) / sizeof(context_modes[0]); ++i) {
    ...
    throw tl::Exception(
        tl::to_string(QObject::tr("Invalid cell browser context mode: ")) +
        value);
  }

  std::string to_string(BrowseShapesForm::mode_type mode) {
    for (unsigned int i = 0;
         i < sizeof(context_modes) / sizeof(context_modes[0]); ++i) {
...
static struct {
  ...
  const char *string;
} window_modes[] = {{BrowseShapesForm::DontChange, "dont-change"},
...
public:
  void from_string(const std::string &value,
    ...
    throw tl::Exception(
        tl::to_string(QObject::tr("Invalid cell browser window mode: ")) +
        value);
  }

  std::string to_string(BrowseShapesForm::window_type mode) {
    for (unsigned int i = 0; i < sizeof(window_modes) / sizeof(window_modes[0]);
         ++i) {
...
BrowseShapesConfigPage::BrowseShapesConfigPage(QWidget *parent)
  ...
  connect(cbx_context, SIGNAL(currentIndexChanged(int)), this,
          SLOT(context_changed(int)));
...
void BrowseShapesConfigPage::setup(lay::Dispatcher *root) {
  ...
  root->config_get(cfg_shb_context_mode, cmode,
                   BrowseShapesContextModeConverter());
  cbx_context->setCurrentIndex(int(cmode));

  //  window mode
  BrowseShapesForm::window_type wmode = BrowseShapesForm::FitMarker;
  root->config_get(cfg_shb_window_mode, wmode,
                   BrowseShapesWindowModeConverter());
...
void BrowseShapesConfigPage::window_changed(int m) {
  mrg_window->setEnabled(m == int(BrowseShapesForm::FitMarker) ||
                         m == int(BrowseShapesForm::CenterSize));
}
...
void BrowseShapesConfigPage::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_shb_context_mode,
                   BrowseShapesForm::mode_type(cbx_context->currentIndex()),
                   BrowseShapesContextModeConverter());
  root->config_set(cfg_shb_window_mode,
                   BrowseShapesForm::window_type(cbx_window->currentIndex()),
                   BrowseShapesWindowModeConverter());
...
public:
  BrowseShapesFormLVI(const std::string &text)
      : QTreeWidgetItem(), m_value(0.0), m_value_flat(0.0) {
    setText(0, tl::to_qstring(text));
  }

  virtual bool operator<(const QTreeWidgetItem &i) const {
    const BrowseShapesFormLVI *other =
        dynamic_cast<const BrowseShapesFormLVI *>(&i);
    if (other) {
  ...
  void set_value_flat(double v) { m_value_flat = v; }
...
public:
  BrowseShapesFormCellLVI(const std::string &cn,
                          lay::CellView::cell_index_type index,
                          unsigned int lindex)
      : BrowseShapesFormLVI(cn), m_index(index), m_lindex(lindex) {
    //  .. nothing yet ..
  }

  lay::CellView::cell_index_type index() const { return m_index; }

  unsigned int lindex() const { return m_lindex; }
...
public:
  BrowseShapesFormCellInstanceLVI(const std::string &text,
                                  const std::string &path,
                                  const db::ICplxTrans &trans,
                                  lay::CellView::cell_index_type index)
      : QTreeWidgetItem(), m_trans(trans), m_index(index) {
    setText(0, tl::to_qstring(text));
    setText(1, tl::to_qstring(path));
  }

  const db::ICplxTrans &trans() const { return m_trans; }
...
public:
  ...
  const db::Shape &shape() const { return *(m_iter.operator->()); }
...
BrowseShapesForm::BrowseShapesForm(lay::Dispatcher *root, LayoutViewBase *vw)
    : lay::Browser(root, vw), Ui::BrowseShapesForm(), m_cv_index(-1),
      m_cell_changed_enabled(true), m_view_changed(false),
      m_cell_inst_changed_enabled(true), m_shape_inst_changed_enabled(true),
      m_ef_enabled(true), m_mode(AnyTop), m_window(FitMarker), m_window_dim(),
      m_max_inst_count(0), m_max_shape_count(0) {
  Ui::BrowseShapesForm::setupUi(this);

  lv_cell->setSortingEnabled(
      false); // don't enable sorting: this messes up the list completely
  ...
  connect(lv_cell,
          ...
          this, SLOT(cell_changed(QTreeWidgetItem *, QTreeWidgetItem *)));
  connect(lv_cell_instance,
          SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
          this, SLOT(cell_inst_changed(QTreeWidgetItem *, QTreeWidgetItem *)));
  connect(lv_shape_instance, SIGNAL(itemSelectionChanged()), this,
          SLOT(shape_inst_changed()));
...
BrowseShapesForm::~BrowseShapesForm() { remove_marker(); }
...
bool BrowseShapesForm::configure(const std::string &name,
...
void BrowseShapesForm::remove_marker() {
  for (std::vector<lay::ShapeMarker *>::iterator m = mp_markers.begin();
       m != mp_markers.end(); ++m) {
    delete *m;
  }
  mp_markers.clear();
...
void BrowseShapesForm::cell_changed(QTreeWidgetItem *item, QTreeWidgetItem *) {
  ...
  db::ICplxTrans trans = db::VCplxTrans(1.0 / dbu) *
                         m_lprops[it->lindex()]->trans()[0] *
                         db::CplxTrans(dbu);
  const std::set<db::properties_id_type> *prop_sel =
      &m_lprops[it->lindex()]->prop_sel();
  ...
  if (m_mode == AnyTop) {
    shortened = fill_cell_instances(db::ICplxTrans(), layout, &cell, 0, false,
                                    std::string(), items, count);
  } else if (m_mode == ToCellView) {
    if (m_cellview.is_valid()) {
      shortened = fill_cell_instances(db::ICplxTrans(), layout, &cell,
                                      m_cellview.cell(), false, std::string(),
                                      items, count);
    }
  }
  ...
  for (shape = cell.shapes(layer).begin(db::ShapeIterator::All, prop_sel,
                                        inv_prop_sel);
       !shape.at_end() && count++ < m_max_shape_count; ++shape) {
    ...
    items.append(new BrowseShapesFormShapeInstanceLVI(
        (name + std::string(" at (") +
         tl::micron_to_string(0.5 * dbu * (box.left() + box.right())) + "," +
...
void BrowseShapesForm::activated() {
  ...
  std::vector<lay::LayerPropertiesConstIterator> sel_layers =
      ...
      if (m_cv_index >= 0) {
        if (cv_index != m_cv_index) {
          throw tl::Exception(tl::to_string(
              QObject::tr("Layers selected for shape browsing must originate "
                          "from the same cellview")));
        }
      } else {
        m_cv_index = cv_index;
...
struct BrowseShapesCellInfo {
  BrowseShapesCellInfo(const std::string &n, size_t s, size_t sf,
                       lay::CellView::cell_index_type i)
  ...
  bool operator<(const BrowseShapesCellInfo &d) const { return name < d.name; }
};

template <class T>
size_t num_shape_instances(unsigned int layer, const db::Cell &cell) {
  typename T::tag tag;
  return std::distance(cell.shapes(layer).begin(tag),
                       cell.shapes(layer).end(tag));
}
...
void BrowseShapesForm::update_cell_list() {
  ...
  for (unsigned int lindex = 0; lindex < m_lprops.size(); ++lindex) {
    ...
    if (m_cv_index >= 0) {
      ...
      BrowseShapesFormLayerLVI *layer_root = new BrowseShapesFormLayerLVI(
          m_lprops[lindex]->display_string(view(), true /*real*/));
      ...
      for (db::Layout::const_iterator c = layout.begin(); c != layout.end();
           ++c) {
        size_t shapes = 0;
        for (db::ShapeIterator iter =
                 (*c).shapes(m_lprops[lindex]->layer_index())
                     .begin(db::ShapeIterator::All, prop_sel, inv_prop_sel);
             !iter.at_end(); ++iter) {
          ++shapes;
        }
        if (shapes > 0) {
          ...
          cell_info.push_back(
              BrowseShapesCellInfo(layout.cell_name(c->cell_index()), shapes,
                                   shapes * cm, c->cell_index()));
          all_shapes += shapes;
      ...
      for (std::vector<BrowseShapesCellInfo>::const_iterator cn =
               cell_info.end();
           cn != cell_info.begin();) {
        --cn;
...
bool BrowseShapesForm::fill_cell_instances(
    const db::ICplxTrans &t, const db::Layout &layout, const db::Cell *from,
    const db::Cell *to, bool to_parent, const std::string &path,
    QList<QTreeWidgetItem *> &items, unsigned int &count) {
  if (from == to || (!to_parent && to == 0 && from->is_top())) {
    ...
    items.append(
        new BrowseShapesFormCellInstanceLVI(text, path, t, from->cell_index()));

  } else {

    //  traverse the parents
    for (db::Cell::parent_inst_iterator p = from->begin_parent_insts();
         !p.at_end(); ++p) {
      ...
      if (fill_cell_instances(tt.inverted() * t, layout, cell,
                              to_parent ? cell : to, false, new_path, items,
                              count)) {
...
void BrowseShapesForm::highlight_current() {
  ...
  BrowseShapesFormCellLVI *item =
      dynamic_cast<BrowseShapesFormCellLVI *>(lv_cell->currentItem());
  if (!item) {
    return;
  }

  if (m_mode != Local) {
    BrowseShapesFormCellInstanceLVI *ci_item =
        dynamic_cast<BrowseShapesFormCellInstanceLVI *>(
            lv_cell_instance->currentItem());
    if (!ci_item) {
      return;
    }
    t = ci_item->trans();
  ...
  for (QList<QTreeWidgetItem *>::const_iterator s = selected_items.begin();
       s != selected_items.end(); ++s) {

    BrowseShapesFormShapeInstanceLVI *shape_item =
        dynamic_cast<BrowseShapesFormShapeInstanceLVI *>(*s);
    if (shape_item) {
      ...
      dbox += (db::CplxTrans(layout.dbu()) * shape_item->trans() * t) *
              shape_item->shape().bbox();
    }
  }
...
bool BrowseShapesForm::adv_cell(bool up) {
  ...
  QKeyEvent ke(QEvent::KeyPress, up ? Qt::Key_Up : Qt::Key_Down,
               Qt::NoModifier);
  ...
  if (lv_cell->currentItem() != current) {
    ...
    BrowseShapesFormLayerLVI *litem =
        dynamic_cast<BrowseShapesFormLayerLVI *>(lv_cell->currentItem());
    if (litem) {
      if (!up) {
        while (litem && litem->childCount() == 0) {
          litem =
              dynamic_cast<BrowseShapesFormLayerLVI *>(lv_cell->topLevelItem(
                  lv_cell->indexOfTopLevelItem(litem) + 1)); // TODO: slow!
        }
        if (litem) {
        ...
        do {
          ...
          if (i > 0) {
            litem = dynamic_cast<BrowseShapesFormLayerLVI *>(
                lv_cell->topLevelItem(i - 1)); // TODO: slow!
          } else {
            litem = 0;
          }
        } while (litem && litem->childCount() == 0);
...
bool BrowseShapesForm::adv_shape(bool up) {
  ...
  if (lv_shape_instance->currentItem() == current) {
    ...
    if (adv_cell(up)) {
      ...
      if (up) {
        QTreeWidgetItem *ni = lv_shape_instance->topLevelItem(
            lv_shape_instance->topLevelItemCount() - 1);
        if (ni) {
...
bool BrowseShapesForm::adv_cell_inst(bool up) {
  ...
  if (lv_cell_instance->currentItem() == current) {
    ...
    if (adv_shape(up)) {
      ...
      QTreeWidgetItem *ni;
      if (up) {
        ni = lv_cell_instance->topLevelItem(
            lv_cell_instance->topLevelItemCount() - 1);
      } else {
