...
struct CurrentPatternOp : public db::Op {
  CurrentPatternOp(int pi, int ni) : db::Op(), prev_index(pi), new_index(ni) {
    //  .. nothing yet ..
  }
...
EditStipplesForm::EditStipplesForm(QWidget *parent, lay::LayoutViewBase *view,
                                   const lay::DitherPattern &pattern)
    : QDialog(parent), db::Object(0), m_selected(-1), m_pattern(pattern),
      m_manager(true), mp_view(view) {
  ...
  connect(mp_ui->new_button, SIGNAL(clicked()), this,
          SLOT(new_button_clicked()));
  connect(mp_ui->delete_button, SIGNAL(clicked()), this,
          SLOT(delete_button_clicked()));
  connect(mp_ui->clone_button, SIGNAL(clicked()), this,
          SLOT(clone_button_clicked()));
  connect(mp_ui->up_button, SIGNAL(clicked()), this, SLOT(up_button_clicked()));
  connect(mp_ui->down_button, SIGNAL(clicked()), this,
          SLOT(down_button_clicked()));
  connect(mp_ui->invert_button, SIGNAL(clicked()), this,
          SLOT(invert_button_clicked()));
  connect(mp_ui->clear_button, SIGNAL(clicked()), this,
          SLOT(clear_button_clicked()));
  connect(mp_ui->rotate_button, SIGNAL(clicked()), this,
          SLOT(rotate_button_clicked()));
  connect(mp_ui->fliph_button, SIGNAL(clicked()), this,
          SLOT(fliph_button_clicked()));
  connect(mp_ui->flipv_button, SIGNAL(clicked()), this,
          SLOT(flipv_button_clicked()));
  connect(mp_ui->sleft_button, SIGNAL(clicked()), this,
          SLOT(sleft_button_clicked()));
  connect(mp_ui->sright_button, SIGNAL(clicked()), this,
          SLOT(sright_button_clicked()));
  connect(mp_ui->sup_button, SIGNAL(clicked()), this,
          SLOT(sup_button_clicked()));
  connect(mp_ui->sdown_button, SIGNAL(clicked()), this,
          SLOT(sdown_button_clicked()));
  connect(mp_ui->undo_button, SIGNAL(clicked()), this,
          SLOT(undo_button_clicked()));
  connect(mp_ui->redo_button, SIGNAL(clicked()), this,
          SLOT(redo_button_clicked()));
  connect(mp_ui->h_spin_box, SIGNAL(valueChanged(int)), this,
          SLOT(size_changed()));
  connect(mp_ui->w_spin_box, SIGNAL(valueChanged(int)), this,
          SLOT(size_changed()));

  connect(mp_ui->editor, SIGNAL(changed()), this, SLOT(edited()));
  connect(mp_ui->editor, SIGNAL(size_changed()), this,
          SLOT(editor_size_changed()));

  mp_ui->stipple_items->setCurrentItem(
      mp_ui->stipple_items->item(mp_ui->stipple_items->count() - 1));
...
static QIcon icon_from_data(const uint32_t *const *p) {
  ...
  QBitmap bitmap(
      QBitmap::fromData(QSize(34, 36), data, QImage::Format_MonoLSB));
  QIcon icon(bitmap);
#ifdef _WIN32
...
void EditStipplesForm::update() {
  ...
  QColor cdis((c0.red() + c1.red()) / 2, (c0.green() + c1.green()) / 2,
              (c0.blue() + c1.blue()) / 2);
  ...
  for (lay::DitherPattern::iterator i = m_pattern.begin();
    ...
    QListWidgetItem *item =
        new QListWidgetItem(icon_from_data(i->pattern()), tl::to_qstring(name),
                            mp_ui->stipple_items);
...
void EditStipplesForm::double_clicked(QListWidgetItem *citem) {
  ...
  if (i != m_pattern.end() && i >= m_pattern.begin_custom()) {
    ...
    QString new_name = QInputDialog::getText(
        this, QObject::tr("Edit Stipple Description"),
        QObject::tr("Enter new description of pattern"), QLineEdit::Normal,
        tl::to_qstring(i->name()), &ok);
    if (ok) {
...
void EditStipplesForm::sel_changed(QListWidgetItem *, QListWidgetItem *) {
  ...
  if (manager()) {
    ...
    manager()->queue(this, new CurrentPatternOp(
                               m_selected, mp_ui->stipple_items->currentRow()));
    manager()->commit();
  }
...
void EditStipplesForm::update_current_item() {
  ...
  lay::DitherPattern::iterator i =
      index_of(mp_ui->stipple_items->currentItem());
  if (i == m_pattern.end()) {
...
void EditStipplesForm::new_button_clicked() {
  ...
  unsigned int oi =
      m_pattern.begin()[m_pattern.add_pattern(p)].order_index() - 1;
...
void EditStipplesForm::delete_button_clicked() {
  ...
  if (i != m_pattern.end() && i >= m_pattern.begin_custom()) {
    ...
    for (lay::LayerPropertiesConstIterator l = mp_view->begin_layers();
         !l.at_end(); ++l) {
      if (int(l->eff_dither_pattern(true /*real*/)) ==
          std::distance(m_pattern.begin(), i)) {
        throw tl::Exception(
            tl::to_string(QObject::tr(
                "Cannot delete stipple: stipple is being used by layer '")) +
            l->display_string(mp_view, true /*real*/) + "'");
      }
    }
    ...
    if (mp_ui->stipple_items->currentRow() + 1 ==
        mp_ui->stipple_items->count()) {
    ...
    if (oi > 1) {
      ...
      for (lay::DitherPattern::iterator i = m_pattern.begin_custom();
        ...
        if (i->order_index() == oi - 1) {
          ...
          if (manager()) {
            manager()->transaction(
                tl::to_string(QObject::tr("Move pattern up")));
          }
  ...
  if (c != m_pattern.end() && c >= m_pattern.begin_custom()) {
    ...
    for (lay::DitherPattern::iterator i = m_pattern.begin_custom();
      ...
      if (i->order_index() == oi + 1) {
        ...
        if (manager()) {
          manager()->transaction(
              tl::to_string(QObject::tr("Move pattern down")));
        }
...
lay::DitherPattern::iterator EditStipplesForm::current() {
  return index_of(mp_ui->stipple_items->currentItem());
}

lay::DitherPattern::iterator EditStipplesForm::index_of(QListWidgetItem *item) {
...
void EditStipplesForm::edited() {
  if (mp_ui->stipple_items->currentItem()) {
    ...
    if (i != m_pattern.end() && i >= m_pattern.begin_custom()) {
      ...
      info.set_pattern(mp_ui->editor->pattern(), mp_ui->editor->sx(),
                       mp_ui->editor->sy());
      m_pattern.replace_pattern(std::distance(m_pattern.begin(), i), info);

      mp_ui->stipple_items->currentItem()->setIcon(
          icon_from_data(info.pattern()));
    }
  }
}
...
void EditStipplesForm::handle_op(db::Op *op, bool undo) {
  ...
  if (cp_op) {
    ...
    mp_ui->stipple_items->setCurrentItem(mp_ui->stipple_items->item(
        undo ? cp_op->prev_index : cp_op->new_index));
    update_current_item();
...
void EditStipplesForm::undo(db::Op *op) { handle_op(op, true); }

void EditStipplesForm::redo(db::Op *op) { handle_op(op, false); }
