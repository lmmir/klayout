...
FileDialog::FileDialog(QWidget *parent, const std::string &title,
                       const std::string &filters,
                       const std::string &def_suffix)
    : QObject(parent) {
#ifdef _WIN32
#if QT_VERSION >= 0x050000
  m_dir =
...
#else
  //  On Windows the current directory does not have any meaning - it's usually
...
int FileDialog::find_selected_filter(const QString &fs,
...
std::string FileDialog::add_default_extension(const std::string &path,
                                              const QString &selected_filter) {
...
bool FileDialog::get_open(std::string &fp, const std::string &title) {
  ...
  QString f = QFileDialog::getOpenFileName(
      ...
      (title.empty() ? m_title : tl::to_qstring(title)), file_name, m_filters,
      &m_sel_filter);
...
bool FileDialog::get_open(std::vector<std::string> &fp, const std::string &dir,
  ...
  QStringList files = QFileDialog::getOpenFileNames(
      QApplication::activeWindow(),
      (title.empty() ? m_title : tl::to_qstring(title)), m_dir.absolutePath(),
      m_filters, &m_sel_filter);
