...
static bool operator<(const QList<QString> &a, const QList<QString> &b) {
  if (a.size() != b.size()) {
    ...
    for (QList<QString>::const_iterator ia = a.begin(), ib = b.begin();
         ia != a.end(); ++ia, ++ib) {
      if (*ia != *ib) {
        return *ia < *ib;
      }
...
inline bool is_word_char(QChar c) {
 ...
 */
static QString replace_tokens(const QString &input,
                              const QList<QString> &input_args) {
  QString output = input;

  int i = 1;
  for (QList<QString>::const_iterator a = input_args.begin();
       a != input_args.end(); ++a, ++i) {
...
GenericSyntaxHighlighterRuleStringList::GenericSyntaxHighlighterRuleStringList(
    const QList<QString> &sl) {
...
GenericSyntaxHighlighterRuleStringList::GenericSyntaxHighlighterRuleStringList(
    const std::set<QString> &s, int ml)
    : m_s(s), m_min_length(ml) {
  //  .. nothing yet ..
}

GenericSyntaxHighlighterRuleBase *
GenericSyntaxHighlighterRuleStringList::clone() const {
  return new GenericSyntaxHighlighterRuleStringList(m_s, m_min_length);
}

bool GenericSyntaxHighlighterRuleStringList::match(
    const QString &input, unsigned int /*generation_id*/, int index,
    int &end_index, const QList<QString> & /*input_args*/,
    QList<QString> & /*output_args*/) const {
  ...
  if (sp != m_s.begin()) {
    --sp;
    if (ms.startsWith(*sp) && (index + sp->length() == input.length() ||
                               !is_word_char(input[index + sp->length()]))) {
...
/**
 *  @brief A specialization of GenericSyntaxHighlighterRuleBase which looks for
 * a string
 */
class GenericSyntaxHighlighterRuleString
    : public GenericSyntaxHighlighterRuleBase {
public:
  GenericSyntaxHighlighterRuleString(const QString &s, bool insensitive = false,
                                     bool dynamic = false)
  ...
  virtual bool match(const QString &input, unsigned int /*generation_id*/,
  ...
  virtual void dump() const {
    std::cout << "    rule(string) '" << tl::to_string(m_s)
              << "' dynamic=" << m_dynamic << ", insensitive=" << m_insensitive
              << std::endl;
  }
...
/**
 *  @brief A specialization of GenericSyntaxHighlighterRuleBase which looks for
 * a range between two strings
 */
class GenericSyntaxHighlighterRuleRange
    : public GenericSyntaxHighlighterRuleBase {
public:
  GenericSyntaxHighlighterRuleRange(const QString &s1, const QString &s2,
                                    bool dynamic = false)
  ...
  virtual void dump() const {
    std::cout << "    rule(range) '" << tl::to_string(m_s1) << "'..'"
              << tl::to_string(m_s2) << "' dynamic=" << m_dynamic << std::endl;
  }
...
public:
  ...
  virtual bool match(const QString &input, unsigned int /*generation_id*/,
    ...
    if (end_index < input.length() &&
        input[end_index] == QChar::fromLatin1('-')) {
      ++end_index;
    }
  ...
  virtual void dump() const { std::cout << "    rule(int)" << std::endl; }
};
...
public:
  ...
  virtual bool match(const QString &input, unsigned int /*generation_id*/,
    ...
    if (end_index < input.length() &&
        input[end_index].toLower() == QChar::fromLatin1('e')) {
      ++end_index;
      if (end_index < input.length() &&
          input[end_index] == QChar::fromLatin1('-')) {
        ++end_index;
  ...
  virtual void dump() const { std::cout << "    rule(float)" << std::endl; }
};
...
public:
  ...
  virtual void dump() const { std::cout << "    rule(spaces)" << std::endl; }
};

/**
 *  @brief A specialization of GenericSyntaxHighlighterRuleBase which looks for
 * a character from a range
 */
class GenericSyntaxHighlighterRuleAnyChar
    : public GenericSyntaxHighlighterRuleBase {
public:
  GenericSyntaxHighlighterRuleAnyChar(const QString &s) : m_s(s) {
    //  .. nothing yet ..
  }
...
public:
  ...
  GenericSyntaxHighlighterRuleRegExp(const QString &re, bool dynamic = false)
      : m_re(re), m_dynamic(dynamic), m_c(QChar::Null), m_anchor(AnchorNone),
        m_last_generation_id(0), m_last_index(-1) {
  ...
  GenericSyntaxHighlighterRuleRegExp(const QRegExp &re, bool dynamic, QChar c)
      : m_re(re), m_dynamic(dynamic), m_c(c), m_anchor(AnchorNone),
        m_last_generation_id(0), m_last_index(-1) {
    //  .. nothing yet ..
  }
  ...
  virtual bool match(const QString &input, unsigned int generation_id,
    ...
    //  shortcut
    if (m_c != QChar::Null &&
        (input.length() <= index || input[index] != m_c)) {
      return false;
    }
    ...
    if (!m_dynamic && generation_id == m_last_generation_id &&
        (index < m_last_index || m_last_index < 0)) {
      return false;
    }
  ...
  virtual void dump() const {
    std::cout << "    rule(regexp) '" << tl::to_string(m_re.pattern())
              << "' dynamic=" << m_dynamic << std::endl;
  }
...
GenericSyntaxHighlighterRule::GenericSyntaxHighlighterRule()
    : mp_rule(0), m_attribute_id(-1), m_target_context_id(-1), m_owner(true),
      m_lookahead(false), m_first_non_space(false), m_column(-1) {
  //  .. nothing yet ..
}

GenericSyntaxHighlighterRule::GenericSyntaxHighlighterRule(
    GenericSyntaxHighlighterRuleBase *rule, int attribute_id,
    int target_context_id, bool take_ownership)
    : mp_rule(rule), m_attribute_id(attribute_id),
      m_target_context_id(target_context_id), m_owner(take_ownership),
      m_lookahead(false), m_first_non_space(false), m_column(-1) {
  //  .. nothing yet ..
}

GenericSyntaxHighlighterRule::GenericSyntaxHighlighterRule(
    const GenericSyntaxHighlighterRule &d) {
  m_owner = d.m_owner;
  if (d.m_owner) {
...
bool GenericSyntaxHighlighterRule::match(const QString &input,
                                         unsigned int generation_id, int index,
                                         int &end_index,
                                         const QList<QString> &input_args,
                                         QList<QString> &output_args) const {
  ...
  if (mp_rule && mp_rule->match(input, generation_id, index, end_index,
    ...
    for (std::list<GenericSyntaxHighlighterRule>::const_iterator r =
             m_child_rules.begin();
         r != m_child_rules.end(); ++r) {
      if (r->match(input, generation_id, end_index, new_ei, input_args,
                   new_oa)) {
        end_index = new_ei;
        break;
      }
...
void GenericSyntaxHighlighterRule::dump() const {
  std::cout << "    [attribute=" << m_attribute_id
            << ", context_id=" << m_target_context_id << ", column=" << m_column
            << ", first-non-space=" << m_first_non_space
            << ", lookahead=" << m_lookahead << "]" << std::endl;
  mp_rule->dump();
...
GenericSyntaxHighlighterContext::GenericSyntaxHighlighterContext()
    : m_id(-1), m_fallthrough_context(no_context),
      m_linebegin_context(no_context), m_lineend_context(no_context),
      m_attribute_id(-1) {
  // .. nothing yet ..
}

void GenericSyntaxHighlighterContext::add_rule(
    const GenericSyntaxHighlighterRule &rule) {
  if (!rule.is_null()) {
    m_rules.push_back(rule);
  }
}

bool GenericSyntaxHighlighterContext::match(
    const QString &string, unsigned int generation_id, int index,
    int &end_index, const QList<QString> &input_args,
    QList<QString> &output_args, int &new_context, int &attribute_id) const {
  end_index = index;
  output_args.clear();
  ...
  if (index == string.length()) {
    if (m_lineend_context != no_context &&
        m_lineend_context != 0 /*#stay cannot be a lineend context*/) {
      end_index = index;
  ...
  bool has_fallthrough =
      (m_fallthrough_context != no_context &&
       m_fallthrough_context != 0 /*fallthrough cannot be #stay*/);
  ...
  for (std::list<GenericSyntaxHighlighterRule>::const_iterator r =
    ...
    int ei = 0;
    QList<QString> oa;
    if (r->match(string, generation_id, index, ei, input_args, oa) &&
        ei > end_index /*also avoids zero-width matches*/) {
      end_index = ei;
      output_args = oa;
...
void GenericSyntaxHighlighterContext::include(
    const GenericSyntaxHighlighterContext &other) {
  //  TODO: don't create copies here but rather reference?
  for (std::list<GenericSyntaxHighlighterRule>::const_iterator r =
           other.m_rules.begin();
       r != other.m_rules.end(); ++r) {
    add_rule(*r);
  }
}

void GenericSyntaxHighlighterContext::dump() const {
  std::cout << "  [context id=" << m_id
            << ", fallthrough=" << m_fallthrough_context
            << ", linebegin=" << m_linebegin_context
            << ", lineend=" << m_lineend_context
            << ", attribute=" << m_attribute_id << "]" << std::endl;
  for (std::list<GenericSyntaxHighlighterRule>::const_iterator r =
           m_rules.begin();
       r != m_rules.end(); ++r) {
    std::cout << "  ";
...
void GenericSyntaxHighlighterContexts::insert(
    const QString &name, const GenericSyntaxHighlighterContext &c) {
...
GenericSyntaxHighlighterContexts::context(const QString &name) {
  std::map<QString, GenericSyntaxHighlighterContext>::iterator c =
      m_contexts_by_name.find(name);
  if (c != m_contexts_by_name.end()) {
    return c->second;
  } else {
    GenericSyntaxHighlighterContext *context =
        &m_contexts_by_name
             .insert(std::make_pair(name, GenericSyntaxHighlighterContext()))
             .first->second;
...
GenericSyntaxHighlighterContexts::context(const QString &name) const {
  std::map<QString, GenericSyntaxHighlighterContext>::const_iterator c =
      m_contexts_by_name.find(name);
...
void GenericSyntaxHighlighterContexts::dump() const {
  ...
  for (std::map<QString, GenericSyntaxHighlighterContext>::const_iterator c =
           m_contexts_by_name.begin();
       c != m_contexts_by_name.end(); ++c) {
...
GenericSyntaxHighlighterAttributes::GenericSyntaxHighlighterAttributes(
    const GenericSyntaxHighlighterAttributes *basic_attributes)
    : mp_basic_attributes(basic_attributes) {
  if (!basic_attributes) {
    add(QString::fromUtf8("Normal"), dsNormal, false, false, false, false, 0, 0,
        0, 0);
    add(QString::fromUtf8("Alert"), dsAlert, true, false, false, false,
        "#BF0303", "#9C0D0D", "#F7E7E7", 0);
    add(QString::fromUtf8("Base-N Integer"), dsBaseN, false, false, false,
        false, "#B07E00", "#FFDD00", 0, 0);
    add(QString::fromUtf8("Character"), dsChar, false, false, false, false,
        "#FF80E0", "#FF80E0", 0, 0);
    add(QString::fromUtf8("Comment"), dsComment, false, true, false, false,
        "#888786", "#A6C2E4", 0, 0);
    add(QString::fromUtf8("Data Type"), dsDataType, false, false, false, false,
        "#0057AE", "#00316E", 0, 0);
    add(QString::fromUtf8("Decimal/Value"), dsDecVal, false, false, false,
        false, "#B07E00", "#FFDD00", 0, 0);
    add(QString::fromUtf8("Error"), dsError, false, false, true, false,
        "#BF0303", "#9C0D0D", 0, 0);
    add(QString::fromUtf8("Floating Point"), dsFloat, false, false, false,
        false, "#B07E00", "#FFDD00", 0, 0);
    add(QString::fromUtf8("Function"), dsFunction, false, false, false, false,
        "#442886", "#442886", 0, 0);
    add(QString::fromUtf8("Keyword"), dsKeyword, true, false, false, false, 0,
        0, 0, 0);
    add(QString::fromUtf8("Others"), dsOthers, false, false, false, false,
        "#006E26", "#80FF80", 0, 0);
    add(QString::fromUtf8("Region Marker"), dsRegionMarker, false, false, false,
        false, "#0057AE", "#00316E", "#E1EAF8", 0);
    add(QString::fromUtf8("String"), dsString, false, false, false, false,
        "#BF0303", "#9C0D0D", 0, 0);
    add(QString::fromUtf8("Operator"), dsOperator, false, false, false, false,
        "#1F1C1B", 0, 0, 0);
    add(QString::fromUtf8("Control Flow"), dsControlFlow, true, false, false,
        false, "#1F1C1B", 0, 0, 0);
    add(QString::fromUtf8("Built-in"), dsBuiltIn, true, false, false, false,
        "#644A9B", "#452886", 0, 0);
    add(QString::fromUtf8("Variable"), dsVariable, false, false, false, false,
        "#0057AE", "#00316e", 0, 0);
    add(QString::fromUtf8("Extension"), dsExtension, false, false, false, false,
        "#0095FF", 0, 0, 0);
    add(QString::fromUtf8("Preprocessor"), dsPreprocessor, false, false, false,
        false, "#006E28", "#006e28", 0, 0);
    add(QString::fromUtf8("Import"), dsImport, false, false, false, false,
        "#FF5500", "#FF5500", 0, 0);
    add(QString::fromUtf8("Verbatim String"), dsVerbatimString, false, false,
        false, false, "#BF0303", "#9C0E0E", 0, 0);
    add(QString::fromUtf8("Special String"), dsSpecialString, false, false,
        false, false, "#FF5500", "#FF5500", 0, 0);
    add(QString::fromUtf8("Special Character"), dsSpecialChar, false, false,
        false, false, "#3DAEE9", "#FCFCFC", 0, 0);
    add(QString::fromUtf8("Attribute"), dsAttribute, false, false, false, false,
        "#0057AE", "#00316E", 0, 0);
  }
}

void GenericSyntaxHighlighterAttributes::add(
    const QString &name, int id, bool bold, bool italic, bool underline,
    bool strikeout, const char *foreground, const char * /*fg_selected*/,
    const char *background, const char * /*bg_selected*/) {
  QTextCharFormat fmt;
  if (bold) {
...
void GenericSyntaxHighlighterAttributes::assign(
    const GenericSyntaxHighlighterAttributes &other) {
  m_attributes = other.m_attributes;
  m_ids = other.m_ids;
}

bool GenericSyntaxHighlighterAttributes::has_attribute(
...
void GenericSyntaxHighlighterAttributes::set_style(
    int id, const QTextCharFormat &format) {
  if (id < 0 || id >= int(m_attributes.size())) {
    return;
  }
...
void GenericSyntaxHighlighterAttributes::set_styles(
    int id, int basic_style_id, const QTextCharFormat &format) {
  if (id < 0 || id >= int(m_attributes.size())) {
    return;
  }
...
QTextCharFormat GenericSyntaxHighlighterAttributes::format_for(int id) const {
  if (id < 0 || id >= int(m_attributes.size())) {
    return QTextCharFormat();
  } else {
...
std::string GenericSyntaxHighlighterAttributes::to_string() const {
  std::string s;

  for (std::map<QString, int>::const_iterator id = m_ids.begin();
       id != m_ids.end(); ++id) {
    ...
    if (style.hasProperty(QTextFormat::FontUnderline)) {
      ...
      a += "underline:" +
           tl::to_string(style.boolProperty(QTextFormat::FontUnderline));
    }
    if (style.hasProperty(QTextFormat::FontStrikeOut)) {
      ...
      a += "strikeout:" +
           tl::to_string(style.boolProperty(QTextFormat::FontStrikeOut));
    }
    if (style.hasProperty(QTextFormat::FontItalic)) {
      ...
      a += "italic:" +
           tl::to_string(style.boolProperty(QTextFormat::FontItalic));
    }
    if (style.hasProperty(QTextFormat::FontWeight)) {
      if (!a.empty()) {
        a += ",";
      }
      a += "bold:" + tl::to_string(style.intProperty(QTextFormat::FontWeight) ==
                                   QFont::Bold);
    }
    if (style.hasProperty(QTextFormat::ForegroundBrush)) {
      ...
      a += "color:" + tl::to_quoted_string(tl::to_string(
                          style.brushProperty(QTextFormat::ForegroundBrush)
                              .color()
                              .name()));
    }
    if (style.hasProperty(QTextFormat::BackgroundBrush)) {
      ...
      a += "background:" + tl::to_quoted_string(tl::to_string(
                               style.brushProperty(QTextFormat::BackgroundBrush)
                                   .color()
                                   .name()));
    }
  ...
  while (!ex.at_end() && !ex.test(";")) {
    ...
    while (!ex.at_end() && !ex.test(")")) {
      ...
      if (ex.test("underline")) {
        ...
        bool f = false;
        ex.read(f);
        style.setProperty(QTextFormat::FontWeight,
                          f ? QFont::Bold : QFont::Normal);
        ...
        std::string cs;
        ex.read_quoted(cs);
        style.setProperty(QTextFormat::ForegroundBrush,
                          QBrush(QColor(tl::to_qstring(cs))));
        ...
        std::string cs;
        ex.read_quoted(cs);
        style.setProperty(QTextFormat::BackgroundBrush,
                          QBrush(QColor(tl::to_qstring(cs))));
...
GenericSyntaxHighlighterState::GenericSyntaxHighlighterState(
    const GenericSyntaxHighlighterContexts *contexts)
    : mp_contexts(contexts) {
  //  Start with the first context and an empty input argument list
  m_stack.push_back(
      std::make_pair(mp_contexts->initial_context_id(), QList<QString>()));
}

bool GenericSyntaxHighlighterState::operator<(
    const GenericSyntaxHighlighterState &d) const {
...
bool GenericSyntaxHighlighterState::match(const QString &string,
                                          unsigned int generation_id, int index,
                                          int &end_index, int &def_attribute_id,
                                          int &attribute_id) {
  const GenericSyntaxHighlighterContext &ctx =
      mp_contexts->context(m_stack.back().first);
  def_attribute_id = ctx.attribute_id();

  int nc = 0;
  QList<QString> oa;
  if (ctx.match(string, generation_id, index, end_index, m_stack.back().second,
                oa, nc, attribute_id)) {
    ...
    if (nc > 0) {
      ...
      if (m_stack.empty()) {
        m_stack.push_back(std::make_pair(mp_contexts->initial_context_id(),
                                         QList<QString>()));
      }
    }
...
static GenericSyntaxHighlighterRuleStringList parse_list(QDomElement e) {
  QList<QString> items;
...
static int context_name_to_id(const QString &nr,
                              GenericSyntaxHighlighterContexts &contexts) {
...
static bool string_to_bool(const QString &n) {
...
parse_rule(QDomElement e, GenericSyntaxHighlighterContexts &contexts,
  ...
  if (e.hasAttribute(QString::fromUtf8("dynamic"))) {
    dynamic =
        string_to_bool(e.attributeNode(QString::fromUtf8("dynamic")).value());
  }

  if (e.tagName() == QString::fromUtf8("LineContinue")) {

    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleLineContinue());

  } else if (e.tagName() == QString::fromUtf8("RegExpr")) {

    QString s = e.attributeNode(QString::fromUtf8("String")).value();
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleRegExp(s, dynamic));
    ...
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleString(s1 + s2, false, dynamic));

  } else if (e.tagName() == QString::fromUtf8("DetectChar")) {

    QString s = e.attributeNode(QString::fromUtf8("char")).value();
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleString(s, false, dynamic));

  } else if (e.tagName() == QString::fromUtf8("DetectSpaces")) {

    rule =
        GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleSpaces());

  } else if (e.tagName() == QString::fromUtf8("DetectIdentifier")) {

    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleIdentifier());

  } else if (e.tagName() == QString::fromUtf8("AnyChar")) {

    QString s = e.attributeNode(QString::fromUtf8("String")).value();
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleAnyChar(s));
    ...
    QString s2 = e.attributeNode(QString::fromUtf8("char1")).value();
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleRange(s1, s2, dynamic));

  } else if (e.tagName() == QString::fromUtf8("StringDetect")) {

    QString s = e.attributeNode(QString::fromUtf8("String")).value();
    bool insensitive = string_to_bool(
        e.attributeNode(QString::fromUtf8("insensitive")).value());
    rule = GenericSyntaxHighlighterRule(
        new GenericSyntaxHighlighterRuleString(s, insensitive, dynamic));
    ...
    rule =
        GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleFloat());

  } else if (e.tagName() == QString::fromUtf8("HlCOct")) {

    //  TODO: can be done more efficiently
    rule = GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleRegExp(
        QString::fromUtf8("0[0-9]+"), false));

  } else if (e.tagName() == QString::fromUtf8("HlCHex")) {

    //  TODO: can be done more efficiently
    rule = GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleRegExp(
        QString::fromUtf8("0x[0-9a-fA-F]+"), false));

  } else if (e.tagName() == QString::fromUtf8("HlCStringChar")) {

    //  TODO: can be done more efficiently
    rule = GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleRegExp(
        QString::fromUtf8("\\[abefnrtv\"\'\\]|\\x[0-9a-fA-F]+|\\0[0-9]{1,3}"),
        false));
    ...
    rule = GenericSyntaxHighlighterRule(new GenericSyntaxHighlighterRuleRegExp(
        QString::fromUtf8(
            "'(?:.|\\[abefnrtv\"\'\\]|\\x[0-9a-fA-F]+|\\0[0-9]{1,3})'"),
        false));
  ...
  if (e.hasAttribute(QString::fromUtf8("context"))) {
    rule.set_target_context_id(context_name_to_id(
        e.attributeNode(QString::fromUtf8("context")).value(), contexts));
  }
  ...
  if (e.hasAttribute(QString::fromUtf8("lookAhead"))) {
    rule.set_lookahead(string_to_bool(
        e.attributeNode(QString::fromUtf8("lookAhead")).value()));
  }

  if (e.hasAttribute(QString::fromUtf8("firstNonSpace"))) {
    rule.set_first_non_space(string_to_bool(
        e.attributeNode(QString::fromUtf8("firstNonSpace")).value()));
  }

  if (e.hasAttribute(QString::fromUtf8("column"))) {
    rule.set_column(
        e.attributeNode(QString::fromUtf8("column")).value().toInt());
  }
...
parse_context(QDomElement e,
              const std::map<QString, QDomElement> &contexts_by_name,
              GenericSyntaxHighlighterContexts &contexts,
              std::map<QString, GenericSyntaxHighlighterRuleStringList> &lists,
              GenericSyntaxHighlighterAttributes &attributes) {
  GenericSyntaxHighlighterContext context;
  ...
  for (QDomNode n = e.firstChild(); !n.isNull(); n = n.nextSibling()) {
    if (n.isElement()) {
      ...
      if (ee.tagName() == QString::fromUtf8("IncludeRules")) {
        QString included_name =
            ee.attributeNode(QString::fromUtf8("context")).value();
        std::map<QString, QDomElement>::const_iterator c2n =
            contexts_by_name.find(included_name);
        if (c2n != contexts_by_name.end()) {
          context.include(parse_context(c2n->second, contexts_by_name, contexts,
                                        lists, attributes));
        }
      } else {
  ...
  if (e.hasAttribute(QString::fromUtf8("lineEndContext"))) {
    context.set_lineend_context(context_name_to_id(
        e.attributeNode(QString::fromUtf8("lineEndContext")).value(),
        contexts));
  }
  if (e.hasAttribute(QString::fromUtf8("lineBeginContext"))) {
    context.set_linebegin_context(context_name_to_id(
        e.attributeNode(QString::fromUtf8("lineBeginContext")).value(),
        contexts));
  }

  if (e.hasAttribute(QString::fromUtf8("fallthrough")) &&
      string_to_bool(
          e.attributeNode(QString::fromUtf8("fallthrough")).value())) {
    context.set_fallthrough_context(context_name_to_id(
        e.attributeNode(QString::fromUtf8("fallthroughContext")).value(),
        contexts));
  }
...
static void parse_item_data(QDomElement e,
                            GenericSyntaxHighlighterAttributes &attributes,
                            bool initialize) {
  ...
  if (e.hasAttribute(QString::fromUtf8("color"))) {
    format.setForeground(
        QColor(e.attributeNode(QString::fromUtf8("color")).value()));
  }
  ...
  if (e.hasAttribute(QString::fromUtf8("bold"))) {
    format.setFontWeight(
        string_to_bool(e.attributeNode(QString::fromUtf8("bold")).value())
            ? QFont::Bold
            : QFont::Normal);
  }
  ...
  if (e.hasAttribute(QString::fromUtf8("italic"))) {
    format.setFontItalic(
        string_to_bool(e.attributeNode(QString::fromUtf8("italic")).value()));
  }

  if (e.hasAttribute(QString::fromUtf8("underline"))) {
    format.setFontUnderline(string_to_bool(
        e.attributeNode(QString::fromUtf8("underline")).value()));
  }

  if (e.hasAttribute(QString::fromUtf8("strikeout"))) {
    format.setFontStrikeOut(string_to_bool(
        e.attributeNode(QString::fromUtf8("strikeout")).value()));
  }
...
GenericSyntaxHighlighter::GenericSyntaxHighlighter(
    QObject *parent, QIODevice &input,
    GenericSyntaxHighlighterAttributes *attributes, bool initialize_attributes)
    : QSyntaxHighlighter(parent), mp_attributes(attributes),
      m_generation_id(0) {
  QDomDocument d;
  d.setContent(&input, true);

  QDomElement highlighting;
  for (QDomNode n = d.documentElement().firstChild(); !n.isNull();
       n = n.nextSibling()) {
    if (n.isElement()) {
      QDomElement e = n.toElement();
  ...
  for (QDomNode n = highlighting.firstChild(); !n.isNull();
       n = n.nextSibling()) {
    if (n.isElement()) {
      ...
      if (e.tagName() == QString::fromUtf8("list")) {
        ...
        m_lists.insert(std::make_pair(
            e.attributeNode(QString::fromUtf8("name")).value(), parse_list(e)));
        ...
        for (QDomNode nn = e.firstChild(); !nn.isNull();
             nn = nn.nextSibling()) {
          if (nn.isElement()) {
            QDomElement ee = nn.toElement();
            if (ee.tagName() == QString::fromUtf8("context")) {
              QString context_name =
                  ee.attributeNode(QString::fromUtf8("name")).value();
        ...
        for (QDomNode nn = e.firstChild(); !nn.isNull();
          ...
          if (nn.isElement()) {
            ...
            if (ee.tagName() == QString::fromUtf8("context")) {
              QString context_name =
                  ee.attributeNode(QString::fromUtf8("name")).value();
              m_contexts.insert(context_name,
                                parse_context(ee, contexts_by_name, m_contexts,
                                              m_lists, *mp_attributes));
            }
          }
        }
...
void GenericSyntaxHighlighter::highlightBlock(const QString &text) {
...
#if defined(DEBUG_HIGHLIGHTER)
  std::cout << "Highlighting '" << text.toAscii().constData() << "'"
            << std::endl;
#endif
...
#if defined(DEBUG_HIGHLIGHTER)
  std::cout << index << ":"
            << m_contexts.context(state.current_context_id())
                   .name()
                   .toAscii()
                   .constData()
            << std::endl;
  ...
  std::unique_ptr<SyntaxHighlighterUserData> user_data(
      new SyntaxHighlighterUserData());

  while (std::max(0, index) < text.length()) {

    if (state.match(text, m_generation_id, index, end_index, def_attribute_id,
                    attribute_id)) {
      ...
      if (last_index >= 0 && def_attribute_id >= 0) {
        setFormat(last_index, index - last_index,
                  mp_attributes->format_for(def_attribute_id));
      }
      ...
      if (end_index > index && attribute_id >= 0) {
        setFormat(index, end_index - index,
                  mp_attributes->format_for(attribute_id));
      }
  ...
  if (index < text.length() && def_attribute_id >= 0) {
    setFormat(index, text.length() - index,
              mp_attributes->format_for(def_attribute_id));
  }

  //  match potential line-end context
  state.match(text, m_generation_id, index, end_index, def_attribute_id,
              attribute_id);

  //  set the new state
  std::map<GenericSyntaxHighlighterState, int>::iterator s =
      m_state_cache.find(state);
