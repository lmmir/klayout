...
QVariant BrowserTextWidget::loadResource(int type, const QUrl &url) {
...
std::string BookmarkItem::to_string() const {
  std::string r;
...
BrowserPanel::BrowserPanel(QWidget *parent)
    : QWidget(parent), m_back_dm(this, &BrowserPanel::back),
      m_new_url_dm(this, &BrowserPanel::new_url), mp_dispatcher(0) {
  init();
}
...
void BrowserPanel::init() {
  ...
  connect(mp_ui->search_edit, SIGNAL(textEdited(const QString &)), this,
          SLOT(search_text_changed(const QString &)));
  connect(mp_ui->search_edit, SIGNAL(returnPressed()), this,
          SLOT(search_edited()));
  connect(mp_ui->search_button, SIGNAL(clicked()), this, SLOT(search_edited()));
  connect(mp_ui->browser, SIGNAL(sourceChanged(const QUrl &)), this,
          SLOT(source_changed()));
  connect(mp_ui->browser, SIGNAL(anchorClicked(const QUrl &)), this,
          SLOT(anchor_clicked(const QUrl &)));
  connect(mp_ui->browser, SIGNAL(backwardAvailable(bool)), mp_ui->back_pb,
          SLOT(setEnabled(bool)));
  connect(mp_ui->browser, SIGNAL(forwardAvailable(bool)), mp_ui->forward_pb,
          SLOT(setEnabled(bool)));
  connect(mp_ui->outline_tree, SIGNAL(itemActivated(QTreeWidgetItem *, int)),
          this, SLOT(outline_item_clicked(QTreeWidgetItem *)));
  connect(mp_ui->on_page_search_edit, SIGNAL(textChanged(const QString &)),
          this, SLOT(page_search_edited()));
  connect(mp_ui->search_close_button, SIGNAL(clicked()), this,
          SLOT(page_search_edited()), Qt::QueuedConnection);
  connect(mp_ui->on_page_search_edit, SIGNAL(returnPressed()), this,
          SLOT(page_search_next()));
  connect(mp_ui->on_page_search_next, SIGNAL(clicked()), this,
          SLOT(page_search_next()));
  ...
  connect(mp_ui->action_delete_bookmark, SIGNAL(triggered()), this,
          SLOT(delete_bookmark()));
  connect(mp_ui->browser_bookmark_view,
          SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this,
          SLOT(bookmark_item_selected(QTreeWidgetItem *)));
...
void BrowserPanel::set_dispatcher(lay::Dispatcher *dispatcher,
                                  const std::string &cfg_bookmarks) {
...
std::string BrowserPanel::title() const {
  return tl::to_string(m_current_title);
}

std::string BrowserPanel::url() const {
...
void BrowserPanel::bookmark() {
  ...
  QString title =
      mp_ui->browser->document()->metaInformation(QTextDocument::DocumentTitle);
...
void BrowserPanel::store_bookmarks() {
  if (mp_dispatcher) {
    ...
    std::string s;
    for (std::list<BookmarkItem>::const_iterator i = m_bookmarks.begin();
         i != m_bookmarks.end(); ++i) {
      s += i->to_string();
    }
...
void BrowserPanel::clear_bookmarks() { m_bookmarks.clear(); }

void BrowserPanel::add_bookmark(const BookmarkItem &item) {
  for (std::list<BookmarkItem>::iterator i = m_bookmarks.begin();
       i != m_bookmarks.end();) {
...
void BrowserPanel::page_search_edited() {
  ...
  if (!mp_ui->search_frame->isVisible() ||
      mp_ui->on_page_search_edit->text().size() < 2) {
  ...
  for (QTextBlock b = doc->firstBlock(); b.isValid(); b = b.next()) {
    ...
    while ((index = t.indexOf(search_text, from, Qt::CaseInsensitive)) >= 0) {
      ...
      highlight.movePosition(QTextCursor::NextCharacter,
                             QTextCursor::MoveAnchor, index);
      highlight.movePosition(QTextCursor::NextCharacter,
                             QTextCursor::KeepAnchor, search_text.size());
...
void BrowserPanel::search_text_changed(const QString &text) {
  ...
  if (!text.isEmpty() && mp_source.get()) {
    ...
    for (std::list<std::string>::const_iterator i = cl.begin(); i != cl.end();
         ++i) {
...
void BrowserPanel::source_changed() { m_new_url_dm(); }
...
void BrowserPanel::reload() {
  ...
  //  disable reload while we are in a loadResource call - clearing the cache
  //  will be sufficient
  if (m_enable_load) {
...
void BrowserPanel::back() { mp_ui->browser->backward(); }

void BrowserPanel::forward() { mp_ui->browser->forward(); }
...
QSize BrowserPanel::sizeHint() const { return QSize(800, 600); }
...
void BrowserPanel::search(const std::string &s) {
  if (!s.empty()) {
    ...
    qi.push_back(QPair<QString, QString>(tl::to_qstring(m_search_query_item),
                                         tl::to_qstring(s)));
...
#if QT_VERSION >= 0x050000
    QUrlQuery qi;
    qi.addQueryItem(tl::to_qstring(m_search_query_item),
                    mp_ui->search_edit->text());
    url.setQuery(qi);
#else
    QList<QPair<QString, QString>> qi;
    qi.push_back(QPair<QString, QString>(tl::to_qstring(m_search_query_item),
...
void BrowserPanel::set_search_url(const std::string &url,
...
static void update_item_with_outline(const BrowserOutline &ol,
  ...
  int i = 0;
  for (BrowserOutline::const_child_iterator c = ol.begin(); c != ol.end();
       ++c, ++i) {
...
void BrowserPanel::update_navigation_panel() {
  bool navigation_visible =
      mp_ui->outline_tree->topLevelItemCount() > 0 ||
      mp_ui->browser_bookmark_view->topLevelItemCount() > 0;
...
QVariant BrowserPanel::loadResource(int type, const QUrl &url) {
  if (type == QTextDocument::ImageResource) {
    ...
    try {
      return QVariant(
          tl::to_qstring(mp_source->get_css(tl::to_string(url.toString()))));
    } catch (tl::Exception &ex) {
      tl::error << ex.msg();
    } catch (...) {
    }
    ...
    //  recursion sentinel: avoid recursion by any action within mp_source->get
    //  that causes a "loadResource"
    if (!m_enable_load || !mp_source.get()) {
      //  return any dummy in this case - otherwise the QTestBrowser complains
    ...
    //  Qt sets the override cursor in response to link clicks - this is not
    //  appropriate for some GSI callback implementations that show InputDialogs
    //  for example. Therefore we install out own (normal) override cursor.
    ...
    try {
      ...
      if (s.empty()) {
        ...
        //  the Url is to schedule a delayed "back" signal. In Qt >= 4.2 we
        //  could register an external handler for "int" schemes that would do
        //  nothing ..
        if (m_enable_reject) {
          m_back_dm();
        }
      } else {
        //  to avoid regeneration of text on artificial "back" events, the last
        //  page is cached
        m_cached_text = s;
        m_cached_url = u;
...
BrowserSource::BrowserSource(const std::string &html) : m_default_html(html) {
  //  .. nothing yet ..
}
...
BrowserSource::~BrowserSource() {
  ...
  for (std::set<BrowserPanel *>::const_iterator o = owners.begin();
       o != owners.end(); ++o) {
    (*o)->set_source(0);
  }
}

std::string BrowserSource::get_css(const std::string & /*url*/) {
  return std::string();
}

QImage BrowserSource::get_image(const std::string & /*url*/) {
  return QImage();
}

BrowserOutline BrowserSource::get_outline(const std::string & /*url*/) {
  return BrowserOutline();
}

void BrowserSource::search_completers(const std::string & /*search_string*/,
                                      std::list<std::string> & /*completers*/) {
  //  .. nothing here ..
}

std::string BrowserSource::get(const std::string & /*url*/) {
  return m_default_html;
}

void BrowserSource::detach(lay::BrowserPanel *d) { mp_owners.erase(d); }

void BrowserSource::attach(lay::BrowserPanel *d) { mp_owners.insert(d); }
