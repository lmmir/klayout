...
class BrowseInstancesPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_options(std::vector<std::pair<std::string, std::string>> &options) const {
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_context_cell, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_context_mode, "any-top"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_window_mode, "fit-marker"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_window_state, ""));
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_window_dim, "1.0"));
    options.push_back(
        std::pair<std::string, std::string>(cfg_cib_max_inst_count, "1000"));
  }
  ...
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(lay::menu_item(
        "browse_instances::show", "browse_instances", "tools_menu.end",
        tl::to_string(QObject::tr("Browse Instances"))));
  }
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new BrowseInstancesPluginDeclaration(), 11000,
                "BrowseInstancesPlugin");
...
static struct {
  ...
  const char *string;
} context_modes[] = {{BrowseInstancesForm::AnyTop, "any-top"},
...
public:
  ...
  std::string to_string(BrowseInstancesForm::mode_type mode) {
    for (unsigned int i = 0;
         i < sizeof(context_modes) / sizeof(context_modes[0]); ++i) {
...
static struct {
  ...
  const char *string;
} window_modes[] = {{BrowseInstancesForm::DontChange, "dont-change"},
...
public:
  ...
  std::string to_string(BrowseInstancesForm::window_type mode) {
    for (unsigned int i = 0; i < sizeof(window_modes) / sizeof(window_modes[0]);
         ++i) {
...
void BrowseInstancesConfigPage::setup(lay::Dispatcher *root) {
  ...
  root->config_get(cfg_cib_context_mode, cmode,
                   BrowseInstancesContextModeConverter());
  cbx_context->setCurrentIndex(int(cmode));

  //  window mode
  BrowseInstancesForm::window_type wmode = BrowseInstancesForm::FitMarker;
  root->config_get(cfg_cib_window_mode, wmode,
                   BrowseInstancesWindowModeConverter());
...
void BrowseInstancesConfigPage::window_changed(int m) {
  mrg_window->setEnabled(m == int(BrowseInstancesForm::FitMarker) ||
                         m == int(BrowseInstancesForm::CenterSize));
}
...
void BrowseInstancesConfigPage::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_cib_context_mode,
                   BrowseInstancesForm::mode_type(cbx_context->currentIndex()),
                   BrowseInstancesContextModeConverter());
  root->config_set(cfg_cib_window_mode,
                   BrowseInstancesForm::window_type(cbx_window->currentIndex()),
                   BrowseInstancesWindowModeConverter());
...
public:
  BrowseInstancesFormCellLVI(const std::string &cn,
                             lay::CellView::cell_index_type index)
      : QTreeWidgetItem(), m_index(index) {
...
public:
  BrowseInstancesFormCellInstanceLVI(const std::string &text,
...
BrowseInstancesForm::BrowseInstancesForm(lay::Dispatcher *root,
                                         LayoutViewBase *vw)
    : lay::Browser(root, vw), Ui::BrowseInstancesForm(), m_cv_index(0),
      m_cell_index(0), m_cell_changed_enabled(true), m_view_changed(false),
      m_cell_inst_changed_enabled(true), m_ef_enabled(true), m_mode(AnyTop),
      m_window(FitMarker), m_window_dim(), m_max_inst_count(0),
      m_current_count(0) {
  ...
  connect(lv_cell_instance, SIGNAL(itemSelectionChanged()), this,
          SLOT(cell_inst_changed()));
  ...
  connect(chooseCellButton, SIGNAL(clicked()), this,
          SLOT(choose_cell_pressed()));
}
...
BrowseInstancesForm::~BrowseInstancesForm() { remove_marker(); }

void BrowseInstancesForm::choose_cell_pressed() {
  BEGIN_PROTECTED
  CellSelectionForm form(this, view(), "browse_cell", true /*simple mode*/);
  if (form.exec()) {
    change_cell(form.selected_cellview().cell_index(),
                form.selected_cellview_index());
  }
  END_PROTECTED
}

bool BrowseInstancesForm::configure(const std::string &name,
...
void BrowseInstancesForm::remove_marker() {
  for (std::vector<lay::Marker *>::const_iterator m = mp_markers.begin();
       m != mp_markers.end(); ++m) {
    delete *m;
  }
  mp_markers.clear();
...
void BrowseInstancesForm::cell_changed(QTreeWidgetItem *item,
                                       QTreeWidgetItem *) {
  ...
  BrowseInstancesFormCellLVI *it =
      dynamic_cast<BrowseInstancesFormCellLVI *>(item);
  if (!it) {

    //  "All item" - fetch the parents of all other items
    for (QList<QTreeWidgetItem *>::const_iterator i = m_items.begin();
         i != m_items.end(); ++i) {
      BrowseInstancesFormCellLVI *it =
          dynamic_cast<BrowseInstancesFormCellLVI *>(*i);
      if (it) {
  ...
  for (std::vector<const db::Cell *>::const_iterator parent = parents.begin();
       parent != parents.end() && !shortened; ++parent) {
    if (m_mode == AnyTop) {
      shortened = fill_cell_instances(db::ICplxTrans(), layout, *parent, &cell,
                                      0, false, std::string(), items);
    } else if (m_mode == ToCellView) {
      if (m_context_cv.is_valid()) {
        shortened = fill_cell_instances(db::ICplxTrans(), layout, *parent,
                                        &cell, m_context_cv.cell(), false,
                                        std::string(), items);
      }
    } else if (m_mode == Parent) {
      shortened = fill_cell_instances(db::ICplxTrans(), layout, *parent, &cell,
                                      0, true, std::string(), items);
    }
  }
...
struct BrowseInstancesCellInfo {
  BrowseInstancesCellInfo(const std::string &n,
...
void BrowseInstancesForm::change_cell(db::cell_index_type cell_index,
                                      int cv_index) {
  //  obtain active cellview index and cell index
  m_cv_index = cv_index;
  ...
  setWindowTitle(
      tl::to_qstring(tl::to_string(QObject::tr("Browse Instances Of Cell")) +
                     " '" + layout.cell_name(m_cell_index) + "'"));
  ...
  for (std::vector<BrowseInstancesCellInfo>::const_iterator cn =
           cell_info.begin();
       cn != cell_info.end(); ++cn) {
...
bool BrowseInstancesForm::fill_cell_instances(
    const db::ICplxTrans &t, const db::Layout &layout, const db::Cell *parent,
    const db::Cell *from, const db::Cell *to, bool to_parent,
    const std::string &path, QList<QTreeWidgetItem *> &items) {
  if (from == to || (!to_parent && to == 0 && from->is_top())) {
    ...
    items.append(new BrowseInstancesFormCellInstanceLVI(text, path, t,
                                                        from->cell_index()));
    ...
    for (db::Cell::parent_inst_iterator p = from->begin_parent_insts();
      ...
      if (fill_cell_instances(tt.inverted() * t, layout, 0, cell,
...
void BrowseInstancesForm::highlight_current() {
  ...
  for (QList<QTreeWidgetItem *>::const_iterator s = selected_items.begin();
    ...
    BrowseInstancesFormCellInstanceLVI *inst_item =
        dynamic_cast<BrowseInstancesFormCellInstanceLVI *>(*s);
    if (inst_item) {
      ...
      if (index == inst_item->index()) {
        ...
        for (std::vector<db::DCplxTrans>::const_iterator gt =
                 m_global_trans.begin();
             gt != m_global_trans.end(); ++gt) {
          dbox +=
              (*gt * db::CplxTrans(layout.dbu()) * inst_item->trans()) * box;
        }
      }
    }
  }
