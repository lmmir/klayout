...
/**
 *  @brief Gets a suitable parent widget for the modal dialogs used in this
 * module
 */
static QWidget *parent_widget() { return QApplication::activeWindow(); }

static void collect_cells_to_delete(const db::Layout &layout,
                                    const db::Cell &cell,
...
static bool validate_cell_path(const db::Layout &layout,
...
LayoutViewFunctions::LayoutViewFunctions(db::Manager *manager,
                                         LayoutViewBase *view)
...
void LayoutViewFunctions::menu_activated(const std::string &symbol) {
  ...
  if (symbol == "cm_show_properties") {
    ...
    view()->del();
    //  because a "delete" might involve objects currently edited, we cancel the
    //  edit after we have deleted the object
    view()->cancel();
    ...
    if (view()->active_cellview_index() >= 0) {
      ...
      if (form.exec() == QDialog::Accepted &&
        ...
        view()->select_cell(form.selected_cellview().combined_unspecific_path(),
                            form.selected_cellview_index());
        view()->set_current_cell_path(
            form.selected_cellview_index(),
            form.selected_cellview().combined_unspecific_path());
        view()->zoom_fit();
      }
    }
    ...
    lay::LayoutPropertiesForm lp_form(parent_widget(), view(),
                                      "layout_props_form");
    lp_form.exec();
  } else if (symbol == "cm_layout_stats") {
    lay::LayoutStatisticsForm lp_form(parent_widget(), view(),
                                      "layout_props_form");
    lp_form.exec();
    ...
    view()->set_hier_levels(
        std::make_pair(view()->get_min_hier_levels(), new_to));
  } else if (symbol == "cm_dec_max_hier") {
    int new_to = view()->get_max_hier_levels() > 0
                     ? view()->get_max_hier_levels() - 1
                     : 0;
    view()->set_hier_levels(std::make_pair(
        std::min(view()->get_min_hier_levels(), new_to), new_to));
  } else if (symbol == "cm_max_hier") {
    view()->max_hier();
  } else if (symbol == "cm_max_hier_0") {
    view()->set_hier_levels(
        std::make_pair(std::min(view()->get_min_hier_levels(), 0), 0));
  } else if (symbol == "cm_max_hier_1") {
    view()->set_hier_levels(
        std::make_pair(std::min(view()->get_min_hier_levels(), 0), 1));
...
void LayoutViewFunctions::cm_cell_user_properties() {
  ...
  if (cv_index >= 0 && path.size() > 0) {
    ...
    if (props_form.show(view(), cv_index, prop_id,
                        layout.begin_meta(cell.cell_index()),
                        layout.end_meta(cell.cell_index()))) {
  ...
  if (cv_index >= 0 && paths.size() > 0) {
    ...
    if (paths.size() > 1) {
      throw tl::Exception(tl::to_string(
          tr("Replace cell cannot be used when multiple cells are selected")));
    }

    db::Layout &layout = view()->cellview(cv_index)->layout();

    bool needs_to_ask = false;
    for (std::vector<lay::LayoutViewBase::cell_path_type>::const_iterator p =
             paths.begin();
         p != paths.end() && !needs_to_ask; ++p) {
      if (layout.is_valid_cell_index(p->back()) &&
          !layout.cell(p->back()).is_leaf()) {
        needs_to_ask = true;
      }
    }
    ...
    if (mode_dialog.exec_dialog(view()->cellview(cv_index), mode, with_cell)) {
      ...
      if (with_cell != paths.front().back()) {
        ...
        lay::LayoutViewBase::cell_path_type cell_path(
            view()->cellview(cv_index).combined_unspecific_path());
        ...
        for (std::vector<lay::LayoutViewBase::cell_path_type>::const_iterator
                 p = paths.begin();
             p != paths.end(); ++p) {
        ...
        layout.cell(with_cell).collect_called_cells(
            cells_below_replacement_cell);
        for (std::set<db::cell_index_type>::const_iterator c =
                 cells_below_replacement_cell.begin();
             c != cells_below_replacement_cell.end(); ++c) {
  ...
  if (cv_index >= 0) {
    ...
    for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
      for (db::Cell::const_iterator i = c->begin(); !i.at_end(); ++i) {
        std::map<db::cell_index_type, db::cell_index_type>::const_iterator cm =
            cell_map.find(i->cell_index());
        if (cm != cell_map.end()) {
...
void LayoutViewFunctions::cm_cell_convert_to_static() {
  ...
  if (cv_index >= 0 && paths.size() > 0) {
    ...
    for (std::vector<lay::LayoutViewBase::cell_path_type>::iterator p =
             paths.begin();
         p != paths.end(); ++p) {
      if (!p->empty() && layout.is_valid_cell_index(p->back())) {
    ...
    if (!needs_to_ask || mode_dialog.exec_dialog(mode)) {
      ...
      for (std::vector<lay::LayoutViewBase::cell_path_type>::const_iterator p =
        ...
        if (!p->empty() && layout.is_valid_cell_index(p->back())) {
          cells_to_delete.insert(p->back());
          if (mode == 2) {
            collect_cells_to_delete(layout, layout.cell(p->back()),
                                    cells_to_delete);
          }
        }
      }
...
void LayoutViewFunctions::cm_cell_cut() {
  if (view()->hierarchy_panel()) {
    //  TODO: currently the hierarchy panel's cut function does its own
    //  transaction handling. Otherwise the cut function is not working
    //  propertly.
...
void LayoutViewFunctions::cm_cell_flatten() {
  ...
  if (cv_index >= 0) {
    ...
    if (cv.is_valid()) {
      ...
      if (paths.empty()) {
        throw tl::Exception(
            tl::to_string(tr("No cells selected for flattening")));
      }

      for (std::vector<HierarchyControlPanel::cell_path_type>::const_iterator
               p = paths.begin();
           p != paths.end(); ++p) {
        if (p->size() > 0 && cv->layout().cell(p->back()).is_proxy()) {
          throw tl::Exception(tl::to_string(
              tr("Cannot use this function on a PCell or library cell")));
        }
      }
      ...
      bool prune = true;
      if (options_dialog.exec_dialog(flatten_insts_levels, prune) &&
          flatten_insts_levels != 0) {
        ...
        if (manager() && manager()->is_enabled()) {
          ...
          lay::TipDialog td(
              ...
              tl::to_string(
                  tr("Undo buffering for the following operation can be memory "
                     "and time consuming.\nChoose \"Yes\" to use undo "
                     "buffering or \"No\" for no undo buffering. Warning: in "
                     "the latter case, the undo history will be "
                     "lost.\n\nChoose undo buffering?")),
              "flatten-undo-buffering", lay::TipDialog::yesnocancel_buttons);
        ...
        for (std::vector<HierarchyControlPanel::cell_path_type>::const_iterator
             ...
             p != paths.end(); ++p) {
          if (p->size() > 0 &&
              child_cells.find(p->back()) == child_cells.end()) {
            cells_to_flatten.insert(p->back());
          }
        }

        for (std::set<db::cell_index_type>::const_iterator c =
                 cells_to_flatten.begin();
             c != cells_to_flatten.end(); ++c) {
...
void LayoutViewFunctions::cm_open_current_cell() {
  view()->set_current_cell_path(view()->active_cellview_index(),
                                view()
                                    ->cellview(view()->active_cellview_index())
                                    .combined_unspecific_path());
}
...
void LayoutViewFunctions::cm_cell_hide() {
  ...
  for (std::vector<HierarchyControlPanel::cell_path_type>::const_iterator p =
           paths.begin();
       p != paths.end(); ++p) {
    if (!p->empty()) {
...
void LayoutViewFunctions::cm_remove_unused() { view()->remove_unused_layers(); }
...
void LayoutViewFunctions::cm_new_cell() {
  ...
  if (!cv.is_valid()) {
    throw tl::Exception(
        tl::to_string(tr("No layout present to add a cell to")));
  }
  ...
  if (cell_prop_dia.exec_dialog(&cv->layout(), s_new_cell_cell_name,
                                s_new_cell_window_size)) {

    db::cell_index_type new_ci = view()->new_cell(
        view()->active_cellview_index(), s_new_cell_cell_name.c_str());
    ...
    if (view()->get_max_hier_levels() < 1 ||
        view()->get_min_hier_levels() > 0) {
...
void LayoutViewFunctions::cm_reload() {
  ...
  if (view()->cellviews() > 1) {
    ...
    lay::SelectCellViewForm form(0, view(),
                                 tl::to_string(tr("Select Layouts To Reload")));
    form.select_all();
  ...
  if (selected.size() > 0) {
    ...
    if (dirty_layouts != 0) {
      ...
      mbox.setText(tl::to_qstring(
          tl::to_string(tr("The following layouts need saving:\n\n")) +
          dirty_files +
          "\n\nPress 'Reload Without Saving' to reload anyhow and discard "
          "changes."));
      mbox.setWindowTitle(tr("Save Needed"));
      mbox.setIcon(QMessageBox::Warning);
      QAbstractButton *yes_button =
          mbox.addButton(tr("Reload Without Saving"), QMessageBox::YesRole);
...
void LayoutViewFunctions::transform_layout(const db::DCplxTrans &tr_mic) {
  ...
  if (cv_index >= 0) {
    ...
    db::ICplxTrans trans(db::DCplxTrans(1.0 / layout.dbu()) * tr_mic *
                         db::DCplxTrans(layout.dbu()));

    bool has_proxy = false;
    for (db::Layout::const_iterator c = layout.begin();
         !has_proxy && c != layout.end(); ++c) {
      has_proxy = c->is_proxy();
    }

    if (has_proxy &&
        QMessageBox::question(
            parent_widget(), tr("Transforming PCells Or Library Cells"),
            tr("The layout contains PCells or library cells or both.\n"
               "Any changes to such cells may be lost when their layout is "
               "refreshed later.\n"
               "Consider using 'Convert all cells to static' before "
...
void LayoutViewFunctions::cm_lay_free_rot() {
  bool ok = false;
  QString s =
      QInputDialog::getText(QApplication::activeWindow(), tr("Free rotation"),
...
void LayoutViewFunctions::cm_lay_scale() {
  bool ok = false;
  QString s = QInputDialog::getText(QApplication::activeWindow(), tr("Scaling"),
                                    tr("Scaling factor"), QLineEdit::Normal,
                                    QString::fromUtf8("1.0"), &ok);
...
void LayoutViewFunctions::cm_sel_flip_x() {
  ...
  if (!sel_bbox.empty()) {
    tr = db::DCplxTrans(sel_bbox.center() - db::DPoint()) * tr *
         db::DCplxTrans(db::DPoint() - sel_bbox.center());
  }
  do_transform(tr);
}
...
void LayoutViewFunctions::cm_sel_move_to() {
  ...
  double x = sel_bbox.left() +
             (sel_bbox.width() * (1 + m_move_to_origin_mode_x) * 0.5);
  double y = sel_bbox.bottom() +
             (sel_bbox.height() * (1 + m_move_to_origin_mode_y) * 0.5);
  db::DPoint move_target(x, y);

  lay::MoveToOptionsDialog options(parent_widget());
  if (options.exec_dialog(m_move_to_origin_mode_x, m_move_to_origin_mode_y,
                          move_target)) {
...
void LayoutViewFunctions::cm_copy_layer() {
  struct {
    int *cv;
    int *layer;
  } specs[] = {{&m_copy_cva, &m_copy_layera}, {&m_copy_cvr, &m_copy_layerr}};
  ...
  for (unsigned int i = 0; i < sizeof(specs) / sizeof(specs[0]); ++i) {
    ...
    if (cv < 0 ||
        !view()->cellview(cv)->layout().is_valid_layer((unsigned int)layer)) {
      layer = -1;
    }
  }

  lay::DuplicateLayerDialog dialog(parent_widget());
  if (dialog.exec_dialog(view(), m_copy_cva, m_copy_layera, m_copy_cvr,
                         m_copy_layerr, m_duplicate_hier_mode,
                         m_clear_before)) {
    ...
    if (manager() && manager()->is_enabled()) {
      ...
      lay::TipDialog td(
          QApplication::activeWindow(),
          tl::to_string(
              tr("Undo buffering for the following operation can be memory and "
                 "time consuming.\nChoose \"Yes\" to use undo buffering or "
                 "\"No\" for no undo buffering. Warning: in the latter case, "
                 "the undo history will be lost.\n\nChoose undo buffering?")),
          "copy-layer-undo-buffering", lay::TipDialog::yesnocancel_buttons);
    ...
    try {
      ...
      bool same_layout = (&view()->cellview(m_copy_cvr)->layout() ==
                          &view()->cellview(m_copy_cva)->layout());
      if (same_layout && m_copy_layera == m_copy_layerr) {
        throw tl::Exception(
            tl::to_string(tr("Source and target layer must not be identical "
      ...
      if (m_duplicate_hier_mode == 0) {
        ...
        if (m_clear_before) {
          ...
          view()
              ->cellview(m_copy_cvr)
              .cell()
              ->collect_called_cells(called_cells);
          for (std::set<db::cell_index_type>::const_iterator c =
                   called_cells.begin();
               c != called_cells.end(); ++c) {
            view()
                ->cellview(m_copy_cvr)
                ->layout()
                .cell(*c)
                .shapes(m_copy_layerr)
                .clear();
          }
        }
        ...
        if (!same_layout) {
          ...
          db::PropertyMapper pm(&view()->cellview(m_copy_cvr)->layout(),
                                &view()->cellview(m_copy_cva)->layout());
          for (db::RecursiveShapeIterator si(
                   view()->cellview(m_copy_cva)->layout(),
                   *view()->cellview(m_copy_cva).cell(), m_copy_layera);
               !si.at_end(); ++si) {
          ...
          try {
            ...
            //  using update/start_layout and end_changes improves the
            //  performance since changing the shapes collection will invalidate
            //  the layout and cause updates inside the RecursiveShapeIerator
            layout.update();
        ...
        if (m_copy_cvr == m_copy_cva) {
          ...
          db::PropertyMapper pm(&view()->cellview(m_copy_cvr)->layout(),
                                &view()->cellview(m_copy_cva)->layout());
          for (db::Shapes::shape_iterator si =
                   view()
                       ->cellview(m_copy_cva)
                       .cell()
                       ->shapes(m_copy_layera)
    ...
    if (prop_dia.exec_dialog(cv, m_new_layer_props)) {
      ...
      for (unsigned int l = 0; l < cv->layout().layers(); ++l) {
        if (cv->layout().is_valid_layer(l) &&
            cv->layout().get_properties(l).log_equal(m_new_layer_props)) {
          throw tl::Exception(
              tl::to_string(
                  tr("A layer with that signature already exists: ")) +
              m_new_layer_props.to_string());
        }
      }
...
void LayoutViewFunctions::cm_align_cell_origin() {
  ...
  if (cv_index >= 0) {
    ...
    if (dialog.exec_dialog(m_align_cell_options)) {
      ...
      if (m_align_cell_options.visible_only) {
        for (lay::LayerPropertiesConstIterator l = view()->begin_layers();
          ...
          if (!l->has_children() && l->layer_index() >= 0 &&
              l->cellview_index() == cv_index && l->visible(true /*real*/)) {
      ...
      db::Trans t(
          db::Vector(-refx + db::coord_traits<db::Coord>::rounded(
                                 m_align_cell_options.xpos / layout.dbu()),
                     -refy + db::coord_traits<db::Coord>::rounded(
                                 m_align_cell_options.ypos / layout.dbu())));

      for (unsigned int i = 0; i < layout.layers(); ++i) {
        if (layout.is_valid_layer(i)) {
          db::Shapes &shapes = nc_cell.shapes(i);
          for (db::Shapes::shape_iterator s =
      ...
      for (db::Cell::const_iterator inst = nc_cell.begin(); !inst.at_end();
           ++inst) {
      ...
      if (m_align_cell_options.adjust_parents) {
        ...
        for (db::Cell::parent_inst_iterator pi = nc_cell.begin_parent_insts();
             !pi.at_end(); ++pi) {
          insts_to_modify.push_back(std::make_pair(
              &layout.cell(pi->parent_cell_index()), pi->child_inst()));
        }

        db::Trans ti(db::Vector(refx, refy));
        for (std::vector<std::pair<db::Cell *, db::Instance>>::const_iterator
                 im = insts_to_modify.begin();
             im != insts_to_modify.end(); ++im) {
          im->first->transform(
              im->second, db::Trans(db::Vector(im->second.complex_trans().trans(
                              db::Vector(refx, refy)))));
        }
      }
...
void LayoutViewFunctions::cm_edit_layer() {
  ...
  if (sel.is_null()) {
    throw tl::Exception(
        tl::to_string(tr("No layer selected for editing its properties")));
  }

  int index = sel->cellview_index();
  if (sel->has_children() || index < 0 || int(view()->cellviews()) <= index ||
      sel->layer_index() < 0) {
    throw tl::Exception(tl::to_string(
        tr("No valid layer selected for editing its properties")));
  }

  const lay::CellView &cv = view()->cellview(index);
  db::Layout &layout = cv->layout();

  db::LayerProperties layer_props =
      layout.get_properties((unsigned int)sel->layer_index());
  ...
  if (prop_dia.exec_dialog(cv, layer_props)) {
    ...
    for (unsigned int l = 0; l < layout.layers(); ++l) {
      if (cv->layout().is_valid_layer(l) && int(l) != sel->layer_index() &&
          layout.get_properties(l).log_equal(layer_props)) {
        throw tl::Exception(
            tl::to_string(tr("A layer with that signature already exists: ")) +
            layer_props.to_string());
      }
    }
    ...
    std::vector<std::pair<db::cell_index_type, const db::PCellDeclaration *>>
        pcell_variants;
    for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
      const db::PCellDeclaration *pcell_decl =
          layout.pcell_declaration_for_pcell_variant(c->cell_index());
      if (pcell_decl) {
    ...
    for (auto c = pcell_variants.begin(); c != pcell_variants.end(); ++c) {
      ...
      const std::vector<tl::Variant> &old_param =
          layout.get_pcell_parameters(c->first);
      std::vector<tl::Variant> new_param;
      const std::vector<db::PCellParameterDeclaration> &pd =
          c->second->parameter_declarations();
      auto v = old_param.begin();
      auto p = pd.begin();
      while (v != old_param.end() && p != pd.end()) {
        if (p->get_type() == db::PCellParameterDeclaration::t_layer &&
            v->to_user<db::LayerProperties>().log_equal(old_props)) {
          if (new_param.empty()) {
            new_param = old_param;
          }
      ...
      if (!new_param.empty()) {
        db::cell_index_type new_cell =
            layout.get_pcell_variant_cell(c->first, new_param);
...
void LayoutViewFunctions::cm_delete_layer() {
  ...
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator si =
    ...
    if (!(*si)->has_children() && cv_index >= 0 &&
        int(view()->cellviews()) > cv_index && (*si)->layer_index() >= 0 &&
        cv.is_valid()) {
  ...
  if (valid_sel.empty()) {
    throw tl::Exception(
        tl::to_string(tr("No or no valid layer selected for deleting them")));
  }
  ...
  //  Hint: delete_layer must come before the layers are actually deleted in
  //  because for undo this must be the last thing to do (otherwise the layout
  //  is not propertly set up)
  ...
  for (std::set<std::pair<db::Layout *, unsigned int>>::const_iterator li =
           valid_layers.begin();
       li != valid_layers.end(); ++li) {
...
void LayoutViewFunctions::cm_clear_layer() {
  ...
  if (mode_dialog.exec_dialog(m_layer_hier_mode)) {
    ...
    for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator si =
      ...
      if (!(*si)->has_children() && (*si)->layer_index() >= 0 &&
          view()->cellview((*si)->cellview_index()).is_valid()) {
        ...
        if (m_layer_hier_mode == 0) {
          ...
          for (std::set<db::cell_index_type>::const_iterator cc =
                   called_cells.begin();
               cc != called_cells.end(); ++cc) {
...
class LayoutViewPluginDeclaration : public lay::PluginDeclaration {
public:
  virtual void
  get_menu_entries(std::vector<lay::MenuEntry> &menu_entries) const {
    ...
    menu_entries.push_back(
        lay::menu_item("cm_paste_interactive", "paste_interactive:edit", at,
                       tl::to_string(tr("Paste Interactive"))));
    menu_entries.push_back(
        lay::menu_item("cm_duplicate_interactive", "duplicate_interactive:edit",
                       at, tl::to_string(tr("Duplicate Interactive"))));
    menu_entries.push_back(
        lay::menu_item("cm_sel_move_interactive", "sel_move_interactive:edit",
                       at, tl::to_string(tr("Move Interactive"))));
    menu_entries.push_back(
        lay::menu_item("cm_select_next_item", "select_next_item:edit", at,
                       tl::to_string(tr("Select Next Item(Space)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_select_next_item_add", "select_next_item_add:edit", at,
        tl::to_string(tr("Select Next Item too(Shift+Space)"))));

    at = "edit_menu.edit_options_group";
    menu_entries.push_back(lay::menu_item("cm_undo", "undo:edit", at,
                                          tl::to_string(tr("Undo(Ctrl+Z)"))));
    menu_entries.push_back(lay::menu_item("cm_redo", "redo:edit", at,
                                          tl::to_string(tr("Redo(Ctrl+Y)"))));

    menu_entries.push_back(lay::separator("basic_group", at));
    menu_entries.push_back(lay::submenu("layout_menu:edit:edit_mode", at,
                                        tl::to_string(tr("Layout"))));
    {
      std::string at = "edit_menu.layout_menu.end";
      menu_entries.push_back(
          lay::menu_item("cm_lay_flip_x", "lay_flip_x:edit_mode", at,
                         tl::to_string(tr("Flip Horizontally"))));
      menu_entries.push_back(
          lay::menu_item("cm_lay_flip_y", "lay_flip_y:edit_mode", at,
                         tl::to_string(tr("Flip Vertically"))));
      menu_entries.push_back(
          lay::menu_item("cm_lay_rot_cw", "lay_rot_cw:edit_mode", at,
                         tl::to_string(tr("Rotate Clockwise"))));
      menu_entries.push_back(
          lay::menu_item("cm_lay_rot_ccw", "lay_rot_ccw:edit_mode", at,
                         tl::to_string(tr("Rotate Counterclockwise"))));
      menu_entries.push_back(
          lay::menu_item("cm_lay_free_rot", "lay_free_rot:edit_mode", at,
                         tl::to_string(tr("Rotation By Angle"))));
      menu_entries.push_back(lay::menu_item("cm_lay_scale",
                                            "lay_scale:edit_mode", at,
                                            tl::to_string(tr("Scale"))));
      menu_entries.push_back(lay::menu_item("cm_lay_move", "lay_move:edit_mode",
                                            at, tl::to_string(tr("Move By"))));
      menu_entries.push_back(lay::separator("cellop_group", at));
      menu_entries.push_back(lay::menu_item(
          "cm_lay_convert_to_static", "lay_convert_to_static:edit_mode", at,
          tl::to_string(tr("Convert All Cells To Static"))));
    }

    menu_entries.push_back(lay::submenu("cell_menu:edit:edit_mode", at,
                                        tl::to_string(tr("Cell"))));
    {
      std::string at = "edit_menu.cell_menu.end";
      menu_entries.push_back(lay::menu_item("cm_new_cell",
                                            "new_cell:edit:edit_mode", at,
                                            tl::to_string(tr("New Cell"))));
      menu_entries.push_back(lay::menu_item("cm_cell_delete",
                                            "delete_cell:edit:edit_mode", at,
                                            tl::to_string(tr("Delete Cell"))));
      menu_entries.push_back(lay::menu_item("cm_cell_rename",
                                            "rename_cell:edit:edit_mode", at,
                                            tl::to_string(tr("Rename Cell"))));
      menu_entries.push_back(lay::menu_item("cm_cell_replace",
                                            "replace_cell:edit:edit_mode", at,
                                            tl::to_string(tr("Replace Cell"))));
      menu_entries.push_back(lay::menu_item("cm_cell_flatten",
                                            "flatten_cell:edit:edit_mode", at,
                                            tl::to_string(tr("Flatten Cell"))));
      menu_entries.push_back(lay::separator("ops_group", at));
      menu_entries.push_back(lay::menu_item(
          "cm_adjust_origin", "adjust_cell_origin:edit:edit_mode", at,
          tl::to_string(tr("Adjust Origin"))));
      menu_entries.push_back(lay::menu_item(
          "cm_cell_convert_to_static", "convert_cell_to_static:edit_mode", at,
          tl::to_string(tr("Convert Cell To Static"))));
      menu_entries.push_back(lay::separator("props_group", at));
      menu_entries.push_back(
          lay::menu_item("cm_cell_user_properties", "user_properties", at,
                         tl::to_string(tr("User Properties"))));
    }

    menu_entries.push_back(lay::submenu("layer_menu:edit:edit_mode", at,
                                        tl::to_string(tr("Layer"))));
    {
      std::string at = "edit_menu.layer_menu.end";
      menu_entries.push_back(lay::menu_item("cm_new_layer",
                                            "new_layer:edit:edit_mode", at,
                                            tl::to_string(tr("New Layer"))));
      menu_entries.push_back(lay::menu_item("cm_clear_layer",
                                            "clear_layer:edit:edit_mode", at,
                                            tl::to_string(tr("Clear Layer"))));
      menu_entries.push_back(lay::menu_item("cm_delete_layer",
                                            "delete_layer:edit:edit_mode", at,
                                            tl::to_string(tr("Delete Layer"))));
      menu_entries.push_back(lay::menu_item("cm_copy_layer",
                                            "copy_layer:edit:edit_mode", at,
                                            tl::to_string(tr("Copy Layer"))));
      menu_entries.push_back(
          lay::menu_item("cm_edit_layer", "edit_layer:edit:edit_mode", at,
                         tl::to_string(tr("Edit Layer Specification"))));
    }

    menu_entries.push_back(lay::submenu("selection_menu:edit", at,
                                        tl::to_string(tr("Selection"))));
    {
      std::string at = "edit_menu.selection_menu.end";
      menu_entries.push_back(
          lay::menu_item("cm_sel_flip_x", "sel_flip_x", at,
                         tl::to_string(tr("Flip Horizontally"))));
      menu_entries.push_back(
          lay::menu_item("cm_sel_flip_y", "sel_flip_y", at,
                         tl::to_string(tr("Flip Vertically"))));
      menu_entries.push_back(
          lay::menu_item("cm_sel_rot_cw", "sel_rot_cw", at,
                         tl::to_string(tr("Rotate Clockwise"))));
      menu_entries.push_back(
          lay::menu_item("cm_sel_rot_ccw", "sel_rot_ccw", at,
                         tl::to_string(tr("Rotate Counterclockwise"))));
      menu_entries.push_back(
          lay::menu_item("cm_sel_free_rot", "sel_free_rot", at,
                         tl::to_string(tr("Rotation By Angle"))));
      menu_entries.push_back(lay::menu_item("cm_sel_scale", "sel_scale", at,
                                            tl::to_string(tr("Scale"))));
      menu_entries.push_back(lay::menu_item("cm_sel_move", "sel_move", at,
                                            tl::to_string(tr("Move By"))));
      menu_entries.push_back(lay::menu_item("cm_sel_move_to", "sel_move_to", at,
                                            tl::to_string(tr("Move To"))));
    }

    menu_entries.push_back(lay::separator("utils_group", at));
    menu_entries.push_back(lay::submenu("utils_menu:edit:edit_mode", at,
                                        tl::to_string(tr("Utilities"))));

    menu_entries.push_back(lay::separator("misc_group", at));
    menu_entries.push_back(lay::menu_item("cm_delete", "delete:edit", at,
                                          tl::to_string(tr("Delete(Del)"))));
    menu_entries.push_back(lay::menu_item("cm_show_properties",
                                          "show_properties:edit", at,
                                          tl::to_string(tr("Properties(Q)"))));

    menu_entries.push_back(lay::separator("cpc_group", at));
    menu_entries.push_back(lay::menu_item("cm_copy", "copy:edit", at,
                                          tl::to_string(tr("Copy(Ctrl+C)"))));
    menu_entries.push_back(lay::menu_item("cm_cut", "cut:edit", at,
                                          tl::to_string(tr("Cut(Ctrl+X)"))));
    menu_entries.push_back(lay::menu_item("cm_paste", "paste:edit", at,
                                          tl::to_string(tr("Paste(Ctrl+V)"))));
    menu_entries.push_back(
        lay::menu_item("cm_duplicate", "duplicate:edit", at,
                       tl::to_string(tr("Duplicate(Ctrl+B)"))));

    menu_entries.push_back(lay::separator("modes_group", at));
    menu_entries.push_back(
        lay::submenu("mode_menu", at, tl::to_string(tr("Mode"))));

    menu_entries.push_back(
        lay::submenu("select_menu", at, tl::to_string(tr("Select"))));
    {
      std::string at = "edit_menu.select_menu.end";
      menu_entries.push_back(lay::menu_item("cm_select_all", "select_all", at,
                                            tl::to_string(tr("Select All"))));
      menu_entries.push_back(lay::menu_item("cm_unselect_all", "unselect_all",
                                            at,
                                            tl::to_string(tr("Unselect All"))));
      menu_entries.push_back(lay::separator("edit_select_basic_group", at));
      menu_entries.push_back(lay::menu_item("lv:enable_all", "enable_all", at,
                                            tl::to_string(tr("Enable All"))));
      menu_entries.push_back(lay::menu_item("lv:disable_all", "disable_all", at,
                                            tl::to_string(tr("Disable All"))));
      menu_entries.push_back(
          lay::separator("edit_select_individual_group", at));
    };

    menu_entries.push_back(lay::separator("cancel_group", at));
    menu_entries.push_back(lay::menu_item("cm_cancel", "cancel", at,
                                          tl::to_string(tr("Cancel(Esc)"))));

    at = "bookmark_menu.end";
    menu_entries.push_back(lay::submenu("goto_bookmark_menu", at,
                                        tl::to_string(tr("Goto Bookmark"))));
    menu_entries.push_back(
        lay::menu_item("cm_bookmark_view", "bookmark_view", at,
                       tl::to_string(tr("Bookmark This View"))));

    menu_entries.push_back(lay::separator("bookmark_mgm_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_manage_bookmarks", "manage_bookmarks", at,
                       tl::to_string(tr("Manage Bookmarks"))));
    menu_entries.push_back(lay::menu_item("cm_load_bookmarks", "load_bookmarks",
                                          at,
                                          tl::to_string(tr("Load Bookmarks"))));
    menu_entries.push_back(lay::menu_item("cm_save_bookmarks", "save_bookmarks",
                                          at,
                                          tl::to_string(tr("Save Bookmarks"))));
    menu_entries.push_back(
        lay::submenu("open_recent_menu_bookmarks", at,
                     tl::to_string(tr("Recent Bookmark Files"))));

    at = "zoom_menu.end";
    menu_entries.push_back(lay::submenu(
        "global_trans", at, tl::to_string(tr("Global Transformation"))));
    {
      std::string at = "zoom_menu.global_trans.end";
      menu_entries.push_back(lay::config_menu_item(
          "r0", at, tl::to_string(tr("\\(r0\\)<:/r0_24px.png>")),
          cfg_global_trans, "?r0 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "r90", at, tl::to_string(tr("\\(r90\\)<:/r90_24px.png>")),
          cfg_global_trans, "?r90 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "r180", at, tl::to_string(tr("\\(r180\\)<:/r180_24px.png>")),
          cfg_global_trans, "?r180 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "r270", at, tl::to_string(tr("\\(r270\\)<:/r270_24px.png>")),
          cfg_global_trans, "?r270 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "m0", at, tl::to_string(tr("\\(m0\\)<:/m0_24px.png>")),
          cfg_global_trans, "?m0 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "m45", at, tl::to_string(tr("\\(m45\\)<:/m45_24px.png>")),
          cfg_global_trans, "?m45 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "m90", at, tl::to_string(tr("\\(m90\\)<:/m90_24px.png>")),
          cfg_global_trans, "?m90 *1 0,0"));
      menu_entries.push_back(lay::config_menu_item(
          "m135", at, tl::to_string(tr("\\(m135\\)<:/m135_24px.png>")),
          cfg_global_trans, "?m135 *1 0,0"));
    }

    menu_entries.push_back(lay::separator("hier_group", at));
    menu_entries.push_back(lay::menu_item(
        "cm_max_hier", "max_hier", at, tl::to_string(tr("Full Hierarchy(*)"))));
    menu_entries.push_back(lay::menu_item("cm_max_hier_0", "max_hier_0", at,
                                          tl::to_string(tr("Box Only(0)"))));
    menu_entries.push_back(
        lay::menu_item("cm_max_hier_1", "max_hier_1", at,
                       tl::to_string(tr("Top Level Only(1)"))));
    menu_entries.push_back(
        lay::menu_item("cm_inc_max_hier", "inc_max_hier", at,
                       tl::to_string(tr("Increment Hierarchy(+)"))));
    menu_entries.push_back(
        lay::menu_item("cm_dec_max_hier", "dec_max_hier", at,
                       tl::to_string(tr("Decrement Hierarchy(-)"))));

    menu_entries.push_back(lay::separator("zoom_group", at));
    menu_entries.push_back(lay::menu_item("cm_zoom_fit", "zoom_fit", at,
                                          tl::to_string(tr("Zoom Fit(F2)"))));
    menu_entries.push_back(
        lay::menu_item("cm_zoom_fit_sel", "zoom_fit_sel", at,
                       tl::to_string(tr("Zoom Fit Selection(Shift+F2)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_zoom_in", "zoom_in", at, tl::to_string(tr("Zoom In(Ctrl++)"))));
    menu_entries.push_back(lay::menu_item(
        "cm_zoom_out", "zoom_out", at, tl::to_string(tr("Zoom Out(Ctrl+-)"))));
    /* disabled because that interferes with the use of the arrow keys for
    moving the selection MenuLayoutEntry::separator ("pan_group");
    menu_entries.push_back (lay::menu_item ("cm_pan_up", "pan_up", at,
    tl::to_string (tr ("Pan Up(Up)")))); menu_entries.push_back (lay::menu_item
    ("cm_pan_down", "pan_down", at, tl::to_string (tr ("Pan Down(Down)"))));
    menu_entries.push_back (lay::menu_item ("cm_pan_left", "pan_left", at,
    tl::to_string (tr ("Pan Left(Left)")))); menu_entries.push_back
    (lay::menu_item ("cm_pan_right", "pan_right", at, tl::to_string (tr ("Pan
    Right(Right)"))));
    */

    menu_entries.push_back(lay::separator("redraw_group", at));
    menu_entries.push_back(
        lay::menu_item("cm_redraw", "redraw", at, tl::to_string(tr("Redraw"))));
    menu_entries.push_back(lay::separator("state_group", at));
    menu_entries.push_back(lay::menu_item_copy("cm_prev_display_state",
                                               "prev_display_state", at,
                                               "@toolbar.prev_display_state"));
    menu_entries.push_back(lay::menu_item_copy("cm_next_display_state",
                                               "next_display_state", at,
                                               "@toolbar.next_display_state"));

    menu_entries.push_back(lay::separator("select_group", at));
    menu_entries.push_back(lay::menu_item("cm_select_cell", "select_cell:edit",
                                          at,
                                          tl::to_string(tr("Select Cell"))));
    menu_entries.push_back(
        lay::menu_item("cm_select_current_cell", "select_current_cell", at,
                       tl::to_string(tr("Show As New Top(Ctrl+S)"))));
    menu_entries.push_back(
        lay::menu_item("cm_goto_position", "goto_position", at,
                       tl::to_string(tr("Goto Position(Ctrl+G)"))));
  ...
  void implements_primary_mouse_modes(
    ...
    int mode_id = 0;
    for (std::vector<std::string>::const_iterator t = mode_titles.begin();
         t != mode_titles.end(); ++t, --mode_id) {
      //  modes: pair(title, pair(insert_pos, id))
      modes.push_back(std::make_pair(
          *t, std::make_pair("edit_menu.mode_menu.end;@toolbar.end_modes",
                             mode_id)));
    }
  }

  lay::Plugin *create_plugin(db::Manager *manager, Dispatcher *,
...
static tl::RegisteredClass<lay::PluginDeclaration>
    config_decl(new LayoutViewPluginDeclaration(), -10, "LayoutViewPlugin");
