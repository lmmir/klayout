 ...
 */
LAYUI_PUBLIC std::string save_dialog_state(QWidget *dialog,
                                           bool with_section_sizes = true);

/**
 *  @brief Restore the dialog's state from the given string
 */
LAYUI_PUBLIC void restore_dialog_state(QWidget *dialog, const std::string &s,
                                       bool with_section_sizes = true);

/**
 *  @brief A utility function connecting a label's linkActivated event with the
 * help browser
 */
...
/**
 *  @brief A utility function connecting a label's linkActivated event with the
 * help browser (modal help dialogs)
 */
LAYUI_PUBLIC void activate_modal_help_links(QLabel *label);

/**
 *  @brief Register the help handler (object and slots for non-modal and modal
 * help dialogs)
 */
LAYUI_PUBLIC void register_help_handler(QObject *object, const char *slot,
