...
TEST(1) {
  ...
  std::unique_ptr<lay::NetlistBrowserTreeModel> model(
      new lay::NetlistBrowserTreeModel(0, &l2n));
  ...
  EXPECT_EQ(
      tl::to_string(
          model->data(model->index(0, 0, ringoIndex), Qt::UserRole).toString()),
      "INV2");
  EXPECT_EQ(
      tl::to_string(model->data(model->index(0, 0, ringoIndex), Qt::DisplayRole)
                        .toString()),
      "INV2");
  EXPECT_EQ(model->parent(model->index(0, 0, ringoIndex)).isValid(), true);
  EXPECT_EQ(model->parent(model->index(0, 0, ringoIndex)).internalId() ==
                ringoIndex.internalId(),
            true);
...
TEST(2) {
  ...
  std::unique_ptr<lay::NetlistBrowserTreeModel> model(
      new lay::NetlistBrowserTreeModel(0, &lvs));
  ...
  EXPECT_EQ(tl::to_string(
            ...
            "- \u21D4 INV2PAIRX");
  EXPECT_EQ(
      tl::to_string(model->data(model->index(1, 0, QModelIndex()), Qt::UserRole)
                        .toString()),
      "RINGO|RINGO");
  EXPECT_EQ(tl::to_string(
  ...
  EXPECT_EQ(
      tl::to_string(model->data(model->index(0, 0, inv2PairIndex), Qt::UserRole)
                        .toString()),
      "INV2");
  EXPECT_EQ(tl::to_string(
                model->data(model->index(0, 0, inv2PairIndex), Qt::DisplayRole)
                    .toString()),
            "- \u21D4 INV2");
  EXPECT_EQ(
      tl::to_string(model->data(model->index(1, 0, inv2PairIndex), Qt::UserRole)
                        .toString()),
      "INV2");
  EXPECT_EQ(tl::to_string(
                model->data(model->index(1, 0, inv2PairIndex), Qt::DisplayRole)
                    .toString()),
            "INV2 \u21D4 -");
  ...
  EXPECT_EQ(model->parent(model->index(0, 0, inv2PairIndex)).internalId() ==
                inv2PairIndex.internalId(),
            true);
  EXPECT_EQ(model->parent(model->index(1, 0, inv2PairIndex)).isValid(), true);
  EXPECT_EQ(model->parent(model->index(1, 0, inv2PairIndex)).internalId() ==
                inv2PairIndex.internalId(),
            true);
}
