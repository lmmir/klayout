...
class TestCollectorConsole : public gsi::Console {
public:
  TestCollectorConsole() {}
  ~TestCollectorConsole() {}

  virtual void write_str(const char *text, output_stream) { m_text += text; }
...
TEST(3_RubyInclude) {
  ...
  EXPECT_EQ(np(console.text()),
            np("An error in " + tl::testsrc() + "/testdata/lym/b_inc.rb:3\n"));
}
...
TEST(4_RubyIncludeFromXML) {
  ...
  EXPECT_EQ(np(console.text()),
            np("An error in " + tl::testsrc() + "/testdata/lym/b_inc.lym:3\n"));
}
...
TEST(11_DRCBasic) {
  ...
  EXPECT_EQ(console.text(),
            "Result: (500,500;500,2000;1000,2000;1000,500) in m1.drc:20\n");
}
...
TEST(12_DRCBasic) {
  ...
  EXPECT_EQ(console.text(),
            "Result: (500,500;500,2000;1000,2000;1000,500) in m2.drc:14\n");
}
