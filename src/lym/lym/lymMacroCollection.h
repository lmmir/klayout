...
public:
  ...
  typedef std::map<std::string, MacroCollection *>::const_iterator
   ...
   *
   *  If force_create is true (the default), the folder will be created if it
   * does not exist yet. On error, 0 is returned.
   */
  MacroCollection *add_folder(const std::string &description,
   ...
   *
   *  A category tag can be used to categorize the collections. For example, DRC
   * categories are handled differently from the other categories.
   */
   ...
   */
  void set_category(const std::string &d) { m_category = d; }

  /**
   *  @brief Collect all Macro and MacroCollection objects inside a tree
   * starting from this collection
   */
  void collect_used_nodes(std::set<Macro *> &macros,
                          std::set<MacroCollection *> &macro_collections);
   ...
   *  The folder cannot be deleted if it contains any files, also some that are
   * not listed because they don't end with .lym, .rb or similar.
   */
  bool del();
   ...
   */
  int virtual_mode() const { return m_virtual_mode; }
   ...
   */
  void set_virtual_mode(int m) { m_virtual_mode = m; }
   ...
   *  In contrast to the private \set_readonly method, this version delivers a
   * "changed" signal when the flag changed.
   */
   ...
   * well. This method will return true, if the rename was successful.
   */
   ...
   *  the location of the macro using the path information of the macro and
   * insert the macro there.
   *
  ...
  /**
   *  @brief Erases the given macro collection from the list of child
   * collections
   *
   *  This does not remove the directory but just removes the macro collection
   * object. This will also delete the macro collection object.
   */
   ...
   *
   *  If a name is given, it is used as a prefix to create a unique name for a
   * macro with that format.
   */
  lym::Macro *create(const char *name = 0,
   ...
   */
  iterator begin() { return m_macros.begin(); }

  /**
   *  @brief Gets the end iterator of the macros
   */
  iterator end() { return m_macros.end(); }

  /**
   *  @brief Gets the begin iterator of the macros (const version)
   */
  const_iterator begin() const { return m_macros.begin(); }

  /**
   *  @brief Gets the end iterator of the macros (const version)
   */
  const_iterator end() const { return m_macros.end(); }
   ...
   *  necessarily live inside the directory of the parent folder. Specifically
   * for the root folder, children with any kind of paths may be present.
   */
  child_iterator begin_children() { return m_folders.begin(); }

  /**
   *  @brief Gets the end iterator of the folders
   */
  child_iterator end_children() { return m_folders.end(); }

  /**
   *  @brief Gets the begin iterator of the folders (const version)
   */
  const_child_iterator begin_children() const { return m_folders.begin(); }

  /**
   *  @brief Gets the end iterator of the folders (const version)
   */
  const_child_iterator end_children() const { return m_folders.end(); }
   ...
   *  If no macro with that name exists, this method will return 0.
   */
  const Macro *macro_by_name(const std::string &name,
   ...
   *
   *  This method is called from the root collection and delivers the macro
   * which matches the given path or 0.
   */
   ...
   *
   *  This method is similar to rescan, but it will also remove folders and
   * macros. In safe mode (safe = true), modified macros won't be overwritten.
   */
  void reload(bool safe);

  /**
   *  @brief Gets the root of the macro hierarchy corresponding to the
   * configuration space
   */
...
signals:
  ...
  /**
   *  @brief This signal is sent by collection when a child collection is
   * deleted in this collection
   */
  void child_deleted(lym::MacroCollection *);

  /**
   *  @brief This signal is sent by the root object when a macro collection is
   * deleted
   */
  void macro_collection_deleted(lym::MacroCollection *);

  /**
   *  @brief This signal is sent by collection when a macro is deleted in this
   * collection
   */
  ...
  /**
   *  @brief This signal is sent by the root object when a macro collection
   * changes
   *
   *  This signal is only emitted by the root, but it may originate from a
  ...
  /**
   *  @brief This signal is sent by the root object befor the macro collection
   * changes
   */
  void about_to_change();

  /**
   *  @brief This signal is emitted from the collection root if the menu needs
   * to be updated
   */
...
private:
  ...
  void set_name(const std::string &n) { m_path = n; }
