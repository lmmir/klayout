...
Macro::Macro()
    : m_modified(true), m_readonly(false), m_autorun(false),
      m_autorun_default(false), m_autorun_early(false), m_priority(0),
      m_show_in_menu(false), m_is_file(false), m_interpreter(None),
      m_format(Macro::NoFormat) {
  mp_parent = 0;
}

void Macro::on_menu_needs_update() {
#if defined(HAVE_QT)
  //  forward the signal to the root collection - the main window will attach to
  //  this
...
bool Macro::operator==(const Macro &other) const {
  return m_description == other.m_description && m_version == other.m_version &&
         m_epilog == other.m_epilog && m_prolog == other.m_prolog &&
         m_category == other.m_category && m_text == other.m_text &&
         m_autorun == other.m_autorun &&
         m_autorun_early == other.m_autorun_early &&
         m_priority == other.m_priority &&
         m_show_in_menu == other.m_show_in_menu &&
 ...
 */
static tl::XMLStruct<lym::Macro> xml_struct(
    "klayout-macro",
    tl::make_member(&Macro::description, &Macro::set_description,
                    "description") +
        ...
        tl::make_member(&Macro::is_autorun_early, &Macro::set_autorun_early,
                        "autorun-early") +
        tl::make_member(&Macro::priority, &Macro::set_priority, "priority") +
        tl::make_member(&Macro::shortcut, &Macro::set_shortcut, "shortcut") +
        tl::make_member(&Macro::show_in_menu, &Macro::set_show_in_menu,
                        "show-in-menu") +
        tl::make_member(&Macro::group_name, &Macro::set_group_name,
                        "group-name") +
        tl::make_member(&Macro::menu_path, &Macro::set_menu_path, "menu-path") +
        tl::make_member(&Macro::interpreter, &Macro::set_interpreter,
                        "interpreter", Interpreter2s()) +
        tl::make_member(&Macro::dsl_interpreter, &Macro::set_dsl_interpreter,
...
void Macro::load_from(const std::string &fn) {
  ...
  std::pair<bool, std::string> f = format_from_filename(
      fn, m_interpreter, m_dsl_interpreter, m_autorun_default, m_format);
  if (f.first) {
    ...
    if (m_format == MacroFormat) {
      ...
      //  default interpreter for .lym files is Ruby - but should be mentioned
      //  in the file anyway
      m_interpreter = Ruby;
      ...
      xml_struct.parse(source, *this);
               ...
               m_format == PlainTextWithHashAnnotationsFormat) {
...
void Macro::load_from_string(const std::string &text, const std::string &url) {
  ...
  if (format_from_suffix(tl::URI(url).path(), m_interpreter, m_dsl_interpreter,
                         m_autorun_default, m_format)) {
...
void Macro::load() { load_from(path()); }

bool Macro::format_from_suffix(const std::string &fn,
                               Macro::Interpreter &interpreter,
                               std::string &dsl_name, bool &autorun_pref,
                               Macro::Format &format) {
  return format_from_suffix_string(tl::extension_last(fn), interpreter,
                                   dsl_name, autorun_pref, format);
}

std::pair<bool, std::string> Macro::format_from_filename(
    const std::string &fn, Macro::Interpreter &interpreter,
    std::string &dsl_name, bool &autorun_pref, Macro::Format &format) {
  tl::GlobPattern pat("(*)\\[(*)\\]");
  std::vector<std::string> pat_parts;
  if (pat.match(fn, pat_parts) && pat_parts.size() == 2) {
    return std::make_pair(format_from_suffix_string(pat_parts[1], interpreter,
                                                    dsl_name, autorun_pref,
                                                    format),
                          pat_parts[0]);
  } else {
    return std::make_pair(
        format_from_suffix(fn, interpreter, dsl_name, autorun_pref, format),
        fn);
  }
}

bool Macro::format_from_suffix_string(const std::string &suffix,
                                      Macro::Interpreter &interpreter,
                                      std::string &dsl_name, bool &autorun_pref,
                                      Macro::Format &format) {
  interpreter = None;
  ...
  if (suffix == "rb" || suffix == "rbm") {
    ...
    for (tl::Registrar<lym::MacroInterpreter>::iterator cls =
      ...
      if (cls->suffix() == suffix) {
        ...
        interpreter =
            DSLInterpreter; // by default - may be overridden by content of file
...
std::string Macro::suffix_for_format(Macro::Interpreter interpreter,
...
std::string Macro::summary() const {
  return std::string("<html><body><b>") + interpreter_name() + "</b> " +
         path() + "</body></html>";
}
...
std::string Macro::path() const {
  ...
  std::string suffix =
      suffix_for_format(m_interpreter, m_dsl_interpreter, m_format);
  if (mp_parent) {
...
void Macro::set_file_path(const std::string &fp) { m_file_path = fp; }
...
bool Macro::rename(const std::string &n) {
  if (m_is_file && parent()) {
    ...
    if (!tl::rename_file(path(),
                         tl::combine_path(parent()->path(), n + suffix))) {
      return false;
    }
  }
...
const std::string &Macro::text() const { return m_text; }
...
static PropertyField property_fields[] = {
    {"description", &lym::Macro::description, &lym::Macro::set_description, 0,
     0, 0, 0},
    ...
    {"autorun-early", 0, 0, &lym::Macro::is_autorun_early,
     &lym::Macro::set_autorun_early, 0, 0},
    {"show-in-menu", 0, 0, &lym::Macro::show_in_menu,
     &lym::Macro::set_show_in_menu, 0, 0},
    {"group-name", &lym::Macro::group_name, &lym::Macro::set_group_name, 0, 0,
     0, 0},
    {"menu-path", &lym::Macro::menu_path, &lym::Macro::set_menu_path, 0, 0, 0,
     0},
...
void Macro::sync_text_with_properties() {
  ...
  for (size_t i = 0; i < sizeof(property_fields) / sizeof(property_fields[0]);
       ++i) {
  ...
  for (std::vector<std::string>::const_iterator l = lines.begin();
    ...
    if (stop_fishing) {
      ...
      for (size_t i = 0;
           i < sizeof(property_fields) / sizeof(property_fields[0]) && !taken;
           ++i) {
...
void Macro::sync_properties_with_text() {
  ...
  for (std::vector<std::string>::const_iterator l = lines.begin();
    ...
    if (ex.test("#") && ex.test("$")) {
      ...
      for (size_t i = 0;
           i < sizeof(property_fields) / sizeof(property_fields[0]); ++i) {
...
class ExternalMethod : public gsi::MethodBase {
public:
  ExternalMethod(const std::string &name, const std::string &doc, bool c,
                 bool s)
  ...
  virtual MethodBase *clone() const { return new ExternalMethod(*this); }

  //  this class is not intended to go functional. It's just a hook for the
  //  documentation
 ...
 *
 *  This declaration is not intended to go functional. It's just a hook for the
 * documentation.
 */
class ExternalClass : public gsi::ClassBase {
public:
  ExternalClass(const std::string &module, const std::string &name,
                const std::string &category, const gsi::ClassBase *base,
                const std::string &doc, const gsi::Methods &mm)
  ...
  if (!lines.empty() && tl::trim(lines[0]).find("@class") == 0) {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
      ...
      if (ex.test("@class")) {
        ...
        if (cls) {
          tl::error << tl::to_string(tr("Reading class doc from ")) << path()
                    << ": " << tl::to_string(tr("Duplicate @class"));
          return;
        }
        ...
        if (!super_cls_name.empty()) {
          for (gsi::ClassBase::class_iterator c =
                   gsi::ClassBase::begin_classes();
               c != gsi::ClassBase::end_classes(); ++c) {
          ...
          if (!super_cls) {
            tl::error << tl::to_string(tr("Reading class doc from ")) << path()
                      << ": " << tl::to_string(tr("Cannot find super class: "))
                      << super_cls_name;
            return;
          }
        }
        ...
        if (!cls) {
          ...
          ExternalClass *ext_cls = new ExternalClass(
              module, cls_name, category(), super_cls, doc, gsi::Methods());
          ext_classes.push_back(ext_cls);
          cls = ext_cls;
        }

      } else if (ex.test("@method") ||
                 (st = ex.test("@static_method")) == true) {

        if (cls == 0) {
          tl::error << tl::to_string(tr("Reading class doc from ")) << path()
                    << ": "
                    << tl::to_string(tr("@method without preceding @class"));
        } else {
  ...
  try {
    ...
    if (ip) {
      ...
      std::pair<std::string, std::string> ep =
          def_interpreter.include_expansion(this);
      ...
      throw tl::Exception(
          tl::to_string(tr("Can't run macro (no interpreter): ")) + path());
    }
