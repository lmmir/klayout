...
class LYM_PUBLIC MacroInterpreter : public gsi::ObjectBase {
public:
  /**
   ...
   *
   *  The storage scheme is used to determine how the macro's text shall be
   * stored. The scheme can be MacroFormat for the macro XML format or
   * PlainTextFormat for plain text.
   */
   ...
   *  Ruby highlighting.
   */
  virtual std::string syntax_scheme() const { return std::string(); }
   ...
   */
  virtual Macro::Interpreter debugger_scheme() const { return Macro::None; }
   ...
   *
   *  The templates are required by the macro editor in order to allow creation
   * of new macros. The template objects must be new'd and added to the
   * templates vector by the implementation of this method. These objects then
   ...
   *
   *  This method takes a given macro and substitutes include statements of the
   * form '# %include ...' by the content of the respective file. Recursive
   * include is supported. The return value of this method is a two-element
   * array with two strings: the first one is a path string which holds the
   * encoded information for translating back path/line number information into
   * the original paths and line numbers. This first string needs to be passed
   * to the actual script interpreter as the 'file path'. The second component
   * of the returned array is the text of the macro with the include files
   * substituted.
   */
  ...
  /**
   *  @brief Returns true, if a DSL interpreter is registered for the given
   * macro
   */
