...
MacroCollection::~MacroCollection() { do_clear(); }
...
void MacroCollection::begin_changes() {
  //  Note: it is very important that each on_changed occurs after exactly one
  //  begin_changes. (See #459 for example)
  if (mp_parent) {
...
void MacroCollection::collect_used_nodes(
    std::set<Macro *> &macros, std::set<MacroCollection *> &macro_collections) {
  for (MacroCollection::child_iterator c = begin_children();
       c != end_children(); ++c) {
...
Macro *MacroCollection::macro_by_name(const std::string &name,
                                      Macro::Format format) {
...
const Macro *MacroCollection::macro_by_name(const std::string &name,
                                            Macro::Format format) const {
...
MacroCollection::folder_by_name(const std::string &name) const {
  std::map<std::string, MacroCollection *>::const_iterator i =
      m_folders.find(name);
  if (i != m_folders.end()) {
    return i->second;
  } else {
...
MacroCollection *MacroCollection::add_folder(const std::string &description,
                                             const std::string &p,
                                             const std::string &cat,
                                             bool readonly, bool auto_create) {
  if (!p.empty() && p[0] == ':') {
    ...
    if (!tl::file_exists(fp)) {
      ...
      if (readonly || !auto_create) {
        ...
        if (tl::verbosity() >= 20) {
          tl::log << tl::to_string(tr("Folder does not exist - skipping: "))
                  << fp;
        }
        return 0;
        ...
        if (tl::verbosity() >= 20) {
          tl::log << tl::to_string(tr(
                         "Folder does not exist yet - trying to create it: "))
                  << fp;
        }
        if (!tl::mkpath(fp)) {
          if (tl::verbosity() >= 20) {
            tl::error << tl::to_string(tr("Unable to create folder path: "))
                      << fp;
          }
          return 0;
        }
    ...
    if (!tl::is_dir(fp)) {
      if (tl::verbosity() >= 20) {
        tl::error << tl::to_string(tr("Folder is not a directory - skipping: "))
                  << fp;
      }
      return 0;
    }
  ...
  MacroCollection *mc =
      m_folders.insert(std::make_pair(p, new MacroCollection())).first->second;
  mc->set_name(p);
...
void MacroCollection::rescan() {
  for (std::map<std::string, MacroCollection *>::const_iterator m =
           m_folders.begin();
       m != m_folders.end(); ++m) {
    m->second->scan();
  }
 ...
 */
class ResourceWithChildren : public QResource {
public:
...
void MacroCollection::scan() {
  ...
  if (tl::verbosity() >= 20) {
    tl::info << tl::to_string(tr("Scanning macro path ")) << p
             << " (readonly=" << m_readonly << ")";
  }
  ...
  if (!p.empty() && p[0] == ':') {
    ...
    std::vector<std::string> files = tl::dir_entries(
        p, true /*with_files*/, false /*with_dirs*/, true /*without_dotfiles*/);
    ...
    std::vector<std::string> dirs = tl::dir_entries(
        p, true /*with_files*/, true /*with_dirs*/, true /*without_dotfiles*/);
    for (auto f = dirs.begin(); f != dirs.end(); ++f) {
      ...
      try {
        ...
        MacroCollection *&mc =
            m_folders.insert(std::make_pair(*f, (MacroCollection *)0))
                .first->second;
        if (!mc) {
...
void MacroCollection::create_entry(const std::string &path) {
  try {
    ...
    if (Macro::format_from_suffix(path, interpreter, dsl_name, autorun,
                                  format)) {

      iterator mm = m_macros.find(n);
      bool found = false;
      while (mm != m_macros.end() && mm->first == n && !found) {
        if ((interpreter == Macro::None ||
             mm->second->interpreter() == interpreter) &&
...
void MacroCollection::save() {
  ...
  for (iterator m = m_macros.begin(); m != m_macros.end(); ++m) {
    if (m->second->is_modified() && !m->second->is_readonly() &&
        !m->second->path().empty()) {
      try {
        m->second->save();
      } catch (tl::Exception &ex) {
...
bool MacroCollection::rename(const std::string &n) {
  if (tl::verbosity() >= 20) {
    tl::info << tl::to_string(tr("Renaming macro folder ")) << path() << " to "
             << n;
  }
  begin_changes();
...
lym::MacroCollection *MacroCollection::create_folder(const char *prefix,
                                                     bool mkdir) {
  std::string name;
  int n = 0;
  do {
  ...
  lym::MacroCollection *m =
      m_folders.insert(std::make_pair(name, new lym::MacroCollection()))
          .first->second;
...
lym::Macro *MacroCollection::create(const char *prefix, Macro::Format format) {
  ...
  lym::Macro *m =
      m_macros.insert(std::make_pair(name, new lym::Macro()))->second;
  m->set_name(name);
  m->set_parent(this);
...
bool MacroCollection::add(lym::Macro *m) {
  ...
  if (d == dd) {
    ...
    //  try to detect new child folders. If that is the case, create that folder
    //  and add the macro there.
    ...
    while (true) {
      ...
      if (dm == d) {
        ...
        lym::MacroCollection *mc =
            m_folders.insert(std::make_pair(folder_name, new MacroCollection()))
                .first->second;
        mc->set_virtual_mode(NotVirtual);
...
MacroCollection &MacroCollection::root() { return ms_root; }

static bool sync_macros(lym::MacroCollection *current,
                        lym::MacroCollection *actual, bool safe) {
  bool ret = false;
  ...
  for (lym::MacroCollection::child_iterator m = current->begin_children();
       m != current->end_children(); ++m) {
  ...
  if (actual) {
    for (lym::MacroCollection::child_iterator m = actual->begin_children();
         m != actual->end_children(); ++m) {
  ...
  for (lym::MacroCollection::iterator m = current->begin(); m != current->end();
       ++m) {
    lym::Macro *cm =
        actual ? actual->macro_by_name(m->first, m->second->format()) : 0;
    if (!cm) {
  ...
  if (actual) {
    for (lym::MacroCollection::iterator m = actual->begin(); m != actual->end();
         ++m) {
  ...
  for (std::vector<lym::Macro *>::const_iterator m = macros_to_delete.begin();
       m != macros_to_delete.end(); ++m) {
    current->erase(*m);
    ret = true;
  }
...
void MacroCollection::reload(bool safe) {
  ...
  for (lym::MacroCollection::child_iterator c = begin_children();
       c != end_children(); ++c) {
    new_collection.add_folder(c->second->description(), c->second->path(),
                              c->second->category(), c->second->is_readonly(),
                              false /* don't auto-create folder */);
  }
...
static bool has_autorun_for(const lym::MacroCollection &collection,
                            bool early) {
  ...
  for (lym::MacroCollection::const_iterator c = collection.begin();
       c != collection.end(); ++c) {
    if ((early && c->second->is_autorun_early()) ||
        (!early && c->second->is_autorun() && !c->second->is_autorun_early())) {
      return true;
    }
  }
...
static int collect_priority(lym::MacroCollection &collection, bool early,
                            int from_prio) {
  int p = -1;
  ...
  for (lym::MacroCollection::iterator c = collection.begin();
       c != collection.end(); ++c) {
    if (c->second->can_run() && ((early && c->second->is_autorun_early()) ||
                                 (!early && c->second->is_autorun() &&
                                  !c->second->is_autorun_early()))) {
...
static void autorun_for_prio(lym::MacroCollection &collection, bool early,
                             std::set<std::string> *executed_already,
                             int prio) {
  for (lym::MacroCollection::child_iterator c = collection.begin_children();
       c != collection.end_children(); ++c) {
  ...
  for (lym::MacroCollection::iterator c = collection.begin();
    ...
    if (c->second->priority() == prio && c->second->can_run() &&
        ((early && c->second->is_autorun_early()) ||
         (!early && c->second->is_autorun() &&
          !c->second->is_autorun_early()))) {

      if (!executed_already || executed_already->find(c->second->path()) ==
                                   executed_already->end()) {
