...
GDS2ReaderBase::GDS2ReaderBase()
    : m_dbu(0.001), m_dbuu(1.0), m_read_texts(true), m_read_properties(true),
      m_allow_multi_xy_records(false), m_box_mode(0) {
  // .. nothing yet ..
}
...
void GDS2ReaderBase::init(const db::LoadLayoutOptions &options) {
  ...
  db::GDS2ReaderOptions gds2_options =
      options.get_options<db::GDS2ReaderOptions>();
...
void GDS2ReaderBase::finish_element() {
  while (true) {
    ...
    if (rec_id == sENDEL) {
      ...
      //  skip this record
    } else if (rec_id == sTEXT || rec_id == sPATH || rec_id == sBOUNDARY ||
               rec_id == sBOX || rec_id == sAREF || rec_id == sSREF ||
               rec_id == sENDSTR) {
      unget_record(rec_id);
...
inline db::Point pt_conv(const GDS2XY &p) {
  //  TODO: this can be done more efficiently ..
  int x = (int(p.x[0]) << 24) | (int(p.x[1]) << 16) | (int(p.x[2]) << 8) |
          int(p.x[3]);
  int y = (int(p.y[0]) << 24) | (int(p.y[1]) << 16) | (int(p.y[2]) << 8) |
          int(p.y[3]);
  return db::Point(x, y);
}

inline db::Vector v_conv(const GDS2XY &p) { return pt_conv(p) - db::Point(); }

inline bool eq_x(const GDS2XY &a, const GDS2XY &b) {
  /// Re-cast pointer to char[4] as pointer to 32bit int for faster comparison
  return *((int *)a.x) == *((int *)b.x);
}

inline bool eq_y(const GDS2XY &a, const GDS2XY &b) {
  /// Re-cast pointer to char[4] as pointer to 32bit int for faster comparison
...
void GDS2ReaderBase::do_read(db::Layout &layout) {
  ...
  layout.add_meta_info(
      "mod_time", MetaInfo(tl::to_string(tr("Modification Time")),
                           tl::sprintf("%d/%d/%d %d:%02d:%02d", mod_time[1],
                                       mod_time[2], mod_time[0], mod_time[3],
                                       mod_time[4], mod_time[5])));
  layout.add_meta_info(
      "access_time",
      MetaInfo(tl::to_string(tr("Access Time")),
               tl::sprintf("%d/%d/%d %d:%02d:%02d", access_time[1],
                           access_time[2], access_time[0], access_time[3],
                           access_time[4], access_time[5])));
  ...
  do {
    rec_id = get_record();
    if (rec_id == sLIBDIRSIZE || rec_id == sSRFNAME || rec_id == sREFLIBS ||
        rec_id == sFONTS || rec_id == sATTRTABLE || rec_id == sGENERATIONS ||
        rec_id == sFORMAT || rec_id == sMASK || rec_id == sENDMASKS) {
      ...
      const char *value = get_string();
      if (m_read_properties) {
        layout_properties.insert(std::make_pair(
            layout.properties_repository().prop_name_id(tl::Variant(attr)),
            tl::Variant(value)));
      }
      ...
      layout.add_meta_info(
          "dbuu", MetaInfo(tl::to_string(tr("Database unit in user units")),
                           tl::to_string(dbuu)));
      layout.add_meta_info("dbum",
                           MetaInfo(tl::to_string(tr("Database unit in meter")),
                                    tl::to_string(dbum)));
      layout.add_meta_info(
          "libname", MetaInfo(tl::to_string(tr("Library name")), m_libname));
  ...
  if (!layout_properties.empty()) {
    layout.prop_id(
        layout.properties_repository().properties_id(layout_properties));
  }
  ...
  while ((rec_id = get_record()) == sBGNSTR) {
    ...
    instances_with_props.erase(instances_with_props.begin(),
                               instances_with_props.end());
    ...
    //  read this cell in a special way and store the context information
    //  separately.
    if (first_cell && m_cellname == "$$$CONTEXT_INFO$$$") {
      ...
      if (ctx != m_context_info.end()) {
        ...
        LayoutOrCellContextInfo ci = LayoutOrCellContextInfo::deserialize(
            ctx->second.begin(), ctx->second.end());

        if (ci.has_proxy_info() &&
            layout.recover_proxy_as(cell_index, ci, &layer_mapping)) {
      ...
      while ((rec_id = get_record()) != sENDSTR) {
        ...
        if (cell == 0) {
          ...
          //  ignore everything in proxy cells: these are created from the
          //  libraries or PCells.
          ...
          const char *value = get_string();
          if (m_read_properties) {
            cell_properties.insert(std::make_pair(
                layout.properties_repository().prop_name_id(tl::Variant(attr)),
                tl::Variant(value)));
          }
          ...
          if (m_box_mode == 1) {
            ...
            error(
                tl::to_string(tr("BOX record encountered (reader is configured "
                                 "to produce an error in this case)")));
          } else {
      ...
      if (!cell_properties.empty()) {
        cell->prop_id(
            layout.properties_repository().properties_id(cell_properties));
      }
    }
  ...
  while ((rec_id = get_record()) != sENDSTR) {
    ...
    if (valid_hook) {
      ...
      std::vector<std::string> &strings =
          m_context_info.insert(std::make_pair(cn, std::vector<std::string>()))
              .first->second;
      ...
      while (true) {
        ...
        if (rec_id == sENDEL) {
          ...
          error(tl::to_string(
              tr("ENDEL, PROPATTR or PROPVALUE record expected")));
        }
      }
...
void GDS2ReaderBase::read_boundary(db::Layout &layout, db::Cell &cell,
                                   bool from_box_record) {
  LDPair ld;
  short rec_id = 0;
  ...
  if (ll.first) {
    ...
    if ((xy_length == 4 ||
        ...
        ((eq_x(xy[0], xy[1]) && eq_x(xy[2], xy[3]) && eq_y(xy[1], xy[2]) &&
          eq_y(xy[0], xy[3])) ||
         (eq_x(xy[1], xy[2]) && eq_x(xy[0], xy[3]) && eq_y(xy[0], xy[1]) &&
          eq_y(xy[2], xy[3])))) {
      ...
      std::pair<bool, db::properties_id_type> pp =
          finish_element(layout.properties_repository());
      if (pp.first) {
        cell.shapes(ll.second).insert(
            db::BoxWithProperties(db::Box(p1, p2), pp.second));
      } else {
      ...
      //  Try to detect Multi-XY records. A good indication may be a huge point
      //  count.
      if (xy_length > 2000) {

        m_all_points.clear();
        m_all_points.reserve(xy_length *
                             2); // allocate some (hopefully enough) elements
        ...
        while (true) {
          ...
          if ((rec_id = get_record()) == sXY) {
            ...
            if (!m_allow_multi_xy_records) {
              error(tl::to_string(
                  tr("Multiple XY records detected on BOUNDARY element (reader "
                     "is configured not to allow this)")));
            }
          } else {
            unget_record(rec_id);
            break;
        ...
        if (!m_all_points.empty() &&
            m_all_points.back() == m_all_points.front()) {
          m_all_points.pop_back();
        }

        poly.assign_hull(m_all_points.begin(), m_all_points.end(),
                         false /*no compression*/);

      } else {

        //  remove redundant start and endpoint
        if (xy_length > 1 && eq_x(xy_data[0], xy_data[xy_length - 1]) &&
            eq_y(xy_data[0], xy_data[xy_length - 1])) {
          --xy_length;
        }

        poly.assign_hull(xy_data, xy_data + xy_length, pt_conv,
                         false /*no compression*/);
      }
      ...
      if (poly.hull().size() < 3) {
        ...
        std::pair<bool, db::properties_id_type> pp =
            finish_element(layout.properties_repository());
        if (pp.first) {
          cell.shapes(ll.second).insert(db::SimplePolygonRefWithProperties(
              db::SimplePolygonRef(poly, layout.shape_repository()),
              pp.second));
        } else {
          cell.shapes(ll.second).insert(
              db::SimplePolygonRef(poly, layout.shape_repository()));
    ...
    while ((rec_id = get_record()) == sXY) {
      ...
      if (!m_allow_multi_xy_records) {
        error(
            tl::to_string(tr("Multiple XY records detected on BOUNDARY element "
                             "(reader is configured not to allow this)")));
      }
    }
    unget_record(rec_id);
  ...
  if (ll.first) {
    ...
    if (xy_length > 2000) {
      ...
      while (true) {
        ...
        if ((rec_id = get_record()) == sXY) {
          ...
          if (!m_allow_multi_xy_records) {
            error(
                tl::to_string(tr("Multiple XY records detected on PATH element "
                                 "(reader is configured not to allow this)")));
          }
        } else {
          unget_record(rec_id);
    ...
    if (path.points() < 1) {
      ...
      if (path.points() < 2 && type != 1) {
        warn(tl::to_string(
            tr("PATH with less than two points encountered - interpretation "
               "may be different in other tools")));
      }
      std::pair<bool, db::properties_id_type> pp =
          finish_element(layout.properties_repository());
      if (pp.first) {
        cell.shapes(ll.second).insert(db::PathRefWithProperties(
            db::PathRef(path, layout.shape_repository()), pp.second));
      } else {
        cell.shapes(ll.second).insert(
            db::PathRef(path, layout.shape_repository()));
      }
    }
    ...
    while ((rec_id = get_record()) == sXY) {
      // read over multi-XY records
      if (!m_allow_multi_xy_records) {
        error(tl::to_string(tr("Multiple XY records detected on PATH element "
                               "(reader is configured not to allow this)")));
      }
    }
    unget_record(rec_id);
...
void GDS2ReaderBase::read_text(db::Layout &layout, db::Cell &cell) {
  ...
  if (rec_id == sPRESENTATION) {
    ...
    ha = db::HAlign(p & 3);
    va = db::VAlign((p >> 2) & 3);
    // HINT: currently we don't read the font since the font is not well
    // standardized .. font = (db::Font) ((p >> 4) & 0xfff);
    rec_id = get_record();
  }
  ...
  while (rec_id == sSTRANS || rec_id == sMAG || rec_id == sANGLE) {
    ...
    if (rec_id == sSTRANS) {
      ...
      if (ll.first) {
        ...
        if (a < -4 || a > 4) {
          warn(tl::sprintf(
              tl::to_string(tr("Invalid text rotation angle (%g is less than "
                               "-360 or larger than 360)")),
              aorg));
        }
        ...
        if (fabs(double(angle) - a) > 1e-9) {
          warn(tl::sprintf(
              tl::to_string(tr(
                  "Invalid text rotation angle (%g is not a multiple of 90)")),
              aorg));
        }
        while (angle < 0) {
          angle += 4;
  ...
  if (ll.first) {
    ...
    std::pair<bool, db::properties_id_type> pp =
        finish_element(layout.properties_repository());
    if (pp.first) {
      cell.shapes(ll.second).insert(db::TextRefWithProperties(
          db::TextRef(text, layout.shape_repository()), pp.second));
    } else {
      cell.shapes(ll.second).insert(
          db::TextRef(text, layout.shape_repository()));
    }
...
void GDS2ReaderBase::read_ref(
    db::Layout &layout, db::Cell & /*cell*/, bool array,
    tl::vector<db::CellInstArray> &instances,
    tl::vector<db::CellInstArrayWithProperties> &instances_with_props) {
  short rec_id = 0;
  ...
  while (rec_id == sSTRANS || rec_id == sMAG || rec_id == sANGLE) {
    ...
    if (rec_id == sSTRANS) {
      ...
      if (a < -4 || a > 4) {
        warn(tl::sprintf(tl::to_string(tr("Invalid rotation angle (%g is less "
                                          "than -360 or larger than 360)")),
                         angle_deg));
      }
      angle = int(a < 0 ? (a - 0.5) : (a + 0.5));
      if (fabs(double(angle) - a) > 1e-9) {
  ...
  if (array) {
    ...
    if (cols > 1 && (c.x() % cols != 0 || c.y() % cols != 0)) {
      warn(tl::to_string(tr("Off-grid AREF column vector - AREF will be split "
                            "into subarrays to preserve locations")));
      split_cols = true;
    }
    if (rows > 1 && (r.x() % rows != 0 || r.y() % rows != 0)) {
      warn(tl::to_string(tr("Off-grid AREF row vector - AREF will be split "
                            "into subarrays to preserve locations")));
      split_rows = true;
    }
    ...
    if (split_cols || split_rows) {
      ...
      while (ic < cols) {
        ...
        if (!split_cols) {
          ...
          do {
            ...
            p1d += cd;
            p1 += c;
          } while (ic < cols && fabs(p1d.x() - p1.x()) < 0.5 &&
                   fabs(p1d.y() - p1.y()) < 0.5);
        }
        ...
        while (ir < rows) {
          ...
          if (!split_rows) {
            ...
            do {
              ...
              p2d += rd;
              p2 += r;
            } while (ir < rows && fabs(p2d.x() - p2.x()) < 0.5 &&
                     fabs(p2d.y() - p2.y()) < 0.5);
          }
          ...
          if (is_mag || angle < 0) {
            inst = db::CellInstArray(db::CellInst(ci),
                                     db::ICplxTrans(mag, angle_deg, mirror, p),
                                     r, c, ir - ir0, ic - ic0);
          } else {
            inst =
                db::CellInstArray(db::CellInst(ci), db::Trans(angle, mirror, p),
                                  r, c, ir - ir0, ic - ic0);
          }

          if (pp.first) {
            instances_with_props.push_back(
                db::CellInstArrayWithProperties(inst, pp.second));
          } else {
      ...
      if (is_mag || angle < 0) {
        inst = db::CellInstArray(db::CellInst(ci),
                                 db::ICplxTrans(mag, angle_deg, mirror, xy), r,
                                 c, rows, cols);
      } else {
    ...
    if (is_mag || angle < 0) {
      inst = db::CellInstArray(db::CellInst(ci),
                               db::ICplxTrans(mag, angle_deg, mirror, xy));
    } else {
      inst = db::CellInstArray(db::CellInst(ci), db::Trans(angle, mirror, xy));
    }
