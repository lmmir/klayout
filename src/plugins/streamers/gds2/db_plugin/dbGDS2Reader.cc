...
GDS2Reader::GDS2Reader(tl::InputStream &s)
    : m_stream(s), m_recnum(0), m_reclen(0), m_recptr(0), mp_rec_buf(0),
      m_stored_rec(0), m_allow_big_records(true),
...
void GDS2Reader::init(const db::LoadLayoutOptions &options) {
  ...
  m_allow_big_records =
      options.get_options<db::GDS2ReaderOptions>().allow_big_records;
...
short GDS2Reader::get_record() {
  ...
  if (m_reclen >= 0x8000) {
    if (m_allow_big_records) {
      warn(tl::to_string(tr("Record length larger than 0x8000 encountered: "
                            "interpreting as unsigned")));
    } else {
      error(tl::to_string(
          tr("Record length larger than 0x8000 encountered (reader is "
             "configured not to allow such records)")));
    }
  }
  if (m_reclen % 2 == 1) {
...
inline short GDS2Reader::get_short() {
  unsigned char *b = mp_rec_buf + m_recptr;
...
inline unsigned short GDS2Reader::get_ushort() {
  unsigned char *b = mp_rec_buf + m_recptr;
  if ((m_recptr += 2) > m_reclen) {
    record_underflow_error();
  }
...
inline double GDS2Reader::get_double() {
  unsigned char *b = mp_rec_buf + m_recptr;
...
const char *GDS2Reader::get_string() {
  if (m_reclen == 0) {
    return "";
  }
...
GDS2XY *GDS2Reader::get_xy_data(unsigned int &length) {
  length = (unsigned int)(m_reclen / sizeof(GDS2XY));
  return (GDS2XY *)mp_rec_buf;
}

void GDS2Reader::progress_checkpoint() { m_progress.set(m_stream.pos()); }

std::string GDS2Reader::path() const { return m_stream.source(); }
...
void GDS2Reader::warn(const std::string &msg, int wl) {
  ...
  // TODO: compress
  tl::warn << msg << tl::to_string(tr(" (position=")) << m_stream.pos()
