 ...
 */
class DB_PLUGIN_PUBLIC GDS2ReaderException : public ReaderException {
public:
  GDS2ReaderException(const std::string &msg, size_t p, size_t n,
                      const std::string &cell)
      : ReaderException(tl::sprintf(
            tl::to_string(tr("%s (position=%ld, record number=%ld, cell=%s)")),
            msg, p, n, cell)) {}
};
 ...
 */
class DB_PLUGIN_PUBLIC GDS2Reader : public GDS2ReaderBase {
public:
  /**
...
private:
  ...
  size_t m_recnum;           //记录record的数量
  size_t m_reclen;           // record长度
  size_t m_recptr;           // 相对于mp_rec_buf的指针。
  unsigned char *mp_rec_buf; // record 数据的起始指针
  tl::string m_string_buf;   //字符串缓冲区
