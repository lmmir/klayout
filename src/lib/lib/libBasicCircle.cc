...
bool BasicCircle::can_create_from_shape(const db::Layout & /*layout*/,
                                        const db::Shape &shape,
                                        unsigned int /*layer*/) const {
  return (shape.is_polygon() || shape.is_box() || shape.is_path());
}

db::Trans BasicCircle::transformation_from_shape(const db::Layout & /*layout*/,
                                                 const db::Shape &shape,
                                                 unsigned int /*layer*/) const {
...
BasicCircle::parameters_from_shape(const db::Layout &layout,
                                   const db::Shape &shape,
                                   unsigned int layer) const {
  ...
  nm.insert(
      std::make_pair(p_actual_radius,
                     tl::Variant(0.5 * std::min(dbox.width(), dbox.height()))));
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration> BasicCircle::get_layer_declarations(
...
void BasicCircle::coerce_parameters(
...
void BasicCircle::produce(const db::Layout &layout,
...
std::string BasicCircle::get_display_name(
    const db::pcell_parameters_type &parameters) const {
