...
BasicStrokedPolygon::BasicStrokedPolygon(bool box) : m_box(box) {
  //  .. nothing yet ..
}

bool BasicStrokedPolygon::can_create_from_shape(const db::Layout & /*layout*/,
...
BasicStrokedPolygon::parameters_from_shape(const db::Layout &layout,
                                           const db::Shape &shape,
                                           unsigned int layer) const {
  db::Polygon poly;
  shape.polygon(poly);
  ...
  if (m_box) {
    nm.insert(std::make_pair(
        p_shape, tl::Variant(db::CplxTrans(layout.dbu()) * poly.box())));
  } else {
    nm.insert(std::make_pair(p_shape,
                             tl::Variant(db::CplxTrans(layout.dbu()) * poly)));
  }
  //  use 1/10 of the minimum bbox dimension as a rough initialisation of the
  //  width
  nm.insert(std::make_pair(
      p_width,
      tl::Variant(layout.dbu() *
                  (std::min(poly.box().width(), poly.box().height()) / 10))));
  nm.insert(std::make_pair(p_radius, tl::Variant(0.0)));
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration>
BasicStrokedPolygon::get_layer_declarations(
    const db::pcell_parameters_type &parameters) const {
  std::vector<db::PCellLayerDeclaration> layers;
  if (parameters.size() > p_layer &&
      parameters[p_layer].is_user<db::LayerProperties>()) {
...
void BasicStrokedPolygon::produce(const db::Layout &layout,
  ...
  if (parameters[p_shape].is_user<db::DPolygon>()) {
    ...
    shapes.back() = db::Polygon(db::DCplxTrans(1.0 / layout.dbu()) *
                                parameters[p_shape].to_user<db::DPolygon>());
  } else if (parameters[p_shape].is_user<db::DBox>()) {
    shapes.push_back(db::Polygon());
    shapes.back() = db::Polygon(db::Box(
        parameters[p_shape].to_user<db::DBox>() * (1.0 / layout.dbu())));
  }

  //  create the outer contour
  std::vector<db::Polygon> outer;
  ep.size(shapes, db::coord_traits<db::Coord>::rounded(w * 0.5),
          db::coord_traits<db::Coord>::rounded(w * 0.5), outer, 4, false);
  if (r > 0.5) {
    for (std::vector<db::Polygon>::iterator p = outer.begin(); p != outer.end();
         ++p) {
  ...
  ep.size(outer, -db::coord_traits<db::Coord>::rounded(w),
          -db::coord_traits<db::Coord>::rounded(w), inner, 4, false);

  //  subtract inner from outer
  shapes.clear();
  ep.boolean(outer, inner, shapes, db::BooleanOp::ANotB,
             true /*resolve holes*/);

  //  Produce the shapes
  for (std::vector<db::Polygon>::const_iterator p = shapes.begin();
       p != shapes.end(); ++p) {
    cell.shapes(layer_ids[p_layer]).insert(*p);
  }
}

std::string BasicStrokedPolygon::get_display_name(
    const db::pcell_parameters_type &parameters) const {
...
BasicStrokedPolygon::get_parameter_declarations() const {
  ...
  if (m_box) {
    parameters.back().set_default(
        db::DBox(db::DPoint(-0.2, -0.2), db::DPoint(0.2, 0.2)));
  } else {
    db::DPolygon p;
    db::DPoint pts[] = {db::DPoint(-0.2, -0.2), db::DPoint(0.2, -0.2),
                        db::DPoint(0.2, 0.2), db::DPoint(-0.2, 0.2)};
  ...
  parameters.back().set_description(
      tl::to_string(tr("Number of points / full circle.")));
