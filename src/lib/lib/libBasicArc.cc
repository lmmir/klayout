...
std::vector<db::PCellLayerDeclaration> BasicArc::get_layer_declarations(
...
void BasicArc::coerce_parameters(const db::Layout & /*layout*/,
  ...
  if (fabs(ru1 - r1) > 1e-6 || fabs(ru2 - r2) > 1e-6 || fabs(a1u - a1) > 1e-6 ||
      fabs(a2u - a2) > 1e-6) {
...
void BasicArc::produce(const db::Layout &layout,
  ...
  int n =
      std::max(2, int(floor(0.5 + std::max(8, parameters[p_npoints].to_int()) *
                                      (a2 - a1) / 360.0)));
  ...
  for (int i = 0; i < n; ++i) {
    ...
    points.push_back(
        db::Point(db::coord_traits<db::Coord>::rounded(rr1 * cos(a)),
                  db::coord_traits<db::Coord>::rounded(rr1 * sin(a))));
  }

  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r1 * cos(a2 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r1 * sin(a2 * M_PI / 180.0))));
  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r2 * cos(a2 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r2 * sin(a2 * M_PI / 180.0))));

  for (int i = 0; i < n; ++i) {
    double a = (n - 1 - i + 0.5) * da + M_PI * a1 / 180.0;
    points.push_back(
        db::Point(db::coord_traits<db::Coord>::rounded(rr2 * cos(a)),
                  db::coord_traits<db::Coord>::rounded(rr2 * sin(a))));
  }

  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r2 * cos(a1 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r2 * sin(a1 * M_PI / 180.0))));
  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r1 * cos(a1 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r1 * sin(a1 * M_PI / 180.0))));
...
BasicArc::get_parameter_declarations() const {
  ...
  //  This is a shadow parameter to keep the current handle position and to
  //  determine whether the handle changed
