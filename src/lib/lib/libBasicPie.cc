...
std::vector<db::PCellLayerDeclaration> BasicPie::get_layer_declarations(
...
void BasicPie::coerce_parameters(const db::Layout & /*layout*/,
...
void BasicPie::produce(const db::Layout &layout,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    points.push_back(
        db::Point(db::coord_traits<db::Coord>::rounded(rr * cos(a)),
                  db::coord_traits<db::Coord>::rounded(rr * sin(a))));
  }

  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r * cos(a2 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r * sin(a2 * M_PI / 180.0))));
  points.push_back(db::Point(0, 0));
  points.push_back(db::Point(
      db::coord_traits<db::Coord>::rounded(r * cos(a1 * M_PI / 180.0)),
      db::coord_traits<db::Coord>::rounded(r * sin(a1 * M_PI / 180.0))));
...
BasicPie::get_parameter_declarations() const {
  ...
  //  parameter #1: radius
  //  This parameter is updated by "coerce_parameters" from "actual_radius" or
  //  the handles, whichever changed.
