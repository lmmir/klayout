...
bool BasicRoundPolygon::can_create_from_shape(const db::Layout & /*layout*/,
...
BasicRoundPolygon::parameters_from_shape(const db::Layout &layout,
                                         const db::Shape &shape,
                                         unsigned int layer) const {
  db::Polygon poly;
  shape.polygon(poly);
  ...
  nm.insert(std::make_pair(p_polygon,
...
BasicRoundPolygon::get_layer_declarations(
...
void BasicRoundPolygon::produce(const db::Layout &layout,
  ...
  poly.push_back(db::complex_trans<db::DCoord, db::Coord>(1.0 / layout.dbu()) *
                 parameters[p_polygon].to_user<db::DPolygon>());
  ...
  for (std::vector<db::Polygon>::const_iterator p = merged.begin();
       p != merged.end(); ++p) {
...
std::string BasicRoundPolygon::get_display_name(
    const db::pcell_parameters_type &parameters) const {
