...
bool BasicRoundPath::can_create_from_shape(const db::Layout & /*layout*/,
...
BasicRoundPath::parameters_from_shape(const db::Layout &layout,
                                      const db::Shape &shape,
                                      unsigned int layer) const {
  db::Path path;
  shape.path(path);
  ...
  nm.insert(
      std::make_pair(p_path, tl::Variant(db::CplxTrans(layout.dbu()) * path)));
  //  use 1/10 of the minimum bbox dimension as a rough initialisation of the
  //  radius
  nm.insert(std::make_pair(
      p_radius,
      tl::Variant(layout.dbu() *
                  (std::min(path.box().width(), path.box().height()) / 10))));
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration> BasicRoundPath::get_layer_declarations(
...
void BasicRoundPath::produce(const db::Layout &layout,
  ...
  db::DPath path = db::DCplxTrans(1.0 / layout.dbu()) *
                   parameters[p_path].to_user<db::DPath>();
  ...
  poly.assign_hull(hull.begin(), hull.end(),
                   db::cast_op<db::Point, db::DPoint>());

  //  Produce the shape
  cell.shapes(layer_ids[p_layer]).insert(poly);
}

std::string BasicRoundPath::get_display_name(
    const db::pcell_parameters_type &parameters) const {
...
BasicRoundPath::get_parameter_declarations() const {
  ...
  db::DPath p;
  p.width(0.1);
  db::DPoint pts[] = {db::DPoint(0, 0), db::DPoint(0.2, 0),
                      db::DPoint(0.2, 0.2)};
