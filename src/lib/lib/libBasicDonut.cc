...
bool BasicDonut::can_create_from_shape(const db::Layout & /*layout*/,
                                       const db::Shape &shape,
                                       unsigned int /*layer*/) const {
  return (shape.is_polygon() || shape.is_box() || shape.is_path());
}

db::Trans BasicDonut::transformation_from_shape(const db::Layout & /*layout*/,
                                                const db::Shape &shape,
                                                unsigned int /*layer*/) const {
...
BasicDonut::parameters_from_shape(const db::Layout &layout,
                                  const db::Shape &shape,
                                  unsigned int layer) const {
  ...
  nm.insert(
      std::make_pair(p_actual_radius1,
                     tl::Variant(0.5 * std::min(dbox.width(), dbox.height()))));
  nm.insert(std::make_pair(
      p_actual_radius2,
      tl::Variant(0.25 * std::min(dbox.width(), dbox.height()))));
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration> BasicDonut::get_layer_declarations(
...
void BasicDonut::coerce_parameters(
...
void BasicDonut::produce(const db::Layout &layout,
  ...
  points.push_back(db::Point(db::coord_traits<db::Coord>::rounded(-r1),
                             db::coord_traits<db::Coord>::rounded(0.0)));
  for (int i = 0; i < n; ++i) {
    double a = (i + 0.5) * da;
    points.push_back(
  ...
  points.push_back(db::Point(db::coord_traits<db::Coord>::rounded(-r2),
                             db::coord_traits<db::Coord>::rounded(0.0)));
  for (int i = 0; i < n; ++i) {
...
std::string BasicDonut::get_display_name(
    const db::pcell_parameters_type &parameters) const {
