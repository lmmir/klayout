...
bool BasicEllipse::can_create_from_shape(const db::Layout & /*layout*/,
...
db::Trans
BasicEllipse::transformation_from_shape(const db::Layout & /*layout*/,
                                        const db::Shape &shape,
                                        unsigned int /*layer*/) const {
...
BasicEllipse::parameters_from_shape(const db::Layout &layout,
                                    const db::Shape &shape,
                                    unsigned int layer) const {
  ...
  nm.insert(
      std::make_pair(p_actual_radius_y, tl::Variant(0.5 * dbox.height())));
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration> BasicEllipse::get_layer_declarations(
...
void BasicEllipse::coerce_parameters(
...
void BasicEllipse::produce(const db::Layout &layout,
  ...
  for (int i = 0; i < n; ++i) {
    double a = (i + 0.5) * da;
    points.push_back(
        db::Point(db::coord_traits<db::Coord>::rounded(-rr_x * cos(a)),
                  db::coord_traits<db::Coord>::rounded(rr_y * sin(a))));
  }
...
std::string BasicEllipse::get_display_name(
    const db::pcell_parameters_type &parameters) const {
