...
bool BasicText::can_create_from_shape(const db::Layout & /*layout*/,
                                      const db::Shape &shape,
                                      unsigned int /*layer*/) const {
  return shape.is_text();
}

db::Trans BasicText::transformation_from_shape(const db::Layout & /*layout*/,
                                               const db::Shape &shape,
                                               unsigned int /*layer*/) const {
...
BasicText::parameters_from_shape(const db::Layout &layout,
                                 const db::Shape &shape,
                                 unsigned int layer) const {
  ...
  if (shape.text_size() > 0) {
    ...
    if (!db::TextGenerator::generators().empty()) {
      hfont = db::TextGenerator::generators().front().height() *
              db::TextGenerator::generators().front().dbu();
    }
    nm.insert(
        std::make_pair(p_magnification,
                       tl::Variant(layout.dbu() * shape.text_size() / hfont)));
  }
  return map_parameters(nm);
}

std::vector<db::PCellLayerDeclaration> BasicText::get_layer_declarations(
...
int BasicText::get_font_index(
  ...
  if (!fn.empty()) {
    for (std::vector<db::TextGenerator>::const_iterator i =
             db::TextGenerator::generators().begin();
         i != db::TextGenerator::generators().end(); ++i) {
      if (i->name() == fn) {
...
void BasicText::coerce_parameters(const db::Layout &layout,
...
void BasicText::produce(const db::Layout &layout,
                        const std::vector<unsigned int> &layer_ids,
                        const db::pcell_parameters_type &parameters,
                        db::Cell &cell) const {
  if (parameters.size() < 6 || layer_ids.size() < 1 ||
      db::TextGenerator::generators().empty()) {
    return;
  }
  ...
  for (std::vector<db::Polygon>::const_iterator d = data.begin();
       d != data.end(); ++d) {
...
BasicText::get_display_name(const db::pcell_parameters_type &parameters) const {
  ...
  return "TEXT(l=" + std::string(parameters[p_layer].to_string()) + ",'" + t +
         "')";
}
...
BasicText::get_parameter_declarations() const {
  ...
  for (std::vector<db::TextGenerator>::const_iterator f =
           db::TextGenerator::generators().begin();
       f != db::TextGenerator::generators().end(); ++f) {
  ...
  parameters.back().set_description(
      tl::to_string(tr("Additional character spacing")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  ...
  parameters.back().set_type(db::PCellParameterDeclaration::t_double);
  parameters.back().set_description(
      tl::to_string(tr("Additional line spacing")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  ...
  parameters.back().set_type(db::PCellParameterDeclaration::t_double);
  parameters.back().set_description(
      tl::to_string(tr("Computed parameters\tCell width")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  ...
  parameters.back().set_type(db::PCellParameterDeclaration::t_double);
  parameters.back().set_description(
      tl::to_string(tr("Computed parameters\tCell height")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  parameters.back().set_readonly(true);
  ...
  parameters.back().set_type(db::PCellParameterDeclaration::t_double);
  parameters.back().set_description(
      tl::to_string(tr("Computed parameters\tLine width")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  parameters.back().set_readonly(true);
  ...
  parameters.back().set_type(db::PCellParameterDeclaration::t_double);
  parameters.back().set_description(
      tl::to_string(tr("Computed parameters\tDesign raster")));
  parameters.back().set_default(0.0);
  parameters.back().set_unit(tl::to_string(tr("micron")));
  parameters.back().set_readonly(true);
