...
void run_test(tl::TestBase *_this, const std::string &suffix,
              const std::string &layout, bool with_l2n = false,
...
TEST(1_simple) { run_test(_this, "ringo_simple", "ringo.gds"); }
...
TEST(2_simple_io) { run_test(_this, "ringo_simple_io", "ringo.gds"); }

TEST(3_simple_io2) { run_test(_this, "ringo_simple_io2", "ringo.gds", true); }

TEST(4_simple_implicit_connections) {
  run_test(_this, "ringo_simple_implicit_connections",
...
TEST(6_simple_pin_swapping) {
  ...
  //  change case
  run_test(_this, "ringo_simple_pin_swapping", "ringo.gds", false,
           std::string(), true);
}

TEST(7_net_and_circuit_equivalence) {
  run_test(_this, "ringo_simple_net_and_circuit_equivalence",
           "ringo_renamed.gds");
  //  change case
  run_test(_this, "ringo_simple_net_and_circuit_equivalence",
           "ringo_renamed.gds", false, std::string(), true);
}

TEST(8_simplification) {
  run_test(_this, "ringo_simple_simplification",
...
TEST(9b_blackboxing_netter) {
  run_test(_this, "ringo_simple_blackboxing_netter",
           "ringo_for_blackboxing.gds");
}

TEST(10_simplification_with_align) {
  run_test(_this, "ringo_simple_simplification_with_align",
           "ringo_for_simplification.gds");
}
...
TEST(13_simple_ringo_device_subcircuits) {
  ...
  //  change case
  run_test(_this, "ringo_device_subcircuits", "ringo.gds", false, std::string(),
           true);
}
...
TEST(18_cheats) { run_test(_this, "invchain_cheat", "invchain_for_cheat.gds"); }

//  testing cell specific net joining for VSS of the double-height inverter
//  standard cell
...
//  empty gds
TEST(23_issue709) { run_test(_this, "empty_subcells", "empty_subcells.gds"); }

TEST(24_issue806) { run_test(_this, "custom_compare", "custom_compare.gds"); }
