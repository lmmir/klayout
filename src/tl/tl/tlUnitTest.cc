...
bool verbose() { return s_verbose_flag; }

void set_verbose(bool f) { s_verbose_flag = f; }

void set_indent(int i) { s_indent = i; }

int indent() { return s_indent; }

bool xml_format() { return s_xml_format; }

void set_xml_format(bool f) { s_xml_format = f; }

void set_continue_flag(bool f) { s_continue_flag = f; }

bool is_debug_mode() { return s_debug_mode; }

void set_debug_mode(bool f) { s_debug_mode = f; }
...
std::string testdata() { return tl::combine_path(tl::testsrc(), "testdata"); }
...
bool equals(double a, double b) {
  ...
  if (m < 1e-30) {
    ...
    double d = fabs(a - b);
    //  we consider two values equal for the purpose of unit tests if they have
    //  the same value within 1e-10 (0.00000001%).
    return d < 1e-10 * m;
  }
}
...
CaptureChannel::CaptureChannel() {
  ...
  //  Because we don't want to capture logger messages, we switch verbosity to
  //  "silent" during capturing
  m_saved_verbosity = tl::verbosity();
  tl::verbosity(0);
}

CaptureChannel::~CaptureChannel() { tl::verbosity(m_saved_verbosity); }

void CaptureChannel::puts(const char *s) { m_text << s; }

void CaptureChannel::endl() { m_text << "\n"; }
...
TestRegistrar::TestRegistrar() : m_tests() {
  //  .. nothing yet ..
}
...
TestRegistrar *TestRegistrar::instance() { return ms_instance; }

const std::vector<tl::TestBase *> &TestRegistrar::tests() const {
  return m_tests;
}
...
void TestBase::raise(const std::string &file, int line,
...
void TestBase::write_detailed_diff(std::ostream &os, const std::string &subject,
                                   const std::string &ref) {
  os << replicate(" ", tl::indent())
     << "Actual value is:    " << tl::to_string(subject) << std::endl
     << replicate(" ", tl::indent())
     << "Reference value is: " << tl::to_string(ref) << std::endl;
}
...
void TestBase::compare_text_files(const std::string &path_a,
  ...
  if (!equal) {
    throw tl::Exception(tl::sprintf(
        "Compare failed - see\n  actual: %s\n  golden: %s%s",
        tl::absolute_file_path(path_a), tl::absolute_file_path(path_b),
        (n > 1 ? "\nand variants" : "")));
  }
}
