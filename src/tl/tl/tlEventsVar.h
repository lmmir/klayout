...
public:
  event_function(void (T::*m)(_TMPLARGLIST)) : m_m(m) {
    //  .. nothing yet ..
  }
  ...
  virtual bool equals(const event_function_base<_TMPLARGLISTP> &other) {
    const event_function<_JOIN(T, _TMPLARGLIST)> *o =
        dynamic_cast<const event_function<_JOIN(T, _TMPLARGLIST)> *>(&other);
    return o && o->m_m == m_m;
  }
...
public:
  ...
  virtual bool equals(const event_function_base<_TMPLARGLISTP> &other) {
    const event_function_with_data<T, _JOIN(D, _TMPLARGLIST)> *o = dynamic_cast<
        const event_function_with_data<T, _JOIN(D, _TMPLARGLIST)> *>(&other);
...
class TL_PUBLIC_TEMPLATE generic_event_function<T, _TMPLARGLISTP>
    : public event_function_base<_TMPLARGLIST> {
public:
  generic_event_function(void (T::*m)(int, void **)) : m_m(m) {
    //  .. nothing yet ..
  }
  ...
  virtual bool equals(const event_function_base<_TMPLARGLISTP> &other) {
    const generic_event_function<_JOIN(T, _TMPLARGLIST)> *o =
        dynamic_cast<const generic_event_function<_JOIN(T, _TMPLARGLIST)> *>(
            &other);
    return o && o->m_m == m_m;
  }
...
public:
  ...
  virtual bool equals(const event_function_base<_TMPLARGLISTP> &other) {
    const generic_event_function_with_data<T, _JOIN(D, _TMPLARGLIST)> *o =
        dynamic_cast<const generic_event_function_with_data<
            T, _JOIN(D, _TMPLARGLIST)> *>(&other);
    return o && o->m_m == m_m && o->m_d == m_d;
  }
...
template <_TMPLARGS> class TL_PUBLIC_TEMPLATE event<_TMPLARGLISTP> {
public:
  typedef event_function_base<_TMPLARGLISTP> func;
  typedef std::vector<std::pair<tl::weak_ptr<tl::Object>, tl::shared_ptr<func>>>
      receivers;
#if _COUNT == 0
  //  NOTE: for gcc 4.4.7 (and may others), event is not a template in this case
  //  and typename must not be given.
  ...
  void operator()(_CALLARGLIST) {
    //  Issue the events. Because inside the call, other receivers might be
    //  added, we make a copy first. This way added events won't be called now.
    receivers tmp_receivers = m_receivers;
    for (receivers_iterator r = tmp_receivers.begin(); r != tmp_receivers.end();
         ++r) {
      if (r->first.get()) {
        try {
    ...
    for (receivers_iterator r = m_receivers.begin(); r != m_receivers.end();
         ++r) {
      if (r->first.get()) {
        if (w != r) {
          *w = *r;
  ...
  void clear() { m_receivers.clear(); }

  template <class T> T *find_receiver() {
    for (receivers_iterator r = m_receivers.begin(); r != m_receivers.end();
         ++r) {
  ...
  template <class T> void add(T *obj, void (T::*m)(_TMPLARGLIST)) {
  ...
  template <class T> void remove(T *obj, void (T::*m)(_TMPLARGLIST)) {
    event_function<T, _TMPLARGLISTP> f(m);
    for (receivers_iterator r = m_receivers.begin(); r != m_receivers.end();
         ++r) {
      if (r->first.get() == obj && r->second->equals(f)) {
  ...
  void add(T *obj, void (T::*m)(_JOIN(D, _TMPLARGLIST)), D d) {
    ...
    m_receivers.push_back(typename receivers::value_type());
    m_receivers.back().first.reset(obj, true /*is an event*/);
    m_receivers.back().second.reset(
        new event_function_with_data<T, D, _TMPLARGLISTP>(f));
  }
  ...
  template <class T> void add(T *obj, void (T::*m)(int, void **)) {
    ...
    m_receivers.back().first.reset(obj, true /*is an event*/);
    m_receivers.back().second.reset(
        new generic_event_function<T, _TMPLARGLISTP>(f));
  }

  template <class T> void remove(T *obj, void (T::*m)(int, void **)) {
    generic_event_function<T, _TMPLARGLISTP> f(m);
    for (receivers_iterator r = m_receivers.begin(); r != m_receivers.end();
         ++r) {
      if (r->first.get() == obj && r->second->equals(f)) {
  ...
  void add(T *obj, void (T::*m)(D, int, void **), D d) {
    ...
    m_receivers.back().first.reset(obj, true /*is an event*/);
    m_receivers.back().second.reset(
        new generic_event_function_with_data<T, D, _TMPLARGLISTP>(f));
  }
