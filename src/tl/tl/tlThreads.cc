...
public:
  WaitConditionPrivate() : m_initialized(false) {
...
WaitCondition::WaitCondition() { mp_data = new WaitConditionPrivate(); }
...
void WaitCondition::wakeAll() { mp_data->wake_all(); }

void WaitCondition::wakeOne() { mp_data->wake_one(); }
...
Thread::Thread() : mp_data(new ThreadPrivateData()) {
  //  .. nothing yet ..
}
...
void Thread::start() {
  ...
  if (pthread_create(&mp_data->pthread, NULL, &start_thread, (void *)this) !=
      0) {
...
bool Thread::wait(unsigned long time) {
  ...
  if (time < std::numeric_limits<unsigned long>::max()) {
    ...
    while (isRunning()) {
      ...
      if (end_time.tv_sec < current_time.tv_sec ||
          (end_time.tv_sec == current_time.tv_sec &&
           end_time.tv_nsec < current_time.tv_nsec)) {
        return false;
      }
    ...
    int res = pthread_timedjoin_np(mp_data->pthread, &mp_data->return_code,
                                   &end_time);
    if (res == ETIMEDOUT) {
      return false;
    } else if (res) {
...
public:
  ...
  ~ThreadStorageObjectList() {
    for (std::map<void *, ThreadStorageHolderBase *>::iterator i =
             m_objects.begin();
         i != m_objects.end(); ++i) {
      delete i->second;
    }
  ...
  void add(void *index, ThreadStorageHolderBase *holder) {
    std::map<void *, ThreadStorageHolderBase *>::iterator h =
        m_objects.find(holder);
    if (h != m_objects.end()) {
      delete h->second;
      h->second = holder;
    } else {
  ...
  ThreadStorageHolderBase *get(void *index) {
    std::map<void *, ThreadStorageHolderBase *>::const_iterator h =
        m_objects.find(index);
...
static void free_key(void *data) { delete (ThreadStorageObjectList *)data; }

static void create_key() { pthread_key_create(&s_storage_key, free_key); }
...
void ThreadStorageBase::add(ThreadStorageHolderBase *holder) {
  ...
  ((ThreadStorageObjectList *)pthread_getspecific(s_storage_key))
      ->add((void *)this, holder);
}
...
ThreadStorageHolderBase *ThreadStorageBase::holder() {
  ...
  if (!pthread_getspecific(s_storage_key)) {
    ...
    return ((ThreadStorageObjectList *)pthread_getspecific(s_storage_key))
        ->get((void *)this);
  }
}
