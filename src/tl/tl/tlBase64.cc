  ...
  for (const char *t = s; *t; ++t) {
    ...
    if ((unsigned char)*t <= ' ') {
      ...
      if (data.empty() || data.back() != 0) {
        throw tl::Exception(
            tl::to_string(tr("Error decoding base64 data: padding character "
                             "does not match zero byte")));
      }
      data.pop_back();
      break;
      ...
      if (b >= 64) {
        throw tl::Exception(
            tl::to_string(
                tr("Error decoding base64 data: invalid character '%c'")),
            *t);
      }
...
std::string to_base64(const unsigned char *data, size_t size) {
  ...
  for (size_t b = 0; b < bits; b += 6) {
    ...
    if (bit <= 2) {
      ...
      s += s_enc.c(
          ((data[byte] << (bit - 2)) | (data[byte + 1] >> (10 - bit))) & 0x3f);
    } else {
