...
ExtractorNotImplementedException::ExtractorNotImplementedException(
    ...
    : Exception(tl::to_string(tr("No string extractor available for type: ")) +
                ti.name()) {
  //  .. nothing yet ..
}

StringConversionException::StringConversionException(const std::type_info &ti)
    : Exception(tl::to_string(tr("No string conversion available for type: ")) +
                ti.name()) {
  //  .. nothing yet ..
}
...
uint32_t utf32_from_utf8(const char *&cp, const char *cpe) {
  ...
  if (c32 >= 0xf0 &&
      ((cpe && cp + 2 < cpe) || (!cpe && cp[0] && cp[1] && cp[2]))) {
    c32 = ((c32 & 0x7) << 18) | ((uint32_t(cp[0]) & 0x3f) << 12) |
          ((uint32_t(cp[1]) & 0x3f) << 6) | (uint32_t(cp[2]) & 0x3f);
    cp += 3;
  } else if (c32 >= 0xe0 &&
             ((cpe && cp + 1 < cpe) || (!cpe && cp[0] && cp[1]))) {
    c32 = ((c32 & 0xf) << 12) | ((uint32_t(cp[0]) & 0x3f) << 6) |
          (uint32_t(cp[1]) & 0x3f);
    cp += 2;
...
// -------------------------------------------------------------------------
//  safe versions (assertion-less) of safe_isdigit, safe_isprint, safe_isalpha,
//  safe_isalnum (required for debug mode of MSVC)
...
std::string to_local(const std::string &s) {
  std::unique_ptr<char[]> buffer(
      new char[MB_CUR_MAX]); //  MB_CUR_MAX isn't a constant
  std::string ls;
...
std::string to_string(double d, int prec) {
  //  For small values less than 1e-(prec) simply return "0" to avoid ugly
  //  values like "1.2321716e-14".
...
std::string to_string(float d, int prec) {
  //  For small values less than 1e-(prec) simply return "0" to avoid ugly
  //  values like "1.2321716e-14".
  if (fabs(d) < pow(10.0, -prec)) {
    return "0";
...
template <> std::string to_string(const int &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const unsigned int &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const long &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const long long &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const unsigned long &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const unsigned long long &d) {
  std::ostringstream os;
  os.imbue(c_locale);
  os << d;
  return os.str();
...
template <> std::string to_string(const __int128 &d) {
  if (d < 0) {
...
template <> std::string to_string(const unsigned __int128 &_x) {
  std::string r;
...
template <> std::string to_string(char *const &cp) { return std::string(cp); }

template <> std::string to_string(const char *const &cp) {
  return std::string(cp);
}

template <> std::string to_string(unsigned char *const &cp) {
  return std::string((const char *)cp);
}

template <> std::string to_string(const unsigned char *const &cp) {
  return std::string((const char *)cp);
}

std::string to_string(const char *cp, int length) {
  return std::string(cp, length);
}

std::string to_string(const unsigned char *cp, int length) {
  return std::string((const char *)cp, length);
}

template <> std::string to_string(const bool &b) {
  return b ? "true" : "false";
}
...
std::string to_quoted_string(const std::string &s) {
  std::string r;
  r.reserve(s.size() + 2);
  r += '\'';
...
std::string to_word_or_quoted_string(const std::string &s,
                                     const char *non_term) {
...
void escape_to_html(std::string &out, const std::string &in,
...
std::string escaped_to_html(const std::string &in, bool replace_newlines) {
  std::string s;
  escape_to_html(s, in, replace_newlines);
  return s;
}

std::string replicate(const std::string &s, unsigned int n) {
  if (n == 0) {
    return std::string();
  }
...
std::string pad_string_right(unsigned int columns, const std::string &text) {
  std::string s = text;
  s.reserve(columns);
...
std::string pad_string_left(unsigned int columns, const std::string &text) {
  std::string s;
  s.reserve(columns);
...
std::string replaced(const std::string &subject, const std::string &before,
...
void from_string(const std::string &s, const char *&result) {
  result = s.c_str();
}

void from_string(const std::string &s, const unsigned char *&result) {
  result = (unsigned char *)s.c_str();
}

static void from_string_numeric(const std::string &s, double &v, bool eval) {
  const char *cp = s.c_str();
  ...
  if (!*cp) {
    throw tl::Exception(
        tl::to_string(tr("Got empty string where a real number was expected")));
  }
  ...
  if (*cp_end) {
    if (eval) {
      //  try using an expression (using a clean environment disables all global
      //  features and leaves only some static functions)
      v = tl::Eval(0, 0, false).parse(s).execute().to_double();
    } else {
      throw tl::Exception(
          tl::to_string(tr("Unexpected text after numeric value: '...")) +
          cp_end + "'");
    }
  }
}
...
template <class T>
static void convert_string_to_int(const std::string &s, T &v, bool eval) {
  double x;
  ...
  if (x != v) {
    throw tl::Exception(
        tl::to_string(tr("Number cannot be represented precisely: ")) + s);
  }
}

void from_string(const std::string &s, double &v) {
  return from_string_numeric(s, v, false);
}

void from_string(const std::string &s, int &v) {
  convert_string_to_int(s, v, false);
}

void from_string(const std::string &s, long &v) {
  convert_string_to_int(s, v, false);
}

void from_string(const std::string &s, long long &v) {
  convert_string_to_int(s, v, false);
}

void from_string(const std::string &s, unsigned int &v) {
  convert_string_to_int(s, v, false);
}

void from_string(const std::string &s, unsigned long &v) {
  convert_string_to_int(s, v, false);
}

void from_string(const std::string &s, unsigned long long &v) {
  convert_string_to_int(s, v, false);
}

void from_string_ext(const std::string &s, double &v) {
  return from_string_numeric(s, v, true);
}

void from_string_ext(const std::string &s, int &v) {
  convert_string_to_int(s, v, true);
}

void from_string_ext(const std::string &s, long &v) {
  convert_string_to_int(s, v, true);
}

void from_string_ext(const std::string &s, long long &v) {
  convert_string_to_int(s, v, true);
}

void from_string_ext(const std::string &s, unsigned int &v) {
  convert_string_to_int(s, v, true);
}

void from_string_ext(const std::string &s, unsigned long &v) {
  convert_string_to_int(s, v, true);
}

void from_string_ext(const std::string &s, unsigned long long &v) {
  convert_string_to_int(s, v, true);
}

void from_string(const std::string &s, bool &b) {
...
std::vector<std::string> split(const std::string &t, const std::string &s) {
...
Extractor::Extractor(const char *s) : m_cp(s) {
  //  .. nothing yet ..
}

Extractor::Extractor(const std::string &str) : m_str(str) {
  m_cp = m_str.c_str();
}

Extractor &Extractor::read(unsigned int &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(unsigned char &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(unsigned long &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(unsigned long long &value) {
  if (!try_read(value)) {
    error(tl::to_string(tr("Expected an unsigned long integer value")));
  }
  return *this;
...
Extractor &Extractor::read(double &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(float &value) {
  if (!try_read(value)) {
    error(tl::to_string(tr("Expected a real number")));
  }
  return *this;
...
Extractor &Extractor::read(int &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(long &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(long long &value) {
  if (!try_read(value)) {
    error(tl::to_string(tr("Expected a long integer value")));
  }
  return *this;
...
Extractor &Extractor::read(bool &value) {
  if (!try_read(value)) {
...
Extractor &Extractor::read(std::string &value, const char *term) {
...
Extractor &Extractor::read_word(std::string &value, const char *non_term) {
...
Extractor &Extractor::read_name(std::string &value, const char *non_term) {
...
Extractor &Extractor::read_word_or_quoted(std::string &value,
                                          const char *non_term) {
...
Extractor &Extractor::read_quoted(std::string &value) {
...
template <class T> bool Extractor::try_read_signed_int(T &value) {
  if (!*skip()) {
    return false;
  }
...
template <class T> bool Extractor::try_read_unsigned_int(T &value) {
  if (!*skip()) {
    return false;
  }
...
bool Extractor::try_read(unsigned char &value) {
  return try_read_unsigned_int(value);
}

bool Extractor::try_read(unsigned int &value) {
  return try_read_unsigned_int(value);
}

bool Extractor::try_read(unsigned long &value) {
  return try_read_unsigned_int(value);
}

bool Extractor::try_read(unsigned long long &value) {
  return try_read_unsigned_int(value);
}

bool Extractor::try_read(int &value) { return try_read_signed_int(value); }

bool Extractor::try_read(long &value) { return try_read_signed_int(value); }

bool Extractor::try_read(long long &value) {
  return try_read_signed_int(value);
}

bool Extractor::try_read(float &value) {
  double d = value;
  if (try_read(d)) {
    value = d;
    return true;
...
bool Extractor::try_read(double &value) {
  if (!*skip()) {
    return false;
  }
...
bool Extractor::try_read(bool &value) {
...
bool Extractor::try_read_name(std::string &string, const char *non_term) {
  if (!*skip()) {
    return false;
  }
...
bool Extractor::try_read_word(std::string &string, const char *non_term) {
  if (!*skip()) {
    return false;
  }
...
bool Extractor::try_read_word_or_quoted(std::string &string,
                                        const char *non_term) {
  return try_read_word(string, non_term) || try_read_quoted(string);
}

bool Extractor::try_read_quoted(std::string &string) {
  char q = *skip();
...
bool Extractor::try_read(std::string &string, const char *term) {
  //  if the terminating characters contain line feed for blank, we must not
  //  skip over them
  if (strchr(term, '\n') || strchr(term, ' ')) {
  ...
  string.clear();
  while (*m_cp && (term_is_space || !safe_isspace(*m_cp)) &&
         strchr(term, *m_cp) == NULL) {
    string += *m_cp;
    ++m_cp;
  }
  return true;
...
Extractor &Extractor::expect_end() {
  if (!at_end()) {
...
Extractor &Extractor::expect_more() {
  if (at_end()) {
...
Extractor &Extractor::expect(const char *token) {
  if (!test(token)) {
...
bool Extractor::test(const char *token) {
  skip();
...
bool Extractor::test_without_case(const char *token) {
  skip();
...
void Extractor::error(const std::string &msg) {
  std::string m(msg);
...
void string::assign(const char *c, size_t from, size_t to) {
  m_size = to - from;
  if (m_size > 0) {
...
void string::assign(const tl::string &s, size_t from, size_t to) {
  if (&s != this) {
...
void string::assign(const std::string &s, size_t from, size_t to) {
  assign(s.c_str(), from, to);
}
...
bool string::operator==(const tl::string &s) const {
...
bool string::operator<(const char *c) const { return strcmp(c_str(), c) < 0; }
 ...
 *  To work around that problem we first create one digit too much and delete
 * the trailing '0' in front of 'E' or 'e'.
 */
...
std::string sprintf(const char *f, const std::vector<tl::Variant> &vv,
                    unsigned int a0) {
  std::ostringstream os;
  os.imbue(c_locale);
  ...
  for (const char *cp = f; *cp;) {
    ...
    if (*cp == '%' && cp[1] == '%') {
      ...
      if (*cp == 'c' || *cp == 'C') {
        ...
        if (a < vv.size()) {
#if defined(_STLPORT_VERSION) && _STLPORT_VERSION == 0x521 && defined(_MSC_VER)
          os << format_sci_stlport_fix(vv[a].to_double(), os.precision(),
                                       os.flags())
                    .c_str();
#else
          os << vv[a].to_double();
...
std::string sprintf(const std::string &f, const std::vector<tl::Variant> &vv,
                    unsigned int a0) {
