...
template <class X> class stable_vector {
public:
  ...
  public:
    ...
    const_iterator(const const_iterator &d) : m_iter(d.m_iter) {
      //  .. nothing yet ..
    }

    const_iterator(const iterator &d) : m_iter(d.m_iter) {
      //  .. nothing yet ..
    }

    const_iterator(typename std::vector<X *>::const_iterator d) : m_iter(d) {
      //  .. nothing yet ..
    }
    ...
    bool operator==(const_iterator d) const { return m_iter == d.m_iter; }

    bool operator!=(const_iterator d) const { return m_iter != d.m_iter; }

    bool operator<(const_iterator d) const { return m_iter < d.m_iter; }

    ptrdiff_t operator-(const_iterator d) const { return m_iter - d.m_iter; }

    const_iterator operator-(ptrdiff_t n) const { return m_iter - n; }
    ...
    const X &operator[](ptrdiff_t n) const { return *m_iter[n]; }

    const X &operator*() const { return **m_iter; }
  ...
  public:
    ...
    iterator(const iterator &d) : m_iter(d.m_iter) {
      //  .. nothing yet ..
    }

    iterator(typename std::vector<X *>::iterator d) : m_iter(d) {
      //  .. nothing yet ..
    }
    ...
    bool operator==(iterator d) const { return m_iter == d.m_iter; }

    bool operator!=(iterator d) const { return m_iter != d.m_iter; }

    bool operator<(iterator d) const { return m_iter < d.m_iter; }

    ptrdiff_t operator-(iterator d) const { return m_iter - d.m_iter; }

    iterator operator-(ptrdiff_t n) const { return m_iter - n; }
    ...
    X &operator[](ptrdiff_t n) const { return *m_iter[n]; }

    X &operator*() const { return **m_iter; }
  ...
  public:
    ...
    stable_const_iterator() : mp_vector(0), m_index(0) {
      //  .. nothing yet ..
    }
    ...
    bool operator!=(stable_const_iterator d) const { return !operator==(d); }
    ...
    const X &operator*() const { return *(*mp_vector)[m_index]; }
  ...
  public:
    ...
    stable_iterator() : mp_vector(0), m_index(0) {
      //  .. nothing yet ..
    }
    ...
    bool operator!=(stable_iterator d) const { return !operator==(d); }

    bool operator<(stable_iterator d) const { return m_index < d.m_index; }

    ptrdiff_t operator-(stable_iterator d) const { return m_index - d.m_index; }
    ...
    X &operator[](ptrdiff_t n) const { return *(*mp_vector)[m_index + n]; }

    X &operator*() const { return *(*mp_vector)[m_index]; }
  ...
  stable_vector() : m_objects() {
    //  .. nothing yet ..
  }

  stable_vector(const stable_vector<X> &d) : m_objects() { operator=(d); }

  ~stable_vector() { delete_objects(); }
  ...
  stable_vector &operator=(const stable_vector<X> &d) {
    if (&d != this) {
      ...
      for (typename std::vector<X *>::const_iterator c = d.m_objects.begin();
           c != d.m_objects.end(); ++c) {
  ...
  bool operator==(const stable_vector<X> &d) const {
    ...
    for (typename std::vector<X *>::const_iterator a = m_objects.begin(),
                                                   b = d.m_objects.begin();
         a != m_objects.end(); ++a, ++b) {
      if (**a != **b) {
        return false;
  ...
  bool operator!=(const stable_vector<X> &d) const { return !operator==(d); }
  ...
  void clear() { delete_objects(); }

  bool empty() const { return m_objects.empty(); }

  void swap(stable_vector<X> &d) { m_objects.swap(d.m_objects); }

  void push_back(const X &o) { m_objects.push_back(new X(o)); }

  void push_back(X *o) { m_objects.push_back(o); }

  const X &back() const { return *m_objects.back(); }

  const X &front() const { return *m_objects.front(); }

  X &back() { return *m_objects.back(); }

  X &front() { return *m_objects.front(); }
  ...
  void erase(iterator from, iterator to) {
    typename std::vector<X *>::iterator p =
        m_objects.begin() + (from - begin());
  ...
  X &operator[](size_t i) { return *m_objects[i]; }

  const X &operator[](size_t i) const { return *m_objects[i]; }

  stable_iterator begin_stable() { return stable_iterator(m_objects, 0); }

  stable_const_iterator begin_stable() const {
    return stable_const_iterator(m_objects, 0);
  }

  stable_iterator end_stable() { return stable_iterator(m_objects, size()); }
  ...
  const_iterator begin() const { return const_iterator(m_objects.begin()); }

  iterator end() { return iterator(m_objects.end()); }

  const_iterator end() const { return const_iterator(m_objects.end()); }
...
private:
  ...
  void delete_objects() {
    for (typename std::vector<X *>::iterator c = m_objects.begin();
         c != m_objects.end(); ++c) {
      delete *c;
    }
    m_objects.clear();
