...
class GlobPatternOp : public GlobPatternOpBase {
public:
  GlobPatternOp() : m_next_owned(false), mp_next(0) {}

  virtual ~GlobPatternOp() { set_next(0, false); }
  ...
  virtual bool is_catchall() const { return false; }
  ...
  GlobPatternOpBase *next() { return mp_next; }

  const GlobPatternOpBase *next() const { return mp_next; }
...
class GlobPatternString : public GlobPatternOp {
public:
  ...
  virtual bool is_const() const { return next() == 0; }
...
class GlobPatternEmpty : public GlobPatternOp {
public:
  GlobPatternEmpty() : GlobPatternOp() {
    //  .. nothing yet ..
  }
...
class GlobPatternPass : public GlobPatternOp {
public:
  GlobPatternPass() : GlobPatternOp() {
    //  .. nothing yet ..
  }
  ...
  virtual bool is_catchall() const { return true; }
...
class GlobPatternAny : public GlobPatternOp {
public:
...
class GlobPatternCharClass : public GlobPatternOp {
public:
  GlobPatternCharClass(bool negate, bool cs) : m_negate(negate), m_cs(cs) {
    //  .. nothing yet ..
  }

  GlobPatternCharClass(
      const std::vector<std::pair<uint32_t, uint32_t>> &intervals, bool negate,
      bool cs)
  ...
  void add_interval(uint32_t c1, uint32_t c2) {
    if (m_cs) {
      ...
      m_intervals.push_back(
          std::make_pair(utf32_downcase(c1), utf32_downcase(c2)));
    }
  }

  virtual GlobPatternOp *clone() const {
    GlobPatternCharClass *op =
        new GlobPatternCharClass(m_intervals, m_negate, m_cs);
    init_clone(op);
    return op;
  }
  ...
  virtual bool match(const char *s, std::vector<std::string> *e) const {
    ...
    for (std::vector<std::pair<uint32_t, uint32_t>>::const_iterator i =
             m_intervals.begin();
         i != m_intervals.end(); ++i) {
...
template <class T> class GlobPatternContinuator : public GlobPatternOpBase {
public:
  GlobPatternContinuator(T *br) : mp_br(br) {
    //  .. nothing yet ..
  }
...
class GlobPatternBranch : public GlobPatternOp {
public:
  GlobPatternBranch() : GlobPatternOp(), m_cont(this) {
    //  .. nothing yet ..
  }

  ~GlobPatternBranch() {
    for (std::vector<GlobPatternOp *>::const_iterator i = m_choices.begin();
         i != m_choices.end(); ++i) {
      delete *i;
    }
    m_choices.clear();
  ...
  virtual bool continue_match(const char *s,
...
class GlobPatternBracket : public GlobPatternOp {
public:
  GlobPatternBracket()
...
private:
  ...
  //  NOTE: this isn't thread-safe unless GlobPattern objects live in different
  //  threads
...
static GlobPatternOp *compile(const char *&p, bool exact, bool cs, bool hm,
                              bool for_brace);

void compile_emit_op(GlobPatternOp *&op_head, GlobPatternOp *&op,
                     GlobPatternOp *no) {
  if (op) {
    op->set_next(no, true);
  } else {
...
void compile_emit_string(std::string &str, GlobPatternOp *&op_head,
                         GlobPatternOp *&op, bool cs) {
  if (!str.empty()) {
...
void compile_emit_char_class(GlobPatternOp *&op_head, GlobPatternOp *&op,
                             const char *&p, bool cs) {
  bool negate = false;
  if (*p && *p == '^') {
    ++p;
    negate = true;
...
void compile_emit_alt(GlobPatternOp *&op_head, GlobPatternOp *&op,
                      const char *&p, bool cs) {
...
void compile_emit_bracket(GlobPatternOp *&op_head, GlobPatternOp *&op,
                          const char *&p, bool cs) {
  ...
  while (*p) {
    ...
    if (exact) {
      ...
      if (p[1]) {
        compile_emit_op(
            op_head, op,
            new GlobPatternAny(0, std::numeric_limits<size_t>::max()));
      } else {
...
GlobPattern &GlobPattern::operator=(const GlobPattern &other) {
  if (this != &other) {
...
bool GlobPattern::case_sensitive() const { return m_case_sensitive; }
...
bool GlobPattern::exact() const { return m_exact; }
...
bool GlobPattern::header_match() const { return m_header_match; }
...
bool GlobPattern::is_catchall() const { return op()->is_catchall(); }

bool GlobPattern::is_const() const { return op()->is_const(); }

bool GlobPattern::match(const char *s) const { return op()->match(s, 0); }
...
bool GlobPattern::match(const std::string &s,
                        std::vector<std::string> &e) const {
  if (!e.empty()) {
    e.clear();
  }
