...
public:
  ...
  XMLException(const std::string &msg)
      : Exception(fmt(-1, -1).c_str(), msg.c_str()), m_msg(msg) {
    //  .. nothing yet ..
  }
   ...
   */
  const std::string &raw_msg() const { return m_msg; }
...
/**
 *  @brief A XML parser error exception class that additionally provides line
...
public:
  XMLLocatedException(const std::string &msg, int line, int column)
      : XMLException(msg, line, column), m_line(line), m_column(column) {
    //  .. nothing yet ..
  }
...
/**
 *  @brief An object wrapper base class for target object management specialized
 * to a certain class
 */

template <class Obj>
class TL_PUBLIC_TEMPLATE XMLReaderProxy : public XMLReaderProxyBase {
public:
  XMLReaderProxy(Obj *obj, bool owns_obj) : mp_obj(obj), m_owns_obj(owns_obj) {}
  ...
  virtual void detach() { m_owns_obj = false; }

  Obj *ptr() const { return mp_obj; }
...
template <class Obj> struct XMLObjTag {
  XMLObjTag() {}
  typedef Obj obj;
};
...
public:
   ...
   */
  template <class Obj> void push(XMLObjTag<Obj> /*tag*/) {
   ...
   */
  template <class Obj> void push(Obj *obj) {
   ...
   */
  template <class Obj> void push(Obj *obj, bool owner) {
   ...
   */
  template <class Obj> Obj *back(XMLObjTag<Obj> /*tag*/) {
   ...
   */
  template <class Obj> Obj *detach_back(XMLObjTag<Obj> /*tag*/) {
    tl_assert(!m_objects.empty());
   ...
   */
  template <class Obj> void pop(XMLObjTag<Obj> /*tag*/) {
    tl_assert(!m_objects.empty());
   ...
   */
  template <class Obj> Obj *parent(XMLObjTag<Obj> /*tag*/) {
...
public:
  ...
  XMLSourcePrivateData *source() { return mp_source; }

  void reset();

protected:
  void set_source(XMLSourcePrivateData *source) { mp_source = source; }
...
public:
  ...
  void end_element(const std::string &uri, const std::string &lname,
                   const std::string &qname);
  void start_element(const std::string &uri, const std::string &lname,
                     const std::string &qname);
...
public:
  ...
  XMLElementBase *operator->() const { return mp_ptr; }

  XMLElementBase *get() const { return mp_ptr; }
...
public:
  ...
  static XMLElementList empty() { return XMLElementList(); }
...
public:
   ...
   *  @brief Push a new object on the stack
   */
  template <class Obj> void push(const Obj *obj) { m_objects.push_back(obj); }

  /**
   *  @brief Pop an object from the stack
   */
  template <class Obj> const Obj *pop(XMLObjTag<Obj> /*tag*/) {
    tl_assert(!m_objects.empty());
   ...
   *  @brief Obtain the parent object from the stack
   */
  template <class Obj> const Obj *back(XMLObjTag<Obj> /*tag*/) {
...
public:
  ...
  XMLElementBase(const std::string &name, const XMLElementList &children)
      : m_name(name), mp_children(new XMLElementList(children)),
        m_owns_child_list(true) {
    // .. nothing yet ..
  }
  ...
  virtual void create(const XMLElementBase *parent, XMLReaderState &objs,
                      const std::string &uri, const std::string &lname,
                      const std::string &qname) const = 0;
  virtual void cdata(const std::string &cdata, XMLReaderState &objs) const = 0;
  virtual void finish(const XMLElementBase *parent, XMLReaderState &objs,
                      const std::string &uri, const std::string &lname,
                      const std::string &qname) const = 0;

  virtual void write(const XMLElementBase * /*parent*/,
                     tl::OutputStream & /*os*/, int /*indent*/,
                     XMLWriterState & /*objs*/) const {}
  ...
  bool check_name(const std::string & /*uri*/, const std::string &lname,
  ...
  iterator begin() const { return mp_children->begin(); }

  iterator end() const { return mp_children->end(); }
...
class TL_PUBLIC_TEMPLATE XMLElement : public XMLElementBase {
public:
  XMLElement(const Read &r, const Write &w, const std::string &name,
             const XMLElementList &children)
  ...
  virtual void create(const XMLElementBase *, XMLReaderState &objs,
  ...
  virtual void write(const XMLElementBase * /*parent*/, tl::OutputStream &os,
                     int indent, XMLWriterState &objs) const {
    XMLObjTag<Parent> parent_tag;
    Read r(m_r);
...
private:
  ...
  void write_obj(Obj obj, tl::OutputStream &os, int indent,
                 tl::pass_by_value_tag, XMLWriterState &objs) const {
    XMLObjTag<Obj> tag;
    objs.push(&obj);
  ...
  void write_obj(const Obj &obj, tl::OutputStream &os, int indent,
                 tl::pass_by_ref_tag, XMLWriterState &objs) const {
    XMLObjTag<Obj> tag;
    objs.push(&obj);
    for (XMLElementBase::iterator c = this->begin(); c != this->end(); ++c) {
      c->get()->write(this, os, indent + 1, objs);
 ...
 *
 *  This declares a XML element with a constructor that takes a (object *)
 * arguments to the parent.
 */

template <class Obj, class Parent, class Read, class Write>
class TL_PUBLIC_TEMPLATE XMLElementWithParentRef : public XMLElementBase {
public:
  XMLElementWithParentRef(const Read &r, const Write &w,
  ...
  XMLElementWithParentRef(
      const XMLElementWithParentRef<Obj, Parent, Read, Write> &d)
      : XMLElementBase(d), m_r(d.m_r), m_w(d.m_w) {
    // .. nothing yet ..
  }
...
class TL_PUBLIC_TEMPLATE XMLMember : public XMLElementBase {
public:
  XMLMember(const Read &r, const Write &w, const std::string &name,
...
class TL_PUBLIC_TEMPLATE XMLWildcardMember : public XMLElementBase {
public:
...
template <class Obj>
class TL_PUBLIC_TEMPLATE XMLStruct : public XMLElementBase {
public:
  ...
  XMLStruct(const XMLStruct<Obj> &d) : XMLElementBase(d) {
    // .. nothing yet ..
  }

  virtual XMLElementBase *clone() const { return new XMLStruct<Obj>(*this); }

  virtual void create(const XMLElementBase *, XMLReaderState &,
                      const std::string & /*uri*/,
                      const std::string & /*lname*/,
                      const std::string & /*qname*/) const {
    // .. nothing yet ..
  ...
  virtual void finish(const XMLElementBase *, XMLReaderState &,
                      const std::string & /*uri*/,
                      const std::string & /*lname*/,
                      const std::string & /*qname*/) const {
    // .. nothing yet ..
...
private:
  virtual void write(const XMLElementBase *, tl::OutputStream &, int,
                     XMLWriterState &) const {
    // .. see write (os)
  }
};
 ...
 */
inline XMLElementList operator+(const XMLElementList &l,
                                const XMLElementBase &e) {
 ...
 */
inline XMLElementList operator+(const XMLElementList &l, XMLElementBase *e) {
  return XMLElementList(l, e);
}

//  Several adaptor classes needed to implement the various make_* utilities

template <class Value, class Parent> struct XMLMemberDummyWriteAdaptor {
...
template <class Value, class Parent> struct XMLMemberWriteAdaptor {
  XMLMemberWriteAdaptor(Value Parent::*member) : mp_member(member) {
    // .. nothing yet ..
  }
...
template <class Value, class Parent> struct XMLMemberAccRefWriteAdaptor {
...
template <class Value, class Parent> struct XMLMemberTransferWriteAdaptor {
...
template <class Value, class Parent> struct XMLMemberAccWriteAdaptor {
  XMLMemberAccWriteAdaptor(void (Parent::*member)(Value)) : mp_member(member) {
    // .. nothing yet ..
  }
...
template <class Value, class Parent> struct XMLMemberDummyReadAdaptor {
...
template <class Value, class Parent> struct XMLMemberReadAdaptor {
  typedef pass_by_ref_tag tag;
  ...
  const Value &operator()() const { return mp_owner->*mp_member; }
...
template <class Value, class Parent> struct XMLMemberAccRefReadAdaptor {
  typedef pass_by_ref_tag tag;
...
template <class Value, class Parent> struct XMLMemberAccReadAdaptor {
  ...
  Value operator()() const { return (mp_owner->*mp_member)(); }
...
struct XMLMemberIterReadAdaptor {
  ...
  XMLMemberIterReadAdaptor(Iter (Parent::*begin)() const,
                           Iter (Parent::*end)() const)
      : mp_begin(begin), mp_end(end) {
    // .. nothing yet ..
  }

  Value operator()() const { return *m_iter; }
...
XMLElement<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
           XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element(void (Parent::*setter)(const Value &), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
                    XMLMemberAccRefWriteAdaptor<Value, Parent>>(
...
template <class Value, class Parent>
XMLElement<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
           XMLMemberTransferWriteAdaptor<Value, Parent>>
make_element(void (Parent::*setter)(Value *), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
                    XMLMemberTransferWriteAdaptor<Value, Parent>>(
      XMLMemberDummyReadAdaptor<Value, Parent>(),
 ...
 */
template <class Value, class Parent>
XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
           XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element(const Value &(Parent::*getter)() const,
             void (Parent::*setter)(const Value &), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
                    XMLMemberAccRefWriteAdaptor<Value, Parent>>(
...
XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
           XMLMemberTransferWriteAdaptor<Value, Parent>>
make_element(const Value &(Parent::*getter)() const,
             void (Parent::*setter)(Value *), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
                    XMLMemberTransferWriteAdaptor<Value, Parent>>(
      XMLMemberAccRefReadAdaptor<Value, Parent>(getter),
...
template <class Value, class Parent>
XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
           XMLMemberAccWriteAdaptor<Value, Parent>>
make_element(const Value &(Parent::*getter)() const,
             void (Parent::*setter)(Value), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
 ...
 */
template <class Value, class Parent>
XMLElement<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
           XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element(Value (Parent::*getter)() const,
             void (Parent::*setter)(const Value &), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                    XMLMemberAccRefWriteAdaptor<Value, Parent>>(
...
XMLElement<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
           XMLMemberTransferWriteAdaptor<Value, Parent>>
make_element(Value (Parent::*getter)() const, void (Parent::*setter)(Value *),
             const std::string &name, const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                    XMLMemberTransferWriteAdaptor<Value, Parent>>(
      XMLMemberAccReadAdaptor<Value, Parent>(getter),
      XMLMemberTransferWriteAdaptor<Value, Parent>(setter), name, children);
 ...
 */
template <class Value, class Parent>
XMLElement<Value, Parent, XMLMemberReadAdaptor<Value, Parent>,
           XMLMemberWriteAdaptor<Value, Parent>>
make_element(Value Parent::*member, const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent, XMLMemberReadAdaptor<Value, Parent>,
                    XMLMemberWriteAdaptor<Value, Parent>>(
      XMLMemberReadAdaptor<Value, Parent>(member),
 ...
 */
template <class Value, class Iter, class Parent>
XMLElement<Value, Parent, XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
           XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element(Iter (Parent::*begin)() const, Iter (Parent::*end)() const,
             void (Parent::*setter)(const Value &), const std::string &name,
             const XMLElementList *children) {
  return XMLElement<Value, Parent,
                    XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
                    XMLMemberAccRefWriteAdaptor<Value, Parent>>(
...
template <class Value, class Parent>
XMLElementWithParentRef<Value, Parent,
                        XMLMemberAccRefReadAdaptor<Value, Parent>,
                        XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element_with_parent_ref(const Value &(Parent::*getter)() const,
                             void (Parent::*setter)(const Value &),
                             const std::string &name,
                             const XMLElementList &children) {
  return XMLElementWithParentRef<Value, Parent,
                                 XMLMemberAccRefReadAdaptor<Value, Parent>,
                                 XMLMemberAccRefWriteAdaptor<Value, Parent>>(
      XMLMemberAccRefReadAdaptor<Value, Parent>(getter),
      XMLMemberAccRefWriteAdaptor<Value, Parent>(setter), name, children);
...
XMLElementWithParentRef<Value, Parent,
                        XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
                        XMLMemberAccRefWriteAdaptor<Value, Parent>>
make_element_with_parent_ref(Iter (Parent::*begin)() const,
                             Iter (Parent::*end)() const,
                             void (Parent::*setter)(const Value &),
                             const std::string &name,
                             const XMLElementList &children) {
  return XMLElementWithParentRef<
      Value, Parent, XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
      XMLMemberAccRefWriteAdaptor<Value, Parent>>(
      XMLMemberIterReadAdaptor<const Value &, Iter, Parent>(begin, end),
      XMLMemberAccRefWriteAdaptor<Value, Parent>(setter), name, children);
}
...
template <class Value> struct XMLStdConverter {
  std::string to_string(const Value &v) const { return tl::to_string(v); }
...
XMLMember<std::string, Parent, XMLMemberDummyReadAdaptor<std::string, Parent>,
          XMLMemberDummyWriteAdaptor<std::string, Parent>,
          XMLStdConverter<std::string>>
make_member(const std::string &name) {
  return XMLMember<std::string, Parent,
 ...
 */
template <class Value, class Parent>
XMLMember<Value, Parent, XMLMemberReadAdaptor<Value, Parent>,
          XMLMemberWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(Value Parent::*member, const std::string &name) {
  return XMLMember<Value, Parent, XMLMemberReadAdaptor<Value, Parent>,
                   XMLMemberWriteAdaptor<Value, Parent>,
                   XMLStdConverter<Value>>(
      XMLMemberReadAdaptor<Value, Parent>(member),
 ...
 */
template <class Value, class Parent>
XMLMember<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
          XMLMemberAccRefWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(void (Parent::*setter)(const Value &), const std::string &name) {
  return XMLMember<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
                   XMLMemberAccRefWriteAdaptor<Value, Parent>,
                   XMLStdConverter<Value>>(
      XMLMemberDummyReadAdaptor<Value, Parent>(),
...
template <class Value, class Parent>
XMLMember<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
          XMLMemberAccWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
 ...
 */
template <class Value, class Parent>
XMLMember<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
          XMLMemberAccRefWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(const Value &(Parent::*getter)() const,
            void (Parent::*setter)(const Value &), const std::string &name) {
  return XMLMember<Value, Parent, XMLMemberAccRefReadAdaptor<Value, Parent>,
                   XMLMemberAccRefWriteAdaptor<Value, Parent>,
                   XMLStdConverter<Value>>(
      XMLMemberAccRefReadAdaptor<Value, Parent>(getter),
      XMLMemberAccRefWriteAdaptor<Value, Parent>(setter), name);
 ...
 */
template <class Value, class Parent>
XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
          XMLMemberAccWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(Value (Parent::*getter)() const, void (Parent::*setter)(Value),
            const std::string &name) {
  return XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                   XMLMemberAccWriteAdaptor<Value, Parent>,
                   XMLStdConverter<Value>>(
      XMLMemberAccReadAdaptor<Value, Parent>(getter),
      XMLMemberAccWriteAdaptor<Value, Parent>(setter), name);
...
XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
          XMLMemberAccRefWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(Value (Parent::*getter)() const,
            void (Parent::*setter)(const Value &), const std::string &name) {
  return XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                   XMLMemberAccRefWriteAdaptor<Value, Parent>,
                   XMLStdConverter<Value>>(
 ...
 */
template <class Value, class Iter, class Parent>
XMLMember<Value, Parent, XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
          XMLMemberAccRefWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>
make_member(Iter (Parent::*begin)() const, Iter (Parent::*end)() const,
            void (Parent::*setter)(const Value &), const std::string &name) {
  return XMLMember<
      Value, Parent, XMLMemberIterReadAdaptor<const Value &, Iter, Parent>,
      XMLMemberAccRefWriteAdaptor<Value, Parent>, XMLStdConverter<Value>>(
...
XMLMember<Value, Parent, XMLMemberReadAdaptor<Value, Parent>,
          XMLMemberWriteAdaptor<Value, Parent>, Converter>
...
template <class Value, class Parent, class Converter>
XMLMember<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
          XMLMemberAccRefWriteAdaptor<Value, Parent>, Converter>
make_member(void (Parent::*setter)(const Value &), const std::string &name,
            Converter conv) {
  return XMLMember<Value, Parent, XMLMemberDummyReadAdaptor<Value, Parent>,
                   XMLMemberAccRefWriteAdaptor<Value, Parent>, Converter>(
...
template <class Value, class Parent, class Converter>
XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
          XMLMemberDummyWriteAdaptor<Value, Parent>, Converter>
make_member(void (Parent::*setter)(Value), const std::string &name,
            Converter conv) {
  return XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                   XMLMemberDummyWriteAdaptor<Value, Parent>, Converter>(
...
template <class Value, class Parent, class Converter>
XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
          XMLMemberAccWriteAdaptor<Value, Parent>, Converter>
make_member(Value (Parent::*getter)() const, void (Parent::*setter)(Value),
            const std::string &name, Converter conv) {
  return XMLMember<Value, Parent, XMLMemberAccReadAdaptor<Value, Parent>,
                   XMLMemberAccWriteAdaptor<Value, Parent>, Converter>(
      XMLMemberAccReadAdaptor<Value, Parent>(getter),
