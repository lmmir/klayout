...
inline void tl_iter_swap(_ForwardIter1 __a, _ForwardIter2 __b) {
  std::swap(*__a, *__b);
}
...
inline void __unguarded_insertion_sort(_RandomAccessIter __first,
  ...
  typedef
      typename std::iterator_traits<_RandomAccessIter>::value_type _ValueType;
...
inline void push_heap(_RandomAccessIterator __first,
  ...
  tl::__push_heap(__first, _DistanceType((__last - __first) - 1),
...
void __introsort_loop(_RandomAccessIter __first, _RandomAccessIter __last,
  ...
  while (__last - __first > tl::_M_threshold) {
    ...
    --__depth_limit;
    _RandomAccessIter __cut = tl::__unguarded_partition(
        __first, __last,
        _ValueType(tl::__median(*__first, *(__first + (__last - __first) / 2),
                                *(__last - 1))));
...
inline void sort(_RandomAccessIter __first, _RandomAccessIter __last) {
  if (__first != __last) {
    ...
    for (_RandomAccessIter __i = __first + 1; __i != __last && !__unsorted;
         ++__i) {
      if (!(__i[-1] < *__i))
        __unsorted = true;
    }
