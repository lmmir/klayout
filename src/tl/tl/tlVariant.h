...
/**
 *  @brief A base class which describes a class, i.e. an object capable of
 * converting and handling void *
 *
 *  There must be one particular class object per class. In particular the
 * equality of the class object pointer's must indicate that two object's share
 * the same class.
 */
...
public:
  ...
  static void register_user_class(const std::string &name,
                                  const VariantUserClassBase *cls);
  static const VariantUserClassBase *find_cls_by_name(const std::string &name);

protected:
  static const tl::VariantUserClassBase *instance(const std::type_info &type,
                                                  bool is_const);
  static void register_instance(const tl::VariantUserClassBase *inst,
                                const std::type_info &type, bool is_const);
  static void unregister_instance(const tl::VariantUserClassBase *inst,
                                  const std::type_info &type, bool is_const);
};
 ...
 */
template <class T>
class TL_PUBLIC_TEMPLATE VariantUserClass : public VariantUserClassBase {
public:
  VariantUserClass() {}
...
protected:
  ...
  void unregister_instance(const tl::VariantUserClassBase *inst,
                           bool is_const) {
 ...
 *
 *   This variant is capable of storing long, double, std::string, void (nil)
 * and lists of other variants.
 */
...
public:
   ...
   *
   *  This constructor will convert a QVariant into a tl::Variant as far as
   * possible.
   */
  ...
  /**
   *  @brief Initialize the Variant with "unsigned int" (actually "unsigned
   * long")
   */
   ...
   *
   *  The "id" type is basically a size_t, but is supposed to be used as a
   * representative for another value. One application for that type is a
   * placeholder for an OASIS name until it is associated with a real value.
   */
   ...
   *
   *  If shared is true, the variant will use a shared pointer to manage the
   * ownership of the object (i.e. if the object is new'd it will be deleted by
   * the variant). If shared is false, a weak pointer will be employed that
   * watches the object.
   */
   ...
   */
  template <class T> Variant(const T &obj) : m_type(t_user), m_string(0) {
    const tl::VariantUserClassBase *c =
        tl::VariantUserClass<T>::instance(false);
    tl_assert(c != 0);
   ...
   */
  Variant(const std::vector<tl::Variant> &list) : m_type(t_list), m_string(0) {
   ...
   */
  template <class Iter>
  Variant(Iter from, Iter to) : m_type(t_list), m_string(0) {
   ...
   */
  template <class T> static tl::Variant make_variant_ref(T *t) {
   ...
   *  @brief Utility: initialize a variant from GSI type reference
   */
  template <class T> static tl::Variant make_variant_ref(const T *t) {
   ...
   */
  template <class T> static tl::Variant make_variant(T *t) {
    const tl::VariantUserClassBase *c = gsi::cls_decl<T>()->var_cls(false);
    tl_assert(c != 0);
   ...
   *  @brief Initialize with a user type
   */
  void set_user_ref(tl::Object *obj, const VariantUserClassBase *cls,
                    bool shared) {
    reset();
    m_type = t_user_ref;
   ...
   *  For user types, this is not implemented yet.
   */
  bool operator!=(const Variant &d) const { return !operator==(d); }
   ...
   */
  template <class T> T to() const { tl_assert(false); }
  ...
  const void *to_user() const {
    if (m_type == t_user) {
      ...
      return m_var.mp_user_ref.cls->deref_proxy_const(
          reinterpret_cast<const WeakOrSharedPtr *>(m_var.mp_user_ref.ptr)
              ->get());
    } else {
      return 0;
    }
  ...
  void *to_user() {
    if (m_type == t_user) {
      ...
      return m_var.mp_user_ref.cls->deref_proxy(
          reinterpret_cast<WeakOrSharedPtr *>(m_var.mp_user_ref.ptr)->get());
    } else {
      return 0;
    }
  }
  ...
  const tl::Object *to_object() const {
    if (m_type == t_user_ref) {
      return reinterpret_cast<const WeakOrSharedPtr *>(m_var.mp_user_ref.ptr)
          ->get();
    } else {
      return 0;
    }
  ...
  /**
   *  @brief Returns a value indicating whether the user object is a const
   * reference or object
   */
   ...
   *  References do not own the object and upon destruction of the tl::Variant
   ...
   *  An object can only be deleted if it is owned by the variant, i.e.
   * user_is_ref is false.
   */
  void user_destroy();
   ...
   *
   *  "other" needs to be a user object and the class of "other" needs to be
   * same as self.
   */
   ...
   */
  template <class T> const T &to_user() const {
    if (is_user()) {
      const VariantUserClass<T> *tcls =
          dynamic_cast<const VariantUserClass<T> *>(user_cls());
      tl_assert(tcls != 0);
   ...
   *
   *  After morphing the variant, the variant will use the given type
   * internally. The native pointer can be used to access the value then. A nil
   * value is not morphed and remains nil. In that case, the native pointer will
   * be 0.
   */
  template <class T> tl::Variant &morph() {
    if (!is_nil()) {
      *this = to<T>();
    }
    return *this;
   ...
   */
  template <class T> Variant cast() const { return Variant(to<T>()); }
   ...
   */
  size_t size() const { return m_type == t_list ? m_var.m_list->size() : 0; }
  ...
  /**
   *  @brief Returns the value for the given key or 0 if the variant is not an
   * array or does not contain the key
   */
   ...
   *
   *  All numeric types can convert to double. That is double and the integer
   * types.
   */
   ...
   *
   *  All numeric types can convert to float. That is double and the integer
   * types unless the double value is outside the float range.
   */
   ...
   *
   *  All numeric types can convert to char unless the value is outside the
   * allowed range.
   */
   ...
   *
   *  All numeric types can convert to signed char unless the value is outside
   * the allowed range.
   */
   ...
   *
   *  All numeric types can convert to unsigned char unless the value is outside
   * the allowed range.
   */
   ...
   *
   *  All numeric types can convert to short unless the value is outside the
   * allowed range.
   */
   ...
   *
   *  All numeric types can convert to unsigned short unless the value is
   ...
   *
   *  All numeric types can convert to int unless the value is outside the
   * allowed range.
   */
   ...
   *
   *  All numeric types can convert to unsigned int unless the value is outside
   * the allowed range.
   */
   ...
   *
   *  All numeric types can convert to long unless the value is outside the
   * allowed range.
   */
   ...
   *
   *  All numeric types can convert to unsigned long unless the value is outside
   * the allowed range.
   */
   ...
   *
   *  This is a templatized version of the various can_convert_to_... methods.
   * This does not include conversion to a user type, arrays or lists. This is
   * the generic version. Specializations follow.
   */
  template <class T> bool can_convert_to() const { return false; }

  /**
   *  @brief Test, if it is a double or can be converted to a double
   */
  bool is_double() const { return m_type == t_double || m_type == t_float; }

  /**
   *  @brief Test, if it is a char
   */
  bool is_char() const { return m_type == t_char; }

  /**
   *  @brief Test, if it is a long or can be converted to a long
   */
  bool is_long() const {
    return m_type == t_long || m_type == t_int || m_type == t_short ||
           m_type == t_schar;
  }
   ...
   */
  bool is_ulong() const {
    return m_type == t_ulong || m_type == t_uint || m_type == t_ushort ||
           m_type == t_uchar;
  }
   ...
   */
  bool is_longlong() const { return m_type == t_longlong; }

  /**
   *  @brief Test, if it is a unsigned long long
   */
  bool is_ulonglong() const { return m_type == t_ulonglong; }

#if defined(HAVE_64BIT_COORD)
  /**
   *  @brief Test, if it is an int128
   */
  bool is_int128() const { return m_type == t_int128; }
#endif

  /**
   *  @brief Test, if it is a bool
   */
  bool is_bool() const { return m_type == t_bool; }

  /**
   *  @brief Test, if it is a id
   */
  bool is_id() const { return m_type == t_id; }

  /**
   *  @brief Test, if it is a std::vector<char> byte array
   */
  bool is_bytearray() const { return m_type == t_bytearray; }
   ...
   */
  bool is_qbytearray() const { return m_type == t_qbytearray; }

  /**
   *  @brief Test, if it is a QString
   */
  bool is_qstring() const { return m_type == t_qstring; }
   ...
   */
  bool is_stdstring() const { return m_type == t_stdstring; }

  /**
   *  @brief Test, if it is a "C" string
   */
  bool is_cstring() const { return m_type == t_string; }
   ...
   */
  bool is_nil() const { return m_type == t_nil; }

  /**
   *  @brief Test, if it is an array
   */
  bool is_array() const { return m_type == t_array; }

  /**
   *  @brief Test, if it is a list
   */
  bool is_list() const { return m_type == t_list; }

  /**
   *  @brief Get the type code
   */
  type type_code() const { return m_type; }

  /**
   *  @brief Test, if this is a user type
   */
  bool is_user() const { return m_type == t_user || m_type == t_user_ref; }

  /**
   *  @brief Test, if this is a user type and can convert to the given type
   */
  template <class T> bool is_user() const {
    if (m_type == t_user) {
      const VariantUserClass<T> *tcls =
          dynamic_cast<const VariantUserClass<T> *>(m_var.mp_user.cls);
      return tcls != 0;
    } else if (m_type == t_user_ref) {
      const VariantUserClass<T> *tcls =
          dynamic_cast<const VariantUserClass<T> *>(m_var.mp_user_ref.cls);
      return tcls != 0;
    } else {
      return false;
    }
   ...
   */
  bool is_object() const { return m_type == t_user_ref; }

  /**
   *  @brief Test, if this is a user type and can convert to the given type
   */
  template <class T> bool is_object() const {
    if (m_type == t_user_ref) {
      const VariantUserClass<T> *tcls =
          dynamic_cast<const VariantUserClass<T> *>(m_var.mp_user_ref.cls);
      return tcls != 0;
  ...
  /**
   *  @brief Convert the Variant to a string that can be parsed with the
   * Extractor
   *
...
template <> inline unsigned char Variant::to<unsigned char>() const {
  return to_uchar();
}
template <> inline signed char Variant::to<signed char>() const {
  return to_schar();
}
template <> inline short Variant::to<short>() const { return to_short(); }
template <> inline unsigned short Variant::to<unsigned short>() const {
  return to_ushort();
}
template <> inline int Variant::to<int>() const { return to_int(); }
template <> inline unsigned int Variant::to<unsigned int>() const {
  return to_uint();
}
template <> inline long Variant::to<long>() const { return to_long(); }
template <> inline unsigned long Variant::to<unsigned long>() const {
  return to_ulong();
}
template <> inline long long Variant::to<long long>() const {
  return to_longlong();
}
template <> inline unsigned long long Variant::to<unsigned long long>() const {
  return to_ulonglong();
}
#if defined(HAVE_64BIT_COORD)
template <> inline __int128 Variant::to<__int128>() const {
  return to_int128();
}
#endif
...
template <> inline std::string Variant::to<std::string>() const {
  return to_stdstring();
}
template <> inline std::vector<char> Variant::to<std::vector<char>>() const {
  return to_bytearray();
}
#if defined(HAVE_QT)
template <> inline QString Variant::to<QString>() const { return to_qstring(); }
template <> inline QByteArray Variant::to<QByteArray>() const {
  return to_qbytearray();
}
#endif
template <> inline const char *Variant::to<const char *>() const {
  return to_string();
}
...
template <> inline bool Variant::is<unsigned char>() const {
  return m_type == t_uchar;
}
template <> inline bool Variant::is<signed char>() const {
  return m_type == t_schar;
}
template <> inline bool Variant::is<short>() const { return m_type == t_short; }
template <> inline bool Variant::is<unsigned short>() const {
  return m_type == t_ushort;
}
template <> inline bool Variant::is<int>() const { return m_type == t_int; }
template <> inline bool Variant::is<unsigned int>() const {
  return m_type == t_uint;
}
template <> inline bool Variant::is<long>() const { return m_type == t_long; }
template <> inline bool Variant::is<unsigned long>() const {
  return m_type == t_ulong;
}
template <> inline bool Variant::is<long long>() const {
  return m_type == t_longlong;
}
template <> inline bool Variant::is<unsigned long long>() const {
  return m_type == t_ulonglong;
}
#if defined(HAVE_64BIT_COORD)
template <> inline bool Variant::is<__int128>() const {
  return m_type == t_int128;
}
#endif
template <> inline bool Variant::is<double>() const {
  return m_type == t_double;
}
template <> inline bool Variant::is<float>() const { return m_type == t_float; }
template <> inline bool Variant::is<std::string>() const {
  return m_type == t_stdstring;
}
template <> inline bool Variant::is<std::vector<char>>() const {
  return m_type == t_bytearray;
}
#if defined(HAVE_QT)
template <> inline bool Variant::is<QString>() const {
  return m_type == t_qstring;
}
template <> inline bool Variant::is<QByteArray>() const {
  return m_type == t_qbytearray;
}
#endif
template <> inline bool Variant::is<const char *>() const {
...
template <> inline bool Variant::can_convert_to<char>() const {
  return can_convert_to_char();
}
template <> inline bool Variant::can_convert_to<unsigned char>() const {
  return can_convert_to_uchar();
}
template <> inline bool Variant::can_convert_to<signed char>() const {
  return can_convert_to_schar();
}
template <> inline bool Variant::can_convert_to<short>() const {
  return can_convert_to_short();
}
template <> inline bool Variant::can_convert_to<unsigned short>() const {
  return can_convert_to_ushort();
}
template <> inline bool Variant::can_convert_to<int>() const {
  return can_convert_to_int();
}
template <> inline bool Variant::can_convert_to<unsigned int>() const {
  return can_convert_to_uint();
}
template <> inline bool Variant::can_convert_to<long>() const {
  return can_convert_to_long();
}
template <> inline bool Variant::can_convert_to<unsigned long>() const {
  return can_convert_to_ulong();
}
template <> inline bool Variant::can_convert_to<long long>() const {
  return can_convert_to_longlong();
}
template <> inline bool Variant::can_convert_to<unsigned long long>() const {
  return can_convert_to_ulonglong();
}
#if defined(HAVE_64BIT_COORD)
template <> inline bool Variant::can_convert_to<__int128>() const {
  return can_convert_to_int128();
}
#endif
template <> inline bool Variant::can_convert_to<double>() const {
  return can_convert_to_double();
}
template <> inline bool Variant::can_convert_to<float>() const {
  return can_convert_to_float();
}
template <> inline bool Variant::can_convert_to<std::string>() const {
  return true;
}
template <> inline bool Variant::can_convert_to<std::vector<char>>() const {
  return true;
}
#if defined(HAVE_QT)
template <> inline bool Variant::can_convert_to<QString>() const {
  return true;
}
template <> inline bool Variant::can_convert_to<QByteArray>() const {
  return true;
}
#endif
template <> inline bool Variant::can_convert_to<const char *>() const {
  return true;
}
 ...
 */
template <>
TL_PUBLIC void extractor_impl<Variant>(tl::Extractor &ex, Variant &v);
template <>
TL_PUBLIC bool test_extractor_impl<Variant>(tl::Extractor &ex, Variant &v);
