 ...
 */
void TL_PUBLIC set_ui_exception_handlers(
    void (*handler_tl)(const tl::Exception &, QWidget *parent),
    void (*handler_std)(const std::exception &, QWidget *parent),
    void (*handler_default)(QWidget *parent));

//  The BEGIN_PROTECTED .. END_PROTECTED macros are supposed to enclose a
//  protected block The main purpose is to provide a unified method of handling
//  exceptions
...
#define BEGIN_PROTECTED_CLEANUP                                                \
  bool __error_caught = false;                                                 \
  try {
