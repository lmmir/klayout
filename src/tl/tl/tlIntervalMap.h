 ...
 */
template <class I, class T> class iv_compare_f {
public:
 ...
 */
template <class I, class T> class interval_map {
public:
   ...
   */
  template <class Iter, class J> void add(Iter from, Iter to, J &j) {
   ...
   *  @param compress Set this parameter to true to enable compression of the
   * intervals where possible
   */
  template <class J> void add(I i1, I i2, const T &t, J &j) {
    typename index_map::iterator lb = std::lower_bound(
        m_index_map.begin(), m_index_map.end(), i1, iv_compare_f<I, T>());
    ...
    while (i1 < i2) {
      ...
      if (lb == m_index_map.end() || !(lb->first.first < i2)) {
        ...
        if (lb->first.first < i1) {
          index_value_pair v(*lb); //  this copy is important since the insert
                                   //  may invalid the element *lb!
    ...
    //  (only the one before the current interval and the one following can
    //  participate)
    ...
    while (lb != m_index_map.begin() + nf) {
      ...
      do {
        ++lb;
               ...
               lbb->first.second == lb->first.first &&
               lbb->second == lb->second);
  ...
  void erase(I i1, I i2) {
    ...
    if (n == 0) {
      ...
      index_value_pair v(*lb0); //  this copy is important since the insert may
                                //  invalid the element *lb!
  ...
  T *mapped(I i) {
    typename index_map::iterator lb = std::lower_bound(
        m_index_map.begin(), m_index_map.end(), i, iv_compare_f<I, T>());
  ...
  const T *mapped(I i) const {
    typename index_map::const_iterator lb = std::lower_bound(
        m_index_map.begin(), m_index_map.end(), i, iv_compare_f<I, T>());
    if (lb == m_index_map.end() || i < lb->first.first) {
      return 0;
    } else {
   ...
   *
   *  This will return the iterator for the interval which contains the index.
   * If there is no such interval, this method will return end().
   *
   *  If there is no interval for the given index (not set), the returned
   * iterator will not be end(), but it's start value will not be less or equal
   * to the index.
   *
   *  @param i The index to search for
   *  @return The iterator to the corresponding interval or end() if there is no
   * such interval
   */
  const_iterator find(I i) const {
    return std::lower_bound(m_index_map.begin(), m_index_map.end(), i,
                            iv_compare_f<I, T>());
  }
   ...
   */
  size_t size() const { return m_index_map.size(); }

  /**
   *  @brief Clear the index map
   */
  void clear() { m_index_map.clear(); }
   ...
   *  @brief begin Iterator
   */
  iterator begin() { return m_index_map.begin(); }

  /**
   *  @brief end Iterator
   */
  iterator end() { return m_index_map.end(); }
