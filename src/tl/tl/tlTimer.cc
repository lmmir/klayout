...
void current_utc_time(struct timespec *ts) {
  ...
  uint64_t t =
      (uint64_t(ft.dwHighDateTime) << (sizeof(ft.dwHighDateTime) * 8)) |
...
Timer::Timer()
    : m_user_ms(0), m_sys_ms(0), m_wall_ms(0), m_user_ms_res(0),
      m_sys_ms_res(0), m_wall_ms_res(0) {
  // ..
}
...
#ifdef _WIN32
  timer_t clks_ms =
      (timer_t)((double)clock() * (1000.0 / CLOCKS_PER_SEC) + 0.5);
  m_user_ms += clks_ms;
...
void SelfTimer::report() const {
  ...
  tl::info << m_desc << ": " << sec_user() << " (user) " << sec_sys()
           << " (sys) " << sec_wall() << " (wall)" << tl::noendl;

  if (memsize > 0) {
    tl::info << " " << tl::sprintf("%.2fM", double(memsize) / (1024.0 * 1024.0))
             << " (mem)";
  } else {
    tl::info << "";
  }
...
Clock::Clock(double s) { m_clock_ms = s * 1000.0; }

double Clock::seconds() const { return double(m_clock_ms) * 0.001; }
