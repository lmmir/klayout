...
/**
 *  @brief A template class mapping a begin .. end iterator pair to the at_end
 * semantics
 */

template <class Iter> class iterator_pair {
public:
   ...
   */
  iterator_pair(iter_type from, iter_type to) : m_from(from), m_to(to) {}

  /**
   *  @brief direct resolution
   */
  const value_type &operator*() const { return *m_from; }
   ...
   *  @brief Equality
   */
  bool operator==(const iterator_pair &d) const { return m_from == d.m_from; }

  /**
   *  @brief Inequality
   */
  bool operator!=(const iterator_pair &d) const { return !operator==(d); }

  /**
   *  @brief Return true, if a the end
   */
  bool at_end() const { return m_from == m_to; }

  /**
   *  @brief Return the basic iterator
   */
  const iter_type &basic_iter() const { return m_from; }
 ...
 */
template <class Value> class ident_map {
public:
 ...
 *  A mapping is in general implemented by the () operator taking one
 *  argument and delivering an argument of the same type.
 *  This implementation of this concept delivers the same value irregardless of
 * the input.
 */
template <class Value> class const_map {
public:
  typedef Value value_type;

  const_map(const Value &v) : m_value(v) {}

  const Value &operator()(const Value &) const { return m_value; }
...
/**
 *  @brief A mapping function: map one element to a value using an associative
 * container
 *
 *  A mapping is in general implemented by the () operator taking one
 *  argument and delivering an argument of the same type.
 ...
 */
template <class Value, class Assoc> class assoc_map {
public:
  typedef Value value_type;

  assoc_map(const Assoc &assoc) : m_assoc(assoc) {}
...
/**
 *  @brief A specialization of assoc_map using a std::map as associative
 * container
 */
template <class Value>
struct map_map : public assoc_map<Value, std::map<Value, Value>> {
 ...
 */
template <class Value> struct func_delegate_base {
...
struct func_delegate : public func_delegate_base<Value> {
  func_delegate(Op &op) : m_op(op) {}

  virtual Value operator()(const Value &v) { return m_op(v); }
...
template <> struct type_from_value<true> { typedef True value; };

template <> struct type_from_value<false> { typedef False value; };
 ...
 */
template <class A, class B> struct is_equal_type {
  typedef False value;
 ...
 */
template <class A> struct is_equal_type<A, A> {
  typedef True value;
 ...
 */
template <class B, class D> struct is_derived {
private:
  static char m_check(...);
...
public:
  typedef typename type_from_value<sizeof(m_check(m_d)) == sizeof(long)>::value
      value;
...
template <class T, class S> struct __try_cast<T, S, True> {
  T *operator()(S *s) const { return static_cast<T *>(s); }
};

template <class T, class S> struct __try_cast<T, S, False> {
  T *operator()(S * /*s*/) const { return 0; }
};

/**
 *  @brief A static cast that works always and will return 0 if the object
 * cannot be cast
 */
template <class T, class S> T *try_static_cast(S *s) {
 ...
 */
template <class X> struct get_inner_type { typedef X result; };

template <class X> struct get_inner_type<const X *> { typedef X result; };

template <class X> struct get_inner_type<const X &> { typedef X result; };

template <class X> struct get_inner_type<X *> { typedef X result; };
