...
template <class Tree, class Cmp> class kd_tree_it {
public:
  ...
  kd_tree_it(const Tree &t, value_picker_type p, const Cmp &c)
      : m_j(1), m_n(t.size()), m_l(0), m_tree(t), m_picker(p), m_compare(c) {
    if (m_n > 0 && need_visit()) {
      traverse();
      while (!at_end()) {
        if (check()) {
  ...
  const object_type &operator*() const { return m_tree.objects()[m_j - 1]; }

  operator difference_type() const { return index(); }

  /**
   *  @brief Return the index of the position indicated by this iterator
   */
  difference_type index() const { return m_j - 1; }

  bool operator==(const kd_tree_it<Tree, Cmp> &i) { return m_j == i.m_j; }

  bool operator!=(const kd_tree_it<Tree, Cmp> &i) { return m_j != i.m_j; }

  bool operator==(size_type j) { return m_j == j; }
   ...
   */
  bool at_end() const { return m_j == 0; }
...
private:
  ...
  bool check() const { return m_compare(m_tree.objects()[m_j - 1]); }
...
template <class Obj, class Val, class ValPicker, class Cmp> class kd_comp_f {
public:
...
template <class Cont> class kd_n_it {
public:
  ...
  value_type &operator*() const { return m_begin[m_index]; }
  ...
  bool operator==(const kd_n_it &i) const { return m_index == i.m_index; }

  bool operator!=(const kd_n_it &i) const { return !operator==(i); }
...
template <class Obj, class Val, class ValPicker, class Cmp,
          class ObjV = std::vector<Obj>, class ValV = std::vector<Val>>
class kd_tree {
public:
   ...
   */
  size_type size() const { return m_objs.size(); }

  /**
   *  @brief Reserves space for a given number of elements
   */
  void reserve(size_type n) { m_objs.reserve(n); }

  /**
   *  @brief Resize the tree to a given number of elements inserting default
   * elements
   */
  void resize(size_type n) { m_objs.resize(n); }
   ...
   */
  void erase(iterator pos) { m_objs.erase(pos); }
   ...
   *  Erasing objects will required resorting.
   */
  void erase(iterator from, iterator to) { m_objs.erase(from, to); }
  ...
  void sort(ValPicker picker, Cmp comp) {
    ...
    partial_sort(0, 0, kd_n_it(1, m_objs.begin(), 0),
                 kd_n_it(1, m_objs.begin(), m_objs.size()), picker, comp);
  }

  /**
   *  @brief Direct access to the objects vector
   */
  const obj_vector_type &objects() const { return m_objs; }
   ...
   */
  void bounds(const bound_vector_type &bounds) { m_bounds = bounds; }

  /**
   *  @brief Direct access to the bounds vector
   */
  const bound_vector_type &bounds() const { return m_bounds; }

  /**
   *  @brief Direct iterator access to the element vector without search
   */
  const_iterator begin() const { return m_objs.begin(); }

  /**
   *  @brief Direct iterator access to the element vector without search
   */
  const_iterator end() const { return m_objs.end(); }
  ...
  kd_tree_it<tree_type, SelCmp> sel_begin(value_picker_type picker,
                                          const SelCmp &cmp) const {
    return kd_tree_it<tree_type, SelCmp>(*this, picker, cmp);
  }

  size_type sel_end() const { return 0; }
...
private:
  ...
  void partial_sort(typename bound_vector_type::difference_type bi,
                    unsigned int l, const kd_n_it &from, const kd_n_it &to,
    ...
    if (n > 2) {
      ...
      partial_sort(bi + from.step() * 2, l + 1, s1, s1 + (n - n1 - 1), picker,
                   comp);
