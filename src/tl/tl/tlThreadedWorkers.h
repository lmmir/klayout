 ...
 *
 *  The threaded worker framework provides a way to control multiple workers
 * performing operations on some database or data repository object. The
 * framework provides a way to control the workers, in particular kick off and
 * shut down worker threads. It does not deal with the issue of the database
 *  object being thread-safe.
 *
 *  The general concept involves
 *  1.) A boss (tl::Boss class). There is one boss instance per "database"
 * instance. The boss must be in some way associated with the database. The boss
 * provides a way to control several jobs, i.e. stop them if required. 2.) Jobs:
 * multiple jobs (tl::Job objects) can be registered under one boss. A job is
 * not necessarily running or terminated. It can be started, stopped, restarted
 * or terminated. A job is initialized in the main thread and sets up the
 * threads which actually do the job (workers). A job may be associated with
 * multiple boss instances. 3.) Workers: a job can be split into multiple tasks
 * which are executed by the workers. A worker is a thread which receives tasks
...
public:
   ...
   */
  bool is_empty() const { return mp_first == 0; }
   ...
   */
  const Task *peek() const { return mp_first; }
...
public:
   ...
   *
   *  @param nworkers The number of workers to provide. Can be 0 for
   * non-threaded (synchronous) execution of the tasks.
   */
   ...
   *  @brief Stop the job
   *
   *  It is guaranteed that the job is stopped and no worker is active after
   ...
   *
   *  This does not trigger the actual operation yet. It should be done
   * separately before \start is called. However, it is possible to schedule
   * jobs while the job is running and even from within other tasks. It is
   * guaranteed that the order of processing of the tasks is maintained.
   * However, it is not guaranteed that previous tasks have been processed
   * already because they might be send to a different thread.
   */
   ...
   */
  size_t tasks() const { return m_task_list.size(); }
   ...
   */
  int num_workers() const { return m_nworkers; }
...
protected:
   ...
   *
   *  This method is called from the main thread where start() is being called
   * to set up all workers. This method is called from the main thread from
   * which start () was called.
   */
  virtual void setup_worker(Worker * /*worker*/) {}

  /**
   *  @brief This method is called before the given task is started in sync mode
   * (workers == 0)
   */
  virtual void before_sync_task(Task * /*task*/) {}

  /**
   *  @brief This method is called after the given task has finished in sync
   * mode (workers == 0)
   */
   ...
   *  Caution: this method is called from the thread that was executing the last
   * task. It is called before the controlling thread is awakened inside wait().
   */
   ...
   */
  tl::Worker *worker(int n) { return mp_workers[n]; }
 ...
 */
template <class W> class Job : public JobBase {
public:
  /**
   *  @brief Constructor: create a job based on the given worker class
   *
   *  @param nworkers The number of workers or 0 if synchronous operation is
   * requested.
   */
  Job(int nworkers = 1) : JobBase(nworkers) {}

protected:
  virtual Worker *create_worker() { return new W(); }
};

/**
 *  @brief A worker
 *
 *  The worker is the thread doing the actual work. A worker must be
 * reimplemented to provide the operation implementation by implementing
 * "perform_task". A worker is provided with a sequence of tasks with define the
 * operations that the worker is supposed to perform.
 */
...
public:
   ...
   */
  int worker_index() const { return m_worker_index; }

protected:
  /**
   *  @brief Perform one task
   *
   *  The implementation of this method is supposed to regularly call
   * \checkpoint in order to receive asynchronous abort requests. The scheduler
   * uses this feature to stop operations asynchronously.
   */
   ...
   *
   *  This method should be called regularly. It does nothing if no stop request
   * if present. Otherwise it throws an exception which is supposed to make
   * \perform_task exit.
   */
  void checkpoint();
   ...
   *
   *  This is the same condition that will make checkpoint () throw an
   * exception.
   */
   ...
   */
  void set_idle(bool w) { m_is_idle = w; }
