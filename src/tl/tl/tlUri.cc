...
static std::string escape(const std::string &s) {
  ...
  std::string res;
  for (const char *cp = s.c_str(); *cp; ++cp) {
    if ((unsigned char)*cp <= 32 || (unsigned char)*cp >= 128 ||
        strchr(special, *cp) != 0) {
      res += "%";
      res += int2hex(*cp >> 4);
      res += int2hex(*cp);
...
URI::URI(const std::string &uri) {
  ...
  //  NOTE: to distinguish a windows drive letter from a scheme, we expect the
  //  scheme to be longer than one character
...
std::string URI::to_string() const {
  ...
  if (!m_query.empty()) {
    for (std::map<std::string, std::string>::const_iterator p = m_query.begin();
         p != m_query.end(); ++p) {
      res += (p == m_query.begin() ? "?" : "&");
      res += escape(p->first);
      if (!p->second.empty()) {
...
std::string URI::to_abstract_path() const {
  if (m_scheme.empty()) {
    return path();
  } else {
    return to_string();
