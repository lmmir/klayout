 ...
 *  using this tree.
 *  As specified by RFC1951, the code tree is constructed from a list of code
 * lengths vs. value alone.
 */
...
public:
  ...
  /**
   *  @brief Initialize the code tree with the fixed Huffmann code table for
   * literals/lengths
   *
   ...
   */
  template <class Iter> void init_codes(Iter begin_lengths, Iter end_lengths) {
   ...
   *
   *  This method takes the next value from the bit stream decoding the bits
   * with the code tree currently loaded.
   */
...
InflateFilter::InflateFilter(tl::InputStream &input)
    : m_input(input), m_b_insert(0), m_b_read(0), m_at_end(false),
      m_last_block(false),
...
const char *InflateFilter::get(size_t n) {
    ...
    if (new_block) {
      ...
      if (t == 0) {
        ...
        if (t == 1) {
          ...
          //  KLUDGE: should use a different decoder object, so we save time to
          //  do this:
          ...
          for (unsigned int i = 0; i < sizeof(hclengths) / sizeof(hclengths[0]);
               ++i) {
            hclengths[i] = 0;
          }
          ...
          ldecoder.init_codes(hclengths, hclengths + sizeof(hclengths) /
                                                         sizeof(hclengths[0]));
        ...
        throw tl::Exception(tl::to_string(tr("Invalid compression type: %d")),
                            t);
      }
...
DeflateFilter::DeflateFilter(tl::OutputStream &output)
  ...
  int err = deflateInit2(mp_stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
                         -15 /* == raw deflate data*/,
                         8 /* == default memory level */, Z_DEFAULT_STRATEGY);
  tl_assert(err == Z_OK);
}

DeflateFilter::~DeflateFilter() { delete mp_stream; }
