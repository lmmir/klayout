 ...
 *
 *  The mapping is given by a table of sorted values x and associated y which
 * are interpolated linearly. A data mapping object must be capable of
 * generating such a table. Data mapping expressions can be built by
 * concatenating data mapping operators.
 */
...
public:
   ...
   *
   *  Any reimplementation of this class must provide a list of x/y pairs sorted
   * by x. Mapping for x values outside this range is performed by constant
   * extrapolation.
   */
  virtual void
  generate_table(std::vector<std::pair<double, double>> &table) = 0;
 ...
 *
 *  The table is to be built by adding new entries, whose x values must be
 * sorted.
 */
class TL_PUBLIC TableDataMapping : public DataMappingBase {
public:
  /**
   *  @brief The constructor
   */
   ...
   *  x value that is data mapping is covering.
   */
  virtual double xmin() const { return m_xmin; }
   ...
   *  x value that is data mapping is covering.
   */
  virtual double xmax() const { return m_xmax; }
 ...
 *  y = o(i(x)).
 */
class TL_PUBLIC CombinedDataMapping : public DataMappingBase {
public:
  /**
   *  @brief Create a combined data mapping
   *
   *  @param o The outer data mapping. This object will be owned by
   * CombinedDataMapping.
   *  @param i The inner data mapping. This object will be owned by
   * CombinedDataMapping.
   */
 ...
 *
 *  This data mapping is composed of two mappings a and b and offers two
 * coefficients ca and cb and a constant c. The result is: y = ca * a(x) + cb *
 * b(x) + c.
 */
class TL_PUBLIC LinearCombinationDataMapping : public DataMappingBase {
public:
  /**
   *  @brief Create a combined data mapping
   *
   *  @param a The first data mapping. This object will be owned by
   * CombinedDataMapping.
   *  @param ca The first coefficient
   *  @param b The second data mapping. This object will be owned by
   * CombinedDataMapping. This parameter can be 0.
   *  @param cb The first coefficient
   *  @param c The constant.
   *
   *  If the b object is 0, the linear combination degenerates to y = ca * a(x)
   * + c. If the a and b object is 0, the linear combination degenerates to y =
   * c.
   */
  LinearCombinationDataMapping(double c, DataMappingBase *a = 0,
                               double ca = 1.0, DataMappingBase *b = 0,
                               double cb = 1.0);
...
public:
   ...
   *
   *  @param The underlying data mapping - will become owned by the
   * DataMappingLookupTable object.
   */
   ...
   *
   *  This method must be called to produce a lookup table with the given
   * granularity (resolution) of y values and the given x range. This method
   * must be called when the data mapping base object has been changed.
   *
   *  @param xmin The minimum x value covered. No x value below this value can
   * be mapped.
   *  @param xmax The maximum x value covered. No x value above this value can
   * be mapped.
   *  @param delta_y The y granularity. This determines the resolution of the
   * lookup table.
   *  @param ifactor the factor by which the integer table entries are
   * multiplied
   */
  void update_table(double xmin, double xmax, double delta_y,
