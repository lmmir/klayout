 ...
 */
template <class C> class list_node {
public:
  ...
  C *self() { return static_cast<C *>(this); }

  const C *self() const { return static_cast<const C *>(this); }
...
template <class C> class list_impl<C, false> {
public:
  ...
  bool empty() const { return m_head.mp_next == &m_back; }

  C *first() { return !empty() ? static_cast<C *>(m_head.mp_next) : 0; }

  const C *first() const {
    return !empty() ? static_cast<C *>(m_head.mp_next) : 0;
  }

  C *last() { return !empty() ? static_cast<C *>(m_back.mp_prev) : 0; }
  ...
  void insert(C *after, C *new_obj) { insert_impl(after, new_obj, true); }

  void insert_before(C *before, C *new_obj) {
    insert_before_impl(before, new_obj, true);
  }

  void push_back(C *new_obj) { push_back_impl(new_obj, true); }

  void push_front(C *new_obj) { push_front_impl(new_obj, true); }

  void insert(C *after, C &new_obj) { insert_impl(after, &new_obj, false); }

  void insert_before(C *before, C &new_obj) {
    insert_before_impl(before, &new_obj, false);
  }

  void push_back(C &new_obj) { push_back_impl(&new_obj, false); }

  void push_front(C &new_obj) { push_front_impl(&new_obj, false); }
...
protected:
  list_node<C> &head() { return m_head; }

  const list_node<C> &head() const { return m_head; }

  list_node<C> &back() { return m_back; }

  const list_node<C> &back() const { return m_back; }
...
template <class C> class list_impl<C, true> : public list_impl<C, false> {
public:
  ...
  list_impl(const list_impl &other) : list_impl<C, false>() {
    operator=(other);
  }
  ...
  void insert(C *after, const C &obj) { insert(after, new C(obj)); }

  void insert_before(C *before, const C &obj) {
    insert_before(before, new C(obj));
  }

  void push_back(const C &obj) { insert_before(0, new C(obj)); }

  void push_front(const C &obj) { insert(0, new C(obj)); }
};

/**
 *  @brief An iterator for the linked list
 */
template <class C> class list_iterator {
public:
 ...
 */
template <class C> class reverse_list_iterator {
public:
  typedef std::bidirectional_iterator_tag category;
  typedef C value_type;
  typedef C &reference;
  ...
  bool operator==(reverse_list_iterator other) const {
    return mp_p == other.mp_p;
  }
  bool operator!=(reverse_list_iterator other) const {
    return mp_p != other.mp_p;
  }
...
class list : public list_impl<C, CanCopy> {
public:
  ...
  iterator begin() { return iterator(static_cast<C *>(head().mp_next)); }

  iterator end() { return iterator(static_cast<C *>(&back())); }
  ...
  bool operator!=(const list &other) const { return !operator==(other); }
