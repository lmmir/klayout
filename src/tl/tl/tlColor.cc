...
Color::Color(color_t color) : m_color(color | 0xff000000) {
  //  .. nothing yet ..
}

#if defined(HAVE_QT)
Color::Color(const QColor &qc) : m_color(0) {
  if (qc.isValid()) {
    m_color = qc.rgba();
  }
}
...
Color::Color(unsigned int r, unsigned int g, unsigned int b, unsigned int alpha)
    : m_color((b & 0xff) | ((g & 0xff) << 8) | ((r & 0xff) << 16) |
              ((alpha & 0xff) << 24)) {
  //  .. nothing yet ..
}
...
void Color::init_from_string(const char *s) {
  ...
  if (n == 0) {
    ...
    m_color = ((m_color & 0xf) * 0x11) | ((m_color & 0xf0) * 0x110) |
              ((m_color & 0xf00) * 0x1100) | 0xff000000;
  } else if (n <= 4) {
    m_color = ((m_color & 0xf) * 0x11) | ((m_color & 0xf0) * 0x110) |
              ((m_color & 0xf00) * 0x1100) | ((m_color & 0xf000) * 0x11000);
  } else if (n <= 6) {
    m_color |= 0xff000000;
  }
}
...
#if defined(HAVE_QT)
QColor Color::to_qc() const { return is_valid() ? QColor(rgb()) : QColor(); }
#endif

bool Color::is_valid() const { return (m_color & 0xff000000) != 0; }

void Color::get_hsv(unsigned int &hue, unsigned int &saturation,
...
static tl::Color color_d(double r, double g, double b) {
  return tl::Color(tl::round(r * 255.0, 1), tl::round(g * 255.0, 1),
                   tl::round(b * 255.0, 1));
}

tl::Color Color::from_hsv(unsigned int hue, unsigned int saturation,
                          unsigned int value) {
  if (saturation == 0) {
