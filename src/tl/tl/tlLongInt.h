 ...
 */
template <unsigned int N, class B, class BI> class long_uint {
public:
   ...
   */
  template <class T> long_uint(T t) {
    unsigned int i = 0;
  ...
  /**
   *  @brief Initialize the universal unsigned int from another one with a
   * different width
   */
  template <unsigned int N2> long_uint(const long_uint<N2, B, BI> &o) {
    for (unsigned int i = 0; i < N; ++i) {
      b[i] = i < N2 ? o.b[i] : 0;
    }
  }
   ...
   *  @brief Inequality
   */
  bool operator!=(const long_uint<N, B, BI> &b) const { return !operator==(b); }
   ...
   *  @brief Less or equal
   */
  bool operator<=(const long_uint<N, B, BI> &b) const { return b >= *this; }
   ...
   *  element of the returned pair and the modulo value (remainder) in the
   * second.
   */
  std::pair<long_uint<N, B, BI>, long_uint<N, B, BI>>
  divmod(const long_uint<N, B, BI> &d) const {
...
class long_int : public long_uint<N, B, BI> {
public:
  /**
   *  @brief Default constructor
   *  This will initialize the value to 0.
   */
  long_int() : long_uint<N, B, BI>() {
    //  .. nothing yet ..
  }
  ...
  /**
   *  @brief Initialize the universal unsigned int from an unsigned one with a
   * different width Sign inversion will happen, if the unsigned int is bigger
   * than the maximum value representable by our type.
   */
  template <unsigned int N2>
  long_int(const long_uint<N2, B, BI> &o) : long_uint<N, B, BI>(o) {
    //  .. nothing yet ..
  }

  /**
   *  @brief Initialize the universal int from another one with a different
   * width
   */
  template <unsigned int N2>
  long_int(const long_int<N2, B, BI> &o) : long_uint<N, B, BI>(o) {
    //  .. nothing yet ..
  }
   ...
   */
  bool is_neg() const {
    return (long_uint<N, B, BI>::b[N - 1] &
            (1 << (long_uint<N, B, BI>::bits - 1))) != 0;
  }
   ...
   *  @brief Greater or equal
   */
  bool operator>=(const long_int<N, B, BI> &b) const { return !operator<(b); }
   ...
   *  @brief Less or equal
   */
  bool operator<=(const long_int<N, B, BI> &b) const { return b >= *this; }
   ...
   * second.
   */
  std::pair<long_int<N, B, BI>, long_int<N, B, BI>>
  divmod(const long_int<N, B, BI> &d) const {
    if (is_neg() && !d.is_neg()) {
      std::pair<long_uint<N, B, BI>, long_uint<N, B, BI>> res =
          (-*this).long_uint<N, B, BI>::divmod(d);
      return std::make_pair(-long_int<N, B, BI>(res.first),
                            -long_int<N, B, BI>(res.second));
    } else if (!is_neg() && d.is_neg()) {
      std::pair<long_uint<N, B, BI>, long_uint<N, B, BI>> res =
          long_uint<N, B, BI>::divmod(-d);
...
long_uint<N, B, BI> operator*(const long_uint<N, B, BI> &a,
                              const long_int<N2, B, BI> &b) {
  return a.operator*(b);
}
