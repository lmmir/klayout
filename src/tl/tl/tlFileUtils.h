...
/**
 *  @brief Returns a value indicating whether the parent path is a parent
 * directory of the path (version with std::string)
 */
bool TL_PUBLIC is_parent_path(const std::string &parent,
...
/**
 *  @brief Recursively remove the given directory, the files from that directory
 * and all sub-directories (version with std::string)
 ...
 *  @return True, if successful. false otherwise.
 */
bool TL_PUBLIC cp_dir_recursive(const std::string &source,
...
/**
 *  @brief Gets the filename for a given file path (file name without directory
 * part)
 */
std::string TL_PUBLIC filename(const std::string &s);

/**
 *  @brief Gets the basename for a given file path (file name without any
 * extensions) This will strip all extensions (i.e. "archive.tar.gz" will become
 * "archive").
 */
std::string TL_PUBLIC basename(const std::string &s);

/**
 *  @brief Gets the basename for a given file path (file name without the last
 * extensions) This will strip all extensions (i.e. "archive.tar.gz" will become
 * "archive.tar").
 */
 ...
 *  If the path is a directory, file_exists will return true, if the directory
 * exists.
 */
 ...
 */
std::vector<std::string> TL_PUBLIC dir_entries(const std::string &s,
                                               bool with_files = true,
                                               bool with_dirs = true,
                                               bool without_dotfiles = false);

/**
 *  @brief Rename the given file
 */
bool TL_PUBLIC rename_file(const std::string &path,
...
/**
 *  @brief Returns true, if the given path is the same directory of file than
 * the other one
 */
 ...
 */
std::string TL_PUBLIC relative_path(const std::string &base,
                                    const std::string &p);
...
/**
 *  @brief Change the current directory and returns true if the change was
 * successful
 */
 ...
 */
std::vector<std::string> TL_PUBLIC split_path(const std::string &p,
...
/**
 *  @brief Gets the absolute path of the module (DLL/.so) which contains the
 * given address "address" is supposed to be the address of a function inside
 * the module.
 */
