...
template <class T> struct _var_user_clone_impl<T, true> {
  static T *call(const T *a) { return new T(*a); }
};

template <class T> struct _var_user_clone_impl<T, false> {
...
template <class T> struct _var_user_assign_impl<T, true> {
  static void call(T *a, const T *b) { *a = *b; }
};

template <class T> struct _var_user_assign_impl<T, false> {
...
/**
 *  @brief A utility implementation of tl::VariantUserClass using type traits
 * for the implementation
 */
template <class T> class VariantUserClassImpl : public tl::VariantUserClass<T> {
public:
  virtual void *create() const { return new T(); }

  virtual void destroy(void *a) const { delete (T *)a; }

  virtual bool equal(const void *a, const void *b) const {
    return _var_user_equal_impl<T, tl::has_equal_operator<T>::value>::call(
        (const T *)a, (const T *)b);
  }

  virtual bool less(const void *a, const void *b) const {
    return _var_user_less_impl<T, tl::has_less_operator<T>::value>::call(
        (const T *)a, (const T *)b);
  }

  virtual void *clone(const void *a) const {
    return _var_user_clone_impl<T, std::is_copy_constructible<T>::value>::call(
        (const T *)a);
  }

  virtual void assign(void *a, const void *b) const {
    _var_user_assign_impl<T, std::is_copy_assignable<T>::value>::call(
        (T *)a, (const T *)b);
  }

  virtual std::string to_string(const void *a) const {
    return _var_user_to_string_impl<T, tl::has_to_string<T>::value>::call(
        (const T *)a);
  }

  virtual int to_int(const void *a) const {
    return _var_user_to_int_impl<T, tl::has_to_int<T>::value>::call(
        (const T *)a);
  }

  virtual double to_double(const void *a) const {
    return _var_user_to_double_impl<T, tl::has_to_double<T>::value>::call(
        (const T *)a);
  }

  virtual void to_variant(const void *a, tl::Variant &v) const {
    v = _var_user_to_variant_impl<T, tl::has_to_variant<T>::value>::call(
        (const T *)a);
  }

  virtual void read(void *a, tl::Extractor &ex) const { ex.read(*(T *)a); }
  ...
  virtual void *deref_proxy(tl::Object *obj) const {
    //  By default, the tl::Object is considered to be the first base class of
    //  the actual object
    return obj;
  }
