 ...
 *  main thread can be terminated in the debugger through ExitException
 * unconditionally, cleanup() provides a way to implement actions after such an
 * event.
 */
 ...
 */
class TL_PUBLIC ExecutableWithParameters : public Executable {
public:
   ...
   */
  template <class T>
  static T get_value(const std::map<std::string, tl::Variant> &params,
                     const std::string &pname, const T &def_value) {
 ...
 */
class TL_PUBLIC Recipe : public tl::RegisteredClass<Recipe> {
public:
  /**
   ...
   *
   *  Returns nil if the recipe can't be executed, e.g. because the recipe isn't
   * known. Additional parameters can be passed in the second argument. They
   * have lower priority than the parameters kept in the generator argument.
   */
  ...
  /**
   *  @brief Returns the executable object which actually implements the action
   * to take
   *
   *  The returned object is deleted by the caller.
   */
  virtual Executable *
  executable(const std::map<std::string, tl::Variant> &params) const = 0;
