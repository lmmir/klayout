...
public:
  XMLSourcePrivateData(tl::InputStream *stream)
      : mp_stream_holder(stream), m_has_error(false) {
    mp_stream = stream;
  }
  ...
  XMLSourcePrivateData(tl::InputStream &stream) : m_has_error(false) {
    mp_stream = &stream;
  }
  ...
  const std::string &error_msg() const { return m_error; }

  void reset() { mp_stream->reset(); }
...
XMLSource::XMLSource() : mp_source(0) {
  //  .. nothing yet ..
}
...
void XMLSource::reset() { mp_source->reset(); }

// --------------------------------------------------------------------
//  XMLStringSource implementation

XMLStringSource::XMLStringSource(const std::string &string) : m_copy(string) {
  set_source(new XMLSourcePrivateData(new tl::InputStream(
      new tl::InputMemoryStream(m_copy.c_str(), string.size()))));
}

XMLStringSource::XMLStringSource(const char *cp) {
  set_source(new XMLSourcePrivateData(
      new tl::InputStream(new tl::InputMemoryStream(cp, strlen(cp)))));
}

XMLStringSource::XMLStringSource(const char *cp, size_t len) {
  set_source(new XMLSourcePrivateData(
      new tl::InputStream(new tl::InputMemoryStream(cp, len))));
}
...
XMLFileSource::XMLFileSource(const std::string &path,
                             const std::string &progress_message) {
  set_source(
      new XMLSourcePrivateData(new tl::InputStream(path), progress_message));
}
...
void XMLCALL start_element_handler(void *user_data, const XML_Char *name,
                                   const XML_Char **atts);
...
public:
  XMLParserPrivateData() : mp_struct_handler(0) {
  ...
  void parse(tl::XMLSource &source, XMLStructureHandler &struct_handler) {
    ...
    XML_SetElementHandler(mp_parser, start_element_handler,
                          end_element_handler);
    ...
    do {
      ...
      try {
        ...
        XML_Status status =
            XML_Parse(mp_parser, buffer, int(n), n < chunk /*is final*/);
...
void start_element_handler(void *user_data, const XML_Char *name,
                           const XML_Char ** /*atts*/) {
...
XMLParser::XMLParser() : mp_data(new XMLParserPrivateData()) {
  //  .. nothing yet ..
}
...
bool XMLParser::is_available() { return true; }
...
bool SAXHandler::startElement(const QString &qs_uri, const QString &qs_lname,
                              const QString &qs_qname,
  ...
  try {
    ...
    throw tl::XMLLocatedException(ex.raw_msg(), mp_locator->lineNumber(),
                                  mp_locator->columnNumber());
  } catch (tl::Exception &ex) {
    throw tl::XMLLocatedException(ex.msg(), mp_locator->lineNumber(),
                                  mp_locator->columnNumber());
  }
...
bool SAXHandler::endElement(const QString &qs_uri, const QString &qs_lname,
                            const QString &qs_qname) {
  std::string uri(tl::to_string(qs_uri));
  std::string lname(tl::to_string(qs_lname));
  std::string qname(tl::to_string(qs_qname));
...
class XMLSourcePrivateData : public QXmlInputSource {
public:
  XMLSourcePrivateData() : QXmlInputSource() {
    //  .. nothing yet ..
  }

  XMLSourcePrivateData(QIODevice *dev) : QXmlInputSource(dev) {
    //  .. nothing yet ..
  }
};
...
public:
  StreamIODevice(tl::InputStream &stream)
      : mp_stream(&stream), mp_progress(0), m_has_error(false) {
  ...
  StreamIODevice(const std::string &path)
      : mp_stream_holder(new tl::InputStream(path)), mp_progress(0),
        m_has_error(false) {
  ...
  qint64 writeData(const char *, qint64) { tl_assert(false); }
  ...
  qint64 readData(char *data, qint64 n) {
    try {
      ...
      for (const char *rd = 0; n > 0 && (rd = mp_stream->get(1)) != 0;) {
        //  NOTE: we skip CR to compensate for Windows CRLF line terminators
        //  (issue #419).
        if (*rd != '\r') {
          *data++ = *rd;
          --n;
...
class XMLStreamSourcePrivateData : public XMLSourcePrivateData {
public:
  ...
  virtual void fetchData() {
    ...
    //  This feature is actually missing in the original implementation: throw
...
XMLFileSource::XMLFileSource(const std::string &path,
                             const std::string &progress_message) {
  set_source(new XMLStreamSourcePrivateData(
      new StreamIODevice(path, progress_message)));
}
...
XMLStreamSource::XMLStreamSource(tl::InputStream &s,
                                 const std::string &progress_message) {
  set_source(
      new XMLStreamSourcePrivateData(new StreamIODevice(s, progress_message)));
}
...
class XMLParserPrivateData : public QXmlSimpleReader {
public:
...
XMLStringSource::XMLStringSource(const std::string &) { tl_assert(false); }

XMLStringSource::XMLStringSource(const char *) { tl_assert(false); }

XMLStringSource::XMLStringSource(const char *, size_t) { tl_assert(false); }
...
XMLFileSource::XMLFileSource(const std::string &) { tl_assert(false); }
...
XMLStreamSource::XMLStreamSource(tl::InputStream &) { tl_assert(false); }
...
void XMLParser::parse(XMLSource &, XMLStructureHandler &) { tl_assert(false); }

bool XMLParser::is_available() { return false; }
...
XMLElementProxy::XMLElementProxy(const XMLElementBase &d) : mp_ptr(d.clone()) {
  //  .. nothing yet ..
}

XMLElementProxy::XMLElementProxy(XMLElementBase *d) : mp_ptr(d) {
  //  .. nothing yet ..
}
...
XMLStructureHandler::XMLStructureHandler(const XMLElementBase *root,
                                         XMLReaderState *reader_state)
    : mp_root(root), mp_state(reader_state) {
  // .. nothing yet ..
}

void XMLStructureHandler::start_element(const std::string &uri,
  ...
  if (m_stack.size() == 0) {
    if (!mp_root->check_name(uri, lname, qname)) {
      throw tl::XMLException(tl::to_string(tr("Root element must be ")) +
                             mp_root->name());
    }
    new_element = mp_root;
  } else {
    parent = m_stack.back();
    if (parent) {
      for (XMLElementBase::iterator c = parent->begin(); c != parent->end();
           ++c) {
...
void XMLStructureHandler::end_element(const std::string &uri,
                                      const std::string &lname,
                                      const std::string &qname) {
  if (m_stack.empty()) {
    return;
...
XMLReaderState::~XMLReaderState() {
  for (std::vector<XMLReaderProxyBase *>::const_iterator o = m_objects.begin();
       o != m_objects.end(); ++o) {
    (*o)->release();
    delete *o;
  }
