 ...
 */
    class DefaultDeferredMethodScheduler : public DeferredMethodScheduler {
    public:
...
// -----------------------------------------------------------------------------------
    ...
    DeferredMethodScheduler::~DeferredMethodScheduler() { s_inst = 0; }

    DeferredMethodScheduler *DeferredMethodScheduler::instance() {
...
//  TODO: provide a way to register non-Qt schedulers
    ...
    bool DeferredMethodScheduler::is_disabled() const { return m_disabled; }
    ...
    void DeferredMethodScheduler::unqueue(DeferredMethodBase *method) {
        tl::MutexLocker locker(&m_lock);
        for (std::list<DeferredMethodBase *>::iterator m = m_methods.begin();
             m != m_methods.end();) {
        ...
        for (std::list<DeferredMethodBase *>::iterator m = m_executing.begin();
             m != m_executing.end(); ++m) {
            if (*m == method) {
    ...
    bool DeferredMethodScheduler::do_execute() {
        ...
        //  do the execution outside the locked range to avoid deadlocks if the
        //  method's execution schedules another call.
        for (std::list<DeferredMethodBase *>::iterator m = m_executing.begin();
             m != m_executing.end(); ++m) {
            ...
            m_lock.lock();
            //  during execution a method may be unqueued - make sure this is not
            //  executed
            ...
            if (still_valid) {
                ...
                //  execute() may have triggered another do_execute which we should
                //  consider here and stop:
                if (m_executing.empty()) {
                    //当execute嵌套执行其他延时方法
                    // 多线程场景或者execute中的消息嵌套消息有eventloop或者QApplication::processEvents()的时候，可能会进入此逻辑。
                    break;
                }
            }
        }
