...
ArgBase::ArgBase(const std::string &option, const std::string &brief_doc,
...
std::string ArgBase::option_desc() const {
  std::string res;
...
class HelpArg : public ArgBase {
public:
  HelpArg() : ArgBase("-h|--help", "Shows the usage and exits", "") {
    //  .. nothing yet ..
  }

  ArgBase *clone() const { return new HelpArg(); }
...
class AdvancedHelpArg : public ArgBase {
public:
  AdvancedHelpArg()
      : ArgBase("/--help-all",
                "Shows all options (including advanced) and exits", "") {
    //  .. nothing yet ..
  }

  ArgBase *clone() const { return new AdvancedHelpArg(); }
...
class LicenseArg : public ArgBase {
public:
  LicenseArg() : ArgBase("--license", "Shows the license and exits", "") {
    //  .. nothing yet ..
  }

  ArgBase *clone() const { return new LicenseArg(); }
...
class VersionArg : public ArgBase {
public:
  VersionArg() : ArgBase("--version", "Shows the version and exits", "") {
    //  .. nothing yet ..
  }

  ArgBase *clone() const { return new VersionArg(); }
...
class VerbosityArg : public ArgBase {
public:
  VerbosityArg()
  ...
  ArgBase *clone() const { return new VerbosityArg(); }

  bool wants_value() const { return true; }
...
CommandLineOptions::CommandLineOptions() {
  ...
  *this << HelpArg() << AdvancedHelpArg() << VersionArg() << LicenseArg()
        << VerbosityArg();
}

CommandLineOptions::~CommandLineOptions() {
  for (std::vector<ArgBase *>::const_iterator a = m_args.begin();
       a != m_args.end(); ++a) {
    delete *a;
  }
  m_args.clear();
...
CommandLineOptions &CommandLineOptions::operator<<(const ArgBase &a) {
  m_args.push_back(a.clone());
  return *this;
}

static void print_string_formatted(const std::string &indent,
...
struct NameCompare {
  bool operator()(ArgBase *a, ArgBase *b) {
    ...
    if (a->option().short_option.empty() != b->option().short_option.empty()) {
      return a->option().short_option.empty() <
             b->option().short_option.empty();
    }
...
void CommandLineOptions::produce_help(const std::string &program_name,
                                      bool advanced) {
  int columns = 70;
  ...
  for (std::vector<ArgBase *>::const_iterator a = sorted_args.begin();
       a != sorted_args.end(); ++a) {
    if (!(*a)->is_option()) {
  ...
  for (std::vector<ArgBase *>::const_iterator a = sorted_args.begin();
    ...
    tl::info << "    " << pad_string_right(arg_width + 4, (*a)->option_desc())
             << (*a)->brief_doc();
    tl::info << "";
...
void CommandLineOptions::produce_license() { tl::info << m_license; }

void CommandLineOptions::produce_version() { tl::info << m_version; }
...
void CommandLineOptions::parse(int argc, char *argv[]) {
  ...
  for (std::vector<ArgBase *>::const_iterator i = m_args.begin();
       i != m_args.end(); ++i) {
    if ((*i)->is_option()) {
      if (!(*i)->option().short_option.empty()) {
        if (arg_by_short_option.find((*i)->option().short_option) !=
            arg_by_short_option.end()) {
          throw tl::Exception("Command line parser setup: duplicate option -" +
                              (*i)->option().short_option);
        }
        arg_by_short_option.insert(
            std::make_pair((*i)->option().short_option, *i));
      }
      if (!(*i)->option().long_option.empty()) {
        if (arg_by_long_option.find((*i)->option().long_option) !=
            arg_by_long_option.end()) {
          throw tl::Exception("Command line parser setup: duplicate option --" +
                              (*i)->option().long_option);
        }
        arg_by_long_option.insert(
            std::make_pair((*i)->option().long_option, *i));
      }
    } else {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (ex.test("--")) {
      ...
      std::string n;
      ex.read_word(n, "_-");
      std::map<std::string, ArgBase *>::const_iterator a =
          arg_by_long_option.find(n);
      if (a == arg_by_long_option.end()) {
        throw tl::Exception(tl::sprintf(
            tl::to_string(
                tr("Unknown command line option --%s (use -h for help)")),
            n));
      }
      arg = a->second;
      ...
      if (next_plain_arg == plain_args.end()) {
        throw tl::Exception(tl::sprintf(
            tl::to_string(tr("Unknown command line component %s - no further "
                             "plain argument expected (use -h for help)")),
            arg_as_utf8));
      }
  ...
  if (next_plain_arg != plain_args.end() &&
      !(*next_plain_arg)->option().optional) {
    throw tl::Exception(
        tl::to_string(tr("Additional arguments required (use -h for help)")));
  }
}
