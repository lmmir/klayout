...
class TL_PUBLIC Channel : public tl::Object {
public:
  /**
   ...
   */
  template <class T> ChannelProxy operator<<(const T &t);
...
template <class T> inline ChannelProxy Channel::operator<<(const T &t) {
  ChannelProxy p = issue_proxy();
  puts(tl::to_string(t).c_str());
  return p;
}

inline ChannelProxy Channel::operator<<(const char *s) {
  ChannelProxy p = issue_proxy();
  puts(s);
  return p;
}

inline ChannelProxy Channel::operator<<(ChannelEndl) {
  ChannelProxy p = issue_proxy();
  endl();
  return p;
}
...
/// The log channel is identical to the info channel but is silent depending on
/// the verbosity and the output mode. It should be used for general
/// notifications like the beginning of a operation.
