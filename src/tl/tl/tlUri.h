...
public:
  ...
  /**
   *  @brief Returns the authority part or an empty string if there is no
   * authority The leading slashes and not part of the authority. Percent
   ...
   */
  void set_authority(const std::string &s) { m_authority = s; }
   ...
   * (non-const version)
   */
  std::map<std::string, std::string> &query() { return m_query; }
   ...
   */
  void set_fragment(const std::string &s) { m_fragment = s; }
   ...
   *  URIs with scheme "file", "http" and "https" are equivalent to their
   * abstract path. URIs without a scheme turn into system file paths. Other
   * schemes are not allowed.
   *
   *  Abstract paths are more powerful as they support pipes and Qt resource
   * access. These modes are not supported by URIs.
   */
