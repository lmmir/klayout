...
/**
 *  @brief A utility class forming clusters based on equivalence of a certain
 * attribute
 *
 ...
 */
template <class T> class equivalence_clusters {
public:
  typedef size_t cluster_id_type;
  typedef T attribute_type;
  typedef typename std::vector<
      typename std::map<T, size_t>::iterator>::const_iterator cluster_iterator;
  ...
  /**
   *  @brief Returns the cluster ID for the given attribute of 0 if the
   * attribute is not assigned to a cluster
   */
  cluster_id_type cluster_id(const T &attr) const {
    typename std::map<T, size_t>::const_iterator c =
        m_cluster_id_by_attr.find(attr);
   ...
   *
   *  This method will join clusters already within this equivalence cluster
   * collection based on the equivalences listed in the other clusters
   * collection.
   *
  ...
  void apply_equivalences(const equivalence_clusters &other) {
    std::vector<T> attrs;
    for (typename std::map<T, size_t>::const_iterator a =
             m_cluster_id_by_attr.begin();
         a != m_cluster_id_by_attr.end(); ++a) {
      if (other.has_attribute(a->first)) {
        attrs.push_back(a->first);
      }
    }

    for (typename std::vector<T>::const_iterator a = attrs.begin();
         a != attrs.end(); ++a) {
   ...
   *
   *  This method will add all attributes from the other cluster collection and
   * join clusters based on the equivalences there.
   */
...
private:
  void insert(const T &attr, cluster_id_type into) {
    typename std::map<T, size_t>::iterator c =
        m_cluster_id_by_attr.insert(std::make_pair(attr, into)).first;
  ...
  void join(cluster_id_type id, cluster_id_type with_id) {
    tl_assert(id > 0);
    tl_assert(with_id > 0);
    std::vector<typename std::map<T, size_t>::iterator> &cnew =
        m_clusters[id - 1];
    std::vector<typename std::map<T, size_t>::iterator> &c =
        m_clusters[with_id - 1];
    for (typename std::vector<typename std::map<T, size_t>::iterator>::iterator
             i = c.begin();
         i != c.end(); ++i) {
      (*i)->second = id;
      cnew.push_back(*i);
  ...
  cluster_id_type new_cluster() {
    if (!m_free_slots.empty()) {
      ...
      m_clusters.push_back(
          std::vector<typename std::map<T, size_t>::iterator>());
