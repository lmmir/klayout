...
static void png_read_warn_f(png_structp /*png_ptr*/,
                            png_const_charp error_message) {
  tl::warn << tl::to_string(tr("Warning reading PNG: ")) << error_message;
}

static void png_read_error_f(png_structp /*png_ptr*/,
                             png_const_charp error_message) {
  throw PixelBufferReadError(error_message);
}

static void png_write_warn_f(png_structp /*png_ptr*/,
                             png_const_charp error_message) {
  tl::warn << tl::to_string(tr("Warning writing PNG: ")) << error_message;
}

static void png_write_error_f(png_structp /*png_ptr*/,
                              png_const_charp error_message) {
  throw PixelBufferReadError(error_message);
}

static void read_from_stream_f(png_structp png_ptr, png_bytep bytes,
                               size_t length) {
...
static void write_to_stream_f(png_structp png_ptr, png_bytep bytes,
                              size_t length) {
...
PixelBuffer::PixelBuffer(unsigned int w, unsigned int h,
                         const tl::color_t *data, unsigned int stride)
    : m_data() {
  m_width = w;
  m_height = h;
  m_transparent = false;
...
PixelBuffer::PixelBuffer(const PixelBuffer &other) { operator=(other); }

PixelBuffer::PixelBuffer(PixelBuffer &&other) { swap(other); }
...
void PixelBuffer::set_transparent(bool f) { m_transparent = f; }
...
tl::color_t *PixelBuffer::scan_line(unsigned int n) {
  tl_assert(n < m_height);
  return m_data->data() + n * m_width;
}

const tl::color_t *PixelBuffer::scan_line(unsigned int n) const {
  tl_assert(n < m_height);
  return m_data->data() + n * m_width;
}

tl::color_t *PixelBuffer::data() { return m_data->data(); }

const tl::color_t *PixelBuffer::data() const { return m_data->data(); }

#if defined(HAVE_QT)
QImage PixelBuffer::to_image() const {
  return QImage((const uchar *)data(), m_width, m_height,
                m_transparent ? QImage::Format_ARGB32 : QImage::Format_RGB32);
}

QImage PixelBuffer::to_image_copy() const {
  QImage img(m_width, m_height,
             m_transparent ? QImage::Format_ARGB32 : QImage::Format_RGB32);
...
PixelBuffer PixelBuffer::from_image(const QImage &img) {
  if (img.format() != QImage::Format_ARGB32 &&
      img.format() != QImage::Format_RGB32) {
    QImage img_argb32 = img.convertToFormat(QImage::Format_ARGB32);
    return PixelBuffer(img_argb32.width(), img_argb32.height(),
                       (const tl::color_t *)img_argb32.bits());
  } else {
    return PixelBuffer(img.width(), img.height(),
                       (const tl::color_t *)img.bits());
  }
}
#endif
...
PixelBuffer PixelBuffer::diff(const PixelBuffer &other) const {
  tl_assert(width() == other.width());
  tl_assert(height() == other.height());
...
void PixelBuffer::subsample(tl::PixelBuffer &dest, unsigned int os, double g) {
  ...
  for (unsigned int i = 0; i < 256; ++i) {
    ...
    lut1[i] = (unsigned short)std::min(
        f, std::max(0.0, floor(0.5 + pow(i / 255.0, g) * f)));
  }
  ...
  for (unsigned int i = 0; i < 65536; ++i) {
    ...
    lut2[i] = (unsigned char)std::min(
        255.0, std::max(0.0, floor(0.5 + pow(i / f, 1.0 / g) * 255.0)));
  }
  ...
  for (unsigned int i = 0; i < 256; ++i) {
    double f = (65536 / (os * os)) - 1;
    luta1[i] = (unsigned short)std::min(
        f, std::max(0.0, floor(0.5 + (i / 255.0) * f)));
  }
  ...
  for (unsigned int i = 0; i < 65536; ++i) {
    double f = os * os * ((65536 / (os * os)) - 1);
    luta2[i] = (unsigned char)std::min(
        255.0, std::max(0.0, floor(0.5 + (i / f) * 255.0)));
  }
...
PixelBuffer PixelBuffer::read_png(tl::InputStream &input) {
  png_structp png_ptr = NULL;
  png_infop info_ptr = NULL;

  png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL,
                                   &png_read_error_f, &png_read_warn_f);
  ...
  PixelBuffer res(png_get_image_width(png_ptr, info_ptr),
  ...
  if (fmt == PNG_COLOR_TYPE_RGBA && bd == 8) {
    ...
    tl_assert(png_get_rowbytes(png_ptr, info_ptr) ==
              res.width() * sizeof(tl::color_t));

    png_bytepp row_pointers = png_get_rows(png_ptr, info_ptr);
    for (unsigned int i = 0; i < res.height(); ++i) {
      memcpy((void *)res.scan_line(i), (void *)row_pointers[i],
             sizeof(tl::color_t) * res.width());
    }
    ...
    //  GA format has 2 bytes per pixel (alpha, gray) which need to be
    //  transformed into ARGB
    ...
    //  G format has 1 byte per pixel (gray) which need to be transformed into
    //  ARGB
    ...
    throw PixelBufferReadError(tl::sprintf(
        tl::to_string(tr("PNG reader supports 8 bit G, GA, RGB or RGBA files "
                         "only (file: %s, format is %d, bit depth is %d)")),
        input.filename(), fmt, bd));
  }
...
void PixelBuffer::write_png(tl::OutputStream &output) const {
  ...
  png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL,
                                    &png_write_error_f, &png_write_warn_f);
  tl_assert(png_ptr != NULL);

  info_ptr = png_create_info_struct(png_ptr);
  tl_assert(info_ptr != NULL);

  png_set_write_fn(png_ptr, (void *)&output, &write_to_stream_f,
                   &flush_stream_f);
  png_set_bgr(png_ptr); // compatible with tl::color_t
  ...
  png_set_IHDR(png_ptr, info_ptr, width(), height(), bd, fmt,
...
static unsigned int stride_from_width(unsigned int w) {
...
BitmapBuffer::BitmapBuffer(unsigned int w, unsigned int h, const uint8_t *data,
                           unsigned int stride) {
  m_width = w;
  m_height = h;
  m_stride = stride_from_width(w);
...
BitmapBuffer::BitmapBuffer(const BitmapBuffer &other) { operator=(other); }

BitmapBuffer::BitmapBuffer(BitmapBuffer &&other) { swap(other); }
...
BitmapBuffer &BitmapBuffer::operator=(BitmapBuffer &&other) {
  if (this != &other) {
    swap(other);
  }
  return *this;
...
uint8_t *BitmapBuffer::scan_line(unsigned int n) {
  tl_assert(n < m_height);
  return m_data->data() + n * m_stride;
}

const uint8_t *BitmapBuffer::scan_line(unsigned int n) const {
  tl_assert(n < m_height);
  return m_data->data() + n * m_stride;
}

uint8_t *BitmapBuffer::data() { return m_data->data(); }

const uint8_t *BitmapBuffer::data() const { return m_data->data(); }

#if defined(HAVE_QT)
QImage BitmapBuffer::to_image() const {
  QImage img =
      QImage((const uchar *)data(), m_width, m_height, QImage::Format_MonoLSB);
  img.setColor(0, 0xff000000);
  img.setColor(1, 0xffffffff);
  return img;
}

QImage BitmapBuffer::to_image_copy() const {
...
BitmapBuffer BitmapBuffer::from_image(const QImage &img) {
  if (img.format() != QImage::Format_MonoLSB) {
    QImage img_monolsb = img.convertToFormat(QImage::Format_MonoLSB);
    return BitmapBuffer(img_monolsb.width(), img_monolsb.height(),
                        (const uint8_t *)img_monolsb.bits());
  } else {
...
BitmapBuffer BitmapBuffer::read_png(tl::InputStream &input) {
  png_structp png_ptr = NULL;
  png_infop info_ptr = NULL;
  ...
  BitmapBuffer res(png_get_image_width(png_ptr, info_ptr),
                   png_get_image_height(png_ptr, info_ptr));

  unsigned int fmt = png_get_color_type(png_ptr, info_ptr);
  unsigned int bd = png_get_bit_depth(png_ptr, info_ptr);

  if ((fmt == PNG_COLOR_TYPE_GRAY || fmt == PNG_COLOR_TYPE_PALETTE) &&
      bd == 1) {
    ...
    png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
    throw PixelBufferReadError(tl::sprintf(
        tl::to_string(tr("PNG bitmap reader supports monochrome files only "
                         "(file: %s, format is %d, bit depth is %d)")),
        input.filename(), fmt, bd));
  }
