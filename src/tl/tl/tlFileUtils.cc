...
const char *line_separator() { return is_win() ? "\r\n" : "\n"; }
...
std::vector<std::string> split_path(const std::string &p, bool keep_last) {
  ...
  if (is_win()) {
    ...
    } else if ((*cp == '\\' || *cp == '/') && cp[1] && isalpha(cp[1]) &&
               cp[2] == ':') {
...
static std::vector<std::string> split_filename(const std::string &fn) {
  ...
  while (*cp) {
    ...
    ++cp;
    while (*cp && *cp != '.') {
      //  backslash escaping (ineffective on Windows because that is a path
      //  separator)
      if (*cp == '\\' && cp[1]) {
        ++cp;
      }
...
bool is_parent_path(const std::string &parent, const std::string &path) {
  if (!file_exists(parent)) {
    //  If the parent path does not exist, we always return false. This cannot
    //  be a parent.
    return false;
  }
  ...
  return (is_same_file(parent, tl::combine_path(tl::join(parts, ""), "",
                                                true /*always add slash*/)));
}

std::vector<std::string> dir_entries(const std::string &s, bool with_files,
                                     bool with_dirs, bool without_dotfiles) {
  ...
  if (h != -1) {
    ...
    do {
      ...
      if ((e[0] != '.' || !without_dotfiles) &&
          ((is_dir && with_dirs) || (!is_dir && with_files))) {
        ee.push_back(e);
      }
...
#if defined(_WIN32)
  return _wrename(tl::to_wstring(path).c_str(),
                  tl::to_wstring(new_path).c_str()) == 0;
#else
  return rename(tl::to_local(path).c_str(), tl::to_local(new_path).c_str()) ==
         0;
#endif
}
...
bool rm_dir_recursive(const std::string &p) {
  ...
  for (std::vector<std::string>::const_iterator e = entries.begin();
       e != entries.end(); ++e) {
...
#if defined(FILE_UTILS_VERBOSE)
      tl::error << tr("Unable to copy file ") << tl::combine_path(path_to, *e)
                << tr(" to ") << tl::combine_path(path, *e) << tr("(Error ")
                << ex.msg() << ")";
#endif
      return false;
    }
...
absolute_path_of_existing(const std::string &s) {
#if defined(_WIN32)
...
bool is_same_file(const std::string &a, const std::string &b) {
  ...
  HANDLE h1 =
      ::CreateFileW(tl::to_wstring(a).c_str(), 0,
                    FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
                    NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
  HANDLE h2 =
      ::CreateFileW(tl::to_wstring(b).c_str(), 0,
                    FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
                    NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
std::string get_home_path() {
  ...
  tl::warn << tl::to_string(
      tr("Unable to get home directory (set HOME environment variable)"));
#else
  ...
  tl::warn << tl::to_string(
      tr("Unable to get home directory (no HOMEDRIVE/HOMEPATH, "
         "HOMESHARE/HOMEPATH or USERPROFILE environment variables)"));
#endif
  return std::string(".");
}

static std::string get_inst_path_internal() {
#if defined(_WIN32)
  ...
  if (ret > 0) {
    //  TODO: does this correctly translate paths? (MacOS uses UTF-8 encoding
    //  with D-like normalization)
...
std::string get_module_path(void *addr) {
#if defined(_WIN32)

  HMODULE h_module = NULL;
  if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                            GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                        (LPCTSTR)addr, &h_module)) {

    wchar_t buffer[MAX_PATH];
    int len;
    if ((len = GetModuleFileNameW(h_module, buffer, MAX_PATH)) > 0) {
      return tl::absolute_file_path(
          tl::to_string(std::wstring(buffer, 0, len)));
    }
  }
  ...
  if (dladdr(addr, &info)) {
    ...
    tl::warn << tl::to_string(tr(
        "Unable to get path of db library (as basis for loading db_plugins)"));
    return std::string();
  }
