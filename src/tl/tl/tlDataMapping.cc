...
inline double
interpolate(const std::vector<std::pair<double, double>> &v,
  ...
  if (i == v.end()) {
    ...
    return i[-1].second + (x - i[-1].first) * (i->second - i[-1].second) /
                              (i->first - i[-1].first);
  }
}
...
inline double interpolate(const std::vector<std::pair<double, double>> &v,
                          double x) {
  std::vector<std::pair<double, double>>::const_iterator i =
      std::lower_bound(v.begin(), v.end(), std::make_pair(x, 0.0),
                       compare_first_double_of_pair());
...
double CombinedDataMapping::xmin() const { return mp_i->xmin(); }

double CombinedDataMapping::xmax() const { return mp_i->xmax(); }

void CombinedDataMapping::generate_table(
  ...
  table.push_back(
      std::make_pair(ti.front().first, interpolate(to, ti.front().second)));

  for (std::vector<std::pair<double, double>>::const_iterator t =
           ti.begin() + 1;
       t != ti.end(); ++t) {
    ...
    std::vector<std::pair<double, double>>::const_iterator tt1 =
        std::lower_bound(to.begin(), to.end(), std::make_pair(y1, 0.0),
                         compare_first_double_of_pair());
    std::vector<std::pair<double, double>>::const_iterator tt2 =
        std::lower_bound(to.begin(), to.end(), std::make_pair(y2, 0.0),
                         compare_first_double_of_pair());

    while (tt1 < tt2) {

      //  Hint: can we be sure that tt1 == tt2 if y2 == y1 thus always y2 != y1
      //  in this case?
      double y = tt1->first;
  ...
  for (std::vector<std::pair<double, double>>::const_iterator t = table.begin();
       t != table.end(); ++t) {
    if (t + 1 != table.end() && t->first + epsilon > t[1].first) {
      *tw = std::make_pair(0.5 * (t->first + t[1].first),
                           0.5 * (t->second + t[1].second));
      ++t;
    } else {
      *tw = *t;
    }
...
LinearCombinationDataMapping::LinearCombinationDataMapping(
    double c, DataMappingBase *a, double ca, DataMappingBase *b, double cb)
...
double LinearCombinationDataMapping::xmin() const {
  if (!mp_a) {
...
double LinearCombinationDataMapping::xmax() const {
  if (!mp_a) {
...
void LinearCombinationDataMapping::generate_table(
    std::vector<std::pair<double, double>> &table) {
  if (!mp_a) {
    ...
    for (std::vector<std::pair<double, double>>::iterator t = table.begin();
         t != table.end(); ++t) {
    ...
    while (a != ta.end() || b != tb.end()) {
      ...
      if (a == ta.end()) {
        table.push_back(std::make_pair(b->first, m_c + m_ca * ta.back().second +
                                                     m_cb * b->second));
        ++b;
      } else if (b == tb.end()) {
        table.push_back(std::make_pair(a->first, m_c + m_ca * a->second +
                                                     m_cb * tb.back().second));
        ++a;
      } else if (a->first < b->first - epsilon) {
        table.push_back(
            std::make_pair(a->first, m_c + m_ca * a->second +
                                         m_cb * interpolate(tb, b, a->first)));
        ++a;
      } else if (a->first > b->first + epsilon) {
        table.push_back(
            std::make_pair(b->first, m_c + m_ca * interpolate(ta, a, b->first) +
                                         m_cb * b->second));
        ++b;
      } else {
        table.push_back(
            std::make_pair(0.5 * (a->first + b->first),
                           m_c + m_ca * a->second + m_cb * b->second));
        ++a;
        ++b;
      }
    }
...
void TableDataMapping::dump() const {
  ...
  for (std::vector<std::pair<double, double>>::const_iterator p =
           m_table.begin();
       p != m_table.end(); ++p) {
...
DataMappingLookupTable::~DataMappingLookupTable() { release(); }
...
void DataMappingLookupTable::update_table(double xmin, double xmax,
                                          double delta_y,
  ...
  if (table.size() < 1) {
    ...
    //  Limit the number of interpolation points (this is an arbitrary number -
    //  it could be somewhat else)
...
std::string DataMappingLookupTable::dump() const {
  std::string r;
