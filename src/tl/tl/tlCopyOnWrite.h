 ...
 */
template <class X> struct copy_duplicator {
  X *operator()(const X &o) { return new X(o); }
};

/**
 *  @brief A clone duplicator (see copy_on_write_ptr below)
 */
template <class X> struct clone_duplicator {
  X *operator()(const X &o) { return o.clone(); }
};
 ...
 */
template <class X> class copy_on_write_holder {
public:
  copy_on_write_holder(X *x) : m_ref_count(1), mp_x(x) {}
 ...
 *
 *  The idea is to provide a smart, "unique" pointer providing a copy
 * constructor and assignment, but sharing the object as long as the object is
 * not written.
 *
 *  Write access is assumed as soon as the non-const pointer is retrieved.
 *
 *  In order to duplicate the object, a "duplicator" needs to be defined. By
 * default, the object is duplicated using the copy constructor
 * ("copy_duplicator"). An alternative implementation is provided through the
 * "clone_duplicator", which assumes a "clone" method to supply the duplicated
 *  object.
 */
template <class X, class Dup = copy_duplicator<X>>
class copy_on_write_ptr : public CopyOnWritePtrBase {
public:
  typedef X value_type;

  copy_on_write_ptr() : mp_holder(0) {}

  copy_on_write_ptr(X *x) : mp_holder(x ? new copy_on_write_holder<X>(x) : 0) {}
   ...
   *  This is when we will create a new copy if the object is shared.
   */
  X *operator->() { return get_non_const(); }

  /**
   *  @brief Gets the const pointer
   *  No copy will be created.
   */
  const X *operator->() const { return get_const(); }
