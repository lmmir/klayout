 ...
 */
template <class X> class RegistrarNode {
private:
  RegistrarNode() : mp_object(0), m_owned(true), m_position(0), mp_next(0) {
    // .. nothing else ..
  }
 ...
 */
template <class X> class RegisteredClass {
public:
  /**
  ...
  RegisteredClass(X *inst, int position = 0, const char *name = "",
    ...
    if (tl::verbosity() >= 40) {
      tl::info << "Registered object '" << name << "' with priority "
               << position;
    }
  }
 ...
 */
TL_PUBLIC void set_registrar_instance_by_type(const std::type_info &ti,
                                              RegistrarBase *rb);
 ...
 *  will receive an new'ed object of type Y and register it in the <X>
 * registration space. This object will then be owned by the registrar and is
 * destroyed upon shut down.
 */
template <class X> class Registrar : public RegistrarBase {
public:
  class iterator {
  public:
    iterator(RegistrarNode<X> *p) : mp_pos(p) {
      //  .. nothing yet ..
    }

    bool operator==(iterator d) const { return mp_pos == d.mp_pos; }
    ...
    const std::string &current_name() const { return mp_pos->m_name; }

    int current_position() const { return mp_pos->m_position; }

    X &operator*() const { return *(mp_pos->mp_object); }
...
private:
  ...
  RegistrarNode<X> *insert(X *cls, bool owned, int position,
