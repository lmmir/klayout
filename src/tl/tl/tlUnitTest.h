 ...
 *  test. Specifically if layout compare is involved, it shall display the
 * golden file name and the actual one and terminate to allow updating the
 * files.
 */
 ...
 *  This path is specified through the environment variable $TESTSRC, "testdata"
 * subdirectory and the private testdata directory. If no private test data is
 * available, this method will throw a CancelException which makes the test
 * skipped.
 */
 ...
 */
struct TestException : public tl::Exception {
  TestException(const std::string &msg) : tl::Exception(msg) {}
};

/**
 *  @brief A generic compare operator
 */
template <class X, class Y> inline bool equals(const X &a, const Y &b) {
  return a == b;
}
...
/**
 *  @brief Specialization of comparison of pointers vs. integers (specifically
 * "0")
 */
template <class X> inline bool equals(X *a, int b) {
  return a == (X *)size_t(b);
}
 ...
 */
template <class Y> inline bool equals(double a, const Y &b) {
 ...
 */
template <class X> inline bool equals(const X &a, double b) {
...
/**
 *  @brief A specialization of the compare operator for std::string vs. const
 * char *
 */
...
public:
  ...
  std::string captured_text() const { return m_text.str(); }

  void clear() { m_text.str(std::string()); }
...
public:
  ...
  /**
   *  @brief Indicates that the test is a test that should run in editable mode
   * only. In this case the test is skipped unless in editable mode.
   */
  void test_is_editable_only();

  /**
   *  @brief Indicates that the test is a test that should run in non-editable
   * mode only. In this case the test is skipped unless in non-editable mode.
   */
   ...
   */
  const std::string &name() const { return m_test; }
  ...
  void diff(const std::string &file, int line, const std::string &msg,
            const X &subject, const Y & /*ref*/) {
   ...
   */
  template <class X, class Y>
  void detailed_diff(const std::string &file, int line, const std::string &msg,
                     const X &subject, const Y &ref) {
    std::ostringstream sstr;
    sstr << msg << std::endl;
  ...
  template <class Y>
  void diff(const std::string &file, int line, const std::string &msg,
            unsigned int subject, const Y &ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
  ...
  template <class Y>
  void diff(const std::string &file, int line, const std::string &msg,
            unsigned long subject, const Y &ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
  ...
  template <class Y>
  void diff(const std::string &file, int line, const std::string &msg,
            unsigned long long subject, const Y &ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
   ...
   */
  inline void diff(const std::string &file, int line, const std::string &msg,
                   bool subject, bool ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
   ...
   */
  inline void diff(const std::string &file, int line, const std::string &msg,
                   const std::string &subject, const std::string &ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
   ...
   */
  inline void diff(const std::string &file, int line, const std::string &msg,
                   const std::string &subject, const char *ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
   ...
   */
  inline void diff(const std::string &file, int line, const std::string &msg,
                   const char *subject, const std::string &ref) {
    detailed_diff(file, line, msg, subject, ref);
  }
   ...
   */
  inline void diff(const std::string &file, int line, const std::string &msg,
                   const char *subject, const char *ref) {
  ...
  void eq_helper(bool eq, const T1 &a, const T2 &b, const char *what_expr,
                 const char *equals_expr, const char *file, int line) {
    if (tl::equals(a, b) != eq) {
      std::ostringstream sstr;
...
protected:
   ...
   */
  bool is_slow() const { return m_slow; }

private:
  virtual void execute(tl::TestBase *_this) = 0;

  void write_detailed_diff(std::ostream &os, const std::string &subject,
...
#define TEST(NAME)                                                             \
  namespace {                                                                  \
  struct TestImpl##NAME : public tl::TestBase {                                \
...
#define CHECKPOINT() _this->checkpoint(__FILE__, __LINE__);
