...
public:
   ...
   *    "[group]..."              - List the option under this group (group =
   * group title)
   *    "#..."                    - Advanced option - listed with --help-all
   * only
   *    "/..."                    - Non-advanced option - listed with -h|--help
   * only
   *    "*..."                    - Multiple occurrences allowed - values needs
   * to be an array and values are accumulated. Without *, the value string is
   * evaluated to a comma-separated list.
   *                                "*" means one occurrence at least unless
   * combined with "?".
   */
  ArgBase(const std::string &option, const std::string &brief_doc,
   ...
   */
  const ParsedOption &option() const { return m_option; }

  /**
   *  @brief The short documentation
   */
  const std::string &brief_doc() const { return m_brief_doc; }

  /**
   *  @brief The long documentation
   */
  const std::string &long_doc() const { return m_long_doc; }
   ...
   */
  virtual ArgBase *clone() const { return new ArgBase(*this); }
   ...
   *  This method is used to implement "active" arguments such as "-h" or
   * "--version".
   */
 ...
 *  @param enumerated Is true, if the value parser shall stop at a comma to
 * allow for more entries in the next iteration
 */
template <class T>
inline void extract(tl::Extractor &ex, T &t, bool /*repeated_hint*/,
 ...
 */
inline void extract(tl::Extractor &ex, std::string &t, bool /*repeated_hint*/,
                    bool enumerated = false) {
  if (*ex == '"' || *ex == '\'') {
    ex.read_quoted(t);
    if (!enumerated) {
 ...
 */
template <class T>
inline void extract(tl::Extractor &ex, std::vector<T> &t, bool repeated_hint,
                    bool /*enumerated*/ = false) {
  while (!ex.at_end()) {
    t.push_back(T());
 ...
 */
template <class T> inline void mark_presence(T &) {
  //  .. the default implementation does nothing ..
}

inline void mark_presence(bool &t) { t = true; }

/**
 *  @brief A helper to invert "presence"
 */
template <class T> inline void invert_presence(T &) {
  //  .. the default implementation does nothing ..
}

inline void invert_presence(bool &t) { t = !t; }

/**
 *  @brief A helper template to extract the actual type from (T) or (const T &)
 */
template <class T> struct type_without_const_ref { typedef T inner_type; };

template <class T> struct type_without_const_ref<const T &> {
 ...
 */
template <class T> struct wants_value_traits {
  bool operator()() const { return true; }
};

template <> struct wants_value_traits<bool> {
  bool operator()() const { return false; }
};

/**
 *  @brief A specialization of the ArgBase class for a setter with direct access
 * to the value
 */
template <class T> class arg_direct_setter : public ArgBase {
public:
  arg_direct_setter(const std::string &option, T *value,
                    const std::string &brief_doc, const std::string &long_doc)
  ...
  virtual void mark_present() { mark_presence(*mp_value); }

  virtual void invert_present() { invert_presence(*mp_value); }

  virtual ArgBase *clone() const { return new arg_direct_setter<T>(*this); }

  virtual bool wants_value() const { return wants_value_traits<T>()(); }
...
/**
 *  @brief A specialization of the ArgBase class for a setter that is a method
 * of a class
 */
template <class C, class T> class arg_method_setter : public ArgBase {
public:
  typedef typename type_without_const_ref<T>::inner_type inner_type;

  arg_method_setter(const std::string &option, C *object, void (C::*setter)(T),
                    const std::string &brief_doc, const std::string &long_doc)
      : ArgBase(option, brief_doc, long_doc), m_value(), mp_object(object),
        mp_setter(setter) {
    //  .. nothing yet ..
  }
  ...
  virtual ArgBase *clone() const { return new arg_method_setter<C, T>(*this); }
...
arg_method_setter<C, T> arg(const std::string &option, C *object,
                            void (C::*setter)(T), const std::string &brief_doc,
                            const std::string &long_doc = "") {
  return arg_method_setter<C, T>(option, object, setter, brief_doc, long_doc);
}

template <class T>
arg_direct_setter<T> arg(const std::string &option, T *value,
                         const std::string &brief_doc,
                         const std::string &long_doc = "") {
 ...
 *    cmd << arg("-x|--long-option", &has_x, "X Option", "This is a long
 * documentation for X option")
...
public:
   ...
   */
  void brief(const std::string &text) { m_brief = text; }

  /**
   *  @brief Parses the command line and executes the getters
   *
   *  This method will throw a tl::CancelException if the help function was
   * used. It will throw other exceptions on syntax errors or if mandatory
   ...
   */
  static void set_version(const std::string &text) { m_version = text; }
   ...
   */
  static void set_license(const std::string &text) { m_license = text; }
