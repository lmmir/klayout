 ...
 *
 *  Second, this object provides the database unit value for physical unit
 * conversions.
 */
 ...
 */
class TL_PUBLIC EvalError : public tl::Exception {
public:
 ...
 */
class TL_PUBLIC NoMethodError : public EvalError {
public:
  NoMethodError(const std::string &cls_name, const std::string &method,
 ...
 */
class TL_PUBLIC ExpressionParserContext : public tl::Extractor {
public:
  /**
   ...
   */
  void set_expr(const Expression *expr) { mp_expr = expr; }
...
protected:
   ...
   *  @brief Sets the expression parent
   */
  void set_expr(const tl::Expression *expr) { m_context.set_expr(expr); }
};

/**
 *  @brief A class handler for user objects within tl::Variant
 *
 *  In order to enable objects for expressions, the user object in tl::Variant
 * must be provided with a class derived from tl::VariantUserClassBase which
 * implements eval_cls to return an EvalClass implementation which executes the
 * method.
 */
...
public:
   ...
   *  @brief Constructor
   *
   *  @param test_function_name The name of the function which will be created
   * and which tests if the variant is of the given type.
   */
  EvalClass() {}
   ...
   *
   *  If no method of this kind exists, the implementation may throw a
   * NoMethodError.
   */
  virtual void execute(const ExpressionParserContext &context, tl::Variant &out,
                       tl::Variant &object, const std::string &method,
                       const std::vector<tl::Variant> &args) const = 0;
};
...
public:
   ...
   */
  void set_text(const std::string &s) { m_local_text = s; }

  /**
   *  @brief Sets the external text of the expression
   */
  void set_text(const char *s) { mp_text = s; }
...
private:
   ...
   */
  std::unique_ptr<ExpressionNode> &root() { return m_root; }
};
...
public:
   ...
   */
  static void define_global_function(const std::string &name,
   ...
   *  @param top If true, an expression is parsed at top level (as eval does).
   * If false, the exression is parsed at 'atomic' level (as the string
   * interpolation after the '$' does.
   *  @param expr An expression that can be evaluated (out)
   */
  void parse(Expression &expr, tl::Extractor &ex, bool top = true);

  /**
   *  @brief Convenience method that returns the expression object (caution:
   * poor performance)
   *
   *  @param ex The extractor from which to parse the expression
   *  @param top If true, an expression is parsed at top level (as eval does).
   ...
   * If false, the exression is parsed at 'atomic' level (as the string
   * interpolation after the '$' does.
   *  @return An expression string that can be used later to construct an
   * Expression from using the parse method
   */
  static std::string parse_expr(tl::Extractor &ex, bool top = true);

  /**
   *  @brief A convenience method to evaluate an expression (by string) in this
   * context
   */
   ...
   */
  std::vector<std::string> &match_substrings() { return m_match_substrings; }
...
private:
  ...
  void eval_top(ExpressionParserContext &context,
                std::unique_ptr<ExpressionNode> &v);
  void eval_assign(ExpressionParserContext &context,
                   std::unique_ptr<ExpressionNode> &v);
  void eval_if(ExpressionParserContext &context,
               std::unique_ptr<ExpressionNode> &v);
  void eval_boolean(ExpressionParserContext &context,
                    std::unique_ptr<ExpressionNode> &v);
  void eval_conditional(ExpressionParserContext &context,
                        std::unique_ptr<ExpressionNode> &v);
  void eval_shift(ExpressionParserContext &context,
                  std::unique_ptr<ExpressionNode> &v);
  void eval_addsub(ExpressionParserContext &context,
                   std::unique_ptr<ExpressionNode> &v);
  void eval_product(ExpressionParserContext &context,
                    std::unique_ptr<ExpressionNode> &v);
  void eval_bitwise(ExpressionParserContext &context,
                    std::unique_ptr<ExpressionNode> &v);
  void eval_unary(ExpressionParserContext &context,
                  std::unique_ptr<ExpressionNode> &v);
  void eval_atomic(ExpressionParserContext &context,
                   std::unique_ptr<ExpressionNode> &v, int am);
  void eval_suffix(ExpressionParserContext &context,
                   std::unique_ptr<ExpressionNode> &v);
  void resolve_name(const std::string &name, const EvalFunction *&function,
