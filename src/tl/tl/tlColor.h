...
public:
   ...
   */
  bool operator==(const Color &color) const { return m_color == color.m_color; }
   ...
   */
  color_t rgb() const { return m_color; }

  /**
   *  @brief Gets the alpha component
   */
  unsigned int alpha() const { return (m_color & 0xff000000) >> 24; }

  /**
   *  @brief Gets the red component
   */
  unsigned int red() const { return (m_color & 0xff0000) >> 16; }

  /**
   *  @brief Gets the green component
   */
  unsigned int green() const { return (m_color & 0xff00) >> 8; }

  /**
   *  @brief Gets the blue component
   */
  unsigned int blue() const { return (m_color & 0xff); }

  /**
   *  @brief Converts the color into monochrome "on" value
   */
  bool to_mono() const { return (m_color & 0x8000) != 0; }
   ...
   *  value: 0..255
   */
  void get_hsv(unsigned int &hue, unsigned int &saturation,
               unsigned int &value) const;

  /**
   *  @brief Creates the color from a HSV color
   */
  static tl::Color from_hsv(unsigned int hue, unsigned int saturation,
                            unsigned int value);
 ...
 */
inline QColor c2qc(color_t c) { return Color(c).to_qc(); }

/**
 *  @brief Provides conversion of a QColor to color_t
 */
inline color_t qc2c(const QColor &qc) { return Color(qc).rgb(); }
