 ...
 */
class TL_PUBLIC PixelBufferReadError : public tl::Exception {
public:
 ...
 */
class TL_PUBLIC PixelBufferWriteError : public tl::Exception {
public:
...
public:
   ...
   *
   *  If data is given, the image is initialized with the given data and will
   * take ownership over the data block.
   *
   ...
   *  @brief Creates an image with the given height and width
   *
   *  If data is given, the image is initialized with the given data. A copy of
   * the data is created.
   *
   *  "stride" specifies the stride (distance between two rows of data).
   *  The size of the data block needs to be stride*h elements or w*h if stride
   * is not given.
   */
  PixelBuffer(unsigned int w, unsigned int h, const tl::color_t *data = 0,
   ...
   *  @brief Inequality
   */
  bool operator!=(const PixelBuffer &other) const { return !operator==(other); }
   ...
   */
  unsigned int stride() const { return sizeof(tl::color_t) * m_width; }
   ...
   *  pixels with an alpha value >= 128 from the other image are patched into
   * this image.
   */
  ...
  /**
   *  @brief Subsamples the image and puts the subsampled image into the
   * destination image
   *
   ...
   *
   *  The dimension of the destination image must be set to the corresponding
   * fraction of self's dimension.
   */
   ...
   *
   *  The destination images dimension must have been set of self's dimension
   * times os.
   */
   ...
   */
  void
  set_texts(const std::vector<std::pair<std::string, std::string>> &texts) {
    m_texts = texts;
  }
...
private:
  class ImageData {
  public:
    ImageData() : mp_data(0), m_length(0) {
      //  .. nothing yet ..
    }
...
public:
   ...
   * the data is created.
   *
   *  "stride" specifies the stride (distance in bytes between two rows of
   * data). The size of the data block needs to be stride*h elements or
   * bytes(w)*h if stride is not given.
   */
  BitmapBuffer(unsigned int w, unsigned int h, const uint8_t *data = 0,
               unsigned int stride = 0);
...
private:
  class MonoImageData {
  public:
    MonoImageData() : mp_data(0), m_length(0) {
      //  .. nothing yet ..
    }
