...
template <class Value, bool trivial_relocate> class reuse_vector_iterator {
public:
   ...
   */
  reuse_vector_iterator() : mp_v(0), m_n(0) {}
   ...
   */
  bool operator==(
      const reuse_vector_const_iterator<Value, trivial_relocate> &d) const {
   ...
   *  This does not take care about the container, but just about elements
   * inside it. If the container is deleted, the iterators become invalid
   * without further notice.
   */
  bool is_valid() const { return mp_v->is_used(m_n); }
   ...
   */
  bool at_end() const { return index() >= mp_v->last(); }

  /**
   *  @brief The index of the element pointed to
   */
  size_t index() const { return m_n; }

  /**
   *  @brief The pointer to the vector that this iterator points into
   */
  reuse_vector<Value, trivial_relocate> *vector() const { return mp_v; }

  /**
   *  @brief A distance between two vectors
   */
  difference_type operator-(reuse_vector_iterator d) const {
    //  KLUDGE: this is slow and can be optimized if the vector does not have
    //  deleted items
    difference_type n = 0;
    while (d != *this) {
      ++d;
...
public:
   ...
   *  @brief The default constructor
   */
  reuse_vector_const_iterator() : mp_v(0), m_n(0) {}

  /**
   *  @brief The constructor
   */
  reuse_vector_const_iterator(const reuse_vector<Value, trivial_relocate> *v,
                              size_type n)
      : mp_v(v), m_n(n) {}
  ...
  reuse_vector_const_iterator(
      const reuse_vector_iterator<Value, trivial_relocate> &d)
  ...
  reuse_vector_iterator<Value, trivial_relocate> to_non_const() const {
    return reuse_vector_iterator<Value, trivial_relocate>(
        const_cast<reuse_vector<Value, trivial_relocate> *>(mp_v), m_n);
  }
   ...
   *
   *  This method is intended for special use cases such as the cached box
   * picker in the box tree
   */
  const value_type *unsafe_target_addr() const { return &mp_v->item(m_n); }
   ...
   *  @brief The pointer to the vector that this iterator points into
   */
  const reuse_vector<Value, trivial_relocate> *vector() const { return mp_v; }
...
public:
  ...
  ReuseData() : m_first_used(0), m_last_used(0), m_next_free(0), m_size(0) {}
  ...
  size_type first() const { return m_first_used; }

  size_type last() const { return m_last_used; }
  ...
  bool can_allocate() const { return (m_next_free < m_used.size()); }
  ...
  void reserve(size_type n) { m_used.reserve(n); }

  bool is_used(size_type n) const { return m_used[n]; }

  size_t mem_reqd() const { return (m_used.size() + 7) / 8 + sizeof(*this); }

  size_t mem_used() const { return m_used.capacity() / 8 + sizeof(*this); }
...
/**
 *  @brief A vector that maintains the order of elements but allows one to
 * reference elements in a stable way
 *
 *  This container allows inserting and deleting of elements while references to
 * them (through iterators) remain stable. The insert does not necessarily
 * happen at a certain position. Instead, the vector keeps a reuseable member
 * list (hence reuse_vector). In addition, the iterators deliver stable
 * references by using indices and a container pointer. This way, the iterators
 * point to the same element even after delete and insert (and potentially
 * reallocation) actions.
 *
 *  The memory requirements of this container are the same than that of a
 * std::vector.
 *
...
#if __GNUC__ >= 5
template <class Value, bool trivial_relocate =
                           std::is_trivially_copy_constructible<Value>::value>
#else
...
public:
  ...
  typedef reuse_vector_const_iterator<value_type, trivial_relocate>
      const_iterator;
   ...
   */
  template <class Iter> reuse_vector(Iter from, Iter to) {
    init();
   ...
   *  @brief Destructor
   */
  ~reuse_vector() { release(); }

  /**
   *  @brief Assignment
   *
   *  The assignment will not only copy the items but also compact the vector,
   * i.e. create a linear chain of elements without holes for unused ones and a
   * capacity exactly matching the required count.
   */
   ...
   */
  template <class Iter> void assign(Iter from, Iter to) {
    release();
    reserve(std::distance(from, to));
    insert(from, to);
  }
   ...
   *  @brief Inequality
   */
  bool operator!=(const reuse_vector &d) const { return !operator==(d); }
  ...
  /**
   *  @brief For a given pointer, tell if the element is a member of this
   * container
   */
  template <class V> bool is_member_of(const V *ptr) const {
   ...
   *
   *  The basic index is the internal index reported from the iterator's index()
   * method
   */
  value_type &item(size_type n) { return mp_start[n]; }
   ...
   * method
   */
  const value_type &item(size_type n) const { return mp_start[n]; }
  ...
  iterator insert(const value_type &item) {
    ...
    if (mp_rdata) {
      ...
      if (mp_finish == mp_capacity) {
        //  Special case: we are inserting an element from our own space - since
        //  reserve will first release the element we have to create a copy
        //  before we insert it.
   ...
   */
  template <class Iter> void insert(const Iter &from, const Iter &to) {
    if (!(from == to)) {
      //  don't reserve if the first element of the sequence comes from ourself.
      //  This implies that we try to duplicate our own elements which would
      //  invalidate the source upon reservation.
   ...
   *
   *  The capacity is the number of elements that can be stored without
   * reallocation to happen.
   */
  size_type capacity() const { return size_type(mp_capacity - mp_start); }

  /**
   *  @brief Reserve space for a given number of elements
   *
   *  This guarantees that for the next n-size() inserts no reallocation will
   * occur No resizing will happen, if n is less than the current capacity.
   */
   ...
   */
  ReuseData *reuse_data() const { return mp_rdata; }
  ...
  void internal_reserve_trivial(size_type n) {
    if (n > capacity()) {
      ...
      if (mp_start) {
        ...
        size_type l = last();
        size_type i = first();
        memcpy((void *)(new_start + i), (void *)(mp_start + i),
               (l - i) * sizeof(Value));
