...
EvalError::EvalError(const std::string &what,
                     const ExpressionParserContext &context)
...
NoMethodError::NoMethodError(const std::string &cls_name,
                             const std::string &method,
                             const ExpressionParserContext &context)
    : EvalError(
          tl::sprintf(
              tl::to_string(tr(
                  "'%s' is not a valid method name for objects of class '%s'")),
              method, cls_name),
          context) {
  // .. nothing yet ..
}
...
ExpressionParserContext::ExpressionParserContext() : mp_expr(0) {
  //  .. nothing yet ..
}

ExpressionParserContext::ExpressionParserContext(const Expression *expr,
                                                 const tl::Extractor &ex)
...
std::string ExpressionParserContext::where() const {
  if (mp_expr) {
    ...
    if (pos >= len) {
      ...
      if (line == 1) {
        ...
        os << tl::to_string(tr("line")) << " " << line << ", "
           << tl::to_string(tr("position")) << " " << col;
      }
...
static double to_double(const ExpressionParserContext &context,
                        const tl::Variant &v) {
  if (v.can_convert_to_double()) {
    ...
    throw EvalError(
        tl::to_string(tr("Double precision floating point value expected")),
        context);
  }
}

static double to_double(const ExpressionParserContext &context,
                        const std::vector<tl::Variant> &v) {
  if (v.size() != 1) {
    throw EvalError(
        tl::to_string(tr("Function expects a single numeric argument")),
        context);
  }
...
static long to_long(const ExpressionParserContext &context,
                    const tl::Variant &v) {
...
static unsigned long to_ulong(const ExpressionParserContext &context,
                              const tl::Variant &v) {
  if (v.can_convert_to_ulong()) {
    ...
    throw EvalError(tl::to_string(tr("Unsigned integer value expected")),
                    context);
  }
}

static long long to_longlong(const ExpressionParserContext &context,
                             const tl::Variant &v) {
...
static unsigned long long to_ulonglong(const ExpressionParserContext &context,
                                       const tl::Variant &v) {
...
public:
  EvalTarget() : mp_lvalue(0), m_rvalue() {
    //  .. nothing yet ..
  }
  ...
  tl::Variant *lvalue() { return mp_lvalue; }
...
class TL_PUBLIC ListClass : public EvalClass {
public:
  void execute(const ExpressionParserContext &context, tl::Variant &out,
               tl::Variant &object, const std::string &method,
               const std::vector<tl::Variant> &args) const {
    if (method == "push") {

      if (args.size() != 1) {
        throw EvalError(tl::to_string(tr("'push' method expects one argument")),
                        context);
      }
      ...
      if (args.size() != 0) {
        throw EvalError(
            tl::to_string(tr("'size' method does not accept an argument")),
            context);
      }
      ...
      throw EvalError(tl::to_string(tr("Unknown method")) + " '" + method +
                          "' for list",
                      context);
    }
  }
...
class TL_PUBLIC ArrayClass : public EvalClass {
public:
  void execute(const ExpressionParserContext &context, tl::Variant &out,
               tl::Variant &object, const std::string &method,
               const std::vector<tl::Variant> &args) const {
    if (method == "insert") {
      ...
      if (args.size() != 2) {
        throw EvalError(
            tl::to_string(tr("'insert' method expects two arguments")),
            context);
      }
      ...
      if (args.size() != 0) {
        throw EvalError(
            tl::to_string(tr("'keys' method does not accept an argument")),
            context);
      }

      out.set_list(object.array_size());
      for (tl::Variant::const_array_iterator a = object.begin_array();
           a != object.end_array(); ++a) {
        out.push(a->first);
      }
...
ExpressionNode::ExpressionNode(const ExpressionNode &other,
                               const tl::Expression *expr)
    : m_context(other.m_context) {
  m_context.set_expr(expr);
  m_c.reserve(other.m_c.size());

  for (std::vector<ExpressionNode *>::const_iterator c = other.m_c.begin();
       c != other.m_c.end(); ++c) {
...
ExpressionNode::~ExpressionNode() {
  for (std::vector<ExpressionNode *>::const_iterator c = m_c.begin();
       c != m_c.end(); ++c) {
    delete *c;
  }
  m_c.clear();
...
void ExpressionNode::add_child(ExpressionNode *node) { m_c.push_back(node); }
 ...
 */
class TL_PUBLIC AssignExpressionNode : public ExpressionNode {
public:
  AssignExpressionNode(const ExpressionParserContext &context,
  ...
  void execute(EvalTarget &v) const {
    ...
    if (!v.lvalue()) {
      throw EvalError(tl::to_string(tr("Assignment needs a lvalue")),
                      m_context);
    }
    a.swap(*v.lvalue());
  }
 ...
 */
class TL_PUBLIC LessExpressionNode : public ExpressionNode {
public:
  LessExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                     ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  LessExpressionNode(const LessExpressionNode &other,
                     const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      if (!c) {
        throw EvalError(
            tl::to_string(
                tr("Not a valid object for a method call (not an object)")),
            m_context);
      }
 ...
 */
class TL_PUBLIC LessOrEqualExpressionNode : public ExpressionNode {
public:
  LessOrEqualExpressionNode(const ExpressionParserContext &context,
                            ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  LessOrEqualExpressionNode(const LessOrEqualExpressionNode &other,
                            const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC GreaterExpressionNode : public ExpressionNode {
public:
  GreaterExpressionNode(const ExpressionParserContext &context,
                        ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  GreaterExpressionNode(const GreaterExpressionNode &other,
                        const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC GreaterOrEqualExpressionNode : public ExpressionNode {
public:
  GreaterOrEqualExpressionNode(const ExpressionParserContext &context,
                               ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  GreaterOrEqualExpressionNode(const GreaterOrEqualExpressionNode &other,
                               const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC EqualExpressionNode : public ExpressionNode {
public:
  EqualExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  EqualExpressionNode(const EqualExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC NotEqualExpressionNode : public ExpressionNode {
public:
  NotEqualExpressionNode(const ExpressionParserContext &context,
                         ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  NotEqualExpressionNode(const NotEqualExpressionNode &other,
                         const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC MatchExpressionNode : public ExpressionNode {
public:
  MatchExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b, tl::Eval *eval)
 ...
 */
class TL_PUBLIC MatchSubstringReferenceNode : public ExpressionNode {
public:
  MatchSubstringReferenceNode(const ExpressionParserContext &context,
                              tl::Eval *eval, int index)
      : ExpressionNode(context, 0), mp_eval(eval), m_index(index) {
    //  .. nothing yet ..
  }

  MatchSubstringReferenceNode(const MatchSubstringReferenceNode &other,
                              const tl::Expression *expr)
      : ExpressionNode(other, expr), mp_eval(other.mp_eval),
 ...
 */
class TL_PUBLIC NoMatchExpressionNode : public ExpressionNode {
public:
  NoMatchExpressionNode(const ExpressionParserContext &context,
                        ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  NoMatchExpressionNode(const NoMatchExpressionNode &other,
                        const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC LogAndExpressionNode : public ExpressionNode {
public:
  LogAndExpressionNode(const ExpressionParserContext &context,
                       ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  LogAndExpressionNode(const LogAndExpressionNode &other,
                       const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC LogOrExpressionNode : public ExpressionNode {
public:
  LogOrExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  LogOrExpressionNode(const LogOrExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC IfExpressionNode : public ExpressionNode {
public:
  IfExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                   ExpressionNode *b, ExpressionNode *c)
 ...
 */
class TL_PUBLIC ShiftLeftExpressionNode : public ExpressionNode {
public:
  ShiftLeftExpressionNode(const ExpressionParserContext &context,
                          ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  ShiftLeftExpressionNode(const ShiftLeftExpressionNode &other,
                          const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC ShiftRightExpressionNode : public ExpressionNode {
public:
  ShiftRightExpressionNode(const ExpressionParserContext &context,
                           ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  ShiftRightExpressionNode(const ShiftRightExpressionNode &other,
                           const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC PlusExpressionNode : public ExpressionNode {
public:
  PlusExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                     ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  PlusExpressionNode(const PlusExpressionNode &other,
                     const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      v.set(tl::Variant(to_ulonglong(m_context, *v) +
                        to_ulonglong(m_context, *b)));
    } else if (v->is_longlong() || b->is_longlong()) {
      v.set(
          tl::Variant(to_longlong(m_context, *v) + to_longlong(m_context, *b)));
 ...
 */
class TL_PUBLIC MinusExpressionNode : public ExpressionNode {
public:
  MinusExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  MinusExpressionNode(const MinusExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC StarExpressionNode : public ExpressionNode {
public:
  StarExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                     ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  StarExpressionNode(const StarExpressionNode &other,
                     const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      if (x < 0) {
        throw EvalError(tl::to_string(tr("Numeric argument of '*' operator "
                                         "with string must be positive")),
                        m_context);
      }
 ...
 */
class TL_PUBLIC SlashExpressionNode : public ExpressionNode {
public:
  SlashExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  SlashExpressionNode(const SlashExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC PercentExpressionNode : public ExpressionNode {
public:
  PercentExpressionNode(const ExpressionParserContext &context,
                        ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  PercentExpressionNode(const PercentExpressionNode &other,
                        const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC AmpersandExpressionNode : public ExpressionNode {
public:
  AmpersandExpressionNode(const ExpressionParserContext &context,
                          ExpressionNode *a, ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  AmpersandExpressionNode(const AmpersandExpressionNode &other,
                          const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC PipeExpressionNode : public ExpressionNode {
public:
  PipeExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                     ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  PipeExpressionNode(const PipeExpressionNode &other,
                     const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC AcuteExpressionNode : public ExpressionNode {
public:
  AcuteExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  AcuteExpressionNode(const AcuteExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC IndexExpressionNode : public ExpressionNode {
public:
  IndexExpressionNode(const ExpressionParserContext &context, ExpressionNode *a,
                      ExpressionNode *b)
      : ExpressionNode(context, 2) {
    add_child(a);
    add_child(b);
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      if (!e->can_convert_to_ulong()) {
        throw EvalError(tl::to_string(tr("Invalid index for [] operator")),
                        m_context);
      }
      ...
      throw EvalError(
          tl::to_string(tr("[] operator expects a list or an array")),
          m_context);
    }
  }
};
 ...
 */
class TL_PUBLIC UnaryMinusExpressionNode : public ExpressionNode {
public:
  UnaryMinusExpressionNode(const ExpressionParserContext &context,
                           ExpressionNode *a)
      : ExpressionNode(context, 1) {
    add_child(a);
  }

  UnaryMinusExpressionNode(const UnaryMinusExpressionNode &other,
                           const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      throw EvalError(
          tl::to_string(tr("Unary minus not implemented for objects")),
          m_context);
      /*
      const EvalClass *c = v->user_cls () ? v->user_cls ()->eval_cls () : 0;
      if (! c) {
        throw EvalError (tl::to_string (tr ("Not a valid object for a method
      call (not an object)")), m_context);
      }
 ...
 */
class TL_PUBLIC UnaryTildeExpressionNode : public ExpressionNode {
public:
  UnaryTildeExpressionNode(const ExpressionParserContext &context,
                           ExpressionNode *a)
      : ExpressionNode(context, 1) {
    add_child(a);
  }

  UnaryTildeExpressionNode(const UnaryTildeExpressionNode &other,
                           const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const {
    ...
    if (v->is_user()) {
      ...
      throw EvalError(
          tl::to_string(tr("Unary tilde not implemented for objects")),
          m_context);
      /*
      const EvalClass *c = v->user_cls () ? v->user_cls ()->eval_cls () : 0;
      if (! c) {
 ...
 */
class TL_PUBLIC UnaryNotExpressionNode : public ExpressionNode {
public:
  UnaryNotExpressionNode(const ExpressionParserContext &context,
                         ExpressionNode *a)
      : ExpressionNode(context, 1) {
    add_child(a);
  }

  UnaryNotExpressionNode(const UnaryNotExpressionNode &other,
                         const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC ConstantExpressionNode : public ExpressionNode {
public:
  ConstantExpressionNode(const ExpressionParserContext &context,
                         const tl::Variant &value)
      : ExpressionNode(context), m_value(value) {
    //  .. nothing yet ..
  }

  ConstantExpressionNode(const ConstantExpressionNode &other,
                         const tl::Expression *expr)
  ...
  void execute(EvalTarget &v) const { v.set(m_value); }
 ...
 */
class TL_PUBLIC ContextEvaluationNode : public ExpressionNode {
public:
  ContextEvaluationNode(const ExpressionParserContext &context,
                        const ContextHandler *ctx_handler, ExpressionNode *a,
                        bool double_bracket)
      : ExpressionNode(context, 1), mp_ctx_handler(ctx_handler),
        m_double_bracket(double_bracket) {
    add_child(a);
  }

  ContextEvaluationNode(const ContextEvaluationNode &other,
                        const tl::Expression *expr)
      : ExpressionNode(other, expr), mp_ctx_handler(other.mp_ctx_handler),
        m_double_bracket(other.m_double_bracket) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC MethodExpressionNode : public ExpressionNode {
public:
  MethodExpressionNode(const ExpressionParserContext &context,
                       const std::string &method)
      : ExpressionNode(context), m_method(method) {
    //  .. nothing yet ..
  }

  MethodExpressionNode(const MethodExpressionNode &other,
                       const tl::Expression *expr)
  ...
  void execute(EvalTarget &v) const {
    ...
    for (std::vector<ExpressionNode *>::const_iterator c = m_c.begin() + 1;
         c != m_c.end(); ++c) {
      EvalTarget a;
      (*c)->execute(a);
      vv.push_back(*a);
    ...
    if (v->is_list()) {
      ...
      if (!c) {
        throw EvalError(
            tl::sprintf(tl::to_string(tr("Not a valid object for a method call "
                                         "(not an object) - value is %s")),
                        v->to_parsable_string()),
            m_context);
      }
    } else {
      throw EvalError(
          tl::sprintf(tl::to_string(tr("Not a valid object for a method call "
                                       "(wrong type) - value is %s")),
                      v->to_parsable_string()),
          m_context);
    }
 ...
 */
class TL_PUBLIC ListExpressionNode : public ExpressionNode {
public:
  ...
  ListExpressionNode(const ListExpressionNode &other,
                     const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC ArrayExpressionNode : public ExpressionNode {
public:
  ...
  ArrayExpressionNode(const ArrayExpressionNode &other,
                      const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC SequenceExpressionNode : public ExpressionNode {
public:
  ...
  SequenceExpressionNode(const SequenceExpressionNode &other,
                         const tl::Expression *expr)
      : ExpressionNode(other, expr) {
    //  .. nothing yet ..
  }
 ...
 */
class TL_PUBLIC StaticFunctionExpressionNode : public ExpressionNode {
public:
  StaticFunctionExpressionNode(const ExpressionParserContext &context,
                               const EvalFunction *func)
      : ExpressionNode(context), mp_func(func) {
    //  .. nothing yet ..
  }

  StaticFunctionExpressionNode(const StaticFunctionExpressionNode &other,
                               const tl::Expression *expr)
 ...
 */
class TL_PUBLIC RVariableExpressionNode : public ExpressionNode {
public:
  RVariableExpressionNode(const ExpressionParserContext &context,
                          const tl::Variant *var)
      : ExpressionNode(context), mp_var(var) {
    //  .. nothing yet ..
  }

  RVariableExpressionNode(const RVariableExpressionNode &other,
                          const tl::Expression *expr)
  ...
  void execute(EvalTarget &v) const { v.set(*mp_var); }
 ...
 */
class TL_PUBLIC LVariableExpressionNode : public ExpressionNode {
public:
  LVariableExpressionNode(const ExpressionParserContext &context,
                          tl::Variant *var)
      : ExpressionNode(context), mp_var(var) {
    //  .. nothing yet ..
  }

  LVariableExpressionNode(const LVariableExpressionNode &other,
                          const tl::Expression *expr)
      : ExpressionNode(other, expr), mp_var(other.mp_var) {
    //  .. nothing yet ..
  }
  ...
  void execute(EvalTarget &v) const { v.set_lvalue(mp_var); }
...
static void sin_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  out = sin(to_double(context, v));
}

static void sinh_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = sinh(to_double(context, v));
}

static void cos_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  out = cos(to_double(context, v));
}

static void cosh_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = cosh(to_double(context, v));
}

static void tan_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  out = tan(to_double(context, v));
}

static void tanh_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = tanh(to_double(context, v));
}

static void log_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  out = ::log(to_double(context, v));
}

static void log10_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
  out = log10(to_double(context, v));
}

static void exp_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  out = exp(to_double(context, v));
}

static void floor_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
  out = floor(to_double(context, v));
}

static void ceil_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = ceil(to_double(context, v));
}

static void round_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
...
static void sqrt_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = sqrt(to_double(context, v));
}

static void abs_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &v) {
  if (v.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'abs' function expects exactly one argument")),
        context);
  }
...
static void acos_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = acos(to_double(context, v));
}

#ifndef _MSC_VER // not available on MS VC++
static void acosh_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
  out = acosh(to_double(context, v));
}
#endif

static void asin_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = asin(to_double(context, v));
}

#ifndef _MSC_VER // not available on MS VC++
static void asinh_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
  out = asinh(to_double(context, v));
}
#endif

static void atan_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &v) {
  out = atan(to_double(context, v));
}

#ifndef _MSC_VER // not available on MS VC++
static void atanh_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &v) {
  out = atanh(to_double(context, v));
}
#endif

static void min_f(const ExpressionParserContext &, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  out = tl::Variant();
  for (std::vector<tl::Variant>::const_iterator v = vv.begin(); v != vv.end();
       ++v) {
...
static void max_f(const ExpressionParserContext &, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  out = tl::Variant();
  for (std::vector<tl::Variant>::const_iterator v = vv.begin(); v != vv.end();
       ++v) {
...
static void pow_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'pow' function expects exactly two arguments")),
        context);
  }
...
static void atan2_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'atan2' function expects exactly two arguments")),
        context);
  }
...
static void to_f_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_f' function expects exactly one argument")),
        context);
  }
...
static void to_s_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_s' function expects exactly one argument")),
        context);
  }
...
static void to_i_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_i' function expects exactly one argument")),
        context);
  }
...
static void to_ui_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_ui' function expects exactly one argument")),
        context);
  }
...
static void to_l_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_l' function expects exactly one argument")),
        context);
  }
...
static void to_ul_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'to_ul' function expects exactly one argument")),
        context);
  }
...
static void is_string_f(const ExpressionParserContext &context,
                        tl::Variant &out, const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'is_string' function expects exactly one argument")),
        context);
  }
...
static void is_numeric_f(const ExpressionParserContext &context,
                         tl::Variant &out, const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'is_numeric' function expects exactly one argument")),
        context);
  }
...
static void is_array_f(const ExpressionParserContext &context, tl::Variant &out,
                       const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'is_array' function expects exactly one argument")),
        context);
  }
...
static void is_nil_f(const ExpressionParserContext &context, tl::Variant &out,
                     const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'is_nil' function expects exactly one argument")),
        context);
  }
...
static void gsub_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 3) {
    throw EvalError(
        tl::to_string(tr("'gsub' function expects exactly three arguments")),
        context);
  }
  ...
  size_t p = 0;
  for (size_t pp = 0; (pp = s.find(x, p)) != std::string::npos;
       p = pp + x.size()) {
...
static void sub_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  if (vv.size() != 3) {
    throw EvalError(
        tl::to_string(tr("'sub' function expects exactly three arguments")),
        context);
  }
...
static void find_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'find' function expects exactly two arguments")),
        context);
  }
...
static void rfind_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'rfind' function expects exactly two arguments")),
        context);
  }
...
static void len_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'len' function expects exactly one argument")),
        context);
  }
...
static void substr_f(const ExpressionParserContext &context, tl::Variant &out,
                     const std::vector<tl::Variant> &vv) {
  if (vv.size() != 3 && vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'substr' function expects two or three arguments")),
        context);
  }
...
static void join_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'join' function expects exactly two arguments")),
        context);
  }

  if (!vv[0].is_list()) {
    throw EvalError(
        tl::to_string(tr("First argument of 'join' function must be a list")),
        context);
  }
...
static void item_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'item' function expects exactly two arguments")),
        context);
  }

  if (!vv[0].is_list()) {
    throw EvalError(
        tl::to_string(tr("First argument of 'item' function must be a list")),
        context);
  }
...
static void split_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'split' function expects exactly two arguments")),
        context);
  }
  ...
  size_t p = 0;
  for (size_t pp = 0; (pp = t.find(s, p)) != std::string::npos;
       p = pp + s.size()) {
...
static void true_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 0) {
    throw EvalError(
        tl::to_string(tr("'true' function must not have arguments")), context);
  }

  out = true;
}

static void false_f(const ExpressionParserContext &context, tl::Variant &out,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 0) {
    throw EvalError(
        tl::to_string(tr("'false' function must not have arguments")), context);
  }

  out = false;
}

static void nil_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  if (vv.size() != 0) {
    throw EvalError(tl::to_string(tr("'nil' function must not have arguments")),
                    context);
  }
...
static void env_f(const ExpressionParserContext &context, tl::Variant &out,
                  const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'env' function expects exactly two arguments")),
        context);
  }
...
static void error_f(const ExpressionParserContext &context, tl::Variant &,
                    const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'error' function expects exactly one argument")),
        context);
  }
...
static void absolute_file_path_f(const ExpressionParserContext &context,
                                 tl::Variant &out,
                                 const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(
            tr("'absolute_file_path' function expects exactly one argument")),
        context);
  }
...
static void absolute_path_f(const ExpressionParserContext &context,
                            tl::Variant &out,
                            const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(
            tr("'absolute_path' function expects exactly one argument")),
        context);
  }
...
static void path_f(const ExpressionParserContext &context, tl::Variant &out,
                   const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'path' function expects exactly one argument")),
        context);
  }
...
static void basename_f(const ExpressionParserContext &context, tl::Variant &out,
                       const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'basename' function expects exactly one argument")),
        context);
  }
...
static void extension_f(const ExpressionParserContext &context,
                        tl::Variant &out, const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'extension' function expects exactly one argument")),
        context);
  }
...
static void file_exists_f(const ExpressionParserContext &context,
                          tl::Variant &out,
                          const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(tl::to_string(tr(
                        "'file_exists' function expects exactly one argument")),
                    context);
  }
...
static void is_dir_f(const ExpressionParserContext &context, tl::Variant &out,
                     const std::vector<tl::Variant> &vv) {
  if (vv.size() != 1) {
    throw EvalError(
        tl::to_string(tr("'is_dir' function expects exactly one argument")),
        context);
  }
...
static void combine_f(const ExpressionParserContext &context, tl::Variant &out,
                      const std::vector<tl::Variant> &vv) {
  if (vv.size() != 2) {
    throw EvalError(
        tl::to_string(tr("'combine' function expects two arguments")), context);
  }

  out = tl::combine_path(vv[0].to_string(), vv[1].to_string());
}

static void sprintf_f(const ExpressionParserContext &context, tl::Variant &out,
                      const std::vector<tl::Variant> &vv) {
  if (vv.size() < 1) {
    throw EvalError(
        tl::to_string(tr("'sprintf' function expects at least one argument")),
        context);
  }
...
static void printf_f(const ExpressionParserContext &context, tl::Variant &,
                     const std::vector<tl::Variant> &vv) {
  if (vv.size() < 1) {
    throw EvalError(
        tl::to_string(tr("'printf' function expects at least one argument")),
        context);
  }
...
class EvalStaticFunction : public EvalFunction {
  typedef void (*function_t)(const ExpressionParserContext &context,
                             tl::Variant &out,
                             const std::vector<tl::Variant> &v);
...
public:
  ...
  ~EvalStaticFunction() { ms_functions.erase(m_name); }
...
public:
  ...
  static const tl::Variant *constant_by_name(const std::string &name) {
    std::map<std::string, tl::Variant>::const_iterator c =
        ms_constants.find(name);
...
Expression::Expression() : mp_text(0), mp_eval(0) {
  // .. nothing yet ..
}

Expression::Expression(const Expression &d) : mp_text(0), mp_eval(0) {
  operator=(d);
}
...
Expression &Expression::operator=(const Expression &d) {
  if (&d != this) {
    mp_eval = d.mp_eval;
...
tl::Variant Expression::execute() const {
  EvalTarget v;
  execute(v);
...
Eval::Eval(Eval *parent, bool sloppy)
    : mp_parent(parent), mp_global(&Eval::m_global), m_sloppy(sloppy),
      mp_ctx_handler(0) {
  // .. nothing yet ..
}

Eval::Eval(Eval *global, Eval *parent, bool sloppy)
    : mp_parent(parent), mp_global(global), m_sloppy(sloppy),
      mp_ctx_handler(0) {
  // .. nothing yet ..
}

Eval::~Eval() {
  for (std::map<std::string, EvalFunction *>::iterator f =
           m_local_functions.begin();
       f != m_local_functions.end(); ++f) {
    delete f->second;
  }
...
void Eval::define_function(const std::string &name, EvalFunction *function) {
  EvalFunction *&f =
      m_local_functions.insert(std::make_pair(name, (EvalFunction *)0))
          .first->second;
  if (f != 0) {
    delete f;
  }
...
void Eval::eval_top(ExpressionParserContext &ex,
...
void Eval::eval_assign(ExpressionParserContext &ex,
                       std::unique_ptr<ExpressionNode> &n) {
  eval_if(ex, n);
...
void Eval::eval_if(ExpressionParserContext &ex,
                   std::unique_ptr<ExpressionNode> &n) {
  eval_boolean(ex, n);
...
void Eval::eval_boolean(ExpressionParserContext &ex,
                        std::unique_ptr<ExpressionNode> &n) {
...
void Eval::eval_conditional(ExpressionParserContext &ex,
                            std::unique_ptr<ExpressionNode> &n) {
  eval_shift(ex, n);
...
void Eval::eval_shift(ExpressionParserContext &ex,
                      std::unique_ptr<ExpressionNode> &n) {
  eval_addsub(ex, n);
...
void Eval::eval_addsub(ExpressionParserContext &ex,
                       std::unique_ptr<ExpressionNode> &n) {
  eval_product(ex, n);
...
void Eval::eval_product(ExpressionParserContext &ex,
                        std::unique_ptr<ExpressionNode> &n) {
  eval_bitwise(ex, n);
...
void Eval::eval_bitwise(ExpressionParserContext &ex,
                        std::unique_ptr<ExpressionNode> &n) {
  eval_unary(ex, n);
...
void Eval::eval_unary(ExpressionParserContext &ex,
                      std::unique_ptr<ExpressionNode> &n) {
  ExpressionParserContext ex1 = ex;
  if (ex.test("!")) {
...
void Eval::eval_suffix(ExpressionParserContext &ex,
                       std::unique_ptr<ExpressionNode> &n) {
  eval_atomic(ex, n, 1);
        ...
        if (!ex.test(")")) {
          ...
          do {
            ...
            if (ex.test(")")) {
              ...
              throw EvalError(tl::to_string(tr("Expected closing bracket ')'")),
                              ex);
            }
...
static void scan_angle_bracket(tl::Extractor &ex, const char *term,
                               std::string &s) {
...
void Eval::eval_atomic(ExpressionParserContext &ex,
                       std::unique_ptr<ExpressionNode> &n, int am) {
  double g = 0.0;
  std::string t;
  ...
  if (ex.test("(")) {
    ...
    if (ex.test("$") || ex.test("\"") || ex.test("\'") || ex.test("(")) {
      ...
      if (m_sloppy) {
        ...
        n.reset(new ContextEvaluationNode(ex1, ctx_handler(), n.release(),
                                          true /*double bracket*/));
      } else {
        throw EvalError(tl::to_string(tr(
                            "<<..>> expression not available in this context")),
                        ex1);
      }
      ...
      if (m_sloppy) {
        ...
        n.reset(new ConstantExpressionNode(
            ex1, ctx_handler()->eval_double_bracket(s)));
      } else {
        throw EvalError(tl::to_string(tr(
                            "<<..>> expression not available in this context")),
                        ex1);
    ...
    if (ex.test("$") || ex.test("\"") || ex.test("\'") || ex.test("(")) {
      ...
      if (m_sloppy) {
        ...
        n.reset(
            new ConstantExpressionNode(ex1, ctx_handler()->eval_bracket(s)));
      } else {
        throw EvalError(
            tl::to_string(tr("<..> expression not available in this context")),
            ex1);
      }
    }
    ...
    while (!ex.at_end()) {
      if (isdigit(*ex) || (tolower(*ex) <= 'f' && tolower(*ex) >= 'a')) {
        if ((x * 16) / 16 != x) {
          throw EvalError(tl::to_string(tr("Hexadecimal number overflow")),
                          ex1);
        }
        x *= 16;
        if (isdigit(*ex)) {
    ...
    if (m_sloppy) {
      ...
      if (dbu_units && !ctx_handler()) {
        throw EvalError(
            tl::to_string(
                tr("Length or area value with unit requires a layout context")),
            ex1);
      }
      ...
      if (dbu_units) {
        ...
        if (fabs(g) < 1e12 && fabs(g - gg) > 1e-3) {
          throw EvalError(
              tl::to_string(tr("Value is not a multiple of the database unit")),
              ex1);
        }
      }
    ...
    if (function) {
      ...
      throw EvalError(tl::to_string(tr("Unknown variable or function")) + " '" +
                          t + "'",
                      ex1);
    }
    ...
    throw EvalError(
        tl::to_string(tr("Expected constant, function or bracket expression")),
        ex1);
  }
}
...
void Eval::resolve_name(const std::string &t, const EvalFunction *&function,
                        const tl::Variant *&value, tl::Variant *&var) {
  function = 0;
  value = 0;
  var = 0;
...
tl::Variant Eval::eval(const std::string &s) {
  Expression expr;
  parse(expr, s, true);
...
std::string Eval::parse_expr(tl::Extractor &ex, bool top) {
  tl::Eval eval(0, true);
...
std::string Eval::interpolate(const std::string &str) {
  std::ostringstream os;
  os.imbue(c_locale);
  os.precision(8);
