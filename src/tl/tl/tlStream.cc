...
class FileWriteErrorException : public tl::Exception {
public:
  FileWriteErrorException(const std::string &f, int en)
      : tl::Exception(tl::to_string(tr("Write error on file: %s (errno=%d)")),
                      f, en) {}
};

class FileReadErrorException : public tl::Exception {
public:
  FileReadErrorException(const std::string &f, int en)
      : tl::Exception(tl::to_string(tr("Read error on file: %s (errno=%d)")), f,
                      en) {}
};

class ZLibWriteErrorException : public tl::Exception {
public:
  ZLibWriteErrorException(const std::string &f, const char *em)
      : tl::Exception(tl::to_string(tr("Write error on file in decompression "
                                       "library: %s (message=%s)")),
                      f, em) {}
};

class ZLibReadErrorException : public tl::Exception {
public:
  ZLibReadErrorException(const std::string &f, const char *em)
      : tl::Exception(tl::to_string(tr("Read error on file in decompression "
                                       "library: %s (message=%s)")),
                      f, em) {}
};

class FileOpenErrorException : public tl::Exception {
public:
  FileOpenErrorException(const std::string &f, int en)
      : tl::Exception(tl::to_string(tr("Unable to open file: %s (errno=%d)")),
                      f, en) {}
};

class FilePOpenErrorException : public tl::Exception {
public:
  FilePOpenErrorException(const std::string &f, int en)
      : tl::Exception(tl::to_string(tr("Unable to get input from command "
                                       "through pipe: %s (errno=%d)")),
                      f, en) {}
};

class FilePReadErrorException : public tl::Exception {
public:
  FilePReadErrorException(const std::string &f, int en)
      : tl::Exception(
            tl::to_string(tr("Read error on pipe from command: %s (errno=%d)")),
            f, en) {}
};

class FilePWriteErrorException : public tl::Exception {
public:
  FilePWriteErrorException(const std::string &f, int en)
      : tl::Exception(tl::to_string(tr(
                          "Write error on pipe from command: %s (errno=%d)")),
                      f, en) {}
};
...
/**
 *  @brief A dummy delegate to provide for the case of raw data stashed inside
 * the stream itself
 */
class RawDataDelegate : public InputStreamBase {
public:
  RawDataDelegate(const std::string &source) : m_source(source) {}
...
InputStream::InputStream(InputStreamBase &delegate)
    : m_pos(0), mp_bptr(0), mp_delegate(&delegate), m_owns_delegate(false),
      mp_inflate(0), m_inflate_always(false) {
...
InputStream::InputStream(InputStreamBase *delegate)
    : m_pos(0), mp_bptr(0), mp_delegate(delegate), m_owns_delegate(true),
      mp_inflate(0), m_inflate_always(false) {
  m_bcap = 4096; // initial buffer capacity
  m_blen = 0;
  mp_buffer = new char[m_bcap];
...
InputStream::InputStream(const std::string &abstract_path)
    : m_pos(0), mp_bptr(0), mp_delegate(0), m_owns_delegate(false),
      mp_inflate(0), m_inflate_always(false) {
  m_bcap = 4096; // initial buffer capacity
  m_blen = 0;
  mp_buffer = 0;
  ...
  if (ex.test(":")) {
    ...
    if (res.size() == 0) {
      throw tl::Exception(tl::to_string(tr("Resource not found: ")) +
                          abstract_path);
    }
    ...
    std::pair<tl::InputStreamBase *, bool> rr =
        tl::get_resource_reader(ex.get());
    if (!rr.first) {
      throw tl::Exception(tl::to_string(tr("Resource not found: ")) +
                          abstract_path);
    }
    ...
    if (uri.scheme() == "http" || uri.scheme() == "https") {
      ...
      throw tl::Exception(tl::to_string(
          tr("HTTP support not enabled - HTTP/HTTPS paths are not available")));
#endif
    } else if (uri.scheme() == "file") {
      mp_delegate = new InputZLibFile(uri.path());
    } else if (!uri.scheme().empty()) {
      throw tl::Exception(tl::to_string(tr("URI scheme not supported: ")) +
                          uri.scheme());
    } else {
...
const char *InputStream::get(size_t n, bool bypass_inflate) {
...
std::string InputStream::read_all(size_t max_count) {
  std::string str;

  if (mp_inflate) {

    //  Inflate is special - it does not have a guaranteed byte delivery, so we
    //  have to go the hard way and pick the file byte by byte
    while (max_count > 0) {
      const char *b = get(1);
      if (b) {
        str += *b;
...
std::string TextInputStream::read_all() {
  return read_all(std::numeric_limits<size_t>::max());
}

std::string TextInputStream::read_all(size_t max_count) {
  std::string text;
...
const std::string &TextInputStream::get_line() {
  ...
  while (!at_end()) {
    char c = get_char();
    if (c == '\n') {
      //  set at_end if there is nothing after this terminal LF -> this will
      //  avoid emitting an empty dummy line as the last one
      if (peek_char() == 0) {
        m_at_end = true;
      }
      break;
...
InputFile::InputFile(const std::string &path) : m_fd(-1) {
  m_source = tl::absolute_file_path(path);
  ;
#if defined(_WIN32)
  int fd = _wopen(tl::to_wstring(m_source).c_str(),
...
size_t InputFile::read(char *b, size_t n) {
  tl_assert(m_fd >= 0);
#if defined(_WIN32)
...
std::string InputFile::absolute_path() const {
  return tl::absolute_file_path(m_source);
}

std::string InputFile::filename() const { return tl::filename(m_source); }
...
size_t InputZLibFile::read(char *b, size_t n) {
...
std::string InputZLibFile::absolute_path() const {
  return tl::absolute_file_path(m_source);
}

std::string InputZLibFile::filename() const { return tl::filename(m_source); }
...
OutputStream::OutputStream(OutputStreamBase *delegate, bool as_text)
    : m_pos(0), mp_delegate(delegate), m_owns_delegate(true),
      m_as_text(as_text) {
  m_buffer_capacity = 16384;
  m_buffer_pos = 0;
  mp_buffer = new char[m_buffer_capacity];
...
OutputStream::output_mode_from_filename(const std::string &abstract_path,
                                        OutputStream::OutputStreamMode om) {
  if (om == OM_Auto) {
    if (tl::match_filename_to_format(abstract_path,
                                     "(*.gz *.gzip *.GZ *.GZIP)")) {
      om = OM_Zlib;
    } else {
      om = OM_Plain;
...
static OutputStreamBase *create_file_stream(const std::string &path,
                                            OutputStream::OutputStreamMode om,
                                            int keep_backups) {
...
OutputStream::OutputStream(const std::string &abstract_path,
                           OutputStreamMode om, bool as_text, int keep_backups)
    : m_pos(0), mp_delegate(0), m_owns_delegate(false), m_as_text(as_text),
      m_path(abstract_path) {
  ...
  if (ex.test("http:") || ex.test("https:")) {
    throw tl::Exception(
        tl::to_string(tr("Cannot write to http:, https: or pipe: URL's")));
  } else if (ex.test("pipe:")) {
...
void OutputStream::set_as_text(bool f) { m_as_text = f; }
...
void OutputStream::put(const char *b, size_t n) {
  ...
  if (m_as_text) {
    //  skip CR, but replace LF by CRLF -> this will normalize the line
    //  terminators to CRLF
    while (n > 0) {
      if (*b == '\r') {
        ++b;
...
OutputFileBase::OutputFileBase(const std::string &p, int keep_backups)
    : m_keep_backups(keep_backups), m_path(tl::absolute_file_path(p)),
      m_has_error(false) {
  if (tl::file_exists(m_path)) {
    ...
    if (tl::file_exists(m_backup_path)) {
      if (!tl::rm_file(m_backup_path)) {
        tl::warn << tl::sprintf(
            tl::to_string(tr("Could not create backup file: unable to remove "
                             "existing file '%s'")),
            m_backup_path);
    ...
    if (!m_backup_path.empty()) {
      if (!tl::rename_file(m_path, tl::filename(m_backup_path))) {
        tl::warn << tl::sprintf(
            tl::to_string(tr("Could not create backup file: unable to rename "
                             "original file '%s' to backup file")),
            m_path, m_backup_path);
        m_backup_path = std::string();
      }
    }
  }
  ...
  if (!m_backup_path.empty()) {
    ...
    if (m_has_error) {
      ...
      if (!tl::rm_file(m_path)) {
        tl::warn << tl::sprintf(
            tl::to_string(tr(
                "Could not restore backup file: unable to remove file '%s'")),
            m_path);
      } else if (!tl::rename_file(m_backup_path, m_path)) {
        tl::warn << tl::sprintf(
            tl::to_string(tr("Could not restore backup file: unable to rename "
                             "file '%s' back to '%s'")),
            m_backup_path, m_path);
      }
      ...
      if (m_keep_backups == 0) {
        ...
        if (!tl::rm_file(m_backup_path)) {
          tl::warn << tl::sprintf(
              tl::to_string(tr("Could not remove backup file '%s'")),
              m_backup_path);
        }
        ...
        while (n > 0) {
          std::string p = m_path + "." + tl::to_string(n);
          std::string pprev =
              n > 1 ? (m_path + "." + tl::to_string(n - 1)) : m_backup_path;
          if (tl::file_exists(p)) {
            if (!tl::rm_file(p)) {
              tl::warn << tl::sprintf(
                  tl::to_string(tr("Error shuffling backup files: unable to "
                                   "remove file '%s'")),
                  p);
            }
          }
...
void OutputFileBase::reject() { m_has_error = true; }
...
#if defined(_WIN32)
  int fd = _wopen(tl::to_wstring(path()).c_str(),
                  _O_CREAT | _O_TRUNC | _O_BINARY | _O_WRONLY | _O_SEQUENTIAL,
                  _S_IREAD | _S_IWRITE);
  if (fd < 0) {
...
InputPipe::InputPipe(const std::string &path) : m_file(NULL) {
...
size_t InputPipe::read(char *b, size_t n) {
...
void InputPipe::reset() {
  throw tl::Exception(
      tl::to_string(tr("'reset' is not supported on pipeline input files")));
}

// ---------------------------------------------------------------
//  OutputPipe delegate implementation

OutputPipe::OutputPipe(const std::string &path) : m_file(NULL) {
  std::wstring wpath = tl::to_wstring(path);
  m_source = path;
