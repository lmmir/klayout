...
void verbosity(int level) { m_verbosity_level = level; }

int verbosity() { return m_verbosity_level; }

// ------------------------------------------------
//  Channel implementation

Channel::Channel() : m_no_endl(false), m_active(false), m_in_yield(false) {
  //  .. nothing else ..
}
...
ChannelProxy Channel::issue_proxy() {
  m_no_endl = false;
  m_lock.lock();
  if (!m_active) {
    begin();
...
void Channel::release_proxy() {
  if (!m_no_endl) {
    endl(); //  this helps remembering that there is something implicit going on
            //  here ..
  }
  end();
  m_active = false;
  ...
  //  after we have released the lock we give the receivers an opportunity to
  //  process events. Note that we allow only one thread to yield at the same
  //  time and no recursive yields.
  if (!in_yield) {
    yield();
    //  this should be atomic, but execution reordering may place it before
    //  yield(). Hence the lock.
    m_lock.lock();
    m_in_yield = false;
    m_lock.unlock();
  }
...
LogTee::LogTee(Channel *first, bool owned) { add(first, owned); }
...
void LogTee::puts(const char *s) {
  try {
    for (tl::weak_collection<tl::Channel>::iterator c = m_channels.begin();
         c != m_channels.end(); ++c) {
      c->puts(s);
    }
  } catch (...) {
...
ChannelProxy::ChannelProxy(Channel *channel) : mp_channel(channel) {}
...
ChannelProxy &ChannelProxy::operator=(const ChannelProxy &d) {
...
ChannelProxy::ChannelProxy(const ChannelProxy &d) : mp_channel(d.mp_channel) {
  // transfer channel to the assigned object
  (const_cast<ChannelProxy &>(d)).mp_channel = 0;
}
...
#if defined(_MSC_VER)
static bool can_colorize(FILE * /*stream*/) { return false; }
#else
static bool can_colorize(FILE *stream) { return isatty(fileno(stream)); }
#endif
...
InfoChannel::InfoChannel(int verbosity) : m_verbosity(verbosity) {
...
void WarningChannel::puts(const char *s) { fputs(s, stdout); }
...
void ErrorChannel::puts(const char *s) { fputs(s, stderr); }
