...
/**
 *  @brief An exception indicating that string extraction is not available for a
 * certain type
 */
class TL_PUBLIC ExtractorNotImplementedException : public tl::Exception {
public:
  ExtractorNotImplementedException(const std::type_info &ti);
};

/**
 *  @brief An exception indicating that string conversion is not available for a
 * certain type
 */
class TL_PUBLIC StringConversionException : public tl::Exception {
public:
...
template <class T> void extractor_impl(tl::Extractor &, T &) {
  throw ExtractorNotImplementedException(typeid(T));
}
...
public:
   ...
   */
  string() : m_size(0), m_capacity(0), mp_rep(0) {
    //  nothing yet ..
  }
  ...
  /**
   *  @brief The constructor creating a string from a STL string taking just a
   * substring
   */
   ...
   */
  const char *c_str() const { return mp_rep == 0 ? "" : mp_rep; }
   ...
   *
   *  Unlike assignment of an empty string, "clear" releases the memory
...
template <> inline std::string to_string(const double &d) {
  return to_string(d, 12);
}
template <> inline std::string to_string(const float &d) {
...
//  variants utilize parsable strings because this way the string representation
//  can be translated back to a variant object
template <> inline std::string to_string(const tl::Variant &v) {
  return v.to_parsable_string();
}

#if defined(HAVE_QT)
//  some dummy conversions provided for tl::Variant implementation
template <> inline std::string to_string(const QImage &) {
  return std::string();
}
#endif
 ...
 *
 *  The string returned by this method can be read from a corresponding
 * extractor with the "read_quoted" method.
 */
 ...
 *
 *  This function computes the edit distance ("Levenshtein distance") between
 * two strings.
 */
 ...
 *
 *  The string returned by this method can be read from a corresponding
 * extractor with the "read_word_or_quoted" method with the given "non_term"
 * characters.
 */
TL_PUBLIC std::string to_word_or_quoted_string(const std::string &s,
                                               const char *non_term = "_.$");

/**
 *  @brief Escapes HTML (or XML) characters from in and adds the result to out
 *  If "replace_newlines" is true, "\n" will be replaced by "<br/>".
 */
TL_PUBLIC void escape_to_html(std::string &out, const std::string &in,
                              bool replace_newlines = true);

/**
 *  @brief Escapes HTML (or XML) characters from in and returns the resulting
 * string Double quotes are substituted by "&quot;" which makes the resulting
 * string usable for attributes (in double quotes) too. If "replace_newlines" is
 * true, "\n" will be replaced by "<br/>".
 */
TL_PUBLIC std::string escaped_to_html(const std::string &in,
                                      bool replace_newlines = true);

/**
 *  @brief Replaces the "before" string by "after" in the "subject" string and
 * returns the new string All occurrences are replaced.
 */
TL_PUBLIC std::string replaced(const std::string &subject,
...
/**
 *  @brief Fills the string (to the right) with blanks until the desired length
 * is reached
 */
TL_PUBLIC std::string pad_string_right(unsigned int n, const std::string &s);

/**
 *  @brief Fills the string (to the left) with blanks until the desired length
 * is reached
 */
...
/**
 *  @brief A standard method to convert a coordinate in database units (pixel)
 ...
 * into a string for display
 */
inline std::string db_to_string(int32_t d) { return to_string(d); }

/**
 *  @brief A standard method to convert a coordinate in database units (pixel)
 * into a string for display
 */
inline std::string db_to_string(int64_t d) { return to_string(d); }
 ...
 *  It is possible to customize the extractor by overloading the error method
 * for example.
 */
...
public:
   ...
   */
  template <class T> Extractor &read(T &value) {
   ...
   *
   *  Name strings are like words, but for the first character digits are not
   * allowed.
   */
   ...
   *
   *  The string may be either quoted with single or double quotes. Quotes
   * inside the string may be escaped with a backslash character.
   */
  Extractor &read_quoted(std::string &value);

  /**
   *  @brief Read a string consisting of "word" characters or accept quoted
   * strings
   *
   *  The string may be either given in the form allowed by "read_word" (with
   * the non_term characters) or by "read_quoted".
   */
  Extractor &read_word_or_quoted(std::string &value,
                                 const char *non_term = "_.$");
   ...
   */
  bool try_read_word_or_quoted(std::string &value,
                               const char *non_term = "_.$");
   ...
   */
  template <class T> bool try_read(T &value) {
   ...
   */
  char operator*() const { return *m_cp; }

  /**
   *  @brief Access to the current position
   */
  const char *get() const { return m_cp; }
   ...
   *
   *  This is not really a const method since it does a "skip" to determine if
   * we are at the end already.
   */
  bool at_end() { return *skip() == 0; }
   ...
   */
  template <class X> Extractor &operator>>(X &x) { return read(x); }

  /**
   *  @brief Extract a token (constant string)
   */
  Extractor &operator>>(const char *token) { return expect(token); }

  /**
   *  @brief Expect the end of the string
   */
  Extractor &operator>>(end) { return expect_end(); }
...
TL_PUBLIC std::string sprintf(const char *fmt,
                              const std::vector<tl::Variant> &a,
                              unsigned int a0 = 0);
TL_PUBLIC std::string sprintf(const std::string &fmt,
                              const std::vector<tl::Variant> &a,
                              unsigned int a0 = 0);
...
inline std::string sprintf(const std::string &fmt, const tl::Variant &a1,
                           const tl::Variant &a2, const tl::Variant &a3,
                           const tl::Variant &a4, const tl::Variant &a5) {
  std::vector<tl::Variant> a;
  a.push_back(a1);
  a.push_back(a2);
  a.push_back(a3);
...
inline std::string sprintf(const std::string &fmt, const tl::Variant &a1,
                           const tl::Variant &a2, const tl::Variant &a3,
                           const tl::Variant &a4, const tl::Variant &a5,
                           const tl::Variant &a6, const tl::Variant &a7) {
  std::vector<tl::Variant> a;
  a.push_back(a1);
  a.push_back(a2);
  a.push_back(a3);
...
TL_PUBLIC std::vector<std::string> split(const std::string &s,
                                         const std::string &sep);

/**
 *  @brief Joins a generic iterated list into a single string using the given
 * separator
 */
template <class Iter>
TL_PUBLIC_TEMPLATE std::string join(Iter from, Iter to,
                                    const std::string &sep) {
  std::ostringstream r;
...
/**
 *  @brief Joins a vector of strings into a single string using the given
 * separator
 */
 ...
 */
inline bool is_cr(char c) { return c == '\015'; }

/**
 *  @brief checks if the given character is a LF character
 */
inline bool is_lf(char c) { return c == '\012'; }

/**
 *  @brief checks if the given character is a CR or LF character
 */
inline bool is_newline(char c) { return is_cr(c) || is_lf(c); }
