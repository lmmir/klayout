...
std::string parse_realm(const std::string &header) {
  ...
  for (std::vector<std::string>::const_iterator l = lines.begin();
    ...
    while (!ex.at_end()) {
      ...
      if (!ex.try_read_word(key) || !ex.test("=") ||
          !ex.try_read_word_or_quoted(value)) {
        break;
      }
...
public:
  enum Mode { UseAsIs, Inquire, ForceInquire };

  CurlCredentialManager(bool proxy) : m_proxy(proxy), mp_provider(0) {
    //  .. nothing yet ..
  }

  const std::pair<std::string, std::string> *
  user_password(const std::string &url, const std::string &realm, int attempt,
                Mode mode) {
    ...
    if (mode != ForceInquire) {
      ...
      std::map<std::pair<std::string, std::string>,
               std::pair<std::string, std::string>>::const_iterator c =
          m_credentials.find(std::make_pair(server, realm));
    ...
    if (mode != UseAsIs && mp_provider.get()) {
      ...
      if (!mp_provider->user_password(url, realm, m_proxy, attempt, user,
                                      password)) {
  ...
  void set_credentials(const std::string &url, const std::string &realm,
  ...
  HttpCredentialProvider *provider() { return mp_provider.get(); }
...
private:
  struct ChunkInfo {
    ChunkInfo() : pos(0), start(0), size(0) {
      //  .. nothing yet ..
    }

    ChunkInfo(const ChunkInfo &other) : pos(0), start(0), size(0) {
      operator=(other);
    }
    ...
    size_t available() const { return size - (pos - start); }
...
public:
  ...
  std::string to_string() const {
    std::string s;
    s.reserve(size());
    for (std::list<ChunkInfo>::const_iterator c = m_chunks.begin();
         c != m_chunks.end(); ++c) {
...
public:
   ...
   */
  int http_status() const { return m_http_status; }
...
private:
  ...
  friend size_t read_func(char *buffer, size_t size, size_t nitems,
                          void *userdata);
  friend size_t write_func(char *ptr, size_t size, size_t nmemb,
                           void *userdata);
  friend size_t write_header_func(char *ptr, size_t size, size_t nmemb,
                                  void *userdata);
...
public:
   ...
   *  Connection objects are the basic keys for implementing connections via
   * curl. Use the returned object to send and receive data. The returned object
   ...
   */
  CurlCredentialManager &credentials() { return m_credentials; }

  /**
   *  @brief The proxy_credentials manager object
   */
  CurlCredentialManager &proxy_credentials() { return m_proxy_credentials; }
...
void InputHttpStream::send() { mp_data->send(); }

void InputHttpStream::close() { mp_data->close(); }

void InputHttpStream::set_request(const char *r) { mp_data->set_request(r); }

void InputHttpStream::set_data(const char *data) { mp_data->set_data(data); }

void InputHttpStream::set_data(const char *data, size_t n) {
  mp_data->set_data(data, n);
}

void InputHttpStream::add_header(const std::string &name,
                                 const std::string &value) {
  mp_data->add_header(name, value);
}

tl::Event &InputHttpStream::ready() { return mp_data->ready(); }

bool InputHttpStream::data_available() { return mp_data->data_available(); }

size_t InputHttpStream::read(char *b, size_t n) { return mp_data->read(b, n); }

void InputHttpStream::reset() { mp_data->reset(); }

std::string InputHttpStream::source() const { return mp_data->source(); }

std::string InputHttpStream::absolute_path() const {
  return mp_data->absolute_path();
}

std::string InputHttpStream::filename() const { return mp_data->filename(); }

bool InputHttpStream::is_available() { return true; }
...
void InputHttpStream::set_timeout(double to) { mp_data->set_timeout(to); }

double InputHttpStream::timeout() const { return mp_data->timeout(); }
...
CurlConnection::CurlConnection(CURL *handle) : mp_handle(handle) {
...
void CurlConnection::set_url(const char *url) { m_url = url; }

void CurlConnection::set_request(const char *request) { m_request = request; }

void CurlConnection::add_header(const char *header, const char *value) {
#if defined(DEBUG_CURL)
  std::cerr << "CurlConnection::set_header(" << header << ", " << value << ")"
            << std::endl;
#endif
  if (!value) {
    mp_headers =
        curl_slist_append(mp_headers, (std::string(header) + ";").c_str());
  } else {
    mp_headers = curl_slist_append(
        mp_headers, (std::string(header) + ": " + std::string(value)).c_str());
  }
}
...
size_t CurlConnection::read_available() const { return m_read_data.size(); }
...
#if defined(DEBUG_CURL)
  std::cerr << "CurlConnection::add_header_data(" << n << " bytes)"
            << std::endl;
#endif
...
void CurlConnection::check() const {
  if (m_status < 0) {
    ...
    throw tl::HttpErrorException(
        tl::sprintf(tl::to_string(tr("Connection error (%s)")), m_error_msg),
        m_status, m_url);
    ...
    if (error_text) {
      ...
      throw tl::HttpErrorException(tl::to_string(tr("HTTP error")),
                                   m_http_status, m_url);
    }
  }
}
...
void CurlConnection::finished(int status) {
  ...
  if ((http_code == 401 || http_code == 407)) {
    ...
    try {
      ...
      //  Note: on the second attempt use ForceInquire, so we don't reuse the
      //  wrong credentials
      CurlCredentialManager::Mode mode =
          m_authenticated == 0 ? CurlCredentialManager::Inquire
                               : CurlCredentialManager::ForceInquire;

      if (proxy_auth) {
        pwd = CurlNetworkManager::instance()->proxy_credentials().user_password(
            m_url, realm, m_authenticated + 1, mode);
      } else {
        pwd = CurlNetworkManager::instance()->credentials().user_password(
            m_url, realm, m_authenticated + 1, mode);
      }
...
#if defined(DEBUG_CURL)
      std::cerr << "CurlConnection::finished: resend with authentication data"
                << std::endl;
#endif
...
size_t write_header_func(char *buffer, size_t size, size_t nitems,
                         void *userdata) {
  CurlConnection *connection = (CurlConnection *)userdata;
...
CurlNetworkManager::CurlNetworkManager()
    : dm_tick(this, &CurlNetworkManager::on_tick), m_still_running(0),
      m_credentials(false), m_proxy_credentials(true) {
#if defined(DEBUG_CURL)
  ...
  //  will register the object in the static object repo - it's cleaned up
...
#if defined(DEBUG_CURL)
  std::cerr << "CurlNetworkManager::start(" << (void *)connection->mp_handle
            << ")" << std::endl;
#endif
...
#if defined(DEBUG_CURL)
    std::cerr << "CurlNetworkManager::release_connection("
              << (void *)connection->mp_handle << ")" << std::endl;
#endif
    curl_easy_cleanup(connection->mp_handle);
    m_handle_refcount.erase(connection->mp_handle);

    std::map<CURL *, CurlConnection *>::iterator h =
        m_handle2connection.find(connection->mp_handle);
...
bool CurlNetworkManager::has_reply() const { return m_still_running <= 0; }
...
void CurlNetworkManager::tick() {
  ...
  if (mc != CURLM_OK) {
    throw tl::HttpErrorException(
        tl::to_string(tr("Connection error (curl_multi_fdset() failed)")), mc,
        std::string());
  }
  ...
  if (m_still_running < sr) {
    ...
    while ((msg = curl_multi_info_read(mp_multi_handle, &msgs_left))) {
      ...
      std::map<CURL *, CurlConnection *>::iterator h =
          m_handle2connection.find(msg->easy_handle);
      if (h != m_handle2connection.end()) {
...
InputHttpStreamPrivateData::InputHttpStreamPrivateData(InputHttpStream *stream,
  ...
  m_connection->data_available_event.add(
      this, &InputHttpStreamPrivateData::on_data_available);
  m_connection->finished_event.add(this,
...
void InputHttpStreamPrivateData::set_timeout(double to) { m_timeout = to; }

double InputHttpStreamPrivateData::timeout() const { return m_timeout; }
...
void InputHttpStreamPrivateData::add_header(const std::string &name,
                                            const std::string &value) {
...
size_t InputHttpStreamPrivateData::read(char *b, size_t n) {
  if (!m_sent) {
    send();
  }
  ...
  {
    //  Prevents deferred methods to be executed during the processEvents below
    //  (undesired side effects)
    tl::NoDeferredMethods silent;

    if (!m_progress.get()) {
      m_progress.reset(new tl::AbsoluteProgress(
          tl::to_string(tr("Downloading")) + " " + m_connection->url(), 1));
    }

    tl::Clock start_time = tl::Clock::current();
    while (n > m_connection->read_available() && !m_connection->finished() &&
           (m_timeout <= 0.0 ||
            (tl::Clock::current() - start_time).seconds() < m_timeout) &&
           !tl::CurlNetworkManager::instance()->has_reply()) {
      mp_stream->tick();
  ...
  if (m_connection->finished()) {
    ...
    tl::info << "HTTP response data read: "
             << m_connection->read_data_to_string();
  }
...
void InputHttpStreamPrivateData::reset() {
  throw tl::Exception(
      tl::to_string(tr("'reset' is not supported on HTTP input streams")));
}

std::string InputHttpStreamPrivateData::filename() const {
  return tl::filename(tl::URI(m_connection->url()).path());
}

std::string InputHttpStreamPrivateData::source() const {
  return m_connection->url();
}

std::string InputHttpStreamPrivateData::absolute_path() const {
  return m_connection->url();
}
