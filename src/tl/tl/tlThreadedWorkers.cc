...
TaskList::TaskList() : mp_first(0), mp_last(0) {
  // .. nothing yet ..
}
...
JobBase::JobBase(int nworkers)
    : m_nworkers(nworkers), m_idle_workers(0), m_stopping(false),
      m_running(false) {
  if (nworkers > 0) {
...
void JobBase::log_error(const std::string &s) {
  ...
  if (m_error_messages.size() == max_errors) {
    m_error_messages.push_back(
        tl::to_string(tr("Error list abbreviated (more errors were ignored)")));
...
std::vector<std::string> JobBase::error_messages() {
...
void JobBase::start() {
  ...
  //  This serves as a synchronization measure such that each task gets called
  //  once and the empty queue detection works properly.
...
bool JobBase::is_running() { return m_running; }
...
bool JobBase::wait(long timeout) {
  ...
  m_lock.lock();
  if (m_nworkers > 0 && m_running &&
      !m_queue_empty_condition.wait(
          &m_lock, timeout >= 0 ? (unsigned long)timeout
...
void JobBase::terminate() {
  ...
  if (!mp_workers.empty()) {
    ...
    for (std::vector<Worker *>::iterator w = mp_workers.begin();
         w != mp_workers.end(); ++w) {
      delete (*w);
    }
...
void JobBase::schedule(Task *task) {
  ...
  if (m_stopping) {
    ...
    //  Don't allow tasks to be scheduled while stopping or exiting (waiting for
    //  m_queue_empty_condition)
    delete task;
...
Task *JobBase::get_task(int worker) {
  while (true) {
    ...
    while (m_task_list.is_empty() &&
           mp_per_worker_task_lists[worker].is_empty()) {
...
/**
 *  @brief A progress adaptor that will create a thread-specific progress
 * environment
 *
...
void Worker::reset_stop_request() { m_stop_requested = false; }

void Worker::stop_request() { m_stop_requested = true; }
