...
IncludeExpander IncludeExpander::expand(const std::string &path,
                                        std::string &expanded_text,
                                        const IncludeFileResolver *resolver) {
  IncludeExpander ie;
  int lc = 1;
...
void IncludeExpander::read(const std::string &path, tl::InputStream &is,
...
std::string IncludeExpander::to_string() const {
  if (m_sections.empty()) {
    ...
    for (std::map<int, std::pair<std::string, int>>::const_iterator m =
             m_sections.begin();
         m != m_sections.end(); ++m) {
...
IncludeExpander IncludeExpander::from_string(const std::string &s) {
  IncludeExpander ie;
...
IncludeExpander::translate_to_original(int line_number) {
  std::map<int, std::pair<std::string, int>>::const_iterator m =
      m_sections.lower_bound(line_number);
  if (m != m_sections.begin() &&
      (m == m_sections.end() || m->first > line_number)) {
    --m;
  }
  if (m == m_sections.end()) {
