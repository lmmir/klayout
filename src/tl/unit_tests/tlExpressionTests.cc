...
public:
  ...
  int width() const { return m_p2.x() - m_p1.x(); }

  int height() const { return m_p2.y() - m_p1.y(); }

  bool empty() const { return m_p1.x() > m_p2.x(); }
...
public:
  ...
  Edge(int x1, int y1, int x2, int y2) : m_p1(x1, y1), m_p2(x2, y2) {
    //  .. nothing else ..
  }

  Edge operator*(double s) const {
    return Edge(int(floor(m_p1.x() * s + 0.5)), int(floor(m_p1.y() * s + 0.5)),
                int(floor(m_p2.x() * s + 0.5)), int(floor(m_p2.y() * s + 0.5)));
  }
...
public:
  ...
  virtual void to_variant(const void *, tl::Variant &) const {
    tl_assert(false);
  }
...
void BoxClass::execute(const tl::ExpressionParserContext &context,
                       tl::Variant &out, tl::Variant &object,
                       const std::string &method,
                       const std::vector<tl::Variant> &args) const {
  if (method == "width") {
    ...
    out = tl::Variant(new Box(object.to_user<Box>() & args[0].to_user<Box>()),
                      &BoxClass::instance, true);
  } else if (method == "to_s") {
...
void BoxClassClass::execute(const tl::ExpressionParserContext &context,
                            tl::Variant &out, tl::Variant & /*object*/,
                            const std::string &method,
                            const std::vector<tl::Variant> &args) const {
  if (method == "new") {
    out = tl::Variant(new Box(args[0].to_long(), args[1].to_long(),
                              args[2].to_long(), args[3].to_long()),
                      &BoxClass::instance, true);
  } else {
    throw tl::NoMethodError("Box", method, context);
  }
...
public:
  virtual void execute(const tl::ExpressionParserContext &context,
    ...
    if (method == "dx") {
      ...
      out.set_user(new Edge(object.to_user<Edge>() * args[0].to_double()),
                   object.user_cls(), true);
    } else {
...
void EdgeClassClass::execute(const tl::ExpressionParserContext &context,
                             tl::Variant &out, tl::Variant & /*object*/,
                             const std::string &method,
                             const std::vector<tl::Variant> &args) const {
  if (method == "new") {
    out = tl::Variant(new Edge(args[0].to_long(), args[1].to_long(),
                               args[2].to_long(), args[3].to_long()),
                      &EdgeClass::instance, true);
  } else {
    throw tl::NoMethodError("Edge", method, context);
  }
}
...
TEST(1b) {
  ...
  e.set_var("b",
            tl::Variant(new Box(0, 10, 20, 40), &BoxClass::instance, true));
  e.set_var("e",
            tl::Variant(new Edge(0, 10, 20, 40), &EdgeClass::instance, true));
  ...
  EXPECT_EQ(v.to_string(), std::string("false"));
  v = e.parse("XBox.new(0, 0, 100, 200) & XBox.new(10, 10, 110, 210)")
          .execute();
  EXPECT_EQ(v.to_string(), std::string("(10,10;100,200)"));
  v = e.parse("XBox.new(0, 0, 100, 200) & XBox.new(1000, 1000, 1010, 1010)")
          .execute();
...
TEST(6) {
  ...
  EXPECT_EQ(v.to_string(),
            tl::sprintf("%g %e %f", M_PI, M_PI * 1e6, M_PI * 0.001));
  v = e.parse("sprintf('%g %e %f',M_PI*1e6,M_PI*1e6,M_PI*1e6)").execute();
  EXPECT_EQ(v.to_string(),
            tl::sprintf("%g %e %f", M_PI * 1e6, M_PI * 1e6, M_PI * 1e6));
  v = e.parse(
           "sprintf('%-15g %015.8e %15.12f %g',M_PI,M_PI*1e6,M_PI*0.001,M_PI)")
          .execute();
  EXPECT_EQ(v.to_string(), tl::sprintf("%-15g %015.8e %15.12f %g", M_PI,
                                       M_PI * 1e6, M_PI * 0.001, M_PI));
  v = e.parse("sprintf('%-5s %5s %x %u %d (%s)','a','b',1234,2345,3456)")
          .execute();
  EXPECT_EQ(v.to_string(),
            tl::sprintf("%-5s %5s %x %u %d ()", "a", "b", 1234, 2345, 3456));
  std::string msg;
  try {
...
class F0 : public tl::EvalFunction {
public:
  void execute(const tl::ExpressionParserContext &, tl::Variant &out,
               const std::vector<tl::Variant> &) const {
    out = tl::Variant(17);
  }
};

class F1 : public tl::EvalFunction {
public:
  void execute(const tl::ExpressionParserContext &, tl::Variant &out,
...
class F2 : public tl::EvalFunction {
public:
  void execute(const tl::ExpressionParserContext &, tl::Variant &out,
               const std::vector<tl::Variant> &vv) const {
...
class F3 : public tl::EvalFunction {
public:
  void execute(const tl::ExpressionParserContext &, tl::Variant &out,
               const std::vector<tl::Variant> &vv) const {
...
TEST(18) {
  ...
  v = e.parse("var i=CellInstArray.new(17,tr,a,b,100,200); i.is_complex()")
          .execute();
  EXPECT_EQ(v.to_string(), std::string("false"));
}
...
TEST(19) {
  ...
  tl::Variant v;
  v = e.parse("# A comment\nvar i=CellInstArray.new(17,tr,a,b,100,200); "
              "i.to_s(); # A final comment")
          .execute();
  EXPECT_EQ(v.to_string(), std::string("#17 r90 10,20 [1,2*100;11,22*200]"));
}
...
TEST(20) {
  ...
  e.parse("var i1 = "
          "top.insert(CellInstArray.new(cell.cell_index,Trans.new(Vector.new("
          "100,200))))")
      .execute();
  e.parse("var i2 = "
          "top.insert(CellInstArray.new(cell.cell_index,Trans.new(Vector.new(-"
