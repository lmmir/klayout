...
TEST(1) {
  ...
  EXPECT_EQ(norm_exp(tl::sprintf("%g %e %f", M_PI, M_PI * 1e6, M_PI * 0.001)),
            "3.14159 3.141593e+06 0.003142");
  EXPECT_EQ(
      norm_exp(tl::sprintf("%G %E %F", M_PI * 1e6, M_PI * 1e6, M_PI * 1e6)),
      "3.14159E+06 3.141593E+06 3141592.653590");
  EXPECT_EQ(norm_exp(tl::sprintf("%-15g %015.8e %15.12f %g", M_PI, M_PI * 1e6,
                                 M_PI * 0.001, M_PI)),
            "3.14159         03.14159265e+06  0.003141592654 3.14159");
  EXPECT_EQ(norm_exp(tl::sprintf("%-15g %015.8E %15.12f %g", M_PI, M_PI * 1e6,
                                 M_PI * 0.001, M_PI)),
            "3.14159         03.14159265E+06  0.003141592654 3.14159");
  EXPECT_EQ(tl::sprintf("%-5s %5s %x %u %d (%s)", "a", "b", 1234, 2345, 3456),
            "a         b 4d2 2345 3456 ()");
...
TEST(15) {
  ...
  try {
    EXPECT_EQ(tl::to_string_from_local(
                  tl::to_local("H\xc3\xa4llo\tW\xc3\xb6rld!").c_str()),
              "H\xc3\xa4llo\tW\xc3\xb6rld!");
  ...
  EXPECT_EQ(tl::to_string(
                tl::to_wstring("Utf8 supports emoticons: \xF0\x9F\x98\x81\nand "
                               "Umlauts: \xc3\xa4\xc3\xbc\xc3\xb6"))
                .c_str(),
            "Utf8 supports emoticons: \xF0\x9F\x98\x81\nand Umlauts: "
            "\xc3\xa4\xc3\xbc\xc3\xb6");

  EXPECT_EQ(
      tl::to_upper_case(
          "nOrMaliI(\xc3\xa4\xc3\x84\xc3\xbc\xc3\x9c\xc3\xb6\xc3\x96\xc3\x9f-"
          "42\xc2\xb0+6\xe2\x82\xac)"),
      "NORMALII(\xc3\x84\xc3\x84\xc3\x9c\xc3\x9c\xc3\x96\xc3\x96\xc3\x9f-"
      "42\xc2\xb0+6\xe2\x82\xac)");
  EXPECT_EQ(
      tl::to_lower_case(
          "nOrMaliI(\xc3\xa4\xc3\x84\xc3\xbc\xc3\x9c\xc3\xb6\xc3\x96\xc3\x9f-"
          "42\xc2\xb0+6\xe2\x82\xac)"),
      "normalii(\xc3\xa4\xc3\xa4\xc3\xbc\xc3\xbc\xc3\xb6\xc3\xb6\xc3\x9f-"
      "42\xc2\xb0+6\xe2\x82\xac)");
}
