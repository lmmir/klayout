...
public:
  MyClass(int attr = 0) : tl::Object(), m_attr(attr) { ++s_myclass_instances; }

  ~MyClass() { --s_myclass_instances; }
...
TEST(23) {
...

  {
    ...
    EXPECT_EQ(((const tl::shared_collection<MyClass> &)sc).begin() ==
                  ((const tl::shared_collection<MyClass> &)sc).end(),
              true);
...
public:
  WCMonitor() : changed_count(0), about_to_change_count(0) {}
...
TEST(41) {
  ...
  for (std::vector<MyClass *>::const_iterator i = objects.begin();
       i != objects.end(); ++i) {
    delete *i;
  }
  objects.clear();
