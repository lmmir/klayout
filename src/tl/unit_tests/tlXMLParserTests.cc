...
struct Child {
  ...
  double d;
  bool live;
  bool operator==(const Child &x) const {
    return txt == x.txt && fabs(d - x.d) < 1e-9 && children == x.children;
  }
...
struct Root {
  ...
  bool operator==(const Root &x) const {
    return m == x.m && mi == x.mi && m_subs == x.m_subs &&
  ...
  void add_sub(const double &s) { m_subs.push_back(s); }
  void add_isub(const int &s) { m_isubs.push_back(s); }
  std::vector<double>::const_iterator begin_subs() const {
    return m_subs.begin();
  }
  std::vector<double>::const_iterator end_subs() const { return m_subs.end(); }
  std::vector<int>::const_iterator begin_isubs() const {
    return m_isubs.begin();
  }
  std::vector<int>::const_iterator end_isubs() const { return m_isubs.end(); }
  void add_child_ptr(Child *c) {
    m_children.push_back(*c);
    delete c;
  }
  void add_child(const Child &c) { m_children.push_back(c); }
  std::vector<Child>::const_iterator begin_children() const {
...
TEST(1) {
  ...
  tl::XMLStruct<Root> structure(
      "root", tl::make_member(&Root::begin_subs, &Root::end_subs,
                              &Root::add_sub, "sub") +
                  tl::make_member(&Root::begin_isubs, &Root::end_isubs,
                                  &Root::add_isub, "isub") +
                  tl::make_element(&Root::begin_children, &Root::end_children,
...
TEST(5) {
  ...
  tl::XMLStruct<Root> structure("root", tl::make_member(&Root::m, "member"));
...
#if !defined(HAVE_EXPAT)
  EXPECT_EQ(error, "XML parser error: Unexpected text after numeric value: "
                   "'...a' in line 2, column 27");
#else
  //  expat delivers cdata at beginning of closing tag
  EXPECT_EQ(error, "XML parser error: Unexpected text after numeric value: "
                   "'...a' in line 2, column 18");
#endif
}
...
TEST(7) {
  ...
  tl::XMLStruct<Root> structure(
      "root", tl::make_member(&Root::add_sub, "sub") +
...
TEST(8) {
  ...
  tl::XMLElementList child_struct =
      tl::make_member(&Child::txt, "t") + tl::make_member(&Child::d, "d") +
      tl::make_element(&Child::begin_children, &Child::end_children,
                       &Child::add_child, "child", &child_struct);

  tl::XMLStruct<Root> structure(
      "root", tl::make_element(&Root::begin_children, &Root::end_children,
                               &Root::add_child, "child", &child_struct) +
                  tl::make_member(&Root::m, "member"));
...
TEST(8a) {
  ...
  tl::XMLElementList child_struct =
      tl::make_member(&Child::txt, "t") + tl::make_member(&Child::d, "d") +
      tl::make_element(&Child::begin_children, &Child::end_children,
                       &Child::add_child_ptr, "child", &child_struct);

  tl::XMLStruct<Root> structure(
      "root", tl::make_element(&Root::begin_children, &Root::end_children,
                               &Root::add_child_ptr, "child", &child_struct) +
                  tl::make_member(&Root::m, "member"));
...
TEST(9) {
  ...
  std::string x = "<?xml version=\"1.0\"?>\n"
                  ...
                  "    <child><t>C1</t></child>\n"
                  "    "
                  "<child><t>c2</t><child><t>d2</t><d>-1.25</"
                  "d><unknown-data>blabla</unknown-data></child></child>\n"
                  "    <d>2.5</d><d>125e-3</d>\n"
                  ...
                  "    "
                  "<child><t>c2</t><child><t>d2</t><d>-1.25</"
                  "d><unknown-data>blabal</unknown-data></child></child>\n"
                  "    <d>2.5</d><d>125e-3</d>\n"
                  "    <unknown-data>2.5</unknown-data>\n"
                  "  </child>\n"
...
#if defined(HAVE_EXPAT) ||                                                     \
    QT_VERSION <                                                               \
        0x60000 // #QTBUG-98656 (XML reader does not read encoding properly)
TEST(11) {
  //  iso8859-1 encoding
