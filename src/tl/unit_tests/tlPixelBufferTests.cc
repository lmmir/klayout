...
static bool compare_images(const QImage &qimg, const std::string &au) {
  ...
  if (qimg2.width() == (int)qimg.width() &&
      qimg2.height() == (int)qimg.height()) {
    for (int j = 0; j < qimg.height(); ++j) {
      for (int i = 0; i < qimg.width(); ++i) {
        if (((const tl::color_t *)qimg.scanLine(j))[i] !=
            ((const tl::color_t *)qimg2.scanLine(j))[i]) {
          return false;
        }
      }
    }
...
static bool compare_images_mono(const QImage &qimg, const std::string &au) {
  QImage qimg2;
  qimg2.load(tl::to_qstring(au));
  qimg2 = qimg2.convertToFormat(
      QImage::Format_MonoLSB); // that's the format of BitmapBuffer ..
  ...
  if (qimg2.width() == (int)qimg.width() &&
    ...
    for (int j = 0; j < qimg.height(); ++j) {
      for (int i = 0; i < qimg.width(); ++i) {
        if ((qimg.scanLine(j)[i / 8] & (0x01 << (i % 8))) !=
            (qimg2.scanLine(j)[i / 8] & (0x01 << (i % 8)))) {
          return false;
        }
      }
    }
...
static bool compare_images(const tl::PixelBuffer &img,
                           const tl::PixelBuffer &img2) {
  return img == img2;
}

static bool compare_images(const tl::BitmapBuffer &img,
                           const tl::BitmapBuffer &img2) {
  return img == img2;
}
...
TEST(1) {
  ...
  //  other constructors
  EXPECT_EQ(compare_images(tl::PixelBuffer(img4.width(), img4.height(),
                                           (const tl::color_t *)img4.data()),
                           img4),
            true);
  EXPECT_EQ(compare_images(tl::PixelBuffer(img4.width(), img4.height(),
                                           (const tl::color_t *)img4.data(),
  ...
  tl::color_t *dnew =
      new tl::color_t[img4.width() * img4.height() * sizeof(tl::color_t)];
  memcpy(dnew, (const tl::color_t *)img4.data(),
         img4.width() * img4.height() * sizeof(tl::color_t));
  EXPECT_EQ(
      compare_images(tl::PixelBuffer(img4.width(), img4.height(), dnew), img4),
      true);
}
...
TEST(7) {
  {
    tl::SelfTimer timer(
        "Run time - tl::Image copy, no write (should be very fast)");
  ...
  {
    tl::SelfTimer timer(
        "Run time - QImage copy, no write (should be very fast)");
...
#if defined(HAVE_QT)
  {
    tl::SelfTimer timer("Run time - QImage copy on write (should not be much "
                        "less than tl::Image copy on write)");
  ...
  {
    tl::SelfTimer timer("Run time - tl::Image paint (should not be much more "
                        "than direct QImage paint)");
...
TEST(11) {
  ...
  //  other constructors
  EXPECT_EQ(compare_images(tl::BitmapBuffer(img4.width(), img4.height(),
                                            (const uint8_t *)img4.data()),
                           img4),
            true);
  EXPECT_EQ(compare_images(tl::BitmapBuffer(img4.width(), img4.height(),
                                            (const uint8_t *)img4.data(),
  ...
  EXPECT_EQ(
      compare_images(tl::BitmapBuffer(img4.width(), img4.height(), dnew), img4),
      true);
}
