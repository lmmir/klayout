...
struct TestSearch {
  ...
  std::string to_string() const {
    return (_eq ? ">=" : ">") + std::string("(") + tl::to_string(_i1) + "," +
           tl::to_string(_i2) + ")";
  }
...
static void test_tree(tl::TestBase *_this, const Tree &t, const Picker &p,
                      const Search &s) {
  if (tl::verbose()) {
  ...
  if (tl::verbose()) {
    for (typename Tree::size_type i = 0; i < t.size(); ++i) {
      std::cout << i << " b=" << t.bounds()[i] << ", v=" << p(0, t.objects()[i])
                << "," << p(1, t.objects()[i]) << std::endl;
    }
  }
