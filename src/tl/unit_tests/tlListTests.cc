...
struct MyClass1 : public tl::list_node<MyClass1> {
  ...
  MyClass1(const MyClass1 &other) : tl::list_node<MyClass1>(), n(other.n) {
    ++obj_count;
  }
  ~MyClass1() { --obj_count; }
  int n;
...
template <class C> static std::string l2s(const tl::list<C> &l) {
  std::string x;
...
template <class C> static std::string l2sr(const tl::list<C> &l) {
  std::string x;
  for (typename tl::list<C>::const_reverse_iterator i = l.rbegin();
       i != l.rend(); ++i) {
    if (!x.empty()) {
      x += ",";
    }
...
template <class C> static std::string l2sm(const tl::list<C> &l) {
  std::string x;
...
template <class C> static std::string l2srm(const tl::list<C> &l) {
  std::string x;
  for (typename tl::list<C>::const_reverse_iterator i = l.rend();
       i != l.rbegin();) {
    --i;
    if (!x.empty()) {
      x += ",";
...
template <class C> static std::string l2s_nc(tl::list<C> &l) {
  std::string x;
...
template <class C> static std::string l2sr_nc(tl::list<C> &l) {
  std::string x;
  for (typename tl::list<C>::reverse_iterator i = l.rbegin(); i != l.rend();
       ++i) {
    if (!x.empty()) {
      x += ",";
    }
...
template <class C> static std::string l2sm_nc(tl::list<C> &l) {
  std::string x;
...
template <class C> static std::string l2srm_nc(tl::list<C> &l) {
  std::string x;
