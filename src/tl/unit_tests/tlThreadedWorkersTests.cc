...
TEST(1) {
  ...
  n = 0;
  for (tl::Boss::iterator j = boss1.begin(); j != boss1.end(); ++j) {
    ++n;
  }
  EXPECT_EQ(n, size_t(1));

  n = 0;
  for (tl::Boss::iterator j = boss2.begin(); j != boss2.end(); ++j) {
    ++n;
  }
  EXPECT_EQ(n, size_t(1));
...
TEST(10) {
  ...
  EXPECT_EQ(s_sum[0].sum() + s_sum[1].sum() + s_sum[2].sum() + s_sum[3].sum(),
            10000000);
}
...
TEST(12) {
  ...
  EXPECT_EQ((s_sum[0].sum() % 1000) + (s_sum[1].sum() % 1000) +
                    (s_sum[2].sum() % 1000) + (s_sum[3].sum() % 1000) >
                0,
            true);
  EXPECT_EQ(s_sum[0].sum() + s_sum[1].sum() + s_sum[2].sum() + s_sum[3].sum() <
                10000000,
...
void run_thread_tests(tl::TestBase *_this, int wait) {
  ...
  for (int i = 0; i < tries && !stopped_in_action; ++i) {
    ...
    EXPECT_EQ(job.is_running(), false);
    EXPECT_EQ(s_sum[0].sum() + s_sum[1].sum() + s_sum[2].sum() +
                      s_sum[3].sum() <
                  400000000,
              true);

    //  at least one must be stopped in the perform task - as this is not always
    //  the case, we retry a few times.
    stopped_in_action = (s_sum[0].sum() % 10000) + (s_sum[1].sum() % 10000) +
                            (s_sum[2].sum() % 10000) +
                            (s_sum[3].sum() % 10000) >
                        0;
  }
...
TEST(13) { run_thread_tests(_this, 20000); }

TEST(14) { run_thread_tests(_this, 200000); }
...
TEST(22) {
  ...
  for (int l = 0; l < 20; ++l) {
    ...
    job.start();
    while (!s_sum[0].flag() || !s_sum[1].flag() || !s_sum[2].flag() ||
           !s_sum[3].flag()) {
      tl::usleep(10000);
    }
    job.stop();
...
TEST(24) {
  tl::SelfTimer timer(
      "4 threads, 20 iterations with at least one thread running");
  MyJob job(4);
...
TEST(25) {
  tl::SelfTimer timer(
      "2 threads, 40 iterations with all at least one thread running");
  MyJob job(2);
