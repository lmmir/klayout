...
TEST(1) {
  ...
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("http://www.klayout.de"))),
            "<http>://<www.klayout.de>");
  ...
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("http://www.klayout.de"))),
            "<http>://<www.klayout.de>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("http:///other"))),
            "<http>:</other>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("../other"))),
            "<file>:</c:/path/to/file/../other>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("/other"))), "<file>:</other>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("file:../other"))),
            "<file>:</c:/path/to/file/../other>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("file:../other?a=b#frag"))),
            "<file>:</c:/path/to/file/../other>?<a>=<b>#<frag>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("file:/other"))),
            "<file>:</other>");
  EXPECT_EQ(uri2string(uri.resolved(tl::URI("file:/other?a=b#frag"))),
            "<file>:</other>?<a>=<b>#<frag>");
  ...
  EXPECT_EQ(uri2string(uri),
            "</path/to/file>?<a>=<v1>&<b>=<v2>&<c>=<v3>#<fragment>");
  ...
  EXPECT_EQ(tl::URI(uri.to_string()).to_string(),
            "/path/to/%2C%2C%20%file#fragment");
...
TEST(3_pathsWithPlus) {
  EXPECT_EQ(
      tl::URI("/users/a_plus_b").resolved(tl::URI("file.txt")).to_string(),
      "/users/a_plus_b/file.txt");
  EXPECT_EQ(tl::URI("/users/a+b").resolved(tl::URI("file.txt")).to_string(),
            "/users/a%2Bb/file.txt");
  EXPECT_EQ(
      tl::URI("/users/a+b").resolved(tl::URI("file.txt")).to_abstract_path(),
      "/users/a+b/file.txt");
  EXPECT_EQ(
      tl::URI("file://users/a+b").resolved(tl::URI("file.txt")).to_string(),
      "file://users/a%2Bb/file.txt");
  EXPECT_EQ(tl::URI("file://users/a+b")
                .resolved(tl::URI("file.txt"))
                .to_abstract_path(),
            "file://users/a%2Bb/file.txt");
  //  drive-letter paths
  EXPECT_EQ(tl::URI("c:/users/a+b").resolved(tl::URI("file.txt")).to_string(),
            "c:/users/a%2Bb/file.txt");
  EXPECT_EQ(
      tl::URI("c:/users/a+b").resolved(tl::URI("file.txt")).to_abstract_path(),
      "c:/users/a+b/file.txt");
}
