...
TEST(1) {
  ...
  int count = 0;
  for (tl::Registrar<X>::iterator cls = tl::Registrar<X>::begin();
       cls != tl::Registrar<X>::end(); ++cls) {
    if (count == 0) {
...
TEST(2) {
  ...
  int count = 0;
  for (tl::Registrar<A>::iterator cls = tl::Registrar<A>::begin();
       cls != tl::Registrar<A>::end(); ++cls) {
    if (count == 0) {
