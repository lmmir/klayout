...
std::string to_string(const tl::reuse_vector<std::string> &x) {
  std::string r;
  size_t n = 0;
  for (tl::reuse_vector<std::string>::const_iterator s = x.begin();
       s != x.end(); ++s) {
    if (!r.empty()) {
      r += ",";
    }
...
test_insert(tl::TestBase *_this, tl::reuse_vector<std::string> &rv,
            const std::string &s) {
...
TEST(4) {
  ...
  int i = 0;
  for (tl::reuse_vector<A, true>::const_iterator n = va.begin(); n != va.end();
       ++n, ++i) {
    EXPECT_EQ(n->x, 100 - i * 10);
  }
  i = 0;
  for (tl::reuse_vector<B, false>::const_iterator n = vb.begin(); n != vb.end();
       ++n, ++i) {
    EXPECT_EQ(n->x, 100 - i * 10);
  }
