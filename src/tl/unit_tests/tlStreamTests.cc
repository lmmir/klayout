...
TEST(TextOutputStream) {
  ...
  try {
    ...
    {
      tl::rm_file(
          fn); //  avoids trouble with wrong path delimeters and backup files
...
TEST(DataInputStream) {
  tl::InputStream is("data:"
                     "SGVsbG8sIHdvcmxkIQpXaXRoIGFub3RoZXIgbGluZQoNDQpzZXBhcmF0Z"
                     "WQgYnkgYSBMRkNSIGFuZCBDUkxGLg==");
  tl::TextInputStream tis(is);
  EXPECT_EQ(tis.get_line(), "Hello, world!");
  EXPECT_EQ(tis.line_number(), size_t(1));
  EXPECT_EQ(tis.get_line(), "With another line");
...
class BrokenOutputStream : public tl::OutputFile {
public:
