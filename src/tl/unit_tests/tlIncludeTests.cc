...
TEST(1_simple) {
  ...
  EXPECT_EQ(tl::IncludeExpander::from_string(ie.to_string()).to_string(),
            ie.to_string());
...
TEST(2_single_include) {
  ...
  std::string et;
  tl::IncludeExpander ie =
      tl::IncludeExpander::expand(fn, tl::InputStream(fn).read_all(), et);
  EXPECT_EQ(et, "A line\nincluded.1\nAnother line\n");

  EXPECT_EQ(np(ie.to_string()),
            np("@1*" + tl::testdata() + "/tl/x_inc1.txt*0;2*" + tl::testdata() +
               "/tl/inc1.txt*-1;3*" + tl::testdata() + "/tl/x_inc1.txt*0;"));
  EXPECT_EQ(tl::IncludeExpander::from_string(ie.to_string()).to_string(),
            ie.to_string());

  EXPECT_EQ(ie.translate_to_original(1).first, fn);
  EXPECT_EQ(ie.translate_to_original(1).second, 1);
  EXPECT_EQ(np(ie.translate_to_original(2).first),
            np(tl::testdata() + "/tl/inc1.txt"));
...
TEST(3_multi_include) {
  ...
  EXPECT_EQ(ie.translate_to_original(1).second, 1);
  EXPECT_EQ(np(ie.translate_to_original(2).first),
            np(tl::testdata() + "/tl/inc3.txt"));
  EXPECT_EQ(ie.translate_to_original(2).second, 1);
  EXPECT_EQ(np(ie.translate_to_original(3).first),
            np(tl::testdata() + "/tl/inc2.txt"));
  EXPECT_EQ(ie.translate_to_original(3).second, 1);
  EXPECT_EQ(np(ie.translate_to_original(5).first),
            np(tl::testdata() + "/tl/inc3.txt"));
...
TEST(5_issue946) {
  ...
  EXPECT_EQ(np(ie.to_string()),
            np("@1*" + tl::testdata() + "/tl/x_inc4.txt*0;2*'" +
               tl::testdata() + "/tl/inc 4.txt'*-1;3*" + tl::testdata() +
               "/tl/x_inc4.txt*0;"));
  EXPECT_EQ(tl::IncludeExpander::from_string(ie.to_string()).to_string(),
            ie.to_string());
  ...
  //  no quotes here so this string can be used as the original file name if
  //  there is no include
