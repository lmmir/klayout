...
int main(int argc, char *argv[]) {
  ...
  const char *sz_name[] = {"Small",  "Medium",  "Large",
  ...
  for (int r = 1; r <= resolutions; ++r) {
    ...
    for (int s = 0; s < font_sizes; ++s) {
      ...
      char b[1024];
      sprintf(b,
              "  FixedFont (ff%d_height, ff%d_line_height, ff%d_width, "
              "ff%d_first_char, sizeof (ff%d_data) / sizeof (uint32_t) / "
      ...
      f.setStyleStrategy(QFont::StyleStrategy(
          (f.styleStrategy() & ~QFont::PreferAntialias) | QFont::NoAntialias));
