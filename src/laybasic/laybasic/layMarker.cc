...
void render_cell_inst(const db::Layout &layout, const db::CellInstArray &inst,
                      const db::CplxTrans &trans, lay::Renderer &r,
                      unsigned int font, lay::CanvasPlane *fill,
                      lay::CanvasPlane *contour, lay::CanvasPlane *vertex,
                      lay::CanvasPlane *text, bool cell_name_text_transform,
                      int min_size_for_label, bool draw_outline,
                      size_t max_shapes) {
  ...
  if (max_shapes > 0) {
    ...
    for (db::Layout::layer_iterator l = layout.begin_layers();
         l != layout.end_layers() && draw_shapes; ++l) {
  ...
  if (draw_outline || !draw_shapes) {
    ...
    if (n > 1000) {
      ...
      r.draw(cell_box, db::CplxTrans(trans * (av * long(amax - 1))) * tbox,
             fill, contour, 0, text);
      r.draw(cell_box, db::CplxTrans(trans * (bv * long(bmax - 1))) * tbox,
             fill, contour, 0, text);
      r.draw(
          cell_box,
          db::CplxTrans(trans * (av * long(amax - 1) + bv * long(bmax - 1))) *
              tbox,
          fill, contour, 0, text);
      ...
      db::DPoint points[] = {
          ...
          db::DPoint(cb.lower_left() +
                     trans * (av * long(amax - 1) + bv * long(bmax - 1))),
      ...
      if (text) {
        db::DBox arr_box(
            db::DPoint(),
            db::DPoint() + trans * (av * long(amax - 1) + bv * long(bmax - 1)));
        arr_box *= cb;
        r.draw(arr_box,
               tl::sprintf(tl::to_string(tr("Array %ldx%ld")), amax, bmax),
               db::Font(font), db::HAlignCenter, db::VAlignCenter,
               db::DFTrans(db::DFTrans::r0), 0, 0, 0, text);
      }

    } else {

      for (db::CellInstArray::iterator arr = inst.begin(); !arr.at_end();
           ++arr) {
        ...
        if (text && !cell_name.empty() && dbox.width() > min_size_for_label &&
            dbox.height() > min_size_for_label) {

          //  Hint: we render to contour because the texts plane is reserved for
          //  properties
          r.draw(dbox, cell_name, db::Font(font), db::HAlignCenter,
                 db::VAlignCenter,
                 //  TODO: apply "real" transformation?
                 db::DFTrans(cell_name_text_transform ? tbox.fp_trans().rot()
                                                      : db::DFTrans::r0),
                 0, 0, 0, text);
        }
      }
  ...
  if (draw_shapes) {
    ...
    for (db::Layout::layer_iterator l = layout.begin_layers();
      ...
      while (!shapes.at_end()) {
        ...
        for (db::CellInstArray::iterator arr = inst.begin(); !arr.at_end();
             ++arr) {
          r.draw(*shapes, trans * inst.complex_trans(*arr) * shapes.trans(),
                 fill, contour, 0 /*use vertex for origin*/, text);
        }
...

  {
    ...
    while (!shapes.at_end()) {
      ...
      for (db::CellInstArray::iterator arr = inst.begin(); !arr.at_end();
           ++arr) {
        r.draw(*shapes, trans * inst.complex_trans(*arr) * shapes.trans(), fill,
               contour, 0 /*use vertex for origin*/, text);
      }
...
MarkerBase::MarkerBase(lay::LayoutViewBase *view)
    : lay::ViewObject(view->canvas()), m_line_width(-1), m_vertex_size(-1),
      m_halo(-1), m_text_enabled(true), m_vertex_shape(lay::ViewOp::Rect),
      m_line_style(-1), m_dither_pattern(-1), m_frame_pattern(0),
      mp_view(view) {
  // .. nothing yet ..
}
...
void MarkerBase::get_bitmaps(const Viewport & /*vp*/, ViewObjectCanvas &canvas,
                             lay::CanvasPlane *&fill,
                             lay::CanvasPlane *&contour,
                             lay::CanvasPlane *&vertex,
                             lay::CanvasPlane *&text) {
  ...
  int line_width =
      m_line_width < 0 ? mp_view->default_marker_line_width() : m_line_width;
  int vertex_size =
      m_vertex_size < 0 ? mp_view->default_marker_vertex_size() : m_vertex_size;
  bool halo = m_halo < 0 ? mp_view->default_marker_halo() : (m_halo != 0);
  int dither_pattern = m_dither_pattern < 0 ? mp_view->default_dither_pattern()
                                            : m_dither_pattern;
  int line_style =
      m_line_style < 0 ? mp_view->default_line_style() : m_line_style;
  ...
  if (halo) {
    ...
    if (dither_pattern >= 0) {
      ops[0] = lay::ViewOp(canvas.background_color().rgb(), lay::ViewOp::Copy,
                           0, (unsigned int)dither_pattern, 0,
                           lay::ViewOp::Rect, 3 * basic_width, 0);
      ops[1] = lay::ViewOp(color.rgb(), lay::ViewOp::Copy, 0,
                           (unsigned int)dither_pattern, 0, lay::ViewOp::Rect,
                           basic_width, 1);
    ...
    ops[0] =
        lay::ViewOp(canvas.background_color().rgb(), lay::ViewOp::Copy,
                    (unsigned int)line_style, (unsigned int)m_frame_pattern, 0,
                    lay::ViewOp::Rect,
                    line_width > 0 ? (line_width + 2) * basic_width : 0, 0);
    ops[1] =
        lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy,
                    (unsigned int)line_style, (unsigned int)m_frame_pattern, 0,
                    lay::ViewOp::Rect, line_width * basic_width, 1);
    ...
    if (!m_text_enabled) {
      ...
      ops[0] = lay::ViewOp(canvas.background_color().rgb(), lay::ViewOp::Copy,
                           0, 0, 0, lay::ViewOp::Rect, 3 * basic_width, 0);
      ops[1] = lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                           lay::ViewOp::Rect, basic_width, 1);
      text = canvas.plane(ops);
    }

    if (m_vertex_shape == lay::ViewOp::Rect) {
      ops[0] =
          lay::ViewOp(canvas.background_color().rgb(), lay::ViewOp::Copy, 0, 0,
                      0, m_vertex_shape,
                      vertex_size > 0 ? (vertex_size + 2) * basic_width : 0, 0);
      ops[1] = lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                           m_vertex_shape, vertex_size * basic_width, 1);
      vertex = canvas.plane(ops);
    } else {
      std::vector<lay::ViewOp> ops1;
      ops1.resize(1);
      ops1[0] = lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                            m_vertex_shape, vertex_size * basic_width, 1);
    ...
    if (dither_pattern >= 0) {
      fill = canvas.plane(lay::ViewOp(color.rgb(), lay::ViewOp::Copy, 0,
                                      (unsigned int)dither_pattern, 0,
                                      lay::ViewOp::Rect, basic_width));
    } else {
      fill = 0;
    }

    contour = canvas.plane(
        lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy,
                    (unsigned int)line_style, (unsigned int)m_frame_pattern, 0,
                    lay::ViewOp::Rect, line_width * basic_width));
    vertex =
        canvas.plane(lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                                 m_vertex_shape, vertex_size * basic_width));
    if (!m_text_enabled) {
      text = 0;
    } else if (line_width == 1) {
      text = contour;
    } else {
      text = canvas.plane(lay::ViewOp(frame_color.rgb(), lay::ViewOp::Copy, 0,
                                      0, 0, lay::ViewOp::Rect, basic_width));
    }
  }
}
...
GenericMarkerBase::GenericMarkerBase(lay::LayoutViewBase *view,
                                     unsigned int cv_index)
    : MarkerBase(view), mp_trans_vector(0), mp_view(view),
      m_cv_index(cv_index) {
  // .. nothing yet ..
}
...
void GenericMarkerBase::set(const db::DCplxTrans &t1) {
  ...
  //  Note: this cast is not really correct but we handle float and integer
  //  types in the same fashion now.
...
db::DBox GenericMarkerBase::bbox() const {
  ...
  if (mp_trans_vector) {
    ...
    for (std::vector<db::DCplxTrans>::const_iterator t =
             mp_trans_vector->begin();
         t != mp_trans_vector->end(); ++t) {
...
const db::Layout *GenericMarkerBase::layout() const {
...
double GenericMarkerBase::dbu() const {
...
InstanceMarker::InstanceMarker(LayoutViewBase *view, unsigned int cv_index,
                               bool draw_outline, size_t max_shapes)
    : GenericMarkerBase(view, cv_index), m_draw_outline(draw_outline),
      m_max_shapes(max_shapes), m_inst() {
  // .. nothing yet ..
}
...
void InstanceMarker::set(const db::Instance &instance,
                         const db::ICplxTrans &trans) {
  m_inst = instance;
  GenericMarkerBase::set(trans);
}

void InstanceMarker::set(const db::Instance &instance, const db::ICplxTrans &t1,
                         const std::vector<db::DCplxTrans> &trans) {
  m_inst = instance;
...
void InstanceMarker::render(const Viewport &vp, ViewObjectCanvas &canvas) {
  ...
  if (!trans_vector()) {
    render_cell_inst(*ly, m_inst.cell_inst(), vp.trans() * trans(), r,
                     view()->cell_box_text_font(), fill, contour, vertex, text,
                     label_transform, min_size, m_draw_outline, m_max_shapes);
  } else {
    for (std::vector<db::DCplxTrans>::const_iterator tr =
             trans_vector()->begin();
         tr != trans_vector()->end(); ++tr) {
      render_cell_inst(*ly, m_inst.cell_inst(), vp.trans() * *tr * trans(), r,
                       view()->cell_box_text_font(), fill, contour, vertex,
                       text, label_transform, min_size, m_draw_outline,
                       m_max_shapes);
    }
  }
}
...
db::DBox InstanceMarker::item_bbox() const { return db::DBox(m_inst.bbox()); }
...
void ShapeMarker::set(const db::Shape &shape, const db::ICplxTrans &t1,
                      const std::vector<db::DCplxTrans> &trans) {
  m_shape = shape;
  GenericMarkerBase::set(t1, trans);
}
  ...
  if (trans_vector()) {
    for (std::vector<db::DCplxTrans>::const_iterator tr =
      ...
      if (m_shape.is_text() && text) {
        ...
        db::Text t;
        m_shape.text(t);
        db::DBox box = ti.bbox(trans() * t, vp_trans)
                           .enlarged(text_box_enlargement(vp_trans));
        if (!box.is_point()) {
    ...
    if (m_shape.is_text() && text) {
      ...
      db::Text t;
      m_shape.text(t);
      db::DBox box = ti.bbox(trans() * t, vp.trans())
                         .enlarged(text_box_enlargement(vp.trans()));
      if (!box.is_point()) {
...
db::DBox ShapeMarker::item_bbox() const { return db::DBox(m_shape.bbox()); }

// ------------------------------------------------------------------------

Marker::Marker(lay::LayoutViewBase *view, unsigned int cv_index,
               bool draw_outline, size_t max_shapes)
    : GenericMarkerBase(view, cv_index), m_draw_outline(draw_outline),
...
void Marker::set(const db::DBox &box, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::Box &box, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::Polygon &poly, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::PolygonRef &poly, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::DPolygon &poly, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::EdgePair &edge_pair, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::DEdgePair &edge_pair, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::Edge &edge, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::DEdge &edge, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::Path &path, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::DPath &path, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::Text &text, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::DText &text, const db::DCplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
void Marker::set(const db::CellInstArray &instance,
                 const db::ICplxTrans &trans) {
  remove_object();
...
void Marker::set(const db::CellInstArray &instance, const db::ICplxTrans &t1,
                 const std::vector<db::DCplxTrans> &trans) {
  remove_object();
...
db::DBox Marker::item_bbox() const {
  if (m_type == Box) {
...
void Marker::draw(lay::Renderer &r, const db::CplxTrans &t,
                  lay::CanvasPlane *fill, lay::CanvasPlane *contour,
  ...
  if (m_type == Box) {
    ...
    r.draw(m_object.polygon_ref->obj(),
           t * db::ICplxTrans(m_object.polygon_ref->trans()), fill, contour,
           vertex, text);
  } else if (m_type == DPolygon) {
    ...
    //  TODO: in order to draw the box we'd need a separation of dbu-to-micron
    //  and micron-to-pixel transformations ...
    ...
    if (view() && text) {
      ...
      db::DCplxTrans dt(t);
      db::DBox box =
          ti.bbox(*m_object.dtext, dt).enlarged(text_box_enlargement(dt));
      if (!box.is_point()) {
    ...
    r.draw(m_object.dedge_pair->first(), db::DCplxTrans(t), fill, contour,
           vertex, text);
    r.draw(m_object.dedge_pair->second(), db::DCplxTrans(t), fill, contour,
           vertex, text);
    ...
    bool label_transform = view()->cell_box_text_transform();
    int min_size = view()->min_inst_label_size();
    render_cell_inst(cv->layout(), *m_object.inst, t, r,
                     view()->cell_box_text_font(), fill, contour, vertex, text,
                     label_transform, min_size, m_draw_outline, m_max_shapes);
  }
}
...
DMarker::DMarker(LayoutViewBase *view) : MarkerBase(view), mp_view(view) {
  m_type = None;
  m_object.any = 0;
}

DMarker::~DMarker() { remove_object(); }
...
void DMarker::render(const Viewport &vp, ViewObjectCanvas &canvas) {
  ...
  if (m_type == Box) {
    ...
    if (view() && text) {
      ...
      lay::TextInfo ti(view());
      db::DBox box =
          ti.bbox(*m_object.text, t).enlarged(text_box_enlargement(t));
      if (!box.is_point()) {
