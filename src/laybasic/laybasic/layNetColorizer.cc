...
void NetColorizer::configure(const tl::Color &marker_color,
...
bool NetColorizer::has_color_for_net(const db::Net *net) {
  return net != 0 && (m_auto_colors_enabled ||
                      m_custom_color.find(net) != m_custom_color.end());
}

void NetColorizer::set_color_of_net(const db::Net *net,
...
tl::Color NetColorizer::color_of_net(const db::Net *net) const {
  if (!net) {
    return tl::Color();
  }

  std::map<const db::Net *, tl::Color>::const_iterator c =
      m_custom_color.find(net);
  ...
  if (m_auto_colors_enabled) {
    ...
    std::map<const db::Net *, size_t>::iterator cc =
        m_net_index_by_object.find(net);
    if (cc == m_net_index_by_object.end()) {

      size_t i = 0;
      for (db::Circuit::const_net_iterator n = circuit->begin_nets();
           n != circuit->end_nets(); ++n, ++i) {
