...
/**
 *  @brief A utility function to convert the packed key binding in the
 * cfg_key_bindings string to a vector
 */
LAYBASIC_PUBLIC std::vector<std::pair<std::string, std::string>>
unpack_key_binding(const std::string &packed);

/**
 *  @brief A utility function to convert the key binding (as path/shortcut pair
 * vector) to a packed string for cfg_key_bindings
 */
LAYBASIC_PUBLIC std::string pack_key_binding(
    const std::vector<std::pair<std::string, std::string>> &unpacked);

/**
 *  @brief A utility function to convert the packed hidden flags in the
 * cfg_menu_items_hidden string to a vector
 */
LAYBASIC_PUBLIC std::vector<std::pair<std::string, bool>>
unpack_menu_items_hidden(const std::string &packed);

/**
 *  @brief A utility function to convert the hidden flags (as path/bool pair
 * vector) to a packed string for cfg_menu_items_hidden
 */
LAYBASIC_PUBLIC std::string pack_menu_items_hidden(
    const std::vector<std::pair<std::string, bool>> &unpacked);

/**
 *  @brief The basic Action object
 *
 *  An "action" is the target of a menu action. It allows reimplementing the
 * "triggered" method to implement a specific action. The action encapsulates a
 * QAction object.
 *
 *  Use this object in "insert_item" of AbstractMenu.
 *
 *  This object is typically owned by the AbstractMenu and cannot be copied or
 * assigned.
 */
...
public:
   ...
   *
   *  @param title The title string encoding icon and keyboard shortcut if
   * applicable.
   */
   ...
   *  If the shortcut string is "no_shortcut()", no shortcut will be assigned to
   * the item.
   */
   ...
   *  To get the effective shortcut (combination of default shortcut and
   * shortcut), use "get_effective_shortcut".
   */
   ...
   *  if hidden is true. This feature allows implementation of the menu
   * configuration feature where users can deliberately switch off and on menu
   * items.
   */
   ...
   */
  void add_to_exclusive_group(lay::AbstractMenu *menu,
  ...
  /**
   *  @brief Reimplement this method for a handler called before a sub-menu is
   * opening
   */
   ...
   */
  virtual bool is_for_mode(int /*mode_id*/) const { return false; }

  /**
   *  @brief Gets a value indicating the action is visible (dynamic calls)
   *  In addition to static visibility (visible/hidden), an Action object can
   * request to become invisible dynamically based on conditions. This will work
   * for menu-items for which the system will query the status before the menu
   * is shown.
   */
  virtual bool wants_visible() const { return true; }
   ...
   * is shown.
   */
  virtual bool wants_enabled() const { return true; }
   ...
   */
  Dispatcher *dispatcher() const { return mp_dispatcher; }
  ...
  /**
   *  @brief This event gets called when the action is a sub-menu and the menu
   * is opened
   */
...
/**
 *  @brief A specialization for the Action to issue a "configure" request on
 * "triggered"
 *
 *  When this action is triggered, a "configure" request is issued to the
 * Dispatcher instance (which is the root of the configuration hierarchy). The
 * name and value is given by the respective parameters passed to the
 * constructor or set with the write accessors.
 */
class LAYBASIC_PUBLIC ConfigureAction : public Action {
public:
  ...
  /**
   *  @brief Create an configure action with the given title (icon, keyboard
   * shortcut), name and value
   *
   *  The format of the title string is:
   * <text>["("shortcut")"]["<"icon-resource">"]
   *
   ...
   *  a boolean parameter which is mapped to a checkable action.
   */
  ConfigureAction(const std::string &title, const std::string &cname,
   ...
   */
  void set_cname(const std::string &cname) { m_cname = cname; }

  /**
   *  @brief Configuration parameter name getter
   */
  const std::string &get_cname() const { return m_cname; }

  /**
   *  @brief Configuration parameter value setter
   */
  void set_cvalue(const std::string &cvalue) { m_cvalue = cvalue; }

  /**
   *  @brief Configuration parameter value getter
   */
  const std::string &get_cvalue() const { return m_cvalue; }

  /**
   *  @brief Transfer the value into this action
   *
   *  This method is called by the main window when the respective configuration
   * changes.
   */
...
struct LAYBASIC_PUBLIC AbstractMenuItem {
  ...
  //  No copy constructor semantics because we don't need it (we use list's) and
  //  Action does not provide one.
  ...
  Action *action() { return mp_action.get(); }

  const Action *action() const { return mp_action.get(); }

#if defined(HAVE_QT)
  QMenu *menu() { return mp_action->menu(); }
  ...
  bool has_submenu() const { return m_has_submenu; }

  void set_remove_on_empty();

  bool remove_on_empty() const { return m_remove_on_empty; }
 ...
 *  in a generic form such that they can be manipulated and converted into
 * QMenu's or QMenuBar setups.
 *
 *  Each item can be associated with a Action, which delivers a title,
 * enabled/disable state etc. The Action is either provided when new entries are
 * inserted or created upon initialisation.
 *
 *  The abstract menu class provides methods to manipulate the menu structure
 * (the state of the menu items, their title and shortcut key is provided and
 * manipulated through the Action object).
 *
 *  Menu items and submenus are referred to by a "path". The path is a string
 * with this interpretation:
 *
 *    ""                 is the root
 *    "[<path>.]<name>"  is an element of the submenu given by <path>. If <path>
 * is omitted, this refers to an element in the root
 *    "[<path>.]end      refers to the item past the last item of the submenu
 * given by <path> or root
 *    "[<path>.]begin    refers to the first item of the submenu given by <path>
 * or root
 *    "[<path>.]#<n>     refers to the nth item of the submenu given by <path>
 * or root (n is an integer number)
 *
 *  The abstract menu provides methods to obtain QMenu and Action objects
 * associated with a certain item by specifying a path.
 *
 *  Menu items can be put into groups. The path strings of each group can be
 * obtained with the "group" method. An item is put into a group by appending
 * ":<group-name>" to the item's name. This specification can be used several
 * times.
 *
 *  Detached menus (i.e. for use in context menus) can be created as virtual
 * top-level submenus with a name of the form "@<name>". Such menu's are created
 * as detached QMenu object's and not inserted into the main menu. A special
 * detached menu is "@toolbar" which describes all elements placed into the
 * toolbar passed to build (). Detached menu objects can be retrieved with the
 *  detached_menu () method, passing the desired name (without the "@").
 *  A detached (separate menu bar) can be created as well from detached menu
 * bars. Such menu bars are specified by submenus starting with "@@". Detached
 * menu bars can be filled using the build_detached () method.
 *
 *  The abstract menu class issues a signal ("changed") if the structure has
 * changed. To recreate the QMenu objects and fill the QMenuBar, the "build"
 * method needs to be called. This will delete all QMenu's created so far
 * (except the detached menus), clear the QMenuBar, recreate the QMenu objects
 * (note, that the addresses will change this way!) and refill the QToolBar and
 * QMenuBar.
 */
...
public:
   ...
   *  @return A reference to a QMenu object or 0 if the path is not valid or
   * does not refer to a submenu
   */
   ...
   *  with a "@.." top level entry. In any case, a valid QMenu object is
   * returned which never changes, even if the menu hierarchy is rebuilt. The
   * QMenu returned may be empty.
   *
   *  @param name The name of the detached menu, without the "@"
   ...
   *
   *  The Action object returned is basically a pointer to the underlying
   * QAction object, which is potentially shared between multiple menu items.
   *
   ...
   *
   *  @param path The path to the submenu
   *  @return A vector or path strings for the child items or an empty vector if
   * the path is not valid or the item does not have children
   */
   ...
   *
   *  The action passed to this method is copied as the item's action. The item
   * will refer to this action and issue triggered events on this.
   *
   ...
   */
  void insert_item(const std::string &path, const std::string &name,
                   Action *action);
   ...
   */
  void insert_menu(const std::string &path, const std::string &name,
                   Action *action);
  ...
  configure_actions(const std::string &name);

  /**
   *  @brief Gets the keyboard shortcuts
   *  @param with_defaults Returns the default shortcuts if true. Otherwise
   ...
   */
  const AbstractMenuItem &root() const { return m_root; }
...
private:
  ...
  find_item_for_action(const Action *action,
                       const AbstractMenuItem *from = 0) const;
  AbstractMenuItem *find_item_for_action(const Action *action,
                                         AbstractMenuItem *from = 0);
#if defined(HAVE_QT)
  void build(QMenu *menu, std::list<AbstractMenuItem> &items);
  void build(QToolBar *tbar, std::list<AbstractMenuItem> &items);
#endif
  void collect_group(std::vector<std::string> &grp, const std::string &name,
                     const AbstractMenuItem &item) const;
  void collect_configure_actions(std::vector<ConfigureAction *> &ca,
                                 AbstractMenuItem &item);
  void emit_changed();
  void get_shortcuts(const std::string &root,
