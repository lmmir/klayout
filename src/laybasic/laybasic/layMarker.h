...
class LAYBASIC_PUBLIC MarkerBase : public lay::ViewObject {
public:
  /**
   ...
   *
   *  If the color is invalid, the marker is drawn with the canvases foreground
   * color.
   */
  tl::Color get_color() const { return m_color; }
   ...
   */
  tl::Color get_frame_color() const { return m_frame_color; }
   ...
   */
  int get_vertex_size() const { return m_vertex_size; }
   ...
   */
  lay::ViewOp::Shape get_vertex_shape() const { return m_vertex_shape; }
   ...
   */
  int get_frame_pattern() const { return m_frame_pattern; }
   ...
   */
  int get_dither_pattern() const { return m_dither_pattern; }
   ...
   */
  int get_line_style() const { return m_line_style; }
   ...
   */
  int get_halo() const { return m_halo; }

  /**
   *  @brief Set the halo flag
   *
   *  If the halo is enabled (1), a 1 pixel halo is drawn around that marker
   * this giving it a better visibility in front of complex drawing. If disabled
   * (0), no halo is drawn. A negative value (-1) instructs the marker to use
   * the global setting.
   */
   ...
   *
   *  If this value is false, texts are never drawn. If true (the default),
   * texts are drawn as usual. This is specifically useful for disabling cell
   * boxes of instances.
   */
  bool is_text_enabled() const { return m_text_enabled; }
...
protected:
  void get_bitmaps(const Viewport &vp, ViewObjectCanvas &canvas,
 ...
 *  This object is the base for specialized markers, i.e. instance and shape
 * markers. The basic functionality is to keep a set of transformations. certain
 * instance object in the layout. It is given an instance reference thus
 * pointing to a instance rather than keeping a copy of it.
 */

class LAYBASIC_PUBLIC GenericMarkerBase : public MarkerBase {
public:
  /**
   *  @brief The constructor
   */
   ...
   *  The transformations from the trans vector are applied in micron unit space
   * after the global transformation (t1) and after application of the database
   * unit. t1 is given in database unit space.
   */
   ...
   *  This will be the first part of the transformation including the DBU
   * scaling.
   */
  const db::CplxTrans &trans() const { return m_trans; }
...
class LAYBASIC_PUBLIC ShapeMarker : public GenericMarkerBase {
public:
  /**
   *  @brief The constructor
   */
   ...
   *  must be stored somewhere else.
   */
  void set(const db::Shape &shape, const db::ICplxTrans &t1,
...
class LAYBASIC_PUBLIC InstanceMarker : public GenericMarkerBase {
public:
  /**
   *  @brief The constructor
   *
   *  @param view The view the marker is intended for
   *  @param cv_index The cell view index of the layout that this marker is
   * intended for
   *  @param draw_outline True to have instances drawing their outline
   *  @param max_shapes The maximum number of shapes to draw for instances (just
   * a box is drawn if more shapes are present)
   */
  InstanceMarker(lay::LayoutViewBase *view, unsigned int cv_index,
                 bool draw_outline = true, size_t max_shapes = 0);
   ...
   *  must be stored somewhere else.
   */
  void set(const db::Instance &inst, const db::ICplxTrans &t1,
           const std::vector<db::DCplxTrans> &trans);
   ...
   */
  bool draw_outline() const { return m_draw_outline; }
   ...
   */
  size_t max_shapes() const { return m_max_shapes; }
...
class LAYBASIC_PUBLIC Marker : public GenericMarkerBase {
public:
  /**
   *  The constructor
   *
   ...
   * a box is drawn if more shapes are present)
   */
  Marker(lay::LayoutViewBase *view, unsigned int cv_index,
         bool draw_outline = true, size_t max_shapes = 0);
...
private:
  ...
  void draw(lay::Renderer &r, const db::CplxTrans &t, lay::CanvasPlane *fill,
 ...
 *  object accepts objects with coordinates in floating-point coordinates in
 * micron values. It does not need a DBU value hence no cellview to display
 * itself.
 */

class LAYBASIC_PUBLIC DMarker : public MarkerBase {
public:
  /**
   *  @brief The constructor
   */
...
private:
  ...
  enum { None, Box, Polygon, EdgePair, Edge, Path, Text } m_type;
