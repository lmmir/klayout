...
static void make_circle(double r, const db::DPoint &center, db::DPolygon &poly,
                        bool as_hole) {
...
class TrackingCursorBase : public lay::ViewObject {
public:
...
protected:
  virtual void do_render(const lay::Viewport &vp,
                         lay::ViewObjectCanvas &canvas) = 0;

private:
  lay::EditorServiceBase *mp_service;
};

class MouseCursorViewObject : public TrackingCursorBase {
public:
  MouseCursorViewObject(lay::EditorServiceBase *service,
                        lay::ViewObjectUI *widget, const db::DPoint &pt,
                        bool solid)
...
protected:
  virtual void do_render(const lay::Viewport &vp,
    ...
    ops[0] =
        lay::ViewOp(cursor_color(canvas), lay::ViewOp::Copy, 0,
                    (unsigned int)dither_pattern, 0, lay::ViewOp::Rect, lw, 0);
    lay::CanvasPlane *plane = canvas.plane(ops);

    ops[0] = lay::ViewOp(cursor_color(canvas), lay::ViewOp::Copy, 0,
                         (unsigned int)cross_dither_pattern, 0,
                         lay::ViewOp::Rect, lw, 0);
    ...
    r.draw(db::DEdge(m_pt + db::DVector(0, -rad * 4),
                     m_pt + db::DVector(0, rad * 4)),
           vp.trans(), 0, cross_plane, 0, 0);
    r.draw(db::DEdge(m_pt + db::DVector(-rad * 4, 0),
                     m_pt + db::DVector(rad * 4, 0)),
           vp.trans(), 0, cross_plane, 0, 0);
  }
...
class EdgeMarkerViewObject : public TrackingCursorBase {
public:
  EdgeMarkerViewObject(lay::EditorServiceBase *service,
                       lay::ViewObjectUI *widget, const db::DEdge &edge,
                       bool solid)
...
protected:
  virtual void do_render(const lay::Viewport &vp,
    ...
    std::vector<lay::ViewOp> ops;
    ops.resize(1);
    ops[0] = lay::ViewOp(cursor_color(canvas), lay::ViewOp::Copy, solid_style,
                         0, 0, lay::ViewOp::Rect, lw, 0);
    lay::CanvasPlane *arrow_plane = canvas.plane(ops);

    ops[0] = lay::ViewOp(cursor_color(canvas), lay::ViewOp::Copy,
                         m_solid ? solid_style : dashed_style, 0, 0,
                         lay::ViewOp::Rect, lw, 0);
    ...
    db::DVector n = db::DVector(m_edge.dy(), -m_edge.dx()) *
                    (arrow_width_half / m_edge.length());
    db::DVector d = db::DVector(m_edge.dx(), m_edge.dy()) *
                    (arrow_length / m_edge.length());

    if (m_edge.length() < 2 * arrow_length) {

      r.draw(db::DEdge(m_edge.p1() - n, m_edge.p1() + n), vp.trans(), 0,
             arrow_plane, 0, 0);
      r.draw(db::DEdge(m_edge.p2() - n, m_edge.p2() + n), vp.trans(), 0,
             arrow_plane, 0, 0);
...
EditorServiceBase::EditorServiceBase(LayoutViewBase *view)
    : lay::ViewService(view->canvas()), lay::Editable(view), lay::Plugin(view),
      mp_view(view), m_cursor_enabled(true), m_has_tracking_position(false) {
  //  .. nothing yet ..
}

EditorServiceBase::~EditorServiceBase() { clear_mouse_cursors(); }
...
void EditorServiceBase::add_mouse_cursor(const db::DPoint &pt, bool emphasize) {
  ...
  m_mouse_cursor_markers.push_back(
      new MouseCursorViewObject(this, ui(), pt, emphasize));
}

void EditorServiceBase::add_mouse_cursor(const db::Point &pt,
...
void EditorServiceBase::add_edge_marker(const db::DEdge &e, bool emphasize) {
  m_mouse_cursor_markers.push_back(
      new EdgeMarkerViewObject(this, ui(), e, emphasize));
}

void EditorServiceBase::add_edge_marker(const db::Edge &e,
                                        unsigned int cv_index,
                                        const db::ICplxTrans &gt,
                                        const std::vector<db::DCplxTrans> &tv,
                                        bool emphasize) {
...
void EditorServiceBase::clear_mouse_cursors() {
  ...
  for (std::vector<lay::ViewObject *>::iterator r =
           m_mouse_cursor_markers.begin();
       r != m_mouse_cursor_markers.end(); ++r) {
    delete *r;
  }
  m_mouse_cursor_markers.clear();
}

void EditorServiceBase::mouse_cursor_from_snap_details(
    const lay::PointSnapToObjectResult &snap_details) {
  clear_mouse_cursors();

  add_mouse_cursor(snap_details.snapped_point,
                   snap_details.object_snap ==
                           lay::PointSnapToObjectResult::ObjectVertex ||
                       (snap_details.object_snap ==
                            lay::PointSnapToObjectResult::ObjectUnspecific &&
                        snap_details.object_ref.is_degenerate()));
...
bool EditorServiceBase::configure(const std::string &name,
...
void EditorServiceBase::deactivated() { clear_mouse_cursors(); }
