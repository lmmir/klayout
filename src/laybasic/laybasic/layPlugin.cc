...
std::vector<std::string> PluginDeclaration::menu_symbols() {
  ...
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
    ...
    for (std::vector<lay::MenuEntry>::const_iterator m = menu_entries.begin();
...
class GenericMenuAction : public Action {
public:
  GenericMenuAction(Dispatcher *dispatcher, const std::string &title,
...
public:
  ...
  bool is_for_mode(int mode_id) const { return mode_id == m_mode; }
...
void PluginDeclaration::init_menu(lay::Dispatcher *dispatcher) {
  ...
  //  make all plugins that return true on "implements_editable" into menu
  //  entries and set up the actions accordingly

  if (implements_editable(title) && menu.is_valid("edit_menu.select_menu")) {

    //  extract first part, which is the name, separated by a tab from the
    //  title.
...
#if defined(HAVE_QT)
    gtf::action_connect(mp_editable_mode_action->qaction(), SIGNAL(triggered()),
                        this, SLOT(toggle_editable_enabled()));
#endif
    mp_editable_mode_action->set_checkable(true);
    mp_editable_mode_action->set_checked(m_editable_enabled);

    menu.insert_item("edit_menu.select_menu.end", name,
                     mp_editable_mode_action.get());
  }
  ...
  if (implements_mouse_mode(title)) {
    modes.push_back(std::make_pair(
        title,
        std::make_pair("edit_menu.mode_menu.end;@toolbar.end_modes", id())));
  }

  //  the primary mouse modes (special for LayoutView)
  implements_primary_mouse_modes(modes);

  for (std::vector<std::pair<std::string, std::pair<std::string, int>>>::
           const_iterator m = modes.begin();
       m != modes.end(); ++m) {
    ...
    mp_mouse_mode_action.reset(
        new ModeAction(dispatcher, title, m->second.second));
    mp_mouse_mode_action->add_to_exclusive_group(&menu,
                                                 "mouse_mode_exclusive_group");
    mp_mouse_mode_action->set_checkable(true);

    menu.insert_item(m->second.first, name + ":mode_group",
                     mp_mouse_mode_action.get());
  }
}
...
void PluginDeclaration::remove_menu_items(Dispatcher *dispatcher) {
  ...
  for (tl::weak_collection<lay::Action>::iterator a = m_menu_actions.begin();
       a != m_menu_actions.end(); ++a) {
    if (a.operator->()) {
...
Plugin::Plugin(Plugin *parent, bool standalone)
    : mp_parent(parent), mp_plugin_declaration(0),
      dm_finalize_config(this, &lay::Plugin::config_end),
...
void Plugin::config_set(const std::string &name, const std::string &value) {
  ...
  //  look for plugins that receive that configuration statically if the root is
  //  addressed
  if (!mp_parent && !m_standalone) {
    for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
         ...
         cls != tl::Registrar<lay::PluginDeclaration>::end(); ++cls) {
      try {
        if ((const_cast<lay::PluginDeclaration *>(&*cls))
                ->configure(name, value)) {
          return;
        }
      } catch (tl::Exception &ex) {
        tl::error << tl::to_string(tr("Error on configure")) << " " << name
                  << "='" << value << "': " << ex.msg();
      }
    }
  }
...
void Plugin::do_config_setup(Plugin *target) {
  ...
  //  local configurations override the parent's configuration, i.e. are applied
  //  after the parents settings
  for (std::map<std::string, std::string>::const_iterator p =
           m_repository.begin();
       p != m_repository.end(); ++p) {
...
void Plugin::do_config_end() {
  config_finalize();
  for (tl::weak_collection<Plugin>::iterator c = m_children.begin();
       c != m_children.end(); ++c) {
    c->do_config_end();
  }
}

bool Plugin::do_config_set(const std::string &name, const std::string &value,
                           bool for_child) {
  if (for_child) {
    //  this is the case when we impose a configuration from the parent: in this
    //  case we have to remove it from the repository of local parameters.
...
MenuEntry menu_item_copy(const std::string &symbol,
...
MenuEntry submenu(const std::string &menu_name, const std::string &insert_pos,
...
MenuEntry submenu(const std::string &symbol, const std::string &menu_name,
                  const std::string &insert_pos, const std::string &title) {
  MenuEntry e;
  e.symbol = symbol;
  e.menu_name = menu_name;
...
MenuEntry config_menu_item(const std::string &menu_name,
                           const std::string &insert_pos,
