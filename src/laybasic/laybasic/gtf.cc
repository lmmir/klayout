...
tl::Variant image_to_variant(const QImage &img) {
...
static std::string escape_string(const char *cp) {
  std::string r;
...
inline bool is_widget(QObject *o) {
  return (dynamic_cast<const QDialog *>(o) != 0 ||
          dynamic_cast<const QMainWindow *>(o) != 0 ||
          dynamic_cast<const QWidget *>(o) != 0);
}
...
static bool extract_widget_path(tl::Extractor &x, std::string &name,
                                std::string &cls, int &nwidget) {
  name.clear();
  cls.clear();
  ...
  if (!x.test(".")) {
    if (!x.at_end()) {
      throw tl::Exception(
          tl::to_string(QObject::tr("Invalid widget path: expected '.'")));
    }
    return false;
  } else {
    return true;
...
static QWidget *widget_from_path(const char *p, int xml_line) {
  tl::Extractor x(p);
  ...
  do {
    ...
    if (w == 0) {
      ...
      for (QWidgetList::const_iterator tl = tl_widgets.begin();
           tl != tl_widgets.end(); ++tl) {
        if (is_widget(*tl) && (*tl)->objectName() == tl::to_qstring(name) &&
            (cls.empty() || cls == (*tl)->metaObject()->className()) &&
            --n == 0) {
          w = *tl;
          break;
        }
      ...
      for (QObjectList::const_iterator child = children.begin();
           child != children.end(); ++child) {
        if (is_widget(*child) &&
            (*child)->objectName() == tl::to_qstring(name) &&
            (cls.empty() || cls == (*child)->metaObject()->className()) &&
            --n == 0) {
          w = *child;
          break;
        }
    ...
    if (w == 0) {
      ...
      throw tl::Exception(
          tl::to_string(QObject::tr(
              "Widget path resolution failed: '%s' is not a valid component "
              "with index %d in path '%s' (line %d)\nAlternatives are: %s")),
          name, nwidget, p, xml_line, names);
    }
  ...
  if (!target_widget) {
    throw tl::Exception(
        tl::to_string(QObject::tr(
            "Invalid widget path '%s': does not lead to a widget (line %d)")),
        p, xml_line);
  }
  return target_widget;
}

static std::string widget_to_path(QWidget *w, const char *pf = 0) {
  ...
  if (pw) {
    ...
    for (QObjectList::const_iterator child = children.begin();
         child != children.end(); ++child) {
      if (dynamic_cast<QDialog *>(*child) != 0 ||
          dynamic_cast<QMainWindow *>(*child) != 0 ||
          dynamic_cast<QWidget *>(*child) != 0) {
        if (*child == w) {
          break;
        }
        if ((*child)->objectName() == tl::to_qstring(n) &&
    ...
    for (QWidgetList::const_iterator itl = tlw.begin(); itl != tlw.end();
         ++itl) {
      //  only QDialog or QMainWindow ancestors count as valid top level widgets
      if (dynamic_cast<QDialog *>(*itl) != 0 ||
          dynamic_cast<QMainWindow *>(*itl) != 0 ||
          dynamic_cast<QWidget *>(*itl) != 0) {
        if (*itl == w) {
          break;
        }
        if ((*itl)->objectName() == tl::to_qstring(n) &&
            cls == (*itl)->metaObject()->className()) {
          ++i;
        }
      }
    }
...
MouseTrackerWidget::MouseTrackerWidget(QWidget *parent)
    : QWidget(parent, Qt::Window | Qt::X11BypassWindowManagerHint |
...
MouseTrackerWidget *MouseTrackerWidget::instance() {
...
void MouseTrackerWidget::set(const QMouseEvent &me) {
  mp_current_pixmap = &m_basic_pm;
...
void MouseTrackerWidget::paintEvent(QPaintEvent * /*pe*/) {
...
class GtfXmlHandler : public QXmlDefaultHandler {
public:
...
static void write_data(const tl::Variant &data, std::ostream &os,
                       unsigned int level) {
  if (data.is_list()) {
...
void LogEventBase::write(std::ostream &os, bool with_endl) const {
  std::vector<std::pair<std::string, std::string>> attrs;
  attributes(attrs);

  os << "  <" << name();
  for (std::vector<std::pair<std::string, std::string>>::const_iterator a =
           attrs.begin();
       a != attrs.end(); ++a) {
    os << " " << a->first << "=\"" << a->second << "\"";
  }
  ...
  if (!data().is_nil() && !(data().is_list() && data().get_list().empty())) {
    os << ">" << std::endl;
    if (data().is_list()) {
      for (tl::Variant::const_iterator b = data().begin(); b != data().end();
           ++b) {
        write_data(*b, os, 2);
      }
    } else {
...
class LogTargetedEvent : public LogEventBase {
public:
  ...
  virtual void
  attributes(std::vector<std::pair<std::string, std::string>> &attr) const {
...
class LogMouseEvent : public LogTargetedEvent {
public:
  LogMouseEvent(const std::string &target, const QMouseEvent &me,
                int xml_line = 0)
      : LogTargetedEvent(target, xml_line),
#if QT_VERSION >= 0x60000
        m_mouse_event(new QMouseEvent(me.type(), me.position(),
                                      me.globalPosition(), me.button(),
                                      me.buttons(), me.modifiers()))
#else
        m_mouse_event(new QMouseEvent(me.type(), me.pos(), me.button(),
                                      me.buttons(), me.modifiers()))
#endif
  {
    //  .. nothing yet ..
  ...
  virtual void issue_event() {
    ...
    QMouseEvent me(m_mouse_event->type(), m_mouse_event->pos(),
                   m_mouse_event->globalPos(), m_mouse_event->button(),
                   m_mouse_event->buttons(), m_mouse_event->modifiers());
  ...
  attributes(std::vector<std::pair<std::string, std::string>> &attr) const {
    ...
    attr.push_back(
        std::make_pair(std::string("xpos"), tl::to_string(m_mouse_event->x())));
    attr.push_back(
        std::make_pair(std::string("ypos"), tl::to_string(m_mouse_event->y())));
    if (m_mouse_event->type() == QEvent::MouseMove) {
      attr.push_back(
          std::make_pair(std::string("buttons"),
                         tl::sprintf("%x", int(m_mouse_event->buttons()))));
    } else {
      attr.push_back(
          std::make_pair(std::string("button"),
                         tl::sprintf("%x", int(m_mouse_event->button()))));
    }
    attr.push_back(
        std::make_pair(std::string("modifiers"),
                       tl::sprintf("%x", int(m_mouse_event->modifiers()))));
  }

  const QMouseEvent &event() const { return *m_mouse_event; }

  void move(const QPoint &p) {
    m_mouse_event.reset(
        new QMouseEvent(m_mouse_event->type(), m_mouse_event->pos() + p,
                        m_mouse_event->globalPos() + p, m_mouse_event->button(),
                        m_mouse_event->buttons(), m_mouse_event->modifiers()));
  }
...
class LogKeyEvent : public LogTargetedEvent {
public:
  LogKeyEvent(const std::string &target, const QKeyEvent &ke, int xml_line = 0)
      : LogTargetedEvent(target, xml_line),
        m_key_event(new QKeyEvent(ke.type(), ke.key(), ke.modifiers())) {
    //  .. nothing yet ..
  }

  virtual void issue_event() {
    std::unique_ptr<QKeyEvent> ke(new QKeyEvent(
        m_key_event->type(), m_key_event->key(), m_key_event->modifiers()));
  ...
  attributes(std::vector<std::pair<std::string, std::string>> &attr) const {
    ...
    attr.push_back(std::make_pair(std::string("key"),
                                  tl::sprintf("%x", int(m_key_event->key()))));
    attr.push_back(std::make_pair(std::string("code"),
                                  tl::sprintf("%x", int(ch.unicode()))));
    attr.push_back(
        std::make_pair(std::string("modifiers"),
                       tl::sprintf("%x", int(m_key_event->modifiers()))));
  }
...
class LogActionEvent : public LogTargetedEvent {
public:
  LogActionEvent(const std::string &target, const std::string &action_name,
                 int xml_line = 0)
  ...
  virtual void issue_event() {
    QList<QAction *> actions =
        target_widget()->findChildren<QAction *>(tl::to_qstring(m_action_name));
    if (actions.size() == 0) {
      throw tl::Exception(tl::to_string(QObject::tr(
                              "'%s' is not a valid action name (line %d)")),
                          m_action_name, m_xml_line);
    } else {
  ...
  bool equals(const LogEventBase &b) const {
    ...
    return LogTargetedEvent::equals(b) && m_action_name == be->m_action_name;
  }
...
class LogResizeEvent : public LogTargetedEvent {
public:
  LogResizeEvent(const std::string &target, QSize size, QSize old_size,
                 int xml_line = 0)
      : LogTargetedEvent(target, xml_line), m_size(size), m_old_size(old_size) {
    //  .. nothing yet ..
  }

  virtual void issue_event() { target_widget()->resize(m_size); }

  virtual const char *name() const { return "resize"; }

  virtual void
  attributes(std::vector<std::pair<std::string, std::string>> &attr) const {
    LogTargetedEvent::attributes(attr);
    attr.push_back(
        std::make_pair(std::string("xsize"), tl::to_string(m_size.width())));
    attr.push_back(
        std::make_pair(std::string("ysize"), tl::to_string(m_size.height())));
    attr.push_back(std::make_pair(std::string("xsize_old"),
                                  tl::to_string(m_old_size.width())));
    attr.push_back(std::make_pair(std::string("ysize_old"),
                                  tl::to_string(m_old_size.height())));
  }

  QSize old_size() const { return m_old_size; }
  ...
  bool equals(const LogEventBase &b) const {
    ...
    return LogTargetedEvent::equals(b) && m_size == be->m_size &&
           m_old_size == be->m_old_size;
  }

  virtual bool spontaneous() const { return true; }
...
class LogProbeEvent : public LogTargetedEvent {
public:
  ...
  virtual void issue_event() {
    if (gtf::Recorder::instance() && gtf::Recorder::instance()->recording()) {
      ...
      if (!event.isAccepted()) {
        gtf::Recorder::instance()->probe(
            target, gtf::Recorder::instance()->probe_std(target));
      }
    }
  }

  virtual const char *name() const { return "probe"; }
};

class LogErrorEvent : public LogEventBase {
public:
  LogErrorEvent(int xml_line) : LogEventBase(xml_line) {
    //  .. for XML reader ..
  }

  LogErrorEvent(std::string &text) : LogEventBase(0) {
  ...
  virtual void attributes(
      std::vector<std::pair<std::string, std::string>> & /*attr*/) const {
...
/**
 *  @brief A helper class to store ActionInterceptor objects associated with the
 * connections monitored
 */
class ConnectionMap {
public:
  void register_connection(const ConnectionSpec &cs) {
    std::map<ConnectionSpec,
             std::pair<ActionInterceptor *, unsigned int>>::iterator i =
        m_map.find(cs);
    if (i == m_map.end()) {
      ActionInterceptor *handler = new ActionInterceptor(cs.sender, cs.sender);
      m_map.insert(
          std::make_pair(cs, std::make_pair(handler, (unsigned int)1)));
      QObject::connect(cs.sender, cs.signal.c_str(), handler,
                       SLOT(triggered()));
    } else {
      i->second.second++;
    }
  ...
  void unregister_connection(const ConnectionSpec &cs) {
    ...
    if (--i->second.second == 0) {
      QObject::disconnect(cs.sender, cs.signal.c_str(), i->second.first,
                          SLOT(triggered()));
...
void action_connect(QAction *action, const char *signal, QObject *receiver,
                    const char *slot) {
...
void action_disconnect(QAction *action, const char *signal, QObject *receiver,
                       const char *slot) {
  if (Recorder::instance()) {
...
void ActionInterceptor::triggered() {
  if (Recorder::instance()) {
...
EventList::~EventList() {
  for (std::vector<gtf::LogEventBase *>::const_iterator e = m_events.begin();
       e != m_events.end(); ++e) {
    delete *e;
  }
  m_events.clear();
...
void EventList::load(const std::string &filename, bool no_spontaneous) {
  QFile file(tl::to_qstring(filename));
  if (!file.exists()) {
    throw tl::Exception(tl::to_string(QObject::tr("File does not exist: ")) +
                        filename);
  }
...
void EventList::save(const std::string &filename) {
  std::ostream *os;
  if (filename != "-") {
    ...
    if (!os->good()) {
      delete os;
      throw tl::Exception(tl::to_string(QObject::tr(
                              "Unable to open file %s to write GUI test log")),
                          filename);
    }
  } else {
    os = &std::cout;
 ...
 *
 *  This method defines which widgets not to log for. These include ToolBar
 * widgets and Menu widgets as well as special ones used internally by Qt.
 */
static bool is_valid_widget(QWidget *w) {
  if (dynamic_cast<QToolBar *>(w) != 0 || dynamic_cast<QMenuBar *>(w) != 0 ||
      dynamic_cast<QMenu *>(w) != 0) {
    return false;
  } else if (w->parentWidget() == 0) {
    return (dynamic_cast<QDialog *>(w) != 0 ||
            dynamic_cast<QMainWindow *>(w) != 0);
  } else {
    return is_valid_widget(w->parentWidget());
  }
}

class ErrorLogRecorder : public tl::Channel {
public:
  ErrorLogRecorder(Recorder *rec) : mp_rec(rec) {}
...
Recorder::Recorder(QObject *parent, const std::string &log_file)
    : QObject(parent), m_recording(false), m_save_incremental(false),
      m_log_file(log_file) {
...
void Recorder::action(QAction *action) {
  if (m_recording) {
    QWidget *parent = dynamic_cast<QWidget *>(action->parent());
    tl_assert(parent != 0);
    m_events.add(new LogActionEvent(widget_to_path(parent),
                                    tl::to_string(action->objectName())));
  }
}

void Recorder::probe(QWidget *widget, const tl::Variant &data) {
  if (m_recording) {
...
void Recorder::save_incremental(bool si) { m_save_incremental = si; }

static tl::Variant probe_widget(QTreeView *tv) {
  ...
  if (rows > 0 && columns > 0) {
    ...
    while (col0.isValid()) {
      ...
      el.push(tl::Variant((tv->selectionModel()->isSelected(col0))
                              ? "Selected"
                              : "Not selected"));
...
static tl::Variant probe_widget(QTextEdit *te) {
...
static tl::Variant probe_widget(QLineEdit *le) {
  return tl::Variant(tl::to_string(le->text()));
}

static tl::Variant probe_widget(QSpinBox *sb) {
  return tl::Variant(long(sb->value()));
}

static tl::Variant probe_widget(QCheckBox *cb) {
  return tl::Variant(long(cb->isChecked()));
}

static tl::Variant probe_widget(QComboBox *cmb) {
  return tl::Variant(tl::to_string(cmb->lineEdit()->text()));
}

static tl::Variant probe_widget(QListView *lv) {
  tl::Variant list = tl::Variant::empty_list();
  ...
  if (rows > 0 && columns > 0) {
    ...
    while (col0.isValid()) {
      ...
      tl::Variant el = tl::Variant::empty_list();
      el.push(tl::Variant((lv->selectionModel()->isSelected(col0))
                              ? "Selected"
                              : "Not selected"));
...
static tl::Variant probe_widget(QRadioButton *rb) {
  return tl::Variant(long(rb->isChecked()));
}

static tl::Variant probe_widget(QPushButton *pb) {
  if (!pb->icon().isNull()) {
    return tl::Variant(
        image_to_variant(pb->icon().pixmap(pb->iconSize()).toImage()));
  } else {
    return tl::Variant(tl::to_string(pb->text()));
  }
}

tl::Variant Recorder::probe_std(QWidget *w) {
...
/*
 * HINT: it was not a good idea to suppress mouse clicks to the context menu -
this is caught by the
 * system anyway (the release event is not received) and the additional event
may be required to perform
 * selects on cell trees for example.
static bool has_context_menu (QWidget *rec)
{
  if (rec->contextMenuPolicy () == Qt::CustomContextMenu ||
rec->contextMenuPolicy () == Qt::ActionsContextMenu) { return true; } else if
(rec->contextMenuPolicy () != Qt::PreventContextMenu && rec->parentWidget ()) {
...
bool Recorder::eventFilter(QObject *object, QEvent *event) {
  ...
  if (Player::instance() && Player::instance()->playing()) {
    ...
    //  suppress spontaneous input events in playing mode - the user may not
    //  interact
    if (!Player::instance()->event_issued() &&
    ...
    if (event->type() != QEvent::Resize &&
        (Player::instance()->event_issued() != event ||
         Player::instance()->event_target() != object)) {
      return false;
    }

  } else {

    //  only log key events that are targeted towards widgets that do not have
    //  the focus this propagation of events is done automatically on replay in
    //  the same fashion.
  ...
  if (event->type() == QEvent::KeyPress ||
    ...
    if (keyEvent && keyEvent->key() != Qt::Key_Control &&
        keyEvent->key() != Qt::Key_Alt && keyEvent->key() != Qt::Key_Shift &&
        is_valid_widget(rec)) {
    ...
    if (mouseEvent && (mouseEvent->button() & Qt::LeftButton) != 0 &&
      ...
      if (event->type() == QEvent::MouseButtonPress) {
        ...
        for (QWidget *w = rec; w != 0; w = w->parentWidget()) {
          ...
          if (event.isAccepted()) {
            tl::info << tl::to_string(QObject::tr("Probed widget "))
                     << widget_to_path(w);
            return true;
          }
        }
...
Player::Player(QObject *parent)
    : QObject(parent), m_ms(0), m_playing_active(false), m_playing_index(0),
      m_breakpoint(-1), mp_event_issued(0), mp_event_target(0) {
  tl_assert(ms_instance == 0);
  ms_instance = this;
...
void Player::timer() {
  if (m_playing_active && m_playing_index < m_events.size() &&
      (m_breakpoint < 0 ||
       m_events[m_playing_index]->xml_line() <= m_breakpoint)) {
    //  restart timer (before issueing the event - because this might block in a
    //  modal dialog)
...
void Player::replay(int ms, int stop_at_line) {
  m_ms = ms;
...
void Player::issue_event(QWidget *target, QEvent *event) {
...
GtfXmlHandler::GtfXmlHandler(EventList *list)
    : mp_locator(0), mp_list(list), m_in_event(false) {
  //  .. nothing yet ..
}

void GtfXmlHandler::setDocumentLocator(QXmlLocator *locator) {
  mp_locator = locator;
}

bool GtfXmlHandler::characters(const QString &ch) {
  m_cdata += ch;
  //  continue
  return true;
}

bool GtfXmlHandler::endElement(const QString & /*namespaceURI*/,
...
bool GtfXmlHandler::startElement(const QString & /*namespaceURI*/,
                                 const QString &localName,
                                 const QString & /*qName*/,
  ...
  if (localName == QString::fromUtf8("mouse_button_release") ||
    ...
    QMouseEvent mouse_event(type, QPoint(xpos, ypos), Qt::MouseButton(button),
                            Qt::MouseButtons(button),
                            Qt::KeyboardModifiers(modifiers));
    enter_event(new LogMouseEvent(
        tl::to_string(atts.value(QString::fromUtf8("target"))), mouse_event,
        mp_locator->lineNumber()));
    ...
    QMouseEvent mouse_event(
        QEvent::MouseMove, QPoint(xpos, ypos), Qt::MouseButton(Qt::NoButton),
    ...
    QKeyEvent key_event(type, Qt::Key(key), Qt::KeyboardModifiers(modifiers),
                        text);
    enter_event(
        new LogKeyEvent(tl::to_string(atts.value(QString::fromUtf8("target"))),
                        key_event, mp_locator->lineNumber()));

  } else if (localName == QString::fromUtf8("action")) {

    enter_event(new LogActionEvent(
        tl::to_string(atts.value(QString::fromUtf8("target"))),
        tl::to_string(atts.value(QString::fromUtf8("action"))),
        mp_locator->lineNumber()));
    ...
    enter_event(new LogResizeEvent(
        tl::to_string(atts.value(QString::fromUtf8("target"))),
        QSize(xsize, ysize), QSize(xsize_old, ysize_old),
        mp_locator->lineNumber()));

  } else if (localName == QString::fromUtf8("probe")) {

    enter_event(new LogProbeEvent(
        tl::to_string(atts.value(QString::fromUtf8("target"))),
        mp_locator->lineNumber()));
...
void GtfXmlHandler::enter_event(LogEventBase *event) {
  if (m_in_event) {
...
void GtfXmlHandler::leave_event() {
  if (!m_in_event) {
    // TODO: (?) throw tl::XMLLocatedException (tl::to_string (QObject::tr
    // ("Unexpected element")), ex.lineNumber (), ex.columnNumber ());
    return;
  }
...
bool GtfXmlHandler::error(const QXmlParseException &ex) {
  throw tl::XMLLocatedException(tl::to_string(ex.message()), ex.lineNumber(),
                                ex.columnNumber());
}

bool GtfXmlHandler::fatalError(const std::string &msg) {
  throw tl::XMLLocatedException(msg.c_str(), mp_locator->lineNumber(),
                                mp_locator->columnNumber());
}

bool GtfXmlHandler::fatalError(const QXmlParseException &ex) {
  throw tl::XMLLocatedException(tl::to_string(ex.message()), ex.lineNumber(),
                                ex.columnNumber());
}

bool GtfXmlHandler::warning(const QXmlParseException &ex) {
  tl::XMLLocatedException lex(tl::to_string(ex.message()), ex.lineNumber(),
                              ex.columnNumber());
  tl::warn << lex.msg();
  //  continue
  return true;
