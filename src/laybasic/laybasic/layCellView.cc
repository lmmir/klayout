...
static std::string filename_for_caption(const std::string &fn) {
...
LayoutHandle::LayoutHandle(db::Layout *layout, const std::string &filename)
    : mp_layout(layout), m_ref_count(0), m_filename(filename), m_dirty(false),
      m_save_options_valid(false) {
  layout->technology_changed_event.add(this,
  ...
  mp_layout->layer_properties_changed_event.add(this,
...
void LayoutHandle::on_technology_changed() { technology_changed_event(); }

void LayoutHandle::layout_changed() { m_dirty = true; }
...
db::Layout &LayoutHandle::layout() const { return *mp_layout; }
...
const std::string &LayoutHandle::filename() const { return m_filename; }

const std::string &LayoutHandle::name() const { return m_name; }
...
const std::string &LayoutHandle::tech_name() const {
  static std::string s_empty;
  return mp_layout ? mp_layout->technology_name() : s_empty;
}

const db::Technology *LayoutHandle::technology() const {
...
LayoutHandle *LayoutHandle::find(const std::string &name) {
...
void LayoutHandle::get_names(std::vector<std::string> &names) {
  ...
  for (std::map<std::string, LayoutHandle *>::const_iterator h =
           ms_dict.begin();
       h != ms_dict.end(); ++h) {
...
void LayoutHandle::set_save_options(const db::SaveLayoutOptions &options,
                                    bool valid) {
  m_save_options = options;
  m_save_options_valid = valid;
}

void LayoutHandle::update_save_options(db::SaveLayoutOptions &options) {
#if defined(HAVE_QT)
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
    ...
    const lay::StreamWriterPluginDeclaration *decl =
        dynamic_cast<const lay::StreamWriterPluginDeclaration *>(&*cls);
    ...
    if (specific_options.get()) {
      decl->initialize_options_from_layout_handle(specific_options.get(),
                                                  *this);
...
void LayoutHandle::save_as(const std::string &fn,
  ...
  if (update) {
    ...
    //  We must not load with the original options after we have saved the file
    //  - hence we reset the reader options.
    ...
    //  NOTE: we don't use set_filename since this would re-attach the file
    //  watcher
    m_filename = fn;
  }
  ...
  try {
    ...
    {
      //  The write needs to be finished before the file watcher gets the new
      //  modification time
...
db::LayerMap LayoutHandle::load(const db::LoadLayoutOptions &options,
...
#if defined(HAVE_QT)
tl::FileSystemWatcher &LayoutHandle::file_watcher() {
  if (!mp_file_watcher) {
...
LayoutHandleRef::LayoutHandleRef() : mp_handle(0) {
  // .. nothing yet ..
}

LayoutHandleRef::LayoutHandleRef(LayoutHandle *h) : mp_handle(0) { set(h); }

LayoutHandleRef::LayoutHandleRef(const LayoutHandleRef &r) : mp_handle(0) {
  set(r.mp_handle);
}

LayoutHandleRef::~LayoutHandleRef() { set(0); }

bool LayoutHandleRef::operator==(const LayoutHandleRef &r) const {
  return mp_handle == r.mp_handle;
}

LayoutHandleRef &LayoutHandleRef::operator=(const LayoutHandleRef &r) {
  if (&r != this) {
    set(r.mp_handle);
  }
  return *this;
...
LayoutHandle *LayoutHandleRef::operator->() const { return mp_handle; }

LayoutHandle *LayoutHandleRef::get() const { return mp_handle; }

// -------------------------------------------------------------
//  CellView implementation

CellView::CellView()
    : mp_ctx_cell(0), m_ctx_cell_index(0), mp_cell(0),
      m_cell_index(cell_index_type(-1)) {}

bool CellView::operator==(const CellView &cv) const {
  return m_layout_href == cv.m_layout_href && mp_ctx_cell == cv.mp_ctx_cell &&
         m_ctx_cell_index == cv.m_ctx_cell_index && mp_cell == cv.mp_cell &&
         m_cell_index == cv.m_cell_index &&
         m_unspecific_path == cv.m_unspecific_path &&
         m_specific_path == cv.m_specific_path;
}
...
bool CellView::is_valid() const {
  ...
  for (unspecific_cell_path_type::const_iterator pp = m_unspecific_path.begin();
       pp != m_unspecific_path.end(); ++pp) {
    if (!m_layout_href.get()->layout().is_valid_cell_index(*pp)) {
      return false;
    }
  }
  for (specific_cell_path_type::const_iterator pp = m_specific_path.begin();
       pp != m_specific_path.end(); ++pp) {
    if (!pp->inst_ptr.instances() ||
        !pp->inst_ptr.instances()->is_valid(pp->inst_ptr) ||
        !m_layout_href.get()->layout().is_valid_cell_index(
            pp->inst_ptr.cell_index())) {
      return false;
    }
  }
...
void CellView::set_unspecific_path(const unspecific_cell_path_type &p) {
  ...
  if (p.size() > 0 && m_layout_href.get() &&
      p.back() < m_layout_href->layout().cells()) {
    m_cell_index = p.back();
...
void CellView::set_specific_path(const specific_cell_path_type &p) {
  ...
  m_specific_path = p;
  for (specific_cell_path_type::iterator pp = m_specific_path.begin();
       pp != m_specific_path.end(); ++pp) {
    //  fix elements of the path not associated with a certain array instance
    //  (this may happen if unspecific selections are put into the path)
  ...
  if (p.empty()) {
    ...
    mp_cell = mp_ctx_cell;
  } else if (m_layout_href.get() &&
             p.back().inst_ptr.cell_index() < m_layout_href->layout().cells()) {
...
CellView::unspecific_cell_path_type CellView::combined_unspecific_path() const {
  ...
  for (CellView::specific_cell_path_type::const_iterator p =
           m_specific_path.begin();
       p != m_specific_path.end(); ++p) {
...
void CellView::set_cell(const std::string &name) {
  ...
  std::pair<bool, db::cell_index_type> cp =
      m_layout_href->layout().cell_by_name(name.c_str());
  if (cp.first) {
    set_cell(cp.second);
  } else {
    reset_cell();
...
CellView CellView::deep_copy(db::Manager *manager) const {
  CellView r;
...
db::ICplxTrans CellView::context_trans() const {
  db::ICplxTrans trans;
  for (std::vector<db::InstElement>::const_iterator p = specific_path().begin();
       p != specific_path().end(); ++p) {
    trans = trans * p->complex_trans();
  }
  return trans;
}

db::DCplxTrans CellView::context_dtrans() const {
  tl_assert(m_layout_href.get() != 0);
...
bool CellViewRef::is_valid() const { return mp_view && mp_cv; }
...
lay::LayoutViewBase *CellViewRef::view() { return mp_view.get(); }

lay::LayoutHandle *CellViewRef::operator->() const {
  if (mp_cv) {
    return mp_cv->handle();
  } else {
    return 0;
...
void CellViewRef::set_unspecific_path(
    const CellViewRef::unspecific_cell_path_type &p) {
  if (is_valid()) {
...
void CellViewRef::set_specific_path(
    const CellViewRef::specific_cell_path_type &p) {
  if (is_valid()) {
    lay::CellView cv = *mp_cv;
...
db::Cell *CellViewRef::ctx_cell() const {
  return is_valid() ? mp_cv->ctx_cell() : 0;
}

db::Cell *CellViewRef::cell() const { return is_valid() ? mp_cv->cell() : 0; }
...
const CellViewRef::specific_cell_path_type &CellViewRef::specific_path() const {
  if (is_valid()) {
...
db::ICplxTrans CellViewRef::context_trans() const {
  if (is_valid()) {
...
db::DCplxTrans CellViewRef::context_dtrans() const {
  if (is_valid()) {
