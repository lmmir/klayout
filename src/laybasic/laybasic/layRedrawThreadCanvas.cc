...
BitmapCanvasData::BitmapCanvasData() : m_width(0), m_height(0) {
  //  .. nothing yet ..
}

BitmapCanvasData::~BitmapCanvasData() { clear_planes(); }

BitmapCanvasData::BitmapCanvasData(
    const std::vector<lay::Bitmap *> &plane_buffers,
    const std::vector<std::vector<lay::Bitmap *>> &drawing_plane_buffers,
...
void BitmapCanvasData::fetch(
    std::vector<lay::Bitmap *> &plane_buffers,
    std::vector<std::vector<lay::Bitmap *>> &drawing_plane_buffers,
...
bool BitmapCanvasData::can_fetch(
    const std::vector<lay::Bitmap *> &plane_buffers,
    const std::vector<std::vector<lay::Bitmap *>> &drawing_plane_buffers,
    unsigned int width, unsigned int height) const {
...
void BitmapCanvasData::assign(std::vector<lay::Bitmap *> &to,
                              const std::vector<lay::Bitmap *> &from) {
  while (!to.empty()) {
    delete to.back();
    to.pop_back();
  }

  for (std::vector<lay::Bitmap *>::const_iterator b = from.begin();
       b != from.end(); ++b) {
    to.push_back(new lay::Bitmap(**b));
  }
}

void BitmapCanvasData::assign(
    std::vector<std::vector<lay::Bitmap *>> &to,
    const std::vector<std::vector<lay::Bitmap *>> &from) {
  while (!to.empty()) {
    while (!to.back().empty()) {
      delete to.back().back();
      to.back().pop_back();
  ...
  for (std::vector<std::vector<lay::Bitmap *>>::const_iterator f = from.begin();
       f != from.end(); ++f) {
    to.push_back(std::vector<lay::Bitmap *>());
    for (std::vector<lay::Bitmap *>::const_iterator ff = f->begin();
         ff != f->end(); ++ff) {
...
BitmapRedrawThreadCanvas::BitmapRedrawThreadCanvas() : m_width(1), m_height(1) {
  // .. nothing yet ..
}
...
bool BitmapRedrawThreadCanvas::shift_supported() const { return true; }

bool BitmapRedrawThreadCanvas::is_plane_empty(unsigned int n) {
  bool ret = true;
...
static void shift_bitmap(const lay::Bitmap *from, lay::Bitmap *to, int dx,
                         int dy) {
...
void BitmapRedrawThreadCanvas::prepare(unsigned int nlayers, unsigned int width,
                                       unsigned int height, double resolution,
                                       const db::Vector *shift_vector,
                                       const std::vector<int> *planes,
                                       const lay::Drawings *drawings) {
  RedrawThreadCanvas::prepare(nlayers, width, height, resolution, shift_vector,
                              planes, drawings);
  ...
  if (shift_vector) {
    ...
    for (size_t i = 0; i < mp_plane_buffers.size(); ++i) {
      ...
      lay::Bitmap *to = mp_plane_buffers[i] =
          new lay::Bitmap(width, height, resolution);
    ...
    for (lay::Drawings::const_iterator d = drawings->begin();
      ...
      for (unsigned int i = 0; i < d->num_planes(); ++i) {
        ...
        lay::Bitmap *to = mp_drawing_plane_buffers[di][i] =
            new lay::Bitmap(width, height, resolution);
        shift_bitmap(from, to, shift_vector->x(), shift_vector->y());
        delete from;
      }
    ...
    for (std::vector<int>::const_iterator l = planes->begin();
         l != planes->end(); ++l) {
      ...
      if (*l < 0) {
        ...
        for (lay::Drawings::const_iterator d = drawings->begin();
             d != drawings->end(); ++d, ++di) {
          for (unsigned int i = 0; i < d->num_planes(); ++i) {
            if (mp_drawing_plane_buffers.size() > di &&
                mp_drawing_plane_buffers[di].size() > i) {
    ...
    for (lay::Drawings::const_iterator d = drawings->begin();
      ...
      for (unsigned int i = 0; i < d->num_planes(); ++i) {
        mp_drawing_plane_buffers.back().push_back(
            new lay::Bitmap(width, height, resolution));
      }
    }
  }
...
void BitmapRedrawThreadCanvas::set_plane(unsigned int n,
                                         const lay::CanvasPlane *plane) {
  lock();
...
void BitmapRedrawThreadCanvas::set_drawing_plane(
    unsigned int d, unsigned int n, const lay::CanvasPlane *plane) {
  lock();
  if (d < mp_drawing_plane_buffers.size() &&
      n < mp_drawing_plane_buffers[d].size()) {
    const lay::Bitmap *bitmap = dynamic_cast<const lay::Bitmap *>(plane);
    tl_assert(bitmap != 0);
...
void BitmapRedrawThreadCanvas::clear_planes() {
...
lay::CanvasPlane *BitmapRedrawThreadCanvas::create_drawing_plane() {
  return new lay::Bitmap(m_width, m_height, resolution());
}

void BitmapRedrawThreadCanvas::initialize_plane(lay::CanvasPlane *plane,
                                                unsigned int n) {
  lock();
  if (n < mp_plane_buffers.size()) {
...
void BitmapRedrawThreadCanvas::to_image(
    const std::vector<lay::ViewOp> &view_ops, const lay::DitherPattern &dp,
    const lay::LineStyles &ls, double dpr, tl::Color background,
    tl::Color foreground, tl::Color active, const lay::Drawings *drawings,
    tl::PixelBuffer &img, unsigned int width, unsigned int height) {
  if (width > m_width) {
    width = m_width;
  }
  if (height > m_height) {
                              ...
                              //查看图片debugdata/layRedrawThreadCanvas1.png
  {
    ...
    //    bitmaps_to_image(view_ops, mp_plane_buffers, dp, ls, dpr, &pb, width,
    //                     height, true, &mutex());
    //    pb.to_image().save("/home/yangqi/layRedrawThreadCanvas1.png");
  }
  ...
  for (lay::Drawings::const_iterator d = drawings->begin();
       d != drawings->end() && bt != mp_drawing_plane_buffers.end();
       ++d, ++bt) {
    bitmaps_to_image(d->get_view_ops(*this, background, foreground, active),
...
void BitmapRedrawThreadCanvas::to_image_mono(
    const std::vector<lay::ViewOp> &view_ops, const lay::DitherPattern &dp,
    const lay::LineStyles &ls, double dpr, bool background, bool foreground,
    bool active, const lay::Drawings *drawings, tl::BitmapBuffer &img,
    unsigned int width, unsigned int height) {
  if (width > m_width) {
    width = m_width;
  }
  ...
  for (lay::Drawings::const_iterator d = drawings->begin();
       d != drawings->end() && bt != mp_drawing_plane_buffers.end();
       ++d, ++bt) {
    bitmaps_to_image(d->get_view_ops(*this, background ? all_one : 0,
                                     foreground ? all_one : 0,
                                     active ? all_one : 0),
                     *bt, dp, ls, dpr, &img, width, height, true, &mutex());
  }
}
