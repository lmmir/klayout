...
PixelBufferPainter::PixelBufferPainter(tl::PixelBuffer &img, unsigned int width,
                                       unsigned int height, double resolution)
    : mp_img(&img), m_resolution(resolution), m_width(width), m_height(height) {
  // .. nothing yet ..
}
...
void PixelBufferPainter::draw_line(const db::Point &p1, const db::Point &p2,
                                   tl::Color c) {
  if (p1.x() == p2.x()) {
...
void PixelBufferPainter::fill_rect(const db::Point &p1, const db::Point &p2,
                                   tl::Color c) {
  int y1 = std::min(p1.y(), p2.y());
  int y2 = std::max(p1.y(), p2.y());
  for (int y = y1; y <= y2; ++y) {
...
void PixelBufferPainter::draw_rect(const db::Point &p1, const db::Point &p2,
                                   tl::Color c) {
  int y1 = std::min(p1.y(), p2.y());
  int y2 = std::max(p1.y(), p2.y());
  int x1 = std::min(p1.x(), p2.x());
...
void PixelBufferPainter::draw_text(const char *t, const db::Point &p,
                                   tl::Color c, int halign, int valign) {
  ...
  for (; *t; ++t) {
    ...
    if (x < -int(ff.width()) || x >= int(mp_img->width()) || y < 0 ||
        y >= int(mp_img->height() + ff.height())) {
      continue;
    }
    ...
    const uint32_t *dc =
        ff.data() + size_t(ch - ff.first_char()) * ff.height() * ff.stride();
