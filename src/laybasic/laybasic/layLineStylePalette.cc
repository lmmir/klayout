...
static LineStylePalette def_palette_object =
    LineStylePalette::default_palette();

LineStylePalette LineStylePalette::default_palette() {
  LineStylePalette p;
...
LineStylePalette LineStylePalette::operator=(const LineStylePalette &d) {
  if (&d != this) {
    m_styles = d.m_styles;
  }
  return *this;
...
unsigned int LineStylePalette::style_by_index(unsigned int n) const {
  if (styles() == 0) {
...
unsigned int LineStylePalette::styles() const {
...
void LineStylePalette::clear_styles() { m_styles.clear(); }

std::string LineStylePalette::to_string() const {
  std::string res;
...
void LineStylePalette::from_string(const std::string &s) {
  try {
    ...
    if (styles() == 0) {
      throw tl::Exception(
          tl::to_string(tr("invalid line style palette - no styles")));
    }

  } catch (std::exception &ex) {
    //  reformat error message
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr("Line style palette string format error: %s")),
        ex.what()));
  }
}
