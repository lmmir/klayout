...
/**
 *  @brief Returns a value indicating whether the needed entry can make use of
 * the one in the cache
 */
static bool applies(const lay::RedrawLayerInfo &in_cache,
                    const lay::RedrawLayerInfo &needed) {
...
ImageCacheEntry::ImageCacheEntry(
    const lay::Viewport &vp, const std::vector<lay::RedrawLayerInfo> &layers,
    bool precious)
    : m_opened(true), m_trans(vp.trans()), m_layers(layers),
      m_width(vp.width()), m_height(vp.height()), m_precious(precious) {
  //  .. nothing yet ..
}

bool ImageCacheEntry::equals(
    const lay::Viewport &vp,
    const std::vector<lay::RedrawLayerInfo> &layers) const {
  if (!m_trans.equal(vp.trans()) || m_width != vp.width() ||
      m_height != vp.height()) {
    return false;
  }
...
std::string ImageCacheEntry::to_string() const {
  return std::string(m_opened ? "(" : "") +
         std::string(m_precious ? "*" : " ") + tl::to_string(m_width) + "x" +
         tl::to_string(m_height) + " " + m_trans.to_string() +
         std::string(m_opened ? ")" : "");
}
...
LayoutCanvas::LayoutCanvas(lay::LayoutViewBase *view)
    : lay::ViewObjectUI(), mp_view(view), mp_image(0), mp_image_bg(0),
      mp_image_fg(0), m_background(0), m_foreground(0), m_active(0),
      m_oversampling(1), m_hrm(false), m_need_redraw(false),
      m_redraw_clearing(false), m_redraw_force_update(true),
      m_update_image(true), m_drawing_finished(false),
  ...
  m_viewport_l.set_size(m_viewport.width() * m_oversampling,
                        m_viewport.height() * m_oversampling);
...
double LayoutCanvas::resolution() const {
  if (m_hrm) {
...
void LayoutCanvas::init_ui(QWidget *parent) {
  ...
  if (widget()) {
    ...
    tl::Color bg = tl::Color(
        widget()->palette().color(QPalette::Normal, QPalette::Window).rgb());
    tl::Color fg = tl::Color(
        widget()->palette().color(QPalette::Normal, QPalette::Text).rgb());
    tl::Color active = tl::Color(
        widget()->palette().color(QPalette::Normal, QPalette::Mid).rgb());
    set_colors(bg, fg, active);
...
void LayoutCanvas::set_image_cache_size(size_t sz) { m_image_cache_size = sz; }
...
void LayoutCanvas::set_colors(tl::Color background, tl::Color foreground,
                              tl::Color active) {
...
const std::vector<lay::ViewOp> &LayoutCanvas::scaled_view_ops(unsigned int lw) {
  if (lw <= 1) {
    return m_view_ops;
  }
  ...
  for (std::vector<lay::ViewOp>::iterator vo = scaled_view_ops.begin();
       vo != scaled_view_ops.end(); ++vo) {
...
void LayoutCanvas::prepare_drawing() {
  if (m_need_redraw) {
    ...
    BitmapViewObjectCanvas::set_size(m_viewport_l.width(),
                                     m_viewport_l.height(), resolution());

    if (!mp_image || (unsigned int)mp_image->width() != m_viewport_l.width() ||
        (unsigned int)mp_image->height() != m_viewport_l.height()) {
      if (mp_image) {
        delete mp_image;
      }
      mp_image =
          new tl::PixelBuffer(m_viewport_l.width(), m_viewport_l.height());
      if (mp_image_fg) {
        delete mp_image_fg;
        mp_image_fg = 0;
      }
    ...
    //  Cancel any pending "finish" event so there is no race between finish and
    //  restart (important for caching)
    ...
    if (c != m_image_cache.end()) {
      ...
      for (size_t i = 0; i < m_image_cache.size(); ++i) {
        ...
        if (m_image_cache[i].opened() ||
            m_image_cache[i].equals(m_viewport_l, m_layers)) {
      ...
      if (m_image_cache_size == 0) {
        ...
        if (m_image_cache_size == 1) {
          if (precious ||
              (!m_image_cache.empty() && !m_image_cache.front().precious())) {
            m_image_cache.clear();
          }
        } else if (m_image_cache.size() > m_image_cache_size - 1) {
          for (size_t i = 0; i < m_image_cache.size() &&
                             m_image_cache.size() > m_image_cache_size - 1;
               ++i) {
        ...
        if (m_image_cache.size() < m_image_cache_size) {
          m_image_cache.push_back(
              ImageCacheEntry(m_viewport_l, m_layers, precious));
        }
      }

      if (m_redraw_clearing) {
        mp_redraw_thread->start(
            mp_view->synchronous() ? 0 : mp_view->drawing_workers(), m_layers,
            m_viewport_l, resolution(), m_redraw_force_update);
      } else {
        mp_redraw_thread->restart(m_need_redraw_layer);
      }
    }

    //  for short draw jobs, the drawing is already done now. For others display
    //  the busy cursor.
...
void LayoutCanvas::update_image() {
  // this will make the image being redone (except for background objects which
  // will only be redrawn on touch_bg)
  m_update_image = true;
...
void LayoutCanvas::paint_event() {
  ...
  tl::SelfTimer timer_info(tl::verbosity() >= 41,
                           tl::to_string(QObject::tr("PaintEvent")));
  ...
  if (mp_image) {
    ...
    if (m_update_image || needs_update_bg()) {
      ...
      to_image(scaled_view_ops(1.0 / resolution()), dither_pattern(),
               ...
               m_viewport_l.width(), m_viewport_l.height());
    ...
    if (!mp_image_fg || needs_update_static() ||
        int(mp_image->width()) !=
            (int)mp_image_fg->width() * int(m_oversampling) ||
        int(mp_image->height()) !=
            (int)mp_image_fg->height() * int(m_oversampling)) {
      ...
      if (fg_bitmaps() > 0) {
        ...
        bitmaps_to_image(fg_view_op_vector(), fg_bitmap_vector(),
                         dither_pattern(), line_styles(), 1.0 / resolution(),
                         &full_image, m_viewport_l.width(),
                         m_viewport_l.height(), false, &m_mutex);

        //  render the foreground parts ..
        if (m_oversampling == 1) {
          *mp_image_fg = full_image;
        } else {
          tl::PixelBuffer subsampled_image(m_viewport.width(),
                                           m_viewport.height());
    ...
    if (fg_bitmaps() > 0) {
      ...
      bitmaps_to_image(fg_view_op_vector(), fg_bitmap_vector(),
                       dither_pattern(), line_styles(), 1.0 / resolution(),
                       &full_image, m_viewport_l.width(), m_viewport_l.height(),
                       false, &m_mutex);
...
class DetachedViewObjectCanvas : public BitmapViewObjectCanvas {
public:
  DetachedViewObjectCanvas(tl::Color bg, tl::Color fg, tl::Color ac,
                           unsigned int width_l, unsigned int height_l,
                           double resolution, tl::PixelBuffer *img)
      : BitmapViewObjectCanvas(width_l, height_l, resolution), m_bg(bg),
        m_fg(fg), m_ac(ac), mp_image(img) {
    //  TODO: Good choice?
    m_gamma = 2.0;
  ...
  tl::Color background_color() const { return m_bg; }

  tl::Color foreground_color() const { return m_fg; }

  tl::Color active_color() const { return m_ac; }

  virtual tl::PixelBuffer *bg_image() {
    return mp_image_l ? mp_image_l : mp_image;
  }

  void transfer_to_image(const lay::DitherPattern &dp,
                         const lay::LineStyles &ls, unsigned int width,
                         unsigned int height) {
    if (mp_image_l) {
      unsigned int os = mp_image_l->width() / width;
      mp_image->blowup(*mp_image_l, os);
      bitmaps_to_image(fg_view_op_vector(), fg_bitmap_vector(), dp, ls,
                       1.0 / resolution(), mp_image_l, mp_image_l->width(),
                       mp_image_l->height(), false, 0);
      mp_image_l->subsample(*mp_image, os, m_gamma);
    } else {
      bitmaps_to_image(fg_view_op_vector(), fg_bitmap_vector(), dp, ls,
                       1.0 / resolution(), mp_image, width, height, false, 0);
    }
    clear_fg_bitmaps();
  }
 ...
 *
 *  NOTE: this canvas does not support background painting (currently the
 * background objects do not support monochrome background painting anyway). Nor
 * does it support subsampling (that would mean grayscale).
 */
class DetachedViewObjectCanvasMono : public BitmapViewObjectCanvas {
public:
  DetachedViewObjectCanvasMono(bool bg, bool fg, bool ac, unsigned int width,
                               unsigned int height)
      : BitmapViewObjectCanvas(width, height, 1.0), m_bg(bg), m_fg(fg),
        m_ac(ac) {
    //  .. nothing yet ..
  }

  ~DetachedViewObjectCanvasMono() { clear_fg_bitmaps(); }

  tl::Color background_color() const { return m_bg ? 0xffffffff : 0; }

  tl::Color foreground_color() const { return m_fg ? 0xffffffff : 0; }

  tl::Color active_color() const { return m_ac ? 0xffffffff : 0; }

private:
  bool m_bg, m_fg, m_ac;
};

tl::PixelBuffer LayoutCanvas::image(unsigned int width, unsigned int height) {
  return image_with_options(width, height, -1, -1, -1.0, tl::Color(),
                            tl::Color(), tl::Color(), db::DBox());
}

tl::PixelBuffer LayoutCanvas::image_with_options(
  ...
  if (img.width() != width || img.height() != height) {
    throw tl::Exception(
        tl::to_string(tr("Unable to create an image with size %dx%d pixels")),
        width, height);
  }

  img.fill(background.rgb());

  //  provide canvas objects for the layout bitmaps and the
  //  foreground/background objects
  BitmapRedrawThreadCanvas rd_canvas;
  DetachedViewObjectCanvas vo_canvas(background, foreground, active,
                                     width * oversampling,
                                     height * oversampling, resolution, &img);
  ...
  rd_canvas.to_image(scaled_view_ops(linewidth), dither_pattern(),
                     line_styles(), 1.0 / resolution, background, foreground,
                     active, this, *vo_canvas.bg_image(), vp.width(),
                     vp.height());

  //  subsample current image to provide the background for the foreground
  //  objects
...
tl::BitmapBuffer LayoutCanvas::image_with_options_mono(
    unsigned int width, unsigned int height, int linewidth,
    tl::Color background_c, tl::Color foreground_c, tl::Color active_c,
    const db::DBox &target_box) {
  if (linewidth <= 0) {
    linewidth = 1;
  }

  bool background = background_c.is_valid() ? background_c.to_mono()
                                            : background_color().to_mono();
  bool foreground = foreground_c.is_valid() ? foreground_c.to_mono()
                                            : foreground_color().to_mono();
  bool active =
      active_c.is_valid() ? active_c.to_mono() : active_color().to_mono();

  //  provide canvas objects for the layout bitmaps and the
  //  foreground/background objects
  BitmapRedrawThreadCanvas rd_canvas;
  DetachedViewObjectCanvasMono vo_canvas(background, foreground, active, width,
                                         height);
  ...
  rd_canvas.to_image_mono(scaled_view_ops(linewidth), dither_pattern(),
                          line_styles(), linewidth, background, foreground,
                          active, this, img, vp.width(), vp.height());

  return img;
}

tl::PixelBuffer LayoutCanvas::screenshot() {
  //  if required, start the redraw thread ..
  prepare_drawing();
  ...
  DetachedViewObjectCanvas vo_canvas(background_color(), foreground_color(),
                                     active_color(), m_viewport_l.width(),
                                     m_viewport_l.height(), resolution(), &img);

  //  and paint the background objects. It uses "img" to paint on.
  do_render_bg(m_viewport_l, vo_canvas);

  //  paint the layout bitmaps
  to_image(scaled_view_ops(1.0 / resolution()), dither_pattern(), line_styles(),
           1.0 / resolution(), background_color(), foreground_color(),
           active_color(), this, *vo_canvas.bg_image(), m_viewport_l.width(),
           m_viewport_l.height());
  ...
  vo_canvas.transfer_to_image(dither_pattern(), line_styles(),
...
void LayoutCanvas::resize_event(unsigned int width, unsigned int height) {
  ...
  unsigned int wl = width * m_oversampling * dpr() + 0.5,
               hl = height * m_oversampling * dpr() + 0.5;
  ...
  if (m_viewport.width() != w || m_viewport.height() != h ||
    ...
    m_viewport_l.set_size(width * m_oversampling * dpr() + 0.5,
                          height * m_oversampling * dpr() + 0.5);
...
const db::DCplxTrans &LayoutCanvas::global_trans() const {
...
void LayoutCanvas::do_update_image() { update_image(); }

#if defined(HAVE_QT)
void LayoutCanvas::gtf_probe() {
  if (gtf::Recorder::instance() && gtf::Recorder::instance()->recording()) {
    gtf::Recorder::instance()->probe(
        widget(), gtf::image_to_variant(screenshot().to_image_copy()));
  }
}
#endif

void LayoutCanvas::redraw_all() { do_redraw_all(); }
...
void LayoutCanvas::redraw_selected(const std::vector<int> &layers) {
  ...
  m_need_redraw = true;
  m_need_redraw_layer.insert(m_need_redraw_layer.end(), layers.begin(),
                             layers.end());
  std::sort(m_need_redraw_layer.begin(), m_need_redraw_layer.end());
  m_need_redraw_layer.erase(
      std::unique(m_need_redraw_layer.begin(), m_need_redraw_layer.end()),
      m_need_redraw_layer.end());
  m_redraw_force_update = true;
...
void LayoutCanvas::update_drawings() { update_image(); }

void LayoutCanvas::signal_transfer_done() { m_do_update_image_dm(); }

void LayoutCanvas::signal_end_of_drawing() { m_do_end_of_drawing_dm(); }
