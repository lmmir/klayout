...
/**
 *  @brief A pretty heuristic method to determine the "enclosing distance" of a
...
Finder::Finder(bool point_mode, bool top_level_sel)
    : m_min_level(0), m_max_level(0), mp_layout(0), mp_view(0), m_cv_index(0),
      m_point_mode(point_mode), m_catch_all(false),
      m_top_level_sel(top_level_sel) {
...
void Finder::start(lay::LayoutViewBase *view, unsigned int cv_index,
                   const std::vector<db::DCplxTrans> &trans,
                   const db::DBox &region, const db::DBox &scan_region,
  ...
  m_max_level = std::max(
      m_min_level,
      std::min(max_level, m_top_level_sel ? ((int)cv.specific_path().size() + 1)
                                          : max_level));

  if (layers.size() == 1) {

    m_box_convert =
        db::box_convert<db::CellInst>(*mp_layout, (unsigned int)layers[0]);
  ...
  for (std::vector<db::DCplxTrans>::const_iterator t = trans.begin();
    ...
    do_find(*cv.cell(), int(cv.specific_path().size()),
            view->viewport().trans() * *t, cv.context_trans());
  }
}

void Finder::test_edge(const db::ICplxTrans &trans, const db::Edge &edge,
...
unsigned int Finder::test_edge(const db::ICplxTrans &trans, const db::Edge &edg,
...
void Finder::do_find(const db::Cell &cell, int level, const db::DCplxTrans &vp,
                     const db::ICplxTrans &t) {
  if (level <= m_max_level /*take level of cell itself*/
      && cell.is_proxy() && m_layers.size() == 1 &&
      (unsigned int)m_layers[0] == mp_layout->guiding_shape_layer()) {

    //  when looking at the guiding shape layer, we can visit this cell as well
    //  allowing to find the guiding shapes
    ...
    while (!inst.at_end()) {
      ...
      for (db::CellInstArray::iterator p =
               cell_inst.begin_touching(scan_box, m_box_convert);
           !p.at_end(); ++p) {

        m_path.push_back(db::InstElement(*inst, p));

        do_find(mp_layout->cell(cell_inst.object().cell_index()), level + 1, vp,
...
ShapeFinder::ShapeFinder(bool point_mode, bool top_level_sel,
                         db::ShapeIterator::flags_type flags,
                         const std::set<lay::ObjectInstPath> *excludes)
    : Finder(point_mode, top_level_sel),
      mp_excludes((excludes && !excludes->empty()) ? excludes : 0),
      m_flags(flags), m_cv_index(0), m_topcell(0), mp_text_info(0),
...
bool ShapeFinder::find(LayoutViewBase *view, const db::DBox &region_mu) {
  ...
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator llp =
           lprops.begin();
       llp != lprops.end();) {
    ...
    find_internal(view, lp0->cellview_index(), &lp0->prop_sel(),
                  lp0->inverse_prop_sel(), lp0->hier_levels(), lp0->trans(),
                  layers, region_mu);
  }

  //  search on the guiding shapes layer as well

  //  Use the visible layers for the context: the guiding shape is only looked
  //  up for cells having one of these layers
  m_context_layers.clear();
  for (std::vector<lay::LayerPropertiesConstIterator>::const_iterator llp =
           lprops.begin();
       llp != lprops.end(); ++llp) {
  ...
  for (std::set<std::pair<db::DCplxTrans, int>>::const_iterator v =
    ...
    find_internal(view, (unsigned int)v->second, 0, false,
                  lay::HierarchyLevelSelection(), trans, layers, region_mu);
  }
...
bool ShapeFinder::find(lay::LayoutViewBase *view,
                       const lay::LayerProperties &lprops,
  ...
  bool result = find_internal(view, lprops.cellview_index(), &lprops.prop_sel(),
                              lprops.inverse_prop_sel(), lprops.hier_levels(),
                              lprops.trans(), layers, region_mu);

  mp_progress = 0;
  return result;
}

bool ShapeFinder::find_internal(
    lay::LayoutViewBase *view, unsigned int cv_index,
    const std::set<db::properties_id_type> *prop_sel, bool inv_prop_sel,
    const lay::HierarchyLevelSelection &hier_sel,
    const std::vector<db::DCplxTrans> &trans_mu, const std::vector<int> &layers,
    const db::DBox &region_mu) {
  m_cv_index = cv_index;
  ...
  try {
    ...
    if ((m_flags & db::ShapeIterator::Texts) != 0 && mp_text_info &&
      ...
      start(view, m_cv_index, trans_mu, region_mu, scan_region_mu, min_level,
            max_level, layers);

      m_flags =
          db::ShapeIterator::flags_type(flags_saved - db::ShapeIterator::Texts);
    }

    //  another pass with tight search box and without texts
    start(view, m_cv_index, trans_mu, region_mu, region_mu, min_level,
          max_level, layers);
...
void ShapeFinder::visit_cell(const db::Cell &cell, const db::Box &hit_box,
  ...
  if (!m_context_layers.empty()) {
    ...
    std::map<db::cell_index_type, bool>::const_iterator ctx =
        m_cells_with_context.find(cell.cell_index());
    if (ctx == m_cells_with_context.end()) {

      bool has_context = false;
      for (std::vector<int>::const_iterator l = m_context_layers.begin();
           l != m_context_layers.end() && !has_context; ++l) {
        if (!cell.bbox((unsigned int)*l).empty()) {
          has_context = true;
        }
      }

      ctx = m_cells_with_context
                .insert(std::make_pair(cell.cell_index(), has_context))
                .first;
    }
    ...
    for (std::vector<int>::const_iterator l = layers().begin();
      ...
      if (layers().size() == 1 ||
        ...
        db::ShapeIterator shape = shapes.begin_touching(
            scan_box, m_flags, mp_prop_sel, m_inv_prop_sel);
        while (!shape.at_end()) {
          ...
          if (bbox.inside(hit_box)) {
            ...
            if (mp_excludes != 0 &&
                mp_excludes->find(m_founds.back()) != mp_excludes->end()) {
              m_founds.pop_back();
            }
          }
        ...
        while (!shape.at_end()) {
          ...
          if (match) {
            ...
            if (mp_excludes) {
              ...
              //  with an exclude list first create the selection item so we can
              //  check if it's part of the exclude set.

              //  in point mode just store the found object that has the least
              //  "distance" and is not in the exclude set
...
InstFinder::InstFinder(bool point_mode, bool top_level_sel, bool full_arrays,
                       bool enclose_inst,
                       const std::set<lay::ObjectInstPath> *excludes,
                       bool visible_layers)
    : Finder(point_mode, top_level_sel), m_cv_index(0), m_topcell(0),
      mp_excludes((excludes && !excludes->empty()) ? excludes : 0),
      m_full_arrays(full_arrays), m_enclose_insts(enclose_inst),
      m_visible_layers(visible_layers), mp_view(0), mp_progress(0) {
...
bool InstFinder::find(LayoutViewBase *view, unsigned int cv_index,
                      const db::DCplxTrans &trans_mu,
                      const db::DBox &region_mu) {
  tl::AbsoluteProgress progress(tl::to_string(tr("Selecting ...")));
  progress.set_unit(1000);
  progress.set_format("");
...
bool InstFinder::find_internal(LayoutViewBase *view, unsigned int cv_index,
                               const db::DCplxTrans &trans_mu,
                               const db::DBox &region_mu) {
  const lay::CellView &cv = view->cellview(cv_index);
  if (!cv.is_valid()) {
  ...
  if (m_visible_layers) {
    for (lay::LayerPropertiesConstIterator l = view->begin_layers();
         !l.at_end(); ++l) {
      if (!l->has_children() && l->visible(true /*real*/) &&
          l->valid(true /*real*/) && l->cellview_index() == int(cv_index)) {
        m_visible_layer_indexes.push_back(l->layer_index());
      }
    }
    //  add guiding shape and error layers so we can select cells by error
    //  markers or guiding shapes
  ...
  try {
    ...
    start(view, cv_index, tv, region_mu, region_mu, view->get_min_hier_levels(),
          view->get_max_hier_levels(), std::vector<int>());
  } catch (StopException) {
    // ..
  }
...
void InstFinder::visit_cell(const db::Cell &cell, const db::Box &search_box,
  ...
  if (!point_mode()) {
    ...
    while (!inst.at_end()) {
      ...
      const db::CellInstArray &cell_inst = inst->cell_inst();
      const db::Cell &inst_cell =
          layout().cell(cell_inst.object().cell_index());
      ...
      //  are hidden.
      if (level == max_level() - 1 || inst_cell.is_proxy() ||
          mp_view->is_cell_hidden(inst_cell.cell_index(), m_cv_index)) {

        db::box_convert<db::CellInst, false> bc(layout());
        for (db::CellInstArray::iterator p =
                 cell_inst.begin_touching(search_box, bc);
             !p.at_end(); ++p) {
          ...
          } else if (!m_visible_layers ||
                     level == mp_view->get_max_hier_levels() - 1 ||
                     mp_view->is_cell_hidden(inst_cell.cell_index(),
                                             m_cv_index)) {
            ibox = inst_cell.bbox();
          } else {
            for (std::vector<int>::const_iterator l =
                     m_visible_layer_indexes.begin();
                 l != m_visible_layer_indexes.end(); ++l) {
          ...
          if (!ibox.empty()) {
            ...
            if (!m_enclose_insts || box.inside(search_box)) {
              ...
              //  in "full arrays" mode, a single reference to that array is
              //  sufficient
              if (m_full_arrays) {
                break;
              }
        ...
        for (db::CellInstArray::iterator p =
          ...
          if (!ibox.empty()) {
            ...
            if (ibox.width() == 0 && ibox.height() == 0) {
              ...
              if (!match &&
                  db::inside_poly(poly.begin_edge(), search_box.center())) {
                d = t.ctrans(poly_dist(poly.begin_edge(), search_box.center()));
                match = true;
              }
            }

            d += 1; // the instance has a small penalty so that shapes win over
                    // instances
          }
