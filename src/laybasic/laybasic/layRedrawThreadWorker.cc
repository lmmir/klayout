...
static inline db::Box safe_transformed_box(const db::Box &box,
...
void RedrawThreadWorker::perform_task(tl::Task *task) {
  ...
  if (task_id >= 0) {
    ...
    //  HINT: the order in which the planes are delivered (the index stored in
    //  the first member of the pair below) must correspond with the order by
    //  which the ViewOp's are created inside LayoutView::set_view_ops
    m_buffers.clear();
    for (unsigned int i = 0; i < (unsigned int)planes_per_layer / 3; ++i) {

      //  context level planes
      unsigned int i1 =
          task_id * (planes_per_layer / 3) + special_planes_before + i;
      mp_canvas->initialize_plane(m_planes[i], i1);
      m_buffers.push_back(std::make_pair(i1, m_planes[i]));

      //  child level planes (if used)
      unsigned int i2 = (task_id + m_nlayers) * (planes_per_layer / 3) +
                        special_planes_before + i;
      mp_canvas->initialize_plane(m_planes[i + planes_per_layer / 3], i2);
      m_buffers.push_back(
          std::make_pair(i2, m_planes[i + planes_per_layer / 3]));

      //  current level planes
      unsigned int i3 = (task_id + m_nlayers * 2) * (planes_per_layer / 3) +
                        special_planes_before + i;
      mp_canvas->initialize_plane(m_planes[i + 2 * (planes_per_layer / 3)], i3);
      m_buffers.push_back(
          std::make_pair(i3, m_planes[i + 2 * (planes_per_layer / 3)]));
    }

    //  detect whether the text planes are empty. If not, the whole text plane
    //  must be redrawn to account for clipped texts
    bool text_planes_empty = true;
    for (unsigned int i = 0;
         i < (unsigned int)planes_per_layer && text_planes_empty;
         i += (unsigned int)planes_per_layer / 3) {
    ...
    if (!text_planes_empty) {
      ...
      text_redraw_regions.push_back(
          db::Box(0, 0, mp_canvas->canvas_width(), mp_canvas->canvas_height()));
    ...
    if (li.cellview_index >= 0) {
      ...
      if (cv.is_valid() && !cv->layout().under_construction() &&
          !(cv->layout().manager() && cv->layout().manager()->transacting())) {
        ...
        int ctx_path_length =
            int(m_cellviews[m_cv_index].specific_path().size());

        if (li.hier_levels.has_from_level()) {
          m_from_level =
              li.hier_levels.from_level(ctx_path_length, m_from_level);
        }
        ...
        if (li.layer_index >= 0) {
          ...
          if (tl::verbosity() >= 40) {
            tl::info << tl::to_string(tr("Drawing layer: "))
                     << mp_layout->get_properties(m_layer).name;
          }
          tl::SelfTimer timer(tl::verbosity() >= 41,
                              tl::to_string(tr("Drawing layer")));
          ...
          mp_renderer->default_text_size(
              db::Coord(m_default_text_size / mp_layout->dbu()));
          mp_renderer->set_font(db::Font(m_text_font));
          mp_renderer->apply_text_trans(m_apply_text_trans);

          for (std::vector<db::DCplxTrans>::const_iterator t = li.trans.begin();
               t != li.trans.end(); ++t) {
            db::CplxTrans trans =
                m_vp_trans * *t * db::CplxTrans(mp_layout->dbu());
            iterate_variants(m_redraw_region, ci, trans,
                             &RedrawThreadWorker::draw_layer);
            iterate_variants(text_redraw_regions, ci, trans,
                             &RedrawThreadWorker::draw_text_layer);
          }
          ...
          //  if no specific layer is assigned, draw cell boxes with the style
          //  given
          if (tl::verbosity() >= 40) {
            tl::info << tl::to_string(tr("Drawing custom frames"));
          }
          tl::SelfTimer timer(tl::verbosity() >= 41,
                              tl::to_string(tr("Drawing frames")));

          for (std::set<std::pair<db::DCplxTrans, int>>::const_iterator b =
                   m_box_variants.begin();
               b != m_box_variants.end(); ++b) {
            if (b->second == li.cellview_index) {
              db::CplxTrans trans =
                  m_vp_trans * b->first * db::CplxTrans(mp_layout->dbu());
              iterate_variants(m_redraw_region, ci, trans,
                               &RedrawThreadWorker::draw_boxes);
              iterate_variants(text_redraw_regions, ci, trans,
                               &RedrawThreadWorker::draw_box_properties);
            }
          }
        }
    ...
    tl::SelfTimer timer(tl::verbosity() >= 41,
                        tl::to_string(tr("Drawing frames and guiding shapes")));
    ...
    for (std::set<std::pair<db::DCplxTrans, int>>::const_iterator b =
      ...
      if (cv.is_valid() && !cv->layout().under_construction() &&
        ...
        iterate_variants(m_redraw_region, cv.cell_index(), trans,
                         &RedrawThreadWorker::draw_boxes);
        iterate_variants(text_redraw_regions, cv.cell_index(), trans,
                         &RedrawThreadWorker::draw_box_properties);
      }
    }
    ...
    for (std::set<std::pair<db::DCplxTrans, int>>::const_iterator b =
      ...
      if (cv.is_valid() && !cv->layout().under_construction() &&
        ...
        mp_prop_sel = 0;
        m_inv_prop_sel = false;
        //  draw one level more to show the guiding shapes as part of the
        //  instance
    ...
    tl::SelfTimer timer(tl::verbosity() >= 41,
                        tl::to_string(tr("Drawing decorations")));

    m_buffers.clear();
    mp_canvas->initialize_plane(m_planes[0], m_nlayers * planes_per_layer +
                                                 special_planes_before);
    m_buffers.push_back(std::make_pair(
        m_nlayers * planes_per_layer + special_planes_before, m_planes[0]));

    unsigned int nd = 0;
    for (std::vector<lay::Drawing *>::iterator d = mp_drawings.begin();
         d != mp_drawings.end(); ++d, ++nd) {
  ...
  if (tl::verbosity() >= 30) {
    for (cell_cache_t::iterator cc = m_cell_cache.begin();
         cc != m_cell_cache.end(); ++cc) {
      tl::info << "Cell cache: " << mp_layout->cell_name(cc->first.ci) << " ("
               << cc->first.nlevels << ":" << cc->first.trans.to_string()
               << ") " << cc->second.fill->width() << " x "
               << cc->second.fill->height() << " -> " << cc->second.hits
               << " hits";
    }
  }
...
void RedrawThreadWorker::finish() {
...
void RedrawThreadWorker::setup(LayoutViewBase *view, RedrawThreadCanvas *canvas,
                               const std::vector<db::Box> &redraw_region,
                               const db::DCplxTrans &vp_trans) {
  ...
  mp_drawings.clear();
  for (lay::Drawings::iterator d = view->drawings()->begin();
       d != view->drawings()->end(); ++d) {
  ...
  //  first update event.
  m_clock = tl::Clock::current() -
            tl::Clock((update_interval - first_snapshot_delay) * 0.001);
...
void RedrawThreadWorker::transfer() {
  for (std::vector<std::pair<unsigned int, lay::CanvasPlane *>>::iterator b =
           m_buffers.begin();
       b != m_buffers.end(); ++b) {
...
void RedrawThreadWorker::test_snapshot(
    const UpdateSnapshotCallback *update_snapshot) {
  checkpoint();
...
void RedrawThreadWorker::draw_cell(bool drawing_context, int level,
                                   const db::CplxTrans &trans,
                                   const db::Box &box, const std::string &txt) {
  ...
  lay::CanvasPlane *contour =
      m_planes[1 + plane_group * (planes_per_layer / 3)];

  r.draw(box, trans, fill, contour, 0, 0);

  if (!txt.empty() && dbox.width() > m_min_size_for_label &&
      dbox.height() > m_min_size_for_label) {
    //  Hint: we render to contour because the texts plane is reserved for
    //  properties
    r.draw(dbox, txt, db::Font(m_box_font), db::HAlignCenter, db::VAlignCenter,
           //  TODO: apply "real" transformation?
           db::DFTrans(m_box_text_transform ? trans.fp_trans().rot()
                                            : db::DFTrans::r0),
           0, 0, 0, contour);
  }
}

void RedrawThreadWorker::draw_cell_properties(bool drawing_context, int level,
                                              const db::CplxTrans &trans,
                                              const db::Box &box,
  ...
  r.draw_propstring(prop_id, &mp_layout->properties_repository(),
                    (trans * box).p1(), texts, trans);
}

static bool cells_in(const db::Layout *layout, const db::Cell &cell,
                     const std::set<db::cell_index_type> &selected, int levels,
                     std::set<std::pair<int, db::cell_index_type>> &cache) {
  if (selected.find(cell.cell_index()) != selected.end()) {
    return true;
  }
  if (levels > 0) {
    for (db::Cell::child_cell_iterator c = cell.begin_child_cells();
         !c.at_end(); ++c) {
...
static bool
need_draw_box(const db::Layout *layout, const db::Cell &cell, int level,
              int to_level,
              const std::vector<std::set<db::cell_index_type>> &hidden_cells,
  ...
  if (hidden_cells.size() > cv_index && !hidden_cells[cv_index].empty()) {
    std::set<std::pair<int, db::cell_index_type>> cache;
    if (cells_in(layout, cell, hidden_cells[cv_index], to_level - level,
                 cache)) {
      return true;
    }
  }
...
void RedrawThreadWorker::draw_boxes(bool drawing_context,
                                    db::cell_index_type ci,
                                    const db::CplxTrans &trans,
                                    const std::vector<db::Box> &redraw_regions,
                                    int level) {
  ...
  if (!need_draw_box(mp_layout, cell, level, m_to_level, m_hidden_cells,
                     m_cv_index)) {
    return;
  }
...
void RedrawThreadWorker::draw_boxes(bool drawing_context,
                                    db::cell_index_type ci,
                                    const db::CplxTrans &trans,
                                    const db::Box &redraw_box, int level) {
  lay::Renderer &r = *mp_renderer;
  const db::Cell &cell = mp_layout->cell(ci);
  ...
  if (bbox.empty()) {
  ...
  } else if (level == m_to_level || (m_cv_index < int(m_hidden_cells.size()) &&
                                     m_hidden_cells[m_cv_index].find(ci) !=
                                         m_hidden_cells[m_cv_index].end())) {
    ...
    if (dbbox.width() < 1.5 && dbbox.height() < 1.5) {
      ...
      db::Coord aw = db::coord_traits<db::Coord>::rounded(
          m_abstract_mode_width / mp_layout->dbu());
      std::vector<db::Box> vv;
      if (level == 1 && m_abstract_mode_width > 0 &&
          bbox.width() > db::Box::distance_type(aw * 2) &&
          bbox.height() > db::Box::distance_type(aw * 2)) {
        vv.reserve(4);
        vv.push_back(redraw_box & db::Box(bbox.left(), bbox.bottom(),
                                          bbox.left() + aw, bbox.top()));
        vv.push_back(redraw_box & db::Box(bbox.right() - aw, bbox.bottom(),
                                          bbox.right(), bbox.top()));
        vv.push_back(redraw_box & db::Box(bbox.left() + aw, bbox.bottom(),
                                          bbox.right() - aw,
                                          bbox.bottom() + aw));
        vv.push_back(redraw_box & db::Box(bbox.left() + aw, bbox.top() - aw,
                                          bbox.right() - aw, bbox.top()));
      } else {
        vv.reserve(1);
      ...
      for (std::vector<db::Box>::const_iterator v = vv.begin(); v != vv.end();
           ++v) {

        if (!v->empty()) {

          bool anything = false;
          db::cell_index_type last_ci =
          ...
          while (!inst.at_end()) {
            ...
            if (last_ci != new_ci) {
              //  Hint: don't use any_cell_box on partially visible cells
              //  because that will degrade performance
            ...
            if (anything) {
              ...
              if (simplify) {
                ...
                //  The array can be simplified if there are levels below to
                //  draw
                if (need_draw_box(mp_layout, mp_layout->cell(new_ci), level + 1,
                                  m_to_level, m_hidden_cells, m_cv_index)) {
                ...
                //  The array (or single instance) must be iterated instance by
                //  instance
                for (db::CellInstArray::iterator p =
                         cell_inst.begin_touching(*v, bc);
                     !p.at_end();) {

                  test_snapshot(0);
                  db::ICplxTrans t(cell_inst.complex_trans(*p));
                  db::Box new_vp = safe_transformed_box(*v, t.inverted());
                  draw_boxes(drawing_context, new_ci, trans * t, new_vp,
                             level + 1);
                  ...
                  if (p.quad_id() > 0 && p.quad_id() != qid) {
                    ...
                    //  if the quad is very small we don't gain anything from
                    //  looking further into the quad - skip this one
...
void RedrawThreadWorker::draw_box_properties(bool drawing_context,
                                             db::cell_index_type ci,
                                             const db::CplxTrans &trans,
                                             const std::vector<db::Box> &vp,
                                             int level) {
  if (!m_text_visible) {
    return;
  }
...
void RedrawThreadWorker::draw_box_properties(
    bool drawing_context, db::cell_index_type ci, const db::CplxTrans &trans,
    const std::vector<db::Box> &vp, int level, db::properties_id_type prop_id) {
  //  do not draw, if there is nothing to draw
  ...
  for (std::vector<db::Box>::const_iterator b = vp.begin(); b != vp.end();
       ++b) {
  ...
  if (bbox.empty()) {
    ...
    if (dbbox.width() < 1.5 && dbbox.height() < 1.5) {
      ...
      if (level == 1 && m_abstract_mode_width > 0 &&
          ...
          bbox.height() > db::Box::distance_type(aw * 2)) {
        vv.reserve(4);
        vv.push_back(vp & db::Box(bbox.left(), bbox.bottom(), bbox.left() + aw,
                                  bbox.top()));
        vv.push_back(vp & db::Box(bbox.right() - aw, bbox.bottom(),
                                  bbox.right(), bbox.top()));
        vv.push_back(vp & db::Box(bbox.left() + aw, bbox.bottom(),
                                  bbox.right() - aw, bbox.bottom() + aw));
        vv.push_back(vp & db::Box(bbox.left() + aw, bbox.top() - aw,
                                  bbox.right() - aw, bbox.top()));
      } else {
        vv.reserve(1);
        vv.push_back(vp);
            ...
            if (anything) {
              ...
              if (!simplify) {
                ...
                for (db::CellInstArray::iterator p =
                  ...
                  db::ICplxTrans t(cell_inst.complex_trans(*p));
                  db::Box new_vp = safe_transformed_box(*v, t.inverted());
                  draw_box_properties(drawing_context, new_ci, trans * t,
                                      new_vp, level + 1, cell_inst_prop);
                }
              }
            }
...
/**
 *  @brief A helper function to determine if there are any area-type shapes on
 * the cell below to a certain hierarchy level
 *
 *  @return The number of empty hierarchy levels below the cell (0: there are
 * area-type shapes in this cell)
 */
bool RedrawThreadWorker::any_shapes(db::cell_index_type cell_index,
                                    unsigned int levels) {
  //  if the cell is "hidden", it does not need to be drawn
  if (int(m_hidden_cells.size()) > m_cv_index) {
    if (m_hidden_cells[m_cv_index].find(cell_index) !=
        m_hidden_cells[m_cv_index].end()) {
      return false;
    }
  }
  ...
  RedrawThreadWorker::micro_instance_cache_t::const_iterator c =
      m_mi_cache.find(std::make_pair(cell_index, levels));
  if (c == m_mi_cache.end()) {

    int ret = false;

    const db::Cell &cell = mp_layout->cell(cell_index);
    if (!cell.shapes(m_layer)
             .begin(db::ShapeIterator::Polygons | db::ShapeIterator::Edges |
                        db::ShapeIterator::Paths | db::ShapeIterator::Boxes |
                        db::ShapeIterator::Points,
                    mp_prop_sel, m_inv_prop_sel)
             .at_end()) {
      ret = true;
    } else if (levels > 1) {
    ...
    c = m_mi_cache
            .insert(std::make_pair(std::make_pair(cell_index, levels), ret))
            .first;
  }
 ...
 * area-type shapes in this cell)
 */
bool RedrawThreadWorker::any_cell_box(db::cell_index_type cell_index,
                                      unsigned int levels) {
  ...
  //  the cache contains all cells that are visited already
  RedrawThreadWorker::micro_instance_cache_t::const_iterator c =
      m_mi_cell_box_cache.find(std::make_pair(cell_index, levels));
...
/**
 *  @brief A helper function to determine if there are any text shapes on the
 * cell below to a certain hierarchy level
 *
 *  @return The number of empty hierarchy levels below the cell (0: there are
 * texts in this cell)
 */
bool RedrawThreadWorker::any_text_shapes(db::cell_index_type cell_index,
                                         unsigned int levels) {
  //  if the cell is "hidden", it does not need to be drawn
  if (int(m_hidden_cells.size()) > m_cv_index) {
    if (m_hidden_cells[m_cv_index].find(cell_index) !=
  ...
  //  the cache contains all cells that are visited already
  RedrawThreadWorker::micro_instance_cache_t::const_iterator c =
      m_mi_text_cache.find(std::make_pair(cell_index, levels));
  if (c == m_mi_text_cache.end()) {
    ...
    const db::Cell &cell = mp_layout->cell(cell_index);
    if (!cell.shapes(m_layer)
             .begin(db::ShapeIterator::Texts, mp_prop_sel, m_inv_prop_sel)
             .at_end() ||
        (m_show_properties && !cell.shapes(m_layer)
                                   .begin(db::ShapeIterator::AllWithProperties,
                                          mp_prop_sel, m_inv_prop_sel)
                                   .at_end())) {
      ret = true;
    } else if (levels > 1) {
...
static bool has_zero_bit(const lay::Bitmap *bitmap, unsigned int ixmin,
                         unsigned int iymin, unsigned int ixmax,
                         unsigned int iymax) {
  ...
  if (imin == imax) {
    ...
    uint32_t m = ((unsigned int)0xffffffff << (ixmin % 32)) &
                 ((unsigned int)0xffffffff >> (31 - (ixmax % 32)));
...
static bool skip_quad(const db::Box &qb, const lay::Bitmap *vertex_bitmap,
                      const db::CplxTrans &trans) {
  ...
  db::DBox qb_trans =
      (trans * qb) & db::DBox(0, 0, vertex_bitmap->width() - 1.0 - 1e-6,
                              vertex_bitmap->height() - 1.0 - 1e-6);
  if (qb_trans.empty()) {
    return true;
  }
...
inline void copy_bitmap(const lay::Bitmap *from, lay::Bitmap *to, int dx,
                        int dy) {
  if (to) {
    to->merge(from, dx, dy);
  }
...
std::vector<db::Box>
RedrawThreadWorker::search_regions(const db::Box &cell_bbox, const db::Box &vp,
                                   int level) {
  std::vector<db::Box> vv;

  //  create a set of boxes to look into
  db::Coord aw = db::coord_traits<db::Coord>::rounded(m_abstract_mode_width /
                                                      mp_layout->dbu());
  if (vp == db::Box::world()) {
    vv.push_back(vp);
  } else if (level == 1 && m_abstract_mode_width > 0 &&
             cell_bbox.width() > db::Box::distance_type(aw * 2) &&
             cell_bbox.height() > db::Box::distance_type(aw * 2)) {
    vv.push_back(vp & db::Box(cell_bbox.left(), cell_bbox.bottom(),
                              cell_bbox.left() + aw, cell_bbox.top()));
    vv.push_back(vp & db::Box(cell_bbox.right() - aw, cell_bbox.bottom(),
                              cell_bbox.right(), cell_bbox.top()));
    vv.push_back(vp & db::Box(cell_bbox.left() + aw, cell_bbox.bottom(),
                              cell_bbox.right() - aw, cell_bbox.bottom() + aw));
    vv.push_back(vp & db::Box(cell_bbox.left() + aw, cell_bbox.top() - aw,
                              cell_bbox.right() - aw, cell_bbox.top()));
  } else {
    vv.push_back(vp);
  }
...
void RedrawThreadWorker::draw_text_layer(bool drawing_context,
                                         db::cell_index_type ci,
                                         const db::CplxTrans &trans,
                                         const std::vector<db::Box> &vp,
                                         int level) {
  ...
  //  do not draw, if there is nothing to draw
  if (mp_layout->cells() <= ci || vp.empty() ||
      mp_layout->cell(ci).bbox(m_layer).empty()) {
    return;
  }
  if (cell_var_cached(ci, trans)) {
    return;
  ...
  if (m_text_lazy_rendering && vertex_bitmap) {
    opt_bitmap.reset(new lay::Bitmap(vertex_bitmap->width(),
                                     vertex_bitmap->height(),
                                     vertex_bitmap->resolution()));
  }

  for (std::vector<db::Box>::const_iterator b = vp.begin(); b != vp.end();
       ++b) {
    draw_text_layer(drawing_context, ci, trans, *b, level, fill, frame, vertex,
                    text, opt_bitmap.get());
  }
}

void RedrawThreadWorker::draw_text_layer(
    bool drawing_context, db::cell_index_type ci, const db::CplxTrans &trans,
    const db::Box &vp, int level, CanvasPlane *fill, CanvasPlane *frame,
    CanvasPlane *vertex, CanvasPlane *text, lay::Bitmap *opt_bitmap) {
  test_snapshot(0);
  ...
  if (m_drop_small_cells && drop_cell(cell, trans)) {
    ...
    bool hidden = (m_cv_index < int(m_hidden_cells.size()) &&
                   m_hidden_cells[m_cv_index].find(ci) !=
                       m_hidden_cells[m_cv_index].end());
    ...
    if (level >= m_from_level && level < m_to_level && !hidden) {
      ...
      if (m_text_lazy_rendering &&
          ((dbbox_tot.width() < 2.5 && dbbox_tot.height() < 1.5) ||
           (dbbox_tot.width() < 1.5 && dbbox_tot.height() < 2.5))) {

        bool anything = true;
        if (level == 0 && cell_bbox.inside(vp)) {
          //  Hint: on levels below 0 we enter this procedure only if there is a
          //  text Hint: don't use any_text_shapes on partially visible cells
          //  because that will degrade performance
        ...
        bool text_simplified =
            m_text_lazy_rendering &&
            (dbbox_tot.width() <= 8.0 || dbbox_tot.height() <= 8.0);

        const db::Shapes &shapes = cell.shapes(m_layer);

        //  In lazy text rendering mode, all texts are only rendered if the cell
        //  is "reasonable large". Empirically, a reasonable minimum dimension
        //  of 8x8 pixels was determined. otherwise just a few texts are
        //  rendered.
        ...
        for (std::vector<db::Box>::const_iterator v = vv.begin(); v != vv.end();
          ...
          if (!v->empty()) {
            ...
            db::ShapeIterator shape = shapes.begin_touching(
                *v, db::ShapeIterator::Texts, mp_prop_sel, m_inv_prop_sel);
            ...
            shape =
                shapes.begin_touching(*v, db::ShapeIterator::AllWithProperties,
                                      mp_prop_sel, m_inv_prop_sel);
            while (!shape.at_end() && ntexts > 0) {

              test_snapshot(0);

              r.draw_propstring(*shape, &mp_layout->properties_repository(),
                                text, trans);
              ++shape;
        ...
        if (!v->empty()) {
          ...
          while (!inst.at_end()) {
            ...
            if (m_text_lazy_rendering && qid != current_quad_id) {
              current_quad_id = qid;
              skip = opt_bitmap &&
                     skip_quad(inst.quad_box() & bbox, opt_bitmap, trans);
            }
            ...
            if (skip) {
              ...
              db::cell_index_type new_ci = cell_inst.object().cell_index();
              bool hidden = (m_cv_index < int(m_hidden_cells.size()) &&
                             m_hidden_cells[m_cv_index].find(new_ci) !=
                                 m_hidden_cells[m_cv_index].end());
              ...
              if (!cell_box.empty() && !hidden) {
                ...
                if (new_ci != last_ci) {
                  //  Hint: don't use any_text_shapes on partially visible cells
                  //  because that will degrade performance
                  if (cell_box.inside(vp)) {
                    last_ci = new_ci;
                    anything =
                        any_text_shapes(new_ci, m_to_level - (level + 1));
                  } else {
                    anything = true;
                  }
                }

                if (anything && m_text_lazy_rendering &&
                    cell_inst.is_regular_array(a, b, amax, bmax)) {
                ...
                if (simplify) {
                  ...
                  for (db::CellInstArray::iterator p =
                    ...
                    if (!m_draw_array_border_instances || p.index_a() <= 0 ||
                        (unsigned long)p.index_a() == amax - 1 ||
                        p.index_b() <= 0 ||
                        (unsigned long)p.index_b() == bmax - 1) {

                      db::ICplxTrans t(cell_inst.complex_trans(*p));
                      db::Box new_vp = safe_transformed_box(*v, t.inverted());
                      draw_text_layer(drawing_context, new_ci, trans * t,
                                      new_vp, level + 1, fill, frame, vertex,
                                      text, opt_bitmap);
                    }
                  }
                }
...
bool draw_array_simplified(lay::Renderer *r, const db::Shape &array_shape,
                           lay::CanvasPlane *frame, lay::CanvasPlane *vertex,
                           const db::CplxTrans &trans) {
  ...
  if (n >= 2) {
    ...
    if (shape_box_trans.width() < 1.5 && shape_box_trans.height() < 1.5) {
      ...
      } else if (is_regular && (a.x() == 0 || a.y() == 0) && na > 1 &&
                 trans.ctrans(a.length()) < 1.5) {
      ...
      } else if (is_regular && (b.x() == 0 || b.y() == 0) && nb > 1 &&
                 trans.ctrans(b.length()) < 1.5) {
...
void RedrawThreadWorker::draw_layer_wo_cache(
    int from_level, int to_level, db::cell_index_type ci,
  ...
  if (level >= from_level && level < to_level) {
    ...
    for (std::vector<db::Box>::const_iterator v = vv.begin(); v != vv.end();
      ...
      db::ShapeIterator shape(shapes.begin_touching(
          *v,
          db::ShapeIterator::Boxes | db::ShapeIterator::Polygons |
              db::ShapeIterator::Edges | db::ShapeIterator::Paths |
              db::ShapeIterator::Points,
          mp_prop_sel, m_inv_prop_sel));
      while (!shape.at_end()) {
        ...
        if (shape.in_array()) {
          ...
          if (last_array != shape.array()) {
            ...
            if (last_array.type() == db::Shape::PolygonPtrArray) {
              simplified =
                  draw_array_simplified<db::Shape::polygon_ptr_array_type>(
                      mp_renderer.get(), last_array, frame, vertex, trans);
            } else if (last_array.type() == db::Shape::SimplePolygonPtrArray) {
              simplified = draw_array_simplified<
                  db::Shape::simple_polygon_ptr_array_type>(
                  mp_renderer.get(), last_array, frame, vertex, trans);
            } else if (last_array.type() == db::Shape::PathPtrArray) {
              simplified =
                  draw_array_simplified<db::Shape::path_ptr_array_type>(
                      mp_renderer.get(), last_array, frame, vertex, trans);
            } else if (last_array.type() == db::Shape::BoxArray) {
              simplified = draw_array_simplified<db::Shape::box_array_type>(
                  mp_renderer.get(), last_array, frame, vertex, trans);
            } else if (last_array.type() == db::Shape::ShortBoxArray) {
              simplified =
                  draw_array_simplified<db::Shape::short_box_array_type>(
                      mp_renderer.get(), last_array, frame, vertex, trans);
            }
            ...
            if (simplify) {
              ...
              for (db::CellInstArray::iterator p =
                ...
                if (!m_draw_array_border_instances || p.index_a() <= 0 ||
                  ...
                  db::ICplxTrans t(cell_inst.complex_trans(*p));
                  db::Box new_vp = safe_transformed_box(*v, t.inverted());
                  draw_layer(from_level, to_level, new_ci, trans * t, new_vp,
                             level + 1, fill, frame, vertex, text,
                             update_snapshot);
...
class UpdateSnapshotWithCache : public UpdateSnapshotCallback {
public:
  UpdateSnapshotWithCache(const UpdateSnapshotCallback *parent,
                          const db::CplxTrans *trans, const CellCacheInfo *info,
                          lay::CanvasPlane *fill, lay::CanvasPlane *frame,
                          lay::CanvasPlane *vertex, lay::CanvasPlane *text)
      : mp_parent(parent), mp_trans(trans), mp_info(info), mp_fill(fill),
        mp_frame(frame), mp_vertex(vertex), mp_text(text) {
    //  .. nothing yet ..
  }
  ...
  void trigger() const {
    ...
    copy_bitmap(mp_info->fill, dynamic_cast<lay::Bitmap *>(mp_fill), t.x(),
                t.y());
    copy_bitmap(mp_info->frame, dynamic_cast<lay::Bitmap *>(mp_frame), t.x(),
                t.y());
    copy_bitmap(mp_info->vertex, dynamic_cast<lay::Bitmap *>(mp_vertex), t.x(),
                t.y());
    copy_bitmap(mp_info->text, dynamic_cast<lay::Bitmap *>(mp_text), t.x(),
                t.y());
  }
...
void RedrawThreadWorker::draw_layer(
    int from_level, int to_level, db::cell_index_type ci,
    const db::CplxTrans &trans, const std::vector<db::Box> &vp, int level,
    lay::CanvasPlane *fill, lay::CanvasPlane *frame, lay::CanvasPlane *vertex,
    lay::CanvasPlane *text, const UpdateSnapshotCallback *update_snapshot) {
  //  do not draw, if there is nothing to draw
  if (mp_layout->cells() <= ci || vp.empty()) {
  ...
  for (std::vector<db::Box>::const_iterator b = vp.begin(); b != vp.end();
       ++b) {
    draw_layer(from_level, to_level, ci, trans, *b, level, fill, frame, vertex,
               text, update_snapshot);
  }
}

void RedrawThreadWorker::draw_layer(
    int from_level, int to_level, db::cell_index_type ci,
    const db::CplxTrans &trans, const db::Box &vp, int level,
    lay::CanvasPlane *fill, lay::CanvasPlane *frame, lay::CanvasPlane *vertex,
    lay::CanvasPlane *text, const UpdateSnapshotCallback *update_snapshot) {
  test_snapshot(update_snapshot);
  ...
  bool hidden =
      (m_cv_index < int(m_hidden_cells.size()) &&
       m_hidden_cells[m_cv_index].find(ci) != m_hidden_cells[m_cv_index].end());
  if (hidden) {
    return;
  }
  ...
  if (level >= from_level && level < to_level) {
    ...
    if ((dbbox.width() < 2.5 && dbbox.height() < 1.5) ||
      ...
      if (bbox.touches(vp)) {
        ...
        bool anything = true;
        if (level == 0) {
          //  Hint: on levels below 0 we know that there is anything. Otherwise
          //  we would not enter this procedure Hint: don't use any_text_shapes
          //  on partially visible cells because that will degrade performance
      ...
      //  use the presence of a lay::Bitmap for the drawing plane as an
      //  indicator that we can cache the drawings
      bool can_cache =
          (m_bitmap_caching && dynamic_cast<lay::Bitmap *>(fill) != 0);
      ...
      if (can_cache) {
        ...
        if (cached_cell == m_cell_cache.end()) {
          ...
          cached_cell =
              m_cell_cache.insert(std::make_pair(key, CellCacheInfo())).first;
          ...
          int width = int(cell_box_trans.width() +
                          3); //  +3 = one pixel for a one-pixel frame at both
          ...
          //  this object is responsible for doing updates when a snapshot is
          //  taken
          UpdateSnapshotWithCache update_cached_snapshot(
              update_snapshot, &trans, &cached_cell->second, fill, frame,
              vertex, text);

          draw_layer_wo_cache(
              from_level, to_level, ci, drawing_trans, vv, level,
        ...
        copy_bitmap(cached_cell->second.fill, dynamic_cast<lay::Bitmap *>(fill),
                    t.x(), t.y());
        copy_bitmap(cached_cell->second.frame,
                    dynamic_cast<lay::Bitmap *>(frame), t.x(), t.y());
        copy_bitmap(cached_cell->second.vertex,
                    dynamic_cast<lay::Bitmap *>(vertex), t.x(), t.y());
        copy_bitmap(cached_cell->second.text, dynamic_cast<lay::Bitmap *>(text),
                    t.x(), t.y());

      } else {
        draw_layer_wo_cache(from_level, to_level, ci, trans, vv, level, fill,
                            frame, vertex, text, update_snapshot);
      }
    }
    ...
    std::vector<db::Box> vv;
    vv.push_back(vp);
    draw_layer_wo_cache(from_level, to_level, ci, trans, vv, level, fill, frame,
                        vertex, text, update_snapshot);
  }
}

void RedrawThreadWorker::draw_layer(bool drawing_context,
                                    db::cell_index_type ci,
                                    const db::CplxTrans &trans,
                                    const std::vector<db::Box> &redraw_regions,
                                    int level) {
  if (drawing_context) {
    ...
    if (m_to_level > m_from_level) {
      ...
      draw_layer(m_from_level, m_to_level, ci, trans, redraw_regions, level,
                 fill, frame, vertex, text, 0);
    }
    ...
    if (1 > m_from_level) {
      ...
      draw_layer(m_from_level, 1, ci, trans, redraw_regions, level, fill, frame,
                 vertex, text, 0);
    }
    ...
    if (m_to_level > 1) {
      ...
      draw_layer(1, m_to_level, ci, trans, redraw_regions, level, fill, frame,
                 vertex, text, 0);
    }
  }
}

bool RedrawThreadWorker::drop_cell(const db::Cell &cell,
                                   const db::CplxTrans &trans) {
...
bool RedrawThreadWorker::cell_var_cached(db::cell_index_type ci,
                                         const db::CplxTrans &trans) {
  if (mp_cell_var_cache) {
    //  Use the native transformation (just including cell instantiation
    //  components) to enable fuzzy comparison of floating-point coordinates:
    //  this requires a well-defined unit system to allow the definition of an
    //  uncertainty value.
    db::CplxTrans db_trans((m_vp_trans * mp_layout->dbu()).inverted() *
                           db::DCplxTrans(trans));
    if (mp_cell_var_cache->find(std::make_pair(db_trans, ci)) !=
        mp_cell_var_cache->end()) {
      ++m_cache_hits;
      return true;
    }
    ++m_cache_misses;
...
void RedrawThreadWorker::iterate_variants(
    const std::vector<db::Box> &redraw_regions, db::cell_index_type ci,
    db::CplxTrans trans,
    void (RedrawThreadWorker::*what)(bool, db::cell_index_type,
                                     const db::CplxTrans &,
                                     const std::vector<db::Box> &, int)) {
  //  save current state
  int from_level = m_from_level;
  int to_level = m_to_level;

  //  if a context path is given, we adjust the levels such that the target (not
  //  the context cell) is drawn and the context cell is visible through
  //  addressing negative levels. The iterate_variants_rec methods takes care of
  //  converting the negative hierarchy levels into traversals along the context
  //  path bottom up.
  ...
  if (m_from_level_default < 0 || ctx_path_length > 0) {
    ...
    //  multiple times.
    std::set<std::pair<db::CplxTrans, db::cell_index_type>,
             lay::CellVariantCacheCompare>
        cell_var_cache;
    ...
    cell_var_cache.insert(std::make_pair(
        db::CplxTrans(m_cellviews[m_cv_index].context_trans()), ci));
    ...
    //  draw the current instance without context (using a minimum of
    //  from_level=0 for this)
    mp_cell_var_cache = 0;
    int fl = m_from_level;
    if (m_from_level < 0) {
      m_from_level = 0;
    ...
    if (tl::verbosity() >= 40) {
      tl::info << tl::to_string(tr("Cell variant cache hits/misses: "))
...
void RedrawThreadWorker::iterate_variants_rec(
    const std::vector<db::Box> &redraw_regions, db::cell_index_type ci,
    const db::CplxTrans &trans, int level,
    void (RedrawThreadWorker::*what)(bool, db::cell_index_type,
                                     const db::CplxTrans &,
                                     const std::vector<db::Box> &, int),
    bool drawing_context) {
  ...
  if ((drawing_context || level > m_from_level) &&
      level + context_path_length > 0) {

    //  pull an specific instance from the instance stack and move this one up
    const db::InstElement &ie =
        m_cellviews[m_cv_index].specific_path().end()[level - 1];

    db::cell_index_type new_ci;
    if (level + context_path_length > 1) {
      new_ci = m_cellviews[m_cv_index]
                   .specific_path()
                   .end()[level - 2]
                   .inst_ptr.cell_index();
    } else {
    ...
    iterate_variants_rec(redraw_regions, new_ci, trans * t.inverted(),
                         level - 1, what, drawing_context);

  } else if (level > (drawing_context
                          ? (m_from_level_default - context_path_length)
                          : m_from_level) &&
             !p.at_end()) {
    ...
    while (!p.at_end()) {
      ...
      for (db::Cell::cell_inst_array_type::iterator pp = pi.begin();
           !pp.at_end(); ++pp) {

        db::ICplxTrans t(pi.complex_trans(*pp));
        iterate_variants_rec(redraw_regions, new_ci, trans * t, level - 1, what,
                             drawing_context);
      }
    ...
    for (std::vector<db::Box>::const_iterator rr = redraw_regions.begin();
         rr != redraw_regions.end(); ++rr) {

      db::Coord lim = std::numeric_limits<db::Coord>::max();
      db::DBox world(trans *
                     db::Box(db::Point(-lim, -lim), db::Point(lim, lim)));
      db::Box vp = db::Box(trans.inverted() * (world & db::DBox(*rr)));
      vp &= mp_layout->cell(ci).bbox(); // this avoids problems when accessing
                                        // designs through very large viewports
      if (!vp.empty()) {
