...
ZoomService::ZoomService(lay::LayoutViewBase *view)
    : lay::ViewService(view->canvas()), mp_view(view), mp_box(0), m_color(0) {}

ZoomService::~ZoomService() { drag_cancel(); }
...
bool ZoomService::mouse_move_event(const db::DPoint &p,
  ...
  if (prio) {
    ...
    if (mp_box) {
      ...
      m_vp.move(m_p1 - p);
      mp_view->pop_state(); //  we will overwrite the previous state so we don't
                            //  collect tiny move events
...
bool ZoomService::mouse_press_event(const db::DPoint &p, unsigned int buttons,
                                    bool prio) {
...
bool ZoomService::mouse_click_event(const db::DPoint &p, unsigned int buttons,
                                    bool prio) {
  if (!prio && (buttons & lay::RightButton) != 0) {
...
bool ZoomService::mouse_release_event(const db::DPoint & /*p*/,
                                      unsigned int /*buttons*/, bool prio) {
  if (prio) {
    ...
    if (mp_box) {
      ...
      if (mp_view) {
        ...
        //  we need to use the original screen coordinate to find the move
        //  direction
...
bool ZoomService::wheel_event(int delta, bool /*horizontal*/,
  ...
  if (!prio) {
    ...
    if (mp_view && vp.contains(p) && vp.width() > 0 && vp.height() > 0) {
      ...
      if (direction == vertical) {
        ...
        mp_view->zoom_box(db::DBox(
            p.x() - (p.x() - vp.left()) * f, p.y() - (p.y() - vp.bottom()) * f,
            p.x() - (p.x() - vp.right()) * f, p.y() - (p.y() - vp.top()) * f));
      }
    }
  }
  return false;
