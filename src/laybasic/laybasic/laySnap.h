 ...
 */
LAYBASIC_PUBLIC std::pair<db::DPoint, db::DPoint> snap(const db::DPoint &p1,
                                                       const db::DPoint &p2);
 ...
 *  resulting vector with respect to the original (angle-wise and length-wise).
 */
LAYBASIC_PUBLIC std::pair<db::DPoint, db::DPoint>
...
struct LAYBASIC_PUBLIC PointSnapToObjectResult {
  enum ObjectSnap { NoObject = 0, ObjectVertex, ObjectEdge, ObjectUnspecific };
  ...
  /**
   *  @brief Indicates the edge the point was snapped against unless in NoObject
   * mode
   */
  db::DEdge object_ref;
};
...
/**
 *  @brief combined grid-, projection- and object snapping provided to
 * implementing "magnetic features"
 *
 *  This method will snap a point pt to the grid, project it to one of the
 * cutlines or try to snap it to an object edge or point. It will use a
 * heuristics to determine what criterion is applied if a conflict is detected.
 *
 *  The function will return a PointSnapToObjectResult object.
 *
 *  @param view The layout view used for object snapping. Can be 0 to disable
 * object snapping
 *  @param pt The point to snap
 *  @param grid Either (0,0) to disable grid snapping or a (gx,gy) value for the
 * (potentially anisotropic grid)
 *  @param snap_range The search range for objects
 */
LAYBASIC_PUBLIC PointSnapToObjectResult obj_snap(lay::LayoutViewBase *view,
                                                 const db::DPoint &pt,
 ...
 * implementing "magnetic features"
 *
 *  This is a convenience method that creates the projection axes from a
 * reference point and an angle mode. "pr" is the reference point, "pt" is the
 * point to snap.
 */
LAYBASIC_PUBLIC PointSnapToObjectResult obj_snap(
    lay::LayoutViewBase *view, const db::DPoint &pr, const db::DPoint &pt,
    const db::DVector &grid, lay::angle_constraint_type ac, double snap_range);

/**
 *  @brief A structure describing the snap result for a two-sided object snap
...
struct LAYBASIC_PUBLIC TwoPointSnapToObjectResult {
  ...
  TwoPointSnapToObjectResult()
      : any(false), object_snap_first(NoObject), object_snap_second(NoObject) {}
  ...
  /**
   *  @brief Indicates the edge the point was snapped against unless in NoObject
   * mode Two values are provided for the first and second snap.
   */
  db::DEdge object_ref_first, object_ref_second;
};

/**
 *  @brief Same than obj_snap, but delivers two points on two opposite sides of
 * the initial point
 *
 *  This method basically implements "auto measure". The first value of the
 * returned pair is true if such an edge could be found. Otherwise it's false.
 */
LAYBASIC_PUBLIC TwoPointSnapToObjectResult obj_snap2(LayoutViewBase *view,
                                                     const db::DPoint &pt,
                                                     const db::DVector &grid,
 ...
 * returned pair is true if such an edge could be found. Otherwise it's false.
 *
 *  This version accepts two points defining different search regions for first
 * and second edge.
 */
LAYBASIC_PUBLIC TwoPointSnapToObjectResult obj_snap2(
    lay::LayoutViewBase *view, const db::DPoint &pt1, const db::DPoint &pt2,
    const db::DVector &grid, double min_search_range, double max_search_range);

/**
 *  @brief Same than the previous obj_snap2, but allows specification of an
 * angle constraint
 *
 *  Measurements will be confined to the direction specified.
 */
LAYBASIC_PUBLIC TwoPointSnapToObjectResult
 ...
 */
LAYBASIC_PUBLIC db::DVector snap_angle(const db::DVector &in,
 ...
 */
LAYBASIC_PUBLIC std::pair<db::Point, db::Point>
