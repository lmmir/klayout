...
MoveService::MoveService(lay::LayoutViewBase *view)
    : lay::ViewService(view->canvas()), m_dragging(false),
      m_dragging_transient(false), mp_editables(view), mp_view(view),
      m_global_grid(0.001) {
  //  .. nothing yet ..
}

MoveService::~MoveService() { drag_cancel(); }
...
bool MoveService::key_event(unsigned int key, unsigned int /*buttons*/) {
  ...
  if (!m_dragging && fabs(dx + dy) > 0.0 && mp_editables->has_selection()) {
    ...
    //  determine a shift distance which is 2, 5 or 10 times the grid and is
    //  more than 5 pixels
    double dmin =
        double(5 /*pixels min shift*/) / ui()->mouse_event_trans().mag();
    ...
    std::string pos = std::string("dx: ") + tl::micron_to_string(m_shift.x()) +
                      "  dy: " + tl::micron_to_string(m_shift.y());
    mp_view->message(pos);
...
bool MoveService::mouse_move_event(const db::DPoint &p, unsigned int buttons,
                                   bool prio) {
  bool ret = false;
...
bool MoveService::mouse_click_event(const db::DPoint &p, unsigned int buttons,
                                    bool prio) {
  if (prio && (buttons & lay::RightButton) != 0 && m_dragging) {
    if ((buttons & lay::ShiftButton) != 0) {
      mp_editables->move_transform(p, db::DFTrans(db::DFTrans::m90),
                                   ac_from_buttons(buttons));
    } else {
      mp_editables->move_transform(p, db::DFTrans(db::DFTrans::r90),
                                   ac_from_buttons(buttons));
    }
    return true;
  }
...
bool MoveService::mouse_double_click_event(const db::DPoint &p,
...
bool MoveService::mouse_release_event(const db::DPoint &p, unsigned int buttons,
                                      bool prio) {
  if (prio) {
    lay::SelectionService *selector = mp_view->selection_service();
    if (selector) {
...
bool MoveService::wheel_event(int delta, bool horizontal, const db::DPoint &p,
                              unsigned int buttons, bool prio) {
  if (prio) {
    lay::SelectionService *selector = mp_view->selection_service();
    if (selector) {
...
bool MoveService::mouse_press_event(const db::DPoint &p, unsigned int buttons,
                                    bool prio) {
  if (prio && (buttons & lay::LeftButton) != 0) {
    if (handle_click(p, buttons, false, 0)) {
      return true;
...
bool MoveService::begin_move(db::Transaction *transaction,
                             bool transient_selection) {
  if (m_dragging) {
    return false;
  }
  ...
  //  emulate a "begin move" at the current mouse position if inside the box or
...
bool MoveService::handle_click(const db::DPoint &p, unsigned int buttons,
                               bool drag_transient,
