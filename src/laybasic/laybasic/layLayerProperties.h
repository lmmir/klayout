 ...
 *  -255 and 255.
 */
class LAYBASIC_PUBLIC LayerProperties : public gsi::ObjectBase {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  "uninitialized" state.
   */
  size_t gen_id() const { return m_gen_id; }

  /**
   *  @brief Assignment alias for GSI binding
   */
  void assign_lp(const LayerProperties &d) { operator=(d); }
   ...
   *  @brief Inequality
   */
  bool operator!=(const LayerProperties &d) const { return !operator==(d); }

  /**
   *  @brief Utility: compute the effective color from a color with brightness
   * correction
   */
   ...
   *
   *  The effective fill color is computed from the frame color brightness and
   * the frame color.
   */
  tl::color_t eff_fill_color(bool real) const;

  /**
   *  @brief render the effective frame color plus an additional brightness
   * adjustment
   *
   *  This method returns the effective frame color with an additional
   ...
   * adjustment
   *
   *  This method returns the effective fill color with an additional brightness
   * adjustment applied.
   */
   ...
   */
  void set_frame_color(tl::color_t c) { set_frame_color_code(c | 0xff000000); }

  /**
   *  @brief Reset the frame color
   */
  void clear_frame_color() { set_frame_color_code(0); }

  /**
   *  @brief Test, if the frame color is set
   */
  bool has_frame_color(bool real) const { return frame_color(real) != 0; }
   ...
   */
  void set_fill_color(tl::color_t c) { set_fill_color_code(c | 0xff000000); }

  /**
   *  @brief Reset the fill color
   */
  void clear_fill_color() { set_fill_color_code(0); }

  /**
   *  @brief Test, if the frame color is set
   */
  bool has_fill_color(bool real) const { return fill_color(real) != 0; }
   ...
   */
  void clear_dither_pattern() { set_dither_pattern(-1); }

  /**
   *  @brief Test, if the dither pattern is set
   */
  bool has_dither_pattern(bool real) const { return dither_pattern(real) >= 0; }
   ...
   */
  void clear_line_style() { set_line_style(-1); }

  /**
   *  @brief Test, if the line style is set
   */
  bool has_line_style(bool real) const { return line_style(real) >= 0; }
   ...
   *
   *  A layer "shows something" if it is visible and it displays some
   * information, either shapes or cell boxes. Invalid layers, i.e. such that
   * have a layer selection which does not match one of the layers in the layout
   * or invalid layers, are not considered "visual".
   *
   ...
   */
  bool is_shape_layer() const { return layer_index() >= 0; }
  ...
  bool is_cell_box_layer() const {
    return source(true).special_purpose() ==
           lay::ParsedLayerSource::SP_CellFrame;
  }
   ...
   *  If it is set to false, the view's always_show_source attribute with
   * determine whether the source is shown.
   */
  std::string display_string(const lay::LayoutViewBase *view, bool real,
                             bool always_with_source = false) const;
   ...
   *
   *  Compute the "effective" properties (if the object is part of a hierarchy)
   * and create a new object that represents the same properties but as "local".
   */
...
protected:
  /**
   *  @brief Merge the parents "real" style with the child's (*this) local style
   * and store the style in the child's real style.
   *
  ...
  /**
   *  @brief Merge the parents "real" source with the child's (*this) source and
   * store the style in the child's real style.
   *
   ...
   */
  bool realize_needed_visual() const { return m_realize_needed_visual; }

  /**
   *  @brief Tell, if a realize of the source properties is needed
   */
  bool realize_needed_source() const { return m_realize_needed_source; }
  ...
  /**
   *  @brief Tells the children that a realize of the visual properties is
   * needed This is also the "forward sync" for the LayerPropertiesNodeRef.
   */
  ...
  /**
   *  @brief Fetches the current status from the original properties for the
...
protected:
  enum { nr_visual = 1, nr_source = 2, nr_meta = 4, nr_hierarchy = 8 };
 ...
 */
class LAYBASIC_PUBLIC LayerPropertiesNode : public LayerProperties,
                                            public tl::Object {
public:
   ...
   *  @brief Assignment alias for GSI binding
   */
  void assign(const LayerPropertiesNode &d) { operator=(d); }
   ...
   *  @brief Inequality
   */
  bool operator!=(const LayerPropertiesNode &d) const { return !operator==(d); }
   ...
   */
  LayerPropertiesNode &last_child() { return m_children.back(); }

  /**
   *  @brief Return a reference to the last child
   */
  const LayerPropertiesNode &last_child() const { return m_children.back(); }
   ...
   */
  const LayerPropertiesNode *parent() const { return mp_parent.get(); }
  ...
  void set_expanded_silent(bool ex) { m_expanded = ex; }
 ...
 */
class LAYBASIC_PUBLIC LayerPropertiesConstIterator : public tl::Object {
public:
  /**
   ...
   *  This requires the node pointer to point to a node that is addressable by
   * an iterator - i.e. lives within a view and is either a direct member or a
   * child of a member of a layer properties list. If this is not the case, the
   * resulting iterator will be a null iterator.
   */
  LayerPropertiesConstIterator(const lay::LayerPropertiesNode *node);

  /**
   *  @brief Constructor: create an iterator pointing either at the beginning or
   * at the end
   */
  LayerPropertiesConstIterator(const LayerPropertiesList &list,
                               bool last = false);
   ...
   */
  LayerPropertiesConstIterator &
  operator=(const LayerPropertiesConstIterator &d);
   ...
   *
   *  This predicate is true if the iterator is at the end of either all
   * elements or at the end of the child list (if down_next_child() or
   ...
   */
  bool is_null() const { return m_uint == 0; }
   ...
   */
  size_t uint() const { return m_uint; }
   ...
   */
  const LayerPropertiesList *list() const { return m_list.get(); }
...
struct LAYBASIC_PUBLIC CompareLayerIteratorBottomUp {
  bool operator()(const lay::LayerPropertiesConstIterator &a,
                  const lay::LayerPropertiesConstIterator &b) {
...
public:
   ...
   *  @brief Default constructor
   */
  LayerPropertiesIterator() : LayerPropertiesConstIterator() {
    // .. nothing yet ..
  }
  ...
  LayerPropertiesNode &operator*() const {
    return const_cast<LayerPropertiesNode &>(
        LayerPropertiesConstIterator::operator*());
  }
  ...
  std::pair<LayerPropertiesNode *, size_t> parent_obj() const {
    std::pair<const LayerPropertiesNode *, size_t> pp =
        LayerPropertiesConstIterator::parent_obj();
    return std::make_pair(const_cast<LayerPropertiesNode *>(pp.first),
                          pp.second);
  }
 ...
 */
class LAYBASIC_PUBLIC LayerPropertiesList : public tl::Object {
public:
   ...
   *  @brief Inequality
   */
  bool operator!=(const LayerPropertiesList &d) const { return !operator==(d); }
  ...
  /**
   *  @brief Remove all entries related to a certain cv index or not related to
   * a certain cv index
   *
   *  @param cv_index The index of the cellview whose references shall be
   * removed if except is false or the index of the cellview which shall be
   * retained if except is true
   */
   ...
   *
   *  This does not attach the view. It is provided to encapsulate the merging
   ...
   *  and past-the-end of the respective child list (stack.back
   * ().first==stack.back ().second). In this case, the element is inserted at
   * the end of the respective parent's child list. A reference to the element
   * created is returned.
   */
  LayerPropertiesNode &insert(const LayerPropertiesIterator &iter,
   ...
   */
  const lay::DitherPattern &dither_pattern() const { return m_dither_pattern; }
   ...
   */
  const lay::LineStyles &line_styles() const { return m_line_styles; }
  ...
  /**
   *  @brief Get the name of the layer properties set (for tabbed layer
   * properties)
   */
   ...
   */
  static void load(tl::XMLSource &stream,
   ...
   *  This method has the side effect of recomputing the layer source
   * parameters.
   */
  void attach_view(LayoutViewBase *view, unsigned int list_index);
 ...
 */
class LAYBASIC_PUBLIC LayerPropertiesNodeRef : public LayerPropertiesNode {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  This method returns null if the reference does not point to a specific
   * target. This method returns a const pointer since the target is not
   * supposed to be modified directly.
   */
  const LayerPropertiesNode *target() const { return mp_node.get(); }
