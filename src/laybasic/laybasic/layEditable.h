 ...
 */
class LAYBASIC_PUBLIC Editable : virtual public tl::Object {
public:
   ...
   */
  virtual db::DBox selection_bbox() { return db::DBox(); }
   ...
   *  @param mode The mode of the selection for which the closest object is
   * looked for.
   *  @return The distance in micron
   */
  virtual double click_proximity(const db::DPoint & /*pos*/,
                                 SelectionMode /*mode*/) {
   ...
   *
   *  The catch distance is a typical value for the "fuzzyness" of a mouse
   * click. It is given in micron.
   */
  virtual double catch_distance() { return 0.0; }

  /**
   *  @brief The catch distance (for box)
   *
   *  The catch distance is a typical value for the "fuzzyness" of a box
   * selection. It is given in micron.
   */
  virtual double catch_distance_box() { return 0.0; }
   ...
   */
  virtual bool transient_select(const db::DPoint & /*pos*/) { return false; }
   ...
   *
   *  This method is used by the single-point selection cycling protocol to
   * clear the plugin's single-point selection state. The cycling protocol is
   * used when a certain point is clicked at multiple times. A plugin is
   * supposed to remember such selections and exclude them from further checks.
   * If all objects in question are selected, no further object would be
   * selected. clear_previous_selection is called in that case to indicate that
   ...
   *  @return Should return true if anything was selected, in particular in
   * point mode
   */
   ...
   *
   *  The move operation can be requested in three modes: In "Any" mode, the
   * plugin can decide whether it wants to catch a move request and return true.
   *  In "Selected" mode, the plugin is supposed to move all selected objects.
   *  In "Partial" mode, the plugin is supposed to pick an object from the
   * selected ones and perform a move that modifies that object. In "Any" or
   * "Selected" mode, the plugin must return true to catch the selection. Only
   * one plugin will receive the selection.
   *
   ...
   *    1.) if anything is selected, call begin_move on every Editable with
   * sel=Partial. If a plugin returns true, the method returns. 2.) else if
   * anything is selected, call begin_move on every Editable with sel=Selected.
   *    3.) if nothing is selected, call begin_move on every Editable with
   * sel=Any. Stop if one of these returns true. 4.) if none returned true,
   * select pointwise around "p" and proceed as in 2.)
   *
   *  @param sel See above.
   *  @param p The point at which the mouse was clicked
   *  @param ac The angle constraint imposed (lay::AC_Global if no specific is
   * requested)
   *  @return See above.
   */
   ...
   * requested)
   */
  virtual void move(const db::DPoint & /*p*/,
                    lay::angle_constraint_type /*ac*/) {
    //  .. by default, nothing is implemented ..
  }
   ...
   *
   *  These events are sent whenever the right mouse button is clicked in order
   * to request a rotation of the content currently moved.
   *
   ...
   * requested)
   */
  virtual void move_transform(const db::DPoint & /*p*/, db::DFTrans /*tr*/,
                              lay::angle_constraint_type /*ac*/) {
    //  .. by default, nothing is implemented ..
  }
   ...
   * requested)
   */
  virtual void end_move(const db::DPoint & /*p*/,
                        lay::angle_constraint_type /*ac*/) {
    //  .. by default, nothing is implemented ..
  }
   ...
   */
  virtual bool has_selection() { return false; }

  /**
   *  @brief Indicates how many objects are selected
   */
  virtual size_t selection_size() { return 0; }

  /**
   *  @brief Indicates if any objects are selected in the transient selection
   */
  virtual bool has_transient_selection() { return false; }
   ...
   */
  virtual std::vector<lay::PropertiesPage *>
  properties_pages(db::Manager * /*manager*/, QWidget * /*parent*/) {
...
protected:
  Editables *editables() { return mp_editables; }
 ...
 */
class LAYBASIC_PUBLIC Editables : public db::Object {
public:
   ...
   *
   *  If a transaction is given, the operation will be appended to this pending
   * transaction The Editables object takes ownership over the Transaction
   * object.
   */
   ...
   *
   *  The transient selection is supposed to highlight the object which would be
   * scope of the next select or move operation. Since move is usually defined
   * to move the current selection, the transient selection should not be done
   * if the view is in move mode and something is selected. Exceptions are:
   * highlighting an object within the current selection.
   */
   ...
   *
   *  The previous selection is used to implement the cycling protocol for
   ...
   *
   *  This method will not do anything if there is no previous, click-at
   * selection.
   */
   ...
   */
  void move_transform(const db::DPoint &p, db::DFTrans tr,
                      lay::angle_constraint_type ac);
   ...
   */
  iterator begin() { return m_editables.begin(); }

  /**
   *  @brief Editable iterator: end
   */
  iterator end() { return m_editables.end(); }
   ...
   */
  virtual void signal_selection_changed() { selection_changed_event(); }
   ...
   *
   *  This method is called when the client code wishes to (temporarily) enable
   * to disable user functions that may cause an edit operation. This method can
   * be reimplemented to disable menu entries etc.
   */
...
protected:
   ...
   *
   *  This method can be overridden in order to implement special behaviour on
   * cancel of edits (i.e. release the mouse). Make sure, the base
   * implementation is called as well.
   */
