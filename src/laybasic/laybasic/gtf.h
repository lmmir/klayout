...
public:
   ...
   *   @brief Constructor
   */
  LogEventBase(int xml_line) : m_xml_line(xml_line) {
    //  .. nothing yet ..
  }
   ...
   *
   *  Spontaneous events are created internally rather than in response to a
   * user action. These are recorded for playback purposes but not compared to
   * other spontaneous ones.
   */
  virtual bool spontaneous() const { return false; }
   ...
   *  @brief Inequality
   */
  bool operator!=(const LogEventBase &b) const { return !operator==(b); }

  /**
   *  @brief Get the line corresponding to the XML file where the element is
   * stored
   */
  int xml_line() const { return m_xml_line; }

  /**
   *  @brief Access the data object
   */
  const tl::Variant &data() const { return m_data; }

  /**
   *  @brief Set the data object
   */
  void set_data(const tl::Variant &d) { m_data = d; }
 ...
 */
class ActionInterceptor : public QObject {
  Q_OBJECT
public:
...
public:
   ...
   */
  const gtf::LogEventBase *back() const { return m_events.back(); }

  /**
   *  @brief Last element of the list (non-const version)
   */
  gtf::LogEventBase *back() { return m_events.back(); }

  /**
   *  @brief pop an element
   */
  void pop_back() { m_events.pop_back(); }
   ...
   */
  unsigned int size() const { return int(m_events.size()); }
   ...
   */
  gtf::LogEventBase *operator[](unsigned int index) { return m_events[index]; }
   ...
   */
  void add(gtf::LogEventBase *e) { m_events.push_back(e); }
 ...
 */
class LAYBASIC_PUBLIC Player : public QObject {
  Q_OBJECT
public:
  /**
   ...
   *
   *  This method starts replaying the events loaded formerly. The rate by which
   * the events are played is given by the "ms" argument, which tells how many
   *  milliseconds to wait before the next event is issued.
   *  The method will return immediately. QApplication::exec() will be required
   * in order to actually replay the events.
   */
   ...
   *
   *  Returns true, if the player is actually playing, i.e. if replay has been
   * issued and there are still events to issue.
   */
  bool playing() const { return m_playing_active; }
   ...
   */
  QEvent *event_issued() const { return mp_event_issued; }

  /**
   *  @brief Tell, to which widget the event was actually issued by the Player
   *
   *  Returns a reference to the widget to which the recent event was issued by
   * the Player. It returns 0 if no event was issued by the player.
   */
  QWidget *event_target() const { return mp_event_target; }
   ...
   */
  static Player *instance() { return ms_instance; }
 ...
 */
class LAYBASIC_PUBLIC Recorder : public QObject {
public:
  /**
   ...
   *
   *  Probe statements are used to express a window's property in form of a
   * tl::Variant.
   */
   ...
   *
   *  This provides probing implementation for some standard widgets such as
   * line edit etc.
   */
   ...
   *
   *  After issueing this method with a "true" argument, the recorder saves the
   * events to the given file each time a new event is recorded. This mode may
   * be useful to record actions in test mode. If a crash happens, a log exists
   * that records the events up to that point.
   */
   ...
   *  If the file cannot be written, an exception is thrown
   */
  void save() { m_events.save(m_log_file); }
   ...
   */
  static Recorder *instance() { return ms_instance; }
...
/**
 *  @brief special connect method that replaces the original connect for logged
 * actions
 *
 *  This method can be used instead of a usual "connect" of a logged action's
 * trigger signal with a corresponding slot of a receiver object. The purpose is
 * to install an interceptor in case GTF is enabled. This will perform logging
 * and replay.
 */
LAYBASIC_PUBLIC void action_connect(QAction *action, const char *signal,
                                    QObject *receiver, const char *slot);

/**
 *  @brief special disconnect method that must be used to disconnect connections
 * that were build using action_connect
 */
LAYBASIC_PUBLIC void action_disconnect(QAction *action, const char *signal,
                                       QObject *receiver, const char *slot);
