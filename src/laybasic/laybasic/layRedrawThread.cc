...
RedrawThread::RedrawThread(lay::RedrawThreadCanvas *canvas,
                           LayoutViewBase *view)
    : tl::Object() {
...
std::vector<db::DBox> subtract_box(const db::DBox &subject,
                                   const db::DBox &with) {
  ...
  inverted.reserve(4);
  inverted.push_back(
      db::DBox(db::DPoint(-lim, -lim), db::DPoint(lim, with.bottom())));
  inverted.push_back(
      db::DBox(db::DPoint(-lim, with.top()), db::DPoint(lim, lim)));
  inverted.push_back(db::DBox(db::DPoint(-lim, with.bottom()),
                              db::DPoint(with.left(), with.top())));
  inverted.push_back(db::DBox(db::DPoint(with.right(), with.bottom()),
                              db::DPoint(lim, with.top())));

  for (std::vector<db::DBox>::const_iterator i = inverted.begin();
       i != inverted.end(); ++i) {
...
void RedrawThread::commit(const std::vector<lay::RedrawLayerInfo> &layers,
  ...
  db::DBox new_region =
      m_vp_trans.inverted() *
      db::DBox(db::DPoint(0, 0), db::DPoint(m_width, m_height));
...
void RedrawThread::start(int workers,
  ...
  if (!force_redraw && mp_canvas->shift_supported() &&
      ...
      m_stored_fp == m_vp_trans.fp_trans() &&
      fabs(new_region.width() - m_stored_region.width()) < epsilon &&
      fabs(new_region.height() - m_stored_region.height()) < epsilon) {
    ...
    for (std::vector<db::DBox>::const_iterator p = parts.begin();
         p != parts.end(); ++p) {
      if (p->width() > epsilon && p->height() > epsilon) {
        db::Box rr =
            db::Box((m_vp_trans * *p)
                        .enlarged(db::DVector(1.0, 1.0) /*safety overlap*/));
        rr &= full;
        if (!rr.empty()) {
    ...
    sv = db::Vector(m_vp_trans *
                    db::DVector(m_last_center - new_region.center()));
    shift_vector = &sv;
    ...
    //  no, we can't ...
    m_redraw_regions.clear();
    m_redraw_regions.push_back(
        db::Box(db::Point(0, 0), db::Point(m_width, m_height)));
...
void RedrawThread::do_start(bool clear, const db::Vector *shift_vector,
                            const std::vector<lay::RedrawLayerInfo> *layers,
                            const std::vector<int> &restart, int nworkers) {
  ...
  //  Recursion however is forbidden because of the deadlocking locks used
  //  originally to lock out the redraw thread.
...

  {
    ...
    tl::SelfTimer timer(tl::verbosity() >= 41,
                        tl::to_string(tr("Preparing to draw")));
    ...
    for (unsigned int i = 0; i < mp_view->cellviews(); ++i) {
      ...
      if (cv.is_valid() && !cv->layout().under_construction() &&
        ...
        cv->layout().update();
        //  attach to the layout object to receive change notifications to stop
        //  the redraw thread
        cv->layout().hier_changed_event.add(this,
                                            &RedrawThread::layout_changed);
        cv->layout().bboxes_changed_any_event.add(
            this, &RedrawThread::layout_changed);
      }
    }
    mp_view->annotation_shapes().update();
    //  attach to the layout object to receive change notifications to stop the
    //  redraw thread
    mp_view->annotation_shapes().hier_changed_event.add(
        this, &RedrawThread::layout_changed); //  not really required, since the
                                              //  shapes have no hierarchy, but
                                              //  for completeness ..
    mp_view->annotation_shapes().bboxes_changed_any_event.add(
        this, &RedrawThread::layout_changed);
    mp_view->cellviews_about_to_change_event.add(this,
                                                 &RedrawThread::layout_changed);
    mp_view->cellview_about_to_change_event.add(
        this, &RedrawThread::layout_changed_with_int);
    ...
    if (mp_view->cellviews() > 0) {
      ...
      if (clear) {
        ...
        mp_canvas->prepare(m_nlayers * planes_per_layer +
                               special_planes_before + special_planes_after,
                           m_width, m_height, m_resolution, shift_vector, 0,
                           mp_view->drawings());
        m_boxes_already_drawn = false;
        m_custom_already_drawn = false;
        ...
        for (std::vector<int>::const_iterator l = restart.begin();
             l != restart.end(); ++l) {
          if (*l == draw_custom_queue_entry) {
            planes_to_init.push_back(-1);
          } else if (*l >= 0 && *l < int(m_layers.size())) {
            for (int i = 0; i < planes_per_layer / 3; ++i) {
              planes_to_init.push_back(*l * (planes_per_layer / 3) +
                                       special_planes_before + i);
              planes_to_init.push_back((*l + m_nlayers) *
                                           (planes_per_layer / 3) +
                                       special_planes_before + i);
              planes_to_init.push_back((*l + m_nlayers * 2) *
                                           (planes_per_layer / 3) +
                                       special_planes_before + i);
            }
          }
        ...
        mp_canvas->prepare(m_nlayers * planes_per_layer +
                               special_planes_before + special_planes_after,
                           m_width, m_height, m_resolution, shift_vector,
                           &planes_to_init, mp_view->drawings());
      ...
      mp_canvas->prepare(1, m_width, m_height, m_resolution, 0, 0,
                         mp_view->drawings());
    }
  }
  ...
  //  Don't wait on restart - that happens while a drawing is under way which
  //  was interrupted. Waiting is not necessary in this case and blocks the
  //  application.
...
void RedrawThread::start() { JobBase::start(); }
...
void RedrawThread::setup_worker(tl::Worker *worker) {
  RedrawThreadWorker *redraw_thread_worker =
      dynamic_cast<RedrawThreadWorker *>(worker);
  if (redraw_thread_worker) {
    redraw_thread_worker->setup(mp_view, mp_canvas, m_redraw_regions,
                                m_vp_trans);
  }
}

void RedrawThread::stopped() {
  // because we may have already shifted, we can only reuse the part that was
  // inside the new viewport
  m_stored_region = m_vp_trans.inverted() *
                    db::DBox(db::DPoint(0, 0), db::DPoint(m_width, m_height));
...
void RedrawThread::finished() {
  m_stored_region = m_valid_region =
      m_vp_trans.inverted() *
      db::DBox(db::DPoint(0, 0), db::DPoint(m_width, m_height));
  m_stored_fp = m_vp_trans.fp_trans();
