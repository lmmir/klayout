...
/**
 *  @brief A class encapsulating an instantiation path and the element addressed
 * by it
 *
 *  This class either addresses an instance (in which case the path is that to
 * the instance addressed) or a shape (in which case the path leads to the cell
 * that has this shape and the layer and shape is specified additionally).
 */
...
public:
   ...
   */
  void set_cv_index(unsigned int cv_index) { m_cv_index = cv_index; }

  /**
   *  @brief Set the topcell
   */
  void set_topcell(db::cell_index_type topcell) { m_topcell = topcell; }
   ...
   */
  void add_path(const db::InstElement &elem) { m_path.push_back(elem); }

  /**
   *  @brief Add the given sequence of elements to the instantiation path
   */
  template <class Iter> void add_path(Iter first, Iter last) {
   ...
   */
  template <class Iter> void assign_path(Iter first, Iter last) {
  ...
  /**
   *  @brief Obtain the index of the target cell, including the instantiated
   * cell, if the path describes an instance
   */
   ...
   *  @brief Set the layer
   *
   *  Setting a layer explicitly makes this path point to a shape (which has to
   * be specified additionally).
   */
  void set_layer(unsigned int layer) { m_layer = layer; }
   ...
   */
  void set_shape(const db::Shape &shape) { m_shape = shape; }
  ...
  /**
   *  @brief Obtain the combined transformation for this instantiation path
   * including the instance transformation, if the path describes an instance
   */
  db::ICplxTrans trans_tot() const;

  /**
   *  @brief Return true, if this selection represents a cell instance, not a
   * shape
   */
  bool is_cell_inst() const { return m_layer < 0; }
  ...
  /**
   *  @brief Insert the given element at the front setting the top cell to a new
   * one
   */
   ...
   */
  non_const_iterator begin() { return m_path.begin(); }

  /**
   *  @brief The "end" iterator for the path
   */
  non_const_iterator end() { return m_path.end(); }

  /**
   *  @brief The "begin" iterator for the path
   */
  iterator begin() const { return m_path.begin(); }

  /**
   *  @brief The "end" iterator for the path
   */
  iterator end() const { return m_path.end(); }

  /**
   *  @brief The "back" element of the path
   */
  db::InstElement &back() { return m_path.back(); }

  /**
   *  @brief The "back" element of the path
   */
  const db::InstElement &back() const { return m_path.back(); }
   ...
   *  @brief Inequality
   */
  bool operator!=(const ObjectInstPath &d) const { return !operator==(d); }
   ...
   */
  unsigned long seq() const { return m_seq; }

  /**
   *  @brief Assign a sequence number
   */
  void set_seq(unsigned long s) { m_seq = s; }
