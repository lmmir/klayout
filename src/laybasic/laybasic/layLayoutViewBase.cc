...
// Enable this if you have both Qt and libpng and want to use libpng for saving
// images:
...
struct OpHideShowCell : public db::Op {
...
struct OpSetDitherPattern : public db::Op {
...
struct OpSetLineStyles : public db::Op {
...
struct OpSetLayerProps : public db::Op {
  OpSetLayerProps(unsigned int li, unsigned int i,
                  const lay::LayerProperties &o, const lay::LayerProperties &n)
...
struct OpSetLayerPropsNode : public db::Op {
  OpSetLayerPropsNode(unsigned int li, unsigned int i,
                      const lay::LayerPropertiesNode &o,
                      const lay::LayerPropertiesNode &n)
      : m_list_index(li), m_index(i), m_old(o), m_new(n) {
    //  .. nothing yet ..
  }
...
struct OpDeleteLayerList : public db::Op {
...
struct OpInsertLayerList : public db::Op {
...
struct OpRenameProps : public db::Op {
  OpRenameProps(unsigned int li, const std::string &old_name,
...
struct OpSetAllProps : public db::Op {
  OpSetAllProps(unsigned int li, const lay::LayerPropertiesList &o,
                const lay::LayerPropertiesList &n)
...
struct OpLayerList : public db::Op {
  enum Mode { Delete, Insert };

  OpLayerList(unsigned int li, unsigned int i,
              const lay::LayerPropertiesNode &n, Mode m)
...
struct OpInsertLayerProps : public OpLayerList {
  OpInsertLayerProps(unsigned int li, unsigned int i,
                     const lay::LayerPropertiesNode &n)
...
struct OpDeleteLayerProps : public OpLayerList {
  OpDeleteLayerProps(unsigned int li, unsigned int i,
                     const lay::LayerPropertiesNode &n)
...
LayoutViewBase::LayoutViewBase(db::Manager *manager, bool editable,
                               lay::Plugin *plugin_parent, unsigned int options)
    : lay::Dispatcher(plugin_parent, false /*not standalone*/), mp_ui(0),
      dm_redraw(this, &LayoutViewBase::redraw), m_editable(editable),
      m_options(options), m_annotation_shapes(manager) {
...
LayoutViewBase::LayoutViewBase(lay::LayoutView *ui, db::Manager *manager,
                               bool editable, lay::Plugin *plugin_parent,
                               unsigned int options)
    : lay::Dispatcher(plugin_parent, false /*not standalone*/), mp_ui(ui),
      dm_redraw(this, &LayoutViewBase::redraw), m_editable(editable),
      m_options(options), m_annotation_shapes(manager) {
  //  either it's us or the parent has a dispatcher
  tl_assert(dispatcher() != 0);
...
void LayoutViewBase::copy_from(lay::LayoutViewBase *source) {
  ...
  for (size_t i = 0; i < source->m_layer_properties_lists.size(); ++i) {
    if (i >= m_layer_properties_lists.size()) {
      m_layer_properties_lists.push_back(
          new lay::LayerPropertiesList(*source->m_layer_properties_lists[i]));
    } else {
  ...
  if (!m_layer_properties_lists.empty()) {
    mp_canvas->set_dither_pattern(
        m_layer_properties_lists[0]->dither_pattern());
...
void LayoutViewBase::init(db::Manager *mgr) {
  ...
  m_layer_properties_lists.back()->attach_view(
      this, (unsigned int)(m_layer_properties_lists.size() - 1));
...
void LayoutViewBase::finish() {
  //  if we're the root dispatcher initialize the menu and build the context
  //  menus. No other menus are built so far.
  if (dispatcher() == this) {
    init_menu();
  }
}
...
void LayoutViewBase::init_menu() {
  ...
  //  if not in editable mode, hide all entries from "edit_mode" group and show
  //  all from the "view_mode" group and vice versa
...
void LayoutViewBase::shutdown() {
  ...
  //  This is to prevent signals to partially destroyed observers that own a
  //  LayoutViewBase
  ...
  //  detach ourselves from any observed objects to prevent signals while
  //  destroying
  ...
  for (std::vector<LayerPropertiesList *>::iterator l =
           layer_properties_lists.begin();
       l != layer_properties_lists.end(); ++l) {
    if (*l) {
      delete *l;
    }
  }
  ...
  for (std::vector<lay::Plugin *>::iterator p = plugins.begin();
       p != plugins.end(); ++p) {
    delete *p;
  }
...
void LayoutViewBase::unregister_plugin(lay::Plugin *pi) {
  for (std::vector<lay::Plugin *>::iterator p = mp_plugins.begin();
       p != mp_plugins.end(); ++p) {
    if (pi == *p) {
      mp_plugins.erase(p);
      break;
...
void LayoutViewBase::update_event_handlers() {
  ...
  for (std::vector<lay::Plugin *>::iterator p = mp_plugins.begin();
       p != mp_plugins.end(); ++p) {
    //  TODO: get rid of the const_cast hack
    const_cast<lay::PluginDeclaration *>((*p)->plugin_declaration())
        ->editable_enabled_changed_event.add(
            this, &LayoutViewBase::signal_plugin_enabled_changed);
  }

  for (unsigned int i = 0; i < cellviews(); ++i) {
    cellview(i)->layout().hier_changed_event.add(
        this, &LayoutViewBase::signal_hier_changed);
    cellview(i)->layout().bboxes_changed_event.add(
        this, &LayoutViewBase::signal_bboxes_from_layer_changed, i);
    cellview(i)->layout().dbu_changed_event.add(
        this, &LayoutViewBase::signal_bboxes_changed);
    cellview(i)->layout().prop_ids_changed_event.add(
        this, &LayoutViewBase::signal_prop_ids_changed);
    cellview(i)->layout().layer_properties_changed_event.add(
        this, &LayoutViewBase::signal_layer_properties_changed);
    cellview(i)->layout().cell_name_changed_event.add(
        this, &LayoutViewBase::signal_cell_name_changed);
    cellview(i)->apply_technology_with_sender_event.add(
        this, &LayoutViewBase::signal_apply_technology);
  }

  annotation_shapes().bboxes_changed_any_event.add(
      this, &LayoutViewBase::signal_annotations_changed);

  mp_canvas->viewport_changed_event.add(this,
                                        &LayoutViewBase::viewport_changed);
  ...
  mp_canvas->left_arrow_key_pressed_with_shift.add(
      this, &LayoutViewBase::pan_left_fast);
  mp_canvas->up_arrow_key_pressed_with_shift.add(this,
                                                 &LayoutViewBase::pan_up_fast);
  mp_canvas->right_arrow_key_pressed_with_shift.add(
      this, &LayoutViewBase::pan_right_fast);
  mp_canvas->down_arrow_key_pressed_with_shift.add(
      this, &LayoutViewBase::pan_down_fast);
}

void LayoutViewBase::viewport_changed() { viewport_changed_event(); }

bool LayoutViewBase::accepts_drop(const std::string &path_or_url) const {
  for (std::vector<lay::Plugin *>::const_iterator p = mp_plugins.begin();
       p != mp_plugins.end(); ++p) {
...
void LayoutViewBase::create_plugins(const lay::PluginDeclaration *except_this) {
  ...
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
    ...
    if (&*cls != except_this) {
      ...
      if (cls.current_name() == "ant::Plugin" ||
          cls.current_name() == "img::Plugin") {
        //  ant and img are created always
        create_plugin(&*cls);
      } else if ((options() & LV_NoPlugins) == 0) {
        //  others: only create unless LV_NoPlugins is set
        create_plugin(&*cls);
      } else if ((options() & LV_NoGrid) == 0 &&
                 cls.current_name() == "GridNetPlugin") {
...
lay::Plugin *LayoutViewBase::create_plugin(const lay::PluginDeclaration *cls) {
  ...
  if (p) {
    ...
    //  unhook the plugin from the script side if created there (prevent GC from
    //  destroying it)
...
Plugin *LayoutViewBase::get_plugin_by_name(const std::string &name) const {
  ...
  for (tl::Registrar<lay::PluginDeclaration>::iterator cls =
           tl::Registrar<lay::PluginDeclaration>::begin();
       !decl && cls != tl::Registrar<lay::PluginDeclaration>::end(); ++cls) {
...
void LayoutViewBase::set_synchronous(bool s) { m_synchronous = s; }

void LayoutViewBase::message(const std::string & /*s*/, int /*timeout*/) {
  //  .. nothing yet ..
}

bool LayoutViewBase::is_dirty() const { return m_dirty; }

std::string LayoutViewBase::title() const {
  if (!m_title.empty()) {
    return m_title;
  } else if (cellviews() == 0) {
...
bool LayoutViewBase::configure(const std::string &name,
...
void LayoutViewBase::set_line_styles(const lay::LineStyles &styles) {
  if (mp_canvas->line_styles() != styles) {
    ...
    if (transacting()) {
      manager()->queue(this,
                       new OpSetLineStyles(mp_canvas->line_styles(), styles));
...
void LayoutViewBase::set_dither_pattern(const lay::DitherPattern &pattern) {
  if (mp_canvas->dither_pattern() != pattern) {
    ...
    if (transacting()) {
      manager()->queue(
          this, new OpSetDitherPattern(mp_canvas->dither_pattern(), pattern));
    } else if (manager() && !replaying()) {
      manager()->clear();
    }
...
void LayoutViewBase::insert_layer_list(unsigned index,
                                       const LayerPropertiesList &props) {
  if (index > layer_lists()) {
    return;
  }
  ...
  m_layer_properties_lists.insert(m_layer_properties_lists.begin() + index,
                                  new LayerPropertiesList(props));
...
void LayoutViewBase::delete_layer_list(unsigned index) {
  ...
  if (transacting()) {
    manager()->queue(
        this, new OpDeleteLayerList(index, *m_layer_properties_lists[index]));
  } else if (manager() && !replaying()) {
    manager()->clear();
  }
...
void LayoutViewBase::rename_properties(unsigned int index,
                                       const std::string &new_name) {
  if (index >= layer_lists()) {
    return;
  }
  ...
  if (transacting()) {
    manager()->queue(
        this, new OpRenameProps(index, m_layer_properties_lists[index]->name(),
                                new_name));
  } else if (manager() && !replaying()) {
    manager()->clear();
  }
...
bool LayoutViewBase::set_current_layer(unsigned int cv_index,
                                       const db::LayerProperties &lp) {
  //  rename the ones that got shifted.
  lay::LayerPropertiesConstIterator l = begin_layers();
  while (!l.at_end()) {
    if (l->source(true).cv_index() == int(cv_index) &&
        l->source(true).layer_props().log_equal(lp)) {
      set_current_layer(l);
      return true;
    }
    ++l;
...
void LayoutViewBase::set_current_layer(
...
lay::LayerPropertiesConstIterator LayoutViewBase::current_layer() const {
...
void LayoutViewBase::set_selected_layers(
    const std::vector<lay::LayerPropertiesConstIterator> &sel) {
 ...
 */
static void single_bitmap_to_image(const lay::ViewOp &view_op,
                                   lay::Bitmap &bitmap, tl::PixelBuffer *pimage,
                                   const lay::DitherPattern &dither_pattern,
                                   const lay::LineStyles &line_styles,
                                   double dpr, unsigned int width,
                                   unsigned int height) {
  ...
  lay::bitmaps_to_image(view_ops, pbitmaps, dither_pattern, line_styles, dpr,
                        pimage, width, height, false, 0);
}

tl::PixelBuffer
LayoutViewBase::icon_for_layer(const LayerPropertiesConstIterator &iter,
                               unsigned int w, unsigned int h, double dpr,
                               unsigned int di_off, bool no_state) {
  if (dpr < 0.0) {
    dpr = canvas()->dpr();
  }
  ...
  tl::color_t fill_color =
      iter->has_fill_color(true) ? iter->eff_fill_color(true) : def_color;
  tl::color_t frame_color =
      iter->has_frame_color(true) ? iter->eff_frame_color(true) : def_color;
  ...
  //  create fill
  single_bitmap_to_image(
      lay::ViewOp(fill_color, mode, 0, iter->eff_dither_pattern(true), di_off),
      fill, &image, dither_pattern(), line_styles(), dpr_drawing, w, h);
  //  create frame
  if (lw == 0) {
    single_bitmap_to_image(
        lay::ViewOp(frame_color, mode, 0 /*solid line*/, 2 /*dotted*/, 0),
        frame, &image, dither_pattern(), line_styles(), dpr_drawing, w, h);
  } else {
    single_bitmap_to_image(
        lay::ViewOp(frame_color, mode, iter->eff_line_style(true), 0, 0,
                    lay::ViewOp::Rect, lw),
        frame, &image, dither_pattern(), line_styles(), dpr_drawing, w, h);
  }
  //  create text
  single_bitmap_to_image(lay::ViewOp(frame_color, mode, 0, 0, 0), text, &image,
                         dither_pattern(), line_styles(), dpr_drawing, w, h);
  //  create vertex
  single_bitmap_to_image(
      lay::ViewOp(frame_color, mode, 0, 0, 0, lay::ViewOp::Cross,
                  iter->marked(true) ? int(9 * dpr_drawing + 0.5) : 0),
      vertex, &image, dither_pattern(), line_styles(), dpr_drawing, w, h);

  if (oversampling > 1) {
    tl::PixelBuffer subsampled(image.width() / oversampling,
                               image.height() / oversampling);
...
void LayoutViewBase::merge_dither_pattern(lay::LayerPropertiesList &props) {
  {
    ...
    for (lay::LayerPropertiesIterator l = props.begin_recursive();
         l != props.end_recursive(); ++l) {
      int dpi = l->dither_pattern(false /*local*/);
      std::map<unsigned int, unsigned int>::iterator m =
          index_map.find((unsigned int)dpi);
      if (m != index_map.end()) {
...

  {
    ...
    for (lay::LayerPropertiesIterator l = props.begin_recursive();
      ...
      std::map<unsigned int, unsigned int>::iterator m =
          index_map.find((unsigned int)lsi);
      if (m != index_map.end()) {
...
bool LayoutViewBase::always_show_source() const { return false; }

bool LayoutViewBase::always_show_ld() const { return true; }

bool LayoutViewBase::always_show_layout_index() const { return false; }

void LayoutViewBase::set_properties(unsigned int index,
                                    const LayerPropertiesList &props) {
  //  If index is not a valid tab index, don't do anything except for the case
  //  of index 0 in which the first entry is created (this can happen as a
  //  result of delete_properties).
  if (index >= layer_lists()) {
    if (index > 0) {
      return;
    } else {
  ...
  //  Since it has some desired side effects such as forcing a recomputation of
  //  the internals, it should be executed in any case, even if props ==
  //  get_properties ().
...
void LayoutViewBase::expand_properties(const std::map<int, int> &map_cv_index,
                                       bool add_default) {
  for (unsigned int i = 0; i < cellviews(); ++i) {
...
void LayoutViewBase::expand_properties(unsigned int index,
                                       const std::map<int, int> &map_cv_index,
                                       bool add_default) {
...
void LayoutViewBase::replace_layer_node(
    unsigned int index, const LayerPropertiesConstIterator &iter,
    const LayerPropertiesNode &node) {
  if (index >= layer_lists()) {
    return;
  }
  ...
  if (*iter != node) {
    ...
    if (transacting()) {
      manager()->queue(this,
                       new OpSetLayerPropsNode(index, (unsigned int)iter.uint(),
                                               *iter, node));
    } else if (manager() && !replaying()) {
      manager()->clear();
    }
...
void LayoutViewBase::set_layer_node_expanded(
    unsigned int index, const LayerPropertiesConstIterator &iter, bool ex) {
  if (ex != iter->expanded()) {
...
void LayoutViewBase::set_properties(unsigned int index,
  ...
  if (l != props) {
    ...
    if (transacting()) {
      manager()->queue(this, new OpSetLayerProps(
                                 index, (unsigned int)iter.uint(), l, props));
    } else if (manager() && !replaying()) {
      manager()->clear();
    }

    bool need_redraw =
        (l.source(false /*local*/) != props.source(false /*local*/) ||
         l.xfill(false /*local*/) != props.xfill(false /*local*/));
    bool visible_changed =
        (l.visible(true /*real*/) != props.visible(true /*real*/));
    ...
    *non_const_iter = props;
    ...
    if (index == current_layer_list()) {
      ...
      //  perform the callbacks asynchronously to collect the necessary calls
...
LayoutViewBase::insert_layer(unsigned int index,
                             const LayerPropertiesConstIterator &before,
                             const LayerPropertiesNode &node) {
  tl_assert(index < layer_lists());

  if (transacting()) {
    manager()->queue(
        this, new OpInsertLayerProps(index, (unsigned int)before.uint(), node));
  } else if (manager() && !replaying()) {
    manager()->clear();
  }
  ...
  const LayerPropertiesNode &ret = m_layer_properties_lists[index]->insert(
      LayerPropertiesIterator(*m_layer_properties_lists[index], before.uint()),
      node);
...
void LayoutViewBase::delete_layer(unsigned int index,
                                  LayerPropertiesConstIterator &iter) {
  if (index >= layer_lists()) {
    return;
  }
  ...
  //  delete the element
  m_layer_properties_lists[index]->erase(
      LayerPropertiesIterator(*m_layer_properties_lists[index], iter.uint()));

  if (transacting()) {
    manager()->queue(
        this, new OpDeleteLayerProps(index, (unsigned int)iter.uint(), orig));
  } else if (manager() && !replaying()) {
    manager()->clear();
  }
...
void LayoutViewBase::save_as(unsigned int index, const std::string &filename,
                             tl::OutputStream::OutputStreamMode om,
                             const db::SaveLayoutOptions &options, bool update,
                             int keep_backups) {
    ...
    if (sop->m_list_index < m_layer_properties_lists.size()) {
      set_properties(
          ...
          lay::LayerPropertiesConstIterator(
              *m_layer_properties_lists[sop->m_list_index], sop->m_index),
          sop->m_new);
    }
    return;
  }
  ...
  if (snop) {
    if (snop->m_list_index < m_layer_properties_lists.size()) {
      replace_layer_node(
          snop->m_list_index,
          lay::LayerPropertiesConstIterator(
              *m_layer_properties_lists[snop->m_list_index], snop->m_index),
          snop->m_new);
    }
    return;
  }
    ...
    if (lop->m_list_index < m_layer_properties_lists.size()) {
      if (lop->m_mode == OpLayerList::Insert) {
        insert_layer(
            lop->m_list_index,
            lay::LayerPropertiesConstIterator(
                *m_layer_properties_lists[lop->m_list_index], lop->m_index),
            lop->m_node);
      } else {
...
void LayoutViewBase::signal_bboxes_from_layer_changed(
    unsigned int cv_index, unsigned int layer_index) {
  if (layer_index == std::numeric_limits<unsigned int>::max()) {
    ...
    for (std::vector<lay::RedrawLayerInfo>::const_iterator l =
             mp_canvas->get_redraw_layers().begin();
         l != mp_canvas->get_redraw_layers().end(); ++l) {
      if (l->cellview_index == int(cv_index) &&
          l->layer_index == int(layer_index)) {
        redraw_layer(
            (unsigned int)(l - mp_canvas->get_redraw_layers().begin()));
      }
    }
...
void LayoutViewBase::signal_cell_name_changed() {
  cell_visibility_changed_event(); // HINT: that is not what actually is
                                   // intended, but it serves the function ...
  redraw_later();                  //  needs redraw
}

void LayoutViewBase::signal_layer_properties_changed() {
  //  recompute the source
  //  TODO: this is a side effect of this method - provide a special method for
  //  this purpose
  for (unsigned int i = 0; i < layer_lists(); ++i) {
    m_layer_properties_lists[i]->attach_view(this, i);
  }

  //  schedule a redraw request - since the layer views might not have changed,
  //  this is necessary
  redraw_later();
}

void LayoutViewBase::signal_prop_ids_changed() {
  //  inform the layer list observers that they need to recompute the property
  //  selectors
  layer_list_changed_event(1);
  ...
  for (std::vector<lay::Plugin *>::iterator p = mp_plugins.begin();
    ...
    if ((*p)->editable_interface()) {
      enable((*p)->editable_interface(),
             (*p)->plugin_declaration()->editable_enabled());
    }
  }
}
...
std::list<lay::CellView>::iterator LayoutViewBase::cellview_iter(int cv_index) {
...
void LayoutViewBase::erase_cellview(unsigned int index) {
  ...
  if (m_current_cell_per_cellview.size() > index) {
    m_current_cell_per_cellview.erase(m_current_cell_per_cellview.begin() +
                                      index);
  }
  ...
  for (unsigned int lindex = 0; lindex < layer_lists(); ++lindex) {
    ...
    while (!l.at_end()) {
      ...
      if (source.cv_index() >= int(index)) {
        ...
        source.cv_index(
            source.cv_index() == int(index) ? -1 : source.cv_index() - 1);
        new_props.set_source(source);
        LayerPropertiesIterator non_const_iter(
            *m_layer_properties_lists[lindex], l.uint());
...
const CellView &LayoutViewBase::cellview(unsigned int index) const {
...
CellViewRef LayoutViewBase::cellview_ref(unsigned int index) {
  if (index >= m_cellviews.size()) {
    return CellViewRef();
  } else {
...
int LayoutViewBase::index_of_cellview(const lay::CellView *cv) const {
  int index = 0;
  for (std::list<CellView>::const_iterator i = m_cellviews.begin();
       i != m_cellviews.end(); ++i, ++index) {
    if (cv == i.operator->()) {
      return index;
    }
  }
...
void LayoutViewBase::set_layout(const lay::CellView &cv, unsigned int cvindex) {
  ...
  *cellview_iter(cvindex) = cv;
  ...
  //  since the hierarchy panel may hold cellviews, we explicitly request an
  //  initialization of the tree. This will release such references. This way,
  //  set_layout guarantees that the layouts are released as far as possible.
  //  This is important for reload () for example.
  ...
  for (unsigned int i = 0; i < cellviews(); ++i) {
    ...
    if (cellview(i).handle() == layout_handle) {
      ...
      const db::Technology *tech =
          db::Technologies::instance()->technology_by_name(
              cellview(i)->tech_name());
...
void LayoutViewBase::bookmark_view(const std::string &name) {
  DisplayState state(box(), get_min_hier_levels(), get_max_hier_levels(),
                     cellview_list());
...
bool LayoutViewBase::is_single_cv_layer_properties_file(const std::string &fn) {
  //  If the file contains information for a single layout but we have multiple
  //  ones, show the dialog to determine what layout to apply the information
  //  to.
  ...
  std::set<int> cv;
  for (std::vector<lay::LayerPropertiesList>::const_iterator p = props.begin();
       p != props.end(); ++p) {
    for (lay::LayerPropertiesConstIterator lp = p->begin_const_recursive();
         !lp.at_end(); ++lp) {
      if (!lp->has_children()) {
...
void LayoutViewBase::load_layer_props(const std::string &fn, int cv_index,
                                      bool add_default) {
  do_load_layer_props(fn, true, cv_index, add_default);
}

void LayoutViewBase::do_load_layer_props(const std::string &fn, bool map_cv,
                                         int cv_index, bool add_default) {
  std::vector<lay::LayerPropertiesList> props;
  ...
  for (std::vector<lay::LayerPropertiesList>::iterator p = props.begin();
       p != props.end(); ++p) {
...
void LayoutViewBase::add_new_layers(const std::vector<unsigned int> &layer_ids,
                                    int cv_index) {
  if (cv_index >= 0 && cv_index < int(cellviews())) {
    ...
    //  don't create new layers for those, for which there are layers already:
    //  compute a set of layers already present
    std::set<db::LayerProperties, db::LPLogicalLessFunc> present_layers;
    for (LayerPropertiesConstIterator lay_iter = begin_layers();
         !lay_iter.at_end(); ++lay_iter) {
    ...
    for (std::vector<unsigned int>::const_iterator l = layer_ids.begin();
         l != layer_ids.end(); ++l) {
    ...
    for (std::vector<db::LayerProperties>::const_iterator l =
             new_layers.begin();
         l != new_layers.end(); ++l) {
...
void LayoutViewBase::init_layer_properties(
    LayerProperties &p, const LayerPropertiesList &lp_list) const {
  tl::color_t c = 0;
  if (m_palette.luminous_colors() > 0) {
    c = m_palette.luminous_color_by_index(
        p.source(true /*real*/).color_index());
  }

  p.set_dither_pattern(m_stipple_palette.standard_stipple_by_index(
      lp_list.end_const() - lp_list.begin_const()));
  p.set_fill_color(c);
  p.set_frame_color(c);
...
#if defined(HAVE_QT)
QImage LayoutViewBase::get_screenshot() {
  tl::SelfTimer timer(tl::verbosity() >= 11,
                      tl::to_string(tr("Save screenshot")));
...
tl::PixelBuffer LayoutViewBase::get_screenshot_pb() {
  tl::SelfTimer timer(tl::verbosity() >= 11,
                      tl::to_string(tr("Save screenshot")));
...
png_texts(const lay::LayoutViewBase *view, const db::DBox &box) {
  ...
  for (unsigned int i = 0; i < view->cellviews(); ++i) {
    if (view->cellview(i).is_valid()) {
      std::string name =
          view->cellview(i)->layout().cell_name(view->cellview(i).cell_index());
      texts.push_back(std::make_pair(
          std::string("Cell") + tl::to_string(int(i) + 1), name));
    }
  }
...
void LayoutViewBase::save_screenshot(const std::string &fn) {
  ...
  if (!writer.write(mp_canvas->screenshot().to_image())) {
    throw tl::Exception(
        tl::to_string(tr("Unable to write screenshot to file: %s (%s)")), fn,
        tl::to_string(writer.errorString()));
  }
...
void LayoutViewBase::save_screenshot(const std::string &) {
  throw tl::Exception(tl::to_string(
      tr("Unable to write screenshot - PNG library not compiled in")));
}
#endif

#if defined(HAVE_QT)
QImage LayoutViewBase::get_image(unsigned int width, unsigned int height) {
...
tl::PixelBuffer LayoutViewBase::get_pixels(unsigned int width,
                                           unsigned int height) {
  tl::SelfTimer timer(tl::verbosity() >= 11, tl::to_string(tr("Get image")));
...
#if defined(HAVE_QT)
QImage LayoutViewBase::get_image_with_options(
    unsigned int width, unsigned int height, int linewidth, int oversampling,
    double resolution, tl::Color background, tl::Color foreground,
    tl::Color active, const db::DBox &target_box, bool monochrome) {
  tl::SelfTimer timer(tl::verbosity() >= 11, tl::to_string(tr("Get image")));
  ...
  if (monochrome) {
    return mp_canvas
        ->image_with_options_mono(width, height, linewidth, background,
                                  foreground, active, target_box)
        .to_image_copy();
  } else {
    return mp_canvas
        ->image_with_options(width, height, linewidth, oversampling, resolution,
                             background, foreground, active, target_box)
        .to_image_copy();
  }
}
#endif

tl::PixelBuffer LayoutViewBase::get_pixels_with_options(
    unsigned int width, unsigned int height, int linewidth, int oversampling,
    double resolution, tl::Color background, tl::Color foreground,
    tl::Color active, const db::DBox &target_box) {
  tl::SelfTimer timer(tl::verbosity() >= 11, tl::to_string(tr("Get image")));

  refresh();

  return mp_canvas->image_with_options(width, height, linewidth, oversampling,
                                       resolution, background, foreground,
                                       active, target_box);
}

tl::BitmapBuffer LayoutViewBase::get_pixels_with_options_mono(
    unsigned int width, unsigned int height, int linewidth,
    tl::Color background, tl::Color foreground, tl::Color active,
    const db::DBox &target_box) {
  tl::SelfTimer timer(tl::verbosity() >= 11, tl::to_string(tr("Get image")));

  refresh();

  return mp_canvas->image_with_options_mono(
      width, height, linewidth, background, foreground, active, target_box);
}

#if defined(HAVE_QT) && !defined(PREFER_LIBPNG_FOR_SAVE)
void LayoutViewBase::save_image(const std::string &fn, unsigned int width,
                                unsigned int height) {
...
#else
void LayoutViewBase::save_image(const std::string &, unsigned int,
                                unsigned int) {
  throw tl::Exception(
      tl::to_string(tr("Unable to save image - PNG library not compiled in")));
}
#endif

#if defined(HAVE_QT) && !defined(PREFER_LIBPNG_FOR_SAVE)
void LayoutViewBase::save_image_with_options(
    const std::string &fn, unsigned int width, unsigned int height,
    int linewidth, int oversampling, double resolution, tl::Color background,
    tl::Color foreground, tl::Color active, const db::DBox &target_box,
    bool monochrome) {
  tl::SelfTimer timer(tl::verbosity() >= 11, tl::to_string(tr("Save image")));
  ...
  if (monochrome) {
    if (!writer.write(mp_canvas
                          ->image_with_options_mono(width, height, linewidth,
    ...
    if (!writer.write(mp_canvas
                          ->image_with_options(width, height, linewidth,
...
void LayoutViewBase::save_image_with_options(
  ...
  if (monochrome) {
    ...
    tl::BitmapBuffer img = mp_canvas->image_with_options_mono(
        width, height, linewidth, background, foreground, active, target_box);
    img.set_texts(texts);
    img.write_png(stream);
    ...
    tl::PixelBuffer img = mp_canvas->image_with_options(
        width, height, linewidth, oversampling, resolution, background,
        foreground, active, target_box);
    img.set_texts(texts);
    img.write_png(stream);
  }
...
#else
void LayoutViewBase::save_image_with_options(const std::string &, unsigned int,
                                             unsigned int, int, int, double,
                                             tl::Color, tl::Color, tl::Color,
                                             const db::DBox &, bool) {
  throw tl::Exception(
      tl::to_string(tr("Unable to save image - PNG library not compiled in")));
}
...
void LayoutViewBase::reload_layout(unsigned int cv_index) {
  ...
  //  this is the cellview at the given index (use a copy since the original is
  //  overwritten)
  ...
  if (m_hidden_cells.size() > cv_index) {
    ...
    for (std::set<cell_index_type>::const_iterator ci =
             m_hidden_cells[cv_index].begin();
         ci != m_hidden_cells[cv_index].end(); ++ci) {
  ...
  handle =
      new lay::LayoutHandle(new db::Layout(is_editable(), manager()), filename);
  handle->set_tech_name(technology);
  cv_empty.set(handle);

  for (std::map<unsigned int, db::LayerProperties>::const_iterator ol =
           org_layers.begin();
       ol != org_layers.end(); ++ol) {
  ...
  if (m_hidden_cells.size() > cv_index) {
    ...
    for (std::vector<std::string>::const_iterator cn = hidden_cells.begin();
         cn != hidden_cells.end(); ++cn) {
      std::pair<bool, cell_index_type> cid =
          cv->layout().cell_by_name(cn->c_str());
      if (cid.first) {
  ...
  //  clear the current cell (NOTE: this is for providing a cell target for some
  //  UI functions only)
  if (m_current_cell_per_cellview.size() > cv_index) {
    m_current_cell_per_cellview[cv_index] = cell_path_type();
  }

  //  Determine which layers to create as new layers. New layer need to be
  //  created if these have not been present in the original layout and there
  //  are no layer views referring to them.
  std::vector<db::LayerProperties> new_layers;
  for (unsigned int i = 0; i < cv->layout().layers(); ++i) {
    if (cv->layout().is_valid_layer(i)) {
      std::map<unsigned int, db::LayerProperties>::iterator ol =
          org_layers.find(i);
      if (ol == org_layers.end() &&
          present_layers.find(cv->layout().get_properties(i)) ==
              present_layers.end()) {
  ...
  for (std::vector<db::LayerProperties>::const_iterator l = new_layers.begin();
       l != new_layers.end(); ++l) {
    lay::LayerProperties p;
...
static void get_lyp_from_meta_info(const db::Layout &layout,
                                   std::string &lyp_file,
                                   bool &add_other_layers) {
  db::Layout::meta_info_name_id_type layer_properties_file_name_id =
      layout.meta_info_name_id("layer-properties-file");
  db::Layout::meta_info_name_id_type layer_properties_add_other_layers_name_id =
      layout.meta_info_name_id("layer-properties-add-other-layers");

  for (db::Layout::meta_info_iterator meta = layout.begin_meta();
       meta != layout.end_meta(); ++meta) {
...
unsigned int LayoutViewBase::add_layout(lay::LayoutHandle *layout_handle,
                                        bool add_cellview,
  ...
  try {
    ...
    for (db::Layout::top_down_const_iterator t = cv->layout().begin_top_down();
         t != cv->layout().end_top_cells(); ++t) {
      if (cv->layout().cell(*t).bbox().area() >
    ...
    //  to support applications with an active cellview (that is however
    //  invalid)
    ...
    if (initialize_layers) {
      ...
      //  Use the "layer-properties-file" meta info from the handle to get the
      //  layer properties file. If no such file is present, use the default
      //  file or the technology specific file.
      std::string lyp_file = m_def_lyp_file;
      const db::Technology *tech =
          db::Technologies::instance()->technology_by_name(
              layout_handle->tech_name());
      if (tech && !tech->eff_layer_properties_file().empty()) {
        lyp_file = tech->eff_layer_properties_file();
        add_other_layers = tech->add_other_layers();
      }

      //  Give the layout object a chance to specify a certain layer property
      //  file
...
unsigned int LayoutViewBase::create_layout(const std::string &technology,
                                           bool add_cellview,
                                           bool initialize_layers) {
  const db::Technology *tech =
      db::Technologies::instance()->technology_by_name(technology);
...
unsigned int LayoutViewBase::load_layout(const std::string &filename,
                                         const std::string &technology,
                                         bool add_cellview) {
  return load_layout(filename, db::LoadLayoutOptions(), technology,
                     add_cellview);
}
...
unsigned int LayoutViewBase::load_layout(const std::string &filename,
  ...
  try {
    ...
    {
      tl::log << tl::to_string(tr("Loading file: ")) << filename
              << tl::to_string(tr(" with technology: ")) << technology;
  ...
  try {
    ...
    //  Use the "layer-properties-file" meta info from the handle to get the
    //  layer properties file. If no such file is present, use the default file
    //  or the technology specific file.
    std::string lyp_file = m_def_lyp_file;
    if (tech && !tech->eff_layer_properties_file().empty()) {
      lyp_file = tech->eff_layer_properties_file();
      add_other_layers = tech->add_other_layers();
...
void LayoutViewBase::create_initial_layer_props(int cv_index,
                                                const std::string &lyp_file,
                                                bool add_missing) {
  std::vector<lay::LayerPropertiesList> props;
  bool loaded = false;
  ...
  if (!lyp_file.empty()) {
    ...
    try {
      ...
      try {
        ...
        props.clear();
        tl::XMLFileSource in(lyp_file);
        tl::log << tl::to_string(tr("Loading layer properties file: "))
                << lyp_file;
        lay::LayerPropertiesList::load(in, props);
        loaded = true;
      }
      ...
      tl::warn << tl::to_string(tr("Initialization of layers failed: "))
               << ex.msg();
    } catch (...) {
      tl::warn << tl::to_string(
          tr("Initialization of layers failed: unspecific error"));
    }
  }
  ...
  if (!loaded) {
    ...
    std::set<int> cv;
    for (std::vector<lay::LayerPropertiesList>::const_iterator p =
             props.begin();
         p != props.end(); ++p) {
      for (lay::LayerPropertiesConstIterator lp = p->begin_const_recursive();
           !lp.at_end(); ++lp) {
...
void LayoutViewBase::merge_layer_props(
    const std::vector<lay::LayerPropertiesList> &props) {
  ...
  //  If the specific list is a single list (no tabs), it is merged into every
  //  tab present.
  if (props.size() == 1) {
...
void LayoutViewBase::pop_state() {
  if (m_display_state_ptr > 0) {
    m_display_states.erase(m_display_states.begin() + m_display_state_ptr,
                           m_display_states.end());
    --m_display_state_ptr;
  }
}
...
void LayoutViewBase::store_state() {
  ...
  if (m_display_state_ptr + 1 < m_display_states.size()) {
    m_display_states.erase(m_display_states.begin() + m_display_state_ptr + 1,
                           m_display_states.end());
  }
...
db::DBox LayoutViewBase::box() const { return mp_canvas->viewport().box(); }

LayoutView *LayoutViewBase::get_ui() { return mp_ui; }

//  NOTE: this methods needs to be called "frequently"
void LayoutViewBase::timer() {
  bool dirty = false;
  for (std::list<lay::CellView>::const_iterator i = m_cellviews.begin();
       i != m_cellviews.end() && !dirty; ++i) {
...
void LayoutViewBase::force_update_content() { set_view_ops(); }
...
void LayoutViewBase::zoom_fit_sel() {
  ...
  if (!bbox.empty()) {
    bbox = db::DBox(bbox.left() - 0.025 * bbox.width(),
                    bbox.bottom() - 0.025 * bbox.height(),
                    bbox.right() + 0.025 * bbox.width(),
                    bbox.top() + 0.025 * bbox.height());
    zoom_box(bbox);
  }
}

db::DBox LayoutViewBase::full_box() const {
  //  compute the bounding box over all layers
  //  this will trigger the update procedures of the layout objects if not done
  //  yet ..

  db::DBox bbox;

  for (LayerPropertiesConstIterator l =
           get_properties().begin_const_recursive();
       !l.at_end(); ++l) {
    bbox += l->bbox();
  }

  for (lay::AnnotationShapes::iterator a = annotation_shapes().begin();
       !a.at_end(); ++a) {
    bbox += a->box();
  }
...
void LayoutViewBase::zoom_box_and_set_hier_levels(
    const db::DBox &bbox, const std::pair<int, int> &levels) {
...
void LayoutViewBase::pan_left() { shift_window(1.0, -m_pan_distance, 0.0); }

void LayoutViewBase::pan_right() { shift_window(1.0, m_pan_distance, 0.0); }

void LayoutViewBase::pan_up() { shift_window(1.0, 0.0, m_pan_distance); }

void LayoutViewBase::pan_down() { shift_window(1.0, 0.0, -m_pan_distance); }
...
void LayoutViewBase::zoom_in() { zoom_by(zoom_factor); }

void LayoutViewBase::zoom_out() { zoom_by(1.0 / zoom_factor); }
...
void LayoutViewBase::shift_window(double f, double dx, double dy) {
  ...
  db::DPoint s =
      mp_canvas->viewport().global_trans().inverted() * db::DPoint(dx, dy);
  db::DPoint c =
      b.center() + db::DVector(b.width() * s.x(), b.height() * s.y());
...
void LayoutViewBase::goto_window(const db::DPoint &p, double s) {
  if (s > 1e-6) {
    db::DBox b(p.x() - s * 0.5, p.y() - s * 0.5, p.x() + s * 0.5,
               p.y() + s * 0.5);
    zoom_box(b);
  } else {
    db::DBox b(box());
...
void LayoutViewBase::redraw_layer(unsigned int index) { do_redraw(index); }
...
void LayoutViewBase::redraw_later() { dm_redraw(); }
...
void LayoutViewBase::save_view(DisplayState &state) const {
  state = DisplayState(box(), get_min_hier_levels(), get_max_hier_levels(),
                       m_cellviews);
}
...
void LayoutViewBase::do_prop_changed() {
  if (m_visibility_changed) {
    ...
    for (lay::LayerPropertiesConstIterator l = begin_layers(); !l.at_end();
         ++l) {
      if (!l->has_children()) {
...
void LayoutViewBase::set_view_ops() {
  ...
  int brightness_for_context =
      ((bright_background ? m_ctx_dimming : -m_ctx_dimming) * 256) / 100;
  int brightness_for_child_context =
      ((bright_background ? m_child_ctx_dimming : -m_child_ctx_dimming) * 256) /
      100;

  //  count the layers to be able to reserve the number of view_ops
  size_t nlayers = 0;
  for (LayerPropertiesConstIterator lp =
           get_properties().begin_const_recursive();
       !lp.at_end(); ++lp) {
    if (!lp->has_children()) {
      ++nlayers;
  ...
  if (m_cell_box_visible) {
    ...
    if (m_ctx_color.is_valid()) {
      ...
      vop = lay::ViewOp(lay::LayerProperties::brighter(box_color.rgb(),
                                                       brightness_for_context),
                        lay::ViewOp::Copy, 0, 0, 0);
    }
    ...
    //  invisible
    for (unsigned int i = 0; i < (unsigned int)planes_per_layer;
         ++i) { //  frame, fill, vertex, text
      view_ops.push_back(lay::ViewOp(0, lay::ViewOp::Or, 0, 0, 0));
    }
  }

  //  sanity check: number of planes defined in layRedrawThreadWorker must match
  //  to view_ops layout
  ...
  for (int ctx = 0; ctx < 3; ++ctx) { // 0 (context), 1 (child), 2 (current)
    ...
    if (ctx == 0) {
      ...
      if (m_ctx_color.is_valid()) {
        ...
        frame_color = text_color = fill_color =
            lay::LayerProperties::brighter(gs_color, brightness_for_context);
      }
      ...
      if (m_child_ctx_color.is_valid()) {
        ...
        frame_color = text_color = fill_color = lay::LayerProperties::brighter(
            gs_color, brightness_for_child_context);
      }
    ...
    if (m_guiding_shape_visible) {
      ...
      //  frame
      view_ops.push_back(lay::ViewOp(frame_color, mode, 0, 0, 0,
                                     lay::ViewOp::Rect,
                                     m_guiding_shape_line_width));
      ...
      // vertex
      view_ops.push_back(
          lay::ViewOp(frame_color, mode, 0, 0, 0, lay::ViewOp::Rect,
                      m_guiding_shape_vertex_size /*mark size*/)); // vertex
  ...
  for (int ctx = 0; ctx < 3; ++ctx) { // 0 (context), 1 (child), 2 (current)
    ...
    for (LayerPropertiesConstIterator lp =
      ...
      //  because accessing the LayerPropertiesNode with lp->... is not quite
      //  efficient, we get the pointer here:
      ...
      if (l->visible(true /*real*/) && animate_visible) {
        ...
        if (ctx == 0) {
          ...
          if (m_ctx_color.is_valid()) {
            ...
            fill_color = l->eff_fill_color_brighter(true /*real*/,
                                                    brightness_for_context);
            frame_color = l->eff_frame_color_brighter(true /*real*/,
                                                      brightness_for_context);
            if (m_text_color.is_valid()) {
              text_color = lay::LayerProperties::brighter(
                  m_text_color.rgb(), brightness_for_context);
            } else {
          ...
          if (m_child_ctx_color.is_valid()) {
            ...
            fill_color = l->eff_fill_color_brighter(
                true /*real*/, brightness_for_child_context);
            frame_color = l->eff_frame_color_brighter(
                true /*real*/, brightness_for_child_context);
            if (m_text_color.is_valid()) {
              text_color = lay::LayerProperties::brighter(
                  m_text_color.rgb(), brightness_for_child_context);
            } else {
              text_color = frame_color;
            }
          }
        ...
        //  fill
        view_ops.push_back(
            lay::ViewOp(fill_color, mode, 0, dp, di_off)); // fill
        ...
        // vertex
        view_ops.push_back(lay::ViewOp(
            frame_color, mode, 0, 0, 0, lay::ViewOp::Cross,
            l->marked(true /*real*/) ? 9 /*mark size*/ : 0)); // vertex
...
std::pair<int, int> LayoutViewBase::get_hier_levels() const {
...
void LayoutViewBase::max_hier() {
  ...
  if (max_level > 0) {
    set_hier_levels(
        std::make_pair(std::min(m_from_level, max_level), max_level));
  }
}
...
int LayoutViewBase::max_hier_level() const {
  int max_level = 0;
  for (std::list<CellView>::const_iterator cv = m_cellviews.begin();
       cv != m_cellviews.end(); ++cv) {
    if (cv->is_valid()) {
...
void LayoutViewBase::set_palette(const lay::ColorPalette &p) { m_palette = p; }
...
tl::Color LayoutViewBase::default_background_color() {
  return tl::Color(0, 0, 0); // black.
}

void LayoutViewBase::do_set_background_color(tl::Color /*color*/,
                                             tl::Color /*contrast*/) {
  //  .. nothing yet ..
}
...
void LayoutViewBase::dbu_coordinates(bool f) { m_dbu_coordinates = f; }
...
const lay::CellView &LayoutViewBase::active_cellview() const {
  return cellview((unsigned int)active_cellview_index());
}

lay::CellViewRef LayoutViewBase::active_cellview_ref() {
...
void LayoutViewBase::selected_cells_paths(
    int /*cv_index*/, std::vector<cell_path_type> & /*paths*/) const {
  //  TODO: not implemented yet as there is no setter so far.
  //  (but it is implemented in the UI version where it is bound to the
  //  hierarchy control panel)
}

void LayoutViewBase::current_cell_path(int cv_index,
                                       cell_path_type &path) const {
...
void LayoutViewBase::set_current_cell_path(int cv_index,
                                           const cell_path_type &path) {
  if (cv_index >= 0) {
...
void LayoutViewBase::enable_active_cellview_changed_event(bool enable,
                                                          bool silent) {
  ...
  if (enable) {
    ...
    if (!silent && !m_active_cellview_changed_events.empty()) {
      ...
      //  we need to cancel pending drawing or dragging operations to reflect
      //  the new cellview (different target, may have different technology
      //  etc.)
      cancel_esc();

      //  we need to setup the editor option pages because the technology may
      //  have changed
      do_change_active_cellview();

      active_cellview_changed_event();
      for (std::set<int>::const_iterator i =
               m_active_cellview_changed_events.begin();
           i != m_active_cellview_changed_events.end(); ++i) {
...
void LayoutViewBase::active_cellview_changed(int index) {
  if (m_active_cellview_changed_event_enabled) {
    ...
    //  we need to cancel pending drawing or dragging operations to reflect the
    //  new cellview (different target, may have different technology etc.)
    cancel_esc();

    //  we need to setup the editor option pages because the technology may have
    //  changed
    do_change_active_cellview();
...
void LayoutViewBase::select_cell_dispatch(const cell_path_type &path,
                                          int cellview_index) {
  bool set_max_hier = (m_full_hier_new_cell || has_max_hier());
  if (m_clear_ruler_new_cell) {

    //  This is a HACK, but the clean solution would be to provide a new
    //  editable method like "clear_annotations":
...
void LayoutViewBase::select_cell_fit(const cell_path_type &path, int index) {
  if (index >= 0 && int(m_cellviews.size()) > index &&
      (cellview_iter(index)->specific_path().size() > 0 ||
       cellview_iter(index)->unspecific_path() != path)) {

    cellview_about_to_change_event(index);

    set_min_hier_levels(0);
    cancel();
    cellview_iter(index)->set_specific_path(
        lay::CellView::specific_cell_path_type());
...
void LayoutViewBase::select_cell_fit(cell_index_type cell_index, int index) {
  if (index >= 0 && int(m_cellviews.size()) > index &&
      cellview_iter(index)->cell_index() != cell_index) {
...
bool LayoutViewBase::is_cell_hidden(cell_index_type ci,
                                    int cellview_index) const {
  if (int(m_hidden_cells.size()) > cellview_index && cellview_index >= 0) {
    return m_hidden_cells[cellview_index].find(ci) !=
           m_hidden_cells[cellview_index].end();
  } else {
    return false;
  }
}
...
void LayoutViewBase::hide_cell(cell_index_type ci, int cellview_index) {
  ...
  if (m_hidden_cells[cellview_index].insert(ci).second) {
    if (transacting()) {
      manager()->queue(this,
                       new OpHideShowCell(ci, cellview_index, false /*=hide*/));
    } else if (manager() && !replaying()) {
      manager()->clear();
    }
...
void LayoutViewBase::show_cell(cell_index_type ci, int cellview_index) {
  ...
  if (int(m_hidden_cells.size()) > cellview_index) {
    if (m_hidden_cells[cellview_index].erase(ci) > 0) {
      if (transacting()) {
        manager()->queue(
            this, new OpHideShowCell(ci, cellview_index, true /*=show*/));
      } else if (manager() && !replaying()) {
        manager()->clear();
      }
      cell_visibility_changed_event();
  ...
  if (!m_hidden_cells[cv_index].empty()) {
    if (transacting()) {
      for (std::set<cell_index_type>::const_iterator ci =
           ...
           ci != m_hidden_cells[cv_index].end(); ++ci) {
        manager()->queue(this,
                         new OpHideShowCell(*ci, cv_index, true /*=show*/));
      }
    } else if (manager() && !replaying()) {
      manager()->clear();
    }
...
void LayoutViewBase::show_all_cells() {
  ...
  for (unsigned int i = 0; i < m_hidden_cells.size(); ++i) {
    if (!m_hidden_cells[i].empty()) {
      if (transacting()) {
        for (std::set<cell_index_type>::const_iterator ci =
                 m_hidden_cells[i].begin();
             ci != m_hidden_cells[i].end(); ++ci) {
...
void LayoutViewBase::text_visible(bool vis) {
  if (m_text_visible != vis) {
    m_text_visible = vis;
    update_content();
    redraw(); //  required because we do some optimizations is text is not
              //  visible ..
  }
}
...
void LayoutViewBase::full_hier_new_cell(bool f) { m_full_hier_new_cell = f; }

double LayoutViewBase::pan_distance() const { return m_pan_distance; }

void LayoutViewBase::pan_distance(double pd) { m_pan_distance = pd; }

void LayoutViewBase::fit_new_cell(bool f) { m_fit_new_cell = f; }
...
bool LayoutViewBase::has_selection() { return lay::Editables::has_selection(); }
...
void LayoutViewBase::paste_interactive(bool transient_mode) {
  ...
  std::unique_ptr<db::Transaction> trans(
      new db::Transaction(manager(), tl::to_string(tr("Paste and move"))));

  lay::Editables::paste();

  //  temporarily close the transaction and pass to the move service for
  //  appending it's own operations.
  trans->close();
...
void LayoutViewBase::copy() { copy_view_objects(); }
...
void LayoutViewBase::add_missing_layers() {
  ...
  for (std::vector<lay::ParsedLayerSource>::const_iterator a = actual.begin();
       a != actual.end(); ++a) {
...
LayerState LayoutViewBase::layer_snapshot() const {
  LayerState state;
  LayerPropertiesConstIterator l = begin_layers();
  while (!l.at_end()) {
    if (!l->has_children()) {
...
void LayoutViewBase::current_layer_changed_slot(
...
void LayoutViewBase::add_new_layers(const LayerState &state) {
  ...
  for (std::vector<lay::ParsedLayerSource>::const_iterator a = actual.begin();
    ...
    if (state.present.find(*a) == state.present.end()) {
      ...
      node.attach_view(this, current_layer_list());
      node.set_source(*a);
      //  HINT: in editable mode it is desireable to present all layers because
      //  otherwise they cannot be made visible to populate them.
...
void LayoutViewBase::free_resources() { mp_canvas->free_resources(); }
...
bool LayoutViewBase::is_activated() const { return true; }

void LayoutViewBase::switch_mode(int m) { mode(m); }
...
bool LayoutViewBase::is_move_mode() const { return m_mode == -1; }

bool LayoutViewBase::is_selection_mode() const { return m_mode == 0; }

unsigned int
LayoutViewBase::intrinsic_mouse_modes(std::vector<std::string> *descriptions) {
  if (descriptions) {
    descriptions->push_back("select\t" + tl::to_string(tr("Select")) +
                            "<:select_24px.png>");
    descriptions->push_back("move\t" + tl::to_string(tr("Move")) +
                            "<:move_24px.png>");
  }
  return 2;
}
...
static std::string name_from_title(const std::string &title) {
  std::string s = title;
...
static bool edit_mode_from_title(const std::string &title) {
  std::string s = title;
  std::string::size_type tab = s.find('\t');
  if (tab != std::string::npos) {
    s = std::string(s, 0, tab);
...
std::vector<std::string> LayoutViewBase::mode_names() const {
  ...
  for (auto i = mp_plugins.begin(); i != mp_plugins.end(); ++i) {
    std::string title;
    if ((*i) && (*i)->plugin_declaration() &&
        (*i)->plugin_declaration()->implements_mouse_mode(title)) {
...
std::string LayoutViewBase::mode_name() const {
  if (m_mode <= 0) {
    ...
    for (auto i = mp_plugins.begin(); i != mp_plugins.end(); ++i) {
      std::string title;
      if ((*i) && (*i)->plugin_declaration() &&
          (*i)->plugin_declaration()->id() == m_mode &&
          (*i)->plugin_declaration()->implements_mouse_mode(title)) {
...
std::vector<std::string> LayoutViewBase::menu_symbols() {
...
void LayoutViewBase::menu_activated(const std::string &symbol) {
  ...
  for (std::vector<lay::Plugin *>::const_iterator p = plugins().begin();
       p != plugins().end(); ++p) {
...
void LayoutViewBase::rename_cellview(const std::string &name,
                                     int cellview_index) {
...
LayoutViewBase::cv_transform_variants(int cv_index) const {
  ...
  return std::vector<db::DCplxTrans>(trns_variants.begin(),
                                     trns_variants.end());
}
...
LayoutViewBase::cv_transform_variants_by_layer(int cv_index) const {
  ...
  for (lay::LayerPropertiesConstIterator l = begin_layers(); !l.at_end(); ++l) {
    if (!l->has_children() && l->layer_index() >= 0) {
      int cvi = l->cellview_index() >= 0 ? l->cellview_index() : 0;
      if (cv_index < int(cellviews()) && cvi == cv_index) {
        std::vector<db::DCplxTrans> &v =
            tv_map
                .insert(std::make_pair(l->layer_index(),
                                       std::vector<db::DCplxTrans>()))
                .first->second;
  ...
  for (std::map<unsigned int, std::vector<db::DCplxTrans>>::iterator m =
           tv_map.begin();
       m != tv_map.end(); ++m) {
    std::sort(m->second.begin(), m->second.end());
    m->second.erase(std::unique(m->second.begin(), m->second.end()),
                    m->second.end());
  }
...
LayoutViewBase::cv_transform_variants() const {
  ...
  for (lay::LayerPropertiesConstIterator l = begin_layers(); !l.at_end(); ++l) {
    if (!l->has_children()) {
      unsigned int cv_index =
          l->cellview_index() >= 0 ? (unsigned int)l->cellview_index() : 0;
      if (cv_index < cellviews()) {
        for (std::vector<db::DCplxTrans>::const_iterator t = l->trans().begin();
             t != l->trans().end(); ++t) {
...
db::InstElement LayoutViewBase::ascend(int index) {
  tl_assert(int(m_cellviews.size()) > index &&
            cellview_iter(index)->is_valid());

  cellview_about_to_change_event(index);

  lay::CellView::specific_cell_path_type spath(
      cellview_iter(index)->specific_path());
  if (spath.empty()) {
...
void LayoutViewBase::descend(const std::vector<db::InstElement> &path,
                             int index) {
  if (!path.empty() && index >= 0 && int(m_cellviews.size()) > index &&
      cellview_iter(index)->is_valid()) {
...
bool LayoutViewBase::is_editable() const { return m_editable; }

unsigned int LayoutViewBase::search_range() { return m_search_range; }

void LayoutViewBase::set_search_range(unsigned int sr) { m_search_range = sr; }

unsigned int LayoutViewBase::search_range_box() { return m_search_range_box; }

void LayoutViewBase::set_search_range_box(unsigned int sr) {
  m_search_range_box = sr;
}

db::cell_index_type LayoutViewBase::new_cell(int cv_index,
  ...
  if (cv_index >= 0 && int(m_cellviews.size()) > cv_index) {
    ...
    if (!cell_name.empty() && layout.cell_by_name(cell_name.c_str()).first) {
      throw tl::Exception(
          tl::to_string(tr("A cell with that name already exists: %s")),
          cell_name);
    }
...
unsigned int LayoutViewBase::add_l2ndb(db::LayoutToNetlist *l2ndb) {
...
unsigned int LayoutViewBase::replace_l2ndb(unsigned int db_index,
                                           db::LayoutToNetlist *l2ndb) {
  tl_assert(l2ndb != 0);
...
db::LayoutToNetlist *LayoutViewBase::get_l2ndb(int index) {
...
const db::LayoutToNetlist *LayoutViewBase::get_l2ndb(int index) const {
  if (index >= 0 && index < int(m_l2ndbs.size())) {
    return m_l2ndbs[index];
  } else {
    return 0;
...
unsigned int LayoutViewBase::add_rdb(rdb::Database *rdb) {
...
unsigned int LayoutViewBase::replace_rdb(unsigned int db_index,
                                         rdb::Database *rdb) {
  tl_assert(rdb != 0);
...
rdb::Database *LayoutViewBase::get_rdb(int index) {
...
const rdb::Database *LayoutViewBase::get_rdb(int index) const {
  if (index >= 0 && index < int(m_rdbs.size())) {
    return m_rdbs[index];
  } else {
    return 0;
