...
std::string Margin::to_string() const {
  std::string res;
  if (m_relative_mode) {
...
Margin Margin::from_string(const std::string &s) {
  Margin res;
...
double Margin::get(double dim) const {
  return m_relative_mode ? dim * m_relative_value : m_absolute_value;
}

double Margin::get(const db::DBox &box) const {
