...
db::cell_index_type ObjectInstPath::cell_index_tot() const {
  if (m_path.empty()) {
    return m_topcell;
  } else {
...
db::ICplxTrans ObjectInstPath::trans_tot() const {
  db::ICplxTrans t;
...
db::cell_index_type ObjectInstPath::cell_index() const {
  if (m_path.empty()) {
    return m_topcell;
...
db::ICplxTrans ObjectInstPath::trans() const {
  db::ICplxTrans t;
  //  For instances, the last element is the instance itself - do not count that
...
void ObjectInstPath::insert_front(db::cell_index_type topcell,
                                  const db::InstElement &elem) {
