...
/**
 *  @brief Stores a layer reference to create layers which have been added by
 * some action
 *
 *  This object is delivered by LayoutViewBase::layer_snapshot and can be used
 * in add_missing_layers to create new layer views for layers which have been
 * created between layer_snapshot and add_missing_layers.
 */
struct LayerState {
  /**
   *  @brief Constructor
...
struct LAYBASIC_PUBLIC LayerDisplayProperties {
   ...
   *
   *  The effective frame color is computed from the frame color brightness and
   * the frame color.
   */
 ...
 */
class LAYBASIC_PUBLIC LayoutViewBase : public lay::Editables,
...
public:
   ...
   */
  LayoutViewBase(db::Manager *mgr, bool editable, lay::Plugin *plugin_parent,
   ...
   *
   *  This is the one explicitly set, not the one displayed. The displayed text
   * is composed of internal information if no title string is set.
   */
  const std::string &title_string() const { return m_title; }
  ...
  /**
   *  @brief Returns true, if the layer source shall be shown always in the
   * layer properties tree
   */
  virtual bool always_show_source() const;

  /**
   *  @brief Returns true, if the layer/datatype shall be shown always in the
   * layer properties tree
   */
  virtual bool always_show_ld() const;

  /**
   *  @brief Returns true, if the layout index shall be shown always in the
   * layer properties tree
   */
   ...
   *
   *  The layer's source must be set already to allow computing of the initial
   * color. It is assumed that the layer is appended at the end of the layer
   * list. This is important to set the dither pattern index accordingly.
   */
  void init_layer_properties(LayerProperties &props) const;

  /**
   *  @brief Create a set of new layers for the given layers of the given
   * cellview
   */
  void add_new_layers(const std::vector<unsigned int> &layer_ids, int cv_index);

  /**
   *  @brief Set the layer properties of a layer with the given position (by
   ...
   */
  void set_properties(const LayerPropertiesConstIterator &iter,
                      const LayerProperties &props) {
    set_properties(current_layer_list(), iter, props);
  }

  /**
   *  @brief Sets a value indicating whether the given node is expanded in the
   * layer tree
   *
   *  @param iter Points to the layer node to be modified
   *  @param ex True if the layer node shall be expanded, false if it shall be
   * collapsed
   */
  void set_layer_node_expanded(const LayerPropertiesConstIterator &iter,
                               bool ex) {
  ...
  /**
   *  @brief Set the layer properties of a layer with the given position (by
   * iterator) for the layer list with the given index
   *
   *  @param index The layer list's index
   *  @param iter Points to the layer to be replaced
   *  @param props The properties to replace the current properties
   */
  void set_properties(unsigned int index,
   ...
   * collapsed
   */
  void set_layer_node_expanded(unsigned int index,
                               const LayerPropertiesConstIterator &iter,
                               bool ex);
   ...
   *
   *  This method will replace the wildcard specifications in the layer
   * properties of all tabs. All unspecific cv index specifications will be
   * expanded to all cellviews, all layer and datatype specs will be expanded
   * into all available (remaining) specs.
   */
   ...
   *
   *  @param map_cv_index Maps a specified cv index to the one to use. Use -1
   * for the first entry to map any present cv index. Map to -1 to specify
   * expansion to all available cv indices.
   *  @param add_default Set this parameter to true to implicitly add an entry
   * for all "missing" layers.
   */
  void expand_properties(const std::map<int, int> &map_cv_index,
                         bool add_default);
   ...
   */
  void replace_layer_node(const LayerPropertiesConstIterator &iter,
                          const LayerPropertiesNode &props) {
    replace_layer_node(current_layer_list(), iter, props);
  }

  /**
   *  @brief Replace the layer node with a new one for the layer list with the
   * given index
   */
  void replace_layer_node(unsigned int index,
                          const LayerPropertiesConstIterator &iter,
                          const LayerPropertiesNode &props);

  /**
   *  @brief Insert the given layer properties node into the list for the
   * current layer list
   *
   *  This method returns a reference to the element created.
   */
  const LayerPropertiesNode &
  insert_layer(const LayerPropertiesConstIterator &iter,
               const LayerPropertiesNode &props) {
    return insert_layer(current_layer_list(), iter, props);
  }

  /**
   *  @brief Insert the given layer properties node into the list before the
   * given index
   *
   *  This method returns a reference to the element created.
   */
  const LayerPropertiesNode &
  insert_layer(unsigned int index, const LayerPropertiesConstIterator &iter,
               const LayerPropertiesNode &props);
   ...
   *  the iterator since the object that the iterator points to is no longer
   * valid.
   */
  ...
  /**
   *  @brief Delete the layer properties node for the layer list with the given
   * index
   *
   *  This method deletes the object that the iterator points to and invalidates
   *  the iterator since the object that the iterator points to is no longer
   ...
   */
  void clear_layers() { set_properties(LayerPropertiesList()); }
   ...
   */
  unsigned int current_layer_list() const { return m_current_layer_list; }
  ...
  /**
   *  @brief Insert the layer properties list at the given index and make it the
   * current list.
   */
  ...
  /**
   *  @brief Sets the currently active layer by layer properties and cell view
   * index
   *
   *  This method will look up that layer in the layer view tree and select that
   * layer. This method will also select this layer.
   *
   *  Returns false if the layer is not a valid one.
   */
  bool set_current_layer(unsigned int cv_index,
   ...
   */
  virtual std::vector<lay::LayerPropertiesConstIterator>
  selected_layers() const;

  /**
   *  @brief Gets a pixmap representing the given layer
   *
   *  @param iter indicates the layer
   *  @param w The width in logical pixels of the generated pixmap (will be
   * multiplied by dpr)
   *  @param h The height in logical pixels of the generated pixmap (will be
   * multiplied by dpr)
   *  @param dpr The device pixel ratio (number of image pixes per logical
   * pixel) - negative values mean auto-detect
   *  @param di_off The dither pattern offset (used for animation)
   *  @param no_state If true, the state will not be indicated
   */
  tl::PixelBuffer icon_for_layer(const lay::LayerPropertiesConstIterator &iter,
                                 unsigned int w, unsigned int h,
                                 double dpr = -1.0, unsigned int di_off = 0,
                                 bool no_state = false);

  /**
   *  @brief Sets the layers that are selected in the layer browser
   */
  virtual void set_selected_layers(
   ...
   */
  const LineStyles &line_styles() const { return mp_canvas->line_styles(); }

  /**
   *  @brief An event signalling the change in the number of hierarchy levels
   * shown
   */
   ...
   *  event is triggered. This may happed due to the removal or insertion of
   * shapes or cell instances. In general, this indicates the need for redrawing
   * of the layout for example.
   */
   ...
   *
   *  This event is triggered before something on the cellview (i.e. the current
   * cell of of set of the cellview) is changed. The argument is the index of
   * the cell view to be changed. This event is followed by a corresponding
   * cellview_changed event after the cellview has changed.
   */
   ...
   *
   *  This event is triggered before something on the cellviews is changed, i.e.
   * a cellview is deleted or inserted. Each of these events is followed by a
   * cellviews_changed event after the change has been made.
   */
   ...
   *
   *  When a cellview is added or removed, this event is triggered after the
   * change has been made. The corresponding event that is triggered before the
   * change is made is cellviews_about_to_change_event.
   */
   ...
   *
   *  If a cellview is changed (i.e. the cell is changed) this event is
   * triggered. The integer argument will receive the index of the cellview that
   * has changed. The corresponding event that is triggered before the change is
   * made is cellview_about_to_change_event.
   */
   ...
   *  The argument is the index of the cellview that received the new
   * technology.
   */
   ...
   *
   *  If the layer list changes, this event is triggered with an integer
   * argument. The arguments's bit 0 is set, if the properties have changed. If
   * the arguments bit 1 is set, the hierarchy has changed. If the name of layer
   * properties is changed, bit 2 is set.
   */
   ...
   *
   *  If a layer list is deleted from the layer list set, the event is triggered
   * with the index of the deleted list as an integer parameter.
   */
   ...
   *
   *  If a layer list is inserted into the layer list set, the event is
   * triggered with the index of the new list as an integer parameter.
   */
   ...
   */
  tl::event<const lay::LayerPropertiesConstIterator &>
      current_layer_changed_event;
   ...
   *  If "update" is true, the cell view's properties will be updated (options,
   * filename etc.).
   */
  void save_as(unsigned int index, const std::string &filename,
   ...
   *  @param fn The file to load
   *
   *  This version allows one to specify whether defaults should be used for all
   * other layers by setting add_default to true
   */
   ...
   *
   *  This version allows one to specify whether defaults should be used for all
   * other layers by setting add_default to true. In addition, this version will
   * apply the .lyp definitions to a specific cellview after removing all
   * definitions for this one. If cv_index is set to -1, the .lyp file will be
   * applied to each cellview. In any case, the cv index specs in the .lyp file
   * will be overridden.
   */
  void load_layer_props(const std::string &fn, int cv_index, bool add_default);

  /**
   *  @brief Determine whether a given layer properties file is a single-layout
   * file
   *
   ...
   */
  const BookmarkList &bookmarks() const { return m_bookmarks; }
   ...
   */
  void save_image(const std::string &fn, unsigned int width,
                  unsigned int height);
   ...
   *  @param linewidth The width of a line in pixels (usually 1) or 0 for
   * default
   *  @param oversampling The oversampling factor (1..3) or 0 for default
   *  @param resolution The resolution (pixel size compared to a screen pixel
   * size, i.e 1/oversampling) or 0 for default
   ...
   */
  void save_image_with_options(const std::string &fn, unsigned int width,
                               unsigned int height, int linewidth,
                               int oversampling, double resolution,
                               tl::Color background, tl::Color foreground,
                               tl::Color active_color,
                               const db::DBox &target_box, bool monochrome);

#if defined(HAVE_QT)
  /**
   *  @brief Get the screen content as a QImage object with the given width and
   * height
   */
   ...
   *  @param monochrome If true, monochrome images will be produced
   */
  QImage get_image_with_options(unsigned int width, unsigned int height,
                                int linewidth, int oversampling,
                                double resolution, tl::Color background,
                                tl::Color foreground, tl::Color active_color,
                                const db::DBox &target_box, bool monochrome);
#endif

  /**
   *  @brief Get the screen content as a tl::PixelBuffer object with the given
   * width and height
   *
   *  @param width The width of the image in pixels
   *  @param height The height of the image
   ...
   */
  tl::PixelBuffer
  get_pixels_with_options(unsigned int width, unsigned int height,
                          int linewidth, int oversampling, double resolution,
                          tl::Color background, tl::Color foreground,
                          tl::Color active_color, const db::DBox &target_box);

  /**
   *  @brief Get the screen content as a monochrome tl::BitmapBuffer object with
   * the given options
   *
   *  @param width The width of the image in pixels
   *  @param height The height of the image
   ...
   *  @param target_box The box to draw or db::DBox() for default
   *
   *  The colors will are converted to "on" pixels with a green channel value >=
   * 50%.
   */
  tl::BitmapBuffer
  get_pixels_with_options_mono(unsigned int width, unsigned int height,
                               int linewidth, tl::Color background,
                               tl::Color foreground, tl::Color active_color,
   ...
   *  @brief Hierarchy level selection getter
   */
  int get_min_hier_levels() const { return get_hier_levels().first; }

  /**
   *  @brief Hierarchy level selection getter
   */
  int get_max_hier_levels() const { return get_hier_levels().second; }
   ...
   */
  tl::Color cell_box_color() const { return m_box_color; }
   ...
   */
  bool cell_box_text_transform() const { return m_box_text_transform; }
   ...
   *  @brief Label font setter
   */
  unsigned int cell_box_text_font() const { return m_box_font; }
   ...
   *  @brief Visibility of cell boxes
   */
  bool cell_box_visible() const { return m_cell_box_visible; }
   ...
   */
  int min_inst_label_size() const { return m_min_size_for_label; }
   ...
   *  @brief Visibility of text objects
   */
  bool text_visible() const { return m_text_visible; }
   ...
   *  @brief Show properties
   */
  bool show_properties_as_text() { return m_show_properties; }
   ...
   */
  bool bitmap_caching() { return m_bitmap_caching; }
   ...
   *  @brief Lazy rendering of text objects
   */
  bool text_lazy_rendering() { return m_text_lazy_rendering; }
   ...
   */
  unsigned int text_font() const { return m_text_font; }
   ...
   *  @brief The default text size property
   */
  double default_text_size() const { return m_default_text_size; }
   ...
   */
  bool text_point_mode() const { return m_text_point_mode; }
   ...
   */
  bool show_markers() const { return m_show_markers; }
   ...
   */
  bool no_stipples() const { return m_no_stipples; }
   ...
   */
  bool offset_stipples() const { return m_stipple_offset; }
   ...
   *  @brief Apply text transformation property
   */
  bool apply_text_trans() const { return m_apply_text_trans; }
   ...
   *  @brief Text object color
   */
  tl::Color text_color() const { return m_text_color; }
   ...
   *  if the cell is changed.
   */
  bool clear_ruler_new_cell() const { return m_clear_ruler_new_cell; }
   ...
   *  if the cell is changed.
   */
  bool full_hier_new_cell() const { return m_full_hier_new_cell; }
   ...
   *  changed if the cell is changed.
   */
  bool fit_new_cell() const { return m_fit_new_cell; }
   ...
   */
  int mouse_wheel_mode() const { return m_wheel_mode; }
   ...
   *  shift is left/right, ctrl is zoom).
   */
  void mouse_wheel_mode(int m) { m_wheel_mode = m; }
   ...
   */
  const lay::ColorPalette &get_palette() const { return m_palette; }

  /**
   *  @brief Sets the stipple palette
   *
   *  The stipple palette is used for selecting stipples for new layers and is
   * shown in the layer toolbox.
   */
   ...
   *
   *  The line style palette is used for selecting line styles and is shown in
   * the layer toolbox.
   */
  ...
  /**
   *  @brief Load a (new) file into the layout associating it with the given
   * technology
   *
   *  The add_cellview param controls whether to create a new cellview
   *  or clear all cellviews before.
   ...
   *  @return The index of the cellview loaded.
   */
  unsigned int load_layout(const std::string &filename,
                           const std::string &technology, bool add_cellview);
  ...
  /**
   *  @brief Load a (new) file into the layout with the options and using the
   ...
   *
   *  The add_cellview param controls whether to create a new cellview
   *  or clear all cellviews before. This version allows one to specify whether
   * layer properties shall be created.
   *
   *  @return The index of the cellview created.
   */
  unsigned int create_layout(const std::string &technology, bool add_cellview,
  ...
  /**
   *  @brief Add an existing layout or replace the current cellview by the given
   * layout
   *
   *  The add_cellview param controls whether to add the layout as a new
   * cellview or clear all cellviews before.
   *
   *  The "initialize_layers" parameter allows one to specify whether the layer
   * properties shall be initialized.
   *
   *  @return The index of the cellview loaded.
   */
  unsigned int add_layout(lay::LayoutHandle *layout_handle, bool add_cellview,
                          bool initialize_layers = true);
   ...
   */
  unsigned int cellviews() const { return (unsigned int)m_cellviews.size(); }

  /**
   *  @brief Obtain the cellviews as a vector
   */
  const std::list<lay::CellView> &cellview_list() const { return m_cellviews; }
   ...
   *  Based on the pointer passed to the function, the index of the respective
   * cellview will be returned. If a null pointer is given or the pointer is not
   * a valid cellview, -1 will be returned.
   */
   ...
   */
  lay::AnnotationShapes &annotation_shapes() { return m_annotation_shapes; }
   ...
   *  @brief Goto window
   *
   *  Position the window to the new position x and y with a size of s
   * (approximately). If s is negative or zero, the windows is just shifted.
   */
   ...
   *  @param cell_name The name of the cell (an exception is thrown if a cell of
   ...
   */
  static unsigned int
  intrinsic_mouse_modes(std::vector<std::string> *descriptions);
  ...
  /**
   *  @brief Get the global transform variants for a given cellview index and
   * layer
   */
  std::vector<db::DCplxTrans> cv_transform_variants(int cv_index,
                                                    unsigned int layer) const;

  /**
   *  @brief Get the transformation variants for a given cellview index ordered
   * by layer
   */
  std::map<unsigned int, std::vector<db::DCplxTrans>>
  cv_transform_variants_by_layer(int cv_index) const;
   ...
   *
   *  If this flag is true, the property dialogs and other display functions
   * should use database units to display coordinates etc.
   */
  bool dbu_coordinates() const { return m_dbu_coordinates; }
   ...
   *
   *  If this flag is true, the property dialogs and other display functions
   * should use absolute (on top level) coordinates for points etc. "absolute"
   * may as well refer to orientation, not only to coordinates, if
   * transformations are considered.
   */
  bool absolute_coordinates() const { return m_absolute_coordinates; }
  ...
  /**
   *  @brief Gets the canvas object (where the layout is drawn and view objects
   * are placed)
   */
  lay::LayoutCanvas *canvas() { return mp_canvas; }

  /**
   *  @brief Gets the canvas object (const version)
   */
  const lay::LayoutCanvas *canvas() const { return mp_canvas; }

#if defined(HAVE_QT)
  /**
   *  @brief Gets the layer control panel
   */
  virtual lay::LayerControlPanel *control_panel() { return 0; }

  /**
   *  @brief Gets the hierarchy panel
   */
  virtual lay::HierarchyControlPanel *hierarchy_panel() { return 0; }

  /**
   *  @brief Gets the editor options page
   */
  virtual lay::EditorOptionsPages *editor_options_pages() { return 0; }
#endif

  /**
   *  @brief Get the current viewport
   */
  const lay::Viewport &viewport() const { return mp_canvas->viewport(); }
   ...
   */
  tl::Color background_color() const { return mp_canvas->background_color(); }

  /**
   *  @brief Foreground color property
   */
  tl::Color foreground_color() const { return mp_canvas->foreground_color(); }

  /**
   *  @brief Active color property
   */
  tl::Color active_color() const { return mp_canvas->active_color(); }
   ...
   */
  bool drop_small_cells() const { return m_drop_small_cells; }
   ...
   */
  bool guiding_shapes_visible() const { return m_guiding_shape_visible; }
   ...
   */
  tl::Color guiding_shapes_color() const { return m_guiding_shape_color; }
   ...
   */
  int guiding_shapes_line_width() const { return m_guiding_shape_line_width; }
   ...
   */
  int guiding_shapes_vertex_size() const { return m_guiding_shape_vertex_size; }
   ...
   *
   *  If child context is enabled, a third set of bit planes must be created by
   * the redraw thread containing the shapes for the below-top-level objects.
   */
  bool child_context_enabled() const { return m_child_ctx_enabled; }
   ...
   */
  lay::Drawings *drawings() { return mp_canvas; }
   ...
   */
  int drawing_workers() const { return m_drawing_workers; }

  /**
   *  @brief Gets a value indicating whether the view will accept a dropped file
   * with the given URL or path
   */
   ...
   */
  virtual bool layer_model_updated() { return false; }

  /**
   *  @brief Gets a list of all plugins
   */
  const std::vector<lay::Plugin *> &plugins() { return mp_plugins; }
   ...
   *  This method will return 0, if no such plugin is registered
   */
  template <class PI> std::vector<PI *> get_plugins() const {
    std::vector<PI *> pi;
    for (std::vector<lay::Plugin *>::const_iterator p = mp_plugins.begin();
         p != mp_plugins.end(); ++p) {
   ...
   *
   *  This method can be used to selectively create plugins when the NoPlugin
   * option is used. If no such plugin is registered, no plugin is created. If a
   * plugin with that name is already registered, it is not created again. PD is
   * the type of the declaration.
   */
  template <class PD> void create_plugin() {
    for (std::vector<lay::Plugin *>::const_iterator p = mp_plugins.begin();
         p != mp_plugins.end(); ++p) {
   ...
   */
  bool edits_enabled() const { return m_disabled_edits <= 0; }
  ...
  /**
   *  @brief Descend into the hierarchy along the given specific path for the
   * given cellview
   */
  ...
  /**
   *  @brief An event triggered if the active cellview changes
   *  This event is triggered after the active cellview changed. The integer
   * parameter is the index of the new cellview.
   */
   ...
   *
   *  The current cell is the one highlighted in the browser with the focus
   * rectangle. The current path is returned for the cellview given by cv_index.
   */
  virtual void selected_cells_paths(int cv_index,
                                    std::vector<cell_path_type> &paths) const;
   ...
   *
   *  The current cell is the one highlighted in the browser with the focus
   * rectangle. The cell given by the path is highlighted and scrolled into
   * view.
   */
   ...
   *
   *  @param cv_index The cellview for which to produce a set of layer
   * properties
   *  @param lyp_file The layer properties file to load or empty if no file
   * should be loaded
   *  @param add_missing True, if missing layers should be added (ignored if
   * lyp_file is empty)
   */
  void create_initial_layer_props(int cv_index, const std::string &lyp_file,
                                  bool add_missing);
   ...
   */
  bool select_inside_pcells_mode() const { return m_sel_inside_pcells; }
   ...
   */
  bool transient_selection_mode() const { return m_transient_selection_mode; }

  /**
   *  @brief Get the default color for markers
   */
  tl::Color default_marker_color() const { return m_marker_color; }

  /**
   *  @brief Get the default line width for markers
   */
  int default_marker_line_width() const { return m_marker_line_width; }

  /**
   *  @brief Get the default marker dither pattern index
   */
  int default_dither_pattern() const { return m_marker_dither_pattern; }

  /**
   *  @brief Get the default marker line style index
   */
  int default_line_style() const { return m_marker_line_style; }

  /**
   *  @brief Get the default vertex size for markers
   */
  int default_marker_vertex_size() const { return m_marker_vertex_size; }

  /**
   *  @brief Get the default halo flag for markers
   */
  int default_marker_halo() const { return m_marker_halo; }

  /**
   *  @brief Gets the "search range" in pixels (for single click)
   *  The search range applies whenever some object is looked up in the vicinity
   * of the mouse cursor. This value gives the search range in pixels.
   */
  ...
  /**
   *  @brief Gets a snapshot of the current layers (can be used for
   * add_missing_layers later)
   */
   ...
   *
   *  The layout view will become owner of the database.
   *  If the index is not valid, the database will be added and the new index
   * will be returned.
   *
   ...
   */
  unsigned int num_rdbs() const { return (unsigned int)m_rdbs.size(); }
   ...
   */
  unsigned int num_l2ndbs() const { return (unsigned int)m_l2ndbs.size(); }
   ...
   */
  void zoom_box_and_set_hier_levels(const db::DBox &b,
                                    const std::pair<int, int> &levels);
  ...
  /**
   *  @brief Move the viewport such that the given point is at the center of the
   * viewport
   */
  ...
  lay::SelectionService *selection_service() const {
    return mp_selection_service;
  }
  ...
  /**
   *  @brief Cancels all edit operations, clears the selection and resets the
   * mode to "Select"
   */
  void cancel_esc();
   ...
   */
  void
  current_layer_changed_slot(const lay::LayerPropertiesConstIterator &iter);
...
private:
  ...
  void signal_bboxes_from_layer_changed(unsigned int cv_index,
...
private:
  ...
  void do_load_layer_props(const std::string &fn, bool map_cv, int cv_index,
                           bool add_default);
  void finish_cellviews_changed();
  void init_layer_properties(LayerProperties &props,
                             const LayerPropertiesList &lp_list) const;
...
protected:
   ...
   */
  LayoutViewBase(lay::LayoutView *ui, db::Manager *mgr, bool editable,
                 lay::Plugin *plugin_parent,
                 unsigned int options = (unsigned int)LV_Normal);

  void init(db::Manager *mgr);

  lay::Plugin *active_plugin() const { return mp_active_plugin; }
