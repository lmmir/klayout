...
DitherPatternInfo::DitherPatternInfo(const DitherPatternInfo &d)
    : m_width(d.m_width), m_height(d.m_height), m_order_index(d.m_order_index),
      m_name(d.m_name) {
  operator=(d);
}

DitherPatternInfo &DitherPatternInfo::operator=(const DitherPatternInfo &d) {
  if (&d != this) {
...
bool DitherPatternInfo::same_bitmap(const DitherPatternInfo &d) const {
  ...
  unsigned int n =
      m_pattern_stride * (sizeof(m_pattern) / sizeof(m_pattern[0]));
...
bool DitherPatternInfo::operator==(const DitherPatternInfo &d) const {
  return same_bitmap(d) && m_name == d.m_name &&
         m_order_index == d.m_order_index;
}
...
// TODO including a scaling algorithm in this formula, or give more resolution
// to the dither
QBitmap DitherPatternInfo::get_bitmap(int width, int height,
  ...
  for (unsigned int i = 0; i < (unsigned int)height; ++i) {
    ...
    for (unsigned int j = 0; j < (unsigned int)width; ++j) {
      if (j < fw || j >= (unsigned int)width - fw ||
          (w & (1 << (j % m_width))) != 0) {
  ...
  QBitmap bitmap(
      QBitmap::fromData(QSize(width, height), data, QImage::Format_MonoLSB));
  delete[] data;
...
void DitherPatternInfo::set_pattern(const uint32_t *pt, unsigned int w,
                                    unsigned int h) {
  tl::MutexLocker locker(&s_mutex);
...
void DitherPatternInfo::set_pattern_impl(const uint32_t *pt, unsigned int w,
                                         unsigned int h) {
...
void DitherPatternInfo::set_pattern(const uint64_t *pt, unsigned int w,
                                    unsigned int h) {
  tl::MutexLocker locker(&s_mutex);
  m_scaled_pattern.reset(0);
...
void DitherPatternInfo::set_pattern_impl(const uint64_t *pt, unsigned int w,
                                         unsigned int h) {
  //  pattern size must be 1x1 at least
  if (w == 0 || h == 0) {
    uint32_t zero = 0;
...
const DitherPatternInfo &DitherPatternInfo::scaled(unsigned int n) const {
  if (n <= 1) {
    return *this;
  }
...
void DitherPatternInfo::scale_pattern(unsigned int n) {
  //  limit scale factor such that the width and height do not get larger than
  //  64
      ...
      for (unsigned int c = 0; c < m_width; ++c) {
        for (unsigned int b = 0; b < n; ++b) {
          if ((*p & m) != 0) {
            ...
            //  Try interpolation.
            //  In the following cases, the center's pixel lower-right quadrant
            //  fill be filled:
            //
            //  (A1)     (A2)     (A3)
            //  x 0 0    x 0 0    x 0 1
            ...
            //  0 1 x    x 1 x
            //
            //  For easy implementation, we encode the pattern into a byte k
            //  with the following significant bits (for lower-right subpixel,
            //  mirrored accordingly for the other subpixels)
            //
            //  k bits:
            //  0 1 2
            //  3 - 4
            ...
            uint8_t k =
                ((*py2 & mx2) != 0 ? 1 : 0) | ((*py2 & m) != 0 ? 2 : 0) |
                ((*py2 & mx1) != 0 ? 4 : 0) | ((*p & mx2) != 0 ? 8 : 0) |
                ((*p & mx1) != 0 ? 16 : 0) | ((*py1 & mx2) != 0 ? 32 : 0) |
                ((*py1 & m) != 0 ? 64 : 0) | ((*py1 & mx1) != 0 ? 128 : 0);
            if ((k & 0x7e) == 0x50 /*(A1)*/ || (k & 0x7e) == 0x70 /*(A2)*/ ||
                (k & 0x7e) == 0x54 /*(A3)*/ || (k & 0x7b) == 0x52 /*(B1)*/ ||
                (k & 0x5f) == 0x58 /*(B2)*/) {
              d |= mm;
            }
          }
...
std::string DitherPatternInfo::to_string() const {
  std::string res;
...
std::vector<std::string> DitherPatternInfo::to_strings() const {
...
struct ReplaceDitherPatternOp : public db::Op {
  ReplaceDitherPatternOp(unsigned int i, const DitherPatternInfo &o,
                         const DitherPatternInfo &n)
...
DitherPattern &DitherPattern::operator=(const DitherPattern &p) {
  if (this != &p) {
    unsigned int i;
    for (i = 0; i < p.count(); ++i) {
...
const DitherPatternInfo &DitherPattern::pattern(unsigned int i) const {
  if (i < count()) {
    return m_pattern[i];
  } else {
...
void DitherPattern::replace_pattern(unsigned int i,
                                    const DitherPatternInfo &p) {
  while (i >= count()) {
    m_pattern.push_back(DitherPatternInfo());
  }
...
unsigned int DitherPattern::add_pattern(const DitherPatternInfo &p) {
  unsigned int oi = 0;
...
DitherPattern::iterator DitherPattern::begin_custom() const {
  return m_pattern.begin() +
         sizeof(dither_strings) / sizeof(dither_strings[0]) / 2;
}

const DitherPattern &DitherPattern::default_pattern() {
  static DitherPattern empty;
  return empty;
}

void DitherPattern::undo(db::Op *op) {
  const ReplaceDitherPatternOp *rop =
      dynamic_cast<const ReplaceDitherPatternOp *>(op);
  if (rop) {
...
struct pattern_less_f {
  bool operator()(const DitherPatternInfo &a,
                  const DitherPatternInfo &b) const {
    return a.less_bitmap(b);
  }
};

void DitherPattern::merge(const DitherPattern &other,
  ...
  for (iterator c = begin(); c != begin_custom(); ++c) {
    index_map.insert(std::make_pair((unsigned int)std::distance(begin(), c),
                                    (unsigned int)std::distance(begin(), c)));
  }
  ...
  for (iterator c = begin_custom(); c != end(); ++c) {
    patterns.insert(
        std::make_pair(*c, (unsigned int)std::distance(begin(), c)));
  }

  //  map the pattern of other into *this, possibly creating new ones
  for (iterator c = other.begin_custom(); c != other.end(); ++c) {
    std::map<DitherPatternInfo, unsigned int, pattern_less_f>::const_iterator
        p = patterns.find(*c);
    ...
    index_map.insert(std::make_pair(
        (unsigned int)std::distance(other.begin(), c), new_index));
  }
}
