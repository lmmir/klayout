...
class ActionStub : public lay::Action {
public:
  ...
  virtual bool wants_visible() const {
    if (wants_visible_cb.can_issue()) {
      return wants_visible_cb.issue<lay::Action, bool>(
          &lay::Action::wants_visible);
    } else {
      return true;
    }
  }
  ...
  virtual bool wants_enabled() const {
    if (wants_enabled_cb.can_issue()) {
      return wants_enabled_cb.issue<lay::Action, bool>(
          &lay::Action::wants_enabled);
    } else {
      return true;
    }
  }
...
static std::string
pack_key_binding(const std::map<std::string, std::string> &kb) {
...
unpack_key_binding(const std::string &s) {
  std::vector<std::pair<std::string, std::string>> v =
      lay::unpack_key_binding(s);
...
static std::string
pack_menu_items_hidden(const std::map<std::string, bool> &kb) {
...
unpack_menu_items_hidden(const std::string &s) {
  std::vector<std::pair<std::string, bool>> v =
      lay::unpack_menu_items_hidden(s);
  std::map<std::string, bool> kb;
  kb.insert(v.begin(), v.end());
  return kb;
}

static lay::AbstractMenu *new_menu() { return new lay::AbstractMenu(0); }

Class<lay::AbstractMenu> decl_AbstractMenu(
    "lay", "AbstractMenu",
    //  for test purposes mainly:
    constructor("new", &new_menu, "@hide") +
        method(
            "pack_key_binding", &pack_key_binding, gsi::arg("path_to_keys"),
            ...
            "This method will take an array of path/key definitions (including "
            "the \\Action#NoKeyBound option) "
            "and convert it to a single string suitable for assigning to the "
            "config key.\n"
            "\n"
        ...
        method("unpack_key_binding", &unpack_key_binding, gsi::arg("s"),
               ...
               "\n"
               "This method has been introduced in version 0.26.") +
        method("pack_menu_items_hidden", &pack_menu_items_hidden,
               gsi::arg("path_to_visibility"),
               "@brief Serializes a menu item visibility definition into a "
               "single string\n"
               "The serialized format is used by the 'menu-items-hidden' "
               "config key. "
               "This method will take an array of path/visibility flag "
               "definitions "
               "and convert it to a single string suitable for assigning to "
               "the config key.\n"
               "\n"
               "This method has been introduced in version 0.26.") +
        method("unpack_menu_items_hidden", &unpack_menu_items_hidden,
               gsi::arg("s"),
               ...
               "This method has been introduced in version 0.26.") +
        method("action",
               (lay::Action * (lay::AbstractMenu::*)(const std::string &path)) &
                   lay::AbstractMenu::action,
               gsi::arg("path"),
               "@brief Gets the reference to a Action object associated with "
               "the given path\n"
               "\n"
               "@param path The path to the item.\n"
               "@return A reference to a Action object associated with this "
               "path or nil if the path is not valid\n") +
        method("items", &lay::AbstractMenu::items, gsi::arg("path"),
               ...
               "@return A vector or path strings for the child items or an "
               "empty vector if the path is not valid or the item does not "
               "have children\n") +
        ...
        method("insert_item",
               (void (lay::AbstractMenu::*)(const std::string &,
                                            const std::string &,
                                            const lay::Action *)) &
                   lay::AbstractMenu::insert_item,
               gsi::arg("path"), gsi::arg("name"), gsi::arg("action"),
               "@brief Inserts a new item before the one given by the path\n"
               "\n"
               "The Action object passed as the third parameter references the "
               "handler which both implements the "
               "action to perform and the menu item's appearance such as "
               "title, icon and keyboard shortcut.\n"
               "\n"
               "@param path The path to the item before which to insert the "
               "new item\n"
               "@param name The name of the item to insert \n"
               "@param action The Action object to insert\n") +
        method(
            "insert_separator", &lay::AbstractMenu::insert_separator,
            gsi::arg("path"), gsi::arg("name"),
        ...
        method(
            ...
            (void (lay::AbstractMenu::*)(const std::string &,
                                         const std::string &,
                                         const std::string &)) &
                lay::AbstractMenu::insert_menu,
            gsi::arg("path"), gsi::arg("name"), gsi::arg("title"),
            "@brief Inserts a new submenu before the item given by the path\n"
            "\n"
            "The title string optionally encodes the key shortcut and icon "
            "resource\n"
            "in the form "
    ...
    "\n"
    "The abstract menu is a class that stores a main menu and several popup "
    "menus\n"
    ...
    "enabled/disable state etc.\n"
    "The Action is either provided when new entries are inserted or created "
    "upon initialisation.\n"
    "\n"
    "The abstract menu class provides methods to manipulate the menu structure "
    "(the state of the\n"
    "menu items, their title and shortcut key is provided and manipulated "
    "through the Action object). \n"
    "\n"
    "Menu items and submenus are referred to by a \"path\". The path is a "
    "string with this interpretation:\n"
    "\n"
    "@<table>\n"
    "  @<tr>@<td>\"\"                 @</td>@<td>is the root@</td>@</tr> \n"
    "  @<tr>@<td>\"[<path>.]<name>\"  @</td>@<td>is an element of the submenu "
    "given by <path>. If <path> is omitted, this refers to an element in the "
    "root@</td>@</tr> \n"
    "  @<tr>@<td>\"[<path>.]end\"     @</td>@<td>refers to the item past the "
    "last item of the submenu given by <path> or root@</td>@</tr>\n"
    "  @<tr>@<td>\"[<path>.]begin\"   @</td>@<td>refers to the first item of "
    "the submenu given by <path> or root@</td>@</tr>\n"
    "  @<tr>@<td>\"[<path>.]#<n>\"    @</td>@<td>refers to the nth item of the "
    "submenu given by <path> or root (n is an integer number)@</td>@</tr>\n"
    "@</table>\n"
    "\n"
    ...
    "A special detached menu is \"@toolbar\" which represents the tool bar of "
    "the main window. "
    "\n"
    "Menus are closely related to the \\Action class. Actions are used to "
    "represent selectable items "
    "inside menus, provide the title and other configuration settings. Actions "
    "also link the menu items "
    "with code. See the \\Action class description for further details.\n");

Class<lay::Action> decl_ActionBase(
    "lay", "ActionBase",
    method("title=", &lay::Action::set_title, gsi::arg("title"),
        ...
        method("shortcut=",
               (void (lay::Action::*)(const std::string &)) &
                   lay::Action::set_shortcut,
               gsi::arg("shortcut"),
               "@brief Sets the keyboard shortcut\n"
               "If the shortcut string is empty, the default shortcut will be "
               "used. If the string "
               "is equal to \\Action#NoKeyBound, no keyboard shortcut will be "
               "assigned.\n"
               "\n"
               "@param shortcut The keyboard shortcut in Qt notation (i.e. "
               "\"Ctrl+C\")\n"
               "\n"
               "The NoKeyBound option has been added in version 0.26.") +
        constant("NoKeyBound", &lay::Action::no_shortcut,
                 "@brief Gets a shortcut value indicating that no shortcut "
                 "shall be assigned\n"
                 "This method has been introduced in version 0.26.") +
        method("shortcut", &lay::Action::get_shortcut,
               "@brief Gets the keyboard shortcut\n"
               "@return The keyboard shortcut as a string\n") +
        method("default_shortcut=",
               (void (lay::Action::*)(const std::string &)) &
                   lay::Action::set_default_shortcut,
               gsi::arg("shortcut"),
        ...
        method("effective_shortcut", &lay::Action::get_effective_shortcut,
               ...
               "\n"
               "The effective shortcut is the one that is taken. It's either "
               "\\shortcut or \\default_shortcut.\n"
               "\n"
               "This attribute has been introduced in version 0.25.\n") +
        method(
            "is_separator?", &lay::Action::is_separator,
            "@brief Gets a value indicating whether the item is a separator\n"
            "This method has been introduced in version 0.25.\n") +
        method(
            "is_checkable?", &lay::Action::is_checkable,
        ...
        method("is_visible?", &lay::Action::is_visible,
               ...
               "The visibility combines with \\is_hidden?. To get the true "
               "visiblity, use \\is_effective_visible?.") +
        method("is_hidden?", &lay::Action::is_hidden,
               "@brief Gets a value indicating whether the item is hidden\n"
               "If an item is hidden, it's always hidden and \\is_visible? "
               "does not have an effect."
               "\n"
               "This attribute has been introduced in version 0.25.\n") +
        method("is_effective_visible?", &lay::Action::is_effective_visible,
               "@brief Gets a value indicating whether the item is really "
               "visible\n"
               "This is the combined visibility from \\is_visible? and "
               "\\is_hidden? and dynamic visibility (\\wants_visible)."
               "\n"
               "This attribute has been introduced in version 0.25.\n") +
        method("is_effective_enabled?", &lay::Action::is_effective_enabled,
               "@brief Gets a value indicating whether the item is really "
               "enabled\n"
               "This is the combined value from \\is_enabled? and dynamic "
               "value (\\wants_enabled)."
               "\n"
        ...
        method("icon_text=", &lay::Action::set_icon_text, gsi::arg("icon_text"),
               ...
               "\n"
               "If an icon text is set, this will be used for the text below "
               "the icon.\n"
               "If no icon text is set, the normal text will be used for the "
               "icon.\n"
        ...
        method("tool_tip=", &lay::Action::set_tool_tip, gsi::arg("text"),
               ...
               "\n"
               "The tool tip text is displayed in the tool tip window of the "
               "menu entry.\n"
        ...
        gsi::event("on_triggered", &ActionStub::on_triggered_event,
                   ...
                   "\n"
                   "This event has been introduced in version 0.21 and moved "
                   "to the ActionBase class in 0.28.\n") +
        gsi::event("on_menu_opening", &ActionStub::on_menu_opening_event,
                   "@brief This event is called if the menu item is a sub-menu "
                   "and before the menu is opened.\n"
                   "\n"
                   "This event provides an opportunity to populate the menu "
                   "before it is opened.\n"
                   "\n"
...
Class<ActionStub> decl_Action(
    decl_ActionBase, "lay", "Action",
    gsi::callback(
        "triggered", &ActionStub::triggered, &ActionStub::triggered_cb,
        ...
        gsi::callback(
            "menu_opening", &ActionStub::menu_opening,
            &ActionStub::menu_opening_cb,
            "@brief This method is called if the menu item is a sub-menu and "
        ...
        gsi::callback("wants_visible", &ActionStub::wants_visible,
                      &ActionStub::wants_visible_cb,
                      "@brief Returns a value whether the action wants to "
                      "become visible\n"
                      "This is a dynamic query for visibility which the system "
                      "uses to dynamically show or hide "
                      "menu items, for example in the MRU lists. This "
                      "visibility information is evaluated in addition "
                      "to \\is_visible? and \\is_hidden? and contributes to "
                      "the effective visibility status from "
                      "\\is_effective_visible?.\n"
                      "\n"
                      "This feature has been introduced in version 0.28.\n") +
        gsi::callback("wants_enabled", &ActionStub::wants_enabled,
                      &ActionStub::wants_enabled_cb,
                      "@brief Returns a value whether the action wants to "
                      "become enabled.\n"
                      "This is a dynamic query for enabled state which the "
                      "system uses to dynamically show or hide "
                      "menu items. This information is evaluated in addition "
                      "to \\is_enabled? and contributes to the effective "
                      "enabled status from "
    ...
    "\n"
    "Actions act as a generalization of menu entries. The action provides the "
    "appearance of a menu "
    "entry such as title, key shortcut etc. and dispatches the menu events. "
    "The action can be manipulated "
    "to change to appearance of a menu entry and can be attached an observer "
    "that receives the events "
    "when the menu item is selected.\n"
    "\n"
    "Multiple action objects can refer to the same action internally, in which "
    "case the information and event handler is copied between the "
    "incarnations. This way, a single implementation "
    "can be provided for multiple places where an action appears, for example "
    "inside the toolbar and "
    "in addition as a menu entry. Both actions will shared the same icon, "
    "text, shortcut etc.\n"
    "\n"
    "Actions are mainly used for providing new menu items inside the "
    "\\AbstractMenu class. This is some sample Ruby code for that case:\n"
    "\n"
    "@code\n"
    "a = RBA::Action.new\n"
    "a.title = \"Push Me!\"\n"
    ...
    "@/code\n"
    "\n"
    "This code will register a custom action in the toolbar. When the toolbar "
    "button is pushed "
    "a message is printed. The toolbar is addressed by a path starting with "
    "the pseudo root \"@toolbar\".\n"
    "\n"
    "In Version 0.23, the Action class has been merged with the ActionBase "
    "class.\n");
