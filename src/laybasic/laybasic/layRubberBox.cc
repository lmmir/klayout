...
RubberBox::RubberBox(lay::ViewObjectUI *widget, unsigned int color,
                     const db::DPoint &p1, const db::DPoint &p2)
    : lay::ViewObject(widget, false /*not static*/), m_p1(p1), m_p2(p2),
      m_color(color), m_stipple(0) {}
...
void RubberBox::render(const Viewport &vp, ViewObjectCanvas &canvas) {
  ...
  lay::CanvasPlane *plane = canvas.plane(lay::ViewOp(
      m_color, lay::ViewOp::Copy, 0, m_stipple, 0, lay::ViewOp::Rect, lw));
  if (plane) {
