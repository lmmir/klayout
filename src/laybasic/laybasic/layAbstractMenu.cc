...
std::string pack_key_binding(
    const std::vector<std::pair<std::string, std::string>> &unpacked) {
  std::string packed;
...
std::string pack_menu_items_hidden(
    const std::vector<std::pair<std::string, bool>> &unpacked) {
  std::string packed;

  for (std::vector<std::pair<std::string, bool>>::const_iterator p =
           unpacked.begin();
       p != unpacked.end(); ++p) {
    if (!packed.empty()) {
      packed += ";";
...
static void parse_menu_title(const std::string &s, std::string &title,
                             std::string &shortcut, std::string &icon_res,
                             std::string &tool_tip) {
...
AbstractMenuItem::AbstractMenuItem(Dispatcher *dispatcher)
    : mp_action(new Action()), mp_dispatcher(dispatcher), m_has_submenu(false),
      m_remove_on_empty(false) {
  //  ... nothing yet ..
}

AbstractMenuItem::AbstractMenuItem(const AbstractMenuItem &item)
    : mp_action(new Action()), mp_dispatcher(item.dispatcher()),
      m_has_submenu(false), m_remove_on_empty(false) {
  //  ... nothing yet ..
}

void AbstractMenuItem::setup_item(const std::string &pn, const std::string &s,
                                  Action *a) {
  m_basename.clear();
...
void AbstractMenuItem::set_action(Action *a, bool copy_properties) {
  tl_assert(a != 0);
...
void AbstractMenuItem::set_action_title(const std::string &s) {
  mp_action->set_title(s);
}

void AbstractMenuItem::set_has_submenu() { m_has_submenu = true; }

void AbstractMenuItem::set_remove_on_empty() { m_remove_on_empty = true; }
 ...
 */
class ActionObject : public QAction {
public:
  ...
  bool event(QEvent *e) {
    if (e->type() == QEvent::Shortcut) {
      ...
      if (se->isAmbiguous() && sp_actionHandles) {
        ...
        for (std::set<Action *>::const_iterator a = sp_actionHandles->begin();
             a != sp_actionHandles->end(); ++a) {
          if (!(*a)->qaction()->shortcut().isEmpty() &&
              (*a)->qaction()->shortcut().matches(se->key()) !=
...
static size_t id_from_action(QAction *action) {
...
Action::Action()
    :
#if defined(HAVE_QT)
      mp_menu(0), mp_action(lay::has_gui() ? new ActionObject(0) : 0),
#endif
      m_checked(false), m_checkable(false), m_enabled(true), m_separator(false),
      mp_dispatcher(0), m_owned(true), m_visible(true), m_hidden(false),
  ...
  if (mp_action) {
    connect(mp_action, SIGNAL(destroyed(QObject *)), this,
            SLOT(was_destroyed(QObject *)));
...
Action::Action(QAction *action, bool owned)
    : mp_menu(0), mp_action(action), m_checked(false), m_checkable(false),
      m_enabled(true), m_separator(false), mp_dispatcher(0), m_owned(owned),
      m_visible(true), m_hidden(false), m_no_key_sequence(false) {
  if (!sp_actionHandles) {
    sp_actionHandles = new std::set<Action *>();
  }
  sp_actionHandles->insert(this);
...
Action::Action(QMenu *menu, bool owned)
    : mp_menu(menu), mp_action(menu->menuAction()), m_checked(false),
      m_checkable(false), m_enabled(true), m_separator(false), mp_dispatcher(0),
      m_owned(owned), m_visible(true), m_hidden(false),
      m_no_key_sequence(false) {
  if (!sp_actionHandles) {
    sp_actionHandles = new std::set<Action *>();
  }
  ...
  //  catch the destroyed signal to tell if the QAction object is deleted.
  connect(mp_menu, SIGNAL(destroyed(QObject *)), this,
          SLOT(was_destroyed(QObject *)));
...
void Action::set_dispatcher(Dispatcher *dispatcher) {
...
void Action::configure_from_title(const std::string &s) {
  std::string title;
  std::string shortcut;
  std::string res;
  std::string tool_tip;
...
#if defined(HAVE_QT)
void Action::menu_about_to_show() {
...
void Action::menu_opening() {
  //  .. no action yet, the reimplementation must provide some ..
}

#if defined(HAVE_QT)
QAction *Action::qaction() const { return mp_action; }

QMenu *Action::menu() const { return mp_menu; }

void Action::set_menu(QMenu *menu, bool owned) {
...
void Action::was_destroyed(QObject *obj) {
  if (obj == mp_action) {
    mp_action = 0;
  }
  if (obj == mp_menu) {
...
#if defined(HAVE_QT)
void Action::configure_action(QAction *target) const {
...
void Action::set_visible(bool v) {
  if (m_visible != v) {
    m_visible = v;
    sync_qaction();
  }
...
bool Action::is_visible() const { return m_visible; }

bool Action::is_hidden() const { return m_hidden; }

bool Action::is_effective_visible() const {
  return m_visible && !m_hidden && wants_visible();
}

void Action::set_default_shortcut(const std::string &sc) {
...
void Action::set_shortcut(const std::string &sc) {
  if (m_shortcut != sc) {
    m_shortcut = sc;
    m_no_key_sequence = (sc == Action::no_shortcut());
#if defined(HAVE_QT)
    m_key_sequence = m_no_key_sequence
                         ? QKeySequence()
                         : QKeySequence(tl::to_qstring(m_shortcut));
    if (mp_action) {
      mp_action->setShortcut(get_key_sequence());
    }
#endif
...
std::string Action::get_default_shortcut() const { return m_default_shortcut; }

std::string Action::get_shortcut() const {
  return m_no_key_sequence ? Action::no_shortcut() : m_shortcut;
}

#if defined(HAVE_QT)
QKeySequence Action::get_key_sequence() const {
  if (m_hidden) {
...
QKeySequence Action::get_key_sequence_for(const std::string &sc) const {
  if (m_hidden) {
    //  A hidden menu item does not have a key sequence too.
    return QKeySequence();
  } else if (sc.empty()) {
...
const std::string &Action::no_shortcut() {
  static const std::string no_shortcut("none");
  return no_shortcut;
}

void Action::set_title(const std::string &t) {
#if defined(HAVE_QT)
  if (qaction()) {
...
std::string Action::get_title() const { return m_title; }

std::string Action::get_effective_shortcut() const {
...
std::string Action::get_effective_shortcut_for(const std::string &sc) const {
  if (m_hidden) {
    //  A hidden menu item does not have a key sequence too.
    return std::string();
  } else if (sc.empty()) {
...
void Action::add_to_exclusive_group(lay::AbstractMenu *menu,
...
bool Action::is_checkable() const { return m_checkable; }
...
bool Action::is_enabled() const { return m_enabled; }

bool Action::is_separator() const { return m_separator; }
...
bool Action::is_effective_enabled() const {
  return m_enabled && wants_enabled();
}

void Action::set_checked(bool c) {
#if defined(HAVE_QT)
  if (qaction()) {
...
void Action::set_checkable(bool c) {
#if defined(HAVE_QT)
  if (qaction()) {
...
void Action::set_separator(bool s) {
#if defined(HAVE_QT)
  if (qaction()) {
...
void Action::set_icon(const std::string &filename) {
#if defined(HAVE_QT)
  if (qaction()) {
    if (filename.empty()) {
...
#if defined(HAVE_QT)
void Action::set_qicon(const QIcon &icon) {
  if (qaction()) {
...
std::string Action::get_tool_tip() const { return m_tooltip; }

void Action::set_tool_tip(const std::string &text) {
#if defined(HAVE_QT)
  if (qaction()) {
    if (text.empty()) {
...
std::string Action::get_icon_text() const { return m_icontext; }

void Action::set_icon_text(const std::string &icon_text) {
#if defined(HAVE_QT)
  if (qaction()) {
    if (icon_text.empty()) {
...
void Action::set_object_name(const std::string &name) {
#if defined(HAVE_QT)
  if (qaction()) {
...
ConfigureAction::ConfigureAction(const std::string &cname,
                                 const std::string &cvalue)
    : Action(), m_cname(cname), m_cvalue(cvalue),
      m_type(ConfigureAction::setter_type) {
  if (cvalue == "?") {
    m_type = boolean_type;
    set_checkable(true);
  }
...
ConfigureAction::ConfigureAction(const std::string &title,
                                 const std::string &cname,
                                 const std::string &cvalue)
    : Action(title), m_cname(cname), m_cvalue(cvalue),
      m_type(ConfigureAction::setter_type) {
  if (cvalue == "?") {
...
void ConfigureAction::triggered() {
  if (dispatcher()) {
...
void ConfigureAction::configure(const std::string &value) {
  if (m_type == boolean_type) {
...
#if defined(HAVE_QT)
QActionGroup *AbstractMenu::make_exclusive_group(const std::string &name) {
  std::map<std::string, QActionGroup *>::const_iterator a =
      m_action_groups.find(name);
...
void AbstractMenu::build_detached(const std::string &name, QFrame *mbar) {
  //  Clean up the menu bar before rebuilding
  if (mbar->layout()) {
    delete mbar->layout();
  }
  QObjectList children = mbar->children();
  for (QObjectList::const_iterator c = children.begin(); c != children.end();
       ++c) {
  ...
  for (std::list<AbstractMenuItem>::iterator c = item->children.begin();
       c != item->children.end(); ++c) {
...
static QAction *insert_action_after(QWidget *widget, QAction *after,
                                    QAction *action) {
...
void AbstractMenu::build(QMenuBar *mbar, QToolBar *tbar) {
  if (tbar) {
    tbar->clear();
  }
  ...
  for (std::list<AbstractMenuItem>::iterator c = m_root.children.begin();
       c != m_root.children.end(); ++c) {
    ...
    if (c->has_submenu()) {
      ...
      if (c->name() == "@toolbar") {
        ...
        // HINT: it is necessary to add the menu action to a widget below the
        // main window. Otherwise, the keyboard shortcuts do not work for menu
        // items inside such a popup menu. It seems not to have a negative
        // effect to add the menu to the main widget.
        if (mp_dispatcher->menu_parent_widget()) {
          mp_dispatcher->menu_parent_widget()->addAction(
              c->menu()->menuAction());
        }
        ...
        if (c->menu() == 0) {
          ...
          prev_action =
              insert_action_after(mbar, prev_action, menu->menuAction());
        } else {
          //  Move the action to the end if present in the menu already
          std::set<std::pair<size_t, QAction *>>::iterator a =
              present_actions.find(
                  std::make_pair(id_from_action(c->menu()->menuAction()),
                                 c->menu()->menuAction()));
          if (a != present_actions.end()) {
            ...
            prev_action =
                insert_action_after(mbar, prev_action, c->menu()->menuAction());
          }
        }
      ...
      //  Move the action to the end if present in the menu already
      std::set<std::pair<size_t, QAction *>>::iterator a =
          present_actions.find(std::make_pair(
              id_from_action(c->action()->qaction()), c->action()->qaction()));
      if (a != present_actions.end()) {
        if (s_can_move_menu) {
          mbar->removeAction(a->second);
          insert_action_after(mbar, prev_action, a->second);
        ...
        prev_action =
            insert_action_after(mbar, prev_action, c->action()->qaction());
      }
    }
  }
  ...
  if (mbar) {
    for (std::set<std::pair<size_t, QAction *>>::iterator a =
             present_actions.begin();
         a != present_actions.end(); ++a) {
      mbar->removeAction(a->second);
    }
  }
}

void AbstractMenu::build(QMenu *m, std::list<AbstractMenuItem> &items) {
  std::set<std::pair<size_t, QAction *>> present_actions;
  ...
  for (std::list<AbstractMenuItem>::iterator c = items.begin();
       c != items.end(); ++c) {
    ...
    if (c->has_submenu()) {
      ...
      if (!c->menu()) {
        ...
        if (a != present_actions.end()) {
          ...
          prev_action =
              insert_action_after(m, prev_action, c->menu()->menuAction());
        }
      }
      ...
      if (a != present_actions.end()) {
        ...
        prev_action =
            insert_action_after(m, prev_action, c->action()->qaction());
      }
    }
  }
...
void AbstractMenu::build(QToolBar *t, std::list<AbstractMenuItem> &items) {
  for (std::list<AbstractMenuItem>::iterator c = items.begin();
       c != items.end(); ++c) {
...
QMenu *AbstractMenu::detached_menu(const std::string &name) {
  AbstractMenuItem *item = find_item_exact("@" + name);
  tl_assert(item != 0);
  return item->menu();
}

QMenu *AbstractMenu::menu(const std::string &path) {
...
bool AbstractMenu::is_valid(const std::string &path) const {
  const AbstractMenuItem *item = find_item_exact(path);
  return item != 0;
}

bool AbstractMenu::is_menu(const std::string &path) const {
  const AbstractMenuItem *item = find_item_exact(path);
  return item != 0 && item->has_submenu();
}

bool AbstractMenu::is_separator(const std::string &path) const {
  const AbstractMenuItem *item = find_item_exact(path);
...
std::vector<std::string> AbstractMenu::items(const std::string &path) const {
  std::vector<std::string> res;

  const AbstractMenuItem *item = find_item_exact(path);
  if (item) {
    res.reserve(item->children.size());
    for (std::list<AbstractMenuItem>::const_iterator c = item->children.begin();
         c != item->children.end(); ++c) {
...
void AbstractMenu::insert_item(const std::string &p, const std::string &name,
                               Action *action) {
  ...
  while (!extr.at_end()) {
    ...
    if (!path.empty()) {
      ...
      for (std::list<AbstractMenuItem>::iterator existing =
               parent->children.begin();
           existing != parent->children.end();) {
...
void AbstractMenu::insert_separator(const std::string &p,
...
void AbstractMenu::insert_menu(const std::string &p, const std::string &name,
                               Action *action) {
#if defined(HAVE_QT)
...
void AbstractMenu::insert_menu(const std::string &path, const std::string &name,
                               const std::string &title) {
  insert_menu(path, name, new Action(title));
}

void AbstractMenu::clear_menu(const std::string &p) {
  typedef std::vector<
      std::pair<AbstractMenuItem *, std::list<AbstractMenuItem>::iterator>>
      path_type;
  tl::Extractor extr(p.c_str());
...
void AbstractMenu::delete_item(const std::string &p) {
  typedef std::vector<
      std::pair<AbstractMenuItem *, std::list<AbstractMenuItem>::iterator>>
      path_type;
  tl::Extractor extr(p.c_str());
  ...
  if (!path.empty()) {
    ...
    for (path_type::const_reverse_iterator p = path.rbegin(); p != path.rend();
         ++p) {

      if (p->second == p->first->children.end()) {
        break;
      } else if (p != path.rbegin() && (!p->second->remove_on_empty() ||
                                        !p->second->children.empty())) {
...
static void do_delete_items(AbstractMenuItem &parent, Action *action) {
  for (std::list<AbstractMenuItem>::iterator l = parent.children.begin();
       l != parent.children.end();) {
...
void AbstractMenu::delete_items(Action *action) {
  if (action) {
...
AbstractMenu::find_item_for_action(const Action *action,
                                   const AbstractMenuItem *from) const {
  return (const_cast<AbstractMenu *>(this))
      ->find_item_for_action(action, const_cast<AbstractMenuItem *>(from));
}

AbstractMenuItem *AbstractMenu::find_item_for_action(const Action *action,
                                                     AbstractMenuItem *from) {
  if (!from) {
...
AbstractMenuItem *AbstractMenu::find_item_exact(const std::string &path) {
  ...
  while (!extr.at_end()) {
    ...
    if (extr.test("#")) {
      ...
      item = 0;
      for (std::list<AbstractMenuItem>::iterator c = p->children.begin();
           c != p->children.end() && item == 0; ++c) {
        if (c->name() == name) {
          item = &*c;
        }
      }
...
std::vector<std::string> AbstractMenu::group(const std::string &name) const {
...
AbstractMenu::group_actions(const std::string &name) {
  ...
  for (std::vector<std::string>::const_iterator g = grp.begin(); g != grp.end();
       ++g) {
...
void AbstractMenu::collect_group(std::vector<std::string> &grp,
                                 const std::string &name,
                                 const AbstractMenuItem &item) const {
  for (std::list<AbstractMenuItem>::const_iterator c = item.children.begin();
       c != item.children.end(); ++c) {
...
void AbstractMenu::emit_changed() {
  m_config_actions_valid = false;
#if defined(HAVE_QT)
  emit changed();
#endif
...
std::vector<ConfigureAction *>
AbstractMenu::configure_actions(const std::string &name) {
  if (!m_config_actions_valid) {
    ...
    for (std::vector<lay::ConfigureAction *>::const_iterator c = ca.begin();
         c != ca.end(); ++c) {
  ...
  std::map<std::string, std::vector<lay::ConfigureAction *>>::const_iterator c =
      m_config_action_by_name.find(name);
  return c == m_config_action_by_name.end()
             ? std::vector<lay::ConfigureAction *>()
             : c->second;
}

void AbstractMenu::collect_configure_actions(
    std::vector<lay::ConfigureAction *> &ca, AbstractMenuItem &item) {
  for (std::list<AbstractMenuItem>::iterator c = item.children.begin();
       c != item.children.end(); ++c) {
...
void AbstractMenu::get_shortcuts(const std::string &root,
  ...
  for (std::vector<std::string>::const_iterator i = items.begin();
    ...
    if (i->size() > 0) {
      if (is_valid(*i) && action(*i)->is_visible()) {
        if (is_menu(*i)) {
          //  a menu must be listed (so it can be hidden), but does not have a
          //  shortcut but we don't include special menus
          if (i->at(0) != '@') {
          ...
          bindings.insert(std::make_pair(
              *i, with_defaults ? action(*i)->get_default_shortcut()
                                : action(*i)->get_effective_shortcut()));
        }
      }
    }
  }
