...
Bitmap::Bitmap(unsigned int w, unsigned int h, double r) : m_empty_scanline(0) {
  init(w, h);
  m_resolution = r;
}

Bitmap::Bitmap(const Bitmap &d) : m_empty_scanline(0) {
  init(d.m_width, d.m_height);
  operator=(d);
}

Bitmap &Bitmap::operator=(const Bitmap &d) {
  if (&d != this) {
...
void Bitmap::clear() {
  for (std::vector<uint32_t *>::iterator i = m_scanlines.begin();
       i != m_scanlines.end(); ++i) {
    if (*i) {
      m_free.push_back(*i);
    }
...
void Bitmap::cleanup() {
  ...
  for (std::vector<uint32_t *>::iterator i = m_free.begin(); i != m_free.end();
       ++i) {
    delete[] * i;
  }
  m_free.clear();
...
void Bitmap::fill_pattern(int y, int x, const uint32_t *pp, unsigned int stride,
                          unsigned int n) {
  if (x < int(m_width)) {
      ...
      if (!e->is_horizontal()) {
        ...
        if (c == 0) { //  this is implementing the != 0 rule
          ...
          if (e->pos() > 0) {
            ...
            fill(yint, x1int,
                 (unsigned int)std::min(double(width() - 1), e->pos()) + 1);
          }
          x1set = false;
        } else if (!x1set) {
          x1 = e->pos();
      ...
      if (!e->is_horizontal()) {
        ...
        if (c == 0) { //  this is implementing the != 0 rule
          if (e->x1() > 0) {
            ...
            fill(yint, x1int,
                 (unsigned int)std::min(double(width() - 1), e->x1()) + 1);
          }
          x1set = false;
        } else if (!x1set) {
          x1 = e->x1();
...
void Bitmap::render_vertices(std::vector<lay::RenderEdge> &edges, int mode) {
  ...
  for (std::vector<lay::RenderEdge>::iterator e = edges.begin();
       e != edges.end(); ++e) {
  ...
  for (std::vector<lay::RenderEdge>::iterator e = edges.begin();
    ...
    if (!e->is_horizontal()) {
      ...
      if (e->y1() < double(height()) - 0.5 && e->y2() >= -0.5 &&
          x < (double)width() - 0.5 && x >= -0.5) {

        unsigned int xint = (unsigned int)(std::max(
            0.0, std::min(double(width() - 1), x) + 0.5));
        unsigned int yint = (unsigned int)std::max(floor(e->y1() + 0.5), 0.0);
        unsigned int yeint = (unsigned int)std::min(
            double(height() - 1), std::max(floor(e->y2() + 0.5), 0.0));
      ...
      double y = e->y1();
      if (y < (double)height() - 0.5 && y >= -0.5 &&
          x1 < (double)width() - 0.5 && x2 >= -0.5) {

        unsigned int x1int = (unsigned int)(std::max(
            0.0, std::min(double(width() - 1), x1) + 0.5));
        unsigned int x2int = (unsigned int)(std::max(
            0.0, std::min(double(width() - 1), x2) + 0.5));
...
void Bitmap::render_contour(std::vector<lay::RenderEdge> &edges) {
  ...
  for (std::vector<lay::RenderEdge>::iterator e = edges.begin();
    ...
    if (e->y1() < double(height()) - 0.5 && e->y2() >= -0.5) {
      ...
      unsigned int xint =
          (unsigned int)(std::max(0.0, std::min(double(width() - 1), x) + 0.5));
...
void Bitmap::render_text(const lay::RenderText &text) {
  if (text.font == db::DefaultFont) {
    ...
    while (*cp1) {
      ...
      if (y > -0.5 && y < double(height() + ff.height() - 1) - 0.5) {
        ...
        while (cp1 != cp) {
          ...
          if (c < uint32_t(ff.first_char()) ||
              c >= uint32_t(ff.n_chars()) + ff.first_char()) {
            //  NOTE: '?' needs to be a valid character always
            c = uint32_t('?');
          }

          if (xx > -100.0 && xx < double(width())) {
            fill_pattern(int(y + 0.5), int(floor(xx)),
                         ff.data() +
                             (c - ff.first_char()) * ff.height() * ff.stride(),
                         ff.stride(), ff.height());
          }
    ...
    ht.justify(text.b.transformed(text.trans.inverted()), text.halign,
               text.valign);
