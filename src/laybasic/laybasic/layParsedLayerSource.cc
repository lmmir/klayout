...
public:
  ...
  virtual bool
  check(const db::PropertiesRepository &rep,
        const db::PropertiesRepository::properties_set &set) const = 0;
  virtual bool selection(const db::PropertiesRepository &rep,
                         std::set<db::properties_id_type> &ids) const = 0;
};

/**
 *  @brief A expression graph node combining n arguments with either a "and" or
 * "or" operation
 */
class PropertySelectorOp : public PropertySelectorBase {
public:
  ...
  PropertySelectorOp(const PropertySelectorOp &d) : m_op(d.m_op) {
    m_args.reserve(d.m_args.size());
    for (std::vector<const PropertySelectorBase *>::const_iterator b =
             d.m_args.begin();
         b != d.m_args.end(); ++b) {
  ...
  ~PropertySelectorOp() {
    for (std::vector<const PropertySelectorBase *>::iterator b = m_args.begin();
         b != m_args.end(); ++b) {
  ...
  void add_arg(const PropertySelectorBase *arg) { m_args.push_back(arg); }

  PropertySelectorBase *clone() const { return new PropertySelectorOp(*this); }

  void join(const PropertySelectorBase *other) {
    const PropertySelectorOp *op =
        dynamic_cast<const PropertySelectorOp *>(other);
  ...
  op_type_enum op_type() const { return m_op; }

  bool check(const db::PropertiesRepository &rep,
             const db::PropertiesRepository::properties_set &set) const {
    if (m_op == And) {
      for (std::vector<const PropertySelectorBase *>::const_iterator b =
               m_args.begin();
           b != m_args.end(); ++b) {
  ...
  bool selection(const db::PropertiesRepository &rep,
                 std::set<db::properties_id_type> &ids) const {
    //  this algorithm computes the "or" of two sets by using this relationship:
    //  a or b or c or .. = !((!a) and (!b) and (!c) and ..)

    //  get the selection of the first operand into ids
    std::vector<const PropertySelectorBase *>::const_iterator b =
        m_args.begin();
    ...
    for (++b; b != m_args.end() && !(ids.empty() && !inv); ++b) {
      ...
      if (ids2.empty() && !inv2) {
        //  shortcut: if the second operand is empty, just clear and terminate
        //  the loop then
        ids.clear();
        inv = false;
      } else if (!inv && !inv2) {
        for (std::set<db::properties_id_type>::iterator id = ids.begin();
             id != ids.end();) {
        ...
        for (std::set<db::properties_id_type>::iterator id = ids2.begin();
             id != ids2.end(); ++id) {
          ids.insert(*id);
        }
      } else {
  ...
  unsigned int type_id() const { return m_op == And ? 1 : 2; }
 ...
 */
class PropertySelectorNot : public PropertySelectorBase {
public:
  PropertySelectorNot(PropertySelectorBase *arg) : mp_arg(arg) {
    //  .. nothing yet ..
  }
  ...
  unsigned int type_id() const { return 10; }

  int compare(const PropertySelectorBase *b) const {
    if (type_id() != b->type_id()) {
      return type_id() < b->type_id() ? -1 : 1;
    }
    const PropertySelectorNot *bb =
        dynamic_cast<const PropertySelectorNot *>(b);
 ...
 */
class PropertySelectorEqual : public PropertySelectorBase {
public:
  PropertySelectorEqual(const tl::Variant &name, const tl::Variant &value,
                        bool equal)
  ...
  bool check(const db::PropertiesRepository &rep,
    ...
    db::PropertiesRepository::properties_set::const_iterator i =
        set.find(p.second);
    if (i == set.end()) {
  ...
  bool selection(const db::PropertiesRepository &rep,
    ...
    const db::PropertiesRepository::properties_id_vector &idv =
        rep.properties_ids_by_name_value(std::make_pair(p.second, m_value));
    for (db::PropertiesRepository::properties_id_vector::const_iterator id =
             idv.begin();
         id != idv.end(); ++id) {
      ids.insert(*id);
    }
  ...
  unsigned int type_id() const { return m_equal ? 20 : 21; }

  int compare(const PropertySelectorBase *b) const {
    if (type_id() != b->type_id()) {
      return type_id() < b->type_id() ? -1 : 1;
    }
    const PropertySelectorEqual *bb =
        dynamic_cast<const PropertySelectorEqual *>(b);
    if (bb) {
      if (m_name != bb->m_name) {
 ...
 */
static PropertySelectorBase *extract_base(tl::Extractor &ex) {
  bool eq = true;
  tl::Variant n, v;
  ex.read(n);
  if (ex.test("==")) {
 ...
 */
static PropertySelectorBase *extract_element(tl::Extractor &ex) {
  if (ex.test("(")) {
 ...
 */
static PropertySelectorBase *extract_unary(tl::Extractor &ex) {
  if (ex.test("!")) {
 ...
 */
static PropertySelectorBase *extract_or_seq(tl::Extractor &ex) {
  PropertySelectorBase *expr = extract_unary(ex);
  if (ex.test("||")) {
    PropertySelectorOp *op =
        new PropertySelectorOp(PropertySelectorOp::Or, expr);
    expr = op;
    do {
 ...
 */
static PropertySelectorBase *extract_and_seq(tl::Extractor &ex) {
  PropertySelectorBase *expr = extract_or_seq(ex);
  if (ex.test("&&")) {
    PropertySelectorOp *op =
        new PropertySelectorOp(PropertySelectorOp::And, expr);
    expr = op;
    do {
...
PropertySelector::PropertySelector() : mp_base(0) {
  //  .. nothing yet ..
}

PropertySelector::PropertySelector(const PropertySelector &sel) : mp_base(0) {
  operator=(sel);
}

PropertySelector &PropertySelector::operator=(const PropertySelector &sel) {
  if (this != &sel) {
    if (mp_base) {
      delete mp_base;
      mp_base = 0;
...
std::string PropertySelector::to_string(size_t max_len) const {
  if (mp_base) {
...
bool PropertySelector::check(const db::PropertiesRepository &rep,
...
bool PropertySelector::matching(const db::PropertiesRepository &rep,
                                std::set<db::properties_id_type> &ids) const {
  if (is_null()) {
    return true;
  } else {
...
PartialTreeSelector::PartialTreeSelector(const db::Layout &layout,
...
PartialTreeSelector::PartialTreeSelector(const PartialTreeSelector &d)
    : mp_layout(d.mp_layout), m_state(d.m_state), m_selected(d.m_selected),
      m_state_stack(d.m_state_stack), m_selected_stack(d.m_selected_stack),
      m_state_machine(d.m_state_machine) {
  //  .. nothing yet ..
}

PartialTreeSelector &
PartialTreeSelector::operator=(const PartialTreeSelector &d) {
  if (this != &d) {
...
int PartialTreeSelector::is_child_selected(db::cell_index_type child) const {
  if (m_state >= 0 && m_state < int(m_state_machine.size())) {
    ...
    const std::map<db::cell_index_type, std::pair<int, int>> &m =
        m_state_machine[m_state];

    std::map<db::cell_index_type, std::pair<int, int>>::const_iterator i =
        m.find(child);
    if (i == m.end()) {
    ...
    if (i != m.end()) {
      ...
      if (i->second.first < 0 ||
          i->second.first >= int(m_state_machine.size())) {
        return sel ? 1 : 0;
      } else {
        return sel ? 1 : -1;
      }
...
void PartialTreeSelector::add_state_transition(int initial_state,
                                               db::cell_index_type cell_index,
                                               int target_state, int selected) {
  if (initial_state >= 0) {
    while (int(m_state_machine.size()) <= initial_state) {
      m_state_machine.push_back(
          std::map<db::cell_index_type, std::pair<int, int>>());
    }
    m_state_machine[initial_state][cell_index] =
        std::make_pair(target_state, selected);
  }
}
...
void PartialTreeSelector::add_state_transition(int initial_state,
  ...
  if (initial_state >= 0) {
    ...
    m_state_machine[initial_state][db::cell_index_type(-1)] =
        std::make_pair(target_state, selected);
  }
}
...
CellSelector::CellSelector(const CellSelector &d) : m_selectors(d.m_selectors) {
  //  .. nothing yet ..
}
...
std::string CellSelector::to_string() const {
  ...
  for (std::vector<std::vector<std::pair<bool, std::string>>>::const_iterator
           i = m_selectors.begin();
       i != m_selectors.end(); ++i) {
    ...
    for (std::vector<std::pair<bool, std::string>>::const_iterator j =
             i->begin();
         j != i->end(); ++j) {
      if (j != i->begin()) {
        r += " ";
...
PartialTreeSelector
CellSelector::create_tree_selector(const db::Layout &layout,
  ...
  if (!m_selectors.empty() && !m_selectors.front().empty() &&
      m_selectors.front().front().first) {
    initial_sel = false;
  }

  //  if the first level matches the initial cell, use the selection state to
  //  enable this cell
  bool consume_first = false;
  if (!m_selectors.empty() && layout.is_valid_cell_index(initial_cell)) {
    for (std::vector<std::pair<bool, std::string>>::const_iterator j =
             m_selectors.front().begin();
         j != m_selectors.front().end(); ++j) {
...
ParsedLayerSource::ParsedLayerSource(const std::string &src)
    : m_has_name(false), m_special_purpose(SP_None), m_layer_index(-1),
      m_layer(-1), m_datatype(-1), m_cv_index(-1) {
...
ParsedLayerSource::ParsedLayerSource(const db::LayerProperties &lp,
                                     int cv_index)
    : m_has_name(!lp.name.empty()), m_special_purpose(SP_None),
      m_layer_index(-1), m_layer(lp.layer), m_datatype(lp.datatype),
      m_name(lp.name), m_cv_index(cv_index) {
  m_trans.push_back(db::DCplxTrans());
}

ParsedLayerSource::ParsedLayerSource(int layer, int datatype, int cv_index)
    : m_has_name(false), m_special_purpose(SP_None), m_layer_index(-1),
      m_layer(layer), m_datatype(datatype), m_cv_index(cv_index) {
  m_trans.push_back(db::DCplxTrans());
}

ParsedLayerSource::ParsedLayerSource(int layer_index, int cv_index)
    : m_has_name(false), m_special_purpose(SP_None), m_layer_index(layer_index),
      m_layer(-1), m_datatype(-1), m_cv_index(cv_index) {
  m_trans.push_back(db::DCplxTrans());
}

ParsedLayerSource::ParsedLayerSource(const std::string &name, int cv_index)
    : m_has_name(true), m_special_purpose(SP_None), m_layer_index(-1),
      m_layer(-1), m_datatype(-1), m_name(name), m_cv_index(cv_index) {
  m_trans.push_back(db::DCplxTrans());
}
...
ParsedLayerSource &ParsedLayerSource::operator=(const ParsedLayerSource &d) {
  if (this != &d) {
...
ParsedLayerSource &ParsedLayerSource::operator+=(const ParsedLayerSource &d) {
  ...
  for (std::vector<db::DCplxTrans>::const_iterator a = m_trans.begin();
       a != m_trans.end(); ++a) {
    for (std::vector<db::DCplxTrans>::const_iterator b = d.m_trans.begin();
         b != d.m_trans.end(); ++b) {
      tr.push_back(*a * *b);
    }
  }
...
hier_levels_to_string(const HierarchyLevelSelection &hier_levels) {
  ...
  if (hier_levels.has_from_level()) {
    ...
    if (hier_levels.from_level_mode() == HierarchyLevelSelection::minimum) {
      m = "<";
    } else if (hier_levels.from_level_mode() ==
               HierarchyLevelSelection::maximum) {
      m = ">";
    }
  ...
  if (hier_levels.has_to_level()) {
    ...
    if (hier_levels.to_level_mode() == HierarchyLevelSelection::minimum) {
      m = "<";
    } else if (hier_levels.to_level_mode() ==
               HierarchyLevelSelection::maximum) {
      m = ">";
    }
...
std::string ParsedLayerSource::to_string() const {
  std::string r;
  ...
  if (!m_trans.empty() &&
      (m_trans.size() > 1 || m_trans[0] != db::DCplxTrans())) {
    for (std::vector<db::DCplxTrans>::const_iterator t = m_trans.begin();
         t != m_trans.end(); ++t) {
      if (!r.empty()) {
        r += " ";
      }
...
ParsedLayerSource::display_string(const lay::LayoutViewBase *view) const {
  ...
  if (m_layer_index >= 0) {
    ...
    if (!view || m_cv_index < 0 || m_cv_index >= int(view->cellviews()) ||
        !view->cellview(m_cv_index)->layout().is_valid_layer(m_layer_index)) {
      r = tl::sprintf("%%%d", m_layer_index);
    } else {
      const db::LayerProperties &lp =
          view->cellview(m_cv_index)->layout().get_properties(m_layer_index);
      if (!lp.name.empty()) {
        r = lp.name;
  ...
  if (m_cv_index >= 0 && (!view || view->always_show_layout_index() ||
                          m_cv_index > 0 || view->cellviews() > 1)) {
    r += tl::sprintf("@%d", m_cv_index + 1);
  }
...
void ParsedLayerSource::parse_from_string(const char *cp) {
  ...
  while (!x.at_end()) {
    ...
    if (x.test("!")) {
      ...
      if (sp == "CellFrame" || sp == "cellframe" || sp == "CF" ||
          sp == "cell-frame") {
        m_special_purpose = SP_CellFrame;
      } else {
        throw tl::Exception(tl::to_string(tr("Invalid special purpose '%s'")),
                            sp);
      }
      ...
      //  The hierarchy level selection format is that (C=number of context
      //  levels, above current cell)
      //   #n           -> = #0..n
      //   #(n)         -> = #(0)..(n)
      //   #..n         -> = #0..n
      //   #..(n)       -> = #0..(n)
      ...
      //   #m..(n)      -> from=m, to=C+n
      //   #(m)..n      -> from=C+m, to=n
      //   m and n can be "<l" or ">l" to indicate minimum or maximum mode
      //   respectively

      m_hier_levels = HierarchyLevelSelection();
      if (x.test("*")) {

        m_hier_levels.set_from_level(0, false,
                                     HierarchyLevelSelection::absolute);
        m_hier_levels.set_to_level(std::numeric_limits<int>::max(), false,
                                   HierarchyLevelSelection::absolute);
        ...
        m_hier_levels.set_from_level(0, false,
                                     HierarchyLevelSelection::absolute);
        m_hier_levels.set_to_level(std::numeric_limits<int>::max(), false,
                                   HierarchyLevelSelection::minimum);

      } else if (x.test("(*)")) {

        m_hier_levels.set_from_level(0, true,
                                     HierarchyLevelSelection::absolute);
        m_hier_levels.set_to_level(std::numeric_limits<int>::max(), false,
                                   HierarchyLevelSelection::absolute);
        ...
        if (x.test("..")) {
          ...
          //  No explicit "to" spec: use "from" as "to" and put "0" into "from"
          //  place.
          bool fr = m_hier_levels.from_level_relative();
          int f = m_hier_levels.from_level();
          HierarchyLevelSelection::level_mode_type m =
              m_hier_levels.from_level_mode();
          m_hier_levels.set_to_level(f, fr, m);
          m_hier_levels.set_from_level(0, fr,
                                       HierarchyLevelSelection::absolute);
        }
      }
...
db::LayerProperties ParsedLayerSource::layer_props() const {
...
unsigned int ParsedLayerSource::color_index() const {
  if (layer() >= 0) {
...
bool ParsedLayerSource::is_wildcard_layer() const {
  return (m_special_purpose == SP_None && !has_name() && m_layer < 0 &&
          m_datatype < 0 && m_layer_index < 0);
}
