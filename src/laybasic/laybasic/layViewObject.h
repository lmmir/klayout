...
public:
   ...
   */
  virtual bool drag_enter_event(const db::DPoint & /*p*/,
                                const DragDropDataBase * /*data*/) {
    return false;
  }
   ...
   */
  virtual bool drag_move_event(const db::DPoint & /*p*/,
                               const DragDropDataBase * /*data*/) {
    return false;
  }
   ...
   */
  virtual bool drop_event(const db::DPoint & /*p*/,
                          const DragDropDataBase * /*data*/) {
    return false;
  }
#endif
   ...
   *  the mouse will receive the events with prio "true" in the reverse order
   * they grabbed the mouse (last one first). This loop will terminate if one of
   * the objects has returned "true". If no service has grabbed the mouse or
   * none of them was taking the event, the active service receives the event
   * with prio set to "true". If this is not the case, then all objects will
   * receive the event with "prio" set to "false", unless one of the objects
   *  returns "true".
   ...
   */
  virtual bool mouse_press_event(const db::DPoint & /*p*/,
   ...
   *
   *  Analogous to mouse_press_event (see above), but sent if the mouse was not
   * moved. The click event is coincident with the release of the mouse button.
   */
  virtual bool mouse_click_event(const db::DPoint & /*p*/,
                                 unsigned int /*buttons*/, bool /*prio*/) {
    return false;
  }
   ...
   *
   *  Analogous to mouse_press_event (see above), but sent if a double-click was
   * detected.
   */
   ...
   */
  virtual bool mouse_move_event(const db::DPoint & /*p*/,
                                unsigned int /*buttons*/, bool /*prio*/) {
    return false;
  }
   ...
   */
  virtual bool wheel_event(int /*delta*/, bool /*horizontal*/,
                           const db::DPoint & /*p*/, unsigned int /*buttons*/,
                           bool /*prio*/) {
    return false;
  }
   ...
   *
   *  This method can be called from a mouse event handler to set the cursor for
   * the view. It is only effective if called from a mouse event handler.
   */
   ...
   */
  ViewObjectUI *ui() const { return mp_widget; }
  ...
  /**
   *  @brief This method is called to set the background and text (foreground)
   * color
   */
...
class LAYBASIC_PUBLIC BackgroundViewObject : virtual public tl::Object {
public:
  /**
   ...
   */
  int z_order() const { return m_z_order; }
...
class LAYBASIC_PUBLIC ViewObject : public tl::Object {
public:
  /**
   *  @brief The constructor
   *
  ...
  /**
   *  @brief Gets a value indicating whether the marker can be dismissed (made
   * invisible)
   *
   *  Markers with this flag set to true can be hidden by using
   * ViewObjectCanvas::show_markers.
   */
  bool get_dismissable() const { return m_dismissable; }

  /**
   *  @brief Sets a value indicating whether the marker can be dismissed (made
   * invisible)
   *
   ...
   *
   *  This is a convenience function that avoids having to store a widget
   * pointer
   */
  void thaw();
...
public:
  ...
  typedef tl::weak_collection<BackgroundViewObject>::iterator
      background_object_iterator;
   ...
   */
  bool needs_update_static() const { return m_needs_update_static; }
   ...
   */
  bool needs_update_bg() const { return m_needs_update_bg; }
   ...
   */
  service_iterator begin_services() { return m_services.begin(); }

  /**
   *  @brief Services iterator: end
   */
  service_iterator end_services() { return m_services.end(); }
   ...
   */
  object_iterator begin_objects() { return m_objects.begin(); }

  /**
   *  @brief Objects iterator: end
   */
  object_iterator end_objects() { return m_objects.end(); }
   ...
   *  of this event.
   */
  virtual void mouse_double_click_event(const db::DPoint & /*p*/,
                                        unsigned int /*buttons*/) {}
   ...
   *  of this event.
   */
  virtual void mouse_click_event(const db::DPoint & /*p*/,
                                 unsigned int /*buttons*/) {}
   ...
   *  of this event.
   */
  virtual void mouse_press_event(const db::DPoint & /*p*/,
                                 unsigned int /*buttons*/) {}
   ...
   *  of this event.
   */
  virtual void mouse_release_event(const db::DPoint & /*p*/,
                                   unsigned int /*buttons*/) {}
   ...
   *  of this event.
   */
  virtual void mouse_move_event(const db::DPoint & /*p*/,
                                unsigned int /*buttons*/) {}
   ...
   *  of this event.
   */
  virtual void wheel_event(int /*delta*/, bool /*horizontal*/,
                           const db::DPoint & /*p*/, unsigned int /*buttons*/) {
  }
   ...
   */
  const db::DCplxTrans &mouse_event_trans() const { return m_trans; }
  ...
  /**
   *  @brief Sets a value indicating whether dismissable view objects shall be
   * drawn or not
   *
   *  Markers with dismissable = false are always drawn. The default value is
   * "false".
   */
  void set_dismiss_view_objects(bool dismissed);

  /**
   *  @brief Gets a value indicating whether dismissable markers shall be drawn
   * or not
   */
  bool dismiss_view_objects() const { return m_view_objects_dismissed; }

  /**
   *  @brief Gets the current mouse position
   */
  db::DPoint mouse_position() const { return m_mouse_pos; }

  /**
   *  @brief Gets the current mouse position in micrometer units
   */
  db::DPoint mouse_position_um() const { return pixel_to_um(m_mouse_pos); }
   ...
   */
  bool mouse_in_window() const { return m_mouse_inside; }
   ...
   */
  QWidget *widget() const { return mp_widget; }
#endif
   ...
   */
  void send_mouse_double_clicked_event(const db::DPoint &pt,
                                       unsigned int buttons);
   ...
   */
  void send_wheel_event(int delta, bool horizontal, const db::DPoint &pt,
                        unsigned int buttons);
...
protected:
   ...
   *
   *  After calling this method, the next image_updated() call will return true
   * while also resetting the update needed flag.
   */
  void update();
...
private:
   ...
   *
   *  This method is called by the service constructor and registers the service
   * in the widget.
   */
   ...
   *  @brief Register a service
   *
   *  This method is called by the service destructor and unregisters the
   * service in the widget.
   */
 ...
 *
 *  The canvas provides ways and attributes that allow the view objects to
 * render themselves. The basic functionality of this interface is to provide
 * planes for painting.
 */
...
public:
  ...
  /**
   *  @brief Foreground color property: foreground color of the canvas (some
   * "contrast" color to background)
   */
  virtual tl::Color foreground_color() const = 0;

  /**
   *  @brief Active color property: color of active elements on the canvas (some
   * "contrast" color to background and different from foreground)
   */
   ...
   *
   *  The resolution describes the size of one pixel in relation to "one line
   * width". "One line width" is the unit of the drawing and describes the width
   * of a "one unit" wide line. In that sense, the resolution is the reciprocal
   ...
   *  be displayed. The operators are executed in the order given on the same
   * plane. CanvasPlanes may be shared if the styles are identical. The plane
   * index of the ViewOp is not used.
   */
 ...
 * planes for painting.
 */
class LAYBASIC_PUBLIC BitmapViewObjectCanvas : public ViewObjectCanvas {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  @brief Constructor
   */
  BitmapViewObjectCanvas(unsigned int width, unsigned int height,
                         double resolution);
   ...
   *  @brief Provide the renderer
   */
  virtual lay::Renderer &renderer() { return m_renderer; }

  /**
   *  @brief Get the resolution
   */
  virtual double resolution() const { return m_resolution; }

  /**
   *  @brief Return the number of bitmaps stored so far
   */
  size_t fg_bitmaps() const { return mp_fg_bitmaps.size(); }

  /**
   *  @brief Return the foreground bitmap with the given index
   *
   *  The returned pointer may be 0. The index must be less than the one
   * returned by fg_bitmaps ().
   */
   ...
   */
  const lay::ViewOp &fg_style(unsigned int i) const { return m_fg_view_ops[i]; }
   ...
   *  @brief Get the width
   */
  unsigned int canvas_width() const { return m_width; }

  /**
   *  @brief Get the height
   */
  unsigned int canvas_height() const { return m_height; }
