...
void AnnotationLayerOp::erase(AnnotationShapes *shapes) {
  if (size_t(std::distance(shapes->begin(), shapes->end())) <=
      m_shapes.size()) {
    ...
    //  pass this to the erase_positions method of the shapes object
    ...
    for (AnnotationShapes::layer_type::iterator lsh = shapes->begin();
         lsh != shapes->end(); ++lsh) {
      std::vector<shape_type>::const_iterator s =
...
AnnotationShapes::~AnnotationShapes() { clear(); }

AnnotationShapes &AnnotationShapes::operator=(const AnnotationShapes &d) {
  if (&d != this) {
    clear();
    if (manager() && manager()->transacting()) {
      manager()->queue(this,
                       new AnnotationLayerOp(true /*insert*/, d.m_layer.begin(),
                                             d.m_layer.end()));
    }
    m_layer = d.m_layer;
  }
...
void AnnotationShapes::clear() {
  if (manager() && manager()->transacting()) {
    manager()->queue(this,
                     new AnnotationLayerOp(false /*not insert*/,
                                           m_layer.begin(), m_layer.end()));
  }
...
void AnnotationShapes::reserve(size_t n) { m_layer.reserve(n); }
