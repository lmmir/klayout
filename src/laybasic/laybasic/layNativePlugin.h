 ...
 *    0,     //  (void (*)()) pointer to early autorun function or 0 if not
 * present "1.0", //  (const char *) version information - should be given at
 * least 0      //  (const char *) description or 0/empty if no description is
 * given
 *  };
 ...
 * expose.
 */
typedef void (*klp_init_func_t)(void (**autorun)(), void (**autorun_early)(),
...
#define DECLARE_NATIVE_PLUGIN(desc)                                            \
  extern "C" {                                                                 \
  KLP_PUBLIC void klp_init(void (**autorun)(), void (**autorun_early)(),       \
                           const char **version, const char **description) {   \
...
/**
 *  @brief Some (opaque) types for representing some gsi classes in the native
 * API
 */
...
LAYBASIC_PUBLIC void klp_assign(const klp_class_t *cls, void *target,
                                const void *source);
