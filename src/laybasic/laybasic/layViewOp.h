...
/**
 *  @brief The operator that describes how a bitmap is combined with the others
 * on the stack
 *
 *  Each operator is described by these properties:
 *    - a mode (copy, or, and, xor) which describes what boolean operation to
 * use
 ...
 *  Usually bitmaps and combination operators are organized in two vectors: one
 * for the bitmaps and one for the operators. By default, each operator is
 * associated with the corresponding bitmap.
 *
 *  However, an operator can be associated with any bitmap by setting the bitmap
 * index. In this case, the length of the operator list does not need to have
 * the same length than the bitmap list.
 *
 *  In the context of "floating" bitmaps (foreground bitmaps for markers, rulers
 * when dragging etc.), the bitmap index is not used to map a bitmap but to
 * specify a order of bitmaps: the planes are drawn in the order of the bitmap
 * index.
 */
...
public:
   ...
   */
  void set_dither_index(unsigned int di) { m_dither_index = di; }

  /**
   *  @brief Read accessor to the dither pattern index
   */
  unsigned int dither_index() const { return m_dither_index; }

  /**
   *  @brief Write accessor to the dither pattern offset
   */
  void set_dither_offset(unsigned int d) { m_dither_offset = d; }

  /**
   *  @brief Read accessor to the dither pattern offset
   */
  unsigned int dither_offset() const { return m_dither_offset; }

  /**
   *  @brief Write accessor to the line style index
   */
  void set_line_style_index(unsigned int lsi) { m_line_style_index = lsi; }
   ...
   */
  void bitmap_index(int bi) { m_bitmap_index = bi; }
  ...
  bool operator==(const ViewOp &d) const {
    return (m_or == d.m_or && m_and == d.m_and && m_xor == d.m_xor &&
            m_line_style_index == d.m_line_style_index &&
            m_dither_index == d.m_dither_index &&
            m_dither_offset == d.m_dither_offset && m_width == d.m_width &&
            m_shape == d.m_shape && m_bitmap_index == d.m_bitmap_index);
  }
