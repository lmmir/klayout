...
public:
   ...
   */
  bool operator!=(const DitherPatternInfo &d) const { return !operator==(d); }
   ...
   */
  unsigned int order_index() const { return m_order_index; }

  /**
   *  @brief Write access to the name
   */
  void set_order_index(unsigned int oi) { m_order_index = oi; }
   ...
   */
  QBitmap get_bitmap(int width = -1, int height = -1,
                     int frame_width = -1) const;
#endif
   ...
   *  size (32*pattern_stride)x64 bits. If the actual width or height is
   * smaller, the pattern is repeated to fill this area. The pattern stride may
   * be bigger than 1 to accommodate pattern with a width that is not a fraction
   * of 32. Such pattern are repeated until they fill a multiple of 32 bits.
   */
  const uint32_t *const *pattern() const { return &(m_pattern[0]); }
   ...
   */
  unsigned int pattern_stride() const { return m_pattern_stride; }
...
private:
  ...
  std::string m_name;
  mutable std::unique_ptr<std::map<unsigned int, DitherPatternInfo>>
      m_scaled_pattern;

  void set_pattern_impl(const uint32_t *pattern, unsigned int w,
                        unsigned int h);
  void set_pattern_impl(const uint64_t *pattern, unsigned int w,
                        unsigned int h);
...
public:
   ...
   *  the pattern is virtually deleted (such pattern are not shown in the
   * editor)
   */
  void replace_pattern(unsigned int i, const DitherPatternInfo &p);

  /**
   *  @brief Add a new pattern, searching for a empty slot and returning that
   * index
   *
   ...
   *  plus one thus placing the new pattern at the end of the list in the
   * editor.
   */
   ...
   */
  void merge(const DitherPattern &other,
             std::map<unsigned int, unsigned int> &index_map);

  /**
   *  @brief Return the number stipples
   */
  unsigned int count() const { return (unsigned int)m_pattern.size(); }
   ...
   */
  iterator begin() const { return m_pattern.begin(); }

  /**
   *  @brief The begin iterator delivering the past-the-end pattern object
   */
  iterator end() const { return m_pattern.end(); }
