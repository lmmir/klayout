...
//  NOTE: we add the Qt-Bindings-related methods of tl::PixelBuffer and
//  tl::BitmapBuffer here instead of inside gsiDeclTl. Reasoning: by doing so,
//  gsi and tl remain independent of the Qt binding library which is good for
//  lean applications such as strmrun
...
ClassExt<tl::BitmapBuffer> decl_BitmapBuffer(
    gsi::method("to_qimage", &tl::BitmapBuffer::to_image_copy,
                "@brief Converts the pixel buffer to a \\QImage object") +
    gsi::method("from_qimage", &tl::BitmapBuffer::from_image,
                gsi::arg("qimage"),
                "@brief Creates a pixel buffer object from a QImage object\n"));
