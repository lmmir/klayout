...
static const char *def_palette = "0 1 2 3 "
                                 "4 5[1] 6 7 "
                                 "8 9[0] 10 11 "
                                 "12 13 14 15 ";
...
StipplePalette StipplePalette::default_palette() {
  StipplePalette p;
...
StipplePalette::StipplePalette(const std::vector<unsigned int> &stipples,
...
StipplePalette StipplePalette::operator=(const StipplePalette &d) {
  if (&d != this) {
...
unsigned int StipplePalette::stipple_by_index(unsigned int n) const {
  if (stipples() == 0) {
...
unsigned int StipplePalette::stipples() const {
...
unsigned int StipplePalette::standard_stipples() const {
...
void StipplePalette::clear_stipples() { m_stipples.clear(); }

void StipplePalette::set_standard_stipple_index(unsigned int n,
                                                unsigned int si) {
...
void StipplePalette::clear_standard_stipples() { m_standard.clear(); }

std::string StipplePalette::to_string() const {
  std::string res;
  ...
  try {
    ...
    if (!x.at_end()) {
      throw tl::Exception(tl::sprintf(
          tl::to_string(tr("unexpected characters: %s")), x.skip()));
    }

    if (stipples() == 0 || standard_stipples() == 0) {
      throw tl::Exception(tl::to_string(
          tr("invalid palette - no stipples and/or standard stipples")));
    }

  } catch (std::exception &ex) {
    //  reformat error message
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr("Stipple palette string format error: %s")),
        ex.what()));
  }
}
