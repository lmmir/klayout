...
public:
  ...
  /**
   *  @brief Control whether draw(db::Shape..) draws precise (does not simplify
   * to box)
   *
   *  Default is "not precise".
   */
  void set_precise(bool f) { m_precise = f; }

  /**
   *  @brief Sets a value indicating whether to draw a diagonal cross across
   * boxes and polygons
   */
  void set_xfill(bool f) { m_xfill = f; }

  /**
   *  @brief Control whether draw(db::Shape..) draws properties on the text
   * layer
   */
  void draw_properties(bool f) { m_draw_properties = f; }

  /**
   *  @brief Control whether draw(db::Shape..) draws the "description"
   * properties on the text layer
   */
  void draw_description_property(bool f) { m_draw_description_property = f; }

  /**
   *  @brief Control whether draw(db::Text..) draws texts but rather the pixel
   */
  void draw_texts(bool f) { m_draw_texts = f; }

  /**
   *  @brief The default text size to use for draw(db::Text..)
   */
  void default_text_size(db::Coord sz) { m_default_text_size = sz; }

  /**
   *  @brief Get the default text size
   */
  db::Coord default_text_size() const { return m_default_text_size; }

  /**
   *  @brief The default text size to use for draw(db::DText..)
   */
  void default_text_size(double sz) { m_default_text_size_dbl = sz; }

  /**
   *  @brief Get the default text size for draw (DText ...)
   */
  double default_text_size_dbl() const { return m_default_text_size_dbl; }

  /**
   *  @brief The font to use for draw(db::Text..).
   */
  void set_font(db::Font f) { m_font = f; }

  /**
   *  @brief Get the font to use for draw(db::Text..)
   */
  db::Font font() const { return m_font; }

  /**
   *  @brief Apply text transformations to text or not for draw(db::Text..).
   */
  void apply_text_trans(bool f) { m_apply_text_trans = f; }

  /**
   *  @brief Get the flag which determines to apply text transformations for
   * draw(db::Text..)
   */
   ...
   *  @param fill The bitmap to which to render the interior. Can be 0 for not
   * drawing this.
   *  @param frame The bitmap to which to render the contour. Can be 0 for not
   * drawing this.
   *  @param vertices The bitmap to which to render the vertices. Can be 0 for
   * not drawing this.
   *  @param texts Currenty not used. Used for drawing properties if drawing of
   * these is required.
   */
  virtual void draw(const db::Shape &shape, const db::CplxTrans &trans,
                    lay::CanvasPlane *fill, lay::CanvasPlane *frame,
                    lay::CanvasPlane *vertices, lay::CanvasPlane *texts) = 0;
   ...
   */
  virtual void draw(const db::DPolygon &poly, lay::CanvasPlane *fill,
                    lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                    lay::CanvasPlane *texts) = 0;
   ...
   *  @brief Same for double coordinates
   */
  virtual void draw(const db::DBox &box, lay::CanvasPlane *fill,
                    lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                    lay::CanvasPlane *texts) = 0;
   ...
   *  @brief Same for double coordinates
   */
  virtual void draw(const db::DPath &path, lay::CanvasPlane *fill,
                    lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                    lay::CanvasPlane *texts) = 0;
   ...
   *  @brief Same for double coordinates
   */
  virtual void draw(const db::DText &text, lay::CanvasPlane *fill,
                    lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                    lay::CanvasPlane *texts) = 0;
   ...
   *
   *  @box The box in which to inscribe the text (or the point at which to align
   * if the width and height is zero)
   ...
   *  @param texts The bitmap to which to render the text.
   */
  virtual void draw(const db::DBox &box, const std::string &text, db::Font font,
                    db::HAlign halign, db::VAlign valign, db::DFTrans trans,
                    lay::CanvasPlane *fill, lay::CanvasPlane *frame,
                    lay::CanvasPlane *vertices, lay::CanvasPlane *texts) = 0;
   ...
   *  @brief Same for double coordinates
   */
  virtual void draw(const db::DEdge &edge, lay::CanvasPlane *fill,
                    lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                    lay::CanvasPlane *texts) = 0;
   ...
   *  The shape can either be a polygon, box, edge, path or text
   *
   *  This method draws the properties if "draw_properties" is set to true
   * and/or the "description" property when "draw_description_property" is set
   * to true.
   */
  void draw_propstring(const db::Shape &shape,
                       const db::PropertiesRepository *prep,
                       lay::CanvasPlane *text, const db::CplxTrans &trans);

  /**
   *  @brief Draw a properties string at the given position
   *
   *  The transformation is not used to transform the point (it must be given in
   * micron units already) but to compute the position of the text, given the
   * text height.
   */
  void draw_propstring(db::properties_id_type id,
                       const db::PropertiesRepository *prep,
                       const db::DPoint &pref, lay::CanvasPlane *text,
   ...
   *
   *  The resolution value is used to convert dimensions on the output device
   * into canvas units. A resolution of 1 means that one canvas unit roughly
   * corresponds to 0.01" (0.25mm). A resolution value of 2 means that one
   * canvas unit roughly corresponds to 0.02".
   */
