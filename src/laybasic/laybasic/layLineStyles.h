...
public:
   ...
   */
  bool operator!=(const LineStyleInfo &d) const { return !operator==(d); }
   ...
   */
  const uint32_t *pattern() const { return m_pattern; }
...
private:
  ...
  std::string m_name;
  mutable std::unique_ptr<std::map<unsigned int, LineStyleInfo>>
      m_scaled_pattern;
...
public:
   ...
   *  @brief Equality
   */
  bool operator==(const LineStyles &p) const { return m_styles == p.m_styles; }
  ...
  /**
   *  @brief Add a new style, searching for a empty slot and returning that
   * index
   *
   ...
   */
  void merge(const LineStyles &other,
             std::map<unsigned int, unsigned int> &index_map);

  /**
   *  @brief Return the number of styles
   */
  unsigned int count() const { return (unsigned int)m_styles.size(); }
   ...
   */
  iterator begin() const { return m_styles.begin(); }

  /**
   *  @brief The begin iterator delivering the past-the-end style object
   */
  iterator end() const { return m_styles.end(); }
