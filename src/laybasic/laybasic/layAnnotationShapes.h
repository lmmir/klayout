 ...
 */
class LAYBASIC_PUBLIC AnnotationLayerOp : public db::Op {
public:
  typedef db::DUserObject shape_type;

  AnnotationLayerOp(bool insert, const shape_type &sh) : m_insert(insert) {
    m_shapes.push_back(sh);
  }

  template <class Iter>
  AnnotationLayerOp(bool insert, Iter from, Iter to) : m_insert(insert) {
...
/**
 *  @brief A collection of DUserObject objects that serves as a container for
 * annotation shapes
 */

class LAYBASIC_PUBLIC AnnotationShapes : public db::LayoutStateModel,
                                         public db::Object {
public:
  ...
  template <class I> void erase_positions(I first, I last) {
    if (manager() && manager()->transacting()) {
      manager()->queue(this, new AnnotationLayerOp(false /*not insert*/, first,
                                                   last, true /*dummy*/));
    }
  ...
  /**
   *  @brief Replace an element at the given position with another shape (move
   * semantics)
   */
  const shape_type &replace(iterator pos, const shape_type &&sh);
   ...
   */
  void update_bbox() { m_layer.update_bbox(); }
   ...
   *  requires an update.
   */
  bool is_bbox_dirty() const { return m_layer.is_bbox_dirty(); }
   ...
   */
  box_type bbox() const { return m_layer.bbox(); }
   ...
   */
  layer_type::iterator begin() const { return m_layer.begin(); }
   ...
   */
  layer_type::iterator end() const { return m_layer.end(); }
 ...
 *  @brief Collect memory usage
 */
inline void mem_stat(db::MemStatistics *stat,
                     db::MemStatistics::purpose_t purpose, int cat,
                     const AnnotationShapes &x, bool no_self = false,
                     void *parent = 0) {
