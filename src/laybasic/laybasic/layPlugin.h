 ...
 */
LAYBASIC_PUBLIC MenuEntry separator(const std::string &menu_name,
 ...
 *  @param menu_name The name of the menu item (see layAbstractMenu.h)
 *  @param insert_pos The position where to insert (see layAbstractMenu.h)
 *  @param title The title to display plus optional icon resource and keyboard
 * shortcut. The format of the string is:
 * <text>["("shortcut")"]["<"icon-resource">"][{"tool-tip"}].
 */
LAYBASIC_PUBLIC MenuEntry menu_item(const std::string &symbol,
 ...
 */
LAYBASIC_PUBLIC MenuEntry menu_item_copy(const std::string &symbol,
                                         const std::string &menu_name,
                                         const std::string &insert_pos,
 ...
 * <text>["("shortcut")"]["<"icon-resource">"][{"tool-tip"}].
 */
LAYBASIC_PUBLIC MenuEntry submenu(const std::string &menu_name,
                                  const std::string &insert_pos,
                                  const std::string &title);
 ...
 * <text>["("shortcut")"]["<"icon-resource">"][{"tool-tip"}].
 */
LAYBASIC_PUBLIC MenuEntry submenu(const std::string &symbol,
                                  const std::string &menu_name,
                                  const std::string &insert_pos,
                                  const std::string &title);
 ...
 *  @param cvalue The value to set for the configuration item (optional for
 * boolean configuration items)
 */
LAYBASIC_PUBLIC MenuEntry
config_menu_item(const std::string &menu_name, const std::string &insert_pos,
                 const std::string &title, const std::string &cname,
                 const std::string &cvalue = std::string());
...
public:
   ...
   *
   *  This method can be reimplemented alternatively to "config_page" when
   * multiple pages shall be used. The pages are inserted in the order they are
   * defined.
   */
   ...
   *  Otherwise the individual plugin object (associated with a view) must
   * handle the configuration. One example for handling the configuration
   * globally is dynamic menu configuration.
   */
   ...
   *  The implementation is supposed to return true if it handles the menu
   * entry.
   */
   ...
   *
   *  Reimplementation of this method offers a chance to initialize static
   * resources such as dialogs etc.
   */
   ...
   *
   *  Reimplementation of this method offers a chance to initialize static
   * resources such as dialogs etc. While initialize is called before any
   * configuration is loaded, "initialized" will be called after the plugin
   ...
   *
   *  If the plugin wants to prevent the application from closing, it may return
   * false in this method.
   */
  virtual bool can_exit(lay::Dispatcher * /*dispatcher*/) const { return true; }
   ...
   *
   *  This method may return 0 for "dummy" plugins that just register menu
   * entries or configuration options.
   */
   ...
   *  "Edit" menu. The string returned in "title" is displayed in the "Edit"
   * menu's corresponding mode entry.
   */
   ...
   *
   *  Editor options pages are shown in the editor options dockable tool widget.
   * Each plugin can create such pages and these will be shown in tabs inside
   * this widget.
   *
   *  The new pages are returned in the "pages" vector. The layout view will
   * take ownership of these pages.
   */
  virtual void
  ...
  /**
   *  @brief Tells if the plugin implements a "lay::ViewService" active mouse
   * mode
   *
   *  This method is supposed to return true if the plugin implements the
   ...
   *
   *  These are built-in modes from the LayoutViewBase. This method is intended
   * for the LayoutViewBase's standard modes only.
   */
  virtual void implements_primary_mouse_modes(
   ...
   *
   *  If this plugin wants to register a technology component, it's declaration
   * must include a technology component provider. This is responsible for
   * creating a technology component and an associated editor.
   *
   *  @return The provide object (no ownership is taken) or 0 if no technology
   * component is created.
   */
  virtual const TechnologyComponentProvider *
  technology_component_provider() const {
    return 0;
  }
   ...
   *
   *  If the editable part of this plugin is enabled, the objects managed by
   * this mode can be manipulated. Otherwise, the are not considered for
   * selection. This property can be monitored with the editable_enabled
   * observer.
   */
  void set_editable_enabled(bool f);

  /**
   *  @brief Gets a value indicating whether the editable part of the plugin is
   * enabled
   */
  bool editable_enabled() const { return m_editable_enabled; }
   ...
   *
   *  This method must be called when a plugin is dynamically created. It is
   * important that when this method is called, the menu items and other
   * properties are set already.
   */
 ...
 *
 *  Each object participating in the plugin scheme must be derived from this
 * interface. An plugin can have a parent. This way, a hierarchy of plugin
 * objects is created. The root object not having a parent acts as the main
 * entry point: it will try to dispatch configuration requests to the children.
 *
 *  Each node has a local configuration space which overrides the configuration
 * changes made to be parent.
 *
 ...
 *
 *  The "Dispatcher" adds the concept of a (singleton) root plugin to this
 * hierarchical configuration tree.
 */

class LAYBASIC_PUBLIC Plugin : public gsi::ObjectBase,
...
public:
   ...
   *  a string.
   */
  template <class T> void config_set(const std::string &name, const T &value) {
    config_set(name, tl::to_string(value));
  }

  /**
   *  @brief Set a certain configuration parameter with a given type using a
   ...
   *  configuration value for the given configuration option name. However, if
   * the configuration option with the given name does not exist, simply an
   ...
   */
  template <class T> bool config_get(const std::string &name, T &value) const {
    T t;
    std::string s;
   ...
   */
  iterator begin() const { return m_repository.begin(); }

  /**
   *  @brief Iterator for the key/value pairs: end iterator
   */
  iterator end() const { return m_repository.end(); }
   ...
   *
   *  If the object implements the lay::Browser interface, return the pointer to
   * this. Otherwise the implementation is supposed to return 0.
   */
   ...
   *
   *  If the object implements the lay::ViewService interface, return the
   * pointer to this. Otherwise the implementation is supposed to return 0.
   */
   ...
   *
   *  If the object implements the lay::Drawing interface, return the pointer to
   * this. Otherwise the implementation is supposed to return 0.
   */
   ...
   *
   *  If the object implements the lay::Editable interface, return the pointer
   * to this. Otherwise the implementation is supposed to return 0.
   */
  ...
  /**
   *  @brief Associate a plugin with the plugin declaration for that service
   * (getter)
   */
   ...
   */
  Plugin *plugin_parent() { return mp_parent; }

  /**
   *  @brief Associate a service with the plugin declaration for that service
   * (setter)
   *
   *  The association is done when the plugin is created. It should not be
   * changed.
   */
 ...
 *  updated.
 */
template <class T> inline bool test_and_set(T &target, const T &source) {
  if (target != source) {
    target = source;
    return true;
  } else {
