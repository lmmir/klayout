...
LineStyleInfo &LineStyleInfo::operator=(const LineStyleInfo &d) {
  if (&d != this) {
...
QBitmap LineStyleInfo::get_bitmap(int width, int height) const {
  ...
  for (unsigned int i = 0; i < (unsigned int)(height - 2); ++i) {
    if (is_bit_set(i)) {
      ...
      data[(height - 2 - i) * stride + (width - 1) / 8] |=
          (1 << ((width - 1) % 8));
    }
  }
...
const LineStyleInfo &LineStyleInfo::scaled(unsigned int n) const {
  if (n <= 1) {
    return *this;
  }
...
void LineStyleInfo::scale_pattern(unsigned int n) {
  ...
  for (unsigned int i = 0; i < m_pattern_stride; ++i) {
    uint32_t dout = 0;
    for (uint32_t m = 1; m != 0; m <<= 1) {
      //  NOTE: we do not fully expand "1" fields with a following "0" as pixel
      //  expansion will take care of this.
...
std::string LineStyleInfo::to_string() const {
  std::string res;
...
struct ReplaceLineStyleOp : public db::Op {
  ReplaceLineStyleOp(unsigned int i, const LineStyleInfo &o,
                     const LineStyleInfo &n)
...
LineStyles::LineStyles() : db::Object(0) {
  for (unsigned int d = 0; d < sizeof(style_strings) / sizeof(style_strings[0]);
       d += 2) {
...
LineStyles &LineStyles::operator=(const LineStyles &p) {
  if (this != &p) {
    unsigned int i;
    for (i = 0; i < p.count(); ++i) {
...
const LineStyleInfo &LineStyles::style(unsigned int i) const {
  if (i < count()) {
    return m_styles[i];
  } else {
...
unsigned int LineStyles::add_style(const LineStyleInfo &p) {
  unsigned int oi = 0;
...
LineStyles::iterator LineStyles::begin_custom() const {
  return m_styles.begin() +
         sizeof(style_strings) / sizeof(style_strings[0]) / 2;
}

const LineStyles &LineStyles::default_style() {
...
void LineStyles::merge(const LineStyles &other,
  ...
  for (iterator c = other.begin_custom(); c != other.end(); ++c) {
    std::map<LineStyleInfo, unsigned int, style_less_f>::const_iterator p =
        styles.find(*c);
