...
public:
  /**
   *  @brief Constructor
   *  On creation, the entry is in "open" state and will be closed once the data
   * has been committed with "close". "Precious" entries will be preserved as
   * far as possible.
   */
  ImageCacheEntry(const lay::Viewport &vp,
                  const std::vector<lay::RedrawLayerInfo> &layers,
                  bool precious);

  /**
   *  @brief Returns a value indicating whether the entry complies with the
   * given viewport
   */
  bool equals(const lay::Viewport &vp,
   ...
   */
  bool precious() const { return m_precious; }

  /**
   *  @brief Sets a value indicating whether the entry is precious
   */
  void set_precious(bool p) { m_precious = p; }

  /**
   *  @brief Gets the stored bitmap data
   */
  const BitmapCanvasData &data() const { return m_data; }
...
class LAYBASIC_PUBLIC LayoutCanvas : public lay::ViewObjectUI,
...
public:
   ...
   *
   *  This method needs to be called after the constructor to establish the
   * drawing widget.
   */
   ...
   *
   *  The box gives the target box for the viewport. It may have a different
   * aspect ratio than the viewport (in pixels) which is important if the size
   * changes. Then, the original target box will be used which may result in a
   * better fit. If the "precious" flag is set, the zoom box will be regarded
   * "precious" and kept in the cache for longer.
   */
   ...
   *
   *  This sets the transformation of the viewport directly. Thus, an arbitrary
   * transformation may be specified.
   */
   ...
   *
   *  The layers vector gets swapped with the current one for performance
   * reasons.
   */
   ...
   *
   *  This method delivers the redraw layer info vector as set by the last
   * redraw_new call.
   */
   ...
   *
   *  The index must correspond to lay::RedrawLayerInfo objects from former
   * redraw_new calls.
   */
   ...
   */
  unsigned int oversampling() const { return m_oversampling; }
   ...
   */
  bool highres_mode() const { return m_hrm; }
   ...
   *
   *  The vector must correspond to the  lay::RedrawLayerInfo objects from
   * former redraw_new
   */
   ...
   */
  tl::Color background_color() const { return tl::Color(m_background); }

  /**
   *  @brief Reimplementation of ViewObjectCanvas: Foreground color
   */
  tl::Color foreground_color() const { return tl::Color(m_foreground); }

  /**
   *  @brief Reimplementation of ViewObjectCanvas: Active color
   */
  tl::Color active_color() const { return tl::Color(m_active); }

  /**
   *  @brief Reimplementation of ViewObjectCanvas: background image
   */
  tl::PixelBuffer *bg_image() { return mp_image; }
   ...
   */
  const lay::Viewport &viewport() const { return m_viewport; }
   ...
   *  If the viewport (the rectangle that is shown) changes, this event is
   * fired.
   */
