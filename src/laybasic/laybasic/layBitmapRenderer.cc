...
BitmapRenderer::BitmapRenderer(unsigned int width, unsigned int height,
                               double resolution)
    : Renderer(width, height, resolution), m_xmin(0.0), m_xmax(0.0),
      m_ymin(0.0), m_ymax(0.0), m_ortho(true) {
  // .. nothing else ..
}

void BitmapRenderer::reserve_edges(size_t n) { m_edges.reserve(n); }

void BitmapRenderer::reserve_texts(size_t n) { m_texts.reserve(n); }
...
void BitmapRenderer::insert(const db::DBox &box, const std::string &text,
                            db::Font font, db::HAlign halign, db::VAlign valign,
                            db::DFTrans trans) {
...
void BitmapRenderer::draw(const db::DBox &box, const std::string &text,
                          db::Font font, db::HAlign halign, db::VAlign valign,
                          db::DFTrans trans, lay::CanvasPlane * /*fill*/,
                          lay::CanvasPlane * /*frame*/,
                          lay::CanvasPlane * /*vertices*/,
                          lay::CanvasPlane *texts) {
  clear();
...
void BitmapRenderer::insert(const db::DBox &b) {
  db::DEdge edges[] = {db::DEdge(b.p1(), db::DPoint(b.p1().x(), b.p2().y())),
...
static inline bool point_inside_box(const db::DPoint &pt, const db::DBox &box) {
  return (
      !(db::coord_traits<db::DBox::coord_type>::equal(pt.x(), box.left()) ||
        db::coord_traits<db::DBox::coord_type>::equal(pt.x(), box.right())) &&
      !(db::coord_traits<db::DBox::coord_type>::equal(pt.y(), box.bottom()) ||
        db::coord_traits<db::DBox::coord_type>::equal(pt.y(), box.top())));
}

void BitmapRenderer::add_xfill() {
  db::DBox box;
  for (std::vector<lay::RenderEdge>::const_iterator e = m_edges.begin();
       e != m_edges.end(); ++e) {
    if (!e->is_ortho()) {
      return;
    }
...
void BitmapRenderer::render_texts(lay::CanvasPlane &plane) {
  ...
  for (std::vector<lay::RenderText>::const_iterator t = m_texts.begin();
       t != m_texts.end(); ++t) {
...
void BitmapRenderer::render_contour(lay::CanvasPlane &plane) {
  ...
  if (floor(m_xmax + 0.5) == floor(m_xmin + 0.5)) {
    ...
    unsigned int y2int = (unsigned int)(std::max(
        0.0, std::min(m_ymax + 0.5, double(bitmap->height() - 1))));
  ...
  if (floor(m_ymax + 0.5) == floor(m_ymin + 0.5)) {
    ...
    unsigned int x2int = (unsigned int)(std::max(
        0.0, std::min(m_xmax + 0.5, double(bitmap->width() - 1))));
    unsigned int yint = (unsigned int)(std::max(
        0.0, std::min(m_ymin + 0.5, double(bitmap->height() - 1))));
...
void BitmapRenderer::render_dot(double x, double y, lay::CanvasPlane *plane) {
  ...
  //  basic optimizations: outside window
  if (x < 0.0 || x >= bitmap->width() || y < 0.0 || y >= bitmap->height()) {
    return;
  }
...
void BitmapRenderer::render_box(double xmin, double ymin, double xmax,
                                double ymax, lay::CanvasPlane *plane) {
  lay::Bitmap *bitmap = static_cast<lay::Bitmap *>(plane);
  ...
  //  basic optimizations: outside window
  if (xmax < 0.0 || xmin >= bitmap->width() || ymax < 0.0 ||
      ymin >= bitmap->height()) {
    return;
  }

  unsigned int y1int = (unsigned int)(std::max(
      0.0, std::min(ymin, double(bitmap->height() - 1))));
  unsigned int y2int = (unsigned int)(std::max(
      0.0, std::min(ymax, double(bitmap->height() - 1))));
  unsigned int x1int = (unsigned int)(std::max(
      0.0, std::min(xmin, double(bitmap->width() - 1))));
  unsigned int x2int = (unsigned int)(std::max(
      0.0, std::min(xmax, double(bitmap->width() - 1))));
  for (unsigned int y = y1int; y <= y2int; ++y) {
...
void BitmapRenderer::draw(const db::Shape &shape, const db::CplxTrans &trans,
                          ...
                          lay::CanvasPlane *vertices, lay::CanvasPlane *text) {
  if (shape.is_text()) {

    db::Point p = db::Point() + shape.text_trans().disp();
    db::DPoint dp = trans * p;

    if ((vertices || frame) && dp.x() < m_width - 0.5 && dp.x() > -0.5 &&
    ...
    if (m_draw_texts && text) {
      ...
      insert(db::DBox(dp + fp(tp1), dp + fp(tp2)), shape.text_string(), font,
             halign, valign, fp);
...
void BitmapRenderer::draw(const db::DPolygon &poly, const db::DCplxTrans &trans,
  ...
  if (b.width() < threshold && b.height() < threshold) {
    ...
    if (simplify_box(b, trans)) {
      ...
      for (db::DPolygon::polygon_edge_iterator e = poly.begin_edge();
           !e.at_end(); ++e) {
        insert(trans * *e);
      }
    }
...
void BitmapRenderer::draw(const db::DPolygon &poly, lay::CanvasPlane *fill,
                          lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                          lay::CanvasPlane * /*text*/) {
  //  simplify to a rectangle or dot if possible
  db::DBox b(poly.box());
...
void BitmapRenderer::draw(const db::DBox &box, lay::CanvasPlane *fill,
                          lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                          lay::CanvasPlane * /*text*/) {
  if (!box.empty()) {
    ...
    if (box.width() < 1.0 && box.height() < 1.0) {
      ...
      if (fill &&
          (fill != frame || (box.width() > 1.0 && box.height() > 1.0))) {
        render_fill(*fill);
      }
      if (frame) {
        if (m_xfill) {
...
void BitmapRenderer::draw(const db::Path &path, const db::CplxTrans &trans,
  ...
  if (b.width() < threshold && b.height() < threshold) {
    ...
    //  generate the hull and produce the edges from this only if the path is
    //  considerably wide otherwise just render the spine
    ...
    if (!quite_thin || thin) {
      ...
      if (q != path.end()) {
        ...
        while (qq != path.end()) {
          ...
          //  If the path is simplified, the spine is drawn instead of the
          //  contour. We must apply the begin/end extensions then.

          if (thin && qq == path.end()) {
            if (path.extensions().second != 0 &&
                (seg.dx() != 0 || seg.dy() != 0)) {
              db::DVector ed(seg.p2() - seg.p1());
              ed *= 1.0 / ed.double_length();
              seg = db::Edge(
                  seg.p1(),
                  seg.p2() + db::Vector(ed * double(path.extensions().second)));
            }
          }

          if (first) {
            first = false;
            if (thin && path.extensions().first != 0 &&
                (seg.dx() != 0 || seg.dy() != 0)) {
              db::DVector ed(seg.p2() - seg.p1());
              ed *= 1.0 / ed.double_length();
              seg = db::Edge(
                  seg.p1() - db::Vector(ed * double(path.extensions().first)),
                  seg.p2());
            }
          }
...
void BitmapRenderer::draw(const db::DPath &path, lay::CanvasPlane *fill,
                          lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                          lay::CanvasPlane *text) {
...
void BitmapRenderer::draw(const db::Text &txt, const db::CplxTrans &trans,
  ...
  if (m_draw_texts && text) {
    ...
    insert(db::DBox(dp + fp(tp1), dp + fp(tp2)), txt.string(), font,
           txt.halign(), txt.valign(), fp);

    render_texts(*text);
  }
}

void BitmapRenderer::draw(const db::DText &txt, lay::CanvasPlane *fill,
                          lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                          lay::CanvasPlane *text) {
...
void BitmapRenderer::draw(const db::DEdge &edge, lay::CanvasPlane * /*fill*/,
                          lay::CanvasPlane *frame, lay::CanvasPlane *vertices,
                          lay::CanvasPlane * /*text*/) {
...
bool BitmapRenderer::simplify_box(Box &b, const Trans &trans) {
  ...
  if (!m_precise && ((ortho && trans.ctrans(std::min(b.width(), b.height())) <
                                   small_size_threshold) ||
                     (!ortho && trans.ctrans(std::max(b.width(), b.height())) <
                                    small_size_threshold))) {
