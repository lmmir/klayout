...
db::DCoord snap(db::DCoord c, db::DCoord grid) {
  if (grid <= 1e-10) {
    return c;
  } else {
...
db::DPoint snap(const db::DPoint &p, db::DCoord grid) {
  if (grid <= 1e-10) {
    return p;
  } else {
...
std::pair<db::DPoint, db::DPoint> snap(const db::DPoint &p1,
                                       const db::DPoint &p2) {
...
std::pair<db::DPoint, db::DPoint> snap(const db::DPoint &p1,
                                       const db::DPoint &p2, db::DCoord grid) {
  if (grid <= 1e-10) {
    return std::make_pair(p1, p2);
  } else {
    std::pair<db::DPoint, db::DPoint> v =
        snap(db::DPoint(p1.x() / grid, p1.y() / grid),
...
std::pair<db::Point, db::Point> draw_round(const db::DPoint &p1,
                                           const db::DPoint &p2, int h) {
...
db::DVector snap_angle(const db::DVector &in, lay::angle_constraint_type ac) {
  ...
  if (elen > 1e-6) {
    ...
    for (std::vector<db::DVector>::const_iterator re = ref_dir.begin();
         re != ref_dir.end(); ++re) {
      double proj;
 ...
 *  It will start from a given point and look into the direction of given
 * cutlines. It will try to find the closest snapping point while also
 * considering snapping.
 *
 ...
 *
 *  Vertex and non-vertex snapping is available: with vertex snapping, the
 * search point will snap to vertexes too. Without vertex snapping, the snap
 * point is guaranteed to sit on an edge.
 *
 *  The finder will deliver two solutions if available: an exact one where the
 *  point sits exactly on the constraint and a more generic one where the
 * resulting point is allowed to deviate a little from that direction. The exact
 * solution is priority in the generic case.
 */
class ContourFinder {
public:
  /**
   *  @brief Constructor
   */
  ContourFinder(const db::DPoint &original, const db::DVector &grid,
                const std::vector<db::DEdge> &cutlines, bool with_vertex = true,
                bool directed = false)
      : m_any(false), m_any_exact(false), m_original(original),
        m_is_vertex(false), m_is_vertex_exact(false),
        m_tests(10000 /* max. number of tests, TODO: make variable? */),
        mp_layout(0), m_cutlines(cutlines), mp_prop_sel(0),
        m_inv_prop_sel(false), m_with_vertex(with_vertex),
        m_directed(directed) {
    ...
    if (m_cutlines.empty() && grid != db::DVector()) {
      ...
      m_cutlines.push_back(db::DEdge(db::DPoint(original.x(), ps.y()),
                                     db::DPoint(original.x() + 1.0, ps.y())));
      m_cutlines.push_back(
          db::DEdge(db::DPoint(original.x(), ps.y() + dy),
                    db::DPoint(original.x() + 1.0, ps.y() + dy)));
      m_cutlines.push_back(db::DEdge(db::DPoint(ps.x(), original.y()),
                                     db::DPoint(ps.x(), original.y() + 1.0)));
      m_cutlines.push_back(
          db::DEdge(db::DPoint(ps.x() + dx, original.y()),
                    db::DPoint(ps.x() + dx, original.y() + 1.0)));
    }
  }
    ...
    for (lay::LayerPropertiesConstIterator l = view->begin_layers();
      ...
      if (l->is_visual()) {
        ...
        if (cv.is_valid()) {
          ...
          db::DBox dregion =
              dworld & db::DBox(px - dd, py - dd, px + dd, py + dd);
          if (!dregion.empty()) {
            ...
            if (l->hier_levels().has_from_level()) {
              min_hier_level =
                  l->hier_levels().from_level(ctx_levels, min_hier_level);
            }
            if (l->hier_levels().has_to_level()) {
              max_hier_level =
                  l->hier_levels().to_level(ctx_levels, max_hier_level);
            }
            ...
            for (std::vector<db::DCplxTrans>::const_iterator t = trans.begin();
                 t != trans.end() && m_tests > 0; ++t) {
              do_find(view, cv_index, *cv.cell(), l->layer_index(),
                      min_hier_level, max_hier_level,
                      *t * db::CplxTrans(mp_layout->dbu()) *
                          cv.context_trans());
            }
          }
        }
   ...
   */
  bool any() const { return (m_any || m_any_exact); }

  /**
   *  @brief Returns true if there is an exact solution
   */
  bool any_exact() const { return m_any_exact; }

private:
  void find_closest_exact(const db::DPoint &p, const db::DEdge &e) {
    if (!m_any_exact ||
        m_original.distance(p) < m_original.distance(m_closest_exact)) {

      if (m_directed) {
        for (std::vector<db::DEdge>::const_iterator cl = m_cutlines.begin();
             cl != m_cutlines.end(); ++cl) {
  ...
  void find_closest(const db::DPoint &p, const db::DEdge &e) {
  ...
  void closest(const db::DEdge &e) {
    ...
    if (!any_point) {
      ...
      db::DPoint e1(m_original.x() - n.x() * f, m_original.y() - n.y() * f);
      db::DPoint e2(m_original.x() + n.x() * f, m_original.y() + n.y() * f);
  ...
  void do_find(lay::LayoutViewBase *view, int cv_index, const db::Cell &cell,
               unsigned int l, int min_level, int max_level,
               const db::CplxTrans &t) {
    db::Box touch_box = t.inverted() * m_region;

    if (max_level <= 0 || !cell.bbox(l).touches(touch_box) ||
        view->is_cell_hidden(cell.cell_index(), cv_index)) {
      return;
    }
    ...
    if (min_level <= 0) {
      ...
      db::ShapeIterator shape = shapes.begin_touching(
          touch_box,
          db::ShapeIterator::Polygons | db::ShapeIterator::Paths |
              db::ShapeIterator::Boxes | db::ShapeIterator::Points,
      ...
      while (!shape.at_end() && m_tests > 0) {
        ...
        if (shape->is_polygon()) {
          ...
          for (db::Shape::polygon_edge_iterator e = shape->begin_edge();
               !e.at_end(); ++e) {
            test_edge(t * *e);
          }
          ...
          test_edge(t *
                    db::Edge(box.p2(), db::Point(box.right(), box.bottom())));
          test_edge(t *
                    db::Edge(db::Point(box.right(), box.bottom()), box.p1()));
    ...
    while (!inst.at_end() && m_tests > 0) {
      ...
      for (db::CellInstArray::iterator p =
               cell_inst.begin_touching(touch_box, bc);
           !p.at_end() && m_tests > 0; ++p) {

        --m_tests;

        do_find(view, cv_index,
                mp_layout->cell(cell_inst.object().cell_index()), l,
                min_level - 1, max_level - 1, t * cell_inst.complex_trans(*p));
      }
  ...
  void test_edge(const db::DEdge &edg) {
    if (m_with_vertex) {

      //  vertex snap is just annoying when trying to measure the width of
      //  simulation contours .. But: when measuring corner-to-corner distances
      //  it is very valuable ..
...
do_obj_snap(lay::LayoutViewBase *view, const db::DPoint &pt,
            const db::DVector &grid, double snap_range,
            const std::vector<db::DEdge> &cutlines) {
  db::DPoint dp(pt);
  ...
  for (std::vector<db::DEdge>::const_iterator c = cutlines.begin();
       c != cutlines.end(); ++c) {
    std::pair<bool, db::DPoint> r =
        c->cut_point(db::DEdge(dp, dp + db::DVector(-c->dy(), c->dx())));
    if (r.first) {
  ...
  //  if both the projection and the finder are successful, decide by a
  //  heuristic criterion which to take (the projection gets a penalty (of the
  //  snap range) to make it count less than the finder's choice) This avoids
  //  extreme distortions of the ruler due to projection on long edges.
  if (finder.any() && anyp &&
      (dp.distance(closest) + snap_range) * 5.0 <
          dp.distance(finder.get_found())) {
...
do_obj_snap2(lay::LayoutViewBase *view, const db::DPoint &pt1,
             const db::DPoint &pt2, const db::DVector &grid,
             double min_search_range, double max_search_range,
             const std::vector<db::DEdge> &cutlines) {
  db::DPoint dp1(pt1);
  db::DPoint dp2(pt2);

  ContourFinder finder(dp1, grid, cutlines,
                       cutlines.empty() /*vertex snap on "any direction"*/);
  ...
  while (sr < max_search_range + 1e-6) {
    ...
    if ((cutlines.empty() && finder.any()) || finder.any_exact()) {
      ...
      if (!cutlines.empty()) {
        ...
        for (std::vector<db::DEdge>::const_iterator i = cutlines.begin();
             i != cutlines.end(); ++i) {
        ...
        n = finder.get_found_edge().d().transformed(
            db::DTrans(db::DTrans::r90));
      ...
      ContourFinder finder2(dp2, grid, cl, false /*no vertex snap*/,
                            true /*directional cutlines*/);
      ...
      while (sr2 < max_search_range + 1e-6) {
        ...
        if (finder2.any_exact()) {
          ...
          if (finder.is_vertex()) {
            ...
            res.object_snap_first =
                TwoPointSnapToObjectResult::ObjectUnspecific;
          }
...
static void make_cutlines(lay::angle_constraint_type snap_mode,
                          const db::DPoint &p1,
...
PointSnapToObjectResult obj_snap(lay::LayoutViewBase *view,
                                 const db::DPoint &pt, const db::DVector &grid,
                                 double snap_range) {
  return do_obj_snap(view, pt, grid, snap_range, std::vector<db::DEdge>());
}

PointSnapToObjectResult obj_snap(lay::LayoutViewBase *view,
                                 const db::DPoint &p1, const db::DPoint &p2,
                                 const db::DVector &grid,
...
TwoPointSnapToObjectResult obj_snap2(lay::LayoutViewBase *view,
                                     const db::DPoint &pt,
                                     const db::DVector &grid,
...
obj_snap2(lay::LayoutViewBase *view, const db::DPoint &pt,
          const db::DVector &grid, lay::angle_constraint_type ac,
          double min_search_range, double max_search_range) {
  return obj_snap2(view, pt, pt, grid, ac, min_search_range, max_search_range);
}

TwoPointSnapToObjectResult
obj_snap2(lay::LayoutViewBase *view, const db::DPoint &pt1,
          const db::DPoint &pt2, const db::DVector &grid,
          double min_search_range, double max_search_range) {
  db::DPoint dp1 = lay::snap_xy(pt1, grid);
  db::DPoint dp2 = lay::snap_xy(pt2, grid);

  return do_obj_snap2(view, dp1, dp2, db::DVector(), min_search_range,
                      max_search_range, std::vector<db::DEdge>());
}

TwoPointSnapToObjectResult
obj_snap2(lay::LayoutViewBase *view, const db::DPoint &pt1,
          const db::DPoint &pt2, const db::DVector &grid,
          lay::angle_constraint_type snap_mode, double min_search_range,
          double max_search_range) {
  db::DPoint dp1 = lay::snap_xy(pt1, grid);
  db::DPoint dp2 = lay::snap_xy(pt2, grid);
