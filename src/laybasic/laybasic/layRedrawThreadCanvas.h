...
public:
   ...
   *  @brief Constructor
   */
  RedrawThreadCanvas() : m_resolution(1.0), m_width(0), m_height(0) {}
   ...
   *
   *  This method is called (from the redraw thread) if a transfer has been
   ...
   */
  virtual bool shift_supported() const { return false; }
   ...
   *  @param shifting The shift vector by which the original image should be
   * shifted to form the background or 0 if no shifting is required
   *  @param layers The set of plane indexes to initialize (if null, all planes
   * are initialized). A negative value initializes the drawing planes.
   *  @param resolution The resolution in which the image is drawn
   *  @param drawings The custom drawing interface which is responsible to draw
   * user objects
   */
  virtual void prepare(unsigned int /*nlayers*/, unsigned int width,
   ...
   *  @brief Set a plane
   *
   *  This method is called from the redraw thread to transfer data for a
   * certain plane. The planes have been reserved before with init_plane ().
   */
  virtual void set_plane(unsigned int n, const lay::CanvasPlane *plane) = 0;

  /**
   *  @brief Set a plane for the drawing number d and index n within the
   * drawing.
   *
   *  This method is called from the redraw thread.
   */
  virtual void set_drawing_plane(unsigned int d, unsigned int n,
                                 const lay::CanvasPlane *plane) = 0;
  ...
  /**
   *  @brief Initialize a drawing plane for drawing on drawing d and plane
   * number n
   */
  virtual void initialize_plane(lay::CanvasPlane *plane, unsigned int d,
                                unsigned int n) = 0;
...
public:
   ...
   */
  BitmapCanvasData(const BitmapCanvasData &data) : m_width(0), m_height(0) {
    operator=(data);
  }
   ...
   */
  void fetch(std::vector<lay::Bitmap *> &plane_buffers,
...
private:
  void clear_planes();
  static void assign(std::vector<lay::Bitmap *> &to,
                     const std::vector<lay::Bitmap *> &from);
  static void assign(std::vector<std::vector<lay::Bitmap *>> &to,
...
class BitmapRedrawThreadCanvas : public RedrawThreadCanvas {
public:
  /**
   *  @brief Constructor
   */
   ...
   *
   *  This method is called from the redraw thread to transfer data for a
   * certain plane. The planes have been reserved before with prepare ().
   */
   ...
   */
  void to_image(const std::vector<lay::ViewOp> &view_ops,
                const lay::DitherPattern &dp, const lay::LineStyles &ls,
                double dpr, tl::Color background, tl::Color foreground,
                tl::Color active, const lay::Drawings *drawings,
                tl::PixelBuffer &img, unsigned int width, unsigned int height);

  /**
   *  @brief Transfer the content to a BitmapBuffer (monochrome)
   */
  void to_image_mono(const std::vector<lay::ViewOp> &view_ops,
                     const lay::DitherPattern &dp, const lay::LineStyles &ls,
                     double dpr, bool background, bool foreground, bool active,
                     const lay::Drawings *drawings, tl::BitmapBuffer &img,
                     unsigned int width, unsigned int height);
  ...
  BitmapCanvasData store_data() const {
    return BitmapCanvasData(mp_plane_buffers, mp_drawing_plane_buffers, m_width,
                            m_height);
  }

  /**
   *  @brief Gets a value indicating whether we can restore the given data
   * object
   */
  bool can_restore_data(const BitmapCanvasData &data) const {
    return data.can_fetch(mp_plane_buffers, mp_drawing_plane_buffers, m_width,
                          m_height);
  }
