...
static const char *def_palette = "255,157,157[0] "
                                 "255,128,168[1] "
                                 "192,128,255[2] "
                                 "149,128,255[3] "
                                 "128,134,255[4] "
...
ColorPalette::ColorPalette(const std::vector<tl::color_t> &colors,
...
bool ColorPalette::operator==(const ColorPalette &d) const {
  return m_colors == d.m_colors &&
         m_luminous_color_indices == d.m_luminous_color_indices;
}

tl::color_t ColorPalette::color_by_index(unsigned int n) const {
...
void ColorPalette::clear_colors() { m_colors.clear(); }
...
void ColorPalette::clear_luminous_colors() { m_luminous_color_indices.clear(); }

std::string ColorPalette::to_string() const {
  std::string res;
...
void ColorPalette::from_string(const std::string &s, bool simple) {
  try {
    ...
    while (true) {
      ...
      m_colors.push_back(0xff000000 | (tl::color_t(r & 0xff) << 16) |
                         (tl::color_t(g & 0xff) << 8) | tl::color_t(b & 0xff));
    ...
    if (!simple && (colors() == 0 || luminous_colors() == 0)) {
      throw tl::Exception(tl::to_string(
          tr("invalid palette - no colors and/or default colors")));
    }

  } catch (std::exception &ex) {
    //  reformat error message
    throw tl::Exception(tl::sprintf(
        tl::to_string(tr("Color palette string format error: %s")), ex.what()));
  }
}
