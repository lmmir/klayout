...
//  The distance by which the mouse must move in order to create a
//  press/move/release event rather than a single click event:
...
#if defined(HAVE_QT)
static unsigned int qt_to_buttons(Qt::MouseButtons b, Qt::KeyboardModifiers m) {
  ...
  return ((b & Qt::LeftButton)
              ? ((m & Qt::MetaModifier) ? RightButton : LeftButton)
              : 0) |
...
BackgroundViewObject::~BackgroundViewObject() { redraw(); }

void BackgroundViewObject::visible(bool vis) {
  if (vis != m_visible) {
    m_visible = vis;
    redraw();
  }
...
void BackgroundViewObject::redraw() {
  if (widget()) {
    widget()->touch_bg();
  }
}

void BackgroundViewObject::z_order(int z) {
  if (z != m_z_order) {
    m_z_order = z;
    redraw();
  }
...
ViewObject::~ViewObject() { redraw(); }

void ViewObject::set_dismissable(bool dismissable) {
...
void ViewObject::visible(bool vis) {
  if (vis != m_visible) {
    m_visible = vis;
    redraw();
  }
...
void ViewObject::thaw() {
  if (widget()) {
    widget()->thaw(this);
  }
}

void ViewObject::freeze() {
  if (widget()) {
    widget()->freeze(this);
  }
}
...
void ViewService::enable(bool en) { m_enabled = en; }

void ViewService::set_cursor(lay::Cursor::cursor_shape cursor) {
...
public:
  ...
  void resizeEvent(QResizeEvent *) { mp_view->resize_event(width(), height()); }
  ...
  void dragEnterEvent(QDragEnterEvent *event) {
    ...
    if (dd) {
      ...
      db::DPoint p =
          mp_view->pixel_to_um(db::Point(event->pos().x(), event->pos().y()));
  ...
  void mouseMoveEvent(QMouseEvent *e) {
    ...
    mp_view->send_mouse_move_event(p,
                                   qt_to_buttons(e->buttons(), e->modifiers()));
  ...
  void mouseDoubleClickEvent(QMouseEvent *e) {
    ...
    mp_view->send_mouse_double_clicked_event(
        p, qt_to_buttons(e->buttons(), e->modifiers()));
  ...
  void wheelEvent(QWheelEvent *e) {
    ...
    mp_view->send_wheel_event(delta, horizontal, p,
                              qt_to_buttons(e->buttons(), e->modifiers()));
  ...
  void mousePressEvent(QMouseEvent *e) {
    ...
    mp_view->send_mouse_press_event(
        p, qt_to_buttons(e->buttons(), e->modifiers()));
  ...
  void mouseReleaseEvent(QMouseEvent *e) {
    ...
    mp_view->send_mouse_release_event(
        p, qt_to_buttons(e->buttons(), e->modifiers()));
...
ViewObjectUI::ViewObjectUI()
    : m_view_objects_dismissed(false), m_needs_update_static(false),
      m_needs_update_bg(false), mp_active_service(0),
      m_mouse_pressed_state(false), m_mouse_buttons(0), m_in_mouse_move(false),
      m_mouse_inside(false), m_cursor(lay::Cursor::none),
      m_default_cursor(lay::Cursor::none), m_widget_width(0),
      m_widget_height(0), m_image_updated(false) {
...
#if defined(HAVE_QT)
void ViewObjectUI::init_ui(QWidget *parent) {
...
void ViewObjectUI::register_service(lay::ViewService *svc) {
  m_services.push_back(svc);
}

void ViewObjectUI::unregister_service(lay::ViewService *svc) {
  ...
  for (std::list<lay::ViewService *>::iterator s = m_services.begin();
       s != m_services.end(); ++s) {
    if (*s == svc) {
      m_services.erase(s);
      break;
...
void ViewObjectUI::activate(lay::ViewService *service) {
...
void ViewObjectUI::set_cursor(lay::Cursor::cursor_shape cursor) {
  m_cursor = cursor;
}

void ViewObjectUI::set_default_cursor(lay::Cursor::cursor_shape cursor) {
...
void ViewObjectUI::ensure_entered() {
  if (!m_mouse_inside) {
    send_enter_event();
  }
}

void ViewObjectUI::begin_mouse_event(lay::Cursor::cursor_shape cursor) {
  m_cursor = cursor;
}

void ViewObjectUI::end_mouse_event() {
#if defined(HAVE_QT)
  if (mp_widget) {
...
void ViewObjectUI::send_key_press_event(unsigned int key,
                                        unsigned int buttons) {
  bool done = false;
  if (mp_active_service) {
    done = (mp_active_service->enabled() &&
            mp_active_service->key_event(key, buttons));
  }
...
void ViewObjectUI::do_mouse_move() {
  m_in_mouse_move = true;

  if (m_mouse_pressed_state &&
      (abs(m_mouse_pos.x() - m_mouse_pressed.x()) > click_tolerance ||
       abs(m_mouse_pos.y() - m_mouse_pressed.y()) > click_tolerance)) {
    ...
    for (std::list<ViewService *>::iterator g = m_grabbed.begin();
         !done && g != m_grabbed.end();) {
      std::list<ViewService *>::iterator gg = g;
      ++gg;
      done = ((*g)->enabled() &&
              (*g)->mouse_press_event(p, m_mouse_buttons, true));
      g = gg;
    }

    if (!done && mp_active_service) {
      done = (mp_active_service->enabled() &&
              mp_active_service->mouse_press_event(p, m_mouse_buttons, true));
    }
    ...
    while (svc != end_services() && !done) {
      ...
      ++next;
              ...
              (*svc)->mouse_press_event(p, m_mouse_buttons, false));
      svc = next;
    }
  ...
  if (!m_mouse_pressed_state) {
    ...
    for (std::list<ViewService *>::iterator g = m_grabbed.begin();
      ...
      ++gg;
          ...
          ((*g)->enabled() && (*g)->mouse_move_event(p, m_mouse_buttons, true));
      g = gg;
    }

    if (!done && mp_active_service) {
      done = (mp_active_service->enabled() &&
              mp_active_service->mouse_move_event(p, m_mouse_buttons, true));
    }
    ...
    while (svc != end_services() && !done) {
      ...
      ++next;
              ...
              (*svc)->mouse_move_event(p, m_mouse_buttons, false));
      svc = next;
    }
...
void ViewObjectUI::send_mouse_move_event(const db::DPoint &pt,
                                         unsigned int buttons) {
  ensure_entered();
  m_mouse_pos = pt;
...
void ViewObjectUI::send_leave_event() {
  try {
    ...
    if (!done && mp_active_service) {
              ...
              mp_active_service->leave_event(true));
    }
...
void ViewObjectUI::send_enter_event() {
  m_mouse_inside = true;
  ...
  if (!done && mp_active_service) {
        ...
        (mp_active_service->enabled() && mp_active_service->enter_event(true));
  }
...
void ViewObjectUI::send_mouse_press_event(const db::DPoint &pt,
                                          unsigned int buttons) {
  ensure_entered();
#if defined(HAVE_QT)
  if (mp_widget) {
...
void ViewObjectUI::send_mouse_double_clicked_event(const db::DPoint &pt,
                                                   unsigned int buttons) {
  ensure_entered();
  begin_mouse_event(lay::Cursor::none);
  ...
  for (std::list<ViewService *>::iterator g = m_grabbed.begin();
    ...
    ++gg;
        ...
        ((*g)->m_enabled && (*g)->mouse_double_click_event(p, buttons, true));
    g = gg;
  }

  if (!done && mp_active_service) {
    done = (mp_active_service->enabled() &&
            mp_active_service->mouse_double_click_event(p, buttons, true));
  }
  ...
  while (svc != end_services() && !done) {
    ...
    ++next;
            ...
            (*svc)->mouse_double_click_event(p, buttons, false));
    svc = next;
  }
...
void ViewObjectUI::send_mouse_release_event(const db::DPoint &pt,
                                            unsigned int /*buttons*/) {
  try {
    ...
    for (std::list<ViewService *>::iterator g = m_grabbed.begin();
      ...
      if (m_mouse_pressed_state) {
               ...
               (*g)->mouse_click_event(p, m_mouse_buttons, true);
      } else {
        done = (*g)->enabled() &&
               (*g)->mouse_release_event(p, m_mouse_buttons, true);
      }
      g = gg;
    }
...
void ViewObjectUI::send_wheel_event(int delta, bool horizontal,
                                    const db::DPoint &pt,
                                    unsigned int buttons) {
  ensure_entered();
  begin_mouse_event();
  ...
  for (std::list<ViewService *>::iterator g = m_grabbed.begin();
    ...
    ++gg;
            ...
            (*g)->wheel_event(delta, horizontal, p, buttons, true));
    g = gg;
  }

  if (!done && mp_active_service) {
    done =
        (mp_active_service->enabled() &&
         mp_active_service->wheel_event(delta, horizontal, p, buttons, true));
  }
  ...
  while (svc != end_services() && !done) {
    ...
    ++next;
            ...
            (*svc)->wheel_event(delta, horizontal, p, buttons, false));
    svc = next;
  }
...
void ViewObjectUI::resize(unsigned int w, unsigned int h) {
  m_widget_width = w;
  m_widget_height = h;
  ...
  //  don't wait until the layout system informs us - which may never take place
  //  when the widget isn't shown. In the non-Qt case we need it anyway here.
  resize_event(w, h);
}

int ViewObjectUI::widget_height() const {
#if defined(HAVE_QT)
...
int ViewObjectUI::widget_width() const {
#if defined(HAVE_QT)
...
db::DPoint ViewObjectUI::pixel_to_um(const db::Point &pt) const {
...
db::DPoint ViewObjectUI::pixel_to_um(const db::DPoint &pt) const {
  ...
  // db::DPoint (pt.x (), widget_height () - 1 - pt.y ())
  // 这部分是视窗坐标转为画布坐标
  // m_trans.inverted ()*上面的db::DPoint(画布坐标)结果就是微米坐标
  return m_trans.inverted() * db::DPoint(pt.x(), widget_height() - 1 - pt.y());
}

void ViewObjectUI::mouse_event_trans(const db::DCplxTrans &trans) {
  if (trans != m_trans) {
    m_trans = trans;
    //  issue a move event in order to reposition the mouse in the new
    //  coordinate system since this may be called from within a mouse move
    //  event handler we need the recursion sentinel
    if (!m_in_mouse_move) {
      do_mouse_move();
    }
  }
...
struct z_order_compare_f {
  bool operator()(lay::BackgroundViewObject *a,
                  lay::BackgroundViewObject *b) const {
    int za = a->z_order();
    int zb = b->z_order();
    return za < zb;
  }
...
void ViewObjectUI::do_render_bg(const lay::Viewport &vp,
                                lay::ViewObjectCanvas &canvas) {
  m_needs_update_bg = false;

  std::vector<lay::BackgroundViewObject *> bg_objects;

  for (background_object_iterator obj = begin_background_objects();
       obj != end_background_objects(); ++obj) {
    if (obj->is_visible()) {
  ...
  for (std::vector<lay::BackgroundViewObject *>::const_iterator obj =
           bg_objects.begin();
       obj != bg_objects.end(); ++obj) {
    BEGIN_PROTECTED_SILENT(*obj)->render_bg(vp, canvas);
    END_PROTECTED_SILENT
  }
}

void ViewObjectUI::do_render(const lay::Viewport &vp,
                             lay::ViewObjectCanvas &canvas, bool st) {
  if (st) {
  ...
  for (object_iterator obj = begin_objects(); obj != end_objects(); ++obj) {
    if (obj->m_static == st && obj->is_visible() &&
        (!m_view_objects_dismissed || !obj->get_dismissable())) {
...
void ViewObjectUI::grab_mouse(ViewService *obj, bool a) {
...
void ViewObjectUI::ungrab_mouse(ViewService *obj) {
  std::list<ViewService *>::iterator g;
  for (g = m_grabbed.begin(); g != m_grabbed.end() && *g != obj; ++g) {
    ;
  }
...
void ViewObjectUI::freeze(ViewObject *obj) {
  if (!obj->m_static) {
    obj->m_static = true;
...
void ViewObjectUI::thaw(ViewObject *obj) {
  if (obj->m_static) {
    obj->m_static = false;
    m_needs_update_static = true;
    //  no update needed since the display will not change through this.
...
#if !defined(HAVE_QT)
void ViewObjectUI::update() {
  //  NOTE: this does not need to be thread-safe as we make sure (as in Qt) that
  //  update() is always called from the main thread.
  m_image_updated = true;
}

bool ViewObjectUI::image_updated() {
...
void ViewObjectUI::resize_event(unsigned int /*w*/, unsigned int /*h*/) {
  //  .. nothing yet ..
}

void ViewObjectUI::paint_event() {
  //  .. nothing yet ..
}

void ViewObjectUI::gtf_probe() {
  //  .. nothing yet ..
}

void ViewObjectUI::touch() {
...
void ViewObjectUI::set_dismiss_view_objects(bool dismiss) {
...
void ViewObjectUI::objects_changed() {
  touch();
  update();
}

db::DBox ViewObjectUI::mouse_event_viewport() const {
  db::DPoint p1 = m_trans.inverted() * db::DPoint(0, 0);
  db::DPoint p2 =
      m_trans.inverted() * db::DPoint(widget_width(), widget_height());
...
BitmapViewObjectCanvas::BitmapViewObjectCanvas()
    : ViewObjectCanvas(), m_renderer(1, 1, 1.0), m_width(1), m_height(1),
      m_resolution(1.0) {
  // .. nothing yet ..
}

BitmapViewObjectCanvas::BitmapViewObjectCanvas(unsigned int width,
                                               unsigned int height,
                                               double resolution)
    : ViewObjectCanvas(), m_renderer(width, height, resolution), m_width(width),
      m_height(height), m_resolution(resolution) {
  // .. nothing yet ..
}

BitmapViewObjectCanvas::~BitmapViewObjectCanvas() { clear_fg_bitmaps(); }

lay::CanvasPlane *BitmapViewObjectCanvas::plane(const lay::ViewOp &style) {
  std::map<lay::ViewOp, unsigned int>::iterator b =
      m_fg_bitmap_table.find(style);
  if (b == m_fg_bitmap_table.end()) {

    //  we need to create a new plane
    m_fg_bitmap_table.insert(
        std::make_pair(style, (unsigned int)mp_alloc_bitmaps.size()));
...
BitmapViewObjectCanvas::plane(const std::vector<lay::ViewOp> &style) {
  std::map<std::vector<lay::ViewOp>, unsigned int>::iterator b =
      m_fgv_bitmap_table.find(style);
  if (b == m_fgv_bitmap_table.end()) {

    //  we need to create a new bitmap
    m_fgv_bitmap_table.insert(
        std::make_pair(style, (unsigned int)mp_alloc_bitmaps.size()));
    lay::Bitmap *bm = new lay::Bitmap(m_width, m_height, m_resolution);
    mp_alloc_bitmaps.push_back(bm);
    for (std::vector<lay::ViewOp>::const_iterator s = style.begin();
         s != style.end(); ++s) {
      mp_fg_bitmaps.push_back(bm);
...
void BitmapViewObjectCanvas::clear_fg_bitmaps() {
  for (std::vector<lay::Bitmap *>::iterator i = mp_alloc_bitmaps.begin();
       i != mp_alloc_bitmaps.end(); ++i) {
    if (*i) {
      delete *i;
    }
  }
...
void BitmapViewObjectCanvas::sort_planes() {
  //  sort the planes by view operator - this ensures a certain plane order as
  //  implied by the plane index of the plane operators.
...
void BitmapViewObjectCanvas::set_size(unsigned int width, unsigned int height,
                                      double resolution) {
...
void BitmapViewObjectCanvas::set_size(double resolution) {
  m_renderer = lay::BitmapRenderer(m_width, m_height, resolution);
  m_resolution = resolution;
}

tl::PixelBuffer *BitmapViewObjectCanvas::bg_image() { return 0; }

tl::BitmapBuffer *BitmapViewObjectCanvas::bg_bitmap() { return 0; }
