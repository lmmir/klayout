 ...
 *
 *  This is a normalized representation of a db::InstElement object suitable to
 * being stored in a XML document or disconnected from a layout object.
 */
struct LAYBASIC_PUBLIC SpecificInst {
  ...
  /**
   *  @brief Convert the specific instance to a db::InstElement object with the
   * given parent cell
   *
  ...
  to_inst_element(const db::Layout &layout, const db::Cell &parent_cell) const;
 ...
 *
 *  Basically this structure is just needed to provide a nice adaptor for the
 * XML reader/writer in the BookmarkList
 */
struct LAYBASIC_PUBLIC CellPath {
  ...
  void push_back_path(const std::string &name) { path.push_back(name); }
...
struct LAYBASIC_PUBLIC DisplayState {
   ...
   */
  DisplayState(const db::DBox &b, int min_hier, int max_hier,
   ...
   */
  int min_hier() const { return m_min_hier; }

  /**
   *  @brief Set minimum drawn hierarchy level
   */
  void set_min_hier(int l) { m_min_hier = l; }

  /**
   *  @brief Obtain maximum drawn hierarchy level
   */
  int max_hier() const { return m_max_hier; }

  /**
   *  @brief Set maximum drawn hierarchy level
   */
  void set_max_hier(int l) { m_max_hier = l; }

  /**
   *  @brief Obtain the box
   */
  db::DBox box() const { return db::DBox(m_left, m_bottom, m_right, m_top); }

  double xleft() const { return m_left; }

  double xright() const { return m_right; }

  double ytop() const { return m_top; }

  double ybottom() const { return m_bottom; }

  void set_xleft(double c) { m_left = c; }

  void set_xright(double c) { m_right = c; }

  void set_ytop(double c) { m_top = c; }

  void set_ybottom(double c) { m_bottom = c; }

  /**
   *  @brief Obtain the raw cell name list
   */
  const std::list<CellPath> &paths() const { return m_paths; }

  /**
   *  @brief Add a new path
   */
  void set_paths(const std::list<CellPath> &p) { m_paths = p; }
