...
template <class X, class Y> struct first_of_pair_cmp_f {
...
Editable::Editable(lay::Editables *editables) : mp_editables(editables) {
  if (editables) {
...
Editable::~Editable() {
  //  Reasoning for reset (): on MSVC, virtual functions must not be called
  //  inside the destructor. reset () avoids that lay::Editables::enable calls
  //  us.
  reset();
...
Editables::Editables(db::Manager *manager)
    : db::Object(manager), m_move_selection(false),
      m_any_move_operation(false) {}

Editables::~Editables() { cancel_edits(); }

void Editables::del(db::Transaction *transaction) {
  std::unique_ptr<db::Transaction> trans_holder(
      transaction
          ? transaction
          : new db::Transaction(manager(), tl::to_string(tr("Delete"))));
...
db::DBox Editables::selection_bbox() {
  db::DBox sel_bbox;
  for (iterator e = begin(); e != end(); ++e) {
...
db::DBox Editables::selection_catch_bbox() {
  db::DBox sel_bbox;
  for (iterator e = begin(); e != end(); ++e) {
...
void Editables::transform(const db::DCplxTrans &t,
  ...
  std::unique_ptr<db::Transaction> trans_holder(
      transaction
          ? transaction
          : new db::Transaction(manager(), tl::to_string(tr("Transform"))));
...
void Editables::transient_select(const db::DPoint &pt) {
  bool same_point =
      (m_last_selected_point.is_point() &&
       m_last_selected_point.center().sq_double_distance(pt) < 1e-10);
  if (!same_point) {
    clear_previous_selection();
  }

  //  in a first pass evaluate the point selection proximity value to select
  //  those plugins that are active. This code is a copy of the code for the
  //  single-point selection below.
  ...
  for (iterator e = begin(); e != end(); ++e) {
    if (m_enabled.find(&*e) != m_enabled.end()) {
      plugins.push_back(
          std::make_pair(e->click_proximity(pt, lay::Editable::Replace), e));
    }
  }

  //  sort the plugins found by the proximity
  std::sort(plugins.begin(), plugins.end(),
            first_of_pair_cmp_f<double, iterator>());

  //  and call select on those objects until the first one (with the least
  //  proximity) has something selected
  ...
  //  if no plugin has selected anything, try a reset (clear previous selection)
  //  and select again: this is supposed to implement the cycling protocol which
  //  allows the plugins to cycle through different selections for repeated
  //  clicks on the same point. Let this happen for replace mode only because
  //  otherwise clearing the selection does not make sense.
...
void Editables::select(const db::DPoint &pt,
  ...
  //  if no plugin has selected anything, try a reset (clear_previous selection)
  //  and select again: this is supposed to implement the cycling protocol which
  //  allows the plugins to cycle through different selections for repeated
  //  clicks on the same point. Let this happen for replace mode only because
  //  otherwise clearing the selection does not make sense.
...
bool Editables::begin_move(const db::DPoint &p, lay::angle_constraint_type ac) {
  ...
  //  In a first pass evaluate the point selection proximity value to select
  //  those plugins that are close to the given point. This code is a copy of
  //  the code for the single-point selection below.
  std::vector<std::pair<double, iterator>> plugins;

  for (iterator e = begin(); e != end(); ++e) {
    if (m_enabled.find(&*e) != m_enabled.end()) {
      plugins.push_back(
          std::make_pair(e->click_proximity(p, lay::Editable::Replace), e));
    }
  }
  ...
  if (has_selection() && selection_catch_bbox().contains(p)) {
    ...
    //  issue a move operation on all editables: first try a Partial mode
    //  begin_move
    for (std::vector<std::pair<double, iterator>>::const_iterator pi =
             plugins.begin();
         pi != plugins.end(); ++pi) {

      if (pi->second->begin_move(Editable::Partial, p, ac)) {

        //  clear the selection on all other plugins, because we focus on one
        //  plugin
        for (std::vector<std::pair<double, iterator>>::const_iterator pii =
                 plugins.begin();
             pii != plugins.end(); ++pii) {
    ...
    std::vector<std::pair<double, iterator>>::const_iterator pi =
        plugins.begin();
    //  HACK: only the first plugin (measured through click_proximity) has a
    //  chance to intercept the standard procedure which is select + move
    //  selected. The reason behind that is that for example edtService does not
    //  implement Any but rather relies on select + Selected mode. Hence
    //  allowing some "later" plugin to override it at this point would break
...
void Editables::move(const db::DPoint &p, lay::angle_constraint_type ac) {
...
void Editables::move_transform(const db::DPoint &p, db::DFTrans t,
...
void Editables::end_move(const db::DPoint &p, lay::angle_constraint_type ac,
                         db::Transaction *transaction) {
  std::unique_ptr<db::Transaction> trans_holder(
      transaction ? transaction
                  : new db::Transaction(manager(), tl::to_string(tr("Move"))));
  ...
  if (m_any_move_operation) {
    ...
    //  if nothing was moved, treat the end_move as a select which makes the
    //  move sticky or replaces a complex selection by a simple one
    edit_cancel();
    select(p, Editable::Replace);
  }
}

size_t Editables::selection_size() {
  size_t c = 0;
  for (iterator e = begin(); e != end(); ++e) {
    c += e->selection_size();
  }
