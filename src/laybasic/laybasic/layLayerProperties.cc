 ...
 */
tl::color_t LayerProperties::brighter(tl::color_t in, int x) {
...
LayerProperties::LayerProperties()
    : m_gen_id(0), m_frame_color(0), m_frame_color_real(0), m_fill_color(0),
      m_fill_color_real(0), m_frame_brightness(0), m_frame_brightness_real(0),
      m_fill_brightness(0), m_fill_brightness_real(0), m_dither_pattern(-1),
      m_dither_pattern_real(-1), m_line_style(-1), m_line_style_real(-1),
      m_valid(true), m_valid_real(true), m_visible(true), m_visible_real(true),
      m_transparent(false), m_transparent_real(false), m_width(-1),
      m_width_real(-1), m_marked(false), m_marked_real(false), m_xfill(false),
      m_xfill_real(false), m_animation(0), m_animation_real(0), m_source(),
      m_source_real(), m_layer_index(-1), m_cellview_index(-1),
      m_inv_prop_set(false), m_realize_needed_source(true),
      m_realize_needed_visual(true) {
  // .. nothing yet ..
}

LayerProperties::LayerProperties(const LayerProperties &d)
    : gsi::ObjectBase(d), m_gen_id(0), m_frame_color(0), m_frame_color_real(0),
      m_fill_color(0), m_fill_color_real(0), m_frame_brightness(0),
      m_frame_brightness_real(0), m_fill_brightness(0),
      m_fill_brightness_real(0), m_dither_pattern(-1),
      m_dither_pattern_real(-1), m_line_style(-1), m_line_style_real(-1),
      m_valid(true), m_valid_real(true), m_visible(true), m_visible_real(true),
      m_transparent(false), m_transparent_real(false), m_width(-1),
      m_width_real(-1), m_marked(false), m_marked_real(false), m_xfill(false),
...
LayerProperties &LayerProperties::operator=(const LayerProperties &d) {
  if (&d != this) {
    ...
    if (m_frame_color != d.m_frame_color || m_fill_color != d.m_fill_color ||
        m_frame_brightness != d.m_frame_brightness ||
        m_fill_brightness != d.m_fill_brightness ||
        m_dither_pattern != d.m_dither_pattern ||
        m_line_style != d.m_line_style || m_valid != d.m_valid ||
        m_visible != d.m_visible || m_transparent != d.m_transparent ||
        m_width != d.m_width || m_marked != d.m_marked ||
        m_xfill != d.m_xfill || m_animation != d.m_animation) {
...
bool LayerProperties::operator==(const LayerProperties &d) const {
  ...
  //  do not consider the derived and "real" properties  - these is not really a
  //  property
  return m_frame_color == d.m_frame_color && m_fill_color == d.m_fill_color &&
         m_frame_brightness == d.m_frame_brightness &&
         m_fill_brightness == d.m_fill_brightness &&
         m_dither_pattern == d.m_dither_pattern &&
         m_line_style == d.m_line_style && m_valid == d.m_valid &&
         ...
         m_width == d.m_width && m_marked == d.m_marked &&
         m_xfill == d.m_xfill && m_animation == d.m_animation &&
         m_name == d.m_name && m_source == d.m_source;
}

bool LayerProperties::is_visual() const {
  return valid(true) && visible(true) &&
         (layer_index() >= 0 || is_cell_box_layer());
}

tl::color_t LayerProperties::eff_frame_color(bool real) const {
  return brighter(frame_color(real) & 0xffffff, frame_brightness(real));
}

tl::color_t LayerProperties::eff_fill_color(bool real) const {
  return brighter(fill_color(real) & 0xffffff, fill_brightness(real));
}

tl::color_t
LayerProperties::eff_frame_color_brighter(bool real,
                                          int plus_brightness) const {
  return brighter(frame_color(real) & 0xffffff,
                  frame_brightness(real) + plus_brightness);
}

tl::color_t
LayerProperties::eff_fill_color_brighter(bool real, int plus_brightness) const {
  return brighter(fill_color(real) & 0xffffff,
                  fill_brightness(real) + plus_brightness);
}
...
LayerProperties LayerProperties::flat() const {
  ensure_realized();
...
class LayerSourceEval : public tl::Eval {
public:
  LayerSourceEval(const lay::LayerProperties &lp,
                  const lay::LayoutViewBase *view, bool real)
      : m_lp(lp), mp_view(view), m_real(real) {
    // .. nothing yet ..
  }

  const lay::ParsedLayerSource &source() const { return m_lp.source(m_real); }

  const lay::LayoutViewBase *view() const { return mp_view; }
...
class LayerSourceEvalFunction : public tl::EvalFunction {
public:
  ...
  void execute(const tl::ExpressionParserContext &context, tl::Variant &out,
               const std::vector<tl::Variant> &vv) const {
    if (vv.size() != 0) {
      throw tl::EvalError(
          tl::to_string(tr("Layer source function must not have arguments")),
          context);
    }
    ...
    if (m_function == 'N') {
      ...
      const lay::CellView &cv =
          mp_eval->view()->cellview(mp_eval->source().cv_index());
      if (cv.is_valid()) {
        out = cv->name();
      }
    }
...
std::string LayerProperties::display_string(const lay::LayoutViewBase *view,
                                            bool real,
    ...
    if (!m_name.empty()) {
      ...
      if (m_name.find("$") == std::string::npos) {
        ...
        eval.define_function(
            "N", new LayerSourceEvalFunction('N', &eval)); // layer name
        eval.define_function(
            "L", new LayerSourceEvalFunction('L', &eval)); // layer number
        eval.define_function(
            "D", new LayerSourceEvalFunction('D', &eval)); // datatype
        eval.define_function(
            "I", new LayerSourceEvalFunction('I', &eval)); // layer index
        eval.define_function(
            "C", new LayerSourceEvalFunction('C', &eval)); // cv index
        eval.define_function(
            "S", new LayerSourceEvalFunction('S', &eval)); // layer source
        eval.define_function("T",
                             new LayerSourceEvalFunction('T', &eval)); // title
  ...
  if (view) {
    ...
    if (m_cellview_index >= 0) {
      ...
      if (!m_source_real.property_selector().is_null()) {
        m_inv_prop_set = m_source_real.property_selector().matching(
            cv->layout().properties_repository(), m_prop_set);
      }
      ...
      if (m_source_real.special_purpose() == ParsedLayerSource::SP_None) {
        ...
        if (m_layer_index < 0 && !m_source_real.is_wildcard_layer()) {
          m_layer_index =
              cv->layout().get_layer_maybe(m_source_real.layer_props());
        }
      }
    }
  }
...
LayerPropertiesNode::LayerPropertiesNode()
    : LayerProperties(), m_list_index(0), m_expanded(false) {
  m_id = ++s_unique_id;
}
...
LayerPropertiesNode::LayerPropertiesNode(const LayerProperties &d)
    : LayerProperties(d), m_list_index(0), m_expanded(false) {
  m_id = ++s_unique_id;
}

LayerPropertiesNode::LayerPropertiesNode(const LayerPropertiesNode &d)
    : LayerProperties(d), tl::Object(), m_list_index(0),
      m_expanded(d.m_expanded), m_children(d.m_children), m_id(d.m_id) {
...
unsigned int LayerPropertiesNode::list_index() const { return m_list_index; }
...
void LayerPropertiesNode::expanded_state_changed() { touch(); }
...
void LayerPropertiesNode::need_realize(unsigned int flags, bool force) {
  ...
  if ((flags & (nr_visual + nr_source)) != 0 &&
      (force || !realize_needed_visual() || !realize_needed_source())) {
    for (iterator c = m_children.begin(); c != m_children.end(); ++c) {
...
db::DBox LayerPropertiesNode::bbox() const {
  tl_assert(mp_view);
  ...
  if (!cv.is_valid()) {
    ...
    for (std::vector<db::DCplxTrans>::const_iterator t = trans().begin();
         t != trans().end(); ++t) {
    ...
    for (std::vector<db::DCplxTrans>::const_iterator t = trans().begin();
         t != trans().end(); ++t) {
      b += (*t * db::CplxTrans(dbu) * cv.context_trans()) *
           cv.cell()->bbox(layer_index());
    }
    return b;
  }
}

void LayerPropertiesNode::attach_view(LayoutViewBase *view,
  ...
  //  Attachment of a view is a strong indication that something significant
  //  changed - recompute the source specifications on next request.
  m_realize_needed_source = true;
}

LayerPropertiesNode &
LayerPropertiesNode::insert_child(const iterator &iter,
                                  const LayerPropertiesNode &child) {
  refresh();
...
LayerPropertiesConstIterator::LayerPropertiesConstIterator(
    const lay::LayerPropertiesNode *node)
    : m_uint(0), m_list() {
  if (!node) {
    return;
  }
  ...
  while (node->parent()) {
    size_t index = 0;
    bool found = false;
    for (lay::LayerPropertiesNode::const_iterator c =
             node->parent()->begin_children();
         c != node->parent()->end_children(); ++c, ++index) {
      if (&*c == node) {
        found = true;
        break;
      }
  ...
  {
    const lay::LayerPropertiesList &list =
        node->view()->get_properties(node->list_index());
    size_t index = 0;
    bool found = false;
    for (lay::LayerPropertiesList::const_iterator c = list.begin_const();
         c != list.end_const(); ++c, ++index) {
      if (&*c == node) {
        found = true;
        break;
      }
...
LayerPropertiesConstIterator::LayerPropertiesConstIterator(
    const LayerPropertiesList &list, bool last)
...
LayerPropertiesConstIterator::LayerPropertiesConstIterator(
    const LayerPropertiesList &list, size_t uint)
    //  NOTE: there should be some "const_weak_ptr"
...
LayerPropertiesConstIterator::LayerPropertiesConstIterator(
    const LayerPropertiesConstIterator &d)
...
bool LayerPropertiesConstIterator::operator<(
    const LayerPropertiesConstIterator &d) const {
  tl_assert(m_list);
...
std::pair<size_t, size_t> LayerPropertiesConstIterator::factor() const {
  tl_assert(m_list);
...
LayerPropertiesConstIterator &LayerPropertiesConstIterator::up() {
  m_uint %= factor().first;
  mp_obj.reset(0);
  return *this;
}
...
size_t LayerPropertiesConstIterator::num_siblings() const {
  std::pair<size_t, size_t> f = factor();
  return f.second - 2;
}

LayerPropertiesConstIterator &LayerPropertiesConstIterator::down_first_child() {
  std::pair<size_t, size_t> f = factor();
...
LayerPropertiesConstIterator &LayerPropertiesConstIterator::down_last_child() {
  std::pair<size_t, size_t> f = factor();
  const LayerPropertiesNode *o = obj();
  m_uint +=
      f.first * f.second * ((o->end_children() - o->begin_children()) + 1);
  mp_obj.reset(0);
  return *this;
}
...
size_t LayerPropertiesConstIterator::child_index() const {
  std::pair<size_t, size_t> f = factor();
  return ((m_uint / f.first) % f.second) - 1;
}
...
LayerPropertiesList::LayerPropertiesList() : tl::Object(), m_list_index(0) {
  //  .. nothing yet ..
}
...
static bool has_cv_ref(const LayerPropertiesNode &node, int cv_ref) {
  if (!node.has_children()) {
    ...
    return node.source(true).cv_index() == cv_ref &&
           (node.is_cell_box_layer() || node.is_standard_layer());

  } else {

    for (LayerPropertiesNode::const_iterator c = node.begin_children();
         c != node.end_children(); ++c) {
...
void LayerPropertiesList::remove_cv_references(int cv_index, bool except) {
  ...
  for (std::vector<LayerPropertiesIterator>::const_iterator ll = cv_ref.begin();
       ll != cv_ref.end(); ++ll) {
    erase(*ll);
  }
}
...
static bool has_wildcard_layout(const LayerPropertiesNode &node, bool any) {
  if (!node.has_children()) {
    ...
    return node.source(true).cv_index() < 0 &&
           (node.is_cell_box_layer() || node.is_standard_layer());
...
static LayerPropertiesNode
expand_wildcard_layout(const LayerPropertiesNode &source, int new_cv_index) {
  ...
  if (!source.has_children()) {
    ...
    for (LayerPropertiesNode::const_iterator l = source.begin_children();
         l != source.end_children(); ++l) {
...
expand_wildcard_layers(const LayerPropertiesNode &lp,
                       const LayerPropertiesList &current_props,
                       lay::LayoutViewBase *view, unsigned int list_index) {
  ...
  if (cv_index >= 0 && cv_index < int(view->cellviews())) {
    ...
    //  NOTE: we use lay::ParsedLayerSource, but in a normalized form that does
    //  not include transformations and such. Hence we really figure out which
    //  layer is missing or not.

    std::set<lay::ParsedLayerSource> present;
    for (LayerPropertiesConstIterator l = current_props.begin_const_recursive();
         !l.at_end(); ++l) {
      if (!l->has_children()) {
    ...
    for (unsigned int l = 0; l < layout.layers(); ++l) {
      if (layout.is_valid_layer(l)) {
        actual.push_back(
            lay::ParsedLayerSource(layout.get_properties(l), cv_index));
      }
    }
    ...
    for (std::vector<lay::ParsedLayerSource>::const_iterator a = actual.begin();
      ...
      if (present.find(*a) == present.end()) {
        ...
        //  Build a new ParsedLayerSource combining the transformation,
        //  hierarchy levels and property selections from the wildcard one and
        //  the requested layer source
...
void LayerPropertiesList::append(const LayerPropertiesList &other) {
  {
    ...
    for (lay::LayerPropertiesIterator l = begin_recursive();
         l != end_recursive(); ++l) {
  ...
  for (lay::LayerPropertiesList::const_iterator l = other.begin_const();
       l != other.end_const(); ++l) {
    push_back(*l);
  }
}

void LayerPropertiesList::expand(const std::map<int, int> &map_cv_index,
                                 bool add_default) {
  tl_assert(view() != 0);

  //  Add a default element if required unless there already is one at top
  //  level. If there already is one, this one will be ignored.
  if (add_default) {
  ...
  if (!map_cv_index.empty()) {
    ...
    for (std::set<int>::const_iterator cv = cvrefs_to_erase.begin();
         cv != cvrefs_to_erase.end(); ++cv) {
  ...
  if (lywc) {
    ...
    //  If that is the case, iterate over all layouts (outer iteration) and
    //  create the wildcarded

    std::vector<lay::LayerPropertiesNode> new_nodes;

    for (unsigned int cv_index = 0; cv_index < view()->cellviews();
         ++cv_index) {
    ...
    for (std::vector<lay::LayerPropertiesNode>::const_iterator n =
             new_nodes.begin();
         n != new_nodes.end(); ++n) {
      push_back(*n);
    ...
    for (std::vector<LayerPropertiesIterator>::const_iterator ll =
             expanded.begin();
         ll != expanded.end(); ++ll) {
      erase(*ll);
    }
  ...
  //  after this sort we can modify the entries without invalidating the
  //  following iterators
  std::sort(lwc.begin(), lwc.end(), CompareLayerIteratorBottomUp());

  for (std::vector<LayerPropertiesIterator>::const_iterator ll = lwc.begin();
       ll != lwc.end(); ++ll) {
    LayerPropertiesIterator pos = *ll;
    //  Note: expand_wildcard_layers will recompute the already present layers
    //  on every call. Thus, only the first matching wildcard is effective.
    std::vector<LayerPropertiesNode> new_nodes =
        expand_wildcard_layers(*pos, *this, view(), list_index());
    for (std::vector<LayerPropertiesNode>::const_iterator n = new_nodes.begin();
         n != new_nodes.end(); ++n) {
      insert(pos, *n);
      pos.next_sibling();
    }
  ...
  for (LayerPropertiesIterator l = begin_recursive(); !l.at_end(); ++l) {
    ...
    if (!l->has_children()) {
      ...
      if (l->frame_color(true) == 0) {
        l->set_frame_color(view()->get_palette().luminous_color_by_index(
            l->source(true /*real*/).color_index()));
      }

      if (l->fill_color(true) == 0) {
        l->set_fill_color(view()->get_palette().luminous_color_by_index(
            l->source(true /*real*/).color_index()));
      }

      if (l->dither_pattern(true) < 0) {
        l->set_dither_pattern(
            view()->get_stipple_palette().standard_stipple_by_index(
                stipple_index));
      }
...
LayerPropertiesConstIterator LayerPropertiesList::end_const_recursive() const {
  return LayerPropertiesConstIterator(*this, true);
}

LayerPropertiesIterator LayerPropertiesList::begin_recursive() {
  return LayerPropertiesIterator(*this);
}

LayerPropertiesIterator LayerPropertiesList::end_recursive() {
  return LayerPropertiesIterator(*this, true);
}

LayerPropertiesList::const_iterator LayerPropertiesList::begin_const() const {
  return m_layer_properties.begin();
}

LayerPropertiesList::const_iterator LayerPropertiesList::end_const() const {
  return m_layer_properties.end();
}

LayerPropertiesList::iterator LayerPropertiesList::begin() {
  return m_layer_properties.begin();
}

LayerPropertiesList::iterator LayerPropertiesList::end() {
  return m_layer_properties.end();
}

LayerPropertiesNode &LayerPropertiesList::back() {
  return m_layer_properties.back();
}

const LayerPropertiesNode &LayerPropertiesList::back() const {
  return m_layer_properties.back();
}

/**
 *  @brief A helper class for XML parser: convert a string to a color and vice
 * versa
 */
struct UIntColorConverter : private ColorConverter {
...
/**
 *  @brief A helper class for XML parser: convert a string to a integer index
 * and vice versa (with -1 being a blank string)
 */
...
struct DitherPatternIndexConverter {
  std::string to_string(int b) const {
    if (b < 0) {
      return "";
    } else if (b < std::distance(
                       lay::DitherPattern::default_pattern().begin(),
                       lay::DitherPattern::default_pattern().begin_custom())) {
      //  intrinsic pattern
  ...
  void from_string(const std::string &s, int &b) const {
    if (s.empty()) {
      ...
      tl::from_string(s.c_str() + 1, b);
      b = b +
          std::distance(lay::DitherPattern::default_pattern().begin(),
                        lay::DitherPattern::default_pattern().begin_custom());
    } else {
      tl::from_string(s, b);
      if (b >= 16) {
...
/**
 *  @brief A helper class for XML parser: convert a style string to a integer
...
struct LineStyleIndexConverter {
  std::string to_string(int b) const {
    if (b < 0) {
      return "";
               ...
               std::distance(lay::LineStyles::default_style().begin(),
                             lay::LineStyles::default_style().begin_custom())) {
      //  intrinsic pattern
      return "I" + tl::to_string(b);
    } else {
      //  custom pattern
      return "C" +
             tl::to_string(
                 b - std::distance(
                         lay::LineStyles::default_style().begin(),
                         lay::LineStyles::default_style().begin_custom()));
    }
  }
  ...
  void from_string(const std::string &s, int &b) const {
    if (s.empty()) {
      ...
      tl::from_string(s.c_str() + 1, b);
      b = b + std::distance(lay::LineStyles::default_style().begin(),
                            lay::LineStyles::default_style().begin_custom());
    } else {
      tl::from_string(s, b);
      if (b >= 16) {
...
static const tl::XMLElementList layer_element = tl::XMLElementList(
    //  HINT: these make_member calls want to be qualified: otherwise an
    //  internal error was observed ..
    tl::make_member<bool, LayerPropertiesNode>(
        &LayerPropertiesNode::expanded, &LayerPropertiesNode::set_expanded,
        "expanded") +
    tl::make_member<tl::color_t, LayerPropertiesNode>(
        &LayerPropertiesNode::frame_color_loc,
        &LayerPropertiesNode::set_frame_color_code, "frame-color",
        UIntColorConverter()) +
    tl::make_member<tl::color_t, LayerPropertiesNode>(
        &LayerPropertiesNode::fill_color_loc,
        &LayerPropertiesNode::set_fill_color_code, "fill-color",
        UIntColorConverter()) +
    tl::make_member<int, LayerPropertiesNode>(
        &LayerPropertiesNode::frame_brightness_loc,
        &LayerPropertiesNode::set_frame_brightness, "frame-brightness") +
    tl::make_member<int, LayerPropertiesNode>(
        &LayerPropertiesNode::fill_brightness_loc,
        &LayerPropertiesNode::set_fill_brightness, "fill-brightness") +
    tl::make_member<int, LayerPropertiesNode>(
        &LayerPropertiesNode::dither_pattern_loc,
        &LayerPropertiesNode::set_dither_pattern, "dither-pattern",
        DitherPatternIndexConverter()) +
    tl::make_member<int, LayerPropertiesNode>(
        &LayerPropertiesNode::line_style_loc,
        &LayerPropertiesNode::set_line_style, "line-style",
        LineStyleIndexConverter()) +
    tl::make_member<bool, LayerPropertiesNode>(&LayerPropertiesNode::valid_loc,
                                               &LayerPropertiesNode::set_valid,
                                               "valid") +
    tl::make_member<bool, LayerPropertiesNode>(
        &LayerPropertiesNode::visible_loc, &LayerPropertiesNode::set_visible,
        "visible") +
    tl::make_member<bool, LayerPropertiesNode>(
        &LayerPropertiesNode::transparent_loc,
        &LayerPropertiesNode::set_transparent, "transparent") +
    tl::make_member<int, LayerPropertiesNode>(&LayerPropertiesNode::width_loc,
                                              &LayerPropertiesNode::set_width,
                                              "width", WidthConverter()) +
    tl::make_member<bool, LayerPropertiesNode>(&LayerPropertiesNode::marked_loc,
                                               &LayerPropertiesNode::set_marked,
                                               "marked") +
    tl::make_member<bool, LayerPropertiesNode>(&LayerPropertiesNode::xfill_loc,
                                               &LayerPropertiesNode::set_xfill,
                                               "xfill") +
    tl::make_member<int, LayerPropertiesNode>(
        &LayerPropertiesNode::animation_loc,
        &LayerPropertiesNode::set_animation, "animation") +
    tl::make_member<std::string, LayerPropertiesNode>(
        &LayerPropertiesNode::name, &LayerPropertiesNode::set_name, "name") +
    tl::make_member<std::string, LayerPropertiesNode>(
        &LayerPropertiesNode::source_string_loc,
        &LayerPropertiesNode::set_source, "source") +
    tl::make_element<LayerPropertiesNode, LayerPropertiesNode::const_iterator,
                     LayerPropertiesNode>(&LayerPropertiesNode::begin_children,
                                          &LayerPropertiesNode::end_children,
                                          &LayerPropertiesNode::add_child,
                                          "group-members", &layer_element));

static const tl::XMLElementList layer_prop_list = tl::XMLElementList(
    tl::make_element<LayerPropertiesNode, LayerPropertiesList::const_iterator,
                     LayerPropertiesList>(
        &LayerPropertiesList::begin_const, &LayerPropertiesList::end_const,
        &LayerPropertiesList::push_back, "properties", &layer_element) +
    tl::make_member(&LayerPropertiesList::name, &LayerPropertiesList::set_name,
                    "name") +
    tl::make_element(
        &LayerPropertiesList::begin_custom_dither_pattern,
        &LayerPropertiesList::end_custom_dither_pattern,
        &LayerPropertiesList::push_custom_dither_pattern,
        "custom-dither-pattern",
        tl::make_element(
            &lay::DitherPatternInfo::to_strings,
            &lay::DitherPatternInfo::from_strings, "pattern",
            tl::make_member<std::string,
                            std::vector<std::string>::const_iterator,
                            std::vector<std::string>>(
                &std::vector<std::string>::begin,
                &std::vector<std::string>::end,
                &std::vector<std::string>::push_back, "line")) +
            tl::make_member(&lay::DitherPatternInfo::order_index,
                            &lay::DitherPatternInfo::set_order_index, "order") +
            tl::make_member(&lay::DitherPatternInfo::name,
                            &lay::DitherPatternInfo::set_name, "name")) +
    tl::make_element(
        &LayerPropertiesList::begin_custom_line_styles,
        &LayerPropertiesList::end_custom_line_styles,
        &LayerPropertiesList::push_custom_line_style, "custom-line-style",
        tl::make_member(&lay::LineStyleInfo::to_string,
                        &lay::LineStyleInfo::from_string, "pattern") +
            tl::make_member(&lay::LineStyleInfo::order_index,
                            &lay::LineStyleInfo::set_order_index, "order") +
            tl::make_member(&lay::LineStyleInfo::name,
                            &lay::LineStyleInfo::set_name, "name")));
...
static const tl::XMLStruct<std::vector<LayerPropertiesList>>
    layer_prop_lists_structure(
        "layer-properties-tabs",
        tl::make_element<LayerPropertiesList,
                         std::vector<LayerPropertiesList>::const_iterator,
                         std::vector<LayerPropertiesList>>(
            &std::vector<LayerPropertiesList>::begin,
            &std::vector<LayerPropertiesList>::end,
            &std::vector<LayerPropertiesList>::push_back, "layer-properties",
            &layer_prop_list));

const tl::XMLElementList *LayerPropertiesList::xml_format() {
...
void LayerPropertiesList::load(
    tl::XMLSource &stream,
    std::vector<lay::LayerPropertiesList> &properties_lists) {
  try {
    // "old" way
    ...
    try {
      ...
      //  the first exception is likely to be the root cause, so let's rather
      //  throw this one
      throw ex;
    }
  }
...
void LayerPropertiesList::save(
    tl::OutputStream &os,
    const std::vector<lay::LayerPropertiesList> &properties_lists) {
  layer_prop_lists_structure.write(os, properties_lists);
}

void LayerPropertiesList::attach_view(lay::LayoutViewBase *view,
                                      unsigned int list_index) {
  mp_view.reset(view);
  m_list_index = list_index;

  //  HINT: this method has the side effect of recomputing the layer source
  //  parameters. The action must not be suppressed if view == mp_view
  for (layer_list::iterator c = m_layer_properties.begin();
       c != m_layer_properties.end(); ++c) {
    c->attach_view(view, list_index);
  }
}

lay::LayoutViewBase *LayerPropertiesList::view() const {
  return const_cast<lay::LayoutViewBase *>(mp_view.get());
}

unsigned int LayerPropertiesList::list_index() const { return m_list_index; }

void LayerPropertiesList::push_back(const LayerPropertiesNode &d) {
  m_layer_properties.push_back(d);
}

void LayerPropertiesList::set_dither_pattern(
...
LayerPropertiesNode &
LayerPropertiesList::insert(const LayerPropertiesIterator &iter,
  ...
  if (parent.is_null()) {
    if (iter.child_index() > m_layer_properties.size()) {
      throw tl::Exception(tl::to_string(
          tr("Iterator is out of range in LayerPropertiesList::insert")));
    }
    ret = &*(m_layer_properties.insert(
        m_layer_properties.begin() + iter.child_index(), node));
  } else {
    if (iter.child_index() >
    ...
    ret = &(parent->insert_child(parent->begin_children() + iter.child_index(),
                                 node));
  }
...
void LayerPropertiesList::erase(const LayerPropertiesIterator &iter) {
  ...
  if (pp.first == 0) {
    if (pp.second >= m_layer_properties.size()) {
      throw tl::Exception(tl::to_string(
          tr("Iterator is out of range in LayerPropertiesList::erase")));
    }
    m_layer_properties.erase(m_layer_properties.begin() + pp.second);
  } else {
    if (pp.second >=
        size_t(pp.first->end_children() - pp.first->begin_children())) {
      throw tl::Exception(tl::to_string(
          tr("Iterator is out of range in LayerPropertiesList::erase")));
    }
...
LayerPropertiesNodeRef::LayerPropertiesNodeRef(LayerPropertiesNode *node)
  ...
  if (node) {
    ...
    //  NOTE: we do assignment before we set the iterator reference - hence
    //  there won't be updates triggered.
    LayerPropertiesNode::operator=(*node);

    //  Makes ourself a perfect copy of the original (including reference into
    //  the view)
...
LayerPropertiesNodeRef::LayerPropertiesNodeRef() : m_synched_gen_id(0) {
  //  .. nothing yet ..
}

LayerPropertiesNodeRef::LayerPropertiesNodeRef(
    const LayerPropertiesNodeRef &other)
...
const lay::LayerPropertiesConstIterator &LayerPropertiesNodeRef::iter() const {
  return m_iter;
}
...
void LayerPropertiesNodeRef::refresh() const {
  ...
  LayerPropertiesNodeRef *non_const_this =
      const_cast<LayerPropertiesNodeRef *>(this);
