 ...
 */
class LAYBASIC_PUBLIC LayoutHandle : public tl::Object {
public:
  /**
   ...
   *
   *  If "force" is set to true, the layout will be given that name,
   * irregardless if the name already is being used. If "force" is false, a new
   ...
   *  @return 0, if there is no layout object with this name. Otherwise a
   * pointer to its handle
   */
   ...
   *
   *  Load the layout from the file given in the constructor using the given
   * layer map. The dirty flag is reset.
   *
   *  @param lmap The layer map specifying the layers to read
   *  @param technology The technology to use for layer map and other settings
   * or empty for "default technology"
   *  @return The new layer map (can differ from the input since layers may be
   * created)
   */
   ...
   *  If update is true, this method updates the cell view's filename, title,
   * save options and dirty flag.
   */
  void save_as(const std::string &filename,
               tl::OutputStream::OutputStreamMode om,
               const db::SaveLayoutOptions &options, bool update = true,
               int keep_backups = 0);

  /**
   *  @brief Sets the save options and a flag indicating whether they are valid
   *
   *  This method is mainly used by the session restore feature to restore the
   * handle's state.
   */
   ...
   */
  const db::SaveLayoutOptions &save_options() const { return m_save_options; }

  /**
   *  @brief Updates the given save options with attributes from this cell view
   *
   *  Some formats will initialize attributes from the cell view and the
   * layout's metadata (example: libname of GDS2). This method will update the
   * options if the layout provides attributes for initializing the latter.
   */
   ...
   */
  bool save_options_valid() const { return m_save_options_valid; }
   ...
   */
  const db::LoadLayoutOptions &load_options() const { return m_load_options; }
   ...
   *  This handler is attached to a layout changed event and will invalidate the
   * handle.
   */
  void layout_changed();
...
#if defined(HAVE_QT)
  /**
   *  @brief Gets the file system watcher that delivers events when one of the
   * layouts gets updated
   */
...
public:
  ...
  bool operator!=(const LayoutHandleRef &r) const { return !operator==(r); }
 ...
 *  If the context path is empty, the context cell is identical with the target
 * cell.
 */

class LAYBASIC_PUBLIC CellView : public tl::Object {
public:
   ...
   *  @brief Inequality
   */
  bool operator!=(const CellView &cv) const { return !operator==(cv); }
   ...
   */
  lay::LayoutHandle *operator->() const { return m_layout_href.get(); }

  /**
   *  @brief Return the layout handle (not via operator->)
   */
  lay::LayoutHandle *handle() const { return m_layout_href.get(); }
   ...
   */
  db::Cell *ctx_cell() const { return mp_ctx_cell; }

  /**
   *  @brief Get the context cell index
   */
  cell_index_type ctx_cell_index() const { return m_ctx_cell_index; }
  ...
  /**
   *  @brief Retrive the accumulated transformation induced by the context part
   * of the path
   */
  ...
  /**
   *  @brief Retrive the accumulated transformation induced by the context part
   * of the path as a micron-unit transformation
   */
   ...
   *  A layout must be set already. Rather the creating another reference to the
   * layout (which is done on operator= for example), this method copies the
   * content of the source layout to the current one and transfers cell path and
   * other parameters.
   *
   *  @param manager The database object manager that the new layout is put
   * under
   */
 ...
 *
 *  This object acts like a proxy to a lay::CellView object. It is connected to
 * a cellview and a LayoutView and upon changes, the LayoutView will be
 * configured accordingly.
 */
class LAYBASIC_PUBLIC CellViewRef : public gsi::ObjectBase {
public:
   ...
   *  @brief Inequality
   */
  bool operator!=(const CellViewRef &cv) const { return !operator==(cv); }
   ...
   *  @brief Return the layout handle (not via operator->)
   */
  lay::LayoutHandle *handle() const { return operator->(); }
