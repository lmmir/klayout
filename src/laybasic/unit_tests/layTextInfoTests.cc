...
TEST(1) {
  ...
  //  global transformation changes the dimension as the default font is not
  //  scaled or rotated
  EXPECT_EQ(ti.bbox(text, db::DCplxTrans(2.0)).to_string(), "(11,21;23,28.5)");
  EXPECT_EQ(ti.bbox(text, db::DCplxTrans(db::DFTrans(1))).to_string(),
            "(12,-6;27,18)");
  //  long text
  ...
  //  global trans only scales pixel-based border but does not modify the
  //  outline in "apply transformation" mode
  ...
  text3 = text2;
  text3.valign(db::VAlignCenter);
  EXPECT_EQ(ti.bbox(text3, db::DCplxTrans()).to_string(),
            "(12,-17.5;134,50.5)");
  text3.valign(db::VAlignTop);
  ...
  EXPECT_EQ(ti.bbox(text2, db::DCplxTrans()).to_string(),
            "(12,20.6;36.4,34.2)");
  ...
  EXPECT_EQ(ti.bbox(text3, db::DCplxTrans()).to_string(),
            "(12,20.6;36.4,34.2)");
  //  with apply_text_trans false, the global transformation does change the
  //  text bounding box.
  EXPECT_EQ(ti.bbox(text, db::DCplxTrans()).to_string(), "(12,20.6;24,27)");
