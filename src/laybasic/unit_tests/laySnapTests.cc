...
TEST(1) {
  ...
  db::Polygon poly;
  db::Point pts[] = {db::Point(0, 0), db::Point(1000, 0), db::Point(0, 1000)};
  ...
  res = lay::obj_snap(&view, db::DPoint(0.485, 0.505), db::DVector(0.01, 0.01),
                      0.1);
  EXPECT_EQ(res.object_snap, lay::PointSnapToObjectResult::ObjectEdge);
  EXPECT_EQ(res.snapped_point.to_string(), "0.49,0.51");
  EXPECT_EQ(res.object_ref.to_string(), "(0,1;1,0)");

  res = lay::obj_snap(&view, db::DPoint(0.205, 0.215), db::DVector(0.01, 0.025),
                      0.1);
  EXPECT_EQ(res.object_snap, lay::PointSnapToObjectResult::NoObject);
  ...
  res = lay::obj_snap(&view, db::DPoint(1.000, 0.505), db::DPoint(0.505, 0.500),
                      db::DVector(), lay::AC_Horizontal, 0.1);
  EXPECT_EQ(res.object_snap, lay::PointSnapToObjectResult::ObjectEdge);
  EXPECT_EQ(res.snapped_point.to_string(), "0.495,0.505");

  //  projected snapping
  res = lay::obj_snap(&view, db::DPoint(1.000, 0.505), db::DPoint(0.005, 1.005),
                      db::DVector(), lay::AC_Horizontal, 0.1);
  ...
  res2 =
      lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(), 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0.3525,0.6475;0,0.295)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(),
                        lay::AC_Horizontal, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(), "(0,0.5;0.5,0.5)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(0.03, 0.03),
                        lay::AC_Horizontal, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0,0.51;0.49,0.51)");
  ...
  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(),
                        lay::AC_Vertical, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0.205,0.795;0.205,0)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(),
                        lay::AC_Diagonal, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0.3525,0.6475;0,0.295)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.505), db::DVector(),
                        lay::AC_Ortho, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0,0.505;0.495,0.505)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.5), db::DVector(),
                        lay::AC_Any, 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0.3525,0.6475;0,0.295)");

  res2 = lay::obj_snap2(&view, db::DPoint(0.205, 0.495),
                        db::DVector(0.01, 0.01), 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(),
            "(0.355,0.645;0,0.29)");
  ...
  res2 =
      lay::obj_snap2(&view, db::DPoint(0.005, 0.5), db::DVector(), 0.005, 1.0);
  EXPECT_EQ(res2.any, true);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(), "(0,0.5;0.5,0.5)");
  ...
  res2 =
      lay::obj_snap2(&view, db::DPoint(-0.2, 0.5), db::DVector(), 0.005, 1.0);
  EXPECT_EQ(res2.any, false);
  EXPECT_EQ(db::DEdge(res2.first, res2.second).to_string(), "(0,0;0,0)");
}
