...
size_t size(const lay::LayerPropertiesList &list) {
  size_t n = 0;
  for (lay::LayerPropertiesConstIterator i = list.begin_const_recursive();
       !i.at_end(); ++i) {
    ++n;
  }
  return n;
...
bool compare_real(const lay::LayerPropertiesList &a,
                  const lay::LayerPropertiesList &b) {
  bool debug = true;
...
TEST(2a) {
  ...
  const char *ref = "<?xml version=\"1.0\"?>\n"
                    "<layer-properties>\n"
                    " <properties>\n"
                    "  <frame-color>#aabbcc</frame-color>\n"
                    " </properties>\n"
...
TEST(9) {
  ...
  nn = size(list);
  n = 0;
  for (lay::LayerPropertiesIterator iter = list.begin_recursive();
       !iter.at_end(); ++iter, ++n) {
...
void build_list(lay::LayerPropertiesConstIterator &iter,
                lay::LayerPropertiesList &list,
...
TEST(13) {
  ...
  EXPECT_EQ(flat.frame_brightness(false) == node.frame_brightness(false),
            false);
...
TEST(16) {
  ...
  tl::XMLStringSource s("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"
                        "<layer-properties>\n"
                        " <properties>\n"
