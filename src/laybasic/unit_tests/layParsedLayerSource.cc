...
TEST(1) {
  ...
  EXPECT_EQ(lay::ParsedLayerSource("4/0@*") == lay::ParsedLayerSource(4, 0, -1),
            true);
}
...
TEST(5) {
  ...
  EXPECT_EQ(ps3.to_string(),
            "*/*@2 [!(('X'==#2||('X'=='Y'&&'Y'=='X'))&&!('Z'!=##4))]");

  lay::ParsedLayerSource ps4("@2 "
                             "[X==#2||X==#3||X==#20||X==#120||X==#210||X==#5||"
                             "X==#15||X==#11||X==#17||X==#18]");
  EXPECT_EQ(ps4.to_string(),
            "*/*@2 "
            "['X'==#2||'X'==#3||'X'==#20||'X'==#120||'X'==#210||'X'==#5||'X'==#"
            "15||'X'==#11||'X'==#17||'X'==#18]");
  EXPECT_EQ(ps4.display_string(0),
            "@2 ['X'==#2||'X'==#3||'X'==#20||'X'==#120||...]");
  ...
  lay::ParsedLayerSource ps4c("@2 "
                              "[X==#3||X==#20||X==#120||X==#210||X==#5||X==#15|"
                              "|X==#11||X==#17||X==#18]");
  EXPECT_EQ(ps4c == ps4, false);
  EXPECT_EQ(ps4c < ps4, true);
  EXPECT_EQ(ps4 < ps4c, false);

  lay::ParsedLayerSource ps4d("@2 "
                              "[X==#2||X==#4||X==#20||X==#120||X==#210||X==#5||"
                              "X==#15||X==#11||X==#17||X==#18]");
  EXPECT_EQ(ps4d == ps4, false);
  EXPECT_EQ(ps4d < ps4, false);
  EXPECT_EQ(ps4 < ps4d, true);

  lay::ParsedLayerSource ps4e("@2 "
                              "[X==##2||X==#3||X==#20||X==#120||X==#210||X==#5|"
                              "|X==#15||X==#11||X==#17||X==#18]");
  ...
  lay::ParsedLayerSource ps4f("@2 "
                              "[X==#222||X==#3||X==#4||X==#20||X==#120||X==#"
                              "210||X==#5||X==#15||X==#11||X==#17||X==#18]");
  EXPECT_EQ(ps4f == ps4, false);
  EXPECT_EQ(ps4f < ps4, false);
  EXPECT_EQ(ps4 < ps4f, true);

  lay::ParsedLayerSource ps4g("@2 "
                              "[X!=#2||X==#3||X==#20||X==#120||X==#210||X==#5||"
                              "X==#15||X==#11||X==#17||X==#18]");
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(2l)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(3l)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(2l)));
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("Z")), tl::Variant(4.0)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(2l)));
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("Z")), tl::Variant(6l)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(2l)));
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("Z")), tl::Variant(5.0)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("Z")), tl::Variant(5l)));
  ...
  ps.insert(
      std::make_pair(rep.prop_name_id(tl::Variant("X")), tl::Variant(15l)));
...
TEST(7) {
  ...
  ps1 = ps2;
  ps1 += lay::ParsedLayerSource("(*2) (*3)");
  EXPECT_EQ(ps1.to_string(),
            "*/*@1 (r0 *4 0,0) (r0 *6 0,0) (r0 *3 0,0) (r0 *4.5 0,0)");
}
...
std::string tspath(const db::Layout &l, db::cell_index_type c,
                   lay::PartialTreeSelector &pt) {
  std::string r;
  if (pt.is_selected()) {
    r += "+";
  } else {
  ...
  for (db::Cell::child_cell_iterator cc = l.cell(c).begin_child_cells();
       !cc.at_end(); ++cc) {
...
TEST(11) {
  ...
  lay::PartialTreeSelector pt(
      selector_from_string("").create_tree_selector(layout, c1.cell_index()));
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "+C1(+C2(+C3(+C5+CC3)+C4(+CC4)+CC2)+CC1)");

  pt =
      selector_from_string("+C1").create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "+C1(+C2(+C3(+C5+CC3)+C4(+CC4)+CC2)+CC1)");
  ...
  pt =
      selector_from_string("-C2").create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt), "+C1(+CC1)");

  pt = selector_from_string("+C1 -C2").create_tree_selector(layout,
                                                            c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt), "+C1(+CC1)");

  pt = selector_from_string("+C1 ( -C* +CC* )")
           .create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt), "+C1(+CC1)");

  pt = selector_from_string("-C2 +C3").create_tree_selector(layout,
                                                            c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "+C1(-C2(+C3(+C5+CC3)-C4(-CC4)-CC2)+CC1)");

  pt = selector_from_string("-C2 +CC*")
           .create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "+C1(-C2(-C3(-C5+CC3)-C4(+CC4)+CC2)+CC1)");

  pt = selector_from_string("+CC*").create_tree_selector(layout,
                                                         c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "-C1(-C2(-C3(-C5+CC3)-C4(+CC4)+CC2)+CC1)");
  ...
  pt = selector_from_string("-* ( -* +CC* )")
           .create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt), "-C1(+CC1)");

  pt = selector_from_string("-C3 +CC*")
           .create_tree_selector(layout, c1.cell_index());
  EXPECT_EQ(tspath(layout, c1.cell_index(), pt),
            "+C1(+C2(-C3(-C5+CC3)+C4(+CC4)+CC2)+CC1)");
}
