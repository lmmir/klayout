...
template <class Sh> class shape_as_user_object : public db::DUserObjectBase {
public:
  shape_as_user_object<Sh>(const Sh &sh) : m_shape(sh) {}

  virtual bool equals(const db::DUserObjectBase *d) const {
    return m_shape ==
           (dynamic_cast<const shape_as_user_object<Sh> *>(d))->m_shape;
  }
  ...
  virtual void transform(const db::DFTrans &t) { m_shape.transform(t); }
...
template <class Sh> db::DUserObject us(const Sh &sh) {
...
std::string shapes_to_string(const lay::AnnotationShapes &shapes) {
  std::string r;
  for (lay::AnnotationShapes::iterator shape = shapes.begin();
       shape != shapes.end(); ++shape) {
    if (dynamic_cast<const shape_as_user_object<db::DPolygon> *>(
            shape->ptr())) {
      db::DPolygon p(
          dynamic_cast<const shape_as_user_object<db::DPolygon> *>(shape->ptr())
              ->shape());
      r += "polygon " + p.to_string();
    } else if (dynamic_cast<const shape_as_user_object<db::DPath> *>(
                   shape->ptr())) {
      db::DPath p(
          dynamic_cast<const shape_as_user_object<db::DPath> *>(shape->ptr())
              ->shape());
      r += "path " + p.to_string();
    } else if (dynamic_cast<const shape_as_user_object<db::DText> *>(
                   shape->ptr())) {
      db::DText p(
          dynamic_cast<const shape_as_user_object<db::DText> *>(shape->ptr())
              ->shape());
      r += "text " + p.to_string();
    } else if (dynamic_cast<const shape_as_user_object<db::DBox> *>(
                   shape->ptr())) {
      db::DBox p(
          dynamic_cast<const shape_as_user_object<db::DBox> *>(shape->ptr())
              ->shape());
      r += "box " + p.to_string();
    } else {
      r += "*unknown type*";
...
void read_testdata(lay::AnnotationShapes &shapes, unsigned int what = 0xff) {
  ...
  if ((what & 0x4) != 0) {
    ...
    static db::DPath r2;
    static db::DPath r3;
    db::DPoint pts1[] = {db::DPoint(0, 100), db::DPoint(0, 500),
                         db::DPoint(200, 700)};
    db::DPoint pts2[] = {db::DPoint(0, 1100), db::DPoint(0, 1500),
                         db::DPoint(200, 1300)};
    db::DPoint pts3[] = {db::DPoint(0, 2100), db::DPoint(0, 2500),
                         db::DPoint(-200, 2700)};
...
TEST(6) {
  ...
  EXPECT_EQ(shapes_to_string(shapes), "box (0,100;2000,1000)\n"
  ...
  copy.clear();
  for (lay::AnnotationShapes::touching_iterator shape =
           shapes.begin_touching(db::DBox(0, 100, 100, 200));
       !shape.at_end(); ++shape) {
    copy.insert(*shape);
  }

  EXPECT_EQ(shapes_to_string(copy), "box (0,100;2000,1000)\n"
                                    "box (100,200;2100,1100)\n");
}
...
TEST(7) {
  ...
  copy.clear();
  for (lay::AnnotationShapes::overlapping_iterator shape =
           shapes.begin_overlapping(db::DBox(0, 100, 100, 200));
       !shape.at_end(); ++shape) {
    copy.insert(*shape);
  }
