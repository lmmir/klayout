...
static void fail(const char *file, int line) {
  throw tl::ScriptError(
      tl::to_string(tr("Python support not compiled in")).c_str(), file, line,
      "missing_feature", std::vector<tl::BacktraceElement>());
}

static PythonInterpreter *sp_pya_interpreter = 0;

PythonInterpreter::PythonInterpreter() : gsi::Interpreter(0, "pya") {
...
PythonInterpreter *PythonInterpreter::instance() { return sp_pya_interpreter; }

void PythonInterpreter::add_path(const std::string &) {
  // .. nothing ..
}
...
void PythonInterpreter::eval_string(const char *, const char *file, int line,
                                    int) {
  fail(file, line);
}

void PythonInterpreter::eval_string_and_print(const char *, const char *file,
                                              int line, int) {
  fail(file, line);
}

gsi::Inspector *PythonInterpreter::inspector(int) { return 0; }

void PythonInterpreter::define_variable(const std::string &,
                                        const tl::Variant &) {
  // .. nothing ..
}

bool PythonInterpreter::available() const { return false; }
...
std::string PythonInterpreter::version() const { return std::string(); }

tl::Variant PythonInterpreter::eval_expr(const char *, const char *file,
                                         int line, int) {
  fail(file, line);
  return tl::Variant();
}
