 ...
 */
class DB_PUBLIC Technologies : public tl::Object {
public:
   ...
   */
  const_iterator begin() const { return m_technologies.begin(); }

  /**
   *  @brief Const iterator - end
   */
  const_iterator end() const { return m_technologies.end(); }

  /**
   *  @brief iterator - begin
   */
  iterator begin() { return m_technologies.begin(); }

  /**
   *  @brief Const iterator - end
   */
  iterator end() { return m_technologies.end(); }

  /**
   *  @brief The number of technologies
   */
  size_t technologies() const { return m_technologies.size(); }
   ...
   */
  void add(Technology *technology) { add_tech(technology, true /*replace*/); }
   ...
   *  This method will disable "technologies_changed" events until (later)
   * end_updates () is called.
   */
  void begin_updates();
  ...
  /**
   *  @brief Ends a bulk operation
   *  This version does not send an technologies_changed event but just cancels
   * the bulk operation. begin_updates/end_updates_no_event is essentially
   * equivalent to blocking signals.
   */
   ...
   *  the system directly. Only bulk changes (such as adding or removing
   * technologies are). To inform the system of individual technology updates,
   * call this method after a technology or multiple technologies have been
   * changed.
   */
   ...
   *
   *  If no technology with that name exists, the default technology is
   * returned.
   */
   ...
   *  If a technology is modified, this event is triggered with that technology
 ...
 */
class DB_PUBLIC Technology : public tl::Object {
public:
  /**
   ...
   */
  void set_grain_name(const std::string &g) { m_grain_name = g; }

  /**
   *  @brief Gets the package source
   */
  const std::string &grain_name() const { return m_grain_name; }

  /**
   *  @brief Gets the base path
   *
   *  The base path is an effective path - if the explicit path is set, it is
   *  used. If not, the default path is used. The default path is the one from
   * which a technology file was imported. The explicit one is the one that is
   ...
   *
   *  Only files below the base path will be made relative. Files above or
   * beside won't be made relative.
   */
   ...
   */
  const std::string &default_base_path() const { return m_default_base_path; }
   ...
   */
  const std::string &explicit_base_path() const { return m_explicit_base_path; }
  ...
  /**
   *  @brief Gets the path of the tech file if the technology was loaded from a
   * tech file
   */
  const std::string &tech_file_path() const { return m_lyt_file; }
   ...
   */
  const std::string &layer_properties_file() const { return m_lyp_path; }

  /**
   *  @brief Gets the effective layer properties file path (empty if none is
   * specified)
   *
  ...
  /**
   *  @brief Sets the layer properties file path (set to empty string to remove
  ...
  /**
   *  @brief Gets the flag indicating whether to add other layers to the layer
   * properties
   */
  bool add_other_layers() const { return m_add_other_layers; }

  /**
   *  @brief Sets the flag indicating whether to add other layers to the layer
   * properties
   *
   *  If "add_other_layers" is true, the layers in the layout but not specified
   * in the layer properties file will be added automatically.
   */
  ...
  /**
   *  @brief Delivers the XMLElementList that specifies the technology's XML
   * representation
   */
  ...
  component_by_name(const std::string &component_name) const;

  /**
   *  @brief Gets the technology component by the component name (non-const
   * version)
   *
   *  If no component with that name exists, 0 is returned.
   */
  ...
  /**
   *  @brief Builds the effective path from a relative or absolute one using the
   * base path if necessary
   */
  std::string build_effective_path(const std::string &p) const;

  /**
   *  @brief Returns a flag indicating whether the technology is persisted or
   * not
   *
   *  If the flag is false, this technology is not included into the XML string
   *  of the technologies.
   */
  bool is_persisted() const { return m_persisted; }

  /**
   *  @brief Sets a flag indicating whether the technology is persisted
   */
  void set_persisted(bool f) { m_persisted = f; }

  /**
   *  @brief Returns a flag indicating whether the technology is readonly
   *
   *  If the flag is false, the technology can be edited. Otherwise it's locked
   * for editing.
   */
  ...
  /**
   *  @brief An event indicating that the technology has changed (with a sender
   * argument)
   */
...
public:
  ...
  /**
   *  @brief Delivers the XMLElement object that represents this component
...
/**
 *  @brief A helper class for the XML serialization of the technology component
 * (custom read adaptor)
 */

template <class TC> class TechnologyComponentReadAdaptor {
public:
  ...
  const TC &operator()() const {
    const TC *tc = dynamic_cast<const TC *>(
        (const_cast<db::Technology *>(mp_t))->component_by_name(m_name));
    if (!tc) {
      throw tl::Exception(tl::to_string(tr("Unknown technology component: ")) +
                          m_name);
    }
...
/**
 *  @brief A helper class for the XML serialization of the technology component
 * (custom write adaptor)
 */

template <class TC> class TechnologyComponentWriteAdaptor {
public:
  TechnologyComponentWriteAdaptor(const std::string &name) : m_name(name) {
    // .. nothing yet ..
  }
  ...
  void operator()(db::Technology &t, tl::XMLReaderState &reader) const {
    ...
    if (!tc_basic) {
      ...
      if (!tc) {
        throw tl::Exception(
            tl::to_string(tr("Invalid technology component: ")) + m_name);
      }
    }
...
class TechnologyComponentXMLElement
    : public tl::XMLElement<TC, db::Technology,
                            TechnologyComponentReadAdaptor<TC>,
                            TechnologyComponentWriteAdaptor<TC>> {
public:
  TechnologyComponentXMLElement(const std::string &name,
                                const tl::XMLElementList &children)
      : tl::XMLElement<TC, db::Technology, TechnologyComponentReadAdaptor<TC>,
                       TechnologyComponentWriteAdaptor<TC>>(
            TechnologyComponentReadAdaptor<TC>(name),
            TechnologyComponentWriteAdaptor<TC>(name), name, children) {
    //  .. nothing yet ..
  }

  TechnologyComponentXMLElement(const TechnologyComponentXMLElement &d)
      : tl::XMLElement<TC, db::Technology, TechnologyComponentReadAdaptor<TC>,
                       TechnologyComponentWriteAdaptor<TC>>(d) {
    //  .. nothing yet ..
  }
