...
class DB_PUBLIC Cell : public db::Object, public gsi::ObjectBase {
public:
   ...
   *
   *  If the shapes container is not part of the cell,
   ...
   *
   *  The target layer is not overwritten. Instead, the shapes are added to the
   * target layer's shapes.
   */
  void copy(unsigned int src, unsigned int dest);

  /**
   *  @brief Copy the shapes from layer src to dest (only shapes from given
   * classes)
   *
   *  The target layer is not overwritten. Instead, the shapes are added to the
   * target layer's shapes.
  ...
  /**
   *  @brief Move the shapes from layer src to dest (only shapes from given
   * classes)
   *
   *  The target layer is not overwritten. Instead, the shapes are added to the
   * target layer's shapes.
  ...
  /**
   *  @brief Clear the shapes on the given layer (only the shapes from the given
   * classes)
   */
   ...
   */
  void erase(const instance_type &ref) { m_instances.erase(ref); }
   ...
   */
  void erase(const_iterator e) { m_instances.erase(e); }
   ...
   *  explicitly before the bounding box can be updated.
   */
  template <class Inst> Instance insert(const Inst &inst) {
   ...
   */
  template <class I> void insert(I from, I to) { m_instances.insert(from, to); }
  ...
  /**
   *  @brief Transform the instance pointed to by the instance reference into a
   ...
   *
   *  The transformation is applied to all instances and shapes. Magnified
   * transformations will render magnified instances. See \transform_into for a
  ...
  template <class Trans> void transform(const Trans &t) {
    ...
    for (typename shapes_map::iterator s = m_shapes_map.begin();
         s != m_shapes_map.end(); ++s) {
      if (!s->second.empty()) {
        //  Note: don't use the copy ctor here - it will copy the attachment to
        //  the manager and create problems when destroyed. Plus: swap would be
        //  more efficient. But by using assign_transformed we get undo support
        //  for free.
        shapes_type d;
        d = s->second;
   ...
   *
   *  The transformation is not applied to the children, but this method enables
   * propagation because the transformation is only applied to the instances
   * insofar that it can be applied to the children as well.
   *
   *  @param t The transformation to apply
   */
   ...
   * instance
   *
   *  This method will delete the former object if it is not of the correct kind
   * - i.e. it will delete an object without a property and insert a new one
   * with a property. This may invalidate other references in non-editable mode.
   *  If the inserted instance is one without a property and the existing
   * instance has a property, the latter is not touched.
   */
   ...
   */
  tl::Variant get_pcell_parameter(const instance_type &ref,
                                  const std::string &name) const;

  /**
   *  @brief Gets the PCell parameters of a PCell instance as a name to value
   * map
   *
   *  @return The PCell parameter values by name
   */
  std::map<std::string, tl::Variant>
  get_named_pcell_parameters(const instance_type &ref) const;

  /**
   *  @brief Changes the PCell parameters of a PCell instance
   *
   *  For the order of the parameters, ask the PCell declaration (available
   * trough Layout::pcell_declaration from the PCell id. This method is only
   * allowed in editable mode.
   *
   *  @return A reference to the new instance. The original reference may be
   * invalid.
   */
  instance_type
  change_pcell_parameters(const instance_type &ref,
   ...
   */
  const cell_index_type &cell_index() const { return m_cell_index; }
   ...
   */
  size_t cell_instances() const { return m_instances.cell_instances(); }
   ...
   *
   *  The iterator returned will point to the i-th element, where i is the index
   * in the list of instances sorted by cell index. The iterator follows the
   * "at_end" semantics.
   */
  instance_type sorted_inst_ptr(size_t i) const {
    return m_instances.instance_from_pointer(
        m_instances.begin_sorted_insts()[i]);
  }

  /**
   *  @brief The cell instance access by index in list of sorted instances
   *
   *  The pointer returned will point to the i-th element, where i is the index
   * in the list of instances sorted by cell index. This is just a
   * "basic_inst_type" pointer without any further hint about what object is
   ...
   */
  shape_iterator begin(unsigned int layer, unsigned int flags,
   ...
   */
  shape_iterator
  begin_overlapping(unsigned int layer, const box_type &box, unsigned int flags,
                    const shape_iterator::property_selector *prop_sel = 0,
                    bool inv_prop_sel = false) const {
   ...
   *
   *  This method ensures that the box tress are updated before the query is
   * done. The db::Shape-specific iterator does not do that.
   *
   *  @param box The region to query the shapes from
   *  @param layer The layer from which to query the shapes
   *  @param flags The flags of shapes to query (see db::shape documentation)
   ...
   */
  shape_iterator
  begin_touching(unsigned int layer, const box_type &box, unsigned int flags,
                 const shape_iterator::property_selector *prop_sel = 0,
                 bool inv_prop_sel = false) const {
  ...
  /**
   *  @brief A quick, recursive test whether the cell has shapes touching the
   * given box on the given layer
   */
  bool has_shapes_touching(unsigned int layer, const db::Box &box) const;

  /**
   *  @brief Collect all calling cells (either calling this cell directly or
   * indirectly)
   *
   *  This method adds all cell indices of all cells that either directly or
   * indirectly call this cell to the set given. It is assumed that the if the
   * set contains a cell, it will also contain all called cells, so it may act
   * as a cache.
   *
  ...
  /**
   *  @brief Collect all calling cells (either calling this cell directly or
   * indirectly) up to a certain level of hierarchy
   *
   *  This method adds all cell indices of all cells that either directly or
   * indirectly call this cell to the set given. It is assumed that the if the
   * set contains a cell, it will also contain all called cells, so it may act
   ...
   * ...)
   */
  void collect_caller_cells(std::set<cell_index_type> &callers,
                            int levels) const;
   ...
   * as a cache.
   *
   *  This version allows one to restrict the search to a cone of the hierarchy
   * tree, that is a set of cells which are collected from another call of
   * "collect_called_cells" with another initial cell.
   *
   *  @param callers The set of called cells (used as cache)
   ...
   */
  void collect_caller_cells(std::set<cell_index_type> &callers,
                            const std::set<cell_index_type> &cone,
                            int levels) const;

  /**
   *  @brief Collect all cells called either directly or indirectly by this cell
   *
   *  This method adds all cell indices of all cells called either directly or
   * indirectly by this cell to the set given. It is assumed that the if the set
   * contains a cell, it will also contain all called cells, so it may act as a
   * cache.
   *
  ...
  /**
   *  @brief Collect all cells called either directly or indirectly by this cell
   * down to a certain level of hierarchy
   *
   *  This method adds all cell indices of all cells called either directly or
   * indirectly by this cell to the set given. It is assumed that the if the set
   * contains a cell, it will also contain all called cells, so it may act as a
   ...
   *
   *  This method does not have any effect except for derived classes where it
   * updates the layout with the current state, i.e. if a PCell declaration has
   * changed.
   */
   ...
   *
   *  If a cell with that name already exists, a new name is created by
   * appending a suffix.
   */
   ...
   *  the cell is not located inside the file. Upon writing the reader can
   * determine whether to write the cell or not. To satisfy the references
   * inside the layout, a dummy cell is created in this case which has the
   * "ghost cell" flag set to true.
   */
  bool is_ghost_cell() const { return m_ghost_cell; }
   ...
   */
  void set_ghost_cell(bool g) { m_ghost_cell = g; }
  ...
  /**
   *  @brief Invalidate the instances bounding box (to be called by the
   * instances container)
   */
  void invalidate_insts();

  /**
   *  @brief Invalidate the hierarchical structure (to be called by the
   * instances container)
   */
   ...
   */
  db::Layout *layout() { return mp_layout; }
   ...
   *
   *  If the source and target layout are different ones, the whole cell tree of
   * the source cell will be copied. This will create new cells in the target
   * layout to accommodate the source cell tree. Returns an array with the
   * freshly created cells which accommodate the cell tree.
   */
  ...
  /**
   *  @brief Copies all shapes from the source cell to this cell using the given
   * layer mapping
   */
  void copy_shapes(const db::Cell &source_cell,
  ...
  /**
   *  @brief Moves all shapes from the source cell to this cell using the given
   * layer mapping
   */
  void move_shapes(db::Cell &source_cell,
                   const db::LayerMapping &layer_mapping);
...
private:
   ...
   */
  void set_cell_index(db::cell_index_type ci) { m_cell_index = ci; }

  /**
   *  @brief Return a reference to the instances object
   */
  instances_type &instances() { return m_instances; }

  /**
   *  @brief Return a reference to the instances object
   */
  const instances_type &instances() const { return m_instances; }
   ...
   *
   *  The child instances are first sorted by cell index, then by raw
   * transformation (excluding displacement). This allows one to simplify the
   * bbox computation by convolution of the displacements bboxes with the object
   * bboxes.
   */
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const db::Cell &x, bool no_self = false,
                     void *parent = 0) {
