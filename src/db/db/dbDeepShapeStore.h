...
public:
   ...
   *  This requires the Region to be a DeepRegion. Otherwise, this constructor
   * will assert
   */
   ...
   *  This requires the texts to be a DeepTexts. Otherwise, this constructor
   * will assert
   */
   ...
   *  This requires the edges to be a DeepEdges. Otherwise, this constructor
   * will assert
   */
   ...
   *  This requires the edge pairs to be a DeepEdgePairs. Otherwise, this
   ...
   */
  unsigned int layout_index() const { return m_layout; }

  /**
   *  @brief Gets the list of breakout cells if there are some
   *  "breakout cells" are cells which are not considered to participate in
   * hierarchical operations, neither as sibling nor in parent-child
   * relationships.
   */
  const std::set<db::cell_index_type> *breakout_cells() const;

  /**
   *  @brief Inserts the layer into the given layout, starting from the given
   * cell and into the given layer
   */
  void insert_into(Layout *into_layout, db::cell_index_type into_cell,
                   unsigned int into_layer) const;

  /**
   *  @brief Inserts the edge pairs layer into the given layout, starting from
   * the given cell and into the given layer The edge pairs are converted to
   * polygons with the given enlargement.
   */
  void insert_into_as_polygons(db::Layout *into_layout,
                               db::cell_index_type into_cell,
                               unsigned int into_layer, db::Coord enl) const;
   ...
   */
  template <class VarCollector> void separate_variants(VarCollector &collector);
  ...
  void
  commit_shapes(VarCollector &collector,
                std::map<db::cell_index_type,
                         std::map<db::ICplxTrans, db::Shapes>> &to_propagate);
...
public:
  ...
  breakout_cells(unsigned int layout_index) const;
  void clear_breakout_cells(unsigned int layout_index);
  void set_breakout_cells(unsigned int layout_index,
                          const std::set<db::cell_index_type> &boc);
  void add_breakout_cell(unsigned int layout_index, db::cell_index_type ci);
  void add_breakout_cells(unsigned int layout_index,
                          const std::set<db::cell_index_type> &cc);
...
private:
  ...
  ensure_breakout_cells(unsigned int layout_index) {
    if (m_breakout_cells.size() <= size_t(layout_index)) {
      m_breakout_cells.resize(layout_index + 1,
                              std::set<db::cell_index_type>());
    }
...
struct DB_PUBLIC RecursiveShapeIteratorCompareForTargetHierarchy {
  bool operator()(const std::pair<db::RecursiveShapeIterator,
                                  std::pair<size_t, db::ICplxTrans>> &a,
                  const std::pair<db::RecursiveShapeIterator,
                                  std::pair<size_t, db::ICplxTrans>> &b) const {
    int cmp_iter = db::compare_iterators_with_respect_to_target_hierarchy(
        a.first, b.first);
    if (cmp_iter != 0) {
      return cmp_iter < 0;
    }
...
/**
 *  @brief An object holding a cell mapping with the hierarchy generation Ids of
 * the involved layouts
 */
class DB_PUBLIC CellMappingWithGenerationIds : public db::CellMapping {
public:
  ...
  bool is_valid(const db::Layout &into_layout,
                const db::Layout &from_layout) const {
    return into_layout.hier_generation_id() == m_into_generation_id &&
           from_layout.hier_generation_id() == m_from_generation_id;
  }

  void set_generation_ids(const db::Layout &into_layout,
                          const db::Layout &from_layout) {
...
/**
 *  @brief The "deep shape store" is a working model for the hierarchical
 * ("deep") processor
 *
 ...
 *
 *  The deep shape store provides the basis for working with deep regions. On
 * preparation, shapes are copied into the deep shape store. After finishing,
 * the shapes are copied back into the original layout. The deep shape store
 * provides the methods and algorithms for doing the preparation and transfer.
 */
class DB_PUBLIC DeepShapeStore : public tl::Object, public gsi::ObjectBase {
public:
  /**
   ...
   *
   *  If this value is set to true, layouts are not released when their
   * reference count goes down to zero.
   */
  void set_keep_layouts(bool f) { m_keep_layouts = f; }

  /**
   *  @brief Gets a value indicating whether to keep layouts
   */
  bool keep_layouts() const { return m_keep_layouts; }
   ...
   */
  DeepLayer create_from_flat(const db::Region &region, bool for_netlist,
                             double max_area_ratio = 0.0,
                             size_t max_vertex_count = 0,
                             const db::ICplxTrans &trans = db::ICplxTrans());

  /**
   *  @brief Creates a new layer from a flat edge collection (or the edge
   * collection is made flat)
   *
   *  This method is intended for use with singular-created DSS objects (see
   *  singular constructor).
   *
   ...
   *  from the region later with layer_for_flat (region).
   */
  DeepLayer create_from_flat(const db::Edges &edges,
                             const db::ICplxTrans &trans = db::ICplxTrans());

  /**
   *  @brief Creates a new layer from a flat text collection (or the text
   * collection is made flat)
   *
   *  This method is intended for use with singular-created DSS objects (see
   *  singular constructor).
   ...
   *  from the region later with layer_for_flat (region).
   */
  DeepLayer create_from_flat(const db::Texts &texts,
                             const db::ICplxTrans &trans = db::ICplxTrans());

  /**
   *  @brief Gets the layer for a given flat collection (Region, Edges, Texts,
   * EdgePairs)
   *
   *  If a layer has been created for a flat region with create_from_flat, it
   * can be retrieved with this method. The first return value is true in this
   * case.
   */
  ...
  /**
   *  @brief Creates a layout with the given iterator and transformation for the
   * given index
   *
   *  This method is intended for classes that need more control over the
   * layouts per index (LayoutToNetlist).
   */
  void make_layout(unsigned int layout_index,
   ...
   */
  DeepLayer create_polygon_layer(const db::RecursiveShapeIterator &si,
                                 double max_area_ratio = 0.0,
                                 size_t max_vertex_count = 0,
   ...
   *  for edges.
   *
   *  If "as_edges" is true, polygons and boxes will be converted into edges.
   * Otherwise only edge objects are taken from the shape iterator. Note that
   * the shape iterator must be configured to deliver all shape types if
   * "as_edges" is true.
   */
   ...
   *  for texts.
   */
  DeepLayer create_text_layer(const db::RecursiveShapeIterator &si,
                              const ICplxTrans &trans = db::ICplxTrans());

  /**
   *  @brief Inserts a polygon layer into the deep shape store using a custom
   * preparation pipeline
   *
   *  This method will create a new layer inside the deep shapes store and
   *  feed it through the given pipeline. The pipeline may perform shape
   * translations and finally will feed the target hierarchy.
   */
  DeepLayer create_custom_layer(const db::RecursiveShapeIterator &si,
   ...
   */
  DeepLayer create_copy(const DeepLayer &source,
                        HierarchyBuilderShapeReceiver *pipe);

  /**
   *  @brief Inserts the deep layer's shapes into some target layout
   */
  void insert(const DeepLayer &layer, db::Layout *into_layout,
              db::cell_index_type into_cell, unsigned int into_layer);
   ...
   */
  void insert_as_polygons(const DeepLayer &deep_layer, db::Layout *into_layout,
                          db::cell_index_type into_cell,
                          unsigned int into_layer, db::Coord enl);

  /**
   *  @brief Gets the cell mapping suitable to returning a layout from the deep
   * shape store into the original layout hierarchy
   *
   *  If necessary, this method will modify the original layout and add new
   * cells. "layout_index" is the layout to return to it's original.
   * "into_layout" is the original layout, "into_cell" the original cell.
   *
   *  "excluded_cells" - if not 0 - will exclude the given cells
   *
   *  "included_cells" - if not 0 - will only include the given cells in the
   * cell mapping
   */
  const db::CellMapping &cell_mapping_to_original(
      unsigned int layout_index, db::Layout *into_layout,
      db::cell_index_type into_cell,
      const std::set<db::cell_index_type> *excluded_cells = 0,
      const std::set<db::cell_index_type> *included_cells = 0);

  /**
   *  @brief Gets the cell mapping from one internal layout to another
   */
  const db::CellMapping &internal_cell_mapping(unsigned int from_layout_index,
                                               unsigned int into_layout_index);

  /**
   *  @brief Create cell variants from the given variant collector
   *
   *  To use this method, first create a variant collector
   * (db::cell_variant_collector) with the required reducer and collect the
   * variants. Then call this method on the desired layout index to create the
   * variants.
   */
  template <class VarCollector>
  void separate_variants(unsigned int layout_index, VarCollector &coll) {
    ...
    std::map<db::cell_index_type, std::map<db::ICplxTrans, db::cell_index_type>>
        var_map;
    coll.separate_variants(layout(layout_index), initial_cell(layout_index),
                           &var_map);
    if (var_map.empty()) {
      //  nothing to do.
      return;
   ...
   *  To use this method, first create a variant collector
   * (db::cell_variant_collector) with the required reducer and collect the
   * variants. Then call this method on the desired layout index to commit the
   * shapes for the respective variants.
   */
  template <class VarCollector>
  void commit_shapes(
      unsigned int layout_index, VarCollector &coll, unsigned int layer,
      std::map<db::cell_index_type, std::map<db::ICplxTrans, db::Shapes>>
          &to_commit) {
    tl_assert(is_valid_layout_index(layout_index));

    coll.commit_shapes(layout(layout_index), initial_cell(layout_index), layer,
                       to_commit);
  }
   ...
   *
   *  Don't try to mess too much with the layout object, you'll screw up the
   * internals.
   */
   ...
   *
   *  Don't try to mess too much with the cell object, you'll screw up the
   * internals.
   */
   ...
   *
   *  This method will throw an exception if the deep shape store is not
   * singular.
   */
   ...
   *
   *  This method will throw an exception if the deep shape store is not
   * singular. Don't try to mess too much with the layout object, you'll screw
   * up the internals.
   */
  db::Layout &layout() {
    require_singular();
   ...
   *
   *  This method will throw an exception if the deep shape store is not
   * singular. Don't try to mess too much with the cell object, you'll screw up
   * the internals.
   */
   ...
   */
  unsigned int layouts() const { return (unsigned int)m_layouts.size(); }

  /**
   *  @brief Gets a value indicating whether the given index is a valid layout
   * index
   */
  bool is_valid_layout_index(unsigned int n) const;

  /**
   *  @brief Gets the net builder object for a given layout index and
   * LayoutToNetlist database
   *
   *  If no net builder is available, one will be created. Use \\has_net_builder
   * to check whether one is already created.
   */
  db::NetBuilder &net_builder_for(unsigned int layout_index,
                                  db::LayoutToNetlist *l2n);

  /**
   *  @brief Gets the net builder object for a given LayoutToNetlist database
   * (requires the DSS to be singular)
   */
  ...
  /**
   *  @brief The deep shape store also keeps the number of threads to allocate
   * for the hierarchical processor
   *
  ...
  /**
   *  @brief Sets a flag indicating whether the working layouts will store the
   * whole original hierarchy
   *
  ...
  /**
   *  @brief Gets a flag indicating whether the working layouts will store the
   * whole original hierarchy
   */
   ...
   *
   *  Some kind of "odd" (e.g. non-orientable) polygons may spoil the
   * functionality because they cannot be handled properly. By using this flag,
   * the shape store we reject these kind of polygons. The default is "accept"
   * (without warning).
   */
  void set_reject_odd_polygons(bool f);
   ...
   *
   *  If set to a non-null variant, text strings are attached to the generated
   * boxes as properties with this particular name. This option has an effect
   * only if the text_enlargement property is not negative. By default, the name
   * is empty.
   */
  void set_text_property_name(const tl::Variant &pn);
   ...
   *
   *  If set to a non-negative value, text objects are converted to boxes with
   * the given enlargement (width = 2 * enlargement). The box centers are
   * identical to the original location of the text. If this value is negative
   * (the default), texts are ignored.
   */
  void set_text_enlargement(int enl);
  ...
  /**
   *  @brief Sets a value indicating whether to build a subcircuit hierarchy per
   * net
   *
   *  This flag is used to determine the way, net subcircuit hierarchies are
   * built: when true, subcells are created for subcircuits on a net. Otherwise
   * the net shapes are produced flat inside the cell they appear on.
   */
  void set_subcircuit_hierarchy_for_nets(bool f);

  /**
   *  @brief Gets a value indicating whether to build a subcircuit hierarchy per
   * net
   */
  bool subcircuit_hierarchy_for_nets() const;
   ...
   *  The state involves threads, max_area_ratio, max_vertex_count, the breakout
   * cells and the text representation properties (enlargement, property name).
   */
  void push_state();
...
private:
  ...
  typedef std::map<
      std::pair<db::RecursiveShapeIterator, std::pair<size_t, db::ICplxTrans>>,
      unsigned int, RecursiveShapeIteratorCompareForTargetHierarchy>
      layout_map_type;
  ...
  struct DeliveryMappingCacheKey {
    //  NOTE: we shouldn't keep pointers here as the layouts may get deleted and
    //  recreated with the same address. But as we don't access these objects
    //  that's fairly safe.
    DeliveryMappingCacheKey(unsigned int _from_index, tl::id_type _into_layout,
                            db::cell_index_type _into_cell)
        : from_index(_from_index), into_layout(_into_layout),
          into_cell(_into_cell) {
      //  .. nothing yet ..
    }
  ...
  std::map<DeliveryMappingCacheKey, CellMappingWithGenerationIds>
      m_delivery_mapping_cache;
  std::map<std::pair<unsigned int, unsigned int>, CellMappingWithGenerationIds>
      m_internal_mapping_cache;
};
