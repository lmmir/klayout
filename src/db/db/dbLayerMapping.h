 ...
 *
 *  The layer mapping represents corresponding layers between two layouts, layer
 * A and B. Layer mappings can be generated from the layer properties (layer,
 * datatype, name) or a custom layer mapping can be created using the "map"
 * method. The purpose of the layer mapping is to find a layer in layout A for a
 * corresponding layer in Layout B.
 *
...
public:
  ...
  /**
   *  @brief Create a mapping for layout_b to layout_a employing the layer
   * properties
   *
  ...
  /**
   *  @brief Create a full mapping for layout_b to layout_a employing the layer
   * properties
   *
   *  A full mapping means that all layers of layout_b are mapped. Layers
   * missing in layout A are created.
   *
   *  Layers with null properties (temporary layers) are not mapped and not
   * created.
   *
   *  @return A list of newly created layers.
   */
  std::vector<unsigned int> create_full(db::Layout &layout_a,
                                        const db::Layout &layout_b);

  /**
   *  @brief Determine layer mapping to a layout_b layer to the corresponding
   * layout_a layer.
   *
   *  @param layer_b The index of the layer in layout_b whose mapping is
   * requested.
   *  @return First: true, if a unique mapping is given, Second: the layer index
   * in layout_a.
   */
   ...
   *
   *  @param layer_b The index of the layer in layout_a (the source of the
   * mapping)
   *  @param layer_a The index of the layer in layout_a (the target of the
   * mapping)
   */
