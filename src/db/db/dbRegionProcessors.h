...
public:
  virtual void make_point(const db::Point &pt, const db::Edge &e1,
                          const db::Edge &e2) const = 0;
};

/**
 *  @brief An interface to accept corners and turns them into rectangles with
 * 2*dim x 2*dim
 */
class DB_PUBLIC CornerRectDelivery : public CornerPointDelivery {
public:
  CornerRectDelivery(db::Coord dim, std::vector<db::Polygon> &result)
      : m_d(dim, dim), mp_result(&result) {}

  virtual void make_point(const db::Point &pt, const db::Edge &,
                          const db::Edge &) const {
...
/**
 *  @brief An interface to accept corners and turns them into degenerated edges
 * (dots)
 */
class DB_PUBLIC CornerDotDelivery : public CornerPointDelivery {
public:
  CornerDotDelivery(std::vector<db::Edge> &result) : mp_result(&result) {}
 ...
 */
class DB_PUBLIC CornerEdgePairDelivery : public CornerPointDelivery {
public:
  CornerEdgePairDelivery(std::vector<db::EdgePair> &result)
      : mp_result(&result) {}

  virtual void make_point(const db::Point &, const db::Edge &e1,
                          const db::Edge &e2) const {
...
public:
  CornerDetectorCore(double angle_start, bool include_angle_start,
                     double angle_end, bool include_angle_end);
  virtual ~CornerDetectorCore() {}

  void detect_corners(const db::Polygon &poly,
                      const CornerPointDelivery &delivery) const;
...
/**
 *  @brief A corner detector delivering small retangles (2*dim x 2*dim) per
 * detected corner
 */
class DB_PUBLIC CornersAsRectangles : public db::PolygonProcessorBase,
                                      private CornerDetectorCore {
public:
  CornersAsRectangles(double angle_start, bool include_angle_start,
                      double angle_end, bool include_angle_end,
                      db::Coord dim = 1)
      : CornerDetectorCore(angle_start, include_angle_start, angle_end,
                           include_angle_end),
        m_dim(dim) {
    //  .. nothing yet ..
  }

  void process(const db::Polygon &poly,
               std::vector<db::Polygon> &result) const {
...
class DB_PUBLIC CornersAsDots : public db::PolygonToEdgeProcessorBase,
                                private CornerDetectorCore {
public:
  CornersAsDots(double angle_start, bool include_angle_start, double angle_end,
 ...
 */
class DB_PUBLIC CornersAsEdgePairs : public db::PolygonToEdgePairProcessorBase,
                                     private CornerDetectorCore {
public:
  CornersAsEdgePairs(double angle_start, bool include_angle_start,
                     double angle_end, bool include_angle_end)
      : CornerDetectorCore(angle_start, include_angle_start, angle_end,
                           include_angle_end) {
    //  .. nothing yet ..
  ...
  void process(const db::Polygon &poly,
               std::vector<db::EdgePair> &result) const {
...
/**
 *  @brief A processor delivering the extents (bounding box) of the merged
 * polygons
 */
class DB_PUBLIC Extents : public db::PolygonProcessorBase {
public:
  Extents() {
    //  .. nothing yet ..
  }
  ...
  virtual bool wants_variants() const {
    return false;
  } //  variants are too common, so don't do this
};

/**
 *  @brief A processor delivering the relative extents (bounding box) of the
 * merged polygons This processor allows over- or undersizing of the resulting
 * box by a given amount and delivery of a box relative to the original box.
 */
class DB_PUBLIC RelativeExtents : public db::PolygonProcessorBase {
public:
  RelativeExtents(double fx1, double fy1, double fx2, double fy2, db::Coord dx,
                  db::Coord dy)
 ...
 *  The edge runs from the relative coordinate fx1, fy1 (0: left/bottom, 1:
 * right/top) to fx2, fy2. This processor allows over- or undersizing of the
 * resulting box by a given amount and delivery of a box relative to the
 * original box.
 */
class DB_PUBLIC RelativeExtentsAsEdges : public db::PolygonToEdgeProcessorBase {
public:
 ...
 */
class DB_PUBLIC PolygonToEdgeProcessor : public db::PolygonToEdgeProcessorBase {
public:
 ...
 */
class DB_PUBLIC ConvexDecomposition : public db::PolygonProcessorBase {
public:
  ConvexDecomposition(db::PreferredOrientation mode) : m_mode(mode) {
    //  .. nothing yet ..
  }
  ...
  virtual bool result_must_not_be_merged() const {
    return true;
  } //  would spoil the decomposition otherwise
  virtual bool requires_raw_input() const { return false; }
 ...
 */
class DB_PUBLIC TrapezoidDecomposition : public db::PolygonProcessorBase {
public:
  TrapezoidDecomposition(db::TrapezoidDecompositionMode mode) : m_mode(mode) {
    //  .. nothing yet ..
  }
 ...
 */
class DB_PUBLIC PolygonBreaker : public db::PolygonProcessorBase {
public:
 ...
 */
class DB_PUBLIC PolygonSizer : public db::PolygonProcessorBase {
public:
...
public:
  minkowski_sum_computation(const K &q) : m_q(q) {
    //  .. nothing yet ..
  }
