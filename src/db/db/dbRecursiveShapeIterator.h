...
/**
 *  @brief An iterator delivering shapes that touch or overlap the given region
 * recursively
 *
 ...
 * considering subcells as well. Some options can be specified, i.e. the level
 * to which to look into or shape classes and shape properties.
 */
...
public:
  ...
  typedef db::box_tree<db::Box, db::Box, db::box_convert<db::Box>, 20, 20>
      box_tree_type;
   ...
   *
   *  This iterator will iterate the shapes from the given shapes container
   * using the given search region in overlapping or touching mode.
   */
  RecursiveShapeIterator(const shapes_type &shapes, const box_type &region,
   ...
   *
   *  This iterator will iterate the shapes from the given shapes container
   * using the given search region in overlapping or touching mode. It allows
   * specification of a complex search region.
   */
  RecursiveShapeIterator(const shapes_type &shapes, const region_type &region,
                         bool overlapping = false);
   ...
   *  @param overlapping Specify overlapping mode
   *
   *  By default the iterator operates in touching mode - i.e. shapes that touch
   * the given region are returned. By specifying the "overlapping" flag with a
   * true value, the iterator delivers shapes that overlap the given region by
   * at least one database unit.
   */
  RecursiveShapeIterator(const layout_type &layout, const cell_type &cell,
                         unsigned int layer, const box_type &region,
                         bool overlapping = false);
   ...
   * the given region are returned. By specifying the "overlapping" flag with a
   * true value, the iterator delivers shapes that overlap the given region by
   * at least one database unit. It allows specification of a complex search
   * region.
   */
  RecursiveShapeIterator(const layout_type &layout, const cell_type &cell,
                         unsigned int layer, const region_type &region,
                         bool overlapping = false);

  /**
   *  @brief Standard constructor for "world" iteration
   *
   *  This iterator delivers all shapes recursively. The same effect can be
   * achieved by using a "world" region.
   *
   *  @param layout The layout from which to get the cell hierarchy
   *  @param cell The starting cell
   *  @param layer The layer from which to deliver the shapes
  ...
  RecursiveShapeIterator(const layout_type &layout, const cell_type &cell,
                         ...
                         const box_type &region, bool overlapping = false);
  ...
  RecursiveShapeIterator(const layout_type &layout, const cell_type &cell,
                         const std::vector<unsigned int> &layers,
                         const region_type &region, bool overlapping = false);
   ...
   */
  RecursiveShapeIterator(const layout_type &layout, const cell_type &cell,
                         const std::set<unsigned int> &layers,
                         const box_type &region, bool overlapping = false);
   ...
   *
   *  A depth of 0 instructs the iterator to deliver only shapes from the
   * initial cell. The depth must be specified before the shapes are being
   * retrieved.
   */
   ...
   *
   *  Alternatively to layout/cell, the shape iterator can iterate shapes which
   * will deliver the shapes of the shape container rather than the cell. Layers
   * and hierarchy levels don't have a meaning in that case.
   */
  const shapes_type *shapes() const { return mp_shapes; }

  /**
   *  @brief Gets the layout
   */
  const layout_type *layout() const { return mp_layout.get(); }
   ...
   */
  const cell_type *top_cell() const { return mp_top_cell; }

  /**
   *  @brief Gets the installed property translator
   *
   *  The property translator is not automatically applied, but available to
   * consumers of shapes to perform property translation.
   */
  ...
  /**
   *  @brief Gets the basic region the iterator is using (will be world if none
   * is set) In addition to the basic region, a complex region may be defined
   * that is further confining the search to a subregion of the basic region.
   */
  const box_type &region() const { return m_region; }

  /**
   *  @brief Returns true if a complex region is given
   */
  bool has_complex_region() const { return mp_complex_region.get() != 0; }
  ...
  /**
   *  @brief Gets a flag indicating whether overlapping shapes are selected when
   * a region is used
   */
  bool overlapping() const { return m_overlapping; }

  /**
   *  @brief Sets a flag indicating whether overlapping shapes are selected when
   * a region is used
   */
   ...
   *
   *  The global transformation will be applied to all shapes delivered by
   * biasing the "trans" attribute
   */
   ...
   */
  cplx_trans_type global_trans() const { return m_global_trans; }

  /**
   *  @brief Gets the transformation which is to be applied always in push mode
   *
   *  The reasoning behind this method is that in push mode and with the
   * presence of a global transformation we need to somehow reflect the fact
   * that the top-level is transformed. Instead of transforming every shape and
   * instance we use this attribute. It is unity for all cells below top level
   * and equal to the global transformation for the top cell.
   */
   ...
   */
  void reset() { m_needs_reinit = true; }

  /**
   *  @brief Select cells
   *
   *  If no specific cells have been selected before, this method will confine
   * the selection to the given cells (plus their sub-hierarchy). If cells have
   * been selected before, this will add the given cells to the selection.
   */
   ...
   *
   *  This method will remove the given cells (plus their sub-hierarchy) from
   * the selection.
   */
   ...
   */
  const std::set<db::cell_index_type> &enables() const { return m_start; }

  /**
   *  @brief Returns the cells in the "disable" selection
   */
  const std::set<db::cell_index_type> &disables() const { return m_stop; }
   ...
   */
  unsigned int shape_flags() const { return m_shape_flags; }

  /**
   *  @brief Changes the layer to be traversed
   *
   *  This method must be used before shapes are being retrieved. Using this
   * method may reset the iterator.
   */
   ...
   *  It is not copied and the object must stay valid as long as the iterator is
   * used. The selector must be specified before the shapes are being retrieved.
   */
  void
  shape_property_selector(const shape_iterator::property_selector *prop_sel) {
   ...
   */
  const std::vector<unsigned int> &layers() const { return m_layers; }
   ...
   */
  bool multiple_layers() const { return m_has_layers; }

  /**
   *  @brief Get the current transformation by which the shapes must be
   * transformed into the initial cell
   *
   *  The shapes delivered are not transformed. Instead, this transformation
   * must be applied to get the shape in the coordinate system of the top cell.
   */
   ...
   *
   *  This version employs the property translator to deliver the real property
   * ID.
   */
   ...
   */
  db::cell_index_type cell_index() const { return cell()->cell_index(); }
  ...
  /**
   *  @brief Returns a value indicating whether the current cell is inactive
   * (disabled)
   */
  ...
  /**
   *  @brief Returns a value indicating whether a new child cell of the current
   * cell will be inactive
   */
...
public:
  ...
  /**
   *  @brief Returns true, if the receivers wants the full hierarchy and not
   * just non-empty cells
   */
   ...
   * "iter->trans()" will already be updated.
   *
   *  @param iter The iterator
   *  @param cell The cell which is entered
   *  @param region The clip box as seen from "cell" or db::Box::world if there
   * is no clip box
   *  @param complex_region A complex clip region if one is supplied together
   * with "region"
   */
  virtual void enter_cell(const RecursiveShapeIterator * /*iter*/,
                          const db::Cell * /*cell*/, const db::Box & /*region*/,
                          const box_tree_type * /*complex_region*/) {}

  /**
   *  @brief Leaves the current cell
   *
   *  This method is the counterpart for "enter_cell". It is called when
   * traversal of "cell" ended.
   */
  virtual void leave_cell(const RecursiveShapeIterator * /*iter*/,
                          const db::Cell * /*cell*/) {}

  /**
   *  @brief Enters a new instance
   *
   *  This method is called before "enter_cell" and "new_inst_member" is called
   * and will indicate the instance to follow. The sequence of events is
   *
   *    new_inst(A)
   ...
   *    ...
   *
   *  The "all" parameter is true, if all instances of the array will be
   * addressed.
   *
   ...
   */
  virtual new_inst_mode new_inst(const RecursiveShapeIterator * /*iter*/,
                                 const db::CellInstArray & /*inst*/,
                                 const db::ICplxTrans & /*always_apply*/,
                                 const db::Box & /*region*/,
                                 const box_tree_type * /*complex_region*/,
                                 bool /*all*/) {
    return NI_all;
   ...
   *  the array.
   *
   *  "all" is true, if an instance array is iterated in "all" mode (see
   * new_inst).
   *
   *  If this method returns false, this array instance (but not the whole
   * array) is skipped and the cell is not entered.
   */
  virtual bool new_inst_member(const RecursiveShapeIterator * /*iter*/,
                               const db::CellInstArray & /*inst*/,
                               const db::ICplxTrans & /*always_apply*/,
   ...
   */
  virtual void shape(const RecursiveShapeIterator * /*iter*/,
