...
public:
  ...
  typedef simple_polygon_ptr_array_type::iterator
  ...
  typedef db::box<coord_type, db::coord_traits<coord_type>::short_coord_type>
      short_box_type;
  ...
  enum region_mode { None, Overlapping, Touching };
  ...
  enum flags_type {
    ...
    Polygons = (1 << Polygon) | (1 << PolygonRef) | (1 << PolygonPtrArray) |
               (1 << SimplePolygon) | (1 << SimplePolygonRef) |
    ...
    Boxes =
        (1 << Box) | (1 << BoxArray) | (1 << ShortBox) | (1 << ShortBoxArray),
   ...
   *  @param flags The kind of shapes to iterate over (or-ed constants of
   * flags_type)
   */
  ShapeIterator(const shapes_type &shapes, unsigned int flags = All,
                const property_selector *prop_sel = 0,
                bool inv_prop_sel = false);

  /**
   *  @brief The region iterator
   *
   *  This constructor creates an iterator that iterates over all shapes of
   *  the selected kind that interact with the given region with their bounding
   * box. The kind of shape is selected by the "flags" argument which is an
   * or-ed sum of flags_type constants. The region_mode argument can be either
   * Overlapping or Touching. In the former case all shapes whose bounding box
   * overlaps with the given region are selected. In the latter case all shapes
   * whose bounding box touches the given region are selected.
   *
   *  @param shapes The shapes container to iterate over
   ...
   *  @param inv_prop_sel True, if shapes not matching the property selection
   * shall be reported
   */
  ShapeIterator(const shapes_type &shapes, const box_type &box,
                region_mode mode, unsigned int flags = All,
                const property_selector *prop_sel = 0,
                bool inv_prop_sel = false);
   ...
   *  Hint: for automation purposes, this function must return a copy, not a
   * reference! For a reference we have operator->
   */
  shape_type operator*() const { return m_shape; }

  /**
   *  @brief Access the object (see operator*)
   */
  const shape_type *operator->() const { return &m_shape; }

  /**
   *  @brief Returns true if we are inside an array
   */
  bool in_array() const { return m_array_iterator_valid; }

  /**
   *  @brief Finishes the array and move to the next shape which might be an
   * array as well
   */
  void finish_array();
   ...
   */
  const shape_type &array() const { return m_array; }
   ...
   */
  bool at_end() const { return m_type == Null; }

  /**
   *  @brief Gets the quad ID
   *
   *  The quad ID is a unique identifier for the current quad. This can be used
   * to detect whether the iterator entered a new quad and optimize the search
   * in that case.
   */
  size_t quad_id() const { return m_quad_id; }

  /**
   *  @brief Gets the quad box
   *
   *  Gets the box the current quad uses. This box may be larger than the actual
   * shape containers bounding box. Specifically if there is no quad tree at
   * all, this box is the world box.
   */
   ...
   *
   *  Moves to the next quad. This method can be used to shortcut searching if
   * we are inside a quad that is not relevant for the search.
   */
  void skip_quad() { advance(-1); }

  /**
   *  @brief Gets the arrays quad ID
   *
   *  The arrays quad ID is a unique identifier for the current quad for
   * iterated arrays. This can be used to detect whether the iterator entered a
   * new quad and optimize the search in that case.
   */
   ...
   * we are inside a quad that is not relevant for the search.
   */
  void skip_array_quad() { advance(2); }

private:
  //  a helper union for the iter_size union
  //  (basically computing the size required for all iterators for a certain
  //  shape/array type)
  template <class Shape> class per_shape_iter_size {
    union {
      //  stable layer iterators
      char sz_n[sizeof(
          typename db::layer<Shape, db::stable_layer_tag>::flat_iterator)];
      char sz_np[sizeof(
          typename db::layer<db::object_with_properties<Shape>,
                             db::stable_layer_tag>::flat_iterator)];
      char sz_t[sizeof(
          typename db::layer<Shape, db::stable_layer_tag>::touching_iterator)];
      char sz_tp[sizeof(
          typename db::layer<db::object_with_properties<Shape>,
                             db::stable_layer_tag>::touching_iterator)];
      char sz_o[sizeof(typename db::layer<
                       Shape, db::stable_layer_tag>::overlapping_iterator)];
      char sz_op[sizeof(
          typename db::layer<db::object_with_properties<Shape>,
                             db::stable_layer_tag>::overlapping_iterator)];
      //  unstable layer iterators
      char sz_nu[sizeof(
          typename db::layer<Shape, db::unstable_layer_tag>::iterator)];
      char sz_npu[sizeof(typename db::layer<db::object_with_properties<Shape>,
                                            db::unstable_layer_tag>::iterator)];
      char sz_tu[sizeof(typename db::layer<
                        Shape, db::unstable_layer_tag>::touching_iterator)];
      char sz_tpu[sizeof(
          typename db::layer<db::object_with_properties<Shape>,
                             db::unstable_layer_tag>::touching_iterator)];
      char sz_ou[sizeof(typename db::layer<
                        Shape, db::unstable_layer_tag>::overlapping_iterator)];
      char sz_opu[sizeof(
          typename db::layer<db::object_with_properties<Shape>,
                             db::unstable_layer_tag>::overlapping_iterator)];
    } all_iterators;
  };
  ...
  size_t quad_id_by_shape(TouchingRegionTag) const;
  template <class Sh, class StableTag>
  size_t quad_id_by_shape(OverlappingRegionTag) const;
  template <class RegionTag, class StableTag> size_t quad_id_generic() const;

  template <class Sh, class StableTag>
  db::Box quad_box_by_shape(TouchingRegionTag) const;
  template <class Sh, class StableTag>
  db::Box quad_box_by_shape(OverlappingRegionTag) const;
...
public:
  ...
  virtual void translate_into(Shapes *target, GenericRepository &rep,
                              ArrayRepository &array_rep) const = 0;
  virtual void translate_into(Shapes *target, GenericRepository &rep,
                              ArrayRepository &array_rep,
                              pm_delegate_type &pm) const = 0;
  virtual void transform_into(Shapes *target, const Trans &trans,
  ...
  virtual void transform_into(Shapes *target, const Trans &trans,
                              ...
                              ArrayRepository &array_rep,
                              pm_delegate_type &pm) const = 0;
  virtual void transform_into(Shapes *target, const ICplxTrans &trans,
                              GenericRepository &rep,
                              ArrayRepository &array_rep) const = 0;
  virtual void transform_into(Shapes *target, const ICplxTrans &trans,
                              GenericRepository &rep,
  ...
  virtual void deref_and_transform_into(Shapes *target, const Trans &trans,
                                        pm_delegate_type &pm) = 0;
  virtual void deref_and_transform_into(Shapes *target,
                                        const ICplxTrans &trans) = 0;
  virtual void deref_and_transform_into(Shapes *target, const ICplxTrans &trans,
                                        pm_delegate_type &pm) = 0;
  virtual unsigned int type_mask() const = 0;

  virtual void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
...
class DB_PUBLIC Shapes : public db::Object {
public:
  typedef db::Coord coord_type;
  ...
  /**
   *  @brief Default ctor: create an empty collection of shapes without external
   * references
   *
   *  Such shape containers can be used for example to store temporary shape
   * sets
   *
   *  Standalone shape containers are usually used for temporary storage. Such
   * containers are created in editable mode to allow insertion and deletion of
   * shapes by default.
   */
  Shapes() : db::Object(0), mp_cell(0) { set_editable(true); }
   ...
   *
   *  Such shape containers can be used for example to store temporary shape
   * sets This version allows one to specify whether the container should be
   * created in editable mode or insert-once mode.
   */
  Shapes(bool editable) : db::Object(0), mp_cell(0) { set_editable(editable); }

  /**
   *  @brief Standard ctor: create an empty collection referencing a graph (via
   * state model)
   *
   *  The state model reference is used to invalid the bbox flag of the graph
   *  whenever something changes on the shapes list.
   *
   *  Since such containers are usually used in a layout context, they are
   * created according to the editable settings of the database by default.
   */
  Shapes(db::Manager *manager, db::Cell *cell, bool editable)
      : db::Object(manager), mp_cell(cell) {
    set_dirty(false);
   ...
   *  @brief Copy ctor
   *
   *  HINT: This method can duplicate shape containers from one layout to
   * another. The current implementation does not translate property Id's. It is
   * mainly intended for 1-to-1 copies of layouts where the whole property
  ...
  /**
   *  @brief Insert all shapes from another container using the given shape
   * types only
   *
   *  This method insert all shapes from the given shape container.
   *
   ...
   * repository is copied.
   */
  template <class T> void assign_transformed(const Shapes &d, const T &trans) {
    clear();
   ...
   *
   *  This version allows one to specify a property mapping function. That way,
   * shapes can be copied from one layout space to another.
   */
   ...
   *
   *  In contrast to the operator= version allows one to specify a property
   * mapping function. That way, shapes can be copied from one layout space to
   * another.
   */
  template <class PropIdMap> void assign(const Shapes &d, PropIdMap &pm) {
    clear();
    insert(d, pm);
  }
   ...
   *
   *  This method insert all shapes from the given shape container using the
   ...
   * repository is copied.
   */
  template <class T> void insert_transformed(const Shapes &d, const T &trans) {
    tl_assert(&d != this);
    ...
    if (manager() && manager()->transacting()) {
      ...
      if (layout() == 0) {
        ...
        for (tl::vector<LayerBase *>::const_iterator l = d.m_layers.begin();
             l != d.m_layers.end(); ++l) {
          (*l)->transform_into(this, trans, shape_repository(),
                               array_repository());
        }
      }
    }
   ...
   * another.
   */
  template <class PropIdMap> void insert(const Shapes &d, PropIdMap &pm) {
    tl_assert(&d != this);
   ...
   */
  template <class Sh> shape_type insert(const Sh &sh) {
    if (manager() && manager()->transacting()) {
      check_is_editable_for_undo_redo();
      if (is_editable()) {
        ...
        db::layer_op<Sh, db::unstable_layer_tag>::queue_or_append(
            manager(), this, true /*insert*/, sh);
      }
    }
    ...
    if (is_editable()) {
      ...
      return shape_type(this,
                        *get_layer<Sh, db::unstable_layer_tag>().insert(sh));
    }
  }
  ...
  shape_type insert(const db::array<Obj, Trans> &arr) {
    if (is_editable()) {
      ...
      if (manager() && manager()->transacting()) {
        check_is_editable_for_undo_redo();
        db::layer_op<db::array<Obj, Trans>,
                     db::unstable_layer_tag>::queue_or_append(manager(), this,
                                                              true /*insert*/,
                                                              arr);
      }
      invalidate_state(); //  HINT: must come before the change is done!
      return shape_type(
          this,
          *get_layer<db::array<Obj, Trans>, db::unstable_layer_tag>().insert(
              arr));
    }
  }
  ...
  template <class Obj, class Trans>
  shape_type
  insert(const db::object_with_properties<db::array<Obj, Trans>> &arr) {
    if (is_editable()) {
      ...
      if (manager() && manager()->transacting()) {
        check_is_editable_for_undo_redo();
        db::layer_op<db::object_with_properties<db::array<Obj, Trans>>,
                     db::unstable_layer_tag>::queue_or_append(manager(), this,
                                                              true /*insert*/,
                                                              arr);
      }
      invalidate_state(); //  HINT: must come before the change is done!
      return shape_type(
          this, *get_layer<db::object_with_properties<db::array<Obj, Trans>>,
   ...
   *
   *  It is not allowed to insert arrays in editable mode this way, because
   * these are not expanded in editable mode.
   *
   *  Inserts a sequence of shapes [from,to)
   */
  template <class Iter> void insert(Iter from, Iter to) {
    typedef typename std::iterator_traits<Iter>::value_type value_type;
    if (manager() && manager()->transacting()) {
      check_is_editable_for_undo_redo();
      if (is_editable()) {
        db::layer_op<value_type, db::stable_layer_tag>::queue_or_append(
            manager(), this, true /*insert*/, from, to);
      } else {
        db::layer_op<value_type, db::unstable_layer_tag>::queue_or_append(
            manager(), this, true /*insert*/, from, to);
      }
    }
    invalidate_state();
  ...
  shape_type insert(const shape_type &shape) {
    tl::ident_map<db::properties_id_type> pm;
    tl::func_delegate<tl::ident_map<db::properties_id_type>,
   ...
   *
   *  @param shape The reference of the shape to insert
   *  @param pm The property ID mapper - the property ID will be mapped
   * accordingly, i.e. for transferring shapes from one layout to another
   *  @return The reference to the new shape
   */
  ...
  shape_type insert(const shape_type &shape, const Trans &trans,
                    PropIdMap &pm) {
    tl::func_delegate<PropIdMap, db::properties_id_type> pm_delegate(pm);
    return do_insert(shape, trans, pm_delegate);
  }
   ...
   */
  template <class Tag> void reserve(Tag /*tag*/, size_t n) {
    if (is_editable()) {
  ...
  void erase(
      ...
      typename db::layer<typename Tag::object_type, StableTag>::iterator pos) {
    if (!is_editable()) {
   ...
   *  @brief Erase an element by the shape reference
   *
   *  Given a set of ordered shape reference, the corresponding shapes are
   * erased. Hint: for the following code it is important that the ArrayMember
   * types come *after* the non-member types. This is because this way it is
   * ensured that array members get erased after the respective single
   * shapes/shape instances. That guarantees that if single shapes are created
   * on array resolution these shapes can be added to the single shape lists
   * without invalidating other shape references into them in db::shapes::erase
   * with multi-shape erase. This method is only allowed in editable mode.
   *
   *  @param shapes The reference to the shapes to delete. This array must be
   * sorted using the Shape's operator<
   */
  ...
  template <class Tag, class StableTag>
  void
  erase(Tag /*tag*/, StableTag /*stable_tag*/,
        typename db::layer<typename Tag::object_type, StableTag>::iterator from,
    ...
    if (manager() && manager()->transacting()) {
      check_is_editable_for_undo_redo();
      db::layer_op<typename Tag::object_type, StableTag>::queue_or_append(
          manager(), this, false /*not insert*/, from, to);
    }
    invalidate_state(); //  HINT: must come before the change is done!
  ...
  void erase_positions(Tag /*tag*/, StableTag /*stable_tag*/, I first, I last) {
    ...
    if (manager() && manager()->transacting()) {
      check_is_editable_for_undo_redo();
      db::layer_op<typename Tag::object_type, StableTag>::queue_or_append(
          manager(), this, false /*not insert*/, first, last, true /*dummy*/);
    }
    invalidate_state(); //  HINT: must come before the change is done!
    get_layer<typename Tag::object_type, StableTag>().erase_positions(first,
                                                                      last);
  }
   ...
   */
  shape_type replace_prop_id(const shape_type &ref,
                             db::properties_id_type prop_id);

  /**
   *  @brief Replace an element by a given shape
   *
   *  Replaces the shape pointed to by the given shape reference by the given
   * shape. It is not possible to replace an array member by something else
   * currently.
   *
   *  If the original shape has a property, just the basic shape is replaced.
   * The property id is not touched.
   *
   *  @param ref The shape reference which to replace the shape
   *  @param sh The shape to replace
   *  @return The reference to the new object
   */
  template <class Sh> shape_type replace(const shape_type &ref, const Sh &sh);

  /**
   *  @brief Replace an element by a given shape with properties
   *
   *  Replaces the shape pointed to by the given shape reference by the given
   * shape with properties. It is not possible to replace an array member by
   * something else currently. If the shape has a property, just the basic shape
   * is replaced. The property id is not touched. This method is only allowed in
   * editable mode.
   *
   *  @param ref The shape reference which to replace the shape
   *  @param sh The shape to replace
   ...
   */
  template <class Sh>
  shape_type replace(const shape_type &ref,
                     const db::object_with_properties<Sh> &sh) {
  ...
  /**
   *  @brief Transform an element given by the shape reference by the given
   * transformation
   *
   *  Replaces the shape pointed to by the given shape reference by the one
   * transformed with the given transformation. It is not possible to transform
   * an array member by something else currently. If the shape has a property,
   * just the basic shape is replaced. The property id is not touched. This
   * method is only allowed in editable mode.
   *
   *  @param ref The shape reference which to replace the shape
  ...
  /**
   *  @brief Returns a value indicating whether the shape container is modified
   * and needs update
   */
  ...
  size_t size(unsigned int flags) const {
    ...
    for (tl::vector<LayerBase *>::const_iterator l = m_layers.begin();
      ...
      if (((flags & db::ShapeIterator::Properties) == 0 ||
  ...
  /**
   *  @brief Deliver the flat iterator (see box_tree::flat_iterator for a
   * description)
   *
   *  @param tag The shape type's tag (i.e. db::Polygon::tag)
   *
   ...
   */
  template <class Tag, class StableTag>
  typename db::layer<typename Tag::object_type, StableTag>::flat_iterator
  begin_flat(Tag /*tag*/, StableTag /*stable_tag*/) const {
    //  ensure that the box tree is established for stable shape containers in
    //  editable mode (in this case, the iterator uses the flat_iterator which
    //  accesses the elements through the sorted elements table which is more
    //  stable).
    const db::layer<typename Tag::object_type, StableTag> &l =
        layer<typename Tag::object_type, StableTag>();
    if (is_editable()) {
   ...
   */
  template <class Tag, class StableTag>
  typename db::layer<typename Tag::object_type, StableTag>::touching_iterator
  begin_touching(const box_type &b, Tag /*tag*/,
                 StableTag /*stable_tag*/) const {
   ...
   */
  template <class Tag, class StableTag>
  typename db::layer<typename Tag::object_type, StableTag>::overlapping_iterator
  begin_overlapping(const box_type &b, Tag /*tag*/,
                    StableTag /*stable_tag*/) const {
    //  ensure that the box tree is established
    const db::layer<typename Tag::object_type, StableTag> &l =
        layer<typename Tag::object_type, StableTag>();
  ...
  begin(Tag /*tag*/, StableTag /*stable_tag*/) const {
  ...
  template <class Tag, class StableTag>
  typename db::layer<typename Tag::object_type, StableTag>::iterator
  end(Tag /*tag*/, StableTag /*stable_tag*/) const {
   ...
   *  @param inv_prop_sel true, if prop_sel should select all ids that should
   * not be iterated
   *
   *  @return The generic iterator
   */
  shape_iterator begin(unsigned int flags,
                       const shape_iterator::property_selector *prop_sel = 0,
                       bool inv_prop_sel = false) const {
    //  ensure that the box tree is established for stable shape containers in
    //  editable mode (in this case, the iterator uses the flat_iterator which
    //  accesses the elements through the sorted elements table which is more
    //  stable).
   ...
   */
  shape_iterator
  begin_touching(const box_type &box, unsigned int flags,
                 const shape_iterator::property_selector *prop_sel = 0,
                 bool inv_prop_sel = false) const {
    //  ensure that the box tree is established
    (const_cast<Shapes *>(this))->sort();
    flags &= ((~shape_iterator::All) | type_mask());
    return shape_iterator(*this, box, shape_iterator::Touching, flags, prop_sel,
                          inv_prop_sel);
  }
   ...
   */
  shape_iterator
  begin_overlapping(const box_type &box, unsigned int flags,
                    const shape_iterator::property_selector *prop_sel = 0,
                    bool inv_prop_sel = false) const {
    //  ensure that the box tree is established
    (const_cast<Shapes *>(this))->sort();
    flags &= ((~shape_iterator::All) | type_mask());
    return shape_iterator(*this, box, shape_iterator::Overlapping, flags,
                          prop_sel, inv_prop_sel);
  }
   ...
   *  @brief find a given shape (exactly)
   *
   *  @param s The shape to find (maybe a reference to a shape in a different
   * container)
   *
   ...
   *
   *  Hint: this method is provided to the shape iterator mainly. Do not modify
   * the layer object.
   */
  template <class Sh, class StableTag>
   ...
   * the layer object.
   */
  template <class Sh, class StableTag> db::layer<Sh, StableTag> &get_layer();
   ...
   */
  db::Cell *cell() const { return (db::Cell *)(size_t(mp_cell) & ~3); }

  /**
   *  @brief Gets a flag indicating whether an update is needed
   *
   *  This flag means that the shape collection has been modified and the
   * bounding box and the quad trees will be recomputed (internally).
   */
  bool is_dirty() const { return (size_t(mp_cell) & 1) != 0; }

  /**
   *  @brief Gets a value indicating that the shape collection is constructed
   * with editable scope
   */
  bool is_editable() const { return (size_t(mp_cell) & 2) != 0; }
...
private:
  ...
  tl::vector<LayerBase *> &get_layers() { return m_layers; }
  ...
  shape_type insert_array_by_tag(Tag tag, const shape_type &shape,
   ...
   */
  template <class Tag, class PropIdMap>
  shape_type insert_by_tag(Tag tag, const shape_type &shape,
                           repository_type &rep, PropIdMap &pm);
  ...
  template <class Tag, class StableTag>
  void erase_shape_by_tag_ws(Tag tag, StableTag stable_tag,
   ...
   */
  template <class Tag>
  void erase_shapes_by_tag(Tag tag,
                           typename std::vector<shape_type>::const_iterator s1,
                           typename std::vector<shape_type>::const_iterator s2);

  template <class Tag, class StableTag>
  void
  erase_shapes_by_tag_ws(Tag tag, StableTag stable_tag,
                         typename std::vector<shape_type>::const_iterator s1,
                         typename std::vector<shape_type>::const_iterator s2);

  //  The insert delegate
  template <class Trans>
  shape_type do_insert(const shape_type &shape, const Trans &trans,
                       tl::func_delegate_base<db::properties_id_type> &pm);

  //  The insert delegate, specialization for unit_trans
  shape_type do_insert(const shape_type &shape, const unit_trans_type &trans,
                       tl::func_delegate_base<db::properties_id_type> &pm);
  ...
  shape_type replace_prop_id_iter(typename db::object_tag<Sh>, const Iter &iter,
                                  db::properties_id_type prop_id);

  //  A helper function to replace a shape given by a generic reference by doing
  //  an erase & insert Sh2 must not be a shape with properties
  template <class Sh1, class Sh2>
  shape_type reinsert_member_with_props(typename db::object_tag<Sh1>,
  ...
  template <class Sh1, class Sh2>
  shape_type replace_member_with_props(typename db::object_tag<Sh1>,
                                       const shape_type &ref, const Sh2 &sh);

  //  A helper function to replace a shape given by a generic reference (no
  //  conversion) Sh must not be a shape with properties
  template <class Sh>
  shape_type replace_member_with_props(typename db::object_tag<Sh>,
                                       const shape_type &ref, const Sh &sh);

  template <class ResType, class Array>
  void insert_array_typeof(const ResType &,
                           const db::object_with_properties<Array> &arr) {
    typedef db::object_with_properties<ResType> res_wp_type;

    //  expand arrays in editable mode
    invalidate_state(); //  HINT: must come before the change is done!
    db::layer<res_wp_type, db::stable_layer_tag> &l =
        get_layer<res_wp_type, db::stable_layer_tag>();
    for (typename Array::iterator a = arr.begin(); !a.at_end(); ++a) {
      res_wp_type obj_wp(*a * arr.object(), arr.properties_id());
      if (manager() && manager()->transacting()) {
        check_is_editable_for_undo_redo();
        db::layer_op<res_wp_type, db::stable_layer_tag>::queue_or_append(
            manager(), this, true /*insert*/, obj_wp);
      }
      l.insert(obj_wp);
    }
  }
  ...
  void insert_array_typeof(const ResType &, const Array &arr) {
    //  expand arrays in editable mode
    invalidate_state(); //  HINT: must come before the change is done!
    db::layer<ResType, db::stable_layer_tag> &l =
        get_layer<ResType, db::stable_layer_tag>();
    for (typename Array::iterator a = arr.begin(); !a.at_end(); ++a) {
      if (manager() && manager()->transacting()) {
        check_is_editable_for_undo_redo();
        db::layer_op<ResType, db::stable_layer_tag>::queue_or_append(
            manager(), this, true /*insert*/, *a * arr.object());
      }
 ...
 */
class DB_PUBLIC LayerOpBase : public db::Op {
public:
  LayerOpBase() : db::Op() {}
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const db::Shapes &x, bool no_self, void *parent) {
 ...
 */
template <class Sh, class StableTag>
class DB_PUBLIC_TEMPLATE layer_op : public LayerOpBase {
public:
  layer_op(bool insert, const Sh &sh) : m_insert(insert) {
    m_shapes.reserve(1);
    m_shapes.push_back(sh);
  }

  template <class Iter>
  layer_op(bool insert, Iter from, Iter to) : m_insert(insert) {
    m_shapes.insert(m_shapes.end(), from, to);
  }

  template <class Iter>
  layer_op(bool insert, Iter from, Iter to, bool /*dummy*/) : m_insert(insert) {
  ...
  static void queue_or_append(db::Manager *manager, db::Shapes *shapes,
                              bool insert, const Sh &sh) {
    db::layer_op<Sh, StableTag> *old_op =
        dynamic_cast<db::layer_op<Sh, StableTag> *>(
            manager->last_queued(shapes));
  ...
  template <class Iter>
  static void queue_or_append(db::Manager *manager, db::Shapes *shapes,
                              bool insert, Iter from, Iter to, bool dummy) {
    db::layer_op<Sh, StableTag> *old_op =
        dynamic_cast<db::layer_op<Sh, StableTag> *>(
            manager->last_queued(shapes));
    if (!old_op || old_op->m_insert != insert) {
      manager->queue(shapes,
                     new db::layer_op<Sh, StableTag>(insert, from, to, dummy));
    } else {
      for (Iter i = from; i != to; ++i) {
...
class FullLayerOp : public LayerOpBase {
public:
