 ...
 *
 *  This class delivers the multiplicity for a cell (number of "as if flat"
 * instances of the cell in all top cells. It is instantiated with a reference
 * to a cell graph object. This object caches cell counts for multiple cells. It
 * is efficient to instantiate this object once and use it as often as possible.
 */
...
public:
  ...
  /**
   *  @brief Instantiate a counter object with a reference to the given cell
   * graph
   */
   ...
   * graph
   *
   *  This version allows one to specify a initial (starting) cell where only
   * the cell tree below the staring cell is considered. Multiplicity refers to
   * the number of instances below the initial cell.
   */
   ...
   *
   *  The iterator pair delivers all selected cells (only applicable if an
   * initial cell is specified).
   */
  selection_iterator begin() const { return m_selection.begin(); }

  /**
   *  @brief End iterator for the cells in the selection
   */
  selection_iterator end() const { return m_selection.end(); }

  /**
   *  @brief Get the selection cone
   */
  const selection_t &selection() const { return m_selection; }
 ...
 *
 *  This class provides a way to efficiently run a cell instance statistics over
 * a cell graph or a sub tree of the graph.
 *
 *  The Value specifies the statistics object that is collected over the
 * hierarchy. It must provide the following methods:
 *    - ctor (const db::Layout &layout, const db::Cell &cell);
 *    - Value transformed (const db::CellInstArray &trans) const;
 *    - add (const Value &other);
 *
 *  The ctor is supposed to create a value for the cell representing "no
 * instance". The transformed method is supposed to transform a value from a
 * parent to a child cell. The instance specifies the parent instance which
 * transforms into the child cell. The add method is supposed to add value for a
 * cell (but a different instance) to *this.
 *
 ...
 */
template <class Value> class InstanceStatistics {
public:
   ...
   * graph
   */
  InstanceStatistics(const db::Layout *layout) : m_cache(), mp_layout(layout) {
    //  .. nothing yet ..
  }
   ...
   *  top cells of the graph. The top cell delivers the default value (default
   * ctor of Value).
   */
  Value value(db::cell_index_type ci) {
    ...
    if (c != m_cache.end()) {
      return c->second;
               ...
               m_selection.find(ci) == m_selection.end()) {
      ...
      for (db::Cell::parent_inst_iterator p = cell->begin_parent_insts();
        ...
        if (m_selection.empty() ||
            m_selection.find(p->parent_cell_index()) != m_selection.end()) {
          res.add(value(p->parent_cell_index())
                      .transformed(p->child_inst().cell_inst()));
        }
      }
