 ...
 *
 *  Text sets are created from a text-delivering recursive shape iterator for
 * example. Text sets can be converted to polygons (representing a small box
 * around the text's point) or to dot-like edges representing the point of the
 * text.
 */
class DB_PUBLIC Texts : public db::ShapeCollection {
public:
   ...
   */
  explicit Texts(const db::Text &s) : mp_delegate(0) { insert(s); }
   ...
   *  Creates an text set representing a single instance of that object
   */
  explicit Texts(const db::Shape &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Sequence constructor
   *
   *  Creates an edge set from a sequence of objects. The objects need to be
   * texts. This version accepts iterators of the begin ... end style.
   */
  template <class Iter>
  explicit Texts(const Iter &b, const Iter &e) : mp_delegate(0) {
    reserve(e - b);
    for (Iter i = b; i != e; ++i) {
      insert(*i);
    }
   ...
   *
   *  Creates an text set from a recursive shape iterator. This allows one to
   * feed an text set from a hierarchy of cells.
   */
   ...
   *
   *  Creates an text set from a recursive shape iterator. This allows one to
   * feed an text set from a hierarchy of cells. The transformation is useful to
   ...
   * representation
   *
   *  This version will create a hierarchical text collection. The
   ...
   */
  const TextsDelegate *delegate() const { return mp_delegate; }

  /**
   *  @brief Gets the underlying delegate object
   */
  TextsDelegate *delegate() { return mp_delegate; }
   ...
   */
  const_iterator begin() const { return TextsIterator(mp_delegate->begin()); }

  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the texts plus the
   ...
   *
   *  nmax specifies how many texts are included (set to
  ...
  /**
   *  @brief Selects all polygons of the other region set which include the
   * texts of this text collection
   *
  ...
  /**
   *  @brief Selects all texts of this text set which are inside the polygons
   * from the region
   */
  ...
  /**
   *  @brief Returns all texts of this text set which are inside the polygons
   * from the region
   *
  ...
  /**
   *  @brief Selects all texts of this text set which are not inside the
  ...
  /**
   *  @brief Returns all texts of this text set which are not inside the
   * polygons from the region
   *
   ...
   */
  template <class T> Texts &transform(const T &trans);

  /**
   *  @brief Returns the transformed text set
   */
  template <class T> Texts transformed(const T &trans) const {
    Texts d(*this);
    d.transform(trans);
    return d;
  }
   ...
   */
  void swap(db::Texts &other) { std::swap(other.mp_delegate, mp_delegate); }
   ...
   */
  const db::Text *nth(size_t n) const { return mp_delegate->nth(n); }
   ...
   *
   *  If the region has valid texts, it is permissable to use the text's
   * addresses from the iterator. Furthermore, the random access operator nth()
   * is available.
   */
  bool has_valid_texts() const { return mp_delegate->has_valid_texts(); }
   ...
   *  width and height. The center of the boxes will be the position of the
   * texts.
   */
   ...
   * layer If the text collection is a hierarchical region, the hierarchy is
   * copied into the layout's hierarchy.
   */
  ...
  /**
   *  @brief Inserts the edge pair collection into the given layout, cell and
   * layer as polygons with the given enlargement If the text collection is a
   * hierarchical region, the hierarchy is copied into the layout's hierarchy.
   */
