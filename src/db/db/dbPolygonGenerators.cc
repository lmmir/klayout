...
struct PGPoint {
  ...
  PGPoint(const db::Point &p, size_t i, bool f)
      : point(p), contour(i), first(f) {}
...
public:
  ...
  PGPolyContour() : m_is_hole(false), m_next(-1), m_last(-1) {
    // ...
  }

  PGPolyContour(const PGPolyContour &d)
      : m_contour(d.m_contour), m_is_hole(d.m_is_hole), m_next(d.m_next),
        m_last(d.m_last) {
    // ...
  }
  ...
  void last(long n) { m_last = n; }

  long last() const { return m_last; }

  void next(long n) { m_next = n; }

  long next() const { return m_next; }

  bool is_hole() const { return m_is_hole; }

  void is_hole(bool hole) { m_is_hole = hole; }
  ...
  void erase(iterator from, iterator to) { m_contour.erase(from, to); }

  template <class I> iterator insert(iterator at, I from, I to) {
...
class PGContourList {
public:
  PGContourList() : m_free_contours(-1) {}

  PGPolyContour &operator[](size_t n) { return m_contours[n]; }

  const PGPolyContour &operator[](size_t n) const { return m_contours[n]; }
  ...
  void append(size_t what, size_t to) {
    m_contours[m_contours[to].next() < 0 ? long(to) : m_contours[to].last()]
        .next(long(what));
...
PolygonGenerator::PolygonGenerator(PolygonSink &psink, bool resolve_holes,
                                   bool min_coherence)
    : EdgeSink(), mp_contours(new PGContourList()),
      m_y(std::numeric_limits<db::Coord>::min()), m_open_pos(m_open.end()),
      mp_psink(&psink), mp_spsink(0), m_resolve_holes(resolve_holes),
      m_open_contours(false), m_min_coherence(min_coherence), m_compress(true) {
  //  .. nothing yet ..
}

PolygonGenerator::PolygonGenerator(SimplePolygonSink &spsink,
                                   bool min_coherence)
    : EdgeSink(), mp_contours(new PGContourList()),
      m_y(std::numeric_limits<db::Coord>::min()), m_open_pos(m_open.end()),
      mp_psink(0), mp_spsink(&spsink), m_resolve_holes(true),
      m_open_contours(false), m_min_coherence(min_coherence), m_compress(true) {
  //  .. nothing yet ..
}
...
void PolygonGenerator::flush() {
  ...
  for (open_map_iterator_type i = m_open.begin(); i != m_open.end(); ++i) {
    printf("%ld:%s%c%c ", i->contour, i->point.to_string().c_str(),
           i->first ? '!' : ' ', i == m_open_pos ? '*' : ' ');
  }
  printf("\n");
#endif
...
#ifdef DEBUG_POLYGON_GENERATOR
  printf("m_open=");
  for (open_map_type::const_iterator o = m_open.begin(); o != m_open.end();
       ++o) {
...
void PolygonGenerator::put(const db::Edge &e) {
#ifdef DEBUG_POLYGON_GENERATOR
  printf("put(%s) y=%d m_open(%ld)=", e.to_string().c_str(), m_y,
         std::distance(m_open.begin(), m_open_pos));
  for (open_map_iterator_type i = m_open.begin(); i != m_open.end(); ++i) {
    printf("%ld:%s%c%c ", i->contour, i->point.to_string().c_str(),
           i->first ? '!' : ' ', i == m_open_pos ? '*' : ' ');
  }
  ...
  if (m_open_pos != m_open.end() && e.p1().y() == m_y &&
      m_open_pos->point == e.p1() && (!m_min_coherence || e.dy() == 0)) {
  ...
  } else if (m_open_pos != m_open.end() && e.p2().y() == m_y &&
             m_open_pos->point == e.p2() && (m_min_coherence || e.dy() == 0)) {
...
void PolygonGenerator::eliminate_hole() {
  ...
  //  We found the initial edges of a new hole: connect the (partial) hole with
  //  a stitch line to the left. This way we can turn the hole into a non-hole
  //  contour.
  ...
  //  Build a separate contour that continues the contours to the left of the
  //  hole
  PGPolyContour cc = c;
  cc.clear();
  ...
  cprev.back() = pprev;
  while (cprev.size() > 2 && cprev.back().y() == m_y &&
         cprev.end()[-2].y() == m_y &&
         cprev.back().x() <= cprev.end()[-2].x()) {
    cprev.pop_back();
  }
...
void PolygonGenerator::join_contours(db::Coord x) {
  while (m_open_pos != m_open.end()) {
    ...
    ++n;
    if (n == m_open.end() || m_open_pos->point.y() != m_y ||
        m_open_pos->point != n->point || m_open_pos->point.x() > x) {
      return;
    }
    ...
    //  don't join, except -/+ pair (for max. coherence) or +/- pair (for min
    //  coherence)
    bool minus0 =
        (m_open_pos->first == (*mp_contours)[m_open_pos->contour].is_hole());
    ...
    for (open_map_iterator_type i = m_open.begin(); i != m_open.end(); ++i) {
      printf("%ld:%s%c%c%c%c ", i->contour, i->point.to_string().c_str(),
             i->first ? '!' : ' ', i == m_open_pos ? '*' : ' ',
             i == n ? '+' : ' ', i == nn ? '#' : ' ');
    }
    printf("\n");
    ...
    if (i1 != i2) {
      ...
      if (m_open_contours && !c1.is_hole() && !c2.is_hole()) {
        ...
        db::Edge eprev(cprev.end()[-2], cprev.back());
        db::Coord xprev =
            db::coord_traits<db::Coord>::rounded(edge_xaty(eprev, m_y));
        db::Point pprev(xprev, m_y);
        ...
        if (iprev == i1) {
          ...
          if (cprev.begin()->y() == m_y && cprev.begin()[1].y() == m_y &&
              cprev.front().x() >= cprev.begin()[1].x()) {
      ...
      if (!c1.is_hole()) {
...
#if 1
        //  shallow analysis: insert the cutline at the end of the sequence -
        //  this may cut lines collinear with contour edges

        eprev = db::Edge(ins[-2], ins[-1]);
        xprev = db::coord_traits<db::Coord>::rounded(
            edge_xaty(db::Edge(ins[-2], ins[-1]), m_y));
#else
        //  deep analysis: determine insertion point: pick the one where the
        //  cutline is shortest

        for (PGPolyContour::iterator i = ins; i > cprev.begin() + 1; --i) {

          db::Edge ecut(i[-2], i[-1]);
          db::Coord xcut = db::coord_traits<db::Coord>::rounded(
              edge_xaty(db::Edge(i[-2], i[-1]), m_y));

          if (ins == i || (i[-1].y() >= m_y && i[-2].y() < m_y &&
                           xcut < c1.back().x() && xcut > xprev)) {
            xprev = xcut;
            eprev = ecut;
            ins = i;
          }
        ...
        ins[-1] = pprev;
        while (ins - cprev.begin() > 1 && ins[-2].y() == m_y &&
               ins[-1].y() == m_y) {
...
bool PolygonGenerator::ms_compress = true;

void PolygonGenerator::produce_poly(const PGPolyContour &c) {
  size_t n = 0;
  for (long inext = c.next(); inext >= 0;
       inext = (*mp_contours)[inext].next()) {
    ++n;
  }
  ...
  if (mp_spsink) {
    ...
    tl_assert(n == 0); //  there should not be holes since we forced
                       //  resolve_holes to true ...
...
TrapezoidGenerator::TrapezoidGenerator(PolygonSink &psink)
    : EdgeSink(), m_y(std::numeric_limits<db::Coord>::min()), mp_psink(&psink),
      mp_spsink(0) {
  //  .. nothing yet ..
}

TrapezoidGenerator::TrapezoidGenerator(SimplePolygonSink &spsink)
    : EdgeSink(), m_y(std::numeric_limits<db::Coord>::min()), mp_psink(0),
      mp_spsink(&spsink) {
  //  .. nothing yet ..
}
...
void TrapezoidGenerator::end_scanline(db::Coord y) {
  ...
  for (edge_map_type_iterator e = m_edges.begin(); e != m_edges.end();) {
    ...
    size_t r1 = std::numeric_limits<size_t>::max(),
           r2 = std::numeric_limits<size_t>::max();
    ...
    } else if ((e1->second.p2().y() == y && e2->second.p2().y() < y) ||
               (e2->second.p1().y() == y && e1->second.p1().y() < y)) {

      //  Create trapezoid and continue
      db::Point pts[4];
      pts[0] = e1->second.p1();
      pts[1] = db::Point(
          db::coord_traits<db::Coord>::rounded(db::edge_xaty(e1->first, y)), y);
      pts[2] = db::Point(
          db::coord_traits<db::Coord>::rounded(db::edge_xaty(e2->first, y)), y);
      pts[3] = e2->second.p2();
  ...
  for (edge_map_type_iterator e = m_new_edges.begin();
       e != m_new_edges.end();) {
    ...
    if (e1->second.p1().y() < y && e2->second.p2().y() == y) {
      ...
      pts[1] = db::Point(
          db::coord_traits<db::Coord>::rounded(db::edge_xaty(e1->first, y)), y);
      pts[2] = db::Point(
          db::coord_traits<db::Coord>::rounded(db::edge_xaty(ee->first, y)), y);
      pts[3] = ee->second.p2();
...
void TrapezoidGenerator::crossing_edge(const db::Edge &e) {
  ...
  while (m_current_edge != m_edges.end()) {
    db::Point pc = m_current_edge->second.dy() < 0
                       ? m_current_edge->second.p1()
                       : m_current_edge->second.p2();
    if (pc.y() == m_y && pc.x() <= x) {
      ++m_current_edge;
...
void TrapezoidGenerator::skip_n(size_t n) {
  ...
  while (m_current_edge != m_edges.end() &&
         db::edge_ymax(m_current_edge->second) == m_y) {
    m_new_edge_refs.push_back(std::numeric_limits<size_t>::max());
    ++m_current_edge;
  }
...
void TrapezoidGenerator::put(const db::Edge &e) {
  ...
  //  ignore horizontal edges
  if (e.dy() != 0) {
    //  create a new edge entry: the first edge will be the original (used for
    //  snapping) and the second the working edge (used for trapezoid
    //  generation)
