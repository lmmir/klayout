...
const char *s_reserved_words[] = {
    s_select, s_delete, s_or,    s_of,    s_on,        s_do,     s_from,
    s_layer,  s_layers, s_cell,  s_cells, s_where,     s_shapes, s_polygons,
    s_boxes,  s_edges,  s_paths, s_texts, s_instances, s_arrays, s_sorted,
    s_unique, s_by,     s_with,  s_pass};

bool check_trailing_reserved_word(const tl::Extractor &ex0) {
  tl::Extractor ex = ex0;
  for (size_t i = 0; i < sizeof(s_reserved_words) / sizeof(s_reserved_words[0]);
       ++i) {
...
class DB_PUBLIC FilterSingleState : public FilterStateBase {
public:
  FilterSingleState(const FilterBase *filter, db::Layout *layout,
                    tl::Eval &eval)
  ...
  virtual void next(bool) { m_done = true; }

  virtual bool at_end() { return m_done; }
...
public:
  NameFilterArgument() : m_needs_eval(false) {
    //  .. nothing yet ..
  }
  ...
  bool empty() const { return !m_needs_eval && m_pattern.empty(); }
...
public:
  ...
  bool is_catchall() const { return !m_needs_eval && m_pattern.is_catchall(); }

  bool is_const() const { return !m_needs_eval && m_pattern.is_const(); }

  bool needs_eval() const { return m_needs_eval; }

  const std::string &pattern() const { return m_pattern.pattern(); }
...
struct ShapeFilterPropertyIDs {
  ...
  unsigned int dbbox; // dbbox                -> The shape's bounding box in
                      // micrometer units
  unsigned int shape_bbox;  // shape_bbox           -> == box
  unsigned int shape_dbbox; // shape_dbbox          -> == dbox
  unsigned int shape;       // shape                -> The shape object
  unsigned int
      layer_info; // layer_info           -> The layer (a LayerInfo object)
  unsigned int layer_index; // layer_index          -> The layer index
  unsigned int cell_index;  // cell_index           -> The cell index
};

class DB_PUBLIC ShapeFilterState : public FilterStateBase {
public:
  ShapeFilterState(const FilterBase *filter, const db::LayerMap &layers,
                   db::ShapeIterator::flags_type flags, tl::Eval &eval,
                   db::Layout *layout, bool reading,
                   const ShapeFilterPropertyIDs &pids)
      : FilterStateBase(filter, layout, eval), m_flags(flags), mp_parent(0),
        m_reading(reading), m_pids(pids), m_lindex(0) {
  ...
  virtual bool at_end() { return !mp_parent || m_lindex >= m_layers.size(); }
  ...
  bool get_property(unsigned int id, tl::Variant &v) {
    if (id == m_pids.bbox || id == m_pids.shape_bbox) {
      ...
      v = tl::Variant::make_variant(db::CplxTrans(mp_parent->layout()->dbu()) *
                                    m_shape->bbox());
      return true;
      ...
      v = tl::Variant::make_variant(
          layout()->get_properties(m_layers[m_lindex]));
      return true;
...
class DB_PUBLIC ShapeFilter : public FilterBracket {
public:
  ShapeFilter(LayoutQuery *q, const db::LayerMap &layers,
              db::ShapeIterator::flags_type flags, bool reading)
      : FilterBracket(q), m_pids(q), m_layers(layers), m_flags(flags),
        m_reading(reading) {
    // .. nothing yet ..
  }

  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    return new ShapeFilterState(this, m_layers, m_flags, eval, layout,
                                m_reading, m_pids);
  }
...
struct ChildCellFilterPropertyIDs {
  ChildCellFilterPropertyIDs(LayoutQuery *q,
                             ChildCellFilterInstanceMode instance_mode) {
  ...
  unsigned int path; // path                 -> Variant array with the indexes
                     // of the cells in that path
  unsigned int path_names;   // path                 -> Variant array with the
                             // names of the cells in that path
  unsigned int initial_cell; // initial_cell         -> Initial cell object
                             // (first of path)
  unsigned int initial_cell_index; // initial_cell_index   -> Index of initial
                                   // cell (first of path)
  unsigned int initial_cell_name;  // initial_cell_name    -> Name of initial
                                   // cell (first of path)
  unsigned int cell; // cell                 -> Current cell (last of path)
  unsigned int cell_index; // cell_index           -> Index of current cell
                           // (last of path)
  unsigned int
      cell_name; // cell_name            -> Name of current cell (last of path)
  unsigned int
      parent_cell; // parent_cell          -> Parent cell (next in path) or nil
  unsigned int parent_cell_index; // parent_cell_index    -> Index of parent
                                  // cell (next in path) or nil
  unsigned int parent_cell_name;  // parent_cell_name     -> Name of parent cell
                                  // (next in path) or nil
  unsigned int hier_levels;       // hier_levels          -> Number of hierarchy
                                  // levels in path (length of path - 1)
  unsigned int bbox;              // bbox                 -> Cell bounding box
  unsigned int
      dbbox; // dbbox                -> Cell bounding box in micrometer units
  unsigned int cell_bbox;  // cell_bbox            -> == bbox
  unsigned int cell_dbbox; // cell_dbbox           -> == dbbox

  //  with instance_mode == NoInstances:
  unsigned int references; // references           -> The number of instances
                           // (arefs count as 1) of this cell in the parent cell
  unsigned int weight; // weight               -> The number of instances (arefs
                       // are flattened) of this cell in the parent cell
  unsigned int tot_weight; // tot_weight           -> The number of instances of
                           // this cell in the initial cell along the given path

  //  with instance_mode != NoInstances:
  unsigned int path_trans; // path_trans           -> The transformation of that
                           // instance into the top cell
  unsigned int
      path_dtrans;    // path_dtrans          -> The transformation of that
                      // instance into the top cell in micrometer units
  unsigned int trans; // trans                -> The transformation of that
                      // instance (first instance if an array)
  unsigned int
      dtrans; // dtrans               -> The transformation of that instance
              // (first instance if an array) in micrometer units
  unsigned int inst_bbox;  // inst_bbox            -> The instance bounding box
                           // in the top cell
  unsigned int inst_dbbox; // inst_dbbox           -> The instance bounding box
                           // in the top cell in micrometer units
  unsigned int inst;       // inst                 -> The instance object
  unsigned int
      array_a; // array_a              -> The a vector for an array instance
  unsigned int array_da; // array_da             -> The a vector for an array
                         // instance in micrometer units
  unsigned int array_na; // array_na             -> The a axis array dimension
  unsigned int
      array_b; // array_b              -> The b vector for an array instance
  unsigned int array_db; // array_db             -> The b vector for an array
                         // instance in micrometer units
  unsigned int array_nb; // array_nb             -> The b axis array dimension

  //  with instance_mode == ExplodedInstances:
  unsigned int
      array_ia; // array_ia             -> The a index when an array is iterated
  unsigned int
      array_ib; // array_ib             -> The b index when an array is iterated
};

class DB_PUBLIC ChildCellFilterState : public FilterStateBase {
public:
  ChildCellFilterState(const FilterBase *filter,
                       const NameFilterArgument &pattern,
                       ChildCellFilterInstanceMode instance_mode,
                       tl::Eval &eval, db::Layout *layout, bool reading,
                       const ChildCellFilterPropertyIDs &pids)
      : FilterStateBase(filter, layout, eval), m_pattern(pattern, eval),
        m_instance_mode(instance_mode), mp_parent(0), m_pids(pids), m_weight(0),
        m_references(0), m_weight_set(false), m_references_set(false),
        m_reading(reading),
  ...
  virtual void do_init() {
    if (m_pattern.is_catchall() || m_pattern.needs_eval()) {
      ...
      if (!objectives().wants_all_cells()) {
        ...
        //  wildcard or unknown filter: include the parent cells if specific
        //  child cells are looked for

        int levels = 1;
        for (size_t i = 0; i < followers().size(); ++i) {
          if (followers()[i] == 0) {
            //  this is a sign of recursion - collect caller cells from all
            //  levels.
            levels = -1;
          }
        }

        //  this means, one follower wants only certain cells. We can optimize
        //  by only checking for potential parents
        std::set<db::cell_index_type> callers;
        for (FilterStateObjectives::cell_iterator c =
                 objectives().begin_cells();
             c != objectives().end_cells(); ++c) {
          layout()->cell(*c).collect_caller_cells(callers, levels);
        }

        for (std::set<db::cell_index_type>::const_iterator c = callers.begin();
             c != callers.end(); ++c) {
  ...
  virtual void reset(FilterStateBase *previous) {
    ...
    tl::Variant parent_id;
    if (FilterStateBase::get_property(m_pids.cell_index, parent_id)) {
      if (layout()->is_valid_cell_index(
              db::cell_index_type(parent_id.to_ulong()))) {
        mp_parent = &layout()->cell(db::cell_index_type(parent_id.to_ulong()));
      }
    }
    ...
    if (!mp_parent) {
      ...
      while (m_top_cell != m_top_cell_end &&
             (!layout()->is_valid_cell_index(*m_top_cell) ||
              !cell_matches(*m_top_cell))) {
        ++m_top_cell;
      }
      ...
      if (m_instance_mode == NoInstances) {
        ...
        while (!m_child_cell.at_end() &&
               (!layout()->is_valid_cell_index(*m_child_cell) ||
                !cell_matches(*m_child_cell))) {
          ++m_child_cell;
        }
        ...
        while (m_inst != m_inst_end) {
          ...
          ++m_inst;
          while (m_inst != m_inst_end &&
                 (*m_inst)->object().cell_index() == cid) {
            ++m_inst;
          }
        }

        if (m_inst != m_inst_end && !m_reading) {
          m_i = mp_parent->sorted_inst_ptr(
              std::distance(mp_parent->begin_sorted_insts(), m_inst));
        }
    ...
    if (mp_parent) {
      ...
      if (m_instance_mode == NoInstances) {
        ...
        if (m_instance_mode != ExplodedInstances || m_array_iter.at_end()) {
          ...
          if (!m_reading &&
              m_i != mp_parent->sorted_inst_ptr(std::distance(
                         mp_parent->begin_sorted_insts(), m_inst))) {
  ...
  bool get_property(unsigned int id, tl::Variant &v) {
    if (id == m_pids.bbox || id == m_pids.cell_bbox) {
      ...
      if (!layout()->is_valid_cell_index(cell_index())) {
        ...
        if (m_reading) {
          v = tl::Variant::make_variant_ref(
              (const db::Cell *)&layout()->cell(cell_index()));
        } else {
          v = tl::Variant::make_variant_ref(
              (db::Cell *)&layout()->cell(cell_index()));
        }
      }
      return true;
      ...
      if (mp_parent) {
        ...
        if (m_instance_mode == ExplodedInstances) {
          ...
          db::Box box(t *
                      layout()->cell((*m_inst)->object().cell_index()).bbox());
      ...
      if (mp_parent) {
        ...
        if (m_instance_mode == ExplodedInstances) {
          ...
          db::ICplxTrans t = m_parent_trans;
          t *= (*m_inst)->complex_trans(*m_array_iter);
          db::DBox box(db::CplxTrans(layout()->dbu()) * t *
                       layout()->cell((*m_inst)->object().cell_index()).bbox());
          v = tl::Variant::make_variant(box);
          return true;
      ...
      if (mp_parent) {
        ...
        if (m_instance_mode == ExplodedInstances) {
          ...
          v = tl::Variant::make_variant(
              (*m_inst)->complex_trans(*m_array_iter));
          return true;
      ...
      if (mp_parent) {
        ...
        if (m_instance_mode == ExplodedInstances) {
          ...
          db::CplxTrans tdbu(layout()->dbu());
          v = tl::Variant::make_variant(
              tdbu * (*m_inst)->complex_trans(*m_array_iter) * tdbu.inverted());
          return true;

        } else if (m_instance_mode == ArrayInstances) {

          db::CplxTrans tdbu(layout()->dbu());
          v = tl::Variant::make_variant(tdbu * (*m_inst)->complex_trans() *
                                        tdbu.inverted());
          return true;
    ...
    } else if (id == m_pids.array_a || id == m_pids.array_b ||
               id == m_pids.array_da || id == m_pids.array_db ||
               id == m_pids.array_na || id == m_pids.array_nb) {
...
class DB_PUBLIC ChildCellFilter : public FilterBracket {
public:
  ChildCellFilter(LayoutQuery *q, const NameFilterArgument &pattern,
                  ChildCellFilterInstanceMode instance_mode, bool reading)
      : FilterBracket(q), m_pids(q, instance_mode), m_pattern(pattern),
        m_instance_mode(instance_mode), m_reading(reading) {
    // .. nothing yet ..
  }

  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    return new ChildCellFilterState(this, m_pattern, m_instance_mode, eval,
                                    layout, m_reading, m_pids);
  }
  ...
  virtual void dump(unsigned int l) const {
    ...
    std::cout << "ChildCellFilter (" << m_pattern.pattern() << ", "
              << (int)m_instance_mode << ") :" << std::endl;
    FilterBracket::dump(l + 1);
  }
...
struct CellFilterPropertyIDs {
  ...
  unsigned int path; // path                 -> Variant array with the indexes
                     // of the cells in that path
  unsigned int path_names;   // path_names           -> Variant array with the
                             // names of the cells in that path
  unsigned int initial_cell; // initial_cell         -> Pointer to initial cell
                             // (first of path)
  unsigned int initial_cell_index; // initial_cell_index   -> Index of initial
                                   // cell (first of path)
  unsigned int
      initial_cell_name; // initial_cell_name    -> Name of initial cell (first
                         // of path)
                         //                         The path is a dummy path
                         //                         consisting of the cell as a
                         //                         single element. It is
                         //                         provided to allow using the
                         //                         CellFilter as the front of a
                         //                         instantiation path. Also,
                         //                         the initial cell is
                         //                         identical to cell.
  unsigned int
      cell; // cell                 -> Pointer to current cell (last of path)
  unsigned int cell_index; // cell_index           -> Index of current cell
                           // (last of path)
  unsigned int
      cell_name; // cell_name            -> Name of current cell (last of path)
  ...
  unsigned int tot_weight; // tot_weight           -> The number of instances of
                           // this cell in the initial cell along the given path
  unsigned int instances;  // instances            -> The number of instances of
                           // this cell in the previous cell (or over all if
  ...
  unsigned int cell_dbbox; // cell_dbbox           -> == dbbox
  unsigned int
      path_trans; // parent_trans         -> transformation to initial cell
  unsigned int path_dtrans; // parent_dtrans        -> transformation to initial
                            // cell in micrometer units
};

class DB_PUBLIC CellFilterState : public FilterStateBase {
public:
  CellFilterState(const FilterBase *filter, const NameFilterArgument &pattern,
                  tl::Eval &eval, db::Layout *layout, bool reading,
                  const CellFilterPropertyIDs &pids)
      : FilterStateBase(filter, layout, eval), m_pids(pids),
        m_pattern(pattern, eval), mp_parent(0), m_reading(reading),
        m_cell_index(std::numeric_limits<db::cell_index_type>::max()) {
    //  .. nothing yet ..
  }
  ...
  virtual bool at_end() { return m_cell == m_cell_end; }
  ...
  bool get_property(unsigned int id, tl::Variant &v) {
    if (id == m_pids.bbox || id == m_pids.cell_bbox) {
      ...
      if (m_reading) {
        v = tl::Variant::make_variant_ref(
            (const db::Cell *)&layout()->cell(*m_cell));
      } else {
    ...
    } else if (id == m_pids.references || id == m_pids.weight ||
               id == m_pids.tot_weight) {
      ...
      if (!m_cell_counter.get()) {
        if (mp_parent) {
          m_cell_counter.reset(
              new db::CellCounter(layout(), mp_parent->cell_index()));
        } else {
...
class DB_PUBLIC CellFilter : public FilterBracket {
public:
  CellFilter(LayoutQuery *q, const NameFilterArgument &pattern, bool reading)
      : FilterBracket(q), m_pids(q), m_pattern(pattern), m_reading(reading) {
    // .. nothing yet ..
  }

  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    return new CellFilterState(this, m_pattern, eval, layout, m_reading,
                               m_pids);
  }
  ...
  NameFilterArgument &name_filter() { return m_pattern; }
...
class DB_PUBLIC DeleteFilterState : public FilterStateBase {
public:
  DeleteFilterState(const FilterBase *filter, tl::Eval &eval,
                    db::Layout *layout, const DeleteFilterPropertyIDs &pids,
                    bool transparent)
      : FilterStateBase(filter, layout, eval), m_pids(pids),
        m_transparent(transparent), m_count(0) {
    //  .. nothing yet ..
  }
  ...
  virtual bool at_end() { return !m_transparent || m_count > 0; }
...
class DB_PUBLIC DeleteFilter : public FilterBracket {
public:
  ...
  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    if (!layout->is_editable()) {
      throw tl::Exception(tl::to_string(
          tr("Cannot execute a delete query on a non-editable layout")));
    }
...
class DB_PUBLIC WithDoFilterState : public FilterStateBase {
public:
  WithDoFilterState(const FilterBase *filter, const std::string &do_expression,
                    tl::Eval &eval, db::Layout *layout, bool transparent)
      : FilterStateBase(filter, layout, eval), m_transparent(transparent),
        m_count(0) {
...
class DB_PUBLIC WithDoFilter : public FilterBracket {
public:
  WithDoFilter(LayoutQuery *q, const std::string &do_expression,
               bool transparent)
      : FilterBracket(q), m_do_expression(do_expression),
  ...
  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    if (!layout->is_editable()) {
      throw tl::Exception(tl::to_string(
          tr("Cannot execute a with .. do query on a non-editable layout")));
    }
    return new WithDoFilterState(this, m_do_expression, eval, layout,
                                 m_transparent);
  }
...
class DB_PUBLIC SelectFilterReportingState : public FilterStateBase {
public:
  SelectFilterReportingState(const FilterBase *filter, tl::Eval &eval,
                             db::Layout *layout, bool unique,
                             const SelectFilterPropertyIDs &pids)
  ...
  virtual void next(bool) {
    ...
    do {
      ++m_sorted;
    } while (m_unique && m_sorted != m_sorted_data.end() &&
             m_sorted->second == s->second);
  }

  virtual bool at_end() { return (m_sorted == m_sorted_data.end()); }
...
class DB_PUBLIC SelectFilterState : public FilterStateBase {
public:
  SelectFilterState(const FilterBase *filter,
                    const std::vector<std::string> &expressions,
                    const std::string &sort_expression, bool unique,
                    tl::Eval &eval, db::Layout *layout,
                    const SelectFilterPropertyIDs &pids)
      : FilterStateBase(filter, layout, eval), m_pids(pids),
        m_has_sorting(false), m_unique(unique), m_done(false),
        m_in_data_eval(false), mp_reporter_state(0) {
    for (std::vector<std::string>::const_iterator e = expressions.begin();
         e != expressions.end(); ++e) {
  ...
  void get_data(tl::Variant &v) {
    ...
    try {
      ...
      std::vector<tl::Variant> vd;
      v = tl::Variant(vd.begin(), vd.end());
      for (std::vector<tl::Expression>::const_iterator e =
               m_expressions.begin();
           e != m_expressions.end(); ++e) {
        v.push(e->execute());
      }
  ...
  virtual void reset(FilterStateBase *previous) {
    if (m_has_sorting) {
      ...
      if (!mp_reporter_state) {
        ...
        //  Install the reporter state at top level - this will finally report
        //  all results
        mp_reporter_state = new SelectFilterReportingState(
            filter(), eval(), layout(), m_unique, m_pids);
...
class DB_PUBLIC SelectFilter : public FilterBracket {
public:
  SelectFilter(LayoutQuery *q, const std::vector<std::string> &expressions,
               const std::string &sort_expression, bool unique)
      : FilterBracket(q), m_pids(q), m_expressions(expressions),
        m_sort_expression(sort_expression), m_unique(unique) {
    // .. nothing yet ..
  }

  FilterStateBase *do_create_state(db::Layout *layout, tl::Eval &eval) const {
    return new SelectFilterState(this, m_expressions, m_sort_expression,
                                 m_unique, eval, layout, m_pids);
  }
...
class DB_PUBLIC ConditionalFilterState : public FilterStateBase {
public:
  ConditionalFilterState(const FilterBase *filter, tl::Eval &eval,
                         const std::string &expr, db::Layout *layout)
  ...
  virtual void next(bool) { m_select = false; }

  virtual bool at_end() { return !m_select; }
...
class DB_PUBLIC ConditionalFilter : public FilterBracket {
public:
...
// --------------------------------------------------------------------------------
//  FilterStateFunction adaptor class (creates a function for a tl::Eval object
//  that gets a property)

class FilterStateFunction : public tl::EvalFunction {
public:
  FilterStateFunction(unsigned int prop_id,
                      const std::vector<FilterStateBase *> *states)
      : m_prop_id(prop_id), mp_states(states) {
    //  .. nothing yet ..
  }

  void execute(const tl::ExpressionParserContext &context, tl::Variant &out,
               const std::vector<tl::Variant> &args) const {
    if (args.size() > 0) {
      throw tl::EvalError(
          tl::to_string(tr("Query function does not allow parameters")),
          context);
    }
...
LayoutQueryIterator::LayoutQueryIterator(const LayoutQuery &q,
                                         db::Layout *layout,
                                         tl::Eval *parent_eval,
                                         tl::AbsoluteProgress *progress)
    : mp_q(const_cast<db::LayoutQuery *>(&q)), mp_layout(layout),
      m_eval(parent_eval), m_layout_ctx(layout, true /*can modify*/),
      mp_progress(progress), m_initialized(false) {
  ...
  for (unsigned int i = 0; i < mp_q->properties(); ++i) {
    m_eval.define_function(mp_q->property_name(i),
                           new FilterStateFunction(i, &m_state));
  }
...
LayoutQueryIterator::LayoutQueryIterator(const LayoutQuery &q,
                                         ...
                                         tl::Eval *parent_eval,
                                         tl::AbsoluteProgress *progress)
    : mp_q(const_cast<db::LayoutQuery *>(&q)),
      mp_layout(const_cast<db::Layout *>(layout)), m_eval(parent_eval),
      m_layout_ctx(layout), mp_progress(progress), m_initialized(false) {
...
void LayoutQueryIterator::ensure_initialized() {
  if (!m_initialized) {
    init();
    m_initialized = true;
  }
...
void LayoutQueryIterator::cleanup() {
  std::set<FilterStateBase *> states;
  collect(mp_root_state, states);
  for (std::set<FilterStateBase *>::iterator s = states.begin();
       s != states.end(); ++s) {
    delete *s;
  }
  m_state.clear();
...
void LayoutQueryIterator::reset() {
  if (m_initialized) {
...
bool LayoutQueryIterator::at_end() const {
  const_cast<LayoutQueryIterator *>(this)->ensure_initialized();
  return m_state.empty();
}

bool LayoutQueryIterator::get(const std::string &name, tl::Variant &v) {
  ensure_initialized();
...
bool LayoutQueryIterator::get(unsigned int id, tl::Variant &v) {
  ensure_initialized();
...
void LayoutQueryIterator::dump() const {
  const_cast<LayoutQueryIterator *>(this)->ensure_initialized();
  mp_root_state->dump();
  std::cout << std::endl;
}

void LayoutQueryIterator::collect(FilterStateBase *state,
                                  std::set<FilterStateBase *> &states) {
  if (states.find(state) == states.end()) {
    states.insert(state);
    for (std::vector<FilterStateBase *>::const_iterator s =
             state->followers().begin();
         s != state->followers().end(); ++s) {
      if (*s) {
        collect(*s, states);
      }
    }
...
void LayoutQueryIterator::next(bool skip) {
  ensure_initialized();
  do {
    next_up(skip);
  } while (!next_down());
...
void LayoutQueryIterator::next_up(bool skip) {
  while (!m_state.empty()) {
    if (mp_progress) {
      ++*mp_progress;
    }
...
bool LayoutQueryIterator::next_down() {
  if (!m_state.empty()) {
...
static void parse_cell_name_filter_seq(
    tl::Extractor &ex, LayoutQuery *q, FilterBracket *bracket,
    ChildCellFilterInstanceMode instance_mode, bool reading);

static FilterBase *
parse_cell_name_filter_element(tl::Extractor &ex, LayoutQuery *q,
                               ChildCellFilterInstanceMode instance_mode,
                               bool reading) {
  tl::Extractor ex0 = ex;
  ...
  if (ex.test(")") || ex.test(",")) {
    ...
    if (b->children().size() == 1 &&
        dynamic_cast<FilterBracket *>(b->children()[0])) {
      FilterBracket *binner =
          dynamic_cast<FilterBracket *>(b->children()[0]->clone(q));
      if (binner) {
        b.reset(binner);
      }
    }
    ...
    std::unique_ptr<FilterBracket> b(new ChildCellFilter(
        q, NameFilterArgument("*"), instance_mode, reading));
    b->set_loopmin(0);
    b->set_loopmax(std::numeric_limits<unsigned int>::max());
    return b.release();
...
static void parse_cell_name_filter_seq(
  ...
  while (!ex.at_end()) {
    ...
    if (check_trailing_reserved_word(ex) ||
        (f = parse_cell_name_filter_element(ex, q, instance_mode, reading)) ==
            0) {
      break;
    }
  ...
  CellFilter *cf;
  if (instance_mode != NoInstances && f0 == fl &&
      (cf = dynamic_cast<CellFilter *>(f0)) != 0) {
...
void parse_cell_filter(tl::Extractor &ex, LayoutQuery *q,
                       FilterBracket *bracket, bool with_where_clause,
                       bool reading) {
  if (ex.test("(")) {
    ...
    if (ex.test(s_instances)) {
      ...
      //  Because an array member cannot be modified we use ArrayInstances in
      //  the modification case always
      parse_cell_name_filter_seq(ex, q, b.get(),
                                 reading ? ExplodedInstances : ArrayInstances,
                                 reading);
...
void parse_filter(tl::Extractor &ex, LayoutQuery *q, FilterBracket *bracket,
                  bool reading) {
...
void parse_statement(tl::Extractor &ex, LayoutQuery *q, FilterBracket *bracket,
                     bool reading) {
  if (ex.test(s_select)) {
...
LayoutQuery::LayoutQuery(const std::string &query) : mp_root(0) {
...
void LayoutQuery::dump() const { mp_root->dump(0); }

void LayoutQuery::execute(db::Layout &layout, tl::Eval *context) {
...
unsigned int LayoutQuery::register_property(const std::string &name,
                                            LayoutQueryPropertyType type) {
  std::map<std::string, unsigned int>::const_iterator p =
      m_property_ids_by_name.find(name);
...
const std::string &LayoutQuery::property_name(unsigned int index) const {
  tl_assert(index < properties());
  return m_properties[index].name;
}

LayoutQueryPropertyType LayoutQuery::property_type(unsigned int index) const {
  tl_assert(index < properties());
  return m_properties[index].type;
}

unsigned int LayoutQuery::property_by_name(const std::string &name) const {
  std::map<std::string, unsigned int>::const_iterator p =
      m_property_ids_by_name.find(name);
...
bool LayoutQuery::has_property(const std::string &name) const {
  std::map<std::string, unsigned int>::const_iterator p =
      m_property_ids_by_name.find(name);
...
FilterBase::FilterBase(LayoutQuery *q) : mp_q(q) {
  // .. nothing yet ..
}

FilterStateBase *
FilterBase::create_state(const std::vector<FilterStateBase *> &followers,
                         db::Layout *layout, tl::Eval &eval,
                         bool /*single*/) const {
...
void FilterBase::connect(FilterBase *follower) {
  m_followers.push_back(follower);
}

void FilterBase::connect(const std::vector<FilterBase *> &followers) {
  m_followers.insert(m_followers.end(), followers.begin(), followers.end());
}

FilterBase *FilterBase::clone(LayoutQuery *q) const {
  return new FilterBase(q);
}

FilterStateBase *FilterBase::do_create_state(db::Layout *layout,
                                             tl::Eval &eval) const {
  return new FilterSingleState(this, layout, eval);
}

unsigned int FilterBase::register_property(const std::string &name,
                                           LayoutQueryPropertyType type) {
  return mp_q->register_property(name, type);
}

void FilterBase::dump(unsigned int l) const {
  for (unsigned int i = 0; i < l; ++i) {
    std::cout << "  ";
  }
...
FilterBracket::FilterBracket(LayoutQuery *q, unsigned int loopmin,
                             unsigned int loopmax)
    : FilterBase(q), m_initial(q), m_closure(q), m_loopmin(loopmin),
      m_loopmax(loopmax) {
  tl_assert(loopmin <= loopmax);
}

FilterBracket::~FilterBracket() {
  for (std::vector<FilterBase *>::const_iterator c = m_children.begin();
       c != m_children.end(); ++c) {
    delete *c;
  }
  m_children.clear();
...
FilterStateBase *
FilterBracket::create_state(const std::vector<FilterStateBase *> &followers,
                            db::Layout *layout, tl::Eval &eval,
                            bool single) const {
  bool greedy = false; // TODO: make variable. In greedy mode (true), the
                       // children are reported before the nodes, hence the
                       // longest match is reported first.
  ...
  if ((m_loopmin == 1 && m_loopmax == 1) || single) {
    ...
    for (int l = int(m_loopmax == std::numeric_limits<unsigned int>::max()
                         ? m_loopmin
                         : m_loopmax);
         l >= 0; --l) {
...
const std::vector<FilterBase *> &FilterBracket::children() const {
  return m_children;
}

void FilterBracket::add_child(FilterBase *follower) {
...
void FilterBracket::connect_entry(FilterBase *child) {
  m_initial.connect(child);
}

void FilterBracket::connect_exit(FilterBase *child) {
  child->connect(&m_closure);
}

FilterBase *FilterBracket::clone(LayoutQuery *q) const {
  ...
  for (std::vector<FilterBase *>::const_iterator o =
           m_initial.followers().begin();
       o != m_initial.followers().end(); ++o) {
    std::map<const FilterBase *, FilterBase *>::const_iterator f =
        fmap.find(*o);
    if (f != fmap.end()) {
  ...
  for (std::vector<FilterBase *>::const_iterator c = m_children.begin();
       c != m_children.end(); ++c) {
    std::map<const FilterBase *, FilterBase *>::const_iterator fc =
        fmap.find(*c);
    for (std::vector<FilterBase *>::const_iterator o =
             (*c)->followers().begin();
         o != (*c)->followers().end(); ++o) {
...
FilterStateBase *FilterBracket::create_state_helper(
    std::map<const FilterBase *, FilterStateBase *> &fmap,
    const FilterBase *child, FilterStateBase *closure_state, db::Layout *layout,
    tl::Eval &eval) const {
  std::vector<FilterStateBase *> followers;
  followers.reserve(child->followers().size());

  for (std::vector<FilterBase *>::const_iterator o = child->followers().begin();
       o != child->followers().end(); ++o) {
    if (*o == &m_closure) {
      followers.push_back(closure_state);
    } else {
      std::map<const FilterBase *, FilterStateBase *>::const_iterator f =
          fmap.find(*o);
      if (f != fmap.end()) {
        followers.push_back(f->second);
      } else {
        FilterStateBase *fs =
            create_state_helper(fmap, *o, closure_state, layout, eval);
...
void FilterBracket::dump(unsigned int l) const {
  if (!m_children.empty()) {
    ...
    while (!todo.empty()) {
      ...
      for (std::vector<FilterBase *>::const_iterator c = f->followers().begin();
           c != f->followers().end(); ++c) {
    ...
    for (std::vector<const FilterBase *>::const_iterator f = filters.begin();
         f != filters.end(); ++f) {
      ...
      (*f)->dump(l + 1);
      for (std::vector<FilterBase *>::const_iterator c =
               (*f)->followers().begin();
           c != (*f)->followers().end(); ++c) {
        for (unsigned int i = 0; i < l + 1; ++i) {
          std::cout << "  ";
...
FilterStateObjectives::FilterStateObjectives() : m_wants_all_cells(false) {
  //  .. nothing yet ..
}
...
FilterStateObjectives::operator+=(const FilterStateObjectives &other) {
  if (!m_wants_all_cells) {
    ...
    if (!m_wants_all_cells) {
      m_wants_cells.insert(other.m_wants_cells.begin(),
                           other.m_wants_cells.end());
    }
  }
...
void FilterStateObjectives::set_wants_all_cells(bool f) {
  m_wants_cells.clear();
  m_wants_all_cells = f;
}

void FilterStateObjectives::request_cell(db::cell_index_type ci) {
  if (!m_wants_all_cells) {
    m_wants_cells.insert(ci);
  }
}

bool FilterStateObjectives::wants_cell(db::cell_index_type ci) const {
...
FilterStateBase::FilterStateBase(const FilterBase *filter, db::Layout *layout,
                                 tl::Eval &eval)
    : mp_previous(0), mp_filter(filter), mp_layout(layout), m_follower(0),
      mp_eval(&eval) {}

void FilterStateBase::init(bool recursive) {
  if (m_followers.empty()) {
    ...
    for (std::vector<FilterStateBase *>::const_iterator f = m_followers.begin();
         f != m_followers.end(); ++f) {
      if (*f) {
        if (recursive) {
          (*f)->init();
...
void FilterStateBase::do_init() {
  //  .. nothing yet ..
}

void FilterStateBase::dump() const {
  std::cout << "[";
...
FilterStateBase *FilterStateBase::child() const {
  if (m_followers.empty()) {
    return 0;
  } else {
    ...
    if (!b && mp_filter && mp_layout) {
      //  dynamically create a new recursive state execution graph snippet if
      //  required
...
void FilterStateBase::connect(FilterStateBase *follower) {
  m_followers.push_back(follower);
}

void FilterStateBase::connect(const std::vector<FilterStateBase *> &followers) {
  m_followers.insert(m_followers.end(), followers.begin(), followers.end());
}

void FilterStateBase::proceed(bool skip) {
  if (m_followers.empty()) {
    next(skip);
  } else {
    ++m_follower;
