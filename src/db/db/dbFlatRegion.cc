...
FlatRegion::FlatRegion()
    : MutableRegion(), mp_polygons(new db::Shapes(false)),
      mp_merged_polygons(new db::Shapes(false)),
      mp_properties_repository(new db::PropertiesRepository()) {
  init();
}

FlatRegion::FlatRegion(const FlatRegion &other)
    : MutableRegion(other), mp_polygons(other.mp_polygons),
      mp_merged_polygons(other.mp_merged_polygons),
...
FlatRegion::FlatRegion(const db::Shapes &polygons, bool is_merged)
    : MutableRegion(), mp_polygons(new db::Shapes(polygons)),
      mp_merged_polygons(new db::Shapes(false)),
      mp_properties_repository(new db::PropertiesRepository()) {
  init();
...
void FlatRegion::set_is_merged(bool m) { m_is_merged = m; }
...
FlatRegion::begin_iter() const {
  return std::make_pair(db::RecursiveShapeIterator(*mp_polygons),
                        db::ICplxTrans());
}

std::pair<db::RecursiveShapeIterator, db::ICplxTrans>
FlatRegion::begin_merged_iter() const {
  if (!merged_semantics() || m_is_merged) {
    return begin_iter();
  } else {
    ensure_merged_polygons_valid();
    return std::make_pair(db::RecursiveShapeIterator(*mp_merged_polygons),
                          db::ICplxTrans());
  }
}

bool FlatRegion::empty() const { return mp_polygons->empty(); }

size_t FlatRegion::count() const { return mp_polygons->size(); }

size_t FlatRegion::hier_count() const { return mp_polygons->size(); }

bool FlatRegion::is_merged() const { return m_is_merged; }

Box FlatRegion::compute_bbox() const { return mp_polygons->bbox(); }

RegionDelegate *FlatRegion::filter_in_place(const PolygonFilterBase &filter) {
  db::layer<db::Polygon, db::unstable_layer_tag> &poly_layer =
      mp_polygons->get_layer<db::Polygon, db::unstable_layer_tag>();
  db::layer<db::PolygonWithProperties, db::unstable_layer_tag> &poly_layer_wp =
      mp_polygons
          ->get_layer<db::PolygonWithProperties, db::unstable_layer_tag>();
    ...
    if (filter.selected(*p)) {
      if (p.prop_id() != 0) {
        if (pw_wp == poly_layer_wp.end()) {
          ...
          poly_layer_wp.replace(pw_wp++,
                                db::PolygonWithProperties(*p, p.prop_id()));
        }
      } else {
...
RegionDelegate *
FlatRegion::process_in_place(const PolygonProcessorBase &filter) {
  db::layer<db::Polygon, db::unstable_layer_tag> &poly_layer =
      mp_polygons->get_layer<db::Polygon, db::unstable_layer_tag>();
...
RegionDelegate *FlatRegion::merged_in_place(bool min_coherence,
                                            unsigned int min_wc) {
  if (empty()) {
...
RegionDelegate *FlatRegion::add(const Region &other) const {
  ...
  if (other_flat) {
    ...
    new_region->raw_polygons().insert(
        other_flat->raw_polygons()
            .get_layer<db::Polygon, db::unstable_layer_tag>()
            .begin(),
        other_flat->raw_polygons()
            .get_layer<db::Polygon, db::unstable_layer_tag>()
            .end());
...
RegionDelegate *FlatRegion::add_in_place(const Region &other) {
  ...
  if (other_flat) {
    ...
    polygons.insert(other_flat->raw_polygons()
                        .get_layer<db::Polygon, db::unstable_layer_tag>()
                        .begin(),
                    other_flat->raw_polygons()
                        .get_layer<db::Polygon, db::unstable_layer_tag>()
...
const db::Polygon *FlatRegion::nth(size_t n) const {
  //  NOTE: this assumes that we iterate over non-property polygons first and
  //  then over polygons with properties

  if (n >= mp_polygons->size()) {
    return 0;
  }

  const db::layer<db::Polygon, db::unstable_layer_tag> &l =
      mp_polygons->get_layer<db::Polygon, db::unstable_layer_tag>();
  if (n < l.size()) {
    return &l.begin()[n];
  }
  ...
  const db::layer<db::PolygonWithProperties, db::unstable_layer_tag> &lp =
      mp_polygons
          ->get_layer<db::PolygonWithProperties, db::unstable_layer_tag>();
  if (n < lp.size()) {
    return &lp.begin()[n];
...
bool FlatRegion::has_valid_polygons() const { return true; }

bool FlatRegion::has_valid_merged_polygons() const { return true; }

const db::RecursiveShapeIterator *FlatRegion::iter() const { return 0; }
...
void FlatRegion::insert_into(Layout *layout, db::cell_index_type into_cell,
...
void FlatRegion::do_insert(const db::Polygon &polygon,
                           properties_id_type prop_id) {
