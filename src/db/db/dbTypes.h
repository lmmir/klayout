...
struct generic_coord_traits {
   ...
   *
   *  This is a special type mainly used to represent "short" boxes (i.e. small
   * ones) with a small memory footprint. It is used mainly for 32bit
   * coordinates and mask data.
   */
   ...
   *  Specialization for double: Converts the given double to the coordinate
   * type.
   */
  static coord_type rounded(double v) {
    return coord_type(v > 0 ? v + 0.5 : v - 0.5);
  }

  /**
   *  @brief The rounding method
   *  Specialization for float: Converts the given double to the coordinate
   * type.
   */
  static coord_type rounded(float v) {
    return coord_type(v > 0 ? v + 0.5 : v - 0.5);
  }
   ...
   */
  template <class X> static coord_type rounded(X v) { return coord_type(v); }
  ...
  static distance_type rounded_distance(double v) {
    return distance_type(v > 0 ? v + 0.5 : v - 0.5);
  }

  /**
   *  @brief The rounding method for perimeters
   */
  static perimeter_type rounded_perimeter(double v) {
    return perimeter_type(v > 0 ? v + 0.5 : v - 0.5);
  }

  /**
   *  @brief (Fuzzy) equality of coordinates
   */
  static bool equal(coord_type c1, coord_type c2) { return c1 == c2; }

  /**
   *  @brief (Fuzzy) less comparison of coordinates
   */
  static bool less(coord_type c1, coord_type c2) { return c1 < c2; }
   ...
   */
  static area_type sq_length(coord_type ax, coord_type ay, coord_type bx,
                             coord_type by) {
    return ((area_type)ax - (area_type)bx) * ((area_type)ax - (area_type)bx) +
           ((area_type)ay - (area_type)by) * ((area_type)ay - (area_type)by);
  }
   ...
   */
  static area_type sprod(coord_type ax, coord_type ay, coord_type bx,
                         coord_type by, coord_type cx, coord_type cy) {
    return ((area_type)ax - (area_type)cx) * ((area_type)bx - (area_type)cx) +
           ((area_type)ay - (area_type)cy) * ((area_type)by - (area_type)cy);
  }
   ...
   *  0: zero, +1: positive)
   */
  static int sprod_sign(coord_type ax, coord_type ay, coord_type bx,
                        coord_type by, coord_type cx, coord_type cy) {
    area_type p1 =
        ((area_type)ax - (area_type)cx) * ((area_type)bx - (area_type)cx);
    area_type p2 =
        -(((area_type)ay - (area_type)cy) * ((area_type)by - (area_type)cy));
    if (p1 > p2) {
      return 1;
   ...
   */
  static area_type vprod(coord_type ax, coord_type ay, coord_type bx,
                         coord_type by, coord_type cx, coord_type cy) {
    return ((area_type)ax - (area_type)cx) * ((area_type)by - (area_type)cy) -
           ((area_type)ay - (area_type)cy) * ((area_type)bx - (area_type)cx);
  }
   ...
   *  0: zero, +1: positive)
   */
  static int vprod_sign(coord_type ax, coord_type ay, coord_type bx,
                        coord_type by, coord_type cx, coord_type cy) {
    area_type p1 =
        ((area_type)ax - (area_type)cx) * ((area_type)by - (area_type)cy);
    area_type p2 =
        ((area_type)ay - (area_type)cy) * ((area_type)bx - (area_type)cx);
    if (p1 > p2) {
      return 1;
 ...
 */
template <class C> struct coord_traits {};
...
struct coord_traits<int32_t>
    : public generic_coord_traits<int32_t, int64_t /*area*/, uint32_t /*dist*/,
                                  uint64_t /*perimeter*/, int16_t /*short*/> {};
...
struct coord_traits<int16_t>
    : public generic_coord_traits<int16_t, int32_t /*area*/, uint32_t /*dist*/,
                                  uint32_t /*perimeter*/, int16_t /*short*/> {};
...
struct coord_traits<int64_t>
    : public generic_coord_traits<int64_t, __int128 /*area*/, uint64_t /*dist*/,
                                  uint64_t /*perimeter*/, int32_t /*short*/> {};
#endif
 ...
 */
template <> struct coord_traits<double> {
  ...
  template <class X> static double rounded(X v) { return double(v); }
  ...
  static int sprod_sign(double ax, double ay, double bx, double by, double cx,
                        double cy) {
    double dx1 = ax - cx, dy1 = ay - cy;
    double dx2 = bx - cx, dy2 = by - cy;
    double pa =
        (sqrt(dx1 * dx1 + dy1 * dy1) + sqrt(dx2 * dx2 + dy2 * dy2)) * prec();
  ...
  static std::pair<area_type, int> sprod_with_sign(double ax, double ay,
                                                   double bx, double by,
                                                   double cx, double cy) {
    double dx1 = ax - cx, dy1 = ay - cy;
    double dx2 = bx - cx, dy2 = by - cy;
  ...
  static int vprod_sign(double ax, double ay, double bx, double by, double cx,
                        double cy) {
    double dx1 = ax - cx, dy1 = ay - cy;
    double dx2 = bx - cx, dy2 = by - cy;
    double pa =
  ...
  static std::pair<area_type, int> vprod_with_sign(double ax, double ay,
                                                   double bx, double by,
                                                   double cx, double cy) {
    double dx1 = ax - cx, dy1 = ay - cy;
    double dx2 = bx - cx, dy2 = by - cy;
  ...
  static bool equal(double c1, double c2) { return fabs(c1 - c2) < prec(); }

  static bool less(double c1, double c2) { return c1 < c2 - prec() * 0.5; }
 ...
 */
template <class D, class C> struct coord_converter {
  D operator()(C c) const { return coord_traits<D>::rounded(c); }
};

/**
 *  @brief A very generic cast operator from T to U
 */
template <class U, class T> struct cast_op {
  U operator()(const T &t) const { return U(t); }
};

/**
 *  @brief A generic constant describing the "fuzzyness" of a double comparison
 * of a value around 1
 */
const double epsilon = 1e-10;

/**
 *  @brief A generic constant describing the "fuzzyness" of a float comparison
 * of a value around 1
 */
 ...
 */
template <class F> struct epsilon_f {
 ...
 */
template <> struct epsilon_f<double> {
 ...
 */
template <> struct epsilon_f<float> {
