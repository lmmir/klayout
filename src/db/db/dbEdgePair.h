 ...
 *
 *  An edge pair is a pair of edges which usually is used to mark a DRC
 * violation. It flags the relationship of two edges. It's a compoisition of two
 * edges: first and second.
 *
 *  By default, an edge pair is directed: first and second are not commutable
 * and indicate a relationship ("from first towards second"). The edge pair
 * carries a flag which allows indicating symmetric mode: in this mode, the
 * first and second edge are commutable. As a consequence, when the symmetric
 * flag is used, edge_pair(e1, e2, true) == edge_pair(e2, e1, true).
 */
template <class C> class DB_PUBLIC_TEMPLATE edge_pair {
public:
   ...
   */
  edge_pair() : m_first(), m_second(), m_symmetric(false) {
    //  .. nothing else ..
  }
   ...
   */
  void set_symmetric(bool s) { m_symmetric = s; }

  /**
   *  @brief The (dummy) translation operator
   */
  void translate(const edge_pair<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const edge_pair<C> &d, const T &t, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
    transform(t);
  }
   ...
   */
  bool operator!=(const edge_pair<C> &b) const { return !operator==(b); }
   ...
   */
  bool not_equal(const edge_pair<C> &b) const { return !equal(b); }
  ...
  distance_type distance() const {
    ...
    if (!e1.intersect(e2)) {
      distance_type d12 = std::min(e2.euclidian_distance(e1.p1()),
                                   e2.euclidian_distance(e1.p2()));
      distance_type d21 = std::min(e1.euclidian_distance(e2.p1()),
                                   e1.euclidian_distance(e2.p2()));
   ...
   */
  template <class Tr> edge_pair<C> &transform(const Tr &t) {
   ...
   */
  void set_first(const edge_type &e) { m_first = e; }

  /**
   *  @brief Sets the second edge.
   */
  void set_second(const edge_type &e) { m_second = e; }

  /**
   *  @brief The first edge (non-const).
   */
  edge_type &first() { return m_first; }

  /**
   *  @brief The second edge (non-const).
   */
  edge_type &second() { return m_second; }

  /**
   *  @brief The first edge.
   */
  const edge_type &first() const { return m_first; }

  /**
   *  @brief The second edge.
   */
  const edge_type &second() const { return m_second; }
  ...
  const box_type bbox() const {
    return box_type(m_first.p1(), m_first.p2()) +
           box_type(m_second.p1(), m_second.p2());
  }

  /**
   *  @brief Gets the perimeter of the edge pair
   *
   *  The perimeter is defined by then sum of the lengths of the edges ("active
   * perimeter")
   */
   ...
   */
  bool is_ortho() const { return m_first.is_ortho() && m_second.is_ortho(); }
  ...
  std::string to_string(double dbu = 0.0) const {
    return lesser().to_string(dbu) + (m_symmetric ? "|" : "/") +
           greater().to_string(dbu);
  }
   ...
   */
  bool parallel() const { return m_first.parallel(m_second); }
   ...
   */
  bool coincident() const { return m_first.coincident(m_second); }

  /**
   *  @brief Swaps the edges
   */
  void swap_edges() { std::swap(m_first, m_second); }

  /**
   *  @brief Normalize the edge orientation
   *
   *  This method modifies the orientation of the first edge such that both
   *  edge are anti-parallel. Such edge pairs will generate polygons which are
   * non-self overlapping. In addition, the edges are sorted such that the edges
   * form a closed loop in clockwise direction.
   */
  edge_pair<C> &normalize() {
    area_type a1 =
        db::vprod(m_first.p2() - m_second.p2(), m_first.p1() - m_second.p1());
    area_type a2 =
        db::vprod(m_first.p1() - m_second.p2(), m_first.p2() - m_second.p1());
    ...
    if (a1 < 0) {
      ...
      if (m_first.side_of(m_second.p1()) > 0 ||
          m_second.side_of(m_first.p1()) > 0) {
        m_first.swap_points();
        m_second.swap_points();
      }
    }
   ...
   *
   *  The given extension is applied to start and end points as well as
   * perpendicular. This way it is possible to map degenerated edge pairs
   * (points, coincident etc.) to get an area and hence they can be mapped to
   * polygons without vanishing. This method does not automatically normalize
   * the edge pairs but it is recommended to normalize them before converting
   * them to polygons.
   */
  template <class Poly> Poly to_polygon_generic(coord_type e) const {
    DEdge e1(m_first);
    DEdge e2(m_second);
 ...
 */
template <class C>
inline edge_pair<double> operator*(const edge_pair<C> &e, double s) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const edge_pair<C> &e) {
...
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::EdgePair &b);
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DEdgePair &b);
