...
std::string EdgeBoolAndOrNotLocalOperation::description() const {
...
void EdgeBoolAndOrNotLocalOperation::do_compute_local(
    db::Layout * /*layout*/,
  ...
  bool is_and =
      (m_op == EdgeAnd || m_op == EdgeAndNot || m_op == EdgeIntersections);
...
EdgeToPolygonLocalOperation::EdgeToPolygonLocalOperation(
    EdgePolygonOp::mode_t op, bool include_borders)
...
EdgeToPolygonLocalOperation::on_empty_intruder_hint() const {
  return m_op == EdgePolygonOp::Inside
             ? Drop
             : (m_op == EdgePolygonOp::Outside ? Copy : CopyToSecond);
}

std::string EdgeToPolygonLocalOperation::description() const {
...
void EdgeToPolygonLocalOperation::do_compute_local(
    db::Layout * /*layout*/,
  ...
  if (!others.empty() || any_subject) {
    ...
    for (std::set<db::PolygonRef>::const_iterator o = others.begin();
         o != others.end(); ++o) {
      for (db::PolygonRef::polygon_edge_iterator e = o->begin_edge();
           !e.at_end(); ++e) {
        ep.insert(*e, 0);
      }
    }
    ...
    if (result2) {
      cc_second.reset(
          new db::EdgeToEdgeSetGenerator(*result2, 2 /*second tag*/));
    }
