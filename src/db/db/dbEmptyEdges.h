 ...
 */
class DB_PUBLIC EmptyEdges : public EdgesDelegate {
public:
  EmptyEdges();
  virtual ~EmptyEdges();
  ...
  virtual EdgePairsDelegate *width_check(db::Coord,
                                         const db::EdgesCheckOptions &) const;
  virtual EdgePairsDelegate *space_check(db::Coord,
                                         const db::EdgesCheckOptions &) const;
  virtual EdgePairsDelegate *
  enclosing_check(const Edges &, db::Coord,
                  const db::EdgesCheckOptions &) const;
  virtual EdgePairsDelegate *overlap_check(const Edges &, db::Coord,
                                           const db::EdgesCheckOptions &) const;
  virtual EdgePairsDelegate *
  separation_check(const Edges &, db::Coord,
                   const db::EdgesCheckOptions &) const;
  virtual EdgePairsDelegate *inside_check(const Edges &, db::Coord,
                                          const db::EdgesCheckOptions &) const;

  virtual EdgesDelegate *filter_in_place(const EdgeFilterBase &) {
    return this;
  }
  ...
  virtual EdgesDelegate *and_with(const Edges &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *not_with(const Edges &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  andnot_with(const Edges &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
  virtual EdgesDelegate *and_with(const Region &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *not_with(const Region &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  andnot_with(const Region &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
  ...
  virtual EdgesDelegate *intersections(const Edges &) const {
    return new EmptyEdges();
  }

  virtual RegionDelegate *extended(coord_type, coord_type, coord_type,
                                   coord_type, bool) const;

  virtual EdgesDelegate *inside_part(const Region &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *outside_part(const Region &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  inside_outside_part_pair(const Region &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Region &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Edges &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }

  virtual EdgesDelegate *selected_outside(const Region &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *selected_not_outside(const Region &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  ...
  virtual EdgesDelegate *selected_inside(const Region &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *selected_not_inside(const Region &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  ...
  virtual EdgesDelegate *selected_outside(const Edges &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *selected_not_outside(const Edges &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_outside_pair(const Edges &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
  virtual EdgesDelegate *selected_inside(const Edges &) const {
    return new EmptyEdges();
  }
  virtual EdgesDelegate *selected_not_inside(const Edges &) const {
    return new EmptyEdges();
  }
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_inside_pair(const Edges &) const {
    return std::make_pair(new EmptyEdges(), new EmptyEdges());
  }
