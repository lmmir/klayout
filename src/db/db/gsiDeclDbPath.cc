...
template <class C> struct path_defs {
  ...
  static C *new_pwx(const std::vector<point_type> &pts, coord_type width,
                    coord_type bgn_ext, coord_type end_ext) {
    return new C(pts.begin(), pts.end(), width, bgn_ext, end_ext);
  }

  static C *new_pwxr(const std::vector<point_type> &pts, coord_type width,
                     coord_type bgn_ext, coord_type end_ext, bool round) {
  ...
  static distance_type length(const C *path) { return path->length(); }
  ...
  static gsi::Methods methods() {
    return constructor("new", &new_v,
                       ...
                       "path with width 0") +
           constructor("new|#new_pw", &new_pw, gsi::arg("pts"),
                       gsi::arg("width"),
                       "@brief Constructor given the points of the path's "
                       "spine and the width\n"
                       "\n"
                       "\n"
                       "@param pts The points forming the spine of the path\n"
                       "@param width The width of the path\n") +
           constructor("new|#new_pwx", &new_pwx, gsi::arg("pts"),
                       gsi::arg("width"), gsi::arg("bgn_ext"),
                       gsi::arg("end_ext"),
                       "@brief Constructor given the points of the path's "
                       "spine, the width and the extensions\n"
                       "\n"
                       "\n"
                       "@param pts The points forming the spine of the path\n"
                       "@param width The width of the path\n"
           ...
           constructor(
               "new|#new_pwxr", &new_pwxr, gsi::arg("pts"), gsi::arg("width"),
               gsi::arg("bgn_ext"), gsi::arg("end_ext"), gsi::arg("round"),
               "@brief Constructor given the points of the path's spine, the "
               "width, the extensions and the round end flag\n"
               "\n"
               "\n"
               "@param pts The points forming the spine of the path\n"
               "@param width The width of the path\n"
               "@param bgn_ext The begin extension of the path\n"
               "@param end_ext The end extension of the path\n"
               "@param round If this flag is true, the path will get rounded "
               "ends\n") +
           method("<", &C::less, gsi::arg("p"),
                  "@brief Less operator\n"
                  "@param p The object to compare against\n"
                  "This operator is provided to establish some, not "
                  "necessarily a certain sorting order") +
           ...
           method("bgn_ext=", (void (C::*)(coord_type)) & C::bgn_ext,
                  gsi::arg("ext"), "@brief Set the begin extension\n") +
           method("bgn_ext", (coord_type(C::*)() const) & C::bgn_ext,
                  "@brief Get the begin extension\n") +
           method("end_ext=", (void (C::*)(coord_type)) & C::end_ext,
                  gsi::arg("ext"), "@brief Set the end extension\n") +
           method("end_ext", (coord_type(C::*)() const) & C::end_ext,
                  "@brief Get the end extension\n") +
           method("round=", (void (C::*)(bool)) & C::round,
                  gsi::arg("round_ends_flag"),
                  "@brief Set the 'round ends' flag\n"
                  "A path with round ends show half circles at the ends, "
                  "instead of square or rectangular ends. "
                  "Paths with this flag set should use a begin and end "
                  "extension of half the width (see \\bgn_ext and \\end_ext). "
                  "The interpretation of such paths in other tools may differ "
                  "otherwise.") +
           ...
           method("simple_polygon", &C::simple_polygon,
                  ...
                  "The returned polygon is not guaranteed to be "
                  "non-selfoverlapping. This may happen if the path overlaps "
                  "itself or contains very short segments.") +
           method("polygon", &C::polygon,
                  "@brief Convert the path to a polygon\n"
                  "The returned polygon is not guaranteed to be non-self "
                  "overlapping. This may happen if the path overlaps "
                  "itself or contains very short segments.") +
           method("perimeter", &C::perimeter,
                  "@brief Returns the approximate perimeter of the path\n"
                  "This method returns the approximate value of the perimeter. "
                  "It is computed from the length and the width. "
                  "end extensions are taken into account correctly, but not "
                  "effects of the corner interpolation.\n"
                  "This method was added in version 0.24.4.\n") +
           method_ext("area", &area,
                      "@brief Returns the approximate area of the path\n"
                      "This method returns the approximate value of the area. "
                      "It is computed from the length times the width. "
                      "end extensions are taken into account correctly, but "
                      "not effects of the corner interpolation.\n"
                      "This method was added in version 0.22.\n") +
           method_ext("length", &length,
                      "@brief Returns the length of the path\n"
                      "the length of the path is determined by summing the "
                      "lengths of the segments and "
                      "adding begin and end extensions. For round-ended paths "
                      "the length of the paths between the tips "
                      "of the ends.\n"
                      "\n"
...
static db::Path *path_from_dpath(const db::DPath &p) { return new db::Path(p); }

static db::DPath path_to_dpath(const db::Path *p, double dbu) {
  return db::DPath(*p * dbu);
}

static db::Path path_round_corners(const db::Path *p, double radius,
                                   int npoints) {
  return db::round_path_corners(*p, radius, npoints);
}

Class<db::Path> decl_Path(
    "db", "Path",
    constructor("new|#from_dpath", &path_from_dpath, gsi::arg("dpath"),
                "@brief Creates an integer coordinate path from a "
                "floating-point coordinate path\n"
                "\n"
                "This constructor has been introduced in version 0.25 and "
                "replaces the previous static method 'from_dpath'.") +
        method_ext(
            "to_dtype", &path_to_dpath, gsi::arg("dbu", 1.0),
            "@brief Converts the path to a floating-point coordinate path\n"
            "\n"
            "The database unit can be specified to translate the "
            "integer-coordinate path into a floating-point coordinate "
            "path in micron units. The database unit is basically a scaling "
            "factor.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        method("transformed", &db::Path::transformed<db::ICplxTrans>,
               ...
               "@param t The transformation to apply.\n"
               "\n"
               "@return The transformed path (in this case an integer "
               "coordinate path).\n"
               "\n"
               "This method has been introduced in version 0.18.\n") +
        method_ext("round_corners", &path_round_corners, gsi::arg("radius"),
                   gsi::arg("npoints"),
                   "@brief Creates a new path whose corners are interpolated "
                   "with circular bends\n"
                   "\n"
                   "@param radius The radius of the bends\n"
                   "@param npoints The number of points (per full circle) used "
                   "for interpolating the bends\n"
                   "\n"
                   "This method has been introduced in version 0.25.") +
        path_defs<db::Path>::methods(),
    "@brief A path class\n"
    "\n"
    "A path consists of an sequence of line segments forming the 'spine' of "
    "the path "
    "and a width. In addition, the starting point can be drawn back by a "
    "certain extent (the 'begin extension') "
    "and the end point can be pulled forward somewhat (by the 'end "
    "extension').\n"
    "\n"
    "A path may have round ends for special purposes. In particular, a "
    "round-ended path with a single point "
    "can represent a circle. Round-ended paths should have being and end "
    "extensions equal to half the width. "
    "Non-round-ended paths with a single point are allowed but the definition "
    "of the resulting shape in not well defined and may differ in other "
    "tools.\n"
    "\n"
...
static db::DPath dpath_round_corners(const db::DPath *p, double radius,
                                     int npoints, double accuracy) {
  return db::round_path_corners(*p, radius, npoints, accuracy);
}

Class<db::DPath> decl_DPath(
    "db", "DPath",
    constructor("new|#from_ipath", &dpath_from_ipath, gsi::arg("path"),
                "@brief Creates a floating-point coordinate path from an "
                "integer coordinate path\n"
                "\n"
                "This constructor has been introduced in version 0.25 and "
                "replaces the previous static method 'from_ipath'.") +
        method_ext("to_itype", &dpath_to_path, gsi::arg("dbu", 1.0),
                   ...
                   "path in micron units to an integer-coordinate path in "
                   "database units. The path's' "
                   "coordinates will be divided by the database unit.\n"
                   "\n"
                   "This method has been introduced in version 0.25.") +
        method_ext("round_corners", &dpath_round_corners, gsi::arg("radius"),
                   gsi::arg("npoints"), gsi::arg("accuracy"),
                   "@brief Creates a new path whose corners are interpolated "
                   "with circular bends\n"
                   "\n"
                   "@param radius The radius of the bends\n"
                   ...
                   "\n"
                   "The accuracy parameter controls the numerical resolution "
                   "of the approximation process and should be in the "
                   "order of half the database unit. This accuracy is used for "
                   "suppressing redundant points and simplification of the "
                   "resulting path.\n"
                   "\n"
                   "This method has been introduced in version 0.25.") +
