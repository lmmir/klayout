...
PCellVariant::PCellVariant(db::cell_index_type ci, db::Layout &layout,
                           db::pcell_id_type pcell_id,
                           const pcell_parameters_type &parameters)
    : Cell(ci, layout), m_parameters(parameters), m_pcell_id(pcell_id),
      m_registered(false) {
  PCellVariant::reregister(); // actually, no "re-register", but the first
                              // registration ..
}

PCellVariant::~PCellVariant() { PCellVariant::unregister(); }

Cell *PCellVariant::clone(Layout &layout) const {
...
void PCellVariant::unregister() {
  // A PCellVariant that is saved as a member of a transaction will explicitly
  // be unregistered ..
  if (m_registered) {
...
std::string PCellVariant::get_basic_name() const {
...
tl::Variant PCellVariant::parameter_by_name(const std::string &name) const {
  const PCellHeader *header = pcell_header();
  if (header && header->declaration()) {
    ...
    for (std::vector<PCellParameterDeclaration>::const_iterator pd =
             pcp.begin();
         pd != pcp.end() && pp != parameters().end(); ++pd, ++pp) {
      if (pd->get_name() == name) {
        return *pp;
      }
    }
...
std::map<std::string, tl::Variant> PCellVariant::parameters_by_name() const {
  return parameters_by_name_from_list(parameters());
}

std::map<std::string, tl::Variant> PCellVariant::parameters_by_name_from_list(
    const db::pcell_parameters_type &list) const {
...
void PCellVariant::update(ImportLayerMapping *layer_mapping) {
  ...
  if (header && header->declaration()) {
    ...
    db::property_names_id_type pn =
        layout()->properties_repository().prop_name_id(tl::Variant("name"));
    db::property_names_id_type dn =
        layout()->properties_repository().prop_name_id(
            tl::Variant("description"));

    std::vector<unsigned int> layer_ids;
    try {

      layer_ids =
          header->get_layer_indices(*layout(), m_parameters, layer_mapping);

      //  call coerce prior to produce to make sure we have a validated
      //  parameter set (note that we cannot persist parameters from here)
    ...
    //  produce the shape parameters on the guiding shape layer so they can be
    //  edited
    size_t i = 0;
    const std::vector<db::PCellParameterDeclaration> &pcp =
        header->declaration()->parameter_declarations();
      ...
      if (i < m_parameters.size() &&
        ...
        if (m_parameters[i].is_user<db::DBox>()) {
          ...
          shapes(layout()->guiding_shape_layer())
              .insert(db::BoxWithProperties(
                  db::Box(m_parameters[i].to_user<db::DBox>() *
                          (1.0 / layout()->dbu())),
          ...
          shapes(layout()->guiding_shape_layer())
              .insert(db::BoxWithProperties(
                  m_parameters[i].to_user<db::Box>(),
                  layout()->properties_repository().properties_id(props)));

        } else if (m_parameters[i].is_user<db::DEdge>()) {

          shapes(layout()->guiding_shape_layer())
              .insert(db::EdgeWithProperties(
                  db::Edge(m_parameters[i].to_user<db::DEdge>() *
                           (1.0 / layout()->dbu())),
                  layout()->properties_repository().properties_id(props)));
          ...
          shapes(layout()->guiding_shape_layer())
              .insert(db::EdgeWithProperties(
                  m_parameters[i].to_user<db::Edge>(),
                  layout()->properties_repository().properties_id(props)));
          ...
          shapes(layout()->guiding_shape_layer())
              .insert(db::PointWithProperties(
                  p, layout()->properties_repository().properties_id(props)));

        } else if (m_parameters[i].is_user<db::DPolygon>()) {

          db::complex_trans<db::DCoord, db::Coord> dbu_trans(1.0 /
                                                             layout()->dbu());
          db::Polygon poly =
              m_parameters[i].to_user<db::DPolygon>().transformed(dbu_trans,
                                                                  false);
          //  Hint: we don't compress the polygon since we don't want to loose
          //  information
          shapes(layout()->guiding_shape_layer())
              .insert(db::PolygonWithProperties(
                  poly,
                  layout()->properties_repository().properties_id(props)));
          ...
          db::complex_trans<db::DCoord, db::Coord> dbu_trans(1.0 /
                                                             layout()->dbu());
          shapes(layout()->guiding_shape_layer())
              .insert(db::PathWithProperties(
                  dbu_trans * m_parameters[i].to_user<db::DPath>(),
                  layout()->properties_repository().properties_id(props)));
          ...
          shapes(layout()->guiding_shape_layer())
              .insert(db::PathWithProperties(
                  m_parameters[i].to_user<db::Path>(),
                  layout()->properties_repository().properties_id(props)));
        }
      }
    }
