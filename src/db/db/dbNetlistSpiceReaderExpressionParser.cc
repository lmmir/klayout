...
NetlistSpiceReaderExpressionParser::NetlistSpiceReaderExpressionParser(
    const variables_type *vars, double def_scale)
...
NetlistSpiceReaderExpressionParser::NetlistSpiceReaderExpressionParser(
    const variables_type *vars1, const variables_type *vars2, double def_scale)
    : m_def_scale(def_scale) {
  mp_variables1 = vars1;
  mp_variables2 = vars2;
}
...
static double nint_f(double v) {
  return nearbyint(v);
} //  we basically should we the rounding mode before ...
static double floor_f(double v) { return floor(v); }
static double ceil_f(double v) { return ceil(v); }
static double sgn_f(double v) {
  return v == 0.0 ? 0.0 : (v < 0.0 ? -1.0 : 1.0);
}
static double int_f(double v) { return sgn_f(v) * floor(sgn_f(v) * v); }

tl::Variant NetlistSpiceReaderExpressionParser::eval_func(
    const std::string &name, const std::vector<tl::Variant> &params,
    bool * /*status*/) const {
  double (*f)(double) = 0;
  ...
  if (f != 0) {
    ...
    if (params.size() < 2 || !params[0].can_convert_to_double() ||
        !params[1].can_convert_to_double()) {
      return tl::Variant();
    } else {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_atomic_value(tl::Extractor &ex,
                                                      bool *status) const {
  double vd = 0.0;
  std::string var;
    ...
    if (status) {
      *status = false;
      ...
      throw tl::Exception(tl::sprintf(
          tl::to_string(tr("Expected number of variable name here: '...%s'")),
          ex.get()));
    }
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_pwr_expr(tl::Extractor &ex,
                                                  bool *status) const {
  tl::Variant v = read_atomic_value(ex, status);
  if (status && !*status) {
    return tl::Variant();
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_dot_expr(tl::Extractor &ex,
                                                  bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_bar_expr(tl::Extractor &ex,
                                                  bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_compare_expr(tl::Extractor &ex,
                                                      bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_logical_op(tl::Extractor &ex,
                                                    bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_ternary_op(tl::Extractor &ex,
                                                    bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read_tl_expr(tl::Extractor &ex,
                                                 bool *status) const {
...
tl::Variant
NetlistSpiceReaderExpressionParser::read(const std::string &s) const {
...
bool NetlistSpiceReaderExpressionParser::try_read(const std::string &s,
                                                  tl::Variant &value) const {
  tl::Extractor ex(s.c_str());
  return try_read(ex, value);
}

bool NetlistSpiceReaderExpressionParser::try_read(tl::Extractor &ex,
                                                  tl::Variant &value) const {
