...
LayoutVsSchematic::LayoutVsSchematic(db::DeepShapeStore *dss,
                                     unsigned int layout_index)
    : LayoutToNetlist(dss, layout_index) {
  //  .. nothing yet ..
}

LayoutVsSchematic::LayoutVsSchematic(const std::string &topcell_name,
                                     double dbu)
    : LayoutToNetlist(topcell_name, dbu) {
  //  .. nothing yet ..
}

LayoutVsSchematic::LayoutVsSchematic() : LayoutToNetlist() {
  //  .. nothing yet ..
}
...
bool LayoutVsSchematic::compare_netlists(db::NetlistComparer *compare) {
  ...
  if (!reference_netlist()) {
    throw tl::Exception(
        tl::to_string(tr("The reference netlist has not been set yet")));
  }
