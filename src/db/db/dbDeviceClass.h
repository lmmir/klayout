...
public:
   ...
   */
  DeviceTerminalDefinition() : m_name(), m_description(), m_id(0) {
    //  .. nothing yet ..
  }

  /**
   *  @brief Creates a device terminal definition with the given name and
   * description
   */
  DeviceTerminalDefinition(const std::string &name,
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const DeviceTerminalDefinition &x, bool no_self,
                     void *parent) {
...
public:
  ...
  DeviceParameterDefinition()
      : m_name(), m_description(), m_default_value(0.0), m_id(0),
        m_is_primary(true), m_si_scaling(1.0), m_geo_scaling(0.0) {
    //  .. nothing yet ..
  }

  /**
   *  @brief Creates a device parameter definition with the given name and
   * description
   */
  DeviceParameterDefinition(const std::string &name,
                            const std::string &description,
                            double default_value = 0.0, bool is_primary = true,
                            double si_scaling = 1.0, double geo_scaling = 0.0)
      : m_name(name), m_description(description),
        m_default_value(default_value), m_id(0), m_is_primary(is_primary),
        m_si_scaling(si_scaling), m_geo_scaling(geo_scaling) {
    //  .. nothing yet ..
  }
   ...
   *
   *  Some parameters are given in micrometers - for example W and L of MOS
   * devices. This scaling factor gives the translation to SI units (1e-6 for
   * micrometers).
   */
  double si_scaling() const { return m_si_scaling; }

  /**
   *  @brief Set the SI unit scaling factor
   */
  void set_si_scaling(double s) { m_si_scaling = s; }

  /**
   *  @brief Gets the geometry scaling exponent
   *
   *  The geometry scaling exponent is used for example when applying .option
   * scale in Spice reading. It is 0 for "no scaling", 1 for linear scaling and
   * 2 for quadratic scaling.
   */
  double geo_scaling_exponent() const { return m_geo_scaling; }

  /**
   *  @brief Sets the geometry scaling exponent
   */
  void set_geo_scaling_exponent(double e) { m_geo_scaling = e; }
   ...
   */
  void set_default_value(double d) { m_default_value = d; }

  /**
   *  @brief Sets a value indicating whether the parameter is a primary
   * parameter
   *
   *  If this flag is set to true (the default), the parameter is considered a
   * primary parameter. Only primary parameters are compared by default.
   */
  void set_is_primary(bool p) { m_is_primary = p; }

  /**
   *  @brief Gets a value indicating whether the parameter is a primary
   * parameter
   */
 ...
 */
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const DeviceParameterDefinition &x, bool no_self,
                     void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
 ...
 */
class DB_PUBLIC DeviceParameterCompareDelegate : public gsi::ObjectBase,
                                                 public tl::Object {
public:
...
/**
 *  @brief A parameter compare delegate that compares several parameters either
 * relative or absolute (or both)
 *
 *  The reasoning behind this class is to supply a chainable compare delegate:
 * ab = a + b where a and b are compare delegates for two different parameters
 * and ab is the combined compare delegate.
 */
class DB_PUBLIC EqualDeviceParameters : public DeviceParameterCompareDelegate {
public:
...
/**
 *  @brief A parameter compare delegate that compares all parameters in a
 * relative fashion
 */
 ...
 */
class DB_PUBLIC DeviceCombiner : public gsi::ObjectBase, public tl::Object {
public:
  DeviceCombiner() {}
   ...
   *
   *  This method shall test, whether the two devices can be combined. Both
   * devices are guaranteed to share the same device class. If they cannot be
   * combined, this method shall do nothing and return false. If they can be
   * combined, this method shall reconnect the nets of the first device and
   * entirely disconnect the nets of the second device. The second device will
 ...
 */
class DB_PUBLIC DeviceClass : public gsi::ObjectBase,
                              public tl::Object,
                              public tl::UniqueId {
public:
   ...
   */
  db::Netlist *netlist() { return mp_netlist; }

  /**
   *  @brief Gets the netlist the device class lives in (const version)
   */
  const db::Netlist *netlist() const { return mp_netlist; }

  /**
   *  @brief Sets a value indicating whether this class performs strict terminal
   * mapping
   *
   *  Classes with this flag set don't allow terminal swapping, independently of
   * the "normalize_terminal_id" implementation. If two classes are involved in
   * a compare, both classes are treated strict if one of them operates in
   * strict mode.
   */
  void set_strict(bool s) { m_strict = s; }

  /**
   *  @brief Gets a value indicating whether this class performs strict terminal
   * mapping
   */
  ...
  add_terminal_definition(const DeviceTerminalDefinition &pd);
  ...
  add_parameter_definition(const DeviceParameterDefinition &pd);
   ...
   */
  virtual DeviceClass *clone() const { return new DeviceClass(*this); }

  /**
   *  @brief Combines two devices
   *
   *  This method shall test, whether the two devices can be combined. Both
   * devices are guaranteed to share the same device class (this). If they
   * cannot be combined, this method shall do nothing and return false. If they
   * can be combined, this method shall reconnect the nets of the first device
   * and entirely disconnect the nets of the second device. The second device
   * will be deleted afterwards.
   */
  bool combine_devices(db::Device *a, db::Device *b) const {
    return mp_device_combiner.get() ? mp_device_combiner->combine_devices(a, b)
                                    : false;
  }

  /**
   *  @brief Returns true if the device class supports device combination in
   * parallel mode
   */
  ...
  /**
   *  @brief Sets a value indicating that the class supports device combination
   * in parallel mode
   */
   ...
   */
  void clear_equivalent_terminal_ids() { m_equivalent_terminal_ids.clear(); }
   ...
   *
   *  a and b are expected to originate from this or an equivalent device class
   * having the same parameters. This is the "less" operation. If a parameter
   * compare delegate is registered, this compare request will be forwarded to
   * the delegate.
   *
   ...
   *
   *  a and b are expected to originate from this or an equivalent device class
   * having the same parameters. This is the "equal" operation. If a parameter
   * compare delegate is registered, this compare request will be forwarded to
   * the delegate.
   *
   *  If two devices with different device classes are compared and only one of
   ...
   */
  void
  set_parameter_compare_delegate(db::DeviceParameterCompareDelegate *delegate) {
    if (delegate) {
  ...
  /**
   *  @brief Gets the parameter compare delegate or null if no such delegate is
   * registered
   */
   ...
   * (non-const version)
   */
  db::DeviceCombiner *device_combiner() { return mp_device_combiner.get(); }

  /**
   *  @brief Internally used by the netlist comparer to temporarily attach a
   * device class pointing to the primary one
   */
   ...
   * device class pointing to the primary one
   */
  const db::DeviceClass *primary_class() const { return mp_primary_class; }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    db::mem_stat(stat, purpose, cat, m_name, true, (void *)this);
    db::mem_stat(stat, purpose, cat, m_description, true, (void *)this);
    db::mem_stat(stat, purpose, cat, m_terminal_definitions, true,
                 (void *)this);
    db::mem_stat(stat, purpose, cat, m_parameter_definitions, true,
                 (void *)this);
  }
...
private:
  ...
  void set_netlist(db::Netlist *nl) { mp_netlist = nl; }
};
 ...
 */
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const DeviceClass &x, bool no_self,
                     void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
public:
  DeviceClassTemplateBase(const std::string &name) : m_name(name) {
    //  .. nothing yet ..
  }
...
public:
  ...
  virtual DeviceClass *create() const { return new T(); }
