...
EdgePairs::EdgePairs() : mp_delegate(new EmptyEdgePairs()) {
  //  .. nothing yet ..
}
...
EdgePairs::EdgePairs(EdgePairsDelegate *delegate) : mp_delegate(delegate) {
  //  .. nothing yet ..
}
...
template <class Sh> void EdgePairs::insert(const Sh &shape) {
...
template DB_PUBLIC void EdgePairs::insert(const db::Shape &,
                                          const db::ICplxTrans &);
...
void EdgePairs::clear() { set_delegate(new EmptyEdgePairs()); }

void EdgePairs::reserve(size_t n) { mutable_edge_pairs()->reserve(n); }

void EdgePairs::flatten() { mutable_edge_pairs()->flatten(); }

template <class T> EdgePairs &EdgePairs::transform(const T &trans) {
...
const db::RecursiveShapeIterator &EdgePairs::iter() const {
  static db::RecursiveShapeIterator def_iter;
  const db::RecursiveShapeIterator *i = mp_delegate ? mp_delegate->iter() : 0;
  return *(i ? i : &def_iter);
}

const db::PropertiesRepository &EdgePairs::properties_repository() const {
...
db::PropertiesRepository &EdgePairs::properties_repository() {
...
template <> DB_PUBLIC void extractor_impl(tl::Extractor &ex, db::EdgePairs &b) {
  if (!test_extractor_impl(ex, b)) {
    ex.error(
        tl::to_string(tr("Expected an edge pair collection specification")));
  }
}
} // namespace tl
