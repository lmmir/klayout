...
Manager::Manager(bool enabled)
    : m_transactions(), m_current(m_transactions.begin()), m_opened(false),
      m_replay(false), m_enabled(enabled) {
  //  .. nothing yet ..
}
...
Manager::~Manager() {
  ...
  for (std::vector<db::Object *>::const_iterator obj = m_id_table.begin();
       obj != m_id_table.end(); ++obj) {
    if (*obj) {
      (*obj)->manager(0);
    }
  }
...
db::Object *Manager::object_by_id(ident_t id) {
...
Manager::ident_t Manager::next_id(db::Object *obj) {
...
void Manager::erase_transactions(transactions_t::iterator from,
                                 transactions_t::iterator to) {
  for (transactions_t::iterator i = from; i != to; ++i) {
    for (operations_t::iterator o = i->first.begin(); o != i->first.end();
         ++o) {
      delete o->second;
    }
  }
...
Manager::transaction_id_t Manager::transaction(const std::string &description,
                                               transaction_id_t join_with) {
  if (m_enabled) {

    //  close transactions that are still open (was an assertion before)
    if (m_opened) {
      tl::warn << tl::to_string(tr("Transaction still opened: "))
               << m_current->second;
      commit();
    }
    ...
    if (!m_transactions.empty() && reinterpret_cast<transaction_id_t>(
                                       &m_transactions.back()) == join_with) {
  ...
  size_t id = m_transactions.empty()
                  ? 0
                  : reinterpret_cast<transaction_id_t>(&m_transactions.back());
  return id;
}

Manager::transaction_id_t Manager::last_transaction_id() const {
  return m_transactions.empty()
             ? 0
             : reinterpret_cast<transaction_id_t>(&m_transactions.back());
}

Manager::transaction_id_t Manager::transaction_id_for_undo() const {
...
Manager::transaction_id_t Manager::transaction_id_for_redo() const {
...
void Manager::cancel() {
  //  equivalent to commit and undo. But takes care that an empty commit is not
  //  followed by undo (which would undo the previous transaction!)
  if (m_enabled) {
    ...
    //  wipe following history as we don't want the cancelled operation to be
    //  redoable
    erase_transactions(m_current, m_transactions.end());
    m_current = m_transactions.end();
  }
...
void Manager::undo() {
  ...
  tl::RelativeProgress progress(tl::to_string(tr("Undoing")),
                                m_current->first.size(), 10);

  try {

    for (operations_t::reverse_iterator o = m_current->first.rbegin();
         o != m_current->first.rend(); ++o) {
...
void Manager::redo() {
  ...
  tl::RelativeProgress progress(tl::to_string(tr("Redoing")),
                                m_current->first.size(), 10);

  try {

    m_replay = true;
    for (operations_t::iterator o = m_current->first.begin();
         o != m_current->first.end(); ++o) {
...
std::pair<bool, std::string> Manager::available_undo() const {
...
std::pair<bool, std::string> Manager::available_redo() const {
...
db::Op *Manager::last_queued(db::Object *object) {
  tl_assert(m_opened);
  tl_assert(!m_replay);

  if (m_current == m_transactions.end() || m_current->first.empty() ||
      (object && m_current->first.back().first != object->id())) {
    return 0;
  } else {
