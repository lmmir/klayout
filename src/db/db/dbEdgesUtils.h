...
struct DB_PUBLIC EdgeLengthFilter : public EdgeFilterBase {
   ...
   *  @param inverse If set to true, only polygons not matching this criterion
   * will be filtered
   */
  ...
  bool selected(const std::unordered_set<db::Edge> &edges) const {
    length_type l = 0;
    for (std::unordered_set<db::Edge>::const_iterator e = edges.begin();
         e != edges.end(); ++e) {
      l += e->length();
    }
    return check(l);
 ...
 *
 *  This detector can check whether the angle between two edges is within a
 * certain angle interval. It takes two edges: a and b. If b "turns left" (b
 * following a), the angle will be positive, if it "turns" right, the angle will
 * be negative. The angle can be between -180 and 180 degree. The case of
 * reflection (exactly 180 degree) is not considered.
 *
 *  The constraint can be given in terms of a minimum and maximum angle.
 * "include" specifies whether the angle value itself is included. The
 * operator() will return true, if the angle between the given edges a and b in
 * matching the constraint.
 */
class DB_PUBLIC EdgeAngleChecker {
public:
  EdgeAngleChecker(double angle_start, bool include_angle_start,
...
/**
 *  @brief An edge orientation filter for use with Edges::filter or
 * Edges::filtered
 *
 *  This filter has two parameters: amin and amax.
 *  It will filter all edges for which the orientation angle is >= amin and <
 * amax. The orientation angle is measured in degree against the x axis in the
 * mathematical sense. There is an "invert" flag which allows selecting all
 * edges not matching the criterion.
 */

struct DB_PUBLIC EdgeOrientationFilter : public EdgeFilterBase {
  /**
   *  @brief Constructor
   *
   *  @param amin The minimum angle (measured against the x axis)
   *  @param amax The maximum angle (measured against the x axis)
   *  @param inverse If set to true, only edges not matching this criterion will
   * be filtered
   *
   *  This filter will filter out all edges whose angle against x axis
   *  is larger or equal to amin and less than amax.
   */
  EdgeOrientationFilter(double amin, bool include_amin, double amax,
                        bool include_amax, bool inverse);
...
struct DB_PUBLIC SpecialEdgeOrientationFilter : public EdgeFilterBase {
  enum FilterType {
...
/**
 *  @brief A helper class for the edge interaction functionality which acts as
 * an edge pair receiver
 */
  ...
  void add(const db::Edge *o1, size_t p1, const db::Edge *o2, size_t p2) {
    ...
    if (p1 != p2) {
      ...
      if ((m_mode == EdgesInteract && db::edge_interacts(*o, *oo)) ||
        ...
        //  In this case we need to collect edges which are outside always - we
        //  report those on "finished".
        m_seen.insert(o);
      }
    }
  }
...
/**
 *  @brief A helper class for the edge to region interaction functionality which
 * acts as an edge pair receiver
 *
 *  Note: This special scanner uses pointers to two different objects: edges and
 * polygons. It uses odd value pointers to indicate pointers to polygons and
 * even value pointers to indicate pointers to edges.
 *
...
class edge2edge_check_for_edges
    : public db::box_scanner_receiver<db::Edge, size_t> {
public:
  edge2edge_check_for_edges(const EdgeRelationFilter &check, Output &output,
 ...
 */
struct JoinEdgesCluster : public db::cluster<db::Edge, size_t> {
  typedef db::Edge::coord_type coord_type;

  JoinEdgesCluster(db::PolygonSink *output, coord_type ext_b, coord_type ext_e,
 ...
 */
db::Polygon extended_edge(const db::Edge &edge, db::Coord ext_b,
                          db::Coord ext_e, db::Coord ext_o, db::Coord ext_i);

/**
 *  @brief Wraps the extension algorithm into a edge to polygon processor
 */
class DB_PUBLIC ExtendedEdgeProcessor : public db::EdgeToPolygonProcessorBase {
public:
  ExtendedEdgeProcessor(db::Coord e)
      : m_ext_b(e), m_ext_e(e), m_ext_o(e), m_ext_i(e) {}

  ExtendedEdgeProcessor(db::Coord ext_b, db::Coord ext_e, db::Coord ext_o,
                        db::Coord ext_i)
 ...
 */
class DB_PUBLIC EdgeSegmentSelector : public EdgeProcessorBase {
public:
