 ...
 */
template <class C> class DB_PUBLIC_TEMPLATE user_object_base {
public:
   ...
   *  get_unique_user_object_class_id once in the initialisation of a static
   * member Used for operator< () implementation.
   */
   ...
   *
   *  This class name can be used to construct an object from a string using the
   * generic factory. The class name can be 0 indicating that it is not possible
   * to create an object from a string and that the object is not registered in
   * the generic factory.
   */
...
public:
   ...
   *  passed.
   */
  user_object(user_object_base<C> *obj) : mp_obj(obj) {
    //  .. nothing else ..
  }
   ...
   */
  user_object(const user_object<C> &d) : mp_obj(0) {
    if (d.mp_obj) {
   ...
   */
  user_object(user_object<C> &&d) : mp_obj(0) {
    if (d.mp_obj) {
      set_ptr(d.mp_obj);
      d.mp_obj = 0;
    }
   ...
   */
  ~user_object() { set_ptr(0); }

  /**
   *  @brief The (dummy) translation operator
   */
  void translate(const user_object<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const user_object<C> &d, const T &t,
   ...
   */
  bool operator!=(const user_object<C> &b) const { return !operator==(b); }
   ...
   */
  const user_object_base<C> *ptr() const { return mp_obj; }

  /**
   *  @brief Get the pointer to the base object (non-const version)
   */
  user_object_base<C> *ptr() { return mp_obj; }
   ...
   *
   *  This may cause coordinate overflow if the user object cannot be
   * represented in the target coordinate types.
   */
  db::box<C> box() const {
    if (mp_obj) {
      return mp_obj->box();
   ...
   *  The actual behaviour is implemented by the base object.
   */
  template <class Trans> db::user_object<C> transformed(const Trans &t) const {
    user_object o(*this);
    o.transform(t);
    return o;
  }
   ...
   */
  void swap(db::user_object<C> &other) { std::swap(mp_obj, other.mp_obj); }
 ...
 */
template <class C> class DB_PUBLIC_TEMPLATE user_object_factory_base {
public:
 ...
 *
 *  This implements a factory for objects of class X with coordinate base type
 * C.
 */
...
public:
  ...
  virtual const char *class_name() const { return mp_class_name; }

  virtual user_object_base<C> *create() const { return new X(); }
 ...
 */
template <class C>
class user_object_factory : public tl::Registrar<user_object_factory_base<C>> {
public:
  /**
   *  @brief Create a UserObject from a string with the given class and using
   * the provided string to create the object from.
   *
   *  If the class name is not registered, no object is created and 0 is
   * returned.
   */
  static user_object_base<C> *create(const char *class_name, const char *string,
                                     const char *base_path) {
    tl::Registrar<user_object_factory_base<C>> *factory =
        tl::Registrar<user_object_factory_base<C>>::get_instance();
    if (factory != 0) {
      for (typename tl::Registrar<user_object_factory_base<C>>::iterator i =
               factory->begin();
           i != factory->end(); ++i) {
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const user_object<X> &x, bool no_self = false,
                     void *parent = 0) {
 ...
 *  @brief typedef for a factory object for integer coordinate user objects
 */
typedef tl::RegisteredClass<user_object_factory_base<db::Coord>>
    UserObjectDeclaration;
 ...
 *  @brief typedef for a factory object for double coordinate user objects
 */
typedef tl::RegisteredClass<user_object_factory_base<db::DCoord>>
    DUserObjectDeclaration;
