 ...
 *
 *  Mutable regions offer insert, transform, flatten and other manipulation
 * functions.
 */
class DB_PUBLIC MutableRegion : public AsIfFlatRegion {
public:
  MutableRegion();
  ...
  void insert(const db::PolygonWithProperties &polygon) {
    do_insert(polygon, polygon.properties_id());
  }
