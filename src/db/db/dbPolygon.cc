...
template <class C>
static db::DEdge compute_shifted(const db::edge<C> &e, C dx, C dy, double ext,
                                 int nsign) {
  ...
  //  We create two test lines for the adjacent edges that extend somewhat
  //  further (i.e by half the width). These test lines define the limits of the
  //  area where the segments are responsible
...
template <class C>
static void compute_normals(const db::vector<C> &d, C dx, C dy, int nsign,
                            db::DVector &ed, db::DVector &nd) {
  if (db::coord_traits<C>::equal(dx, dy)) {
    ...
    if (f < db::coord_traits<DCoord>::prec_distance()) {
      ...
      //  dpx is a smart multiplication trying to preserve 45 degree edges on
      //  grid
    ...
    double f = sqrt(double(dx) * double(dx) * double(d.y()) * double(d.y()) +
                    double(dy) * double(dy) * double(d.x()) * double(d.x()));
    if (f < db::coord_traits<DCoord>::prec_area()) {
      ...
      nd = db::DVector(double(-d.y()) * double(dx) * double(dx),
                       double(d.x()) * double(dy) * double(dy));
      nd *= nsign / f;
    }
  }
}
...
/**
 *  @brief Provides a special DVector vprod sign for the purpose of representing
 * integer-coordinate vectors The "zero" criterion is somewhat tighter than that
 * of the normal integer value vectors. Hence, parallelity is somewhat more
 * strict which makes the size function produce a better approximation to the
 * desired target contour.
 */
static inline int vprod_sign_for(const db::DVector &a, const db::DVector &b,
                                 const db::Vector &) {
...
/**
 *  @brief Provides a special DVector sprod sign for the purpose of representing
 * integer-coordinate vectors The "zero" criterion is somewhat tighter than that
 * of the normal integer value vectors. Hence, orthogonality is somewhat more
 * strict which makes the size function produce a better approximation to the
 * desired target contour.
 */
static inline int sprod_sign_for(const db::DVector &a, const db::DVector &b,
                                 const db::Vector &) {
...
void polygon_contour<C>::size(C dx, C dy, unsigned int mode) {
  ...
  //  assign the results
  assign(new_points.begin(), new_points.end(), db::unit_trans<C>(), is_hole(),
         true /*compress*/, false /*don't normalize*/);
  ...
  //  create a vector where we remember what edge is obsolete since it became
  //  inverted
  ...
  while (nvalid >= 2 && nvalid_last != nvalid) {
    ...
    if (ip.first) {
      ...
      //  If the test lines to not cross, we have the case of an acute angle
      //  bend. This is a normal outer bend: we insert both points to define the
      //  contour in a confined, cut-off fashion.
  ...
  //  assign the results
  assign(new_points.begin(), new_points.end(), db::unit_trans(), is_hole(),
         true /*compress*/, false /*don't normalize*/);
...
template <class C>
DB_PUBLIC bool _test_extractor_impl(tl::Extractor &ex, db::polygon<C> &p) {
...
template <class C>
DB_PUBLIC bool _test_extractor_impl(tl::Extractor &ex,
                                    db::simple_polygon<C> &p) {
  typedef db::point<C> point_type;
  std::vector<point_type> points;
