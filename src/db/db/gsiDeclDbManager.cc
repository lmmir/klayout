...
static db::Manager::transaction_id_t
transaction1(db::Manager *manager, const std::string &description) {
  return manager->transaction(description);
}

static db::Manager::transaction_id_t
transaction2(db::Manager *manager, const std::string &description,
             db::Manager::transaction_id_t id) {
  return manager->transaction(description, id);
}

Class<db::Manager> decl_Manager(
    "db", "Manager",
    gsi::method_ext(
        "transaction", &transaction1, gsi::arg("description"),
        ...
        "\n"
        "@return The ID of the transaction (can be used to join other "
        "transactions with this one)\n") +
        gsi::method_ext(
            "transaction", &transaction2, gsi::arg("description"),
            gsi::arg("join_with"),
            "@brief Begin a joined transaction\n"
            "\n"
            "\n"
            "This call will open a new transaction and join if with the "
            "previous transaction.\n"
            "The ID of the previous transaction must be equal to the ID given "
            "with 'join_with'.\n"
            "\n"
            "This overload was introduced in version 0.22.\n"
            "\n"
            "@param description The description for this transaction (ignored "
            "if joined).\n"
            "@param description The ID of the previous transaction.\n"
            "\n"
            "@return The ID of the new transaction (can be used to join "
            "more)\n") +
        ...
        gsi::method_ext("transaction_for_undo", &transaction_for_undo,
                        "@brief Return the description of the next transaction "
                        "for 'undo'\n") +
        gsi::method_ext(
            "has_redo?", &has_redo,
    ...
    "\n"
    "Manager objects control layout and potentially other objects in the "
    "layout database "
    "and queue operations to form transactions. A transaction is a sequence of "
    "operations that can be undone or redone.\n"
    "\n"
    "In order to equip a layout object with undo/redo support, instantiate the "
    "layout object "
    "with a manager attached and embrace the operations to undo/redo with "
    "transaction/commit calls.\n"
    "\n"
    "The use of transactions is subject to certain constraints, i.e. "
    "transacted sequences may not be "
