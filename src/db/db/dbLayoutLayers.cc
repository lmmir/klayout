...
LayerIterator::LayerIterator(unsigned int layer_index,
                             const db::LayoutLayers &layout)
    : m_layer_index(layer_index), m_layout(layout) {
  while (m_layer_index < m_layout.layers() &&
         m_layout.layer_state(m_layer_index) != db::LayoutLayers::Normal) {
    ++m_layer_index;
  }
}
...
LayerIterator::operator*() const {
  return std::pair<unsigned int, const db::LayerProperties *>(
      m_layer_index, &m_layout.get_properties(m_layer_index));
}
...
LayoutLayers::LayoutLayers()
    : m_guiding_shape_layer(-1), m_waste_layer(-1), m_error_layer(-1) {
  // .. nothing yet ..
}
...
LayoutLayers &LayoutLayers::operator=(const LayoutLayers &d) {
  if (&d != this) {
...
void LayoutLayers::mem_stat(MemStatistics *stat,
...
void LayoutLayers::delete_layer(unsigned int n) {
  ...
  if (!lp.is_null()) {
    for (auto i = m_layers_by_props.find(lp);
         i != m_layers_by_props.end() && i->first.log_equal(lp); ++i) {
      if (i->second == n) {
...
unsigned int LayoutLayers::insert_layer(const LayerProperties &props) {
  unsigned int i = do_insert_layer();
  set_properties(i, props);
  return i;
}

void LayoutLayers::insert_layer(unsigned int index,
...
unsigned int LayoutLayers::error_layer() const {
  if (m_error_layer < 0) {
    ...
    self->m_error_layer =
        (int)self->insert_special_layer(db::LayerProperties("WASTE"));
  }

  return (unsigned int)m_error_layer;
}

unsigned int LayoutLayers::waste_layer() const {
  if (m_waste_layer < 0) {
    //  create the waste layer (since that layer is cached we can do
    //  this in a "const" fashion.
    db::LayoutLayers *self = const_cast<db::LayoutLayers *>(this);
...
unsigned int LayoutLayers::guiding_shape_layer() const {
  if (m_guiding_shape_layer < 0) {
    //  create the guiding shape layer (since that layer is cached we can do
    //  this in a "const" fashion.
    db::LayoutLayers *self = const_cast<db::LayoutLayers *>(this);
    self->m_guiding_shape_layer =
        (int)self->insert_special_layer(db::LayerProperties("GUIDING_SHAPES"));
  }

  return (unsigned int)m_guiding_shape_layer;
}

unsigned int LayoutLayers::insert_special_layer(const LayerProperties &props) {
  unsigned int i = do_insert_layer(true /*special*/);
  set_properties(i, props);
  return i;
}

void LayoutLayers::insert_special_layer(unsigned int index,
                                        const LayerProperties &props) {
  if (layer_state(index) == Normal) {
    delete_layer(index);
  }
...
unsigned int LayoutLayers::do_insert_layer(bool special) {
...
void LayoutLayers::reserve_layers(unsigned int n) { m_layer_states.reserve(n); }

void LayoutLayers::set_properties(unsigned int n,
                                  const LayerProperties &props) {
