 ...
 *  @brief An edge set
 *
 *  An edge set is basically a collection of edges. They do not necessarily need
 * to form closed contours. Edges can be manipulated in various ways. Edge sets
 * closely cooperate with the Region class which is a set of polygons.
 *
 *  Edge sets have some methods in common with regions. Edge sets can also be
 * merged, which means that edges which are continuations of other edges are
 * joined.
 *
 *  Edge sets can contain degenerated edges. Such edges are some which have
 * identical start and end points. Such edges are basically points which have
 * some applications, i.e. as markers for certain locations.
 */

class DB_PUBLIC Edges : public db::ShapeCollection {
public:
   ...
   */
  explicit Edges(const db::Box &s) : mp_delegate(0) { insert(s); }
   ...
   */
  explicit Edges(const db::SimplePolygon &s) : mp_delegate(0) { insert(s); }
   ...
   *  Creates an edge set representing the contour of the polygon
   */
  explicit Edges(const db::Polygon &s) : mp_delegate(0) { insert(s); }
   ...
   */
  explicit Edges(const db::Path &s) : mp_delegate(0) { insert(s); }
   ...
   */
  explicit Edges(const db::Edge &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Sequence constructor
   *
   *  Creates an edge set from a sequence of objects. The objects can be boxes,
   *  polygons, paths, edges or shapes. This version accepts iterators of the
   * begin ... end style.
   */
  template <class Iter>
  explicit Edges(const Iter &b, const Iter &e) : mp_delegate(0) {
    reserve(e - b);
    for (Iter i = b; i != e; ++i) {
      insert(*i);
    }
   ...
   *
   *  Creates an edge set from a recursive shape iterator. This allows feeding
   * an edge set from a hierarchy of cells.
   */
   ...
   *
   *  Creates an edge set from a recursive shape iterator. This allows feeding
   * an edge set from a hierarchy of cells. The transformation is useful to
   * scale to a specific DBU for example.
   */
  explicit Edges(const RecursiveShapeIterator &si, const db::ICplxTrans &trans,
                 bool as_edges = true, bool merged_semantics = true);

  /**
   *  @brief Constructor from a RecursiveShapeIterator providing a deep
   * representation
   *
   *  This version will create a hierarchical edge collection. The
   * DeepShapeStore needs to be provided during the lifetime of the collection
   * and acts as a heap for optimized data.
   */
  explicit Edges(const RecursiveShapeIterator &si, DeepShapeStore &dss,
                 bool as_edges = true);
   ...
   */
  explicit Edges(const RecursiveShapeIterator &si, DeepShapeStore &dss,
                 const db::ICplxTrans &trans, bool as_edges = true,
   ...
   */
  const EdgesDelegate *delegate() const { return mp_delegate; }

  /**
   *  @brief Gets the underlying delegate object
   */
  EdgesDelegate *delegate() { return mp_delegate; }
   ...
   */
  const_iterator begin() const { return EdgesIterator(mp_delegate->begin()); }
  ...
  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the edges plus the
  ...
  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the merged edges plus
   ...
   *
   *  nmax specifies how many edges are included (set to
   ...
   *
   *  If merged semantics is enabled (the default), colinear edges will be
   * considered as single edges.
   */
   ...
   *  Merged semantics applies. In merged semantics, the length is the correct
   * total length of the edges. Without merged semantics, overlapping parts are
   * counted twice.
   *
   ...
   * will be kept in the output edge collection.
   *
   *  Merged semantics applies. In merged semantics, the filter will run over
   *  all merged edges.
   */
   ...
   *  all merged edges.
   *
   *  This method will return a new edge collection with the modified and
   * filtered edges.
   */
   ...
   *  all merged edges.
   */
  void processed(Region &output,
                 const EdgeToPolygonProcessorBase &filter) const;

  /**
   *  @brief Processes the edges into edge pairs
   *
   *  This method will run the processor over all edges and return an edge pair
   * collection with the outputs of the processor.
   *
   *  Merged semantics applies. In merged semantics, the filter will run over
   *  all merged edges.
   */
   ...
   *
   *  "options" specifies various options to configure the check and it's
   * output.
   *
   ...
   */
  EdgePairs width_check(db::Coord d, const db::EdgesCheckOptions &options =
                                         db::EdgesCheckOptions()) const {
   ...
   *  Merged semantics applies.
   */
  EdgePairs space_check(db::Coord d, const db::EdgesCheckOptions &options =
                                         db::EdgesCheckOptions()) const {
    return EdgePairs(mp_delegate->space_check(d, options));
  }

  /**
   *  @brief Applies an enclosing check and returns EdgePairs which correspond
   * to violation markers
   *
   *  The check will return true for edges from this edge set and the other edge
   * set, where the other edge is located on the "inside" side of the edge from
   * this edge set, the orientation is parallel and the distance is less than
   * the specified threshold d.
   *
  ...
  EdgePairs enclosing_check(
      const Edges &other, db::Coord d,
      const db::EdgesCheckOptions &options = db::EdgesCheckOptions()) const {
    return EdgePairs(mp_delegate->enclosing_check(other, d, options));
  }

  /**
   *  @brief Applies an overlap check and returns EdgePairs which correspond to
   * violation markers
   *
   *  The check will return true for edges from this edge set and the other edge
   * set, where the other edge is located on the "inside" side of the edge from
   * this edge set, the orientation is anti-parallel and the distance is less
   * than the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   * edges will be those of "other".
   *
  ...
  /**
   *  @brief Applies an separation check and returns EdgePairs which correspond
   * to violation markers
   *
   *  The check will return true for edges from this edge set and the other edge
   * set, where the other edge is located on the "outside" side of the edge from
   * this edge set, the orientation is anti-parallel and the distance is less
   * than the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   ...
   *
   *  The check will return true for edges from this edge set and the other edge
   * set, where the other edge is located on the "outide" side of the edge from
   * this edge set, the orientation is parallel and the distance is less than
   * the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   ...
   */
  template <class T> Edges &transform(const T &trans);

  /**
   *  @brief Returns the transformed edge set
   */
  template <class T> Edges transformed(const T &trans) const {
    Edges d(*this);
    d.transform(trans);
    return d;
  }
   ...
   */
  void swap(db::Edges &other) { std::swap(other.mp_delegate, mp_delegate); }

  /**
   *  @brief Merges the edge set
   *
   *  This method merges the edges of the edge set if they are not merged
   * already. It returns a reference to this edge set. An out-of-place merge
   * version is "merged".
   */
  Edges &merge() {
   ...
   *
   *  This is the out-of-place merge. It returns a new edge set but does not
   * modify the edge set it is called on. An in-place version is "merge".
   */
  Edges merged() const { return Edges(mp_delegate->merged()); }
   ...
   *
   *  This method does not necessarily merge the edge set. To ensure the edge
   * set is merged, call merge afterwards.
   */
  Edges &operator&=(const Edges &other) {
  ...
  std::pair<Edges, Edges> andnot(const Edges &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->andnot_with(other);
   ...
   *
   *  Edges are extended by creating a rectangle on each edge. The rectangle is
   * constructed on the edge by applying the extensions given by ext_o, ext_b,
   * ext_e, ext_i at the outside, the beginning, the end and the inside. If the
   * edge is laid flat pointing from left to right, the outside is at the top,
   * the inside is at the bottom.
   *
   *  For degenerated edges with length 0, the orientation is assumed to the
   * horizontal. The extended method creates a rectangle with specified
   * dimensions: ext_b to the left, ext_o to the top, ext_i to the bottom and
   * ext_e to the right.
   *
   *  If the joined parameter is set to true, adjacent edges are joined before
   * the extension is applied. A the join points, the extension is created
   * similar to what the sizing function does.
   *
   *  Note: the output is given as an out parameter since because of the include
   * hierarchy we can't use Region as a return value directly.
   *
   *  Merged semantics applies.
   */
  void extended(Region &output, coord_type ext_b, coord_type ext_e,
                coord_type ext_o, coord_type ext_i, bool join = false) const;

  /**
   *  @brief Returns edges (point-like) representing the start part of the edges
   *
   *  The length of the part can be chosen by length or a fraction of the
   * original length. If length and fraction are 0, a point at the beginning of
   * the edge will be created. If length is non-zero and fraction is 0, a
   * segment in the direction of the edge with the given length is created, even
   * if the length is larger than the original edge.
   *
   *  If fraction is given and length is 0, the segment will have a length which
   * is the specified fraction of the original edge. If both values are given,
   * the resulting edge will have a length which is the maximum of both the
   * fixed length and the length derived from the fraction.
   *
   *  Length and fraction can be negative in which case the resulting segment
   * will point in the opposite direction of the original edge.
   *
   *  Merged semantics applies.
   */
   ...
   *
   *  This method behaves similar to \start_segments but creates segments at the
   * end of the edges.
   *
   *  Merged semantics applies.
   */
   ...
   *  As a side effect, the edges are made non-intersecting by introducing cut
   * points where edges intersect.
   */
  ...
  /**
   *  @brief Returns the eges inside the given region and the ones outside the
   * region.
   *
  ...
  std::pair<Edges, Edges> inside_outside_part(const Region &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->inside_outside_part_pair(other);
    return std::pair<Edges, Edges>(Edges(p.first), Edges(p.second));
  }

  /**
   *  @brief Selects all polygons of the other region set which overlap or touch
   * edges from this edge set
   *
   *  Merged semantics applies for the other region. Merged polygons will be
   * selected from the other region if merged semantics is enabled.
   */
  void pull_interacting(Region &output, const Region &other) const;

  /**
   *  @brief Selects all edges of the other edge set which overlap or touch
   * edges from this edge set
   *
   *  Merged semantics applies. If merged semantics is chosen, the connected
   * edge parts will be selected as a whole from other.
   */
  ...
  /**
   *  @brief Selects all edges of this edge set which overlap or touch with
   * polygons from the region
   *
   *  Merged semantics applies. If merged semantics is chosen, the connected
   * edge parts will be selected as a whole.
   */
  ...
  /**
   *  @brief Returns all edges of this edge set which overlap or touch with
   * polygons from the region
   *
  ...
  /**
   *  @brief Selects all edges of this edge set which do not overlap or touch
   * with polygons from the region
   *
   *  Merged semantics applies. If merged semantics is chosen, the connected
   * edge parts will be selected as a whole.
   */
  ...
  /**
   *  @brief Returns all edges of this edge set which do not overlap or touch
   * with polygons from the region
   *
  ...
  /**
   *  @brief Returns all edges of this edge set which do not overlap or touch
   * with polygons from the region together with the ones that do not
   */
  std::pair<Edges, Edges>
  selected_interacting_differential(const Region &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->selected_interacting_pair(other);
    return std::pair<Edges, Edges>(Edges(p.first), Edges(p.second));
  }

  /**
   *  @brief Selects all edges of this edge collection which are completely
   * outside polygons from the region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Selects all edges of this edge collection which are not completely
   * outside polygons from the region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all edges of this edge collection which are completely
   * outside polygons from the region
   *
  ...
  /**
   *  @brief Returns all edges of this edge collection which are not completely
   * outside polygons from the region
   *
  ...
  /**
   *  @brief Returns all edges of this which are completely outside polygons
   * from the region and the opposite ones at the same time
   *
  ...
  /**
   *  @brief Selects all edges of this edge collection which are completely
   * inside polygons from the region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all edges of this which are completely inside polygons from
   * the region
   *
  ...
  /**
   *  @brief Returns all edges of this which are not completely inside polygons
   * from the region
   *
  ...
  /**
   *  @brief Returns all edges of this which are completely inside polygons from
   * the region and the opposite ones at the same time
   *
  ...
  /**
   *  @brief Selects all edges of this edge collection which are completely
   * outside edges from the other edge collection
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all edges of this which are completely outside edges from
   * the other edge collection and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_outside and
   * selected_not_outside, but faster.
   *
   ...
   */
  std::pair<Edges, Edges>
  selected_outside_differential(const Edges &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->selected_outside_pair(other);
    return std::pair<Edges, Edges>(Edges(p.first), Edges(p.second));
  }
  ...
  /**
   *  @brief Selects all edges of this edge collection which are completely
   * inside edges from the other edge collection
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all edges of this which are completely inside edgess from
  ...
  /**
   *  @brief Returns all edges of this which are not completely inside edges
   * from the other edge collection
   *
   *  This method is an out-of-place version of select_not_inside.
   *
   *  Merged semantics applies.
  ...
  /**
   *  @brief Returns all edges of this which are completely inside edges from
   * the other edge collection and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_inside and
   * selected_not_inside, but faster.
   ...
   */
  std::pair<Edges, Edges>
  selected_inside_differential(const Edges &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->selected_inside_pair(other);
    return std::pair<Edges, Edges>(Edges(p.first), Edges(p.second));
  }
  ...
  /**
   *  @brief Returns all edges of this edge set which do not overlap or touch
   * with edges from the other edge set together with the ones that do not
   */
  std::pair<Edges, Edges>
  selected_interacting_differential(const Edges &other) const {
    std::pair<db::EdgesDelegate *, db::EdgesDelegate *> p =
        mp_delegate->selected_interacting_pair(other);
    return std::pair<Edges, Edges>(Edges(p.first), Edges(p.second));
  }
  ...
  /**
   *  @brief Selects all edges of this edge set which do not overlap or touch
   * with edges from the other edge set
   *
   *  Merged semantics applies. If merged semantics is chosen, the connected
   * edge parts will be selected as a whole.
   */
  ...
  /**
   *  @brief Returns all edges which are in the other edge set and which are
   * not.
   *
   *  This method is equivalent to calling in with invert = false and true, but
   * more efficient.
   *
   *  Merged semantics applies.
   */
   ...
   */
  const db::Edge *nth(size_t n) const { return mp_delegate->nth(n); }
   ...
   *
   *  If the region has valid edges, it is permissable to use the edge's
   * addresses from the iterator. Furthermore, the random access operator nth()
   * is available.
   */
  bool has_valid_edges() const { return mp_delegate->has_valid_edges(); }
  ...
  /**
   *  @brief Returns true, if the edge set has valid merged edges stored within
   * itself
   *
   *  If the region has valid merged edges, it is permissable to use the edge's
   * addresses from the merged edge iterator. Furthermore, the random access
   * operator nth() is available.
   */
   ...
   *  If the edge collection is a hierarchical region, the hierarchy is copied
   * into the layout's hierarchy.
   */
