...
NetTerminalRef::NetTerminalRef() : m_terminal_id(0), mp_device(0), mp_net(0) {
  //  .. nothing yet ..
}
...
NetTerminalRef::NetTerminalRef(const NetTerminalRef &other)
    : m_terminal_id(other.m_terminal_id), mp_device(other.mp_device),
      mp_net(0) {
  //  .. nothing yet ..
}
...
const DeviceTerminalDefinition *NetTerminalRef::terminal_def() const {
...
const DeviceClass *NetTerminalRef::device_class() const {
...
NetPinRef::NetPinRef() : m_pin_id(0), mp_net(0) {
  //  .. nothing yet ..
}

NetPinRef::NetPinRef(size_t pin_id) : m_pin_id(pin_id), mp_net(0) {
  //  .. nothing yet ..
}
...
Net::Net() : NetlistObject(), m_cluster_id(0), mp_circuit(0) {
  //  .. nothing yet ..
}
...
Net &Net::operator=(const Net &other) {
  if (this != &other) {
    ...
    for (const_subcircuit_pin_iterator i = other.begin_subcircuit_pins();
         i != other.end_subcircuit_pins(); ++i) {
      add_subcircuit_pin(*i);
    }

    for (const_pin_iterator i = other.begin_pins(); i != other.end_pins();
         ++i) {
      add_pin(*i);
    }

    for (const_terminal_iterator i = other.begin_terminals();
         i != other.end_terminals(); ++i) {
      add_terminal(*i);
    }
  }
  return *this;
...
Netlist *Net::netlist() { return mp_circuit ? mp_circuit->netlist() : 0; }
...
std::string Net::expanded_name() const {
  if (name().empty()) {
    if (cluster_id() > std::numeric_limits<size_t>::max() / 2) {
      ...
      return "$I" +
             tl::to_string((std::numeric_limits<size_t>::max() - cluster_id()) +
                           1);
    } else {
...
void Net::add_subcircuit_pin(const NetSubcircuitPinRef &pin) {
  ...
  new_pin.subcircuit()->set_pin_ref_for_pin(new_pin.pin_id(),
                                            --m_subcircuit_pins.end());
}
...
void Net::erase_subcircuit_pin(subcircuit_pin_iterator iter) {
  if (iter->subcircuit()) {
    iter->subcircuit()->set_pin_ref_for_pin(iter->pin_id(),
                                            subcircuit_pin_iterator());
  }
...
void Net::add_terminal(const NetTerminalRef &terminal) {
  ...
  new_terminal.device()->set_terminal_ref_for_terminal(
      new_terminal.terminal_id(), --m_terminals.end());
}

void Net::erase_terminal(terminal_iterator iter) {
  if (iter->device()) {
    iter->device()->set_terminal_ref_for_terminal(iter->terminal_id(),
                                                  terminal_iterator());
  }
  m_terminals.erase(iter);
}

void Net::set_circuit(Circuit *circuit) { mp_circuit = circuit; }
