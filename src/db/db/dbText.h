...
public:
  ...
  /**
   *  @brief Decrement the reference counter and remove the object when it
   * reaches 0
   */
  void remove_ref() {
    --m_ref_count;
   ...
   */
  const StringRepository *rep() const { return mp_rep; }
...
private:
   ...
   */
  StringRef(StringRepository *rep) : mp_rep(rep), m_ref_count(0) {
    //  .. nothing yet ..
  }
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const StringRef &x, bool no_self = false,
                     void *parent = 0) {
...
public:
  ...
  ~StringRepository() {
    ...
    for (std::set<StringRef *>::const_iterator s = st.begin(); s != st.end();
         ++s) {
      delete *s;
    }
  }
   ...
   */
  size_t size() const { return m_string_refs.size(); }

  /**
   *  @brief Iterates over the string refs (begin)
   */
  iterator begin() const { return m_string_refs.begin(); }

  /**
   *  @brief Iterates over the string refs (end)
   */
  iterator end() const { return m_string_refs.end(); }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    for (std::set<StringRef *>::const_iterator r = m_string_refs.begin();
         r != m_string_refs.end(); ++r) {
 ...
 */
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const StringRepository &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
template <class C> class DB_PUBLIC text {
public:
   ...
   *  @param f The font id
   */
  text(const trans_type &t, coord_type h = 0, Font f = NoFont,
       HAlign halign = NoHAlign, VAlign valign = NoVAlign)
      : mp_ptr(0), m_trans(t), m_size(h), m_font(f), m_halign(halign),
        m_valign(valign) {
    //  .. nothing yet ..
  }
   ...
   *  @param f The font id
   */
  text(const StringRef *sref, const trans_type &t, coord_type h = 0,
       Font f = NoFont, HAlign halign = NoHAlign, VAlign valign = NoVAlign)
   ...
   *  @param f The font id
   */
  text(const char *s, const trans_type &t, coord_type h = 0, Font f = NoFont,
       HAlign halign = NoHAlign, VAlign valign = NoVAlign)
      : m_trans(t), m_size(h), m_font(f), m_halign(halign), m_valign(valign) {
   ...
   *  @param f The font id
   */
  text(const std::string &s, const trans_type &t, coord_type h = 0,
       Font f = NoFont, HAlign halign = NoHAlign, VAlign valign = NoVAlign)
      : m_trans(t), m_size(h), m_font(f), m_halign(halign), m_valign(valign) {
   ...
   */
  text()
      : mp_ptr(0), m_trans(), m_size(0), m_font(NoFont), m_halign(NoHAlign),
        m_valign(NoVAlign) {
    // .. nothing yet ..
  }
   ...
   */
  template <class D> text &operator=(const text<D> &d) {
    m_trans = trans_type(d.m_trans);
   ...
   */
  void translate(const text<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    //  don't use StringRef's on translate - since those live in the source
    //  layout, we must not copy them
    m_trans = d.m_trans;
    m_size = d.m_size;
    m_font = d.m_font;
    m_halign = d.m_halign;
   ...
   */
  template <class T>
  void translate(const text<C> &d, const T &t, db::generic_repository<C> &r,
                 db::ArrayRepository &a) {
    translate(d, r, a);
    transform(t);
  }
   ...
   *
   *  Calling this method basically disconnects the text from the string
   * repository and should be used if a text object is about to be transferred
   * to another layout.
   */
  void resolve_ref() {
   ...
   */
  bool operator!=(const text<C> &b) const { return !operator==(b); }
   ...
   */
  bool not_equal(const text<C> &b) const { return !equal(b); }
   ...
   */
  void trans(const trans_type &t) { m_trans = t; }

  /**
   *  @brief The transformation accessor
   */
  const trans_type &trans() const { return m_trans; }

  /**
   *  @brief The size write accessor
   */
  void size(coord_type s) { m_size = s; }

  /**
   *  @brief The size accessor
   */
  coord_type size() const { return m_size; }

  /**
   *  @brief The font id write accessor
   */
  void font(Font f) { m_font = f; }
   ...
   */
  void halign(HAlign a) { m_halign = a; }
   ...
   */
  void valign(VAlign a) { m_valign = a; }
   ...
   */
  template <class Tr> text<C> &transform(const Tr &t) {
    typedef typename Tr::target_coord_type target_coord_type;
    fixpoint_trans<coord_type> fp(t);
    m_trans = simple_trans<target_coord_type>((fp * m_trans.fp_trans()).rot(),
                                              t(point_type() + m_trans.disp()) -
                                                  point<target_coord_type>());
  ...
  text<typename Tr::target_coord_type> transformed(const Tr &t) const {
    ...
    size_t p = (size_t)mp_ptr;
    if (p & 1) {
      return text<target_coord_type>(
          reinterpret_cast<StringRef *>(p - 1),
          simple_trans<target_coord_type>((fp * m_trans.fp_trans()).rot(),
                                          t(point_type() + m_trans.disp()) -
                                              point<target_coord_type>()),
          t.ctrans(m_size), m_font, m_halign, m_valign);
    } else if (mp_ptr) {
      return text<target_coord_type>(
          mp_ptr,
          simple_trans<target_coord_type>((fp * m_trans.fp_trans()).rot(),
   ...
   */
  void move(const vector_type &p) { m_trans = trans_type(p) * m_trans; }
   ...
   */
  box_type box() const {
    return box_type(point_type() + m_trans.disp(),
                    point_type() + m_trans.disp());
  }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    if (!no_self) {
      stat->add(typeid(text<C>), (void *)this, sizeof(text<C>), sizeof(text<C>),
                parent, purpose, cat);
    }
    size_t p = (size_t)mp_ptr;
    if (!(p & 1) && mp_ptr != 0) {
      stat->add(typeid(char *), (void *)mp_ptr, strlen(mp_ptr) + 1,
                strlen(mp_ptr) + 1, (void *)this, purpose, cat);
    }
  }
...
private:
  ...
  bool text_less(const text<C> &b) const {
    ...
    if (((size_t)mp_ptr & 1) == 0 || ((size_t)b.mp_ptr & 1) == 0) {
      ...
      if (mp_ptr != b.mp_ptr) {
        //  if references are present, use their pointers rather than the
        //  strings if they belong to the same collection
...
#if 1
    //  Compare size and presentation flags - without that, the text repository
  ...
  bool text_equal(const text<C> &b) const {
    ...
    return m_font == b.m_font && m_halign == b.m_halign &&
           m_valign == b.m_valign;
#else
...
inline text<typename Tr::target_coord_type> operator*(const Tr &t,
                                                      const text<C> &s) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const text<C> &s) {
...
struct text_ref : public shape_ref<Text, Trans> {
  ...
  /**
   *  @brief The constructor creating a reference from an text pointer and
   * transformation
   *
   ...
   *
   *  This constructor allows one to copy a text reference with a certain
...
inline text_ref<Text, TargetTr> operator*(const TargetTr &t,
                                          const text_ref<Text, Tr> &p) {
 ...
 */
template <class C> inline text<double> operator*(const text<C> &t, double s) {
...
/**
 *  @brief The text reference (without transformation) typedef for double
 * coordinates
 */
...
template <class X>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const text<X> &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
