...
public:
  ...
  virtual void redo(db::Cell *cell) const { cell->swap(m_a, m_b); }

  virtual void undo(db::Cell *cell) const { cell->swap(m_a, m_b); }
...
struct SetCellPropId : public CellOp {
  ...
  virtual void redo(db::Cell *cell) const { cell->prop_id(m_to); }

  virtual void undo(db::Cell *cell) const { cell->prop_id(m_from); }
...
Cell::Cell(cell_index_type ci, db::Layout &l)
    : db::Object(l.manager()), m_cell_index(ci), mp_layout(&l),
      m_instances(this), m_prop_id(0), m_hier_levels(0),
      m_bbox_needs_update(false), m_ghost_cell(false), mp_last(0), mp_next(0) {
  //  .. nothing yet
}

Cell::Cell(const Cell &d)
    : db::Object(d), gsi::ObjectBase(), mp_layout(d.mp_layout),
      m_instances(this), m_prop_id(d.m_prop_id), m_hier_levels(d.m_hier_levels),
      mp_last(0), mp_next(0) {
  m_cell_index = d.m_cell_index;
  operator=(d);
}

Cell &Cell::operator=(const Cell &d) {
  if (this != &d) {

    //  Note: the cell index is part of the cell's identity - hence we do not
    //  change it here. It's copied in the copy ctor however.

    invalidate_hier();

    clear_shapes_no_invalidate();
    for (shapes_map::const_iterator s = d.m_shapes_map.begin();
         s != d.m_shapes_map.end(); ++s) {
...
Cell::~Cell() { clear_shapes(); }

Cell *Cell::clone(db::Layout &layout) const {
  Cell *new_cell = new Cell(cell_index(), layout);
  *new_cell = *this;
  return new_cell;
}

unsigned int Cell::layers() const {
...
bool Cell::empty() const {
  ...
  for (shapes_map::const_iterator s = m_shapes_map.begin();
...
void Cell::clear(unsigned int index) {
  ...
  if (s != m_shapes_map.end() && !s->second.empty()) {
    mp_layout->invalidate_bboxes(
        index); //  HINT: must come before the change is done!
    s->second.clear();
...
Cell::shapes_type &Cell::shapes(unsigned int index) {
  shapes_map::iterator s = m_shapes_map.find(index);
  if (s == m_shapes_map.end()) {
    s = m_shapes_map
            .insert(std::make_pair(
                index,
                shapes_type(0, this,
                            mp_layout ? mp_layout->is_editable() : true)))
            .first;
...
const Cell::shapes_type &Cell::shapes(unsigned int index) const {
...
unsigned int Cell::index_of_shapes(const Cell::shapes_type *shapes) const {
  for (shapes_map::const_iterator s = m_shapes_map.begin();
       s != m_shapes_map.end(); ++s) {
    if (&s->second == shapes) {
      return s->first;
...
void Cell::clear_shapes() {
  mp_layout->invalidate_bboxes(
      std::numeric_limits<unsigned int>::max()); //  HINT: must come before the
                                                 //  change is done!
  clear_shapes_no_invalidate();
}
...
bool Cell::update_bbox(unsigned int layers) {
  ...
  for (instances_type::sorted_inst_iterator o =
           m_instances.begin_sorted_insts();
       o != m_instances.end_sorted_insts();) {
  ...
  for (shapes_map::iterator s = m_shapes_map.begin(); s != m_shapes_map.end();
       ++s) {
...
void Cell::copy(unsigned int src, unsigned int dest) {
  if (src != dest) {
    ...
    //  When duplicating the layer, first create a copy to avoid problems with
    //  non-stable containers Hint: using the assignment and not the copy ctor
    //  does not copy the db::Manager association.
...
const Cell::box_type &Cell::bbox() const {
  mp_layout->update();
  return m_bbox;
}

const Cell::box_type &Cell::bbox(unsigned int l) const {
  mp_layout->update();
...
Cell::overlapping_iterator Cell::begin_overlapping(const box_type &b) const {
  mp_layout->update();
  return m_instances.begin_overlapping(b, mp_layout);
}

Cell::touching_iterator Cell::begin_touching(const box_type &b) const {
  mp_layout->update();
  return m_instances.begin_touching(b, mp_layout);
}

Cell::parent_inst_iterator Cell::begin_parent_insts() const {
  mp_layout->update();
  return m_instances.begin_parent_insts(mp_layout);
}

Cell::child_cell_iterator Cell::begin_child_cells() const {
  mp_layout->update();
  return m_instances.begin_child_cells();
}

size_t Cell::child_cells() const {
  mp_layout->update();
  return m_instances.child_cells();
}

size_t Cell::parent_cells() const {
  mp_layout->update();
  return m_instances.parent_cells();
}

Cell::parent_cell_iterator Cell::begin_parent_cells() const {
  mp_layout->update();
  return m_instances.begin_parent_cells();
}

Cell::parent_cell_iterator Cell::end_parent_cells() const {
  mp_layout->update();
...
bool Cell::is_leaf() const { return m_instances.empty(); }

unsigned int Cell::hierarchy_levels() const {
  mp_layout->update();
  return m_hier_levels;
}

static bool has_shapes_touching_impl(const db::Cell &cell, unsigned int layer,
                                     const db::Box &box) {
  if (!cell.shapes(layer)
           .begin_touching(box, db::ShapeIterator::All)
           .at_end()) {
    return true;
  }

  for (db::Cell::touching_iterator i = cell.begin_touching(box); !i.at_end();
       ++i) {

    for (db::CellInstArray::iterator ia = i->cell_inst().begin_touching(
             box, db::box_convert<db::CellInst>(*cell.layout(), layer));
         !ia.at_end(); ++ia) {
      ...
      if (has_shapes_touching_impl(cell.layout()->cell(i->cell_index()), layer,
                                   cbox)) {
        return true;
      }
    }
...
void Cell::collect_caller_cells(std::set<cell_index_type> &callers,
                                const std::set<cell_index_type> &cone,
                                int levels) const {
  if (levels != 0) {
    for (parent_cell_iterator cc = begin_parent_cells();
         cc != end_parent_cells(); ++cc) {
      if (cone.find(*cc) != cone.end() && callers.find(*cc) == callers.end() &&
          mp_layout->is_valid_cell_index(*cc)) {
        callers.insert(*cc);
        mp_layout->cell(*cc).collect_caller_cells(
            callers, levels < 0 ? levels : levels - 1);
      }
    }
  }
}
...
void Cell::collect_called_cells(std::set<cell_index_type> &called,
                                int levels) const {
  if (levels != 0) {
    for (child_cell_iterator cc = begin_child_cells(); !cc.at_end(); ++cc) {
      if (called.find(*cc) == called.end() &&
          mp_layout->is_valid_cell_index(*cc)) {
        called.insert(*cc);
        mp_layout->cell(*cc).collect_called_cells(
            called, levels < 0 ? levels : levels - 1);
      }
    }
  }
}
...
void Cell::redo(db::Op *op) {
  ...
  if (cell_op) {
    ...
    //  HACK: this is not really a nice concept, but it saves us a pointer to
    //  the manager.
    m_instances.redo(op);
  }
}
...
void Cell::mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                    int cat, bool no_self, void *parent) const {
  if (!no_self) {
    stat->add(typeid(Cell), (void *)this, sizeof(Cell), sizeof(Cell), parent,
              purpose, cat);
  }
  db::mem_stat(stat, purpose, cat, m_bboxes, true, (void *)this);
  db::mem_stat(stat, MemStatistics::Instances, cat, m_instances, true,
               (void *)this);
  db::mem_stat(stat, MemStatistics::ShapesInfo, cat, m_shapes_map, true,
               (void *)this);
}
...
unsigned int Cell::count_hier_levels() const {
  unsigned int l = 0;

  for (const_iterator c = begin(); !c.at_end(); ++c) {
    l = std::max(
        l, (unsigned int)mp_layout->cell(c->cell_index()).m_hier_levels + 1);
  }
...
void Cell::clear_parent_insts(size_t sz) { m_instances.clear_parent_insts(sz); }

void Cell::sort_child_insts() { m_instances.sort_child_insts(false); }
...
tl::Variant Cell::get_pcell_parameter(const instance_type &ref,
...
Cell::instance_type
Cell::change_pcell_parameters(const instance_type &ref,
                              const std::vector<tl::Variant> &new_parameters) {
  cell_index_type new_cell_index =
      mp_layout->get_pcell_variant_cell(ref.cell_index(), new_parameters);
...
std::string Cell::get_basic_name() const {
  tl_assert(layout() != 0);
  return layout()->cell_name(cell_index());
}

std::string Cell::get_qualified_name() const { return get_basic_name(); }

std::string Cell::get_display_name() const {
  tl_assert(layout() != 0);
  if (is_ghost_cell() && empty()) {
    return std::string("(") + layout()->cell_name(cell_index()) +
           std::string(")");
  } else {
    return layout()->cell_name(cell_index());
  }
...
void Cell::copy_shapes(const db::Cell &source_cell,
  ...
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot copy shapes within the same cell")));
  }
  ...
  if (!source_layout) {
    throw tl::Exception(
        tl::to_string(tr("Source cell does not reside in a layout")));
  }
...
void Cell::copy_shapes(const db::Cell &source_cell) {
  ...
  if (target_layout != source_cell.layout()) {
    ...
    for (db::Layout::layer_iterator l = target_layout->begin_layers();
         l != target_layout->end_layers(); ++l) {
...
void Cell::copy_instances(const db::Cell &source_cell) {
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot copy instances within the same cell")));
  }
  if (layout() != source_cell.layout()) {
    throw tl::Exception(
        tl::to_string(tr("Cells do not reside in the same layout")));
  }
...
std::vector<db::cell_index_type> Cell::copy_tree(const db::Cell &source_cell) {
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot copy shapes within the same cell")));
  }
  ...
  std::vector<db::cell_index_type> new_cells = cm.create_single_mapping_full(
      *target_layout, cell_index(), *source_layout, source_cell.cell_index());
  ...
  db::copy_shapes(*target_layout, *source_layout, trans, source_cells,
                  cm.table(), lm.table());

  return new_cells;
}

void Cell::copy_tree_shapes(const db::Cell &source_cell,
...
void Cell::move_shapes(db::Cell &source_cell,
  ...
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot move shapes within the same cell")));
  }
...
void Cell::move_instances(db::Cell &source_cell) {
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot move instances within the same cell")));
  }
  if (layout() != source_cell.layout()) {
    throw tl::Exception(
        tl::to_string(tr("Cells do not reside in the same layout")));
...
std::vector<db::cell_index_type> Cell::move_tree(db::Cell &source_cell) {
  if (this == &source_cell) {
    throw tl::Exception(
        tl::to_string(tr("Cannot move shapes within the same cell")));
  }
  ...
  std::vector<db::cell_index_type> source_cells;
  source_cells.push_back(source_cell.cell_index());
  db::move_shapes(*target_layout, *source_layout, trans, source_cells,
                  cm.table(), lm.table());
