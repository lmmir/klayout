 ...
 *
 *  This class implements EdgeSink. It builds polygons from the edges delivered
 * to it and outputs the polygons to another receiver (PolygonSink). The way how
 * touching corners are resolved can be specified (minimum and maximum
 * coherence). In addition, it can be specified if the resulting polygons
 * contain holes are whether the holes are attached to the hull contour by stich
 * lines.
 */
class DB_PUBLIC PolygonGenerator : public EdgeSink {
public:
   ...
   *  @brief Constructor
   *
   *  This constructor takes the polygon receiver (of which is keeps a
   * reference). It allows one to specify how holes are resolved and how
   * touching corners are resolved.
   *
   *  @param psink The polygon receiver
   *  @param resolve_holes true, if holes should be resolved into the hull using
   * stich lines
   *  @param min_coherence true, if the resulting polygons should be minimized
   * (less holes, more polygons)
   */
  PolygonGenerator(PolygonSink &psink, bool resolve_holes = true,
                   bool min_coherence = true);

  /**
   *  @brief Constructor
   *
   *  This constructor takes the simple polygon receiver (of which is keeps a
   * reference). It allows one to specify how touching corners are resolved.
   * Holes are always resolved since this is the only way to create a simple
   * polygon.
   *
   ...
   *
   *  This property should not be changed why polygons are created (between
   * start and flush)
   */
   ...
   *
   *  With this property set to false (the default), holes are resolved with a
   * single stitch line. This will create self-touching polygons finally. By
   * setting this property to true, a different hole resolution strategy is
   * chosen which resolves holes by inserting a new contour at the left of the
   * hole.
   *
   *  Using is feature will result in a larger number but less complex polygons
   * on output.
   */
   ...
   *
   *  This property should not be changed while polygons are created (between
   * start and flush)
   */
   ...
   *  @brief Disables or enable compression for polygon contours
   *
   *  This method switches the global flag and is intended for regression test
   * purposes only!
   */
 ...
 *
 *  This class implements EdgeSink. It builds simple polygons from the edges
 * delivered to it and outputs the polygons to another receiver (PolygonSink or
 * SimplePolygonSink). The polygons created form a horizontal trapezoid
 * decomposition of the full polygon.
 */
class DB_PUBLIC TrapezoidGenerator : public EdgeSink {
public:
   ...
   *
   *  This constructor takes the polygon receiver (of which is keeps a
   * reference). The trapezoids will be delivered to this sink.
   *
   *  @param psink The polygon receiver
   */
...
public:
   ...
   *  The PolygonGenerator will simply forward the EdgeSink's start method to
   * the polygon sink.
   */
  virtual void start() {}
   ...
   *  The PolygonGenerator will deliver all remaining polygons and call flush
   * then.
   */
  virtual void flush() {}
};
 ...
 */
class DB_PUBLIC SimplePolygonContainer : public SimplePolygonSink {
public:
  /**
   *  @brief Constructor specifying an external vector for storing the polygons
   */
  SimplePolygonContainer(std::vector<db::SimplePolygon> &polygons,
                         bool clear = false)
      : SimplePolygonSink(), mp_polygons(&polygons), m_clear(clear) {}

  /**
   *  @brief Constructor which tells the container to use the internal vector
   * for storing the polygons
   */
   ...
   */
  std::vector<db::SimplePolygon> &polygons() { return *mp_polygons; }
 ...
 *  or an internal vector of polygons.
 */
class DB_PUBLIC PolygonContainer : public PolygonSink {
public:
  /**
   *  @brief Constructor specifying an external vector for storing the polygons
   */
   ...
   *  @brief The polygons collected so far (const version)
   */
  const std::vector<db::Polygon> &polygons() const { return *mp_polygons; }

  /**
   *  @brief The polygons collected so far (non-const version)
   */
  std::vector<db::Polygon> &polygons() { return *mp_polygons; }
 ...
 *
 *  This class implements the PolygonSink interface and delivers the sized
 * polygons to an EdgeSink.
 */
class DB_PUBLIC SizingPolygonFilter : public PolygonSink {
public:
  /**
   *  @brief Constructor
   */
