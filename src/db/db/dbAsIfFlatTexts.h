 ...
 */
class DB_PUBLIC AsIfFlatTexts : public TextsDelegate {
public:
  AsIfFlatTexts();
  ...
  processed_to_polygons(const TextToPolygonProcessorBase &filter) const;

  virtual TextsDelegate *add_in_place(const Texts &other) { return add(other); }
...
private:
  ...
  virtual TextsDelegate *selected_interacting_generic(const Region &other,
                                                      bool inverse) const;
