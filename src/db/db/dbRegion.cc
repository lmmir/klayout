...
Region::Region() : mp_delegate(new EmptyRegion()) {
  //  .. nothing yet ..
}

Region::Region(RegionDelegate *delegate) : mp_delegate(delegate) {
  //  .. nothing yet ..
}
...
Region::Region(const RecursiveShapeIterator &si, const db::ICplxTrans &trans,
               bool merged_semantics) {
  mp_delegate = new OriginalLayerRegion(si, trans, merged_semantics);
}

Region::Region(const RecursiveShapeIterator &si, DeepShapeStore &dss,
...
Region::Region(const RecursiveShapeIterator &si, DeepShapeStore &dss,
               ...
               double area_ratio, size_t max_vertex_count) {
  mp_delegate = new DeepRegion(si, dss, trans, merged_semantics, area_ratio,
                               max_vertex_count);
}
...
Region::Region(DeepShapeStore &dss) {
  ...
  mp_delegate = new db::DeepRegion(db::DeepLayer(
      &dss, layout_index, dss.layout(layout_index).insert_layer()));
}

const db::RecursiveShapeIterator &Region::iter() const {
...
void Region::clear() { set_delegate(new EmptyRegion()); }

void Region::reserve(size_t n) { mutable_region()->reserve(n); }

template <class T> Region &Region::transform(const T &trans) {
...
template <class Sh> void Region::insert(const Sh &shape) {
...
void Region::insert(const db::Shape &shape) { mutable_region()->insert(shape); }

template <class T> void Region::insert(const db::Shape &shape, const T &trans) {
  mutable_region()->insert(shape, trans);
}

template DB_PUBLIC void Region::insert(const db::Shape &,
                                       const db::ICplxTrans &);
...
MutableRegion *Region::mutable_region() {
...
EdgePairs Region::cop_to_edge_pairs(db::CompoundRegionOperationNode &node,
...
Region Region::cop_to_region(db::CompoundRegionOperationNode &node,
                             db::PropertyConstraint prop_constraint) {
...
Edges Region::cop_to_edges(db::CompoundRegionOperationNode &node,
                           db::PropertyConstraint prop_constraint) {
...
tl::Variant Region::cop(db::CompoundRegionOperationNode &node,
                        db::PropertyConstraint prop_constraint) {
  if (node.result_type() == db::CompoundRegionOperationNode::EdgePairs) {
    return tl::Variant::make_variant(
        new EdgePairs(mp_delegate->cop_to_edge_pairs(node, prop_constraint)));
  } else if (node.result_type() == db::CompoundRegionOperationNode::Edges) {
    return tl::Variant::make_variant(
        new Edges(mp_delegate->cop_to_edges(node, prop_constraint)));
  } else if (node.result_type() == db::CompoundRegionOperationNode::Region) {
    return tl::Variant::make_variant(
        new Region(mp_delegate->cop_to_region(node, prop_constraint)));
  } else {
    return tl::Variant();
  }
}

Region &Region::size(coord_type d, unsigned int mode) {
  set_delegate(mp_delegate->sized(d, mode));
  return *this;
}

Region &Region::size(coord_type dx, coord_type dy, unsigned int mode) {
  set_delegate(mp_delegate->sized(dx, dy, mode));
  return *this;
}

Region Region::sized(coord_type d, unsigned int mode) const {
  return Region(mp_delegate->sized(d, mode));
}

Region Region::sized(coord_type dx, coord_type dy, unsigned int mode) const {
...
Region Region::rounded_corners(double rinner, double router,
...
Region Region::smoothed(coord_type d, bool keep_hv) const {
  return processed(SmoothingProcessor(d, keep_hv));
}

db::Region &Region::flatten() {
...
Region Region::snapped(db::Coord gx, db::Coord gy) const {
  return Region(mp_delegate->snapped(gx, gy));
}

void Region::scale_and_snap(db::Coord gx, db::Coord mx, db::Coord dx,
                            db::Coord gy, db::Coord my, db::Coord dy) {
  set_delegate(
      mp_delegate->scaled_and_snapped_in_place(gx, mx, dx, gy, my, dy));
}

Region Region::scaled_and_snapped(db::Coord gx, db::Coord mx, db::Coord dx,
                                  db::Coord gy, db::Coord my,
                                  db::Coord dy) const {
  return Region(mp_delegate->scaled_and_snapped(gx, mx, dx, gy, my, dy));
}

Region Region::strange_polygon_check() const {
  return Region(processed(StrangePolygonCheckProcessor()));
}

Region Region::holes() const {
  return Region(processed(HolesExtractionProcessor()));
}

Region Region::hulls() const {
  return Region(processed(HullExtractionProcessor()));
}

namespace {

template <class Container> struct dot_delivery {
...
template <class Container> struct box_delivery {
  typedef Container container_type;

  box_delivery(db::Coord enl) : m_d(enl, enl) {
    //  .. nothing yet ..
  }
...
static void fill_texts(const Iter &iter, const std::string &pat, bool pattern,
                       const Delivery &delivery,
                       typename Delivery::container_type *container,
                       const db::ICplxTrans &trans,
                       const db::DeepRegion *org_deep) {
  std::pair<bool, db::property_names_id_type> text_annot_name_id;
  const db::Layout *layout = 0;

  if (org_deep) {
    //  NOTE: deep regions can store texts in a special way - as small boxes
    //  with a special property attached. The property will give the text
    //  string. This function can restore these pseudo-texts as Text objects.
    ...
    if (!store->text_property_name().is_nil()) {
      text_annot_name_id = layout->properties_repository().get_id_of_name(
          store->text_property_name());
    }
  }
  ...
  for (Iter si = iter; !si.at_end(); ++si) {
    ...
    if (si->is_text()) {
      ...
      const db::PropertiesRepository::properties_set &ps =
          layout->properties_repository().properties(si->prop_id());
    ...
    if (is_text && (all || (pattern && glob_pat.match(text_string)) ||
                    (!pattern && text_string == pat))) {
...
class text_shape_receiver : public db::HierarchyBuilderShapeReceiver {
public:
  text_shape_receiver(const Delivery &delivery, const std::string &pat,
                      bool pattern, const db::DeepRegion *org_deep)
      : m_delivery(delivery), m_glob_pat(), m_all(false), m_pattern(pattern),
        m_pat(pat), m_text_annot_name_id(false, 0), mp_layout(0) {
    if (org_deep) {
      mp_layout = &org_deep->deep_layer().layout();
      const db::DeepShapeStore *store = org_deep->deep_layer().store();
      if (!store->text_property_name().is_nil()) {
        m_text_annot_name_id =
            mp_layout->properties_repository().get_id_of_name(
                store->text_property_name());
      }
    }
  ...
  push(const db::Shape &shape, db::properties_id_type,
    ...
    if (shape.is_text()) {
      ...
      //  a text marker
      const db::PropertiesRepository::properties_set &ps =
          mp_layout->properties_repository().properties(shape.prop_id());
    ...
    if (is_text && (m_all || (m_pattern && m_glob_pat.match(text_string)) ||
                    (!m_pattern && text_string == m_pat))) {
      ...
      if (!complex_region) {
        ...
        if (!complex_region
                 ->begin_overlapping(db::Box(pt, pt),
  ...
  virtual void push(const db::Box &, db::properties_id_type,
                    const db::ICplxTrans &, const db::Box &,
                    const db::RecursiveShapeReceiver::box_tree_type *,
                    db::Shapes *) {}
  virtual void push(const db::Polygon &, db::properties_id_type,
                    const db::ICplxTrans &, const db::Box &,
                    const db::RecursiveShapeReceiver::box_tree_type *,
                    db::Shapes *) {}
...
Edges Region::texts_as_dots(const std::string &pat, bool pattern) const {
  ...
  if (dr) {
    return texts_as_dots(
        pat, pattern,
        const_cast<db::DeepShapeStore &>(*dr->deep_layer().store()));
  }
  ...
  fill_texts(si.first, pat, pattern, dot_delivery<db::FlatEdges>(), res.get(),
             si.second, dr);

  return Edges(res.release());
}

Edges Region::texts_as_dots(const std::string &pat, bool pattern,
  ...
  text_shape_receiver<dot_delivery<db::Shapes>> pipe =
      text_shape_receiver<dot_delivery<db::Shapes>>(dot_delivery<db::Shapes>(),
                                                    pat, pattern, dr);
  if (dr && dr->deep_layer().store() == &store) {
    return Edges(new db::DeepEdges(store.create_copy(dr->deep_layer(), &pipe)));
  } else {
    return Edges(new db::DeepEdges(
        store.create_custom_layer(si.first, &pipe, si.second)));
  }
}

Region Region::texts_as_boxes(const std::string &pat, bool pattern,
                              db::Coord enl) const {
  const db::DeepRegion *dr = dynamic_cast<const db::DeepRegion *>(delegate());
  if (dr) {
    return texts_as_boxes(
        pat, pattern, enl,
        const_cast<db::DeepShapeStore &>(*dr->deep_layer().store()));
  }
  ...
  fill_texts(si.first, pat, pattern, box_delivery<db::FlatRegion>(enl),
             res.get(), si.second, dr);
...
Region Region::texts_as_boxes(const std::string &pat, bool pattern,
  ...
  text_shape_receiver<box_delivery<db::Shapes>> pipe =
      text_shape_receiver<box_delivery<db::Shapes>>(
          box_delivery<db::Shapes>(enl), pat, pattern, dr);
  if (dr && dr->deep_layer().store() == &store) {
    return Region(
        new db::DeepRegion(store.create_copy(dr->deep_layer(), &pipe)));
  } else {
    return Region(new db::DeepRegion(
        store.create_custom_layer(si.first, &pipe, si.second)));
  }
}
