...
public:
   ...
   */
  Device *device() { return mp_device; }

  /**
   *  @brief Gets the device reference (const version)
   */
  const Device *device() const { return mp_device; }

  /**
   *  @brief Gets the terminal index
   */
  size_t terminal_id() const { return m_terminal_id; }
   ...
   */
  Net *net() { return mp_net; }

  /**
   *  @brief Gets the net the terminal lives in (const version)
   */
  const Net *net() const { return mp_net; }
...
private:
   ...
   */
  void set_net(Net *net) { mp_net = net; }
};
...
public:
   ...
   */
  size_t pin_id() const { return m_pin_id; }
...
public:
   ...
   */
  SubCircuit *subcircuit() { return mp_subcircuit; }

  /**
   *  @brief Gets the subcircuit reference (const version)
   */
  const SubCircuit *subcircuit() const { return mp_subcircuit; }
 ...
 */
class DB_PUBLIC Net : public db::NetlistObject {
public:
   ...
   */
  const_terminal_iterator end_terminals() const { return m_terminals.end(); }

  /**
   *  @brief Begin iterator for the terminals of the net (non-const version)
   */
  terminal_iterator begin_terminals() { return m_terminals.begin(); }

  /**
   *  @brief End iterator for the terminals of the net (non-const version)
   */
  terminal_iterator end_terminals() { return m_terminals.end(); }

  /**
   *  @brief Returns true, if the net is floating (there is no device, no
   * subcircuit and no pin)
   */
  ...
  /**
   *  @brief Returns true, if the net is passive (there is no active element on
   * the net)
   */
  ...
  /**
   *  @brief Returns true, if the net is an internal node (connects two
   * terminals only)
   */
  bool is_internal() const {
    return m_pins.size() == 0 && m_subcircuit_pins.size() == 0 &&
           m_terminals.size() == 2;
  }
   ...
   */
  size_t subcircuit_pin_count() const { return m_subcircuit_pins.size(); }

  /**
   *  @brief Returns the number of terminals connected
   */
  size_t terminal_count() const { return m_terminals.size(); }
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Net &x, bool no_self, void *parent) {
