 ...
 */
template <class ET> struct instances_editable_traits;

template <> struct instances_editable_traits<InstancesEditableTag> {
  typedef tl::true_tag is_editable;

  template <class Tag> struct tree_traits {
    typedef db::box_tree<typename Tag::object_type::box_type,
                         typename Tag::object_type,
                         db::box_convert<typename Tag::object_type, false>>
        tree_type;
    typedef typename tree_type::iterator tree_iter;
  };
};

template <> struct instances_editable_traits<InstancesNonEditableTag> {
  typedef tl::false_tag is_editable;

  template <class Tag> struct tree_traits {
    typedef db::unstable_box_tree<
        typename Tag::object_type::box_type, typename Tag::object_type,
        db::box_convert<typename Tag::object_type, false>>
        tree_type;
    typedef typename tree_type::iterator tree_iter;
  };
...
public:
  ...
  typedef db::object_with_properties<cell_inst_array_type>
      cell_inst_wp_array_type;
  typedef tl::reuse_vector<cell_inst_array_type>::const_iterator
      cell_inst_array_iterator_type;
  typedef tl::reuse_vector<cell_inst_wp_array_type>::const_iterator
      cell_inst_wp_array_iterator_type;

  enum object_type { TNull, TInstance };
  ...
  /**
   *  @brief Initialize a reference with an pointer to an instance with
   * properties
   *
   *  Hint: there is no const Instance, hence we use the const_cast hack.
   */
   ...
   *  Hint: there is no const Instance, hence we use the const_cast hack.
   */
  Instance(const db::Instances *instances,
  ...
  /**
   *  @brief Initialize a reference with an iterator to an instance with
   * properties
   *
   *  Hint: there is no const Instance, hence we use the const_cast hack.
   */
   ...
   * properties
   */
  Instance(db::Instances *instances,
           const cell_inst_wp_array_iterator_type &iter);
   ...
   */
  bool is_null() const { return m_type == TNull; }
  ...
  /**
   *  @brief Check if the instance array is a regular one and return the
   * parameters
   *
   *  This method is basically provided for convenience
   *  and is equivalent to ->cell_inst ().is_regular_array ().
   */
  bool is_regular_array(cell_inst_array_type::vector_type &a,
                        cell_inst_array_type::vector_type &b,
                        unsigned long &amax, unsigned long &bmax) const {
    return cell_inst().is_regular_array(a, b, amax, bmax);
  }

  /**
   *  @brief Check if the instance array is an iterated array and return the
   * parameters
   *
   *  This method is basically provided for convenience
   *  and is equivalent to ->cell_inst ().is_iterated_array ().
   */
  bool is_iterated_array(
      std::vector<cell_inst_array_type::vector_type> *v = 0) const {
  ...
  /**
   *  @brief Return the complex transformation of this instance (for a given
   * base transformation)
   *
   *  This method is basically provided for convenience
   *  and is equivalent to ->cell_inst ().complex_trans (t).
   */
  cell_inst_array_type::complex_trans_type
  complex_trans(const cell_inst_array_type::simple_trans_type &t) const {
   ...
   */
  bool is_complex() const { return cell_inst().is_complex(); }
   ...
   */
  size_t size() const { return cell_inst().size(); }
  ...
  cell_inst_array_type::box_type
  bbox(const db::box_convert<cell_inst_type, AllowEmpty> &bc) const {
   ...
   *
   *  This method uses the pointers provided internally to identify container
   * and cell
   */
   ...
   */
  cell_inst_array_type::iterator begin() const { return cell_inst().begin(); }
   ...
   */
  cell_inst_array_type::iterator
  begin_touching(const cell_inst_array_type::box_type &b,
                 const layout_type *g) const;

  /**
   *  @brief Equality
   *
   *  Equality of instances is not specified by the identity of the objects but
   * by the identity of the pointers - both instances must reference the same
   * object.
   */
   ...
   *
   *  In contrast to the operator==, this method compares values rather than
   * pointers.
   */
   ...
   */
  bool operator!=(const Instance &d) const { return !operator==(d); }

  /**
   *  @brief Less operator
   *
   *  Ordering of instances is not specified by the identity of the objects but
   * by the order of the pointers. Since pointers are volatile objects, the
   * ordering is not strictly reproducible! The order is designed such that
   * different types of instances are separated in an order sequence of instance
   * proxies. This is an important fact for the "erase" functionality of the
   * instances container when erasing a set of objects.
   */
  bool operator<(const Instance &d) const;
   ...
   *
   *  In contrast to operator<, this method compares values rather than
   * pointers.
   */
   ...
   */
  const cell_inst_wp_array_type *
  basic_ptr(cell_inst_wp_array_type::tag /*tag*/) const {
   ...
   */
  const cell_inst_array_type *
  basic_ptr(cell_inst_array_type::tag /*tag*/) const {
    if (m_type != TInstance || m_with_props) {
      return 0;
    } else if (m_stable) {
      return ((cell_inst_array_iterator_type *)(m_generic.iter))->operator->();
   ...
   *
   *  The returned pointer to the iterator will be 0, if this is not an instance
   * with properties.
   */
  const cell_inst_wp_array_iterator_type *
  basic_iter(cell_inst_wp_array_type::tag /*tag*/) const {
   ...
   *  @brief Get the basic pointer to a normal instances
   *
   *  The returned pointer to the iterator will be 0, if this is an instance
   * with properties.
   */
  const cell_inst_array_iterator_type *
  basic_iter(cell_inst_array_type::tag /*tag*/) const {
    if (m_type != TInstance || m_with_props || !m_stable) {
      return 0;
    } else {
  ...
  /**
   *  @brief Gets the pointer to the instance container the instance is
   * contained in
   */
  db::Instances *instances() const { return mp_instances; }
...
template <class IterTraits> class DB_PUBLIC instance_iterator {
public:
  typedef db::Layout layout_type;
  typedef db::CellInst cell_inst_type;
   ...
   *  @brief Destructor
   */
  ~instance_iterator() { release_iter(); }

  /**
   *  @brief Constructor
   */
  instance_iterator(const IterTraits &traits)
      : m_with_props(false), m_stable(traits.instances()->is_editable()),
        m_type(TInstance), m_traits(traits) {
    make_iter();
    make_next();
    update_ref();
  }
   ...
   *  @brief Inequality
   */
  bool operator!=(const instance_iterator &d) const { return !operator==(d); }

  /**
   *  @brief Access to the actual instance
   *
   *  HINT: this method returns a value, not a "const value &" because this way,
   *  we do not loose much (it is not a expensive object) but gain more
   * stability in the GSI interface, in which we need to be careful not to
   ...
   */
  const value_type *operator->() const { return &m_ref; }
   ...
   */
  bool at_end() const { return m_type == TNull; }
  ...
  iter_type &basic_iter(cell_inst_array_type::tag, InstancesNonEditableTag) {
    tl_assert(m_type == TInstance && m_stable == false &&
              m_with_props == false);
   ...
   * (const version)
   */
  const iter_type &basic_iter(cell_inst_array_type::tag,
                              InstancesNonEditableTag) const {
    tl_assert(m_type == TInstance && m_stable == false &&
              m_with_props == false);
    return *((iter_type *)m_generic.iter);
  }
   ...
   */
  stable_iter_type &basic_iter(cell_inst_array_type::tag,
   ...
   * (const version)
   */
  const stable_iter_type &basic_iter(cell_inst_array_type::tag,
                                     InstancesEditableTag) const {
    tl_assert(m_type == TInstance && m_stable == true && m_with_props == false);
    return *((stable_iter_type *)m_generic.stable_iter);
  }
   ...
   */
  iter_wp_type &basic_iter(cell_inst_wp_array_type::tag,
   ...
   * (const version)
   */
  const iter_wp_type &basic_iter(cell_inst_wp_array_type::tag,
                                 InstancesNonEditableTag) const {
    tl_assert(m_type == TInstance && m_stable == false && m_with_props == true);
    return *((iter_wp_type *)m_generic.piter);
  }
   ...
   */
  stable_iter_wp_type &basic_iter(cell_inst_wp_array_type::tag,
                                  InstancesEditableTag) {
  ...
  /**
   *  @brief Gets the basic iterator in the editable case with properties (const
   * version)
   */
  const stable_iter_wp_type &basic_iter(cell_inst_wp_array_type::tag,
                                        InstancesEditableTag) const {
    tl_assert(m_type == TInstance && m_stable == true && m_with_props == true);
    return *((stable_iter_wp_type *)m_generic.pstable_iter);
  }
...
struct DB_PUBLIC NormalInstanceIteratorTraits {
  ...
  typedef db::object_with_properties<cell_inst_array_type>
      cell_inst_wp_array_type;
  typedef db::box_convert<cell_inst_array_type, false>
      cell_inst_array_box_converter;
  typedef db::box_convert<cell_inst_wp_array_type, false>
      cell_inst_wp_array_box_converter;
  typedef db::unstable_box_tree<box_type, cell_inst_array_type,
                                cell_inst_array_box_converter>
      cell_inst_tree_type;
  typedef db::unstable_box_tree<box_type, cell_inst_wp_array_type,
                                cell_inst_wp_array_box_converter>
      cell_inst_wp_tree_type;
  typedef db::box_tree<box_type, cell_inst_array_type,
                       cell_inst_array_box_converter>
      stable_cell_inst_tree_type;
  typedef db::box_tree<box_type, cell_inst_wp_array_type,
                       cell_inst_wp_array_box_converter>
      stable_cell_inst_wp_tree_type;
  typedef db::Instances instances_type;
  typedef db::Instance instance_type;

  typedef tl::iterator_pair<cell_inst_tree_type::const_iterator> iter_type;
  typedef tl::iterator_pair<cell_inst_wp_tree_type::const_iterator>
      iter_wp_type;
  ...
  template <class Iter> size_t quad_id(const Iter & /*iter*/) const {
    return 0;
  }
  template <class Iter> box_type quad_box(const Iter & /*iter*/) const {
    return db::Box();
  }

  const instances_type *instances() const { return mp_insts; }
...
struct DB_PUBLIC TouchingInstanceIteratorTraits {
  ...
  TouchingInstanceIteratorTraits(const instances_type *insts,
                                 const box_type &box,
...
struct DB_PUBLIC OverlappingInstanceIteratorTraits {
  ...
  typedef stable_cell_inst_wp_tree_type::overlapping_iterator
      stable_iter_wp_type;

  OverlappingInstanceIteratorTraits();
  OverlappingInstanceIteratorTraits(const instances_type *insts,
                                    const box_type &box,
                                    const layout_type *layout);
...
public:
  ...
  typedef db::Instances instances_type;
  typedef db::CellInstArray basic_inst_type;
  typedef tl::vector<const basic_inst_type *>::const_iterator
      inst_iterator_type;
   ...
   *  @brief Report the end of the iterator (in non-STL schemes)
   */
  bool at_end() const { return (m_iter == m_end); }

  /**
   *  @brief Returns the weight of this child cell (the total number of
   * instances in the parent cell)
   */
  size_t weight() const;

  /**
   *  @brief Returns the number of instances of this child cell in the parent
   * cell (arefs count as one)
   */
 ...
 * instance iterator takes care of producing the right sequence.
 */
...
public:
   ...
   */
  cell_index_type parent_cell_index() const { return m_parent_cell_index; }
 ...
 *
 *  This object represents a parent instance that can be dereferenced
 * standalone. For this, it requires a "Layout" object pointer for
 * dereferencing.
 */

class DB_PUBLIC ParentInstRep : public ParentInst {
public:
  typedef db::Layout layout_type;
   ...
   *  @brief Retrieve the child instance associated with this parent instance
   *
   *  Return an pointer to the respective child instance (just the basic
   * object).
   */
  const basic_inst_type *basic_child_inst() const;

  /**
   *  @brief Compute the inverse instance by which the parent is seen from the
   * child
   */
...
public:
   ...
   *  @brief Default ctor
   */
  ParentInstIterator() : mp_layout(0), m_iter(), m_end() {}

  /**
   *  @brief Constructor
   *
   *  This creates an iterator pointing to the start of the sequence represented
   *  by the parent instance object at *iter or at the end of the parent
   * instance vector.
   */
  ParentInstIterator(const layout_type *layout,
                     tl::vector<parent_inst_type>::const_iterator iter,
                     tl::vector<parent_inst_type>::const_iterator end)
   ...
   */
  bool operator!=(const ParentInstIterator &b) const { return !operator==(b); }
   ...
   *
   *  In order not to return a reference to the temporary instance, the
   * reference is returned by value. This is important for the GSI binding since
   * this way, a copy is created which is required when the iterator moves on.
   */
  value_type operator*() const { return m_rep; }
   ...
   *  See above.
   */
  const value_type *operator->() const { return &m_rep; }
...
public:
   ...
   *  offset from the representative.
   */
  cell_index_type operator*() const { return m_iter->parent_cell_index(); }
 ...
 *
 *  Internally, the instance list uses a box tree which treats empty cells as
 * single point boxes when a all-layer search is performed. That allows
 * retrieving such instances with a search.
 */
...
public:
  ...
  typedef db::object_with_properties<cell_inst_array_type>
      cell_inst_wp_array_type;
  //  the second parameter for box_convert tells it to use a single point for
  //  empty cells.
  typedef db::box_convert<cell_inst_array_type, false>
      cell_inst_array_box_converter;
  typedef db::box_convert<cell_inst_wp_array_type, false>
  ...
  typedef db::unstable_box_tree<box_type, cell_inst_wp_array_type,
      ...
      cell_inst_wp_tree_type;
  typedef instance_iterator<NormalInstanceIteratorTraits> const_iterator;
  typedef instance_iterator<OverlappingInstanceIteratorTraits>
   ...
   *
   *  Hint: the current implementation does not translate to other array
   * repositories. This implies, that cell instance arrays must not be based on
   * array repositories currently. The current implementation does not translate
   * property Id's. It is mainly intended for 1-to-1 copies of layouts where the
   * whole property repository is copied.
   */
   ...
   *  The iterators in the sequence from, to must be sorted in
   *  "operator<" order.
   *  This method is mainly provided for optimization of the "undo" replay
   * method.
   *
   ...
   */
  template <class InstArray> instance_type insert(const InstArray &inst);

  /**
   *  @brief Insert a sequence [from,to) of cell instances in a "editable safe"
   * way
   *
   ...
   *  Only in editable mode, this method will return a stable reference - that
   * is one that will not be invalidated potentially on further insert
   * operations.
   *
  ...
  /**
   *  @brief Insert an instance given by a instance reference with a different
   * cell index and property ID
   *
   ...
   *  @param im The mapper to new cell index to use (for mapping to a different
   * layout for example)
   *  @param pm The mapper to new cell property ID use (for mapping to a
   * different layout for example)
   */
  ...
  /**
   *  @brief Replace the properties ID of an element (pointed to by the
   * iterator) with the given one
   *
   *  This method will delete the former object if it does not have a property
   * and needs to get one. It will return a reference to the new object. The
   * erase operation will potentially invalidate other references in
   * non-editable mode!
   *
   *  @return The reference to the new instance
   */
  instance_type replace_prop_id(const instance_type &ref,
                                db::properties_id_type prop_id);

  /**
   *  @brief Replace the instance pointed to by the iterator with the given
   * instance
   *
   *  If the object pointed to by ref contains properties, the properties will
   * remain.
   *
   *  @return The reference to the new object
   */
  instance_type replace(const instance_type &ref,
                        const cell_inst_array_type &inst);
   ...
   *
   *  Returns true, if the given instance is valid. It is not valid, if it has
   * been deleted already. However, it may happen, that the instance memory has
   * been reused already. Therefore this method can safely be used only if
   * nothing has been inserted into this container in between. This method can
  ...
  instance_type transform(const instance_type &ref, const Trans &t) {
    return replace(ref, ref.cell_inst().transformed(
                            t, 0 /*don't consider array repository*/));
  }

  /**
   *  @brief Transforms the given instance into a new system
   *
   *  This method transforms the instance while assuming that the transformation
   * will be propagated further. In order worlds, the instance will be valid in
   * a new coordinate system derived from the current one with the given
   * transformation.
   *
   *  @param t The transformation to apply
   */
  ...
  instance_type transform_into(const instance_type &ref, const Trans &t) {
    return replace(ref, ref.cell_inst().transformed_into(
                            t, 0 /*don't consider array repository*/));
  }
   ...
   *  @param t The transformation to apply
   */
  template <class Trans> void transform(const Trans &t);
   ...
   *  @param t The transformation to apply
   *
   *  See the single-instance "transform_into" method for an explanation of this
   * transformation variant.
   */
  template <class Trans> void transform_into(const Trans &t);
  ...
  /**
   *  @brief Establish the instance index list giving the instances by cell
   * index If force is true, the instance tree is always sorted.
   */
   ...
   */
  overlapping_iterator begin_overlapping(const box_type &b,
                                         const layout_type *g) const {
   ...
   *  @return The region iterator for the child cell instances
   */
  touching_iterator begin_touching(const box_type &b,
                                   const layout_type *g) const {
  ...
  /**
   *  @brief The iterator delivering the cell instance (indices) sorted by child
   * cell ID
   */
   ...
   */
  size_t parent_cells() const { return m_parent_insts.size(); }
   ...
   */
  bool is_top() const { return m_parent_insts.end() == m_parent_insts.begin(); }
  ...
  /**
   *  @brief Returns true, if this Instances container belongs to an editable
   * cell
   */
...
private:
  ...
  void set_instance_tree_needs_sort(bool f) {
    mp_cell = reinterpret_cast<cell_type *>((size_t(mp_cell) & ~size_t(1)) |
                                            size_t(f ? 1 : 0));
  }
   ...
   *  @brief Sets a flag indicating that the instance tree needs sorting
   */
  bool instance_tree_needs_sort() const { return (size_t(mp_cell) & 1) != 0; }

  /**
   *  @brief Sets a flag indicating that the instance by cell index cache needs
   * made
   */
  void set_instance_by_cell_index_needs_made(bool f) {
    mp_cell = reinterpret_cast<cell_type *>((size_t(mp_cell) & ~size_t(2)) |
                                            size_t(f ? 2 : 0));
  }
  ...
  /**
   *  @brief Invalidates the instance information - called whenever something
   * changes
   */
   ...
   */
  const cell_inst_wp_tree_type &inst_tree(cell_inst_wp_array_type::tag,
                                          InstancesNonEditableTag) const {
    tl_assert(!is_editable());
    return *(m_generic_wp.unstable_tree ? m_generic_wp.unstable_tree
                                        : &ms_empty_wp_tree);
  }
   ...
   *  @brief Get the non-editable instance tree by instance type
   */
  const cell_inst_tree_type &inst_tree(cell_inst_array_type::tag,
                                       InstancesNonEditableTag) const {
    tl_assert(!is_editable());
    return *(m_generic.unstable_tree ? m_generic.unstable_tree
                                     : &ms_empty_tree);
  }
   ...
   */
  const stable_cell_inst_wp_tree_type &inst_tree(cell_inst_wp_array_type::tag,
                                                 InstancesEditableTag) const {
    tl_assert(is_editable());
    return *(m_generic_wp.stable_tree ? m_generic_wp.stable_tree
                                      : &ms_empty_stable_wp_tree);
  }

  /**
   *  @brief Get the editable instance tree by instance type
   */
  const stable_cell_inst_tree_type &inst_tree(cell_inst_array_type::tag,
                                              InstancesEditableTag) const {
    tl_assert(is_editable());
    return *(m_generic.stable_tree ? m_generic.stable_tree
                                   : &ms_empty_stable_tree);
  }

  /**
   *  @brief Gets the non-editable, non-const instance tree by instance type
   */
  cell_inst_wp_tree_type &inst_tree(cell_inst_wp_array_type::tag,
                                    InstancesNonEditableTag) {
    tl_assert(!is_editable());
   ...
   *  @brief Gets the non-editable, non-const instance tree by instance type
   */
  cell_inst_tree_type &inst_tree(cell_inst_array_type::tag,
                                 InstancesNonEditableTag) {
    tl_assert(!is_editable());
   ...
   */
  stable_cell_inst_wp_tree_type &inst_tree(cell_inst_wp_array_type::tag,
                                           InstancesEditableTag) {
    tl_assert(is_editable());
   ...
   *  @brief Gets the editable, non-const instance tree by instance type
   */
  stable_cell_inst_tree_type &inst_tree(cell_inst_array_type::tag,
                                        InstancesEditableTag) {
    tl_assert(is_editable());
   ...
   */
  template <class Op, class ET> void apply_op(const Op &op, ET editable_tag);
  ...
  void erase_inst_by_tag(Tag tag, ET editable_tag,
                         const typename Tag::object_type &obj);

  template <class Tag, class ET>
  void erase_insts_by_tag(Tag tag, ET editable_tag,
                          std::vector<instance_type>::const_iterator s1,
                          std::vector<instance_type>::const_iterator s2);

  template <class I, class ET> void insert(I from, I to);

  template <class ET> void clear_insts(ET editable_tag);
};
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Instances &x, bool no_self, void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}

template <class Iter>
inline NormalInstanceIteratorTraits::instance_type
NormalInstanceIteratorTraits::instance_from_stable_iter(
    const Iter &iter) const {
  //  box tree flat iterators deliver pointers, not iterators. Use
  //  instance_from_pointer to do this conversion.
  return mp_insts->instance_from_pointer(&*iter);
}

template <class Iter>
inline TouchingInstanceIteratorTraits::instance_type
TouchingInstanceIteratorTraits::instance_from_stable_iter(
    const Iter &iter) const {
  //  box tree iterators deliver pointers, not iterators. Use
  //  instance_from_pointer to do this conversion.
  return mp_insts->instance_from_pointer(&*iter);
}
...
OverlappingInstanceIteratorTraits::instance_from_stable_iter(
    const Iter &iter) const {
  //  box tree iterators deliver pointers, not iterators. Use
  //  instance_from_pointer to do this conversion.
  return mp_insts->instance_from_pointer(&*iter);
