...
inline bool is_subject_regionptr(const db::Region *ptr) {
  return ptr == subject_regionptr() || ptr == foreign_regionptr();
}
...
public:
  template <class TR>
  std::pair<bool, std::vector<std::unordered_set<TR>> *>
  get(const CompoundRegionOperationNode *node) {
    bool valid = false;
...
private:
  std::map<const CompoundRegionOperationNode *,
           std::vector<std::unordered_set<db::PolygonRef>>>
      m_cache_polyref;
  std::map<const CompoundRegionOperationNode *,
           std::vector<std::unordered_set<db::Polygon>>>
      m_cache_poly;
  std::map<const CompoundRegionOperationNode *,
           std::vector<std::unordered_set<db::Edge>>>
      m_cache_edge;
  std::map<const CompoundRegionOperationNode *,
  ...
  template <class TR>
  void get_cache_generic(std::map<const CompoundRegionOperationNode *,
                                  std::vector<std::unordered_set<TR>>> &caches,
                         std::vector<std::unordered_set<TR>> *&cache_ptr,
                         bool &valid, const CompoundRegionOperationNode *node) {
    typename std::map<const CompoundRegionOperationNode *,
                      std::vector<std::unordered_set<TR>>>::iterator c =
        caches.find(node);
    if (c != caches.end()) {
      valid = true;
      cache_ptr = &c->second;
  ...
  void get_cache(std::vector<std::unordered_set<db::PolygonRef>> *&cache_ptr,
                 bool &valid, const CompoundRegionOperationNode *node) {
    get_cache_generic(m_cache_polyref, cache_ptr, valid, node);
  }
  void get_cache(std::vector<std::unordered_set<db::Polygon>> *&cache_ptr,
                 bool &valid, const CompoundRegionOperationNode *node) {
    get_cache_generic(m_cache_poly, cache_ptr, valid, node);
  }
  void get_cache(std::vector<std::unordered_set<db::Edge>> *&cache_ptr,
                 bool &valid, const CompoundRegionOperationNode *node) {
    get_cache_generic(m_cache_edge, cache_ptr, valid, node);
  }
  void get_cache(std::vector<std::unordered_set<db::EdgePair>> *&cache_ptr,
                 bool &valid, const CompoundRegionOperationNode *node) {
    get_cache_generic(m_cache_edge_pair, cache_ptr, valid, node);
  }
};
 ...
 *  will act as the main entrance and is fed into a local processor for
 * performing the complex operation.
 *
 *  The tree nodes will
 */

class DB_PUBLIC CompoundRegionOperationNode : public gsi::ObjectBase,
                                              public tl::Object {
public:
   ...
   */
  const std::string &raw_description() const { return m_description; }

  /**
   *  @brief Gets the interaction distance of this operation
   *  The returned value will be the minimum distance (set_dist) or the computed
   * distance (compute_dist), whichever is larger.
   */
  db::Coord dist() const { return std::max(m_dist, computed_dist()); }

  /**
   *  @brief Sets a minmum distance
   */
  void set_dist(db::Coord d) { m_dist = d; }
  ...
  /**
   *  @brief Returns a value indicating whether this node provides external
   * inputs A node has external inputs if it has inputs and does not refer to
   * the primary alone.
   */
  virtual bool has_external_inputs() const;

  /**
   *  @brief Returns a value giving a hint how to handle the case of empty
   * intruders
   */
  //  NOTE: it's probably going to be difficult to compute a different value
  //  here:
   ...
   *  The returned vector may use subject_regionptr() or forein_regionptr() as
   * abstract values for the subject layer and the subject layer in "foreign"
   * mode (asymmetric: other subjects are treated as different shapes).
   */
   ...
   *  If not true, the processor accepts shape propagation as variant
   * resolution.
   */
  ...
  void compute_local(
      CompoundRegionOperationCache *cache, db::Layout *layout,
      const shape_interactions<db::Polygon, db::Polygon> &interactions,
      std::vector<std::unordered_set<db::Edge>> &results,
      size_t max_vertex_count, double area_ratio) const {
    implement_compute_local(cache, layout, interactions, results,
                            max_vertex_count, area_ratio);
  }
  ...
  template <class T>
  bool compute_local_bool(CompoundRegionOperationCache *cache,
                          db::Layout *layout,
    ...
    if (result_type() == Region) {
      ...
      compute_local(cache, layout, interactions, res, max_vertex_count,
                    area_ratio);
...
protected:
  ...
  virtual void do_compute_local(
      ...
      size_t /*max_vertex_count*/, double /*area_ratio*/) const {}
  virtual void
  do_compute_local(CompoundRegionOperationCache * /*cache*/,
                   db::Layout * /*layout*/,
                   const shape_interactions<db::PolygonRef, db::PolygonRef>
                       & /*interactions*/,
                   std::vector<std::unordered_set<db::Edge>> & /*results*/,
...
private:
  ...
  void implement_compute_local(CompoundRegionOperationCache *cache,
                               db::Layout *layout,
    ...
    std::pair<bool, std::vector<std::unordered_set<TR>> *> cp =
        cache->get<TR>(this);
    ...
    if (!cp.first) {
      ...
      do_compute_local(cache, layout, interactions, uncached_results,
                       max_vertex_count, area_ratio);
...
template <class T> struct compound_operation_type_traits;

template <> struct compound_operation_type_traits<db::PolygonRef> {
  typedef db::Region container_type;
  static CompoundRegionOperationNode::ResultType type() {
    return CompoundRegionOperationNode::Region;
  }
};

template <> struct compound_operation_type_traits<db::Polygon> {
  typedef db::Region container_type;
  static CompoundRegionOperationNode::ResultType type() {
    return CompoundRegionOperationNode::Region;
  }
...
template <> struct compound_operation_type_traits<db::Edge> {
...
template <> struct compound_operation_type_traits<db::EdgePair> {
  typedef db::EdgePairs container_type;
  static CompoundRegionOperationNode::ResultType type() {
    return CompoundRegionOperationNode::EdgePairs;
  }
};
...
public:
  ...
  CompoundRegionMultiInputOperationNode(CompoundRegionOperationNode *a,
  ...
  virtual std::vector<db::Region *> inputs() const { return m_inputs; }
...
protected:
  ...
  const shape_interactions<TS, TI> &
  interactions_for_child(const shape_interactions<TS, TI> &interactions,
                         unsigned int child_index,
                         shape_interactions<TS, TI> &child_interactions) const {
    ...
    for (typename shape_interactions<TS, TI>::iterator i = interactions.begin();
      ...
      if (child(child_index)->on_empty_intruder_hint() ==
        ...
        child_interactions.add_subject_shape(
            i->first, interactions.subject_shape(i->first));
      } else {
        //  this includes the subject-without-intruder "interaction"
        child_interactions.add_subject(i->first,
                                       interactions.subject_shape(i->first));
      }

      for (typename shape_interactions<TS, TI>::iterator2 ii =
               i->second.begin();
           ii != i->second.end(); ++ii) {

        const std::pair<unsigned int, TI> &is =
            interactions.intruder_shape(*ii);

        std::map<std::pair<unsigned int, unsigned int>,
                 unsigned int>::const_iterator lm =
            m_map_layer_to_child.find(std::make_pair(child_index, is.first));
  ...
  unsigned int children() const { return (unsigned int)m_children.size(); }
...
public:
  ...
  CompoundRegionLogicalBoolOperationNode(
...
private:
  ...
  void implement_compute_local(CompoundRegionOperationCache *cache,
                               db::Layout *layout,
                               const shape_interactions<T, T> &interactions,
                               std::vector<std::unordered_set<T>> &results,
                               size_t max_vertex_count,
                               double area_ratio) const;
};
...
class DB_PUBLIC CompoundRegionCountFilterNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionCountFilterNode(CompoundRegionOperationNode *input, bool invert,
  ...
  //  specifies the result type
  virtual ResultType result_type() const { return child(0)->result_type(); }
...
private:
  ...
  void implement_compute_local(CompoundRegionOperationCache *cache,
    ...
    child(0)->compute_local(cache, layout, interactions, one, max_vertex_count,
                            area_ratio);

    if ((one.front().size() >= m_min_count &&
         one.front().size() < m_max_count) != m_invert) {
...
private:
  ...
  void implement_bool(CompoundRegionOperationCache *cache, db::Layout *layout,
                      const shape_interactions<T, T> &interactions,
                      std::vector<std::unordered_set<TR>> &results,
                      size_t max_vertex_count, double area_ratio) const;
};
...
public:
  /**
   *  @brief Constructor
   *  NOTE: the inputs must be given for subjects and one to many intruder
   * layers. Input 0 is for the subject, input 1 for the first intruder etc. If
   * original layers are required for input, CompoundRegionOperationPrimaryNode
   * or CompoundRegionOperationSecondaryNode nodes need to be provided. If no
   * derived node is requested for input, the input-less constructor may be used
   * which is more efficient.
   */
  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op,
      const std::vector<CompoundRegionOperationNode *> &inputs,
      const db::TransformationReducer *vars = 0, bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(inputs), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants) {
    //  .. nothing yet ..
  }

  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op,
      CompoundRegionOperationNode *input,
      const db::TransformationReducer *vars = 0, bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(input), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants) {
    //  .. nothing yet ..
  }

  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op, CompoundRegionOperationNode *a,
      CompoundRegionOperationNode *b, const db::TransformationReducer *vars = 0,
      bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(a, b), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants) {
    //  .. nothing yet ..
  }

  virtual ResultType result_type() const {
    return compound_operation_type_traits<TR>::type();
  }
  ...
  virtual db::Coord computed_dist() const {
    return CompoundRegionMultiInputOperationNode::computed_dist() +
           m_op->dist();
  }
...
protected:
  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op,
      const std::vector<db::Region *> &inputs,
      const db::TransformationReducer *vars = 0, bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants), m_inputs(inputs) {
    //  .. nothing yet ..
  }

  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op, db::Region *input,
      const db::TransformationReducer *vars = 0, bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants) {
  ...
  compound_region_generic_operation_node(
      const db::local_operation<TS, TI, TR> *op, db::Region *a, db::Region *b,
      const db::TransformationReducer *vars = 0, bool want_variants = false)
      : CompoundRegionMultiInputOperationNode(), m_op(op), mp_vars(vars),
        m_wants_variants(want_variants) {
  ...
  void implement_compute_local(db::CompoundRegionOperationCache *cache,
                               db::Layout *layout,
...
private:
  ...
  std::vector<db::Region *> m_inputs;
  //  required if the inner processor is a PolygonRef type while the outer
  //  interface needs Polygon
  db::Layout m_aux_layout;
};

class DB_PUBLIC CompoundRegionInteractOperationNode
    : public compound_region_generic_operation_node<db::Polygon, db::Polygon,
                                                    db::Polygon> {
public:
  CompoundRegionInteractOperationNode(
      CompoundRegionOperationNode *a, CompoundRegionOperationNode *b, int mode,
      bool touching, bool inverse, size_t min_count = 0,
...
private:
  typedef db::interacting_local_operation<db::Polygon, db::Polygon, db::Polygon>
      op_type;
  op_type m_op;
};

class DB_PUBLIC CompoundRegionInteractWithEdgeOperationNode
    : public compound_region_generic_operation_node<db::Polygon, db::Edge,
                                                    db::Polygon> {
public:
  CompoundRegionInteractWithEdgeOperationNode(
      CompoundRegionOperationNode *a, CompoundRegionOperationNode *b,
      bool inverse, size_t min_count = 0,
      size_t max_count = std::numeric_limits<size_t>::max());
...
private:
  db::interacting_with_edge_local_operation<db::Polygon, db::Edge, db::Polygon>
      m_op;
};
...
class DB_PUBLIC CompoundRegionPullOperationNode
    : public compound_region_generic_operation_node<db::Polygon, db::Polygon,
                                                    db::Polygon> {
public:
  CompoundRegionPullOperationNode(CompoundRegionOperationNode *a,
                                  CompoundRegionOperationNode *b, int mode,
                                  bool touching);
  CompoundRegionPullOperationNode(db::Region *a, db::Region *b, int mode,
                                  bool touching);
...
class DB_PUBLIC CompoundRegionJoinOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionJoinOperationNode(
      const std::vector<CompoundRegionOperationNode *> &inputs);
 ...
 *  The first condition is tested. If true, the "then1" result is taken.
 * Otherwise, the "if2" condition is tested and "then2" is taken if true etc. At
 * the end the default is evaluated if no other condition matched.
 */
...
class DB_PUBLIC CompoundRegionFilterOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionFilterOperationNode(PolygonFilterBase *filter,
                                    CompoundRegionOperationNode *input,
                                    bool owns_filter = false,
                                    bool sum_of_set = false);
...
private:
  ...
  void implement_compute_local(db::CompoundRegionOperationCache *cache,
    ...
    if (m_sum_of_set) {
      ...
      for (typename std::unordered_set<T>::const_iterator p =
               one.front().begin();
           p != one.front().end(); ++p) {
...
class DB_PUBLIC CompoundRegionEdgeFilterOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionEdgeFilterOperationNode(EdgeFilterBase *filter,
                                        CompoundRegionOperationNode *input,
                                        bool owns_filter = false,
                                        bool sum_of = false);
...
private:
  ...
  template <class T>
  void
  implement_compute_local(db::CompoundRegionOperationCache *cache,
                          db::Layout *layout,
                          const shape_interactions<T, T> &interactions,
                          std::vector<std::unordered_set<db::Edge>> &results,
                          size_t max_vertex_count, double area_ratio) const {
    ...
    if (m_sum_of) {
      ...
      for (typename std::unordered_set<db::Edge>::const_iterator p =
               one.front().begin();
           p != one.front().end(); ++p) {
        if (mp_filter->selected(*p)) {
          results.front().insert(*p);
...
class DB_PUBLIC CompoundRegionEdgePairFilterOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionEdgePairFilterOperationNode(EdgePairFilterBase *filter,
                                            CompoundRegionOperationNode *input,
                                            bool owns_filter = false);
...
private:
  ...
  void implement_compute_local(
    ...
    for (typename std::unordered_set<db::EdgePair>::const_iterator p =
             one.front().begin();
         p != one.front().end(); ++p) {
      if (is_selected(*p)) {
        results.front().insert(*p);
...
class DB_PUBLIC CompoundRegionProcessingOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionProcessingOperationNode(PolygonProcessorBase *proc,
                                        CompoundRegionOperationNode *input,
                                        bool owns_proc = false,
                                        db::Coord dist_adder = 0);
  ...
  virtual db::Coord computed_dist() const {
    return m_dist_adder +
           CompoundRegionMultiInputOperationNode::computed_dist();
  }
...
private:
  ...
  void processed(db::Layout *, const db::Polygon &p,
                 std::vector<db::Polygon> &res) const;
  void processed(db::Layout *layout, const db::PolygonRef &p,
  ...
  void implement_compute_local(db::CompoundRegionOperationCache *cache,
    ...
    std::vector<T> res;
    for (typename std::unordered_set<T>::const_iterator p = one.front().begin();
         p != one.front().end(); ++p) {
      res.clear();
...
public:
  CompoundRegionSizeOperationNode(db::Coord dx, db::Coord dy, unsigned int mode,
                                  CompoundRegionOperationNode *input)
      : CompoundRegionProcessingOperationNode(&m_proc, input),
        m_proc(dx, dy, mode) {}

  CompoundRegionSizeOperationNode(db::Coord d, unsigned int mode,
                                  CompoundRegionOperationNode *input)
      : CompoundRegionProcessingOperationNode(&m_proc, input),
        m_proc(d, d, mode) {}

  virtual std::string description() const {
    return std::string("sized") +
           CompoundRegionProcessingOperationNode::generated_description();
  }
...
class DB_PUBLIC CompoundRegionMergeOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionMergeOperationNode(bool min_coherence, unsigned int min_wc,
                                   CompoundRegionOperationNode *input)
      : CompoundRegionMultiInputOperationNode(input),
        m_min_coherence(min_coherence), m_min_wc(min_wc) {}
...
private:
  ...
  void processed(db::Layout *, const db::Edge &p,
                 std::vector<db::Polygon> &res) const;
  void processed(db::Layout *layout, const db::Edge &p,
                 std::vector<db::PolygonRef> &res) const;
...
class DB_PUBLIC CompoundRegionEdgeProcessingOperationNode
    : public CompoundRegionMultiInputOperationNode {
public:
  CompoundRegionEdgeProcessingOperationNode(EdgeProcessorBase *proc,
                                            CompoundRegionOperationNode *input,
                                            bool owns_proc = false);
...
public:
  CompoundRegionEdgePairToPolygonProcessingOperationNode(
      EdgePairToPolygonProcessorBase *proc, CompoundRegionOperationNode *input,
      bool owns_proc = false);
  ~CompoundRegionEdgePairToPolygonProcessingOperationNode();
...
private:
  ...
  void processed(db::Layout *, const db::EdgePair &p,
                 std::vector<db::Polygon> &res) const;
  void processed(db::Layout *layout, const db::EdgePair &p,
                 std::vector<db::PolygonRef> &res) const;
 ...
 *
 *  This node takes polygons for input. It will produce edge pairs by means of
 * the PolygonToEdgePairProcessorBase object.
 */
...
public:
  /**
   *  @brief Constructor
   *  @param proc The processor which turns polygons into edge pairs (it's
   * reimplementation) - the node will *not* take ownership unless owns_proc is
   * true
   *  @param input The node for the original (the node will take ownership)
   */
  CompoundRegionToEdgePairProcessingOperationNode(
      PolygonToEdgePairProcessorBase *proc, CompoundRegionOperationNode *input,
      bool owns_proc = false);
  ~CompoundRegionToEdgePairProcessingOperationNode();
 ...
 *
 *  This node takes polygons for input. It will produce edge pairs as result of
 * the DRC check.
 */
...
public:
   ...
   *  @param input The node for the original (the node will take ownership)
   */
  CompoundRegionCheckOperationNode(db::CompoundRegionOperationNode *input,
                                   db::edge_relation_type rel,
                                   bool different_polygons, db::Coord d,
   ...
   */
  CompoundRegionCheckOperationNode(db::CompoundRegionOperationNode *input,
                                   db::CompoundRegionOperationNode *other,
                                   db::edge_relation_type rel,
                                   bool different_polygons, db::Coord d,
                                   const db::RegionCheckOptions &options);
   ...
   */
  CompoundRegionCheckOperationNode(db::edge_relation_type rel,
                                   bool different_polygons, db::Coord d,
                                   const db::RegionCheckOptions &options);
 ...
 *  When put into a local processor, the operation tree will be executed on each
 * interaction.
 */
template <class TS, class TI, class TR>
class DB_PUBLIC compound_local_operation : public local_operation<TS, TI, TR> {
public:
  /**
   *  @brief Constructor
   *
   *  Creates a local operation which utilizes the operation tree. "node" is the
   * root of the operation tree. Ownership of the node is *not* transferred to
   * the local operation.
   */
...
protected:
  virtual void do_compute_local(db::Layout *layout,
    ...
    mp_node->compute_local(&cache, layout, interactions, results,
                           max_vertex_count, area_ratio);
  }

  virtual db::Coord dist() const { return mp_node->dist(); }
  virtual OnEmptyIntruderHint on_empty_intruder_hint() const {
    return mp_node->on_empty_intruder_hint();
  }
...
class DB_PUBLIC compound_local_operation_with_properties
    : public local_operation<db::object_with_properties<TS>,
...
public:
   ...
   * the local operation.
   */
  compound_local_operation_with_properties<TS, TI, TR>(
      CompoundRegionOperationNode *node, db::PropertyConstraint prop_constraint,
      db::PropertiesRepository *target_pr,
      const db::PropertiesRepository *subject_pr,
      const std::vector<const db::PropertiesRepository *> &intruder_prs)
      : mp_node(node), m_prop_constraint(prop_constraint),
        m_pms(target_pr, subject_pr) {
...
protected:
  virtual void do_compute_local(
      ...
      std::vector<std::unordered_set<db::object_with_properties<TR>>> &results,
      size_t max_vertex_count, double area_ratio) const {
    auto interactions_by_prop_id =
        separate_interactions_to_interactions_by_properties(
            interactions, m_prop_constraint, m_pms, m_pmis);
    for (auto s2p = interactions_by_prop_id.begin();
         s2p != interactions_by_prop_id.end(); ++s2p) {

      std::vector<std::unordered_set<TR>> results_wo_props;
      results_wo_props.resize(results.size());

      CompoundRegionOperationCache cache;
      mp_node->compute_local(&cache, layout, s2p->second, results_wo_props,
                             max_vertex_count, area_ratio);

      for (size_t n = 0; n < results.size(); ++n) {
        for (auto i = results_wo_props[n].begin();
             i != results_wo_props[n].end(); ++i) {
          results[n].insert(db::object_with_properties<TR>(
              *i, pc_norm(m_prop_constraint, s2p->first)));
        }
      }
    }
  }
