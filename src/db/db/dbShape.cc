...
static NO_RETURN void raise_no_polygon() {
  throw tl::Exception(
      tl::to_string(tr("Shape is not a general or simple polygon")));
}
...
Shape::point_iterator Shape::begin_hole(unsigned int hole) const {
  if (m_type == SimplePolygon || m_type == SimplePolygonRef ||
      m_type == SimplePolygonPtrArrayMember) {
...
Shape::polygon_ref_type Shape::polygon_ref() const {
  if (m_type == PolygonRef) {
    ...
    return polygon_ref_type(
        &basic_ptr(polygon_ptr_array_type::tag())->object().obj(),
        m_trans.disp());
  } else {
...
Shape::simple_polygon_ref_type Shape::simple_polygon_ref() const {
  if (m_type == SimplePolygonRef) {
    ...
    tl_assert(m_trans.rot() == 0);
    return simple_polygon_ref_type(
        &basic_ptr(simple_polygon_ptr_array_type::tag())->object().obj(),
        m_trans.disp());
  } else {
...
bool Shape::polygon(Shape::polygon_type &p) const {
  if (m_type == Polygon) {
    ...
    p.assign_hull(sp.obj().begin_hull(), sp.obj().end_hull(), sp.trans(),
                  false /*no additional compression*/);
    return true;
  } else if (m_type == Path) {
    p = path().polygon();
    return true;
...
bool Shape::simple_polygon(Shape::simple_polygon_type &p) const {
  if (m_type == Polygon) {
    ...
    return true;
             ...
             m_type == SimplePolygonPtrArrayMember) {
    p.clear();
    p.assign_hull(simple_polygon_ref().obj().begin_hull(),
                  simple_polygon_ref().obj().end_hull(),
                  simple_polygon_ref().trans(),
                  false /*no additional compression*/);
    return true;
  } else if (m_type == Path) {
...
Shape::path_ref_type Shape::path_ref() const {
  if (m_type == PathRef) {
    ...
    tl_assert(m_trans.rot() == 0);
    return path_ref_type(&basic_ptr(path_ptr_array_type::tag())->object().obj(),
                         m_trans.disp());
  } else {
    raise_no_path();
  }
...
Shape::text_ref_type Shape::text_ref() const {
  if (m_type == TextRef) {
    ...
    tl_assert(m_trans.rot() == 0);
    return text_ref_type(&basic_ptr(text_ptr_array_type::tag())->object().obj(),
                         disp_type(m_trans.disp()));
  } else {
    raise_no_text();
  }
}
...
Shape::perimeter_type Shape::perimeter() const {
  ...
  case PolygonPtrArray: {
    const polygon_ptr_array_type *arr =
        basic_ptr(polygon_ptr_array_type::tag());
  ...
  case SimplePolygonPtrArray: {
    const simple_polygon_ptr_array_type *arr =
        basic_ptr(simple_polygon_ptr_array_type::tag());
    return arr->size() * arr->object().perimeter();
  }
  case Path:
...
Shape::box_type Shape::bbox() const {
  ...
  case PolygonPtrArray:
    return basic_ptr(polygon_ptr_array_type::tag())
        ->bbox(db::box_convert<polygon_ptr_type>());
  case SimplePolygon:
  ...
  case SimplePolygonPtrArray:
    return basic_ptr(simple_polygon_ptr_array_type::tag())
        ->bbox(db::box_convert<simple_polygon_ptr_type>());
  case Text:
    return text().box();
  case TextRef:
  case TextPtrArrayMember:
    return text_ref().box();
  case TextPtrArray:
    return basic_ptr(text_ptr_array_type::tag())
        ->bbox(db::box_convert<text_ptr_type>());
  case Edge:
  ...
  case PathPtrArrayMember:
    return path_ref().box();
  case PathPtrArray:
    return basic_ptr(path_ptr_array_type::tag())
        ->bbox(db::box_convert<path_ptr_type>());
  case BoxArray:
    return basic_ptr(box_array_type::tag())->bbox(db::box_convert<box_type>());
  case ShortBoxArray:
    return basic_ptr(short_box_array_type::tag())
        ->bbox(db::box_convert<short_box_type>());
  case Box:
  case ShortBox:
  case BoxArrayMember:
  case ShortBoxArrayMember:
