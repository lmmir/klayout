...
template <class Sh> class repository {
public:
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const repository<Sh> &x, bool no_self = false,
                     void *parent = 0) {
...
public:
   ...
   */
  db::repository<db::polygon<C>> &
  repository(db::object_tag<db::polygon<C>> /*tag*/) {
   ...
   *  @brief Return the repository by tag
   */
  db::repository<db::simple_polygon<C>> &
  repository(db::object_tag<db::simple_polygon<C>> /*tag*/) {
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    db::mem_stat(stat, purpose, cat, m_simple_polygon_repository, no_self,
                 parent);
...
template <class C>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const generic_repository<C> &x,
...
template <class Sh, class Trans> struct shape_ref {
   ...
   */
  shape_ref(const Sh *ptr, const Trans &trans) : m_ptr(ptr), m_trans(trans) {
    // .. nothing yet ..
  }
   ...
   */
  shape_ref(const shape_type &p, repository_type &rep) : m_ptr(0) {
    shape_type p_red(p);
   ...
   */
  shape_ref(const shape_ref<Sh, Trans> &ref, repository_type &rep) : m_ptr(0) {
    if (!ref.is_null()) {
      m_trans = ref.trans();
   ...
   */
  void translate(const shape_ref<Sh, Trans> &ref, repository_type &rep,
                 db::ArrayRepository &) {
    if (!ref.is_null()) {
      m_trans = ref.trans();
      m_ptr = rep.repository(typename Sh::tag()).insert(ref.obj());
   ...
   */
  template <class T>
  void translate(const shape_ref<Sh, Trans> &ref, const T &t,
                 repository_type &rep, db::ArrayRepository &) {
    if (!ref.is_null()) {
   ...
   */
  const shape_type *ptr() const { return m_ptr; }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
                bool no_self, void *parent) const {
    if (!no_self) {
      stat->add(typeid(shape_ref<Sh, Trans>), (void *)this,
                sizeof(shape_ref<Sh, Trans>), sizeof(shape_ref<Sh, Trans>),
                parent, purpose, cat);
    }
    if (m_ptr) {
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const shape_ref<Sh, Tr> &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
 ...
 */
template <class Sh, class Tr>
inline std::ostream &operator<<(std::ostream &os, const shape_ref<Sh, Tr> &p) {
