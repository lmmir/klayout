 ...
 *
 *  This class provides information about various properties of the shape class,
 * i.e.
 ...
 */
template <class Sh> struct shape_traits {
...
template <class Sh, class StableTag> class layer_class : public LayerBase {
public:
  typedef db::layer<Sh, StableTag> layer_type;
  typedef LayerBase::box_type box_type;
  typedef LayerBase::coord_type coord_type;

  layer_class() : LayerBase(), m_layer() {}

  layer_type &layer() { return m_layer; }

  const layer_type &layer() const { return m_layer; }

  virtual box_type bbox() const { return m_layer.bbox(); }

  virtual void update_bbox() { m_layer.update_bbox(); }

  virtual bool is_bbox_dirty() const { return m_layer.is_bbox_dirty(); }

  virtual bool is_tree_dirty() const { return m_layer.is_tree_dirty(); }

  size_t size() const { return m_layer.size(); }

  bool empty() const { return m_layer.empty(); }

  virtual void sort() { m_layer.sort(); }
...
void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
              const layer_class<Sh, Stable> &x, bool no_self, void *parent) {
