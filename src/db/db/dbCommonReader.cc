...
db::cell_index_type CommonReaderBase::make_cell(db::Layout &layout,
                                                const std::string &cn) {
  tl_assert(!cn.empty());

  std::map<std::string, std::pair<size_t, db::cell_index_type>>::iterator
      iname = m_name_map.find(cn);
  if (iname != m_name_map.end()) {

    db::Cell &cell = layout.cell(iname->second.second);

    if (!cell.is_ghost_cell()) {
      common_reader_error(tl::sprintf(
          tl::to_string(tr("A cell with name %s already exists")), cn));
    }
...
CommonReaderBase::cell_by_name(const std::string &cn) const {
  std::map<std::string, std::pair<size_t, db::cell_index_type>>::const_iterator
      iname = m_name_map.find(cn);
  if (iname != m_name_map.end()) {
...
db::cell_index_type CommonReaderBase::make_cell(db::Layout &layout, size_t id) {
  tl_assert(id != null_id);

  std::map<size_t, std::pair<std::string, db::cell_index_type>>::iterator iid =
      m_id_map.find(id);
  if (iid != m_id_map.end()) {
    ...
    if (!cell.is_ghost_cell()) {
      common_reader_error(tl::sprintf(
          tl::to_string(tr("A cell with ID %ld already exists")), id));
    }
...
CommonReaderBase::cell_by_id(size_t id) const {
  std::map<size_t, std::pair<std::string, db::cell_index_type>>::const_iterator
      iid = m_id_map.find(id);
  if (iid != m_id_map.end()) {
...
const std::string &CommonReaderBase::name_for_id(size_t id) const {
...
void CommonReaderBase::rename_cell(db::Layout &layout, size_t id,
                                   const std::string &cn) {
  ...
  if (iid != m_id_map.end() && !iid->second.first.empty() &&
      iid->second.first != cn) {
    common_reader_error(
        tl::sprintf(tl::to_string(tr(
                        "Cell named %s with ID %ld was already given name %s")),
                    cn, id, iid->second.first));
  }
  ...
  if (iid != m_id_map.end() && iname != m_name_map.end()) {
    ...
    if (iname->second.second != iid->second.second) {
      ...
      //  Both cells already exist and are not identical: merge ID-declared cell
      //  into the name-declared one
      layout.force_update();
...
db::cell_index_type CommonReaderBase::cell_for_instance(db::Layout &layout,
                                                        size_t id) {
  tl_assert(id != null_id);
...
void CommonReaderBase::merge_cell(db::Layout &layout,
                                  db::cell_index_type target_cell_index,
                                  db::cell_index_type src_cell_index) const {
  const db::Cell &src_cell = layout.cell(src_cell_index);
  db::Cell &target_cell = layout.cell(target_cell_index);
  target_cell.set_ghost_cell(src_cell.is_ghost_cell() &&
                             target_cell.is_ghost_cell());

  //  copy over the instances
  for (db::Cell::const_iterator i = src_cell.begin(); !i.at_end(); ++i) {
    //  NOTE: cell indexed may be invalid because we delete subcells without
    //  update()
...
void CommonReaderBase::merge_cell_without_instances(
    db::Layout &layout, db::cell_index_type target_cell_index,
    db::cell_index_type src_cell_index) const {
  const db::Cell &src_cell = layout.cell(src_cell_index);
  db::Cell &target_cell = layout.cell(target_cell_index);
...
void CommonReaderBase::finish(db::Layout &layout) {
  ...
  for (std::map<size_t,
                std::pair<std::string, db::cell_index_type>>::const_iterator i =
           m_id_map.begin();
       i != m_id_map.end(); ++i) {
    if (i->second.first.empty()) {
      common_reader_warn(tl::sprintf(
          tl::to_string(tr("No cellname defined for cell name id %ld")),
          i->first));
      any_missing = true;
    }
  }
  ...
  if (any_missing) {
    common_reader_error(tl::to_string(
        tr("Some cell IDs don't have a name (see previous warnings)")));
  }
  ...
  for (std::map<std::string,
                std::pair<size_t, db::cell_index_type>>::const_iterator i =
           m_name_map.begin();
       i != m_name_map.end() && !has_conflict; ++i) {
  ...
  if (!has_conflict) {
    ...
    for (db::Layout::bottom_up_iterator bu = layout.begin_bottom_up();
         bu != layout.end_bottom_up(); ++bu) {

      db::cell_index_type ci_new = *bu;
      std::map<db::cell_index_type, std::string>::const_iterator i =
          new_cells.find(ci_new);
      ...
      std::pair<bool, db::cell_index_type> c2n =
          layout.cell_by_name(i->second.c_str());
      db::cell_index_type ci_org = c2n.second;

      //  NOTE: proxy cells are never resolved. "RenameCell" is a plain and
      //  simple case. Ghost cells are merged rendering the new cell a non-ghost
      //  cell.
      if (c2n.first &&
          (m_cc_resolution != RenameCell ||
           layout.cell(ci_org).is_ghost_cell() ||
           layout.cell(ci_new).is_ghost_cell()) &&
          !layout.cell(ci_org).is_proxy()) {
        cells_with_conflict.push_back(std::make_pair(ci_new, ci_org));
      } else {
        layout.rename_cell(
            ci_new, layout.uniquify_cell_name(i->second.c_str()).c_str());
      }
    }

    //  Then treat all the cells with conflict
    for (std::vector<std::pair<db::cell_index_type, db::cell_index_type>>::
             const_iterator cc = cells_with_conflict.begin();
         cc != cells_with_conflict.end(); ++cc) {
      ...
      if (m_cc_resolution == OverwriteCell &&
          !layout.cell(ci_new).is_ghost_cell()) {

        if (!layout.cell(ci_org).begin().at_end()) {

          //  NOTE: because prune_subcells needs the parents for sub cells and
          //  we are going do delete the current cell, we cannot save the
          //  "update()" just by traversing bottom-up.
          layout.force_update();
      ...
      } else if (m_cc_resolution == SkipNewCell &&
                 !layout.cell(ci_org).is_ghost_cell()) {
  ...
  //  remove temporary cells (some that were "declared" by "rename_cell" but not
  //  used by cell_for_instance)

  for (std::set<db::cell_index_type>::const_iterator ci = m_temp_cells.begin();
       ci != m_temp_cells.end(); ++ci) {
    layout.delete_cell(*ci);
  }

  //  resolve layer multi-mapping

  for (std::map<std::set<unsigned int>, unsigned int>::const_iterator i =
           m_multi_mapping_placeholders.begin();
       i != m_multi_mapping_placeholders.end(); ++i) {

    if (i->first.size() > 1) {

      bool discard_layer = i->first.find(i->second) == i->first.end();

      for (std::set<unsigned int>::const_iterator l = i->first.begin();
           l != i->first.end(); ++l) {
  ...
  for (std::set<unsigned int>::const_iterator i = m_layers_created.begin();
       i != m_layers_created.end(); ++i) {

    const db::LayerProperties &lp = layout.get_properties(*i);

    const tl::interval_map<db::ld_type, std::string> *dtmap =
        layer_names().mapped(lp.layer);
    ...
    if (name) {
      //  need to rename: add a new madding to m_layer_map_out and adjust the
      //  layout's layer properties
...
std::pair<bool, unsigned int> CommonReaderBase::open_dl(db::Layout &layout,
                                                        const LDPair &dl) {
  std::map<db::LDPair, std::pair<bool, unsigned int>>::const_iterator lc =
      m_layer_cache.find(dl);
...
CommonReaderBase::open_dl_uncached(db::Layout &layout, const LDPair &dl) {
  ...
  if (li.empty()) {
    ...
    const tl::interval_map<db::ld_type, std::string> *names_dmap =
        m_layer_names.mapped(dl.layer);
    if (names_dmap != 0) {
    ...
    for (std::set<unsigned int>::const_iterator i = li.begin(); i != li.end();
         ++i) {
      m_layer_map_out.mmap(dl, *i, layout.get_properties(*i));
    }

    std::map<std::set<unsigned int>, unsigned int>::iterator mmp =
        m_multi_mapping_placeholders.find(li);
    if (mmp == m_multi_mapping_placeholders.end()) {
      //  create a placeholder layer
      mmp = m_multi_mapping_placeholders
                .insert(std::make_pair(li, layout.insert_layer()))
                .first;
    }
...
const db::LayerMap &CommonReader::read(db::Layout &layout,
  ...
  //  A cleanup may be necessary because of the following scenario: if library
  //  proxies contain subcells which are proxies itself, the proxy update may
  //  make them orphans (the proxies are regenerated). The cleanup will removed
  //  these.
  layout.cleanup();
...
void CommonReader::init(const LoadLayoutOptions &options) {
  ...
  db::CommonReaderOptions common_options =
      options.get_options<db::CommonReaderOptions>();
 ...
 *  This is a dummy declaration that provides common specifications for both GDS
 * and OASIS readers.
 */
class CommonFormatDeclaration : public db::StreamFormatDeclaration {
public:
  ...
  virtual bool detect(tl::InputStream & /*s*/) const { return false; }

  virtual ReaderBase *create_reader(tl::InputStream & /*s*/) const { return 0; }

  virtual WriterBase *create_writer() const { return 0; }
  ...
  virtual tl::XMLElementBase *xml_reader_options_element() const {
    return new db::ReaderOptionsXMLElement<db::CommonReaderOptions>(
        "common",
        tl::make_member(&db::CommonReaderOptions::create_other_layers,
                        "create-other-layers") +
            tl::make_member(&db::CommonReaderOptions::layer_map, "layer-map") +
            tl::make_member(&db::CommonReaderOptions::enable_properties,
                            "enable-properties") +
            tl::make_member(&db::CommonReaderOptions::enable_text_objects,
                            "enable-text-objects"));
  }
};

static tl::RegisteredClass<db::StreamFormatDeclaration>
    reader_decl(new CommonFormatDeclaration(), 20, "Common");
