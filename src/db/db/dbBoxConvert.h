 ...
 */
template <class Sh, bool AllowEmpty> struct box_convert {};
...
struct box_convert<db::text<C>, AllowEmpty> {
  ...
  box_type operator()(const text_type &t) const { return t.box(); }
};
...
struct box_convert<db::text_ref<Text, Trans>, AllowEmpty> {
  ...
  box_type operator()(const text_ref_type &t) const { return t.box(); }
};
...
struct box_convert<db::array<db::text_ref<Text, Trans>, ArrayTrans>,
                   AllowEmpty> {
  typedef db::text_ref<Text, Trans> text_ref_type;
...
struct box_convert<db::path<C>, AllowEmpty> {
  ...
  box_type operator()(const path_type &p) const { return p.box(); }
};
...
struct box_convert<db::path_ref<Path, Trans>, AllowEmpty> {
  ...
  box_type operator()(const path_ref_type &p) const { return p.box(); }
};
...
struct box_convert<db::array<db::path_ref<Path, Trans>, ArrayTrans>,
                   AllowEmpty> {
  typedef db::path_ref<Path, Trans> path_ref_type;
...
struct box_convert<db::polygon<C>, AllowEmpty> {
  ...
  const box_type &operator()(const polygon_type &p) const { return p.box(); }
};
...
struct box_convert<db::polygon_ref<Poly, Trans>, AllowEmpty> {
  ...
  box_type operator()(const polygon_ref_type &p) const { return p.box(); }
};
...
struct box_convert<db::array<db::polygon_ref<Poly, Trans>, ArrayTrans>,
                   AllowEmpty> {
  typedef db::polygon_ref<Poly, Trans> polygon_ref_type;
...
struct box_convert<db::point<C>, AllowEmpty> {
  ...
  box_type operator()(const point_type &p) const { return box_type(p, p); }
};
...
struct box_convert<db::edge_pair<C>, AllowEmpty> {
  ...
  box_type operator()(const edge_pair_type &e) const { return e.bbox(); }
};
...
struct box_convert<db::user_object<C>, AllowEmpty> {
  ...
  box_type operator()(const user_object_type &r) const { return r.box(); }
};
 ...
 *  The AllowEmpty flag is ignored currently.
 */
template <class C, bool AllowEmpty> struct box_convert<db::box<C>, AllowEmpty> {
  typedef db::box<C> box_type;

  typedef simple_bbox_tag complexity;

  const box_type &operator()(const box_type &b) const { return b; }
};
...
/**
 *  @brief The box-to-box converter for the case of boxes with special internal
 * representation
 *
 *  This is the trivial case where a box becomes a box.
 *
...
struct box_convert<db::box<C, R>, AllowEmpty> {
  ...
  box_type operator()(const db::box<C, R> &b) const { return box_type(b); }
};

/**
 *  @brief The box array box converter for boxes with special internal
 * representation
 *
 *  The AllowEmpty flag is ignored currently.
 */
...
//  Note: the implementation is delegated, so we don't need to make box_convert
//  dllexport.
DB_PUBLIC db::Box cell_box_convert_impl(const db::Cell &c, int layer,
                                        bool allow_empty);
 ...
 *
 *  If AllowEmpty is false, the box converter treats empty cells as single
 * points at 0, 0.
 */

template <bool AllowEmpty> struct DB_PUBLIC box_convert<db::Cell, AllowEmpty> {
  typedef db::Cell cell_type;
  typedef db::Box box_type;

  typedef complex_bbox_tag complexity;

  box_convert() : m_layer(-1) {}

  box_convert(unsigned int l) : m_layer(l) {}
...
//  Note: the implementation is delegated, so we don't need to make box_convert
//  dllexport.
DB_PUBLIC db::Box cellinst_box_convert_impl(const db::CellInst &inst,
                                            const db::Layout *layout, int layer,
                                            bool allow_empty);
...
template <bool AllowEmpty> struct box_convert<db::CellInst, AllowEmpty> {
  ...
  box_convert() : mp_layout(0), m_layer(-1) {}

  box_convert(const layout_type &ly, unsigned int l)
      : mp_layout(&ly), m_layer(l) {}

  box_convert(const layout_type &ly) : mp_layout(&ly), m_layer(-1) {}
...
struct box_convert<db::array<db::CellInst, ArrayTrans>, AllowEmpty> {
  ...
  box_convert(const layout_type &g, unsigned int l) : m_bc(g, l) {}

  box_convert(const layout_type &g) : m_bc(g) {}
...
template <class ArrayTrans, bool AllowEmpty>
struct box_convert<
    db::object_with_properties<db::array<db::CellInst, ArrayTrans>>,
    AllowEmpty> {
  ...
  box_convert() : bconvert() {}

  box_convert(const layout_type &g, unsigned int l) : bconvert(g, l) {}

  box_convert(const layout_type &g) : bconvert(g) {}

  box_type
  operator()(const db::object_with_properties<cell_inst_array> &s) const {
    return bconvert(s);
  }
