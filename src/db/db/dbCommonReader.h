...
/**
 *  @brief Structure that holds the GDS2 and OASIS specific options for the
 * reader
 */
class DB_PUBLIC CommonReaderOptions : public FormatSpecificReaderOptions {
public:
  /**
   *  @brief The constructor
   */
  CommonReaderOptions()
      : create_other_layers(true), enable_text_objects(true),
   ...
   *
   *  If this flag is set to true, text objects are read. Otherwise they are
   * ignored.
   */
   ...
   *
   *  If this flag is set to true, user properties are read. Otherwise they are
   * ignored.
   */
   ...
   *
   *  This enum controls how cells are read if a cell with the requested name
   * already exists.
   *
   *  AddToCell       In this mode, instances or shapes are added to any
   * existing cell OverwriteCell   Overwrite existing cell. If the existing cell
   * has children, those are removed unless used otherwise SkipNewCell Ignore
   * the new cell and it's children RenameCell      Rename the new cell
   *
   *  If the existing opr the new cell is a ghost cell, AddToCell is applied
   * always. In other words, ghost cells are always merged.
   */
...
/**
 *  @brief A common reader base for GDS2 and OASIS providing common services for
 * both readers
 */
class DB_PUBLIC CommonReaderBase {
public:
  typedef tl::interval_map<db::ld_type,
                           tl::interval_map<db::ld_type, std::string>>
      layer_name_map;
  ...
  /**
   *  @brief Returns a pair with a bool (indicating whether the cell name is
   * known) and the cell index for this name
   */
  std::pair<bool, db::cell_index_type>
  cell_by_name(const std::string &name) const;
  ...
  /**
   *  @brief Returns a pair with a bool (indicating whether the cell ID is
   * known) and the cell index for this ID
   */
  ...
  /**
   *  @brief Gets the name for a given cell ID if known, otherwise returns an
   * empty string
   */
   ...
   */
  db::cell_index_type cell_for_instance(db::Layout &layout,
                                        const std::string &cn);
   ...
   *  After this, the cells are renamed and cell conflict resolution will happen
   * in the specified way (cell_conflict_resolution attribute).
   */
   ...
   */
  void set_create_layers(bool f) { m_create_layers = f; }
   ...
   */
  void set_layer_map(const db::LayerMap &lm) { m_layer_map = lm; }
...
protected:
   ...
   */
  void merge_cell(db::Layout &layout, db::cell_index_type target_cell_index,
                  db::cell_index_type src_cell_index) const;

  /**
   * @brief Merge (and delete) the src_cell into target_cell without instances
   */
  void merge_cell_without_instances(db::Layout &layout,
   ...
   */
  layer_name_map &layer_names() { return m_layer_names; }

  /**
   *  @brief Gets the input layer name
   */
  db::LayerMap &layer_map() { return m_layer_map; }

  /**
   *  @brief Gets the output layer name map
   */
  const db::LayerMap &layer_map_out() const { return m_layer_map_out; }
...
private:
  ...
  tl::interval_map<db::ld_type, tl::interval_map<db::ld_type, std::string>>
      m_layer_names;
  ...
  std::pair<bool, unsigned int> open_dl_uncached(db::Layout &layout,
                                                 const LDPair &dl);
};
 ...
 * both readers
 */
class DB_PUBLIC CommonReader : public ReaderBase, public CommonReaderBase {
public:
  typedef tl::interval_map<db::ld_type,
                           tl::interval_map<db::ld_type, std::string>>
      layer_name_map;
 ...
 */
class CommonReaderLayerMapping : public db::ImportLayerMapping {
public:
  ...
  std::pair<bool, unsigned int> map_layer(const db::LayerProperties &lprops) {
    ...
    if (lprops.is_named()) {
      ...
      return mp_reader->open_dl(*mp_layout,
                                LDPair(lprops.layer, lprops.datatype));
    }
  }
