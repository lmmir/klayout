...
bool LibraryManager::initialized() { return sp_instance != 0; }

LibraryManager::LibraryManager() {
  // handle auto-registered libraries:
  for (tl::Registrar<db::Library>::iterator l =
           tl::Registrar<db::Library>::begin();
       l != tl::Registrar<db::Library>::end(); ++l) {
...
std::pair<bool, lib_id_type> LibraryManager::lib_by_name(
    const std::string &name,
    const std::set<std::string> &for_technologies) const {
  ...
  if (!for_technologies.empty()) {
    ...
    while (l != m_lib_by_name.end() && l->first == name) {
      ...
      for (std::set<std::string>::const_iterator t = for_technologies.begin();
           t != for_technologies.end() && found; ++t) {
...
lib_id_type LibraryManager::register_lib(Library *library) {
...

  {
    ...
    //  if the new library replaces the old one, remap existing library proxies
    //  before deleting the library (replacement is done only when all
    //  technologies are substituted)
    lib_name_map::iterator l = m_lib_by_name.find(library->get_name());
    bool found = false;
    while (l != m_lib_by_name.end() && l->first == library->get_name()) {
      if (m_libs[l->second] && m_libs[l->second]->get_technologies() ==
                                   library->get_technologies()) {
        found = true;
        break;
      }
      ++l;
  ...
  if (old_lib) {
    ...
    {
      //  reset the library pointer only after "remap_to" -> this function may
      //  need lib_by_id.
      tl::MutexLocker locker(&m_lock);
  ...
  //  NOTE: this will try to substitute the cold proxies we may have generated
  //  during "remap_to" above, but "restore_proxies" takes care not to
  //  re-substitute cold proxies.

  const tl::weak_collection<db::ColdProxy> &cold_proxies =
      db::ColdProxy::cold_proxies_per_lib_name(library->get_name());
  std::set<db::Layout *> to_refresh;
  for (tl::weak_collection<db::ColdProxy>::const_iterator p =
           cold_proxies.begin();
       p != cold_proxies.end(); ++p) {
    to_refresh.insert(const_cast<db::Layout *>(p->layout()));
  }

  for (std::set<db::Layout *>::const_iterator l = to_refresh.begin();
       l != to_refresh.end(); ++l) {
    (*l)->restore_proxies(0);
  }
...
Library *LibraryManager::lib(lib_id_type id) const {
  tl::MutexLocker locker(&m_lock);
  return lib_internal(id);
}

Library *LibraryManager::lib_internal(lib_id_type id) const {
  if (id >= m_libs.size()) {
    return 0;
  } else {
    return m_libs[id];
...
void LibraryManager::clear() {
...

  {
    ...
    //  empty the library table before we delete them - this avoid accesses to
    //  invalid libraries while doing so.
    libs.swap(m_libs);
    m_lib_by_name.clear();
  }

  for (std::vector<Library *>::iterator l = libs.begin(); l != libs.end();
       ++l) {
    if (*l) {
      (*l)->remap_to(0);
