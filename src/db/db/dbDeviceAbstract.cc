...
DeviceAbstract::DeviceAbstract(db::DeviceClass *device_class,
                               const std::string &name)
    : m_name(name), mp_device_class(device_class),
      m_cell_index(std::numeric_limits<db::cell_index_type>::max()),
      mp_netlist(0) {
  //  .. nothing yet ..
}
...
DeviceAbstract::DeviceAbstract(const DeviceAbstract &other)
    : tl::Object(other), mp_device_class(0),
      m_cell_index(std::numeric_limits<db::cell_index_type>::max()),
      mp_netlist(0) {
  operator=(other);
}
...
void DeviceAbstract::set_netlist(Netlist *netlist) { mp_netlist = netlist; }
...
size_t DeviceAbstract::cluster_id_for_terminal(size_t terminal_id) const {
  return terminal_id < m_terminal_cluster_ids.size()
             ? m_terminal_cluster_ids[terminal_id]
             : 0;
}

void DeviceAbstract::set_cluster_id_for_terminal(size_t terminal_id,
                                                 size_t cluster_id) {
