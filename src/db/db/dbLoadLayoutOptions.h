 ...
 */
class DB_PUBLIC FormatSpecificReaderOptions : public gsi::ObjectBase {
public:
...
public:
   ...
   *
   *  The warning level is a reader-specific setting which enables or disables
   * warnings on specific levels. Level 0 is always "warnings off". The default
   * level is 1 which means "reasonable warnings emitted".
   */
   ...
   */
  void set_warn_level(int w) { m_warn_level = w; }
   ...
   * to the LoadLayoutOptions object.
   *
  ...
  template <class T> void set_options(T *options) {
    std::map<std::string, FormatSpecificReaderOptions *>::iterator o =
  ...
  template <class T> const T &get_options() const {
    ...
    std::map<std::string, FormatSpecificReaderOptions *>::const_iterator o =
   ...
   *
   *  This version takes a generic FormatSpecificReaderOptions object and
