...
template <class E> struct inside_poly_test_edge_max_compare_f {
  bool operator()(const E &e1, const E &e2) {
    return std::max(e1.p1().y(), e1.p2().y()) <
           std::max(e2.p1().y(), e2.p2().y());
  }
};

template <class P> inside_poly_test<P>::inside_poly_test(const P &polygon) {
  m_edges.reserve(polygon.vertices());
  for (typename P::polygon_edge_iterator e = polygon.begin_edge(); !e.at_end();
       ++e) {
    m_edges.push_back(*e);
  }
  std::sort(m_edges.begin(), m_edges.end(),
            inside_poly_test_edge_max_compare_f<edge_type>());
}

template <class P>
int inside_poly_test<P>::operator()(
    const typename inside_poly_test<P>::point_type &pt) const {
  int wrapcount_left = 0;

  typename std::vector<edge_type>::const_iterator e =
      std::lower_bound(m_edges.begin(), m_edges.end(), edge_type(pt, pt),
                       inside_poly_test_edge_max_compare_f<edge_type>());
 ...
 */
template <class PointType> struct cut_polygon_edge {
  ...
  cut_polygon_edge(int c, unsigned int n, projection_type p,
                   const PointType &pt, const PointType &lpt)
      : contour(c), index(n), projected(p), point(pt), last_point(lpt) {}

  edge_type edge() const { return edge_type(last_point, point); }
...
template <class CuttingEdgeType> struct cut_polygon_segment {
  ...
  cut_polygon_segment() : leave(), enter(), segment(-1) {
    // .. nothing yet ..
  }
};

template <class CuttingEdgeType> struct loose_end_struct {
public:
  typedef typename CuttingEdgeType::edge_type edge_type;
  typedef typename CuttingEdgeType::projection_type projection_type;
  ...
  loose_end_struct(
      bool _enter,
      typename std::vector<cut_polygon_segment<CuttingEdgeType>>::const_iterator
          _iter)
...
static bool
_cut_polygon_internal(const PolygonType &input, const Edge &line,
  ...
  for (unsigned int nc = 0; nc < input.holes() + 1; ++nc) {
    ...
    for (unsigned int n = 0; n < contour.size(); ++n) {
      ...
      if (ip.first) {
        ...
        double p = line_length * double(db::vprod(e.p1() - line.p1(), e.d())) /
                   double(db::vprod(line.d(), e.d()));
        ...
        if (s1 < 0 && s2 >= 0) {
          ...
          cutting_segments.back().leave =
              cut_polygon_edge_type(int(nc), nn, p, ip.second, e.p1());
          any = true;
        }
        ...
        if (s1 >= 0 && s2 < 0) {
          ...
          ++sc;
          cutting_segments.push_back(cutting_segment_type());
          cutting_segments.back().enter =
              cut_polygon_edge_type(int(nc), nn, p, ip.second, e.p2());
    ...
    if (any) {
      ...
      if (do_hole_assignment) {
        ...
        if (nc == 0) {
          //  the hull is fully on the right side -> just output the input
          //  polygon and that's it.
  ...
  for (typename std::vector<cutting_segment_type>::const_iterator i =
           cutting_segments.begin();
       i != cutting_segments.end(); ++i) {
  ...
  //  we allow single pairs of collinear entry/leave edges (cut lines) and bring
  //  them in the right order

  bool enter = false;
  for (typename std::vector<loose_end_struct<cut_polygon_edge_type>>::iterator
           i = loose_ends.begin();
       i != loose_ends.end(); ++i) {
  ...
  //  the points now have to be in strict enter/leave order - otherwise fallback
  //  to merge
  ...
  typedef std::map<
      std::pair<int, int>,
      std::pair<typename std::vector<cut_polygon_edge_type>::iterator,
                typename std::vector<cut_polygon_edge_type>::iterator>>
      cut_point_map_type;
  cut_point_map_type cut_points;

  for (typename std::vector<cut_polygon_edge_type>::iterator c =
           cutting_edges.begin();
       c != cutting_edges.end(); c += 2) {
    cut_points.insert(std::make_pair(std::make_pair(c[0].contour, c[0].index),
                                     std::make_pair(c, c + 1)));
    cut_points.insert(std::make_pair(std::make_pair(c[1].contour, c[1].index),
                                     std::make_pair(c + 1, c)));
  }
    ...
    if (c->contour >= 0) {
      ...
      do {
        ...
        while (n != n0) {
          ...
          typename cut_point_map_type::iterator cp =
              cut_points.find(std::make_pair(nc, n));
      ...
      if (do_hole_assignment) {
        ...
        if (is_hull) {
          ...
          hull_polygons.back().assign_hull(contour_points.begin(),
                                           contour_points.end(), true,
                                           true /*remove reflected*/);
        } else {
          hole_polygons.push_back(PolygonType());
          hole_polygons.back().assign_hull(contour_points.begin(),
                                           contour_points.end(), true,
                                           true /*remove reflected*/);
        }
        ...
        PolygonType poly;
        poly.assign_hull(contour_points.begin(), contour_points.end(), true,
                         true /*remove reflected*/);
        //  it might happen in some cases, that cut pieces may vanish (i.e. all
        //  points on a line). Thus we check, if that is the case and do not
        //  produce a polygon then.
        if (poly.vertices() > 0) {
          right_of_line->put(poly);
        }
      }
  ...
  //  do hole assignment
  for (typename std::vector<PolygonType>::iterator hull = hull_polygons.begin();
       hull != hull_polygons.end(); ++hull) {

    //  it might happen in some cases, that cut pieces may vanish (i.e. all
    //  points on a line). Thus we check, if that is the case and do not produce
    //  a polygon then.
    if (hull->vertices() > 0) {

      db::inside_poly_test<PolygonType> inside_hull(*hull);
      for (typename std::vector<PolygonType>::iterator hole =
               hole_polygons.begin();
           hole != hole_polygons.end(); ++hole) {
  ...
  // TODO: precisely, this is possible only if the orientation is clockwise.
  // Since we loose the orientation because we assign to a PolygonType, this
  // check is not possible.
  for (typename std::vector<PolygonType>::iterator hole = hole_polygons.begin();
       hole != hole_polygons.end(); ++hole) {
    if (hole->vertices() > 0) {
...
template <class PolygonType> struct get_sink_type;

template <> struct get_sink_type<db::Polygon> {
  typedef db::PolygonSink result;
};

template <> struct get_sink_type<db::SimplePolygon> {
  typedef db::SimplePolygonSink result;
};

/**
 *  @brief A polygon sink for the edge processor that feeds the polygon into the
 * cut algorithm
 */
template <class Sink, class PolygonType>
struct cut_polygon_bool_sink : public Sink {
  cut_polygon_bool_sink(cut_polygon_receiver_base<PolygonType> *_right_of_line)
      : right_of_line(_right_of_line) {
    //  .. nothing yet ..
  }

  virtual void put(const PolygonType &poly) { right_of_line->put(poly); }

  cut_polygon_receiver_base<PolygonType> *right_of_line;
};

/**
 *  @brief The cut algorithm driver with fallback to polygon merging when the
 * cut fails
 *  TODO: this is kind of inefficient as we will first merge all and the cut
 * just a half. This means for producing both parts we do this twice. But
 * remember, this is just a fallback.
 */
template <class PolygonType, class Edge>
void cut_polygon_internal_int(
    const PolygonType &input, const Edge &line,
    cut_polygon_receiver_base<PolygonType> *right_of_line) {
  bool ok = _cut_polygon_internal(input, line, right_of_line);
  if (!ok) {

    //  If the fast cut operation fails, use boolean AND to perform the cut
    //  operation
    ...
    if (!mask.empty()) {
      ...
      cut_polygon_bool_sink<typename get_sink_type<PolygonType>::result,
                            PolygonType>
          sink(right_of_line);
...
/**
 *  A transforming receiver that is put between a int cut algorithm and the
 * double output receiver
 */
...
public:
  cut_polygon_receiver_double_impl() : mp_next(0) {}

  void set_next(cut_polygon_receiver_base<PolygonType> *next) {
    mp_next = next;
  }

  void set_trans(const db::CplxTrans &tr) { m_tr = tr; }
...
template <class PolygonType> class cut_polygon_receiver_double;

//  template specializations for the double types
template <>
class cut_polygon_receiver_double<db::DPolygon>
    : public cut_polygon_receiver_double_impl<db::DPolygon, db::Polygon> {};
template <>
class cut_polygon_receiver_double<db::DSimplePolygon>
    : public cut_polygon_receiver_double_impl<db::DSimplePolygon,
                                              db::SimplePolygon> {};

/**
 *  @brief The cut algorithm driver for double types
 *  This driver will first try to guess a database unit (based on the bounding
 * box) and then transform the polygon to int. On output, the polygon is
 * transformed back to double.
 */
template <class PolygonType, class Edge>
void cut_polygon_internal_double(
    const PolygonType &input, const Edge &line,
    cut_polygon_receiver_base<PolygonType> *right_of_line) {
  ...
  //  guess DBU
  double dbu = std::max(1e-10, std::max(bbox.width(), bbox.height()) /
                                   (std::numeric_limits<db::Coord>::max() / 2));
  ...
  cut_polygon_internal_int(input.transformed(tr.inverted(), false),
                           line.transformed(tr.inverted()), &rec);
}
...
template <>
DB_PUBLIC void
cut_polygon_internal(const db::Polygon &polygon,
                     const db::Polygon::edge_type &line,
                     cut_polygon_receiver_base<db::Polygon> *right_of_line) {
  cut_polygon_internal_int(polygon, line, right_of_line);
}

template <>
DB_PUBLIC void cut_polygon_internal(
    const db::SimplePolygon &polygon, const db::SimplePolygon::edge_type &line,
    cut_polygon_receiver_base<db::SimplePolygon> *right_of_line) {
  cut_polygon_internal_int(polygon, line, right_of_line);
}

template <>
DB_PUBLIC void
cut_polygon_internal(const db::DPolygon &polygon,
                     const db::DPolygon::edge_type &line,
                     cut_polygon_receiver_base<db::DPolygon> *right_of_line) {
  cut_polygon_internal_double(polygon, line, right_of_line);
}

template <>
DB_PUBLIC void cut_polygon_internal(
    const db::DSimplePolygon &polygon,
    const db::DSimplePolygon::edge_type &line,
    cut_polygon_receiver_base<db::DSimplePolygon> *right_of_line) {
  cut_polygon_internal_double(polygon, line, right_of_line);
}
...
void split_polygon(const PolygonType &polygon,
                   std::vector<PolygonType> &output) {
  typedef typename PolygonType::coord_type coord_type;
  typedef typename PolygonType::point_type point_type;
  ...
  for (typename PolygonType::polygon_contour_iterator e = polygon.begin_hull();
       e != polygon.end_hull(); ++e) {
    if ((*e).x() != bbox.left() && (*e).x() != bbox.right() &&
        (std::abs((*e).x() - x) < std::abs(xx - x) || !xx_set)) {
      xx = (*e).x();
      xx_set = true;
    }
    if ((*e).y() != bbox.top() && (*e).y() != bbox.bottom() &&
        (std::abs((*e).y() - y) < std::abs(yy - y) || !yy_set)) {
      yy = (*e).y();
      yy_set = true;
    }
  }
  ...
  if (!xx_set && !yy_set) {
    ...
    //  an empiric threshold for splitting polygons in one direction: don't
    //  split along the long axis for polygons with a aspect ratio (of the
    //  bounding box) of larger than 3
  ...
  if (xx_set) {
    ...
    db::cut_polygon(polygon, edge_type(point_type(xx, 0), point_type(xx, 1)),
                    std::back_inserter(xx_polygons));
    db::cut_polygon(polygon, edge_type(point_type(xx, 1), point_type(xx, 0)),
                    std::back_inserter(xx_polygons));

    xx_n = 0;
    for (typename std::vector<PolygonType>::const_iterator p =
             xx_polygons.begin();
         p != xx_polygons.end(); ++p) {
      xx_n += p->vertices();
    }
  ...
  if (yy_set) {
    ...
    db::cut_polygon(polygon, edge_type(point_type(0, yy), point_type(1, yy)),
                    std::back_inserter(yy_polygons));
    db::cut_polygon(polygon, edge_type(point_type(1, yy), point_type(0, yy)),
                    std::back_inserter(yy_polygons));
...
template DB_PUBLIC void split_polygon<>(const db::Polygon &polygon,
                                        std::vector<db::Polygon> &output);
template DB_PUBLIC void split_polygon<>(const db::SimplePolygon &polygon,
                                        std::vector<db::SimplePolygon> &output);
template DB_PUBLIC void split_polygon<>(const db::DPolygon &polygon,
                                        std::vector<db::DPolygon> &output);
template DB_PUBLIC void
split_polygon<>(const db::DSimplePolygon &polygon,
                std::vector<db::DSimplePolygon> &output);

// -------------------------------------------------------------------------
//  Smoothing tools

void smooth_contour(db::Polygon::polygon_contour_iterator from,
                    db::Polygon::polygon_contour_iterator to,
                    std::vector<db::Point> &points, db::Coord d, bool keep_hv) {
  points.clear();
  ...
  while (points.size() >= 3 && cont > 0) {
    ...
    for (i = (even ? 0 : 1); i < points.size(); i += 2) {
      ...
      if (keep_hv && (p1.x() == p0.x() || p1.y() == p0.y() ||
                      p2.x() == p1.x() || p2.y() == p1.y())) {
        //  keep points which participate in either a vertical or horizontal
        //  edge
      } else if (db::Coord(p1.distance(p0)) <= d &&
                 db::sprod_sign(p2 - p1, p0 - pm1) > 0 &&
                 std::abs(db::vprod(p2 - p1, p0 - pm1)) <
                     0.8 * p2.distance(p1) * p0.distance(pm1)) {
        ...
        //  convex corners enclosing a little more than 45 degree are candidates
        //  too
        can_drop = (db::sprod_sign(p2 - p1, p1 - p0) > 0 &&
                    std::abs(db::vprod(p2 - p1, p1 - p0)) <
                        0.8 * p2.distance(p1) * p1.distance(p0));
      }
...
db::Polygon smooth(const db::Polygon &polygon, db::Coord d, bool keep_hv) {
  db::Polygon new_poly;
  ...
  if (new_pts.size() >= 3) {
    ...
    new_poly.assign_hull(new_pts.begin(), new_pts.end(),
    ...
    for (unsigned int h = 0; h < polygon.holes(); ++h) {
      ...
      smooth_contour(polygon.begin_hole(h), polygon.end_hole(h), new_pts, d,
                     keep_hv);
      if (new_pts.size() >= 3) {
        new_poly.insert_hole(new_pts.begin(), new_pts.end(),
                             false /*don't compress*/);
      }
    }
...
struct StrangePolygonInsideFunc {
  inline bool operator()(int wc) const { return wc < 0 || wc > 1; }
};
...
struct NonOrientablePolygonFunc {
  inline bool operator()(int wc) const {
    //  As polygon contours are normalized by default to positive wrap count a
    //  negative wrap count indicates non-orientability
    return wc < 0;
  }
};
 ...
 */
class ErrorCatchingEdgeSink : public db::EdgeSink {
  //  TODO: we should not use exceptions to indicate a condition, but right now,
  //  there is no good alternative and this is considered an error anyway.
...
template <class F>
bool check_wrapcount(const db::Polygon &poly,
                     std::vector<db::Polygon> *error_parts) {
...
bool is_strange_polygon(const db::Polygon &poly,
                        std::vector<db::Polygon> *strange_parts) {
  return check_wrapcount<StrangePolygonInsideFunc>(poly, strange_parts);
}

bool is_non_orientable_polygon(const db::Polygon &poly,
                               std::vector<db::Polygon> *strange_parts) {
...
template <class C>
static bool do_extract_rad_from_contour(
    typename db::polygon<C>::polygon_contour_iterator from,
    typename db::polygon<C>::polygon_contour_iterator to, double &rinner,
    double &router, unsigned int &n, std::vector<db::point<C>> *new_pts,
    bool fallback) {
  if (from == to) {
    return false;
  }
  ...
  do {
    ...
    bool first_or_last =
        fallback ||
        (e.double_length() > circle_segment_thr * ep.double_length() ||
         ep.double_length() > circle_segment_thr * e.double_length()) ||
        (db::vprod_sign(em, ep) * db::vprod_sign(ep, e) < 0 ||
         db::vprod_sign(ep, e) * db::vprod_sign(e, en) < 0);

    if (first_or_last &&
        db::sprod(ep, e) > cos_thr * e.double_length() * ep.double_length()) {
      double ls = (db::vprod_sign(ep, e) > 0 ? ls_inner : ls_outer);
      if (!fallback && ((e.double_length() < circle_segment_thr * ls &&
                         ep.double_length() > circle_segment_thr * ls) ||
                        db::vprod_sign(em, ep) * db::vprod_sign(ep, e) < 0)) {
        found = true;
        break;
  ...
  //  create a list of new points without the rounded corners and compute
  //  rounding radii
  ...
  do {
    ...
    //  In non-fallback mode either a long edge followed by a short one
    //  indicates the beginning of a new circle segment or a circle segment is
    //  detected when the curvature changes. The latter case detects situations
    //  where two circle segments directly attach to each other with different
    //  bending direction.
    ...
    if (db::sprod(ep, e) > cos_thr * e.double_length() * ep.double_length()) {
      ...
      if ((!fallback && first_or_last &&
           ((e.double_length() < circle_segment_thr * ls &&
             ep.double_length() > circle_segment_thr * ls) ||
            db::vprod_sign(em, ep) * db::vprod_sign(ep, e) < 0)) ||
      ...
      } else if ((!fallback && first_or_last &&
                  ((e.double_length() > circle_segment_thr * ls_corner &&
                    ep.double_length() < circle_segment_thr * ls_corner) ||
                   db::vprod_sign(ep, e) * db::vprod_sign(e, en) < 0)) ||
        ...
        if (in_corner) {
          ...
          if (!cp.first || db::sprod(elast, e) < acute_cos_thr *
                                                     elast.double_length() *
                                                     e.double_length()) {

            //  We have a full 180 degree bend without a stop (actually two
            //  corners). Use the segment in between that is perpendicular to
            //  the start and end segment as stop edge.
            ...
            while (pp1 != p1) {
              ...
              asum_part +=
                  db::vprod(*pp1 - db::point<C>(), *pp2 - db::point<C>());
              ++nseg_part;
            ...
            asum_part += db::vprod(cp.second - db::point<C>(),
                                   elast.p2() - db::point<C>());
            asum_part +=
                db::vprod(*pp1 - db::point<C>(), cp.second - db::point<C>());
            ...
            double rx = sqrt(
                fabs(asum_part) * 0.5 /
                (tan(atot * 0.5) - tan(atot * 0.5 / nseg_part) * nseg_part));
          ...
          asum += db::vprod(cp.second - db::point<C>(),
                            elast.p2() - db::point<C>());
          ...
          double rx = sqrt(fabs(asum) * 0.5 /
                           (tan(atot * 0.5) - tan(atot * 0.5 / nseg) * nseg));
          double da = atot / nseg;
...
bool extract_rad_from_contour(db::Polygon::polygon_contour_iterator from,
                              db::Polygon::polygon_contour_iterator to,
                              double &rinner, double &router, unsigned int &n,
                              std::vector<db::Point> *new_pts, bool fallback) {
  return do_extract_rad_from_contour(from, to, rinner, router, n, new_pts,
                                     fallback);
}

bool extract_rad_from_contour(db::DPolygon::polygon_contour_iterator from,
                              db::DPolygon::polygon_contour_iterator to,
                              double &rinner, double &router, unsigned int &n,
                              std::vector<db::DPoint> *new_pts, bool fallback) {
  return do_extract_rad_from_contour(from, to, rinner, router, n, new_pts,
                                     fallback);
}

template <class C>
static bool do_extract_rad(const db::polygon<C> &polygon, double &rinner,
                           double &router, unsigned int &n,
                           db::polygon<C> *new_polygon) {
  if (new_polygon) {

    std::vector<db::point<C>> new_pts;

    if (!do_extract_rad_from_contour(polygon.begin_hull(), polygon.end_hull(),
                                     rinner, router, n, &new_pts, false) &&
        !do_extract_rad_from_contour(polygon.begin_hull(), polygon.end_hull(),
                                     rinner, router, n, &new_pts, true)) {
      //  no radius found
      return false;
    } else {
    ...
    for (unsigned int h = 0; h < polygon.holes(); ++h) {
      ...
      new_pts.clear();
      if (!do_extract_rad_from_contour(polygon.begin_hole(h),
                                       polygon.end_hole(h), rinner, router, n,
                                       &new_pts, false) &&
          !do_extract_rad_from_contour(polygon.begin_hole(h),
                                       polygon.end_hole(h), rinner, router, n,
                                       &new_pts, true)) {
        //  no radius found
        return false;
    ...
    if (!do_extract_rad_from_contour(polygon.begin_hull(), polygon.end_hull(),
                                     rinner, router, n,
                                     (std::vector<db::point<C>> *)0, false)) {
      if (!do_extract_rad_from_contour(polygon.begin_hull(), polygon.end_hull(),
                                       rinner, router, n,
                                       (std::vector<db::point<C>> *)0, true)) {
        return false;
      }
    }
    ...
    for (unsigned int h = 0; h < polygon.holes(); ++h) {
      if (!do_extract_rad_from_contour(polygon.begin_hole(h),
                                       polygon.end_hole(h), rinner, router, n,
                                       (std::vector<db::point<C>> *)0, false)) {
        if (!do_extract_rad_from_contour(
                polygon.begin_hole(h), polygon.end_hole(h), rinner, router, n,
                (std::vector<db::point<C>> *)0, true)) {
          return false;
        }
      }
...
bool extract_rad(const db::Polygon &polygon, double &rinner, double &router,
                 unsigned int &n, db::Polygon *new_polygon) {
  return do_extract_rad(polygon, rinner, router, n, new_polygon);
}

bool extract_rad(const db::DPolygon &polygon, double &rinner, double &router,
                 unsigned int &n, db::DPolygon *new_polygon) {
  return do_extract_rad(polygon, rinner, router, n, new_polygon);
}

template <class C>
static void do_compute_rounded_contour(
    typename db::polygon<C>::polygon_contour_iterator from,
    typename db::polygon<C>::polygon_contour_iterator to,
    std::vector<db::point<C>> &new_pts, double rinner, double router,
    unsigned int n) {
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    ...
    db::DVector e1 =
        (db::DPoint(p1) - db::DPoint(p0)) * (1.0 / p0.double_distance(p1));
    db::DVector e2 =
        (db::DPoint(p2) - db::DPoint(p1)) * (1.0 / p1.double_distance(p2));
    ...
    if (r > 0.0) {
      ...
      if (nseg == 0) {
        ...
        for (double aa = 0.0; aa < a - 1e-6; aa += da) {
          ...
          db::DPoint q1 =
              pr + n1 * (r * cos(aa + da)) + e1 * (r * sin(aa + da));

          //  do a interpolation by computing the crossing point of the tangents
          //  of the circle at a and a+da. This scheme guarantees a low
          //  distortion of the original polygon and enables reverting back to
          //  the original polygon to some degree.
          db::DPoint qm = q0 + (q1 - q0) * 0.5;
          db::DPoint q =
              qm + (qm - pr) * (q0.sq_distance(qm) / pr.sq_distance(qm));
...
void compute_rounded_contour(db::Polygon::polygon_contour_iterator from,
                             db::Polygon::polygon_contour_iterator to,
                             std::vector<db::Point> &new_pts, double rinner,
                             double router, unsigned int n) {
  do_compute_rounded_contour(from, to, new_pts, rinner, router, n);
}

void compute_rounded_contour(db::DPolygon::polygon_contour_iterator from,
                             db::DPolygon::polygon_contour_iterator to,
                             std::vector<db::DPoint> &new_pts, double rinner,
                             double router, unsigned int n) {
  do_compute_rounded_contour(from, to, new_pts, rinner, router, n);
}

template <class C>
static db::polygon<C> do_compute_rounded(const db::polygon<C> &polygon,
                                         double rinner, double router,
                                         unsigned int n) {
  db::polygon<C> new_poly;
  std::vector<db::point<C>> new_pts;

  compute_rounded_contour(polygon.begin_hull(), polygon.end_hull(), new_pts,
                          rinner, router, n);
  new_poly.assign_hull(new_pts.begin(), new_pts.end(),
                       false /*don't compress*/);
  ...
  for (unsigned int h = 0; h < polygon.holes(); ++h) {
    new_pts.clear();
    compute_rounded_contour(polygon.begin_hole(h), polygon.end_hole(h), new_pts,
                            rinner, router, n);
    new_poly.insert_hole(new_pts.begin(), new_pts.end(),
                         false /*don't compress*/);
  }
...
db::Polygon compute_rounded(const db::Polygon &polygon, double rinner,
                            double router, unsigned int n) {
  return do_compute_rounded(polygon, rinner, router, n);
}

db::DPolygon compute_rounded(const db::DPolygon &polygon, double rinner,
                             double router, unsigned int n) {
  return do_compute_rounded(polygon, rinner, router, n);
}
...
AreaMap::AreaMap() : m_nx(0), m_ny(0) { mp_av = 0; }

AreaMap::AreaMap(const AreaMap &other) : m_nx(0), m_ny(0) {
  mp_av = 0;
  operator=(other);
}

AreaMap &AreaMap::operator=(const AreaMap &other) {
  if (this != &other) {
...
AreaMap::AreaMap(const db::Point &p0, const db::Vector &d, const db::Vector &p,
                 size_t nx, size_t ny)
    : m_p0(p0), m_d(d), m_p(std::min(d.x(), p.x()), std::min(d.y(), p.y())),
      m_nx(nx), m_ny(ny) {
  mp_av = new area_type[nx * ny];
  clear();
}
...
void AreaMap::reinitialize(const db::Point &p0, const db::Vector &d, size_t nx,
                           size_t ny) {
  reinitialize(p0, d, d, nx, ny);
}

void AreaMap::reinitialize(const db::Point &p0, const db::Vector &d,
                           const db::Vector &p, size_t nx, size_t ny) {
  m_p0 = p0;
  m_d = d;
...
void AreaMap::swap(AreaMap &other) {
...
AreaMap::area_type AreaMap::total_area() const {
  area_type asum = 0;
  if (mp_av) {
...
db::Box AreaMap::bbox() const {
  if (m_nx == 0 || m_ny == 0) {
    return db::Box();
  } else {
    return db::Box(m_p0,
                   m_p0 + db::Vector(db::Coord(m_nx - 1) * m_d.x() + m_p.x(),
                                     db::Coord(m_ny - 1) * m_d.y() + m_p.y()));
  }
}
...
static bool edge_is_partially_left_of(const db::Edge &e,
                                      const db::Edge &e_original, db::Coord x) {
  Coord xmin = db::edge_xmin(e);
  if (xmin < x) {
    return true;
  } else if (xmin == x && e_original.dx() != 0) {
    //  the skew edge is cut partially rendering a straight vertical line (due
    //  to rounding) which we will count as "left of"
    return true;
  } else {
    return false;
  }
...
bool rasterize(const db::Polygon &polygon, db::AreaMap &am) {
  ...
  //  check if the polygon overlaps the rasterization area. Otherwise, we simply
  //  do nothing.
  ...
  size_t iy0 =
      std::min(ny, size_t(std::max(db::Coord(0), (pbox.bottom() - am.p0().y()) /
                                                     am.d().y())));
  size_t iy1 =
      std::min(ny, size_t(std::max(db::Coord(0),
                                   (pbox.top() - am.p0().y() + am.d().y() - 1) /
                                       am.d().y())));

  size_t ix0 = std::min(
      nx,
      size_t(std::max(db::Coord(0), (pbox.left() - am.p0().x()) / am.d().x())));
  size_t ix1 =
      std::min(nx, size_t(std::max(db::Coord(0), (pbox.right() - am.p0().x() +
                                                  am.d().x() - 1) /
                                                     am.d().x())));
  ...
  //  collect edges
  size_t n = 0;
  for (db::Polygon::polygon_edge_iterator e = polygon.begin_edge(); !e.at_end();
       ++e) {
    if ((*e).dy() != 0 && db::edge_ymax(*e) > ymin &&
        db::edge_ymin(*e) < ymax) {
      ++n;
    }
  }
  ...
  for (size_t iy = iy0; iy < iy1; ++iy) {
    ...
    for (size_t ix = ix0; ix < ix1; ++ix) {
      ...
      // TODO: edge_xmin_at_interval(y, yy) and edge_xmax.. would be more
      // efficient in the all-angle case. However, it is crucial that the edge
      // clipping produces connected edge segments and it is questionable
      // whether the at_interval functions produce a sorting/filter criterion
      ...
      if (dx == py) {
        ...
        for (std::vector<db::Edge>::iterator e = cc; e != ff; ++e) {
          ...
          if (ec.first && edge_is_partially_left_of(ec.second, *e, xx)) {
            aa += (area_type(ec.second.dy()) *
                   area_type(2 * xx -
                             (ec.second.p2().x() + ec.second.p1().x()))) /
                  2;
            a += area_type(ec.second.dy()) * area_type(px);
          }
        }
        ...
        for (std::vector<db::Edge>::iterator e = cc; e != fff; ++e) {
          ...
          if (wide_ec.first &&
              edge_is_partially_left_of(wide_ec.second, *e, x + dx)) {
...
/**
 *  @brief A helper class that produces edges into an EdgeProcessor from a
 * sequence of points
 */
...
/**
 *  @brief Produce edges for the partial Minkowski sum of an edge with an input
 * polygon
 */
static void ms_production(const db::Polygon &a, const db::Point &p1,
                          const db::Point &p2, db::EdgeProcessor &ep) {
  ...
  //  Look for the points in the contour bounding the partial sum perpendicular
  //  to the edge
  ...
  while (true) {
    ...
    // detect inversion due to a convex pattern and create a cover polygon for
    // that case
    if (pcc_set) {
...
static db::Polygon ms_extraction(db::EdgeProcessor &ep, bool resolve_holes) {
  db::SimpleMerge op(-1);
...
static db::Polygon do_minkowski_sum(const db::Polygon &a, const db::Edge &b,
                                    bool resolve_holes) {
...
db::Polygon minkowski_sum(const db::Polygon &a, const db::Edge &b, bool rh) {
  if (a.holes() > 0) {
...
static db::Polygon do_minkowski_sum(const db::Polygon &a, const db::Polygon &b,
                                    bool resolve_holes) {
  ...
  db::EdgeProcessor ep;
  for (db::Polygon::polygon_edge_iterator e = b.begin_edge(); !e.at_end();
       ++e) {
...
db::Polygon minkowski_sum(const db::Polygon &a, const db::Polygon &b, bool rh) {
  if (a.holes() > 0) {
    return do_minkowski_sum(db::resolve_holes(a), b, rh);
  } else {
    return do_minkowski_sum(a, b, rh);
...
static db::Polygon do_minkowski_sum(const db::Polygon &a, const db::Box &b,
                                    bool resolve_holes) {
  return minkowski_sum(a, db::Polygon(b), resolve_holes);
}

db::Polygon minkowski_sum(const db::Polygon &a, const db::Box &b, bool rh) {
  if (a.holes() > 0) {
    return do_minkowski_sum(db::resolve_holes(a), b, rh);
  } else {
    return do_minkowski_sum(a, b, rh);
...
static db::Polygon do_minkowski_sum(const db::Polygon &a,
                                    const std::vector<db::Point> &c,
                                    bool resolve_holes) {
  db::EdgeProcessor ep;
...
db::Polygon minkowski_sum(const db::Polygon &a, const std::vector<db::Point> &c,
                          bool rh) {
  if (a.holes() > 0) {
...
db::Polygon resolve_holes(const db::Polygon &p) {
  db::EdgeProcessor ep;
  ep.insert_sequence(p.begin_edge());

  std::vector<db::Polygon> polygons;
  db::PolygonContainer pc(polygons);
  db::PolygonGenerator out(pc, true /*resolve holes*/,
                           false /*max coherence to get one polygon*/);
...
db::Polygon simple_polygon_to_polygon(const db::SimplePolygon &sp) {
  db::Polygon p;
  p.assign_hull(sp.begin_hull(), sp.end_hull());
  return p;
}

db::SimplePolygon polygon_to_simple_polygon(const db::Polygon &p) {
  if (p.holes() > 0) {
...
static void decompose_convex_helper(int depth, PreferredOrientation po,
                                    const db::SimplePolygon &sp,
                                    SimplePolygonSink &sink) {
  ...
  db::coord_traits<db::Coord>::distance_type min_edge =
      std::numeric_limits<db::coord_traits<db::Coord>::distance_type>::max();
  ...
  while (true) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (db::vprod_sign(ep, ec) > 0 &&
          skipped.find(ep.p2()) == skipped.end()) {
    ...
    db::Vector pv =
        db::Vector(ep.dy(), -ep.dx()) + db::Vector(ec.dy(), -ec.dx());
    bool ortho =
        ((ep.dx() == 0 || ep.dy() == 0) && (ec.dx() == 0 || ec.dy() == 0));

    if (po == PO_any || po == PO_horizontal || po == PO_htrapezoids ||
        (po == PO_vtrapezoids && ortho)) {
      if (pv.x() >= 0) {
    ...
    if (po == PO_any || po == PO_vertical || po == PO_vtrapezoids ||
        (po == PO_htrapezoids && ortho)) {
      if (pv.y() >= 0) {
    ...
    for (std::set<db::Vector>::const_iterator c = cuts.begin(); c != cuts.end();
         ++c) {
      ...
      db::coord_traits<db::Coord>::area_type min_dist =
          std::numeric_limits<db::coord_traits<db::Coord>::area_type>::max();

      for (size_t j = 1; j != n - 1; ++j) {

        db::Edge efc(sp.hull()[(imed + j) % n], sp.hull()[(imed + j + 1) % n]);
        db::Edge efp(sp.hull()[(imed + j + n - 1) % n],
                     sp.hull()[(imed + j) % n]);
        ...
        if (db::vprod_sign(nv, efc.d()) == 0) {
          ...
          if (efc.side_of(p) == 0) {
            ...
            db::coord_traits<db::Coord>::area_type d1 =
                db::sprod(efc.p1() - p, nv);
            db::coord_traits<db::Coord>::area_type d2 =
                db::sprod(efc.p2() - p, nv);
        ...
        if (x.first && x.second != efc.p2()) {
          ...
          db::coord_traits<db::Coord>::area_type dist =
              db::sprod(x.second - p, nv);
          if (dist >= 0 && dist < min_dist) {

            //  a will be the area of the half we cut off
            db::coord_traits<db::Coord>::area_type a =
                asum +
            ...
            db::coord_traits<db::Coord>::area_type atot_eff =
                atot +
            ...
            if (db::vprod_sign(nv, efc.d()) <= 0 && a >= 0 && a <= atot_eff) {
              ...
              if (x.second == efc.p1()) {
                ...
                db::coord_traits<db::Coord>::distance_type el = std::min(
                    x.second.distance(efc.p1()), x.second.distance(efc.p2()));
                if (el >= min_edge) {
      ...
      if (jmin_inner > 0 &&
          (!jmin || cut_rating_inner > cut_rating ||
           (cut_rating_inner == cut_rating && acutoff_inner < acutoff))) {
...
class ConvexDecompositionFilter : public db::SimplePolygonSink {
public:
  ConvexDecompositionFilter(PreferredOrientation po, bool swap_xy,
                            db::SimplePolygonSink *out)
  ...
  virtual void put(const db::SimplePolygon &polygon) {
    if (m_swap_xy) {
      ...
      decompose_convex_helper(std::numeric_limits<int>::max(), m_po, p,
                              *mp_out);
    } else {
      decompose_convex_helper(std::numeric_limits<int>::max(), m_po, polygon,
                              *mp_out);
    }
  }
...
void decompose_convex(const db::Polygon &p, PreferredOrientation po,
                      db::SimplePolygonSink &sink) {
  if (p.is_box()) {
    ...
    //  Because the hole resolution strategy favours horizontal lines we need to
    //  swap x and y for the PO_vertical and PO_vtrapezoids case
    ...
    if (swap_xy) {
      for (db::Polygon::polygon_edge_iterator e = p.begin_edge(); !e.at_end();
           ++e) {
...
void decompose_convex(const db::SimplePolygon &sp, PreferredOrientation po,
                      db::SimplePolygonSink &sink) {
  if (sp.is_box()) {
    sink.put(sp);
  } else {
...
template <class P> bool is_convex_helper(const P &p) {
...
bool is_convex(const db::SimplePolygon &p) { return is_convex_helper(p); }
...
static void decompose_convex_to_trapezoids(const db::SimplePolygon &sp,
                                           bool horizontal,
                                           db::SimplePolygonSink &sink) {
  if (sp.hull().size() < 3) {
    return;
  ...
  for (db::SimplePolygon::polygon_edge_iterator e = sp.begin_edge();
       !e.at_end(); ++e) {
    db::Edge ee = *e;
    if (!horizontal) {
  ...
  while (c != edges.end()) {
    ...
    if (x1 == x2) {
      ...
      db::Point pts[] = {db::Point(x1, y), db::Point(xx1, yy),
                         db::Point(xx2, yy)};
...
struct TrapezoidConverter : public db::SimplePolygonSink {
...
void decompose_trapezoids(const db::Polygon &p, TrapezoidDecompositionMode mode,
                          db::SimplePolygonSink &sink) {
  if (mode == TD_htrapezoids || mode == TD_vtrapezoids) {
    ...
    if (p.is_box()) {
      ...
      ConvexDecompositionFilter cd(mode == TD_htrapezoids ? PO_htrapezoids
                                                          : PO_vtrapezoids,
                                   swap_xy, &trap_maker);
...
void decompose_trapezoids(const db::SimplePolygon &sp,
  ...
  if (mode == TD_htrapezoids || mode == TD_vtrapezoids) {
    ...
    if (sp.is_box()) {
      ...
      TrapezoidConverter trap_maker(mode == TD_htrapezoids, &sink);
      decompose_convex_helper(std::numeric_limits<int>::max(),
                              mode == TD_htrapezoids ? PO_htrapezoids
                                                     : PO_vtrapezoids,
                              sp, trap_maker);
    }
...
db::Polygon snapped_polygon(const db::Polygon &poly, db::Coord gx, db::Coord gy,
  ...
  for (size_t i = 0; i < poly.holes() + 1; ++i) {
    ...
    for (db::Polygon::polygon_contour_iterator pt = b; pt != e; ++pt) {
      heap.push_back(
          db::Point(snap_to_grid((*pt).x(), gx), snap_to_grid((*pt).y(), gy)));
    }
...
db::Polygon scaled_and_snapped_polygon(const db::Polygon &poly, db::Coord gx,
                                       db::Coord mx, db::Coord dx, db::Coord ox,
                                       db::Coord gy, db::Coord my, db::Coord dy,
                                       db::Coord oy,
                                       std::vector<db::Point> &heap) {
  db::Polygon pnew;
  ...
  for (size_t i = 0; i < poly.holes() + 1; ++i) {
    ...
    for (db::Polygon::polygon_contour_iterator pt = b; pt != e; ++pt) {
      int64_t x = snap_to_grid(int64_t((*pt).x()) * mx + int64_t(ox), dgx) /
                  int64_t(dx);
      int64_t y = snap_to_grid(int64_t((*pt).y()) * my + int64_t(oy), dgy) /
                  int64_t(dy);
...
db::Vector scaled_and_snapped_vector(const db::Vector &v, db::Coord gx,
                                     db::Coord mx, db::Coord dx, db::Coord ox,
                                     db::Coord gy, db::Coord my, db::Coord dy,
                                     db::Coord oy) {
  int64_t dgx = int64_t(gx) * int64_t(dx);
  ...
  int64_t x =
      snap_to_grid(int64_t(v.x()) * mx + int64_t(ox), dgx) / int64_t(dx);
  int64_t y =
      snap_to_grid(int64_t(v.y()) * my + int64_t(oy), dgy) / int64_t(dy);
