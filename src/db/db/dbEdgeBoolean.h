...
enum EdgeBoolOp {
  ...
  EdgeAnd,
  EdgeIntersections,
  EdgeAndNot /*not always supported*/
};

struct OrJoinOp {
  void operator()(int &v, int n) { v += n; }
};
...
struct EdgeBooleanCluster : public db::cluster<db::Edge, size_t> {
  ...
  EdgeBooleanCluster(OutputContainer *output, OutputContainer *output2,
                     EdgeBoolOp op)
  ...
  void finish() {
    ...
    a.add(0, db::coord_traits<db::Coord>::rounded(d.double_length()), 0,
          or_jop);
    b.add(0, db::coord_traits<db::Coord>::rounded(d.double_length()), 0,
          or_jop);

    for (iterator o = begin(); o != end(); ++o) {
      db::Coord l1 = db::coord_traits<db::Coord>::rounded(
          db::sprod(db::Vector(o->first->p1() - p1), d) * n);
      db::Coord l2 = db::coord_traits<db::Coord>::rounded(
          db::sprod(db::Vector(o->first->p2() - p1), d) * n);
    ...
    for (tl::interval_map<db::Coord, int>::const_iterator ia = a.begin();
         ia != a.end(); ++ia) {
      q.add(ia->first.first, ia->first.second,
            ia->second > 0 ? 1 : (ia->second < 0 ? -1 : 0), or_jop);
    }
    ...
    if (b.begin() == b.end()) {
      ...
      if (oc) {
        for (tl::interval_map<db::Coord, int>::const_iterator ib = b.begin();
             ib != b.end(); ++ib) {
          if (ib->second > 0) {
            oc->insert(db::Edge(p1 + db::Vector(d * (ib->first.first * n)),
                                p1 + db::Vector(d * (ib->first.second * n))));
          } else if (ib->second < 0) {
            oc->insert(db::Edge(p1 + db::Vector(d * (ib->first.second * n)),
                                p1 + db::Vector(d * (ib->first.first * n))));
          }
        }
      }
      ...
      for (tl::interval_map<db::Coord, int>::const_iterator iq = q.begin();
           iq != q.end(); ++iq) {
        if (iq->second > 0) {
          mp_output->insert(
              db::Edge(p1 + db::Vector(d * (iq->first.first * n)),
                       p1 + db::Vector(d * (iq->first.second * n))));
        } else if (iq->second < 0) {
          mp_output->insert(
              db::Edge(p1 + db::Vector(d * (iq->first.second * n)),
                       p1 + db::Vector(d * (iq->first.first * n))));
        }
      }

      for (tl::interval_map<db::Coord, int>::const_iterator iq = q2.begin();
           iq != q2.end(); ++iq) {
        if (iq->second > 0) {
          mp_output2->insert(
              db::Edge(p1 + db::Vector(d * (iq->first.first * n)),
...
struct EdgeBooleanClusterCollector
    : public db::cluster_collector<db::Edge, size_t,
                                   EdgeBooleanCluster<OutputContainer>> {
  EdgeBooleanClusterCollector(OutputContainer *output, EdgeBoolOp op,
                              OutputContainer *output2 = 0)
      : db::cluster_collector<db::Edge, size_t,
                              EdgeBooleanCluster<OutputContainer>>(
            EdgeBooleanCluster<OutputContainer>(
                output, output2, op == EdgeIntersections ? EdgeAnd : op),
            op != EdgeAnd && op != EdgeIntersections /*report single*/),
        mp_output(output),
        mp_intersections(op == EdgeIntersections ? output : 0) {
    //  .. nothing yet ..
  }

  EdgeBooleanClusterCollector(OutputContainer *output,
                              OutputContainer *intersections, EdgeBoolOp op)
      : db::cluster_collector<db::Edge, size_t,
                              EdgeBooleanCluster<OutputContainer>>(
            EdgeBooleanCluster<OutputContainer>(output, op),
            op != EdgeAnd /*report single*/),
  ...
  void add(const db::Edge *o1, size_t p1, const db::Edge *o2, size_t p2) {
    ...
    if (!o1->is_degenerate() && !o2->is_degenerate() &&
        fabs((double)db::vprod(*o1, *o2)) <
            db::coord_traits<db::Coord>::prec_distance() *
                std::min(o1->double_length(), o2->double_length()) &&
        (o1->p1() == o2->p1() || o1->p1() == o2->p2() || o1->p2() == o2->p1() ||
         o1->p2() == o2->p2() || o1->coincident(*o2))) {

      db::cluster_collector<db::Edge, size_t,
                            EdgeBooleanCluster<OutputContainer>>::add(o1, p1,
                                                                      o2, p2);
  ...
  /**
   *  @brief A receiver for the reducer which removes points that are on the
   * edges
   */
  ...
  public:
    ...
    void add(const db::Edge *e, const size_t &, const db::Point *pt,
             const size_t &) {
      if (e->contains(*pt)) {
  ...
  struct PointInserter
    ...
    PointInserter(OutputContainer *output) : mp_output(output) {}
   ...
   *  This method pushes those points which don't interact with the edges to the
   * output container as degenerate edges. It needs to be called after the pass
   * has been made.
   */
  void finalize(bool) {
    ...
    db::box_scanner2<db::Edge, size_t, db::Point, size_t>
        intersections_to_edge_scanner;
    for (typename OutputContainer::const_iterator e = mp_output->begin();
         e != mp_output->end(); ++e) {
      intersections_to_edge_scanner.insert1(e.operator->(), 0);
    }
    for (std::set<db::Point>::const_iterator p = m_intersections.begin();
         p != m_intersections.end(); ++p) {
    ...
    intersections_to_edge_scanner.process(rpoe, 1, db::box_convert<db::Edge>(),
                                          db::box_convert<db::Point>());

    std::set_difference(m_intersections.begin(), m_intersections.end(),
                        points_to_remove.begin(), points_to_remove.end(),
                        PointInserter(mp_intersections));
  }
...
/**
 *  @brief A helper class to use db::Shapes as container for
 * EdgeBooleanClusterCollector
 */
struct DB_PUBLIC ShapesToOutputContainerAdaptor {
public:
  struct Iterator : public db::Shapes::shape_iterator {
    Iterator(const db::Shapes::shape_iterator &iter)
        : db::Shapes::shape_iterator(iter) {}

    Iterator() : db::Shapes::shape_iterator() {}

    const db::Edge *operator->() const {
      return (db::Shapes::shape_iterator::operator*())
          .basic_ptr(db::Edge::tag());
    }
  ...
  ShapesToOutputContainerAdaptor() : mp_shapes(0), m_prop_id(0) {
    //  .. nothing yet ..
  }
...
/**
 *  @brief A specialization of the EdgeBooleanClusterCollector for a Shapes
 * output container
 */
struct DB_PUBLIC EdgeBooleanClusterCollectorToShapes
  ...
  EdgeBooleanClusterCollectorToShapes(db::Shapes *output, EdgeBoolOp op,
                                      db::properties_id_type prop_id = 0)
      : EdgeBooleanClusterCollector<ShapesToOutputContainerAdaptor>(&m_adaptor,
                                                                    op),
        m_adaptor(*output, prop_id) {}

  EdgeBooleanClusterCollectorToShapes(db::Shapes *output, EdgeBoolOp op,
                                      db::Shapes *output2,
                                      db::properties_id_type prop_id = 0)
      : EdgeBooleanClusterCollector<ShapesToOutputContainerAdaptor>(
            &m_adaptor, op, &m_adaptor2),
        m_adaptor(*output, prop_id), m_adaptor2(*output2, prop_id) {}
