...
RecursiveShapeIterator::RecursiveShapeIterator(
    const RecursiveShapeIterator &d) {
  operator=(d);
}
...
RecursiveShapeIterator::RecursiveShapeIterator(const layout_type &layout,
...
RecursiveShapeIterator::RecursiveShapeIterator(
    const layout_type &layout, const cell_type &cell,
    const std::vector<unsigned int> &layers, const box_type &region,
    bool overlapping)
...
RecursiveShapeIterator::RecursiveShapeIterator(
    const layout_type &layout, const cell_type &cell,
    const std::vector<unsigned int> &layers, const region_type &region,
    bool overlapping)
    : m_box_convert(layout) {
  m_layer = 0;
  m_layers = layers;
...
RecursiveShapeIterator::RecursiveShapeIterator(
    const layout_type &layout, const cell_type &cell,
    const std::set<unsigned int> &layers, const box_type &region,
    bool overlapping)
    : m_box_convert(layout) {
  m_layer = 0;
...
RecursiveShapeIterator::RecursiveShapeIterator(
    const layout_type &layout, const cell_type &cell,
    const std::set<unsigned int> &layers, const region_type &region,
    bool overlapping)
    : m_box_convert(layout) {
  m_layer = 0;
  m_layers.insert(m_layers.end(), layers.begin(), layers.end());
...
void RecursiveShapeIterator::init_region(
    const RecursiveShapeIterator::box_type &region) {
  m_region = region;
  mp_complex_region.reset(0);
}

void RecursiveShapeIterator::init_region(
    const RecursiveShapeIterator::region_type &region) {
  if (region.empty()) {
    ...
    m_region = region.bbox();
    //  A small optimization. We can do this since we merge and translate to
    //  trapezoids anyway.
...
void RecursiveShapeIterator::set_layers(
    const std::vector<unsigned int> &layers) {
...
struct BoxTreePusher : public db::SimplePolygonSink {
  BoxTreePusher(RecursiveShapeIterator::box_tree_type *bt) : mp_bt(bt) {
    //  .. nothing yet ..
  }

  void put(const db::SimplePolygon &sp) { mp_bt->insert(sp.box()); }
...
void RecursiveShapeIterator::validate(RecursiveShapeReceiver *receiver) const {
  ...
  if (mp_complex_region.get()) {
    ...
    //  Use a merge and the trapezoid generator to produce a decomposition that
    //  goes into the complex region

    db::EdgeProcessor ep;
    size_t n = 0;
    for (region_type::const_iterator p = mp_complex_region->begin();
         !p.at_end(); ++p, ++n) {
      ep.insert(*p, n);
    }
  ...
  if (mp_shapes) {
    //  Ensures the trees are built properly - this is important in MT contexts
    //  (i.e. TilingProcessor)
    //  TODO: get rid of that const cast
    (const_cast<db::Shapes *>(mp_shapes))->update();
    start_shapes();
  } else if (mp_layout &&
             (!m_has_layers || m_current_layer < m_layers.size())) {
    //  Ensures the trees are built properly - this is important in MT contexts
    //  (i.e. TilingProcessor)
    mp_layout->update();
    new_cell(receiver);
...
void RecursiveShapeIterator::unselect_cells(
    const std::set<db::cell_index_type> &cells) {
  if (mp_layout) {

    for (std::set<db::cell_index_type>::const_iterator c = cells.begin();
         c != cells.end(); ++c) {
      m_stop.insert(*c);
      m_start.erase(*c);
    }
...
void RecursiveShapeIterator::unselect_all_cells() {
  if (mp_layout) {
    ...
    m_start.clear();
    for (db::Layout::const_iterator c = mp_layout->begin();
         c != mp_layout->end(); ++c) {
...
void RecursiveShapeIterator::select_cells(
    const std::set<db::cell_index_type> &cells) {
  if (mp_layout) {
...
std::vector<db::InstElement> RecursiveShapeIterator::path() const {
  std::vector<db::InstElement> elements;
  for (size_t i = 0;
       i < m_inst_array_iterators.size() && i < m_inst_iterators.size(); ++i) {
    elements.push_back(
        db::InstElement(*m_inst_iterators[i], m_inst_array_iterators[i]));
  }
  return elements;
}

RecursiveShapeIterator::box_type RecursiveShapeIterator::bbox() const {
  box_type box;
  if (mp_shapes) {
    box = mp_shapes->bbox();
  } else if (mp_top_cell) {
    if (m_has_layers) {
      for (std::vector<unsigned int>::const_iterator l = m_layers.begin();
           l != m_layers.end(); ++l) {
...
void RecursiveShapeIterator::next_shape(
    RecursiveShapeReceiver *receiver) const {
  while (at_end()) {
    ...
    if (m_has_layers && m_current_layer < m_layers.size() && !is_inactive()) {
      ...
      if (!m_inst.at_end() && int(m_inst_iterators.size()) < m_max_depth) {
        ...
        //  determine whether the cell is empty with respect to the layers
        //  specified
        bool is_empty = false;
        if (receiver && receiver->wants_all_cells()) {
          ...
          is_empty =
              mp_layout->cell(m_inst->cell_index()).bbox(m_layer).empty();

        } else {

          std::map<db::cell_index_type, bool>::const_iterator ec =
              m_empty_cells_cache.find(m_inst->cell_index());
          if (ec != m_empty_cells_cache.end()) {
            ...
            is_empty = true;
            for (std::vector<unsigned int>::const_iterator l = m_layers.begin();
                 l != m_layers.end() && is_empty; ++l) {
              is_empty = mp_layout->cell(m_inst->cell_index()).bbox(*l).empty();
            }

            m_empty_cells_cache.insert(
                std::make_pair(m_inst->cell_index(), is_empty));
          }
        }
...
void RecursiveShapeIterator::down(RecursiveShapeReceiver *receiver) const {
  tl_assert(mp_layout);
  ...
  //  don't transform the world region, since transformation of that region
  ...
  if (!m_local_complex_region_stack.empty()) {
    ...
    if (!new_region.empty()) {
      ...
      db::CellInstArray::complex_trans_type tinst =
          m_inst->complex_trans(*m_inst_array);
      db::CellInstArray::complex_trans_type tinst_inv = tinst.inverted();

      db::Box bb;

      for (box_tree_type::touching_iterator b = pcl.begin_touching(
               new_region.transformed(tinst), db::box_convert<db::Box>());
           !b.at_end(); ++b) {
      ...
      //  re-adjust the new local region, so we take into account additional
      //  clipping by the complex region. in the extreme case, this box is
      //  empty:
  ...
  if (receiver) {
    receiver->enter_cell(this, cell(), m_local_region_stack.back(),
                         m_local_complex_region_stack.empty()
                             ? 0
                             : &m_local_complex_region_stack.back());
  }
...
void RecursiveShapeIterator::start_shapes() const {
  if (!m_overlapping) {
    m_shape =
        mp_shapes->begin_touching(m_local_region_stack.back(), m_shape_flags,
                                  mp_shape_prop_sel, m_shape_inv_prop_sel);
  } else {
    m_shape =
        mp_shapes->begin_overlapping(m_local_region_stack.back(), m_shape_flags,
                                     mp_shape_prop_sel, m_shape_inv_prop_sel);
  }
...
void RecursiveShapeIterator::new_layer() const {
  if (int(m_trans_stack.size()) < m_min_depth ||
      int(m_trans_stack.size()) > m_max_depth) {
    m_shape = shape_iterator();
  } else if (!m_overlapping) {
    m_shape = cell()->shapes(m_layer).begin_touching(
        m_local_region_stack.back(), m_shape_flags, mp_shape_prop_sel,
        m_shape_inv_prop_sel);
  } else {
    m_shape = cell()->shapes(m_layer).begin_overlapping(
        m_local_region_stack.back(), m_shape_flags, mp_shape_prop_sel,
        m_shape_inv_prop_sel);
  }
...
void RecursiveShapeIterator::new_inst(RecursiveShapeReceiver *receiver) const {
  //  look for the next instance with a non-empty array iterator. The latter can
  //  be empty because we use a per-layer box converter for that case what we
  //  don't for the touching instance iterator.
  while (!m_inst.at_end()) {
    ...
    if (m_local_region_stack.back() != box_type::world() &&
        !m_inst->cell_inst()
             .bbox(m_box_convert)
             .inside(m_local_region_stack.back())) {
      with_region = true;
    } else {
      //  TODO: optimization potential: only report all_of_instance == false, if
      //  not entirely within the complex region
      all_of_instance = m_local_complex_region_stack.empty();
    }

    RecursiveShapeReceiver::new_inst_mode ni = RecursiveShapeReceiver::NI_all;
    if (receiver) {
      ni = receiver->new_inst(this, m_inst->cell_inst(), always_apply(),
                              m_local_region_stack.back(),
                              m_local_complex_region_stack.empty()
                                  ? 0
                                  : &m_local_complex_region_stack.back(),
                              all_of_instance);
    ...
    if (ni == RecursiveShapeReceiver::NI_skip) {
      ...
      //  a singular iterator
      m_inst_array =
          db::CellInstArray::iterator(m_inst->cell_inst().front(), false);
    } else if (with_region) {
      m_inst_array = m_inst->cell_inst().begin_touching(
          m_local_region_stack.back(), m_box_convert);
    } else {
...
void RecursiveShapeIterator::new_inst_member(
    RecursiveShapeReceiver *receiver) const {
  if (!m_local_complex_region_stack.empty()) {

    //  skip instance array members not part of the complex region
    while (!m_inst_array.at_end()) {
      db::Box ia_box = m_inst->complex_trans(*m_inst_array) *
                       cell_bbox(m_inst->cell_index());
  ...
  while (!m_inst_array.at_end() && receiver) {
    if (receiver->new_inst_member(this, m_inst->cell_inst(), always_apply(),
                                  m_inst->complex_trans(*m_inst_array),
                                  m_local_region_stack.back(),
                                  m_local_complex_region_stack.empty()
                                      ? 0
...
bool RecursiveShapeIterator::is_outside_complex_region(
    const db::Box &box) const {
  if (m_overlapping) {
    return m_local_complex_region_stack.back()
        .begin_overlapping(box, db::box_convert<db::Box>())
        .at_end();
  } else {
    return m_local_complex_region_stack.back()
        .begin_touching(box, db::box_convert<db::Box>())
        .at_end();
  }
}

bool RecursiveShapeIterator::is_child_inactive(
    db::cell_index_type new_child) const {
...
void RecursiveShapeIterator::push(RecursiveShapeReceiver *receiver) {
  ...
  try {
    ...
    while (!at_end()) {
      receiver->shape(this, *m_shape, always_apply(), m_trans,
                      m_local_region_stack.back(),
                      m_local_complex_region_stack.empty()
                          ? 0
                          : &m_local_complex_region_stack.back());
