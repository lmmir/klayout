...
static tl::RegisteredClass<db::DeviceClassTemplateBase>
    dct_cap(new db::device_class_template<db::DeviceClassCapacitor>("CAP"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_cap_with_bulk(
    new db::device_class_template<db::DeviceClassCapacitorWithBulk>("CAP3"));
static tl::RegisteredClass<db::DeviceClassTemplateBase>
    dct_res(new db::device_class_template<db::DeviceClassResistor>("RES"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_res_with_bulk(
    new db::device_class_template<db::DeviceClassResistorWithBulk>("RES3"));
static tl::RegisteredClass<db::DeviceClassTemplateBase>
    dct_ind(new db::device_class_template<db::DeviceClassInductor>("IND"));
static tl::RegisteredClass<db::DeviceClassTemplateBase>
    dct_diode(new db::device_class_template<db::DeviceClassDiode>("DIODE"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_mos3(
    new db::device_class_template<db::DeviceClassMOS3Transistor>("MOS3"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_mos4(
    new db::device_class_template<db::DeviceClassMOS4Transistor>("MOS4"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_bjt3(
    new db::device_class_template<db::DeviceClassBJT3Transistor>("BJT3"));
static tl::RegisteredClass<db::DeviceClassTemplateBase> dct_bjt4(
    new db::device_class_template<db::DeviceClassBJT4Transistor>("BJT4"));
...
class TwoTerminalDeviceCombiner : public db::DeviceCombiner {
public:
...
class ResistorDeviceCombiner : public TwoTerminalDeviceCombiner {
public:
  void parallel(Device *a, Device *b) const {
    ...
    //  parallel width is sum of both, length is the one that gives the same
    //  value of resistance
    //    R = 1/(1/R1 + 1/R2)
    //    R = L/(W1+W2)
    //    R1 = L1/W1
  ...
  void serial(Device *a, Device *b) const {
    ...
    //  parallel length is sum of both, width is the one that gives the same
    //  value of resistance assuming same sheet rho
    //    R = R1+R2
    //    R = (L1+L2)/W
    //    R1 = L1/W1
    //    R2 = L2/W2
...
class ResistorWithBulkDeviceCombiner : public ResistorDeviceCombiner {
public:
...
class CapacitorDeviceCombiner : public TwoTerminalDeviceCombiner {
public:
  void serial(Device *a, Device *b) const {
    double va = a->parameter_value(0);
    double vb = b->parameter_value(0);
...
class CapacitorWithBulkDeviceCombiner : public CapacitorDeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
    db::Net *nab = a->net_for_terminal(2);
    db::Net *nbb = b->net_for_terminal(2);
...
class InductorDeviceCombiner : public TwoTerminalDeviceCombiner {
public:
  void parallel(Device *a, Device *b) const {
    double va = a->parameter_value(0);
    double vb = b->parameter_value(0);
...
class DiodeDeviceCombiner : public db::DeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
...
class MOS3DeviceCombiner : public db::DeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
    ...
    if (((nas == nbs && nad == nbd) || (nas == nbd && nad == nbs)) &&
        nag == nbg) {
...
class MOS4DeviceCombiner : public MOS3DeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
    const db::Net *nas = a->net_for_terminal(0);
    const db::Net *nag = a->net_for_terminal(1);
...
class BJT3DeviceCombiner : public db::DeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
  ...
  void combine_parameters(Device *a, Device *b) const {
    a->set_parameter_value(
        DeviceClassBJT3Transistor::param_id_AE,
        a->parameter_value(DeviceClassBJT3Transistor::param_id_AE) +
            b->parameter_value(DeviceClassBJT3Transistor::param_id_AE));
    a->set_parameter_value(
        DeviceClassBJT3Transistor::param_id_PE,
        a->parameter_value(DeviceClassBJT3Transistor::param_id_PE) +
            b->parameter_value(DeviceClassBJT3Transistor::param_id_PE));
    a->set_parameter_value(
        DeviceClassBJT3Transistor::param_id_NE,
        a->parameter_value(DeviceClassBJT3Transistor::param_id_NE) +
            b->parameter_value(DeviceClassBJT3Transistor::param_id_NE));
  }
};

class BJT4DeviceCombiner : public BJT3DeviceCombiner {
public:
  bool combine_devices(Device *a, Device *b) const {
    const db::Net *nac = a->net_for_terminal(0);
    const db::Net *nab = a->net_for_terminal(1);
...
DeviceClassResistor::DeviceClassResistor() {
  ...
  add_parameter_definition(
      db::DeviceParameterDefinition("R", "Resistance (Ohm)", 0.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "L", "Length (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "W", "Width (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "A", "Area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "P", "Perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
}
...
DeviceClassResistorWithBulk::DeviceClassResistorWithBulk()
  ...
  add_terminal_definition(
      db::DeviceTerminalDefinition("W", "Terminal W (well, bulk)"));
}
...
DeviceClassCapacitor::DeviceClassCapacitor() {
  ...
  add_parameter_definition(
      db::DeviceParameterDefinition("C", "Capacitance (Farad)", 0.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "A", "Area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "P", "Perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
...
DeviceClassInductor::DeviceClassInductor() {
  ...
  add_parameter_definition(
      db::DeviceParameterDefinition("L", "Inductance (Henry)", 0.0));
}
...
DeviceClassMOS3Transistor::DeviceClassMOS3Transistor() {
  ...
  add_parameter_definition(db::DeviceParameterDefinition(
      "L", "Gate length (micrometer)", 0.0, true, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "W", "Gate width (micrometer)", 0.0, true, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "AS", "Source area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "AD", "Drain area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "PS", "Source perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "PD", "Drain perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
}
...
bool DeviceClassMOS3Transistor::is_source_terminal(size_t tid) const {
  if (is_strict()) {
    ...
    return tid == DeviceClassMOS3Transistor::terminal_id_S ||
           tid == DeviceClassMOS3Transistor::terminal_id_D;
  }
}
...
bool DeviceClassMOS3Transistor::lengths_are_identical(const db::Device *a,
                                                      const db::Device *b) {
  return (fabs(a->parameter_value(DeviceClassMOS3Transistor::param_id_L) -
               b->parameter_value(DeviceClassMOS3Transistor::param_id_L)) <
          1e-6);
}

bool DeviceClassMOS3Transistor::net_is_source_drain_connection(
  ...
  return ((is_source_terminal(t1->terminal_id()) &&
           is_drain_terminal(t2->terminal_id())) ||
          (is_drain_terminal(t1->terminal_id()) &&
           is_source_terminal(t2->terminal_id())));
}
...
public:
  ...
  bool is_compatible(const SplitGateDeviceChain &other, bool with_bulk) const {
    ...
    for (; d != end_devices(); ++d, ++dd) {
      if ((*d)->net_for_terminal(DeviceClassMOS3Transistor::terminal_id_G) !=
          (*dd)->net_for_terminal(DeviceClassMOS3Transistor::terminal_id_G)) {
        return false;
      }
      if (with_bulk &&
          (*d)->net_for_terminal(DeviceClassMOS4Transistor::terminal_id_B) !=
              (*dd)->net_for_terminal(
                  DeviceClassMOS4Transistor::terminal_id_B)) {
        return false;
      }
  ...
  void join_nets(const SplitGateDeviceChain &other,
                 db::Circuit *circuit) const {
    net_iterator n = begin_nets(), nn = other.begin_nets();
    for (; n != end_nets(); ++n, ++nn) {
      if (tl::verbosity() >= 40) {
        tl::log << "Joining nets: " << (*n)->expanded_name() << " and "
                << (*nn)->expanded_name();
      }
...
void DeviceClassMOS3Transistor::join_split_gates(db::Circuit *circuit) const {
  tl::SelfTimer timer(tl::verbosity() >= 31,
                      tl::to_string(tr("join split gates ")) + name() + " (" +
                          circuit->name() + ")");
  ...
  for (db::Circuit::net_iterator n = circuit->begin_nets();
    ...
    for (db::Net::const_terminal_iterator t = n->begin_terminals();
      ...
      if (t->device_class() == this && is_source_terminal(t->terminal_id())) {
        ...
        while (true) {
          ...
          size_t other_tid = (tid == DeviceClassMOS3Transistor::terminal_id_S
                                  ? DeviceClassMOS3Transistor::terminal_id_D
                                  : DeviceClassMOS3Transistor::terminal_id_S);
          ...
          for (db::Net::const_terminal_iterator tt = nn->begin_terminals();
               tt != nn->end_terminals(); ++tt) {
            if (tt->device() != d) {
    ...
    for (std::map<const db::Net *, std::list<SplitGateDeviceChain>>::iterator
             cs = chains.begin();
         cs != chains.end(); ++cs) {
      ...
      while (!cs->second.empty()) {
        ...
        for (std::vector<
                 std::list<SplitGateDeviceChain>::iterator>::const_iterator i =
                 compatibles.begin();
             i != compatibles.end(); ++i) {
...
bool DeviceClassMOS3Transistor::has_bulk_pin() const { return false; }
...
bool DeviceClassMOS4Transistor::has_bulk_pin() const { return true; }
...
DeviceClassBJT3Transistor::DeviceClassBJT3Transistor() {
  ...
  add_parameter_definition(db::DeviceParameterDefinition(
      "AE", "Emitter area (square micrometer)", 0.0, true, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "PE", "Emitter perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "AB", "Base area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "PB", "Base perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "AC", "Collector area (square micrometer)", 0.0, false, 1e-12, 2.0));
  add_parameter_definition(db::DeviceParameterDefinition(
      "PC", "Collector perimeter (micrometer)", 0.0, false, 1e-6, 1.0));
  add_parameter_definition(
      db::DeviceParameterDefinition("NE", "Emitter count", 1.0, true));
}
