...
const std::vector<db::Polygon> &TextGenerator::glyph(char c) const {
  std::map<char, std::vector<db::Polygon>>::const_iterator dc =
      m_data.find(m_lowercase_supported ? c : toupper(c));
  if (dc != m_data.end()) {
    return dc->second;
  } else {
...
db::Region TextGenerator::glyph_as_region(char c) const {
  db::Region region;
  std::map<char, std::vector<db::Polygon>>::const_iterator dc =
      m_data.find(m_lowercase_supported ? c : toupper(c));
  if (dc != m_data.end()) {
    for (std::vector<db::Polygon>::const_iterator p = dc->second.begin();
         p != dc->second.end(); ++p) {
      region.insert(*p);
    }
  }
...
void TextGenerator::text(const std::string &t, double target_dbu, double mag,
                         bool inv, double bias, double char_spacing,
                         double line_spacing,
                         std::vector<db::Polygon> &data) const {
  data.clear();
  db::EdgeProcessor ep;
  ...
  db::Coord x = 0, y = 0;
  db::Coord dx = db::coord_traits<db::Coord>::rounded(
      m * width() + char_spacing / target_dbu);
  db::Coord dy = db::coord_traits<db::Coord>::rounded(
      m * height() + line_spacing / target_dbu);
  ...
  for (const char *cp = t.c_str(); *cp; ++cp) {
    ...
    if (c) {
      ...
      for (std::vector<db::Polygon>::const_iterator d = g.begin(); d != g.end();
           ++d) {
...
db::Region TextGenerator::text_as_region(const std::string &t,
                                         double target_dbu, double mag,
                                         bool inv, double bias,
                                         double char_spacing,
...
void TextGenerator::load_from_data(const char *data, size_t ndata,
...
void TextGenerator::read_from_layout(const db::Layout &layout, unsigned int l1,
  ...
  if (cn.first) {
    ...
    db::Shapes::shape_iterator sh =
        layout.cell(cn.second).shapes(l1).begin(db::ShapeIterator::All);
    while (!sh.at_end()) {
  ...
  m_lowercase_supported =
      layout.cell_by_name("a").first || layout.cell_by_name("065").first;
  ...
  for (int ch = 32; ch < 128; ++ch) {
    ...
    if (cn.first) {
      ...
      std::vector<db::Polygon> &data =
          m_data.insert(std::make_pair(char(ch), std::vector<db::Polygon>()))
              .first->second;
...
const TextGenerator *TextGenerator::generator_by_name(const std::string &name) {
  const std::vector<TextGenerator> &fonts = generators();
  for (std::vector<TextGenerator>::const_iterator f = fonts.begin();
       f != fonts.end(); ++f) {
    if (f->name() == name) {
      return f.operator->();
    }
...
const TextGenerator *TextGenerator::default_generator() {
  const std::vector<TextGenerator> &fonts = generators();
...
std::vector<std::string> TextGenerator::font_paths() { return s_font_paths; }

const std::vector<TextGenerator> &TextGenerator::generators() {
  if (!s_fonts_loaded) {
    ...
    //  scan for font files
    for (std::vector<std::string>::const_iterator p = s_font_paths.begin();
         p != s_font_paths.end(); ++p) {

      if (tl::file_exists(*p)) {

        std::vector<std::string> font_files =
            tl::dir_entries(*p, true, false, true);
        for (std::vector<std::string>::const_iterator ff = font_files.begin();
             ff != font_files.end(); ++ff) {
