...
LayoutStateModel::LayoutStateModel(bool busy)
    : m_hier_dirty(false), m_hier_generation_id(0), m_all_bboxes_dirty(false),
      m_busy(busy) {
  //  .. nothing yet ..
}
...
LayoutStateModel::LayoutStateModel(const LayoutStateModel &d)
    : m_hier_dirty(d.m_hier_dirty),
      ...
      m_all_bboxes_dirty(d.m_all_bboxes_dirty), m_busy(d.m_busy) {
  //  .. nothing yet ..
}

LayoutStateModel &LayoutStateModel::operator=(const LayoutStateModel &d) {
  m_hier_dirty = d.m_hier_dirty;
  m_hier_generation_id = d.m_hier_generation_id;
  m_bboxes_dirty = d.m_bboxes_dirty;
...
void LayoutStateModel::do_invalidate_hier() { hier_changed_event(); }
  ...
  if (index == std::numeric_limits<unsigned int>::max()) {
    if (!m_all_bboxes_dirty || m_busy) {
      do_invalidate_bboxes(
          index); //  must be called before the bboxes are invalidated (stopping
                  //  of redraw thread requires this)
      m_all_bboxes_dirty = true;
    }
  } else {
    if ((!m_all_bboxes_dirty && (index >= (unsigned int)m_bboxes_dirty.size() ||
                                 !m_bboxes_dirty[index])) ||
        m_busy) {
      do_invalidate_bboxes(
          index); //  must be called before the bboxes are invalidated (stopping
                  //  of redraw thread requires this)
