 ...
 *
 *  This object is used in the netlist extractor and represents either a polygon
 * or a text. The TextRef shall utilize a StringRef to represent the string.
 */
...
public:
  ...
  /**
   *  @brief A NetShape object representing a Polygon from the given shape
   * repository
   */
  ...
  /**
   *  @brief A NetShape object representing a Text from the given shape
   * repository
   */
  ...
  bool operator==(const NetShape &net_shape) const {
    return m_ptr == net_shape.m_ptr && m_dx == net_shape.m_dx &&
           m_dy == net_shape.m_dy;
  }
  ...
  /**
   *  @brief Inserts the object into a Shapes collection with the given
   * properties ID
   */
   ...
   */
  template <class Tr>
  bool interacts_with_transformed(const db::NetShape &other,
 ...
 */
template <> struct box_convert<db::NetShape> {
...
template <> struct addressable_object_from_shape<db::NetShape> {
