...
void insert_transformed(db::Layout &layout, db::Shapes &shapes, const Shape &s,
                        const Trans &t);

template <class Trans>
void insert_transformed(db::Layout &layout, db::Shapes &shapes,
                        const db::PolygonRef &s, const Trans &t) {
...
template <class Trans>
void insert_transformed(db::Layout &layout, db::Shapes &shapes,
                        const db::NetShape &s, const Trans &t) {
...
static bool
is_breakout_cell(const std::set<db::cell_index_type> *breakout_cells,
...
Connectivity::Connectivity() : m_ec(Connectivity::EdgesConnectCollinear) {
  //  .. nothing yet ..
}

Connectivity::Connectivity(edge_connectivity_type ec) : m_ec(ec) {
  //  .. nothing yet ..
}

void Connectivity::connect(unsigned int la, unsigned int lb) {
...
void Connectivity::connect(unsigned int l) {
  m_connected[l].insert(l);
  m_all_layers.insert(l);
}

void Connectivity::connect(const db::DeepLayer &l) { connect(l.layer()); }

void Connectivity::connect(const db::DeepLayer &la, const db::DeepLayer &lb) {
...
Connectivity::begin_global_connections(unsigned int l) const {
  std::map<unsigned int, global_nets_type>::const_iterator g =
      m_global_connections.find(l);
...
size_t Connectivity::connect_global(unsigned int l, const std::string &gn) {
...
size_t Connectivity::connect_global(const db::DeepLayer &l,
                                    const std::string &gn) {
  return connect_global(l.layer(), gn);
}

const std::string &Connectivity::global_net_name(size_t id) const {
  tl_assert(id < m_global_net_names.size());
  return m_global_net_names[id];
}

size_t Connectivity::global_net_id(const std::string &gn) {
  for (std::vector<std::string>::const_iterator i = m_global_net_names.begin();
       i != m_global_net_names.end(); ++i) {
    if (*i == gn) {
...
size_t Connectivity::global_nets() const { return m_global_net_names.size(); }

Connectivity::layer_iterator Connectivity::begin_layers() const {
  return m_all_layers.begin();
}

Connectivity::layer_iterator Connectivity::end_layers() const {
...
Connectivity::begin_connected(unsigned int layer) const {
  std::map<unsigned int, layers_type>::const_iterator i =
      m_connected.find(layer);
...
template <class Trans>
static bool interaction_test(const db::PolygonRef &a, const db::PolygonRef &b,
                             const Trans &trans,
                             db::Connectivity::edge_connectivity_type) {
  //  TODO: this could be part of db::interact (including transformation)
  if (a.obj().is_box() && b.obj().is_box()) {
    return db::interact(a.obj().box().transformed(a.trans()),
                        b.obj().box().transformed(trans * Trans(b.trans())));
  } else {
    return db::interact(a.obj().transformed(a.trans()),
                        b.obj().transformed(trans * Trans(b.trans())));
  }
}

template <class C>
static bool interaction_test(const db::PolygonRef &a, const db::PolygonRef &b,
                             const db::unit_trans<C> &,
                             db::Connectivity::edge_connectivity_type) {
  //  TODO: this could be part of db::interact (including transformation)
  if (a.obj().is_box() && b.obj().is_box()) {
    return db::interact(a.obj().box().transformed(a.trans()),
                        b.obj().box().transformed(b.trans()));
  } else {
    return db::interact(a.obj().transformed(a.trans()),
                        b.obj().transformed(b.trans()));
  }
}

template <class Trans>
static bool interaction_test(const db::NetShape &a, const db::NetShape &b,
                             const Trans &trans,
                             db::Connectivity::edge_connectivity_type) {
...
template <class Trans>
static bool interaction_test(const db::Edge &a, const db::Edge &b,
                             const Trans &trans,
                             db::Connectivity::edge_connectivity_type ec) {
...
bool Connectivity::interacts(const T &a, unsigned int la, const T &b,
                             unsigned int lb, const Trans &trans) const {
...
bool Connectivity::interacts(const std::set<unsigned int> &la,
                             const std::set<unsigned int> &lb) const {
  for (std::set<unsigned int>::const_iterator i = la.begin(); i != la.end();
       ++i) {
    db::Connectivity::layer_iterator je = end_connected(*i);
    for (db::Connectivity::layer_iterator j = begin_connected(*i); j != je;
         ++j) {
      if (lb.find(*j) != lb.end()) {
        return true;
      }
...
bool Connectivity::interact(const db::Cell &a, const db::Cell &b) const {
  for (std::map<unsigned int, layers_type>::const_iterator i =
           m_connected.begin();
       i != m_connected.end(); ++i) {
    if (!a.bbox(i->first).empty()) {
      for (layers_type::const_iterator j = i->second.begin();
           j != i->second.end(); ++j) {
        if (!b.bbox(*j).empty()) {
          return true;
        }
...
template <class T>
bool Connectivity::interact(const db::Cell &a, const T &ta, const db::Cell &b,
                            const T &tb) const {
  for (std::map<unsigned int, layers_type>::const_iterator i =
           m_connected.begin();
       i != m_connected.end(); ++i) {
...
template DB_PUBLIC bool
Connectivity::interacts<db::NetShape>(const db::NetShape &a, unsigned int la,
                                      const db::NetShape &b, unsigned int lb,
                                      const db::UnitTrans &trans) const;
template DB_PUBLIC bool
Connectivity::interacts<db::NetShape>(const db::NetShape &a, unsigned int la,
                                      const db::NetShape &b, unsigned int lb,
                                      const db::ICplxTrans &trans) const;
template DB_PUBLIC bool Connectivity::interacts<db::PolygonRef>(
    const db::PolygonRef &a, unsigned int la, const db::PolygonRef &b,
    unsigned int lb, const db::UnitTrans &trans) const;
template DB_PUBLIC bool Connectivity::interacts<db::PolygonRef>(
    const db::PolygonRef &a, unsigned int la, const db::PolygonRef &b,
    unsigned int lb, const db::ICplxTrans &trans) const;
template DB_PUBLIC bool
Connectivity::interacts<db::Edge>(const db::Edge &a, unsigned int la,
                                  const db::Edge &b, unsigned int lb,
                                  const db::UnitTrans &trans) const;
template DB_PUBLIC bool
Connectivity::interacts<db::Edge>(const db::Edge &a, unsigned int la,
                                  const db::Edge &b, unsigned int lb,
                                  const db::ICplxTrans &trans) const;
template DB_PUBLIC bool Connectivity::interact<db::ICplxTrans>(
    const db::Cell &a, const db::ICplxTrans &ta, const db::Cell &b,
    const db::ICplxTrans &tb) const;
...
template <class T> void local_cluster<T>::clear() {
  m_shapes.clear();
...
template <class T> bool local_cluster<T>::empty() const {
  return m_global_nets.empty() && m_shapes.empty();
}

template <class T>
void local_cluster<T>::set_global_nets(const global_nets &gn) {
  m_global_nets = gn;
}

template <class T> void local_cluster<T>::add_attr(attr_id a) {
  if (a > 0) {
    m_attrs.insert(a);
  }
}

template <class T> void local_cluster<T>::add(const T &s, unsigned int la) {
...
template <class T>
void local_cluster<T>::join_with(const local_cluster<T> &other) {
  for (typename std::map<unsigned int, tree_type>::const_iterator s =
           other.m_shapes.begin();
       s != other.m_shapes.end(); ++s) {
...
template <class T> void local_cluster<T>::ensure_sorted() {
  if (!m_needs_update) {
    return;
  }

  //  sort the shape trees
  for (typename std::map<unsigned int, tree_type>::iterator s =
           m_shapes.begin();
       s != m_shapes.end(); ++s) {
  ...
  for (typename std::map<unsigned int, tree_type>::const_iterator s =
           m_shapes.begin();
       s != m_shapes.end(); ++s) {
    for (typename tree_type::const_iterator i = s->second.begin();
         i != s->second.end(); ++i) {
      m_bbox += bc(*i);
    }
  }
...
template <class T>
void local_cluster<T>::mem_stat(MemStatistics *stat,
                                MemStatistics::purpose_t purpose, int cat,
                                bool no_self, void *parent) const {
  if (!no_self) {
    stat->add(typeid(*this), (void *)this, sizeof(*this), sizeof(*this), parent,
              purpose, cat);
  }
...
public:
  ...
  hnp_interaction_receiver(const Connectivity &conn,
...
template <class T, class Trans> struct transformed_box {
  typedef db::box_convert<T> base_bc;
  typedef typename T::box_type box_type;

  transformed_box(const Trans &trans) : m_trans(trans) {
    //  .. nothing yet ..
  }

  box_type operator()(const T &t) const { return m_bc(t).transformed(m_trans); }
...
template <class T>
typename local_cluster<T>::shape_iterator
local_cluster<T>::begin(unsigned int l) const {
  static tree_type s_empty_tree;

  typename std::map<unsigned int, tree_type>::const_iterator i =
      m_shapes.find(l);
  if (i == m_shapes.end()) {
...
template <class T>
bool local_cluster<T>::interacts(const db::Cell &cell,
                                 const db::ICplxTrans &trans,
                                 const Connectivity &conn) const {
  db::box_convert<T> bc;
  ...
  for (typename std::map<unsigned int, tree_type>::const_iterator s =
    ...
    for (Connectivity::layer_iterator l = conn.begin_connected(s->first);
         l != le; ++l) {
      box += cell.bbox(*l);
    }

    if (!box.empty() &&
        !s->second.begin_touching(box.transformed(trans), bc).at_end()) {
      return true;
    }
  }
...
template <class T>
bool local_cluster<T>::interacts(const local_cluster<T> &other,
                                 const db::ICplxTrans &trans,
                                 const Connectivity &conn) const {
  db::box_convert<T> bc;
  ...
  for (typename std::map<unsigned int, tree_type>::const_iterator s =
           m_shapes.begin();
       s != m_shapes.end(); ++s) {
    for (typename tree_type::touching_iterator i =
             s->second.begin_touching(common, bc);
         !i.at_end(); ++i) {
  ...
  for (typename std::map<unsigned int, tree_type>::const_iterator s =
       ...
       s != other.m_shapes.end(); ++s) {
    for (typename tree_type::touching_iterator i =
             s->second.begin_touching(common_for_other, bc);
         !i.at_end(); ++i) {
...
template <class T> double local_cluster<T>::area_ratio() const {
  box_type bx = bbox();
  if (bx.empty()) {
    return 0.0;
  }
  ...
  //  just the sum of the areas of the bounding boxes - this is precise if no
  //  overlaps happen and the polygons are rather rectangular. This criterion is
  //  coarse enough to prevent recursion in the split algorithm and still be
  //  fine enough - consider that we a planning to use splitted polygons for
...
template <class T> std::vector<unsigned int> local_cluster<T>::layers() const {
...
size_t split_cluster(const local_cluster<T> &cl, double max_area_ratio,
                     Iter &output) {
  ...
  //  split along the longer axis - decide the position according to the bbox
  //  center
  ...
  for (std::vector<unsigned int>::const_iterator l = layers.begin();
       l != layers.end(); ++l) {
    for (typename local_cluster<T>::shape_iterator s = cl.begin(*l);
         !s.at_end(); ++s) {
...
template DB_PUBLIC size_t local_cluster<db::NetShape>::split<
    std::back_insert_iterator<std::list<local_cluster<db::NetShape>>>>(
    double,
    std::back_insert_iterator<std::list<local_cluster<db::NetShape>>> &) const;
template DB_PUBLIC size_t local_cluster<db::PolygonRef>::split<
    std::back_insert_iterator<std::list<local_cluster<db::PolygonRef>>>>(
    double,
    std::back_insert_iterator<std::list<local_cluster<db::PolygonRef>>> &)
    const;
template DB_PUBLIC size_t local_cluster<db::Edge>::split<
    std::back_insert_iterator<std::list<local_cluster<db::Edge>>>>(
    double,
    std::back_insert_iterator<std::list<local_cluster<db::Edge>>> &) const;
...
template <class T> void local_clusters<T>::clear() {
  m_needs_update = false;
  m_clusters.clear();
  m_bbox = box_type();
  m_next_dummy_id = 0;
...
template <class T>
void local_clusters<T>::remove_cluster(typename local_cluster<T>::id_type id) {
  if (id == 0 || id > m_clusters.size()) {
    return;
  }

  //  TODO: this const_cast is required. But we know what we're doing ...
  //  NOTE: we cannot really delete a cluster as this would shift the indexes.
  //  So we just clear them.
  local_cluster<T> *to_delete =
      const_cast<local_cluster<T> *>(&m_clusters.objects().item(id - 1));
  to_delete->clear();
  m_needs_update = true;
}

template <class T>
void local_clusters<T>::join_cluster_with(
    typename local_cluster<T>::id_type id,
    typename local_cluster<T>::id_type with_id) {
  tl_assert(id > 0);
  ...
  //  TODO: this const_cast is required. But we know what we're doing ...
  local_cluster<T> *with =
      const_cast<local_cluster<T> *>(&m_clusters.objects().item(with_id - 1));
  local_cluster<T> *first =
      const_cast<local_cluster<T> *>(&m_clusters.objects().item(id - 1));
...
template <class T> local_cluster<T> *local_clusters<T>::insert() {
...
template <class T> void local_clusters<T>::ensure_sorted() {
  if (!m_needs_update) {
    return;
  }
  ...
  //  recompute bounding box
  m_bbox = box_type();
  for (typename tree_type::const_iterator i = m_clusters.begin();
       i != m_clusters.end(); ++i) {
    m_bbox += i->bbox();
  }
...
template <class T>
void local_clusters<T>::mem_stat(MemStatistics *stat,
                                 MemStatistics::purpose_t purpose, int cat,
                                 bool no_self, void *parent) const {
  if (!no_self) {
    stat->add(typeid(*this), (void *)this, sizeof(*this), sizeof(*this), parent,
...
struct cluster_building_receiver
  ...
  cluster_building_receiver(
      ...
      const tl::equivalence_clusters<size_t> *attr_equivalence,
    ...
    if (mp_attr_equivalence) {
      ...
      for (size_t gid = 0; gid < conn.global_nets(); ++gid) {
        tl::equivalence_clusters<size_t>::cluster_id_type cl =
            attr_equivalence->cluster_id(global_net_id_to_attr(gid));
        if (cl > 0) {
  ...
  void generate_clusters(local_clusters<T> &clusters) {
    ...
    for (typename std::list<cluster_value>::const_iterator c =
             m_clusters.begin();
         c != m_clusters.end(); ++c) {

      //  TODO: reserve?
      local_cluster<T> *cluster = clusters.insert();
      for (typename shape_vector::const_iterator s = c->first.begin();
           s != c->first.end(); ++s) {
      ...
      //  Add the global nets we derive from the attribute equivalence
      //  (join_nets of labelled vs. global nets)
      if (mp_attr_equivalence) {
        ...
        for (typename shape_vector::const_iterator s = c->first.begin();
          ...
          tl::equivalence_clusters<size_t>::cluster_id_type cl =
              mp_attr_equivalence->cluster_id(a);
          if (cl > 0) {
            std::map<size_t, std::set<size_t>>::const_iterator gn =
                m_global_nets_by_attribute_cluster.find(cl);
  ...
  void add(const T *s1, std::pair<unsigned int, size_t> p1, const T *s2,
           std::pair<unsigned int, size_t> p2) {
    ...
    typename std::map<
        const T *, typename std::list<cluster_value>::iterator>::iterator ic1 =
        m_shape_to_clusters.find(s1);
    typename std::map<
        const T *, typename std::list<cluster_value>::iterator>::iterator ic2 =
        m_shape_to_clusters.find(s2);
  ...
  void finish(const T *s, std::pair<unsigned int, size_t> p) {
    //  if the shape has not been handled yet, insert a single cluster with only
    //  this shape
    typename std::map<
        const T *, typename std::list<cluster_value>::iterator>::iterator ic =
        m_shape_to_clusters.find(s);
    ...
    db::Connectivity::global_nets_iterator ge =
        mp_conn->end_global_connections(p.first);
    for (db::Connectivity::global_nets_iterator g =
             mp_conn->begin_global_connections(p.first);
         g != ge; ++g) {

      typename std::map<
          size_t, typename std::list<cluster_value>::iterator>::iterator icg =
          m_global_to_clusters.find(*g);
...
private:
  ...
  std::map<const T *, typename std::list<cluster_value>::iterator>
      m_shape_to_clusters;
  std::map<size_t, typename std::list<cluster_value>::iterator>
      m_global_to_clusters;
  ...
  void join(typename std::list<cluster_value>::iterator ic1,
            typename std::list<cluster_value>::iterator ic2) {
    ic1->first.insert(ic1->first.end(), ic2->first.begin(), ic2->first.end());
    ic1->second.insert(ic2->second.begin(), ic2->second.end());

    for (typename shape_vector::const_iterator i = ic2->first.begin();
         i != ic2->first.end(); ++i) {
      m_shape_to_clusters[i->first] = ic1;
    }
    for (typename global_nets::const_iterator i = ic2->second.begin();
         i != ic2->second.end(); ++i) {
...
template <class T> struct attr_accessor {
  size_t operator()(const db::Shape &shape) const { return shape.prop_id(); }
};

template <> struct attr_accessor<db::NetShape> {
...
template <class T>
void local_clusters<T>::build_clusters(
    const db::Cell &cell, const db::Connectivity &conn,
    const tl::equivalence_clusters<size_t> *attr_equivalence,
    bool report_progress, bool separate_attributes) {
  ...
  for (db::Connectivity::layer_iterator l = conn.begin_layers();
       l != conn.end_layers(); ++l) {
    const db::Shapes &shapes = cell.shapes(*l);
    for (db::Shapes::shape_iterator s = shapes.begin(shape_flags); !s.at_end();
         ++s) {
...
template <class T>
void local_clusters<T>::apply_attr_equivalences(
    const tl::equivalence_clusters<size_t> &attr_equivalence) {
  //  identify the layout clusters joined into one attribute cluster and join
  //  them

  std::map<tl::equivalence_clusters<size_t>::cluster_id_type, std::set<size_t>>
      c2c;

  for (const_iterator c = begin(); c != end(); ++c) {

    for (typename local_cluster<T>::attr_iterator a = c->begin_attr();
         a != c->end_attr(); ++a) {
      tl::equivalence_clusters<size_t>::cluster_id_type cl =
          attr_equivalence.cluster_id(*a);
      if (cl > 0) {
        c2c[cl].insert(c->id());
      }
    }

    for (typename local_cluster<T>::global_nets_iterator g =
             c->begin_global_nets();
         g != c->end_global_nets(); ++g) {
  ...
  for (std::map<tl::equivalence_clusters<size_t>::cluster_id_type,
                std::set<size_t>>::const_iterator c = c2c.begin();
       c != c2c.end(); ++c) {
    if (c->second.size() > 1) {
...
template <class T>
connected_clusters_iterator<T>::connected_clusters_iterator(
    const connected_clusters<T> &c)
    : m_lc_iter(c.begin()) {
  size_t max_id = 0;
  for (typename connected_clusters<T>::const_iterator i = c.begin();
       i != c.end(); ++i) {
    if (i->id() > max_id) {
      max_id = i->id();
    }
...
connected_clusters<T>::connections_for_cluster(
    typename local_cluster<T>::id_type id) const {
  typename std::map<typename local_cluster<T>::id_type,
                    connections_type>::const_iterator c =
...
template <class T>
void connected_clusters<T>::add_connection(
    typename local_cluster<T>::id_type id, const ClusterInstance &inst) {
  m_connections[id].push_back(inst);
  m_rev_connections[inst] = id;
}

template <class T>
void connected_clusters<T>::join_cluster_with(
    typename local_cluster<T>::id_type id,
    typename local_cluster<T>::id_type with_id) {
  if (id == with_id) {
    return;
  ...
  typename std::map<typename local_cluster<T>::id_type,
                    connections_type>::iterator tc =
      m_connections.find(with_id);
  if (tc != m_connections.end()) {

    connections_type &to_join = tc->second;

    for (connections_type::const_iterator c = to_join.begin();
         c != to_join.end(); ++c) {
...
connected_clusters<T>::find_cluster_with_connection(
    const ClusterInstance &inst) const {
  typename std::map<ClusterInstance,
                    typename local_cluster<T>::id_type>::const_iterator rc =
      m_rev_connections.find(inst);
...
template <class T>
void connected_clusters<T>::mem_stat(MemStatistics *stat,
                                     MemStatistics::purpose_t purpose, int cat,
                                     bool no_self, void *parent) const {
  if (!no_self) {
    stat->add(typeid(*this), (void *)this, sizeof(*this), sizeof(*this), parent,
...
template <class T> class DB_PUBLIC cell_clusters_box_converter {
public:
  typedef db::simple_bbox_tag complexity;
  typedef typename hier_clusters<T>::box_type box_type;

  cell_clusters_box_converter(const db::Layout &layout,
                              const hier_clusters<T> &tree)
  ...
  const box_type &operator()(db::cell_index_type cell_index) const {
    typename std::map<db::cell_index_type, box_type>::const_iterator b =
        m_cache.find(cell_index);
    if (b != m_cache.end()) {
      ...
      const connected_clusters<T> &clusters =
          mp_tree->clusters_per_cell(cell_index);
      box_type box = clusters.bbox();

      const db::Cell &cell = mp_layout->cell(cell_index);
      for (db::Cell::const_iterator inst = cell.begin(); !inst.at_end();
           ++inst) {
...
template <class T>
const db::cell_index_type hier_clusters<T>::top_cell_index =
    std::numeric_limits<db::cell_index_type>::max();

template <class T> hier_clusters<T>::hier_clusters() : m_base_verbosity(20) {
  //  .. nothing yet ..
}

template <class T> void hier_clusters<T>::set_base_verbosity(int bv) {
  m_base_verbosity = bv;
}

template <class T> void hier_clusters<T>::clear() {
  m_per_cell_clusters.clear();
}

template <class T>
void hier_clusters<T>::mem_stat(MemStatistics *stat,
                                MemStatistics::purpose_t purpose, int cat,
                                bool no_self, void *parent) const {
  if (!no_self) {
    stat->add(typeid(*this), (void *)this, sizeof(*this), sizeof(*this), parent,
...
template <class T>
void hier_clusters<T>::build(
    const db::Layout &layout, const db::Cell &cell,
    const db::Connectivity &conn,
    const std::map<db::cell_index_type, tl::equivalence_clusters<size_t>>
        *attr_equivalence,
...
/**
 *  @brief The central interaction tester between clusters on a hierarchical
 * level
 *
 *  This receiver is both used for the instance-to-instance and the
 * local-to-instance interactions. It is employed on cell level for in two box
 * scanners: one investigating the instance-to-instance interactions and another
 * one investigating local cluster to instance interactions.
 */
template <class T>
struct hc_receiver
    : public db::box_scanner_receiver<db::Instance, unsigned int>,
      public db::box_scanner_receiver2<local_cluster<T>, unsigned int,
                                       db::Instance, unsigned int> {
public:
  typedef typename hier_clusters<T>::box_type box_type;
  typedef typename local_cluster<T>::id_type id_type;
  ...
  struct ClusterInstanceInteraction {
    ClusterInstanceInteraction(size_t _cluster_id,
                               const ClusterInstance &_other_ci)
   ...
   *  @brief Constructor
   */
  hc_receiver(const db::Layout &layout, const db::Cell &cell,
              db::connected_clusters<T> &cell_clusters, hier_clusters<T> &tree,
              const cell_clusters_box_converter<T> &cbc,
              const db::Connectivity &conn,
              const std::set<db::cell_index_type> *breakout_cells,
              typename hier_clusters<T>::instance_interaction_cache_type
   ...
   */
  size_t cluster_cache_hits() const { return m_cluster_cache_hits; }

  /**
   *  @brief Gets the cache misses
   */
  size_t cluster_cache_misses() const { return m_cluster_cache_misses; }

  /**
   *  @brief Receiver main event for instance-to-instance interactions
   */
  void add(const db::Instance *i1, unsigned int /*p1*/, const db::Instance *i2,
           unsigned int /*p2*/) {
    db::ICplxTrans t;

    std::list<std::pair<ClusterInstance, ClusterInstance>> ic;
    consider_instance_pair(box_type::world(), *i1, t,
                           db::CellInstArray::iterator(), *i2, t,
                           db::CellInstArray::iterator(), ic);

    connect_clusters(ic);
  }

  /**
   *  @brief Single-instance treatment - may be required because of interactions
   ...
   */
  void add(const local_cluster<T> *c1, unsigned int /*p1*/,
           const db::Instance *i2, unsigned int /*p2*/) {
   ...
   *
   *  This step is postponed because doing this while the iteration happens
   * would invalidate the box trees.
   */
  void finish_cluster_to_instance_interactions() {
    for (typename std::list<ClusterInstanceInteraction>::const_iterator ii =
             m_ci_interactions.begin();
         ii != m_ci_interactions.end(); ++ii) {
      mp_tree->propagate_cluster_inst(*mp_layout, *mp_cell, ii->other_ci,
                                      mp_cell->cell_index(), false);
    }
    ...
    for (typename std::list<ClusterInstanceInteraction>::const_iterator ii =
      ...
      id_type other =
          mp_cell_clusters->find_cluster_with_connection(ii->other_ci);
      if (other > 0) {

        //  we found a child cluster that connects two clusters on our own
        //  level: we must join them into one, but not now. We're still
        //  iterating and would invalidate the box trees. So keep this now and
        //  combine the clusters later.
    ...
    for (typename std::list<std::set<id_type>>::const_iterator sc =
             m_cm2join_sets.begin();
         sc != m_cm2join_sets.end(); ++sc) {
...
private:
  ...
  instance_interaction_cache<interaction_key_for_clusters<box_type>,
                             std::list<std::pair<size_t, size_t>>>
      m_interaction_cache_for_clusters;
  size_t m_cluster_cache_misses, m_cluster_cache_hits;
  typename hier_clusters<T>::instance_interaction_cache_type
      *mp_instance_interaction_cache;
  bool m_separate_attributes;
   ...
   *  @param i1element selects a specific instance from i1 (unless ==
   * db::CellInstArray::iterator())
   *  @param i2 The second instance to investigate
   *  @param t2 The parent instances' cumulated transformation
   *  @param i2element selects a specific instance from i2 (unless ==
   * db::CellInstArray::iterator())
   *  @param interacting_clusters_out Receives the cluster interaction
   * descriptors
   *
   *  "interacting_clusters_out" will be cluster interactions in the parent
   * instance space of i1 and i2 respectively. Cluster ID will be valid in the
   * parent cells containing i1 and i2.
   */
  void consider_instance_pair(
      const box_type &common, const db::Instance &i1, const db::ICplxTrans &t1,
      const db::CellInstArray::iterator &i1element, const db::Instance &i2,
      const db::ICplxTrans &t2, const db::CellInstArray::iterator &i2element,
      cluster_instance_pair_list_type &interacting_clusters) {
    if (is_breakout_cell(mp_breakout_cells, i1.cell_index()) ||
        is_breakout_cell(mp_breakout_cells, i2.cell_index())) {
      return;
    }
    ...
    //  gross shortcut: the cells do no comprise a constellation which will ever
    //  interact
    if (!mp_conn->interact(mp_layout->cell(i1.cell_index()),
                           mp_layout->cell(i2.cell_index()))) {
      return;
    }
    ...
    if ((n1 == 1 || !i1.is_iterated_array()) &&
      ...
      i1t = i1element.at_end() ? i1.complex_trans()
                               : i1.complex_trans(*i1element);
      db::ICplxTrans tt1 = t1 * i1t;

      i2t = i2element.at_end() ? i2.complex_trans()
                               : i2.complex_trans(*i2element);
      db::ICplxTrans tt2 = t2 * i2t;

      db::ICplxTrans cache_norm = tt1.inverted();

      ii_key = InstanceToInstanceInteraction(
          (!i1element.at_end() || i1.size() == 1) ? 0
                                                  : i1.cell_inst().delegate(),
          (!i2element.at_end() || i2.size() == 1) ? 0
                                                  : i2.cell_inst().delegate(),
          cache_norm, cache_norm * tt2);

      const cluster_instance_pair_list_type *cached =
          mp_instance_interaction_cache->find(i1.cell_index(), i2.cell_index(),
                                              ii_key);
      if (cached) {

        //  use cached interactions
        interacting_clusters = *cached;
        for (cluster_instance_pair_list_type::iterator i =
                 interacting_clusters.begin();
             i != interacting_clusters.end(); ++i) {
      ...
      //  avoid caching few-to-many interactions as this typically does not
      //  contribute anything
      fill_cache = true;
    }

    //  shortcut: if the instances cannot interact because their bounding boxes
    //  do no comprise a valid constellation, reject the pair

    bool any = false;
    for (db::Connectivity::layer_iterator la = mp_conn->begin_layers();
         la != mp_conn->end_layers() && !any; ++la) {
      ...
      if (!bb1.empty()) {
        ...
        for (db::Connectivity::layer_iterator lb =
                 mp_conn->begin_connected(*la);
             lb != lbe && !any; ++lb) {
    ...
    if (!any) {
      if (fill_cache) {
        mp_instance_interaction_cache->insert(i1.cell_index(), i2.cell_index(),
                                              ii_key);
      }
      return;
    }
    ...
    for (db::CellInstArray::iterator ii1 =
             i1element.at_end()
                 ? i1.begin_touching(common_all.transformed(t1i), mp_layout)
                 : i1element;
         !ii1.at_end(); ++ii1) {
      ...
      for (db::CellInstArray::iterator ii2 =
               i2element.at_end()
                   ? i2.begin_touching(ib1.transformed(t2i), mp_layout)
                   : i2element;
           !ii2.at_end(); ++ii2) {
        ...
        if (i1.cell_index() == i2.cell_index() && tt1 == tt2) {
          //  skip interactions between identical instances (duplicate instance
          //  removal)
          if (!i2element.at_end()) {
            break;
          } else {
        ...
        if (!common12.empty()) {
          ...
          const std::list<std::pair<size_t, size_t>> &i2i_interactions =
              compute_instance_interactions(common12, i1.cell_index(), tt1,
                                            i2.cell_index(), tt2);

          for (std::list<std::pair<size_t, size_t>>::const_iterator ii =
                   i2i_interactions.begin();
               ii != i2i_interactions.end(); ++ii) {
          ...
          for (db::Cell::touching_iterator jj2 =
                   cell2.begin_touching(common12.transformed(tt2.inverted()));
               !jj2.at_end(); ++jj2) {

            std::list<std::pair<ClusterInstance, ClusterInstance>>
                ii_interactions;
            consider_instance_pair(common12, i1, t1, ii1, *jj2, tt2,
                                   db::CellInstArray::iterator(),
                                   ii_interactions);

            for (std::list<
                     std::pair<ClusterInstance, ClusterInstance>>::iterator i =
                     ii_interactions.begin();
                 i != ii_interactions.end(); ++i) {
              propagate_cluster_inst(i->second, i2.cell_index(), i2t,
                                     i2.prop_id());
            }

            ii_interactions.unique();
            interacting_clusters.splice(
                interacting_clusters.end(), ii_interactions,
                ii_interactions.begin(), ii_interactions.end());
          }

          //  dive into cell of ii1
          const db::Cell &cell1 = mp_layout->cell(i1.cell_index());
          for (db::Cell::touching_iterator jj1 =
                   cell1.begin_touching(common12.transformed(tt1.inverted()));
               !jj1.at_end(); ++jj1) {

            std::list<std::pair<ClusterInstance, ClusterInstance>>
                ii_interactions;
            consider_instance_pair(common12, *jj1, tt1,
                                   db::CellInstArray::iterator(), i2, t2, ii2,
                                   ii_interactions);
            ...
            for (std::list<
                     ...
                     ii_interactions.begin();
                 i != ii_interactions.end(); ++i) {
              propagate_cluster_inst(i->first, i1.cell_index(), i1t,
                                     i1.prop_id());
            }
    ...
    //  NOTE: duplicates may happen due to manifold child/child interactions
    //  which boil down to identical parent cluster interactions.
    std::vector<std::pair<ClusterInstance, ClusterInstance>>
        sorted_interactions;
    sorted_interactions.reserve(interacting_clusters.size());
    sorted_interactions.insert(sorted_interactions.end(),
                               interacting_clusters.begin(),
                               interacting_clusters.end());
    interacting_clusters.clear();
    std::sort(sorted_interactions.begin(), sorted_interactions.end());
    sorted_interactions.erase(
        std::unique(sorted_interactions.begin(), sorted_interactions.end()),
        sorted_interactions.end());

    //  return the list of unique interactions
    interacting_clusters.insert(interacting_clusters.end(),
                                sorted_interactions.begin(),
                                sorted_interactions.end());

    if (fill_cache && sorted_interactions.size() <
                          instance_to_instance_cache_set_size_threshold) {

      //  normalize transformations for cache
      db::ICplxTrans i1ti = i1t.inverted(), i2ti = i2t.inverted();
      for (std::vector<std::pair<ClusterInstance, ClusterInstance>>::iterator
               i = sorted_interactions.begin();
           i != sorted_interactions.end(); ++i) {
        i->first.transform(i1ti);
        i->second.transform(i2ti);
      }

      cluster_instance_pair_list_type &cached =
          mp_instance_interaction_cache->insert(i1.cell_index(),
                                                i2.cell_index(), ii_key);
      cached.insert(cached.end(), sorted_interactions.begin(),
                    sorted_interactions.end());
    }
  }
   ...
   */
  const std::list<std::pair<size_t, size_t>> &compute_instance_interactions(
      const box_type &common, db::cell_index_type ci1, const db::ICplxTrans &t1,
      db::cell_index_type ci2, const db::ICplxTrans &t2) {
    if (is_breakout_cell(mp_breakout_cells, ci1) ||
        is_breakout_cell(mp_breakout_cells, ci2)) {
    ...
    const std::list<std::pair<size_t, size_t>> *ici =
        m_interaction_cache_for_clusters.find(ci1, ci2, ikey);
    if (ici) {
      ...
      std::list<std::pair<size_t, size_t>> &new_interactions =
          m_interaction_cache_for_clusters.insert(ci1, ci2, ikey);
      db::ICplxTrans t12 = t2i * t1;

      for (typename db::local_clusters<T>::touching_iterator i =
               cl1.begin_touching(common2.transformed(t21));
           !i.at_end(); ++i) {
        ...
        for (typename db::local_clusters<T>::touching_iterator j =
                 cl2.begin_touching(bc2);
             !j.at_end(); ++j) {

          if ((!m_separate_attributes || i->same_attrs(*j)) &&
              i->interacts(*j, t21, *mp_conn)) {
  ...
  void consider_single_inst(const db::Instance &i) {
    ...
    for (db::CellInstArray::iterator ii = i.begin(); !ii.at_end(); ++ii) {
      ...
      pp.push_back(ClusterInstElement(i.cell_index(), i.complex_trans(*ii),
                                      i.prop_id()));

      bool any = false;

      for (db::CellInstArray::iterator ii2 = i.begin_touching(ib, mp_layout);
           !ii2.at_end(); ++ii2) {
        ...
        if (ib.touches(ib2)) {
          ...
          pp2.push_back(ClusterInstElement(i.cell_index(),
                                           i.complex_trans(*ii2), i.prop_id()));

          cluster_instance_pair_list_type interacting_clusters;

          box_type common = (ib & ib2);
          const std::list<std::pair<size_t, size_t>> &i2i_interactions =
              compute_instance_interactions(common, i.cell_index(), tt,
                                            i.cell_index(), tt2);
          for (std::list<std::pair<size_t, size_t>>::const_iterator ii =
                   i2i_interactions.begin();
               ii != i2i_interactions.end(); ++ii) {
          ...
          for (db::Cell::touching_iterator jj2 =
                   cell.begin_touching(common.transformed(tt2.inverted()));
               !jj2.at_end(); ++jj2) {
            ...
            std::list<std::pair<ClusterInstance, ClusterInstance>>
                ii_interactions;
            consider_instance_pair(common, i, t, ii, *jj2, tt2,
                                   db::CellInstArray::iterator(),
                                   ii_interactions);

            for (std::list<
                     std::pair<ClusterInstance, ClusterInstance>>::iterator ii =
                     ii_interactions.begin();
                 ii != ii_interactions.end(); ++ii) {
              propagate_cluster_inst(ii->second, i.cell_index(), tt2,
                                     i.prop_id());
            }
  ...
  /**
   *  @brief Handles a local clusters vs. the clusters of a specific child
   ...
   */
  void consider_cluster_instance_pair(
      const local_cluster<T> &c1, const db::Instance &i2,
      const db::ICplxTrans &t2,
      std::list<ClusterInstanceInteraction> &interactions_out) {
    ...
    for (db::CellInstArray::iterator ii2 =
             i2.begin_touching((b1 & b2).transformed(t2.inverted()), mp_layout);
         !ii2.at_end(); ++ii2) {
      ...
      if (b1.touches(ib2) && c1.interacts(cell2, tt2, *mp_conn)) {
        ...
        compute_cluster_instance_interactions(c1, i2.cell_index(), tt2,
                                              c2i_interactions);

        for (std::vector<std::pair<size_t, size_t>>::const_iterator ii =
                 c2i_interactions.begin();
             ii != c2i_interactions.end(); ++ii) {
        ...
        //  dive into cell of ii2
        for (db::Cell::touching_iterator jj2 =
                 cell2.begin_touching((b1 & ib2).transformed(tt2.inverted()));
             !jj2.at_end(); ++jj2) {

          std::list<ClusterInstanceInteraction> ci_interactions;
          consider_cluster_instance_pair(c1, *jj2, tt2, ci_interactions);

          for (typename std::list<ClusterInstanceInteraction>::iterator ii =
                   ci_interactions.begin();
               ii != ci_interactions.end(); ++ii) {
            propagate_cluster_inst(ii->other_ci, i2.cell_index(), i2t,
                                   i2.prop_id());
          }

          interactions_out.splice(interactions_out.end(), ci_interactions,
                                  ci_interactions.begin(),
                                  ci_interactions.end());
        }
      }
   ...
   *  @param p2 The instantiation path to the child cell (last element is the
   * instance to ci2)
   *  @param t2 The accumulated transformation of the path
   */
  void compute_cluster_instance_interactions(
      const local_cluster<T> &c1, db::cell_index_type ci2,
      const db::ICplxTrans &t2,
    ...
    for (typename db::local_clusters<T>::touching_iterator j =
             cl2.begin_touching(c1.bbox().transformed(t2.inverted()));
         !j.at_end(); ++j) {

      if ((!m_separate_attributes || c1.same_attrs(*j)) &&
          c1.interacts(*j, t2, *mp_conn)) {
  ...
  void mark_to_join(id_type a, id_type b) {
    ...
    typename std::map<id_type, typename join_set_list::iterator>::const_iterator
        x = m_cm2join_map.find(a);
    typename std::map<id_type, typename join_set_list::iterator>::const_iterator
        y = m_cm2join_map.find(b);
    ...
    if (x == m_cm2join_map.end()) {
      ...
      for (typename std::set<id_type>::const_iterator i = yset->begin();
           i != yset->end(); ++i) {
    ...
    for (typename std::map<id_type,
                           typename join_set_list::iterator>::const_iterator j =
             m_cm2join_map.begin();
         j != m_cm2join_map.end(); ++j) {
      tl_assert(j->second->find(j->first) != j->second->end());
    }

    for (typename std::list<std::set<id_type>>::const_iterator i =
             m_cm2join_sets.begin();
         i != m_cm2join_sets.end(); ++i) {
      for (typename std::set<id_type>::const_iterator j = i->begin();
           j != i->end(); ++j) {
  ...
  /**
   *  @brief Makes cluster connections from all parents of a cell into this cell
   * and to the given cluster
   *
   *  After calling this method, the cluster instance in ci is guaranteed to
   * have connections from all parent cells. One of these connections represents
   * the instance ci.
   *
   *  Returns false if the connection was already there in the same place
   * (indicating duplicate instances). In this case, the cluster instance should
   * be skipped. In the other case, the cluster instance is updated to reflect
   * the connected cluster.
   */
  void propagate_cluster_inst(ClusterInstance &ci, db::cell_index_type pci,
                              const db::ICplxTrans &trans,
                              db::properties_id_type prop_id) const {
    size_t id_new =
        mp_tree->propagate_cluster_inst(*mp_layout, *mp_cell, ci, pci, true);
    tl_assert(id_new != 0);
    ci = db::ClusterInstance(id_new, pci, trans, prop_id);
  }

  /**
   *  @brief Establishes connections between the cluster instances listed in the
   * argument
   */
  void connect_clusters(
      const cluster_instance_pair_list_type &interacting_clusters) {
    for (cluster_instance_pair_list_type::const_iterator ic =
             interacting_clusters.begin();
         ic != interacting_clusters.end(); ++ic) {

      const ClusterInstance &k1 = ic->first;
      const ClusterInstance &k2 = ic->second;

      //  Note: "with_self" is false as we're going to create a connected
      //  cluster anyway
      id_type x1 = mp_tree->propagate_cluster_inst(
          *mp_layout, *mp_cell, k1, mp_cell->cell_index(), false);
      id_type x2 = mp_tree->propagate_cluster_inst(
          *mp_layout, *mp_cell, k2, mp_cell->cell_index(), false);
      ...
      if (x1 == 0) {
        ...
        //  for instance-to-instance interactions the number of connections is
        //  more important for the cost of the join operation: make the one with
        //  more connections the target
        //  TODO: this will be SLOW for STL's not providing a fast size()
        if (mp_cell_clusters->connections_for_cluster(x1).size() <
            mp_cell_clusters->connections_for_cluster(x2).size()) {
          std::swap(x1, x2);
        }
...
template <class T> struct cell_inst_clusters_box_converter {
  typedef typename local_cluster<T>::box_type box_type;
  typedef db::simple_bbox_tag complexity;
...
template <class T>
size_t hier_clusters<T>::propagate_cluster_inst(const db::Layout &layout,
                                                const db::Cell &cell,
                                                const ClusterInstance &ci,
                                                db::cell_index_type pci,
                                                bool with_self) {
  ...
  if (parent_cluster > 0) {
    ...
    if (child_cc.is_root(id)) {
      ...
      std::set<std::pair<db::cell_index_type, ClusterInstance>>
          seen; //  to avoid duplicate connections

      const db::Cell &child_cell = layout.cell(ci.inst_cell_index());
      for (db::Cell::parent_inst_iterator pi = child_cell.begin_parent_insts();
           !pi.at_end(); ++pi) {

        db::Instance child_inst = pi->child_inst();

        connected_clusters<T> &parent_cc =
            clusters_per_cell(pi->parent_cell_index());
        for (db::CellInstArray::iterator pii = child_inst.begin();
             !pii.at_end(); ++pii) {

          ClusterInstance ci2(id, child_inst.cell_index(),
                              child_inst.complex_trans(*pii),
                              child_inst.prop_id());
          if ((with_self || cell.cell_index() != pi->parent_cell_index() ||
               ci != ci2) &&
              seen.find(std::make_pair(pi->parent_cell_index(), ci2)) ==
                  seen.end()) {

            size_t id_dummy;

            const typename db::local_cluster<T>::global_nets &gn =
                child_cc.cluster_by_id(id).get_global_nets();
            if (gn.empty()) {
...
template <class T>
void hier_clusters<T>::do_build(
    cell_clusters_box_converter<T> &cbc, const db::Layout &layout,
    const db::Cell &cell, const db::Connectivity &conn,
    const std::map<db::cell_index_type, tl::equivalence_clusters<size_t>>
        *attr_equivalence,
    const std::set<db::cell_index_type> *breakout_cells,
  ...
  tl::SelfTimer timer(tl::verbosity() > m_base_verbosity,
                      tl::to_string(tr("Computing shape clusters")));
...

  {
    tl::SelfTimer timer(tl::verbosity() > m_base_verbosity + 10,
                        tl::to_string(tr("Computing local shape clusters")));
    tl::RelativeProgress progress(tl::to_string(tr("Computing local clusters")),
                                  called.size(), 1);

    for (std::set<db::cell_index_type>::const_iterator c = called.begin();
         c != called.end(); ++c) {

      //  look for the net label joining spec - for the top cell the
      //  "top_cell_index" entry is looked for. If there is no such entry or the
      //  cell is not the top cell, look for the entry by cell index.
      std::map<db::cell_index_type,
               tl::equivalence_clusters<size_t>>::const_iterator ae;
  ...
  //  build the hierarchical connections bottom-up and for all cells whose
...

  {
    tl::SelfTimer timer(
        ...
        tl::to_string(tr("Computing hierarchical shape clusters")));
    tl::RelativeProgress progress(
        tl::to_string(tr("Computing hierarchical clusters")), called.size(), 1);

    std::set<db::cell_index_type> done;
    std::vector<db::cell_index_type> todo;
    for (db::Layout::bottom_up_const_iterator c = layout.begin_bottom_up();
         c != layout.end_bottom_up(); ++c) {

      if (called.find(*c) != called.end()) {

        bool all_available = true;
        const db::Cell &cell = layout.cell(*c);
        for (db::Cell::child_cell_iterator cc = cell.begin_child_cells();
             !cc.at_end() && all_available; ++cc) {
        ...
        if (all_available) {
          ...
          build_hier_connections_for_cells(
              cbc, layout, todo, conn, breakout_cells, progress,
    ...
    build_hier_connections_for_cells(cbc, layout, todo, conn, breakout_cells,
                                     progress, instance_interaction_cache,
                                     separate_attributes);
  }

  if (tl::verbosity() >= m_base_verbosity + 20) {
    tl::info
        << "Cluster build cache statistics (instance to instance cache): size="
        << instance_interaction_cache.size()
        << ", hits=" << instance_interaction_cache.hits()
        << ", misses=" << instance_interaction_cache.misses();
  }
}

template <class T>
void hier_clusters<T>::build_local_cluster(
    const db::Layout &layout, const db::Cell &cell,
    const db::Connectivity &conn,
    const tl::equivalence_clusters<size_t> *attr_equivalence,
    bool separate_attributes) {
  std::string msg = tl::to_string(tr("Computing local clusters for cell: ")) +
                    std::string(layout.cell_name(cell.cell_index()));
  if (tl::verbosity() >= m_base_verbosity + 20) {
    tl::log << msg;
  }
...
template <class T>
void hier_clusters<T>::build_hier_connections_for_cells(
    cell_clusters_box_converter<T> &cbc, const db::Layout &layout,
    const std::vector<db::cell_index_type> &cells, const db::Connectivity &conn,
    const std::set<db::cell_index_type> *breakout_cells,
    tl::RelativeProgress &progress,
    instance_interaction_cache_type &instance_interaction_cache,
    bool separate_attributes) {
  for (std::vector<db::cell_index_type>::const_iterator c = cells.begin();
       c != cells.end(); ++c) {
...
public:
  ...
  void add(const std::set<size_t> &global_nets, size_t cluster_id) {
    add(global_nets, ClusterInstance(cluster_id));
  }

  void add(const std::set<size_t> &global_nets, const ClusterInstance &inst) {
    ...
    std::map<size_t, entry_list::iterator>::iterator k =
        m_global_net_to_entries.find(*g0);
    if (k == m_global_net_to_entries.end()) {

      m_entries.push_back(entry_type());
      m_entries.back().first.insert(*g0);

      k = m_global_net_to_entries.insert(std::make_pair(*g0, --m_entries.end()))
              .first;
    }

    k->second->second.insert(inst);

    for (std::set<size_t>::const_iterator g = ++g0; g != global_nets.end();
         ++g) {

      std::map<size_t, entry_list::iterator>::iterator j =
          m_global_net_to_entries.find(*g);
      if (j == m_global_net_to_entries.end()) {
        ...
        //  joining required
        k->second->first.insert(j->second->first.begin(),
                                j->second->first.end());
        k->second->second.insert(j->second->second.begin(),
                                 j->second->second.end());
  ...
  entry_iterator begin() const { return m_entries.begin(); }

  entry_iterator end() const { return m_entries.end(); }
...
template <class T>
void hier_clusters<T>::build_hier_connections(
    cell_clusters_box_converter<T> &cbc, const db::Layout &layout,
    const db::Cell &cell, const db::Connectivity &conn,
    const std::set<db::cell_index_type> *breakout_cells,
    instance_interaction_cache_type &instance_interaction_cache,
    bool separate_attributes) {
  std::string msg =
      tl::to_string(tr("Computing hierarchical clusters for cell: ")) +
      std::string(layout.cell_name(cell.cell_index()));
  if (tl::verbosity() >= m_base_verbosity + 20) {
    tl::log << msg;
  }
  ...
  std::unique_ptr<hc_receiver<T>> rec(
      new hc_receiver<T>(layout, cell, local, *this, cbc, conn, breakout_cells,
                         &instance_interaction_cache, separate_attributes));
...

  {
    static std::string desc =
        tl::to_string(tr("Instance to instance treatment"));
    tl::SelfTimer timer(tl::verbosity() > m_base_verbosity + 30, desc);

    db::box_scanner<db::Instance, unsigned int> bs(true, desc);

    for (std::vector<db::Instance>::const_iterator inst = inst_storage.begin();
         inst != inst_storage.end(); ++inst) {
...

  {
    ...
    db::box_scanner2<db::local_cluster<T>, unsigned int, db::Instance,
                     unsigned int>
        bs2(true, desc);

    for (typename connected_clusters<T>::const_iterator c = local.begin();
         c != local.end(); ++c) {

      //  we do not actually need the original clusters. For a better
      //  performance we optimize the area ratio and split, but we keep the ID
      //  the same.
      std::back_insert_iterator<std::list<local_cluster<T>>> iout =
  ...
  if (tl::verbosity() >= m_base_verbosity + 20) {
    tl::info
        << "Cluster build cache statistics (instance to shape cache): size="
        << rec->cluster_cache_size() << ", hits=" << rec->cluster_cache_hits()
        << ", misses=" << rec->cluster_cache_misses();
  }
  ...
  //  finally connect global nets
  {
    ...
    for (std::vector<db::Instance>::const_iterator inst = inst_storage.begin();
      ...
      const db::connected_clusters<T> &cc =
          m_per_cell_clusters[inst->cell_index()];
      for (typename db::connected_clusters<T>::const_iterator cl = cc.begin();
           cl != cc.end(); ++cl) {

        if (!cl->get_global_nets().empty()) {
          for (db::Instance::cell_inst_array_type::iterator i = inst->begin();
               !i.at_end(); ++i) {
            global_net_clusters.add(
                cl->get_global_nets(),
                db::ClusterInstance(cl->id(), inst->cell_index(),
                                    inst->complex_trans(*i), inst->prop_id()));
          }
        }
      }
    }
    ...
    for (typename db::connected_clusters<T>::const_iterator cl = local.begin();
         cl != local.end(); ++cl) {
      const typename db::local_cluster<T>::global_nets &gn =
          cl->get_global_nets();
      if (!gn.empty()) {
    ...
    //  now global_net_clusters knows what clusters need to be made for the
    //  global nets

    for (GlobalNetClusterMaker::entry_iterator ge = global_net_clusters.begin();
         ge != global_net_clusters.end(); ++ge) {

      db::local_cluster<T> *gc = local.insert();
      gc->set_global_nets(ge->first);
      //  NOTE: don't use the gc pointer - it may become invalid during
      //  make_path (will also do a local.insert)
      size_t gcid = gc->id();

      for (std::set<ClusterInstance>::const_iterator i = ge->second.begin();
           i != ge->second.end(); ++i) {
        ...
        if (!i->has_instance()) {
          ...
          //  ensures the cluster is propagated so we can connect it with
          //  another
          size_t other_id = propagate_cluster_inst(layout, cell, *i,
                                                   cell.cell_index(), false);
...
hier_clusters<T>::clusters_per_cell(db::cell_index_type cell_index) const {
  typename std::map<db::cell_index_type, connected_clusters<T>>::const_iterator
      c = m_per_cell_clusters.find(cell_index);
...
hier_clusters<T>::clusters_per_cell(db::cell_index_type cell_index) {
  ...
  if (c == m_per_cell_clusters.end()) {
    c = m_per_cell_clusters
            .insert(std::make_pair(cell_index, connected_clusters<T>()))
            .first;
  }
  return c->second;
}
...
template <class T>
void hier_clusters<T>::return_to_hierarchy(
    db::Layout &layout, const std::map<unsigned int, unsigned int> &lm) const {
  for (db::Layout::bottom_up_iterator c = layout.begin_bottom_up();
       c != layout.end_bottom_up(); ++c) {

    const db::connected_clusters<T> &cc = clusters_per_cell(*c);
    db::Cell &target_cell = layout.cell(*c);

    for (typename db::connected_clusters<T>::all_iterator lc = cc.begin_all();
         !lc.at_end(); ++lc) {

      if (cc.is_root(*lc)) {

        for (typename std::map<unsigned int, unsigned int>::const_iterator m =
                 lm.begin();
             m != lm.end(); ++m) {

          db::Shapes &shapes = target_cell.shapes(m->second);

          for (recursive_cluster_shape_iterator<T> si(*this, m->first, *c, *lc);
               !si.at_end(); ++si) {
...
template <class T>
recursive_cluster_shape_iterator<T>::recursive_cluster_shape_iterator(
    const hier_clusters<T> &hc, unsigned int layer, db::cell_index_type ci,
    typename local_cluster<T>::id_type id, const CircuitCallback *callback)
...
recursive_cluster_shape_iterator<T>::inst_path() const {
...
recursive_cluster_shape_iterator<T>::operator++() {
  ++m_shape_iter;
...
template <class T> void recursive_cluster_shape_iterator<T>::skip_cell() {
...
template <class T> void recursive_cluster_shape_iterator<T>::next_conn() {
...
template <class T> void recursive_cluster_shape_iterator<T>::up() {
...
template <class T>
void recursive_cluster_shape_iterator<T>::down(
    db::cell_index_type ci, typename db::local_cluster<T>::id_type id,
    const db::ICplxTrans &t) {
  const connected_clusters<T> &clusters = mp_hc->clusters_per_cell(ci);
  const typename connected_clusters<T>::connections_type &conn =
      clusters.connections_for_cluster(id);
  ...
  const local_cluster<T> &cluster =
      mp_hc->clusters_per_cell(cell_index()).cluster_by_id(cluster_id());
...
template <class T>
recursive_cluster_iterator<T>::recursive_cluster_iterator(
    const hier_clusters<T> &hc, db::cell_index_type ci,
    typename local_cluster<T>::id_type id)
    : mp_hc(&hc), m_id(id) {
  down(ci, id);
}
...
template <class T> void recursive_cluster_iterator<T>::next_conn() {
  while (m_conn_iter_stack.back().first == m_conn_iter_stack.back().second) {
...
template <class T> void recursive_cluster_iterator<T>::up() {
  m_conn_iter_stack.pop_back();
  m_cell_index_stack.pop_back();
}

template <class T>
void recursive_cluster_iterator<T>::down(
    db::cell_index_type ci, typename db::local_cluster<T>::id_type id) {
  const connected_clusters<T> &clusters = mp_hc->clusters_per_cell(ci);
  const typename connected_clusters<T>::connections_type &conn =
      clusters.connections_for_cluster(id);
...
template <class T>
incoming_cluster_connections<T>::incoming_cluster_connections(
    const db::Layout &layout, const db::Cell &cell, const hier_clusters<T> &hc)
    : mp_layout(const_cast<db::Layout *>(&layout)),
      mp_hc(const_cast<hier_clusters<T> *>(&hc)) {
  cell.collect_called_cells(m_called_cells);
  m_called_cells.insert(cell.cell_index());
}

template <class T>
bool incoming_cluster_connections<T>::has_incoming(db::cell_index_type ci,
                                                   size_t cluster_id) const {
  std::map<db::cell_index_type,
           std::map<size_t, incoming_connections>>::const_iterator i =
      m_incoming.find(ci);
  if (i == m_incoming.end()) {
    ensure_computed(ci);
...
incoming_cluster_connections<T>::incoming(db::cell_index_type ci,
                                          size_t cluster_id) const {
  std::map<db::cell_index_type,
           std::map<size_t, incoming_connections>>::const_iterator i =
      m_incoming.find(ci);
  ...
  std::map<size_t, incoming_connections>::const_iterator ii =
      i->second.find(cluster_id);
  if (ii != i->second.end()) {
    return ii->second;
  } else {
...
template <class T>
void incoming_cluster_connections<T>::ensure_computed(
    db::cell_index_type ci) const {
  tl_assert(mp_layout.get() != 0);
  m_incoming.insert(
      std::make_pair(ci, std::map<size_t, incoming_connections>()));

  const db::Cell &cell = mp_layout->cell(ci);
  for (db::Cell::parent_cell_iterator pc = cell.begin_parent_cells();
       pc != cell.end_parent_cells(); ++pc) {
...
template <class T>
void incoming_cluster_connections<T>::ensure_computed_parent(
    db::cell_index_type ci) const {
  ensure_computed(ci);

  const connected_clusters<T> &cc =
      ((const hier_clusters<T> *)mp_hc.get())->clusters_per_cell(ci);
  for (typename connected_clusters<T>::connections_iterator x =
           cc.begin_connections();
       x != cc.end_connections(); ++x) {
    for (typename connected_clusters<T>::connections_type::const_iterator xx =
             x->second.begin();
         xx != x->second.end(); ++xx) {
      m_incoming[xx->inst_cell_index()][xx->id()].push_back(
          IncomingClusterInstance(ci, x->first, *xx));
    }
  }
}
