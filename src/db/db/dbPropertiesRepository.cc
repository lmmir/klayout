...
PropertiesRepository::get_id_of_name(const tl::Variant &name) const {
  std::map<tl::Variant, property_names_id_type>::const_iterator pi =
      m_propname_ids_by_name.find(name);
...
void PropertiesRepository::change_properties(property_names_id_type id,
                                             const properties_set &new_props) {
  const properties_set &old_props = properties(id);

  std::map<properties_set, properties_id_type>::const_iterator pi =
      m_properties_ids_by_set.find(old_props);
  if (pi != m_properties_ids_by_set.end()) {

    //  erase the id from the component table
    for (properties_set::const_iterator nv = old_props.begin();
         nv != old_props.end(); ++nv) {
      if (m_properties_component_table.find(*nv) !=
          m_properties_component_table.end()) {
    ...
    for (properties_set::const_iterator nv = new_props.begin();
         nv != new_props.end(); ++nv) {
      m_properties_component_table
          .insert(std::make_pair(*nv, properties_id_vector()))
          .first->second.push_back(id);
    }

    //  signal the change of the properties ID's. This way for example, the
    //  layer views can recompute the property selectors
    if (mp_state_model) {
...
void PropertiesRepository::change_name(property_names_id_type id,
                                       const tl::Variant &new_name) {
  std::map<property_names_id_type, tl::Variant>::iterator pi =
      m_propnames_by_id.find(id);
...
PropertiesRepository::properties_id(const properties_set &props) {
  std::map<properties_set, properties_id_type>::const_iterator pi =
      m_properties_ids_by_set.find(props);
  if (pi == m_properties_ids_by_set.end()) {
    ...
    for (properties_set::const_iterator nv = props.begin(); nv != props.end();
         ++nv) {
      m_properties_component_table
          .insert(std::make_pair(*nv, properties_id_vector()))
          .first->second.push_back(id);
...
PropertiesRepository::properties_ids_by_name_value(
    const name_value_pair &nv) const {
  std::map<name_value_pair, properties_id_vector>::const_iterator idv =
      m_properties_component_table.find(nv);
...
properties_id_type
PropertiesRepository::translate(const PropertiesRepository &rep,
  ...
  for (properties_set::const_iterator pp = pset.begin(); pp != pset.end();
       ++pp) {
    new_pset.insert(
        std::make_pair(prop_name_id(rep.prop_name(pp->first)), pp->second));
  }
...
PropertiesTranslator::PropertiesTranslator() : m_pass(true), m_null(true) {
  //  .. nothing yet ..
}
...
PropertiesTranslator::operator*(const PropertiesTranslator &other) const {
  if (other.m_pass) {
    ...
    //  NOTE: by handling this first, "pass_all * null" will give "pass_all"
    //  which is desired for RecursiveShapeIterator::apply_property_translator.
    return *this;
...
PropertiesTranslator PropertiesTranslator::make_remove_all() {
  return PropertiesTranslator(false);
}

PropertiesTranslator PropertiesTranslator::make_pass_all() {
  return PropertiesTranslator(true);
}

PropertiesTranslator
PropertiesTranslator::make_filter(db::PropertiesRepository &repo,
...
PropertiesTranslator PropertiesTranslator::make_key_mapper(
    db::PropertiesRepository &repo,
    const std::map<tl::Variant, tl::Variant> &keys) {
  std::map<db::properties_id_type, db::properties_id_type> map;
  std::map<db::property_names_id_type, db::property_names_id_type> name_map;

  for (auto k = keys.begin(); k != keys.end(); ++k) {
    name_map.insert(std::make_pair(repo.prop_name_id(k->first),
                                   repo.prop_name_id(k->second)));
  }
