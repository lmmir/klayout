 ...
 */
db::Coord DB_PUBLIC div_exact(db::Coord a,
                              db::coord_traits<db::Coord>::area_type b,
                              db::coord_traits<db::Coord>::area_type d);

/**
 *  @brief An overload of div_exact for double types
 */
inline db::DCoord div_exact(db::DCoord a,
                            db::coord_traits<db::DCoord>::area_type b,
                            db::coord_traits<db::DCoord>::area_type d) {
  return db::coord_traits<db::DCoord>::rounded(double(a) * double(b) /
                                               double(d));
}
...
template <class C> class DB_PUBLIC_TEMPLATE edge {
public:
   ...
   */
  template <class D> edge(D x1, D y1, D x2, D y2) : m_p1(x1, y1), m_p2(x2, y2) {
    //  .. nothing else ..
  }
   ...
   */
  template <class D>
  edge(const point<D> &p1, const point<D> &p2) : m_p1(p1), m_p2(p2) {
    //  .. nothing else ..
  }
   ...
   */
  template <class D>
  edge(const point<D> &p, const vector<D> &v) : m_p1(p), m_p2(p + v) {
    //  .. nothing else ..
  }
   ...
   */
  template <class D>
  explicit edge(const edge<D> &e) : m_p1(e.p1()), m_p2(e.p2()) {
    //  .. nothing else ..
  }
   ...
   */
  void translate(const edge<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const edge<C> &d, const T &t, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
    transform(t);
  }
   ...
   */
  bool operator!=(const edge<C> &b) const { return !operator==(b); }
   ...
   */
  bool not_equal(const edge<C> &b) const { return !equal(b); }
  ...
  edge<C> &extend(C e) {
    ...
    *this = edge<C>(point<C>(point<double>(p1()) - dp),
                    point<C>(point<double>(p2()) + dp));
    return *this;
  }
  ...
  edge<C> extended(C e) const {
    ...
    return edge<C>(point<C>(point<double>(p1()) - dp),
                   point<C>(point<double>(p2()) + dp));
  }
   ...
   */
  template <class Tr> edge<C> &transform(const Tr &t) {
    if (t.is_mirror()) {
      //  NOTE: in case of mirroring transformations we swap p1 and p2. The
      //  reasoning is that this way we maintain the orientation semantics:
      //  "right" of the edge is "inside" of an area.
   ...
   */
  void set_p1(const point<C> &p) { m_p1 = p; }

  /**
   *  @brief Set the second point.
   */
  void set_p2(const point<C> &p) { m_p2 = p; }

  /**
   *  @brief The first point.
   */
  const point<C> &p1() const { return m_p1; }

  /**
   *  @brief The second point.
   */
  const point<C> &p2() const { return m_p2; }

  /**
   *  @brief Returns the bounding box
   */
  box_type bbox() const { return box_type(m_p1, m_p2); }

  /**
   *  @brief The horizontal extend of the edge.
   */
  vector_type d() const { return vector_type(dx(), dy()); }

  /**
   *  @brief The horizontal extend of the edge.
   */
  C dx() const { return m_p2.x() - m_p1.x(); }

  /**
   *  @brief The vertical extend of the edge.
   */
  C dy() const { return m_p2.y() - m_p1.y(); }

  /**
   *  @brief Shortcut for p1().x()
   */
  C x1() const { return m_p1.x(); }

  /**
   *  @brief Shortcut for p1().y()
   */
  C y1() const { return m_p1.y(); }

  /**
   *  @brief Shortcut for p2().x()
   */
  C x2() const { return m_p2.x(); }

  /**
   *  @brief Shortcut for p2().y()
   */
  C y2() const { return m_p2.y(); }
   ...
   */
  bool is_ortho() const { return m_p1.x() == m_p2.x() || m_p1.y() == m_p2.y(); }

  /**
   *  @brief Test for degenerated edge
   */
  bool is_degenerate() const { return m_p1 == m_p2; }
   ...
   */
  distance_type ortho_length() const { return dx_abs() + dy_abs(); }
  ...
  bool contains(const db::point<C> &p) const {
    if (is_degenerate()) {
      ...
      return distance_abs(p) < coord_traits::prec_distance() &&
             coord_traits::sprod_sign(p.x(), p.y(), m_p2.x(), m_p2.y(),
                                      m_p1.x(), m_p1.y()) >= 0 &&
             coord_traits::sprod_sign(p.x(), p.y(), m_p1.x(), m_p1.y(),
                                      m_p2.x(), m_p2.y()) >= 0;
    }
  }
  ...
  bool coincident(const db::edge<C> &e) const {
    return !is_degenerate() && !e.is_degenerate() &&
           ...
           (sprod_sign(*this, e) < 0
                ? (coord_traits::sprod_sign(e.p2().x(), e.p2().y(), p1().x(),
                                            p1().y(), p2().x(), p2().y()) > 0 &&
                   coord_traits::sprod_sign(e.p1().x(), e.p1().y(), p2().x(),
                                            p2().y(), p1().x(), p1().y()) > 0)
                : (coord_traits::sprod_sign(e.p1().x(), e.p1().y(), p1().x(),
                                            p1().y(), p2().x(), p2().y()) > 0 &&
                   coord_traits::sprod_sign(e.p2().x(), e.p2().y(), p2().x(),
                                            p2().y(), p1().x(), p1().y()) > 0));
  }
  ...
  std::pair<bool, db::point<C>> intersect_point(const db::edge<C> &e) const {
    if (is_degenerate()) {
      ...
      coord_type x = std::max(std::min(p1().x(), p2().x()),
                              std::min(e.p1().x(), e.p2().x()));
      coord_type y = std::max(std::min(p1().y(), p2().y()),
                              std::min(e.p1().y(), e.p2().y()));
      ...
      std::pair<area_type, int> vsa = coord_traits::vprod_with_sign(
          e.p2().x(), e.p2().y(), m_p1.x(), m_p1.y(), e.p1().x(), e.p1().y());
      ...
      std::pair<area_type, int> vsb = coord_traits::vprod_with_sign(
          e.p2().x(), e.p2().y(), m_p2.x(), m_p2.y(), e.p1().x(), e.p1().y());
  ...
  coord_type distance(const db::point<C> &p) const {
    ...
    if (is_degenerate()) {
      ...
      area_type axb = coord_traits::vprod(m_p2.x(), m_p2.y(), p.x(), p.y(),
                                          m_p1.x(), m_p1.y());
   ...
   *
   *  The distance is computed as the minimum distance of the point to any of
   * the edge's points.
   *
  ...
  int side_of(const db::point<C> &p) const {
    ...
    if (is_degenerate()) {
      ...
      return coord_traits::vprod_sign(m_p2.x(), m_p2.y(), p.x(), p.y(),
                                      m_p1.x(), m_p1.y());
    }
  }
   ...
   *
   *  Determines the part of the line (given by the edge) that runs through the
   * given box. If the line does not hit the box, false is returned in the first
   * member of the return value.
   *
   *  @return first: false if the line does not hit the box, second: the part of
  ...
  std::pair<bool, edge> clipped_line(const db::box<C> &box) const {
    ...
    std::pair<bool, db::point<C>> pc1(false, db::point<C>()),
        pc2(false, db::point<C>());

    pc1 = cut_point(
        db::edge<C>(box.p1(), db::point<C>(box.p1().x(), box.p2().y())));
    if (pc1.first) {
      pc2 = cut_point(
          db::edge<C>(db::point<C>(box.p2().x(), box.p1().y()), box.p2()));
    }

    if (!pc1.first || !pc2.first) {
      pc1 = cut_point(
          db::edge<C>(box.p1(), db::point<C>(box.p2().x(), box.p1().y())));
      if (pc1.first) {
        pc2 = cut_point(
            db::edge<C>(db::point<C>(box.p1().x(), box.p2().y()), box.p2()));
      }
    }
  ...
  std::pair<bool, edge> clipped(const db::box<C> &box) const {
    ...
    if (p2.x() < box.left()) {
      ...
      p1 = db::point<C>(box.left(),
                        m_p1.y() + db::coord_traits<C>::rounded(
                                       (double)(box.left() - m_p1.x()) *
                                       double(dy()) / double(dx())));
    }
    if (p1.x() > box.right()) {
      return std::make_pair(false, db::edge<C>());
    } else if (p2.x() > box.right()) {
      p2 = db::point<C>(box.right(),
                        m_p1.y() + db::coord_traits<C>::rounded(
                                       (double)(box.right() - m_p1.x()) *
                                       double(dy()) / double(dx())));
    }
    ...
    if (p2.y() < box.bottom()) {
      ...
      p1 = db::point<C>(
          std::max(box.left(),
                   std::min(box.right(),
                            m_p1.x() + db::coord_traits<C>::rounded(
                                           (double)(box.bottom() - m_p1.y()) *
                                           double(dx()) / double(dy())))),
          box.bottom());
    }
    if (p1.y() > box.top()) {
  ...
  bool crossed_by(const db::edge<C> &e) const {
    ...
    int vsa = coord_traits::vprod_sign(m_p2.x(), m_p2.y(), e.p1().x(),
                                       e.p1().y(), m_p1.x(), m_p1.y());
    if (vsa < 0) {
      res = false;
    } else if (vsa == 0) {
      return true;
    ...
    int vsb = coord_traits::vprod_sign(m_p2.x(), m_p2.y(), e.p2().x(),
                                       e.p2().y(), m_p1.x(), m_p1.y());
    if (vsb > 0) {
      res = !res;
    } else if (vsb == 0) {
      return true;
  ...
  std::pair<bool, db::point<C>> crossed_by_point(const db::edge<C> &e) const {
    ...
    std::pair<area_type, int> vsa = coord_traits::vprod_with_sign(
        m_p2.x(), m_p2.y(), e.p1().x(), e.p1().y(), m_p1.x(), m_p1.y());
    area_type vxa = vsa.first;
    if (vsa.second < 0) {
      res = false;
    } else if (vsa.second == 0) {
    ...
    std::pair<area_type, int> vsb = coord_traits::vprod_with_sign(
        m_p2.x(), m_p2.y(), e.p2().x(), e.p2().y(), m_p1.x(), m_p1.y());
    area_type vxb = -vsb.first;
    if (vsb.second > 0) {
      res = !res;
    } else if (vsb.second == 0) {
  ...
  std::pair<bool, db::point<C>> projected(const db::point<C> &pt) const {
    return db::edge<C>(pt, pt + db::vector<C>(dy(), -dx()))
        .crossed_by_point(*this);
  }
  ...
  std::pair<bool, db::point<C>> cut_point(const db::edge<C> &e2) const {
    std::pair<typename coord_traits::area_type, int> vps =
        coord_traits::vprod_with_sign(e2.dx(), e2.dy(), this->dx(), this->dy(),
                                      0, 0);
    if (vps.second != 0) {
      double pr1 = double(coord_traits::vprod(e2.p1().x(), e2.p1().y(),
                                              this->p2().x(), this->p2().y(),
                                              this->p1().x(), this->p1().y()));
      double pr2 = double(vps.first);
      db::point<C> p =
          e2.p1() - db::vector<C>((e2.p2() - e2.p1()) * (pr1 / pr2));
 ...
 */
template <class Edge> struct edges_intersect {
 ...
 */
template <class C> inline edge<double> operator*(const edge<C> &e, double s) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const edge<C> &e) {
...
/**
 *  @brief Convenience wrappers for coord_traits functions: vector product: p x
 * q
 */
template <class C>
typename db::coord_traits<C>::area_type vprod(const db::edge<C> &p,
                                              const db::edge<C> &q) {
  return db::coord_traits<C>::vprod(p.dx(), p.dy(), q.dx(), q.dy(), 0, 0);
}

/**
 *  @brief Convenience wrappers for coord_traits functions: vector product sign:
 * sign(p x q)
 */
template <class C> int vprod_sign(const db::edge<C> &p, const db::edge<C> &q) {
  return db::coord_traits<C>::vprod_sign(p.dx(), p.dy(), q.dx(), q.dy(), 0, 0);
}

/**
 *  @brief Convenience wrappers for coord_traits functions: scalar product: 0->p
 * x 0->q
 */
template <class C>
typename db::coord_traits<C>::area_type sprod(const db::edge<C> &p,
                                              const db::edge<C> &q) {
  return db::coord_traits<C>::sprod(p.dx(), p.dy(), q.dx(), q.dy(), 0, 0);
}

/**
 *  @brief Convenience wrappers for coord_traits functions: scalar product sign:
 * sign(0->p x 0->q)
 */
template <class C> int sprod_sign(const db::edge<C> &p, const db::edge<C> &q) {
 ...
 */
template <class C> inline C edge_ymin(const db::edge<C> &e) {
 ...
 */
template <class C> inline C edge_ymax(const db::edge<C> &e) {
 ...
 */
template <class C> inline C edge_xmin(const db::edge<C> &e) {
 ...
 */
template <class C> inline C edge_xmax(const db::edge<C> &e) {
 ...
 */
template <class C> inline double edge_xaty(db::edge<C> e, C y) {
  if (e.p1().y() > e.p2().y()) {
    e.swap_points();
  }
 ...
 */
template <class C> class edge_ymin_compare {
public:
 ...
 */
template <class C> class edge_ymax_compare {
public:
  bool operator()(const db::edge<C> &a, const db::edge<C> &b) const {
    C ya = edge_ymax(a);
    C yb = edge_ymax(b);
 ...
 */
template <class C> class edge_xmin_compare {
public:
  bool operator()(const db::edge<C> &a, const db::edge<C> &b) const {
    C ya = edge_xmin(a);
    C yb = edge_xmin(b);
 ...
 */
template <class C> class edge_xmax_compare {
public:
  bool operator()(const db::edge<C> &a, const db::edge<C> &b) const {
    C ya = edge_xmax(a);
    C yb = edge_xmax(b);
 ...
 */
template <class C> struct edge_xmin_at_yinterval_compare {
  edge_xmin_at_yinterval_compare(C y1, C y2) : m_y1(y1), m_y2(y2) {
    // .. nothing yet ..
  }
