...
struct DB_PUBLIC PCellParametersCompareFunc {
  bool operator()(const pcell_parameters_type *a,
                  const pcell_parameters_type *b) const;
};

/**
 *  @brief A PCell header
 *
 *  The PCell header manages the PCell variants along with the PCell
 * declaration.
 */
class DB_PUBLIC PCellHeader {
public:
  typedef std::map<const pcell_parameters_type *, db::PCellVariant *,
                   PCellParametersCompareFunc>
      variant_map_t;
   ...
   */
  PCellHeader(size_t pcell_id, const std::string &name,
              PCellDeclaration *declaration);
  ...
  /**
   *  @brief Get the layer index list for this PCell declaration for the given
   * parameter set
   */
  std::vector<unsigned int>
  get_layer_indices(db::Layout &layout, const pcell_parameters_type &parameters,
                    db::ImportLayerMapping *layer_mapping = 0);
   ...
   */
  PCellVariant *get_variant(db::Layout &layout,
