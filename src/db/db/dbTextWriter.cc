...
void TextWriter::end_sorted_section() {
  ...
  for (std::vector<std::string>::const_iterator l = m_cc.begin();
       l != m_cc.end(); ++l) {
...
TextWriter &TextWriter::operator<<(const std::string &s) {
  if (m_in_cell) {
    m_cc_line += s;
  } else {
...
TextWriter &TextWriter::operator<<(endl_t) {
  *this << endl_str();
...
TextWriter &TextWriter::operator<<(const char *s) {
  *this << std::string(s);
  return *this;
}

TextWriter &TextWriter::operator<<(int64_t n) {
  *this << tl::sprintf("%d", n);
  return *this;
}

TextWriter &TextWriter::operator<<(int32_t n) {
  *this << tl::sprintf("%d", n);
  return *this;
}

TextWriter &TextWriter::operator<<(double d) {
  *this << tl::sprintf("%.12g", d);
  return *this;
}

TextWriter &TextWriter::operator<<(db::Point p) {
  *this << tl::sprintf("{%d %d}", p.x(), p.y());
  return *this;
}

TextWriter &TextWriter::operator<<(db::Vector p) {
  *this << tl::sprintf("{%d %d}", p.x(), p.y());
  return *this;
}

TextWriter::endl_t TextWriter::endl() { return endl_t(); }

const char *TextWriter::endl_str() { return "\n"; }
...
void TextWriter::write(const db::Layout &layout) {
  ...
  *this << "begin_lib" << pfx << " " << layout.dbu() << endl();
  ...
  for (db::Layout::bottom_up_const_iterator cell = layout.begin_bottom_up();
       cell != layout.end_bottom_up(); ++cell) {
    ...
    *this << "begin_cell" << pfx << " {" << layout.cell_name(*cell) << "}"
          << endl();
    ...
    for (db::Cell::const_iterator inst = cref.begin(); !inst.at_end(); ++inst) {
      ...
      for (db::CellInstArray::iterator i = inst->begin(); !i.at_end(); ++i) {
        ...
        *this << (is_reg ? "aref" : "sref") << pfx << " {"
              << layout.cell_name(inst->cell_index()) << "}";
    ...
    for (unsigned int l = 0; l < layout.layers(); ++l) {
      ...
      if (layout.is_valid_layer(l)) {
        ...
        db::ShapeIterator shape(cref.shapes(l).begin(
        ...
        while (!shape.at_end()) {
          ...
          if (shape->is_text()) {
            ...
            *this << "text" << pfx << " " << layer << " " << datatype << " "
                  << ((trans.rot() % 4) * 90.0) << " "
                  << (trans.is_mirror() ? 1 : 0) << " " << trans.disp() << " {"
                  << shape->text_string() << "}" << endl();

          } else if (shape->is_polygon()) {

            *this << "boundary" << pfx << " " << layer << " " << datatype;
            ...
            *this << "path" << pfx << " " << layer << " " << datatype << " "
                  << path.width() << " " << path.extensions().first << " "
