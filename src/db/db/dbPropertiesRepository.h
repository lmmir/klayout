...
public:
  ...
  typedef std::map<properties_id_type, properties_set>::iterator
      non_const_iterator;
   ...
   *  with the tl::Variant variant. In principle it could even be a list or
   * void. This method will assign a new Id to the given name if required and
   ...
   *
   *  All properties with the given name Id will get the new name. This method
   * is particular useful for the OASIS reader, which may associate a name with
   * an Id at a late stage. The new name must not be associated with an Id
   * already. The old name will stay associated with the Id.
   */
   ...
   */
  void change_properties(properties_id_type id,
                         const properties_set &new_props);

  /**
   *  @brief Get the id for a name
   *
   *  This method checks whether the given name is present as a name and returns
   * the id in the second member of the pair. The first member is true, if the
   * name is present.
   */
  std::pair<bool, property_names_id_type>
  get_id_of_name(const tl::Variant &name) const;
   ...
   */
  non_const_iterator begin_non_const() { return m_properties_by_id.begin(); }

  /**
   *  @brief Iterate over Id/Properties sets: end iterator (non-const)
   */
  non_const_iterator end_non_const() { return m_properties_by_id.end(); }

  /**
   *  @brief Iterate over Id/Properties sets
   */
  iterator begin() const { return m_properties_by_id.begin(); }

  /**
   *  @brief Iterate over Id/Properties sets: end iterator
   */
  iterator end() const { return m_properties_by_id.end(); }

  /**
   *  @brief Obtain the first properties id in the repository
   */
  properties_id_type begin_id() { return 0; }

  /**
   *  @brief Obtain the last properties id in the repository plus 1
   */
  properties_id_type end_id() { return m_properties_by_id.size(); }
   ...
   */
  const properties_id_vector &
  properties_ids_by_name_value(const name_value_pair &nv) const;
   ...
   */
  properties_id_type translate(const PropertiesRepository &rep,
                               properties_id_type id);
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    db::mem_stat(stat, purpose, cat, m_properties_component_table, true,
                 parent);
  }
...
public:
  ...
  /**
   *  @brief Creates a "pass all" (pass = true) or "remove all" (pass = false)
   * translator
   */
   ...
   */
  PropertiesTranslator(
      const std::map<db::properties_id_type, db::properties_id_type> &map);

  /**
   *  @brief Gets a value indicating whether the translator is "pass"
   */
  bool is_pass() const { return m_pass; }

  /**
   *  @brief Gets a value indicating whether the translator is "empty" (remove
   * all)
   */
  bool is_empty() const { return !m_pass && m_map.empty(); }

  /**
   *  @brief Gets a value indicating whether the translator is "null"
   * (default-constructed)
   */
  ...
  /**
   *  @brief Concatenates two translators (the right one first) - in place
   * version
   */
   ...
   *
   *  The translator delivered by this function will leave only the given keys
   * in the properties.
   */
  static PropertiesTranslator make_filter(db::PropertiesRepository &repo,
                                          const std::set<tl::Variant> &keys);

  /**
   *  @brief Factory: create a key mapper translator
   *
   *  The translator delivered by this function will translate the given keys to
   * new ones and remove non-listed keys.
   */
  static PropertiesTranslator
  make_key_mapper(db::PropertiesRepository &repo,
                  const std::map<tl::Variant, tl::Variant> &keys);
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const PropertiesRepository &x,
