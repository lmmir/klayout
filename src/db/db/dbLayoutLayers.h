 ...
 *
 *  The layer iterator delivers layer indices and layer properties of layer
 * layers.
 */
...
public:
   ...
   *  allocated.
   */
  unsigned int layers() const { return (unsigned int)m_layer_states.size(); }

  /**
   *  @brief The iterator of valid layers: begin
   */
  layer_iterator begin_layers() const { return layer_iterator(0, *this); }

  /**
   *  @brief The iterator of valid layers: end
   */
  layer_iterator end_layers() const { return layer_iterator(layers(), *this); }
   ...
   */
  int guiding_shape_layer_maybe() const { return m_guiding_shape_layer; }
...
private:
  ...
  std::multimap<LayerProperties, unsigned int, db::LPLogicalLessFunc>
      m_layers_by_props;
