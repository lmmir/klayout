 ...
 */
class DB_PUBLIC SubCircuit : public db::NetlistObject {
public:
   ...
   * Circuit::subcircuit_by_id. When assigned, the subcircuit ID is not 0.
   */
   ...
   */
  const Circuit *circuit_ref() const { return m_circuit_ref.get(); }

  /**
   *  @brief Gets the circuit the reference points to (non-const version)
   */
  Circuit *circuit_ref() { return m_circuit_ref.get(); }
  ...
  /**
   *  @brief Gets the net attached to a specific pin as a subcircuit pin ref
   * object Returns 0 if no net is attached.
   */
  const NetSubcircuitPinRef *netref_for_pin(size_t pin_id) const;

  /**
   *  @brief Gets the net attached to a specific pin as a subcircuit pin ref
   * object (non-const version) Returns 0 if no net is attached.
   */
  NetSubcircuitPinRef *netref_for_pin(size_t pin_id) {
    return const_cast<NetSubcircuitPinRef *>(
        ((const SubCircuit *)this)->netref_for_pin(pin_id));
  }
...
private:
   ...
   */
  void set_circuit(Circuit *c) { mp_circuit = c; }
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const SubCircuit &x, bool no_self, void *parent) {
