 ...
 *  Names are words (alphanumerical plus "$", "_", ".") or enclosed in single or
 * double quotes. Escape character is backslash. Separator is either , or
 * whitespace. Keywords and names are case sensitive. Short keys are provided
 * for compacter representation. Short keys can be non-alpha (e.g. "*") or
 * empty. Single-valued attributes can be given without brackets. All dimensions
 * are in units of database unit. The file follows the declaration-before-use
 * principle (circuits before subcircuits, nets before use ...)
 *
 *  Main body:
 ...
 *
 *  [description]:
 *    description(<text>)           - an arbitrary description text [short key:
 * B]
 *
 *  [unit]:
 ...
 *
 *  [top]:
 *    top(<circuit>)                - specifies the name of the top circuit
 * [short key: W]
 *
 *  [layer]:
 ...
 *
 *  [connect]:
 *    connect(<layer1> <name> ...)  - connects layer1 with the following layers
 * [short key: C]
 *
 *  [global]:
 *    global(<layer> <net-name> ...)
 *                                  - connects the shapes of the layer with the
 * given global nets [short key: G]
 *
 *  [circuit]:
 *    circuit(<name> [circuit-def]) - circuit (cell) [short key: X]
 *
 *  [class]:
 *    class(<name> <template> [template-def])  - a device class definition
 * (template: RES,CAP,...) [short key: K]
 *
 *  [device]:
 ...
 *                                    A net declaration shall be there also if
 * no geometry is present. The ID is a numerical shortcut for the net.
 *
 *  [circuit-pin]:
 ...
 *
 *  [circuit-device]:
 *    device(<id> <abstract-or-class>
 * [name|trans|combined-device|terminal-route|param|device-terminal|any]*)
 ...
 *                                    (for combined devices) with abstract
 * <abstract> and offset dx, dy.
 *
 *  [terminal-route]:
 *    connect(<device-index> <outer-terminal-name> <inner-terminal-name>)
 *                                  - connects the outer terminal with the
 * terminal of the device component with <device-index>: 0 is the basic device,
 * 1 the first combined device etc.
 *
 *  [name]:
 ...
 *                                  - specifies a property value/key pair [short
 * key: F] prop-name and prop-value are variant specifications in klayout
 * notation: #x is an integer, ##y a floating-point value, a word or quoted
 * literal is a string.
 *
 *  [polygon]:
 ...
 *    (<x> <y>)                     - relative coordinates (reference is reset
 * to 0,0 for each net or terminal in device abstract)
 *
 *  [template-def]:
 *    [template-param|template-terminal|any]*
 *
 *  [template-param]:
 *    param(<name> <primary>? <default-value>*)    - defines a template
 * parameter [short key: E]
 ...
 *                                  - specifies the terminal geometry [short
 * key: T]
 *
 *  [param]:
 ...
 *
 *  [subcircuit-pin]:
 *    pin(<pin-id> <net-id>)        - specifies connection of the pin with a net
 * [short key: P]
 *
 *  [trans]:
 *    location(<x> <y>)             - location of the instance [short key: Y]
 *    rotation(<angle>)             - rotation angle (in degree, default is 0)
 * [short key: O] mirror                        - if specified, the instance is
 * mirrored before rotation [short key: M] scale(<mag>)                  -
 * magnification (default is 1) [short key: S]
 *
 *  [any]:
 *    * |
 *    <token> |
