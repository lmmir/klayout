...
template <class T> class DB_PUBLIC generic_shape_iterator_delegate_base {
public:
  ...
  virtual bool
  equals(const generic_shape_iterator_delegate_base<T> *other) const = 0;
};
...
public:
  ...
  virtual bool is_addressable() const { return addressable; }

  virtual void do_reset(const db::Box &, bool) { m_iter = m_from; }

  virtual bool at_end() const { return m_iter == m_to; }

  virtual void increment() { ++m_iter; }

  virtual const value_type *get() const { return m_iter.operator->(); }

  virtual db::properties_id_type prop_id() const { return 0; }
  ...
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const generic_shape_iterator_delegate2<Iter> *o =
        dynamic_cast<const generic_shape_iterator_delegate2<Iter> *>(other);
...
public:
  ...
  virtual bool
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const generic_shape_iterator_delegate1<Iter> *o =
        dynamic_cast<const generic_shape_iterator_delegate1<Iter> *>(other);
    return o && o->m_iter == m_iter;
  }
...
public:
  generic_shapes_iterator_delegate(const db::Shapes *shapes)
      : mp_shapes(shapes) {
    //  NOTE: to allow multiple iterators acting on the same Shapes container at
    //  once, we always sort before we deliver the iterator - also in the
    //  non-region case. Without this, sorting may happen while another iterator
    //  is progressing.
    ...
    m_is_addressable =
        !shape_flags_with_props<T>() &&
        (shape_flags<T>() == shape_flags_pure<T>() ||
         mp_shapes->begin(shape_flags<T>() - shape_flags_pure<T>()).at_end());
    set();
  }

  virtual bool is_addressable() const { return m_is_addressable; }
  ...
  virtual db::properties_id_type prop_id() const { return m_iter->prop_id(); }

  generic_shape_iterator_delegate_base<T> *clone() const {
    return new generic_shapes_iterator_delegate<T>(*this);
  }

  virtual db::Box bbox() const { return mp_shapes->bbox(); }

  virtual bool
  equals(const generic_shape_iterator_delegate_base<T> *other) const {
    const generic_shapes_iterator_delegate<T> *o =
        dynamic_cast<const generic_shapes_iterator_delegate<T> *>(other);
    return o && o->mp_shapes == mp_shapes &&
           o->m_iter.at_end() == m_iter.at_end() &&
           (m_iter.at_end() || *o->m_iter == *m_iter);
  }
...
private:
  ...
  generic_shapes_iterator_delegate(
      const generic_shapes_iterator_delegate &other)
      : mp_shapes(other.mp_shapes), m_iter(other.m_iter),
        m_is_addressable(other.m_is_addressable) {
    set();
  }
...
template <class T> class DB_PUBLIC generic_shape_iterator {
public:
  typedef T value_type;
  ...
  generic_shape_iterator() : mp_delegate(0) {}
  ...
  generic_shape_iterator(generic_shape_iterator &&other) : mp_delegate(0) {
  ...
  generic_shape_iterator &
  set_delegate(generic_shape_iterator_delegate_base<T> *delegate) {
    delete mp_delegate;
    mp_delegate = delegate;
    return *this;
  }
  ...
  bool operator==(const generic_shape_iterator<T> &other) const {
    return mp_delegate && other.mp_delegate &&
           mp_delegate->equals(other.mp_delegate);
  }
  ...
  reference operator*() const { return *mp_delegate->get(); }

  pointer operator->() const { return mp_delegate->get(); }
  ...
  bool at_end() const { return !mp_delegate || mp_delegate->at_end(); }

  generic_shape_iterator confined(const db::Box &box, bool overlapping) const {
  ...
  db::Box bbox() const { return mp_delegate ? mp_delegate->bbox() : db::Box(); }
...
class DB_PUBLIC generic_shape_iterator_with_properties_delegate
    : public generic_shape_iterator_delegate_base<
          db::object_with_properties<T>> {
public:
  generic_shape_iterator_with_properties_delegate(
      generic_shape_iterator<T> &&basic)
      : m_basic(basic) {
    set();
  }

  generic_shape_iterator_with_properties_delegate(
      generic_shape_iterator_delegate_base<T> *delegate)
      : m_basic(delegate) {
    set();
  }

  generic_shape_iterator_with_properties_delegate(
      const generic_shape_iterator<T> &basic)
      : m_basic(basic) {
    set();
  }
  ...
  virtual db::Box bbox() const { return m_basic.bbox(); }

  virtual bool is_addressable() const { return false; }

  virtual bool at_end() const { return m_basic.at_end(); }
  ...
  virtual const db::object_with_properties<T> *get() const { return &m_object; }
  ...
  virtual bool equals(
      const generic_shape_iterator_delegate_base<db::object_with_properties<T>>
          *other) const {
    const generic_shape_iterator_with_properties_delegate<T> *other_cast =
        dynamic_cast<
            const generic_shape_iterator_with_properties_delegate<T> *>(other);
...
template <class T>
generic_shape_iterator<db::object_with_properties<T>>
make_wp_iter(generic_shape_iterator<T> &&basic) {
  return generic_shape_iterator<db::object_with_properties<T>>().set_delegate(
      new generic_shape_iterator_with_properties_delegate<T>(basic));
}

template <class T>
generic_shape_iterator<db::object_with_properties<T>>
make_wp_iter(db::generic_shape_iterator_delegate_base<T> *delegate) {
  return generic_shape_iterator<db::object_with_properties<T>>().set_delegate(
      new generic_shape_iterator_with_properties_delegate<T>(delegate));
}
 ...
 *  The addressable_shape_delivery class help providing this ability by keeping
 * temporary copies if required on a heap.
 */

template <class Iter> class DB_PUBLIC addressable_shape_delivery_impl {
public:
  typedef typename Iter::value_type value_type;
  ...
  addressable_shape_delivery_impl(const Iter &iter,
                                  bool iterator_is_addressable)
  ...
  db::properties_id_type prop_id() const { return m_iter.prop_id(); }
...
public:
  ...
  addressable_shape_delivery() : addressable_shape_delivery_impl<iter_type>() {}

  explicit addressable_shape_delivery(const iter_type &iter)
      : addressable_shape_delivery_impl<iter_type>(iter,
                                                   iter.is_addressable()) {}
