...
class DB_PUBLIC_TEMPLATE shape_collection_processor : public tl::Object {
public:
  /**
   *  @brief Constructor
   */
 ...
 */
template <> struct DB_PUBLIC shape_collection_processor_delivery<db::Polygon> {
 ...
 */
template <> struct DB_PUBLIC shape_collection_processor_delivery<db::Text> {
  shape_collection_processor_delivery(db::Layout *layout, db::Shapes *shapes)
      : mp_layout(layout), mp_shapes(shapes) {}
 ...
 */
template <class Result> struct DB_PUBLIC shape_collection_processor_delivery {
  shape_collection_processor_delivery(db::Layout *, db::Shapes *shapes)
      : mp_shapes(shapes) {}

  void put(const Result &result) { mp_shapes->insert(result); }
...
DB_PUBLIC_TEMPLATE OutputContainer *shape_collection_processed_impl(
    const db::DeepLayer &input,
    const shape_collection_processor<Shape, Result> &filter) {
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    if (vars.get()) {
      ...
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
        ...
        shape_collection_processor_delivery<db::object_with_properties<Result>>
            delivery_wp(&layout, st);
        ...
        for (db::Shapes::shape_iterator si = s.begin(db::ShapeIterator::All);
          ...
          for (typename std::vector<Result>::const_iterator i = heap.begin();
               i != heap.end(); ++i) {
            if (si->prop_id()) {
              delivery_wp.put(db::object_with_properties<Result>(
                  i->transformed(trinv), si->prop_id()));
            } else {
      ...
      for (db::Shapes::shape_iterator si = s.begin(db::ShapeIterator::All);
        ...
        for (typename std::vector<Result>::const_iterator i = heap.begin();
          ...
          if (si->prop_id()) {
            delivery_wp.put(
                db::object_with_properties<Result>(*i, si->prop_id()));
          } else {
            delivery.put(*i);
          }
        }
...
public:
  extents_processor(db::Coord dx, db::Coord dy) : m_dx(dx), m_dy(dy) {}
