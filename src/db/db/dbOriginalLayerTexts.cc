...
class OriginalLayerTextsIterator : public TextsIteratorDelegate {
public:
  typedef db::Text value_type;

  OriginalLayerTextsIterator(const db::RecursiveShapeIterator &iter,
  ...
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const OriginalLayerTextsIterator *o =
        dynamic_cast<const OriginalLayerTextsIterator *>(other);
...
OriginalLayerTexts::OriginalLayerTexts() : AsIfFlatTexts() { init(); }

OriginalLayerTexts::OriginalLayerTexts(const OriginalLayerTexts &other)
    : AsIfFlatTexts(other), m_iter(other.m_iter),
...
TextsDelegate *OriginalLayerTexts::clone() const {
  return new OriginalLayerTexts(*this);
}

TextsIteratorDelegate *OriginalLayerTexts::begin() const {
...
bool OriginalLayerTexts::empty() const { return m_iter.at_end(); }

const db::Text *OriginalLayerTexts::nth(size_t) const {
  throw tl::Exception(tl::to_string(
      tr("Random access to texts is available only for flat collections")));
}

bool OriginalLayerTexts::has_valid_texts() const { return false; }

const db::RecursiveShapeIterator *OriginalLayerTexts::iter() const {
  return &m_iter;
}

void OriginalLayerTexts::apply_property_translator(
    const db::PropertiesTranslator &pt) {
  m_iter.apply_property_translator(pt);
}

db::PropertiesRepository *OriginalLayerTexts::properties_repository() {
...
bool OriginalLayerTexts::equals(const Texts &other) const {
  const OriginalLayerTexts *other_delegate =
      dynamic_cast<const OriginalLayerTexts *>(other.delegate());
