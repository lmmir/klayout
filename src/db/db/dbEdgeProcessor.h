...
public:
   ...
   *
   *  This method is called to deliver an edge that is not starting or ending at
   * the current scanline. Another delivery of a set of crossing edges may
   * happen through the skip_n method which delivers a set of (unspecified)
   * edges which are guaranteed to form a closed sequence, that is one which is
   * starting and ending at a wrap count of 0.
   */
   ...
   */
  bool can_stop() const { return m_can_stop; }

  /**
   *  @brief Resets the stop request
   */
  void reset_stop() { m_can_stop = false; }
...
protected:
   ...
   */
  void request_stop() { m_can_stop = true; }
 ...
 */
class DB_PUBLIC EdgeContainer : public EdgeSink {
public:
  /**
   *  @brief Constructor connecting this receiver to an external edge vector
   */
  EdgeContainer(std::vector<db::Edge> &edges, bool clear = false, int tag = 0,
                EdgeContainer *chained = 0)
      : EdgeSink(), mp_edges(&edges), m_clear(clear), m_tag(tag),
        mp_chained(chained) {}
  ...
  EdgeContainer(int tag = 0, EdgeContainer *chained = 0)
      : EdgeSink(), mp_edges(&m_edges), m_clear(false), m_tag(tag),
        mp_chained(chained) {}

  /**
   *  @brief Get the edges collected so far (const version)
   */
  const std::vector<db::Edge> &edges() const { return *mp_edges; }

  /**
   *  @brief Get the edges collected so far (non-const version)
   */
  std::vector<db::Edge> &edges() { return *mp_edges; }
  ...
  virtual void start() {
    if (m_clear) {
      mp_edges->clear();
      //  The single-shot scheme is a easy way to overcome problems with
      //  multiple start/flush brackets (i.e. on size filter)
      m_clear = false;
    }
    if (mp_chained) {
      mp_chained->start();
 ...
 *  evaluator into a defined state. Each edge has an integer property that can
 * be used to distinguish edges from different polygons or layers.
 */
 ...
 */
class DB_PUBLIC InteractionDetector : public EdgeEvaluatorBase {
public:
   ...
   *
   *  Use set_include_touching(f) to specify whether to include or not include
   * the touching case as interacting for mode 0.
   *
   *  In modes -2, -1 and +1, finish () needs to be called before the
   * interactions can be used.
   *
   *  All modes require property IDs to differentiate both inputs into primary
   * and secondary. Property IDs from 0 to the given last primary ID value are
   * considered to belong to the primary region. Property IDs above the last
   * primary ID are considered to belong to the secondary region. This last
   * property ID must be specified in the last_primary_id parameter. The
   * reported interactions will be (primary_id,secondary_id) even for outside
   * mode. For outside mode, the primary_id is always last_primary_id. In
   * outside mode, the interactions are pseudo-interactions as by definition
   * outside polygons don't interact.
   */
   ...
   *
   *  If this flag is set, the interaction will include "touching" interactions
   * in mode 0, i.e. touching shapes will be counted as interacting. In the
   * other modes, this flag should be true (the default).
   */
  void set_include_touching(bool f) { m_include_touching = f; }

  /**
   *  @brief Gets the "touching" flag
   */
  bool include_touching() const { return m_include_touching; }

  /**
   *  @brief Finish the collection
   *
   *  This method must be called in mode -1 and +1 in order to finish the
   * collection.
   */
  void finish();
   ...
   *
   *  The iterator delivers pairs of property values. The lower value will be
   * the first one of the pair.
   */
  iterator begin() const { return m_interactions.begin(); }

  /**
   *  @brief Iterator delivering the interactions (end iterator)
   */
  iterator end() const { return m_interactions.end(); }
  ...
  virtual bool is_reset() const {
    return m_inside_s.empty() && m_inside_n.empty();
  }
 ...
 */
template <class F>
class DB_PUBLIC_TEMPLATE GenericMerge : public EdgeEvaluatorBase {
public:
  /**
   *  @brief Constructor
   */
  ...
  virtual void reset() { m_wc_n = m_wc_s = 0; }
  ...
  virtual bool is_reset() const { return (m_wc_n == 0 && m_wc_s == 0); }
...
struct ParametrizedInsideFunc {
  ParametrizedInsideFunc(int mode) : m_mode(mode) {
    //  .. nothing yet ..
  }
 ...
 *  merge criterion for a set of edges. A result is generated if the wrap count
 * (wc) of the edge set satisfies a condition given by "mode". Specifically:
 ...
 *    mode == -n: wc >= n || wc <= -n
 */
class DB_PUBLIC SimpleMerge : public GenericMerge<ParametrizedInsideFunc> {
public:
  /**
   *  @brief Constructor
   */
 ...
 *  (AND, A NOT B, B NOT A, XOR, OR). The mode can be specified in the
 * constructor. It relies on the properties being set in a certain way: bit 0
 * codes the layer (0 for A, 1 for B) while the other bits are used to
 * distinguish the polygons. For each polygon, a non-zero wrap count rule is
 * applied before the boolean operation's output is formed.
 */
class DB_PUBLIC BooleanOp : public EdgeEvaluatorBase {
public:
  enum BoolOp { And = 1, ANotB = 2, BNotA = 3, Xor = 4, Or = 5 };
  ...
  virtual int compare_ns() const;
  virtual bool is_reset() const {
    return m_zeroes == m_wcv_n.size() + m_wcv_s.size();
  }

protected:
  template <class InsideFunc>
  bool result(int wca, int wcb, const InsideFunc &inside_a,
              const InsideFunc &inside_b) const;
  template <class InsideFunc>
  int edge_impl(bool north, bool enter, property_type p,
                const InsideFunc &inside_a, const InsideFunc &inside_b);
  template <class InsideFunc>
  int compare_ns_impl(const InsideFunc &inside_a,
                      const InsideFunc &inside_b) const;
 ...
 */
class DB_PUBLIC EdgePolygonOp : public db::EdgeEvaluatorBase {
public:
  /**
   ...
   *  @brief Constructor
   *
   *  @param outside If true, the operator will deliver edges outside the
   * polygon
   *  @param include_touching If true, edges on the polygon's border will be
   * considered "inside" of polygons
   *  @param polygon_mode Determines how the polygon edges on property 0 are
   * interpreted (see merge operators)
   */
  EdgePolygonOp(mode_t mode = Inside, bool include_touching = true,
                int polygon_mode = -1);
 ...
 */
class DB_PUBLIC BooleanOp2 : public BooleanOp {
public:
  /**
   *  @brief Constructor
   *
 ...
 */
class DB_PUBLIC MergeOp : public EdgeEvaluatorBase {
public:
  /**
   *  @brief Constructor
   *
 ...
 *
 *  An edge processor takes a set of edges, processes them by removing
 * intersections and applying a custom operator for computing the output edge
 * sets which then are delivered to an EdgeSink receiver object.
 */
...
public:
   ...
   *
   *  @param report_progress If true, a tl::Progress object will be created to
   * report any progress (warning: this will impose a performance penalty)
   *  @param progress_text The description text of the progress object
   */
  EdgeProcessor(bool report_progress = false,
                const std::string &progress_desc = std::string());
   ...
   *
   *  This method does not reserve for the number of elements required. This
   * must be done explicitly for performance benefits.
   */
  template <class Iter>
   ...
   * must be done explicitly for performance benefits.
   */
  template <class Iter> void insert_sequence(Iter i, property_type p = 0) {
    for (; !i.at_end(); ++i) {
      insert(*i, p);
    }
  }
   ...
   */
  void
  process(const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>
              &gen);
   ...
   */
  void
  redo(const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>
           &gen);

  /**
   *  @brief Merge the given polygons in a simple "non-zero wrapcount" fashion
   *
   *  The wrapcount is computed over all polygons, i.e. overlapping polygons may
   * "cancel" if they have different orientation (since a polygon is oriented by
   * construction that is not easy to achieve). The other merge operation
   * provided for this purpose is "merge" which normalizes each polygon
   * individually before merging them. "simple_merge" is somewhat faster and
   * consumes less memory.
   *
   *  The result is presented as a set of edges forming closed contours. Hulls
   * are oriented clockwise while holes are oriented counter-clockwise.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a SimpleMerge operator and puts the result into an output vector.
   *
   *  @param in The input polygons
   *  @param out The output edges
   *  @param mode The merge mode (see SimpleMerge constructor)
   */
  void simple_merge(const std::vector<db::Polygon> &in,
                    std::vector<db::Edge> &out, int mode = -1);
   ...
   * consumes less memory.
   *
   *  This method produces polygons and allows fine-tuning the parameters for
   * that purpose.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a SimpleMerge operator and puts the result into an output vector.
   ...
   *  @param min_coherence true, if touching corners should be resolved into
   * less connected contours
   *  @param mode The merge mode (see SimpleMerge constructor)
   */
  void simple_merge(const std::vector<db::Polygon> &in,
                    std::vector<db::Polygon> &out, bool resolve_holes = true,
                    bool min_coherence = true, int mode = -1);

  /**
   *  @brief Merge the given edges in a simple "non-zero wrapcount" fashion
   *
   *  The edges provided must form valid closed contours. Contours oriented
   * differently "cancel" each other. Overlapping contours are merged when the
   ...
   *  @param mode The merge mode (see SimpleMerge constructor)
   */
  void simple_merge(const std::vector<db::Edge> &in, std::vector<db::Edge> &out,
                    int mode = -1);

  /**
   *  @brief Merge the given edges in a simple "non-zero wrapcount" fashion into
   * polygons
   *
   *  The edges provided must form valid closed contours. Contours oriented
   * differently "cancel" each other. Overlapping contours are merged when the
   ...
   *  @param mode The merge mode (see SimpleMerge constructor)
   */
  void simple_merge(const std::vector<db::Edge> &in,
                    std::vector<db::Polygon> &out, bool resolve_holes = true,
                    bool min_coherence = true, int mode = -1);
   ...
   *
   *  In contrast to "simple_merge", this merge implementation considers each
   * polygon individually before merging them. Thus self-overlaps are
   * effectively removed before the output is computed and holes are correctly
   * merged with the hull. In addition, this method allows one to select areas
   * with a higher wrap count which allows one to compute overlaps of polygons
   * on the same layer. Because this method merges the polygons before the
   * overlap is computed, self-overlapping polygons do not contribute to higher
   * wrap count areas.
   *
   *  The result is presented as a set of edges forming closed contours. Hulls
   * are oriented clockwise while holes are oriented counter-clockwise.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a Merge operator and puts the result into an output vector.
   *
   *  @param in The input polygons
   *  @param out The output edges
   *  @param min_wc The minimum wrap count for output (0: all polygons, 1: at
   * least two overlapping)
   */
  void merge(const std::vector<db::Polygon> &in, std::vector<db::Edge> &out,
   ...
   * wrap count areas.
   *
   *  This method produces polygons and allows one to fine-tune the parameters
   * for that purpose.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a Merge operator and puts the result into an output vector.
   ...
   * less connected contours
   */
  void merge(const std::vector<db::Polygon> &in, std::vector<db::Polygon> &out,
             unsigned int min_wc = 0, bool resolve_holes = true,
             bool min_coherence = true);

  /**
   *  @brief Size the given polygons
   *
   *  This method sizes a set of polygons. Before the sizing is applied, the
   * polygons are merged. After that, sizing is applied on the individual result
   * polygons of the merge step. The result may contain overlapping contours,
   * but no self-overlaps.
   *
   *  dx and dy describe the sizing. A positive value indicates oversize
   * (outwards) while a negative one describes undersize (inwards). The sizing
   * applied can be chosen differently in x and y direction. In this case, the
   * sign must be identical for both dx and dy.
   *
   *  The result is presented as a set of edges forming closed contours. Hulls
   * are oriented clockwise while holes are oriented counter-clockwise.
   *
   *  This is a convenience method that bundles filling of the edges and
   * processing them and which puts the result into an output vector.
   *
   *  @param in The input polygons
   ...
   */
  void size(const std::vector<db::Polygon> &in, db::Coord dx, db::Coord dy,
            std::vector<db::Edge> &out, unsigned int mode = 2);
   ...
   *  This method sizes a set of polygons. Before the sizing is applied, the
   * polygons are merged. After that, sizing is applied on the individual result
   * polygons of the merge step. The result may contain overlapping polygons,
   * but no self-overlapping ones. Polygon overlap occurs if the polygons are
   * close enough, so a positive sizing makes polygons overlap.
   *
   *  dx and dy describe the sizing. A positive value indicates oversize
   * (outwards) while a negative one describes undersize (inwards). The sizing
   * applied can be chosen differently in x and y direction. In this case, the
   ...
   */
  void size(const std::vector<db::Polygon> &in, db::Coord dx, db::Coord dy,
            std::vector<db::Polygon> &out, unsigned int mode = 2,
            bool resolve_holes = true, bool min_coherence = true);

  /**
   *  @brief Size the given polygons (isotropic)
   *
   *  This method is equivalent to calling the anisotropic version with
   * identical dx and dy.
   *
   *  @param in The input polygons
   ...
   *  @param mode The sizing mode (see db::Polygon for a description)
   */
  void size(const std::vector<db::Polygon> &in, db::Coord d,
            std::vector<db::Edge> &out, unsigned int mode = 2) {
   ...
   *
   *  This method computes the result for the given boolean operation on two
   * sets of polygons. The result is presented as a set of edges forming closed
   * contours. Hulls are oriented clockwise while holes are oriented
   * counter-clockwise.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a Boolean operator and puts the result into an output vector.
   *
   ...
   */
  void boolean(const std::vector<db::Polygon> &a,
               const std::vector<db::Polygon> &b, std::vector<db::Edge> &out,
               int mode);
   ...
   *
   *  This method computes the result for the given boolean operation on two
   * sets of polygons. This method produces polygons on output and allows one to
   * fine-tune the parameters for that purpose.
   *
   *  This is a convenience method that bundles filling of the edges, processing
   * with a Boolean operator and puts the result into an output vector.
   *
   ...
   */
  void boolean(const std::vector<db::Polygon> &a,
               const std::vector<db::Polygon> &b, std::vector<db::Polygon> &out,
               int mode, bool resolve_holes = true, bool min_coherence = true);

  /**
   *  @brief Boolean operation for a set of given edges, creating edges
   *
   *  This method computes the result for the given boolean operation on two
   * sets of edges. The input edges must form closed contours where holes and
   * hulls must be oriented differently. The input edges are processed with a
   * simple non-zero wrap count rule as a whole.
   *
   *  The result is presented as a set of edges forming closed contours. Hulls
   * are oriented clockwise while holes are oriented counter-clockwise.
   *
   ...
   *  @param mode The boolean mode
   */
  void boolean(const std::vector<db::Edge> &a, const std::vector<db::Edge> &b,
   ...
   * simple non-zero wrap count rule as a whole.
   *
   *  This method produces polygons on output and allows one to fine-tune the
...
private:
  ...
  static size_t count_edges(const std::vector<db::Polygon> &v) {
    size_t n = 0;
    for (std::vector<db::Polygon>::const_iterator p = v.begin(); p != v.end();
         ++p) {
      n += count_edges(*p);
    }
    return n;
