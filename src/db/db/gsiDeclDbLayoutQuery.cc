...
static LayoutQueryIteratorWrapper
iterate(const db::LayoutQuery *q, const db::Layout *layout, tl::Eval *eval) {
  return LayoutQueryIteratorWrapper(*q, layout, eval);
}

static tl::Variant iter_get(db::LayoutQueryIterator *iter,
...
tl::Variant iter_get_named(db::LayoutQueryIterator *iter) {
  return iter_get(iter, NAME);
}

template <const char *NAME> gsi::Methods make_shortcut_method() {
  return gsi::method_ext(NAME, &iter_get_named<NAME>,
                         std::string("@brief A shortcut for 'get(\"") + NAME +
                             std::string("\")'\n"));
}

//  String literals cannot be used as template arguments but objects with
//  external linkage can be:
...
Class<db::LayoutQueryIterator> decl_LayoutQueryIterator(
    "db", "LayoutQueryIterator",
    gsi::method("layout", &db::LayoutQueryIterator::layout,
        ...
        gsi::method_ext(
            "get", &iter_get, gsi::arg("name"),
            "@brief Gets the query property with the given name\n"
            "The query properties available can be obtained from the query "
            "object using \\LayoutQuery#property_names.\n"
            "Some shortcut methods are available. For example, the \\data "
            "method provides a shortcut for 'get(\"data\")'.\n"
            "\n"
            "If a property with the given name is not available, nil will be "
            "returned.") +
    ...
    "\n"
    "This object is used by \\LayoutQuery#each to deliver the results of a "
    "query in an iterative fashion. "
...
Class<db::LayoutQuery> decl_LayoutQuery(
    "db", "LayoutQuery",
    gsi::constructor(
        "new", &new_query, gsi::arg("query"),
        ...
        gsi::method_ext("property_names", &query_prop_names,
                        ...
                        "The list of properties available from the query "
                        "depends on the nature of the query. "
                        "This method allows detection of the properties "
                        "available. Within the query, all of these "
                        "properties can be obtained from the query iterator "
                        "using \\LayoutQueryIterator#get.\n") +
        gsi::method(
            "execute", &db::LayoutQuery::execute, gsi::arg("layout"),
            gsi::arg("context", (tl::Eval *)0, "nil"),
            "@brief Executes the query\n"
            "\n"
            "This method can be used to execute \"active\" queries such\n"
            "as \"delete\" or \"with ... do\".\n"
            "It is basically equivalent to iterating over the query until it "
            "is\n"
            "done.\n"
            "\n"
            "The context argument allows supplying an expression execution "
            "context. This context can be used for "
            "example to supply variables for the execution. It has been added "
            "in version 0.26.\n") +
        gsi::iterator_ext(
            "each", &iterate, gsi::arg("layout"),
            gsi::arg("context", (tl::Eval *)0, "nil"),
            "@brief Executes the query and delivered the results iteratively.\n"
            "The argument to the block is a \\LayoutQueryIterator object which "
            "can be "
    ...
    "Layout queries are the backbone of the \"Search & replace\" feature. "
    "Layout queries allow retrieval of "
    "data from layouts and manipulation of layouts. This object provides "
    "script binding for this feature.\n"
    "Layout queries are used by first creating a query object. Depending on "
    "the nature of the query, either \\execute "
    "or \\each can be used to execute the query. \\execute will run the query "
    "and return once the query is finished. "
    "\\execute is useful for running queries that don't return results such as "
    "\"delete\" or \"with ... do\" queries.\n"
    "\\each can be used when the results of the query need to be retrieved.\n"
    "\n"
    "The \\each method will call a block a of code for every result available. "
    "It will provide a \\LayoutQueryIterator "
    "object that allows accessing the results of the query. Depending on the "
    "query, different attributes of the "
    "iterator object will be available. For example, \"select\" queries will "
    "fill the \"data\" attribute with an array of values "
