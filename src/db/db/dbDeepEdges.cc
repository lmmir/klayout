 ...
 */
class DB_PUBLIC DeepEdgesIterator : public EdgesIteratorDelegate {
public:
  ...
  virtual const value_type *get() const { return &m_edge; }
...
DeepEdges::DeepEdges() : MutableEdges() { init(); }

DeepEdges::DeepEdges(const DeepLayer &dl) : MutableEdges() {
  set_deep_layer(dl);
  init();
}
...
DeepEdges &DeepEdges::operator=(const DeepEdges &other) {
  if (this != &other) {
...
EdgesDelegate *DeepEdges::clone() const { return new DeepEdges(*this); }
  ...
  if (t.equal(Trans(db::Disp(t.disp())))) {
    ...
    if (layout.begin_top_down() != layout.end_top_down()) {
      ...
      for (db::RecursiveShapeIterator iter(layout, top_cell,
           ...
           !iter.at_end(); ++iter) {
        flat_shapes.insert(
            iter->edge().transformed(iter.trans()).transformed(t));
      }
...
EdgesIteratorDelegate *DeepEdges::begin() const {
  return new DeepEdgesIterator(begin_iter().first);
}

EdgesIteratorDelegate *DeepEdges::begin_merged() const {
  if (!merged_semantics()) {
    return begin();
  } else {
...
DeepEdges::begin_merged_iter() const {
  if (!merged_semantics()) {
    ...
    if (layout.cells() == 0) {
      ...
      const db::Cell &top_cell = layout.cell(*layout.begin_top_down());
      db::RecursiveShapeIterator iter(m_merged_edges.layout(), top_cell,
                                      m_merged_edges.layer());
      return std::make_pair(iter, db::ICplxTrans());
    }
  }
}

bool DeepEdges::empty() const { return begin_iter().first.at_end(); }

bool DeepEdges::is_merged() const { return m_is_merged; }

const db::Edge *DeepEdges::nth(size_t /*n*/) const {
  throw tl::Exception(tl::to_string(tr(
      "Random access to edges is available only for flat edge collections")));
}

bool DeepEdges::has_valid_edges() const { return false; }

bool DeepEdges::has_valid_merged_edges() const { return false; }

const db::RecursiveShapeIterator *DeepEdges::iter() const { return 0; }
...
bool DeepEdges::equals(const Edges &other) const {
  const DeepEdges *other_delegate =
      dynamic_cast<const DeepEdges *>(other.delegate());
  if (other_delegate &&
...
class ClusterMerger {
public:
  ClusterMerger(unsigned int layer, const db::hier_clusters<db::Edge> &hc,
...
private:
  ...
  db::properties_id_type property_id(size_t cid, db::cell_index_type ci,
    ...
    if (c.begin_attr() != c.end_attr()) {
      ...
      const db::connected_clusters<db::Edge>::connections_type &conn =
          cc.connections_for_cluster(cid);
      for (db::connected_clusters<db::Edge>::connections_type::const_iterator
               i = conn.begin();
  ...
  db::Shapes &compute_merged(size_t cid, db::cell_index_type ci, bool initial) {
    ...
    const db::connected_clusters<db::Edge>::connections_type &conn =
        cc.connections_for_cluster(cid);
    for (db::connected_clusters<db::Edge>::connections_type::const_iterator i =
             conn.begin();
         i != conn.end(); ++i) {
      const db::Shapes &cc_shapes =
          compute_merged(i->id(), i->inst_cell_index(), false);
    ...
    for (std::list<
             std::pair<const db::Shapes *, db::ICplxTrans>>::const_iterator i =
             merged_child_clusters.begin();
    ...
    for (db::local_cluster<db::Edge>::shape_iterator s = c.begin(m_layer);
         !s.at_end(); ++s) {
      heap.push_back(*s);
      m_scanner.insert(&heap.back(), 0);
    }
...
const DeepLayer &DeepEdges::merged_deep_layer() const {
  if (merged_semantics()) {
    ensure_merged_edges_valid();
...
void DeepEdges::ensure_merged_edges_valid() const {
  if (!m_merged_edges_valid) {
    ...
    if (m_is_merged) {
      ...
      ClusterMerger cm(deep_layer().layer(), hc, report_progress(),
                       progress_desc());
      cm.set_base_verbosity(base_verbosity() + 10);

      //  TODO: iterate only over the called cells?
      for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
        const db::connected_clusters<db::Edge> &cc =
            hc.clusters_per_cell(c->cell_index());
        for (db::connected_clusters<db::Edge>::all_iterator cl = cc.begin_all();
             !cl.at_end(); ++cl) {
          if (cc.is_root(*cl)) {
...
void DeepEdges::set_is_merged(bool f) {
  m_is_merged = f;
  m_merged_edges_valid = false;
  m_merged_edges = db::DeepLayer();
}

void DeepEdges::insert_into(db::Layout *layout, db::cell_index_type into_cell,
...
EdgesDelegate *DeepEdges::processed(const EdgeProcessorBase &filter) const {
  return shape_collection_processed_impl<db::Edge, db::Edge, db::DeepEdges>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}

EdgePairsDelegate *DeepEdges::processed_to_edge_pairs(
    const EdgeToEdgePairProcessorBase &filter) const {
  return shape_collection_processed_impl<db::Edge, db::EdgePair,
                                         db::DeepEdgePairs>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}

RegionDelegate *DeepEdges::processed_to_polygons(
    const EdgeToPolygonProcessorBase &filter) const {
  return shape_collection_processed_impl<db::Edge, db::Polygon, db::DeepRegion>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}
...
DeepEdges *DeepEdges::apply_filter(const EdgeFilterBase &filter) const {
  const db::DeepLayer &edges =
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    if (vars.get()) {
      ...
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
        ...
        for (db::Shapes::shape_iterator si = s.begin(db::ShapeIterator::Edges);
             !si.at_end(); ++si) {
...
EdgesDelegate *DeepEdges::merged() const {
  ...
  std::unique_ptr<db::DeepEdges> res(
      new db::DeepEdges(m_merged_edges.derived()));
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
...
DeepLayer DeepEdges::and_or_not_with(const DeepEdges *other,
                                     EdgeBoolOp op) const {
  ...
  proc.run(&local_op, deep_layer().layer(), other->deep_layer().layer(),
           dl_out.layer());
...
DeepEdges::edge_region_op(const DeepRegion *other, EdgePolygonOp::mode_t mode,
                          bool include_borders) const {
  ...
  db::local_processor<db::Edge, db::PolygonRef, db::Edge> proc(
      const_cast<db::Layout *>(&deep_layer().layout()),
      const_cast<db::Cell *>(&deep_layer().initial_cell()),
      &other->deep_layer().layout(), &other->deep_layer().initial_cell());
  proc.set_base_verbosity(base_verbosity());
...
EdgesDelegate *DeepEdges::and_with(const Region &other) const {
  ...
  if (empty()) {
    ...
    return new DeepEdges(edge_region_op(other_deep, EdgePolygonOp::Inside,
...
DeepEdges::andnot_with(const Region &other) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
  ...
  if (empty()) {
    ...
    auto res = edge_region_op(other_deep, EdgePolygonOp::Both,
                              true /*include borders*/);
...
EdgesDelegate *DeepEdges::not_with(const Region &other) const {
  ...
  if (empty() || other.empty()) {
    ...
    return new DeepEdges(edge_region_op(other_deep, EdgePolygonOp::Outside,
                                        true /*include borders*/)
                             .first);
  }
}
...
EdgesDelegate *DeepEdges::or_with(const Edges &other) const {
  //  NOTE: in the hierarchical case we don't do a merge on "or": just map to
  //  add
  return add(other);
}

EdgesDelegate *DeepEdges::add_in_place(const Edges &other) {
  if (other.empty()) {
    return this;
  }
...
EdgesDelegate *DeepEdges::inside_part(const Region &other) const {
  ...
  if (empty()) {
    ...
    return new DeepEdges(edge_region_op(other_deep, db::EdgePolygonOp::Inside,
                                        false /*include borders*/)
                             .first);
  }
}
...
EdgesDelegate *DeepEdges::outside_part(const Region &other) const {
  ...
  if (empty() || other.empty()) {
    ...
    return new DeepEdges(edge_region_op(other_deep, db::EdgePolygonOp::Outside,
                                        false /*include borders*/)
                             .first);
  }
}
...
std::pair<EdgesDelegate *, EdgesDelegate *>
DeepEdges::inside_outside_part_pair(const Region &other) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
...
RegionDelegate *DeepEdges::extended(coord_type ext_b, coord_type ext_e,
                                    coord_type ext_o, coord_type ext_i,
                                    bool join) const {
  const db::DeepLayer &edges = merged_deep_layer();
  ...
  //  TODO: there is a special case when we'd need a
  //  MagnificationAndOrientationReducer: dots formally don't have an
  //  orientation, hence the interpretation is x and y.
  db::MagnificationReducer red;
  db::cell_variants_collector<db::MagnificationReducer> vars(red);
  vars.collect(edges.layout(), edges.initial_cell());
      ...
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
        ...
        for (db::connected_clusters<db::Edge>::all_iterator cl = cc.begin_all();
          ...
          if (cc.is_root(*cl)) {
            ...
            polygon_transformation_filter<db::ICplxTrans> ptrans(
                &prgen, v->first.inverted());
            JoinEdgesCluster jec(&ptrans, ext_b, ext_e, ext_o, ext_i);

            std::list<db::Edge> heap;
            for (db::recursive_cluster_shape_iterator<db::Edge> rcsi(
                     hc, edges.layer(), c->cell_index(), *cl);
                 !rcsi.at_end(); ++rcsi) {
    ...
    for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
      ...
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
        ...
        for (db::Shapes::shape_iterator si =
                 c->shapes(edges.layer()).begin(db::ShapeIterator::Edges);
             !si.at_end(); ++si) {
          prgen.set_prop_id(si->prop_id());
          prgen.put(extended_edge(si->edge().transformed(v->first), ext_b,
                                  ext_e, ext_o, ext_i)
                        .transformed(v->first.inverted()));
        }
      }
    }
  }
...
public:
  ...
  Edge2EdgeInteractingLocalOperation(EdgeInteractionMode mode,
  ...
  do_compute_local(db::Layout * /*layout*/,
    ...
    for (shape_interactions<db::Edge, db::Edge>::iterator i =
             interactions.begin();
         i != interactions.end(); ++i) {
      for (shape_interactions<db::Edge, db::Edge>::iterator2 j =
               i->second.begin();
           j != i->second.end(); ++j) {
    ...
    for (std::set<db::Edge>::const_iterator o = others.begin();
         o != others.end(); ++o) {
  ...
  virtual OnEmptyIntruderHint on_empty_intruder_hint() const {
    if (m_mode == EdgesOutside) {
      return m_output_mode == Both ? Copy
                                   : (m_output_mode == Inverse ? Drop : Copy);
    } else {
      return m_output_mode == Both ? CopyToSecond
                                   : (m_output_mode == Inverse ? Copy : Drop);
    }
  }
...
public:
  ...
  virtual OnEmptyIntruderHint on_empty_intruder_hint() const { return Drop; }
...
public:
  ...
  Edge2PolygonInteractingLocalOperation(EdgeInteractionMode mode,
                                        output_mode_t output_mode)
      : m_mode(mode), m_output_mode(output_mode) {
    //  .. nothing yet ..
  }
  ...
  virtual void do_compute_local(
    ...
    for (shape_interactions<db::Edge, db::PolygonRef>::iterator i =
             interactions.begin();
         i != interactions.end(); ++i) {
      for (shape_interactions<db::Edge, db::PolygonRef>::iterator2 j =
               i->second.begin();
           j != i->second.end(); ++j) {
        others.insert(interactions.intruder_shape(*j).second);
      }
    ...
    for (std::set<db::PolygonRef>::const_iterator o = others.begin();
         o != others.end(); ++o) {
    ...
    if (m_output_mode == Inverse || m_output_mode == Both) {
      ...
      std::unordered_set<db::Edge> interacting;
      edge_to_region_interaction_filter<std::unordered_set<db::Edge>> filter(
          &interacting, m_mode);
      scanner.process(filter, 1, db::box_convert<db::Edge>(),
                      db::box_convert<db::Polygon>());
  ...
  virtual std::string description() const {
    if (m_mode == EdgesInteract) {
      if (m_output_mode == Inverse) {
        ...
        return tl::to_string(
            tr("Select interacting and non-interacting edges"));
      }
...
EdgesDelegate *DeepEdges::selected_interacting_generic(const Region &other,
                                                       EdgeInteractionMode mode,
                                                       bool inverse) const {
  ...
  db::Edge2PolygonInteractingLocalOperation op(
      mode, inverse ? db::Edge2PolygonInteractingLocalOperation::Inverse
                    : db::Edge2PolygonInteractingLocalOperation::Normal);

  db::local_processor<db::Edge, db::PolygonRef, db::Edge> proc(
      const_cast<db::Layout *>(&edges.layout()),
  ...
  proc.run(&op, edges.layer(),
           (mode == EdgesInside ? other_deep->merged_deep_layer()
...
std::pair<EdgesDelegate *, EdgesDelegate *>
DeepEdges::selected_interacting_pair_generic(const Region &other,
                                             EdgeInteractionMode mode) const {
  std::unique_ptr<db::DeepRegion> dr_holder;
  const db::DeepRegion *other_deep =
      dynamic_cast<const db::DeepRegion *>(other.delegate());
  if (!other_deep) {
  ...
  db::Edge2PolygonInteractingLocalOperation op(
      mode, db::Edge2PolygonInteractingLocalOperation::Both);
  ...
  proc.run(&op, edges.layer(), other_deep->merged_deep_layer().layer(),
           output_layers);
...
EdgesDelegate *DeepEdges::selected_interacting_generic(const Edges &other,
  ...
  if (!other_deep) {
    //  if the other edge collection isn't deep, turn into a top-level only deep
    //  edge collection to facilitate re-hierarchization
  ...
  db::Edge2EdgeInteractingLocalOperation op(
      mode, inverse ? db::Edge2EdgeInteractingLocalOperation::Inverse
                    : db::Edge2EdgeInteractingLocalOperation::Normal);
...
std::pair<EdgesDelegate *, EdgesDelegate *>
DeepEdges::selected_interacting_pair_generic(const Edges &other,
                                             EdgeInteractionMode mode) const {
  std::unique_ptr<db::DeepEdges> dr_holder;
  const db::DeepEdges *other_deep =
      dynamic_cast<const db::DeepEdges *>(other.delegate());
  ...
  db::Edge2EdgeInteractingLocalOperation op(
      mode, db::Edge2EdgeInteractingLocalOperation::Both);
...
RegionDelegate *DeepEdges::pull_generic(const Region &other) const {
  ...
  db::local_processor<db::Edge, db::PolygonRef, db::PolygonRef> proc(
      const_cast<db::Layout *>(&edges.layout()),
      const_cast<db::Cell *>(&edges.initial_cell()), &other_polygons.layout(),
      &other_polygons.initial_cell());
  proc.set_base_verbosity(base_verbosity());
  proc.set_threads(edges.store()->threads());
...
EdgesDelegate *DeepEdges::pull_generic(const Edges &other) const {
  ...
  db::local_processor<db::Edge, db::Edge, db::Edge> proc(
      const_cast<db::Layout *>(&edges.layout()),
      const_cast<db::Cell *>(&edges.initial_cell()), &other_edges.layout(),
      &other_edges.initial_cell());
  proc.set_base_verbosity(base_verbosity());
  proc.set_threads(edges.store()->threads());
...
std::pair<EdgesDelegate *, EdgesDelegate *>
DeepEdges::in_and_out(const Edges &other) const {
  std::unique_ptr<db::DeepEdges> dr_holder;
  const db::DeepEdges *other_deep =
      dynamic_cast<const db::DeepEdges *>(other.delegate());
  if (!other_deep) {
...
public:
  ...
  do_compute_local(db::Layout * /*layout*/,
    ...
    edge2edge_check_for_edges<std::unordered_set<db::EdgePair>> edge_check(
        m_check, result, m_has_other);
    ...
    scanner.process(edge_check, m_check.distance(),
                    db::box_convert<db::Edge>());
  }
...
EdgePairsDelegate *
DeepEdges::run_check(db::edge_relation_type rel, const Edges *other,
                     db::Coord d, const db::EdgesCheckOptions &options) const {
  ...
  std::unique_ptr<db::DeepEdgePairs> res(
      new db::DeepEdgePairs(edges.derived()));

  db::EdgesCheckLocalOperation op(check, other_deep != 0);

  db::local_processor<db::Edge, db::Edge, db::EdgePair> proc(
      const_cast<db::Layout *>(&edges.layout()),
      const_cast<db::Cell *>(&edges.initial_cell()),
      other_deep ? &other_deep->deep_layer().layout()
                 : const_cast<db::Layout *>(&edges.layout()),
      other_deep ? &other_deep->deep_layer().initial_cell()
                 : const_cast<db::Cell *>(&edges.initial_cell()));
  ...
  proc.run(&op, edges.layer(),
           other_deep ? other_deep->deep_layer().layer() : edges.layer(),
