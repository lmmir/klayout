 ...
 *
 *  This delegate is supposed to provide the mapping of devices to parametrized
 * SPICE subcircuits. It is generic, so it can be used for other cases of device
 * mapping.
 */
class DB_PUBLIC NetlistSpiceWriterDelegate : public tl::Object {
public:
  ...
  std::string format_terminals(
      const db::Device &dev,
      size_t max_terminals = std::numeric_limits<size_t>::max()) const;
  std::string
  format_params(const db::Device &dev,
                size_t without_id = std::numeric_limits<size_t>::max(),
                bool only_primary = false) const;
 ...
 */
class DB_PUBLIC NetlistSpiceWriter : public NetlistWriter {
public:
  ...
  bool use_net_names() const { return m_use_net_names; }

  void set_with_comments(bool f);
  bool with_comments() const { return m_with_comments; }
