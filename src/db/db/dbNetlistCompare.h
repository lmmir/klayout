...
public:
   ...
   */
  virtual void begin_netlist(const db::Netlist * /*a*/,
                             const db::Netlist * /*b*/) {}

  /**
   *  @brief End logging for netlist a and b
   */
  virtual void end_netlist(const db::Netlist * /*a*/,
                           const db::Netlist * /*b*/) {}
   ...
   */
  virtual void
  device_class_mismatch(const db::DeviceClass * /*a*/,
   ...
   */
  virtual void begin_circuit(const db::Circuit * /*a*/,
                             const db::Circuit * /*b*/) {}
   ...
   */
  virtual void circuit_skipped(const db::Circuit * /*a*/,
                               const db::Circuit * /*b*/,
                               const std::string & /*msg*/ = std::string()) {}
   ...
   *  "a" is null if there is no match for b and vice versa.
   */
  virtual void circuit_mismatch(const db::Circuit * /*a*/,
                                const db::Circuit * /*b*/,
                                const std::string & /*msg*/ = std::string()) {}
   ...
   *  Other nets might also match with a and also with b. Matching this a and b
   * is an arbitrary decision.
   */
  virtual void
  match_ambiguous_nets(const db::Net * /*a*/, const db::Net * /*b*/,
                       const std::string & /*msg*/ = std::string()) {}
   ...
   */
  virtual void match_devices(const db::Device * /*a*/,
                             const db::Device * /*b*/) {}
   ...
   */
  virtual void
  match_devices_with_different_parameters(const db::Device * /*a*/,
                                          const db::Device * /*b*/) {}
   ...
   *  "a" is null if there is no match for b and vice versa.
   */
  virtual void device_mismatch(const db::Device * /*a*/,
                               const db::Device * /*b*/,
                               const std::string & /*msg*/ = std::string()) {}
   ...
   */
  virtual void
  subcircuit_mismatch(const db::SubCircuit * /*a*/,
                      const db::SubCircuit * /*b*/,
                      const std::string & /*msg*/ = std::string()) {}
...
public:
   ...
   *  net nb in netlist b.
   *  By default nets are not identical expect through their topology.
   *  This version allows mapping one net to a null net because the circuits are
   * explicitly specified.
   */
  void same_nets(const db::Circuit *ca, const db::Circuit *cb,
                 const db::Net *na, const db::Net *nb, bool must_match);

  /**
   *  @brief Mark two pins as equivalent (i.e. can be swapped)
   *
   *  Only circuits from the *second* input can be given swappable pins.
   *  This will imply the same swappable pins on the equivalent circuit of the
   * first input. To mark multiple pins as swappable, use the version that takes
   * a list of pins.
   */
   ...
   * first input.
   */
  void equivalent_pins(const db::Circuit *cb,
   ...
   */
  void same_device_classes(const db::DeviceClass *ca,
   ...
   */
  void set_max_depth(size_t n) { m_max_depth = n; }

  /**
   *  @brief Gets the maximum search depth
   */
  size_t max_depth() const { return m_max_depth; }
   ...
   */
  void set_with_log(bool f) { m_with_log = f; }

  /**
   *  @brief Gets a value indicating that log messages are generated
   */
  bool with_log() const { return m_with_log; }

  /**
   *  @brief Sets a value indicating whether not to consider net names
   *  This feature is mainly intended for testing.
   */
  void set_dont_consider_net_names(bool f) { m_dont_consider_net_names = f; }

  /**
   *  @brief Gets a value indicating whether not to consider net names
   */
  bool dont_consider_net_names() const { return m_dont_consider_net_names; }

  /**
   *  @brief Sets the maximum branch complexity
   *
   *  This value limits the maximum branch complexity of the backtracking
   * algorithm. The complexity is the accumulated number of branch options with
   * ambiguous net matches. Backtracking will stop when the maximum number of
   * options has been exceeded. As the computational complexity is the square of
   * the branch count, this value should be adjusted carefully.
   */
  void set_max_branch_complexity(size_t n) { m_max_n_branch = n; }

  /**
   *  @brief Gets the maximum branch complexity
   */
  size_t max_branch_complexity() const { return m_max_n_branch; }

  /**
   *  @brief Sets a value indicating depth-first traversal
   *
   *  With depth first (the default), the algorithm looks for further identities
   * before moving to another node. With breadth first (false), the algorithm
   * will work in "waves" rather than digging deeply into the direction of a
   * node.
   */
  void set_depth_first(bool df) { m_depth_first = df; }

  /**
   *  @brief Gets a value indicating depth-first traversal
   */
  bool depth_first() const { return m_depth_first; }

  /**
   *  @brief Gets the list of circuits without matching circuit in the other
   * netlist The result can be used to flatten these circuits prior to compare.
   *  Mismatching top level circuits are not reported because they cannot be
   * flattened.
   */
  void unmatched_circuits(db::Netlist *a, db::Netlist *b,
                          std::vector<db::Circuit *> &in_a,
   ...
   */
  bool compare(const db::Netlist *a, const db::Netlist *b,
               db::NetlistCompareLogger *logger) const;
   ...
   *  This will simplify the circuit and can be applied before device
   * combination to render a schematic-equivalent netlist in some cases (split
   * gate option).
   *
...
protected:
  ...
  bool compare_circuits(
      ...
      const;
  bool all_subcircuits_verified(
      const db::Circuit *c,
      const std::set<const db::Circuit *> &verified_circuits) const;
  std::string generate_subcircuits_not_verified_warning(
      const db::Circuit *ca,
      const std::set<const db::Circuit *> &verified_circuits_a,
      const db::Circuit *cb,
      const std::set<const db::Circuit *> &verified_circuits_b) const;
  static void derive_pin_equivalence(const db::Circuit *ca,
                                     const db::Circuit *cb,
                                     CircuitPinCategorizer *circuit_pin_mapper);
  void do_pin_assignment(
      const db::Circuit *c1, const db::NetGraph &g1, const db::Circuit *c2,
      const db::NetGraph &g2,
      std::map<const db::Circuit *, CircuitMapper> &c12_circuit_and_pin_mapping,
      std::map<const db::Circuit *, CircuitMapper> &c22_circuit_and_pin_mapping,
      bool &pin_mismatch, bool &good) const;
  void do_device_assignment(const db::Circuit *c1, const db::NetGraph &g1,
                            const db::Circuit *c2, const db::NetGraph &g2,
                            const db::DeviceFilter &device_filter,
                            DeviceCategorizer &device_categorizer,
                            db::DeviceEquivalenceTracker &device_eq,
                            bool &good) const;
  void do_subcircuit_assignment(
      const db::Circuit *c1, const db::NetGraph &g1, const db::Circuit *c2,
      const db::NetGraph &g2, CircuitCategorizer &circuit_categorizer,
      const db::CircuitPinCategorizer &circuit_pin_mapper,
      std::map<const db::Circuit *, CircuitMapper> &c12_circuit_and_pin_mapping,
      std::map<const db::Circuit *, CircuitMapper> &c22_circuit_and_pin_mapping,
      db::SubCircuitEquivalenceTracker &subcircuit_eq, bool &good) const;
  bool handle_pin_mismatch(const NetGraph &g1, const db::Circuit *c1,
                           const db::Pin *pin1, const NetGraph &g2,
                           const db::Circuit *c2, const db::Pin *p2) const;
