...
static std::map<std::string, tl::weak_collection<ColdProxy> *>
    s_proxies_per_library_name;
...
ColdProxy::cold_proxies_per_lib_name(const std::string &libname) {
  ...
  std::map<std::string, tl::weak_collection<ColdProxy> *>::const_iterator i =
      s_proxies_per_library_name.find(libname);
...
ColdProxy::ColdProxy(db::cell_index_type ci, db::Layout &layout,
                     const LayoutOrCellContextInfo &info)
    : Cell(ci, layout), mp_context_info(new LayoutOrCellContextInfo(info)) {
  if (!info.lib_name.empty()) {
    tl::MutexLocker locker(&s_map_mutex);
    std::map<std::string, tl::weak_collection<ColdProxy> *>::iterator i =
        s_proxies_per_library_name.find(info.lib_name);
    if (i == s_proxies_per_library_name.end()) {
      i = s_proxies_per_library_name
              .insert(std::make_pair(info.lib_name,
                                     new tl::weak_collection<ColdProxy>()))
              .first;
    }
...
Cell *ColdProxy::clone(Layout &layout) const {
...
std::string ColdProxy::get_basic_name() const {
...
std::string ColdProxy::get_display_name() const {
...
std::string ColdProxy::get_qualified_name() const {
  if (!mp_context_info->lib_name.empty()) {
    std::string stem = "<defunct>" + mp_context_info->lib_name + ".";
    if (!mp_context_info->pcell_name.empty()) {
