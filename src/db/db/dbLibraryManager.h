 ...
 *
 *  The LibraryManager implements tl::Observed and delivers a signal when the
...
public:
   ...
   */
  iterator begin() const { return m_lib_by_name.begin(); }

  /**
   *  @brief End iterator for the name/id pairs of the libraries registered.
   */
  iterator end() const { return m_lib_by_name.end(); }

  /**
   *  @brief Get the library by name which is valid for all given technologies
   *
   *  This method looks up a library which is valid for all technologies listed
   * in "for_technologies". It may be responsible for more than that too.
   *
   *  @return A pair, the boolean is true, if the name is valid. The second
   * member is the library id.
   */
  std::pair<bool, lib_id_type>
  lib_by_name(const std::string &name,
              const std::set<std::string> &for_technologies) const;

  /**
   *  @brief Get the library by name which is valid for the given technology
   *
   *  This method looks up a library which is valid for the given technology. It
   * may be responsible for more than that too.
   *
   *  @return A pair, the boolean is true, if the name is valid. The second
   * member is the library id.
   */
  std::pair<bool, lib_id_type>
  lib_by_name(const std::string &name,
              const std::string &for_technology) const {
   ...
   *
   *  @return The pointer to the library or 0, if there is no library with that
   * name.
   */
   ...
   * name.
   */
  Library *lib_ptr_by_name(const std::string &name,
                           const std::string &for_technology) const {
   ...
   */
  Library *
  lib_ptr_by_name(const std::string &name,
                  const std::set<std::string> &for_technologies) const {
   ...
   *  If a library with that name already exists, the existing library will
   * still be available by id, but not by name.
   *
   ...
   *
   *  This will release the library from the manager's control and lifetime
   * management.
   */
