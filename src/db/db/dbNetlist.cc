...
Netlist::Netlist(NetlistManipulationCallbacks *callbacks)
    : m_case_sensitive(true), mp_callbacks(callbacks), m_valid_topology(false),
      m_lock_count(0),
      m_circuit_by_name(this, &Netlist::begin_circuits, &Netlist::end_circuits),
      m_circuit_by_cell_index(this, &Netlist::begin_circuits,
                              &Netlist::end_circuits),
      m_device_abstract_by_name(this, &Netlist::begin_device_abstracts,
                                &Netlist::end_device_abstracts),
      m_device_abstract_by_cell_index(this, &Netlist::begin_device_abstracts,
                                      &Netlist::end_device_abstracts) {
...
Netlist::Netlist(const Netlist &other)
    : gsi::ObjectBase(other), tl::Object(other), m_valid_topology(false),
      m_lock_count(0),
      m_circuit_by_name(this, &Netlist::begin_circuits, &Netlist::end_circuits),
      m_circuit_by_cell_index(this, &Netlist::begin_circuits,
                              &Netlist::end_circuits),
...
Netlist &Netlist::operator=(const Netlist &other) {
  if (this != &other) {
    ...
    for (const_device_class_iterator dc = other.begin_device_classes();
         dc != other.end_device_classes(); ++dc) {
    ...
    for (const_abstract_model_iterator dm = other.begin_device_abstracts();
         dm != other.end_device_abstracts(); ++dm) {
    ...
    for (const_circuit_iterator i = other.begin_circuits();
         i != other.end_circuits(); ++i) {
...
void Netlist::mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
  ...
  db::mem_stat(stat, purpose, cat, m_device_abstract_by_name, true,
               (void *)this);
  db::mem_stat(stat, purpose, cat, m_device_abstract_by_cell_index, true,
               (void *)this);
}

void Netlist::set_case_sensitive(bool f) { m_case_sensitive = f; }

int Netlist::name_compare(bool case_sensitive, const std::string &n1,
                          const std::string &n2) {
...
void Netlist::validate_topology() {
  ...
  for (circuit_iterator c = begin_circuits(); c != end_circuits(); ++c) {
    ...
    for (Circuit::subcircuit_iterator sc = c->begin_subcircuits();
    ...
    for (Circuit::refs_iterator sc = c->begin_refs(); sc != c->end_refs();
         ++sc) {
      if (sc->circuit()) {
  ...
  while (m_top_down_circuits.size() != max_index) {
    ...
    for (tl::vector<Circuit *>::const_iterator ii =
             m_top_down_circuits.begin() + n_top_down_circuits;
         ii != m_top_down_circuits.end(); ++ii) {
      const tl::vector<Circuit *> &cc = m_child_circuits[(*ii)->index()];
      for (tl::vector<Circuit *>::const_iterator icc = cc.begin();
           icc != cc.end(); ++icc) {
        tl_assert(num_parents[(*icc)->index()] !=
    ...
    if (n_top_down_circuits == m_top_down_circuits.size()) {
      throw tl::Exception(
          tl::to_string(tr("Recursive hierarchy detected in netlist")));
    }

    //  doing this reverse will mean we preserve bottom-up order. This is useful
    //  for netlists where subcircuits have to be defined before they are used.
    std::reverse(m_top_down_circuits.begin() + n_top_down_circuits,
                 m_top_down_circuits.end());
  }

  //  Determine the number of top cells
  for (tl::vector<Circuit *>::const_iterator e = m_top_down_circuits.begin();
       e != m_top_down_circuits.end() &&
       m_parent_circuits[(*e)->index()].empty();
       ++e) {
    ++m_top_circuits;
  }
...
Netlist::const_top_down_circuit_iterator Netlist::begin_top_down() const {
  ...
  return reinterpret_cast<const tl::vector<const Circuit *> &>(
             m_top_down_circuits)
      .begin();
}
...
void Netlist::add_circuit(Circuit *circuit) {
  ...
  if (circuit->netlist()) {
    throw tl::Exception(
        tl::to_string(tr("Circuit already contained in a netlist")));
  }
...
void Netlist::flatten_circuits(const std::vector<Circuit *> &circuits) {
  ...
  //  Before flatten, we sort top-down. This optimizes for the case of
  //  flattening away some hierarchy above a certain circuit.
  for (top_down_circuit_iterator c = begin_top_down(); c != end_top_down();
       ++c) {
  ...
  for (std::vector<Circuit *>::const_iterator c = to_flatten.begin();
       c != to_flatten.end(); ++c) {
    flatten_circuit(*c);
  }
}
...
void Netlist::flatten_circuit(Circuit *circuit) {
  ...
  for (db::Circuit::refs_iterator sc = circuit->begin_refs();
       sc != circuit->end_refs(); ++sc) {
    refs.push_back(sc.operator->());
  }

  for (std::vector<db::SubCircuit *>::const_iterator r = refs.begin();
       r != refs.end(); ++r) {
...
void Netlist::flatten() {
  ...
  for (db::Netlist::top_down_circuit_iterator tc = begin_top_down();
       tc != end_top_down() && ntop > 0; ++tc) {
    top_circuits.insert(tc.operator->());
    --ntop;
  }

  for (db::Netlist::bottom_up_circuit_iterator c = begin_bottom_up();
       c != end_bottom_up(); ++c) {
...
DeviceClass *Netlist::device_class_by_name(const std::string &name) {
  ...
  for (device_class_iterator d = begin_device_classes();
       d != end_device_classes(); ++d) {
    if (d->name() == nn) {
      return d.operator->();
    }
  }
...
void Netlist::add_device_class(DeviceClass *device_class) {
  ...
  if (device_class->netlist()) {
    throw tl::Exception(
        tl::to_string(tr("Device class already contained in a netlist")));
  }
...
void Netlist::remove_device_class(DeviceClass *device_class) {
  ...
  if (device_class->netlist() != this) {
    throw tl::Exception(
        tl::to_string(tr("Device class not within given netlist")));
  }
...
void Netlist::add_device_abstract(DeviceAbstract *device_abstract) {
  ...
  if (device_abstract->netlist()) {
    throw tl::Exception(
        tl::to_string(tr("Device abstract already contained in a netlist")));
  }
...
void Netlist::remove_device_abstract(DeviceAbstract *device_abstract) {
  ...
  if (device_abstract->netlist() != this) {
    throw tl::Exception(
        tl::to_string(tr("Device abstract not within given netlist")));
  }
...
void Netlist::purge_nets() {
  for (bottom_up_circuit_iterator c = begin_bottom_up(); c != end_bottom_up();
       ++c) {
    c->purge_nets();
  }
}
...
void Netlist::make_top_level_pins() {
  size_t ntop = top_circuit_count();
  for (top_down_circuit_iterator c = begin_top_down();
       c != end_top_down() && ntop > 0; ++c, --ntop) {
    ...
    if (circuit->pin_count() == 0) {
      ...
      for (Circuit::net_iterator n = circuit->begin_nets();
           n != circuit->end_nets(); ++n) {
        if (!n->name().empty() &&
            n->terminal_count() + n->subcircuit_pin_count() > 0) {
...
void Netlist::purge() {
  //  This locking is very important as we do not want to recompute the
  //  bottom-up list while iterating.
  ...
  for (bottom_up_circuit_iterator c = begin_bottom_up(); c != end_bottom_up();
    ...
    for (db::Circuit::net_iterator n = circuit->begin_nets();
         n != circuit->end_nets() && purge_candidate; ++n) {
      purge_candidate = n->is_passive();
    }

    if (purge_candidate) {

      //  No nets left: delete the subcircuits that refer to us and finally
      //  delete the circuit
...
static std::string pin2string(const db::Pin &pin) {
  if (pin.name().empty()) {
    //  the pin ID is zero-based and essentially the index, so we add 1 to make
    //  it compliant with the other IDs
...
std::string Netlist::to_string() const {
  std::string res;
  for (db::Netlist::const_circuit_iterator c = begin_circuits();
       c != end_circuits(); ++c) {

    std::string ps;
    for (db::Circuit::const_pin_iterator p = c->begin_pins();
         p != c->end_pins(); ++p) {
      if (!ps.empty()) {
        ps += ",";
      }
    ...
    res += std::string("circuit ") + tl::to_word_or_quoted_string(c->name()) +
           " (" + ps + ");\n";
    ...
    for (db::Circuit::const_device_iterator d = c->begin_devices();
      ...
      const std::vector<db::DeviceParameterDefinition> &pd =
          d->device_class()->parameter_definitions();
      for (std::vector<db::DeviceParameterDefinition>::const_iterator p =
        ...
        ps +=
            p->name() + "=" + tl::sprintf("%.12g", d->parameter_value(p->id()));
      }
      res += std::string("  device ") +
             tl::to_word_or_quoted_string(d->device_class()->name()) + " " +
             device2string(*d) + " (" + ts + ") (" + ps + ");\n";
    }
    ...
    for (db::Circuit::const_subcircuit_iterator sc = c->begin_subcircuits();
      ...
      if (circuit) {
        for (db::Circuit::const_pin_iterator p = circuit->begin_pins();
          ...
          ps += pin2string(*p) + "=" +
                net2string(subcircuit.net_for_pin(p->id()));
        }
        res += std::string("  subcircuit ") +
               tl::to_word_or_quoted_string(circuit->name()) + " " +
               subcircuit2string(*sc) + " (" + ps + ");\n";
      } else {
...
static db::Net *read_net(tl::Extractor &ex, db::Circuit *circuit,
...
static void read_pins(tl::Extractor &ex, db::Circuit *circuit,
                      std::map<std::string, db::Net *> &n2n) {
  ...
  while (!ex.test(")")) {
    ...
    if (circuit->pin_count() < org_pins.size() &&
        pn != org_pins[circuit->pin_count()]) {
      ex.error(tl::sprintf(
          tl::to_string(tr("Circuit defines different name for pin than "
                           "subcircuit: %s (circuit) vs. %s (subcircuit)")),
          pn, org_pins[circuit->pin_count()]));
    }
...
static void read_device_terminals(tl::Extractor &ex, db::Device *device,
                                  std::map<std::string, db::Net *> &n2n) {
  ex.expect("(");
  while (!ex.test(")")) {
...
static void read_device(tl::Extractor &ex, db::Circuit *circuit,
                        std::map<std::string, db::Net *> &n2n) {
  ...
  db::DeviceClass *dc = 0;
  for (db::Netlist::device_class_iterator i = netlist->begin_device_classes();
       i != netlist->end_device_classes(); ++i) {
    if (i->name() == dcn) {
      dc = i.operator->();
    }
  }
...
static void read_subcircuit_pins(tl::Extractor &ex, db::Circuit *circuit,
                                 db::SubCircuit *subcircuit,
                                 std::map<std::string, db::Net *> &n2n) {
...
static void read_subcircuit(tl::Extractor &ex, db::Circuit *circuit,
                            std::map<std::string, db::Net *> &n2n,
