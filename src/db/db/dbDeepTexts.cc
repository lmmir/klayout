 ...
 */
class DB_PUBLIC DeepTextsIterator : public TextsIteratorDelegate {
public:
  ...
  virtual const value_type *get() const { return &m_text; }
...
DeepTexts::DeepTexts() : MutableTexts() {
  //  .. nothing yet ..
}
...
DeepTexts &DeepTexts::operator=(const DeepTexts &other) {
  if (this != &other) {
...
DeepTexts::DeepTexts(const DeepLayer &dl) : MutableTexts() {
  set_deep_layer(dl);
}
...
TextsDelegate *DeepTexts::clone() const { return new DeepTexts(*this); }
...
void DeepTexts::do_insert(const db::Text &text) {
  ...
  if (layout.begin_top_down() != layout.end_top_down()) {
    ...
    top_cell.shapes(deep_layer().layer())
        .insert(db::TextRef(text, layout.shape_repository()));
  }
...
static void transform_deep_layer(db::DeepLayer &deep_layer, const Trans &t) {
  //  TODO: this is a pretty cheap implementation. At least a plain move can be
  //  done with orientation variants.
...
void DeepTexts::flatten() {
  ...
  if (layout.begin_top_down() != layout.end_top_down()) {
    ...
    for (db::RecursiveShapeIterator iter(layout, top_cell,
      ...
      db::Text text;
      iter->text(text);
      flat_shapes.insert(db::TextRef(text.transformed(iter.trans()),
...
bool DeepTexts::empty() const { return begin_iter().first.at_end(); }

const db::Text *DeepTexts::nth(size_t) const {
  throw tl::Exception(tl::to_string(tr(
      "Random access to texts is available only for flat text collections")));
}

bool DeepTexts::has_valid_texts() const { return false; }

const db::RecursiveShapeIterator *DeepTexts::iter() const { return 0; }
...
TextsDelegate *DeepTexts::add_in_place(const Texts &other) {
  if (other.empty()) {
    return this;
  }

  const DeepTexts *other_deep =
      dynamic_cast<const DeepTexts *>(other.delegate());
  if (other_deep) {
...
RegionDelegate *DeepTexts::processed_to_polygons(
    const TextToPolygonProcessorBase &filter) const {
  return shape_collection_processed_impl<db::Text, db::Polygon, db::DeepRegion>(
      deep_layer(), filter);
}
...
RegionDelegate *DeepTexts::polygons(db::Coord e) const {
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator s =
             c->shapes(deep_layer().layer()).begin(db::ShapeIterator::Texts);
         !s.at_end(); ++s) {
      db::Box box = s->bbox();
...
bool DeepTexts::equals(const Texts &other) const {
  const DeepTexts *other_delegate =
      dynamic_cast<const DeepTexts *>(other.delegate());
  if (other_delegate &&
...
void DeepTexts::insert_into(Layout *layout, db::cell_index_type into_cell,
                            unsigned int into_layer) const {
  deep_layer().insert_into(layout, into_cell, into_layer);
}

void DeepTexts::insert_into_as_polygons(Layout *layout,
...
public:
  Text2PolygonInteractingLocalOperation(bool inverse) : m_inverse(inverse) {
    //  .. nothing yet ..
  }
  ...
  virtual void do_compute_local(
      db::Layout * /*layout*/,
      const shape_interactions<db::TextRef, db::PolygonRef> &interactions,
      std::vector<std::unordered_set<db::TextRef>> &results,
    ...
    for (shape_interactions<db::TextRef, db::PolygonRef>::iterator i =
             interactions.begin();
         i != interactions.end(); ++i) {
      for (shape_interactions<db::TextRef, db::PolygonRef>::iterator2 j =
               i->second.begin();
           j != i->second.end(); ++j) {
    ...
    if (m_inverse) {
      ...
      text_to_region_interaction_filter<std::unordered_set<db::TextRef>,
                                        db::TextRef>
          filter(interacting);
      scanner.process(filter, 1, db::box_convert<db::TextRef>(),
...
public:
  ...
  virtual void do_compute_local(
    ...
    text_to_region_interaction_filter<ResultInserter, db::TextRef> filter(
        inserter);
    scanner.process(filter, 1, db::box_convert<db::TextRef>(),
                    db::box_convert<db::Polygon>());
  }
...
TextsDelegate *DeepTexts::selected_interacting_generic(const Region &other,
                                                       bool inverse) const {
  ...
  db::local_processor<db::TextRef, db::PolygonRef, db::TextRef> proc(
      const_cast<db::Layout *>(&texts.layout()),
      const_cast<db::Cell *>(&texts.initial_cell()),
  ...
  proc.run(&op, texts.layer(), other_deep->deep_layer().layer(),
           dl_out.layer());
...
RegionDelegate *DeepTexts::pull_generic(const Region &other) const {
  ...
  db::local_processor<db::TextRef, db::PolygonRef, db::PolygonRef> proc(
      const_cast<db::Layout *>(&texts.layout()),
      const_cast<db::Cell *>(&texts.initial_cell()), &other_polygons.layout(),
