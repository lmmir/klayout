 ...
 */
class DB_PUBLIC AsIfFlatEdges : public EdgesDelegate {
public:
  AsIfFlatEdges();
  ...
  width_check(db::Coord d, const db::EdgesCheckOptions &options) const {
    return run_check(db::WidthRelation, 0, d, options);
  }

  virtual EdgePairsDelegate *
  space_check(db::Coord d, const db::EdgesCheckOptions &options) const {
  ...
  virtual EdgePairsDelegate *
  processed_to_edge_pairs(const EdgeToEdgePairProcessorBase &) const;
  virtual RegionDelegate *
  processed_to_polygons(const EdgeToPolygonProcessorBase &) const;
  ...
  virtual EdgesDelegate *merged_in_place() { return merged(); }

  virtual EdgesDelegate *merged() const { return boolean(0, EdgeOr); }
  ...
  andnot_with(const Edges &other) const {
    return boolean_andnot(&other);
  }

  virtual EdgesDelegate *and_with(const Region &other) const {
    return edge_region_op(other, db::EdgePolygonOp::Inside,
                          true /*include borders*/)
        .first;
  }

  virtual EdgesDelegate *not_with(const Region &other) const {
    return edge_region_op(other, db::EdgePolygonOp::Outside,
                          true /*include borders*/)
        .first;
  }

  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  andnot_with(const Region &other) const {
    return edge_region_op(other, db::EdgePolygonOp::Both,
                          true /*include borders*/);
  }
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  inside_outside_part_pair(const Region &other) const {
    return edge_region_op(other, db::EdgePolygonOp::Both,
                          false /*don't include borders*/);
  }
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Region &other) const;
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Edges &other) const;
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_outside_pair(const Edges &other) const;
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_inside_pair(const Edges &other) const;
...
protected:
  ...
  EdgePairsDelegate *run_check(db::edge_relation_type rel, const Edges *other,
                               db::Coord d,
                               const EdgesCheckOptions &options) const;
  virtual EdgesDelegate *pull_generic(const Edges &edges) const;
  virtual RegionDelegate *pull_generic(const Region &region) const;
  virtual EdgesDelegate *selected_interacting_generic(const Edges &edges,
                                                      EdgeInteractionMode mode,
                                                      bool inverse) const;
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair_generic(const Edges &edges,
                                    EdgeInteractionMode mode) const;
  virtual EdgesDelegate *selected_interacting_generic(const Region &region,
                                                      EdgeInteractionMode mode,
                                                      bool inverse) const;
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair_generic(const Region &region,
                                    EdgeInteractionMode mode) const;
...
private:
  ...
  boolean_andnot(const Edges *other) const;
  std::pair<EdgesDelegate *, EdgesDelegate *>
  edge_region_op(const Region &other, db::EdgePolygonOp::mode_t mode,
