...
template <class C> struct vector_defs {
  ...
  static C *new_xy(coord_type x, coord_type y) { return new C(x, y); }

  static db::point<coord_type> to_point(const C *p) {
    return db::point<coord_type>() + *p;
  }

  static C scale(const C *p, double s) { return C(*p * s); }

  static C divide(const C *p, double s) { return C(*p / s); }
  ...
  static C negate(const C *p) { return -*p; }
  ...
  static size_t hash_value(const C *v) { return std::hfunc(*v); }

  static db::point<coord_type> add_with_point(const C *v,
                                              const db::point<coord_type> &p) {
    return p + *v;
  }

  static gsi::Methods methods() {
    return constructor("new", &new_v,
                       "@brief Default constructor: creates a null vector with "
                       "coordinates (0,0)") +
           constructor(
               "new", &new_point, gsi::arg("p"),
               "@brief Default constructor: creates a vector from a point\n"
               "This constructor is equivalent to computing p-point(0,0).\n"
               "This method has been introduced in version 0.25.") +
           constructor(
               "new", &new_xy, gsi::arg("x"), gsi::arg("y"),
               "@brief Constructor for a vector from two coordinate values\n"
               "\n") +
           method_ext("to_p", &to_point,
                      "@brief Turns the vector into a point\n"
                      "This method returns the point resulting from adding the "
                      "vector to (0,0)."
                      "\n"
                      "This method has been introduced in version 0.25.") +
           method_ext("-@", &negate,
           ...
           method("-", (C(C::*)(const C &) const) & C::subtract, gsi::arg("v"),
                  ...
                  "\n"
                  "\n"
                  "Subtract vector v from self by subtracting the "
                  "coordinates.\n") +
           ...
           method_ext("hash", &hash_value,
                      ...
                      "Returns a hash value for the given vector. This method "
                      "enables vectors as hash keys.\n"
                      "\n"
                      "This method has been introduced in version 0.25.\n") +
           method("x", &C::x, "@brief Accessor to the x coordinate\n") +
           method("y", &C::y, "@brief Accessor to the y coordinate\n") +
           ...
           method_ext("*=", &iscale, gsi::arg("f"),
                      ...
                      "\n"
                      "\n"
                      "Scales object in place. All coordinates are multiplied "
                      "with the given factor and if "
                      "necessary rounded.") +
           method_ext("/", &divide, gsi::arg("d"),
                      "@brief Division by some divisor\n"
                      "\n"
                      "\n"
                      "Returns the scaled object. All coordinates are divided "
                      "with the given divisor and if "
                      "necessary rounded.") +
           method_ext("/=", &idiv, gsi::arg("d"),
                      "@brief Division by some divisor\n"
                      "\n"
                      "\n"
                      "Divides the object in place. All coordinates are "
                      "divided with the given divisor and if "
                      "necessary rounded.") +
           method_ext("vprod", &vprod, gsi::arg("v"),
                      "@brief Computes the vector product between self and the "
                      "given vector\n"
                      "\n"
                      "\n"
                      "The vector product of a and b is defined as: vp = "
                      "ax*by-ay*bx.\n") +
           method_ext(
               "vprod_sign", &vprod_sign, gsi::arg("v"),
               "@brief Computes the vector product between self and the given "
               "vector and returns a value indicating the sign of the product\n"
               "\n"
               "\n"
               "@return 1 if the vector product is positive, 0 if it is zero "
               "and -1 if it is negative.\n") +
           method_ext("sprod", &sprod, gsi::arg("v"),
                      "@brief Computes the scalar product between self and the "
                      "given vector\n"
                      "\n"
                      "\n"
                      "The scalar product of a and b is defined as: vp = "
                      "ax*bx+ay*by.\n") +
           method_ext("*", &sprod, gsi::arg("v"),
                      ...
                      "ax*bx+ay*by.\n") +
           method_ext(
               "sprod_sign", &sprod_sign, gsi::arg("v"),
               "@brief Computes the scalar product between self and the given "
               "vector and returns a value indicating the sign of the product\n"
               "\n"
               "\n"
               "@return 1 if the scalar product is positive, 0 if it is zero "
               "and -1 if it is negative.\n") +
           method("length|abs", (double (C::*)() const) & C::double_length,
                  "@brief Returns the length of the vector\n"
                  "'abs' is an alias provided for compatibility with the "
                  "former point type.") +
           method("sq_length|sq_abs",
                  (double (C::*)() const) & C::sq_double_length,
                  "@brief The square length of the vector\n"
                  "'sq_abs' is an alias provided for compatibility with the "
                  "former point type.") +
...
Class<db::DVector> decl_DVector(
    "db", "DVector",
    constructor("new", &dvector_from_ivector, gsi::arg("vector"),
                "@brief Creates a floating-point coordinate vector from an "
        ...
        method_ext("to_itype", &dvector_to_vector, gsi::arg("dbu", 1.0),
                   ...
                   "vector in micron units to an integer-coordinate vector in "
                   "database units. The vector's' "
    ...
    "A vector is a distance in cartesian, 2 dimensional space. A vector is "
    "given by two coordinates (x and y) and represents "
    "the distance between two points. Being the distance, transformations act "
    "differently on vectors: the displacement is not applied. "
    "\n"
    "Vectors are not geometrical objects by itself. But they are frequently "
    "used in the database API "
    "for various purposes. Other than the integer variant (\\Vector), points "
    "with floating-point coordinates can represent fractions of "
...
Class<db::Vector> decl_Vector(
    "db", "Vector",
    constructor("new", &vector_from_dvector, gsi::arg("dvector"),
                "@brief Creates an integer coordinate vector from a "
                "floating-point coordinate vector\n") +
        method_ext(
            "to_dtype", &vector_to_dvector, gsi::arg("dbu", 1.0),
            "@brief Converts the vector to a floating-point coordinate vector"
            "\n"
            "The database unit can be specified to translate the "
            "integer-coordinate vector into a floating-point coordinate "
            "vector in micron units. The database unit is basically a scaling "
            "factor.\n") +
