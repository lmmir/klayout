 ...
 */
class DB_PUBLIC FlatEdges : public MutableEdges {
public:
  ...
  typedef db::layer<db::EdgeWithProperties, db::unstable_layer_tag>
      edge_layer_wp_type;
  ...
  EdgesDelegate *clone() const { return new FlatEdges(*this); }
  ...
  void do_transform(const db::IMatrix2d &t) { transform_generic(t); }

  void do_transform(const db::IMatrix3d &t) { transform_generic(t); }
...
private:
  ...
  template <class Trans> void transform_generic(const Trans &trans) {
    if (!trans.is_unity()) {
      ...
      for (edge_iterator_type p =
               e.template get_layer<db::Edge, db::unstable_layer_tag>().begin();
           p != e.get_layer<db::Edge, db::unstable_layer_tag>().end(); ++p) {
        e.get_layer<db::Edge, db::unstable_layer_tag>().replace(
            p, p->transformed(trans));
      }
      for (edge_iterator_wp_type p =
               e.template get_layer<db::EdgeWithProperties,
