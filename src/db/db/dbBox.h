...
template <class C, class R> struct DB_PUBLIC_TEMPLATE box {
  ...
  box(const point<C> &p1, const point<C> &p2)
      : m_p1(p1.x() < p2.x() ? p1.x() : p2.x(),
             p1.y() < p2.y() ? p1.y() : p2.y()),
        m_p2(p2.x() > p1.x() ? p2.x() : p1.x(),
             p2.y() > p1.y() ? p2.y() : p1.y()) {
    //  .. nothing else ..
  }
  ...
  explicit box(const box<D, DR> &b) : m_p1(b.p1()), m_p2(b.p2()) {
    //  .. nothing else ..
  }
   ...
   *
   *  Hint: this box is likely to be somewhat misfunctional. It cannot be
   * transformed well for example.
   */
  static box world() {
    return box(std::numeric_limits<C>::min(), std::numeric_limits<C>::min(),
               std::numeric_limits<C>::max(), std::numeric_limits<C>::max());
  }

  /**
   *  @brief The (dummy) translation operator
   */
  void translate(const box<C, R> &d, db::generic_repository<C> &,
                 db::ArrayRepository &);
   ...
   */
  template <class T>
  void translate(const box<C, R> &d, const T &t, db::generic_repository<C> &,
                 db::ArrayRepository &);
   ...
   *  @brief Fuzzy comparison of boxes
   */
  bool not_equal(const box_type &b) const { return !equal(b); }
  ...
  /**
   *  @brief A method version for scaling operator* (mainly for automation
   * purposes)
   */
   ...
   */
  template <class Tr> box<C, R> &transform(const Tr &t);
   ...
   *
   *  @param tr Receives the transformation that must be applied to render the
   * original box
   */
   ...
   *
   *  This method is mainly provided for template argument substitution
   *  of path and polygon objects by boxes. It basically does nothing (like the
   * same methods in path etc.)
   */
   ...
   *
   *  If the left margin gets larger that the right one, the value given will
   * become the right margin
   */
  void set_left(C l);
   ...
   *
   *  If the right margin gets less that the left one, the value given will
   * become the left margin
   */
  void set_right(C r);
   ...
   *
   *  If the bottom margin gets larger that the top one, the value given will
   * become the top margin
   */
  void set_bottom(C b);
   ...
   *
   *  If the top margin gets less that the bottom one, the value given will
   * become the bottom margin
   */
  void set_top(C t);
...
inline void box<C, R>::translate(const box<C, R> &d,
                                 db::generic_repository<C> &,
                                 db::ArrayRepository &) {
  *this = d;
}
...
template <class C, class R>
template <class T>
inline void box<C, R>::translate(const box<C, R> &d, const T &t,
                                 db::generic_repository<C> &,
                                 db::ArrayRepository &) {
  *this = d;
  transform(t);
...
template <class C, class R>
inline bool box<C, R>::operator<(const box<C, R> &b) const {
  return m_p1 < b.m_p1 || (m_p1 == b.m_p1 && m_p2 < b.m_p2);
}

template <class C, class R>
inline bool box<C, R>::less(const box<C, R> &b) const {
  if (!m_p1.equal(b.p1())) {
...
template <class C, class R>
inline bool box<C, R>::equal(const box<C, R> &b) const {
  return m_p1.equal(b.p1()) && m_p2.equal(b.p2());
}

template <class C, class R>
inline box<C, R> &box<C, R>::operator*=(const box<C, R> &b) {
  if (!b.empty() && !empty()) {
...
template <class C, class R> inline box<C, R> box<C, R>::scaled(double s) const {
  return box<C, R>(*this * s);
}

template <class C, class R>
inline box<C, R> box<C, R>::convolved(const box<C, R> &b) const {
  box<C, R> r(*this);
  r *= b;
  return r;
}
...
template <class C, class R>
inline box<C, R> box<C, R>::joined(const box<C, R> &b) const {
  box<C, R> r(*this);
  r += b;
  return r;
}
...
template <class C, class R>
inline box<C, R> &box<C, R>::operator+=(const point<C> &p) {
  if (empty()) {
    m_p1 = point<R>(p);
    m_p2 = point<R>(p);
  } else {
...
template <class C, class R>
inline box<C, R> box<C, R>::intersection(const box<C, R> &b) const {
  box<C, R> r(*this);
  r &= b;
  return r;
}
...
template <class C, class R>
inline box<C, R> box<C, R>::moved(const vector<C> &p) const {
  box<C, R> b(*this);
  b.move(p);
  return b;
}
...
template <class C, class R>
inline box<C, R> box<C, R>::enlarged(const vector<C> &p) const {
  box<C, R> b(*this);
  b.enlarge(p);
  return b;
}
...
template <class C, class R>
template <class Tr>
inline box<C, R> &box<C, R>::transform(const Tr &t) {
  if (!empty()) {
    if (t.is_ortho()) {
...
template <class C, class R>
inline box<C, R> &box<C, R>::move(const vector<C> &p) {
  if (!empty()) {
    m_p1 += p;
    m_p2 += p;
  }
...
template <class C, class R>
inline box<C, R> &box<C, R>::enlarge(const vector<C> &p) {
  if (!empty()) {
    m_p1 -= p;
    m_p2 += p;
  }
...
template <class C, class R> inline const point<R> &box<C, R>::p1() const {
  return m_p1;
}

template <class C, class R> inline const point<R> &box<C, R>::p2() const {
  return m_p2;
}

template <class C, class R>
inline const point<R> &box<C, R>::lower_left() const {
  return m_p1;
}

template <class C, class R>
inline const point<R> &box<C, R>::upper_right() const {
  return m_p2;
}

template <class C, class R> inline point<R> box<C, R>::upper_left() const {
  return point<R>(m_p1.x(), m_p2.y());
}

template <class C, class R> inline point<R> box<C, R>::lower_right() const {
  return point<R>(m_p2.x(), m_p1.y());
}

template <class C, class R> inline bool box<C, R>::is_point() const {
  return m_p1 == m_p2;
}

template <class C, class R> inline void box<C, R>::set_p1(const point<C> &_p1) {
  *this = box_type(_p1, p2());
}

template <class C, class R> inline void box<C, R>::set_p2(const point<C> &_p2) {
  *this = box_type(p1(), _p2);
}

template <class C, class R> inline point<C> box<C, R>::center() const {
  return point<C>(m_p1.x() + width() / 2, m_p1.y() + height() / 2);
}

template <class C, class R>
inline typename box<C, R>::distance_type box<C, R>::width() const {
  return m_p2.x() - m_p1.x();
}

template <class C, class R>
inline typename box<C, R>::distance_type box<C, R>::height() const {
  return m_p2.y() - m_p1.y();
}

template <class C, class R> inline C box<C, R>::left() const {
  return m_p1.x();
}

template <class C, class R> inline C box<C, R>::right() const {
  return m_p2.x();
}

template <class C, class R> inline C box<C, R>::top() const { return m_p2.y(); }

template <class C, class R> inline C box<C, R>::bottom() const {
  return m_p1.y();
}

template <class C, class R> inline void box<C, R>::set_left(C l) {
  if (empty()) {
    *this = box(l, 0, l, 0);
  } else {
...
template <class C, class R> inline void box<C, R>::set_right(C r) {
  if (empty()) {
    *this = box(r, 0, r, 0);
  } else {
...
template <class C, class R> inline void box<C, R>::set_bottom(C b) {
  if (empty()) {
    *this = box(0, b, 0, b);
  } else {
...
template <class C, class R> inline void box<C, R>::set_top(C t) {
  if (empty()) {
    *this = box(0, t, 0, t);
  } else {
...
template <class C, class R> inline bool box<C, R>::empty() const {
  return m_p1.x() > m_p2.x() || m_p1.y() > m_p2.y();
}

template <class C, class R>
inline bool box<C, R>::contains(const point<C> &p) const {
  if (empty()) {
    return false;
  } else {
...
template <class C, class R>
inline bool box<C, R>::inside(const box<C, R> &b) const {
  if (b.empty() || empty()) {
    return false;
  } else {
...
template <class C, class R>
inline bool box<C, R>::touches(const box<C, R> &b) const {
  if (b.empty() || empty()) {
    return false;
  } else {
...
template <class C, class R>
inline bool box<C, R>::overlaps(const box<C, R> &b) const {
  if (b.empty() || empty()) {
    return false;
  } else {
    return (m_p1.x() < b.m_p2.x() && b.m_p1.x() < m_p2.x()) &&
...
template <class C, class R>
inline typename box<C, R>::area_type box<C, R>::area() const {
  if (empty()) {
    return area_type(0);
  } else {
...
template <class C, class R>
inline typename box<C, R>::perimeter_type box<C, R>::perimeter() const {
  if (empty()) {
...
template <class C, class R> inline double box<C, R>::double_area() const {
  if (empty()) {
    return 0.0;
  } else {
...
#endif
{
  typename Box::coord_type operator()(const Box &b) const { return b.left(); }
};
...
#endif
{
  typename Box::coord_type operator()(const Box &b) const { return b.right(); }
};
...
#endif
{
  typename Box::coord_type operator()(const Box &b) const { return b.bottom(); }
};
...
#endif
{
  typename Box::coord_type operator()(const Box &b) const { return b.top(); }
};
...
#endif
{
  bool operator()(const Box &b1, const Box &b2) const { return b1.touches(b2); }
};
 ...
 */
template <class C> inline box<C> operator&(const box<C> &b1, const box<C> &b2) {
  box<C> bb(b1);
  bb &= b2;
  return bb;
}
 ...
 */
template <class C> inline box<double> operator*(const box<C> &b, double s) {
  if (b.empty()) {
    return box<double>();
  } else {
 ...
 *  @return The scaled box
 */
template <class C> inline box<C> operator*(const box<C> &b, long s) {
  if (b.empty()) {
    return box<C>();
  } else {
 ...
 *  @return The scaled box
 */
template <class C> inline box<C> operator*(const box<C> &b, int s) {
  if (b.empty()) {
    return box<C>();
  } else {
    return box<C>(b.p1() * s, b.p2() * s);
 ...
 *  @return The scaled box
 */
template <class C> inline box<C> operator*(const box<C> &b, unsigned int s) {
  if (b.empty()) {
    return box<C>();
  } else {
    return box<C>(b.p1() * s, b.p2() * s);
 ...
 *  @return The scaled box
 */
template <class C> inline box<C> operator*(const box<C> &b, unsigned long s) {
  if (b.empty()) {
    return box<C>();
  } else {
    return box<C>(b.p1() * s, b.p2() * s);
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const box<C> &b) {
...
template <class Box> class box_inserter {
public:
  box_inserter(Box &box) : mp_box(&box) {
    //  .. nothing yet ..
  }
  ...
  template <class Point> box_inserter &operator=(const Point &p) {
    *mp_box += p;
    return *this;
  }
