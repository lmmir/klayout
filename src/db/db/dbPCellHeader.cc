...
bool PCellParametersCompareFunc::operator()(
    const pcell_parameters_type *a, const pcell_parameters_type *b) const {
  if (a->size() != b->size()) {
...
PCellHeader::PCellHeader(size_t pcell_id, const std::string &name,
...
PCellHeader::PCellHeader(const PCellHeader &d)
    : mp_declaration(d.mp_declaration), m_pcell_id(d.m_pcell_id),
      m_name(d.m_name) {
  if (mp_declaration) {
    mp_declaration->add_ref();
  }
...
PCellHeader::get_layer_indices(db::Layout &layout,
  ...
  std::vector<db::PCellLayerDeclaration> layer_declarations =
      mp_declaration->get_layer_declarations(parameters);
...
PCellVariant *
PCellHeader::get_variant(db::Layout & /*layout*/,
