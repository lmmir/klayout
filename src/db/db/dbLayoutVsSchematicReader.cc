...
LayoutVsSchematicStandardReader::LayoutVsSchematicStandardReader(
...
void LayoutVsSchematicStandardReader::do_read_lvs(db::LayoutVsSchematic *lvs) {
  try {
    ...
    throw tl::Exception(tl::sprintf(tl::to_string(tr("%s in line: %d of %s")),
                                    ex.msg(), stream().line_number(), path()));
  }
}
...
void LayoutVsSchematicStandardReader::read_netlist(db::LayoutVsSchematic *lvs) {
  ...
  while (!at_end()) {
    ...
    if (test(skeys::version_key) || test(lkeys::version_key)) {
      ...
      Brace br(this);
      LayoutToNetlistStandardReader::read_netlist(0, lvs, &br,
                                                  &m_map_per_circuit_a);
      br.done();
      ...
      LayoutToNetlistStandardReader::read_netlist(netlist.get(), 0, &br,
                                                  &m_map_per_circuit_b);
      ...
      if (!lvs->reference_netlist() || !lvs->netlist()) {
        throw tl::Exception(tl::to_string(
            tr("xref section before reference or layout netlist")));
      }
...
bool LayoutVsSchematicStandardReader::read_status(
...
bool LayoutVsSchematicStandardReader::read_severity(
    db::NetlistCrossReference::Severity &severity) {
  if (test(skeys::info_severity_key) || test(lkeys::info_severity_key)) {
    severity = db::NetlistCrossReference::Info;
    return true;
  } else if (test(skeys::warning_severity_key) ||
             test(lkeys::warning_severity_key)) {
    severity = db::NetlistCrossReference::Warning;
    return true;
  } else if (test(skeys::error_severity_key) ||
             test(lkeys::error_severity_key)) {
...
void LayoutVsSchematicStandardReader::read_log_entry(
    db::NetlistCrossReference *xref) {
  db::NetlistCrossReference::Severity severity =
      db::NetlistCrossReference::NoSeverity;
  std::string msg;
...
void LayoutVsSchematicStandardReader::read_logs_for_circuits(
    db::NetlistCrossReference *xref) {
  Brace br(this);
  while (br) {
    ...
    if (test(skeys::log_entry_key) || test(lkeys::log_entry_key)) {
      ...
      throw tl::Exception(
          tl::to_string(tr("Unexpected end of file inside circuit definition "
                           "(net, pin, device or circuit expected)")));
    } else {
      skip_element();
    }
  }
...
void LayoutVsSchematicStandardReader::read_xrefs_for_circuits(
    db::NetlistCrossReference *xref, const db::Circuit *circuit_a,
    const db::Circuit *circuit_b) {
  Brace br(this);
  while (br) {
...
void LayoutVsSchematicStandardReader::read_xref(
    db::NetlistCrossReference *xref) {
  Brace br(this);
  while (br) {
    ...
    if (test(skeys::circuit_key) || test(lkeys::circuit_key)) {
      ...
      if (non_a.second) {
        ...
        if (!circuit_a) {
          throw tl::Exception(tl::to_string(tr("Not a valid circuit name: ")) +
                              non_a.first);
        }
      }
      ...
      while (br) {
        ...
        if (read_status(status)) {
          ...
          throw tl::Exception(tl::to_string(
              tr("Unexpected end of file inside circuit definition (status "
                 "keyword of xrefs expected)")));
        } else {
          skip_element();
        }
      }
...
static const db::Net *net_by_numerical_id(
    const db::Circuit *circuit, const std::pair<unsigned int, bool> &ion,
    std::map<const db::Circuit *, db::LayoutToNetlistStandardReader::ObjectMap>
        &map_per_circuit) {
  if (ion.second && circuit) {

    std::map<const db::Circuit *,
             db::LayoutToNetlistStandardReader::ObjectMap>::const_iterator i =
        map_per_circuit.find(circuit);
    if (i != map_per_circuit.end()) {

      std::map<unsigned int, Net *>::const_iterator j =
          i->second.id2net.find(ion.first);
    ...
    throw tl::Exception(tl::to_string(tr("Not a valid net ID: ")) +
                        tl::to_string(ion.first));
  }

  return 0;
}

static const db::Device *device_by_numerical_id(
    const db::Circuit *circuit, const std::pair<unsigned int, bool> &ion,
    std::map<const db::Circuit *, db::LayoutToNetlistStandardReader::ObjectMap>
        &map_per_circuit) {
  if (ion.second && circuit) {
    ...
    if (i != map_per_circuit.end()) {
      ...
      std::map<unsigned int, Device *>::const_iterator j =
          i->second.id2device.find(ion.first);
      if (j != i->second.id2device.end()) {
        return j->second;
      }
    }

    throw tl::Exception(tl::to_string(tr("Not a valid device ID: ")) +
                        tl::to_string(ion.first));
  }
...
static const db::SubCircuit *subcircuit_by_numerical_id(
    const db::Circuit *circuit, const std::pair<unsigned int, bool> &ion,
    std::map<const db::Circuit *, db::LayoutToNetlistStandardReader::ObjectMap>
        &map_per_circuit) {
  if (ion.second && circuit) {
    ...
    if (i != map_per_circuit.end()) {
      ...
      std::map<unsigned int, SubCircuit *>::const_iterator j =
          i->second.id2subcircuit.find(ion.first);
      if (j != i->second.id2subcircuit.end()) {
        return j->second;
      }
    }

    throw tl::Exception(tl::to_string(tr("Not a subcircuit device ID: ")) +
                        tl::to_string(ion.first));
  }
...
static const db::Pin *
pin_by_numerical_id(const db::Circuit *circuit,
                    const std::pair<unsigned int, bool> &ion) {
  if (ion.second && circuit) {

    const db::Pin *pin = circuit->pin_by_id(ion.first);
    if (!pin) {
      throw tl::Exception(tl::to_string(tr("Not a valid pin ID: ")) +
                          tl::to_string(ion.first));
    }
...
void LayoutVsSchematicStandardReader::read_net_pair(
    db::NetlistCrossReference *xref, const db::Circuit *circuit_a,
    const db::Circuit *circuit_b) {
  Brace br(this);
  ...
  xref->gen_nets(net_by_numerical_id(circuit_a, ion_a, m_map_per_circuit_a),
                 net_by_numerical_id(circuit_b, ion_b, m_map_per_circuit_b),
                 status, msg);
}

void LayoutVsSchematicStandardReader::read_pin_pair(
    db::NetlistCrossReference *xref, const db::Circuit *circuit_a,
    const db::Circuit *circuit_b) {
  Brace br(this);
  ...
  xref->gen_pins(pin_by_numerical_id(circuit_a, ion_a),
                 pin_by_numerical_id(circuit_b, ion_b), status, msg);
}

void LayoutVsSchematicStandardReader::read_device_pair(
    db::NetlistCrossReference *xref, const db::Circuit *circuit_a,
    const db::Circuit *circuit_b) {
  Brace br(this);
  ...
  xref->gen_devices(
      device_by_numerical_id(circuit_a, ion_a, m_map_per_circuit_a),
      device_by_numerical_id(circuit_b, ion_b, m_map_per_circuit_b), status,
      msg);
}

void LayoutVsSchematicStandardReader::read_subcircuit_pair(
    db::NetlistCrossReference *xref, const db::Circuit *circuit_a,
    const db::Circuit *circuit_b) {
  Brace br(this);
  ...
  xref->gen_subcircuits(
      subcircuit_by_numerical_id(circuit_a, ion_a, m_map_per_circuit_a),
      subcircuit_by_numerical_id(circuit_b, ion_b, m_map_per_circuit_b), status,
      msg);
}
