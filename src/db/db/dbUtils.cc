 ...
 *
 *  The idea is bisection of the segments until the desired degree of accuracy
 * has been reached.
 *
 ...
 *
 *  New points are going to be inserted after current_curve_point and
 * current_curve_point + 1 to achieve the required curvature.
 */
static void spline_interpolate(
    std::list<db::DPoint> &curve_points,
    typename std::list<db::DPoint>::iterator current_curve_point,
    double t_start, double dt,
    const std::vector<std::pair<db::DPoint, double>> &control_points,
    int degree, const std::vector<double> &knots, double sin_da, double accu) {
  ...
  db::DPoint s1 =
      b_spline_point(t_start + 0.5 * dt, control_points, degree, knots);
  db::DPoint s2 =
      b_spline_point(t_start + 1.5 * dt, control_points, degree, knots);
  ...
  if (curve_points.size() < control_points.size() - degree - 1) {
    ...
    spline_interpolate(curve_points, current_curve_point, t_start, dt * 0.5,
                       control_points, degree, knots, sin_da, accu);

    curve_points.insert(pe, s2);
    spline_interpolate(curve_points, pm, t_start + dt, dt * 0.5, control_points,
                       degree, knots, sin_da, accu);
    ...
    if (fabs(db::vprod(p, q)) > pl * ql * sin_da ||
      ...
      //  angle between the segments is bigger than 2PI/n -> circle resolution
      //  is too small. Or: the angle between the new segments that we would
      //  introduce is also bigger (hence, the original segments may have a
      //  small angle but the curve "wiggles" when we increase the resolution)

      if (fabs(db::vprod(p1, p)) > pl * accu) {

        //  In addition, the estimated accuracy is not good enough on the first
        //  segment: bisect this segment.
      ...
      if (fabs(db::vprod(q1, q)) > ql * accu) {
        ...
        curve_points.insert(pe, s2);
        spline_interpolate(curve_points, pm, t_start + dt, dt * 0.5,
                           control_points, degree, knots, sin_da, accu);
      }
    }
  }
...
static std::list<db::DPoint> do_spline_interpolation(
    const std::vector<std::pair<db::DPoint, double>> &control_points,
    int degree, const std::vector<double> &knots, double relative_accuracy,
    double absolute_accuracy) {
  //  TODO: this is quite inefficient
  if (int(knots.size()) != int(control_points.size() + degree + 1)) {
    throw tl::Exception(tl::to_string(
        tr("Spline interpolation failed: mismatch between number of knots and "
           "points (#knots must be #points+degree+1)")));
  }
  ...
  spline_interpolate(new_points, new_points.begin(), t0, dt, control_points,
                     degree, knots, relative_accuracy, absolute_accuracy);
...
template <class P>
std::list<P>
spline_interpolation(const std::vector<std::pair<P, double>> &control_points,
                     int degree, const std::vector<double> &knots,
                     double relative_accuracy, double absolute_accuracy) {
  std::vector<std::pair<db::DPoint, double>> cp;
  cp.reserve(control_points.size());

  for (size_t i = 0; i < control_points.size(); ++i) {
    cp.push_back(std::make_pair(db::DPoint(control_points[i].first),
                                control_points[i].second));
  }

  std::list<db::DPoint> result = do_spline_interpolation(
      cp, degree, knots, relative_accuracy, absolute_accuracy);

  std::list<P> ret;
  for (std::list<db::DPoint>::const_iterator i = result.begin();
       i != result.end(); ++i) {
    ret.push_back(P(*i));
  }
  return ret;
...
template <>
DB_PUBLIC std::list<db::DPoint> spline_interpolation(
    const std::vector<std::pair<db::DPoint, double>> &control_points,
    int degree, const std::vector<double> &knots, double relative_accuracy,
    double absolute_accuracy) {
  return do_spline_interpolation(control_points, degree, knots,
                                 relative_accuracy, absolute_accuracy);
}

template <class P>
std::list<P> spline_interpolation(const std::vector<P> &control_points,
  ...
  return spline_interpolation(cp, degree, knots, relative_accuracy,
                              absolute_accuracy);
}
...
template DB_PUBLIC std::list<db::Point>
spline_interpolation(const std::vector<db::Point> &, int,
                     const std::vector<double> &, double, double);
template DB_PUBLIC std::list<db::Point>
spline_interpolation(const std::vector<db::Point> &,
                     const std::vector<double> &, int,
                     const std::vector<double> &, double, double);
template DB_PUBLIC std::list<db::Point>
spline_interpolation(const std::vector<std::pair<db::Point, double>> &, int,
                     const std::vector<double> &, double, double);

template DB_PUBLIC std::list<db::DPoint>
spline_interpolation(const std::vector<db::DPoint> &, int,
                     const std::vector<double> &, double, double);
template DB_PUBLIC std::list<db::DPoint>
spline_interpolation(const std::vector<db::DPoint> &,
                     const std::vector<double> &, int,
                     const std::vector<double> &, double, double);
