 ...
 */
template <class T> struct id_attribute {
 ...
 */
template <class T> struct cluster_id_attribute {
  typedef size_t attr_type;
 ...
 *  @brief A getter for the cluster ID of an object
 */
template <class T> struct cell_index_attribute {
 ...
 */
template <class T> struct name_attribute {
 ...
 */
template <class T, class I, class ATTR> class object_by_attr {
public:
  typedef typename ATTR::attr_type attr_type;
  typedef typename I::value_type value_type;

  object_by_attr(T *self, I (T::*bi)(), I (T::*ei)())
      : mp_self(self), m_bi(bi), m_ei(ei), m_valid(false) {
    //  .. nothing yet ..
  }
  ...
  value_type *object_by(const attr_type &attr) const {
    ...
    typename std::map<attr_type, value_type *>::const_iterator m =
        m_map.find(attr);
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    for (typename std::map<attr_type, value_type *>::const_iterator i =
             m_map.begin();
         i != m_map.end(); ++i) {
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const object_by_attr<T, I, ATTR> &x, bool no_self,
                     void *parent) {
