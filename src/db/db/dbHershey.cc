...
struct HersheyFont {
  HersheyFont(const short (*e)[4], const HersheyCharInfo *c, unsigned char c1,
              unsigned char c2, int y2, int y1)
...
std::vector<std::string> hershey_font_names() {
...
size_t hershey_count_edges(const std::string &s, unsigned int f) {
  ...
  for (const char *cp = s.c_str(); *cp;) {
    ...
    if (tl::skip_newline(cp)) {
      ...
      if (c < fp->end_char && c >= fp->start_char) {
        n += fp->chars[c - fp->start_char].edge_end -
             fp->chars[c - fp->start_char].edge_start;
      } else if (invalid_char < fp->end_char &&
                 invalid_char >= fp->start_char) {
        n += fp->chars[invalid_char - fp->start_char].edge_end -
             fp->chars[invalid_char - fp->start_char].edge_start;
      }
    }
  }
...
int hershey_font_width(unsigned int f) { return fonts[f]->width; }

int hershey_font_height(unsigned int f) { return fonts[f]->height; }

db::DBox hershey_text_box(const std::string &s, unsigned int f) {
  HersheyFont *fp = fonts[f];
...
void hershey_justify(const std::string &s, unsigned int f, db::DBox bx,
                     HAlign halign, VAlign valign,
                     std::vector<db::DPoint> &linestarts, double &left,
                     double &bottom) {
  left = 0.0;
  bottom = 0.0;
  ...
  for (std::vector<db::DPoint>::iterator l = linestarts.begin();
       l != linestarts.end(); ++l) {
...
basic_hershey_edge_iterator::basic_hershey_edge_iterator(
    const std::string &s, unsigned int f,
    const std::vector<db::DPoint> &line_starts)
    : m_line(0), m_string(s), m_edge(0), m_edge_end(0),
      m_linestarts(line_starts) {
  m_fp = fonts[f];
...
db::DEdge basic_hershey_edge_iterator::get() {
  ...
  if (!at_end()) {
    ...
    return db::DEdge(m_pos + db::DVector(ep[0], ep[1]),
                     m_pos + db::DVector(ep[2], ep[3]));
  } else {
    return db::DEdge();
  }
}
