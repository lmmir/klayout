...
public:
   ...
   *  If this method returns true, the edges are kept. Otherwise they are
   * discarded.
   */
 ...
 */
template <class Result> class DB_PUBLIC edge_processor {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  This method will take the input edge from "edge" and puts the results into
   * "res". "res" can be empty - in this case, the edge will be skipped.
   */
  virtual void process(const db::Edge &edge,
...
typedef shape_collection_processor<db::Edge, db::Polygon>
    EdgeToPolygonProcessorBase;
typedef shape_collection_processor<db::Edge, db::EdgePair>
    EdgeToEdgePairProcessorBase;
 ...
 */
typedef db::generic_shape_iterator_delegate_base<db::Edge>
    EdgesIteratorDelegate;

/**
 *  @brief The delegate for the actual edge set implementation
 */
class DB_PUBLIC EdgesDelegate : public ShapeCollectionDelegateBase {
public:
  ...
  width_check(db::Coord d, const db::EdgesCheckOptions &options) const = 0;
  virtual EdgePairsDelegate *
  space_check(db::Coord d, const db::EdgesCheckOptions &options) const = 0;
  virtual EdgePairsDelegate *
  enclosing_check(const Edges &other, db::Coord d,
                  const db::EdgesCheckOptions &options) const = 0;
  virtual EdgePairsDelegate *
  overlap_check(const Edges &other, db::Coord d,
                const db::EdgesCheckOptions &options) const = 0;
  virtual EdgePairsDelegate *
  separation_check(const Edges &other, db::Coord d,
                   const db::EdgesCheckOptions &options) const = 0;
  virtual EdgePairsDelegate *
  inside_check(const Edges &other, db::Coord d,
               const db::EdgesCheckOptions &options) const = 0;
  ...
  virtual EdgePairsDelegate *
  processed_to_edge_pairs(const EdgeToEdgePairProcessorBase &filter) const = 0;
  virtual RegionDelegate *
  processed_to_polygons(const EdgeToPolygonProcessorBase &filter) const = 0;
  ...
  virtual RegionDelegate *extended(coord_type ext_b, coord_type ext_e,
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  inside_outside_part_pair(const Region &other) const = 0;
  ...
  virtual EdgesDelegate *
  selected_not_interacting(const Region &other) const = 0;
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Region &other) const = 0;
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_interacting_pair(const Edges &other) const = 0;
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_outside_pair(const Edges &other) const = 0;
  ...
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  selected_inside_pair(const Edges &other) const = 0;

  virtual EdgesDelegate *in(const Edges &other, bool invert) const = 0;
  virtual std::pair<EdgesDelegate *, EdgesDelegate *>
  in_and_out(const Edges &) const = 0;
