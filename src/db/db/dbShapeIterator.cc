...
inline db::Shape
iterator_to_shape(db::Shapes *shapes,
                  const db::layer<Sh, db::unstable_layer_tag> & /*l*/,
                  const Iter &iter) {
...
template <class Sh, class Iter>
inline db::Shape iterator_to_shape(db::Shapes *shapes,
                                   const db::layer<Sh, db::stable_layer_tag> &l,
                                   const Iter &iter) {
...
template <class Sh, class Iter>
inline db::Shape
iterator_to_shape(const db::Shapes *shapes,
                  const db::layer<Sh, db::unstable_layer_tag> & /*l*/,
                  const Iter &iter) {
  //  for unstable containers, we simply use the pointer as a reference
  return db::Shape(shapes, *iter);
...
template <class Sh, class Iter>
inline db::Shape iterator_to_shape(const db::Shapes *shapes,
                                   const db::layer<Sh, db::stable_layer_tag> &l,
                                   const Iter &iter) {
  //  for stable containers, we derive the primitive iterator via the pointer
  return db::Shape(shapes, l.iterator_from_pointer(&*iter));
...
ShapeIterator::ShapeIterator()
    : m_region_mode(None), m_type(Null), m_box(), m_flags(0), mp_shapes(0),
      mp_prop_sel(0), m_inv_prop_sel(false), m_array_iterator_valid(false),
      m_editable(false), m_quad_id(0) {
  m_valid = false;
  m_with_props = false;
}
...
ShapeIterator::ShapeIterator(const shapes_type &shapes, unsigned int flags,
                             const property_selector *prop_sel,
                             bool inv_prop_sel)
    : m_region_mode(None), m_type(object_type(0)), m_box(), m_flags(flags),
      mp_shapes(&shapes), mp_prop_sel(prop_sel), m_inv_prop_sel(inv_prop_sel),
      m_array_iterator_valid(false), m_editable(shapes.is_editable()),
      m_quad_id(0) {
  //  optimize: empty property selection plus inverse = no property selection at
  //  all
  //            any property selection and not inverse = only shapes with
  //            properties
  if (mp_prop_sel) {
...
ShapeIterator::ShapeIterator(const shapes_type &shapes, const box_type &box,
                             region_mode mode, unsigned int flags,
                             const property_selector *prop_sel,
                             bool inv_prop_sel)
    : m_region_mode(mode), m_type(object_type(0)), m_box(box), m_flags(flags),
      mp_shapes(&shapes), mp_prop_sel(prop_sel), m_inv_prop_sel(inv_prop_sel),
      m_array_iterator_valid(false), m_editable(shapes.is_editable()),
      m_quad_id(0) {
  //  optimize: empty property selection plus inverse = no property selection at
...
ShapeIterator &ShapeIterator::operator=(const ShapeIterator &d) {
  if (&d != this) {
    ...
    if (m_type != Null) {
      ...
      if (m_array_iterator_valid) {
        ...
        if (m_type == PolygonPtrArray) {
          ...
          polygon_ptr_array_iterator_type *d_arr_iter =
              (polygon_ptr_array_iterator_type *)d.m_ad.iter;
          polygon_ptr_array_iterator_type *arr_iter =
              (polygon_ptr_array_iterator_type *)m_ad.iter;
          new (arr_iter) polygon_ptr_array_iterator_type(*d_arr_iter);

        } else if (m_type == SimplePolygonPtrArray) {

          simple_polygon_ptr_array_iterator_type *d_arr_iter =
              (simple_polygon_ptr_array_iterator_type *)d.m_ad.iter;
          simple_polygon_ptr_array_iterator_type *arr_iter =
              (simple_polygon_ptr_array_iterator_type *)m_ad.iter;
          new (arr_iter) simple_polygon_ptr_array_iterator_type(*d_arr_iter);

        } else if (m_type == PathPtrArray) {

          path_ptr_array_iterator_type *d_arr_iter =
              (path_ptr_array_iterator_type *)d.m_ad.iter;
          path_ptr_array_iterator_type *arr_iter =
              (path_ptr_array_iterator_type *)m_ad.iter;
          new (arr_iter) path_ptr_array_iterator_type(*d_arr_iter);

        } else if (m_type == TextPtrArray) {

          text_ptr_array_iterator_type *d_arr_iter =
              (text_ptr_array_iterator_type *)d.m_ad.iter;
          text_ptr_array_iterator_type *arr_iter =
              (text_ptr_array_iterator_type *)m_ad.iter;
          new (arr_iter) text_ptr_array_iterator_type(*d_arr_iter);

        } else if (m_type == BoxArray) {

          box_array_iterator_type *d_arr_iter =
              (box_array_iterator_type *)d.m_ad.iter;
          box_array_iterator_type *arr_iter =
              (box_array_iterator_type *)m_ad.iter;
          new (arr_iter) box_array_iterator_type(*d_arr_iter);

        } else if (m_type == ShortBoxArray) {

          short_box_array_iterator_type *d_arr_iter =
              (short_box_array_iterator_type *)d.m_ad.iter;
          short_box_array_iterator_type *arr_iter =
              (short_box_array_iterator_type *)m_ad.iter;
...
template <class Iter> inline void ShapeIterator::skip_array_iter() {
...
struct advance_algorithm_traits<Sh, StableTag, ShapeIterator::NoRegionTag> {
  ...
  typedef typename db::layer<db::object_with_properties<Sh>,
                             StableTag>::flat_iterator iterator_with_props_type;

  inline static void advance(iterator_type *iter, int /*mode*/) { ++*iter; }

  inline static void advance(iterator_with_props_type *iter, int /*mode*/) {
    ++*iter;
  }

  inline static size_t quad_id(iterator_type * /*iter*/) { return 0; }

  inline static size_t quad_id(iterator_with_props_type * /*iter*/) {
    return 0;
  }

  inline static iterator_type begin(const db::Shapes *shapes,
                                    const db::Box & /*box*/) {
    //  use get_layer().begin..() in order to suppress update() - this might
    //  change the container while iterating.
    return shapes->template get_layer<Sh, StableTag>().begin_flat();
  }

  inline static iterator_with_props_type
  begin_with_props(const db::Shapes *shapes, const db::Box & /*box*/) {
    //  use get_layer().begin..() in order to suppress update() - this might
    //  change the container while iterating.
    return shapes
        ->template get_layer<db::object_with_properties<Sh>, StableTag>()
        .begin_flat();
  }
};

template <class Sh, class StableTag>
struct advance_algorithm_traits<Sh, StableTag,
                                ShapeIterator::TouchingRegionTag> {
  typedef typename db::layer<Sh, StableTag>::touching_iterator iterator_type;
  typedef
      typename db::layer<db::object_with_properties<Sh>,
                         StableTag>::touching_iterator iterator_with_props_type;
  ...
  inline static size_t quad_id(iterator_type *iter) { return iter->quad_id(); }
...
bool ShapeIterator::advance_shape(int &mode) {
  ...
  typedef typename algorithm_traits::iterator_with_props_type
      iterator_with_props_type;
  ...
  if (mode) {
    ...
    if (!m_with_props) {
      ...
      do {
        algorithm_traits::advance(iter, mode);
      } while (mp_prop_sel && !iter->at_end() &&
               (mp_prop_sel->find((*iter)->properties_id()) ==
                mp_prop_sel->end()) != m_inv_prop_sel);
    }
  ...
  if (!m_with_props && !props_only) {
    ...
    if (m_valid) {
      if (!sel || iter->at_end()) {
        ...
        m_shape = iterator_to_shape(
            mp_shapes, mp_shapes->template get_layer<Sh, StableTag>(), *iter);
...

  {
    ...
    if (!m_valid && sel) {
      //  use get_layer().begin_flat() in order to suppress update() - this
      //  might change the container while iterating.
      iterator_with_props_type i =
          algorithm_traits::begin_with_props(mp_shapes, m_box);
      if (mp_prop_sel) {
        while (!i.at_end() && (mp_prop_sel->find(i->properties_id()) ==
                               mp_prop_sel->end()) != m_inv_prop_sel) {
          ++i;
        }
      }
    ...
    if (m_valid) {
      if (!sel || iter->at_end()) {
        ...
        m_shape = iterator_to_shape(
            mp_shapes,
            mp_shapes->template get_layer<db::object_with_properties<Sh>,
                                          StableTag>(),
            *iter);
        m_quad_id = algorithm_traits::quad_id(iter);
        return true;
...
void ShapeIterator::init_array_iter(typename ShapeIterator::NoRegionTag) {
  typedef typename Array::iterator array_iterator;

  array_iterator *arr_iter = (array_iterator *)m_ad.iter;
  if (m_with_props) {
    new (arr_iter) array_iterator(
        m_array.basic_ptr(typename db::object_with_properties<Array>::tag())
            ->begin());
  } else {
    new (arr_iter)
        array_iterator(m_array.basic_ptr(typename Array::tag())->begin());
  }
}

template <class Array>
void ShapeIterator::init_array_iter(typename ShapeIterator::TouchingRegionTag) {
  typedef typename Array::iterator array_iterator;
  ...
  new (arr_iter) array_iterator(
      m_array.basic_ptr(typename Array::tag())->begin_touching(m_box, bc));
}

template <class Array>
void ShapeIterator::init_array_iter(
    typename ShapeIterator::OverlappingRegionTag) {
  typedef typename Array::iterator array_iterator;
  typedef typename Array::object_type shape_ptr_type;
...
bool ShapeIterator::advance_aref(int &mode) {
  ...
  if (mode && m_array_iterator_valid) {
    ...
    if (mode == 1) {
      ...
      //  skip array quad -> skip rest of array quad and move to shape in the
      //  next quad or to end
      do_skip_array_quad();
      mode = 1;
    } else {
  ...
  while (true) {
    ...
    //  move to next item (increment on mode == 1, skip quad on mode == -1) or
    //  validate this one (if mode == 0)
  ...
  if (m_editable) {
    if (m_with_props) {
      m_shape = shape_type(
          mp_shapes,
          m_array.basic_iter(typename db::object_with_properties<Array>::tag()),
          t);
    } else {
      m_shape =
          shape_type(mp_shapes, m_array.basic_iter(typename Array::tag()), t);
    }
  } else {
    if (m_with_props) {
      m_shape = shape_type(
      ...
      m_shape =
          shape_type(mp_shapes, *m_array.basic_ptr(typename Array::tag()), t);
    }
  }
...
void ShapeIterator::advance_generic(int mode) {
  while (m_type != Null) {
    ...
    case SimplePolygonPtrArray:
      if (advance_aref<simple_polygon_ptr_array_type, StableTag, RegionTag>(
              mode))
        return;
      break;
    case Edge:
    ...
    m_type = object_type((unsigned int)m_type + 1);
    for (unsigned int m = 1 << (unsigned int)m_type;
         m_type != Null && (m_flags & m) == 0; m <<= 1) {
      m_type = object_type((unsigned int)m_type + 1);
    }
  }
}
...
template <class Sh, class StableTag>
db::Box ShapeIterator::quad_box_by_shape(
    typename ShapeIterator::TouchingRegionTag) const {
  tl_assert(m_valid);
  if (!m_with_props) {
    typename db::layer<Sh, StableTag>::touching_iterator *iter =
        (typename db::layer<Sh, StableTag>::touching_iterator *)m_d.iter;
    return iter->quad_box();
  } else {
    typename db::layer<db::object_with_properties<Sh>,
                       StableTag>::touching_iterator *iter =
        (typename db::layer<db::object_with_properties<Sh>,
                            StableTag>::touching_iterator *)m_d.iter;
    return iter->quad_box();
  }
}
...
template <class Sh, class StableTag>
db::Box ShapeIterator::quad_box_by_shape(
    typename ShapeIterator::OverlappingRegionTag) const {
  tl_assert(m_valid);
  if (!m_with_props) {
    typename db::layer<Sh, StableTag>::overlapping_iterator *iter =
        (typename db::layer<Sh, StableTag>::overlapping_iterator *)m_d.iter;
    return iter->quad_box();
  } else {
    typename db::layer<db::object_with_properties<Sh>,
                       StableTag>::overlapping_iterator *iter =
        (typename db::layer<db::object_with_properties<Sh>,
                            StableTag>::overlapping_iterator *)m_d.iter;
    return iter->quad_box();
  }
}

template <class RegionTag, class StableTag>
db::Box ShapeIterator::quad_box_generic() const {
  ...
  case SimplePolygonPtrArray:
    return (quad_box_by_shape<simple_polygon_ptr_array_type, StableTag>(
        region_tag));
  case Edge:
...
db::Box ShapeIterator::quad_box() const {
  if (m_editable) {
    if (m_region_mode == None) {
      return db::Box::world();
    } else if (m_region_mode == Touching) {
...
template <class Iter> void ShapeIterator::do_skip_array_quad_iter() {
  Iter *arr_iter = (Iter *)m_ad.iter;
  arr_iter->skip_quad();
}
...
template <class Iter> size_t ShapeIterator::get_array_quad_id() const {
  Iter *arr_iter = (Iter *)m_ad.iter;
  return arr_iter->quad_id();
}

size_t ShapeIterator::array_quad_id() const {
  if (m_array_iterator_valid) {
    if (m_type == PolygonPtrArray) {
...
db::Box ShapeIterator::get_array_quad_box() const {
...
db::Box ShapeIterator::array_quad_box() const {
  if (m_array_iterator_valid) {
    if (m_type == PolygonPtrArray) {
      return get_array_quad_box<polygon_ptr_array_iterator_type,
                                polygon_ptr_array_type>();
    } else if (m_type == SimplePolygonPtrArray) {
      return get_array_quad_box<simple_polygon_ptr_array_iterator_type,
                                simple_polygon_ptr_array_type>();
    } else if (m_type == PathPtrArray) {
      return get_array_quad_box<path_ptr_array_iterator_type,
                                path_ptr_array_type>();
    } else if (m_type == TextPtrArray) {
      return get_array_quad_box<text_ptr_array_iterator_type,
                                text_ptr_array_type>();
    } else if (m_type == BoxArray) {
      return get_array_quad_box<box_array_iterator_type, box_array_type>();
    } else if (m_type == ShortBoxArray) {
      return get_array_quad_box<short_box_array_iterator_type,
...
void ShapeIterator::cleanup() {
  //  this trick destroys all iterators that have been allocated in the generic
  //  union
  if (m_type != Null) {
