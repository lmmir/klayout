...
NetlistSpiceWriterDelegate::NetlistSpiceWriterDelegate() : mp_writer(0) {
  //  .. nothing yet ..
}
...
std::string
NetlistSpiceWriterDelegate::net_to_string(const db::Net *net) const {
  tl_assert(mp_writer != 0);
  return mp_writer->net_to_string(net);
}

std::string
NetlistSpiceWriterDelegate::format_name(const std::string &name) const {
  tl_assert(mp_writer != 0);
...
void NetlistSpiceWriterDelegate::emit_comment(
...
void NetlistSpiceWriterDelegate::write_device_intro(
    const db::DeviceClass &) const {
  //  .. nothing yet ..
}

void NetlistSpiceWriterDelegate::write_device(const db::Device &dev) const {
  const db::DeviceClass *dc = dev.device_class();
  const db::DeviceClassCapacitor *cap =
      dynamic_cast<const db::DeviceClassCapacitor *>(dc);
  const db::DeviceClassCapacitor *cap3 =
      dynamic_cast<const db::DeviceClassCapacitorWithBulk *>(dc);
  const db::DeviceClassInductor *ind =
      dynamic_cast<const db::DeviceClassInductor *>(dc);
  const db::DeviceClassResistor *res =
      dynamic_cast<const db::DeviceClassResistor *>(dc);
  const db::DeviceClassResistor *res3 =
      dynamic_cast<const db::DeviceClassResistorWithBulk *>(dc);
  const db::DeviceClassDiode *diode =
      dynamic_cast<const db::DeviceClassDiode *>(dc);
  const db::DeviceClassMOS3Transistor *mos3 =
      dynamic_cast<const db::DeviceClassMOS3Transistor *>(dc);
  const db::DeviceClassMOS4Transistor *mos4 =
      dynamic_cast<const db::DeviceClassMOS4Transistor *>(dc);
  const db::DeviceClassBJT3Transistor *bjt3 =
      dynamic_cast<const db::DeviceClassBJT3Transistor *>(dc);
  const db::DeviceClassBJT3Transistor *bjt4 =
  ...
  if (cap || cap3) {
    ...
    os << " ";
    os << tl::sprintf(
        "%.12g", dev.parameter_value(db::DeviceClassCapacitor::param_id_C));
    ...
    os << " ";
    os << tl::sprintf("%.12g",
                      dev.parameter_value(db::DeviceClassInductor::param_id_L));
    if (!dev.device_class()->name().empty()) {
      os << " ";
      os << format_name(dev.device_class()->name());
    }
    ...
    os << " ";
    os << tl::sprintf("%.12g",
                      dev.parameter_value(db::DeviceClassResistor::param_id_R));
    if (!dev.device_class()->name().empty()) {
      os << " ";
      os << format_name(dev.device_class()->name());
    }
    ...
    os << " ";
    os << net_to_string(
        dev.net_for_terminal(db::DeviceClassMOS3Transistor::terminal_id_D));
    os << " ";
    os << net_to_string(
        dev.net_for_terminal(db::DeviceClassMOS3Transistor::terminal_id_G));
    os << " ";
    os << net_to_string(
        dev.net_for_terminal(db::DeviceClassMOS3Transistor::terminal_id_S));
    os << " ";
    ...
    if (!mos4) {
      ...
      os << net_to_string(
          dev.net_for_terminal(db::DeviceClassMOS4Transistor::terminal_id_B));
    }
...
std::string NetlistSpiceWriterDelegate::format_terminals(const db::Device &dev,
                                                         size_t nmax) const {
  std::ostringstream os;

  const std::vector<db::DeviceTerminalDefinition> &td =
      dev.device_class()->terminal_definitions();
  size_t n = 0;
  for (std::vector<db::DeviceTerminalDefinition>::const_iterator i = td.begin();
       i != td.end() && n < nmax; ++i, ++n) {
...
std::string NetlistSpiceWriterDelegate::format_params(const db::Device &dev,
                                                      size_t without_id,
                                                      bool only_primary) const {
  std::ostringstream os;

  const std::vector<db::DeviceParameterDefinition> &pd =
      dev.device_class()->parameter_definitions();
...
NetlistSpiceWriter::NetlistSpiceWriter(NetlistSpiceWriterDelegate *delegate)
    : mp_netlist(0), mp_stream(0), mp_delegate(delegate), m_next_net_id(0),
      m_use_net_names(false), m_with_comments(true) {
...
void NetlistSpiceWriter::write(tl::OutputStream &stream,
                               const db::Netlist &netlist,
                               const std::string &description) {
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Writing netlist ")) + stream.path());
...
std::string NetlistSpiceWriter::net_to_string(const db::Net *net) const {
  if (m_use_net_names) {
    ...
    if (!net) {
      ...
      //  Tested with ngspice: this tool likes in net names: . $ ! & \ # + : |
      //  (but not at beginning) It does not like: , ; We translate , to | for
      //  the net separator

      std::map<const db::Net *, std::string>::const_iterator ni =
          m_net_to_spice_name.find(net);
    ...
    std::map<const db::Net *, size_t>::const_iterator n =
        m_net_to_spice_id.find(net);
...
void NetlistSpiceWriter::do_write(const std::string &description) {
  ...
  for (db::Netlist::const_device_class_iterator dc =
           mp_netlist->begin_device_classes();
       dc != mp_netlist->end_device_classes(); ++dc) {
    mp_delegate->write_device_intro(*dc);
  }

  for (db::Netlist::const_top_down_circuit_iterator c =
           mp_netlist->begin_top_down();
       c != mp_netlist->end_top_down(); ++c) {
    ...
    if (!m_use_net_names) {
      ...
      for (db::Circuit::const_net_iterator n = circuit.begin_nets();
        ...
        m_net_to_spice_id.insert(
            std::make_pair(n.operator->(), ++m_next_net_id));
      }
      ...
      //  determine the next net id for non-connected nets such that there is no
    ...
    for (db::Circuit::const_subcircuit_iterator i = circuit.begin_subcircuits();
         i != circuit.end_subcircuits(); ++i) {
      write_subcircuit_call(*i);
    }

    for (db::Circuit::const_device_iterator i = circuit.begin_devices();
         i != circuit.end_devices(); ++i) {

      if (m_with_comments) {
        std::string comment = "device instance " + i->expanded_name() + " " +
                              i->trans().to_string() + " " +
                              i->device_class()->name();
...
void NetlistSpiceWriter::write_subcircuit_call(
    const db::SubCircuit &subcircuit) const {
  if (m_with_comments) {
    std::string comment = "cell instance " + subcircuit.expanded_name() + " " +
                          subcircuit.trans().to_string();
    emit_comment(comment);
  }
...
void NetlistSpiceWriter::write_circuit_header(
