...
struct DB_PUBLIC EdgeRelationFilter {
   ...
   */
  EdgeRelationFilter(
      edge_relation_type r, distance_type d,
      metrics_type metrics = db::Euclidian, double ignore_angle = 90,
      distance_type min_projection = 0,
      distance_type max_projection = std::numeric_limits<distance_type>::max());

  /**
   *  @brief Tests whether two edges fulfil the check fail criterion
   *
   *  If the output pointer is non-null, the object will receive the edge pair
   * that represents the marker for this type of check.
   */
  bool check(const db::Edge &a, const db::Edge &b,
             db::EdgePair *output = 0) const;

  /**
   *  @brief Sets a flag indicating whether to report whole edges instead of
   * partial ones
   */
  void set_whole_edges(bool f) { m_whole_edges = f; }

  /**
   *  @brief Gets a flag indicating whether to report whole edges instead of
   * partial ones
   */
  bool whole_edges() const { return m_whole_edges; }

  /**
   *  @brief Sets a flag indicating whether zero distance shall be included in
   * the check
   */
  void set_include_zero(bool f) { m_include_zero = f; }

  /**
   *  @brief Gets a flag indicating whether zero distance shall be included in
   * the check
   */
  bool include_zero() const { return m_include_zero; }

  /**
   *  @brief Sets the metrics type
   */
  void set_metrics(metrics_type m) { m_metrics = m; }

  /**
   *  @brief Gets the metrics type
   */
  metrics_type metrics() const { return m_metrics; }

  /**
   *  @brief Sets the ignore corner angle parameter
   *
   *  This is the minimum angle connected edges must have so they are not
   * ignored.
   */
   ...
   */
  double ignore_angle() const { return m_ignore_angle; }

  /**
   *  @brief Sets the minimum projection parameter in database units
   */
  void set_min_projection(distance_type d) { m_min_projection = d; }

  /**
   *  @brief Gets the minimum projection parameter
   */
  distance_type min_projection() const { return m_min_projection; }

  /**
   *  @brief Sets the maximum projection parameter in database units
   */
  void set_max_projection(distance_type d) { m_max_projection = d; }

  /**
   *  @brief Gets the maximum projection parameter
   */
  distance_type max_projection() const { return m_max_projection; }

  /**
   *  @brief Gets the check distance
   */
  distance_type distance() const { return m_d; }

  /**
   *  @brief Gets the relation
   */
  edge_relation_type relation() const { return m_r; }
...
DB_PUBLIC bool projected_near_part_of_edge(
    bool include_zero, db::coord_traits<db::Coord>::distance_type d,
    const db::Edge &e, const db::Edge &g, db::Edge *output);
DB_PUBLIC bool square_near_part_of_edge(
    bool include_zero, db::coord_traits<db::Coord>::distance_type d,
    const db::Edge &e, const db::Edge &g, db::Edge *output);
DB_PUBLIC bool euclidian_near_part_of_edge(
    bool include_zero, db::coord_traits<db::Coord>::distance_type d,
    const db::Edge &e, const db::Edge &g, db::Edge *output);
DB_PUBLIC db::Edge::distance_type edge_projection(const db::Edge &a,
                                                  const db::Edge &b);
