...
template <class T> struct shape_flags_traits {
...
template <>
struct shape_flags_traits<db::PolygonRef> : public shape_flags_traits<void> {
...
template <class T> unsigned int shape_flags() {
  return shape_flags_traits<T>::generic();
}
template <class T> unsigned int shape_flags_pure() {
  return shape_flags_traits<T>::pure();
}
template <class T> bool shape_flags_with_props() {
  return shape_flags_traits<T>::with_props();
}

/**
 *  @brief Converter helpers for changing a shape to an object of a specific
 * type
 *
 *  These converters a volatile. The pointer delivered is not valid after the
 * next object has been retrieved.
 */

template <class T> struct DB_PUBLIC shape_to_object_impl {
  typedef T value_type;

  void set(const db::Shape &) {}
  const value_type *get(const db::Shape &s) const {
    return s.basic_ptr(typename T::tag());
  }
};
...
template <> struct DB_PUBLIC shape_to_object_impl<db::Polygon> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::PolygonWithProperties> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::SimplePolygon> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::Path> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::PathWithProperties> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::Text> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::TextWithProperties> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::Box> {
...
template <> struct DB_PUBLIC shape_to_object_impl<db::BoxWithProperties> {
...
template <class T>
struct DB_PUBLIC shape_to_object : public shape_to_object_impl<T> {
  const typename shape_to_object_impl<T>::value_type &
  operator()(const db::Shape &s) {
 ...
 *
 *  This object can deliver addressable objects from shapes. It will keep
 * temporary objects internally if required.
 */

template <class T> struct addressable_object_from_shape {
  typedef T value_type;
...
struct addressable_object_from_shape<db::object_with_properties<T>> {
  ...
  const db::object_with_properties<T> *operator()(const db::Shape &shape) {
    if (shape.has_prop_id()) {
      ...
      typename T::tag object_tag;
      m_heap.push_back(
          db::object_with_properties<T>(*shape.basic_ptr(object_tag), 0));
      return &m_heap.back();
    }
  }
...
template <> struct addressable_object_from_shape<db::Box> {
  typedef db::Box value_type;
...
template <> struct addressable_object_from_shape<db::BoxWithProperties> {
  typedef db::BoxWithProperties value_type;
...
template <> struct addressable_object_from_shape<db::Polygon> {
  typedef db::Polygon value_type;
...
template <> struct addressable_object_from_shape<db::PolygonWithProperties> {
  typedef db::PolygonWithProperties value_type;
...
template <> struct addressable_object_from_shape<db::SimplePolygon> {
  typedef db::SimplePolygon value_type;
...
template <> struct addressable_object_from_shape<db::Path> {
  typedef db::Path value_type;
...
template <> struct addressable_object_from_shape<db::PathWithProperties> {
  typedef db::PathWithProperties value_type;
...
template <> struct addressable_object_from_shape<db::Edge> {
...
template <> struct addressable_object_from_shape<db::EdgeWithProperties> {
...
template <> struct addressable_object_from_shape<db::EdgePair> {
...
template <> struct addressable_object_from_shape<db::EdgePairWithProperties> {
...
template <> struct addressable_object_from_shape<db::Text> {
  typedef db::Text value_type;
...
template <> struct addressable_object_from_shape<db::TextWithProperties> {
  typedef db::TextWithProperties value_type;
