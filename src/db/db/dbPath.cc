...
template <class C> typename path<C>::distance_type path<C>::length() const {
...
template <class C> typename path<C>::perimeter_type path<C>::perimeter() const {
  double l;
  if (m_width < 0) {
    //  correction for round path ends. Uses the average radius approximation
    //  for the ellipse ends.
    l = M_PI * 0.5 *
        (sqrt(m_bgn_ext * m_bgn_ext / 2.0 + m_width * m_width / 8.0) +
         sqrt(m_end_ext * m_end_ext / 2.0 + m_width * m_width / 8.0));
  } else {
...
template <class C> typename path<C>::area_type path<C>::area() const {
...
template <class C> std::string path<C>::to_string() const {
  std::string s = "(";
...
template <class C> void path<C>::update_bbox() const {
  if (m_bbox.empty() && !m_points.empty()) {
    ...
    create_shifted_points(m_bgn_ext, m_end_ext, width(), true,
                          tmp_points.begin(), tmp_points.end(), 2,
                          db::box_inserter<box_type>(m_bbox));
    create_shifted_points(m_end_ext, m_bgn_ext, width(), false,
                          tmp_points.rbegin(), tmp_points.rend(), 2,
                          db::box_inserter<box_type>(m_bbox));
#endif
  }
}
  ...
  for (typename pointlist_type::const_iterator p = m_points.begin();
    ...
    if (p != m_points.end()) {
      ...
      while (++p != m_points.end()) {
        ...
        if (p == m_points.end() ||
            !edge<C>(real_pts.back(), *p).contains(*pn)) {
          break;
        }
        pn = p;
      }
...
template <class C> inline db::DVector dpx(const db::DVector &p, double d);

template <> inline db::DVector dpx<Coord>(const db::DVector &p, double d) {
  if (fabs(p.x()) < db::epsilon || fabs(p.y()) < db::epsilon) {
    ...
    //  45 degree case: try to round d such that if p is on the grid it will be
    //  later
...
template <> inline db::DVector dpx<DCoord>(const db::DVector &p, double d) {
...
void path<C>::create_shifted_points(C start, C end, C width, bool forward,
                                    Iter from, Iter to, int ncircle,
                                    Inserter pts) const {
  //  for safety reasons
  if (from == to) {
    return;
  ...
  if (pp == to) {
    ...
    //  Special case of degenerated path with one point: treat as infinitely
    //  small segment with direction (1,0)
    db::DVector ed(forward ? 1.0 : -1.0, 0.0);
    db::DVector nd(-ed.y(), ed.x());
    db::DVector edd, ndd;

    //  The first point is taken as being simply shifted normally and pulled
    //  back by start_ext or, in round mode, approximated by a set of segments
    if (ncircle > 2) {
  ...
  while (pp != to) {
    ...
    if (ppp == to) {
      ...
      //  The last point is taken as being simply shifted normally and pulled
      //  forward by end_ext or, in round mode, approximated by a set of
      //  segments
      if (ncircle > 2) {
      ...
      if (fabs(dv) > db::epsilon) {
        ...
        if ((l1 < -db::epsilon) != (l2 < -db::epsilon)) {
          ...
          *pts++ = *pp + vector<C>(nnd);
          ...
          //  Segments are too short - the won't intersect: In this case we
          //  create a loop of three points which define the area in
          //  self-overlapping way but confined to the path within the limits of
          //  its width. HINT: the execution of this code is a pretty strong
          //  evidence for the existence to loops in the contour delivered. A
          //  proof however is missing ..
          *pts++ = *pp + vector<C>(nd);
          *pts++ = *pp;
          *pts++ = *pp + vector<C>(nnd);
...
DPath round_path_corners(const db::DPath &input, double r, int n,
                         double accuracy) {
  ...
  if (p != path_points.end()) {
    ...
    if (pp != path_points.end()) {
      ...
      while (ppp != path_points.end()) {
        ...
        double d2 =
            (ppp + 1 == path_points.end() ? 1.0 : 0.5) * (*ppp).distance(*pp);
        ...
        if (fabs(ract) < accuracy) {
          ...
          for (int i = 0; i < npts; ++i) {
            ...
            x = xc + ract_outer * s1.y() * cos(aa) -
                fabs(ract_outer) * s1.x() * sin(aa);
            y = yc - ract_outer * s1.x() * cos(aa) -
                fabs(ract_outer) * s1.y() * sin(aa);
...
path<Coord>::create_shifted_points(Coord, Coord, Coord, bool,
                                   path<Coord>::pointlist_type::iterator,
                                   path<Coord>::pointlist_type::iterator, int,
                                   box_inserter<path<Coord>::box_type>) const;
template DB_PUBLIC void path<Coord>::create_shifted_points(
    Coord, Coord, Coord, bool, path<Coord>::pointlist_type::iterator,
    path<Coord>::pointlist_type::iterator, int,
    std::back_insert_iterator<tl::vector<path<Coord>::point_type>>) const;
template DB_PUBLIC void path<Coord>::create_shifted_points(
    Coord, Coord, Coord, bool, path<Coord>::pointlist_type::iterator,
    path<Coord>::pointlist_type::iterator, int,
    std::back_insert_iterator<std::vector<path<Coord>::point_type>>) const;
template DB_PUBLIC void path<Coord>::create_shifted_points(
    Coord, Coord, Coord, bool, path<Coord>::pointlist_type::reverse_iterator,
    path<Coord>::pointlist_type::reverse_iterator, int,
    std::back_insert_iterator<tl::vector<path<Coord>::point_type>>) const;
template DB_PUBLIC void path<Coord>::create_shifted_points(
    Coord, Coord, Coord, bool, path<Coord>::pointlist_type::reverse_iterator,
    path<Coord>::pointlist_type::reverse_iterator, int,
...
template DB_PUBLIC void
path<DCoord>::create_shifted_points(DCoord, DCoord, DCoord, bool,
                                    path<DCoord>::pointlist_type::iterator,
                                    path<DCoord>::pointlist_type::iterator, int,
                                    box_inserter<path<DCoord>::box_type>) const;
template DB_PUBLIC void path<DCoord>::create_shifted_points(
    DCoord, DCoord, DCoord, bool, path<DCoord>::pointlist_type::iterator,
    path<DCoord>::pointlist_type::iterator, int,
    std::back_insert_iterator<tl::vector<path<DCoord>::point_type>>) const;
template DB_PUBLIC void path<DCoord>::create_shifted_points(
    DCoord, DCoord, DCoord, bool, path<DCoord>::pointlist_type::iterator,
    path<DCoord>::pointlist_type::iterator, int,
    std::back_insert_iterator<std::vector<path<DCoord>::point_type>>) const;
template DB_PUBLIC void path<DCoord>::create_shifted_points(
    DCoord, DCoord, DCoord, bool,
    path<DCoord>::pointlist_type::reverse_iterator,
    path<DCoord>::pointlist_type::reverse_iterator, int,
    std::back_insert_iterator<tl::vector<path<DCoord>::point_type>>) const;
template DB_PUBLIC void path<DCoord>::create_shifted_points(
    DCoord, DCoord, DCoord, bool,
    path<DCoord>::pointlist_type::reverse_iterator,
...
template <class C>
bool _test_extractor_impl(tl::Extractor &ex, db::path<C> &p) {
...
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DPath &p) {
  return _test_extractor_impl(ex, p);
}
