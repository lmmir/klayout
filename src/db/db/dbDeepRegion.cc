 ...
 */
class DB_PUBLIC DeepRegionIterator : public RegionIteratorDelegate {
public:
  ...
  virtual bool at_end() const { return m_iter.at_end(); }
  ...
  virtual const value_type *get() const { return &m_polygon; }

  virtual db::properties_id_type prop_id() const { return m_prop_id; }

  virtual bool
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const DeepRegionIterator *o =
        dynamic_cast<const DeepRegionIterator *>(other);
  ...
  virtual db::Box bbox() const { return m_iter.bbox(); }
...
DeepRegion::DeepRegion(const RecursiveShapeIterator &si, DeepShapeStore &dss,
                       double area_ratio, size_t max_vertex_count)
...
DeepRegion::DeepRegion(const RecursiveShapeIterator &si, DeepShapeStore &dss,
    ...
    : MutableRegion(), m_merged_polygons() {
  set_deep_layer(
      dss.create_polygon_layer(si, area_ratio, max_vertex_count, trans));
  init();
...
DeepRegion::DeepRegion() : MutableRegion() { init(); }

DeepRegion::DeepRegion(const DeepLayer &dl) : MutableRegion() {
  set_deep_layer(dl);
  init();
}
...
DeepRegion &DeepRegion::operator=(const DeepRegion &other) {
  if (this != &other) {
...
RegionDelegate *DeepRegion::clone() const { return new DeepRegion(*this); }
...
void DeepRegion::do_insert(const db::Polygon &polygon,
  ...
  if (layout.begin_top_down() != layout.end_top_down()) {
    ...
    if (prop_id == 0) {
      ...
      shapes.insert(db::PolygonRefWithProperties(
          db::PolygonRef(polygon, layout.shape_repository()), prop_id));
    }
  }
...
static void transform_deep_layer(db::DeepLayer &deep_layer, const Trans &t) {
  if (t.equal(Trans(db::Disp(t.disp())))) {
    ...
    for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
      ...
      const std::map<db::ICplxTrans, size_t> &v =
          vars.variants(c->cell_index());
      tl_assert(v.size() == size_t(1));

      db::Trans tr(v.begin()->first.inverted() * t.disp());

      db::Shapes &shapes = c->shapes(deep_layer.layer());
      db::Shapes new_shapes(layout.manager(), c.operator->(),
                            layout.is_editable());
    ...
    if (layout.begin_top_down() != layout.end_top_down()) {
      ...
      for (db::RecursiveShapeIterator iter(layout, top_cell,
                                           deep_layer.layer());
           !iter.at_end(); ++iter) {
        db::Polygon poly;
        iter->polygon(poly);
...
void DeepRegion::do_transform(const db::Trans &t) {
  ...
  if (m_merged_polygons_valid &&
      m_merged_polygons.layer() != deep_layer().layer()) {
...
static void flatten_layer(db::DeepLayer &deep_layer) {
  db::Layout &layout = deep_layer.layout();
  if (layout.begin_top_down() != layout.end_top_down()) {
    ...
    for (db::RecursiveShapeIterator iter(layout, top_cell, deep_layer.layer());
         !iter.at_end(); ++iter) {
      if (iter->is_polygon()) {
        db::Polygon poly;
        iter->polygon(poly);
        if (!iter->prop_id()) {
          flat_shapes.insert(db::PolygonRef(poly.transformed(iter.trans()),
                                            layout.shape_repository()));
        } else {
          flat_shapes.insert(db::PolygonRefWithProperties(
              db::PolygonRef(poly.transformed(iter.trans()),
                             layout.shape_repository()),
              iter->prop_id()));
        }
      }
...
RegionIteratorDelegate *DeepRegion::begin() const {
  return new DeepRegionIterator(begin_iter().first);
}

RegionIteratorDelegate *DeepRegion::begin_merged() const {
  if (!merged_semantics()) {
    return begin();
  } else {
...
DeepRegion::begin_iter() const {
  ...
  if (layout.cells() == 0) {
    ...
    db::RecursiveShapeIterator iter(deep_layer().layout(), top_cell,
                                    deep_layer().layer());
...
DeepRegion::begin_merged_iter() const {
  if (!merged_semantics()) {
    ...
    if (layout.cells() == 0) {
      ...
      const db::Cell &top_cell = layout.cell(*layout.begin_top_down());
      db::RecursiveShapeIterator iter(m_merged_polygons.layout(), top_cell,
                                      m_merged_polygons.layer());
      return std::make_pair(iter, db::ICplxTrans());
    }
  }
}

bool DeepRegion::empty() const { return begin_iter().first.at_end(); }

bool DeepRegion::is_merged() const { return m_is_merged; }

const db::Polygon *DeepRegion::nth(size_t) const {
  throw tl::Exception(tl::to_string(
      tr("Random access to polygons is available only for flat regions")));
}

db::properties_id_type DeepRegion::nth_prop_id(size_t) const {
  throw tl::Exception(tl::to_string(
      tr("Random access to polygons is available only for flat regions")));
}

bool DeepRegion::has_valid_polygons() const { return false; }

bool DeepRegion::has_valid_merged_polygons() const { return false; }

const db::RecursiveShapeIterator *DeepRegion::iter() const { return 0; }
...
bool DeepRegion::equals(const Region &other) const {
  const DeepRegion *other_delegate =
      dynamic_cast<const DeepRegion *>(other.delegate());
  if (other_delegate &&
      &other_delegate->deep_layer().layout() == &deep_layer().layout() &&
...
bool DeepRegion::less(const Region &other) const {
  const DeepRegion *other_delegate =
      dynamic_cast<const DeepRegion *>(other.delegate());
  if (other_delegate &&
      &other_delegate->deep_layer().layout() == &deep_layer().layout()) {
...
class ClusterMerger {
public:
  ClusterMerger(unsigned int layer, db::Layout &layout,
                const db::hier_clusters<db::PolygonRef> &hc, bool min_coherence,
                bool report_progress, const std::string &progress_desc)
      : m_layer(layer), mp_layout(&layout), mp_hc(&hc),
        m_min_coherence(min_coherence), m_ep(report_progress, progress_desc) {
    //  .. nothing yet ..
  }

  void set_base_verbosity(int vb) { m_ep.set_base_verbosity(vb); }

  db::Shapes &merged(size_t cid, db::cell_index_type ci,
...
private:
  ...
  std::map<std::pair<size_t, db::cell_index_type>, db::properties_id_type>
  ...
  db::properties_id_type property_id(size_t cid, db::cell_index_type ci,
                                     bool initial) {
    std::map<std::pair<size_t, db::cell_index_type>,
             db::properties_id_type>::iterator s =
        m_property_id_per_cluster.find(std::make_pair(cid, ci));

    //  some sanity checks: initial clusters are single-use, are never generated
    //  twice and cannot be retrieved again
    if (initial) {
    ...
    s = m_property_id_per_cluster
            .insert(std::make_pair(std::make_pair(cid, ci),
                                   db::properties_id_type(0)))
            .first;

    const db::connected_clusters<db::PolygonRef> &cc =
        mp_hc->clusters_per_cell(ci);
    ...
    if (c.begin_attr() != c.end_attr()) {
      ...
      const db::connected_clusters<db::PolygonRef>::connections_type &conn =
          cc.connections_for_cluster(cid);
      for (db::connected_clusters<
               db::PolygonRef>::connections_type::const_iterator i =
               conn.begin();
           i != conn.end() && s->second == db::properties_id_type(0); ++i) {
  ...
  db::Shapes &compute_merged(size_t cid, db::cell_index_type ci, bool initial,
                             unsigned int min_wc) {
    std::map<std::pair<size_t, db::cell_index_type>, db::Shapes>::iterator s =
        m_merged_cluster.find(std::make_pair(cid, ci));
    ...
    s = m_merged_cluster
            .insert(std::make_pair(std::make_pair(cid, ci), db::Shapes(false)))
            .first;
    ...
    if (min_wc > 0) {
      ...
      //  We cannot merge bottom-up in min_wc mode, so we just use the recursive
      //  cluster iterator
      ...
      for (db::recursive_cluster_shape_iterator<db::PolygonRef> s =
               db::recursive_cluster_shape_iterator<db::PolygonRef>(
                   *mp_hc, m_layer, ci, cid);
           !s.at_end(); ++s) {
      ...
      std::list<std::pair<const db::Shapes *, db::ICplxTrans>>
          merged_child_clusters;
      ...
      for (db::connected_clusters<
               ...
               conn.begin();
           i != conn.end(); ++i) {
        const db::Shapes &cc_shapes =
            compute_merged(i->id(), i->inst_cell_index(), false, min_wc);
        merged_child_clusters.push_back(
            std::make_pair(&cc_shapes, i->inst_trans()));
      }
      ...
      for (std::list<std::pair<const db::Shapes *, db::ICplxTrans>>::
               const_iterator i = merged_child_clusters.begin();
           i != merged_child_clusters.end(); ++i) {
        for (db::Shapes::shape_iterator s =
                 i->first->begin(db::ShapeIterator::All);
             !s.at_end(); ++s) {
          if (s->is_polygon()) {
            db::Polygon poly;
            s->polygon(poly);
      ...
      for (db::local_cluster<db::PolygonRef>::shape_iterator s =
               c.begin(m_layer);
           !s.at_end(); ++s) {
        db::Polygon poly = s->obj();
...
const DeepLayer &DeepRegion::merged_deep_layer() const {
  if (merged_semantics()) {
    ensure_merged_polygons_valid();
...
bool DeepRegion::merged_polygons_available() const {
  return m_is_merged || m_merged_polygons_valid;
}

void DeepRegion::ensure_merged_polygons_valid() const {
  if (!m_merged_polygons_valid) {
    ...
    if (m_is_merged) {
      ...
      tl::SelfTimer timer(tl::verbosity() > base_verbosity(),
                          "Ensure merged polygons");
      ...
      hc.build(layout, deep_layer().initial_cell(), conn, 0, 0,
               true /*separate_attributes*/);

      //  collect the clusters and merge them into big polygons
      //  NOTE: using the ClusterMerger we merge bottom-up forming bigger and
      //  bigger polygons. This is hopefully more efficient that collecting
      //  everything and will lead to reuse of parts.

      ClusterMerger cm(deep_layer().layer(), layout, hc, min_coherence(),
                       report_progress(), progress_desc());
      cm.set_base_verbosity(base_verbosity() + 10);

      //  TODO: iterate only over the called cells?
      for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
        const db::connected_clusters<db::PolygonRef> &cc =
            hc.clusters_per_cell(c->cell_index());
        for (db::connected_clusters<db::PolygonRef>::all_iterator cl =
                 cc.begin_all();
             !cl.at_end(); ++cl) {
          if (cc.is_root(*cl)) {
...
void DeepRegion::set_is_merged(bool f) {
  m_is_merged = f;
  m_merged_polygons_valid = false;
  m_merged_polygons = db::DeepLayer();
}

void DeepRegion::insert_into(db::Layout *layout, db::cell_index_type into_cell,
                             unsigned int into_layer) const {
  deep_layer().insert_into(layout, into_cell, into_layer);
}

RegionDelegate *
DeepRegion::nets(LayoutToNetlist *l2n, NetPropertyMode prop_mode,
                 const tl::Variant &net_prop_name,
                 const std::vector<const db::Net *> *nets) const {
  db::NetBuilder &net_builder =
      deep_layer().store_non_const()->net_builder_for(l2n);

  if (&l2n->dss() != deep_layer().store()) {
    throw tl::Exception(
        tl::to_string(tr("Extracted netlist is from different scope as this "
                         "layer - cannot pull net shapes")));
  }
  ...
  if (!region_for_layer) {
    throw tl::Exception(tl::to_string(tr("The given layer is not an original "
                                         "layer used in netlist extraction")));
  }
...
RegionDelegate *
DeepRegion::and_with(const Region &other,
                     PropertyConstraint property_constraint) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
  ...
  if (empty()) {
    ...
    return other.delegate()->clone()->remove_properties(
        pc_remove(property_constraint));
    ...
    return new DeepRegion(
        and_or_not_with(other_deep, true, property_constraint));
  }
}

RegionDelegate *
DeepRegion::not_with(const Region &other,
                     PropertyConstraint property_constraint) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
  ...
  if (empty() || other.empty()) {
    ...
    return new DeepRegion(
        and_or_not_with(other_deep, false, property_constraint));
  }
}

RegionDelegate *
DeepRegion::or_with(const Region &other,
...
DeepRegion::andnot_with(const Region &other,
                        PropertyConstraint property_constraint) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
  ...
  if (empty()) {
    ...
    return std::make_pair(
        clone()->remove_properties(pc_remove(property_constraint)),
        clone()->remove_properties(pc_remove(property_constraint)));

  } else if (other.empty()) {

    return std::make_pair(
        other.delegate()->clone()->remove_properties(
            pc_remove(property_constraint)),
        clone()->remove_properties(pc_remove(property_constraint)));
    ...
    std::pair<DeepLayer, DeepLayer> res =
        and_and_not_with(other_deep, property_constraint);
    return std::make_pair(new DeepRegion(res.first),
                          new DeepRegion(res.second));
  }
}

DeepLayer
DeepRegion::and_or_not_with(const DeepRegion *other, bool and_op,
                            db::PropertyConstraint property_constraint) const {
  ...
  if (pc_skip(property_constraint)) {
    ...
    db::local_processor<db::PolygonRef, db::PolygonRef, db::PolygonRef> proc(
        const_cast<db::Layout *>(&deep_layer().layout()),
        const_cast<db::Cell *>(&deep_layer().initial_cell()),
        &other->deep_layer().layout(), &other->deep_layer().initial_cell(),
        deep_layer().breakout_cells(), other->deep_layer().breakout_cells());
    configure_proc(proc);
    ...
    proc.run(&op, deep_layer().layer(), other->deep_layer().layer(),
             dl_out.layer());

  } else {

    db::BoolAndOrNotLocalOperationWithProperties op(
        and_op, &dl_out.layout().properties_repository(),
        &deep_layer().layout().properties_repository(),
        &other->deep_layer().layout().properties_repository(),
        property_constraint);

    db::local_processor<db::PolygonRefWithProperties,
                        db::PolygonRefWithProperties,
                        db::PolygonRefWithProperties>
        proc(const_cast<db::Layout *>(&deep_layer().layout()),
             const_cast<db::Cell *>(&deep_layer().initial_cell()),
             &other->deep_layer().layout(), &other->deep_layer().initial_cell(),
             deep_layer().breakout_cells(),
             other->deep_layer().breakout_cells());
    configure_proc(proc);
    proc.set_threads(deep_layer().store()->threads());
    proc.set_area_ratio(deep_layer().store()->max_area_ratio());
    proc.set_max_vertex_count(deep_layer().store()->max_vertex_count());
...
DeepRegion::and_and_not_with(const DeepRegion *other,
                             PropertyConstraint property_constraint) const {
  ...
  if (pc_skip(property_constraint)) {
    ...
    db::PropertiesRepository *pr_out1 =
        &dl_out1.layout().properties_repository();
    db::PropertiesRepository *pr_out2 =
        &dl_out2.layout().properties_repository();
    const db::PropertiesRepository *pr =
        &deep_layer().layout().properties_repository();
    db::TwoBoolAndNotLocalOperationWithProperties op(pr_out1, pr_out2, pr, pr,
                                                     property_constraint);
...
RegionDelegate *
DeepRegion::xor_with(const Region &other,
                     db::PropertyConstraint property_constraint) const {
  const DeepRegion *other_deep =
      dynamic_cast<const DeepRegion *>(other.delegate());
  ...
  if (empty()) {
    ...
    //  Prepare a version of "other_deep" that is mapped into the hierarchy
    //  space of "this"
    ...
    if (report_progress()) {
      other_deep_mapped->enable_progress(progress_desc() +
                                         tl::to_string(tr(" - reverse part")));
    } else {
      other_deep_mapped->disable_progress();
    }

    DeepLayer n1(
        and_or_not_with(other_deep_mapped.get(), false, property_constraint));
    DeepLayer n2(
        other_deep_mapped->and_or_not_with(this, false, property_constraint));
    n1.add_from(n2);
...
RegionDelegate *DeepRegion::add_in_place(const Region &other) {
  if (other.empty()) {
    return this;
  }
  ...
  if (other_deep) {
    ...
    db::Shapes &shapes =
        deep_layer().initial_cell().shapes(deep_layer().layer());
    db::PolygonRefToShapesGenerator pr(
        const_cast<db::Layout *>(&deep_layer().layout()), &shapes);
...
RegionDelegate *DeepRegion::add(const Region &other) const {
  if (other.empty()) {
    return clone();
  } else if (empty()) {
    return other.delegate()->clone();
...
bool DeepRegion::is_box() const { return is_box_from_iter(begin_iter().first); }

size_t DeepRegion::count() const {
  if (empty()) {
    return 0;
  }
  ...
  for (db::Layout::top_down_const_iterator c = layout.begin_top_down();
       c != layout.end_top_down(); ++c) {
...
size_t DeepRegion::hier_count() const {
  if (empty()) {
    return 0;
  }
...
DeepRegion::area_type DeepRegion::area(const db::Box &box) const {
  if (empty()) {
    return 0;
  }
  ...
  if (box.empty()) {
    ...
    for (db::Layout::top_down_const_iterator c = layout.begin_top_down();
      ...
      for (db::ShapeIterator s = layout.cell(*c)
                                     .shapes(polygons.layer())
                                     .begin(db::ShapeIterator::All);
           !s.at_end(); ++s) {
        ac += s->area();
      }
      const std::map<db::ICplxTrans, size_t> &vv = vars.variants(*c);
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
           v != vv.end(); ++v) {
...
DeepRegion::perimeter_type DeepRegion::perimeter(const db::Box &box) const {
  if (empty()) {
    return 0;
  }
...
Box DeepRegion::bbox() const {
  return deep_layer().initial_cell().bbox(deep_layer().layer());
}

std::string DeepRegion::to_string(size_t nmax) const {
  return db::AsIfFlatRegion::to_string(nmax);
}

EdgePairsDelegate *DeepRegion::grid_check(db::Coord gx, db::Coord gy) const {
  if (empty()) {
  ...
  if (gx < 0 || gy < 0) {
    throw tl::Exception(
        tl::to_string(tr("Grid check requires a positive grid value")));
  }
  ...
  std::unique_ptr<db::DeepEdgePairs> res(
      new db::DeepEdgePairs(polygons.derived()));
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
      ...
      for (db::Shapes::shape_iterator si = shapes.begin(db::ShapeIterator::All);
           !si.at_end(); ++si) {

        db::Polygon poly;
        si->polygon(poly);

        AsIfFlatRegion::produce_markers_for_grid_check(poly, v->first, gx, gy,
                                                       *markers);
      }
    }
  }

  //  propagate the markers with a similar algorithm used for producing the
  //  variants
...
EdgePairsDelegate *DeepRegion::angle_check(double min, double max,
                                           bool inverse) const {
  if (empty()) {
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator si = shapes.begin(db::ShapeIterator::All);
      ...
      db::Polygon poly;
      si->polygon(poly);
      AsIfFlatRegion::produce_markers_for_angle_check(
          poly, db::UnitTrans(), min, max, inverse, markers);
    }
  }
...
RegionDelegate *DeepRegion::snapped(db::Coord gx, db::Coord gy) {
  if (empty()) {
    return clone();
  }
  ...
  if (gx < 0 || gy < 0) {
    throw tl::Exception(
        tl::to_string(tr("Snapping requires a positive grid value")));
  }
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator si = s.begin(db::ShapeIterator::All);
         !si.at_end(); ++si) {
...
EdgesDelegate *DeepRegion::edges(const EdgeFilterBase *filter) const {
  ...
  if (!filter && merged_semantics() && !merged_polygons_available()) {
    ...
    db::PolygonToEdgeLocalOperation op(
        res->properties_repository(),
        &polygons.layout().properties_repository());

    db::local_processor<db::PolygonRefWithProperties,
                        db::PolygonRefWithProperties, db::EdgeWithProperties>
        proc(&res->deep_layer().layout(), &res->deep_layer().initial_cell(),
    ...
    proc.run(&op, polygons.layer(), foreign_idlayer(),
             res->deep_layer().layer());

  } else {

    const db::DeepLayer &polygons = merged_deep_layer();
    db::PropertyMapper pm(res->properties_repository(),
                          &polygons.layout().properties_repository());
    ...
    for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
      ...
      for (db::Shapes::shape_iterator si = s.begin(db::ShapeIterator::All);
        ...
        for (db::Polygon::polygon_edge_iterator e = poly.begin_edge();
             !e.at_end(); ++e) {
...
DeepRegion::processed_to_edges(const PolygonToEdgeProcessorBase &filter) const {
  ...
  return shape_collection_processed_impl<db::Polygon, db::Edge, db::DeepEdges>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}

EdgePairsDelegate *DeepRegion::processed_to_edge_pairs(
    const PolygonToEdgePairProcessorBase &filter) const {
  if (empty()) {
    return new db::DeepEdgePairs(deep_layer().derived());
  }

  return shape_collection_processed_impl<db::Polygon, db::EdgePair,
                                         db::DeepEdgePairs>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}
...
DeepRegion::processed(const PolygonProcessorBase &filter) const {
  ...
  return shape_collection_processed_impl<db::Polygon, db::Polygon,
                                         db::DeepRegion>(
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer(), filter);
}

RegionDelegate *DeepRegion::filter_in_place(const PolygonFilterBase &filter) {
  if (empty()) {
    return this;
  }
...
RegionDelegate *DeepRegion::filtered(const PolygonFilterBase &filter) const {
  if (empty()) {
    return clone();
  }
...
DeepRegion *DeepRegion::apply_filter(const PolygonFilterBase &filter) const {
  const db::DeepLayer &polygons =
      filter.requires_raw_input() ? deep_layer() : merged_deep_layer();
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    if (vars.get()) {
      ...
      const std::map<db::ICplxTrans, size_t> &vv =
          vars->variants(c->cell_index());
      for (std::map<db::ICplxTrans, size_t>::const_iterator v = vv.begin();
           v != vv.end(); ++v) {
...
RegionDelegate *DeepRegion::merged_in_place() {
  if (empty()) {
    return this;
  }
...
RegionDelegate *DeepRegion::merged_in_place(bool min_coherence,
                                            unsigned int min_wc) {
  //  TODO: implement to be really in-place
  return merged(min_coherence, min_wc);
}

RegionDelegate *DeepRegion::merged() const {
  if (empty()) {
    return clone();
  }
  ...
  std::unique_ptr<db::DeepRegion> res(
      new db::DeepRegion(m_merged_polygons.derived()));
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
...
RegionDelegate *DeepRegion::merged(bool min_coherence,
                                   unsigned int min_wc) const {
  if (empty()) {
    return clone();
  }
  ...
  //  collect the clusters and merge them into big polygons
  //  NOTE: using the ClusterMerger we merge bottom-up forming bigger and bigger
  //  polygons. This is hopefully more efficient that collecting everything and
  //  will lead to reuse of parts.
...
RegionDelegate *DeepRegion::sized(coord_type d, unsigned int mode) const {
  if (empty()) {
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    db::PolygonRefToShapesGenerator pr(&layout, &st);
    db::PolygonGenerator pg2(pr, false /*don't resolve holes*/,
                             true /*min. coherence*/);
  ...
  //  in case of negative sizing the output polygons will still be merged (on
  //  positive sizing they might overlap after size and are not necessarily
  //  merged)
...
RegionDelegate *DeepRegion::sized(coord_type dx, coord_type dy,
...
static Output *region_cop_impl(DeepRegion *region,
  ...
  for (std::vector<db::Region *>::const_iterator i = inputs.begin();
       i != inputs.end(); ++i) {
    if (!is_subject_regionptr(*i) &&
        !dynamic_cast<const db::DeepRegion *>((*i)->delegate())) {
      return 0;
    }
  }
  ...
  db::local_processor<db::PolygonRef, db::PolygonRef, TR> proc(
      &res->deep_layer().layout(), &res->deep_layer().initial_cell(),
  ...
  for (std::vector<db::Region *>::const_iterator i = inputs.begin();
    ...
    if (is_subject_regionptr(*i)) {
      ...
      const db::DeepRegion *other_deep =
          dynamic_cast<const db::DeepRegion *>((*i)->delegate());
      tl_assert(other_deep != 0);
      if (&other_deep->deep_layer().layout() !=
              &region->deep_layer().layout() ||
          &other_deep->deep_layer().initial_cell() !=
              &region->deep_layer().initial_cell()) {
        throw tl::Exception(
            tl::to_string(tr("Complex DeepRegion operations need to use the "
                             "same layout and top cell for all inputs")));
      }
...
template <class TR, class Output>
static Output *
region_cop_with_properties_impl(DeepRegion *region,
                                db::CompoundRegionOperationNode &node,
  ...
  const db::PropertiesRepository *subject_pr =
      &polygons.layout().properties_repository();
  ...
  compound_local_operation_with_properties<db::PolygonRef, db::PolygonRef, TR>
      op(&node, prop_constraint, res->properties_repository(), subject_pr,
         intruder_prs);
  proc.run(&op, polygons.layer(), other_layers, res->deep_layer().layer());
...
DeepRegion::cop_to_edge_pairs(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    output = region_cop_with_properties_impl<db::EdgePair, DeepEdgePairs>(
        this, node, prop_constraint);
  }
  if (!output) {
...
DeepRegion::cop_to_region(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    output = region_cop_with_properties_impl<db::PolygonRef, DeepRegion>(
        this, node, prop_constraint);
  }
  if (!output) {
...
DeepRegion::cop_to_edges(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    output = region_cop_with_properties_impl<db::Edge, DeepEdges>(
        this, node, prop_constraint);
  }
  if (!output) {
...
EdgePairsDelegate *
DeepRegion::run_check(db::edge_relation_type rel, bool different_polygons,
                      const Region *other, db::Coord d,
                      const RegionCheckOptions &options) const {
  if (empty()) {
    return new db::DeepEdgePairs(deep_layer().derived());
  } else if (other && !is_subject_regionptr(other) && other->empty() &&
             !options.negative) {
    return new db::DeepEdgePairs(deep_layer().derived());
  }

  //  force different polygons in the different properties case to skip
  //  intra-polygon checks
  if (pc_always_different(options.prop_constraint)) {
    //  TODO: this forces merged primaries, so maybe that is not a good
    //  optimization?
  ...
  bool primary_is_merged =
      !merged_semantics() || needs_merged_primary || is_merged();
  ...
  if (other == subject_regionptr()) {
    ...
    if (!other_deep) {
      return db::AsIfFlatRegion::run_check(rel, different_polygons, other, d,
                                           options);
    }
  ...
  const db::DeepLayer &polygons =
      needs_merged_primary ? merged_deep_layer() : deep_layer();
  ...
  const db::Layout *intruder_layout =
      other_deep ? &other_deep->deep_layer().layout() : &polygons.layout();
  const db::Cell *intruder_top = other_deep
                                     ? &other_deep->deep_layer().initial_cell()
                                     : &polygons.initial_cell();
  const std::set<db::cell_index_type> *subject_breakout_cells =
      deep_layer().breakout_cells();
  const std::set<db::cell_index_type> *intruder_breakout_cells =
      other_deep ? other_deep->deep_layer().breakout_cells() : 0;

  if (options.prop_constraint == db::IgnoreProperties) {

    db::CheckLocalOperation op(check, different_polygons, primary_is_merged,
                               other_deep != 0, other_is_merged, options);

    db::local_processor<db::PolygonRef, db::PolygonRef, db::EdgePair> proc(
        subject_layout, subject_top, intruder_layout, intruder_top,
    ...
    db::check_local_operation_with_properties<db::PolygonRef, db::PolygonRef>
        op(check, different_polygons, primary_is_merged, other_deep != 0,
           other_is_merged, options, res->properties_repository(),
           properties_repository(),
           other_deep ? other_deep->properties_repository()
                      : &polygons.layout().properties_repository());

    db::local_processor<db::PolygonRefWithProperties,
                        db::PolygonRefWithProperties,
                        db::EdgePairWithProperties>
        proc(subject_layout, subject_top, intruder_layout, intruder_top,
             subject_breakout_cells, intruder_breakout_cells);
...
EdgePairsDelegate *
DeepRegion::run_single_polygon_check(db::edge_relation_type rel, db::Coord d,
                                     const RegionCheckOptions &options) const {
  if (empty()) {
    return new db::DeepEdgePairs(deep_layer().derived());
  }
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator s =
             shapes.begin(db::ShapeIterator::Polygons);
         !s.at_end(); ++s) {

      edge2edge_check_negative_or_positive<db::Shapes> edge_check(
          check, result, options.negative,
          false /*does not require different polygons*/,
          false /*does not require different layers*/, options.shielded,
          true /*symmetric edge pairs*/,
          pc_remove(options.prop_constraint) ? 0 : s->prop_id());
...
public:
  InteractingResultHolder(InteractingOutputMode output_mode, bool is_merged,
                          const db::DeepLayer &polygons)
...
std::pair<RegionDelegate *, RegionDelegate *>
DeepRegion::in_and_out_generic(const Region &other,
                               InteractingOutputMode output_mode) const {
  if (output_mode == None) {
    return std::pair<RegionDelegate *, RegionDelegate *>((RegionDelegate *)0,
                                                         (RegionDelegate *)0);
  } else if (empty()) {
    ...
    if (output_mode == PositiveAndNegative) {
      ...
      return std::make_pair(new DeepRegion(deep_layer().derived()),
                            (RegionDelegate *)0);
    }
  }
  ...
  const db::DeepRegion *other_deep =
      dynamic_cast<const db::DeepRegion *>(other.delegate());
  if (!other_deep) {
    //  if the other region isn't deep, turn into a top-level only deep region
    //  to facilitate re-hierarchization
    dr_holder.reset(new db::DeepRegion(
        other, const_cast<db::DeepShapeStore &>(*deep_layer().store())));
  ...
  db::local_processor<db::PolygonRef, db::PolygonRef, db::PolygonRef> proc(
      const_cast<db::Layout *>(&polygons.layout()),
      const_cast<db::Cell *>(&polygons.initial_cell()),
      &other_polygons.layout(), &other_polygons.initial_cell(),
      polygons.breakout_cells(), other_polygons.breakout_cells());
  configure_proc(proc);
  proc.set_threads(polygons.store()->threads());
...
std::pair<RegionDelegate *, RegionDelegate *>
DeepRegion::selected_interacting_generic(const Region &other, int mode,
                                         bool touching,
                                         InteractingOutputMode output_mode,
                                         size_t min_count,
  ...
  bool counting =
      !(min_count == 1 && max_count == std::numeric_limits<size_t>::max());
  ...
  const db::DeepLayer &other_polygons =
      counting ? other_deep->merged_deep_layer() : other_deep->deep_layer();

  db::InteractingLocalOperation op(mode, touching, output_mode, min_count,
                                   max_count, true);
...
std::pair<RegionDelegate *, RegionDelegate *>
DeepRegion::selected_interacting_generic(const Edges &other,
                                         InteractingOutputMode output_mode,
                                         size_t min_count,
                                         size_t max_count) const {
  if (output_mode == None) {
  ...
  const db::DeepEdges *other_deep =
      dynamic_cast<const db::DeepEdges *>(other.delegate());
  if (!other_deep) {
    //  if the other region isn't deep, turn into a top-level only deep region
    //  to facilitate re-hierarchization
    dr_holder.reset(new db::DeepEdges(
        other, const_cast<db::DeepShapeStore &>(*deep_layer().store())));
    other_deep = dr_holder.get();
  }
  ...
  db::InteractingWithEdgeLocalOperation op(output_mode, min_count, max_count,
                                           true);

  db::local_processor<db::PolygonRef, db::Edge, db::PolygonRef> proc(
      const_cast<db::Layout *>(&polygons.layout()),
      const_cast<db::Cell *>(&polygons.initial_cell()),
      &other_deep->deep_layer().layout(),
      &other_deep->deep_layer().initial_cell(), polygons.breakout_cells(),
      other_deep->deep_layer().breakout_cells());
  configure_proc(proc);
  proc.set_threads(polygons.store()->threads());
  if (split_after) {
    proc.set_area_ratio(polygons.store()->max_area_ratio());
  ...
  proc.run(&op, polygons.layer(),
           counting ? other_deep->merged_deep_layer().layer()
                    : other_deep->deep_layer().layer(),
           orh.layers());

  return orh.result_pair();
}

RegionDelegate *DeepRegion::pull_generic(const Region &other, int mode,
                                         bool touching) const {
  if (empty()) {
    return clone();
  } else if (other.empty()) {
  ...
  db::DeepRegion *res = new db::DeepRegion(dl_out);
  res->set_is_merged(!split_after &&
                     (other.merged_semantics() || other.is_merged()));
  return res;
}

EdgesDelegate *DeepRegion::pull_generic(const Edges &other) const {
  if (empty() || other.empty()) {
  ...
  db::local_processor<db::PolygonRef, db::Edge, db::Edge> proc(
      const_cast<db::Layout *>(&polygons.layout()),
      const_cast<db::Cell *>(&polygons.initial_cell()), &other_edges.layout(),
      &other_edges.initial_cell(), polygons.breakout_cells(),
      other_edges.breakout_cells());
  configure_proc(proc);
  proc.set_threads(polygons.store()->threads());
...
TextsDelegate *DeepRegion::pull_generic(const Texts &other) const {
  if (empty() || other.empty()) {
    return new DeepTexts(deep_layer().derived());
  }

  std::unique_ptr<db::DeepTexts> dr_holder;
  const db::DeepTexts *other_deep =
      dynamic_cast<const db::DeepTexts *>(other.delegate());
  if (!other_deep) {
    //  if the other region isn't deep, turn into a top-level only deep region
    //  to facilitate re-hierarchization
    dr_holder.reset(new db::DeepTexts(
        other, const_cast<db::DeepShapeStore &>(*deep_layer().store())));
    other_deep = dr_holder.get();
  }
  ...
  db::local_processor<db::PolygonRef, db::TextRef, db::TextRef> proc(
      const_cast<db::Layout *>(&polygons.layout()),
      const_cast<db::Cell *>(&polygons.initial_cell()), &other_texts.layout(),
      &other_texts.initial_cell(), polygons.breakout_cells(),
      other_texts.breakout_cells());
  configure_proc(proc);
  proc.set_threads(polygons.store()->threads());
...
std::pair<RegionDelegate *, RegionDelegate *>
DeepRegion::selected_interacting_generic(const Texts &other,
                                         InteractingOutputMode output_mode,
                                         size_t min_count,
                                         size_t max_count) const {
  if (output_mode == None) {
  ...
  db::local_processor<db::PolygonRef, db::TextRef, db::PolygonRef> proc(
      const_cast<db::Layout *>(&polygons.layout()),
      const_cast<db::Cell *>(&polygons.initial_cell()),
      &other_deep->deep_layer().layout(),
      &other_deep->deep_layer().initial_cell(), polygons.breakout_cells(),
  ...
  proc.run(&op, polygons.layer(), other_deep->deep_layer().layer(),
           orh.layers());
