...
struct ResultCountingInserter {
  ...
  ResultCountingInserter(
      std::unordered_map<const db::Polygon *, size_t,
                         std::ptr_hash_from_value<db::Polygon>> &result)
      : mp_result(&result) {
    //  .. nothing yet ..
  }

  void insert(const db::Polygon &p) { (*mp_result)[&p] += 1; }

  void init(const db::Polygon *p) { (*mp_result)[p] = 0; }

private:
  std::unordered_map<const db::Polygon *, size_t,
                     std::ptr_hash_from_value<db::Polygon>> *mp_result;
};
...
static RegionDelegate *region_from_box(const db::Box &b,
  ...
  if (!b.empty() && b.width() > 0 && b.height() > 0) {
    ...
    if (prop_id != 0) {
      db::PropertyMapper pm(const_cast<db::PropertiesRepository *>(
                                new_region->properties_repository()),
                            pr);
...
AsIfFlatRegion::AsIfFlatRegion() : RegionDelegate(), m_bbox_valid(false) {
  //  .. nothing yet ..
}
...
AsIfFlatRegion &AsIfFlatRegion::operator=(const AsIfFlatRegion &other) {
  if (this != &other) {
...
std::string AsIfFlatRegion::to_string(size_t nmax) const {
...
EdgesDelegate *AsIfFlatRegion::edges(const EdgeFilterBase *filter) const {
  std::unique_ptr<FlatEdges> result(new FlatEdges());
  db::PropertyMapper pm(result->properties_repository(),
...
AsIfFlatRegion::area_type AsIfFlatRegion::area(const db::Box &box) const {
  area_type a = 0;
  ...
  for (RegionIterator p(begin_merged()); !p.at_end(); ++p) {
    if (box.empty() || p->box().inside(box)) {
      ...
      for (std::vector<db::Polygon>::const_iterator c = clipped.begin();
           c != clipped.end(); ++c) {
        a += c->area();
      }
    }
...
void AsIfFlatRegion::invalidate_bbox() { m_bbox_valid = false; }

void AsIfFlatRegion::merge_polygons_to(
    db::Shapes &output, bool min_coherence, unsigned int min_wc,
    db::PropertiesRepository *target_rp) const {
  ...
  if (need_split_props) {
    ...
    std::vector<std::pair<db::properties_id_type, const db::Polygon *>>
        polygons_by_prop_id;
    polygons_by_prop_id.reserve(n);

    db::AddressablePolygonDelivery addressable_polygons(begin());
    while (!addressable_polygons.at_end()) {
      polygons_by_prop_id.push_back(std::make_pair(
          addressable_polygons.prop_id(), addressable_polygons.operator->()));
      addressable_polygons.inc();
    }

    std::sort(polygons_by_prop_id.begin(), polygons_by_prop_id.end());

    for (auto p = polygons_by_prop_id.begin();
         p != polygons_by_prop_id.end();) {
...
AsIfFlatRegion::processed(const PolygonProcessorBase &filter) const {
  ...
  for (RegionIterator p(filter.requires_raw_input() ? begin() : begin_merged());
       !p.at_end(); ++p) {

    poly_res.clear();
    filter.process(*p, poly_res);
    for (std::vector<db::Polygon>::const_iterator pr = poly_res.begin();
         pr != poly_res.end(); ++pr) {
      if (p.prop_id() != 0) {
...
EdgesDelegate *AsIfFlatRegion::processed_to_edges(
    const PolygonToEdgeProcessorBase &filter) const {
  ...
  for (RegionIterator p(filter.requires_raw_input() ? begin() : begin_merged());
    ...
    for (std::vector<db::Edge>::const_iterator er = edge_res.begin();
         er != edge_res.end(); ++er) {
      if (p.prop_id() != 0) {
...
EdgePairsDelegate *AsIfFlatRegion::processed_to_edge_pairs(
  ...
  for (RegionIterator p(filter.requires_raw_input() ? begin() : begin_merged());
    ...
    for (std::vector<db::EdgePair>::const_iterator epr = edge_pair_res.begin();
         epr != edge_pair_res.end(); ++epr) {
      if (p.prop_id() != 0) {
...
public:
  OutputPairHolder(InteractingOutputMode output_mode, bool merged_semantics) {
    ...
    if (output_mode == Positive || output_mode == Negative ||
        output_mode == PositiveAndNegative) {
...
AsIfFlatRegion::in_and_out_generic(const Region &other,
...
std::pair<RegionDelegate *, RegionDelegate *>
AsIfFlatRegion::selected_interacting_generic(const Edges &other,
  ...
  db::interacting_with_edge_local_operation<db::Polygon, db::Edge, db::Polygon>
      op(output_mode, min_count, max_count, true);
...
std::pair<RegionDelegate *, RegionDelegate *>
AsIfFlatRegion::selected_interacting_generic(const Texts &other,
                                             InteractingOutputMode output_mode,
                                             size_t min_count,
                                             size_t max_count) const {
  OutputPairHolder oph(output_mode, merged_semantics() || is_merged());
  ...
  db::interacting_with_text_local_operation<db::Polygon, db::Text, db::Polygon>
      op(output_mode, min_count, max_count);
...
std::pair<RegionDelegate *, RegionDelegate *>
AsIfFlatRegion::selected_interacting_generic(const Region &other, int mode,
                                             bool touching,
                                             InteractingOutputMode output_mode,
                                             size_t min_count,
                                             size_t max_count) const {
  ...
  if (empty()) {
    ...
    //   * mode is inside, enclosing or interacting and inverse is false
  ...
  db::interacting_local_operation<db::Polygon, db::Polygon, db::Polygon> op(
      mode, touching, output_mode, min_count, max_count, true);
...
EdgesDelegate *AsIfFlatRegion::pull_generic(const Edges &other) const {
  if (other.empty()) {
  ...
  std::unique_ptr<FlatEdges> output(
      new FlatEdges(other.merged_semantics() || other.is_merged()));
  std::vector<db::Shapes *> results;
...
TextsDelegate *AsIfFlatRegion::pull_generic(const Texts &other) const {
  db::RegionIterator polygons(begin());
...
RegionDelegate *AsIfFlatRegion::pull_generic(const Region &other, int mode,
                                             bool touching) const {
  db::RegionIterator polygons(begin());

  db::pull_local_operation<db::Polygon, db::Polygon, db::Polygon> op(mode,
                                                                     touching);
  ...
  std::unique_ptr<FlatRegion> output(
      new FlatRegion(other.merged_semantics() || other.is_merged()));
  std::vector<db::Shapes *> results;
  results.push_back(&output->raw_polygons());
...
void AsIfFlatRegion::produce_markers_for_grid_check(const db::Polygon &poly,
                                                    const Trans &tr,
                                                    db::Coord gx, db::Coord gy,
                                                    db::Shapes &shapes) {
  ...
  for (size_t i = 0; i < poly.holes() + 1; ++i) {
    ...
    for (db::Polygon::polygon_contour_iterator pt = b; pt != e; ++pt) {
      ...
      if ((p.x() % gx) != 0 || (p.y() % gy) != 0) {
        shapes.insert(
            EdgePair(db::Edge(p, p), db::Edge(p, p)).transformed(tr_inv));
      }
    }
  }
}

template void AsIfFlatRegion::produce_markers_for_grid_check<db::ICplxTrans>(
    const db::Polygon &poly, const db::ICplxTrans &tr, db::Coord gx,
    db::Coord gy, db::Shapes &shapes);
template void AsIfFlatRegion::produce_markers_for_grid_check<db::UnitTrans>(
    const db::Polygon &poly, const db::UnitTrans &tr, db::Coord gx,
    db::Coord gy, db::Shapes &shapes);

EdgePairsDelegate *AsIfFlatRegion::grid_check(db::Coord gx,
                                              db::Coord gy) const {
  if (gx < 0 || gy < 0) {
    throw tl::Exception(
  ...
  for (RegionIterator p(begin_merged()); !p.at_end(); ++p) {
    produce_markers_for_grid_check(*p, db::UnitTrans(), gx, gy,
                                   res->raw_edge_pairs());
  }
...
template <class Trans>
void AsIfFlatRegion::produce_markers_for_angle_check(const db::Polygon &poly,
                                                     const Trans &tr,
  ...
  for (size_t i = 0; i < poly.holes() + 1; ++i) {
    ...
    for (size_t j = 0; j < np; ++j) {
      ...
      if ((ac_less(cos_a, gt180_a, cos_max, gt180_max) &&
           !ac_less(cos_a, gt180_a, cos_min, gt180_min)) == !inverse) {
...
template void AsIfFlatRegion::produce_markers_for_angle_check<db::ICplxTrans>(
    const db::Polygon &poly, const db::ICplxTrans &tr, double min, double max,
    bool inverse, db::Shapes &shapes);
template void AsIfFlatRegion::produce_markers_for_angle_check<db::UnitTrans>(
    const db::Polygon &poly, const db::UnitTrans &tr, double min, double max,
    bool inverse, db::Shapes &shapes);

EdgePairsDelegate *AsIfFlatRegion::angle_check(double min, double max,
                                               bool inverse) const {
  std::unique_ptr<db::FlatEdgePairs> res(new db::FlatEdgePairs());

  for (RegionIterator p(begin_merged()); !p.at_end(); ++p) {
    produce_markers_for_angle_check(*p, db::UnitTrans(), min, max, inverse,
                                    res->raw_edge_pairs());
  }
...
RegionDelegate *AsIfFlatRegion::snapped(db::Coord gx, db::Coord gy) {
  if (gx < 0 || gy < 0) {
    throw tl::Exception(
        tl::to_string(tr("Grid snap requires a positive grid value")));
  }
...
RegionDelegate *AsIfFlatRegion::scaled_and_snapped(db::Coord gx, db::Coord mx,
                                                   db::Coord dx, db::Coord gy,
  ...
  if (mx <= 0 || dx <= 0 || my <= 0 || dy <= 0) {
    throw tl::Exception(
        tl::to_string(tr("Scale and snap requires positive and non-null "
                         "magnification or divisor values")));
  }
  ...
  for (RegionIterator p(begin_merged()); !p.at_end(); ++p) {
    new_region->raw_polygons().insert(
        scaled_and_snapped_polygon(*p, gx, mx, dx, 0, gy, my, dy, 0, heap));
  }
...
template <class TR>
static void region_cop_impl(AsIfFlatRegion *region, db::Shapes *output_to,
...
template <class TR>
static void
region_cop_with_properties_impl(AsIfFlatRegion *region, db::Shapes *output_to,
                                db::PropertiesRepository *target_pr,
                                db::CompoundRegionOperationNode &node,
                                db::PropertyConstraint prop_constraint) {
  db::local_processor<db::PolygonWithProperties, db::PolygonWithProperties,
  ...
  db::generic_shape_iterator<db::PolygonWithProperties> polygons(
      db::make_wp_iter(region->begin_merged()));
  ...
  compound_local_operation_with_properties<db::Polygon, db::Polygon, TR> op(
      &node, prop_constraint, target_pr, subject_pr, intruder_prs);
  proc.run_flat(polygons, others, foreign, &op, results);
}
...
AsIfFlatRegion::cop_to_edge_pairs(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    region_cop_with_properties_impl<db::EdgePair>(
        this, &output->raw_edge_pairs(), output->properties_repository(), node,
        prop_constraint);
  }
  return output.release();
}
...
AsIfFlatRegion::cop_to_region(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    region_cop_with_properties_impl<db::Polygon>(
        this, &output->raw_polygons(), output->properties_repository(), node,
        prop_constraint);
  }
  return output.release();
}
...
AsIfFlatRegion::cop_to_edges(db::CompoundRegionOperationNode &node,
  ...
  if (pc_skip(prop_constraint)) {
    ...
    region_cop_with_properties_impl<db::Edge>(this, &output->raw_edges(),
                                              output->properties_repository(),
                                              node, prop_constraint);
  }
  return output.release();
}

EdgePairsDelegate *
AsIfFlatRegion::width_check(db::Coord d,
                            const RegionCheckOptions &options) const {
  return run_single_polygon_check(db::WidthRelation, d, options);
}

EdgePairsDelegate *AsIfFlatRegion::space_or_isolated_check(
    db::Coord d, const RegionCheckOptions &options, bool isolated) const {
  if (options.opposite_filter != NoOppositeFilter ||
      options.rect_filter != NoRectFilter || options.shielded) {
    //  NOTE: we have to use the "foreign" scheme with a filter because only
    //  this scheme guarantees that all subject shapes are visited.
    return run_check(db::SpaceRelation, isolated, foreign_regionptr(), d,
                     options);
  } else {
    return run_check(db::SpaceRelation, isolated, subject_regionptr(), d,
                     options);
  }
}

EdgePairsDelegate *
AsIfFlatRegion::space_check(db::Coord d,
                            const RegionCheckOptions &options) const {
  return space_or_isolated_check(d, options, false);
}

EdgePairsDelegate *
AsIfFlatRegion::isolated_check(db::Coord d,
                               const RegionCheckOptions &options) const {
  return space_or_isolated_check(d, options, true);
}

EdgePairsDelegate *
AsIfFlatRegion::notch_check(db::Coord d,
                            const RegionCheckOptions &options) const {
  return run_single_polygon_check(db::SpaceRelation, d, options);
}

EdgePairsDelegate *
AsIfFlatRegion::enclosing_check(const Region &other, db::Coord d,
                                const RegionCheckOptions &options) const {
  return run_check(db::OverlapRelation, true, &other, d, options);
}

EdgePairsDelegate *
AsIfFlatRegion::overlap_check(const Region &other, db::Coord d,
                              const RegionCheckOptions &options) const {
  return run_check(db::WidthRelation, true, &other, d, options);
}

EdgePairsDelegate *
AsIfFlatRegion::separation_check(const Region &other, db::Coord d,
                                 const RegionCheckOptions &options) const {
  return run_check(db::SpaceRelation, true, &other, d, options);
}

EdgePairsDelegate *
AsIfFlatRegion::inside_check(const Region &other, db::Coord d,
                             const RegionCheckOptions &options) const {
  return run_check(db::InsideRelation, true, &other, d, options);
}

EdgePairsDelegate *
AsIfFlatRegion::run_check(db::edge_relation_type rel, bool different_polygons,
  ...
  if (pc_skip(options.prop_constraint)) {
    ...
    db::check_local_operation<db::Polygon, db::Polygon> op(
        check, different_polygons, primary_is_merged, has_other,
        other_is_merged, options);
    ...
    const db::PropertiesRepository *intruder_pr =
        (other == subject_regionptr() || other == foreign_regionptr())
            ? subject_pr
            : &other->properties_repository();

    db::check_local_operation_with_properties<db::Polygon, db::Polygon> op(
        check, different_polygons, primary_is_merged, has_other,
        other_is_merged, options, output->properties_repository(), subject_pr,
        intruder_pr);
    ...
    std::vector<db::generic_shape_iterator<db::PolygonWithProperties>>
        others_wp;
    for (auto o = others.begin(); o != others.end(); ++o) {
      others_wp.push_back(db::make_wp_iter(std::move(*o)));
    }

    proc.run_flat(db::make_wp_iter(std::move(polygons)), others_wp, foreign,
                  &op, results);
  }
...
EdgePairsDelegate *AsIfFlatRegion::run_single_polygon_check(
    db::edge_relation_type rel, db::Coord d,
    const RegionCheckOptions &options) const {
  ...
  for (RegionIterator p(begin_merged()); !p.at_end(); ++p) {
    ...
    edge2edge_check_negative_or_positive<db::Shapes> edge_check(
        check, result->raw_edge_pairs(), options.negative,
        false /*=same polygons*/, false /*=same layers*/, options.shielded,
        true /*symmetric edge pairs*/,
        pc_remove(options.prop_constraint) ? 0 : pm(p.prop_id()));
...
RegionDelegate *AsIfFlatRegion::merged(bool min_coherence,
...
RegionDelegate *AsIfFlatRegion::sized(coord_type d, unsigned int mode) const {
  return sized(d, d, mode);
}

RegionDelegate *AsIfFlatRegion::sized(coord_type dx, coord_type dy,
  ...
  if (empty()) {
    ...
    //  Generic case
    std::unique_ptr<FlatRegion> new_region(new FlatRegion());
    db::PropertyMapper pm(new_region->properties_repository(),
                          properties_repository());
...
RegionDelegate *
AsIfFlatRegion::and_with(const Region &other,
  ...
  if (empty() || other.empty()) {
    ...
    if (pc_skip(property_constraint) ||
        pc_match(property_constraint, begin()->prop_id(),
                 other.begin().prop_id())) {
      ...
      db::properties_id_type prop_id_out =
          pc_norm(property_constraint, begin()->prop_id());
    ...
    db::properties_id_type self_prop_id =
        pc_skip(property_constraint) ? 0 : begin()->prop_id();
    ...
    db::properties_id_type prop_id_out =
        pm(pc_norm(property_constraint, self_prop_id));
      ...
      if (pc_match(property_constraint, self_prop_id, prop_id)) {
        ...
        if (prop_id_out == 0) {
          ...
          for (auto i = clipped.begin(); i != clipped.end(); ++i) {
            new_region->raw_polygons().insert(
                db::PolygonWithProperties(*i, prop_id_out));
          }
        }
      }
    }
    ...
    db::properties_id_type other_prop_id =
        pc_skip(property_constraint) ? 0 : other.begin().prop_id();
    ...
    for (RegionIterator p(begin()); !p.at_end(); ++p) {
      ...
      if (pc_match(property_constraint, prop_id, other_prop_id)) {
        ...
        db::properties_id_type prop_id_out =
            pm(pc_norm(property_constraint, prop_id));
        if (prop_id_out == 0) {
          new_region->raw_polygons().insert(clipped.begin(), clipped.end());
        } else {
          for (auto i = clipped.begin(); i != clipped.end(); ++i) {
...
RegionDelegate *
AsIfFlatRegion::not_with(const Region &other,
                         PropertyConstraint property_constraint) const {
  if (empty()) {
...
RegionDelegate *
AsIfFlatRegion::and_or_not_with(bool is_and, const Region &other,
                                PropertyConstraint property_constraint) const {
  if (pc_skip(property_constraint)) {
    ...
    db::generic_shape_iterator<db::PolygonWithProperties> polygons(
        db::make_wp_iter(begin()));

    std::unique_ptr<FlatRegion> output(new FlatRegion());
    std::vector<db::Shapes *> results;
    results.push_back(&output->raw_polygons());

    db::bool_and_or_not_local_operation_with_properties<
        db::Polygon, db::Polygon, db::Polygon>
        op(is_and, output->properties_repository(), properties_repository(),
           &other.properties_repository(), property_constraint);
...
std::pair<RegionDelegate *, RegionDelegate *>
AsIfFlatRegion::andnot_with(const Region &other,
                            PropertyConstraint property_constraint) const {
  if (empty()) {
    ...
    db::PolygonGenerator pg1(pc1, false /*don't resolve holes*/,
                             min_coherence());
    ...
    db::PolygonGenerator pg2(pc2, false /*don't resolve holes*/,
                             min_coherence());
    ...
    db::two_bool_and_not_local_operation_with_properties<
        db::Polygon, db::Polygon, db::Polygon>
        op(output1->properties_repository(), output2->properties_repository(),
           properties_repository(), &other.properties_repository(),
           property_constraint);
...
RegionDelegate *
AsIfFlatRegion::xor_with(const Region &other,
                         PropertyConstraint prop_constraint) const {
...
RegionDelegate *
AsIfFlatRegion::or_with(const Region &other,
                        PropertyConstraint /*prop_constraint*/) const {
  if (empty() && !other.strict_handling()) {
...
RegionDelegate *AsIfFlatRegion::add(const Region &other) const {
  const FlatRegion *other_flat =
      dynamic_cast<const FlatRegion *>(other.delegate());
  if (other_flat) {
    ...
    std::unique_ptr<FlatRegion> new_region(
        new FlatRegion(false /*not merged*/));
...
static void deliver_shapes_of_nets_recursive(
    db::Shapes &out, db::PropertiesRepository *pr, const db::Circuit *circuit,
    const LayoutToNetlist *l2n, const db::Region *of_layer,
    NetPropertyMode prop_mode, const tl::Variant &net_prop_name,
    const db::ICplxTrans &tr, const std::set<const db::Net *> *net_filter) {
  ...
  for (auto n = circuit->begin_nets(); n != circuit->end_nets(); ++n) {
    ...
    if (!net_filter || net_filter->find(n.operator->()) != net_filter->end()) {
      db::properties_id_type prop_id = db::NetBuilder::make_netname_propid(
          *pr, prop_mode, net_prop_name, *n);
      l2n->shapes_of_net(*n, *of_layer, true, out, prop_id, tr);
    }

    //  dive into subcircuits
    for (auto sc = circuit->begin_subcircuits();
      ...
      deliver_shapes_of_nets_recursive(out, pr, circuit_ref, l2n, of_layer,
                                       prop_mode, net_prop_name, tr_ref,
                                       net_filter);
    }
  }
}
...
RegionDelegate *
AsIfFlatRegion::nets(LayoutToNetlist *l2n, NetPropertyMode prop_mode,
                     const tl::Variant &net_prop_name,
                     const std::vector<const db::Net *> *net_filter) const {
  ...
  if (l2n->netlist()->top_circuit_count() == 0) {
    ...
    throw tl::Exception(
        tl::to_string(tr("More than one top circuit found in netlist")));
  }
  const db::Circuit *top_circuit =
      l2n->netlist()->begin_top_down().operator->();
  ...
  deliver_shapes_of_nets_recursive(
      result->raw_polygons(), result->properties_repository(), top_circuit, l2n,
      region_for_layer.get(), prop_mode, net_prop_name, db::ICplxTrans(),
      net_filter ? &net_filter_set : 0);

  return result.release();
}

void AsIfFlatRegion::insert_into(Layout *layout, db::cell_index_type into_cell,
                                 unsigned int into_layer) const {
  //  improves performance when inserting an original layout into the same
  //  layout
  db::LayoutLocker locker(layout);
  db::PropertyMapper pm(&layout->properties_repository(),
                        properties_repository());
