...
public:
  ...
  box_tree_picker(const BoxConv &box_conv) : m_box_conv(box_conv) {}

  box_type operator()(const Obj *o) const { return m_box_conv(*o); }

  void rotate_boxes(int /*q*/, const_iterator /*e*/, const_iterator /*o0*/,
                    const_iterator /*o1*/, const_iterator /*o2*/,
                    const_iterator /*o3*/, const_iterator /*o4*/) {
    //  .. nothing yet ..
  }
...
/// @brief a helper class providing a linear-time iterator difference which is
/// not necessarily
///        the actual difference but monotonous

template <class X, bool R>
size_t box_tree_lt_difference(const tl::reuse_vector_const_iterator<X, R> &a,
                              const tl::reuse_vector_const_iterator<X, R> &b) {
  return a.index() - b.index();
}

template <class X, bool R>
size_t
box_tree_lt_difference_ptr(const X *a,
                           const tl::reuse_vector_const_iterator<X, R> &b) {
...
public:
  ...
  box_tree_cached_picker(const BoxConv &box_conv, const_iterator from,
                         const_iterator to)
      : m_from(from) {
  ...
  const box_type &bbox() const { return m_bbox; }

  void rotate_boxes(int q, const_iterator e, const_iterator o0,
                    const_iterator o1, const_iterator o2, const_iterator o3,
                    const_iterator o4) {
    size_t qi[5] = {
        box_tree_lt_difference(o0, m_from), box_tree_lt_difference(o1, m_from),
        box_tree_lt_difference(o2, m_from), box_tree_lt_difference(o3, m_from),
...
template <class Tree> class box_tree_node {
public:
  ...
  box_tree_node(box_tree_node *parent, const point_type &center,
                const box_type &qbox, unsigned int quad) {
    point_type corner;
    if (quad == 0) {
  ...
  const point_type &center() const { return m_center; }
...
private:
  ...
  box_tree_node(box_tree_node *parent, const point_type &center,
                const point_type &corner, unsigned int quad) {
    init(parent, center, corner, quad);
  }

  void init(box_tree_node *parent, const point_type &center,
            const point_type &corner, unsigned int quad) {
    m_center = center;
    m_corner = corner;
 ...
 *  This iterator is therefore recommended for being used in interleaved
 * access/change operations.
 */

template <class Tree> class box_tree_flat_it {
public:
  ...
  box_tree_flat_it() : mp_tree(0) { m_index = 0; }

  box_tree_flat_it(const Tree &t) : mp_tree(&t) { m_index = 0; }
...
template <class Tree, class Cmp> class box_tree_it {
public:
  typedef typename Tree::object_type object_type;
  typedef typename Tree::point_type point_type;
  typedef typename Tree::coord_type coord_type;
  ...
  box_tree_it() : mp_tree(0), m_picker(), m_compare() {
    mp_node = 0;
    m_index = 0;
    m_offset = 0;
    m_quad = -1;
  ...
  size_t position() const { return m_index + m_offset; }
   ...
   *
   *  The quad ID is a value that can be used to determine whether the iterator
   * entered the next quad. It is possible to optimize the search if there is no
   * need to look into that quad and skip it.
   */
...
private:
  ...
  bool need_visit() const {
    ...
    if (m_quad < 0) {
      ...
      //  TODO: simplify this, i.e. by providing a special method on the compare
      //  function ..
...
public:
  ...
  box_tree_sel() : m_b(), m_bpred(), m_conv() {
    //  .. nothing yet ..
  }
  ...
  bool matches_obj(const Obj &o) const { return m_bpred(m_conv(o), m_b); }

  bool matches_box(const Box &b) const { return m_bpred(b, m_b); }
...
template <class Box, class Obj, class BoxConv, size_t min_bin = 100,
          size_t min_quads = 100, unsigned int thin_aspect = 4>
class box_tree {
public:
  typedef Box box_type;
  ...
  typedef box_tree<box_type, object_type, box_conv_type, min_bin, min_quads>
      box_tree_type;
  typedef db::box_tree_node<box_tree_type> box_tree_node;
  typedef box_tree_sel<box_type, object_type, box_conv_type,
                       db::boxes_overlap<box_type>>
      box_tree_sel_overlap_type;
  typedef box_tree_sel<box_type, object_type, box_conv_type,
                       db::boxes_touch<box_type>>
      box_tree_sel_touch_type;
  typedef box_tree_it<box_tree_type, box_tree_sel_touch_type> touching_iterator;
  typedef box_tree_it<box_tree_type, box_tree_sel_overlap_type>
      overlapping_iterator;
  typedef box_tree_flat_it<box_tree_type> flat_iterator;
  typedef box_tree_picker<Box, Obj, BoxConv, obj_vector_type>
      box_tree_picker_type;
  ...
  box_tree(const box_tree &b)
      : m_objects(b.m_objects), m_elements(b.m_elements),
        mp_root(b.mp_root ? b.mp_root->clone() : 0) {
    // .. nothing else ..
  }
   ...
   */
  iterator insert(const Obj &o) { return m_objects.insert(o); }

  /**
   *  @brief reserve space for a given number of elements
   */
  void reserve(size_t n) { m_objects.reserve(n); }

  /**
   *  @brief resize to a given number of elements
   */
  void resize(size_t n) { m_objects.resize(n); }
   ...
   */
  void erase(iterator pos) { m_objects.erase(pos); }
   ...
   *  @param pos The position of the object to erase
   */
  void erase(iterator from, iterator to) { m_objects.erase(from, to); }

  /**
   *  @brief Erase several objects
   *
   *  Erase the objects given by a set of iterators (given by a sequence
   * [from,to)). The iterators given must be sorted in ascending order. Erasing
   * an object will invalidate sorting. Erasing is not a cheap operation and its
   * complexity is O(n).
   *
  ...
  void make_index() {
    ...
    for (typename obj_vector_type::const_iterator o = m_objects.begin();
         o != m_objects.end(); ++o) {
   ...
   */
  const obj_vector_type &objects() const { return m_objects; }
   ...
   */
  const element_vector_type &elements() const { return m_elements; }

  /**
   *  @brief Get the iterator for an object given by a pointer (non-const
   * version)
   */
   ...
   */
  template <class V> bool is_member_of(const V *p) const {
   ...
   */
  box_tree_node *root() const { return mp_root; }
...
private:
  ...
  void sort(const box_conv_type &conv,
            const db::complex_bbox_tag & /*complexity*/) {
    m_elements.clear();
    m_elements.reserve(m_objects.size());
    ...
    if (!m_objects.empty()) {
      ...
      box_tree_cached_picker<object_type, box_type, box_conv_type,
                             obj_vector_type>
          picker(conv, m_objects.begin(), m_objects.end());
      ...
      tree_sort(0, m_elements.begin(), m_elements.end(), picker, picker.bbox(),
                0);
    }
  }

  template <class CoordPicker>
  void tree_sort(box_tree_node *parent, element_iterator from,
                 element_iterator to, const CoordPicker &picker,
                 const box_type &bbox, int quad) {
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat,
                     const box_tree<Box, Obj, BoxConv, min_bin, min_quads> &x,
                     bool no_self = false, void *parent = 0) {
...
template <class Tree> class unstable_box_tree_flat_it {
public:
  typedef typename Tree::object_type object_type;
  typedef typename Tree::point_type point_type;
  typedef typename Tree::coord_type coord_type;
  ...
  unstable_box_tree_flat_it() : mp_tree(0) { m_index = 0; }

  unstable_box_tree_flat_it(const Tree &t) : mp_tree(&t) { m_index = 0; }
  ...
  const object_type &operator*() const { return mp_tree->objects()[m_index]; }
   ...
   *  @brief returns true if at end
   */
  bool at_end() const { return mp_tree == 0 || m_index == mp_tree->size(); }
...
template <class Tree, class Cmp> class unstable_box_tree_it {
public:
  typedef typename Tree::object_type object_type;
  typedef typename Tree::point_type point_type;
  typedef typename Tree::coord_type coord_type;
  ...
  unstable_box_tree_it() : mp_tree(0), m_picker(), m_compare() {
    mp_node = 0;
    m_index = 0;
    m_offset = 0;
    m_quad = -1;
...
public:
  ...
  typedef typename obj_vector_type::const_iterator const_iterator;
  typedef typename obj_vector_type::iterator iterator;
  typedef unstable_box_tree<box_type, object_type, box_conv_type, min_bin,
                            min_quads>
      box_tree_type;
  typedef db::box_tree_node<box_tree_type> box_tree_node;
  typedef box_tree_sel<box_type, object_type, box_conv_type,
  ...
  typedef unstable_box_tree_it<box_tree_type, box_tree_sel_touch_type>
      touching_iterator;
  typedef unstable_box_tree_it<box_tree_type, box_tree_sel_overlap_type>
      overlapping_iterator;
  typedef box_tree_picker<box_type, object_type, box_conv_type, obj_vector_type>
      box_tree_picker_type;

  /**
   *  @brief Creates a empty box tree object
   */
  unstable_box_tree() : mp_root(0) {
    // .. nothing else ..
  }
   ...
   *  not a cheap operation and its complexity is O(n).
   *
   *  @param pos The positions of the objects to erase. This vector must be
   * sorted, otherwise this method will assert.
   */
  ...
  touching_iterator begin_touching(const Box &b, BoxConv conv) const {
    box_tree_picker_type p(conv);
    box_tree_sel_touch_type s(b, conv);
    return unstable_box_tree_it<box_tree_type, box_tree_sel_touch_type>(*this,
                                                                        p, s);
  }
  ...
  overlapping_iterator begin_overlapping(const Box &b, BoxConv conv) const {
    box_tree_picker_type p(conv);
    box_tree_sel_overlap_type s(b, conv);
    return unstable_box_tree_it<box_tree_type, box_tree_sel_overlap_type>(*this,
                                                                          p, s);
  }
...
private:
  ...
  template <class CoordPicker>
  void tree_sort(box_tree_node *parent, obj_iterator from, obj_iterator to,
                 CoordPicker &picker, const box_type &bbox, int quad) {
    size_t ntot = size_t(to - from);
    if (ntot <= min_bin || (bbox.width() < 2 && bbox.height() < 2)) {
      return; //  not worth splitting
    }
    ...
    for (obj_iterator e = from; e != to; ++e) {
      ...
      if (q < 4) {
        ...
        //  since for the unstable tree there is no guarantee of stability of
        //  the objects, we have to rotate the box cache of the caching picker
        //  as well:
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const db::unstable_box_tree<Box, Obj, BoxConv> &x,
                     bool no_self = false, void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
