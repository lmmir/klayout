...
ClipboardData::ClipboardData() : m_layout(), m_incomplete_cells() {
...
void ClipboardData::add(const db::Layout &layout, unsigned int layer,
                        const db::Shape &shape) {
  //  create the layer in our temporary layout if we need to
  //  HINT: this requires all add operations are done from the same layout
  //  object
  ...
  m_layout.cell(m_container_cell_index)
      .shapes(layer)
      .insert(shape, m_prop_id_map);
}

void ClipboardData::add(const db::Layout &layout, unsigned int layer,
                        const db::Shape &shape, const db::ICplxTrans &trans) {
  //  create the layer in our temporary layout if we need to
  //  HINT: this requires all add operations are done from the same layout
  //  object
  if (!m_layout.is_valid_layer(layer)) {
  ...
  m_prop_id_map.set_source(&layout);
  db::Shape new_shape = m_layout.cell(m_container_cell_index)
                            .shapes(layer)
                            .insert(shape, m_prop_id_map);
  m_layout.cell(m_container_cell_index)
      .shapes(layer)
      .transform(new_shape, trans);
}

void ClipboardData::add(const db::Layout &layout, const db::Instance &inst,
                        unsigned int mode) {
  ...
  //  Insert the instance mapping the cell to the target cell_index and the
  //  property ID using the map
  m_prop_id_map.set_source(&layout);
...
void ClipboardData::add(const db::Layout &layout, const db::Instance &inst,
  ...
  //  in mode 1 (deep) first add the target cell
  //  (don't use deep mode for proxy cells because we use the context
  //  information to restore a proxy cell)
  if (mode == 1 && !layout.cell(source_cell_index).is_proxy()) {
    const db::Cell &target_cell = layout.cell(source_cell_index);
    target_cell_index = add(layout, target_cell, 1);
  } else {
...
db::cell_index_type ClipboardData::add(const db::Layout &layout,
                                       const db::Cell &cell,
                                       unsigned int mode) {
  ...
  std::map<db::cell_index_type, db::cell_index_type>::const_iterator cm =
      m_cell_index_map.find(cell.cell_index());
  if (cm != m_cell_index_map.end() &&
      !(m_incomplete_cells.find(cm->second) != m_incomplete_cells.end() &&
        mode >= 1)) {
    return cm->second;
  }

  db::cell_index_type target_cell_index =
      cell_for_cell(layout, cell.cell_index(), mode == 0);
  if (mode >= 1) {
  ...
  for (unsigned int l = 0; l < layout.layers(); ++l) {
    ...
    if (layout.is_valid_layer(l)) {
      ...
      for (db::Shapes::shape_iterator sh =
               cell.shapes(l).begin(db::Shapes::shape_iterator::All);
           !sh.at_end(); ++sh) {
  ...
  for (db::Instances::const_iterator inst = cell.begin(); !inst.at_end();
       ++inst) {
    add(layout, *inst,
        mode == 2 ? 0 /*continue with "incomplete" on the next level*/ : 1);
  }
  std::swap(m_container_cell_index, target_cell_index);
...
ClipboardData::do_insert(db::Layout &layout, const db::ICplxTrans *trans,
                         db::Cell *cell,
  ...
  for (db::Layout::const_iterator c = m_layout.begin(); c != m_layout.end();
       ++c) {

    if (c->cell_index() != m_container_cell_index) {

      std::map<db::cell_index_type, std::vector<std::string>>::const_iterator
          ctx = m_context_info.find(c->cell_index());
      if (ctx != m_context_info.end()) {

        //  remember current layers
        std::set<unsigned int> layers;
        for (db::LayerIterator l = layout.begin_layers();
        ...
        db::Cell *pc =
            layout.recover_proxy(ctx->second.begin(), ctx->second.end());
        ...
        std::pair<bool, db::cell_index_type> ci =
            layout.cell_by_name(m_layout.cell_name(c->cell_index()));
        if (m_incomplete_cells.find(c->cell_index()) !=
            m_incomplete_cells.end()) {
          if (ci.first) {
            cell_map.insert(std::make_pair(c->cell_index(), ci.second));
          } else {
            //  make ghost cells from incomplete new ones.
            db::cell_index_type tc =
                layout.add_cell(m_layout.cell_name(c->cell_index()));
  ...
  for (unsigned int l = 0; l < m_layout.layers(); ++l) {
    if (m_layout.is_valid_layer(l)) {
      ...
      std::map<db::LayerProperties, unsigned int,
               db::LPLogicalLessFunc>::const_iterator lm =
          layer_map.find(m_layout.get_properties(l));
      if (lm != layer_map.end()) {
        tl = lm->second;
      } else {
      ...
      for (db::Layout::const_iterator c = m_layout.begin(); c != m_layout.end();
        ...
        std::map<db::cell_index_type, db::cell_index_type>::const_iterator cp =
            cell_map.find(c->cell_index());
        if (cp != cell_map.end()) {

          db::Shapes &t = layout.cell(cp->second).shapes(tl);

          for (db::Shapes::shape_iterator sh =
                   c->shapes(l).begin(db::Shapes::shape_iterator::All);
               !sh.at_end(); ++sh) {
  ...
  for (db::Layout::const_iterator c = m_layout.begin(); c != m_layout.end();
    ...
    if (cp != cell_map.end()) {
      ...
      for (db::Cell::const_iterator inst = c->begin(); !inst.at_end(); ++inst) {
        ...
        tl::const_map<db::cell_index_type> im(
            cell_map.find(inst->cell_index())->second);
  ...
  if (new_tops) {
    for (db::Layout::top_down_const_iterator tc = m_layout.begin_top_down();
         tc != m_layout.end_top_cells(); ++tc) {
...
db::cell_index_type ClipboardData::cell_for_cell(const db::Layout &layout,
                                                 db::cell_index_type cell_index,
                                                 bool incomplete) {
  std::map<db::cell_index_type, db::cell_index_type>::const_iterator cm =
      m_cell_index_map.find(cell_index);
  if (cm != m_cell_index_map.end()) {
    return cm->second;
  }
