...
template <class X> inline bool default_compression() { return true; }

template <> inline bool default_compression<db::Coord>() { return true; }

template <> inline bool default_compression<db::DCoord>() { return false; }
...
template <class C> class DB_PUBLIC polygon_contour {
public:
  ...
  typedef polygon_contour_iterator<polygon_contour, db::unit_trans<C>>
      simple_iterator;

private:
  /**
   *  @brief A helper predicate function that returns true if p1-p2 is colinear
   * with p2-p3
   */
  static bool is_colinear(const point_type &p1, const point_type &p2,
                          const point_type &p3, bool remove_reflected) {
    if (db::coord_traits<C>::vprod_sign(p1.x(), p1.y(), p3.x(), p3.y(), p2.x(),
                                        p2.y()) == 0) {
      return remove_reflected ||
             (db::coord_traits<C>::sprod_sign(p1.x(), p1.y(), p3.x(), p3.y(),
                                              p2.x(), p2.y()) < 0);
    } else {
      return false;
    }
...
public:
   ...
   */
  polygon_contour() : mp_points(0), m_size(0) {
    //  .. nothing yet ..
  }
   ...
   *  @brief Copy ctor
   */
  polygon_contour(const polygon_contour &d) : m_size(d.m_size) {
    if (d.mp_points == 0) {
      mp_points = 0;
    } else {
   ...
   *  @param compress true, if the sequence shall be compressed (colinear
   * segments joined)
   *  @param remove_reflected true, to remove reflecting spikes if compress is
   * true
   */
  template <class Iter>
  polygon_contour(Iter from, Iter to, bool hole,
                  bool compress = default_compression<C>(),
                  bool normalize = true, bool remove_reflected = false)
  ...
  polygon_contour(Iter from, Iter to, Trans tr, bool hole,
                  bool compress = default_compression<C>(),
                  bool normalize = true, bool remove_reflected = false)
      : mp_points(0), m_size(0) {
   ...
   *  @param to End of the sequence
   *  @param hole true, if a hole is constructed, false for a hull
   *  @param compress true, if the sequence shall be compressed (colinear points
   * removed)
   *  @param remove_reflected true, to remove reflecting spikes if compress is
   * true
   */
  template <class Iter>
  void assign(Iter from, Iter to, bool hole,
              bool compress = default_compression<C>(), bool normalize = true,
              bool remove_reflected = false) {
    assign(from, to, db::unit_trans<C>(), hole, compress, normalize,
           remove_reflected);
  }
   ...
   */
  template <class Iter, class Trans>
  void assign(Iter from, Iter to, Trans tr, bool hole,
              bool compress = default_compression<C>(), bool normalize = true,
              bool remove_reflected = false) {
    if (compress && remove_reflected) {
      ...
      //  the remove_reflected case is somewhat more complicated: it may happen
      //  that bends vanish because the next edge is member of a spike edge
      //  pair. Thus the simple single-pass approach no longer holds. We
      //  therefore need to create an intermediate buffer that is used to
      //  eliminate redundant points successively.
      ...
      while (any_skipped) {
        ...
        do {
          ...
          if (pcurr == plast || pcurr == pnext ||
              is_colinear(plast, pcurr, pnext, remove_reflected)) {
            //  skip this point ..
            any_skipped = true;
          } else {
      ...
      assign(points.begin(), points.end(), db::unit_trans<C>(), hole, compress,
             normalize, false);
      ...
      //  count distinct points, determine minimum and if the contour is
      //  manhattan
      size_type n = 0;
      ...
      //  manhattan
      bool ortho =
          normalize; // don't compress manhattan sequences if not normalizing -
                     // normalisation is a prerequisite for compression!
      size_type n = 0;
      ...
      do {
        ...
        if (pcurr.equal(plast) || pcurr.equal(pnext) ||
            is_colinear(plast, pcurr, pnext, remove_reflected)) {
          ...
          //  there is a strict criterion what is "manhattan": only such segment
          //  pairs that really bend by 90 degree are recognized as manhattan.
          if (ortho && !((coord_traits::equals(plast.x(), pcurr.x()) &&
                          !coord_traits::equals(plast.y(), pcurr.y()) &&
                          !coord_traits::equals(pcurr.x(), pnext.x()) &&
                          coord_traits::equals(pcurr.y(), pnext.y())) ||
                         (!coord_traits::equals(plast.x(), pcurr.x()) &&
                          coord_traits::equals(plast.y(), pcurr.y()) &&
                          coord_traits::equals(pcurr.x(), pnext.x()) &&
                          !coord_traits::equals(pcurr.y(), pnext.y())))) {
            //  non-manhattan
      ...
      if (ortho) {
        ...
        while (n < m_size) {
          do {
            ...
            pnext = point_type(tr(*p));
          } while (coord_traits::equals(plast.x(), pcurr.x()) ||
                   coord_traits::equals(plast.y(), pcurr.y()) ||
                   coord_traits::equals(pnext.x(), pcurr.x()) != eqx ||
                   coord_traits::equals(pnext.y(), pcurr.y()) != eqy);
          plast = pts[n++] = pcurr;
        }
      ...
      //下面是骚操作，这个类是8字节大小，正常分配的地址是能被4整除的，通过拨动指针，来表示该数据结构的一些标记。
      //比如分配指针的时候，是0x40000, 如果这个hole是true ,ortho
  ...
  polygon_contour<C> &transform(const Trans &tr,
                                bool compress = default_compression<C>(),
                                bool remove_reflected = false) {
    ...
    assign(buffer.begin(), buffer.end(), tr, is_hole(), compress, true,
           remove_reflected);
    return *this;
  }
   ...
   *  @return The transformed contour.
   */
  polygon_contour<C> &transform(const trans_type &tr,
                                bool compress = default_compression<C>(),
                                bool remove_reflected = false) {
   ...
   */
  template <class Tr>
  polygon_contour<typename Tr::target_coord_type> transformed(
      const Tr &t,
      bool compress = default_compression<typename Tr::target_coord_type>(),
      bool remove_reflected = false) const {
    //  construct the transformed polygon
    typedef polygon_contour_iterator<polygon_contour<C>, db::unit_trans<C>>
        iter;
    return polygon_contour<typename Tr::target_coord_type>(
        iter(this, 0), iter(this, size()), t, is_hole(), compress, true,
        remove_reflected);
  }
   ...
   */
  simple_iterator begin() const { return simple_iterator(this, 0); }

  /**
   *  @brief end iterator for the points of this contour
   */
  simple_iterator end() const { return simple_iterator(this, size()); }
  ...
  bool is_rectilinear() const {
    ...
    for (size_t i = 0; i < m_size; ++i) {
      ...
      if (!coord_traits::equals(p.x(), pl.x()) &&
          !coord_traits::equals(p.y(), pl.y())) {
        return false;
      }
      pl = p;
    }
  ...
  /**
   *  @brief returns true if the contour is a half-manhattan contour (multiples
   * of 45 degree)
   */
  bool is_halfmanhattan() const {
    ...
    for (size_t i = 0; i < m_size; ++i) {
      ...
      if (!coord_traits::equals(p.x(), pl.x()) &&
          !coord_traits::equals(p.y(), pl.y()) &&
          !coord_traits::equals(std::abs(p.x() - pl.x()),
                                std::abs(p.y() - pl.y()))) {
        return false;
      }
      pl = p;
   ...
   *
   *  Since this method employs the orientation property the results are only
   * valid for contours with more than 2 points.
   */
  bool is_hole() const { return ((size_t)mp_points & 2) != 0; }

  /**
   *  @brief The area of the contour
   */
  area_type area() const { return area2() / 2; }
  ...
  point_type operator[](size_type index) const {
    ...
    if ((f & 1) != 0) {
      if ((index & 1) != 0) {
        if ((f & 2) != 0) {
          return point_type(pts[((index + 1) / 2) % m_size].x(),
                            pts[(index - 1) / 2].y());
        } else {
          return point_type(pts[(index - 1) / 2].x(),
                            pts[((index + 1) / 2) % m_size].y());
        }
      } else {
        return pts[index / 2];
      }
   ...
   */
  bool operator!=(const polygon_contour<C> &d) const { return !operator==(d); }
   ...
   */
  bool not_equal(const polygon_contour<C> &d) const { return !equal(d); }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    stat->add(typeid(point_type[]), (void *)mp_points,
              sizeof(point_type) * m_size, sizeof(point_type) * m_size,
              (void *)this, purpose, cat);
  }
...
public:
   ...
   */
  polygon_contour_iterator() : mp_contour(0), m_index(0), m_reverse(false) {
    //  .. nothing yet ..
  }
   ...
   */
  polygon_contour_iterator(const Contour *contour, size_t n,
                           bool reverse = false)
   ...
   */
  template <class T>
  polygon_contour_iterator(const polygon_contour_iterator<Contour, T> &d,
                           const trans_type &trans, bool reverse = false)
      : mp_contour(d.mp_contour), m_index(d.m_index), m_trans(trans),
        m_reverse(reverse) {
    //  .. nothing yet ..
  }
   ...
   *  @brief Point access
   */
  point_type operator*() const { return m_trans((*mp_contour)[m_index]); }
   ...
   */
  polygon_contour_iterator &operator++() { return operator+=(1); }
...
template <class P, class Tr> class polygon_edge_iterator {
public:
  ...
  polygon_edge_iterator(const polygon_type &polygon)
      : mp_polygon(&polygon), m_ctr(0), m_num_ctr(polygon.holes() + 1), m_pt(0),
        m_trans() {
  ...
  polygon_edge_iterator(const polygon_type &polygon, unsigned int ctr)
      : mp_polygon(&polygon), m_ctr(ctr),
        m_num_ctr(std::min(polygon.holes(), ctr) + 1), m_pt(0), m_trans() {
  ...
  /**
   *  @brief The standard constructor for one specific contour with a
   * transformation
   */
  polygon_edge_iterator(const polygon_type &polygon, unsigned int ctr,
                        const trans_type &trans)
      : mp_polygon(&polygon), m_ctr(ctr),
        m_num_ctr(std::min(polygon.holes(), ctr) + 1), m_pt(0), m_trans(trans) {
    //  The contour may be "empty"
    while (m_ctr < m_num_ctr && mp_polygon->contour(m_ctr).size() == 0) {
      ++m_ctr;
    }
   ...
   */
  bool at_end() const { return m_ctr >= m_num_ctr; }

  /**
   *  @brief Gets the contour number
   */
  unsigned int contour() const { return m_ctr; }
...
private:
  ...
  const contour_type *get_ctr() const { return &(mp_polygon->contour(m_ctr)); }
};
...
template <class C> class DB_PUBLIC_TEMPLATE polygon {
public:
  typedef C coord_type;
  typedef db::edge<coord_type> edge_type;
  ...
  typedef db::polygon_edge_iterator<polygon<C>, db::unit_trans<C>>
      polygon_edge_iterator;
  typedef db::polygon_contour_iterator<contour_type, db::unit_trans<C>>
   ...
   *
   *  The transformation must have the ability to transform a point to another
   * point of type C.
   *
   ...
   *  @param remove_reflected True, if reflecting spikes shall be removed on
   * compression
   */
  template <class D, class T>
  polygon(const db::polygon<D> &p, const T &tr,
          bool compress = default_compression<C>(),
          bool remove_reflected = false) {
    //  create an entry for the hull contour
    m_bbox = box_type(tr(p.box().p1()), tr(p.box().p2()));
    m_ctrs.resize(p.holes() + 1);
    m_ctrs[0].assign(p.begin_hull(), p.end_hull(), tr, false, compress,
                     true /*normalize*/, remove_reflected);
    for (unsigned int i = 0; i < m_ctrs.size() - 1; ++i) {
      m_ctrs[i + 1].assign(p.begin_hole(i), p.end_hole(i), tr, true, compress,
                           true /*normalize*/, remove_reflected);
    }
  }

  /**
   *  @brief The copy constructor from another polygon a different coordinate
   * type
   *
   *  @param p The source polygon
   *  @param compress True, if the contours shall be compressed
   ...
   */
  template <class D>
  explicit polygon(const db::polygon<D> &p,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
   ...
   */
  void translate(const polygon<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const polygon<C> &d, const T &t, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
    transform(t);
  }
   ...
   *  @brief Inequality test
   */
  bool operator!=(const polygon<C> &b) const { return !operator==(b); }
   ...
   *  @brief Fuzzy inequality test
   */
  bool not_equal(const polygon<C> &b) const { return !equal(b); }

  /**
   *  @brief Returns true, if the polygon is a simple box
   */
  bool is_box() const {
    return m_ctrs.size() == 1 && m_ctrs[0].size() == 4 &&
           m_ctrs[0].is_rectilinear();
  }
  ...
  size_t vertices() const {
    size_t n = 0;
    for (typename contour_list_type::const_iterator h = m_ctrs.begin();
         h != m_ctrs.end(); ++h) {
      n += h->size();
    }
    return n;
  ...
  /**
   *  @brief Returns the area ratio between the polygon's bounding box and
   * actual area
   *
   *  This number is a measure how well the polygon is approximated by the
   * bounding box. Values are bigger than 1 for well-formed polygons. Bigger
   * values mean worse approximation.
   */
   ...
   *
   *  @param remove_reflected Removes reflected edges (spiked) if this parameter
   * is true.
   *  @return The compressed polygon.
   */
  polygon<C> &compress(bool remove_reflected = false) {
    //  compress the contours by employing the transform method with a unit
    //  transformation
    for (typename contour_list_type::iterator h = m_ctrs.begin();
         h != m_ctrs.end(); ++h) {
   ...
   */
  template <class Trans>
  polygon<C> &transform(const Trans &t,
                        bool compress = default_compression<C>(),
                        bool remove_reflected = false) {
   ...
   */
  template <class Tr>
  polygon<typename Tr::target_coord_type> transformed(
      const Tr &t,
      bool compress = default_compression<typename Tr::target_coord_type>(),
      bool remove_reflected = false) const {
    ...
    for (unsigned int h = 0; h < holes(); ++h) {
      poly.insert_hole(begin_hole(h), end_hole(h), t, compress,
                       remove_reflected);
    }
   ...
   */
  const db::box<C> &box() const { return m_bbox; }
   ...
   *  @param n The number of holes to reserve memory for
   */
  void reserve_holes(unsigned int n) { m_ctrs.reserve(n + 1); }
   ...
   */
  template <class I>
  void assign_hull(I start, I end, bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_ctrs[0].assign(start, end, false, compress, true /*normalize*/,
                     remove_reflected);
    m_bbox = m_ctrs[0].bbox();
  }
  ...
  void assign_hull(I start, I end, T op,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_ctrs[0].assign(start, end, op, false, compress, true /*normalize*/,
                     remove_reflected);
    m_bbox = m_ctrs[0].bbox();
  }
   ...
   */
  template <class I>
  void assign_hole(unsigned int h, I start, I end,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_ctrs[h + 1].assign(start, end, true, compress, true /*normalize*/,
                         remove_reflected);
  }
   ...
   */
  template <class I, class T>
  void assign_hole(unsigned int h, I start, I end, T op,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_ctrs[h + 1].assign(start, end, op, true, compress, true /*normalize*/,
                         remove_reflected);
  }
   ...
   */
  template <class I>
  void insert_hole(I start, I end, bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
   ...
   */
  template <class I, class T>
  void insert_hole(I start, I end, T op,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    //  add the hole
    contour_type &h = add_hole();
    h.assign(start, end, op, true, compress, true /*normalize*/,
             remove_reflected);
  }
   ...
   *  @brief Outer contour
   */
  const contour_type &hull() const { return m_ctrs[0]; }

  /**
   *  @brief The contour of the nth hole
   */
  const contour_type &hole(unsigned int h) const { return m_ctrs[h + 1]; }

  /**
   *  @brief The number of holes
   */
  unsigned int holes() const { return (unsigned int)m_ctrs.size() - 1; }
   ...
   */
  const contour_type &contour(unsigned int n) const { return m_ctrs[n]; }
   ...
   *  way.
   *
   *  @return The transformation that must be applied to render the original
   * polygon
   */
   ...
   */
  void size(coord_type d, unsigned int mode = 2) { size(d, d, mode); }

  /**
   *  @brief Sizing with anisotropic values
   *
   *  Shifts the polygon contours outwards (dx,dy>0) or inwards (dx,dy<0).
   *  May create invalid (self-overlapping, reverse oriented) polygons. In
   * particular, holes may grow larger than the hull (in which case the bbox is
   * not correct because it is computed from the hull only) and contours do not
   * vanish if shrinked below their own size but are reversed. This method is
   * intended to be used in simple cases or as preparation step for a full-blown
   * sizing algorithm using a merge step. The sign of dx and dy should be
   * identical.
   *
   *  The mode defines at which bending angle cutoff occurs
   *    0: >0
  ...
  void size(coord_type dx, coord_type dy, unsigned int mode = 2) {
    for (typename contour_list_type::iterator c = m_ctrs.begin();
         c != m_ctrs.end(); ++c) {
      c->size(dx, dy, mode);
    }
    m_bbox = m_ctrs[0].bbox();
...
template <class C> class DB_PUBLIC_TEMPLATE simple_polygon {
public:
  typedef C coord_type;
  typedef db::edge<coord_type> edge_type;
  typedef db::simple_trans<coord_type> trans_type;
  ...
  typedef polygon_contour<C> contour_type;
  typedef tl::vector<contour_type> contour_list_type;
  typedef db::polygon_edge_iterator<simple_polygon<C>, db::unit_trans<C>>
      polygon_edge_iterator;
  typedef db::polygon_contour_iterator<contour_type, db::unit_trans<C>>
      polygon_contour_iterator;
   ...
   */
  template <class D, class T>
  simple_polygon(const db::simple_polygon<D> &p, const T &tr,
                 bool compress = default_compression<C>(),
                 bool remove_reflected = false) {
    //  create an entry for the hull contour
    m_bbox = box_type(tr(p.box().p1()), tr(p.box().p2()));
    m_hull.assign(p.begin_hull(), p.end_hull(), tr, false, compress,
                  true /*normalize*/, remove_reflected);
  }

  /**
   *  @brief The copy constructor from another polygon with a different
   * coordinate type
   *
   *  @param p The source polygon
   *  @param compress true, if the sequence shall be compressed (colinear points
   ...
   */
  template <class D>
  explicit simple_polygon(const db::simple_polygon<D> &p,
                          bool compress = default_compression<C>(),
                          bool remove_reflected = false) {
    db::point_coord_converter<C, D> tr;
   ...
   *  @brief The (dummy) translation operator
   */
  void translate(const simple_polygon<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const simple_polygon<C> &d, const T &t,
                 db::generic_repository<C> &, db::ArrayRepository &) {
    *this = d;
    transform(t);
  }
   ...
   *  @brief Inequality test
   */
  bool operator!=(const simple_polygon<C> &b) const { return !operator==(b); }
   ...
   *  @brief Fuzzy inequality test
   */
  bool not_equal(const simple_polygon<C> &b) const { return !equal(b); }
   ...
   */
  template <class Tr>
  simple_polygon<C> &transform(const Tr &t,
                               bool compress = default_compression<C>(),
                               bool remove_reflected = false) {
   ...
   */
  template <class Tr>
  simple_polygon<typename Tr::target_coord_type> transformed(
      const Tr &t,
      bool compress = default_compression<typename Tr::target_coord_type>(),
      bool remove_reflected = false) const {
    typedef typename Tr::target_coord_type target_coord_type;
  ...
  void assign_hull(I start, I end, bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_hull.assign(start, end, false, compress, true /*normalize*/,
                  remove_reflected);
    m_bbox = m_hull.bbox();
  }
  ...
  void assign_hull(I start, I end, T op,
                   bool compress = default_compression<C>(),
                   bool remove_reflected = false) {
    m_hull.assign(start, end, op, false, compress, true /*normalize*/,
                  remove_reflected);
    m_bbox = m_hull.bbox();
  }
   ...
   *  @brief Outer contour
   */
  const contour_type &hull() const { return m_hull; }

  /**
   *  @brief Returns true, if the polygon is a simple box
   */
  bool is_box() const { return m_hull.size() == 4 && m_hull.is_rectilinear(); }

  /**
   *  @brief Returns true, if the polygon is rectilinear
   */
  bool is_rectilinear() const { return m_hull.is_rectilinear(); }

  /**
   *  @brief Returns true, if the polygon is halfmanhattan
   */
  bool is_halfmanhattan() const { return m_hull.is_halfmanhattan(); }

  /**
   *  @brief The number of holes
   *
   *  This method is provided for compatibility with the standard polygon but
   * does always return 0.
   */
  unsigned int holes() const { return 0; }

  /**
   *  @brief Hole iterator: start
   *
   *  This method is provided for compatibility with the standard polygon but
   * does always return the begin iterator of an empty list.
   */
  typename contour_list_type::const_iterator begin_holes() const {
    //  we exploit here the fact that the iterator of an empty vector is always
    //  0
   ...
   *
   *  This method is provided for compatibility with the standard polygon but
   * does always return the end iterator of an empty list.
   */
  typename contour_list_type::const_iterator end_holes() const {
    //  we exploit here the fact that the iterator of an empty vector is always
    //  0
  ...
  /**
   *  @brief A dummy implementation of "insert_hole" provided for template
   * instantiation
   *
   ...
   */
  template <class I, class T>
  void insert_hole(I, I, const T &,
                   bool /*compress*/ = default_compression<C>()) {
    tl_assert(false);
  }

  /**
   *  @brief A dummy implementation of "sort_holes" provided for template
   * instantiation
   *
   *  Asserts, if begin called.
   */
  void sort_holes() { tl_assert(false); }

  /**
   *  @brief A dummy implementation of "hole" provided for template
   * instantiation
   *
   *  Asserts, if begin called.
   */
   ...
   *  @brief The area of the polygon
   */
  area_type area() const { return m_hull.area(); }
   ...
   *  by 2 might round off.
   */
  area_type area2() const { return m_hull.area2(); }

  /**
   *  @brief The perimeter of the polygon
   */
  perimeter_type perimeter() const { return m_hull.perimeter(); }
   ...
   *  @brief Direct access to the contours (used for iterators)
   */
  const contour_type &contour(unsigned int /*n*/) const { return m_hull; }
   ...
   */
  size_t vertices() const { return m_hull.size(); }
...
class polygon_ref : public shape_ref<Poly, Trans> {
public:
  ...
  typedef db::polygon_contour_iterator<typename polygon_type::contour_type,
                                       trans_type>
      polygon_contour_iterator;
   ...
   */
  polygon_ref() : shape_ref<Poly, Trans>() {
    // .. nothing yet ..
  }
  ...
  /**
   *  @brief The constructor creating a reference from an polygon pointer and
   * transformation
   *
   ...
   *
   *  This constructor allows one to copy a polygon reference with a certain
   * transformation to one with another transformation
   */
  template <class TransIn>
  ...
  polygon_contour_iterator begin_hull() const {
    if (this->trans().is_mirror()) {
      return polygon_contour_iterator(--(this->obj().end_hull()), this->trans(),
                                      true);
    } else {
      return polygon_contour_iterator(this->obj().begin_hull(), this->trans(),
                                      false);
    }
  }
  ...
  polygon_contour_iterator end_hull() const {
    if (this->trans().is_mirror()) {
      return polygon_contour_iterator(--(this->obj().begin_hull()),
                                      this->trans(), true);
    } else {
      return polygon_contour_iterator(this->obj().end_hull(), this->trans(),
                                      false);
  ...
  polygon_contour_iterator begin_hole(unsigned int h) const {
    if (this->trans().is_mirror()) {
      return polygon_contour_iterator(--(this->obj().end_hole(h)),
                                      this->trans(), true);
    } else {
      return polygon_contour_iterator(this->obj().begin_hole(h), this->trans(),
                                      false);
    }
  }
  ...
  polygon_contour_iterator end_hole(unsigned int h) const {
    if (this->trans().is_mirror()) {
      return polygon_contour_iterator(--(this->obj().begin_hole(h)),
                                      this->trans(), true);
    } else {
      return polygon_contour_iterator(this->obj().end_hole(h), this->trans(),
                                      false);
    }
  }
   ...
   */
  double area_ratio() const { return this->obj().area_ratio(); }

  /**
   *  @brief The area of the polygon
   */
  area_type area() const { return this->obj().area(); }
   ...
   *  by 2 might round off.
   */
  area_type area2() const { return this->obj().area2(); }

  /**
   *  @brief The perimeter of the polygon
   */
  perimeter_type perimeter() const { return this->obj().perimeter(); }

  /**
   *  @brief Returns a value indicating whether the polygon is a box
   */
  bool is_box() const { return this->obj().is_box(); }

  /**
   *  @brief Returns a value indicating whether the polygon is rectilinear
   */
  bool is_rectilinear() const { return this->obj().is_rectilinear(); }

  /**
   *  @brief Returns the number of vertices
   */
  size_t vertices() const { return this->obj().vertices(); }
...
inline polygon_ref<Poly, TargetTr> operator*(const TargetTr &t,
                                             const polygon_ref<Poly, Tr> &p) {
 ...
 */
template <class C>
inline polygon<double> operator*(const polygon<C> &p, double s) {
 ...
 */
template <class C>
inline simple_polygon<double> operator*(const simple_polygon<C> &p, double s) {
  db::complex_trans<C, double> ct(s);
  return ct * p;
}
...
template <class Iter, class Point> int inside_poly(Iter edge, const Point &pt) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const polygon<C> &p) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const simple_polygon<C> &p) {
  return (os << p.to_string());
}
...
/**
 *  @brief The double coordinate polygon reference (without transformation)
 * typedef
 */
...
/**
 *  @brief The double coordinate simple polygon reference (without
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const polygon_contour<X> &x, bool no_self = false,
                     void *parent = 0) {
...
template <class X>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const polygon<X> &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
template <class X>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const simple_polygon<X> &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
//  injecting a global std::swap for polygons into the
//  std namespace
template <class C> void swap(db::polygon<C> &a, db::polygon<C> &b) {
  a.swap(b);
}
...
template <>
DB_PUBLIC void extractor_impl(tl::Extractor &ex, db::SimplePolygon &p);
template <>
DB_PUBLIC void extractor_impl(tl::Extractor &ex, db::DSimplePolygon &p);

template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::Polygon &p);
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DPolygon &p);
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::SimplePolygon &p);
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DSimplePolygon &p);
