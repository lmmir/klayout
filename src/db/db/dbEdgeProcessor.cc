...
struct NonZeroInsideFunc {
  inline bool operator()(int wc) const { return wc != 0; }
};

struct ProjectionCompare {
  ProjectionCompare(const db::Edge &e) : m_e(e) {}

  bool operator()(const db::Point &a, const db::Point &b) const {
    db::coord_traits<db::Coord>::area_type sp1 =
        db::sprod(m_e, db::Edge(m_e.p1(), a));
    db::coord_traits<db::Coord>::area_type sp2 =
        db::sprod(m_e, db::Edge(m_e.p1(), b));
    if (sp1 != sp2) {
      return sp1 < sp2;
    } else {
      return a < b;
...
struct PolyMapCompare {
  PolyMapCompare(db::Coord y) : m_y(y) {}

  bool operator()(const std::pair<db::Edge, size_t> &a,
                  const std::pair<db::Edge, size_t> &b) const {
    //  simple cases ..
...
inline bool is_point_on_exact(const db::Edge &e, const db::Point &pt) {
...
inline bool is_point_on_fuzzy(const db::Edge &e, const db::Point &pt) {
...
//  In some cases (i.e. (3,-3;-8,-1) and (-4,-2;13,-4)), the intersection test
//  gives different results for the intersection point if the edges are swapped.
//  This test is robust since it operates on ordered edges.
inline std::pair<bool, db::Point> safe_intersect_point(const db::Edge &e1,
                                                       const db::Edge &e2) {
  if (e1 < e2) {
...
/**
 *  @brief A structure for storing data in the first phase of the scanline
 * algorithm
 *
 *  This data are the cut points (intersection points with other edges). The
 * has_cutpoints flag indicates that the edge has cutpoints which the edge must
 * follow. If the edge has strong cutpoints (strong_cutpoints), they will change
 * the edge, hence non-cutpoints (attractors) must be included.
 */
struct CutPoints {
  ...
  CutPoints() : has_cutpoints(false), strong_cutpoints(false) {}
  ...
  void add(const db::Point &p, std::vector<CutPoints> *cpvector,
           bool strong = true) {
    has_cutpoints = true;
    if (strong && !strong_cutpoints) {
      ...
      if (!attractors.empty()) {
        ...
        for (std::vector<std::pair<db::Point, size_t>>::const_iterator a =
                 attr.begin();
             a != attr.end(); ++a) {
 ...
 */
struct WorkEdge : public db::Edge {
  WorkEdge() : db::Edge(), data(0), prop(0) {}

  WorkEdge(const db::Edge &e, EdgeProcessor::property_type p = 0, size_t d = 0)
      : db::Edge(e), data(d), prop(p) {}

  WorkEdge(const WorkEdge &d) : db::Edge(d), data(d.data), prop(d.prop) {}
...
struct EdgeXAtYCompare {
  EdgeXAtYCompare(db::Coord y) : m_y(y) {}
  ...
  bool operator()(const db::Edge &a, const db::Edge &b) const {
    ...
    if (a.dx() == 0 && b.dx() == 0) {
      ...
      //  complex case:
      //  HINT: "volatile" forces xa and xb into memory and disables FPU
...
static inline double edge_xaty2(db::Edge e, db::Coord y) {
  ...
  if (y <= e.p1().y()) {
    ...
    return double(e.p1().x()) +
           double(e.dx()) * double(y - e.p1().y()) / double(e.dy());
  }
}
...
struct EdgeXAtYCompare2 {
  EdgeXAtYCompare2(db::Coord y) : m_y(y) {}
  ...
  bool operator()(const db::Edge &a, const db::Edge &b) const {
    ...
    if (a.dx() == 0 && b.dx() == 0) {
      ...
      if (xa != xb) {
        ...
        //  In that case the edges will not intersect but rather touch in one
        //  point. This defines a sorting which preserves the scanline order of
        //  the edges when y advances.
...
EdgePolygonOp::EdgePolygonOp(EdgePolygonOp::mode_t mode, bool include_touching,
                             int polygon_mode)
    : m_mode(mode), m_include_touching(include_touching),
      m_function(polygon_mode), m_wcp_n(0), m_wcp_s(0) {}

void EdgePolygonOp::reset() { m_wcp_n = m_wcp_s = 0; }
...
bool EdgePolygonOp::is_reset() const { return (m_wcp_n == 0 && m_wcp_s == 0); }

bool EdgePolygonOp::prefer_touch() const { return m_include_touching; }

bool EdgePolygonOp::selects_edges() const { return true; }
...
int InteractionDetector::edge(bool north, bool enter, property_type p) {
  ...
  *wcv += (enter ? 1 : -1);
  ...
  //  In "interacting" and "enclosing" mode we need to handle both north and
  //  south events because we have to catch interactions between objects north
  //  and south to the scanline
  if (north || (m_mode == 0 && m_include_touching) ||
      (m_mode < -1 && m_include_touching)) {
    ...
    if (inside_after < inside_before) {
      ...
      //  (due to prefer_touch == true and the sorting of coincident edges by
      //  property id) hence every remaining parts count as non-interacting
      //  (outside)
      if (p <= m_last_primary_id) {
        for (std::set<property_type>::const_iterator i = inside->begin();
             i != inside->end(); ++i) {
      ...
      if (m_mode != 0) {
        ...
        if (p > m_last_primary_id) {
          ...
          //  note that the primary parts will be delivered first of all
          //  coincident edges hence we can check whether the primary is present
          //  even for coincident edges
          bool any = false;
          for (std::set<property_type>::const_iterator i = inside->begin();
               i != inside->end(); ++i) {
            if (*i <= m_last_primary_id) {
          ...
          for (std::set<property_type>::const_iterator i = inside->begin();
            ...
            if (*i > m_last_primary_id) {
              if (m_mode < -1) {
                //  enclosing mode: an opening primary (= enclosing one) with
                //  open secondaries means the secondary has been opened before
                //  and did not close. Because we sort by property ID this must
                //  have happened before, hence the secondary is overlapping.
                //  Make them non-interactions. We still have to record them as
                //  interactions because this is how we skip the primaries
                //  later.
                m_non_interactions.insert(*i);
              }
        ...
        for (std::set<property_type>::const_iterator i = m_inside_n.begin();
             i != m_inside_n.end(); ++i) {
          if (*i < p) {
            m_interactions.insert(std::make_pair(*i, p));
          } else if (p < *i) {
        ...
        for (std::set<property_type>::const_iterator i = m_inside_s.begin();
             i != m_inside_s.end(); ++i) {
          if (*i < p) {
            m_interactions.insert(std::make_pair(*i, p));
          } else if (p < *i) {
...
void InteractionDetector::finish() {
  if (m_mode < -1) {
    ...
    //  In enclosing mode remove those objects which have an interaction with a
    //  secondary having a non-interaction: these are the ones where secondaries
    //  overlap and stick to the outside.
    std::set<property_type> primaries_to_delete;
    for (std::set<std::pair<property_type, property_type>>::iterator i =
             m_interactions.begin();
    ...
    //  In inside mode remove those objects which have a non-interaction with a
    //  primary
    for (std::set<std::pair<property_type, property_type>>::iterator i =
             m_interactions.begin();
         i != m_interactions.end();) {
    ...
    //  In outside mode leave those objects which don't participate in an
    //  interaction
    ...
    for (std::set<property_type>::const_iterator p = m_non_interactions.begin();
         p != m_non_interactions.end(); ++p) {
      m_interactions.insert(m_interactions.end(),
                            std::make_pair(m_last_primary_id, *p));
    }
  }
...
BooleanOp::BooleanOp(BoolOp mode)
    : m_wc_na(0), m_wc_nb(0), m_wc_sa(0), m_wc_sb(0), m_mode(mode),
      m_zeroes(0) {
  //  .. nothing yet ..
}
...
inline bool BooleanOp::result(int wca, int wcb, const InsideFunc &inside_a,
                              const InsideFunc &inside_b) const {
  switch (m_mode) {
  case BooleanOp::And:
...
template <class InsideFunc>
inline int BooleanOp::edge_impl(bool north, bool enter, property_type p,
                                const InsideFunc &inside_a,
                                const InsideFunc &inside_b) {
  tl_assert(p < m_wcv_n.size() && p < m_wcv_s.size());
...
template <class InsideFunc>
inline int BooleanOp::compare_ns_impl(const InsideFunc &inside_a,
                                      const InsideFunc &inside_b) const {
  return result(m_wc_na, m_wc_nb, inside_a, inside_b) -
         result(m_wc_sa, m_wc_sb, inside_a, inside_b);
}
...
EdgeProcessor::EdgeProcessor(bool report_progress,
...
void EdgeProcessor::disable_progress() { m_report_progress = false; }
...
void EdgeProcessor::set_base_verbosity(int bv) { m_base_verbosity = bv; }

void EdgeProcessor::reserve(size_t n) { mp_work_edges->reserve(n); }

void EdgeProcessor::insert(const db::Edge &e, EdgeProcessor::property_type p) {
  if (e.p1() != e.p2()) {
    mp_work_edges->push_back(WorkEdge(e, p));
  }
}

void EdgeProcessor::insert(const db::SimplePolygon &q,
                           EdgeProcessor::property_type p) {
  for (db::SimplePolygon::polygon_edge_iterator e = q.begin_edge(); !e.at_end();
       ++e) {
    insert(*e, p);
  }
}

void EdgeProcessor::insert(const db::Polygon &q,
                           EdgeProcessor::property_type p) {
  for (db::Polygon::polygon_edge_iterator e = q.begin_edge(); !e.at_end();
       ++e) {
    insert(*e, p);
  }
}

void EdgeProcessor::insert(const db::PolygonRef &q,
                           EdgeProcessor::property_type p) {
  for (db::PolygonRef::polygon_edge_iterator e = q.begin_edge(); !e.at_end();
       ++e) {
    insert(*e, p);
  }
}
...
static void add_hparallel_cutpoints(WorkEdge &e1, WorkEdge &e2,
                                    const db::Box &cell,
                                    std::vector<CutPoints> &cutpoints) {
 ...
 *  it is important that this method delivers exactly (!) the same x for the
 * same edge (after normalization to dy()>0) and same y!
 */
template <class C> inline double edge_xaty_double(db::edge<C> e, double y) {
  if (e.p1().y() > e.p2().y()) {
    e.swap_points();
  }
  ...
  if (y <= e.p1().y()) {
    ...
    return double(e.p1().x()) +
           double(e.dx()) * (y - double(e.p1().y())) / double(e.dy());
  }
}

/**
 *  @brief Computes the left bound of the edge geometry for a given band
 * [y1..y2].
 */
template <class C>
inline C edge_xmin_at_yinterval_double(const db::edge<C> &e, double y1,
                                       double y2) {
  if (e.dx() == 0) {
    return e.p1().x();
  } else if (e.dy() == 0) {
    ...
    return C(floor(
        edge_xaty_double(e, ((e.dy() < 0) ^ (e.dx() < 0)) == 0 ? y1 : y2)));
  }
}

/**
 *  @brief Computes the right bound of the edge geometry for a given band
 * [y1..y2].
 */
template <class C>
inline C edge_xmax_at_yinterval_double(const db::edge<C> &e, double y1,
                                       double y2) {
  if (e.dx() == 0) {
    return e.p1().x();
  } else if (e.dy() == 0) {
...
/**
 *  @brief Functor that compares two edges by their left bound for a given
 * interval [y1..y2].
 *
 *  This function is intended for use in scanline scenarios to determine what
 * edges are interacting in a certain y interval.
 */
template <class C> struct edge_xmin_at_yinterval_double_compare {
...
static void
get_intersections_per_band_any(std::vector<CutPoints> &cutpoints,
                               std::vector<WorkEdge>::iterator current,
                               std::vector<WorkEdge>::iterator future,
                               db::Coord y, db::Coord yy, bool with_h) {
  double dy = y - 0.5;
  double dyy = yy + 0.5;
  std::vector<std::pair<const WorkEdge *, WorkEdge *>>
      p1_weak; // holds weak interactions of edge endpoints with other edges

  std::sort(current, future,
            edge_xmin_at_yinterval_double_compare<db::Coord>(dy, dyy));
  ...
  for (std::vector<WorkEdge>::iterator c = current; c != future;) {
    ...
    if (std::distance(c, f) > 1) {
      ...
      std::set<db::Point>
          weak_points; // holds points that need to go in all other edges
      p1_weak.clear();
          ...
          if (c2->dy() == 0) {
            ...
            if ((with_h || c1->dy() != 0) && c1 < c2) {
              ...
              if (c1->dy() == 0) {
                ...
                if (cp.first && cell.contains(cp.second)) {
                  //  Stash the cutpoint as it must be inserted into other edges
                  //  as well.
            ...
            //  both edges are coincident - produce the ends of the edges
            //  involved as cut points
            if (c1p1_in_cell && c2->contains(c1->p1()) &&
                c2->p1() != c1->p1() && c2->p2() != c1->p1()) {
              c2->make_cutpoints(cutpoints)->add(
                  c1->p1(), &cutpoints, !is_point_on_exact(*c2, c1->p1()));
#ifdef DEBUG_EDGE_PROCESSOR
              if (!is_point_on_exact(*c2, c1->p1())) {
                printf("intersection point %s between %s and %s.\n",
                       c1->p1().to_string().c_str(), c1->to_string().c_str(),
                       c2->to_string().c_str());
              } else {
            ...
            if (c1p2_in_cell && c2->contains(c1->p2()) &&
                c2->p1() != c1->p2() && c2->p2() != c1->p2()) {
              c2->make_cutpoints(cutpoints)->add(
                  c1->p2(), &cutpoints, !is_point_on_exact(*c2, c1->p2()));
#ifdef DEBUG_EDGE_PROCESSOR
              if (!is_point_on_exact(*c2, c1->p2())) {
                printf("intersection point %s between %s and %s.\n",
                       c1->p2().to_string().c_str(), c1->to_string().c_str(),
                       c2->to_string().c_str());
              } else {
                printf("weak intersection point %s between %s and %s.\n",
                       c1->p2().to_string().c_str(), c1->to_string().c_str(),
            ...
            //  We first collect these endpoints because we have to decide
            //  whether that can be a weak attractor or, if it affects two or
            //  more edges in which case it will become a strong attractor. It's
            //  sufficient to do this for p1 only because we made sure we caught
            //  all edges in the +-0.5DBU vicinity by choosing the cell large
            //  enough (.._double operators). For end points exactly on the line
            //  we insert a cutpoint to ensure we use the endpoints as cutpoints
            //  in any case.
            if (c1p1_in_cell && is_point_on_fuzzy(*c2, c1->p1())) {
              if (is_point_on_exact(*c2, c1->p1())) {
#ifdef DEBUG_EDGE_PROCESSOR
                printf("end point %s gives intersection point between %s and "
                       "%s.\n",
                       c1->p1().to_string().c_str(), c1->to_string().c_str(),
                       c2->to_string().c_str());
#endif
                ...
                p1_weak.push_back(
                    std::make_pair(c1.operator->(), c2.operator->()));
              }
            }
          }
        }
      ...
      //  insert weak intersection points into all relevant edges - weak into
      //  edges where the point is on and strong into edges where the point is
      //  on in a fuzzy way.
      ...
      //  go through the list of "p1 to other edges" and insert p1 either as
      //  cutpoint (if there are other strong cutpoints already) or weak
      //  attractor.
      ...
      while (p1w_from != p1_weak.end()) {
        ...
        while (p1w_to != p1_weak.end() && p1w_to->first == p1w_from->first) {
          if (p1w_to->second->data > 0 &&
              cutpoints[p1w_to->second->data - 1].strong_cutpoints) {
            strong = true;
          }
          ++p1w_to;
        }
        ...
        for (auto cp = p1w_from; cp != p1w_to; ++cp) {
          ...
          if (strong) {
            ...
            printf("Insert strong attractor %s in %s.\n",
                   cp->first->p1().to_string().c_str(),
                   cp->second->to_string().c_str());
#endif
          } else {
    ...
    for (std::vector<WorkEdge>::iterator cc = c; cc != f; ++cc) {
      if (edge_xmax(*cc) < x ||
          edge_xmax_at_yinterval_double(*cc, dy, dyy) < x) {
        if (c != cc) {
          std::swap(*cc, *c);
        }
        ++c;
...
public:
  EdgeProcessorState(db::EdgeSink *es, db::EdgeEvaluatorBase *op)
      : mp_es(es), mp_op(op), m_vertex(false), m_x(0), m_y(0), m_hx(0), m_ho(0),
        m_pn(0), m_ps(0) {}

  void start() { mp_es->start(); }

  void flush() { mp_es->flush(); }
  ...
  bool is_reset() { return mp_op->is_reset(); }

  bool can_stop() { return mp_es->can_stop(); }

  void reserve(size_t n) { mp_op->reserve(n); }
  ...
  void end_scanline(db::Coord y) { mp_es->end_scanline(y); }
  ...
  void next_coincident() { m_pn = m_ps = 0; }

  void end_coincident() {
    if (!m_vertex && (m_ps != 0 || m_pn != 0)) {

      if (m_ho != 0) {
        db::Edge he(db::Point(m_hx, m_y),
                    db::Point(db::coord_traits<db::Coord>::rounded(m_x), m_y));
        if (m_ho > 0) {
          he.swap_points();
        }
        mp_es->put(he);
  ...
  void skip_n(size_t n) { mp_es->skip_n(n); }
...
//  NOTE: set this to 0 to force memory-allocation storage for SkipInfo always
//  (testing)
...
public:
  ...
  struct SkipInfo {
    ...
    SkipInfo() : skip(0), m_skip_res_n(0), m_skip_res(0) {}

    SkipInfo(const SkipInfo &si) : skip(0), m_skip_res_n(0), m_skip_res(0) {
      operator=(si);
    }
    ...
    template <class Iter> void set_skip_res(Iter b, Iter e) {
      if (m_skip_res_n > skip_info_storage_threshold) {
  ...
  /**
   *  @brief Creates a generator stage state object from the given sinks and
   * operators
   */
  EdgeProcessorStates(
      const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>
          &procs)
      : m_selects_edges(false), m_prefer_touch(false) {
    m_states.reserve(procs.size());
    for (std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>::
             const_iterator p = procs.begin();
         p != procs.end(); ++p) {
   ...
   */
  bool selects_edges() const { return m_selects_edges; }

  /**
   *  @brief Returns true if the processors prefer touching mode
   */
  bool prefer_touch() const { return m_prefer_touch; }
   ...
   */
  void start() {
    for (std::vector<EdgeProcessorState>::iterator s = m_states.begin();
         s != m_states.end(); ++s) {
      s->start();
    }
  }
  ...
  /**
   *  @brief Announces a batch of edges crossing the same point and begin
   * coincident This event is a sub-event of "next_vertex".
   */
   ...
   *
   *  This method will return true if at least one of the edge sinks received
   * the edge
   */
   ...
   *
   *  This is for optimization of the polygon generation. Stitching of edges
   * does not happen if there are no news.
   */
   ...
   */
  void release_skip_entry(size_t n) { m_skip_queue.push_front(n - 1); }
   ...
   *
   *  Returns the index of a new skip interval entry containing the skip
   * information.
   */
...
void EdgeProcessor::redo(
    const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>
        &gen) {
  redo_or_process(gen, true);
}

void EdgeProcessor::process(
    const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>>
        &gen) {
  redo_or_process(gen, false);
}

void EdgeProcessor::redo_or_process(
    const std::vector<std::pair<db::EdgeSink *, db::EdgeEvaluatorBase *>> &gen,
    bool redo) {
  ...
  for (std::vector<WorkEdge>::iterator e = mp_work_edges->begin();
       e != mp_work_edges->end(); ++e) {
    if (e->prop > n_props) {
      n_props = e->prop;
    }
  }
  ...
  if (m_report_progress) {
    if (m_progress_desc.empty()) {
      progress.reset(
          new tl::AbsoluteProgress(tl::to_string(tr("Processing")), 1000));
    } else {
  ...
  if (redo) {
    ...
    for (std::vector<WorkEdge>::iterator c = mp_work_edges->begin();
         c != mp_work_edges->end(); ++c) {
      c->data = 0;
    }
    ...
    for (std::vector<WorkEdge>::iterator current = mp_work_edges->begin();
      ...
      //  Use as many scanlines as to fetch approx. 50% new edges into the
      //  scanline (this is an empirically determined factor)
      do {
      ...
      } while (future != mp_work_edges->end() &&
               std::distance(current, future) < long(n * fill_factor));

      bool is90 = true;

      if (current != future) {

        for (std::vector<WorkEdge>::iterator c = current; c != future && is90;
             ++c) {
          if (c->dx() != 0 && c->dy() != 0) {
            is90 = false;
          }
        ...
        if (is90) {
          get_intersections_per_band_90(*mp_cpvector, current, future, y, yy,
                                        selects_edges);
        } else {
          get_intersections_per_band_any(*mp_cpvector, current, future, y, yy,
                                         selects_edges);
        }
      }
      ...
      for (std::vector<WorkEdge>::iterator c = current; c != future; ++c) {
        //  Hint: we have to keep the edges ending a y (the new lower band
        //  limit) in the all angle case because these edges may receive
        //  cutpoints because the enter the -0.5DBU region below the band
    ...
    //
    //  Hint: when we create the edges from the cutpoints we use the projection
    //  to sort the cutpoints along the edge. However, we have some freedom to
    //  connect the points which we use to avoid "z" configurations which could
    ...
    for (size_t n = 0; n < n_work; ++n) {
      ...
      if (ew.dy() == 0 && !selects_edges) {
        ...
        if (cut_points->has_cutpoints && !cut_points->cut_points.empty()) {
          ...
          std::sort(cut_points->cut_points.begin(),
                    cut_points->cut_points.end(), ProjectionCompare(e));

          db::Point pll = e.p1();
          db::Point pl = e.p1();

          for (std::vector<db::Point>::iterator cp =
                   cut_points->cut_points.begin();
               cp != cut_points->cut_points.end(); ++cp) {
            if (*cp != pl) {
              WorkEdge ne = WorkEdge(db::Edge(pl, *cp), p);
              if (pl.y() == pll.y() && ne.p2().x() != pl.x() &&
                  ne.p2().x() == pll.x()) {
                ne = db::Edge(pll, ne.p2());
              } else if (pl.x() == pll.x() && ne.p2().y() != pl.y() &&
                         ne.p2().y() == pll.y()) {
    ...
    if (nw != n_work) {
      mp_work_edges->erase(mp_work_edges->begin() + nw,
                           mp_work_edges->begin() + n_work);
    }

#ifdef DEBUG_EDGE_PROCESSOR
    printf("Output edges:\n");
    for (std::vector<WorkEdge>::iterator c1 = mp_work_edges->begin();
         c1 != mp_work_edges->end(); ++c1) {
      printf("%s\n", c1->to_string().c_str());
    }
#endif
  }

  tl::SelfTimer timer2(tl::verbosity() >= m_base_verbosity + 10,
                       "EdgeProcessor: production");
...
void EdgeProcessor::simple_merge(const std::vector<db::Edge> &in,
                                 std::vector<db::Edge> &edges, int mode) {
  clear();
  reserve(in.size());
...
void EdgeProcessor::simple_merge(const std::vector<db::Edge> &in,
                                 ...
                                 bool resolve_holes, bool min_coherence,
                                 int mode) {
  clear();
  reserve(in.size());
  insert_sequence(in.begin(), in.end());
...
void EdgeProcessor::simple_merge(const std::vector<db::Polygon> &in,
                                 std::vector<db::Edge> &edges, int mode) {
  clear();
  reserve(count_edges(in));
  for (std::vector<db::Polygon>::const_iterator q = in.begin(); q != in.end();
       ++q) {
    insert(*q);
  }
...
void EdgeProcessor::merge(const std::vector<db::Polygon> &in,
                          std::vector<db::Edge> &edges, unsigned int min_wc) {
  clear();
  reserve(count_edges(in));
...
void EdgeProcessor::merge(const std::vector<db::Polygon> &in,
                          std::vector<db::Polygon> &out, unsigned int min_wc,
                          bool resolve_holes, bool min_coherence) {
  clear();
  reserve(count_edges(in));
...
void EdgeProcessor::size(const std::vector<db::Polygon> &in, db::Coord dx,
                         db::Coord dy, std::vector<db::Edge> &out,
                         unsigned int mode) {
  clear();
  reserve(count_edges(in));
  ...
  db::PolygonGenerator pg(siz, false /*don't resolve holes*/,
                          false /*min. coherence*/);
...
void EdgeProcessor::size(const std::vector<db::Polygon> &in, db::Coord dx,
                         db::Coord dy, std::vector<db::Polygon> &out,
                         unsigned int mode, bool resolve_holes,
                         bool min_coherence) {
  clear();
  reserve(count_edges(in));
...
void EdgeProcessor::boolean(const std::vector<db::Polygon> &a,
                            const std::vector<db::Polygon> &b,
                            std::vector<db::Edge> &out, int mode) {
  clear();
  ...
  n = 0;
  for (std::vector<db::Polygon>::const_iterator q = a.begin(); q != a.end();
       ++q, n += 2) {
    insert(*q, n);
  }

  n = 1;
  for (std::vector<db::Polygon>::const_iterator q = b.begin(); q != b.end();
       ++q, n += 2) {
    insert(*q, n);
  }
...
void EdgeProcessor::boolean(const std::vector<db::Polygon> &a,
                            const std::vector<db::Polygon> &b,
                            std::vector<db::Polygon> &out, int mode,
                            bool resolve_holes, bool min_coherence) {
  clear();
  reserve(count_edges(a) + count_edges(b));
...
void EdgeProcessor::boolean(const std::vector<db::Edge> &a,
                            const std::vector<db::Edge> &b,
                            std::vector<db::Edge> &out, int mode) {
  clear();
