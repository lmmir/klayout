...
AsIfFlatEdges::AsIfFlatEdges() : EdgesDelegate(), m_bbox_valid(false) {
  //  .. nothing yet ..
}
...
AsIfFlatEdges &AsIfFlatEdges::operator=(const AsIfFlatEdges &other) {
  if (this != &other) {
...
std::string AsIfFlatEdges::to_string(size_t nmax) const {
...
EdgesDelegate *AsIfFlatEdges::selected_interacting_generic(
    const Region &other, EdgeInteractionMode mode, bool inverse) const {
  //  shortcuts
  ...
  AddressablePolygonDelivery p =
      (mode == EdgesInside ? other.addressable_merged_polygons()
                           : other.addressable_polygons());
...
EdgesDelegate *AsIfFlatEdges::selected_interacting_generic(
    const Edges &edges, EdgeInteractionMode mode, bool inverse) const {
  //  shortcuts
  ...
  //  NOTE: "inside" needs merged edges for the other edges as the algorithm
  //  works edge by edge
  AddressableEdgeDelivery ee =
      (mode == EdgesInside ? edges.addressable_merged_edges()
                           : edges.addressable_edges());
...
AsIfFlatEdges::selected_interacting_pair_generic(
    const Region &region, EdgeInteractionMode mode) const {
  //  shortcuts
...
std::pair<EdgesDelegate *, EdgesDelegate *>
AsIfFlatEdges::selected_interacting_pair_generic(
    const Edges &other, EdgeInteractionMode mode) const {
  //  shortcuts
  if (other.empty() || empty()) {
    if (mode != EdgesOutside) {
      return std::make_pair(new EmptyEdges(), clone());
...
EdgesDelegate *AsIfFlatEdges::pull_generic(const Edges &edges) const {
  db::box_scanner<db::Edge, size_t> scanner(report_progress(), progress_desc());
...
RegionDelegate *AsIfFlatEdges::pull_generic(const Region &other) const {
  //  shortcuts
  if (other.empty() || empty()) {
  ...
  edge_to_region_interaction_filter<FlatRegion> filter(output.get(),
                                                       EdgesInteract);
  scanner.process(filter, 1, db::box_convert<db::Edge>(),
                  db::box_convert<db::Polygon>());
...
EdgesDelegate *AsIfFlatEdges::pull_interacting(const Edges &other) const {
  return pull_generic(other);
}

RegionDelegate *AsIfFlatEdges::pull_interacting(const Region &other) const {
  return pull_generic(other);
}

EdgesDelegate *AsIfFlatEdges::selected_interacting(const Edges &other) const {
...
EdgesDelegate *AsIfFlatEdges::selected_interacting(const Region &other) const {
  return selected_interacting_generic(other, EdgesInteract, false);
}
...
EdgesDelegate *AsIfFlatEdges::selected_outside(const Region &other) const {
  return selected_interacting_generic(other, EdgesOutside, false);
}

EdgesDelegate *AsIfFlatEdges::selected_not_outside(const Region &other) const {
...
EdgesDelegate *AsIfFlatEdges::selected_inside(const Region &other) const {
  return selected_interacting_generic(other, EdgesInside, false);
}

EdgesDelegate *AsIfFlatEdges::selected_not_inside(const Region &other) const {
...
EdgesDelegate *AsIfFlatEdges::selected_outside(const Edges &other) const {
  return selected_interacting_generic(other, EdgesOutside, false);
}

EdgesDelegate *AsIfFlatEdges::selected_not_outside(const Edges &other) const {
  return selected_interacting_generic(other, EdgesOutside, true);
}
...
EdgesDelegate *AsIfFlatEdges::selected_inside(const Edges &other) const {
  return selected_interacting_generic(other, EdgesInside, false);
}

EdgesDelegate *AsIfFlatEdges::selected_not_inside(const Edges &other) const {
  return selected_interacting_generic(other, EdgesInside, true);
}
...
struct JoinEdgesClusterCollector
  ...
  JoinEdgesClusterCollector(db::PolygonSink *output, coord_type ext_b,
                            coord_type ext_e, coord_type ext_o,
                            coord_type ext_i)
      : db::cluster_collector<db::Edge, size_t, JoinEdgesCluster>(
            JoinEdgesCluster(output, ext_b, ext_e, ext_o, ext_i), true) {
    //  .. nothing yet ..
  }

  void add(const db::Edge *o1, size_t p1, const db::Edge *o2, size_t p2) {
    if (o1->p2() == o2->p1() || o1->p1() == o2->p2()) {
      db::cluster_collector<db::Edge, size_t, JoinEdgesCluster>::add(o1, p1, o2,
                                                                     p2);
    }
  }
};
...
RegionDelegate *AsIfFlatEdges::extended(coord_type ext_b, coord_type ext_e,
  ...
  if (join) {
    ...
    JoinEdgesClusterCollector cluster_collector(&sg, ext_b, ext_e, ext_o,
                                                ext_i);

    db::box_scanner<db::Edge, size_t> scanner(report_progress(),
                                              progress_desc());
    ...
    std::unique_ptr<FlatRegion> output(new FlatRegion());
    db::PropertyMapper pm(output->properties_repository(),
                          properties_repository());

    for (EdgesIterator e(begin_merged()); !e.at_end(); ++e) {
      db::properties_id_type prop_id = pm(e.prop_id());
      if (prop_id != 0) {
        output->insert(db::PolygonWithProperties(
            extended_edge(*e, ext_b, ext_e, ext_o, ext_i), prop_id));
      } else {
...
EdgesDelegate *AsIfFlatEdges::in(const Edges &other, bool invert) const {
  if (other.empty()) {
...
size_t AsIfFlatEdges::count() const {
  size_t n = 0;
...
size_t AsIfFlatEdges::hier_count() const { return count(); }

AsIfFlatEdges::length_type AsIfFlatEdges::length(const db::Box &box) const {
  distance_type l = 0;
...
void AsIfFlatEdges::invalidate_bbox() { m_bbox_valid = false; }

EdgesDelegate *AsIfFlatEdges::processed(const EdgeProcessorBase &filter) const {
  ...
  for (EdgesIterator e(filter.requires_raw_input() ? begin() : begin_merged());
       !e.at_end(); ++e) {
    res_edges.clear();
    filter.process(*e, res_edges);
    for (std::vector<db::Edge>::const_iterator er = res_edges.begin();
         er != res_edges.end(); ++er) {
      edges->insert(*er);
    }
  }
...
EdgePairsDelegate *AsIfFlatEdges::processed_to_edge_pairs(
  ...
  for (EdgesIterator e(filter.requires_raw_input() ? begin() : begin_merged());
    ...
    for (std::vector<db::EdgePair>::const_iterator epr = res_edge_pairs.begin();
         epr != res_edge_pairs.end(); ++epr) {
...
RegionDelegate *AsIfFlatEdges::processed_to_polygons(
  ...
  for (EdgesIterator e(filter.requires_raw_input() ? begin() : begin_merged());
    ...
    for (std::vector<db::Polygon>::const_iterator pr = res_polygons.begin();
         pr != res_polygons.end(); ++pr) {
      region->insert(*pr);
    }
  }
...
EdgesDelegate *AsIfFlatEdges::filtered(const EdgeFilterBase &filter) const {
...
EdgePairsDelegate *
AsIfFlatEdges::run_check(db::edge_relation_type rel, const Edges *other,
                         db::Coord d,
                         const db::EdgesCheckOptions &options) const {
  ...
  edge2edge_check_for_edges<db::FlatEdgePairs> edge_check(check, *result,
                                                          other != 0);
...
EdgesDelegate *AsIfFlatEdges::boolean(const Edges *other, EdgeBoolOp op) const {
  std::unique_ptr<FlatEdges> output(new FlatEdges(true));
  EdgeBooleanClusterCollectorToShapes cluster_collector(&output->raw_edges(),
                                                        op);
...
AsIfFlatEdges::boolean_andnot(const Edges *other) const {
  std::unique_ptr<FlatEdges> output(new FlatEdges(true));
  std::unique_ptr<FlatEdges> output2(new FlatEdges(true));
  EdgeBooleanClusterCollectorToShapes cluster_collector(
      &output->raw_edges(), EdgeAndNot, &output2->raw_edges());
...
std::pair<EdgesDelegate *, EdgesDelegate *>
AsIfFlatEdges::edge_region_op(const Region &other,
                              db::EdgePolygonOp::mode_t mode,
  ...
  if (mode == db::EdgePolygonOp::Both) {
    ...
    cc_second.reset(new db::EdgeShapeGenerator(
        output_second->raw_edges(), true /*clear*/, 2 /*second tag*/));
  }

  std::unique_ptr<FlatEdges> output(new FlatEdges(false));
  db::EdgeShapeGenerator cc(output->raw_edges(), true /*clear*/, 1 /*tag*/,
                            cc_second.get());
...
EdgesDelegate *AsIfFlatEdges::add(const Edges &other) const {
  const FlatEdges *other_flat =
      dynamic_cast<const FlatEdges *>(other.delegate());
  if (other_flat) {
...
bool AsIfFlatEdges::equals(const Edges &other) const {
...
bool AsIfFlatEdges::less(const Edges &other) const {
  if (empty() != other.empty()) {
...
void AsIfFlatEdges::insert_into(Layout *layout, db::cell_index_type into_cell,
