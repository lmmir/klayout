...
Technologies::Technologies(const Technologies &other) : tl::Object() {
  m_changed = false;
  m_in_update = false;
  operator=(other);
}
...
Technologies &Technologies::operator=(const Technologies &other) {
  if (&other != this) {
    m_technologies = other.m_technologies;
    for (iterator i = begin(); i != end(); ++i) {
      i->technology_changed_with_sender_event.add(
          this, &Technologies::technology_changed);
    }
...
db::Technologies *Technologies::instance() {
...
static tl::XMLElementList xml_elements() {
  return make_element((Technologies::const_iterator(Technologies::*)() const) &
                          Technologies::begin,
                      (Technologies::const_iterator(Technologies::*)() const) &
                          Technologies::end,
                      &Technologies::add, "technology",
                      Technology::xml_elements());
}

std::string Technologies::to_xml() const {
...
void Technologies::add_tech(Technology *tech, bool replace_same) {
  if (!tech) {
    return;
  }
  ...
  Technology *t = 0;
  for (tl::stable_vector<Technology>::iterator i = m_technologies.begin();
       !t && i != m_technologies.end(); ++i) {
    if (i->name() == tech->name()) {
      t = i.operator->();
    }
  }
  ...
  if (t) {
    if (replace_same) {
      *t = *tech;
      ...
      throw tl::Exception(
          tl::to_string(tr("A technology with this name already exists: ")) +
          tech->name());
    }
  } else {
    m_technologies.push_back(tech_ptr.release());
    tech->technology_changed_with_sender_event.add(
        this, &Technologies::technology_changed);
  }
...
void Technologies::remove(const std::string &name) {
  for (tl::stable_vector<Technology>::iterator t = m_technologies.begin();
...
void Technologies::notify_technologies_changed() { technologies_changed(); }
...
bool Technologies::has_technology(const std::string &name) const {
  for (tl::stable_vector<Technology>::const_iterator t = m_technologies.begin();
       t != m_technologies.end(); ++t) {
    if (t->name() == name) {
      return true;
    }
...
Technology *Technologies::technology_by_name(const std::string &name) {
  for (tl::stable_vector<Technology>::iterator t = m_technologies.begin();
       t != m_technologies.end(); ++t) {
    if (t->name() == name) {
      return &*t;
...
Technology::Technology()
    : m_name(), m_description(), m_group(), m_dbu(0.001), m_persisted(true),
      m_readonly(false) {
  init();
}
...
Technology::Technology(const std::string &name, const std::string &description,
    ...
    : m_name(name), m_description(description), m_group(group), m_dbu(0.001),
      m_persisted(true), m_readonly(false) {
  init();
}
...
void Technology::init() {
  ...
  for (tl::Registrar<db::TechnologyComponentProvider>::iterator cls =
           tl::Registrar<db::TechnologyComponentProvider>::begin();
       cls != tl::Registrar<db::TechnologyComponentProvider>::end(); ++cls) {
    m_components.push_back(cls->create_component());
  }
}

Technology::~Technology() {
  for (std::vector<TechnologyComponent *>::const_iterator c =
           m_components.begin();
       c != m_components.end(); ++c) {
    delete *c;
  }
  m_components.clear();
}
...
Technology::Technology(const Technology &d)
    : tl::Object(), m_name(d.m_name), m_description(d.m_description),
      m_group(d.m_group), m_grain_name(d.m_grain_name), m_dbu(d.m_dbu),
      ...
      m_save_layout_options(d.m_save_layout_options), m_lyp_path(d.m_lyp_path),
      m_add_other_layers(d.m_add_other_layers), m_persisted(d.m_persisted),
      m_readonly(d.m_readonly), m_lyt_file(d.m_lyt_file) {
  for (std::vector<TechnologyComponent *>::const_iterator c =
           d.m_components.begin();
       c != d.m_components.end(); ++c) {
...
std::string Technology::get_display_string() const {
  std::string d = name();
...
tl::XMLElementList Technology::xml_elements() {
  tl::XMLElementList elements =
      tl::make_member(&Technology::name, &Technology::set_name, "name") +
      tl::make_member(&Technology::description, &Technology::set_description,
                      "description") +
      tl::make_member(&Technology::group, &Technology::set_group, "group") +
      tl::make_member(&Technology::dbu, &Technology::set_dbu, "dbu") +
      tl::make_member(&Technology::explicit_base_path,
                      &Technology::set_explicit_base_path, "base-path") +
      tl::make_member(&Technology::default_base_path,
                      &Technology::set_default_base_path,
                      "original-base-path") +
      tl::make_member(&Technology::layer_properties_file,
                      &Technology::set_layer_properties_file,
                      "layer-properties_file") +
      tl::make_member(&Technology::add_other_layers,
                      &Technology::set_add_other_layers, "add-other-layers") +
      tl::make_element(&Technology::load_layout_options,
                       &Technology::set_load_layout_options, "reader-options",
                       db::load_options_xml_element_list()) +
      tl::make_element(&Technology::save_layout_options,
                       &Technology::set_save_layout_options, "writer-options",
...
std::vector<std::string> Technology::component_names() const {
...
void Technology::set_component(TechnologyComponent *component) {
  for (std::vector<TechnologyComponent *>::iterator c = m_components.begin();
       c != m_components.end(); ++c) {
...
std::string Technology::base_path() const {
  tl::Eval expr;
  expr.set_var("tech_dir", m_default_base_path);
  expr.set_var("tech_file", m_lyt_file);
  expr.set_var("tech_name", name());
  return expr.interpolate(m_explicit_base_path.empty() ? m_default_base_path
                                                       : m_explicit_base_path);
}

std::string Technology::correct_path(const std::string &fp) const {
...
std::string Technology::build_effective_path(const std::string &p) const {
  std::string bp = base_path();
  if (p.empty() || bp.empty()) {
    return p;
  }
