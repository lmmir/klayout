...
struct DB_PUBLIC LayerProperties {
  ...
  /**
   *  @brief Returns true, if the layer specification is not a null
   * specification
   *
   *  A null specification is one created by the default constructor. It does
   * not have a layer, datatype or name assigned.
   */
  bool is_null() const;
   ...
   *
   *  This currently reflects only equality of layers and datatypes, name is of
   * second order and used only if no layer or datatype is given
   */
   ...
   *
   *  This currently reflects only order of layer and datatype, name is of
   * second order
   */
 ...
 *
 *  This struct defines a layer offset which can be "added" to a LayerProperties
 * object If the layer offset is defined with a name, any occurrence of '*' in
 * the string is replaced with the original name. This way, applying "*_A" with
 * "+" yields a postfix "_A" to the original layer name (if it is named).
 */
 ...
 */
inline LayerProperties operator+(const LayerProperties &props,
 ...
 */
inline LayerProperties &operator+=(LayerProperties &props,
                                   const LayerOffset &offset) {
...
namespace tl {
template <>
DB_PUBLIC void extractor_impl<db::LayerProperties>(tl::Extractor &ex,
                                                   db::LayerProperties &p);
template <>
DB_PUBLIC void extractor_impl<db::LayerOffset>(tl::Extractor &ex,
                                               db::LayerOffset &p);

template <>
DB_PUBLIC bool test_extractor_impl<db::LayerProperties>(tl::Extractor &ex,
                                                        db::LayerProperties &p);
template <>
DB_PUBLIC bool test_extractor_impl<db::LayerOffset>(tl::Extractor &ex,
                                                    db::LayerOffset &p);
} // namespace tl
