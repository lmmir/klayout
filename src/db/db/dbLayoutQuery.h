 ...
 *
 *  A filter component is one stage in the query path. It defines what items to
 * look for below a given item. This is the base class for all filters.
 */
...
public:
   ...
   *  The state object is basically the selector and acts as a iterator for the
   * property that this filter represents. This method is provided for
   * implementation by FilterBracket mainly. Custom classes should implement
   * do_create_state.
   */
  virtual FilterStateBase *
  create_state(const std::vector<FilterStateBase *> &followers,
               db::Layout *layout, tl::Eval &eval, bool single) const;

  /**
   *  @brief Clones this instance
   *
   *  This method must be implemented by derived classes to create a copy of
   * themselves.
   */
   ...
   */
  const std::vector<FilterBase *> &followers() const { return m_followers; }

  /**
   *  @brief Gets the follower filters (non-const version)
   */
  std::vector<FilterBase *> &followers() { return m_followers; }

  /**
   *  @brief Connect this filter to the given follower filter
   *
   *  Connections form a graph of filters that are evaluated to render the
   * sequence of results.
   */
...
protected:
   ...
   *  This method must be implemented by a filter to provide the state object.
   *  The state object is basically the selector and acts as a iterator for the
   * property that this filter represents.
   *
   *  @param layout The layout that this query refers to.
   *  @param eval The expression context to use for late evaluation of the
   * filter's arguments
   */
  virtual FilterStateBase *do_create_state(db::Layout *layout,
                                           tl::Eval &eval) const;
   ...
   *  Filter specific state objects must report a value if they are asked for
   * the property with the returned property ID.
   *
 ...
 *
 *  A bracket is a bracket around a filter graph. In addition, brackets can
 * specify a multiplicity (loopmin to loopmax). A bracket defines two virtual
 * nodes: the entry and the exit node. The entry node is the input of the filter
 * and internally connected to the inputs of the children. The exit node is the
 * output of the filter and the node the children connect to.
 */
...
public:
   ...
   */
  void set_loopmin(unsigned int v) { m_loopmin = v; }

  /**
   *  @brief Set the max multiplicity explicitly
   */
  void set_loopmax(unsigned int v) { m_loopmax = v; }
...
private:
  ...
  FilterStateBase *
  create_state_helper(std::map<const FilterBase *, FilterStateBase *> &fmap,
                      const FilterBase *child, FilterStateBase *closure_state,
                      db::Layout *layout, tl::Eval &eval) const;
};
...
public:
  ...
  bool wants_all_cells() const { return m_wants_all_cells; }

  void request_cell(db::cell_index_type ci);
  bool wants_cell(db::cell_index_type ci) const;

  cell_iterator begin_cells() const { return m_wants_cells.begin(); }

  cell_iterator end_cells() const { return m_wants_cells.end(); }
...
public:
   ...
   *
   *  This method is called after the state graph has been created. It will
   * initialize all followers and the call the virtual "do_init" method.
   */
   ...
   */
  virtual void reset(FilterStateBase *previous) { mp_previous = previous; }

  /**
   *  @brief Increment the iterator
   *
   *  Implement this method to increment the iterator to the next state.
   *  Skip is a flag passed from the query iterator's next function. It
   * indicates whether an associated operation (i.e. delete or expression
   * evaluation) shall be skipped.
   */
   ...
   *
   *  Implement this method to return true, if the iterator is at the end of the
   * sequence.
   */
  virtual bool at_end() = 0;

  /**
   *  @brief Implementation of the get method (override to implement the actual
   * getter)
   */
   ...
   */
  const FilterBase *filter() const { return mp_filter; }

  /**
   *  @brief Returns the expression evaluation object
   */
  tl::Eval &eval() const { return *mp_eval; }
   ...
   */
  FilterStateBase *previous() const { return mp_previous; }
...
protected:
  ...
  FilterStateObjectives &objectives() { return m_objectives; }

private:
  friend class LayoutQueryIterator;

  FilterStateBase *mp_previous;
  //  Hint: this member is mutable because the child method may dynamically add
  //  items to create a recursion
 ...
 */
class DB_PUBLIC LayoutQuery : public tl::Object {
public:
  /**
   *  @brief Constructor
   *
   ...
   */
  FilterBracket &root() { return *mp_root; }

  /**
   *  @brief Gets the root bracket of the query (const)
   */
  const FilterBracket &root() const { return *mp_root; }
   ...
   */
  unsigned int properties() const { return (unsigned int)m_properties.size(); }
  ...
  /**
   *  @brief Gets a value that indicates whether the property with the given
   * name exists.
   */
...
private:
  struct PropertyDescriptor {
    PropertyDescriptor(LayoutQueryPropertyType t, unsigned int i,
                       const std::string &n)
        : type(t), id(i), name(n) {}
 ...
 */
class DB_PUBLIC LayoutQueryIterator : public tl::Object {
public:
  /**
   *  @brief Constructor
   *
   ...
   */
  LayoutQueryIterator(const LayoutQuery &q, db::Layout *layout,
                      tl::Eval *parent_eval = 0,
                      tl::AbsoluteProgress *progress = 0);
   ...
   *  @param layout The layout to which the query is applied
   */
  LayoutQueryIterator(const LayoutQuery &q, const db::Layout *layout,
                      tl::Eval *parent_eval = 0,
                      tl::AbsoluteProgress *progress = 0);
   ...
   *  skipped. This only applies to queries with an action like "delete" or
   * "with".
   */
  void next(bool skip);
   ...
   */
  const LayoutQuery *query() const { return mp_q.get(); }

  /**
   *  @brief Gets the layout object that this iterator runs for
   */
  const Layout *layout() const { return mp_layout; }
  ...
  /**
   *  @brief Get the eval object which provides access to the properties through
   * expressions
   */
  tl::Eval &eval() { return m_eval; }
