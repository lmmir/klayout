 ...
 */
class DB_PUBLIC FlatTexts : public MutableTexts {
public:
  ...
  TextsDelegate *clone() const { return new FlatTexts(*this); }
...
private:
  ...
  template <class Trans> void transform_generic(const Trans &trans) {
    if (!trans.is_unity()) {
      ...
      for (text_iterator_type p =
               texts.template get_layer<db::Text, db::unstable_layer_tag>()
                   .begin();
           p !=
           texts.template get_layer<db::Text, db::unstable_layer_tag>().end();
           ++p) {
        texts.get_layer<db::Text, db::unstable_layer_tag>().replace(
