 ...
 *  One of the devices lives in a primary netlist. This one is taken for the
 * device class.
 */
static const db::DeviceClass *primary_device_class(const db::Device &a,
                                                   const db::Device &b) {
  tl_assert(a.device_class() != 0);
  tl_assert(b.device_class() != 0);

  const db::DeviceClass *dca = a.device_class()->primary_class()
                                   ? a.device_class()->primary_class()
                                   : a.device_class();
  const db::DeviceClass *dcb = b.device_class()->primary_class()
                                   ? b.device_class()->primary_class()
                                   : b.device_class();

  if (dca != dcb) {
    //  different devices, same category while sorting devices - take the one
    //  with the "lower" name
...
static int compare_parameters(double pa, double pb,
                              double absolute = default_absolute_tolerance,
                              double relative = default_relative_tolerance) {
...
EqualDeviceParameters::EqualDeviceParameters(size_t parameter_id, bool ignore) {
  m_compare_set.push_back(
      std::make_pair(parameter_id, std::make_pair(ignore ? -1.0 : 0.0, 0.0)));
}

EqualDeviceParameters::EqualDeviceParameters(size_t parameter_id,
                                             double absolute, double relative) {
  m_compare_set.push_back(
      std::make_pair(parameter_id, std::make_pair(std::max(0.0, absolute),
                                                  std::max(0.0, relative))));
}

std::string EqualDeviceParameters::to_string() const {
  std::string res;
  for (std::vector<std::pair<size_t, std::pair<double, double>>>::const_iterator
           c = m_compare_set.begin();
       c != m_compare_set.end(); ++c) {
    if (!res.empty()) {
      res += ";";
    }
    ...
    if (c->second.first < 0.0) {
      ...
      res += "A" + tl::to_string(c->second.first) + "/R" +
             tl::to_string(c->second.second);
    }
  }
  return res;
}

bool EqualDeviceParameters::less(const db::Device &a,
                                 const db::Device &b) const {
  for (std::vector<std::pair<size_t, std::pair<double, double>>>::const_iterator
           c = m_compare_set.begin();
       c != m_compare_set.end(); ++c) {
    int cmp = compare_parameters(a.parameter_value(c->first),
                                 b.parameter_value(c->first), c->second.first,
                                 c->second.second);
    if (cmp != 0) {
      return cmp < 0;
    }
  ...
  const std::vector<db::DeviceParameterDefinition> &pd =
      primary_device_class(a, b)->parameter_definitions();
  for (std::vector<db::DeviceParameterDefinition>::const_iterator p =
           pd.begin();
       p != pd.end(); ++p) {
    if (p->is_primary() && seen.find(p->id()) == seen.end()) {
      int cmp = compare_parameters(a.parameter_value(p->id()),
                                   b.parameter_value(p->id()));
      if (cmp != 0) {
        return cmp < 0;
      }
    }
...
EqualDeviceParameters::operator+=(const EqualDeviceParameters &other) {
  for (std::vector<std::pair<size_t, std::pair<double, double>>>::const_iterator
           c = other.m_compare_set.begin();
       c != other.m_compare_set.end(); ++c) {
...
bool AllDeviceParametersAreEqual::less(const db::Device &a,
                                       const db::Device &b) const {
  const std::vector<db::DeviceParameterDefinition> &parameters =
      a.device_class()->parameter_definitions();
  for (std::vector<db::DeviceParameterDefinition>::const_iterator c =
           parameters.begin();
       c != parameters.end(); ++c) {
    int cmp = compare_parameters(a.parameter_value(c->id()),
                                 b.parameter_value(c->id()), 0.0, m_relative);
    if (cmp != 0) {
      return cmp < 0;
    }
  }
...
DeviceClass::DeviceClass()
    : m_strict(false), mp_netlist(0), m_supports_parallel_combination(false),
      m_supports_serial_combination(false), mp_primary_class(0) {
  // .. nothing yet ..
}

DeviceClass::DeviceClass(const DeviceClass &other)
    : gsi::ObjectBase(other), tl::Object(other), tl::UniqueId(other),
      m_strict(false), mp_netlist(0), m_supports_parallel_combination(false),
      m_supports_serial_combination(false), mp_primary_class(0) {
  operator=(other);
}
...
DeviceClass &DeviceClass::operator=(const DeviceClass &other) {
  if (this != &other) {
    ...
    mp_pc_delegate.reset(const_cast<DeviceParameterCompareDelegate *>(
        other.mp_pc_delegate.get()));
    mp_device_combiner.reset(
        const_cast<DeviceCombiner *>(other.mp_device_combiner.get()));
...
DeviceClass::add_terminal_definition(const DeviceTerminalDefinition &pd) {
...
const DeviceTerminalDefinition *
DeviceClass::terminal_definition(size_t id) const {
...
DeviceClass::add_parameter_definition(const DeviceParameterDefinition &pd) {
...
const DeviceParameterDefinition *
DeviceClass::parameter_definition(size_t id) const {
...
DeviceClass::parameter_definition_non_const(size_t id) {
  if (id < m_parameter_definitions.size()) {
    return &m_parameter_definitions[id];
  } else {
    return 0;
...
size_t DeviceClass::parameter_id_for_name(const std::string &name) const {
  ...
  throw tl::Exception(tl::to_string(tr("Invalid parameter name")) + ": '" +
                      name + "'");
}
...
size_t DeviceClass::terminal_id_for_name(const std::string &name) const {
  ...
  throw tl::Exception(tl::to_string(tr("Invalid terminal name")) + ": '" +
                      name + "'");
}
...
bool DeviceClass::less(const db::Device &a, const db::Device &b) {
  ...
  const db::DeviceParameterCompareDelegate *pcd =
      primary_device_class(a, b)->parameter_compare_delegate();
  if (!pcd) {
    pcd = &default_compare;
  }
...
DeviceClassTemplateBase::template_by_name(const std::string &name) {
  for (tl::Registrar<db::DeviceClassTemplateBase>::iterator i =
           tl::Registrar<db::DeviceClassTemplateBase>::begin();
       i != tl::Registrar<db::DeviceClassTemplateBase>::end(); ++i) {
    if (i->name() == name) {
      return i.operator->();
    }
  }
