 ...
 *
 *  A parameter is described by a name (potentially a variable name), a
 * description text (for a UI label), a default value (a variant), a type
 * (requested type for the variant), a choice list (if the parameter can accept
 * discrete values only) and a hidden flag (if this is set to true, the
 * parameter cannot be changed by the UI, but is accessible and changeable by
 * the production code).
 *
 *  There is also a readonly flag (in this case, the user interface displays the
 * property but does not allow to edit it) and a unit string (displayed left to
 * the edit field for double and integer values).
 */
...
public:
   ...
   */
  PCellParameterDeclaration(const std::string &name, type t,
                            const std::string &description)
      : m_hidden(false), m_readonly(false), m_type(t), m_name(name),
  ...
  PCellParameterDeclaration(const std::string &name, type t,
                            const std::string &description,
                            const tl::Variant &def)
      : m_default(def), m_hidden(false), m_readonly(false), m_type(t),
  ...
  PCellParameterDeclaration(const std::string &name, type t,
                            const std::string &description,
                            const tl::Variant &def, const std::string &unit)
      : m_default(def), m_hidden(false), m_readonly(false), m_type(t),
        m_name(name), m_description(description), m_unit(unit) {
    // .. nothing yet ..
  }
  ...
  PCellParameterDeclaration(const std::string &name, type t,
                            const std::string &description,
                            const std::vector<tl::Variant> &choices,
                            const std::vector<std::string> &choice_descriptions)
      : m_choices(choices), m_choice_descriptions(choice_descriptions),
        m_hidden(false), m_readonly(false), m_type(t), m_name(name),
        m_description(description) {
    // .. nothing yet ..
  }
   ...
   */
  const tl::Variant &get_default() const { return m_default; }

  /**
   *  @brief Setter for the default value
   */
  void set_default(const tl::Variant &def) { m_default = def; }

  /**
   *  @brief Getter for the choices
   */
  const std::vector<tl::Variant> &get_choices() const { return m_choices; }
   ...
   */
  void
  set_choice_descriptions(const std::vector<std::string> &choice_descriptions) {
  ...
  bool operator==(const db::PCellParameterDeclaration &d) const {
    return m_choices == d.m_choices &&
           ...
           m_default == d.m_default && m_hidden == d.m_hidden &&
           m_readonly == d.m_readonly && m_type == d.m_type &&
 ...
 *  Such a layer declaration consists of a db::LayerProperties description
 * (layer, datatype and/or name) and a symbolic name which can potentially be
 * used as a variable name.
 */
class DB_PUBLIC PCellLayerDeclaration : public db::LayerProperties {
public:
  /**
   *  @brief The default constructor
   */
...
public:
   ...
   */
  enum Icon { NoIcon = 0, InfoIcon = 1, ErrorIcon = 2, WarningIcon = 3 };

  /**
   *  @brief Parameterized constructor
   */
  ParameterState()
      : m_value(), m_visible(true), m_enabled(true), m_readonly(false),
        m_icon(NoIcon) {
    //  .. nothing yet ..
  }
   ...
   */
  const tl::Variant &value() const { return m_value; }

  /**
   *  @brief Sets the value
   */
  void set_value(const tl::Variant &v) { m_value = v; }
   ...
   */
  void set_readonly(bool f) { m_readonly = f; }
   ...
   */
  void set_tooltip(const std::string &s) { m_tooltip = s; }
...
/**
 *  @brief Represents the state of call parameters for the callback
 * implementation
 */
...
public:
  ...
  /**
   *  @brief Gets a value indicating whether a parameter with that name is
   * present
   */
 ...
 */
class DB_PUBLIC PCellDeclaration : public gsi::ObjectBase, public tl::Object {
public:
  /**
   *  @brief The default constructor
   */
   ...
   *  layout is produced, the production method will receive a vector of layer
   * id's corresponding to the layer declarations delivered by this method. This
   * method receives the current PCell parameters which allows it to deduce
   * layers from the parameters.
   */
  virtual std::vector<PCellLayerDeclaration>
  get_layer_declarations(const pcell_parameters_type & /*parameters*/) const {
   ...
   *
   *  This method can be reimplemented to change the parameter set according to
   * some constraints for example. The reimplementation may modify the
   * parameters in a way that they are usable for the "produce" method The
   * method receives a reference to the layout so it is able to verify the
   * parameters against layout properties. It can throw an exception to indicate
   * that something is not correct.
   */
  virtual void
  coerce_parameters(const db::Layout & /*layout*/,
   ...
   *
   *  This method allows implementing dynamic behavior on the change of a
   * parameter value. A ParameterStatus object is supplied that allows changing
   * parameter enabled status, visibility and value. The callback also acts as
   * receiver for t_callback type parameters which only present a button.
   *
   *  The callback function receives the name of the parameter that was changed.
   *  On some occasions, the callback is called unspecifically, for example for
   * the initialization. In that case, the parameter name is empty.
   *
   *  Exceptions from this implementation are ignored.
   */
  virtual void callback(const db::Layout & /*layout*/,
                        const std::string & /*name*/,
                        ParameterStates & /*states*/) const {
    //  the default implementation does nothing
  }

  /**
   *  @brief Produces a layout for the given parameter set and using the given
   * layers.
   *
   *  A reimplementation of that method should produce the desired layout for
   * the given parameter set. The layout shall be put into the given cell. This
   * code may create cell instances to other cells inside the given layout.
   */
  ...
  /**
   *  @brief Returns true, if the PCell can be created from the given shape on
   * the given layer
   *
   *  If this method returns true, KLayout allows this PCell to be created from
   * the given shape. It will use "parameters_from_shape" and
   * "transformation_from_shape" to derive the parameters for the new instance.
   */
  ...
  /**
   *  @brief Returns the parameter set for a PCell of that kind of shape on the
   * given layer
   *
   *  This method is used to derive the parameters for a new instance from a
   * shape when can_create_from_shape is true.
   */
  virtual pcell_parameters_type
  parameters_from_shape(const db::Layout & /*layout*/,
                        const db::Shape & /*shape*/,
                        unsigned int /*layer*/) const {
  ...
  /**
   *  @brief Returns the cell transformation for a PCell of that kind of shape
   * on the given layer
   *
   *  This method is used to derive the initial cell's transformation a new
   * instance from a shape when can_create_from_shape is true.
   */
   ...
   *
   *  In lazy evaluation mode, the PCell is not immediately updated when a
   * parameter is changed in the UI, but only when it is requested to be
   * updated.
   */
  virtual bool wants_lazy_evaluation() const { return false; }

  /**
   *  @brief Gets the Layout object the PCell is registered inside or NULL if it
   * is not registered
   */
   ...
   *
   *  Since the declaration is supposed to be a static object, it can be shared
   * between multiple PCellHeader instances. Using reference counting avoids
   * having to create copies which would involve clone() methods.
   */
  void add_ref();
   ...
   */
  db::pcell_id_type id() const { return m_id; }

  /**
   *  @brief Gets the cached parameter declarations
   *
   *  This method should be preferred over get_parameter_declarations because it
   * returns the cached declarations and therefore is much faster, in particular
   * if the actual implementation is done in a script.
   */
   ...
   *
   *  This method is supposed to return a set of parameter declarations. When
   * the layout is produced, the production method will receive a vector of
   * tl::Variant objects corresponding to the parameter declarations delivered
   * by this method.
   */
  virtual std::vector<PCellParameterDeclaration>
  ...
  map_parameters(const std::map<size_t, tl::Variant> &indexed_parameters) const;

  /**
   *  @brief Map a named parameter set to a parameter vector
   */
  pcell_parameters_type map_parameters(
      const std::map<std::string, tl::Variant> &named_parameters) const;

  /**
   *  @brief Converts a parameter vector to named parameters
   */
  std::map<std::string, tl::Variant>
  named_parameters(const pcell_parameters_type &pv) const;

protected:
  /**
   *  @brief Gets a value indicating whether the PCell wants caching of the
   * parameter declarations
   *
   *  Some PCells with a dynamic parameter definition may not want paramater
   * declaration caching. These PCells can override this method and return
   * false.
   */
  virtual bool wants_parameter_declaration_caching() const { return true; }
