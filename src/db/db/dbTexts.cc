...
Texts::Texts() : mp_delegate(new EmptyTexts()) {
  //  .. nothing yet ..
}
...
Texts::Texts(TextsDelegate *delegate) : mp_delegate(delegate) {
  //  .. nothing yet ..
}
...
template <class Sh> void Texts::insert(const Sh &shape) {
  mutable_texts()->insert(shape);
}

template DB_PUBLIC void Texts::insert(const db::Text &);

void Texts::insert(const db::Shape &shape) { mutable_texts()->insert(shape); }

template <class T> void Texts::insert(const db::Shape &shape, const T &trans) {
  mutable_texts()->insert(shape, trans);
}

template DB_PUBLIC void Texts::insert(const db::Shape &,
                                      const db::ICplxTrans &);
...
void Texts::clear() { set_delegate(new EmptyTexts()); }

void Texts::reserve(size_t n) { mutable_texts()->reserve(n); }

void Texts::flatten() { mutable_texts()->flatten(); }

template <class T> Texts &Texts::transform(const T &trans) {
...
const db::RecursiveShapeIterator &Texts::iter() const {
  static db::RecursiveShapeIterator def_iter;
  const db::RecursiveShapeIterator *i = mp_delegate ? mp_delegate->iter() : 0;
  return *(i ? i : &def_iter);
}

const db::PropertiesRepository &Texts::properties_repository() const {
...
db::PropertiesRepository &Texts::properties_repository() {
...
void Texts::processed(Region &output,
...
namespace tl {
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::Texts &b) {
  db::Text ep;
