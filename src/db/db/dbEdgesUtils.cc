...
JoinEdgesCluster::JoinEdgesCluster(db::PolygonSink *output, coord_type ext_b,
                                   coord_type ext_e, coord_type ext_o,
                                   coord_type ext_i)
    : mp_output(output), m_ext_b(ext_b), m_ext_e(ext_e), m_ext_o(ext_o),
      m_ext_i(ext_i) {
  //  .. nothing yet ..
}
...
void JoinEdgesCluster::finish() {
  ...
  while (!objects_by_p2.empty()) {
    ...
    do {
      ...
      std::multimap<db::Point, iterator>::iterator jj;
      for (jj = objects_by_p2.find(i->first->p2());
           jj != objects_by_p2.end() && jj->first == i->first->p2(); ++jj) {
        if (jj->second == i) {
          break;
        }
      }
      ...
      //  TODO: this chooses any solution in case of forks. Choose a specific
      //  one?
    ...
    if (!cyclic) {
      ...
      //  we have a loop: form a contour by using the polygon size functions and
      //  a "Not" to form the hole
      db::Polygon poly;
...
EdgeSegmentSelector::EdgeSegmentSelector(int mode, Edge::distance_type length,
                                         double fraction)
    : m_mode(mode), m_length(length), m_fraction(fraction) {}

EdgeSegmentSelector::~EdgeSegmentSelector() {}

void EdgeSegmentSelector::process(const db::Edge &edge,
                                  std::vector<db::Edge> &res) const {
  double l = std::max(edge.double_length() * m_fraction, double(m_length));

  if (m_mode < 0) {

    res.push_back(db::Edge(
        edge.p1(),
        db::Point(db::DPoint(edge.p1()) +
                  db::DVector(edge.d()) * (l / edge.double_length()))));

  } else if (m_mode > 0) {

    res.push_back(
        db::Edge(db::Point(db::DPoint(edge.p2()) -
                           db::DVector(edge.d()) * (l / edge.double_length())),
                 edge.p2()));
    ...
    db::DPoint center =
        db::DPoint(edge.p1()) + db::DVector(edge.p2() - edge.p1()) * 0.5;
...
EdgeAngleChecker::EdgeAngleChecker(double angle_start, bool include_angle_start,
...
bool EdgeAngleChecker::check(const db::Vector &a, const db::Vector &b) const {
  ...
  bool vp1 = !opp1 && (m_include_start ? (db::vprod_sign(v1, vout) >= 0)
                                       : (db::vprod_sign(v1, vout) > 0));
  bool vp2 = !opp2 && (m_include_end ? (db::vprod_sign(v2, vout) <= 0)
                                     : (db::vprod_sign(v2, vout) < 0));
...
EdgeOrientationFilter::EdgeOrientationFilter(double amin, bool include_amin,
...
bool EdgeOrientationFilter::selected(const db::Edge &edge) const {
  //  NOTE: this edge normalization confines the angle to a range between (-90
  //  .. 90] (-90 excluded). A horizontal edge has 0 degree, a vertical one has
  //  90 degree.
...
SpecialEdgeOrientationFilter::SpecialEdgeOrientationFilter(FilterType type,
                                                           bool inverse)
...
bool SpecialEdgeOrientationFilter::selected(const db::Edge &edge) const {
  ...
  case Ortho:
    eb = s_ortho_checkers;
    ee = s_ortho_checkers +
         sizeof(s_ortho_checkers) / sizeof(s_ortho_checkers[0]);
    break;
  case Diagonal:
    eb = s_diagonal_checkers;
    ee = s_diagonal_checkers +
         sizeof(s_diagonal_checkers) / sizeof(s_diagonal_checkers[0]);
    break;
  case OrthoDiagonal:
  default:
    eb = s_orthodiagonal_checkers;
    ee = s_orthodiagonal_checkers +
         sizeof(s_orthodiagonal_checkers) / sizeof(s_orthodiagonal_checkers[0]);
    break;
  }
...
struct DetectTagEdgeSink : public db::EdgeSink {
  DetectTagEdgeSink(int tag) : fail_tag(tag), result(true) {}
...
static bool edge_is_inside_or_outside(bool outside, const db::Edge &a,
                                      const db::Polygon &b) {
  db::EdgeProcessor ep;
  ep.insert(b, 0);
  ...
  DetectTagEdgeSink es(outside ? 1 : 2); //  2 is the "outside" tag in "Both"
                                         //  mode -> this makes inside fail
