...
LibraryProxy::LibraryProxy(db::cell_index_type ci, db::Layout &layout,
                           lib_id_type lib_id, cell_index_type lib_cell_index)
...
Cell *LibraryProxy::clone(Layout &layout) const {
  Cell *cell = new LibraryProxy(db::Cell::cell_index(), layout, lib_id(),
                                library_cell_index());
...
std::vector<int>
LibraryProxy::get_layer_indices(db::Layout &layout,
                                db::ImportLayerMapping *layer_mapping) {
  ...
  for (unsigned int i = 0; i < m_layer_indices.size() && reuse_layer_list;
       ++i) {
    reuse_layer_list = layout.is_valid_layer(m_layer_indices[i]) &&
                       lib->layout().is_valid_layer(i) &&
                       layout.get_properties(m_layer_indices[i])
                           .log_equal(lib->layout().get_properties(i));
  }
  ...
  if (!reuse_layer_list) {
    ...
    for (unsigned int i = 0; i < lib->layout().layers(); ++i) {
      ...
      if (i == lib->layout().guiding_shape_layer()) {
        ...
        std::pair<bool, unsigned int> lm =
            layer_mapping->map_layer(lib->layout().get_properties(i));
        if (lm.first) {
...
void LibraryProxy::update(db::ImportLayerMapping *layer_mapping) {
  ...
  for (unsigned int l = 0; l < lib->layout().layers(); ++l) {
    if (layer_indices[l] >= 0) {
      shapes((unsigned int)layer_indices[l])
          .assign_transformed(source_cell.shapes(l), tr, prop_id_map);
    }
  }

  LibraryCellIndexMapper cell_index_mapper(*layout(), lib);

  for (Cell::const_iterator inst = source_cell.begin(); !inst.at_end();
       ++inst) {
...
std::string LibraryProxy::get_basic_name() const {
  Library *lib = LibraryManager::instance().lib(lib_id());
  if (lib) {
...
std::string LibraryProxy::get_display_name() const {
  Library *lib = LibraryManager::instance().lib(lib_id());
  if (lib) {
    const db::Cell *lib_cell = &lib->layout().cell(library_cell_index());
    if (!lib_cell) {
...
std::string LibraryProxy::get_qualified_name() const {
  Library *lib = LibraryManager::instance().lib(lib_id());
  if (lib) {
    const db::Cell *lib_cell = &lib->layout().cell(library_cell_index());
    if (!lib_cell) {
