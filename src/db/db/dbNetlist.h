 ...
 *  Specifically this interface allows manipulating the layout in sync with the
 * netlist.
 */
class DB_PUBLIC NetlistManipulationCallbacks : public tl::Object {
public:
  NetlistManipulationCallbacks() {}
  virtual ~NetlistManipulationCallbacks() {}

  virtual size_t link_net_to_parent_circuit(const db::Net *subcircuit_net,
                                            db::Circuit *parent_circuit,
                                            const db::DCplxTrans &trans) = 0;
 ...
 */
class DB_PUBLIC Netlist : public gsi::ObjectBase, public tl::Object {
public:
  ...
  typedef dereferencing_iterator<tl::vector<Circuit *>::iterator, Circuit>
      top_down_circuit_iterator;
  typedef dereferencing_iterator<tl::vector<const Circuit *>::const_iterator,
                                 const Circuit>
      const_top_down_circuit_iterator;
  typedef dereferencing_iterator<tl::vector<Circuit *>::reverse_iterator,
                                 Circuit>
      bottom_up_circuit_iterator;
  typedef dereferencing_iterator<
      tl::vector<const Circuit *>::const_reverse_iterator, const Circuit>
  ...
  /**
   *  @brief Returns a value indicating whether the netlist names are case
   * sensitive
   */
  bool is_case_sensitive() const { return m_case_sensitive; }

  /**
   *  @brief Sets a value indicating whether the netlist names are case
   * sensitive
   */
   ...
   *
   *  The device classes have to be installed so it's possible to identify the
   * devices by their class.
   */
  void from_string(const std::string &s);

  /**
   *  @brief Starts a sequence of operations during which topology updates are
   * not desired
   *
   *  If the hierarchy is modified, the topology information (top-down order,
   * children and parent information) may be recomputed frequently. This may
   * cause performance issues and may not be desired.
   *
   *  Calling this method will bring the netlist into a state in which updates
   * on the list will not happen. Using "unlock" will end this state.
   *
   *  "lock" and "unlock" are incremental and can be nested. Use "NetlistLocker"
   * for safe locking and unlocking.
   *
   *  Before lock, the state will be validated, so inside the locked operation,
   * the topology information will be valid with respect to the initial state.
   */
  void lock();

  /**
   *  @brief Ends a sequence of operations during which topology updates are not
   * desired See "lock" for more details.
   */
  void unlock();
   ...
   *  This is basically equivalent to calling flatten on all given circuits, but
   * more efficient.
   */
   ...
   */
  circuit_iterator begin_circuits() { return m_circuits.begin(); }

  /**
   *  @brief End iterator for the circuits of the netlist (non-const version)
   */
  circuit_iterator end_circuits() { return m_circuits.end(); }

  /**
   *  @brief Begin iterator for the circuits of the netlist (const version)
   */
  const_circuit_iterator begin_circuits() const { return m_circuits.begin(); }

  /**
   *  @brief End iterator for the circuits of the netlist (const version)
   */
  const_circuit_iterator end_circuits() const { return m_circuits.end(); }
   ...
   *  This iterator will deliver the circuits in a top-down way - i.e. child
   * circuits will always come after parent circuits. The first
   * "top_circuit_count" elements will be top circuits (those which are not
   ...
   *  This iterator will deliver the circuits in a bottom-up way - i.e. child
   * circuits will always come before parent circuits.
   */
  ...
  /**
   *  @brief Begin iterator for the device classes of the netlist (non-const
   * version)
   */
  ...
  /**
   *  @brief End iterator for the device classes of the netlist (non-const
   * version)
   */
  device_class_iterator end_device_classes() { return m_device_classes.end(); }

  /**
   *  @brief Begin iterator for the device classes of the netlist (const
   * version)
   */
  ...
  /**
   *  @brief Begin iterator for the device abstracts of the netlist (non-const
   * version)
   */
  ...
  /**
   *  @brief End iterator for the device abstracts of the netlist (non-const
   * version)
   */
  ...
  /**
   *  @brief Begin iterator for the device abstracts of the netlist (const
   * version)
   */
  ...
  /**
   *  @brief End iterator for the device abstracts of the netlist (const
   * version)
   */
   ...
   */
  DeviceAbstract *
  device_abstract_by_cell_index(db::cell_index_type cell_index) {
   ...
   *  If no device abstract with that cell index exists, null is returned.
   */
  const DeviceAbstract *
  device_abstract_by_cell_index(db::cell_index_type cell_index) const {
    return m_device_abstract_by_cell_index.object_by(cell_index);
  }
   ...
   *
   *  This method will turn all named nets of top-level circuits (such that are
   * not referenced by subcircuits) into pins. This method can be used before
   * purge to avoid that purge will remove nets which are directly connecting to
   * subcircuits.
   */
   ...
   *
   *  This method will purge all nets which return "floating". Circuits which
   * don't have any nets (or only floating ones) and removed. Their subcircuits
   * are disconnected.
   *
   *  This method respects the circuit's "dont_purge" flag and will not purge
   ...
   */
  static int name_compare(bool case_sensitive, const std::string &n1,
                          const std::string &n2);
...
private:
  ...
  object_by_attr<Netlist, Netlist::circuit_iterator, name_attribute<Circuit>>
      m_circuit_by_name;
  object_by_attr<Netlist, Netlist::circuit_iterator,
                 cell_index_attribute<Circuit>>
      m_circuit_by_cell_index;
  object_by_attr<Netlist, Netlist::device_abstract_iterator,
                 name_attribute<DeviceAbstract>>
      m_device_abstract_by_name;
  object_by_attr<Netlist, Netlist::device_abstract_iterator,
                 cell_index_attribute<DeviceAbstract>>
      m_device_abstract_by_cell_index;

  db::NetlistManipulationCallbacks *callbacks() { return mp_callbacks.get(); }
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Netlist &x, bool no_self, void *parent) {
...
public:
  NetlistLocker(Netlist *netlist) : mp_netlist(netlist) {
    if (mp_netlist.get()) {
      mp_netlist->lock();
    }
  }
