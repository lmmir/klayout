 ...
 */
class DB_PUBLIC DeepEdgePairsIterator : public EdgePairsIteratorDelegate {
public:
  ...
  virtual const value_type *get() const { return &m_edge_pair; }
  ...
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const DeepEdgePairsIterator *o =
        dynamic_cast<const DeepEdgePairsIterator *>(other);
...
DeepEdgePairs::DeepEdgePairs() : MutableEdgePairs() {
  //  .. nothing yet ..
}

DeepEdgePairs::DeepEdgePairs(const RecursiveShapeIterator &si,
                             DeepShapeStore &dss)
    : MutableEdgePairs() {
...
DeepEdgePairs::DeepEdgePairs(const RecursiveShapeIterator &si,
                             DeepShapeStore &dss, const db::ICplxTrans &trans)
    : MutableEdgePairs() {
...
DeepEdgePairs::DeepEdgePairs(const DeepLayer &dl) : MutableEdgePairs() {
  set_deep_layer(dl);
}
...
static void transform_deep_layer(db::DeepLayer &deep_layer, const Trans &t) {
  ...
  if (layout.begin_top_down() != layout.end_top_down()) {
    ...
    for (db::RecursiveShapeIterator iter(layout, top_cell, deep_layer.layer());
         !iter.at_end(); ++iter) {
      flat_shapes.insert(
          iter->edge_pair().transformed(iter.trans()).transformed(t));
    }
...
DeepEdgePairs::begin_iter() const {
...
bool DeepEdgePairs::empty() const { return begin_iter().first.at_end(); }

const db::EdgePair *DeepEdgePairs::nth(size_t) const {
  throw tl::Exception(
      tl::to_string(tr("Random access to edge pairs is available only for flat "
                       "edge pair collections")));
}

bool DeepEdgePairs::has_valid_edge_pairs() const { return false; }

const db::RecursiveShapeIterator *DeepEdgePairs::iter() const { return 0; }

void DeepEdgePairs::apply_property_translator(
...
EdgePairsDelegate *DeepEdgePairs::add_in_place(const EdgePairs &other) {
  if (other.empty()) {
    return this;
  }

  const DeepEdgePairs *other_deep =
      dynamic_cast<const DeepEdgePairs *>(other.delegate());
  if (other_deep) {
...
DeepEdgePairs::apply_filter(const EdgePairFilterBase &filter) const {
  ...
  std::unique_ptr<db::DeepEdgePairs> res(
      new db::DeepEdgePairs(edge_pairs.derived()));
...
RegionDelegate *DeepEdgePairs::processed_to_polygons(
    const EdgePairToPolygonProcessorBase &filter) const {
  return shape_collection_processed_impl<db::EdgePair, db::Polygon,
                                         db::DeepRegion>(deep_layer(), filter);
}

EdgesDelegate *DeepEdgePairs::processed_to_edges(
    const EdgePairToEdgeProcessorBase &filter) const {
  return shape_collection_processed_impl<db::EdgePair, db::Edge, db::DeepEdges>(
      deep_layer(), filter);
}
...
RegionDelegate *DeepEdgePairs::polygons(db::Coord e) const {
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator s =
             c->shapes(deep_layer().layer())
                 .begin(db::ShapeIterator::EdgePairs);
         !s.at_end(); ++s) {
      db::Polygon poly = s->edge_pair().normalized().to_polygon(e);
      if (poly.vertices() >= 3) {
        if (s->prop_id() != 0) {
          output.insert(db::PolygonRefWithProperties(
              db::PolygonRef(poly, layout.shape_repository()), s->prop_id()));
        } else {
...
EdgePairsDelegate *DeepEdgePairs::in(const EdgePairs &other,
                                     bool invert) const {
  //  TODO: implement
...
bool DeepEdgePairs::equals(const EdgePairs &other) const {
  const DeepEdgePairs *other_delegate =
      dynamic_cast<const DeepEdgePairs *>(other.delegate());
  if (other_delegate &&
...
void DeepEdgePairs::insert_into(Layout *layout, db::cell_index_type into_cell,
                                unsigned int into_layer) const {
  deep_layer().insert_into(layout, into_cell, into_layer);
}

void DeepEdgePairs::insert_into_as_polygons(Layout *layout,
