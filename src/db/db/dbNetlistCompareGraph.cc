...
static bool is_non_trivial_net(const db::Net *net) {
  return net->pin_count() == 0 && net->terminal_count() == 0 &&
...
Transition::Transition(const db::Device *device, size_t device_category,
                       size_t terminal1_id, size_t terminal2_id) {
  m_ptr = (void *)device;
...
Transition::Transition(const db::SubCircuit *subcircuit,
                       size_t subcircuit_category, size_t pin1_id,
                       size_t pin2_id) {
...
size_t Transition::first_unique_pin_id() {
  return std::numeric_limits<size_t>::max() / 4;
}

CatAndIds Transition::make_key() const {
...
bool Transition::operator<(const Transition &other) const {
  ...
  if (is_for_subcircuit()) {
    ...
    if (subcircuit() != 0) {
      ...
      if (!scc.equals(std::make_pair(subcircuit(), cat()),
                      std::make_pair(other.subcircuit(), other.cat()))) {
        return scc(std::make_pair(subcircuit(), cat()),
                   std::make_pair(other.subcircuit(), other.cat()));
      }
    }
    ...
    if (device() != 0) {
      DeviceCompare dc;
      if (!dc.equals(std::make_pair(device(), cat()),
                     std::make_pair(other.device(), other.cat()))) {
        return dc(std::make_pair(device(), cat()),
                  std::make_pair(other.device(), other.cat()));
      }
    }
...
std::string Transition::to_string() const {
  if (is_for_subcircuit()) {
    ...
    return std::string("X") + sc->expanded_name() + " " + c->name() + " " +
           c->pin_by_id(m_id2)->expanded_name() + " (virtual)";
  } else {
    ...
    return std::string("D") + d->expanded_name() + " " + dc->name() + " " +
           "(" + dc->terminal_definitions()[term_id1].name() + ")->(" +
           dc->terminal_definitions()[term_id2].name() + ")";
  }
}
...
NetGraphNode::NetGraphNode(
    const db::Net *net, DeviceCategorizer &device_categorizer,
    CircuitCategorizer &circuit_categorizer, const DeviceFilter &device_filter,
    const std::map<const db::Circuit *, CircuitMapper> *circuit_map,
    const CircuitPinCategorizer *pin_map, size_t *unique_pin_id)
  ...
  for (db::Net::const_subcircuit_pin_iterator i = net->begin_subcircuit_pins();
    ...
    if (icm == circuit_map->end()) {
      //  this can happen if the other circuit is not present - this is allowed
      //  for single-pin circuits.
      continue;
    }

    const CircuitMapper *cm = &icm->second;

    //  A pin assignment may be missing because there is no (real) net for a pin
    //  -> skip this pin
    ...
    if (!cm->has_other_pin_for_this_pin(pin_id)) {
      ...
      //  NOTE: if cm is given, cr and pin_id are given in terms of the
      //  canonical "other" circuit. For c1 this is the c1->c2 mapper, for c2
      //  this is the c2->c2 dummy mapper.
    ...
    //  Subcircuits are routed to a null node and descend from a virtual node
    //  inside the subcircuit. The reasoning is that this way we don't need
    //  #pins*(#pins-1) edges but rather #pins.

    Transition ed(sc, circuit_cat, pin_id, original_pin_id);

    std::map<const void *, size_t>::const_iterator in =
        n2entry.find((const void *)sc);
    if (in == n2entry.end()) {
      in = n2entry.insert(std::make_pair((const void *)sc, m_edges.size()))
               .first;
      m_edges.push_back(
          edge_type(std::vector<Transition>(),
                    std::make_pair(size_t(0), (const db::Net *)0)));
    }
  ...
  for (db::Net::const_terminal_iterator i = net->begin_terminals();
    ...
    size_t terminal1_id = is_strict
                              ? i->terminal_id()
                              : translate_terminal_id(i->terminal_id(), d);

    const std::vector<db::DeviceTerminalDefinition> &td =
        d->device_class()->terminal_definitions();
    for (std::vector<db::DeviceTerminalDefinition>::const_iterator it =
             td.begin();
         it != td.end(); ++it) {
      ...
      if (it->id() != i->terminal_id()) {
        ...
        size_t terminal2_id =
            is_strict ? it->id() : translate_terminal_id(it->id(), d);
        ...
        std::map<const void *, size_t>::const_iterator in =
            n2entry.find((const void *)net2);
        if (in == n2entry.end()) {
          in =
              n2entry.insert(std::make_pair((const void *)net2, m_edges.size()))
                  .first;
          m_edges.push_back(edge_type(std::vector<Transition>(),
                                      std::make_pair(size_t(0), net2)));
        }
...
NetGraphNode::NetGraphNode(
    const db::SubCircuit *sc, CircuitCategorizer &circuit_categorizer,
    const std::map<const db::Circuit *, CircuitMapper> *circuit_map,
    const CircuitPinCategorizer *pin_map, size_t *unique_pin_id)
  ...
  for (db::Circuit::const_pin_iterator p = cr->begin_pins();
    ...
    std::map<const db::Net *, size_t>::const_iterator in =
        n2entry.find(net_at_pin);
    if (in == n2entry.end()) {
      in = n2entry
               .insert(
                   std::make_pair((const db::Net *)net_at_pin, m_edges.size()))
               .first;
      m_edges.push_back(edge_type(std::vector<Transition>(),
                                  std::make_pair(size_t(0), net_at_pin)));
    }
...
void NetGraphNode::expand_subcircuit_nodes(NetGraph *graph) {
  ...
  for (std::list<edge_type>::const_iterator e = sc_edges.begin();
       e != sc_edges.end(); ++e) {
    ...
    for (NetGraphNode::edge_iterator de = dn.begin(); de != dn.end(); ++de) {
      ...
      if (in == n2entry.end()) {
        in = n2entry
                 .insert(std::make_pair((const db::Net *)net_at_pin,
                                        m_edges.size()))
                 .first;
        m_edges.push_back(edge_type(std::vector<Transition>(), de->second));
      }

      m_edges[in->second].first.insert(m_edges[in->second].first.end(),
                                       de->first.begin(), de->first.end());
    }
  }

  //  "deep sorting" of the edge descriptor
  for (std::vector<edge_type>::iterator i = m_edges.begin(); i != m_edges.end();
       ++i) {
...
std::string NetGraphNode::to_string() const {
  ...
  for (std::vector<edge_type>::const_iterator e = m_edges.begin();
       e != m_edges.end(); ++e) {
    res += "  (\n";
    for (std::vector<Transition>::const_iterator i = e->first.begin();
         i != e->first.end(); ++i) {
    ...
    if (!e->second.second) {
      ...
      res += e->second.second->expanded_name() + "[#" +
             tl::to_string(e->second.first) + "]";
    }
    res += "\n";
  }
  return res;
...
void NetGraphNode::apply_net_index(
    const std::map<const db::Net *, size_t> &ni) {
  for (std::vector<edge_type>::iterator i = m_edges.begin(); i != m_edges.end();
       ++i) {
    std::map<const db::Net *, size_t>::const_iterator j =
        ni.find(i->second.second);
    tl_assert(j != ni.end());
...
bool NetGraphNode::net_less(const db::Net *a, const db::Net *b,
                            bool with_name) {
  if ((a != 0) != (b != 0)) {
    return (a != 0) < (b != 0);
  }
...
bool NetGraphNode::net_equal(const db::Net *a, const db::Net *b,
                             bool with_name) {
  if ((a != 0) != (b != 0)) {
    return false;
  }
...
void NetGraph::build(
    const db::Circuit *c, DeviceCategorizer &device_categorizer,
    CircuitCategorizer &circuit_categorizer,
    const db::DeviceFilter &device_filter,
    const std::map<const db::Circuit *, CircuitMapper> *circuit_and_pin_mapping,
    const CircuitPinCategorizer *circuit_pin_mapper, size_t *unique_pin_id) {
  tl::SelfTimer timer(tl::verbosity() >= 31,
                      tl::to_string(tr("Building net graph for circuit: ")) +
                          c->name());
  ...
  m_nodes.push_back(NetGraphNode(0, device_categorizer, circuit_categorizer,
                                 device_filter, circuit_and_pin_mapping,
                                 circuit_pin_mapper, unique_pin_id));
  ...
  for (db::Circuit::const_net_iterator n = c->begin_nets(); n != c->end_nets();
       ++n) {
    NetGraphNode node(n.operator->(), device_categorizer, circuit_categorizer,
                      device_filter, circuit_and_pin_mapping,
                      circuit_pin_mapper, unique_pin_id);
  ...
  for (std::vector<NetGraphNode>::const_iterator i = m_nodes.begin();
       i != m_nodes.end(); ++i) {
    m_net_index.insert(std::make_pair(i->net(), i - m_nodes.begin()));
  }

  for (std::vector<NetGraphNode>::iterator i = m_nodes.begin();
       i != m_nodes.end(); ++i) {
  ...
  for (db::Circuit::const_subcircuit_iterator i = c->begin_subcircuits();
       i != c->end_subcircuits(); ++i) {
    ...
    std::map<const db::Circuit *, CircuitMapper>::const_iterator icm =
        circuit_and_pin_mapping->find(cr);
    if (icm == circuit_and_pin_mapping->end()) {
      continue;
    }

    m_virtual_nodes.insert(std::make_pair(
        i.operator->(), NetGraphNode(i.operator->(), circuit_categorizer,
                                     circuit_and_pin_mapping,
                                     circuit_pin_mapper, unique_pin_id)));
  }

  for (std::map<const db::SubCircuit *, NetGraphNode>::iterator i =
           m_virtual_nodes.begin();
       i != m_virtual_nodes.end(); ++i) {
...
NetGraphNode NetGraph::joined(const NetGraphNode &a,
                              const NetGraphNode &b) const {
  NetGraphNode nj = a;
  ...
  for (int m = 0; m < 2; ++m) {
    ...
    for (auto i = n.begin(); i != n.end(); ++i) {
      ...
      if (i->second.second) {
        ...
        const db::Net *net =
            i->second.second == b.net() ? a.net() : i->second.second;

        auto j = joined.find(net);
        if (j != joined.end()) {
          j->second.first.insert(j->second.first.end(), i->first.begin(),
                                 i->first.end());
        } else {
