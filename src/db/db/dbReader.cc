...
ReaderBase::ReaderBase() : m_warnings_as_errors(false), m_warn_level(1) {}

ReaderBase::~ReaderBase() {}

void ReaderBase::set_warnings_as_errors(bool f) { m_warnings_as_errors = f; }
...
Reader::Reader(tl::InputStream &stream)
  ...
  for (tl::Registrar<db::StreamFormatDeclaration>::iterator rdr =
       ...
       rdr != tl::Registrar<db::StreamFormatDeclaration>::end() &&
       !mp_actual_reader;
       ++rdr) {
    m_stream.reset();
  ...
  if (!mp_actual_reader) {
    ...
    throw db::ReaderUnknownFormatException(
        tl::to_string(tr("Stream has unknown format: ")) + stream.source(),
        head, has_more);
  }
}
...
const db::LayerMap &Reader::read(db::Layout &layout,
                                 const db::LoadLayoutOptions &options) {
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Reading file: ")) + m_stream.source());
