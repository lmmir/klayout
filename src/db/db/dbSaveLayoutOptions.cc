...
SaveLayoutOptions::SaveLayoutOptions()
    : m_format("GDS2"), m_all_layers(true), m_all_cells(true), m_dbu(0.0),
      m_scale_factor(1.0), m_keep_instances(false), m_write_context_info(true),
      m_dont_write_empty_cells(false) {
  // .. nothing yet ..
}

SaveLayoutOptions::~SaveLayoutOptions() { release(); }

SaveLayoutOptions::SaveLayoutOptions(const SaveLayoutOptions &d) {
  operator=(d);
}

SaveLayoutOptions &SaveLayoutOptions::operator=(const SaveLayoutOptions &d) {
  if (this != &d) {
    ...
    release();
    for (std::map<std::string, FormatSpecificWriterOptions *>::const_iterator
             o = d.m_options.begin();
         o != d.m_options.end(); ++o) {
...
void SaveLayoutOptions::release() {
  for (std::map<std::string, FormatSpecificWriterOptions *>::const_iterator o =
           m_options.begin();
       o != m_options.end(); ++o) {
    delete o->second;
  }
...
void SaveLayoutOptions::set_options(
    const FormatSpecificWriterOptions &options) {
...
void SaveLayoutOptions::set_options(FormatSpecificWriterOptions *options) {
  ...
  std::map<std::string, FormatSpecificWriterOptions *>::iterator o =
      m_options.find(options->format_name());
  if (o != m_options.end()) {
    delete o->second;
    m_options.erase(o);
  }
...
SaveLayoutOptions::get_options(const std::string &format) const {
  std::map<std::string, FormatSpecificWriterOptions *>::const_iterator o =
      m_options.find(format);
  if (o != m_options.end()) {
    return o->second;
  } else {
    return 0;
...
void SaveLayoutOptions::set_option_by_name(const std::string &method,
...
tl::Variant SaveLayoutOptions::get_option_by_name(const std::string &method) {
  //  Utilizes the GSI binding to set the values
  tl::Variant options_ref = tl::Variant::make_variant_ref(this);
  const tl::EvalClass *eval_cls = options_ref.user_cls()->eval_cls();
  tl::ExpressionParserContext context;
...
void SaveLayoutOptions::add_layer(unsigned int layer,
...
void SaveLayoutOptions::set_dbu(double dbu) { m_dbu = dbu; }

void SaveLayoutOptions::set_scale_factor(double f) { m_scale_factor = f; }

void SaveLayoutOptions::set_dont_write_empty_cells(bool f) {
  m_dont_write_empty_cells = f;
}

void SaveLayoutOptions::get_valid_layers(
    const db::Layout &layout,
    std::vector<std::pair<unsigned int, db::LayerProperties>> &layers,
    db::SaveLayoutOptions::LayerAssignmentMode lm) const {
  ...
  if (m_all_layers) {
    ...
    for (unsigned int l = 0; l < layout.layers(); ++l) {
      if (layout.is_valid_layer(l)) {
        const db::LayerProperties &prop = layout.get_properties(l);
        std::map<unsigned int, db::LayerProperties>::const_iterator ll =
            m_layers.find(l);
        if (ll != m_layers.end()) {
  ...
  if (lm == LP_OnlyNumbered) {
    ...
    for (std::vector<
             std::pair<unsigned int, db::LayerProperties>>::const_iterator l =
             all_layers.begin();
         l != all_layers.end(); ++l) {
    ...
    for (std::vector<
      ...
      if (l->second.name.empty()) {
        layers.back().second =
            tl::sprintf("L%dD%d", l->second.layer, l->second.datatype);
      } else if (l->second.layer >= 0 && l->second.datatype >= 0) {
        layers.back().second =
            tl::sprintf("L%dD%d", l->second.layer, l->second.datatype) + "_" +
            l->second.name;
      }
    }
...
void SaveLayoutOptions::get_cells(
    const db::Layout &layout, std::set<db::cell_index_type> &cells,
    const std::vector<std::pair<unsigned int, db::LayerProperties>>
        &valid_layers,
    bool require_unique_names) const {
  if (m_all_cells) {

    for (db::Layout::const_iterator cell = layout.begin(); cell != layout.end();
         ++cell) {
    ...
    for (std::set<db::cell_index_type>::const_iterator c = m_cells.begin();
         c != m_cells.end(); ++c) {
      cells.insert(*c);
  ...
  if (m_dont_write_empty_cells) {
    ...
    for (std::set<db::cell_index_type>::const_iterator c = cells.begin();
      ...
      if (is_empty) {
        ...
        //  treat only cells as empty which are referenced within the cell set
        //  we have defined - other cells will become top cells and must not be
        //  deleted
        bool is_top_cell = true;

        for (db::Cell::parent_cell_iterator p = cref.begin_parent_cells();
             p != cref.end_parent_cells() && is_top_cell; ++p) {
    ...
    for (std::set<db::cell_index_type>::const_iterator c = empty_cells.begin();
         c != empty_cells.end(); ++c) {
      cells.erase(*c);
    }
  }
  ...
  if (require_unique_names) {
    ...
    for (std::set<db::cell_index_type>::const_iterator c = cells.begin();
         c != cells.end(); ++c) {
      use_count.insert(std::make_pair(std::string(layout.cell_name(*c)), 0))
          .first->second += 1;
    }

    std::vector<std::string> multi;
    for (std::map<std::string, unsigned int>::const_iterator u =
             use_count.begin();
         u != use_count.end(); ++u) {
      if (u->second > 1) {
    ...
    if (!multi.empty()) {
      throw tl::Exception(
          tl::to_string(tr("The following cell name(s) are used for more than "
                           "one cell - can't write this layout:\n  ")) +
          tl::join(multi, "\n  "));
    }
  }
}

bool SaveLayoutOptions::set_format_from_filename(const std::string &fn) {
