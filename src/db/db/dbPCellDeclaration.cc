...
ParameterStates::ParameterStates() : m_states() {
  //  .. nothing yet ..
}
...
ParameterStates &ParameterStates::operator=(const ParameterStates &other) {
  if (this != &other) {
    m_states = other.m_states;
  }
  return *this;
...
void ParameterStates::set_parameter(const std::string &name,
                                    const ParameterState &ps) {
  m_states[name] = ps;
}

ParameterState &ParameterStates::parameter(const std::string &name) {
...
bool ParameterStates::values_are_equal(const db::ParameterStates &other) const {
  ...
  while (i != m_states.end() && j != other.m_states.end() &&
         i->first == j->first && i->second.value() == j->second.value()) {
    ++i;
    ++j;
  }
...
PCellDeclaration::PCellDeclaration()
    : m_ref_count(0), m_id(0), mp_layout(0),
      m_has_parameter_declarations(false) {
  // .. nothing yet ..
}

void PCellDeclaration::add_ref() { ++m_ref_count; }
...
PCellDeclaration::parameter_declarations() const {
  if (!m_has_parameter_declarations || !wants_parameter_declaration_caching()) {
    ...
    //  NOTE: this ensures that reallocation of the vector only happens if the
    //  parameters change. This makes the returned reference more stable and
    //  iterators over this reference don't get invalidated so easily if
    //  wants_parameter_declaration_caching is false.
...
const std::string &PCellDeclaration::parameter_name(size_t index) {
...
pcell_parameters_type PCellDeclaration::map_parameters(
    const std::map<size_t, tl::Variant> &param_by_name) const {
  db::pcell_parameters_type new_param;
  size_t i = 0;
  const std::vector<db::PCellParameterDeclaration> &pcp =
      parameter_declarations();
  for (std::vector<PCellParameterDeclaration>::const_iterator pd = pcp.begin();
       pd != pcp.end(); ++pd, ++i) {
...
pcell_parameters_type PCellDeclaration::map_parameters(
    const std::map<std::string, tl::Variant> &param_by_name) const {
  db::pcell_parameters_type new_param;
  const std::vector<db::PCellParameterDeclaration> &pcp =
      parameter_declarations();
  for (std::vector<PCellParameterDeclaration>::const_iterator pd = pcp.begin();
    ...
    std::map<std::string, tl::Variant>::const_iterator p =
        param_by_name.find(pd->get_name());
    if (p != param_by_name.end()) {
      new_param.push_back(p->second);
    } else {
      new_param.push_back(pd->get_default());
