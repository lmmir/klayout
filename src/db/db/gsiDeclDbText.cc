...
template <class C> struct text_defs {
  ...
  static C *new_sthf(const char *s, const simple_trans_type &t, coord_type h,
                     int f) {
  ...
  static coord_type get_x(const C *t) { return t->trans().disp().x(); }
  ...
  static coord_type get_y(const C *t) { return t->trans().disp().y(); }

  static void set_font(C *t, int f) { t->font(db::Font(f)); }

  static int get_font(C *t) { return t->font(); }

  static point_type get_pos(C *t) { return t->trans() * point_type(); }
  ...
  static void set_halign(C *t, db::HAlign f) { t->halign(f); }

  static void set_halign_int(C *t, int f) { t->halign(db::HAlign(f)); }

  static db::HAlign get_halign(C *t) { return t->halign(); }

  static void set_valign(C *t, db::VAlign f) { t->valign(f); }

  static void set_valign_int(C *t, int f) { t->valign(db::VAlign(f)); }

  static db::VAlign get_valign(C *t) { return t->valign(); }
  ...
  static gsi::Methods methods() {
    return constructor(
           ...
           constructor(
               "new", &new_st, gsi::arg("string"), gsi::arg("trans"),
               "@brief Constructor with string and transformation\n"
               "\n"
               "\n"
               "A string and a transformation is provided to this constructor. "
               "The transformation "
               "specifies the location and orientation of the text object.") +
           constructor("new", &new_sxy, gsi::arg("string"), gsi::arg("x"),
                       gsi::arg("y"),
                       "@brief Constructor with string and location\n"
                       "\n"
                       "\n"
                       "A string and a location is provided to this "
                       "constructor. The location "
                       "is specifies as a pair of x and y coordinates.\n"
                       "\n"
                       "This method has been introduced in version 0.23.") +
           constructor("new", &new_sthf, gsi::arg("string"), gsi::arg("trans"),
                       gsi::arg("height"), gsi::arg("font"),
                       "@brief Constructor with string, transformation, text "
                       "height and font\n"
                       "\n"
                       "\n"
                       "A string and a transformation is provided to this "
                       "constructor. The transformation "
                       "specifies the location and orientation of the text "
                       "object. In addition, the text height "
           ...
           method_ext("bbox", get_bbox,
                      ...
                      "The bounding box of the text is a single point - the "
           ...
           method("trans=", (void (C::*)(const simple_trans_type &)) & C::trans,
                  gsi::arg("t"),
                  "@brief Assign a transformation (text position and "
                  "orientation) to this object\n") +
           ...
           method_ext("font=", &set_font, gsi::arg("f"),
                      ...
                      "The font number does not play a role for KLayout. This "
                      "property is provided "
                      "for compatibility with other systems which allow using "
                      "different fonts for the text objects.") +
           ...
           method_ext("#halign=", &set_halign_int, gsi::arg("a"),
                      ...
                      "\n"
                      "This is the version accepting integer values. It's "
                      "provided for backward compatibility.\n") +
           method_ext("halign=", &set_halign, gsi::arg("a"),
                      "@brief Sets the horizontal alignment\n"
                      "\n"
                      "This property specifies how the text is aligned "
                      "relative to the anchor point. "
                      "\n"
                      "This property has been introduced in version 0.22 and "
                      "extended to enums in 0.28.\n") +
           ...
           method_ext(
               "move", &move_xy, gsi::arg("dx"), gsi::arg("dy"),
               "@brief Moves the text by a certain distance (modifies self)\n"
               "\n"
               "\n"
               "Moves the text by a given distance in x and y direction and "
               "returns the moved\n"
               "text. Does not check for coordinate overflows.\n"
               "\n"
           ...
           method_ext(
               "moved", &moved, gsi::arg("distance"),
               "@brief Returns the text moved by a certain distance (does not "
               "modify self)\n"
               "\n"
               "\n"
               "Moves the text by a given offset and returns the moved\n"
           ...
           method_ext(
               "moved", &moved_xy, gsi::arg("dx"), gsi::arg("dy"),
               "@brief Returns the text moved by a certain distance (does not "
               "modify self)\n"
               "\n"
               "\n"
           ...
           method("transformed", &C::template transformed<simple_trans_type>,
                  gsi::arg("t"),
                  "@brief Transforms the text with the given simple "
                  "transformation\n"
                  "\n"
                  "\n"
           ...
           method("transformed", &C::template transformed<complex_trans_type>,
                  gsi::arg("t"),
                  "@brief Transforms the text with the given complex "
                  "transformation\n"
                  "\n"
                  "\n"
           ...
           method("==", &C::equal, gsi::arg("text"),
                  ...
                  "\n"
                  "\n"
                  "Return true, if this text object and the given text are "
                  "equal ") +
           method("!=", &C::not_equal, gsi::arg("text"),
                  "@brief Inequality\n"
                  "\n"
                  "\n"
                  "Return true, if this text object and the given text are not "
                  "equal ") +
           method_ext("hash", &hash_value,
                      "@brief Computes a hash value\n"
                      "Returns a hash value for the given text object. This "
                      "method enables texts as hash keys.\n"
                      "\n"
...
static db::Text *text_from_dtext(const db::DText &t) { return new db::Text(t); }

static db::DText text_to_dtext(const db::Text *t, double dbu) {
  return db::DText(*t * dbu);
}

Class<db::Text> decl_Text(
    "db", "Text",
    constructor("new", &text_from_dtext, gsi::arg("dtext"),
                "@brief Creates an integer coordinate text from a "
                "floating-point coordinate text"
                "\n"
                "This constructor has been introduced in version 0.25 and "
                "replaces the previous static method 'from_dtext'.") +
        method_ext(
            "to_dtype", &text_to_dtext, gsi::arg("dbu", 1.0),
            "@brief Converts the text to a floating-point coordinate text"
            "\n"
            "The database unit can be specified to translate the "
            "integer-coordinate text into a floating-point coordinate "
            "text in micron units. The database unit is basically a scaling "
            "factor.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        method(
            "transformed", &db::Text::transformed<db::ICplxTrans>,
            gsi::arg("t"),
            "@brief Transform the text with the given complex transformation\n"
            "\n"
            "\n"
            "@param t The magnifying transformation to apply\n"
            "@return The transformed text (in this case an integer coordinate "
            "object now)\n"
            "\n"
    ...
    "Text objects are used as labels (i.e. for pins) or to indicate a "
    "particular position.\n"
    "\n"
    "The \\Text class uses integer coordinates. A class that operates with "
...
Class<db::DText> decl_DText(
    "db", "DText",
    constructor("new", &dtext_from_itext, gsi::arg("Text"),
                "@brief Creates a floating-point coordinate text from an "
                "integer coordinate text\n"
                "\n"
                "This constructor has been introduced in version 0.25 and "
                "replaces the previous static method 'from_itext'.") +
        method_ext("to_itype", &dtext_to_text, gsi::arg("dbu", 1.0),
                   ...
                   "Text in micron units to an integer-coordinate text in "
                   "database units. The text's "
                   "coordinates will be divided by the database unit.\n"
                   "\n"
                   "This method has been introduced in version 0.25.") +
        method(
            "transformed", &db::DText::transformed<db::VCplxTrans>,
            gsi::arg("t"),
    ...
    "particular position.\n"
    "\n"
    "The \\DText class uses floating-point coordinates. A class that operates "
