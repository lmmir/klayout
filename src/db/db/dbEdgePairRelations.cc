...
db::Edge::distance_type edge_projection(const db::Edge &a, const db::Edge &b) {
  ...
  return db::coord_traits<db::Coord>::rounded(a.double_length() *
                                              fabs(l2 - l1));
}

/**
 *  @brief Returns the part of the "other" edge which is on the inside side of e
 * and within distance d
 *
 *  This function applies Euclidian metrics.
 *  If no such part is found, this function returns false.
 */
bool euclidian_near_part_of_edge(bool include_zero,
                                 db::coord_traits<db::Coord>::distance_type d,
                                 const db::Edge &e, const db::Edge &other,
                                 db::Edge *output) {
  ...
  //  "kissing corner" issue: force include zero if the edges are collinear and
  //  overlap.
  ...
  double l1 = std::numeric_limits<double>::max(),
         l2 = -std::numeric_limits<double>::max();
  ...
  if (e.parallel(g)) {
    ...
    double lp1 =
        db::vprod(db::DVector(e1d - db::DPoint(g.p1())), db::DVector(e.d())) /
        det;
    double lp2 =
        db::vprod(db::DVector(e.p1() - g.p1()), db::DVector(e.d())) / det;
    if (lp1 > lp2) {
      std::swap(lp1, lp2);
    }
  ...
  if (l1 >= l2) {
    ...
    if (output) {
      *output = db::Edge(g.p1() + db::Vector(g.d() * l1),
                         g.p1() + db::Vector(g.d() * l2));
    }
    return true;
  }
}
 ...
 *  If no such part is found, this function returns false.
 */
static bool var_near_part_of_edge(bool include_zero,
                                  db::coord_traits<db::Coord>::distance_type d,
                                  db::coord_traits<db::Coord>::distance_type dd,
                                  const db::Edge &e, const db::Edge &other,
                                  db::Edge *output) {
  //  Handle the case of point-like basic edge: cannot determine
  //  orientation
  ...
  if (g.is_degenerate()) {
    ...
    if (db::sprod(db::Vector(g.p1() - e.p1()), e.d()) <
        -(dd * e.double_length())) {
      return false;
    }
    if (db::sprod(db::Vector(e.p2() - g.p1()), e.d()) <
        -(dd * e.double_length())) {
      return false;
    }
    if (output) {
  ...
  double l1 = std::numeric_limits<double>::min(),
         l2 = std::numeric_limits<double>::max();
  ...
  if (db::sprod_sign(e, g) == 0) {
    if (db::sprod(db::Vector(g.p1() - e.p1()), e.d()) <
            -(dd * e.double_length()) ||
        db::sprod(db::Vector(e.p2() - g.p1()), e.d()) <
            -(dd * e.double_length())) {
      return false;
    }
  } else {
    ...
    double det = db::vprod(db::DVector(g.d()), en);
    double lt1 =
        db::vprod(db::DVector(e.p1() - g.p1()) - ep * double(dd), en) / det;
    double lt2 =
        db::vprod(db::DVector(e.p2() - g.p1()) + ep * double(dd), en) / det;
    if (lt1 > lt2) {
      std::swap(lt1, lt2);
    }
 ...
 *  If no such part is found, this function returns false.
 */
bool projected_near_part_of_edge(bool include_zero,
                                 db::coord_traits<db::Coord>::distance_type d,
                                 const db::Edge &e, const db::Edge &other,
                                 db::Edge *output) {
 ...
 *  If no such part is found, this function returns false.
 */
bool square_near_part_of_edge(bool include_zero,
                              db::coord_traits<db::Coord>::distance_type d,
                              const db::Edge &e, const db::Edge &other,
                              db::Edge *output) {
...
EdgeRelationFilter::EdgeRelationFilter(edge_relation_type r, distance_type d,
                                       metrics_type metrics,
                                       double ignore_angle,
                                       distance_type min_projection,
                                       distance_type max_projection)
    : m_whole_edges(false), m_include_zero(true), m_r(r), m_d(d),
      m_metrics(metrics), m_ignore_angle(0), m_min_projection(min_projection),
      m_max_projection(max_projection) {
...
bool EdgeRelationFilter::check(const db::Edge &a, const db::Edge &b,
                               db::EdgePair *output) const {
  //  check projection criterion

  if (m_min_projection > 0 ||
      m_max_projection < std::numeric_limits<distance_type>::max()) {
  ...
  if (m_ignore_angle == 90.0) {
    ...
    if (-db::sprod(aa, b) / (aa.double_length() * b.double_length()) <
        m_ignore_angle_cos + 1e-10) {
      return false;
    }
  }
  ...
  if (m_metrics == Euclidian) {
    in2 = euclidian_near_part_of_edge(
        m_include_zero, m_d, an, bn,
        !m_whole_edges && output ? &output->second() : 0);
    in1 = euclidian_near_part_of_edge(
        m_include_zero, m_d, bn, an,
        !m_whole_edges && output ? &output->first() : 0);
  } else if (m_metrics == Square) {
    in2 = square_near_part_of_edge(m_include_zero, m_d, an, bn,
                                   !m_whole_edges && output ? &output->second()
                                                            : 0);
    in1 = square_near_part_of_edge(m_include_zero, m_d, bn, an,
                                   !m_whole_edges && output ? &output->first()
                                                            : 0);
  } else {
    in2 = projected_near_part_of_edge(
        m_include_zero, m_d, an, bn,
        !m_whole_edges && output ? &output->second() : 0);
    in1 = projected_near_part_of_edge(
        m_include_zero, m_d, bn, an,
        !m_whole_edges && output ? &output->first() : 0);
  }
