...
template <class Path, class Tr> class path_point_iterator {
public:
   ...
   */
  path_point_iterator() : mp_pointlist(0), m_index(0) {
    //  .. nothing yet ..
  }
   ...
   */
  template <class T>
  path_point_iterator(const path_point_iterator<Path, T> &d,
   ...
   *  @brief Point access
   */
  point_type operator*() const { return m_trans((*mp_pointlist)[m_index]); }
...
template <class C> class DB_PUBLIC path {
public:
   ...
   *  a zero.
   */
  path() : m_width(0), m_bgn_ext(0), m_end_ext(0) {
    // .. nothing yet ..
  }
   ...
   */
  template <class Iter>
  path(Iter from, Iter to, coord_type width, coord_type bgn_ext = 0,
       coord_type end_ext = 0, bool round = false)
      : m_width(round ? -width : width), m_bgn_ext(bgn_ext),
        m_end_ext(end_ext) {
  ...
  path(const path<D> &p, const TP &tp, const TL &tl)
    ...
    for (typename path<D>::pointlist_type::const_iterator pp =
             p.m_points.begin();
         pp != p.m_points.end(); ++pp) {
  ...
  explicit path(const path<D> &p)
      : m_width(p.m_width < 0 ? -db::coord_converter<C, D>()(-p.m_width)
                              : db::coord_converter<C, D>()(p.m_width)),
   ...
   */
  void translate(const path<C> &d, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
  }
   ...
   */
  template <class T>
  void translate(const path<C> &d, const T &t, db::generic_repository<C> &,
                 db::ArrayRepository &) {
    *this = d;
    transform(t);
  }
   ...
   *  @brief Inequality test
   */
  bool operator!=(const path<C> &b) const { return !operator==(b); }
   ...
   */
  bool not_equal(const path<C> &b) const { return !equal(b); }
   ...
   *  @brief Get the width
   */
  coord_type width() const { return m_width < 0 ? -m_width : m_width; }
   ...
   */
  bool round() const { return m_width < 0; }
   ...
   */
  coord_type bgn_ext() const { return m_bgn_ext; }
   ...
   */
  coord_type end_ext() const { return m_end_ext; }
   ...
   */
  template <class Iter, class Op> void assign(Iter from, Iter to, Op t) {
    m_bbox = box_type();
    m_points.clear();
   ...
   */
  iterator begin() const { return iterator(m_points, 0); }
   ...
   */
  iterator end() const { return iterator(m_points, m_points.size()); }
   ...
   */
  template <class Tr> path<C> &transform(const Tr &t) {
    m_bbox = box_type();
    if (m_width < 0) {
    ...
    for (typename pointlist_type::iterator p = m_points.begin();
         p != m_points.end(); ++p) {
      p->transform(t);
    }
  ...
  path<typename Tr::target_coord_type> transformed(const Tr &t) const {
    ...
    for (typename pointlist_type::const_iterator p = m_points.begin();
         p != m_points.end(); ++p) {
   ...
   *  It is basically two times the length plus width. Extensions are taken into
   * account but the precise effect of the corner treatment is not.
   */
   ...
   *  It is basically the length times the width. Extensions are taken into
   * account but the precise effect of the corner treatment is not.
   */
  ...
  void hull(PointList &pts,
            int semi_circ_pts = db::num_circle_points() / 2) const {
    ...
    create_shifted_points(m_bgn_ext, m_end_ext, width(), true,
                          tmp_points.begin(), tmp_points.end(),
                          round() ? semi_circ_pts : 2, std::back_inserter(pts));
    create_shifted_points(m_end_ext, m_bgn_ext, width(), false,
                          tmp_points.rbegin(), tmp_points.rend(),
                          round() ? semi_circ_pts : 2, std::back_inserter(pts));
  }
   ...
   *  be self-overlapping.
   *
   *  This version allows one to override the left and right side's width hence
   * creating asymmetric paths. dleft is the shift to the left (as seen in the
   * direction of the path) and dright the shift to the right. The default path
   * is created if dleft+dright=width.
   */
  template <class PointList>
  void hull(PointList &pts, C dleft, C dright,
            int semi_circ_pts = db::num_circle_points() / 2) const {
    pts.reserve(m_points.size() * 2); //  minimum number of points required
    ...
    create_shifted_points(m_bgn_ext, m_end_ext, dleft * 2, true,
                          tmp_points.begin(), tmp_points.end(),
                          round() ? semi_circ_pts : 2, std::back_inserter(pts));
    create_shifted_points(m_end_ext, m_bgn_ext, dright * 2, false,
                          tmp_points.rbegin(), tmp_points.rend(),
                          round() ? semi_circ_pts : 2, std::back_inserter(pts));
  }
...
private:
  ...
  void create_shifted_points(coord_type start, coord_type end, coord_type width,
                             bool forward, Iter from, Iter to, int ncircle,
                             Inserter pts) const;
};
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const path<C> &x, bool no_self = false,
                     void *parent = 0) {
 ...
 */
template <class C> inline path<double> operator*(const path<C> &p, double s) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const path<C> &p) {
...
struct path_ref : public shape_ref<Path, Trans> {
  ...
  /**
   *  @brief The constructor creating a reference from an path pointer and
   * transformation
   *
   ...
   *
   *  This constructor allows one to copy a path reference with a certain
   ...
   */
  iterator end() const { return iterator(this->obj().end(), this->trans()); }
...
inline path_ref<Path, TargetTr> operator*(const TargetTr &t,
                                          const path_ref<Path, Tr> &p) {
  return p.transformed(t);
}
...
/**
 *  @brief The path reference (without transformation) typedef for double
 * coordinates
 */
 ...
 *  @param npoints The number of points per full circle used for the circle
 * approximation
 *  @param accuracy The internal numerical accuracy. This value should be half
 * the database unit.
 *  @return The new path with the corners replaced by an arc interpolation
 */
DB_PUBLIC DPath round_path_corners(const DPath &input, double rad, int npoints,
                                   double accuracy);
...
//  std namespace
template <class C> void swap(db::path<C> &a, db::path<C> &b) { a.swap(b); }
