 ...
 *
 *  Mutable edge collections offer insert, transform, flatten and other
 * manipulation functions.
 */
class DB_PUBLIC MutableEdges : public AsIfFlatEdges {
public:
  MutableEdges();
  ...
  void insert(const db::EdgeWithProperties &edge) {
    do_insert(edge, edge.properties_id());
  }
  ...
  template <class Iter> void insert(const Iter &b, const Iter &e) {
    reserve(count() + (e - b));
    for (Iter i = b; i != e; ++i) {
      insert(*i);
    }
  ...
  template <class Iter> void insert_seq(const Iter &seq) {
