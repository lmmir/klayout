 ...
 */
template <class C> class DB_PUBLIC_TEMPLATE cell_list_iterator {
public:
  typedef C cell_type;
   ...
   */
  cell_list_iterator() : mp_cell(0) {}

  /**
   *  @brief Constructor pointing to a certain element
   */
  cell_list_iterator(cell_type *cell) : mp_cell(cell) {}
   ...
   *  @brief Access
   */
  cell_type &operator*() const { return *mp_cell; }
...
public:
   ...
   *  @brief Default constructor
   */
  cell_list_const_iterator() : mp_cell(0) {}

  /**
   *  @brief Constructor pointing to a certain element
   */
  cell_list_const_iterator(const cell_type *cell) : mp_cell(cell) {}
   ...
   *  @brief Access
   */
  const cell_type &operator*() const { return *mp_cell; }
...
public:
   ...
   */
  cell_list() : mp_first(0), mp_last(0) {
    //  .. nothing yet ..
  }
   ...
   */
  bool empty() const { return mp_last == 0 && mp_first == 0; }
   ...
   */
  void erase(iterator iter) { delete take(iter); }
...
/**
 *  @brief An interface that is used to map layer between libraries and PCells
 * and the layout
 */
...
public:
  ...
  /**
   *  @brief Perform the mapping, i.e. deliver a layer index for a given
   * LayerProperties information
   *
   *  This method can return false in the first member of the returned pair to
   * indicate that no mapping shall be performed. Otherwise it must return the
   * layer index in the second member.
   */
  virtual std::pair<bool, unsigned int>
  map_layer(const LayerProperties &lprops) = 0;
};

/**
 *  @brief A binary object representing context information for regenerating
 * library proxies and PCells
 */
struct DB_PUBLIC LayoutOrCellContextInfo {
  ...
  deserialize(std::vector<std::string>::const_iterator from,
...
class DB_PUBLIC Layout : public db::Object,
...
public:
  ...
  typedef std::map<std::pair<lib_id_type, cell_index_type>, cell_index_type>
      lib_proxy_map;
   ...
   *  The default is OFF.
   */
  void do_cleanup(bool f) { m_do_cleanup = f; }
   ...
   */
  const std::string &technology_name() const { return m_tech_name; }

  /**
   *  @brief Gets the library the layout lives in or NULL if the layout is not
   * part of a library
   */
  Library *library() const { return mp_library; }

  /**
   *  @brief Sets the library pointer
   */
  void set_library(db::Library *library) { mp_library = library; }

  /**
   *  @brief Gets the technology object the layout is associated with or null if
   * no valid technology is associated
   */
  const db::Technology *technology() const;

  /**
   *  @brief Changes the technology, the layout is associated with
   *  Changing the layout may re-assess all the library references as libraries
   * can be technology specific
   */
  void set_technology_name(const std::string &tech);

  /**
   *  @brief Changes the technology name
   *  This method will only change the technology name, but does not re-assess
   * the library links. It's provided mainly to support undo/redo and testing.
   */
   ...
   */
  ArrayRepository &array_repository() { return m_array_repository; }

  /**
   *  @brief Accessor to the string repository
   */
  StringRepository &string_repository() { return m_string_repository; }
   ...
   */
  GenericRepository &shape_repository() { return m_shape_repository; }
   ...
   *  This is a generic lock that can be used to lock modifications against
   * multiple threads.
   */
  tl::Mutex &lock() { return m_lock; }
   ...
   */
  db::properties_id_type prop_id() const { return m_prop_id; }
   ...
   *  created before and has gone "invalid" in between. The cell object's
   ...
   *
   *  This method is basically supposed to be used for "undo" and "redo".
   *  It is not intended for general use. It takes out the cell object and
   * released ownership within the layout object.
   *
   *  @param id The name of the new cell
   ...
   *
   *  The cell is created, but cannot be found by name. The name returned is an
   * empty string. The cell is created with the purpose of being renamed later.
   *
   *  @return The index of the new cell
   */
   ...
   *
   *  @param from A begin type iterator that delivers the ids of the cells to
   * delete
   *  @param to A end type iterator that delivers the ids of the cells to delete
   */
  template <class Iter> void delete_cells(Iter from, Iter to) {
   ...
   *  @return The index of the new cell or cell_index if the cell is not a PCell
   * or library proxy
   */
   ...
   *  @return The index of the new cell
   */
  cell_index_type get_pcell_variant(pcell_id_type pcell_id,
                                    const std::vector<tl::Variant> &parameters);

  /**
   *  @brief Gets a PCell variant
   *
   *  Unlike the first version, this one allows specification of the parameters
   * through a key/value dictionary. Parameters not listed there are replaced by
   * their defaults.
   *
   *  @param pcell_id The Id of the PCell declaration
   ...
   */
  cell_index_type
  get_pcell_variant_dict(pcell_id_type pcell_id,
   ...
   *  @param pcell_id The Id of the PCell declaration
   *  @param parameters The PCell parameters
   *  @param cell_index The cell index which is to be replaced by the PCell
   * variant proxy
   *  @param layer_mapping The optional layer mapping object that maps the PCell
   * layers to the layout's layers
   *  @param retain_layout Set to true for not using update() on the PCell but
   * to retain existing layout (conservative approach)
   */
  void get_pcell_variant_as(pcell_id_type pcell_id,
                            const std::vector<tl::Variant> &parameters,
                            cell_index_type cell_index,
                            ImportLayerMapping *layer_mapping = 0,
                            bool retain_layout = false);
   ...
   *
   *  @param cell_index The current PCell variant proxy cell (must be a
   * PCellVariant)
   *  @param new_parameters The new parameters
   *  @return The cell index of the new PCell variant proxy cell.
   */
  cell_index_type
  get_pcell_variant_cell(cell_index_type cell_index,
   ...
   *  @return A pair of an id and a boolean which is true, if the PCell with
   * that name exists.
   */
   ...
   *
   *  If a PCell with that name is already registered, the definition will be
   * overwritten, but no update of any cell content is performed. The layout
   * object becomes owner of the declaration object and will delete it when it
   * is destroyed.
   */
  pcell_id_type register_pcell(const std::string &name,
                               pcell_declaration_type *declaration);

  /**
   *  @brief The PCell iterator (begin)
   *
   *  This iterator delivers the PCell names and ID's. iterator->first is the
   * PCell name and iterator->second the PCell ID.
   */
  pcell_iterator begin_pcells() const { return m_pcell_ids.begin(); }

  /**
   *  @brief The PCell iterator (end)
   */
  pcell_iterator end_pcells() const { return m_pcell_ids.end(); }

  /**
   *  @brief Test, if the given cell is a PCell variant
   *
   *  The return value is a pair of a boolean which is true when the instance is
   * a PCell instance and the PCell id of the instance if this is the case.
   *
   *  @param ref The cell to test
   *  @return See above
   */
  std::pair<bool, db::pcell_id_type>
  is_pcell_instance(cell_index_type cell_index) const;

  /**
   *  @brief Returns the library where the cell is finally defined
   *
   *  In the first part of the returned pair, this method returns the pointer to
   * the library where the cell is defined. If the cell is not a library proxy,
   * the return value is 0. If the cell is defined in a library, the second part
   * of the returned pair will contain the cell index inside this library.
   */
  std::pair<db::Library *, db::cell_index_type>
  defining_library(cell_index_type cell_index) const;

  /**
   *  @brief Gets the PCell declaration object for a PCell instance
   *
   *  This method determines the PCell declaration object for a given PCell
   * variant cell. Note, that the declaration may originate from a different
   * layout than this, if the PCell is imported from a library.
   *
   *  The cell given cell is not a PCell variant cell, 0 is returned.
   */
  const Layout::pcell_declaration_type *
  pcell_declaration_for_pcell_variant(cell_index_type cell_index) const;

  /**
   *  @brief Get the PCell parameters of a PCell instance
   *
   *  For the order of the parameters, ask the PCell declaration (available
   * trough Layout::pcell_declaration from the PCell id or from
   * Layout::pcell_declaration_for_pcell_variant from the cell_index).
   *
   *  @return A list of parameters in the order they are declared.
   */
  const std::vector<tl::Variant> &
  get_pcell_parameters(cell_index_type cell_index) const;

  /**
   *  @brief Gets a PCell parameter of a PCell instance
   *
   *  @return The value of the PCell parameter with the given name or a nil
   * variant if there is no such parameter
   */
  tl::Variant get_pcell_parameter(cell_index_type cell_index,
  ...
  get_named_pcell_parameters(cell_index_type cell_index) const;

  /**
   *  @brief Get the proxy cell (index) for a given library an cell index
   * (inside that library)
   *
   ...
   * to retain existing layout (conservative approach)
   *
   *  This method replaces the cell with the given target cell index by a
   * library.
   */
  void get_lib_proxy_as(Library *lib, cell_index_type cell_index,
  ...
  /**
   *  @brief Subsitutes the given cell by a cold proxy representing the given
   * context information
   */
  void create_cold_proxy_as(const db::LayoutOrCellContextInfo &info,
                            cell_index_type cell_index);

  /**
   *  @brief Gets a value indicating whether layout context info is provided /
   * needed
   */
  ...
  /**
   *  @brief Get the context information for the layout (for writing into a
   * file)
   *
   *  The context information is a sequence of strings which is pushed onto the
   * given vector. It can be used to fill meta information with
   * fill_meta_info_from_context.
   */
   ...
   */
  void
  fill_meta_info_from_context(std::vector<std::string>::const_iterator from,
                              std::vector<std::string>::const_iterator to);
  ...
  /**
   *  @brief Get the context information for a given cell (for writing into a
   * file)
   *
   *  The context information is a sequence of strings which is pushed onto the
   * given vector. It can be used to recover a respective proxy cell with the
   * recover_proxy method or to fill meta information using
   * fill_meta_info_from_context.
   */
  bool get_context_info(cell_index_type cell_index,
                        std::vector<std::string> &context_info) const;

  /**
   *  @brief Gets the context information as a binary object
   */
  bool get_context_info(cell_index_type cell_index,
                        LayoutOrCellContextInfo &context_info) const;

  /**
   *  @brief Fills the layout's meta information from the context
   */
  void
  fill_meta_info_from_context(cell_index_type cell_index,
   ...
   *  @brief Fills the layout's meta information from the binary context
   */
  void fill_meta_info_from_context(cell_index_type cell_index,
                                   const LayoutOrCellContextInfo &context_info);

  /**
   *  @brief Recover a proxy cell from the given context info.
   *
   *  Creates a proxy cell from the context information given by two iterators
   * into a string list. If no cell can be created from the context information,
   * null is returned.
   *
   *  @param from The begin iterator for the strings from which to recover the
   * cell
   *  @param to The end iterator for the strings from which to recover the cell
   */
  db::Cell *recover_proxy(std::vector<std::string>::const_iterator from,
                          std::vector<std::string>::const_iterator to);
   ...
   *  Creates a proxy cell from the context information given by two iterators
   * into a string list. If no cell can be created from the context information,
   * null is returned. This creates the proxy as a cell with the given cell
   * index.
   *
   ...
   */
  bool recover_proxy_as(cell_index_type cell_index,
                        std::vector<std::string>::const_iterator from,
                        std::vector<std::string>::const_iterator to,
                        ImportLayerMapping *layer_mapping = 0);
   ...
   *
   *  This feature can be used after a library update to make sure that proxies
   * are updated. Library updates may enabled lost connections which are help in
   * cold proxies. This method will recover these connections.
   */
   ...
   */
  void replace_cell(cell_index_type target_cell_index, db::Cell *new_cell,
                    bool retain_layout);

  /**
   *  @brief Replaces all instances of src_cell_index by target_cell_index
   */
  void replace_instances_of(cell_index_type src_cell_index,
                            cell_index_type target_cell_index);
   ...
   *
   *  @param id The index of the cell to delete
   *  @param levels The number of hierarchy levels to look for (-1: all, 0:
   * none, 1: one level etc.)
   */
   ...
   * none, 1: one level etc.)
   */
  template <class Iter> void prune_cells(Iter from, Iter to, int levels = -1) {
    std::set<cell_index_type> cells_to_delete;
    cells_to_delete.insert(from, to);
   ...
   *
   *  All subcells of the given cell which are referenced directly or indirectly
   * but not used otherwise are deleted.
   *
   ...
   *  All subcells referenced directly or indirectly but not used otherwise
   *  are deleted as well.
   *  This method is more efficient than calling prune_subcells for single cells
   * multiple times.
   *
   *  @param from A begin iterator delivering the cell id's to delete
   *  @param to An end iterator delivering the cell id's to delete
   ...
   *  @param levels The number of levels to flatten or -1 for all levels (0:
   * just copy and transform if source cell != target cell, 1: flatten on level
   * of instances etc.)
   */
  void flatten(const db::Cell &source_cell, db::Cell &target_cell,
   ...
   *  @param levels The number of levels to flatten or -1 for all levels (0:
   * nothing, 1: flatten on level of instances etc.)
   *  @param prune If true, remove all cells which became orphans by the
   * flattening
   */
  void flatten(db::Cell &cell, int levels, bool prune = false);

  /**
   *  @brief Inserts a region (potentially hierarchical) into the given cell and
   * layer
   *
  ...
  /**
   *  @brief Inserts a edge collection (potentially hierarchical) into the given
   * cell and layer
   *
   *  If the edge collection is flat (conceptionally), it will be put into the
   * cell. If the edge collection is hierarchical, a cell hierarchy will be
   * built below the given cell.
   */
  void insert(db::cell_index_type cell, int layer, const db::Edges &edges);

  /**
   *  @brief Inserts a edge pair collection (potentially hierarchical) into the
   * given cell and layer
   *
   *  If the edge pair collection is flat (conceptionally), it will be put into
   * the cell. If the edge pair collection is hierarchical, a cell hierarchy
   * will be built below the given cell.
   */
  void insert(db::cell_index_type cell, int layer,
              const db::EdgePairs &edge_pairs);

  /**
   *  @brief Inserts a text collection (potentially hierarchical) into the given
   * cell and layer
   *
   *  If the text collection is flat (conceptionally), it will be put into the
   * cell. If the text collection is hierarchical, a cell hierarchy will be
   * built below the given cell.
   */
   ...
   */
  template <class T> void transform(const T &t) {
   ...
   */
  cell_index_type cells() const { return (cell_index_type)m_cell_ptrs.size(); }
   ...
   */
  cell_type &cell(cell_index_type i) { return *(m_cell_ptrs[i]); }
   ...
   *  @return A reference to the cell
   */
  const cell_type &cell(cell_index_type i) const { return *(m_cell_ptrs[i]); }

  /**
   *  @brief Reserve space for the given number of cells
   */
  void reserve(size_t n) { m_cell_ptrs.reserve(n); }
   ...
   *  @brief Move a layer
   *
   *  Move a layer from the source to the target. The target is not cleared
   * before, so that this method merges shapes from the source with the target
   * layer. The source layer is empty after that operation.
   */
   ...
   *  @brief Copy a layer
   *
   *  Copy a layer from the source to the target. The target is not cleared
   * before, so that this method merges shapes from the source with the target
   * layer.
   */
   ...
   *
   *  Clears the layer: removes the shapes of the type given the flags
   * (ShapeIterator::shapes_type)
   */
  ...
  /**
   *  @brief Copies the shapes of certain cells from the given source layout
   * into this layout
   *
   *  The affected cells are derived from the cell mapping object.
   */
  void copy_tree_shapes(const db::Layout &source_layout,
                        const db::CellMapping &cm);

  /**
   *  @brief Copies the shapes of certain cells from the given source layout
   * into this layout using the given layer mapping
   *
   *  The affected cells are derived from the cell mapping object.
   */
  void copy_tree_shapes(const db::Layout &source_layout,
                        const db::CellMapping &cm, const db::LayerMapping &lm);

  /**
   *  @brief Moves the shapes of certain cells from the given source layout into
   * this layout
   *
   *  The affected cells are derived from the cell mapping object.
   */
  ...
  /**
   *  @brief Moves the shapes of certain cells from the given source layout into
   * this layout using the given layer mapping
   *
   *  The affected cells are derived from the cell mapping object.
   */
  void move_tree_shapes(db::Layout &source_layout, const db::CellMapping &cm,
  ...
  /**
   *  @brief Returns true, if a layer index is a valid index for a normal layout
   * layer
   */
   ...
   *  allocated.
   */
  unsigned int layers() const { return m_layers.layers(); }

  /**
   *  @brief The iterator of valid layers: begin
   */
  layer_iterator begin_layers() const { return m_layers.begin_layers(); }

  /**
   *  @brief The iterator of valid layers: end
   */
  layer_iterator end_layers() const { return m_layers.end_layers(); }

  /**
   *  @brief Reserve space for n layers
   */
  void reserve_layers(unsigned int n) { m_layers.reserve_layers(n); }
   ...
   */
  void cleanup(const std::set<db::cell_index_type> &keep =
   ...
   */
  void insert_layer(unsigned int index,
                    const LayerProperties &props = LayerProperties());

  /**
   *  @brief Gets or creates a layer with the given properties
   *
   *  If there already is a layer matching the given properties, it's index will
   * be returned. Otherwise a new layer with these properties is created.
   */
  unsigned int get_layer(const db::LayerProperties &props);

  /**
   *  @brief Gets the layer with the given properties or -1 if such a layer does
   * not exist
   */
   ...
   */
  unsigned int
  insert_special_layer(const LayerProperties &props = LayerProperties());
   ...
   *  A special layers is used for example to represent rulers.
   */
  void insert_special_layer(unsigned int index,
                            const LayerProperties &props = LayerProperties());
   ...
   *
   *  The waste layer is used to store shapes that should not be visible and can
   * be cleared at any time.
   */
  unsigned int waste_layer() const { return m_layers.waste_layer(); }
   ...
   */
  unsigned int error_layer() const { return m_layers.error_layer(); }
  ...
  /**
   *  @brief Signal the start of an operation bringing the layout into invalid
   * state
   *
   ...
   */
  bool under_construction() const { return m_invalid > 0; }
   ...
   */
  meta_info_iterator begin_meta() const { return m_meta_info.begin(); }

  /**
   *  @brief Delivers the meta information (end iterator)
   */
  meta_info_iterator end_meta() const { return m_meta_info.end(); }
  ...
  /**
   *  @brief Gets the meta information name ID for a specific string (const
   * version)
   */
   ...
   */
  template <class I> void add_meta_info(const I &b, const I &e) {
    for (I i = b; i != e; ++i) {
  ...
  /**
   *  @brief Gets a value indicating whether a meta info with the given name is
   * present
   */
   ...
   *  The given meta information object is to the meta information list for the
   * given cell. If a meta info object with the same name already exists it is
   * overwritten.
   */
  void add_meta_info(db::cell_index_type ci, const std::string &name,
                     const MetaInfo &i) {
   ...
   *  The given meta information object is appended at the end of the meta
   * information list. If a meta info object with the same name already exists
   * it is overwritten.
   */
  void add_meta_info(db::cell_index_type ci, meta_info_name_id_type name_id,
                     const MetaInfo &i);
   ...
   * present for the given cell
   */
  bool has_meta_info(db::cell_index_type ci,
                     meta_info_name_id_type name_id) const;

  /**
   *  @brief Removes the meta information object with the given name from the
   * given cell The method will do nothing if no object with that name exists.
   */
  ...
  /**
   *  @brief Removes the meta information object with the given name ID from the
   * given cell The method will do nothing if no object with that name exists.
   */
  ...
  /**
   *  @brief Gets the meta info value for a meta info object with the given name
   * for the given cell If no object with that name exists, an empty string is
   * returned
   */
  const MetaInfo &meta_info(db::cell_index_type ci,
                            const std::string &name) const {
  ...
  /**
   *  @brief Gets the meta info value for a meta info object with the given name
   * ID for the given cell If no object with that name exists, an empty string
   * is returned
   */
  const MetaInfo &meta_info(db::cell_index_type ci,
                            meta_info_name_id_type name_id) const;
...
private:
   ...
   */
  void do_prune_cells_or_subcells(const std::set<cell_index_type> &ids,
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Layout &x, bool no_self, void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}

/**
 *  @brief A nice helper class that employs RAII for locking the layout against
 * updates
 *
 *  If a layout shall be locked against internal updates temporarily, use this
 * locker:
 *  @code
 *  Layout *ly = ...;
 *  {
