...
NetlistDeviceExtractorError::NetlistDeviceExtractorError(
    const std::string &cell_name, const std::string &msg)
...
NetlistDeviceExtractor::NetlistDeviceExtractor(const std::string &name)
    : mp_layout(0), m_cell_index(0), mp_breakout_cells(0),
      m_device_scaling(1.0), mp_circuit(0) {
  m_name = name;
...
static void insert_into_region(const db::NetShape &s, const db::ICplxTrans &tr,
                               db::Region &region) {
...
void NetlistDeviceExtractor::extract(
    db::DeepShapeStore &dss, unsigned int layout_index,
    const NetlistDeviceExtractor::input_layers &layer_map, db::Netlist &nl,
    hier_clusters_type &clusters, double device_scaling) {
  initialize(&nl);

  std::vector<unsigned int> layers;
  layers.reserve(m_layer_definitions.size());

  for (layer_definitions::const_iterator ld = begin_layer_definitions();
       ld != end_layer_definitions(); ++ld) {

    size_t ld_index = ld->index;
    input_layers::const_iterator l =
        layer_map.find(m_layer_definitions[ld_index].name);
    while (l == layer_map.end() &&
           m_layer_definitions[ld_index].fallback_index <
               m_layer_definitions.size()) {
      //  try fallback layer
    ...
    if (l == layer_map.end()) {
      ...
      throw tl::Exception(tl::sprintf(
          tl::to_string(
              tr("Missing input layer for device extraction (device %s): %s")),
          name(), layer_names));
    }
    ...
    if (dr == 0) {
      ...
      std::pair<bool, db::DeepLayer> alias =
          dss.layer_for_flat(tl::id_of(l->second->get_delegate()));
      if (alias.first) {
        ...
        throw tl::Exception(tl::sprintf(
            tl::to_string(
                tr("Invalid region passed to input layer '%s' for device "
                   "extraction (device %s): must be of deep region kind")),
            ld->name, name()));
      }

    } else {

      if (&dr->deep_layer().layout() != &dss.layout(layout_index) ||
          &dr->deep_layer().initial_cell() != &dss.initial_cell(layout_index)) {
        throw tl::Exception(tl::sprintf(
            tl::to_string(tr("Invalid region passed to input layer '%s' for "
                             "device extraction (device %s): not originating "
  ...
  extract_without_initialize(dss.layout(layout_index),
                             dss.initial_cell(layout_index), clusters, layers,
                             device_scaling, dss.breakout_cells(layout_index));
}

void NetlistDeviceExtractor::extract(
    db::Layout &layout, db::Cell &cell, const std::vector<unsigned int> &layers,
    db::Netlist *nl, hier_clusters_type &clusters, double device_scaling,
...
void NetlistDeviceExtractor::extract_without_initialize(
    db::Layout &layout, db::Cell &cell, hier_clusters_type &clusters,
    const std::vector<unsigned int> &layers, double device_scaling,
    const std::set<db::cell_index_type> *breakout_cells) {
  ...
  //  terminal properties are kept in a property with the terminal_property_name
  //  name
  m_terminal_id_propname_id = mp_layout->properties_repository().prop_name_id(
      terminal_id_property_name());
  m_device_id_propname_id = mp_layout->properties_repository().prop_name_id(
      device_id_property_name());
  m_device_class_propname_id = mp_layout->properties_repository().prop_name_id(
      device_class_property_name());

  tl_assert(m_netlist.get() != 0);

  //  build a cell-id-to-circuit lookup table
  std::map<db::cell_index_type, db::Circuit *> circuits_by_cell;
  for (db::Netlist::circuit_iterator c = m_netlist->begin_circuits();
       c != m_netlist->end_circuits(); ++c) {
  ...
  for (std::set<db::cell_index_type>::const_iterator ci =
           all_called_cells.begin();
       ci != all_called_cells.end(); ++ci) {
  ...
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Extracting devices")));

  //  count effort and make a progress reporter

  size_t n = 0;
  for (std::set<db::cell_index_type>::const_iterator ci = called_cells.begin();
       ci != called_cells.end(); ++ci) {
    db::connected_clusters<shape_type> cc =
        device_clusters.clusters_per_cell(*ci);
    for (db::connected_clusters<shape_type>::all_iterator c = cc.begin_all();
         !c.at_end(); ++c) {
      if (cc.is_root(*c)) {
        ++n;
      }
  ...
  typedef std::map<std::vector<db::Region>, ExtractorCacheValueType>
      extractor_cache_type;
  ...
  for (std::set<db::cell_index_type>::const_iterator ci = called_cells.begin();
    ...
    std::map<db::cell_index_type, db::Circuit *>::const_iterator c2c =
        circuits_by_cell.find(*ci);
    ...
    for (db::connected_clusters<shape_type>::all_iterator c = cc.begin_all();
      ...
      //  take only root clusters - others have upward connections and are not
      //  "whole"
      if (!cc.is_root(*c)) {
        continue;
      }
      ...
      for (std::vector<unsigned int>::const_iterator l = layers.begin();
        ...
        for (db::recursive_cluster_shape_iterator<shape_type> si(
                 device_clusters, *l, *ci, *c);
             !si.at_end(); ++si) {
      ...
      db::Box box;
      for (std::vector<db::Region>::const_iterator g = layer_geometry.begin();
           g != layer_geometry.end(); ++g) {
        box += g->bbox();
      }

      db::Vector disp = box.p1() - db::Point();
      for (std::vector<db::Region>::iterator g = layer_geometry.begin();
           g != layer_geometry.end(); ++g) {
        g->transform(db::Disp(-disp));
      }

      extractor_cache_type::const_iterator ec =
          extractor_cache.find(layer_geometry);
      if (ec == extractor_cache.end()) {
        ...
        for (std::map<size_t,
                      std::pair<db::Device *, geometry_per_terminal_type>>::
                 const_iterator d = m_new_devices.begin();
             d != m_new_devices.end(); ++d) {
...
void NetlistDeviceExtractor::push_new_devices(const db::Vector &disp_cache) {
  ...
  for (std::map<size_t, std::pair<db::Device *, geometry_per_terminal_type>>::
           const_iterator d = m_new_devices.begin();
       d != m_new_devices.end(); ++d) {
    ...
    device->set_trans(
        db::DCplxTrans(device->trans().disp() + dbu * disp_cache));

    DeviceCellKey key;

    for (geometry_per_terminal_type::const_iterator t =
             d->second.second.begin();
         t != d->second.second.end(); ++t) {
      std::map<unsigned int, std::set<db::NetShape>> &gt =
          key.geometry[t->first];
      for (geometry_per_layer_type::const_iterator l = t->second.begin();
           l != t->second.end(); ++l) {
        std::set<db::NetShape> &gl = gt[l->first];
        for (std::vector<db::NetShape>::const_iterator p = l->second.begin();
             p != l->second.end(); ++p) {
          db::NetShape pr = *p;
    ...
    const std::vector<db::DeviceParameterDefinition> &pd =
        mp_device_class->parameter_definitions();
    for (std::vector<db::DeviceParameterDefinition>::const_iterator p =
             pd.begin();
         p != pd.end(); ++p) {
      key.parameters.insert(
          std::make_pair(p->id(), device->parameter_value(p->id())));
    }

    db::PropertiesRepository::properties_set ps;

    std::map<DeviceCellKey,
             std::pair<db::cell_index_type, db::DeviceAbstract *>>::iterator c =
        m_device_cells.find(key);
    if (c == m_device_cells.end()) {

      std::string cell_name = "D$" + mp_device_class->name();
      db::Cell &device_cell =
          mp_layout->cell(mp_layout->add_cell(cell_name.c_str()));

      db::DeviceAbstract *dm = new db::DeviceAbstract(
          mp_device_class.get(),
          mp_layout->cell_name(device_cell.cell_index()));
      m_netlist->add_device_abstract(dm);
      dm->set_cell_index(device_cell.cell_index());

      c = m_device_cells
              .insert(std::make_pair(
                  key, std::make_pair(device_cell.cell_index(), dm)))
              .first;
      ...
      for (geometry_per_terminal_type::const_iterator t =
        ...
        ps.clear();
        ps.insert(
            std::make_pair(m_terminal_id_propname_id, tl::Variant(t->first)));
        db::properties_id_type pi =
            mp_layout->properties_repository().properties_id(ps);
        ...
        for (geometry_per_layer_type::const_iterator l = t->second.begin();
          ...
          for (std::vector<db::NetShape>::const_iterator s = l->second.begin();
               s != l->second.end(); ++s) {
            db::NetShape pr = *s;
            pr.transform(db::NetShape::trans_type(-disp));
    ...
    db::CellInstArrayWithProperties inst(
        db::CellInstArray(db::CellInst(c->second.first),
                          db::Trans(disp_cache + disp)),
        pi);
    mp_layout->cell(m_cell_index).insert(inst);
  }
}

void NetlistDeviceExtractor::push_cached_devices(
    const tl::vector<db::Device *> &cached_devices,
    const db::Vector &disp_cache, const db::Vector &new_disp) {
  db::CplxTrans dbu = db::CplxTrans(mp_layout->dbu());
  db::VCplxTrans dbu_inv = dbu.inverted();
  db::PropertiesRepository::properties_set ps;

  for (std::vector<db::Device *>::const_iterator d = cached_devices.begin();
       d != cached_devices.end(); ++d) {
    ...
    device->set_trans(
        db::DCplxTrans(device->trans().disp() + dbu * (new_disp - disp_cache)));

    //  Build a property set for the device ID
    ps.clear();
    ps.insert(
        std::make_pair(m_device_id_propname_id, tl::Variant(device->id())));
    db::properties_id_type pi =
        mp_layout->properties_repository().properties_id(ps);

    db::CellInstArrayWithProperties inst(
        db::CellInstArray(db::CellInst(device->device_abstract()->cell_index()),
                          db::Trans(new_disp + disp)),
        pi);
    mp_layout->cell(m_cell_index).insert(inst);
  }
}
...
db::Connectivity NetlistDeviceExtractor::get_connectivity(
...
void NetlistDeviceExtractor::extract_devices(
...
void NetlistDeviceExtractor::register_device_class(DeviceClass *device_class) {
  ...
  if (m_name.empty()) {
    throw tl::Exception(
        tl::to_string(tr("No device extractor/device class name set")));
  }
  ...
  if (existing) {
    ...
    if (typeid(*existing) != typeid(*device_class)) {
      throw tl::Exception(tl::to_string(
          tr("Different device class already registered with the same name")));
    }
...
NetlistDeviceExtractor::define_layer(const std::string &name,
                                     const std::string &description) {
  m_layer_definitions.push_back(db::NetlistDeviceExtractorLayerDefinition(
      name, description, m_layer_definitions.size(),
      std::numeric_limits<size_t>::max()));
  return m_layer_definitions.back();
}

const db::NetlistDeviceExtractorLayerDefinition &
NetlistDeviceExtractor::define_layer(const std::string &name, size_t fallback,
                                     const std::string &description) {
  m_layer_definitions.push_back(db::NetlistDeviceExtractorLayerDefinition(
      name, description, m_layer_definitions.size(), fallback));
  return m_layer_definitions.back();
}
...
void NetlistDeviceExtractor::define_terminal(Device *device, size_t terminal_id,
  ...
  std::pair<db::Device *, geometry_per_terminal_type> &dd =
      m_new_devices[device->id()];
  dd.first = device;
...
void NetlistDeviceExtractor::define_terminal(Device *device, size_t terminal_id,
  ...
  db::Vector dv(1, 1);
  define_terminal(device, terminal_id, layer_index,
                  db::Polygon(db::Box(point - dv, point + dv)));
}
...
void NetlistDeviceExtractor::error(const std::string &category_name,
                                   const std::string &category_description,
