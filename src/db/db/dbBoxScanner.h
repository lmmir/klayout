...
struct bs_side_compare_func
    ...
    : std::binary_function<std::pair<const Obj *, Prop>,
                           std::pair<const Obj *, Prop>, bool>
#endif
{
  typedef typename BoxConvert::box_type box_type;

  bs_side_compare_func(const BoxConvert &bc) : m_bc(bc) {
    //  .. nothing yet ..
  }

  bool operator()(const std::pair<const Obj *, Prop> &a,
                  const std::pair<const Obj *, Prop> &b) const {
    SideOp sideop;
...
/**
 *  @brief A predicate the checks two boxes for overlap while applying an
 * enlargement to right and top
 */
template <class Box>
bool bs_boxes_overlap(const Box &b1, const Box &b2,
                      typename Box::coord_type enl) {
  if (b1.empty() || b2.empty()) {
    return false;
  } else {
    return (b1.p1().x() < b2.p2().x() + enl &&
            b2.p1().x() < b1.p2().x() + enl) &&
 ...
 *
 *  This template specifies the methods or provides a default implementation for
 * them for use as the output receiver of the box scanner.
 */
template <class Obj, class Prop> struct box_scanner_receiver {
  virtual ~box_scanner_receiver() {}

  /**
   *  @brief Indicates that the given object is no longer used
   *
   *  The finish method is called when an object is no longer in the queue and
   * can be discarded.
   */
   ...
   *
   *  This method is called when the object o1 interacts with o2 within the
   * current definition.
   */
  virtual void add(const Obj * /*o1*/, Prop /*p1*/, const Obj * /*o2*/,
                   Prop /*p2*/) {}

  /**
   *  @brief Indicates whether the scanner may stop
   *
   *  The scanner will stop if this method returns true. This feature can be
   * used to terminate the scan process early if the outcome is known.
   */
   ...
   *
   *  This method is called after the scan has finished (without exception). The
   * argument is the return value (false if "stop" stopped the process).
   */
 ...
 *
 *  A box scanner receives a series of objects of type Obj which can be
 * converted to boxes and associated properties (of type Prop). It will store
 * pointers to these objects, so they lifetime of these objects must exceed that
 * of the box scanner.
 *
 *  The basic function of the box scanner is to derive interactions. This is
 * done in the process method. After the box scanner has been filled with object
 * pointers, the process method can be called. The process method will derive
 * all interactions and report these to the Rec argument of the process method.
 *
 *  "Rec" is the interaction receiver. It will receive events when an
 * interaction is encountered. See the box_scanner_receiver template for a
 * description of the methods this object must provide.
 *
 *  See the process method for the description of the options of the interaction
 * test.
 */
template <class Obj, class Prop> class box_scanner {
public:
   ...
   *  @brief Default ctor
   */
  box_scanner(bool report_progress = false,
   ...
   *
   *  This value determines for how many elements the implementation switches to
   * the scanner implementation instead of the plain element-by-element
   * interaction test. The default value is 100.
   */
  void set_scanner_threshold(size_t n) { m_scanner_thr = n; }

  /**
   *  @brief Gets the scanner threshold
   */
  size_t scanner_threshold() const { return m_scanner_thr; }

  /**
   *  @brief Sets the fill factor
   *
   *  The fill factor determines how many new entries will be collected for a
   * band. A fill factor of 2 means that the number of elements in the band will
   * be doubled after elements outside of the band have been removed. The
   * default fill factor is 2.
   */
  void set_fill_factor(double ff) { m_fill_factor = ff; }

  /**
   *  @brief Gets the fill factor
   */
  double fill_factor() const { return m_fill_factor; }

  /**
   *  @brief Reserve for n elements
   */
  void reserve(size_t n) { m_pp.reserve(n); }
   ...
   *  scanner does not need it any longer. An additional property can be
   * attached to the object which will be stored along with the object.
   */
   ...
   *
   *  Two objects interact if the boxes of the objects enlarged by the given
   * value overlap. The enlargement is specified in units of width and height,
   * i.e. half of the enlargement is applied to one side before the overlap
   * check.
   *
   *  An enlargement of 1 means that boxes have to touch only in order to get an
   * interaction.
   *
   *  The box scanner will report all interactions to the receiver object. See
   * box_scanner_receiver for details about the methods that this object must
   * provide.
   *
  ...
  bool process(Rec &rec, typename BoxConvert::box_type::coord_type enl,
               const BoxConvert &bc = BoxConvert()) {
    rec.initialize();
...
private:
  ...
  template <class Rec, class BoxConvert>
  bool do_process(Rec &rec, typename BoxConvert::box_type::coord_type enl,
                  const BoxConvert &bc = BoxConvert()) {
    typedef typename BoxConvert::box_type box_type;
    typedef typename box_type::coord_type coord_type;
    typedef bs_side_compare_func<BoxConvert, Obj, Prop, box_bottom<Box>>
        bottom_side_compare_func;
    typedef bs_side_compare_func<BoxConvert, Obj, Prop, box_left<Box>>
        left_side_compare_func;
    typedef bs_side_compare_vs_const_func<BoxConvert, Obj, Prop, box_top<Box>>
        below_func;
    typedef bs_side_compare_vs_const_func<BoxConvert, Obj, Prop, box_right<Box>>
        left_func;

    //  sort out the entries with an empty bbox (we must not put that into sort)

    typename container_type::iterator wi = m_pp.begin();
    for (typename container_type::iterator ri = m_pp.begin(); ri != m_pp.end();
         ++ri) {
    ...
    if (m_pp.size() <= m_scanner_thr) {
      ...
      //  below m_scanner_thr elements use the brute force approach which is
      //  faster in that case
      ...
      if (m_report_progress) {
        if (m_progress_desc.empty()) {
          progress.reset(new tl::RelativeProgress(
              tl::to_string(tr("Processing")), m_pp.size(), 1000));
        } else {
          progress.reset(
              new tl::RelativeProgress(m_progress_desc, m_pp.size(), 1000));
        }
      }
      ...
      while (future != m_pp.end()) {
        ...
        typename std::iterator_traits<iterator_type>::difference_type
            min_band_size = size_t((future - current) * m_fill_factor);
        coord_type yy = y;
        do {
        ...
        while (f != future) {
          ...
          //  add at least the required items per band
          typename std::iterator_traits<iterator_type>::difference_type
              min_box_size = size_t((f - c) * m_fill_factor);
          coord_type xx = x;
          do {
            xx = bc(*f->first).left();
            do {
          ...
          if (m_report_progress) {
            //  Note: there is no better estimation of the progress than
            //  "current" ...
          ...
          for (iterator_type i = f0; i != f; ++i) {
            for (iterator_type j = c; j < i; ++j) {
              if (bs_boxes_overlap(bc(*i->first), bc(*j->first), enl)) {
                if (seen.find(std::make_pair(i->first, j->first)) ==
                        seen.end() &&
                    seen.find(std::make_pair(j->first, i->first)) ==
                        seen.end()) {
 ...
 *
 *  This template specifies the methods or provides a default implementation for
 * them for use as the output receiver of the twofold box scanner.
 */
...
struct box_scanner_receiver2 {
   ...
   *
   *  The finish1 method is called when an object of the first type is no longer
   * in the queue and can be discarded.
   */
   ...
   *
   *  The finish method is called when an object of the second type is no longer
   * in the queue and can be discarded.
   */
   ...
   * current definition.
   */
  void add(const Obj1 * /*o1*/, const Prop1 & /*p1*/, const Obj2 * /*o2*/,
           const Prop2 & /*p2*/) {}
 ...
 *
 *  This implementation provides a box scanner for two different types. Apart
 * from that it is similar to the uniform-type box scanner.
 *
 *  It will not report interactions within the Obj1 or Obj2 group, but only
 *  interactions between Obj1 and Obj2 objects.
 */
template <class Obj1, class Prop1, class Obj2, class Prop2> class box_scanner2 {
public:
   ...
   *  @brief Default ctor
   */
  box_scanner2(bool report_progress = false,
               const std::string &progress_desc = std::string())
   ...
   *
   *  This value determines for how many elements in one class the
   * implementation switches to the scanner implementation instead of the plain
   * element-by-element interaction test. The default value is 10.
   */
  void set_scanner_threshold1(size_t n) { m_scanner_thr1 = n; }

  /**
   *  @brief Gets the scanner threshold per class
   */
  size_t scanner_threshold1() const { return m_scanner_thr1; }
   ...
   */
  void reserve1(size_t n) { m_pp1.reserve(n); }

  /**
   *  @brief Reserve for n elements of Obj2 type
   */
  void reserve2(size_t n) { m_pp2.reserve(n); }
   ...
   * interaction.
   *
   *  The box scanner will report all interactions of type Obj1 and Obj2 objects
   * to the receiver object. See box_scanner_receiver2 for details about the
   * methods that this object must provide.
   *
   *  The box converter 1 must be capable of converting the Obj1 object into a
   * box. It must provide a box_type typedef. The box converter 2 must be able
   * to convert Obj2 to a box. The box type of both box converters must be
   * identical.
   *
   *  The scanner process can be terminated early by making the receiver's
   *  stop() method return true. In this case, this method will return false.
  ...
  bool process(Rec &rec, typename BoxConvert1::box_type::coord_type enl,
               const BoxConvert1 &bc1 = BoxConvert1(),
               const BoxConvert2 &bc2 = BoxConvert2()) {
    rec.initialize();
...
private:
  ...
  template <class Rec, class BoxConvert1, class BoxConvert2>
  bool do_process(Rec &rec, typename BoxConvert1::box_type::coord_type enl,
                  const BoxConvert1 &bc1 = BoxConvert1(),
                  const BoxConvert2 &bc2 = BoxConvert2()) {
    typedef typename BoxConvert1::box_type
        box_type; //  must be same as BoxConvert2::box_type
    typedef typename box_type::coord_type coord_type;
    typedef bs_side_compare_func<BoxConvert1, Obj1, Prop1, box_bottom<Box>>
        bottom_side_compare_func1;
    typedef bs_side_compare_func<BoxConvert1, Obj1, Prop1, box_left<Box>>
        left_side_compare_func1;
    typedef bs_side_compare_vs_const_func<BoxConvert1, Obj1, Prop1,
                                          box_top<Box>>
        below_func1;
    typedef bs_side_compare_vs_const_func<BoxConvert1, Obj1, Prop1,
                                          box_right<Box>>
        left_func1;
    typedef bs_side_compare_func<BoxConvert2, Obj2, Prop2, box_bottom<Box>>
        bottom_side_compare_func2;
    typedef bs_side_compare_func<BoxConvert2, Obj2, Prop2, box_left<Box>>
        left_side_compare_func2;
    typedef bs_side_compare_vs_const_func<BoxConvert2, Obj2, Prop2,
                                          box_top<Box>>
        below_func2;
    typedef bs_side_compare_vs_const_func<BoxConvert2, Obj2, Prop2,
                                          box_right<Box>>
    ...
    for (typename container_type1::iterator ri1 = m_pp1.begin();
         ri1 != m_pp1.end(); ++ri1) {
    ...
    for (typename container_type2::iterator ri2 = m_pp2.begin();
         ri2 != m_pp2.end(); ++ri2) {
    ...
    if (m_pp1.empty() || m_pp2.empty()) {
      ...
      for (iterator_type2 i = m_pp2.begin(); i != m_pp2.end(); ++i) {
        rec.finish2(i->first, i->second);
               ...
               m_pp2.size() <= m_scanner_thr1 ||
               m_pp1.size() <= m_scanner_thr1) {
      ...
      coord_type y = std::min(bc1(*m_pp1.front().first).bottom(),
                              bc2(*m_pp2.front().first).bottom());
      ...
      if (m_report_progress) {
        if (m_progress_desc.empty()) {
          progress.reset(
              new tl::RelativeProgress(tl::to_string(tr("Processing")),
                                       m_pp1.size() + m_pp2.size(), 1000));
        } else {
          progress.reset(new tl::RelativeProgress(
              m_progress_desc, m_pp1.size() + m_pp2.size(), 1000));
        }
      }
      ...
      while (future1 != m_pp1.end() || future2 != m_pp2.end()) {
        ...
        current1 =
            std::partition(current1, future1, below_func1(bc1, y + 1 - enl));
        current2 =
            std::partition(current2, future2, below_func2(bc2, y + 1 - enl));

        while (cc1 != current1) {
          rec.finish1(cc1->first, cc1->second);
          auto s =
              seen1.lower_bound(std::make_pair(cc1->first, (const Obj2 *)0));
          auto s0 = s;
        ...
        while (cc2 != current2) {
          ...
          auto s =
              seen2.lower_bound(std::make_pair(cc2->first, (const Obj1 *)0));
          auto s0 = s;
        ...
        //  add at least the required items per band
        size_t min_band_size = size_t((future1 - current1) * m_fill_factor) +
                               size_t((future2 - current2) * m_fill_factor);
        coord_type yy = y;
        do {
          if (future1 != m_pp1.end() && future2 != m_pp2.end()) {
            yy = std::min(bc1(*future1->first).bottom(),
                          bc2(*future2->first).bottom());
          } else if (future1 != m_pp1.end()) {
            yy = bc1(*future1->first).bottom();
          } else {
            yy = bc2(*future2->first).bottom();
          }
          while (future1 != m_pp1.end() &&
                 bc1(*future1->first).bottom() == yy) {
            ++future1;
          }
          while (future2 != m_pp2.end() &&
                 bc2(*future2->first).bottom() == yy) {
            ++future2;
          }
        } while ((future1 != m_pp1.end() || future2 != m_pp2.end()) &&
                 size_t(future1 - current1) + size_t(future2 - current2) <
                     min_band_size);
        ...
        if (current1 != future1 && current2 != future2) {
          ...
          coord_type x =
              std::min(bc1(*c1->first).left(), bc2(*c2->first).left());
          ...
          while (f1 != future1 || f2 != future2) {
            ...
            //  add at least the required items per band
            size_t min_box_size = size_t((f1 - c1) * m_fill_factor) +
                                  size_t((f2 - c2) * m_fill_factor);
            coord_type xx = x;
            do {
...
public:
  typedef typename std::vector<std::pair<const Obj *, Prop>>::const_iterator
      iterator;
 ...
 */
template <class Obj, class Prop, class Cluster>
class cluster_collector : public box_scanner_receiver<Obj, Prop> {
public:
  typedef Obj object_type;
  typedef Prop property_type;
  ...
  typedef
      typename std::list<std::pair<size_t, Cluster>>::iterator cl_iterator_type;
   ...
   *
   *  It is important to provide a cluster seed (template) which is used to
   * derive new clusters by copying this one.
   */
  ...
  void finish(const Obj *obj, Prop prop) {
    ...
    if (omi != m_om.end()) {
      ...
      //  single-object entry: create a cluster and feed it a single-object
      //  signature
  ...
  void add(const Obj *o1, Prop p1, const Obj *o2, Prop p2) {
    ...
    if (om1 == m_om.end() && om2 == m_om.end()) {
      ...
      for (typename Cluster::iterator o = c2->second.begin();
           o != c2->second.end(); ++o) {
