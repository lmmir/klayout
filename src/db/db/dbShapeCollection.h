...
public:
  ...
  DeepShapeCollectionDelegateBase &
  operator=(const DeepShapeCollectionDelegateBase &other);

  const db::DeepLayer &deep_layer() const { return m_deep_layer; }

  db::DeepLayer &deep_layer() { return m_deep_layer; }

  void apply_property_translator(const db::PropertiesTranslator &pt);

protected:
  virtual void set_deep_layer(const db::DeepLayer &dl) { m_deep_layer = dl; }
 ...
 */
class DB_PUBLIC ShapeCollectionDelegateBase : public tl::UniqueId {
public:
...
/**
 *  @brief A base class for the shape collections such as Region, Edges,
 * EdgePairs etc.
 */
class DB_PUBLIC ShapeCollection : public gsi::ObjectBase {
public:
  ShapeCollection() {}
   ...
   *
   *  This method will translate the property IDs according to the given
   * property translator.
   *
   *  Note that the property translator needs to be built from the
   * PropertiesRepository delivered by "properties_repository".
   */
  void apply_property_translator(const db::PropertiesTranslator &pt);
  ...
  /**
   *  @brief Gets a value indicating whether a properties repository is
   * available
   */
