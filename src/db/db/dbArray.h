...
template <class Coord> struct basic_array_iterator {
 ...
 */
struct ArrayBase {
  ArrayBase() : in_repository(false) {
    //  .. nothing yet ..
  }

  ArrayBase(const ArrayBase &) : in_repository(false) {
    //  .. nothing yet ..
  }

  ArrayBase &operator=(const ArrayBase &) { return *this; }
  ...
  virtual void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                        int cat, bool no_self = false,
                        void *parent = 0) const = 0;
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const ArrayBase &x, bool no_self, void *parent) {
...
template <class Coord> struct basic_array : public ArrayBase {
  typedef Coord coord_type;
  typedef db::box<coord_type> box_type;
  typedef db::point<coord_type> point_type;
  typedef db::vector<coord_type> vector_type;
  ...
  begin_touching(const box_type &b) const = 0;
  ...
  virtual ArrayBase *basic_clone() const { return clone(); }
  ...
  virtual bool is_regular_array(vector_type & /*a*/, vector_type & /*b*/,
                                unsigned long & /*amax*/,
                                unsigned long & /*bmax*/) const {
    return false;
  }
 ...
 */
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const ArrayRepository &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
template <class Coord>
struct regular_array_iterator : public basic_array_iterator<Coord> {
  ...
  regular_array_iterator(const vector_type &a, const vector_type &b,
                         unsigned long amin, unsigned long amax,
                         unsigned long bmin, unsigned long bmax)
      : m_a(a), m_b(b), m_amin(amin), m_amax(amax), m_bmin(bmin), m_bmax(bmax),
        m_ai(amin), m_bi(bmin) {
  ...
  virtual disp_type get() const {
    return disp_type(vector_type(m_ai * m_a.x() + m_bi * m_b.x(),
                                 m_ai * m_a.y() + m_bi * m_b.y()));
  }
  ...
  virtual bool at_end() const { return m_bi >= m_bmax; }

  virtual basic_array_iterator<Coord> *clone() const {
    return new regular_array_iterator<Coord>(*this);
  }

  virtual long index_a() const { return long(m_ai); }

  virtual long index_b() const { return long(m_bi); }
...
template <class Coord> struct regular_array : public basic_array<Coord> {
  ...
  regular_array(const vector_type &a, const vector_type &b, unsigned long amax,
                unsigned long bmax)
  ...
  begin_touching(const box_type &b) const {
    if (b.empty()) {
      ...
      return std::make_pair(
          new regular_array_iterator<Coord>(m_a, m_b, 0, 0, 0, 0), false);
      ...
      std::pair<double, double> ab[4] = {
          ab_coord(b.p1()), ab_coord(point_type(b.left(), b.top())),
          ab_coord(point_type(b.right(), b.bottom())), ab_coord(b.p2())};

      //  NOTE: we need to take some care we don't overiterate in case of
      //  vanishing row or column vectors. Hence eff_amax and eff_bmax which are
      //  1 in this case.
      ...
      return std::make_pair(new regular_array_iterator<Coord>(
                                m_a, m_b, amini, amaxi, bmini, bmaxi),
                            false);
    }
  }
  ...
  begin_regular(long a, long b) const {
    return std::make_pair(new regular_array_iterator<Coord>(
                              m_a, m_b, (unsigned long)std::max(long(0), a),
                              m_amax, (unsigned long)std::max(long(0), b),
                              m_bmax),
                          false);
  }

  virtual std::pair<basic_array_iterator<Coord> *, bool> begin() const {
    return std::make_pair(
        new regular_array_iterator<Coord>(m_a, m_b, 0, m_amax, 0, m_bmax),
        false);
  }
  ...
  virtual box_type bbox(const box_type &obox) const {
    if (obox.empty()) {
      ...
      return box_type(obox.p1() + (b.p1() - point_type()),
                      obox.p2() + (b.p2() - point_type()));
    }
  }

  virtual size_t size() const { return m_amax * m_bmax; }
  ...
  virtual bool equal(const ArrayBase *b) const {
    const regular_array<Coord> *d =
        static_cast<const regular_array<Coord> *>(b);
    return (m_a == d->m_a && m_b == d->m_b && m_amax == d->m_amax &&
            m_bmax == d->m_bmax);
  }
  ...
  virtual bool fuzzy_equal(const ArrayBase *b) const {
    const regular_array<Coord> *d =
        static_cast<const regular_array<Coord> *>(b);
    return (m_a.equal(d->m_a) && m_b.equal(d->m_b) && m_amax == d->m_amax &&
            m_bmax == d->m_bmax);
  }
  ...
  virtual bool less(const ArrayBase *b) const {
    ...
    return m_a < d->m_a ||
           (m_a == d->m_a &&
            (m_b < d->m_b ||
             (m_b == d->m_b && (m_amax < d->m_amax ||
                                (m_amax == d->m_amax && m_bmax < d->m_bmax)))));
  }

  virtual bool fuzzy_less(const ArrayBase *b) const {
    const regular_array<Coord> *d =
        static_cast<const regular_array<Coord> *>(b);
    return m_a.less(d->m_a) ||
           (m_a.equal(d->m_a) &&
            (m_b.less(d->m_b) ||
             (m_b.equal(d->m_b) &&
              (m_amax < d->m_amax ||
  ...
  virtual unsigned int type() const { return 1; }
  ...
  virtual void transform(const complex_trans_type &ct) {
    //  transform with the matrix, do not displace, since a and b are
    //  displacements already
...
protected:
  ...
  std::pair<double, double> ab_coord(const point_type &p) const {
    ...
    double ia =
        (double(p.x()) * double(b.y()) - double(p.y()) * double(b.x())) / m_det;
    double ib =
        (double(a.x()) * double(p.y()) - double(a.y()) * double(p.x())) / m_det;
 ...
 *  further rotation by a residual angle given by acos, which is actually the
 * cosine of this angle.
 */

template <class Coord>
struct regular_complex_array : public regular_array<Coord> {
  typedef typename basic_array<Coord>::point_type point_type;
  typedef typename basic_array<Coord>::vector_type vector_type;
  typedef typename basic_array<Coord>::box_type box_type;
  typedef typename basic_array<Coord>::coord_type coord_type;
  ...
  regular_complex_array(double acos, double mag, const vector_type &a,
                        const vector_type &b, unsigned long amax,
                        unsigned long bmax)
  ...
  virtual void invert(simple_trans_type &t) {
    //  recompute the array parameters such that every per-instance
  ...
  virtual bool equal(const ArrayBase *b) const {
    const regular_complex_array<Coord> *d =
        static_cast<const regular_complex_array<Coord> *>(b);
  ...
  virtual unsigned int type() const { return 2; }
 ...
 *
 *  The iterated case is where each placement is described by a displacement
 * vector
 */

template <class Coord>
struct iterated_array_iterator : public basic_array_iterator<Coord> {
  typedef typename basic_array_iterator<Coord>::coord_type coord_type;
  typedef typename basic_array_iterator<Coord>::point_type point_type;
  typedef typename basic_array_iterator<Coord>::vector_type vector_type;
  typedef typename basic_array_iterator<Coord>::box_type box_type;
  ...
  typedef unstable_box_tree<box_type, vector_type, box_convert_type>
      box_tree_type;
  ...
  iterated_array_iterator(box_tree_const_iterator from,
                          box_tree_const_iterator to)
      : m_normal(true) {
    m_b = from;
    m_e = to;
  }

  iterated_array_iterator(box_tree_touching_iterator from) : m_normal(false) {
    m_t = from;
  }
  ...
  virtual disp_type get() const { return disp_type(m_normal ? *m_b : *m_t); }
  ...
  virtual size_t quad_id() const { return m_t.quad_id(); }

  virtual box_type quad_box() const { return m_t.quad_box(); }

  virtual void skip_quad() { m_t.skip_quad(); }
...
template <class Coord> struct iterated_array : public basic_array<Coord> {
  typedef typename basic_array<Coord>::point_type point_type;
  typedef typename basic_array<Coord>::vector_type vector_type;
  typedef typename basic_array<Coord>::box_type box_type;
  typedef typename basic_array<Coord>::coord_type coord_type;
  ...
  template <class I> iterated_array(I from, I to) { assign(from, to); }
  ...
  template <class I> void assign(I from, I to) {
    m_v.clear();
    m_v.insert(from, to);
  ...
  void sort() { m_v.sort(db::box_convert<vector_type>()); }

  virtual std::pair<basic_array_iterator<Coord> *, bool>
  begin_touching(const box_type &b) const {
    if (b.empty() || !b.touches(m_box)) {
      return std::make_pair(
          new iterated_array_iterator<Coord>(m_v.end(), m_v.end()), false);
    } else {
      box_convert_type bc;
      return std::make_pair(
          new iterated_array_iterator<Coord>(m_v.begin_touching(b, bc)), false);
    }
  }

  virtual std::pair<basic_array_iterator<Coord> *, bool> begin() const {
    return std::make_pair(
        new iterated_array_iterator<Coord>(m_v.begin(), m_v.end()), false);
  }
  ...
  virtual box_type bbox(const box_type &obox) const {
    if (obox.empty()) {
      ...
      return box_type(obox.p1() + (m_box.p1() - point_type()),
                      obox.p2() + (m_box.p2() - point_type()));
    }
  }

  virtual size_t size() const { return m_v.size(); }
  ...
  virtual bool equal(const ArrayBase *b) const {
    const iterated_array<Coord> *d =
        static_cast<const iterated_array<Coord> *>(b);
    if (m_v.size() != d->m_v.size()) {
      return false;
    }
    for (const_iterator p1 = m_v.begin(), p2 = d->m_v.begin(); p1 != m_v.end();
         ++p1, ++p2) {
      if (*p1 != *p2) {
        return false;
      }
  ...
  virtual unsigned int type() const { return 3; }
...
template <class Coord>
struct iterated_complex_array : public iterated_array<Coord> {
  typedef typename basic_array<Coord>::point_type point_type;
  typedef typename basic_array<Coord>::vector_type vector_type;
  typedef typename basic_array<Coord>::box_type box_type;
  typedef typename basic_array<Coord>::coord_type coord_type;
  ...
  virtual void invert(simple_trans_type &t) {
    //  recompute the array parameters such that every per-instance
    //  transformation is inverted. This code is somewhat complex to maintain
    //  the splitting between the simple transformation part and the residual
    //  given by m_acos and m_mag.
    complex_trans_type r = complex_trans_type(t, m_acos, m_mag).inverted();
    m_mag = r.mag();
    m_acos = r.rcos();
    ...
    t = simple_trans_type(r);
    this->m_box = box_type();
    for (typename iterated_array<Coord>::iterator p = this->m_v.begin();
         p != this->m_v.end(); ++p) {
      *p = -r(*p);
  ...
  virtual bool equal(const ArrayBase *b) const {
    const iterated_complex_array<Coord> *d =
        static_cast<const iterated_complex_array<Coord> *>(b);
    if (fabs(m_acos - d->m_acos) > epsilon) {
      return false;
    }
    if (fabs(m_mag - d->m_mag) > epsilon) {
  ...
  virtual unsigned int type() const { return 4; }
...
template <class Coord> struct single_complex_inst : public basic_array<Coord> {
  typedef typename basic_array<Coord>::point_type point_type;
  typedef typename basic_array<Coord>::box_type box_type;
  typedef typename basic_array<Coord>::coord_type coord_type;
  typedef typename basic_array<Coord>::disp_type disp_type;
  ...
  single_complex_inst(double acos, double mag) : m_acos(acos), m_mag(mag) {
    //  .. nothing yet ..
  }

  virtual std::pair<basic_array_iterator<Coord> *, bool>
  begin_touching(const box_type &b) const {
    return std::make_pair((basic_array_iterator<Coord> *)0,
                          !b.contains(point_type(0, 0)));
  }
  ...
  virtual box_type bbox(const box_type &obox) const { return obox; }
  ...
  virtual bool equal(const ArrayBase *b) const {
    ...
    const single_complex_inst<Coord> *d =
        static_cast<const single_complex_inst<Coord> *>(b);
    if (fabs(m_mag - d->m_mag) > epsilon) {
      return false;
    }
    if (fabs(m_acos - d->m_acos) > epsilon) {
  ...
  virtual bool fuzzy_equal(const ArrayBase *b) const { return equal(b); }
  ...
  virtual bool fuzzy_less(const ArrayBase *b) const { return less(b); }
  ...
  virtual unsigned int type() const { return 5; }
...
//  a transformation and the minimum required capabilities to represent a
//  displacement

template <class C, class T> struct compute_result_trans {
  typedef T result_trans;
};

template <class C> struct compute_result_trans<C, unit_trans<C>> {
  typedef disp_trans<C> result_trans;
};

template <class C> struct compute_result_trans<C, fixpoint_trans<C>> {
  typedef disp_trans<C> result_trans;
};
...
template <class Coord, class Trans> struct array_iterator {
  typedef Coord coord_type;
  typedef db::point<coord_type> point_type;
  typedef db::vector<coord_type> vector_type;
  typedef db::complex_trans<coord_type, coord_type> complex_trans_type;
  typedef Trans trans_type;
  typedef typename compute_result_trans<coord_type, trans_type>::result_trans
      result_type;
   ...
   */
  array_iterator() : m_trans(), mp_base(0), m_done(true) {
    // .. nothing yet ..
  }
   ...
   */
  array_iterator(const trans_type &trans,
                 std::pair<db::basic_array_iterator<Coord> *, bool> base)
   ...
   */
  bool at_end() const { return mp_base ? mp_base->at_end() : m_done; }
   ...
   */
  long index_a() const { return mp_base ? mp_base->index_a() : -1; }
   ...
   *  If the iterator is not a regular one, returns -1.
   */
  long index_b() const { return mp_base ? mp_base->index_b() : -1; }

  /**
   *  @brief For iterators supporting quads (iterated arrays), this method will
   * return the quad ID
   */
  size_t quad_id() const { return mp_base ? mp_base->quad_id() : 0; }

  /**
   *  @brief For iterators supporting quads (iterated arrays), this method will
   * return the quad bounding box
   *
   *  Note that this method will only return a valid quad box is the quad_id is
   * non-null.
   *
...
template <class Obj, class Trans> struct array {
   ...
   */
  array(const Obj &obj, const trans_type &trans, const vector_type &a,
        const vector_type &b, unsigned long amax, unsigned long bmax)
      : m_obj(obj), m_trans(trans),
        mp_base(new regular_array<coord_type>(a, b, amax, bmax)) {
    //  .. nothing yet ..
  }
   ...
   *  This is basically a convenience function that creates
   *  an appropriate basic_array object.
   *  It uses the array repository to store the base object rather than
   * providing it's own storage.
   */
  array(const Obj &obj, const trans_type &trans, ArrayRepository &rep,
        const vector_type &a, const vector_type &b, unsigned long amax,
        unsigned long bmax)
      : m_obj(obj), m_trans(trans),
        mp_base(rep.insert(regular_array<coord_type>(a, b, amax, bmax))) {
    //  .. nothing yet ..
  }
  ...
  array(const Obj &obj, const trans_type &trans, double acos, double mag,
        ...
        unsigned long bmax)
      : m_obj(obj), m_trans(trans),
        mp_base(new regular_complex_array<coord_type>(acos, mag, a, b, amax,
                                                      bmax)) {
    //  .. nothing yet ..
  }
   ...
   */
  array(const Obj &obj, const complex_trans_type &ct, const vector_type &a,
        const vector_type &b, unsigned long amax, unsigned long bmax)
      : m_obj(obj), m_trans(ct),
        mp_base(ct.is_complex()
                    ? new regular_complex_array<coord_type>(ct.rcos(), ct.mag(),
                                                            a, b, amax, bmax)
                    : new regular_array<coord_type>(a, b, amax, bmax)) {
    //  .. nothing yet ..
  }
   ...
   */
  array(const Obj &obj, const trans_type &trans, ArrayRepository &rep,
        double acos, double mag, const vector_type &a, const vector_type &b,
        unsigned long amax, unsigned long bmax)
      : m_obj(obj), m_trans(trans),
        mp_base(rep.insert(
            regular_complex_array<coord_type>(acos, mag, a, b, amax, bmax))) {
    //  .. nothing yet ..
  }
  ...
  array(const Obj &obj, const complex_trans_type &ct, ArrayRepository &rep,
      ...
      : m_obj(obj), m_trans(ct),
        mp_base(ct.is_complex()
                    ? rep.insert(regular_complex_array<coord_type>(
                          ct.rcos(), ct.mag(), a, b, amax, bmax))
                    : rep.insert(regular_array<coord_type>(a, b, amax, bmax))) {
    //  .. nothing yet ..
  }
   ...
   *  an appropriate basic_array object.
   */
  explicit array(const Obj &obj, const trans_type &trans, double acos,
                 double mag)
      : m_obj(obj), m_trans(trans),
        mp_base(new single_complex_inst<coord_type>(acos, mag)) {
    //  .. nothing yet ..
  }
  ...
  array(const Obj &obj, const trans_type &trans, Iter from, Iter to)
      : m_obj(obj), m_trans(trans),
        mp_base(new iterated_array<coord_type>(from, to)) {
    //  .. nothing yet ..
  }
  ...
  array(const Obj &obj, const complex_trans_type &ct, Iter from, Iter to)
      : m_obj(obj), m_trans(ct),
        mp_base(ct.is_complex() ? new iterated_complex_array<coord_type>(
                                      ct.rcos(), ct.mag(), from, to)
                                : new iterated_array<coord_type>(from, to)) {
    //  .. nothing yet ..
  }
  ...
  explicit array(const Obj &obj, const complex_trans_type &ct)
      : m_obj(obj), m_trans(ct),
        mp_base(ct.is_complex()
                    ? new single_complex_inst<coord_type>(ct.rcos(), ct.mag())
                    : 0) {
    //  .. nothing yet ..
  }
   ...
   * providing it's own storage.
   */
  explicit array(const Obj &obj, const trans_type &trans, ArrayRepository &rep,
                 double acos, double mag)
      : m_obj(obj), m_trans(trans),
        mp_base(rep.insert(single_complex_inst<coord_type>(acos, mag))) {
    //  .. nothing yet ..
  }
  ...
  explicit array(const Obj &obj, const complex_trans_type &ct,
                 ArrayRepository &rep)
      : m_obj(obj), m_trans(ct),
        mp_base(ct.is_complex() ? rep.insert(single_complex_inst<coord_type>(
                                      ct.rcos(), ct.mag()))
                                : 0) {
    //  .. nothing yet ..
  }
   ...
   *  @brief The copy constructor
   */
  array(const array &d) : m_obj(d.m_obj), m_trans(d.m_trans), mp_base(0) {
    if (d.mp_base) {
   ...
   *
   *  If the array repository pointer is 0, the d array will be detached from an
   * repository. Otherwise it will be translated to the given repository.
   */
  ...
  array_iterator<coord_type, Trans> begin_touching(const box_type &b,
                                                   const BoxConv &bc) const {
    if (b.empty()) {
      ...
      if (ob.empty()) {
        ...
        if (mp_base->is_complex()) {
          complex_trans_type ct =
              mp_base->complex_trans(simple_trans_type(m_trans));
        ...
        return array_iterator<coord_type, Trans>(
            m_trans, mp_base->begin_touching(
                         box_type(point_type() + (b.p1() - (ob.p2() + d)),
                                  point_type() + (b.p2() - (ob.p1() + d)))));
      }
    } else {
      box_type ob(bc(m_obj));
      if (ob.empty()) {
        ...
        ob.transform(db::fixpoint_trans<coord_type>(m_trans.rot()));
        return array_iterator<coord_type, Trans>(
            m_trans, !box_type(point_type() + (b.p1() - (ob.p2() + d)),
                               point_type() + (b.p2() - (ob.p1() + d)))
                          .contains(point_type()));
      }
    }
   ...
   *
   *  This method delivers an array iterator pointing to the member with the
   * given indexes, provided the array is a regular one. If the array is not
   * regular this method will deliver the same iterator than begin (). There is
   * no end() method. Rather use the at_end() method of the iterator to
   * determine the end of the sequence.
   */
  array_iterator<coord_type, Trans> begin(long a, long b) const {
    if (mp_base) {
      return array_iterator<coord_type, Trans>(m_trans,
                                               mp_base->begin_regular(a, b));
    } else {
   ...
   */
  template <class BoxConv> box_type bbox(const BoxConv &bc) const {
    if (mp_base) {
      if (mp_base->is_complex()) {
        return mp_base->bbox(box_type(
            mp_base->complex_trans(simple_trans_type(m_trans)) * bc(m_obj)));
      } else {
  ...
  box_type bbox_from_raw_bbox(const box_type &rb, const BoxConv &bc) const {
    if (mp_base) {
      if (mp_base->is_complex()) {
        return rb * (box_type(mp_base->complex_trans(
                                  simple_trans_type(m_trans.fp_trans())) *
                              bc(m_obj)));
      } else {
  ...
  box_type quad_box(const Iter &iter, const BoxConv &bc) const {
    ...
    if (mp_base) {
      if (mp_base->is_complex()) {
        return bb *
               box_type(mp_base->complex_trans(simple_trans_type(m_trans)) *
                        bc(m_obj));
      } else {
   ...
   */
  const trans_type &front() const { return m_trans; }

  /**
   *  @brief Access to the object in the instance
   */
  Obj &object() { return m_obj; }

  /**
   *  @brief Access to the object in the instance (const version)
   */
  const Obj &object() const { return m_obj; }
  ...
  void invert() {
    if (mp_base) {
      ...
      //  Hint: this assumes that the delegate does not carry a "superior"
      //  transformation over trans_type - i.e. no complex trans inside arrays
      //  stating a unit_trans type for example. The assertion checks that.
   ...
   */
  bool operator!=(const array<Obj, Trans> &d) const { return !operator==(d); }
  ...
  bool equal(const array<Obj, Trans> &d) const {
    if (!mp_base) {
      ...
      if (!m_trans.equal(d.m_trans) || !(m_obj == d.m_obj) ||
          type() != d.type()) {
        return false;
      }
   ...
   */
  bool in_repository() const { return mp_base && mp_base->in_repository; }

  /**
   *  @brief Test, if the array is a complex array
   *
   *  Returns true if the array represents complex instances (that is, with
   * magnification and arbitrary rotation angles).
   */
  bool is_complex() const { return mp_base && mp_base->is_complex(); }
   ...
   */
  int type() const { return mp_base ? mp_base->type() : 0; }

  /**
   *  @brief Return the complex_transformation
   *
   *  Returns the complex transformation that incorporates magnification and
   * arbitrary angle rotation. This method returns the complex transformation
   * for the first element in the array. To obtain the complex transformation
   * for a specific element, use 'complex_trans (*iter)' instead.
   */
  complex_trans_type complex_trans() const {
    return mp_base ? mp_base->complex_trans(simple_trans_type(m_trans))
                   : complex_trans_type(m_trans);
  }
   ...
   *
   *  Returns the complex transformation that incorporates magnification and
   * arbitrary angle rotation for a given base transformation. This method is
   * intended to be used with the base transformation delivered by an iterator.
   */
  complex_trans_type complex_trans(const trans_type &trans) const {
    return mp_base ? mp_base->complex_trans(simple_trans_type(trans))
                   : complex_trans_type(trans);
  }
   ...
   *  one.
   */
  bool is_regular_array(vector_type &a, vector_type &b, unsigned long &amax,
                        unsigned long &bmax) const {
   ...
   *
   *  This operator is required since shape reference arrays are put into
   * db::Layer objects
   *
   *  Hint this method is supposed to be used for shape arrays. Instance arrays
   * to not support the translate method currently.
   *  TODO: this functionality should be put into the Shapes translate method
   * rather than into this class ...
   */
  void translate(const array<Obj, Trans> &d,
    ...
    if (d.mp_base) {
      mp_base = d.mp_base->in_repository ? array_rep.insert(*d.mp_base)
                                         : d.mp_base->clone();
    } else {
      mp_base = 0;
    }
   ...
   */
  template <class T>
  void translate(const array<Obj, Trans> &d, const T &t,
                 db::generic_repository<coord_type> &rep,
                 db::ArrayRepository &array_rep) {
    //  Translate the array into the new system and if necessary, transform and
   ...
   */
  void swap(db::array<Obj, Trans> &b) {
   ...
   */
  const ArrayBase *delegate() const { return mp_base; }

  /**
   *  @brief In-place transformation of an array
   *
   *  This method will transform the array specification according to the given
   * transformation.
   *
   *  @param tr The transformation to apply
   *  @param rep The repository where to enter the new array or 0 if no
   * repository should be used
   *
   *  Hint: this method is supposed to be used for arrays with
   * simple_transformation for the transformation type mainly. The reason is
   * that this method will create complex arrays from complex transformations.
   *  Arrays based on reduced transformations (unit_trans, disp_trans) must not
   * use complex arrays because that is not supported currently. In other words,
   * this method will not try to transform the object itself and rather put all
   * transformation aspects into the array, which does not work on reduced
   * transformation types, except if the applied transformation "tr" is also a
   * reduced type. If required, the remaining component can be computed from the
   * previous complex transformation and the new one using this formula: trem =
   * t_new.inverted() * tr * t_old where t_new is complex_trans() or front()
   * after this method has been called and t_old is complex_trans() or front()
   ...
   *     simple    all                   yes
   *
   *  For the non-compatible combinations, %translate is provided. This however
   * requires that the object can be transformed ("translated" in the more
   * general sense). That specifically applies to shape arrays and references
   * which are based on unit or disp transformations.
   */
  template <class T>
   ...
   *
   *  Arrays are transformed into a new system by applying a formal
   * transformation A' = T A Ti (Ti is the inverse of T). Hence T A = A' T. In
   * other words: the transformation T is propagated over the array A. This
   * transformation is useful for transformation of hierarchies, since the
   * transformation T can be propagated further down in the hierarchy therefore.
   *
   *  See the remarks on "transform" regarding compatibility of the
   * transformation argument and the Trans type of this array.
   *
   *  @param tr The transformation to apply
   *  @param rep The repository where to enter the new array or 0 if no
   * repository should be used
   ...
   */
  template <class T>
  array transformed_into(const T &tr,
                         db::ArrayRepository *array_rep = 0) const {
    array a(*this);
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    if (!no_self) {
      stat->add(typeid(array<Obj, Trans>), (void *)this,
                sizeof(array<Obj, Trans>), sizeof(array<Obj, Trans>), parent,
                purpose, cat);
    }
...
private:
  ...
  void transform_into_from(const unit_trans_type & /*tr*/,
                           const array<Obj, Trans> & /*d*/) {
    //  .. nothing to do ..
  }

  void transform_into_from(const disp_trans_type &tr,
                           const array<Obj, Trans> &d) {
    if (is_complex()) {
      ...
      simple_trans_type t =
          simple_trans_type(tr) * d.front() * simple_trans_type(tr.inverted());
      m_trans = Trans(t);
    }
  }

  void transform_into_from(const simple_trans_type &tr,
                           const array<Obj, Trans> &d) {
    if (is_complex()) {
      complex_trans_type t = d.complex_trans().transform_into(tr);
      m_trans = trans_type(t);
  ...
  void transform_into_from(const complex_trans_type &tr,
                           const array<Obj, Trans> &d) {
    complex_trans_type t = d.complex_trans().transform_into(tr);
    m_trans = trans_type(t);
    set_complex(t.mag(), t.rcos(), d);
  ...
  void translate_from(const unit_trans_type & /*tr*/,
                      const array<Obj, Trans> & /*d*/,
                      db::generic_repository<coord_type> & /*rep*/,
                      db::ArrayRepository & /*array_rep*/) {
    //  .. nothing to do ..
  ...
  void translate_from(const disp_trans_type &tr, const array<Obj, Trans> &d,
                      db::generic_repository<coord_type> &rep,
                      db::ArrayRepository &array_rep) {
    if (is_complex()) {
      ...
      m_trans = trans_type(t);
      set_complex(t.mag(), t.rcos(), d);
      m_obj.translate(d.m_obj,
                      complex_trans().inverted() * complex_trans_type(t), rep,
                      array_rep);
    } else {
      simple_trans_type t =
          simple_trans_type(tr) * simple_trans_type(d.front());
      m_trans = trans_type(t);
      m_obj.translate(d.m_obj, simple_trans_type(m_trans.inverted()) * t, rep,
                      array_rep);
    }
  }

  void translate_from(const simple_trans_type &tr, const array<Obj, Trans> &d,
                      db::generic_repository<coord_type> &rep,
                      db::ArrayRepository &array_rep) {
    if (is_complex()) {
      complex_trans_type t = complex_trans_type(tr) * d.complex_trans();
  ...
  void translate_from(const complex_trans_type &tr, const array<Obj, Trans> &d,
                      db::generic_repository<coord_type> &rep,
                      db::ArrayRepository &array_rep) {
    if (is_complex()) {
      ...
      //  don't create a complex array - delegate the transformation to the
      //  translate method
      complex_trans_type t = tr * complex_trans_type(d.front());
      m_trans = trans_type(t);
      m_obj.translate(d.m_obj, complex_trans_type(m_trans.inverted()) * t, rep,
                      array_rep);
    }
  }

  void transform_from(const unit_trans_type & /*tr*/,
                      const array<Obj, Trans> & /*d*/) {
    //  .. nothing to do ..
  }
  ...
  void transform_from(const complex_trans_type &tr,
                      const array<Obj, Trans> &d) {
    complex_trans_type t = tr * d.complex_trans();
  ...
  void set_complex(double mag, double rcos, const array<Obj, Trans> &d) {
    ...
    if (fabs(mag - 1.0) > epsilon || fabs(rcos - 1.0) > epsilon) {
      ...
      if (is_reg) {
        new_base =
            new regular_complex_array<coord_type>(rcos, mag, a, b, amax, bmax);
      } else if (is_iterated) {
        new_base = new iterated_complex_array<coord_type>(rcos, mag, v.begin(),
                                                          v.end());
      } else {
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const array<Obj, Trans> &x, bool no_self = false,
                     void *parent = 0) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
