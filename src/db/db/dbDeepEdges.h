 ...
 */
class DB_PUBLIC DeepEdges : public db::MutableEdges,
...
public:
  ...
  virtual EdgePairsDelegate *
  processed_to_edge_pairs(const EdgeToEdgePairProcessorBase &filter) const;
  virtual RegionDelegate *
  processed_to_polygons(const EdgeToPolygonProcessorBase &filter) const;
...
private:
  ...
  std::pair<DeepLayer, DeepLayer> edge_region_op(const DeepRegion *other,
                                                 EdgePolygonOp::mode_t op,
  ...
  OutputContainer *processed_impl(const edge_processor<Result> &filter) const;
};
