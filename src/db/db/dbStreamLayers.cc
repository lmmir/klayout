...
LayerMap::LayerMap() : m_ld_map(), m_next_index(0) {
  //  .. nothing yet ..
}
...
bool LayerMap::is_mapped(const std::string &n) const {
  std::map<std::string, std::set<unsigned int>>::const_iterator m =
      m_name_map.find(n);
...
std::set<unsigned int> LayerMap::logical(const LDPair &p) const {
  return logical_internal(p, false);
}

std::set<unsigned int> LayerMap::logical(const std::string &n) const {
  return logical_internal(n, false);
}

std::set<unsigned int> LayerMap::logical(const db::LayerProperties &p) const {
  return logical_internal(p, false);
}
...
LayerMap::logical_internal(const std::string &n, bool allow_placeholder) const {
  std::map<std::string, std::set<unsigned int>>::const_iterator m =
      m_name_map.find(n);
  if (m != m_name_map.end() &&
      (allow_placeholder || !is_placeholder(m->second))) {
    return m->second;
  } else {
    return std::set<unsigned int>();
  }
...
std::set<unsigned int>
LayerMap::logical_internal(const db::LayerProperties &p,
                           bool allow_placeholder) const {
  std::set<unsigned int> m;
  if (p.layer >= 0 && p.datatype >= 0) {
...
bool LayerMap::is_placeholder(const std::set<unsigned int> &m) const {
  for (std::set<unsigned int>::const_iterator i = m.begin(); i != m.end();
       ++i) {
...
const db::LayerProperties *LayerMap::target(unsigned int l) const {
  std::map<unsigned int, LayerProperties>::const_iterator i =
      m_target_layers.find(l);
...
std::set<unsigned int> LayerMap::logical(const db::LayerProperties &p,
...
std::set<unsigned int> LayerMap::logical(const db::LDPair &p,
                                         db::Layout &layout) const {
  std::set<unsigned int> l = logical_internal(p, true);
  if (is_placeholder(l)) {
    return const_cast<LayerMap *>(this)->substitute_placeholder(
        db::LayerProperties(p.layer, p.datatype), l, layout);
  } else {
    return l;
  }
}
...
std::set<unsigned int>
LayerMap::substitute_placeholder(const db::LayerProperties &p,
                                 const std::set<unsigned int> &m,
                                 db::Layout &layout) {
  ...
  for (std::set<unsigned int>::const_iterator i = m.begin(); i != m.end();
    ...
    if (m_placeholders.size() > std::numeric_limits<unsigned int>::max() - *i) {
      ...
      const db::LayerProperties &lp_ph =
          m_placeholders[std::numeric_limits<unsigned int>::max() - *i];
...
extract_dt_intervals(const LayerMap::datatype_map &dt_map, int ll,
                     bool &has_others) {
  std::vector<std::pair<ld_type, ld_type>> res;

  for (LayerMap::datatype_map::const_iterator d = dt_map.begin();
       d != dt_map.end();) {
    ...
    if (d->second.find(ll) != d->second.end()) {
      ...
      ++dd;
      while (dd != dt_map.end() && dd->first.first == dpi.second &&
             dd->second.find(ll) != dd->second.end()) {
        if (dd->second.size() > 1) {
          has_others = true;
        }
...
std::string LayerMap::mapping_str(unsigned int ll) const {
  std::string s;
  bool f1 = true;
  bool is_mmap = false;
  ...
  for (ld_map::const_iterator l = m_ld_map.begin(); l != m_ld_map.end();) {
    ...
    std::vector<std::pair<ld_type, ld_type>> dti =
        extract_dt_intervals(l->second, ll, is_mmap);
    ++l;
    while (l != m_ld_map.end() && lti.second == l->first.first &&
           extract_dt_intervals(l->second, ll, is_mmap) == dti) {
      lti.second = l->first.second;
      ++l;
    }

    bool f2 = true;
    for (std::vector<std::pair<ld_type, ld_type>>::const_iterator d =
             dti.begin();
         d != dti.end(); ++d) {
  ...
  for (std::map<std::string, std::set<unsigned int>>::const_iterator l =
           m_name_map.begin();
       l != m_name_map.end(); ++l) {
  ...
  std::map<unsigned int, LayerProperties>::const_iterator t =
      m_target_layers.find(ll);
...
void LayerMap::prepare(db::Layout &layout) {
  ...
  //  determine the mapping of existing layers to real layers and create layers
  //  if required
  DirectLayerMapping layer_mapping(&layout);

  std::vector<unsigned int> old_layers = get_layers();
  for (std::vector<unsigned int>::const_iterator l = old_layers.begin();
       l != old_layers.end(); ++l) {
    ...
    if (layout.is_valid_layer(*l)) {
      ...
      if (lp.is_named() ||
          (db::is_static_ld(lp.layer) && db::is_static_ld(lp.datatype))) {
  ...
  for (ld_map::iterator l = m_ld_map.begin(); l != m_ld_map.end(); ++l) {
    for (datatype_map::iterator d = l->second.begin(); d != l->second.end();
         ++d) {
      std::set<unsigned int> dn;
      for (std::set<unsigned int>::const_iterator i = d->second.begin();
           i != d->second.end(); ++i) {
  ...
  for (std::map<std::string, std::set<unsigned int>>::iterator n =
           m_name_map.begin();
       n != m_name_map.end(); ++n) {
    std::set<unsigned int> dn;
    for (std::set<unsigned int>::const_iterator i = n->second.begin();
         i != n->second.end(); ++i) {
      dn.insert(real_layers[*i]);
    }
    n->second = dn;
  ...
  for (std::map<unsigned int, LayerProperties>::const_iterator tl =
           old_target_layers.begin();
       tl != old_target_layers.end(); ++tl) {
    m_target_layers[real_layers[tl->first]] = tl->second;
  }

  //  In addition, map other existing layers as well, so merging of layout is
  //  somewhat better supported
  for (db::Layout::layer_iterator l = layout.begin_layers();
       l != layout.end_layers(); ++l) {
    if (!(*l).second->is_null() &&
        mapped_layers.find((*l).first) == mapped_layers.end()) {
      map(*(*l).second, (*l).first);
    }
  }
}

std::vector<unsigned int> LayerMap::get_layers() const {
  std::set<unsigned int> layers;

  for (ld_map::const_iterator l = m_ld_map.begin(); l != m_ld_map.end(); ++l) {
    for (datatype_map::const_iterator d = l->second.begin();
         d != l->second.end(); ++d) {
      layers.insert(d->second.begin(), d->second.end());
    }
  }
  for (const_iterator_names n = m_name_map.begin(); n != m_name_map.end();
       ++n) {
...
LayerProperties LayerMap::mapping(unsigned int ll) const {
  //  try to combine source and target spec into a final specification (if for
  //  example, just a name is specified and layer/datatype are given in the
  //  source spec).
  ...
  if (t != m_target_layers.end()) {
    ...
    //  special case: if it is a name mapping, add the layer mapping (for
...
void parse_interval(tl::Extractor &ex, ld_interval &p) {
  ...
  if (ex.test("*")) {
    ...
    if (ex.test("-")) {
      if (ex.test("*")) {
        //  NOTE: as the upper limit is stored as n2 + 1, this will map to
        //  max():
        n2 = std::numeric_limits<ld_type>::max() - 1;
      } else {
        ex.try_read(n2);
  ...
  try {
    ...
    do {
      ...
      if (!ex.try_read(n) && ex.try_read_word_or_quoted(name)) {
        ...
        datatype_map dm;
        for (ld_interval_vector::const_iterator di = vd.begin(); di != vd.end();
             ++di) {
          LmapJoinOp1 op1;
        ...
        for (ld_interval_vector::const_iterator li = vl.begin(); li != vl.end();
             ++li) {
          LmapJoinOp2 op2;
...
void LayerMap::insert(const std::string &name, unsigned int l,
                      const LayerProperties *target) {
  if (target) {
...
void LayerMap::insert(const LDPair &p1, const LDPair &p2, unsigned int l,
                      const LayerProperties *target) {
  if (target) {
    m_target_layers[l] = *target;
  }
...
void LayerMap::unmap(const LDPair &f) { unmap(f, f); }

void LayerMap::unmap(const std::string &name) { m_name_map.erase(name); }
...
void LayerMap::unmap(const LDPair &p1, const LDPair &p2) {
  ...
  if (db::is_static_ld(p1.layer) && db::is_static_ld(p2.layer)) {
    ...
    m_ld_map.add(m_ld_map.begin()->first.first,
                 (--m_ld_map.end())->first.second, LayerMap::datatype_map(),
                 op);
  }
}
  ...
  try {
    ...
    do {
      ...
      if (!ex.try_read(n) && ex.try_read_word_or_quoted(name)) {
        ...
        for (ld_interval_vector::const_iterator li = vl.begin(); li != vl.end();
             ++li) {
          for (ld_interval_vector::const_iterator di = vd.begin();
               di != vd.end(); ++di) {
...
std::string LayerMap::to_string() const {
...
std::string LayerMap::to_string_file_format() const {
  std::vector<unsigned int> layers = get_layers();
  std::ostringstream os;
  for (std::vector<unsigned int>::const_iterator l = layers.begin();
       l != layers.end(); ++l) {
...
db::LayerMap LayerMap::from_string_file_format(const std::string &s) {
  db::LayerMap lm;
  unsigned int l = 0;
  ...
  try {
    ...
    throw tl::Exception(ex.msg() + tl::to_string(tr(" in line ")) +
                        tl::to_string(lnr));
  }
...
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::LayerMap &t) {
  t = db::LayerMap();
