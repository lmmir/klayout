 ...
 */
class DB_PUBLIC LayerSpecFormatException : public tl::Exception {
public:
  LayerSpecFormatException(const char *s)
      : tl::Exception(tl::to_string(tr(
                          "Not a valid layer map expression: '..%s' (use '/' "
                          "to separated layer and datatype, ',' to list "
                          "numbers for layer or datatype, '-' to create ranges "
                          "and ';' to concatenate multiple subexpressions)")),
                      s) {}
};
 ...
 */
inline ld_type any_ld() { return -1; }

inline bool is_any_ld(ld_type ld) { return ld == -1; }

inline bool is_static_ld(ld_type ld) { return ld >= 0; }
...
inline bool is_relative_ld(ld_type ld) { return ld < 0; }
...
struct DB_PUBLIC LDPair {
  LDPair() : layer(0), datatype(0) {}

  LDPair(ld_type l, ld_type d) : layer(l), datatype(d) {}

  static LDPair invalid() { return LDPair(-1, -1); }
  ...
  bool operator!=(const LDPair &d) const { return !operator==(d); }
 ...
 */
class DB_PUBLIC LayerMap : public gsi::ObjectBase {
public:
  ...
  typedef std::map<std::string, std::set<unsigned int>>::const_iterator
   ...
   */
  template <class L>
  std::pair<bool, unsigned int> first_logical(const L &p,
   ...
   */
  std::set<unsigned int> logical(const db::LayerProperties &p,
                                 db::Layout &layout) const;
  ...
  /**
   *  @brief Returns a value indicating whether a layer (given by
   * layer/datatype) is mapped
   */
   ...
   */
  template <class S> void map(const S &p, unsigned int l) {
    unmap(p);
    mmap(p, l);
  }
  ...
  /**
   *  @brief Same a map_expr with a string argument but taking the expression
   * for a tl::Extractor
   */
   ...
   *  the name if no LDPair is given. If the source LayerProperties structure
   * does not specify a valid layer, no mapping is provided.
   *
  ...
  /**
   *  @brief Multi-map a range of ldpair's to a logical layer with a target
   * layer
   *
   *  The range is given by two pairs p1,p2. The layers
   *  mapped are [p1.l,p2.l], the datatypes mapped are [p1.d,p2.d].
   ...
   *
   *  This generic mapping function takes a mapping expression. If it starts
   * with "+", "mmap" is used, if it starts with "-", "unmap" is used.
   ...
   *
   *  This replaces all layer indexes by ones from the layout or it will create
   * new layers if required. Note that for relative and wildcard targets (except
   * '* / *'), the layer indexes will not be the true indexes but placeholders.
   * They will be replaced later when calling logical with a layout argument.
   */
   ...
   */
  unsigned int next_index() const { return m_next_index; }
   ...
   */
  const_iterator_layers begin() const { return m_ld_map.begin(); }

  /**
   *  @brief Get the iterator for the mapping (end)
   */
  const_iterator_layers end() const { return m_ld_map.end(); }

  /**
   *  @brief Get the iterator for the name mapping (begin)
   */
  const_iterator_names begin_name_mapping() const { return m_name_map.begin(); }

  /**
   *  @brief Get the iterator for the name mapping (end)
   */
  const_iterator_names end_name_mapping() const { return m_name_map.end(); }
...
private:
  ...
  void insert(const LDPair &p1, const LDPair &p2, unsigned int l,
              const LayerProperties *t);
  void insert(const std::string &name, unsigned int l,
              const LayerProperties *t);

  std::set<unsigned int> logical_internal(const LDPair &p,
                                          bool allow_placeholder) const;
  std::set<unsigned int> logical_internal(const std::string &name,
                                          bool allow_placeholder) const;
  std::set<unsigned int> logical_internal(const db::LayerProperties &p,
                                          bool allow_placeholder) const;

  std::set<unsigned int>
  substitute_placeholder(const db::LayerProperties &p,
                         const std::set<unsigned int> &ph, db::Layout &layout);
