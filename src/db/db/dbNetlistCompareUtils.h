 ...
 *  This name is used for comparing the net names and also employs the pin name
 * if one is given
 */
...
/**
 *  @brief The device compare function with "less" (operator()) and "equal"
 * predicates
 *
 *  Device comparison is based on the equivalence of device classes (by
 * category) and in a second step, by equivalence of the devices. The device
 * class will implement the device equivalence function.
 */
struct DB_PUBLIC DeviceCompare {
  bool operator()(const std::pair<const db::Device *, size_t> &d1,
                  const std::pair<const db::Device *, size_t> &d2) const;
  bool equals(const std::pair<const db::Device *, size_t> &d1,
              const std::pair<const db::Device *, size_t> &d2) const;
};
 ...
 *
 *  As Subcircuits are not parametrized, the comparison of subcircuits is only
 * based on the circuit equivalence (via category).
 */
struct DB_PUBLIC SubCircuitCompare {
  bool operator()(const std::pair<const db::SubCircuit *, size_t> &sc1,
                  const std::pair<const db::SubCircuit *, size_t> &sc2) const;
  bool equals(const std::pair<const db::SubCircuit *, size_t> &sc1,
              const std::pair<const db::SubCircuit *, size_t> &sc2) const;
};
...
/**
 *  @brief The Circuit pin categorizer handles swappable pin definitions per
 * circuit
 *
 ...
 *  netlist A and B and also the pin mapping between the circuits from these
 * netlists.
 *
...
public:
  ...
  void set_other(const db::Circuit *other) { mp_other = other; }

  const db::Circuit *other() const { return mp_other; }
...
template <class Obj> class generic_equivalence_tracker {
public:
  ...
  bool map(const Obj *a, const Obj *b) {
    std::pair<typename std::map<const Obj *, const Obj *>::iterator, bool>
        inserted1 = m_eq.insert(std::make_pair(a, b));
    tl_assert(inserted1.first->second == b);
    std::pair<typename std::map<const Obj *, const Obj *>::iterator, bool>
        inserted2 = m_eq.insert(std::make_pair(b, a));
  ...
  const Obj *other(const Obj *o) const {
    typename std::map<const Obj *, const Obj *>::const_iterator i =
        m_eq.find(o);
...
public:
  SubCircuitEquivalenceTracker()
      : generic_equivalence_tracker<db::SubCircuit>() {}
};
 ...
 */
template <class Obj> class DB_PUBLIC generic_categorizer {
public:
 ...
 *
 *  The objective of this class is to supply a category ID for a given device
 * class. The category ID also identities equivalent device classes from netlist
 * A and B.
 */
 ...
 *
 *  The objective of this class is to supply a category ID for a given device
 * circuit. The category ID also identities equivalent circuit from netlist A
 * and B.
 */
class DB_PUBLIC CircuitCategorizer : private generic_categorizer<db::Circuit> {
public:
  CircuitCategorizer();
