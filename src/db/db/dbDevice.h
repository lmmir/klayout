...
/**
 *  @brief A structure describing a terminal reference into another device
 * abstract
 */
struct DeviceReconnectedTerminal {
  DeviceReconnectedTerminal(size_t _device_index,
                            unsigned int _other_terminal_id)
      : device_index(_device_index), other_terminal_id(_other_terminal_id) {
    //  .. nothing yet ..
  }

  DeviceReconnectedTerminal() : device_index(0), other_terminal_id(0) {
    //  .. nothing yet ..
  }
...
struct DeviceAbstractRef {
  DeviceAbstractRef(const db::DeviceAbstract *_device_abstract,
                    const db::DCplxTrans &_trans)
      : device_abstract(_device_abstract), trans(_trans) {
    //  .. nothing yet ..
  }

  DeviceAbstractRef() : device_abstract(0), trans() {
    //  .. nothing yet ..
  }
 ...
 */
class DB_PUBLIC Device : public db::NetlistObject {
public:
   ...
   *  @brief The constructor
   */
  Device(DeviceClass *device_class, DeviceAbstract *device_abstract,
   ...
   */
  const DeviceClass *device_class() const { return mp_device_class; }

  /**
   *  @brief Sets the device class
   */
  void set_device_class(DeviceClass *dc) { mp_device_class = dc; }

  /**
   *  @brief Gets the device abstract
   */
  const DeviceAbstract *device_abstract() const { return mp_device_abstract; }

  /**
   *  @brief Sets the device abstract
   */
  void set_device_abstract(DeviceAbstract *dm) { mp_device_abstract = dm; }

  /**
   *  @brief Gets the device ID
   *  The ID is a unique integer which identifies the device.
   *  It can be used to retrieve the device from the circuit using
   * Circuit::device_by_id. When assigned, the device ID is not 0.
   */
   ...
   */
  const Circuit *circuit() const { return mp_circuit; }

  /**
   *  @brief Gets the circuit the device lives in (non-const version)
   *  This pointer is 0 if the device isn't added to a circuit
   */
  Circuit *circuit() { return mp_circuit; }
   ...
   *  The device position should be the center and orientation of the
   * recognition shape or something similar. Giving the device a position allows
   * combining multiple devices with the same relative geometry into a single
   * cell. The transformation has to be given in micrometer units.
   */
  ...
  Net *net_for_terminal(size_t terminal_id) {
    return const_cast<Net *>(
        ((const Device *)this)->net_for_terminal(terminal_id));
  }
   ...
   */
  void join_terminals(unsigned int this_terminal, db::Device *other,
                      unsigned int other_terminal);

  /**
   *  @brief Used for device combination: reroute terminal to other device
   *
   *  This will disconnect "this_terminal" from the device and make a connection
   * to "other_terminal" of the "other" device instead.
   *
   *  An internal connection between "this_terminal" and "from_other_terminal"
   * is implied.
   */
  void reroute_terminal(unsigned int this_terminal, db::Device *other,
                        unsigned int from_other_terminal,
                        unsigned int other_terminal);

  /**
   *  @brief Gets the set of other terminal references
   *
   *  This method will return 0 if the device isn't a combined device or the
   * given terminal is not connector to a different abstract.
   *
   *  The returned vector (if any) is a complete list of terminals connected to
   * the given logical device terminal.
   */
  const std::vector<DeviceReconnectedTerminal> *
  reconnected_terminals_for(unsigned int this_terminal) const {
    std::map<unsigned int,
             std::vector<DeviceReconnectedTerminal>>::const_iterator t =
        m_reconnected_terminals.find(this_terminal);
   ...
   */
  const std::map<unsigned int, std::vector<DeviceReconnectedTerminal>> &
  reconnected_terminals() const {
   ...
   *
   *  NOTE: don't use this method to modify this container! It's provided for
   * persistence implementation only.
   */
  std::map<unsigned int, std::vector<DeviceReconnectedTerminal>> &
  reconnected_terminals() {
    return m_reconnected_terminals;
  }
  ...
  void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose, int cat,
    ...
    db::mem_stat(stat, purpose, cat, m_reconnected_terminals, true,
                 (void *)this);
  }
...
private:
   ...
   */
  void set_terminal_ref_for_terminal(size_t terminal_id,
                                     Net::terminal_iterator iter);
  ...
  void add_others_terminals(unsigned int this_terminal, db::Device *other,
                            unsigned int other_terminal);
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Device &x, bool no_self, void *parent) {
