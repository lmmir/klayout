...
SubCircuit::SubCircuit() : db::NetlistObject(), m_id(0), mp_circuit(0) {
  //  .. nothing yet ..
}

SubCircuit::~SubCircuit() {
  for (std::vector<Net::subcircuit_pin_iterator>::const_iterator p =
           m_pin_refs.begin();
       p != m_pin_refs.end(); ++p) {
...
SubCircuit::SubCircuit(Circuit *circuit, const std::string &name)
    : db::NetlistObject(), m_circuit_ref(0), m_name(name), m_id(0),
      mp_circuit(0) {
...
void SubCircuit::set_trans(const db::DCplxTrans &t) { m_trans = t; }

void SubCircuit::set_pin_ref_for_pin(size_t pin_id,
                                     Net::subcircuit_pin_iterator iter) {
