 ...
 * db::NetlistDeviceExtractor and more. It is supposed to provide a framework
 * for extracting a netlist from a layout.
 *
 *  The use model of this class consists of five steps which need to be executed
 * in this order.
 *
 *  @ul
 ...
 * "set_threads", "set_area_ratio" or "set_max_vertex_count". The constructor
 * for the LayoutToNetlist object receives a db::RecursiveShapeIterator object
 * which basically supplies the hierarchy and the layout taken as input.
 *  @/li
 *  @li Preparation
 *      In this step, the device recognitions and extraction layers are drawn
 * from the framework. Derived can now be computed using boolean operations.
 ...
 * "extract_devices". This method needs to be called for each device extractor
 * required. Each time, a device extractor needs to be given plus a map of
 * device layers. The device layers are device extractor specific. Either
 * original or derived layers may be specified here. Layer preparation may
 * happen between calls to "extract_devices".
 *  @/li
 *  @li Once the devices are derived, the netlist connectivity can be defined
 * and the netlist extracted. The connectivity is defined with "connect" and
 * it's flavours. The actual netlist extraction happens with "extract_netlist".
 *  @/li
 ...
 *  @li
 */
class DB_PUBLIC LayoutToNetlist : public gsi::ObjectBase,
...
public:
  ...
  /**
   *  @brief Makes the extractor take over ownership over the DSS when it was
   * created with an external DSS
   */
  void keep_dss();
   ...
   *  was derived from.
   */
  const std::string &original_file() const { return m_original_file; }
  ...
  /**
   *  @brief Sets the number of threads to use for operations which support
   * multiple threads
   */
  ...
  /**
   *  @brief Sets the area_ratio parameter for the hierarchical network
   * processor This parameter controls splitting of large polygons in order to
   * reduce the error made by the bounding box approximation.
   */
  ...
  /**
   *  @brief Sets the max_vertex_count parameter for the hierarchical network
   * processor This parameter controls splitting of large polygons in order to
   * enhance performance for very big polygons.
   */
   ...
   *  accordingly. The scale factor applies an isotropic shrink (<1) or
   * expansion (>1).
   */
   ...
   *  Using a name or layer properties (see below) enhances readability of
   * backannotated information if layers are involved. Use this method to attach
   * a name to a region derived by boolean operations for example.
   *
   ...
   *  Including texts in "connect" makes net names begin assigned from the text
   * strings.
   */
  void register_layer(const ShapeCollection &collection,
   ...
   *  object.
   */
  template <class Collection> bool is_persisted(const Collection &coll) const {
   ...
   */
  db::Region *
  layer_by_original(const ShapeCollectionDelegateBase *original_delegate);

  /**
   *  @brief Iterates over the layer indexes and names managed by this object
   * (begin)
   */
  layer_iterator begin_layers() const { return m_name_of_layer.begin(); }

  /**
   *  @brief Iterates over the layer indexes and names managed by this object
   * (end)
   */
  layer_iterator end_layers() const { return m_name_of_layer.end(); }
   ...
   *  "layer_index" is the layer index of the desired layer in the original
   * layout. This variant produces polygons and takes texts for net name
   * annotation. A variant not taking texts is "make_polygon_layer". A Variant
   * only taking texts is "make_text_layer". All variants return a new'd object
   * which must be deleted by the caller. Named regions are considered
   * "precious". The LayoutToNetlist object will keep a reference on all named
   * layers, so they persist during the lifetime of the LayoutToNetlist object.
  ...
  /**
   *  @brief Creates a new text collection representing an original layer taking
   * texts only See "make_layer" for details.
   */
  db::Texts *make_text_layer(unsigned int layer_index,
                             const std::string &name = std::string());

  /**
   *  @brief Creates a new region representing an original layer taking polygons
   * and texts See "make_layer" for details.
   */
   ...
   *  This method will run device extraction for the given extractor. The layer
   * map is specific for the extractor and uses the region objects derived with
   * "make_layer" and it's variants.
   *
   *  In addition, derived regions/text collections can be passed too. Certain
   * limitations apply. It's safe to use boolean operations for deriving layers.
   * Other operations are applicable as long as they are capable of delivering
   * hierarchical layers.
   *
   *  If errors occur, the device extractor will contain theses errors.
   */
  void
  extract_devices(db::NetlistDeviceExtractor &extractor,
                  const std::map<std::string, db::ShapeCollection *> &layers);

  /**
   *  @brief Resets the extracted netlist
   *
   *  This method will invalidate the netlist and extraction. It is called
   * automatically when cone of the connect methods is called.
   */
  void reset_extracted();

  /**
   *  @brief Defines an intra-layer connection for the given layer.
   *  The layer is either an original layer created with "make_layer" and it's
   * variants or a derived layer. Certain limitations apply. It's safe to use
   *  boolean operations for deriving layers. Other operations are applicable as
   * long as they are capable of delivering hierarchical layers.
   */
  void connect(const db::Region &l);

  /**
   *  @brief Defines an inter-layer connection for the given layers.
   *  The conditions mentioned with intra-layer "connect" apply for this method
   * too.
   */
  void connect(const db::Region &a, const db::Region &b) { connect_impl(a, b); }

  /**
   *  @brief Defines an inter-layer connection for the given layers.
   *  As one layer is a texts layer, this connection will basically add net
   * labels.
   */
  void connect(const db::Region &a, const db::Texts &b) { connect_impl(a, b); }
   ...
   * labels.
   */
  void connect(const db::Texts &a, const db::Region &b) { connect_impl(b, a); }
   ...
   *
   *  Using this function will *add* one more rule. To clear all registered
   * rules, use "clear_join_net_names". These pattern will only act on top level
   * cells.
   */
   ...
   *  @brief Joins net names matching the given expression
   *
   *  Using this function will *add* one more rule specific to cells matching
   * the first glob pattern. To clear all registered rules, use
   * "clear_join_net_names". Pattern registered with this function will act on
   * the given cells, regardless of whether it's top level or not.
   */
   ...
   *  This method is mainly provided to test purposes.
   */
  const std::list<std::pair<tl::GlobPattern, tl::GlobPattern>> &
  joined_net_names_per_cell() const {
   ...
   *
   *  This method will make an explicit connection between the nets given in the
   * name set. This applies implicit joining of different nets with the same
   * label (intra-net joining) and of nets with different names (inter-net
   * joining). Intra-net joining is implied always.
   */
   ...
   *  Using this function will *add* one more rule specific to cells matching
   * the first glob pattern. To clear all registered rules, use
   * "clear_join_nets". Pattern registered with this function will act on the
   * given cells, regardless of whether it's top level or not.
   */
   ...
   *  This method is mainly provided to test purposes.
   */
  const std::list<std::pair<tl::GlobPattern, std::set<std::string>>> &
  joined_nets_per_cell() const {
   ...
   *  NOTE: this method is provided for special cases such as netlist readers.
   * Don't use it.
   */
   ...
   */
  bool is_netlist_extracted() const { return m_netlist_extracted; }
   ...
   */
  const db::Connectivity &connectivity() const { return m_conn; }

  /**
   *  @brief Gets the internal layer for a given extraction layer
   *  This method is required to derive the internal layer index - for example
   * for investigating the cluster tree.
   */
  ...
  /**
   *  @brief Creates a cell mapping for copying shapes from the internal layout
   * to the given target layout. If 'with_device_cells' is true, cells will be
   * produced for devices. These are cells not corresponding to circuits, so
   * they are disabled normally. Use this option, if you want to access device
   * terminal shapes per device. CAUTION: This function may create new cells in
   * "layout".
   */
  ...
  /**
   *  @brief Creates a cell mapping for copying shapes from the internal layout
   * to the given target layout for a given list of nets This version will only
   * create cells which are required to represent the given nets. If
   * 'with_device_cells' is true, cells will be produced for devices. These are
   * cells not corresponding to circuits, so they are disabled normally. Use
   * this option, if you want to access device terminal shapes per device.
  ...
  /**
   *  @brief Creates a cell mapping for copying shapes from the internal layout
   * to the given target layout. This version will not create new cells in the
   * target layout. If the required cells do not exist there yet, flattening
   * will happen.
   */
  db::CellMapping const_cell_mapping_into(const db::Layout &layout,
                                          const db::Cell &cell);

  /**
   *  @brief Creates a layer mapping for build_nets etc.
   *  This method will create new layers inside the target layout corresponding
   * to the original layers as kept inside the LayoutToNetlist database. It will
   * return a layer mapping table suitable for use with build_all_nets,
   * build_nets etc. Layers without original layer information will be given
   * layer numbers ln, ln+1 etc.
   */
  std::map<unsigned int, const db::Region *>
  create_layermap(db::Layout &target_layout, int ln) const;
   ...
   * (non-conver version)
   */
  db::hier_clusters<db::NetShape> &net_clusters() { return m_net_clusters; }
   ...
   */
  db::Region *
  shapes_of_net(const db::Net &net, const db::Region &of_layer, bool recursive,
                const db::ICplxTrans &trans = db::ICplxTrans()) const;

  /**
   *  @brief Delivers all shapes of a specific net and layer to the given Shapes
   * container.
   *
   *  If "recursive" is true, the returned region will contain the shapes of
   *  all subcircuits too.
   ...
   *  to delete this object.
   *
   *  propid is an optional properties ID which is attached to the shapes if not
   * 0.
   */
  void shapes_of_net(const db::Net &net, const db::Region &of_layer,
                     bool recursive, db::Shapes &to,
                     properties_id_type propid = 0,
                     const db::ICplxTrans &trans = db::ICplxTrans()) const;

  /**
   *  @brief Builds a net representation in the given layout and cell
   *
   *  This method puts the shapes of a net into the given target cell using a
   * variety of options to represent the net name and the hierarchy of the net.
   *
   *  If the netname_prop name is not nil, a property with the given name is
   * created and assigned the net name.
   *
   *  Net hierarchy is covered in three ways:
   *   * No connection indicated (hier_mode == BNH_Disconnected: the net shapes
   * are simply put into their respective circuits. The connections are not
   * indicated.
   *   * Subnet hierarchy (hier_mode == BNH_SubcircuitCells): for each root net,
   * a full hierarchy is built to accommodate the subnets (see build_net in
   * recursive mode).
   *   * Flat (hier_mode == BNH_Flatten): each net is flattened and put into the
   * circuit it belongs to.
   *
   *  If a device cell name prefix is given, cells will be produced for each
   * device abstract using a name like device_cell_name_prefix + device name.
   * Otherwise the device shapes are treated as part of the net.
   *
   ...
   *  @param netname_prop An (optional) property name to which to attach the net
   * name
   *  @param cell_name_prefix Chooses recursive mode if non-null
   *  @param device_cell_name_prefix See above
   */
  void build_net(const db::Net &net, db::Layout &target, db::Cell &target_cell,
                 const std::map<unsigned int, const db::Region *> &lmap,
                 NetPropertyMode prop_mode, const tl::Variant &netname_prop,
   ...
   *
   *  This method copies all nets into cells corresponding to the circuits. It
   * uses the cmap object to determine the target cell (create them with
   * "cell_mapping_into" or "const_cell_mapping_into"). If no mapping is
   * requested, the specific circuit it skipped.
   *
   *  The method has three net annotation modes:
   *   * No annotation (net_cell_name_prefix == 0 and netname_prop == nil): the
   * shapes will be put into the target cell simply
   *   * Net name property (net_cell_name_prefix == 0 and netname_prop != nil):
   * the shapes will be annotated with a property named with netname_prop and
   * containing the net name string.
   *   * Individual subcells per net (net_cell_name_prefix != 0): for each net,
   * a subcell is created and the net shapes will be put there (name of the
   * subcell = net_cell_name_prefix + net name). (this mode can be combined with
   * netname_prop too).
   *
   ...
   * Otherwise the device shapes are treated as part of the net.
   *
   *  @param cmap The mapping of internal layout to target layout for the
   * circuit mapping
   *  @param target The target layout
   *  @param lmap Target layer indexes (keys) and net regions (values)
   *  @param net_cell_name_prefix See method description
   *  @param net_prop_mode How to attach properties to shapes
   *  @param netname_prop The property key to use for the net name or "nil" for
   * no netname properties
   *  @param hier_mode See description of this method
   *  @param circuit_cell_name_prefix See method description
   *  @param device_cell_name_prefix See above
   */
  void build_all_nets(const db::CellMapping &cmap, db::Layout &target,
                      const std::map<unsigned int, const db::Region *> &lmap,
                      const char *net_cell_name_prefix,
                      NetPropertyMode prop_mode,
                      const tl::Variant &netname_prop,
                      BuildNetHierarchyMode hier_mode,
   ...
   */
  void build_nets(const std::vector<const Net *> *nets,
                  const db::CellMapping &cmap, db::Layout &target,
                  const std::map<unsigned int, const db::Region *> &lmap,
                  const char *net_cell_name_prefix, NetPropertyMode prop_mode,
                  const tl::Variant &netname_prop,
                  BuildNetHierarchyMode hier_mode,
                  const char *circuit_cell_name_prefix,
                  const char *device_cell_name_prefix) const;
   ...
   *
   *  This method will find a net looking at the given layer at the specific
   * position. It will traverse the hierarchy below if no shape in the requested
   * layer is found in the specified location.
   *
   *  If no net is found at all, 0 is returned.
   *
   *  This variant accepts a micrometer-unit location. The location is given in
   * the coordinate space of the initial cell.
   *
   *  The subcircuit path leading to the topmost net is stored in *sc_path_out
   * if this pointer is non-null.
   */
  db::Net *probe_net(const db::Region &of_region, const db::DPoint &point,
                     std::vector<SubCircuit *> *sc_path_out = 0,
                     Circuit *initial_circuit = 0);

  /**
   *  @brief Finds the net by probing a specific location on the given layer
   *  See the description of the other "probe_net" variant.
   *  This variant accepts a database-unit location. The location is given in
   * the coordinate space of the initial cell.
   */
   ...
   *  In other words: any diode will make the net safe against antenna
   * discharge.
   */
  db::Region antenna_check(
      const db::Region &gate, double gate_perimeter_factor,
      const db::Region &metal, double metal_perimeter_factor, double ratio,
      const std::vector<std::pair<const db::Region *, double>> &diodes =
          std::vector<std::pair<const db::Region *, double>>(),
      db::Texts *values = 0) {
    return antenna_check(gate, 1.0, gate_perimeter_factor, metal, 1.0,
                         metal_perimeter_factor, ratio, diodes, values);
  }

  /**
   *  @brief Variant of the antenna check not using the perimeter
   *  This version uses 0 for the perimeter factor hence not taking into account
   * the perimeter at all.
   */
  db::Region antenna_check(
      const db::Region &gate, const db::Region &metal, double ratio,
      const std::vector<std::pair<const db::Region *, double>> &diodes =
          std::vector<std::pair<const db::Region *, double>>(),
      db::Texts *values = 0) {
   ...
   *
   *  This version provides an additional area scale factor f, so the effective
   * area becomes
   *
   *    A(antenna) = A * f + P * t
   *
   *  where f is the area scale factor and t the perimeter scale factor. This
   * version allows to ignore the area contribution entirely and switch to a
   * perimeter-based antenna check by setting f to zero.
   *
...
private:
  ...
  std::list<std::pair<tl::GlobPattern, tl::GlobPattern>>
      m_joined_net_names_per_cell;
  std::list<std::set<std::string>> m_joined_nets;
  std::list<std::pair<tl::GlobPattern, std::set<std::string>>>
      m_joined_nets_per_cell;

  void init();
  void ensure_netlist();
  size_t search_net(const db::ICplxTrans &trans, const db::Cell *cell,
                    const db::local_cluster<NetShape> &test_cluster,
 ...
 *
 *  This object can be used to persist netlist builder information - e.g.
 * reusing net cells when building individual layers from nets. In this case,
 * build nets with a layer selection and call the build_net function many times.
 */
...
public:
  ...
  /**
   *  @brief Constructs a net builder with a target layout, a cell mapping table
   * and a LayoutToNetlist source
   *
   *  @param target The target layout
   *  @param cmap The cell mapping from the internal layout (inside
   * LayoutToNetlist) to the target - use LayoutInfo::cell_mapping_into to
   * generate this map
   *  @param source The LayoutToNetlist source.
   *
   *  A cell map needs to be supplied only if intending to build many nets in
   * hierarchical mode from multiple circuits.
   */
  NetBuilder(db::Layout *target, const db::CellMapping &cmap,
   ...
   *  @param source The LayoutToNetlist source.
   *
   *  This net builder can be used to build single nets into dedicated target
   * cells.
   */
   ...
   */
  void set_hier_mode(BuildNetHierarchyMode hm) { m_hier_mode = hm; }
   ...
   */
  void build_net(db::Cell &target_cell, const db::Net &net,
                 const std::map<unsigned int, const db::Region *> &lmap,
                 NetPropertyMode prop_mode,
                 const tl::Variant &netname_prop) const;

  /**
   *  @brief See \LayoutToNetlist for details of this function
   */
  void build_all_nets(const std::map<unsigned int, const db::Region *> &lmap,
                      NetPropertyMode prop_mode,
                      const tl::Variant &netname_prop) const;
  ...
  /**
   *  @brief A helper function to create a property ID for a given net, net
   * property name and net property mode
   */
  static db::properties_id_type
  make_netname_propid(db::PropertiesRepository &pr,
                      NetPropertyMode net_prop_mode,
                      const tl::Variant &netname_prop, const db::Net &net,
                      const std::string &net_name_prefix = std::string());

private:
  struct CellReuseTableKey {
    CellReuseTableKey(db::cell_index_type _cell_index,
                      db::properties_id_type _netname_propid,
                      size_t _cluster_id)
        : cell_index(_cell_index), netname_propid(_netname_propid),
  ...
  typedef std::map<CellReuseTableKey, std::pair<db::cell_index_type, bool>>
      cell_reuse_table_type;
  ...
  void build_net_rec(const db::Net &net, cell_index_type circuit_cell,
                     ...
                     db::properties_id_type netname_propid,
                     const ICplxTrans &tr) const;
  void build_net_rec(const db::Net &net, db::Cell &target_cell,
                     const std::map<unsigned int, const db::Region *> &lmap,
                     const std::string &add_net_cell_name_prefix,
                     db::properties_id_type netname_propid,
                     const ICplxTrans &tr) const;
  void build_net_rec(db::cell_index_type ci, size_t cid, db::Cell &target_cell,
                     const std::map<unsigned int, const db::Region *> &lmap,
                     const Net *net,
                     const std::string &add_net_cell_name_prefix,
                     db::properties_id_type netname_propid,
  ...
  db::Layout &target() const { return const_cast<db::Layout &>(*mp_target); }
};
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const LayoutToNetlist &x, bool no_self,
                     void *parent) {
