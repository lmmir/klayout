...
NetShape::NetShape() : m_ptr(0), m_dx(0), m_dy(0) {}
...
db::PolygonRef NetShape::polygon_ref() const {
  if ((size_t(m_ptr) & 1) != 0) {
    return db::PolygonRef(reinterpret_cast<db::Polygon *>(m_ptr - 1),
                          db::Disp(db::Vector(m_dx, m_dy)));
  }
  tl_assert(false);
}
...
db::TextRef NetShape::text_ref() const {
  if ((size_t(m_ptr) & 1) == 0) {
    return db::TextRef(reinterpret_cast<db::Text *>(m_ptr),
                       db::Disp(db::Vector(m_dx, m_dy)));
  }
  tl_assert(false);
}
...
void NetShape::insert_into(db::Shapes &shapes,
                           db::properties_id_type pi) const {
  if ((m_ptr & 1) != 0) {
...
bool NetShape::interacts_with(const db::NetShape &other) const {
  ...
  if ((m_ptr & 1) != 0) {
    ...
    if ((other.m_ptr & 1) != 0) {
      ...
      db::Polygon p = pr_other.obj().transformed(
          pr.trans().inverted() * pr_other.trans(), false);
...
template <class Tr>
bool NetShape::interacts_with_transformed(const db::NetShape &other,
                                          const Tr &trans) const {
  if (m_ptr == 0 || other.m_ptr == 0 ||
      !bbox().touches(other.bbox().transformed(trans))) {
    return false;
  }
  ...
  if ((m_ptr & 1) != 0) {
    ...
    if ((other.m_ptr & 1) != 0) {
      ...
      db::PolygonRef pr_other = other.polygon_ref();
      db::PolygonRef pr = polygon_ref();
      db::Polygon p = pr_other.obj().transformed(Tr(pr.trans().inverted()) *
                                                 trans * Tr(pr_other.trans()));
      return db::interact_pp(pr.obj(), p);
      ...
      db::PolygonRef pr = polygon_ref();
      db::Point pt =
          trans * db::Point(other.m_dx, other.m_dy) - pr.trans().disp();
      return db::inside_poly(pr.obj().begin_edge(), pt) >= 0;
    }
    ...
    if ((other.m_ptr & 1) == 0) {
      ...
      db::PolygonRef pr_other = other.polygon_ref();
      db::Point pt =
          trans.inverted() * db::Point(m_dx, m_dy) - pr_other.trans().disp();
      return db::inside_poly(pr_other.obj().begin_edge(), pt) >= 0;
    }
  }
}
...
template DB_PUBLIC bool NetShape::interacts_with_transformed<db::ICplxTrans>(
    const db::NetShape &other, const db::ICplxTrans &trans) const;
template DB_PUBLIC bool
NetShape::interacts_with_transformed<db::Trans>(const db::NetShape &other,
                                                const db::Trans &trans) const;
