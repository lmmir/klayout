 ...
 *  consists of two edges whose relationship is to be marked by objects in the
 * edge set. Depending on the origin of the edge pairs, the first and second
 * edge may be derived from one specific source, i.e. one region while the other
 * is derived from another source.
 *
 *  Edge pair sets are created by Region::width_check for example. Edge pair
 * sets can be converted to polygons or to individual edges.
 */
class DB_PUBLIC EdgePairs : public db::ShapeCollection {
public:
   ...
   */
  explicit EdgePairs(const db::EdgePair &s) : mp_delegate(0) { insert(s); }
   ...
   *  Creates an edge pair set representing a single instance of that object
   */
  explicit EdgePairs(const db::Shape &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Sequence constructor
   *
   *  Creates an edge set from a sequence of objects. The objects need to be
   * edge pairs. This version accepts iterators of the begin ... end style.
   */
  template <class Iter>
  explicit EdgePairs(const Iter &b, const Iter &e) : mp_delegate(0) {
    reserve(e - b);
    for (Iter i = b; i != e; ++i) {
      insert(*i);
    }
   ...
   *
   *  Creates an edge pair set from a recursive shape iterator. This allows one
   * to feed an edge pair set from a hierarchy of cells.
   */
   ...
   *
   *  Creates an edge pair set from a recursive shape iterator. This allows one
   * to feed an edge pair set from a hierarchy of cells. The transformation is
   * useful to scale to a specific DBU for example.
   */
  explicit EdgePairs(const RecursiveShapeIterator &si,
   ...
   * representation
   *
   *  This version will create a hierarchical edge pair collection. The
   ...
   */
  const EdgePairsDelegate *delegate() const { return mp_delegate; }

  /**
   *  @brief Gets the underlying delegate object
   */
  EdgePairsDelegate *delegate() { return mp_delegate; }
  ...
  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the edge pairs plus
  ...
  /**
   *  @brief Returns the number of (hierarchical) edge pairs in the edge pair
   * set
   */
   ...
   *
   *  nmax specifies how many edge pairs are included (set to
  ...
  void processed(Region &output,
                 const EdgePairToPolygonProcessorBase &filter) const;

  /**
   *  @brief Processes the edge pairs into edges
   *
   *  This method will run the processor over all edge pairs and return a edge
   * collection with the outputs of the processor.
   */
  void processed(Edges &output,
                 const EdgePairToEdgeProcessorBase &filter) const;

  /**
   *  @brief Transforms the edge pair set
   */
  template <class T> EdgePairs &transform(const T &trans);

  /**
   *  @brief Returns the transformed edge pair set
   */
  template <class T> EdgePairs transformed(const T &trans) const {
    EdgePairs d(*this);
    d.transform(trans);
    return d;
  }
   ...
   */
  void swap(db::EdgePairs &other) { std::swap(other.mp_delegate, mp_delegate); }
   ...
   *
   *  This method will return all edge pairs which are part of another edge pair
   * set. The match is done exactly. The "invert" flag can be used to invert the
   * sense, i.e. with "invert" set to true, this method will return all edge
   * pairs not in the other edge pair set.
   */
   ...
   */
  const db::EdgePair *nth(size_t n) const { return mp_delegate->nth(n); }
  ...
  /**
   *  @brief Returns true, if the edge pair set has valid edges stored within
   * itself
   *
   *  If the region has valid edges, it is permissable to use the edge pair's
   ...
   *  appended.
   *
   *  The given extension is applied to the edges in parallel and perpendicular
   * direction. This way a minimum extension is achieved which converts
   * vanishing edge pairs to polygons with an area.
   */
  ...
  /**
   *  @brief Inserts the edge pair collection into the given layout, cell and
   * layer If the edge pair collection is a hierarchical region, the hierarchy
   * is copied into the layout's hierarchy.
   */
  ...
  /**
   *  @brief Inserts the edge pair collection into the given layout, cell and
   * layer as polygons with the given enlargement If the edge pair collection is
   * a hierarchical region, the hierarchy is copied into the layout's hierarchy.
   */
  void insert_into_as_polygons(Layout *layout, db::cell_index_type into_cell,
                               unsigned int into_layer, db::Coord enl) const {
    return mp_delegate->insert_into_as_polygons(layout, into_cell, into_layer,
                                                enl);
  }
...
namespace tl {
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::EdgePairs &b);
