...
public:
  ...
  /**
   *  @brief Creates an error with a cell name and a message (the minimum
   * information)
   */
  NetlistDeviceExtractorError(const std::string &cell_name,
                              const std::string &msg);

  /**
   *  @brief The category name of the error
   *  Specifying the category name is optional. If a category is given, it will
   * be used for the report.
   */
  const std::string &category_name() const { return m_category_name; }

  /**
   *  @brief Sets the category name
   */
  void set_category_name(const std::string &s) { m_category_name = s; }

  /**
   *  @brief The category description of the error
   *  Specifying the category description is optional. If a category is given,
   * this attribute will be used for the category description.
   */
   ...
   */
  const db::DPolygon &geometry() const { return m_geometry; }

  /**
   *  @brief Sets the geometry
   */
  void set_geometry(const db::DPolygon &g) { m_geometry = g; }
   ...
   */
  void set_message(const std::string &n) { m_message = n; }

  /**
   *  @brief Gets the cell name the error occurred in
   */
  const std::string &cell_name() const { return m_cell_name; }

  /**
   *  @brief Sets the cell name
   */
  void set_cell_name(const std::string &n) { m_cell_name = n; }
...
public:
  NetlistDeviceExtractorLayerDefinition() : index(0) {
    //  .. nothing yet ..
  }

  NetlistDeviceExtractorLayerDefinition(const std::string &_name,
                                        const std::string &_description,
                                        size_t _index, size_t _fallback_index)
   ...
   *  This is the layer to be used when this layer isn't specified for input or
 ...
 */
class DB_PUBLIC NetlistDeviceExtractor : public gsi::ObjectBase,
                                         public tl::Object {
public:
  typedef std::list<db::NetlistDeviceExtractorError> error_list;
  typedef error_list::const_iterator error_iterator;
  typedef std::vector<db::NetlistDeviceExtractorLayerDefinition>
      layer_definitions;
   ...
   *
   *  layout and cell specify the layout and the top cell from which to perform
   * the extraction.
   *
   *  The netlist will be filled with circuits (unless not present yet) to
   * represent the cells from the layout.
   *
   *  Devices will be generated inside the netlist's circuits as they are
   * extracted from the layout. Inside the layout, device terminal annotation
   * shapes are created with the corresponding DeviceTerminalProperty objects
   * attached. The will be used when extracting the nets later to associate nets
   * with device terminals.
   *
   *  The definition of the input layers is device class specific.
   */
  void extract(Layout &layout, Cell &cell,
               const std::vector<unsigned int> &layers, Netlist *netlist,
   ...
   */
  void extract(DeepShapeStore &dss, unsigned int layout_index,
               const input_layers &layers, Netlist &netlist,
               hier_clusters_type &clusters, double device_scaling = 1.0);

  /**
   *  @brief Gets the error iterator, begin
   */
  error_iterator begin_errors() { return m_errors.begin(); }

  /**
   *  @brief Gets the error iterator, end
   */
  error_iterator end_errors() { return m_errors.end(); }
   ...
   *
   *  This method is supposed to set up the device extractor. This involves two
   * basic steps: defining the device classes and setting up the device layers.
   *
   *  Use "register_device_class" to register the device class you need.
   *
   *  The device layers need to be defined by calling "define_layer" once or
   * several times.
   */
  virtual void setup();
   ...
   *  This method shall raise an error, if the input layer are not properly
   * defined (e.g. too few etc.)
   */
   ...
   *
   *  The shape cluster is a set of geometries belonging together in terms of
   * the connectivity defined by "get_connectivity". The cluster might cover
   * multiple devices, so the implementation needs to consider this case. The
   * geometries are already merged.
   *
   *  The implementation of this method shall use "create_device" to create new
   *  devices based on the geometry found. It shall use "define_terminal" to
   * define terminals by which the nets extracted in the network extraction step
   * connect to the new devices.
   */
  virtual void extract_devices(const std::vector<db::Region> &layer_geometry);

  /**
   *  @brief Registers a device class
   *  The device class object will become owned by the netlist and must not be
   * deleted by the caller. The name of the device class will be changed to the
   * name given to the device extractor. This method shall be used inside the
   * implementation of "setup" to register the device classes.
   */
   ...
   * "extract_devices" in the order the layers are defined.
   */
  const db::NetlistDeviceExtractorLayerDefinition &
  define_layer(const std::string &name,
               const std::string &description = std::string());

  /**
   *  @brief Defines a layer with a fallback layer
   *  Like "define_layer" without fallback layer, but will fall back to the
   * given layer (by index) if this layer isn't specified for input or terminal
   * markup.
   */
  const db::NetlistDeviceExtractorLayerDefinition &
  define_layer(const std::string &name, size_t fallback,
               const std::string &description = std::string());

  /**
   *  @brief Creates a device
   *  The device object returned can be configured by the caller, e.g. set
   * parameters. It will be owned by the netlist and must not be deleted by the
   * caller.
   */
   ...
   *
   *  This member is set in 'extract_devices' and holds the device class object
   * used during extraction.
   */
  DeviceClass *device_class() { return mp_device_class.get(); }

  /**
   *  @brief Defines a device terminal in the layout (a region)
   */
  void define_terminal(Device *device, size_t terminal_id, size_t layer_index,
   ...
   */
  double dbu() const { return mp_layout->dbu(); }

  /**
   *  @brief Gets the scaled database unit
   *  Use this unit to compute device properties. It is the database unit
   * multiplied with the device scaling factor.
   */
  double sdbu() const { return m_device_scaling * mp_layout->dbu(); }
  ...
  /**
   *  @brief Issues an error with the given category name, description and
   * message
   */
  void error(const std::string &category_name,
             const std::string &category_description, const std::string &msg);

  /**
   *  @brief Issues an error with the given category name, description and
   * message and error shape
   */
  void error(const std::string &category_name,
             const std::string &category_description, const std::string &msg,
   ...
   *  This method will produce the device classes required for the device
   * extraction. It is mainly provided for test purposes. Don't call it
   * directly.
   */
...
private:
  ...
  typedef std::map<unsigned int, std::vector<db::NetShape>>
      geometry_per_layer_type;
  typedef std::map<size_t, geometry_per_layer_type> geometry_per_terminal_type;

  tl::weak_ptr<db::Netlist> m_netlist;
  db::Layout *mp_layout;
  db::properties_id_type m_terminal_id_propname_id, m_device_id_propname_id,
  ...
  error_list m_errors;
  std::map<size_t, std::pair<db::Device *, geometry_per_terminal_type>>
      m_new_devices;
  std::map<DeviceCellKey, std::pair<db::cell_index_type, db::DeviceAbstract *>>
      m_device_cells;
  ...
  void extract_without_initialize(
  ...
  void push_cached_devices(const tl::vector<Device *> &cached_devices,
                           const db::Vector &disp_cache,
                           const db::Vector &new_disp);
};
