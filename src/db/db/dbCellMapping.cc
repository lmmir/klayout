...
// -------------------------------------------------------------------------------------
//  Some utility class: an iterator for cell instances delivered sorted by cell
//  index
...
struct SortedCellIndexIterator {
  ...
  SortedCellIndexIterator() : mp_cell(0), m_n(0) {
    // .. nothing yet ..
  }
  ...
  bool operator<(const SortedCellIndexIterator &d) const { return m_n < d.m_n; }

  db::Instance instance() const { return mp_cell->sorted_inst_ptr(m_n); }
...
// -------------------------------------------------------------------------------------
//  Some utility class: a compare function for a instance set of two cells in
//  the context of two layouts and two initial cells.

class InstanceSetCompareFunction {
public:
  typedef std::multiset<db::ICplxTrans, db::trans_less_func<db::ICplxTrans>>
      trans_set_t;

  InstanceSetCompareFunction(const db::Layout &layout_a,
                             db::cell_index_type initial_cell_a,
                             const db::Layout &layout_b,
                             db::cell_index_type initial_cell_b)
  ...
  bool compare(db::cell_index_type cell_a,
               const std::set<db::cell_index_type> &selection_cone_a,
               db::cell_index_type cell_b,
               const std::set<db::cell_index_type> &selection_cone_b) {
    if (cell_a != m_cell_a) {

      m_cell_a = cell_a;

      m_callers_a.clear();
      m_layout_a.cell(cell_a).collect_caller_cells(m_callers_a,
                                                   selection_cone_a, -1);
      m_callers_a.insert(cell_a);

      m_trans.clear();
      insert(m_layout_a, m_initial_cell_a, m_cell_a, m_callers_a, m_trans,
             db::ICplxTrans());
    }

    m_repr_set = false;

    std::map<db::cell_index_type, db::ICplxTrans>::const_iterator r =
        m_repr.find(cell_b);
    if (r != m_repr.end()) {
      m_repr_set = true;
    ...
    m_layout_b.cell(cell_b).collect_caller_cells(callers_b, selection_cone_b,
                                                 -1);
    ...
    if (!compare(m_layout_b, m_initial_cell_b, cell_b, callers_b, trans,
                 db::ICplxTrans(mag), db::ICplxTrans(1.0 / mag))) {
      return false;
    }
...
private:
  ...
  void insert(const db::Layout &layout, db::cell_index_type current_cell,
              db::cell_index_type cell,
              const std::set<db::cell_index_type> &cone, trans_set_t &trans,
              const db::ICplxTrans &current_trans) {
    if (current_cell == cell) {
      ...
      for (std::set<db::cell_index_type>::const_iterator c = cone.begin();
           c != cone.end() && i != end; ++c) {
        if (*i <= *c) {
          for (i = std::lower_bound(i, end, *c); i != end && *i == *c; ++i) {
            for (db::CellInstArray::iterator arr = i.instance().begin();
                 !arr.at_end(); ++arr) {
              insert(layout, *c, cell, cone, trans,
                     current_trans * i.instance().complex_trans(*arr));
            }
          }
        }
      }
  ...
  bool compare(const db::Layout &layout, db::cell_index_type current_cell,
               db::cell_index_type cell,
               const std::set<db::cell_index_type> &cone, trans_set_t &trans,
               const db::ICplxTrans &current_trans,
               const db::ICplxTrans &local_trans) {
    if (current_cell == cell) {
      ...
      for (std::set<db::cell_index_type>::const_iterator c = cone.begin();
        ...
        if (*i <= *c) {
          for (i = std::lower_bound(i, end, *c); i != end && *i == *c; ++i) {
            for (db::CellInstArray::iterator arr = i.instance().begin();
                 !arr.at_end(); ++arr) {
              if (!compare(layout, *c, cell, cone, trans,
                           current_trans * i.instance().complex_trans(*arr),
                           local_trans)) {
                return false;
              }
...
void CellMapping::clear() { m_b2a_mapping.clear(); }
...
void CellMapping::create_single_mapping(const db::Layout & /*layout_a*/,
                                        db::cell_index_type cell_index_a,
                                        const db::Layout & /*layout_b*/,
                                        db::cell_index_type cell_index_b) {
  clear();
  map(cell_index_b, cell_index_a);
}

void CellMapping::create_multi_mapping(
    const db::Layout & /*layout_a*/,
    const std::vector<db::cell_index_type> &cell_index_a,
    const db::Layout & /*layout_b*/,
    const std::vector<db::cell_index_type> &cell_index_b) {
  clear();
  if (cell_index_a.size() != cell_index_b.size()) {
    throw tl::Exception(
        tl::to_string(tr("cell index arrays for A and B cells must have same "
                         "length in 'create_multi_mapping'")));
  }
  for (std::vector<db::cell_index_type>::const_iterator
           ia = cell_index_a.begin(),
           ib = cell_index_b.begin();
       ia != cell_index_a.end(); ++ia, ++ib) {
    map(*ib, *ia);
  }
}

void CellMapping::create_from_names(const db::Layout &layout_a,
                                    db::cell_index_type cell_index_a,
                                    const db::Layout &layout_b,
                                    db::cell_index_type cell_index_b) {
  clear();
  ...
  for (std::set<db::cell_index_type>::const_iterator b = called_b.begin();
       b != called_b.end(); ++b) {
    std::pair<bool, db::cell_index_type> ac =
        layout_a.cell_by_name(layout_b.cell_name(*b));
    if (ac.first) {
      map(*b, ac.second);
    }
  }
...
std::vector<db::cell_index_type> CellMapping::create_missing_mapping(
    db::Layout &layout_a, const db::Layout &layout_b,
    const std::vector<db::cell_index_type> &cell_index_b,
    const std::set<db::cell_index_type> *exclude_cells,
    const std::set<db::cell_index_type> *include_cells) {
  std::vector<db::cell_index_type> new_cells;
  do_create_missing_mapping(layout_a, layout_b, cell_index_b, exclude_cells,
                            include_cells, &new_cells, 0);
  return new_cells;
}
...
CellMapping::create_missing_mapping2(
  ...
  do_create_missing_mapping(layout_a, layout_b, cell_index_b, exclude_cells,
                            include_cells, 0, &cell_pairs);
  return cell_pairs;
}

void CellMapping::do_create_missing_mapping(
    db::Layout &layout_a, const db::Layout &layout_b,
    const std::vector<db::cell_index_type> &cell_index_b,
    const std::set<db::cell_index_type> *exclude_cells,
    const std::set<db::cell_index_type> *include_cells,
    std::vector<db::cell_index_type> *new_cells_ptr,
    std::vector<std::pair<db::cell_index_type, db::cell_index_type>>
        *mapped_pairs) {
  ...
  if (!new_cells.empty()) {
    ...
    //  Note: this avoids frequent cell index table rebuilds if source and
    //  target layout are identical
    layout_a.start_changes();

    //  Create instances for the new cells in layout A according to their
    //  instantiation in layout B
    double mag = layout_b.dbu() / layout_a.dbu();
    for (size_t i = 0; i < new_cells.size(); ++i) {

      const db::Cell &b = layout_b.cell(new_cells_b[i]);
      for (db::Cell::parent_inst_iterator pb = b.begin_parent_insts();
           !pb.at_end(); ++pb) {
...
void CellMapping::create_from_geometry(const db::Layout &layout_a,
                                       db::cell_index_type cell_index_a,
                                       const db::Layout &layout_b,
                                       db::cell_index_type cell_index_b) {
  ...
  if (tl::verbosity() >= 40) {
    tl::info << "Cell mapping - first step: mapping instance count and "
                "instance identity";
  }
  ...
  for (db::CellCounter::selection_iterator c = cc_b.begin(); c != cc_b.end();
       ++c) {
    cm_b.insert(std::make_pair(*c == cell_index_b ? 0 : cc_b.weight(*c), *c));
  }

  std::multimap<size_t, db::cell_index_type> cm_a;
  for (db::CellCounter::selection_iterator c = cc_a.begin(); c != cc_a.end();
       ++c) {
    cm_a.insert(std::make_pair(*c == cell_index_a ? 0 : cc_a.weight(*c), *c));
  }

  std::map<db::cell_index_type, std::vector<db::cell_index_type>>
      candidates; // key = index(a), value = indices(b)

  InstanceSetCompareFunction cmp(layout_a, cell_index_a, layout_b,
                                 cell_index_b);

  std::multimap<size_t, db::cell_index_type>::const_iterator a = cm_a.begin(),
                                                             b = cm_b.begin();
  while (a != cm_a.end() && b != cm_b.end()) {
    ...
    if (b == cm_b.end()) {
      ...
      candidates.insert(
          std::make_pair(a->second, std::vector<db::cell_index_type>()));
      ++a;
    } else {

      if (tl::verbosity() >= 50) {
        size_t na = 0, nb = 0;
        for (std::multimap<size_t, db::cell_index_type>::const_iterator aa = a;
             aa != cm_a.end() && aa->first == w; ++aa) {
          ++na;
        }
        for (std::multimap<size_t, db::cell_index_type>::const_iterator bb = b;
             bb != cm_b.end() && bb->first == w; ++bb) {
          ++nb;
        }
        tl::info << "Multiplicity group (" << w << " instances) - " << na
                 << " vs. " << nb << " cells";
      }
      ...
      while (a != cm_a.end() && a->first == w) {
        ...
        while (bb != cm_b.end() && bb->first == w) {
          ...
          std::map<db::cell_index_type, unsigned int>::const_iterator bg =
              b_group_of_cell.find(bb->second);
          if (bg != b_group_of_cell.end()) {

            if (groups_taken.find(bg->second) == groups_taken.end()) {
              if (cmp.compare(a->second, cc_a.selection(), bb->second,
                              cc_b.selection())) {
            ...
            if (cmp.compare(a->second, cc_a.selection(), bb->second,
              ...
              b_group
                  .insert(std::make_pair(g, std::vector<db::cell_index_type>()))
                  .first->second.push_back(bb->second);
            }
          }
        ...
        if (tl::verbosity() >= 60) {
          tl::info << "Checked cell " << layout_a.cell_name(a->second) << ": "
                   << candidates[a->second].size() << " candidates remaining.";
        }
  ...
  for (std::map<db::cell_index_type,
                std::vector<db::cell_index_type>>::const_iterator cand =
           candidates.begin();
  ...
  while (reduction) {
    ...
    if (tl::verbosity() >= 40) {
      tl::info << "Cell mapping - iteration " << iteration
               << ": cross-instance cone reduction";
    }

    // This map stores that layout_b cells with the corresponding layout_a cell
    // for such cells which have their mapping reduced to a unique one
    std::map<db::cell_index_type, std::pair<db::cell_index_type, int>>
        unique_candidates;

    std::vector<db::cell_index_type> refined_cand;

    for (std::map<db::cell_index_type,
                  std::vector<db::cell_index_type>>::iterator cand =
             candidates.begin();
         cand != candidates.end(); ++cand) {
      ...
      if (cand->second.size() > 1) {
        ...
        refined_cand.clear();
        refined_cand.insert(refined_cand.end(), cand->second.begin(),
                            cand->second.end());
        ...
        if (tl::verbosity() >= 70) {
          ...
          for (size_t i = 0; i < refined_cand.size(); ++i) {
            tl::info << " " << layout_b.cell_name(refined_cand[i])
                     << tl::noendl;
          }
          tl::info << "";
        }
        ...
        layout_a.cell(cand->first)
            .collect_caller_cells(callers, cc_a.selection(), -1);

        for (std::set<db::cell_index_type>::const_iterator c = callers.begin();
             c != callers.end() && refined_cand.size() > 0; ++c) {

          if (*c != cell_index_a) {

            const std::vector<db::cell_index_type> &others =
                candidates.find(*c)->second;
            if (others.size() == 1) {

              std::set<db::cell_index_type> cross_cone_b;
              layout_b.cell(others.front()).collect_called_cells(cross_cone_b);

              std::vector<db::cell_index_type>::iterator cout =
                  refined_cand.begin();
              for (std::vector<db::cell_index_type>::const_iterator cc =
                       refined_cand.begin();
                   cc != refined_cand.end(); ++cc) {
              ...
              if (tl::verbosity() >= 70 && cout != refined_cand.end()) {
                tl::info << "Reduction because of caller mapping: "
                         << layout_a.cell_name(*c) << " <-> "
                         << layout_b.cell_name(others[0]);
                tl::info << "  -> " << tl::noendl;
                for (size_t i = 0; i < size_t(cout - refined_cand.begin());
                     ++i) {
                  tl::info << " " << layout_b.cell_name(refined_cand[i])
                           << tl::noendl;
                }
        ...
        if (refined_cand.size() > 0) {
          ...
          for (std::set<db::cell_index_type>::const_iterator c = called.begin();
               c != called.end() && refined_cand.size() > 0; ++c) {
            ...
            if (others.size() == 1) {
              ...
              std::set<db::cell_index_type> cross_cone_b;
              layout_b.cell(others.front())
                  .collect_caller_cells(cross_cone_b, cc_b.selection(), -1);
              ...
              if (tl::verbosity() >= 70 && cout != refined_cand.end()) {
                tl::info << "Reduction because of callee mapping: "
                         << layout_a.cell_name(*c) << " <-> "
                         << layout_b.cell_name(others[0]);
                tl::info << "  -> " << tl::noendl;
                for (size_t i = 0; i < size_t(cout - refined_cand.begin());
          ...
          if (refined_cand.size() == 1) {
            ...
            //  The remaining cell is a candidate for layout_b to layout_a
            //  mapping
            db::cell_index_type cb = refined_cand[0];
            db::cell_index_type ca = cand->first;

            std::map<db::cell_index_type,
                     std::pair<db::cell_index_type, int>>::iterator uc =
                unique_candidates.find(cb);
            if (uc != unique_candidates.end()) {
              if (uc->second.first != ca) {
                int ed = tl::edit_distance(layout_a.cell_name(ca),
                                           layout_b.cell_name(cb));
                if (ed < uc->second.second) {
                  uc->second = std::make_pair(ca, ed);
                  if (tl::verbosity() >= 60) {
                    tl::info << "Choosing " << layout_b.cell_name(cb)
                             << " (layout_b) as new unique mapping for "
                             << layout_a.cell_name(ca) << " (layout_a)";
                  }
                }
              }
            } else {
              ...
              unique_candidates.insert(
                  std::make_pair(cb, std::make_pair(ca, ed)));
              if (tl::verbosity() >= 60) {
                tl::info << "Choosing " << layout_b.cell_name(cb)
                         << " (layout_b) as unique mapping for "
                         << layout_a.cell_name(ca) << " (layout_a)";
              }
            }
          }
    ...
    for (std::map<db::cell_index_type,
                  std::pair<db::cell_index_type, int>>::const_iterator uc =
             unique_candidates.begin();
         uc != unique_candidates.end(); ++uc) {

      std::map<db::cell_index_type, std::vector<db::cell_index_type>>::iterator
          cand = candidates.find(uc->second.first);
    ...
    if (tl::verbosity() >= 40) {
      tl::info << "Cell mapping - iteration " << iteration
               << ": removal of uniquely mapped cells on B side";
    }
    ...
    for (std::map<db::cell_index_type,
      ...
      if (cand->second.size() > 1) {
        ...
        std::vector<db::cell_index_type> refined_cand;
        for (std::vector<db::cell_index_type>::const_iterator c =
                 cand->second.begin();
             c != cand->second.end(); ++c) {
          std::map<db::cell_index_type, db::cell_index_type>::const_iterator
              um = m_b2a_mapping.find(*c);
  ...
  if (tl::verbosity() >= 40) {
    ...
    tl::info << "  Non unique = " << non_unique << " (total " << alternatives
             << " of alternatives)";
  }
  ...
  for (std::map<db::cell_index_type, std::vector<db::cell_index_type>>::iterator
...
void CellMapping::extract_unique(
    std::map<db::cell_index_type,
             std::vector<db::cell_index_type>>::const_iterator cand,
  ...
  if (cand->second.size() == 1) {
    ...
    if (tl::verbosity() >= 40) {
      tl::info << "  (U) " << layout_a.cell_name(cand->first) << " -> "
               << layout_b.cell_name(cand->second.front()) << " ("
               << cand->first << " -> " << cand->second.front() << ")";
    }
    unique_mapping.insert(std::make_pair(cand->second.front(), cand->first));

  } else if (tl::verbosity() >= 50) {

    tl::info << "      " << layout_a.cell_name(cand->first) << " ->"
             << tl::noendl;
    int n = 5;
    for (std::vector<db::cell_index_type>::const_iterator c =
             cand->second.begin();
         c != cand->second.end() && --n > 0; ++c) {
...
CellMapping::cell_mapping_pair(db::cell_index_type cell_index_b) const {
  std::map<db::cell_index_type, db::cell_index_type>::const_iterator m =
      m_b2a_mapping.find(cell_index_b);
