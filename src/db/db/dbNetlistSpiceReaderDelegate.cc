...
void NetlistSpiceReaderDelegate::do_start() { start(mp_netlist); }

void NetlistSpiceReaderDelegate::do_finish() { finish(mp_netlist); }
...
bool NetlistSpiceReaderDelegate::control_statement(
    const std::string & /*line*/) {
  return false;
}

bool NetlistSpiceReaderDelegate::wants_subcircuit(
    const std::string & /*circuit_name*/) {
  return false;
}

std::string
NetlistSpiceReaderDelegate::translate_net_name(const std::string &nn) {
...
template <class Cls>
static db::DeviceClass *make_device_class(db::Circuit *circuit,
...
void NetlistSpiceReaderDelegate::parse_element_components(
    const std::string &s, std::vector<std::string> &strings,
    std::map<std::string, tl::Variant> &pv,
  ...
  while (!ex.at_end()) {
    ...
    if (ex.test_without_case("params:")) {
      ...
      if (ex.try_read_word(n) && ex.test("=")) {
        ...
        //  a parameter
        std::string pn =
            mp_netlist ? mp_netlist->normalize_name(n) : tl::to_upper_case(n);
        ...
        ex = ex0;
        if (in_params) {
          ex.error(tl::to_string(tr("Invalid syntax for parameter assignment - "
                                    "needs keyword followed by '='")));
        }

        std::string comp_name = parse_component(ex);
        comp_name = mp_netlist ? mp_netlist->normalize_name(comp_name)
                               : tl::to_upper_case(comp_name);
...
void NetlistSpiceReaderDelegate::def_values_per_element(
    const std::string &element, std::map<std::string, tl::Variant> &pv) {
  if (element == "M") {
...
void NetlistSpiceReaderDelegate::parse_element(
    const std::string &s, const std::string &element, std::string &model,
    double &value, std::vector<std::string> &nn,
    std::map<std::string, tl::Variant> &pv,
    const std::map<std::string, tl::Variant> &variables) {
  ...
  if (element == "X") {
    ...
    if (nn.size() < 2) {
      error(tl::to_string(
          tr("Not enough specs (nodes, value, model) for a R, C or L device")));
    } else if (nn.size() > 5) {
      error(tl::to_string(
          tr("Too many specs (nodes, value, model) for a R, C or L device")));
    }
    ...
    //      Cname n1 n2 n3 C=value [other params] -> not supported, cannot tell
    //      from (3) without further analysis
    //  (4) Cname n1 n2 model value [params]
    //      Cname n1 n2 n3 value [params] -> not supported, cannot tell from (4)
    ...
    if (nn.empty()) {
      error(tl::sprintf(
          tl::to_string(tr("No model name given for element '%s'")), element));
    }
...
bool NetlistSpiceReaderDelegate::element(
    db::Circuit *circuit, const std::string &element, const std::string &name,
    const std::string &model, double value, const std::vector<db::Net *> &nets,
    const std::map<std::string, tl::Variant> &pv) {
  ...
  if (mult < 1e-10) {
    error(tl::sprintf(tl::to_string(tr("Invalid multiplier value (M=%.12g) - "
  ...
  if (element == "R") {
    ...
    if (nets.size() == 2) {
      if (cls) {
        if (!dynamic_cast<db::DeviceClassResistor *>(cls)) {
          error(
              tl::sprintf(tl::to_string(tr("Class %s is not a resistor device "
                                           "class as required by 'R' element")),
                          cn));
        }
      } else {
        if (cn.empty()) {
      ...
      if (cls) {
        if (!dynamic_cast<db::DeviceClassResistorWithBulk *>(cls)) {
          error(tl::sprintf(
              tl::to_string(tr("Class %s is not a three-terminal resistor "
                               "device class as required by 'R' element")),
              cn));
        }
      } else {
        if (cn.empty()) {
    ...
    for (size_t i = 0; i < sizeof(scale_params) / sizeof(scale_params[0]);
         ++i) {
    ...
    if (nets.size() == 2) {
      if (cls) {
        if (!dynamic_cast<db::DeviceClassInductor *>(cls)) {
          error(
              tl::sprintf(tl::to_string(tr("Class %s is not a inductor device "
    ...
    if (nets.size() == 2) {
      if (cls) {
        if (!dynamic_cast<db::DeviceClassCapacitor *>(cls)) {
          error(
              tl::sprintf(tl::to_string(tr("Class %s is not a capacitor device "
                                           "class as required by 'C' element")),
                          cn));
        }
      } else {
        if (cn.empty()) {
      ...
      if (cls) {
        if (!dynamic_cast<db::DeviceClassCapacitorWithBulk *>(cls)) {
          error(tl::sprintf(
              tl::to_string(tr("Class %s is not a three-terminal capacitor "
                               "device class as required by 'C' element")),
              cn));
        }
      } else {
        if (cn.empty()) {
    ...
    if (cls) {
      if (!dynamic_cast<db::DeviceClassDiode *>(cls)) {
        error(tl::sprintf(tl::to_string(tr("Class %s is not a diode device "
                                           "class as required by 'D' element")),
                          cn));
      }
    } else {
      if (cn.empty()) {
    ...
    if (nets.size() != 3 && nets.size() != 4) {
      ...
      if (nets.size() == 3) {
        if (!dynamic_cast<db::DeviceClassBJT3Transistor *>(cls)) {
          error(tl::sprintf(
              tl::to_string(tr("Class %s is not a 3-terminal BJT device class "
                               "as required by 'Q' element")),
              cn));
        }
      } else {
        if (!dynamic_cast<db::DeviceClassBJT4Transistor *>(cls)) {
          error(tl::sprintf(
              tl::to_string(tr("Class %s is not a 4-terminal BJT device class "
                               "as required by 'Q' element")),
              cn));
        }
      }
    ...
    if (cls) {
      if (!dynamic_cast<db::DeviceClassMOS4Transistor *>(cls)) {
        error(tl::sprintf(
            tl::to_string(tr("Class %s is not a 4-terminal MOS device class as "
                             "required by 'M' element")),
            cn));
      }
    } else {
      if (nets.size() == 4) {
    ...
    error(tl::sprintf(tl::to_string(tr("Not a known element type: '%s'")),
                      element));
  }
  ...
  if (td.size() != nets.size()) {
    error(tl::sprintf(tl::to_string(tr("Wrong number of terminals: class '%s' "
                                       "expects %d, but %d are given")),
                      cn, int(td.size()), int(nets.size())));
  }
  ...
  std::vector<db::DeviceParameterDefinition> &pd =
      cls->parameter_definitions_non_const();
  for (std::vector<db::DeviceParameterDefinition>::iterator i = pd.begin();
       i != pd.end(); ++i) {
...
void NetlistSpiceReaderDelegate::apply_parameter_scaling(
    db::Device *device) const {
  ...
  for (auto i = pd.begin(); i != pd.end(); ++i) {
    ...
    device->set_parameter_value(
        i->id(),
        pv / i->si_scaling() * pow(m_options.scale, i->geo_scaling_exponent()));
  }
}

tl::Variant NetlistSpiceReaderDelegate::read_value(
    tl::Extractor &ex, const std::map<std::string, tl::Variant> &variables) {
  NetlistSpiceReaderExpressionParser parser(&variables);
  return parser.read(ex);
}

tl::Variant NetlistSpiceReaderDelegate::read_value(
    tl::Extractor &ex, const std::map<std::string, tl::Variant> &variables1,
    const std::map<std::string, tl::Variant> &variables2) {
  NetlistSpiceReaderExpressionParser parser(&variables1, &variables2);
  return parser.read(ex);
}

bool NetlistSpiceReaderDelegate::try_read_value(
