...
void FlatEdgePairs::invalidate_cache() { invalidate_bbox(); }
...
FlatEdgePairs::begin_iter() const {
  return std::make_pair(db::RecursiveShapeIterator(*mp_edge_pairs),
                        db::ICplxTrans());
}

bool FlatEdgePairs::empty() const { return mp_edge_pairs->empty(); }

size_t FlatEdgePairs::count() const { return mp_edge_pairs->size(); }

size_t FlatEdgePairs::hier_count() const { return mp_edge_pairs->size(); }

Box FlatEdgePairs::compute_bbox() const { return mp_edge_pairs->bbox(); }
...
FlatEdgePairs::filter_in_place(const EdgePairFilterBase &filter) {
  ...
  edge_pair_iterator_type pw =
      ep.get_layer<db::EdgePair, db::unstable_layer_tag>().begin();
  ...
  ep.get_layer<db::EdgePair, db::unstable_layer_tag>().erase(
      pw, ep.get_layer<db::EdgePair, db::unstable_layer_tag>().end());
...
EdgePairsDelegate *FlatEdgePairs::add_in_place(const EdgePairs &other) {
  ...
  db::PropertyMapper pm(properties_repository(),
                        &other.properties_repository());
...
const db::EdgePair *FlatEdgePairs::nth(size_t n) const {
  return n < mp_edge_pairs->size()
             ? &mp_edge_pairs->get_layer<db::EdgePair, db::unstable_layer_tag>()
                    .begin()[n]
             : 0;
}

bool FlatEdgePairs::has_valid_edge_pairs() const { return true; }

const db::RecursiveShapeIterator *FlatEdgePairs::iter() const { return 0; }

void FlatEdgePairs::apply_property_translator(
...
void FlatEdgePairs::insert_into_as_polygons(Layout *layout,
  ...
  for (EdgePairsIterator p(begin()); !p.at_end(); ++p) {
    ...
    if (prop_id != 0) {
      out.insert(db::SimplePolygonWithProperties(
          p->normalized().to_simple_polygon(enl), prop_id));
    } else {
...
void FlatEdgePairs::insert_into(Layout *layout, db::cell_index_type into_cell,
...
void FlatEdgePairs::do_insert(const db::EdgePair &ep) {
