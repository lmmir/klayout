...
void FlatTexts::invalidate_cache() { invalidate_bbox(); }

void FlatTexts::reserve(size_t n) { mp_texts->reserve(db::Text::tag(), n); }
...
FlatTexts::begin_iter() const {
  return std::make_pair(db::RecursiveShapeIterator(*mp_texts),
                        db::ICplxTrans());
}

bool FlatTexts::empty() const { return mp_texts->empty(); }

size_t FlatTexts::count() const { return mp_texts->size(); }

size_t FlatTexts::hier_count() const { return mp_texts->size(); }

Box FlatTexts::compute_bbox() const { return mp_texts->bbox(); }

TextsDelegate *FlatTexts::filter_in_place(const TextFilterBase &filter) {
  db::Shapes &texts = *mp_texts;

  text_iterator_type pw =
      texts.get_layer<db::Text, db::unstable_layer_tag>().begin();
  ...
  texts.get_layer<db::Text, db::unstable_layer_tag>().erase(
      pw, texts.get_layer<db::Text, db::unstable_layer_tag>().end());
...
TextsDelegate *FlatTexts::add(const Texts &other) const {
  ...
  if (other_flat) {
    ...
    new_texts->raw_texts().insert(
        other_flat->raw_texts()
            .get_layer<db::Text, db::unstable_layer_tag>()
            .begin(),
        other_flat->raw_texts()
            .get_layer<db::Text, db::unstable_layer_tag>()
            .end());
...
TextsDelegate *FlatTexts::add_in_place(const Texts &other) {
  ...
  if (other_flat) {
    ...
    texts.insert(other_flat->raw_texts()
                     .get_layer<db::Text, db::unstable_layer_tag>()
                     .begin(),
                 other_flat->raw_texts()
                     .get_layer<db::Text, db::unstable_layer_tag>()
...
const db::Text *FlatTexts::nth(size_t n) const {
  return n < mp_texts->size()
             ? &mp_texts->get_layer<db::Text, db::unstable_layer_tag>()
                    .begin()[n]
             : 0;
}

bool FlatTexts::has_valid_texts() const { return true; }

const db::RecursiveShapeIterator *FlatTexts::iter() const { return 0; }
...
void FlatTexts::insert_into_as_polygons(Layout *layout,
...
void FlatTexts::insert_into(Layout *layout, db::cell_index_type into_cell,
                            unsigned int into_layer) const {
  layout->cell(into_cell).shapes(into_layer).insert(*mp_texts);
}

void FlatTexts::do_insert(const db::Text &t) {
  mp_texts->insert(t);
  invalidate_cache();
}
