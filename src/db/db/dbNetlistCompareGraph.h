...
public:
  Transition(const db::Device *device, size_t device_category,
             size_t terminal1_id, size_t terminal2_id);
  Transition(const db::SubCircuit *subcircuit, size_t subcircuit_category,
             size_t pin1_id, size_t pin2_id);
  ...
  const db::Device *device() const { return (const db::Device *)m_ptr; }

  const db::SubCircuit *subcircuit() const {
    return (const db::SubCircuit *)m_ptr;
  }

  size_t cat() const { return m_cat; }

  size_t id1() const { return m_id1; }

  size_t id2() const { return m_id2; }
...
public:
  typedef std::pair<std::vector<Transition>, std::pair<size_t, const db::Net *>>
      edge_type;
  ...
  struct EdgeToEdgeOnlyCompare {
    bool operator()(const edge_type &a,
                    const std::vector<Transition> &b) const {
      return a.first < b;
    }
  };
  ...
  NetGraphNode() : mp_net(0), m_other_net_index(invalid_id) {
    //  .. nothing yet ..
  }
   ...
   */
  NetGraphNode(const db::Net *net, DeviceCategorizer &device_categorizer,
               CircuitCategorizer &circuit_categorizer,
               const DeviceFilter &device_filter,
   ...
   */
  NetGraphNode(const db::SubCircuit *sc,
               CircuitCategorizer &circuit_categorizer,
               const std::map<const db::Circuit *, CircuitMapper> *circuit_map,
               const CircuitPinCategorizer *pin_map, size_t *unique_pin_id);
  ...
  const db::Net *net() const { return mp_net; }

  bool has_other() const {
    return m_other_net_index != invalid_id && m_other_net_index != unknown_id;
  }

  bool has_any_other() const { return m_other_net_index != invalid_id; }

  bool has_unknown_other() const { return m_other_net_index == unknown_id; }
  ...
  void unset_other_net() { m_other_net_index = invalid_id; }
  ...
  bool operator==(const NetGraphNode &node) const { return equal(node, false); }

  bool operator<(const NetGraphNode &node) const { return less(node, false); }
  ...
  edge_iterator begin() const { return m_edges.begin(); }

  edge_iterator end() const { return m_edges.end(); }

  edge_iterator find_edge(const std::vector<Transition> &edge) const {
    edge_iterator res =
        std::lower_bound(begin(), end(), edge, EdgeToEdgeOnlyCompare());
  ...
  std::vector<edge_type> &edges() { return m_edges; }
...
namespace std {
inline void swap(db::NetGraphNode &a, db::NetGraphNode &b) { a.swap(b); }
} // namespace std
...
public:
   ...
   */
  void build(const db::Circuit *c, DeviceCategorizer &device_categorizer,
             CircuitCategorizer &circuit_categorizer,
   ...
   */
  db::NetGraphNode &node(size_t net_index) { return m_nodes[net_index]; }
  ...
  const db::NetGraphNode &virtual_node(const db::SubCircuit *sc) const {
    std::map<const db::SubCircuit *, db::NetGraphNode>::const_iterator j =
        m_virtual_nodes.find(sc);
  ...
  db::NetGraphNode &virtual_node(const db::SubCircuit *sc) {
    return const_cast<db::NetGraphNode &>(
        ((const NetGraph *)this)->virtual_node(sc));
  }
  ...
  /**
   *  @brief Establishes an equivalence between two nodes of netlist A (this)
   * and B (other)
   */
   ...
   */
  void unidentify(size_t net_index) { m_nodes[net_index].unset_other_net(); }

  /**
   *  @brief Iterator over the nodes in this graph (begin)
   */
  node_iterator begin() const { return m_nodes.begin(); }

  /**
   *  @brief Iterator over the nodes in this graph (end)
   */
  node_iterator end() const { return m_nodes.end(); }

  /**
   *  @brief The circuit this graph is associated with
   */
  const db::Circuit *circuit() const { return mp_circuit; }
