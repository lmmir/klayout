...
struct DB_PUBLIC_TEMPLATE dereferencing_iterator : public Iter {
public:
 ...
 *  devices.
 */
class DB_PUBLIC Circuit : public db::NetlistObject, public gsi::ObjectBase {
public:
  ...
  typedef dereferencing_iterator<tl::vector<Circuit *>::const_iterator, Circuit>
      child_circuit_iterator;
  typedef dereferencing_iterator<tl::vector<const Circuit *>::const_iterator,
                                 const Circuit>
      const_child_circuit_iterator;
  typedef dereferencing_iterator<tl::vector<Circuit *>::const_iterator, Circuit>
      parent_circuit_iterator;
  typedef dereferencing_iterator<tl::vector<const Circuit *>::const_iterator,
                                 const Circuit>
      const_parent_circuit_iterator;
   ...
   */
  Netlist *netlist() { return mp_netlist; }

  /**
   *  @brief Gets the netlist the circuit lives in (const version)
   *  This pointer is 0 if the circuit is not part of a netlist.
   */
  const Netlist *netlist() const { return mp_netlist; }
   ...
   */
  const db::DPolygon &boundary() const { return m_boundary; }

  /**
   *  @brief Sets or resets the "don't purge" flag
   *  This flag will prevent "purge" from deleting this circuit. It is set by
   * "blank".
   */
   ...
   */
  bool dont_purge() const { return m_dont_purge; }

  /**
   *  @brief The index of the circuit in the netlist
   *  CAUTION: this attribute is used for internal purposes and may not be valid
   * always.
   */
   ...
   *  The parents circuits are the circuits referencing this circuit via
   * subcircuits.
   */
   ...
   */
  pin_iterator begin_pins() { return m_pins.begin(); }

  /**
   *  @brief End iterator for the pins of the circuit (non-const version)
   */
  pin_iterator end_pins() { return m_pins.end(); }
  ...
  /**
   *  @brief Gets the pin by ID (the ID is basically the index) - non-const
   * version
   */
   ...
   *  NOTE: this is a linear search, so it's performance may not be good for
   * many pins.
   */
   ...
   */
  const_pin_iterator begin_pins() const { return m_pins.begin(); }

  /**
   *  @brief End iterator for the pins of the circuit (const version)
   */
  const_pin_iterator end_pins() const { return m_pins.end(); }
   ...
   *  If the ID is not valid, null is returned.
   */
  Device *device_by_id(size_t id) { return m_device_by_id.object_by(id); }
   ...
   */
  device_iterator begin_devices() { return m_devices.begin(); }

  /**
   *  @brief End iterator for the devices of the circuit (non-const version)
   */
  device_iterator end_devices() { return m_devices.end(); }

  /**
   *  @brief Begin iterator for the devices of the circuit (const version)
   */
  const_device_iterator begin_devices() const { return m_devices.begin(); }

  /**
   *  @brief End iterator for the devices of the circuit (const version)
   */
  const_device_iterator end_devices() const { return m_devices.end(); }
   ...
   */
  size_t subcircuit_count() const { return m_subcircuits.size(); }
  ...
  /**
   *  @brief Begin iterator for the subcircuits of the circuit (non-const
   * version)
   */
  subcircuit_iterator begin_subcircuits() { return m_subcircuits.begin(); }

  /**
   *  @brief End iterator for the subcircuits of the circuit (non-const version)
   */
  subcircuit_iterator end_subcircuits() { return m_subcircuits.end(); }
  ...
  /**
   *  @brief Gets the connected net for a pin with the given id (non-const
   * version)
   *
   *  Returns 0 if the pin is not connected to a net.
   */
...
private:
  ...
  object_by_attr<Circuit, Circuit::device_iterator, id_attribute<Device>>
      m_device_by_id;
  object_by_attr<Circuit, Circuit::subcircuit_iterator,
                 id_attribute<SubCircuit>>
      m_subcircuit_by_id;
  object_by_attr<Circuit, Circuit::net_iterator, cluster_id_attribute<Net>>
      m_net_by_cluster_id;
  object_by_attr<Circuit, Circuit::device_iterator, name_attribute<Device>>
      m_device_by_name;
  object_by_attr<Circuit, Circuit::subcircuit_iterator,
                 name_attribute<SubCircuit>>
      m_subcircuit_by_name;
  object_by_attr<Circuit, Circuit::net_iterator, name_attribute<Net>>
      m_net_by_name;
  ...
  void translate_device_classes(
      const std::map<const DeviceClass *, DeviceClass *> &map);
  void translate_device_abstracts(
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const Circuit &x, bool no_self, void *parent) {
