...
void compare_layouts(tl::TestBase *_this, const db::Layout &layout,
                     const std::string &au_file, NormalizationMode norm,
                     db::Coord tolerance) {
  compare_layouts(_this, layout, au_file, db::LayerMap(), true, norm,
                  tolerance);
}

void compare_layouts(tl::TestBase *_this, const db::Layout &layout,
                     const std::string &au_file, const db::LayerMap &lm,
                     bool read_other_layers, NormalizationMode norm,
                     db::Coord tolerance) {
  ...
  if ((norm & db::NormFileMask) == WriteGDS2) {
    ...
    tmp_file =
        _this->tmp_file(tl::sprintf("tmp_%x." + tl::extension(au_file), hash));
    ...
    if (tl::file_exists(fn)) {
      ...
      if (n == 1 && any) {
        throw tl::Exception(tl::sprintf(
            "Inconsistent reference variants for %s: there can be either "
            "variants (.1,.2,... suffix) or a single file (without suffix)",
            au_file));
      }
      ...
      equal = db::compare_layouts(
          *subject, layout_au,
          ...
          /*| db::layout_diff::f_no_text_details |
             db::layout_diff::f_no_text_orientation*/
          ,
          tolerance, 100 /*max diff lines*/);
      if (equal && n > 0) {
        tl::info << tl::sprintf("Found match on golden reference variant %s",
                                fn);
      }
  ...
  if (!equal) {
    _this->raise(tl::sprintf(
        "Compare failed - see\n  actual: %s\n  golden: %s%s",
        tl::absolute_file_path(tmp_file), tl::absolute_file_path(au_file),
        (n > 1 ? "\nand variants" : "")));
  }
}

class CompareLogger : public db::NetlistCompareLogger {
public:
  CompareLogger() : m_new_circuit(true) {}
  ...
  virtual void device_class_mismatch(const db::DeviceClass *a,
    ...
    out("device_class_mismatch " + device_class2str(a) + " " +
        device_class2str(b));
  }
...
void DB_PUBLIC compare_netlist(tl::TestBase *_this, const db::Netlist &netlist,
                               ...
                               bool exact_parameter_match, bool with_names) {
  db::Netlist au_nl;
  for (db::Netlist::const_device_class_iterator d =
           netlist.begin_device_classes();
       d != netlist.end_device_classes(); ++d) {
...
void DB_PUBLIC compare_netlist(tl::TestBase *_this, const db::Netlist &netlist,
  ...
  if (exact_parameter_match) {
    //  install a "all parameters are equal" device parameter comparer so we
    //  make sure the devices are compared exactly
    for (db::Netlist::device_class_iterator dc =
             netlist_copy.begin_device_classes();
         dc != netlist_copy.end_device_classes(); ++dc) {
      db::DeviceClass &cls = *dc;
      cls.set_parameter_compare_delegate(
          new db::AllDeviceParametersAreEqual(0.01));
    }
  }
...
static bool do_compare(const Container &cont, const std::string &string) {
  ...
  if (a != b) {
    ...
    for (typename std::set<Shape>::const_iterator i = a.begin(); i != a.end();
         ++i) {
      if (b.find(*i) == b.end()) {
    ...
    for (typename std::set<Shape>::const_iterator i = b.begin(); i != b.end();
         ++i) {
      if (a.find(*i) == a.end()) {
        tl::error << "  " << i->to_string();
      }
 ...
 * representation
 */
std::string prop2string(const db::PropertiesRepository &pr,
