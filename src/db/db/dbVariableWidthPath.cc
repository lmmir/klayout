...
template <class C> void variable_width_path<C>::init() {
  ...
  for (typename std::vector<std::pair<size_t, C>>::const_iterator j =
           m_org_widths.begin();
       j != m_org_widths.end(); ++j) {
    ...
    if (j->first == i) {
      ...
      //  interpolation: first determine the whole length from last point to
      //  next and then interpolate each segment
      ...
      for (size_t ii = i; ii <= j->first; ++ii) {
        if (!last_set) {
          width_type ww =
              db::coord_traits<C>::rounded(w0 + (w - w0) * (l / ll));
  ...
  //  fill up the remaining widths (should not happen if the last width_spec is
  //  for the last point)
...
static void create_shifted_points(C /*c*/, bool forward, Iter from, Iter to,
                                  WIter wfrom, WIter wto, Inserter pts) {
  //  for safety reasons
  if (from == to) {
    return;
  }
        ...
        *pts++ = *pp + vector<C>(nnd1);
        ...
        if (fabs(dv) > db::epsilon) {
          ...
          if (l1 < l1min - db::epsilon || l2 < l2min - db::epsilon) {
            ...
            //  self-overlapping way but confined to the path within the limits
            //  of its width. HINT: the execution of this code is a pretty
            //  strong evidence for the existence to loops in the contour
            //  delivered. A proof however is missing ..
            *pts++ = *pp + vector<C>(nd2);
            *pts++ = *pp;
            *pts++ = *pp + vector<C>(nnd1);
...
template <class C>
typename variable_width_path<C>::simple_polygon_type
variable_width_path<C>::to_poly() const {
  std::vector<point<C>> pts;
  pts.reserve(m_points.size() * 2); //  minimum number of points required

  C c = 0;
  create_shifted_points(c, true, m_points.begin(), m_points.end(),
                        m_widths.begin(), m_widths.end(),
                        std::back_inserter(pts));
  create_shifted_points(c, false, m_points.rbegin(), m_points.rend(),
                        m_widths.rbegin(), m_widths.rend(),
                        std::back_inserter(pts));
