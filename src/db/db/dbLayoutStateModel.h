 ...
 *  the layout object (i.e. db::Shapes) and "collected" in the layout state
 * model. Calling "update" will reset this state and call "do_update" to bring
 * the layout object into a consistent state. In addition, observers using the
 * tl::Observer interface can attach to this state model to track if the layout
 * changes it's state.
 */
...
public:
   ...
   *
   *  This constructor does not copy the attachments to the tl::Observed
   * members!
   */
  ...
  void invalidate_hier() {
    ...
    if (!m_hier_dirty || m_busy) {
      do_invalidate_hier(); //  must be called before the hierarchy is
                            //  invalidated (stopping of redraw thread requires
                            //  this)
      m_hier_dirty = true;
    }
  }
   ...
   */
  void dbu_changed() { dbu_changed_event(); }

  /**
   *  @brief This method resets the layout's state back to valid hierarchy and
   * bounding boxes
   *
   ...
   *
   *  This attribute is true, if the hierarchy has changed since the last
   * "update" call
   */
  bool hier_dirty() const { return m_hier_dirty; }

  /**
   *  @brief Gets the hierarchy generation ID
   *
   *  The hierarchy generation ID is a number which is incremented on every
   * hierarchy change.
   */
  size_t hier_generation_id() const { return m_hier_generation_id; }

  /**
   *  @brief The "dirty bounding box" attribute
   *
   *  This attribute is true, if the bounding boxes have changed since the last
   * "update" call
   */
...
protected:
  ...
  /**
   *  @brief Reimplement this method to update anything related to the hierarchy
   * or bounding boxes.
   */
   ...
   */
  void prop_ids_changed() { prop_ids_changed_event(); }

  /**
   *  @brief Issue a "prop id's changed event"
   */
  void cell_name_changed() { cell_name_changed_event(); }

  /**
   *  @brief Issue a "layer properties changed event"
   */
  void layer_properties_changed() { layer_properties_changed_event(); }
