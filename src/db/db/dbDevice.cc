...
Device::Device()
    : db::NetlistObject(), mp_device_class(0), mp_device_abstract(0), m_id(0),
      mp_circuit(0) {
  //  .. nothing yet ..
}

Device::~Device() {
  for (std::vector<Net::terminal_iterator>::const_iterator t =
           m_terminal_refs.begin();
       t != m_terminal_refs.end(); ++t) {
...
Device::Device(DeviceClass *device_class, const std::string &name)
    : db::NetlistObject(), mp_device_class(device_class), mp_device_abstract(0),
      m_name(name), m_id(0), mp_circuit(0) {
  //  .. nothing yet ..
}

Device::Device(DeviceClass *device_class, DeviceAbstract *device_abstract,
               const std::string &name)
    : db::NetlistObject(), mp_device_class(device_class),
      mp_device_abstract(device_abstract), m_name(name), m_id(0),
      mp_circuit(0) {
  //  .. nothing yet ..
}

Device::Device(const Device &other)
    : db::NetlistObject(other), mp_device_class(0), mp_device_abstract(0),
      m_id(0), mp_circuit(0) {
  operator=(other);
}
...
void Device::set_circuit(Circuit *circuit) { mp_circuit = circuit; }

const Netlist *Device::netlist() const {
  return mp_circuit ? mp_circuit->netlist() : 0;
}

Netlist *Device::netlist() { return mp_circuit ? mp_circuit->netlist() : 0; }
...
void Device::set_trans(const db::DCplxTrans &tr) { m_trans = tr; }

void Device::set_terminal_ref_for_terminal(size_t terminal_id,
...
double Device::parameter_value(size_t param_id) const {
  if (m_parameters.size() > param_id) {
    ...
    const db::DeviceParameterDefinition *pd =
        mp_device_class->parameter_definition(param_id);
    if (pd) {
  ...
  if (m_parameters.size() <= param_id) {
    ...
    if (mp_device_class) {
      for (size_t n = from_size; n < param_id; ++n) {
        const db::DeviceParameterDefinition *pd =
            mp_device_class->parameter_definition(n);
        if (pd) {
...
double Device::parameter_value(const std::string &name) const {
  return device_class()
             ? parameter_value(device_class()->parameter_id_for_name(name))
             : 0.0;
}
...
void Device::add_others_terminals(unsigned int this_terminal, db::Device *other,
                                  unsigned int other_terminal) {
  std::vector<DeviceReconnectedTerminal> &terminals =
      m_reconnected_terminals[this_terminal];

  std::map<unsigned int, std::vector<DeviceReconnectedTerminal>>::const_iterator
      ot = other->m_reconnected_terminals.find(other_terminal);
  if (ot == other->m_reconnected_terminals.end()) {

    terminals.push_back(DeviceReconnectedTerminal(other_abstracts().size() + 1,
                                                  other_terminal));
...
void Device::init_terminal_routes() {
  ...
  for (size_t i = 0; i < n; ++i) {
    m_reconnected_terminals[(unsigned int)i].push_back(
        DeviceReconnectedTerminal(0, (unsigned int)i));
  }
}

void Device::join_terminals(unsigned int this_terminal, db::Device *other,
                            unsigned int other_terminal) {
...
void Device::reroute_terminal(unsigned int this_terminal, db::Device *other,
...
void Device::join_device(db::Device *other) {
  ...
  m_other_abstracts.reserve(m_other_abstracts.size() + 1 +
                            other->m_other_abstracts.size());

  m_other_abstracts.push_back(
      db::DeviceAbstractRef(other->device_abstract(), d));

  for (std::vector<db::DeviceAbstractRef>::const_iterator a =
           other->m_other_abstracts.begin();
       a != other->m_other_abstracts.end(); ++a) {
...
map_da(const std::map<const DeviceAbstract *, DeviceAbstract *> &map,
       const db::DeviceAbstract *da) {
  if (!da) {
    return 0;
  } else {
    std::map<const DeviceAbstract *, DeviceAbstract *>::const_iterator m =
        map.find(da);
    tl_assert(m != map.end());
    return m->second;
  }
...
void Device::translate_device_abstracts(
    const std::map<const DeviceAbstract *, DeviceAbstract *> &map) {
  set_device_abstract(map_da(map, device_abstract()));

  for (std::vector<db::DeviceAbstractRef>::iterator a =
           m_other_abstracts.begin();
       a != m_other_abstracts.end(); ++a) {
