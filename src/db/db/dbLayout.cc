...
struct SetLayoutPropId : public LayoutOp {
  ...
  virtual void redo(db::Layout *layout) const { layout->prop_id(m_to); }

  virtual void undo(db::Layout *layout) const { layout->prop_id(m_from); }
...
struct SetLayoutDBU : public LayoutOp {
  ...
  virtual void redo(db::Layout *layout) const { layout->dbu(m_to); }

  virtual void undo(db::Layout *layout) const { layout->dbu(m_from); }
...
struct RenameCellOp : public LayoutOp {
  RenameCellOp(db::cell_index_type i, const std::string &f,
               const std::string &t)
...
struct NewRemoveCellOp : public LayoutOp {
  NewRemoveCellOp(db::cell_index_type i, const std::string &name, bool remove,
                  db::Cell *cell)
...
struct SetLayerPropertiesOp : public LayoutOp {
  SetLayerPropertiesOp(unsigned int l, const db::LayerProperties &new_props,
                       const db::LayerProperties &old_props)
...
struct InsertRemoveLayerOp : public LayoutOp {
  InsertRemoveLayerOp(unsigned int l, const db::LayerProperties &props,
                      bool insert)
...
LayoutOrCellContextInfo LayoutOrCellContextInfo::deserialize(
    std::vector<std::string>::const_iterator from,
    std::vector<std::string>::const_iterator to) {
...
void LayoutOrCellContextInfo::serialize(std::vector<std::string> &strings) {
  ...
  for (auto p = pcell_parameters.begin(); p != pcell_parameters.end(); ++p) {
    strings.push_back("P(" + tl::to_word_or_quoted_string(p->first) +
                      ")=" + p->second.to_parsable_string());
  }
  if (!pcell_name.empty()) {
...
Layout::Layout(db::Manager *manager)
    : db::Object(manager), mp_library(0), m_cells_size(0), m_invalid(0),
      m_top_cells(0), m_dbu(0.001), m_prop_id(0), m_properties_repository(this),
      m_do_cleanup(false), m_editable(db::default_editable_mode()) {
  // .. nothing yet ..
}
...
Layout::Layout(bool editable, db::Manager *manager)
    : db::Object(manager), mp_library(0), m_cells_size(0), m_invalid(0),
      m_top_cells(0), m_dbu(0.001), m_prop_id(0), m_properties_repository(this),
      m_do_cleanup(false), m_editable(editable) {
  // .. nothing yet ..
}

Layout::Layout(const db::Layout &layout)
    : db::Object(layout), db::LayoutStateModel(), gsi::ObjectBase(),
      tl::Object(), tl::UniqueId(), mp_library(0), m_cells_size(0),
      m_invalid(0), m_top_cells(0), m_dbu(0.001), m_prop_id(0),
      m_properties_repository(this), m_do_cleanup(false),
      m_editable(layout.m_editable) {
  *this = layout;
}

Layout::~Layout() {
  //  since it the cell graph (or the derived layout) might produce some
  //  transactions that refer to this object, we need to clear the manager's
  //  transaction list before the cell graph is deleted.
  if (manager()) {
    manager()->clear();
  }
...
void Layout::clear() {
  ...
  for (std::vector<const char *>::const_iterator p = m_cell_names.begin();
       p != m_cell_names.end(); ++p) {
    if (*p) {
      delete[] * p;
    }
  }
  ...
  for (std::vector<pcell_header_type *>::const_iterator pc = m_pcells.begin();
       pc != m_pcells.end(); ++pc) {
    delete *pc;
  }
  m_pcells.clear();
...
Layout &Layout::operator=(const Layout &d) {
  if (&d != this) {
    ...
    for (std::vector<pcell_header_type *>::const_iterator pc =
             d.m_pcells.begin();
         pc != d.m_pcells.end(); ++pc) {
      if (*pc) {
    ...
    m_properties_repository =
        d.m_properties_repository; // because the cell assign operator does not
                                   // map property ID's ..
    ...
    cell_index_type i = 0;
    for (std::vector<const char *>::const_iterator p = d.m_cell_names.begin();
         p != d.m_cell_names.end(); ++p) {
      if (*p) {
...
const db::Technology *Layout::technology() const {
  return db::Technologies::instance()->has_technology(m_tech_name)
             ? db::Technologies::instance()->technology_by_name(m_tech_name)
             : 0;
}
...
void Layout::set_technology_name(const std::string &tech) {
  ...
  for (db::Layout::iterator c = begin(); c != end(); ++c) {
    ...
    if (lib_proxy && seen.find(lib_proxy->lib_id()) == seen.end()) {
      ...
      const db::Library *l =
          db::LibraryManager::instance().lib(lib_proxy->lib_id());
      if (l) {
        new_id =
            db::LibraryManager::instance().lib_by_name(l->get_name(), tech);
      }
  ...
  if (!mapping.empty() || !lost.empty()) {
    ...
    std::vector<std::pair<db::LibraryProxy *, db::PCellVariant *>>
        pcells_to_map;
    ...
    for (db::Layout::iterator c = begin(); c != end(); ++c) {
      ...
      if ((m = mapping.find(lib_proxy->lib_id())) != mapping.end()) {
        ...
        db::Library *lib =
            db::LibraryManager::instance().lib(lib_proxy->lib_id());
        db::Cell *lib_cell =
            &lib->layout().cell(lib_proxy->library_cell_index());
        db::PCellVariant *lib_pcell =
            dynamic_cast<db::PCellVariant *>(lib_cell);
        if (lib_pcell) {
    ...
    //  We do PCell resolution before the library proxy resolution. The reason
    //  is that PCells may generate library proxies in their instantiation.
    //  Hence we must instantiate the PCells before we can resolve them.
    for (std::vector<std::pair<db::LibraryProxy *, db::PCellVariant *>>::
             const_iterator lp = pcells_to_map.begin();
         lp != pcells_to_map.end(); ++lp) {

      db::cell_index_type ci = lp->first->Cell::cell_index();
      db::PCellVariant *lib_pcell = lp->second;

      std::pair<bool, pcell_id_type> pn = lib_pcell->layout()->pcell_by_name(
          lp->first->get_basic_name().c_str());
      ...
      if (!pn.first) {
        ...
        db::Library *new_lib =
            db::LibraryManager::instance().lib(mapping[lp->first->lib_id()]);

        const db::PCellDeclaration *old_pcell_decl =
            lib_pcell->layout()->pcell_declaration(lib_pcell->pcell_id());
        const db::PCellDeclaration *new_pcell_decl =
            new_lib->layout().pcell_declaration(pn.second);
        if (!old_pcell_decl || !new_pcell_decl) {
          ...
          std::map<std::string, tl::Variant> param_by_name =
              lib_pcell->parameters_by_name();
          lp->first->remap(
              new_lib->get_id(),
              new_lib->layout().get_pcell_variant(
                  pn.second, new_pcell_decl->map_parameters(param_by_name)));
        }
      }
    }

    for (std::vector<db::LibraryProxy *>::const_iterator lp =
             lib_cells_to_map.begin();
         lp != lib_cells_to_map.end(); ++lp) {

      db::Library *new_lib =
          db::LibraryManager::instance().lib(mapping[(*lp)->lib_id()]);

      db::cell_index_type ci = (*lp)->Cell::cell_index();

      std::pair<bool, cell_index_type> cn =
          new_lib->layout().cell_by_name((*lp)->get_basic_name().c_str());
    ...
    for (std::vector<db::LibraryProxy *>::const_iterator lp =
             lib_cells_lost.begin();
         lp != lib_cells_lost.end(); ++lp) {
  ...
  //  we may have re-established a connection for pending ("cold") proxies so we
  //  can try to restore them
  restore_proxies();
}

void Layout::mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                      int cat, bool no_self, void *parent) const {
  if (!no_self) {
  ...
  for (std::vector<const char *>::const_iterator i = m_cell_names.begin();
       i != m_cell_names.end(); ++i) {
    stat->add(typeid(char[]), (void *)*i, *i ? (strlen(*i) + 1) : 0,
              *i ? (strlen(*i) + 1) : 0, (void *)this, purpose, cat);
  }
  for (cell_list::const_iterator i = m_cells.begin(); i != m_cells.end(); ++i) {
    db::mem_stat(stat, MemStatistics::CellInfo, int(i->id()), *i, false,
                 (void *)this);
  }
  for (std::vector<pcell_header_type *>::const_iterator i = m_pcells.begin();
       i != m_pcells.end(); ++i) {
...
std::pair<bool, cell_index_type> Layout::cell_by_name(const char *name) const {
...
const char *Layout::cell_name(cell_index_type index) const {
...
void Layout::delete_cells(const std::set<cell_index_type> &cells_to_delete) {
  ...
  for (std::set<cell_index_type>::const_iterator c = cells_to_delete.begin();
       c != cells_to_delete.end(); ++c) {
    const db::Cell &cref = cell(*c);
    for (db::Cell::parent_cell_iterator pc = cref.begin_parent_cells();
         pc != cref.end_parent_cells(); ++pc) {
      pcs.insert(*pc);
    }
  }
  ...
  for (std::set<cell_index_type>::const_iterator c = cells_to_delete.begin();
    ...
    //  If transacting, do not use clear_shapes here. This will delete all the
    //  shapes containers will will disable us saving undo data with reference
    //  to them.
    if (manager() && manager()->transacting()) {
  ...
  for (std::set<cell_index_type>::const_iterator pc = pcs.begin();
       pc != pcs.end(); ++pc) {

    db::Cell &parent_cref = cell(*pc);

    insts_to_delete.clear();
    for (db::Cell::const_iterator pci = parent_cref.begin(); !pci.at_end();
         ++pci) {
  ...
  //  If transacting, the cell is not deleted yet. Instead, the transaction
  //  object acts as a backup container for the cell. This is necessary since
  //  the ID's within manager are given to cell child objects that must remain.
  for (std::set<cell_index_type>::const_iterator c = cells_to_delete.begin();
       c != cells_to_delete.end(); ++c) {
    ...
    if (manager() && manager()->transacting()) {
      ...
      manager()->queue(
          this, new NewRemoveCellOp(*c, cn, true /*remove*/, take_cell(*c)));

    } else {

      //  remove the cell - we use take_cell and delete to avoid recursion
      //  issues
      delete take_cell(*c);
    }
  }
...
void Layout::delete_cell(cell_index_type id) {
  ...
  //  If transacting, do not use clear_shapes here. This will delete all the
  //  shapes containers will will disable us saving undo data with reference to
  //  them.
  if (manager() && manager()->transacting()) {
    for (unsigned int i = 0; i < layers(); ++i) {
      if (is_valid_layer(i)) {
  ...
  std::vector<db::Instance> insts_to_delete;
  for (std::vector<cell_index_type>::const_iterator pc = pcs.begin();
       pc != pcs.end(); ++pc) {
  ...
  if (manager() && manager()->transacting()) {
    ...
    manager()->queue(
        this, new NewRemoveCellOp(id, cn, true /*remove*/, take_cell(id)));
...
void Layout::insert(db::cell_index_type cell, int layer,
...
void Layout::flatten(const db::Cell &source_cell, db::Cell &target_cell,
  ...
  if (&source_cell != &target_cell) {
    ...
    for (unsigned int l = 0; l < nlayers; ++l) {
      ...
      if (is_valid_layer(l)) {
        ...
        for (db::Shapes::shape_iterator sh =
                 source_shapes.begin(db::ShapeIterator::All);
             !sh.at_end(); ++sh) {
  ...
  if (levels == 0) {
    ...
    if (&source_cell != &target_cell) {
      for (db::Cell::const_iterator inst = source_cell.begin(); !inst.at_end();
           ++inst) {
    ...
    try {
      ...
      //  Note: suppressing the update speeds up the flatten process
      //  considerably since even an iteration of the instances requires an
      //  update.
      start_changes();
      ...
      for (db::Cell::const_iterator inst = old_instances.begin();
           !inst.at_end(); ++inst) {
        ...
        for (db::CellInstArray::iterator a = cell_inst.begin(); !a.at_end();
          ...
          flatten(cell(cell_inst.object().cell_index()), target_cell, tinst,
...
void Layout::flatten(db::Cell &cell_to_flatten, int levels, bool prune) {
  ...
  if (prune) {
    ...
    for (std::set<db::cell_index_type>::iterator dc = direct_children.begin();
         dc != direct_children.end();) {
...
void Layout::do_prune_cell_or_subcell(cell_index_type id, int levels,
                                      bool subcells) {
  db::Cell &cref = cell(id);
  ...
  //  From these cells erase all cells that have parents outside the subtree of
  //  our cell. Make sure this is done recursively by doing this top-down.
  for (top_down_iterator c = begin_top_down(); c != end_top_down(); ++c) {
    if (*c != id && called.find(*c) != called.end()) {
      db::Cell &ccref = cell(*c);
      for (db::Cell::parent_cell_iterator pc = ccref.begin_parent_cells();
           pc != ccref.end_parent_cells(); ++pc) {
        if (*pc != id && called.find(*pc) == called.end()) {
          //  we have a parent outside the subset considered currently (either
          //  the cell was never in or it was removed itself already): remove
          //  this cell from the set of valid subcells.
          called.erase(*c);
          break;
        }
      }
  ...
  //  erase all instances in the subcells case (because, by definition we don't
...
void Layout::do_prune_cells_or_subcells(const std::set<cell_index_type> &ids,
                                        int levels, bool subcells) {
  //  collect the called cells
  std::set<cell_index_type> called;
  for (std::set<cell_index_type>::const_iterator id = ids.begin();
       id != ids.end(); ++id) {
    db::Cell &cref = cell(*id);
    cref.collect_called_cells(called, levels);
  }
...
void Layout::insert_cell(cell_index_type ci, const std::string &name,
                         db::Cell *cell) {
...
db::Cell *Layout::take_cell(cell_index_type ci) {
  ...
  //  The cellview references cannot be uniquely classified as being invalid -
  //  because the ID might be reused. This causes problems, when a cell is being
  //  deleted and subsequently a cell is being created - a crash occurs.
  //  Therefore the free index feature is disabled. If this causes memory
  //  consumption problems, it should be considered to use a map and an
  //  arbitrary ID.
...
std::string Layout::uniquify_cell_name(const char *name) const {
...
cell_index_type Layout::add_cell(const db::Layout &other,
  ...
  if (&other == this) {
    ...
    for (auto m = other.begin_meta(ci); m != other.end_meta(ci); ++m) {
      add_meta_info(ci_new, meta_info_name_id(other.meta_info_name(m->first)),
                    m->second);
    }
  }
...
cell_index_type Layout::add_cell(const char *name) {
  std::string b;
  ...
  if (manager() && manager()->transacting()) {
    manager()->queue(this,
                     new NewRemoveCellOp(new_index, m_cell_names[new_index],
                                         false /*new*/, 0));
  }
...
cell_index_type Layout::add_anonymous_cell() {
  std::string b;
...
bool Layout::topological_sort() {
  ...
  //  Reason is that this is somewhat safer, specifically directly after take()
  //  when the cell list is already reduced, but the cell pointers are still
  //  containing the cell (issue #905)
  size_t ncells = 0;
  ...
  while (m_top_down_list.size() != ncells) {
    ...
    for (const_iterator c = begin(); c != end(); ++c) {
      if (c->parent_cells() == num_parents[c->cell_index()]) {
        ...
        num_parents[c->cell_index()] =
            std::numeric_limits<cell_index_type>::max();
      }
    }

    //  For all these a cells, increment the reported parent instance
    //  count in all the child cells.
    for (cell_index_vector::const_iterator ii =
             m_top_down_list.begin() + n_top_down_cells;
         ii != m_top_down_list.end(); ++ii) {
      for (cell_type::child_cell_iterator cc = cell(*ii).begin_child_cells();
           !cc.at_end(); ++cc) {
        tl_assert(num_parents[*cc] !=
                  std::numeric_limits<cell_index_type>::max());
        num_parents[*cc] += 1;
      }
    }
  ...
  for (top_down_iterator e = m_top_down_list.begin();
       e != m_top_down_list.end() && cell(*e).is_top(); ++e) {
    ++m_top_cells;
  }
...
void Layout::copy_tree_shapes(const db::Layout &source_layout,
  ...
  db::copy_shapes(*this, source_layout, trans, cm.source_cells(), cm.table(),
                  lm.table());
}
...
void Layout::move_tree_shapes(db::Layout &source_layout,
                              const db::CellMapping &cm) {
  if (this == &source_layout) {
    throw tl::Exception(
        tl::to_string(tr("Cannot copy shapes within the same layout")));
  ...
  db::move_shapes(*this, source_layout, trans, cm.source_cells(), cm.table(),
                  lm.table());
}
...
cell_index_type Layout::allocate_new_cell() {
  invalidate_hier();
...
void Layout::cleanup(const std::set<db::cell_index_type> &keep) {
  //  only managed layouts will receive cleanup requests. Never library
  //  container layouts - these cannot know if their proxies are not referenced
  //  by other proxies.
  if (!m_do_cleanup) {
    return;
  }

  //  deleting cells may create new top cells which need to be deleted as well,
  //  hence we iterate until there are no more cells to delete
  while (true) {

    //  delete all cells that are top cells and are proxies. Those cells are
    ...
    for (std::set<db::cell_index_type>::const_iterator k = keep.begin();
         k != keep.end(); ++k) {
...
Layout::top_down_iterator Layout::end_top_cells() {
  update();
  return m_top_down_list.begin() + m_top_cells;
}

Layout::top_down_const_iterator Layout::end_top_cells() const {
  update();
  return m_top_down_list.begin() + m_top_cells;
}
...
void Layout::force_update() {
  if (hier_dirty() || bboxes_dirty()) {
    ...
    try {
      ...
      m_invalid =
          std::numeric_limits<unsigned int>::max(); //  prevent recursion

      db::LayoutStateModel *state_model = const_cast<db::LayoutStateModel *>(
          (const db::LayoutStateModel *)this);
      state_model->update();
...
void Layout::do_update() {
  tl::SelfTimer timer(tl::verbosity() > layout_base_verbosity,
                      tl::to_string(tr("Sorting")));
  ...
  //  object does not work. We overcome this problem by creating the object with
  //  new and catching exceptions. As this operation is critical we don't want
  //  to have it cancelled. Plus: do_update is called during ~LayoutLocker and
  //  if we throw exceptions then, we'll get a runtime assertion.
  tl::RelativeProgress *pr =
      new tl::RelativeProgress(tl::to_string(tr("Sorting layout")),
                               m_cells_size, 0, false /*can't cancel*/);
  pr->set_desc("");
  ...
  try {
    ...
    if (hier_dirty()) {
      {
        tl::SelfTimer timer(tl::verbosity() > layout_base_verbosity + 10,
                            "Updating relations");
    ...
    if (bboxes_dirty()) {

      {
        ...
        for (bottom_up_iterator c = begin_bottom_up(); c != end_bottom_up();
             ++c) {
          ++*pr;
          cell_type &cp(cell(*c));
          if (cp.is_shape_bbox_dirty() ||
              dirty_parents.find(*c) != dirty_parents.end()) {
            if (cp.update_bbox(layers)) {
              //  the bounding box has changed - need to insert parents into
              //  "dirty parents" list
              for (cell_type::parent_cell_iterator p = cp.begin_parent_cells();
                   p != cp.end_parent_cells(); ++p) {
...
Layout::meta_info_iterator Layout::begin_meta(db::cell_index_type ci) const {
...
Layout::meta_info_iterator Layout::end_meta(db::cell_index_type ci) const {
  auto m = m_meta_info_by_cell.find(ci);
  if (m != m_meta_info_by_cell.end()) {
    return m->second.end();
  } else {
...
Layout::meta_info_name(Layout::meta_info_name_id_type name_id) const {
  ...
  return name_id < m_meta_info_names.size() ? m_meta_info_names[name_id]
                                            : empty;
}
...
Layout::meta_info_name_id(const std::string &name) const {
  auto n = m_meta_info_name_map.find(name);
  return n != m_meta_info_name_map.end()
             ? n->second
             : std::numeric_limits<meta_info_name_id_type>::max();
}

void Layout::clear_meta() { m_meta_info.clear(); }
...
const MetaInfo &Layout::meta_info(meta_info_name_id_type name_id) const {
...
void Layout::add_meta_info(db::cell_index_type ci,
                           meta_info_name_id_type name_id, const MetaInfo &i) {
  m_meta_info_by_cell[ci][name_id] = i;
}

void Layout::remove_meta_info(db::cell_index_type ci,
                              meta_info_name_id_type name_id) {
...
const MetaInfo &Layout::meta_info(db::cell_index_type ci,
                                  meta_info_name_id_type name_id) const {
  auto c = m_meta_info_by_cell.find(ci);
  if (c != m_meta_info_by_cell.end()) {
...
bool Layout::has_meta_info(db::cell_index_type ci,
                           meta_info_name_id_type name_id) const {
  auto c = m_meta_info_by_cell.find(ci);
  if (c != m_meta_info_by_cell.end()) {
...
void Layout::delete_layer(unsigned int n) {
  ...
  if (manager() && manager()->transacting()) {
    manager()->queue(this,
                     new InsertRemoveLayerOp(n, m_layers.get_properties(n),
                                             false /*delete*/));
  }
...
unsigned int Layout::get_layer(const db::LayerProperties &props) {
...
unsigned int Layout::insert_layer(const LayerProperties &props) {
...
void Layout::insert_layer(unsigned int index, const LayerProperties &props) {
  ...
  if (manager() && manager()->transacting()) {
    manager()->queue(this,
                     new InsertRemoveLayerOp(index, props, true /*insert*/));
  }

  layer_properties_changed();
}

unsigned int Layout::insert_special_layer(const LayerProperties &props) {
...
void Layout::set_properties(unsigned int i, const LayerProperties &props) {
  if (m_layers.get_properties(i) != props) {
    ...
    if (manager() && manager()->transacting()) {
      manager()->queue(
          this, new SetLayerPropertiesOp(i, props, m_layers.get_properties(i)));
    }
...
void Layout::insert_special_layer(unsigned int index,
...
static const std::vector<tl::Variant> &
gauge_parameters(const std::vector<tl::Variant> &p,
                 const db::PCellDeclaration *pcell_decl,
                 std::vector<tl::Variant> &buffer) {
  const std::vector<db::PCellParameterDeclaration> &pcp =
      pcell_decl->parameter_declarations();
  ...
  if (pcp.size() > p.size()) {
    ...
    buffer = p;
    for (std::vector<PCellParameterDeclaration>::const_iterator i =
             pcp.begin() + p.size();
         i != pcp.end(); ++i) {
...
void Layout::replace_cell(cell_index_type target_cell_index, db::Cell *new_cell,
                          bool retain_layout) {
  invalidate_hier();
  ...
  if (manager() && manager()->transacting()) {
    //  note the "take" method - this takes out the cell but does not delete it
    //  (we need it inside undo)
    m_cells.take(iterator(old_cell));
    manager()->queue(this, new NewRemoveCellOp(target_cell_index,
                                               cell_name(target_cell_index),
                                               true /*remove*/, old_cell));
  } else {
  ...
  if (manager() && manager()->transacting()) {
    manager()->queue(this, new NewRemoveCellOp(target_cell_index,
                                               m_cell_names[target_cell_index],
                                               false /*new*/, 0));
  }
}

void Layout::replace_instances_of(cell_index_type src_cell_index,
                                  cell_index_type target_cell_index) {
  //  replace all instances of the new cell with the original one
  std::vector<std::pair<db::cell_index_type, db::Instance>> parents;
  for (db::Cell::parent_inst_iterator pi =
           cell(src_cell_index).begin_parent_insts();
       !pi.at_end(); ++pi) {
    parents.push_back(
        std::make_pair(pi->parent_cell_index(), pi->child_inst()));
  }

  for (std::vector<std::pair<db::cell_index_type, db::Instance>>::const_iterator
           p = parents.begin();
       p != parents.end(); ++p) {
...
void Layout::get_pcell_variant_as(pcell_id_type pcell_id,
                                  const std::vector<tl::Variant> &p,
                                  cell_index_type target_cell_index,
                                  ImportLayerMapping *layer_mapping,
                                  bool retain_layout) {
  ...
  const std::vector<tl::Variant> &parameters =
      gauge_parameters(p, header->declaration(), buffer);
  ...
  pcell_variant_type *variant =
      new pcell_variant_type(target_cell_index, *this, pcell_id, parameters);
...
cell_index_type
Layout::get_pcell_variant_dict(pcell_id_type pcell_id,
                               const std::map<std::string, tl::Variant> &p) {
  pcell_header_type *header = pcell_header(pcell_id);
  tl_assert(header != 0);

  std::vector<tl::Variant> parameters;
  const std::vector<db::PCellParameterDeclaration> &pcp =
      header->declaration()->parameter_declarations();
  parameters.reserve(pcp.size());
  for (std::vector<db::PCellParameterDeclaration>::const_iterator pd =
           pcp.begin();
       pd != pcp.end(); ++pd) {
    std::map<std::string, tl::Variant>::const_iterator pp =
        p.find(pd->get_name());
    if (pp == p.end()) {
...
cell_index_type Layout::get_pcell_variant(pcell_id_type pcell_id,
                                          const std::vector<tl::Variant> &p) {
  pcell_header_type *header = pcell_header(pcell_id);
  tl_assert(header != 0);
...
Layout::pcell_header_type *Layout::pcell_header(pcell_id_type pcell_id) {
...
std::pair<bool, pcell_id_type> Layout::pcell_by_name(const char *name) const {
  std::map<std::string, pcell_id_type>::const_iterator pcid =
      m_pcell_ids.find(std::string(name));
...
pcell_id_type Layout::register_pcell(const std::string &name,
                                     pcell_declaration_type *declaration) {
  //  No undo/redo support for PCell registration. The interactions with PCell
  //  variants (for which undo/redo support is available) is too complex ...
...
db::cell_index_type Layout::convert_cell_to_static(db::cell_index_type ci) {
...
Layout::is_pcell_instance(cell_index_type cell_index) const {
  ...
  const LibraryProxy *lib_proxy =
      dynamic_cast<const LibraryProxy *>(child_cell);
  if (lib_proxy) {
  ...
  const PCellVariant *pcell_variant =
      dynamic_cast<const PCellVariant *>(child_cell);
  if (pcell_variant) {
...
const Layout::pcell_declaration_type *
Layout::pcell_declaration_for_pcell_variant(
    cell_index_type variant_cell_index) const {
  const Cell *variant_cell = &cell(variant_cell_index);

  const LibraryProxy *lib_proxy =
      dynamic_cast<const LibraryProxy *>(variant_cell);
  if (lib_proxy) {
    Library *lib = LibraryManager::instance().lib(lib_proxy->lib_id());
    tl_assert(lib != 0);
    return lib->layout().pcell_declaration_for_pcell_variant(
        lib_proxy->library_cell_index());
  }

  const PCellVariant *pcell_variant =
      dynamic_cast<const PCellVariant *>(variant_cell);
  if (pcell_variant) {
...
tl::Variant Layout::get_pcell_parameter(cell_index_type cell_index,
  ...
  if (lib_proxy) {
    Library *lib = LibraryManager::instance().lib(lib_proxy->lib_id());
    tl_assert(lib != 0);
    return lib->layout().get_pcell_parameter(lib_proxy->library_cell_index(),
                                             name);
  }
...
Layout::get_named_pcell_parameters(cell_index_type cell_index) const {
  ...
  if (lib_proxy) {
    Library *lib = LibraryManager::instance().lib(lib_proxy->lib_id());
    tl_assert(lib != 0);
    return lib->layout().get_named_pcell_parameters(
        lib_proxy->library_cell_index());
  }
...
cell_index_type
Layout::get_pcell_variant_cell(cell_index_type cell_index,
                               const std::vector<tl::Variant> &new_parameters) {
  ...
  if (lib_proxy) {
    ...
    Library *lib = LibraryManager::instance().lib(lib_proxy->lib_id());
    cell_index_type new_lib_cell_index = lib->layout().get_pcell_variant_cell(
        lib_proxy->library_cell_index(), new_parameters);
...
bool Layout::get_context_info(LayoutOrCellContextInfo &info) const {
  for (auto i = m_meta_info.begin(); i != m_meta_info.end(); ++i) {
    if (i->second.persisted) {
      std::pair<tl::Variant, std::string> &mi =
          info.meta_info[m_meta_info_names[i->first]];
...
void Layout::fill_meta_info_from_context(
    std::vector<std::string>::const_iterator from,
    std::vector<std::string>::const_iterator to) {
...
void Layout::fill_meta_info_from_context(
    const LayoutOrCellContextInfo &context_info) {
  if (!context_info.meta_info.empty()) {
    for (auto i = context_info.meta_info.begin();
         i != context_info.meta_info.end(); ++i) {
...
bool Layout::get_context_info(cell_index_type cell_index,
...
bool Layout::get_context_info(cell_index_type cell_index,
                              LayoutOrCellContextInfo &info) const {
  bool any_meta = false;
  ...
  while (ly != 0 &&
         (lib_proxy = dynamic_cast<const db::LibraryProxy *>(cptr)) != 0) {
  ...
  const db::PCellVariant *pcell_variant =
      dynamic_cast<const db::PCellVariant *>(cptr);
  if (pcell_variant) {

    const db::PCellDeclaration *pcell_decl =
        ly->pcell_declaration(pcell_variant->pcell_id());

    const std::vector<db::PCellParameterDeclaration> &pcp =
        pcell_decl->parameter_declarations();
    std::vector<db::PCellParameterDeclaration>::const_iterator pd = pcp.begin();
    for (std::vector<tl::Variant>::const_iterator p =
             pcell_variant->parameters().begin();
         p != pcell_variant->parameters().end() && pd != pcp.end(); ++p, ++pd) {
...
void Layout::fill_meta_info_from_context(
    cell_index_type cell_index, std::vector<std::string>::const_iterator from,
    std::vector<std::string>::const_iterator to) {
  fill_meta_info_from_context(cell_index,
                              LayoutOrCellContextInfo::deserialize(from, to));
}

void Layout::fill_meta_info_from_context(
    cell_index_type cell_index, const LayoutOrCellContextInfo &context_info) {
  if (!context_info.meta_info.empty()) {
...
void Layout::restore_proxies(ImportLayerMapping *layer_mapping) {
  ...
  bool needs_cleanup = false;
  for (std::vector<db::ColdProxy *>::const_iterator p = cold_proxies.begin();
       p != cold_proxies.end(); ++p) {
    if (recover_proxy_as((*p)->cell_index(), (*p)->context_info(),
                         layer_mapping)) {
      needs_cleanup = true;
    }
  }
...
bool Layout::recover_proxy_as(cell_index_type cell_index,
                              std::vector<std::string>::const_iterator from,
                              std::vector<std::string>::const_iterator to,
                              ImportLayerMapping *layer_mapping) {
  if (from == to) {
  ...
  return recover_proxy_as(cell_index,
                          LayoutOrCellContextInfo::deserialize(from, to),
                          layer_mapping);
}
...
bool Layout::recover_proxy_as(cell_index_type cell_index,
  ...
  if (!info.lib_name.empty()) {
    ...
    Library *lib = db::LibraryManager::instance().lib_ptr_by_name(info.lib_name,
                                                                  m_tech_name);
    if (lib) {
    ...
    if (!info.pcell_name.empty()) {
      ...
      std::pair<bool, pcell_id_type> pc =
          pcell_by_name(info.pcell_name.c_str());
      if (pc.first) {
        get_pcell_variant_as(
            pc.second,
            pcell_declaration(pc.second)->map_parameters(info.pcell_parameters),
            cell_index, layer_mapping);
        return true;
      }

    } else if (!info.cell_name.empty()) {

      //  This should not happen. A cell (given by the cell name) cannot be
      //  proxy to another cell in the same layout.
      tl_assert(false);
    }
  }

  if (!dynamic_cast<db::ColdProxy *>(m_cell_ptrs[cell_index])) {
    //  create a cold proxy representing the context information so we can
    //  restore it
...
db::Cell *Layout::recover_proxy(std::vector<std::string>::const_iterator from,
                                std::vector<std::string>::const_iterator to) {
  if (from == to) {
    return 0;
  }
...
db::Cell *Layout::recover_proxy(const LayoutOrCellContextInfo &info) {
  if (!info.lib_name.empty()) {
...
db::Cell *Layout::recover_proxy_no_lib(const LayoutOrCellContextInfo &info) {
  if (!info.pcell_name.empty()) {

    std::pair<bool, pcell_id_type> pc = pcell_by_name(info.pcell_name.c_str());
    if (pc.first) {
      cell_index_type cell_index = get_pcell_variant(
          pc.second,
          pcell_declaration(pc.second)->map_parameters(info.pcell_parameters));
...
std::string Layout::display_name(cell_index_type cell_index) const {
  return cell(cell_index).get_display_name();
}

std::string Layout::basic_name(cell_index_type cell_index) const {
  return cell(cell_index).get_basic_name();
}

void Layout::register_lib_proxy(db::LibraryProxy *lib_proxy) {
  m_lib_proxy_map.insert(std::make_pair(
      std::make_pair(lib_proxy->lib_id(), lib_proxy->library_cell_index()),
      lib_proxy->Cell::cell_index()));
}

void Layout::unregister_lib_proxy(db::LibraryProxy *lib_proxy) {
  m_lib_proxy_map.erase(
      std::make_pair(lib_proxy->lib_id(), lib_proxy->library_cell_index()));
}

void Layout::get_lib_proxy_as(Library *lib, cell_index_type cell_index,
                              cell_index_type target_cell_index,
                              ImportLayerMapping *layer_mapping,
                              bool retain_layout) {
  tl_assert(m_cell_ptrs[target_cell_index] != 0);
  ...
  LibraryProxy *proxy =
      new LibraryProxy(target_cell_index, *this, lib->get_id(), cell_index);
...
cell_index_type Layout::get_lib_proxy(Library *lib,
                                      cell_index_type cell_index) {
  lib_proxy_map::const_iterator lp =
      m_lib_proxy_map.find(std::make_pair(lib->get_id(), cell_index));
  if (lp != m_lib_proxy_map.end()) {
    ...
    LibraryProxy *proxy =
        new LibraryProxy(new_index, *this, lib->get_id(), cell_index);
...
void Layout::create_cold_proxy_as(const db::LayoutOrCellContextInfo &info,
                                  cell_index_type target_cell_index) {
  tl_assert(m_cell_ptrs[target_cell_index] != 0);
