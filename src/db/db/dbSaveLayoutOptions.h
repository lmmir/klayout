...
public:
   ...
   */
  void add_layer(unsigned int layer,
                 const db::LayerProperties &props = db::LayerProperties());
   ...
   *
   *  Give the cell index of the cell. The actual cell is determined when the
   * layout is given. Clears the "all_cells" flag.
   */
   ...
   */
  bool dont_write_empty_cells() const { return m_dont_write_empty_cells; }

  /**
   *  @brief The "keep instances" property (getter)
   *
   *  With this property set to true, instances of dropped cells (i.e. empty
   * cells, hidden cells) are kept if the format allows. This leads to ghost
   * cells.
   */
  bool keep_instances() const { return m_keep_instances; }
   ...
   */
  void set_keep_instances(bool ki) { m_keep_instances = ki; }

  /**
   *  @brief The "write context information" property (getter)
   *
   *  With this property set to true, context information for cells is written
   * to the file in a suitable way. This way it is possible to recover PCell
   * instantiation information and library references.
   */
  bool write_context_info() const { return m_write_context_info; }
   ...
   *  In this version, the ownership over the options object is not transferred
   * to the SaveLayoutOptions object.
   *
   *  @param options The options to use for writing the file
   *  @param format The format name for which to use these options
   */
  template <class T> void set_options(const T &options) {
   ...
   *
   *  T is a type derived from FormatSpecificWriterOptions.
   *  The ownership over the options object is transferred to the
   * SaveLayoutOptions object.
   *
   *  @param options The options to use for writing the file
   *  @param format The format name for which to use these options
   */
  template <class T> void set_options(T *options) {
   ...
   */
  template <class T> T &get_options() {
    static const T default_format;
    std::map<std::string, FormatSpecificWriterOptions *>::iterator o =
        m_options.find(default_format.format_name());
   ...
   *
   *  This version takes a generic FormatSpecificWriterOptions object and
   * replaces or installs the options under the name delivered by the option
   * object.
   */
   ...
   *  The lm mode specifies how to create layer properties for "halfway defined"
   * layers -
   *    - LP_OnlyNamed will only select named ones
   *    - LP_OnlyNumbered will select only numbered ones
   *    - LP_AssignName will assign a name when no name is given plus encode
   * layer/datatype when given
   *    - LP_AssignNameWithPriority will assign a name when no name is given and
   * does not encore layer/datatype together with a name
   *    - LP_AssignNumber will assign numbers when no number is given
   */
  void get_valid_layers(
      const db::Layout &layout,
      std::vector<std::pair<unsigned int, db::LayerProperties>> &valid_layers,
      LayerAssignmentMode lm) const;

  /**
   *  @brief Report all cells to write
   *
   *  It must be given a list of valid layers which is used to determine empty
   * cells if dont_save_empty_cells is true.
   */
  void get_cells(const db::Layout &layout, std::set<db::cell_index_type> &cells,
                 const std::vector<std::pair<unsigned int, db::LayerProperties>>
                     &valid_layers,
                 bool require_unique_names = true) const;
