 ...
 *
 *  A vector basically describes the relation of two points. q = p + v where v
 * is a vector. Hence, a vector does not describe an absolute but a relative
 * position is space.
 */

template <class C> class DB_PUBLIC_TEMPLATE vector {
public:
   ...
   */
  explicit vector(const point_type &p) : m_x(p.x()), m_y(p.y()) {
    //  .. nothing yet ..
  }
   ...
   */
  vector(const vector<C> &d) : m_x(d.x()), m_y(d.y()) {
    //  .. nothing yet ..
  }
   ...
   *  @param d The source from which to take the data
   */
  template <class D> vector &operator=(const point<C> &d) {
   ...
   *  @return The transformed vector
   */
  template <class Tr> vector &transform(const Tr &t);
   ...
   */
  bool not_equal(const vector<C> &p) const { return !equal(p); }
   ...
   *
   *  Scaling by a double value in general involves rounding when the coordinate
   * type is integer.
   */
...
template <class C> inline vector<C> &vector<C>::operator+=(const vector<C> &p) {
  m_x += p.x();
  m_y += p.y();
  return *this;
}

template <class C> inline vector<C> vector<C>::add(const vector<C> &p) const {
  vector<C> r(*this);
  r += p;
  return r;
}
...
template <class C>
inline vector<C> vector<C>::subtract(const vector<C> &p) const {
  vector<C> r(*this);
  r -= p;
  return r;
}

template <class C> inline bool vector<C>::operator<(const vector<C> &p) const {
  return m_y < p.m_y || (m_y == p.m_y && m_x < p.m_x);
}

template <class C> inline bool vector<C>::less(const vector<C> &p) const {
...
template <class C> inline bool vector<C>::equal(const vector<C> &p) const {
...
template <class C>
template <class Tr>
inline vector<C> &vector<C>::transform(const Tr &t) {
  *this = vector<C>(t(*this));
  return *this;
}

template <class C> inline C vector<C>::x() const { return m_x; }

template <class C> inline C vector<C>::y() const { return m_y; }

template <class C> inline void vector<C>::set_x(C _x) { m_x = _x; }

template <class C> inline void vector<C>::set_y(C _y) { m_y = _y; }

template <class C> inline vector<double> vector<C>::operator*(double s) const {
  return vector<double>(m_x * s, m_y * s);
}

template <class C> inline vector<C> vector<C>::operator*(long s) const {
  return vector<C>(m_x * s, m_y * s);
}

template <class C, typename Number>
inline vector<C> operator/(const db::vector<C> &p, Number s) {
  double mult = 1.0 / static_cast<double>(s);
  return vector<C>(p.x() * mult, p.y() * mult);
}

template <class C> inline vector<C> &vector<C>::operator/=(double s) {
  double mult = 1.0 / static_cast<double>(s);
  *this *= mult;
  return *this;
}

template <class C> inline vector<C> &vector<C>::operator/=(long s) {
  double mult = 1.0 / static_cast<double>(s);
  *this *= mult;
  return *this;
}
...
template <class C>
inline typename vector<C>::distance_type vector<C>::length() const {
  double ddx(x());
  double ddy(y());
  return coord_traits::rounded_distance(sqrt(ddx * ddx + ddy * ddy));
}

template <class C> inline double vector<C>::double_length() const {
  double ddx(x());
  double ddy(y());
...
template <class C>
inline typename vector<C>::area_type vector<C>::sq_length() const {
  return coord_traits::sq_length(0, 0, x(), y());
}

template <class C> inline double vector<C>::sq_double_length() const {
  double ddx(x());
  double ddy(y());
 ...
 */
template <class C>
inline vector<C> operator+(const vector<C> &p1, const vector<C> &p2) {
  vector<C> p(p1);
  p += p2;
  return p;
}
 ...
 *  @return -p = (-p.x, -p.y)
 */
template <class C> inline vector<C> operator-(const vector<C> &p) {
 ...
 */
template <class C>
inline std::ostream &operator<<(std::ostream &os, const vector<C> &p) {
...
/**
 *  @brief Convenience wrappers for coord_traits functions: vector product: 0->p
 * x 0->q
 */
template <class C>
typename db::coord_traits<C>::area_type vprod(const db::vector<C> &p,
                                              const db::vector<C> &q) {
...
/**
 *  @brief Convenience wrappers for coord_traits functions: vector product with
 * sign
 */
template <class C>
std::pair<typename db::coord_traits<C>::area_type, int>
vprod_with_sign(const db::vector<C> &p, const db::vector<C> &q) {
 ...
 */
template <class C>
typename db::coord_traits<C>::area_type sprod(const db::vector<C> &p,
                                              const db::vector<C> &q) {
...
/**
 *  @brief Convenience wrappers for coord_traits functions: scalar product with
 * sign
 */
template <class C>
std::pair<typename db::coord_traits<C>::area_type, int>
sprod_with_sign(const db::vector<C> &p, const db::vector<C> &q) {
 ...
 */
template <class C> struct from_double_vector {
...
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::Vector &p);
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DVector &p);
