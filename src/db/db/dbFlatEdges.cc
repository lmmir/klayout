...
FlatEdges::FlatEdges()
    : MutableEdges(), mp_edges(new db::Shapes(false)),
      mp_merged_edges(new db::Shapes(false)),
      mp_properties_repository(new db::PropertiesRepository()) {
  init();
}
...
FlatEdges::FlatEdges(const FlatEdges &other)
    : MutableEdges(other), mp_edges(other.mp_edges),
      mp_merged_edges(other.mp_merged_edges),
      mp_properties_repository(other.mp_properties_repository) {
  init();
...
FlatEdges::FlatEdges(const db::Shapes &edges, bool is_merged)
    : MutableEdges(), mp_edges(new db::Shapes(edges)),
      mp_merged_edges(new db::Shapes(false)),
      mp_properties_repository(new db::PropertiesRepository()) {
  init();
...
void FlatEdges::set_is_merged(bool m) { m_is_merged = m; }
...
void FlatEdges::insert_into(Layout *layout, db::cell_index_type into_cell,
                            unsigned int into_layer) const {
  db::PropertyMapper pm(&layout->properties_repository(),
                        mp_properties_repository.get_const());
...
void FlatEdges::reserve(size_t n) { mp_edges->reserve(db::Edge::tag(), n); }
...
void FlatEdges::ensure_merged_edges_valid() const {
  if (!m_merged_edges_valid) {
    ...
    if (!need_split_props) {
      ...
      std::map<db::properties_id_type, std::vector<const db::Edge *>>
          edges_by_props;
      ...
      for (auto s2p = edges_by_props.begin(); s2p != edges_by_props.end();
           ++s2p) {
...
FlatEdges::begin_iter() const {
  return std::make_pair(db::RecursiveShapeIterator(*mp_edges),
                        db::ICplxTrans());
}

std::pair<db::RecursiveShapeIterator, db::ICplxTrans>
FlatEdges::begin_merged_iter() const {
  if (!merged_semantics() || m_is_merged) {
    return begin_iter();
  } else {
    ensure_merged_edges_valid();
    return std::make_pair(db::RecursiveShapeIterator(*mp_merged_edges),
                          db::ICplxTrans());
  }
}

bool FlatEdges::empty() const { return mp_edges->empty(); }

size_t FlatEdges::count() const { return mp_edges->size(); }

size_t FlatEdges::hier_count() const { return mp_edges->size(); }

bool FlatEdges::is_merged() const { return m_is_merged; }

Box FlatEdges::compute_bbox() const { return mp_edges->bbox(); }

EdgesDelegate *FlatEdges::processed_in_place(const EdgeProcessorBase &filter) {
  std::vector<db::Edge> edge_res;

  db::Shapes &e = *mp_edges;

  edge_iterator_type pw =
      e.get_layer<db::Edge, db::unstable_layer_tag>().begin();
  edge_iterator_wp_type pw_wp =
      e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().begin();

  for (EdgesIterator p(filter.requires_raw_input() ? begin() : begin_merged());
       !p.at_end(); ++p) {

    edge_res.clear();
    filter.process(*p, edge_res);

    for (std::vector<db::Edge>::const_iterator pr = edge_res.begin();
         pr != edge_res.end(); ++pr) {
      if (p.prop_id() != 0) {
        if (pw_wp ==
            e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>()
                .end()) {
          e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().insert(
              db::EdgeWithProperties(*pr, p.prop_id()));
          pw_wp = e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>()
                      .end();
        } else {
          e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().replace(
              pw_wp++, db::EdgeWithProperties(*pr, p.prop_id()));
        }
      } else {
  ...
  e.get_layer<db::Edge, db::unstable_layer_tag>().erase(
      pw, e.get_layer<db::Edge, db::unstable_layer_tag>().end());
  e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().erase(
      pw_wp,
      e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().end());
...
EdgesDelegate *FlatEdges::filter_in_place(const EdgeFilterBase &filter) {
  db::Shapes &e = *mp_edges;
      ...
      if (p.prop_id() != 0) {
        if (pw_wp ==
            e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>()
                .end()) {
          e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().insert(
              db::EdgeWithProperties(*p, p.prop_id()));
          pw_wp = e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>()
                      .end();
        } else {
          e.get_layer<db::EdgeWithProperties, db::unstable_layer_tag>().replace(
              pw_wp++, db::EdgeWithProperties(*p, p.prop_id()));
        }
      } else {
        if (pw == e.get_layer<db::Edge, db::unstable_layer_tag>().end()) {
...
EdgesDelegate *FlatEdges::add(const Edges &other) const {
  ...
  if (other_flat) {
    ...
    new_region->raw_edges().insert(
        other_flat->raw_edges()
            .get_layer<db::Edge, db::unstable_layer_tag>()
            .begin(),
        other_flat->raw_edges()
            .get_layer<db::Edge, db::unstable_layer_tag>()
            .end());
...
const db::Edge *FlatEdges::nth(size_t n) const {
  return n < mp_edges->size()
             ? &mp_edges->get_layer<db::Edge, db::unstable_layer_tag>()
                    .begin()[n]
             : 0;
}

bool FlatEdges::has_valid_edges() const { return true; }

bool FlatEdges::has_valid_merged_edges() const { return true; }

const db::RecursiveShapeIterator *FlatEdges::iter() const { return 0; }
...
void FlatEdges::do_insert(const db::Edge &edge,
