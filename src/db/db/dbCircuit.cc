...
Circuit::Circuit()
    : db::NetlistObject(), gsi::ObjectBase(), m_dont_purge(false),
      m_cell_index(0), mp_netlist(0),
      m_device_by_id(this, &Circuit::begin_devices, &Circuit::end_devices),
      m_subcircuit_by_id(this, &Circuit::begin_subcircuits,
                         &Circuit::end_subcircuits),
      ...
      m_subcircuit_by_name(this, &Circuit::begin_subcircuits,
                           &Circuit::end_subcircuits),
...
Circuit::Circuit(const db::Layout &layout, db::cell_index_type ci)
    : db::NetlistObject(), gsi::ObjectBase(), m_name(layout.cell_name(ci)),
      m_dont_purge(false), m_cell_index(ci), mp_netlist(0),
      m_device_by_id(this, &Circuit::begin_devices, &Circuit::end_devices),
      m_subcircuit_by_id(this, &Circuit::begin_subcircuits,
                         &Circuit::end_subcircuits),
      m_net_by_cluster_id(this, &Circuit::begin_nets, &Circuit::end_nets),
  ...
  set_boundary(
      db::DPolygon(db::CplxTrans(layout.dbu()) * layout.cell(ci).bbox()));
}

Circuit::Circuit(const Circuit &other)
    : db::NetlistObject(other), gsi::ObjectBase(other), m_dont_purge(false),
      m_cell_index(0), mp_netlist(0),
      m_device_by_id(this, &Circuit::begin_devices, &Circuit::end_devices),
      m_subcircuit_by_id(this, &Circuit::begin_subcircuits,
                         &Circuit::end_subcircuits),
...
Circuit &Circuit::operator=(const Circuit &other) {
  if (this != &other) {
    ...
    for (const_device_iterator i = other.begin_devices();
         i != other.end_devices(); ++i) {
    ...
    for (const_subcircuit_iterator i = other.begin_subcircuits();
         i != other.end_subcircuits(); ++i) {
      SubCircuit *sc = new SubCircuit(*i);
      sc_table[i.operator->()] = sc;
      add_subcircuit(sc);
    }

    for (const_net_iterator i = other.begin_nets(); i != other.end_nets();
         ++i) {
      ...
      for (Net::const_terminal_iterator p = i->begin_terminals();
           p != i->end_terminals(); ++p) {
        std::map<const Device *, Device *>::const_iterator m =
            device_table.find(p->device());
        tl_assert(m != device_table.end());
        n->add_terminal(NetTerminalRef(m->second, p->terminal_id()));
      }

      for (Net::const_pin_iterator p = i->begin_pins(); p != i->end_pins();
           ++p) {
        n->add_pin(NetPinRef(p->pin_id()));
      }

      for (Net::const_subcircuit_pin_iterator p = i->begin_subcircuit_pins();
           p != i->end_subcircuit_pins(); ++p) {
        std::map<const SubCircuit *, SubCircuit *>::const_iterator m =
            sc_table.find(p->subcircuit());
...
void Circuit::set_netlist(Netlist *netlist) { mp_netlist = netlist; }
...
void Circuit::set_dont_purge(bool dp) { m_dont_purge = dp; }
...
Circuit::const_child_circuit_iterator Circuit::begin_children() const {
  ...
  return reinterpret_cast<const tl::vector<const Circuit *> &>(
             mp_netlist->child_circuits(const_cast<Circuit *>(this)))
      .begin();
}
...
Circuit::const_child_circuit_iterator Circuit::begin_parents() const {
  tl_assert(mp_netlist != 0);
  return reinterpret_cast<const tl::vector<const Circuit *> &>(
             mp_netlist->parent_circuits(const_cast<Circuit *>(this)))
      .begin();
}
...
Net *Circuit::net_by_name(const std::string &name) {
  return m_net_by_name.object_by(mp_netlist ? mp_netlist->normalize_name(name)
                                            : name);
}
...
void Circuit::join_nets(Net *net, Net *with) {
  ...
  while (with->begin_terminals() != with->end_terminals()) {
    db::Device *device =
        const_cast<db::Device *>(with->begin_terminals()->device());
    device->connect_terminal(with->begin_terminals()->terminal_id(), net);
  }

  while (with->begin_subcircuit_pins() != with->end_subcircuit_pins()) {
    db::SubCircuit *subcircuit = const_cast<db::SubCircuit *>(
        with->begin_subcircuit_pins()->subcircuit());
...
Device *Circuit::device_by_name(const std::string &name) {
  return m_device_by_name.object_by(
      mp_netlist ? mp_netlist->normalize_name(name) : name);
}
...
void Circuit::remove_subcircuit(SubCircuit *subcircuit) {
  ...
  if (subcircuit->circuit() != this) {
    throw tl::Exception(
        tl::to_string(tr("Subcircuit not withing given circuit")));
  }
...
SubCircuit *Circuit::subcircuit_by_name(const std::string &name) {
  return m_subcircuit_by_name.object_by(
      mp_netlist ? mp_netlist->normalize_name(name) : name);
}

void Circuit::register_ref(SubCircuit *r) { m_refs.push_back(r); }

void Circuit::unregister_ref(SubCircuit *r) { m_refs.erase(r); }
...
void Circuit::flatten_subcircuit(SubCircuit *subcircuit) {
  ...
  for (db::Circuit::const_net_iterator n = c->begin_nets(); n != c->end_nets();
    ...
    if (n->pin_count() > 0) {
      ...
      for (db::Net::const_pin_iterator p = n->begin_pins(); p != n->end_pins();
           ++p) {
      ...
      if (netlist()->callbacks()) {
        outside_net->set_cluster_id(
            netlist()->callbacks()->link_net_to_parent_circuit(
                n.operator->(), this, subcircuit->trans()));
      }
    }
  ...
  for (db::Circuit::const_device_iterator d = c->begin_devices();
    ...
    for (std::vector<db::DeviceTerminalDefinition>::const_iterator t =
      ...
      if (tnet) {
        std::map<const db::Net *, db::Net *>::const_iterator n2n =
            net2net.find(tnet);
  ...
  for (db::Circuit::const_subcircuit_iterator sc = c->begin_subcircuits();
    ...
    if (!new_subcircuit->name().empty()) {
      new_subcircuit->set_name(subcircuit->expanded_name() + "." +
                               new_subcircuit->name());
    }
...
void Circuit::translate_circuits(
    const std::map<const Circuit *, Circuit *> &map) {
  for (subcircuit_iterator i = m_subcircuits.begin(); i != m_subcircuits.end();
       ++i) {
    std::map<const Circuit *, Circuit *>::const_iterator m =
        map.find(i->circuit_ref());
    tl_assert(m != map.end());
    i->set_circuit_ref(m->second);
  }
}

void Circuit::translate_device_classes(
    const std::map<const DeviceClass *, DeviceClass *> &map) {
  for (device_iterator i = m_devices.begin(); i != m_devices.end(); ++i) {
    std::map<const DeviceClass *, DeviceClass *>::const_iterator m =
        map.find(i->device_class());
    tl_assert(m != map.end());
    i->set_device_class(m->second);
  }
}

void Circuit::translate_device_abstracts(
    const std::map<const DeviceAbstract *, DeviceAbstract *> &map) {
  for (device_iterator i = m_devices.begin(); i != m_devices.end(); ++i) {
...
void Circuit::blank() {
  ...
  //  weak pointers are good because deleting a subcircuit might delete others
  //  ahead in this list:
  std::list<tl::weak_ptr<db::Circuit>> called_circuits;
  for (std::set<db::Circuit *>::const_iterator c = cs.begin(); c != cs.end();
       ++c) {
  ...
  for (std::list<tl::weak_ptr<db::Circuit>>::iterator c =
           called_circuits.begin();
       c != called_circuits.end(); ++c) {
    if (c->get() && !(*c)->has_refs()) {
...
void Circuit::purge_nets_keep_pins() { do_purge_nets(true); }

void Circuit::purge_nets() { do_purge_nets(false); }
...
void Circuit::do_purge_nets(bool keep_pins) {
  ...
  for (std::vector<db::Net *>::const_iterator n = nets_to_be_purged.begin();
       n != nets_to_be_purged.end(); ++n) {
    if (!keep_pins) {
      for (db::Net::pin_iterator p = (*n)->begin_pins(); p != (*n)->end_pins();
           ++p) {
  ...
  if (!pins_to_delete.empty()) {
    ...
    for (refs_iterator r = begin_refs(); r != end_refs(); ++r) {
      ...
      for (std::set<size_t>::const_iterator p = pins_to_delete.begin();
           p != pins_to_delete.end(); ++p) {
        db::Net *net = subcircuit->net_for_pin(*p);
        for (db::Net::subcircuit_pin_iterator sp = net->begin_subcircuit_pins();
             sp != net->end_subcircuit_pins(); ++sp) {
...
static void check_device_before_remove(db::Circuit *c, const db::Device *d) {
  if (d->device_class() == 0) {
    throw tl::Exception(
        tl::to_string(tr("Internal error: No device class after removing "
                         "device in device combination")) +
        ": name=" + d->name() + ", circuit=" + c->name());
  }
  const std::vector<db::DeviceTerminalDefinition> &pd =
      d->device_class()->terminal_definitions();
  for (std::vector<db::DeviceTerminalDefinition>::const_iterator p = pd.begin();
       p != pd.end(); ++p) {
    if (d->net_for_terminal(p->id()) != 0) {
      throw tl::Exception(
          tl::to_string(tr("Internal error: Terminal still connected after "
                           "removing device in device combination")) +
          ": name=" + d->name() + ", circuit=" + c->name() +
          ", terminal=" + p->name());
    }
  }
...
bool Circuit::combine_parallel_devices(const db::DeviceClass &cls) {
  ...
  //  identify the candidates for combination - all devices sharing the same
  //  nets are candidates for combination in parallel mode
  for (device_iterator d = begin_devices(); d != end_devices(); ++d) {
    ...
    key_type k;
    const std::vector<db::DeviceTerminalDefinition> &terminals =
        cls.terminal_definitions();
    for (std::vector<db::DeviceTerminalDefinition>::const_iterator p =
             terminals.begin();
         p != terminals.end(); ++p) {
  ...
  for (std::map<key_type, std::vector<db::Device *>>::iterator cc =
           combination_candidates.begin();
       cc != combination_candidates.end(); ++cc) {
...
static std::pair<db::Device *, db::Device *>
attached_two_devices(db::Net &net, const db::DeviceClass &cls) {
  ...
  if (p == net.end_terminals() ||
      tl::id_of(p->device_class()) != tl::id_of(&cls)) {
    return std::make_pair((db::Device *)0, (db::Device *)0);
  } else {
    d1 = p->device();
  }
...
template <class T>
static bool same_or_swapped(const std::pair<T, T> &p1,
...
bool Circuit::combine_serial_devices(const db::DeviceClass &cls) {
  ...
  for (net_iterator n = begin_nets(); n != end_nets(); ++n) {
    ...
    //  The net is an internal node: the devices attached to this internal node
    //  are combination candidates if the number of nets emerging from the
    //  attached device pair (not counting the internal node we just found) does
    //  not exceed the number of pins available for the new device.
    ...
    other_nets.erase(std::unique(other_nets.begin(), other_nets.end()),
                     other_nets.end());
...
void Circuit::combine_devices() {
  ...
  for (Netlist::device_class_iterator dc = netlist()->begin_device_classes();
       dc != netlist()->end_device_classes(); ++dc) {

    //  repeat the combination step unless no combination happens - this is
    //  required to take care of combinations that arise after other
    //  combinations have been realized.
    bool any = true;
    while (any) {
