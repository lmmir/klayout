 ...
 */
template <class C> class generic_polygon_edge_iterator {
public:
  ...
  typedef
      typename polygon_type::polygon_edge_iterator polygon_edge_iterator_type;
  typedef db::simple_polygon<C> simple_polygon_type;
  typedef typename simple_polygon_type::polygon_edge_iterator
      simple_polygon_edge_iterator_type;
  typedef db::polygon_ref<polygon_type, disp_type> polygon_ref_type;
  typedef typename polygon_ref_type::polygon_edge_iterator
      polygon_ref_edge_iterator_type;
  typedef db::polygon_ref<simple_polygon_type, disp_type>
      simple_polygon_ref_type;
  typedef typename simple_polygon_ref_type::polygon_edge_iterator
      simple_polygon_ref_edge_iterator_type;
   ...
   *  @brief Default ctor
   */
  generic_polygon_edge_iterator() { m_type = None; }
  ...
  /**
   *  @brief This constructor creates an edge iterator at "begin" for polygon
   * references
   *
   *  See former ctor.
   */
  ...
  /**
   *  @brief This constructor creates an edge iterator at "begin" for simple
   * polygons
   *
   *  See former ctor.
   */
  generic_polygon_edge_iterator(
      const simple_polygon_edge_iterator_type &begin) {
   ...
   */
  generic_polygon_edge_iterator(
      const simple_polygon_ref_edge_iterator_type &begin) {
   ...
   */
  bool at_end() const { return generic_const_f<bool, at_end_f>(); }
   ...
   */
  edge_type operator*() const { return generic_const_f<edge_type, deref_f>(); }
...
private:
  ...
  //  The strange construction and the local dummy class helps to guarantee
  //  alignment of the "iter" space
  union {
    class _align_helper {
      long l;
    } _ah;
    //  Hint: without the sizeof(size_t) offset, gcc 4.4.1 produces invalid
    //  iterators for the edge iterator. For example, these iterators do not
    //  correctly stop but produce more edges than are available.
  ...
  struct contour_f {
    ...
    unsigned int operator()() const { return 0; }

    template <class Iter> unsigned int operator()(const Iter &iter) const {
      return iter.contour();
    }
  };
  ...
  struct at_end_f {
    ...
    template <class Iter> bool operator()(const Iter &iter) const {
      return iter.at_end();
    }
  };
  ...
  struct inc_f {
    ...
    template <class Iter> bool operator()(Iter &iter) const {
      iter.operator++();
      return false;
    }
  };
  ...
  struct deref_f {
    ...
    edge_type operator()() const { return edge_type(); }

    template <class Iter> edge_type operator()(const Iter &iter) const {
  ...
  template <class Ret, class Func> Ret generic_const_f() const {
    Func f;
    if (m_type == Polygon) {
  ...
  template <class Ret, class Func> Ret generic_f() {
    Func f;
    if (m_type == Polygon) {
 ...
 *  copy semantics but can be copied by bytewise copy simply.
 */
template <class C> class generic_point_iterator {
public:
  typedef C coord_type;
  typedef db::point<C> point_type;
  typedef db::polygon<C> polygon_type;
  typedef db::disp_trans<C> disp_type;
  typedef typename polygon_type::polygon_contour_iterator
      polygon_point_iterator_type;
  typedef db::polygon_ref<polygon_type, disp_type> polygon_ref_type;
  typedef typename polygon_ref_type::polygon_contour_iterator
      polygon_ref_point_iterator_type;
  ...
  enum iterator_type { Polygon, PolygonRef, Path, PathRef };
  ...
  /**
   *  @brief This constructor creates an point iterator at "begin" for polygon
   * references
   *
  ...
  /**
   *  @brief This constructor creates an point iterator at "begin" for path
   * references
   *
   *  See former ctor.
   */
  ...
  bool operator<(const generic_point_iterator &d) const {
    ...
    if (m_type == Polygon) {
      return *((const polygon_point_iterator_type *)m_d.iter) <
             *((const polygon_point_iterator_type *)d.m_d.iter);
    } else if (m_type == PolygonRef) {
      return *((const polygon_ref_point_iterator_type *)m_d.iter) <
             *((const polygon_ref_point_iterator_type *)d.m_d.iter);
    } else if (m_type == Path) {
      return *((const path_point_iterator_type *)m_d.iter) <
             *((const path_point_iterator_type *)d.m_d.iter);
    } else {
      return *((const path_ref_point_iterator_type *)m_d.iter) <
             *((const path_ref_point_iterator_type *)d.m_d.iter);
    }
  }
...
private:
  ...
  union {
    ...
    //  Hint: without the sizeof(size_t) offset, gcc 4.4.1 produces invalid
    //  iterators for the edge iterator. For safety the offset is added here as
    //  well:
    char iter[sizeof(iter_size) + sizeof(size_t)];
  } m_d;
  ...
  struct deref_f {
    template <class Iter> point_type operator()(const Iter &iter) const {
      return iter.operator*();
    }
  };
...
public:
  ...
  typedef db::polygon_ref<simple_polygon_type, disp_type>
      simple_polygon_ref_type;
  typedef db::polygon_ref<simple_polygon_type, unit_trans_type>
      simple_polygon_ptr_type;
  typedef db::array<simple_polygon_ptr_type, disp_type>
      simple_polygon_ptr_array_type;
  ...
  typedef db::box<coord_type, coord_traits<coord_type>::short_coord_type>
      short_box_type;
  ...
  typedef tl::reuse_vector<db::polygon<coord_type>>::const_iterator
      polygon_iter_type;
  typedef tl::reuse_vector<db::simple_polygon<coord_type>>::const_iterator
      simple_polygon_iter_type;
  typedef tl::reuse_vector<db::polygon_ref<polygon_type, disp_type>>::
      const_iterator polygon_ref_iter_type;
  typedef tl::reuse_vector<db::polygon_ref<polygon_type, unit_trans_type>>::
      const_iterator polygon_ptr_iter_type;
  typedef tl::reuse_vector<db::array<polygon_ptr_type, disp_type>>::
      const_iterator polygon_ptr_array_iter_type;
  typedef tl::reuse_vector<db::polygon_ref<simple_polygon_type, disp_type>>::
      const_iterator simple_polygon_ref_iter_type;
  typedef tl::reuse_vector<
      db::polygon_ref<simple_polygon_type, unit_trans_type>>::const_iterator
      simple_polygon_ptr_iter_type;
  typedef tl::reuse_vector<db::array<simple_polygon_ptr_type, disp_type>>::
      const_iterator simple_polygon_ptr_array_iter_type;
  typedef tl::reuse_vector<db::path<coord_type>>::const_iterator path_iter_type;
  typedef tl::reuse_vector<db::path_ref<path_type, disp_type>>::const_iterator
      path_ref_iter_type;
  typedef tl::reuse_vector<db::path_ref<path_type, unit_trans_type>>::
      const_iterator path_ptr_iter_type;
  typedef tl::reuse_vector<db::array<path_ptr_type, disp_type>>::const_iterator
      path_ptr_array_iter_type;
  typedef tl::reuse_vector<db::edge<coord_type>>::const_iterator edge_iter_type;
  typedef tl::reuse_vector<db::edge_pair<coord_type>>::const_iterator
      edge_pair_iter_type;
  typedef tl::reuse_vector<db::point<coord_type>>::const_iterator
      point_iter_type;
  typedef tl::reuse_vector<db::text<coord_type>>::const_iterator text_iter_type;
  typedef tl::reuse_vector<db::text_ref<text_type, disp_type>>::const_iterator
      text_ref_iter_type;
  typedef tl::reuse_vector<db::text_ref<text_type, unit_trans_type>>::
      const_iterator text_ptr_iter_type;
  typedef tl::reuse_vector<db::array<text_ptr_type, disp_type>>::const_iterator
      text_ptr_array_iter_type;
  typedef tl::reuse_vector<db::user_object<coord_type>>::const_iterator
      user_object_iter_type;
  typedef tl::reuse_vector<db::box<coord_type>>::const_iterator box_iter_type;
  typedef tl::reuse_vector<db::array<box_type, unit_trans_type>>::const_iterator
      box_array_iter_type;
  typedef tl::reuse_vector<db::box<
      coord_type, coord_traits<coord_type>::short_coord_type>>::const_iterator
      short_box_iter_type;
  typedef tl::reuse_vector<db::array<short_box_type, unit_trans_type>>::
      const_iterator short_box_array_iter_type;

  typedef tl::reuse_vector<
      db::object_with_properties<db::polygon<coord_type>>>::const_iterator
      ppolygon_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::simple_polygon<coord_type>>>::const_iterator
      psimple_polygon_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::polygon_ref<polygon_type, disp_type>>>::const_iterator
      ppolygon_ref_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::polygon_ref<polygon_type, unit_trans_type>>>::const_iterator
      ppolygon_ptr_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<polygon_ptr_type, disp_type>>>::const_iterator
      ppolygon_ptr_array_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::polygon_ref<simple_polygon_type, disp_type>>>::const_iterator
      psimple_polygon_ref_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::polygon_ref<simple_polygon_type, unit_trans_type>>>::const_iterator
      psimple_polygon_ptr_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<simple_polygon_ptr_type, disp_type>>>::const_iterator
      psimple_polygon_ptr_array_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::path<coord_type>>>::
      const_iterator ppath_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::path_ref<path_type, disp_type>>>::const_iterator ppath_ref_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::path_ref<path_type, unit_trans_type>>>::const_iterator
      ppath_ptr_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<path_ptr_type, disp_type>>>::const_iterator
      ppath_ptr_array_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::edge<coord_type>>>::
      const_iterator pedge_iter_type;
  typedef tl::reuse_vector<
      db::object_with_properties<db::edge_pair<coord_type>>>::const_iterator
      pedge_pair_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::point<coord_type>>>::
      const_iterator ppoint_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::text<coord_type>>>::
      const_iterator ptext_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::text_ref<text_type, disp_type>>>::const_iterator ptext_ref_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::text_ref<text_type, unit_trans_type>>>::const_iterator
      ptext_ptr_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<text_ptr_type, disp_type>>>::const_iterator
      ptext_ptr_array_iter_type;
  typedef tl::reuse_vector<
      db::object_with_properties<db::user_object<coord_type>>>::const_iterator
      puser_object_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::box<coord_type>>>::
      const_iterator pbox_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<box_type, unit_trans_type>>>::const_iterator
      pbox_array_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<db::box<
      coord_type, coord_traits<coord_type>::short_coord_type>>>::const_iterator
      pshort_box_iter_type;
  typedef tl::reuse_vector<db::object_with_properties<
      db::array<short_box_type, unit_trans_type>>>::const_iterator
      pshort_box_array_iter_type;

  //  Hint: in the following enum it is important that the ArrayMember types
  //  come *after* the non-member types. See db::shapes::erase with multi-shape
  //  erase capabilities.
  enum object_type {
   ...
   */
  Shape() : mp_shapes(0), m_with_props(false), m_stable(false), m_type(Null) {
    m_generic.any = 0;
  }
  ...
  Shape(const db::Shapes *shapes, const Obj &obj)
      : mp_shapes(const_cast<db::Shapes *>(shapes)), m_with_props(false),
        m_stable(false) {
   ...
   *  @brief Construct as a proxy to a certain object
   *
   *  This form will copy the object itself. A temporary object may be passed to
   * "obj". An implementation is provided only for the array types
   *
   *  Hint: there is no "const shape reference" currently and having one would
   * mean we would probably create too much constness. Hence we use const_cast
   * to convert it to a non-const one.
   */
  ...
  Shape(const db::Shapes *shapes,
        const tl::reuse_vector_const_iterator<Obj, R> &tree_iter)
      : mp_shapes(const_cast<db::Shapes *>(shapes)), m_with_props(false),
        m_stable(true) {
    typename Obj::tag tag = typename Obj::tag();
    ...
    new ((char *)m_generic.iter)
        typename tl::reuse_vector<Obj>::const_iterator(tree_iter);
  }
   ...
   */
  db::Shapes *shapes() const { return mp_shapes; }
  ...
  /**
   *  @brief Construct a shape proxy as a shape with properties for an array
   * member
   *
   *  This form will copy the object itself. A temporary object may be passed to
   * "obj". An implementation is provided only for a few types. This forwards
   * the initialisation to the basic shape type.
   */
  template <class Sh, class Trans>
  void init_member(db::object_tag<db::object_with_properties<Sh>>,
                   const Trans &trans) {
   ...
   */
  void init(polygon_type::tag) { m_type = Polygon; }

  /**
   *  @brief Construct a shape proxy as a reference to a polygon reference
   */
  void init(polygon_ref_type::tag) { m_type = PolygonRef; }
   ...
   */
  void init(polygon_ptr_array_type::tag) { m_type = PolygonPtrArray; }

  /**
   *  @brief Construct a shape proxy as a reference to a simple polygon
   */
  void init(simple_polygon_type::tag) { m_type = SimplePolygon; }

  /**
   *  @brief Construct a shape proxy as a reference to a polygon reference
   */
  void init(simple_polygon_ref_type::tag) { m_type = SimplePolygonRef; }

  /**
   *  @brief Construct a shape proxy as a reference to a simple polygon array
   * member
   */
  void init_member(simple_polygon_ptr_array_type::tag,
   ...
   */
  void init(path_type::tag) { m_type = Path; }

  /**
   *  @brief Construct a shape proxy as a reference to a path reference
   */
  void init(path_ref_type::tag) { m_type = PathRef; }
   ...
   */
  void init(path_ptr_array_type::tag) { m_type = PathPtrArray; }

  /**
   *  @brief Construct a shape proxy as a reference to a text
   */
  void init(text_type::tag) { m_type = Text; }

  /**
   *  @brief Construct a shape proxy as a reference to a text reference
   */
  void init(text_ref_type::tag) { m_type = TextRef; }
   ...
   */
  void init(box_array_type::tag) { m_type = BoxArray; }

  /**
   *  @brief Construct a shape proxy as a reference to a small box
   */
  void init(short_box_type::tag) { m_type = ShortBox; }
   ...
   *  @brief Construct a shape proxy as a reference to a box array
   */
  void init(short_box_array_type::tag) { m_type = ShortBoxArray; }

  /**
   *  @brief Construct a shape proxy as a reference to a user object
   */
  void init(user_object_type::tag) { m_type = UserObject; }
   ...
   */
  bool with_props() const { return m_with_props; }
   ...
   */
  bool has_prop_id() const { return m_with_props; }
   ...
   *
   *  This is a generalization of the polygon (), etc. methods using a tag to
   * identify the target object.
   */
  const polygon_type *basic_ptr(polygon_type::tag) const {
    tl_assert(m_type == Polygon);
    if (m_stable) {
      return m_with_props ? &**(((ppolygon_iter_type *)m_generic.iter))
                          : &**(((polygon_iter_type *)m_generic.iter));
    } else {
  ...
  const simple_polygon_type *basic_ptr(simple_polygon_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((psimple_polygon_iter_type *)m_generic.iter))
                          : &**(((simple_polygon_iter_type *)m_generic.iter));
    } else {
      return m_with_props ? m_generic.psimple_polygon
                          : m_generic.simple_polygon;
    }
  }
  ...
  const polygon_ref_type *basic_ptr(polygon_ref_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ppolygon_ref_iter_type *)m_generic.iter))
                          : &**(((polygon_ref_iter_type *)m_generic.iter));
    } else {
  ...
  const polygon_ptr_array_type *basic_ptr(polygon_ptr_array_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props
                 ? &**(((ppolygon_ptr_array_iter_type *)m_generic.iter))
                 : &**(((polygon_ptr_array_iter_type *)m_generic.iter));
    } else {
  ...
  const simple_polygon_ref_type *basic_ptr(simple_polygon_ref_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props
                 ? &**(((psimple_polygon_ref_iter_type *)m_generic.iter))
                 : &**(((simple_polygon_ref_iter_type *)m_generic.iter));
    } else {
      return m_with_props ? m_generic.psimple_polygon_ref
                          : m_generic.simple_polygon_ref;
    }
  }
   ...
   * identify the target object.
   */
  const simple_polygon_ptr_array_type *
  basic_ptr(simple_polygon_ptr_array_type::tag) const {
    tl_assert(m_type == SimplePolygonPtrArray ||
              m_type == SimplePolygonPtrArrayMember);
    if (m_stable) {
      return m_with_props
                 ? &**(((psimple_polygon_ptr_array_iter_type *)m_generic.iter))
                 : &**(((simple_polygon_ptr_array_iter_type *)m_generic.iter));
    } else {
      return m_with_props ? m_generic.psimple_polygon_aref
                          : m_generic.simple_polygon_aref;
    }
  }
  ...
  const path_type *basic_ptr(path_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ppath_iter_type *)m_generic.iter))
                          : &**(((path_iter_type *)m_generic.iter));
    } else {
  ...
  const path_ref_type *basic_ptr(path_ref_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ppath_ref_iter_type *)m_generic.iter))
                          : &**(((path_ref_iter_type *)m_generic.iter));
    } else {
  ...
  const path_ptr_array_type *basic_ptr(path_ptr_array_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ppath_ptr_array_iter_type *)m_generic.iter))
                          : &**(((path_ptr_array_iter_type *)m_generic.iter));
    } else {
  ...
  const edge_type *basic_ptr(edge_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pedge_iter_type *)m_generic.iter))
                          : &**(((edge_iter_type *)m_generic.iter));
    } else {
  ...
  const edge_pair_type *basic_ptr(edge_pair_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pedge_pair_iter_type *)m_generic.iter))
                          : &**(((edge_pair_iter_type *)m_generic.iter));
    } else {
  ...
  const point_type *basic_ptr(point_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ppoint_iter_type *)m_generic.iter))
                          : &**(((point_iter_type *)m_generic.iter));
    } else {
  ...
  const text_type *basic_ptr(text_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ptext_iter_type *)m_generic.iter))
                          : &**(((text_iter_type *)m_generic.iter));
    } else {
  ...
  const text_ref_type *basic_ptr(text_ref_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ptext_ref_iter_type *)m_generic.iter))
                          : &**(((text_ref_iter_type *)m_generic.iter));
    } else {
  ...
  const text_ptr_array_type *basic_ptr(text_ptr_array_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((ptext_ptr_array_iter_type *)m_generic.iter))
                          : &**(((text_ptr_array_iter_type *)m_generic.iter));
    } else {
  ...
  const user_object_type *basic_ptr(user_object_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((puser_object_iter_type *)m_generic.iter))
                          : &**(((user_object_iter_type *)m_generic.iter));
    } else {
  ...
  const box_type *basic_ptr(box_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pbox_iter_type *)m_generic.iter))
                          : &**(((box_iter_type *)m_generic.iter));
    } else {
  ...
  const box_array_type *basic_ptr(box_array_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pbox_array_iter_type *)m_generic.iter))
                          : &**(((box_array_iter_type *)m_generic.iter));
    } else {
  ...
  const short_box_type *basic_ptr(short_box_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pshort_box_iter_type *)m_generic.iter))
                          : &**(((short_box_iter_type *)m_generic.iter));
    } else {
  ...
  const short_box_array_type *basic_ptr(short_box_array_type::tag) const {
    ...
    if (m_stable) {
      return m_with_props ? &**(((pshort_box_array_iter_type *)m_generic.iter))
                          : &**(((short_box_array_iter_type *)m_generic.iter));
    } else {
      return m_with_props ? m_generic.pshort_box_array
                          : m_generic.short_box_array;
    }
  }
   ...
   * identify the target object.
   */
  const db::object_with_properties<polygon_type> *
  basic_ptr(db::object_with_properties<polygon_type>::tag) const {
    tl_assert(m_type == Polygon);
   ...
   * identify the target object.
   */
  const db::object_with_properties<simple_polygon_type> *
  basic_ptr(db::object_with_properties<simple_polygon_type>::tag) const {
    tl_assert(m_type == SimplePolygon);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<polygon_ref_type> *
  basic_ptr(db::object_with_properties<polygon_ref_type>::tag) const {
    tl_assert(m_type == PolygonRef);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<polygon_ptr_array_type> *
  basic_ptr(db::object_with_properties<polygon_ptr_array_type>::tag) const {
    tl_assert(m_type == PolygonPtrArray || m_type == PolygonPtrArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<simple_polygon_ref_type> *
  basic_ptr(db::object_with_properties<simple_polygon_ref_type>::tag) const {
    tl_assert(m_type == SimplePolygonRef);
    tl_assert(m_with_props);
    if (m_stable) {
  ...
  const db::object_with_properties<simple_polygon_ptr_array_type> *basic_ptr(
      db::object_with_properties<simple_polygon_ptr_array_type>::tag) const {
    tl_assert(m_type == SimplePolygonPtrArray ||
              m_type == SimplePolygonPtrArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<path_type> *
  basic_ptr(db::object_with_properties<path_type>::tag) const {
    tl_assert(m_type == Path);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<path_ref_type> *
  basic_ptr(db::object_with_properties<path_ref_type>::tag) const {
    tl_assert(m_type == PathRef);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<path_ptr_array_type> *
  basic_ptr(db::object_with_properties<path_ptr_array_type>::tag) const {
    tl_assert(m_type == PathPtrArray || m_type == PathPtrArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<edge_type> *
  basic_ptr(db::object_with_properties<edge_type>::tag) const {
    tl_assert(m_type == Edge);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<edge_pair_type> *
  basic_ptr(db::object_with_properties<edge_pair_type>::tag) const {
    tl_assert(m_type == EdgePair);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<point_type> *
  basic_ptr(db::object_with_properties<point_type>::tag) const {
    tl_assert(m_type == Point);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<text_type> *
  basic_ptr(db::object_with_properties<text_type>::tag) const {
    tl_assert(m_type == Text);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<text_ref_type> *
  basic_ptr(db::object_with_properties<text_ref_type>::tag) const {
    tl_assert(m_type == TextRef);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<text_ptr_array_type> *
  basic_ptr(db::object_with_properties<text_ptr_array_type>::tag) const {
    tl_assert(m_type == TextPtrArray || m_type == TextPtrArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<user_object_type> *
  basic_ptr(db::object_with_properties<user_object_type>::tag) const {
    tl_assert(m_type == UserObject);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<box_type> *
  basic_ptr(db::object_with_properties<box_type>::tag) const {
    tl_assert(m_type == Box);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<box_array_type> *
  basic_ptr(db::object_with_properties<box_array_type>::tag) const {
    tl_assert(m_type == BoxArray || m_type == BoxArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<short_box_type> *
  basic_ptr(db::object_with_properties<short_box_type>::tag) const {
    tl_assert(m_type == ShortBox);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   * identify the target object.
   */
  const db::object_with_properties<short_box_array_type> *
  basic_ptr(db::object_with_properties<short_box_array_type>::tag) const {
    tl_assert(m_type == ShortBoxArray || m_type == ShortBoxArrayMember);
    tl_assert(m_with_props);
    if (m_stable) {
   ...
   *  @brief Return the iterator (in stable reference mode) by tag
   */
  simple_polygon_ptr_array_iter_type
  basic_iter(simple_polygon_ptr_array_type::tag) const {
    tl_assert((m_type == SimplePolygonPtrArray ||
               m_type == SimplePolygonPtrArrayMember) &&
              !m_with_props);
  ...
  /**
   *  @brief Return the iterator (in stable reference mode) by tag for objects
   * with properties
   */
  ppolygon_iter_type
  basic_iter(db::object_with_properties<polygon_type>::tag) const {
    tl_assert(m_type == Polygon && m_with_props);
    return *(((ppolygon_iter_type *)m_generic.iter));
  }
   ...
   */
  psimple_polygon_iter_type
  basic_iter(db::object_with_properties<simple_polygon_type>::tag) const {
    tl_assert(m_type == SimplePolygon && m_with_props);
    return *(((psimple_polygon_iter_type *)m_generic.iter));
  }
   ...
   */
  ppolygon_ref_iter_type
  basic_iter(db::object_with_properties<polygon_ref_type>::tag) const {
    tl_assert(m_type == PolygonRef && m_with_props);
    return *(((ppolygon_ref_iter_type *)m_generic.iter));
  }
   ...
   */
  ppolygon_ptr_array_iter_type
  basic_iter(db::object_with_properties<polygon_ptr_array_type>::tag) const {
    tl_assert((m_type == PolygonPtrArray || m_type == PolygonPtrArrayMember) &&
              m_with_props);
    return *(((ppolygon_ptr_array_iter_type *)m_generic.iter));
  }
   ...
   */
  psimple_polygon_ref_iter_type
  basic_iter(db::object_with_properties<simple_polygon_ref_type>::tag) const {
    tl_assert(m_type == SimplePolygonRef && m_with_props);
    return *(((psimple_polygon_ref_iter_type *)m_generic.iter));
  }
   ...
   * with properties
   */
  psimple_polygon_ptr_array_iter_type basic_iter(
      db::object_with_properties<simple_polygon_ptr_array_type>::tag) const {
    tl_assert((m_type == SimplePolygonPtrArray ||
               m_type == SimplePolygonPtrArrayMember) &&
              m_with_props);
   ...
   */
  ppath_ref_iter_type
  basic_iter(db::object_with_properties<path_ref_type>::tag) const {
    tl_assert(m_type == PathRef && m_with_props);
    return *(((ppath_ref_iter_type *)m_generic.iter));
  }
   ...
   */
  ppath_ptr_array_iter_type
  basic_iter(db::object_with_properties<path_ptr_array_type>::tag) const {
    tl_assert((m_type == PathPtrArray || m_type == PathPtrArrayMember) &&
              m_with_props);
    return *(((ppath_ptr_array_iter_type *)m_generic.iter));
  }
   ...
   */
  pedge_pair_iter_type
  basic_iter(db::object_with_properties<edge_pair_type>::tag) const {
    tl_assert(m_type == EdgePair && m_with_props);
    return *(((pedge_pair_iter_type *)m_generic.iter));
  }
   ...
   */
  ppoint_iter_type
  basic_iter(db::object_with_properties<point_type>::tag) const {
    tl_assert(m_type == Point && m_with_props);
    return *(((ppoint_iter_type *)m_generic.iter));
  }
   ...
   */
  ptext_ref_iter_type
  basic_iter(db::object_with_properties<text_ref_type>::tag) const {
    tl_assert(m_type == TextRef && m_with_props);
    return *(((ptext_ref_iter_type *)m_generic.iter));
  }
   ...
   */
  ptext_ptr_array_iter_type
  basic_iter(db::object_with_properties<text_ptr_array_type>::tag) const {
    tl_assert((m_type == TextPtrArray || m_type == TextPtrArrayMember) &&
              m_with_props);
    return *(((ptext_ptr_array_iter_type *)m_generic.iter));
  }
   ...
   */
  puser_object_iter_type
  basic_iter(db::object_with_properties<user_object_type>::tag) const {
    tl_assert(m_type == UserObject && m_with_props);
    return *(((puser_object_iter_type *)m_generic.iter));
  }
   ...
   */
  pbox_array_iter_type
  basic_iter(db::object_with_properties<box_array_type>::tag) const {
   ...
   */
  pshort_box_iter_type
  basic_iter(db::object_with_properties<short_box_type>::tag) const {
    tl_assert(m_type == ShortBox && m_with_props);
    return *(((pshort_box_iter_type *)m_generic.iter));
  }
   ...
   */
  pshort_box_array_iter_type
  basic_iter(db::object_with_properties<short_box_array_type>::tag) const {
    tl_assert((m_type == ShortBoxArray || m_type == ShortBoxArrayMember) &&
              m_with_props);
    return *(((pshort_box_array_iter_type *)m_generic.iter));
  }
  ...
  bool is_polygon() const {
    return (m_type == Polygon || m_type == PolygonRef ||
            m_type == PolygonPtrArrayMember || m_type == SimplePolygon ||
            m_type == SimplePolygonRef ||
            m_type == SimplePolygonPtrArrayMember);
  }
   ...
   *  Returns true, if the conversion was successful.
   */
  bool instantiate(polygon_type &p) const { return polygon(p); }

  /**
   *  @brief Test if the shape proxy points to a simple polygon
   */
  bool is_simple_polygon() const {
    return (m_type == SimplePolygon || m_type == SimplePolygonRef ||
            m_type == SimplePolygonPtrArrayMember);
  }
   ...
   *  Returns true, if the conversion was successful.
   */
  bool instantiate(simple_polygon_type &p) const { return simple_polygon(p); }

  /**
   *  @brief Return a reference to the path if one is referenced
   */
  const path_type &path() const { return *basic_ptr(path_type::tag()); }
   ...
   */
  bool is_path() const {
    return (m_type == Path || m_type == PathRef ||
            m_type == PathPtrArrayMember);
  }
   ...
   *  @brief Alias for polymorphic expansion
   */
  bool instantiate(path_type &p) const { return path(p); }
   ...
   */
  bool is_edge() const { return (m_type == Edge); }
   ...
   *  Returns true, if the conversion was successful.
   */
  bool instantiate(edge_type &p) const { return edge(p); }
   ...
   */
  bool is_edge_pair() const { return (m_type == EdgePair); }
   ...
   *  Returns true, if the conversion was successful.
   */
  bool instantiate(edge_pair_type &p) const { return edge_pair(p); }
   ...
   */
  bool is_point() const { return (m_type == Point); }
   ...
   *  Returns true, if the conversion was successful.
   */
  bool instantiate(point_type &p) const { return point(p); }

  /**
   *  @brief Return a reference to the text if one is referenced
   */
  const text_type &text() const { return *basic_ptr(text_type::tag()); }
   ...
   */
  bool is_text() const {
    return (m_type == Text || m_type == TextRef ||
            m_type == TextPtrArrayMember);
  }
   ...
   *  Returns true, if the conversion was successful
   */
  bool instantiate(text_type &p) const { return text(p); }
   ...
   */
  bool is_box() const {
    return (m_type == Box || m_type == ShortBox || m_type == BoxArrayMember ||
            m_type == ShortBoxArrayMember);
  }
   ...
   *  @brief Alias for polymorphic expansion
   */
  bool instantiate(box_type &p) const { return box(p); }
   ...
   */
  bool is_user_object() const { return (m_type == UserObject); }
  ...
  bool is_array_member() const {
    return m_type == PolygonPtrArrayMember ||
           m_type == SimplePolygonPtrArrayMember ||
           m_type == PathPtrArrayMember || m_type == BoxArrayMember ||
           m_type == ShortBoxArrayMember || m_type == TextPtrArrayMember;
  }
   ...
   */
  const trans_type &array_trans() const { return m_trans; }

  /**
   *  @brief inequality
   */
  bool operator!=(const Shape &d) const { return !operator==(d); }

  /**
   *  @brief equality
   *
   *  Equality of shapes is not specified by the identity of the objects but by
   * the identity of the pointers - both shapes must reference the same object.
   */
  bool operator==(const Shape &d) const {
    ...
    if (m_stable) {
      ...
      //  there are supposed to have the same size for all types, thus all bytes
      //  are supposed to be meaningful
      for (unsigned int i = 0;
           i < sizeof(tl::reuse_vector<box_type>::const_iterator); ++i) {
   ...
   *
   *  Ordering of shapes is not specified by the identity of the objects but by
   * the order of the pointers. Since pointers are volatile objects, the
   * ordering is not strictly reproducible!
   */
  bool operator<(const Shape &d) const {
    if (m_type != d.m_type) {
      return m_type < d.m_type;
...
public:
  ...
  union generic {
    ...
    const db::object_with_properties<simple_polygon_ref_type>
        *psimple_polygon_ref;
    const db::object_with_properties<simple_polygon_ptr_array_type>
        *psimple_polygon_aref;
