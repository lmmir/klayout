 ...
 */
class DB_PUBLIC FlatEdgePairs : public MutableEdgePairs {
public:
  ...
  EdgePairsDelegate *clone() const { return new FlatEdgePairs(*this); }
  ...
  virtual void do_transform(const db::Trans &t) { transform_generic(t); }

  virtual void do_transform(const db::ICplxTrans &t) { transform_generic(t); }
...
private:
  ...
  template <class Trans> void transform_generic(const Trans &trans) {
    if (!trans.is_unity()) {
      ...
      for (edge_pair_iterator_type p =
               ep.template get_layer<db::EdgePair, db::unstable_layer_tag>()
                   .begin();
           p !=
           ep.template get_layer<db::EdgePair, db::unstable_layer_tag>().end();
           ++p) {
        ep.get_layer<db::EdgePair, db::unstable_layer_tag>().replace(
