...
const std::list<db::PluginDescriptor> &plugins() { return s_plugins; }
...
static PluginDescriptor load_plugin(const std::string &pp) {
  ...
  if (!handle) {
    throw tl::Exception(
        tl::to_string(tr("Unable to load plugin: %s with error message: %s ")),
        pp, GetLastError());
    return desc;
  }
  init_func =
      reinterpret_cast<dbp_init_func_t>(GetProcAddress(handle, init_func_name));
