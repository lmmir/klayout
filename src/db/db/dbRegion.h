 ...
 *  @brief A region
 *
 *  A region basically is a set of polygons. It supports a variety of
 * operations, i.e. boolean operations with other regions, sizing etc.
 *
 *  Regions can have different states. Specifically a region can be merged (no
 * overlapping polygons are present, touching polygons are merged,
 * self-intersections of polygons are removed) or non-merged (polygons may
 * overlap or polygons may be self-intersecting). In merged state, the wrap
 * count at every point is either zero or 1, in non-merged state it can be every
 * value.
 *
 *  Polygons inside the region may contain holes if the region is merged.
 */
class DB_PUBLIC Region : public db::ShapeCollection {
public:
   ...
   */
  explicit Region(const db::Box &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Constructor from a box with properties
   */
  explicit Region(const db::BoxWithProperties &s) : mp_delegate(0) {
    insert(s);
  }
   ...
   */
  explicit Region(const db::Polygon &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Constructor from a polygon with properties
   */
  explicit Region(const db::PolygonWithProperties &s) : mp_delegate(0) {
    insert(s);
  }
   ...
   */
  explicit Region(const db::SimplePolygon &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Constructor from a simple polygon with properties
   */
  explicit Region(const db::SimplePolygonWithProperties &s) : mp_delegate(0) {
    insert(s);
  }
   ...
   */
  explicit Region(const db::Path &s) : mp_delegate(0) { insert(s); }

  /**
   *  @brief Constructor from a path with properties
   */
  explicit Region(const db::PathWithProperties &s) : mp_delegate(0) {
    insert(s);
  }
   ...
   *  polygons, paths or shapes. This version accepts iterators of the begin ...
   * end style.
   */
  template <class Iter>
  explicit Region(const Iter &b, const Iter &e) : mp_delegate(0) {
    reserve(e - b);
    for (Iter i = b; i != e; ++i) {
      insert(*i);
    }
   ...
   *
   *  Creates a region from a recursive shape iterator. This allows feeding a
   * region from a hierarchy of cells.
   */
   ...
   *
   *  Creates a region from a recursive shape iterator. This allows feeding a
   * region from a hierarchy of cells. The transformation is useful to scale to
   * a specific DBU for example.
   */
  explicit Region(const RecursiveShapeIterator &si, const db::ICplxTrans &trans,
                  bool merged_semantics = true);

  /**
   *  @brief Constructor from a RecursiveShapeIterator providing a deep
   * representation
   *
   *  This version will create a hierarchical region. The DeepShapeStore needs
   * to be provided during the lifetime of the region and acts as a heap for
   * optimized data.
   *
   *  "area_ratio" and "max_vertex_count" are optimization parameters for the
   *  shape splitting algorithm.
   */
  explicit Region(const RecursiveShapeIterator &si, DeepShapeStore &dss,
                  double area_ratio = 3.0, size_t max_vertex_count = 16);

  /**
   *  @brief Constructor from a RecursiveShapeIterator providing a deep
   * representation with transformation
   */
  explicit Region(const RecursiveShapeIterator &si, DeepShapeStore &dss,
                  const db::ICplxTrans &trans, bool merged_semantics = true,
                  double area_ratio = 3.0, size_t max_vertex_count = 16);
   ...
   */
  ShapeCollectionDelegateBase *get_delegate() const { return mp_delegate; }

  /**
   *  @brief Gets the underlying delegate object
   */
  const RegionDelegate *delegate() const { return mp_delegate; }

  /**
   *  @brief Gets the underlying delegate object
   */
  RegionDelegate *delegate() { return mp_delegate; }
   ...
   */
  void set_base_verbosity(int vb) { mp_delegate->set_base_verbosity(vb); }

  /**
   *  @brief Gets the base verbosity
   */
  unsigned int base_verbosity() const { return mp_delegate->base_verbosity(); }
   ...
   */
  void disable_progress() { mp_delegate->disable_progress(); }
   ...
   */
  const_iterator begin() const { return RegionIterator(mp_delegate->begin()); }

  /**
   *  @brief Returns the merged polygons if merge semantics applies
   *
   *  If merge semantics is not enabled, this iterator delivers the individual
   * polygons.
   */
  ...
  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the polygons plus the
  ...
  /**
   *  @brief Delivers a RecursiveShapeIterator pointing to the merged polygons
   * plus the necessary transformation
   */
   ...
   */
  template <class Sh> void insert(const Sh &shape);
   ...
   */
  template <class T> void insert(const db::Shape &shape, const T &trans);

  /**
   *  @brief Returns true if the region is empty
   */
  bool empty() const { return mp_delegate->empty(); }

  /**
   *  @brief Returns the number of (flat) polygons in the region
   */
  size_t count() const { return mp_delegate->count(); }

  /**
   *  @brief Returns the number of (hierarchical) polygons in the region
   */
  size_t hier_count() const { return mp_delegate->hier_count(); }

  /**
   *  @brief Returns a string representing the region
   *
   *  nmax specifies how many polygons are included (set to
   * std::numeric_limits<size_t>::max() for "all".
   */
   ...
   *
   *  If minimum coherence is set, the merge operations (explicit merge with
   * \merge or implicit merge through merged_semantics) are performed using
   * minimum coherence mode. The coherence mode determines how kissing-corner
   * situations are resolved. If minimum coherence is selected, they are
   * resolved such that multiple polygons are created which touch at a corner).
   */
  void set_min_coherence(bool f) { mp_delegate->set_min_coherence(f); }

  /**
   *  @brief Gets the minimum coherence flag
   */
  bool min_coherence() const { return mp_delegate->min_coherence(); }

  /**
   *  @brief Sets the merged-semantics flag
   *
   *  If merged semantics is enabled (the default), coherent polygons will be
   * considered as single regions and artificial edges such as cut-lines will
   * not be considered. Merged semantics thus is equivalent to considering
   * coherent areas rather than single polygons.
   */
  void set_merged_semantics(bool f) { mp_delegate->set_merged_semantics(f); }

  /**
   *  @brief Gets the merged-semantics flag
   */
  bool merged_semantics() const { return mp_delegate->merged_semantics(); }
   ...
   */
  void set_strict_handling(bool f) { mp_delegate->set_strict_handling(f); }

  /**
   *  @brief Gets a valid indicating whether strict handling is enabled
   */
  bool strict_handling() const { return mp_delegate->strict_handling(); }
   ...
   */
  bool is_box() const { return mp_delegate->is_box(); }

  /**
   *  @brief Returns true if the region is merged
   */
  bool is_merged() const { return mp_delegate->is_merged(); }

  /**
   *  @brief Returns the area of the region
   *
   *  This method returns the area sum over all polygons.
   *  Merged semantics applies. In merged semantics, the area is the correct
   * area covered by the polygons. Without merged semantics, overlapping parts
   * are counted twice.
   *
   ...
   *  Merged semantics applies. In merged semantics, the perimeter is the true
   * perimeter. Without merged semantics, inner edges contribute to the
   * perimeter.
   *
   *  If a box is given, the computation is restricted to that box.
   *  Edges coincident with the box edges are counted only if the form outer
   * edges at the box edge.
   */
   ...
   */
  Box bbox() const { return mp_delegate->bbox(); }
   ...
   *  The processing filter can apply modifications too. These modifications
   * will be kept in the output region.
   *
   *  Merged semantics applies. In merged semantics, the filter will run over
   *  all merged polygons.
   */
   ...
   *  all merged polygons.
   *
   *  This method will return a new region with the modified and filtered
   * polygons.
   */
   ...
   */
  EdgePairs
  cop_to_edge_pairs(db::CompoundRegionOperationNode &node,
                    PropertyConstraint prop_constraint = db::IgnoreProperties);
   ...
   */
  Region
  cop_to_region(db::CompoundRegionOperationNode &node,
                PropertyConstraint prop_constraint = db::IgnoreProperties);
  ...
  /**
   *  @brief Applies a width check and returns EdgePairs which correspond to
   * violation markers
   *
   *  The width check will create a edge pairs if the width of the area between
   * the edges is less than the specified threshold d.
   *
   ...
   */
  EdgePairs width_check(db::Coord d, const RegionCheckOptions &options =
                                         db::RegionCheckOptions()) const {
    return EdgePairs(mp_delegate->width_check(d, options));
  }

  /**
   *  @brief Applies a space check and returns EdgePairs which correspond to
   * violation markers
   *
   *  For the parameters see \width_check. The space check reports edges for
   * which the space is less than the specified threshold d.
   *
   *  Merged semantics applies.
   */
  EdgePairs space_check(db::Coord d, const RegionCheckOptions &options =
                                         db::RegionCheckOptions()) const {
    return EdgePairs(mp_delegate->space_check(d, options));
  }

  /**
   *  @brief Applies a isolation check (space of polygon vs. other polygons) and
   * returns EdgePairs which correspond to violation markers
   *
   *  For the parameters see \width_check. The space check reports edges for
   * which the notch space is less than the specified threshold d.
   *
   *  Merged semantics applies.
   */
  EdgePairs isolated_check(db::Coord d, const RegionCheckOptions &options =
                                            db::RegionCheckOptions()) const {
    return EdgePairs(mp_delegate->isolated_check(d, options));
  }

  /**
   *  @brief Applies a notch check (space in polygon vs. itself) and returns
   * EdgePairs which correspond to violation markers
   *
   *  For the parameters see \width_check. The space check reports edges for
   * which the notch space is less than the specified threshold d.
   *
   *  Merged semantics applies.
   */
  EdgePairs notch_check(db::Coord d, const RegionCheckOptions &options =
                                         db::RegionCheckOptions()) const {
    return EdgePairs(mp_delegate->notch_check(d, options));
  }

  /**
   *  @brief Applies a enclosed check and returns EdgePairs which correspond to
   * violation markers
   *
   *  The check will return true, where this region is enclosing the polygons of
   * the other region by less than the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   * edges will be those of "other".
   *
   *  For the other parameters see \width_check.
   *
   *  Merged semantics applies.
   */
  EdgePairs enclosing_check(
      const Region &other, db::Coord d,
      const RegionCheckOptions &options = db::RegionCheckOptions()) const {
    return EdgePairs(mp_delegate->enclosing_check(other, d, options));
  }

  /**
   *  @brief Applies a overlap check and returns EdgePairs which correspond to
   * violation markers
   *
   *  The check will return true, where this region overlaps the polygons of the
   * other region by less than the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   * edges will be those of "other".
   *
  ...
  /**
   *  @brief Applies a separation check and returns EdgePairs which correspond
   * to violation markers
   *
   *  The check will return true, where this region is separated by polygons of
   * the other region by less than the specified threshold d.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   * edges will be those of "other".
  ...
  /**
   *  @brief Applies a inside check and returns EdgePairs which correspond to
   * violation markers
   *
   *  The check will return true, where this region is inside by less than the
   * threshold d inside the polygons of the other region.
   *
   *  The first edges of the edge pairs will be the ones from "this", the second
   * edges will be those of "other".
   *
  ...
  /**
   *  @brief Returns an edge set containing all edges of the polygons in this
   * region
   *
   *  Merged semantics applies. In merged semantics, only full, outer edges are
   * delivered.
   */
  Edges edges() const { return Edges(mp_delegate->edges(0)); }

  /**
   *  @brief Returns an edge set containing all edges of the polygons in this
   * region
   *
   *  This version allows one to specify a filter by which the edges are
   * filtered before they are returned.
   *
   *  Merged semantics applies. In merged semantics, only full, outer edges are
   * delivered.
   */
   ...
   */
  template <class T> Region &transform(const T &trans);

  /**
   *  @brief Returns the transformed region
   */
  template <class T> Region transformed(const T &trans) const {
    Region d(*this);
    d.transform(trans);
    return d;
  }
   ...
   *
   *  This method will scale the region by mx/dx in horizontal and by my/dy in
   * vertical direction and then snap to gx and gy respectively.
   */
  void scale_and_snap(db::Coord gx, db::Coord mx, db::Coord dx, db::Coord gy,
                      db::Coord my, db::Coord dy);

  /**
   *  @brief Returns the scaled and snapped region
   */
  Region scaled_and_snapped(db::Coord gx, db::Coord mx, db::Coord dx,
                            db::Coord gy, db::Coord my, db::Coord dy) const;
   ...
   */
  void swap(db::Region &other) { std::swap(other.mp_delegate, mp_delegate); }

  /**
   *  @brief Merge the region
   *
   *  This method merges the polygons of the region if they are not merged
   * already. It returns a reference to this region. An out-of-place merge
   * version is "merged".
   */
  Region &merge() {
   ...
   *
   *  This is the out-of-place merge. It returns a new region but does not
   * modify the region it is called on. An in-place version is "merge".
   */
  Region merged() const { return Region(mp_delegate->merged()); }
   ...
   *  A min_wrapcount of 1 will produce output where at least two polygons
   * overlap.
   *
   *  This method will always execute the merge, even if the region is already
   * merged.
   *
   *  @param min_coherence Set this parameter to true to get minimum polygons
   * (kissing corner problem)
   ...
   *
   *  The result is a set of polygons which may be overlapping, but are not
   * self- intersecting.
   *
   *  Merged semantics applies.
   *
   *  @param d The (isotropic) sizing value
   *  @param mode The sizing mode (see EdgeProcessor) for a description of the
   * sizing mode which controls the miter distance.
   ...
   *
   *  This version provides anisotropic sizing by allowing to specify a distance
   * int x and y direction.
   *
   *  Merged semantics applies.
   *
   ...
   *  "merged polygon" semantics applies if merged_polygon_semantics is true
   * (see set_auto_merge).
   *
   *  Merged semantics applies.
   */
   ...
   */
  Region
  bool_and(const Region &other,
           PropertyConstraint prop_constraint = db::IgnoreProperties) const {
   ...
   */
  Region &
  bool_and_with(const Region &other,
                PropertyConstraint prop_constraint = db::IgnoreProperties) {
   ...
   */
  Region
  bool_not_with(const Region &other,
                PropertyConstraint prop_constraint = db::IgnoreProperties) {
   ...
   */
  Region &
  bool_xor_with(const Region &other,
                PropertyConstraint prop_constraint = db::IgnoreProperties) {
   ...
   */
  Region &
  bool_or_with(const Region &other,
               PropertyConstraint prop_constraint = db::IgnoreProperties) {
   ...
   *
   *  The first region delivered will be the AND result, the second one the NOT
   * result.
   */
  std::pair<Region, Region>
  andnot(const Region &other,
         PropertyConstraint prop_constraint = db::IgnoreProperties) const {
    std::pair<RegionDelegate *, RegionDelegate *> res =
        mp_delegate->andnot_with(other, prop_constraint);
    return std::make_pair(Region(res.first), Region(res.second));
  }

  /**
   *  @brief Selects all polygons of this region which are completely outside
  ...
  /**
   *  @brief Selects all polygons of this region which are not completely
  ...
  /**
   *  @brief Returns all polygons of this which are completely outside polygons
  ...
  /**
   *  @brief Returns all polygons of this which are not completely outside
   * polygons from the other region
   *
  ...
  /**
   *  @brief Returns all polygons of this which are completely outside polygons
   * from the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_outside and
   * selected_not_outside, but faster.
   *
   *  Merged semantics applies.
   */
  std::pair<Region, Region>
  selected_outside_differential(const Region &other) const {
    std::pair<db::RegionDelegate *, db::RegionDelegate *> p =
        mp_delegate->selected_outside_pair(other);
    return std::pair<Region, Region>(Region(p.first), Region(p.second));
  }

  /**
   *  @brief Selects all polygons of this region which are completely inside
   * polygons from the other region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Selects all polygons of this region which are not completely inside
   * polygons from the other region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all polygons of this which are completely inside polygons
   * from the other region
   *
  ...
  /**
   *  @brief Returns all polygons of this which are not completely inside
   * polygons from the other region
   *
   ...
   * from the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_inside and
   * selected_not_inside, but faster.
   *
   *  Merged semantics applies.
   */
  std::pair<Region, Region>
  selected_inside_differential(const Region &other) const {
    std::pair<db::RegionDelegate *, db::RegionDelegate *> p =
        mp_delegate->selected_inside_pair(other);
    return std::pair<Region, Region>(Region(p.first), Region(p.second));
  }

  /**
   *  @brief Returns all polygons of this which are enclosing polygons from the
   * other region
   *
   *  Merged semantics applies.
   */
  Region &
  select_enclosing(const Region &other, size_t min_count = 1,
  ...
  /**
   *  @brief Returns all polygons of this which are not enclosing polygons from
   * the other region
   *
   *  Merged semantics applies.
   */
  Region &
  select_not_enclosing(const Region &other, size_t min_count = 1,
                       size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_not_enclosing(other, min_count, max_count));
    return *this;
  }
   ...
   *  Merged semantics applies.
   */
  Region selected_enclosing(
      const Region &other, size_t min_count = 1,
  ...
  /**
   *  @brief Returns all polygons of this which are completely enclosing
   * polygons from the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_enclosing and
   * selected_not_enclosing, but faster.
   *
   *  Merged semantics applies.
   */
  std::pair<Region, Region> selected_enclosing_differential(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) const {
    std::pair<db::RegionDelegate *, db::RegionDelegate *> p =
        mp_delegate->selected_enclosing_pair(other, min_count, max_count);
    return std::pair<Region, Region>(Region(p.first), Region(p.second));
  }

  /**
   *  @brief Selects all polygons of this region which overlap or touch polygons
   * from the other region
   *
   *  The argument (if given) specifies and range of interaction counts:
   * polygons will only be selected if the number of interacting (different)
   * polygons from the other region is between min_count and max_count
   * (inclusive).
   *
   *  Merged semantics applies.
   */
  Region &
  select_interacting(const Region &other, size_t min_count = 1,
                     size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_interacting(other, min_count, max_count));
    return *this;
  }

  /**
   *  @brief Selects all polygons of this region which do not overlap or touch
   * polygons from the other region
   *
   *  The argument (if given) specifies and range of interaction counts:
   * polygons will not be selected if the number of interacting (different)
   * polygons from the other region is between min_count and max_count
   * (inclusive).
   *
   *  Merged semantics applies.
  ...
  Region &select_not_interacting(
      ...
      size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_not_interacting(other, min_count, max_count));
    return *this;
  }

  /**
   *  @brief Returns all polygons of this which overlap or touch polygons from
   * the other region
   *
  ...
  /**
   *  @brief Returns all polygons of this which do not overlap or touch polygons
   * from the other region
   *
   ...
   *  Merged semantics applies.
   */
  Region selected_not_interacting(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) const {
    return Region(
        mp_delegate->selected_not_interacting(other, min_count, max_count));
  ...
  /**
   *  @brief Returns all polygons of this which are interacting with polygons
   * from the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_interacting and
   * selected_not_interacting, but faster.
   *
   *  Merged semantics applies.
   */
  std::pair<Region, Region> selected_interacting_differential(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) const {
    std::pair<db::RegionDelegate *, db::RegionDelegate *> p =
        mp_delegate->selected_interacting_pair(other, min_count, max_count);
    return std::pair<Region, Region>(Region(p.first), Region(p.second));
  }

  /**
   *  @brief Selects all polygons of this region which overlap or touch edges
   * from the given edge collection
   *
   *  Merged semantics applies to both operators.
   */
  Region &
  select_interacting(const Edges &other, size_t min_count = 1,
                     size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_interacting(other, min_count, max_count));
    return *this;
  ...
  /**
   *  @brief Selects all polygons of this region which do not overlap or touch
   * edges from the edge collection
   *
   *  Merged semantics applies to both operators.
   */
  Region &select_not_interacting(
      const Edges &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_not_interacting(other, min_count, max_count));
    return *this;
  ...
  /**
   *  @brief Returns all polygons of this which overlap or touch edges from the
   * edge collection
   *
   *  This method is an out-of-place version of select_interacting.
   *
  ...
  /**
   *  @brief Returns all polygons of this which are interacting with edges from
   * the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_interacting and
   * selected_not_interacting, but faster.
   *
  ...
  /**
   *  @brief Selects all polygons of this region which overlap or touch texts
   * from the text collection
   *
   *  Merged semantics applies.
   */
  Region &
  select_interacting(const Texts &other, size_t min_count = 1,
                     size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_interacting(other, min_count, max_count));
    return *this;
  ...
  /**
   *  @brief Selects all polygons of this region which do not overlap or touch
   * texts from the text collection
   *
   *  Merged semantics applies.
   */
  Region &select_not_interacting(
      const Texts &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_not_interacting(other, min_count, max_count));
    return *this;
  ...
  /**
   *  @brief Returns all polygons of this which overlap or touch texts from the
   * text collection
   *
   *  This method is an out-of-place version of select_interacting.
   *
  ...
  /**
   *  @brief Returns all polygons of this which do not overlap or touch texts
   * from the text collection
   *
   *  This method is an out-of-place version of select_not_interacting.
   *
  ...
  /**
   *  @brief Returns all polygons of this which are interacting with texts from
   * the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_interacting and
   * selected_not_interacting, but faster.
  ...
  /**
   *  @brief Selects all polygons of this region which overlap polygons from the
   * other region
   *
   *  Merged semantics applies.
   */
  Region &
  select_overlapping(const Region &other, size_t min_count = 1,
                     size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_overlapping(other, min_count, max_count));
    return *this;
  }

  /**
   *  @brief Selects all polygons of this region which do not overlap polygons
   * from the other region
   *
   *  Merged semantics applies.
   */
  Region &select_not_overlapping(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) {
    set_delegate(
        mp_delegate->selected_not_overlapping(other, min_count, max_count));
    return *this;
  }

  /**
   *  @brief Returns all polygons of this which overlap polygons from the other
   * region
   *
  ...
  /**
   *  @brief Returns all polygons of this which do not overlap polygons from the
   * other region
   *
   ...
   *  Merged semantics applies.
   */
  Region selected_not_overlapping(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) const {
    return Region(
        mp_delegate->selected_not_overlapping(other, min_count, max_count));
  ...
  /**
   *  @brief Returns all polygons of this which are overlapping polygons from
   * the other region and the opposite ones at the same time
   *
   *  This method is equivalent to calling selected_overlapping and
   * selected_not_overlapping, but faster.
   *
   *  Merged semantics applies.
   */
  std::pair<Region, Region> selected_overlapping_differential(
      const Region &other, size_t min_count = 1,
      size_t max_count = std::numeric_limits<size_t>::max()) const {
    std::pair<db::RegionDelegate *, db::RegionDelegate *> p =
        mp_delegate->selected_overlapping_pair(other, min_count, max_count);
    return std::pair<Region, Region>(Region(p.first), Region(p.second));
  }

  /**
   *  @brief Returns all polygons of "other" which are inside polygons of this
   * region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all edges of "other" which are interacting (touching or
   * overlapping with) polygons of this region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all texts of "other" which are interacting (touching or
   * overlapping with) polygons of this region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all polygons of "other" which are interacting (touching or
   * overlapping with) polygons of this region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all polygons of "other" which are overlapping with polygons
   * of this region
   *
   *  Merged semantics applies.
   */
  ...
  /**
   *  @brief Returns all polygons which are in the other region plus the ones
   * which are not
   *
   *  This method is similar to calling in with inverse = false and true, but
   * more efficient.
   *
   *  Merged semantics applies.
   */
   ...
   */
  const db::Polygon *nth(size_t n) const { return mp_delegate->nth(n); }
   ...
   *
   *  If the region has valid polygons, it is permissable to use the polygon's
   * addresses from the iterator. Furthermore, the random access operator nth()
   * is available.
   */
  bool has_valid_polygons() const { return mp_delegate->has_valid_polygons(); }
  ...
  /**
   *  @brief Returns true, if the region has valid merged polygons stored within
   * itself
   *
   *  If the region has valid merged polygons, it is permissable to use the
   * polygon's addresses from the merged polygon iterator. Furthermore, the
   * random access operator nth() is available.
   */
   ...
   *  layout's hierarchy.
   */
  void insert_into(Layout *layout, db::cell_index_type into_cell,
   ...
   *
   *  This will pull the net shapes from the LayoutToNetlist database, provided
   * that this layer was an input to the netlist extraction.
   *
   *  Netlist names will be attached as properties according to prop_mode and
   * net_prop_name. A net filter can be provided so that only certain nets are
   * produced.
   */
  Region nets(LayoutToNetlist &l2n,
              NetPropertyMode prop_mode = db::NPM_NoProperties,
              const tl::Variant &net_prop_name = tl::Variant(0),
              const std::vector<const db::Net *> *nets = 0) const {
   ...
   *  treated as a glob pattern. Otherwise, the text is taken if "pat" is equal
   * to the text..
   */
  db::Edges texts_as_dots(const std::string &pat, bool as_pattern) const;

  /**
   *  @brief Delivers texts as dots (degenerated edges) in a deep edge
   * collection
   *
   *  "pat" is a text selector. If "as_pattern" is true, this pattern will be
   *  treated as a glob pattern. Otherwise, the text is taken if "pat" is equal
   * to the text..
   */
  db::Edges texts_as_dots(const std::string &pat, bool as_pattern,
                          db::DeepShapeStore &store) const;
   ...
   *  "pat" is a text selector. If "as_pattern" is true, this pattern will be
   *  treated as a glob pattern. Otherwise, the text is taken if "pat" is equal
   * to the text. "enl" is the half size of the box (the box is 2*enl wide and
   * 2*enl high).
   */
  db::Region texts_as_boxes(const std::string &pat, bool as_pattern,
                            db::Coord enl) const;
   ...
   */
  db::Region texts_as_boxes(const std::string &pat, bool as_pattern,
                            db::Coord enl, db::DeepShapeStore &store) const;
 ...
 *  Like EdgeContainer, this receiver collects the objects into an existing
 * Region object.
 */
class DB_PUBLIC RegionPolygonSink : public db::PolygonSink {
public:
  /**
   ...
   */
  virtual void put(const db::Polygon &polygon) { mp_region->insert(polygon); }
...
namespace tl {
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::Region &b);
