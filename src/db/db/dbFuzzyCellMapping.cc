...
public:
  ...
  TransformationMatrixSum(const db::Matrix2d &m) { m_m = m; }
  ...
  void add(const TransformationMatrixSum &other) { m_m += other.m_m; }
...
public:
  InstanceReferenceSum(const db::Layout &, const db::Cell &) : m_count(0) {
    // .. nothing yet ..
  }
  ...
  InstanceReferenceSum transformed(const db::CellInstArray &inst) const {
    ...
    if (m_count == 0) {
      ...
      return InstanceReferenceSum(m_count * inst.size(), m_m * m_res,
                                  m_p + m_m * p_res);
    }
  }
...
void FuzzyCellMapping::clear() { m_b2a_mapping.clear(); }
...
struct CellSignature {
  ...
  bool distance_less_or_equal(const CellSignature &other, double dmin,
                              double &dmin_out) const {
    double d;
    ...
    for (s1 = shapes.begin(), s2 = other.shapes.begin(); s1 != shapes.end();
         ++s1, ++s2) {
  ...
  std::string to_string() const {
    std::string st;
    for (std::vector<size_t>::const_iterator s = shapes.begin();
         s != shapes.end(); ++s) {
      if (!st.empty()) {
        st += ",";
      }
    ...
    return "weight=" + tl::to_string(weight) + " bbox=" + bbox.to_string() +
           " instances=" + tl::to_string(instances) + " shapes=" + st +
           " tm_avg=" + tm_avg.to_string() + " p_avg=" + p_avg.to_string();
  }
};

static void
collect_cell_signatures(const db::Layout &layout,
                        const std::vector<unsigned int> &layers,
                        db::cell_index_type cell_index,
                        std::map<db::cell_index_type, CellSignature> &metrics,
                        const std::string &progress_report) {
  ...
  for (db::CellCounter::selection_iterator c = rs.begin(); c != rs.end(); ++c) {
    ...
    CellSignature &m =
        metrics.insert(std::make_pair(*c, CellSignature())).first->second;
    ...
    for (std::vector<unsigned int>::const_iterator l = layers.begin();
      ...
      size_t n = 0;
      //  TODO: right now, the only way to get the "true" shape count is to
      //  iterate ...
...
void FuzzyCellMapping::create(const db::Layout &layout_a,
  ...
  for (db::Layout::layer_iterator l = layout_a.begin_layers();
       l != layout_a.end_layers(); ++l) {
    for (db::Layout::layer_iterator ll = layout_b.begin_layers();
         ll != layout_b.end_layers(); ++ll) {
  ...
  collect_cell_signatures(layout_a, la, cell_index_a, ma,
                          tl::to_string(tr("Collecting cell signatures (A)")));
  ...
  collect_cell_signatures(layout_b, lb, cell_index_b, mb,
                          tl::to_string(tr("Collecting cell signatures (B)")));

  tl::RelativeProgress progress(tl::to_string(tr("Finding matching cells")),
                                ma.size() * ma.size());

  for (std::map<db::cell_index_type, CellSignature>::const_iterator m =
           ma.begin();
       m != ma.end(); ++m) {
    ...
    for (std::map<db::cell_index_type, CellSignature>::const_iterator n =
             mb.begin();
         n != mb.end(); ++n) {
    ...
    if (tl::verbosity() >= 40) {
      ...
      for (std::vector<db::cell_index_type>::const_iterator c = cmin.begin();
           c != cmin.end(); ++c) {
    ...
    //  if a unique match was found, confirm the match by looking if there is a
    //  "a" cell matching better.
    for (std::vector<db::cell_index_type>::const_iterator c = cmin.begin();
         c != cmin.end(); ++c) {
      ...
      for (; mm != ma.end() && confirmed; ++mm) {
        ...
        ++progress;
        double d = -1.0;
        if (mmin.distance_less_or_equal(mm->second, dmin, d) &&
            distance_func(d, dmin) > 1e-6) {
          confirmed = false;
        }
      }
    ...
    //  If there is no unique mapping, use the name similarity (measured by the
    //  edit distance) as a distance measure
    if (cmin.size() > 1) {
      ...
      for (std::vector<db::cell_index_type>::const_iterator c = cmin.begin();
        ...
        int ed = tl::edit_distance(layout_a.cell_name(m->first),
                                   layout_b.cell_name(*c));
        if (ed < min_ed) {
          min_ed = ed;
          min_ed_ci = *c;
        }
    ...
    if (cmin.size() > 0) {
      ...
      if (tl::verbosity() >= 30) {
        tl::info << "Cell mapping - found a matching pair "
                 << layout_a.cell_name(m->first) << " and "
                 << layout_b.cell_name(cmin[0]);
      }
      ...
      if (tl::verbosity() >= 30) {
        if (cmin.size() == 0) {
          tl::info << "Cell mapping - no match found for "
                   << layout_a.cell_name(m->first);
        } else {
          tl::info << "Cell mapping - multiple matches found for "
                   << layout_a.cell_name(m->first) << ": " << tl::noendl;
...
void FuzzyCellMapping::dump_mapping(
