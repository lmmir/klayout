...
Brace::Brace(db::LayoutToNetlistStandardReader *reader)
    : mp_reader(reader), m_checked(false) {
...
LayoutToNetlistStandardReader::LayoutToNetlistStandardReader(
    tl::InputStream &stream)
...
db::Coord LayoutToNetlistStandardReader::read_coord() {
  db::Coord i = 0;
  m_ex.read(i);
  return i;
}

double LayoutToNetlistStandardReader::read_double() {
  double d = 0;
  m_ex.read(d);
  return d;
}
...
void LayoutToNetlistStandardReader::do_read(db::LayoutToNetlist *l2n) {
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("File read: ")) + m_path);

  try {
    read_netlist(0, l2n);
  } catch (tl::Exception &ex) {
    throw tl::Exception(tl::sprintf(tl::to_string(tr("%s in line: %d of %s")),
                                    ex.msg(), m_stream.line_number(), m_path));
  }
}

static db::Region &layer_by_name(db::LayoutToNetlist *l2n,
...
void LayoutToNetlistStandardReader::read_netlist(
    db::Netlist *netlist, db::LayoutToNetlist *l2n,
    LayoutToNetlistStandardReader::Brace *nested,
    std::map<const db::Circuit *, ObjectMap> *map_per_circuit) {
  m_dbu = 0.001;
  int version = 0;
  ...
  while (nested ? *nested : !at_end()) {
    ...
    if (test(skeys::version_key) || test(lkeys::version_key)) {
      ...
      l2n->internal_layout()->rename_cell(
          l2n->internal_top_cell()->cell_index(), top.c_str());
      br.done();
      ...
      if (netlist->device_class_by_name(class_name) != 0) {
        throw tl::Exception(
            tl::to_string(tr("Duplicate definition of device class: ")) +
            class_name);
      }

      db::DeviceClassTemplateBase *dct =
          db::DeviceClassTemplateBase::template_by_name(templ_name);
      if (!dct) {
        throw tl::Exception(
            tl::to_string(tr("Invalid device class template: ")) + templ_name);
      }
      ...
      while (br) {
        ...
        if (test(skeys::terminal_key) || test(lkeys::terminal_key)) {
          ...
          if (!dc->has_parameter_with_name(param_name)) {
            ...
            db::DeviceParameterDefinition *pd =
                dc->parameter_definition_non_const(
                    dc->parameter_id_for_name(param_name));
            pd->set_default_value(default_value);
            pd->set_is_primary(primary);
          }
      ...
      if (l2n) {
        ...
        std::pair<bool, db::cell_index_type> ci_old =
            ly->cell_by_name(name.c_str());
        device_cell_index =
            ci_old.first ? ci_old.second : ly->add_cell(name.c_str());
      ...
      while (br) {
        ...
        if (test(skeys::property_key) || test(lkeys::property_key)) {
          ...
          throw tl::Exception(tl::to_string(
              tr("Unexpected end of file inside circuit definition (rect, "
                 "polygon, net, pin, device or circuit expected)")));
        } else {
          skip_element();
        }
      }
      ...
      if (l2n) {
        ...
        //  connections needs to be made after the instances (because in a
        //  readonly Instances container the Instance pointers will invalidate
        //  when new instances are added)
        for (db::Cell::const_iterator i = ccell.begin(); !i.at_end(); ++i) {
          std::map<db::CellInstArray, std::list<Connections>>::const_iterator
              c = connections.find(i->cell_inst());
          if (c != connections.end()) {
            for (std::list<Connections>::const_iterator j = c->second.begin();
                 j != c->second.end(); ++j) {
              l2n->net_clusters()
                  .clusters_per_cell(device_cell_index)
                  .add_connection(
                      j->from_cluster,
                      db::ClusterInstance(j->to_cluster, i->cell_index(),
                                          i->complex_trans(), i->prop_id()));
            }
          }
        }
      }
      ...
      while (br) {
        ...
        if (test(skeys::terminal_key) || test(lkeys::terminal_key)) {
          ...
          throw tl::Exception(
              tl::to_string(tr("Unexpected end of file inside device abstract "
                               "definition (terminal expected)")));
        } else {
          skip_element();
        }
      }
  ...
  if (version > 1) {
    throw tl::Exception(
        tl::to_string(tr("This program version only supports version 1 of the "
                         "L2N DB format. File version is: ")) +
        tl::to_string(version));
  }
}

db::Point LayoutToNetlistStandardReader::read_point() {
...
db::Box LayoutToNetlistStandardReader::read_rect() {
  m_ref = db::Point();
...
db::Polygon LayoutToNetlistStandardReader::read_polygon() {
  m_ref = db::Point();
...
void LayoutToNetlistStandardReader::read_geometries(
    db::NetlistObject *obj, Brace &br, db::LayoutToNetlist *l2n,
    db::local_cluster<db::NetShape> &lc, db::Cell &cell) {
  m_ref = db::Point();
  std::string lname;
  ...
  while (br) {
    ...
    if (test(skeys::property_key) || test(lkeys::property_key)) {
      ...
      NetShape n(db::PolygonRef(db::Polygon(box),
                                l2n->internal_layout()->shape_repository()));
      ...
      poly.assign_hull(pt.begin(), pt.end());
      NetShape n(
          db::PolygonRef(poly, l2n->internal_layout()->shape_repository()));
      ...
      NetShape n(db::TextRef(db::Text(text, db::Trans(pt - db::Point())),
                             l2n->internal_layout()->shape_repository()));
      ...
      throw tl::Exception(tl::to_string(
          tr("Unexpected end of file (polygon, text or rect expected)")));
    } else {
      skip_element();
    }
  }
...
void LayoutToNetlistStandardReader::read_net(db::Netlist * /*netlist*/,
  ...
  if (l2n) {
    ...
    db::connected_clusters<db::NetShape> &cc =
        l2n->net_clusters().clusters_per_cell(circuit->cell_index());
...
void LayoutToNetlistStandardReader::read_pin(db::Netlist * /*netlist*/,
                                             db::LayoutToNetlist * /*l2n*/,
                                             db::Circuit *circuit,
                                             ObjectMap &map) {
  Brace br(this);
  ...
  while (br) {
    ...
    if (test(skeys::name_key) || test(lkeys::name_key)) {
      ...
      if (!net) {
        throw tl::Exception(tl::to_string(tr("Not a valid net ID: ")) +
                            tl::to_string(netid));
      }
  ...
  //  NOTE: because we identify pins by their order and not by ID we need to
  //  ensure the pin IDs are generated sequentially.
...
size_t
LayoutToNetlistStandardReader::terminal_id(const db::DeviceClass *device_class,
                                           const std::string &tname) {
  const std::vector<db::DeviceTerminalDefinition> &td =
      device_class->terminal_definitions();
  ...
  throw tl::Exception(tl::to_string(tr("Not a valid terminal name: ")) + tname +
                      tl::to_string(tr(" for device class: ")) +
                      device_class->name());
}

std::pair<db::DeviceAbstract *, const db::DeviceClass *>
LayoutToNetlistStandardReader::device_model_by_name(db::Netlist *netlist,
                                                    const std::string &dmname) {
  for (db::Netlist::device_abstract_iterator i =
           netlist->begin_device_abstracts();
       i != netlist->end_device_abstracts(); ++i) {
    if (i->name() == dmname) {
  ...
  if (!cls) {
    throw tl::Exception(
        tl::to_string(tr("Not a valid device abstract name: ")) + dmname);
  }

  return std::make_pair((db::DeviceAbstract *)0, cls);
}

void LayoutToNetlistStandardReader::read_device(
    db::Netlist *netlist, db::LayoutToNetlist *l2n, db::Circuit *circuit,
    ObjectMap &map,
    std::map<db::CellInstArray, std::list<Connections>> &connections) {
  Brace br(this);
  ...
  std::pair<db::DeviceAbstract *, const db::DeviceClass *> dm =
      device_model_by_name(netlist, dmname);
    ...
    if (test(skeys::name_key) || test(lkeys::name_key)) {
      ...
      while (br2) {
        if (!read_trans_part(dm_trans)) {
          throw tl::Exception(tl::to_string(
              tr("Invalid keyword inside device definition (location, scale, "
                 "rotation or mirror expected)")));
        }
      }
      ...
      if (device_comp_index < 0 ||
          device_comp_index > int(device->other_abstracts().size())) {
        throw tl::Exception(
            tl::to_string(tr("Not a valid device component index: ")) +
            tl::to_string(device_comp_index));
      }

      size_t touter_id = terminal_id(dm.second, touter);
      size_t tinner_id = terminal_id(dm.second, tinner);

      device->reconnected_terminals()[(unsigned int)touter_id].push_back(
          db::DeviceReconnectedTerminal(size_t(device_comp_index),
                                        (unsigned int)tinner_id));
      ...
      const std::vector<db::DeviceParameterDefinition> &pd =
          dm.second->parameter_definitions();
      ...
      if (pid == std::numeric_limits<size_t>::max()) {
        ...
        pid = dc->add_parameter_definition(
                    db::DeviceParameterDefinition(pname, std::string()))
                  .id();
      }
      ...
      throw tl::Exception(tl::to_string(
          tr("Unexpected end of file inside device definition (location, "
             "scale, mirror, rotation, param or terminal expected)")));
    } else {
      skip_element();
    }
  }
  ...
  if (l2n && dm.first) {
    ...
    db::CellInstArray inst(db::CellInst(dm.first->cell_index()),
                           dbu_inv * trans * dbu);
    ccell.insert(inst);
    insts.push_back(inst);

    const std::vector<db::DeviceAbstractRef> &other_devices =
        device->other_abstracts();
    for (std::vector<db::DeviceAbstractRef>::const_iterator i =
             other_devices.begin();
         i != other_devices.end(); ++i) {

      db::CellInstArray other_inst(
          db::CellInst(i->device_abstract->cell_index()),
          dbu_inv * trans * i->trans * dbu);
    ...
    for (size_t tid = 0; tid < max_tid; ++tid) {
      ...
      if (!device->reconnected_terminals().empty()) {
        ...
        const std::vector<db::DeviceReconnectedTerminal> *tr =
            device->reconnected_terminals_for((unsigned int)tid);
        if (tr) {

          for (std::vector<db::DeviceReconnectedTerminal>::const_iterator i =
                   tr->begin();
               i != tr->end(); ++i) {
            const db::DeviceAbstract *da = dm.first;
            if (i->device_index > 0) {
              da = device->other_abstracts()[i->device_index - 1]
                       .device_abstract;
            }
            Connections ref(net->cluster_id(),
                            da->cluster_id_for_terminal(i->other_terminal_id));
        ...
        Connections ref(net->cluster_id(),
                        dm.first->cluster_id_for_terminal(tid));
...
bool LayoutToNetlistStandardReader::read_trans_part(db::DCplxTrans &tr) {
  if (test(skeys::location_key) || test(lkeys::location_key)) {
    ...
    tr = db::DCplxTrans(tr.mag(), tr.angle(), tr.is_mirror(),
                        db::DVector(m_dbu * x, m_dbu * y));
    return true;
...
void LayoutToNetlistStandardReader::read_subcircuit(
    db::Netlist *netlist, db::LayoutToNetlist *l2n, db::Circuit *circuit,
    ObjectMap &map,
    std::map<db::CellInstArray, std::list<Connections>> &connections) {
  Brace br(this);
  ...
  if (!circuit_ref) {
    throw tl::Exception(tl::to_string(tr("Not a valid device circuit name: ")) +
                        xname);
  }
  ...
  while (br) {
    ...
    if (test(skeys::name_key) || test(lkeys::name_key)) {
      ...
      if (!sc_pin) {
        throw tl::Exception(
            tl::to_string(tr("Not a valid pin ID: ")) + tl::to_string(pin_id) +
            tl::to_string(tr(" for circuit: ")) + circuit_ref->name());
      }
      ...
      throw tl::Exception(tl::to_string(
          tr("Unexpected end of file inside subcircuit definition (location, "
             "rotation, mirror, scale or pin expected)")));
    } else {
      skip_element();
    }
  }
  ...
  if (id > 0) {
    map.id2subcircuit.insert(
        std::make_pair((unsigned int)id, subcircuit.get()));
  }
  ...
  if (l2n) {
    ...
    db::CellInstArray inst(db::CellInst(circuit_ref->cell_index()),
                           db::CplxTrans(m_dbu).inverted() * trans *
                               db::CplxTrans(m_dbu));
    db::Cell &ccell = l2n->internal_layout()->cell(circuit->cell_index());
    ccell.insert(inst);
...
void LayoutToNetlistStandardReader::read_abstract_terminal(
    db::LayoutToNetlist *l2n, db::DeviceAbstract *dm, db::DeviceClass *dc) {
  Brace br(this);
  ...
  if (tid == std::numeric_limits<size_t>::max()) {
    if (!dc) {
      throw tl::Exception(tl::to_string(tr("Not a valid terminal name: ")) +
                          name + tl::to_string(tr(" for device class: ")) +
                          dm->device_class()->name());
    }
  ...
  if (l2n) {
    ...
    db::connected_clusters<db::NetShape> &cc =
        l2n->net_clusters().clusters_per_cell(dm->cell_index());
    db::local_cluster<db::NetShape> &lc = *cc.insert();
