...
class OriginalLayerRegionIterator : public RegionIteratorDelegate {
public:
  OriginalLayerRegionIterator(const db::RecursiveShapeIterator &iter,
  ...
  virtual bool at_end() const { return m_rec_iter.at_end(); }
  ...
  equals(const generic_shape_iterator_delegate_base<value_type> *other) const {
    const OriginalLayerRegionIterator *o =
        dynamic_cast<const OriginalLayerRegionIterator *>(other);
    return o && o->m_rec_iter == m_rec_iter &&
           o->m_iter_trans.equal(m_iter_trans);
  }
  ...
  virtual db::Box bbox() const { return m_iter_trans * m_rec_iter.bbox(); }
...
private:
  ...
  void set() {
    while (!m_rec_iter.at_end() &&
           !(m_rec_iter->is_polygon() || m_rec_iter->is_path() ||
             m_rec_iter->is_box())) {
      ++m_rec_iter;
    }
...
OriginalLayerRegion::OriginalLayerRegion(const OriginalLayerRegion &other)
    : AsIfFlatRegion(other), m_is_merged(other.m_is_merged),
      m_merged_polygons(other.m_merged_polygons),
      m_merged_polygons_valid(other.m_merged_polygons_valid),
      m_iter(other.m_iter), m_iter_trans(other.m_iter_trans) {
  //  .. nothing yet ..
}

OriginalLayerRegion::OriginalLayerRegion(const RecursiveShapeIterator &si,
                                         bool is_merged)
...
OriginalLayerRegion::OriginalLayerRegion(const RecursiveShapeIterator &si,
                                         const db::ICplxTrans &trans,
                                         bool merged_semantics, bool is_merged)
    : AsIfFlatRegion(), m_merged_polygons(false), m_iter(si),
      m_iter_trans(trans) {
  init();
...
RegionDelegate *OriginalLayerRegion::clone() const {
...
size_t OriginalLayerRegion::count() const {
  //  NOTE: we should to make sure the iterator isn't validated as this would
  //  spoil the usability or OriginalLayerRegion upon layout changes
  db::RecursiveShapeIterator iter = m_iter;

  if (iter.has_complex_region() || iter.region() != db::Box::world() ||
      !iter.enables().empty() || !iter.disables().empty()) {

    //  complex case with a search region - use the iterator to determine the
    //  count (expensive)
    size_t n = 0;
    ...
    if (iter.shapes()) {
      return iter.shapes()->size(iter.shape_flags() &
    ...
    for (db::Layout::top_down_const_iterator c = layout.begin_top_down();
      ...
      if (iter.multiple_layers()) {
        for (std::vector<unsigned int>::const_iterator l =
                 iter.layers().begin();
             l != iter.layers().end(); ++l) {
          nn += layout.cell(*c).shapes(*l).size(
              iter.shape_flags() &
              (db::ShapeIterator::Regions | db::ShapeIterator::Properties));
        }
      } else if (iter.layer() < layout.layers()) {
        nn += layout.cell(*c)
                  .shapes(iter.layer())
                  .size(iter.shape_flags() & (db::ShapeIterator::Regions |
                                              db::ShapeIterator::Properties));
      }
      n += cc.weight(*c) * nn;
    }
...
size_t OriginalLayerRegion::hier_count() const {
  //  NOTE: we should to make sure the iterator isn't validated as this would
  //  spoil the usability or OriginalLayerRegion upon layout changes
  db::RecursiveShapeIterator iter = m_iter;
...
RegionIteratorDelegate *OriginalLayerRegion::begin() const {
  return new OriginalLayerRegionIterator(m_iter, m_iter_trans);
}

RegionIteratorDelegate *OriginalLayerRegion::begin_merged() const {
...
OriginalLayerRegion::begin_merged_iter() const {
  if (!merged_semantics() || m_is_merged) {
    ...
    ensure_merged_polygons_valid();
    return std::make_pair(db::RecursiveShapeIterator(m_merged_polygons),
                          db::ICplxTrans());
  }
}

bool OriginalLayerRegion::empty() const { return m_iter.at_end(); }

bool OriginalLayerRegion::is_merged() const { return m_is_merged; }

const db::Polygon *OriginalLayerRegion::nth(size_t) const {
  throw tl::Exception(tl::to_string(
      tr("Random access to polygons is available only for flat regions")));
}

db::properties_id_type OriginalLayerRegion::nth_prop_id(size_t) const {
  throw tl::Exception(tl::to_string(
      tr("Random access to polygons is available only for flat regions")));
}

bool OriginalLayerRegion::has_valid_polygons() const { return false; }

bool OriginalLayerRegion::has_valid_merged_polygons() const {
  return merged_semantics() && !m_is_merged;
}

const db::RecursiveShapeIterator *OriginalLayerRegion::iter() const {
  return &m_iter;
}

void OriginalLayerRegion::apply_property_translator(
...
db::PropertiesRepository *OriginalLayerRegion::properties_repository() {
  return m_iter.layout() ? &const_cast<db::Layout *>(m_iter.layout())
...
bool OriginalLayerRegion::equals(const Region &other) const {
  const OriginalLayerRegion *other_delegate =
      dynamic_cast<const OriginalLayerRegion *>(other.delegate());
  if (other_delegate && other_delegate->m_iter == m_iter &&
      other_delegate->m_iter_trans == m_iter_trans) {
    return true;
  } else {
...
void OriginalLayerRegion::insert_into(Layout *layout,
