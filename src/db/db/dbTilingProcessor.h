 ...
 */
class DB_PUBLIC TileOutputReceiver : public gsi::ObjectBase, public tl::Object {
public:
  /**
   *  @brief Constructor
   */
  TileOutputReceiver() : mp_proc(0) {
    //  .. nothing yet ..
  }
   ...
   *
   *  All coordinates are given in micron. If tiles are not used, nx and ny are
   * 0.
   */
...
private:
   ...
   */
  void set_processor(TilingProcessor *proc) { mp_proc = proc; }
 ...
 *  This function is responsible for preparing (i.e. clipping) and delivering
 * the output.
 */
template <class X>
...
void insert(X &inserter, const db::Polygon &o, const db::Box &tile, bool clip) {
  if (o.is_box()) {
    ...
    if (o.box().touches(tile)) {
      ...
      for (std::vector<db::Polygon>::const_iterator cp = clipped_poly.begin();
           cp != clipped_poly.end(); ++cp) {
        inserter(*cp);
      }
    }
  } else {
 ...
 */
template <class X>
void insert(X &inserter, const db::SimplePolygon &o, const db::Box &tile,
            bool clip) {
  if (o.is_box()) {
    //  simple case: box
    insert(inserter, o.box(), tile, clip);
 ...
 */
template <class X>
void insert(X &inserter, const db::EdgePair &o, const db::Box &tile,
            bool clip) {
  if (clip) {
    //  no real clipping for edge pairs - just select the ones that touch the
    //  clip rectangle
...
void insert(X &inserter, const db::Edge &o, const db::Box &tile, bool clip) {
  if (clip) {
    //  apply clipping. To ensure an edge is just added once, clipping is
    //  applied in a somewhat special way: edges coincident with the clip box
    //  are only taken if their inside side is towards the clip box (see
    //  Region::perimeter implementation).
 ...
 */
template <class X>
void insert(X &inserter, const db::Region &data, const db::Box &tile,
            bool clip) {
 ...
 */
template <class X>
void insert(X &inserter, const db::Edges &data, const db::Box &tile,
            bool clip) {
 ...
 */
template <class X>
void insert(X &inserter, const db::EdgePairs &data, const db::Box &tile,
            bool clip) {
 ...
 */
template <class X>
void insert(X &inserter, const db::Texts &data, const db::Box &tile,
            bool clip) {
 ...
 */
template <class X>
bool insert_var(X &inserter, const tl::Variant &obj, const db::Box &tile,
                bool clip) {
 ...
 *  Multiple outputs can also be specified. Each output channel is assigned a
 * name which will be available to the script through a variable. To output an
 * object use _output(name, object).
 *
 *  Inside the scripts provided, the following variables and functions are
 *  available:
 *   _tile       a function, delivers the region describing the current tile or
 * nil if tiling is not enabled _dbu        the database unit used for the data
 *   _output     output the data: _output(name, object)
 *   _count      report a count for a specific output for display during
 * execution <name>      fetch the input <name> as specified in the "input"
 * method
 */
...
public:
   ...
   *
   *  The name is the variable name which will receive the given value in the
   * script.
   */
   ...
   *
   *  The name is the variable name which will receive the shapes for the
   * individual tile. The transformation can be used to convert between database
   * units. If "as_region" is false, the input is taken as edge input.
   */
  void input(const std::string &name, const db::RecursiveShapeIterator &iter,
             const db::ICplxTrans &trans = db::ICplxTrans(),
             Type type = TypeRegion, bool merged_semantics = true);

  /**
   *  @brief Specifies the output
   *
   *  The name is the variable which carries the context information.
   *  In the script, the output given by this specification can be referred to
   * as _output(name, object).
   *
   *  This version will employ the output receiver given by "rec". "id" will be
   * passed to the "id" parameter of the receiver's "put" implementation.
   *
   *  The ownership of the receiver object will be passed over to the processor.
   */
  void output(const std::string &name, size_t id, TileOutputReceiver *rec,
              const db::ICplxTrans &trans);

  /**
   *  @brief Specifies output to a layout
   *
   *  This version will specify output to a layout object to the specified cell
   * and layer. The ep_ext parameter specifies what extension to apply when
   * converting edge pairs to polygons.
   */
  void output(const std::string &name, db::Layout &layout,
              db::cell_index_type cell, const db::LayerProperties &lp,
              db::Coord ep_ext = 1);
   ...
   */
  void output(const std::string &name, db::Layout &layout,
              db::cell_index_type cell, unsigned int layer,
              db::Coord ep_ext = 1);
   ...
   *  The ep_ext parameter specifies what extension to apply when converting
   * edge pairs to polygons.
   */
  void output(const std::string &name, db::Region &region,
              db::Coord ep_ext = 1);
   ...
   */
  double dbu() const { return m_dbu_specific_set ? m_dbu_specific : m_dbu; }

  /**
   *  @brief Sets the database unit under which the computation will be done
   *
   *  Using this method will establish the given database unit as the target
   * unit. Otherwise, the first input's database unit will be used.
   */
   ...
   */
  void set_scale_to_dbu(bool sdbu) { m_scale_to_dbu = sdbu; }

  /**
   *  @brief Returns a value indicating whether automatic scaling to database
   * unit is enabled
   */
  bool scale_to_dbu() const { return m_scale_to_dbu; }

  /**
   *  @brief Sets the layout frame
   *
   *  The layout frame is the box (in micron units) taken into account for
   * computing the tiles if the tile counts are not given. If the layout frame
   * is not set or set to an empty box, the processor will try to derive the
   * frame from the given inputs.
   */
   ...
   */
  const db::DBox &frame() const { return m_frame; }

  /**
   *  @brief Sets the tile size
   *
   *  The tile size is given in micron units.
   *  Alternatively, the tiling can be specified in terms of tile number (tiles
   * method). The tiles method will override the tile size specified in
   * tile_size.
   */
   ...
   *  Alternatively, the tiling can be specified in terms of tile size
   * (tile_size method). The tile_size method will override the tile count
   * specified in tiles.
   */
   ...
   *
   *  Multiple scripts can be installed. They will be executed in parallel (if
   * the number of threads allows).
   */
   ...
   */
  static tl::Mutex &output_lock() { return s_output_lock; }
...
private:
  ...
  std::vector<InputSpec>::const_iterator begin_inputs() const {
    return m_inputs.begin();
  }
  std::vector<InputSpec>::const_iterator end_inputs() const {
    return m_inputs.end();
  }

  void put(size_t ix, size_t iy, const db::Box &tile,
