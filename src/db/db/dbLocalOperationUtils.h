...
class polygon_transformation_filter : public PolygonSink {
public:
  /**
...
template <class T> class DB_PUBLIC polygon_ref_generator;

template <>
class DB_PUBLIC polygon_ref_generator<db::PolygonRef> : public PolygonSink {
public:
  /**
   *  @brief Constructor
   */
  polygon_ref_generator(db::Layout *layout,
                        std::unordered_set<db::PolygonRef> &polyrefs)
...
template <>
class DB_PUBLIC polygon_ref_generator<db::Polygon> : public PolygonSink {
public:
  /**
   *  @brief Constructor
   */
   ...
   *  @brief Implementation of the PolygonSink interface
   */
  virtual void put(const db::Polygon &polygon) { mp_polygons->insert(polygon); }
...
template <class T> class DB_PUBLIC polygon_ref_generator_with_properties;

template <>
class DB_PUBLIC
    polygon_ref_generator_with_properties<db::PolygonRefWithProperties>
    : public PolygonSink {
public:
  /**
   *  @brief Constructor
   */
  polygon_ref_generator_with_properties(
      db::Layout *layout,
      std::unordered_set<db::PolygonRefWithProperties> &polyrefs,
...
public:
   ...
   */
  polygon_ref_generator_with_properties(
      db::Layout *, std::unordered_set<db::PolygonWithProperties> &polygons,
      db::properties_id_type prop_id)
...
class DB_PUBLIC edge_to_edge_set_generator : public EdgeSink {
public:
  /**
   *  @brief Constructor
   */
  edge_to_edge_set_generator(Container &edges, int tag = 0,
                             EdgeSink *chained = 0)
...
typedef edge_to_edge_set_generator<std::unordered_set<db::Edge>>
    EdgeToEdgeSetGenerator;

class DB_PUBLIC PolygonRefToShapesGenerator : public PolygonSink {
public:
  /**
   *  @brief Constructor specifying an external vector for storing the polygons
   */
...
class DB_PUBLIC PolygonSplitter : public PolygonSink {
public:
  PolygonSplitter(PolygonSink &sink, double max_area_ratio,
...
template <class T, class Container> class DB_PUBLIC property_injector {
public:
  ...
  const_iterator begin() const { return mp_container->begin(); }

  const_iterator end() const { return mp_container->end(); }
 ...
 *
 *  Returns a map of property ID, subject shapes and intruder shapes belonging
 * to the subject shapes. Depending on the property constraint the intruders
 * will either be ones with and properties (NoPropertyConstraint), the same
 * properties than the subject (SamePropertiesConstraint) or different
 * properties (DifferentPropertiesConstraint).
 */
template <class TS, class TI>
DB_PUBLIC_TEMPLATE
    std::map<db::properties_id_type,
             std::pair<std::vector<const TS *>, std::set<const TI *>>>
    separate_interactions_by_properties(
        const shape_interactions<db::object_with_properties<TS>,
                                 db::object_with_properties<TI>> &interactions,
        db::PropertyConstraint property_constraint, db::PropertyMapper &pms,
        db::PropertyMapper &pmi) {
  std::map<db::properties_id_type,
           std::pair<std::vector<const TS *>, std::set<const TI *>>>
      by_prop_id;
  ...
  for (auto i = interactions.begin(); i != interactions.end(); ++i) {
    ...
    std::pair<std::vector<const TS *>, std::set<const TI *>> &s2p =
        by_prop_id[prop_id];
    s2p.first.push_back(&subject);

    for (auto ii = i->second.begin(); ii != i->second.end(); ++ii) {

      const std::pair<unsigned int, db::object_with_properties<TI>> &intruder =
          interactions.intruder_shape(*ii);

      if (pc_match(property_constraint, prop_id,
                   pmi(intruder.second.properties_id()))) {
...
DB_PUBLIC_TEMPLATE
    ...
    separate_interactions_to_interactions_by_properties(
        const shape_interactions<db::object_with_properties<TS>,
                                 db::object_with_properties<TI>> &interactions,
        db::PropertyConstraint property_constraint, db::PropertyMapper &pms,
        std::vector<db::PropertyMapper> &pmis) {
  std::map<db::properties_id_type, db::shape_interactions<TS, TI>> by_prop_id;
  std::map<db::properties_id_type, std::set<unsigned int>>
      intruder_ids_by_prop_id;
  ...
  for (auto i = interactions.begin(); i != interactions.end(); ++i) {
    ...
    for (auto ii = i->second.begin(); ii != i->second.end(); ++ii) {
      ...
      if (pc_match(property_constraint, prop_id,
                   pmis[intruder.first](intruder.second.properties_id()))) {
  ...
  for (auto i = intruder_ids_by_prop_id.begin();
       i != intruder_ids_by_prop_id.end(); ++i) {

    db::shape_interactions<TS, TI> &s2p = by_prop_id[i->first];
    const std::set<unsigned int> &intruder_ids =
