...
template <class C> std::string text<C>::to_string(double dbu) const {
  std::string s = std::string("(") + tl::to_quoted_string(string()) + "," +
                  m_trans.to_string(dbu) + ")";
...
template <class C>
bool _test_extractor_impl(tl::Extractor &ex, db::text<C> &t) {
  if (ex.test("(")) {
...
template <>
DB_PUBLIC bool test_extractor_impl(tl::Extractor &ex, db::DText &p) {
  return _test_extractor_impl(ex, p);
}
