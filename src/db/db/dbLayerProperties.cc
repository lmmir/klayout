...
std::string LayerProperties::to_string(bool as_target) const {
  std::string r;
  if (!name.empty()) {
    if (is_named()) {
      r = tl::to_word_or_quoted_string(name);
    } else {
      r = tl::to_word_or_quoted_string(name) + " (" + format_ld(layer) + "/" +
          format_ld(datatype) + ")";
    }
...
LayerOffset::LayerOffset() : name(), layer(-1), datatype(-1) {}

LayerOffset::LayerOffset(int l, int d) : name(), layer(l), datatype(d) {}
...
bool LayerOffset::is_named() const { return layer < 0 || datatype < 0; }

std::string LayerOffset::to_string() const {
  std::string r;
  if (!name.empty()) {
    if (is_named()) {
...
LayerProperties LayerOffset::apply(const LayerProperties &props) const {
...
template <>
bool test_extractor_impl(tl::Extractor &ex, db::LayerProperties &e) {
  e.read(ex);
...
template <>
DB_PUBLIC void extractor_impl(tl::Extractor &ex, db::LayerProperties &e) {
...
template <>
DB_PUBLIC void extractor_impl(tl::Extractor &ex, db::LayerOffset &e) {
  if (!test_extractor_impl(ex, e)) {
