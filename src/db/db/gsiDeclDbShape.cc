...
static double shape_dbu(const db::Shape *s) {
  ...
  if (!layout) {
    throw tl::Exception(
        tl::to_string(tr("Shape does not reside inside a layout - cannot "
...
static db::Shapes *shapes_checked(db::Shape *s) {
  ...
  if (!shapes) {
    throw tl::Exception(
        tl::to_string(tr("Shape does not reside inside a shape container - "
...
static void transform_shape_dtrans(db::Shape *s, const db::DTrans &trans) {
  ...
  db::Shapes *shapes = shapes_checked(s);
  *s = shapes->transform(*s, dbu_trans.inverted() * db::DCplxTrans(trans) *
                                 dbu_trans);
}
...
static int object_type(const db::Shape *s) { return int(s->type()); }
...
static void set_text_dpos(db::Shape *s, const db::DVector &q) {
  ...
  s->text(p);
  p.trans(
      db::Trans(p.trans().rot(), db::CplxTrans(shape_dbu(s)).inverted() * q));
  *s = shapes->replace(*s, p);
}
...
static db::DTrans text_dtrans(const db::Shape *s) {
  db::CplxTrans dbu_trans(shape_dbu(s));
  check_is_text(s);
  return db::DTrans(dbu_trans * db::ICplxTrans(s->text_trans()) *
                    dbu_trans.inverted());
}
...
static void set_text_dtrans(db::Shape *s, const db::DTrans &t) {
  db::CplxTrans dbu_trans(shape_dbu(s));
  set_text_trans(
      s, db::Trans(dbu_trans.inverted() * db::DCplxTrans(t) * dbu_trans));
}
...
static void set_box_width(db::Shape *s, db::Coord w) {
  ...
  s->box(p);
  *s = shapes->replace(
      *s, db::Shape::box_type(p.center().x() - w / 2, p.p1().y(),
                              p.center().x() + (w - w / 2), p.p2().y()));
}
...
static void set_box_height(db::Shape *s, db::Coord h) {
  ...
  s->box(p);
  *s = shapes->replace(
      *s, db::Shape::box_type(p.p1().x(), p.center().y() - h / 2, p.p2().x(),
                              p.center().y() + (h - h / 2)));
}
...
static db::Shapes *shapes_ptr(db::Shape *s) { return s->shapes(); }

static unsigned int shape_layer_index(const db::Shape *s) {
  db::Shapes *shapes = s->shapes();
  if (!shapes) {
    throw tl::Exception(
        tl::to_string(tr("Shape does not belong to a shape container")));
  }
...
static void set_shape_layer_index(db::Shape *s, unsigned int layer) {
  ...
  if (!layout->is_valid_layer(layer)) {
    throw tl::Exception(
        tl::to_string(tr("Layer index does not point to a valid layer")));
  }
...
static void set_shape_layer(db::Shape *s, const db::LayerProperties &lp) {
  ...
  throw tl::Exception(
      tl::to_string(tr("Layer info object is not giving a valid layer")));
}
...
static void set_cell_ptr(db::Shape *s, db::Cell *new_cell) {
  ...
  if (layout_ptr(s) != new_cell->layout()) {
    throw tl::Exception(
        tl::to_string(tr("Current and new cell belong to a different layout")));
  }
...
template <class SH> void set_shape(db::Shape *s, const SH &obj) {
  db::Shapes *shapes = shapes_checked(s);
  *s = shapes->replace(*s, obj);
}

template <class SH> void set_dshape(db::Shape *s, const SH &obj) {
  db::CplxTrans dbu_trans(shape_dbu(s));
  db::Shapes *shapes = shapes_checked(s);
...
static void delete_property(db::Shape *s, const tl::Variant &key) {
  ...
  if (!layout) {
    throw tl::Exception(tl::to_string(tr(
        "Shape does not reside inside a layout - cannot delete properties")));
  }
...
static void set_property(db::Shape *s, const tl::Variant &key,
  ...
  if (!layout) {
    throw tl::Exception(tl::to_string(
        tr("Shape does not reside inside a layout - cannot set properties")));
  }
...
static tl::Variant get_property(const db::Shape *s, const tl::Variant &key) {
  ...
  if (!layout) {
    throw tl::Exception(tl::to_string(tr(
        "Shape does not reside inside a layout - cannot retrieve properties")));
  }
...
template <class I, class T> struct ConvertingIteratorWrapper {
  ...
  reference operator*() const { return *m_b * m_dbu; }
...
template <class I, class T> struct ConvertingFreeIteratorWrapper {
  typedef void difference_type;
  typedef T value_type;
  typedef T reference;
  typedef void pointer;
  ...
  ConvertingFreeIteratorWrapper(double dbu, const I &b) : m_b(b), m_dbu(dbu) {
    //  .. nothing yet ..
  }
  ...
  bool at_end() const { return m_b.at_end(); }
...
static ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>
shape_dpoint_iter(const db::Shape *s) {
  return ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>(
      shape_dbu(s), s->begin_point(), s->end_point());
}

static ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>
shape_dpoint_hull_iter(const db::Shape *s) {
  return ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>(
      shape_dbu(s), s->begin_hull(), s->end_hull());
}

static ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>
shape_dpoint_hole_iter(const db::Shape *s, unsigned int hole) {
  return ConvertingIteratorWrapper<db::Shape::point_iterator, db::DPoint>(
      shape_dbu(s), s->begin_hole(hole), s->end_hole(hole));
}

static ConvertingFreeIteratorWrapper<db::Shape::polygon_edge_iterator,
                                     db::DEdge>
shape_edge_iter(const db::Shape *s) {
  return ConvertingFreeIteratorWrapper<db::Shape::polygon_edge_iterator,
                                       db::DEdge>(shape_dbu(s),
                                                  s->begin_edge());
}

static ConvertingFreeIteratorWrapper<db::Shape::polygon_edge_iterator,
                                     db::DEdge>
shape_edge_iter_per_contour(const db::Shape *s, unsigned int contour) {
  return ConvertingFreeIteratorWrapper<db::Shape::polygon_edge_iterator,
                                       db::DEdge>(shape_dbu(s),
                                                  s->begin_edge(contour));
}

static db::DTrans array_dtrans(const db::Shape *s) {
  db::CplxTrans dbu_trans(shape_dbu(s));
  return db::DTrans(dbu_trans * db::ICplxTrans(s->array_trans()) *
                    dbu_trans.inverted());
}
...
static int t_polygonPtrArrayMember() {
  return db::Shape::PolygonPtrArrayMember;
}
static int t_simplePolygon() { return db::Shape::SimplePolygon; }
static int t_simplePolygonRef() { return db::Shape::SimplePolygonRef; }
static int t_simplePolygonPtrArray() {
  return db::Shape::SimplePolygonPtrArray;
}
static int t_simplePolygonPtrArrayMember() {
...
Class<db::Shape> decl_Shape(
    "db", "Shape",
    gsi::method("prop_id",
                (db::properties_id_type(db::Shape::*)() const) &
                    db::Shape::prop_id,
                "@brief Gets the properties ID associated with the shape\n"
                "\n"
                "The \\Layout object can be used to retrieve the actual "
                "properties associated with the ID.") +
        gsi::method_ext("prop_id=", &set_prop_id,
                        "@brief Sets the properties ID of this shape\n"
                        "\n"
                        "The \\Layout object can be used to retrieve an ID for "
                        "a given set of properties. "
                        "Calling this method will invalidate any iterators. It "
                        "should not be called inside a "
                        "loop iterating over shapes.\n"
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::method("has_prop_id?", &db::Shape::has_prop_id,
                    "@brief Returns true, if the shape has properties, i.e. "
                    "has a properties ID\n") +
        gsi::method_ext("shapes", &shapes_ptr,
                        "@brief Gets a reference to the Shapes container the "
                        "shape lives in\n"
                        "\n"
                        "This reference can be nil, if the Shape object is not "
                        "referring to an actual shape.\n"
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::method_ext(
            "cell", &cell_ptr,
            ...
            "\n"
            "This reference can be nil, if the Shape object is not living "
            "inside a cell\n"
            "\n"
            "This method has been introduced in version 0.22.") +
        gsi::method_ext("cell=", &set_cell_ptr, gsi::arg("cell"),
                        ...
                        "\n"
                        "Both the current and the target cell must reside in "
                        "the same layout.\n"
                        "\n"
        ...
        gsi::method_ext("box=", &set_shape<db::Box>, gsi::arg("box"),
                        ...
                        "This method replaces the shape by the given box. This "
                        "method can only be called "
                        "for editable layouts. It does not change the user "
                        ...
                        "This method has been introduced in version 0.22.") +
        gsi::method_ext(
            "box=|dbox=", &set_dshape<db::DBox>, gsi::arg("box"),
            "@brief Replaces the shape by the given box (in micrometer units)\n"
            "This method replaces the shape by the given box, like \\box= with "
            "a \\Box argument does. "
            "This version translates the box from micrometer units to database "
            "units internally.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        gsi::method_ext("path=", &set_shape<db::Path>, gsi::arg("box"),
                        "@brief Replaces the shape by the given path object\n"
                        "This method replaces the shape by the given path "
                        "object. This method can only be called "
                        "for editable layouts. It does not change the user "
                        "properties of the shape.\n"
                        "Calling this method will invalidate any iterators. It "
                        "should not be called inside a "
                        ...
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::method_ext("path=|dpath=", &set_dshape<db::DPath>,
                        gsi::arg("path"),
                        "@brief Replaces the shape by the given path (in "
                        "micrometer units)\n"
                        "This method replaces the shape by the given path, "
                        "like \\path= with a \\Path argument does. "
                        "This version translates the path from micrometer "
                        "units to database units internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.") +
        gsi::method_ext(
            "polygon=", &set_shape<db::Polygon>, gsi::arg("box"),
            "@brief Replaces the shape by the given polygon object\n"
            "This method replaces the shape by the given polygon object. This "
            "method can only be called "
            "for editable layouts. It does not change the user properties of "
            "the shape.\n"
            "Calling this method will invalidate any iterators. It should not "
            "be called inside a "
            "loop iterating over shapes.\n"
            "\n"
            "This method has been introduced in version 0.22.") +
        gsi::method_ext("polygon=|dpolygon=", &set_dshape<db::DPolygon>,
                        gsi::arg("polygon"),
                        "@brief Replaces the shape by the given polygon (in "
                        "micrometer units)\n"
                        "This method replaces the shape by the given polygon, "
                        "like \\polygon= with a \\Polygon argument does. "
                        "This version translates the polygon from micrometer "
                        "units to database units internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.") +
        gsi::method_ext("text=", &set_shape<db::Text>, gsi::arg("box"),
                        "@brief Replaces the shape by the given text object\n"
                        "This method replaces the shape by the given text "
                        "object. This method can only be called "
                        "for editable layouts. It does not change the user "
                        "properties of the shape.\n"
                        "Calling this method will invalidate any iterators. It "
                        ...
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::method_ext("text=|dtext=", &set_dshape<db::DText>,
                        gsi::arg("text"),
                        "@brief Replaces the shape by the given text (in "
                        "micrometer units)\n"
                        "This method replaces the shape by the given text, "
                        "like \\text= with a \\Text argument does. "
                        "This version translates the text from micrometer "
                        "units to database units internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.") +
        gsi::method_ext("edge=", &set_shape<db::Edge>, gsi::arg("edge"),
                        "@brief Replaces the shape by the given edge\n"
                        "This method replaces the shape by the given edge. "
                        ...
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::method_ext("edge=|dedge=", &set_dshape<db::DEdge>,
                        gsi::arg("edge"),
                        "@brief Replaces the shape by the given edge (in "
                        "micrometer units)\n"
                        "This method replaces the shape by the given edge, "
                        "like \\edge= with a \\Edge argument does. "
                        "This version translates the edge from micrometer "
                        "units to database units internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.") +
        gsi::method_ext("point=", &set_shape<db::Point>, gsi::arg("point"),
                        "@brief Replaces the shape by the given point\n"
                        "This method replaces the shape by the given point. "
                        "This method can only be called "
                        "for editable layouts. It does not change the user "
                        "properties of the shape.\n"
                        "Calling this method will invalidate any iterators. It "
        ...
        gsi::method_ext("point=|dpoint=", &set_dshape<db::DPoint>,
                        gsi::arg("point"),
                        "@brief Replaces the shape by the given point (in "
                        "micrometer units)\n"
                        "This method replaces the shape by the given point, "
                        "like \\point= with a \\Point argument does. "
                        "This version translates the point from micrometer "
                        "units to database units internally.\n"
                        "\n"
                        "This method has been introduced in version 0.28.") +
        gsi::method_ext("edge_pair=", &set_shape<db::EdgePair>,
                        gsi::arg("edge_pair"),
                        "@brief Replaces the shape by the given edge pair\n"
                        "This method replaces the shape by the given edge "
                        "pair. This method can only be called "
                        "for editable layouts. It does not change the user "
                        "properties of the shape.\n"
                        "Calling this method will invalidate any iterators. It "
                        "should not be called inside a "
        ...
        gsi::method_ext(
            "edge_pair=|dedge_pair=", &set_dshape<db::DEdgePair>,
            gsi::arg("edge_pair"),
            "@brief Replaces the shape by the given edge pair (in micrometer "
            "units)\n"
            "This method replaces the shape by the given edge pair, like "
            "\\edge_pair= with a \\EdgePair argument does. "
            "This version translates the edge pair from micrometer units to "
        ...
        gsi::method_ext("property", &get_property, gsi::arg("key"),
                        ...
                        "property with the given key. "
                        "If no property with that key does not exist, it will "
                        "return nil. Using that method is more "
                        "convenient than using the layout object and the "
                        "properties ID to retrieve the property value. "
                        "\n"
                        "This method has been introduced in version 0.22.") +
        gsi::iterator("each_point", &db::Shape::begin_point,
                      &db::Shape::end_point,
                      "@brief Iterates over all points of the object\n"
                      "\n"
                      "This method applies to paths and delivers all points of "
                      "the path's center line.\n"
                      "It will throw an exception for other objects.\n") +
        gsi::iterator_ext("each_dpoint", &shape_dpoint_iter,
                          "@brief Iterates over all points of the object and "
                          "returns points in micrometer units\n"
                          "\n"
                          "This method iterates over all points of the object "
                          "like \\each_point, but it returns "
                          "\\DPoint objects that are given in micrometer units "
                          "already. Multiplication with "
                          "the database unit happens internally.\n"
                          "\n"
                          "This method has been introduced in version 0.25.") +
        gsi::iterator("each_point_hull", &db::Shape::begin_hull,
                      &db::Shape::end_hull,
                      "@brief Iterates over the hull contour of the object\n"
                      "\n"
                      "This method applies to polygons and delivers all points "
                      "of the polygon hull contour.\n"
                      "It will throw an exception for other objects.\n") +
        gsi::iterator_ext(
            "each_dpoint_hull", &shape_dpoint_hull_iter,
            "@brief Iterates over the hull contour of the object and returns "
            "points in micrometer units\n"
            "\n"
            "This method iterates over all points of the object's contour' "
            "like \\each_point_hull, but it returns "
            "\\DPoint objects that are given in micrometer units already. "
            "Multiplication with "
            "the database unit happens internally.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        gsi::iterator("each_point_hole", &db::Shape::begin_hole,
                      &db::Shape::end_hole, gsi::arg("hole_index"),
                      "@brief Iterates over the points of a hole contour\n"
                      "\n"
                      "This method applies to polygons and delivers all points "
                      "of the respective hole contour.\n"
                      "It will throw an exception for other objects.\n"
                      "Simple polygons deliver an empty sequence.\n"
                      "\n"
                      "@param hole The hole index (see holes () method)\n") +
        gsi::iterator_ext(
            "each_dpoint_hole", &shape_dpoint_hole_iter, gsi::arg("hole_index"),
            "@brief Iterates over a hole contour of the object and returns "
            "points in micrometer units\n"
            "\n"
            "This method iterates over all points of the object's contour' "
            "like \\each_point_hole, but it returns "
            "\\DPoint objects that are given in micrometer units already. "
            "Multiplication with "
            "the database unit happens internally.\n"
            "\n"
        ...
        gsi::method("holes", &db::Shape::holes,
                    ...
                    "\n"
                    "This method applies to polygons and will throw an "
                    "exception for other objects..\n"
                    "Simple polygons deliver a value of zero.\n") +
        gsi::iterator(
            "each_edge",
            (db::Shape::polygon_edge_iterator(db::Shape::*)() const) &
                db::Shape::begin_edge,
            "@brief Iterates over the edges of the object\n"
            "\n"
            "This method applies to polygons and simple polygons and delivers "
            "all edges that form the polygon's contours. "
            "Hole edges are oriented counterclockwise while hull edges are "
            "oriented clockwise.\n"
            "\n"
            "It will throw an exception if the object is not a polygon.\n") +
        gsi::iterator_ext("each_dedge", &shape_edge_iter,
                          "@brief Iterates over the edges of the object and "
                          "returns edges in micrometer units\n"
                          "\n"
                          "This method iterates over all edges of polygons and "
                          "simple polygons like \\each_edge, but will deliver "
                          "edges in micrometer units. Multiplication by the "
                          "database unit is done internally.\n"
                          "\n"
                          "This method has been introduced in version 0.25.") +
        gsi::iterator(
            "each_edge",
            (db::Shape::polygon_edge_iterator(db::Shape::*)(unsigned int)
                 const) &
                db::Shape::begin_edge,
            gsi::arg("contour"),
            ...
            "\n"
            "This method applies to polygons and simple polygons and delivers "
            "all edges that form the given contour of the polygon. "
        ...
        gsi::iterator_ext(
            "each_dedge", &shape_edge_iter_per_contour, gsi::arg("contour"),
            "@brief Iterates over the edges of a single contour of the object "
            "and returns edges in micrometer units\n"
            "\n"
            "This method iterates over all edges of polygons and simple "
            "polygons like \\each_edge, but will deliver "
            "edges in micrometer units. Multiplication by the database unit is "
            "done internally.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        gsi::method_ext("box_width", &box_width,
                        ...
                        "\n"
                        "Applies to boxes only. Returns the width of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_dwidth", &box_dwidth,
            "@brief Returns the width of the box in micrometer units\n"
            "\n"
            "Applies to boxes only. Returns the width of the box in "
            "micrometers and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext("box_width=", &set_box_width, gsi::arg("w"),
                        "@brief Sets the width of the box\n"
                        "\n"
                        "Applies to boxes only. Changes the width of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext("box_dwidth=", &set_box_dwidth, gsi::arg("w"),
                        ...
                        "\n"
                        "Applies to boxes only. Changes the width of the box "
                        "to the value given in micrometer units and throws an "
                        "exception if the shape is not a box.\n"
        ...
        gsi::method_ext("box_height", &box_height,
                        ...
                        "\n"
                        "Applies to boxes only. Returns the height of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_dheight", &box_dheight,
            "@brief Returns the height of the box in micrometer units\n"
            "\n"
            "Applies to boxes only. Returns the height of the box in "
            "micrometers and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext("box_height=", &set_box_height, gsi::arg("h"),
                        ...
                        "\n"
                        "Applies to boxes only. Changes the height of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        ...
        gsi::method_ext("box_center", &box_center,
                        ...
                        "\n"
                        "Applies to boxes only. Returns the center of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext("box_dcenter", &box_dcenter,
                        "@brief Returns the center of the box as a \\DPoint "
                        "object in micrometer units\n"
                        "\n"
                        "Applies to boxes only. Returns the center of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "Conversion from database units to micrometers is done "
                        "internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.\n") +
        gsi::method_ext("box_center=", &set_box_center, gsi::arg("c"),
                        ...
                        "\n"
                        "Applies to boxes only. Changes the center of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext("box_center=|box_dcenter=", &set_box_dcenter,
                        gsi::arg("c"),
                        "@brief Sets the center of the box with the point "
                        "being given in micrometer units\n"
                        "\n"
                        "Applies to boxes only. Changes the center of the box "
                        "and throws an exception if the shape is not a box.\n"
                        "Translation from micrometer units to database units "
                        "is done internally.\n"
                        "\n"
                        "This method has been introduced in version 0.25.\n") +
        gsi::method_ext(
            ...
            "\n"
            "Applies to boxes only. Returns the lower left point of the box "
            "and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_dp1", &box_dp1,
            "@brief Returns the lower left point of the box as a \\DPoint "
            "object in micrometer units\n"
            "\n"
            "Applies to boxes only. Returns the lower left point of the box "
            "and throws an exception if the shape is not a box.\n"
            ...
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext(
            "box_p1=", &set_box_p1, gsi::arg("p"),
            "@brief Sets the lower left point of the box\n"
            "\n"
            "Applies to boxes only. Changes the lower left point of the box "
            "and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_p1=|box_dp1=", &set_box_dp1, gsi::arg("p"),
            "@brief Sets the lower left corner of the box with the point being "
            "given in micrometer units\n"
            "\n"
            "Applies to boxes only. Changes the lower left point of the box "
            "and throws an exception if the shape is not a box.\n"
            "Translation from micrometer units to database units is done "
            "internally.\n"
            "\n"
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext(
            ...
            "\n"
            "Applies to boxes only. Returns the upper right point of the box "
            "and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_dp2", &box_dp2,
            "@brief Returns the upper right point of the box as a \\DPoint "
            "object in micrometer units\n"
            "\n"
            "Applies to boxes only. Returns the upper right point of the box "
            "and throws an exception if the shape is not a box.\n"
            ...
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext(
            "box_p2=", &set_box_p2, gsi::arg("p"),
            "@brief Sets the upper right point of the box\n"
            "\n"
            "Applies to boxes only. Changes the upper right point of the box "
            "and throws an exception if the shape is not a box.\n"
            "\n"
            "This method has been introduced in version 0.23.\n") +
        gsi::method_ext(
            "box_p2=|box_dp2=", &set_box_dp2, gsi::arg("p"),
            "@brief Sets the upper right corner of the box with the point "
            "being given in micrometer units\n"
            "\n"
            "Applies to boxes only. Changes the upper right point of the box "
            "and throws an exception if the shape is not a box.\n"
        ...
        gsi::method_ext("type", &object_type,
                        ...
                        "\n"
                        "The returned values are the t_... constants available "
                        "through the corresponding class members.\n") +
        gsi::method("is_null?", &db::Shape::is_null,
                    "@brief Returns true, if the shape reference is a null "
                    "reference (not referring to a shape)\n") +
        gsi::method("is_polygon?", &db::Shape::is_polygon,
                    "@brief Returns true, if the shape is a polygon\n"
                    "\n"
                    "This method returns true only if the object is a polygon "
                    "or a simple polygon. "
                    "Other objects can convert to polygons, for example paths, "
                    "so it may be possible to use the \\polygon method also "
                    "if is_polygon? does not return true.") +
        gsi::method_ext("polygon", &get_polygon,
                        "@brief Returns the polygon object\n"
                        "\n"
                        "Returns the polygon object that this shape refers to "
                        "or converts the object to a polygon. "
                        "Paths, boxes and simple polygons are converted to "
                        "polygons. For paths this operation renders the "
                        "path's hull contour.\n"
                        "\n"
                        "Starting with version 0.23, this method returns nil, "
                        "if the shape does not represent a geometrical "
        ...
        gsi::method_ext(
            ...
            "\n"
            "Returns the polygon object that this shape refers to or converts "
            "the object to a polygon. "
            "The method returns the same object than \\polygon, but translates "
            "it to micrometer units internally.\n"
            "\n"
            "This method has been introduced in version 0.25.\n") +
        gsi::method("is_simple_polygon?", &db::Shape::is_simple_polygon,
                    "@brief Returns true, if the shape is a simple polygon\n"
                    "\n"
                    "This method returns true only if the object is a simple "
                    "polygon. The simple polygon identity is "
                    "contained in the polygon identity, so usually it is "
                    "sufficient to use \\is_polygon? and "
                    "\\polygon instead of specifically handle simply polygons. "
                    "This method is provided only for "
                    "specific optimisation purposes.") +
        gsi::method_ext(
            "simple_polygon", &get_simple_polygon,
            "@brief Returns the simple polygon object\n"
            "\n"
            "Returns the simple polygon object that this shape refers to or "
            "converts the object to a simple polygon. "
            "Paths, boxes and polygons are converted to simple polygons. "
            "Polygons with holes will have their holes removed but introducing "
            "cut lines that connect the hole contours with the outer contour. "
            "\n"
            "Starting with version 0.23, this method returns nil, if the shape "
            "does not represent a geometrical "
            "primitive that can be converted to a simple polygon.\n") +
        gsi::method_ext(
            "dsimple_polygon", &get_dsimple_polygon,
            "@brief Returns the simple polygon object in micrometer units\n"
            "\n"
            "Returns the simple polygon object that this shape refers to or "
            "converts the object to a simple polygon. "
            "The method returns the same object than \\simple_polygon, but "
            "translates it to micrometer units internally.\n"
            "\n"
            "This method has been introduced in version 0.25.\n") +
        gsi::method_ext(
            "simple_polygon=", &set_shape<db::SimplePolygon>,
            gsi::arg("polygon"),
            "@brief Replaces the shape by the given simple polygon object\n"
            "This method replaces the shape by the given simple polygon "
            "object. This method can only be called "
            "for editable layouts. It does not change the user properties of "
            "the shape.\n"
            "Calling this method will invalidate any iterators. It should not "
            ...
            "This method has been introduced in version 0.22.") +
        gsi::method_ext(
            "simple_polygon=|dsimple_polygon=", &set_dshape<db::DSimplePolygon>,
            gsi::arg("polygon"),
            "@brief Replaces the shape by the given simple polygon (in "
            "micrometer units)\n"
            "This method replaces the shape by the given text, like "
            "\\simple_polygon= with a \\SimplePolygon argument does. "
            "This version translates the polygon from micrometer units to "
            "database units internally.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        ...
        gsi::method_ext("path_width", &path_width,
                        ...
                        "\n"
                        "Applies to paths only. Will throw an exception if the "
        ...
        gsi::method_ext("round_path=", &set_round_path, gsi::arg("r"),
                        "@brief The path will be a round-ended path if this "
                        "property is set to true\n"
                        "\n"
                        "Applies to paths only. Will throw an exception if the "
                        "object is not a path.\n"
                        "Please note that the extensions will apply as well. "
                        "To get a path with circular ends, set the begin and "
        ...
        gsi::method_ext("path_dbgnext", &path_dbgnext,
                        "@brief Gets the path's starting vertex extension in "
                        "micrometer units\n"
                        "\n"
                        "Applies to paths only. Will throw an exception if the "
                        "object is not a path.\n"
        ...
        gsi::method_ext("path_endext", &path_endext,
                        ...
                        "object is not a path.\n") +
        gsi::method_ext(
            "path_dendext", &path_dendext,
            "@brief Gets the path's end vertex extension in micrometer units\n"
            "\n"
            "Applies to paths only. Will throw an exception if the object is "
            "not a path.\n"
            "\n"
            "This method has been introduced in version 0.25.") +
        ...
        gsi::method_ext("path_dbgnext=", &set_path_dbgnext, gsi::arg("e"),
                        "@brief Sets the path's starting vertex extension in "
                        "micrometer units\n"
                        "Applies to paths only. Will throw an exception if the "
                        "object is not a path.\n"
                        "\n"
        ...
        gsi::method_ext("path_endext=", &set_path_endext, gsi::arg("e"),
                        ...
                        "This method has been introduced in version 0.23.") +
        gsi::method_ext(
            "path_dendext=", &set_path_dendext, gsi::arg("e"),
        ...
        gsi::method_ext("path_length", &path_length,
                        ...
                        "Applies to paths only. Will throw an exception if the "
                        "object is not a path.\n"
                        "This method returns the length of the spine plus "
                        "extensions if present.\n"
                        "\n"
                        "This method has been added in version 0.23.\n") +
        gsi::method_ext(
            "path_dlength", &path_dlength,
            "@brief Returns the length of the path in micrometer units\n"
            "\n"
            "Applies to paths only. Will throw an exception if the object is "
            "not a path.\n"
            "This method returns the length of the spine plus extensions if "
            "present.\n"
        ...
        gsi::method_ext("path", &get_path,
                        ...
                        "\n"
                        "Starting with version 0.23, this method returns nil, "
                        "if the shape does not represent a path.") +
        gsi::method_ext("dpath", &get_dpath,
                        "@brief Returns the path object as a \\DPath object in "
                        "micrometer units\n"
                        "See \\path for a description of this method. This "
                        "method returns the path after translation to "
                        "micrometer units.\n"
                        "\n"
                        "This method has been added in version 0.25.\n") +
        ...
        gsi::method_ext("edge", &get_edge,
                        ...
                        "\n"
                        "Starting with version 0.23, this method returns nil, "
                        "if the shape does not represent an edge.") +
        gsi::method_ext("dedge", &get_dedge,
                        "@brief Returns the edge object as a \\DEdge object in "
                        "micrometer units\n"
                        "See \\edge for a description of this method. This "
                        "method returns the edge after translation to "
                        "micrometer units.\n"
                        "\n"
                        "This method has been added in version 0.25.\n") +
        ...
        gsi::method_ext("edge_pair", &get_edge_pair,
                        ...
                        "This method has been introduced in version 0.26.") +
        gsi::method_ext(
            "dedge_pair", &get_dedge_pair,
            "@brief Returns the edge pair object as a \\DEdgePair object in "
            "micrometer units\n"
            "See \\edge_pair for a description of this method. This method "
            "returns the edge pair after translation to "
            "micrometer units.\n"
            "\n"
        ...
        gsi::method_ext("dpoint", &get_dpoint,
                        "@brief Returns the point object as a \\DPoint object "
                        "in micrometer units\n"
                        "See \\point for a description of this method. This "
                        "method returns the point after translation to "
                        "micrometer units.\n"
                        "\n"
                        "This method has been introduced in version 0.28.\n") +
        ...
        gsi::method_ext("text", &get_text,
                        ...
                        "\n"
                        "Starting with version 0.23, this method returns nil, "
                        "if the shape does not represent a text.") +
        gsi::method_ext("dtext", &get_dtext,
                        "@brief Returns the path object as a \\DText object in "
                        "micrometer units\n"
                        "See \\text for a description of this method. This "
                        "method returns the text after translation to "
                        "micrometer units.\n"
                        "\n"
                        "This method has been added in version 0.25.\n") +
        gsi::method_ext("text_string", &text_string,
                        ...
                        "\n"
                        "Applies to texts only. Will throw an exception if the "
        ...
        gsi::method_ext(
            ...
            "\n"
            "Applies to texts only. Will throw an exception if the object is "
            "not a text.\n") +
        gsi::method_ext(
            "text_rot=", &set_text_rot, gsi::arg("o"),
        ...
        gsi::method_ext("text_trans=", &set_text_trans, gsi::arg("trans"),
                        ...
                        "This method has been introduced in version 0.23.") +
        gsi::method_ext(
            "text_trans=|text_dtrans=", &set_text_dtrans, gsi::arg("trans"),
        ...
        gsi::method_ext("text_halign", &text_halign,
                        ...
                        "Applies to texts only. Will throw an exception if the "
                        "object is not a text.\n"
                        "The return value is 0 for left alignment, 1 for "
                        "center alignment and 2 to right alignment.\n"
                        "\n"
                        "This method has been introduced in version 0.22.\n") +
        gsi::method_ext(
            "text_halign=", &set_text_halign, gsi::arg("a"),
        ...
        gsi::method_ext("text_valign", &text_valign,
                        ...
                        "Applies to texts only. Will throw an exception if the "
                        "object is not a text.\n"
                        "The return value is 0 for top alignment, 1 for center "
                        "alignment and 2 to bottom alignment.\n"
                        "\n"
                        "This method has been introduced in version 0.22.\n") +
        gsi::method_ext(
            "text_valign=", &set_text_valign, gsi::arg("a"),
        ...
        gsi::method_ext("is_valid?", &shape_is_valid,
                        ...
                        "\n"
                        "After the shape is deleted, the shape object is no "
                        "longer valid and this method returns false.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method_ext("delete", &delete_shape,
                        "@brief Deletes the shape\n"
                        "\n"
                        "After the shape is deleted, the shape object is "
                        "emptied and points to nothing.\n"
                        "\n"
                        "This method has been introduced in version 0.23.\n") +
        gsi::method(
            "array_trans", &db::Shape::array_trans,
            "@brief Gets the array instance member transformation\n"
            "\n"
            "This attribute is valid only if \\is_array_member? is true.\n"
            "The transformation returned describes the relative transformation "
            "of the \n"
            "array member addressed.\n") +
        gsi::method_ext(
            "array_dtrans", &array_dtrans,
            "@brief Gets the array instance member transformation in "
            "micrometer units\n"
            "\n"
            "This attribute is valid only if \\is_array_member? is true.\n"
            "The transformation returned describes the relative transformation "
            "of the \n"
            "array member addressed. The displacement is given in micrometer "
            "units.\n"
            "\n"
            "This method has been added in version 0.25.\n") +
        ...
        gsi::method("perimeter", &db::Shape::perimeter,
                    ...
                    "\n"
                    "This method will return an approximation of the perimeter "
                    "for paths.\n"
                    "\n"
                    "This method has been added in version 0.23.\n") +
        gsi::method_ext(
            ...
            "\n"
            "This method will return an approximation of the perimeter for "
            "paths.\n"
            "\n"
            "This method has been added in version 0.25.\n") +
        gsi::method_ext("layer_info", &shape_layer,
                        "@brief Returns the \\LayerInfo object of the layer "
                        "the shape is on\n"
                        "If the shape does not reside inside a cell, an empty "
                        "layer is returned.\n"
                        "\n"
                        "This method has been added in version 0.23.\n") +
        gsi::method_ext(
            "layer_info=", &set_shape_layer, gsi::arg("layer_info"),
            "@brief Moves the shape to a layer given by a \\LayerInfo object\n"
            "If no layer with the given properties exists, an exception is "
            "thrown.\n"
            "\n"
            "This method has been added in version 0.23.\n") +
        gsi::method_ext(
            "layer", &shape_layer_index,
        ...
        gsi::method_ext("layer=", &set_shape_layer_index,
                        gsi::arg("layer_index"),
                        "@brief Moves the shape to a layer given by the layer "
                        "index object\n"
                        "\n"
                        "This method has been added in version 0.23.\n") +
        ...
        gsi::method("==", &db::Shape::operator==, gsi::arg("other"),
                    ...
                    "\n"
                    "Equality of shapes is not specified by the identity of "
                    "the objects but by the\n"
                    "identity of the pointers - both shapes must refer to the "
                    "same object.\n") +
        gsi::method(
            "to_s",
            &db::Shape::to_string,
        ...
        gsi::method("TPolygonPtrArray|#t_polygon_ptr_array",
                    &t_polygonPtrArray) +
        gsi::method("TPolygonPtrArrayMember|#t_polygon_ptr_array_member",
                    &t_polygonPtrArrayMember) +
        gsi::method("TSimplePolygon|#t_simple_polygon", &t_simplePolygon) +
        gsi::method("TSimplePolygonRef|#t_simple_polygon_ref",
                    &t_simplePolygonRef) +
        gsi::method("TSimplePolygonPtrArray|#t_simple_polygon_ptr_array",
                    &t_simplePolygonPtrArray) +
        gsi::method(
            "TSimplePolygonPtrArrayMember|#t_simple_polygon_ptr_array_member",
            &t_simplePolygonPtrArrayMember) +
        ...
        gsi::method("TPathPtrArrayMember|#t_path_ptr_array_member",
                    &t_pathPtrArrayMember) +
        ...
        gsi::method("TShortBoxArrayMember|#t_short_box_array_member",
                    &t_shortBoxArrayMember) +
        ...
        gsi::method("TTextPtrArrayMember|#t_text_ptr_array_member",
                    &t_textPtrArrayMember) +
    ...
    "No copy of the shape is created: if the shape proxy is copied the copy "
    "still\n"
    "points to the original shape. If the original shape is modified or "
    "deleted,\n"
    ...
    "stores the actual shape objects and uses Shape references as pointers "
    "inside the\n"
    "actual data storage. Shape references are used in various places, i.e. "
    "when removing or\n"
