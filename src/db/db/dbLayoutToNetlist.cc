...
//  Note: the iterator provides the hierarchical selection (enabling/disabling
//  cells etc.)

LayoutToNetlist::LayoutToNetlist(const db::RecursiveShapeIterator &iter)
    : m_iter(iter), m_layout_index(0), m_netlist_extracted(false),
      m_is_flat(false), m_device_scaling(1.0),
      m_include_floating_subcircuits(false) {
  //  check the iterator
  if (iter.has_complex_region() || iter.region() != db::Box::world()) {
    throw tl::Exception(tl::to_string(
        tr("The netlist extractor cannot work on clipped layouts")));
  }
...
LayoutToNetlist::LayoutToNetlist(db::DeepShapeStore *dss,
    ...
    : mp_dss(dss), m_layout_index(layout_index), m_netlist_extracted(false),
      m_is_flat(false), m_device_scaling(1.0),
      m_include_floating_subcircuits(false) {
  if (dss->is_valid_layout_index(m_layout_index)) {
    m_iter = db::RecursiveShapeIterator(dss->layout(m_layout_index),
                                        dss->initial_cell(m_layout_index),
...
LayoutToNetlist::LayoutToNetlist(const std::string &topcell_name, double dbu)
    : m_iter(), m_netlist_extracted(false), m_is_flat(true),
      m_device_scaling(1.0), m_include_floating_subcircuits(false) {
...
LayoutToNetlist::LayoutToNetlist()
    : m_iter(), mp_internal_dss(new db::DeepShapeStore()),
      mp_dss(mp_internal_dss.get()), m_layout_index(0),
...
void LayoutToNetlist::set_threads(int n) { dss().set_threads(n); }

int LayoutToNetlist::threads() const { return dss().threads(); }

void LayoutToNetlist::set_area_ratio(double ar) {
  dss().set_max_area_ratio(ar);
}

double LayoutToNetlist::area_ratio() const { return dss().max_area_ratio(); }
...
void LayoutToNetlist::set_device_scaling(double s) { m_device_scaling = s; }

double LayoutToNetlist::device_scaling() const { return m_device_scaling; }
...
db::Texts *LayoutToNetlist::make_text_layer(unsigned int layer_index,
                                            const std::string &n) {
  db::RecursiveShapeIterator si(m_iter);
  si.set_layer(layer_index);
...
db::Region *LayoutToNetlist::make_polygon_layer(unsigned int layer_index,
  ...
  db::RecursiveShapeIterator si(m_iter);
  si.set_layer(layer_index);
  si.shape_flags(db::ShapeIterator::Paths | db::ShapeIterator::Polygons |
                 db::ShapeIterator::Boxes);
...
void LayoutToNetlist::link_nets(const db::Net *net, const db::Net *with) {
  if (!net->circuit() || net->circuit() != with->circuit() ||
      !internal_layout() ||
  ...
  connected_clusters<db::NetShape> &clusters =
      m_net_clusters.clusters_per_cell(net->circuit()->cell_index());
  clusters.join_cluster_with(net->cluster_id(), with->cluster_id());
}

size_t LayoutToNetlist::link_net_to_parent_circuit(const Net *subcircuit_net,
                                                   Circuit *parent_circuit,
                                                   const DCplxTrans &dtrans) {
  ...
  connected_clusters<db::NetShape> &parent_net_clusters =
      m_net_clusters.clusters_per_cell(parent_circuit->cell_index());

  size_t id = parent_net_clusters.insert_dummy();

  parent_net_clusters.add_connection(
      id,
      db::ClusterInstance(subcircuit_net->cluster_id(),
                          subcircuit_net->circuit()->cell_index(), trans, 0));
  return id;
}
...
void LayoutToNetlist::extract_devices(
    db::NetlistDeviceExtractor &extractor,
    const std::map<std::string, db::ShapeCollection *> &layers) {
  if (m_netlist_extracted) {
    throw tl::Exception(
        tl::to_string(tr("The netlist has already been extracted")));
  }
  ensure_netlist();
...
void LayoutToNetlist::connect_impl(const db::ShapeCollection &a,
                                   const db::ShapeCollection &b) {
  reset_extracted();
...
size_t LayoutToNetlist::connect_global_impl(const db::ShapeCollection &l,
                                            const std::string &gn) {
  reset_extracted();
...
void LayoutToNetlist::join_net_names(const tl::GlobPattern &cell,
                                     const tl::GlobPattern &gp) {
...
void LayoutToNetlist::join_nets(const tl::GlobPattern &cell,
                                const std::set<std::string> &gp) {
...
void LayoutToNetlist::extract_netlist() {
  ...
  for (std::list<std::pair<tl::GlobPattern, tl::GlobPattern>>::const_iterator
           j = m_joined_net_names_per_cell.begin();
       j != m_joined_net_names_per_cell.end(); ++j) {
    if (j->first.is_const()) {
      jp_per_cell[j->first.pattern()].push_back(j->second);
    } else {
      for (db::Layout::const_iterator c = layout.begin(); c != layout.end();
           ++c) {
  ...
  for (std::map<std::string, std::list<tl::GlobPattern>>::const_iterator i =
           jp_per_cell.begin();
       i != jp_per_cell.end(); ++i) {
  ...
  for (std::list<std::pair<tl::GlobPattern, std::set<std::string>>>::
           const_iterator j = m_joined_nets_per_cell.begin();
       j != m_joined_nets_per_cell.end(); ++j) {
    if (j->first.is_const()) {
  ...
  for (std::map<std::string, std::list<std::set<std::string>>>::const_iterator
           i = jn_per_cell.begin();
       i != jn_per_cell.end(); ++i) {
...
void LayoutToNetlist::mem_stat(MemStatistics *stat,
  ...
  db::mem_stat(stat, purpose, cat, m_joined_net_names_per_cell, true,
               (void *)this);
  ...
  m_net_clusters.mem_stat(stat, MemStatistics::LayoutToNetlist, cat, true,
                          (void *)this);
  if (mp_netlist.get()) {
    db::mem_stat(stat, MemStatistics::Netlist, cat, *mp_netlist, false,
                 (void *)this);
  }
}

void LayoutToNetlist::set_netlist_extracted() { m_netlist_extracted = true; }
...
void LayoutToNetlist::ensure_layout() const {
  if (!dss().is_valid_layout_index(m_layout_index)) {
    ...
    non_const_this->dss().make_layout(m_layout_index,
                                      db::RecursiveShapeIterator());

    //  the dummy layer acts as a reference holder for the layout
    unsigned int dummy_layer_index =
        non_const_this->dss().layout(m_layout_index).insert_layer();
    non_const_this->m_dummy_layer = db::DeepLayer(
        &non_const_this->dss(), m_layout_index, dummy_layer_index);
  }
}
...
std::string LayoutToNetlist::name(unsigned int l) const {
  std::map<unsigned int, std::string>::const_iterator n =
      m_name_of_layer.find(l);
...
db::Region *LayoutToNetlist::layer_by_name(const std::string &name) {
  std::map<std::string, db::DeepLayer>::const_iterator l =
      m_named_regions.find(name);
...
db::Region *LayoutToNetlist::layer_by_original(
    const ShapeCollectionDelegateBase *original_delegate) {
  auto n = m_region_by_original.find(tl::id_of(original_delegate));
  if (n == m_region_by_original.end()) {

    DeepShapeCollectionDelegateBase *dl =
        const_cast<ShapeCollectionDelegateBase *>(original_delegate)->deep();
...
static db::DeepLayer dss_create_from_flat(db::DeepShapeStore &dss,
                                          const db::ShapeCollection &coll) {
...
std::string LayoutToNetlist::name(const ShapeCollection &coll) const {
  std::map<unsigned int, std::string>::const_iterator n =
      m_name_of_layer.find(layer_of(coll));
  if (n != m_name_of_layer.end()) {
    return n->second;
  } else {
    return std::string();
...
void LayoutToNetlist::register_layer(const ShapeCollection &collection,
                                     const std::string &n_in) {
  if (m_region_by_original.find(tl::id_of(collection.get_delegate())) !=
      m_region_by_original.end()) {
  ...
  if (!n_in.empty() && m_named_regions.find(n_in) != m_named_regions.end()) {
    throw tl::Exception(tl::to_string(tr("Layer name is already used: ")) +
                        n_in);
  }

  //  Caution: this make create names which clash with future explicit names.
  //  Hopefully, the generated names are unique enough.
  ...
  if (m_is_flat) {
    ...
    db::DeepShapeCollectionDelegateBase *delegate =
        collection.get_delegate()->deep();
    if (!delegate) {
...
db::DeepLayer
LayoutToNetlist::deep_layer_of(const db::ShapeCollection &coll) const {
  ...
  if (!dr) {
    ...
    if (lff.first) {
      ...
      throw(tl::Exception(tl::to_string(
          tr("Non-hierarchical layers cannot be used in netlist extraction"))));
    }
...
bool LayoutToNetlist::is_persisted_impl(const db::ShapeCollection &coll) const {
  if (coll.get_delegate()->deep() &&
      coll.get_delegate()->deep()->deep_layer().store() == mp_dss.get()) {
    //  implicitly persisted because the collection is inside our DSS
    return true;
  } else {
    //  explicitly persisted through "register"
    return m_region_by_original.find(tl::id_of(coll.get_delegate())) !=
           m_region_by_original.end();
  }
}

db::CellMapping LayoutToNetlist::make_cell_mapping_into(
    db::Layout &layout, db::Cell &cell,
    const std::vector<const db::Net *> *nets, bool with_device_cells) {
  std::set<db::cell_index_type> device_cells;
  if (!with_device_cells && mp_netlist.get()) {
    for (db::Netlist::device_abstract_iterator i =
             mp_netlist->begin_device_abstracts();
         i != mp_netlist->end_device_abstracts(); ++i) {
  ...
  if (nets) {
    //  Compute the "included cell" list for cell_mapping_to_original: these are
    //  all cells which are required to represent the net hierarchically.
    for (std::vector<const db::Net *>::const_iterator n = nets->begin();
         n != nets->end(); ++n) {
      const db::Net *net = *n;
  ...
  return dss().cell_mapping_to_original(m_layout_index, &layout,
                                        cell.cell_index(), &device_cells,
                                        nets ? &net_cells : 0);
}

db::CellMapping
LayoutToNetlist::cell_mapping_into(db::Layout &layout, db::Cell &cell,
                                   const std::vector<const db::Net *> &nets,
                                   bool with_device_cells) {
  return make_cell_mapping_into(layout, cell, &nets, with_device_cells);
}

db::CellMapping LayoutToNetlist::cell_mapping_into(db::Layout &layout,
                                                   db::Cell &cell,
                                                   bool with_device_cells) {
...
db::CellMapping
LayoutToNetlist::const_cell_mapping_into(const db::Layout &layout,
                                         const db::Cell &cell) {
  db::CellMapping cm;
  if (layout.cells() == 1) {
    cm.create_single_mapping(layout, cell.cell_index(), *internal_layout(),
                             internal_top_cell()->cell_index());
  } else {
    cm.create_from_geometry(layout, cell.cell_index(), *internal_layout(),
                            internal_top_cell()->cell_index());
  }
  return cm;
}
...
LayoutToNetlist::create_layermap(db::Layout &target_layout, int ln) const {
  ...
  for (db::Connectivity::layer_iterator layer = conn.begin_layers();
       layer != conn.end_layers(); ++layer) {
    layers_to_copy.insert(*layer);
  }

  for (std::set<unsigned int>::const_iterator l = layers_to_copy.begin();
       l != layers_to_copy.end(); ++l) {
    ...
    lm.insert(std::make_pair(
        tl, const_cast<LayoutToNetlist *>(this)->layer_by_index(*l)));
  }

  return lm;
}

db::Netlist *LayoutToNetlist::netlist() const { return mp_netlist.get(); }
...
template <class Tr>
static bool deliver_shape(const db::NetShape &, StopOnFirst, const Tr &,
                          db::properties_id_type) {
  return false;
}

template <class Tr>
static bool deliver_shape(const db::NetShape &s, db::Region &region,
                          const Tr &tr, db::properties_id_type /*propid*/) {
...
template <class Tr>
static bool deliver_shape(const db::NetShape &s, db::Shapes &shapes,
                          const Tr &tr, db::properties_id_type propid) {
  if (s.type() == db::NetShape::Polygon) {
    ...
    if (layout) {
      //  NOTE: by maintaining the PolygonRefs we can directly use the output of
      //  "build_nets" as input for a hierarchical processor.
      db::PolygonRef polygon_ref(
          pr.obj().transformed(pr.trans()).transformed(tr),
          layout->shape_repository());
      if (propid) {
      ...
      if (propid) {
        shapes.insert(db::BoxWithProperties(
            pr.obj().box().transformed(pr.trans()).transformed(tr), propid));
      } else {
    ...
    db::Layout *layout = shapes.layout();
    if (layout) {
      db::TextRef text_ref(pr.obj().transformed(pr.trans()).transformed(tr),
                           layout->shape_repository());
      if (propid) {
...
static bool deliver_shapes_of_net_recursive(
    const db::Netlist * /*nl*/, const db::hier_clusters<Shape> &clusters,
    db::cell_index_type ci, size_t cid, unsigned int layer_id,
    const db::ICplxTrans &tr, To &to, db::properties_id_type propid) {
  //  deliver the net shapes
  for (db::recursive_cluster_shape_iterator<Shape> rci(clusters, layer_id, ci,
                                                       cid);
       !rci.at_end(); ++rci) {
...
template <class To, class Shape>
static bool deliver_shapes_of_net(bool recursive, const db::Netlist *nl,
                                  const db::hier_clusters<Shape> &clusters,
                                  db::cell_index_type ci, size_t cid,
                                  const std::map<unsigned int, To *> &lmap,
                                  const db::ICplxTrans &tr,
                                  db::properties_id_type propid) {
  //  shortcut
  if (lmap.empty()) {
    return true;
  }
  ...
  for (typename std::map<unsigned int, To *>::const_iterator l = lmap.begin();
       l != lmap.end(); ++l) {
    for (typename db::local_cluster<Shape>::shape_iterator s =
             lc.begin(l->first);
         !s.at_end(); ++s) {
  ...
  const typename db::connected_clusters<Shape>::connections_type &conn =
      cc.connections_for_cluster(cid);
  for (typename db::connected_clusters<Shape>::connections_type::const_iterator
           c = conn.begin();
       c != conn.end();) {

    db::cell_index_type cci = c->inst_cell_index();
    if (!recursive && (!nl || nl->circuit_by_cell_index(cci) ||
                       nl->device_abstract_by_cell_index(cci))) {
      //  skip this cell in non-recursive mode (and all following instances of
      //  the same cell too)
      typename db::connected_clusters<Shape>::connections_type::const_iterator
          cc = c;
      while (++cc != conn.end()) {
    ...
    if (!deliver_shapes_of_net(recursive, nl, clusters, cci, c->id(), lmap,
                               tr * c->inst_trans(), propid)) {
      return false;
    }
    ++c;
  }
...
void LayoutToNetlist::shapes_of_net(const db::Net &net,
                                    const db::Region &of_layer, bool recursive,
                                    db::Shapes &to,
                                    db::properties_id_type propid,
                                    const ICplxTrans &trans) const {
  ...
  deliver_shapes_of_net(recursive, mp_netlist.get(), m_net_clusters,
                        circuit->cell_index(), net.cluster_id(), lmap, trans,
                        propid);
}

db::Region *LayoutToNetlist::shapes_of_net(const db::Net &net,
                                           const db::Region &of_layer,
                                           bool recursive,
...
void LayoutToNetlist::build_net(
    const db::Net &net, db::Layout &target, db::Cell &target_cell,
    const std::map<unsigned int, const db::Region *> &lmap,
    NetPropertyMode net_prop_mode, const tl::Variant &netname_prop,
    BuildNetHierarchyMode hier_mode, const char *cell_name_prefix,
...
void LayoutToNetlist::build_all_nets(
    const db::CellMapping &cmap, db::Layout &target,
    const std::map<unsigned int, const db::Region *> &lmap,
    const char *net_cell_name_prefix, NetPropertyMode net_prop_mode,
    const tl::Variant &netname_prop, BuildNetHierarchyMode hier_mode,
    const char *circuit_cell_name_prefix,
    const char *device_cell_name_prefix) const {
...
void LayoutToNetlist::build_nets(
    const std::vector<const db::Net *> *nets, const db::CellMapping &cmap,
...
db::Net *LayoutToNetlist::probe_net(const db::Region &of_region,
                                    const db::DPoint &point,
                                    std::vector<db::SubCircuit *> *sc_path_out,
                                    db::Circuit *initial_circuit) {
  return probe_net(of_region,
                   db::CplxTrans(internal_layout()->dbu()).inverted() * point,
                   sc_path_out, initial_circuit);
}

size_t
LayoutToNetlist::search_net(const db::ICplxTrans &trans, const db::Cell *cell,
                            const db::local_cluster<db::NetShape> &test_cluster,
                            std::vector<db::InstElement> &rev_inst_path) {
  db::Box local_box = trans * test_cluster.bbox();

  const db::local_clusters<db::NetShape> &lcc =
      net_clusters().clusters_per_cell(cell->cell_index());
  for (db::local_clusters<db::NetShape>::touching_iterator i =
           lcc.begin_touching(local_box);
       !i.at_end(); ++i) {
  ...
  for (db::Cell::touching_iterator i = cell->begin_touching(local_box);
       !i.at_end(); ++i) {

    for (db::CellInstArray::iterator ia =
             i->begin_touching(local_box, internal_layout());
         !ia.at_end(); ++ia) {

      db::ICplxTrans trans_inst = i->complex_trans(*ia);
      db::ICplxTrans t = trans_inst.inverted() * trans;
      size_t cluster_id =
          search_net(t, &internal_layout()->cell(i->cell_index()), test_cluster,
                     rev_inst_path);
      if (cluster_id > 0) {
...
db::Net *LayoutToNetlist::probe_net(const db::Region &of_region,
                                    const db::Point &point,
                                    std::vector<db::SubCircuit *> *sc_path_out,
                                    db::Circuit *initial_circuit) {
  if (!m_netlist_extracted) {
    throw tl::Exception(
        tl::to_string(tr("The netlist has not been extracted yet")));
  }
  ...
  if (initial_circuit &&
      internal_layout()->is_valid_cell_index(initial_circuit->cell_index())) {
  ...
  size_t cluster_id =
      search_net(db::ICplxTrans(), top_cell, test_cluster, inst_path);
  if (cluster_id > 0) {
    ...
    for (std::vector<db::InstElement>::const_iterator i = inst_path.begin();
         i != inst_path.end(); ++i) {
    ...
    while (true) {
      ...
      cluster_id = m_net_clusters.clusters_per_cell(cell_indexes.back())
    ...
    //  follow the path up in the net hierarchy using the transformation and the
    //  upper cell index as the guide line
    while (circuit && !inst_path.empty()) {
      ...
      db::DCplxTrans dtrans =
          dbu_trans * inst_path.back().complex_trans() * dbu_trans_inv;

      //  try to find a parent circuit which connects to this net
      db::Circuit *upper_circuit = 0;
      db::SubCircuit *subcircuit = 0;
      db::Net *upper_net = 0;
      for (db::Circuit::refs_iterator r = circuit->begin_refs();
           r != circuit->end_refs() && !upper_circuit; ++r) {
        if (r->trans().equal(dtrans) && r->circuit() &&
            r->circuit()->cell_index() == cell_indexes.back()) {
...
class PolygonAreaAndPerimeterCollector : public db::PolygonSink {
public:
  ...
  PolygonAreaAndPerimeterCollector() : m_area(0), m_perimeter(0) {}

  area_type area() const { return m_area; }

  perimeter_type perimeter() const { return m_perimeter; }
...
static void compute_area_and_perimeter_of_net_shapes(
    const db::hier_clusters<db::NetShape> &clusters, db::cell_index_type ci,
    size_t cid, unsigned int layer_id, db::Polygon::area_type &area,
    db::Polygon::perimeter_type &perimeter) {
  db::EdgeProcessor ep;

  //  count vertices and reserve space
  size_t n = 0;
  for (db::recursive_cluster_shape_iterator<db::NetShape> rci(
           clusters, layer_id, ci, cid);
       !rci.at_end(); ++rci) {
...
class AntennaShapeGenerator : public PolygonSink {
public:
  AntennaShapeGenerator(db::Layout *layout, db::Shapes &shapes,
...
static db::Point
get_merged_shapes_of_net(const db::hier_clusters<db::NetShape> &clusters,
                         db::cell_index_type ci, size_t cid,
                         unsigned int layer_id, db::Layout *layout,
                         db::Shapes &shapes, db::properties_id_type prop_id) {
  db::Point ref;
  bool any_ref = false;
  db::EdgeProcessor ep;
...
static std::vector<std::pair<std::string, tl::Variant>> create_antenna_values(
    double agate, db::Polygon::area_type agate_int, double gate_area_factor,
    db::Polygon::perimeter_type pgate_int, double gate_perimeter_factor,
    double ametal, db::Polygon::area_type ametal_int, double metal_area_factor,
    db::Polygon::perimeter_type pmetal_int, double metal_perimeter_factor,
    const std::vector<std::pair<const db::Region *, double>> &diodes,
    const std::vector<db::Polygon::area_type> &adiodes_int, double r,
    double ratio, double dbu) {
  std::vector<std::pair<std::string, tl::Variant>> values;

  if (fabs(gate_area_factor - 1.0) <= db::epsilon &&
      fabs(gate_perimeter_factor) <= db::epsilon) {
  ...
  if (fabs(metal_area_factor - 1.0) <= db::epsilon &&
      fabs(metal_perimeter_factor) <= db::epsilon) {
...
db::Region LayoutToNetlist::antenna_check(
    const db::Region &gate, double gate_area_factor,
    double gate_perimeter_factor, const db::Region &metal,
    double metal_area_factor, double metal_perimeter_factor, double ratio,
    const std::vector<std::pair<const db::Region *, double>> &diodes,
    db::Texts *values) {
  ...
  for (db::Layout::bottom_up_const_iterator cid = ly.begin_bottom_up();
       cid != ly.end_bottom_up(); ++cid) {

    const connected_clusters<db::NetShape> &clusters =
        m_net_clusters.clusters_per_cell(*cid);
    if (clusters.empty()) {
      continue;
    }
    ...
    for (connected_clusters<db::NetShape>::all_iterator c =
             clusters.begin_all();
         !c.at_end(); ++c) {
      ...
      for (auto d = diodes.begin(); d != diodes.end() && !skip; ++d) {
        ...
        compute_area_and_perimeter_of_net_shapes(m_net_clusters, *cid, *c,
                                                 layer_of(*d->first),
                                                 adiode_int, pdiode_int);
      ...
      if (!skip) {
        ...
        compute_area_and_perimeter_of_net_shapes(
            m_net_clusters, *cid, *c, layer_of(gate), agate_int, pgate_int);
        ...
        if (agate > dbu * dbu) {
          ...
          if (tl::verbosity() >= 50) {
            std::vector<std::pair<std::string, tl::Variant>> antenna_values =
                create_antenna_values(agate, agate_int, gate_area_factor,
                                      pgate_int, gate_perimeter_factor, ametal,
                                      ametal_int, metal_area_factor, pmetal_int,
                                      metal_perimeter_factor, diodes,
                                      adiodes_int, r, ratio, dbu);
            tl::info << "cell [" << ly.cell_name(*cid) << "]: ";
            for (auto v = antenna_values.begin(); v != antenna_values.end();
                 ++v) {
          ...
          if (ametal / agate > r + db::epsilon) {
            ...
            if (!values) {
              ...
              for (auto v = antenna_values.begin(); v != antenna_values.end();
                ...
                ps.insert(std::make_pair(
                    ly.properties_repository().prop_name_id(v->first),
                    v->second));
              }
            ...
            db::Point ref =
                get_merged_shapes_of_net(m_net_clusters, *cid, *c,
                                         layer_of(metal), &ly, shapes, prop_id);
...
db::LayoutToNetlist *
LayoutToNetlist::create_from_file(const std::string &path) {
...
void LayoutToNetlist::set_generator(const std::string &g) { m_generator = g; }

// -----------------------------------------------------------------------------------------------
//  NetBuilder implementation

NetBuilder::NetBuilder()
    : m_hier_mode(BNH_Flatten), m_has_net_cell_name_prefix(false),
      m_has_cell_name_prefix(false), m_has_device_cell_name_prefix(false) {
  //  .. nothing yet ..
}

NetBuilder::NetBuilder(db::Layout *target, const db::CellMapping &cmap,
                       const db::LayoutToNetlist *source)
    : mp_target(target), m_cmap(cmap),
      mp_source(const_cast<db::LayoutToNetlist *>(source)),
      m_hier_mode(BNH_Flatten), m_has_net_cell_name_prefix(false),
      m_has_cell_name_prefix(false), m_has_device_cell_name_prefix(false) {
  //  .. nothing yet ..
}
...
NetBuilder::NetBuilder(const db::NetBuilder &other) { operator=(other); }

NetBuilder::NetBuilder(db::NetBuilder &&other) { operator=(other); }

NetBuilder &NetBuilder::operator=(const db::NetBuilder &other) {
  if (this != &other) {
...
NetBuilder &NetBuilder::operator=(db::NetBuilder &&other) {
  if (this != &other) {
    mp_target = other.mp_target;
    ...
    std::swap(m_has_device_cell_name_prefix,
              other.m_has_device_cell_name_prefix);
...
void NetBuilder::set_cell_name_prefix(const char *s) {
  ...
  if (has_cell_name_prefix != m_has_cell_name_prefix ||
      cell_name_prefix != m_cell_name_prefix) {
    m_reuse_table.clear();
...
void NetBuilder::set_device_cell_name_prefix(const char *s) {
  ...
  if (has_device_cell_name_prefix != m_has_device_cell_name_prefix ||
      device_cell_name_prefix != m_device_cell_name_prefix) {
    m_reuse_table.clear();
...
void NetBuilder::prepare_build_nets() const {
  ...
  //  Resets the "initialized" flag so existing cells are reused but freshly
  //  filled
...
void NetBuilder::build_net(
    db::Cell &target_cell, const db::Net &net,
    const std::map<unsigned int, const db::Region *> &lmap,
    NetPropertyMode net_prop_mode, const tl::Variant &netname_prop) const {
  prepare_build_nets();

  double mag = mp_source->internal_layout()->dbu() / mp_target->dbu();

  db::properties_id_type netname_propid = make_netname_propid(
      target().properties_repository(), net_prop_mode, netname_prop, net);
  build_net_rec(net, target_cell, lmap, std::string(), netname_propid,
                db::ICplxTrans(mag));
}

void NetBuilder::build_all_nets(
    const std::map<unsigned int, const db::Region *> &lmap,
    NetPropertyMode net_prop_mode, const tl::Variant &netname_prop) const {
  build_nets(0, lmap, net_prop_mode, netname_prop);
}

void NetBuilder::build_nets(
    const std::vector<const Net *> *nets,
    const std::map<unsigned int, const db::Region *> &lmap,
    NetPropertyMode prop_mode, const tl::Variant &netname_prop) const {
  prepare_build_nets();
  ...
  for (db::Netlist::const_circuit_iterator c = netlist->begin_circuits();
    ...
    for (db::Circuit::const_net_iterator n = c->begin_nets();
         n != c->end_nets(); ++n) {

      //  exclude local nets in recursive mode except if they are explicitly
      //  selected
      if (!nets && m_hier_mode != BNH_Disconnected && !is_top_circuit &&
          n->pin_count() > 0) {
        continue;
      }
      ...
      if (!nets || net_set.find(n.operator->()) != net_set.end()) {
        db::properties_id_type netname_propid = make_netname_propid(
            target().properties_repository(), prop_mode, netname_prop, *n);
        build_net_rec(*n, c->cell_index(), lmap, std::string(), netname_propid,
                      db::ICplxTrans());
      }
    }
    ...
    if (m_hier_mode != BNH_Disconnected && !nets) {
      ...
      //  With recursive nets we skip nets in subcircuits which are connected
      //  upwards. This means, nets will get lost if there is no connection to
      //  this pin from the outside. Hence we need to deliver nets from
      //  subcircuits as part of the circuit which calls the subcircuit - but
      //  NOT in a subcircuit cell, because this will just apply to nets from
      //  certain instances. But the net cell name will be formed as
      //  "subcircuit:net"
      //
      //  In explicit selection mode we don't care about this as nets are
      //  explicitly taken or not.

      const db::Circuit &circuit = *c;
      for (db::Circuit::const_subcircuit_iterator sc =
               circuit.begin_subcircuits();
           sc != circuit.end_subcircuits(); ++sc) {

        const db::SubCircuit &subcircuit = *sc;
        for (db::Circuit::const_pin_iterator p =
                 subcircuit.circuit_ref()->begin_pins();
             p != subcircuit.circuit_ref()->end_pins(); ++p) {
          ...
          if (!subcircuit.net_for_pin(p->id())) {
            ...
            if (n) {
              ...
              db::ICplxTrans tr = db::CplxTrans(dbu).inverted() *
                                  subcircuit.trans() * db::CplxTrans(dbu);
              ...
              build_net_rec(*n, c->cell_index(), lmap, net_name_prefix,
                            netname_propid, tr);
            }
          }
        }
...
void NetBuilder::build_net_rec(
    const db::Net &net, db::Cell &target_cell,
    const std::map<unsigned int, const db::Region *> &lmap,
    const std::string &add_net_cell_name_prefix,
    db::properties_id_type netname_propid, const db::ICplxTrans &tr) const {
  const db::Circuit *circuit = net.circuit();
  tl_assert(circuit != 0);

  build_net_rec(circuit->cell_index(), net.cluster_id(), target_cell, lmap,
                &net, add_net_cell_name_prefix, netname_propid, tr);
}

void NetBuilder::build_net_rec(
    db::cell_index_type ci, size_t cid, db::Cell &tc,
    const std::map<unsigned int, const db::Region *> &lmap, const db::Net *net,
    const std::string &add_net_cell_name_prefix,
    db::properties_id_type netname_propid, const db::ICplxTrans &tr) const {
  ...
  if (net && m_has_net_cell_name_prefix) {
    ...
    const db::connected_clusters<db::NetShape> &ccl =
        mp_source->net_clusters().clusters_per_cell(ci);

    bool any_connections =
        m_has_cell_name_prefix && !ccl.connections_for_cluster(cid).empty();
    if (!any_connections) {

      StopOnFirst sof;
      std::map<unsigned int, StopOnFirst *> sof_lmap;
      for (std::map<unsigned int, const db::Region *>::const_iterator l =
               lmap.begin();
           l != lmap.end(); ++l) {
        if (l->second) {
          sof_lmap.insert(
              std::make_pair(mp_source->layer_of(*l->second), &sof));
        }
      }

      bool consider_cell = !deliver_shapes_of_net(
          m_hier_mode == BNH_Flatten, mp_source->netlist(),
          mp_source->net_clusters(), ci, cid, sof_lmap, tr, 0);
      if (!consider_cell) {
    ...
    target_cell = &target().cell(
        target().add_cell((m_net_cell_name_prefix + add_net_cell_name_prefix +
                           net->expanded_name())
                              .c_str()));
    tc.insert(db::CellInstArray(db::CellInst(target_cell->cell_index()),
                                db::Trans()));
  }
  ...
  for (std::map<unsigned int, const db::Region *>::const_iterator l =
       ...
       l != lmap.end(); ++l) {
    if (l->second) {
      target_lmap.insert(std::make_pair(mp_source->layer_of(*l->second),
                                        &target_cell->shapes(l->first)));
    }
  }

  deliver_shapes_of_net(m_hier_mode == BNH_Flatten, mp_source->netlist(),
                        mp_source->net_clusters(), ci, cid, target_lmap, tr,
                        netname_propid);
  ...
  //  NOTE: we propagate the magnification part of tr down, but keep the
  //  rotation/translation part in the instance (we want to avoid magnified
  //  instances)
  db::ICplxTrans tr_wo_mag = tr * db::ICplxTrans(1.0 / tr.mag());
  db::ICplxTrans tr_mag(tr.mag());

  const db::connected_clusters<db::NetShape> &clusters =
      mp_source->net_clusters().clusters_per_cell(ci);
  typedef db::connected_clusters<db::NetShape>::connections_type
      connections_type;
  const connections_type &connections = clusters.connections_for_cluster(cid);
  for (connections_type::const_iterator c = connections.begin();
       c != connections.end(); ++c) {
    ...
    if (cm == m_reuse_table.end()) {
      ...
      if (has_name_prefix) {
        ...
        db::cell_index_type target_ci =
            target().add_cell((name_prefix + cell_name).c_str());
        cm = m_reuse_table
                 .insert(
                     std::make_pair(cmap_key, std::make_pair(target_ci, true)))
                 .first;

        build_net_rec(subci, subcid, target().cell(target_ci), lmap, 0,
                      std::string(), netname_propid, tr_mag);
      ...
      build_net_rec(subci, subcid, target().cell(cm->second.first), lmap, 0,
                    std::string(), netname_propid, tr_mag);
      cm->second.second = true;
    }

    if (cm->second.first != std::numeric_limits<db::cell_index_type>::max()) {
      db::CellInstArray ci(db::CellInst(cm->second.first),
                           tr_wo_mag * c->inst_trans());
...
void NetBuilder::build_net_rec(
    const db::Net &net, db::cell_index_type circuit_cell,
    const std::map<unsigned int, const db::Region *> &lmap,
    const std::string &add_net_cell_name_prefix,
    db::properties_id_type netname_propid, const ICplxTrans &tr) const {
  if (!m_cmap.has_mapping(circuit_cell)) {

    const db::Cell &cc = mp_source->internal_layout()->cell(circuit_cell);

    for (db::Cell::parent_inst_iterator p = cc.begin_parent_insts();
         !p.at_end(); ++p) {
      ...
      for (db::CellInstArray::iterator ia = ci.begin(); !ia.at_end(); ++ia) {
        ...
        build_net_rec(net, p->parent_cell_index(), lmap,
                      add_net_cell_name_prefix, netname_propid, tr_parent);
      }
    }
    ...
    build_net_rec(net, target().cell(target_ci), lmap, add_net_cell_name_prefix,
                  netname_propid, db::ICplxTrans(mag) * tr);
  }
}

db::properties_id_type NetBuilder::make_netname_propid(
    db::PropertiesRepository &pr, NetPropertyMode net_prop_mode,
    const tl::Variant &netname_prop, const db::Net &net,
    const std::string &net_name_prefix) {
  if (net_prop_mode == NPM_NoProperties) {

    return 0;

  } else if (!netname_prop.is_nil() ||
             (net_prop_mode == NPM_AllProperties &&
              net.begin_properties() != net.end_properties())) {

    db::PropertiesRepository::properties_set propset;

    //  add the user properties too (TODO: make this configurable?)
    for (db::Net::property_iterator p = net.begin_properties();
         p != net.end_properties(); ++p) {
    ...
    if (!netname_prop.is_nil()) {
      db::property_names_id_type name_propnameid =
          pr.prop_name_id(netname_prop);
      if (net_prop_mode == NPM_NetQualifiedNameOnly) {
        ...
        propset.insert(std::make_pair(
            name_propnameid, tl::Variant(reinterpret_cast<size_t>(&net))));
      } else {
        propset.insert(
            std::make_pair(name_propnameid,
                           tl::Variant(net_name_prefix + net.expanded_name())));
      }
    }
