...
public:
  ...
  enum edge_connectivity_type {
    ...
    /**
     *  @brief Edges connect if the end point of one edge is the start point of
     * the other edge
     */
  ...
  /**
   *  @brief Creates a connectivity object without connections and the given
   * edge connectivity mode
   */
   ...
   *  It is assumed that all those layers originate from the same deep shape
   * store.
   */
  ...
  bool interacts(const T &a, unsigned int la, const T &b, unsigned int lb,
   ...
   */
  template <class T>
  bool interacts(const T &a, unsigned int la, const T &b,
   ...
   */
  bool interacts(const std::set<unsigned int> &la,
                 const std::set<unsigned int> &lb) const;

  /**
   *  @brief Returns true, if two cells basically (without considering
   ...
   */
  template <class T>
  bool interact(const db::Cell &a, const T &ta, const db::Cell &b,
                const T &tb) const;
 ...
 *
 *  A cluster of shapes is a set of shapes of type T which are connected in
 * terms of a given connectivity. The shapes will still be organized in layers.
 */
template <class T> class DB_PUBLIC_TEMPLATE local_cluster {
public:
   ...
   *
   *  The ID is a unique identifier for the cluster. An ID value of 0 is
   * reserved for "no cluster".
   */
   ...
   *  the test.
   */
  bool interacts(const local_cluster<T> &other, const db::ICplxTrans &trans,
   ...
   *  the test.
   */
  bool interacts(const db::Cell &cell, const db::ICplxTrans &trans,
                 const Connectivity &conn) const;
  ...
  /**
   *  @brief Computes the "area ratio" of the cluster - this is a rough
   * approximation of the area covered The algorithm used assumes no overlap
   * between the polygons of the cluster.
   */
  double area_ratio() const;

  /**
   *  @brief Splits the cluster into multiple other clusters until the desired
   * area ratio is achieved. The result is sent to the output iterator. The
   * return value is the number of clusters produced. If the area ratio of the
   * cluster is below the limit, no splitting happens and 0 is returned.
   */
  template <class Iter> size_t split(double max_area_ratio, Iter &output) const;
  ...
  /**
   *  @brief Gets a value indicating whether the attributes of the clusters are
   * identical
   */
   ...
   */
  attr_iterator begin_attr() const { return m_attrs.begin(); }

  /**
   *  @brief Gets the attribute iterator (end)
   */
  attr_iterator end_attr() const { return m_attrs.end(); }
   ...
   */
  global_nets_iterator end_global_nets() const { return m_global_nets.end(); }

  /**
   *  @brief Gets the global nets set
   */
  const global_nets &get_global_nets() const { return m_global_nets; }
...
private:
  ...
  void set_id(id_type id) { m_id = id; }

  const T &shape(unsigned int l, size_t index) const {
    typename std::map<unsigned int, tree_type>::const_iterator s =
        m_shapes.find(l);
...
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const local_cluster<T> &x, bool no_self,
                     void *parent) {
 ...
 */
template <class T> struct DB_PUBLIC_TEMPLATE local_cluster_box_convert {
  typedef typename local_cluster<T>::box_type box_type;
  typedef typename db::simple_bbox_tag complexity;

  box_type operator()(const local_cluster<T> &c) const { return c.bbox(); }
};
 ...
 */
template <class T> class DB_PUBLIC_TEMPLATE local_clusters {
public:
  typedef typename local_cluster<T>::id_type id_type;
  typedef typename local_cluster<T>::box_type box_type;
  typedef typename local_cluster<T>::attr_id attr_id;
  typedef db::box_tree<box_type, local_cluster<T>, local_cluster_box_convert<T>>
      tree_type;
  ...
  cluster_by_id(typename local_cluster<T>::id_type id) const;
   ...
   */
  void join_cluster_with(typename local_cluster<T>::id_type id,
   ...
   */
  void
  build_clusters(const db::Cell &cell, const db::Connectivity &conn,
                 const tl::equivalence_clusters<size_t> *attr_equivalence = 0,
                 bool report_progress = false,
                 bool separate_attributes = false);
...
template <class T>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const local_clusters<T> &x, bool no_self,
                     void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
...
public:
  ...
  ClusterInstElement(db::cell_index_type inst_cell_index,
                     const db::ICplxTrans &inst_trans,
                     db::properties_id_type inst_prop_id)
      : m_inst_cell_index(inst_cell_index), m_inst_trans(inst_trans),
        m_inst_prop_id(inst_prop_id) {
    //  .. nothing yet ..
  }

  ClusterInstElement()
      : m_inst_cell_index(std::numeric_limits<db::cell_index_type>::max()),
        m_inst_trans(), m_inst_prop_id(0) {
    //  .. nothing yet ..
  }
   ...
   */
  db::cell_index_type inst_cell_index() const { return m_inst_cell_index; }

  /**
   *  @brief Gets the instance transformation
   */
  const db::ICplxTrans &inst_trans() const { return m_inst_trans; }

  /**
   *  @brief Gets the instance properties id
   */
  db::properties_id_type inst_prop_id() const { return m_inst_prop_id; }

  /**
   *  @brief Sets the instance properties id
   */
  void set_inst_prop_id(db::properties_id_type pid) { m_inst_prop_id = pid; }

  /**
   *  @brief Transform with the given transformation
   */
  void transform(const db::ICplxTrans &tr) { m_inst_trans = tr * m_inst_trans; }
  ...
  bool operator==(const ClusterInstElement &other) const {
    return m_inst_cell_index == other.m_inst_cell_index &&
           m_inst_trans.equal(other.m_inst_trans) &&
           m_inst_prop_id == other.m_inst_prop_id;
  }
 ...
 */
class DB_PUBLIC ClusterInstance : public ClusterInstElement {
public:
  ClusterInstance(size_t id, db::cell_index_type inst_cell_index,
                  const db::ICplxTrans &inst_trans,
                  db::properties_id_type inst_prop_id)
      : ClusterInstElement(inst_cell_index, inst_trans, inst_prop_id),
        m_id(id) {
    //  .. nothing yet ..
  }
  ...
  ClusterInstance(size_t id) : ClusterInstElement(), m_id(id) {
    //  .. nothing yet ..
  }

  ClusterInstance() : ClusterInstElement(), m_id(0) {
    //  .. nothing yet ..
  }
...
typedef std::list<std::pair<ClusterInstance, ClusterInstance>>
    cluster_instance_pair_list_type;

inline bool equal_array_delegates(const db::ArrayBase *a,
                                  const db::ArrayBase *b) {
  if ((a == 0) != (b == 0)) {
    return false;
  } else if (a) {
    return !db::array_base_ptr_cmp_f()(a, b) &&
           !db::array_base_ptr_cmp_f()(b, a);
  } else {
    return true;
  }
}

inline bool less_array_delegates(const db::ArrayBase *a,
                                 const db::ArrayBase *b) {
  if ((a == 0) != (b == 0)) {
    return (a == 0) < (b == 0);
  } else if (a) {
...
/**
 *  @brief A helper struct to describe a pair of cell instances with a specific
 * relative transformation
 */
struct DB_PUBLIC InstanceToInstanceInteraction {
  InstanceToInstanceInteraction(const db::ArrayBase *_array1,
                                const db::ArrayBase *_array2,
                                const db::ICplxTrans &_tn,
                                const db::ICplxTrans &_t21)
  ...
  InstanceToInstanceInteraction() : array1(0), array2(0) {
    //  .. nothing yet ..
  }

  InstanceToInstanceInteraction(const InstanceToInstanceInteraction &other)
      : array1(other.array1 ? other.array1->basic_clone() : 0),
        array2(other.array2 ? other.array2->basic_clone() : 0), t21(other.t21) {
    //  .. nothing yet ..
  }

  InstanceToInstanceInteraction &
  operator=(const InstanceToInstanceInteraction &other) {
    if (this != &other) {
...
struct DB_PUBLIC_TEMPLATE interaction_key_for_clusters
  ...
  interaction_key_for_clusters(const db::ICplxTrans &_t1,
                               const db::ICplxTrans &_t21, const Box &_box)
...
public:
  instance_interaction_cache() : m_hits(0), m_misses(0) {}
  ...
  const Value *find(db::cell_index_type ci1, db::cell_index_type ci2,
                    const Key &key) const {
    typename std::map<std::pair<db::cell_index_type, db::cell_index_type>,
                      std::list<std::pair<Key, Value>>>::iterator i1 =
        m_map.find(std::make_pair(ci1, ci2));
    if (i1 == m_map.end()) {
      ++m_misses;
      return 0;
    }
    ...
    if (i == i1->second.end()) {
      ...
      //  move the element to the front so the most frequently used ones are at
      //  the front
  ...
  Value &insert(db::cell_index_type ci1, db::cell_index_type ci2,
                const Key &key) {
...
private:
  ...
  mutable std::map<std::pair<db::cell_index_type, db::cell_index_type>,
 ...
 */
template <class T> class DB_PUBLIC_TEMPLATE connected_clusters_iterator {
public:
  ...
  bool at_end() const { return m_lc_iter.at_end() && m_x_iter == m_x_iter_end; }
...
private:
  ...
  typename std::map<typename local_cluster<T>::id_type,
                    connections_type>::const_iterator m_x_iter,
      m_x_iter_end;
};
 ...
 *
 *  Clusters can get connected. There are incoming connections (from above the
 * hierarchy) and outgoing connections (down to a child cell).
 *
 *  "root" clusters are some that don't have incoming connections. There are
 * only root clusters or clusters which are connected from every parent cell.
 * There are no "half connected" clusters.
 */
template <class T>
class DB_PUBLIC_TEMPLATE connected_clusters : public local_clusters<T> {
public:
  ...
  typedef
      typename std::map<typename local_cluster<T>::id_type,
                        connections_type>::const_iterator connections_iterator;

  /**
   *  @brief Constructor
   */
  connected_clusters() : local_clusters<T>() {
    //  .. nothing yet ..
  }
   ...
   */
  const connections_type &
  connections_for_cluster(typename local_cluster<T>::id_type id) const;
  ...
  find_cluster_with_connection(const ClusterInstance &inst) const;

  /**
   *  @brief Adds a connection between a local cluster and one from a child
   * instance
   */
  void add_connection(typename local_cluster<T>::id_type,
                      const ClusterInstance &inst);

  /**
   *  @brief Joins the cluster id with the cluster with_id
   *
   *  The "with_id" cluster is removed. All connections of "with_id" are
   * transferred to the first one. All shapes of "with_id" are transferred to
   * "id".
   */
  void join_cluster_with(typename local_cluster<T>::id_type id,
                         typename local_cluster<T>::id_type with_id);
   ...
   *  @brief Begin iterator for the connections
   */
  connections_iterator end_connections() const { return m_connections.end(); }
   ...
   */
  void reset_root(id_type id) { m_connected_clusters.insert(id); }
...
private:
  ...
  std::map<ClusterInstance, typename local_cluster<T>::id_type>
      m_rev_connections;
...
template <class T>
inline void mem_stat(MemStatistics *stat, MemStatistics::purpose_t purpose,
                     int cat, const connected_clusters<T> &x, bool no_self,
                     void *parent) {
  x.mem_stat(stat, purpose, cat, no_self, parent);
}
 ...
 */
template <class T> class DB_PUBLIC_TEMPLATE hier_clusters : public tl::Object {
public:
  typedef typename local_cluster<T>::box_type box_type;

  typedef instance_interaction_cache<InstanceToInstanceInteraction,
                                     cluster_instance_pair_list_type>
      instance_interaction_cache_type;
   ...
   *
   *  The default value is 30. Basic timing will be reported for >
   * base_verbosity, detailed timing for > base_verbosity + 10.
   */
  ...
  /**
   *  @brief Builds a hierarchy of clusters from a cell hierarchy and given
   * connectivity
   */
  void
  build(const db::Layout &layout, const db::Cell &cell,
        const db::Connectivity &conn,
        const std::map<db::cell_index_type, tl::equivalence_clusters<size_t>>
            *attr_equivalence = 0,
        const std::set<cell_index_type> *breakout_cells = 0,
        bool separate_attributes = false);
  ...
  clusters_per_cell(db::cell_index_type cell_index) const;
   ...
   *
   *  The layout object is supposed to be the original layout or one with
   * identical cell indexes. "lm" is a layer mapping table from the connection
   * layer indexes to the target layer indexes.
   *
   *  The backannotation process usually involves propagation of shapes up in
   * the hierarchy to resolve variants.
   */
  void
  return_to_hierarchy(db::Layout &layout,
                      const std::map<unsigned int, unsigned int> &lm) const;
  ...
  /**
   *  @brief Ensures a cluster instance is connected from all parents of the
   * instantiated cell
   *
   *  If "with_self" is true, the specified instance "ci" is included in the
   * connections. Otherwise there is not connection made for this instance.
   */
  size_t propagate_cluster_inst(const db::Layout &layout, const Cell &cell,
                                const ClusterInstance &ci,
                                db::cell_index_type parent_ci, bool with_self);
...
private:
  ...
  void build_hier_connections_for_cells(
      ...
      bool separate_attributes);
  void
  do_build(cell_clusters_box_converter<T> &cbc, const db::Layout &layout,
           const db::Cell &cell, const db::Connectivity &conn,
           const std::map<cell_index_type, tl::equivalence_clusters<size_t>>
               *attr_equivalence,
           const std::set<cell_index_type> *breakout_cells,
           bool separate_attributes);
...
/**
 *  @brief A callback function for the recursive cluster shape and cluster
 * iterator selecting cells/circuits
 *
 *  Reimplement the "new_circuit" method to receive a callback on a new cell or
 * circuit.
 */
...
public:
  ...
  /**
   *  @brief This method is called whenever a circuit is entered when
   * descending. Return false to skip this circuit and all of it's children.
   * This method is called before the new cell is entered.
 ...
 *
 *  This iterator will deliver the shapes of a cluster including the shapes for
 * the connected child clusters.
 *
 *  This iterator applies to one layer.
 */
template <class T> class DB_PUBLIC_TEMPLATE recursive_cluster_shape_iterator {
public:
   ...
   *  @brief Constructor
   */
  recursive_cluster_shape_iterator(const hier_clusters<T> &hc,
                                   unsigned int layer, db::cell_index_type ci,
                                   typename local_cluster<T>::id_type id,
                                   const CircuitCallback *callback = 0);

  /**
   *  @brief Returns a value indicating whether there are any more shapes
   */
  bool at_end() const { return m_shape_iter.at_end(); }

  /**
   *  @brief Returns the shape (untransformed)
   */
  reference operator*() const { return *m_shape_iter; }

  /**
   *  @brief Returns the shape pointer (untransformed)
   */
  pointer operator->() const { return m_shape_iter.operator->(); }
  ...
  /**
   *  @brief Returns the transformation applicable for transforming the shape to
   * the root cluster
   */
  const db::ICplxTrans &trans() const { return m_trans_stack.back(); }

  /**
   *  @brief Returns the cell index the shape lives in
   */
  db::cell_index_type cell_index() const { return m_cell_index_stack.back(); }
...
private:
  ...
  std::vector<std::pair<typename connections_type::const_iterator,
                        typename connections_type::const_iterator>>
      m_conn_iter_stack;
 ...
 */
template <class T> class DB_PUBLIC_TEMPLATE recursive_cluster_iterator {
public:
  /**
   *  @brief Constructor
   */
  recursive_cluster_iterator(const hier_clusters<T> &hc, db::cell_index_type ci,
                             typename local_cluster<T>::id_type id);

  /**
   *  @brief Returns a value indicating whether there are any more shapes
   */
  bool at_end() const { return m_cell_index_stack.empty(); }
...
public:
  IncomingClusterInstance(db::cell_index_type pc, size_t parent_cluster_id,
                          const ClusterInstance &inst)
      : m_parent_cell(pc), m_parent_cluster_id(parent_cluster_id),
        m_inst(inst) {
    //  .. nothing yet ..
  }
   ...
   */
  size_t parent_cell() const { return m_parent_cell; }

  /**
   *  @brief Gets the cluster ID from which the cluster is connected to
   *  The parent cluster lives in the parent cell
   */
  size_t parent_cluster_id() const { return m_parent_cluster_id; }

  /**
   *  @brief Gets the instance path
   */
  const ClusterInstance &inst() const { return m_inst; }
  ...
  bool operator==(const IncomingClusterInstance &other) const {
    return m_parent_cluster_id == other.m_parent_cluster_id &&
           m_parent_cell == other.m_parent_cell && m_inst == other.m_inst;
  }
 ...
 *
 *  This class can be used to quickly identify the connections made to a
 * specific cluster from a parent cluster.
 */
template <class T> class incoming_cluster_connections {
public:
  typedef std::list<IncomingClusterInstance> incoming_connections;

  incoming_cluster_connections(const db::Layout &layout, const db::Cell &cell,
                               const hier_clusters<T> &hc);

  bool has_incoming(db::cell_index_type ci, size_t cluster_id) const;
  const incoming_connections &incoming(db::cell_index_type ci,
                                       size_t cluster_id) const;

private:
  mutable std::set<db::cell_index_type> m_called_cells;
  mutable std::map<db::cell_index_type, std::map<size_t, incoming_connections>>
      m_incoming;
 ...
 *  This function is used to provide a generic attribute wrapping a property ID,
 * text ID and global net ID
 */
 ...
 */
inline bool is_prop_id_attr(size_t attr) { return (attr & 3) == 0; }
 ...
 * text ID and global net ID
 */
inline size_t global_net_id_to_attr(size_t id) { return size_t(id) * 4 + 2; }

/**
 *  @brief Gets a value indicating whether the attribute is a global net ID
 */
inline bool is_global_net_id_attr(size_t attr) { return (attr & 3) == 2; }

/**
 *  @brief Gets the global net ID from an attribute
 */
inline size_t global_net_id_from_attr(size_t attr) { return attr / 4; }
 ...
 */
inline bool is_text_ref_attr(size_t attr) { return (attr & 3) == 1; }
