...
/**
 *  @brief A helper class for the generic implementation of the layout insert
 * functionality
 */
class ShapesInserter {
public:
  ShapesInserter(db::Shapes *shapes, const db::ICplxTrans &trans,
                 db::Coord ep_sizing)
  ...
  template <class P> void insert_polygon(const P &p) {
    if (p.is_box() && !m_trans.is_complex()) {
      mp_shapes->insert(p.box().transformed(m_trans));
    } else {
      if (mp_shapes->cell() && mp_shapes->cell()->layout()) {
        db::polygon_ref<P, db::Disp> pr(
            p.transformed(m_trans),
            mp_shapes->cell()->layout()->shape_repository());
        mp_shapes->insert(pr);
      } else {
  ...
  void operator()(const db::Polygon &p) { insert_polygon(p); }

  void operator()(const db::SimplePolygon &p) { insert_polygon(p); }

  void operator()(const db::EdgePair &ep) {
    mp_shapes->insert(
        ep.normalized().to_polygon(m_ep_sizing).transformed(m_trans));
  }
...
/**
 *  @brief A helper class for the generic implementation of the region insert
 * functionality
 */
class RegionInserter {
public:
  RegionInserter(db::Region *region, const db::ICplxTrans &trans,
                 db::Coord ep_sizing)
...
/**
 *  @brief A helper class for the generic implementation of the edge collection
...
/**
 *  @brief A helper class for the generic implementation of the edge pair
 * collection insert functionality
 */
...
/**
 *  @brief A helper class for the generic implementation of the text collection
 * insert functionality
 */
class TextsInserter {
public:
...
class TileLayoutOutputReceiver : public db::TileOutputReceiver {
public:
  TileLayoutOutputReceiver(db::Layout *layout, db::Cell *cell,
                           unsigned int layer, db::Coord e)
  ...
  void begin(size_t /*nx*/, size_t /*ny*/, const db::DPoint & /*p0*/,
             double /*dx*/, double /*dy*/, const db::DBox & /*frame*/) {
    mp_layout->start_changes();
  }

  void finish(bool /*success*/) { mp_layout->end_changes(); }
...
class TileRegionOutputReceiver : public db::TileOutputReceiver {
public:
...
class TileEdgesOutputReceiver : public db::TileOutputReceiver {
public:
  TileEdgesOutputReceiver(db::Edges *edges) : mp_edges(edges) {
    //  .. nothing yet ..
  }
...
class TileEdgePairsOutputReceiver : public db::TileOutputReceiver {
public:
...
class TileTextsOutputReceiver : public db::TileOutputReceiver {
public:
  TileTextsOutputReceiver(db::Texts *texts) : mp_texts(texts) {
    //  .. nothing yet ..
  }
...
class TilingProcessorJob : public tl::JobBase {
public:
  TilingProcessorJob(TilingProcessor *proc, int nworkers, bool has_tiles)
      : tl::JobBase(nworkers), mp_proc(proc), m_has_tiles(has_tiles),
        m_progress_count(0), m_progress(std::string()) {
    //  .. nothing yet ..
  }

  void start(const std::string &job_description);

  bool has_tiles() const { return m_has_tiles; }
  ...
  TilingProcessor *processor() const { return mp_proc; }
...
class TilingProcessorTask : public tl::Task {
public:
  TilingProcessorTask(const std::string &tile_desc, size_t ix, size_t iy,
                      const db::DBox &clip_box, const db::DBox &region,
                      const std::string &script, size_t script_index)
      : m_tile_desc(tile_desc), m_ix(ix), m_iy(iy), m_clip_box(clip_box),
        m_region(region), m_script(script), m_script_index(script_index) {
    //  .. nothing yet ..
  }

  const std::string &tile_desc() const { return m_tile_desc; }

  const db::DBox &clip_box() const { return m_clip_box; }

  size_t ix() const { return m_ix; }

  size_t iy() const { return m_iy; }

  const db::DBox &region() const { return m_region; }

  const std::string &script() const { return m_script; }

  size_t script_index() const { return m_script_index; }
...
class TilingProcessorWorker : public tl::Worker {
public:
  TilingProcessorWorker(TilingProcessorJob *job) : tl::Worker(), mp_job(job) {
    //  .. nothing yet ..
  }
...
private:
  ...
  void make_input_var(const TilingProcessor::InputSpec &is,
                      const db::RecursiveShapeIterator *iter, tl::Eval &eval,
                      double sf);
};

class TilingProcessorReceiverFunction : public tl::EvalFunction {
public:
  TilingProcessorReceiverFunction(TilingProcessor *proc) : mp_proc(proc) {
    //  .. nothing yet ..
  }
...
class TilingProcessorOutputFunction : public tl::EvalFunction {
public:
  TilingProcessorOutputFunction(TilingProcessor *proc, size_t ix, size_t iy,
                                const db::Box &tile_box)
...
class TilingProcessorCountFunction : public tl::EvalFunction {
public:
...
void TilingProcessorWorker::make_input_var(
    const TilingProcessor::InputSpec &is,
    const db::RecursiveShapeIterator *iter, tl::Eval &eval, double sf) {
  if (!iter) {
    iter = &is.iter;
  }

  if (is.type == TilingProcessor::TypeRegion) {
    eval.set_var(is.name,
                 tl::Variant(db::Region(*iter, db::ICplxTrans(sf) * is.trans,
                                        is.merged_semantics)));
  } else if (is.type == TilingProcessor::TypeEdges) {
    eval.set_var(is.name,
                 tl::Variant(db::Edges(*iter, db::ICplxTrans(sf) * is.trans,
                                       is.merged_semantics)));
  } else if (is.type == TilingProcessor::TypeEdgePairs) {
    eval.set_var(is.name, tl::Variant(db::EdgePairs(*iter, db::ICplxTrans(sf) *
                                                               is.trans)));
  } else if (is.type == TilingProcessor::TypeTexts) {
    eval.set_var(is.name,
                 tl::Variant(db::Texts(*iter, db::ICplxTrans(sf) * is.trans)));
  }
}
...
void TilingProcessorWorker::do_perform(const TilingProcessorTask *tile_task) {
  ...
  if (!mp_job->has_tiles()) {
    ...
    clip_box_dbu = db::Box(tile_task->clip_box().transformed(
        db::DCplxTrans(mp_job->processor()->dbu()).inverted()));
  ...
  {
    db::Box frame_box_dbu = db::Box(mp_job->processor()->frame().transformed(
        db::DCplxTrans(mp_job->processor()->dbu()).inverted()));
  ...
  for (std::vector<TilingProcessor::InputSpec>::const_iterator i =
           mp_job->processor()->begin_inputs();
       i != mp_job->processor()->end_inputs(); ++i) {
    ...
    if (!mp_job->has_tiles()) {
      ...
      db::Box region_dbu = db::Box(tile_task->region().transformed(
          (db::DCplxTrans(dbu) * db::DCplxTrans(i->trans)).inverted()));
  ...
  eval.define_function("_output", new TilingProcessorOutputFunction(
                                      mp_job->processor(), tile_task->ix(),
                                      tile_task->iy(), clip_box_dbu));
  eval.define_function(
      "_rec", new TilingProcessorReceiverFunction(mp_job->processor()));
  eval.define_function("_count",
                       new TilingProcessorCountFunction(mp_job->processor()));

  if (tl::verbosity() >= (mp_job->has_tiles() ? 20 : 10)) {
    tl::info << "TilingProcessor: script #" << (tile_task->script_index() + 1)
             << ", tile " << tile_task->tile_desc();
  }

  tl::SelfTimer timer(tl::verbosity() >= (mp_job->has_tiles() ? 21 : 11),
                      "Elapsed time");
...
tl::Worker *TilingProcessorJob::create_worker() {
...
TilingProcessor::TilingProcessor()
    : m_tile_width(0.0), m_tile_height(0.0), m_ntiles_w(0), m_ntiles_h(0),
      m_tile_size_given(false), m_tile_count_given(false), m_tile_origin_x(0.0),
      m_tile_origin_y(0.0), m_tile_origin_given(false), m_tile_bx(0.0),
      m_tile_by(0.0), m_threads(0), m_dbu(0.001), m_dbu_specific(0.001),
      m_dbu_specific_set(false), m_scale_to_dbu(true) {
  //  .. nothing yet ..
}

void TilingProcessor::input(const std::string &name,
                            const db::RecursiveShapeIterator &iter,
                            const db::ICplxTrans &trans, Type type,
...
TilingProcessor::~TilingProcessor() { m_outputs.clear(); }

void TilingProcessor::set_frame(const db::DBox &frame) { m_frame = frame; }
...
void TilingProcessor::set_threads(size_t n) { m_threads = n; }
...
void TilingProcessor::output(const std::string &name, size_t id,
                             TileOutputReceiver *rec,
...
void TilingProcessor::output(const std::string &name, db::Layout &layout,
                             db::cell_index_type cell_index,
                             const db::LayerProperties &lp, db::Coord ep_ext) {
...
void TilingProcessor::output(const std::string &name, db::Layout &layout,
                             db::cell_index_type cell_index, unsigned int layer,
                             db::Coord ep_ext) {
  m_top_eval.set_var(name, m_outputs.size());
  m_outputs.push_back(OutputSpec());
  m_outputs.back().name = name;
  m_outputs.back().id = 0;
  m_outputs.back().receiver = new TileLayoutOutputReceiver(
      &layout, &layout.cell(cell_index), layer, ep_ext);
}

void TilingProcessor::output(const std::string &name, db::Region &region,
                             db::Coord ep_ext) {
  m_top_eval.set_var(name, m_outputs.size());
  m_outputs.push_back(OutputSpec());
  m_outputs.back().name = name;
...
void TilingProcessor::output(const std::string &name,
...
tl::Variant TilingProcessor::receiver(const std::vector<tl::Variant> &args) {
  tl::MutexLocker locker(&s_output_lock);

  if (args.size() != 1) {
    throw tl::Exception(tl::to_string(tr("_rec function requires one argument: "
  ...
  if (index >= m_outputs.size()) {
    throw tl::Exception(
        tl::to_string(tr("Invalid handle in _rec function call")));
  }
  ...
  return tl::Variant(
      proxy, gsi::cls_decl<TileOutputReceiver>()->var_cls(true /*const*/),
      true);
}

void TilingProcessor::put(size_t ix, size_t iy, const db::Box &tile,
                          const std::vector<tl::Variant> &args) {
  tl::MutexLocker locker(&s_output_lock);

  if (args.size() < 2 || args.size() > 3) {
    throw tl::Exception(
        tl::to_string(tr("_output function requires two or three arguments: "
                         "handle and object and a clip flag (optional)")));
  }
  ...
  if (index >= m_outputs.size()) {
    throw tl::Exception(tl::to_string(
        tr("Invalid handle (first argument) in _output function call")));
  }

  m_outputs[index].receiver->put(ix, iy, tile, m_outputs[index].id, args[1],
                                 dbu(), m_outputs[index].trans, clip);
}
...
void TilingProcessor::execute(const std::string &desc) {
  ...
  if (tot_box.empty()) {
    for (std::vector<InputSpec>::const_iterator i = m_inputs.begin();
         i != m_inputs.end(); ++i) {
      if (!i->iter.at_end()) {
        if (scale_to_dbu()) {
          double dbu_value = i->iter.layout() ? i->iter.layout()->dbu() : dbu();
          tot_box += i->iter.bbox().transformed(db::CplxTrans(dbu_value) *
                                                db::CplxTrans(i->trans));
        } else {
          tot_box += i->iter.bbox().transformed(db::CplxTrans(dbu()) *
                                                db::CplxTrans(i->trans));
        }
      }
    }
  ...
  if (tot_box.empty() &&
      !(m_tile_count_given && m_tile_origin_given && m_tile_size_given)) {
    return;
  }

  db::DBox frame = tot_box;

  //  add the border to the total box. The reasoning for this is that for
  //  example an oversize might grow into the outer region and therefore be
  //  clipped.
  ...
  if (m_tile_size_given && !m_tile_count_given) {
    ...
    ntiles_w = (m_tile_width > 1e-6
                    ? size_t(ceil(tot_box.width() / m_tile_width - 1e-10))
                    : 1);
    ntiles_h = (m_tile_height > 1e-6
                    ? size_t(ceil(tot_box.height() / m_tile_height - 1e-10))
                    : 1);
    ...
    tile_width =
        (m_ntiles_w > 0
             ? dbu() * ceil(tot_box.width() / (dbu() * m_ntiles_w) - 1e-10)
             : 0.0);
    tile_height =
        (m_ntiles_h > 0
             ? dbu() * ceil(tot_box.height() / (dbu() * m_ntiles_h) - 1e-10)
             : 0.0);
  ...
  if (has_tiles) {
    ...
    if (m_tile_origin_given) {
      ...
      l = dbu() *
          floor(0.5 +
                (tot_box.center().x() - ntiles_w * 0.5 * tile_width) / dbu() +
                1e-10);
      b = dbu() *
          floor(0.5 +
                (tot_box.center().y() - ntiles_h * 0.5 * tile_height) / dbu() +
                1e-10);
    }
    ...
    for (size_t ix = 0; ix < ntiles_w; ++ix) {
      ...
      for (size_t iy = 0; iy < ntiles_h; ++iy) {
        ...
        db::DBox clip_box(l + ix * tile_width, b + iy * tile_height,
                          l + (ix + 1) * tile_width,
                          b + (iy + 1) * tile_height);
        db::DBox region = clip_box.enlarged(db::DVector(m_tile_bx, m_tile_by));

        std::string tile_desc =
            tl::sprintf("%d/%d,%d/%d", ix + 1, ntiles_w, iy + 1, ntiles_h);

        size_t si = 0;
        for (std::vector<std::string>::const_iterator s = m_scripts.begin();
             s != m_scripts.end(); ++s, ++si) {
          job.schedule(new TilingProcessorTask(tile_desc, ix, iy, clip_box,
                                               region, *s, si));
        }
      }
    }
    ...
    for (std::vector<std::string>::const_iterator s = m_scripts.begin();
         s != m_scripts.end(); ++s, ++si) {
      job.schedule(
          new TilingProcessorTask("all", 0, 0, db::DBox(), db::DBox(), *s, si));
    }
  }
  ...
  try {
    ...
    try {
      ...
      for (std::vector<OutputSpec>::iterator o = m_outputs.begin();
           o != m_outputs.end(); ++o) {
        if (o->receiver) {
          o->receiver->set_processor(this);
          o->receiver->begin(ntiles_w, ntiles_h, db::DPoint(l, b), tile_width,
                             tile_height, frame);
        }
      }
