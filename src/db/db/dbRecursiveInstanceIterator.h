 ...
 *  It simplifies retrieval of instances from a geometrical region while
 * considering subcells as well. Some options can be specified, i.e. the level
 * to which to look into or which cells to select.
 *
 *  The general iteration scheme is iterating is depth-first and child instances
 * before parent instances.
 */
...
public:
   ...
   *
   *  By default the iterator operates in touching mode - i.e. instances that
   * touch the given region are returned. By specifying the "overlapping" flag
   * with a true value, the iterator delivers instances that overlap the given
   * region by at least one database unit. The cell instances are selected
   * according to their overall bounding box.
   */
   ...
   * with a true value, the iterator delivers instances that overlap the given
   * region by at least one database unit. The cell instances are selected
   * according to their overall bounding box. This version offers a complex
   * search region instead of a simple box.
   */
  RecursiveInstanceIterator(const layout_type &layout, const cell_type &cell,
   ...
   *
   *  A depth of 0 instructs the iterator to deliver only instances from the
   * initial cell. A higher depth instructs the iterator to look deeper. The
   * depth must be specified before the instances are being retrieved.
   */
   ...
   *
   *  A depth of 0 instructs the iterator to deliver instance from the top level
   * and below. 1 instructs to deliver instance from the first child level. The
   * minimum depth must be specified before the instances are being retrieved.
   */
  ...
  /**
   *  @brief Gets a flag indicating whether overlapping instances are selected
   * when a region is used
   */
  bool overlapping() const { return m_overlapping; }

  /**
   *  @brief Sets a flag indicating whether overlapping instances are selected
   * when a region is used
   */
   ...
   */
  const std::set<db::cell_index_type> &targets() const { return m_targets; }

  /**
   *  @brief Gets a flags indicating whether all targets are selected
   *  See \targets for more details.
   */
  bool all_targets_enabled() const { return m_all_targets; }
   ...
   *  hierarchy tree. This happens by "selecting" and "unselecting" cells in the
   * traversal path. "selected" cells will make iterator traverse the tree below
   * this cell while "unselected" cells make the iterator ignore this cell.
   *  Cells which are neither selected nor unselected will be traversed
   * depending on their parent's state. They are traversed if their parents are
   * and are not traversed if their parents are not.
   *
  ...
  /**
   *  @brief Get the current transformation by which the instances must be
   * transformed into the initial cell
   *
   *  The instances delivered are not transformed. Instead, this transformation
   * must be applied to get the instance in the coordinate system of the top
   * cell.
   */
   ...
   */
  instance_element_type instance() const { return *operator->(); }
   ...
   */
  instance_element_type operator*() const { return *operator->(); }
 ...
 *
 *  In push mode, the iterator will deliver the instances and hierarchy
 * transitions to this interface. See "RecursiveInstanceIterator::push" for
...
public:
   ...
   * with "region"
   */
  virtual void enter_cell(const RecursiveInstanceIterator * /*iter*/,
   ...
   */
  virtual void leave_cell(const RecursiveInstanceIterator * /*iter*/,
                          const db::Cell * /*cell*/) {}
   ...
   */
  virtual new_inst_mode new_inst(const RecursiveInstanceIterator * /*iter*/,
   ...
   */
  virtual bool new_inst_member(const RecursiveInstanceIterator * /*iter*/,
                               const db::CellInstArray & /*inst*/,
