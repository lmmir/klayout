 ...
 */
class DB_PUBLIC DeepRegion : public MutableRegion,
...
public:
  ...
  DeepRegion(const RecursiveShapeIterator &si, DeepShapeStore &dss,
             double area_ratio = 0.0, size_t max_vertex_count = 0);
  DeepRegion(const RecursiveShapeIterator &si, DeepShapeStore &dss,
  ...
  and_with(const Region &other,
           db::PropertyConstraint property_constraint) const;
  virtual RegionDelegate *
  not_with(const Region &other,
           db::PropertyConstraint property_constraint) const;
  virtual RegionDelegate *
  xor_with(const Region &other,
           db::PropertyConstraint property_constraint) const;
  virtual RegionDelegate *
  or_with(const Region &other,
          db::PropertyConstraint property_constraint) const;
  virtual std::pair<RegionDelegate *, RegionDelegate *>
  andnot_with(const Region &, db::PropertyConstraint property_constraint) const;
  ...
  virtual RegionDelegate *nets(LayoutToNetlist *l2n, NetPropertyMode prop_mode,
                               const tl::Variant &net_prop_name,
                               const std::vector<const Net *> *nets) const;

  virtual DeepShapeCollectionDelegateBase *deep() { return this; }
...
private:
  ...
  DeepLayer and_or_not_with(const DeepRegion *other, bool and_op,
                            PropertyConstraint property_constraint) const;
  std::pair<DeepLayer, DeepLayer>
  and_and_not_with(const DeepRegion *other,
                   PropertyConstraint property_constraint) const;
  DeepRegion *apply_filter(const PolygonFilterBase &filter) const;

  template <class Result, class OutputContainer>
  OutputContainer *
  processed_impl(const polygon_processor<Result> &filter) const;

  template <class Proc> void configure_proc(Proc &proc) const {
