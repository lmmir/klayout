...
public:
  ...
  HullEdgeCollector(const db::Edge &e) : m_e(e) {
    int rot = 0;
    if (e.dx() > 0) {
      rot = db::FTrans::r0;
    } else if (e.dy() > 0) {
  ...
  void produce(std::vector<db::Point> &points) {
    ...
    for (tl::interval_map<db::Coord, db::Coord>::const_iterator cm =
             m_cmap.begin();
         cm != m_cmap.end(); ++cm) {
  ...
  void reduce(size_t n) {
    ...
    while ((ntot = m_cmap.size()) > n) {
      ...
      if (ntot > 1) {
        ...
        tl::interval_map<db::Coord, db::Coord>::const_iterator cl =
            m_cmap.begin();
        for (tl::interval_map<db::Coord, db::Coord>::const_iterator cm =
                 m_cmap.begin();
             cm != m_cmap.end();) {
          ...
          if ((cc == m_cmap.begin() || cc->second < cl->second) &&
              (cm == m_cmap.end() || cc->second < cm->second)) {
            if (cc == m_cmap.begin()) {
              pockets.push_back(
                  db::Box(db::Point(cc->first.first, cm->second),
                          db::Point(cc->first.second, cc->second)));
            } else if (cm == m_cmap.end()) {
              pockets.push_back(
                  db::Box(db::Point(cc->first.first, cl->second),
                          db::Point(cc->first.second, cc->second)));
            } else {
              pockets.push_back(db::Box(
                  db::Point(cc->first.first, std::max(cl->second, cm->second)),
                  db::Point(cc->first.second, cc->second)));
            }
          }
      ...
      if (pockets.size() > ntot - n) {
        ...
        std::nth_element(pockets.begin(), pockets.begin() + (ntot - n),
                         pockets.end(), ac_op);
      ...
      for (std::vector<db::Box>::const_iterator p = pockets.begin();
           p != pockets.end(); ++p) {
        ECJoinOp jo;
...
CellHullGenerator::CellHullGenerator(const db::Layout &layout)
    : m_all_layers(true), m_small_cell_size(100),
      m_complexity(default_complexity) {
...
void CellHullGenerator::generate_hull(const db::Cell &cell,
  ...
  if (bbox.height() <=
          db::coord_traits<db::Coord>::distance_type(m_small_cell_size) &&
      bbox.width() <=
          db::coord_traits<db::Coord>::distance_type(m_small_cell_size)) {
  ...
  db::Box sectors[4] = {db::Box(bbox.lower_left(), bbox.center()),
  ...
  for (unsigned int i = 0; i < 4; ++i) {
    ...
    unsigned int j = 0;
    for (db::Polygon::polygon_edge_iterator es = ps.begin_edge();
         !es.at_end() && j < 4; ++es, ++j) {
  ...
  for (std::vector<unsigned int>::const_iterator l = m_layers.begin();
    ...
    for (db::ShapeIterator s = cell.shapes(*l).begin(
             db::ShapeIterator::Polygons | db::ShapeIterator::Boxes |
      ...
      for (unsigned int is = 0; is < 4; ++is) {
        ...
        if (poly.box().inside(sectors[is])) {
          ...
          for (std::vector<db::Polygon>::const_iterator p =
                   clipped_polygons.begin();
               p != clipped_polygons.end(); ++p) {
            for (unsigned int ie = 0; ie < 4; ++ie) {
              ec[is][ie].add(*p);
            }
          }
