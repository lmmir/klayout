...
AsIfFlatEdgePairs::AsIfFlatEdgePairs(const AsIfFlatEdgePairs &other)
    : EdgePairsDelegate(other), m_bbox_valid(other.m_bbox_valid),
      m_bbox(other.m_bbox) {
  operator=(other);
}
...
std::string AsIfFlatEdgePairs::to_string(size_t nmax) const {
...
EdgePairsDelegate *AsIfFlatEdgePairs::in(const EdgePairs &other,
                                         bool invert) const {
...
size_t AsIfFlatEdgePairs::count() const {
  size_t n = 0;
...
size_t AsIfFlatEdgePairs::hier_count() const { return count(); }
...
void AsIfFlatEdgePairs::invalidate_bbox() { m_bbox_valid = false; }

RegionDelegate *AsIfFlatEdgePairs::processed_to_polygons(
    const EdgePairToPolygonProcessorBase &filter) const {
  std::unique_ptr<FlatRegion> region(new FlatRegion());
  db::PropertyMapper pm(region->properties_repository(),
...
EdgesDelegate *AsIfFlatEdgePairs::processed_to_edges(
    const EdgePairToEdgeProcessorBase &filter) const {
  std::unique_ptr<FlatEdges> edges(new FlatEdges());
  db::PropertyMapper pm(edges->properties_repository(),
                        properties_repository());
  ...
  for (EdgePairsIterator e(begin()); !e.at_end(); ++e) {
    ...
    for (std::vector<db::Edge>::const_iterator pr = res_edges.begin();
         pr != res_edges.end(); ++pr) {
      db::properties_id_type prop_id = pm(e.prop_id());
      if (prop_id != 0) {
...
RegionDelegate *AsIfFlatEdgePairs::polygons(db::Coord e) const {
...
EdgesDelegate *AsIfFlatEdgePairs::edges() const {
...
EdgesDelegate *AsIfFlatEdgePairs::first_edges() const {
  std::unique_ptr<FlatEdges> output(new FlatEdges());
  db::PropertyMapper pm(output->properties_repository(),
                        properties_repository());
...
EdgesDelegate *AsIfFlatEdgePairs::second_edges() const {
  std::unique_ptr<FlatEdges> output(new FlatEdges());
  db::PropertyMapper pm(output->properties_repository(),
                        properties_repository());
...
EdgePairsDelegate *AsIfFlatEdgePairs::add(const EdgePairs &other) const {
  const FlatEdgePairs *other_flat =
      dynamic_cast<const FlatEdgePairs *>(other.delegate());
  if (other_flat) {

    std::unique_ptr<FlatEdgePairs> new_edge_pairs(
        new FlatEdgePairs(*other_flat));
    ...
    for (EdgePairsIterator p(begin()); !p.at_end(); ++p) {
      db::properties_id_type prop_id = pm(p.prop_id());
      if (prop_id) {
        new_edge_pairs->raw_edge_pairs().insert(
            db::EdgePairWithProperties(*p, prop_id));
      } else {
    ...
    db::PropertyMapper pm(new_edge_pairs->properties_repository(),
                          properties_repository());
    db::PropertyMapper pm_other(new_edge_pairs->properties_repository(),
...
void AsIfFlatEdgePairs::insert_into(Layout *layout,
...
void AsIfFlatEdgePairs::insert_into_as_polygons(Layout *layout,
                                                db::cell_index_type into_cell,
  ...
  for (EdgePairsIterator e(begin()); !e.at_end(); ++e) {
    db::properties_id_type prop_id = pm(e.prop_id());
    if (prop_id) {
      shapes.insert(db::SimplePolygonWithProperties(
          e->normalized().to_simple_polygon(enl), prop_id));
    } else {
