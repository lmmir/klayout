 ...
 */
class DB_PUBLIC NetlistSpiceReaderDelegate : public tl::Object {
public:
   ...
   */
  const NetlistSpiceReaderOptions &options() const { return m_options; }

  /**
   *  @brief Gets the reader options (non-const)
   */
  NetlistSpiceReaderOptions &options() { return m_options; }
  ...
  /**
   *  @brief Returns true, if the delegate wants subcircuit elements with this
   * name
   *
   ...
   *
   *  The default implementation will unescape backslash sequences into plain
   * characters.
   */
   ...
   *  @param value The default value (e.g. resistance for resistors) and may be
   * zero.
   *  @param nets The nets given in the element line.
   *  @param parameters The parameters of the element statement (parameter names
   * are upper case).
   *
   ...
   */
  virtual void parse_element(const std::string &s, const std::string &element,
                             std::string &model, double &value,
   ...
   */
  void
  parse_element_components(const std::string &s,
   ...
   */
  static tl::Variant
  read_value(tl::Extractor &ex,
             const std::map<std::string, tl::Variant> &variables);

  /**
   *  @brief Reads a value from the extractor (with formula evaluation and two
   * levels of variables)
   */
  static tl::Variant
  read_value(tl::Extractor &ex,
             const std::map<std::string, tl::Variant> &variables1,
   ...
   */
  static bool
  try_read_value(const std::string &s, double &v,
                 const std::map<std::string, tl::Variant> &variables);
...
private:
  ...
  void def_values_per_element(const std::string &element,
                              std::map<std::string, tl::Variant> &pv);
};
