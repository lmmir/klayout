...
RecursiveInstanceIterator::RecursiveInstanceIterator(
    const RecursiveInstanceIterator &d) {
  operator=(d);
}
...
RecursiveInstanceIterator::RecursiveInstanceIterator(const layout_type &layout,
...
void RecursiveInstanceIterator::init_region(
    const RecursiveInstanceIterator::box_type &region) {
  m_region = region;
  mp_complex_region.reset(0);
}

void RecursiveInstanceIterator::init_region(
    const RecursiveInstanceIterator::region_type &region) {
  if (region.empty()) {
...
void RecursiveInstanceIterator::set_targets(
    const std::set<db::cell_index_type> &tgt) {
...
struct BoxTreePusher : public db::SimplePolygonSink {
  BoxTreePusher(RecursiveInstanceIterator::box_tree_type *bt) : mp_bt(bt) {
    //  .. nothing yet ..
  }
...
void RecursiveInstanceIterator::validate(
    RecursiveInstanceReceiver *receiver) const {
  if (!m_needs_reinit) {
    return;
  }
...
void RecursiveInstanceIterator::unselect_cells(
...
void RecursiveInstanceIterator::select_cells(
    const std::set<db::cell_index_type> &cells) {
  if (mp_layout) {
...
std::vector<db::InstElement> RecursiveInstanceIterator::path() const {
...
bool RecursiveInstanceIterator::needs_visit() const {
  return int(m_inst_iterators.size()) >= m_min_depth && !is_inactive() &&
         (m_all_targets ||
          m_targets.find(m_inst->cell_index()) != m_targets.end());
}

void RecursiveInstanceIterator::next_instance(
    RecursiveInstanceReceiver *receiver) const {
  while (true) {
    ...
    while (true) {
      ...
      if (!m_inst.at_end()) {
        ...
        if (int(m_inst_iterators.size()) < m_max_depth &&
            (m_all_targets ||
             m_target_tree.find(m_inst->cell_index()) != m_target_tree.end())) {
          down(receiver);
        } else {
          break;
        }
...
void RecursiveInstanceIterator::down(
    RecursiveInstanceReceiver *receiver) const {
  tl_assert(mp_layout);
  ...
  if (!m_local_complex_region_stack.empty()) {
    ...
    if (!new_region.empty()) {
      ...
      for (box_tree_type::touching_iterator b = pcl.begin_touching(
               correct_box_overlapping(new_region.transformed(tinst)),
...
void RecursiveInstanceIterator::new_cell(
    RecursiveInstanceReceiver *receiver) const {
  ...
  m_inst = cell()->begin_touching(
      correct_box_overlapping(m_local_region_stack.back()));
...
void RecursiveInstanceIterator::new_inst(
    RecursiveInstanceReceiver *receiver) const {
  //  look for the next instance with a non-empty array iterator. The array
  //  iterator can be empty because we use a lookup region.
  while (!m_inst.at_end()) {
    ...
    RecursiveInstanceReceiver::new_inst_mode ni =
        RecursiveInstanceReceiver::NI_all;
    if (receiver) {
      ni = receiver->new_inst(this, m_inst->cell_inst(),
    ...
    if (ni == RecursiveInstanceReceiver::NI_skip) {
      ...
      m_inst_array = m_inst->cell_inst().begin_touching(
          correct_box_overlapping(m_local_region_stack.back()), m_box_convert);
    } else {
...
void RecursiveInstanceIterator::new_inst_member(
    RecursiveInstanceReceiver *receiver) const {
  if (!m_local_complex_region_stack.empty()) {
    ...
    while (!m_inst_array.at_end()) {
      db::Box ia_box = m_inst->complex_trans(*m_inst_array) *
                       m_box_convert(m_inst->cell_inst().object());
  ...
  while (!m_inst_array.at_end() && receiver) {
    if (receiver->new_inst_member(this, m_inst->cell_inst(),
                                  m_inst->complex_trans(*m_inst_array),
                                  m_local_region_stack.back(),
                                  m_local_complex_region_stack.empty()
                                      ? 0
...
bool RecursiveInstanceIterator::is_outside_complex_region(
    const box_type &box) const {
  if (m_overlapping) {
    return m_local_complex_region_stack.back()
        .begin_overlapping(box, db::box_convert<box_type>())
        .at_end();
  } else {
    return m_local_complex_region_stack.back()
        .begin_touching(box, db::box_convert<box_type>())
        .at_end();
  }
}

bool RecursiveInstanceIterator::is_child_inactive(
