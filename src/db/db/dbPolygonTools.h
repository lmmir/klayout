 ...
 *
 *  This class allows an efficient test whether multiple points are inside a
 * given polygon. Since the test is efficiently implemented when the polygon
 * edges are sorted, the sorting and memory allocation step is performed once in
 * the test operator's constructor while each individual test is performed
 * efficiently.
 */

template <class P> class inside_poly_test {
public:
...
template <class Polygon> class DB_PUBLIC cut_polygon_receiver_base {
public:
...
class cut_polygon_receiver : public cut_polygon_receiver_base<Polygon> {
public:
  cut_polygon_receiver(const OutputIter &iter) : m_iter(iter) {}

  virtual void put(const Polygon &polygon) { *m_iter++ = polygon; }
...
void DB_PUBLIC
cut_polygon_internal(const PolygonType &input, const Edge &line,
 ...
 *  and produces all parts of the polygon that are "right" of the line given by
 * "line".
 */
template <class PolygonType, class OutputIter>
void cut_polygon(const PolygonType &input,
                 const typename PolygonType::edge_type &line,
                 OutputIter right_of_line) {
 ...
 *
 *  This function splits a polygon into parts using some heuristics to determine
 * a "suitable" cut line. The cut line is chosen through a vertex close to a
 * center (either horizontal or vertical). The splitting is supposed to create
 * smaller parts with less vertices or a better area ratio of polygon to
 * bounding box area.
 *
...
void DB_PUBLIC split_polygon(const PolygonType &polygon,
                             std::vector<PolygonType> &output);

/**
 *  @brief Determines whether a polygon and a box interact
 *
 *  This function determines whether the polygon and the box share at least on
 * common point and returns true in this case.
 */
...
bool interact_pb(const Polygon &poly, const Box &box) {
  ...
  for (typename Polygon::polygon_edge_iterator e = poly.begin_edge();
       !e.at_end(); ++e) {
...
bool interact_pp(const Polygon1 &poly1, const Polygon2 &poly2) {
  ...
  //  if at least one point of poly2 is inside or at the rim of poly1, return
  //  true
  ...
  //  in all other cases, in intersection happens if at least one of the edges
  //  of poly1 or poly2 intersect. This is checked with a simple scanline
  //  algorithm ...
...
/**
 *  @brief Determines whether a polygon and an edge share at least one common
 * point.
 */
...
bool interact_pe(const Polygon &poly, const Edge &edge) {
  ...
  if (poly.box().contains(edge.p1()) &&
      db::inside_poly(poly.begin_edge(), edge.p1()) >= 0) {
    return true;
  } else {
    for (typename Polygon::polygon_edge_iterator pe = poly.begin_edge();
         !pe.at_end(); ++pe) {
...
inline bool interact(const db::Box &box1, const db::Box &box2) {
  return box1.touches(box2);
}
inline bool interact(const db::DBox &box1, const db::DBox &box2) {
  return box1.touches(box2);
}
inline bool interact(const db::Polygon &poly, const db::Box &box) {
  return interact_pb(poly, box);
}
inline bool interact(const db::SimplePolygon &poly, const db::Box &box) {
  return interact_pb(poly, box);
}
inline bool interact(const db::DPolygon &poly, const db::DBox &box) {
  return interact_pb(poly, box);
}
inline bool interact(const db::DSimplePolygon &poly, const db::DBox &box) {
  return interact_pb(poly, box);
}
inline bool interact(const db::Polygon &poly, const db::Edge &edge) {
  return interact_pe(poly, edge);
}
inline bool interact(const db::SimplePolygon &poly, const db::Edge &edge) {
  return interact_pe(poly, edge);
}
inline bool interact(const db::DPolygon &poly, const db::DEdge &edge) {
  return interact_pe(poly, edge);
}
inline bool interact(const db::DSimplePolygon &poly, const db::DEdge &edge) {
  return interact_pe(poly, edge);
}
inline bool interact(const db::Polygon &poly1, const db::Polygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::SimplePolygon &poly1, const db::Polygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::Polygon &poly1, const db::SimplePolygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::SimplePolygon &poly1,
                     const db::SimplePolygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::DPolygon &poly1, const db::DPolygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::DSimplePolygon &poly1,
                     const db::DPolygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::DPolygon &poly1,
                     const db::DSimplePolygon &poly2) {
  return interact_pp(poly1, poly2);
}
inline bool interact(const db::DSimplePolygon &poly1,
                     const db::DSimplePolygon &poly2) {
...
inline bool interact(const db::Polygon &poly, const db::Text &text) {
  return interact_pt(poly, text);
}
inline bool interact(const db::SimplePolygon &poly, const db::Text &text) {
  return interact_pt(poly, text);
}
inline bool interact(const db::DPolygon &poly, const db::DText &text) {
  return interact_pt(poly, text);
}
inline bool interact(const db::DSimplePolygon &poly, const db::DText &text) {
  return interact_pt(poly, text);
}
 ...
 *
 *  This method will determine the radius of a contour if the contour was formed
 * by rounding another contour. The corners must be formed by soft bending
 * edges. It is possible to retrieve the original contour (or an approximation
 * of the latter) by passing a vector in "new_pts" which will receive the
 * original contour.
 *
 *  @param from, to The iterators describing the contour
 *  @param rinner The inner corner radius (in dbu units) extracted (if return
 * value is true)
 *  @param router The outer corner radius (in dbu units) extracted (if return
 * value is true)
 *  @param n Receives the number of points per full circle (if return value is
 * true)
 *  @param new_pts If != 0, this vector will receive the contour without the
 * rounded corners (if return value is true)
 *  @param fallback Fallback algorithm (less strict) if true
 *  @return True, if the extraction was successful
 */
bool DB_PUBLIC extract_rad_from_contour(
    db::Polygon::polygon_contour_iterator from,
    db::Polygon::polygon_contour_iterator to, double &rinner, double &router,
    unsigned int &n, std::vector<db::Point> *new_pts = 0,
    bool fallback = false);

/**
 *  @brief Extract a corner radius from a contour (version for double
 * coordinates)
 */
bool DB_PUBLIC extract_rad_from_contour(
    db::DPolygon::polygon_contour_iterator from,
    db::DPolygon::polygon_contour_iterator to, double &rinner, double &router,
    unsigned int &n, std::vector<db::DPoint> *new_pts = 0,
    bool fallback = false);

/**
 *  @brief Extract the radius (better: radii) from a polygon and if requested,
 * compute the new polygon without the rounding
 *
 *  See extract_rad_from_contour for details.
 */
bool DB_PUBLIC extract_rad(const db::Polygon &polygon, double &rinner,
                           double &router, unsigned int &n,
                           db::Polygon *new_polygon = 0);

/**
 *  @brief Extract a corner radius from a polygon (version for double
 * coordinates)
 */
bool DB_PUBLIC extract_rad(const db::DPolygon &polygon, double &rinner,
                           double &router, unsigned int &n,
                           db::DPolygon *new_polygon = 0);

/**
 *  @brief Compute the rounded version of a polygon contour
 *
 *  Computes the version of a contour with the corners rounded (inner corners
 * with rinner, outer corners with router, n points per full circle=.
 *
 *  @param from, to The iterators describing the contour
 ...
 */
void DB_PUBLIC compute_rounded_contour(
    db::Polygon::polygon_contour_iterator from,
    db::Polygon::polygon_contour_iterator to, std::vector<db::Point> &new_pts,
    double rinner, double router, unsigned int n);

/**
 *  @brief Compute the rounded version of a polygon contour (double coordinate
 * version)
 */
void DB_PUBLIC compute_rounded_contour(
    db::DPolygon::polygon_contour_iterator from,
    db::DPolygon::polygon_contour_iterator to, std::vector<db::DPoint> &new_pts,
    double rinner, double router, unsigned int n);
 ...
 */
db::Polygon DB_PUBLIC compute_rounded(const db::Polygon &poly, double rinner,
                                      double router, unsigned int n);

/**
 *  @brief Compute the rounded version of the polygon (double coordinate
 * version)
 */
db::DPolygon DB_PUBLIC compute_rounded(const db::DPolygon &poly, double rinner,
                                       double router, unsigned int n);
 ...
 *
 *  Removes vertexes from a contour which deviate from the "average" line by
 * more than "d".
 *
 ...
 */
void DB_PUBLIC smooth_contour(db::Polygon::polygon_contour_iterator from,
                              db::Polygon::polygon_contour_iterator to,
                              std::vector<db::Point> &new_pts, db::Coord d,
                              bool keep_hv);
...
/**
 *  @brief Returns a value indicating whether the polygon is an "strange
 * polygon" "strange polygons" are ones which are non-orientable or have
 * self-overlaps, e.g. their wrap count after orientation normalization is not 0
 * or 1. If "error_parts" is given it will receive markers indicating the parts
 * which violate this wrap count condition.
 */
bool DB_PUBLIC is_strange_polygon(const db::Polygon &poly,
                                  std::vector<db::Polygon> *error_parts = 0);
 ...
 *  If "error_parts" is given it will receive markers indicating the parts which
 * are non-orientable.
 */
bool DB_PUBLIC is_non_orientable_polygon(
    const db::Polygon &poly, std::vector<db::Polygon> *error_parts = 0);
...
public:
   ...
   */
  AreaMap(const db::Point &p0, const db::Vector &d, const db::Vector &p,
          size_t nx, size_t ny);
   ...
   *  @brief Reinitialize
   */
  void reinitialize(const db::Point &p0, const db::Vector &d, size_t nx,
                    size_t ny);

  /**
   *  @brief Reinitialize with pixel size
   */
  void reinitialize(const db::Point &p0, const db::Vector &d,
   ...
   */
  area_type &get(size_t x, size_t y) { return mp_av[y * m_nx + x]; }

  /**
   *  @brief Get the area of one pixel (const version)
   */
  const area_type &get(size_t x, size_t y) const { return mp_av[y * m_nx + x]; }
   ...
   *  @brief The origin
   */
  const db::Point &p0() const { return m_p0; }

  /**
   *  @brief Move the origin
   */
  void move(const db::Vector &d) { m_p0 += d; }

  /**
   *  @brief The per-pixel displacement vector (pixel size)
   */
  const db::Vector &d() const { return m_d; }

  /**
   *  @brief The pixel size (must be less than d)
   */
  const db::Vector &p() const { return m_p; }
 ...
 *
 *  This will decompose the polygon and produce per-pixel area values for the
 * given polygon. The area contributions will be added to the given area map.
 *
 ...
 */
db::Polygon DB_PUBLIC minkowski_sum(const db::Polygon &a, const db::Edge &b,
                                    bool resolve_holes = false);

/**
 *  @brief Minkowski sum of a polygon and a polygon
 */
db::Polygon DB_PUBLIC minkowski_sum(const db::Polygon &a, const db::Polygon &b,
                                    bool resolve_holes = false);

/**
 *  @brief Minkowski sum of a polygon and a box
 */
db::Polygon DB_PUBLIC minkowski_sum(const db::Polygon &a, const db::Box &b,
                                    bool resolve_holes = false);

/**
 *  @brief Minkowski sum of a polygon and a contour
 */
db::Polygon DB_PUBLIC minkowski_sum(const db::Polygon &a,
 ...
 *
 *  The resulting polygons will be sent to the sink. Only "put" events will be
 * generated to facilitate call chaining of multiple "decompose_convex" calls.
 */
void DB_PUBLIC decompose_convex(const db::Polygon &p, PreferredOrientation po,
 ...
 */
void DB_PUBLIC decompose_convex(const db::SimplePolygon &p,
                                PreferredOrientation po,
                                SimplePolygonSink &sink);
 ...
 *
 *  @param horizontal If true, delivers htrapezoid objects, otherwise
 * vtrapezoids
 *
 *  The resulting single polygons will be sent to the sink. Only "put" events
 * will be generated on the sink.
 */
void DB_PUBLIC decompose_trapezoids(const db::Polygon &p,
 ...
 *  See the "Polygon" version of this function for details.
 */
void DB_PUBLIC decompose_trapezoids(const db::SimplePolygon &p,
                                    TrapezoidDecompositionMode mode,
                                    SimplePolygonSink &sink);

template <class C> static inline C snap_to_grid(C c, C g) {
 ...
 */
DB_PUBLIC db::Polygon snapped_polygon(const db::Polygon &poly, db::Coord gx,
                                      db::Coord gy,
 ...
 */
DB_PUBLIC db::Polygon scaled_and_snapped_polygon(const db::Polygon &poly,
                                                 db::Coord gx, db::Coord mx,
                                                 db::Coord dx, db::Coord ox,
                                                 db::Coord gy, db::Coord my,
                                                 db::Coord dy, db::Coord oy,
                                                 std::vector<db::Point> &heap);
 ...
 *  The coordinate transformation is q = ((p * m + o) snap (g * d)) / d.
 */
DB_PUBLIC db::Vector scaled_and_snapped_vector(const db::Vector &v,
                                               db::Coord gx, db::Coord mx,
                                               db::Coord dx, db::Coord ox,
                                               db::Coord gy, db::Coord my,
                                               db::Coord dy, db::Coord oy);
