...
NetlistDeviceExtractorMOS3Transistor::NetlistDeviceExtractorMOS3Transistor(
    const std::string &name, bool strict, db::DeviceClassFactory *factory)
    : db::NetlistDeviceExtractorImplBase(
          name,
          factory ? factory
                  : new db::device_class_factory<DeviceClassMOS3Transistor>()),
      m_strict(strict) {
  //  .. nothing yet ..
}
...
db::Connectivity NetlistDeviceExtractorMOS3Transistor::get_connectivity(
...
void NetlistDeviceExtractorMOS3Transistor::extract_devices(
  ...
  if (!is_strict()) {
    ...
    for (db::Region::const_iterator p = rgates.begin_merged(); !p.at_end();
         ++p) {
      ...
      if (rdiff2gate.empty()) {
        error(tl::to_string(tr("Gate shape touches no diffusion - ignored")),
              *p);
      } else {
        ...
        if (rdiff2gate.count() != 2) {
          error(tl::sprintf(
                    tl::to_string(
                        tr("Expected two polygons on diff interacting with one "
                           "gate shape (found %d) - gate shape ignored")),
                    int(rdiff2gate.count())),
                *p);
          continue;
        }

        //  normalize the diffusion polygons so that the S/D assignment is more
        //  predicable
        std::vector<db::Polygon> diffpoly;
        diffpoly.reserve(2);
        for (db::Region::const_iterator d2g = rdiff2gate.begin(); !d2g.at_end();
             ++d2g) {
        ...
        for (std::vector<db::Polygon>::const_iterator d2g = diffpoly.begin();
             d2g != diffpoly.end(); ++d2g) {

          db::Edges edges(rgate.edges() & db::Edges(*d2g));
          db::Edges::length_type l = edges.length();
          if (l == 0) {
            error(
                tl::to_string(tr("Vanishing edges for interaction gate/diff "
                                 "(corner interaction) - gate shape ignored")));
          } else {
            widths.push_back(l);
          }
        }
        ...
        //  Computation of the gate length and width - this scheme is compatible
        //  with non-rectangular gates and circular gates. The computation is
        //  based on the relationship: A(gate) = L(gate) * W(gate). W(gate) is
        //  determined from the accumulated edge lengths (average of left and
        //  right length).
        ...
        device->set_trans(
            db::DCplxTrans((p->box().center() - db::Point()) * dbu()));

        device->set_parameter_value(db::DeviceClassMOS3Transistor::param_id_W,
                                    param_w);
        device->set_parameter_value(db::DeviceClassMOS3Transistor::param_id_L,
                                    param_l);

        int diff_index = 0;
        for (std::vector<db::Polygon>::const_iterator d2g = diffpoly.begin();
             d2g != diffpoly.end() && diff_index < 2; ++d2g, ++diff_index) {

          //  count the number of gate shapes attached to this shape and
          //  distribute the area of the diffusion region to the number of gates
          size_t n = rgates.selected_interacting(db::Region(*d2g)).count();
          tl_assert(n > 0);

          device->set_parameter_value(
              diff_index == 0 ? db::DeviceClassMOS3Transistor::param_id_AS
                              : db::DeviceClassMOS3Transistor::param_id_AD,
              sdbu() * sdbu() * d2g->area() / double(n));
          device->set_parameter_value(
              diff_index == 0 ? db::DeviceClassMOS3Transistor::param_id_PS
                              : db::DeviceClassMOS3Transistor::param_id_PD,
              sdbu() * d2g->perimeter() / double(n));

          unsigned int sd_index = diff_index == 0
                                      ? source_terminal_geometry_index
                                      : drain_terminal_geometry_index;
          define_terminal(device,
                          diff_index == 0
                              ? db::DeviceClassMOS3Transistor::terminal_id_S
                              : db::DeviceClassMOS3Transistor::terminal_id_D,
                          sd_index, *d2g);
        }

        define_terminal(device, db::DeviceClassMOS3Transistor::terminal_id_G,
                        gate_terminal_geometry_index, *p);
    ...
    for (db::Region::const_iterator p = rgates.begin_merged(); !p.at_end();
      ...
      if (sdiff2gate.empty() && ddiff2gate.empty()) {
        ...
        error(tl::to_string(
                  tr("Gate shape touches a single diffusion only - ignored")),
              *p);
      } else {
        ...
        if (sdiff2gate.count() != 1) {
          error(tl::sprintf(
                    tl::to_string(tr(
                        "Expected one polygons on source diff interacting with "
                        "one gate shape (found %d) - gate shape ignored")),
                    int(sdiff2gate.count())),
                *p);
          continue;
        }
        ...
        if (ddiff2gate.count() != 1) {
          error(tl::sprintf(
                    tl::to_string(tr(
                        "Expected one polygons on drain diff interacting with "
                        "one gate shape (found %d) - gate shape ignored")),
                    int(ddiff2gate.count())),
                *p);
          continue;
        }
...

        {
          ...
          if (sdwidth == 0) {
            error(tl::to_string(
                tr("Vanishing edges for interaction gate/source diff (corner "
                   "interaction) - gate shape ignored")));
            continue;
          }
        }

        {
          ...
          if (ddwidth == 0) {
            error(tl::to_string(
                tr("Vanishing edges for interaction gate/drain diff (corner "
                   "interaction) - gate shape ignored")));
            continue;
          }
        }
        ...
        for (int diff_index = 0; diff_index < 2; ++diff_index) {
          ...
          device->set_parameter_value(
              diff_index == 0 ? db::DeviceClassMOS3Transistor::param_id_AS
                              : db::DeviceClassMOS3Transistor::param_id_AD,
              sdbu() * sdbu() * diff->area() / double(n));
          device->set_parameter_value(
              diff_index == 0 ? db::DeviceClassMOS3Transistor::param_id_PS
                              : db::DeviceClassMOS3Transistor::param_id_PD,
              sdbu() * diff->perimeter() / double(n));
...
NetlistDeviceExtractorMOS4Transistor::NetlistDeviceExtractorMOS4Transistor(
    const std::string &name, bool strict, db::DeviceClassFactory *factory)
    : NetlistDeviceExtractorMOS3Transistor(
          name, strict,
          factory
              ? factory
              : new db::device_class_factory<db::DeviceClassMOS4Transistor>()) {
  //  .. nothing yet ..
}
...
void NetlistDeviceExtractorMOS4Transistor::modify_device(
    const db::Polygon &rgate,
    const std::vector<db::Region> & /*layer_geometry*/, db::Device *device) {
  //  see setup() for the layer indexes:
  unsigned int bulk_terminal_geometry_index = is_strict() ? 8 : 7;

  define_terminal(device, db::DeviceClassMOS4Transistor::terminal_id_B,
                  bulk_terminal_geometry_index, rgate);
}

// ---------------------------------------------------------------------------------
//  NetlistDeviceExtractorResistor implementation

NetlistDeviceExtractorResistor::NetlistDeviceExtractorResistor(
    const std::string &name, double sheet_rho, db::DeviceClassFactory *factory)
    : db::NetlistDeviceExtractorImplBase(
          name, factory
                    ? factory
                    : new db::device_class_factory<db::DeviceClassResistor>()),
      m_sheet_rho(sheet_rho) {
  //  .. nothing yet ..
}
...
db::Connectivity NetlistDeviceExtractorResistor::get_connectivity(
    const db::Layout & /*layout*/,
    const std::vector<unsigned int> &layers) const {
...
void NetlistDeviceExtractorResistor::extract_devices(
    const std::vector<db::Region> &layer_geometry) {
  ...
  for (db::Region::const_iterator p = res_merged.begin_merged(); !p.at_end();
       ++p) {
    ...
    if (contacts_per_res.count() != 2) {
      error(tl::sprintf(
                tl::to_string(
                    tr("Expected two polygons on contacts interacting with one "
                       "resistor shape (found %d) - resistor shape ignored")),
                int(contacts_per_res.count())),
            *p);
      continue;
    }
    ...
    //  TODO: this is a very rough approximation for the general case - it
    //  assumes a "good" geometry
    ...
    if (width2 < 1) {
      error(tl::to_string(
                tr("Invalid contact geometry - resistor shape ignored")),
            *p);
      continue;
    }

    device->set_parameter_value(db::DeviceClassResistor::param_id_R,
                                m_sheet_rho * double(length2) / double(width2));
    device->set_parameter_value(db::DeviceClassResistor::param_id_L,
                                sdbu() * 0.5 * length2);
    device->set_parameter_value(db::DeviceClassResistor::param_id_W,
                                sdbu() * 0.5 * width2);
    device->set_parameter_value(db::DeviceClassResistor::param_id_A,
                                sdbu() * sdbu() * p->area());
    device->set_parameter_value(db::DeviceClassResistor::param_id_P,
                                sdbu() * p->perimeter());

    //  collect and normalize the contact polygons (gives better
    //  reproducibility)
    std::vector<db::Polygon> contact_poly;
    contact_poly.reserve(2);
    for (db::Region::const_iterator d = contacts_per_res.begin(); !d.at_end();
         ++d) {
    ...
    int cont_index = 0;
    for (std::vector<db::Polygon>::const_iterator d = contact_poly.begin();
         d != contact_poly.end() && cont_index < 2; ++d, ++cont_index) {
      size_t terminal_geometry_index = cont_index == 0
                                           ? a_terminal_geometry_index
                                           : b_terminal_geometry_index;
      define_terminal(device,
                      cont_index == 0 ? db::DeviceClassResistor::terminal_id_A
                                      : db::DeviceClassResistor::terminal_id_B,
                      terminal_geometry_index, *d);
    }
...
NetlistDeviceExtractorResistorWithBulk::NetlistDeviceExtractorResistorWithBulk(
    const std::string &name, double sheet_rho, db::DeviceClassFactory *factory)
    : NetlistDeviceExtractorResistor(
          name, sheet_rho,
          factory ? factory
                  : new db::device_class_factory<
                        db::DeviceClassResistorWithBulk>()) {
  //  .. nothing yet ..
}
...
void NetlistDeviceExtractorResistorWithBulk::modify_device(
    const db::Polygon &res, const std::vector<db::Region> & /*layer_geometry*/,
    db::Device *device) {
  unsigned int bulk_terminal_geometry_index = 5;
  define_terminal(device, db::DeviceClassResistorWithBulk::terminal_id_W,
                  bulk_terminal_geometry_index, res);
}

// ---------------------------------------------------------------------------------
//  NetlistDeviceExtractorCapacitor implementation

NetlistDeviceExtractorCapacitor::NetlistDeviceExtractorCapacitor(
    const std::string &name, double area_cap, db::DeviceClassFactory *factory)
    : db::NetlistDeviceExtractorImplBase(
          name, factory
                    ? factory
                    : new db::device_class_factory<db::DeviceClassCapacitor>()),
      m_area_cap(area_cap) {
  //  .. nothing yet ..
}
...
db::Connectivity NetlistDeviceExtractorCapacitor::get_connectivity(
  ...
  //  connect the plates (NOTE that this is a logical, not a physical
  //  connection)
  conn.connect(plate1, plate2);
  return conn;
}

void NetlistDeviceExtractorCapacitor::extract_devices(
    const std::vector<db::Region> &layer_geometry) {
  ...
  for (db::Region::const_iterator p = overlap.begin_merged(); !p.at_end();
       ++p) {
    ...
    device->set_parameter_value(db::DeviceClassCapacitor::param_id_C,
                                m_area_cap * area);
    device->set_parameter_value(db::DeviceClassCapacitor::param_id_A, area);
    device->set_parameter_value(db::DeviceClassCapacitor::param_id_P,
                                sdbu() * p->perimeter());

    define_terminal(device, db::DeviceClassCapacitor::terminal_id_A,
                    a_terminal_geometry_index, *p);
    define_terminal(device, db::DeviceClassCapacitor::terminal_id_B,
                    b_terminal_geometry_index, *p);
...
NetlistDeviceExtractorCapacitorWithBulk::
    NetlistDeviceExtractorCapacitorWithBulk(const std::string &name,
                                            double area_cap,
                                            db::DeviceClassFactory *factory)
    : NetlistDeviceExtractorCapacitor(
          name, area_cap,
          factory ? factory
                  : new db::device_class_factory<
                        db::DeviceClassCapacitorWithBulk>()) {
  //  .. nothing yet ..
}
...
void NetlistDeviceExtractorCapacitorWithBulk::modify_device(
    const db::Polygon &cap, const std::vector<db::Region> & /*layer_geometry*/,
    db::Device *device) {
  unsigned int bulk_terminal_geometry_index = 5;
  define_terminal(device, db::DeviceClassCapacitorWithBulk::terminal_id_W,
                  bulk_terminal_geometry_index, cap);
}

// ---------------------------------------------------------------------------------
//  NetlistDeviceExtractorBJT3Transistor implementation

NetlistDeviceExtractorBJT3Transistor::NetlistDeviceExtractorBJT3Transistor(
    const std::string &name, db::DeviceClassFactory *factory)
    : db::NetlistDeviceExtractorImplBase(
          name,
          factory
              ? factory
              : new db::device_class_factory<db::DeviceClassBJT3Transistor>()) {
  //  .. nothing yet ..
}
...
db::Connectivity NetlistDeviceExtractorBJT3Transistor::get_connectivity(
    const db::Layout & /*layout*/,
    const std::vector<unsigned int> &layers) const {
  tl_assert(layers.size() >= 3);
...
void NetlistDeviceExtractorBJT3Transistor::extract_devices(
    const std::vector<db::Region> &layer_geometry) {
  ...
  for (db::Region::const_iterator p = rbases.begin_merged(); !p.at_end(); ++p) {
    ...
    if (remitter2base.empty()) {
      ...
      if (rcollector2base.empty()) {
        ...
        //  vertical transistor: collector entirely covers base -> collector
        //  terminal is collector outside base
      ...
      //  TODO: rbase - rcollector2base above could basically split a base
      //  region into different subregions potentially forming one transistor
      //  each.
      ...
      for (db::Region::const_iterator pe = remitter2base.begin_merged();
           !pe.at_end(); ++pe) {

        db::Device *device = create_device();

        device->set_trans(
            db::DCplxTrans((pe->box().center() - db::Point()) * dbu()));

        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_NE,
                                    1.0);

        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_AE,
                                    sdbu() * sdbu() * pe->area());
        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_PE,
                                    sdbu() * pe->perimeter());

        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_AB,
                                    ab);
        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_PB,
                                    pb);

        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_AC,
                                    ac);
        device->set_parameter_value(db::DeviceClassBJT3Transistor::param_id_PC,
                                    pc);

        define_terminal(device, db::DeviceClassBJT3Transistor::terminal_id_C,
                        collector_terminal_geometry_index, rcollector);
        define_terminal(device, db::DeviceClassBJT3Transistor::terminal_id_B,
                        base_terminal_geometry_index, rbase);
        define_terminal(device, db::DeviceClassBJT3Transistor::terminal_id_E,
                        emitter_terminal_geometry_index, *pe);
...
NetlistDeviceExtractorBJT4Transistor::NetlistDeviceExtractorBJT4Transistor(
    const std::string &name, db::DeviceClassFactory *factory)
    : NetlistDeviceExtractorBJT3Transistor(
          name,
          factory
              ? factory
              : new db::device_class_factory<db::DeviceClassBJT4Transistor>()) {
  //  .. nothing yet ..
}
...
void NetlistDeviceExtractorBJT4Transistor::modify_device(
    const db::Polygon &emitter,
    const std::vector<db::Region> & /*layer_geometry*/, db::Device *device) {
  unsigned int substrate_terminal_geometry_index = 7;
  define_terminal(device, db::DeviceClassBJT4Transistor::terminal_id_S,
                  substrate_terminal_geometry_index, emitter);
}

// ---------------------------------------------------------------------------------
//  NetlistDeviceExtractorDiode implementation

NetlistDeviceExtractorDiode::NetlistDeviceExtractorDiode(
    const std::string &name, db::DeviceClassFactory *factory)
    : db::NetlistDeviceExtractorImplBase(
          name, factory
                    ? factory
                    : new db::device_class_factory<db::DeviceClassDiode>()) {
  //  .. nothing yet ..
}
...
db::Connectivity NetlistDeviceExtractorDiode::get_connectivity(
    const db::Layout & /*layout*/,
    const std::vector<unsigned int> &layers) const {
  tl_assert(layers.size() >= 2);
...
void NetlistDeviceExtractorDiode::extract_devices(
    const std::vector<db::Region> &layer_geometry) {
  ...
  for (db::Region::const_iterator p = overlap.begin_merged(); !p.at_end();
    ...
    device->set_parameter_value(db::DeviceClassDiode::param_id_P,
                                sdbu() * p->perimeter());

    define_terminal(device, db::DeviceClassDiode::terminal_id_A,
                    a_terminal_geometry_index, *p);
    define_terminal(device, db::DeviceClassDiode::terminal_id_C,
                    c_terminal_geometry_index, *p);
