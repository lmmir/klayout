...
AsIfFlatTexts::AsIfFlatTexts() : TextsDelegate(), m_bbox_valid(false) {
  //  .. nothing yet ..
}

AsIfFlatTexts::AsIfFlatTexts(const AsIfFlatTexts &other)
    : TextsDelegate(other), m_bbox_valid(other.m_bbox_valid),
      m_bbox(other.m_bbox) {
  //  .. nothing yet ..
}
...
AsIfFlatTexts &AsIfFlatTexts::operator=(const AsIfFlatTexts &other) {
  if (this != &other) {
...
std::string AsIfFlatTexts::to_string(size_t nmax) const {
...
TextsDelegate *AsIfFlatTexts::in(const Texts &other, bool invert) const {
  std::set<db::Text> op;
...
size_t AsIfFlatTexts::count() const {
  size_t n = 0;
...
size_t AsIfFlatTexts::hier_count() const { return count(); }
...
void AsIfFlatTexts::invalidate_bbox() { m_bbox_valid = false; }

TextsDelegate *AsIfFlatTexts::filtered(const TextFilterBase &filter) const {
  std::unique_ptr<FlatTexts> new_texts(new FlatTexts());
...
RegionDelegate *AsIfFlatTexts::processed_to_polygons(
...
RegionDelegate *AsIfFlatTexts::polygons(db::Coord e) const {
...
EdgesDelegate *AsIfFlatTexts::edges() const {
...
TextsDelegate *AsIfFlatTexts::add(const Texts &other) const {
  const FlatTexts *other_flat =
      dynamic_cast<const FlatTexts *>(other.delegate());
  if (other_flat) {
...
void AsIfFlatTexts::insert_into(Layout *layout, db::cell_index_type into_cell,
...
void AsIfFlatTexts::insert_into_as_polygons(Layout *layout,
...
TextsDelegate *AsIfFlatTexts::selected_interacting_generic(const Region &other,
                                                           bool inverse) const {
  //  shortcuts
  ...
  if (!inverse) {
    ...
    scanner.process(filter, 1, db::box_convert<db::Text>(),
                    db::box_convert<db::Polygon>());

  } else {

    std::set<db::Text> interacting;
    text_to_region_interaction_filter<std::set<db::Text>, db::Text> filter(
        interacting);
    scanner.process(filter, 1, db::box_convert<db::Text>(),
                    db::box_convert<db::Polygon>());
...
RegionDelegate *AsIfFlatTexts::pull_generic(const Region &other) const {
  //  shortcuts
  if (other.empty() || empty()) {
...
RegionDelegate *AsIfFlatTexts::pull_interacting(const Region &other) const {
  return pull_generic(other);
}

TextsDelegate *AsIfFlatTexts::selected_interacting(const Region &other) const {
