...
void read_param_card(tl::Extractor &ex, const db::Netlist *netlist,
                     std::map<std::string, tl::Variant> &variables) {
  //  Syntax is:
...
SpiceReaderStream::SpiceReaderStream()
    : mp_stream(0), m_owns_stream(false), mp_text_stream(0), m_line_number(0),
      m_stored_line(), m_has_stored_line(false) {
  //  .. nothing yet ..
}

SpiceReaderStream::SpiceReaderStream(const std::string &lib)
    : mp_stream(0), m_owns_stream(false), mp_text_stream(0), m_line_number(0),
      m_stored_line(), m_has_stored_line(false), m_lib(lib) {
  //  .. nothing yet ..
}

SpiceReaderStream::~SpiceReaderStream() { close(); }
...
std::pair<std::string, bool> SpiceReaderStream::get_line() {
  if (at_end()) {
  ...
  std::string l =
      m_has_stored_line ? m_stored_line : mp_text_stream->get_line();
...
int SpiceReaderStream::line_number() const { return m_line_number; }

std::string SpiceReaderStream::source() const { return mp_stream->source(); }
...
public:
  ...
  void set_anonymous(bool f) { m_anonymous = f; }

  const std::string &name() const { return m_name; }

  void set_parameters(const parameters_type &pv) { m_parameters = pv; }
  ...
  cards_iterator begin_cards() const { return m_cards.begin(); }

  cards_iterator end_cards() const { return m_cards.end(); }

  void add_card(const SpiceCard &card) { m_cards.push_back(card); }

  size_t pin_count() const { return m_pins.size(); }

  pin_const_iterator begin_pins() const { return m_pins.begin(); }

  pin_const_iterator end_pins() const { return m_pins.end(); }

  void set_pins(const pin_list_type &pins) { m_pins = pins; }

  void set_pins(pin_list_type &&pins) { m_pins = std::move(pins); }
...
static std::string read_name(tl::Extractor &ex, const db::Netlist *netlist) {
  std::string n;
...
public:
  ...
  SpiceCircuitDict(NetlistSpiceReader *reader, Netlist *netlist,
  ...
  circuits_iterator begin_circuits() const { return m_cached_circuits.begin(); }

  circuits_iterator end_circuits() const { return m_cached_circuits.end(); }
...
private:
  ...
  void push_stream(const std::string &path,
                   const std::string &lib = std::string());
  void pop_stream();
  bool at_end();
  void read_subcircuit(const std::string &sc_name, const std::string &nc_name,
...
SpiceCircuitDict::SpiceCircuitDict(NetlistSpiceReader *reader, Netlist *netlist,
                                   NetlistSpiceReaderDelegate *delegate)
...
const std::string &SpiceCircuitDict::file_path(int file_id) const {
...
void SpiceCircuitDict::read(tl::InputStream &stream) {
  try {
    ...
    std::string fmt_msg =
        ex.msg() + tl::sprintf(tl::to_string(tr(" in %s, line %d")),
                               m_stream.source(), m_stream.line_number());
    throw tl::Exception(fmt_msg);
  }
}

void SpiceCircuitDict::push_stream(const std::string &path,
  ...
  if (current_uri.scheme().empty() && new_uri.scheme().empty()) {
    if (tl::is_absolute(path)) {
      ...
      istream = new tl::InputStream(
          tl::combine_path(tl::dirname(m_stream.source()), path));
    }
  } else {
    istream =
        new tl::InputStream(current_uri.resolved(new_uri).to_abstract_path());
  }
...
void SpiceCircuitDict::warn(const std::string &msg) {
  std::string fmt_msg = tl::sprintf("%s in %s, line %d", msg, m_stream.source(),
                                    m_stream.line_number());
  tl::warn << fmt_msg;
}

std::string SpiceCircuitDict::get_line() {
  ...
  while (true) {
    ...
    if (!lp.second) {
      ...
      bool consider_line =
          m_in_lib.empty() ||
          (!m_stream.lib().empty() && m_stream.lib() == m_in_lib.back());
...
bool SpiceCircuitDict::read_card() {
  ...
  if (ex.test_without_case(".model")) {
    ...
    while (!ex.at_end()) {
      std::string n =
          mp_delegate->translate_net_name(read_name(ex, mp_netlist));
...
public:
  ...
  SpiceNetlistBuilder(SpiceCircuitDict *dict, Netlist *netlist,
                      NetlistSpiceReaderDelegate *delegate);
...
private:
  ...
  std::map<const SpiceCachedCircuit *, std::map<parameters_type, db::Circuit *>>
      m_circuits;
  ...
  db::Circuit *circuit_for(const SpiceCachedCircuit *cached_circuit,
                           const parameters_type &pv);
  void register_circuit_for(const SpiceCachedCircuit *cc,
                            const parameters_type &pv, db::Circuit *circuit,
                            bool anonymous_top_level);
  Circuit *build_circuit(const SpiceCachedCircuit *circuit,
                         const parameters_type &pv,
                         bool anonymous_top_level = false);
  std::string get_line();
  void error(const std::string &msg);
  void warn(const std::string &msg);
  ...
  bool process_element(tl::Extractor &ex, const std::string &prefix,
                       const std::string &name);
  void build_global_nets();
};

SpiceNetlistBuilder::SpiceNetlistBuilder(SpiceCircuitDict *dict,
                                         Netlist *netlist,
                                         NetlistSpiceReaderDelegate *delegate)
...
void SpiceNetlistBuilder::warn(const std::string &msg) {
  if (mp_current_card) {
    std::string fmt_msg = tl::sprintf(
        "%s in %s, line %d", msg, mp_dict->file_path(mp_current_card->file_id),
        mp_current_card->line);
    tl::warn << fmt_msg;
  } else {
    tl::warn << msg;
  }
...
db::Circuit *SpiceNetlistBuilder::circuit_for(const SpiceCachedCircuit *cc,
                                              const parameters_type &pv) {
...
void SpiceNetlistBuilder::register_circuit_for(const SpiceCachedCircuit *cc,
                                               const parameters_type &pv,
                                               db::Circuit *circuit,
                                               bool anonymous_top_level) {
...
void SpiceNetlistBuilder::build() {
  try {
    ...
    for (auto c = mp_dict->begin_circuits(); c != mp_dict->end_circuits();
         ++c) {
      if (mp_dict->is_top_circuit(c->first) && !subcircuit_captured(c->first)) {
        ...
        build_circuit(c->second, c->second->parameters(),
                      c->second == mp_dict->anonymous_top_level_circuit());
      }
    }
    ...
    if (mp_current_card) {
      std::string fmt_msg =
          ex.msg() + tl::sprintf(tl::to_string(tr(" in %s, line %d")),
                                 mp_dict->file_path(mp_current_card->file_id),
                                 mp_current_card->line);
      throw tl::Exception(fmt_msg);
    } else {
      throw;
...
static std::string
make_circuit_name(const std::string &name,
                  const NetlistSpiceReader::parameters_type &pv) {
  std::string res = name;
...
db::Circuit *SpiceNetlistBuilder::build_circuit(const SpiceCachedCircuit *cc,
                                                const parameters_type &pv,
                                                bool anonymous_top_level) {
  ...
  std::unique_ptr<std::map<std::string, db::Net *>> n2n(
      mp_nets_by_name.release());
  ...
  for (auto i = mp_circuit->begin_pins(); i != mp_circuit->end_pins(); ++i) {
    std::string net_name =
        mp_delegate->translate_net_name(mp_netlist->normalize_name(*i));
    db::Net *net = make_net(net_name);
    //  use the net name to name the pin (otherwise SPICE pins are always
    //  unnamed)
  ...
  for (auto card = mp_circuit->begin_cards(); card != mp_circuit->end_cards();
       ++card) {
...
db::Net *SpiceNetlistBuilder::make_net(const std::string &name) {
  if (!mp_nets_by_name.get()) {
    mp_nets_by_name.reset(new std::map<std::string, db::Net *>());
  }

  std::map<std::string, db::Net *>::const_iterator n2n =
      mp_nets_by_name->find(name);
...
void SpiceNetlistBuilder::process_card(const SpiceCard &card) {
  ...
  if (ex.try_read_word(name) && ex.test("=")) {
    ...
    m_variables.insert(std::make_pair(
        mp_netlist->normalize_name(name),
        NetlistSpiceReaderDelegate::read_value(ex, m_variables)));
    ...
    if (ex.test_without_case(".param")) {
      ...
      if (!process_element(ex, prefix, name)) {
        warn(tl::sprintf(tl::to_string(tr("Element type '%s' ignored")),
                         prefix));
      }
...
bool SpiceNetlistBuilder::process_element(tl::Extractor &ex,
  ...
  for (std::vector<std::string>::const_iterator i = nn.begin(); i != nn.end();
       ++i) {
  ...
  if (prefix == "X" && !subcircuit_captured(model)) {
    ...
    if (!cc) {
      if (m_strict) {
        error(tl::sprintf(
            tl::to_string(tr("Subcircuit '%s' not found in netlist")), model));
      } else {
    ...
    if (!cc->is_anonymous()) {
      ...
      for (auto p = pv.begin(); p != pv.end(); ++p) {
        if (cc->parameters().find(p->first) == cc->parameters().end()) {
          warn(tl::sprintf(
              tl::to_string(tr("Not a known parameter for circuit '%s': '%s'")),
              cc->name(), p->first));
        }
      }
    }
    ...
    if (cc->pin_count() != nn.size()) {
      error(tl::sprintf(
          tl::to_string(tr("Pin count mismatch between circuit definition and "
                           "circuit call: %d expected, got %d")),
          int(cc->pin_count()), int(nets.size())));
    }
    ...
    for (std::vector<db::Net *>::const_iterator i = nets.begin();
         i != nets.end(); ++i) {
    ...
    return mp_delegate->element(mp_netlist_circuit, prefix, name, model, value,
                                nets, pv);
  }
}

void SpiceNetlistBuilder::build_global_nets() {
  for (auto gn = mp_dict->begin_global_nets(); gn != mp_dict->end_global_nets();
       ++gn) {

    for (auto c = mp_netlist->begin_bottom_up();
         c != mp_netlist->end_bottom_up(); ++c) {
      ...
      for (db::Circuit::refs_iterator r = c->begin_refs(); r != c->end_refs();
           ++r) {
...
void NetlistSpiceReader::read(tl::InputStream &stream, db::Netlist &netlist) {
  tl::SelfTimer timer(tl::verbosity() >= 21,
                      tl::to_string(tr("Reading netlist ")) + stream.source());
