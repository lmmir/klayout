...
void NamedLayerReader::set_create_layers(bool f) { m_create_layers = f; }

void NamedLayerReader::set_keep_layer_names(bool f) { m_keep_layer_names = f; }

void NamedLayerReader::set_layer_map(const LayerMap &lm) { m_layer_map = lm; }

static bool extract_plain_layer(const char *s, int &l) {
  l = 0;
  if (!*s) {
    return false;
  }
...
static bool extract_ld(const char *s, int &l, int &d, std::string &n) {
  l = d = 0;
...
std::pair<bool, unsigned int> NamedLayerReader::open_layer(db::Layout &layout,
                                                           const std::string &n,
                                                           bool keep_layer_name,
                                                           bool create_layer) {
  std::map<std::string, std::pair<bool, unsigned int>>::const_iterator lc =
      m_layer_cache.find(n);
  if (lc != m_layer_cache.end()) {
    return lc->second;
  } else {
    std::pair<bool, unsigned int> res =
        open_layer_uncached(layout, n, keep_layer_name, create_layer);
...
NamedLayerReader::open_layer_uncached(db::Layout &layout, const std::string &n,
                                      bool keep_layer_name, bool create_layer) {
  int l = -1, d = -1;
  std::string on;
  ...
  if (!li.empty()) {
    ...
    std::map<std::string, unsigned int>::const_iterator nl =
        m_new_layers.find(n);
...
void NamedLayerReader::finish_layers(db::Layout &layout) {
  ...
  if (!m_new_layers.empty() && !m_keep_layer_names) {
    ...
    //  assign fixed layer numbers for all layers whose name is a fixed number
    //  unless there is already a layer with that number
    for (std::map<std::string, unsigned int>::iterator i = m_new_layers.begin();
         i != m_new_layers.end();) {
      ...
      int l = -1;
      if (extract_plain_layer(i->first.c_str(), l) &&
          used_ld.find(std::make_pair(l, 0)) == used_ld.end()) {
    ...
    //  assign fixed layer numbers for all layers whose name is a LxDy or Lx
    //  notation unless there is already a layer with that layer/datatype
    for (std::map<std::string, unsigned int>::iterator i = m_new_layers.begin();
         i != m_new_layers.end();) {
      ...
      if (extract_ld(i->first.c_str(), l, d, n) &&
          used_ld.find(std::make_pair(l, d)) == used_ld.end()) {
  ...
  for (std::map<std::string, unsigned int>::const_iterator i =
           m_new_layers.begin();
       i != m_new_layers.end(); ++i) {
    db::LayerProperties lp;
    lp.name = i->first;
    layout.set_properties(i->second, lp);
    m_layer_map.map(lp, i->second);
