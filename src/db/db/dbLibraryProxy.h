 ...
 *
 *  This cell serves as a proxy (or copy) of a cell contained in another
 * library. This cell thus is a reference and a cached copy. The library
 * reference is through the library id and the cell index inside this library.
 */
class DB_PUBLIC LibraryProxy : public Cell {
public:
  /**
   ...
   */
  LibraryProxy(db::cell_index_type ci, db::Layout &layout, lib_id_type lib_id,
   ...
   */
  lib_id_type lib_id() const { return m_lib_id; }

  /**
   *  @brief Get the cell index inside the library
   */
  cell_index_type library_cell_index() const { return m_library_cell_index; }
...
private:
  ...
  std::vector<int> get_layer_indices(db::Layout &layout,
