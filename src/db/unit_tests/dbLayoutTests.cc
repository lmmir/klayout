...
std::string set2string(const std::set<db::cell_index_type> &set) {
  std::string r;
  for (std::set<db::cell_index_type>::const_iterator s = set.begin();
       s != set.end(); ++s) {
    if (s != set.begin()) {
      r += ",";
    }
...
TEST(1) {
  ...
  unsigned int m;
  m = 0;
  for (db::Layout::bottom_up_iterator c = g.begin_bottom_up();
       c != g.end_bottom_up(); ++c) {
    m = (m << 4) + *c;
  }
  ...
  try {
    c5.insert(
        db::array<db::CellInst, db::Trans>(db::CellInst(c2.cell_index()), t));
    g.update();
...
struct EventListener : public tl::Object {
  EventListener()
      : flags(0), bboxes_dirty(false), bboxes_all_dirty(false),
        hier_dirty(false), dbu_dirty(false), cell_name_dirty(false),
        property_ids_dirty(false), layer_properties_dirty(false) {}
...
TEST(2) {
  ...
  EXPECT_EQ(el.bboxes_dirty, false);
  EXPECT_EQ(el.bboxes_all_dirty, false);
  EXPECT_EQ(el.hier_dirty,
            false); //  needs g.update() before being issues again
...
TEST(3) {
  ...
  EXPECT_EQ(el.flags,
            (unsigned int)0);        //  g.update () is missing -> no new events
...
TEST(4) {
  ...
  g.layer_properties_changed_event.add(
      &el, &EventListener::layer_properties_changed);
  ...
  ps.insert(
      std::make_pair(g.properties_repository().prop_name_id(tl::Variant(1)),
                     tl::Variant("XYZ")));
...
TEST(5) {
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("LIB", "A").second,
            lib_a->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib_ptr_by_name("LIB", "A") == lib_a,
            true);
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("LIB", "B").second,
            lib_b->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib_ptr_by_name("LIB", "B") == lib_b,
            true);
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("LIB", "C").second,
            lib_c->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib_ptr_by_name("LIB", "C") == lib_c,
            true);
  ...
  //  now switch to cold proxies again as the technology does not have "LIBCELL"
  //  (but rather LIBCELL2)
...
TEST(6) {
  ...
  EXPECT_EQ(
      l2s(l),
      "begin_lib 0.001\nbegin_cell {CIRCLE}\nboundary 1 0 {-4142 -10000} "
      "{-10000 -4142} {-10000 4142} {-4142 10000} {4142 10000} {10000 4142} "
      "{10000 -4142} {4142 -10000} {-4142 -10000}\nend_cell\nend_lib\n");
  ...
  EXPECT_EQ(l2s(l),
            "begin_lib 0.001\nbegin_cell {CIRCLE}\nboundary 1 0 {-2071 -5000} "
            "{-5000 -2071} {-5000 2071} {-2071 5000} {2071 5000} {5000 2071} "
            "{5000 -2071} {2071 -5000} {-2071 -5000}\nend_cell\nend_lib\n");
  ...
  if (l.is_editable()) {
    ...
    EXPECT_EQ(
        l2s(l),
        "begin_lib 0.001\nbegin_cell {CIRCLE}\nboundary 1 0 {-2071 -5000} "
        "{-5000 -2071} {-5000 2071} {-2071 5000} {2071 5000} {5000 2071} {5000 "
        "-2071} {2071 -5000} {-2071 -5000}\nend_cell\nend_lib\n");
  }
}
