...
static std::string pd2string(const db::DeviceParameterDefinition &pd) {
  return pd.name() + "(" + pd.description() +
         ")=" + tl::to_string(pd.default_value()) + " #" +
         tl::to_string(pd.id());
}

static std::string pins2string(const db::Circuit &c) {
  std::string res;
  for (db::Circuit::const_pin_iterator i = c.begin_pins(); i != c.end_pins();
       ++i) {
    if (!res.empty()) {
      res += ",";
    }
...
static std::string net2string(const db::Net &n) {
  std::string res;
  for (db::Net::const_terminal_iterator i = n.begin_terminals();
       i != n.end_terminals(); ++i) {
    if (!res.empty()) {
      res += ",";
    }
  ...
  for (db::Net::const_subcircuit_pin_iterator i = n.begin_subcircuit_pins();
       i != n.end_subcircuit_pins(); ++i) {
    if (!res.empty()) {
      res += ",";
    }
    res += i->subcircuit()->circuit_ref()
               ? i->subcircuit()->circuit_ref()->name()
               : "(null)";
    res += ":";
    res += i->pin() ? i->pin()->name() : "(null)";
  }
...
static std::string netlist2(const db::Circuit &c) {
  ...
  for (db::Circuit::const_device_iterator d = c.begin_devices();
    ...
    pins.clear();
    for (size_t i = 0; i < d->device_class()->terminal_definitions().size();
         ++i) {
      if (!pins.empty()) {
        pins += ",";
      }
  ...
  for (db::Circuit::const_subcircuit_iterator s = c.begin_subcircuits();
       s != c.end_subcircuits(); ++s) {
    if (!s->circuit_ref()) {
      continue;
    }
    pins.clear();
...
static std::string refs2string(const db::Circuit *c) {
  std::string res;
  for (db::Circuit::const_refs_iterator r = c->begin_refs(); r != c->end_refs();
       ++r) {
    if (!res.empty()) {
      res += ",";
    }
...
static std::string children2string(const db::Circuit *c) {
  std::string res;
  for (db::Circuit::const_child_circuit_iterator r = c->begin_children();
       r != c->end_children(); ++r) {
    if (!res.empty()) {
      res += ",";
    }
...
static std::string parents2string(const db::Circuit *c) {
  std::string res;
  for (db::Circuit::const_parent_circuit_iterator r = c->begin_parents();
       r != c->end_parents(); ++r) {
    if (!res.empty()) {
      res += ",";
    }
...
static std::string td2string(const db::Netlist *nl) {
  std::string res;
  for (db::Netlist::const_top_down_circuit_iterator r = nl->begin_top_down();
       r != nl->end_top_down(); ++r) {
    if (!res.empty()) {
      res += ",";
    }
...
static std::string bu2string(const db::Netlist *nl) {
  std::string res;
  for (db::Netlist::const_bottom_up_circuit_iterator r = nl->begin_bottom_up();
       r != nl->end_bottom_up(); ++r) {
    if (!res.empty()) {
      res += ",";
    }
    res += r->name();
...
TEST(0_DeviceClassTemplates) {
  ...
  EXPECT_EQ(db::DeviceClassTemplateBase::template_by_name("DOESNTEXIST") == 0,
            true);
  EXPECT_EQ(db::DeviceClassTemplateBase::template_by_name("MOS3")->is_of(&mos3),
            true);
  EXPECT_EQ(db::DeviceClassTemplateBase::template_by_name("RES")->is_of(&mos3),
            false);
...
TEST(1_DeviceTerminalDefinition) {
  ...
  EXPECT_EQ(pd2string(dc.terminal_definitions()[1]),
            "name2(now it has something) #1");
...
TEST(2_DeviceClass) {
  ...
  EXPECT_EQ(
      pd2string(*dc.terminal_definition(dc.terminal_definitions()[0].id())),
      "name(nothing yet) #0");
  EXPECT_EQ(
      pd2string(*dc.terminal_definition(dc.terminal_definitions()[1].id())),
      "name2(now it has something) #1");
  ...
  EXPECT_EQ(
      pd2string(*dc2.terminal_definition(dc2.terminal_definitions()[0].id())),
      "name(nothing yet) #0");
  EXPECT_EQ(
      pd2string(*dc2.terminal_definition(dc2.terminal_definitions()[1].id())),
      "name2(now it has something) #1");
...
TEST(3_CircuitBasic) {
  ...
  EXPECT_EQ(c.pin_by_id(0)->begin_properties() ==
                c.pin_by_id(0)->end_properties(),
            true);
...
TEST(4_CircuitDevices) {
  ...
  EXPECT_EQ(nets2string(*c), "d1:S,d2a:A\n"
                             "d1:G,d2a:B,d2b:A\n"
                             "d1:D,d2b:B\n");
  ...
  EXPECT_EQ(nets2string(cc), "d1:S,d2a:A\n"
                             "d1:G,d2a:B,d2b:A\n"
                             "d1:D,d2b:B\n");
...
TEST(4_NetlistSubcircuits) {
  ...
  EXPECT_EQ(nldup->to_string(), "circuit c1 (c1p1=n1a,c1p2=n1c);\n"
                                "  subcircuit c2 sc1 (c2p1=n1a,c2p2=n1b);\n"
                                "  subcircuit c2 sc2 (c2p1=n1b,c2p2=n1c);\n"
                                "end;\n"
                                "circuit c2 (c2p1=n2a,c2p2=n2b);\n"
  ...
  for (db::Netlist::circuit_iterator c = nl->begin_circuits();
       c != nl->end_circuits(); ++c) {
    for (db::Circuit::net_iterator n = c->begin_nets(); n != c->end_nets();
         ++n) {
      for (db::Net::terminal_iterator i = n->begin_terminals();
           i != n->end_terminals(); ++i) {
  ...
  //  check netlist
  for (db::Netlist::circuit_iterator c = nl2.begin_circuits();
       c != nl2.end_circuits(); ++c) {
    for (db::Circuit::net_iterator n = c->begin_nets(); n != c->end_nets();
         ++n) {
      for (db::Net::terminal_iterator i = n->begin_terminals();
           i != n->end_terminals(); ++i) {
...
TEST(9_NetTerminalRefBasics) {
  ...
  EXPECT_NE((db::NetTerminalRef(&d1, 0) < db::NetTerminalRef(&d2, 0)),
            (db::NetTerminalRef(&d2, 0) < db::NetTerminalRef(&d1, 0)));
}

TEST(10_NetPinRefBasics) {
  db::SubCircuit d1, d2;

  EXPECT_EQ(db::NetSubcircuitPinRef(&d1, 0) == db::NetSubcircuitPinRef(&d1, 0),
            true);
  EXPECT_EQ(db::NetSubcircuitPinRef(&d1, 0) == db::NetSubcircuitPinRef(&d1, 1),
            false);
  EXPECT_EQ(db::NetSubcircuitPinRef(&d1, 0) == db::NetSubcircuitPinRef(&d2, 0),
            false);
  ...
  EXPECT_EQ(db::NetSubcircuitPinRef(&d1, 0) < db::NetSubcircuitPinRef(&d1, 1),
            true);
  EXPECT_EQ(db::NetSubcircuitPinRef(&d1, 1) < db::NetSubcircuitPinRef(&d1, 0),
            false);
  EXPECT_NE(
      (db::NetSubcircuitPinRef(&d1, 0) < db::NetSubcircuitPinRef(&d2, 0)),
      (db::NetSubcircuitPinRef(&d2, 0) < db::NetSubcircuitPinRef(&d1, 0)));
}
...
TEST(20_FlattenSubCircuit) {
  ...
  nl.from_string("circuit INV2 (IN=IN,$2=$2,OUT=OUT,$4=$4,$5=$5);\n"
...
TEST(22_FlattenSubCircuitPinsJoinNets) {
  ...
  EXPECT_EQ(
      ...
      "  device PMOS $1 (S=OUT,G=OUT,D=OUT) "
      "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
      "  device PMOS $2 (S=OUT,G=OUT,D=OUT) "
      "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
      "  device NMOS $3 (S=$4,G=OUT,D=OUT) "
      "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
      "  device NMOS $4 (S=$4,G=OUT,D=OUT) "
      "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
      "end;\n");

  nl2.flatten_circuit(nl2.circuit_by_name("INV2"));

  EXPECT_EQ(nl2.to_string(), "circuit RINGO (IN=IN,OSC=OSC,VSS=VSS,VDD=OSC);\n"
                             "  device PMOS $1 (S=OSC,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device PMOS $2 (S=OSC,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device NMOS $3 (S=VSS,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device NMOS $4 (S=VSS,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device PMOS $5 (S=OSC,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device PMOS $6 (S=OSC,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device NMOS $7 (S=VSS,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "  device NMOS $8 (S=VSS,G=OSC,D=OSC) "
                             "(L=0.25,W=0.95,AS=0,AD=0,PS=0,PD=0);\n"
                             "end;\n");
}
...
TEST(23_BlankCircuit) {
  ...
  EXPECT_EQ(nl2.to_string(), "circuit RINGO (IN=IN,OSC=OSC,VSS=VSS,VDD=VDD);\n"
                             "  subcircuit (null);\n"
                             "  subcircuit (null);\n"
                             "end;\n");
