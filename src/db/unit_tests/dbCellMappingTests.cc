...
std::string nc2s(const std::vector<db::cell_index_type> &nc,
                 const db::Layout &a) {
  std::string res;
  for (std::vector<db::cell_index_type>::const_iterator i = nc.begin();
       i != nc.end(); ++i) {
    if (!res.empty()) {
      res += ";";
    }
...
std::string m2s(const db::CellMapping &cm, const db::Layout &a,
                const db::Layout &b) {
  std::string res;
...
std::string m2sr(const db::CellMapping &cm, const db::Layout &a,
                 const db::Layout &b) {
  std::string res;
  for (db::CellMapping::iterator i = cm.begin(); i != cm.end(); ++i) {
    if (!res.empty()) {
...
TEST(1) {
  ...
  a0.insert(db::CellInstArray(db::CellInst(a1.cell_index()), db::Trans(),
                              db::Vector(), db::Vector(), 5, 2));
  a0.insert(db::CellInstArray(db::CellInst(a2.cell_index()), db::Trans()));
  a4.insert(db::CellInstArray(db::CellInst(a2.cell_index()), db::Trans(),
                              db::Vector(), db::Vector(), 3, 4));
  ...
  b0.insert(db::CellInstArray(db::CellInst(b1.cell_index()), db::Trans(),
                              db::Vector(), db::Vector(), 5, 2));
  b0.insert(db::CellInstArray(db::CellInst(b2.cell_index()), db::Trans()));
  b4.insert(db::CellInstArray(db::CellInst(b2.cell_index()), db::Trans(),
                              db::Vector(), db::Vector(), 3, 4));
...
TEST(2) {
  for (int pass = 0; pass < 3; ++pass) {
    ...
    if (pass == 0) {
      a0.insert(db::CellInstArray(db::CellInst(a4.cell_index()),
                                  db::Trans(1 /*r90*/), db::Vector(0, 10),
                                  db::Vector(10, 0), 5, 2));
    } else if (pass == 1) {
      a0.insert(
          db::CellInstArray(db::CellInst(a4.cell_index()),
                            db::ICplxTrans(1.0, 90.0, false, db::Vector(0, 0)),
                            db::Vector(0, 10), db::Vector(10, 0), 5, 2));
    } else if (pass == 2) {
      a0.insert(
          db::CellInstArray(db::CellInst(a4.cell_index()),
                            db::ICplxTrans(0.1, 90.0, false, db::Vector(0, 0)),
                            db::Vector(0, 10), db::Vector(10, 0), 5, 2));
    }
    ...
    if (pass < 2) {
      b0.insert(db::CellInstArray(db::CellInst(b4.cell_index()),
                                  db::Trans(1 /*r90*/), db::Vector(0, 10),
                                  db::Vector(10, 0), 5, 1));
      b0.insert(db::CellInstArray(db::CellInst(b1.cell_index()),
                                  db::Trans(db::Vector(10, 0))));
      b1.insert(db::CellInstArray(db::CellInst(b2.cell_index()), db::Trans()));
      b1.insert(db::CellInstArray(db::CellInst(b2.cell_index()),
                                  db::Trans(db::Vector(0, 20))));
      b1.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 40))));
      b2.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 0))));
      b2.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 10))));
      b3.insert(db::CellInstArray(db::CellInst(b4.cell_index()),
                                  db::Trans(1 /*r90*/)));
    } else {
      b0.insert(
          db::CellInstArray(db::CellInst(b4.cell_index()),
                            db::ICplxTrans(0.1, 90.0, false, db::Vector(0, 0)),
                            db::Vector(0, 10), db::Vector(10, 0), 5, 1));
      b0.insert(db::CellInstArray(
          db::CellInst(b1.cell_index()),
          db::ICplxTrans(0.1, 0.0, false, db::Vector(10, 0))));
      b1.insert(db::CellInstArray(db::CellInst(b2.cell_index()), db::Trans()));
      b1.insert(db::CellInstArray(db::CellInst(b2.cell_index()),
                                  db::Trans(db::Vector(0, 200))));
      b1.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 400))));
      b2.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 0))));
      b2.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                  db::Trans(db::Vector(0, 100))));
      b3.insert(db::CellInstArray(db::CellInst(b4.cell_index()),
                                  db::Trans(1 /*r90*/)));
    }
    ...
    for (int pass = 0; pass < 4; ++pass) {
      ...
      if (pass == 0) {
        a0.insert(db::CellInstArray(db::CellInst(a4.cell_index()),
                                    db::Trans(1 /*r90*/), db::Vector(0, 10),
                                    db::Vector(10, 0), 5, 2));
        a0.insert(db::CellInstArray(db::CellInst(a5.cell_index()),
                                    db::Trans(1 /*r90*/), db::Vector(0, 10),
                                    db::Vector(10, 0), 5, 2));
      } else if (pass == 1) {
        a0.insert(db::CellInstArray(
            db::CellInst(a4.cell_index()),
            db::ICplxTrans(1.0, 90.0, false, db::Vector(0, 0)),
            db::Vector(0, 10), db::Vector(10, 0), 5, 2));
        a0.insert(db::CellInstArray(db::CellInst(a5.cell_index()),
                                    db::Trans(1 /*r90*/), db::Vector(0, 10),
      ...
      if (pass < 2) {
        b0.insert(db::CellInstArray(db::CellInst(b4.cell_index()),
                                    db::Trans(1 /*r90*/), db::Vector(0, 10),
                                    db::Vector(10, 0), 5, 1));
        b0.insert(db::CellInstArray(db::CellInst(b5.cell_index()),
                                    db::Trans(1 /*r90*/), db::Vector(0, 10),
                                    db::Vector(10, 0), 5, 2));
        b0.insert(db::CellInstArray(db::CellInst(b1.cell_index()),
                                    db::Trans(db::Vector(10, 0))));
        b1.insert(
            db::CellInstArray(db::CellInst(b2.cell_index()), db::Trans()));
        b1.insert(db::CellInstArray(db::CellInst(b2.cell_index()),
                                    db::Trans(db::Vector(0, 20))));
        b1.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                                    db::Trans(db::Vector(0, 40))));
        ...
        b0.insert(db::CellInstArray(
            db::CellInst(b4.cell_index()),
            db::ICplxTrans(0.1, 90.0, false, db::Vector(0, 0)),
            db::Vector(0, 10), db::Vector(10, 0), 5, 1));
        b0.insert(db::CellInstArray(
            db::CellInst(b5.cell_index()),
            db::ICplxTrans(0.1, 90.0, false, db::Vector(0, 0)),
            db::Vector(0, 10), db::Vector(10, 0), 5, 2));
        b0.insert(db::CellInstArray(
            db::CellInst(b1.cell_index()),
      ...
      gg = g;
      nc =
          cm.create_from_geometry_full(gg, a0.cell_index(), h, b0.cell_index());
      if (pass < 3) {
...
TEST(4) {
  ...
  b2.insert(db::CellInstArray(db::CellInst(b3.cell_index()),
                              db::Trans(db::Vector(0, 10))));
  b3.insert(
      db::CellInstArray(db::CellInst(b4.cell_index()), db::Trans(1 /*r90*/)));
...
TEST(5) {
  ...
  EXPECT_EQ(l2s(g),
            "a0#0:cell_index=1 r90 0,0 array=(0,10,10,0 5x2),cell_index=2 r90 "
            "0,0 array=(0,10,10,0 5x2);a4#1:;a5#2:");
  EXPECT_EQ(
      l2s(h),
      "a0top#0:cell_index=4 r90 0,0 array=(0,10,10,0 5x1),cell_index=5 r90 0,0 "
      "array=(0,10,10,0 5x2),cell_index=1 r0 10,0;a1#1:cell_index=2 r0 "
      "0,0,cell_index=2 r0 0,20,cell_index=3 r0 0,40;a2#2:cell_index=3 r0 "
      "0,0,cell_index=3 r0 0,10;a3#3:cell_index=4 r90 0,0;a4#4:;a5#5:");
  EXPECT_EQ(l2s(hh),
            "a0top#0:cell_index=4 r90 0,0 array=(0,10,10,0 5x1),cell_index=5 "
            "r90 0,0 array=(0,10,10,0 5x2),cell_index=1 r0 10,0,cell_index=6 "
            "r90 0,0 array=(0,10,10,0 5x2),cell_index=7 r90 0,0 "
            "array=(0,10,10,0 5x2);a1#1:cell_index=2 r0 0,0,cell_index=2 r0 "
            "0,20,cell_index=3 r0 0,40;a2#2:cell_index=3 r0 0,0,cell_index=3 "
            "r0 0,10;a3#3:cell_index=4 r90 0,0;a4#4:;a5#5:;a4$1#6:;a5$1#7:");
  ...
  EXPECT_EQ(l2s(hh),
            "a0top#0:cell_index=4 r90 0,0 array=(0,10,10,0 5x1),cell_index=5 "
            "r90 0,0 array=(0,10,10,0 5x2),cell_index=1 r0 10,0,cell_index=6 "
            "r90 0,0 array=(0,20,20,0 5x2),cell_index=7 r90 0,0 "
            "array=(0,20,20,0 5x2);a1#1:cell_index=2 r0 0,0,cell_index=2 r0 "
            "0,20,cell_index=3 r0 0,40;a2#2:cell_index=3 r0 0,0,cell_index=3 "
            "r0 0,10;a3#3:cell_index=4 r90 0,0;a4#4:;a5#5:;a4$1#6:;a5$1#7:");
}
...
TEST(6) {
  ...
  a0.insert(db::CellInstArray(db::CellInst(a4.cell_index()),
                              db::Trans(1 /*r90*/), g->array_repository(),
                              db::Vector(0, 10), db::Vector(10, 0), 5, 2));
  a0.insert(db::CellInstArray(db::CellInst(a5.cell_index()),
                              db::Trans(1 /*r90*/), g->array_repository(),
                              db::Vector(0, 10), db::Vector(10, 0), 5, 2));
  ...
  EXPECT_EQ(l2s(h),
            "a0top#0:cell_index=1 r90 0,0 array=(0,10,10,0 5x2),cell_index=2 "
            "r90 0,0 array=(0,10,10,0 5x2);a4#1:;a5#2:");
}
...
TEST(7) {
  ...
  EXPECT_EQ(
      l2s(h),
      "b0#0:;b1#1:cell_index=3 r0 0,0,cell_index=4 r0 0,0;b2#2:cell_index=4 r0 "
      "0,0;a3#3:cell_index=4 r0 0,0,cell_index=5 r0 0,0;a4#4:;a5#5:");
}
