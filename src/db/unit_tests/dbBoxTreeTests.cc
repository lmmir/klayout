...
template <class Box> Box qbox(int q, const Box &bbox) {
...
void print_tree_node(const Tree *tree, const Box &bbox, size_t pos,
                     db::box_tree_node<Tree> *node, const std::string &in) {
  std::cout << in << "x [\n";
  if (!node) {
...
template <class Tree> void print_tree(const Tree &t, db::Box bbox) {
  std::cout << "size = " << t.size() << "\n";
  print_tree_node(&t, bbox, 0, t.root(), "> ");
}

template <class Box, class Tree>
void print_unstable_tree_node(const Tree *tree, const Box &bbox, size_t pos,
                              db::box_tree_node<Tree> *node,
                              const std::string &in) {
  std::cout << in << "x [\n";
  if (!node) {
    for (size_t i = pos; i < pos + tree->size(); ++i) {
...
template <class Tree> void print_unstable_tree(const Tree &t, db::Box bbox) {
  std::cout << "size = " << t.size() << "\n";
...
typedef db::unstable_box_tree<db::Box, db::Box, Box2BoxCmplx, 4, 0>
    UnstableTestTreeCmplx;

typedef db::unstable_box_tree<db::Box, db::Box, Box2Box> UnstableTestTreeL;
typedef db::unstable_box_tree<db::Box, db::Box, Box2BoxCmplx>
    UnstableTestTreeCmplxL;

template <class Tree, class Box, class BoxConv>
static void test_tree_overlap(tl::TestBase *_this, const Tree &t, const Box &b,
                              BoxConv conv) {
...
template <class Tree, class Box, class BoxConv>
static void test_tree_touching(tl::TestBase *_this, const Tree &t, const Box &b,
                               BoxConv conv) {
  typedef typename Tree::object_type value_type;
...
inline int rvalue() { return (rand() % 10000) - 5000; }
...
TEST(4) {
  ...
  }
  {
    ...
    for (unsigned int i = 0; i < 10; ++i) {
      TestTreeCmplxL::touching_iterator it =
          t.begin_touching(db::Box(db::Point(-m, -m), db::Point(m, m)), conv);
      while (!it.at_end()) {
        ++it;
        ++n;
      }
...
TEST(4U) {
  ...
  }
  {
    ...
    size_t n = 0;
    for (unsigned int i = 0; i < 10; ++i) {
      UnstableTestTreeCmplxL::touching_iterator it =
          t.begin_touching(db::Box(db::Point(-m, -m), db::Point(m, m)), conv);
      while (!it.at_end()) {
        ++it;
        ++n;
...
TEST(7) {
...

  {
    ...
    for (unsigned int i = 0; i < 2000; ++i) {
      db::Coord sx = 0, sy = 0;
      TestTree::touching_iterator it = t.begin_touching(
          db::Box(db::Point(2000, 0), db::Point(3000, 0)), conv);
      while (!it.at_end()) {
        sx += abs(it->left());
        sy += abs(it->bottom());
        ++it;
...
TEST(7U) {
...

  {
    ...
    for (unsigned int i = 0; i < 2000; ++i) {
      db::Coord sx = 0, sy = 0;
      UnstableTestTree::touching_iterator it = t.begin_touching(
          db::Box(db::Point(2000, 0), db::Point(3000, 0)), conv);
      while (!it.at_end()) {
        sx += abs(it->left());
        sy += abs(it->bottom());
