...
TEST(1_WriterBasic) {
  ...
  std::string au_path = tl::combine_path(
      tl::combine_path(tl::testdata(), "algo"), "l2n_writer_au.txt");
...

  {
    ...
    l2n.build_all_nets(cm, ly2, lmap, "NET_", db::NPM_NoProperties,
                       tl::Variant(), db::BNH_Disconnected, 0, "DEVICE_");
  ...
  l2n.netlist()->begin_circuits()->begin_nets()->set_property(
      "a_\"non_quoted\"_string", "1s");

  l2n.netlist()->circuit_by_name("INV2")->begin_devices()->set_property(17,
                                                                        242);
  l2n.netlist()->circuit_by_name("INV2")->begin_devices()->set_property(
      "a_float", 20.5);
  l2n.netlist()->circuit_by_name("INV2")->begin_devices()->set_property(
      "a_\"non_quoted\"_string", "2s");

  l2n.netlist()->circuit_by_name("RINGO")->begin_subcircuits()->set_property(
      17, 342);
  l2n.netlist()->circuit_by_name("RINGO")->begin_subcircuits()->set_property(
      "a_float", 30.5);
  l2n.netlist()->circuit_by_name("RINGO")->begin_subcircuits()->set_property(
      "a_\"non_quoted\"_string", "3s");
...
TEST(2_WriterWithGlobalNets) {
  ...
  std::unique_ptr<db::Region> rmetal1_lbl(
      l2n.make_layer(metal1_lbl, "metal1_lbl"));
  std::unique_ptr<db::Region> rvia1(l2n.make_polygon_layer(via1, "via1"));
  std::unique_ptr<db::Region> rmetal2(l2n.make_polygon_layer(metal2, "metal2"));
  std::unique_ptr<db::Region> rmetal2_lbl(
      l2n.make_layer(metal2_lbl, "metal2_lbl"));
  ...
  std::string au_path = tl::combine_path(
      tl::combine_path(tl::testdata(), "algo"), "l2n_writer_au_2b.txt");
