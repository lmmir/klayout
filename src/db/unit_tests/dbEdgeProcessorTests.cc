...
void run_test_bool(tl::TestBase *_this, const char *file, int mode,
                   bool min_coherence = true, const char *au_file1 = 0,
                   const char *au_file2 = 0, const char *au_file3 = 0) {
  ...
  for (unsigned int l = 0; l < layout_org.layers(); ++l) {
    if (layout_org.is_valid_layer(l) &&
        layout_org.get_properties(l).layer == 1 &&
  ...
  for (unsigned int l = 0; l < layout_org.layers(); ++l) {
    if (layout_org.is_valid_layer(l) &&
        layout_org.get_properties(l).layer == 2 &&
        layout_org.get_properties(l).datatype == 0) {
      lb = l;
      break;
    }
  ...
  for (unsigned int l = 0; l < layout_org.layers(); ++l) {
    if (layout_org.is_valid_layer(l) &&
        layout_org.get_properties(l).layer == 100 &&
        layout_org.get_properties(l).datatype == 0) {
      lr = l;
      break;
    }
  ...
  proc.boolean(layout_org, layout_org.cell(*layout_org.begin_top_down()), la,
               ...
               layout_org.cell(*layout_org.begin_top_down()).shapes(lr), mode,
               true /*hierarchical*/, true /*resolve holes*/, min_coherence);

  db::LayerMap lmap;
  //  Note: the logical layers have to be non-existing ones because we read to a
  //  layout that has been configured with layers already
  lmap.map(db::LDPair(1, 0), 1000);
  lmap.map(db::LDPair(2, 0), 1001);
  lmap.map(db::LDPair(100, 0), 1002);

  db::compare_layouts(_this, layout_org, au_fn1, lmap,
                      false /*skip other layers*/, db::WriteOAS);

  layout_org.cell(*layout_org.begin_top_down()).shapes(lr).clear();

  unsigned int lr2 = layout_org.insert_layer();
  //  temporarily disable compression for the boolean step to acchieve identical
  //  results with the previous test
  db::PolygonGenerator::enable_compression_global(false);
  proc.boolean(layout_org, layout_org.cell(*layout_org.begin_top_down()), la,
               layout_org, layout_org.cell(*layout_org.begin_top_down()), lb,
               layout_org.cell(*layout_org.begin_top_down()).shapes(lr2), mode,
               true /*hierarchical*/, false /*resolve holes*/);
  db::PolygonGenerator::enable_compression_global(true);

  proc.merge(layout_org, layout_org.cell(*layout_org.begin_top_down()), lr2,
             layout_org.cell(*layout_org.begin_top_down()).shapes(lr),
             true /*hierarchical*/, 0 /*all polygons*/, true /*resolve holes*/,
             min_coherence);

  db::compare_layouts(_this, layout_org, au_fn2, lmap,
                      false /*skip other layers*/, db::WriteOAS);
  ...
  if (au_file3) {
    ...
    db::Shapes &shapes =
        layout_org.cell(*layout_org.begin_top_down()).shapes(lr);
    for (db::ShapeIterator s = shapes.begin(db::ShapeIterator::All);
         !s.at_end(); ++s) {
      ep.insert(s->polygon());
    }
    ...
    db::compare_layouts(_this, layout_org, au_fn3, lmap,
                        false /*skip other layers*/, db::WriteOAS);
  }
}

TEST(1and) {
  run_test_bool(_this, "and1.oas", db::BooleanOp::And, true, 0, 0,
                "and1_tz.oas");
}

TEST(2and) {
  run_test_bool(_this, "and2.oas", db::BooleanOp::And, true, 0, 0,
                "and2_tz.oas");
}

TEST(3and) {
  run_test_bool(_this, "and3.oas", db::BooleanOp::And, true, 0, 0,
                "and3_tz.oas");
}

TEST(4and) {
  run_test_bool(_this, "and4.oas", db::BooleanOp::And, true, 0, 0,
                "and4_tz.oas");
}

TEST(5and) {
  run_test_bool(_this, "and5.oas", db::BooleanOp::And, true, 0, 0,
                "and5_tz.oas");
}

TEST(6and) {
  run_test_bool(_this, "and6.oas", db::BooleanOp::And, true, 0, 0,
                "and6_tz.oas");
}

TEST(1xor) {
  run_test_bool(_this, "xor1.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor1_tz.oas");
}

TEST(2xor) {
  run_test_bool(_this, "xor2.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor2_tz.oas");
}

TEST(3xor) {
  run_test_bool(_this, "xor3.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor3_tz.oas");
}

TEST(4xor) {
  run_test_bool(_this, "xor4.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor4_tz.oas");
}

TEST(5xor) {
  run_test_bool(_this, "xor5.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor5_tz.oas");
}

TEST(6xor) {
  run_test_bool(_this, "xor6.oas", db::BooleanOp::Xor, true, 0, 0,
                "xor6_tz.oas");
}

TEST(7xor) {
  run_test_bool(_this, "xor7.oas", db::BooleanOp::Xor, true, "xor7_au1.oas",
                "xor7_au2.oas", "xor7_au_tz.oas");
}

TEST(8xor) {
  run_test_bool(_this, "xor8.oas", db::BooleanOp::Xor, true, "xor8_au1.oas",
                "xor8_au2.oas", "xor8_au_tz.oas");
}
...
TEST(7xor_max) {
  run_test_bool(_this, "xor7_max.oas", db::BooleanOp::Xor, false,
                "xor7_max_au1.oas", "xor7_max_au2.oas");
}

TEST(1or) { run_test_bool(_this, "or1.oas", db::BooleanOp::Or); }

TEST(2or) { run_test_bool(_this, "or2.oas", db::BooleanOp::Or); }

TEST(3or) { run_test_bool(_this, "or3.oas", db::BooleanOp::Or); }

TEST(4or) { run_test_bool(_this, "or4.oas", db::BooleanOp::Or); }

TEST(5or) { run_test_bool(_this, "or5.oas", db::BooleanOp::Or); }

TEST(6or) { run_test_bool(_this, "or6.oas", db::BooleanOp::Or); }

TEST(1anotb) { run_test_bool(_this, "anotb1.oas", db::BooleanOp::ANotB); }

TEST(2anotb) { run_test_bool(_this, "anotb2.oas", db::BooleanOp::ANotB); }

TEST(3anotb) { run_test_bool(_this, "anotb3.oas", db::BooleanOp::ANotB); }

TEST(4anotb) { run_test_bool(_this, "anotb4.oas", db::BooleanOp::ANotB); }

TEST(5anotb) { run_test_bool(_this, "anotb5.oas", db::BooleanOp::ANotB); }

TEST(6anotb) { run_test_bool(_this, "anotb6.oas", db::BooleanOp::ANotB); }

TEST(1bnota) { run_test_bool(_this, "bnota1.oas", db::BooleanOp::BNotA); }

TEST(2bnota) { run_test_bool(_this, "bnota2.oas", db::BooleanOp::BNotA); }

TEST(3bnota) { run_test_bool(_this, "bnota3.oas", db::BooleanOp::BNotA); }

TEST(4bnota) { run_test_bool(_this, "bnota4.oas", db::BooleanOp::BNotA); }

TEST(5bnota) { run_test_bool(_this, "bnota5.oas", db::BooleanOp::BNotA); }

TEST(6bnota) { run_test_bool(_this, "bnota6.oas", db::BooleanOp::BNotA); }

TEST(1special) {
  run_test_bool(_this, "special1.oas", db::BooleanOp::Xor, true, 0, 0,
                "special1_tz.oas");
}
...
TEST(2special) {
  ...
  run_test_bool(_this, "special2.oas", db::BooleanOp::Xor, true,
                "special2_au1.oas", 0, "special2_au1_tz.oas");
  run_test_bool(_this, "special2.oas", db::BooleanOp::And, true,
                "special2_au2.oas", 0, "special2_au2_tz.oas");
  run_test_bool(_this, "special2.oas", db::BooleanOp::ANotB, true,
                "special2_au3.oas", 0, "special2_au3_tz.oas");
  run_test_bool(_this, "special2.oas", db::BooleanOp::BNotA, true,
                "special2_au4.oas", 0, "special2_au4_tz.oas");
  run_test_bool(_this, "special2.oas", db::BooleanOp::Or, true,
                "special2_au5.oas", 0, "special2_au5_tz.oas");
}

TEST(3special) {
  test_is_long_runner();
  tl::SelfTimer timer("special3 test");
  run_test_bool(_this, "special3.oas", db::BooleanOp::Xor, true,
                "special3_au1.oas");
  run_test_bool(_this, "special3.oas", db::BooleanOp::And, true,
                "special3_au2.oas");
  run_test_bool(_this, "special3.oas", db::BooleanOp::ANotB, true,
                "special3_au3.oas");
  run_test_bool(_this, "special3.oas", db::BooleanOp::BNotA, true,
                "special3_au4.oas");
  run_test_bool(_this, "special3.oas", db::BooleanOp::Or, true,
                "special3_au5.oas");
}

void run_test_size(tl::TestBase *_this, const char *file, const char *au_file,
                   int mode, db::Coord dx, db::Coord dy,
                   bool min_coherence = true, bool flat = true) {
  db::Manager m(true);
  ...
  proc.size(layout_org, layout_org.cell(*layout_org.begin_top_down()), la,
            layout_org.cell(*layout_org.begin_top_down()).shapes(lr), dx, dy,
            mode, flat /*hierarchical*/, true /*resolve holes*/, min_coherence);

  db::LayerMap lmap;
  lmap.map(db::LDPair(1, 0), 1);
  lmap.map(db::LDPair(100, 0), 2);

  db::compare_layouts(_this, layout_org, au_fn, lmap,
                      false /*skip other layers*/, db::WriteOAS);
}

void run_test_twobool(tl::TestBase *_this, const char *file,
                      const char *au_file) {
  db::Manager m(true);
  ...
  for (unsigned int l = 0; l < layout_org.layers(); ++l) {
    if (layout_org.is_valid_layer(l) &&
        layout_org.get_properties(l).layer == 101 &&
        layout_org.get_properties(l).datatype == 0) {
      lrb = l;
      break;
    }
  ...
  for (db::RecursiveShapeIterator iter(
           layout_org, layout_org.cell(*layout_org.begin_top_down()), la);
       !iter.at_end(); ++iter) {
    db::Polygon p;
  ...
  db::ShapeGenerator sg1(
      layout_org.cell(*layout_org.begin_top_down()).shapes(lra),
      true /*clear shapes*/);
  db::PolygonGenerator pg1(sg1, true /*resolve holes*/,
                           false /*min. coherence*/);
  db::BooleanOp op1(db::BooleanOp::And);

  db::ShapeGenerator sg2(
      layout_org.cell(*layout_org.begin_top_down()).shapes(lrb),
      true /*clear shapes*/);
  db::PolygonGenerator pg2(sg2, true /*resolve holes*/,
                           false /*min. coherence*/);
...
TEST(1size) { run_test_size(_this, "size1.oas", "size1_au.oas", 2, -1, -1); }

TEST(2size) { run_test_size(_this, "size2.oas", "size2_au.oas", 2, 1, 1); }
// size5: 100: 0.002 (mode 0), 101: mode 1, 102: mode 2, .. 105; 200: -0.002
// (mode 0), 201: mode 1, 202: mode2 size6: 100: 0.002 (flat), 101: 0.002 (top
// cell), 102: 0.002 (cell by cell)
...
TEST(6size) {
  ...
  run_test_size(_this, "size6.oas", "size6_au2.oas", 2, 2, 2, true,
                false /*top cell only*/);
...
TEST(9twobool) { run_test_twobool(_this, "twobool9.oas", "twobool9_au1.oas"); }

void write(const std::vector<db::Polygon> &q1,
           const std::vector<db::Polygon> &q2, const std::vector<db::Edge> &e1,
           const std::vector<db::Edge> &e2, const std::string &fn) {
  db::Layout out;
  ...
  for (std::vector<db::Polygon>::const_iterator p = q1.begin(); p != q1.end();
       ++p) {
    cell->shapes(l1).insert(*p);
  }
  for (std::vector<db::Polygon>::const_iterator p = q2.begin(); p != q2.end();
       ++p) {
  ...
  for (std::vector<db::Polygon>::const_iterator p = p1.polygons().begin();
       p != p1.polygons().end(); ++p) {
  ...
  for (std::vector<db::Polygon>::const_iterator p = p2.polygons().begin();
       p != p2.polygons().end(); ++p) {
...
TEST(10) {
  ...
  for (std::vector<db::Edge>::const_iterator e = edges.begin();
       e != edges.end(); ++e) {
  ...
  std::set_difference(anotb_or_bnota.polygons().begin(),
                      anotb_or_bnota.polygons().end(),
  ...
  printf("diff1:\n");
  for (std::vector<db::Polygon>::const_iterator d = diff1.begin();
       d != diff1.end(); ++d) {
    printf("%s\n", d->to_string().c_str());
  }

  printf("diff2:\n");
  for (std::vector<db::Polygon>::const_iterator d = diff2.begin();
       d != diff2.end(); ++d) {
    printf("%s\n", d->to_string().c_str());
  }

  write(anotb_or_bnota.polygons(), xor_res.polygons(), edges, edges2,
        "x10.gds");
#endif
}
...
TEST(12) {
  ...
  edges.push_back(
      db::Edge(plast, db::Point(edges.front().p1().x(), plast.y())));
  edges.push_back(db::Edge(db::Point(edges.front().p1().x(), plast.y()),
                           edges.front().p1()));
...
TEST(14) {
  //  This has been a problem which was solved with R228 - it was taking too
  //  many iterations
  std::vector<db::Polygon> in1;
  std::vector<db::Polygon> in2;

  db::Point p1[] = {
      db::Point(71012503, 113882497), db::Point(70124103, 114770787),
      db::Point(70124097, 114875997), db::Point(70198487, 114950397),
      db::Point(70303697, 114950403), db::Point(71192097, 114062113),
      db::Point(71192103, 113956903), db::Point(71117713, 113882503)};

  db::Point p2[] = {
      db::Point(71060295, 113834700), db::Point(70938100, 113956895),
      db::Point(70938100, 114062105), db::Point(71012495, 114136500),
      db::Point(71117705, 114136500), db::Point(71239900, 114014305),
      db::Point(71239900, 113909095), db::Point(71165505, 113834700)};
  ...
  db::Point p3[] = {
      db::Point(71060295, 113834700), db::Point(70972121, 113922874),
      db::Point(70198499, 114696400), db::Point(70198495, 114696400),
      db::Point(70164485, 114730410), db::Point(70124103, 114770787),
      db::Point(70124103, 114770792), db::Point(70124100, 114770795),
      db::Point(70124100, 114823392), db::Point(70124097, 114875997),
      db::Point(70124100, 114876000), db::Point(70124100, 115107255),
      db::Point(71239900, 115107255), db::Point(71239900, 113909095),
  ...
  EXPECT_EQ(out[0].to_string(),
            "(71239900,114014305;71192097,114062108;71192097,114062113;"
            "71151715,114102490;71117705,114136500;71117701,114136500;70303697,"
            "114950403;70198487,114950397;70124100,114876000;70124100,"
            "115107255;71239900,115107255)");
  EXPECT_EQ(out[1].to_string(),
            "(70198495,114696400;70164485,114730410;70198499,114696400)");
  // before R787 was:
  // EXPECT_EQ (out[1].to_string (),
  // "(70972040,113922955;70198499,114696400;70198495,114696400;70164485,114730410;70124103,114770787)");
  EXPECT_EQ(out[2].to_string(),
            "(70124103,114770792;70124100,114770795;70124100,114823392)");
}
...
TEST(15) {
  ...
  db::Point p2[] = {
      db::Point(0, 22388),       db::Point(1467712, 245710),
      db::Point(1510912, 29731), db::Point(1511726, 25637),
  ...
  EXPECT_EQ(
      out[0].to_string(),
      "(43264,22390;1467712,245710;1511719,25671;1511726,25637;1512360,22467)");
}
...
TEST(16) {
  ...
  db::Point p1[] = {db::Point(23, 2345), db::Point(4, 9832),
                    db::Point(10592, 2485)};

  db::Point p2[] = {db::Point(13, 0), db::Point(13, 7486), db::Point(10873, 6)};

  db::Point p3[] = {db::Point(0, 2818), db::Point(27, 10304),
                    db::Point(8643, 2823)};
  ...
  std::sort(out.begin(), out.end());
  EXPECT_EQ(out[0].to_string(),
            "(13,0;13,2818;0,2818;13,6286;13,6422;4,9832;25,9817;27,10304;2825,"
            "7874;10592,2485;7336,2442;10873,6)");
}
...
TEST(17) {
  ...
  db::Point p1[] = {db::Point(113, 64), db::Point(1293, 469),
                    db::Point(1293, 64)};

  db::Point p2[] = {db::Point(204, 100), db::Point(1388, 495),
                    db::Point(1387, 101)};

  db::Point p3[] = {db::Point(0, 18), db::Point(1177, 434),
                    db::Point(1178, 18)};
  ...
  EXPECT_EQ(
      out[0].to_string(),
      "(0,18;130,64;113,64;218,100;204,100;700,266;706,268;1177,434;1177,429;"
      "1293,469;1293,463;1388,495;1387,101;1293,101;1293,64;1178,64;1178,18)");
}
...
TEST(18) {
  ...
  db::Point p1[] = {db::Point(419, 1400.00), db::Point(2281, 1589.00),
                    db::Point(2281, 1400.00)};

  db::Point p2[] = {db::Point(419, 1400.00), db::Point(2284, 1589.00),
                    db::Point(2284, 1400.00)};

  db::Point p3[] = {db::Point(453, 1405.00), db::Point(2316, 1588.00),
  ...
  std::sort(out.begin(), out.end());
  EXPECT_EQ(out[0].to_string(),
            "(419,1400;468,1405;453,1405;926,1451;953,1454;2281,1589;2284,1589;"
            "2284,1585;2316,1588;2316,1405;2284,1405;2284,1400)");
}
...
TEST(19) {
  ...
  db::Point p1[] = {
      db::Point(26029700, 19931900), db::Point(11944600, 24988200),
      db::Point(16663400, 48582400), db::Point(31607400, 45593600)};

  db::Point p2[] = {db::Point(12654800, 0), db::Point(0, 8492100),
                    db::Point(13371200, 28417800),
                    db::Point(27109200, 23471400)};

  db::Point p3[] = {
      db::Point(145086300, 20050900), db::Point(12705400, 28790300),
      db::Point(12851400, 29524600), db::Point(145086300, 35290900)};
  ...
  std::sort(out.begin(), out.end());
  EXPECT_EQ(out[0].to_string(),
            "(12654800,0;0,8492100;12316037,26845401;12771101,29120740;"
            "12851400,29524600;12851876,29524621;16663400,48582400;31607400,"
            "45593600;28260773,30196549;145086300,35290900;145086300,20050900;"
            "27739393,27797800;26821540,23574973;27109200,23471400;26629840,"
            "22693003;26029700,19931900;25128663,20255356)");
}
...
TEST(20) {
  ...
  db::Point p1[] = {db::Point(7394, 2768), db::Point(7533, 2826),
                    db::Point(7404, 2768)};

  db::Point p2[] = {db::Point(7427, 2768), db::Point(7533, 2826),
                    db::Point(7434, 2768)};

  db::Point p3[] = {db::Point(7362, 2768), db::Point(7532, 2826),
                    db::Point(7374, 2768)};
  ...
  EXPECT_EQ(out[1].to_string(),
            "(7394,2768;7533,2826;7434,2768;7427,2768;7533,2826;7404,2768)");
}
...
TEST(21) {
  ...
  db::Point p1[] = {db::Point(2696, 0), db::Point(5297, 13339),
                    db::Point(6592, 2603), db::Point(4217, 5014)};

  db::Point p2[] = {db::Point(2696, 0), db::Point(4217, 5015),
                    db::Point(890, 1381), db::Point(0, 11887),
                    db::Point(4217, 5015)};
...
TEST(24) {
  ...
  {
    db::Point p1[] = {db::Point(0, -9), db::Point(1, 10), db::Point(0, 10),
                      db::Point(1, -9)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }
  ...
  {
    db::Point p1[] = {db::Point(1, 1), db::Point(-2, 2), db::Point(-2, 3)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }

  {
    db::Point p1[] = {db::Point(3, -1), db::Point(0, 1), db::Point(3, 0)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }

  {
    db::Point p1[] = {db::Point(1, 0), db::Point(-2, 1), db::Point(-2, 2)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }

  {
    db::Point p1[] = {db::Point(3, -2), db::Point(0, 0), db::Point(3, -1)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }

  {
    ...
    std::sort(out.begin(), out.end());
    EXPECT_EQ(out[0].to_string(),
              "(0,-9;0,0;-2,1;-2,3;0,1;0,10;1,10;1,1;0,0;3,0;3,-2;1,0;1,-9)");
  }
...
TEST(25) {
  ...
  {
    db::Point p1[] = {
        db::Point(-471, 2264), db::Point(-471, 2367), db::Point(-345, 2367),
        db::Point(-333, 2391), db::Point(-327, 2402), db::Point(-329, 2400),
        db::Point(-327, 2399), db::Point(-329, 2400), db::Point(-323, 2407),
        db::Point(-332, 2407), db::Point(-332, 2391), db::Point(-318, 2393),
        db::Point(-328, 2397), db::Point(-323, 2390), db::Point(-332, 2394),
        db::Point(-330, 2387), db::Point(-326, 2387), db::Point(-333, 2394),
        db::Point(-333, 2388), db::Point(-328, 2402), db::Point(-339, 2402),
        db::Point(-353, 2367), db::Point(-353, 2264)};
    in1.push_back(db::Polygon());
    in1.back().assign_hull(p1 + 0, p1 + sizeof(p1) / sizeof(p1[0]));
  }
...
TEST(26a) {
  ...
  ep.insert(db::Polygon(db::Box(db::Point(-100, -100), db::Point(1000, 1000))),
            4);
...
TEST(26b) {
  ...
  ep.insert(db::Polygon(db::Box(db::Point(-100, -100), db::Point(1000, 1000))),
            4);
  ep.insert(db::Polygon(db::Box(db::Point(1000, 1100), db::Point(1100, 1200))),
            5);
...
TEST(26c) {
  ...
  ep.insert(db::Polygon(db::Box(db::Point(1000, 1100), db::Point(1010, 1110))),
            6);
...
TEST(27) {
  ...
  //  because we don't use mode 1 merging for p2, we get loops at the corners of
  //  p2
  {
    db::EdgeProcessor ep;
    ep.insert(poly, 0);
  ...
  //  BooleanOp2 behaves the same with modes -1
  {
    ...
    EXPECT_EQ(
        out[0].to_string(),
        "(100,0;100,100;0,100;0,900;100,900;100,1000;900,1000;900,900;1000,900;"
        "1000,100;900,100;900,0/100,100;900,100;900,900;100,900)");
  }
  ...
  //  with BooleanOp2 we can solve this issue
  {
    ...
    EXPECT_EQ(out.size(), size_t(1));
    EXPECT_EQ(out[0].to_string(),
              "(0,0;0,1000;1000,1000;1000,0/100,100;900,100;900,900;100,900)");
  }
...
TEST(28) {
  ...
  db::Point b1[] = {db::Point(-518003, -792684), db::Point(-489451, -724867),
                    db::Point(-487680, -724734), db::Point(-485180, -757934),
                    db::Point(-501151, -775469)};
  b.push_back(db::Polygon());
  b.back().assign_hull(b1 + 0, b1 + sizeof(b1) / sizeof(b1[0]));

  std::vector<db::Polygon> a;
  db::Point a1[] = {db::Point(-488720, -758200), db::Point(-491220, -725000),
                    db::Point(-487680, -724734), db::Point(-485180, -757934)};
  a.push_back(db::Polygon());
  a.back().assign_hull(a1 + 0, a1 + sizeof(a1) / sizeof(a1[0]));
  db::Point a2[] = {db::Point(-490953, -726224), db::Point(-505953, -709625),
                    db::Point(-502948, -706909), db::Point(-487948, -723509)};
  a.push_back(db::Polygon());
  a.back().assign_hull(a2 + 0, a2 + sizeof(a2) / sizeof(a2[0]));
  db::Point a3[] = {db::Point(-491225, -724867), db::Point(-491225, -691667),
                    db::Point(-487675, -691667), db::Point(-487675, -724867)};
  a.push_back(db::Polygon());
  a.back().assign_hull(a3 + 0, a3 + sizeof(a3) / sizeof(a3[0]));

  db::EdgeProcessor ep;
  for (std::vector<db::Polygon>::const_iterator i = a.begin(); i != a.end();
       ++i) {
    ep.insert(*i, 0);
  }
  for (std::vector<db::Polygon>::const_iterator i = b.begin(); i != b.end();
       ++i) {
    ep.insert(*i, 1);
  }
  ...
  EXPECT_EQ(out.size(), size_t(1));
  EXPECT_EQ(out[0].to_string(),
            "(-488720,-758200;-490960,-728451;-489451,-724867;-489450,-724867;-"
            "487680,-724734;-487675,-724800;-485180,-757934)");
}

// #644
TEST(29) {
  std::vector<db::Polygon> b;
  db::Point b1[] = {db::Point(0, 0),     db::Point(0, 608),
                    db::Point(172, 602), db::Point(572, 588),
                    db::Point(573, 588), db::Point(710, 583),
                    db::Point(710, 0)};
  b.push_back(db::Polygon());
  b.back().assign_hull(b1 + 0, b1 + sizeof(b1) / sizeof(b1[0]));

  std::vector<db::Polygon> a;
  db::Point a1[] = {db::Point(140, 140), db::Point(140, 603),
                    db::Point(167, 602), db::Point(372, 595),
                    db::Point(580, 588), db::Point(580, 140)};
  a.push_back(db::Polygon());
  a.back().assign_hull(a1 + 0, a1 + sizeof(a1) / sizeof(a1[0]));
  ...
  EXPECT_EQ(out.size(), size_t(1));
  EXPECT_EQ(out[0].to_string(),
            "(0,0;0,608;172,602;572,588;580,588;710,583;710,0)");
}

TEST(30) {
  std::vector<db::Polygon> a;
  db::Point a1[] = {db::Point(0, 0), db::Point(0, 500), db::Point(300, 500),
                    db::Point(500, 300), db::Point(500, 0)};
  a.push_back(db::Polygon());
  a.back().assign_hull(a1 + 0, a1 + sizeof(a1) / sizeof(a1[0]));
...
TEST(31) {
  ...
  EXPECT_EQ(s, "(0,0;0,400);(0,0;500,0);(500,0;500,300);(0,400;0,417);(400,400;"
               "0,400);(394,406;0,417);(0,417;0,500)");
...
TEST(32) {
  ...
  std::vector<db::Edge> out;
  db::EdgeContainer ec(out);
  db::EdgePolygonOp op(db::EdgePolygonOp::Inside,
                       false /*not including touch*/);
...
TEST(33) {
  ...
  EXPECT_EQ(
      s,
      "(0,-100;0,0);(500,-100;500,0);(-100,0;0,0);(500,0;600,0);(500,300;500,"
      "400);"
  ...
  s = tl::join(out.begin(), out.end(), ";");
  EXPECT_EQ(s, "(-100,-100;-100,500);(-100,-100;400,-100);(400,-100;400,0);("
               "400,-100;600,-100);"
               "(600,-100;600,500);(400,400;400,500);(-100,500;-100,600);(-100,"
               "500;0,500);"
               "(300,500;400,500);(400,500;400,600);(400,500;600,500);(600,500;"
               "600,600)");
}
...
TEST(44) {
  db::EdgeProcessor ep;
  db::Point pts[] = {
      db::Point(0, 0),       db::Point(0, 4000),    db::Point(2000, 4000),
      db::Point(2500, 3000), db::Point(3000, 2500), db::Point(6500, 2000),
      db::Point(8000, 4000), db::Point(9000, 4000), db::Point(9000, 0)};
  db::Polygon p;
...
TEST(45) {
  db::EdgeProcessor ep;
  db::Point pts[] = {
      db::Point(0, 0),     db::Point(0, 200),    db::Point(200, 150),
      db::Point(250, 150), db::Point(300, 100),  db::Point(800, 50),
      db::Point(900, 200), db::Point(1000, 200), db::Point(1000, 0)};
  db::Polygon p;
  p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
  ep.insert(p);
...
TEST(46) {
  db::EdgeProcessor ep;
  db::Point pts[] = {db::Point(0, 0),     db::Point(300, 500),
                     db::Point(800, 100), db::Point(300, 250),
                     db::Point(350, 0),   db::Point(400, 150)};
  db::Polygon p;
  p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
  ep.insert(p);
...
TEST(100) {
  ...
  proc.boolean(layout_1, layout_1.cell(*layout_1.begin_top_down()), l1_l1d0,
               ...
               lr_top->shapes(lr_l100d0), db::BooleanOp::Xor,
               true /*hierarchical*/, true /*resolve holes*/,
               true /*min coherence*/);
  ...
  proc.boolean(layout_1, layout_1.cell(*layout_1.begin_top_down()), l1_l1d0,
               layout_2, layout_2.cell(*layout_2.begin_top_down()), l2_l1d0,
               lr_top->shapes(lr_l101d0), db::BooleanOp::Xor,
               false /*hierarchical*/, true /*resolve holes*/,
               true /*min coherence*/);
  ...
  proc.size(layout_1, layout_1.cell(*layout_1.begin_top_down()), l1_l1d0,
            lr_top->shapes(lr_l110d0), 100, 200, 2, true /*hierarchical*/,
            true /*resolve holes*/, true /*min coherence*/);

  unsigned int lr_l111d0 = lr.insert_layer(db::LayerProperties(111, 0));

  proc.size(layout_1, layout_1.cell(*layout_1.begin_top_down()), l1_l1d0,
            lr_top->shapes(lr_l111d0), 100, 200, 2, false /*hierarchical*/,
            true /*resolve holes*/, true /*min coherence*/);
...
std::string run_test101(tl::TestBase *_this, const db::Trans &t) {
  ...
  {
    db::Point pts[] = {db::Point(0, 0), db::Point(0, 10), db::Point(10, 10),
                       db::Point(10, 0)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    p.transform(t);
  ...
  {
    db::Point pts[] = {db::Point(-1, -1), db::Point(-1, 8), db::Point(2, 11),
                       db::Point(2, -1)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    p.transform(t);
  ...
  {
    db::Point pts[] = {db::Point(2, -1), db::Point(2, 11), db::Point(11, 11),
                       db::Point(11, -1)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    p.transform(t);
...
TEST(101) {
  EXPECT_EQ(run_test101(_this, db::Trans(db::Trans::r0)),
            "(0,0;0,9;1,10;10,10;10,0)");
  EXPECT_EQ(run_test101(_this, db::Trans(db::Trans::r90)),
            "(-9,0;-10,1;-10,10;0,10;0,0)");
  EXPECT_EQ(run_test101(_this, db::Trans(db::Trans::r180)),
            "(-10,-10;-10,0;0,0;0,-9;-1,-10)");
  EXPECT_EQ(run_test101(_this, db::Trans(db::Trans::r270)),
            "(0,-10;0,0;9,0;10,-1;10,-10)");
}
...
TEST(102) {
  ...
  {
    db::Point pts[] = {db::Point(0, 0), db::Point(0, 1000),
                       db::Point(1000, 1000), db::Point(1000, 0)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 0);
  }
  ...
  {
    db::Point pts[] = {db::Point(100, 100), db::Point(100, 200),
                       db::Point(200, 200), db::Point(200, 100)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 1);
  }
  ...
  {
    db::Point pts[] = {db::Point(500, 100), db::Point(500, 200),
                       db::Point(600, 200), db::Point(600, 100)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 1);
  }
  ...
  EXPECT_EQ(out.size(), size_t(1));
  EXPECT_EQ(out[0].to_string(),
            "(0,0;0,200;100,200;100,100;200,100;200,200;500,200;500,100;600,"
            "100;600,200;0,200;0,1000;1000,1000;1000,0)");
}
...
TEST(103) {
  ...
  {
    db::Point pts[] = {db::Point(0, 0),      db::Point(0, 500),
                       db::Point(1500, 500), db::Point(1500, 0),
                       db::Point(1000, 0),   db::Point(1000, 400),
                       db::Point(500, 400),  db::Point(500, 0)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 0);
  }
  ...
  {
    db::Point pts[] = {db::Point(100, 100), db::Point(100, 400),
                       db::Point(400, 400), db::Point(400, 100)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 1);
  }
  ...
  {
    db::Point pts[] = {db::Point(1100, 100), db::Point(1100, 400),
                       db::Point(1400, 400), db::Point(1400, 100)};
    db::Polygon p;
    p.assign_hull(&pts[0], &pts[sizeof(pts) / sizeof(pts[0])]);
    ep.insert(p, 1);
  }
  ...
  EXPECT_EQ(
      out[0].to_string(),
      "(0,0;0,400;100,400;100,100;400,100;400,400;1100,400;1100,100;1400,100;"
      "1400,400;0,400;0,500;1500,500;1500,0;1000,0;1000,400;500,400;500,0)");
#else
  //  elaborate hole treatment
  EXPECT_EQ(
      out[0].to_string(),
      "(0,0;0,400;100,400;100,100;400,100;400,400;0,400;0,500;1500,500;1500,0;"
      "1000,0;1000,400;1100,400;1100,100;1400,100;1400,400;500,400;500,0)");
#endif
...
void run_test135a(tl::TestBase *_this, const db::Trans &t) {
  ...
  db::Point pts[] = {db::Point(0, 0), db::Point(19, 19), db::Point(19, 18),
                     db::Point(43, 32), db::Point(37, 27)};
  ...
  //  this is just supposed to work and not fail with internal error
  //  "m_open.empty()"
  std::vector<db::Polygon> out;
  db::PolygonContainer pc(out);
  db::PolygonGenerator pg2(pc, false /*don't resolve holes*/,
                           true /*min. coherence*/);
  db::SimpleMerge op(1 /*wc>0*/);
  ep.process(pg2, op);
...
std::string run_test135b(tl::TestBase *_this, const db::Trans &t) {
  ...
  db::Point pts[] = {db::Point(215, 0), db::Point(145, 11), db::Point(37, 31),
                     db::Point(36, 31), db::Point(0, 43)};
...
TEST(135b) {
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::r0)),
            "(36,33;32,34;37,33)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::r90)),
            "(-35,32;-26,77;-33,37;-33,36)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::r180)),
            "(-33,-35;-78,-26;-37,-33;-36,-33)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::r270)),
            "(25,-78;33,-37;33,-36;34,-33)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::m0)),
            "(32,-35;36,-33;37,-33;77,-26)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::m45)),
            "(34,32;33,36;33,37)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::m90)),
            "(-78,25;-33,34;-36,33;-37,33)");
  EXPECT_EQ(run_test135b(_this, db::Trans(db::Trans::m135)),
            "(-26,-78;-35,-33;-33,-36;-33,-37)");
}
...
TEST(136) {
  ...
  proc.merge(layout_1, layout_1.cell(*layout_1.begin_top_down()), l_l20000d0,
             lr_top->shapes(lr_l100d0), false /*hierarchical*/, 0,
             true /*resolve holes*/, true /*min coherence*/);
