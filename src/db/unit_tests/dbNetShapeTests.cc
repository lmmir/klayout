...
TEST(1) {
  ...
  EXPECT_EQ(db::NetShape(s.polygon_ref()).type() == db::NetShape::Polygon,
            true);
  EXPECT_EQ(db::NetShape(s.polygon_ref()).polygon_ref().obj().to_string(),
  ...
  EXPECT_EQ(db::NetShape(s.text_ref()).text_ref().obj().to_string(),
            "('abc',r0 0,0)");
}
...
TEST(2) {
  ...
  s.transform(db::Disp(db::Vector(10, 20)));
  EXPECT_EQ(s.text_ref().obj().transformed(s.text_ref().trans()).to_string(),
            "('abc',r0 110,220)");
}
...
TEST(5) {
  ...
  EXPECT_EQ(sp1.interacts_with_transformed(db::NetShape(),
                                           db::Trans(db::Vector(1000, 0))),
            false);
  EXPECT_EQ(sp1.interacts_with(sp1), true);
  EXPECT_EQ(sp1.interacts_with_transformed(sp1, db::Trans(db::Vector(1000, 0))),
            false);
  EXPECT_EQ(sp1.interacts_with(sp2), true);
  EXPECT_EQ(sp2.interacts_with(sp1), true);
  EXPECT_EQ(sp1.interacts_with(sp3), false);
  EXPECT_EQ(
      sp1.interacts_with_transformed(sp3, db::Trans(db::Vector(50, -200))),
      true);
  EXPECT_EQ(sp3.interacts_with(sp1), false);
  EXPECT_EQ(sp3.interacts_with_transformed(sp1, db::Trans(db::Vector(50, 200))),
            true);

  EXPECT_EQ(sp1.interacts_with(st1), false);
  EXPECT_EQ(sp1.interacts_with_transformed(st1, db::Trans(db::Vector(10, 20))),
            true);
  EXPECT_EQ(sp1.interacts_with_transformed(st1, db::Trans(db::Vector(5, 20))),
            false);
  EXPECT_EQ(sp1.interacts_with(st2), true);

  EXPECT_EQ(st1.interacts_with(sp1), false);
  EXPECT_EQ(
      st1.interacts_with_transformed(sp1, db::Trans(db::Vector(-10, -20))),
      true);
  EXPECT_EQ(st1.interacts_with_transformed(sp1, db::Trans(db::Vector(-5, -20))),
            false);
  EXPECT_EQ(st2.interacts_with(sp1), true);

  EXPECT_EQ(st1.interacts_with(st1), true);
  EXPECT_EQ(st1.interacts_with_transformed(st1, db::Trans(db::Vector(-5, -20))),
            false);
  EXPECT_EQ(st2.interacts_with(st1), false);
  EXPECT_EQ(st2.interacts_with_transformed(st1, db::Trans(db::Vector(50, 60))),
            true);
}
