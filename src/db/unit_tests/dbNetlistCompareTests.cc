...
class NetlistCompareTestLogger : public db::NetlistCompareLogger {
public:
  ...
  virtual void end_circuit(const db::Circuit *a, const db::Circuit *b,
    ...
    out("end_circuit " + circuit2str(a) + " " + circuit2str(b) + " " +
        (matching ? "MATCH" : "NOMATCH"));
  }
  ...
  virtual void match_devices_with_different_parameters(const db::Device *a,
                                                       const db::Device *b) {
    out("match_devices_with_different_parameters " + device2str(a) + " " +
        device2str(b));
  }
  ...
  match_devices_with_different_device_classes(const db::Device *a,
                                              const db::Device *b) {
    out("match_devices_with_different_device_classes " + device2str(a) + " " +
        device2str(b));
  }
  ...
  std::string text() const { return tl::join(m_texts, "\n"); }

  void clear() { m_texts.clear(); }
...
template <class Obj> static std::string name_of(const Obj *obj) {
  return obj ? obj->name() : std::string("(null)");
}

template <class Obj> static std::string expanded_name_of(const Obj *obj) {
  return obj ? obj->expanded_name() : std::string("(null)");
}

template <class PairData> static std::string dp2s(const PairData &dp) {
  return expanded_name_of(dp.pair.first) + ":" +
         expanded_name_of(dp.pair.second) + " [" + xref_status2s(dp.status) +
         "]";
}

static std::string ne2s(const db::NetTerminalRef *ref) {
  if (ref) {
    return expanded_name_of(ref->device()) + "[" +
           name_of(ref->terminal_def()) + "]";
  } else {
    return "(null)";
  }
}
...
static std::string ne2s(const db::NetSubcircuitPinRef *ref) {
  if (ref) {
    return expanded_name_of(ref->subcircuit()) + "[" +
           expanded_name_of(ref->pin()) + "]";
  } else {
    return "(null)";
  }
}
...
std::string xref2s(const db::NetlistCrossReference &xref) {
  ...
  for (db::NetlistCrossReference::circuits_iterator c = xref.begin_circuits();
       c != xref.end_circuits(); ++c) {

    const db::NetlistCrossReference::PerCircuitData *pcd =
        xref.per_circuit_data_for(*c);
    tl_assert(pcd != 0);

    s += name_of(c->first) + ":" + name_of(c->second) + " [" +
         xref_status2s(pcd->status) + "]:\n";

    for (db::NetlistCrossReference::PerCircuitData::pin_pairs_const_iterator i =
             pcd->pins.begin();
         i != pcd->pins.end(); ++i) {
      s += " pin " + dp2s(*i) + "\n";
    }
    for (db::NetlistCrossReference::PerCircuitData::net_pairs_const_iterator i =
             pcd->nets.begin();
         i != pcd->nets.end(); ++i) {
      s += " net " + dp2s(*i) + "\n";
      const db::NetlistCrossReference::PerNetData *pnd =
          xref.per_net_data_for(i->pair);
      for (db::NetlistCrossReference::PerNetData::terminal_pairs_const_iterator
               j = pnd->terminals.begin();
           j != pnd->terminals.end(); ++j) {
        s += "  terminal " + nep2s(*j) + "\n";
      }
      for (db::NetlistCrossReference::PerNetData::pin_pairs_const_iterator j =
               pnd->pins.begin();
           j != pnd->pins.end(); ++j) {
        s += "  pin " + nep2s(*j) + "\n";
      }
      for (db::NetlistCrossReference::PerNetData::
               subcircuit_pin_pairs_const_iterator j =
                   pnd->subcircuit_pins.begin();
           j != pnd->subcircuit_pins.end(); ++j) {
        s += "  subcircuit_pin " + nep2s(*j) + "\n";
      }
    }
    for (db::NetlistCrossReference::PerCircuitData::device_pairs_const_iterator
             i = pcd->devices.begin();
         i != pcd->devices.end(); ++i) {
      s += " device " + dp2s(*i) + "\n";
    }
    for (db::NetlistCrossReference::PerCircuitData::
             subcircuit_pairs_const_iterator i = pcd->subcircuits.begin();
         i != pcd->subcircuits.end(); ++i) {
      s += " subcircuit " + dp2s(*i) + "\n";
    }
  }
...
TEST(0_EqualDeviceParameters) {
  ...
  *eqp += db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_L,
                                    0.0, 0.0);
...
TEST(1_SimpleInverter) {
  const char *nls1 = "circuit INV ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     "  device PMOS $1 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $2 (S=VSS,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";

  const char *nls2 = "circuit INV ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     "  device NMOS $1 (S=OUT,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $2 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
...
TEST(1_SimpleInverterMatchedDeviceClasses) {
  ...
  const char *nls2 = "circuit INV ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     "  device NMOSB $1 (S=OUT,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOSB $2 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  comp.same_device_classes(nl1.device_class_by_name("PMOS"),
                           nl2.device_class_by_name("PMOSB"));

  bool good = comp.compare(&nl1, &nl2);
  EXPECT_EQ(good, false);

  logger.clear();
  comp.same_device_classes(nl1.device_class_by_name("NMOS"),
                           nl2.device_class_by_name("NMOSB"));
  //  avoids device class mismatch errors:
  comp.same_device_classes(nl1.device_class_by_name("NMOSB"),
                           nl2.device_class_by_name("NMOS"));
  comp.same_device_classes(nl1.device_class_by_name("PMOSB"),
                           nl2.device_class_by_name("PMOS"));
...
TEST(1_SimpleInverterSkippedDevices) {
  const char *nls1 = "circuit INV ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     ...
                     "  device NMOS $3 (S=VSS,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  const char *nls2 = "circuit INV ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     ...
                     "  device PMOS $4 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  EXPECT_EQ(xref2s(xref), "INV:INV [NoMatch]:\n"
                          " pin $0:$1 [Match]\n"
                          " pin $1:$3 [Match]\n"
                          " pin $2:$0 [Match]\n"
                          " pin $3:$2 [Match]\n"
...
TEST(3_Buffer) {
  const char *nls1 = "circuit BUF ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     "  device PMOS $1 (S=VDD,G=IN,D=INT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $2 (S=VSS,G=IN,D=INT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $3 (S=VDD,G=INT,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $4 (S=VSS,G=INT,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";

  const char *nls2 = "circuit BUF ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     "  device PMOS $1 (S=VDD,G=IN,D=$10) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $2 (S=VDD,G=$10,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $3 (S=$10,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $4 (S=OUT,G=$10,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
...
TEST(4_BufferTwoPaths) {
  const char *nls1 = "circuit BUF ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     ...
                     "  device NMOS $4 (S=VSS,G=INT,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $5 (S=VDD,G=IN,D=INT2) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $6 (S=VSS,G=IN,D=INT2) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $7 (S=VDD,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $8 (S=VSS,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  const char *nls2 = "circuit BUF ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     ...
                     "  device PMOS $2 (S=VDD,G=$10,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $3 (S=VDD,G=IN,D=$11) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $4 (S=VDD,G=$11,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $5 (S=$10,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $6 (S=OUT,G=$10,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $7 (S=$11,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $8 (S=OUT,G=$11,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
...
TEST(5_BufferTwoPathsDifferentParameters) {
  ...
  const char *nls2 =
      ...
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $7 (S=$11,G=IN,D=VSS) "
      "(L=0.35,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n" //  NOTE: 0.35
                                                                //  instead of
                                                                //  0.25
      "  device NMOS $8 (S=OUT,G=$11,D=VSS) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
  ...
  //  Forcing the power nets into equality makes the parameter error harder to
  //  detect
  ...
  *eql += db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_L,
                                    1.5, 0.0);
  ...
  logger.clear();
  nl1.device_class_by_name("NMOS")->set_parameter_compare_delegate(
      new db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_L,
                                    0.0, 0.0));
  good = comp.compare(&nl1, &nl2);
  ...
  logger.clear();
  db::EqualDeviceParameters eq_dp =
      db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_W) +
      db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_L, 0.2,
                                0.0);
  nl1.device_class_by_name("NMOS")->set_parameter_compare_delegate(
      new db::EqualDeviceParameters(eq_dp));
  good = comp.compare(&nl1, &nl2);
  ...
  logger.clear();
  eq_dp = db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_W) +
          db::EqualDeviceParameters(db::DeviceClassMOS3Transistor::param_id_L);
  nl1.device_class_by_name("NMOS")->set_parameter_compare_delegate(
      new db::EqualDeviceParameters(eq_dp));
  good = comp.compare(&nl1, &nl2);
...
TEST(5_BufferTwoPathsDifferentDeviceClasses) {
  const char *nls1 = "circuit BUF ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     ...
                     "  device NMOS $6 (S=VSS,G=IN,D=INT2) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOSB $7 (S=VDD,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOSB $8 (S=VSS,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  const char *nls2 = "circuit BUF ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     ...
                     "  device NMOS $7 (S=$11,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOSB $8 (S=OUT,G=$11,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
...
TEST(6_BufferTwoPathsAdditionalResistor) {
  ...
  const char *nls2 = "circuit BUF ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     ...
                     "  device PMOS $3 (S=VDD,G=IN,D=$11) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOSB $4 (S=VDD,G=$11,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $5 (S=$10,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $6 (S=OUT,G=$10,D=VSS) "
  ...
  //  Forcing the power nets into equality makes the resistor error harder to
  //  detect
  const db::Circuit *ca = nl1.circuit_by_name("BUF");
  const db::Circuit *cb = nl2.circuit_by_name("BUF");
  comp.same_nets(ca->net_by_name("VDD"), cb->net_by_name("VDD"));
...
TEST(6_BufferTwoPathsAdditionalDevices) {
  const char *nls1 = "circuit BUF ($1=IN,$2=OUT,$3=VDD,$4=VSS);\n"
                     ...
                     "  device NMOS $8 (S=VSS,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $9 (S=VSS,G=INT2,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
  ...
  const char *nls2 = "circuit BUF ($1=VDD,$2=IN,$3=VSS,$4=OUT);\n"
                     ...
                     "  device PMOS $2 (S=VDD,G=$10,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $3 (S=VDD,G=$10,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $4 (S=VDD,G=IN,D=$11) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device PMOS $5 (S=VDD,G=$11,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $6 (S=$10,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $7 (S=OUT,G=$10,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $8 (S=$11,G=IN,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $9 (S=OUT,G=$11,D=VSS) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n";
...
TEST(7_Resistors) {
  const char *nls1 = "circuit TRIANGLE ($0=P1,$1=P2,$2=P3);\n"
  ...
  EXPECT_EQ(logger.text(), "begin_circuit TRIANGLE TRIANGLE\n"
                           "match_nets P2 P2\n"
                           "match_nets P1 P1\n"
                           "match_nets P3 P3\n"
                           "match_pins $0 $0\n"
...
TEST(11_MismatchingSubcircuits) {
  const char *nls1 = "circuit INV ($0=IN,$1=OUT,$2=VDD,$3=VSS);\n"
                     "  device PMOS $1 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $2 (S=VSS,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
  ...
  EXPECT_EQ(xref2s(xref), "TOP:TOP [Match]:\n"
                          " pin $0:$2 [Match]\n"
                          " pin $1:$0 [Match]\n"
                          " pin $2:$1 [Match]\n"
                          " pin $3:$3 [Match]\n"
...
TEST(13_MismatchingSubcircuitsAdditionalHierarchy) {
  ...
  EXPECT_EQ(logger.text(), "circuit_mismatch VIA (null)\n"
...
TEST(14_Subcircuit2Nand) {
  const char *nls1 =
      ...
      "  device PMOS $1 (S=VDD,G=A,D=OUT) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S=VDD,G=B,D=OUT) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $3 (S=VSS,G=A,D=INT) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=INT,G=B,D=OUT) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "end;\n"
...
TEST(14_Subcircuit2NandMismatchNoSwap) {
  ...
  db::NetlistComparer comp(&logger);
  comp.set_dont_consider_net_names(true);
  //  intentionally missing: comp.equivalent_pins (nl2.circuit_by_name ("NAND"),
  //  0, 1);
  ...
  EXPECT_EQ(xref2s(xref), "NAND:NAND [Match]:\n"
                          " pin $0:$0 [Match]\n"
                          " pin $1:$1 [Match]\n"
                          " pin $2:$2 [Match]\n"
                          " pin $3:$3 [Match]\n"
...
TEST(15_EmptySubCircuitTest) {
  const char *nls1 = "circuit INV2 (IN=IN,$2=$2,OUT=OUT,$4=$4,$5=$5);\n"
  ...
  const char *nls2 =
      ...
      "  device PMOS $1 (S=$2,G=IN,D=$5) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $1 (S=$2,G=IN,D=$4) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $3 (S=$5,G=$2,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $4 (S=$4,G=$2,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
  ...
  EXPECT_EQ(logger.text(), "begin_circuit TRANS TRANS\n"
                           "match_nets $3 $3\n"
                           "match_nets $2 $1\n"
                           "match_nets $1 $2\n"
                           "match_pins S S\n"
...
TEST(15_EmptySubCircuitWithoutPinNames) {
  ...
  const char *nls2 =
      ...
      "end;\n"
      //  This circuit is an abstract and its pins are not defined by the pin
      //  names -> they are internally marked as swappable
      "circuit TRANS ($1=$1,$2=$2,$3=$3);\n"
      "end;\n";
...
TEST(16_UniqueSubCircuitMatching) {
  const char *nls1 =
      ...
      "  device PMOS4 $1 (S=$3,G=IN,D=VDD,B=$1) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS4 $2 (S=VDD,G=$3,D=OUT,B=$1) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS4 $3 (S=$3,G=IN,D=VSS,B=BULK) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS4 $4 (S=VSS,G=$3,D=OUT,B=BULK) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "end;\n";
  const char *nls2 =
      "circuit RINGO ();\n"
      ...
      "  device PMOS4 $1 (S=$3,G=IN,D=VDD,B=$1) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS4 $2 (S=$3,G=IN,D=VSS,B=BULK) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS4 $3 (S=VDD,G=$3,D=OUT,B=$1) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS4 $4 (S=VSS,G=$3,D=OUT,B=BULK) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "end;\n";
  ...
  EXPECT_EQ(logger.text(), "begin_circuit INV2 INV2\n"
                           "match_nets VDD VDD\n"
                           "match_nets OUT OUT\n"
                           "match_nets $3 $3\n"
                           "match_nets IN IN\n"
...
TEST(17_InherentlyAmbiguousDecoder) {
  ...
  logger.clear();
  comp.set_dont_consider_net_names(true);
  comp.same_nets(nl1.circuit_by_name("DECODER")->net_by_name("A"),
                 nl2.circuit_by_name("DECODER")->net_by_name("A"));
  good = comp.compare(&nl1, &nl2);
...
TEST(18_ClockTree) {
  ...
  EXPECT_EQ(txt, "begin_circuit INV INV\n"
                 "match_nets VDD VDD\n"
                 "match_nets OUT OUT\n"
                 "match_nets IN IN\n"
                 "match_nets VSS VSS\n"
...
TEST(19_SymmetricCircuit) {
  //  Test test requires a certain depth, name sensitivity to resolve
  //  ambiguities and tests the backtracking paths.

  const char *nls1 =
      "circuit DECODE "
      "(VDD=VDD,nn1_=nn1_,nn1=nn1,q0=q0,q0_=q0_,q1_=q1_,q1=q1,nn2=nn2,nn2_=nn2_"
      ",a0=a0,a0_=a0_,g1=g1,g0=g0,gtp=gtp,VSS=VSS,WELL=$14);"
  ...
  const char *nls2 =
      "circuit DECODE "
      "(A0=A0,A0_=A0_,Q0=Q0,Q0_=Q0_,Q1=Q1,Q1_=Q1_,NN2=NN2,NN2_=NN2_,NN1=NN1,"
      "NN1_=NN1_,G0=G0,G1=G1,NN3=NN3,VDD=VDD,VSS=VSS,WELL=WELL);"
  ...
  EXPECT_EQ(txt, "begin_circuit DECODE DECODE\n"
  ...
  EXPECT_EQ(logger.text(), "begin_circuit DECODE DECODE\n"
                           "match_nets $41 WL1_EN_\n"
                           "match_nets g0 G0\n"
                           "match_nets g1 G1\n"
                           "match_nets $40 HNET52\n"
...
TEST(20_BusLikeConnections) {
  ...
  const char *nls1 = "circuit INV (IN=IN,OUT=OUT,VDD=VDD,VSS=VSS);\n"
                     "  device PMOS $1 (S=VDD,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "  device NMOS $2 (S=VSS,G=IN,D=OUT) "
                     "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
                     "end;\n"
                     "circuit INV8 "
                     "(IN1=IN1,OUT1=OUT1,IN2=IN2,OUT2=OUT2,IN3=IN3,OUT3=OUT3,"
                     "IN4=IN4,OUT4=OUT4,IN5=IN5,OUT5=OUT5,IN6=IN6,OUT6=OUT6,"
                     "IN7=IN7,OUT7=OUT7,IN8=IN8,OUT8=OUT8,VDD=VDD,VSS=VSS);\n"
  ...
  const char *nls2 =
      ...
      "end;\n"
      "circuit INV8 "
      "(Q1=Q1,A1=A1,Q2=Q2,A2=A2,Q3=Q3,A3=A3,Q4=Q4,A4=A4,Q5=Q5,A5=A5,Q6=Q6,A6="
      "A6,Q7=Q7,A7=A7,Q8=Q8,A8=A8,VDD=VDD,VSS=VSS);\n"
...
TEST(21_BusLikeAmbiguousConnections) {
  ...
  const char *nls2 =
      "circuit INV8 "
      "(IN1=IN1,OUT1=OUT1,IN2=IN2,OUT2=OUT2,IN3=IN3,OUT3=OUT3,IN4=IN4,OUT4="
      "OUT4,IN5=IN5,OUT5=OUT5,IN6=IN6,OUT6=OUT6,IN7=IN7,OUT7=OUT7,IN8=IN8,OUT8="
      "OUT8,VDD=VDD,VSS=VSS);\n"
      //  NOTE: passive nets make the pins ambiguous
      "end;\n"
      "circuit INV8_WRAP "
      "(Q1=Q1,A1=A1,Q2=Q2,A2=A2,Q3=Q3,A3=A3,Q4=Q4,A4=A4,Q5=Q5,A5=A5,Q6=Q6,A6="
      "A6,Q7=Q7,A7=A7,Q8=Q8,A8=A8,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV8 INV8 "
      "(IN1=A1,OUT1=Q1,IN2=A2,OUT2=Q2,IN3=A3,OUT3=Q3,IN4=A4,OUT4=Q4,IN5=A5,"
      "OUT5=Q5,IN6=A6,OUT6=Q6,IN7=A7,OUT7=Q7,IN8=A8,OUT8=Q8,VDD=VDD,VSS=VSS);\n"
      "end;\n"
      "circuit TOP "
      "(Q1=Q1,A1=A1,Q2=Q2,A2=A2,Q3=Q3,A3=A3,Q4=Q4,A4=A4,Q5=Q5,A5=A5,Q6=Q6,A6="
      "A6,Q7=Q7,A7=A7,Q8=Q8,A8=A8,VDD=VDD,VSS=VSS);\n"
  ...
  EXPECT_EQ(txt, "begin_circuit INV8 INV8\n"
                 "match_nets IN1 IN1\n"
                 "match_nets IN2 IN2\n"
                 "match_nets IN3 IN3\n"
                 "match_nets IN4 IN4\n"
...
TEST(22_NodesRemoved) {
  ...
  EXPECT_EQ(txt, "begin_circuit INV2 INV2\n"
                 "match_nets VDD VDD\n"
                 "match_nets OUT OUT\n"
                 "match_nets $3 $3\n"
                 "match_nets IN IN\n"
  ...
  //  additional nodes are not ignored when they come from the reference side
  //  (second)
  EXPECT_EQ(txt, "begin_circuit INV2 INV2\n"
                 "match_nets VDD VDD\n"
                 "match_nets OUT OUT\n"
...
TEST(23_NodesRemovedWithError) {
  const char *nls1 =
      ...
      "end;\n"
      "circuit INV2PAIR ($2=$I8,$3=$I5,$4=$I4,$5=$I3,$6=$I2);\n"
      //  NOTE: $1 pin should not be connected to different nets, although its
      //  not functional
...
TEST(24_NodesRemovedButConnectedInOther) {
  const char *nls1 =
      ...
      "(BULK=VSS,$2=$I25,$3=VDD,$4=VSS,$5=$I6,$6=$I7,$7=VDD);\n"
      "end;\n"
      //  rewired here: BULK->VSS (pin $4), $1->$I3/$I4 (both rewired pins are
      //  deleted in the first netlist) This proves that we can basically do
      //  everything with the dropped pins.
      "circuit INV2PAIR "
      "(BULK=BULK,$2=$I8,$3=$I6,$4=$I5,$5=$I3,$6=$I2,$7=$I1);\n"
      "  subcircuit INV2 $1 "
      "($1=$I3,IN=$I3,$3=$I7,OUT=$I4,VSS=$I5,VDD=$I6,BULK=$I5);\n"
      "  subcircuit INV2 $2 "
      "($1=$I4,IN=$I4,$3=$I8,OUT=$I2,VSS=$I5,VDD=$I6,BULK=$I5);\n"
      "end;\n"
      "circuit INV2 ($1=$1,IN=IN,$3=$3,OUT=OUT,VSS=VSS,VDD=VDD,BULK=BULK);\n"
      "  device PMOS $1 (S=$3,G=IN,D=VDD) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
  ...
  //  NOTE: additional nets are ignored in the first netlist but not from the
  //  second
  EXPECT_EQ(txt, "begin_circuit INV2 INV2\n"
                 "match_nets VDD VDD\n"
                 "match_nets OUT OUT\n"
...
TEST(25c_JoinSymmetricNetsMultipleMessedUp) {
  const char *nls = "circuit NOR3 (A=A,C=C,B=B,OUT=OUT,VSS=VSS,VDD=VDD);\n"
  ...
  EXPECT_EQ(
      ...
      "  device NMOS $2 (S=OUT,G=A,D=VSS) "
      "(L=0.23,W=2.05,AS=0,AD=0,PS=0,PD=0);\n"
      "  device PMOS $3 (S=$3,G=A,D=OUT) (L=0.27,W=3.3,AS=0,AD=0,PS=0,PD=0);\n"
      "  device NMOS $4 (S=OUT,G=B,D=VSS) "
      "(L=0.23,W=2.05,AS=0,AD=0,PS=0,PD=0);\n"
      "  device PMOS $8 (S=VDD,G=C,D=$6) (L=0.27,W=3.3,AS=0,AD=0,PS=0,PD=0);\n"
      "  device NMOS $12 (S=VSS,G=C,D=OUT) "
      "(L=0.23,W=2.05,AS=0,AD=0,PS=0,PD=0);\n"
      "end;\n")
}

TEST(26_JoinSymmetricNets) {
  const char *nls = "circuit RESCUBE (A=A,B=B);\n"
...
TEST(28_NoSymmetryDetectionCases) {
  {
    const char *nls = "circuit NAND2 (A=A,B=B,OUT=OUT,VSS=VSS,VDD=VDD);\n"
                      "  device PMOS $1 (S=OUT,G=A,D=VDD) (L=0.25,W=1);\n"
                      "  device PMOS $2 (S=VDD,G=B,D=OUT) (L=0.25,W=1);\n"
                      //  NOTE: $1 and $2 are separate nets, but cannot be
                      //  joined because of different W
                      "  device NMOS $3 (S=$1,G=A,D=OUT) (L=0.25,W=1);\n"
...

  {
    const char *nls =
        ...
        "  device NMOS $3 (S=$1,G=A,D=OUT) (L=0.25,W=1);\n"
        "  device NMOS $4 (S=$2,G=A,D=OUT) (L=0.25,W=1);\n"
        //  NOTE: $1 and $2 are separate nets, but cannot be joined because of
        //  different D connections
        "  device NMOS $5 (S=$1,G=B,D=VSS) (L=0.25,W=1);\n"
...
TEST(28_JoinSymmetricNets) {
  const char *nls = "circuit INV2LOAD (A=A,VSS=VSS,VDD=VDD);\n"
  ...
  EXPECT_EQ(
      ...
      "  device NMOS $4 (S=VSS,G=A,D=OUT1) "
      "(L=0.25,W=1.5,AS=0,AD=0,PS=0,PD=0);\n"
...
TEST(30_ComparePrimaryAndOtherParameters) {
  ...
  EXPECT_EQ(txt, "begin_circuit X X\n"
                 "match_ambiguous_nets 1 1\n"
                 "match_ambiguous_nets 2 2\n"
                 "match_pins 1 1\n"
                 "match_pins 2 2\n"
  ...
  //  if we install a delegate which introduces a tolerance (absolute 1) it will
  //  still not match

  dc1->set_parameter_compare_delegate(new db::EqualDeviceParameters(
      db::DeviceClassResistor::param_id_R, 1.0, 0.0));
  ...
  //  if we install a delegate which introduces a tolerance (absolute 2.0) it
  //  will match

  dc1->set_parameter_compare_delegate(new db::EqualDeviceParameters(
      db::DeviceClassResistor::param_id_R, 2.0, 0.0));
  ...
  dc1->parameter_definition_non_const(db::DeviceClassResistor::param_id_R)
  ...
  dc1->set_parameter_compare_delegate(
      new db::EqualDeviceParameters(db::DeviceClassResistor::param_id_R, true));
  ...
  dc1->parameter_definition_non_const(db::DeviceClassResistor::param_id_L)
      ->set_is_primary(true);
  ...
  *dynamic_cast<db::EqualDeviceParameters *>(
      dc1->parameter_compare_delegate()) +=
      db::EqualDeviceParameters(db::DeviceClassResistor::param_id_L, 0.11, 0.0);
...
TEST(31_ParallelMOSFets) {
  ...
  EXPECT_EQ(txt, "begin_circuit LVS_TEST LVS_TEST\n"
                 "match_nets 2 2\n"
                 "match_nets 1 1\n"
                 "match_pins 1 1\n"
                 "match_pins 2 2\n"
