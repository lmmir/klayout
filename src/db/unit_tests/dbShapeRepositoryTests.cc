...
TEST(3) {
  ...
  EXPECT_EQ(rep2.shape_repository().repository(db::SimplePolygon::tag()).size(),
            size_t(1));
...
TEST(4) {
  ...
  while (!s.at_end()) {
    ...
    if (s->is_text()) {
      ...
      for (db::Shape::point_iterator pt = s->begin_point();
           pt != s->end_point(); ++pt) {
        EXPECT_EQ(*pt, *r);
        ++r;
      }
      ...
      std::vector<db::Point>::const_iterator r = c1.begin();
      for (db::Shape::point_iterator pt = s->begin_hull(); pt != s->end_hull();
           ++pt) {
        EXPECT_EQ(*pt, *r);
        ++r;
      }
