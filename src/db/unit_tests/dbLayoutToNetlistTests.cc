...
static void
dump_nets_to_layout(const db::LayoutToNetlist &l2n, db::Layout &ly,
                    const std::map<const db::Region *, unsigned int> &lmap,
                    const db::CellMapping &cmap) {
  const db::Netlist &nl = *l2n.netlist();
  for (db::Netlist::const_circuit_iterator c = nl.begin_circuits();
       c != nl.end_circuits(); ++c) {
    ...
    for (db::Circuit::const_net_iterator n = c->begin_nets();
      ...
      for (std::map<const db::Region *, unsigned int>::const_iterator m =
               lmap.begin();
           m != lmap.end(); ++m) {

        std::unique_ptr<db::Region> shapes(
            l2n.shapes_of_net(*n, *m->first, false));
        if (shapes->empty()) {
          continue;
        }
...
static void dump_recursive_nets_to_layout(
    const db::LayoutToNetlist &l2n, db::Layout &ly,
    const std::map<const db::Region *, unsigned int> &lmap,
    const db::CellMapping &cmap) {
  const db::Netlist &nl = *l2n.netlist();
  for (db::Netlist::const_circuit_iterator c = nl.begin_circuits();
    ...
    for (db::Circuit::const_net_iterator n = c->begin_nets();
      ...
      for (std::map<const db::Region *, unsigned int>::const_iterator m =
               lmap.begin();
           m != lmap.end() && !any; ++m) {
        any = !db::recursive_cluster_shape_iterator<db::NetShape>(
                   l2n.net_clusters(), l2n.layer_of(*m->first), c->cell_index(),
                   n->cluster_id())
                   .at_end();
      }
...
static unsigned int define_layer(db::Layout &ly, db::LayerMap &lmap,
                                 int gds_layer, int gds_datatype = 0) {
  unsigned int lid =
      ly.insert_layer(db::LayerProperties(gds_layer, gds_datatype));
...
TEST(0_Basic) {
  ...
  reg2.reset(0);
  //  NOTE: deleting the region does not free the layer as we hold it internally
  //  inside LayoutToNetlist
  EXPECT_EQ(l2n.internal_layout()->is_valid_layer(1), true);
...
TEST(1_BasicExtraction) {
  ...
  db::LayoutToNetlist l2n(
      db::RecursiveShapeIterator(ly, tc, std::set<unsigned int>()));
  ...
  std::unique_ptr<db::Texts> rmetal2_lbl(
      l2n.make_text_layer(metal2_lbl, "metal2_lbl"));
  ...
  dl["G"] = &rpgate;
  dl["P"] =
      rpoly.get(); //  not needed for extraction but to return terminal shapes
  ...
  //  return the computed layers into the original layout and write it for
  //  debugging purposes NOTE: this will include the device layers too

  unsigned int lgate =
      ly.insert_layer(db::LayerProperties(10, 0)); // 10/0 -> Gate
  unsigned int lsd =
      ly.insert_layer(db::LayerProperties(11, 0)); // 11/0 -> Source/Drain
  unsigned int lpdiff =
      ly.insert_layer(db::LayerProperties(12, 0)); // 12/0 -> P Diffusion
  unsigned int lndiff =
      ly.insert_layer(db::LayerProperties(13, 0)); // 13/0 -> N Diffusion
  unsigned int lpoly = ly.insert_layer(
      db::LayerProperties(14, 0)); // 14/0 -> Poly with gate terminal
  ...
  //  create some mess - we have to keep references to the layers to make them
  //  not disappear
  rmetal1_lbl.reset(0);
  rmetal2_lbl.reset(0);
  rpoly_lbl.reset(0);
  ...
  db::CellMapping cm =
      l2n.cell_mapping_into(ly, tc, true /*with device cells*/);
  ...
  db::compare_netlist(
      ...
      "  device PMOS $1 (S=$2,G=IN,D=$5) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S=$5,G=$2,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $3 (S=$2,G=IN,D=$4) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=$4,G=$2,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
  ...
  //  top level
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal2, db::DPoint(0.0, 1.8))),
            "RINGO:FB");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal2, db::Point(0, 1800))), "RINGO:FB");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal2, db::DPoint(-2.0, 1.8))),
            "(null)");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal1, db::DPoint(-1.5, 1.8))),
            "RINGO:FB");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal1, db::DPoint(24.5, 1.8))),
            "RINGO:OSC");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal1, db::DPoint(5.3, 0.0))),
            "RINGO:VSS");

  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal1, db::DPoint(2.6, 1.0))),
            "RINGO:$I39");
  EXPECT_EQ(qnet_name(l2n.probe_net(*rmetal1, db::DPoint(6.4, 1.0))),
            "RINGO:$I2");
...

  {
    ...
    db::CellMapping cm =
        l2n.cell_mapping_into(ly2, top2, true /*with device cells*/);
    ...
    l2n.build_all_nets(cm, ly2, lmap, 0, db::NPM_NoProperties, tl::Variant(),
                       db::BNH_Disconnected, 0, 0);
...

  {
    ...
    l2n.build_all_nets(cm, ly2, lmap, "NET_", db::NPM_NoProperties,
                       tl::Variant(), db::BNH_Disconnected, 0, 0);
...

  {
    ...
    l2n.build_all_nets(cm, ly2, lmap, 0, db::NPM_NoProperties, tl::Variant(),
                       db::BNH_SubcircuitCells, "CIRCUIT_", 0);
...

  {
    ...
    l2n.build_all_nets(cm, ly2, lmap, 0, db::NPM_AllProperties, tl::Variant(42),
                       db::BNH_Flatten, 0, 0);
...

  {
    ...
    l2n.build_all_nets(cm, ly2, lmap, "NET_", db::NPM_NoProperties,
                       tl::Variant(), db::BNH_SubcircuitCells, "CIRCUIT_",
                       "DEVICE_");
  ...
  //  make pins for named nets of top-level circuits - this way they are not
  //  purged
...
TEST(3_GlobalNetConnections) {
  ...
  unsigned int lgate =
      ly.insert_layer(db::LayerProperties(20, 0)); // 20/0 -> Gate
  unsigned int lsd =
      ly.insert_layer(db::LayerProperties(21, 0)); // 21/0 -> Source/Drain
  unsigned int lpdiff =
      ly.insert_layer(db::LayerProperties(22, 0)); // 22/0 -> P Diffusion
  unsigned int lndiff =
      ly.insert_layer(db::LayerProperties(23, 0)); // 23/0 -> N Diffusion
  unsigned int lptie =
      ly.insert_layer(db::LayerProperties(24, 0)); // 24/0 -> P Tie
  unsigned int lntie =
      ly.insert_layer(db::LayerProperties(25, 0)); // 25/0 -> N Tie
  ...
  db::compare_netlist(
      _this, *l2n.netlist(),
      "circuit RINGO ();\n"
      "  subcircuit INV2PAIR $1 "
      "(BULK='BULK,VSS',$2=FB,$3=VDD,$4='BULK,VSS',$5=$I7,$6=OSC,$7=VDD);\n"
      "  subcircuit INV2PAIR $2 "
      "(BULK='BULK,VSS',$2=$I22,$3=VDD,$4='BULK,VSS',$5=FB,$6=$I13,$7=VDD);\n"
      "  subcircuit INV2PAIR $3 "
      "(BULK='BULK,VSS',$2=$I23,$3=VDD,$4='BULK,VSS',$5=$I13,$6=$I5,$7=VDD);\n"
      "  subcircuit INV2PAIR $4 "
      "(BULK='BULK,VSS',$2=$I24,$3=VDD,$4='BULK,VSS',$5=$I5,$6=$I6,$7=VDD);\n"
      "  subcircuit INV2PAIR $5 "
      "(BULK='BULK,VSS',$2=$I25,$3=VDD,$4='BULK,VSS',$5=$I6,$6=$I7,$7=VDD);\n"
      "end;\n"
      "circuit INV2PAIR "
      "(BULK=BULK,$2=$I8,$3=$I6,$4=$I5,$5=$I3,$6=$I2,$7=$I1);\n"
      "  subcircuit INV2 $1 "
      "($1=$I1,IN=$I3,$3=$I7,OUT=$I4,VSS=$I5,VDD=$I6,BULK=BULK);\n"
      "  subcircuit INV2 $2 "
      "($1=$I1,IN=$I4,$3=$I8,OUT=$I2,VSS=$I5,VDD=$I6,BULK=BULK);\n"
      "end;\n"
      "circuit INV2 ($1=$1,IN=IN,$3=$3,OUT=OUT,VSS=VSS,VDD=VDD,BULK=BULK);\n"
      "  device PMOS $1 (S=$3,G=IN,D=VDD) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S=VDD,G=$3,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $3 (S=$3,G=IN,D=VSS) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=VSS,G=$3,D=OUT) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
  ...
  db::compare_netlist(
      ...
      "  subcircuit INV2PAIR $3 "
      "($2=$I23,$3=VDD,$4=VSS,$5=$I13,$6=$I5,$7=VDD);\n"
...
TEST(4_GlobalNetDeviceExtraction) {
  ...
  db::compare_netlist(
      ...
      "end;\n"
      "circuit INV2 ($1=$1,IN=IN,$3=$3,OUT=OUT,VSS=VSS,VDD=VDD,BULK=BULK);\n"
      "  device PMOS $1 (S=$3,G=IN,D=VDD,B=$1) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S=VDD,G=$3,D=OUT,B=$1) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $3 (S=$3,G=IN,D=VSS,B=BULK) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=VSS,G=$3,D=OUT,B=BULK) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  subcircuit TRANS $1 ($1=$3,$2=VSS,$3=IN);\n"
      "  subcircuit TRANS $2 ($1=$3,$2=VDD,$3=IN);\n"
      "  subcircuit TRANS $3 ($1=VDD,$2=OUT,$3=$3);\n"
  ...
  db::compare_netlist(
      ...
      "circuit RINGO (FB=FB,OSC=OSC,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2PAIR $1 "
      "(BULK=VSS,$2=FB,$3=VDD,$4=VSS,$5=$I7,$6=OSC,$7=VDD);\n"
      "  subcircuit INV2PAIR $2 "
      "(BULK=VSS,$2=$I22,$3=VDD,$4=VSS,$5=FB,$6=$I13,$7=VDD);\n"
      "  subcircuit INV2PAIR $3 "
      "(BULK=VSS,$2=$I23,$3=VDD,$4=VSS,$5=$I13,$6=$I5,$7=VDD);\n"
      "  subcircuit INV2PAIR $4 "
      "(BULK=VSS,$2=$I24,$3=VDD,$4=VSS,$5=$I5,$6=$I6,$7=VDD);\n"
      "  subcircuit INV2PAIR $5 "
      "(BULK=VSS,$2=$I25,$3=VDD,$4=VSS,$5=$I6,$6=$I7,$7=VDD);\n"
      "end;\n"
      "circuit INV2PAIR "
      "(BULK=BULK,$2=$I8,$3=$I6,$4=$I5,$5=$I3,$6=$I2,$7=$I1);\n"
      "  subcircuit INV2 $1 "
...
TEST(5_DeviceExtractionWithDeviceCombination) {
  ...
  db::compare_netlist(
      ...
      "circuit RINGO ();\n"
      "  subcircuit INV2PAIR $1 "
      "(BULK='BULK,VSS',$2=VDD,$3='BULK,VSS',$4=FB,$5=$I7,$6=OSC,$7=VDD);\n"
      "  subcircuit INV2PAIR $2 "
      "(BULK='BULK,VSS',$2=VDD,$3='BULK,VSS',$4=$I22,$5=FB,$6=$I13,$7=VDD);\n"
      "  subcircuit INV2PAIR $3 "
      "(BULK='BULK,VSS',$2=VDD,$3='BULK,VSS',$4=$I23,$5=$I13,$6=$I5,$7=VDD);\n"
      "  subcircuit INV2PAIR $4 "
      "(BULK='BULK,VSS',$2=VDD,$3='BULK,VSS',$4=$I24,$5=$I5,$6=$I6,$7=VDD);\n"
      "  subcircuit INV2PAIR $5 "
      "(BULK='BULK,VSS',$2=VDD,$3='BULK,VSS',$4=$I25,$5=$I6,$6=$I7,$7=VDD);\n"
      "end;\n"
      "circuit INV2PAIR "
      "(BULK=BULK,$2=$I6,$3=$I5,$4=$I4,$5=$I3,$6=$I2,$7=$I1);\n"
      "  subcircuit INV2 $1 "
      "($1=$I1,IN=$I3,OUT=$I4,VSS=$I5,VDD=$I6,BULK=BULK);\n"
      "  subcircuit INV2 $2 "
      "($1=$I1,IN=$I4,OUT=$I2,VSS=$I5,VDD=$I6,BULK=BULK);\n"
      "end;\n"
      "circuit INV2 ($1=$1,IN=IN,OUT=OUT,VSS=VSS,VDD=VDD,BULK=BULK);\n"
      "  device PMOS $1 (S=OUT,G=IN,D=VDD,B=$1) "
      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
      "  device PMOS $2 (S=VDD,G=IN,D=OUT,B=$1) "
      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
      "  device NMOS $3 (S=OUT,G=IN,D=VSS,B=BULK) "
      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
      "  device NMOS $4 (S=VSS,G=IN,D=OUT,B=BULK) "
      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
  ...
  db::compare_netlist(
      ...
      "circuit RINGO (FB=FB,OSC=OSC,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2PAIR $1 "
      "(BULK=VSS,$2=VDD,$3=VSS,$4=FB,$5=$I7,$6=OSC,$7=VDD);\n"
      "  subcircuit INV2PAIR $2 "
      "(BULK=VSS,$2=VDD,$3=VSS,$4=$I22,$5=FB,$6=$I13,$7=VDD);\n"
      "  subcircuit INV2PAIR $3 "
      "(BULK=VSS,$2=VDD,$3=VSS,$4=$I23,$5=$I13,$6=$I5,$7=VDD);\n"
      "  subcircuit INV2PAIR $4 "
      "(BULK=VSS,$2=VDD,$3=VSS,$4=$I24,$5=$I5,$6=$I6,$7=VDD);\n"
      "  subcircuit INV2PAIR $5 "
      "(BULK=VSS,$2=VDD,$3=VSS,$4=$I25,$5=$I6,$6=$I7,$7=VDD);\n"
      "end;\n"
      "circuit INV2PAIR "
      "(BULK=BULK,$2=$I6,$3=$I5,$4=$I4,$5=$I3,$6=$I2,$7=$I1);\n"
      "  subcircuit INV2 $1 "
      ...
      "circuit INV2 ($1=$1,IN=IN,OUT=OUT,VSS=VSS,VDD=VDD,BULK=BULK);\n"
      "  device PMOS $1 (S=OUT,G=IN,D=VDD,B=$1) "
      "(L=0.25,W=3.5,AS=1.4,AD=1.4,PS=6.85,PD=6.85);\n"
      "  device NMOS $3 (S=OUT,G=IN,D=VSS,B=BULK) "
      "(L=0.25,W=3.5,AS=1.4,AD=1.4,PS=6.85,PD=6.85);\n"
      "end;\n");
...
TEST(6_MoreDeviceTypes) {
  ...
  db::compare_netlist(_this, *l2n.netlist(),
                      "circuit TOP ();\n"
                      "  device HVPMOS $1 (S=Z,G=$5,D=VDD2,B=$8) "
                      "(L=1.5,W=4.05,AS=5.4675,AD=2.73375,PS=10.8,PD=5.4);\n"
                      "  device HVPMOS $2 (S=VDD2,G=Z,D=$5,B=$8) "
                      "(L=1.5,W=4.05,AS=2.73375,AD=5.4675,PS=5.4,PD=10.8);\n"
                      "  device LVPMOS $3 (S=$10,G=A,D=$6,B=$9) "
                      "(L=1.5,W=2.475,AS=4.77675,AD=3.155625,PS=8.81,PD=7.5);\n"
                      "  device HVNMOS $4 (S=Z,G=$6,D=VSS,B=BULK) "
                      "(L=1.5,W=5.25,AS=7.0875,AD=3.54375,PS=13.2,PD=6.6);\n"
                      "  device HVNMOS $5 (S=VSS,G=A,D=$5,B=BULK) "
                      "(L=1.5,W=5.25,AS=3.54375,AD=7.0875,PS=6.6,PD=13.2);\n"
                      "  device LVNMOS $6 (S=VSS,G=A,D=$6,B=BULK) "
                      "(L=1.2,W=1.7,AS=2.346,AD=2.1165,PS=6.16,PD=5.89);\n"
                      "end;\n");
}
...
TEST(7_MoreByEmptyDeviceTypes) {
  ...
  db::compare_netlist(_this, *l2n.netlist(),
                      "circuit TOP ();\n"
                      "  device LVPMOS $1 (S=Z,G=$5,D=VDD2,B=$8) "
                      "(L=1.5,W=4.05,AS=5.4675,AD=2.73375,PS=10.8,PD=5.4);\n"
                      "  device LVPMOS $2 (S=VDD2,G=Z,D=$5,B=$8) "
                      "(L=1.5,W=4.05,AS=2.73375,AD=5.4675,PS=5.4,PD=10.8);\n"
                      "  device LVPMOS $3 (S=$10,G=A,D=$6,B=$9) "
                      "(L=1.5,W=2.475,AS=4.77675,AD=3.155625,PS=8.81,PD=7.5);\n"
                      "  device LVNMOS $4 (S=VSS,G=A,D=$6,B=BULK) "
                      "(L=1.2,W=1.7,AS=2.346,AD=2.1165,PS=6.16,PD=5.89);\n"
                      "  device LVNMOS $5 (S=Z,G=$6,D=VSS,B=BULK) "
                      "(L=1.5,W=5.25,AS=7.0875,AD=3.54375,PS=13.2,PD=6.6);\n"
                      "  device LVNMOS $6 (S=VSS,G=A,D=$5,B=BULK) "
                      "(L=1.5,W=5.25,AS=3.54375,AD=7.0875,PS=6.6,PD=13.2);\n"
                      "end;\n");
}
...
TEST(8_FlatExtraction) {
  ...
  std::unique_ptr<db::Region> rnwell(
      new db::Region(db::RecursiveShapeIterator(ly, tc, nwell)));
  std::unique_ptr<db::Region> rthickgox(
      new db::Region(db::RecursiveShapeIterator(ly, tc, thickgox)));
  std::unique_ptr<db::Region> ractive(
      new db::Region(db::RecursiveShapeIterator(ly, tc, active)));
  std::unique_ptr<db::Region> rpplus(
      new db::Region(db::RecursiveShapeIterator(ly, tc, pplus)));
  std::unique_ptr<db::Region> rnplus(
      new db::Region(db::RecursiveShapeIterator(ly, tc, nplus)));
  std::unique_ptr<db::Region> rpoly(
      new db::Region(db::RecursiveShapeIterator(ly, tc, poly)));
  std::unique_ptr<db::Region> rpoly_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, poly_lbl)));
  std::unique_ptr<db::Region> rcont(
      new db::Region(db::RecursiveShapeIterator(ly, tc, cont)));
  std::unique_ptr<db::Region> rmetal1(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal1)));
  std::unique_ptr<db::Region> rmetal1_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal1_lbl)));
  std::unique_ptr<db::Region> rvia1(
      new db::Region(db::RecursiveShapeIterator(ly, tc, via1)));
  std::unique_ptr<db::Region> rmetal2(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal2)));
  std::unique_ptr<db::Region> rmetal2_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal2_lbl)));
...
TEST(9_FlatExtractionWithExternalDSS) {
  ...
  //  NOTE: we use a DSS from a LayoutToNetlist object - this one is initialized
  //  properly with the text representation settings.
  ...
  std::unique_ptr<db::Region> rbulk(new db::Region());
  std::unique_ptr<db::Region> rnwell(
      new db::Region(db::RecursiveShapeIterator(ly, tc, nwell), dss));
  std::unique_ptr<db::Region> rthickgox(
      new db::Region(db::RecursiveShapeIterator(ly, tc, thickgox), dss));
  std::unique_ptr<db::Region> ractive(
      new db::Region(db::RecursiveShapeIterator(ly, tc, active), dss));
  std::unique_ptr<db::Region> rpplus(
      new db::Region(db::RecursiveShapeIterator(ly, tc, pplus), dss));
  std::unique_ptr<db::Region> rnplus(
      new db::Region(db::RecursiveShapeIterator(ly, tc, nplus), dss));
  std::unique_ptr<db::Region> rpoly(
      new db::Region(db::RecursiveShapeIterator(ly, tc, poly), dss));
  std::unique_ptr<db::Region> rpoly_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, poly_lbl), dss));
  std::unique_ptr<db::Region> rcont(
      new db::Region(db::RecursiveShapeIterator(ly, tc, cont), dss));
  std::unique_ptr<db::Region> rmetal1(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal1), dss));
  std::unique_ptr<db::Region> rmetal1_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal1_lbl), dss));
  std::unique_ptr<db::Region> rvia1(
      new db::Region(db::RecursiveShapeIterator(ly, tc, via1), dss));
  std::unique_ptr<db::Region> rmetal2(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal2), dss));
  std::unique_ptr<db::Region> rmetal2_lbl(
      new db::Region(db::RecursiveShapeIterator(ly, tc, metal2_lbl), dss));
...
TEST(10_Antenna) {
  ...
  std::unique_ptr<db::Region> rdiode(
      new db::Region(db::RecursiveShapeIterator(ly, tc, diode), dss));
  std::unique_ptr<db::Region> rpoly(
      new db::Region(db::RecursiveShapeIterator(ly, tc, poly), dss));
  std::unique_ptr<db::Region> rcont(
      new db::Region(db::RecursiveShapeIterator(ly, tc, cont), dss));
  ...
  rdiode->insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(1, 0)));
  rpoly->insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(6, 0)));
  rcont->insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(8, 0)));
  rmetal1->insert_into(&ly2, top2.cell_index(),
                       ly2.insert_layer(db::LayerProperties(9, 0)));
  rvia1->insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(11, 0)));
  rmetal2->insert_into(&ly2, top2.cell_index(),
                       ly2.insert_layer(db::LayerProperties(12, 0)));

  {
    ...
    */
    ...
    a1_3.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(100, 0)));
    a1_10.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(101, 0)));
    a1_30.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(102, 0)));
  }

  {
    ...
    a2_5.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(200, 0)));
    a2_10.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(201, 0)));
    a2_17.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(202, 0)));
  }

  {
    ...
    a3_3.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(300, 0)));
    a3_10.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(301, 0)));
    a3_30.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(302, 0)));
  }

  {
    ...
    a4_3.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(400, 0)));
    a4_10.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(401, 0)));
    a4_30.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(402, 0)));
  }

  {
    ...
    a5_5.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(500, 0)));
    a5_15.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(501, 0)));
    a5_29.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(502, 0)));

    //  with area factor
    db::Region b5_5 =
        l2n.antenna_check(*rpoly, 2.0, 0.0, *rmetal2, 1.0, 1.0, 2.5);
    db::Region b5_15 =
        l2n.antenna_check(*rpoly, 2.0, 0.0, *rmetal2, 1.0, 1.0, 7.5);
    db::Region b5_29 =
        l2n.antenna_check(*rpoly, 2.0, 0.0, *rmetal2, 1.0, 1.0, 14.5);

    b5_5.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(550, 0)));
    b5_15.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(551, 0)));
    b5_29.insert_into(&ly2, top2.cell_index(),
                      ly2.insert_layer(db::LayerProperties(552, 0)));
  }

  {
    ...
    a6_3.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(600, 0)));
    a6_5.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(601, 0)));
    a6_9.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(602, 0)));

    //  with area factor
    db::Region b6_3 =
        l2n.antenna_check(*rpoly, 1.0, 0.3, *rmetal2, 2.0, 0.0, 6);
    db::Region b6_5 =
        l2n.antenna_check(*rpoly, 1.0, 0.3, *rmetal2, 2.0, 0.0, 10);
    db::Region b6_9 =
        l2n.antenna_check(*rpoly, 1.0, 0.3, *rmetal2, 2.0, 0.0, 18);

    b6_3.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(650, 0)));
    b6_5.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(651, 0)));
    b6_9.insert_into(&ly2, top2.cell_index(),
                     ly2.insert_layer(db::LayerProperties(652, 0)));
  }
...
TEST(11_DuplicateInstances) {
  ...
  db::compare_netlist(
      ...
      "  subcircuit BLOCK $6 "
      "($1=$I29,$2=VDD,$3=VSS,$4=$I11,$5=$I5,$6=$I4,$7=$I12);\n"
...
TEST(12_FlattenCircuitDoesFlattenLayout) {
  ...
  db::compare_netlist(_this, *l2n.netlist(),
                      "circuit RINGO ();\n"
                      "  device PMOS $1 (S=FB,G=$I7,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $2 (S=VDD,G=$I7,D=FB,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $3 (S=FB,G=$I7,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $4 (S=VSS,G=$I7,D=FB,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $5 (S=OSC,G=FB,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $6 (S=VDD,G=FB,D=OSC,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $7 (S=OSC,G=FB,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $8 (S=VSS,G=FB,D=OSC,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $9 (S=$I22,G=FB,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $10 (S=VDD,G=FB,D=$I22,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $11 (S=$I22,G=FB,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $12 (S=VSS,G=FB,D=$I22,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $13 (S=$I13,G=$I22,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $14 (S=VDD,G=$I22,D=$I13,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $15 (S=$I13,G=$I22,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $16 (S=VSS,G=$I22,D=$I13,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $17 (S=$I23,G=$I13,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $18 (S=VDD,G=$I13,D=$I23,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $19 (S=$I23,G=$I13,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $20 (S=VSS,G=$I13,D=$I23,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $21 (S=$I5,G=$I23,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $22 (S=VDD,G=$I23,D=$I5,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $23 (S=$I5,G=$I23,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $24 (S=VSS,G=$I23,D=$I5,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $25 (S=$I24,G=$I5,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $26 (S=VDD,G=$I5,D=$I24,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $27 (S=$I24,G=$I5,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $28 (S=VSS,G=$I5,D=$I24,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $29 (S=$I6,G=$I24,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $30 (S=VDD,G=$I24,D=$I6,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $31 (S=$I6,G=$I24,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $32 (S=VSS,G=$I24,D=$I6,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $33 (S=$I25,G=$I6,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $34 (S=VDD,G=$I6,D=$I25,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $35 (S=$I25,G=$I6,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $36 (S=VSS,G=$I6,D=$I25,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device PMOS $37 (S=$I7,G=$I25,D=VDD,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device PMOS $38 (S=VDD,G=$I25,D=$I7,B=VDD) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "  device NMOS $39 (S=$I7,G=$I25,D=VSS,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.91875,AD=0.48125,PS=4.55,PD=2.3);\n"
                      "  device NMOS $40 (S=VSS,G=$I25,D=$I7,B=VSS) "
                      "(L=0.25,W=1.75,AS=0.48125,AD=0.91875,PS=2.3,PD=4.55);\n"
                      "end;\n");
...
TEST(13_JoinNetNames) {
  ...
  db::compare_netlist(
      ...
      "circuit RINGO ();\n"
      "  subcircuit INV2 $1 "
      "(IN=$I7,$2=FB,OUT=OSC,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $2 "
      "(IN=FB,$2=$I34,OUT=$I17,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $3 "
      "(IN=$3,$2=$I38,OUT=$I4,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $4 "
      "(IN=$I2,$2=$I37,OUT=$3,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $5 "
      "(IN=$I4,$2=$I39,OUT=$I5,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $6 "
      "(IN=$I5,$2=$I40,OUT=$I6,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $7 "
      "(IN=$I6,$2=$I41,OUT=$I7,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $8 "
      "(IN=$I17,$2=$I35,OUT=$I1,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "  subcircuit INV2 $9 "
      "(IN=$I1,$2=$I36,OUT=$I2,$4=VSS,$5=VDD,VDD=VDD,VSS=VSS);\n"
      "end;\n"
      "circuit INV2 (IN=IN,$2=$2,OUT=OUT,$4=$4,$5=$5,VDD=VDD,VSS=VSS);\n"
      "  device PMOS $1 (S=$2,G=IN,D=$5,B=VDD) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S=$5,G=$2,D=OUT,B=VDD) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $3 (S=$2,G=IN,D=$4,B=VSS) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=$4,G=$2,D=OUT,B=VSS) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  subcircuit TRANS $1 ($1=$2,$2=$4,$3=IN);\n"
      "  subcircuit TRANS $2 ($1=$2,$2=$5,$3=IN);\n"
      "  subcircuit TRANS $3 ($1=$5,$2=OUT,$3=$2);\n"
...
TEST(14_JoinNets) {
  ...
  std::unique_ptr<db::Region> rnwell_lbl(
      l2n.make_layer(nwell_lbl, "nwell_lbl"));
  std::unique_ptr<db::Region> ractive(l2n.make_layer(active, "active"));
  std::unique_ptr<db::Region> rpplus(l2n.make_layer(pplus, "pplus"));
  std::unique_ptr<db::Region> rnplus(l2n.make_layer(nplus, "nplus"));
  std::unique_ptr<db::Region> rpoly(l2n.make_polygon_layer(poly, "poly"));
  ...
  //  This will trigger an implicit connection on top level (side effect of
  ...
  db::compare_netlist(
      ...
      "  subcircuit INV2 $5 "
      "('NWELL,VDD'=VDD,IN=$I12,$3=$I31,OUT=$I13,VSS=VSS);\n"
      "  subcircuit INV2 $6 "
      "('NWELL,VDD'=VDD,IN=$I13,$3=$I32,OUT=$I14,VSS=VSS);\n"
      "  subcircuit INV2 $7 "
      "('NWELL,VDD'=VDD,IN=$I14,$3=$I33,OUT=$I15,VSS=VSS);\n"
      "  subcircuit INV2 $8 "
      "('NWELL,VDD'=VDD,IN=$I25,$3=$I27,OUT=$I9,VSS=VSS);\n"
      "  subcircuit INV2 $9 "
      "('NWELL,VDD'=VDD,IN=$I9,$3=$I28,OUT=$I10,VSS=VSS);\n"
      "end;\n"
      "circuit INV2 ('NWELL,VDD'='NWELL,VDD',IN=IN,$3=$3,OUT=OUT,VSS=VSS);\n"
      "  device PMOS $1 (S=$3,G=IN,D='NWELL,VDD',B='NWELL,VDD') "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device PMOS $2 (S='NWELL,VDD',G=$3,D=OUT,B='NWELL,VDD') "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
      "  device NMOS $3 (S=$3,G=IN,D=VSS,B=VSS) "
      "(L=0.25,W=0.95,AS=0.49875,AD=0.26125,PS=2.95,PD=1.5);\n"
      "  device NMOS $4 (S=VSS,G=$3,D=OUT,B=VSS) "
      "(L=0.25,W=0.95,AS=0.26125,AD=0.49875,PS=1.5,PD=2.95);\n"
