...
TEST(1) {
  ...
  db::DeepLayer dl1 = store.create_polygon_layer(
      db::RecursiveShapeIterator(layout, layout.cell(c1), l1));
  db::DeepLayer dl2 = store.create_polygon_layer(
      db::RecursiveShapeIterator(layout, layout.cell(c1), l2));
  ...
  db::DeepLayer dl3 = store.create_polygon_layer(
      db::RecursiveShapeIterator(layout, layout.cell(c2), l1));
  EXPECT_EQ(dl3.layer(), l1);
  EXPECT_NE(&dl1.layout(), &dl3.layout());
  EXPECT_EQ(store.layouts(), (unsigned int)2);

  db::DeepLayer dl4 = store.create_polygon_layer(db::RecursiveShapeIterator(
      layout, layout.cell(c1), l1, db::Box(0, 1, 2, 3)));
  db::DeepLayer dl5 = store.create_polygon_layer(db::RecursiveShapeIterator(
      layout, layout.cell(c1), l2, db::Box(0, 1, 2, 3)));
  EXPECT_EQ(dl4.layer(), l1);
  EXPECT_EQ(dl5.layer(), l1); //  not l2, because it's a new layout
  EXPECT_EQ(store.layouts(), (unsigned int)4);

  db::DeepLayer dl6 = store.create_polygon_layer(db::RecursiveShapeIterator(
      layout, layout.cell(c1), l1, db::Box(0, 1, 2, 3)));
...
TEST(3_TextTreatment) {
  ...
  unsigned int l1 = layout.insert_layer();
  db::cell_index_type c1 = layout.add_cell("C1");
  layout.cell(c1).shapes(l1).insert(
      db::Text("TEXT", db::Trans(db::Vector(1000, 2000))));
  ...
  dl1 = store.create_polygon_layer(
      db::RecursiveShapeIterator(layout, layout.cell(c1), l1));
  EXPECT_EQ(store.layouts(), (unsigned int)1);

  EXPECT_EQ(dl1.initial_cell().shapes(dl1.layer()).size(), size_t(1));
  EXPECT_EQ(dl1.initial_cell()
                .shapes(dl1.layer())
                .begin(db::ShapeIterator::All)
                ->to_string(),
            "polygon (999,1999;999,2001;1001,2001;1001,1999)");
  ...
  EXPECT_EQ(dl1.initial_cell()
                ...
                .begin(db::ShapeIterator::All)
                ->to_string(),
            "polygon (999,1999;999,2001;1001,2001;1001,1999) prop_id=1");

  const db::Layout *dss_layout = &store.const_layout(0);
  db::PropertiesRepository::properties_set ps =
      dss_layout->properties_repository().properties(1);
  EXPECT_EQ(ps.size(), size_t(1));
  EXPECT_EQ(dss_layout->properties_repository()
                .prop_name(ps.begin()->first)
                .to_string(),
            "text");
...
TEST(4_FlatAndEmptyInput) {
  ...
  EXPECT_EQ((dr1 - dr2).to_string(), "(0,0;0,900;100,900;100,100;900,100;900,"
                                     "900;0,900;0,1000;1000,1000;1000,0)");
...
TEST(5_State) {
...

  {
    ...
    EXPECT_EQ(store.breakout_cells(0)->find(5) !=
                  store.breakout_cells(0)->end(),
              true);
    EXPECT_EQ(store.breakout_cells(0)->find(3) !=
                  store.breakout_cells(0)->end(),
              false);
  ...
  EXPECT_EQ(store.breakout_cells(0) == 0, false);
  EXPECT_EQ(store.breakout_cells(0)->find(5) != store.breakout_cells(0)->end(),
            true);
  EXPECT_EQ(store.breakout_cells(0)->find(3) != store.breakout_cells(0)->end(),
            true);
}
