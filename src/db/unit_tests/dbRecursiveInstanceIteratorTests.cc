...
std::string collect(db::RecursiveInstanceIterator &s,
...
std::string collect_with_copy(db::RecursiveInstanceIterator s,
                              const db::Layout &layout) {
  s.reset();
  return collect(s, layout);
}

std::string collect2(db::RecursiveInstanceIterator &s,
                     const db::Layout &layout) {
  std::string res;
  while (!s.at_end()) {
    if (!res.empty()) {
    ...
    res += std::string(layout.cell_name(s->inst_ptr.cell_index())) + "@" +
           (s.trans() * s.instance().complex_trans()).to_string();
    ++s;
  }
  return res;
}
...
TEST(1) {
  ...
  x = collect(i4, g);
  EXPECT_EQ(x,
            "[$1]$2 r0 0,0/[$3]$4 r0 1100,0/[$1]$3 r0 100,-100/[$1]$4 r90 0,0");
  x = collect_with_copy(i4, g);
  EXPECT_EQ(x,
            "[$1]$2 r0 0,0/[$3]$4 r0 1100,0/[$1]$3 r0 100,-100/[$1]$4 r90 0,0");
...
class FlatPusher : public db::RecursiveInstanceReceiver {
public:
  FlatPusher(std::set<db::Box> *boxes) : mp_boxes(boxes) {}

  void enter_cell(const db::RecursiveInstanceIterator *iter,
...
TEST(2) {
  ...
  for (db::RecursiveInstanceIterator iter =
           db::RecursiveInstanceIterator(g, c0, search_box, true);
       !iter.at_end(); ++iter) {
  ...
  for (db::RecursiveInstanceIterator iter =
           db::RecursiveInstanceIterator(g, c0, reg, true);
       !iter.at_end(); ++iter) {
    selected_boxes.insert(iter.trans() *
                          iter->bbox(db::box_convert<db::CellInst>(g)));
  }
...
TEST(3) {
  ...
  for (int i = 0; i < nboxes; ++i) {
    ...
    do {
      x = rand() % 10000;
      y = rand() % 10000;
    } while (boxes.find(basic_box.moved(db::Vector(x + 1, y - 1))) !=
             boxes.end());
  ...
  db::RecursiveInstanceIterator iter =
      db::RecursiveInstanceIterator(g, c0, search_box, true);
...
TEST(4) {
  ...
  db::RecursiveInstanceIterator iter =
      db::RecursiveInstanceIterator(g, c0, search_box);
  std::set<db::cell_index_type> tc;
  tc.insert(c2.cell_index());
  iter.set_targets(tc);
  int n = 0;
  ...
  for (db::RecursiveInstanceIterator iter =
           db::RecursiveInstanceIterator(g, c0, reg);
       !iter.at_end(); ++iter) {
    selected_boxes.insert(iter.trans() *
                          iter->bbox(db::box_convert<db::CellInst>(g)));
  }
...
TEST(6) {
  ...
  x = collect2(i1, layout);
  EXPECT_EQ(x, "CHILD_CELL_3_1_1@r0 *1 30000,0\n"
               "CHILD_CELL_3_1_1@r0 *1 55000,0\n"
               "CHILD_CELL_3_1_1@r0 *1 55000,20000\n"
               "CHILD_CELL_3_1_1@r0 *1 55000,40000\n"
               "CHILD_CELL_3_1_1@r0 *1 75000,0\n"
  ...
  x = collect2(i2, layout);
  EXPECT_EQ(x, "CHILD_CELL_3_1_1@r0 *1 120000,20000\n"
               "CHILD_CELL_3_1_1@r0 *1 120000,40000\n"
               "CHILD_CELL_3_1_1@r0 *1 120000,0");
}
