...
static std::string c2s(const std::vector<db::Point> &c) {
  std::string s;
  for (std::vector<db::Point>::const_iterator p = c.begin(); p != c.end();
       ++p) {
    if (!s.empty()) {
      s += ";";
    }
...
TEST(1) {
  db::Edge edges[] = {db::Edge(db::Point(0, 0), db::Point(100, 0)),
...
TEST(2) {
  db::Edge edges[] = {db::Edge(db::Point(-100, -100), db::Point(100, -100)),
  ...
  EXPECT_EQ(e2c.contours(), size_t(1));
  EXPECT_EQ(c2s(e2c.contour(0)),
            "100,-100;0,0;200,-50;0,100;-200,-50;0,0;-100,-100");
  EXPECT_EQ(e2c.contour_closed(0), true);
  ...
  EXPECT_EQ(e2c.contours(), size_t(1));
  EXPECT_EQ(c2s(e2c.contour(0)),
            "0,100;-200,-50;0,0;100,-100;-100,-100;0,0;200,-50");
  EXPECT_EQ(e2c.contour_closed(0), true);
}
...
TEST(4) {
  db::Edge edges[] = {db::Edge(db::Point(0, 0), db::Point(0, 100)),
  ...
  EXPECT_EQ(
      c2s(e2c.contour(0)),
      "0,100;-100,100;-100,200;200,200;200,100;0,100;0,200;100,200;100,0;0,0");
  EXPECT_EQ(e2c.contour_closed(0), true);
}
...
TEST(5) {
...

  {
    ...
    EXPECT_EQ(e2c.contours(), size_t(1));
    EXPECT_EQ(c2s(e2c.contour(0)), "0,0;0,100;-100,100;-100,200;200,200;200,"
                                   "100;0,100;0,200;100,200;100,0");
    EXPECT_EQ(e2c.contour_closed(0), false);
  }
...

  {
    ...
    EXPECT_EQ(e2c.contours(), size_t(1));
    EXPECT_EQ(c2s(e2c.contour(0)), "1,1;0,100;-100,100;-100,200;200,200;200,"
                                   "100;0,100;0,200;100,200;100,0");
    EXPECT_EQ(e2c.contour_closed(0), false);
  }
}
...
TEST(7) {
...

  {
    ...
    EXPECT_EQ(e2c.contours(), size_t(1));
    EXPECT_EQ(c2s(e2c.contour(0)), "0,100;200,100;400,100;400,0;300,0;300,100;"
                                   "200,100;200,0;200,100;100,100;100,0;0,0");
    EXPECT_EQ(e2c.contour_closed(0), true);
  }
...
TEST(8) {
  db::Edge edges[] = {db::Edge(db::Point(100, 100), db::Point(200, 100)),
                      db::Edge(db::Point(100, 100), db::Point(100, 200)),
