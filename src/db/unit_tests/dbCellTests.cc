...
TEST(1) {
  ...
  int n;
  n = 0;
  for (db::Cell::touching_iterator r =
           c2.begin_touching(t * db::Box(-100, 0, 0, 100));
       !r.at_end(); ++r)
    ++n;
  EXPECT_EQ(n, 1);
  n = 0;
  for (db::Cell::overlapping_iterator r =
           c2.begin_overlapping(t * db::Box(-100, 0, 0, 100));
       !r.at_end(); ++r)
    ++n;
  EXPECT_EQ(n, 0);
  n = 0;
  for (db::Cell::overlapping_iterator r =
           c2.begin_overlapping(t * db::Box(-100, 0, 1, 100));
       !r.at_end(); ++r)
    ++n;
  EXPECT_EQ(n, 1);
  n = 0;
  for (db::Cell::touching_iterator r =
           c2.begin_touching(t * db::Box(-100, 0, -1, 100));
       !r.at_end(); ++r)
    ++n;
  EXPECT_EQ(n, 0);
...
struct p2s_compare {
  bool operator()(const db::Cell::parent_inst_iterator &p1,
                  const db::Cell::parent_inst_iterator &p2) {
    if (p1->inst().object().cell_index() != p2->inst().object().cell_index()) {
      return p1->inst().object().cell_index() <
             p2->inst().object().cell_index();
    }
...
std::string p2s(const db::Cell &c) {
  ...
  for (db::Cell::parent_inst_iterator p = c.begin_parent_insts(); !p.at_end();
       ++p) {
    pp.push_back(p);
  }
  std::sort(pp.begin(), pp.end(), p2s_compare());
  std::string r;
  for (std::vector<db::Cell::parent_inst_iterator>::const_iterator ppp =
           pp.begin();
       ppp != pp.end(); ++ppp) {
    ...
    r += tl::to_string(p->inst().object().cell_index()) + "[" +
         p->inst().complex_trans().to_string() + "]" + "#" +
         tl::to_string(p->child_inst().prop_id());
  }
  return r;
}

struct pc2s_compare {
  bool operator()(const db::Cell::parent_cell_iterator &p1,
                  const db::Cell::parent_cell_iterator &p2) {
    return *p1 < *p2;
  }
};
...
std::string pc2s(const db::Cell &c) {
  ...
  for (db::Cell::parent_cell_iterator p = c.begin_parent_cells();
       p != c.end_parent_cells(); ++p) {
    pp.push_back(p);
  }
  std::sort(pp.begin(), pp.end(), pc2s_compare());
  std::string r;
  for (std::vector<db::Cell::parent_cell_iterator>::const_iterator ppp =
           pp.begin();
       ppp != pp.end(); ++ppp) {
...
struct c2s_compare {
  bool operator()(const db::Cell::const_iterator &p1,
                  const db::Cell::const_iterator &p2) {
...
std::string c2s(const db::Cell &c) {
  ...
  std::string r;
  for (std::vector<db::Cell::const_iterator>::const_iterator ppp = pp.begin();
       ppp != pp.end(); ++ppp) {
    db::Cell::const_iterator p = *ppp;
    if (!r.empty()) {
      r += ",";
    }
    r += tl::to_string(p->cell_index()) + "[" + p->complex_trans().to_string() +
         "]" + "#" + tl::to_string(p->prop_id());
  }
  return r;
}
...
struct ct2s_compare {
  bool operator()(const db::Cell::touching_iterator &p1,
                  const db::Cell::touching_iterator &p2) {
    if (p1->cell_index() != p2->cell_index()) {
      return p1->cell_index() < p2->cell_index();
    }
    if (p1->complex_trans() != p2->complex_trans()) {
...
std::string ct2s(const db::Cell &c) {
  ...
  for (db::Cell::touching_iterator p =
           c.begin_touching(db::Box(-10000, -10000, 10000, 10000));
       !p.at_end(); ++p) {
    pp.push_back(p);
  }
  std::sort(pp.begin(), pp.end(), ct2s_compare());
  std::string r;
  for (std::vector<db::Cell::touching_iterator>::const_iterator ppp =
           pp.begin();
       ppp != pp.end(); ++ppp) {
...
struct cc2s_compare {
  bool operator()(const db::Cell::child_cell_iterator &p1,
                  const db::Cell::child_cell_iterator &p2) {
    return *p1 < *p2;
  }
};
...
std::string cc2s(const db::Cell &c) {
  ...
  for (db::Cell::child_cell_iterator p = c.begin_child_cells(); !p.at_end();
       ++p) {
    pp.push_back(p);
  }
  std::sort(pp.begin(), pp.end(), cc2s_compare());
  std::string r;
  for (std::vector<db::Cell::child_cell_iterator>::const_iterator ppp =
           pp.begin();
       ppp != pp.end(); ++ppp) {
...
void insert_ci(db::Cell &c, size_t ci, const db::Trans &t) {
  if (pi == 0) {
    ...
    c.insert(db::CellInstArrayWithProperties(
        db::CellInstArray(db::CellInst(db::cell_index_type(ci)), t), pi));
  }
  pi = (pi + 1) % 3;
}
...
TEST(2) {
  ...
  EXPECT_EQ(p2s(c3), "0[r0 *1 0,0]#0,1[r0 *1 0,0]#0,2[r0 *1 0,0]#2");
  EXPECT_EQ(p2s(c4),
            "0[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 0,0]#1,3[r0 *1 0,0]#0");
  EXPECT_EQ(p2s(c5),
            "1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 0,0]#2,4[r0 *1 0,0]#1");
  EXPECT_EQ(pc2s(c0), "");
  EXPECT_EQ(pc2s(c1), "0");
  EXPECT_EQ(pc2s(c2), "0,1");
  EXPECT_EQ(pc2s(c3), "0,1,2");
  ...
  EXPECT_EQ(c2s(c0),
            "1[r0 *1 0,0]#0,2[r0 *1 0,0]#2,3[r0 *1 0,0]#0,4[r0 *1 0,0]#0");
  EXPECT_EQ(c2s(c1),
            "2[r0 *1 0,0]#1,3[r0 *1 0,0]#0,4[r0 *1 0,0]#1,5[r0 *1 0,0]#1");
  EXPECT_EQ(c2s(c2), "3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  EXPECT_EQ(c2s(c3), "4[r0 *1 0,0]#0,5[r0 *1 0,0]#2");
  EXPECT_EQ(c2s(c4), "5[r0 *1 0,0]#1");
  EXPECT_EQ(c2s(c5), "");
  ...
  EXPECT_EQ(p2s(c4),
            "0[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 0,0]#1,3[r0 *1 0,0]#0");
  EXPECT_EQ(p2s(c5), "0[r0 *1 0,0]#2,1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 "
                     "0,0]#2,4[r0 *1 0,0]#1");
  EXPECT_EQ(pc2s(c0), "");
  EXPECT_EQ(pc2s(c1), "0");
  EXPECT_EQ(pc2s(c2), "0,1");
  ...
  EXPECT_EQ(c2s(c0), "1[r0 *1 0,0]#0,2[r0 *1 0,0]#2,3[r0 *1 0,0]#0,4[r0 *1 "
                     "0,0]#0,5[r0 *1 0,0]#2");
  EXPECT_EQ(c2s(c1),
            "2[r0 *1 0,0]#1,3[r0 *1 0,0]#0,4[r0 *1 0,0]#1,5[r0 *1 0,0]#1");
  EXPECT_EQ(c2s(c2), "3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  ...
  EXPECT_EQ(c2s(c4), "5[r0 *1 0,0]#1");
  EXPECT_EQ(c2s(c5), "");
  EXPECT_EQ(ct2s(c0), "1[r0 *1 0,0]#0,2[r0 *1 0,0]#2,3[r0 *1 0,0]#0,4[r0 *1 "
                      "0,0]#0,5[r0 *1 0,0]#2");
  EXPECT_EQ(ct2s(c1),
            "2[r0 *1 0,0]#1,3[r0 *1 0,0]#0,4[r0 *1 0,0]#1,5[r0 *1 0,0]#1");
  EXPECT_EQ(ct2s(c2), "3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  ...
  EXPECT_EQ(p2s(c3),
            "0[r0 *1 0,0]#0,1[r0 *1 0,0]#0,2[r0 *1 0,0]#0,2[r0 *1 0,0]#2");
  EXPECT_EQ(p2s(c4), "0[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 0,0]#1,3[r0 *1 "
                     "0,0]#0,3[r0 *1 0,0]#1");
  EXPECT_EQ(p2s(c5), "0[r0 *1 0,0]#2,1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 "
                     "0,0]#2,4[r0 *1 0,0]#1,4[r0 *1 0,0]#2");
  EXPECT_EQ(pc2s(c0), "");
  EXPECT_EQ(pc2s(c1), "0");
  EXPECT_EQ(pc2s(c2), "0,1");
  EXPECT_EQ(pc2s(c3), "0,1,2");
  EXPECT_EQ(pc2s(c4), "0,1,2,3");
  EXPECT_EQ(pc2s(c5), "0,1,2,3,4");
  EXPECT_EQ(c2s(c0), "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 "
                     "0,0]#0,4[r0 *1 0,0]#0,5[r0 *1 0,0]#2");
  EXPECT_EQ(c2s(c1), "2[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 0,0]#0,4[r0 *1 "
                     "0,0]#1,5[r0 *1 0,0]#1");
  EXPECT_EQ(c2s(c2),
            "3[r0 *1 0,0]#0,3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  ...
  EXPECT_EQ(c2s_unsorted(c0),
            "1[r0 *1 0,0]#0,3[r0 *1 0,0]#0,4[r0 *1 0,0]#0,2[r0 *1 0,0]#2,5[r0 "
            "*1 0,0]#2,1[r0 *1 0,0]#1");
  EXPECT_EQ(c2s_unsorted(c1), "3[r0 *1 0,0]#0,2[r0 *1 0,0]#1,4[r0 *1 "
                              "0,0]#1,5[r0 *1 0,0]#1,2[r0 *1 0,0]#2");
  EXPECT_EQ(c2s_unsorted(c2),
            "3[r0 *1 0,0]#0,3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  ...
  EXPECT_EQ(ct2s(c0), "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 "
                      "0,0]#0,4[r0 *1 0,0]#0,5[r0 *1 0,0]#2");
  EXPECT_EQ(ct2s(c1), "2[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 0,0]#0,4[r0 *1 "
                      "0,0]#1,5[r0 *1 0,0]#1");
  EXPECT_EQ(ct2s(c2),
            "3[r0 *1 0,0]#0,3[r0 *1 0,0]#2,4[r0 *1 0,0]#1,5[r0 *1 0,0]#2");
  ...
  EXPECT_EQ(c2s_unsorted(c0),
            "2[r90 *1 100,-200]#0,3[r0 *1 0,0]#0,4[r0 *1 0,0]#0,2[r0 *1 "
            "0,0]#2,5[r0 *1 0,0]#2,1[r0 *1 0,0]#1");
  ++inst;
  ++inst;
  ++inst;
  c0.replace(*inst,
             db::CellInstArrayWithProperties(
                 db::CellInstArray(db::CellInst(c3.cell_index()), t1), 17));
  EXPECT_EQ(c2s_unsorted(c0),
            "2[r90 *1 100,-200]#0,3[r0 *1 0,0]#0,4[r0 *1 0,0]#0,3[r90 *1 "
            "100,-200]#17,5[r0 *1 0,0]#2,1[r0 *1 0,0]#1");
  c0.replace_prop_id(*inst, 11);
  EXPECT_EQ(c2s_unsorted(c0),
            "2[r90 *1 100,-200]#0,3[r0 *1 0,0]#0,4[r0 *1 0,0]#0,3[r90 *1 "
            "100,-200]#11,5[r0 *1 0,0]#2,1[r0 *1 0,0]#1");
  inst = c0.begin();
  ...
  c0.replace(
      *inst,
      db::CellInstArrayWithProperties(
          db::CellInstArray(db::CellInst(c3.cell_index()), db::Trans()), 13));
  EXPECT_EQ(c2s_unsorted(c0),
            "3[r0 *1 0,0]#0,4[r0 *1 0,0]#0,3[r90 *1 100,-200]#11,5[r0 *1 "
            "0,0]#2,1[r0 *1 0,0]#1,3[r0 *1 0,0]#13");
  ...
  db::Instance i0 = *inst;
  c0.erase(inst);
  //  HINT: doing a c2s_unsorted on c0 would disturb the index order of c0,
  //  because it uses a flat iterator. Therefore we make a copy in order to
  //  prevent that problem. See bug #120.
  cx = c0;
  EXPECT_EQ(c2s_unsorted(cx), "4[r0 *1 0,0]#0,3[r90 *1 100,-200]#11,5[r0 *1 "
                              "0,0]#2,1[r0 *1 0,0]#1,3[r0 *1 0,0]#13");
  ...
  EXPECT_EQ(
      c2s_unsorted(cx),
      "3[r90 *1 100,-200]#11,5[r0 *1 0,0]#2,1[r0 *1 0,0]#1,3[r0 *1 0,0]#13");
  inst = c0.begin();
  db::Instance i2 = *inst;
  c0.erase(inst);
  cx = c0;
...
TEST(3) {
  ...
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t1), 1));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t2), 10));

  EXPECT_EQ(c2s_unsorted(c0),
            "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,1[r0 *1 100,-100]#10");
  ...
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t2), 17));
  EXPECT_EQ(c2s_unsorted(c0),
            "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,1[r0 *1 100,-100]#17");
  ...
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t2), 18));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t2), 21));
  EXPECT_EQ(c2s_unsorted(c0), "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,1[r0 *1 "
                              "100,-100]#18,1[r0 *1 100,-100]#21");
...
TEST(3a) {
  ...
  db::Instance inst =
      c0.insert(db::CellInstArray(db::CellInst(c1.cell_index()), t));
  ...
  EXPECT_EQ((ti * inst.complex_trans() * ti.inverted()).to_string(),
            "m90 *1 -333.553390593,0");
...
TEST(3b) {
  ...
  db::Trans t(db::Vector(100, -100));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t), 5));

  db::Box b(0, 100, 1000, 1200);
  c0.shapes(0).insert(db::BoxWithProperties(b, 17));
  c1.shapes(1).insert(b);

  //  Note: this requires editable mode since db::Shapes::erase is permitted in
  //  editable mode only (erase is triggered by undo)
  if (db::default_editable_mode()) {
    ...
    EXPECT_EQ(c0.shapes(0).begin(db::ShapeIterator::All)->to_string(),
              "box (0,250;2500,3000) prop_id=17");
    EXPECT_EQ(c1.shapes(1).begin(db::ShapeIterator::All)->to_string(),
              "box (0,250;2500,3000)");
    ...
    EXPECT_EQ(c0.shapes(0).begin(db::ShapeIterator::All)->to_string(),
              "box (0,100;1000,1200) prop_id=17");
    EXPECT_EQ(c1.shapes(1).begin(db::ShapeIterator::All)->to_string(),
              "box (0,100;1000,1200)");
...
TEST(3c) {
  ...
  if (db::default_editable_mode()) {
    ...
    EXPECT_EQ(c1.cell_instances(), size_t(0));
    EXPECT_EQ(c0.cell_instances(), size_t(1));
    EXPECT_EQ(c0.begin()->to_string(),
              "cell_index=1 r0 *2.5 250,-250 prop_id=5");
...
TEST(4) {
  ...
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), t1), 1));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c2.cell_index()), t2), 10));

  EXPECT_EQ(c2s_unsorted(c0),
            "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 100,-100]#10");

  db::Cell &c3(g.cell(g.add_cell()));
  for (db::Cell::const_iterator i = c0.begin(); !i.at_end(); ++i) {
    c3.insert(*i);
  }
  EXPECT_EQ(c2s_unsorted(c3),
            "1[r0 *1 0,0]#0,1[r0 *1 0,0]#1,2[r0 *1 100,-100]#10");
  ...
  EXPECT_EQ(c2s_unsorted(c4),
            "2[r0 *1 0,0]#0,2[r0 *1 0,0]#4,1[r0 *1 100,-100]#13");
}
...
TEST(5) {
  ...
  db::Trans trans[] = {db::Trans(), db::Trans(1, db::Vector(100, -200)),
                       db::Trans(6, db::Vector(-20, 1000))};

  db::Trans tt;
  for (unsigned int p = 0; p < 1000; ++p) {
    if ((p % 17) == 0) {
      c0.insert(db::CellInstArray(
          db::CellInst(cells[(p * 23) % (sizeof(cells) / sizeof(cells[0]))]
                           ->cell_index()),
          tt));
    } else {
      c0.insert(db::CellInstArrayWithProperties(
  ...
  std::string r;
  for (db::Cell::child_cell_iterator cc = c0.begin_child_cells(); !cc.at_end();
       ++cc) {
    if (r != "") {
      r += ",";
    }
...
TEST(6) {
  ...
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c1.cell_index()), tt), 0));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c2.cell_index()), tt), 1));
  c0.insert(db::CellInstArrayWithProperties(
      db::CellInstArray(db::CellInst(c3.cell_index()), tt), 2));
  ...
  EXPECT_EQ(c2s_unsorted(c0),
            "1[r0 *1 0,0]#0,2[r0 *1 0,0]#0,3[r0 *1 0,0]#0,1[r0 *1 0,0]#0,2[r0 "
            "*1 0,0]#1,3[r0 *1 0,0]#2");

  db::Cell &cc(g.cell(g.add_cell()));
  cc = c0;

  EXPECT_EQ(c2s_unsorted(cc),
            "1[r0 *1 0,0]#0,2[r0 *1 0,0]#0,3[r0 *1 0,0]#0,1[r0 *1 0,0]#0,2[r0 "
            "*1 0,0]#1,3[r0 *1 0,0]#2");
  ...
  if (db::default_editable_mode()) {
    ...
    EXPECT_EQ(c2s_unsorted(cc),
              "1[r0 *1 0,0]#1,2[r0 *1 0,0]#2,3[r0 *1 0,0]#3,1[r0 *1 "
              "0,0]#1,2[r0 *1 0,0]#1,3[r0 *1 0,0]#1");

    for (db::Cell::const_iterator i = c0.begin(); !i.at_end(); ++i) {
      c0.replace(*i, db::CellInstArrayWithProperties(i->cell_inst(),
                                                     i->prop_id() + 1));
    }
...
TEST(11_HasShapesTouchingWithHier) {
  ...
  a.insert(db::CellInstArray(b.cell_index(), db::Trans(db::Vector(100, 100)),
                             db::Vector(0, 200), db::Vector(200, 0), 2, 2));
