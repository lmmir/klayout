...
class PD : public db::PCellDeclaration {
...
TEST(0) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, layout,
                      tl::testdata() + "/gds/pcell_test_0_au.gds",
                      db::NoNormalization);
}
...
TEST(1) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, layout, tl::testdata() + "/gds/pcell_test.gds",
                      db::NoNormalization);
  ...
  if (db::default_editable_mode()) {
    ...
    CHECKPOINT();
    db::compare_layouts(_this, layout, tl::testdata() + "/gds/pcell_test2.gds",
                        db::NoNormalization);
    ...
    CHECKPOINT();
    db::compare_layouts(_this, layout, tl::testdata() + "/gds/pcell_test3.gds",
                        db::WriteGDS2);
    CHECKPOINT();
    db::compare_layouts(_this, layout, tl::testdata() + "/gds/pcell_test3.gds",
                        db::WriteOAS);
    ...
    CHECKPOINT();
    db::compare_layouts(_this, copy, tl::testdata() + "/gds/pcell_test.gds",
                        db::NoNormalization);

    db::Cell &copy_top = copy.cell(top.cell_index());

    db::Instance i1_copy = *(copy_top.begin());
    const db::PCellVariant *pcv = dynamic_cast<const db::PCellVariant *>(
        &copy.cell(i1_copy.cell_index()));
    ...
    std::vector<tl::Variant> parameters =
        copy_top.get_pcell_parameters(i1_copy);
    ...
    CHECKPOINT();
    db::compare_layouts(_this, copy, tl::testdata() + "/gds/pcell_test4.gds",
                        db::WriteGDS2);
    CHECKPOINT();
    db::compare_layouts(_this, copy, tl::testdata() + "/gds/pcell_test4.gds",
                        db::WriteOAS);
