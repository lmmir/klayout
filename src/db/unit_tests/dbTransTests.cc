...
TEST(6) {
  ...
  EXPECT_EQ((tt3d * db::DVector(1, 0)).to_string(),
            (tt2 * db::DVector(1, 0)).to_string());
  EXPECT_EQ((tt3d * db::DVector(0, 1)).to_string(),
            (tt2 * db::DVector(0, 1)).to_string());
  EXPECT_EQ((tt3d * db::DVector(0, 0)).to_string(),
            (tt2 * db::DVector(0, 0)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(1, 0)).to_string(),
            (tt2 * db::DPoint(1, 0)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(0, 1)).to_string(),
            (tt2 * db::DPoint(0, 1)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(0, 0)).to_string(),
            (tt2 * db::DPoint(0, 0)).to_string());

  db::Matrix2d tt2d = tt2.to_matrix2d();
  EXPECT_EQ((tt2d * db::DVector(1, 0)).to_string(),
            (tt2 * db::DVector(1, 0)).to_string());
  EXPECT_EQ((tt2d * db::DVector(0, 1)).to_string(),
            (tt2 * db::DVector(0, 1)).to_string());
  EXPECT_EQ((tt2d * db::DVector(0, 0)).to_string(),
            (tt2 * db::DVector(0, 0)).to_string());
}

template <class T> T recomposed(const T &t) {
  typedef typename T::target_coord_type tt;
  T r(db::complex_trans<tt, tt>(
      db::simple_trans<tt>(db::complex_trans<tt, tt>(t)), t.rcos(), t.mag()));
  return r;
}
...
TEST(10) {
  ...
  EXPECT_EQ(t.to_matrix2d().to_string(),
            db::DFTrans(db::DFTrans::r90).to_matrix2d().to_string());
  EXPECT_EQ(t.to_matrix3d().to_string(),
            db::DFTrans(db::DFTrans::r90).to_matrix3d().to_string());
  ...
  t = db::DCplxTrans(
      db::ICplxTrans(db::disp_trans<db::Coord>(db::Vector(100, -256))));
  EXPECT_EQ(t.is_unity(), false);
  ...
  t = db::DCplxTrans(db::DTrans(
      db::simple_trans<db::Coord>(db::FTrans::m135, db::Vector(128, -256))));
  EXPECT_EQ(t.is_unity(), false);
  EXPECT_EQ(t.to_string(), "m135 *1 128,-256");
  EXPECT_EQ(t.to_string(), recomposed(t).to_string());
  t = db::DCplxTrans(
      db::simple_trans<double>(db::DFTrans::r180, db::DVector(-0.25, 1.5)));
  ...
  EXPECT_EQ(t.is_unity(), false);
  EXPECT_EQ(t.to_string(), db::DCplxTrans(0.15, 0.2, false,
                                          db::DVector(17 * 0.01, -34 * 0.001))
                               .to_string());
  EXPECT_NE(t.to_string(), db::DCplxTrans(0.15, 0.21, false,
                                          db::DVector(17 * 0.01, -34 * 0.001))
                               .to_string());
  EXPECT_EQ(t.to_string(), db::DCplxTrans(0.15, 0.2, false,
                                          db::DVector(17 * 0.01, -34 * 0.001))
  ...
  EXPECT_NE(t.to_string(), db::DCplxTrans(0.15, 0.21, false,
                                          db::DVector(17 * 0.01, -34 * 0.001))
                               .to_string());
  EXPECT_EQ(t < db::DCplxTrans(0.15, 0.21, false,
                               db::DVector(17 * 0.01, -34 * 0.001)),
            true);
  EXPECT_EQ(db::DCplxTrans(0.15, 0.21, false,
                           db::DVector(17 * 0.01, -34 * 0.001)) < t,
            false);
  EXPECT_EQ(t.to_string(), recomposed(t).to_string());
  ...
  EXPECT_EQ((tt3d * db::Vector(1, 0)).to_string(),
            (tt * db::Vector(1, 0)).to_string());
  EXPECT_EQ((tt3d * db::Vector(0, 1)).to_string(),
            (tt * db::Vector(0, 1)).to_string());
  EXPECT_EQ((tt3d * db::Vector(0, 0)).to_string(),
            (tt * db::Vector(0, 0)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(1, 0)).to_string(),
            (tt * db::DPoint(1, 0)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(0, 1)).to_string(),
            (tt * db::DPoint(0, 1)).to_string());
  EXPECT_EQ((tt3d * db::DPoint(0, 0)).to_string(),
            (tt * db::DPoint(0, 0)).to_string());

  db::Matrix2d tt2d = tt.to_matrix2d();
  EXPECT_EQ((tt2d * db::DVector(1, 0)).to_string(),
            (tt * db::DVector(1, 0)).to_string());
  EXPECT_EQ((tt2d * db::DVector(0, 1)).to_string(),
            (tt * db::DVector(0, 1)).to_string());
  EXPECT_EQ((tt2d * db::DVector(0, 0)).to_string(),
            (tt * db::DVector(0, 0)).to_string());
...
TEST(11) {
  ...
  db::DCplxTrans t3(db::DTrans(0, true, db::DVector(12.4, -17.0)),
                    cos(45 * M_PI / 180.0), 0.55);
