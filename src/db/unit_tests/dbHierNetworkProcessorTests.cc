...
static std::string l2s(db::Connectivity::layer_iterator b,
                       db::Connectivity::layer_iterator e) {
  std::string s;
...
static std::string gn2s(db::Connectivity::global_nets_iterator b,
                        db::Connectivity::global_nets_iterator e) {
  std::string s;
...
TEST(1_Connectivity) {
  ...
  EXPECT_EQ(
      gn2s(conn.begin_global_connections(2), conn.end_global_connections(2)),
      "");
  EXPECT_EQ(
      gn2s(conn.begin_global_connections(0), conn.end_global_connections(0)),
      "0");
  ...
  EXPECT_EQ(
      gn2s(conn2.begin_global_connections(0), conn2.end_global_connections(0)),
      "0,1");
...
TEST(10_LocalClusterBasic) {
  ...
  cluster2.add(
      db::PolygonRef(poly, repo).transformed(db::Trans(db::Vector(10, 20))), 1);
  cluster2.add_attr(2);
...
TEST(11_LocalClusterInteractBasic) {
  ...
  EXPECT_EQ(cluster.interacts(
                cluster2, db::ICplxTrans(db::Trans(db::Vector(10, 20))), conn),
            true);
  EXPECT_EQ(cluster.interacts(
                cluster2, db::ICplxTrans(db::Trans(db::Vector(0, 1000))), conn),
            true);
  EXPECT_EQ(cluster.interacts(
                cluster2, db::ICplxTrans(db::Trans(db::Vector(0, 1001))), conn),
            false);
  EXPECT_EQ(cluster.interacts(
                cluster2, db::ICplxTrans(db::Trans(db::Vector(0, 2000))), conn),
            false);
...
TEST(11_LocalClusterInteractDifferentLayers) {
  ...
  EXPECT_EQ(cluster.interacts(cluster2, db::ICplxTrans(), conn),
            false); //  not connected
...
static std::string obj2string(const db::Edge &ref) { return ref.to_string(); }

template <class T>
static std::string local_cluster_to_string(const db::local_cluster<T> &cluster,
                                           const db::Connectivity &conn) {
  std::string res;
  for (db::Connectivity::layer_iterator l = conn.begin_layers();
       l != conn.end_layers(); ++l) {
    for (typename db::local_cluster<T>::shape_iterator s = cluster.begin(*l);
         !s.at_end(); ++s) {
      if (!res.empty()) {
        res += ";";
      }
  ...
  for (typename db::local_cluster<T>::attr_iterator a = cluster.begin_attr();
       a != cluster.end_attr(); ++a) {
    res += "%" + tl::to_string(*a);
  }
  for (typename db::local_cluster<T>::global_nets_iterator g =
           cluster.begin_global_nets();
       g != cluster.end_global_nets(); ++g) {
...
template <class T>
static std::string
local_clusters_to_string(const db::local_clusters<T> &clusters,
                         const db::Connectivity &conn) {
  std::string s;
  for (typename db::local_clusters<T>::const_iterator c = clusters.begin();
       c != clusters.end(); ++c) {
    if (!s.empty()) {
      s += "\n";
    }
...
TEST(12_LocalClusterSplitByAreaRatio) {
  ...
  cluster.add(db::PolygonRef(db::Polygon(db::Box(1000, 0, 1020, 1000)), repo),
              1);
  cluster.add(db::PolygonRef(db::Polygon(db::Box(0, 1000, 1000, 1020)), repo),
              2);

  std::list<db::local_cluster<db::PolygonRef>> out;
  std::back_insert_iterator<std::list<db::local_cluster<db::PolygonRef>>> iout =
  ...
  EXPECT_EQ(local_cluster_to_string(*i, conn),
            "[0](0,0;0,20;20,20;20,0);[0](0,0;0,20;1000,20;1000,0)");
  EXPECT_EQ(i->id(), size_t(17));
  ++i;
  EXPECT_EQ(local_cluster_to_string(*i, conn),
            "[1](1000,0;1000,1000;1020,1000;1020,0)");
  EXPECT_EQ(i->id(), size_t(17));
  ++i;
  EXPECT_EQ(local_cluster_to_string(*i, conn),
            "[2](0,1000;0,1020;1000,1020;1000,1000)");
  EXPECT_EQ(i->id(), size_t(17));
}
...
TEST(20_LocalClustersBasic) {
  ...
  EXPECT_EQ(local_clusters_to_string(clusters, conn),
            "#1:[0](0,0;0,1000;1000,1000;1000,0)");

  //  one more shape
  cell.shapes(0).insert(
      db::PolygonRef(poly.transformed(db::Trans(db::Vector(10, 20))), repo));

  clusters.clear();
  clusters.build_clusters(cell, conn);
  EXPECT_EQ(local_clusters_to_string(clusters, conn),
            "#1:[0](0,0;0,1000;1000,1000;1000,0);[0](10,20;10,1020;1010,1020;"
            "1010,20)");

  //  one more shape creating a new cluster
  cell.shapes(2).insert(
  ...
  EXPECT_EQ(local_clusters_to_string(clusters, conn),
            "#1:[0](0,0;0,1000;1000,1000;1000,0);[0](10,20;10,1020;1010,1020;"
            "1010,20);[2](0,1000;0,2000;1000,2000;1000,1000);[2](0,1100;0,2100;"
            "1000,2100;1000,1100)");

  //  one more shape opening a new cluster
  cell.shapes(1).insert(
      db::PolygonRef(poly.transformed(db::Trans(db::Vector(0, 1100))), repo));
...
TEST(21_LocalClustersBasicWithAttributes) {
  ...
  //  one more shape
  cell.shapes(0).insert(db::PolygonRefWithProperties(
      db::PolygonRef(poly.transformed(db::Trans(db::Vector(10, 20))), repo),
      1));
  ...
  //  one more shape creating a new cluster
  cell.shapes(2).insert(db::PolygonRefWithProperties(
      db::PolygonRef(poly.transformed(db::Trans(db::Vector(0, 1100))), repo),
      2));
  ...
  EXPECT_EQ(local_clusters_to_string(clusters, conn),
            "#1:[0](0,0;0,1000;1000,1000;1000,0);[0](10,20;10,1020;1010,1020;"
            "1010,20);[2](0,1000;0,2000;1000,2000;1000,1000);[2](0,1100;0,2100;"
            "1000,2100;1000,1100)%1%2%3");

  //  one more shape opening a new cluster
  cell.shapes(1).insert(db::PolygonRefWithProperties(
      db::PolygonRef(poly.transformed(db::Trans(db::Vector(0, 1100))), repo),
      4));
...
TEST(22_LocalClustersWithGlobal) {
  ...
  clusters.build_clusters(cell, conn);
  EXPECT_EQ(
      local_clusters_to_string(clusters, conn),
      "#1:[0](0,0;0,1000;1000,1000;1000,0);[0](10,20;10,1020;1010,1020;1010,20)"
      ";[2](0,1100;0,2100;1000,2100;1000,1100)%1%2+GLOBAL+GLOBAL2");
}
...
TEST(23_LocalClustersWithEdges) {
...

  {
    ...
    clusters.build_clusters(cell, conn);
    EXPECT_EQ(local_clusters_to_string(clusters, conn),
              "#1:[0](0,0;0,500);[0](0,500;0,1000);[0](1500,375;0,0);[0](0,"
              "1000;2000,1000);[0](2000,1000;2000,500);[0](2000,500;1000,250)");
  }
}
...
TEST(30_LocalConnectedClusters) {
  ...
  db::Instance i1 = layout.cell(ci1).insert(
      db::CellInstArray(db::CellInst(ci2), db::Trans()));
  db::Instance i2 = layout.cell(ci2).insert(
      db::CellInstArray(db::CellInst(ci3), db::Trans()));
  ...
  EXPECT_EQ(*ix == db::ClusterInstance(ix->id(), i1.cell_index(),
                                       i1.complex_trans(), i1.prop_id()),
            true);
  ++ix;
  EXPECT_EQ(ix->id(), size_t(2));
  EXPECT_EQ(*ix == db::ClusterInstance(ix->id(), i2.cell_index(),
                                       i2.complex_trans(), i2.prop_id()),
            true);
  ++ix;
  EXPECT_EQ(ix->id(), size_t(1));
  EXPECT_EQ(*ix == db::ClusterInstance(ix->id(), i2.cell_index(),
  ...
  EXPECT_EQ(cc.find_cluster_with_connection(
                db::ClusterInstance(3, db::InstElement(i1))),
            size_t(2));
  EXPECT_EQ(cc.find_cluster_with_connection(
                db::ClusterInstance(2, db::InstElement(i1))),
            size_t(0));
  EXPECT_EQ(cc.find_cluster_with_connection(
                db::ClusterInstance(2, db::InstElement(i2))),
            size_t(1));
  ...
  EXPECT_EQ(cc.find_cluster_with_connection(
            ...
            size_t(1));
  EXPECT_EQ(cc.find_cluster_with_connection(
                db::ClusterInstance(1, db::InstElement(i1))),
            size_t(1));
  EXPECT_EQ(cc.find_cluster_with_connection(
                db::ClusterInstance(2, db::InstElement(i1))),
            size_t(0));
...
TEST(40_HierClustersBasic) {
  ...
  for (db::connected_clusters<db::PolygonRef>::const_iterator i =
           cluster->begin();
       i != cluster->end(); ++i) {
    ++n;
  }
  ...
  EXPECT_EQ(cluster->bbox().to_string(), "(0,0;1000,1000)")
  nc = 0;
  for (db::connected_clusters<db::PolygonRef>::connections_iterator i =
           cluster->begin_connections();
       i != cluster->end_connections(); ++i) {
...
static std::string path2string(const db::Layout &ly, db::cell_index_type ci,
...
static std::string
rcsiter2string(const db::Layout &ly, db::cell_index_type ci,
               db::recursive_cluster_shape_iterator<db::PolygonRef> si,
               db::cell_index_type ci2skip =
...
static std::string
rciter2string(const db::Layout &ly, db::cell_index_type ci,
              db::recursive_cluster_iterator<db::PolygonRef> si) {
  std::string res;
  while (!si.at_end()) {
    if (!res.empty()) {
      res += ";";
...
TEST(41_HierClustersRecursiveClusterShapeIterator) {
  ...
  db::Cell &c1 = ly.cell(ly.add_cell("C1"));
  c1.shapes(l1).insert(make_box(ly, db::Box(0, 0, 2000, 500)));
  top.insert(db::CellInstArray(db::CellInst(c1.cell_index()),
                               db::Trans(db::Vector(0, 10))));

  db::Cell &c2 = ly.cell(ly.add_cell("C2"));
  c2.shapes(l1).insert(make_box(ly, db::Box(0, 0, 500, 2000)));
  c2.insert(db::CellInstArray(db::CellInst(c1.cell_index()),
                              db::Trans(db::Vector(0, 20))));
  top.insert(db::CellInstArray(db::CellInst(c2.cell_index()),
                               db::Trans(db::Vector(0, 30))));
  ...
  std::string res;
  int n = 0;
  db::connected_clusters<db::PolygonRef> *cluster =
      &hc.clusters_per_cell(top.cell_index());
  for (db::connected_clusters<db::PolygonRef>::const_iterator i =
           cluster->begin();
       i != cluster->end(); ++i) {
    res = rcsiter2string(ly, top.cell_index(),
                         db::recursive_cluster_shape_iterator<db::PolygonRef>(
                             hc, l1, top.cell_index(), i->id()));
    ++n;
  }
  EXPECT_EQ(n, 1);
  EXPECT_EQ(res, "TOP:(0,0;0,1000;1000,1000;1000,0);TOP/"
                 "C1:(0,10;0,510;2000,510;2000,10);TOP/"
                 "C2:(0,30;0,2030;500,2030;500,30);TOP/C2/"
                 "C1:(0,50;0,550;2000,550;2000,50)");
  ...
  EXPECT_EQ(
      res,
      "TOP:(0,0;0,1000;1000,1000;1000,0);TOP/C2:(0,30;0,2030;500,2030;500,30)");
}
...
TEST(41_HierClustersRecursiveClusterIterator) {
  ...
  for (db::connected_clusters<db::PolygonRef>::const_iterator i =
           cluster->begin();
       i != cluster->end(); ++i) {
    res = rciter2string(ly, top.cell_index(),
                        db::recursive_cluster_iterator<db::PolygonRef>(
                            hc, top.cell_index(), i->id()));
    ++n;
  }
  EXPECT_EQ(n, 1);
...
static void normalize_layer(db::Layout &layout,
  ...
  for (db::Layout::iterator c = layout.begin(); c != layout.end(); ++c) {
    ...
    for (db::Shapes::shape_iterator i =
             s.begin(db::ShapeIterator::Texts | db::ShapeIterator::Polygons |
                     db::ShapeIterator::Paths | db::ShapeIterator::Boxes);
         !i.at_end(); ++i) {
      if (!i->is_text()) {
        db::Polygon poly;
        i->polygon(poly);
        c->shapes(new_layer).insert(
            db::PolygonRef(poly, layout.shape_repository()));
      } else {
        db::Polygon poly(i->bbox());
        unsigned int attr_id = (unsigned int)strings.size() + 1;
        strings.push_back(i->text_string());
        c->shapes(new_layer).insert(db::PolygonRefWithProperties(
            db::PolygonRef(poly, layout.shape_repository()), attr_id));
      }
    }
  }
...
static void
copy_cluster_shapes(const std::string *&attrs, db::Shapes &out,
                    db::cell_index_type ci,
                    const db::hier_clusters<db::PolygonRef> &hc,
                    db::local_cluster<db::PolygonRef>::id_type cluster_id,
                    const db::ICplxTrans &trans, const db::Connectivity &conn) {
  ...
  const db::connected_clusters<db::PolygonRef> &clusters =
      hc.clusters_per_cell(ci);
  const db::local_cluster<db::PolygonRef> &lc =
      clusters.cluster_by_id(cluster_id);

  //  copy the shapes from this cell
  for (db::Connectivity::layer_iterator l = conn.begin_layers();
       l != conn.end_layers(); ++l) {
    for (db::local_cluster<db::PolygonRef>::shape_iterator s = lc.begin(*l);
         !s.at_end(); ++s) {
      db::Polygon poly =
          s->obj().transformed(trans * db::ICplxTrans(s->trans()));
      out.insert(db::PolygonWithProperties(
          poly, cell_and_attr_pid > 0 ? cell_and_attr_pid : cell_pid));
      cell_and_attr_pid = 0;
      attrs = 0; // used
    }
  }
  ...
  typedef db::connected_clusters<db::PolygonRef>::connections_type
      connections_type;
  const connections_type &connections =
      clusters.connections_for_cluster(cluster_id);
  for (connections_type::const_iterator i = connections.begin();
       i != connections.end(); ++i) {
...
static void run_hc_test(tl::TestBase *_this, const std::string &file,
  ...
  for (db::Layout::top_down_const_iterator td = ly.begin_top_down();
       td != ly.end_top_down(); ++td) {

    const db::connected_clusters<db::PolygonRef> &clusters =
        hc.clusters_per_cell(*td);
    for (db::connected_clusters<db::PolygonRef>::all_iterator c =
             clusters.begin_all();
         !c.at_end(); ++c) {
      ...
      //  collect strings
      std::string attrs;
      for (db::recursive_cluster_iterator<db::PolygonRef> rc(hc, *td, *c);
           !rc.at_end(); ++rc) {
        const db::local_cluster<db::PolygonRef> &rcc =
            hc.clusters_per_cell(rc.cell_index())
                .cluster_by_id(rc.cluster_id());
        for (db::local_cluster<db::PolygonRef>::attr_iterator a =
          ...
          attrs += std::string(ly.cell_name(rc.cell_index())) + ":" +
                   strings[*a - 1];
        }
      }
      ...
      for (db::Shapes::shape_iterator s = out.begin(db::ShapeIterator::All);
           !s.at_end(); ++s) {
        area += s->area();
      }
  ...
  int ln = 1000;
  for (std::vector<
           std::pair<db::Polygon::area_type, unsigned int>>::const_iterator l =
           net_layers.begin();
       l != net_layers.end(); ++l) {
...
static void run_hc_test_with_backannotation(tl::TestBase *_this,
...
static size_t root_nets(const db::connected_clusters<db::PolygonRef> &cc) {
  size_t n = 0;
  for (db::connected_clusters<db::PolygonRef>::all_iterator c = cc.begin_all();
       !c.at_end(); ++c) {
    if (cc.is_root(*c)) {
      ++n;
    }
...
TEST(201_issue1126) {
  {
...

    {
      ...
      db::LoadLayoutOptions options;
      options.get_options<db::CommonReaderOptions>().layer_map = lmap;
      options.get_options<db::CommonReaderOptions>().create_other_layers =
          false;
