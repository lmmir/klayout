...
/**
 *  @brief A new processor class which and/nots with a sized version of the
 * intruder
 */
...
public:
  ...
  do_compute_local(db::Layout *layout,
                   const db::shape_interactions<db::PolygonRef, db::PolygonRef>
                       &interactions,
                   std::vector<std::unordered_set<db::PolygonRef>> &results,
                   size_t max_vertex_count, double area_ratio) const {
    db::shape_interactions<db::PolygonRef, db::PolygonRef> sized_interactions =
        interactions;
    for (db::shape_interactions<db::PolygonRef, db::PolygonRef>::iterator i =
             sized_interactions.begin();
         i != sized_interactions.end(); ++i) {
      for (db::shape_interactions<db::PolygonRef, db::PolygonRef>::iterator2 j =
               i->second.begin();
           j != i->second.end(); ++j) {
        const std::pair<unsigned int, db::PolygonRef> &intruder =
            interactions.intruder_shape(*j);
        db::Polygon poly =
            intruder.second.obj().transformed(intruder.second.trans());
        poly.size(m_dist, m_dist);
        sized_interactions.add_intruder_shape(
            *j, intruder.first,
            db::PolygonRef(poly, layout->shape_repository()));
      }
    }

    db::BoolAndOrNotLocalOperation::do_compute_local(
        layout, sized_interactions, results, max_vertex_count, area_ratio);
  }

  db::Coord dist() const { return m_dist; }
...
public:
  ...
  do_compute_local(db::Layout *layout,
    ...
    for (db::shape_interactions<db::PolygonRef, db::PolygonRef>::iterator i =
      ...
      sized_interactions.add_subject_shape(
          i->first, db::PolygonRef(poly, layout->shape_repository()));
      ...
      for (db::shape_interactions<db::PolygonRef, db::PolygonRef>::iterator2 j =
        ...
        poly.size(m_dist / 2, m_dist / 2);
        sized_interactions.add_intruder_shape(
            *j, il, db::PolygonRef(poly, layout->shape_repository()));
      }
    }

    SelfOverlapMergeLocalOperation::do_compute_local(
        layout, sized_interactions, results, max_vertex_count, area_ratio);
  }
 ...
 *  The hierarchical processor needs polygon references and can't work on
 * polygons directly.
 */
...
static std::string contexts_to_s(
    db::Layout *layout,
    db::local_processor_contexts<db::PolygonRef, db::PolygonRef, db::PolygonRef>
        &contexts) {
  std::string res;

  for (db::Layout::top_down_const_iterator i = layout->begin_top_down();
       i != layout->end_top_down(); ++i) {
    db::local_processor_contexts<db::PolygonRef, db::PolygonRef,
                                 db::PolygonRef>::iterator cc =
        contexts.context_map().find(&layout->cell(*i));
    if (cc != contexts.context_map().end()) {
      int index = 1;
      for (db::local_processor_cell_contexts<db::PolygonRef, db::PolygonRef,
                                             db::PolygonRef>::iterator j =
               cc->second.begin();
           j != cc->second.end(); ++j) {
        size_t nshapes = 0;
        for (std::map<unsigned int, std::set<db::PolygonRef>>::const_iterator
                 i = j->first.second.begin();
             i != j->first.second.end(); ++i) {
          nshapes += i->second.size();
        }
        if (j->first.first.size() > 1) {
          res += tl::sprintf(
              "%s[%d] %d insts, %d shapes/%d layers (%d times)\n",
              layout->cell_name(*i), index, int(j->first.first.size()),
              int(nshapes), int(j->first.second.size()), int(j->second.size()));
        } else {
          res += tl::sprintf("%s[%d] %d insts, %d shapes (%d times)\n",
                             layout->cell_name(*i), index,
                             int(j->first.first.size()), int(nshapes),
                             int(j->second.size()));
        }
        index += 1;
      }
...
static void run_test_bool_gen(tl::TestBase *_this, const char *file,
                              TestMode mode, int out_layer_num1,
                              int out_layer_num2, std::string *context_doc,
                              bool single, db::Coord dist,
...

  {
    ...
    if (out_layer_num2 >= 0) {
      ...
      p.datatype = 0;
      lmap.map(db::LDPair(out_layer_num2, 0),
               lout2 = layout_org.insert_layer());
  ...
  BoolAndOrNotWithSizedLocalOperation sized_bool_op(
      mode == TMAnd || mode == TMAndSwapped, dist);
  ...
  if (single) {
    ...
    db::local_processor<db::PolygonRef, db::PolygonRef, db::PolygonRef> proc(
        &layout_org, &layout_org.cell(*layout_org.begin_top_down()),
        &layout_org2, &layout_org2.cell(*layout_org2.begin_top_down()));
    proc.set_threads(nthreads);
    proc.set_area_ratio(3.0);
    proc.set_max_vertex_count(16);
  ...
  db::compare_layouts(_this, layout_org, testdata(file), lmap,
                      false /*skip other layers*/, db::AsPolygons);
}

static void run_test_bool(tl::TestBase *_this, const char *file, TestMode mode,
                          int out_layer_num, std::string *context_doc = 0,
                          unsigned int nthreads = 0) {
  run_test_bool_gen(_this, file, mode, out_layer_num, -1, context_doc, true, 0,
                    nthreads);
}

static void run_test_bool2(tl::TestBase *_this, const char *file, TestMode mode,
                           int out_layer_num, std::string *context_doc = 0,
                           unsigned int nthreads = 0) {
  run_test_bool_gen(_this, file, mode, out_layer_num, -1, context_doc, false, 0,
                    nthreads);
}

static void run_test_bool_with_size(tl::TestBase *_this, const char *file,
                                    TestMode mode, db::Coord dist,
                                    int out_layer_num,
                                    std::string *context_doc = 0,
                                    unsigned int nthreads = 0) {
  run_test_bool_gen(_this, file, mode, out_layer_num, -1, context_doc, true,
                    dist, nthreads);
}

static void run_test_bool2_with_size(tl::TestBase *_this, const char *file,
                                     TestMode mode, db::Coord dist,
                                     int out_layer_num,
                                     std::string *context_doc = 0,
                                     unsigned int nthreads = 0) {
  run_test_bool_gen(_this, file, mode, out_layer_num, -1, context_doc, false,
                    dist, nthreads);
}

static void run_test_bool22(tl::TestBase *_this, const char *file,
                            TestMode mode, int out_layer_num1,
                            int out_layer_num2, std::string *context_doc = 0,
                            unsigned int nthreads = 0) {
  run_test_bool_gen(_this, file, mode, out_layer_num1, out_layer_num2,
                    context_doc, false, 0, nthreads);
}

static void run_test_bool_gen_flat(tl::TestBase *_this, const char *file,
                                   TestMode mode, int out_layer_num1,
                                   int out_layer_num2, db::Coord dist) {
  db::Layout layout_org;
...
static void run_test_bool22_flat(tl::TestBase *_this, const char *file,
                                 TestMode mode, int out_layer_num1,
                                 int out_layer_num2) {
...
TEST(BasicAnd9) {
  ...
  EXPECT_EQ(doc,
            //  This means: the interaction test is strong enough, so it does
            //  not see interactions between the ring and the cells embedded
            //  inside the ring. So there is only one cell context. Some shapes
            //  from atop the CHILD cell don't interact with shapes inside
            //  CHILD, so there are 4 shapes rather than
            //  6. And the shapes from top inside the ring are not seen by the
            //  RING's subject shapes.
...
TEST(TopWithBelow1) { run_test_bool(_this, "hlp12.oas", TMNot, 100); }

TEST(TopWithBelow2) { run_test_bool(_this, "hlp12.oas", TMNotSwapped, 101); }
