...
TEST(1) {
  ...
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au1.gds");
}
...
TEST(1_WithEmptyLayer) {
  ...
  db::RecursiveShapeIterator iter(ly, ly.cell(top_cell_index),
...
TEST(2_WithoutClip) {
  ...
  for (db::Layout::layer_iterator li = ly.begin_layers(); li != ly.end_layers();
    ...
    db::cell_index_type top_cell_index = *ly.begin_top_down();
    db::RecursiveShapeIterator iter(ly, ly.cell(top_cell_index), li1,
                                    db::Box(5000, -2000, 18500, 6000));

    iter.push(&builder);

    target.cell(target_top)
        .insert(db::CellInstArray(
            db::CellInst(builder.initial_cell()->cell_index()), db::Trans()));
  }

  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2a.gds");
}
...
TEST(2_WithClip) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2b.gds");
}
...
TEST(2_WithClipAndSimplification) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2c.gds");
}
...
TEST(2_WithClipAndRefGeneration) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2d.gds");
}
...
TEST(2_WithEmptyResult) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2e.gds");
}
...
TEST(2_WithClipAndSimplificationAndEmptyLayer) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au2f.gds");
}
...
TEST(3_ComplexRegionWithClip) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au3a.gds");
}
...
TEST(4_ComplexRegionAndLayoutWithClip) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au4a.gds");
}
...
TEST(5_CompareRecursiveShapeIterators) {
...

  {
    ...
    EXPECT_EQ(db::compare_iterators_with_respect_to_target_hierarchy(
                  iter1, iter2) != 0,
              true);
    EXPECT_EQ(
        db::compare_iterators_with_respect_to_target_hierarchy(iter1, iter2) !=
            db::compare_iterators_with_respect_to_target_hierarchy(iter2,
                                                                   iter1),
        true);
  }
...

  {
    ...
    db::RecursiveShapeIterator iter1(ly, ly.cell(ci), 0,
                                     db::Box(0, 1000, 2000, 3000));
    db::RecursiveShapeIterator iter2(ly, ly.cell(ci), ll1,
                                     db::Box(0, 1000, 2000, 3000));
    EXPECT_EQ(db::compare_iterators_with_respect_to_target_hierarchy(
                  iter1, iter2) != 0,
              true);
...
TEST(6_DisjunctLayersPerHierarchyBranch) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au_l4.gds");
}
...
TEST(7_DetachFromOriginalLayout) {
  ...
  for (db::Layout::layer_iterator li = ly->begin_layers();
       li != ly->end_layers(); ++li) {
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au_l5.gds");
}
...
TEST(8a_SimpleWithTrans) {
  ...
  db::Layout target;
  db::HierarchyBuilder builder(&target,
                               db::ICplxTrans(2.0, 45.0, false, db::Vector()));
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au8a.gds");
}
...
TEST(8b_ComplexRegionWithTransformation) {
  ...
  db::HierarchyBuilder builder(
      &target, db::ICplxTrans(2.0, 45.0, false, db::Vector()), &clip);
  ...
  CHECKPOINT();
  db::compare_layouts(_this, target,
                      tl::testdata() + "/algo/hierarchy_builder_au8b.gds");
}
