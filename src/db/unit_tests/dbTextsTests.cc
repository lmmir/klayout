...
TEST(1) {
  ...
  EXPECT_EQ(texts.transformed(db::ICplxTrans(2.0, 0.0, false, db::Vector()))
                .to_string(),
            "('uvw',r0 220,420)");
  EXPECT_EQ(texts.to_string(), "('uvw',r0 110,210)");
...
TEST(2) {
  ...
  db::Region r;
  texts.polygons(r);
  EXPECT_EQ(db::compare(r, "(99,-201;99,-199;101,-199;101,-201);(109,209;109,"
                           "211;111,211;111,209)"),
            true);
}
...
TEST(7) {
  ...
  EXPECT_EQ(texts.selected_interacting(region).to_string(),
            "('abc',r0 100,-200)");
  EXPECT_EQ(texts.selected_not_interacting(region).to_string(),
            "('uvw',r0 110,210)");
