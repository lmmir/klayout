...
struct BoxScannerTestRecorder {
  ...
  void add(const db::Box * /*b1*/, size_t p1, const db::Box * /*b2*/,
           size_t p2) {
...
struct BoxScannerTestRecorderTwo {
  ...
  void add(const db::Box * /*b1*/, size_t p1, const db::SimplePolygon * /*b2*/,
           int p2) {
    str += "(" + tl::to_string(p1) + "-" + tl::to_string(p2) + ")";
  }
...
TEST(1) {
  ...
  for (std::vector<db::Box>::const_iterator b = bb.begin(); b != bb.end();
       ++b) {
  ...
  EXPECT_EQ(tr.str,
            "[i](4-2)(5-2)(5-4)(3-2)(3-4)(5-3)<2><5><4><3>(1-0)<0><1>[f]");
...
void run_test2(tl::TestBase *_this, size_t n, double ff, db::Coord spread,
               bool touch = true) {
  std::vector<db::Box> bb;
  ...
  std::set<std::pair<size_t, size_t>> interactions;
  {
    ...
    for (size_t i = 0; i < bb.size(); ++i) {
      for (size_t j = i + 1; j < bb.size(); ++j) {
        if ((touch && bb[i].touches(bb[j])) ||
            (!touch && bb[i].overlaps(bb[j]))) {
  ...
  if (interactions != tr.interactions) {
    ...
    for (std::set<std::pair<size_t, size_t>>::const_iterator i =
             tr.interactions.begin();
         i != tr.interactions.end(); ++i) {
...
struct TestCluster : public db::cluster<db::Box, size_t> {
  typedef db::cluster<db::Box, size_t> base_class;

  TestCluster(std::set<std::set<size_t>> *cl) : clusters(cl) {}
  ...
  void finish() { clusters->insert(props); }
...
std::string cl2s(const std::set<std::set<size_t>> &clusters) {
  std::string r;
  for (std::set<std::set<size_t>>::const_iterator c = clusters.begin();
       c != clusters.end(); ++c) {
    if (!r.empty()) {
      r += ",";
    }
...
void run_test11(tl::TestBase *_this, size_t n, double ff, db::Coord spread,
                bool touch = true) {
  std::vector<db::Box> bb;
  for (size_t i = 0; i < n; ++i) {
    db::Coord x = rand() % spread;
    ...
    for (size_t i = 0; i < bb.size(); ++i) {
      ...
      while (any) {
        ...
        for (size_t j = 0; j < bb.size(); ++j) {
          ...
          for (std::set<size_t>::const_iterator k = cl.begin(); k != cl.end();
               ++k) {
            if ((touch && bb[*k].touches(bb[j])) ||
                (!touch && bb[*k].overlaps(bb[j]))) {
              cl.insert(j);
              seen.insert(j);
              any = true;
              break;
  ...
  if (clusters != bf_clusters) {
    ...
    for (std::set<std::set<size_t>>::const_iterator i = bf_clusters.begin();
         i != bf_clusters.end(); ++i) {
    ...
    for (std::set<std::set<size_t>>::const_iterator i = clusters.begin();
         i != clusters.end(); ++i) {
...
struct BooleanAndOp : public db::box_scanner_receiver<db::Polygon, size_t> {
  ...
  void add(const db::Polygon *o1, size_t p1, const db::Polygon *o2, size_t p2) {
    ...
    if (o1->holes() == 0 && o1->hull().size() == 4 && o2->holes() == 0 &&
        o2->hull().size() == 4 && o1->area() == o1->box().area() &&
        o2->area() == o2->box().area()) {
...
struct BooleanAndCluster : public db::cluster<db::Polygon, size_t> {
...
struct BooleanAndInteractionClusterCollector
  ...
  typedef db::cluster_collector<db::Polygon, size_t, BooleanAndCluster>
      base_class;
...
TEST(100) {
  ...
  //  classical implementation
  {
    ...
    sp.boolean(layout, top_cell, l3, layout, top_cell, l6,
               top_cell.shapes(lclass), db::BooleanOp::And, true, false);
  }
  ...
  //  alternative implementation - local
  {
...

    {
      ...
      for (size_t i = 0; i < 2; ++i) {
        ...
        si.shape_flags(db::ShapeIterator::Polygons | db::ShapeIterator::Paths |
        ...
        for (std::vector<db::Polygon>::const_iterator p = polygons[i].begin();
             p != polygons[i].end(); ++p) {
          bs.insert(&*p, i);
        }
      }
    ...
    {
      db::ShapeProcessor sp;
      sp.boolean(layout, top_cell, lclass, layout, top_cell, llocal,
                 top_cell.shapes(ltmp1), db::BooleanOp::Xor, true, false);
      sp.boolean(layout, top_cell, lclass, layout, top_cell, lcluster,
                 top_cell.shapes(ltmp2), db::BooleanOp::Xor, true, false);
    }

    if (top_cell.shapes(ltmp1).size() != 0 ||
        top_cell.shapes(ltmp2).size() != 0) {
...
TEST(two_1) {
  ...
  for (std::vector<db::SimplePolygon>::const_iterator b = bb2.begin();
       b != bb2.end(); ++b) {
  ...
  EXPECT_EQ(tr.str, "[i](2-12)(2-14)(4-12)(4-14)(2-15)(4-15)(5-12)(5-14)(5-15)("
                    "2-13)(4-13)(3-12)(3-14)(3-13)(3-15)(5-13)(0-10)<2><5><4><"
                    "3><12><15><14><13>(0-11)(1-10)(1-11)<0><1><10><11>[f]");
}
...
TEST(two_1a) {
  ...
  bs.process(tr, 1, bc1, bc2);
  EXPECT_EQ(tr.str,
            "[i](2-11)(2-12)(1-11)(1-12)<1><2><11><12>(0-10)<0><10>[f]");
}
...
TEST(two_1b) {
  ...
  EXPECT_EQ(tr.str,
            "[i](1-12)(2-12)(1-11)(2-11)<1><2><11><12>(0-10)<0><10>[f]");
...
void run_test2_two(tl::TestBase *_this, size_t n1, size_t n2, double ff,
                   db::Coord spread, bool touch = true,
                   bool no_shortcut = true) {
  std::vector<db::Box> bb;
  ...
  for (std::vector<db::SimplePolygon>::const_iterator b2 = bb2.begin();
       b2 != bb2.end(); ++b2) {
  ...
  std::set<std::pair<size_t, int>> interactions;
  {
    ...
    for (size_t i = 0; i < bb.size(); ++i) {
      for (size_t j = 0; j < bb2.size(); ++j) {
        if ((touch && bb[i].touches(bb2[j].box())) ||
            (!touch && bb[i].overlaps(bb2[j].box()))) {
  ...
  if (interactions != tr.interactions) {
    ...
    for (std::set<std::pair<size_t, int>>::const_iterator i =
             interactions.begin();
         i != interactions.end(); ++i) {
      if (tr.interactions.find(*i) == tr.interactions.end()) {
        tl::info << "   " << i->first << "-" << i->second;
...
TEST(two_2d) { run_test2_two(_this, 1000, 1000, 0.0, 1000); }

TEST(two_2e) { run_test2_two(_this, 1000, 1000, 2, 1000); }

TEST(two_2f) { run_test2_two(_this, 1000, 1000, 2, 1000, false); }

TEST(two_2g) { run_test2_two(_this, 1000, 1000, 2, 500); }

TEST(two_2h) { run_test2_two(_this, 1000, 1000, 2, 100); }

TEST(two_2i) { run_test2_two(_this, 10000, 1000, 2, 10000); }
