...
TEST(2) {
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('10/7-8;XP : XN';'AA : GC';'2/3 : "
                            "17/18';'1/5';'1/0;2/0,2;3-5/0';'WN')");
  EXPECT_EQ(lm.to_string_file_format(),
            "10/7-8;XP : XN\nAA : GC\n2/3 : 17/18\n1/5\n1/0;2/0,2;3-5/0\nWN\n");
  ...
  EXPECT_EQ(lm2.to_string(), "layer_map('10/7-8;XP : XN';'AA : GC';'2/3 : "
                             "17/18';'1/5';'1/0;2/0,2;3-5/0';'WN')");
  tl::Extractor(lm2.to_string()).read(lm2read);
  EXPECT_EQ(lm2read.to_string(), "layer_map('10/7-8;XP : XN';'AA : GC';'2/3 : "
                                 "17/18';'1/5';'1/0;2/0,2;3-5/0';'WN')");
...
TEST(4) {
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('NAME')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  lm.map(db::LayerProperties(1, 3), n++,
         db::LayerProperties(db::any_ld(), db::any_ld()));
  ...
  lm.map(db::LayerProperties(1, 6), n++,
         db::LayerProperties(db::any_ld(), db::relative_ld(3)));
  ...
  lm.map(db::LayerProperties(3, db::any_ld()), n++,
         db::LayerProperties(db::any_ld(), 2));
  lm.map(db::LayerProperties(4, db::any_ld()), n++,
         db::LayerProperties(db::any_ld(), db::any_ld()));
  //  datatype catch-all, relative targets
  lm.map(db::LayerProperties(5, db::any_ld()), n++,
         db::LayerProperties(15, db::relative_ld(0)));
  lm.map(db::LayerProperties(6, db::any_ld()), n++,
         db::LayerProperties(16, db::relative_ld(-1)));
  lm.map(db::LayerProperties(7, db::any_ld()), n++,
         db::LayerProperties(17, db::relative_ld(1)));

  EXPECT_EQ(lm.to_string(), "layer_map('1/*';'2/* : 12/2';'3/* : */2';'4/* : "
                            "*/*';'5/* : 15/*';'6/* : 16/*-1';'7/* : 17/*+1')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  lm.map(db::LayerProperties(db::any_ld(), 3), n++,
         db::LayerProperties(db::any_ld(), 2));
  lm.map(db::LayerProperties(db::any_ld(), 4), n++,
         db::LayerProperties(db::any_ld(), db::any_ld()));
  //  layer catch-all, relative targets
  lm.map(db::LayerProperties(db::any_ld(), 5), n++,
         db::LayerProperties(2, db::relative_ld(0)));
  lm.map(db::LayerProperties(db::any_ld(), 6), n++,
         db::LayerProperties(2, db::relative_ld(-1)));
  lm.map(db::LayerProperties(db::any_ld(), 7), n++,
         db::LayerProperties(2, db::relative_ld(1)));

  EXPECT_EQ(lm.to_string(), "layer_map('*/1';'*/2 : 1/12';'*/3 : */2';'*/4 : "
                            "*/*';'*/5 : 2/*';'*/6 : 2/*-1';'*/7 : 2/*+1')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('*/*')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  lm.map(db::LayerProperties(db::any_ld(), db::any_ld()), n++,
         db::LayerProperties(1, 2));

  EXPECT_EQ(lm.to_string(), "layer_map('*/* : 1/2')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('*/* : */2')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('*/* : 2/*-1')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
...
TEST(5) {
  ...
  EXPECT_EQ(lm.to_string(),
            "layer_map('0-16/0,11-14,16-*;18-*/0,11-14,16-*';'0-16/"
            "1-4,6-10;18-*/1-4,6-10';'0-16/5,15;18-*/5,15';'17/*')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
}
...
TEST(6) {
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('1/0';'3/10-*';'2/0 : 2/0';'2/42 : "
                            "2/42';'2/1-41,43-* : */*')");
}
...
static std::string set2string(const std::set<unsigned int> &set) {
  std::string s;
  for (std::set<unsigned int>::const_iterator i = set.begin(); i != set.end();
       ++i) {
    if (i != set.begin()) {
      s += ",";
    }
...
TEST(8) {
  ...
  //  some
  lm.mmap_expr("*/1-10", n++);
  EXPECT_EQ(lm.to_string(), "layer_map('+*/*';'+*/1-10')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('+*/*';'+*/1-10';'+*/5,15')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
  ...
  //  NOTE: the leading "+" indicates that the listed layers may go somewhere
  //  else, so we can't plainly map them
  EXPECT_EQ(lm.mapping_str(0), "+*/*");
  EXPECT_EQ(lm.mapping_str(1), "+*/1-10");
  ...
  EXPECT_EQ(lm.to_string(), "layer_map('+*/*';'+1-10/0-20')");
  EXPECT_EQ(db::LayerMap::from_string_file_format(lm.to_string_file_format())
                .to_string(),
            lm.to_string());
}
