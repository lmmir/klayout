...
class MyReaderOptions : public db::FormatSpecificReaderOptions {
public:
  MyReaderOptions() : db::FormatSpecificReaderOptions() {}
...
static void set_myreader_value(db::LoadLayoutOptions *options,
                               const std::string &v) {
...
static void set_myreader_lm(db::LoadLayoutOptions *options,
...
TEST(1) {
  ...
  EXPECT_EQ(
      opt.get_option_by_name("myreader_lm").to_user<db::LayerMap>().to_string(),
