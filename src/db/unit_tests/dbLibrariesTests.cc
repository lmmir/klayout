...
class LIBT_PD : public db::PCellDeclaration {
  virtual std::vector<db::PCellLayerDeclaration>
  get_layer_declarations(const db::pcell_parameters_type &) const {
  ...
  virtual void produce(const db::Layout &layout,
                       ...
                       db::Cell &cell) const {
    db::Coord width = db::coord_traits<db::Coord>::rounded(
        parameters[0].to_double() / layout.dbu());
    db::Coord height = db::coord_traits<db::Coord>::rounded(
        parameters[1].to_double() / layout.dbu());
    ...
    cell.insert(db::CellInstArray(
        db::CellInst(cell_a.cell_index()),
        db::Trans(orientation, db::Vector(width / 2 - 50, height / 2 - 100))));

    cell.shapes(l_metal0).insert(db::Box(0, 0, width, height));
  }
};

class LIBT_L : public db::Library {
public:
  LIBT_L(tl::TestBase *_this) : Library() {
    set_name("L");
    ...
    db::Instance i1 = top.insert(
        db::CellInstArray(db::CellInst(pd1), db::Trans(db::Vector(0, 0))));

    width = width.to_double() * 0.1;
    width = width.to_double() * 10.0;

    db::cell_index_type pd2 = layout().get_pcell_variant(pd, parameters);
    db::Instance i2 = top.insert(
        db::CellInstArray(db::CellInst(pd2), db::Trans(db::Vector(0, 2000))));
    ...
    db::Instance i3 = top.insert(
        db::CellInstArray(db::CellInst(pd3), db::Trans(db::Vector(2000, 0))));
  }
...
class LIBT_A : public db::Library {
public:
  LIBT_A() : Library() {
    set_name("A");
...
class LIBT_B : public db::Library {
public:
  LIBT_B() : Library() {
    set_name("B");
    ...
    cell_b.insert(db::CellInstArray(
        db::CellInst(cp),
        db::ICplxTrans(0.1, 0.0, false, db::Vector(1.0, 2.0))));
  }
};

TEST(1) {
  std::vector<std::string> libnames_before;
  for (db::LibraryManager::iterator il = db::LibraryManager::instance().begin();
       il != db::LibraryManager::instance().end(); ++il) {
  ...
  db::Instance i1 = top.insert(
      db::CellInstArray(db::CellInst(lp1), db::Trans(db::Vector(0, 0))));
  ...
  db::Instance i2 = top.insert(
      db::CellInstArray(db::CellInst(lp2), db::Trans(db::Vector(10000, 0))));

  db::Writer writer = db::Writer(db::SaveLayoutOptions());

  CHECKPOINT();
  db::compare_layouts(this, layout, tl::testdata() + "/gds/lib_test.gds",
                      db::NormalizationMode(db::WriteGDS2 + db::NoContext));

  // if not in editable mode, we could have lost the reference to the second
  // instance
  if (db::default_editable_mode()) {
    ...
    */
    ...
    CHECKPOINT();
    db::compare_layouts(this, layout, tl::testdata() + "/gds/lib_test2.gds",
                        db::NormalizationMode(db::WriteGDS2 + db::NoContext));
    ...
    */
    ...
    CHECKPOINT();
    db::compare_layouts(this, layout, tl::testdata() + "/gds/lib_test3.gds",
                        db::NormalizationMode(db::WriteGDS2 + db::NoContext));
...
TEST(2) {
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("L").second,
            lib->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib(lib->get_id()) == lib.get(),
            true);
  ...
  std::string tmp_file =
      tl::TestBase::tmp_file(tl::sprintf("tmp_dbLibraries2.gds"));
  ...
  db::Instance tmp_i2 =
      tmp.cell(tmp_pd2.second).begin_parent_insts()->child_inst();
  ...
  if (db::default_editable_mode()) {
    ...
    CHECKPOINT();
    db::compare_layouts(this, tmp, tl::testdata() + "/gds/lib_test2.gds",
                        db::NormalizationMode(db::WriteGDS2 + db::NoContext));
  }
...
TEST(3) {
  ...
  //  This test tests the ability to reference libraries out of other libraries
  //  ("B" references "A"), the ability to persist that and whether this
  ...
  db::Instance i1 = top.insert(
      db::CellInstArray(db::CellInst(lp), db::Trans(db::Vector(0, 0))));

  std::string tmp_file =
      tl::TestBase::tmp_file(tl::sprintf("tmp_dbLibraries3.gds"));
  ...
  CHECKPOINT();
  db::compare_layouts(this, tmp, tl::testdata() + "/gds/lib_test4.gds",
                      db::NormalizationMode(db::WriteGDS2 + db::NoContext));
}
...
TEST(4) {
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "X").second,
            lib_a1->get_id());
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "Y").second,
            lib_a2->get_id());
  ...
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A").second,
            lib_a4->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "Z").first, true);
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "Z").second,
            lib_a4->get_id());
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "").first, true);
  EXPECT_EQ(db::LibraryManager::instance().lib_by_name("A", "").second,
            lib_a4->get_id());
...
class PCell1Declaration : public db::PCellDeclaration {
  void produce(const db::Layout & /*layout*/,
               const std::vector<unsigned int> & /*layer_ids*/,
               const db::pcell_parameters_type & /*parameters*/,
               db::Cell & /*cell*/) const {
    // ...
...
class PCell2Declaration : public db::PCellDeclaration {
  void produce(const db::Layout & /*layout*/,
               ...
               const db::pcell_parameters_type & /*parameters*/,
               db::Cell &cell) const {
    //  NOTE: this is the self-reference: we use the library which defines the
    //  PCell and create a proxy to itself
    std::pair<bool, db::lib_id_type> l =
        db::LibraryManager::instance().lib_by_name("__PCellLibrary");
    tl_assert(l.first);
    db::Library *lib = db::LibraryManager::instance().lib(l.second);
    std::pair<bool, db::pcell_id_type> pcell_id =
        lib->layout().pcell_by_name("PCell1");
    tl_assert(pcell_id.first);
    db::cell_index_type pcell_var = lib->layout().get_pcell_variant_dict(
        pcell_id.second, std::map<std::string, tl::Variant>());
...
TEST(5_issue905) {
  ...
  db::cell_index_type lib_cell = lib->layout().get_pcell_variant_dict(
      pc.second, std::map<std::string, tl::Variant>());
...
TEST(6_issue996) {
  ...
  CHECKPOINT();
  db::compare_layouts(this, ly, tl::testdata() + "/gds/lib_test6a.gds",
                      db::NormalizationMode(db::WriteGDS2 + db::NoContext));
  ...
  //  updated now
  CHECKPOINT();
  db::compare_layouts(this, ly, tl::testdata() + "/gds/lib_test6b.gds",
                      db::NormalizationMode(db::WriteGDS2 + db::NoContext));
}
