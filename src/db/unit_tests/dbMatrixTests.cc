...
TEST(1) {
  ...
  EXPECT_EQ(
      tl::to_string(
          1e-6 * floor(0.5 + 1e6 * db::Matrix2d::rotation(-115).shear_angle())),
      "0");
  EXPECT_EQ(tl::to_string(db::Matrix2d::shear(17).shear_angle()), "17");
  EXPECT_EQ(
      tl::to_string(1e-6 * floor(0.5 + 1e6 * db::Matrix2d::shear(17).angle())),
      "0");
  ...
  EXPECT_EQ(tl::to_string(db::Matrix2d::shear(-40).shear_angle()), "-40");
  EXPECT_EQ(
      tl::to_string(1.0 / db::Matrix2d::mag(17.5).inverted().mag2().first),
      "17.5");
  EXPECT_EQ(
      tl::to_string(1.0 / db::Matrix2d::mag(17.5).inverted().mag2().second),
      "17.5");
  EXPECT_EQ(
      tl::to_string(1.0 / db::Matrix2d::mag(27.5, 7.5).inverted().mag2().first),
      "27.5");
  EXPECT_EQ(tl::to_string(
                1.0 / db::Matrix2d::mag(27.5, 7.5).inverted().mag2().second),
            "7.5");
  EXPECT_EQ(tl::to_string(db::Matrix2d::mirror(true).inverted().is_mirror()),
            "true");
  EXPECT_EQ(tl::to_string(db::Matrix2d::mirror(false).inverted().is_mirror()),
            "false");
  EXPECT_EQ(tl::to_string(db::Matrix2d::rotation(25).inverted().angle()),
            "-25");
  EXPECT_EQ(tl::to_string(db::Matrix2d::shear(17).inverted().shear_angle()),
            "-17");

  db::Matrix2d m = db::Matrix2d::rotation(25) *
                   (db::Matrix2d::shear(17) * (db::Matrix2d::mirror(true) *
                                               db::Matrix2d::mag(7.5, 27.5)));
  ...
  m = db::Matrix2d::rotation(-25) *
      (db::Matrix2d::shear(-17) *
       (db::Matrix2d::mirror(true) * db::Matrix2d::mag(27.5, 7.5)));
...
TEST(6) {
  ...
  db::Matrix2d n = db::Matrix2d::rotation(-25) *
                   (db::Matrix2d::shear(-17) * (db::Matrix2d::mirror(true) *
                                                db::Matrix2d::mag(17.5, 7.5)));
  ...
  db::Matrix2d mm = db::Matrix2d::rotation(m.angle()) *
                    db::Matrix2d::shear(m.shear_angle()) *
                    db::Matrix2d::mag(m.mag_x(), m.mag_y()) *
                    db::Matrix2d::mirror(m.is_mirror());
...
TEST(7) {
  ...
  m = db::Matrix3d::disp(db::DVector(-5, 3)) *
      db::Matrix3d::perspective(18, -5, 1);
  EXPECT_EQ(tl::to_string(m.is_ortho()), "false");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_x(1)), "18");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_y(1)), "-5");

  m = db::Matrix3d::disp(db::DVector(-5, 3)) *
      db::Matrix3d::perspective(18, -5, 1.5);
  EXPECT_EQ(tl::to_string(m.is_ortho()), "false");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_x(1.5)), "18");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_y(1.5)), "-5");

  m = db::Matrix3d::disp(db::DVector(-5, 3)) *
      db::Matrix3d::perspective(18, -5, 1) * db::Matrix3d::rotation(33) *
      db::Matrix3d::shear(21) * db::Matrix3d::mag(2.5) *
      db::Matrix3d::mirror(true);
  EXPECT_EQ(tl::to_string(m.is_ortho()), "false");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_x(1)), "18");
  EXPECT_EQ(tl::to_string(m.perspective_tilt_y(1)), "-5");
  ...
  db::Matrix3d mm = db::Matrix3d::rotation(m.angle()) *
                    db::Matrix3d::shear(m.shear_angle()) *
                    db::Matrix3d::mag(m.mag_x(), m.mag_y()) *
                    db::Matrix3d::mirror(m.is_mirror());
  EXPECT_EQ(mm.to_string(), "(1,5,0) (3,9,0) (0,0,1)");

  m = db::Matrix3d(1, 5, 3, 3, 9, 4, 6, 1, 1);
  mm = db::Matrix3d::disp(m.disp()) *
       db::Matrix3d::perspective(m.perspective_tilt_x(1),
                                 m.perspective_tilt_y(1), 1) *
       db::Matrix3d::rotation(m.angle()) *
       db::Matrix3d::shear(m.shear_angle()) *
       db::Matrix3d::mag(m.mag_x(), m.mag_y()) *
       db::Matrix3d::mirror(m.is_mirror());
...
TEST(8) {
  db::Matrix3d n = db::Matrix3d::disp(db::DVector(-5, 3)) *
                   db::Matrix3d::perspective(18, -5, 1) *
                   db::Matrix3d::rotation(33) * db::Matrix3d::shear(21) *
                   db::Matrix3d::mag(2.5) * db::Matrix3d::mirror(true);
...
TEST(9) {
  db::Matrix3d n = db::Matrix3d::disp(db::DVector(-5, 3)) *
                   db::Matrix3d::perspective(18, -5, 1) *
                   db::Matrix3d::rotation(33) * db::Matrix3d::shear(21) *
                   db::Matrix3d::mag(2.5, 1.5) * db::Matrix3d::mirror(true);
  ...
  db::Matrix2d n2 = db::Matrix2d::rotation(-25) *
                    (db::Matrix2d::shear(-17) *
                     (db::Matrix2d::mirror(true) * db::Matrix2d::mag(17.5)));
  ...
  EXPECT_EQ(tl::to_string(m2.angle(), 8),
            "-25"); // some roundoff happens here ..
  EXPECT_EQ(tl::to_string(m2.shear_angle(), 8),
            "-17"); // some roundoff happens here ..
}
...
TEST(11) {
  ...
  EXPECT_EQ((db::Matrix2d(1.0, 0.5, -0.5, 2.0) * db::DPoint(1, 2)).to_string(),
            "2,3.5");
  EXPECT_EQ(
      (db::IMatrix2d(1.0, 0.5, -0.5, 2.0) * db::Point(10, 20)).to_string(),
      "20,35");
  EXPECT_EQ((db::Matrix3d(1.0, 0.5, 0.0, -0.5, 2.0, 1.0, 0.0, 0.0, 1.0) *
             db::DPoint(1, 2))
                .to_string(),
            "2,4.5");
  EXPECT_EQ((db::IMatrix3d(1.0, 0.5, 0.0, -0.5, 2.0, 1.0, 0.0, 0.0, 1.0) *
             db::DPoint(10, 20))
                .to_string(),
            "20,36");
}
