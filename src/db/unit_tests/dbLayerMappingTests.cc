...
std::string nl2s(const std::vector<unsigned int> &nl, const db::Layout &a) {
  std::string res;
  for (std::vector<unsigned int>::const_iterator i = nl.begin(); i != nl.end();
       ++i) {
    if (!res.empty()) {
      res += ";";
    }
...
std::string m2s(const db::LayerMapping &lm, const db::Layout &a,
                const db::Layout &b) {
  std::string res;
...
TEST(1) {
  ...
  EXPECT_EQ(m2s(lm, gg, h), "1/0->1/0;L1->L1;L3->L3;L10D17 (10/17)->L10D17 "
                            "(10/17);L10D18 (10/18)->L10D18 (10/18)");
  EXPECT_EQ(nl2s(nl, gg), "L3;L10D18 (10/18)");

  db::Layout hh = h;
  lm.clear();
  nl = lm.create_full(hh, g);
  EXPECT_EQ(m2s(lm, hh, g),
            "1/0->1/0;2/0->2/0;L1->L1;L2->L2;L10D17 (10/17)->L10D17 "
            "(10/17);L11D17 (11/17)->L11D17 (11/17)");
