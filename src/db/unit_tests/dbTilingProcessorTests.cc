...
static std::string to_s(const db::Layout &ly, db::cell_index_type top,
                        unsigned int layer) {
  std::string r;
  for (db::RecursiveShapeIterator i(ly, ly.cell(top), layer); !i.at_end();
       ++i) {
    if (!r.empty()) {
      r += ";";
    }
...
TEST(2) {
...

  {
    ...
    EXPECT_EQ(to_s(ly, top, o1),
              "box (10,10;20,23);box (10,23;20,30);box (20,10;30,23);box "
              "(20,23;30,30);box (60,10;70,20)");
    EXPECT_EQ(to_s(ly, top, o2), "");
    EXPECT_EQ(to_s(ly, top, o3),
              "box (-5,-2;20,23);box (-5,23;20,48);box (-5,48;20,73);box "
              "(20,-2;45,23);box (20,23;45,48);box (20,48;45,73);box "
              "(45,-2;70,23);box (45,23;70,48);box (45,48;70,73);box "
              "(70,-2;95,23);box (70,23;95,48);box (70,48;95,73);box "
              "(95,-2;120,23);box (95,23;120,48);box (95,48;120,73);box "
              "(120,-2;145,23);box (120,23;145,48);box (120,48;145,73)");
  }
...
TEST(3) {
  ...
  EXPECT_EQ(or2.count() / 2000,
            size_t(50)); //  because we use rand () the value may vary - it's
                         //  only accurate to 2%
  EXPECT_EQ(or2_copy.has_valid_polygons(), true);
  EXPECT_EQ(or2_copy.count() / 2000,
            size_t(40)); //  because we use rand () the value may vary - it's
                         //  only accurate to 2%
  ...
  db::Shapes x1;
  sp.boolean(ly, ly.cell(top), o1, ly, ly.cell(top), q1, x1, db::BooleanOp::Xor,
             true);
  ...
  db::Shapes x2;
  sp.boolean(ly, ly.cell(top), o2, ly, ly.cell(top), q2, x2, db::BooleanOp::Xor,
             true);
  ...
  db::Shapes x3;
  sp.boolean(ly, ly.cell(top), o3, ly, ly.cell(top), q3, x3, db::BooleanOp::Xor,
             true);
...
TEST(4) {
...

  {
    ...
    EXPECT_EQ(to_s(o, topo, l1o),
              "polygon "
              "(3000,2000;3000,4000;1000,4000;1000,4010;3010,4010;3010,2000)");
    EXPECT_EQ(to_s(o, topo, l2o), "box (1000,2000;3000,4000)");
    ...
    EXPECT_EQ(to_s(o, topo, l1o),
              "polygon "
              "(3000,3510;3000,4000;2510,4000;2510,4010;3010,4010;3010,3510);"
              "box (1000,4000;1510,4010);box (1510,4000;2510,4010);box "
              "(3000,2000;3010,2510);box (3000,2510;3010,3510)");
    ...
    EXPECT_EQ(to_s(o, topo, l1o),
              "box (100,200;300,400);box (1000,2000;3010,4010)");
    EXPECT_EQ(to_s(o, topo, l2o), "box (100,200;300,400)");
    EXPECT_EQ(to_s(o, topo, l3o), "box (1000,2000;3010,4010)");
  }
}

class MyTilingOutputReceiver : public db::TileOutputReceiver {
public:
  MyTilingOutputReceiver(double *sum, int *n) : mp_sum(sum), mp_n(n) {}

  MyTilingOutputReceiver() : mp_sum(0), mp_n(0) {}
...
gsi::Class<MyTilingOutputReceiver> decl_MyTilingOutputReceiver(
    gsi::dbdecl_TileOutputReceiverBase(), "db", "MyTileOutputReceiver",
