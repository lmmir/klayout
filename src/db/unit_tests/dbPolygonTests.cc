...
class TestMemStatistics : public db::MemStatistics {
public:
  TestMemStatistics() : used(0), reqd(0) {}

  virtual void add(const std::type_info & /*ti*/, void * /*ptr*/, size_t r,
                   size_t u, void * /*parent*/, purpose_t /*purpose*/ = None,
                   int /*cat*/ = 0) {
    used += u;
    reqd += r;
  }

  void clear() { used = reqd = 0; }
...
TEST(1) {
  ...
  EXPECT_EQ(p.to_string(),
            std::string("(0,0;0,1000;100,1000;100,0/10,10;90,10;90,390;10,390/"
                        "10,510;90,510;90,890;10,890)"));
  db::DPolygon dp(p, db::cast_op<db::DPoint, db::Point>());
  EXPECT_EQ(dp.to_string(),
            std::string("(0,0;0,1000;100,1000;100,0/10,10;90,10;90,390;10,390/"
                        "10,510;90,510;90,890;10,890)"));
  ...
  EXPECT_EQ(p.perimeter(),
            db::Polygon::perimeter_type(2000 + 200 + 4 * (380 + 80)));
  EXPECT_EQ(p.is_box(), false);
  EXPECT_EQ(p.box(), b);

  unsigned int e = 0;
  db::Edge::distance_type u = 0;
  for (db::Polygon::polygon_edge_iterator i = p.begin_edge(); !i.at_end();
       ++i) {
    ++e;
    u += (*i).length();
  }
...
TEST(2) {
  ...
  unsigned int e = 0;
  db::Edge::distance_type u = 0;
  for (db::SimplePolygon::polygon_edge_iterator i = p.begin_edge(); !i.at_end();
       ++i) {
    ++e;
    u += (*i).length();
  }
...
TEST(3) {
  db::Point pts[] = {db::Point(100, 120), db::Point(100, 140),
                     db::Point(100, 160), db::Point(100, 180),
                     db::Point(100, 200), db::Point(0, 200),
                     db::Point(0, 300),   db::Point(300, 300),
                     db::Point(300, 100), db::Point(100, 100)};
...
TEST(4) {
  ...
  db::Point pts[] = {db::Point(100, 150), db::Point(100, 200),
                     db::Point(0, 300),   db::Point(300, 300),
                     db::Point(300, 100), db::Point(100, 100)};
...
TEST(5) {
...

  {
    ...
    EXPECT_EQ(*pt, db::Point(0, 0));
    ++pt;
    EXPECT_EQ(*pt, db::Point(0, 1000));
    ++pt;
    EXPECT_EQ(*pt, db::Point(100, 1000));
    ++pt;
    EXPECT_EQ(*pt, db::Point(100, 0));
    ++pt;
...

  {
    ...
    EXPECT_EQ(*pt, db::Point(10, 10));
    ++pt;
    EXPECT_EQ(*pt, db::Point(20, 10));
    ++pt;
    EXPECT_EQ(*pt, db::Point(20, 110));
    ++pt;
    EXPECT_EQ(*pt, db::Point(10, 110));
    ++pt;
  ...
  {
    db::polygon_ref<db::Polygon, db::Trans>::polygon_contour_iterator pt =
        pref.begin_hull();
    EXPECT_EQ(*pt, db::Point(0, 0));
    ++pt;
    EXPECT_EQ(*pt, db::Point(0, 1000));
  ...
  {
    db::Polygon::polygon_contour_iterator pt = p.begin_hull();
    EXPECT_EQ(*pt, db::Point(123, -456));
    ++pt;
    EXPECT_EQ(*pt, db::Point(123, -356));
    ++pt;
    EXPECT_EQ(*pt, db::Point(1123, -356));
    ++pt;
    EXPECT_EQ(*pt, db::Point(1123, -456));
    ++pt;
    EXPECT_EQ(pt == p.end_hull(), true);
  }
  ...
  {
    db::Polygon::polygon_contour_iterator pt = p.begin_hole(0);
    EXPECT_EQ(*pt, db::Point(133, -446));
    ++pt;
    EXPECT_EQ(*pt, db::Point(233, -446));
    ++pt;
    EXPECT_EQ(*pt, db::Point(233, -436));
    ++pt;
    EXPECT_EQ(*pt, db::Point(133, -436));
    ++pt;
    EXPECT_EQ(pt == p.end_hole(0), true);
  }
  ...
  EXPECT_EQ(pref.perimeter(),
            db::Polygon::perimeter_type(200 + 2000 + 20 + 200));
}
...
TEST(7) {
  ...
  db::Point pts[] = {
      db::Point(0, 0), db::Point(0, 4), db::Point(4, 4),
      db::Point(4, 0), db::Point(4, 4), db::Point(0, 4),
  };
...
TEST(8) {
  ...
  for (unsigned int i = 0; i < 16; ++i) {
    ...
    EXPECT_EQ(*h, db::Point(5240, -325));
    h++;
    EXPECT_EQ(*h, db::Point(5240, 5915));
    h++;
    EXPECT_EQ(*h, db::Point(10200, 5915));
    h++;
    EXPECT_EQ(*h, db::Point(10200, 5685));
    h++;
    EXPECT_EQ(*h, db::Point(6800, 5685));
    h++;
    EXPECT_EQ(*h, db::Point(6800, -325));
    h++;
  ...
  for (std::vector<db::Point>::const_iterator pp = pts.begin(); pp != pts.end();
       ++pp) {
    ppts.push_back(*pp);
    ppts.push_back(*pp);
  }
...
TEST(10) {
  ...
  db::SimplePolygon::polygon_contour_iterator h = p.begin_hull();
  EXPECT_EQ(*h, db::Point(0, 0));
  h++;
  EXPECT_EQ(*h, db::Point(0, 2000));
  h++;
  EXPECT_EQ(*h, db::Point(1000, 3000));
  h++;
  EXPECT_EQ(*h, db::Point(1000, 1000));
  h++;
  EXPECT_EQ(h == p.end_hull(), true);
}
...
TEST(11) {
  ...
  EXPECT_EQ(*h, db::Point(1000, 3000));
  h++;
  EXPECT_EQ(*h, db::Point(1000, 2000));
  h++;
  EXPECT_EQ(*h, db::Point(1000, 2000));
  h++;
  EXPECT_EQ(*h, db::Point(1000, 1000));
...
TEST(13) {
  ...
  EXPECT_EQ(p.to_string(), "(0,0;0,2000;1000,2000;1000,3000;1000,2000;1000,"
                           "2000;1000,2000;1000,1500;1000,1000)");
...
TEST(14) {
  ...
  EXPECT_EQ(p.to_string(),
            "(0,0;0,2000;1000,2000;1000,3000;1000,2000;1000,2000;1000,2000;"
            "1000,1500;1000,1000;500,500;200,200)");
...
TEST(13M) {
  ...
  EXPECT_EQ(p.to_string(), "(0,0;0,2000;1000,2000;1000,3000;1000,2000;1000,"
                           "2000;1000,2000;1000,1500;1000,1000;1000,0)");
...
TEST(14S2) {
  ...
  EXPECT_EQ(p.to_string(), "(0,0;0,100;100,100;200,200;200,200;300,100;400,100;"
                           "400,200;500,200;500,0)");
...
TEST(20) {
  ...
  db::Point pts[] = {db::Point(100, 100), db::Point(400, 100),
                     db::Point(400, 400), db::Point(100, 400)};
  ...
  poly.size(-100);
  EXPECT_EQ(poly.to_string(),
            "(100,100;200,100;200,400;100,400;100,300;400,300;400,400;300,400;"
            "300,100;400,100;400,200;100,200)");
  ...
  EXPECT_EQ(
      poly.to_string(),
      "(100,100;200,100;200,400;100,400;400,400;300,400;300,100;400,100)");

  poly.assign_hull(pts, pts + sizeof(pts) / sizeof(pts[0]));
  poly.size(db::Coord(0), db::Coord(-100));
  EXPECT_EQ(
      poly.to_string(),
      "(100,100;100,400;100,300;400,300;400,400;400,100;400,200;100,200)");

  poly.assign_hull(pts, pts + sizeof(pts) / sizeof(pts[0]));
  poly.size(-400);
  EXPECT_EQ(poly.to_string(), "(100,0;400,0;400,400;0,400;0,100;400,100;400,"
                              "500;100,500;100,100;500,100;500,400;100,400)");

  poly.assign_hull(pts, pts + sizeof(pts) / sizeof(pts[0]));
  poly.size(100, 100, 0);
  EXPECT_EQ(poly.to_string(),
            "(100,0;0,100;0,400;100,500;400,500;500,400;500,100;400,0)");

  db::Point pts2[] = {db::Point(0, 0),     db::Point(0, 400),
                      db::Point(100, 400), db::Point(100, 100),
                      db::Point(400, 100), db::Point(400, 0)};

  poly.assign_hull(pts2, pts2 + sizeof(pts2) / sizeof(pts2[0]));
  poly.size(0, -100, 2);
  EXPECT_EQ(poly.to_string(), "(0,0;0,400;0,300;100,300;100,400;100,0;400,0;"
                              "400,100;400,0;400,100;0,100)");

  db::Point pts3[] = {db::Point(0, 0),     db::Point(0, 100),
                      db::Point(50, 100),  db::Point(75, 50),
                      db::Point(150, 300), db::Point(200, 300),
                      db::Point(200, 0)};

  poly.assign_hull(pts3, pts3 + sizeof(pts3) / sizeof(pts3[0]));
  poly.size(100, 100, 4);
  EXPECT_EQ(poly.to_string(), "(-100,-100;-100,200;112,200;164,95;75,50;-21,79;"
                              "76,400;300,400;300,-100)");
  ...
  db::Point pts4[] = {db::Point(0, 0),     db::Point(0, 200),
                      db::Point(100, 300), db::Point(400, 300),
                      db::Point(200, 100), db::Point(200, 0)};

  poly.assign_hull(pts4, pts4 + sizeof(pts4) / sizeof(pts4[0]));
  poly.size(-100, -100, 2);
  EXPECT_EQ(poly.to_string(),
            "(0,0;100,0;100,200;0,200;71,129;171,229;100,300;100,200;400,200;"
            "400,300;329,371;100,142;100,0;200,0;200,100;0,100)");

  db::Point pts5[] = {db::Point(0, 0),     db::Point(0, 100),
                      db::Point(100, 100), db::Point(100, 50),
                      db::Point(150, 250), db::Point(250, 250),
                      db::Point(250, 0)};
  ...
  EXPECT_EQ(
      poly.to_string(),
      "(-50,-50;-50,150;150,150;150,50;100,50;51,62;111,300;300,300;300,-50)");

  db::Point pts6[] = {db::Point(100, 0),   db::Point(100, 100),
                      db::Point(0, 200),   db::Point(50, 250),
                      db::Point(200, 100), db::Point(200, 0)};

  poly.assign_hull(pts6, pts6 + sizeof(pts6) / sizeof(pts6[0]));
  poly.size(2, 2, 4);
  EXPECT_EQ(poly.to_string(),
            "(98,-2;98,100;100,100;99,99;-2,200;50,252;202,100;202,-2)");

  db::Point pts7[] = {db::Point(-90122, -84700), db::Point(-90162, -84652),
                      db::Point(-90195, -84613), db::Point(-90229, -84572),
                      db::Point(-90265, -84528), db::Point(-90304, -84481),
                      db::Point(-90346, -84431), db::Point(-90390, -84378),
                      db::Point(-90400, -84366), db::Point(-90400, -84300),
                      db::Point(-90000, -84300), db::Point(-90000, -84700)};

  poly.assign_hull(pts7, pts7 + sizeof(pts7) / sizeof(pts7[0]));
  poly.size(50, 50, 4);
  EXPECT_EQ(poly.to_string(),
            "(-90145,-84750;-90200,-84684;-90233,-84645;-90267,-84604;-90304,-"
            "84560;-90342,-84513;-90384,-84463;-90428,-84410;-90450,-84384;-"
            "90450,-84250;-89950,-84250;-89950,-84750)");
}
...
TEST(21) {
  ...
  }
  {
    ...
    db::DCplxTrans t(7.02268521, 45, false,
                     db::DVector(123.88147866, -10152.0640046));
    EXPECT_EQ(poly.to_string(), "(0,0;0,1536;2048,1536;2048,0)");
    poly.transform(t);
    EXPECT_EQ(poly.is_box(), false);
    EXPECT_EQ(poly.to_string(),
              "(123.88147866,-10152.0640046;-7503.56940256,-2524.61312338;2666."
              "36510573,7645.32138492;10293.815987,17.8705036972)");
#if !defined(_MSC_VER)
    ms.clear();
    poly.mem_stat(&ms, db::MemStatistics::None, 0);
...

  {
    db::DBox box(0, 0, 2048, 1536);
    db::DPolygon poly(box);
    db::DCplxTrans t(7.02268521, 0, false,
                     db::DVector(123.88147866, -10152.0640046));
    EXPECT_EQ(poly.to_string(), "(0,0;0,1536;2048,1536;2048,0)");
    // This transformation was not terminating in some builds (release):
    poly.transform(t);
    EXPECT_EQ(poly.to_string(),
              "(123.88147866,-10152.0640046;123.88147866,634.78047796;14506."
              "3407887,634.78047796;14506.3407887,-10152.0640046)");
#if !defined(_MSC_VER)
    ms.clear();
    poly.mem_stat(&ms, db::MemStatistics::None, 0);
    EXPECT_EQ(ms.reqd,
              (sizeof(void *) - 4) * 5 + 116); // no compression for doubles!
#endif
  }
}
...
TEST(22) {
  ...
  std::string s("(0,0;0,1000;100,1000;100,0/10,10;90,10;90,390;10,390/"
                "10,510;90,510;90,890;10,890)");
