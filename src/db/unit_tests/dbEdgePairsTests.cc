...
TEST(1) {
  ...
  EXPECT_EQ(ep != db::EdgePairs(), false);
  ep.insert(db::Edge(db::Point(10, 20), db::Point(110, 120)),
            db::Edge(db::Point(-10, -20), db::Point(90, 80)));
  ...
  EXPECT_EQ(ep.bbox().to_string(), "()");
  ep.insert(db::EdgePair(db::Edge(db::Point(10, 20), db::Point(110, 120)),
                         db::Edge(db::Point(-10, -20), db::Point(90, 80))));
  EXPECT_EQ(ep == db::EdgePairs(), false);
  EXPECT_EQ(ep < db::EdgePairs(), true);
  EXPECT_EQ(ep != db::EdgePairs(), true);
  ...
  EXPECT_EQ(
      ep.transformed(db::ICplxTrans(2.0, 0.0, false, db::Vector())).to_string(),
      "(20,40;220,240)/(-20,-40;180,160)");
  EXPECT_EQ(ep.to_string(), "(10,20;110,120)/(-10,-20;90,80)");
...
TEST(2) {
  ...
  ep.insert(db::EdgePair(db::Edge(db::Point(10, 20), db::Point(110, 120)),
                         db::Edge(db::Point(-10, -20), db::Point(90, 80))));
  ep.insert(db::EdgePair(db::Edge(db::Point(10, 20), db::Point(110, 120)),
                         db::Edge(db::Point(90, 80), db::Point(-10, -20))));

  EXPECT_EQ(
      db::compare(
          ep,
          "(10,20;110,120)/(-10,-20;90,80);(10,20;110,120)/(90,80;-10,-20)"),
      true);
  ...
  EXPECT_EQ(
      db::compare(
          e, "(10,20;110,120);(-10,-20;90,80);(10,20;110,120);(90,80;-10,-20)"),
      true);
  e.clear();
  ep.first_edges(e);
  ...
  EXPECT_EQ(
      db::compare(
          r, "(-10,-20;10,20;110,120;90,80);(-10,-20;10,20;110,120;90,80)"),
      true);
}

struct EPTestFilter : public db::EdgePairFilterBase {
  bool selected(const db::EdgePair &ep) const {
    return ep.first().double_length() < 50;
  }

  const db::TransformationReducer *vars() const { return &m_vars; }

  bool wants_variants() const { return false; }
...
TEST(3) {
  db::EdgePairs ep;
  ep.insert(db::EdgePair(db::Edge(db::Point(10, 20), db::Point(50, 50)),
                         db::Edge(db::Point(-10, -20), db::Point(90, 80))));
  ep.insert(db::EdgePair(db::Edge(db::Point(10, 20), db::Point(110, 120)),
                         db::Edge(db::Point(90, 80), db::Point(-10, -20))));
...
TEST(4) {
  ...
  EXPECT_EQ(
      db::compare(r, "(-10,-21;9,20;50,51;91,80);(-10,-21;9,20;110,121;91,80)"),
      true);
}

TEST(5_InternalAngleFilter) {
  db::EdgePair ep0(db::Edge(db::Point(0, 0), db::Point(100, 0)),
                   db::Edge(db::Point(100, 0), db::Point(0, 0)));
  db::EdgePair ep45(db::Edge(db::Point(0, 0), db::Point(100, 0)),
                    db::Edge(db::Point(0, 0), db::Point(100, 100)));
  db::EdgePair ep180(db::Edge(db::Point(0, 0), db::Point(100, 0)),
                     db::Edge(db::Point(0, 0), db::Point(100, 0)));
  db::EdgePair ep90(db::Edge(db::Point(0, 0), db::Point(100, 0)),
                    db::Edge(db::Point(0, 0), db::Point(0, 100)));
  db::EdgePair epm90(db::Edge(db::Point(0, 0), db::Point(100, 0)),
  ...
  EXPECT_EQ(db::InternalAngleEdgePairFilter(90.0, false).selected(ep180),
            false);
  ...
  EXPECT_EQ(db::InternalAngleEdgePairFilter(45.0, false).selected(ep180),
            false);
  EXPECT_EQ(db::InternalAngleEdgePairFilter(45.0, false).selected(ep90), false);
  EXPECT_EQ(db::InternalAngleEdgePairFilter(45.0, false).selected(epm90),
            false);
  ...
  EXPECT_EQ(db::InternalAngleEdgePairFilter(0.0, true, 45.0, true, false)
                .selected(ep0),
            true);
  EXPECT_EQ(db::InternalAngleEdgePairFilter(0.0, true, 45.0, true, false)
                .selected(ep180),
