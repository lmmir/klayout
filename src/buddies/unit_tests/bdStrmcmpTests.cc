...
TEST(2A) {
  ...
  EXPECT_EQ(cap.captured_text(), "Boxes differ for layer 8/0 in cell RINGO\n"
                                 "Not in b but in a:\n"
...
TEST(2C) {
  ...
  const char *argv[] = {"x",   "-am=8/0",       "-as",          "-bm=8/0",
                        "-bs", input_a.c_str(), input_b.c_str()};
...
TEST(2E) {
  ...
  const char *argv[] = {"x", "-ta=INV2", "-tb=INV2", input_a.c_str(),
                        input_b.c_str()};
...
TEST(2F) {
  ...
  EXPECT_EQ(cap.captured_text(),
            "Bounding boxes differ for cell RINGO, (-1720,-800;25160,3800) vs. "
            "(-1700,-800;25160,3800)\n"
            "Per-layer bounding boxes differ for cell RINGO, layer (8/0), "
            "(-1720,-450;25160,3250) vs. (-1520,-450;25160,3250)\n"
...
TEST(4A) {
  ...
  EXPECT_EQ(cap.captured_text(), "Paths differ for layer 3/0 in cell TRANS\n"
                                 "Not in b but in a:\n"
...
TEST(5A) {
  ...
  EXPECT_EQ(cap.captured_text(), "Instances differ in cell RINGO\n"
                                 "Layouts differ\n");
}
...
TEST(5B) {
  ...
  const char *argv[] = {"x", "--expand-arrays", input_a.c_str(),
                        input_b.c_str()};
...
TEST(8B) {
  ...
  EXPECT_EQ(cap.captured_text(), "Cell TRANS in a is renamed to SNART in b\n");
}
...
TEST(9B) {
  ...
  EXPECT_EQ(cap.captured_text(), "Texts differ for layer 8/1 in cell RINGO\n"
                                 "Not in b but in a:\n"
                                 "  ('VSS',r0 0,0)\n"
                                 "  ('FB',r0 0,1800)\n"
                                 "  ('OSC',r0 24560,1800)\n"
