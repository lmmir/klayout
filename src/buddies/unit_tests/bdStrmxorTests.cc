...
TEST(0_Basic_Flat) {
  ...
  EXPECT_EQ(cap.captured_text(), "No differences found\n");
}
...
TEST(1A_Flat) {
  ...
  db::compare_layouts(
      this, layout, au,
      db::NormalizationMode(db::NoNormalization | db::AsPolygons));
...
TEST(1A_Deep) {
  ...
  const char *argv[] = {"x", "--deep", input_a.c_str(), input_b.c_str(),
                        output.c_str()};
...
TEST(1C_Deep) {
  ...
  const char *argv[] = {"x", "-u", "--no-summary", input_a.c_str(),
                        input_b.c_str()};
...
TEST(2_Flat) {
  ...
  const char *argv[] = {
      "x",           "--no-summary", "-l", input_a.c_str(), input_b.c_str(),
      output.c_str()};
...
TEST(3_Flat) {
  ...
  const char *argv[] = {"x",           "--no-summary",  "-p=1.0",
                        "-n=4",        input_a.c_str(), input_b.c_str(),
                        output.c_str()};
...
TEST(3_FlatCount) {
  ...
  const char *argv[] = {"x", "-p=1.0", "-n=4", input_a.c_str(),
                        input_b.c_str()};
...
TEST(3_FlatHeal) {
  ...
  const char *argv[] = {
      "x",    "--heal",        "--no-summary",  "-p=1.0",
      "-n=4", input_a.c_str(), input_b.c_str(), output.c_str()};
...
TEST(3_FlatCountHeal) {
  ...
  const char *argv[] = {
      "x", "-m", "-p=1.0", "-n=4", input_a.c_str(), input_b.c_str()};
...
TEST(4_Flat) {
  ...
  const char *argv[] = {"x",
                        ...
                        "-p=1.0",
                        "-n=4",
                        "-t=0.0,0.005,0.01,0.02,0.09,0.1",
                        input_a.c_str(),
                        input_b.c_str(),
                        output.c_str()};
...
TEST(6_Flat) {
  ...
  const char *argv[] = {"x",           "--no-summary",  "-ta=INV2",
                        "-tb=2VNI",    input_a.c_str(), input_b.c_str(),
                        output.c_str()};
