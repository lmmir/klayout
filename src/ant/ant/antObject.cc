...
static void clean_points_impl(ant::Object::point_list &points) {
...
Object::Object()
    : m_id(-1), m_fmt_x("$X"), m_fmt_y("$Y"), m_fmt("$D"), m_style(STY_ruler),
      m_outline(OL_diag), m_snap(true), m_angle_constraint(lay::AC_Global),
      m_main_position(POS_auto), m_main_xalign(AL_auto), m_main_yalign(AL_auto),
      m_xlabel_xalign(AL_auto), m_xlabel_yalign(AL_auto),
      m_ylabel_xalign(AL_auto), m_ylabel_yalign(AL_auto) {
  //  .. nothing yet ..
}

Object::Object(const db::DPoint &_p1, const db::DPoint &_p2, int id,
    ...
    : m_id(id), m_fmt_x(fmt_x), m_fmt_y(fmt_y), m_fmt(fmt), m_style(style),
      m_outline(outline), m_snap(snap), m_angle_constraint(angle_constraint),
      m_main_position(POS_auto), m_main_xalign(AL_auto), m_main_yalign(AL_auto),
      m_xlabel_xalign(AL_auto), m_xlabel_yalign(AL_auto),
      m_ylabel_xalign(AL_auto), m_ylabel_yalign(AL_auto) {
  p1(_p1);
...
Object::Object(const Object::point_list &pts, int id, const std::string &fmt_x,
               const std::string &fmt_y, const std::string &fmt,
               style_type style, outline_type outline, bool snap,
...
Object::Object(const db::DPoint &_p1, const db::DPoint &_p2, int id,
               const ant::Template &t)
    : m_id(id), m_fmt_x(t.fmt_x()), m_fmt_y(t.fmt_y()), m_fmt(t.fmt()),
      m_style(t.style()), m_outline(t.outline()), m_snap(t.snap()),
      m_angle_constraint(t.angle_constraint()), m_category(t.category()),
      m_main_position(t.main_position()), m_main_xalign(t.main_xalign()),
      m_main_yalign(t.main_yalign()), m_xlabel_xalign(t.xlabel_xalign()),
      m_xlabel_yalign(t.xlabel_yalign()), m_ylabel_xalign(t.ylabel_xalign()),
      m_ylabel_yalign(t.ylabel_yalign()) {
  p1(_p1);
  p2(_p2);
}

Object::Object(const Object::point_list &pts, int id, const ant::Template &t)
    : m_points(pts), m_id(id), m_fmt_x(t.fmt_x()), m_fmt_y(t.fmt_y()),
      m_fmt(t.fmt()), m_style(t.style()), m_outline(t.outline()),
      m_snap(t.snap()), m_angle_constraint(t.angle_constraint()),
      m_category(t.category()), m_main_position(t.main_position()),
...
Object::Object(const ant::Object &d)
    : m_points(d.m_points), m_id(d.m_id), m_fmt_x(d.m_fmt_x),
      m_fmt_y(d.m_fmt_y), m_fmt(d.m_fmt), m_style(d.m_style),
      m_outline(d.m_outline), m_snap(d.m_snap),
      m_angle_constraint(d.m_angle_constraint), m_category(d.m_category),
      m_main_position(d.m_main_position), m_main_xalign(d.m_main_xalign),
      m_main_yalign(d.m_main_yalign), m_xlabel_xalign(d.m_xlabel_xalign),
      m_xlabel_yalign(d.m_xlabel_yalign), m_ylabel_xalign(d.m_ylabel_xalign),
...
Object &Object::operator=(const ant::Object &d) {
  if (this != &d) {
    m_points = d.m_points;
    m_id = d.m_id;
    m_fmt_x = d.m_fmt_x;
...
bool Object::operator==(const ant::Object &d) const {
  return m_points == d.m_points && m_id == d.m_id && m_fmt_x == d.m_fmt_x &&
         m_fmt_y == d.m_fmt_y && m_fmt == d.m_fmt && m_style == d.m_style &&
         m_outline == d.m_outline && m_snap == d.m_snap &&
         m_angle_constraint == d.m_angle_constraint &&
         m_category == d.m_category && m_main_position == d.m_main_position &&
...
db::DPoint Object::seg_p1(size_t seg_index) const {
...
db::DPoint Object::seg_p2(size_t seg_index) const {
  if (seg_index == std::numeric_limits<size_t>::max()) {
    return p2();
...
db::DUserObjectBase *Object::clone() const { return new ant::Object(*this); }
...
class AnnotationEval : public tl::Eval {
public:
...
class AnnotationEvalFunction : public tl::EvalFunction {
public:
  ...
  void execute(const tl::ExpressionParserContext &context, tl::Variant &out,
    ...
    if (vv.size() != 0) {
      throw tl::EvalError(
          tl::to_string(tr("Annotation function must not have arguments")),
          context);
    }
    ...
    if (m_function == 'L') {
      ...
      out = sqrt(delta_x(obj, trans) * delta_x(obj, trans) +
                 delta_y(obj, trans) * delta_y(obj, trans));
  ...
  db::DPoint p1(const Object &obj) const { return obj.seg_p1(m_index); }

  db::DPoint p2(const Object &obj) const { return obj.seg_p2(m_index); }

  double delta_x(const Object &obj, const db::DFTrans &t) const {
  ...
  double delta_y(const Object &obj, const db::DFTrans &t) const {
...
std::string Object::formatted(const std::string &fmt, const db::DFTrans &t,
                              size_t index) const {
  AnnotationEval eval(*this, t);
  eval.define_function(
      "L", new AnnotationEvalFunction('L', &eval, index)); // manhattan length
  eval.define_function(
      "D", new AnnotationEvalFunction('D', &eval, index)); // euclidian distance
  eval.define_function(
      "X", new AnnotationEvalFunction('X', &eval, index)); // x delta
  eval.define_function(
      "Y", new AnnotationEvalFunction('Y', &eval, index)); // y delta
  eval.define_function("U",
                       new AnnotationEvalFunction('U', &eval, index)); // p1.x
  eval.define_function("V",
                       new AnnotationEvalFunction('V', &eval, index)); // p1.y
  eval.define_function("P",
                       new AnnotationEvalFunction('P', &eval, index)); // p2.x
  eval.define_function("Q",
                       new AnnotationEvalFunction('Q', &eval, index)); // p2.y
  eval.define_function(
      "A", new AnnotationEvalFunction('A', &eval, index)); // area mm2
  eval.define_function("G", new AnnotationEvalFunction(
                                'G', &eval, index)); // angle (if applicable)
  return eval.interpolate(fmt);
}

const char *Object::class_name() const { return "ant::Object"; }
...
bool Object::compute_interpolating_circle(double &radius, db::DPoint &center,
                                          double &start_angle,
...
bool Object::compute_angle_parameters(double &radius, db::DPoint &center,
                                      double &start_angle,
                                      double &stop_angle) const {
  if (m_points.size() < 3) {
    return false;
...
static db::DUserObjectDeclaration class_registrar(
    new db::user_object_factory_impl<ant::Object, db::DCoord>("ant::Object"));
