...
class RulerWithModifiedProperty : public ant::Object {
public:
  RulerWithModifiedProperty() : ant::Object(), m_modified(false) {
    //  .. nothing yet ..
  }
...
PropertiesPage::PropertiesPage(ant::Service *rulers, db::Manager *manager,
                               QWidget *parent)
    : lay::PropertiesPage(parent, manager, rulers), mp_rulers(rulers),
      m_enable_cb_callback(true), m_in_something_changed(false) {
  ...
  connect(p1_to_layout, SIGNAL(clicked()), this,
          SLOT(snap_to_layout_clicked()));
  connect(p2_to_layout, SIGNAL(clicked()), this,
          SLOT(snap_to_layout_clicked()));
  connect(both_to_layout, SIGNAL(clicked()), this,
          SLOT(snap_to_layout_clicked()));
  ...
  if (!readonly()) {
    ...
    connect(fmt_x_le, SIGNAL(editingFinished()), this,
            SLOT(something_changed()));
    connect(fmt_y_le, SIGNAL(editingFinished()), this,
            SLOT(something_changed()));
    ...
    connect(outline_cb, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(main_position, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(main_xalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(main_yalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(xlabel_xalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(xlabel_yalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(ylabel_xalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));
    connect(ylabel_yalign, SIGNAL(activated(int)), this,
            SLOT(something_changed()));

    connect(points_edit, SIGNAL(textChanged()), this,
            SLOT(something_changed()));
...
PropertiesPage::~PropertiesPage() { mp_rulers->restore_highlights(); }
...
void PropertiesPage::snap_to_layout_clicked() {
  ...
  if (sender() == p1_to_layout || sender() == p2_to_layout) {
    ...
    double snap_range = service->ui()->mouse_event_trans().inverted().ctrans(
        service->snap_range());
    double max_range = 1000 * snap_range;

    while (snap_range < max_range) {

      lay::PointSnapToObjectResult pp =
          lay::obj_snap(service->view(), snap_p1 ? p2 : p1, snap_p1 ? p1 : p2,
                        g, ac, snap_range);
    ...
    lay::TwoPointSnapToObjectResult ee = lay::obj_snap2(
        service->view(), p1, p2, g, ac, snap_range, snap_range * 1000.0);
    if (ee.any) {
...
const ant::Object &PropertiesPage::current() const {
  const ant::Object *ruler =
      dynamic_cast<const ant::Object *>(m_selection[m_index]->ptr());
  return *ruler;
}
...
std::string PropertiesPage::description(size_t entry) const {
  const ant::Object *obj =
      dynamic_cast<const ant::Object *>(m_selection[entry]->ptr());
  if (!obj) {
  ...
  if (!obj->category().empty()) {
    ...
    if (cat.size() >= 2 && cat[0] == '_') {
      cat = tl::to_upper_case(std::string(cat.begin() + 1, cat.begin() + 2)) +
            std::string(cat.begin() + 2, cat.end());
    }
    d += "[" + cat + "]";
  }
...
void PropertiesPage::leave() { mp_rulers->clear_highlights(); }
...
void PropertiesPage::update_with(const ant::Object &obj) {
  ...
  segments_tab->setTabEnabled(1, obj.points().size() <= 2 &&
                                     obj.points().size() > 0);
  ...
  for (auto p = obj.points().begin(); p != obj.points().end(); ++p) {
    ...
    item->setData(0, Qt::DisplayRole,
                  QVariant(tl::to_qstring(tl::micron_to_string(p->x()))));
    item->setData(1, Qt::DisplayRole,
                  QVariant(tl::to_qstring(tl::micron_to_string(p->y()))));
  }
...
void PropertiesPage::get_object(ant::Object &obj) {
  ...
  Object::outline_type outline =
      Object::outline_type(outline_cb->currentIndex());
  ...
  if (segments_tab->currentIndex() == 0 || segments_tab->currentIndex() == 1) {
    ...
    obj = ant::Object(p1, p2, current().id(), fmt_x, fmt_y, fmt, style, outline,
                      current().snap(), current().angle_constraint());

  } else if (segments_tab->currentIndex() == 2 ||
             segments_tab->currentIndex() == 3) {

    ant::Object::point_list points;
    get_points(points);

    obj = ant::Object(points, current().id(), fmt_x, fmt_y, fmt, style, outline,
                      current().snap(), current().angle_constraint());
  }
