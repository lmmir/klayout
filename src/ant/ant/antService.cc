...
static lay::angle_constraint_type ac_from_buttons(unsigned int buttons) {
...
static void tick_spacings(double d, double min_d, int &minor_ticks,
                          double &ticks) {
  if (min_d > d) {
 ...
 *  @param right True to draw the ruler with ticks to the right (as seem from p1
 * to p2 in transformed space)
 ...
 */
void draw_ruler(const db::DPoint &q1, const db::DPoint &q2, double length_u,
                double min_spc_u, bool sel, bool right,
                ant::Object::style_type style, lay::CanvasPlane *bitmap,
                lay::Renderer &renderer, bool first_segment, bool last_segment,
                bool no_line = false) {
  ...
  if (length_u < 1e-5 /*micron*/ && style != ant::Object::STY_cross_both &&
      style != ant::Object::STY_cross_end &&
      style != ant::Object::STY_cross_start) {
    ...
    double tick_length =
        (style == ant::Object::STY_ruler ? ruler_tick_length : 0) /
        renderer.resolution();
    ...
    if (!no_line && len < double(arrow_length) * 2.4) {
      if ((style == ant::Object::STY_arrow_end ||
           style == ant::Object::STY_arrow_start)) {
    ...
    if (!no_line && style != ant::Object::STY_none) {
      ...
      if (sel) {
        ...
        } else if (style == ant::Object::STY_cross_both ||
                   style == ant::Object::STY_cross_start) {
        ...
        if (!last_segment) {
          //  no end indicator if not last segment
                   ...
                   style == ant::Object::STY_arrow_end) {
    ...
    } else if (style == ant::Object::STY_arrow_end ||
               style == ant::Object::STY_arrow_both) {

      db::DPolygon p;
      db::DPoint points[] = {
          db::DPoint(q2),
          db::DPoint(q2 + qq * double(arrow_width * 0.5) -
                     qu * double(arrow_length)),
          db::DPoint(q2 - qq * double(arrow_width * 0.5) -
                     qu * double(arrow_length)),
      };
      p.assign_hull(points, points + sizeof(points) / sizeof(points[0]));
    ...
    } else if (style == ant::Object::STY_cross_end ||
               style == ant::Object::STY_cross_both) {
    ...
    } else if (style == ant::Object::STY_arrow_start ||
               style == ant::Object::STY_arrow_both) {

      db::DPolygon p;
      db::DPoint points[] = {db::DPoint(q1),
                             db::DPoint(q1 + qq * double(arrow_width * 0.5) +
                                        qu * double(arrow_length)),
                             db::DPoint(q1 - qq * double(arrow_width * 0.5) +
                                        qu * double(arrow_length))};
      p.assign_hull(points, points + sizeof(points) / sizeof(points[0]));
    ...
    } else if (style == ant::Object::STY_cross_start ||
               style == ant::Object::STY_cross_both) {
 ...
 *  @param renderer The renderer object
 */
void draw_text(const db::DPoint &q1, const db::DPoint &q2, double length_u,
               const std::string &label, bool right,
               ant::Object::style_type style, ant::Object::position_type pos,
               ant::Object::alignment_type halign,
               ant::Object::alignment_type valign, lay::CanvasPlane *bitmap,
  ...
  if (length_u < 1e-5 /*micron*/ && style != ant::Object::STY_cross_both &&
    ...
    renderer.draw(db::DBox(q1, q1), label, db::DefaultFont, db::HAlignLeft,
                  db::VAlignBottom, db::DFTrans(db::DFTrans::r0), 0, 0, 0,
                  bitmap);
    ...
    if ((style == ant::Object::STY_arrow_end ||
         style == ant::Object::STY_arrow_start) &&
        len < double(arrow_length) * 1.2) {
      arrow_length = len / 1.2;
      arrow_width = arrow_length * 2.0 / 3.0;
    } else if (style == ant::Object::STY_arrow_both &&
               len < double(arrow_length) * 2.4) {
      arrow_length = len / 2.4;
      arrow_width = arrow_length * 2.0 / 3.0;
    }
    ...
    db::DVector tv_text;
    if (style == ant::Object::STY_arrow_start ||
        style == ant::Object::STY_arrow_both ||
        style == ant::Object::STY_arrow_end) {
    ...
    renderer.draw(db::DBox(tp + tv_text, tp + tv_text), label, db::DefaultFont,
                  text_halign, text_valign, db::DFTrans(db::DFTrans::r0), 0, 0,
                  0, bitmap);
  }
}
 ...
 *  @param q1 The first point in pixel space
 *  @param q2 The second point in pixel space
 *  @param length_u The "typical dimension" - used to simplify for very small
 * ellipses
 *  @param sel True to draw ruler in "selected" mode
 *  @param bitmap The bitmap to draw the ruler on
 *  @param renderer The renderer object
 ...
 */
void draw_ellipse(const db::DPoint &q1, const db::DPoint &q2, double length_u,
                  bool sel, lay::CanvasPlane *bitmap, lay::Renderer &renderer,
                  double start_angle = 0.0, double stop_angle = 2.0 * M_PI) {
  double sel_width = 2 / renderer.resolution();
  ...
  if (length_u < 1e-5 /*micron*/) {
    ...
    int npoints =
        int(floor(200 * abs(stop_angle - start_angle) / (2.0 * M_PI)));
...
void draw_ruler_segment(const ant::Object &ruler, size_t index,
                        const db::DCplxTrans &trans, bool sel,
                        lay::CanvasPlane *bitmap, lay::Renderer &renderer) {
  bool last_segment = (index == ruler.segments() - 1 ||
                       index == std::numeric_limits<size_t>::max());
  bool first_segment =
      (index == 0 || index == std::numeric_limits<size_t>::max());
  ...
  int min_tick_spc = int(
      0.5 + 20 / renderer.resolution()); //  min tick spacing in canvas units
  double mu = double(min_tick_spc) / trans.ctrans(1.0);

  if (ruler.outline() == Object::OL_diag) {
    draw_ruler(q1, q2, lu, mu, sel, q2.x() < q1.x(), ruler.style(), bitmap,
               renderer, first_segment, last_segment);
    draw_text(q1, q2, lu, ruler.text(index), q2.x() < q1.x(), ruler.style(),
              ruler.main_position(), ruler.main_xalign(), ruler.main_yalign(),
              bitmap, renderer);
  }

  if ((!xy_swapped && (ruler.outline() == Object::OL_xy ||
                       ruler.outline() == Object::OL_diag_xy)) ||
      (xy_swapped && (ruler.outline() == Object::OL_yx ||
                      ruler.outline() == Object::OL_diag_yx))) {

    bool r = (q2.x() > q1.x()) ^ (q2.y() < q1.y());

    if (ruler.outline() == Object::OL_diag_xy ||
        ruler.outline() == Object::OL_diag_yx) {
      draw_ruler(q1, q2, lu, mu, sel, !r, ruler.style(), bitmap, renderer,
                 first_segment, last_segment);
      draw_text(q1, q2, lu, ruler.text(index), !r, ruler.style(),
                ruler.main_position(), ruler.main_xalign(), ruler.main_yalign(),
                bitmap, renderer);
    }
    draw_ruler(q1, db::DPoint(q2.x(), q1.y()), lu, mu, sel, r, ruler.style(),
               bitmap, renderer, false, false);
    draw_text(q1, db::DPoint(q2.x(), q1.y()), lu,
              ruler.text_x(index, trans.fp_trans()), r, ruler.style(),
              ant::Object::POS_center, ruler.xlabel_xalign(),
              ruler.xlabel_yalign(), bitmap, renderer);
    draw_ruler(db::DPoint(q2.x(), q1.y()), q2, lu, mu, sel, r, ruler.style(),
               bitmap, renderer, false, false);
    draw_text(db::DPoint(q2.x(), q1.y()), q2, lu,
              ruler.text_y(index, trans.fp_trans()), r, ruler.style(),
              ant::Object::POS_center, ruler.ylabel_xalign(),
              ruler.ylabel_yalign(), bitmap, renderer);
  }

  if ((!xy_swapped && (ruler.outline() == Object::OL_yx ||
                       ruler.outline() == Object::OL_diag_yx)) ||
      (xy_swapped && (ruler.outline() == Object::OL_xy ||
                      ruler.outline() == Object::OL_diag_xy))) {
    ...
    draw_ruler(q1, db::DPoint(q1.x(), q2.y()), lu, mu, sel, r, ruler.style(),
               bitmap, renderer, false, false);
    draw_text(q1, db::DPoint(q1.x(), q2.y()), lu,
              ruler.text_y(index, trans.fp_trans()), r, ruler.style(),
              ant::Object::POS_center, ruler.ylabel_xalign(),
              ruler.ylabel_yalign(), bitmap, renderer);
    draw_ruler(db::DPoint(q1.x(), q2.y()), q2, lu, mu, sel, r, ruler.style(),
               bitmap, renderer, false, false);
    draw_text(db::DPoint(q1.x(), q2.y()), q2, lu,
              ruler.text_x(index, trans.fp_trans()), r, ruler.style(),
              ant::Object::POS_center, ruler.xlabel_xalign(),
              ruler.xlabel_yalign(), bitmap, renderer);
  }
...
void draw_ruler_box(const ant::Object &ruler, const db::DCplxTrans &trans,
                    bool sel, lay::CanvasPlane *bitmap,
                    lay::Renderer &renderer) {
  ...
  draw_ruler(db::DPoint(q1.x(), q2.y()), q2, lu, mu, sel, !r, ruler.style(),
             bitmap, renderer, true, true);
  draw_text(q1, q2, lu, ruler.text(index), !r, ant::Object::STY_none,
            ruler.main_position(), ruler.main_xalign(), ruler.main_yalign(),
            bitmap, renderer);
}

void draw_ruler_ellipse(const ant::Object &ruler, const db::DCplxTrans &trans,
                        bool sel, lay::CanvasPlane *bitmap,
                        lay::Renderer &renderer) {
  db::DPoint p1 = ruler.p1(), p2 = ruler.p2();
  ...
  size_t index = std::numeric_limits<size_t>::max();
  draw_text(q1, db::DPoint(q2.x(), q1.y()), lu,
            ruler.text_x(index, trans.fp_trans()), r, ant::Object::STY_none,
            ant::Object::POS_center, ruler.xlabel_xalign(),
            ruler.xlabel_yalign(), bitmap, renderer);
  draw_text(db::DPoint(q2.x(), q1.y()), q2, lu,
            ruler.text_y(index, trans.fp_trans()), r, ant::Object::STY_none,
            ant::Object::POS_center, ruler.ylabel_xalign(),
            ruler.ylabel_yalign(), bitmap, renderer);
  draw_text(q1, q2, lu, ruler.text(index), !r, ant::Object::STY_none,
            ruler.main_position(), ruler.main_xalign(), ruler.main_yalign(),
...
void draw_ruler_radius(const ant::Object &ruler, const db::DCplxTrans &trans,
                       bool sel, lay::CanvasPlane *bitmap,
                       lay::Renderer &renderer) {
  //  draw crosses for the support points
  for (auto p = ruler.points().begin(); p != ruler.points().end(); ++p) {
    ant::Object supp(*p, *p, 0, std::string(), std::string(), std::string(),
                     ant::Object::STY_cross_start, ant::Object::OL_diag, false,
                     lay::AC_Global);
  ...
  if (ruler.compute_interpolating_circle(radius, center, start_angle,
                                         stop_angle)) {

    //  draw circle segment
    db::DVector rr(radius, radius);
    std::pair<db::DPoint, db::DPoint> v =
        lay::snap(trans * (center - rr), trans * (center + rr));
    draw_ellipse(v.first, v.second, radius * 2.0, sel, bitmap, renderer,
...
void draw_ruler_angle(const ant::Object &ruler, const db::DCplxTrans &trans,
                      bool sel, lay::CanvasPlane *bitmap,
                      lay::Renderer &renderer) {
  ...
  for (int p = 0; p < 2; ++p) {
    ...
    draw_ruler(q1, q2, lu, 0.0, sel, false, ant::Object::STY_line, bitmap,
               renderer, true, true);
  }
  ...
  if (!ruler.compute_angle_parameters(radius, center, start_angle,
                                      stop_angle)) {
    return;
  }
  ...
  for (int p = 0; p < 2; ++p) {
    ...
    draw_ruler(q1, q2, lu, 0.0, sel, false, ruler.style(), bitmap, renderer,
               p == 0, p != 0, true);
  }
  ...
  if (ruler.style() == ant::Object::STY_ruler) {
    ...
    double n =
        floor(db::epsilon + std::min(2 * M_PI, stop_angle - start_angle) / da);
...

  {
    ...
    draw_text(q1, q2, lu, ruler.text(0), false, ruler.style(),
              ruler.main_position(), ruler.main_xalign(), ruler.main_yalign(),
              bitmap, renderer);
  }
}

void draw_ruler(const ant::Object &ruler, const db::DCplxTrans &trans, bool sel,
                lay::CanvasPlane *bitmap, lay::Renderer &renderer) {
...
static bool is_selected_by_circle_segment(const ant::Object &ruler,
                                          const db::DPoint &pos, double enl,
                                          double &distance) {
...
static bool is_selected(const ant::Object &ruler, size_t index,
                        const db::DPoint &pos, double enl, double &distance) {
  ...
  if (outline == ant::Object::OL_ellipse) {
    ...
    if (b.height() > 1e-6 && b.width() > 1e-6) {
      ...
      if (dd > 1e-6) {
        //  ref is the cutpoint between the ray between pos and the ellipse
        //  center and the ellipse itself
        db::DPoint ref = b.center() + db::DVector(dx * b.width() * 0.5 / dd,
                                                  dy * b.height() * 0.5 / dd);
  ...
  if (outline == ant::Object::OL_diag || outline == ant::Object::OL_angle ||
      outline == ant::Object::OL_radius || outline == ant::Object::OL_diag_xy ||
      outline == ant::Object::OL_diag_yx) {
    edges[nedges++] = db::DEdge(p1, p2);
  }
  if (outline == ant::Object::OL_xy || outline == ant::Object::OL_diag_xy ||
      outline == ant::Object::OL_box || outline == ant::Object::OL_ellipse) {
    edges[nedges++] = db::DEdge(p1, db::DPoint(p2.x(), p1.y()));
    edges[nedges++] = db::DEdge(db::DPoint(p2.x(), p1.y()), p2);
  }
  if (outline == ant::Object::OL_yx || outline == ant::Object::OL_diag_yx ||
      outline == ant::Object::OL_box || outline == ant::Object::OL_ellipse) {
...
static bool is_selected(const ant::Object &ruler, const db::DPoint &pos,
                        double enl, double &distance) {
  distance = std::numeric_limits<double>::max();
  bool any = false;

  if (ruler.outline() == ant::Object::OL_box ||
      ruler.outline() == ant::Object::OL_ellipse) {
    return is_selected(ruler, std::numeric_limits<size_t>::max(), pos, enl,
                       distance);
  } else if (ruler.outline() == ant::Object::OL_angle ||
             ruler.outline() == ant::Object::OL_radius) {
...
static bool is_selected(const ant::Object &ruler, const db::DBox &box,
                        double /*enl*/) {
...
View::View(ant::Service *rulers, const ant::Object *ruler, bool selected)
    : lay::ViewObject(rulers->ui()), mp_rulers(rulers), m_selected(selected),
      mp_ruler(ruler) {
  //  .. nothing else ..
}
...
void View::ruler(const ant::Object *r) {
  mp_ruler = r;
  redraw();
}
...
void View::render(const lay::Viewport &vp, lay::ViewObjectCanvas &canvas) {
  ...
  if (mp_rulers->with_halo()) {
    ...
    ops.reserve(2);
    //  we use 2 and 3 for the bitmap index. Since selection markers are using 0
    //  and 1, rulers that are dragged appear in front of them.
    ops.push_back(lay::ViewOp(canvas.background_color().rgb(),
                              lay::ViewOp::Copy, 0, 0, 0, lay::ViewOp::Rect,
                              3 * basic_width, 2));
    ops.push_back(lay::ViewOp(c.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                              lay::ViewOp::Rect, basic_width, 3));
    plane = canvas.plane(ops);
  } else {
    plane = canvas.plane(lay::ViewOp(c.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                                     lay::ViewOp::Rect, basic_width));
  }

  draw_ruler(*mp_ruler, vp.trans() * m_trans, m_selected, plane,
             canvas.renderer());
}
...
Service::Service(db::Manager *manager, lay::LayoutViewBase *view)
    : lay::EditorServiceBase(view), lay::Drawing(1 /*number of planes*/,
                                                 view->drawings()),
      db::Object(manager), m_halo(true), m_snap_mode(lay::AC_Any),
      m_grid(0.001), m_grid_snap(false), m_obj_snap(false), m_snap_range(1),
      m_max_number_of_rulers(-1 /*unlimited*/), mp_view(view),
      mp_active_ruler(0), mp_transient_ruler(0), m_drawing(false), m_current(),
      m_move_mode(MoveNone), m_seg_index(0), m_current_template(0) {
  mp_view->annotations_changed_event.add(this, &Service::annotations_changed);
}

Service::~Service() {
  for (std::vector<ant::View *>::iterator r = m_rulers.begin();
       r != m_rulers.end(); ++r) {
    delete *r;
  }
  m_rulers.clear();
...
const ant::Template &Service::current_template() const {
...
void Service::annotations_changed() {
  //  NOTE: right now, we don't differentiate: every annotation change may be a
  //  change in an image too. We just forward this event as a potential image
  //  changed event
  annotations_changed_event();
}

std::vector<lay::ViewOp> Service::get_view_ops(lay::RedrawThreadCanvas &canvas,
                                               tl::Color background,
                                               tl::Color foreground,
                                               tl::Color /*active*/) const {
  int basic_width = int(0.5 + 1.0 / canvas.resolution());
  ...
  if (m_halo) {
    view_ops.push_back(lay::ViewOp(background.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                                   lay::ViewOp::Rect, 3 * basic_width, 0));
  }
  if (m_color.is_valid()) {
    view_ops.push_back(lay::ViewOp(m_color.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                                   lay::ViewOp::Rect, basic_width, 0));
  } else {
    view_ops.push_back(lay::ViewOp(foreground.rgb(), lay::ViewOp::Copy, 0, 0, 0,
                                   lay::ViewOp::Rect, basic_width, 0));
  }
...
int Service::insert_ruler(const ant::Object &ruler, bool limit_number) {
  //  determine the last id
  int idmax = -1;
  for (lay::AnnotationShapes::iterator r = mp_view->annotation_shapes().begin();
       r != mp_view->annotation_shapes().end(); ++r) {
...
static bool dragging_what_seg(const ant::Object *robj,
                              const db::DBox &search_dbox,
                              ant::Service::MoveMode &mode, db::DPoint &p1,
                              size_t index) {
  ...
  //  HINT: this was implemented returning a std::pair<MoveMode, db::DPoint>,
  //  but I was not able to get it to work in gcc 4.1.2 in -O3 mode ...
...
/**
 *  @brief Helper function to determine which move mode to choose given a
 * certain search box and ant::Object
 */
static bool dragging_what(const ant::Object *robj, const db::DBox &search_dbox,
                          ant::Service::MoveMode &mode, db::DPoint &p1,
                          size_t &index) {
  ant::Object::outline_type outline = robj->outline();
...
bool Service::begin_move(lay::Editable::MoveMode mode, const db::DPoint &p,
                         lay::angle_constraint_type /*ac*/) {
  //  cancel any pending move or drag operations, reset mp_active_ruler
  ui()->drag_cancel(); // KLUDGE: every service does this to the same service
                       // manager
  ...
  if (mode == lay::Editable::Selected) {
    ...
    for (std::map<obj_iterator, unsigned int>::const_iterator r =
             m_selected.begin();
         r != m_selected.end(); ++r) {
      const ant::Object *robj =
          dynamic_cast<const ant::Object *>((*r->first).ptr());
      if (robj) {
        double d;
    ...
    lay::AnnotationShapes::touching_iterator r =
        mp_view->annotation_shapes().begin_touching(search_dbox);
    const ant::Object *robj_min = 0;
    while (!r.at_end()) {
      const ant::Object *robj = dynamic_cast<const ant::Object *>((*r).ptr());
      if (robj) {
    ...
    while (m_move_mode == MoveNone && !r.at_end()) {
      ...
      if (robj && (!robj_min || robj == robj_min)) {
        ...
        if (dragging_what(robj, search_dbox, m_move_mode, m_p1, m_seg_index)) {
          ...
          //  found anything: make the moved ruler the selection
          clear_selection();
          m_selected.insert(std::make_pair(
              mp_view->annotation_shapes().iterator_from_pointer(&*r), 0));
          m_current = *robj;
          m_original = m_current;
          m_rulers.push_back(new ant::View(this, &m_current, true));
          m_rulers.back()->thaw();
...
void Service::move_transform(const db::DPoint &p, db::DFTrans tr,
                             lay::angle_constraint_type /*ac*/) {
  ...
  if (m_move_mode == MoveRuler) {
    ...
    m_original.transform(db::DTrans(m_p1 - db::DPoint()) * db::DTrans(tr) *
                         db::DTrans(db::DPoint() - m_p1));
    ...
    m_trans *= db::DTrans(m_p1 - db::DPoint()) * db::DTrans(tr) *
               db::DTrans(db::DPoint() - m_p1);
...
void Service::move(const db::DPoint &p, lay::angle_constraint_type ac) {
  ...
  if (m_move_mode == MoveP1) {
    ...
    m_current.seg_p1(m_seg_index,
                     db::DPoint(m_current.seg_p1(m_seg_index).x(), p12.y()));
    m_current.seg_p2(m_seg_index,
                     db::DPoint(p12.x(), m_current.seg_p2(m_seg_index).y()));
    m_rulers[0]->redraw();
    ...
    m_current.seg_p1(m_seg_index,
                     db::DPoint(p21.x(), m_current.seg_p1(m_seg_index).y()));
    m_current.seg_p2(m_seg_index,
                     db::DPoint(m_current.seg_p2(m_seg_index).x(), p21.y()));
    m_rulers[0]->redraw();
    ...
    m_current.seg_p1(m_seg_index,
                     db::DPoint(pc.x(), m_current.seg_p1(m_seg_index).y()));
    m_rulers[0]->redraw();
    ...
    db::DPoint pc = snap2(m_p1, p, &m_current, ac).second;
    m_current.seg_p2(m_seg_index,
                     db::DPoint(pc.x(), m_current.seg_p2(m_seg_index).y()));
    m_rulers[0]->redraw();
    ...
    db::DPoint pc = snap2(m_p1, p, &m_current, ac).second;
    m_current.seg_p1(m_seg_index,
                     db::DPoint(m_current.seg_p1(m_seg_index).x(), pc.y()));
    m_rulers[0]->redraw();
    ...
    db::DPoint pc = snap2(m_p1, p, &m_current, ac).second;
    m_current.seg_p2(m_seg_index,
                     db::DPoint(m_current.seg_p2(m_seg_index).x(), pc.y()));
    m_rulers[0]->redraw();
    ...
    db::DVector dp =
        lay::snap_angle(p - m_p1, ac == lay::AC_Global ? m_snap_mode : ac);
    ...
    if ((!r2.first && r1.first) || ((r1.first || (!r1.first && !r2.first)) &&
                                    q1.distance(p1) < q2.distance(p2))) {
    ...
    //  round the drag distance to grid if required: this is the least we can do
    //  in this case
    if (m_grid_snap) {
    ...
    m_trans = db::DTrans(dp + (m_p1 - db::DPoint()) - m_trans.disp()) *
              m_trans * db::DTrans(db::DPoint() - m_p1);
...
void Service::show_message() {
  ...
  std::string pos =
      std::string("lx: ") +
      tl::micron_to_string(m_current.p2().x() - m_current.p1().x()) +
...
void Service::end_move(const db::DPoint &, lay::angle_constraint_type) {
  if (!m_rulers.empty() && !m_selected.empty()) {
    ...
    if (m_move_mode == MoveSelected) {
      ...
      for (std::map<obj_iterator, unsigned int>::const_iterator s =
               m_selected.begin();
           s != m_selected.end(); ++s) {

        const ant::Object *robj =
            dynamic_cast<const ant::Object *>(s->first->ptr());
        if (robj) {
      ...
      mp_view->annotation_shapes().replace(
          m_selected.begin()->first,
          db::DUserObject(new ant::Object(m_current)));
...
void Service::selection_to_view() {
  ...
  //  the selection objects need to be recreated since we destroyed the old
  //  rulers
  for (std::vector<ant::View *>::iterator r = m_rulers.begin();
       r != m_rulers.end(); ++r) {
    delete *r;
  ...
  for (std::map<obj_iterator, unsigned int>::iterator r = m_selected.begin();
       r != m_selected.end(); ++r) {
...
db::DBox Service::selection_bbox() {
  db::DBox box;
  for (std::map<obj_iterator, unsigned int>::iterator r = m_selected.begin();
       r != m_selected.end(); ++r) {
    const ant::Object *robj =
...
bool Service::mouse_press_event(const db::DPoint &p, unsigned int buttons,
                                bool prio) {
  return mouse_click_event(p, buttons, prio);
}

void Service::finish_drawing() {
...
bool Service::mouse_double_click_event(const db::DPoint & /*p*/,
...
bool Service::mouse_click_event(const db::DPoint &p, unsigned int buttons,
                                bool prio) {
  if (prio && (buttons & lay::LeftButton) != 0) {
    ...
    if (!m_drawing) {
      ...
      if (tpl.mode() == ant::Template::RulerSingleClick) {
        ...
        //  for auto-metric we need some cutline constraint - any or global
        //  won't do.
        ...
        double snap_range =
            ui()->mouse_event_trans().inverted().ctrans(m_snap_range);
        snap_range *= 0.5;

        lay::TwoPointSnapToObjectResult ee =
            lay::obj_snap2(mp_view, p, g, ac, snap_range, snap_range * 1000.0);
        if (ee.any) {
        ...
        mp_active_ruler =
            new ant::View(this, &m_current, false /*not selected*/);
    ...
    } else if (tpl.mode() == ant::Template::RulerMultiSegment ||
               tpl.mode() == ant::Template::RulerThreeClicks) {
...
ant::Object Service::create_measure_ruler(const db::DPoint &pt,
                                          lay::angle_constraint_type ac) {
  ...
  lay::TwoPointSnapToObjectResult ee = lay::obj_snap2(
      mp_view, pt, db::DVector(), ac, snap_range, snap_range * 1000.0);
  if (ee.any) {
...
bool Service::mouse_move_event(const db::DPoint &p, unsigned int buttons,
                               bool prio) {
  if (prio) {

    lay::PointSnapToObjectResult snap_details;
    if (m_drawing) {
      snap_details = snap2_details(m_p1, p, mp_active_ruler->ruler(),
                                   ac_from_buttons(buttons));
    } else {
      const ant::Template &tpl = current_template();
  ...
  if (m_drawing && prio) {
    ...
    //  NOTE: we use the direct access path so we do not encounter cleanup by
    //  the p1 and p2 setters otherwise we risk manipulating p1 too.
    ant::Object::point_list pts = m_current.points();
    if (!pts.empty()) {
      pts.back() =
          snap2(m_p1, p, mp_active_ruler->ruler(), ac_from_buttons(buttons))
              .second;
    }
    m_current.set_points_exact(pts);
...
lay::PointSnapToObjectResult Service::snap1_details(const db::DPoint &p,
                                                    bool obj_snap) {
  db::DVector g;
  if (m_grid_snap) {
    g = db::DVector(m_grid, m_grid);
...
std::pair<bool, db::DPoint> Service::snap1(const db::DPoint &p, bool obj_snap) {
  lay::PointSnapToObjectResult res = snap1_details(p, obj_snap);
  return std::make_pair(res.object_snap !=
                            lay::PointSnapToObjectResult::NoObject,
                        res.snapped_point);
}

lay::PointSnapToObjectResult
Service::snap2_details(const db::DPoint &p1, const db::DPoint &p2,
                       const ant::Object *obj, lay::angle_constraint_type ac) {
  db::DVector g;
  if (m_grid_snap) {
    g = db::DVector(m_grid, m_grid);
  }
  ...
  double snap_range = ui()->mouse_event_trans().inverted().ctrans(m_snap_range);
  lay::angle_constraint_type snap_mode =
      ac == lay::AC_Global ? (obj->angle_constraint() == lay::AC_Global
                                  ? m_snap_mode
                                  : obj->angle_constraint())
                           : ac;

  return lay::obj_snap(m_obj_snap && obj->snap() ? mp_view : 0, p1, p2, g,
                       snap_mode, snap_range);
}

std::pair<bool, db::DPoint> Service::snap2(const db::DPoint &p1,
                                           const db::DPoint &p2,
                                           const ant::Object *obj,
                                           lay::angle_constraint_type ac) {
...
struct RulerIdComp {
  bool operator()(const lay::AnnotationShapes::iterator &a,
                  const lay::AnnotationShapes::iterator &b) const {
    return dynamic_cast<const ant::Object &>(*a->ptr()).id() <
           dynamic_cast<const ant::Object &>(*b->ptr()).id();
  }
};

void Service::reduce_rulers(int num) {
  clear_transient_selection();
  ...
  if (num >= 0 && int(n) > num) {
    ...
    //  remaining positions
    tl::sort(positions.begin(), positions.end(),
             RulerIdComp()); // HINT: must be tl::sort, not std::sort because
                             // gcc 3.2.3 has some strange namespace resolution
                             // problems ..
    positions.erase(positions.begin() + (positions.size() - num),
                    positions.end());
    tl::sort(
        positions.begin(),
        positions
            .end()); // HINT: must be tl::sort, not std::sort because gcc 3.2.3
                     // has some strange namespace resolution problems ..

    //  now we can erase these positions
    mp_view->annotation_shapes().erase_positions(positions.begin(),
                                                 positions.end());
  }
}
...
void Service::paste() {
  if (db::Clipboard::instance().begin() != db::Clipboard::instance().end()) {
    ...
    //  determine the last id
    int idmax = -1;
    for (lay::AnnotationShapes::iterator r =
             mp_view->annotation_shapes().begin();
         r != mp_view->annotation_shapes().end(); ++r) {
      const ant::Object *robj = dynamic_cast<const ant::Object *>(r->ptr());
    ...
    for (db::Clipboard::iterator c = db::Clipboard::instance().begin();
      ...
      const db::ClipboardValue<ant::Object> *value =
          dynamic_cast<const db::ClipboardValue<ant::Object> *>(*c);
      if (value) {
...
bool Service::has_selection() { return !m_selected.empty(); }

size_t Service::selection_size() { return m_selected.size(); }

bool Service::has_transient_selection() { return mp_transient_ruler != 0; }
...
void Service::clear_selection() { select(db::DBox(), lay::Editable::Reset); }

double Service::click_proximity(const db::DPoint &pos,
  ...
  while (!r.at_end()) {
    ...
    if (robj &&
        (!exclude ||
         exclude->find(mp_view->annotation_shapes().iterator_from_pointer(
             &*r)) == exclude->end())) {
      double d;
...
bool Service::transient_select(const db::DPoint &pos) {
  ...
  //  if in move mode (which also receives transient_select requests) the move
  //  will take the selection, hence don't do a transient selection if there is
  //  one.
  ...
  while (!r.at_end()) {
    const ant::Object *robj = dynamic_cast<const ant::Object *>((*r).ptr());
    if (robj && m_previous_selection.find(
                    mp_view->annotation_shapes().iterator_from_pointer(&*r)) ==
                    m_previous_selection.end()) {
      double d;
      if (is_selected(*robj, pos, l, d)) {
...
void Service::clear_previous_selection() { m_previous_selection.clear(); }
...
bool Service::select(const db::DBox &box, lay::Editable::SelectionMode mode) {
  ...
  if (box.empty()) {
    ...
    if (mode == lay::Editable::Reset) {
      ...
      lay::AnnotationShapes::iterator rfrom =
          mp_view->annotation_shapes().begin();
      lay::AnnotationShapes::iterator rto = mp_view->annotation_shapes().end();
    ...
    if (!box.is_point()) {
      ...
      while (!r.at_end()) {
        ...
        if (robj &&
          ...
          if (is_selected(*robj, box, l)) {
            any_selected = true;
            if (select(mp_view->annotation_shapes().iterator_from_pointer(&*r),
                       mode)) {
              needs_update = true;
            }
          }
      ...
      if (any_selected) {
        select(mp_view->annotation_shapes().iterator_from_pointer(&*rmin),
               mode);
        m_previous_selection.insert(std::make_pair(
            mp_view->annotation_shapes().iterator_from_pointer(&*rmin), mode));
        needs_update = true;
      }
    }
  }
...
void Service::display_status(bool transient) {
  View *selected_view =
      transient ? mp_transient_ruler : (m_rulers.size() == 1 ? m_rulers[0] : 0);
  if (!selected_view) {
    ...
    if (ruler->segments() > 1) {
      msg += tl::sprintf(tl::to_string(tr("annotation(d=%s x=%s y=%s ...)")),
                         ruler->text(0), ruler->text_x(0), ruler->text_y(0));
    } else {
      msg += tl::sprintf(tl::to_string(tr("annotation(d=%s x=%s y=%s)")),
                         ruler->text(0), ruler->text_x(0), ruler->text_y(0));
    }
...
void Service::delete_ruler(obj_iterator pos) {
  //  delete the object
...
void Service::change_ruler(obj_iterator pos, const ant::Object &to) {
  //  replace the object, keep the ID:
  ant::Object *new_ruler = new ant::Object(to);
  const ant::Object *current_ruler =
      dynamic_cast<const ant::Object *>(pos->ptr());
...
void Service::paint_on_planes(const db::DCplxTrans &trans,
  ...
  db::DBox vp =
      db::DBox(trans.inverted() *
               db::DBox(db::DPoint(0.0, 0.0),
                        db::DPoint(renderer.width(), renderer.height())));

  lay::AnnotationShapes::touching_iterator user_object =
      mp_view->annotation_shapes().begin_touching(vp);
  while (!user_object.at_end()) {
    const ant::Object *ruler =
        dynamic_cast<const ant::Object *>((*user_object).ptr());
    if (ruler) {
      draw_ruler(*ruler, trans, false /*not selected*/, planes.front(),
                 renderer);
    }
    ++user_object;
  }
...
AnnotationIterator Service::begin_annotations() const {
  return AnnotationIterator(mp_view->annotation_shapes().begin(),
                            mp_view->annotation_shapes().end());
}
