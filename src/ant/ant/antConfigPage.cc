...
ConfigPage::ConfigPage(QWidget *parent) : lay::ConfigPage(parent) {
...
void ConfigPage::setup(lay::Dispatcher *root) {
  ...
  mp_ui->ruler_snap_range_edit->setText(
      tl::to_qstring(tl::to_string(snap_range)));
...
void ConfigPage::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_ruler_grid_snap,
                   mp_ui->ruler_grid_snap_cbx->isChecked());

  int sr = 0;
  tl::from_string_ext(tl::to_string(mp_ui->ruler_snap_range_edit->text()), sr);
  if (sr < 1 || sr > 1000) {
    throw tl::Exception(
        tl::to_string(QObject::tr("Not a valid pixel value (must be non-zero "
                                  "positive and not too large): %s")),
        tl::to_string(mp_ui->ruler_snap_range_edit->text()));
  }
...
ConfigPage2::ConfigPage2(QWidget *parent) : lay::ConfigPage(parent) {
...
void ConfigPage2::setup(lay::Dispatcher *root) {
  ...
  if (max_number_of_rulers < 0) {
    ...
    mp_ui->num_rulers_edit->setText(
        tl::to_qstring(tl::to_string(max_number_of_rulers)));
  }
...
void ConfigPage2::commit(lay::Dispatcher *root) {
  ...
  root->config_set(cfg_ruler_color, mp_ui->ruler_color_pb->get_color(),
                   lay::ColorConverter());
...
ConfigPage3::ConfigPage3(QWidget *parent) : lay::ConfigPage(parent) {
...
ConfigPage4::ConfigPage4(QWidget *parent)
    : lay::ConfigPage(parent), m_current_template(0),
  ...
  connect(mp_ui->template_list, SIGNAL(currentRowChanged(int)), this,
          SLOT(current_template_changed(int)));
  connect(mp_ui->template_list, SIGNAL(itemDoubleClicked(QListWidgetItem *)),
...
void ConfigPage4::setup(lay::Dispatcher *root) {
  //  templates
  root->config_get(cfg_ruler_templates, m_ruler_templates,
                   TemplatesConverter());
...
void ConfigPage4::commit(lay::Dispatcher *root) {
  ...
  //  templates
  root->config_set(cfg_ruler_templates, m_ruler_templates,
                   TemplatesConverter());
...
void ConfigPage4::add_clicked() {
  ...
  m_ruler_templates.insert(m_ruler_templates.begin() + m_current_template,
                           new_one);
  m_ruler_templates[m_current_template].title(
      tl::to_string(QObject::tr("New Ruler")));
  update_list();
  show();
  ...
  if (m_current_template >= 0 &&
    ...
    if (!m_ruler_templates[m_current_template].category().empty()) {
      throw tl::Exception(tl::to_string(
          tr("This ruler is a built-in template and cannot be deleted")));
    }
...
void ConfigPage4::up_clicked() {
  if (m_current_template > 0) {
    commit();
    std::swap(m_ruler_templates[m_current_template],
              m_ruler_templates[m_current_template - 1]);
    --m_current_template;
    update_list();
    show();
  }
...
void ConfigPage4::down_clicked() {
  if (m_current_template >= 0 &&
      m_current_template < int(m_ruler_templates.size() - 1)) {
    commit();
    std::swap(m_ruler_templates[m_current_template],
              m_ruler_templates[m_current_template + 1]);
    ++m_current_template;
...
void ConfigPage4::update_list() {
  ...
  for (std::vector<ant::Template>::const_iterator t = m_ruler_templates.begin();
       t != m_ruler_templates.end(); ++t) {
    mp_ui->template_list->addItem(tl::to_qstring(t->title()));
    if (!t->category().empty()) {
      QListWidgetItem *item =
          mp_ui->template_list->item(int(t - m_ruler_templates.begin()));
...
void ConfigPage4::double_clicked(QListWidgetItem *) {
  if (m_current_template >= 0 &&
    ...
    commit();
    bool ok = false;
    QString new_title = QInputDialog::getText(
        this, QObject::tr("Enter New Title"), QObject::tr("New Title"),
        QLineEdit::Normal,
        tl::to_qstring(m_ruler_templates[m_current_template].title()), &ok);
    if (ok) {
...
void ConfigPage4::show() {
  mp_ui->fmt_le->setText(
      tl::to_qstring(m_ruler_templates[m_current_template].fmt()));
  mp_ui->fmt_x_le->setText(
      tl::to_qstring(m_ruler_templates[m_current_template].fmt_x()));
  mp_ui->fmt_y_le->setText(
      tl::to_qstring(m_ruler_templates[m_current_template].fmt_y()));

  mp_ui->style_cb->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].style());
  mp_ui->outline_cb->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].outline());
  mp_ui->t_angle_cb->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].angle_constraint());
  mp_ui->t_snap_cbx->setChecked(m_ruler_templates[m_current_template].snap());
  mp_ui->t_mode_cb->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].mode());

  mp_ui->main_position->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].main_position());
  mp_ui->main_xalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].main_xalign());
  mp_ui->main_yalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].main_yalign());
  mp_ui->xlabel_xalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].xlabel_xalign());
  mp_ui->xlabel_yalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].xlabel_yalign());
  mp_ui->ylabel_xalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].ylabel_xalign());
  mp_ui->ylabel_yalign->setCurrentIndex(
      (unsigned int)m_ruler_templates[m_current_template].ylabel_yalign());
}
...
void ConfigPage4::commit() {
  ...
  ant::Object::style_type style =
      ant::Object::style_type(mp_ui->style_cb->currentIndex());
  m_ruler_templates[m_current_template].style(style);

  ant::Object::outline_type outline =
      ant::Object::outline_type(mp_ui->outline_cb->currentIndex());
  m_ruler_templates[m_current_template].outline(outline);

  lay::angle_constraint_type ac =
      lay::angle_constraint_type(mp_ui->t_angle_cb->currentIndex());
  m_ruler_templates[m_current_template].angle_constraint(ac);

  ant::Template::ruler_mode_type mode =
      ant::Template::ruler_mode_type(mp_ui->t_mode_cb->currentIndex());
  m_ruler_templates[m_current_template].set_mode(mode);

  m_ruler_templates[m_current_template].snap(mp_ui->t_snap_cbx->isChecked());

  m_ruler_templates[m_current_template].set_main_position(
      Object::position_type(mp_ui->main_position->currentIndex()));
  m_ruler_templates[m_current_template].set_main_xalign(
      Object::alignment_type(mp_ui->main_xalign->currentIndex()));
  m_ruler_templates[m_current_template].set_main_yalign(
      Object::alignment_type(mp_ui->main_yalign->currentIndex()));
  m_ruler_templates[m_current_template].set_xlabel_xalign(
      Object::alignment_type(mp_ui->xlabel_xalign->currentIndex()));
  m_ruler_templates[m_current_template].set_xlabel_yalign(
      Object::alignment_type(mp_ui->xlabel_yalign->currentIndex()));
  m_ruler_templates[m_current_template].set_ylabel_xalign(
      Object::alignment_type(mp_ui->ylabel_xalign->currentIndex()));
  m_ruler_templates[m_current_template].set_ylabel_yalign(
      Object::alignment_type(mp_ui->ylabel_yalign->currentIndex()));
}
