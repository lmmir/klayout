...
class ANT_PUBLIC View : public lay::ViewObject {
public:
  /**
   ...
   */
  const ant::Object *ruler() const { return mp_ruler; }
...
public:
  ...
  AnnotationIterator(lay::AnnotationShapes::iterator begin,
                     lay::AnnotationShapes::iterator end)
      : m_current(begin), m_end(end) {
    next_valid();
  }

  AnnotationIterator() : m_current(), m_end() {
    //  .. nothing yet ..
  }
  ...
  bool at_end() const { return m_current == m_end; }

  lay::AnnotationShapes::iterator current() const { return m_current; }

private:
  void next_valid() {
    while (m_current != m_end &&
           dynamic_cast<const ant::Object *>(m_current->ptr()) == 0) {
      ++m_current;
    }
  }
...
class ANT_PUBLIC Service : public lay::EditorServiceBase,
                           public lay::Drawing,
                           public db::Object {
public:
  ...
  /**
   *  @brief Return the bbox of the selection (reimplementation of lay::Editable
   * interface)
   */
  virtual db::DBox selection_bbox();

  /**
   *  @brief Transform the selection (reimplementation of lay::Editable
   * interface)
   */
   ...
   */
  bool with_halo() const { return m_halo; }
   ...
   */
  lay::Drawing *drawing_interface() { return this; }
   ...
   */
  int snap_range() const { return m_snap_range; }

  /**
   *  @brief Gets the global snap mode
   */
  lay::angle_constraint_type snap_mode() const { return m_snap_mode; }

  /**
   *  @brief Gets the grid
   */
  double grid() const { return m_grid; }

  /**
   *  @brief Gets a value indicating whether to snap to grid
   */
  bool grid_snap() const { return m_grid_snap; }
  ...
  /**
   *  @brief Return the annotation iterator that delivers the annotations (and
   * only these)
   */
  AnnotationIterator begin_annotations() const;

  /**
   *  @brief Creates an auto-measure ruler at the given point with the given
   * angle constraint
   */
  ant::Object create_measure_ruler(const db::DPoint &pt,
                                   lay::angle_constraint_type ac);
...
private:
  ...
  lay::PointSnapToObjectResult snap1_details(const db::DPoint &p,
                                             bool obj_snap);
  std::pair<bool, db::DPoint> snap2(const db::DPoint &p1, const db::DPoint &p2,
                                    const ant::Object *obj,
                                    lay::angle_constraint_type ac);
  lay::PointSnapToObjectResult snap2_details(const db::DPoint &p1,
                                             const db::DPoint &p2,
                                             const ant::Object *obj,
                                             lay::angle_constraint_type ac);
   ...
   */
  std::vector<lay::ViewOp> get_view_ops(lay::RedrawThreadCanvas &canvas,
                                        tl::Color background,
                                        tl::Color foreground,
