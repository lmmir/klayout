...
TEST(basic) {
  ...
  bool err = false;
  try {
    pya::PythonInterpreter::instance()->eval_string(
  ...
  try {
    ...
    EXPECT_EQ(ex.basic_msg(),
              std::string("NameError: name 'Quatsch' is not defined"));
    EXPECT_EQ(ex.cls() == std::string("exceptions.NameError") ||
                  ex.cls() == std::string("NameError"),
              true);
    err = true;
  }
