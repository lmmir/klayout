...
PythonError::PythonError(const char *msg, const char *cls,
                         const std::vector<tl::BacktraceElement> &backtrace)
    : tl::ScriptError(msg, cls, backtrace) {}

PythonError::PythonError(const char *msg, const char *sourcefile, int line,
                         const char *cls,
                         const std::vector<tl::BacktraceElement> &backtrace)
    : tl::ScriptError(msg, sourcefile, line, cls, backtrace) {}

PythonError::PythonError(const PythonError &d) : tl::ScriptError(d) {}
...
class PythonStackTraceProvider : public gsi::StackTraceProvider {
public:
  PythonStackTraceProvider(PyFrameObject *frame, const std::string &scope)
    ...
    while (frame != NULL) {
      ...
      if (test_type<std::string>(PyFrame_GetCode(frame)->co_filename, true)) {
        fn = normalize_path(
            python2c<std::string>(PyFrame_GetCode(frame)->co_filename));
#else
  ...
  virtual size_t scope_index() const {
    ...
    //  $KLAYOUT_PYA_DEBUG_SCOPE is set.
    if (consider_scope < 0) {
  ...
  virtual int stack_depth() const { return int(m_stack_trace.size()); }
...
static PyObject *init_pya_module() {
...
PythonInterpreter::PythonInterpreter(bool embedded)
    : gsi::Interpreter(0, "pya"), mp_current_console(0),
      mp_current_exec_handler(0), m_current_exec_level(0), m_in_trace(false),
      m_block_exceptions(false), m_ignore_next_exception(false),
  ...
  Py_SetPythonHome(
      (wchar_t *)L""); //  really ignore $PYTHONHOME + without this, we get
                       //  dummy error message about lacking path for libraries
    ...
    try {
      ...
      if ((len = GetModuleFileNameW(NULL, buffer, MAX_PATH)) > 0) {
        ...
        if (tl::file_exists(path_file)) {
          ...
          tl::log << tl::to_string(tr("Reading Python path from "))
                  << path_file;
      ...
      tl::error << tl::to_string(
                       tr("Evaluation of Python path expression failed"))
                << ": " << ex.msg();
    } catch (...) {
      tl::error << tl::to_string(
          tr("Evaluation of Python path expression failed"));
  ...
  m_stdout_channel =
      PythonRef(PYAChannelObject::create(gsi::Console::OS_stdout));
  m_stdout = PythonPtr(m_stdout_channel.get());
  m_stderr_channel =
      PythonRef(PYAChannelObject::create(gsi::Console::OS_stderr));
...
char *PythonInterpreter::make_string(const std::string &s) {
  m_string_heap.push_back(s);
  return const_cast<char *>(m_string_heap.back().c_str());
}

void PythonInterpreter::add_path(const std::string &p) {
...
void PythonInterpreter::add_package_location(const std::string &package_path) {
  std::string path =
      tl::combine_path(tl::absolute_file_path(package_path), "python");
  if (tl::file_exists(path) &&
      m_package_paths.find(path) == m_package_paths.end()) {
    m_package_paths.insert(path);
    add_path(path);
  }
}

void PythonInterpreter::remove_package_location(
    const std::string & /*package_path*/) {
  //  Currently, we do not really remove the location. Python might get screwed
  //  up this way.
}
...
void PythonInterpreter::require(const std::string & /*filename*/) {
  ...
  throw tl::Exception(
      tl::to_string(tr("'require' not implemented for Python interpreter")));
}
...
void PythonInterpreter::remove_debugger_scope() { m_debugger_scope.clear(); }
 ...
 */
void PythonInterpreter::get_context(int context, PythonRef &globals,
                                    PythonRef &locals, const char *file) {
  globals = PythonRef();
  locals = PythonRef();
  ...
  if (f) {
    ...
    locals =
        PythonRef(PyObject_GetAttrString((PyObject *)f, "f_locals"), false);
#else
    ...
    //  TODO: create a private namespace here? Or let the macros litter the
    //  global namespace?
...
void PythonInterpreter::eval_string(const char *expr, const char *file,
...
#if PY_MAJOR_VERSION < 3
  PythonRef result(
      PyEval_EvalCode((PyCodeObject *)code.get(), globals.get(), locals.get()));
#else
 ...
 *  The expression is given int "string". If "eval_expr" is true, the string is
 * evaluated as expression and the result is returned in the variant. If
 * "eval_expr" is false, the string is evaluated, the result is printed to the
 * currently active console and a nil variant is returned.
 */
tl::Variant PythonInterpreter::eval_int(const char *expr, const char *file,
                                        int /*line*/, bool eval_expr,
                                        int context) {
  tl::Variant ret;
  ...
  //  TODO: what to do with "line"?
  PythonRef code(Py_CompileString(expr, file ? file : "(eval)",
                                  eval_expr ? Py_eval_input : Py_single_input));
  if (!code) {
    check_error();
    return ret;
  }
...
void PythonInterpreter::eval_string_and_print(const char *expr,
                                              const char *file, int line,
                                              int context) {
  eval_int(expr, file, line, false, context);
}

tl::Variant PythonInterpreter::eval_expr(const char *expr, const char *file,
                                         int line, int context) {
  return eval_int(expr, file, line, true, context);
}

gsi::Inspector *PythonInterpreter::inspector(int context) {
  PythonRef globals, locals;
  get_context(context, globals, locals, 0);
  return create_inspector(locals.get(), true /*symbolic*/);
}

void PythonInterpreter::define_variable(const std::string &name,
...
bool PythonInterpreter::available() const { return true; }

void PythonInterpreter::initialize() {
  // .. no implementation required ..
}

size_t PythonInterpreter::prepare_trace(PyObject *fn_object) {
  std::map<PyObject *, size_t>::const_iterator f =
      m_file_id_map.find(fn_object);
  if (f == m_file_id_map.end()) {
    f = m_file_id_map
            .insert(std::make_pair(
                fn_object,
                mp_current_exec_handler->id_for_path(
                    this, normalize_path(python2c<std::string>(fn_object)))))
            .first;
  }
...
//  TODO: make the Python object the interpreter and don't use singleton
//  instances (multi-threading support)
static int pya_trace_func(PyObject * /*obj*/, PyFrameObject *frame, int event,
                          PyObject *arg) {
...
int PythonInterpreter::trace_func(PyFrameObject *frame, int event,
                                  PyObject *arg) {
  ...
  if (event == PyTrace_LINE) {
    ...
    if (exc_type && exc_type.get() != PyExc_StopIteration) {
      ...
      if (m_ignore_next_exception) {
        ...
        PythonStackTraceProvider st_provider(frame, m_debugger_scope);
        mp_current_exec_handler->exception_thrown(this, file_id, line, eclass,
                                                  emsg, &st_provider);
      }
...
void PythonInterpreter::remove_exec_handler(
    gsi::ExecutionHandler *exec_handler) {
  if (mp_current_exec_handler == exec_handler) {
    ...
    for (std::vector<gsi::ExecutionHandler *>::iterator eh =
             m_exec_handlers.begin();
         eh != m_exec_handlers.end(); ++eh) {
      if (*eh == exec_handler) {
...
void PythonInterpreter::remove_console(gsi::Console *console) {
  if (mp_current_console == console) {
    ...
    for (std::vector<gsi::Console *>::iterator c = m_consoles.begin();
         c != m_consoles.end(); ++c) {
      if (*c == console) {
        m_consoles.erase(c);
        break;
...
void PythonInterpreter::end_execution() {
  if (m_current_exec_level > 0 && --m_current_exec_level == 0 &&
      mp_current_exec_handler) {
    mp_current_exec_handler->end_exec(this);
  }
}

std::string PythonInterpreter::python_doc(const gsi::MethodBase *m) {
  return pya::PythonModule::python_doc(m);
}

PythonInterpreter *PythonInterpreter::instance() { return sp_interpreter; }
