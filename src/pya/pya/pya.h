...
/**
 *  Two helper macros that frame a piece of code which potentially executes
 * Python code
 */
 ...
 */
class PYA_PUBLIC PythonError : public tl::ScriptError {
public:
  PythonError(const char *msg, const char *cls,
              const std::vector<tl::BacktraceElement> &backtrace);
  PythonError(const char *msg, const char *sourcefile, int line,
              const char *cls,
              const std::vector<tl::BacktraceElement> &backtrace);
 ...
 */
class PYA_PUBLIC PythonInterpreter : public gsi::Interpreter {
public:
  /**
   *  @brief The constructor
   *
   *  If embedded is true, the interpreter is an embedded one. Only in this
   * case, the Python interpreter is initialized. Otherwise, it is assumed the
   * interpreter already exists and our application runs inside an external
   * interpreter.
   */
...
private:
  ...
  tl::Variant eval_int(const char *string, const char *filename, int line,
                       bool eval_expr, int context);
  void get_context(int context, PythonRef &globals, PythonRef &locals,
                   const char *file);
