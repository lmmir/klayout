...
MethodTableEntry::MethodTableEntry(const std::string &name, bool st, bool prot)
    : m_name(name), m_is_static(st), m_is_protected(prot), m_is_enabled(true),
      m_is_init(false), m_fallback_not_implemented(false) {}

const std::string &MethodTableEntry::name() const { return m_name; }

void MethodTableEntry::set_name(const std::string &n) { m_name = n; }

void MethodTableEntry::set_enabled(bool en) { m_is_enabled = en; }

bool MethodTableEntry::is_enabled() const { return m_is_enabled; }
...
void MethodTableEntry::set_init(bool f) { m_is_init = f; }

bool MethodTableEntry::is_init() const { return m_is_init; }

bool MethodTableEntry::is_static() const { return m_is_static; }

bool MethodTableEntry::is_protected() const { return m_is_protected; }

void MethodTableEntry::add(const gsi::MethodBase *m) { m_methods.push_back(m); }
...
MethodTableEntry::method_iterator MethodTableEntry::begin() const {
  return m_methods.begin();
}

MethodTableEntry::method_iterator MethodTableEntry::end() const {
...
MethodTable::MethodTable(const gsi::ClassBase *cls_decl, PythonModule *module)
    ...
    if (!(*m)->is_callback() && !(*m)->is_signal()) {
      ...
      for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
        ...
        if (!syn->is_getter && !syn->is_setter) {
          if (no_args && is_property_setter(st, syn->name) &&
              !is_property_getter(st, syn->name)) {
            add_getter(syn->name, *m);
          } else if (st && no_args &&
                     (isupper(syn->name[0]) || (*m)->is_const())) {
            //  static methods without arguments which start with a capital
            //  letter are treated as constants
            add_getter(syn->name, *m);
          } else {
  ...
  //  synthesize a getter from is_...? predicates (e.g. is_empty? -> empty
  //  getter)
  for (gsi::ClassBase::method_iterator m = cls_decl->begin_methods();
       m != cls_decl->end_methods(); ++m) {
    ...
    if (!(*m)->is_callback() && !(*m)->is_signal()) {
      ...
      for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
           syn != (*m)->end_synonyms(); ++syn) {
        if (no_args && !syn->is_getter && !syn->is_setter &&
            syn->is_predicate && std::string(syn->name, 0, 3) == "is_") {
...
size_t MethodTable::bottom_mid() const { return m_method_offset; }

size_t MethodTable::top_mid() const { return m_method_offset + m_table.size(); }

size_t MethodTable::bottom_property_mid() const { return m_property_offset; }

size_t MethodTable::top_property_mid() const {
...
MethodTable::find_property(bool st, const std::string &name) const {
  std::map<std::pair<bool, std::string>, size_t>::const_iterator t =
      m_property_name_map.find(std::make_pair(st, name));
...
static bool is_reserved_word(const std::string &name) {
  return (name == "and" || name == "del" || name == "from" || name == "not" ||
          name == "while" || name == "as" || name == "elif" ||
          name == "global" || name == "or" || name == "with" ||
          name == "assert" || name == "else" || name == "if" ||
          name == "pass" || name == "yield" || name == "break" ||
          name == "except" || name == "import" || name == "print" ||
          name == "class" || name == "exec" || name == "in" ||
          name == "raise" || name == "continue" || name == "finally" ||
          name == "is" || name == "return" || name == "def" || name == "for" ||
          name == "lambda" || name == "try" || name == "None");
}
...
/**
 *  @brief Returns true, if the method with the given name shall fallback to
 * NotImplemented
 */
...
void MethodTable::add_method(const std::string &name,
                             const gsi::MethodBase *mb) {
  if (is_reserved_word(name)) {

    //  drop non-standard names
    if (tl::verbosity() >= 20) {
      tl::warn << tl::to_string(tr("Class ")) << mp_cls_decl->name() << ": "
               << tl::to_string(
                      tr("no Python mapping for method (reserved word) "))
               << name;
    }
    ...
    mp_module->add_python_doc(
        mb, tl::sprintf(tl::to_string(tr(
                            "This attribute is available as '%s' in Python")),
                        new_name));

  } else if (name == "new" && mb->ret_type().type() == gsi::T_object &&
             mb->ret_type().pass_obj()) {
    ...
    mp_module->add_python_doc(
        mb, tl::to_string(
                tr("This method is the default initializer of the object")));
    ...
    for (gsi::ClassBase::method_iterator m = mp_cls_decl->begin_methods();
         m != mp_cls_decl->end_methods() && alias_inspect; ++m) {
      if (!(*m)->is_callback() && !(*m)->is_signal()) {
        for (gsi::MethodBase::synonym_iterator syn = (*m)->begin_synonyms();
             syn != (*m)->end_synonyms() && alias_inspect; ++syn) {
    ...
    if (alias_inspect) {
      ...
      mp_module->add_python_doc(
          mb, tl::to_string(tr("This method is also available as 'str(object)' "
                               "and 'repr(object)'")));
    } else {
      mp_module->add_python_doc(
          mb,
          tl::to_string(tr("This method is also available as 'str(object)'")));
    }
    ...
    mp_module->add_python_doc(
        mb,
        tl::to_string(tr("This method is also available as 'hash(object)'")));
    ...
    mp_module->add_python_doc(
        mb,
        tl::to_string(tr("This method is also available as 'repr(object)'")));

  } else if (name == "size" && mb->compatible_with_num_args(0)) {

    //  The size method is also routed via the sequence methods protocol if
    //  there is a [] function
    ...
    mp_module->add_python_doc(
        mb,
        tl::to_string(tr("This method is also available as 'len(object)'")));

  } else if (name == "each" && mb->compatible_with_num_args(0) &&
             mb->ret_type().is_iter()) {
    ...
    if (py_name.empty()) {
      ...
      if (tl::verbosity() >= 20) {
        tl::warn << tl::to_string(tr("Class ")) << mp_cls_decl->name() << ": "
                 << tl::to_string(tr("no Python mapping for method ")) << name;
      }

      add_method_basic(name, mb, false);
      mp_module->add_python_doc(
          mb, tl::to_string(tr("This method is not available for Python")));
      ...
      if (name == "*") {
        ...
        mp_module->add_python_doc(
            mb, tl::to_string(tr("This method also implements '__rmul__'")));
      }
    }
  }
}

void MethodTable::add_setter(const std::string &name,
                             const gsi::MethodBase *setter) {
  if (is_reserved_word(name)) {
    ...
    mp_module->add_python_doc(
        setter, tl::sprintf(tl::to_string(tr(
                                "This member is available as '%s' in Python")),
                            new_name));
...
void MethodTable::add_setter_basic(const std::string &name,
                                   const gsi::MethodBase *setter) {
  ...
  if (n == m_property_name_map.end()) {
    ...
    m_property_name_map.insert(
        std::make_pair(std::make_pair(st, name), m_property_table.size()));
    m_property_table.push_back(std::make_pair(
        MethodTableEntry(name, st, false), MethodTableEntry(name, st, false)));
...
void MethodTable::add_getter(const std::string &name,
                             const gsi::MethodBase *getter) {
  if (is_reserved_word(name)) {
    ...
    mp_module->add_python_doc(
        getter, tl::sprintf(tl::to_string(tr(
                                "This member is available as '%s' in Python")),
                            new_name));
...
void MethodTable::add_getter_basic(const std::string &name,
                                   const gsi::MethodBase *getter) {
...
void MethodTable::alias(size_t mid, const std::string &new_name) {
  ...
  m_name_map.insert(std::make_pair(std::make_pair(st, new_name),
                                   m_table.size() - 1 - m_method_offset));
}
...
const std::string &MethodTable::name(size_t mid) const {
  return m_table[mid - m_method_offset].name();
}

const std::string &MethodTable::property_name(size_t mid) const {
  return m_property_table[mid - m_property_offset].first.name();
}

MethodTableEntry::method_iterator MethodTable::begin_setters(size_t mid) const {
  return m_property_table[mid - m_property_offset].first.begin();
}

MethodTableEntry::method_iterator MethodTable::end_setters(size_t mid) const {
  return m_property_table[mid - m_property_offset].first.end();
}

MethodTableEntry::method_iterator MethodTable::begin_getters(size_t mid) const {
  return m_property_table[mid - m_property_offset].second.begin();
}

MethodTableEntry::method_iterator MethodTable::end_getters(size_t mid) const {
  return m_property_table[mid - m_property_offset].second.end();
}

MethodTableEntry::method_iterator MethodTable::begin(size_t mid) const {
  return m_table[mid - m_method_offset].begin();
}

MethodTableEntry::method_iterator MethodTable::end(size_t mid) const {
...
void MethodTable::finish() {
  for (std::vector<MethodTableEntry>::iterator m = m_table.begin();
    ...
    if (m->is_enabled()) {
      ...
      if (m_property_name_map.find(std::make_pair(m->is_static(), m->name())) !=
          m_property_name_map.end()) {
        m->set_enabled(false);
      }
    }
  }
  for (std::vector<std::pair<MethodTableEntry, MethodTableEntry>>::iterator m =
           m_property_table.begin();
       m != m_property_table.end(); ++m) {
    m->first.finish();
    m->second.finish();
  }
}

void MethodTable::add_method_basic(const std::string &name,
                                   const gsi::MethodBase *mb, bool enabled,
                                   bool init, bool fallback_not_implemented) {
...
MethodTable::method_table_by_class(const gsi::ClassBase *cls_decl) {
  PythonClassClientData *cd = dynamic_cast<PythonClassClientData *>(
      cls_decl->data(gsi::ClientIndex::Python));
...
PythonClassClientData::PythonClassClientData(const gsi::ClassBase *_cls,
                                             PyTypeObject *_py_type,
                                             PyTypeObject *_py_type_static,
                                             PythonModule *module)
    : py_type_object((PyObject *)_py_type),
      py_type_object_static((PyObject *)_py_type_static),
      method_table(_cls, module) {
  //  .. nothing yet ..
}

PythonClassClientData::~PythonClassClientData() {
  //  This destructor is called from the exit code. Python may have shut down
  //  already. We must not try to release the objects in that case and simply
  //  don't care about them any longer.
  py_type_object.release();
  py_type_object_static.release();
}

PyTypeObject *PythonClassClientData::py_type(const gsi::ClassBase &cls_decl,
                                             bool as_static) {
  PythonClassClientData *cd = dynamic_cast<PythonClassClientData *>(
      cls_decl.data(gsi::ClientIndex::Python));
  return (PyTypeObject *)(cd ? (as_static ? cd->py_type_object_static.get()
                                          : cd->py_type_object.get())
                             : 0);
}

void PythonClassClientData::initialize(const gsi::ClassBase &cls_decl,
                                       PyTypeObject *py_type, bool as_static,
                                       PythonModule *module) {
  PythonClassClientData *cd = dynamic_cast<PythonClassClientData *>(
      cls_decl.data(gsi::ClientIndex::Python));
  if (cd) {
    ...
    cls_decl.set_data(
        gsi::ClientIndex::Python,
        new PythonClassClientData(&cls_decl, as_static ? NULL : py_type,
                                  as_static ? py_type : NULL, module));
  }
}
