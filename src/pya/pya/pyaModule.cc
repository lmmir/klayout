...
static void set_type_attr(PyTypeObject *type, const std::string &name,
                          PythonRef &attr) {
  tl_assert(attr.get() != NULL);
  if (type->tp_dict != NULL &&
      PyDict_GetItemString((PyObject *)type, name.c_str()) != NULL) {
    tl::warn << "Ambiguous attribute name " << name << " in class "
             << type->tp_name;
  } else {
...
PythonModule::PythonModule() : mp_mod_def(0) {
  //  .. nothing yet ..
}
...
PyObject *PythonModule::module() { return mp_module.get(); }

PyObject *PythonModule::take_module() { return mp_module.release(); }
...
void PythonModule::init(const char *mod_name, const char *description) {
  ...
  module = Py_InitModule3(m_mod_name.c_str(), module_methods,
                          m_mod_description.c_str());

#else

  struct PyModuleDef mod_def = {
      PyModuleDef_HEAD_INIT, m_mod_name.c_str(),
...
void PythonModule::add_python_doc(const gsi::MethodBase *m,
                                  const std::string &doc) {
...
std::string PythonModule::python_doc(const gsi::MethodBase *method) {
  std::map<const gsi::MethodBase *, std::string>::const_iterator d =
      m_python_doc.find(method);
...
public:
  ...
  PyTypeObject *make_class(const gsi::ClassBase *cls, bool as_static) {
    ...
    if (tl::verbosity() >= 40) {
      tl::info << tl::sprintf(tl::to_string(tr("Creating class %s.%s")),
                              PyModule_GetName(mp_module->module()),
                              cls->name());
    }

    //  NOTE: with as_static = true, this method produces a mixin. This is a
    //  class entirely consisting of static constants and child classes only. It
    //  can be mixed into an existing class for emulation additional base
    //  classes. Everything else, like properties and methods will not work as
    //  the method enumeration scheme is not capable of handling more than a
    //  single base class.
    ...
    PyDict_SetItemString(dict.get(), "__module__",
                         PythonRef(c2python(mp_module->mod_name())).get());
    PyDict_SetItemString(dict.get(), "__doc__",
                         PythonRef(c2python(cls->doc())).get());
    PyDict_SetItemString(dict.get(), "__gsi_id__",
                         PythonRef(c2python(mp_module->next_class_id())).get());
    ...
    PyTypeObject *type = (PyTypeObject *)PyObject_Call((PyObject *)&PyType_Type,
                                                       args.get(), NULL);
    if (type == NULL) {
      try {
        check_error();
    ...
    if (!cls->parent()) {
      ...
      PyModule_AddObject(mp_module->module(), cls->name().c_str(),
                         (PyObject *)type);
    }
    ...
    for (size_t mid = mt->bottom_property_mid(); mid < mt->top_property_mid();
         ++mid) {
      ...
      if (!as_static || is_static) {
        ...
        while ((icls = icls->base()) != 0 &&
               (begin_setters == end_setters || begin_getters == end_getters)) {
          ...
          if (t.first) {
            if (begin_setters == end_setters &&
                mt_base->begin_setters(t.second) !=
                    mt_base->end_setters(t.second)) {
              setter_mid = int(t.second);
              begin_setters = mt_base->begin_setters(t.second);
              end_setters = mt_base->end_setters(t.second);
            }
            if (begin_getters == end_getters &&
                mt_base->begin_getters(t.second) !=
                    mt_base->end_getters(t.second)) {
        ...
        //  add getter and setter documentation, create specific Python
        //  documentation

        for (MethodTableEntry::method_iterator m = begin_getters;
             m != end_getters; ++m) {
          if (!doc.empty()) {
            doc += "\n\n";
          }
          ...
          mp_module->add_python_doc(
              *m, tl::sprintf(
                      tl::to_string(tr("The object exposes a readable "
                                       "attribute '%s'. This is the getter")),
                      name));
        }

        for (MethodTableEntry::method_iterator m = begin_setters;
             m != end_setters; ++m) {
          if (!doc.empty()) {
            doc += "\n\n";
          }
          ...
          mp_module->add_python_doc(
              *m, tl::sprintf(
                      tl::to_string(tr("The object exposes a writable "
                                       "attribute '%s'. This is the setter")),
                      name));
        }
        ...
        if (!is_static) {
          ...
          getset->get =
              begin_getters != end_getters ? &property_getter_func : NULL;
          getset->set =
              begin_setters != end_setters ? &property_setter_func : NULL;
          ...
          PYAStaticAttributeDescriptorObject *desc =
              PYAStaticAttributeDescriptorObject::create(
                  mp_module->make_string(name));

          desc->type = type;
          desc->getter = begin_getters != end_getters
                             ? get_property_getter_adaptor(getter_mid)
                             : NULL;
          desc->setter = begin_setters != end_setters
                             ? get_property_setter_adaptor(setter_mid)
                             : NULL;
          attr = PythonRef(desc);
        }
    ...
    if (!as_static) {
          ...
          if (mt->is_static(mid)) {
            ...
            mp_module->add_python_doc(
                *cls, mt, int(mid),
                tl::sprintf(
                    tl::to_string(
                        tr("This class method is available as '%s' in Python")),
                    name));
          } else {
            name = "_inst_" + name;
            mp_module->add_python_doc(
                *cls, mt, int(mid),
                tl::sprintf(
                    tl::to_string(tr(
                        "This instance method is available as '%s' in Python")),
                    name));
          }
          ...
          if (t.first) {
            ...
            mp_module->add_python_doc(
                *cls, mt, int(mid),
                tl::sprintf(
                    tl::to_string(tr(
                        "This method is available as '%s' in Python to "
                        "distiguish it from the property with the same name")),
                    name));
          }
        }
      ...
      //  Unlike Ruby, Python does not automatically implement != from == for
      //  example. We assume that "==" and "<" are the minimum requirements for
      //  full comparison and "==" is the minimum requirement for equality.
      //  Hence:
      //    * If "==" is given, but no "!=", synthesize
      //        "a != b" by "!a == b"
      //    * If "==" and "<" are given, synthesize if required
      //        "a <= b" by "a < b || a == b"
      //        "a > b" by "!(a < b || a == b)"  (could be b < a, but this
      //        avoids having to switch arguments) "a >= b" by "!a < b"
      ...
      disambiguated_names.erase(
          std::unique(disambiguated_names.begin(), disambiguated_names.end()),
          disambiguated_names.end());

      for (std::vector<std::string>::const_iterator a =
               disambiguated_names.begin();
           a != disambiguated_names.end(); ++a) {
        ...
        PyObject *attr_inst =
            PyObject_GetAttrString((PyObject *)type, ("_inst_" + *a).c_str());
        PyObject *attr_class =
            PyObject_GetAttrString((PyObject *)type, ("_class_" + *a).c_str());
        if (attr_inst == NULL || attr_class == NULL) {
          ...
          if (tl::verbosity() >= 20) {
            tl::warn
                << "Unable to install a static/non-static disambiguator for "
                << *a << " in class " << cls->name();
          }
          ...
          PyObject *desc =
              PYAAmbiguousMethodDispatcher::create(attr_inst, attr_class);
          PythonRef name(c2python(*a));
          //  Note: we use GenericSetAttr since that one allows us setting
          //  attributes on built-in types
...
void PythonModule::make_classes(const char *mod_name) {
  ...
  //  Establish __doc__
  PyObject_SetAttrString(module, "__doc__",
                         PythonRef(c2python(m_mod_description)).get());
  PyList_Append(all_list.get(), PythonRef(c2python("__doc__")).get());

  //  Establish __version__
  PyObject_SetAttrString(module, "__version__",
                         PythonRef(c2python(prg_version)).get());
  ...
  std::list<const gsi::ClassBase *> sorted_classes =
      gsi::ClassBase::classes_in_definition_order(mod_name);

  PythonClassGenerator gen(this, all_list.get());

  if (mod_name) {
    for (std::list<const gsi::ClassBase *>::const_iterator c =
             sorted_classes.begin();
         c != sorted_classes.end(); ++c) {
      if ((*c)->module() != mod_name) {
        //  don't handle classes outside this module, but require them to be
        //  present
        if (!PythonClassClientData::py_type(**c, false)) {
          throw tl::Exception(
              tl::sprintf("class %s.%s required from outside the module %s, "
                          "but that module is not loaded",
                          (*c)->module(), (*c)->name(), mod_name));
        }
      }
    }
  }
