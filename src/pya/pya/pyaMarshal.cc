 ...
 */
class PythonBasedStringAdaptor : public gsi::StringAdaptor {
public:
  ...
  virtual const char *c_str() const { return m_stdstr.c_str(); }

  virtual size_t size() const { return m_stdstr.size(); }
 ...
 */
class PythonBasedByteArrayAdaptor : public gsi::ByteArrayAdaptor {
public:
  ...
  virtual const char *c_str() const { return &m_bytearray.front(); }

  virtual size_t size() const { return m_bytearray.size(); }
 ...
 */
class PythonBasedVariantAdaptor : public gsi::VariantAdaptor {
public:
 ...
 */
class PythonBasedVectorAdaptorIterator : public gsi::VectorAdaptorIterator {
public:
  PythonBasedVectorAdaptorIterator(const PythonPtr &array, size_t len,
 ...
 */
class PythonBasedVectorAdaptor : public gsi::VectorAdaptor {
public:
 ...
 */
class PythonBasedMapAdaptorIterator : public gsi::MapAdaptorIterator {
public:
  PythonBasedMapAdaptorIterator(const PythonPtr &hash,
                                const gsi::ArgType *ainner,
 ...
 */
class PythonBasedMapAdaptor : public gsi::MapAdaptor {
public:
  PythonBasedMapAdaptor(const PythonPtr &hash, const gsi::ArgType *ainner,
                        const gsi::ArgType *ainner_k);
...
template <class R> struct writer {
  void operator()(gsi::SerialArgs *aa, PyObject *arg, const gsi::ArgType &atype,
                  tl::Heap *heap) {
        ...
        if (!vc && atype.is_ref()) {
          throw tl::Exception(tl::to_string(
              tr("Arguments or return values of reference or direct type "
                 "cannot be passed None or an empty boxed value object")));
        }
        aa->write<void *>(vc);
...
template <> struct writer<gsi::VariantType> {
  void operator()(gsi::SerialArgs *aa, PyObject *arg, const gsi::ArgType &,
                  tl::Heap *) {
...
template <> struct writer<gsi::VectorType> {
  void operator()(gsi::SerialArgs *aa, PyObject *arg, const gsi::ArgType &atype,
    ...
    if (arg == Py_None || arg == NULL) {
      ...
      aa->write<void *>(
          (void *)new PythonBasedVectorAdaptor(arg, atype.inner()));
    }
  }
};
...
template <> struct writer<gsi::MapType> {
  void operator()(gsi::SerialArgs *aa, PyObject *arg, const gsi::ArgType &atype,
    ...
    if (arg == Py_None || arg == NULL) {
      ...
      aa->write<void *>((void *)new PythonBasedMapAdaptor(arg, atype.inner(),
                                                          atype.inner_k()));
    }
  }
};
...
template <> struct writer<gsi::ObjectType> {
  void operator()(gsi::SerialArgs *aa, PyObject *arg, const gsi::ArgType &atype,
    ...
    if (arg == Py_None || arg == NULL) {
      ...
      if (!(atype.is_ptr() || atype.is_cptr())) {
        throw tl::Exception(tl::to_string(
            tr("Arguments of reference or direct type cannot be passed null")));
      } else {
        aa->write<void *>((void *)0);
        return;
      }
    ...
    if (atype.is_ptr() || atype.is_cptr() || atype.is_ref() ||
      ...
      if (!cls_decl) {
        throw tl::TypeError(
            tl::sprintf(tl::to_string(tr("Unexpected object type (expected "
                        ...
                        atype.cls()->name(), Py_TYPE(arg)->tp_name));
      }
      ...
      if (cls_decl->is_derived_from(atype.cls())) {
        ...
        //  objects is not transferred. Hence we have to keep them on the heap.
...
template <> struct writer<gsi::VoidType> {
  void operator()(gsi::SerialArgs *, PyObject *, const gsi::ArgType &,
                  tl::Heap *) {
    //  ignore void
  }
};

void push_arg(const gsi::ArgType &atype, gsi::SerialArgs &aserial,
              PyObject *arg, tl::Heap &heap) {
...
template <class R> struct reader {
  void operator()(gsi::SerialArgs *rr, PythonRef *ret, PYAObjectBase * /*self*/,
...
static PyObject *object_from_variant(tl::Variant &var, PYAObjectBase *self,
  ...
  if (var.is_user()) {
    ...
    return object_to_python(obj, self, cls, pass_obj, is_const, prefer_copy,
                            can_destroy);
  ...
  void operator()(gsi::SerialArgs *rr, PythonRef *ret, PYAObjectBase *self,
    ...
    if (!a.get()) {
      *ret = PythonRef(Py_None, false /*borrowed*/);
      ...
      PythonBasedVariantAdaptor *pa =
          dynamic_cast<PythonBasedVariantAdaptor *>(a.get());
      if (aa) {
...
template <> struct reader<gsi::VoidType> {
  void operator()(gsi::SerialArgs *, PythonRef *, PYAObjectBase *,
                  const gsi::ArgType &, tl::Heap *) {
    //  .. nothing: void is not serialized
  }
};

PythonRef pop_arg(const gsi::ArgType &atype, gsi::SerialArgs &aserial,
                  PYAObjectBase *self, tl::Heap &heap) {
  PythonRef ret;
...
PythonBasedVectorAdaptorIterator::PythonBasedVectorAdaptorIterator(
    const PythonPtr &array, size_t len, const gsi::ArgType *ainner)
    : m_array(array), m_i(0), m_len(len), mp_ainner(ainner) {
  //  .. nothing yet ..
}

void PythonBasedVectorAdaptorIterator::get(gsi::SerialArgs &w,
                                           tl::Heap &heap) const {
...
bool PythonBasedVectorAdaptorIterator::at_end() const { return m_i == m_len; }

void PythonBasedVectorAdaptorIterator::inc() { ++m_i; }

// ---------------------------------------------------------------------
//  PythonBasedVectorAdaptor implementation

PythonBasedVectorAdaptor::PythonBasedVectorAdaptor(const PythonPtr &array,
                                                   const gsi::ArgType *ainner)
...
void PythonBasedVectorAdaptor::push(gsi::SerialArgs &r, tl::Heap &heap) {
  if (PyList_Check(m_array.get())) {
    PythonRef member;
    gsi::do_on_type<reader>()(mp_ainner->type(), &r, &member,
                              (PYAObjectBase *)0, *mp_ainner, &heap);
    PyList_Append(m_array.get(), member.get());
  } else if (PyTuple_Check(m_array.get())) {
    throw tl::Exception(tl::to_string(
        tr("Tuples cannot be modified and cannot be used as out parameters")));
  }
}
...
PythonBasedMapAdaptorIterator::PythonBasedMapAdaptorIterator(
    const PythonPtr &hash, const gsi::ArgType *ainner,
    const gsi::ArgType *ainner_k)
...
void PythonBasedMapAdaptorIterator::get(gsi::SerialArgs &w,
                                        tl::Heap &heap) const {
  gsi::do_on_type<writer>()(mp_ainner_k->type(), &w, m_key, *mp_ainner_k,
                            &heap);
  gsi::do_on_type<writer>()(mp_ainner->type(), &w, m_value, *mp_ainner, &heap);
}

bool PythonBasedMapAdaptorIterator::at_end() const { return !m_has_items; }
...
PythonBasedMapAdaptor::PythonBasedMapAdaptor(const PythonPtr &hash,
                                             const gsi::ArgType *ainner,
                                             const gsi::ArgType *ainner_k)
...
void PythonBasedMapAdaptor::insert(gsi::SerialArgs &r, tl::Heap &heap) {
  PythonRef k, v;
  gsi::do_on_type<reader>()(mp_ainner_k->type(), &r, &k, (PYAObjectBase *)0,
                            *mp_ainner_k, &heap);
  gsi::do_on_type<reader>()(mp_ainner->type(), &r, &v, (PYAObjectBase *)0,
                            *mp_ainner, &heap);
  PyDict_SetItem(m_hash.get(), k.get(), v.get());
}

void PythonBasedMapAdaptor::clear() { PyDict_Clear(m_hash.get()); }

size_t PythonBasedMapAdaptor::size() const { return PyDict_Size(m_hash.get()); }
...
template <class R> struct test_arg_func {
  void operator()(bool *ret, PyObject *arg, const gsi::ArgType &atype,
                  bool loose) {
      ...
      *ret = false;
      ...
      if (atype.is_ptr() || atype.is_ref()) {
        ...
        const gsi::ClassBase *cls_decl =
            PythonModule::cls_for_type(Py_TYPE(arg));
        if (cls_decl) {
...
template <> struct test_arg_func<gsi::VariantType> {
...
template <> struct test_arg_func<gsi::StringType> {
...
template <> struct test_arg_func<gsi::ObjectType> {
  void operator()(bool *ret, PyObject *arg, const gsi::ArgType &atype,
    ...
    if (!(cls_decl == atype.cls() ||
          (loose && (cls_decl->is_derived_from(atype.cls()) ||
                     cls_decl->can_convert_to(atype.cls()))))) {
      *ret = false;
      return;
    }
    if ((atype.is_ref() || atype.is_ptr()) &&
        ((PYAObjectBase *)arg)->const_ref()) {
      *ret = false;
      return;
    }
...
bool test_arg(const gsi::ArgType &atype, PyObject *arg, bool loose) {
  bool ret = false;
