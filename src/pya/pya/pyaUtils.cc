...
void check_error() {
  ...
  if (py_exc_type != NULL) {
    ...
    //  fetch traceback
    //  TODO: really decref the stack trace? how about the other objects in the
    //  stack trace?
    ...
    if (exc_traceback) {
      ...
      for (PyTracebackObject *t = traceback; t; t = t->tb_next) {
        ...
        backtrace.push_back(tl::BacktraceElement(
            python2c<std::string>(PyFrame_GetCode(t->tb_frame)->co_filename),
            t->tb_lineno));
#else
        backtrace.push_back(tl::BacktraceElement(
            python2c<std::string>(t->tb_frame->f_code->co_filename),
            t->tb_lineno));
#endif
      }
    ...
    if (PyErr_GivenExceptionMatches(exc_type.get(), PyExc_SyntaxError) &&
        PyTuple_Check(exc_value.get()) && PyTuple_Size(exc_value.get()) >= 2) {
      ...
      try {
        ...
        if (exc_value && PyTuple_Check(exc_value.get()) &&
            PyTuple_Size(exc_value.get()) >= 2) {

          std::string msg_arg =
              python2c<std::string>(PyTuple_GetItem(exc_value.get(), 0));
      ...
      throw PythonError(msg.empty() ? exc_cls.c_str() : msg.c_str(),
                        sourcefile ? sourcefile : "unknown", line,
                        exc_cls.c_str(), backtrace);
