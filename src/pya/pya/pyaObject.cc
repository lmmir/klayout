 ...
 */
class Callee : public gsi::Callee {
public:
  /**
   ...
   *  This method returns a callback ID which can be used to register the
   * callback at an GSI object.
   */
...
Callee::Callee(PYAObjectBase *obj) : mp_obj(obj) {
  //  .. nothing yet ..
}
...
void Callee::clear_callbacks() { m_cbfuncs.clear(); }
...
void Callee::call(int id, gsi::SerialArgs &args, gsi::SerialArgs &ret) const {
  ...
  try {
    ...
    if (callable) {
      ...
      PythonRef argv(
          PyTuple_New(arg4self + std::distance(meth->begin_arguments(),
                                               meth->end_arguments())));
      ...
      for (gsi::MethodBase::argument_iterator a = meth->begin_arguments();
           args && a != meth->end_arguments(); ++a) {
        PyTuple_SetItem(argv.get(), arg4self + (a - meth->begin_arguments()),
                        pop_arg(*a, args, 0, heap).release());
      }
      ...
      tl::Heap heap;
      push_arg(meth->ret_type(), ret,
               meth->ret_type().pass_obj() ? result.release() : result.get(),
               heap);
    ...
    err_with_context.set_context(mp_obj->cls_decl()->name() + "." +
                                 meth->names());
    throw err_with_context;
  } catch (tl::ExitException &) {
    throw;
  } catch (tl::Exception &ex) {
    throw tl::Exception(tl::to_string(tr("Error calling method")) + " '" +
                        mp_obj->cls_decl()->name() + "." + meth->names() +
                        "': " + ex.msg());
  } catch (...) {
    throw;
  }
...
PYAObjectBase::PYAObjectBase(const gsi::ClassBase *_cls_decl,
    ...
    : mp_py_object(py_object),
      ...
      mp_callee(new pya::Callee(this)), m_cls_decl(_cls_decl), m_obj(0),
      m_owned(false), m_const_ref(false), m_destroyed(false),
      m_can_destroy(false) {
  //  .. nothing yet ..
}
...
PYAObjectBase::~PYAObjectBase() {
  try {
    ...
    //  Destroy the object if we are owner. We don't destroy the object if it
    //  was locked (either because we are not owner or from C++ side using
    //  keep())
...
void PYAObjectBase::object_destroyed() {
  //  This may happen outside the Python interpreter, so we safeguard ourselves
  //  against this. In this case, we may encounter a memory leak, but there is
  //  little we can do against this and it will happen in the application
  //  teardown anyway.
...
void PYAObjectBase::release() {
  ...
  if (!m_owned) {
    m_owned = true;
    //  NOTE: this may delete "this"! TODO: this should not happen. Can we
    //  assert that somehow?
    Py_DECREF(py_object());
  }
}
...
void PYAObjectBase::detach() {
  if (m_obj) {
    ...
    if (!m_destroyed && cls && cls->is_managed()) {
      ...
      if (gsi_object) {
        gsi_object->status_changed_event().remove(
            mp_listener, &StatusChangedListener::object_status_changed);
      }
    }

    //  NOTE: m_owned = false might mean the C++ object is already destroyed. We
    //  must not modify in this case and without is_managed() there is no way of
    //  knowing the state.
    if (m_owned) {
      detach_callbacks();
    }
...
void PYAObjectBase::set(void *obj, bool owned, bool const_ref,
                        bool can_destroy) {
  const gsi::ClassBase *cls = cls_decl();
  if (!cls) {
    return;
  ...
  if (cls->is_managed()) {
    ...
    gsi_object->status_changed_event().add(
        mp_listener, &StatusChangedListener::object_status_changed);
  }
...
pya::SignalHandler *PYAObjectBase::signal_handler(const gsi::MethodBase *meth) {
  std::map<const gsi::MethodBase *, pya::SignalHandler>::iterator st =
      m_signal_table.find(meth);
  if (st == m_signal_table.end()) {
    st =
        m_signal_table.insert(std::make_pair(meth, pya::SignalHandler())).first;
...
//  TODO: caching appears to create some leaks ...
  ...
  if (cb == s_callbacks_cache.end()) {
    ...
    cb = s_callbacks_cache
             .insert(std::make_pair(type_ref, callback_methods_type()))
             .first;

    const gsi::ClassBase *cls = cls_decl();

    //  TODO: cache this .. this is taking too much time if done on every
    //  instance we got a new object - hence we have to attach event handlers.
    //  We don't need to install virtual function callbacks because in that
    //  case, no overload is possible (the object has been created on C++ side).
    while (cls) {

      for (gsi::ClassBase::method_iterator m = cls->begin_callbacks();
           m != cls->end_callbacks(); ++m) {

        if (m_owned) {

          //  NOTE: only Python-implemented classes can reimplement methods.
          //  Since we take the attribute from the class object, only Python
          //  instances can overwrite the methods and owned indicates that.
          //  owned == true indicates that.

          //  NOTE: a callback may not have aliases nor overloads
          const char *nstr = (*m)->primary_name().c_str();

          //  NOTE: we just take attributes from the class object. That implies
          //  that it's not possible to reimplement a method through instance
          //  attributes (rare case, I hope). In addition, if we'd use instance
          //  attributes we create circular references (self/callback to method,
          //  method to self).
          //  TODO: That may happen too often, i.e. if the Python class does not
          //  reimplement the virtual method, but the C++ class defines a method
          //  hook that the reimplementation can call. We don't want to produce
          //  a lot of overhead for the Qt classes here.
          PythonRef py_attr =
              PyObject_GetAttrString((PyObject *)Py_TYPE(py_object()), nstr);
          if (!py_attr) {
            ...
            //  Only if a Python-level class defines that method we can link the
            //  virtual method call to the Python method. We should not create
            //  callbacks which we refer to C class implementations because that
            //  may create issues with callbacks during destruction (i.e.
  ...
  for (callback_methods_type::const_iterator m = cb->second.begin();
       m != cb->second.end(); ++m) {
    ...
    (*m)->set_callback(
        m_obj, gsi::Callback(id, mp_callee, (*m)->argsize(), (*m)->retsize()));
  }
  ...
  //  We don't need to install virtual function callbacks because in that case,
  //  no overload is possible (the object has been created on C++ side).
  while (cls) {
    ...
    for (gsi::ClassBase::method_iterator m = cls->begin_methods();
      ...
      if ((*m)->is_callback() && m_owned) {
        ...
        //  NOTE: only Python-implemented classes can reimplement methods. Since
        //  we take the attribute from the class object, only Python instances
        //  can overwrite the methods and owned indicates that. owned == true
        //  indicates that.
        ...
        //  TODO: That may happen too often, i.e. if the Python class does not
        //  reimplement the virtual method, but the C++ class defines a method
        //  hook that the reimplementation can call. We don't want to produce a
        //  lot of overhead for the Qt classes here.
        PythonRef py_attr =
            PyObject_GetAttrString((PyObject *)Py_TYPE(py_object()), nstr);
        if (!py_attr) {
          ...
          if (!PyCFunction_Check(py_attr.get())) {
            ...
            (*m)->set_callback(
                m_obj,
                gsi::Callback(id, mp_callee, (*m)->argsize(), (*m)->retsize()));
          }
        }
      }
    }
...
void PYAObjectBase::clear_callbacks_cache() { s_callbacks_cache.clear(); }
...
void PYAObjectBase::destroy() {
  ...
  if (!(m_owned || m_can_destroy) && m_obj) {
    throw tl::Exception(
        tl::to_string(tr("Object cannot be destroyed explicitly")));
  }

  //  first create the object if it was not created yet and check if it has not
  //  been destroyed already (the former is to ensure that the object is created
  //  at least)
  if (!m_obj) {
    if (m_destroyed) {
      throw tl::Exception(
          tl::to_string(tr("Object has been destroyed already")));
    } else {
...
PYAObjectBase *PYAObjectBase::from_pyobject(PyObject *py_object) {
  if (Py_TYPE(py_object)->tp_init == NULL) {
    throw tl::Exception(tl::to_string(
        tr("Extension classes do not support instance methods or properties")));
  }
