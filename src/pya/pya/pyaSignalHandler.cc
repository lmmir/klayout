...
CallbackFunction::CallbackFunction(PythonRef pym, const gsi::MethodBase *m)
  ...
  if (pym && PyMethod_Check(pym.get()) && PyMethod_Self(pym.get()) != NULL) {
    ...
    m_callable =
        PythonRef(PyMethod_Function(pym.get()), false /* borrowed ref */);
...
const gsi::MethodBase *CallbackFunction::method() const { return mp_method; }
...
PyObject *CallbackFunction::callable_ref() const { return m_callable.get(); }
...
SignalHandler::~SignalHandler() { clear(); }
...
void SignalHandler::call(const gsi::MethodBase *meth, gsi::SerialArgs &args,
  ...
  int args_avail =
      int(std::distance(meth->begin_arguments(), meth->end_arguments()));
  ...
  for (gsi::MethodBase::argument_iterator a = meth->begin_arguments();
    ...
    PyTuple_SetItem(argv.get(), int(a - meth->begin_arguments()),
                    pop_arg(*a, args, 0, heap).release());
  }

  //  NOTE: in case one event handler deletes the object, it's safer to first
  //  collect the handlers and then call them.
  std::vector<PythonRef> callables;
  callables.reserve(m_cbfuncs.size());
  for (std::vector<CallbackFunction>::const_iterator c = m_cbfuncs.begin();
       c != m_cbfuncs.end(); ++c) {
  ...
  for (std::vector<PythonRef>::const_iterator c = callables.begin();
       c != callables.end(); ++c) {
...
void SignalHandler::remove(PyObject *callable) {
  //  To avoid cyclic references, the CallbackFunction holder is employed.
  //  However, the "true" callable no longer is the original one. Hence, we need
  //  to do a strict compare against the effective one.
  CallbackFunction cbref(PythonPtr(callable), 0);
  for (std::vector<CallbackFunction>::iterator c = m_cbfuncs.begin();
       c != m_cbfuncs.end(); ++c) {
    if (*c == cbref) {
      m_cbfuncs.erase(c);
      break;
...
void SignalHandler::clear() { m_cbfuncs.clear(); }
