...
template <> long python2c_func<long>::operator()(PyObject *rval) {
  ...
  if (PyInt_Check(rval)) {
    ...
    throw tl::TypeError(
        tl::to_string(tr("Value cannot be converted to an integer")));
  }
}
...
template <> char python2c_func<char>::operator()(PyObject *rval) {
  ...
  if (PyInt_Check(rval)) {
    ...
    throw tl::TypeError(
        tl::to_string(tr("Value cannot be converted to a character")));
  }
}
...
template <> double python2c_func<double>::operator()(PyObject *rval) {
  ...
  if (PyInt_Check(rval)) {
    ...
    throw tl::TypeError(tl::to_string(
        tr("Value cannot be converted to a floating-point value")));
  }
}
...
template <> std::string python2c_func<std::string>::operator()(PyObject *rval) {
  ...
  if (PyBytes_Check(rval)) {
    ...
    throw tl::TypeError(
        tl::to_string(tr("Value cannot be converted to a string")));
  }
}
...
std::vector<char> python2c_func<std::vector<char>>::operator()(PyObject *rval) {
  ...
  if (PyBytes_Check(rval)) {
    ...
    throw tl::TypeError(
        tl::to_string(tr("Value cannot be converted to a byte array")));
  }
}
...
template <> tl::Variant python2c_func<tl::Variant>::operator()(PyObject *rval) {
  if (rval == NULL || rval == Py_None) {
    ...
    if (cls) {
      ...
      //  employ the tl::Variant binding capabilities of the Expression binding
      //  to derive the variant value.
...
PyObject *object_to_python(void *obj, PYAObjectBase *self,
                           const gsi::ArgType &atype) {
  const gsi::ClassBase *cls = atype.cls()->subclass_decl(obj);

  bool is_direct =
      !(atype.is_ptr() || atype.is_ref() || atype.is_cptr() || atype.is_cref());
...
/**
 *  @brief Correct constness if a reference is const and a non-const reference
 * is required HINT: this is a workaround for the fact that unlike C++, Python
 * does not have const or non-const references. Since a reference is identical
 * with the object it points to, there are only const or non-const objects. We
 * deliver const objects first, but if a non-const version is requested, the
 *  object turns into a non-const one. This may be confusing but provides a
 * certain level of "constness", at least until there is another non-const
...
PyObject *object_to_python(void *obj, PYAObjectBase *self,
                           ...
                           bool is_const, bool prefer_copy, bool can_destroy) {
  if (!obj || !cls) {
    Py_RETURN_NONE;
  }
  ...
  if (self && self->obj() == obj) {
    ...
    StatusChangedListener *client =
        clsact->gsi_object(obj)->find_client<StatusChangedListener>();
    if (client) {
  ...
  if (!pass_obj && prefer_copy && !clsact->adapted_type_info() &&
      !clsact->is_managed() && clsact->can_copy() &&
      clsact->can_default_create()) {
    ...
    if (!type) {
      throw tl::Exception(tl::sprintf(
          tl::to_string(tr("Requested type %s.%s is not bound to a Python "
                           "class (did you load the '%s' module?)")),
          clsact->module(), clsact->name(), clsact->module()));
    }

    //  create a instance and copy the value
    PyObject *new_pyobject = type->tp_alloc(type, 0);
    PYAObjectBase *new_object =
        PYAObjectBase::from_pyobject_unsafe(new_pyobject);
...
PyObject *c2python_func<const tl::Variant &>::operator()(const tl::Variant &c) {
  if (c.is_double()) {
    ...
    for (tl::Variant::const_array_iterator i = c.begin_array();
         i != c.end_array(); ++i) {
    ...
    size_t index = 0;
    for (tl::Variant::const_iterator i = c.begin(); i != c.end();
         ++i, ++index) {
    ...
    if (cls) {
      ...
      return object_to_python(obj, 0, c.user_cls()->gsi_cls(), false, false,
                              true, false);
    } else {
...
template <>
PyObject *c2python_func<const std::vector<char> &>::operator()(
...
template <> PyObject *c2python_func<const char *>::operator()(const char *p) {
  const char *s = p;
  if (!s) {
    s = "(null)";
  }
...
#if PY_MAJOR_VERSION < 3
    return PyByteArray_FromStringAndSize(qba.constData(),
                                         Py_ssize_t(qba.size()));
#else
