...
/**
 *  @brief A helper function for determining whether a Python object is a plain
 * type
 *
...
#endif
  if (PyBool_Check(obj) || PyFloat_Check(obj) || PyLong_Check(obj) ||
      PyUnicode_Check(obj) || PyByteArray_Check(obj)) {
    return true;
  }
 ...
 */
class DictInspector : public gsi::Inspector {
public:
  ...
  tl::Variant keyv(size_t index) const {
    if (m_keys && PyList_Check(m_keys.get()) &&
        Py_ssize_t(index) < PyList_GET_SIZE(m_keys.get())) {
  ...
  tl::Variant value(size_t index) const {
    if (m_values && PyList_Check(m_values.get()) &&
        Py_ssize_t(index) < PyList_Size(m_values.get())) {
  ...
  gsi::Inspector::Visibility visibility(size_t index) const {
    if (m_symbolic && m_keys && m_values && PyList_Check(m_values.get()) &&
        Py_ssize_t(index) < PyList_Size(m_values.get()) &&
        PyList_Check(m_keys.get()) &&
        Py_ssize_t(index) < PyList_Size(m_keys.get())) {
      return visibility_flag(PyList_GET_ITEM(m_values.get(), index),
                             PyList_GET_ITEM(m_keys.get(), index));
    } else {
      return gsi::Inspector::Always;
    }
  }
 ...
 */
class ListInspector : public gsi::Inspector {
public:
  ListInspector(PyObject *list) : m_values(list) {}
 ...
 */
class TupleInspector : public gsi::Inspector {
public:
  TupleInspector(PyObject *tuple) : m_values(tuple) {}
  ...
  std::string type(size_t index) const {
    if (m_values && PyTuple_Check(m_values.get()) &&
        Py_ssize_t(index) < PyTuple_Size(m_values.get())) {
 ...
 */
class ObjectInspector : public gsi::Inspector {
public:
  ObjectInspector(PyObject *obj) : m_obj(obj) {
  ...
  tl::Variant value(size_t index) const {
    if (m_keys && PyList_Check(m_keys.get()) &&
        Py_ssize_t(index) < PyList_Size(m_keys.get())) {
      PythonRef value(
          PyObject_GetAttr(m_obj.get(), PyList_GET_ITEM(m_keys.get(), index)));
      if (!value) {
        check_error();
      }
  ...
  gsi::Inspector::Visibility visibility(size_t index) const {
    if (m_keys && PyList_Check(m_keys.get()) &&
      ...
      if (!value) {
        ...
        return visibility_flag(value.get(),
                               PyList_GET_ITEM(m_keys.get(), index));
      }
    }
    return gsi::Inspector::Always;
...
gsi::Inspector *create_inspector(PyObject *obj, bool symbolic) {
  if (PyDict_Check(obj)) {
